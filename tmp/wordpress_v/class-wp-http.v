...
fn (mut this Class_WP_Http) request(var_url rt.PhpVal, var_args rt.PhpVal) rt.PhpVal {
	...
	mut var_options := { 'timeout': var_parsed_args.array_get('timeout'), 'useragent': var_parsed_args.array_get('user-agent'), 'blocking': var_parsed_args.array_get('blocking'), 'hooks': create_wp_http_requests_hooks(var_url_mutated.clone(), var_parsed_args.clone()) }
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('HEAD'), var_type)))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('GET'), var_type)))))) {
		var_options['data_format'] = rt.new_string('body')
	}
	var_options['verify'] = rt.call_function('apply_filters', [rt.new_string('https_ssl_verify'), var_options.array_get('verify'), var_url_mutated.clone()])
	mut var_proxy := create_wp_http_proxy()
	if rt.is_true(rt.new_bool(rt.is_true(var_proxy.is_enabled()) && rt.is_true(var_proxy.send_through_proxy(var_url_mutated.clone())))) {
	...
	mut var_requests_response := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal, arg_3 rt.PhpVal, arg_4 rt.PhpVal) rt.PhpVal { mut temp := Class_WpOrg_Requests_Requests{}; return temp.request(arg_0, arg_1, arg_2, arg_3, arg_4) }(var_url_mutated.clone(), var_headers.clone(), var_data.clone(), var_type.clone(), var_options.clone())
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_http_response := create_wp_http_requests_response(var_requests_response.clone(), var_parsed_args.array_get('filename'))
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
fn Class_WP_Http.processresponse(var_response rt.PhpVal) rt.PhpVal {
	...
	return rt.create_array([rt.ArrayItem{ key: 'headers', val: var_response_mutated.array_get(0) }, rt.ArrayItem{ key: 'body', val: if !(var_response_mutated.array_get(1)).is_null() { var_response_mutated.array_get(1) } else { rt.new_string('') } }])
}
...
fn Class_WP_Http.processheaders(var_headers rt.PhpVal, url string) rt.PhpVal {
	...
	mut var_response := rt.create_array([rt.ArrayItem{ key: 'code', val: 0 }, rt.ArrayItem{ key: 'message', val: '' }])
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(!rt.is_true(var_headers_mutated.array_get(var_i))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_headers_mutated.array_get(var_i), rt.new_string(':')]))))))) {
	...
	{
		mut iter_1 := rt.cast_array(var_headers_mutated).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_tempheader := item_1.val
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_tempheader.clone(), rt.new_string(':')]))))) {
				...
				mut list_tmp_-20240 := var_stack
				continue
			}
			mut list_tmp_-37408 := rt.call_function('explode', [rt.new_string(':'), var_tempheader.clone(), rt.new_int(2)])
			mut var_key := rt.new_string(var_key.clone().to_string().to_lower())
			mut var_value := rt.new_string(var_value.clone().to_string().trim_space())
			if var_newheaders.array_isset(var_key) {
				if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_newheaders.array_get(var_key).is_array()))))) {
	...
	var_response.array_set('code', rt.new_int((var_response.array_get('code')).to_i64()))
...
fn Class_WP_Http.buildcookieheader(var_r rt.PhpVal)  {
	if !(!rt.is_true(var_r.array_get('cookies'))) {
		{
			mut iter_1 := var_r.array_get('cookies').iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_value := item_1.val
				mut var_name := item_1.key
				if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_value.clone().is_object()))))) {
		...
		{
			mut iter_1 := rt.cast_array(var_r.array_get('cookies')).iterator()
			for {
				item_1 := iter_1.next() or { break }
		...
		var_cookies_header = rt.call_function('substr', [var_cookies_header.clone(), rt.new_int(0), -2])
...
fn Class_WP_Http.chunktransferdecode(var_body rt.PhpVal) rt.PhpVal {
	...
	for true {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_has_chunk)))) || !rt.is_true(var_match.array_get(1)))) {
			return var_body_original.clone()
		}
		mut var_length := rt.call_function('hexdec', [var_match.array_get(1)])
		mut var_chunk_length := rt.new_int(var_match.array_get(0).to_string().len)
...
fn (mut this Class_WP_Http) block_request(var_uri rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('WP_HTTP_BLOCK_EXTERNAL')]))))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.get_constant('WP_HTTP_BLOCK_EXTERNAL'))))))) {
		return false
	}
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('localhost'), var_check.array_get('host'))) || rt.is_true(rt.new_bool(var_home.array_isset(rt.new_string('host')) && rt.is_true(rt.identical(var_home.array_get('host'), var_check.array_get('host'))))))) {
	...
	if rt.is_true(rt.identical(rt.new_null(), var_accessible_hosts)) {
		...
		if rt.is_true(rt.call_function('str_contains', [rt.get_constant('WP_ACCESSIBLE_HOSTS'), rt.new_string('*')])) {
			...
			{
				mut iter_1 := var_accessible_hosts.iterator()
				for {
					item_1 := iter_1.next() or { break }
	...
	if !(!rt.is_true(var_wildcard_regex)) {
		return !(rt.is_true(rt.call_function('preg_match', [var_wildcard_regex.clone(), var_check.array_get('host')])))
	} else {
		return !(rt.is_true(rt.call_function('in_array', [var_check.array_get('host'), var_accessible_hosts.clone(), rt.new_bool(true)])))
	}
	return false
}
...
fn Class_WP_Http.make_absolute_url(var_maybe_relative_path rt.PhpVal, var_url rt.PhpVal) string {
	...
	if !(!rt.is_true(var_relative_url_parts.array_get('scheme'))) {
		return (var_maybe_relative_path).str()
	}
	mut var_absolute_path := rt.new_string((var_url_parts.array_get('scheme')).str() + '://')
	if var_relative_url_parts.array_isset(rt.new_string('host')) {
		var_absolute_path = rt.concat(var_absolute_path, var_relative_url_parts.array_get('host'))
		if var_relative_url_parts.array_isset(rt.new_string('port')) {
			var_absolute_path = rt.concat(var_absolute_path, rt.new_string(':' + (var_relative_url_parts.array_get('port')).str()))
		}
	} else {
		var_absolute_path = rt.concat(var_absolute_path, var_url_parts.array_get('host'))
		if var_url_parts.array_isset(rt.new_string('port')) {
			var_absolute_path = rt.concat(var_absolute_path, rt.new_string(':' + (var_url_parts.array_get('port')).str()))
		}
	}
	mut var_path := if !(!rt.is_true(var_url_parts.array_get('path'))) { var_url_parts.array_get('path') } else { rt.new_string('/') }
	if rt.is_true(rt.new_bool(!(!rt.is_true(var_relative_url_parts.array_get('path'))) && rt.is_true(rt.identical(rt.new_string('/'), var_relative_url_parts.array_get('path').array_get(0))))) {
		var_path = var_relative_url_parts.array_get('path')
	} else if !(!rt.is_true(var_relative_url_parts.array_get('path'))) {
		...
		var_path = rt.concat(var_path, var_relative_url_parts.array_get('path'))
	...
	if !(!rt.is_true(var_relative_url_parts.array_get('query'))) {
		var_path = rt.concat(var_path, rt.new_string('?' + (var_relative_url_parts.array_get('query')).str()))
	}
	if !(!rt.is_true(var_relative_url_parts.array_get('fragment'))) {
		var_path = rt.concat(var_path, rt.new_string('#' + (var_relative_url_parts.array_get('fragment')).str()))
	}
...
fn Class_WP_Http.handle_redirects(var_url rt.PhpVal, var_args rt.PhpVal, var_response rt.PhpVal) bool {
	...
	mut var_args_mutated := var_args
	mut var_response_mutated := var_response
	if rt.is_true(rt.new_bool(!(var_response_mutated.array_get('headers').array_isset(rt.new_string('location'))) || rt.is_true(rt.identical(rt.new_int(0), var_args_mutated.array_get('_redirection'))))) {
		return false
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.greater(var_response_mutated.array_get('response').array_get('code'), rt.new_int(399))) || rt.is_true(rt.less(var_response_mutated.array_get('response').array_get('code'), rt.new_int(300))))) {
		return false
	}
	if rt.is_true(rt.less_equal(rt.post_dec(var_args_mutated.array_get('redirection')), rt.new_int(0))) {
		return (create_wp_error(rt.new_string('http_request_failed'), rt.call_function('__', [rt.new_string('Too many redirects.')]))).to_bool()
	}
	mut var_redirect_location := var_response_mutated.array_get('headers').array_get('location')
	...
	if rt.is_true(rt.identical(rt.new_string('POST'), var_args_mutated.array_get('method'))) {
		if rt.is_true(rt.call_function('in_array', [var_response_mutated.array_get('response').array_get('code'), rt.create_array([rt.ArrayItem{ key: none, val: 302 }, rt.ArrayItem{ key: none, val: 303 }]), rt.new_bool(true)])) {
			var_args_mutated.array_set('method', 'GET')
		}
	}
	if !(!rt.is_true(var_response_mutated.array_get('cookies'))) {
		{
			mut iter_1 := var_response_mutated.array_get('cookies').iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_cookie := item_1.val
...
fn Class_WP_Http.is_ip_address(var_maybe_ip rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('str_contains', [var_maybe_ip.clone(), rt.new_string(':')])) && rt.is_true(rt.call_function('preg_match', [rt.new_string('/^(((?=.*(::))(?!.*\\3.+\\3))\\3?|([\\dA-F]{1,4}(\\3|:\\b|$)|\\2))(?4){5}((?4){2}|(((2[0-4]|1\\d|[1-9])?\\d|25[0-5])\\.?\\b){4})$/i'), rt.new_string(var_maybe_ip.clone().to_string().trim_space())])))) {
		return rt.new_int(6)
	}
...
pub fn run_class_wp_http() rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('ABSPATH')]))))) {
		print((rt.new_string('-1')).str()); return rt.new_int(0)
	}
