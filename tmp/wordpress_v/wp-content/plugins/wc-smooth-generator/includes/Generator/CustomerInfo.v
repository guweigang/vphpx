...
fn Class_WC_SmoothGenerator_Generator_CustomerInfo.get_valid_country_code(mut var_country_code Class_WC_SmoothGenerator_Generator_?string) rt.PhpVal {
	...
	var_country_code_mutated = rt.new_string(if !(!rt.is_true(var_country_code_mutated)) { rt.new_string(var_country_code_mutated.dup().to_string().to_upper()) } else { rt.new_string('') })
	if rt.is_true(rt.new_bool(rt.is_true(var_country_code_mutated) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'country_exists', [var_country_code_mutated.dup()]))))))) {
		var_country_code_mutated = create_wc_smoothgenerator_generator_wp_error(rt.new_string('smoothgenerator_customer_invalid_country'), rt.call_function('sprintf', [rt.new_string('No data for a country with country code "%s"'), rt.call_function('esc_html', [var_country_code_mutated.dup()])]))
		...
		var_country_code_mutated = rt.call_function('array_rand', [var_valid_countries.dup()])
	}
...
fn Class_WC_SmoothGenerator_Generator_CustomerInfo.get_country_locale_info(country_code string) rt.PhpVal {
	...
	return var_all_locale_info.array_get(country_code_mutated)
}
...
fn Class_WC_SmoothGenerator_Generator_CustomerInfo.get_faker(country_code string) rt.PhpVal {
	...
	mut var_default_locale := if !(!rt.is_true(var_locale_info.array_get('default_locale'))) { var_locale_info.array_get('default_locale') } else { rt.new_string('en_US') }
	mut var_faker := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_SmoothGenerator_Generator_Faker_Factory{}; return temp.create(arg_0) }(var_default_locale.dup())
	return var_faker.dup()
}
...
fn Class_WC_SmoothGenerator_Generator_CustomerInfo.get_provider_instance(mut var_faker Class_WC_SmoothGenerator_Generator_Faker_Generator, provider_name string) rt.PhpVal {
	...
	mut var_instance := rt.new_null()
	{
		...
		for {
			...
			if rt.is_true(rt.call_function('str_ends_with', [rt.call_function('get_class', [var_provider.dup()]), rt.new_string(provider_name)])) {
...
fn Class_WC_SmoothGenerator_Generator_CustomerInfo.generate_person(country_code string) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_wp_error', [rt.new_string(country_code_mutated).dup()])) {
	...
	if rt.is_true(rt.less(rt.call_method(var_faker, 'randomDigit', []rt.PhpVal{}), rt.new_int(3))) {
		var_first_name = rt.new_string(rt.new_string(var_first_name.dup().to_string().to_lower()))
		var_last_name = rt.new_string(rt.new_string(var_last_name.dup().to_string().to_lower()))
	}
	...
	mut var_reflected_provider := create_wc_smoothgenerator_generator_reflectionclass(var_person_provider.dup())
...
end_label_1:
	for {
		...
		if !(rt.is_true(rt.call_function('email_exists', [var_person.array_get('email')]))) {
			break
		}
	}
	for {
		var_person.array_set('username', rt.call_method(var_faker, 'userName', []rt.PhpVal{}))
		if !(rt.is_true(rt.call_function('username_exists', [var_person.array_get('username')]))) {
			break
		}
	}
	var_reflected_provider.setstaticpropertyvalue(rt.new_string('firstNameMale'), var_orig_fn_male.dup())
	var_reflected_provider.setstaticpropertyvalue(rt.new_string('firstNameFemale'), var_orig_fn_female.dup())
	var_reflected_provider.setstaticpropertyvalue(rt.new_string('lastName'), var_orig_ln.dup())
	return var_person.dup()
}
...
end_label_2:
	...
	if rt.is_true(rt.less(rt.call_method(var_faker, 'randomDigit', []rt.PhpVal{}), rt.new_int(3))) {
		var_last_names = rt.call_function('array_map', [rt.new_string('strtolower'), var_last_names.dup()])
	}
	mut var_person_provider := Class_WC_SmoothGenerator_Generator_CustomerInfo.get_provider_instance(mut rt.cast_object_ptr[Class_WC_SmoothGenerator_Generator_Faker_Generator](var_faker), 'Person')
	mut var_reflected_provider := create_wc_smoothgenerator_generator_reflectionclass(var_person_provider.dup())
	mut var_orig_ln := var_reflected_provider.getstaticpropertyvalue(rt.new_string('lastName'), rt.new_array())
	var_reflected_provider.setstaticpropertyvalue(rt.new_string('lastName'), var_last_names.dup())
	...
	var_company.array_set('display_name', var_company.array_get('company'))
	var_reflected_provider.setstaticpropertyvalue(rt.new_string('lastName'), rt.create_array([rt.ArrayItem{ key: none, val: rt.call_method(var_faker, 'randomElement', [var_last_names.dup()]) }]))
	for {
...
end_label_3:
		if !(rt.is_true(rt.call_function('email_exists', [var_company.array_get('email')]))) {
			break
		}
	}
	for {
...
end_label_4:
		if !(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('username_exists', [var_company.array_get('username')])) || var_company.array_get('username').to_string().len < 3))) {
			break
		}
	}
	var_reflected_provider.setstaticpropertyvalue(rt.new_string('lastName'), var_orig_ln.dup())
	return var_company.dup()
}
...
fn Class_WC_SmoothGenerator_Generator_CustomerInfo.generate_address(country_code string) rt.PhpVal {
	...
	{
		mut iter_1 := rt.func_array_keys(var_address.dup()).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_line := item_1.val
			if rt.is_true(rt.new_bool(var_exceptions.array_get(country_code_mutated).array_get(var_line).array_isset(rt.new_string('hidden')) && rt.is_true(rt.identical(rt.new_bool(true), var_exceptions.array_get(country_code_mutated).array_get(var_line).array_get('hidden'))))) {
				continue
			}
			if rt.is_true(rt.new_bool(var_exceptions.array_get(country_code_mutated).array_get(var_line).array_isset(rt.new_string('required')) && rt.is_true(rt.identical(rt.new_bool(false), var_exceptions.array_get(country_code_mutated).array_get(var_line).array_get('required'))))) {
			...
			if rt.is_true(rt.equal(switch_val_1, rt.new_string('address1'))) {
				...
				mut var_states := rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'get_states', [rt.new_string(country_code_mutated).dup()])
				if rt.is_true(rt.new_bool(var_states.dup().is_array())) {
					var_address.array_set(var_line, rt.call_method(var_faker, 'randomElement', [rt.func_array_keys(var_states.dup())]))
				}
...
fn create_wc_smoothgenerator_generator_faker_factory() &Class_WC_SmoothGenerator_Generator_Faker_Factory {
...
fn create_wc_smoothgenerator_generator_reflectionclass() &Class_WC_SmoothGenerator_Generator_ReflectionClass {
...
pub fn init_wp_content_plugins_wc_smooth_generator_includes_generator_customerinfo_php() {
}
