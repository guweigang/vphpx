...
fn Class_WC_SmoothGenerator_Admin_BatchProcessor.get_current_job() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_current_job, 'WC_SmoothGenerator_Admin_AsyncJob'))))))
...
fn Class_WC_SmoothGenerator_Admin_BatchProcessor.create_new_job(generator_slug string, amount i64, mut var_args Class_WC_SmoothGenerator_Admin_array) rt.PhpVal {
	...
	{
		return mut rt.cast_object_ptr[Class_WC_SmoothGenerator_Admin_AsyncJob](create_wc_smoothgenerator_admin_wp_error(rt.new_string('smoothgenerator_async_job_already_exists'),
...
fn Class_WC_SmoothGenerator_Admin_BatchProcessor.update_current_job(processed i64) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_current_job,
		'WC_SmoothGenerator_Admin_AsyncJob'))))))
	{
		return create_wc_smoothgenerator_admin_wp_error(rt.new_string('smoothgenerator_async_job_does_not_exist'),
...
fn (mut this Class_WC_SmoothGenerator_Admin_BatchProcessor) get_next_batch_to_process(size i64) rt.PhpVal {
	mut var_current_job := Class_WC_SmoothGenerator_Admin_BatchProcessor.get_current_job()
	mut var_max_batch := fn () rt.PhpVal {
		mut temp := Class_WC_SmoothGenerator_Admin_BatchProcessor{}
		return temp.get_default_batch_size()
	}()
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_current_job,
		'WC_SmoothGenerator_Admin_AsyncJob'))))))
	{
	...
	mut var_amount := rt.call_function('min', [rt.new_int(size),
		rt.get_property(var_current_job, 'pending'), var_max_batch.dup()])
...
fn (mut this Class_WC_SmoothGenerator_Admin_BatchProcessor) process_batch(mut var_batch Class_WC_SmoothGenerator_Admin_array) {
	...
	mut var_result := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal {
		mut temp := Class_WC_SmoothGenerator_Router{}
		return temp.generate_batch(arg_0, arg_1, arg_2)
	}(var_slug.dup(), var_amount.dup(), rt.new_object('WC_SmoothGenerator_Admin_array', []string{},
		var_args))
	...
	Class_WC_SmoothGenerator_Admin_BatchProcessor.update_current_job(var_result.dup().array_count())
}
...
fn (mut this Class_WC_SmoothGenerator_Admin_BatchProcessor) get_default_batch_size() i64 {
	...
	mut var_generator := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_WC_SmoothGenerator_Router{}
		return temp.get_generator_class(arg_0)
...
fn create_wc_smoothgenerator_admin_wp_error() &Class_WC_SmoothGenerator_Admin_WP_Error {
...
fn create_wc_smoothgenerator_admin_asyncjob() &Class_WC_SmoothGenerator_Admin_AsyncJob {
...
fn create_wc_smoothgenerator_admin_exception() &Class_WC_SmoothGenerator_Admin_Exception {
...
pub fn init_wp_content_plugins_wc_smooth_generator_includes_admin_batchprocessor_php() {
