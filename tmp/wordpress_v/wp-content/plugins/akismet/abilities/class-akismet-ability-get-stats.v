...
fn (mut this Class_Akismet_Ability_Get_Stats) execute(mut var_input Class_?array) rt.PhpVal {
	mut var_interval := if var_input.array_isset(rt.new_string('interval')) { var_input.array_get('interval') } else { rt.new_string('6-months') }
	mut var_data := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Akismet{}; return temp.get_stats(arg_0) }(var_interval.dup())
...
fn create_akismet_ability_get_stats() &Class_Akismet_Ability_Get_Stats {
...
pub fn init_wp_content_plugins_akismet_abilities_class_akismet_ability_get_stats_php() {
