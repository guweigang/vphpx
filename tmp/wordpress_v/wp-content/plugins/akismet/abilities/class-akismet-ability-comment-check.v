...
fn (mut this Class_Akismet_Ability_Comment_Check) execute(mut var_input Class_?array) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(fn () rt.PhpVal { mut temp := Class_Akismet{}; return temp.get_api_key() }())))) {
	...
	mut var_result := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Akismet{}; return temp.comment_check(arg_0) }(rt.new_object('?array', []string{}, var_input))
...
fn create_akismet_ability_comment_check() &Class_Akismet_Ability_Comment_Check {
...
pub fn init_wp_content_plugins_akismet_abilities_class_akismet_ability_comment_check_php() {
