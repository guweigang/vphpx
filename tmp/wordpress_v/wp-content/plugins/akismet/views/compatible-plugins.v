...
pub fn init_wp_content_plugins_akismet_views_compatible_plugins_php() {
	mut var_bypass_cache := !(!rt.is_true(rt.get_superglobal('_GET').array_get('akismet_refresh_compatible_plugins')))
	mut var_compatible_plugins := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Akismet_Compatible_Plugins{}
		return temp.get_installed_compatible_plugins(arg_0)
	}(rt.new_bool(var_bypass_cache))
	if rt.is_true(rt.new_bool(var_compatible_plugins.dup().is_array())) {
		mut var_compatible_plugin_count := var_compatible_plugins.dup().array_count()
		...
		if 0 == var_compatible_plugin_count {
			...
			rt.echo_val(rt.call_function('esc_html', [
				rt.call_function('_n', [
					...
					rt.new_int(var_compatible_plugin_count).dup(),
					rt.new_string('akismet'),
				]),
			]))
		}
		...
		if !(!rt.is_true(var_compatible_plugins)) {
			// unsupported statement: Stmt_InlineHTML
			{
				...
				for {
					...
					if !rt.is_true(var_compatible_plugin.array_get('help_url')) {
						continue
					}
					// unsupported statement: Stmt_InlineHTML
					if var_compatible_plugin.array_get('logo').to_string().len > 0 {
						// unsupported statement: Stmt_InlineHTML
						mut var_logo_alt := rt.call_function('sprintf', [
							rt.call_function('__', [rt.new_string('%s logo'),
								rt.new_string('akismet')]),
							var_compatible_plugin.array_get('name'),
						])
						// unsupported statement: Stmt_InlineHTML
						rt.echo_val(rt.call_function('esc_url', [
							var_compatible_plugin.array_get('logo')]))
						// unsupported statement: Stmt_InlineHTML
					...
					// unsupported statement: Stmt_InlineHTML
					rt.echo_val(rt.call_function('esc_url', [
						var_compatible_plugin.array_get('help_url')]))
					// unsupported statement: Stmt_InlineHTML
					rt.echo_val(rt.call_function('esc_attr', [
						rt.call_function('sprintf', [
							rt.call_function('__', [
								rt.new_string('Documentation for %s'),
								rt.new_string('akismet'),
							]),
							var_compatible_plugin.array_get('name'),
						]),
