...
fn (mut this Class_Akismet_Widget) form(var_instance rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_instance_mutated)
		&& var_instance_mutated.array_isset(rt.new_string('title'))))
	{
...
fn (mut this Class_Akismet_Widget) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	if !(!rt.is_true(var_instance_mutated.array_get('title'))) {
		rt.echo_val(var_args.array_get('before_title'))
		rt.echo_val(rt.call_function('esc_html', [var_instance_mutated.array_get('title')]))
		rt.echo_val(var_args.array_get('after_title'))
	}
	// unsupported statement: Stmt_InlineHTML
...
pub fn init_wp_content_plugins_akismet_class_akismet_widget_php() {
