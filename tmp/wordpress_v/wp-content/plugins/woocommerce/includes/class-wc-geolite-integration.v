...
fn (mut this Class_WC_Geolite_Integration) construct(var_database rt.PhpVal)  {
	this.database = var_database.dup()
}

fn (mut this Class_WC_Geolite_Integration) get_country_iso(var_ip_address rt.PhpVal) rt.PhpVal {
	rt.call_function('wc_deprecated_function', [rt.new_string('get_country_iso'), rt.new_string('3.9.0')])
	mut var_iso_code := rt.new_string(rt.new_string(''))
	...
	if var_data.array_get('country').array_isset(rt.new_string('iso_code')) {
		var_iso_code = var_data.array_get('country').array_get('iso_code')
		if rt.has_exception() { unsafe { goto catch_label_1 } }
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	var_reader.close()
...
end_label_1:
	return rt.call_function('sanitize_text_field', [rt.new_string(var_iso_code.dup().to_string().to_upper())])
}
...
fn (mut this Class_WC_Geolite_Integration) log(var_message rt.PhpVal, level string)  {
	...
	rt.call_method(this.log, 'log', [rt.new_string(level), var_message.dup(), rt.create_array([rt.ArrayItem{ key: 'source', val: 'geoip' }])])
}
...
fn (mut this Class_WC_Geolite_Integration) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'database' { this.database = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_geolite_integration_php() {
