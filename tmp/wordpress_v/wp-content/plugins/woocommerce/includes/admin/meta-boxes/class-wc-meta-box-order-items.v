...
fn Class_WC_Meta_Box_Order_Items.output(var_post rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_thepostid.dup().is_long())))))
...
fn Class_WC_Meta_Box_Order_Items.save(var_post_id rt.PhpVal) {
	rt.call_function('wc_save_order_items', [var_post_id.dup(),
		rt.get_superglobal('_POST').dup()])
}
...
fn create_wc_meta_box_order_items() &Class_WC_Meta_Box_Order_Items {
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_meta_boxes_class_wc_meta_box_order_items_php() {
