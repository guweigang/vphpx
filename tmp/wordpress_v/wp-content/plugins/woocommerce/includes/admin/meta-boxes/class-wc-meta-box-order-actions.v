...
fn Class_WC_Meta_Box_Order_Actions.output(var_post rt.PhpVal) {
	...
	fn (arg_0 rt.PhpVal) rt.PhpVal {
		...
		return temp.init_theorder_object(arg_0)
	}(var_post.dup())
	mut var_order := var_theorder
	mut var_order_id := rt.call_method(var_order, 'get_id', []rt.PhpVal{})
	mut var_order_actions :=
		Class_WC_Meta_Box_Order_Actions.get_available_order_actions_for_order(var_order.dup())
	...
	// unsupported statement: Stmt_InlineHTML
	{
		...
		for {
			...
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('esc_attr', [var_action.dup()]))
			// unsupported statement: Stmt_InlineHTML
		...
		var_order_id.dup()]))
	{
		...
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_html', [var_delete_text.dup()]))
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
...
fn Class_WC_Meta_Box_Order_Actions.get_trash_or_delete_order_link(order_id i64) string {
	...
	}())
	{
		mut var_order_type := rt.call_method(rt.call_function('wc_get_order', [
			rt.new_int(order_id_mutated).dup()]), 'get_type', []rt.PhpVal{})
		...
		]), 'get_base_page_url', [var_order_type.dup()])
		mut var_trash_order_url := rt.call_function('add_query_arg', [
			...
			var_order_list_url.dup(),
		])
		return (rt.call_function('wp_nonce_url', [var_trash_order_url.dup(),
			rt.new_string('bulk-orders')])).str()
	}
	return (rt.call_function('get_delete_post_link', [rt.new_int(order_id_mutated).dup()])).str()
}
	...
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get('wc_order_action'))) {
		mut var_action := rt.call_function('wc_clean', [
			rt.call_function('wp_unslash',
				[rt.get_superglobal('_POST').array_get('wc_order_action')]),
		])
		if rt.is_true(rt.identical(rt.new_string('send_order_details'), var_action)) {
			...
			rt.call_method(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'mailer',
				[]rt.PhpVal{}), 'customer_invoice', [var_order.dup()])
			...
			rt.call_method(rt.get_property(rt.call_method(rt.call_function('WC', []rt.PhpVal{}),
				'mailer', []rt.PhpVal{}), 'emails').array_get('WC_Email_New_Order'), 'trigger', [
...
fn Class_WC_Meta_Box_Order_Actions.set_email_sent_message(var_location rt.PhpVal) rt.PhpVal {
	return rt.call_function('add_query_arg', [rt.new_string('message'),
		rt.new_int(11), var_location.dup()])
}
...
fn Class_WC_Meta_Box_Order_Actions.get_available_order_actions_for_order(var_order rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_order_actions'),
		var_actions.dup(), var_order_mutated.dup()])
}
...
fn create_wc_meta_box_order_actions() &Class_WC_Meta_Box_Order_Actions {
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_meta_boxes_class_wc_meta_box_order_actions_php() {
