...
pub fn init_wp_content_plugins_woocommerce_includes_admin_meta_boxes_views_html_product_data_general_php() {
	...
	// unsupported statement: Stmt_InlineHTML
	mut var_tax_label := rt.new_string(rt.new_string(''))
	...
	mut var_sale_price_dates_from := if rt.is_true(var_sale_price_dates_from_timestamp) { rt.call_function('date_i18n', [rt.new_string('Y-m-d'), var_sale_price_dates_from_timestamp.dup()]) } else { rt.new_string('') }
	mut var_sale_price_dates_to := if rt.is_true(var_sale_price_dates_to_timestamp) { rt.call_function('date_i18n', [rt.new_string('Y-m-d'), var_sale_price_dates_to_timestamp.dup()]) } else { rt.new_string('') }
	...
	if rt.is_true(var_downloadable_files) {
		{
			...
			for {
				...
				mut var_disabled_download := var_file.array_isset(rt.new_string('enabled')) && rt.is_true(rt.identical(rt.new_bool(false), var_file.array_get('enabled')))
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('woocommerce_wp_text_input', [rt.create_array([rt.ArrayItem{ key: 'id', val: '_download_limit' }, rt.ArrayItem{ key: 'value', val: if rt.is_true(rt.identical(// unsupported expression: Expr_UnaryMinus, rt.call_method(var_product_object, 'get_download_limit', [rt.new_string('edit')]))) { rt.new_string('') } else { rt.call_method(var_product_object, 'get_download_limit', [rt.new_string('edit')]) } }, rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Download limit'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'placeholder', val: rt.call_function('__', [rt.new_string('Unlimited'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Leave blank for unlimited re-downloads.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'number' }, rt.ArrayItem{ key: 'custom_attributes', val: rt.create_array([rt.ArrayItem{ key: 'step', val: '1' }, rt.ArrayItem{ key: 'min', val: '0' }]) }])])
	rt.call_function('woocommerce_wp_text_input', [rt.create_array([rt.ArrayItem{ key: 'id', val: '_download_expiry' }, rt.ArrayItem{ key: 'value', val: if rt.is_true(rt.identical(// unsupported expression: Expr_UnaryMinus, rt.call_method(var_product_object, 'get_download_expiry', [rt.new_string('edit')]))) { rt.new_string('') } else { rt.call_method(var_product_object, 'get_download_expiry', [rt.new_string('edit')]) } }, rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Download expiry'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'placeholder', val: rt.call_function('__', [rt.new_string('Never'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Enter the number of days before a download link expires, or leave blank.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'number' }, rt.ArrayItem{ key: 'custom_attributes', val: rt.create_array([rt.ArrayItem{ key: 'step', val: '1' }, rt.ArrayItem{ key: 'min', val: '0' }]) }])])
