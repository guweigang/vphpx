...
pub fn init_wp_content_plugins_woocommerce_includes_admin_meta_boxes_views_html_product_data_inventory_php() {
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_url', [var_info_img_url.dup()]))
	// unsupported statement: Stmt_InlineHTML
	...
	])))
	{
		...
		{
			rt.call_function('woocommerce_wp_radio', [var_backorder_args.dup()])
		} else {
			rt.call_function('woocommerce_wp_select', [var_backorder_args.dup()])
		}
		rt.call_function('woocommerce_wp_text_input', [
	...
	mut var_stock_status_options := rt.call_function('wc_get_product_stock_status_options',
		[]rt.PhpVal{})
	mut var_stock_status_count := var_stock_status_options.dup().array_count()
	...
	{
		rt.call_function('woocommerce_wp_radio', [var_stock_status_args.dup()])
	} else {
		rt.call_function('woocommerce_wp_select', [var_stock_status_args.dup()])
	}
	rt.call_function('do_action', [
