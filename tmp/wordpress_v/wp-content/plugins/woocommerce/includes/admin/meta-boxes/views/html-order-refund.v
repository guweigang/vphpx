...
pub fn init_wp_content_plugins_woocommerce_includes_admin_meta_boxes_views_html_order_refund_php() {
	...
	rt.echo_val(if !(!rt.is_true(var_class)) { rt.call_function('esc_attr', [
			var_class.dup()]) } else { rt.new_string('') })
	// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(var_who_refunded.exists()) {
		rt.call_function('printf', [
			...
			rt.call_function('sprintf', [
				...
				rt.call_function('sprintf', [
					rt.call_function('esc_attr__', [rt.new_string('ID: %d'),
						rt.new_string('woocommerce')]),
					rt.call_function('absint', [rt.get_property(var_who_refunded, 'ID')]),
				]),
				rt.call_function('esc_html', [
	...
	rt.call_function('do_action', [rt.new_string('woocommerce_admin_order_item_values'),
		rt.new_null(), var_refund.dup(), rt.call_method(var_refund, 'get_id', []rt.PhpVal{})])
	// unsupported statement: Stmt_InlineHTML
	...
	rt.echo_val(rt.call_function('wp_kses_post', [
		rt.call_function('wc_price', [
			'-' + (rt.call_method(var_refund, 'get_amount', []rt.PhpVal{})).str(),
			rt.create_array([
	...
	if rt.is_true(rt.call_function('wc_tax_enabled', []rt.PhpVal{})) {
		mut var_total_taxes := var_order_taxes.dup().array_count()
		// unsupported statement: Stmt_InlineHTML
		{
			mut var_i := 0
			for {
