...
pub fn init_wp_content_plugins_woocommerce_includes_admin_meta_boxes_views_html_product_attribute_inner_php() {
	...
	if rt.is_true(rt.call_method(var_attribute, 'is_taxonomy', []rt.PhpVal{})) {
		...
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_i.dup()]))
		// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.call_method(var_attribute, 'is_taxonomy', []rt.PhpVal{}))
		&& rt.is_true(rt.call_method(var_attribute, 'get_taxonomy_object', []rt.PhpVal{}))))
	{
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_attribute_types.dup().array_isset(rt.get_property(var_attribute_taxonomy,
			'attribute_type')))))))
		{
			...
			'attribute_type')))
		{
			...
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('esc_attr', [var_term_limit.dup()]))
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('esc_attr_e', [rt.new_string('Select values'),
				rt.new_string('woocommerce')])
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('esc_attr', [var_attribute_orderby.dup()]))
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('esc_attr', [var_i.dup()]))
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('esc_attr', [
			...
			if rt.is_true(var_selected_terms) {
				{
					...
					for {
						...
						print('<option value="' +
							...
							'" selected="selected">' +
							(rt.call_function('esc_html', [rt.call_function('apply_filters', [rt.new_string('woocommerce_product_attribute_term_name'), rt.get_property(var_selected_term, 'name'), var_selected_term.dup()])])).str() +
							'</option>')
					}
				}
			}
		...
		rt.call_function('do_action', [rt.new_string('woocommerce_product_option_terms'),
			var_attribute_taxonomy.dup(), var_i.dup(), var_attribute.dup()])
	} else {
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_i.dup()]))
		// unsupported statement: Stmt_InlineHTML
