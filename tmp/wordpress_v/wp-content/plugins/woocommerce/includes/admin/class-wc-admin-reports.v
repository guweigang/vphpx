...
fn Class_WC_Admin_Reports.delete_legacy_reports_transients(order_id i64, defer bool) {
	if rt.is_true(rt.new_bool(var_defer
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_using_ext_object_cache', []rt.PhpVal{})))))))
	{
		// unsupported statement: Stmt_Static
		mut var_schedule := rt.new_bool(rt.new_bool(
			rt.is_true(rt.new_bool(!(rt.is_true(var_skip_consequent))))
		...
		mut var_skip_consequent := rt.new_bool(rt.new_bool(true))
		return rt.new_null()
	}
...
fn Class_WC_Admin_Reports.replace_dashboard_status_widget_reports(var_status_widget_reports rt.PhpVal) rt.PhpVal {
	...
	return var_status_widget_reports_mutated.dup()
}

fn Class_WC_Admin_Reports.output() {
	mut var_reports := Class_WC_Admin_Reports.get_reports()
	mut var_first_tab := rt.func_array_keys(var_reports.dup())
	mut var_current_tab := if rt.is_true(rt.new_bool(!(!rt.is_true(rt.get_superglobal('_GET').array_get('tab'))) && rt.is_true(rt.new_bool(var_reports.dup().array_isset(rt.get_superglobal('_GET').array_get('tab')))))) { rt.call_function('sanitize_title', [
			rt.get_superglobal('_GET').array_get('tab'),
		]) } else { var_first_tab.array_get(0) }
	mut var_current_report := if rt.get_superglobal('_GET').array_isset(rt.new_string('report')) { rt.call_function('sanitize_title', [
			...
			rt.func_array_keys(var_reports.array_get(var_current_tab).array_get('reports')),
		]) }
	...
	])
	{
		...
		for {
			...
			if var_report_group.array_isset(rt.new_string('charts')) {
				var_report_group.array_set('reports', var_report_group.array_get('charts'))
			}
			...
			}
			{
				...
				for {
					...
					if var_report.array_isset(rt.new_string('function')) {
						var_report.array_set('callback', var_report.array_get('function'))
					}
				}
			}
		}
...
fn Class_WC_Admin_Reports.get_report(var_name rt.PhpVal) {
	...
	mut var_class :=
		rt.new_string('WC_Report_' +(rt.call_function('str_replace', [rt.new_string('-'), rt.new_string('_'), var_name_mutated.dup()])).str())
	rt.include_file((rt.call_function('apply_filters', [
		rt.new_string('wc_admin_reports_path'),
		'reports/class-wc-report-' + var_name_mutated.str() + '.php',
		var_name_mutated.dup(),
		var_class.dup(),
	])).to_string(), '2')
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_class_wc_admin_reports_php() {
