...
fn (mut this Class_WC_Admin_Exporters) add_to_menus() {
	{
		...
		for {
			...
			rt.call_function('add_submenu_page', [var_exporter.array_get('menu'),
				var_exporter.array_get('name'), var_exporter.array_get('name'),
				var_exporter.array_get('capability'), var_id.dup(),
				var_exporter.array_get('callback')])
		}
	}
}
...
fn (mut this Class_WC_Admin_Exporters) hide_from_menus() {
	...
	// unsupported statement: Stmt_Global
	{
		...
		for {
			...
			mut var_exporter := item_1.val
			mut var_id := item_1.key
			if var_submenu.array_isset(var_exporter.array_get('menu')) {
				{
					mut iter_2 := var_submenu.array_get(var_exporter.array_get('menu')).iterator()
					for {
						item_2 := iter_2.next() or { break }
						mut var_menu := item_2.val
						mut var_key := item_2.key
						if rt.is_true(rt.identical(var_id, var_menu.array_get(2))) {
							var_submenu.array_get(var_exporter.array_get('menu')).array_unset(var_key)
						}
					}
				}
			}
...
fn (mut this Class_WC_Admin_Exporters) admin_scripts() {
	mut var_suffix := rt.new_string(if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_Jetpack_Constants{}return temp.is_true(arg_0)}(rt.new_string('SCRIPT_DEBUG')))
	{ rt.new_string('') } else { rt.new_string('.min') })
	mut var_version := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_Jetpack_Constants{}
		return temp.get_constant(arg_0)
...
fn (mut this Class_WC_Admin_Exporters) download_export_file() {
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(rt.new_string('action'))
		&& rt.get_superglobal('_GET').array_isset(rt.new_string('nonce'))
		&& rt.is_true(rt.call_function('wp_verify_nonce', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('nonce')]), rt.new_string('product-csv')]))))
		&& rt.is_true(rt.identical(rt.new_string('download_product_csv'), rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('action')])))))
	{
		rt.include_file(
			(rt.get_constant('WC_ABSPATH')).str() + 'includes/export/class-wc-product-csv-exporter.php',
			'2')
		mut var_exporter := create_wc_product_csv_exporter()
		if !(!rt.is_true(rt.get_superglobal('_GET').array_get('filename'))) {
			rt.call_method(var_exporter, 'set_filename', [
				rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('filename')]),
			])
			// unsupported statement: Stmt_Nop
		}
...
fn (mut this Class_WC_Admin_Exporters) do_ajax_product_export() {
	...
	mut var_step := if rt.get_superglobal('_POST').array_isset(rt.new_string('step')) { rt.call_function('absint', [
			rt.get_superglobal('_POST').array_get('step'),
		]) } else { rt.new_int(1) }
	mut var_exporter := create_wc_product_csv_exporter()
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get('columns'))) {
		rt.call_method(var_exporter, 'set_column_names', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_POST').array_get('columns')]),
		])
		// unsupported statement: Stmt_Nop
	}
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get('selected_columns'))) {
		rt.call_method(var_exporter, 'set_columns_to_export', [
			rt.call_function('wp_unslash',
				[rt.get_superglobal('_POST').array_get('selected_columns')]),
		])
		// unsupported statement: Stmt_Nop
	}
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get('export_meta'))) {
		rt.call_method(var_exporter, 'enable_meta_export', [rt.new_bool(true)])
	}
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get('export_types'))) {
		rt.call_method(var_exporter, 'set_product_types_to_export', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_POST').array_get('export_types')]),
		])
		// unsupported statement: Stmt_Nop
	}
	if rt.is_true(rt.new_bool(
		!(!rt.is_true(rt.get_superglobal('_POST').array_get('export_category')))
		&& rt.is_true(rt.new_bool(rt.get_superglobal('_POST').array_get('export_category').is_array()))))
	{
		rt.call_method(var_exporter, 'set_product_category_to_export', [
			rt.call_function('wp_unslash', [
				rt.call_function('array_values',
					[rt.get_superglobal('_POST').array_get('export_category')]),
			]),
		])
		// unsupported statement: Stmt_Nop
	}
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get('export_product_ids'))) {
		mut var_ids_raw := rt.call_function('explode', [rt.new_string(','),
			rt.call_function('sanitize_text_field', [
				rt.call_function('wp_unslash',
					[rt.get_superglobal('_POST').array_get('export_product_ids')]),
			])])
	...
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get('filename'))) {
		rt.call_method(var_exporter, 'set_filename', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_POST').array_get('filename')]),
		])
		// unsupported statement: Stmt_Nop
	}
	rt.call_method(var_exporter, 'set_page', [var_step.dup()])
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_class_wc_admin_exporters_php() {
