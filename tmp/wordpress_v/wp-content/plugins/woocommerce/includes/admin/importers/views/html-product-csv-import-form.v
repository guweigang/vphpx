...
pub fn init_wp_content_plugins_woocommerce_includes_admin_importers_views_html_product_csv_import_form_php() {
	...
	if !(!rt.is_true(var_upload_dir.array_get('error'))) {
		...
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_bytes.dup()]))
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('printf', [
			rt.call_function('esc_html__', [rt.new_string('Maximum size: %s'),
				rt.new_string('woocommerce')]),
			rt.call_function('esc_html', [var_size.dup()]),
		])
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
	...
	rt.call_function('sort', [var_encodings.dup(), rt.get_constant('SORT_NATURAL')])
	{
		mut iter_1 := var_encodings.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_encoding := item_1.val
			print('<option>' + (rt.call_function('esc_html', [var_encoding.dup()])).str() +
				'</option>')
		}
	}
	// unsupported statement: Stmt_InlineHTML
