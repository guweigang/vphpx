...
pub fn init_wp_content_plugins_woocommerce_includes_admin_settings_views_html_settings_tax_php() {
	...
	rt.echo_val(if rt.get_superglobal('_GET').array_isset(rt.new_string('s')) { rt.call_function('esc_attr', [
			rt.get_superglobal('_GET').array_get('s'),
		]) } else { rt.new_string('') })
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('printf', [
		...
		if rt.is_true(var_current_class) { rt.call_function('esc_html', [
				var_current_class.dup()]) } else { rt.call_function('__', [
