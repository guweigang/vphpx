...
pub fn init_wp_content_plugins_woocommerce_includes_admin_settings_views_html_admin_page_shipping_zones_php() {
	...
	if rt.is_true(fn () rt.PhpVal {
		...
		return temp.is_checkout_block_default()
	}())
	{
	...
	rt.call_function('uasort', [var_methods.dup(),
	...
	if !(!rt.is_true(var_methods)) {
		{
			...
			for {
				...
				print('<li class="wc-shipping-zone-method ' +
					(rt.call_function('esc_attr', [rt.new_string(var_class_name).dup()])).str() +
					'" data-id="' +
	...
	// unsupported statement: Stmt_InlineHTML
	{
		mut iter_1 := rt.call_method(rt.call_method(rt.call_function('WC', []rt.PhpVal{}),
			'shipping', []rt.PhpVal{}), 'load_shipping_methods', []rt.PhpVal{}).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_method := item_1.val
