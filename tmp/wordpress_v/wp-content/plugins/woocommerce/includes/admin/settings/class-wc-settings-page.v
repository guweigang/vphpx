...
fn (mut this Class_WC_Settings_Page) construct() {
	rt.call_function('add_filter', [rt.new_string('woocommerce_settings_tabs_array'),
		...
		rt.new_int(20)])
	rt.call_function('add_action', ['woocommerce_sections_' + (this.id).str(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Settings_Page', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'output_sections' },
		])])
	rt.call_function('add_action', ['woocommerce_settings_' + (this.id).str(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Settings_Page', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'output' },
		])])
	rt.call_function('add_action', ['woocommerce_settings_save_' + (this.id).str(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Settings_Page', []string{}, &this) },
...
fn (mut this Class_WC_Settings_Page) add_settings_page_data(var_pages rt.PhpVal) rt.PhpVal {
	...
	mut var_saved_current_section := var_current_section.dup()
	...
	mut var_sections_data := rt.new_array()
	{
		...
		for {
			...
			mut var_current_section := var_section_id.dup()
			mut var_section_settings_data := this.get_section_settings_data(var_section_id.dup(),
				var_sections.dup())
			...
			var_sections_data.array_set(var_normalized_section_id, rt.create_array([
				rt.ArrayItem{ key: 'label', val: rt.call_function('html_entity_decode', [
					rt.call_function('esc_html', [var_section_label.dup()]),
				]) },
				rt.ArrayItem{ key: 'settings', val: var_section_settings_data },
			]))
		}
	}
	mut var_current_section := var_saved_current_section.dup()
...
fn (mut this Class_WC_Settings_Page) get_section_settings_data(var_section_id rt.PhpVal, var_sections rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(this.output_called) {
		mut var_section_settings := if var_sections_mutated.dup().array_count() > 1 {
			this.get_settings_for_section(var_section_id_mutated.dup())
		} else {
			this.get_settings()
		}
		{
			mut iter_1 := var_section_settings.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_section_setting := item_1.val
				if rt.is_true(rt.new_bool(
					rt.is_true(rt.identical(rt.new_string('sectionend'), var_section_setting.array_get('type')))
					&& !(!rt.is_true(var_section_setting.array_get('id')))))
				{
					var_section_settings_data.array_push(this.get_custom_view(rt.new_string(
						'woocommerce_settings_' +
						(var_section_setting.array_get('id')).str() + '_end'), false))
					var_section_settings_data.array_push(this.get_custom_view(rt.new_string(
						'woocommerce_settings_' +
						(var_section_setting.array_get('id')).str() + '_after'), false))
				}
				var_section_settings_data.array_push(this.populate_setting_value(var_section_setting.dup()))
				if rt.is_true(rt.new_bool(
					rt.is_true(rt.identical(rt.new_string('title'), var_section_setting.array_get('type')))
					&& !(!rt.is_true(var_section_setting.array_get('id')))))
				{
					var_section_settings_data.array_push(this.get_custom_view(rt.new_string(
						'woocommerce_settings_' + (var_section_setting.array_get('id')).str()),
						false))
				}
			}
		}
	...
	if !(!rt.is_true(var_custom_view)) {
		var_section_settings_data.array_push(var_custom_view.dup())
	}
	this.output_called = false
	return var_section_settings_data.dup()
}
...
fn (mut this Class_WC_Settings_Page) populate_setting_value(var_section_setting rt.PhpVal) rt.PhpVal {
	...
	if var_section_setting_mutated.array_isset(rt.new_string('id')) {
		var_section_setting_mutated.array_set('value', if var_section_setting_mutated.array_isset(rt.new_string('default')) {
			rt.call_function('get_option', [var_section_setting_mutated.array_get('id'),
				var_section_setting_mutated.array_get('default')])
		} else {
			rt.call_function('get_option', [var_section_setting_mutated.array_get('id')])
		})
	}
	mut var_type := var_section_setting_mutated.array_get('type')
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_type.dup(), this.types, rt.new_bool(true)])))))
	{
		var_section_setting_mutated = this.get_custom_type_field(rt.new_string(
			'woocommerce_admin_field_' + var_type.str()), var_section_setting_mutated.dup())
	}
	return var_section_setting_mutated.dup()
}
...
fn (mut this Class_WC_Settings_Page) get_custom_view(var_action rt.PhpVal, section_id bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(section_id_mutated)) {
		mut var_saved_current_section := var_current_section.dup()
		mut var_current_section := rt.new_bool(rt.new_bool(section_id_mutated)).dup()
	}
	rt.call_function('ob_start', []rt.PhpVal{})
	rt.call_function('do_action', [var_action.dup()])
	mut var_html := rt.call_function('ob_get_contents', []rt.PhpVal{})
	rt.call_function('ob_end_clean', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(section_id_mutated)) {
		var_current_section = var_saved_current_section.dup()
	}
	mut var_content := rt.new_string(rt.new_string(var_html.dup().to_string().trim_space()))
...
fn (mut this Class_WC_Settings_Page) get_custom_type_field(var_action rt.PhpVal, var_setting rt.PhpVal) rt.PhpVal {
	...
	rt.call_function('ob_start', []rt.PhpVal{})
	rt.call_function('do_action', [var_action.dup(), var_setting_mutated.dup()])
	mut var_html := rt.call_function('ob_get_contents', []rt.PhpVal{})
	rt.call_function('ob_end_clean', []rt.PhpVal{})
	var_setting_mutated.array_set('content', var_html.dup().to_string().trim_space())
	var_setting_mutated.array_set('id', if var_setting_mutated.array_isset(rt.new_string('id')) { var_setting_mutated.array_get('id') } else { rt.call_function('wp_unique_prefixed_id', [
			rt.new_string('settings_custom_view'),
		]) })
	var_setting_mutated.array_set('type', 'custom')
	return var_setting_mutated.dup()
}
...
fn (mut this Class_WC_Settings_Page) get_settings() rt.PhpVal {
	...
	return this.get_settings_for_section(var_section_id.dup())
}
...
fn (mut this Class_WC_Settings_Page) get_settings_for_section(var_section_id rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_string(''), var_section_id_mutated)) {
		mut var_method_name := rt.new_string(rt.new_string('get_settings_for_default_section'))
	} else {
		var_method_name =
			rt.new_string(rt.new_string('get_settings_for_${var_section_id.to_string()}_section'))
	}
	...
	]))
	{
		...
		var_settings = this.get_settings_for_section_core(var_section_id_mutated.dup())
	}
	return rt.call_function('apply_filters', [
		'woocommerce_get_settings_' + (this.id).str(),
		var_settings.dup(),
		var_section_id_mutated.dup(),
	])
}
...
fn (mut this Class_WC_Settings_Page) get_sections() rt.PhpVal {
	...
	return rt.cast_array(rt.call_function('apply_filters', [
		'woocommerce_get_sections_' + (this.id).str(),
		var_sections.dup(),
	]))
}
...
fn (mut this Class_WC_Settings_Page) output_sections() {
	...
	// unsupported statement: Stmt_Global
	mut var_sections := this.get_sections()
	if !rt.is_true(var_sections) || 1 == var_sections.dup().array_count() {
		return rt.new_null()
	}
	print('<ul class="subsubsub">')
	mut var_array_keys := rt.func_array_keys(var_sections.dup())
	{
		mut iter_1 := var_sections.iterator()
		for {
			item_1 := iter_1.next() or { break }
			...
			mut var_url := rt.call_function('admin_url', [
				'admin.php?page=wc-settings&tab=' +(this.id).str() + '&section=' +
					(rt.call_function('sanitize_title', [var_id.dup()])).str(),
			])
			...
			{ rt.new_string('') } else { rt.new_string('|') })
			mut var_text := rt.call_function('esc_html', [var_label.dup()])
...
fn (mut this Class_WC_Settings_Page) output() {
	...
	// unsupported statement: Stmt_Global
	mut var_settings := this.get_settings(var_current_section.dup())
	fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_WC_Admin_Settings{}
		return temp.output_fields(arg_0)
	}(var_settings.dup())
}
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_settings_class_wc_settings_page_php() {
