...
fn (mut this Class_WC_Settings_Integrations) construct()  {
	...
	this.dispatch_set_prop('label', rt.call_function('__', [rt.new_string('Integration'), rt.new_string('woocommerce')]))
	if rt.is_true(rt.new_bool(!(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'integrations')).is_null() && rt.is_true(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'integrations'), 'get_integrations', []rt.PhpVal{})))) {
...
fn (mut this Class_WC_Settings_Integrations) get_own_sections() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(this.wc_is_installing())))) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_current_section)))) && !(!rt.is_true(var_integrations)))) {
			mut var_current_section := rt.get_property(rt.call_function('current', [var_integrations.dup()]), 'id')
		}
		if var_integrations.dup().array_count() > 1 {
			{
				mut iter_1 := var_integrations.iterator()
				for {
					item_1 := iter_1.next() or { break }
					mut var_integration := item_1.val
					mut var_title := if !rt.is_true(rt.get_property(var_integration, 'method_title')) { rt.call_function('ucfirst', [rt.get_property(var_integration, 'id')]) } else { rt.get_property(var_integration, 'method_title') }
					var_sections[rt.get_property(var_integration, 'id').to_string().to_lower()] = rt.call_function('esc_html', [var_title.dup()])
				}
			}
		}
	}
...
fn (mut this Class_WC_Settings_Integrations) get_integrations() rt.PhpVal {
	return rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'integrations'), 'get_integrations', []rt.PhpVal{})
}
...
fn (mut this Class_WC_Settings_Integrations) output()  {
	...
	if var_integrations.array_isset(var_current_section) {
		rt.call_method(var_integrations.array_get(var_current_section), 'admin_options', []rt.PhpVal{})
	}
}
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_settings_class_wc_settings_integrations_php() {
	rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('ABSPATH')])) || rt.is_true(// unsupported expression: Expr_Exit))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [rt.new_string('WC_Settings_Integrations'), rt.new_bool(false)]))))) {
	}
	return create_wc_settings_integrations()
}
