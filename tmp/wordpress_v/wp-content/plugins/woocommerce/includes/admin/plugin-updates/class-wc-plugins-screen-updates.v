...
fn (mut this Class_WC_Plugins_Screen_Updates) in_plugin_update_message(var_args rt.PhpVal, var_response rt.PhpVal) {
	...
	mut var_version_type := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_Jetpack_Constants{}
		return temp.get_constant(arg_0)
	}(rt.new_string('WC_SSR_PLUGIN_UPDATE_RELEASE_VERSION_TYPE'))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_version_type.dup().is_string()))))) {
		var_version_type = rt.new_string(rt.new_string('none'))
	}
	...
	this.dispatch_set_prop('major_untested_plugins', this.get_untested_plugins(rt.get_property(var_response_mutated,
		'new_version'), var_version_type.dup()))
	mut var_current_version_parts := rt.call_function('explode', [
		rt.new_string('.'), fn (arg_0 rt.PhpVal) rt.PhpVal {
			mut temp := Class_Automattic_Jetpack_Constants{}
			return temp.get_constant(arg_0)
	...
	if rt.is_true(rt.call_function('version_compare', [
		(var_current_version_parts.array_get(0)).str() + '.' +
			(var_current_version_parts.array_get(1)).str(),
		(var_new_version_parts.array_get(0)).str() + '.' +
			(var_new_version_parts.array_get(1)).str(),
		rt.new_string('='),
	]))
	{
		return rt.new_null()
	...
	rt.echo_val(rt.call_function('apply_filters', [
		...
		if rt.is_true(this.upgrade_notice) {
...
fn (mut this Class_WC_Plugins_Screen_Updates) get_upgrade_notice(var_version rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_bool(false), var_upgrade_notice)) {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_wp_error', [var_response.dup()])))))
			&& !(!rt.is_true(var_response.array_get('body')))))
		{
			var_upgrade_notice = this.parse_update_notice(var_response.array_get('body'),
				var_version.dup())
			rt.call_function('set_transient', [var_transient_name.dup(),
				var_upgrade_notice.dup(), rt.get_constant('DAY_IN_SECONDS')])
		}
	}
	return var_upgrade_notice.dup()
}
...
fn (mut this Class_WC_Plugins_Screen_Updates) parse_update_notice(var_content rt.PhpVal, var_new_version rt.PhpVal) rt.PhpVal {
	mut var_version_parts := rt.call_function('explode', [rt.new_string('.'),
		var_new_version.dup()])
	mut var_check_for_notices := [(var_version_parts.array_get(0)).str() + '.0', 
		(var_version_parts.array_get(0)).str() + '.0.0',
		(var_version_parts.array_get(0)).str() + '.' + (var_version_parts.array_get(1)).str(),
		(var_version_parts.array_get(0)).str() + '.' + (var_version_parts.array_get(1)).str() +
			'.' + (var_version_parts.array_get(2)).str()]
	mut var_notice_regexp := rt.new_string(
		'~==\\s*Upgrade Notice\\s*==\\s*=\\s*(.*)\\s*=(.*)(=\\s*' +
		(rt.call_function('preg_quote', [var_new_version.dup()])).str() + '\\s*=|$)~Uis')
	mut var_upgrade_notice := rt.new_string(rt.new_string(''))
	for var_check_version in var_check_for_notices {
		if rt.is_true(rt.call_function('version_compare', [fn (arg_0 rt.PhpVal) rt.PhpVal {
			mut temp := Class_Automattic_Jetpack_Constants{}
			return temp.get_constant(arg_0)
		}(rt.new_string('WC_VERSION')),
			rt.new_string(check_version), rt.new_string('>')]))
		{
			continue
		}
		mut var_matches := rt.new_null()
		if rt.is_true(rt.call_function('preg_match', [var_notice_regexp.dup(),
			var_content.dup(), var_matches.dup()]))
		{
			mut var_notices := rt.cast_array(rt.call_function('preg_split', [
				rt.new_string('~[\\r\\n]+~'),
				rt.new_string(var_matches.array_get(2).to_string().trim_space()),
			]))
			if rt.is_true(rt.call_function('version_compare', [
	...
	return rt.call_function('wp_kses_post', [var_upgrade_notice.dup()])
}
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_plugin_updates_class_wc_plugins_screen_updates_php() {
