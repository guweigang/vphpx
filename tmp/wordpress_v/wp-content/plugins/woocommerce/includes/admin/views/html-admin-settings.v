...
pub fn init_wp_content_plugins_woocommerce_includes_admin_views_html_admin_settings_php() {
	...
	mut var_tab_exists :=
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_tabs.array_isset(var_current_tab)
		|| rt.is_true(rt.call_function('has_action', ['woocommerce_sections_' + var_current_tab.str()]))))
		|| rt.is_true(rt.call_function('has_action', ['woocommerce_settings_' + var_current_tab.str()]))))
		|| rt.is_true(rt.call_function('has_action', ['woocommerce_settings_tabs_' + var_current_tab.str()]))
	mut var_current_tab_label := if var_tabs.array_isset(var_current_tab) {
		var_tabs.array_get(var_current_tab)
	} else {
		rt.new_string('')
	}
	if !var_tab_exists {
	...
	mut var_hide_nav := rt.is_true(rt.identical(rt.new_string('checkout'), var_current_tab))
		&& rt.is_true(rt.call_function('in_array', [var_current_section.dup(), rt.create_array([rt.ArrayItem{
		key: none
		val: 'offline'
	...
	if rt.is_true(rt.new_bool(var_tabs.dup().array_isset(rt.new_string('advanced')))) {
		mut var_advanced := var_tabs.array_get('advanced')
		var_tabs.delete('advanced')
		var_tabs['advanced'] = var_advanced.dup()
	}
	...
	rt.call_function('do_action', [
		'woocommerce_before_settings_' + var_current_tab.str(),
	])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [
		rt.call_function('apply_filters', [
			'woocommerce_settings_form_method_tab_' + var_current_tab.str(),
			rt.new_string('post'),
		]),
	]))
	// unsupported statement: Stmt_InlineHTML
	if !var_hide_nav {
		...
		for var_slug, var_label in var_tabs {
			print('<a href="' +
				(rt.call_function('esc_html', [rt.call_function('admin_url', ['admin.php?page=wc-settings&tab=' + (rt.call_function('esc_attr', [rt.new_string(slug)])).str()])])).str() +
				'" class="nav-tab ' +
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_current_tab_label.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', ['woocommerce_sections_' + var_current_tab.str()])
	fn () rt.PhpVal {
		mut temp := Class_WC_Admin_Settings{}
		return temp.show_messages()
	}()
	rt.call_function('do_action', ['woocommerce_settings_' + var_current_tab.str()])
	rt.call_function('do_action', [
		'woocommerce_settings_tabs_' + var_current_tab.str(),
	])
	// unsupported statement: Stmt_Nop
	// unsupported statement: Stmt_InlineHTML
	if !rt.is_true(var_GLOBALS.array_get('hide_save_button')) {
		// unsupported statement: Stmt_InlineHTML
	...
	if var_marketplace_links.array_isset(var_current_tab) {
		...
		if rt.is_true(var_link_config.array_get('is_external')) {
			...
			mut var_external_icon := rt.new_string('<img src="' +
				(rt.call_function('esc_url', [var_icon_url.dup()])).str() + '" alt="" />')
			...
			mut var_link_open := rt.new_string('<a href="' +
				(rt.call_function('esc_url', [var_link_url.dup()])).str() +
			...
			var_link_open = rt.new_string('<a href="' +
				(rt.call_function('esc_url', [var_link_config.array_get('url')])).str() + '">')
			var_link_close = rt.new_string(rt.new_string('</a>'))
		}
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_current_tab.dup()]))
		// unsupported statement: Stmt_InlineHTML
		print(if rt.is_true(var_current_section) {
			' data-settings-section="' +
				(rt.call_function('esc_attr', [var_current_section.dup()])).str() + '"'
		} else {
			''
		})
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('wp_kses', [
			rt.call_function('sprintf', [var_link_config.array_get('message'),
				var_link_open.dup(), var_link_close.dup()]),
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [
		'woocommerce_after_settings_' + var_current_tab.str(),
	])
	// unsupported statement: Stmt_InlineHTML
}
