...
fn (mut this Class_WC_Report_Customer_List) output_report() {
	...
	if rt.is_true(rt.new_bool(!(!rt.is_true(rt.get_superglobal('_GET').array_get('link_orders')))
		&& rt.is_true(rt.call_function('wp_verify_nonce', [rt.get_superglobal('_REQUEST').array_get('_wpnonce'), rt.new_string('link_orders')]))))
	{
		mut var_linked := rt.call_function('wc_update_new_customer_past_orders', [
			rt.call_function('absint', [rt.get_superglobal('_GET').array_get('link_orders')]),
		])
		print('<div class="updated"><p>' +
			(rt.call_function('esc_html', [rt.call_function('sprintf', [rt.call_function('_n', [rt.new_string('%s previous order linked'), rt.new_string('%s previous orders linked'), var_linked.dup(), rt.new_string('woocommerce')]), var_linked.dup()])])).str() +
			'</p></div>')
	}
	if rt.is_true(rt.new_bool(!(!rt.is_true(rt.get_superglobal('_GET').array_get('refresh')))
		&& rt.is_true(rt.call_function('wp_verify_nonce', [rt.get_superglobal('_REQUEST').array_get('_wpnonce'), rt.new_string('refresh')]))))
	{
		mut var_user_id := rt.call_function('absint',
			[rt.get_superglobal('_GET').array_get('refresh')])
		mut var_user := rt.call_function('get_user_by', [rt.new_string('id'),
			var_user_id.dup()])
		rt.call_function('delete_user_meta', [var_user_id.dup(),
...
fn (mut this Class_WC_Report_Customer_List) column_default(var_user rt.PhpVal, var_column_name rt.PhpVal) string {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('customer_name'))) {
		if rt.is_true(rt.new_bool(rt.is_true(rt.get_property(var_user_mutated, 'last_name'))
		...
		if !(!rt.is_true(var_orders)) {
			mut var_order := var_orders.array_get(0)
			return '<a href="' +
				(rt.call_function('admin_url', ['post.php?post=' + (rt.call_method(var_order, 'get_id', []rt.PhpVal{})).str() +
				'&action=edit'])).str() + '">' +
		...
		var_actions.array_set('edit', rt.create_array([
			rt.ArrayItem{ key: 'url', val: rt.call_function('admin_url', [
				'user-edit.php?user_id=' + (rt.get_property(var_user_mutated, 'ID')).str(),
			]) },
			rt.ArrayItem{ key: 'name', val: rt.call_function('__', [
				rt.new_string('Edit'),
				rt.new_string('woocommerce'),
		...
		var_actions.array_set('view', rt.create_array([
			rt.ArrayItem{ key: 'url', val: rt.call_function('admin_url', [
				'edit.php?post_type=shop_order&_customer_user=' +
		...
		])
		{
			...
			for {
				...
				rt.call_function('printf', [
					...
					rt.call_function('esc_attr', [var_action.array_get('action')]),
					rt.call_function('esc_url', [var_action.array_get('url')]),
					rt.call_function('esc_attr', [var_action.array_get('name')]),
					rt.call_function('esc_attr', [var_action.array_get('name')]),
				])
			}
		}
...
fn (mut this Class_WC_Report_Customer_List) order_by_last_name(var_query rt.PhpVal) rt.PhpVal {
	...
	mut var_s := if !(!rt.is_true(rt.get_superglobal('_REQUEST').array_get('s'))) { rt.call_function('wp_unslash', [
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_reports_class_wc_report_customer_list_php() {
