...
fn (mut this Class_WC_Report_Most_Stocked) get_items(var_current_page rt.PhpVal, var_per_page rt.PhpVal)  {
	...
	mut var_stock := rt.call_function('absint', [rt.call_function('max', [rt.call_function('get_option', [rt.new_string('woocommerce_notify_low_stock_amount')]), rt.new_int(0)])])
	mut var_query_from := rt.call_function('apply_filters', [rt.new_string('woocommerce_report_most_stocked_query_from'), rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('\n\t\t\t\tFROM '), rt.get_property(var_wpdb, 'posts')), rt.new_string(' as posts\n\t\t\t\tINNER JOIN ')), rt.get_property(var_wpdb, 'wc_product_meta_lookup')), rt.new_string(' AS lookup ON posts.ID = lookup.product_id\n\t\t\t\tWHERE 1=1\n\t\t\t\tAND posts.post_type IN ( \'product\', \'product_variation\' )\n\t\t\t\tAND posts.post_status = \'publish\'\n\t\t\t\tAND lookup.stock_quantity > %d\n\t\t\t\t')), var_stock.dup()])])
	this.dispatch_set_prop('items', rt.call_method(var_wpdb, 'get_results', [rt.call_method(var_wpdb, 'prepare', [rt.new_string("SELECT SQL_CALC_FOUND_ROWS posts.ID as id, posts.post_parent as parent ${var_query_from.to_string()} ORDER BY lookup.stock_quantity DESC, id ASC LIMIT %d, %d;"), rt.mul(rt.sub(var_current_page, rt.new_int(1)), var_per_page), var_per_page.dup()])]))
...
fn create_wc_report_most_stocked() &Class_WC_Report_Most_Stocked {
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_reports_class_wc_report_most_stocked_php() {
