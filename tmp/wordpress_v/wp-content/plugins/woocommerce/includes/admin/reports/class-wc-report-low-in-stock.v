...
struct Class_WC_Report_Low_In_Stock {
	rt.PhpObjectBase
}

fn (mut this Class_WC_Report_Low_In_Stock) no_items()  {
	rt.call_function('esc_html_e', [rt.new_string('No low in stock products found.'), rt.new_string('woocommerce')])
}

fn (mut this Class_WC_Report_Low_In_Stock) get_items(var_current_page rt.PhpVal, var_per_page rt.PhpVal)  {
	mut var_wpdb := rt.new_null()
	// unsupported statement: Stmt_Global
	this.dispatch_set_prop('max_items', rt.new_int(0))
	this.dispatch_set_prop('items', rt.new_array())
	mut var_stock := rt.call_function('absint', [rt.call_function('max', [rt.call_function('get_option', [rt.new_string('woocommerce_notify_low_stock_amount')]), rt.new_int(1)])])
	mut var_nostock := rt.call_function('absint', [rt.call_function('max', [rt.call_function('get_option', [rt.new_string('woocommerce_notify_no_stock_amount')]), rt.new_int(0)])])
	mut var_query_from := rt.call_function('apply_filters', [rt.new_string('woocommerce_report_low_in_stock_query_from'), rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('\n\t\t\t\tFROM '), rt.get_property(var_wpdb, 'posts')), rt.new_string(' as posts\n\t\t\t\tINNER JOIN ')), rt.get_property(var_wpdb, 'wc_product_meta_lookup')), rt.new_string(' AS lookup ON posts.ID = lookup.product_id\n\t\t\t\tWHERE 1=1\n\t\t\t\tAND posts.post_type IN ( \'product\', \'product_variation\' )\n\t\t\t\tAND posts.post_status = \'publish\'\n\t\t\t\tAND lookup.stock_quantity <= %d\n\t\t\t\tAND lookup.stock_quantity > %d\n\t\t\t\t')), var_stock.dup(), var_nostock.dup()])])
...
fn create_wc_report_low_in_stock() &Class_WC_Report_Low_In_Stock {
	mut obj := &Class_WC_Report_Low_In_Stock{
...
fn (mut this Class_WC_Report_Low_In_Stock) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'no_items' {
			this.no_items()
			return rt.new_null()
...
fn (this &Class_WC_Report_Low_In_Stock) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_WC_Report_Low_In_Stock) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_reports_class_wc_report_low_in_stock_php() {
