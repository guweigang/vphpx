...
fn (mut this Class_WC_Report_Sales_By_Product) construct() {
	if rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(rt.new_string('product_ids'))
		&& rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_get('product_ids').is_array()))))
	{
		this.product_ids = rt.call_function('array_filter', [
			rt.call_function('array_map', [rt.new_string('absint'),
				rt.get_superglobal('_GET').array_get('product_ids')]),
		])
...
fn (mut this Class_WC_Report_Sales_By_Product) get_chart_legend() rt.PhpVal {
	...
	var_legend << rt.create_array([
		rt.ArrayItem{ key: 'title', val: rt.call_function('sprintf', [
			...
			'<strong>' + (rt.call_function('wc_price', [var_total_sales.dup()])).str() + '</strong>',
		]) },
		rt.ArrayItem{ key: 'color', val: this.chart_colours.array_get('sales_amount') },
	...
	var_legend << rt.create_array([
		rt.ArrayItem{ key: 'title', val: rt.call_function('sprintf', [
			rt.call_function('__', [rt.new_string('%s purchases for the selected items'),
				rt.new_string('woocommerce')]),
			'<strong>' + var_total_items.str() + '</strong>',
		]) },
		rt.ArrayItem{ key: 'color', val: this.chart_colours.array_get('item_count') },
...
fn (mut this Class_WC_Report_Sales_By_Product) output_report() {
	...
	mut var_current_range := if !(!rt.is_true(rt.get_superglobal('_GET').array_get('range'))) { rt.call_function('sanitize_text_field', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('range')]),
		]) } else { rt.new_string('7day') }
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_current_range.dup(),
		rt.create_array([
	...
	}
	{
		...
		for {
			...
			if rt.is_true(var_product) {
				this.product_ids_titles.array_push(rt.call_method(var_product,
...
fn (mut this Class_WC_Report_Sales_By_Product) products_widget() {
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(if !(!rt.is_true(rt.get_superglobal('_GET').array_get('range'))) { rt.call_function('esc_attr', [
			rt.get_superglobal('_GET').array_get('range'),
		]) } else { rt.new_string('') })
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(if !(!rt.is_true(rt.get_superglobal('_GET').array_get('start_date'))) { rt.call_function('esc_attr', [
			rt.get_superglobal('_GET').array_get('start_date'),
		]) } else { rt.new_string('') })
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(if !(!rt.is_true(rt.get_superglobal('_GET').array_get('end_date'))) { rt.call_function('esc_attr', [
			rt.get_superglobal('_GET').array_get('end_date'),
		]) } else { rt.new_string('') })
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(if !(!rt.is_true(rt.get_superglobal('_GET').array_get('page'))) { rt.call_function('esc_attr', [
			rt.get_superglobal('_GET').array_get('page'),
		]) } else { rt.new_string('') })
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(if !(!rt.is_true(rt.get_superglobal('_GET').array_get('tab'))) { rt.call_function('esc_attr', [
			rt.get_superglobal('_GET').array_get('tab'),
		]) } else { rt.new_string('') })
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(if !(!rt.is_true(rt.get_superglobal('_GET').array_get('report'))) { rt.call_function('esc_attr', [
			rt.get_superglobal('_GET').array_get('report'),
		]) } else { rt.new_string('') })
	// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(var_top_sellers) {
		{
			mut iter_1 := var_top_sellers.iterator()
			for {
				item_1 := iter_1.next() or { break }
	...
	if rt.is_true(var_top_freebies) {
		{
			mut iter_1 := var_top_freebies.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_product := item_1.val
				print('<tr class="' +
	...
	if rt.is_true(var_top_earners) {
		{
			mut iter_1 := var_top_earners.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_product := item_1.val
				print('<tr class="' +
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_reports_class_wc_report_sales_by_product_php() {
