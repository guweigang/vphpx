...
fn (mut this Class_WC_Report_Customers) get_chart_legend() rt.PhpVal {
	...
	var_legend << rt.create_array([
		rt.ArrayItem{ key: 'title', val: rt.call_function('sprintf', [
			...
			'<strong>' + this.customers.array_count().str() + '</strong>',
		]) },
		rt.ArrayItem{ key: 'color', val: this.chart_colours.array_get('signups') },
...
fn (mut this Class_WC_Report_Customers) customers_vs_guests() {
	...
	rt.echo_val(rt.call_function('esc_attr', [this.chart_colours.array_get('customers')]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Customer sales'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [this.chart_colours.array_get('guests')]))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [this.chart_colours.array_get('customers')]))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [this.chart_colours.array_get('guests')]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [
		' ' +
	...
	this.customers = var_users_query.get_results()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.less(rt.call_function('strtotime', [rt.get_property(var_customer, 'user_registered')]), rt.get_property(rt.new_object('WC_Report_Customers', ['WC_Admin_Report'], &this), 'start_date')))
...
fn (mut this Class_WC_Report_Customers) get_main_chart() {
	...
	var_customer_orders = this.prepare_chart_data(var_customer_orders.dup(),
	...
	var_guest_orders = this.prepare_chart_data(var_guest_orders.dup(), rt.new_string('post_date'),
		rt.new_string('total_orders'), rt.get_property(rt.new_object('WC_Report_Customers', [
		'WC_Admin_Report',
	], &this), 'chart_interval'), rt.get_property(rt.new_object('WC_Report_Customers', [
		'WC_Admin_Report',
	], &this), 'start_date'), rt.get_property(rt.new_object('WC_Report_Customers', [
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [
		' ' +(rt.call_function('__', [rt.new_string('customer orders'), rt.new_string('woocommerce')])).str(),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_js', [
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [
		' ' +(rt.call_function('__', [rt.new_string('guest orders'), rt.new_string('woocommerce')])).str(),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_js', [
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [this.chart_colours.array_get('signups')]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [
		' ' +(rt.call_function('__', [rt.new_string('new users'), rt.new_string('woocommerce')])).str(),
	]))
	// unsupported statement: Stmt_InlineHTML
	...
	rt.echo_val(rt.call_function('rawurlencode', [
		rt.call_function('wp_json_encode', [rt.call_function('array_values', [])]),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', []))
	// unsupported statement: Stmt_InlineHTML
...
fn create_wc_report_customers() &Class_WC_Report_Customers {
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_reports_class_wc_report_customers_php() {
