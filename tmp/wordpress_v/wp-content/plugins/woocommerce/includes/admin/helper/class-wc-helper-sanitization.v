...
fn Class_WC_Helper_Sanitization.sanitize_css(var_css rt.PhpVal) string {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_css_mutated.dup().is_string()))))) {
		return ''
	}
	...
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_url := var_matches.array_get(2)
		mut var_quote := var_matches.array_get(1)
		if rt.is_true(rt.call_function('preg_match', [
			'/^https?:\\/\\/(([\\w-]+\\.)*woocommerce\\.com|' + '([\\w-]+\\.)*woocommerce\\.test|' +
				'([\\w-]+\\.)*WordPress\\.com|' + '([\\w-]+\\.)*wp\\.com)/ix',
			var_url.dup(),
		]))
		{
	...
	var_css_mutated = rt.call_function('str_replace', [rt.new_string('*'),
		rt.new_string('__PRESERVED_ASTERISK__'), var_css_mutated.dup()])
	...
	var_css_mutated = rt.call_function('substr', [var_css_mutated.dup(),
...
fn Class_WC_Helper_Sanitization.sanitize_html(var_html rt.PhpVal) rt.PhpVal {
	...
	var_allowed_html = rt.call_function('array_merge', [var_allowed_html.dup(),
		var_svg_tags.dup()])
	return rt.call_function('wp_kses', [
		Class_WC_Helper_Sanitization.wc_pre_sanitize_svg(var_html.dup()),
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_helper_class_wc_helper_sanitization_php() {
