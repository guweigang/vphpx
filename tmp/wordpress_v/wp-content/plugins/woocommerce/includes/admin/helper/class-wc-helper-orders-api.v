...
fn Class_WC_Helper_Orders_API.register_rest_routes() {
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_argument := if args.len > 0 { args[0].dup() } else { rt.new_null() }
		return rt.new_bool(var_argument.dup().is_long())
	}
...
fn Class_WC_Helper_Orders_API.get_permission() rt.PhpVal {
	return fn () rt.PhpVal {
		mut temp := Class_WC_Helper_Subscriptions_API{}
		return temp.get_permission()
	}()
}
...
fn Class_WC_Helper_Orders_API.create_order(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_response := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp := Class_WC_Helper_API{}
		return temp.post(arg_0, arg_1)
	}(rt.new_string('create-order'), rt.create_array([
		rt.ArrayItem{ key: 'authenticated', val: true },
		rt.ArrayItem{ key: 'body', val: rt.call_function('http_build_query', [
			rt.create_array([
				rt.ArrayItem{ key: 'product_id', val: var_request.array_get('product_id') },
			]),
		]) },
	]))
	if rt.has_exception() {
	...
	return create_wp_rest_response(rt.call_function('json_decode', [
		rt.call_function('wp_remote_retrieve_body', [var_response.dup()]),
		rt.new_bool(true),
...
fn create_wc_helper_orders_api() &Class_WC_Helper_Orders_API {
...
pub fn init_wp_content_plugins_woocommerce_includes_admin_helper_class_wc_helper_orders_api_php() {
