...
fn (mut this Class_WC_Shipping_Rate) magic_set(var_key rt.PhpVal, var_value rt.PhpVal)  {
	if rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this) }, rt.ArrayItem{ key: none, val: "set_${var_key.to_string()}" }])])) {
		rt.call_method(rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this), "set_${var_key.to_string()}", [var_value.dup()])
	} else {
		this.data.array_set(var_key, var_value.dup())
	}
}
...
fn (mut this Class_WC_Shipping_Rate) set_id(var_id rt.PhpVal)  {
	this.data.array_set('id', // unsupported expression: Expr_Cast_String)
}

fn (mut this Class_WC_Shipping_Rate) set_method_id(var_method_id rt.PhpVal)  {
	this.data.array_set('method_id', // unsupported expression: Expr_Cast_String)
}

fn (mut this Class_WC_Shipping_Rate) set_instance_id(var_instance_id rt.PhpVal)  {
	this.data.array_set('instance_id', rt.call_function('absint', [var_instance_id.dup()]))
}

fn (mut this Class_WC_Shipping_Rate) set_label(var_label rt.PhpVal)  {
	this.data.array_set('label', // unsupported expression: Expr_Cast_String)
}

fn (mut this Class_WC_Shipping_Rate) set_cost(var_cost rt.PhpVal)  {
	this.data.array_set('cost', var_cost.dup())
}

fn (mut this Class_WC_Shipping_Rate) set_taxes(var_taxes rt.PhpVal)  {
	mut var_taxes_mutated := var_taxes
	this.data.array_set('taxes', if rt.is_true(rt.new_bool(!(!rt.is_true(var_taxes_mutated)) && rt.is_true(rt.new_bool(var_taxes_mutated.dup().is_array())))) { var_taxes_mutated } else { rt.new_array() })
}
...
fn (mut this Class_WC_Shipping_Rate) set_tax_status(var_value rt.PhpVal)  {
	if rt.is_true(rt.call_function('in_array', [var_value.dup(), rt.create_array([rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Enums_ProductTaxStatus.taxable() }, rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Enums_ProductTaxStatus.none() }]), rt.new_bool(true)])) {
		this.data.array_set('tax_status', var_value.dup())
	}
}

fn (mut this Class_WC_Shipping_Rate) set_description(var_description rt.PhpVal)  {
	this.data.array_set('description', // unsupported expression: Expr_Cast_String)
}

fn (mut this Class_WC_Shipping_Rate) set_delivery_time(var_delivery_time rt.PhpVal)  {
	this.data.array_set('delivery_time', // unsupported expression: Expr_Cast_String)
}

fn (mut this Class_WC_Shipping_Rate) get_id() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_shipping_rate_id'), this.data.array_get('id'), rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this)])
}

fn (mut this Class_WC_Shipping_Rate) get_method_id() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_shipping_rate_method_id'), this.data.array_get('method_id'), rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this)])
}

fn (mut this Class_WC_Shipping_Rate) get_instance_id() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_shipping_rate_instance_id'), this.data.array_get('instance_id'), rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this)])
}

fn (mut this Class_WC_Shipping_Rate) get_label() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_shipping_rate_label'), this.data.array_get('label'), rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this)])
}

fn (mut this Class_WC_Shipping_Rate) get_cost() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_shipping_rate_cost'), this.data.array_get('cost'), rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this)])
}

fn (mut this Class_WC_Shipping_Rate) get_taxes() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_shipping_rate_taxes'), this.data.array_get('taxes'), rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this)])
}

fn (mut this Class_WC_Shipping_Rate) get_shipping_tax() rt.PhpVal {
	mut var_taxes := this.get_taxes()
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_get_shipping_tax'), if rt.is_true(rt.new_bool(var_taxes.dup().array_count() > 0 && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'customer'), 'get_is_vat_exempt', []rt.PhpVal{}))))))) { // unsupported expression: Expr_Cast_Double } else { rt.new_float(0) }, rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this)])
}

fn (mut this Class_WC_Shipping_Rate) get_tax_status() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_shipping_rate_tax_status'), this.data.array_get('tax_status'), rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this)])
}

fn (mut this Class_WC_Shipping_Rate) get_description() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_shipping_rate_description'), this.data.array_get('description'), rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this)])
}

fn (mut this Class_WC_Shipping_Rate) get_delivery_time() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_shipping_rate_delivery_time'), this.data.array_get('delivery_time'), rt.new_object('WC_Shipping_Rate', ['JsonSerializable'], &this)])
}

fn (mut this Class_WC_Shipping_Rate) add_meta_data(var_key rt.PhpVal, var_value rt.PhpVal)  {
	this.meta_data.array_set(rt.call_function('wc_clean', [var_key.dup()]), rt.call_function('wc_clean', [var_value.dup()]))
}
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_shipping_rate_php() {
