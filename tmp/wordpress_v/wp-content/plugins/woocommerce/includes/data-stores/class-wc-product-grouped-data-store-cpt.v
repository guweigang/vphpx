...
fn (mut this Class_WC_Product_Grouped_Data_Store_CPT) update_post_meta(var_product rt.PhpVal, force bool) {
	...
	mut var_props_to_update := if var_force {
		...
		this.get_props_to_update(var_product.dup(), var_meta_key_to_props.dup())
	}
	{
	...
	this.Class_WC_Product_Data_Store_CPT.update_post_meta(var_product.dup(), rt.new_bool(force))
}
...
fn (mut this Class_WC_Product_Grouped_Data_Store_CPT) handle_updated_props(var_product rt.PhpVal) {
	if rt.is_true(rt.call_function('in_array', [rt.new_string('children'),
		...
		rt.new_bool(true)]))
	{
		this.update_prices_from_children(var_product.dup())
	}
	this.Class_WC_Product_Data_Store_CPT.handle_updated_props(var_product.dup())
}
...
fn (mut this Class_WC_Product_Grouped_Data_Store_CPT) update_prices_from_children(var_product rt.PhpVal) {
	...
	if !(!rt.is_true(var_child_ids)) {
		...
		var_child_prices = rt.call_function('array_filter', [
			var_child_prices.dup()])
	}
	rt.call_function('delete_post_meta', [var_product_id.dup(),
	...
	if !(!rt.is_true(var_child_prices)) {
		rt.call_function('add_post_meta', [var_product_id.dup(),
	...
	this.update_lookup_table(var_product_id.dup(), rt.new_string('wc_product_meta_lookup'))
...
fn create_wc_product_grouped_data_store_cpt() &Class_WC_Product_Grouped_Data_Store_CPT {
...
pub fn init_wp_content_plugins_woocommerce_includes_data_stores_class_wc_product_grouped_data_store_cpt_php() {
