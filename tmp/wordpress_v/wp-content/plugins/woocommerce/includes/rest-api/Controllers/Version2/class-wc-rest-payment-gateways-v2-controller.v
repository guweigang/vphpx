...
fn (mut this Class_WC_REST_Payment_Gateways_V2_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_data := rt.new_array()
	{
		...
		for {
			...
			rt.set_property(var_payment_gateway, 'id', var_payment_gateway_id.dup())
			mut var_gateway := this.prepare_item_for_response(var_payment_gateway.dup(), var_request.dup())
			var_gateway = this.prepare_response_for_collection(var_gateway.dup())
			var_data.array_push(var_gateway.dup())
		}
	}
...
fn (mut this Class_WC_REST_Payment_Gateways_V2_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	mut var_gateway := this.get_gateway(var_request.dup())
	if rt.is_true(rt.new_bool(var_gateway.dup().is_null())) {
		return create_wp_error(rt.new_string('woocommerce_rest_payment_gateway_invalid'), rt.call_function('__', [rt.new_string('Resource does not exist.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	var_gateway = this.prepare_item_for_response(var_gateway.dup(), var_request.dup())
	return rt.call_function('rest_ensure_response', [var_gateway.dup()])
}
...
fn (mut this Class_WC_REST_Payment_Gateways_V2_Controller) update_item(var_request rt.PhpVal) rt.PhpVal {
		...
		mut var_errors_found := rt.new_bool(rt.new_bool(false))
		{
			...
			for {
				...
				if var_request.array_get('settings').array_isset(var_key) {
					if rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Payment_Gateways_V2_Controller', ['WC_REST_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'validate_setting_' + (var_field.array_get('type')).str() + '_field' }])])) {
						mut var_value := rt.call_method(rt.new_object('WC_REST_Payment_Gateways_V2_Controller', ['WC_REST_Controller'], &this), 'validate_setting_' + (var_field.array_get('type')).str() + '_field', [var_request.array_get('settings').array_get(var_key), var_field.dup()])
					} else {
					...
					var_settings.array_set(var_key, var_value.dup())
				}
			}
		}
	...
	if var_request.array_isset(rt.new_string('enabled')) {
		var_settings.array_set('enabled', rt.call_function('wc_bool_to_string', [var_request.array_get('enabled')]))
		rt.set_property(var_gateway, 'enabled', var_settings.array_get('enabled'))
	}
	if var_request.array_isset(rt.new_string('title')) {
		var_settings.array_set('title', this.validate_setting_text_field(var_request.array_get('title'), if !(rt.get_property(var_gateway, 'form_fields').array_get('title')).is_null() { rt.get_property(var_gateway, 'form_fields').array_get('title') } else { rt.new_array() }))
		rt.set_property(var_gateway, 'title', var_settings.array_get('title'))
	}
	if var_request.array_isset(rt.new_string('description')) {
		var_settings.array_set('description', this.validate_setting_text_field(var_request.array_get('description'), if !(rt.get_property(var_gateway, 'form_fields').array_get('description')).is_null() { rt.get_property(var_gateway, 'form_fields').array_get('description') } else { rt.new_array() }))
		rt.set_property(var_gateway, 'description', var_settings.array_get('description'))
	}
	rt.set_property(var_gateway, 'settings', var_settings.dup())
	rt.call_function('update_option', [rt.call_method(var_gateway, 'get_option_key', []rt.PhpVal{}), rt.call_function('apply_filters', ['woocommerce_gateway_' + (rt.get_property(var_gateway, 'id')).str() + '_settings_values', var_settings.dup(), var_gateway.dup()])])
	if var_request.array_isset(rt.new_string('order')) {
		...
		var_order.array_set(rt.get_property(var_gateway, 'id'), rt.call_function('absint', [var_request.array_get('order')]))
		rt.call_function('update_option', [rt.new_string('woocommerce_gateway_order'), var_order.dup()])
		rt.set_property(var_gateway, 'order', rt.call_function('absint', [var_request.array_get('order')]))
	}
	var_gateway = this.prepare_item_for_response(var_gateway.dup(), var_request.dup())
	return rt.call_function('rest_ensure_response', [var_gateway.dup()])
}
...
fn (mut this Class_WC_REST_Payment_Gateways_V2_Controller) get_gateway(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_payment_gateways := rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'payment_gateways'), 'payment_gateways', []rt.PhpVal{})
	{
		...
		for {
			...
			rt.set_property(var_payment_gateway, 'id', var_payment_gateway_id.dup())
			var_gateway = var_payment_gateway.dup()
		}
	}
	return var_gateway.dup()
}
...
fn (mut this Class_WC_REST_Payment_Gateways_V2_Controller) prepare_item_for_response(var_gateway rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_order := rt.cast_array(rt.call_function('get_option', [rt.new_string('woocommerce_gateway_order')]))
	mut var_item := { 'id': rt.get_property(var_gateway_mutated, 'id'), 'title': rt.get_property(var_gateway_mutated, 'title'), 'description': rt.get_property(var_gateway_mutated, 'description'), 'order': if var_order.array_isset(rt.get_property(var_gateway_mutated, 'id')) { var_order.array_get(rt.get_property(var_gateway_mutated, 'id')) } else { rt.new_string('') }, 'enabled': rt.identical(rt.new_string('yes'), rt.get_property(var_gateway_mutated, 'enabled')), 'method_title': rt.call_method(var_gateway_mutated, 'get_method_title', []rt.PhpVal{}), 'method_description': rt.call_method(var_gateway_mutated, 'get_method_description', []rt.PhpVal{}), 'settings': this.get_settings(var_gateway_mutated.dup()) }
	...
	mut var_response := rt.call_function('rest_ensure_response', [var_data.dup()])
	rt.call_method(var_response, 'add_links', [this.prepare_links(var_gateway_mutated.dup(), var_request.dup())])
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_prepare_payment_gateway'), var_response.dup(), var_gateway_mutated.dup(), var_request.dup()])
}
...
fn (mut this Class_WC_REST_Payment_Gateways_V2_Controller) get_settings(var_gateway rt.PhpVal) rt.PhpVal {
	...
	{
		mut iter_1 := rt.get_property(var_gateway_mutated, 'form_fields').iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_field := item_1.val
			...
			if rt.is_true(rt.identical(rt.new_string('title'), var_field.array_get('type'))) {
				continue
			}
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version2_class_wc_rest_payment_gateways_v2_controller_php() {
