...
fn (mut this Class_WC_REST_Customers_V2_Controller) get_formatted_item_data(var_object rt.PhpVal) rt.PhpVal {
	mut var_formatted_data := this.get_formatted_item_data_core(var_object.dup())
	var_formatted_data.array_set('orders_count', rt.call_method(var_object, 'get_order_count', []rt.PhpVal{}))
	var_formatted_data.array_set('total_spent', rt.call_method(var_object, 'get_total_spent', []rt.PhpVal{}))
	return var_formatted_data.dup()
}
...
fn (mut this Class_WC_REST_Customers_V2_Controller) get_formatted_item_data_core(var_object rt.PhpVal) rt.PhpVal {
	...
	for var_key in var_format_date {
		mut var_datetime := if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('date_created'), rt.new_string(key))) && rt.is_true(rt.call_function('is_subclass_of', [var_data.array_get(key), rt.new_string('DateTime')])))) { rt.call_function('get_date_from_gmt', [rt.call_function('gmdate', [rt.new_string('Y-m-d H:i:s'), rt.call_method(var_data.array_get(key), 'getTimestamp', []rt.PhpVal{})])]) } else { var_data.array_get(key) }
	...
	mut var_formatted_data := rt.create_array([rt.ArrayItem{ key: 'id', val: rt.call_method(var_object, 'get_id', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'date_created', val: var_data.array_get('date_created') }, rt.ArrayItem{ key: 'date_created_gmt', val: var_data.array_get('date_created_gmt') }, rt.ArrayItem{ key: 'date_modified', val: var_data.array_get('date_modified') }, rt.ArrayItem{ key: 'date_modified_gmt', val: var_data.array_get('date_modified_gmt') }, rt.ArrayItem{ key: 'email', val: var_data.array_get('email') }, rt.ArrayItem{ key: 'first_name', val: var_data.array_get('first_name') }, rt.ArrayItem{ key: 'last_name', val: var_data.array_get('last_name') }, rt.ArrayItem{ key: 'role', val: var_data.array_get('role') }, rt.ArrayItem{ key: 'username', val: var_data.array_get('username') }, rt.ArrayItem{ key: 'billing', val: var_data.array_get('billing') }, rt.ArrayItem{ key: 'shipping', val: var_data.array_get('shipping') }, rt.ArrayItem{ key: 'is_paying_customer', val: var_data.array_get('is_paying_customer') }, rt.ArrayItem{ key: 'avatar_url', val: rt.call_method(var_object, 'get_avatar_url', []rt.PhpVal{}) }])
	if rt.is_true(rt.call_function('wc_current_user_has_role', [rt.new_string('administrator')])) {
		closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_meta := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.new_bool(!(rt.is_true(rt.call_function('is_protected_meta', [rt.get_property(var_meta, 'key'), rt.new_string('user')]))))
	}
	mut var_meta := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.new_bool(!(rt.is_true(rt.call_function('is_protected_meta', [rt.get_property(var_meta, 'key'), rt.new_string('user')]))))
	}
		var_formatted_data.array_set('meta_data', rt.call_function('array_values', [rt.call_function('array_filter', [var_data.array_get('meta_data'), rt.new_closure(closure_1_fn)])]))
	}
	return var_formatted_data.dup()
}
...
fn (mut this Class_WC_REST_Customers_V2_Controller) prepare_item_for_response(var_user_data rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_data := this.get_formatted_item_data(rt.new_object('WC_Customer', []string{}, var_customer))
	...
	rt.call_method(var_response, 'add_links', [this.prepare_links(var_user_data.dup())])
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_prepare_customer'), var_response.dup(), var_user_data.dup(), var_request.dup()])
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version2_class_wc_rest_customers_v2_controller_php() {
