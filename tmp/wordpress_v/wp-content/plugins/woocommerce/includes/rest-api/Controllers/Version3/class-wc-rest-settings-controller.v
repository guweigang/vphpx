...
fn (mut this Class_WC_REST_Settings_Controller) register_routes()  {
	...
	rt.call_function('register_rest_route', [this.namespace, '/' + (rt.get_property(rt.new_object('WC_REST_Settings_Controller', ['WC_REST_Settings_V2_Controller'], &this), 'rest_base')).str() + '/batch', rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.editable() }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Settings_Controller', ['WC_REST_Settings_V2_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'batch_items' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Settings_Controller', ['WC_REST_Settings_V2_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'update_items_permissions_check' }]) }]) }, rt.ArrayItem{ key: 'schema', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Settings_Controller', ['WC_REST_Settings_V2_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_public_batch_schema' }]) }])])
}
...
fn (mut this Class_WC_REST_Settings_Controller) update_item(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_response := var_options_controller.update_item(var_request.dup())
	return var_response.dup()
}

fn (mut this Class_WC_REST_Settings_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := { '$schema': rt.new_string('http://json-schema.org/draft-04/schema#'), 'title': rt.new_string('setting_group'), 'type': rt.new_string('object'), 'properties': { 'id': { 'description': rt.call_function('__', [rt.new_string('A unique identifier that can be used to link settings together.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{} }, 'label': { 'description': rt.call_function('__', [rt.new_string('A human readable label for the setting used in interfaces.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{} }, 'description': { 'description': rt.call_function('__', [rt.new_string('A human readable description for the setting used in interfaces.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{} }, 'parent_id': { 'description': rt.call_function('__', [rt.new_string('ID of parent grouping.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{} }, 'sub_groups': { 'description': rt.call_function('__', [rt.new_string('IDs for settings sub groups.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{} } } }
...
fn create_wc_rest_settings_controller() &Class_WC_REST_Settings_Controller {
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version3_class_wc_rest_settings_controller_php() {
