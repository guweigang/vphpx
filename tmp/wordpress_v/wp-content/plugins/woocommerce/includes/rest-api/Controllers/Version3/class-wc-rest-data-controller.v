...
fn (mut this Class_WC_REST_Data_Controller) register_routes()  {
	rt.call_function('register_rest_route', [this.namespace, '/' + (this.rest_base).str(), rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.readable() }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Data_Controller', ['WC_REST_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_items' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Data_Controller', ['WC_REST_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_items_permissions_check' }]) }]) }, rt.ArrayItem{ key: 'schema', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Data_Controller', ['WC_REST_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_public_item_schema' }]) }])])
}
...
fn (mut this Class_WC_REST_Data_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_resources := [[rt.new_string('continents'), rt.call_function('__', [rt.new_string('List of supported continents, countries, and states.'), rt.new_string('woocommerce')])], [rt.new_string('countries'), rt.call_function('__', [rt.new_string('List of supported states in a given country.'), rt.new_string('woocommerce')])], [rt.new_string('currencies'), rt.call_function('__', [rt.new_string('List of supported currencies.'), rt.new_string('woocommerce')])]]
...
fn (mut this Class_WC_REST_Data_Controller) prepare_item_for_response(var_resource rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_data := rt.create_array([rt.ArrayItem{ key: 'slug', val: rt.get_property(var_resource, 'slug') }, rt.ArrayItem{ key: 'description', val: rt.get_property(var_resource, 'description') }])
	...
	rt.call_method(var_response, 'add_links', [this.prepare_links(var_resource.dup())])
	return var_response.dup()
}

fn (mut this Class_WC_REST_Data_Controller) prepare_links(var_item rt.PhpVal) rt.PhpVal {
	mut var_item_mutated := var_item
	mut var_links := { 'self': { 'href': rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s/%s'), this.namespace, this.rest_base, rt.get_property(var_item_mutated, 'slug')])]) }, 'collection': { 'href': rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('%s/%s'), this.namespace, this.rest_base])]) } }
	return var_links.dup()
}

fn (mut this Class_WC_REST_Data_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := { '$schema': rt.new_string('http://json-schema.org/draft-04/schema#'), 'title': rt.new_string('data_index'), 'type': rt.new_string('object'), 'properties': { 'slug': { 'description': rt.call_function('__', [rt.new_string('Data resource ID.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{}, 'readonly': rt.new_bool(true) }, 'description': { 'description': rt.call_function('__', [rt.new_string('Data resource description.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{}, 'readonly': rt.new_bool(true) } } }
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version3_class_wc_rest_data_controller_php() {
