...
fn (mut this Class_WC_REST_Product_Reviews_V2_Controller) prepare_item_for_response(var_review rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_request_mutated := var_request
	mut var_data := rt.create_array([rt.ArrayItem{ key: 'id', val: // unsupported expression: Expr_Cast_Int }, rt.ArrayItem{ key: 'date_created', val: rt.call_function('wc_rest_prepare_date_response', [rt.get_property(var_review, 'comment_date')]) }, rt.ArrayItem{ key: 'date_created_gmt', val: rt.call_function('wc_rest_prepare_date_response', [rt.get_property(var_review, 'comment_date_gmt')]) }, rt.ArrayItem{ key: 'review', val: rt.get_property(var_review, 'comment_content') }, rt.ArrayItem{ key: 'rating', val: // unsupported expression: Expr_Cast_Int }, rt.ArrayItem{ key: 'name', val: rt.get_property(var_review, 'comment_author') }, rt.ArrayItem{ key: 'email', val: rt.get_property(var_review, 'comment_author_email') }, rt.ArrayItem{ key: 'verified', val: rt.call_function('wc_review_is_from_verified_owner', [rt.get_property(var_review, 'comment_ID')]) }])
	...
	var_data = this.add_additional_fields_to_object(var_data.dup(), var_request_mutated)
	...
	rt.call_method(var_response, 'add_links', [this.prepare_links(var_review.dup(), var_request_mutated)])
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_prepare_product_review'), var_response.dup(), var_review.dup(), var_request_mutated])
}
...
fn (mut this Class_WC_REST_Product_Reviews_V2_Controller) batch_items(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_product_id := var_params.array_get('product_id')
	mut var_body_params := rt.new_array()
	{
		mut iter_1 := rt.create_array([rt.ArrayItem{ key: none, val: 'update' }, rt.ArrayItem{ key: none, val: 'create' }, rt.ArrayItem{ key: none, val: 'delete' }]).iterator()
		for {
			...
			if !(!rt.is_true(var_items.array_get(var_batch_type))) {
				mut var_injected_items := rt.new_array()
				{
					...
					for {
						...
						var_injected_items << if rt.is_true(rt.new_bool(var_item.dup().is_array())) { rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: 'product_id', val: var_product_id }]), var_item.dup()]) } else { var_item }
					}
				}
				var_body_params.array_set(var_batch_type, var_injected_items.dup())
			}
		}
	}
	var_request_mutated = create_wp_rest_request(var_request_mutated.get_method())
	var_request_mutated.set_body_params(var_body_params.dup())
	return this.Class_WC_REST_Product_Reviews_V1_Controller.batch_items(var_request_mutated)
}

fn (mut this Class_WC_REST_Product_Reviews_V2_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := { '$schema': rt.new_string('http://json-schema.org/draft-04/schema#'), 'title': rt.new_string('product_review'), 'type': rt.new_string('object'), 'properties': { 'id': { 'description': rt.call_function('__', [rt.new_string('Unique identifier for the resource.'), rt.new_string('woocommerce')]), 'type': rt.new_string('integer'), 'context': map[string]rt.PhpVal{}, 'readonly': rt.new_bool(true) }, 'review': { 'description': rt.call_function('__', [rt.new_string('The content of the review.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{} }, 'date_created': { 'description': rt.call_function('__', [rt.new_string('The date the review was created, in the site\'s timezone.'), rt.new_string('woocommerce')]), 'type': rt.new_string('date-time'), 'context': map[string]rt.PhpVal{} }, 'date_created_gmt': { 'description': rt.call_function('__', [rt.new_string('The date the review was created, as GMT.'), rt.new_string('woocommerce')]), 'type': rt.new_string('date-time'), 'context': map[string]rt.PhpVal{} }, 'rating': { 'description': rt.call_function('__', [rt.new_string('Review rating (0 to 5).'), rt.new_string('woocommerce')]), 'type': rt.new_string('integer'), 'context': map[string]rt.PhpVal{} }, 'name': { 'description': rt.call_function('__', [rt.new_string('Reviewer name.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{} }, 'email': { 'description': rt.call_function('__', [rt.new_string('Reviewer email.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{} }, 'verified': { 'description': rt.call_function('__', [rt.new_string('Shows if the reviewer bought the product or not.'), rt.new_string('woocommerce')]), 'type': rt.new_string('boolean'), 'context': map[string]rt.PhpVal{}, 'readonly': rt.new_bool(true) } } }
...
fn create_wc_rest_product_reviews_v2_controller() &Class_WC_REST_Product_Reviews_V2_Controller {
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version2_class_wc_rest_product_reviews_v2_controller_php() {
