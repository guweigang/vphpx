...
fn (mut this Class_WC_REST_Order_Notes_V2_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_order)))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return create_wp_error(rt.concat(rt.concat(rt.new_string('woocommerce_rest_'), rt.get_property(rt.new_object('WC_REST_Order_Notes_V2_Controller', ['WC_REST_Order_Notes_V1_Controller'], &this), 'post_type')), rt.new_string('_invalid_id')), rt.call_function('__', [rt.new_string('Invalid order ID.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	...
	if rt.is_true(rt.identical(rt.new_string('customer'), var_request.array_get('type'))) {
		var_args['meta_query'] = rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'key', val: 'is_customer_note' }, rt.ArrayItem{ key: 'value', val: 1 }, rt.ArrayItem{ key: 'compare', val: '=' }]) }])
	} else if rt.is_true(rt.identical(rt.new_string('internal'), var_request.array_get('type'))) {
		var_args['meta_query'] = rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'key', val: 'is_customer_note' }, rt.ArrayItem{ key: 'compare', val: 'NOT EXISTS' }]) }])
	}
...
fn (mut this Class_WC_REST_Order_Notes_V2_Controller) prepare_item_for_response(var_note rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_data := rt.create_array([rt.ArrayItem{ key: 'id', val: // unsupported expression: Expr_Cast_Int }, rt.ArrayItem{ key: 'date_created', val: rt.call_function('wc_rest_prepare_date_response', [rt.get_property(var_note, 'comment_date')]) }, rt.ArrayItem{ key: 'date_created_gmt', val: rt.call_function('wc_rest_prepare_date_response', [rt.get_property(var_note, 'comment_date_gmt')]) }, rt.ArrayItem{ key: 'note', val: rt.get_property(var_note, 'comment_content') }, rt.ArrayItem{ key: 'customer_note', val: // unsupported expression: Expr_Cast_Bool }])
...
fn (mut this Class_WC_REST_Order_Notes_V2_Controller) get_collection_params() rt.PhpVal {
	...
	var_params['type'] = rt.create_array([rt.ArrayItem{ key: 'default', val: 'any' }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Limit result to customers or internal notes.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'enum', val: rt.create_array([rt.ArrayItem{ key: none, val: 'any' }, rt.ArrayItem{ key: none, val: 'customer' }, rt.ArrayItem{ key: none, val: 'internal' }]) }, rt.ArrayItem{ key: 'sanitize_callback', val: 'sanitize_key' }, rt.ArrayItem{ key: 'validate_callback', val: 'rest_validate_request_arg' }])
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version2_class_wc_rest_order_notes_v2_controller_php() {
