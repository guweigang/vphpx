...
fn (mut this Class_WC_REST_Data_Currencies_Controller) get_currency(var_code rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_currencies.dup().array_isset(var_code_mutated.dup())))))) {
		return rt.new_bool(false)
	}
	mut var_currency := rt.create_array([rt.ArrayItem{ key: 'code', val: var_code_mutated }, rt.ArrayItem{ key: 'name', val: var_currencies.array_get(var_code_mutated) }, rt.ArrayItem{ key: 'symbol', val: rt.call_function('get_woocommerce_currency_symbol', [var_code_mutated.dup()]) }])
	return var_currency.dup()
}
...
fn (mut this Class_WC_REST_Data_Currencies_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_data := rt.new_array()
	{
		mut iter_1 := rt.func_array_keys(var_currencies.dup()).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_code := item_1.val
			mut var_currency := this.get_currency(var_code.dup(), var_request.dup())
			mut var_response := this.prepare_item_for_response(var_currency.dup(), var_request.dup())
...
fn (mut this Class_WC_REST_Data_Currencies_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	mut var_data := this.get_currency(rt.new_string(var_request.array_get('currency').to_string().to_upper()), var_request.dup())
	if !rt.is_true(var_data) {
		return create_wp_error(rt.new_string('woocommerce_rest_data_invalid_currency'), rt.call_function('__', [rt.new_string('There are no currencies matching these parameters.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
...
fn (mut this Class_WC_REST_Data_Currencies_Controller) get_current_item(var_request rt.PhpVal) rt.PhpVal {
	...
	return this.prepare_item_for_response(this.get_currency(var_currency.dup(), var_request.dup()), var_request.dup())
}
...
fn (mut this Class_WC_REST_Data_Currencies_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_prepare_data_currency'), var_response.dup(), var_item.dup(), var_request.dup()])
}

fn (mut this Class_WC_REST_Data_Currencies_Controller) prepare_links(var_item rt.PhpVal) rt.PhpVal {
	mut var_code := rt.new_string(rt.new_string(var_item.array_get('code').to_string().to_upper()))
	mut var_links := { 'self': { 'href': rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s/%s'), this.namespace, this.rest_base, var_code.dup()])]) }, 'collection': { 'href': rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s'), this.namespace, this.rest_base])]) } }
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version3_class_wc_rest_data_currencies_controller_php() {
