...
fn (mut this Class_WC_REST_Report_Customers_Totals_Controller) get_reports() rt.PhpVal {
	...
	mut var_total_customers := rt.new_int(rt.new_int(0))
	{
		mut iter_1 := var_users_count.array_get('avail_roles').iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_total := item_1.val
			mut var_role := item_1.key
			if rt.is_true(rt.call_function('in_array', [var_role.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'administrator' }, rt.ArrayItem{ key: none, val: 'shop_manager' }]), rt.new_bool(true)])) {
				continue
			}
			// unsupported expression: Expr_AssignOp_Plus
		}
	}
	mut var_customers_query := create_wp_user_query(rt.create_array([rt.ArrayItem{ key: 'role__not_in', val: rt.create_array([rt.ArrayItem{ key: none, val: 'administrator' }, rt.ArrayItem{ key: none, val: 'shop_manager' }]) }, rt.ArrayItem{ key: 'number', val: 0 }, rt.ArrayItem{ key: 'fields', val: 'ID' }, rt.ArrayItem{ key: 'count_total', val: true }, rt.ArrayItem{ key: 'meta_query', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'key', val: 'paying_customer' }, rt.ArrayItem{ key: 'value', val: 1 }, rt.ArrayItem{ key: 'compare', val: '=' }]) }]) }]))
	mut var_total_paying := // unsupported expression: Expr_Cast_Int
	mut var_data := rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'slug', val: 'paying' }, rt.ArrayItem{ key: 'name', val: rt.call_function('__', [rt.new_string('Paying customer'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'total', val: var_total_paying }]) }, rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'slug', val: 'non_paying' }, rt.ArrayItem{ key: 'name', val: rt.call_function('__', [rt.new_string('Non-paying customer'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'total', val: rt.sub(var_total_customers, var_total_paying) }]) }])
	return var_data.dup()
}
...
fn (mut this Class_WC_REST_Report_Customers_Totals_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_prepare_report_customers_count'), var_response.dup(), var_report.dup(), var_request.dup()])
}

fn (mut this Class_WC_REST_Report_Customers_Totals_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := { '$schema': rt.new_string('http://json-schema.org/draft-04/schema#'), 'title': rt.new_string('report_customer_total'), 'type': rt.new_string('object'), 'properties': { 'slug': { 'description': rt.call_function('__', [rt.new_string('An alphanumeric identifier for the resource.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{}, 'readonly': rt.new_bool(true) }, 'name': { 'description': rt.call_function('__', [rt.new_string('Customer type name.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{}, 'readonly': rt.new_bool(true) }, 'total': { 'description': rt.call_function('__', [rt.new_string('Amount of customers.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{}, 'readonly': rt.new_bool(true) } } }
...
fn create_wc_rest_report_customers_totals_controller() &Class_WC_REST_Report_Customers_Totals_Controller {
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version3_class_wc_rest_report_customers_totals_controller_php() {
