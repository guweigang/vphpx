...
fn (mut this Class_WC_REST_Shipping_Zones_Controller_Base) get_zone(var_zone_id rt.PhpVal) rt.PhpVal {
	mut var_zone := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp := Class_WC_Shipping_Zones{}
		return temp.get_zone_by(arg_0, arg_1)
	}(rt.new_string('zone_id'), var_zone_id.dup())
	if rt.is_true(rt.identical(rt.new_bool(false), var_zone)) {
		return create_wp_error(rt.new_string('woocommerce_rest_shipping_zone_invalid'), rt.call_function('__', [
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version3_class_wc_rest_shipping_zones_controller_base_php() {
