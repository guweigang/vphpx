...
fn (mut this Class_WC_REST_Coupons_V1_Controller) query_args(var_args rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_coupon_code := if !(var_request_mutated.array_get('code')).is_null() { var_request_mutated.array_get('code') } else { rt.new_null() }
	if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Utilities_StringUtil{}; return temp.is_null_or_whitespace(arg_0) }(var_coupon_code.dup()))))) {
		...
		var_args_mutated.array_set('post__in', rt.create_array([rt.ArrayItem{ key: none, val: var_id }]))
	}
...
fn (mut this Class_WC_REST_Coupons_V1_Controller) prepare_item_for_response(var_post rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_request_mutated := var_request
	mut var_coupon := create_wc_coupon(// unsupported expression: Expr_Cast_Int)
	...
	for var_key in var_format_decimal {
		var__data.array_set(key, rt.call_function('wc_format_decimal', [var__data.array_get(key), rt.new_int(2)]))
	}
	for var_key in var_format_date {
		var__data.array_set(key, if rt.is_true(var__data.array_get(key)) { rt.call_function('wc_rest_prepare_date_response', [var__data.array_get(key), rt.new_bool(false)]) } else { rt.new_null() })
	}
	for var_key in var_format_date_utc {
		var__data.array_set(key, if rt.is_true(var__data.array_get(key)) { rt.call_function('wc_rest_prepare_date_response', [var__data.array_get(key)]) } else { rt.new_null() })
	}
	for var_key in var_format_null {
		var__data.array_set(key, if rt.is_true(var__data.array_get(key)) { var__data.array_get(key) } else { rt.new_null() })
	}
	mut var_data := rt.create_array([rt.ArrayItem{ key: 'id', val: var__data.array_get('id') }, rt.ArrayItem{ key: 'code', val: var__data.array_get('code') }, rt.ArrayItem{ key: 'date_created', val: var__data.array_get('date_created') }, rt.ArrayItem{ key: 'date_modified', val: var__data.array_get('date_modified') }, rt.ArrayItem{ key: 'discount_type', val: var__data.array_get('discount_type') }, rt.ArrayItem{ key: 'description', val: var__data.array_get('description') }, rt.ArrayItem{ key: 'amount', val: var__data.array_get('amount') }, rt.ArrayItem{ key: 'expiry_date', val: var__data.array_get('date_expires') }, rt.ArrayItem{ key: 'usage_count', val: var__data.array_get('usage_count') }, rt.ArrayItem{ key: 'individual_use', val: var__data.array_get('individual_use') }, rt.ArrayItem{ key: 'product_ids', val: var__data.array_get('product_ids') }, rt.ArrayItem{ key: 'exclude_product_ids', val: var__data.array_get('excluded_product_ids') }, rt.ArrayItem{ key: 'usage_limit', val: var__data.array_get('usage_limit') }, rt.ArrayItem{ key: 'usage_limit_per_user', val: var__data.array_get('usage_limit_per_user') }, rt.ArrayItem{ key: 'limit_usage_to_x_items', val: var__data.array_get('limit_usage_to_x_items') }, rt.ArrayItem{ key: 'free_shipping', val: var__data.array_get('free_shipping') }, rt.ArrayItem{ key: 'product_categories', val: var__data.array_get('product_categories') }, rt.ArrayItem{ key: 'excluded_product_categories', val: var__data.array_get('excluded_product_categories') }, rt.ArrayItem{ key: 'exclude_sale_items', val: var__data.array_get('exclude_sale_items') }, rt.ArrayItem{ key: 'minimum_amount', val: var__data.array_get('minimum_amount') }, rt.ArrayItem{ key: 'maximum_amount', val: var__data.array_get('maximum_amount') }, rt.ArrayItem{ key: 'email_restrictions', val: var__data.array_get('email_restrictions') }, rt.ArrayItem{ key: 'used_by', val: var__data.array_get('used_by') }])
	...
	rt.call_method(var_response, 'add_links', [this.prepare_links(var_post_mutated.dup(), var_request_mutated.dup())])
...
fn (mut this Class_WC_REST_Coupons_V1_Controller) prepare_item_for_database(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_data_keys := rt.func_array_keys(rt.call_function('array_filter', [var_schema.array_get('properties'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Coupons_V1_Controller', ['WC_REST_Posts_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'filter_writable_props' }])]))
	if rt.is_true(var_request_mutated.array_get('exclude_product_ids')) {
		var_request_mutated.array_set('excluded_product_ids', var_request_mutated.array_get('exclude_product_ids'))
	}
	if rt.is_true(var_request_mutated.array_get('expiry_date')) {
		var_request_mutated.array_set('date_expires', var_request_mutated.array_get('expiry_date'))
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('POST'), rt.call_method(var_request_mutated, 'get_method', []rt.PhpVal{}))) && rt.is_true(rt.identical(rt.new_int(0), rt.call_method(var_coupon, 'get_id', []rt.PhpVal{}))))) {
		if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Utilities_StringUtil{}; return temp.is_null_or_whitespace(arg_0) }(if !(var_request_mutated.array_get('code')).is_null() { var_request_mutated.array_get('code') } else { rt.new_null() })) {
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_value.dup().is_null()))))) {
				...
				if rt.is_true(rt.equal(switch_val_1, rt.new_string('code'))) {
					...
					var_id = if rt.is_true(rt.call_method(var_coupon, 'get_id', []rt.PhpVal{})) { rt.call_method(var_coupon, 'get_id', []rt.PhpVal{}) } else { rt.new_int(0) }
					...
					rt.call_method(var_coupon, 'set_code', [var_coupon_code.dup()])
				} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('description'))) {
					rt.call_method(var_coupon, 'set_description', [rt.call_function('wp_filter_post_kses', [var_value.dup()])])
				} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('expiry_date'))) {
					rt.call_method(var_coupon, 'set_date_expires', [var_value.dup()])
				} else {
	...
	return rt.call_function('apply_filters', [rt.concat(rt.new_string('woocommerce_rest_pre_insert_'), this.post_type), var_coupon.dup(), var_request_mutated.dup()])
}
...
fn (mut this Class_WC_REST_Coupons_V1_Controller) create_item(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_coupon_id := this.save_coupon(var_request_mutated.dup())
	if rt.is_true(rt.call_function('is_wp_error', [var_coupon_id.dup()])) {
		return var_coupon_id.dup()
	}
	mut var_post := rt.call_function('get_post', [var_coupon_id.dup()])
	this.update_additional_fields_for_object(var_post.dup(), var_request_mutated.dup())
	this.add_post_meta_fields(var_post.dup(), var_request_mutated.dup())
	rt.call_function('do_action', [rt.concat(rt.new_string('woocommerce_rest_insert_'), this.post_type), var_post.dup(), var_request_mutated.dup(), rt.new_bool(true)])
	rt.call_method(var_request_mutated, 'set_param', [rt.new_string('context'), rt.new_string('edit')])
	mut var_response := this.prepare_item_for_response(var_post.dup(), var_request_mutated.dup())
...
fn (mut this Class_WC_REST_Coupons_V1_Controller) update_item(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(var_post_id) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
	...
	this.update_additional_fields_for_object(var_post.dup(), var_request_mutated.dup())
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	rt.call_function('do_action', [rt.concat(rt.new_string('woocommerce_rest_insert_'), this.post_type), var_post.dup(), var_request_mutated.dup(), rt.new_bool(false)])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	rt.call_method(var_request_mutated, 'set_param', [rt.new_string('context'), rt.new_string('edit')])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_response := this.prepare_item_for_response(var_post.dup(), var_request_mutated.dup())
...
fn (mut this Class_WC_REST_Coupons_V1_Controller) save_coupon(var_request rt.PhpVal) rt.PhpVal {
	mut var_request_mutated := var_request
	mut var_coupon := this.prepare_item_for_database(var_request_mutated.dup())
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version1_class_wc_rest_coupons_v1_controller_php() {
