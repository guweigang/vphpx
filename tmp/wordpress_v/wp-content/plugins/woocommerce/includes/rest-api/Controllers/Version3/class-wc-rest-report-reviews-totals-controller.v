...
fn (mut this Class_WC_REST_Report_Reviews_Totals_Controller) get_reports() rt.PhpVal {
	...
	mut var_query_data := { 'count': rt.new_bool(true), 'post_type': rt.new_string('product'), 'meta_key': rt.new_string('rating'), 'meta_value': rt.new_string('') }
	{
		mut var_i := rt.new_int(rt.new_int(1))
		for {
			if !(rt.is_true(rt.less_equal(var_i, rt.new_int(5)))) { break }
			var_query_data['meta_value'] = var_i.dup()
			var_data.array_push(rt.create_array([rt.ArrayItem{ key: 'slug', val: 'rated_' + (var_i).str() + '_out_of_5' }, rt.ArrayItem{ key: 'name', val: rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Rated %s out of 5'), rt.new_string('woocommerce')]), var_i.dup()]) }, rt.ArrayItem{ key: 'total', val: // unsupported expression: Expr_Cast_Int }]))
			rt.post_inc(var_i)
		}
	}
	return var_data.dup()
...
fn (mut this Class_WC_REST_Report_Reviews_Totals_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_prepare_report_reviews_count'), var_response.dup(), var_report.dup(), var_request.dup()])
}

fn (mut this Class_WC_REST_Report_Reviews_Totals_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := { '$schema': rt.new_string('http://json-schema.org/draft-04/schema#'), 'title': rt.new_string('report_review_total'), 'type': rt.new_string('object'), 'properties': { 'slug': { 'description': rt.call_function('__', [rt.new_string('An alphanumeric identifier for the resource.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{}, 'readonly': rt.new_bool(true) }, 'name': { 'description': rt.call_function('__', [rt.new_string('Review type name.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{}, 'readonly': rt.new_bool(true) }, 'total': { 'description': rt.call_function('__', [rt.new_string('Amount of reviews.'), rt.new_string('woocommerce')]), 'type': rt.new_string('string'), 'context': map[string]rt.PhpVal{}, 'readonly': rt.new_bool(true) } } }
...
fn create_wc_rest_report_reviews_totals_controller() &Class_WC_REST_Report_Reviews_Totals_Controller {
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version3_class_wc_rest_report_reviews_totals_controller_php() {
