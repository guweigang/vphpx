...
fn (mut this Class_WC_REST_Setting_Options_V2_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_setting := this.get_setting(var_request_mutated.array_get('group_id'), var_request_mutated.array_get('id'))
	if rt.is_true(rt.call_function('is_wp_error', [var_setting.dup()])) {
		return var_setting.dup()
	}
	mut var_response := this.prepare_item_for_response(var_setting.dup(), var_request_mutated.dup())
...
fn (mut this Class_WC_REST_Setting_Options_V2_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	mut var_request_mutated := var_request
	mut var_settings := this.get_group_settings(var_request_mutated.array_get('group_id'))
	if rt.is_true(rt.call_function('is_wp_error', [var_settings.dup()])) {
	...
	mut var_data := rt.new_array()
	{
		...
		for {
			...
			mut var_setting := this.prepare_item_for_response(var_setting_obj.dup(), var_request_mutated.dup())
			var_setting = this.prepare_response_for_collection(var_setting.dup())
			if rt.is_true(this.is_setting_type_valid(var_setting.array_get('type'))) {
				var_data.array_push(var_setting.dup())
			}
		}
	}
...
fn (mut this Class_WC_REST_Setting_Options_V2_Controller) get_group_settings(var_group_id rt.PhpVal) rt.PhpVal {
	if !rt.is_true(var_group_id) {
		return create_wp_error(rt.new_string('rest_setting_setting_group_invalid'), rt.call_function('__', [rt.new_string('Invalid setting group.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	mut var_settings := rt.call_function('apply_filters', ['woocommerce_settings-' + (var_group_id).str(), rt.new_array()])
	...
	mut var_filtered_settings := rt.new_array()
	{
		...
		for {
			...
			mut var_option_key := var_setting.array_get('option_key')
			var_setting = this.filter_setting(var_setting.dup())
			mut var_default := if var_setting.array_isset(rt.new_string('default')) { var_setting.array_get('default') } else { rt.new_string('') }
			if rt.is_true(rt.call_function('in_array', [if !(var_setting.array_get('type')).is_null() { var_setting.array_get('type') } else { rt.new_string('') }, rt.create_array([rt.ArrayItem{ key: none, val: 'title' }, rt.ArrayItem{ key: none, val: 'sectionend' }]), rt.new_bool(true)])) {
				var_filtered_settings << var_setting.dup()
				continue
			}
			if rt.is_true(rt.new_bool(var_option_key.dup().is_array())) {
				mut var_option := rt.call_function('get_option', [var_option_key.array_get(0)])
				var_setting.array_set('value', if var_option.array_isset(var_option_key.array_get(1)) { var_option.array_get(var_option_key.array_get(1)) } else { var_default })
			} else {
				mut var_admin_setting_value := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Admin_Settings{}; return temp.get_option(arg_0, arg_1) }(var_option_key.dup(), var_default.dup())
				var_setting.array_set('value', var_admin_setting_value.dup())
			}
			if rt.is_true(rt.identical(rt.new_string('multi_select_countries'), var_setting.array_get('type'))) {
				var_setting.array_set('options', rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'get_countries', []rt.PhpVal{}))
				var_setting.array_set('type', 'multiselect')
			} else if rt.is_true(rt.identical(rt.new_string('single_select_country'), var_setting.array_get('type'))) {
	...
	return var_filtered_settings.dup()
}
...
fn (mut this Class_WC_REST_Setting_Options_V2_Controller) prime_options_cache_for_settings(mut var_settings Class_array)  {
	...
	mut var_prefetch := rt.new_array()
	{
		...
		for {
			...
			mut var_option_key := var_setting.array_get('option_key')
			if rt.is_true(rt.new_bool(var_option_key.dup().is_array())) {
				var_prefetch << var_option_key.array_get(0)
			} else if rt.is_true(rt.call_function('strstr', [var_option_key.dup(), rt.new_string('[')])) {
				rt.call_function('parse_str', [var_option_key.dup(), var_option_array.dup()])
				var_prefetch << rt.call_function('current', [rt.func_array_keys(var_option_array.dup())])
			} else {
				var_prefetch << var_option_key.dup()
			}
		}
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		rt.call_function('wp_prime_option_caches', [var_prefetch.dup()])
	}
}
...
fn (mut this Class_WC_REST_Setting_Options_V2_Controller) get_countries_and_states() rt.PhpVal {
	...
	mut var_output := rt.new_array()
	{
		...
		for {
			...
			mut var_states := rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'get_states', [var_key.dup()])
			if rt.is_true(var_states) {
				{
					mut iter_2 := var_states.iterator()
					for {
						item_2 := iter_2.next() or { break }
						mut var_state_value := item_2.val
						mut var_state_key := item_2.key
						var_output.array_set((var_key).str() + ':' + (var_state_key).str(), (var_value).str() + ' - ' + (var_state_value).str())
					}
				}
			} else {
				var_output.array_set(var_key, var_value.dup())
			}
		}
	}
...
fn (mut this Class_WC_REST_Setting_Options_V2_Controller) get_setting(var_group_id rt.PhpVal, var_setting_id rt.PhpVal) rt.PhpVal {
	if !rt.is_true(var_setting_id) {
		return create_wp_error(rt.new_string('rest_setting_setting_invalid'), rt.call_function('__', [rt.new_string('Invalid setting.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	mut var_settings := this.get_group_settings(var_group_id.dup())
	if rt.is_true(rt.call_function('is_wp_error', [var_settings.dup()])) {
		return var_settings.dup()
	}
	mut var_array_key := rt.func_array_keys(rt.call_function('wp_list_pluck', [var_settings.dup(), rt.new_string('id')]), var_setting_id.dup(), rt.new_bool(true))
	if !rt.is_true(var_array_key) {
		return create_wp_error(rt.new_string('rest_setting_setting_invalid'), rt.call_function('__', [rt.new_string('Invalid setting.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	mut var_setting := var_settings.array_get(var_array_key.array_get(0))
	if rt.is_true(rt.new_bool(!(rt.is_true(this.is_setting_type_valid(var_setting.array_get('type')))))) {
		return create_wp_error(rt.new_string('rest_setting_setting_invalid'), rt.call_function('__', [rt.new_string('Invalid setting.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	return var_setting.dup()
}
	...
	if !(!rt.is_true(var_items.array_get('update'))) {
		mut var_to_update := rt.new_array()
		{
			...
			for {
				...
				var_to_update << rt.call_function('array_merge', [rt.call_method(var_request_mutated, 'get_url_params', []rt.PhpVal{}), var_item.dup()])
			}
		}
		var_request_mutated = create_wp_rest_request(rt.call_method(var_request_mutated, 'get_method', []rt.PhpVal{}))
		rt.call_method(var_request_mutated, 'set_body_params', [rt.create_array([rt.ArrayItem{ key: 'update', val: var_to_update }])])
	}
	return this.Class_WC_REST_Controller.batch_items(var_request_mutated.dup())
}
...
fn (mut this Class_WC_REST_Setting_Options_V2_Controller) update_item(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Setting_Options_V2_Controller', ['WC_REST_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'validate_setting_' + (var_setting.array_get('type')).str() + '_field' }])])) {
		mut var_value := rt.call_method(rt.new_object('WC_REST_Setting_Options_V2_Controller', ['WC_REST_Controller'], &this), 'validate_setting_' + (var_setting.array_get('type')).str() + '_field', [var_request_mutated.array_get('value'), var_setting.dup()])
	} else {
		var_value = this.validate_setting_text_field(var_request_mutated.array_get('value'), var_setting.dup())
	}
	if rt.is_true(rt.call_function('is_wp_error', [var_value.dup()])) {
		return var_value.dup()
	}
	if rt.is_true(rt.new_bool(var_setting.array_get('option_key').is_array())) {
		var_setting.array_set('value', var_value.dup())
		mut var_option_key := var_setting.array_get('option_key')
		mut var_prev := if !(rt.call_function('get_option', [var_option_key.array_get(0), rt.new_null()])).is_null() { rt.call_function('get_option', [var_option_key.array_get(0), rt.new_null()]) } else { rt.new_array() }
		var_prev.array_set(var_option_key.array_get(1), var_request_mutated.array_get('value'))
		rt.call_function('update_option', [var_option_key.array_get(0), var_prev.dup()])
	} else {
		mut var_update_data := rt.new_array()
		var_update_data.array_set(var_setting.array_get('option_key'), var_value.dup())
		var_setting.array_set('value', var_value.dup())
		fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Admin_Settings{}; return temp.save_fields(arg_0, arg_1) }(rt.create_array([rt.ArrayItem{ key: none, val: var_setting }]), var_update_data.dup())
	}
	mut var_response := this.prepare_item_for_response(var_setting.dup(), var_request_mutated.dup())
	return rt.call_function('rest_ensure_response', [var_response.dup()])
}
...
fn (mut this Class_WC_REST_Setting_Options_V2_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_data := this.filter_setting(var_item.dup())
	var_data = this.add_additional_fields_to_object(var_data.dup(), var_request_mutated.dup())
	var_data = this.filter_response_by_context(var_data.dup(), if !rt.is_true(var_request_mutated.array_get('context')) { rt.new_string('view') } else { var_request_mutated.array_get('context') })
	mut var_response := rt.call_function('rest_ensure_response', [var_data.dup()])
	rt.call_method(var_response, 'add_links', [this.prepare_links(var_data.array_get('id'), var_request_mutated.array_get('group_id'))])
...
fn (mut this Class_WC_REST_Setting_Options_V2_Controller) prepare_links(var_setting_id rt.PhpVal, var_group_id rt.PhpVal) rt.PhpVal {
	mut var_base := rt.call_function('str_replace', [rt.new_string('(?P<group_id>[\\w-]+)'), var_group_id.dup(), this.rest_base])
	mut var_links := { 'self': { 'href': rt.call_function('rest_url', []) }, 'collection': { 'href': rt.call_function('rest_url', []) } }
...
fn (mut this Class_WC_REST_Setting_Options_V2_Controller) get_items_permissions_check(var_request rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wc_rest_check_manager_permissions', [rt.new_string('settings'), rt.new_string('read')]))))) {
		return (create_wp_error(rt.new_string('woocommerce_rest_cannot_view'), rt.call_function('__', [, ]), rt.create_array([rt.ArrayItem{ key: , val:  }]))).to_bool()
	}
	return true
}
...
fn create_wc_rest_setting_options_v2_controller() &Class_WC_REST_Setting_Options_V2_Controller {
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version2_class_wc_rest_setting_options_v2_controller_php() {
