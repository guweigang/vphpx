...
fn (mut this Class_WC_REST_Product_Custom_Fields_Controller) register_routes()  {
	rt.call_function('register_rest_route', [this.namespace, '/' + (this.rest_base).str() + '/names', rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.readable() }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Product_Custom_Fields_Controller', ['WC_REST_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_item_names' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Product_Custom_Fields_Controller', ['WC_REST_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_items_permissions_check' }]) }, rt.ArrayItem{ key: 'args', val: this.get_collection_params() }]) }, rt.ArrayItem{ key: 'schema', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Product_Custom_Fields_Controller', ['WC_REST_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_public_item_schema' }]) }])])
}

fn (mut this Class_WC_REST_Product_Custom_Fields_Controller) get_item_names(var_request rt.PhpVal) rt.PhpVal {
	mut var_wpdb := rt.new_null()
	// unsupported statement: Stmt_Global
	mut var_search := rt.new_string(rt.new_string(var_request.array_get('search').to_string().trim_space()))
	mut var_order := rt.new_string(if rt.is_true(rt.identical(rt.new_string(var_request.array_get('order').to_string().to_upper()), rt.new_string('DESC'))) { rt.new_string('DESC') } else { rt.new_string('ASC') })
	...
	mut var_base_query := rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('SELECT DISTINCT post_metas.meta_key\n\t\t\tFROM '), rt.get_property(var_wpdb, 'postmeta')), rt.new_string(' post_metas LEFT JOIN ')), rt.get_property(var_wpdb, 'posts')), rt.new_string(' posts ON post_metas.post_id = posts.id\n\t\t\tWHERE posts.post_type = %s AND post_metas.meta_key NOT LIKE %s AND post_metas.meta_key LIKE %s')), this.post_type, (rt.call_method(var_wpdb, 'esc_like', [rt.new_string('_')])).str() + '%', '%' + (rt.call_method(var_wpdb, 'esc_like', [var_search.dup()])).str() + '%'])
	mut var_query := var_base_query.dup()
	// unsupported expression: Expr_AssignOp_Concat
	mut var_total_query := rt.new_string(rt.new_string("SELECT COUNT(1) FROM (${var_base_query.to_string()}) AS total"))
	mut var_query_result := rt.call_method(var_wpdb, 'get_results', [var_query.dup()])
	mut var_total_items := rt.call_method(var_wpdb, 'get_var', [var_total_query.dup()])
	...
	mut var_response := rt.call_function('rest_ensure_response', [var_custom_field_names.dup()])
	...
	mut var_base := rt.call_function('add_query_arg', [rt.call_method(var_request, 'get_query_params', []rt.PhpVal{}), rt.call_function('rest_url', ['/' + (this.namespace).str() + '/' + (this.rest_base).str() + '/names'])])
...
fn create_wc_rest_product_custom_fields_controller() &Class_WC_REST_Product_Custom_Fields_Controller {
...
pub fn init_wp_content_plugins_woocommerce_includes_rest_api_controllers_version3_class_wc_rest_product_custom_fields_controller_php() {
