...
fn (mut this Class_WC_Order_Refund) get_post_title() rt.PhpVal {
	return rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Refund &ndash; %s'), rt.new_string('woocommerce')]), rt.call_method(create_datetime(rt.new_string('now')), 'format', [rt.call_function('_x', [rt.new_string('M d, Y @ h:i A'), rt.new_string('Order date parsed by DateTime::format'), rt.new_string('woocommerce')])])])
...
fn (mut this Class_WC_Order_Refund) get_formatted_refund_amount() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_formatted_refund_amount'), rt.call_function('wc_price', [this.get_amount(''), rt.create_array([rt.ArrayItem{ key: 'currency', val: this.get_currency() }])]), rt.new_object('WC_Order_Refund', ['WC_Abstract_Order'], &this)])
}
...
fn (mut this Class_WC_Order_Refund) set_reason(var_value rt.PhpVal)  {
	this.set_prop(rt.new_string('reason'), var_value.dup())
}

fn (mut this Class_WC_Order_Refund) set_refunded_by(var_value rt.PhpVal)  {
	this.set_prop(rt.new_string('refunded_by'), rt.call_function('absint', [var_value.dup()]))
}

fn (mut this Class_WC_Order_Refund) set_refunded_payment(var_value rt.PhpVal)  {
	this.set_prop(rt.new_string('refunded_payment'), // unsupported expression: Expr_Cast_Bool)
}

fn (mut this Class_WC_Order_Refund) magic_get(var_key rt.PhpVal) rt.PhpVal {
	rt.call_function('wc_doing_it_wrong', [var_key.dup(), rt.new_string('Refund properties should not be accessed directly.'), rt.new_string('3.0')])
	...
	return this.Class_WC_Abstract_Order.magic_get(var_key.dup())
}

fn (mut this Class_WC_Order_Refund) get_refund(id i64) bool {
	rt.call_function('wc_deprecated_function', [rt.new_string('get_refund'), rt.new_string('3.0'), rt.new_string('read')])
	if !(var_id != 0) {
		return false
	}
	mut var_result := rt.call_function('wc_get_order', [rt.new_int(id)])
	if rt.is_true(var_result) {
		this.populate(var_result.dup())
		return true
	}
	return false
}
...
fn (mut this Class_WC_Order_Refund) get_refund_amount() rt.PhpVal {
	rt.call_function('wc_deprecated_function', [rt.new_string('get_refund_amount'), rt.new_string('3.0'), rt.new_string('get_amount')])
	return this.get_amount('')
}

fn (mut this Class_WC_Order_Refund) get_refund_reason() rt.PhpVal {
	rt.call_function('wc_deprecated_function', [rt.new_string('get_refund_reason'), rt.new_string('3.0'), rt.new_string('get_reason')])
	return this.get_reason('')
}
...
fn create_wc_order_refund() &Class_WC_Order_Refund {
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_order_refund_php() {
