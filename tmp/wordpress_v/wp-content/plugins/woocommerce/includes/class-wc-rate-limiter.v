...
fn Class_WC_Rate_Limiter.init()  {
	rt.call_function('add_action', [rt.new_string('woocommerce_cleanup_rate_limits'), rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT }, rt.ArrayItem{ key: none, val: 'cleanup' }])])
}
...
fn Class_WC_Rate_Limiter.get_cache_key(var_action_id rt.PhpVal) rt.PhpVal {
	return fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Cache_Helper{}; return temp.get_cache_prefix(arg_0) }(rt.new_string('rate_limit' + (var_action_id).str()))
}

fn Class_WC_Rate_Limiter.get_cached(var_action_id rt.PhpVal) rt.PhpVal {
	return rt.call_function('wp_cache_get', [Class_WC_Rate_Limiter.get_cache_key(var_action_id.dup()), Class_WC_Rate_Limiter.cache_group()])
}

fn Class_WC_Rate_Limiter.set_cache(var_action_id rt.PhpVal, var_expiry rt.PhpVal) rt.PhpVal {
	return rt.call_function('wp_cache_set', [Class_WC_Rate_Limiter.get_cache_key(var_action_id.dup()), var_expiry.dup(), Class_WC_Rate_Limiter.cache_group()])
}

fn Class_WC_Rate_Limiter.retried_too_soon(var_action_id rt.PhpVal) bool {
	mut var_wpdb := rt.new_null()
	// unsupported statement: Stmt_Global
	mut var_next_try_allowed_at := Class_WC_Rate_Limiter.get_cached(var_action_id.dup())
	if rt.is_true(rt.identical(rt.new_bool(false), var_next_try_allowed_at)) {
		var_next_try_allowed_at = rt.call_method(var_wpdb, 'get_var', [rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.new_string('\n\t\t\t\t\t\tSELECT rate_limit_expiry\n\t\t\t\t\t\tFROM '), rt.get_property(var_wpdb, 'prefix')), rt.new_string('wc_rate_limits\n\t\t\t\t\t\tWHERE rate_limit_key = %s\n\t\t\t\t\t')), var_action_id.dup()])])
		Class_WC_Rate_Limiter.set_cache(var_action_id.dup(), var_next_try_allowed_at.dup())
	}
...
fn Class_WC_Rate_Limiter.set_rate_limit(var_action_id rt.PhpVal, var_delay rt.PhpVal) rt.PhpVal {
	mut var_wpdb := rt.new_null()
	// unsupported statement: Stmt_Global
	mut var_next_try_allowed_at := rt.add(rt.call_function('time', []rt.PhpVal{}), var_delay)
	mut var_result := rt.call_method(var_wpdb, 'replace', [(rt.get_property(var_wpdb, 'prefix')).str() + 'wc_rate_limits', rt.create_array([rt.ArrayItem{ key: 'rate_limit_key', val: var_action_id }, rt.ArrayItem{ key: 'rate_limit_expiry', val: var_next_try_allowed_at }]), rt.create_array([rt.ArrayItem{ key: none, val: '%s' }, rt.ArrayItem{ key: none, val: '%d' }])])
	Class_WC_Rate_Limiter.set_cache(var_action_id.dup(), var_next_try_allowed_at.dup())
...
fn Class_WC_Rate_Limiter.cleanup()  {
	mut var_wpdb := rt.new_null()
	// unsupported statement: Stmt_Global
	rt.call_method(var_wpdb, 'query', [rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.new_string('DELETE FROM '), rt.get_property(var_wpdb, 'prefix')), rt.new_string('wc_rate_limits WHERE rate_limit_expiry < %d')), rt.call_function('time', []rt.PhpVal{})])])
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('WC_Cache_Helper')])) {
		fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Cache_Helper{}; return temp.invalidate_cache_group(arg_0) }(rt.new_string(Class_WC_Rate_Limiter.cache_group()))
	}
}
...
fn (mut this Class_WC_Rate_Limiter) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'set_rate_limit' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return Class_WC_Rate_Limiter.set_rate_limit(dispatch_arg_0, dispatch_arg_1)
		}
		'cleanup' {
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_rate_limiter_php() {
