...
fn (mut this Class_WC_Customer_Download_Log) construct(download_log i64)  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.new_int(download_log).is_long() || rt.new_int(download_log).is_double())) && download_log > 0)) {
	...
	} else if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.new_int(download_log).is_object())) && !(!rt.is_true(rt.get_property(rt.new_int(download_log), 'download_log_id'))))) {
	...
	this.dispatch_set_prop('data_store', fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Data_Store{}; return temp.load(arg_0) }(rt.new_string('customer-download-log')))
	if rt.is_true(rt.greater(this.get_id(), rt.new_int(0))) {
		rt.call_method(rt.get_property(rt.new_object('WC_Customer_Download_Log', ['WC_Data'], &this), 'data_store'), 'read', [rt.new_object('WC_Customer_Download_Log', ['WC_Data'], &this)])
	}
}
...
fn (mut this Class_WC_Customer_Download_Log) set_timestamp(var_date rt.PhpVal)  {
	this.set_date_prop(rt.new_string('timestamp'), var_date.dup())
}

fn (mut this Class_WC_Customer_Download_Log) set_permission_id(var_value rt.PhpVal)  {
	this.set_prop(rt.new_string('permission_id'), rt.call_function('absint', [var_value.dup()]))
}
...
fn (mut this Class_WC_Customer_Download_Log) set_user_ip_address(var_value rt.PhpVal)  {
	this.set_prop(rt.new_string('user_ip_address'), var_value.dup())
}
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_customer_download_log_php() {
