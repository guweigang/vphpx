...
fn (mut this Class_WC_Legacy_Coupon) magic_get(var_key rt.PhpVal) rt.PhpVal {
	rt.call_function('wc_doing_it_wrong', [var_key.dup(),
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('id'))) {
		...
		var_value = var_legacy_custom_fields.dup()
...
fn (mut this Class_WC_Legacy_Coupon) format_array(var_array rt.PhpVal) rt.PhpVal {
	...
	rt.call_function('wc_deprecated_function', [rt.new_string('WC_Coupon::format_array'),
		rt.new_string('3.0')])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_array_mutated.dup().is_array()))))) {
		if rt.is_true(rt.call_function('is_serialized', [var_array_mutated.dup()])) {
			var_array_mutated = rt.call_function('maybe_unserialize', [
				var_array_mutated.dup()])
		} else {
			var_array_mutated = rt.call_function('explode', [
				rt.new_string(','), var_array_mutated.dup()])
		}
	}
...
pub fn init_wp_content_plugins_woocommerce_includes_legacy_class_wc_legacy_coupon_php() {
