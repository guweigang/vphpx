...
fn (mut this Class_WC_Legacy_Shipping_Zone) get_zone_id() rt.PhpVal {
	rt.call_function('wc_deprecated_function', [
		rt.new_string('WC_Shipping_Zone::get_zone_id'),
		rt.new_string('3.0'),
		rt.new_string('WC_Shipping_Zone::get_id'),
	])
	return this.get_id()
}

fn (mut this Class_WC_Legacy_Shipping_Zone) read(var_zone_id rt.PhpVal) {
	rt.call_function('wc_deprecated_function', [rt.new_string('WC_Shipping_Zone::read'),
		rt.new_string('3.0'), rt.new_string('a shipping zone initialized with an ID.')])
	this.set_id(var_zone_id.dup())
	mut var_data_store := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_WC_Data_Store{}
		return temp.load(arg_0)
	}(rt.new_string('shipping-zone'))
	rt.call_method(var_data_store, 'read', [
		rt.new_object('WC_Legacy_Shipping_Zone', ['WC_Data'], &this),
	])
}

fn (mut this Class_WC_Legacy_Shipping_Zone) update() {
	rt.call_function('wc_deprecated_function', [
		rt.new_string('WC_Shipping_Zone::update'),
		rt.new_string('3.0'),
		rt.new_string('WC_Shipping_Zone::save instead.'),
	])
	mut var_data_store := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_WC_Data_Store{}
		return temp.load(arg_0)
...
fn (mut this Class_WC_Legacy_Shipping_Zone) create() {
	rt.call_function('wc_deprecated_function', [
		rt.new_string('WC_Shipping_Zone::create'),
		rt.new_string('3.0'),
		rt.new_string('WC_Shipping_Zone::save instead.'),
	])
	mut var_data_store := fn (arg_0 rt.PhpVal) rt.PhpVal {
...
pub fn init_wp_content_plugins_woocommerce_includes_legacy_class_wc_legacy_shipping_zone_php() {
