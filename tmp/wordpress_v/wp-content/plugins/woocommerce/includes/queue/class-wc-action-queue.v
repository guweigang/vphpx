...
fn (mut this Class_WC_Action_Queue) add(var_hook rt.PhpVal, var_args rt.PhpVal, group string) rt.PhpVal {
	return this.schedule_single(rt.call_function('time', []rt.PhpVal{}), var_hook.dup(),
		var_args.dup(), group)
}
...
fn (mut this Class_WC_Action_Queue) schedule_cron(var_timestamp rt.PhpVal, var_cron_schedule rt.PhpVal, var_hook rt.PhpVal, var_args rt.PhpVal, group string) rt.PhpVal {
	return rt.call_function('as_schedule_cron_action', [var_timestamp.dup(),
		var_cron_schedule.dup(), var_hook.dup(), var_args.dup(),
		rt.new_string(group)])
}
...
fn (mut this Class_WC_Action_Queue) cancel_all(var_hook rt.PhpVal, var_args rt.PhpVal, group string) {
	rt.call_function('as_unschedule_all_actions', [var_hook.dup(),
		var_args.dup(), rt.new_string(group)])
}
...
fn (mut this Class_WC_Action_Queue) get_next(var_hook rt.PhpVal, var_args rt.PhpVal, group string) rt.PhpVal {
	mut var_next_timestamp := rt.call_function('as_next_scheduled_action', [
		var_hook.dup(), var_args.dup(), rt.new_string(group)])
	if rt.is_true(rt.new_bool(var_next_timestamp.dup().is_long()
		|| var_next_timestamp.dup().is_double()))
	{
...
fn create_wc_action_queue() &Class_WC_Action_Queue {
...
pub fn init_wp_content_plugins_woocommerce_includes_queue_class_wc_action_queue_php() {
