...
fn Class_WooCommerce_Admin_Experimental_Abtest.in_treatment(experiment_name string, as_auth_wpcom_user bool) rt.PhpVal {
	...
	mut var_abtest := create_woocommerce_admin_self(var_anon_id.dup(), rt.new_string('woocommerce'), var_allow_tracking.dup(), rt.new_bool(as_auth_wpcom_user).dup())
	return rt.identical(var_abtest.get_variation(rt.new_string(experiment_name)), rt.new_string('treatment'))
}

fn (mut this Class_WooCommerce_Admin_Experimental_Abtest) get_variation(var_test_name rt.PhpVal) string {
	if rt.is_true(rt.new_bool(!(rt.is_true(this.consent)))) {
		return 'control'
	}
	mut var_variation := this.fetch_variation(var_test_name.dup())
	if rt.is_true(rt.call_function('is_wp_error', [var_variation.dup()])) {
		if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
			rt.throw_exception(rt.new_object('WooCommerce_Admin_Exception', []string{}, create_woocommerce_admin_exception(rt.call_method(var_variation, 'get_error_message', []rt.PhpVal{}))))
		}
		return 'control'
	}
...
fn (mut this Class_WooCommerce_Admin_Experimental_Abtest) request_assignment(var_args rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(this.as_auth_wpcom_user) && rt.is_true(rt.call_function('class_exists', [Class_Automattic_Jetpack_Connection_Manager.class()])))) {
		...
		if rt.is_true(var_jetpack_connection_manager.is_user_connected()) {
			mut var_response := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Connection_Client{}; return temp.wpcom_json_api_request_as_user(arg_0, arg_1, arg_2) }(rt.new_string('/experiments/0.1.0/assignments/' + this.platform), rt.new_string('2'), var_args_mutated.dup())
		}
	}
	if !(!(var_response).is_null()) {
		if !(var_args_mutated.array_isset(rt.new_string('anon_id'))) || !rt.is_true(var_args_mutated.array_get('anon_id')) {
			return create_woocommerce_admin_wp_error(rt.new_string('invalid_anon_id'), rt.new_string('anon_id must be an none empty string.'))
		}
		mut var_url := rt.call_function('add_query_arg', [var_args_mutated.dup(), rt.call_function('sprintf', [rt.new_string('https://public-api.wordpress.com/wpcom/v2/experiments/0.1.0/assignments/%s'), this.platform])])
		var_response = rt.call_function('wp_remote_get', [var_url.dup()])
	}
...
fn (mut this Class_WooCommerce_Admin_Experimental_Abtest) fetch_variation(var_test_name rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(var_test_name)))) {
		return create_woocommerce_admin_wp_error(rt.new_string('test_name_not_provided'), rt.new_string('A/B test name has not been provided.'))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('/^[a-z0-9_]+$/'), var_test_name.dup()]))))) {
		return create_woocommerce_admin_wp_error(rt.new_string('invalid_test_name'), rt.new_string('Invalid A/B test name.'))
	}
	if this.tests.array_isset(var_test_name) {
		return this.tests.array_get(var_test_name)
	}
	if !(!rt.is_true(rt.call_function('get_transient', ['abtest_variation_' + (var_test_name).str()]))) {
		return rt.call_function('get_transient', ['abtest_variation_' + (var_test_name).str()])
	}
	mut var_args := rt.create_array([rt.ArrayItem{ key: 'experiment_name', val: var_test_name }, rt.ArrayItem{ key: 'anon_id', val: rt.call_function('rawurlencode', [this.anon_id]) }, rt.ArrayItem{ key: 'woo_country_code', val: rt.call_function('rawurlencode', [rt.call_function('get_option', [rt.new_string('woocommerce_default_country'), rt.new_string('US:CA')])]) }, rt.ArrayItem{ key: 'woo_wcadmin_install_timestamp', val: rt.call_function('rawurlencode', [rt.call_function('get_option', [Class_Automattic_WooCommerce_Admin_WCAdminHelper.wc_admin_timestamp_option()])]) }])
	var_args = rt.call_function('apply_filters', [rt.new_string('woocommerce_explat_request_args'), var_args.dup()])
	mut var_response := this.request_assignment(var_args.dup())
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_wp_error', [var_response.dup()])) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_response.dup().is_array()))))))) || !(var_response.array_isset(rt.new_string('body'))))) {
		return create_woocommerce_admin_wp_error(rt.new_string('failed_to_fetch_data'), rt.new_string('Unable to fetch the requested data.'))
	}
	mut var_results := rt.call_function('json_decode', [var_response.array_get('body'), rt.new_bool(true)])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_results.dup().is_array()))))) {
		return create_woocommerce_admin_wp_error(rt.new_string('unexpected_data_format'), rt.new_string('Data was not returned in the expected format.'))
	}
	this.tests.array_set(var_test_name, if !(var_results.array_get('variations').array_get(var_test_name)).is_null() { var_results.array_get('variations').array_get(var_test_name) } else { rt.new_null() })
	mut var_variation := if !(var_results.array_get('variations').array_get(var_test_name)).is_null() { var_results.array_get('variations').array_get(var_test_name) } else { rt.new_string('control') }
	if !(!rt.is_true(var_results.array_get('ttl'))) {
		rt.call_function('set_transient', ['abtest_variation_' + (var_test_name).str(), var_variation.dup(), var_results.array_get('ttl')])
	}
...
fn create_woocommerce_admin_self() &Class_WooCommerce_Admin_self {
...
fn create_woocommerce_admin_exception() &Class_WooCommerce_Admin_Exception {
...
fn create_woocommerce_admin_wp_error() &Class_WooCommerce_Admin_WP_Error {
...
fn (mut this Class_WooCommerce_Admin_Experimental_Abtest) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'in_treatment' {
			...
			return Class_WooCommerce_Admin_Experimental_Abtest.in_treatment(dispatch_arg_0, dispatch_arg_1)
		}
		'get_variation' {
...
fn (mut this Class_WooCommerce_Admin_Experimental_Abtest) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'tests' { this.tests = val; return true }
		'anon_id' { this.anon_id = (val).str(); return true }
		'platform' { this.platform = (val).str(); return true }
		'consent' { this.consent = (val).to_bool(); return true }
		'as_auth_wpcom_user' { this.as_auth_wpcom_user = (val).to_bool(); return true }
...
pub fn init_wp_content_plugins_woocommerce_includes_react_admin_class_experimental_abtest_php() {
}
