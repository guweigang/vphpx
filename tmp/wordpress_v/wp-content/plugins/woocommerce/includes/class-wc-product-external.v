...
fn (mut this Class_WC_Product_External) get_product_url(context string) rt.PhpVal {
	return rt.call_function('esc_url_raw', [this.get_prop(rt.new_string('product_url'), rt.new_string(context))])
}
...
fn (mut this Class_WC_Product_External) set_product_url(var_product_url rt.PhpVal)  {
	this.set_prop(rt.new_string('product_url'), rt.call_function('htmlspecialchars_decode', [var_product_url.dup()]))
}

fn (mut this Class_WC_Product_External) set_button_text(var_button_text rt.PhpVal)  {
	this.set_prop(rt.new_string('button_text'), var_button_text.dup())
}
...
fn (mut this Class_WC_Product_External) set_manage_stock(var_manage_stock rt.PhpVal)  {
	...
	if rt.is_true(rt.identical(rt.new_bool(true), var_manage_stock)) {
		this.error(rt.new_string('product_external_invalid_manage_stock'), rt.call_function('__', [rt.new_string('External products cannot be stock managed.'), rt.new_string('woocommerce')]))
	}
}
...
fn (mut this Class_WC_Product_External) set_stock_status(stock_status string)  {
	...
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		this.error(rt.new_string('product_external_invalid_stock_status'), rt.call_function('__', [rt.new_string('External products cannot be stock managed.'), rt.new_string('woocommerce')]))
	}
}
...
fn (mut this Class_WC_Product_External) set_backorders(var_backorders rt.PhpVal)  {
	...
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		this.error(rt.new_string('product_external_invalid_backorders'), rt.call_function('__', [rt.new_string('External products cannot be backordered.'), rt.new_string('woocommerce')]))
	}
}

fn (mut this Class_WC_Product_External) is_purchasable() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_is_purchasable'), rt.new_bool(false), rt.new_object('WC_Product_External', ['WC_Product'], &this)])
}

fn (mut this Class_WC_Product_External) add_to_cart_url() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_product_add_to_cart_url'), this.get_product_url(''), rt.new_object('WC_Product_External', ['WC_Product'], &this)])
}

fn (mut this Class_WC_Product_External) single_add_to_cart_text() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_product_single_add_to_cart_text'), if rt.is_true(this.get_button_text('')) { this.get_button_text('') } else { rt.call_function('_x', [rt.new_string('Buy product'), rt.new_string('placeholder'), rt.new_string('woocommerce')]) }, rt.new_object('WC_Product_External', ['WC_Product'], &this)])
}

fn (mut this Class_WC_Product_External) add_to_cart_text() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_product_add_to_cart_text'), if rt.is_true(this.get_button_text('')) { this.get_button_text('') } else { rt.call_function('_x', [rt.new_string('Buy product'), rt.new_string('placeholder'), rt.new_string('woocommerce')]) }, rt.new_object('WC_Product_External', ['WC_Product'], &this)])
}

fn (mut this Class_WC_Product_External) add_to_cart_description() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_product_add_to_cart_description'), if rt.is_true(this.get_button_text('')) { this.get_button_text('') } else { rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Buy &ldquo;%s&rdquo;'), rt.new_string('woocommerce')]), this.get_name()]) }, rt.new_object('WC_Product_External', ['WC_Product'], &this)])
}
...
fn create_wc_product_external() &Class_WC_Product_External {
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_product_external_php() {
