...
fn (mut this Class_WC_Widget_Product_Search) construct()  {
	...
	this.dispatch_set_prop('widget_description', rt.call_function('__', [rt.new_string('A search form for your store.'), rt.new_string('woocommerce')]))
	this.dispatch_set_prop('widget_id', rt.new_string('woocommerce_product_search'))
	this.dispatch_set_prop('widget_name', rt.call_function('__', [rt.new_string('Product Search'), rt.new_string('woocommerce')]))
	this.dispatch_set_prop('settings', rt.create_array([rt.ArrayItem{ key: 'title', val: rt.create_array([rt.ArrayItem{ key: 'type', val: 'text' }, rt.ArrayItem{ key: 'std', val: '' }, rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Title'), rt.new_string('woocommerce')]) }]) }]))
...
pub fn init_wp_content_plugins_woocommerce_includes_widgets_class_wc_widget_product_search_php() {
