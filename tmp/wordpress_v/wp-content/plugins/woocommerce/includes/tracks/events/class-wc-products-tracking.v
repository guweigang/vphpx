...
fn (mut this Class_WC_Products_Tracking) init()  {
	...
	rt.call_function('add_action', [Class_WC_Products_Tracking.track_product_published_callback(), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_Products_Tracking', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'track_product_published_maybe_defer' }]), rt.new_int(10), rt.new_int(3)])
}

fn (mut this Class_WC_Products_Tracking) track_products_view()  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(rt.new_string('post_type')) && rt.is_true(rt.identical(rt.new_string('product'), rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('post_type')]))))) && !(rt.get_superglobal('_GET').array_isset(rt.new_string('_wp_http_referer'))))) {
		fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0) }(rt.new_string('products_view'))
		if rt.get_superglobal('_GET').array_isset(rt.new_string('s')) && 0 < rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('s')])]).to_string().len {
			fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0) }(rt.new_string('products_search'))
		}
	}
}

fn (mut this Class_WC_Products_Tracking) track_categories_and_tags_view()  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(rt.new_string('post_type')) && rt.is_true(rt.identical(rt.new_string('product'), rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('post_type')]))))) && rt.get_superglobal('_GET').array_isset(rt.new_string('taxonomy')))) && !(rt.get_superglobal('_GET').array_isset(rt.new_string('_wp_http_referer'))))) {
		mut var_taxonomy := rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('taxonomy')])
		if rt.is_true(rt.identical(rt.new_string('product_cat'), var_taxonomy)) {
			fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0) }(rt.new_string('categories_view'))
		} else if rt.is_true(rt.identical(rt.new_string('product_tag'), var_taxonomy)) {
			fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0) }(rt.new_string('tags_view'))
		}
		if rt.get_superglobal('_GET').array_isset(rt.new_string('s')) && 0 < rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('s')])]).to_string().len {
			if rt.is_true(rt.identical(rt.new_string('product_cat'), var_taxonomy)) {
				fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0) }(rt.new_string('categories_search'))
			} else if rt.is_true(rt.identical(rt.new_string('product_tag'), var_taxonomy)) {
				fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0) }(rt.new_string('tags_search'))
			}
		}
	}
}
...
fn (mut this Class_WC_Products_Tracking) track_product_updated(var_product_id rt.PhpVal, var_post rt.PhpVal)  {
	...
	mut var_source := rt.call_function('apply_filters', [rt.new_string('woocommerce_product_source'), if rt.is_true(fn () rt.PhpVal { mut temp := Class_WC_Products_Tracking{}; return temp.is_importing() }()) { rt.new_string('import') } else { Class_WC_Products_Tracking.tracks_source() }])
	mut var_properties := { 'product_id': var_product_id, 'source': var_source }
	fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0, arg_1) }(rt.new_string('product_edit'), var_properties.dup())
}

fn (mut this Class_WC_Products_Tracking) track_product_updated_client_side(var_post rt.PhpVal)  {
	mut var_handle := rt.new_string(rt.new_string('wc-tracks-product-updated-client-side'))
	rt.call_function('wp_register_script', [var_handle.dup(), rt.new_string(''), rt.create_array([rt.ArrayItem{ key: none, val: 'jquery' }]), rt.get_constant('WC_VERSION'), rt.create_array([rt.ArrayItem{ key: 'in_footer', val: true }])])
...
fn Class_WC_Products_Tracking.get_possible_product_type_options_ids() rt.PhpVal {
	...
	mut var_product_type_option := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return var_product_type_option.array_get('id')
	}
	mut var_product_type_option := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return var_product_type_option.array_get('id')
	}
	...
	mut var_product_type_options_ids := rt.call_function('array_values', [rt.call_function('array_map', [rt.new_closure(closure_1_fn), rt.call_function('apply_filters', [rt.new_string('product_type_options'), rt.call_function('wc_get_default_product_type_options', []rt.PhpVal{})])])])
	return var_product_type_options_ids.dup()
}
...
fn Class_WC_Products_Tracking.get_product_type_options(var_post_id rt.PhpVal) rt.PhpVal {
	...
	mut var_post_meta := rt.call_function('get_post_meta', [var_post_id.dup()])
	mut var_product_type_options := rt.new_array()
	{
		...
		for {
			...
			var_product_type_options.array_set(var_product_type_option_id, if var_post_meta.array_isset(var_product_type_option_id) { var_post_meta.array_get(var_product_type_option_id).array_get(0) } else { rt.new_string('no') })
		}
	}
	return var_product_type_options.dup()
}
...
fn Class_WC_Products_Tracking.get_product_type_options_string(var_product_type_options rt.PhpVal) rt.PhpVal {
	...
	mut var_is_enabled := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.identical(rt.new_string('yes'), var_is_enabled)
	}
	return rt.call_function('implode', [rt.new_string(', '), rt.func_array_keys(rt.call_function('array_filter', [var_product_type_options_mutated.dup(), rt.new_closure(closure_5_fn)]))])
}
...
fn (mut this Class_WC_Products_Tracking) track_product_published(var_post_id rt.PhpVal, var_post rt.PhpVal, var_update rt.PhpVal, var_post_before rt.PhpVal)  {
	...
	mut var_product_type_options := Class_WC_Products_Tracking.get_product_type_options(var_post_id.dup())
	mut var_product_type_options_string := Class_WC_Products_Tracking.get_product_type_options_string(var_product_type_options.dup())
	mut var_is_importing := fn () rt.PhpVal { mut temp := Class_WC_Products_Tracking{}; return temp.is_importing() }()
	mut var_properties := { 'attributes': rt.new_int(rt.call_method(var_product, 'get_attributes', []rt.PhpVal{}).array_count()), 'categories': rt.new_int(rt.call_method(var_product, 'get_category_ids', []rt.PhpVal{}).array_count()), 'cross_sells': if !(!rt.is_true(rt.call_method(var_product, 'get_cross_sell_ids', []rt.PhpVal{}))) { rt.new_string('yes') } else { rt.new_string('no') }, 'description': if rt.is_true(rt.call_method(var_product, 'get_description', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'dimensions': if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) { rt.new_string('yes') } else { rt.new_string('no') }, 'enable_reviews': if rt.is_true(rt.call_method(var_product, 'get_reviews_allowed', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'is_downloadable': if rt.is_true(rt.call_method(var_product, 'is_downloadable', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'is_virtual': if rt.is_true(rt.call_method(var_product, 'is_virtual', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'manage_stock': if rt.is_true(rt.call_method(var_product, 'get_manage_stock', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'menu_order': if rt.is_true(rt.call_method(var_product, 'get_menu_order', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'product_id': var_post_id, 'product_gallery': rt.new_int(rt.call_method(var_product, 'get_gallery_image_ids', []rt.PhpVal{}).array_count()), 'product_image': if rt.is_true(rt.call_method(var_product, 'get_image_id', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'product_type': rt.call_method(var_product, 'get_type', []rt.PhpVal{}), 'product_type_options': var_product_type_options_string, 'purchase_note': if rt.is_true(rt.call_method(var_product, 'get_purchase_note', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'sale_price': if rt.is_true(rt.call_method(var_product, 'get_sale_price', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'source': rt.call_function('apply_filters', [rt.new_string('woocommerce_product_source'), if rt.is_true(var_is_importing) { rt.new_string('import') } else { Class_WC_Products_Tracking.tracks_source() }]), 'short_description': if rt.is_true(rt.call_method(var_product, 'get_short_description', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'tags': rt.new_int(rt.call_method(var_product, 'get_tag_ids', []rt.PhpVal{}).array_count()), 'upsells': if !(!rt.is_true(rt.call_method(var_product, 'get_upsell_ids', []rt.PhpVal{}))) { rt.new_string('yes') } else { rt.new_string('no') }, 'weight': if rt.is_true(rt.call_method(var_product, 'get_weight', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') }, 'global_unique_id': if rt.is_true(rt.call_method(var_product, 'get_global_unique_id', []rt.PhpVal{})) { rt.new_string('yes') } else { rt.new_string('no') } }
...
fn (mut this Class_WC_Products_Tracking) track_product_published_maybe_defer(event_name string, mut var_event_properties Class_array, defer bool)  {
	if var_defer {
		rt.call_function('as_schedule_single_action', [rt.call_function('time', []rt.PhpVal{}), Class_WC_Products_Tracking.track_product_published_callback(), rt.create_array([rt.ArrayItem{ key: none, val: event_name }, rt.ArrayItem{ key: none, val: var_event_properties }]), rt.new_string('woocommerce-tracks')])
	} else {
		fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0, arg_1) }(rt.new_string(event_name), rt.new_object('array', []string{}, var_event_properties))
	}
}

fn (mut this Class_WC_Products_Tracking) track_product_category_created(var_category_id rt.PhpVal)  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.is_defined(arg_0) }(rt.new_string('DOING_AJAX')))))) || !rt.is_true(rt.get_superglobal('_POST').array_get('action')))) || rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))))) {
		return rt.new_null()
	}
	mut var_category := rt.call_function('get_term', [var_category_id.dup(), rt.new_string('product_cat')])
	mut var_parent_category := rt.new_string(if rt.is_true(rt.greater(rt.get_property(var_category, 'parent'), rt.new_int(0))) { rt.new_string('Other') } else { rt.new_string('None') })
	if rt.is_true(rt.greater(rt.get_property(var_category, 'parent'), rt.new_int(0))) {
		mut var_parent := rt.call_function('get_term', [var_category_id.dup(), rt.new_string('product_cat')])
		if rt.is_true(rt.identical(rt.new_string('uncategorized'), rt.get_property(var_parent, 'name'))) {
			var_parent_category = rt.new_string(rt.new_string('Uncategorized'))
		}
	}
	mut var_properties := { 'category_id': var_category_id, 'parent_id': rt.get_property(var_category, 'parent'), 'parent_category': var_parent_category, 'page': if rt.is_true(rt.identical(rt.new_string('add-tag'), rt.get_superglobal('_POST').array_get('action'))) { rt.new_string('categories') } else { rt.new_string('product') }, 'display_type': if rt.get_superglobal('_POST').array_isset(rt.new_string('display_type')) { rt.call_function('wp_unslash', [rt.get_superglobal('_POST').array_get('display_type')]) } else { rt.new_string('') }, 'image': if rt.is_true(rt.new_bool(rt.get_superglobal('_POST').array_isset(rt.new_string('product_cat_thumbnail_id')) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) { rt.new_string('Yes') } else { rt.new_string('No') } }
	fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0, arg_1) }(rt.new_string('product_category_add'), var_properties.dup())
}

fn (mut this Class_WC_Products_Tracking) track_product_category_updated(var_category_id rt.PhpVal)  {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.get_superglobal('_POST').array_get('action')) || rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))))) {
		return rt.new_null()
	}
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0) }(rt.new_string('product_category_update'))
}

fn (mut this Class_WC_Products_Tracking) get_product_screen(var_hook rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('edit.php'), var_hook)) && rt.get_superglobal('_GET').array_isset(rt.new_string('post_type')))) && rt.is_true(rt.identical(rt.new_string('product'), rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('post_type')]))))) {
		return rt.new_string('list')
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('post-new.php'), var_hook)) && rt.get_superglobal('_GET').array_isset(rt.new_string('post_type')))) && rt.is_true(rt.identical(rt.new_string('product'), rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('post_type')]))))) {
		return rt.new_string('new')
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('post.php'), var_hook)) && rt.get_superglobal('_GET').array_isset(rt.new_string('post')))) && rt.is_true(rt.identical(rt.new_string('product'), rt.call_function('get_post_type', [rt.new_int(rt.get_superglobal('_GET').array_get('post').to_i64())]))))) {
...
fn (mut this Class_WC_Products_Tracking) possibly_add_product_tracking_scripts(var_hook rt.PhpVal)  {
	mut var_product_screen := this.get_product_screen(var_hook.dup())
	if rt.is_true(rt.new_bool(!(rt.is_true(var_product_screen)))) {
		return rt.new_null()
	}
	fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets{}; return temp.register_script(arg_0, arg_1, arg_2) }(rt.new_string('wp-admin-scripts'), rt.new_string('product-tracking'), rt.new_bool(false))
...
fn (mut this Class_WC_Products_Tracking) possibly_add_product_import_scripts(var_hook rt.PhpVal)  {
	...
	fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets{}; return temp.register_script(arg_0, arg_1, arg_2) }(rt.new_string('wp-admin-scripts'), rt.new_string('product-import-tracking'), rt.new_bool(false))
}

fn (mut this Class_WC_Products_Tracking) possibly_add_attribute_tracking_scripts(var_hook rt.PhpVal)  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) || !(rt.get_superglobal('_GET').array_isset(rt.new_string('page'))))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return rt.new_null()
	}
	fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets{}; return temp.register_script(arg_0, arg_1, arg_2) }(rt.new_string('wp-admin-scripts'), rt.new_string('attributes-tracking'), rt.new_bool(false))
}

fn (mut this Class_WC_Products_Tracking) possibly_add_tag_tracking_scripts(var_hook rt.PhpVal)  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) || !(rt.get_superglobal('_GET').array_isset(rt.new_string('post_type'))))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(rt.new_string('taxonomy')) && rt.is_true(rt.identical(rt.new_string('product_tag'), rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('taxonomy')]))))) {
		fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets{}; return temp.register_script(arg_0, arg_1, arg_2) }(rt.new_string('wp-admin-scripts'), rt.new_string('tags-tracking'), rt.new_bool(false))
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(rt.new_string('taxonomy')) && rt.is_true(rt.identical(rt.new_string('product_cat'), rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('taxonomy')]))))) {
		fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets{}; return temp.register_script(arg_0, arg_1, arg_2) }(rt.new_string('wp-admin-scripts'), rt.new_string('category-tracking'), rt.new_bool(false))
		return rt.new_null()
	}
	fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets{}; return temp.register_script(arg_0, arg_1, arg_2) }(rt.new_string('wp-admin-scripts'), rt.new_string('add-term-tracking'), rt.new_bool(false))
}

fn (mut this Class_WC_Products_Tracking) is_importing() bool {
	if rt.is_true(rt.new_bool(rt.get_superglobal('_POST').array_isset(rt.new_string('action')) && rt.is_true(rt.identical(rt.new_string('woocommerce_do_ajax_product_import'), rt.get_superglobal('_POST').array_get('action'))))) {
		return true
	}
	return false
...
fn create_wc_products_tracking() &Class_WC_Products_Tracking {
...
pub fn init_wp_content_plugins_woocommerce_includes_tracks_events_class_wc_products_tracking_php() {
