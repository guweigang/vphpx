...
fn (mut this Class_WC_Status_Tracking) init()  {
	rt.call_function('add_action', [rt.new_string('admin_init'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_Status_Tracking', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'track_status_view' }]), rt.new_int(10)])
}

fn (mut this Class_WC_Status_Tracking) track_status_view()  {
	if rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(rt.new_string('page')) && rt.is_true(rt.identical(rt.new_string('wc-status'), rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('page')])]))))) {
		mut var_tab := if rt.get_superglobal('_GET').array_isset(rt.new_string('tab')) { rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('tab')])]) } else { rt.new_string('status') }
		fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.record_event(arg_0, arg_1) }(rt.new_string('status_view'), rt.create_array([rt.ArrayItem{ key: 'tab', val: var_tab }, rt.ArrayItem{ key: 'tool_used', val: if rt.get_superglobal('_GET').array_isset(rt.new_string('action')) { rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('action')])]) } else { rt.new_null() } }]))
		if rt.is_true(rt.identical(rt.new_string('status'), var_tab)) {
			mut var_handle := rt.new_string(rt.new_string('wc-tracks-status-view'))
...
fn create_wc_status_tracking() &Class_WC_Status_Tracking {
...
pub fn init_wp_content_plugins_woocommerce_includes_tracks_events_class_wc_status_tracking_php() {
