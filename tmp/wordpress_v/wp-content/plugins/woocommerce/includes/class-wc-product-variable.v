...
fn (mut this Class_WC_Product_Variable) get_variation_prices(for_display bool) rt.PhpVal {
	mut var_prices := rt.call_method(rt.get_property(rt.new_object('WC_Product_Variable', ['WC_Product'], &this), 'data_store'), 'read_price_data', [rt.new_object('WC_Product_Variable', ['WC_Product'], &this), rt.new_bool(for_display)])
	{
		...
		for {
			...
			var_prices.array_set(var_price_key, this.sort_variation_prices(var_variation_prices.dup()))
		}
	}
	return var_prices.dup()
}
...
fn (mut this Class_WC_Product_Variable) get_variation_regular_price(min_or_max string, for_display bool) rt.PhpVal {
	...
	mut var_price := if rt.is_true(rt.identical(rt.new_string('min'), rt.new_string(min_or_max))) { rt.call_function('current', [var_prices.array_get('regular_price')]) } else { rt.call_function('end', [var_prices.array_get('regular_price')]) }
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_get_variation_regular_price'), var_price.dup(), rt.new_object('WC_Product_Variable', ['WC_Product'], &this), rt.new_string(min_or_max), rt.new_bool(for_display)])
}

fn (mut this Class_WC_Product_Variable) get_variation_sale_price(min_or_max string, for_display bool) rt.PhpVal {
	mut var_prices := this.get_variation_prices(for_display)
	mut var_price := if rt.is_true(rt.identical(rt.new_string('min'), rt.new_string(min_or_max))) { rt.call_function('current', [var_prices.array_get('sale_price')]) } else { rt.call_function('end', [var_prices.array_get('sale_price')]) }
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_get_variation_sale_price'), var_price.dup(), rt.new_object('WC_Product_Variable', ['WC_Product'], &this), rt.new_string(min_or_max), rt.new_bool(for_display)])
}

fn (mut this Class_WC_Product_Variable) get_variation_price(min_or_max string, for_display bool) rt.PhpVal {
	mut var_prices := this.get_variation_prices(for_display)
	mut var_price := if rt.is_true(rt.identical(rt.new_string('min'), rt.new_string(min_or_max))) { rt.call_function('current', [var_prices.array_get('price')]) } else { rt.call_function('end', [var_prices.array_get('price')]) }
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_get_variation_price'), var_price.dup(), rt.new_object('WC_Product_Variable', ['WC_Product'], &this), rt.new_string(min_or_max), rt.new_bool(for_display)])
}
...
fn (mut this Class_WC_Product_Variable) get_price_html(price string) rt.PhpVal {
	...
	if !rt.is_true(var_prices.array_get('price')) {
		...
		mut var_min_price := rt.call_function('current', [var_prices.array_get('price')])
		mut var_max_price := rt.call_function('end', [var_prices.array_get('price')])
		mut var_min_reg_price := rt.call_function('current', [var_prices.array_get('regular_price')])
		mut var_max_reg_price := rt.call_function('end', [var_prices.array_get('regular_price')])
		if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
			price_mutated = (rt.call_function('wc_format_price_range', [var_min_price.dup(), var_max_price.dup()])).str()
		} else if rt.is_true(rt.new_bool(rt.is_true(this.is_on_sale('')) && rt.is_true(rt.identical(var_min_reg_price, var_max_reg_price)))) {
			price_mutated = (rt.call_function('wc_format_sale_price', [rt.call_function('wc_price', [var_max_reg_price.dup()]), rt.call_function('wc_price', [var_min_price.dup()])])).str()
		} else {
			price_mutated = (rt.call_function('wc_price', [var_min_price.dup()])).str()
		}
		price_mutated = (rt.call_function('apply_filters', [rt.new_string('woocommerce_variable_price_html'), price_mutated + (this.get_price_suffix('', 0)).str(), rt.new_object('WC_Product_Variable', ['WC_Product'], &this)])).str()
	}
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_get_price_html'), rt.new_string(price_mutated).dup(), rt.new_object('WC_Product_Variable', ['WC_Product'], &this)])
}
...
fn (mut this Class_WC_Product_Variable) get_price_suffix(price string, qty i64) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('strstr', [var_suffix.dup(), rt.new_string('{')])) {
		return rt.call_function('apply_filters', [rt.new_string('woocommerce_get_price_suffix'), rt.new_string(''), rt.new_object('WC_Product_Variable', ['WC_Product'], &this), rt.new_string(price_mutated).dup(), rt.new_int(qty)])
	} else {
...
fn (mut this Class_WC_Product_Variable) get_children(visible_only string) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_null(), this.children)) {
		...
		this.set_children(var_children.array_get('all'))
		this.set_visible_children(var_children.array_get('visible'))
	}
...
fn (mut this Class_WC_Product_Variable) get_variation_default_attribute(var_attribute_name rt.PhpVal) rt.PhpVal {
	...
	var_attribute_name_mutated = rt.call_function('sanitize_title', [var_attribute_name_mutated.dup()])
...
fn (mut this Class_WC_Product_Variable) get_available_variations(return string) rt.PhpVal {
	...
	}
	{
		...
		for {
			...
			mut var_variation := rt.call_function('wc_get_product', [var_variation_id.dup()])
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_variation)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_variation, 'exists', []rt.PhpVal{}))))))) || rt.is_true(rt.new_bool(rt.is_true(var_hide_out_of_stock_items) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_variation, 'is_in_stock', []rt.PhpVal{}))))))))) {
				continue
			}
			if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('apply_filters', [rt.new_string('woocommerce_hide_invisible_variations'), rt.new_bool(true), this.get_id(), var_variation.dup()])) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_variation, 'variation_is_visible', []rt.PhpVal{}))))))) {
				continue
			}
			if rt.is_true(rt.identical(rt.new_string('array'), rt.new_string(return))) {
				var_available_variations.array_push(this.get_available_variation(var_variation.dup()))
			} else {
				var_available_variations.array_push(var_variation.dup())
			}
		}
	}
	if rt.is_true(rt.identical(rt.new_string('array'), rt.new_string(return))) {
		var_available_variations = rt.call_function('array_values', [rt.call_function('array_filter', [var_available_variations.dup()])])
	}
	return var_available_variations.dup()
}
...
fn (mut this Class_WC_Product_Variable) has_purchasable_variations() bool {
	...
	}
	{
		...
		for {
			...
			mut var_variation_id := item_1.val
			mut var_variation := rt.call_function('wc_get_product', [var_variation_id.dup()])
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_variation)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_variation, 'is_purchasable', []rt.PhpVal{}))))))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_variation, 'is_in_stock', []rt.PhpVal{}))))))) {
				continue
			}
			return true
		}
...
fn (mut this Class_WC_Product_Variable) before_data_store_save_or_update() rt.PhpVal {
	mut var_previous_name := rt.get_property(rt.new_object('WC_Product_Variable', ['WC_Product'], &this), 'data').array_get('name')
...
fn (mut this Class_WC_Product_Variable) after_data_store_save_or_update(var_state rt.PhpVal)  {
	rt.call_method(rt.get_property(rt.new_object('WC_Product_Variable', ['WC_Product'], &this), 'data_store'), 'sync_variation_names', [rt.new_object('WC_Product_Variable', ['WC_Product'], &this), var_state.array_get('previous_name'), var_state.array_get('new_name')])
...
fn (mut this Class_WC_Product_Variable) is_on_sale(context string) rt.PhpVal {
	...
	mut var_on_sale := rt.new_bool(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(rt.identical(var_prices.array_get('sale_price'), var_prices.array_get('price')))))
	return if rt.is_true(rt.identical(rt.new_string('view'), rt.new_string(context))) { rt.call_function('apply_filters', [rt.new_string('woocommerce_product_is_on_sale'), var_on_sale.dup(), rt.new_object('WC_Product_Variable', ['WC_Product'], &this)]) } else { var_on_sale }
}
...
fn (mut this Class_WC_Product_Variable) child_has_weight() rt.PhpVal {
	mut var_transient_name := rt.new_string('wc_child_has_weight_' + (this.get_id()).str())
	mut var_has_weight := rt.call_function('get_transient', [var_transient_name.dup()])
	if rt.is_true(rt.identical(rt.new_bool(false), var_has_weight)) {
		...
		rt.call_function('set_transient', [var_transient_name.dup(), // unsupported expression: Expr_Cast_Int, rt.mul(rt.get_constant('DAY_IN_SECONDS'), rt.new_int(30))])
	}
	return // unsupported expression: Expr_Cast_Bool
}

fn (mut this Class_WC_Product_Variable) child_has_dimensions() rt.PhpVal {
	mut var_transient_name := rt.new_string('wc_child_has_dimensions_' + (this.get_id()).str())
	mut var_has_dimension := rt.call_function('get_transient', [var_transient_name.dup()])
...
fn (mut this Class_WC_Product_Variable) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'child_has_weight' {
			return this.child_has_weight()
		}
		'child_has_dimensions' {
			return this.child_has_dimensions()
		}
		'has_dimensions' {
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_product_variable_php() {
