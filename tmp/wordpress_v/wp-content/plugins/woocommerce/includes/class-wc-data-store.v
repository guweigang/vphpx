...
fn (mut this Class_WC_Data_Store) construct(var_object_type rt.PhpVal)  {
	...
	this.object_type = var_object_type_mutated.dup()
	this.stores = rt.call_function('apply_filters', [rt.new_string('woocommerce_data_stores'), this.stores])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.stores.array_isset(var_object_type_mutated.dup())))))) {
		mut var_pieces := rt.call_function('explode', [rt.new_string('-'), var_object_type_mutated.dup()])
		var_object_type_mutated = var_pieces.array_get(0)
	}
	if rt.is_true(rt.new_bool(this.stores.array_isset(var_object_type_mutated.dup()))) {
		mut var_store := rt.call_function('apply_filters', ['woocommerce_' + (var_object_type_mutated).str() + '_data_store', this.stores.array_get(var_object_type_mutated)])
		if rt.is_true(rt.new_bool(var_store.dup().is_object())) {
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_store, 'WC_Object_Data_Store_Interface')))))) {
			...
			this.current_class_name = rt.call_function('get_class', [var_store.dup()])
			this.instance = var_store.dup()
		} else {
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [var_store.dup()]))))) {
				rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.call_function('__', [rt.new_string('Invalid data store.'), rt.new_string('woocommerce')]))))
			}
			this.current_class_name = var_store.dup()
...
fn Class_WC_Data_Store.load(var_object_type rt.PhpVal) rt.PhpVal {
	mut var_object_type_mutated := var_object_type
	return create_wc_data_store(var_object_type_mutated.dup())
}
...
fn (mut this Class_WC_Data_Store) read(var_data rt.PhpVal)  {
	rt.call_method(this.instance, 'read', [var_data.dup()])
}

fn (mut this Class_WC_Data_Store) read_multiple(var_objects rt.PhpVal)  {
	if rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: this.instance }, rt.ArrayItem{ key: none, val: 'read_multiple' }])])) {
		rt.call_method(this.instance, 'read_multiple', [var_objects.dup()])
	} else {
		{
...
fn (mut this Class_WC_Data_Store) create(var_data rt.PhpVal)  {
	rt.call_method(this.instance, 'create', [var_data.dup()])
}

fn (mut this Class_WC_Data_Store) update(var_data rt.PhpVal)  {
	rt.call_method(this.instance, 'update', [var_data.dup()])
}

fn (mut this Class_WC_Data_Store) delete(var_data rt.PhpVal, var_args rt.PhpVal)  {
	rt.call_method(this.instance, 'delete', [var_data.dup(), var_args.dup()])
}
...
fn (mut this Class_WC_Data_Store) magic_call(var_method rt.PhpVal, var_parameters rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: this.instance }, rt.ArrayItem{ key: none, val: var_method }])])) {
		mut var_object := rt.call_function('array_shift', [var_parameters_mutated.dup()])
		var_parameters_mutated = rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: none, val: var_object }]), var_parameters_mutated.dup()])
		return rt.call_method(this.instance, var_method, [var_parameters_mutated.dup()])
	}
	return rt.new_null()
}

fn (mut this Class_WC_Data_Store) has_callable(method string) bool {
	return (rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: this.instance }, rt.ArrayItem{ key: none, val: method }])])).to_bool()
}
...
fn (mut this Class_WC_Data_Store) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'current_class_name' { this.current_class_name = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_data_store_php() {
