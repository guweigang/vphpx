...
fn (mut this Class_WC_Payment_Token_CC) get_display_name(deprecated string) rt.PhpVal {
	mut var_display := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('%1$s ending in %2$s (expires %3$s/%4$s)'), rt.new_string('woocommerce')]), rt.call_function('wc_get_credit_card_type_label', [this.get_card_type('')]), this.get_last4(''), this.get_expiry_month(''), rt.call_function('substr', [this.get_expiry_year(''), rt.new_int(2)])])
...
fn (mut this Class_WC_Payment_Token_CC) set_card_type(var_type rt.PhpVal)  {
	this.set_prop(rt.new_string('card_type'), var_type.dup())
}
...
fn (mut this Class_WC_Payment_Token_CC) set_expiry_year(var_year rt.PhpVal)  {
	this.set_prop(rt.new_string('expiry_year'), var_year.dup())
}
...
fn (mut this Class_WC_Payment_Token_CC) set_expiry_month(var_month rt.PhpVal)  {
	this.set_prop(rt.new_string('expiry_month'), rt.call_function('str_pad', [var_month.dup(), rt.new_int(2), rt.new_string('0'), rt.get_constant('STR_PAD_LEFT')]))
}
...
fn create_wc_payment_token_cc() &Class_WC_Payment_Token_CC {
...
pub fn init_wp_content_plugins_woocommerce_includes_payment_tokens_class_wc_payment_token_cc_php() {
