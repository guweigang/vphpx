...
fn Class_WC_Twenty_Twenty_One.init()  {
	...
	rt.call_function('add_action', [rt.new_string('admin_enqueue_scripts'), rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT }, rt.ArrayItem{ key: none, val: 'enqueue_admin_styles' }])])
...
fn Class_WC_Twenty_Twenty_One.enqueue_styles(var_styles rt.PhpVal) rt.PhpVal {
	...
	var_styles_mutated.array_set('woocommerce-general', rt.create_array([rt.ArrayItem{ key: 'src', val: (rt.call_function('str_replace', [rt.create_array([rt.ArrayItem{ key: none, val: 'http:' }, rt.ArrayItem{ key: none, val: 'https:' }]), rt.new_string(''), rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'plugin_url', []rt.PhpVal{})])).str() + '/assets/css/twenty-twenty-one.css' }, rt.ArrayItem{ key: 'deps', val: '' }, rt.ArrayItem{ key: 'version', val: fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.get_constant(arg_0) }(rt.new_string('WC_VERSION')) }, rt.ArrayItem{ key: 'media', val: 'all' }, rt.ArrayItem{ key: 'has_rtl', val: true }]))
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_twenty_twenty_one_styles'), var_styles_mutated.dup()])
}

fn Class_WC_Twenty_Twenty_One.enqueue_admin_styles()  {
	rt.call_function('wp_enqueue_style', [rt.new_string('woocommerce-twenty-twenty-one-admin'), (rt.call_function('str_replace', [rt.create_array([rt.ArrayItem{ key: none, val: 'http:' }, rt.ArrayItem{ key: none, val: 'https:' }]), rt.new_string(''), rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'plugin_url', []rt.PhpVal{})])).str() + '/assets/css/twenty-twenty-one-admin.css', rt.new_string(''), fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.get_constant(arg_0) }(rt.new_string('WC_VERSION')), rt.new_string('all')])
}
...
fn create_wc_twenty_twenty_one() &Class_WC_Twenty_Twenty_One {
...
pub fn init_wp_content_plugins_woocommerce_includes_theme_support_class_wc_twenty_twenty_one_php() {
