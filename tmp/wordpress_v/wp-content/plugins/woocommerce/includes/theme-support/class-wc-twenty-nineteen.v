...
fn Class_WC_Twenty_Nineteen.init()  {
	...
	rt.call_function('add_theme_support', [rt.new_string('woocommerce'), rt.create_array([rt.ArrayItem{ key: 'thumbnail_image_width', val: 300 }, rt.ArrayItem{ key: 'single_image_width', val: 450 }])])
	rt.call_function('add_action', [rt.new_string('wp'), rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT }, rt.ArrayItem{ key: none, val: 'tweak_theme_features' }])])
	rt.call_function('add_filter', [rt.new_string('twentynineteen_custom_colors_css'), rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT }, rt.ArrayItem{ key: none, val: 'custom_colors_css' }]), rt.new_int(10), rt.new_int(3)])
}
...
fn Class_WC_Twenty_Nineteen.enqueue_styles(var_styles rt.PhpVal) rt.PhpVal {
	...
	var_styles_mutated.array_set('woocommerce-general', rt.create_array([rt.ArrayItem{ key: 'src', val: (rt.call_function('str_replace', [rt.create_array([rt.ArrayItem{ key: none, val: 'http:' }, rt.ArrayItem{ key: none, val: 'https:' }]), rt.new_string(''), rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'plugin_url', []rt.PhpVal{})])).str() + '/assets/css/twenty-nineteen.css' }, rt.ArrayItem{ key: 'deps', val: '' }, rt.ArrayItem{ key: 'version', val: fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.get_constant(arg_0) }(rt.new_string('WC_VERSION')) }, rt.ArrayItem{ key: 'media', val: 'all' }, rt.ArrayItem{ key: 'has_rtl', val: true }]))
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_twenty_nineteen_styles'), var_styles_mutated.dup()])
}

fn Class_WC_Twenty_Nineteen.tweak_theme_features()  {
	if rt.is_true(rt.call_function('is_woocommerce', []rt.PhpVal{})) {
		rt.call_function('add_filter', [rt.new_string('twentynineteen_can_show_post_thumbnail'), rt.new_string('__return_false')])
	}
}
...
fn Class_WC_Twenty_Nineteen.custom_colors_css(var_css rt.PhpVal, var_primary_color rt.PhpVal, var_saturation rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('function_exists', [rt.new_string('register_block_type')])) && rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})))) {
		return var_css.dup()
	}
	mut var_lightness := rt.call_function('absint', [rt.call_function('apply_filters', [rt.new_string('twentynineteen_custom_colors_lightness'), rt.new_int(33)])])
...
fn create_wc_twenty_nineteen() &Class_WC_Twenty_Nineteen {
...
fn (mut this Class_WC_Twenty_Nineteen) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'custom_colors_css' {
			...
			return Class_WC_Twenty_Nineteen.custom_colors_css(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2)
		}
		else { return none }
	}
}
...
pub fn init_wp_content_plugins_woocommerce_includes_theme_support_class_wc_twenty_nineteen_php() {
