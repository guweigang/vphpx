...
fn Class_WC_Twenty_Seventeen.init()  {
	...
	rt.call_function('add_filter', [rt.new_string('twentyseventeen_custom_colors_css'), rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT }, rt.ArrayItem{ key: none, val: 'custom_colors_css' }]), rt.new_int(10), rt.new_int(3)])
	...
	rt.call_function('add_theme_support', [rt.new_string('woocommerce'), rt.create_array([rt.ArrayItem{ key: 'thumbnail_image_width', val: 250 }, rt.ArrayItem{ key: 'single_image_width', val: 350 }])])
}
...
fn Class_WC_Twenty_Seventeen.enqueue_styles(var_styles rt.PhpVal) rt.PhpVal {
	...
	var_styles_mutated.array_set('woocommerce-general', rt.create_array([rt.ArrayItem{ key: 'src', val: (rt.call_function('str_replace', [rt.create_array([rt.ArrayItem{ key: none, val: 'http:' }, rt.ArrayItem{ key: none, val: 'https:' }]), rt.new_string(''), rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'plugin_url', []rt.PhpVal{})])).str() + '/assets/css/twenty-seventeen.css' }, rt.ArrayItem{ key: 'deps', val: '' }, rt.ArrayItem{ key: 'version', val: fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.get_constant(arg_0) }(rt.new_string('WC_VERSION')) }, rt.ArrayItem{ key: 'media', val: 'all' }, rt.ArrayItem{ key: 'has_rtl', val: true }]))
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_twenty_seventeen_styles'), var_styles_mutated.dup()])
}
...
fn create_wc_twenty_seventeen() &Class_WC_Twenty_Seventeen {
...
fn (mut this Class_WC_Twenty_Seventeen) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'custom_colors_css' {
			...
			return Class_WC_Twenty_Seventeen.custom_colors_css(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2)
		}
		else { return none }
	}
}
...
pub fn init_wp_content_plugins_woocommerce_includes_theme_support_class_wc_twenty_seventeen_php() {
