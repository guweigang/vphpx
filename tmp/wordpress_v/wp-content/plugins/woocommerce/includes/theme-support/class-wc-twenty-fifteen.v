...
struct Class_WC_Twenty_Fifteen {
	rt.PhpObjectBase
}

fn Class_WC_Twenty_Fifteen.init()  {
	...
	rt.call_function('add_theme_support', [rt.new_string('woocommerce'), rt.create_array([rt.ArrayItem{ key: 'thumbnail_image_width', val: 200 }, rt.ArrayItem{ key: 'single_image_width', val: 350 }])])
}

fn Class_WC_Twenty_Fifteen.output_content_wrapper()  {
	print('<div id="primary" role="main" class="content-area twentyfifteen"><div id="main" class="site-main t15wc">')
}

fn Class_WC_Twenty_Fifteen.output_content_wrapper_end()  {
	print('</div></div>')
}

fn create_wc_twenty_fifteen() &Class_WC_Twenty_Fifteen {
	mut obj := &Class_WC_Twenty_Fifteen{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
}

fn (mut this Class_WC_Twenty_Fifteen) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			Class_WC_Twenty_Fifteen.init()
			return rt.new_null()
		}
		'output_content_wrapper' {
			Class_WC_Twenty_Fifteen.output_content_wrapper()
			return rt.new_null()
		}
		'output_content_wrapper_end' {
			Class_WC_Twenty_Fifteen.output_content_wrapper_end()
			return rt.new_null()
		}
		else { return none }
	}
...
fn (this &Class_WC_Twenty_Fifteen) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_WC_Twenty_Fifteen) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_includes_theme_support_class_wc_twenty_fifteen_php() {
