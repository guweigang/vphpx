...
fn (mut this Class_WC_CSV_Exporter) get_column_names() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.concat(rt.concat(rt.new_string('woocommerce_'), this.export_type), rt.new_string('_export_column_names')), this.column_names, rt.new_object('WC_CSV_Exporter', []string{}, &this)])
}
...
fn (mut this Class_WC_CSV_Exporter) set_column_names(var_column_names rt.PhpVal)  {
	this.column_names = rt.new_array()
	{
		...
		for {
			...
			this.column_names.array_set(rt.call_function('wc_clean', [var_column_id.dup()]), rt.call_function('wc_clean', [var_column_name.dup()]))
		}
	}
}
...
fn (mut this Class_WC_CSV_Exporter) get_delimiter() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.concat(rt.concat(rt.new_string('woocommerce_'), this.export_type), rt.new_string('_export_delimiter')), this.delimiter])
}

fn (mut this Class_WC_CSV_Exporter) set_columns_to_export(var_columns rt.PhpVal)  {
	mut var_columns_mutated := var_columns
	this.columns_to_export = rt.call_function('array_map', [rt.new_string('wc_clean'), var_columns_mutated.dup()])
}
...
fn (mut this Class_WC_CSV_Exporter) is_column_exporting(var_column_id rt.PhpVal) bool {
	...
	var_column_id_mutated = if rt.is_true(rt.call_function('strstr', [var_column_id_mutated.dup(), rt.new_string(':')])) { rt.call_function('current', [rt.call_function('explode', [rt.new_string(':'), var_column_id_mutated.dup()])]) } else { var_column_id_mutated }
	mut var_columns_to_export := this.get_columns_to_export()
	if !rt.is_true(var_columns_to_export) {
		return true
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('in_array', [var_column_id_mutated.dup(), var_columns_to_export.dup(), rt.new_bool(true)])) || rt.is_true(rt.identical(rt.new_string('meta'), var_column_id_mutated)))) {
		return true
	}
	return false
}
...
fn (mut this Class_WC_CSV_Exporter) export()  {
	...
	this.send_headers()
	this.send_content(rt.new_string((rt.call_function('chr', [rt.new_int(239)])).str() + (rt.call_function('chr', [rt.new_int(187)])).str() + (rt.call_function('chr', [rt.new_int(191)])).str() + (this.export_column_headers()).str() + (this.get_csv_data()).str()))
...
fn (mut this Class_WC_CSV_Exporter) send_headers()  {
	...
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('apache_setenv')])) {
		rt.call_function('apache_setenv', [rt.new_string('no-gzip'), rt.new_int(1)])
		// unsupported statement: Stmt_Nop
	}
	rt.call_function('ini_set', [rt.new_string('zlib.output_compression'), rt.new_string('Off')])
	rt.call_function('ini_set', [rt.new_string('output_buffering'), rt.new_string('Off')])
	rt.call_function('ini_set', [rt.new_string('output_handler'), rt.new_string('')])
	...
	rt.call_function('header', ['Content-Disposition: attachment; filename=' + (this.get_filename()).str()])
...
fn (mut this Class_WC_CSV_Exporter) set_filename(var_filename rt.PhpVal)  {
	this.filename = rt.call_function('sanitize_file_name', [(rt.call_function('str_replace', [rt.new_string('.csv'), rt.new_string(''), var_filename.dup()])).str() + '.csv'])
}

fn (mut this Class_WC_CSV_Exporter) get_filename() rt.PhpVal {
	return rt.call_function('sanitize_file_name', [rt.call_function('apply_filters', [rt.concat(rt.concat(rt.new_string('woocommerce_'), this.export_type), rt.new_string('_export_get_filename')), this.filename])])
}
...
fn (mut this Class_WC_CSV_Exporter) export_column_headers() rt.PhpVal {
	...
	mut var_buffer := rt.call_function('fopen', [rt.new_string('php://output'), rt.new_string('w')])
	rt.call_function('ob_start', []rt.PhpVal{})
	{
		...
		for {
			...
			mut var_column_name := item_1.val
			mut var_column_id := item_1.key
			if !(this.is_column_exporting(var_column_id.dup())) {
				continue
			}
			var_export_row << this.format_data(var_column_name.dup())
		}
	}
	this.fputcsv(var_buffer.dup(), var_export_row.dup())
...
fn (mut this Class_WC_CSV_Exporter) export_rows() rt.PhpVal {
	...
	mut var_buffer := rt.call_function('fopen', [rt.new_string('php://output'), rt.new_string('w')])
	rt.call_function('ob_start', []rt.PhpVal{})
	rt.call_function('array_walk', [var_data.dup(), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_CSV_Exporter', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'export_row' }]), var_buffer.dup()])
	return rt.call_function('apply_filters', [rt.concat(rt.concat(rt.new_string('woocommerce_'), this.export_type), rt.new_string('_export_rows')), rt.call_function('ob_get_clean', []rt.PhpVal{}), rt.new_object('WC_CSV_Exporter', []string{}, &this)])
}
...
fn (mut this Class_WC_CSV_Exporter) export_row(var_row_data rt.PhpVal, var_key rt.PhpVal, var_buffer rt.PhpVal)  {
	...
	this.fputcsv(var_buffer_mutated.dup(), var_export_row.dup())
	rt.pre_inc(this.exported_row_count)
}

fn (mut this Class_WC_CSV_Exporter) get_limit() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.concat(rt.concat(rt.new_string('woocommerce_'), this.export_type), rt.new_string('_export_batch_limit')), this.limit, rt.new_object('WC_CSV_Exporter', []string{}, &this)])
}

fn (mut this Class_WC_CSV_Exporter) set_limit(var_limit rt.PhpVal)  {
	this.limit = rt.call_function('absint', [var_limit.dup()])
}
...
fn (mut this Class_WC_CSV_Exporter) escape_data(var_data rt.PhpVal) rt.PhpVal {
	...
	mut var_active_content_triggers := [rt.new_string('='), rt.new_string('+'), rt.new_string('-'), rt.new_string('@'), rt.call_function('chr', [rt.new_int(9)]), rt.call_function('chr', [rt.new_int(13)])]
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_data_mutated.dup().is_long())) || rt.is_true(rt.new_bool(var_data_mutated.dup().is_double())))) {
		return var_data_mutated.dup()
	}
	if rt.is_true(rt.call_function('in_array', [rt.call_function('mb_substr', [var_data_mutated.dup(), rt.new_int(0), rt.new_int(1)]), var_active_content_triggers.dup(), rt.new_bool(true)])) {
...
fn (mut this Class_WC_CSV_Exporter) format_data(var_data rt.PhpVal) rt.PhpVal {
	mut var_data_mutated := var_data
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_scalar', [var_data_mutated.dup()]))))) {
		if rt.is_true(rt.call_function('is_a', [var_data_mutated.dup(), rt.new_string('WC_Datetime')])) {
			var_data_mutated = rt.call_method(var_data_mutated, 'date', [rt.new_string('Y-m-d G:i:s')])
		} else {
			var_data_mutated = rt.new_string(rt.new_string(''))
			// unsupported statement: Stmt_Nop
		}
	} else if rt.is_true(rt.new_bool(var_data_mutated.dup().is_bool())) {
		var_data_mutated = rt.new_int(if rt.is_true(var_data_mutated) { rt.new_int(1) } else { rt.new_int(0) })
	}
	mut var_use_mb := rt.call_function('function_exists', [rt.new_string('mb_convert_encoding')])
	if rt.is_true(var_use_mb) {
		mut var_is_valid_utf_8 := rt.call_function('mb_check_encoding', [var_data_mutated.dup(), rt.new_string('UTF-8')])
		if rt.is_true(rt.new_bool(!(rt.is_true(var_is_valid_utf_8)))) {
			var_data_mutated = rt.call_function('mb_convert_encoding', [var_data_mutated.dup(), rt.new_string('UTF-8'), rt.new_string('ISO-8859-1')])
		}
	}
	return this.escape_data(var_data_mutated.dup())
}
...
fn (mut this Class_WC_CSV_Exporter) format_term_ids(var_term_ids rt.PhpVal, var_taxonomy rt.PhpVal) string {
	...
	var_term_ids_mutated = rt.call_function('wp_parse_id_list', [var_term_ids_mutated.dup()])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_int(var_term_ids_mutated.dup().array_count()))))) {
		return ''
	}
	mut var_formatted_terms := rt.new_array()
	if rt.is_true(rt.call_function('is_taxonomy_hierarchical', [var_taxonomy.dup()])) {
		{
			mut iter_1 := var_term_ids_mutated.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_term_id := item_1.val
				mut var_formatted_term := rt.new_array()
				mut var_ancestor_ids := rt.call_function('array_reverse', [rt.call_function('get_ancestors', [var_term_id.dup(), var_taxonomy.dup()])])
				{
					mut iter_2 := var_ancestor_ids.iterator()
					for {
						item_2 := iter_2.next() or { break }
						mut var_ancestor_id := item_2.val
						mut var_term := rt.call_function('get_term', [var_ancestor_id.dup(), var_taxonomy.dup()])
				...
				var_formatted_terms << rt.call_function('implode', [rt.new_string(' > '), var_formatted_term.dup()])
			}
		}
	} else {
		{
	...
	return (this.implode_values(var_formatted_terms.dup())).str()
}
...
fn (mut this Class_WC_CSV_Exporter) implode_values(var_values rt.PhpVal) rt.PhpVal {
	mut var_values_to_implode := rt.new_array()
	{
		...
		for {
			...
			var_value = if rt.is_true(// unsupported expression: Expr_Cast_String) { rt.call_function('html_entity_decode', [var_value.dup(), rt.get_constant('ENT_QUOTES')]) } else { rt.new_string('') }
			var_values_to_implode << rt.call_function('str_replace', [rt.new_string(','), rt.new_string('\\,'), var_value.dup()])
		}
	}
	return rt.call_function('implode', [rt.new_string(', '), var_values_to_implode.dup()])
}
...
fn (mut this Class_WC_CSV_Exporter) fputcsv(var_buffer rt.PhpVal, var_export_row rt.PhpVal)  {
	...
	rt.call_function('fputcsv', [var_buffer_mutated.dup(), var_export_row_mutated.dup(), this.get_delimiter(), rt.new_string('"'), rt.new_string('')])
	// unsupported statement: Stmt_Nop
}
...
fn (mut this Class_WC_CSV_Exporter) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'limit' { this.limit = val; return true }
		'exported_row_count' { this.exported_row_count = val; return true }
		'row_data' { this.row_data = val; return true }
		'total_rows' { this.total_rows = val; return true }
		'column_names' { this.column_names = val; return true }
		'columns_to_export' { this.columns_to_export = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_includes_export_abstract_wc_csv_exporter_php() {
