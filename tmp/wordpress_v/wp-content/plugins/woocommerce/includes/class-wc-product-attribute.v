...
fn (mut this Class_WC_Product_Attribute) get_taxonomy_object() rt.PhpVal {
	...
	return if rt.is_true(this.is_taxonomy()) { var_wc_product_attributes.array_get(this.get_name()) } else { rt.new_null() }
}

fn (mut this Class_WC_Product_Attribute) get_terms() rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(this.is_taxonomy())))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('taxonomy_exists', [this.get_name()]))))))) {
		return rt.new_null()
	}
	mut var_terms := []rt.PhpVal{}
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(var_option.dup().is_long())) {
				mut var_term := rt.call_function('get_term_by', [rt.new_string('id'), var_option.dup(), this.get_name()])
			} else {
				var_term = rt.call_function('get_term_by', [rt.new_string('name'), var_option.dup(), this.get_name()])
				if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_term)))) || rt.is_true(rt.call_function('is_wp_error', [var_term.dup()])))) {
					mut var_new_term := rt.call_function('wp_insert_term', [var_option.dup(), this.get_name()])
					var_term = if rt.is_true(rt.call_function('is_wp_error', [var_new_term.dup()])) { rt.new_bool(false) } else { rt.call_function('get_term_by', [rt.new_string('id'), var_new_term.array_get('term_id'), this.get_name()]) }
				}
			}
...
fn (mut this Class_WC_Product_Attribute) get_data() rt.PhpVal {
	return rt.call_function('array_merge', [this.extra_data, this.data, rt.create_array([rt.ArrayItem{ key: 'is_visible', val: if rt.is_true(this.get_visible()) { 1 } else { 0 } }, rt.ArrayItem{ key: 'is_variation', val: if rt.is_true(this.get_variation()) { 1 } else { 0 } }, rt.ArrayItem{ key: 'is_taxonomy', val: if rt.is_true(this.is_taxonomy()) { 1 } else { 0 } }, rt.ArrayItem{ key: 'value', val: if rt.is_true(this.is_taxonomy()) { rt.new_string('') } else { rt.call_function('wc_implode_text_attributes', [this.get_options()]) } }])])
}

fn (mut this Class_WC_Product_Attribute) set_extra_data(key string, var_value rt.PhpVal)  {
	this.extra_data.array_set(key, var_value.dup())
}

fn (mut this Class_WC_Product_Attribute) set_id(var_value rt.PhpVal)  {
	this.data.array_set('id', rt.call_function('absint', [var_value.dup()]))
}

fn (mut this Class_WC_Product_Attribute) set_name(var_value rt.PhpVal)  {
	this.data.array_set('name', var_value.dup())
}

fn (mut this Class_WC_Product_Attribute) set_options(var_value rt.PhpVal)  {
	this.data.array_set('options', var_value.dup())
}

fn (mut this Class_WC_Product_Attribute) set_position(var_value rt.PhpVal)  {
	this.data.array_set('position', rt.call_function('absint', [var_value.dup()]))
}

fn (mut this Class_WC_Product_Attribute) set_visible(var_value rt.PhpVal)  {
	this.data.array_set('visible', rt.call_function('wc_string_to_bool', [var_value.dup()]))
}

fn (mut this Class_WC_Product_Attribute) set_variation(var_value rt.PhpVal)  {
	this.data.array_set('variation', rt.call_function('wc_string_to_bool', [var_value.dup()]))
}
...
fn (mut this Class_WC_Product_Attribute) get_options() rt.PhpVal {
	return this.data.array_get('options')
}

fn (mut this Class_WC_Product_Attribute) get_position() rt.PhpVal {
	return this.data.array_get('position')
}

fn (mut this Class_WC_Product_Attribute) get_visible() rt.PhpVal {
	return this.data.array_get('visible')
}

fn (mut this Class_WC_Product_Attribute) get_variation() rt.PhpVal {
	return this.data.array_get('variation')
}

fn (mut this Class_WC_Product_Attribute) offsetget(var_offset rt.PhpVal) string {
	mut switch_val_1 := var_offset
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('is_variation'))) {
		return if rt.is_true(this.get_variation()) { 1 } else { 0 }
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('is_visible'))) {
		return if rt.is_true(this.get_visible()) { 1 } else { 0 }
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('is_taxonomy'))) {
		return if rt.is_true(this.is_taxonomy()) { 1 } else { 0 }
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('value'))) {
		return (if rt.is_true(this.is_taxonomy()) { rt.new_string('') } else { rt.call_function('wc_implode_text_attributes', [this.get_options()]) }).str()
	} else {
		if rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_Product_Attribute', ['ArrayAccess'], &this) }, rt.ArrayItem{ key: none, val: "get_${var_offset.to_string()}" }])])) {
			return (rt.call_method(rt.new_object('WC_Product_Attribute', ['ArrayAccess'], &this), "get_${var_offset.to_string()}", []rt.PhpVal{})).str()
		}
...
fn (mut this Class_WC_Product_Attribute) offsetset(var_offset rt.PhpVal, var_value rt.PhpVal)  {
	...
	if rt.is_true(rt.equal(switch_val_2, rt.new_string('is_variation'))) {
		this.set_variation(var_value.dup())
	} else if rt.is_true(rt.equal(switch_val_2, rt.new_string('is_visible'))) {
		this.set_visible(var_value.dup())
	} else if rt.is_true(rt.equal(switch_val_2, rt.new_string('value'))) {
		this.set_options(var_value.dup())
	} else {
		if rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_Product_Attribute', ['ArrayAccess'], &this) }, rt.ArrayItem{ key: none, val: "set_${var_offset.to_string()}" }])])) {
			rt.call_method(rt.new_object('WC_Product_Attribute', ['ArrayAccess'], &this), "set_${var_offset.to_string()}", [var_value.dup()])
			break
		}
		this.extra_data.array_set(var_offset, var_value.dup())
	}
}
...
fn (mut this Class_WC_Product_Attribute) offsetexists(var_offset rt.PhpVal) rt.PhpVal {
	return rt.call_function('in_array', [var_offset.dup(), rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: none, val: 'is_variation' }, rt.ArrayItem{ key: none, val: 'is_visible' }, rt.ArrayItem{ key: none, val: 'is_taxonomy' }, rt.ArrayItem{ key: none, val: 'value' }]), rt.func_array_keys(this.data), rt.func_array_keys(this.extra_data)]), rt.new_bool(true)])
}
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_product_attribute_php() {
