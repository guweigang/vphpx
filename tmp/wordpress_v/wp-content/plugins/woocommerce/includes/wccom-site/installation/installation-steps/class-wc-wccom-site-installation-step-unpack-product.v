...
fn (mut this Class_WC_WCCOM_Site_Installation_Step_Unpack_Product) run() rt.PhpVal {
	...
	mut var_unpacked_path := rt.call_method(var_upgrader, 'unpack_package', [rt.call_method(this.state, 'get_download_path', []rt.PhpVal{}), rt.new_bool(true)])
	if !rt.is_true(var_unpacked_path) {
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{}, create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.missing_unpacked_path())))
	}
	rt.call_method(this.state, 'set_unpacked_path', [var_unpacked_path.dup()])
	return this.state
}
...
pub fn init_wp_content_plugins_woocommerce_includes_wccom_site_installation_installation_steps_class_wc_wccom_site_installation_step_unpack_product_php() {
