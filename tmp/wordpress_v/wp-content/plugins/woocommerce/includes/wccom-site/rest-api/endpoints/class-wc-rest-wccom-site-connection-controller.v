...
fn (mut this Class_WC_REST_WCCOM_Site_Connection_Controller) register_routes()  {
	rt.call_function('register_rest_route', [rt.get_property(rt.new_object('WC_REST_WCCOM_Site_Connection_Controller', ['WC_REST_WCCOM_Site_Controller'], &this), 'namespace'), '/' + (this.rest_base).str() + '/disconnect', rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.editable() }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_WCCOM_Site_Connection_Controller', ['WC_REST_WCCOM_Site_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'handle_disconnect_request' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_WCCOM_Site_Connection_Controller', ['WC_REST_WCCOM_Site_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'check_permission' }]) }]) }])])
	rt.call_function('register_rest_route', [rt.get_property(rt.new_object('WC_REST_WCCOM_Site_Connection_Controller', ['WC_REST_WCCOM_Site_Controller'], &this), 'namespace'), '/' + (this.rest_base).str() + '/status', rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.readable() }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_WCCOM_Site_Connection_Controller', ['WC_REST_WCCOM_Site_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'handle_status_request' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_WCCOM_Site_Connection_Controller', ['WC_REST_WCCOM_Site_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'check_permission' }]) }]) }])])
}
...
fn (mut this Class_WC_REST_WCCOM_Site_Connection_Controller) handle_disconnect_request(var_request rt.PhpVal) rt.PhpVal {
	mut var_request_hash := var_request.array_get('hash')
	if rt.is_true(rt.new_bool(!rt.is_true(var_request_hash) || rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Helper{}; return temp.verify_request_hash(arg_0) }(var_request_hash.dup()))))))) {
...
fn (mut this Class_WC_REST_WCCOM_Site_Connection_Controller) handle_status_request() rt.PhpVal {
	...
	if !rt.is_true(var_auth.array_get('access_token')) || !rt.is_true(var_auth.array_get('access_token_secret')) {
		return this.get_response(rt.create_array([rt.ArrayItem{ key: Class_WC_REST_WCCOM_Site_Connection_Controller.connection_data_found(), val: false }, rt.ArrayItem{ key: Class_WC_REST_WCCOM_Site_Connection_Controller.connection_valid(), val: false }]))
	}
	mut var_connection_data := fn () rt.PhpVal { mut temp := Class_WC_Helper{}; return temp.fetch_helper_connection_info() }()
	if rt.is_true(rt.new_bool(rt.instance_of(var_connection_data, 'WP_Error'))) {
		return var_connection_data.dup()
	}
	if rt.is_true(rt.identical(rt.new_null(), var_connection_data)) {
		return this.get_response(rt.create_array([rt.ArrayItem{ key: Class_WC_REST_WCCOM_Site_Connection_Controller.connection_data_found(), val: true }, rt.ArrayItem{ key: Class_WC_REST_WCCOM_Site_Connection_Controller.connection_valid(), val: false }]))
	}
	return this.get_response(rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: Class_WC_REST_WCCOM_Site_Connection_Controller.connection_data_found(), val: true }, rt.ArrayItem{ key: Class_WC_REST_WCCOM_Site_Connection_Controller.connection_valid(), val: true }]), var_connection_data.dup()]))
}
...
fn create_wc_rest_wccom_site_connection_controller() &Class_WC_REST_WCCOM_Site_Connection_Controller {
...
pub fn init_wp_content_plugins_woocommerce_includes_wccom_site_rest_api_endpoints_class_wc_rest_wccom_site_connection_controller_php() {
