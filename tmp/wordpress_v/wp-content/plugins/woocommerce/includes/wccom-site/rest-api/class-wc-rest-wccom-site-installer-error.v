...
fn (mut this Class_WC_REST_WCCOM_Site_Installer_Error) construct(var_error_code rt.PhpVal, var_error_message rt.PhpVal, var_http_code rt.PhpVal)  {
	this.dispatch_set_prop('error_code', var_error_code.dup())
	this.dispatch_set_prop('error_message', if !(var_error_message).is_null() { var_error_message } else { if !(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.error_messages().array_get(var_error_code)).is_null() { Class_WC_REST_WCCOM_Site_Installer_Error_Codes.error_messages().array_get(var_error_code) } else { rt.new_string('') } })
	this.dispatch_set_prop('http_code', if !(var_http_code).is_null() { var_http_code } else { if !(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.http_codes().array_get(var_error_code)).is_null() { Class_WC_REST_WCCOM_Site_Installer_Error_Codes.http_codes().array_get(var_error_code) } else { rt.new_int(400) } })
	this.Class_Exception.construct(var_error_code.dup())
}

fn (mut this Class_WC_REST_WCCOM_Site_Installer_Error) get_error_code() rt.PhpVal {
	return rt.get_property(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', ['Exception'], &this), 'error_code')
}

fn (mut this Class_WC_REST_WCCOM_Site_Installer_Error) get_error_message() rt.PhpVal {
	return rt.get_property(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', ['Exception'], &this), 'error_message')
}

fn (mut this Class_WC_REST_WCCOM_Site_Installer_Error) get_http_code() rt.PhpVal {
	return rt.get_property(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', ['Exception'], &this), 'http_code')
}
...
pub fn init_wp_content_plugins_woocommerce_includes_wccom_site_rest_api_class_wc_rest_wccom_site_installer_error_php() {
