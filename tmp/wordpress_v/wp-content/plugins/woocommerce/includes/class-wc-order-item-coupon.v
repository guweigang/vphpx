...
fn (mut this Class_WC_Order_Item_Coupon) set_name(var_value rt.PhpVal) rt.PhpVal {
	this.set_code(var_value.dup())
	return rt.new_null()
}

fn (mut this Class_WC_Order_Item_Coupon) set_code(var_value rt.PhpVal)  {
	this.set_prop(rt.new_string('code'), rt.call_function('wc_format_coupon_code', [var_value.dup()]))
}

fn (mut this Class_WC_Order_Item_Coupon) set_discount(var_value rt.PhpVal)  {
	this.set_prop(rt.new_string('discount'), rt.call_function('wc_format_decimal', [var_value.dup()]))
}

fn (mut this Class_WC_Order_Item_Coupon) set_discount_tax(var_value rt.PhpVal)  {
	this.set_prop(rt.new_string('discount_tax'), rt.call_function('wc_format_decimal', [var_value.dup()]))
}
...
fn (mut this Class_WC_Order_Item_Coupon) offsetget(var_offset rt.PhpVal) rt.PhpVal {
	...
	rt.call_function('wc_deprecated_function', [rt.new_string('WC_Order_Item_Coupon::offsetGet'), rt.new_string('4.4.0'), rt.new_string('')])
	if rt.is_true(rt.identical(rt.new_string('discount_amount'), var_offset_mutated)) {
		var_offset_mutated = rt.new_string(rt.new_string('discount'))
	} else if rt.is_true(rt.identical(rt.new_string('discount_amount_tax'), var_offset_mutated)) {
		var_offset_mutated = rt.new_string(rt.new_string('discount_tax'))
	}
...
fn (mut this Class_WC_Order_Item_Coupon) offsetset(var_offset rt.PhpVal, var_value rt.PhpVal)  {
	mut var_offset_mutated := var_offset
	rt.call_function('wc_deprecated_function', [rt.new_string('WC_Order_Item_Coupon::offsetSet'), rt.new_string('4.4.0'), rt.new_string('')])
	if rt.is_true(rt.identical(rt.new_string('discount_amount'), var_offset_mutated)) {
		var_offset_mutated = rt.new_string(rt.new_string('discount'))
	} else if rt.is_true(rt.identical(rt.new_string('discount_amount_tax'), var_offset_mutated)) {
		var_offset_mutated = rt.new_string(rt.new_string('discount_tax'))
...
fn (mut this Class_WC_Order_Item_Coupon) offsetexists(var_offset rt.PhpVal) bool {
	mut var_offset_mutated := var_offset
	if rt.is_true(rt.call_function('in_array', [var_offset_mutated.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'discount_amount' }, rt.ArrayItem{ key: none, val: 'discount_amount_tax' }]), rt.new_bool(true)])) {
		return true
	}
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_order_item_coupon_php() {
