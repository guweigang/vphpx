...
fn (mut this Class_WC_Email_Customer_Reset_Password) trigger(user_login string, reset_key string) {
	...
	if var_user_login.len > 0 && var_user_login != '0' && var_reset_key.len > 0
		...
		this.user_login = rt.new_string(user_login).dup()
		this.reset_key = rt.new_string(reset_key).dup()
...
pub fn init_wp_content_plugins_woocommerce_includes_emails_class_wc_email_customer_reset_password_php() {
	...
	return create_wc_email_customer_reset_password()
}
