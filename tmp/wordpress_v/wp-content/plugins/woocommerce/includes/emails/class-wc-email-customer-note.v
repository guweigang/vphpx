...
fn (mut this Class_WC_Email_Customer_Note) trigger(var_args rt.PhpVal) {
	...
	if !(!rt.is_true(var_args_mutated)) {
		...
		var_args_mutated = rt.call_function('wp_parse_args', [
			var_args_mutated.dup(), var_defaults.dup()])
		mut var_order_id := var_args_mutated.array_get('order_id')
		mut var_customer_note := var_args_mutated.array_get('customer_note')
		if rt.is_true(var_order_id) {
			...
			], &this), 'object'))
			{
				...
				this.customer_note = var_customer_note.dup()
...
pub fn init_wp_content_plugins_woocommerce_includes_emails_class_wc_email_customer_note_php() {
	...
	return create_wc_email_customer_note()
}
