...
struct Class_WC_Email_Customer_Cancelled_Order {
	rt.PhpObjectBase
}

fn (mut this Class_WC_Email_Customer_Cancelled_Order) construct() {
	this.dispatch_set_prop('id', rt.new_string('customer_cancelled_order'))
	this.dispatch_set_prop('customer_email', rt.new_bool(true))
	this.dispatch_set_prop('title', rt.call_function('__', [
		rt.new_string('Cancelled order'),
		rt.new_string('woocommerce'),
	]))
	this.dispatch_set_prop('email_group', rt.new_string('order-changes'))
	this.dispatch_set_prop('template_html', rt.new_string('emails/customer-cancelled-order.php'))
	this.dispatch_set_prop('template_plain',
		rt.new_string('emails/plain/customer-cancelled-order.php'))
	this.dispatch_set_prop('placeholders', rt.create_array([
		rt.ArrayItem{ key: '{order_date}', val: '' },
		rt.ArrayItem{ key: '{order_number}', val: '' },
		rt.ArrayItem{ key: '{order_billing_full_name}', val: '' },
	]))
	rt.call_function('add_action', [
		rt.new_string('woocommerce_order_status_processing_to_cancelled_notification'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Email_Customer_Cancelled_Order', [
				'WC_Email',
			], &this) },
	...
	rt.call_function('add_action', [
		rt.new_string('woocommerce_order_status_on-hold_to_cancelled_notification'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Email_Customer_Cancelled_Order', [
				'WC_Email',
			], &this) },
	...
	this.dispatch_set_prop('description', if rt.is_true(rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
		'WC_Email',
	], &this), 'email_improvements_enabled'))
	{ rt.call_function('__', [
			rt.new_string('Send an email to customers notifying them when their order has been cancelled'),
			rt.new_string('woocommerce'),
		]) } else { rt.call_function('__', [
			rt.new_string('Cancelled order emails are sent to customers when their orders have been marked cancelled (if they were previously processing or on-hold).'),
			rt.new_string('woocommerce'),
		]) })
	if rt.is_true(rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
		'WC_Email',
	], &this), 'block_email_editor_enabled'))
	{
		this.dispatch_set_prop('title', rt.call_function('__', [
			rt.new_string('Order cancelled'),
			rt.new_string('woocommerce'),
		]))
		this.dispatch_set_prop('description', rt.call_function('__', [
			rt.new_string('Notifies customers when their order has been cancelled.'),
			rt.new_string('woocommerce'),
		]))
	}
}

fn (mut this Class_WC_Email_Customer_Cancelled_Order) get_default_subject() rt.PhpVal {
	return rt.call_function('__', [
		rt.new_string('[{site_title}]: Your order #{order_number} has been cancelled'),
		rt.new_string('woocommerce'),
	])
}

fn (mut this Class_WC_Email_Customer_Cancelled_Order) get_default_heading() rt.PhpVal {
	return if rt.is_true(rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
		'WC_Email',
	], &this), 'email_improvements_enabled'))
	{ rt.call_function('__', [rt.new_string('Order cancelled: #{order_number}'),
			rt.new_string('woocommerce')]) } else { rt.call_function('__', [
			rt.new_string('Order Cancelled: #{order_number}'),
			rt.new_string('woocommerce'),
		]) }
}

fn (mut this Class_WC_Email_Customer_Cancelled_Order) trigger(var_order_id rt.PhpVal, order bool) {
		...
		rt.new_string('WC_Order')]))
	{
		...
		this.dispatch_set_prop('recipient', rt.call_method(rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
			'WC_Email',
		], &this), 'object'), 'get_billing_email', []rt.PhpVal{}))
		rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
			'WC_Email',
		], &this), 'placeholders').array_set('{order_date}', rt.call_function('wc_format_datetime', [
			rt.call_method(rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
				'WC_Email',
			], &this), 'object'), 'get_date_created', []rt.PhpVal{}),
		]))
		rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
			'WC_Email',
		], &this), 'placeholders').array_set('{order_number}', rt.call_method(rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
			'WC_Email',
		], &this), 'object'), 'get_order_number', []rt.PhpVal{}))
		rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
			'WC_Email',
		], &this), 'placeholders').array_set('{order_billing_full_name}', rt.call_method(rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
			'WC_Email',
		], &this), 'object'), 'get_formatted_billing_full_name', []rt.PhpVal{}))
	}
...
fn (mut this Class_WC_Email_Customer_Cancelled_Order) get_content_html() rt.PhpVal {
	return rt.call_function('wc_get_template_html', [
		rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
			'WC_Email',
		], &this), 'template_html'),
		rt.create_array([
			rt.ArrayItem{ key: 'order', val: rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
				'WC_Email',
			], &this), 'object') },
			...
			rt.ArrayItem{ key: 'email', val: rt.new_object('WC_Email_Customer_Cancelled_Order', [
				'WC_Email',
			], &this) },
		]),
	])
...
fn (mut this Class_WC_Email_Customer_Cancelled_Order) get_content_plain() rt.PhpVal {
	return rt.call_function('wc_get_template_html', [
		rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
			'WC_Email',
		], &this), 'template_plain'),
...
fn (mut this Class_WC_Email_Customer_Cancelled_Order) get_block_editor_email_template_content() rt.PhpVal {
	return rt.call_function('wc_get_template_html', [
		rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
			'WC_Email',
...
fn (mut this Class_WC_Email_Customer_Cancelled_Order) get_default_additional_content() rt.PhpVal {
	return rt.call_function('__', [rt.new_string('We hope to see you again soon.'),
		rt.new_string('woocommerce')])
}

fn (mut this Class_WC_Email_Customer_Cancelled_Order) init_form_fields() {
	mut var_placeholder_text := rt.call_function('sprintf', [
		rt.call_function('__', [rt.new_string('Available placeholders: %s'),
			rt.new_string('woocommerce')]),
		'<code>' +
			(rt.call_function('esc_html', [rt.call_function('implode', [rt.new_string('</code>, <code>'), rt.func_array_keys(rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', ['WC_Email'], &this), 'placeholders'))])])).str() +
			'</code>',
	])
	this.dispatch_set_prop('form_fields', rt.create_array([
		rt.ArrayItem{ key: 'enabled', val: rt.create_array([
			...
			rt.ArrayItem{
				key: 'default'
				val: if rt.is_true(rt.get_property(rt.new_object('WC_Email_Customer_Cancelled_Order', [
					'WC_Email',
				], &this), 'block_email_editor_enabled'))
				{ 'yes' } else { 'no' }
			},
...
fn create_wc_email_customer_cancelled_order() &Class_WC_Email_Customer_Cancelled_Order {
	mut obj := &Class_WC_Email_Customer_Cancelled_Order{
...
fn (mut this Class_WC_Email_Customer_Cancelled_Order) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			this.construct()
			return rt.new_null()
...
fn (this &Class_WC_Email_Customer_Cancelled_Order) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_WC_Email_Customer_Cancelled_Order) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_includes_emails_class_wc_email_customer_cancelled_order_php() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('WC_Email_Customer_Cancelled_Order'),
		rt.new_bool(false),
	])))))
	{
	}
	return create_wc_email_customer_cancelled_order()
}
