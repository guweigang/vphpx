...
fn (mut this Class_WC_Email_Customer_New_Account) trigger(var_user_id rt.PhpVal, user_pass string, password_generated bool) {
	...
	if rt.is_true(var_user_id) {
		this.dispatch_set_prop('object', create_wp_user(var_user_id.dup()))
		...
		this.user_pass = rt.new_string(user_pass).dup()
		this.password_generated = rt.new_bool(password_generated).dup()
	}
...
fn (mut this Class_WC_Email_Customer_New_Account) generate_set_password_url() rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_key.dup()])) {
...
pub fn init_wp_content_plugins_woocommerce_includes_emails_class_wc_email_customer_new_account_php() {
	...
	return create_wc_email_customer_new_account()
}
