...
fn (mut this Class_WC_DateTime) set_utc_offset(var_offset rt.PhpVal)  {
	this.utc_offset = var_offset.dup().to_i64()
}

fn (mut this Class_WC_DateTime) getoffset() rt.PhpVal {
	return if rt.is_true(this.utc_offset) { this.utc_offset } else { this.Class_DateTime.getoffset() }
}

fn (mut this Class_WC_DateTime) settimezone(var_timezone rt.PhpVal) rt.PhpVal {
	this.utc_offset = 0
	return this.Class_DateTime.settimezone(var_timezone.dup())
}

fn (mut this Class_WC_DateTime) gettimestamp() rt.PhpVal {
	return if rt.is_true(rt.call_function('method_exists', [rt.new_string('DateTime'), rt.new_string('getTimestamp')])) { this.Class_DateTime.gettimestamp() } else { this.format(rt.new_string('U')) }
}
...
fn (mut this Class_WC_DateTime) date(var_format rt.PhpVal) rt.PhpVal {
	return rt.call_function('gmdate', [var_format.dup(), this.getoffsettimestamp()])
}

fn (mut this Class_WC_DateTime) date_i18n(format string) rt.PhpVal {
	return rt.call_function('date_i18n', [rt.new_string(format), this.getoffsettimestamp()])
}
...
fn (mut this Class_WC_DateTime) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'utc_offset' { this.utc_offset = (val).to_i64(); return true }
...
pub fn init_wp_content_plugins_woocommerce_includes_class_wc_datetime_php() {
