...
fn Class_WC_Blocks_Utils.get_all_blocks_from_page(var_woo_page_name rt.PhpVal) rt.PhpVal {
	mut var_page_id := rt.call_function('wc_get_page_id', [var_woo_page_name.dup()])
	mut var_page := rt.call_function('get_post', [var_page_id.dup()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_page)))) {
		return rt.new_array()
	}
	mut var_blocks := rt.call_function('parse_blocks', [rt.get_property(var_page, 'post_content')])
...
fn Class_WC_Blocks_Utils.get_blocks_from_page(var_block_name rt.PhpVal, var_woo_page_name rt.PhpVal) rt.PhpVal {
	mut var_page_blocks := Class_WC_Blocks_Utils.get_all_blocks_from_page(var_woo_page_name.dup())
	...
	return rt.call_function('array_values', [rt.call_function('array_filter', [var_page_blocks.dup(), rt.new_closure(closure_1_fn)])])
}

fn Class_WC_Blocks_Utils.has_block_in_page(var_page rt.PhpVal, var_block_name rt.PhpVal) bool {
	mut var_page_mutated := var_page
	mut var_page_to_check := rt.call_function('get_post', [var_page_mutated.dup()])
	if rt.is_true(rt.identical(rt.new_null(), var_page_to_check)) {
		return false
	}
	mut var_blocks := rt.call_function('parse_blocks', [rt.get_property(var_page_to_check, 'post_content')])
	{
...
fn create_wc_blocks_utils() &Class_WC_Blocks_Utils {
...
fn (mut this Class_WC_Blocks_Utils) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'has_block_in_page' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return rt.new_bool(Class_WC_Blocks_Utils.has_block_in_page(dispatch_arg_0, dispatch_arg_1))
		}
		else { return none }
	}
}
...
pub fn init_wp_content_plugins_woocommerce_includes_blocks_class_wc_blocks_utils_php() {
