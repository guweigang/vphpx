...
fn (mut this Class_WC_Background_Process) is_queue_empty() bool {
	...
	mut var_key := rt.new_string((rt.call_method(var_wpdb, 'esc_like', [(rt.get_property(rt.new_object('WC_Background_Process', ['WP_Background_Process'], &this), 'identifier')).str() + '_batch_'])).str() + '%')
	mut var_count := rt.call_method(var_wpdb, 'get_var', [rt.call_method(var_wpdb, 'prepare', [rt.new_string("SELECT COUNT(*) FROM ${var_table.to_string()} WHERE ${var_column.to_string()} LIKE %s"), var_key.dup()])])
...
fn (mut this Class_WC_Background_Process) get_batch() rt.PhpVal {
	...
	mut var_key := rt.new_string((rt.call_method(var_wpdb, 'esc_like', [(rt.get_property(rt.new_object('WC_Background_Process', ['WP_Background_Process'], &this), 'identifier')).str() + '_batch_'])).str() + '%')
	mut var_query := rt.call_method(var_wpdb, 'get_row', [rt.call_method(var_wpdb, 'prepare', [rt.new_string("SELECT * FROM ${var_table.to_string()} WHERE ${var_column.to_string()} LIKE %s ORDER BY ${var_key_column.to_string()} ASC LIMIT 1"), var_key.dup()])])
	mut var_batch := create_stdclass()
	rt.set_property(var_batch, 'key', rt.get_property(var_query, '{"nodeType":"Expr_Variable","line":74,"name":"column"}'))
	rt.set_property(var_batch, 'data', rt.call_function('array_filter', [rt.cast_array(rt.call_function('maybe_unserialize', [rt.get_property(var_query, '{"nodeType":"Expr_Variable","line":75,"name":"value_column"}')]))]))
...
fn (mut this Class_WC_Background_Process) get_memory_limit() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_memory_limit)))) || rt.is_true(rt.identical(// unsupported expression: Expr_UnaryMinus, rt.new_int(var_memory_limit.dup().to_i64()))))) {
...
fn (mut this Class_WC_Background_Process) schedule_cron_healthcheck(var_schedules rt.PhpVal) rt.PhpVal {
	...
	mut var_interval := rt.call_function('apply_filters', [(rt.get_property(rt.new_object('WC_Background_Process', ['WP_Background_Process'], &this), 'identifier')).str() + '_cron_interval', rt.new_int(5)])
	if rt.is_true(rt.call_function('property_exists', [rt.new_object('WC_Background_Process', ['WP_Background_Process'], &this), rt.new_string('cron_interval')])) {
		var_interval = rt.call_function('apply_filters', [(rt.get_property(rt.new_object('WC_Background_Process', ['WP_Background_Process'], &this), 'identifier')).str() + '_cron_interval', rt.get_property(rt.new_object('WC_Background_Process', ['WP_Background_Process'], &this), 'cron_interval')])
	}
	var_schedules_mutated.array_set((rt.get_property(rt.new_object('WC_Background_Process', ['WP_Background_Process'], &this), 'identifier')).str() + '_cron_interval', rt.create_array([rt.ArrayItem{ key: 'interval', val: rt.mul(rt.get_constant('MINUTE_IN_SECONDS'), var_interval) }, rt.ArrayItem{ key: 'display', val: rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Every %d minutes'), rt.new_string('woocommerce')]), var_interval.dup()]) }]))
...
fn (mut this Class_WC_Background_Process) delete_all_batches() rt.PhpVal {
	...
	mut var_key := rt.new_string((rt.call_method(var_wpdb, 'esc_like', [(rt.get_property(rt.new_object('WC_Background_Process', ['WP_Background_Process'], &this), 'identifier')).str() + '_batch_'])).str() + '%')
	rt.call_method(var_wpdb, 'query', [rt.call_method(var_wpdb, 'prepare', [rt.new_string("DELETE FROM ${var_table.to_string()} WHERE ${var_column.to_string()} LIKE %s"), var_key.dup()])])
...
fn (mut this Class_WC_Background_Process) kill_process()  {
	if !(this.is_queue_empty()) {
		...
		rt.call_function('wp_clear_scheduled_hook', [rt.get_property(rt.new_object('WC_Background_Process', ['WP_Background_Process'], &this), 'cron_hook_identifier')])
	}
}
...
pub fn init_wp_content_plugins_woocommerce_includes_abstracts_class_wc_background_process_php() {
	rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('ABSPATH')])) || rt.is_true(// unsupported expression: Expr_Exit))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [rt.new_string('WP_Async_Request'), rt.new_bool(false)]))))) {
		rt.include_file((rt.call_function('dirname', [rt.get_constant('WC_PLUGIN_FILE')])).str() + '/includes/libraries/wp-async-request.php', '2')
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [rt.new_string('WP_Background_Process'), rt.new_bool(false)]))))) {
		rt.include_file((rt.call_function('dirname', [rt.get_constant('WC_PLUGIN_FILE')])).str() + '/includes/libraries/wp-background-process.php', '2')
	}
}
