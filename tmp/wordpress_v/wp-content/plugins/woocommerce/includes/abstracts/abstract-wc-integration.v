...
fn (mut this Class_WC_Integration) init_settings() {
	...
	this.enabled = if rt.is_true(rt.new_bool(
		!(!rt.is_true(rt.get_property(rt.new_object('WC_Integration', ['WC_Settings_API'], &this), 'settings').array_get('enabled')))
		&& rt.is_true(rt.identical(rt.new_string('yes'), rt.get_property(rt.new_object('WC_Integration', ['WC_Settings_API'], &this), 'settings').array_get('enabled')))))
	{
		'yes'
	} else {
		'no'
...
pub fn init_wp_content_plugins_woocommerce_includes_abstracts_abstract_wc_integration_php() {
