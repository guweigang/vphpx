...
fn (mut this Class_WC_Deprecated_Hooks) construct() {
	...
	rt.call_function('array_walk', [var_new_hooks.dup(),
		rt.create_array([
...
fn (mut this Class_WC_Deprecated_Hooks) get_old_hooks(var_new_hook rt.PhpVal) rt.PhpVal {
	...
	var_old_hooks = if rt.is_true(rt.new_bool(var_old_hooks.dup().is_array())) { var_old_hooks } else { rt.create_array([
			rt.ArrayItem{ key: none, val: var_old_hooks },
		]) }
	return var_old_hooks.dup()
}
...
fn (mut this Class_WC_Deprecated_Hooks) maybe_handle_deprecated_hook() rt.PhpVal {
	...
	mut var_old_hooks := this.get_old_hooks(var_new_hook.dup())
	mut var_new_callback_args := rt.call_function('func_get_args', []rt.PhpVal{})
	mut var_return_value := var_new_callback_args.array_get(0)
	{
		mut iter_1 := var_old_hooks.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_old_hook := item_1.val
			var_return_value = this.handle_deprecated_hook(var_new_hook.dup(), var_old_hook.dup(),
				var_new_callback_args.dup(), var_return_value.dup())
		}
	}
...
fn (mut this Class_WC_Deprecated_Hooks) display_notice(var_old_hook rt.PhpVal, var_new_hook rt.PhpVal) {
	...
	rt.call_function('wc_deprecated_hook', [
		rt.call_function('esc_html', [var_old_hook.dup()]),
		rt.call_function('esc_html', [this.get_deprecated_version(var_old_hook.dup())]),
		rt.call_function('esc_html', [var_new_hook_mutated.dup()]),
	])
}
...
pub fn init_wp_content_plugins_woocommerce_includes_abstracts_abstract_wc_deprecated_hooks_php() {
