...
fn (mut this Class_WC_Payment_Gateway) get_tokens() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})) && rt.is_true(this.supports(Class_Automattic_WooCommerce_Enums_PaymentGatewayFeature.tokenization())))) {
		this.tokens = fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Payment_Tokens{}; return temp.get_customer_tokens(arg_0, arg_1) }(rt.call_function('get_current_user_id', []rt.PhpVal{}), rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'id'))
	}
	return this.tokens
}

fn (mut this Class_WC_Payment_Gateway) get_method_title() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_gateway_method_title'), this.method_title, rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this)])
}

fn (mut this Class_WC_Payment_Gateway) get_method_description() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_gateway_method_description'), this.method_description, rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this)])
}

fn (mut this Class_WC_Payment_Gateway) admin_options()  {
	mut var_offline_payment_gateways := [Class_WC_Gateway_BACS.id(), Class_WC_Gateway_Cheque.id(), Class_WC_Gateway_COD.id()]
	mut var_is_offline_gateway := rt.call_function('in_array', [rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'id'), var_offline_payment_gateways.dup(), rt.new_bool(true)])
	mut var_return_path := rt.new_null()
	if rt.is_true(var_is_offline_gateway) {
		mut var_offline_section := if rt.is_true(rt.call_function('class_exists', [rt.new_string('WC_Settings_Payment_Gateways')])) { Class_WC_Settings_Payment_Gateways.offline_section_name() } else { rt.new_string('offline') }
		var_return_path = rt.new_string('/' + (var_offline_section).str())
	}
	rt.call_function('wc_back_header', [this.get_method_title(), rt.call_function('esc_html__', [rt.new_string('Return to payments'), rt.new_string('woocommerce')]), fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils{}; return temp.wc_payments_settings_url(arg_0) }(var_return_path.dup())])
...
fn (mut this Class_WC_Payment_Gateway) init_settings()  {
	...
	this.enabled = if rt.is_true(rt.new_bool(!(!rt.is_true(rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'settings').array_get('enabled'))) && rt.is_true(rt.identical(rt.new_string('yes'), rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'settings').array_get('enabled'))))) { 'yes' } else { 'no' }
}
...
fn (mut this Class_WC_Payment_Gateway) get_return_url(var_order rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_order_mutated) {
		mut var_return_url := rt.call_method(var_order_mutated, 'get_checkout_order_received_url', []rt.PhpVal{})
	} else {
		var_return_url = rt.call_function('wc_get_endpoint_url', [rt.new_string('order-received'), rt.new_string(''), rt.call_function('wc_get_checkout_url', []rt.PhpVal{})])
	}
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_get_return_url'), var_return_url.dup(), var_order_mutated.dup()])
}

fn (mut this Class_WC_Payment_Gateway) get_transaction_url(var_order rt.PhpVal) rt.PhpVal {
	mut var_order_mutated := var_order
	mut var_return_url := rt.new_string(rt.new_string(''))
	...
	if !(!rt.is_true(this.view_transaction_url)) && !(!rt.is_true(var_transaction_id)) {
		var_return_url = rt.call_function('sprintf', [this.view_transaction_url, var_transaction_id.dup()])
	}
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_get_transaction_url'), var_return_url.dup(), var_order_mutated.dup(), rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this)])
}

fn (mut this Class_WC_Payment_Gateway) get_order_total() rt.PhpVal {
	mut var_total := rt.new_int(rt.new_int(0))
	mut var_order_id := rt.call_function('absint', [rt.call_function('get_query_var', [rt.new_string('order-pay')])])
	if rt.is_true(rt.less(rt.new_int(0), var_order_id)) {
		...
		if rt.is_true(var_order) {
			var_total = // unsupported expression: Expr_Cast_Double
		}
		// unsupported statement: Stmt_Nop
	} else if rt.is_true(rt.less(rt.new_int(0), rt.get_property(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'cart'), 'total'))) {
		var_total = // unsupported expression: Expr_Cast_Double
	}
	return var_total.dup()
}
...
fn (mut this Class_WC_Payment_Gateway) is_available() bool {
	...
	mut var_is_available := rt.new_bool(rt.new_bool(true))
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'cart')) && rt.is_true(rt.less(rt.new_int(0), this.get_order_total())))) && rt.is_true(rt.less(rt.new_int(0), this.max_amount)))) && rt.is_true(rt.less(this.max_amount, this.get_order_total())))) {
		var_is_available = rt.new_bool(rt.new_bool(false))
	}
	return (var_is_available).to_bool()
}

fn (mut this Class_WC_Payment_Gateway) has_fields() rt.PhpVal {
	return // unsupported expression: Expr_Cast_Bool
}

fn (mut this Class_WC_Payment_Gateway) get_title() rt.PhpVal {
	mut var_title := rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_Utilities_HtmlSanitizer.class()]), 'sanitize', [// unsupported expression: Expr_Cast_String, Class_Automattic_WooCommerce_Internal_Utilities_HtmlSanitizer.low_html_balanced_tags_no_links()])
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_gateway_title'), var_title.dup(), rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'id')])
}

fn (mut this Class_WC_Payment_Gateway) get_description() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_gateway_description'), rt.call_function('wp_kses_post', [this.description]), rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'id')])
}

fn (mut this Class_WC_Payment_Gateway) get_icon() rt.PhpVal {
	mut var_icon := rt.new_string(if rt.is_true(this.icon) { '<img src="' + (rt.call_function('esc_url', [fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_HTTPS{}; return temp.force_https_url(arg_0) }(this.icon)])).str() + '" alt="' + (rt.call_function('esc_attr', [this.get_title()])).str() + '" />' } else { rt.new_string('') })
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_gateway_icon'), var_icon.dup(), rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'id')])
}
...
fn (mut this Class_WC_Payment_Gateway) payment_fields()  {
	...
	if rt.is_true(var_description) {
		rt.echo_val(rt.call_function('wpautop', [rt.call_function('wptexturize', [var_description.dup()])]))
		// unsupported statement: Stmt_Nop
	}
...
fn (mut this Class_WC_Payment_Gateway) supports(var_feature rt.PhpVal) rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_payment_gateway_supports'), rt.call_function('in_array', [var_feature.dup(), this.supports, rt.new_bool(true)]), var_feature.dup(), rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this)])
}

fn (mut this Class_WC_Payment_Gateway) can_refund_order(var_order rt.PhpVal) bool {
	mut var_order_mutated := var_order
	return rt.is_true(var_order_mutated) && rt.is_true(this.supports(Class_Automattic_WooCommerce_Enums_PaymentGatewayFeature.refunds()))
}

fn (mut this Class_WC_Payment_Gateway) credit_card_form(var_args rt.PhpVal, var_fields rt.PhpVal)  {
	rt.call_function('wc_deprecated_function', [rt.new_string('credit_card_form'), rt.new_string('2.6'), rt.new_string('WC_Payment_Gateway_CC->form')])
	mut var_cc_form := create_wc_payment_gateway_cc()
	rt.set_property(var_cc_form, 'id', rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'id'))
	rt.set_property(var_cc_form, 'supports', this.supports)
	var_cc_form.form()
}

fn (mut this Class_WC_Payment_Gateway) tokenization_script()  {
	rt.call_function('wp_enqueue_script', [rt.new_string('woocommerce-tokenization-form'), rt.call_function('plugins_url', ['/assets/js/frontend/tokenization-form' + if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.is_true(arg_0) }(rt.new_string('SCRIPT_DEBUG'))) { '' } else { '.min' } + '.js', rt.get_constant('WC_PLUGIN_FILE')]), rt.create_array([rt.ArrayItem{ key: none, val: 'jquery' }]), rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'version'), rt.new_bool(false)])
	rt.call_function('wp_localize_script', [rt.new_string('woocommerce-tokenization-form'), rt.new_string('wc_tokenization_form_params'), rt.create_array([rt.ArrayItem{ key: 'is_registration_required', val: rt.call_method(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'checkout', []rt.PhpVal{}), 'is_registration_required', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'is_logged_in', val: rt.call_function('is_user_logged_in', []rt.PhpVal{}) }])])
}

fn (mut this Class_WC_Payment_Gateway) saved_payment_methods()  {
	mut var_html := rt.new_string('<ul class="woocommerce-SavedPaymentMethods wc-saved-payment-methods" data-count="' + (rt.call_function('esc_attr', [rt.new_int(this.get_tokens().array_count())])).str() + '">')
	{
	...
	// unsupported expression: Expr_AssignOp_Concat
	// unsupported expression: Expr_AssignOp_Concat
	rt.echo_val(rt.call_function('apply_filters', [rt.new_string('wc_payment_gateway_form_saved_payment_methods_html'), var_html.dup(), rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this)]))
	// unsupported statement: Stmt_Nop
}

fn (mut this Class_WC_Payment_Gateway) get_saved_payment_method_option_html(var_token rt.PhpVal) rt.PhpVal {
	mut var_html := rt.call_function('sprintf', [rt.new_string('<li class="woocommerce-SavedPaymentMethods-token">\n\t\t\t\t<input id="wc-%1$s-payment-token-%2$s" type="radio" name="wc-%1$s-payment-token" value="%2$s" style="width:auto;" class="woocommerce-SavedPaymentMethods-tokenInput" %4$s />\n\t\t\t\t<label for="wc-%1$s-payment-token-%2$s">%3$s</label>\n\t\t\t</li>'), rt.call_function('esc_attr', [rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'id')]), rt.call_function('esc_attr', [rt.call_method(var_token, 'get_id', []rt.PhpVal{})]), rt.call_function('esc_html', [rt.call_method(var_token, 'get_display_name', []rt.PhpVal{})]), rt.call_function('checked', [rt.call_method(var_token, 'is_default', []rt.PhpVal{}), rt.new_bool(true), rt.new_bool(false)])])
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_payment_gateway_get_saved_payment_method_option_html'), var_html.dup(), var_token.dup(), rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this)])
}

fn (mut this Class_WC_Payment_Gateway) get_new_payment_method_option_html() rt.PhpVal {
	mut var_label := rt.call_function('apply_filters', [rt.new_string('woocommerce_payment_gateway_get_new_payment_method_option_html_label'), if rt.is_true(this.new_method_label) { this.new_method_label } else { rt.call_function('__', [rt.new_string('Use a new payment method'), rt.new_string('woocommerce')]) }, rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this)])
	mut var_html := rt.call_function('sprintf', [rt.new_string('<li class="woocommerce-SavedPaymentMethods-new">\n\t\t\t\t<input id="wc-%1$s-payment-token-new" type="radio" name="wc-%1$s-payment-token" value="new" style="width:auto;" class="woocommerce-SavedPaymentMethods-tokenInput" />\n\t\t\t\t<label for="wc-%1$s-payment-token-new">%2$s</label>\n\t\t\t</li>'), rt.call_function('esc_attr', [rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'id')]), rt.call_function('esc_html', [var_label.dup()])])
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_payment_gateway_get_new_payment_method_option_html'), var_html.dup(), rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this)])
}

fn (mut this Class_WC_Payment_Gateway) save_payment_method_checkbox()  {
	mut var_html := rt.call_function('sprintf', [rt.new_string('<p class="form-row woocommerce-SavedPaymentMethods-saveNew">\n\t\t\t\t<input id="wc-%1$s-new-payment-method" name="wc-%1$s-new-payment-method" type="checkbox" value="true" style="width:auto;" />\n\t\t\t\t<label for="wc-%1$s-new-payment-method" style="display:inline;">%2$s</label>\n\t\t\t</p>'), rt.call_function('esc_attr', [rt.get_property(rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this), 'id')]), rt.call_function('esc_html__', [rt.new_string('Save to account'), rt.new_string('woocommerce')])])
	rt.echo_val(rt.call_function('apply_filters', [rt.new_string('woocommerce_payment_gateway_save_new_payment_method_option_html'), var_html.dup(), rt.new_object('WC_Payment_Gateway', ['WC_Settings_API'], &this)]))
	// unsupported statement: Stmt_Nop
}

fn (mut this Class_WC_Payment_Gateway) add_payment_method() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: 'result', val: 'failure' }, rt.ArrayItem{ key: 'redirect', val: rt.call_function('wc_get_endpoint_url', [rt.new_string('payment-methods')]) }])
}
...
fn create_wc_payment_gateway_cc() &Class_WC_Payment_Gateway_CC {
...
fn (mut this Class_WC_Payment_Gateway) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'order_button_text' { this.order_button_text = val; return true }
		'has_custom_place_order_button' { this.has_custom_place_order_button = val; return true }
		'enabled' { this.enabled = (val).str(); return true }
		'title' { this.title = val; return true }
		'description' { this.description = val; return true }
		'chosen' { this.chosen = (val).to_bool(); return true }
		'method_title' { this.method_title = val; return true }
		'method_description' { this.method_description = val; return true }
		'has_fields' { this.has_fields = val; return true }
		...
		'max_amount' { this.max_amount = val; return true }
		'view_transaction_url' { this.view_transaction_url = val; return true }
		'new_method_label' { this.new_method_label = val; return true }
		'pay_button_id' { this.pay_button_id = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_includes_abstracts_abstract_wc_payment_gateway_php() {
