...
fn (mut this Class_WP_Async_Request) construct()  {
	...
	rt.call_function('add_action', ['wp_ajax_' + (this.identifier).str(), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WP_Async_Request', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'maybe_handle' }])])
	rt.call_function('add_action', ['wp_ajax_nopriv_' + (this.identifier).str(), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WP_Async_Request', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'maybe_handle' }])])
}
...
fn (mut this Class_WP_Async_Request) dispatch() rt.PhpVal {
	mut var_url := rt.call_function('add_query_arg', [this.get_query_args(), this.get_query_url()])
	mut var_args := this.get_post_args()
	return rt.call_function('wp_remote_post', [rt.call_function('esc_url_raw', [var_url.dup()]), var_args.dup()])
}

fn (mut this Class_WP_Async_Request) get_query_args() rt.PhpVal {
	if rt.is_true(rt.call_function('property_exists', [rt.new_object('WP_Async_Request', []string{}, &this), rt.new_string('query_args')])) {
		return rt.get_property(rt.new_object('WP_Async_Request', []string{}, &this), 'query_args')
	}
	return rt.create_array([rt.ArrayItem{ key: 'action', val: this.identifier }, rt.ArrayItem{ key: 'nonce', val: rt.call_function('wp_create_nonce', [this.identifier]) }])
}

fn (mut this Class_WP_Async_Request) get_query_url() rt.PhpVal {
	if rt.is_true(rt.call_function('property_exists', [rt.new_object('WP_Async_Request', []string{}, &this), rt.new_string('query_url')])) {
...
fn (mut this Class_WP_Async_Request) get_post_args() rt.PhpVal {
	if rt.is_true(rt.call_function('property_exists', [rt.new_object('WP_Async_Request', []string{}, &this), rt.new_string('post_args')])) {
		return rt.get_property(rt.new_object('WP_Async_Request', []string{}, &this), 'post_args')
	}
	return rt.create_array([rt.ArrayItem{ key: 'timeout', val: 0.01 }, rt.ArrayItem{ key: 'blocking', val: false }, rt.ArrayItem{ key: 'body', val: this.data }, rt.ArrayItem{ key: 'cookies', val: rt.get_superglobal('_COOKIE') }, rt.ArrayItem{ key: 'sslverify', val: rt.call_function('apply_filters', [rt.new_string('https_local_ssl_verify'), rt.new_bool(false)]) }])
}
...
fn (mut this Class_WP_Async_Request) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'identifier' { this.identifier = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_includes_libraries_wp_async_request_php() {
