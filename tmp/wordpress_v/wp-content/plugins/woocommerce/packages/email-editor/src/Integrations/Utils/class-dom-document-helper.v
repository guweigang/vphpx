...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Dom_Document_Helper) load_html(html_content string)  {
	...
	if !(html_content == '') {
		rt.call_method(this.dom, 'loadHTML', ['<?xml encoding="UTF-8">' + html_content, rt.bitwise_or(rt.get_constant('LIBXML_HTML_NOIMPLIED'), rt.get_constant('LIBXML_HTML_NODEFDTD'))])
	}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Dom_Document_Helper) find_element(tag_name string) rt.PhpVal {
	mut var_elements := rt.call_method(this.dom, 'getElementsByTagName', [rt.new_string(tag_name)])
	return if rt.is_true(rt.call_method(var_elements, 'item', [rt.new_int(0)])) { rt.call_method(var_elements, 'item', [rt.new_int(0)]) } else { rt.new_null() }
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Dom_Document_Helper) get_attribute_value(mut var_element Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_DOMElement, attribute string) string {
	...
	return (if rt.is_true(rt.call_method(var_element_mutated, 'hasAttribute', [rt.new_string(attribute)])) { rt.call_method(var_element_mutated, 'getAttribute', [rt.new_string(attribute)]) } else { rt.new_string('') }).str()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Dom_Document_Helper) get_attribute_value_by_tag_name(tag_name string, attribute string) string {
	...
	return this.get_attribute_value(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_DOMElement](var_element), attribute)
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Dom_Document_Helper) get_element_inner_html(mut var_element Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_DOMElement) string {
	mut var_element_mutated := var_element
	mut var_inner_html := rt.new_string(rt.new_string(''))
...
fn create_automattic_woocommerce_emaileditor_integrations_utils_domdocument() &Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_DOMDocument {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Dom_Document_Helper) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_attribute_value' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_DOMElement](if args.len > 0 { args[0] } else { rt.new_null() })
		...
		'get_attribute_value_by_tag_name' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).str()
			return rt.new_string(this.get_attribute_value_by_tag_name(dispatch_arg_0, dispatch_arg_1))
		}
		'get_outer_html' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_DOMElement](if args.len > 0 { args[0] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_src_integrations_utils_class_dom_document_helper_php() {
