...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Api_Controller) send_preview_email_data(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	mut var_result := rt.call_function('apply_filters', [rt.new_string('woocommerce_email_editor_send_preview_email'), var_data.dup()])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	return create_wp_rest_response(rt.create_array([rt.ArrayItem{ key: 'success', val: // unsupported expression: Expr_Cast_Bool }, rt.ArrayItem{ key: 'result', val: var_result }]), if rt.is_true(var_result) { rt.new_int(200) } else { rt.new_int(400) })
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_EmailEditor_Engine_Exception') {
		...
		return create_wp_rest_response(rt.create_array([rt.ArrayItem{ key: 'error', val: rt.call_method(var_exception, 'getMessage', []rt.PhpVal{}) }]), rt.new_int(400))
		unsafe { goto end_label_1 }
	}
	else {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Api_Controller) get_personalization_tags() rt.PhpVal {
	...
	mut var_tag := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.create_array([rt.ArrayItem{ key: 'name', val: rt.call_method(var_tag, 'get_name', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'token', val: rt.call_method(var_tag, 'get_token', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'category', val: rt.call_method(var_tag, 'get_category', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'attributes', val: rt.call_method(var_tag, 'get_attributes', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'valueToInsert', val: rt.call_method(var_tag, 'get_value_to_insert', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'postTypes', val: rt.call_method(var_tag, 'get_post_types', []rt.PhpVal{}) }])
	}
	mut var_tag := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.create_array([rt.ArrayItem{ key: 'name', val: rt.call_method(var_tag, 'get_name', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'token', val: rt.call_method(var_tag, 'get_token', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'category', val: rt.call_method(var_tag, 'get_category', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'attributes', val: rt.call_method(var_tag, 'get_attributes', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'valueToInsert', val: rt.call_method(var_tag, 'get_value_to_insert', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'postTypes', val: rt.call_method(var_tag, 'get_post_types', []rt.PhpVal{}) }])
	}
	...
	return create_wp_rest_response(rt.create_array([rt.ArrayItem{ key: 'success', val: true }, rt.ArrayItem{ key: 'result', val: rt.call_function('array_values', [rt.call_function('array_map', [rt.new_closure(closure_1_fn), var_tags.dup()])]) }]), rt.new_int(200))
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Api_Controller) get_personalization_tags_collection(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	var_post_id = if rt.is_true(rt.new_bool(var_post_id.dup().is_long() || var_post_id.dup().is_double())) { // unsupported expression: Expr_Cast_Int } else { rt.new_int(0) }
	if rt.is_true(rt.greater(var_post_id, rt.new_int(0))) {
		rt.call_function('do_action', [rt.new_string('woocommerce_email_editor_personalization_tags_for_post'), var_post_id.dup()])
	}
	mut var_tags := rt.call_method(this.personalization_tags_registry, 'get_all', []rt.PhpVal{})
	...
	return create_wp_rest_response(rt.call_function('array_values', [rt.call_function('array_map', [rt.new_closure(closure_5_fn), var_tags.dup()])]), rt.new_int(200))
}

fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Api_Controller) get_email_data_schema() rt.PhpVal {
	return rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.object() }(), 'to_array', []rt.PhpVal{})
}
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_src_engine_class_email_api_controller_php() {
