...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Coupon_Code) render_content(block_content string, mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) string {
	...
	mut var_attrs := if !(var_parsed_block.array_get('attrs')).is_null() {
		var_parsed_block.array_get('attrs')
	} else {
		rt.new_array()
	}
	mut var_source := if !(var_attrs.array_get('source')).is_null() {
		var_attrs.array_get('source')
	} else {
	...
	if rt.is_true(rt.identical(rt.new_string('createNew'), var_source)) {
		...
		block_content_mutated = (rt.call_function('str_replace', [
			...
			rt.call_function('esc_html', [var_coupon_code.dup()]),
			rt.new_string(block_content_mutated).dup(),
		])).str()
	}
	mut var_align := if !(var_attrs.array_get('align')).is_null() {
		var_attrs.array_get('align')
	} else {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		...
		rt.new_bool(true)])))))
	{
		var_align = rt.new_string(rt.new_string('center'))
	}
	...
	mut var_cell_attrs := rt.create_array([rt.ArrayItem{ key: 'align', val: var_align },
		...
		}(rt.create_array([rt.ArrayItem{ key: 'text-align', val: var_align }]), rt.new_string('')) }])
	...
	}(rt.new_string(block_content_mutated), var_table_attrs.dup(), var_cell_attrs.dup())).str()
}
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_src_integrations_woocommerce_renderer_blocks_class_coupon_code_php() {
