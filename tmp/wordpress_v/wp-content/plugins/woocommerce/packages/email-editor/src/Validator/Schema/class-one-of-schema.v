...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema_One_Of_Schema) construct(mut var_schemas Class_Automattic_WooCommerce_EmailEditor_Validator_Schema_array) {
	{
		mut iter_1 := var_schemas.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_schema := item_1.val
			this.schema.array_get_mut('oneOf').array_push(rt.call_method(var_schema, 'to_array',
				[]rt.PhpVal{}))
		}
	}
}

fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema_One_Of_Schema) nullable() rt.PhpVal {
	mut var_null := rt.create_array([rt.ArrayItem{ key: 'type', val: 'null' }])
	mut var_one_of := this.schema.array_get('oneOf')
	mut var_value := if rt.is_true(rt.call_function('in_array', [
		var_null.dup(), var_one_of.dup(), rt.new_bool(true)]))
	{ var_one_of } else { rt.call_function('array_merge', [var_one_of.dup(),
			rt.create_array([rt.ArrayItem{ key: none, val: var_null }])]) }
	return this.update_schema_property(rt.new_string('oneOf'), var_value.dup())
}

fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema_One_Of_Schema) non_nullable() rt.PhpVal {
	mut var_null := rt.create_array([rt.ArrayItem{ key: 'type', val: 'null' }])
	mut var_one_of := this.schema.array_get('one_of')
	...
	mut var_value := rt.call_function('array_filter', [var_one_of.dup(),
		rt.new_closure(closure_1_fn)])
	return this.update_schema_property(rt.new_string('one_of'), var_value.dup())
}
...
fn create_automattic_woocommerce_emaileditor_validator_schema_one_of_schema(arg_0 rt.PhpVal) &Class_Automattic_WooCommerce_EmailEditor_Validator_Schema_One_Of_Schema {
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_src_validator_schema_class_one_of_schema_php() {
