...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates) construct(mut var_templates_registry Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates_Registry)  {
	this.templates_registry = var_templates_registry.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates) initialize(mut var_post_types Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_array)  {
	...
	rt.call_function('add_filter', [rt.new_string('theme_templates'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_theme_templates' }]), rt.new_int(10), rt.new_int(4)])
	rt.call_function('add_filter', [rt.new_string('woocommerce_email_editor_register_templates'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'register_templates' }])])
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates) get_block_template(var_template_slug rt.PhpVal) rt.PhpVal {
	mut var_template_id := rt.new_string((rt.call_function('get_stylesheet', []rt.PhpVal{})).str() + '//' + (var_template_slug).str())
	return rt.call_function('get_block_template', [var_template_id.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates) register_templates(mut var_templates_registry Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates_Registry) rt.PhpVal {
	mut var_general_email_slug := rt.new_string(rt.new_string('email-general'))
	mut var_template_filename := rt.new_string((var_general_email_slug).str() + '.html')
	mut var_general_email := create_automattic_woocommerce_emaileditor_engine_templates_template(this.template_prefix, var_general_email_slug.dup(), rt.call_function('__', [rt.new_string('General Email'), rt.new_string('woocommerce')]), rt.call_function('__', [rt.new_string('A general template for emails.'), rt.new_string('woocommerce')]), // unsupported expression: Expr_Cast_String, this.post_types)
	var_templates_registry.register(rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Templates_Template', []string{}, var_general_email))
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates) register_post_types_to_api()  {
	mut var_controller := create_automattic_woocommerce_emaileditor_engine_templates_wp_rest_templates_controller(rt.new_string('wp_template'))
	mut var_schema := var_controller.get_item_schema()
	mut var_post_types_context := if !(var_schema.array_get('properties').array_get('post_types').array_get('context')).is_null() { var_schema.array_get('properties').array_get('post_types').array_get('context') } else { rt.new_array() }
	if rt.is_true(rt.call_function('in_array', [rt.new_string('view'), var_post_types_context.dup(), rt.new_bool(true)])) {
		return rt.new_null()
	}
	rt.call_function('register_rest_field', [rt.new_string('wp_template'), rt.new_string('post_types'), rt.create_array([rt.ArrayItem{ key: 'get_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'get_post_types' }]) }, rt.ArrayItem{ key: 'update_callback', val: rt.new_null() }, rt.ArrayItem{ key: 'schema', val: rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.string() }(), 'to_array', []rt.PhpVal{}) }])])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates) get_post_types(var_response_object rt.PhpVal) rt.PhpVal {
	mut var_template := rt.call_method(this.templates_registry, 'get_by_slug', [if !(var_response_object.array_get('slug')).is_null() { var_response_object.array_get('slug') } else { rt.new_string('') }])
	if rt.is_true(var_template) {
		return rt.call_method(var_template, 'get_post_types', []rt.PhpVal{})
	}
	return if !(var_response_object.array_get('post_types')).is_null() { var_response_object.array_get('post_types') } else { rt.new_array() }
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates) add_theme_templates(var_templates rt.PhpVal, var_theme rt.PhpVal, var_post rt.PhpVal, var_post_type rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_post_type) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_post_type.dup(), this.post_types, rt.new_bool(true)]))))))) {
	...
	mut var_email_templates_slugs := rt.call_function('array_map', [rt.new_closure(closure_1_fn), rt.call_method(this.templates_registry, 'get_all', []rt.PhpVal{})])
	{
		mut iter_1 := var_block_templates.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_block_template := item_1.val
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.get_property(var_block_template, 'slug'), var_email_templates_slugs.dup(), rt.new_bool(true)]))))) {
				continue
			}
			if var_templates_mutated.array_isset(rt.get_property(var_block_template, 'slug')) {
				continue
			}
			var_templates_mutated.array_set(rt.get_property(var_block_template, 'slug'), rt.get_property(var_block_template, 'title'))
...
fn create_automattic_woocommerce_emaileditor_engine_templates_wp_rest_templates_controller() &Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_WP_REST_Templates_Controller {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'initialize' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_array](if args.len > 0 { args[0] } else { rt.new_null() })
		...
		'add_theme_templates' {
			...
			return this.add_theme_templates(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2, dispatch_arg_3)
		}
		else { return none }
	}
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'template_directory' { this.template_directory = val; return true }
		'templates_registry' { this.templates_registry = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_src_engine_templates_class_templates_php() {
