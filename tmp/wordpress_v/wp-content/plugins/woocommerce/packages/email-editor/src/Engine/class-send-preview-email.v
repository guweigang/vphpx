...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) construct(mut var_renderer Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer, mut var_personalizer Class_Automattic_WooCommerce_EmailEditor_Engine_Personalizer)  {
	this.renderer = var_renderer.dup()
	this.personalizer = var_personalizer.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) send_preview_email(var_data rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(var_data.dup().is_bool())) {
	...
	mut var_email := var_data.array_get('email')
	mut var_post_id := var_data.array_get('postId')
	mut var_post := this.fetch_post(var_post_id.dup())
	mut var_subject := rt.new_string(this.get_preview_email_subject(var_post.dup()))
	mut var_email_html_content := rt.new_string(this.render_html(var_post.dup()))
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) render_html(var_post rt.PhpVal) string {
	...
	mut var_subject := rt.new_string(this.get_preview_email_subject(var_post_mutated.dup()))
	mut var_language := rt.call_function('get_bloginfo', [rt.new_string('language')])
	rt.call_function('add_filter', [rt.new_string('woocommerce_email_editor_rendering_email_context'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_preview_context' }])])
	mut var_rendered_data := rt.call_method(this.renderer, 'render', [var_post_mutated.dup(), var_subject.dup(), rt.call_function('__', [rt.new_string('Preview'), rt.new_string('woocommerce')]), var_language.dup()])
	rt.call_function('remove_filter', [rt.new_string('woocommerce_email_editor_rendering_email_context'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_preview_context' }])])
	var_rendered_data = rt.call_function('apply_filters', [rt.new_string('woocommerce_email_editor_send_preview_email_rendered_data'), var_rendered_data.dup(), var_post_mutated.dup()])
	return this.set_personalize_content((var_rendered_data.array_get('html')).str())
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) get_preview_email_subject(var_post rt.PhpVal) string {
	mut var_post_mutated := var_post
	mut var_subject := // unsupported expression: Expr_Cast_String
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) add_preview_context(var_email_context rt.PhpVal) rt.PhpVal {
	...
	return var_email_context_mutated.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) set_personalize_content(content string) string {
	...
	mut var_subscriber := if !(!rt.is_true(rt.get_property(var_current_user, 'ID'))) { var_current_user } else { rt.new_null() }
	mut var_personalizer_context := rt.create_array([rt.ArrayItem{ key: 'recipient_email', val: if rt.is_true(var_subscriber) { rt.get_property(var_subscriber, 'user_email') } else { rt.new_null() } }, rt.ArrayItem{ key: 'is_user_preview', val: true }])
	var_personalizer_context = rt.call_function('apply_filters', [rt.new_string('woocommerce_email_editor_send_preview_email_personalizer_context'), var_personalizer_context.dup()])
	rt.call_method(this.personalizer, 'set_context', [var_personalizer_context.dup()])
	return (rt.call_method(this.personalizer, 'personalize_content', [rt.new_string(content)])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) send_email(to string, subject string, body string) bool {
	...
	rt.call_function('do_action', [rt.new_string('woocommerce_email_editor_send_preview_email_before_wp_mail'), rt.new_string(to), rt.new_string(subject_mutated).dup(), rt.new_string(body)])
	rt.call_function('add_filter', [rt.new_string('wp_mail_content_type'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'set_mail_content_type' }])])
	mut var_result := rt.call_function('wp_mail', [rt.new_string(to), rt.new_string(subject_mutated).dup(), rt.new_string(body)])
	rt.call_function('remove_filter', [rt.new_string('wp_mail_content_type'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'set_mail_content_type' }])])
	rt.call_function('do_action', [rt.new_string('woocommerce_email_editor_send_preview_email_after_wp_mail'), rt.new_string(to), rt.new_string(subject_mutated).dup(), rt.new_string(body), var_result.dup()])
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) validate_data(mut var_data Class_Automattic_WooCommerce_EmailEditor_Engine_array)  {
	if !rt.is_true(var_data.array_get('email')) || !rt.is_true(var_data.array_get('postId')) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_InvalidArgumentException', []string{}, create_automattic_woocommerce_emaileditor_engine_invalidargumentexception(rt.call_function('esc_html__', [rt.new_string('Missing required data'), rt.new_string('woocommerce')]))))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_email', [var_data.array_get('email')]))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_InvalidArgumentException', []string{}, create_automattic_woocommerce_emaileditor_engine_invalidargumentexception(rt.call_function('esc_html__', [rt.new_string('Invalid email'), rt.new_string('woocommerce')]))))
	}
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) fetch_post(var_post_id rt.PhpVal) rt.PhpVal {
	...
	mut var_post := rt.call_function('get_post', [rt.new_int(var_post_id_mutated.dup().to_i64())])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_post, 'Automattic_WooCommerce_EmailEditor_Engine_WP_Post')))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Exception', []string{}, create_automattic_woocommerce_emaileditor_engine_exception(rt.call_function('esc_html__', [rt.new_string('Invalid post'), rt.new_string('woocommerce')]))))
	}
	return var_post.dup()
}
...
fn create_automattic_woocommerce_emaileditor_engine_invalidargumentexception() &Class_Automattic_WooCommerce_EmailEditor_Engine_InvalidArgumentException {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer](if args.len > 0 { args[0] } else { rt.new_null() })
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Personalizer](if args.len > 1 { args[1] } else { rt.new_null() })
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'personalizer' { this.personalizer = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_src_engine_class_send_preview_email_php() {
