...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Personalizer) construct(mut var_tags_registry Class_Automattic_WooCommerce_EmailEditor_Engine_PersonalizationTags_Personalization_Tags_Registry)  {
	this.tags_registry = var_tags_registry.dup()
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Personalizer) personalize_content(content string) string {
	...
	mut var_content_processor := create_automattic_woocommerce_emaileditor_engine_personalizationtags_html_tag_processor(rt.new_string(content).dup())
	for rt.is_true(var_content_processor.next_token()) {
		if rt.is_true(rt.identical(var_content_processor.get_token_type(), rt.new_string('#comment'))) {
			mut var_modifiable_text := var_content_processor.get_modifiable_text()
			mut var_token := this.parse_token((var_modifiable_text).str())
			mut var_tag := rt.call_method(this.tags_registry, 'get_by_token', [var_token.array_get('token')])
			if rt.is_true(rt.new_bool(!(rt.is_true(var_tag)))) {
				continue
			}
			mut var_value := rt.call_method(var_tag, 'execute_callback', [this.context, var_token.array_get('arguments')])
			var_content_processor.replace_token(var_value.dup())
		} else if rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_content_processor.get_token_type(), rt.new_string('#tag'))) && rt.is_true(rt.identical(var_content_processor.get_tag(), rt.new_string('TITLE'))))) {
			...
			var_content_processor.set_modifiable_text(var_title.dup())
		} else if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_content_processor.get_token_type(), rt.new_string('#tag'))) && rt.is_true(rt.identical(var_content_processor.get_tag(), rt.new_string('A'))))) && rt.is_true(var_content_processor.get_attribute(rt.new_string('data-link-href'))))) {
			mut var_href := // unsupported expression: Expr_Cast_String
			var_token = this.parse_token((var_href).str())
			var_tag = rt.call_method(this.tags_registry, 'get_by_token', [var_token.array_get('token')])
			if rt.is_true(rt.new_bool(!(rt.is_true(var_tag)))) {
				continue
			}
			var_value = rt.call_method(var_tag, 'execute_callback', [this.context, var_token.array_get('arguments')])
			var_value = rt.new_string(this.replace_link_href((var_href).str(), (rt.call_method(var_tag, 'get_token', []rt.PhpVal{})).str(), (var_value).str()))
			if rt.is_true(var_value) {
				var_content_processor.set_attribute(rt.new_string('href'), var_value.dup())
		...
		} else if rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_content_processor.get_token_type(), rt.new_string('#tag'))) && rt.is_true(rt.identical(var_content_processor.get_tag(), rt.new_string('A'))))) {
			var_href = var_content_processor.get_attribute(rt.new_string('href'))
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_href.dup().is_string()))))) {
				continue
			}
			mut var_decoded_href := rt.call_function('html_entity_decode', [rt.call_function('urldecode', [var_href.dup()]), rt.get_constant('ENT_QUOTES'), rt.new_string('UTF-8')])
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', ['/\\[' + (Class_Automattic_WooCommerce_EmailEditor_Engine_Automattic_WooCommerce_EmailEditor_Engine_Personalizer.tag_name_pattern()).str() + '(?:\\s+[^\\]]+)?\\]/', var_decoded_href.dup(), var_matches.dup()]))))) {
				continue
			}
			var_token = this.parse_token((var_matches.array_get(0)).str())
			var_tag = rt.call_method(this.tags_registry, 'get_by_token', [var_token.array_get('token')])
			if rt.is_true(rt.new_bool(!(rt.is_true(var_tag)))) {
				continue
			}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Personalizer) parse_token(token string) rt.PhpVal {
	...
	mut var_result := rt.create_array([rt.ArrayItem{ key: 'token', val: '' }, rt.ArrayItem{ key: 'arguments', val: rt.new_array() }])
	if rt.is_true(rt.call_function('preg_match', ['/^\\[(' + (Class_Automattic_WooCommerce_EmailEditor_Engine_Automattic_WooCommerce_EmailEditor_Engine_Personalizer.tag_name_pattern()).str() + ')\\s*(.*?)\\]$/', rt.new_string(token_mutated.trim_space()), var_matches.dup()])) {
		var_result.array_set('token', rt.concat(rt.concat(rt.new_string('['), var_matches.array_get(1)), rt.new_string(']')))
		mut var_attributes_string := var_matches.array_get(2)
		if rt.is_true(rt.call_function('preg_match_all', [rt.new_string('/(\\w+)=(?:"([^"]*)"|\'([^\']*)\'|([^\\s\\]]+(?:\\s+(?!\\w+=)[^\\s\\]]+)*))/'), var_attributes_string.dup(), var_attribute_matches.dup(), rt.get_constant('PREG_SET_ORDER')])) {
			{
				mut iter_1 := var_attribute_matches.iterator()
				for {
					item_1 := iter_1.next() or { break }
					mut var_attribute := item_1.val
					mut var_double_quoted_value := if !(var_attribute.array_get(2)).is_null() { var_attribute.array_get(2) } else { rt.new_string('') }
					mut var_single_quoted_value := if !(var_attribute.array_get(3)).is_null() { var_attribute.array_get(3) } else { rt.new_string('') }
					mut var_unquoted_value := if !(var_attribute.array_get(4)).is_null() { var_attribute.array_get(4) } else { rt.new_string('') }
					if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
						var_result.array_get_mut('arguments').array_set(var_attribute.array_get(1), var_double_quoted_value.dup())
					} else if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
						var_result.array_get_mut('arguments').array_set(var_attribute.array_get(1), var_single_quoted_value.dup())
					} else {
						var_result.array_get_mut('arguments').array_set(var_attribute.array_get(1), var_unquoted_value.dup())
					}
				}
			}
		}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Personalizer) replace_link_href(content string, token string, replacement string) string {
	mut token_mutated := token
	mut var_escaped_shortcode := rt.call_function('preg_quote', [rt.call_function('substr', [rt.new_string(token_mutated).dup(), rt.new_int(1), token_mutated.len - 2]), rt.new_string('/')])
	mut var_pattern := rt.new_string('/\\[' + (var_escaped_shortcode).str() + '(?:\\s+[^\\]]+)?\\]/')
	return // unsupported expression: Expr_Cast_String.to_string().trim_space()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Personalizer) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'replace_link_href' {
			...
			return rt.new_string(this.replace_link_href(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2))
		}
		else { return none }
	}
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Personalizer) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'tags_registry' { this.tags_registry = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_src_engine_class_personalizer_php() {
