...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Button) render_content(block_content string, mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) string {
	...
	mut var_collection := if !(var_parsed_block.array_get('context').array_get('collection')).is_null() {
		var_parsed_block.array_get('context').array_get('collection')
	} else {
		rt.new_string('')
	}
	...
	if rt.is_true(var_is_cart_contents) {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.call_method(var_product, 'is_type', [rt.new_string('external')]))
	...
	mut var_block_attributes := rt.call_function('array_replace_recursive', [
		...
		if !(var_parsed_block.array_get('attrs')).is_null() {
	...
	mut var_cell_attrs := rt.create_array([
		rt.ArrayItem{ key: 'class', val: var_wrapper_styles.array_get('classnames') },
	...
	mut var_button_content := rt.call_function('sprintf', [
		...
		rt.call_function('esc_attr', [var_button_styles.array_get('classnames')]),
		rt.call_function('esc_attr', [var_button_styles.array_get('css')]),
		rt.call_function('esc_url', [var_button_url.dup()]),
		rt.call_function('esc_html', [var_button_text.dup()]),
	])
	mut var_button_html := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal {
		mut temp :=
	...
	}(var_button_content.dup(), var_table_attrs.dup(), var_cell_attrs.dup())
	return (fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp :=
			Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Table_Wrapper_Helper{}
		return temp.render_table_wrapper(arg_0, arg_1)
	}(var_button_html.dup(), rt.create_array([
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_src_integrations_woocommerce_renderer_blocks_class_product_button_php() {
