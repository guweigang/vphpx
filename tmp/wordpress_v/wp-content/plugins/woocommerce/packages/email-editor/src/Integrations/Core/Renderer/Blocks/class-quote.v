...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Quote) render_content(block_content string, mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) string {
	...
	mut var_citation_content := rt.new_string(rt.new_string(''))
	mut var_cite_element := var_dom_helper.find_element(rt.new_string('cite'))
	if rt.is_true(var_cite_element) {
		var_citation_content = rt.new_string(this.get_citation_wrapper((var_dom_helper.get_element_inner_html(var_cite_element.dup())).str(), mut
	...
	return (rt.call_function('str_replace', [
		...
		this.get_block_wrapper(block_content, mut var_parsed_block, mut var_rendering_context),
	])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Quote) get_citation_wrapper(citation_content string, mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) string {
	...
	mut var_margin_top := if !(var_theme_styles.array_get('spacing').array_get('blockGap')).is_null() {
		var_theme_styles.array_get('spacing').array_get('blockGap')
	} else {
		rt.new_string('0px')
	}
	mut var_citation_styles := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal {
	...
	}(var_parsed_block.array_get('attrs'), rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context',
		[]string{}, var_rendering_context), rt.create_array([
		rt.ArrayItem{ key: none, val: 'text-align' },
	]))
	var_citation_styles = fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper{}
		return temp.extend_block_styles(arg_0, arg_1)
	}(var_citation_styles.dup(), rt.create_array([
	...
	return (this.add_spacer(rt.call_function('sprintf', [
		...
		rt.new_string(citation_content_mutated).dup(),
		var_citation_styles.array_get('css'),
	]), if !(var_parsed_block.array_get('email_attrs')).is_null() {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Quote) get_block_wrapper(block_content string, mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) string {
	...
	var_table_styles = fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper{}
		return temp.extend_block_styles(arg_0, arg_1)
	}(var_table_styles.dup(), rt.create_array([
		rt.ArrayItem{ key: 'border-collapse', val: 'separate' },
		rt.ArrayItem{
			key: 'background-size'
			val: if !(var_table_styles.array_get('declarations').array_get('background-size')).is_null() {
				var_table_styles.array_get('declarations').array_get('background-size')
			} else {
				rt.new_string('cover')
			}
		},
	...
	}(rt.new_string('{quote_content}{citation_content}'), var_table_attrs.dup(),
		var_cell_attrs.dup())).str()
}
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_src_integrations_core_renderer_blocks_class_quote_php() {
