...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) construct(mut var_email_api_controller Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Api_Controller, mut var_templates Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates, mut var_patterns Class_Automattic_WooCommerce_EmailEditor_Engine_Patterns_Patterns, mut var_send_preview_email Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email, mut var_personalization_tags_controller Class_Automattic_WooCommerce_EmailEditor_Engine_PersonalizationTags_Personalization_Tags_Registry, mut var_logger Class_Automattic_WooCommerce_EmailEditor_Engine_Logger_Email_Editor_Logger, mut var_assets_manager Class_Automattic_WooCommerce_EmailEditor_Engine_Assets_Manager)  {
	this.email_api_controller = var_email_api_controller.dup()
	this.templates = var_templates.dup()
	this.patterns = var_patterns.dup()
	this.send_preview_email = var_send_preview_email.dup()
	this.personalization_tags_registry = var_personalization_tags_controller.dup()
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) initialize()  {
	...
	rt.call_function('add_filter', [rt.new_string('woocommerce_email_editor_rendering_theme_styles'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Email_Editor', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'extend_email_theme_styles' }]), rt.new_int(10), rt.new_int(2)])
	...
	mut var_is_editor_page := rt.call_function('apply_filters', [rt.new_string('woocommerce_is_email_editor_page'), rt.new_bool(false)])
	if rt.is_true(var_is_editor_page) {
		this.extend_email_post_api()
		rt.call_method(this.assets_manager, 'initialize', []rt.PhpVal{})
	}
	rt.call_function('add_action', [rt.new_string('rest_api_init'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Email_Editor', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'register_email_editor_api_routes' }])])
	rt.call_function('add_filter', [rt.new_string('woocommerce_email_editor_send_preview_email'), rt.create_array([rt.ArrayItem{ key: none, val: this.send_preview_email }, rt.ArrayItem{ key: none, val: 'send_preview_email' }]), rt.new_int(11), rt.new_int(1)])
	rt.call_function('add_filter', [rt.new_string('single_template'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Email_Editor', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'load_email_preview_template' }])])
	rt.call_function('add_filter', [rt.new_string('preview_post_link'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Email_Editor', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'update_preview_post_link' }]), rt.new_int(10), rt.new_int(2)])
	rt.call_method(this.logger, 'info', [rt.new_string('Email editor initialized successfully')])
}

fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) register_block_templates()  {
	mut var_request_uri := rt.new_string(rt.new_string(''))
	if rt.is_true(rt.new_bool(rt.get_superglobal('_SERVER').array_isset(rt.new_string('REQUEST_URI')) && rt.is_true(rt.new_bool(rt.get_superglobal('_SERVER').array_get('REQUEST_URI').is_string())))) {
		var_request_uri = rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_SERVER').array_get('REQUEST_URI')])])
	}
	if rt.is_true(rt.identical(rt.call_function('strstr', [var_request_uri.dup(), rt.new_string('site-editor.php')]), rt.new_bool(false))) {
		mut var_post_types := rt.call_function('array_column', [this.get_post_types(), rt.new_string('name')])
		rt.call_method(this.templates, 'initialize', [var_post_types.dup()])
	}
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) register_email_post_types()  {
	{
		...
		for {
			...
			rt.call_function('register_post_type', [var_post_type.array_get('name'), rt.call_function('array_merge', [this.get_default_email_post_args(), var_post_type.array_get('args')])])
		}
	}
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) get_post_types() rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_email_editor_post_types'), var_post_types.dup()])
}

fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) get_default_email_post_args() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: 'public', val: false }, rt.ArrayItem{ key: 'hierarchical', val: false }, rt.ArrayItem{ key: 'show_ui', val: true }, rt.ArrayItem{ key: 'show_in_menu', val: false }, rt.ArrayItem{ key: 'show_in_nav_menus', val: false }, rt.ArrayItem{ key: 'supports', val: rt.create_array([rt.ArrayItem{ key: 'editor', val: rt.create_array([rt.ArrayItem{ key: 'default-mode', val: 'template-locked' }]) }, rt.ArrayItem{ key: none, val: 'title' }, rt.ArrayItem{ key: none, val: 'custom-fields' }]) }, rt.ArrayItem{ key: 'has_archive', val: true }, rt.ArrayItem{ key: 'show_in_rest', val: true }, rt.ArrayItem{ key: 'default_rendering_mode', val: 'template-locked' }, rt.ArrayItem{ key: 'publicly_queryable', val: true }])
}

fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) register_email_post_sent_status()  {
	mut var_default_args := rt.create_array([rt.ArrayItem{ key: 'public', val: false }, rt.ArrayItem{ key: 'exclude_from_search', val: true }, rt.ArrayItem{ key: 'internal', val: true }, rt.ArrayItem{ key: 'show_in_admin_all_list', val: false }, rt.ArrayItem{ key: 'show_in_admin_status_list', val: false }, rt.ArrayItem{ key: 'private', val: true }])
	mut var_args := rt.call_function('apply_filters', [rt.new_string('woocommerce_email_editor_post_sent_status_args'), var_default_args.dup()])
	rt.call_function('register_post_status', [rt.new_string('sent'), var_args.dup()])
}

fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) extend_email_post_api()  {
	mut var_email_post_types := rt.call_function('array_column', [this.get_post_types(), rt.new_string('name')])
	rt.call_function('register_rest_field', [var_email_post_types.dup(), rt.new_string('email_data'), rt.create_array([rt.ArrayItem{ key: 'get_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: this.email_api_controller }, rt.ArrayItem{ key: none, val: 'get_email_data' }]) }, rt.ArrayItem{ key: 'update_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: this.email_api_controller }, rt.ArrayItem{ key: none, val: 'save_email_data' }]) }, rt.ArrayItem{ key: 'schema', val: rt.call_method(this.email_api_controller, 'get_email_data_schema', []rt.PhpVal{}) }])])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) register_email_editor_api_routes()  {
	...
	mut var_post_id := rt.call_method(var_request, 'get_param', [rt.new_string('postId')])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_post_id.dup().is_long() || var_post_id.dup().is_double()))))) || rt.is_true(rt.less_equal(// unsupported expression: Expr_Cast_Int, rt.new_int(0))))) {
		return rt.new_bool(false)
	}
	return rt.call_function('current_user_can', [rt.new_string('edit_post'), // unsupported expression: Expr_Cast_Int])
	}
	rt.call_function('register_rest_route', [rt.new_string('woocommerce-email-editor/v1'), rt.new_string('/send_preview_email'), rt.create_array([rt.ArrayItem{ key: 'methods', val: 'POST' }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: this.email_api_controller }, rt.ArrayItem{ key: none, val: 'send_preview_email_data' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.new_closure(closure_1_fn) }])])
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	return rt.call_function('current_user_can', [rt.new_string('edit_posts')])
	}
	rt.call_function('register_rest_route', [rt.new_string('woocommerce-email-editor/v1'), rt.new_string('/get_personalization_tags'), rt.create_array([rt.ArrayItem{ key: 'methods', val: 'GET' }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: this.email_api_controller }, rt.ArrayItem{ key: none, val: 'get_personalization_tags' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.new_closure(closure_2_fn) }])])
	closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	return rt.call_function('current_user_can', [rt.new_string('edit_posts')])
	}
	rt.call_function('register_rest_route', [rt.new_string('woocommerce-email-editor/v1'), rt.new_string('/personalization_tags'), rt.create_array([rt.ArrayItem{ key: 'methods', val: 'GET' }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: this.email_api_controller }, rt.ArrayItem{ key: none, val: 'get_personalization_tags_collection' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.new_closure(closure_3_fn) }, rt.ArrayItem{ key: 'args', val: rt.create_array([rt.ArrayItem{ key: 'post_id', val: rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('The post ID for context-aware tag filtering.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'integer' }, rt.ArrayItem{ key: 'required', val: false }, rt.ArrayItem{ key: 'sanitize_callback', val: 'absint' }]) }]) }])])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) extend_email_theme_styles(mut var_theme Class_WP_Theme_JSON, mut var_post Class_WP_Post) rt.PhpVal {
	...
	mut var_email_theme := rt.call_function('get_post_meta', [rt.get_property(var_post_mutated, 'ID'), Class_Automattic_WooCommerce_EmailEditor_Engine_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor.woocommerce_email_meta_theme_type(), rt.new_bool(true)])
	if rt.is_true(rt.new_bool(rt.is_true(var_email_theme) && rt.is_true(rt.new_bool(var_email_theme.dup().is_array())))) {
		var_theme.merge(create_wp_theme_json(var_email_theme.dup()))
	}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) get_current_post() rt.PhpVal {
	...
	if rt.get_superglobal('_GET').array_isset(rt.new_string('post')) {
		...
		if rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_get('post').is_string())) {
			var_post_id = rt.new_int(rt.new_int(rt.get_superglobal('_GET').array_get('post').to_i64()))
			// unsupported statement: Stmt_Nop
		}
		mut var_current_post := rt.call_function('get_post', [var_post_id.dup()])
	} else {
		var_current_post = var_GLOBALS.array_get('post')
	}
	return var_current_post.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) current_post_is_email_post_type(var_current_post_type rt.PhpVal) bool {
	...
	mut var_email_post_types := rt.call_function('array_column', [this.get_post_types(), rt.new_string('name')])
	return (rt.call_function('in_array', [var_current_post_type.dup(), var_email_post_types.dup(), rt.new_bool(true)])).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) load_email_preview_template(var_template rt.PhpVal) string {
	...
	return (rt.call_method(this.send_preview_email, 'render_html', [var_post.dup()])).str()
	}
	rt.call_function('add_filter', [rt.new_string('woocommerce_email_editor_preview_post_template_html'), rt.new_closure(closure_4_fn)])
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) update_preview_post_link(var_preview_link rt.PhpVal, var_post rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_post_mutated, 'WP_Post')))))) {
		return var_preview_link.dup()
	}
	if !(this.current_post_is_email_post_type(rt.get_property(var_post_mutated, 'post_type'))) {
		return var_preview_link.dup()
	}
	return rt.call_function('remove_query_arg', [rt.new_string('preview_nonce'), var_preview_link.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Api_Controller](if args.len > 0 { args[0] } else { rt.new_null() })
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Patterns_Patterns](if args.len > 2 { args[2] } else { rt.new_null() })
			mut dispatch_arg_3 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Send_Preview_Email](if args.len > 3 { args[3] } else { rt.new_null() })
			...
			mut dispatch_arg_5 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Logger_Email_Editor_Logger](if args.len > 5 { args[5] } else { rt.new_null() })
			mut dispatch_arg_6 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Assets_Manager](if args.len > 6 { args[6] } else { rt.new_null() })
			this.construct(mut dispatch_arg_0, mut dispatch_arg_1, mut dispatch_arg_2, mut dispatch_arg_3, mut dispatch_arg_4, mut dispatch_arg_5, mut dispatch_arg_6)
			return rt.new_null()
		}
		'initialize' {
			this.initialize()
		...
		'extend_email_theme_styles' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_WP_Post](if args.len > 1 { args[1] } else { rt.new_null() })
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Email_Editor) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'send_preview_email' { this.send_preview_email = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_src_engine_class_email_editor_php() {
