...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Css_StyleRule) construct(mut var_declarationBlock Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_RuleSet_DeclarationBlock, containingAtRule string)  {
	this.declarationBlock = var_declarationBlock.dup()
	this.containingAtRule = containingAtRule.trim_space()
}

fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Css_StyleRule) getselectors() rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Css_StyleRule) getdeclarationastext() string {
	return (rt.call_function('implode', [rt.new_string(' '), rt.call_method(this.declarationBlock, 'getRules', []rt.PhpVal{})])).str()
}

fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Css_StyleRule) hasatleastonedeclaration() bool {
	return (// unsupported expression: Expr_BinaryOp_NotIdentical).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Css_StyleRule) getcontainingatrule() string {
	return this.containingAtRule
}

fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Css_StyleRule) hascontainingatrule() bool {
	return (// unsupported expression: Expr_BinaryOp_NotIdentical).to_bool()
}

fn create_automattic_woocommerce_emaileditorvendor_pelago_emogrifier_css_stylerule(arg_0 rt.PhpVal, containingAtRule string) &Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Css_StyleRule {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Css_StyleRule) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_RuleSet_DeclarationBlock](if args.len > 0 { args[0] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_vendor_prefixed_packages_pelago_emogrifier_css_stylerule_php() {
