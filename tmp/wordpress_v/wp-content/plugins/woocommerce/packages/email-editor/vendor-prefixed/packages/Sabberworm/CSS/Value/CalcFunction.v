...
fn Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CalcFunction.parse(mut var_oParserState Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Parsing_ParserState, bIgnoreCase bool) rt.PhpVal {
	...
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotEqual) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Parsing_UnexpectedTokenException', []string{}, create_automattic_woocommerce_emaileditorvendor_sabberworm_css_parsing_unexpectedtokenexception(rt.new_string('('), var_oParserState.peek(), rt.new_string('literal'), var_oParserState.currentline())))
		...
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Parsing_UnexpectedTokenException', []string{}, create_automattic_woocommerce_emaileditorvendor_sabberworm_css_parsing_unexpectedtokenexception(rt.new_string('calc'), var_sFunction.dup(), rt.new_string('literal'), var_oParserState.currentline())))
	}
	var_oParserState.consume(rt.new_string('('))
	mut var_oCalcList := create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_calcrulevaluelist(var_oParserState.currentline())
	mut var_oList := create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_rulevaluelist(rt.new_string(','), var_oParserState.currentline())
	...
	for rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_oParserState.comes(rt.new_string(')')))))) || rt.is_true(rt.greater(var_iNestingLevel, rt.new_int(0))))) {
		...
		if rt.is_true(// unsupported expression: Expr_BinaryOp_NotEqual) {
			mut var_oVal := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_Value{}; return temp.parseprimitivevalue(arg_0) }(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Parsing_ParserState', []string{}, var_oParserState))
			var_oCalcList.addlistcomponent(var_oVal.dup())
			var_iLastComponentType = Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CalcFunction.t_operand()
		} else {
			if rt.is_true(rt.call_function('in_array', [var_oParserState.peek(), var_aOperators.dup()])) {
				if rt.is_true(rt.new_bool(rt.is_true(var_oParserState.comes(rt.new_string('-'))) || rt.is_true(var_oParserState.comes(rt.new_string('+'))))) {
					if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotEqual) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.is_true(var_oParserState.comes(rt.new_string('- '))) || rt.is_true(var_oParserState.comes(rt.new_string('+ ')))))))))) {
						rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Parsing_UnexpectedTokenException', []string{}, create_automattic_woocommerce_emaileditorvendor_sabberworm_css_parsing_unexpectedtokenexception(rt.concat(rt.concat(rt.new_string(' '), var_oParserState.peek()), rt.new_string(' ')), rt.concat(var_oParserState.peek(rt.new_int(1), // unsupported expression: Expr_UnaryMinus), var_oParserState.peek(rt.new_int(2))), rt.new_string('literal'), var_oParserState.currentline())))
					}
				}
				var_oCalcList.addlistcomponent(var_oParserState.consume(rt.new_int(1)))
				var_iLastComponentType = Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CalcFunction.t_operator()
			} else {
				rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Parsing_UnexpectedTokenException', []string{}, create_automattic_woocommerce_emaileditorvendor_sabberworm_css_parsing_unexpectedtokenexception(rt.call_function('sprintf', [rt.new_string('Next token was expected to be an operand of type %s. Instead "%s" was found.'), rt.call_function('implode', [rt.new_string(', '), var_aOperators.dup()]), var_oParserState.peek()]), rt.new_string(''), rt.new_string('custom'), var_oParserState.currentline())))
			}
		}
		var_oParserState.consumewhitespace()
	}
	var_oList.addlistcomponent(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CalcRuleValueList', []string{}, var_oCalcList))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_oParserState.isend())))) {
		var_oParserState.consume(rt.new_string(')'))
	}
	return create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_calcfunction(var_sFunction.dup(), var_oList.dup(), rt.new_string(','), var_oParserState.currentline())
}
...
fn create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_calcfunction() &Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CalcFunction {
...
fn create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_calcrulevaluelist() &Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CalcRuleValueList {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CalcFunction) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'parse' {
			...
			return Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CalcFunction.parse(mut dispatch_arg_0, dispatch_arg_1)
		}
		else { return none }
	}
}
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_vendor_prefixed_packages_sabberworm_css_value_calcfunction_php() {
}
