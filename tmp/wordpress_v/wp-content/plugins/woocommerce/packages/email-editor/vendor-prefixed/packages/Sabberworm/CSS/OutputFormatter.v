...
catch_label_1:
		...
		if rt.instance_of(var_e_1, 'Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Parsing_OutputException') {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormatter) implode(var_sSeparator rt.PhpVal, mut var_aValues Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_array, bIncreaseLevel bool) rt.PhpVal {
	...
	mut var_bIsFirst := rt.new_bool(rt.new_bool(true))
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.instance_of(var_mValue, 'Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Renderable'))) {
				// unsupported expression: Expr_AssignOp_Concat
			} else {
				// unsupported expression: Expr_AssignOp_Concat
			}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormatter) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'comments' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Comment_Commentable](if args.len > 0 { args[0] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_vendor_prefixed_packages_sabberworm_css_outputformatter_php() {
}
