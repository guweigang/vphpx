...
end_label_1:
	...
	fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_RuleSet_RuleSet{}; return temp.parseruleset(arg_0, arg_1) }(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Parsing_ParserState', []string{}, var_oParserState), rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_RuleSet_DeclarationBlock', []string{}, var_oResult))
	...
	}
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_mSelector_shadow, 'Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Property_Selector')))))) {
				if rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_oList, rt.new_null())) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_oList, 'Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_CSSList_KeyFrame')))))))) {
					if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Property_Selector{}; return temp.isvalid(arg_0) }(var_mSelector_shadow.dup()))))) {
					...
					this.aSelectors.array_set(var_iKey, create_automattic_woocommerce_emaileditorvendor_sabberworm_css_property_selector(var_mSelector_shadow.dup()))
				} else {
					if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Property_KeyframeSelector{}; return temp.isvalid(arg_0) }(var_mSelector_shadow.dup()))))) {
					...
					this.aSelectors.array_set(var_iKey, create_automattic_woocommerce_emaileditorvendor_sabberworm_css_property_keyframeselector(var_mSelector_shadow.dup()))
				}
			}
		}
	}
		...
		for {
			...
			}
			{
				...
				for {
					...
					mut var_oNewRule := create_automattic_woocommerce_emaileditorvendor_sabberworm_css_rule_rule(var_sNewRuleName.dup(), rt.call_method(var_oRule, 'getLineNo', []rt.PhpVal{}), rt.call_method(var_oRule, 'getColNo', []rt.PhpVal{}))
...
fn create_automattic_woocommerce_emaileditorvendor_sabberworm_css_rule_rule() &Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Rule_Rule {
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_vendor_prefixed_packages_sabberworm_css_ruleset_declarationblock_php() {
}
