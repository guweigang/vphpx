...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_Handler_StringHandler) handle(mut var_reader Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_Reader, mut var_stream Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_TokenStream) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_match)))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Exception_InternalErrorException', []string{}, create_automattic_woocommerce_emaileditorvendor_symfony_component_cssselector_exception_internalerrorexception(rt.call_function('sprintf', [rt.new_string('Should have found at least an empty match at %d.'), var_reader.getposition()]))))
	}
	if rt.is_true(rt.identical(rt.new_int(var_match.array_get(0).to_string().len), var_reader.getremaininglength())) {
		rt.throw_exception(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Exception_SyntaxErrorException{}; return temp.unclosedstring(arg_0) }(rt.sub(var_reader.getposition(), rt.new_int(1))))
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		rt.throw_exception(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Exception_SyntaxErrorException{}; return temp.unclosedstring(arg_0) }(rt.sub(var_reader.getposition(), rt.new_int(1))))
	}
	mut var_string := rt.call_method(this.escaping, 'escapeUnicodeAndNewLine', [var_match.array_get(0)])
	var_stream.push(create_automattic_woocommerce_emaileditorvendor_symfony_component_cssselector_parser_token(Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_Token.type_string(), var_string.dup(), var_reader.getposition()))
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_Handler_StringHandler) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_Tokenizer_TokenizerEscaping](if args.len > 1 { args[1] } else { rt.new_null() })
			this.construct(mut dispatch_arg_0, mut dispatch_arg_1)
			return rt.new_null()
		}
		'handle' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_Reader](if args.len > 0 { args[0] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_vendor_prefixed_packages_symfony_component_cssselector_parser_handler_stringhandler_php() {
}
