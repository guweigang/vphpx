...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat) get(var_sName rt.PhpVal) rt.PhpVal {
	mut var_aVarPrefixes := rt.create_array([rt.ArrayItem{ key: none, val: 'a' }, rt.ArrayItem{ key: none, val: 's' }, rt.ArrayItem{ key: none, val: 'm' }, rt.ArrayItem{ key: none, val: 'b' }, rt.ArrayItem{ key: none, val: 'f' }, rt.ArrayItem{ key: none, val: 'o' }, rt.ArrayItem{ key: none, val: 'c' }, rt.ArrayItem{ key: none, val: 'i' }])
	{
		...
		for {
			...
			if !(rt.get_property(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat', []string{}, &this), '{"nodeType":"Expr_Variable","line":261,"name":"sFieldName"}')).is_null() {
				return rt.get_property(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat', []string{}, &this), '{"nodeType":"Expr_Variable","line":262,"name":"sFieldName"}')
			}
		}
	}
	return rt.new_null()
		...
		for {
			...
			mut var_bDidReplace := rt.new_bool(rt.new_bool(false))
			{
				...
				for {
					...
					mut var_sFieldName := rt.new_string(rt.concat(var_sPrefix, rt.call_function('ucfirst', [var_sName.dup()])))
					if !(rt.get_property(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat', []string{}, &this), '{"nodeType":"Expr_Variable","line":293,"name":"sFieldName"}')).is_null() {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat) magic_call(var_sMethodName rt.PhpVal, mut var_aArguments Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_array) rt.PhpVal {
	if rt.is_true(rt.identical(rt.call_function('strpos', [var_sMethodName.dup(), rt.new_string('set')]), rt.new_int(0))) {
	...
	} else if rt.is_true(rt.call_function('method_exists', [Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormatter.class(), var_sMethodName.dup()])) {
		...
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Exception', []string{}, create_automattic_woocommerce_emaileditorvendor_sabberworm_css_exception('Unknown OutputFormat method called: ' + (var_sMethodName).str())))
	}
	return rt.new_null()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat) getformatter() rt.PhpVal {
	if rt.is_true(rt.identical(this.oFormatter, rt.new_null())) {
		this.oFormatter = create_automattic_woocommerce_emaileditorvendor_sabberworm_css_outputformatter(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat', []string{}, &this).dup())
	}
...
fn Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat.create() rt.PhpVal {
	return create_automattic_woocommerce_emaileditorvendor_sabberworm_css_outputformat()
}

fn Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat.createcompact() rt.PhpVal {
	mut var_format := Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat.create()
...
fn create_automattic_woocommerce_emaileditorvendor_sabberworm_css_exception() &Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Exception {
...
fn create_automattic_woocommerce_emaileditorvendor_sabberworm_css_outputformatter() &Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormatter {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_OutputFormat) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'__call' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_array](if args.len > 1 { args[1] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_vendor_prefixed_packages_sabberworm_css_outputformat_php() {
}
