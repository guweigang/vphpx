...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Utilities_CssConcatenator) getcss() string {
	return (rt.call_function('implode', [rt.new_string(''), rt.call_function('array_map', [rt.create_array([rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Utilities_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Utilities_CssConcatenator.class() }, rt.ArrayItem{ key: none, val: 'getMediaRuleCss' }]), this.mediaRules])])).str()
}
...
fn create_automattic_woocommerce_emaileditorvendor_pelago_emogrifier_utilities_cssconcatenator() &Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Utilities_CssConcatenator {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Utilities_CssConcatenator) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'append' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Utilities_array](if args.len > 0 { args[0] } else { rt.new_null() })
		...
		'hasEquivalentSelectors' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Utilities_array](if args.len > 0 { args[0] } else { rt.new_null() })
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Utilities_array](if args.len > 1 { args[1] } else { rt.new_null() })
			return rt.new_bool(Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Utilities_CssConcatenator.hasequivalentselectors(mut dispatch_arg_0, mut dispatch_arg_1))
		}
		'getMediaRuleCss' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditorVendor_Pelago_Emogrifier_Utilities_object](if args.len > 0 { args[0] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_packages_email_editor_vendor_prefixed_packages_pelago_emogrifier_utilities_cssconcatenator_php() {
