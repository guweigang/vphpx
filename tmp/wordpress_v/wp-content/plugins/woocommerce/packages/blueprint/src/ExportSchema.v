...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ExportSchema) construct(var_exporters rt.PhpVal) {
	...
	this.exporters = var_exporters_mutated.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ExportSchema) export(var_steps rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [
		rt.new_string('#^/$|^/[^/].*#'),
		var_loading_page_path.dup(),
	])))))
	{
		return create_wp_error(rt.new_string('wooblueprint_invalid_landing_page_path'),
	...
	var_exporters = rt.call_function('array_filter', [var_exporters.dup(),
		rt.new_closure(closure_1_fn)])
	if rt.is_true(rt.new_int(var_steps.dup().array_count())) {
		{
			...
			for {
				...
				if rt.is_true(rt.new_bool(
					rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_name.dup(), var_steps.dup(), rt.new_bool(true)])))))
					&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_alias.dup(), var_steps.dup(), rt.new_bool(true)])))))))
				{
	...
	}
	{
		...
		for {
			...
			{
				return create_wp_error(rt.new_string('wooblueprint_insufficient_permissions'),
	...
	var_logger.start_export(var_exporters.dup())
	{
		mut iter_1 := var_exporters.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_exporter := item_1.val
			this.publish(rt.new_string('onBeforeExport'), var_exporter.dup())
			if rt.has_exception() {
				unsafe {
					goto catch_label_1
				}
			...
			if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Blueprint_Throwable') {
				...
				var_logger.export_step_failed(var_step_name.dup(), var_e.dup())
				return create_wp_error(rt.new_string('wooblueprint_export_step_failed'),
					'Export step failed: ' +
	...
	var_logger.complete_export(var_exporters.dup())
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ExportSchema) on_before_export(var_step_name rt.PhpVal, var_callback rt.PhpVal) {
	...
	closure_2_fn := fn [var_step_name, var_callback] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		{
			rt.call_callable(var_callback, [var_exporter.dup()])
		}
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ExportSchema) add_result_to_schema(mut var_schema Class_Automattic_WooCommerce_Blueprint_array, var_step rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(var_step_mutated.dup().is_array())) {
		{
			mut iter_1 := var_step_mutated.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var__step := item_1.val
...
pub fn init_wp_content_plugins_woocommerce_packages_blueprint_src_exportschema_php() {
}
