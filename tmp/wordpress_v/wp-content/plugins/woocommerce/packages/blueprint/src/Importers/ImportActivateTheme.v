...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportActivateTheme) process(var_schema rt.PhpVal) rt.PhpVal {
	mut var_result := fn (arg_0 rt.PhpVal) rt.PhpVal {
		...
		mut temp := Class_Automattic_WooCommerce_Blueprint_Steps_ActivateTheme{}
		return temp.get_step_name()
	}())
	mut var_name := rt.get_property(var_schema, 'themeName')
	this.wp_switch_theme(var_name.dup())
...
fn create_automattic_woocommerce_blueprint_steps_activatetheme() &Class_Automattic_WooCommerce_Blueprint_Steps_ActivateTheme {
...
pub fn init_wp_content_plugins_woocommerce_packages_blueprint_src_importers_importactivatetheme_php() {
}
