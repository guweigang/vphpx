import rt

pub fn init_wp_content_plugins_woocommerce_packages_blueprint_src_steps_schemas_definitions_corethemereference_php() {
	return rt.create_array([rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			rt.ArrayItem{ key: 'resource', val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{ key: 'const', val: 'wordpress.org/themes' },
				rt.ArrayItem{
					key: 'description'
					val: 'Identifies the file resource as a WordPress Core theme'
				},
			]) },
			rt.ArrayItem{ key: 'slug', val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{ key: 'description', val: 'The slug of the WordPress Core theme' },
			]) },
		]) }, rt.ArrayItem{ key: 'required', val: rt.create_array([
			rt.ArrayItem{ key: none, val: 'resource' },
			rt.ArrayItem{ key: none, val: 'slug' },
		]) }, rt.ArrayItem{ key: 'additionalProperties', val: false }])
}
