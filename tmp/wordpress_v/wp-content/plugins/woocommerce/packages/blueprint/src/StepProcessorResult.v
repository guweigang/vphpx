...
fn Class_Automattic_WooCommerce_Blueprint_StepProcessorResult.success(stp_name string) rt.PhpVal {
	return create_automattic_woocommerce_blueprint_self(rt.new_bool(true), rt.new_string(stp_name).dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_StepProcessorResult) get_messages(type string) rt.PhpVal {
	...
	return rt.identical(rt.new_string(type), rt.new_string(var_message).array_get('type'))
	}
...
pub fn init_wp_content_plugins_woocommerce_packages_blueprint_src_stepprocessorresult_php() {
}
