		...
		for {
			...
			mut var_step_name := rt.call_method(var_result, 'get_step_name', []rt.PhpVal{})
			{
				...
				for {
					...
					if !(var_data.array_get('messages').array_isset(var_message.array_get('type'))) {
						var_data.array_get_mut('messages').array_set(var_message.array_get('type'),
							rt.new_array())
					}
					var_data.array_get_mut('messages').array_get_mut(var_message.array_get('type')).array_push(rt.create_array([
						rt.ArrayItem{ key: 'step', val: var_step_name },
						rt.ArrayItem{ key: 'type', val: var_message.array_get('type') },
						rt.ArrayItem{ key: 'message', val: var_message.array_get('message') },
					]))
				}
			}
		}
...
pub fn init_wp_content_plugins_woocommerce_packages_blueprint_src_resultformatters_jsonresultformatter_php() {
}
