...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Exporters_ExportInstallPluginSteps) include_private_plugins(boolean bool)  {
	this.include_private_plugins = rt.new_bool(boolean).dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Exporters_ExportInstallPluginSteps) filter(mut var_callback Class_Automattic_WooCommerce_Blueprint_Exporters_callable)  {
	this.filter_callback = var_callback.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Exporters_ExportInstallPluginSteps) export() rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_callable', [this.filter_callback])) {
		var_plugins = rt.call_function('call_user_func', [this.filter_callback, var_plugins.dup()])
	}
	...
	mut var_steps := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.call_function('in_array', [var_plugin.array_get('Name'), var_exclude.dup(), rt.new_bool(true)])) {
				continue
			}
			mut var_slug := rt.call_function('dirname', [var_path.dup()])
			if rt.is_true(rt.identical(rt.new_string('.'), var_slug)) {
				var_slug = rt.call_function('pathinfo', [var_path.dup()]).array_get('filename')
			}
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_bool(false), this.include_private_plugins)) && rt.is_true(rt.new_bool(!(rt.is_true(var_has_download_link)))))) {
				continue
			}
			mut var_resource := rt.new_string(if rt.is_true(var_has_download_link) { rt.new_string('wordpress.org/plugins') } else { rt.new_string('self/plugins') })
			var_steps.array_push(create_automattic_woocommerce_blueprint_steps_installplugin(var_slug.dup(), var_resource.dup(), rt.create_array([rt.ArrayItem{ key: 'activate', val: true }])))
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Exporters_ExportInstallPluginSteps) sort_plugins_by_dep(mut var_plugins Class_Automattic_WooCommerce_Blueprint_Exporters_array) rt.PhpVal {
	...
	mut var_title := rt.new_string(rt.new_string(if !(var_plugins_mutated.array_get(var_key).array_get('Title')).is_null() { var_plugins_mutated.array_get(var_key).array_get('Title') } else { rt.new_string('') }.to_string().to_lower()))
	if rt.is_true(var_title) {
		var_carry.array_set(var_title, var_key.dup())
	}
	return var_carry.dup()
	}
	mut var_title_map := rt.call_function('array_reduce', [rt.func_array_keys(var_plugins_mutated.dup()), rt.new_closure(closure_1_fn), rt.new_array()])
	...
	mut var_plugin_key := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	if var_visited.array_isset(var_plugin_key) {
		return rt.new_null()
	}
	var_visited.array_set(var_plugin_key, true)
	mut var_requires := if !(var_plugins_mutated.array_get(var_plugin_key).array_get('RequiresPlugins')).is_null() { var_plugins_mutated.array_get(var_plugin_key).array_get('RequiresPlugins') } else { rt.new_array() }
	{
		mut iter_1 := rt.cast_array(var_requires).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_dependency := item_1.val
			mut var_dependency_key := if !(var_title_map.array_get(var_dependency.dup().to_string().to_lower())).is_null() { var_title_map.array_get(var_dependency.dup().to_string().to_lower()) } else { rt.new_null() }
			if rt.is_true(var_dependency_key) {
				rt.call_callable(var_visit, [var_dependency_key.dup()])
			}
		}
	}
	...
	{
		mut iter_1 := rt.func_array_keys(var_plugins_mutated.dup()).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_plugin_key := item_1.val
			rt.call_callable(var_visit, [var_plugin_key.dup()])
		}
	}
	return var_sorted.dup()
}

fn (mut this Class_Automattic_WooCommerce_Blueprint_Exporters_ExportInstallPluginSteps) get_step_name() rt.PhpVal {
	return fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blueprint_Steps_InstallPlugin{}; return temp.get_step_name() }()
}
...
pub fn init_wp_content_plugins_woocommerce_packages_blueprint_src_exporters_exportinstallpluginsteps_php() {
}
