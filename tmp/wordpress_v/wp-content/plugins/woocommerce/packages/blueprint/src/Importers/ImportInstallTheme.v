...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportInstallTheme) construct(mut var_storage Class_Automattic_WooCommerce_Blueprint_ResourceStorages)  {
	this.result = fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blueprint_StepProcessorResult{}; return temp.success(arg_0) }(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blueprint_Steps_InstallTheme{}; return temp.get_step_name() }())
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportInstallTheme) process(var_schema rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		rt.call_method(this.result, 'add_info', [rt.new_string('Skipped installing a theme. Unsupported resource type. Only \'wordpress.org/themes\' is supported at the moment.')])
		return this.result
	}
	if !(!(rt.get_property(var_schema_mutated, 'options')).is_null()) {
		rt.set_property(var_schema_mutated, 'options', create_automattic_woocommerce_blueprint_importers_stdclass())
	}
	if var_installed_themes.array_isset(rt.get_property(var_theme, 'slug')) {
		this.activate_theme(var_schema_mutated.dup())
		rt.call_method(this.result, 'add_info', [rt.concat(rt.concat(rt.new_string('Skipped installing '), rt.get_property(var_theme, 'slug')), rt.new_string('. It is already installed.'))])
		return this.result
	}
	if rt.is_true(rt.identical(rt.call_method(this.storage, 'is_supported_resource', [rt.get_property(var_theme, 'resource')]), rt.new_bool(false))) {
		rt.call_method(this.result, 'add_error', [rt.concat(rt.new_string('Invalid resource type for '), rt.get_property(var_theme, 'slug'))])
		return this.result
	}
	mut var_downloaded_path := rt.call_method(this.storage, 'download', [rt.get_property(var_theme, 'slug'), rt.get_property(var_theme, 'resource')])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_downloaded_path)))) {
		rt.call_method(this.result, 'add_error', [rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('Unable to download '), rt.get_property(var_theme, 'slug')), rt.new_string(' with ')), rt.get_property(var_theme, 'resource')), rt.new_string(' resource type.'))])
		return this.result
	}
	rt.call_method(this.result, 'add_debug', [rt.concat(rt.concat(rt.concat(rt.new_string('\''), rt.get_property(var_theme, 'slug')), rt.new_string('\' has been downloaded in ')), var_downloaded_path)])
	mut var_install := rt.new_bool(this.install(var_downloaded_path.dup()))
	if rt.is_true(var_install) {
		rt.call_method(this.result, 'add_debug', [rt.concat(rt.concat(rt.new_string('Theme \''), rt.get_property(var_theme, 'slug')), rt.new_string('\' installed successfully.'))])
	} else {
		rt.call_method(this.result, 'add_error', [rt.concat(rt.concat(rt.new_string('Failed to install theme \''), rt.get_property(var_theme, 'slug')), rt.new_string('\'.'))])
	}
	this.activate_theme(var_schema_mutated.dup())
	return this.result
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportInstallTheme) activate_theme(var_schema rt.PhpVal)  {
	mut var_schema_mutated := var_schema
	mut var_theme := rt.get_property(var_schema_mutated, 'themeData')
	if rt.is_true(rt.new_bool(!(rt.get_property(rt.get_property(var_schema_mutated, 'options'), 'activate')).is_null() && rt.is_true(rt.identical(rt.new_bool(true), rt.get_property(rt.get_property(var_schema_mutated, 'options'), 'activate'))))) {
		...
		if rt.is_true(rt.identical(var_current_theme, rt.get_property(var_theme, 'slug'))) {
			rt.call_method(this.result, 'add_info', [rt.concat(rt.concat(rt.new_string('Switched theme to \''), rt.get_property(var_theme, 'slug')), rt.new_string('\'.'))])
		} else {
			rt.call_method(this.result, 'add_error', [rt.concat(rt.concat(rt.new_string('Failed to switch theme to \''), rt.get_property(var_theme, 'slug')), rt.new_string('\'.'))])
		}
	}
}

fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportInstallTheme) install(var_local_path rt.PhpVal) bool {
	mut var_unzip_result := this.wp_unzip_file(var_local_path.dup(), this.wp_get_theme_root())
	if rt.is_true(this.is_wp_error(var_unzip_result.dup())) {
		return false
	}
	return true
}
...
fn create_automattic_woocommerce_blueprint_steps_installtheme() &Class_Automattic_WooCommerce_Blueprint_Steps_InstallTheme {
...
fn create_automattic_woocommerce_blueprint_importers_stdclass() &Class_Automattic_WooCommerce_Blueprint_Importers_stdClass {
...
pub fn init_wp_content_plugins_woocommerce_packages_blueprint_src_importers_importinstalltheme_php() {
}
