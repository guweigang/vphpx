...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Exporters_ExportInstallThemeSteps) export() rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_callable', [this.filter_callback])) {
		var_themes = rt.call_function('call_user_func', [this.filter_callback, var_themes.dup()])
	}
	mut var_active_theme := this.wp_get_theme()
	{
		...
		for {
			...
			if !(rt.get_property(var_info, 'download_link')).is_null() {
				var_steps.array_push(create_automattic_woocommerce_blueprint_steps_installtheme(var_slug.dup(),
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Exporters_ExportInstallThemeSteps) get_step_name() rt.PhpVal {
	return fn () rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Blueprint_Steps_InstallTheme{}
...
pub fn init_wp_content_plugins_woocommerce_packages_blueprint_src_exporters_exportinstallthemesteps_php() {
}
