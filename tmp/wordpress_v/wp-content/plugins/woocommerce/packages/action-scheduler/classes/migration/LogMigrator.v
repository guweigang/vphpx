...
fn (mut this Class_Action_Scheduler_Migration_LogMigrator) construct(mut var_source_logger Class_ActionScheduler_Logger, mut var_destination_logger Class_ActionScheduler_Logger) {
	this.source = var_source_logger.dup()
	this.destination = var_destination_logger.dup()
}
...
fn (mut this Class_Action_Scheduler_Migration_LogMigrator) migrate(var_source_action_id rt.PhpVal, var_destination_action_id rt.PhpVal) {
		...
		var_source_action_id.dup()])
	{
		...
		for {
			...
			]), rt.call_function('absint', [var_source_action_id.dup()])))
			{
				rt.call_method(this.destination, 'log', [var_destination_action_id.dup(),
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_migration_logmigrator_php() {
}
