...
fn (mut this Class_ActionScheduler_QueueCleaner) construct(mut var_store Class_?ActionScheduler_Store, batch_size i64)  {
	...
	this.batch_size = rt.new_int(batch_size_mutated).dup()
}
...
fn (mut this Class_ActionScheduler_QueueCleaner) delete_old_actions() rt.PhpVal {
	...
	mut var_cutoff := rt.call_function('as_get_datetime_object', [(var_lifespan).str() + ' seconds ago'])
...
fn (mut this Class_ActionScheduler_QueueCleaner) clean_actions(mut var_statuses_to_purge Class_array, mut var_cutoff_date Class_DateTime, var_batch_size rt.PhpVal, context string) rt.PhpVal {
	...
	var_batch_size_mutated = if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_batch_size_mutated.dup().is_null()))))) { var_batch_size_mutated } else { this.batch_size }
	mut var_cutoff := if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_cutoff_date.is_null()))))) { var_cutoff_date } else { rt.call_function('as_get_datetime_object', [(this.month_in_seconds).str() + ' seconds ago']) }
	...
	mut var_deleted_actions := rt.new_array()
	{
		...
		for {
			...
			var_deleted_actions = rt.call_function('array_merge', [var_deleted_actions.dup(), this.delete_actions(mut rt.cast_object_ptr[Class_array](var_actions_to_delete), var_lifespan.dup(), context)])
		}
	}
	return var_deleted_actions.dup()
}
...
fn (mut this Class_ActionScheduler_QueueCleaner) delete_actions(mut var_actions_to_delete Class_array, var_lifespan rt.PhpVal, context string) rt.PhpVal {
	...
	mut var_deleted_actions := rt.new_array()
	if rt.is_true(rt.new_bool(var_lifespan_mutated.dup().is_null())) {
		var_lifespan_mutated = this.month_in_seconds
	}
	{
		mut iter_1 := var_actions_to_delete_mutated.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_action_id := item_1.val
			rt.call_method(this.store, 'delete_action', [var_action_id.dup()])
			if rt.has_exception() { unsafe { goto catch_label_2 } }
			var_deleted_actions.array_push(var_action_id.dup())
			if rt.has_exception() { unsafe { goto catch_label_2 } }
...
catch_label_2:
			...
			if rt.instance_of(var_e_2, 'Exception') {
				...
				rt.call_function('do_action', [rt.new_string("action_scheduler_failed_${var_context}_action_deletion"), var_action_id.dup(), var_e.dup(), var_lifespan_mutated.dup(), rt.new_int(var_actions_to_delete_mutated.dup().array_count())])
				unsafe { goto end_label_2 }
			}
			else {
...
fn (mut this Class_ActionScheduler_QueueCleaner) reset_timeouts(time_limit i64)  {
	...
	mut var_cutoff := rt.call_function('as_get_datetime_object', [(var_timeout).str() + ' seconds ago'])
	...
	{
		mut iter_1 := var_actions_to_reset.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_action_id := item_1.val
			rt.call_method(this.store, 'unclaim_action', [var_action_id.dup()])
			rt.call_function('do_action', [rt.new_string('action_scheduler_reset_action'), var_action_id.dup()])
		}
	}
}
...
fn (mut this Class_ActionScheduler_QueueCleaner) mark_failures(time_limit i64)  {
	...
	mut var_actions_to_reset := rt.call_method(this.store, 'query_actions', [rt.create_array([rt.ArrayItem{ key: 'status', val: Class_ActionScheduler_Store.status_running() }, rt.ArrayItem{ key: 'modified', val: var_cutoff }, rt.ArrayItem{ key: 'modified_compare', val: '<=' }, rt.ArrayItem{ key: 'per_page', val: this.get_batch_size() }, rt.ArrayItem{ key: 'orderby', val: 'none' }])])
	{
		...
		for {
			...
			rt.call_function('do_action', [rt.new_string('action_scheduler_failed_action'), var_action_id.dup(), var_timeout.dup()])
		}
	}
}
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_actionscheduler_queuecleaner_php() {
}
