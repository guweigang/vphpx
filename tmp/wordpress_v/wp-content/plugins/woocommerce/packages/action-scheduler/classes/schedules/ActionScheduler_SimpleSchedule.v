...
fn (mut this Class_ActionScheduler_SimpleSchedule) magic_sleep() rt.PhpVal {
	...
	return rt.call_function('array_merge', [var_sleep_params.dup(),
		rt.create_array([rt.ArrayItem{ key: none, val: 'timestamp' }])])
}

fn (mut this Class_ActionScheduler_SimpleSchedule) magic_wakeup() {
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.get_property(rt.new_object('ActionScheduler_SimpleSchedule', ['ActionScheduler_Abstract_Schedule'], &this), 'scheduled_timestamp').is_null()))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.timestamp.is_null())))))))
	{
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_schedules_actionscheduler_simpleschedule_php() {
}
