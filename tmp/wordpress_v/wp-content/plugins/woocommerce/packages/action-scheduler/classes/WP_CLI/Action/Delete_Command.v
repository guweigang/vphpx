...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Delete_Command) execute()  {
	...
	mut var_progress_bar := rt.call_function('WP_CLI\Utils\make_progress_bar', [rt.call_function('sprintf', [rt.call_function('_n', [rt.new_string('Deleting %d action'), rt.new_string('Deleting %d actions'), this.action_counts.array_get('total'), rt.new_string('woocommerce')]), rt.call_function('number_format_i18n', [this.action_counts.array_get('total')])]), this.action_counts.array_get('total')])
	{
...
catch_label_1:
			...
			if rt.instance_of(var_e_1, 'Action_Scheduler_WP_CLI_Action_Exception') {
				...
				fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}; return temp.warning(arg_0) }(rt.call_method(var_e, 'getMessage', []rt.PhpVal{}))
				unsafe { goto end_label_1 }
			}
			else {
...
end_label_1:
	...
	mut var_format := rt.new_string((rt.call_function('_n', [rt.new_string('Deleted %1$d action'), rt.new_string('Deleted %1$d actions'), this.action_counts.array_get('deleted'), rt.new_string('woocommerce')])).str() + ', ')
	// unsupported expression: Expr_AssignOp_Concat
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}; return temp.success(arg_0) }(rt.call_function('sprintf', [var_format.dup(), rt.call_function('number_format_i18n', [this.action_counts.array_get('deleted')]), rt.call_function('number_format_i18n', [this.action_counts.array_get('failed')])]))
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Delete_Command) on_action_deleted(var_action_id rt.PhpVal)  {
	...
	rt.post_inc(this.action_counts.array_get('deleted'))
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}; return temp.debug(arg_0) }(rt.call_function('sprintf', [rt.new_string('Action %d was deleted.'), var_action_id_mutated.dup()]))
}
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_wp_cli_action_delete_command_php() {
}
