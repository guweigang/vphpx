...
fn (mut this Class_Action_Scheduler_Migration_Config) get_source_store() rt.PhpVal {
	if !rt.is_true(this.source_store) {
		rt.throw_exception(rt.new_object('Action_Scheduler_Migration_RuntimeException', []string{}, create_action_scheduler_migration_runtimeexception(rt.call_function('__', [rt.new_string('Source store must be configured before running a migration'), rt.new_string('woocommerce')]))))
	}
...
fn (mut this Class_Action_Scheduler_Migration_Config) set_source_store(mut var_store Class_ActionScheduler_Store)  {
	this.source_store = var_store.dup()
}

fn (mut this Class_Action_Scheduler_Migration_Config) get_source_logger() rt.PhpVal {
	if !rt.is_true(this.source_logger) {
		rt.throw_exception(rt.new_object('Action_Scheduler_Migration_RuntimeException', []string{}, create_action_scheduler_migration_runtimeexception(rt.call_function('__', [rt.new_string('Source logger must be configured before running a migration'), rt.new_string('woocommerce')]))))
	}
...
fn (mut this Class_Action_Scheduler_Migration_Config) set_source_logger(mut var_logger Class_ActionScheduler_Logger)  {
	this.source_logger = var_logger.dup()
}

fn (mut this Class_Action_Scheduler_Migration_Config) get_destination_store() rt.PhpVal {
	if !rt.is_true(this.destination_store) {
		rt.throw_exception(rt.new_object('Action_Scheduler_Migration_RuntimeException', []string{}, create_action_scheduler_migration_runtimeexception(rt.call_function('__', [rt.new_string('Destination store must be configured before running a migration'), rt.new_string('woocommerce')]))))
	}
...
fn (mut this Class_Action_Scheduler_Migration_Config) set_destination_store(mut var_store Class_ActionScheduler_Store)  {
	this.destination_store = var_store.dup()
}
...
fn (mut this Class_Action_Scheduler_Migration_Config) get_destination_logger() rt.PhpVal {
	if !rt.is_true(this.destination_logger) {
		rt.throw_exception(rt.new_object('Action_Scheduler_Migration_RuntimeException', []string{}, create_action_scheduler_migration_runtimeexception(rt.call_function('__', [rt.new_string('Destination logger must be configured before running a migration'), rt.new_string('woocommerce')]))))
	}
...
fn (mut this Class_Action_Scheduler_Migration_Config) set_destination_logger(mut var_logger Class_ActionScheduler_Logger)  {
	this.destination_logger = var_logger.dup()
}
...
fn (mut this Class_Action_Scheduler_Migration_Config) set_dry_run(var_dry_run rt.PhpVal)  {
	this.dry_run = // unsupported expression: Expr_Cast_Bool
}
...
fn (mut this Class_Action_Scheduler_Migration_Config) set_progress_bar(mut var_progress_bar Class_Action_Scheduler_WP_CLI_ProgressBar)  {
	this.progress_bar = var_progress_bar.dup()
}
...
fn (mut this Class_Action_Scheduler_Migration_Config) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'set_source_logger' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_ActionScheduler_Logger](if args.len > 0 { args[0] } else { rt.new_null() })
		...
		'set_progress_bar' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Action_Scheduler_WP_CLI_ProgressBar](if args.len > 0 { args[0] } else { rt.new_null() })
...
fn (mut this Class_Action_Scheduler_Migration_Config) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'source_store' { this.source_store = val; return true }
		'source_logger' { this.source_logger = val; return true }
		'destination_store' { this.destination_store = val; return true }
		'destination_logger' { this.destination_logger = val; return true }
		'progress_bar' { this.progress_bar = val; return true }
		'dry_run' { this.dry_run = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_migration_config_php() {
}
