...
fn (mut this Class_ActionScheduler_IntervalSchedule) calculate_next(mut var_after Class_DateTime) rt.PhpVal {
	var_after.modify(rt.new_string('+' + (// unsupported expression: Expr_Cast_Int).str() + ' seconds'))
	return rt.new_object('DateTime', []string{}, var_after)
}

fn (mut this Class_ActionScheduler_IntervalSchedule) interval_in_seconds() rt.PhpVal {
	rt.call_function('_deprecated_function', [rt.new_string(@METHOD), rt.new_string('3.0.0'), rt.new_string('(int)ActionScheduler_Abstract_RecurringSchedule::get_recurrence()')])
...
fn (mut this Class_ActionScheduler_IntervalSchedule) magic_sleep() rt.PhpVal {
	...
	this.start_timestamp = rt.get_property(rt.new_object('ActionScheduler_IntervalSchedule', ['ActionScheduler_Abstract_RecurringSchedule', 'ActionScheduler_Schedule'], &this), 'scheduled_timestamp')
	this.interval_in_seconds = rt.get_property(rt.new_object('ActionScheduler_IntervalSchedule', ['ActionScheduler_Abstract_RecurringSchedule', 'ActionScheduler_Schedule'], &this), 'recurrence')
	return rt.call_function('array_merge', [var_sleep_params.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'start_timestamp' }, rt.ArrayItem{ key: none, val: 'interval_in_seconds' }])])
}

fn (mut this Class_ActionScheduler_IntervalSchedule) magic_wakeup()  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.get_property(rt.new_object('ActionScheduler_IntervalSchedule', ['ActionScheduler_Abstract_RecurringSchedule', 'ActionScheduler_Schedule'], &this), 'scheduled_timestamp').is_null())) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.start_timestamp.is_null()))))))) {
		this.dispatch_set_prop('scheduled_timestamp', this.start_timestamp)
		this.start_timestamp = rt.new_null()
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.get_property(rt.new_object('ActionScheduler_IntervalSchedule', ['ActionScheduler_Abstract_RecurringSchedule', 'ActionScheduler_Schedule'], &this), 'recurrence').is_null())) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.interval_in_seconds.is_null()))))))) {
...
fn (mut this Class_ActionScheduler_IntervalSchedule) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'interval_in_seconds' { this.interval_in_seconds = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_schedules_actionscheduler_intervalschedule_php() {
}
