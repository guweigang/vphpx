...
fn (mut this Class_ActionScheduler_DBStoreMigrator) save_action(mut var_action Class_ActionScheduler_Action, mut var_scheduled_date Class_?DateTime, mut var_last_attempt_date Class_?DateTime) rt.PhpVal {
	...
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		...
		if rt.has_exception() { unsafe { goto catch_label_1 } }
		rt.call_method(var_wpdb, 'update', [rt.get_property(var_wpdb, 'actionscheduler_actions'), var_data.dup(), rt.create_array([rt.ArrayItem{ key: 'action_id', val: var_action_id }]), rt.create_array([rt.ArrayItem{ key: none, val: '%s' }, rt.ArrayItem{ key: none, val: '%s' }]), rt.create_array([rt.ArrayItem{ key: none, val: '%d' }])])
		if rt.has_exception() { unsafe { goto catch_label_1 } }
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
fn create_actionscheduler_dbstoremigrator() &Class_ActionScheduler_DBStoreMigrator {
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_migration_actionscheduler_dbstoremigrator_php() {
}
