...
fn (mut this Class_ActionScheduler_QueueRunner) construct(mut var_store Class_?ActionScheduler_Store, mut var_monitor Class_?ActionScheduler_FatalErrorMonitor, mut var_cleaner Class_?ActionScheduler_QueueCleaner, mut var_async_request Class_?ActionScheduler_AsyncRequest_QueueRunner)  {
	...
	this.async_request = var_async_request_mutated.dup()
}
...
fn (mut this Class_ActionScheduler_QueueRunner) init()  {
	...
	mut var_next_timestamp := rt.call_function('wp_next_scheduled', [Class_ActionScheduler_QueueRunner.wp_cron_hook()])
	if rt.is_true(var_next_timestamp) {
		rt.call_function('wp_unschedule_event', [var_next_timestamp.dup(), Class_ActionScheduler_QueueRunner.wp_cron_hook()])
	}
	mut var_cron_context := ['WP Cron']
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_next_scheduled', [Class_ActionScheduler_QueueRunner.wp_cron_hook(), var_cron_context.dup()]))))) {
		mut var_schedule := rt.call_function('apply_filters', [rt.new_string('action_scheduler_run_schedule'), Class_ActionScheduler_QueueRunner.wp_cron_schedule()])
		rt.call_function('wp_schedule_event', [rt.call_function('time', []rt.PhpVal{}), var_schedule.dup(), Class_ActionScheduler_QueueRunner.wp_cron_hook(), var_cron_context.dup()])
	}
	rt.call_function('add_action', [Class_ActionScheduler_QueueRunner.wp_cron_hook(), rt.create_array([rt.ArrayItem{ key: none, val: Class_ActionScheduler_QueueRunner.instance() }, rt.ArrayItem{ key: none, val: 'run' }])])
...
fn (mut this Class_ActionScheduler_QueueRunner) run(context string) i64 {
	fn () rt.PhpVal { mut temp := Class_ActionScheduler_Compatibility{}; return temp.raise_memory_limit() }()
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_ActionScheduler_Compatibility{}; return temp.raise_time_limit(arg_0) }(this.get_time_limit())
	...
	if rt.is_true(rt.identical(rt.new_bool(false), this.has_maximum_concurrent_batches())) {
		for {
			...
			if !(rt.is_true(rt.new_bool(rt.is_true(rt.greater(var_processed_actions_in_batch, rt.new_int(0))) && rt.is_true(rt.new_bool(!(rt.is_true(this.batch_limits_exceeded(rt.new_int(this.processed_actions_count))))))))) {
				break
			}
		}
...
fn (mut this Class_ActionScheduler_QueueRunner) do_batch(size i64, context string) rt.PhpVal {
	...
	rt.call_method(rt.get_property(rt.new_object('ActionScheduler_QueueRunner', ['ActionScheduler_Abstract_QueueRunner'], &this), 'monitor'), 'attach', [var_claim.dup()])
	mut var_processed_actions := rt.new_int(rt.new_int(0))
	{
		mut iter_1 := rt.call_method(var_claim, 'get_actions', []rt.PhpVal{}).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_action_id := item_1.val
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_action_id.dup(), rt.call_method(rt.get_property(rt.new_object('ActionScheduler_QueueRunner', ['ActionScheduler_Abstract_QueueRunner'], &this), 'store'), 'find_actions_by_claim_id', [rt.call_method(var_claim, 'get_id', []rt.PhpVal{})]), rt.new_bool(true)]))))) {
				break
			}
	...
	rt.call_method(rt.get_property(rt.new_object('ActionScheduler_QueueRunner', ['ActionScheduler_Abstract_QueueRunner'], &this), 'store'), 'release_claim', [var_claim.dup()])
	...
	this.clear_caches()
	return var_processed_actions.dup()
}

fn (mut this Class_ActionScheduler_QueueRunner) clear_caches()  {
	mut var_flushing_runtime_cache_explicitly_supported := rt.new_bool(rt.new_bool(rt.is_true(rt.call_function('function_exists', [rt.new_string('wp_cache_supports')])) && rt.is_true(rt.call_function('wp_cache_supports', [rt.new_string('flush_runtime')]))))
	mut var_flushing_runtime_cache_implicitly_supported := rt.new_bool(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('wp_cache_supports')]))))) && rt.is_true(rt.call_function('function_exists', [rt.new_string('wp_cache_flush_runtime')]))))
	if rt.is_true(rt.new_bool(rt.is_true(var_flushing_runtime_cache_explicitly_supported) || rt.is_true(var_flushing_runtime_cache_implicitly_supported))) {
		rt.call_function('wp_cache_flush_runtime', []rt.PhpVal{})
	} else if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_using_ext_object_cache', []rt.PhpVal{}))))) || rt.is_true(rt.call_function('apply_filters', [rt.new_string('action_scheduler_queue_runner_flush_cache'), rt.new_bool(false)])))) {
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_actionscheduler_queuerunner_php() {
}
