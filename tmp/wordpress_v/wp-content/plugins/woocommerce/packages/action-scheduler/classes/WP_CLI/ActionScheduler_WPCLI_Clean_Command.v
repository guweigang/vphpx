...
fn (mut this Class_ActionScheduler_WPCLI_Clean_Command) clean(var_args rt.PhpVal, var_assoc_args rt.PhpVal)  {
	mut var_batch := rt.call_function('absint', [rt.call_function('WP_CLI\Utils\get_flag_value', [var_assoc_args.dup(), rt.new_string('batch-size'), rt.new_int(20)])])
	...
	mut var_status := rt.call_function('explode', [rt.new_string(','), rt.call_function('WP_CLI\Utils\get_flag_value', [var_assoc_args.dup(), rt.new_string('status'), rt.new_string('')])])
	var_status = rt.call_function('array_filter', [rt.call_function('array_map', [rt.new_string('trim'), var_status.dup()])])
	mut var_before := rt.call_function('WP_CLI\Utils\get_flag_value', [var_assoc_args.dup(), rt.new_string('before'), rt.new_string('')])
	...
	mut var_actions_deleted := rt.new_int(rt.new_int(0))
	mut var_unlimited := rt.identical(rt.new_int(0), var_batches)
	mut var_lifespan := rt.call_function('as_get_datetime_object', [var_before.dup()])
...
end_label_1:
	mut var_cleaner := create_actionscheduler_queuecleaner(rt.new_null(), var_batch.dup())
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	for rt.is_true(rt.new_bool(rt.is_true(var_unlimited) || rt.is_true(rt.less(var_batches_completed, var_batches)))) {
		if rt.is_true(rt.new_bool(rt.is_true(var_sleep) && rt.is_true(rt.greater(var_batches_completed, rt.new_int(0))))) {
			rt.call_function('sleep', [var_sleep.dup()])
			if rt.has_exception() { unsafe { goto catch_label_2 } }
		}
		if rt.has_exception() { unsafe { goto catch_label_2 } }
		mut var_deleted := rt.new_int(rt.new_int(var_cleaner.clean_actions(var_status.dup(), var_lifespan.dup(), rt.new_null(), rt.new_string('CLI')).array_count()))
		if rt.has_exception() { unsafe { goto catch_label_2 } }
...
fn (mut this Class_ActionScheduler_WPCLI_Clean_Command) print_total_batches(batches_processed i64)  {
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WP_CLI{}; return temp.log(arg_0) }(rt.call_function('sprintf', [rt.call_function('_n', [rt.new_string('%d batch processed.'), rt.new_string('%d batches processed.'), rt.new_int(batches_processed), rt.new_string('woocommerce')]), rt.new_int(batches_processed)]))
}

fn (mut this Class_ActionScheduler_WPCLI_Clean_Command) print_error(mut var_e Class_Exception)  {
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WP_CLI{}; return temp.error(arg_0) }(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('There was an error deleting an action: %s'), rt.new_string('woocommerce')]), var_e.getmessage()]))
}
...
fn (mut this Class_ActionScheduler_WPCLI_Clean_Command) print_success(actions_deleted i64)  {
	...
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WP_CLI{}; return temp.success(arg_0) }(rt.call_function('sprintf', [rt.call_function('_n', [rt.new_string('%d action deleted.'), rt.new_string('%d actions deleted.'), rt.new_int(actions_deleted_mutated).dup(), rt.new_string('woocommerce')]), rt.new_int(actions_deleted_mutated).dup()]))
}
...
fn create_actionscheduler_wpcli_clean_command() &Class_ActionScheduler_WPCLI_Clean_Command {
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_wp_cli_actionscheduler_wpcli_clean_command_php() {
}
