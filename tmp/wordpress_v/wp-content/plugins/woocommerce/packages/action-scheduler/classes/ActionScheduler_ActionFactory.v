...
fn (mut this Class_ActionScheduler_ActionFactory) get_stored_action(var_status rt.PhpVal, var_hook rt.PhpVal, mut var_args Class_array, mut var_schedule Class_?ActionScheduler_Schedule, group string) rt.PhpVal {
	...
	mut var_priority := if rt.is_true(rt.greater_equal(rt.call_function('func_num_args', []rt.PhpVal{}), rt.new_int(6))) { // unsupported expression: Expr_Cast_Int } else { rt.new_int(10) }
	mut switch_val_1 := var_status
	if rt.is_true(rt.equal(switch_val_1, Class_ActionScheduler_Store.status_pending())) {
		mut var_action_class := rt.new_string(rt.new_string('ActionScheduler_Action'))
	} else if rt.is_true(rt.equal(switch_val_1, Class_ActionScheduler_Store.status_canceled())) {
		var_action_class = rt.new_string(rt.new_string('ActionScheduler_CanceledAction'))
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_schedule_mutated.dup().is_null()))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [var_schedule_mutated.dup(), rt.new_string('ActionScheduler_CanceledSchedule')]))))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [var_schedule_mutated.dup(), rt.new_string('ActionScheduler_NullSchedule')]))))))) {
			var_schedule_mutated = create_actionscheduler_canceledschedule(rt.call_method(var_schedule_mutated, 'get_date', []rt.PhpVal{}))
		}
	} else {
		var_action_class = rt.new_string(rt.new_string('ActionScheduler_FinishedAction'))
	}
	var_action_class = rt.call_function('apply_filters', [rt.new_string('action_scheduler_stored_action_class'), var_action_class.dup(), var_status.dup(), var_hook.dup(), var_args, var_schedule_mutated.dup(), rt.new_string(group)])
	mut var_action := rt.create_object_dynamically(var_action_class, [var_hook.dup(), var_args, var_schedule_mutated.dup(), rt.new_string(group)])
	rt.call_method(var_action, 'set_priority', [var_priority.dup()])
	return rt.call_function('apply_filters', [rt.new_string('action_scheduler_stored_action_instance'), var_action.dup(), var_hook.dup(), var_args, var_schedule_mutated.dup(), rt.new_string(group), var_priority.dup()])
}

fn (mut this Class_ActionScheduler_ActionFactory) async(var_hook rt.PhpVal, var_args rt.PhpVal, group string) rt.PhpVal {
	return this.async_unique(var_hook.dup(), var_args.dup(), group, false)
}
...
fn (mut this Class_ActionScheduler_ActionFactory) async_unique(var_hook rt.PhpVal, var_args rt.PhpVal, group string, unique bool) rt.PhpVal {
	...
	mut var_action := create_actionscheduler_action(var_hook.dup(), var_args.dup(), var_schedule.dup(), rt.new_string(group).dup())
...
fn (mut this Class_ActionScheduler_ActionFactory) single(var_hook rt.PhpVal, var_args rt.PhpVal, var_when rt.PhpVal, group string) rt.PhpVal {
	return this.single_unique(var_hook.dup(), var_args.dup(), var_when.dup(), group, false)
}
...
fn (mut this Class_ActionScheduler_ActionFactory) single_unique(var_hook rt.PhpVal, var_args rt.PhpVal, var_when rt.PhpVal, group string, unique bool) rt.PhpVal {
	mut var_date := rt.call_function('as_get_datetime_object', [var_when.dup()])
	mut var_schedule := create_actionscheduler_simpleschedule(var_date.dup())
	mut var_action := create_actionscheduler_action(var_hook.dup(), var_args.dup(), var_schedule.dup(), rt.new_string(group).dup())
...
fn (mut this Class_ActionScheduler_ActionFactory) recurring(var_hook rt.PhpVal, var_args rt.PhpVal, var_first rt.PhpVal, var_interval rt.PhpVal, group string) rt.PhpVal {
	return this.recurring_unique(var_hook.dup(), var_args.dup(), var_first.dup(), var_interval.dup(), group, false)
}
...
fn (mut this Class_ActionScheduler_ActionFactory) recurring_unique(var_hook rt.PhpVal, var_args rt.PhpVal, var_first rt.PhpVal, var_interval rt.PhpVal, group string, unique bool) rt.PhpVal {
	if !rt.is_true(var_interval) {
		return this.single_unique(var_hook.dup(), var_args.dup(), var_first.dup(), group, unique)
	}
	mut var_date := rt.call_function('as_get_datetime_object', [var_first.dup()])
	mut var_schedule := create_actionscheduler_intervalschedule(var_date.dup(), var_interval.dup())
	mut var_action := create_actionscheduler_action(var_hook.dup(), var_args.dup(), var_schedule.dup(), rt.new_string(group).dup())
	return if var_unique { this.store_unique_action(mut rt.cast_object_ptr[Class_ActionScheduler_Action](var_action)) } else { this.store(mut rt.cast_object_ptr[Class_ActionScheduler_Action](var_action)) }
}
...
fn (mut this Class_ActionScheduler_ActionFactory) cron(var_hook rt.PhpVal, var_args rt.PhpVal, var_base_timestamp rt.PhpVal, var_schedule rt.PhpVal, group string) rt.PhpVal {
	mut var_schedule_mutated := var_schedule
	return this.cron_unique(var_hook.dup(), var_args.dup(), var_base_timestamp.dup(), var_schedule_mutated.dup(), group, false)
}
...
fn (mut this Class_ActionScheduler_ActionFactory) cron_unique(var_hook rt.PhpVal, var_args rt.PhpVal, var_base_timestamp rt.PhpVal, var_schedule rt.PhpVal, group string, unique bool) rt.PhpVal {
	...
	if !rt.is_true(var_schedule_mutated) {
		return this.single_unique(var_hook.dup(), var_args.dup(), var_base_timestamp.dup(), group, unique)
	}
	mut var_date := rt.call_function('as_get_datetime_object', [var_base_timestamp.dup()])
	mut var_cron := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_CronExpression{}; return temp.factory(arg_0) }(var_schedule_mutated.dup())
	var_schedule_mutated = create_actionscheduler_cronschedule(var_date.dup(), var_cron.dup())
	mut var_action := create_actionscheduler_action(var_hook.dup(), var_args.dup(), var_schedule_mutated.dup(), rt.new_string(group).dup())
	return if var_unique { this.store_unique_action(mut rt.cast_object_ptr[Class_ActionScheduler_Action](var_action)) } else { this.store(mut rt.cast_object_ptr[Class_ActionScheduler_Action](var_action)) }
}
...
fn (mut this Class_ActionScheduler_ActionFactory) repeat(var_action rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_next.dup().is_null())) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_schedule, 'is_recurring', []rt.PhpVal{}))))))) {
	...
	mut var_schedule_class := rt.call_function('get_class', [var_schedule.dup()])
	mut var_new_schedule := rt.create_object_dynamically(var_schedule, [var_next.dup(), rt.call_method(var_schedule, 'get_recurrence', []rt.PhpVal{}), rt.call_method(var_schedule, 'get_first_date', []rt.PhpVal{})])
	mut var_new_action := create_actionscheduler_action(rt.call_method(var_action_mutated, 'get_hook', []rt.PhpVal{}), rt.call_method(var_action_mutated, 'get_args', []rt.PhpVal{}), var_new_schedule.dup(), rt.call_method(var_action_mutated, 'get_group', []rt.PhpVal{}))
	var_new_action.set_priority(rt.call_method(var_action_mutated, 'get_priority', []rt.PhpVal{}))
	return this.store(mut var_new_action)
}

fn (mut this Class_ActionScheduler_ActionFactory) create(mut var_options Class_array) rt.PhpVal {
	...
	var_options_mutated = rt.call_function('array_merge', [var_defaults.dup(), var_options_mutated.dup()])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('cron'), var_options_mutated.array_get('type'))) || rt.is_true(rt.identical(rt.new_string('recurring'), var_options_mutated.array_get('type'))))) && !rt.is_true(var_options_mutated.array_get('pattern')))) {
		var_options_mutated.array_set('type', 'single')
	}
	mut switch_val_2 := var_options_mutated.array_get('type')
	if rt.is_true(rt.equal(switch_val_2, rt.new_string('async'))) {
		mut var_schedule := create_actionscheduler_nullschedule()
	} else if rt.is_true(rt.equal(switch_val_2, rt.new_string('cron'))) {
		mut var_date := rt.call_function('as_get_datetime_object', [var_options_mutated.array_get('when')])
		mut var_cron := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_CronExpression{}; return temp.factory(arg_0) }(var_options_mutated.array_get('pattern'))
		var_schedule = create_actionscheduler_cronschedule(var_date.dup(), var_cron.dup())
	} else if rt.is_true(rt.equal(switch_val_2, rt.new_string('recurring'))) {
		var_date = rt.call_function('as_get_datetime_object', [var_options_mutated.array_get('when')])
		var_schedule = create_actionscheduler_intervalschedule(var_date.dup(), var_options_mutated.array_get('pattern'))
	} else if rt.is_true(rt.equal(switch_val_2, rt.new_string('single'))) {
		var_date = rt.call_function('as_get_datetime_object', [var_options_mutated.array_get('when')])
		var_schedule = create_actionscheduler_simpleschedule(var_date.dup())
	} else {
		rt.call_function('error_log', [rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('Unknown action type \''), var_options_mutated.array_get('type')), rt.new_string('\' specified when trying to create an action for \'')), var_options_mutated.array_get('hook')), rt.new_string('\'.'))])
		return rt.new_int(0)
	}
	mut var_action := create_actionscheduler_action(var_options_mutated.array_get('hook'), var_options_mutated.array_get('arguments'), var_schedule.dup(), var_options_mutated.array_get('group'))
	rt.call_method(var_action, 'set_priority', [var_options_mutated.array_get('priority')])
	mut var_action_id := rt.new_int(rt.new_int(0))
	var_action_id = if rt.is_true(var_options_mutated.array_get('unique')) { this.store_unique_action(mut rt.cast_object_ptr[Class_ActionScheduler_Action](var_action)) } else { this.store(mut rt.cast_object_ptr[Class_ActionScheduler_Action](var_action)) }
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Exception') {
		...
		rt.call_function('error_log', [rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Caught exception while enqueuing action "%1$s": %2$s'), rt.new_string('woocommerce')]), var_options_mutated.array_get('hook'), rt.call_method(var_e, 'getMessage', []rt.PhpVal{})])])
		unsafe { goto end_label_1 }
	}
	else {
...
fn (mut this Class_ActionScheduler_ActionFactory) store(mut var_action Class_ActionScheduler_Action) rt.PhpVal {
	...
	return rt.call_method(var_store, 'save_action', [var_action_mutated.dup()])
}

fn (mut this Class_ActionScheduler_ActionFactory) store_unique_action(mut var_action Class_ActionScheduler_Action) i64 {
	mut var_action_mutated := var_action
	mut var_store := fn () rt.PhpVal { mut temp := Class_ActionScheduler_Store{}; return temp.instance() }()
	if rt.is_true(rt.call_function('method_exists', [var_store.dup(), rt.new_string('save_unique_action')])) {
		return (rt.call_method(var_store, 'save_unique_action', [var_action_mutated.dup()])).to_i64()
	} else {
		mut var_existing_action_id := // unsupported expression: Expr_Cast_Int
		if rt.is_true(rt.greater(var_existing_action_id, rt.new_int(0))) {
			return 0
		}
		return (rt.call_method(var_store, 'save_action', [var_action_mutated.dup()])).to_i64()
	}
	return i64(0)
}
...
fn create_actionscheduler_actionfactory() &Class_ActionScheduler_ActionFactory {
...
fn create_actionscheduler_intervalschedule() &Class_ActionScheduler_IntervalSchedule {
...
fn create_actionscheduler_cronschedule() &Class_ActionScheduler_CronSchedule {
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_actionscheduler_actionfactory_php() {
}
