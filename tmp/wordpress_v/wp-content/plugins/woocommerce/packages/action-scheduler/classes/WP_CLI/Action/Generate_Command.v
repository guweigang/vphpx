...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Generate_Command) execute()  {
	mut var_hook := rt.get_property(rt.new_object('Action_Scheduler_WP_CLI_Action_Generate_Command', ['Action_Scheduler_WP_CLI_Action_ActionScheduler_WPCLI_Command'], &this), 'args').array_get(0)
	mut var_schedule_start := rt.get_property(rt.new_object('Action_Scheduler_WP_CLI_Action_Generate_Command', ['Action_Scheduler_WP_CLI_Action_ActionScheduler_WPCLI_Command'], &this), 'args').array_get(1)
	...
	if !(!rt.is_true(var_callback_args)) {
		var_callback_args = rt.call_function('json_decode', [var_callback_args.dup(), rt.new_bool(true)])
	}
	var_schedule_start = rt.call_function('as_get_datetime_object', [var_schedule_start.dup()])
	...
	mut var_actions_added := this.generate(var_function_args.dup(), rt.new_null(), rt.new_null(), rt.new_null(), rt.new_null(), '')
...
end_label_1:
	mut var_num_actions_added := rt.new_int(rt.new_int(rt.cast_array(var_actions_added).array_count()))
	this.print_success(var_num_actions_added.dup(), rt.new_string('single'))
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Generate_Command) generate(var_schedule_start rt.PhpVal, var_interval rt.PhpVal, var_count rt.PhpVal, var_hook rt.PhpVal, mut var_args Class_Action_Scheduler_WP_CLI_Action_array, group string) rt.PhpVal {
	...
	mut var_progress_bar := rt.call_function('WP_CLI\Utils\make_progress_bar', [rt.call_function('sprintf', [rt.call_function('_n', [rt.new_string('Creating %d action'), rt.new_string('Creating %d actions'), var_count_mutated.dup(), rt.new_string('woocommerce')]), rt.call_function('number_format_i18n', [var_count_mutated.dup()])]), var_count_mutated.dup()])
	{
		mut var_i := rt.new_int(rt.new_int(0))
		for {
			if !(rt.is_true(rt.less(var_i, var_count_mutated))) { break }
			var_actions_added.array_push(rt.call_function('as_schedule_single_action', [rt.add(var_schedule_start_mutated, rt.mul(var_i, var_interval_mutated)), var_hook_mutated.dup(), var_args, rt.new_string(group_mutated).dup()]))
			rt.call_method(var_progress_bar, 'tick', []rt.PhpVal{})
			rt.post_inc(var_i)
		}
	}
	rt.call_method(var_progress_bar, 'finish', []rt.PhpVal{})
	return var_actions_added.dup()
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Generate_Command) print_success(var_actions_added rt.PhpVal, var_action_type rt.PhpVal)  {
	...
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}; return temp.success(arg_0) }(rt.call_function('sprintf', [rt.call_function('_n', [rt.new_string('%1$d %2$s action scheduled.'), rt.new_string('%1$d %2$s actions scheduled.'), var_actions_added_mutated.dup(), rt.new_string('woocommerce')]), rt.call_function('number_format_i18n', [var_actions_added_mutated.dup()]), var_action_type.dup()]))
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Generate_Command) print_error(mut var_e Class_Action_Scheduler_WP_CLI_Action_Exception)  {
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}; return temp.error(arg_0) }(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('There was an error creating the scheduled action: %s'), rt.new_string('woocommerce')]), var_e.getmessage()]))
}
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_wp_cli_action_generate_command_php() {
}
