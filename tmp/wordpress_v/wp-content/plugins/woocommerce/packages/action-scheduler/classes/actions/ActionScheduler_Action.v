...
fn (mut this Class_ActionScheduler_Action) construct(var_hook rt.PhpVal, mut var_args Class_array, mut var_schedule Class_?ActionScheduler_Schedule, group string)  {
	...
	this.set_hook(var_hook_mutated.dup())
...
fn (mut this Class_ActionScheduler_Action) execute()  {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('has_action', [var_hook.dup()]))))) {
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Scheduled action for %1$s will not be executed as no callbacks are registered.'), rt.new_string('woocommerce')]), var_hook.dup()]))))
	}
	rt.call_function('do_action_ref_array', [var_hook.dup(), rt.call_function('array_values', [this.get_args()])])
}

fn (mut this Class_ActionScheduler_Action) set_hook(var_hook rt.PhpVal)  {
	mut var_hook_mutated := var_hook
	this.hook = var_hook_mutated.dup()
}
...
fn (mut this Class_ActionScheduler_Action) set_schedule(mut var_schedule Class_ActionScheduler_Schedule)  {
	mut var_schedule_mutated := var_schedule
	this.schedule = var_schedule_mutated.dup()
}
...
fn (mut this Class_ActionScheduler_Action) set_priority(var_priority rt.PhpVal)  {
	...
	if rt.is_true(rt.less(var_priority_mutated, rt.new_int(0))) {
		...
		var_priority_mutated = rt.new_int(rt.new_int(255))
	}
	this.priority = // unsupported expression: Expr_Cast_Int
}
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_classes_actions_actionscheduler_action_php() {
}
