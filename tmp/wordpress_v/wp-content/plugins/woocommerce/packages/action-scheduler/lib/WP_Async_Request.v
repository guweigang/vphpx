...
fn (mut this Class_WP_Async_Request) construct() {
	...
	rt.call_function('add_action', ['wp_ajax_' + (this.identifier).str(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WP_Async_Request', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'maybe_handle' },
		])])
	rt.call_function('add_action', ['wp_ajax_nopriv_' + (this.identifier).str(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WP_Async_Request', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'maybe_handle' },
		])])
...
fn (mut this Class_WP_Async_Request) dispatch() rt.PhpVal {
	...
	return rt.call_function('wp_remote_post', [
		rt.call_function('esc_url_raw', [var_url.dup()]),
		var_args.dup(),
	])
}
...
fn (mut this Class_WP_Async_Request) get_query_args() rt.PhpVal {
	...
	mut var_args := rt.create_array([rt.ArrayItem{ key: 'action', val: this.identifier },
		rt.ArrayItem{ key: 'nonce', val: rt.call_function('wp_create_nonce', [
			this.identifier,
		]) }])
	return rt.call_function('apply_filters', [(this.identifier).str() + '_query_args',
		var_args.dup()])
}
...
fn (mut this Class_WP_Async_Request) get_query_url() rt.PhpVal {
	...
	mut var_url := rt.call_function('admin_url', [rt.new_string('admin-ajax.php')])
	return rt.call_function('apply_filters',
		[(this.identifier).str() + '_query_url', var_url.dup()])
}
...
fn (mut this Class_WP_Async_Request) get_post_args() rt.PhpVal {
	...
	mut var_args := rt.create_array([rt.ArrayItem{ key: 'timeout', val: 0.01 },
	...
	return rt.call_function('apply_filters',
		[(this.identifier).str() + '_post_args', var_args.dup()])
}
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_lib_wp_async_request_php() {
