...
fn (mut this Class_ActionScheduler_Schedule_Deprecated) next(mut var_after Class_?DateTime) rt.PhpVal {
	if !rt.is_true(var_after) {
		...
		mut var_replacement_method := rt.new_string(rt.new_string('get_date()'))
	} else {
		var_return_value = this.get_next(rt.new_object('?DateTime', []string{}, var_after))
		var_replacement_method = rt.new_string(rt.new_string('get_next( $after )'))
	}
	rt.call_function('_deprecated_function', [rt.new_string(@METHOD), rt.new_string('3.0.0'), @STRUCT + '::' + (var_replacement_method).str()])
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_deprecated_actionscheduler_schedule_deprecated_php() {
}
