...
fn (mut this Class_ActionScheduler_Abstract_QueueRunner_Deprecated) get_maximum_execution_time() rt.PhpVal {
	...
	mut var_maximum_execution_time := rt.new_int(rt.new_int(30))
	...
	]))
	{
		...
		var_maximum_execution_time = rt.call_function('apply_filters', [
			rt.new_string('action_scheduler_maximum_execution_time'),
			var_maximum_execution_time.dup(),
		])
	}
	return rt.call_function('absint', [var_maximum_execution_time.dup()])
}
...
pub fn init_wp_content_plugins_woocommerce_packages_action_scheduler_deprecated_actionscheduler_abstract_queuerunner_deprecated_php() {
}
