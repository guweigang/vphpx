...
fn (mut this Class_Automattic_WooCommerce_Vendor_Symfony_Component_CssSelector_Parser_Shortcut_HashParser) parse(source string) rt.PhpVal {
	...
	]))
	{
		return rt.create_array([
			rt.ArrayItem{ key: none, val: create_automattic_woocommerce_vendor_symfony_component_cssselector_node_selectornode(create_automattic_woocommerce_vendor_symfony_component_cssselector_node_hashnode(create_automattic_woocommerce_vendor_symfony_component_cssselector_node_elementnode(if rt.is_true(var_matches.array_get(1)) {
				var_matches.array_get(1)
			} else {
				rt.new_null()
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_symfony_component_cssselector_parser_shortcut_hashparser_php() {
}
