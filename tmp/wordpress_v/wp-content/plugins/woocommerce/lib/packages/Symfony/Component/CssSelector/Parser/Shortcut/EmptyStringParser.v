...
fn (mut this Class_Automattic_WooCommerce_Vendor_Symfony_Component_CssSelector_Parser_Shortcut_EmptyStringParser) parse(source string) rt.PhpVal {
	if rt.is_true(rt.equal(rt.new_string(''), rt.new_string(source))) {
		return rt.create_array([
			rt.ArrayItem{ key: none, val: create_automattic_woocommerce_vendor_symfony_component_cssselector_node_selectornode(create_automattic_woocommerce_vendor_symfony_component_cssselector_node_elementnode(rt.new_null(),
				rt.new_string('*'))) },
		])
	}
	return rt.new_array()
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_symfony_component_cssselector_parser_shortcut_emptystringparser_php() {
}
