...
fn Class_Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80_Php80.get_debug_type(var_value rt.PhpVal) string {
	...
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_bool(rt.instance_of(var_value, 'Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80___PHP_Incomplete_Class')))) {
...
fn Class_Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80_Php80.get_resource_id(var_res rt.PhpVal) i64 {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_resource', [var_res.dup()]))))) && rt.is_true(rt.identical(rt.new_null(), rt.call_function('get_resource_type', [var_res.dup()]))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80_TypeError', []string{}, create_automattic_woocommerce_vendor_symfony_polyfill_php80_typeerror(rt.call_function('sprintf', [rt.new_string('Argument 1 passed to get_resource_id() must be of the type resource, %s given'), rt.call_function('get_debug_type', [var_res.dup()])]))))
	}
...
fn create_automattic_woocommerce_vendor_symfony_polyfill_php80_php80() &Class_Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80_Php80 {
...
fn create_automattic_woocommerce_vendor_symfony_polyfill_php80_typeerror() &Class_Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80_TypeError {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80_Php80) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'fdiv' {
			...
			return rt.new_float(Class_Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80_Php80.fdiv(dispatch_arg_0, dispatch_arg_1))
		}
		'get_debug_type' {
		...
		'str_contains' {
			...
			return rt.new_bool(Class_Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80_Php80.str_contains(dispatch_arg_0, dispatch_arg_1))
		}
		'str_starts_with' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).str()
			return rt.new_bool(Class_Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80_Php80.str_starts_with(dispatch_arg_0, dispatch_arg_1))
		}
		'str_ends_with' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).str()
			return rt.new_bool(Class_Automattic_WooCommerce_Vendor_Symfony_Polyfill_Php80_Php80.str_ends_with(dispatch_arg_0, dispatch_arg_1))
		}
		else { return none }
	}
}
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_symfony_polyfill_php80_php80_php() {
}
