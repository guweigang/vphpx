...
fn (mut this Class_Automattic_WooCommerce_Vendor_Symfony_Component_CssSelector_Node_AbstractNode) getnodename() string {
	if rt.is_true(rt.identical(rt.new_null(), this.nodeName)) {
		this.nodeName = rt.call_function('preg_replace', [
			rt.new_string('~.*\\\\([^\\\\]+)Node$~'),
			rt.new_string('$1'),
			Class_Automattic_WooCommerce_Vendor_Symfony_Component_CssSelector_Node_static.class(),
		])
	}
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_symfony_component_cssselector_node_abstractnode_php() {
}
