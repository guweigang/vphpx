...
struct Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Introspection {
	rt.PhpObjectBase
pub mut:
	cachedInstances rt.PhpVal = rt.new_null()
}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Introspection.getintrospectionquery(mut var_options Class_Automattic_WooCommerce_Vendor_GraphQL_Type_array) string {
	...
	mut var_descriptions := rt.new_string(if rt.is_true(var_optionsWithDefaults.array_get('descriptions')) {
		rt.new_string('description')
	} else {
		rt.new_string('')
	})
	mut var_directiveIsRepeatable := rt.new_string(if rt.is_true(var_optionsWithDefaults.array_get('directiveIsRepeatable')) {
		rt.new_string('isRepeatable')
	} else {
		rt.new_string('')
	})
	mut var_schemaDescription := if rt.is_true(var_optionsWithDefaults.array_get('schemaDescription')) {
		var_descriptions
	} else {
		rt.new_string('')
	}
	mut var_typeIsOneOf := rt.new_string(if rt.is_true(var_optionsWithDefaults.array_get('typeIsOneOf')) {
		rt.new_string('isOneOf')
	} else {
		rt.new_string('')
	})
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Introspection.fromschema(mut var_schema Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Schema, mut var_options Class_Automattic_WooCommerce_Vendor_GraphQL_Type_array) rt.PhpVal {
	...
	mut var_result := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_GraphQL{}
		return temp.executequery(arg_0, arg_1)
	}(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Schema', []string{}, var_schema),
	...
	if rt.is_true(rt.identical(var_data, rt.new_null())) {
		mut var_noDataResult := fn (arg_0 rt.PhpVal) rt.PhpVal {
			mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils{}
			return temp.printsafejson(arg_0)
		}(var_result.dup())
...
fn (this &Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Introspection) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	match prop_name {
		'cachedInstances' { return this.cachedInstances }
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_type_introspection_php() {
