...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Error) construct(message string, var_nodes rt.PhpVal, mut var_source Class_Automattic_WooCommerce_Vendor_GraphQL_Error_?Source, mut var_positions Class_Automattic_WooCommerce_Vendor_GraphQL_Error_?array, mut var_path Class_Automattic_WooCommerce_Vendor_GraphQL_Error_?array, mut var_previous Class_Automattic_WooCommerce_Vendor_GraphQL_Error_?Throwable, mut var_extensions Class_Automattic_WooCommerce_Vendor_GraphQL_Error_?array, mut var_unaliasedPath Class_Automattic_WooCommerce_Vendor_GraphQL_Error_?array)  {
	...
	if rt.is_true(rt.new_bool(rt.instance_of(var_nodes_mutated, 'Automattic_WooCommerce_Vendor_GraphQL_Error_Traversable'))) {
		this.nodes = rt.call_function('array_filter', [rt.call_function('iterator_to_array', [var_nodes_mutated.dup()])])
	} else if rt.is_true(rt.new_bool(var_nodes_mutated.dup().is_array())) {
		this.nodes = rt.call_function('array_filter', [var_nodes_mutated.dup()])
	...
	this.source = var_source_mutated.dup()
	this.positions = var_positions_mutated.dup()
	this.path = var_path_mutated.dup()
	this.unaliasedPath = var_unaliasedPath.dup()
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_extensions_mutated.dup().is_array())) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		this.extensions = var_extensions_mutated.dup()
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Error.createlocatederror(var_error rt.PhpVal, var_nodes rt.PhpVal, mut var_path Class_Automattic_WooCommerce_Vendor_GraphQL_Error_?array, mut var_unaliasedPath Class_Automattic_WooCommerce_Vendor_GraphQL_Error_?array) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.instance_of(var_error, 'Automattic_WooCommerce_Vendor_GraphQL_Error_self'))) {
		...
		var_message = // unsupported expression: Expr_Cast_String
	}
	...
	return create_automattic_woocommerce_vendor_graphql_error_static(var_nonEmptyMessage.dup(), var_nodes_mutated.dup(), var_source.dup(), var_positions.dup(), var_path_mutated.dup(), var_originalError.dup(), var_extensions.dup(), var_unaliasedPath.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Error) islocated() bool {
	...
	return rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical)
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Error) getlocations() rt.PhpVal {
	if !(!(this.locations).is_null()) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
			{
				mut iter_1 := var_positions.iterator()
				for {
					item_1 := iter_1.next() or { break }
					mut var_position := item_1.val
					this.locations.array_push(rt.call_method(var_source, 'getLocation', [var_position.dup()]))
				}
			}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Error) jsonserialize() rt.PhpVal {
	return fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError{}; return temp.createfromexception(arg_0) }(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_Error', []string{}, this))
}

fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Error) magic_tostring() string {
	return (fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError{}; return temp.printerror(arg_0) }(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_Error', []string{}, this))).str()
}
...
fn create_automattic_woocommerce_vendor_graphql_error_exception() &Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Exception {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_error_error_php() {
