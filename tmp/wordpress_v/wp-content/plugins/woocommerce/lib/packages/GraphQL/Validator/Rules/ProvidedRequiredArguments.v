...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ProvidedRequiredArguments) getvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) rt.PhpVal {
	...
	closure_1_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_fieldNode := if args.len > 0 { args[0].dup() } else { rt.new_null() }
		...
		mut var_argNodeMap := rt.new_array()
		{
			...
			for {
				...
				var_argNodeMap.array_set(rt.get_property(rt.get_property(var_argNode, 'name'),
					'value'), var_argNode.dup())
			}
		}
		{
			mut iter_1 := rt.get_property(var_fieldDef, 'args').iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_argDef := item_1.val
				...
				if rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_argNode, rt.new_null()))
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_providedrequiredarguments() &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ProvidedRequiredArguments {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_validator_rules_providedrequiredarguments_php() {
