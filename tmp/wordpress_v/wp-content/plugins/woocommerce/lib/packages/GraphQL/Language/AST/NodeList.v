...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeList) offsetget(var_offset rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_item.dup().is_array())) {
		return this.nodes.array_set(var_offset, fn (arg_0 rt.PhpVal) rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeList) offsetset(var_offset rt.PhpVal, var_value rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(var_value_mutated.dup().is_array())) {
		var_value_mutated = fn (arg_0 rt.PhpVal) rt.PhpVal {
			mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_AST{}
			return temp.fromarray(arg_0)
		}(var_value_mutated.dup())
	}
	if rt.is_true(rt.identical(var_offset, rt.new_null())) {
		this.nodes.array_push(var_value_mutated.dup())
		return rt.new_null()
	}
	this.nodes.array_set(var_offset, var_value_mutated.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeList) splice(offset i64, length i64, var_replacement rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.call_function('is_iterable', [var_replacement_mutated.dup()]))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_replacement_mutated.dup().is_array())))))))
	{
		var_replacement_mutated = rt.call_function('iterator_to_array', [
			var_replacement_mutated.dup()])
	}
	return create_automattic_woocommerce_vendor_graphql_language_ast_nodelist(rt.call_function('array_splice', [
		this.nodes,
		rt.new_int(offset),
		rt.new_int(length),
		var_replacement_mutated.dup(),
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeList) merge(mut var_list Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_iterable) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_list_mutated.dup().is_array()))))) {
		var_list_mutated = rt.call_function('iterator_to_array', [
			var_list_mutated.dup()])
	}
	return create_automattic_woocommerce_vendor_graphql_language_ast_nodelist(rt.call_function('array_merge', [
		this.nodes,
		var_list_mutated.dup(),
	]))
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeList) clonedeep() rt.PhpVal {
	...
	mut var_cloned :=
		create_automattic_woocommerce_vendor_graphql_language_ast_static(var_empty.dup())
	{
		mut iter_1 := this.getiterator().iterator()
		for {
			item_1 := iter_1.next() or { break }
...
fn create_automattic_woocommerce_vendor_graphql_language_ast_static() &Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_static {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_language_ast_nodelist_php() {
