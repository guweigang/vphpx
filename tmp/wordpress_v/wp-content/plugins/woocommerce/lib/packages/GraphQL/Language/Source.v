...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Language_Source) construct(body string, mut var_name Class_Automattic_WooCommerce_Vendor_GraphQL_Language_?string, mut var_location Class_Automattic_WooCommerce_Vendor_GraphQL_Language_?SourceLocation)  {
	...
	this.name = if rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_name, rt.new_string(''))) || rt.is_true(rt.identical(var_name, rt.new_null())))) { rt.new_string('Automattic\\WooCommerce\\Vendor\\GraphQL request') } else { var_name }
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Language_Source) getlocation(position i64) rt.PhpVal {
	...
	rt.call_function('preg_match_all', [var_lineRegexp.dup(), rt.call_function('mb_substr', [this.body, rt.new_int(0), rt.new_int(position), rt.new_string('UTF-8')]), var_matches.dup(), rt.get_constant('PREG_OFFSET_CAPTURE')])
	{
		...
		for {
			...
			rt.pre_inc(var_line)
			var_column = rt.sub(position + 1, rt.add(var_match.array_get(1), rt.call_function('mb_strlen', [var_match.array_get(0), rt.new_string('UTF-8')])))
		}
	}
	return create_automattic_woocommerce_vendor_graphql_language_sourcelocation(var_line.dup(), var_column.dup())
}
...
fn create_automattic_woocommerce_vendor_graphql_language_sourcelocation() &Class_Automattic_WooCommerce_Vendor_GraphQL_Language_SourceLocation {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_language_source_php() {
