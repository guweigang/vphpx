...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter) convertthenable(var_thenable rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_thenable, 'Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromise')))))) {
		...
		mut var_safeThenable := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils{}; return temp.printsafe(arg_0) }(var_thenable.dup())
	...
	return create_automattic_woocommerce_vendor_graphql_executor_promise_promise(var_thenable.dup(), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter', ['PromiseAdapter'], &this).dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter) then(mut var_promise Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Promise, mut var_onFulfilled Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_?callable, mut var_onRejected Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_?callable) rt.PhpVal {
	...
	return create_automattic_woocommerce_vendor_graphql_executor_promise_promise(rt.call_method(var_syncPromise, 'then', [var_onFulfilled, var_onRejected]), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter', ['PromiseAdapter'], &this).dup())
}
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_Throwable') {
		...
		rt.call_method(var_syncPromise, 'reject', [var_e.dup()])
		unsafe { goto end_label_1 }
	}
	else {
...
end_label_1:
	return create_automattic_woocommerce_vendor_graphql_executor_promise_promise(var_syncPromise.dup(), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter', ['PromiseAdapter'], &this).dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter) createfulfilled(var_value rt.PhpVal) rt.PhpVal {
	mut var_syncPromise := create_automattic_woocommerce_vendor_graphql_executor_promise_adapter_syncpromise()
	return create_automattic_woocommerce_vendor_graphql_executor_promise_promise(rt.call_method(var_syncPromise, 'resolve', [var_value.dup()]), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter', ['PromiseAdapter'], &this).dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter) createrejected(mut var_reason Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_Throwable) rt.PhpVal {
	mut var_syncPromise := create_automattic_woocommerce_vendor_graphql_executor_promise_adapter_syncpromise()
	return create_automattic_woocommerce_vendor_graphql_executor_promise_promise(rt.call_method(var_syncPromise, 'reject', [var_reason]), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter', ['PromiseAdapter'], &this).dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter) all(mut var_promisesOrValues Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_iterable) rt.PhpVal {
	...
	mut var_total := if rt.is_true(rt.new_bool(var_promisesOrValues.is_array())) { rt.new_int(var_promisesOrValues.array_count()) } else { rt.call_function('iterator_count', [var_promisesOrValues]) }
	mut var_count := rt.new_int(rt.new_int(0))
	mut var_result := rt.new_array()
	closure_1_fn := fn [mut var_count, mut var_total, var_all, mut var_result] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.identical(var_count, var_total)) {
		var_all.resolve(var_result.dup())
	}
	return rt.new_null()
	}
	mut var_resolveAllWhenFinished := rt.new_closure(closure_1_fn)
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.instance_of(var_promiseOrValue, 'Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Promise'))) {
				...
				closure_2_fn := fn [mut var_result, var_index, mut var_count, mut var_resolveAllWhenFinished] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_value := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	var_result.array_set(var_index, var_value.dup())
	rt.pre_inc(var_count)
	rt.call_callable(var_resolveAllWhenFinished, []rt.PhpVal{})
	return rt.new_null()
	}
				rt.call_method(var_promiseOrValue, 'then', [rt.new_closure(closure_2_fn), rt.create_array([rt.ArrayItem{ key: none, val: var_all }, rt.ArrayItem{ key: none, val: 'reject' }])])
				continue
			}
			var_result.array_set(var_index, var_promiseOrValue.dup())
			rt.pre_inc(var_count)
		}
	}
	rt.call_callable(var_resolveAllWhenFinished, []rt.PhpVal{})
	return create_automattic_woocommerce_vendor_graphql_executor_promise_promise(var_all.dup(), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_SyncPromiseAdapter', ['PromiseAdapter'], &this).dup())
}
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_executor_promise_adapter_syncpromiseadapter_php() {
