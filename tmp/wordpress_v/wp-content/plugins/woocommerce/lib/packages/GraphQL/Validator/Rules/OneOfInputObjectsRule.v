...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_OneOfInputObjectsRule) getvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) rt.PhpVal {
	...
	mut var_namedType := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type{}; return temp.getnamedtype(arg_0) }(var_type.dup())
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_namedType, 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_InputObjectType')))))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_namedType, 'isOneOf', []rt.PhpVal{}))))))) {
		return rt.new_null()
	}
	...
	mut var_nullFields := rt.new_array()
	{
		...
		for {
			...
			var_providedFields.array_push(var_fieldName.dup())
			if rt.is_true(rt.identical(rt.get_property(rt.get_property(var_fieldNode, 'value'), 'kind'), Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.null())) {
				var_nullFields.array_push(var_fieldName.dup())
			}
		}
	}
	mut var_fieldCount := rt.new_int(rt.new_int(var_providedFields.dup().array_count()))
	...
	if var_nullFields.dup().array_count() > 0 {
		var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_OneOfInputObjectsRule.oneofinputobjectfieldvaluemustnotbenullmessage((rt.get_property(var_namedType, 'name')).str(), (var_nullFields.array_get(0)).str()), rt.create_array([rt.ArrayItem{ key: none, val: var_node }])))
	}
	return rt.new_null()
	}
	return rt.create_array([rt.ArrayItem{ key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.object(), val: rt.new_closure(closure_1_fn) }])
}
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_oneofinputobjectsrule() &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_OneOfInputObjectsRule {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_validator_rules_oneofinputobjectsrule_php() {
