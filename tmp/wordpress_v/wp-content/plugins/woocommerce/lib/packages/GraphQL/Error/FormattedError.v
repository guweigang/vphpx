...
struct Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError {
	rt.PhpObjectBase
pub mut:
		internalErrorMessage rt.PhpVal = rt.new_string('Internal server error')
}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError.printerror(mut var_error Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Error) string {
	...
	if rt.is_true(rt.new_bool(!(var_nodes).is_null() && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		{
	...
	} else if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		mut var_source := var_error.getsource()
		{
			mut iter_1 := var_error.getlocations().iterator()
			for {
				item_1 := iter_1.next() or { break }
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError.createfromexception(mut var_exception Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Throwable, debugFlag i64, mut var_internalErrorMessage Class_Automattic_WooCommerce_Vendor_GraphQL_Error_?string) rt.PhpVal {
	...
	mut var_message := if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_Throwable', []string{}, var_exception), 'Automattic_WooCommerce_Vendor_GraphQL_Error_ClientAware'))) && rt.is_true(var_exception.isclientsafe()))) { var_exception.getmessage() } else { var_internalErrorMessage }
	...
	mut var_loc := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.call_method(var_loc, 'toSerializableArray', []rt.PhpVal{})
	}
	mut var_loc := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.call_method(var_loc, 'toSerializableArray', []rt.PhpVal{})
	}
		mut var_locations := rt.call_function('array_map', [rt.new_closure(closure_1_fn), var_exception.getlocations()])
		if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
			var_formattedError.array_set('locations', var_locations.dup())
		}
	...
	if rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_Throwable', []string{}, var_exception), 'Automattic_WooCommerce_Vendor_GraphQL_Error_ProvidesExtensions'))) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_extensions.dup().is_array())) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
			var_formattedError.array_set('extensions', var_extensions.dup())
		}
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError.adddebugentries(mut var_formattedError Class_Automattic_WooCommerce_Vendor_GraphQL_Error_array, mut var_e Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Throwable, debugFlag i64) rt.PhpVal {
	...
	mut var_isUnsafe := rt.new_bool(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_Throwable', []string{}, var_e), 'Automattic_WooCommerce_Vendor_GraphQL_Error_ClientAware')))))) || rt.is_true(rt.new_bool(!(rt.is_true(var_e.isclientsafe()))))))
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(var_isUnsafe))) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		rt.throw_exception(var_e.getprevious())
	}
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(var_isUnsafe))) {
	...
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		...
		mut var_isTrivial := rt.new_bool(rt.new_bool(rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_Throwable', []string{}, var_e), 'Automattic_WooCommerce_Vendor_GraphQL_Error_Error'))) && rt.is_true(rt.identical(var_e.getprevious(), rt.new_null()))))
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError.prepareformatter(mut var_formatter Class_Automattic_WooCommerce_Vendor_GraphQL_Error_?callable, debug i64) rt.PhpVal {
	...
	return if rt.is_true(rt.identical(var_formatter, rt.new_null())) { rt.new_closure(closure_3_fn) } else { rt.new_closure(closure_4_fn) }
}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError.tosafetrace(mut var_error Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Throwable) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_trace.array_get(0).array_isset(rt.new_string('function')) && var_trace.array_get(0).array_isset(rt.new_string('class')) && rt.is_true(rt.identical((var_trace.array_get(0).array_get('class')).str() + '::' + (var_trace.array_get(0).array_get('function')).str(), rt.new_string('Automattic\\WooCommerce\\Vendor\\GraphQL\\Utils\\Utils::invariant'))))) {
		rt.call_function('array_shift', [var_trace.dup()])
	} else if !(var_trace.array_get(0).array_isset(rt.new_string('file'))) {
		rt.call_function('array_shift', [var_trace.dup()])
	}
	mut var_formatted := rt.new_array()
	{
		...
		for {
			...
			if var_err.array_isset(rt.new_string('file')) {
				var_safeErr.array_set('file', var_err.array_get('file'))
			}
			if var_err.array_isset(rt.new_string('line')) {
				var_safeErr.array_set('line', var_err.array_get('line'))
			}
			mut var_func := var_err.array_get('function')
			mut var_args := rt.call_function('array_map', [rt.create_array([rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError.class() }, rt.ArrayItem{ key: none, val: 'printVar' }]), if !(var_err.array_get('args')).is_null() { var_err.array_get('args') } else { rt.new_array() }])
			mut var_funcStr := rt.new_string((var_func).str() + '(' + (rt.call_function('implode', [rt.new_string(', '), var_args.dup()])).str() + ')')
			if var_err.array_isset(rt.new_string('class')) {
				var_safeErr.array_set('call', (var_err.array_get('class')).str() + '::' + (var_funcStr).str())
			} else {
				var_safeErr.array_set('function', var_funcStr.dup())
			}
			var_formatted.array_push(var_safeErr.dup())
		}
	}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError.printvar(var_var rt.PhpVal) string {
	...
	if rt.is_true(rt.new_bool(var_var.dup().is_object())) {
		mut var_count := rt.new_string(if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_var, 'PHPUnit_Framework_Test')))))) && rt.is_true(rt.new_bool(rt.instance_of(var_var, 'Automattic_WooCommerce_Vendor_GraphQL_Error_Countable'))))) { '(' + var_var.dup().array_count().str() + ')' } else { rt.new_string('') })
		return 'instance of ' + (rt.call_function('get_class', [var_var.dup()])).str() + (var_count).str()
	}
	if rt.is_true(rt.new_bool(var_var.dup().is_array())) {
		return 'array(' + var_var.dup().array_count().str() + ')'
	}
	if rt.is_true(rt.identical(var_var, rt.new_string(''))) {
		return '(empty string)'
	}
	if rt.is_true(rt.new_bool(var_var.dup().is_string())) {
		return '\'' + (rt.call_function('addcslashes', [var_var.dup(), rt.new_string('\'')])).str() + '\''
	}
	if rt.is_true(rt.new_bool(var_var.dup().is_bool())) {
		return if rt.is_true(var_var) { 'true' } else { 'false' }
	}
	if rt.is_true(rt.call_function('is_scalar', [var_var.dup()])) {
		return (// unsupported expression: Expr_Cast_String).str()
	}
	if rt.is_true(rt.identical(var_var, rt.new_null())) {
		return 'null'
	}
	return (rt.call_function('gettype', [var_var.dup()])).str()
}
...
fn create_automattic_woocommerce_vendor_graphql_error_formattederror() &Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError {
	mut obj := &Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError{
		...
		internalErrorMessage: rt.new_string('Internal server error')
	}
	return obj
}
...
fn (this &Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	match prop_name {
		'internalErrorMessage' { return this.internalErrorMessage }
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'internalErrorMessage' { this.internalErrorMessage = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_error_formattederror_php() {
