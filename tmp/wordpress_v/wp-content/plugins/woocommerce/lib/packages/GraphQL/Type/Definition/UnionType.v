...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_UnionType) gettypes() rt.PhpVal {
	if !(!(this.types).is_null()) {
		...
		mut var_types := if !(this.config.array_get('types')).is_null() {
			this.config.array_get('types')
		} else {
			rt.new_null()
		}
		if rt.is_true(rt.call_function('is_callable', [var_types.dup()])) {
		...
		}
		{
			...
			for {
				...
				this.types.array_push(fn (arg_0 rt.PhpVal) rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_UnionType) resolvevalue(var_objectValue rt.PhpVal, var_context rt.PhpVal, mut var_info Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ResolveInfo) rt.PhpVal {
	if this.config.array_isset(rt.new_string('resolveValue')) {
		return rt.call_callable(this.config.array_get('resolveValue'), [
			var_objectValue.dup(), var_context.dup(), var_info])
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_UnionType) resolvetype(var_objectValue rt.PhpVal, var_context rt.PhpVal, mut var_info Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ResolveInfo) rt.PhpVal {
	if this.config.array_isset(rt.new_string('resolveType')) {
		return rt.call_callable(this.config.array_get('resolveType'), [
			var_objectValue.dup(), var_context.dup(), var_info])
	}
	return rt.new_null()
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_UnionType) assertvalid() {
	...
	}(rt.get_property(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_UnionType', [
		'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type',
		'AbstractType',
		'OutputType',
		'CompositeType',
	...
	mut var_resolveType := if !(this.config.array_get('resolveType')).is_null() {
		this.config.array_get('resolveType')
	} else {
		rt.new_null()
	}
	if rt.is_true(rt.new_bool(!var_resolveType.is_null()
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_callable', [var_resolveType.dup()])))))))
	{
		mut var_notCallable := fn (arg_0 rt.PhpVal) rt.PhpVal {
			mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils{}
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_type_definition_uniontype_php() {
