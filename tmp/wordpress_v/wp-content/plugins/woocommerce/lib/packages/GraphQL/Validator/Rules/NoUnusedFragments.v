	...
	closure_3_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
				...
				this.operationDefs.array_push(var_node.dup())
				return fn () rt.PhpVal {
			...
			this.fragmentDefs.array_push(var_def.dup())
			return fn () rt.PhpVal {
				mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Language_Visitor{}
				return temp.skipnode()
			}()
		...
		mut var_fragmentNameUsed := rt.new_array()
		{
			...
			for {
				...
				{
					mut iter_2 :=
						var_context.getrecursivelyreferencedfragments(var_operation.dup()).iterator()
					for {
						item_2 := iter_2.next() or { break }
						mut var_fragment := item_2.val
						var_fragmentNameUsed.array_set(rt.get_property(rt.get_property(var_fragment,
							'name'), 'value'), true)
					}
				}
			}
		...
		{
			mut iter_1 := this.fragmentDefs.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_fragmentDef := item_1.val
				mut var_fragName := rt.get_property(rt.get_property(var_fragmentDef, 'name'),
					'value')
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_nounusedfragments() &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoUnusedFragments {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_validator_rules_nounusedfragments_php() {
