	...
	closure_2_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		closure_1_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_fragType, 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_CompositeType'))))))
		...
		mut var_parentType := var_context.getparenttype()
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_fragType, rt.new_null()))
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_PossibleFragmentSpreads) dotypesoverlap(mut var_schema Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Schema, mut var_fragType Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_CompositeType, mut var_parentType Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_CompositeType) bool {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_CompositeType', []string{}, var_parentType), 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_AbstractType')))
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_CompositeType', []string{}, var_parentType), 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ObjectType')))
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_CompositeType', []string{}, var_parentType), 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_InterfaceType')))
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_CompositeType', []string{}, var_parentType), 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_UnionType')))
		&& rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_CompositeType', []string{}, var_fragType), 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_InterfaceType')))))
	{
		{
	...
	{
		{
			mut iter_1 := var_fragType.gettypes().iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_type := item_1.val
				if rt.is_true(rt.call_method(var_type, 'implementsInterface', [
	...
	{
		{
			...
			for {
				item_1 := iter_1.next() or { break }
				mut var_type := item_1.val
				if rt.is_true(var_parentType.ispossibletype(var_type.dup())) {
					return true
				}
			}
		}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_PossibleFragmentSpreads) getfragmenttype(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext, name string) rt.PhpVal {
	...
	mut var_type := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_AST{}
		return temp.typefromast(arg_0, arg_1)
	}(rt.create_array([rt.ArrayItem{ key: none, val: var_context.getschema() },
		rt.ArrayItem{ key: none, val: 'getType' }]), rt.get_property(var_frag, 'typeCondition'))
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_possiblefragmentspreads() &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_PossibleFragmentSpreads {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_validator_rules_possiblefragmentspreads_php() {
