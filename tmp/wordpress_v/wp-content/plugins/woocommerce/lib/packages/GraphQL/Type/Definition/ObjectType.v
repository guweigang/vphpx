...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ObjectType) construct(mut var_config Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array) {
	...
	this.resolveFieldFn = if !(var_config.array_get('resolveField')).is_null() {
		var_config.array_get('resolveField')
	} else {
		rt.new_null()
	}
	this.argsMapper = if !(var_config.array_get('argsMapper')).is_null() {
		var_config.array_get('argsMapper')
	} else {
		rt.new_null()
	}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ObjectType.assertobjecttype(var_type rt.PhpVal) rt.PhpVal {
	...
	{
		mut var_notObjectType := fn (arg_0 rt.PhpVal) rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ObjectType) istypeof(var_objectValue rt.PhpVal, var_context rt.PhpVal, mut var_info Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ResolveInfo) rt.PhpVal {
	return if this.config.array_isset(rt.new_string('isTypeOf')) { rt.call_callable(this.config.array_get('isTypeOf'), [
			var_objectValue.dup(),
			var_context.dup(),
			var_info,
		]) } else { rt.new_null() }
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ObjectType) assertvalid() {
	...
	}(rt.get_property(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ObjectType', [
	...
	mut var_isTypeOf := if !(this.config.array_get('isTypeOf')).is_null() {
		this.config.array_get('isTypeOf')
	} else {
		rt.new_null()
	}
	if rt.is_true(rt.new_bool(!var_isTypeOf.is_null()
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_callable', [var_isTypeOf.dup()])))))))
	{
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_type_definition_objecttype_php() {
