	...
	closure_2_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		closure_1_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			mut var_type := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
				...
				rt.ArrayItem{ key: none, val: 'getType' }]), rt.get_property(var_node,
				'typeCondition'))
			if rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_type, rt.new_null())) || rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal {
				mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type{}
				return temp.iscompositetype(arg_0)
			}(var_type.dup()))))
			{
				return rt.new_null()
			}
		...
		mut var_type := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
			...
			rt.ArrayItem{ key: none, val: 'getType' }]), rt.get_property(var_node, 'typeCondition'))
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_type, rt.new_null())) || rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal {
			mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type{}
			return temp.iscompositetype(arg_0)
		}(var_type.dup()))))
		...
		var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_FragmentsOnCompositeTypes.fragmentonnoncompositeerrormessage((rt.get_property(rt.get_property(var_node,
			'name'), 'value')).str(), (fn (arg_0 rt.PhpVal) rt.PhpVal {
			mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Language_Printer{}
			return temp.doprint(arg_0)
		}(rt.get_property(var_node, 'typeCondition'))).str()), rt.create_array([
			rt.ArrayItem{ key: none, val: rt.get_property(var_node, 'typeCondition') },
		])))
		return rt.new_null()
	}
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_fragmentsoncompositetypes() &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_FragmentsOnCompositeTypes {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_validator_rules_fragmentsoncompositetypes_php() {
