...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Server_ServerConfig) seterrorshandler(mut var_handler Class_Automattic_WooCommerce_Vendor_GraphQL_Server_callable) rt.PhpVal {
	this.errorsHandler = var_handler.dup()
	return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Server_ServerConfig', []string{}, this)
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Server_ServerConfig) setvalidationrules(var_validationRules rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_validationRules.dup().is_array()))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_callable', [var_validationRules.dup()]))))))) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		mut var_invalidValidationRules := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils{}; return temp.printsafe(arg_0) }(var_validationRules.dup())
	...
	this.validationRules = var_validationRules.dup()
	return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Server_ServerConfig', []string{}, this)
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Server_ServerConfig) setquerybatching(enableBatching bool) rt.PhpVal {
	this.queryBatching = rt.new_bool(enableBatching).dup()
	return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Server_ServerConfig', []string{}, this)
}
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_server_serverconfig_php() {
