...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoFragmentCycles) getvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) rt.PhpVal {
	...
	this.detectcyclerecursive(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_FragmentDefinitionNode](var_node), mut rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext', []string{}, var_context))
	return fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Language_Visitor{}; return temp.skipnode() }()
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoFragmentCycles) detectcyclerecursive(mut var_fragment Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_FragmentDefinitionNode, mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext)  {
	...
	mut var_spreadNodes := var_context.getfragmentspreads(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Language_AST_FragmentDefinitionNode', []string{}, var_fragment))
	...
	this.spreadPathIndexByName.array_set(var_fragmentName, this.spreadPath.array_count())
	{
		...
		for {
			...
			mut var_cycleIndex := if !(this.spreadPathIndexByName.array_get(var_spreadName)).is_null() { this.spreadPathIndexByName.array_get(var_spreadName) } else { rt.new_null() }
			this.spreadPath.array_push(var_spreadNode.dup())
			if rt.is_true(rt.identical(var_cycleIndex, rt.new_null())) {
				mut var_spreadFragment := var_context.getfragment(var_spreadName.dup())
				if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
					this.detectcyclerecursive(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_FragmentDefinitionNode](var_spreadFragment), mut var_context)
				}
			} else {
				mut var_cyclePath := rt.call_function('array_slice', [this.spreadPath, var_cycleIndex.dup()])
				mut var_fragmentNames := rt.new_array()
				{
					mut iter_2 := rt.call_function('array_slice', [var_cyclePath.dup(), rt.new_int(0), // unsupported expression: Expr_UnaryMinus]).iterator()
					for {
						item_2 := iter_2.next() or { break }
						mut var_frag := item_2.val
						var_fragmentNames.array_push(rt.get_property(rt.get_property(var_frag, 'name'), 'value'))
					}
				}
				var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoFragmentCycles.cycleerrormessage((var_spreadName).str(), mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_array](var_fragmentNames)), var_cyclePath.dup()))
			}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoFragmentCycles.cycleerrormessage(fragName string, mut var_spreadNames Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_array) string {
	mut var_via := rt.new_string(if rt.is_true(rt.identical(var_spreadNames, rt.new_array())) { rt.new_string('') } else { ' via ' + (rt.call_function('implode', [rt.new_string(', '), var_spreadNames])).str() })
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_nofragmentcycles() &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoFragmentCycles {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoFragmentCycles) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'detectCycleRecursive' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_FragmentDefinitionNode](if args.len > 0 { args[0] } else { rt.new_null() })
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext](if args.len > 1 { args[1] } else { rt.new_null() })
		...
		'cycleErrorMessage' {
			...
			return rt.new_string(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoFragmentCycles.cycleerrormessage(dispatch_arg_0, mut dispatch_arg_1))
		}
		else { return none }
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoFragmentCycles) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'visitedFrags' { this.visitedFrags = val; return true }
		'spreadPath' { this.spreadPath = val; return true }
		'spreadPathIndexByName' { this.spreadPathIndexByName = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_validator_rules_nofragmentcycles_php() {
