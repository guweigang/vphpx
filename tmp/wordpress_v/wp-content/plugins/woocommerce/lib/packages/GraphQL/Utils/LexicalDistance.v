...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_LexicalDistance) construct(input string) {
	...
	this.inputArray =
		Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_LexicalDistance.stringtoarray(this.inputLowerCase)
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_LexicalDistance) measure(option string, threshold f64) i64 {
	...
	mut var_optionLowerCase := rt.new_string(rt.new_string(option.to_lower()))
	if rt.is_true(rt.identical(this.inputLowerCase, var_optionLowerCase)) {
		return 1
	}
	mut var_a :=
		Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_LexicalDistance.stringtoarray(var_optionLowerCase.str())
	mut var_b := this.inputArray
	if var_a.dup().array_count() < var_b.dup().array_count() {
		mut var_tmp := var_a.dup()
		var_a = var_b.dup()
		var_b = var_tmp.dup()
	}
	mut var_aLength := rt.new_int(rt.new_int(var_a.dup().array_count()))
	mut var_bLength := rt.new_int(rt.new_int(var_b.dup().array_count()))
	...
	mut var_rows := rt.new_array()
	{
		...
		for {
			...
			var_rows.array_get_mut(0).array_set(var_i, var_i.dup())
			rt.pre_inc(var_i)
		}
	}
	{
		...
		for {
			...
			// unsupported expression: Expr_AssignRef
			mut var_smallestCell := var_currentRow.array_set(0, var_i.dup())
			{
				mut var_j := rt.new_int(rt.new_int(1))
				for {
					if !(rt.is_true(rt.less_equal(var_j, var_bLength))) { break
					 }
					mut var_cost := rt.new_int(if rt.is_true(rt.identical(var_a.array_get(rt.sub(var_i,
						rt.new_int(1))), var_b.array_get(rt.sub(var_j, rt.new_int(1)))))
					{
						rt.new_int(0)
					} else {
						rt.new_int(1)
					})
					mut var_currentCell := rt.call_function('min', [
						rt.add(var_upRow.array_get(var_j), rt.new_int(1)),
						rt.add(var_currentRow.array_get(rt.sub(var_j, rt.new_int(1))),
							rt.new_int(1)),
						rt.add(var_upRow.array_get(rt.sub(var_j, rt.new_int(1))), var_cost),
					])
					if rt.is_true(rt.new_bool(
						rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.greater(var_i, rt.new_int(1)))
						&& rt.is_true(rt.greater(var_j, rt.new_int(1)))))
						&& rt.is_true(rt.identical(var_a.array_get(rt.sub(var_i, rt.new_int(1))), var_b.array_get(rt.sub(var_j, rt.new_int(2)))))))
						&& rt.is_true(rt.identical(var_a.array_get(rt.sub(var_i, rt.new_int(2))), var_b.array_get(rt.sub(var_j, rt.new_int(1)))))))
					{
						mut var_doubleDiagonalCell := var_rows.array_get(rt.mod_(rt.sub(var_i,
							rt.new_int(2)), rt.new_int(3))).array_get(rt.sub(var_j, rt.new_int(2)))
						var_currentCell = rt.call_function('min', [
							var_currentCell.dup(), rt.add(var_doubleDiagonalCell, rt.new_int(1))])
					}
					if rt.is_true(rt.less(var_currentCell, var_smallestCell)) {
						var_smallestCell = var_currentCell.dup()
					}
					var_currentRow.array_set(var_j, var_currentCell.dup())
					rt.pre_inc(var_j)
				}
			}
	...
	mut var_distance :=
		var_rows.array_get(rt.mod_(var_aLength, rt.new_int(3))).array_get(var_bLength)
	return (if rt.is_true(rt.less_equal(var_distance, rt.new_float(threshold))) {
		var_distance
	} else {
		rt.new_null()
	}).to_i64()
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_LexicalDistance.stringtoarray(str string) rt.PhpVal {
	mut var_array := rt.new_array()
	{
		...
		for {
			...
			var_array.array_push(rt.call_function('mb_ord', [
				var_char.dup()]))
		}
	}
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_utils_lexicaldistance_php() {
