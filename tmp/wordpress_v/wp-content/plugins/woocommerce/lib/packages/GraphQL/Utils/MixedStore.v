...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_MixedStore) offsetexists(var_offset rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_offset.dup().is_long())) || rt.is_true(rt.new_bool(var_offset.dup().is_string())))) {
		return this.standardStore.array_isset(var_offset.dup())
	}
	if rt.is_true(rt.new_bool(var_offset.dup().is_double())) {
		return this.floatStore.array_isset(// unsupported expression: Expr_Cast_String)
	}
	if rt.is_true(rt.new_bool(var_offset.dup().is_object())) {
		return (rt.call_method(this.objectStore, 'offsetExists', [var_offset.dup()])).to_bool()
	}
	if rt.is_true(rt.new_bool(var_offset.dup().is_array())) {
		{
			...
			for {
				...
				if rt.is_true(rt.identical(var_entry, var_offset)) {
					this.lastArrayKey = var_offset.dup()
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_MixedStore) offsetget(var_offset rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_offset.dup().is_double())) {
		return this.floatStore.array_get(// unsupported expression: Expr_Cast_String)
	}
	if rt.is_true(rt.new_bool(var_offset.dup().is_object())) {
		return rt.call_method(this.objectStore, 'offsetGet', [var_offset.dup()])
	}
	if rt.is_true(rt.new_bool(var_offset.dup().is_array())) {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_MixedStore) offsetset(var_offset rt.PhpVal, var_value rt.PhpVal)  {
	if rt.is_true(rt.identical(var_offset, rt.new_bool(false))) {
		this.falseValue = var_value.dup()
		this.falseValueIsSet = true
	} else if rt.is_true(rt.identical(var_offset, rt.new_bool(true))) {
		this.trueValue = var_value.dup()
		this.trueValueIsSet = true
	} else if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_offset.dup().is_long())) || rt.is_true(rt.new_bool(var_offset.dup().is_string())))) {
		this.standardStore.array_set(var_offset, var_value.dup())
	} else if rt.is_true(rt.new_bool(var_offset.dup().is_double())) {
		this.floatStore.array_set(// unsupported expression: Expr_Cast_String, var_value.dup())
	} else if rt.is_true(rt.new_bool(var_offset.dup().is_object())) {
		this.objectStore.array_set(var_offset, var_value.dup())
	} else if rt.is_true(rt.new_bool(var_offset.dup().is_array())) {
		this.arrayKeys.array_push(var_offset.dup())
		this.arrayValues.array_push(var_value.dup())
	} else if rt.is_true(rt.identical(var_offset, rt.new_null())) {
		this.nullValue = var_value.dup()
		this.nullValueIsSet = true
	} else {
		mut var_unexpectedOffset := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils{}; return temp.printsafe(arg_0) }(var_offset.dup())
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Utils_InvalidArgumentException', []string{}, create_automattic_woocommerce_vendor_graphql_utils_invalidargumentexception(rt.new_string("Unexpected offset type: ${var_unexpectedOffset.to_string()}"))))
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_MixedStore) offsetunset(var_offset rt.PhpVal)  {
	if rt.is_true(rt.identical(var_offset, rt.new_bool(true))) {
		...
		this.floatStore.array_unset(// unsupported expression: Expr_Cast_String)
	} else if rt.is_true(rt.new_bool(var_offset.dup().is_object())) {
		rt.call_method(this.objectStore, 'offsetUnset', [var_offset.dup()])
	} else if rt.is_true(rt.new_bool(var_offset.dup().is_array())) {
		mut var_index := rt.call_function('array_search', [var_offset.dup(), this.arrayKeys, rt.new_bool(true)])
		if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
			rt.call_function('array_splice', [this.arrayKeys, var_index.dup(), rt.new_int(1)])
			rt.call_function('array_splice', [this.arrayValues, var_index.dup(), rt.new_int(1)])
		}
	} else if rt.is_true(rt.identical(var_offset, rt.new_null())) {
...
fn create_automattic_woocommerce_vendor_graphql_utils_splobjectstorage() &Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_SplObjectStorage {
...
fn create_automattic_woocommerce_vendor_graphql_utils_invalidargumentexception() &Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_InvalidArgumentException {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_MixedStore) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'standardStore' { this.standardStore = val; return true }
		'floatStore' { this.floatStore = val; return true }
		'objectStore' { this.objectStore = val; return true }
		'arrayKeys' { this.arrayKeys = val; return true }
		'arrayValues' { this.arrayValues = val; return true }
		'lastArrayKey' { this.lastArrayKey = val; return true }
		'lastArrayValue' { this.lastArrayValue = val; return true }
		'nullValue' { this.nullValue = val; return true }
		'nullValueIsSet' { this.nullValueIsSet = (val).to_bool(); return true }
		'trueValue' { this.trueValue = val; return true }
		'trueValueIsSet' { this.trueValueIsSet = (val).to_bool(); return true }
		'falseValue' { this.falseValue = val; return true }
		'falseValueIsSet' { this.falseValueIsSet = (val).to_bool(); return true }
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_utils_mixedstore_php() {
