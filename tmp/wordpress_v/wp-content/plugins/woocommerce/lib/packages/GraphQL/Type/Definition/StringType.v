...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_StringType) serialize(var_value rt.PhpVal) string {
	mut var_canCast := rt.new_bool(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_scalar', [var_value.dup()])) || rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_value.dup().is_object())) && rt.is_true(rt.call_function('method_exists', [var_value.dup(), rt.new_string('__toString')])))))) || rt.is_true(rt.identical(var_value, rt.new_null()))))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_canCast)))) {
		mut var_notStringable := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils{}; return temp.printsafe(arg_0) }(var_value.dup())
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_StringType) parsevalue(var_value rt.PhpVal) string {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_value.dup().is_string()))))) {
		mut var_notString := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils{}; return temp.printsafejson(arg_0) }(var_value.dup())
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_StringType) parseliteral(mut var_valueNode Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_Node, mut var_variables Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_?array) string {
	...
	mut var_notString := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Language_Printer{}; return temp.doprint(arg_0) }(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Language_AST_Node', []string{}, var_valueNode))
	rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_Error', []string{}, create_automattic_woocommerce_vendor_graphql_error_error(rt.new_string("String cannot represent a non string value: ${var_notString.to_string()}"), var_valueNode.dup())))
	return ''
}
...
fn create_automattic_woocommerce_vendor_graphql_type_definition_stringtype() &Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_StringType {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_type_definition_stringtype_php() {
