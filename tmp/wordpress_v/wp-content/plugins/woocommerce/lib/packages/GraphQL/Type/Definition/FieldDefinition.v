...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_FieldDefinition) construct(mut var_config Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array)  {
	...
	this.resolveFn = if !(var_config.array_get('resolve')).is_null() { var_config.array_get('resolve') } else { rt.new_null() }
	this.args = if var_config.array_isset(rt.new_string('args')) { fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Argument{}; return temp.listfromconfig(arg_0) }(var_config.array_get('args')) } else { rt.new_array() }
	this.argsMapper = if !(var_config.array_get('argsMapper')).is_null() { var_config.array_get('argsMapper') } else { rt.new_null() }
	this.description = if !(var_config.array_get('description')).is_null() { var_config.array_get('description') } else { rt.new_null() }
	this.visible = if !(var_config.array_get('visible')).is_null() { var_config.array_get('visible') } else { rt.new_bool(true) }
	...
	this.complexityFn = if !(var_config.array_get('complexity')).is_null() { var_config.array_get('complexity') } else { rt.new_null() }
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_FieldDefinition.definefieldmap(mut var_parentType Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type, var_fields rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_callable', [var_fields_mutated.dup()])) {
		var_fields_mutated = rt.call_callable(var_fields_mutated, []rt.PhpVal{})
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_iterable', [var_fields_mutated.dup()]))))) {
	...
	mut var_map := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(var_field.dup().is_array())) {
				if !(var_field.array_isset(rt.new_string('name'))) {
					if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_maybeName.dup().is_string()))))) {
					...
					var_field.array_set('name', var_maybeName.dup())
				}
				mut var_fieldDef := create_automattic_woocommerce_vendor_graphql_type_definition_self(var_field.dup())
			} else if rt.is_true(rt.new_bool(rt.instance_of(var_field, 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_self'))) {
				var_fieldDef = var_field.dup()
			} else if rt.is_true(rt.call_function('is_callable', [var_field.dup()])) {
				if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_maybeName.dup().is_string()))))) {
				...
				var_fieldDef = create_automattic_woocommerce_vendor_graphql_type_definition_unresolvedfielddefinition(var_maybeName.dup(), var_field.dup())
				...
				mut var_invalidFieldConfig := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils{}; return temp.printsafe(arg_0) }(var_field.dup())
			...
			var_map.array_set(rt.call_method(var_fieldDef, 'getName', []rt.PhpVal{}), var_fieldDef.dup())
		}
	}
	return var_map.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_FieldDefinition) assertvalid(mut var_parentType Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type)  {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_type, 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_OutputType')))))) {
		mut var_safeType := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils{}; return temp.printsafe(arg_0) }(this.prop_type)
	...
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_callable', [this.resolveFn]))))))) {
		mut var_safeResolveFn := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils{}; return temp.printsafe(arg_0) }(this.resolveFn)
	...
	}
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_fieldArgument := item_1.val
			rt.call_method(var_fieldArgument, 'assertValid', [rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_FieldDefinition', []string{}, &this), var_type.dup()])
		}
	}
}
...
fn create_automattic_woocommerce_vendor_graphql_type_definition_unresolvedfielddefinition() &Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_UnresolvedFieldDefinition {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_type_definition_fielddefinition_php() {
