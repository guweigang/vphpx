...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_UniqueDirectiveNames) getsdlvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_SDLValidationContext) rt.PhpVal {
	...
	closure_1_fn := fn [var_context, var_schema, mut var_knownDirectiveNames] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error('Directive "@' + (var_directiveName).str() + '" already exists in the schema. It cannot be redefined.', rt.get_property(var_node, 'name')))
		return rt.new_null()
	}
	if var_knownDirectiveNames.array_isset(var_directiveName) {
		var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error('There can be only one directive named "@' + (var_directiveName).str() + '".', rt.create_array([rt.ArrayItem{ key: none, val: var_knownDirectiveNames.array_get(var_directiveName) }, rt.ArrayItem{ key: none, val: rt.get_property(var_node, 'name') }])))
	} else {
	...
	return rt.create_array([rt.ArrayItem{ key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.directive_definition(), val: rt.new_closure(closure_1_fn) }])
}
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_uniquedirectivenames() &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_UniqueDirectiveNames {
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_validator_rules_uniquedirectivenames_php() {
