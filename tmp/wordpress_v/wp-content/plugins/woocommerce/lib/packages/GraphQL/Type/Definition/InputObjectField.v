...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_InputObjectField) assertvalid(mut var_parentType Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type)  {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_type, 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_InputType')))))) {
		...
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_InvariantViolation', []string{}, create_automattic_woocommerce_vendor_graphql_error_invariantviolation(rt.concat(rt.concat(rt.concat(rt.concat(rt.get_property(var_parentType, 'name'), rt.new_string('.')), this.name), rt.new_string(' field type must be Input Type but got: ')), var_notInputType))))
	}
	if rt.is_true(rt.new_bool(this.config.array_isset(rt.new_string('resolve')))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_InvariantViolation', []string{}, create_automattic_woocommerce_vendor_graphql_error_invariantviolation(rt.concat(rt.concat(rt.concat(rt.get_property(var_parentType, 'name'), rt.new_string('.')), this.name), rt.new_string(' field has a resolve property, but Input Types cannot define resolvers.')))))
	}
	if this.isrequired() && this.isdeprecated() {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_InvariantViolation', []string{}, create_automattic_woocommerce_vendor_graphql_error_invariantviolation(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('Required input field '), rt.get_property(var_parentType, 'name')), rt.new_string('.')), this.name), rt.new_string(' cannot be deprecated.')))))
	}
}
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_graphql_type_definition_inputobjectfield_php() {
