	...
	if rt.is_true(rt.identical(var_sQuote, rt.new_null())) {
		...
		for rt.is_true(rt.new_bool(!(rt.is_true(var_oParserState.comes(var_sQuote.dup()))))) {
			...
			if rt.is_true(rt.identical(var_sContent, rt.new_null())) {
				rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_SourceException', []string{}, create_automattic_woocommerce_vendor_sabberworm_css_parsing_sourceexception(rt.concat(rt.new_string('Non-well-formed quoted string '), var_oParserState.peek(rt.new_int(3))), var_oParserState.currentline())))
			}
			// unsupported expression: Expr_AssignOp_Concat
		}
		var_oParserState.consume(var_sQuote.dup())
	}
	return create_automattic_woocommerce_vendor_sabberworm_css_value_cssstring(var_sResult, var_oParserState.currentline())
}
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_sabberworm_css_value_cssstring_php() {
}
