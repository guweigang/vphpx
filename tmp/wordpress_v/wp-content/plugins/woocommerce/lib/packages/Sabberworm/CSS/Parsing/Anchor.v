...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_Anchor) construct(var_iPosition rt.PhpVal, mut var_oParserState Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_ParserState) {
	this.iPosition = var_iPosition.dup()
	this.oParserState = var_oParserState.dup()
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_Anchor) backtrack() {
	rt.call_method(this.oParserState, 'setPosition', [this.iPosition])
}

fn create_automattic_woocommerce_vendor_sabberworm_css_parsing_anchor(arg_0 rt.PhpVal, arg_1 rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_Anchor {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_Anchor) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_ParserState](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_sabberworm_css_parsing_anchor_php() {
}
