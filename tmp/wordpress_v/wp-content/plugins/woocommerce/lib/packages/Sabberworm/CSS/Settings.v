...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings) construct() {
	this.bMultibyteSupport = rt.call_function('extension_loaded', [
		rt.new_string('mbstring'),
	])
}

fn Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings.create() rt.PhpVal {
	return create_automattic_woocommerce_vendor_sabberworm_css_settings()
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings) withmultibytesupport(bMultibyteSupport bool) rt.PhpVal {
	this.bMultibyteSupport = rt.new_bool(bMultibyteSupport).dup()
	return rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings', []string{}, this)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings) withdefaultcharset(var_sDefaultCharset rt.PhpVal) rt.PhpVal {
	this.sDefaultCharset = var_sDefaultCharset.dup()
	return rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings', []string{}, this)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings) withlenientparsing(bLenientParsing bool) rt.PhpVal {
	this.bLenientParsing = rt.new_bool(bLenientParsing).dup()
	return rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings', []string{}, this)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings) bestrict() rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'create' {
			return Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings.create()
		}
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_sabberworm_css_settings_php() {
}
