	...
	if rt.is_true(var_oParserState.comes(rt.new_string('#'))) {
		...
		if rt.is_true(rt.identical(var_oParserState.strlen(var_sValue.dup()), rt.new_int(8))) {
			var_aColor = rt.create_array([
				rt.ArrayItem{ key: 'r', val: create_automattic_woocommerce_vendor_sabberworm_css_value_size(rt.new_int(rt.concat(var_sValue.array_get(0),
					var_sValue.array_get(1)).to_i64()), rt.new_null(), rt.new_bool(true),
					var_oParserState.currentline()) },
				rt.ArrayItem{ key: 'g', val: create_automattic_woocommerce_vendor_sabberworm_css_value_size(rt.new_int(rt.concat(var_sValue.array_get(2),
					var_sValue.array_get(3)).to_i64()), rt.new_null(), rt.new_bool(true),
					var_oParserState.currentline()) },
				rt.ArrayItem{ key: 'b', val: create_automattic_woocommerce_vendor_sabberworm_css_value_size(rt.new_int(rt.concat(var_sValue.array_get(4),
				...
				rt.ArrayItem{ key: 'a', val: create_automattic_woocommerce_vendor_sabberworm_css_value_size(rt.call_function('round', [
					Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_Color.maprange(rt.new_int(rt.concat(var_sValue.array_get(6),
		...
		mut var_iLength := var_oParserState.strlen(var_sColorMode.dup())
		{
			...
			for {
				...
				if rt.is_true(var_oParserState.comes(rt.new_string('var'))) {
					...
					}(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_ParserState',
					...
					var_aColor.array_set(var_sColorMode.array_get(var_i), fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
						mut temp := Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_Size{}
		...
		if rt.is_true(var_bContainsVar) {
			return create_automattic_woocommerce_vendor_sabberworm_css_value_cssfunction(var_sColorMode.dup(), rt.call_function('array_values', [
				var_aColor.dup(),
			]), rt.new_string(','), var_oParserState.currentline())
		}
	}
	return create_automattic_woocommerce_vendor_sabberworm_css_value_color(var_aColor.dup(),
		var_oParserState.currentline())
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_Color) render(var_oOutputFormat rt.PhpVal) string {
	...
	return (this.Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_CSSFunction.render(var_oOutputFormat.dup())).str()
}
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_sabberworm_css_value_color_php() {
}
