...
fn Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule.parse(mut var_oParserState Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_ParserState, var_commentsBeforeRule rt.PhpVal) rt.PhpVal {
	...
	mut var_oRule := create_automattic_woocommerce_vendor_sabberworm_css_rule_rule(var_oParserState.parseidentifier(rt.new_bool(!(rt.is_true(var_oParserState.comes(rt.new_string('--')))))), var_oParserState.currentline(), var_oParserState.currentcolumn())
	...
	mut var_oValue := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_Value{}; return temp.parsevalue(arg_0, arg_1) }(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_ParserState', []string{}, var_oParserState), Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule.listdelimiterforrule(var_oRule.getrule()))
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule) setvalues(mut var_aSpaceSeparatedValues Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_array) rt.PhpVal {
	...
	if var_aSpaceSeparatedValues.array_count() > 1 {
		var_oSpaceSeparatedList = create_automattic_woocommerce_vendor_sabberworm_css_value_rulevaluelist(rt.new_string(' '), rt.get_property(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule', ['Commentable', 'CSSElement', 'Positionable'], &this), 'iLineNo'))
	}
	{
		...
		for {
			...
			if var_aCommaSeparatedValues.dup().array_count() > 1 {
				var_oCommaSeparatedList = create_automattic_woocommerce_vendor_sabberworm_css_value_rulevaluelist(rt.new_string(','), rt.get_property(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule', ['Commentable', 'CSSElement', 'Positionable'], &this), 'iLineNo'))
			}
			{
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule) getvalues() rt.PhpVal {
	...
	mut var_aResult := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_mValue, 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_RuleValueList')))))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule) addvalue(var_mValue rt.PhpVal, sType string)  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(this.mValue, 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_RuleValueList')))))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		mut var_mCurrentValue := this.mValue
		this.mValue = create_automattic_woocommerce_vendor_sabberworm_css_value_rulevaluelist(rt.new_string(sType).dup(), this.getlinenumber())
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule) render(var_oOutputFormat rt.PhpVal) rt.PhpVal {
	mut var_sResult := rt.new_string(rt.concat(rt.concat(rt.concat(rt.call_method(var_oOutputFormat, 'comments', [rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule', ['Commentable', 'CSSElement', 'Positionable'], &this)]), this.sRule), rt.new_string(':')), rt.call_method(var_oOutputFormat, 'spaceAfterRuleName', []rt.PhpVal{})))
	if rt.is_true(rt.new_bool(rt.instance_of(this.mValue, 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_Value'))) {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'parse' {
			...
			return Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_Rule.parse(mut dispatch_arg_0, dispatch_arg_1)
		}
		...
		'setValues' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Rule_array](if args.len > 0 { args[0] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_sabberworm_css_rule_rule_php() {
}
