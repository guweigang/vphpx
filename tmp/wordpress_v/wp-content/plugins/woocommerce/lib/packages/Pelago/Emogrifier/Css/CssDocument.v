...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) construct(css string, debug bool)  {
	mut var_parserSettings := rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Settings{}; return temp.create() }(), 'withLenientParsing', [rt.new_bool(!(var_debug) || this.hasnestedatrule(css))])
	this.sabberwormCssDocument = rt.call_method(create_automattic_woocommerce_vendor_sabberworm_css_parser(rt.new_string(css.trim_space()), var_parserSettings.dup()), 'parse', []rt.PhpVal{})
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) hasnestedatrule(css string) bool {
	return (// unsupported expression: Expr_BinaryOp_NotIdentical).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) getstylerulesdata(mut var_allowedMediaTypes Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_array) rt.PhpVal {
	mut var_ruleMatches := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.instance_of(var_rule, 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_CSSList_AtRuleBlockList'))) {
				mut var_containingAtRule := rt.new_string(this.getfilteredatidentifierandrule(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_CSSList_AtRuleBlockList](var_rule), mut var_allowedMediaTypes))
				if rt.is_true(rt.new_bool(var_containingAtRule.dup().is_string())) {
					{
						...
						for {
							...
							if rt.is_true(rt.new_bool(rt.instance_of(var_nestedRule, 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_RuleSet_DeclarationBlock'))) {
								var_ruleMatches.array_push(create_automattic_woocommerce_vendor_pelago_emogrifier_css_stylerule(var_nestedRule.dup(), var_containingAtRule.dup()))
							}
						}
					}
				}
			} else if rt.is_true(rt.new_bool(rt.instance_of(var_rule, 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_RuleSet_DeclarationBlock'))) {
				var_ruleMatches.array_push(create_automattic_woocommerce_vendor_pelago_emogrifier_css_stylerule(var_rule.dup()))
			}
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) rendernonconditionalatrules() string {
	this.isImportRuleAllowed = true
	mut var_cssContents := rt.call_method(this.sabberwormCssDocument, 'getContents', []rt.PhpVal{})
	mut var_atRules := rt.call_function('array_filter', [var_cssContents.dup(), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'isValidAtRuleToRender' }])])
	if rt.is_true(rt.identical(var_atRules, rt.new_array())) {
		return ''
	}
	mut var_atRulesDocument := create_automattic_woocommerce_vendor_sabberworm_css_csslist_document()
	var_atRulesDocument.setcontents(var_atRules.dup())
	return (var_atRulesDocument.render()).str()
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) getfilteredatidentifierandrule(mut var_rule Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_CSSList_AtRuleBlockList, mut var_allowedMediaTypes Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_array) string {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) isvalidatruletorender(mut var_rule Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable) bool {
	if rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable', []string{}, var_rule), 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Property_Charset'))) {
		return false
	}
	if rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable', []string{}, var_rule), 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Property_Import'))) {
		return this.isImportRuleAllowed
	}
	this.isImportRuleAllowed = false
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable', []string{}, var_rule), 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Property_AtRule')))))) {
		return false
	}
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('media'))) {
		...
		var_result = rt.new_bool(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable', []string{}, var_rule), 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_RuleSet_RuleSet'))) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical)))
	} else {
...
fn create_automattic_woocommerce_vendor_pelago_emogrifier_css_cssdocument(css string, debug bool) &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument {
...
fn create_automattic_woocommerce_vendor_sabberworm_css_parser() &Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parser {
...
fn create_automattic_woocommerce_vendor_pelago_emogrifier_css_stylerule() &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_StyleRule {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'getStyleRulesData' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_array](if args.len > 0 { args[0] } else { rt.new_null() })
		...
		'getFilteredAtIdentifierAndRule' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_CSSList_AtRuleBlockList](if args.len > 0 { args[0] } else { rt.new_null() })
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_array](if args.len > 1 { args[1] } else { rt.new_null() })
			return rt.new_string(this.getfilteredatidentifierandrule(mut dispatch_arg_0, mut dispatch_arg_1))
		}
		'isValidAtRuleToRender' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable](if args.len > 0 { args[0] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_lib_packages_pelago_emogrifier_css_cssdocument_php() {
