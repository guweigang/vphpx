...
pub fn Class_Automattic_WooCommerce_Gateways_PayPal_Notices.paypal_account_restriction_issues() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Gateways_PayPal_Constants.paypal_issue_payee_account_locked_or_closed() }, rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Gateways_PayPal_Constants.paypal_issue_payee_account_restricted() }])
}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Notices) construct()  {
	this.gateway = fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Gateways_PayPal_WC_Gateway_Paypal{}; return temp.get_instance() }()
	if rt.is_true(rt.new_bool(!(rt.is_true(this.gateway)))) {
		return
	}
	if rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) {
		rt.call_function('add_action', [rt.new_string('admin_notices'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Gateways_PayPal_Notices', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_paypal_notices' }])])
		rt.call_function('add_action', [rt.new_string('admin_head'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Gateways_PayPal_Notices', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_paypal_notices_on_payments_settings_page' }])])
	}
}

fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Notices) add_paypal_notices()  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('manage_woocommerce')]))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('manage_options')]))))))) {
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Gateways_PayPal_Helper{}; return temp.is_paypal_gateway_available() }())))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.gateway, 'should_use_orders_v2', []rt.PhpVal{}))))))) {
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Notices) add_paypal_notices_on_payments_settings_page()  {
	...
	mut var_is_payments_settings_page := rt.new_bool(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('woocommerce_page_wc-settings'), var_screen_id)) && rt.is_true(rt.identical(rt.new_string('checkout'), var_current_tab)))) && !rt.is_true(var_current_section)))
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Notices) add_paypal_migration_notice()  {
	...
	mut var_doc_url := rt.new_string(rt.new_string('https://woocommerce.com/document/woocommerce-paypal-payments/paypal-payments-upgrade-guide/'))
	mut var_dismiss_url := rt.new_string(this.get_dismiss_url((Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_Notices.paypal_migration_notice()).str()))
	mut var_message := rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('WooCommerce has upgraded your PayPal integration from PayPal Standard to PayPal Payments (PPCP), for a more reliable and modern checkout experience. If you do not prefer the upgraded integration in WooCommerce, we recommend switching to %1$sPayPal Payments%2$s extension.'), rt.new_string('woocommerce')]), '<a href="' + (rt.call_function('esc_url', [var_doc_url.dup()])).str() + '" target="_blank" rel="noopener noreferrer">', rt.new_string('</a>')])
	mut var_notice_html := rt.new_string('<div class="notice notice-warning is-dismissible">' + '<a class="woocommerce-message-close notice-dismiss" style="text-decoration: none;" href="' + (rt.call_function('esc_url', [var_dismiss_url.dup()])).str() + '" aria-label="' + (rt.call_function('esc_attr__', [rt.new_string('Dismiss this notice'), rt.new_string('woocommerce')])).str() + '"></a>' + '<p>' + (var_message).str() + '</p>' + '</div>')
	rt.echo_val(rt.call_function('wp_kses_post', [var_notice_html.dup()]))
}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Notices) add_paypal_account_restricted_notice()  {
	...
	mut var_support_url := rt.new_string(rt.new_string('https://www.paypal.com/smarthelp/contact-us'))
	mut var_dismiss_url := rt.new_string(this.get_dismiss_url((Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_Notices.paypal_account_restricted_notice()).str()))
	mut var_message := rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Your PayPal account has been restricted by PayPal. This may prevent customers from completing payments. Please %1$scontact PayPal support%2$s to resolve this issue and restore full functionality to your account.'), rt.new_string('woocommerce')]), '<a href="' + (rt.call_function('esc_url', [var_support_url.dup()])).str() + '" target="_blank" rel="noopener noreferrer">', rt.new_string('</a>')])
	mut var_notice_html := rt.new_string('<div class="notice notice-error is-dismissible">' + '<a class="woocommerce-message-close notice-dismiss" style="text-decoration: none;" href="' + (rt.call_function('esc_url', [var_dismiss_url.dup()])).str() + '" aria-label="' + (rt.call_function('esc_attr__', [rt.new_string('Dismiss this notice'), rt.new_string('woocommerce')])).str() + '"></a>' + '<p><strong>' + (rt.call_function('esc_html__', [rt.new_string('PayPal Account Restricted'), rt.new_string('woocommerce')])).str() + '</strong></p>' + '<p>' + (var_message).str() + '</p>' + '</div>')
	rt.echo_val(rt.call_function('wp_kses_post', [var_notice_html.dup()]))
}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Notices) add_paypal_unsupported_currency_notice()  {
	...
	mut var_dismiss_url := rt.new_string(this.get_dismiss_url((Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_Notices.paypal_unsupported_currency_notice()).str()))
	mut var_message := rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('PayPal Standard does not support your store currency (%s).'), rt.new_string('woocommerce')]), var_currency.dup()])
	mut var_notice_html := rt.new_string('<div class="notice notice-error is-dismissible">' + '<a class="woocommerce-message-close notice-dismiss" style="text-decoration: none;" href="' + (rt.call_function('esc_url', [var_dismiss_url.dup()])).str() + '" aria-label="' + (rt.call_function('esc_attr__', [rt.new_string('Dismiss this notice'), rt.new_string('woocommerce')])).str() + '"></a>' + '<p>' + (var_message).str() + '</p>' + '</div>')
	rt.echo_val(rt.call_function('wp_kses_post', [var_notice_html.dup()]))
}

fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Notices) get_dismiss_url(notice_name string) string {
	return (rt.call_function('wp_nonce_url', [rt.call_function('add_query_arg', [rt.new_string('wc-hide-notice'), rt.new_string(notice_name)]), rt.new_string('woocommerce_hide_notices_nonce'), rt.new_string('_wc_notice_nonce')])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Notices) has_account_restriction_flag() bool {
	return (rt.identical(rt.new_string('yes'), rt.call_function('get_option', [rt.new_string('woocommerce_paypal_account_restricted_status'), rt.new_string('no')]))).to_bool()
}

fn Class_Automattic_WooCommerce_Gateways_PayPal_Notices.set_account_restriction_flag()  {
	if rt.is_true(rt.identical(rt.new_string('no'), rt.call_function('get_option', [rt.new_string('woocommerce_paypal_account_restricted_status'), rt.new_string('no')]))) {
		rt.call_function('update_option', [rt.new_string('woocommerce_paypal_account_restricted_status'), rt.new_string('yes'), rt.new_bool(false)])
	}
}

fn Class_Automattic_WooCommerce_Gateways_PayPal_Notices.clear_account_restriction_flag()  {
	if rt.is_true(rt.identical(rt.new_string('yes'), rt.call_function('get_option', [rt.new_string('woocommerce_paypal_account_restricted_status'), rt.new_string('no')]))) {
		rt.call_function('update_option', [rt.new_string('woocommerce_paypal_account_restricted_status'), rt.new_string('no')])
	}
}
...
fn Class_Automattic_WooCommerce_Gateways_PayPal_Notices.manage_account_restriction_flag_for_notice(var_http_code rt.PhpVal, mut var_response_data Class_Automattic_WooCommerce_Gateways_PayPal_array, mut var_order Class_WC_Order)  {
	if rt.is_true(rt.call_function('in_array', [// unsupported expression: Expr_Cast_Int, rt.create_array([rt.ArrayItem{ key: none, val: 200 }, rt.ArrayItem{ key: none, val: 201 }]), rt.new_bool(true)])) {
	...
	if rt.is_true(rt.identical(rt.new_int(422), // unsupported expression: Expr_Cast_Int)) {
		...
		if rt.is_true(rt.call_function('in_array', [var_issue.dup(), Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_Notices.paypal_account_restriction_issues(), rt.new_bool(true)])) {
			fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Gateways_PayPal_WC_Gateway_Paypal{}; return temp.log(arg_0) }(rt.new_string('PayPal account restriction flag set due to issues when handling the order: ' + (var_order.get_id()).str()))
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Notices) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'manage_account_restriction_flag_for_notice' {
			...
			Class_Automattic_WooCommerce_Gateways_PayPal_Notices.manage_account_restriction_flag_for_notice(dispatch_arg_0, mut dispatch_arg_1, mut dispatch_arg_2)
			return rt.new_null()
		}
		else { return none }
	}
...
pub fn init_wp_content_plugins_woocommerce_src_gateways_paypal_notices_php() {
