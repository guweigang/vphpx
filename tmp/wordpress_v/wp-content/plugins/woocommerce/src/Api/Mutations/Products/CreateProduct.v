...
fn (mut this Class_Automattic_WooCommerce_Api_Mutations_Products_CreateProduct) init(mut var_repository Class_Automattic_WooCommerce_Api_Utils_Products_ProductRepository)  {
	this.repository = var_repository.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Api_Mutations_Products_CreateProduct) execute(mut var_input Class_Automattic_WooCommerce_Api_InputTypes_Products_CreateProductInput) rt.PhpVal {
	mut var_existing := create_automattic_woocommerce_api_mutations_products_wp_query(rt.create_array([rt.ArrayItem{ key: 'post_type', val: 'product' }, rt.ArrayItem{ key: 'title', val: rt.get_property(var_input, 'name') }, rt.ArrayItem{ key: 'post_status', val: rt.create_array([rt.ArrayItem{ key: none, val: 'publish' }, rt.ArrayItem{ key: none, val: 'draft' }, rt.ArrayItem{ key: none, val: 'pending' }, rt.ArrayItem{ key: none, val: 'private' }]) }, rt.ArrayItem{ key: 'fields', val: 'ids' }]))
	if rt.is_true(rt.greater(rt.get_property(var_existing, 'found_posts'), rt.new_int(0))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Api_ApiException', []string{}, create_automattic_woocommerce_api_apiexception(rt.new_string('A product with this name already exists.'), rt.new_string('VALIDATION_ERROR'), rt.create_array([rt.ArrayItem{ key: 'field', val: 'name' }]), rt.new_int(422))))
	}
	...
	{
		mut iter_1 := rt.create_array([rt.ArrayItem{ key: none, val: 'slug' }, rt.ArrayItem{ key: none, val: 'sku' }, rt.ArrayItem{ key: none, val: 'description' }, rt.ArrayItem{ key: none, val: 'short_description' }, rt.ArrayItem{ key: none, val: 'manage_stock' }, rt.ArrayItem{ key: none, val: 'stock_quantity' }]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_field := item_1.val
			if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
				rt.call_method(var_wc_product, "set_${var_field.to_string()}", [rt.get_property(var_input, '{"nodeType":"Expr_Variable","line":90,"name":"field"}')])
			}
		}
	}
	{
		...
		for {
			...
			mut var_field := item_1.val
			if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
				rt.call_method(var_wc_product, "set_${var_field.to_string()}", [// unsupported expression: Expr_Cast_String])
			}
		}
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
	...
	return fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Api_Utils_Products_ProductMapper{}; return temp.from_wc_product(arg_0) }(rt.new_object('Automattic_WooCommerce_Api_Mutations_Products_WC_Product', []string{}, var_wc_product))
}
...
fn create_automattic_woocommerce_api_mutations_products_createproduct() &Class_Automattic_WooCommerce_Api_Mutations_Products_CreateProduct {
...
fn create_automattic_woocommerce_api_mutations_products_wp_query() &Class_Automattic_WooCommerce_Api_Mutations_Products_WP_Query {
...
fn create_automattic_woocommerce_api_mutations_products_wc_product() &Class_Automattic_WooCommerce_Api_Mutations_Products_WC_Product {
...
fn (mut this Class_Automattic_WooCommerce_Api_Mutations_Products_CreateProduct) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Api_Utils_Products_ProductRepository](if args.len > 0 { args[0] } else { rt.new_null() })
			this.init(mut dispatch_arg_0)
			return rt.new_null()
		}
		'execute' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Api_InputTypes_Products_CreateProductInput](if args.len > 0 { args[0] } else { rt.new_null() })
...
fn (mut this Class_Automattic_WooCommerce_Api_Mutations_Products_CreateProduct) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'repository' { this.repository = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_api_mutations_products_createproduct_php() {
