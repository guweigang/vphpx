...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskListSection) construct(var_data rt.PhpVal, var_task_list rt.PhpVal)  {
	...
	mut var_defaults := rt.create_array([rt.ArrayItem{ key: 'id', val: '' }, rt.ArrayItem{ key: 'title', val: '' }, rt.ArrayItem{ key: 'description', val: '' }, rt.ArrayItem{ key: 'image', val: '' }, rt.ArrayItem{ key: 'tasks', val: rt.new_array() }])
	...
	this.description = var_data_mutated.array_get('description')
	this.image = var_data_mutated.array_get('image')
	this.task_names = var_data_mutated.array_get('task_names')
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskListSection) is_complete() rt.PhpVal {
	mut var_complete := rt.new_bool(rt.new_bool(true))
	{
		mut iter_1 := this.task_names.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_task_name := item_1.val
			if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.get_property(this.task_list, 'task_class_id_map').array_isset(var_task_name))) {
				mut var_task := rt.call_method(this.task_list, 'get_task', [rt.get_property(this.task_list, 'task_class_id_map').array_get(var_task_name)])
				if rt.is_true(rt.new_bool(rt.is_true(rt.call_method(var_task, 'can_view', []rt.PhpVal{})) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_task, 'is_complete', []rt.PhpVal{}))))))) {
					var_complete = rt.new_bool(rt.new_bool(false))
					break
				}
			}
		}
	}
	return var_complete.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskListSection) get_json() rt.PhpVal {
	...
	mut var_task_name := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.get_property(this.task_list, 'task_class_id_map').array_isset(var_task_name))) {
	...
	return rt.create_array([rt.ArrayItem{ key: 'id', val: this.id }, rt.ArrayItem{ key: 'title', val: this.title }, rt.ArrayItem{ key: 'description', val: this.description }, rt.ArrayItem{ key: 'image', val: this.image }, rt.ArrayItem{ key: 'tasks', val: rt.call_function('array_map', [rt.new_closure(closure_1_fn), this.task_names]) }, rt.ArrayItem{ key: 'isComplete', val: this.is_complete() }])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskListSection) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'task_names' { this.task_names = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_onboardingtasks_tasklistsection_php() {
}
