...
fn Class_Automattic_WooCommerce_Admin_Features_PaymentGatewaySuggestions_PaymentGatewaySuggestionsDataSourcePoller.get_data_sources() rt.PhpVal {
	mut var_data_sources := rt.create_array([rt.ArrayItem{ key: none, val: (fn () rt.PhpVal { mut temp := Class_WC_Helper{}; return temp.get_woocommerce_com_base_url() }()).str() + 'wp-json/wccom/payment-gateway-suggestions/2.0/suggestions.json' }])
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_paymentgatewaysuggestions_paymentgatewaysuggestionsdatasourcepoller_php() {
}
