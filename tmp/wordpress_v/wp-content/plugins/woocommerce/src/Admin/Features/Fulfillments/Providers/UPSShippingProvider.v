...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_UPSShippingProvider) get_tracking_url(tracking_number string) string {
	...
	return 'https://www.ups.com/track?tracknum=' + (rt.call_function('rawurlencode', [rt.new_string(tracking_number_mutated).dup()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_UPSShippingProvider) try_parse_tracking_number(tracking_number string, shipping_from string, shipping_to string) rt.PhpVal {
	...
	closure_3_fn := fn [var_shipping_from] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	...
	return rt.new_int(if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils{}; return temp.validate_ups_1z_check_digit(arg_0) }(rt.new_string(tracking_number_mutated))) { 100 } else { 95 })
	}
	...
	mut var_patterns := rt.create_array([rt.ArrayItem{ key: '/^1Z[0-9A-Z]{16}$/', val: rt.new_closure(closure_1_fn) }, rt.ArrayItem{ key: '/^\\d{12}$/', val: rt.new_closure(closure_2_fn) }, rt.ArrayItem{ key: '/^\\d{10}$/', val: 75 }, rt.ArrayItem{ key: '/^\\d{9}$/', val: 70 }, rt.ArrayItem{ key: '/^[THV]\\d{10}$/', val: 85 }, rt.ArrayItem{ key: '/^J\\d{10}$/', val: 80 }, rt.ArrayItem{ key: '/^MI\\d{6}[A-Z0-9]{6,22}$/', val: 80 }, rt.ArrayItem{ key: '/^9\\d{21,33}$/', val: rt.new_closure(closure_3_fn) }, rt.ArrayItem{ key: '/^[A-Z]{2}\\d{9}[A-Z]{2}$/', val: rt.new_closure(closure_4_fn) }, rt.ArrayItem{ key: '/^\\d{22}$/', val: 60 }])
	mut var_match := rt.new_bool(rt.new_bool(false))
	mut var_ambiguity_score := rt.new_int(rt.new_int(0))
	{
		...
		for {
			...
			if rt.is_true(rt.call_function('preg_match', [var_pattern.dup(), rt.new_string(tracking_number_mutated).dup()])) {
				var_match = rt.new_bool(rt.new_bool(true))
				var_ambiguity_score = if rt.is_true(rt.call_function('is_callable', [var_score.dup()])) { rt.call_callable(var_score, []rt.PhpVal{}) } else { var_score }
				break
			}
		}
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_match) && rt.is_true(var_is_domestic_shipping))) && rt.is_true(rt.call_function('in_array', [rt.new_string(shipping_from), this.domestic_but_international_tracking, rt.new_bool(true)])))) {
...
fn create_automattic_woocommerce_admin_features_fulfillments_providers_upsshippingprovider() &Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_UPSShippingProvider {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_fulfillments_providers_upsshippingprovider_php() {
