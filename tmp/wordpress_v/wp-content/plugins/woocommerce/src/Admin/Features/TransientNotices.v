...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_TransientNotices) construct()  {
	rt.call_function('add_filter', [rt.new_string('woocommerce_admin_preload_options'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_TransientNotices', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'preload_options' }])])
}

fn Class_Automattic_WooCommerce_Admin_Features_TransientNotices.get_queue() rt.PhpVal {
	return rt.call_function('get_option', [Class_Automattic_WooCommerce_Admin_Features_Automattic_WooCommerce_Admin_Features_TransientNotices.queue_option(), rt.new_array()])
}
...
fn Class_Automattic_WooCommerce_Admin_Features_TransientNotices.get_queue_by_user(var_user_id rt.PhpVal) rt.PhpVal {
	...
	mut var_notice := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.new_bool(rt.is_true(rt.new_bool(!(var_notice.array_isset(rt.new_string('user_id'))) || rt.is_true(rt.identical(rt.new_null(), var_notice.array_get('user_id'))))) || rt.is_true(rt.identical(var_user_id, var_notice.array_get('user_id'))))
	}
	return rt.call_function('array_filter', [var_notices.dup(), rt.new_closure(closure_1_fn)])
}
...
fn Class_Automattic_WooCommerce_Admin_Features_TransientNotices.add(var_notice rt.PhpVal)  {
	mut var_queue := Class_Automattic_WooCommerce_Admin_Features_TransientNotices.get_queue()
	mut var_defaults := rt.create_array([rt.ArrayItem{ key: 'user_id', val: rt.new_null() }, rt.ArrayItem{ key: 'status', val: 'info' }, rt.ArrayItem{ key: 'options', val: rt.new_array() }])
	mut var_notice_data := rt.call_function('array_merge', [var_defaults.dup(), var_notice.dup()])
	var_notice_data.array_set('options', // unsupported expression: Expr_Cast_Object)
	var_queue.array_set(var_notice.array_get('id'), var_notice_data.dup())
	rt.call_function('update_option', [Class_Automattic_WooCommerce_Admin_Features_Automattic_WooCommerce_Admin_Features_TransientNotices.queue_option(), var_queue.dup()])
}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_transientnotices_php() {
}
