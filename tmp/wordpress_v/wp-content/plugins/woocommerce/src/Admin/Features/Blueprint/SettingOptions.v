	...
	mut var_page_options := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.call_function('in_array', [var_setting.array_get('type'), this.ignore_setting_types, rt.new_bool(true)]))
				|| !(var_setting.array_isset(rt.new_string('id')))))
			{
				continue
			}
			mut var_key := if rt.is_true(rt.new_bool(var_setting.array_get('option_key').is_array())) {
				var_setting.array_get('option_key').array_get(0)
			} else {
				var_setting.array_get('option_key')
			}
			if rt.is_true(rt.call_function('in_array', [var_key.dup(),
				var_page_options.dup(), rt.new_bool(true)]))
			{
				continue
			}
			mut var_default_value := if !(var_setting.array_get('default')).is_null() {
				var_setting.array_get('default')
			} else {
				rt.new_null()
			}
			var_page_options.array_set(var_key, rt.call_function('get_option', [
				var_key.dup(), var_default_value.dup()]))
		}
	}
	return var_page_options.dup()
}
...
fn create_automattic_woocommerce_admin_features_blueprint_wc_rest_setting_options_controller() &Class_Automattic_WooCommerce_Admin_Features_Blueprint_WC_REST_Setting_Options_Controller {
...
fn create_automattic_woocommerce_admin_features_blueprint_exception() &Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exception {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_blueprint_settingoptions_php() {
