...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exporters_ExportWCSettingsTax) generatetaxratesteps(table string) rt.PhpVal {
	...
	mut var_record := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return create_automattic_woocommerce_blueprint_steps_runsql(fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blueprint_Util{}; return temp.array_to_insert_sql(arg_0, arg_1, arg_2) }(var_record.dup(), rt.new_string(table_mutated), rt.new_string('replace into')))
	}
	mut var_record := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return create_automattic_woocommerce_blueprint_steps_runsql(fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blueprint_Util{}; return temp.array_to_insert_sql(arg_0, arg_1, arg_2) }(var_record.dup(), rt.new_string(table_mutated), rt.new_string('replace into')))
	}
	return rt.call_function('array_map', [rt.new_closure(closure_1_fn), rt.call_method(var_wpdb, 'get_results', [rt.call_method(var_wpdb, 'prepare', [rt.new_string('SELECT * FROM %i'), rt.new_string(table_mutated).dup()]), rt.get_constant('ARRAY_A')])])
}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_blueprint_exporters_exportwcsettingstax_php() {
