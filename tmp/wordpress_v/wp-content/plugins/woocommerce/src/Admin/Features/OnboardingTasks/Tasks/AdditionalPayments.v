...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_AdditionalPayments) get_title() rt.PhpVal {
	return rt.call_function('__', [rt.new_string('Set up additional payment options'), rt.new_string('woocommerce')])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_AdditionalPayments) can_view() bool {
	...
	if rt.is_true(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_AdditionalPayments{}; return temp.has_gateways() }()) {
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_AdditionalPayments) get_action_url() string {
	return (rt.call_function('admin_url', ['admin.php?page=wc-settings&tab=checkout&other_pes_section=expanded&from=' + (Class_Automattic_WooCommerce_Internal_Admin_Settings_Payments.from_additional_payments_task()).str()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_AdditionalPayments) has_enabled_non_psp_payment_suggestion() bool {
	mut var_providers := this.get_payment_providers()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(!rt.is_true(var_provider.array_get('state').array_get('enabled'))) && !(!rt.is_true(var_provider.array_get('_suggestion_category_id'))) && rt.is_true(rt.call_function('in_array', [var_provider.array_get('_suggestion_category_id'), rt.create_array([rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders.category_bnpl() }, rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders.category_express_checkout() }, rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders.category_crypto() }]), rt.new_bool(true)])))) {
				return true
			}
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_AdditionalPayments) get_payment_providers() rt.PhpVal {
	...
	mut var_providers := rt.call_method(var_settings_payments_service, 'get_payment_providers', [rt.call_method(var_settings_payments_service, 'get_country', []rt.PhpVal{}), rt.new_bool(false)])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_tasks_additionalpayments() &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_AdditionalPayments {
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_AdditionalPayments) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'can_view_result' { this.can_view_result = (val).to_bool(); return true }
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_onboardingtasks_tasks_additionalpayments_php() {
}
