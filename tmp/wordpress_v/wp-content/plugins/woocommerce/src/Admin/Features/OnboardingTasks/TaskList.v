...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) construct(var_data rt.PhpVal)  {
	...
	mut var_defaults := rt.create_array([rt.ArrayItem{ key: 'id', val: rt.new_null() }, rt.ArrayItem{ key: 'hidden_id', val: rt.new_null() }, rt.ArrayItem{ key: 'title', val: '' }, rt.ArrayItem{ key: 'tasks', val: rt.new_array() }, rt.ArrayItem{ key: 'sort_by', val: rt.new_array() }, rt.ArrayItem{ key: 'event_prefix', val: rt.new_null() }, rt.ArrayItem{ key: 'options', val: rt.new_array() }, rt.ArrayItem{ key: 'visible', val: true }, rt.ArrayItem{ key: 'display_progress_header', val: false }])
	var_data_mutated = rt.call_function('wp_parse_args', [var_data_mutated.dup(), var_defaults.dup()])
	this.id = var_data_mutated.array_get('id')
	this.hidden_id = var_data_mutated.array_get('hidden_id')
	this.title = var_data_mutated.array_get('title')
	this.sort_by = var_data_mutated.array_get('sort_by')
	this.event_prefix = var_data_mutated.array_get('event_prefix')
	this.options = var_data_mutated.array_get('options')
	this.visible = var_data_mutated.array_get('visible')
	this.display_progress_header = var_data_mutated.array_get('display_progress_header')
	{
		mut iter_1 := var_data_mutated.array_get('tasks').iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_task_name := item_1.val
			mut var_class := rt.new_string('Automattic\\WooCommerce\\Admin\\Features\\OnboardingTasks\\Tasks\\' + (var_task_name).str())
			mut var_task := rt.create_object_dynamically(var_class, [rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList', []string{}, &this)])
			this.add_task(var_task.dup())
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) is_hidden() rt.PhpVal {
	mut var_hidden := rt.call_function('get_option', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.hidden_option(), rt.new_array()])
	return rt.call_function('in_array', [if rt.is_true(this.hidden_id) { this.hidden_id } else { this.id }, var_hidden.dup(), rt.new_bool(true)])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) is_visible() bool {
	...
	mut var_no_viewable_tasks := rt.new_bool(rt.new_bool(this.get_viewable_tasks().array_count() == 0))
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) hide() rt.PhpVal {
	...
	mut var_total := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	mut var_task := if args.len > 1 { args[1].dup() } else { rt.new_null() }
	return if rt.is_true(rt.call_method(var_task, 'is_complete', []rt.PhpVal{})) { rt.add(var_total, rt.new_int(1)) } else { var_total }
	}
	mut var_completed_count := rt.call_function('array_reduce', [var_viewable_tasks.dup(), rt.new_closure(closure_1_fn), rt.new_int(0)])
	this.record_tracks_event(rt.new_string('completed'), rt.create_array([rt.ArrayItem{ key: 'action', val: 'remove_card' }, rt.ArrayItem{ key: 'completed_task_count', val: var_completed_count }, rt.ArrayItem{ key: 'incomplete_task_count', val: rt.sub(rt.new_int(var_viewable_tasks.dup().array_count()), var_completed_count) }, rt.ArrayItem{ key: 'tasklist_id', val: this.id }]))
	mut var_hidden := rt.call_function('get_option', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.hidden_option(), rt.new_array()])
	var_hidden.array_push(if rt.is_true(this.hidden_id) { this.hidden_id } else { this.id })
	this.maybe_set_default_layout(var_hidden.dup())
	return rt.call_function('update_option', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.hidden_option(), rt.call_function('array_unique', [var_hidden.dup()])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) maybe_set_default_layout(var_completed_or_hidden_tasklist_ids rt.PhpVal)  {
	if rt.is_true(rt.call_function('in_array', [rt.new_string('setup'), var_completed_or_hidden_tasklist_ids.dup(), rt.new_bool(true)])) {
		rt.call_function('update_option', [rt.new_string('woocommerce_default_homepage_layout'), rt.new_string('two_columns')])
	}
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) unhide() rt.PhpVal {
	mut var_hidden := rt.call_function('get_option', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.hidden_option(), rt.new_array()])
	var_hidden = rt.call_function('array_diff', [var_hidden.dup(), rt.create_array([rt.ArrayItem{ key: none, val: if rt.is_true(this.hidden_id) { this.hidden_id } else { this.id } }])])
	return rt.call_function('update_option', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.hidden_option(), var_hidden.dup()])
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) is_complete() bool {
	{
		mut iter_1 := this.get_viewable_tasks().iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_viewable_task := item_1.val
			if rt.is_true(rt.identical(rt.call_method(var_viewable_task, 'is_complete', []rt.PhpVal{}), rt.new_bool(false))) {
				return false
			}
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) has_previously_completed() rt.PhpVal {
	mut var_complete := rt.call_function('get_option', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.completed_option(), rt.new_array()])
	return rt.call_function('in_array', [this.get_list_id(), var_complete.dup(), rt.new_bool(true)])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) add_task(var_task rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_subclass_of', [var_task_mutated.dup(), rt.new_string('Automattic\\WooCommerce\\Admin\\Features\\OnboardingTasks\\Task')]))))) {
		return create_automattic_woocommerce_admin_features_onboardingtasks_wp_error(rt.new_string('woocommerce_task_list_invalid_task'), rt.call_function('__', [rt.new_string('Task is not a subclass of `Task`'), rt.new_string('woocommerce')]))
	}
	if rt.is_true(rt.call_function('array_search', [var_task_mutated.dup(), this.tasks, rt.new_bool(true)])) {
		return rt.new_null()
	}
	this.tasks.array_push(var_task_mutated.dup())
	return rt.new_null()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) get_task(var_task_id rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('current', [rt.call_function('array_filter', [this.tasks, rt.new_closure(closure_2_fn)])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) get_viewable_tasks() rt.PhpVal {
	...
	return rt.call_function('array_values', [rt.call_function('array_filter', [this.tasks, rt.new_closure(closure_4_fn)])])
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) get_sections() rt.PhpVal {
	rt.call_function('wc_deprecated_function', [@STRUCT + '::' + @FN, rt.new_string('7.2.0')])
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) possibly_track_completion()  {
	...
	mut var_completed_lists := rt.call_function('get_option', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.completed_option(), rt.new_array()])
	var_completed_lists.array_push(this.get_list_id())
	rt.call_function('update_option', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.completed_option(), var_completed_lists.dup(), rt.new_bool(true)])
	this.maybe_set_default_layout(var_completed_lists.dup())
	this.record_tracks_event(rt.new_string('tasks_completed'), rt.create_array([rt.ArrayItem{ key: 'tasklist_id', val: this.id }]))
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) sort_tasks(var_sort_by rt.PhpVal) rt.PhpVal {
	...
	var_sort_by_mutated = if var_sort_by_mutated.dup().array_count() > 0 { var_sort_by_mutated } else { this.sort_by }
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		closure_6_fn := fn [var_sort_by] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_a := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	mut var_b := if args.len > 1 { args[1].dup() } else { rt.new_null() }
	return fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Task{}; return temp.sort(arg_0, arg_1, arg_2) }(var_a.dup(), var_b.dup(), var_sort_by_mutated.dup())
	}
		rt.call_function('usort', [this.tasks, rt.new_closure(closure_6_fn)])
	}
	return rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList', []string{}, this)
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) get_keep_completed_task_list() rt.PhpVal {
	return rt.call_function('get_option', [rt.new_string('woocommerce_task_list_keep_completed'), rt.new_string('no')])
}

fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.possibly_remove_reminder_bar()  {
	mut var_bar_hidden := rt.call_function('get_option', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.reminder_bar_hidden_option(), rt.new_string('no')])
	mut var_active_for_four_weeks := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_WCAdminHelper{}; return temp.is_wc_admin_active_for(arg_0) }(rt.mul(rt.get_constant('WEEK_IN_SECONDS'), rt.new_int(4)))
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('yes'), var_bar_hidden)) || rt.is_true(rt.new_bool(!(rt.is_true(var_active_for_four_weeks)))))) {
		return rt.new_null()
	}
	rt.call_function('update_option', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList.reminder_bar_hidden_option(), rt.new_string('yes')])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) get_json() rt.PhpVal {
	...
	{
		mut iter_1 := this.tasks.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_task := item_1.val
			mut var_list_is_visible := rt.new_bool(rt.new_bool(this.is_visible() || rt.is_true(rt.identical(rt.new_string('secret_tasklist'), this.id))))
			if rt.is_true(rt.new_bool(rt.is_true(var_list_is_visible) || rt.is_true(rt.new_bool(rt.is_true(rt.call_function('method_exists', [var_task.dup(), rt.new_string('is_always_accessible')])) && rt.is_true(rt.call_method(var_task, 'is_always_accessible', []rt.PhpVal{})))))) {
				mut var_json := rt.call_method(var_task, 'get_json', []rt.PhpVal{})
				if rt.is_true(var_json.array_get('canView')) {
					var_tasks_json.array_push(var_json.dup())
				}
			}
		}
	}
	return rt.create_array([rt.ArrayItem{ key: 'id', val: this.get_list_id() }, rt.ArrayItem{ key: 'title', val: this.title }, rt.ArrayItem{ key: 'isHidden', val: this.is_hidden() }, rt.ArrayItem{ key: 'isVisible', val: this.is_visible() }, rt.ArrayItem{ key: 'isComplete', val: this.is_complete() }, rt.ArrayItem{ key: 'tasks', val: var_tasks_json }, rt.ArrayItem{ key: 'eventPrefix', val: this.prefix_event(rt.new_string('')) }, rt.ArrayItem{ key: 'displayProgressHeader', val: this.display_progress_header }, rt.ArrayItem{ key: 'keepCompletedTaskList', val: this.get_keep_completed_task_list() }])
}
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_wp_error() &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_WP_Error {
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'hidden_id' { this.hidden_id = val; return true }
		'display_progress_header' { this.display_progress_header = val; return true }
		'title' { this.title = val; return true }
		'tasks' { this.tasks = val; return true }
		'sort_by' { this.sort_by = val; return true }
		'event_prefix' { this.event_prefix = val; return true }
		'visible' { this.visible = val; return true }
		'options' { this.options = val; return true }
		'sections' { this.sections = val; return true }
		'task_class_id_map' { this.task_class_id_map = val; return true }
...
fn init_registry() {
	rt.register_class_factory('Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList', fn(args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList', []string{}, obj)
	})
	rt.register_class_factory('Automattic_WooCommerce_Admin_Features_OnboardingTasks_WP_Error', fn(args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_WP_Error', []string{}, obj)
	})
	rt.register_class_factory('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Task', fn(args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Task', []string{}, obj)
	})
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_onboardingtasks_tasklist_php() {
}
