...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_USPSShippingProvider) get_tracking_url(tracking_number string) string {
	...
	return 'https://tools.usps.com/go/TrackConfirmAction?tLabels=' + (rt.call_function('rawurlencode', [rt.new_string(tracking_number_mutated).dup()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_USPSShippingProvider) try_parse_tracking_number(tracking_number string, shipping_from string, shipping_to string) rt.PhpVal {
	...
	closure_3_fn := fn [var_tracking_number] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	...
	return rt.new_int(if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils{}; return temp.validate_mod10_check_digit(arg_0) }(rt.new_string(tracking_number_mutated))) { 100 } else { 95 })
	}
	return rt.new_int(if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils{}; return temp.check_s10_upu_format(arg_0) }(rt.new_string(tracking_number_mutated))) { 98 } else { 90 })
	}
	return rt.new_int(if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils{}; return temp.validate_mod10_check_digit(arg_0) }(rt.new_string(tracking_number_mutated))) { 90 } else { 80 })
	}
	return rt.new_int(if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils{}; return temp.validate_mod10_check_digit(arg_0) }(rt.new_string(tracking_number_mutated))) { 88 } else { 80 })
	}
	mut var_patterns := rt.create_array([rt.ArrayItem{ key: '/^(94|93|92|95|96|94|94|94|94)\\d{18,22}$/', val: rt.new_closure(closure_1_fn) }, rt.ArrayItem{ key: '/^[A-Z]{2}\\d{9}[A-Z]{2}$/', val: rt.new_closure(closure_2_fn) }, rt.ArrayItem{ key: '/^82\\d{8,9}$/', val: 95 }, rt.ArrayItem{ key: '/^420\\d{23,31}$/', val: 90 }, rt.ArrayItem{ key: '/^\\d{20,22}$/', val: 80 }, rt.ArrayItem{ key: '/^9\\d{21,33}$/', val: 75 }, rt.ArrayItem{ key: '/^91\\d{18,20}$/', val: rt.new_closure(closure_3_fn) }, rt.ArrayItem{ key: '/^030[67]\\d{16,20}$/', val: rt.new_closure(closure_4_fn) }])
	{
		...
		for {
			...
			if rt.is_true(rt.call_function('preg_match', [var_pattern.dup(), rt.new_string(tracking_number_mutated).dup()])) {
				mut var_ambiguity_score := if rt.is_true(rt.call_function('is_callable', [var_score.dup()])) { rt.call_callable(var_score, []rt.PhpVal{}) } else { var_score }
	...
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^[A-Z]{2}\\d{9}US$/'), rt.new_string(tracking_number_mutated).dup()])) {
	...
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^\\d{20,34}$/'), rt.new_string(tracking_number_mutated).dup()])) {
...
fn create_automattic_woocommerce_admin_features_fulfillments_providers_uspsshippingprovider() &Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_USPSShippingProvider {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_fulfillments_providers_uspsshippingprovider_php() {
