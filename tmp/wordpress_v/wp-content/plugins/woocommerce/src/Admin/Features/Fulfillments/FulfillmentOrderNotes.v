...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) register()  {
	rt.call_function('add_action', [rt.new_string('woocommerce_fulfillment_after_create'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_fulfillment_created_note' }]), rt.new_int(10), rt.new_int(1)])
	rt.call_function('add_action', [rt.new_string('woocommerce_fulfillment_after_update'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_fulfillment_updated_note' }]), rt.new_int(10), rt.new_int(3)])
	rt.call_function('add_action', [rt.new_string('woocommerce_fulfillment_after_delete'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_fulfillment_deleted_note' }]), rt.new_int(10), rt.new_int(1)])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) add_fulfillment_created_note(mut var_fulfillment Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment)  {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_order, 'Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Order')))))) {
		return rt.new_null()
	}
	mut var_items_text := rt.new_string(this.format_items(mut var_fulfillment, mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Order](var_order)))
	mut var_tracking_text := rt.new_string(this.format_tracking(mut var_fulfillment))
	mut var_status := if !(var_fulfillment.get_status()).is_null() { var_fulfillment.get_status() } else { rt.new_string('unfulfilled') }
	mut var_status_label := rt.new_string(this.get_fulfillment_status_label((var_status).str()))
	mut var_message := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Fulfillment #%1$d created (status: %2$s). Items: %3$s.'), rt.new_string('woocommerce')]), var_fulfillment.get_id(), var_status_label.dup(), var_items_text.dup()])
	if !(!rt.is_true(var_tracking_text)) {
		// unsupported expression: Expr_AssignOp_Concat
	}
	var_message = rt.call_function('apply_filters', [rt.new_string('woocommerce_fulfillment_created_order_note'), var_message.dup(), var_fulfillment, var_order.dup()])
	var_message = rt.new_string(this.normalize_note_message(var_message.dup()))
	if rt.is_true(rt.identical(rt.new_null(), var_message)) {
		return rt.new_null()
	}
	rt.call_method(var_order, 'add_order_note', [var_message.dup(), rt.new_int(0), rt.new_bool(false), rt.create_array([rt.ArrayItem{ key: 'note_group', val: Class_Automattic_WooCommerce_Internal_Orders_OrderNoteGroup.fulfillment() }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) add_fulfillment_updated_note(mut var_fulfillment Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment, mut var_changes Class_Automattic_WooCommerce_Admin_Features_Fulfillments_array, previous_status string)  {
	...
	if rt.is_true(rt.new_bool(var_changes.array_isset(rt.new_string('status')))) {
		mut var_new_status := if !(var_changes.array_get('status')).is_null() { var_changes.array_get('status') } else { rt.new_string('unfulfilled') }
		this.add_fulfillment_status_changed_note(mut var_fulfillment, mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Order](var_order), previous_status, (var_new_status).str())
		return rt.new_null()
	}
	mut var_items_text := rt.new_string(this.format_items(mut var_fulfillment, mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Order](var_order)))
	mut var_tracking_text := rt.new_string(this.format_tracking(mut var_fulfillment))
	mut var_message := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Fulfillment #%1$d updated. Items: %2$s.'), rt.new_string('woocommerce')]), var_fulfillment.get_id(), var_items_text.dup()])
	if !(!rt.is_true(var_tracking_text)) {
		// unsupported expression: Expr_AssignOp_Concat
	}
	var_message = rt.call_function('apply_filters', [rt.new_string('woocommerce_fulfillment_updated_order_note'), var_message.dup(), var_fulfillment, var_order.dup()])
	var_message = rt.new_string(this.normalize_note_message(var_message.dup()))
	if rt.is_true(rt.identical(rt.new_null(), var_message)) {
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) add_fulfillment_deleted_note(mut var_fulfillment Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment)  {
	...
	mut var_message := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Fulfillment #%d deleted.'), rt.new_string('woocommerce')]), var_fulfillment.get_id()])
	var_message = rt.call_function('apply_filters', [rt.new_string('woocommerce_fulfillment_deleted_order_note'), var_message.dup(), var_fulfillment, var_order.dup()])
	var_message = rt.new_string(this.normalize_note_message(var_message.dup()))
	if rt.is_true(rt.identical(rt.new_null(), var_message)) {
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) add_order_fulfillment_status_changed_note(mut var_order Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Order, old_status string, new_status string)  {
	...
	mut var_new_status_label := rt.new_string(this.get_order_fulfillment_status_label(new_status_mutated))
	mut var_message := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Order fulfillment status changed from %1$s to %2$s.'), rt.new_string('woocommerce')]), var_old_status_label.dup(), var_new_status_label.dup()])
	var_message = rt.call_function('apply_filters', [rt.new_string('woocommerce_fulfillment_order_status_changed_order_note'), var_message.dup(), var_order_mutated.dup(), rt.new_string(old_status), rt.new_string(new_status_mutated).dup()])
	var_message = rt.new_string(this.normalize_note_message(var_message.dup()))
	if rt.is_true(rt.identical(rt.new_null(), var_message)) {
		return rt.new_null()
	}
	rt.call_method(var_order_mutated, 'add_order_note', [var_message.dup(), rt.new_int(0), rt.new_bool(false), rt.create_array([rt.ArrayItem{ key: 'note_group', val: Class_Automattic_WooCommerce_Internal_Orders_OrderNoteGroup.fulfillment() }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) add_fulfillment_status_changed_note(mut var_fulfillment Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment, mut var_order Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Order, old_status string, new_status string)  {
	...
	mut var_message := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Fulfillment #%1$d status changed from %2$s to %3$s.'), rt.new_string('woocommerce')]), var_fulfillment.get_id(), var_old_status_label.dup(), var_new_status_label.dup()])
	var_message = rt.call_function('apply_filters', [rt.new_string('woocommerce_fulfillment_status_changed_order_note'), var_message.dup(), var_fulfillment, var_order_mutated.dup(), rt.new_string(old_status), rt.new_string(new_status_mutated).dup()])
	var_message = rt.new_string(this.normalize_note_message(var_message.dup()))
	if rt.is_true(rt.identical(rt.new_null(), var_message)) {
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) format_items(mut var_fulfillment Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment, mut var_order Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Order) string {
	...
	mut var_parts := rt.new_array()
	{
		...
		for {
			...
			mut var_item_id := if var_item.array_isset(rt.new_string('item_id')) { // unsupported expression: Expr_Cast_Int } else { rt.new_int(0) }
			mut var_qty := if var_item.array_isset(rt.new_string('qty')) { // unsupported expression: Expr_Cast_Int } else { rt.new_int(0) }
			mut var_name := rt.new_string(rt.new_string(''))
			{
				...
				for {
					...
					if rt.is_true(rt.identical(// unsupported expression: Expr_Cast_Int, var_item_id)) {
			...
			if !rt.is_true(var_name) {
				var_name = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Item #%d'), rt.new_string('woocommerce')]), var_item_id.dup()])
			}
			var_parts.array_push(rt.call_function('sprintf', [rt.new_string('%s x%s'), var_name.dup(), var_qty.dup()]))
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) format_tracking(mut var_fulfillment Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) string {
	...
	mut var_parts := rt.create_array([rt.ArrayItem{ key: none, val: var_tracking_number }])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		var_parts.array_push(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Provider: %s'), rt.new_string('woocommerce')]), var_shipping_provider.dup()]))
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		var_parts.array_push(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('URL: %s'), rt.new_string('woocommerce')]), var_tracking_url.dup()]))
	}
	return (rt.call_function('implode', [rt.new_string(', '), var_parts.dup()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) get_fulfillment_status_label(status string) string {
	...
	return (if !(var_statuses.array_get(status_mutated).array_get('label')).is_null() { var_statuses.array_get(status_mutated).array_get('label') } else { rt.new_string(status_mutated) }).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) get_order_fulfillment_status_label(status string) string {
	mut status_mutated := status
	mut var_statuses := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils{}; return temp.get_order_fulfillment_statuses() }()
	return (if !(var_statuses.array_get(status_mutated).array_get('label')).is_null() { var_statuses.array_get(status_mutated).array_get('label') } else { rt.new_string(status_mutated) }).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) normalize_note_message(var_message rt.PhpVal) string {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_message_mutated)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_message_mutated.dup().is_string()))))))) {
		return (rt.new_null()).str()
	}
	var_message_mutated = rt.call_function('wp_kses_post', [var_message_mutated.dup()])
	var_message_mutated = rt.new_string(rt.new_string(var_message_mutated.dup().to_string().trim_space()))
...
fn create_automattic_woocommerce_admin_features_fulfillments_fulfillmentordernotes() &Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes {
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentOrderNotes) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'add_fulfillment_updated_note' {
			...
			this.add_fulfillment_updated_note(mut dispatch_arg_0, mut dispatch_arg_1, dispatch_arg_2)
			return rt.new_null()
		}
		...
		'add_order_fulfillment_status_changed_note' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Order](if args.len > 0 { args[0] } else { rt.new_null() })
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).str()
			dispatch_arg_2 := (if args.len > 2 { args[2] } else { rt.new_null() }).str()
			this.add_order_fulfillment_status_changed_note(mut dispatch_arg_0, dispatch_arg_1, dispatch_arg_2)
			return rt.new_null()
		}
		'add_fulfillment_status_changed_note' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment](if args.len > 0 { args[0] } else { rt.new_null() })
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Order](if args.len > 1 { args[1] } else { rt.new_null() })
			dispatch_arg_2 := (if args.len > 2 { args[2] } else { rt.new_null() }).str()
			dispatch_arg_3 := (if args.len > 3 { args[3] } else { rt.new_null() }).str()
			this.add_fulfillment_status_changed_note(mut dispatch_arg_0, mut dispatch_arg_1, dispatch_arg_2, dispatch_arg_3)
			return rt.new_null()
		}
		'format_items' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment](if args.len > 0 { args[0] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_fulfillments_fulfillmentordernotes_php() {
