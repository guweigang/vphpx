...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Tracks) init()  {
	rt.call_function('add_filter', [rt.new_string('woocommerce_product_source'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Tracks', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_product_source' }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Tracks) is_product_editor_page(var_url rt.PhpVal) bool {
	...
	mut var_query_string := rt.call_function('wp_parse_url', [rt.call_function('wp_get_referer', []rt.PhpVal{}), rt.get_constant('PHP_URL_QUERY')])
	rt.call_function('parse_str', [var_query_string.dup(), var_query.dup()])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(var_query.array_isset(rt.new_string('page'))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) || !(var_query.array_isset(rt.new_string('path'))))) {
		return false
	}
	mut var_path_pieces := rt.call_function('explode', [rt.new_string('/'), var_query.array_get('path')])
	mut var_route := var_path_pieces.array_get(1)
	return rt.is_true(rt.identical(rt.new_string('add-product'), var_route)) || rt.is_true(rt.identical(rt.new_string('product'), var_route))
}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_productblockeditor_tracks_php() {
}
