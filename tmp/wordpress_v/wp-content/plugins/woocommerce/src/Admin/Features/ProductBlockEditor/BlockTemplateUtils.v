...
fn Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_BlockTemplateUtils.get_templates_directory(template_type string) rt.PhpVal {
	...
	mut var_templates_directory := rt.new_string(rt.concat(var_root_path,
		Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_BlockTemplateUtils.directory_names().array_get('TEMPLATES')))
	mut var_template_parts_directory := rt.new_string(rt.concat(var_root_path,
		Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_BlockTemplateUtils.directory_names().array_get('TEMPLATE_PARTS')))
	if rt.is_true(rt.identical(rt.new_string('wp_template_part'), rt.new_string(template_type))) {
		return var_template_parts_directory.dup()
	}
	return var_templates_directory.dup()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_BlockTemplateUtils.get_block_template_path(var_slug rt.PhpVal) bool {
	...
	mut var_path := rt.new_string(
		(rt.call_function('trailingslashit', [var_directory.dup()])).str() + var_slug.str() + '.php')
...
fn Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_BlockTemplateUtils.get_template_file_data(var_file_path rt.PhpVal) rt.PhpVal {
	...
	mut var_file_data := rt.call_function('get_file_data', [var_file_path.dup(),
	...
	var_file_data.array_set('product_types', rt.call_function('explode', [
		rt.new_string(','),
		rt.new_string(var_file_data.array_get('product_types').to_string().trim_space()),
	]))
	return var_file_data.dup()
}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_productblockeditor_blocktemplateutils_php() {
}
