...
pub mut:
		...
		lists rt.PhpVal = rt.new_array()
		default_tasks_loaded rt.PhpVal = rt.new_bool(false)
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.is_experiment_treatment(var_name rt.PhpVal) rt.PhpVal {
	...
	mut var_abtest := create_automattic_woocommerce_admin_features_onboardingtasks_woocommerce_admin_experimental_abtest(var_anon_id.dup(), rt.new_string('woocommerce'), var_allow_tracking.dup())
	...
	mut var_experiment_name := rt.call_function('sprintf', [rt.new_string('%s_%s_%s'), var_name.dup(), var_date.format(rt.new_string('Y')), var_date.format(rt.new_string('m'))])
	return rt.identical(var_abtest.get_variation(var_experiment_name.dup()), rt.new_string('treatment'))
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.init_default_lists()  {
	...
	if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_Features{}; return temp.is_enabled(arg_0) }(rt.new_string('core-profiler'))) {
		mut var_key := rt.call_function('array_search', [rt.new_string('StoreDetails'), var_tasks.dup(), rt.new_bool(true)])
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.init_tasks()  {
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('method_exists', [var_class.dup(), rt.new_string('init')]))))) {
				continue
			}
			fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_{"nodeType":"Expr_Variable","line":207,"name":"class"}{}; return temp.init() }()
		}
	}
}

fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.set_active_task()  {
	if rt.is_true(rt.new_bool(!(rt.get_superglobal('_GET').array_isset(Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Task.active_task_transient())) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('manage_woocommerce')]))))))) {
		return rt.new_null()
	}
	...
	mut var_task := Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.get_task(var_task_id.dup())
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.add_list(var_args rt.PhpVal) rt.PhpVal {
	if // unsupported expression: Expr_StaticPropertyFetch.array_isset(var_args.array_get('id')) {
		return create_automattic_woocommerce_admin_features_onboardingtasks_wp_error(rt.new_string('woocommerce_task_list_exists'), rt.call_function('__', [rt.new_string('Task list ID already exists'), rt.new_string('woocommerce')]))
	}
	// unsupported expression: Expr_StaticPropertyFetch.array_set(var_args.array_get('id'), create_automattic_woocommerce_admin_features_onboardingtasks_tasklist(var_args.dup()))
	return // unsupported expression: Expr_StaticPropertyFetch.array_get(var_args.array_get('id'))
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.add_task(var_list_id rt.PhpVal, var_task rt.PhpVal) rt.PhpVal {
	...
	if !(// unsupported expression: Expr_StaticPropertyFetch.array_isset(var_list_id)) {
		return create_automattic_woocommerce_admin_features_onboardingtasks_wp_error(rt.new_string('woocommerce_task_list_invalid_list'), rt.call_function('__', [rt.new_string('Task list ID does not exist'), rt.new_string('woocommerce')]))
	}
	rt.call_method(// unsupported expression: Expr_StaticPropertyFetch.array_get(var_list_id), 'add_task', [var_task_mutated.dup()])
	return rt.new_null()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.maybe_add_extended_tasks(var_extended_tasks rt.PhpVal)  {
	mut var_tasks := if !(var_extended_tasks).is_null() { var_extended_tasks } else { rt.new_array() }
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_task_list := item_1.val
			if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
				continue
			}
			{
				mut iter_2 := var_tasks.iterator()
				for {
					item_2 := iter_2.next() or { break }
					mut var_args := item_2.val
					mut var_task := create_automattic_woocommerce_admin_features_onboardingtasks_deprecatedextendedtask(var_task_list.dup(), var_args.dup())
					rt.call_method(var_task_list, 'add_task', [var_task.dup()])
				}
			}
		}
	}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.get_lists_by_ids(var_ids rt.PhpVal) rt.PhpVal {
	closure_1_fn := fn [var_ids] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_task_list := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.call_function('in_array', [rt.call_method(var_task_list, 'get_list_id', []rt.PhpVal{}), var_ids.dup(), rt.new_bool(true)])
	}
	return rt.call_function('array_filter', [// unsupported expression: Expr_StaticPropertyFetch, rt.new_closure(closure_1_fn)])
}

fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.get_list_ids() rt.PhpVal {
	return rt.func_array_keys(// unsupported expression: Expr_StaticPropertyFetch)
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.get_visible() rt.PhpVal {
	...
	return rt.call_function('array_filter', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.get_lists(), rt.new_closure(closure_2_fn)])
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.get_list(var_id rt.PhpVal) rt.PhpVal {
	if // unsupported expression: Expr_StaticPropertyFetch.array_isset(var_id) {
		return // unsupported expression: Expr_StaticPropertyFetch.array_get(var_id)
	}
	return rt.new_null()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.get_task(var_id rt.PhpVal, var_task_list_id rt.PhpVal) rt.PhpVal {
	mut var_task_list := if rt.is_true(var_task_list_id) { Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.get_list(var_task_list_id.dup()) } else { rt.new_null() }
	if rt.is_true(rt.new_bool(rt.is_true(var_task_list_id) && rt.is_true(rt.new_bool(!(rt.is_true(var_task_list)))))) {
		return rt.new_null()
	}
	closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_all := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	mut var_curr := if args.len > 1 { args[1].dup() } else { rt.new_null() }
	return rt.call_function('array_merge', [var_all.dup(), rt.get_property(var_curr, 'tasks')])
	}
	mut var_tasks_to_search := if rt.is_true(var_task_list) { rt.get_property(var_task_list, 'tasks') } else { rt.call_function('array_reduce', [Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.get_lists(), rt.new_closure(closure_3_fn), rt.new_array()]) }
	{
		mut iter_1 := var_tasks_to_search.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_task := item_1.val
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.setup_tasks_remaining() i64 {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_setup_list)))) || rt.is_true(rt.call_method(var_setup_list, 'is_hidden', []rt.PhpVal{})))) || rt.is_true(rt.call_method(var_setup_list, 'has_previously_completed', []rt.PhpVal{})))) {
		return 0
	}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_completed_tasks.dup().is_array()))))) {
		var_completed_tasks = rt.new_array()
	}
	closure_5_fn := fn [var_completed_tasks] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	closure_4_fn := fn [var_completed_tasks] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_task := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return !(rt.is_true(rt.call_function('in_array', [rt.call_method(var_task, 'get_id', []rt.PhpVal{}), var_completed_tasks.dup(), rt.new_bool(true)])))
	}
	mut var_task := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return !(rt.is_true(rt.call_function('in_array', [rt.call_method(var_task, 'get_id', []rt.PhpVal{}), var_completed_tasks.dup(), rt.new_bool(true)])))
	}
	return rt.call_function('array_filter', [var_viewable_tasks.dup(), rt.new_closure(closure_4_fn)]).array_count()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.menu_task_count()  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_tasks_count)))) || !(var_submenu.array_isset(rt.new_string('woocommerce'))))) {
		return rt.new_null()
	}
	{
		...
		for {
			...
			if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [var_menu_item.array_get(0), rt.call_function('_x', [rt.new_string('Home'), rt.new_string('Admin menu name'), rt.new_string('woocommerce')])]))) {
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.all_hidden() bool {
	...
	return rt.new_bool(var_hidden_lists.dup().array_count() == Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists.get_lists().array_count())
}
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_tasklists() &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists {
	mut obj := &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists{
		...
		lists: rt.new_array()
		default_tasks_loaded: rt.new_bool(false)
	}
	return obj
}

fn create_automattic_woocommerce_admin_features_onboardingtasks_woocommerce_admin_experimental_abtest() &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_WooCommerce_Admin_Experimental_Abtest {
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_datetime() &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_DateTime {
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_datetimezone() &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_DateTimeZone {
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_tasks_reviewshippingoptions() &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_ReviewShippingOptions {
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_{"nodetype":"expr_variable","line":207,"name":"class"}() &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_{"nodeType":"Expr_Variable","line":207,"name":"class"} {
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_tasklist() &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskList {
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_deprecatedextendedtask() &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_DeprecatedExtendedTask {
...
fn (this &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	match prop_name {
		...
		'lists' { return this.lists }
		'default_tasks_loaded' { return this.default_tasks_loaded }
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_TaskLists) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'lists' { this.lists = val; return true }
		'default_tasks_loaded' { this.default_tasks_loaded = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_admin_features_onboardingtasks_tasklists_php() {
}
