...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Stats_Controller) get_datastore_data(var_query_args rt.PhpVal) rt.PhpVal {
	mut var_query := create_automattic_woocommerce_admin_api_reports_orders_stats_query(var_query_args.dup())
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Stats_Controller) prepare_reports_query(var_request rt.PhpVal) rt.PhpVal {
	...
	if !rt.is_true(var_request.array_get('customer_type')) && !(!rt.is_true(var_request.array_get('customer'))) {
		var_args.array_set('customer_type', var_request.array_get('customer'))
	}
	return var_args.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Stats_Controller) get_item_schema() rt.PhpVal {
	...
	var_schema.array_get('properties').array_get('intervals').array_get('items').array_get('properties').array_get('subtotals').array_get('properties').array_unset(rt.new_string('products'))
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Stats_Controller) get_collection_params() rt.PhpVal {
	...
	var_params.array_get_mut('orderby').array_set('enum', this.apply_custom_orderby_filters(rt.create_array([rt.ArrayItem{ key: none, val: 'date' }, rt.ArrayItem{ key: none, val: 'net_revenue' }, rt.ArrayItem{ key: none, val: 'orders_count' }, rt.ArrayItem{ key: none, val: 'avg_order_value' }])))
	...
	var_params.array_set('customer', rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Alias for customer_type (deprecated).'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'enum', val: rt.create_array([rt.ArrayItem{ key: none, val: 'new' }, rt.ArrayItem{ key: none, val: 'returning' }]) }, rt.ArrayItem{ key: 'validate_callback', val: 'rest_validate_request_arg' }]))
	var_params.array_set('customer_type', rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Limit result set to orders that have the specified customer_type'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'enum', val: rt.create_array([rt.ArrayItem{ key: none, val: 'new' }, rt.ArrayItem{ key: none, val: 'returning' }]) }, rt.ArrayItem{ key: 'validate_callback', val: 'rest_validate_request_arg' }]))
...
fn create_automattic_woocommerce_admin_api_reports_orders_stats_controller() &Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Stats_Controller {
...
fn create_automattic_woocommerce_admin_api_reports_orders_stats_query() &Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Stats_Query {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_reports_orders_stats_controller_php() {
