...
fn (mut this Class_Automattic_WooCommerce_Admin_API_OnboardingProducts) create_products(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_products.dup()])) {
		return rt.call_function('rest_ensure_response', [rt.create_array([rt.ArrayItem{ key: 'success', val: false }])])
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_OnboardingProducts) update_item_permissions_check(var_request rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('manage_options')]))))) {
		return (create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_rest_cannot_update'), rt.call_function('__', [rt.new_string('Sorry, you cannot create dummy products.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code', []rt.PhpVal{}) }]))).to_bool()
	}
	return true
}
...
fn create_automattic_woocommerce_admin_api_onboardingproducts() &Class_Automattic_WooCommerce_Admin_API_OnboardingProducts {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_onboardingproducts_php() {
