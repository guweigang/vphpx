...
fn (mut this Class_Automattic_WooCommerce_Admin_API_DataDownloadIPs) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	if var_request.array_isset(rt.new_string('match')) {
		mut var_downloads := rt.call_method(var_wpdb, 'get_results', [rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.new_string('SELECT DISTINCT( user_ip_address ) FROM '), rt.get_property(var_wpdb, 'prefix')), rt.new_string('wc_download_log\n\t\t\t\t\tWHERE user_ip_address LIKE %s\n\t\t\t\t\tLIMIT 10')), (var_request.array_get('match')).str() + '%'])])
	} else {
		return create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_rest_data_download_ips_invalid_request'), rt.call_function('__', [rt.new_string('Invalid request. Please pass the match parameter.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))
	}
	...
	if !(!rt.is_true(var_downloads)) {
		{
			...
			for {
				...
				mut var_response := this.prepare_item_for_response(var_download.dup(), var_request.dup())
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_DataDownloadIPs) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_prepare_data_download_ip'), var_response.dup(), var_item.dup(), var_request.dup()])
}

fn (mut this Class_Automattic_WooCommerce_Admin_API_DataDownloadIPs) prepare_links(var_item rt.PhpVal) rt.PhpVal {
	mut var_links := rt.create_array([rt.ArrayItem{ key: 'collection', val: rt.create_array([rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s'), this.namespace, this.rest_base])]) }]) }])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_DataDownloadIPs) get_collection_params() rt.PhpVal {
	...
	var_params.array_set('match', rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('A partial IP address can be passed and matching results will be returned.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'validate_callback', val: 'rest_validate_request_arg' }]))
	return var_params.dup()
}

fn (mut this Class_Automattic_WooCommerce_Admin_API_DataDownloadIPs) get_item_schema() rt.PhpVal {
	mut var_schema := rt.create_array([rt.ArrayItem{ key: '$schema', val: 'http://json-schema.org/draft-04/schema#' }, rt.ArrayItem{ key: 'title', val: 'data_download_ips' }, rt.ArrayItem{ key: 'type', val: 'object' }, rt.ArrayItem{ key: 'properties', val: rt.create_array([rt.ArrayItem{ key: 'user_ip_address', val: rt.create_array([rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('IP address.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'context', val: rt.create_array([rt.ArrayItem{ key: none, val: 'view' }]) }, rt.ArrayItem{ key: 'readonly', val: true }]) }]) }])
...
fn create_automattic_woocommerce_admin_api_datadownloadips() &Class_Automattic_WooCommerce_Admin_API_DataDownloadIPs {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_datadownloadips_php() {
