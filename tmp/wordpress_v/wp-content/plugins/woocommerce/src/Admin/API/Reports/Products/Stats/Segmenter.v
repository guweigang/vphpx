...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Products_Stats_Segmenter) get_segments(var_type rt.PhpVal, var_query_params rt.PhpVal, var_table_name rt.PhpVal) rt.PhpVal {
	mut var_wpdb := rt.new_null()
	// unsupported statement: Stmt_Global
	if rt.is_true(rt.new_bool(!(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Products_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_isset(rt.new_string('segmentby'))) || rt.is_true(rt.identical(rt.new_string(''), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Products_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get('segmentby'))))) {
		return rt.new_array()
	}
	mut var_product_segmenting_table := rt.new_string((rt.get_property(var_wpdb, 'prefix')).str() + 'wc_order_product_lookup')
	mut var_unique_orders_table := rt.new_string(rt.new_string('uniq_orders'))
	mut var_segmenting_where := rt.new_string(rt.new_string(''))
	if rt.is_true(rt.identical(rt.new_string('product'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Products_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get('segmentby'))) {
	...
	} else if rt.is_true(rt.identical(rt.new_string('variation'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Products_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get('segmentby'))) {
		...
		var_segmenting_selections = rt.create_array([rt.ArrayItem{ key: 'product_level', val: this.prepare_selections(var_product_level_columns.dup()) }])
		this.dispatch_set_prop('report_columns', var_product_level_columns.dup())
		var_segmenting_from = rt.new_string(rt.new_string(''))
		var_segmenting_where = rt.new_string(rt.concat(rt.concat(rt.concat(rt.new_string('AND '), var_product_segmenting_table), rt.new_string('.product_id = ')), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Products_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get('product_includes').array_get(0)))
	...
	} else if rt.is_true(rt.identical(rt.new_string('category'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Products_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get('segmentby'))) {
		var_product_level_columns = this.get_segment_selections_product_level(var_product_segmenting_table.dup())
		var_segmenting_selections = rt.create_array([rt.ArrayItem{ key: 'product_level', val: this.prepare_selections(var_product_level_columns.dup()) }])
		this.dispatch_set_prop('report_columns', var_product_level_columns.dup())
		...
		if rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Products_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_isset(rt.new_string('category_includes')) {
			mut var_category_ids := rt.call_function('implode', [rt.new_string(','), this.get_all_segments()])
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_reports_products_stats_segmenter_php() {
