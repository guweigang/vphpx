...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Segmenter) get_segment_selections_order_level(var_lookup_table rt.PhpVal) rt.PhpVal {
	mut var_columns_mapping := rt.create_array([rt.ArrayItem{ key: 'tax_codes', val: "COUNT(DISTINCT ${var_lookup_table.to_string()}.tax_rate_id) as tax_codes" }, rt.ArrayItem{ key: 'total_tax', val: "SUM(${var_lookup_table.to_string()}.total_tax) AS total_tax" }, rt.ArrayItem{ key: 'order_tax', val: "SUM(${var_lookup_table.to_string()}.order_tax) as order_tax" }, rt.ArrayItem{ key: 'shipping_tax', val: "SUM(${var_lookup_table.to_string()}.shipping_tax) as shipping_tax" }, rt.ArrayItem{ key: 'orders_count', val: "COUNT(DISTINCT ${var_lookup_table.to_string()}.order_id) as orders_count" }])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Segmenter) get_segments(var_type rt.PhpVal, var_query_params rt.PhpVal, var_table_name rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_isset(rt.new_string('segmentby'))) || rt.is_true(rt.identical(rt.new_string(''), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get('segmentby'))))) {
		return rt.new_array()
	}
	...
	if rt.is_true(rt.identical(rt.new_string('tax_rate_id'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get('segmentby'))) {
		mut var_tax_rate_level_columns := this.get_segment_selections_order_level(var_table_name.dup())
		mut var_segmenting_select := this.prepare_selections(var_tax_rate_level_columns.dup())
		this.dispatch_set_prop('report_columns', var_tax_rate_level_columns.dup())
		mut var_segmenting_groupby := rt.new_string((var_table_name).str() + '.tax_rate_id')
		var_segments = this.get_order_related_segments(var_type.dup(), var_segmenting_select.dup(), var_segmenting_from.dup(), var_segmenting_where.dup(), var_segmenting_groupby.dup(), var_table_name.dup(), var_query_params.dup())
	}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_reports_taxes_stats_segmenter_php() {
