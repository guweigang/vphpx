...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Products_Controller) get_datastore_data(var_query_args rt.PhpVal) rt.PhpVal {
	mut var_query := create_automattic_woocommerce_admin_api_reports_genericquery(var_query_args.dup(), rt.new_string('products'))
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Products_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_filtered_response := rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_prepare_report_products'), var_response.dup(), var_report.dup(), var_request.dup()])
	if rt.get_property(var_filtered_response, 'data').array_get('extended_info').array_isset(rt.new_string('name')) {
		rt.get_property(var_filtered_response, 'data').array_get_mut('extended_info').array_set('name', rt.call_function('wp_strip_all_tags', [rt.get_property(var_filtered_response, 'data').array_get('extended_info').array_get('name')]))
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Products_Controller) prepare_links(var_object rt.PhpVal) rt.PhpVal {
	mut var_links := rt.create_array([rt.ArrayItem{ key: 'product', val: rt.create_array([rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s/%d'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Products_Controller', ['Automattic_WooCommerce_Admin_API_Reports_GenericController', 'ExportableInterface'], &this), 'namespace'), rt.new_string('products'), var_object.array_get('product_id')])]) }]) }])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Products_Controller) get_collection_params() rt.PhpVal {
	...
	var_params.array_get_mut('orderby').array_set('enum', this.apply_custom_orderby_filters(rt.create_array([rt.ArrayItem{ key: none, val: 'date' }, rt.ArrayItem{ key: none, val: 'net_revenue' }, rt.ArrayItem{ key: none, val: 'orders_count' }, rt.ArrayItem{ key: none, val: 'items_sold' }, rt.ArrayItem{ key: none, val: 'product_name' }, rt.ArrayItem{ key: none, val: 'variations' }, rt.ArrayItem{ key: none, val: 'sku' }])))
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Products_Controller) get_categories(var_category_ids rt.PhpVal) rt.PhpVal {
	mut var_category_names := rt.call_function('get_terms', [rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: 'product_cat' }, rt.ArrayItem{ key: 'include', val: var_category_ids }, rt.ArrayItem{ key: 'fields', val: 'names' }])])
	return rt.call_function('implode', [rt.new_string(', '), var_category_names.dup()])
}

fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Products_Controller) get_export_columns() rt.PhpVal {
	mut var_export_columns := rt.create_array([rt.ArrayItem{ key: 'product_name', val: rt.call_function('__', [rt.new_string('Product title'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'sku', val: rt.call_function('__', [rt.new_string('SKU'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'items_sold', val: rt.call_function('__', [rt.new_string('Items sold'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'net_revenue', val: rt.call_function('__', [rt.new_string('N. Revenue'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'orders_count', val: rt.call_function('__', [rt.new_string('Orders'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'product_cat', val: rt.call_function('__', [rt.new_string('Category'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'variations', val: rt.call_function('__', [rt.new_string('Variations'), rt.new_string('woocommerce')]) }])
	...
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_report_products_export_columns'), var_export_columns.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Products_Controller) prepare_item_for_export(var_item rt.PhpVal) rt.PhpVal {
	mut var_export_item := rt.create_array([rt.ArrayItem{ key: 'product_name', val: var_item.array_get('extended_info').array_get('name') }, rt.ArrayItem{ key: 'sku', val: var_item.array_get('extended_info').array_get('sku') }, rt.ArrayItem{ key: 'items_sold', val: var_item.array_get('items_sold') }, rt.ArrayItem{ key: 'net_revenue', val: var_item.array_get('net_revenue') }, rt.ArrayItem{ key: 'orders_count', val: var_item.array_get('orders_count') }, rt.ArrayItem{ key: 'product_cat', val: this.get_categories(var_item.array_get('extended_info').array_get('category_ids')) }, rt.ArrayItem{ key: 'variations', val: if var_item.array_get('extended_info').array_isset(rt.new_string('variations')) { var_item.array_get('extended_info').array_get('variations').array_count() } else { 0 } }])
	if rt.is_true(rt.identical(rt.new_string('yes'), rt.call_function('get_option', [rt.new_string('woocommerce_manage_stock')]))) {
		if rt.is_true(var_item.array_get('extended_info').array_get('manage_stock')) {
			...
			var_export_item.array_set('stock_status', rt.call_function('__', [rt.new_string('N/A'), rt.new_string('woocommerce')]))
			var_export_item.array_set('stock', rt.call_function('__', [rt.new_string('N/A'), rt.new_string('woocommerce')]))
		}
	}
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_report_products_prepare_export_item'), var_export_item.dup(), var_item.dup()])
}
...
fn create_automattic_woocommerce_admin_api_reports_products_controller() &Class_Automattic_WooCommerce_Admin_API_Reports_Products_Controller {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_reports_products_controller_php() {
