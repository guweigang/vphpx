...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Notice) register_routes()  {
	rt.call_function('register_rest_route', [this.namespace, '/' + (this.rest_base).str() + '/dismiss', rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: 'POST' }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_Notice', ['Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'dissmiss_notice' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_Notice', ['Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_permission' }]) }]) }])])
}

fn (mut this Class_Automattic_WooCommerce_Admin_API_Notice) dissmiss_notice(var_request rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(var_request.array_isset(rt.new_string('dismiss_notice_nonce'))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_verify_nonce', [var_request.array_get('dismiss_notice_nonce'), rt.new_string('dismiss_notice')]))))))) {
		return create_automattic_woocommerce_admin_api_wp_error(rt.new_string('unauthorized'), rt.new_string('Invalid nonce.'), rt.create_array([rt.ArrayItem{ key: 'status', val: 401 }]))
	}
	mut var_notice_id := if var_request.array_isset(rt.new_string('notice_id')) { rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [var_request.array_get('notice_id')])]) } else { rt.new_string('') }
	mut var_dismissed := rt.new_bool(rt.new_bool(false))
	mut switch_val_1 := var_notice_id
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('woo-subscription-expired-notice'))) {
		rt.call_function('update_user_meta', [rt.call_function('get_current_user_id', []rt.PhpVal{}), Class_Automattic_WooCommerce_Admin_PluginsHelper.dismiss_expired_subs_notice(), rt.call_function('time', []rt.PhpVal{})])
		var_dismissed = rt.new_bool(rt.new_bool(true))
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('woo-subscription-expiring-notice'))) {
		rt.call_function('update_user_meta', [rt.call_function('get_current_user_id', []rt.PhpVal{}), Class_Automattic_WooCommerce_Admin_PluginsHelper.dismiss_expiring_subs_notice(), rt.call_function('time', []rt.PhpVal{})])
		var_dismissed = rt.new_bool(rt.new_bool(true))
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('woo-disconnect-notice'))) {
		rt.call_function('update_user_meta', [rt.call_function('get_current_user_id', []rt.PhpVal{}), Class_Automattic_WooCommerce_Admin_PluginsHelper.dismiss_disconnect_notice(), rt.call_function('time', []rt.PhpVal{})])
		var_dismissed = rt.new_bool(rt.new_bool(true))
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('woo-connect-notice'))) {
		rt.call_function('update_user_meta', [rt.call_function('get_current_user_id', []rt.PhpVal{}), Class_Automattic_WooCommerce_Admin_PluginsHelper.dismiss_connect_notice(), rt.call_function('time', []rt.PhpVal{})])
		var_dismissed = rt.new_bool(rt.new_bool(true))
	}
	return rt.call_function('rest_ensure_response', [rt.create_array([rt.ArrayItem{ key: 'success', val: var_dismissed }])])
}
...
fn create_automattic_woocommerce_admin_api_notice() &Class_Automattic_WooCommerce_Admin_API_Notice {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_notice_php() {
