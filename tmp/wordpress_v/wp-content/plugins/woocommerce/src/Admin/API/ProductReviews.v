...
fn (mut this Class_Automattic_WooCommerce_Admin_API_ProductReviews) prepare_links(var_review rt.PhpVal) rt.PhpVal {
	mut var_links := rt.create_array([rt.ArrayItem{ key: 'self', val: rt.create_array([rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s/%d'), this.namespace, rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_ProductReviews', ['Automattic_WooCommerce_Admin_API_WC_REST_Product_Reviews_Controller'], &this), 'rest_base'), rt.get_property(var_review, 'comment_ID')])]) }]) }, rt.ArrayItem{ key: 'collection', val: rt.create_array([rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s'), this.namespace, rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_ProductReviews', ['Automattic_WooCommerce_Admin_API_WC_REST_Product_Reviews_Controller'], &this), 'rest_base')])]) }]) }])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		var_links.array_set('up', rt.create_array([rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/products/%d'), this.namespace, rt.get_property(var_review, 'comment_post_ID')])]) }, rt.ArrayItem{ key: 'embeddable', val: true }]))
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		var_links.array_set('reviewer', rt.create_array([rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', ['wp/v2/users/' + (rt.get_property(var_review, 'user_id')).str()]) }, rt.ArrayItem{ key: 'embeddable', val: true }]))
	}
...
fn create_automattic_woocommerce_admin_api_productreviews() &Class_Automattic_WooCommerce_Admin_API_ProductReviews {
...
fn create_automattic_woocommerce_admin_api_wc_rest_product_reviews_controller() &Class_Automattic_WooCommerce_Admin_API_WC_REST_Product_Reviews_Controller {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_productreviews_php() {
