...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_GenericController) add_pagination_headers(var_request rt.PhpVal, var_response rt.PhpVal, total i64, page i64, max_pages i64) rt.PhpVal {
	...
	var_response_mutated = rt.call_function('rest_ensure_response', [var_response_mutated.dup()])
	rt.call_method(var_response_mutated, 'header', [rt.new_string('X-WP-Total'), rt.new_int(total)])
	rt.call_method(var_response_mutated, 'header', [rt.new_string('X-WP-TotalPages'), rt.new_int(max_pages)])
	mut var_base := rt.call_function('add_query_arg', [rt.call_method(var_request, 'get_query_params', []rt.PhpVal{}), rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s'), this.namespace, rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_GenericController', ['Automattic_WooCommerce_Admin_API_Reports_WC_REST_Reports_Controller'], &this), 'rest_base')])])])
	if page > 1 {
		mut var_prev_page := rt.new_int(page - 1)
		if rt.is_true(rt.greater(var_prev_page, rt.new_int(max_pages))) {
			var_prev_page = rt.new_int(rt.new_int(max_pages))
		}
		mut var_prev_link := rt.call_function('add_query_arg', [rt.new_string('page'), var_prev_page.dup(), var_base.dup()])
		rt.call_method(var_response_mutated, 'link_header', [rt.new_string('prev'), var_prev_link.dup()])
	}
	if max_pages > page {
		...
		rt.call_method(var_response_mutated, 'link_header', [rt.new_string('next'), var_next_link.dup()])
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_GenericController) get_datastore_data(var_query_args rt.PhpVal) rt.PhpVal {
	...
	mut var_data_store := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_API_Reports_WC_Data_Store{}; return temp.load(arg_0) }(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_GenericController', ['Automattic_WooCommerce_Admin_API_Reports_WC_REST_Reports_Controller'], &this), 'rest_base'))
	return rt.call_method(var_data_store, 'get_data', [var_query_args_mutated.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_GenericController) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_report_data.dup()])) {
		return var_report_data.dup()
	}
	if !(!(rt.get_property(var_report_data, 'data')).is_null()) || !(!(rt.get_property(var_report_data, 'page_no')).is_null()) || !(!(rt.get_property(var_report_data, 'pages')).is_null()) {
		return create_automattic_woocommerce_admin_api_reports_wp_error(rt.new_string('woocommerce_rest_reports_invalid_response'), rt.call_function('__', [rt.new_string('Invalid response from data store.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 500 }]))
	}
	mut var_out_data := rt.new_array()
	{
		...
		for {
			...
			mut var_item := this.prepare_item_for_response(var_datum.dup(), var_request.dup())
			var_out_data.array_push(this.prepare_response_for_collection(var_item.dup()))
		}
	}
	return this.add_pagination_headers(var_request.dup(), var_out_data.dup(), (// unsupported expression: Expr_Cast_Int).to_i64(), (// unsupported expression: Expr_Cast_Int).to_i64(), (// unsupported expression: Expr_Cast_Int).to_i64())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_GenericController) prepare_reports_query(var_request rt.PhpVal) rt.PhpVal {
	mut var_args := rt.call_function('wp_parse_args', [rt.call_function('array_intersect_key', [rt.call_method(var_request, 'get_query_params', []rt.PhpVal{}), this.get_collection_params()]), rt.call_method(var_request, 'get_default_params', []rt.PhpVal{})])
	return var_args.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_GenericController) apply_custom_orderby_filters(var_orderby_enum rt.PhpVal) rt.PhpVal {
	return rt.call_function('apply_filters', [rt.concat(rt.new_string('woocommerce_analytics_orderby_enum_'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_GenericController', ['Automattic_WooCommerce_Admin_API_Reports_WC_REST_Reports_Controller'], &this), 'rest_base')), var_orderby_enum.dup()])
}
...
fn create_automattic_woocommerce_admin_api_reports_wc_rest_reports_controller() &Class_Automattic_WooCommerce_Admin_API_Reports_WC_REST_Reports_Controller {
...
fn create_automattic_woocommerce_admin_api_reports_wc_data_store() &Class_Automattic_WooCommerce_Admin_API_Reports_WC_Data_Store {
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_GenericController) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'add_pagination_headers' {
			...
			return this.add_pagination_headers(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2, dispatch_arg_3, dispatch_arg_4)
		}
		'get_datastore_data' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_reports_genericcontroller_php() {
