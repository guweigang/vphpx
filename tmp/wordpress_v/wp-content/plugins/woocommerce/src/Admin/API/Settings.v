...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Settings) register_routes()  {
	rt.call_function('register_rest_route', [this.namespace, '/' + (this.rest_base).str(), rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: Class_Automattic_WooCommerce_Admin_API_WP_REST_Server.editable() }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_Settings', ['Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'save_settings' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_Settings', ['Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'save_items_permissions_check' }]) }, rt.ArrayItem{ key: 'args', val: rt.create_array([rt.ArrayItem{ key: 'schema', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_Settings', ['Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'save_items_schema' }]) }]) }]) }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Settings) save_settings(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('check_ajax_referer', [rt.new_string('wp_rest'), rt.new_bool(false), rt.new_bool(false)]))))) {
		return create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_settings_invalid_nonce'), rt.call_function('__', [rt.new_string('Invalid nonce.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 403 }]))
	}
	mut var_params := rt.call_method(var_request, 'get_params', []rt.PhpVal{})
	mut var_current_tab := if !rt.is_true(var_params.array_get('tab')) { rt.new_string('general') } else { rt.call_function('sanitize_title', [rt.call_function('wp_unslash', [var_params.array_get('tab')])]) }
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_current_section := if !rt.is_true(var_params.array_get('section')) { rt.new_string('') } else { rt.call_function('sanitize_title', [rt.call_function('wp_unslash', [var_params.array_get('section')])]) }
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_filter_name := rt.new_string(if rt.is_true(rt.identical(rt.new_string(''), var_current_section)) { rt.new_string("woocommerce_save_settings_${var_current_tab.to_string()}") } else { rt.new_string("woocommerce_save_settings_${var_current_tab.to_string()}_${var_current_section.to_string()}") })
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	if rt.is_true(rt.call_function('apply_filters', [var_filter_name.dup(), rt.new_bool(!(!rt.is_true(rt.get_superglobal('_POST').array_get('save'))))])) {
		fn () rt.PhpVal { mut temp := Class_WC_Admin_Settings{}; return temp.save() }()
		if rt.has_exception() { unsafe { goto catch_label_1 } }
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_setting_pages := fn () rt.PhpVal { mut temp := Class_WC_Admin_Settings{}; return temp.get_settings_pages() }()
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	{
		...
		for {
			...
			mut var_class_name := rt.call_function('get_class', [var_setting_page.dup()])
			if rt.has_exception() { unsafe { goto catch_label_1 } }
			var_setting_pages.array_set(var_key, rt.create_object_dynamically(var_class_name, []rt.PhpVal{}))
			if rt.has_exception() { unsafe { goto catch_label_1 } }
		}
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_data := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_Settings_Init{}; return temp.get_page_data(arg_0, arg_1) }(rt.new_array(), var_setting_pages.dup())
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	return create_automattic_woocommerce_admin_api_wp_rest_response(rt.create_array([rt.ArrayItem{ key: 'status', val: 'success' }, rt.ArrayItem{ key: 'data', val: var_data }]))
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Admin_API_Exception') {
		...
		return create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_settings_save_error'), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Failed to save settings: %s'), rt.new_string('woocommerce')]), rt.call_method(var_e, 'getMessage', []rt.PhpVal{})]), rt.create_array([rt.ArrayItem{ key: 'status', val: 500 }]))
		unsafe { goto end_label_1 }
	}
	else {
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Settings) save_items_schema() rt.PhpVal {
	mut var_schema := rt.create_array([rt.ArrayItem{ key: '$schema', val: 'http://json-schema.org/draft-04/schema#' }, rt.ArrayItem{ key: 'title', val: 'options' }, rt.ArrayItem{ key: 'type', val: 'object' }, rt.ArrayItem{ key: 'properties', val: rt.create_array([rt.ArrayItem{ key: 'options', val: rt.create_array([rt.ArrayItem{ key: 'type', val: 'array' }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Array of options with associated values.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'context', val: rt.create_array([rt.ArrayItem{ key: none, val: 'view' }]) }, rt.ArrayItem{ key: 'readonly', val: true }]) }, rt.ArrayItem{ key: 'tab', val: rt.create_array([rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Settings tab.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'context', val: rt.create_array([rt.ArrayItem{ key: none, val: 'view' }, rt.ArrayItem{ key: none, val: 'edit' }]) }, rt.ArrayItem{ key: 'default', val: 'general' }]) }, rt.ArrayItem{ key: 'section', val: rt.create_array([rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Settings section.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'context', val: rt.create_array([rt.ArrayItem{ key: none, val: 'view' }, rt.ArrayItem{ key: none, val: 'edit' }]) }, rt.ArrayItem{ key: 'default', val: '' }]) }]) }])
...
fn create_automattic_woocommerce_admin_api_settings() &Class_Automattic_WooCommerce_Admin_API_Settings {
...
fn init_registry() {
	rt.register_class_factory('Automattic_WooCommerce_Admin_API_Settings', fn(args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_object('Automattic_WooCommerce_Admin_API_Settings', ['Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller'], obj)
	})
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_settings_php() {
