...
fn (mut this Class_Automattic_WooCommerce_Admin_API_OnboardingFreeExtensions) register_routes()  {
	rt.call_function('register_rest_route', [this.namespace, '/' + (this.rest_base).str(), rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.readable() }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_OnboardingFreeExtensions', ['WC_REST_Data_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_available_extensions' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_OnboardingFreeExtensions', ['WC_REST_Data_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_items_permissions_check' }]) }]) }, rt.ArrayItem{ key: 'schema', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_OnboardingFreeExtensions', ['WC_REST_Data_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'get_public_item_schema' }]) }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_OnboardingFreeExtensions) get_available_extensions(var_request rt.PhpVal) rt.PhpVal {
	mut var_extensions := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_RemoteFreeExtensions_Init{}; return temp.get_extensions() }()
	if rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('apply_filters', [rt.new_string('woocommerce_suggest_jetpack'), rt.new_bool(true)]))) {
		{
	...
	return // unsupported expression: Expr_BinaryOp_NotIdentical
				...
				var_extension.array_set('plugins', rt.call_function('array_filter', [var_extension.array_get('plugins'), rt.new_closure(closure_1_fn)]))
			}
		}
	}
	return create_wp_rest_response(var_extensions.dup())
}
...
fn create_automattic_woocommerce_admin_api_onboardingfreeextensions() &Class_Automattic_WooCommerce_Admin_API_OnboardingFreeExtensions {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_api_onboardingfreeextensions_php() {
