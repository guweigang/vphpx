...
fn (mut this Class_Automattic_WooCommerce_Admin_ReportCSVExporter) construct(type bool, var_args rt.PhpVal) {
	...
	if !(!rt.is_true(var_args)) {
		this.set_report_args(var_args.dup())
	}
}
	...
	])
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('file_exists', [
				rt.call_function('trailingslashit', [var_file.array_get('base')]),
			])))))
			{
				rt.call_function('wp_mkdir_p', [var_file.array_get('base')])
			}
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('file_exists', [
				rt.concat(rt.call_function('trailingslashit', [
					var_file.array_get('base')]), var_file.array_get('file')),
			])))))
			{
				...
				if rt.is_true(var_file_handle) {
					rt.call_function('fwrite', [var_file_handle.dup(),
						var_file.array_get('content')])
					rt.call_function('fclose', [var_file_handle.dup()])
...
fn Class_Automattic_WooCommerce_Admin_ReportCSVExporter.get_reports_directory() string {
	...
	return (rt.call_function('trailingslashit', [var_upload_dir.array_get('basedir')])).str() +
...
fn (mut this Class_Automattic_WooCommerce_Admin_ReportCSVExporter) set_report_args(var_args rt.PhpVal) {
	mut var_report_args := rt.call_function('array_merge', [var_args.dup(),
	...
	if var_report_args.array_isset(rt.new_string('page')) {
		this.set_page(var_report_args.array_get('page'))
	}
	this.report_args = var_report_args.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_ReportCSVExporter) get_report_columns() rt.PhpVal {
	...
	if var_report_schema.array_isset(rt.new_string('properties')) {
		{
			mut iter_1 := var_report_schema.array_get('properties').iterator()
			for {
				item_1 := iter_1.next() or { break }
				...
				if rt.is_true(rt.identical(rt.new_string('extended_info'), var_column_name)) {
					mut var_extended_info := rt.call_function('array_diff', [
						rt.func_array_keys(var_column_info.dup()),
						rt.create_array([rt.ArrayItem{ key: none, val: 'image' }]),
					])
					var_report_columns = rt.call_function('array_merge', [
						var_report_columns.dup(), var_extended_info.dup()])
				} else {
					var_report_columns.array_push(var_column_name.dup())
				}
			}
		}
	}
	return var_report_columns.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_ReportCSVExporter) prepare_data_to_export() {
	...
	var_request.set_default_params(var_defaults.dup())
	...
	mut var_report_data := rt.call_method(var_rest_server, 'response_to_data', [
		var_response.dup(), rt.new_bool(true)])
	...
	this.dispatch_set_prop('total_rows', var_report_meta.array_get('X-WP-Total'))
...
fn (mut this Class_Automattic_WooCommerce_Admin_ReportCSVExporter) get_raw_row_data(var_item rt.PhpVal) rt.PhpVal {
	...
	if var_item_mutated.array_isset(rt.new_string('extended_info')) {
		mut var_extended_info := rt.cast_array(var_item_mutated.array_get('extended_info'))
		var_item_mutated.array_unset(rt.new_string('extended_info'))
		var_item_mutated = rt.call_function('array_merge', [var_item_mutated.dup(),
			var_extended_info.dup()])
	}
	{
...
fn (mut this Class_Automattic_WooCommerce_Admin_ReportCSVExporter) generate_row_data(var_item rt.PhpVal) rt.PhpVal {
		...
		'Automattic_WooCommerce_Admin_API_Reports_ExportableInterface')))
	{
		...
		var_row = this.get_raw_row_data(var_item_mutated.dup())
	}
...
fn create_automattic_woocommerce_admin_wc_csv_batch_exporter() &Class_Automattic_WooCommerce_Admin_WC_CSV_Batch_Exporter {
...
fn create_automattic_woocommerce_admin_wp_rest_request() &Class_Automattic_WooCommerce_Admin_WP_REST_Request {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_reportcsvexporter_php() {
