...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_EvaluateOverrides) evaluate(mut var_spec Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_array, mut var_context Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_array) rt.PhpVal {
	mut var_rule_evaluator := create_automattic_woocommerce_admin_remotespecs_ruleprocessors_ruleevaluator(create_automattic_woocommerce_admin_remotespecs_ruleprocessors_getruleprocessorforcontext(var_context.dup()))
	{
		mut iter_1 := var_spec.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_spec_item := item_1.val
			if rt.is_true(rt.new_bool(!(rt.get_property(var_spec_item, 'overrides')).is_null() && rt.is_true(rt.new_bool(rt.get_property(var_spec_item, 'overrides').is_array())))) {
				{
					mut iter_2 := rt.get_property(var_spec_item, 'overrides').iterator()
					for {
						item_2 := iter_2.next() or { break }
						mut var_override := item_2.val
						if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(!(rt.get_property(var_override, 'rules')).is_null()) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_property(var_override, 'rules').is_array()))))))) || !(!(rt.get_property(var_override, 'field')).is_null()))) || !(!(rt.get_property(var_override, 'value')).is_null()))) {
							continue
						}
						if rt.is_true(var_rule_evaluator.evaluate(rt.get_property(var_override, 'rules'))) {
							if !(rt.get_property(var_spec_item, '{"nodeType":"Expr_PropertyFetch","line":31,"var":{"nodeType":"Expr_Variable","line":31,"name":"override"},"name":"field"}')).is_null() {
								...
								this.set_value_with_dot_notation(var_spec_item.dup(), rt.get_property(var_override, 'field'), rt.get_property(var_override, 'value'))
							}
						}
					}
				}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_EvaluateOverrides) set_value_with_dot_notation(var_data rt.PhpVal, var_path rt.PhpVal, var_new_value rt.PhpVal) rt.PhpVal {
	...
	mut var_keys := rt.call_function('explode', [rt.new_string('.'), var_path.dup()])
	mut var_last_key := rt.call_function('array_pop', [var_keys.dup()])
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(var_key.dup().is_long() || var_key.dup().is_double())) {
				var_key = // unsupported expression: Expr_Cast_Int
				if rt.is_true(rt.new_bool(!(var_data_mutated.array_isset(var_key)) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_data_mutated.array_get(var_key).is_object()))))))) {
				...
				if rt.is_true(rt.new_bool(!(!(rt.get_property(var_data_mutated, '{"nodeType":"Expr_Variable","line":68,"name":"key"}')).is_null()) || rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_property(var_data_mutated, '{"nodeType":"Expr_Variable","line":68,"name":"key"}').is_array()))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_property(var_data_mutated, '{"nodeType":"Expr_Variable","line":68,"name":"key"}').is_object()))))))))) {
	...
	if rt.is_true(rt.new_bool(var_last_key.dup().is_long() || var_last_key.dup().is_double())) {
		var_data_mutated.array_set(// unsupported expression: Expr_Cast_Int, var_new_value.dup())
	} else {
		rt.set_property(var_data_mutated, '{"nodeType":"Expr_Variable","line":79,"name":"last_key"}', var_new_value.dup())
	}
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_getruleprocessorforcontext() &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_GetRuleProcessorForContext {
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_stdclass() &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_stdClass {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_evaluateoverrides_php() {
