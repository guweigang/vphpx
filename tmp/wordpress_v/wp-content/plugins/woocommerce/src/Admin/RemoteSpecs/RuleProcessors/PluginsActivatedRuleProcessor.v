...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_PluginsActivatedRuleProcessor) process(var_rule rt.PhpVal, var_stored_state rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_countable', [rt.get_property(var_rule, 'plugins')]))))) || 0 == rt.get_property(var_rule, 'plugins').array_count())) {
		return false
	}
	mut var_active_plugin_slugs := rt.call_method(this.plugins_provider, 'get_active_plugin_slugs', []rt.PhpVal{})
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_plugin_slug.dup().is_string()))))) {
				mut var_logger := rt.call_function('wc_get_logger', []rt.PhpVal{})
				rt.call_method(var_logger, 'warning', [rt.call_function('__', [rt.new_string('Invalid plugin slug provided in the plugins activated rule.'), rt.new_string('woocommerce')])])
				return false
			}
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_plugin_slug.dup(), var_active_plugin_slugs.dup(), rt.new_bool(true)]))))) {
				return false
			}
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_PluginsActivatedRuleProcessor) validate(var_rule rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(!(!(rt.get_property(var_rule, 'plugins')).is_null()) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_property(var_rule, 'plugins').is_array()))))))) {
		return false
	}
	return true
}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_pluginsactivatedruleprocessor_php() {
