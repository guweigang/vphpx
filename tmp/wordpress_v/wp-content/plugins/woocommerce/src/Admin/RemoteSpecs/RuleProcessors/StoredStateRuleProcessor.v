...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateRuleProcessor) process(var_rule rt.PhpVal, var_stored_state rt.PhpVal) bool {
	if !(!(rt.get_property(var_stored_state, '{"nodeType":"Expr_PropertyFetch","line":25,"var":{"nodeType":"Expr_Variable","line":25,"name":"rule"},"name":"index"}')).is_null()) {
		return false
	}
	return (fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_ComparisonOperation{}; return temp.compare(arg_0, arg_1, arg_2) }(rt.get_property(var_stored_state, '{"nodeType":"Expr_PropertyFetch","line":30,"var":{"nodeType":"Expr_Variable","line":30,"name":"rule"},"name":"index"}'), rt.get_property(var_rule, 'value'), rt.get_property(var_rule, 'operation'))).to_bool()
}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_storedstateruleprocessor_php() {
