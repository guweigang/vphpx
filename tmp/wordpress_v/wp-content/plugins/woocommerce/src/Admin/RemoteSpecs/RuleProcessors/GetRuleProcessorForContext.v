...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_GetRuleProcessorForContext) get_processor(var_rule_type rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('context_plugins'))) {
		return create_automattic_woocommerce_admin_remotespecs_ruleprocessors_contextpluginsruleprocessor(if !(this.context.array_get('plugins')).is_null() {
			this.context.array_get('plugins')
		} else {
			rt.new_array()
		})
	}
	return fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_GetRuleProcessor{}
		return temp.get_processor(arg_0)
	}(var_rule_type.dup())
}
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_contextpluginsruleprocessor() &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_ContextPluginsRuleProcessor {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_getruleprocessorforcontext_php() {
