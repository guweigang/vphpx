...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_WCAdminActiveForRuleProcessor) construct(var_wcadmin_active_for_provider rt.PhpVal)  {
	this.wcadmin_active_for_provider = if rt.is_true(rt.identical(rt.new_null(), var_wcadmin_active_for_provider)) { create_automattic_woocommerce_admin_remotespecs_ruleprocessors_wcadminactiveforprovider() } else { var_wcadmin_active_for_provider }
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_WCAdminActiveForRuleProcessor) process(var_rule rt.PhpVal, var_stored_state rt.PhpVal) bool {
	mut var_active_for_seconds := rt.call_method(this.wcadmin_active_for_provider, 'get_wcadmin_active_for_in_seconds', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_active_for_seconds)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_active_for_seconds.dup().is_long() || var_active_for_seconds.dup().is_double()))))))) || rt.is_true(rt.less(var_active_for_seconds, rt.new_int(0))))) {
		return false
	}
	mut var_rule_seconds := rt.mul(rt.get_property(var_rule, 'days'), rt.get_constant('DAY_IN_SECONDS'))
	return (fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_ComparisonOperation{}; return temp.compare(arg_0, arg_1, arg_2) }(var_active_for_seconds.dup(), var_rule_seconds.dup(), rt.get_property(var_rule, 'operation'))).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_WCAdminActiveForRuleProcessor) validate(var_rule rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(!(rt.get_property(var_rule, 'days')).is_null()) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_property(var_rule, 'days').is_long() || rt.get_property(var_rule, 'days').is_double()))))))) || rt.is_true(rt.less(rt.get_property(var_rule, 'days'), rt.new_int(0))))) {
		return false
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_WCAdminActiveForRuleProcessor) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'wcadmin_active_for_provider' { this.wcadmin_active_for_provider = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_wcadminactiveforruleprocessor_php() {
