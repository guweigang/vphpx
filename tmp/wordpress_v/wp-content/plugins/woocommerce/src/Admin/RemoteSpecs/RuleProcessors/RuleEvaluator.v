...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_RuleEvaluator) construct(var_get_rule_processor rt.PhpVal)  {
	this.get_rule_processor = if rt.is_true(rt.identical(rt.new_null(), var_get_rule_processor)) { create_automattic_woocommerce_admin_remotespecs_ruleprocessors_getruleprocessor() } else { var_get_rule_processor }
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_RuleEvaluator) evaluate(var_rules rt.PhpVal, var_stored_state rt.PhpVal, var_logger_args rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(var_rules_mutated.dup().is_bool())) {
		return (var_rules_mutated).to_bool()
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_rules_mutated.dup().is_array()))))) {
		var_rules_mutated = rt.create_array([rt.ArrayItem{ key: none, val: var_rules_mutated }])
	}
	if 0 == var_rules_mutated.dup().array_count() {
		return false
	}
	mut var_evaluation_logger := rt.new_null()
	if rt.is_true(rt.new_int(var_logger_args.dup().array_count())) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_logger_args.dup().array_isset(rt.new_string('slug'))))))) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_InvalidArgumentException', []string{}, create_automattic_woocommerce_admin_remotespecs_ruleprocessors_invalidargumentexception(rt.new_string('Missing required field: slug in $logger_args.'))))
		}
		mut var_source := if var_logger_args.array_isset(rt.new_string('source')) { var_logger_args.array_get('source') } else { rt.new_null() }
		var_evaluation_logger = create_automattic_woocommerce_admin_remotespecs_ruleprocessors_evaluationlogger(var_logger_args.array_get('slug'), var_source.dup())
	}
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_rule.dup().is_object()))))) {
				rt.new_bool(rt.is_true(var_evaluation_logger) && rt.is_true(rt.call_method(var_evaluation_logger, 'add_result', [rt.new_string('rule not an object'), rt.new_bool(false)])))
				rt.new_bool(rt.is_true(var_evaluation_logger) && rt.is_true(rt.call_method(var_evaluation_logger, 'log', []rt.PhpVal{})))
				return false
			}
			mut var_processor := rt.call_method(this.get_rule_processor, 'get_processor', [rt.get_property(var_rule, 'type')])
			mut var_processor_result := rt.call_method(var_processor, 'process', [var_rule.dup(), var_stored_state.dup()])
			rt.new_bool(rt.is_true(var_evaluation_logger) && rt.is_true(rt.call_method(var_evaluation_logger, 'add_result', [rt.get_property(var_rule, 'type'), var_processor_result.dup()])))
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_invalidargumentexception() &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_InvalidArgumentException {
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_evaluationlogger() &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_EvaluationLogger {
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_RuleEvaluator) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'get_rule_processor' { this.get_rule_processor = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_ruleevaluator_php() {
