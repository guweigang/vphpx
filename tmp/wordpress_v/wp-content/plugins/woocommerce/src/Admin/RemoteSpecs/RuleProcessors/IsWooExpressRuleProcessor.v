...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_IsWooExpressRuleProcessor) process(var_rule rt.PhpVal, var_stored_state rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('wc_calypso_bridge_is_woo_express_plan')]))))) {
		return (rt.identical(rt.new_bool(false), rt.get_property(var_rule, 'value'))).to_bool()
	}
	if !(!(rt.get_property(var_rule, 'plan')).is_null()) {
		return (rt.identical(rt.call_function('wc_calypso_bridge_is_woo_express_plan', []rt.PhpVal{}), rt.get_property(var_rule, 'value'))).to_bool()
	}
	if rt.is_true(rt.call_function('wc_calypso_bridge_is_woo_express_plan', []rt.PhpVal{})) {
		mut var_fn := rt.new_string('wc_calypso_bridge_is_woo_express_' + (// unsupported expression: Expr_Cast_String).str() + '_plan')
		if rt.is_true(rt.call_function('function_exists', [var_fn.dup()])) {
			return (rt.identical(rt.call_callable(var_fn, []rt.PhpVal{}), rt.get_property(var_rule, 'value'))).to_bool()
		}
		return (rt.identical(rt.new_bool(false), rt.get_property(var_rule, 'value'))).to_bool()
	}
	return false
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_iswooexpressruleprocessor_php() {
