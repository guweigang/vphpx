...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_EvaluationLogger) log()  {
	mut var_should_log := rt.new_bool(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('WC_ADMIN_DEBUG_RULE_EVALUATOR')])) && rt.is_true(rt.identical(rt.new_bool(true), rt.call_function('constant', [rt.new_string('WC_ADMIN_DEBUG_RULE_EVALUATOR')])))))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('apply_filters', [rt.new_string('woocommerce_admin_remote_specs_evaluator_should_log'), var_should_log.dup()]))))) {
		return rt.new_null()
	}
	{
		...
		for {
			...
			rt.call_method(this.logger, 'debug', [rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('['), this.slug), rt.new_string('] ')), var_result.array_get('rule')), rt.new_string(': ')), var_result.array_get('result')), rt.create_array([rt.ArrayItem{ key: 'source', val: this.source }])])
		}
	}
}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_evaluationlogger_php() {
}
