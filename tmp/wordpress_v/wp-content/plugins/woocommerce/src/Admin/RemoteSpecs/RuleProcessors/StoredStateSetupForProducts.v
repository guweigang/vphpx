...
fn Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateSetupForProducts.admin_init()  {
	rt.call_function('add_action', [rt.new_string('product_page_product_importer'), rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT }, rt.ArrayItem{ key: none, val: 'run_on_product_importer' }])])
	rt.call_function('add_action', [rt.new_string('transition_post_status'), rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT }, rt.ArrayItem{ key: none, val: 'run_on_transition_post_status' }]), rt.new_int(10), rt.new_int(3)])
}

fn Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateSetupForProducts.init()  {
	rt.call_function('add_action', [Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateSetupForProducts.async_run_remote_notifications_action_name(), rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT }, rt.ArrayItem{ key: none, val: 'run_remote_notifications' }])])
}
...
fn Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateSetupForProducts.init_stored_state(var_stored_state rt.PhpVal) rt.PhpVal {
	...
	rt.set_property(var_stored_state_mutated, 'there_were_no_products', rt.new_bool(!(rt.is_true(Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateSetupForProducts.are_there_products()))))
	rt.set_property(var_stored_state_mutated, 'there_are_now_products', rt.new_bool(!(rt.is_true(rt.get_property(var_stored_state_mutated, 'there_were_no_products')))))
	return var_stored_state_mutated.dup()
}
...
fn Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateSetupForProducts.are_there_products() rt.PhpVal {
	mut var_query := create_automattic_woocommerce_admin_remotespecs_ruleprocessors_wc_product_query(rt.create_array([rt.ArrayItem{ key: 'limit', val: 1 }, rt.ArrayItem{ key: 'paginate', val: true }, rt.ArrayItem{ key: 'return', val: 'ids' }, rt.ArrayItem{ key: 'status', val: rt.create_array([rt.ArrayItem{ key: none, val: 'publish' }]) }]))
...
fn Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateSetupForProducts.update_stored_state_and_possibly_run_remote_notifications()  {
	mut var_stored_state := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_RemoteInboxNotifications_RemoteInboxNotificationsEngine{}; return temp.get_stored_state() }()
	if rt.is_true(rt.identical(rt.new_bool(true), rt.get_property(var_stored_state, 'there_are_now_products'))) {
		return rt.new_null()
	}
	rt.set_property(var_stored_state, 'there_are_now_products', rt.new_bool(true))
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_RemoteInboxNotifications_RemoteInboxNotificationsEngine{}; return temp.update_stored_state(arg_0) }(var_stored_state.dup())
	rt.call_function('as_enqueue_async_action', [Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateSetupForProducts.async_run_remote_notifications_action_name()])
}
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_wc_product_query() &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_WC_Product_Query {
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateSetupForProducts) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'run_on_transition_post_status' {
			...
			Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_StoredStateSetupForProducts.run_on_transition_post_status(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2)
			return rt.new_null()
		}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_storedstatesetupforproducts_php() {
