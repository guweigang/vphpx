...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_OnboardingProfileRuleProcessor) process(var_rule rt.PhpVal, var_stored_state rt.PhpVal) bool {
	mut var_onboarding_profile := rt.call_function('get_option', [rt.new_string('woocommerce_onboarding_profile')])
	if rt.is_true(rt.new_bool(!rt.is_true(var_onboarding_profile) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_onboarding_profile.dup().is_array()))))))) {
		return false
	}
	if !(var_onboarding_profile.array_isset(rt.get_property(var_rule, 'index'))) {
		return false
	}
	return (fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_ComparisonOperation{}; return temp.compare(arg_0, arg_1, arg_2) }(var_onboarding_profile.array_get(rt.get_property(var_rule, 'index')), rt.get_property(var_rule, 'value'), rt.get_property(var_rule, 'operation'))).to_bool()
}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_onboardingprofileruleprocessor_php() {
