...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_IsEcommerceRuleProcessor) process(var_rule rt.PhpVal, var_stored_state rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('wc_calypso_bridge_is_ecommerce_plan')]))))) {
		return rt.identical(rt.new_bool(false), rt.get_property(var_rule, 'value'))
	}
	return rt.identical(// unsupported expression: Expr_Cast_Bool, rt.get_property(var_rule, 'value'))
}
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_isecommerceruleprocessor_php() {
