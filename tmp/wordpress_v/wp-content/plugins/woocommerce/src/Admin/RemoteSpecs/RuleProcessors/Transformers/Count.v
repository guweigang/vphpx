...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_Count) transform(var_value rt.PhpVal, mut var_arguments Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_?stdClass, var_default_value rt.PhpVal) i64 {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_value.dup().is_array()))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_value, 'Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_Countable')))))))) {
		return (var_default_value).to_i64()
	}
	return var_value.dup().array_count()
}
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_transformers_count() &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_Count {
...
pub fn init_wp_content_plugins_woocommerce_src_admin_remotespecs_ruleprocessors_transformers_count_php() {
}
