...
fn (mut this Class_Automattic_WooCommerce_Admin_Overrides_Order) get_data_without_line_items() rt.PhpVal {
	return rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: 'id', val: this.get_id() }]), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_Overrides_Order', ['Automattic_WooCommerce_Admin_Overrides_WC_Order'], &this), 'data'), rt.create_array([rt.ArrayItem{ key: 'number', val: this.get_order_number() }, rt.ArrayItem{ key: 'meta_data', val: this.get_meta_data() }])])
}

fn (mut this Class_Automattic_WooCommerce_Admin_Overrides_Order) get_line_item_data(var_type rt.PhpVal) bool {
	mut var_type_to_items := rt.create_array([rt.ArrayItem{ key: 'line_items', val: Class_Automattic_WooCommerce_Enums_OrderItemType.line_item() }, rt.ArrayItem{ key: 'tax_lines', val: Class_Automattic_WooCommerce_Enums_OrderItemType.tax() }, rt.ArrayItem{ key: 'shipping_lines', val: Class_Automattic_WooCommerce_Enums_OrderItemType.shipping() }, rt.ArrayItem{ key: 'fee_lines', val: Class_Automattic_WooCommerce_Enums_OrderItemType.fee() }, rt.ArrayItem{ key: 'coupon_lines', val: Class_Automattic_WooCommerce_Enums_OrderItemType.coupon() }])
...
fn (mut this Class_Automattic_WooCommerce_Admin_Overrides_Order) get_report_customer_id() rt.PhpVal {
	if rt.is_true(rt.new_bool(this.customer_id.is_null())) {
		this.customer_id = fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_API_Reports_Customers_DataStore{}; return temp.get_or_create_customer_from_order(arg_0) }(rt.new_object('Automattic_WooCommerce_Admin_Overrides_Order', []string{}, this))
	}
	return this.customer_id
}

fn (mut this Class_Automattic_WooCommerce_Admin_Overrides_Order) is_returning_customer() rt.PhpVal {
	return fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Stats_DataStore{}; return temp.is_returning_customer(arg_0, arg_1) }(rt.new_object('Automattic_WooCommerce_Admin_Overrides_Order', []string{}, this), this.get_report_customer_id())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Overrides_Order) get_customer_first_name() rt.PhpVal {
	if rt.is_true(this.get_user_id()) {
		return rt.call_function('get_user_meta', [this.get_user_id(), rt.new_string('first_name'), rt.new_bool(true)])
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Overrides_Order) get_customer_last_name() rt.PhpVal {
	if rt.is_true(this.get_user_id()) {
		return rt.call_function('get_user_meta', [this.get_user_id(), rt.new_string('last_name'), rt.new_bool(true)])
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
...
fn create_automattic_woocommerce_admin_overrides_order() &Class_Automattic_WooCommerce_Admin_Overrides_Order {
...
fn create_automattic_woocommerce_admin_overrides_wc_order() &Class_Automattic_WooCommerce_Admin_Overrides_WC_Order {
...
fn (mut this Class_Automattic_WooCommerce_Admin_Overrides_Order) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'order_class_name' {
			...
			return rt.new_string(Class_Automattic_WooCommerce_Admin_Overrides_Order.order_class_name(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2))
		}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Overrides_Order) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'refunded_line_items' { this.refunded_line_items = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_admin_overrides_order_php() {
