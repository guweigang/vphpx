...
fn (mut this Class_Automattic_WooCommerce_StoreApi_SchemaController) get(var_name rt.PhpVal, version i64) rt.PhpVal {
	mut var_schema := if !(this.schemas.array_get('v${var_version.str()}').array_get(var_name)).is_null() {
		this.schemas.array_get('v${var_version.str()}').array_get(var_name)
	} else {
		rt.new_bool(false)
	}
	...
	return rt.create_object_dynamically(var_schema, [this.extend,
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_schemacontroller_php() {
}
