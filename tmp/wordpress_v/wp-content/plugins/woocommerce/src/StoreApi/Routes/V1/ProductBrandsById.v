...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductBrandsById) get_route_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	if rt.is_true(rt.new_bool(var_request.array_isset(rt.new_string('identifier')) && rt.is_true(rt.new_bool(var_request.array_get('identifier').is_long() || var_request.array_get('identifier').is_double())))) {
		mut var_object := rt.call_function('get_term', [// unsupported expression: Expr_Cast_Int, rt.new_string('product_brand')])
	} else {
		var_object = rt.call_function('get_term_by', [rt.new_string('slug'), var_request.array_get('identifier'), rt.new_string('product_brand')])
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_object)))) {
		if rt.is_true(rt.new_bool(var_request.array_isset(rt.new_string('identifier')) && rt.is_true(rt.new_bool(var_request.array_get('identifier').is_long() || var_request.array_get('identifier').is_double())))) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Exceptions_RouteException', []string{}, create_automattic_woocommerce_storeapi_exceptions_routeexception(rt.new_string('woocommerce_rest_brand_invalid_id'), rt.call_function('esc_html__', [rt.new_string('Invalid brand ID.'), rt.new_string('woocommerce')]), rt.new_int(404))))
		} else {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Exceptions_RouteException', []string{}, create_automattic_woocommerce_storeapi_exceptions_routeexception(rt.new_string('woocommerce_rest_brand_invalid_slug'), rt.call_function('esc_html__', [rt.new_string('Invalid brand slug.'), rt.new_string('woocommerce')]), rt.new_int(404))))
		}
	}
...
fn create_automattic_woocommerce_storeapi_routes_v1_productbrandsbyid() &Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductBrandsById {
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_routes_v1_productbrandsbyid_php() {
