	...
	mut var_items := rt.new_array()
	{
		...
		for {
			...
			mut var_data := this.prepare_item_for_response(var_cart_item.dup(), rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request',
				[]string{}, var_request))
			var_items.array_push(this.prepare_response_for_collection(var_data.dup()))
		}
	}
		...
		rt.ArrayItem{ key: 'self', val: rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				rt.concat(rt.call_function('trailingslashit', [
					var_base.dup()]), var_cart_item.array_get('key')),
			]) },
		]) },
...
fn create_automattic_woocommerce_storeapi_routes_v1_cartitems() &Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartItems {
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_routes_v1_cartitems_php() {
}
