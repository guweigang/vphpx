...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_Agentic_AgenticCheckoutSession) get_or_set_checkout_session_id() string {
	...
	mut var_session_id := rt.call_method(var_wc_session, 'get', [Class_Automattic_WooCommerce_StoreApi_Routes_V1_Agentic_Enums_SessionKey.agentic_checkout_session_id()])
	if rt.is_true(rt.identical(rt.new_null(), var_session_id)) {
		var_session_id = fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_StoreApi_Utilities_CartTokenUtils{}; return temp.get_cart_token(arg_0) }(// unsupported expression: Expr_Cast_String)
		rt.call_method(var_wc_session, 'set', [Class_Automattic_WooCommerce_StoreApi_Routes_V1_Agentic_Enums_SessionKey.agentic_checkout_session_id(), var_session_id.dup()])
	}
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_routes_v1_agentic_agenticcheckoutsession_php() {
