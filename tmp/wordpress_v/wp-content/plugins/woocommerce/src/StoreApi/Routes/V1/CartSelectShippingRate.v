...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartSelectShippingRate) get_route_post_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	...
	mut var_package_id := if var_request.array_isset(rt.new_string('package_id')) { rt.call_function('sanitize_text_field', [
			rt.call_function('wp_unslash', [var_request.array_get('package_id')]),
		]) } else { rt.new_null() }
	mut var_rate_id := rt.call_function('sanitize_text_field', [
		rt.call_function('wp_unslash', [var_request.array_get('rate_id')]),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_package_id.dup().is_null()))))) {
		rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartSelectShippingRate', [
			...
			var_package_id.dup(), var_rate_id.dup()])
		if rt.has_exception() {
			unsafe {
				goto catch_label_1
			}
...
fn create_automattic_woocommerce_storeapi_routes_v1_cartselectshippingrate() &Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartSelectShippingRate {
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_routes_v1_cartselectshippingrate_php() {
}
