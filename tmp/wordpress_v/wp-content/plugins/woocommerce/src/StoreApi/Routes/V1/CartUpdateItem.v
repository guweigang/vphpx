...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateItem) get_route_post_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	mut var_cart := rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateItem', ['Automattic_WooCommerce_StoreApi_Routes_V1_AbstractCartRoute'], &this), 'cart_controller'), 'get_cart_instance', []rt.PhpVal{})
	if var_request.array_isset(rt.new_string('quantity')) {
		mut var_cart_item := rt.call_method(var_cart, 'get_cart_item', [var_request.array_get('key')])
		mut var_old_quantity := if !(var_cart_item.array_get('quantity')).is_null() { var_cart_item.array_get('quantity') } else { rt.new_int(0) }
		rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateItem', ['Automattic_WooCommerce_StoreApi_Routes_V1_AbstractCartRoute'], &this), 'cart_controller'), 'set_cart_item_quantity', [var_request.array_get('key'), var_request.array_get('quantity')])
		if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
			rt.call_function('do_action', [rt.new_string('internal_woocommerce_cart_item_updated_from_user_request'), var_request.array_get('key'), // unsupported expression: Expr_Cast_Int, var_old_quantity.dup(), var_cart.dup()])
		}
	}
	return rt.call_function('rest_ensure_response', [rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateItem', ['Automattic_WooCommerce_StoreApi_Routes_V1_AbstractCartRoute'], &this), 'schema'), 'get_item_response', [var_cart.dup()])])
}
...
fn create_automattic_woocommerce_storeapi_routes_v1_cartupdateitem() &Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateItem {
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_routes_v1_cartupdateitem_php() {
}
