...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer) validate_address_params(var_request rt.PhpVal, var_billing rt.PhpVal, var_shipping rt.PhpVal) bool {
	...
	if rt.is_true(var_posted_billing) {
		mut var_billing_validation_check := rt.call_method(rt.get_property(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer', [
			...
			var_billing_mutated.dup(), var_request.dup(), rt.new_string('billing_address')])
		if rt.is_true(rt.identical(rt.new_bool(false), var_billing_validation_check)) {
			...
			var_billing_validation_check.dup()]))
		{
	...
	if rt.is_true(var_posted_shipping) {
		mut var_shipping_validation_check := rt.call_method(rt.get_property(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer', [
			...
			var_shipping_mutated.dup(), var_request.dup(), rt.new_string('shipping_address')])
		if rt.is_true(rt.identical(rt.new_bool(false), var_shipping_validation_check)) {
			...
			var_shipping_validation_check.dup()]))
		{
	...
	if rt.is_true(var_invalid_params) {
		return (create_automattic_woocommerce_storeapi_routes_v1_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('sprintf', [
			...
			rt.call_function('implode', [rt.new_string(', '),
				rt.func_array_keys(var_invalid_params.dup())]),
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer) get_route_post_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	...
	mut var_billing := rt.call_function('wp_parse_args', [if !(var_request.array_get('billing_address')).is_null() {
		var_request.array_get('billing_address')
	} else {
		rt.new_array()
	},
	...
	mut var_shipping := rt.call_function('wp_parse_args', [if !(var_request.array_get('shipping_address')).is_null() {
		var_request.array_get('shipping_address')
	} else {
		rt.new_array()
	},
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_cart, 'needs_shipping', []rt.PhpVal{})))))
		&& !(var_request.array_isset(rt.new_string('shipping_address')))))
	{
		var_shipping = var_billing.dup()
	}
	var_billing = rt.call_method(rt.get_property(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer', [
		...
		var_billing.dup(), var_request, rt.new_string('billing_address')])
	var_shipping = rt.call_method(rt.get_property(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer', [
		...
		var_shipping.dup(), var_request, rt.new_string('shipping_address')])
	mut var_validation_check := rt.new_bool(this.validate_address_params(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request',
		[]string{}, var_request), var_billing.dup(), var_shipping.dup()))
	if rt.is_true(rt.call_function('is_wp_error', [var_validation_check.dup()])) {
		return rt.call_function('rest_ensure_response', [var_validation_check.dup()])
	}
	rt.call_method(var_customer, 'set_props', [
		rt.create_array([
			rt.ArrayItem{
				key: 'billing_first_name'
				val: if !(var_billing.array_get('first_name')).is_null() {
					var_billing.array_get('first_name')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_billing.array_get('last_name')).is_null() {
					var_billing.array_get('last_name')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'billing_company'
				val: if !(var_billing.array_get('company')).is_null() {
					var_billing.array_get('company')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_billing.array_get('address_1')).is_null() {
					var_billing.array_get('address_1')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_billing.array_get('address_2')).is_null() {
					var_billing.array_get('address_2')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'billing_city'
				val: if !(var_billing.array_get('city')).is_null() {
					var_billing.array_get('city')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'billing_state'
				val: if !(var_billing.array_get('state')).is_null() {
					var_billing.array_get('state')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_billing.array_get('postcode')).is_null() {
					var_billing.array_get('postcode')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'billing_country'
				val: if !(var_billing.array_get('country')).is_null() {
					var_billing.array_get('country')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'billing_phone'
				val: if !(var_billing.array_get('phone')).is_null() {
					var_billing.array_get('phone')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'billing_email'
				val: if !(var_billing.array_get('email')).is_null() {
					var_billing.array_get('email')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_shipping.array_get('first_name')).is_null() {
					var_shipping.array_get('first_name')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_shipping.array_get('last_name')).is_null() {
					var_shipping.array_get('last_name')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_shipping.array_get('company')).is_null() {
					var_shipping.array_get('company')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_shipping.array_get('address_1')).is_null() {
					var_shipping.array_get('address_1')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_shipping.array_get('address_2')).is_null() {
					var_shipping.array_get('address_2')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'shipping_city'
				val: if !(var_shipping.array_get('city')).is_null() {
					var_shipping.array_get('city')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'shipping_state'
				val: if !(var_shipping.array_get('state')).is_null() {
					var_shipping.array_get('state')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_shipping.array_get('postcode')).is_null() {
					var_shipping.array_get('postcode')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				...
				val: if !(var_shipping.array_get('country')).is_null() {
					var_shipping.array_get('country')
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'shipping_phone'
				val: if !(var_shipping.array_get('phone')).is_null() {
					var_shipping.array_get('phone')
				} else {
					rt.new_null()
				}
			},
	...
	mut var_additional_shipping_values := rt.call_function('array_diff_key', [
		var_shipping.dup(), rt.call_function('array_flip', [var_core_fields.dup()])])
	mut var_additional_billing_values := rt.call_function('array_diff_key', [
		var_billing.dup(), rt.call_function('array_flip', [var_core_fields.dup()])])
	{
	...
	{
		mut iter_1 := var_additional_billing_values.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_value := item_1.val
			mut var_key := item_1.key
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer) get_customer_billing_address(mut var_customer Class_Automattic_WooCommerce_StoreApi_Routes_V1_WC_Customer) rt.PhpVal {
	...
	mut var_additional_fields := rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer', [
		...
		var_customer_mutated.dup(), rt.new_string('billing')])
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer) get_customer_shipping_address(mut var_customer Class_Automattic_WooCommerce_StoreApi_Routes_V1_WC_Customer) rt.PhpVal {
	...
	mut var_additional_fields := rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer', [
		...
		var_customer_mutated.dup(), rt.new_string('shipping')])
	return rt.call_function('array_merge', [
		rt.create_array([
			rt.ArrayItem{ key: 'first_name', val: rt.call_method(var_customer_mutated,
...
fn create_automattic_woocommerce_storeapi_routes_v1_cartupdatecustomer() &Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartUpdateCustomer {
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_routes_v1_cartupdatecustomer_php() {
}
