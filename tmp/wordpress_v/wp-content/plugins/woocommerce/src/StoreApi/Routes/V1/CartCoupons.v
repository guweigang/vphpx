	...
	mut var_items := rt.new_array()
	{
		...
		for {
			...
			rt.call_method(var_response, 'add_links', [
				this.prepare_links(var_coupon_code.dup(), rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request',
					[]string{}, var_request)),
			])
			var_response = this.prepare_response_for_collection(var_response.dup())
			var_items.array_push(var_response.dup())
		}
	}
	mut var_response := rt.call_function('rest_ensure_response', [
		var_items.dup()])
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartCoupons) get_route_post_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	...
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartCoupons', [
		'Automattic_WooCommerce_StoreApi_Routes_V1_AbstractCartRoute',
	], &this), 'cart_controller'), 'apply_coupon', [var_request.array_get('code')])
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	end_label_1:
	mut var_response := this.prepare_item_for_response(var_request.array_get('code'), rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request',
		[]string{}, var_request))
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartCoupons) get_route_delete_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	...
	return create_automattic_woocommerce_storeapi_routes_v1_wp_rest_response(rt.new_array(),
		rt.new_int(200))
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartCoupons) prepare_links(var_coupon_code rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_base := rt.new_string(rt.concat(this.get_namespace(), this.get_path()))
...
fn create_automattic_woocommerce_storeapi_routes_v1_cartcoupons() &Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartCoupons {
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_routes_v1_cartcoupons_php() {
}
