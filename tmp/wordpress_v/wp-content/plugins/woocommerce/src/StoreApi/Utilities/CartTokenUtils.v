...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_CartTokenUtils.get_cart_token(customer_id string) string {
	return (fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_StoreApi_Utilities_JsonWebToken{}
		return temp.create(arg_0, arg_1)
	}(rt.create_array([rt.ArrayItem{ key: 'user_id', val: customer_id },
		rt.ArrayItem{
			key: 'exp'
		...
		Class_Automattic_WooCommerce_StoreApi_Utilities_CartTokenUtils.get_cart_token_secret())).str()
}

fn Class_Automattic_WooCommerce_StoreApi_Utilities_CartTokenUtils.validate_cart_token(cart_token string) bool {
	return (fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_StoreApi_Utilities_JsonWebToken{}
		return temp.validate(arg_0, arg_1)
	}(rt.new_string(cart_token),
		Class_Automattic_WooCommerce_StoreApi_Utilities_CartTokenUtils.get_cart_token_secret())).to_bool()
}

fn Class_Automattic_WooCommerce_StoreApi_Utilities_CartTokenUtils.get_cart_token_payload(cart_token string) rt.PhpVal {
	mut var_parts := rt.get_property(fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_StoreApi_Utilities_JsonWebToken{}
		return temp.get_parts(arg_0)
	}(rt.new_string(cart_token)), 'payload')
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_utilities_carttokenutils_php() {
