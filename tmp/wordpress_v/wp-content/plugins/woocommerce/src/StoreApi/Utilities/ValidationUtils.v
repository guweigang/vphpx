...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Utilities_ValidationUtils) validate_state(var_state rt.PhpVal, var_country rt.PhpVal) bool {
	...
	mut var_states := this.get_states_for_country(var_country.dup())
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_int(var_states.dup().array_count()))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.call_function('wc_strtoupper', [var_state_mutated.dup()]), rt.call_function('array_map', [rt.new_string('\\wc_strtoupper'), rt.func_array_keys(var_states.dup())]), rt.new_bool(true)])))))))
	{
		return false
	}
	return true
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Utilities_ValidationUtils) format_state(var_state rt.PhpVal, var_country rt.PhpVal) rt.PhpVal {
	mut var_state_mutated := var_state
	mut var_states := this.get_states_for_country(var_country.dup())
	if rt.is_true(rt.new_int(var_states.dup().array_count())) {
		var_state_mutated = rt.call_function('wc_strtoupper', [
			var_state_mutated.dup()])
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_utilities_validationutils_php() {
}
