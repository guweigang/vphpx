...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Utilities_ProductQueryFilters) get_filtered_price(var_request rt.PhpVal) rt.PhpVal {
	...
	rt.call_function('add_filter', [rt.new_string('posts_clauses'), rt.create_array([rt.ArrayItem{ key: none, val: var_product_query }, rt.ArrayItem{ key: none, val: 'add_query_clauses' }]), rt.new_int(10), rt.new_int(2)])
	rt.call_function('add_filter', [rt.new_string('posts_pre_query'), rt.new_string('__return_empty_array')])
	mut var_query_args := var_product_query.prepare_objects_query(var_request.dup())
	var_query_args.array_set('no_found_rows', true)
	var_query_args.array_set('posts_per_page', // unsupported expression: Expr_UnaryMinus)
	mut var_query := create_automattic_woocommerce_storeapi_utilities_wp_query()
	mut var_result := var_query.query(var_query_args.dup())
	mut var_product_query_sql := rt.get_property(var_query, 'request')
	rt.call_function('remove_filter', [rt.new_string('posts_clauses'), rt.create_array([rt.ArrayItem{ key: none, val: var_product_query }, rt.ArrayItem{ key: none, val: 'add_query_clauses' }]), rt.new_int(10)])
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Utilities_ProductQueryFilters) get_stock_status_counts(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_hide_outofstock_items := rt.call_function('get_option', [rt.new_string('woocommerce_hide_out_of_stock_items')])
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Utilities_ProductQueryFilters) get_attribute_counts(var_request rt.PhpVal, var_attributes rt.PhpVal) rt.PhpVal {
	...
	// unsupported statement: Stmt_Global
	rt.call_method(var_request, 'set_param', [rt.new_string('page'), rt.new_null()])
	rt.call_method(var_request, 'set_param', [rt.new_string('per_page'), rt.new_null()])
	rt.call_method(var_request, 'set_param', [rt.new_string('order'), rt.new_null()])
	rt.call_method(var_request, 'set_param', [rt.new_string('orderby'), rt.new_null()])
	mut var_product_query := create_automattic_woocommerce_storeapi_utilities_productquery()
	rt.call_function('add_filter', [rt.new_string('posts_clauses'), rt.create_array([rt.ArrayItem{ key: none, val: var_product_query }, rt.ArrayItem{ key: none, val: 'add_query_clauses' }]), rt.new_int(10), rt.new_int(2)])
	rt.call_function('add_filter', [rt.new_string('posts_pre_query'), rt.new_string('__return_empty_array')])
	mut var_query_args := var_product_query.prepare_objects_query(var_request.dup())
	...
	rt.call_function('remove_filter', [rt.new_string('posts_clauses'), rt.create_array([rt.ArrayItem{ key: none, val: var_product_query }, rt.ArrayItem{ key: none, val: 'add_query_clauses' }]), rt.new_int(10)])
	rt.call_function('remove_filter', [rt.new_string('posts_pre_query'), rt.new_string('__return_empty_array')])
	if var_attributes_mutated.dup().array_count() == rt.call_function('array_filter', [var_attributes_mutated.dup(), rt.new_string('is_numeric')]).array_count() {
		var_attributes_mutated = rt.call_function('array_map', [rt.new_string('wc_attribute_taxonomy_name_by_id'), rt.call_function('wp_parse_id_list', [var_attributes_mutated.dup()])])
	}
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_attribute := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	var_attribute = rt.call_function('wc_sanitize_taxonomy_name', [var_attribute.dup()])
	return rt.call_function('esc_sql', [var_attribute.dup()])
	}
	mut var_attribute := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	var_attribute = rt.call_function('wc_sanitize_taxonomy_name', [var_attribute.dup()])
	return rt.call_function('esc_sql', [var_attribute.dup()])
	}
	mut var_attributes_to_count := rt.call_function('array_map', [rt.new_closure(closure_1_fn), var_attributes_mutated.dup()])
	mut var_attributes_to_count_sql := rt.new_string('AND term_taxonomy.taxonomy IN (\'' + (rt.call_function('implode', [rt.new_string('\',\''), var_attributes_to_count.dup()])).str() + '\')')
	mut var_attribute_count_sql := rt.new_string(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('\n\t\t\tSELECT COUNT( DISTINCT posts.ID ) as term_count, terms.term_id as term_count_id\n\t\t\tFROM '), rt.get_property(var_wpdb, 'posts')), rt.new_string(' AS posts\n\t\t\tINNER JOIN ')), rt.get_property(var_wpdb, 'term_relationships')), rt.new_string(' AS term_relationships ON posts.ID = term_relationships.object_id\n\t\t\tINNER JOIN ')), rt.get_property(var_wpdb, 'term_taxonomy')), rt.new_string(' AS term_taxonomy USING( term_taxonomy_id )\n\t\t\tINNER JOIN ')), rt.get_property(var_wpdb, 'terms')), rt.new_string(' AS terms USING( term_id )\n\t\t\tWHERE posts.ID IN ( ')), var_product_query_sql), rt.new_string(' )\n\t\t\t')), var_attributes_to_count_sql), rt.new_string('\n\t\t\tGROUP BY terms.term_id\n\t\t')))
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Utilities_ProductQueryFilters) get_taxonomy_counts(var_request rt.PhpVal, var_taxonomies rt.PhpVal) rt.PhpVal {
	...
	rt.call_method(var_request, 'set_param', [rt.new_string('orderby'), rt.new_null()])
	mut var_product_query := create_automattic_woocommerce_storeapi_utilities_productquery()
	mut var_query_vars := var_product_query.prepare_objects_query(var_request.dup())
	mut var_container := rt.call_function('wc_get_container', []rt.PhpVal{})
	mut var_filter_data_provider := rt.call_method(var_container, 'get', [Class_Automattic_WooCommerce_StoreApi_Utilities_Automattic_WooCommerce_Internal_ProductFilters_FilterDataProvider.class()])
	mut var_filter_data := rt.call_method(var_filter_data_provider, 'with', [var_product_query])
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_utilities_productqueryfilters_php() {
}
