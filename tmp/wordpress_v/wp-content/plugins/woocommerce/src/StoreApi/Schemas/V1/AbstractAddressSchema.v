...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractAddressSchema) construct(mut var_extend Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema, mut var_controller Class_Automattic_WooCommerce_StoreApi_SchemaController) {
	...
	this.additional_fields_controller = rt.call_method(fn () rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractAddressSchema) sanitize_callback(var_address rt.PhpVal, var_request rt.PhpVal, var_param rt.PhpVal) rt.PhpVal {
	...
	var_address_mutated = rt.call_function('array_intersect_key', [
		var_address_mutated.dup(), var_schema.dup()])
	closure_1_fn := fn [var_address, var_validation_util, var_schema] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		if rt.is_true(rt.equal(switch_val_1, rt.new_string('country'))) {
			...
			var_carry.array_set(var_key, var_validation_util.format_state(rt.call_function('sanitize_text_field', [
				rt.call_function('wp_unslash', [var_address_mutated.array_get(var_key)]),
			]), var_address_mutated.array_get('country')))
		} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('postcode'))) {
			var_carry.array_set(var_key, if rt.is_true(var_address_mutated.array_get('postcode')) { rt.call_function('wc_format_postcode', [
					rt.call_function('sanitize_text_field', [
						rt.call_function('wp_unslash', [var_address_mutated.array_get('postcode')]),
					]),
					var_address_mutated.array_get('country'),
				]) } else { rt.new_string('') })
		} else {
		...
		]))
		{
			var_carry.array_set(var_key, rt.call_method(this.additional_fields_controller,
				'sanitize_field', [var_key.dup(), var_carry.array_get(var_key)]))
		}
		return var_carry.dup()
	}
	var_address_mutated = rt.call_function('array_reduce', [
		rt.func_array_keys(var_address_mutated.dup()),
		rt.new_closure(closure_1_fn),
		rt.new_array(),
	])
	return var_sanitization_util.wp_kses_array(var_address_mutated.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractAddressSchema) validate_callback(var_address rt.PhpVal, var_request rt.PhpVal, var_param rt.PhpVal) rt.PhpVal {
		...
		for {
			...
			if rt.is_true(rt.call_function('is_wp_error', [
				rt.call_function('rest_validate_value_from_schema', [
					var_value.dup(), var_schema.array_get(var_key),
					var_key.dup()]),
			]))
			{
	...
	var_address_mutated = this.sanitize_callback(var_address_mutated.dup(), var_request.dup(),
		var_param.dup())
	if rt.is_true(rt.new_bool(!(!rt.is_true(var_address_mutated.array_get('country')))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_address_mutated.array_get('country'), rt.func_array_keys(rt.call_method(rt.get_property(rt.call_function('wc', []rt.PhpVal{}), 'countries'), 'get_countries', []rt.PhpVal{})), rt.new_bool(true)])))))))
	{
	...
	if rt.is_true(rt.new_bool(!(!rt.is_true(var_address_mutated.array_get('state')))
		&& rt.is_true(rt.new_bool(!(rt.is_true(var_validation_util.validate_state(var_address_mutated.array_get('state'), var_address_mutated.array_get('country'))))))))
	{
		var_errors.add(rt.new_string('invalid_state'), rt.call_function('sprintf', [
			...
			rt.call_function('esc_html', [
				var_address_mutated.array_get('state'),
			]),
			rt.call_function('implode', [
				rt.new_string(', '),
				rt.func_array_keys(var_validation_util.get_states_for_country(var_address_mutated.array_get('country'))),
			]),
		]))
	}
	if rt.is_true(rt.new_bool(!(!rt.is_true(var_address_mutated.array_get('postcode'))) && rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_StoreApi_Schemas_V1_WC_Validation{}
		return temp.is_postcode(arg_0, arg_1)
	}(var_address_mutated.array_get('postcode'), var_address_mutated.array_get('country'))))))))
	{
	...
	{
		mut iter_1 := rt.func_array_keys(var_address_mutated.dup()).iterator()
		for {
			item_1 := iter_1.next() or { break }
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractAddressSchema) get_additional_address_fields_schema() rt.PhpVal {
	...
	mut var_fields := rt.call_method(this.additional_fields_controller, 'get_additional_fields',
		[]rt.PhpVal{})
	closure_2_fn := fn [var_additional_fields_keys] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_key := if args.len > 0 { args[0].dup() } else { rt.new_null() }
		return rt.call_function('in_array', [var_key.dup(), var_additional_fields_keys.dup(),
			rt.new_bool(true)])
	}
	mut var_address_fields := rt.call_function('array_filter', [
		var_fields.dup(), rt.new_closure(closure_2_fn), rt.get_constant('ARRAY_FILTER_USE_KEY')])
	mut var_schema := rt.new_array()
	{
		...
		for {
			...
			mut var_field_schema := rt.create_array([
				rt.ArrayItem{ key: 'description', val: var_field.array_get('label') },
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'view' },
					rt.ArrayItem{ key: none, val: 'edit' },
				...
				rt.ArrayItem{
					...
					]))
					{
						...
						rt.identical(rt.new_bool(true), var_field.array_get('required'))
					}
				},
			])
...
fn create_automattic_woocommerce_storeapi_schemas_v1_wc_validation() &Class_Automattic_WooCommerce_StoreApi_Schemas_V1_WC_Validation {
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_schemas_v1_abstractaddressschema_php() {
