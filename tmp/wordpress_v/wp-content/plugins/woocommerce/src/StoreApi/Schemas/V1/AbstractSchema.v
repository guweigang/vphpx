...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractSchema) remove_arg_options(var_properties rt.PhpVal) rt.PhpVal {
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_property.dup().is_array()))))) {
				return var_property.dup()
			}
			if var_property.array_isset(rt.new_string('properties')) {
				var_property.array_set('properties',
					this.remove_arg_options(var_property.array_get('properties')))
			} else if var_property.array_get('items').array_isset(rt.new_string('properties')) {
				var_property.array_get_mut('items').array_set('properties',
					this.remove_arg_options(var_property.array_get('items').array_get('properties')))
			}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractSchema) get_public_item_schema() rt.PhpVal {
	...
	if var_schema.array_isset(rt.new_string('properties')) {
		var_schema.array_set('properties',
			this.remove_arg_options(var_schema.array_get('properties')))
	}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractSchema) get_extended_data(var_endpoint rt.PhpVal, var_passed_args rt.PhpVal) rt.PhpVal {
	return rt.call_method(this.extend, 'get_endpoint_data', [
		var_endpoint.dup(), var_passed_args.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractSchema) get_recursive_schema_property_defaults(var_properties rt.PhpVal) rt.PhpVal {
	mut var_defaults := rt.new_array()
	{
		...
		for {
			...
			if var_property_value.array_get('arg_options').array_isset(rt.new_string('default')) {
				var_defaults.array_set(var_property_key,
					var_property_value.array_get('arg_options').array_get('default'))
			} else if var_property_value.array_isset(rt.new_string('properties')) {
				var_defaults.array_set(var_property_key,
					this.get_recursive_schema_property_defaults(var_property_value.array_get('properties')))
			}
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractSchema) get_recursive_validate_callback(var_properties rt.PhpVal) rt.PhpVal {
	closure_3_fn := fn [var_properties] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_param := if args.len > 2 { args[2].dup() } else { rt.new_null() }
		{
			mut iter_1 := var_properties.iterator()
			for {
				item_1 := iter_1.next() or { break }
				...
				mut var_property_type := if rt.is_true(rt.new_bool(var_property_value.array_get('type').is_array())) { var_property_value.array_get('type') } else { rt.create_array([
						rt.ArrayItem{ key: none, val: var_property_value.array_get('type') },
					]) }
				if rt.is_true(rt.new_bool(!rt.is_true(var_current_value)
					&& rt.is_true(rt.call_function('in_array', [rt.new_string('null'), var_property_type.dup(), rt.new_bool(true)]))))
				{
					continue
				}
				if var_property_value.array_get('arg_options').array_isset(rt.new_string('validate_callback')) {
					mut var_callback :=
						var_property_value.array_get('arg_options').array_get('validate_callback')
					mut var_result := if rt.is_true(rt.call_function('is_callable', [
						var_callback.dup(),
					]))
					{ rt.call_callable(var_callback, [var_current_value.dup(),
							var_request.dup(), var_param.dup()]) } else { rt.new_bool(false) }
				} else {
					var_result = rt.call_function('rest_validate_value_from_schema', [
						var_current_value.dup(),
						var_property_value.dup(),
						var_param.str() + ' > ' + var_property_key.str(),
					])
				}
				if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_result))))
					|| rt.is_true(rt.call_function('is_wp_error', [var_result.dup()]))))
				{
					return var_result.dup()
				}
				if var_property_value.array_isset(rt.new_string('properties')) {
					mut var_validate_callback :=
						this.get_recursive_validate_callback(var_property_value.array_get('properties'))
					var_result = rt.call_callable(var_validate_callback, [
						var_current_value.dup(), var_request.dup(),
						var_param.str() + ' > ' +
							var_property_key.str()])
					if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_result))))
						|| rt.is_true(rt.call_function('is_wp_error', [var_result.dup()]))))
					{
						return var_result.dup()
	...
	closure_4_fn := fn [var_properties] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_sanitized_values := rt.new_array()
		{
			...
			for {
				...
				if var_property_value.array_get('arg_options').array_isset(rt.new_string('sanitize_callback')) {
					mut var_callback :=
						var_property_value.array_get('arg_options').array_get('sanitize_callback')
					var_current_value = if rt.is_true(rt.call_function('is_callable', [
						var_callback.dup(),
					]))
					{ rt.call_callable(var_callback, [var_current_value.dup(),
							var_request.dup(), var_param.dup()]) } else { var_current_value }
				} else {
				...
				if rt.is_true(rt.call_function('is_wp_error', [
					var_current_value.dup()]))
				{
					return var_current_value.dup()
				}
				if var_property_value.array_isset(rt.new_string('properties')) {
					mut var_sanitize_callback :=
						this.get_recursive_sanitize_callback(var_property_value.array_get('properties'))
					...
					var_sanitized_values.array_set(var_property_key, var_current_value.dup())
				}
			}
		}
		return var_sanitized_values.dup()
	}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractSchema) get_extended_schema(var_endpoint rt.PhpVal, var_passed_args rt.PhpVal) rt.PhpVal {
	...
	mut var_defaults := this.get_recursive_schema_property_defaults(var_extended_schema.dup())
	return rt.create_array([rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'context', val: rt.create_array([
			...
			rt.ArrayItem{
				...
				val: this.get_recursive_validate_callback(var_extended_schema.dup())
			},
			rt.ArrayItem{
				key: 'sanitize_callback'
				val: this.get_recursive_sanitize_callback(var_extended_schema.dup())
			},
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractSchema) get_item_responses_from_schema(mut var_schema Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractSchema, var_items rt.PhpVal) rt.PhpVal {
	...
	var_items_mutated = rt.call_function('array_filter', [var_items_mutated.dup()])
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractSchema) get_endpoint_args_for_item_schema(var_method rt.PhpVal) rt.PhpVal {
	...
	var_endpoint_args = this.remove_arg_options(var_endpoint_args.dup())
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractSchema) force_schema_readonly(var_properties rt.PhpVal) rt.PhpVal {
	closure_6_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		closure_5_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			if var_property.array_get('items').array_isset(rt.new_string('properties')) {
				var_property.array_get_mut('items').array_set('properties',
					this.force_schema_readonly(var_property.array_get('items').array_get('properties')))
			}
			return var_property.dup()
		}
		mut var_property := if args.len > 0 { args[0].dup() } else { rt.new_null() }
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_schemas_v1_abstractschema_php() {
}
