...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CheckoutSchema) get_item_response(var_item rt.PhpVal) rt.PhpVal {
	mut var_cart := if rt.is_true(rt.call_function('property_exists', [var_item.dup(), rt.new_string('cart')])) { rt.get_property(var_item, 'cart') } else { rt.new_null() }
	mut var_payment_result := if rt.is_true(rt.call_function('property_exists', [var_item.dup(), rt.new_string('payment_result')])) { rt.get_property(var_item, 'payment_result') } else { rt.new_null() }
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CheckoutSchema) get_checkout_response(mut var_order Class_Automattic_WooCommerce_StoreApi_Schemas_V1_WC_Order, mut var_payment_result Class_Automattic_WooCommerce_StoreApi_Schemas_V1_?PaymentResult, mut var_cart Class_Automattic_WooCommerce_StoreApi_Schemas_V1_?WC_Cart) rt.PhpVal {
	...
	return rt.create_array([rt.ArrayItem{ key: 'order_id', val: var_order.get_id() }, rt.ArrayItem{ key: 'status', val: var_order.get_status() }, rt.ArrayItem{ key: 'order_key', val: var_order.get_order_key() }, rt.ArrayItem{ key: 'order_number', val: var_order.get_order_number() }, rt.ArrayItem{ key: 'customer_note', val: var_order.get_customer_note() }, rt.ArrayItem{ key: 'customer_id', val: var_order.get_customer_id() }, rt.ArrayItem{ key: 'billing_address', val: // unsupported expression: Expr_Cast_Object }, rt.ArrayItem{ key: 'shipping_address', val: // unsupported expression: Expr_Cast_Object }, rt.ArrayItem{ key: 'payment_method', val: var_order.get_payment_method() }, rt.ArrayItem{ key: 'payment_result', val: var_payment_result_mutated }, rt.ArrayItem{ key: 'additional_fields', val: // unsupported expression: Expr_Cast_Object }, rt.ArrayItem{ key: '__experimentalCart', val: if rt.is_true(var_cart_mutated) { // unsupported expression: Expr_Cast_Object } else { rt.new_null() } }, rt.ArrayItem{ key: Class_Automattic_WooCommerce_StoreApi_Schemas_V1_Automattic_WooCommerce_StoreApi_Schemas_V1_CheckoutSchema.extending_key(), val: this.get_extended_data(Class_Automattic_WooCommerce_StoreApi_Schemas_V1_Automattic_WooCommerce_StoreApi_Schemas_V1_CheckoutSchema.identifier()) }])
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CheckoutSchema) prepare_payment_details_for_response(mut var_payment_details Class_Automattic_WooCommerce_StoreApi_Schemas_V1_array) rt.PhpVal {
	...
	return rt.call_function('array_map', [rt.new_closure(closure_1_fn), rt.func_array_keys(var_payment_details), var_payment_details])
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CheckoutSchema) get_additional_fields_response(mut var_order Class_Automattic_WooCommerce_StoreApi_Schemas_V1_WC_Order) rt.PhpVal {
	...
	mut var_additional_field_schema := this.get_additional_fields_schema()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(var_additional_field_schema.array_get(var_key).array_isset(rt.new_string('type')) && rt.is_true(rt.identical(rt.new_string('boolean'), var_additional_field_schema.array_get(var_key).array_get('type'))))) {
				var_fields.array_set(var_key, // unsupported expression: Expr_Cast_Bool)
			} else {
				var_fields.array_set(var_key, this.prepare_html_response(var_value.dup()))
			}
		}
	}
	return // unsupported expression: Expr_Cast_Object
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CheckoutSchema) generate_additional_fields_schema(var_args rt.PhpVal) rt.PhpVal {
	mut var_additional_fields := rt.call_function('array_merge', [var_args.dup()])
	...
	return var_option.array_get('value')
	}
	mut var_option := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return var_option.array_get('value')
	}
				var_field_schema.array_set('enum', rt.call_function('array_map', [rt.new_closure(closure_3_fn), var_field.array_get('options')]))
				if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) || rt.is_true(rt.call_method(this.additional_fields_controller, 'is_conditional_field', [var_field.dup()])))) {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('checkbox'), var_field.array_get('type'))) && rt.is_true(rt.identical(rt.new_bool(true), var_field.array_get('required'))))) {
				var_field_schema.array_get_mut('enum').array_push(true)
			}
			var_schema.array_set(var_key, var_field_schema.dup())
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CheckoutSchema) schema_has_required_property(var_additional_fields_schema rt.PhpVal) rt.PhpVal {
	closure_5_fn := fn [var_additional_fields_schema] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_carry := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	mut var_key := if args.len > 1 { args[1].dup() } else { rt.new_null() }
	return rt.new_bool(rt.is_true(var_carry) || rt.is_true(rt.identical(rt.new_bool(true), var_additional_fields_schema.array_get(var_key).array_get('required'))))
	}
	return rt.call_function('array_reduce', [rt.func_array_keys(var_additional_fields_schema.dup()), rt.new_closure(closure_5_fn), rt.new_bool(false)])
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CheckoutSchema) sanitize_additional_fields(var_fields rt.PhpVal) rt.PhpVal {
	...
	closure_6_fn := fn [var_fields, var_properties] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_carry := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	mut var_key := if args.len > 1 { args[1].dup() } else { rt.new_null() }
	...
	mut var_rest_sanitized := rt.call_function('rest_sanitize_value_from_schema', [rt.call_function('wp_unslash', [var_fields_mutated.array_get(var_key)]), var_field_schema.dup(), var_key.dup()])
	var_rest_sanitized = rt.call_method(this.additional_fields_controller, 'sanitize_field', [var_key.dup(), var_rest_sanitized.dup()])
	var_carry.array_set(var_key, var_rest_sanitized.dup())
	return var_carry.dup()
	}
	var_fields_mutated = var_sanitization_utils.wp_kses_array(rt.call_function('array_reduce', [rt.func_array_keys(var_fields_mutated.dup()), rt.new_closure(closure_6_fn), rt.new_array()]))
	return var_sanitization_utils.wp_kses_array(var_fields_mutated.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CheckoutSchema) validate_additional_fields(var_fields rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	var_fields_mutated = this.sanitize_additional_fields(var_fields_mutated.dup())
	mut var_additional_field_schema := this.get_additional_fields_schema()
	if rt.is_true(rt.identical(rt.call_method(var_request, 'get_method', []rt.PhpVal{}), rt.new_string('PUT'))) {
		var_additional_field_schema = rt.call_function('array_intersect_key', [var_additional_field_schema.dup(), var_fields_mutated.dup()])
	}
	{
		mut iter_1 := var_additional_field_schema.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_schema := item_1.val
			mut var_key := item_1.key
			if rt.is_true(rt.new_bool(!(var_fields_mutated.array_isset(var_key)) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
				continue
			}
			mut var_result := rt.call_function('rest_validate_value_from_schema', [if !(var_fields_mutated.array_get(var_key)).is_null() { var_fields_mutated.array_get(var_key) } else { rt.new_null() }, var_schema.dup(), var_key.dup()])
			if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_wp_error', [var_result.dup()])) && rt.is_true(rt.call_method(var_result, 'has_errors', []rt.PhpVal{})))) {
				mut var_location := rt.call_method(this.additional_fields_controller, 'get_field_location', [var_key.dup()])
				{
					mut iter_2 := rt.call_method(var_result, 'get_error_codes', []rt.PhpVal{}).iterator()
					for {
						item_2 := iter_2.next() or { break }
						mut var_code := item_2.val
						rt.call_method(var_result, 'add_data', [rt.create_array([rt.ArrayItem{ key: 'location', val: var_location }, rt.ArrayItem{ key: 'key', val: var_key }]), var_code.dup()])
					}
				}
				var_errors.merge_from(var_result.dup())
			}
		}
	}
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_schemas_v1_checkoutschema_php() {
