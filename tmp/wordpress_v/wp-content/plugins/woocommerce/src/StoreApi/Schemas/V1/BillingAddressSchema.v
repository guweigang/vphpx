...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_BillingAddressSchema) get_properties() rt.PhpVal {
	mut var_properties := this.Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractAddressSchema.get_properties()
	return rt.call_function('array_merge', [var_properties.dup(), rt.create_array([rt.ArrayItem{ key: 'email', val: rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Email'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'context', val: rt.create_array([rt.ArrayItem{ key: none, val: 'view' }, rt.ArrayItem{ key: none, val: 'edit' }]) }, rt.ArrayItem{ key: 'required', val: true }]) }])])
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_BillingAddressSchema) sanitize_callback(var_address rt.PhpVal, var_request rt.PhpVal, var_param rt.PhpVal) rt.PhpVal {
	...
	var_address_mutated = this.Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractAddressSchema.sanitize_callback(var_address_mutated.dup(), var_request.dup(), var_param.dup())
	if var_address_mutated.array_isset(rt.new_string('email')) {
		var_address_mutated.array_set('email', rt.call_function('sanitize_email', [rt.call_function('wp_unslash', [var_address_mutated.array_get('email')])]))
	}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_BillingAddressSchema) validate_callback(var_address rt.PhpVal, var_request rt.PhpVal, var_param rt.PhpVal) rt.PhpVal {
	mut var_address_mutated := var_address
	mut var_errors := this.Class_Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractAddressSchema.validate_callback(var_address_mutated.dup(), var_request.dup(), var_param.dup())
	var_address_mutated = rt.cast_array(var_address_mutated)
	var_errors = if rt.is_true(rt.call_function('is_wp_error', [var_errors.dup()])) { var_errors } else { create_automattic_woocommerce_storeapi_schemas_v1_wp_error() }
	if rt.is_true(rt.new_bool(!(!rt.is_true(var_address_mutated.array_get('email'))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_email', [var_address_mutated.array_get('email')]))))))) {
		rt.call_method(var_errors, 'add', [rt.new_string('invalid_email'), rt.call_function('__', [rt.new_string('The provided email address is not valid'), rt.new_string('woocommerce')])])
	}
	return if rt.is_true(rt.call_method(var_errors, 'has_errors', [var_errors.dup()])) { var_errors } else { rt.new_bool(true) }
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_BillingAddressSchema) get_item_response(var_address rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.instance_of(var_address_mutated, 'Automattic_WooCommerce_StoreApi_Schemas_V1_WC_Customer'))) || rt.is_true(rt.new_bool(rt.instance_of(var_address_mutated, 'Automattic_WooCommerce_StoreApi_Schemas_V1_WC_Order'))))) {
		mut var_billing_country := rt.call_method(var_address_mutated, 'get_billing_country', []rt.PhpVal{})
		mut var_billing_state := rt.call_method(var_address_mutated, 'get_billing_state', []rt.PhpVal{})
		if rt.is_true(rt.new_bool(!(rt.is_true(var_validation_util.validate_state(var_billing_state.dup(), var_billing_country.dup()))))) {
			var_billing_state = rt.new_string(rt.new_string(''))
		}
		mut var_additional_address_fields := rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Schemas_V1_BillingAddressSchema', ['Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractAddressSchema'], &this), 'additional_fields_controller'), 'get_all_fields_from_object', [var_address_mutated.dup(), rt.new_string('billing')])
		mut var_address_object := rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: 'first_name', val: rt.call_method(var_address_mutated, 'get_billing_first_name', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'last_name', val: rt.call_method(var_address_mutated, 'get_billing_last_name', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'company', val: rt.call_method(var_address_mutated, 'get_billing_company', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'address_1', val: rt.call_method(var_address_mutated, 'get_billing_address_1', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'address_2', val: rt.call_method(var_address_mutated, 'get_billing_address_2', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'city', val: rt.call_method(var_address_mutated, 'get_billing_city', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'state', val: var_billing_state }, rt.ArrayItem{ key: 'postcode', val: rt.call_method(var_address_mutated, 'get_billing_postcode', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'country', val: var_billing_country }, rt.ArrayItem{ key: 'email', val: rt.call_method(var_address_mutated, 'get_billing_email', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'phone', val: rt.call_method(var_address_mutated, 'get_billing_phone', []rt.PhpVal{}) }]), var_additional_address_fields.dup()])
		{
			mut iter_1 := rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Schemas_V1_BillingAddressSchema', ['Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractAddressSchema'], &this), 'additional_fields_controller'), 'get_address_fields_keys', []rt.PhpVal{}).iterator()
			for {
	...
	rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Exceptions_RouteException', []string{}, create_automattic_woocommerce_storeapi_exceptions_routeexception(rt.new_string('invalid_object_type'), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('%1$s requires an instance of %2$s or %3$s for the address'), rt.new_string('woocommerce')]), rt.new_string('BillingAddressSchema::get_item_response'), rt.new_string('WC_Customer'), rt.new_string('WC_Order')]), rt.new_int(500))))
	return rt.new_null()
}
...
fn create_automattic_woocommerce_storeapi_schemas_v1_billingaddressschema() &Class_Automattic_WooCommerce_StoreApi_Schemas_V1_BillingAddressSchema {
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_schemas_v1_billingaddressschema_php() {
}
