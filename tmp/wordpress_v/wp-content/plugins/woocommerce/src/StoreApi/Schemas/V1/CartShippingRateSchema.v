...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartShippingRateSchema) get_item_response(var_package rt.PhpVal) rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'package_id', val: var_package.array_get('package_id') },
		rt.ArrayItem{ key: 'name', val: var_package.array_get('package_name') },
		rt.ArrayItem{
			key: 'destination'
			val: this.prepare_package_destination_response(var_package.dup())
		},
		rt.ArrayItem{ key: 'items', val: this.prepare_package_items_response(var_package.dup()) },
		rt.ArrayItem{
			key: 'shipping_rates'
			val: this.prepare_package_shipping_rates_response(var_package.dup())
		},
	])
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartShippingRateSchema) prepare_package_destination_response(var_package rt.PhpVal) rt.PhpVal {
	mut var_address := if var_package.array_get('destination').array_isset(rt.new_string('address_1')) {
		var_package.array_get('destination').array_get('address_1')
	} else {
		var_package.array_get('destination').array_get('address')
	}
	return
}
	...
	mut var_items := rt.new_array()
	{
		...
		for {
			...
			var_items.array_push(rt.create_array([
				rt.ArrayItem{ key: 'key', val: var_values.array_get('key') },
				rt.ArrayItem{ key: 'name', val: rt.call_method(var_values.array_get('data'),
					'get_name', []rt.PhpVal{}) },
				rt.ArrayItem{ key: 'quantity', val: var_values.array_get('quantity') },
			]))
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartShippingRateSchema) prepare_package_shipping_rates_response(var_package rt.PhpVal) rt.PhpVal {
	mut var_rates := var_package.array_get('rates')
	mut var_selected_rates := rt.call_method(rt.get_property(rt.call_function('wc', []rt.PhpVal{}),
		...
		rt.new_array()])
	mut var_selected_rate := if var_selected_rates.array_isset(var_package.array_get('package_id')) {
		var_selected_rates.array_get(var_package.array_get('package_id'))
	} else {
		rt.new_string('')
	}
	if !rt.is_true(var_selected_rate) && !(!rt.is_true(var_package.array_get('rates'))) {
		var_selected_rate = rt.call_function('wc_get_chosen_shipping_method_for_package', [
			var_package.array_get('package_id'),
			var_package.dup(),
		])
	}
	mut var_response := rt.new_array()
	{
		mut iter_1 := var_package.array_get('rates').iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_rate := item_1.val
			var_response.array_push(this.get_rate_response(var_rate.dup(), var_selected_rate.str()))
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartShippingRateSchema) get_rate_response(var_rate rt.PhpVal, selected_rate string) rt.PhpVal {
	...
	return this.prepare_currency_response(rt.create_array([
		rt.ArrayItem{ key: 'rate_id', val: this.get_rate_prop(var_rate.dup(), rt.new_string('id')) },
		rt.ArrayItem{ key: 'name', val: this.prepare_html_response(this.get_rate_prop(var_rate.dup(),
			rt.new_string('label'))) },
		rt.ArrayItem{ key: 'description', val: this.prepare_html_response(this.get_rate_prop(var_rate.dup(),
			rt.new_string('description'))) },
		rt.ArrayItem{ key: 'delivery_time', val: this.prepare_html_response(this.get_rate_prop(var_rate.dup(),
			rt.new_string('delivery_time'))) },
		rt.ArrayItem{ key: 'price', val: this.prepare_money_response(this.get_rate_prop(var_rate.dup(),
		...
		rt.ArrayItem{ key: 'taxes', val: this.prepare_money_response(rt.call_function('array_sum', [
			rt.cast_array(this.get_rate_prop(var_rate.dup(), rt.new_string('taxes'))),
		]), rt.call_function('wc_get_price_decimals', []rt.PhpVal{})) },
		rt.ArrayItem{ key: 'instance_id', val: this.get_rate_prop(var_rate.dup(),
			rt.new_string('instance_id')) },
		rt.ArrayItem{ key: 'method_id', val: this.get_rate_prop(var_rate.dup(),
			rt.new_string('method_id')) },
		rt.ArrayItem{ key: 'meta_data', val: this.get_rate_meta_data(var_rate.dup()) },
		rt.ArrayItem{ key: 'selected', val: rt.identical(rt.new_string(selected_rate_mutated), this.get_rate_prop(var_rate.dup(),
			rt.new_string('id'))) },
	]))
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartShippingRateSchema) get_rate_meta_data(var_rate rt.PhpVal) rt.PhpVal {
	...
	closure_1_fn := fn [var_meta_data] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_return := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	...
	return rt.call_function('array_reduce', [rt.func_array_keys(var_meta_data.dup()),
...
fn create_automattic_woocommerce_storeapi_schemas_v1_cartshippingrateschema() &Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartShippingRateSchema {
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_schemas_v1_cartshippingrateschema_php() {
}
