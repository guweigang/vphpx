...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductBrandSchema) construct(mut var_extend Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema, mut var_controller Class_Automattic_WooCommerce_StoreApi_SchemaController)  {
	...
	this.image_attachment_schema = rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Schemas_V1_ProductBrandSchema', ['Automattic_WooCommerce_StoreApi_Schemas_V1_TermSchema'], &this), 'controller'), 'get', [Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ImageAttachmentSchema.identifier()])
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductBrandSchema) get_item_response(var_term rt.PhpVal) rt.PhpVal {
	...
	mut var_count := rt.call_function('get_term_meta', [rt.get_property(var_term, 'term_id'), rt.new_string('product_count_product_brand'), rt.new_bool(true)])
	...
	var_response.array_set('review_count', this.get_brand_review_count(var_term.dup()))
	var_response.array_set('permalink', rt.call_function('get_term_link', [rt.get_property(var_term, 'term_id'), rt.new_string('product_brand')]))
	return var_response.dup()
}

fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductBrandSchema) get_brand_review_count(var_term rt.PhpVal) rt.PhpVal {
	mut var_wpdb := rt.new_null()
	// unsupported statement: Stmt_Global
	mut var_children := rt.call_function('get_term_children', [rt.get_property(var_term, 'term_id'), rt.new_string('product_brand')])
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductBrandSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_brand_review_count' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			return this.get_brand_review_count(dispatch_arg_0)
		}
		else { return none }
	}
}
...
pub fn init_wp_content_plugins_woocommerce_src_storeapi_schemas_v1_productbrandschema_php() {
