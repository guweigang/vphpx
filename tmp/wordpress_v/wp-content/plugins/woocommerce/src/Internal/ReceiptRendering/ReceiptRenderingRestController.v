...
fn (mut this Class_Automattic_WooCommerce_Internal_ReceiptRendering_ReceiptRenderingRestController) register_routes() {
	...
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			return this.run(var_request.dup(), rt.new_string('create_order_receipt'))
		}
		mut var_request := if args.len > 0 { args[0].dup() } else { rt.new_null() }
		return this.check_permission(var_request.dup(), rt.new_string('read_shop_order'), rt.call_method(var_request,
	...
	closure_4_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_request := if args.len > 0 { args[0].dup() } else { rt.new_null() }
			return this.run(var_request.dup(), rt.new_string('get_order_receipt'))
		}
		mut var_request := if args.len > 0 { args[0].dup() } else { rt.new_null() }
		return this.check_permission(var_request.dup(), rt.new_string('read_shop_order'), rt.call_method(var_request,
			'get_param', [rt.new_string('id')]))
...
fn (mut this Class_Automattic_WooCommerce_Internal_ReceiptRendering_ReceiptRenderingRestController) get_order_receipt(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	]), 'get_existing_receipt', [var_order_id.dup()])
	return if rt.is_true(rt.new_bool(var_filename.dup().is_null())) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_ReceiptRendering_ReceiptRenderingRestController) create_order_receipt(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	mut var_filename := rt.call_method(rt.call_method(rt.call_function('wc_get_container',
		[]rt.PhpVal{}), 'get', [
		Class_Automattic_WooCommerce_Internal_ReceiptRendering_ReceiptRenderingEngine.class(),
	]), 'generate_receipt', [var_order_id.dup(), var_expiration_date.dup(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_ReceiptRendering_ReceiptRenderingRestController) get_response_for_file(filename string) rt.PhpVal {
	...
	mut var_expiration_date := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Internal_TransientFiles_TransientFilesEngine{}
		return temp.get_expiration_date(arg_0)
	}(rt.new_string(filename_mutated))
	...
	]), 'get_public_url', [rt.new_string(filename_mutated).dup()])
...
fn create_automattic_woocommerce_internal_receiptrendering_receiptrenderingrestcontroller() &Class_Automattic_WooCommerce_Internal_ReceiptRendering_ReceiptRenderingRestController {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_receiptrendering_receiptrenderingrestcontroller_php() {
}
