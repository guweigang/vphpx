...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_QueryCache) resolve(mut var_query Class_Automattic_WooCommerce_Internal_Api_?string, mut var_extensions Class_Automattic_WooCommerce_Internal_Api_array) rt.PhpVal {
	mut var_apq := if !(var_extensions.array_get('persistedQuery')).is_null() { var_extensions.array_get('persistedQuery') } else { rt.new_null() }
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_apq.dup().is_array())) && rt.is_true(rt.identical(rt.new_int(1), if !(var_apq.array_get('version')).is_null() { var_apq.array_get('version') } else { rt.new_null() })))) && !(!rt.is_true(var_apq.array_get('sha256Hash'))))) {
		return this.resolve_apq(mut var_query, (var_apq.array_get('sha256Hash')).str())
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_QueryCache) get_cached_document(hash string) bool {
	...
	mut var_cached := rt.call_function('wp_cache_get', [this.build_cache_key(hash_mutated), Class_Automattic_WooCommerce_Internal_Api_Automattic_WooCommerce_Internal_Api_QueryCache.cache_group()])
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_bool(false), var_cached)) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_cached.dup().is_array()))))))) {
		return false
	}
	return (fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_AST{}; return temp.fromarray(arg_0) }(var_cached.dup())).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Api_QueryCache) parse_and_cache(query string, hash string) rt.PhpVal {
	mut hash_mutated := hash
	mut var_document := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Language_Parser{}; return temp.parse(arg_0, arg_1) }(rt.new_string(query), rt.create_array([rt.ArrayItem{ key: 'noLocation', val: true }]))
...
end_label_1:
	rt.call_function('wp_cache_set', [this.build_cache_key(hash_mutated), rt.call_method(var_document, 'toArray', []rt.PhpVal{}), Class_Automattic_WooCommerce_Internal_Api_Automattic_WooCommerce_Internal_Api_QueryCache.cache_group(), Class_Automattic_WooCommerce_Internal_Api_QueryCache.get_cache_ttl()])
	return var_document.dup()
}
...
fn create_automattic_woocommerce_internal_api_querycache() &Class_Automattic_WooCommerce_Internal_Api_QueryCache {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_api_querycache_php() {
