...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_GraphQLController) init(mut var_query_cache Class_Automattic_WooCommerce_Internal_Api_QueryCache)  {
	this.query_cache = var_query_cache.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_GraphQLController) register()  {
	mut var_methods := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Api_Main{}; return temp.filter_methods_against_settings(arg_0) }(rt.create_array([rt.ArrayItem{ key: none, val: 'GET' }, rt.ArrayItem{ key: none, val: 'POST' }]))
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Internal_Api_Throwable') {
		...
		mut var_status := rt.new_int(this.get_error_status(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Api_array](var_output.array_get('errors'))))
		return create_automattic_woocommerce_internal_api_wp_rest_response(var_output.dup(), var_status.dup())
		unsafe { goto end_label_1 }
	}
	else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_GraphQLController) process_request(mut var_request Class_Automattic_WooCommerce_Internal_Api_WP_REST_Request) rt.PhpVal {
	...
	mut var_source := rt.call_method(this.query_cache, 'resolve', [var_query.dup(), var_extensions.dup()])
	if rt.is_true(rt.new_bool(var_source.dup().is_array())) {
		return create_automattic_woocommerce_internal_api_wp_rest_response(var_source.dup(), this.get_resolve_error_status(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Api_array](var_source)))
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('GET'), var_request.get_method())) && this.document_has_mutation(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_DocumentNode](var_source), mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Api_?string](var_operation_name)))) {
		return create_automattic_woocommerce_internal_api_wp_rest_response(rt.create_array([rt.ArrayItem{ key: 'errors', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'message', val: 'Mutations are not allowed over GET requests. Use POST instead.' }, rt.ArrayItem{ key: 'extensions', val: rt.create_array([rt.ArrayItem{ key: 'code', val: 'METHOD_NOT_ALLOWED' }]) }]) }]) }]), rt.new_int(405))
	}
	...
	mut var_validation_rules := rt.call_function('array_values', [fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_DocumentValidator{}; return temp.allrules() }()])
	...
	var_validation_rules.array_push(var_complexity_rule.dup())
	...
	mut var_result := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal, arg_3 rt.PhpVal, arg_4 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Vendor_GraphQL_GraphQL{}; return temp.executequery(arg_0, arg_1, arg_2, arg_3, arg_4) }(var_schema.dup(), var_source.dup(), var_variables.dup(), var_operation_name.dup(), var_validation_rules.dup())
	mut var_debug_mode := rt.new_bool(this.is_debug_mode(mut var_request))
	closure_1_fn := fn [var_debug_mode] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_error := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	mut var_formatted := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Api_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError{}; return temp.createfromexception(arg_0) }(var_error.dup())
	if !(var_formatted.array_get('extensions').array_isset(rt.new_string('code'))) {
		mut var_client_safe := rt.new_bool(rt.new_bool(rt.is_true(rt.new_bool(rt.instance_of(var_error, 'Automattic_WooCommerce_Internal_Api_Automattic_WooCommerce_Vendor_GraphQL_Error_ClientAware'))) && rt.is_true(rt.call_method(var_error, 'isClientSafe', []rt.PhpVal{}))))
	...
	if rt.is_true(var_debug_mode) {
		...
		if !(!rt.is_true(var_chain)) {
			var_formatted.array_get_mut('extensions').array_set('previous', var_chain.dup())
		}
	}
	return var_formatted.dup()
	}
	rt.call_method(var_result, 'setErrorFormatter', [rt.new_closure(closure_1_fn)])
	mut var_debug_flags := rt.new_int(this.get_debug_flags(mut var_request))
	mut var_output := rt.call_method(var_result, 'toArray', [var_debug_flags.dup()])
	if this.is_debug_mode(mut var_request) {
		...
		if !(var_output.array_get('extensions').array_isset(rt.new_string('debug'))) {
	...
	mut var_status := rt.new_int(if var_output.array_isset(rt.new_string('errors')) { this.get_error_status(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Api_array](var_output.array_get('errors'))) } else { rt.new_int(200) })
	return create_automattic_woocommerce_internal_api_wp_rest_response(var_output.dup(), var_status.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_GraphQLController) decode_json_param(var_value rt.PhpVal, name string) rt.PhpVal {
	...
	mut var_decoded := rt.call_function('json_decode', [var_value.dup(), rt.new_bool(true)])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_decoded.dup().is_array()))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_Api_InvalidArgumentException', []string{}, create_automattic_woocommerce_internal_api_invalidargumentexception(rt.call_function('sprintf', [rt.new_string('Argument `%s` must be a JSON object (got %s).'), rt.new_string(name), rt.call_function('gettype', [var_decoded.dup()])]))))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_GraphQLController) is_debug_mode(mut var_request Class_Automattic_WooCommerce_Internal_Api_WP_REST_Request) bool {
	if rt.is_true(rt.new_bool(!(this.is_local_environment()) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('manage_options')]))))))) {
		return false
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_GraphQLController) format_exception(mut var_e Class_Automattic_WooCommerce_Internal_Api_Throwable, mut var_request Class_Automattic_WooCommerce_Internal_Api_WP_REST_Request) rt.PhpVal {
	...
	if this.is_debug_mode(mut var_request) {
		...
		if !(!rt.is_true(var_chain)) {
			var_error.array_get_mut('extensions').array_get_mut('debug').array_set('previous', var_chain.dup())
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_GraphQLController) extract_previous_chain(mut var_e Class_Automattic_WooCommerce_Internal_Api_Throwable) rt.PhpVal {
	mut var_chain := rt.new_array()
	{
		...
		for {
			if !(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical)) { break }
			var_chain.array_push(rt.create_array([rt.ArrayItem{ key: 'class', val: rt.call_function('get_class', [var_prev.dup()]) }, rt.ArrayItem{ key: 'message', val: rt.call_method(var_prev, 'getMessage', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'file', val: rt.call_method(var_prev, 'getFile', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'line', val: rt.call_method(var_prev, 'getLine', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'trace', val: rt.call_function('explode', [rt.new_string('\n'), rt.call_method(var_prev, 'getTraceAsString', []rt.PhpVal{})]) }]))
			var_prev = rt.call_method(var_prev, 'getPrevious', []rt.PhpVal{})
		}
	}
	return var_chain.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_GraphQLController) get_error_status(mut var_errors Class_Automattic_WooCommerce_Internal_Api_array) i64 {
	mut var_status := rt.new_int(rt.new_int(200))
	{
		mut iter_1 := var_errors.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_error := item_1.val
			mut var_code := if !(var_error.array_get('extensions').array_get('code')).is_null() { var_error.array_get('extensions').array_get('code') } else { rt.new_null() }
...
fn create_automattic_woocommerce_internal_api_graphqlcontroller() &Class_Automattic_WooCommerce_Internal_Api_GraphQLController {
...
fn create_automattic_woocommerce_internal_api_wp_rest_response() &Class_Automattic_WooCommerce_Internal_Api_WP_REST_Response {
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_querycomplexity() &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryComplexity {
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_querydepth() &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryDepth {
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_disableintrospection() &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_DisableIntrospection {
...
fn create_automattic_woocommerce_internal_api_automattic_woocommerce_vendor_graphql_error_formattederror() &Class_Automattic_WooCommerce_Internal_Api_Automattic_WooCommerce_Vendor_GraphQL_Error_FormattedError {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_api_graphqlcontroller_php() {
