...
fn (mut this Class_Automattic_WooCommerce_Internal_AddressProvider_AddressProviderController) get_registered_providers() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(var_provider_items)
		&& rt.is_true(rt.new_bool(var_provider_items.dup().is_array()))))
	{
		return rt.new_array()
	}
	mut var_logger := rt.call_function('wc_get_logger', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_provider_items.dup().is_array()))))) {
	...
	mut var_seen_ids := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_provider_item.dup().is_string()))
				&& rt.is_true(rt.call_function('class_exists', [var_provider_item.dup()]))))
			{
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [
				var_provider_item.dup(), Class_WC_Address_Provider.class()])))))
			{
				rt.call_method(var_logger, 'error', [
					rt.call_function('sprintf', [
						rt.new_string('Invalid address provider item "%s", expected a string class name or WC_Address_Provider instance.'),
						if rt.is_true(rt.new_bool(var_provider_item.dup().is_object())) { rt.call_function('get_class', [
								var_provider_item.dup(),
			...
			if !rt.is_true(rt.get_property(var_provider_item, 'id'))
				...
				rt.call_method(var_logger, 'error', [
						...
						(rt.call_function('get_class', [var_provider_item.dup()])).str(),
					rt.create_array([
						rt.ArrayItem{ key: 'context', val: 'address_provider_service' },
					]),
				])
			...
			if var_seen_ids.array_isset(rt.get_property(var_provider_item, 'id')) {
				rt.call_method(var_logger, 'error', [
					rt.call_function('sprintf', [
						...
						rt.call_function('get_class', [var_provider_item.dup()]),
					]),
					rt.create_array([
						rt.ArrayItem{ key: 'context', val: 'address_provider_service' },
					]),
			...
			var_providers.array_push(var_provider_item.dup())
		}
	}
	if !(!rt.is_true(this.preferred_provider_option)) && !(!rt.is_true(var_providers)) {
		{
			...
			for {
					...
					this.preferred_provider_option))
				{
					...
					rt.call_function('array_unshift', [var_providers.dup(),
						var_preferred_provider.dup()])
					break
				}
			}
		}
...
fn (mut this Class_Automattic_WooCommerce_Internal_AddressProvider_AddressProviderController) get_preferred_provider() string {
	...
	return (if !(rt.get_property(this.providers.array_get(0), 'id')).is_null() {
		rt.get_property(this.providers.array_get(0), 'id')
	} else {
		rt.new_string('')
	}).str()
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_addressprovider_addressprovidercontroller_php() {
