...
struct Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer {
	rt.PhpObjectBase
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return !(rt.is_true(rt.call_function('wc_string_to_bool', [var_payment_gateway.get_option(rt.new_string('live_mode'))])))
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	mut var_sandbox_prefix := rt.new_string(if this.is_in_test_mode(mut var_payment_gateway) { rt.new_string('sandbox_') } else { rt.new_string('') })
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	return !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('merchant_code')))) && !(!rt.is_true(var_payment_gateway.get_option(rt.new_string((var_sandbox_prefix).str() + 'merchant_token')))) && !(!rt.is_true(var_payment_gateway.get_option(rt.new_string((var_sandbox_prefix).str() + 'store_code'))))
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer) is_in_test_mode_onboarding(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_payoneer() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer {
	mut obj := &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer{
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'is_in_test_mode' {
...
fn (this &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_payoneer_php() {
