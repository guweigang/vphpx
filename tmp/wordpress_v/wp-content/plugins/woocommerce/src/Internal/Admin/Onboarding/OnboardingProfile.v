...
fn Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingProfile.init() {
	rt.call_function('add_action', [
		'update_option_' +(Class_Automattic_WooCommerce_Internal_Admin_Onboarding_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingProfile.data_option()).str(),
...
fn Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingProfile.trigger_complete(var_old_value rt.PhpVal, var_value rt.PhpVal) {
	if rt.is_true(rt.new_bool(var_old_value.array_isset(rt.new_string('completed'))
		&& rt.is_true(var_old_value.array_get('completed'))))
	{
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(!(var_value.array_isset(rt.new_string('completed')))
		|| rt.is_true(rt.new_bool(!(rt.is_true(var_value.array_get('completed')))))))
	{
		return rt.new_null()
	}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingProfile.needs_completion() bool {
	...
	mut var_is_completed := rt.new_bool(rt.new_bool(
		var_onboarding_data.array_isset(rt.new_string('completed'))
		&& rt.is_true(rt.identical(rt.new_bool(true), var_onboarding_data.array_get('completed')))))
	mut var_is_skipped := rt.new_bool(rt.new_bool(
		var_onboarding_data.array_isset(rt.new_string('skipped'))
		&& rt.is_true(rt.identical(rt.new_bool(true), var_onboarding_data.array_get('skipped')))))
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_onboarding_onboardingprofile_php() {
}
