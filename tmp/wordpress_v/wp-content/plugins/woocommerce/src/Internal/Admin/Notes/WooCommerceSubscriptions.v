...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommerceSubscriptions.get_note() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(var_onboarding_data.array_isset(rt.new_string('product_types'))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string('subscriptions'), var_onboarding_data.array_get('product_types'), rt.new_bool(true)]))))))) {
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommerceSubscriptions{}; return temp.is_wc_admin_active_in_date_range(arg_0, arg_1) }(rt.new_string('week-1'), rt.get_constant('DAY_IN_SECONDS')))))) {
		return rt.new_null()
	}
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [rt.new_string('Do you need more info about WooCommerce Subscriptions?'), rt.new_string('woocommerce')]))
	var_note.set_content(rt.call_function('__', [rt.new_string('WooCommerce Subscriptions allows you to introduce a variety of subscriptions for physical or virtual products and services. Create product-of-the-month clubs, weekly service subscriptions or even yearly software billing packages. Add sign-up fees, offer free trials, or set expiration periods.'), rt.new_string('woocommerce')]))
	...
	var_note.add_action(rt.new_string('learn-more'), rt.call_function('__', [rt.new_string('Learn More'), rt.new_string('woocommerce')]), rt.new_string('https://woocommerce.com/products/woocommerce-subscriptions/?utm_source=inbox&utm_medium=product'), Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_unactioned(), rt.new_bool(true))
	return mut var_note
}
...
fn create_automattic_woocommerce_internal_admin_notes_woocommercesubscriptions() &Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommerceSubscriptions {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_woocommercesubscriptions_php() {
