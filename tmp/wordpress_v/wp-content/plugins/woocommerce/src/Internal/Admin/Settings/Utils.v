...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.order_map_move_at_order(mut var_order_map Class_Automattic_WooCommerce_Internal_Admin_Settings_array, id string, order i64) rt.PhpVal {
	...
	if rt.is_true(rt.identical(var_order_map_mutated.array_get(id), rt.new_int(order))) {
		return rt.new_object('Automattic_WooCommerce_Internal_Admin_Settings_array', []string{}, var_order_map_mutated)
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_int(order), var_order_map_mutated.dup(), rt.new_bool(true)]))))) {
		var_order_map_mutated.array_set(id, order)
		return rt.new_object('Automattic_WooCommerce_Internal_Admin_Settings_array', []string{}, var_order_map_mutated)
	}
	mut var_existing_order := var_order_map_mutated.array_get(id)
	if rt.is_true(rt.greater(rt.new_int(order), var_existing_order)) {
		{
			mut iter_1 := var_order_map_mutated.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_value := item_1.val
				mut var_key := item_1.key
				if rt.is_true(rt.new_bool(rt.is_true(rt.less_equal(var_value, rt.new_int(order))) && rt.is_true(rt.greater_equal(var_value, var_existing_order)))) {
	...
	} else {
		{
			...
			for {
				...
				mut var_value := item_1.val
				mut var_key := item_1.key
				if rt.is_true(rt.new_bool(rt.is_true(rt.greater_equal(var_value, rt.new_int(order))) && rt.is_true(rt.less_equal(var_value, var_existing_order)))) {
...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.order_map_place_at_order(mut var_order_map Class_Automattic_WooCommerce_Internal_Admin_Settings_array, id string, order i64) rt.PhpVal {
	mut var_order_map_mutated := var_order_map
	if rt.is_true(rt.new_bool(var_order_map_mutated.array_isset(rt.new_string(id)) && rt.is_true(rt.identical(var_order_map_mutated.array_get(id), rt.new_int(order))))) {
		return rt.new_object('Automattic_WooCommerce_Internal_Admin_Settings_array', []string{}, var_order_map_mutated)
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_int(order), var_order_map_mutated.dup(), rt.new_bool(true)]))))) {
		var_order_map_mutated.array_set(id, order)
...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.order_map_normalize(mut var_order_map Class_Automattic_WooCommerce_Internal_Admin_Settings_array) rt.PhpVal {
	...
	return rt.new_bool(var_key.dup().is_string())
	}
	var_order_map_mutated = rt.call_function('array_filter', [var_order_map_mutated.dup(), rt.new_closure(closure_1_fn), rt.get_constant('ARRAY_FILTER_USE_KEY')])
	rt.call_function('asort', [var_order_map_mutated.dup()])
	return rt.call_function('array_flip', [rt.func_array_keys(var_order_map_mutated.dup())])
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.order_map_change_min_order(mut var_order_map Class_Automattic_WooCommerce_Internal_Admin_Settings_array, new_min_order i64) rt.PhpVal {
	...
	mut var_bump := rt.sub(rt.new_int(new_min_order), rt.call_function('min', [var_order_map_mutated.dup()]))
	{
		mut iter_1 := var_order_map_mutated.iterator()
		for {
			item_1 := iter_1.next() or { break }
	...
	rt.call_function('asort', [var_updated_map.dup()])
	return var_updated_map.dup()
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.generate_testing_plugin_slugs(slug string, include_original bool) rt.PhpVal {
	...
	{
		mut iter_1 := Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.get_testing_plugin_slug_suffixes().iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_suffix := item_1.val
			var_slugs.array_push(slug_mutated + (var_suffix).str())
		}
	}
	return var_slugs.dup()
}

fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.normalize_plugin_slug(slug string) string {
	mut var_matches := rt.new_null()
	mut slug_mutated := slug
	if rt.is_true(rt.new_bool(slug_mutated == '' || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('/^[\\w-]+$/'), rt.new_string(slug_mutated).dup(), var_matches.dup()]))))))) {
		return slug_mutated
	}
	slug_mutated = slug_mutated.to_lower()
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_suffix := item_1.val
			slug_mutated = (if rt.is_true(rt.call_function('str_ends_with', [rt.new_string(slug_mutated).dup(), var_suffix.dup()])) { rt.call_function('substr', [rt.new_string(slug_mutated).dup(), rt.new_int(0), // unsupported expression: Expr_UnaryMinus]) } else { rt.new_string(slug_mutated) }).str()
		}
	}
	return slug_mutated
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.trim_php_file_extension(path string) string {
	...
	if rt.is_true(rt.new_bool(!(path_mutated == '') && rt.is_true(rt.call_function('str_ends_with', [rt.new_string(path_mutated).dup(), rt.new_string('.php')])))) {
		path_mutated = (rt.call_function('substr', [rt.new_string(path_mutated).dup(), rt.new_int(0), // unsupported expression: Expr_UnaryMinus])).str()
	}
	return path_mutated
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.truncate_with_words(text string, target_length i64, append string) string {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('str_starts_with', [rt.call_function('wp_get_word_count_type', []rt.PhpVal{}), rt.new_string('characters')])) && rt.is_true(rt.call_function('preg_match', [rt.new_string('/^utf\\-?8$/i'), rt.call_function('get_option', [rt.new_string('blog_charset')])])))) {
		text_mutated = rt.call_function('preg_replace', [rt.new_string('/[\n\r\t ]+/'), rt.new_string(' '), rt.new_string(text_mutated).dup()]).to_string().trim_space()
		rt.call_function('preg_match_all', [rt.new_string('/./u'), rt.new_string(text_mutated).dup(), var_words_array.dup()])
		if var_words_array.array_get(0).array_count() <= target_length {
			return text_mutated
		}
		mut var_words_array := rt.call_function('array_slice', [var_words_array.array_get(0), rt.new_int(0), rt.new_int(target_length)])
		mut var_truncated := rt.call_function('implode', [rt.new_string(''), var_words_array.dup()])
	...
	var_words_array = rt.call_function('preg_split', [rt.new_string('/[\n\r\t ]+/'), rt.new_string(text_mutated).dup(), // unsupported expression: Expr_UnaryMinus, rt.get_constant('PREG_SPLIT_NO_EMPTY')])
	mut var_sep := rt.new_string(rt.new_string(' '))
	var_truncated = rt.new_string(rt.new_string(''))
	mut var_remaining_length := rt.new_int(rt.new_int(target_length))
	for rt.is_true(rt.new_bool(rt.is_true(rt.greater(var_remaining_length, rt.new_int(0))) && !(!rt.is_true(var_words_array)))) {
		mut var_word := rt.call_function('array_shift', [var_words_array.dup()])
		// unsupported expression: Expr_AssignOp_Concat
		// unsupported expression: Expr_AssignOp_Minus
	}
	var_truncated = rt.new_string(rt.new_string(var_truncated.dup().to_string().trim_right(' \t\n\r')))
...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.wc_payments_settings_url(mut var_path Class_Automattic_WooCommerce_Internal_Admin_Settings_?string, mut var_query Class_Automattic_WooCommerce_Internal_Admin_Settings_array) string {
	...
	var_path_mutated = rt.new_string(if rt.is_true(var_path_mutated) { '&path=' + (var_path_mutated).str() } else { rt.new_string('') })
	...
	return (rt.call_function('admin_url', ['admin.php?page=wc-settings&tab=checkout' + (var_path_mutated).str() + (var_query_string).str()])).str()
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.rest_endpoint_get_request(endpoint string, mut var_params Class_Automattic_WooCommerce_Internal_Admin_Settings_array) rt.PhpVal {
	mut var_request := create_wp_rest_request(rt.new_string('GET'), rt.new_string(endpoint).dup())
	...
	mut var_response_data := rt.call_function('json_decode', [rt.call_function('wp_json_encode', [rt.call_method(var_server, 'response_to_data', [var_response.dup(), rt.new_bool(false)])]), rt.new_bool(true)])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		return create_automattic_woocommerce_internal_admin_settings_wp_error(rt.new_string('woocommerce_settings_payments_rest_error'), rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('REST request GET %1$s failed with: (%2$s) %3$s'), rt.new_string('woocommerce')]), rt.new_string(endpoint), if !(var_response_data.array_get('code')).is_null() { var_response_data.array_get('code') } else { rt.new_string('unknown_error') }, if !(var_response_data.array_get('message')).is_null() { var_response_data.array_get('message') } else { rt.call_function('esc_html__', [rt.new_string('Unknown error'), rt.new_string('woocommerce')]) }]), var_response_data.dup())
	}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.rest_endpoint_post_request(endpoint string, mut var_params Class_Automattic_WooCommerce_Internal_Admin_Settings_array) rt.PhpVal {
	mut var_request := create_wp_rest_request(rt.new_string('POST'), rt.new_string(endpoint).dup())
	if rt.is_true(var_params) {
	...
	mut var_response_data := rt.call_function('json_decode', [rt.call_function('wp_json_encode', [rt.call_method(var_server, 'response_to_data', [var_response.dup(), rt.new_bool(false)])]), rt.new_bool(true)])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		return create_automattic_woocommerce_internal_admin_settings_wp_error(rt.new_string('woocommerce_settings_payments_rest_error'), rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('REST request POST %1$s failed with: (%2$s) %3$s'), rt.new_string('woocommerce')]), rt.new_string(endpoint), if !(var_response_data.array_get('code')).is_null() { var_response_data.array_get('code') } else { rt.new_string('unknown_error') }, if !(var_response_data.array_get('message')).is_null() { var_response_data.array_get('message') } else { rt.call_function('esc_html__', [rt.new_string('Unknown error'), rt.new_string('woocommerce')]) }]), var_response_data.dup())
	}
	return var_response_data.dup()
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils.get_wpcom_connection_authorization(return_url string) rt.PhpVal {
	mut var_result := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Jetpack_JetpackConnection{}; return temp.get_authorization_url(arg_0) }(rt.new_string(return_url))
	if !(!rt.is_true(var_result.array_get('url'))) {
		var_result.array_set('url', rt.call_function('add_query_arg', [rt.create_array([rt.ArrayItem{ key: 'from', val: 'woocommerce-onboarding' }, rt.ArrayItem{ key: 'plugin_name', val: 'woocommerce-payments' }, rt.ArrayItem{ key: 'color_scheme', val: .array_get() }]), var_result.array_get('url')]))
	}
...
fn create_automattic_woocommerce_internal_admin_settings_wp_error() &Class_Automattic_WooCommerce_Internal_Admin_Settings_WP_Error {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_utils_php() {
