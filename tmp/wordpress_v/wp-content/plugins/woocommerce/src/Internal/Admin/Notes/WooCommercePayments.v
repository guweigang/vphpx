...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments) construct()  {
	rt.call_function('add_action', [rt.new_string('init'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'install_on_action' }])])
	rt.call_function('add_action', [rt.new_string('wc-admin-woocommerce-payments_add_note'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_note' }])])
}

fn Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.possibly_add_note()  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments{}; return temp.is_wc_admin_active_in_date_range(arg_0) }(rt.new_string('week-1-4')))))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return rt.new_null()
	}
	mut var_data_store := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Notes_Notes{}; return temp.load_data_store() }()
	mut var_note_ids := rt.call_method(var_data_store, 'get_notes_with_name', [Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.note_name()])
	if !(!rt.is_true(var_note_ids)) {
		mut var_note_id := rt.call_function('array_pop', [var_note_ids.dup()])
		...
		if rt.is_true(rt.new_bool(rt.is_true(Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.is_installed()) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
	...
	mut var_publish_date := create_automattic_woocommerce_internal_admin_notes_datetime(rt.new_string('2020-04-14'))
	if rt.is_true(rt.greater_equal(var_current_date, var_publish_date)) {
		...
		if rt.is_true(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments{}; return temp.can_be_added() }()) {
			rt.call_method(var_note, 'save', []rt.PhpVal{})
		}
		return rt.new_null()
	} else {
		mut var_hook_name := rt.call_function('sprintf', [rt.new_string('%s_add_note'), Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.note_name()])
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'queue', []rt.PhpVal{}), 'get_next', [var_hook_name.dup()]))))) {
			rt.call_method(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'queue', []rt.PhpVal{}), 'schedule_single', [var_publish_date.gettimestamp(), var_hook_name.dup()])
		}
	}
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.get_note() rt.PhpVal {
	...
	rt.call_method(var_note, 'set_title', [rt.call_function('__', [rt.new_string('Try the new way to get paid'), rt.new_string('woocommerce')])])
	rt.call_method(var_note, 'set_content', [(rt.call_function('__', [rt.new_string('Securely accept credit and debit cards on your site. Manage transactions without leaving your WordPress dashboard. Only with <strong>WooPayments</strong>.'), rt.new_string('woocommerce')])).str() + '<br><br>' + (rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('By clicking "Get started", you agree to our %1$sTerms of Service%2$s'), rt.new_string('woocommerce')]), rt.new_string('<a href="https://wordpress.com/tos/" target="_blank">'), rt.new_string('</a>')])).str()])
	rt.call_method(var_note, 'set_content_data', [// unsupported expression: Expr_Cast_Object])
	rt.call_method(var_note, 'set_type', [Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_marketing()])
	rt.call_method(var_note, 'set_name', [Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.note_name()])
	rt.call_method(var_note, 'set_source', [rt.new_string('woocommerce-admin')])
	rt.call_method(var_note, 'add_action', [rt.new_string('learn-more'), rt.call_function('__', [rt.new_string('Learn more'), rt.new_string('woocommerce')]), rt.new_string('https://woocommerce.com/payments/?utm_medium=product'), Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_unactioned()])
	rt.call_method(var_note, 'add_action', [rt.new_string('get-started'), rt.call_function('__', [rt.new_string('Get started'), rt.new_string('woocommerce')]), rt.call_function('wc_admin_url', [rt.new_string('&action=setup-woocommerce-payments')]), Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_actioned(), rt.new_bool(true)])
	rt.call_method(var_note, 'add_nonce_to_action', [rt.new_string('get-started'), rt.new_string('setup-woocommerce-payments'), rt.new_string('')])
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.is_installed() bool {
	...
	return (rt.identical(rt.new_int(0), rt.call_function('validate_plugin', [Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.plugin_file()]))).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments) install_and_activate_wcpay() bool {
	mut var_install_request := rt.create_array([rt.ArrayItem{ key: 'plugins', val: Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.plugin_slug() }])
	mut var_installer := create_automattic_woocommerce_internal_admin_notes_automattic_woocommerce_admin_api_plugins()
	mut var_result := var_installer.install_plugins(var_install_request.dup())
	if rt.is_true(rt.call_function('is_wp_error', [var_result.dup()])) {
		return false
	}
	rt.call_function('wc_admin_record_tracks_event', [rt.new_string('woocommerce_payments_install'), rt.create_array([rt.ArrayItem{ key: 'context', val: 'inbox' }])])
	mut var_activate_request := rt.create_array([rt.ArrayItem{ key: 'plugins', val: Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.plugin_slug() }])
	var_result = var_installer.activate_plugins(var_activate_request.dup())
	if rt.is_true(rt.call_function('is_wp_error', [var_result.dup()])) {
		return false
	}
	return true
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments) install_on_action()  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.get_superglobal('_GET').array_isset(rt.new_string('page'))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) || !(rt.get_superglobal('_GET').array_isset(rt.new_string('action'))))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return rt.new_null()
	}
	mut var_data_store := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Notes_Notes{}; return temp.load_data_store() }()
	mut var_note_ids := rt.call_method(var_data_store, 'get_notes_with_name', [Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_WooCommercePayments.note_name()])
	...
	mut var_action := rt.call_method(var_note, 'get_action', [rt.new_string('get-started')])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_action)))) || rt.is_true(rt.new_bool(!(rt.get_property(var_action, 'nonce_action')).is_null() && rt.is_true(rt.new_bool(!rt.is_true(rt.get_superglobal('_GET').array_get('_wpnonce')) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_verify_nonce', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('_wpnonce')]), rt.get_property(var_action, 'nonce_action')]))))))))))) {
		return rt.new_null()
	}
	...
	mut var_connect_url := rt.call_function('add_query_arg', [rt.create_array([rt.ArrayItem{ key: 'wcpay-connect', val: '1' }, rt.ArrayItem{ key: '_wpnonce', val: rt.call_function('wp_create_nonce', [rt.new_string('wcpay-connect')]) }]), rt.call_function('admin_url', []rt.PhpVal{})])
	rt.call_function('wp_safe_redirect', [var_connect_url.dup()])
...
fn create_automattic_woocommerce_internal_admin_notes_datetime() &Class_Automattic_WooCommerce_Internal_Admin_Notes_DateTime {
...
fn create_automattic_woocommerce_internal_admin_notes_automattic_woocommerce_admin_api_plugins() &Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Admin_API_Plugins {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_woocommercepayments_php() {
