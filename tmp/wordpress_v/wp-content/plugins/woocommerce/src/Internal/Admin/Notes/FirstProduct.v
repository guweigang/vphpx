...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_FirstProduct.get_note() rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_FirstProduct{}; return temp.is_wc_admin_active_in_date_range(arg_0) }(rt.new_string('week-1-4')))))) {
		return rt.new_null()
	}
	...
	var_note.set_title(rt.call_function('__', [rt.new_string('Do you need help with adding your first product?'), rt.new_string('woocommerce')]))
	var_note.set_content(rt.call_function('__', [rt.new_string('This video tutorial will help you go through the process of adding your first product in WooCommerce.'), rt.new_string('woocommerce')]))
	...
	var_note.add_action(rt.new_string('first-product-watch-tutorial'), rt.call_function('__', [rt.new_string('Watch tutorial'), rt.new_string('woocommerce')]), rt.new_string('https://www.youtube.com/watch?v=sFtXa00Jf_o&list=PLHdG8zvZd0E575Ia8Mu3w1h750YLXNfsC&index=24'))
	return mut var_note
}
...
fn create_automattic_woocommerce_internal_admin_notes_firstproduct() &Class_Automattic_WooCommerce_Internal_Admin_Notes_FirstProduct {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_firstproduct_php() {
