...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Suggestions_Incentives_Incentive) get_all(country_code string, incentive_type string) rt.PhpVal {
	...
	mut var_incentive := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	...
	mut var_incentive := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.identical(var_incentive.array_get('type'), rt.new_string(incentive_type))
	}
		var_incentives = rt.call_function('array_filter', [var_incentives.dup(), rt.new_closure(closure_2_fn)])
	}
	return rt.call_function('array_values', [var_incentives.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Suggestions_Incentives_Incentive) get_by_promo_id(promo_id string, country_code string, incentive_type string) rt.PhpVal {
	...
	mut var_incentive := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.identical(var_incentive.array_get('promo_id'), rt.new_string(promo_id))
	}
	...
	return rt.call_function('reset', [var_incentives.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Suggestions_Incentives_Incentive) get_by_id(incentive_id string, country_code string) rt.PhpVal {
	...
	mut var_incentive := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.identical(var_incentive.array_get('id'), rt.new_string(incentive_id))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Suggestions_Incentives_Incentive) dismiss(id string, context string, mut var_timestamp Class_Automattic_WooCommerce_Internal_Admin_Suggestions_Incentives_?int) bool {
	...
	if !rt.is_true(var_all_dismissed_incentives.array_get(this.suggestion_id)) {
		var_all_dismissed_incentives.array_set(this.suggestion_id, rt.new_array())
		rt.call_function('ksort', [var_all_dismissed_incentives.dup()])
	}
	...
	rt.call_function('do_action', [rt.new_string('woocommerce_admin_payments_extension_suggestion_incentive_dismissed'), rt.new_string(id), this.suggestion_id, rt.new_string(context)])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Suggestions_Incentives_Incentive) is_dismissed(id string, context string) bool {
	...
	mut var_all_dismissed_incentives := this.get_all_dismissed_incentives()
	mut var_dismissed_incentives := if !(var_all_dismissed_incentives.array_get(this.suggestion_id)).is_null() { var_all_dismissed_incentives.array_get(this.suggestion_id) } else { rt.new_array() }
	if !rt.is_true(var_dismissed_incentives) {
		return false
	}
	closure_6_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	closure_5_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_dismissed_incentive := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.new_bool(rt.is_true(rt.identical(rt.new_string('all'), var_dismissed_incentive.array_get('context'))) || rt.is_true(rt.identical(rt.new_string(context), var_dismissed_incentive.array_get('context'))))
	}
	mut var_dismissed_incentive := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.new_bool(rt.is_true(rt.identical(rt.new_string('all'), var_dismissed_incentive.array_get('context'))) || rt.is_true(rt.identical(rt.new_string(context), var_dismissed_incentive.array_get('context'))))
	}
	if rt.is_true(rt.call_function('in_array', [rt.new_string(id), rt.call_function('array_column', [rt.call_function('array_filter', [var_dismissed_incentives.dup(), rt.new_closure(closure_5_fn)]), rt.new_string('id')]), rt.new_bool(true)])) {
		return true
	}
	return false
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Suggestions_Incentives_Incentive) get_dismissals(id string) rt.PhpVal {
	...
	mut var_dismissed_incentive := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.identical(rt.new_string(id), var_dismissed_incentive.array_get('id'))
	}
	mut var_dismissed_incentive := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.identical(rt.new_string(id), var_dismissed_incentive.array_get('id'))
	}
	mut var_dismissals := rt.call_function('array_values', [rt.call_function('array_filter', [var_dismissed_incentives.dup(), rt.new_closure(closure_7_fn)])])
	closure_10_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	closure_9_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_dismissed_incentive := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.create_array([rt.ArrayItem{ key: 'timestamp', val: var_dismissed_incentive.array_get('timestamp') }, rt.ArrayItem{ key: 'context', val: var_dismissed_incentive.array_get('context') }])
	}
	mut var_dismissed_incentive := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.create_array([rt.ArrayItem{ key: 'timestamp', val: var_dismissed_incentive.array_get('timestamp') }, rt.ArrayItem{ key: 'context', val: var_dismissed_incentive.array_get('context') }])
	}
	return rt.call_function('array_map', [rt.new_closure(closure_9_fn), var_dismissals.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Suggestions_Incentives_Incentive) get_all_dismissed_incentives() rt.PhpVal {
	...
	return var_all_dismissed_incentives.dup()
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_suggestions_incentives_incentive_php() {
