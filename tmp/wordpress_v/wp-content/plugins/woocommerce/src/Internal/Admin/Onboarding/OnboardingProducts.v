...
fn Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingProducts.get_allowed_product_types() rt.PhpVal {
	mut var_products := rt.create_array([rt.ArrayItem{ key: 'physical', val: rt.create_array([rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Physical products'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'default', val: true }]) }, rt.ArrayItem{ key: 'downloads', val: rt.create_array([rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Downloads'), rt.new_string('woocommerce')]) }]) }, rt.ArrayItem{ key: 'subscriptions', val: rt.create_array([rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Subscriptions'), rt.new_string('woocommerce')]) }]) }, rt.ArrayItem{ key: 'memberships', val: rt.create_array([rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Memberships'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'product', val: 958589 }]) }, rt.ArrayItem{ key: 'bookings', val: rt.create_array([rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Bookings'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'product', val: 390890 }]) }, rt.ArrayItem{ key: 'product-bundles', val: rt.create_array([rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Bundles'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'product', val: 18716 }]) }, rt.ArrayItem{ key: 'product-add-ons', val: rt.create_array([rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Customizable products'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'product', val: 18618 }]) }])
	...
	if rt.is_true(rt.identical(rt.new_string('US'), var_base_location.array_get('country'))) {
		...
		if !(!rt.is_true(var_profile.array_get('industry'))) {
			var_has_cbd_industry = rt.call_function('in_array', [rt.new_string('cbd-other-hemp-derived-products'), rt.call_function('array_column', [var_profile.array_get('industry'), rt.new_string('slug')]), rt.new_bool(true)])
		}
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_Features_Features{}; return temp.is_enabled(arg_0) }(rt.new_string('subscriptions')))))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) || rt.is_true(var_has_cbd_industry))) {
		var_products.array_get_mut('subscriptions').array_set('product', 27147)
	}
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_admin_onboarding_product_types'), var_products.dup()])
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingProducts.get_product_data(var_product_types rt.PhpVal) rt.PhpVal {
	...
	mut var_transient_value := rt.call_function('get_transient', [Class_Automattic_WooCommerce_Internal_Admin_Onboarding_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingProducts.product_data_transient()])
	var_transient_value = if rt.is_true(rt.new_bool(var_transient_value.dup().is_array())) { var_transient_value } else { rt.new_array() }
	mut var_woocommerce_products := if !(var_transient_value.array_get(var_locale)).is_null() { var_transient_value.array_get(var_locale) } else { rt.new_bool(false) }
	if rt.is_true(rt.identical(rt.new_bool(false), var_woocommerce_products)) {
		var_woocommerce_products = rt.call_function('wp_remote_get', [rt.call_function('add_query_arg', [rt.create_array([rt.ArrayItem{ key: 'locale', val: var_locale }]), rt.new_string('https://woocommerce.com/wp-json/wccom-extensions/1.0/search')]), rt.create_array([rt.ArrayItem{ key: 'user-agent', val: 'WooCommerce/' + (rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'version')).str() + '; ' + (rt.call_function('get_bloginfo', [rt.new_string('url')])).str() }])])
		if rt.is_true(rt.call_function('is_wp_error', [var_woocommerce_products.dup()])) {
			return var_product_types_mutated.dup()
		}
		var_transient_value.array_set(var_locale, var_woocommerce_products.dup())
		rt.call_function('set_transient', [Class_Automattic_WooCommerce_Internal_Admin_Onboarding_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingProducts.product_data_transient(), var_transient_value.dup(), rt.get_constant('DAY_IN_SECONDS')])
	}
	mut var_data := rt.call_function('json_decode', [var_woocommerce_products.array_get('body')])
	...
	if !(var_data).is_null() && !(rt.get_property(var_data, 'products')).is_null() {
		{
			...
			for {
				...
				if !(rt.get_property(var_product_datum, 'id')).is_null() {
					var_products.array_set(rt.get_property(var_product_datum, 'id'), var_product_datum.dup())
				}
			}
		}
	}
	{
		mut iter_1 := var_product_types_mutated.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_product_type := item_1.val
			mut var_key := item_1.key
			var_product_data.array_set(var_key, var_product_types_mutated.array_get(var_key))
			if var_product_type.array_isset(rt.new_string('product')) && var_products.array_isset(var_product_type.array_get('product')) {
				mut var_price := rt.call_function('html_entity_decode', [rt.get_property(var_products.array_get(var_product_type.array_get('product')), 'price')])
				mut var_yearly_price := // unsupported expression: Expr_Cast_Double
				var_product_data.array_get_mut(var_key).array_set('yearly_price', var_yearly_price.dup())
				var_product_data.array_get_mut(var_key).array_set('description', rt.get_property(var_products.array_get(var_product_type.array_get('product')), 'excerpt'))
				var_product_data.array_get_mut(var_key).array_set('more_url', rt.get_property(var_products.array_get(var_product_type.array_get('product')), 'link'))
				var_product_data.array_get_mut(var_key).array_set('slug', rt.call_function('preg_replace', [rt.new_string('~[^\\pL\\d]+~u'), rt.new_string('-'), rt.get_property(var_products.array_get(var_product_type.array_get('product')), 'slug')]).to_string().to_lower())
			}
		}
	}
	return var_product_data.dup()
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingProducts.get_relevant_products() rt.PhpVal {
	...
	mut var_installed := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Admin_PluginsHelper{}; return temp.get_installed_plugin_slugs() }()
	...
	mut var_product_data := Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingProducts.get_product_types_with_data()
	...
	mut var_remaining := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_product_data.array_get(var_type).array_get('slug'), var_installed.dup(), rt.new_bool(true)]))))) {
				var_remaining.array_push(var_product_data.array_get(var_type).array_get('label'))
			}
		}
	}
	return rt.create_array([rt.ArrayItem{ key: 'purchaseable', val: var_purchaseable }, rt.ArrayItem{ key: 'remaining', val: var_remaining }])
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_onboarding_onboardingproducts_php() {
}
