...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_MobileApp.get_note() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_MobileApp{}; return temp.is_wc_admin_active_in_date_range(arg_0, arg_1) }(rt.new_string('week-1'), var_two_days_in_seconds.dup()))))) {
		return rt.new_null()
	}
	mut var_content := rt.call_function('__', [rt.new_string('Install the WooCommerce mobile app to manage orders, receive sales notifications, and view key metrics — wherever you are.'), rt.new_string('woocommerce')])
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [rt.new_string('Install Woo mobile app'), rt.new_string('woocommerce')]))
	...
	var_note.add_action(rt.new_string('learn-more'), rt.call_function('__', [rt.new_string('Learn more'), rt.new_string('woocommerce')]), rt.new_string('https://woocommerce.com/mobile/?utm_medium=product'))
	return mut var_note
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_mobileapp_php() {
