...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_SearchListTable) prepare_items() {
	...
	mut var_search := var_file_args.array_get('search')
	...
	this.dispatch_set_prop('items', var_results.dup())
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_SearchListTable) column_file_id(mut var_item Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_array) string {
	mut var_file_id := rt.call_function('preg_replace', [
		rt.new_string('/\\.([0-9])+\\-/'),
		rt.new_string('.\\1<wbr>-'),
		var_item.array_get('file_id'),
	])
	return (rt.call_function('wp_kses', [var_file_id.dup(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_SearchListTable) column_line_number(mut var_item Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_array) string {
	mut var_match_url := rt.call_function('add_query_arg', [
		rt.create_array([rt.ArrayItem{ key: 'view', val: 'single_file' },
			rt.ArrayItem{ key: 'file_id', val: var_item.array_get('file_id') }]),
		(rt.call_method(this.page_controller, 'get_logs_tab_url', []rt.PhpVal{})).str() + '#L' +
			(rt.call_function('absint', [var_item.array_get('line_number')])).str(),
	])
	return (rt.call_function('sprintf', [rt.new_string('<a href="%1$s">%2$s</a>'),
		rt.call_function('esc_url', [var_match_url.dup()]),
		rt.call_function('sprintf', [
			rt.call_function('esc_html__', [
			...
			rt.call_function('number_format_i18n', [
				rt.call_function('absint', [var_item.array_get('line_number')]),
			])])])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_SearchListTable) column_line(mut var_item Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_array) string {
	...
	mut var_line := var_item.array_get('line')
	mut var_pattern := rt.call_function('preg_quote', [var_params.array_get('search'),
		rt.new_string('/')])
	rt.call_function('preg_match_all', [rt.new_string('/${var_pattern.to_string()}/i'),
		var_line.dup(), var_matches.dup(), rt.get_constant('PREG_OFFSET_CAPTURE')])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_matches.array_get(0).is_array()))
		&& var_matches.array_get(0).array_count() >= 1))
	{
		mut var_length_change := rt.new_int(rt.new_int(0))
		{
			...
			for {
				...
				mut var_replace := rt.new_string('<span class="search-match">' +
					(var_match.array_get(0)).str() + '</span>')
				mut var_offset := rt.add(var_match.array_get(1), var_length_change)
				mut var_orig_length :=
					rt.new_int(rt.new_int(var_match.array_get(0).to_string().len))
				mut var_replace_length := rt.new_int(rt.new_int(var_replace.dup().to_string().len))
				var_line = rt.call_function('substr_replace', [
					var_line.dup(), var_replace.dup(), var_offset.dup(),
					var_orig_length.dup()])
				// unsupported expression: Expr_AssignOp_Plus
			}
		}
	}
	return (rt.call_function('wp_kses_post', [var_line.dup()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_SearchListTable) get_per_page_default() i64 {
	return (Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_{
		...
		name:     'file_controller'
	}.defaults_search_within_files().array_get('per_page')).to_i64()
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_logging_filev2_searchlisttable_php() {
