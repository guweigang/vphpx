...
struct Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail {
	rt.PhpObjectBase
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (rt.call_function('wc_string_to_bool', [var_payment_gateway.get_option(rt.new_string('enable_test_mode'), rt.new_string('no'))])).to_bool()
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if this.is_in_test_mode_onboarding(mut var_payment_gateway) {
		return true
	}
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	return !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('merchant_id')))) && !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('secret_key'))))
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail) is_in_test_mode_onboarding(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_paytrail() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail {
	mut obj := &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail{
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'is_in_test_mode' {
...
fn (this &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_paytrail_php() {
