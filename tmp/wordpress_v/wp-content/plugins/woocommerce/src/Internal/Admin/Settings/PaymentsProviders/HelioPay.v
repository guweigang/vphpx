...
struct Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay {
	rt.PhpObjectBase
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('defined', [rt.new_string('HELIO_DEVNET_ENABLED')])) {
		return (rt.call_function('wc_string_to_bool', [var_payment_gateway.get_option(rt.get_constant('HELIO_DEVNET_ENABLED'))])).to_bool()
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if this.is_in_test_mode(mut var_payment_gateway) {
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('HELIO_API_KEY_DEVNET')])) && rt.is_true(rt.call_function('defined', [rt.new_string('HELIO_API_SECRET_DEVNET')])))) {
			return !(!rt.is_true(var_payment_gateway.get_option(rt.get_constant('HELIO_API_KEY_DEVNET')))) && !(!rt.is_true(var_payment_gateway.get_option(rt.get_constant('HELIO_API_SECRET_DEVNET'))))
		}
		if rt.has_exception() { unsafe { goto catch_label_2 } }
	} else if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('HELIO_API_KEY_MAINNET')])) && rt.is_true(rt.call_function('defined', [rt.new_string('HELIO_API_SECRET_MAINNET')])))) {
		return !(!rt.is_true(var_payment_gateway.get_option(rt.get_constant('HELIO_API_KEY_MAINNET')))) && !(!rt.is_true(var_payment_gateway.get_option(rt.get_constant('HELIO_API_SECRET_MAINNET'))))
	}
	if rt.has_exception() { unsafe { goto catch_label_2 } }
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay) is_in_test_mode_onboarding(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return this.is_in_test_mode(mut var_payment_gateway)
}
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_heliopay() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay {
	mut obj := &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay{
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'is_in_test_mode' {
...
fn (this &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_heliopay_php() {
