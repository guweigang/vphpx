...
struct Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Monei {
	rt.PhpObjectBase
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Monei) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: var_payment_gateway }, rt.ArrayItem{ key: none, val: 'getAccountId' }])])) && rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: var_payment_gateway }, rt.ArrayItem{ key: none, val: 'getApiKey' }])])))) {
		return !(!rt.is_true(var_payment_gateway.getaccountid())) && !(!rt.is_true(var_payment_gateway.getapikey()))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Monei) is_in_test_mode_onboarding(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_monei() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Monei {
	mut obj := &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Monei{
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Monei) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
...
fn (this &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Monei) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Monei) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_monei_php() {
