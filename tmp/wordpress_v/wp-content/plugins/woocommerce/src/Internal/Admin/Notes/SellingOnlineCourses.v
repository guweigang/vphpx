...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_SellingOnlineCourses.check_onboarding_profile(var_old_value rt.PhpVal, var_value rt.PhpVal, var_option rt.PhpVal)  {
	...
	mut var_industry_slugs := rt.call_function('array_column', [var_value.array_get('industry'), rt.new_string('slug')])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string('education-and-learning'), var_industry_slugs.dup(), rt.new_bool(true)]))))) {
		return rt.new_null()
	}
	fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_SellingOnlineCourses{}; return temp.possibly_add_note() }()
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_SellingOnlineCourses.get_note() rt.PhpVal {
	...
	var_note.set_title(rt.call_function('__', [rt.new_string('Do you want to sell online courses?'), rt.new_string('woocommerce')]))
	var_note.set_content(rt.call_function('__', [rt.new_string('Online courses are a great solution for any business that can teach a new skill. Since courses don’t require physical product development or shipping, they’re affordable, fast to create, and can generate passive income for years to come. In this article, we provide you more information about selling courses using WooCommerce.'), rt.new_string('woocommerce')]))
	...
	var_note.add_action(rt.new_string('learn-more'), rt.call_function('__', [rt.new_string('Learn more'), rt.new_string('woocommerce')]), rt.new_string('https://woocommerce.com/posts/how-to-sell-online-courses-wordpress/?utm_source=inbox&utm_medium=product'), Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_actioned())
	return mut var_note
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_SellingOnlineCourses) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'check_onboarding_profile' {
			...
			Class_Automattic_WooCommerce_Internal_Admin_Notes_SellingOnlineCourses.check_onboarding_profile(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2)
			return rt.new_null()
		}
		'get_note' {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_sellingonlinecourses_php() {
