...
struct Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payfast {
	rt.PhpObjectBase
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payfast) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('merchant_id')))) && !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('merchant_key')))) && !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('pass_phrase')))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wc_string_to_bool', [rt.call_function('get_option', [rt.new_string('woocommerce_payfast_invalid_credentials'), rt.new_string('no')])])))))
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payfast) is_in_test_mode_onboarding(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_payfast() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payfast {
	mut obj := &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payfast{
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payfast) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
...
fn (this &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payfast) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payfast) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_payfast_php() {
