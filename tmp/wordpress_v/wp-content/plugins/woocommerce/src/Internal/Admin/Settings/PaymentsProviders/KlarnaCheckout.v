...
struct Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout {
	rt.PhpObjectBase
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout) needs_setup(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return !(!rt.is_true(rt.call_function('get_option', [rt.new_string('kco_credentials_error')])))
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('KCO_WC')])) {
		mut var_credentials := rt.get_property(rt.call_function('KCO_WC', []rt.PhpVal{}), 'credentials')
		if rt.has_exception() { unsafe { goto catch_label_1 } }
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_credentials.dup().is_object())) && rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: var_credentials }, rt.ArrayItem{ key: none, val: 'get_credentials_from_session' }])])))) {
			return !(!rt.is_true(rt.call_method(var_credentials, 'get_credentials_from_session', []rt.PhpVal{})))
		}
		if rt.has_exception() { unsafe { goto catch_label_1 } }
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout) is_in_test_mode_onboarding(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_klarnacheckout() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout {
	mut obj := &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout{
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'needs_setup' {
...
fn (this &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_klarnacheckout_php() {
