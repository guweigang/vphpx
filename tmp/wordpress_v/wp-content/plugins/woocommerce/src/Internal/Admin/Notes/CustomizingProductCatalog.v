...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_CustomizingProductCatalog.get_note() rt.PhpVal {
	mut var_query := create_automattic_woocommerce_internal_admin_notes_wc_product_query(rt.create_array([rt.ArrayItem{ key: 'limit', val: 1 }, rt.ArrayItem{ key: 'paginate', val: true }, rt.ArrayItem{ key: 'status', val: rt.create_array([rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Enums_ProductStatus.publish() }]) }, rt.ArrayItem{ key: 'orderby', val: 'post_date' }, rt.ArrayItem{ key: 'order', val: 'DESC' }]))
	mut var_products := var_query.get_products()
	if rt.is_true(rt.identical(rt.new_int(0), rt.get_property(var_products, 'total'))) {
		return rt.new_null()
	}
	mut var_product := rt.get_property(var_products, 'products').array_get(0)
	mut var_created_timestamp := rt.call_method(rt.call_method(var_product, 'get_date_created', []rt.PhpVal{}), 'getTimestamp', []rt.PhpVal{})
	mut var_is_a_day_old := rt.greater_equal(rt.sub(rt.call_function('time', []rt.PhpVal{}), var_created_timestamp), rt.get_constant('DAY_IN_SECONDS'))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_is_a_day_old)))) {
		return rt.new_null()
	}
	if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_CustomizingProductCatalog{}; return temp.wc_admin_active_for(arg_0) }(rt.mul(rt.get_constant('DAY_IN_SECONDS'), rt.new_int(14)))) {
		return rt.new_null()
	}
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [rt.new_string('How to customize your product catalog'), rt.new_string('woocommerce')]))
	var_note.set_content(rt.call_function('__', [rt.new_string('You want your product catalog and images to look great and align with your brand. This guide will give you all the tips you need to get your products looking great in your store.'), rt.new_string('woocommerce')]))
	...
	var_note.add_action(rt.new_string('day-after-first-product'), rt.call_function('__', [rt.new_string('Learn more'), rt.new_string('woocommerce')]), rt.new_string('https://woocommerce.com/document/woocommerce-customizer/?utm_source=inbox&utm_medium=product'))
	return mut var_note
}
...
fn create_automattic_woocommerce_internal_admin_notes_customizingproductcatalog() &Class_Automattic_WooCommerce_Internal_Admin_Notes_CustomizingProductCatalog {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_customizingproductcatalog_php() {
