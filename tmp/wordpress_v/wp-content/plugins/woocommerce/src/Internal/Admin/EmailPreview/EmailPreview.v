...
struct Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview {
	rt.PhpObjectBase
pub mut:
		email_style_setting_ids rt.PhpVal = rt.new_array()
		email_content_setting_ids rt.PhpVal = rt.new_array()
		email_setting_ids_initialized rt.PhpVal = rt.new_bool(false)
...
fn Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview.get_all_email_setting_ids() rt.PhpVal {
	...
	return rt.call_function('array_merge', [// unsupported expression: Expr_StaticPropertyFetch, // unsupported expression: Expr_StaticPropertyFetch])
}

fn Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview.get_email_style_setting_ids() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_email_preview_email_style_setting_ids'), // unsupported expression: Expr_StaticPropertyFetch])
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview.get_email_content_setting_ids(mut var_email_id Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_?string) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_email_preview_email_content_setting_ids'), var_setting_ids.dup(), var_email_id])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview) set_email_type(email_type string)  {
	...
	mut var_emails := rt.call_function('array_combine', [rt.call_function('array_map', [rt.new_string('get_class'), var_wc_emails.dup()]), var_wc_emails.dup()])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string(email_type), rt.func_array_keys(var_emails.dup()), rt.new_bool(true)]))))) {
	...
	this.email_type = rt.new_string(email_type).dup()
	this.email = var_emails.array_get(email_type)
	...
	if rt.is_true(rt.call_function('in_array', [rt.new_string(email_type), Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview.user_object_emails(), rt.new_bool(true)])) {
		...
		rt.call_method(this.email, 'set_object', [var_object.dup()])
	} else {
	...
	rt.set_property(this.email, 'placeholders', rt.call_function('array_merge', [rt.get_property(this.email, 'placeholders'), this.get_placeholders(var_object.dup())]))
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview) ensure_links_open_in_new_tab(content string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(content_mutated == '' || rt.is_true(rt.identical(rt.call_function('strpos', [rt.new_string(content_mutated).dup(), rt.new_string('<a')]), rt.new_bool(false))))) {
	...
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	var_dom.loadhtml(var_html_with_encoding.dup(), rt.new_int(rt.bitwise_or(rt.bitwise_or(rt.bitwise_or(rt.get_constant('LIBXML_HTML_NOIMPLIED'), rt.get_constant('LIBXML_HTML_NODEFDTD')), rt.get_constant('LIBXML_NOWARNING')), rt.get_constant('LIBXML_NOERROR'))))
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	...
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	var_result = rt.call_function('preg_replace', [rt.new_string('/<\\?xml[^>]*>\\s*/i'), rt.new_string(''), var_result.dup()])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
finally_label_1:
	rt.call_function('libxml_use_internal_errors', [var_previous_use_internal_errors.dup()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview) render_preview_email() rt.PhpVal {
	...
	mut var_inlined := rt.call_method(this.email, 'style_inline', [var_content.dup()])
	this.clean_up_filters()
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_mail_content'), var_inlined.dup()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview) get_dummy_order() rt.PhpVal {
	...
	if rt.is_true(var_variation) {
		...
		var_item.set_props(rt.create_array([rt.ArrayItem{ key: , val:  }, rt.ArrayItem{ key: , val:  }, rt.ArrayItem{ key: , val:  }, rt.ArrayItem{ key: , val:  }, rt.ArrayItem{ key: , val:  }, rt.ArrayItem{ key: , val:  }, rt.ArrayItem{ key: , val:  }, rt.ArrayItem{ key: , val:  }]))
		rt.call_method(, 'add_item', [])
	}
...
fn create_automattic_woocommerce_internal_admin_emailpreview_emailpreview() &Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview {
	mut obj := &Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview{
		...
		email_style_setting_ids: rt.new_array()
		email_content_setting_ids: rt.new_array()
		email_setting_ids_initialized: rt.new_bool(false)
...
fn create_automattic_woocommerce_internal_admin_emailpreview_invalidargumentexception() &Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_InvalidArgumentException {
...
fn create_automattic_woocommerce_internal_admin_emailpreview_domdocument() &Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_DOMDocument {
...
fn (this &Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	match prop_name {
		'email_style_setting_ids' { return this.email_style_setting_ids }
		'email_content_setting_ids' { return this.email_content_setting_ids }
		'email_setting_ids_initialized' { return this.email_setting_ids_initialized }
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreview) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'email_style_setting_ids' { this.email_style_setting_ids = val; return true }
		'email_content_setting_ids' { this.email_content_setting_ids = val; return true }
		'email_setting_ids_initialized' { this.email_setting_ids_initialized = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_emailpreview_emailpreview_php() {
