...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_RealTimeOrderAlerts.get_note() rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_RealTimeOrderAlerts{}; return temp.is_wc_admin_active_in_date_range(arg_0) }(rt.new_string('month-3-6')))))) {
		return rt.new_null()
	}
	...
	mut var_content := rt.call_function('__', [rt.new_string('Get notifications about store activity, including new orders and product reviews directly on your mobile devices with the Woo app.'), rt.new_string('woocommerce')])
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [rt.new_string('Get real-time order alerts anywhere'), rt.new_string('woocommerce')]))
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_realtimeorderalerts_php() {
