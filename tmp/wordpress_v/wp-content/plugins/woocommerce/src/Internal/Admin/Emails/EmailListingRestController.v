...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Emails_EmailListingRestController) initialize_template_generator()  {
	rt.call_method(this.email_template_generator, 'init_default_transactional_emails', []rt.PhpVal{})
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Emails_EmailListingRestController) register_routes()  {
	...
	rt.call_function('register_rest_route', [this.route_namespace, '/' + (this.rest_base).str() + '/recreate-email-post', rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: Class_Automattic_WooCommerce_Internal_Admin_Emails_WP_REST_Server.creatable() }, rt.ArrayItem{ key: 'callback', val: rt.new_closure(closure_1_fn) }, rt.ArrayItem{ key: 'permission_callback', val: rt.new_closure(closure_2_fn) }, rt.ArrayItem{ key: 'args', val: this.get_args_for_recreate_email_post() }, rt.ArrayItem{ key: 'schema', val: this.get_schema_with_message() }]) }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Emails_EmailListingRestController) get_args_for_recreate_email_post() rt.PhpVal {
	...
	return rt.create_array([rt.ArrayItem{ key: 'email_id', val: rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('The email ID to recreate the post for.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'required', val: true }, rt.ArrayItem{ key: 'validate_callback', val: rt.new_closure(closure_3_fn) }, rt.ArrayItem{ key: 'sanitize_callback', val: 'sanitize_text_field' }]) }])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Emails_EmailListingRestController) get_schema_with_message() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: '$schema', val: 'http://json-schema.org/draft-04/schema#' }, rt.ArrayItem{ key: 'title', val: 'email-listing-with-message' }, rt.ArrayItem{ key: 'type', val: 'object' }, rt.ArrayItem{ key: 'properties', val: rt.create_array([rt.ArrayItem{ key: 'message', val: rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('A message indicating that the action completed successfully.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'context', val: rt.create_array([rt.ArrayItem{ key: none, val: 'view' }, rt.ArrayItem{ key: none, val: 'edit' }]) }, rt.ArrayItem{ key: 'readonly', val: true }]) }, rt.ArrayItem{ key: 'post_id', val: rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('The post ID of the generated email post.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'context', val: rt.create_array([rt.ArrayItem{ key: none, val: 'view' }, rt.ArrayItem{ key: none, val: 'edit' }]) }, rt.ArrayItem{ key: 'readonly', val: true }]) }]) }])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Emails_EmailListingRestController) validate_email_id(email_id string) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string(email_id_mutated).dup(), fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_EmailEditor_WCTransactionalEmails_WCTransactionalEmails{}; return temp.get_transactional_emails() }(), rt.new_bool(true)]))))) {
		return (create_wp_error(rt.new_string('woocommerce_rest_not_allowed_email_id'), rt.call_function('sprintf', [rt.new_string('The provided email ID "%s" is not allowed.'), rt.new_string(email_id_mutated).dup()]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))).to_bool()
	}
	return true
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Emails_EmailListingRestController) check_permissions(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_verify_nonce', [var_nonce.dup(), Class_Automattic_WooCommerce_Internal_Admin_Emails_Automattic_WooCommerce_Internal_Admin_Emails_EmailListingRestController.nonce_key()]))))) {
		return create_wp_error(rt.new_string('invalid_nonce'), rt.call_function('__', [rt.new_string('Invalid nonce.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 403 }]))
	}
	return this.check_permission(rt.new_object('WP_REST_Request', []string{}, var_request), rt.new_string('manage_woocommerce'))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Emails_EmailListingRestController) recreate_email_post(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	mut var_generated_post_id := rt.new_string(rt.new_string(''))
	var_generated_post_id = rt.call_method(this.email_template_generator, 'generate_email_template_if_not_exists', [var_email_id.dup()])
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Internal_Admin_Emails_Exception') {
		...
		return create_wp_error(rt.new_string('woocommerce_rest_email_post_generation_failed'), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Error generating email post. Error: %s.'), rt.new_string('woocommerce')]), rt.call_method(var_e, 'getMessage', []rt.PhpVal{})]), rt.create_array([rt.ArrayItem{ key: 'status', val: 500 }]))
		unsafe { goto end_label_1 }
	}
	else {
...
end_label_1:
	if rt.is_true(var_generated_post_id) {
		return rt.create_array([rt.ArrayItem{ key: 'message', val: rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Email post generated for %s.'), rt.new_string('woocommerce')]), var_email_id.dup()]) }, rt.ArrayItem{ key: 'post_id', val: // unsupported expression: Expr_Cast_String }])
	}
	return create_wp_error(rt.new_string('woocommerce_rest_email_post_generation_error'), rt.call_function('__', [rt.new_string('Error unable to generate email post.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 500 }]))
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_emails_emaillistingrestcontroller_php() {
