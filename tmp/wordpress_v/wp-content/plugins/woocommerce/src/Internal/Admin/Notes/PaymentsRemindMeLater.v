...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_PaymentsRemindMeLater.should_display_note() bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_WcPayWelcomePage{}; return temp.instance() }(), 'has_incentive', []rt.PhpVal{}))))) {
		return false
	}
	mut var_view_timestamp := rt.call_function('get_option', [rt.new_string('wcpay_welcome_page_viewed_timestamp'), rt.new_bool(false)])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_view_timestamp)))) || rt.is_true(rt.less(rt.sub(rt.call_function('time', []rt.PhpVal{}), var_view_timestamp), rt.mul(rt.new_int(3), rt.get_constant('DAY_IN_SECONDS')))))) {
		return false
	}
	return true
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_PaymentsRemindMeLater.get_note() rt.PhpVal {
	...
	mut var_content := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Save up to $800 in fees by managing transactions with %1$s. With %1$s, you can securely accept major cards, Apple Pay, and payments in over 100 currencies.'), rt.new_string('woocommerce')]), rt.new_string('WooPayments')])
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Save big with %s'), rt.new_string('woocommerce')]), rt.new_string('WooPayments')]))
	...
	var_note.add_action(rt.new_string('learn-more'), rt.call_function('__', [rt.new_string('Learn more'), rt.new_string('woocommerce')]), rt.call_function('admin_url', [rt.new_string('admin.php?page=wc-admin&path=/wc-pay-welcome-page')]))
	return mut var_note
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_paymentsremindmelater_php() {
