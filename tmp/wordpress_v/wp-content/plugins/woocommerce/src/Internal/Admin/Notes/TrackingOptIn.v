...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_TrackingOptIn) construct()  {
	rt.call_function('add_action', [rt.new_string('woocommerce_note_action_tracking-opt-in'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Notes_TrackingOptIn', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'opt_in_to_tracking' }])])
}

fn Class_Automattic_WooCommerce_Internal_Admin_Notes_TrackingOptIn.get_note() rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_string('yes'), rt.call_function('get_option', [rt.new_string('woocommerce_allow_tracking'), rt.new_string('no')]))) {
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_TrackingOptIn{}; return temp.is_wc_admin_active_in_date_range(arg_0) }(rt.new_string('week-1-4')))))) {
		return rt.new_null()
	}
	mut var_content_format := rt.call_function('__', [rt.new_string('Gathering usage data allows us to improve WooCommerce. Your store will be considered as we evaluate new features, judge the quality of an update, or determine if an improvement makes sense. You can always visit the %1$sSettings%3$s and choose to stop sharing data. %2$sRead more%3$s about what data we collect.'), rt.new_string('woocommerce')])
	mut var_note_content := rt.call_function('sprintf', [var_content_format.dup(), '<a href="' + (rt.call_function('esc_url', [rt.call_function('admin_url', [rt.new_string('admin.php?page=wc-settings&tab=advanced&section=woocommerce_com')])])).str() + '" target="_blank">', rt.new_string('<a href="https://woocommerce.com/usage-tracking?utm_medium=product" target="_blank">'), rt.new_string('</a>')])
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [rt.new_string('Help WooCommerce improve with usage tracking'), rt.new_string('woocommerce')]))
	var_note.set_content(var_note_content.dup())
	...
	var_note.add_action(rt.new_string('tracking-opt-in'), rt.call_function('__', [rt.new_string('Activate usage tracking'), rt.new_string('woocommerce')]), rt.new_bool(false), Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_actioned(), rt.new_bool(true), rt.call_function('__', [rt.new_string('Usage tracking activated'), rt.new_string('woocommerce')]))
	return mut var_note
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_TrackingOptIn) opt_in_to_tracking(var_note rt.PhpVal)  {
	...
	if rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_TrackingOptIn.note_name(), var_note_mutated.get_name())) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('class_exists', [rt.new_string('WC_Tracks')])) && rt.is_true(rt.identical(rt.new_string('no'), var_prev_value)))) {
			fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_WC_Tracks{}; return temp.track_woocommerce_allow_tracking_toggled(arg_0, arg_1, arg_2) }(var_prev_value.dup(), rt.new_string('yes'), rt.new_string('usage_tracking_note'))
		}
		rt.call_function('wp_schedule_single_event', [rt.add(rt.call_function('time', []rt.PhpVal{}), rt.new_int(10)), rt.new_string('woocommerce_tracker_send_event'), rt.create_array([rt.ArrayItem{ key: none, val: true }])])
	}
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_trackingoptin_php() {
