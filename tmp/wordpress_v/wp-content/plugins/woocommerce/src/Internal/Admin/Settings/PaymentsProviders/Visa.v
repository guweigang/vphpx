...
struct Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa {
	rt.PhpObjectBase
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: var_payment_gateway }, rt.ArrayItem{ key: none, val: 'get_config_settings' }])])) && rt.is_true(rt.call_function('defined', [rt.new_string('VISA_ACCEPTANCE_ENVIRONMENT_TEST')])))) && rt.is_true(rt.call_function('defined', [rt.new_string('VISA_ACCEPTANCE_ENVIRONMENT_PRODUCTION')])))) {
		mut var_settings := var_payment_gateway.get_config_settings()
		if rt.has_exception() { unsafe { goto catch_label_1 } }
		return rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_settings.dup().is_array())) && var_settings.array_isset(rt.new_string('environment')))) && rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.get_constant('VISA_ACCEPTANCE_ENVIRONMENT_TEST'), var_settings.array_get('environment'))) && !(!rt.is_true(var_settings.array_get('test_merchant_id'))))) && !(!rt.is_true(var_settings.array_get('test_api_key'))))) && !(!rt.is_true(var_settings.array_get('test_api_shared_secret'))))) || rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.get_constant('VISA_ACCEPTANCE_ENVIRONMENT_PRODUCTION'), var_settings.array_get('environment'))) && !(!rt.is_true(var_settings.array_get('merchant_id'))))) && !(!rt.is_true(var_settings.array_get('api_key'))))) && !(!rt.is_true(var_settings.array_get('api_shared_secret')))))))
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_visa_in_sandbox_mode(mut var_payment_gateway)).is_null() { this.is_visa_in_sandbox_mode(mut var_payment_gateway) } else { this.Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PaymentGateway.is_in_test_mode(rt.new_object('WC_Payment_Gateway', []string{}, var_payment_gateway)) }).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) is_in_dev_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_visa_in_sandbox_mode(mut var_payment_gateway)).is_null() { this.is_visa_in_sandbox_mode(mut var_payment_gateway) } else { this.Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PaymentGateway.is_in_dev_mode(rt.new_object('WC_Payment_Gateway', []string{}, var_payment_gateway)) }).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) is_in_test_mode_onboarding(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_visa_in_sandbox_mode(mut var_payment_gateway)).is_null() { this.is_visa_in_sandbox_mode(mut var_payment_gateway) } else { this.Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PaymentGateway.is_in_test_mode_onboarding(rt.new_object('WC_Payment_Gateway', []string{}, var_payment_gateway)) }).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) is_visa_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: var_payment_gateway }, rt.ArrayItem{ key: none, val: 'get_config_settings' }])])) && rt.is_true(rt.call_function('defined', [rt.new_string('VISA_ACCEPTANCE_ENVIRONMENT_TEST')])))) && rt.is_true(rt.call_function('defined', [rt.new_string('VISA_ACCEPTANCE_ENVIRONMENT_PRODUCTION')])))) {
		mut var_settings := var_payment_gateway.get_config_settings()
		if rt.has_exception() { unsafe { goto catch_label_2 } }
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_settings.dup().is_array())) && var_settings.array_isset(rt.new_string('environment')))) {
			if rt.is_true(rt.identical(rt.get_constant('VISA_ACCEPTANCE_ENVIRONMENT_TEST'), var_settings.array_get('environment'))) {
				return true
			}
			if rt.has_exception() { unsafe { goto catch_label_2 } }
			if rt.is_true(rt.identical(rt.get_constant('VISA_ACCEPTANCE_ENVIRONMENT_PRODUCTION'), var_settings.array_get('environment'))) {
				return false
			}
			if rt.has_exception() { unsafe { goto catch_label_2 } }
		}
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_visa() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa {
	mut obj := &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa{
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'is_visa_in_sandbox_mode' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WC_Payment_Gateway](if args.len > 0 { args[0] } else { rt.new_null() })
			return rt.new_bool(this.is_visa_in_sandbox_mode(mut dispatch_arg_0))
		}
		else { return none }
	}
}

fn (this &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_visa_php() {
