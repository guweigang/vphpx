...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_EditProductsOnTheMove.get_note() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_EditProductsOnTheMove{}; return temp.wc_admin_active_for(arg_0) }(var_year_in_seconds.dup()))))) {
		return rt.new_null()
	}
	...
	if rt.is_true(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_PerformanceOnMobile{}; return temp.has_note_been_actioned() }()) {
		return rt.new_null()
	}
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [rt.new_string('Edit products on the move'), rt.new_string('woocommerce')]))
	var_note.set_content(rt.call_function('__', [rt.new_string('Edit and create new products from your mobile devices with the Woo app'), rt.new_string('woocommerce')]))
...
fn create_automattic_woocommerce_internal_admin_notes_editproductsonthemove() &Class_Automattic_WooCommerce_Internal_Admin_Notes_EditProductsOnTheMove {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_editproductsonthemove_php() {
