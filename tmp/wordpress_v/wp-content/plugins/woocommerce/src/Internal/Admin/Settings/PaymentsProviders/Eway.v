...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Eway) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('customer_api')))) && !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('customer_password'))))
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Eway) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_eway_in_sandbox_mode(mut var_payment_gateway)).is_null() { this.is_eway_in_sandbox_mode(mut var_payment_gateway) } else { this.Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PaymentGateway.is_in_test_mode(rt.new_object('WC_Payment_Gateway', []string{}, var_payment_gateway)) }).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Eway) is_eway_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return (rt.call_function('wc_string_to_bool', [var_raw_option.dup()])).to_bool()
	}
	if rt.has_exception() { unsafe { goto catch_label_2 } }
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_eway() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Eway {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_eway_php() {
