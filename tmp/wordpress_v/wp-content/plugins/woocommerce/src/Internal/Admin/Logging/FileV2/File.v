...
fn Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File.parse_path(path string) rt.PhpVal {
	...
	mut var_parsed := rt.call_function('array_merge', [var_defaults.dup(), rt.call_function('pathinfo', [rt.new_string(path)])])
	mut var_segments := rt.call_function('explode', [rt.new_string('-'), var_parsed.array_get('filename')])
	mut var_timestamp := rt.call_function('strtotime', [rt.call_function('implode', [rt.new_string('-'), rt.call_function('array_slice', [var_segments.dup(), // unsupported expression: Expr_UnaryMinus, rt.new_int(3)])])])
	if rt.is_true(rt.new_bool(var_segments.dup().array_count() >= 5 && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		var_parsed.array_set('source', rt.call_function('implode', [rt.new_string('-'), rt.call_function('array_slice', [var_segments.dup(), rt.new_int(0), // unsupported expression: Expr_UnaryMinus])]))
		var_parsed.array_set('created', var_timestamp.dup())
		var_parsed.array_set('hash', rt.call_function('array_slice', [var_segments.dup(), // unsupported expression: Expr_UnaryMinus]).array_get(0))
	} else {
		var_parsed.array_set('source', rt.call_function('implode', [rt.new_string('-'), var_segments.dup()]))
	}
	mut var_rotation_marker := rt.call_function('strrpos', [var_parsed.array_get('source'), rt.new_string('.'), // unsupported expression: Expr_UnaryMinus])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		mut var_rotation := rt.call_function('substr', [var_parsed.array_get('source'), // unsupported expression: Expr_UnaryMinus])
		if rt.is_true(rt.new_bool(var_rotation.dup().is_long() || var_rotation.dup().is_double())) {
			var_parsed.array_set('rotation', var_rotation.dup().to_i64())
		}
		var_parsed.array_set('source', rt.call_function('substr', [var_parsed.array_get('source'), rt.new_int(0), var_rotation_marker.dup()]))
	}
	var_parsed.array_set('file_id', Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File.generate_file_id((var_parsed.array_get('source')).str(), mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_?int](var_parsed.array_get('rotation')), (var_parsed.array_get('created')).to_i64()))
...
fn Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File.generate_file_id(source string, mut var_rotation Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_?int, created i64) string {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_rotation_mutated.dup().is_null()))))) {
...
fn Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File.generate_hash(file_id string) string {
	...
	mut var_key := if !(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.get_constant(arg_0) }(rt.new_string('AUTH_SALT'))).is_null() { fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.get_constant(arg_0) }(rt.new_string('AUTH_SALT')) } else { rt.new_string('wc-logs') }
	return (rt.call_function('hash_hmac', [rt.new_string('md5'), rt.new_string(file_id_mutated).dup(), var_key.dup()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File) ingest_path()  {
	...
	this.source = var_parsed_path.array_get('source')
	this.rotation = var_parsed_path.array_get('rotation')
	this.created = var_parsed_path.array_get('created')
	this.hash = var_parsed_path.array_get('hash')
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File) is_readable() bool {
	...
	mut var_is_readable := rt.new_bool(rt.new_bool(rt.is_true(rt.call_method(var_filesystem, 'is_file', [this.path])) && rt.is_true(rt.call_method(var_filesystem, 'is_readable', [this.path]))))
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File) is_writable() bool {
	...
	mut var_is_writable := rt.new_bool(rt.new_bool(rt.is_true(rt.call_method(var_filesystem, 'is_file', [this.path])) && rt.is_true(rt.call_method(var_filesystem, 'is_writable', [this.path]))))
	if rt.has_exception() { unsafe { goto catch_label_2 } }
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File) get_created_timestamp() i64 {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(this.created)))) && this.is_readable())) {
		this.created = rt.call_function('filectime', [this.path])
	}
	return (this.created).to_i64()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File) get_modified_timestamp() rt.PhpVal {
	mut var_filesystem := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil{}; return temp.get_wp_filesystem() }()
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File) write(text string) bool {
	...
	if !(this.is_writable()) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_created)))) || !(this.is_writable()))) {
			return false
		}
	}
	mut var_eol_pos := rt.call_function('strrpos', [rt.new_string(text), rt.get_constant('PHP_EOL')])
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_bool(false), var_eol_pos)) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		// unsupported expression: Expr_AssignOp_Concat
	}
	...
	mut var_bytes_written := rt.call_function('fwrite', [var_resource.dup(), rt.new_string(text)])
	rt.call_function('reset_mbstring_encoding', []rt.PhpVal{})
	rt.call_function('fclose', [var_resource.dup()])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		return false
	}
	return true
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File) rotate() bool {
	...
	if rt.is_true(rt.new_bool(this.get_rotation().is_null())) {
		mut var_new_rotation := rt.new_int(rt.new_int(0))
	} else {
	...
	mut var_replace := rt.create_array([rt.ArrayItem{ key: none, val:  }])
	if this.has_standard_filename() {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_File) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_modified_timestamp' {
			return this.get_modified_timestamp()
		}
		'get_file_size' {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_logging_filev2_file_php() {
