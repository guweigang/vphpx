...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsController) register()  {
	rt.call_function('add_action', [rt.new_string('admin_init'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsController', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'handle_returns_from_wpcom' }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsController) handle_returns_from_wpcom()  {
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsService.wpcom_connection_return_param()))) {
		if !rt.is_true(rt.get_superglobal('_GET').array_get('source')) {
			return rt.new_null()
		}
		mut var_source := rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('source')])])
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_source.dup(), rt.create_array([rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsService.session_entry_default() }, rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsService.session_entry_lys() }]), rt.new_bool(true)]))))) {
			return rt.new_null()
		}
		mut var_location := rt.call_method(this.payments, 'get_country', []rt.PhpVal{})
		mut var_wpcom_connected := rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsService.onboarding_step_status_completed(), rt.call_method(this.woopayments, 'get_onboarding_step_status', [Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsService.onboarding_step_wpcom_connection(), var_location.dup()]))
		mut var_event_props := rt.create_array([rt.ArrayItem{ key: 'step_id', val: Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsService.onboarding_step_wpcom_connection() }, rt.ArrayItem{ key: 'source', val: var_source }])
		rt.call_method(this.woopayments, 'record_event', [if rt.is_true(var_wpcom_connected) { rt.new_string('wpcom_connection_success') } else { rt.new_string('wpcom_connection_failure') }, var_location.dup(), var_event_props.dup()])
		if rt.is_true(var_wpcom_connected) {
			rt.call_method(this.woopayments, 'mark_onboarding_step_completed', [Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsService.onboarding_step_wpcom_connection(), var_location.dup()])
		}
	}
}

fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_woopayments_woopaymentscontroller() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsController {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_woopayments_woopaymentscontroller_php() {
