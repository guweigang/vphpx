...
struct Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay {
	rt.PhpObjectBase
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: var_payment_gateway }, rt.ArrayItem{ key: none, val: 'get_merchant_id' }])])) && rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: var_payment_gateway }, rt.ArrayItem{ key: none, val: 'get_secret_key' }])])))) {
		return !(!rt.is_true(var_payment_gateway.get_merchant_id())) && !(!rt.is_true(var_payment_gateway.get_secret_key()))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_afterpay_clearpay_in_sandbox_mode(mut var_payment_gateway)).is_null() { this.is_afterpay_clearpay_in_sandbox_mode(mut var_payment_gateway) } else { this.Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PaymentGateway.is_in_test_mode(rt.new_object('WC_Payment_Gateway', []string{}, var_payment_gateway)) }).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) is_in_dev_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_afterpay_clearpay_in_sandbox_mode(mut var_payment_gateway)).is_null() { this.is_afterpay_clearpay_in_sandbox_mode(mut var_payment_gateway) } else { this.Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PaymentGateway.is_in_dev_mode(rt.new_object('WC_Payment_Gateway', []string{}, var_payment_gateway)) }).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) is_in_test_mode_onboarding(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_afterpay_clearpay_in_sandbox_mode(mut var_payment_gateway)).is_null() { this.is_afterpay_clearpay_in_sandbox_mode(mut var_payment_gateway) } else { this.Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PaymentGateway.is_in_test_mode_onboarding(rt.new_object('WC_Payment_Gateway', []string{}, var_payment_gateway)) }).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) is_afterpay_clearpay_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: var_payment_gateway }, rt.ArrayItem{ key: none, val: 'get_api_env' }])])) {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_afterpayclearpay() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay {
	mut obj := &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay{
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'is_afterpay_clearpay_in_sandbox_mode' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WC_Payment_Gateway](if args.len > 0 { args[0] } else { rt.new_null() })
			return rt.new_bool(this.is_afterpay_clearpay_in_sandbox_mode(mut dispatch_arg_0))
		}
		else { return none }
	}
}

fn (this &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_afterpayclearpay_php() {
