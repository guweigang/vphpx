...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Schedulers_MailchimpScheduler) construct(mut var_logger Class_Automattic_WooCommerce_Internal_Admin_Schedulers_?WC_Logger_Interface)  {
	...
	this.logger = var_logger_mutated.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Schedulers_MailchimpScheduler) run() bool {
	...
	if rt.is_true(rt.new_bool(!(var_profile_data.array_isset(rt.new_string('is_agree_marketing'))) || rt.is_true(rt.identical(rt.new_bool(false), var_profile_data.array_get('is_agree_marketing'))))) {
		return false
	}
	...
	mut var_response := this.make_request(var_profile_data.array_get('store_email'), var_address.dup())
	...
	mut var_body := rt.call_function('json_decode', [var_response.array_get('body')])
	if rt.is_true(rt.new_bool(!(rt.get_property(var_body, 'success')).is_null() && rt.is_true(rt.identical(rt.new_bool(true), rt.get_property(var_body, 'success'))))) {
	...
	this.handle_request_error(var_body.dup())
	return false
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Schedulers_MailchimpScheduler) make_request(var_store_email rt.PhpVal, var_address rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_bool(true), rt.call_function('defined', [rt.new_string('WP_ENVIRONMENT_TYPE')]))) && rt.is_true(rt.identical(rt.new_string('development'), rt.call_function('constant', [rt.new_string('WP_ENVIRONMENT_TYPE')]))))) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Schedulers_MailchimpScheduler) handle_request_error(var_extra_msg rt.PhpVal)  {
	mut var_msg := rt.new_string(if !(var_extra_msg).is_null() { 'Incorrect response from Mailchimp API with: ' + (println(var_extra_msg.dup().to_string())).str() } else { rt.new_string('Error getting a response from Mailchimp API.') })
	rt.call_method(this.logger, 'error', [var_msg.dup(), rt.create_array([rt.ArrayItem{ key: 'source', val: Class_Automattic_WooCommerce_Internal_Admin_Schedulers_Automattic_WooCommerce_Internal_Admin_Schedulers_MailchimpScheduler.logger_context() }])])
	...
	rt.call_function('update_option', [Class_Automattic_WooCommerce_Internal_Admin_Schedulers_Automattic_WooCommerce_Internal_Admin_Schedulers_MailchimpScheduler.subscribed_error_count_option_name(), var_accumulated_error_count.dup()])
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_schedulers_mailchimpscheduler_php() {
}
