...
pub fn Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack.backup_ids() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: none, val: 2010 }, rt.ArrayItem{ key: none, val: 2011 }, rt.ArrayItem{ key: none, val: 2012 }, rt.ArrayItem{ key: none, val: 2013 }, rt.ArrayItem{ key: none, val: 2014 }, rt.ArrayItem{ key: none, val: 2015 }, rt.ArrayItem{ key: none, val: 2100 }, rt.ArrayItem{ key: none, val: 2101 }, rt.ArrayItem{ key: none, val: 2102 }, rt.ArrayItem{ key: none, val: 2103 }, rt.ArrayItem{ key: none, val: 2005 }, rt.ArrayItem{ key: none, val: 2006 }, rt.ArrayItem{ key: none, val: 2000 }, rt.ArrayItem{ key: none, val: 2003 }, rt.ArrayItem{ key: none, val: 2001 }, rt.ArrayItem{ key: none, val: 2004 }])
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack.possibly_add_note()  {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string('jetpack'), var_installed_plugins.dup(), rt.new_bool(true)]))))) {
		return rt.new_null()
	}
	mut var_data_store := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_WC_Data_Store{}; return temp.load(arg_0) }(rt.new_string('admin-note'))
	mut var_note_ids := rt.call_method(var_data_store, 'get_notes_with_name', [Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack.note_name()])
	if !(!rt.is_true(var_note_ids)) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack.has_backups()) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack{}; return temp.is_wc_admin_active_in_date_range(arg_0, arg_1) }(rt.new_string('week-1-4'), rt.mul(rt.get_constant('DAY_IN_SECONDS'), rt.new_int(3))))))) || rt.is_true(rt.new_bool(!(rt.is_true(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack{}; return temp.can_be_added() }())))))) || rt.is_true(Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack.has_backups()))) {
		return rt.new_null()
	}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack.get_note() rt.PhpVal {
	...
	rt.call_method(var_note, 'set_title', [rt.call_function('__', [rt.new_string('Protect your WooCommerce Store with Jetpack Backup.'), rt.new_string('woocommerce')])])
	rt.call_method(var_note, 'set_content', [rt.call_function('__', [rt.new_string('Store downtime means lost sales. One-click restores get you back online quickly if something goes wrong.'), rt.new_string('woocommerce')])])
	rt.call_method(var_note, 'set_type', [Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_marketing()])
	rt.call_method(var_note, 'set_name', [Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack.note_name()])
	rt.call_method(var_note, 'set_layout', [rt.new_string('thumbnail')])
	rt.call_method(var_note, 'set_image', [(rt.get_constant('WC_ADMIN_IMAGES_FOLDER_URL')).str() + '/admin_notes/marketing-jetpack-2x.png'])
	...
	rt.call_method(var_note, 'add_action', [rt.new_string('jetpack-backup-woocommerce'), rt.call_function('__', [rt.new_string('Get backups'), rt.new_string('woocommerce')]), rt.call_function('esc_url', [rt.new_string('https://jetpack.com/upgrade/backup-woocommerce/?utm_source=inbox&utm_medium=automattic_referred&utm_campaign=jp_backup_to_woo')]), Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_actioned()])
	return var_note.dup()
}

fn Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack.has_backups() rt.PhpVal {
	...
	if !(!rt.is_true(var_plan)) {
		var_product_ids.array_push(var_plan.array_get('product_id'))
	}
	mut var_products := rt.call_function('get_option', [rt.new_string('jetpack_site_products')])
	if !(!rt.is_true(var_products)) {
		{
			...
			for {
				...
				var_product_ids.array_push(var_product.array_get('product_id'))
			}
		}
	}
...
fn create_automattic_woocommerce_internal_admin_notes_marketingjetpack() &Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack {
...
fn create_automattic_woocommerce_internal_admin_notes_wc_data_store() &Class_Automattic_WooCommerce_Internal_Admin_Notes_WC_Data_Store {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'has_backups' {
			return Class_Automattic_WooCommerce_Internal_Admin_Notes_MarketingJetpack.has_backups()
		}
		else { return none }
	}
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_marketingjetpack_php() {
