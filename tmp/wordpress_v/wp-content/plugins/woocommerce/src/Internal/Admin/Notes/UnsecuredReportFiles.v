...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_UnsecuredReportFiles.get_note() rt.PhpVal {
	...
	rt.call_method(var_note, 'set_title', [rt.call_function('__', [rt.new_string('Potentially unsecured files were found in your uploads directory'), rt.new_string('woocommerce')])])
	rt.call_method(var_note, 'set_content', [rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Files that may contain %1$sstore analytics%2$s reports were found in your uploads directory - we recommend assessing and deleting any such files.'), rt.new_string('woocommerce')]), rt.new_string('<a href="https://woocommerce.com/document/woocommerce-analytics/" target="_blank">'), rt.new_string('</a>')])])
	rt.call_method(var_note, 'set_content_data', [// unsupported expression: Expr_Cast_Object])
	rt.call_method(var_note, 'set_type', [Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_error()])
	rt.call_method(var_note, 'set_name', [Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_UnsecuredReportFiles.note_name()])
	rt.call_method(var_note, 'set_source', [rt.new_string('woocommerce-admin')])
	rt.call_method(var_note, 'add_action', [rt.new_string('learn-more'), rt.call_function('__', [rt.new_string('Learn more'), rt.new_string('woocommerce')]), rt.new_string('https://developer.woocommerce.com/2021/09/22/important-security-patch-released-in-woocommerce/'), Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_unactioned(), rt.new_bool(true)])
	rt.call_method(var_note, 'add_action', [rt.new_string('dismiss'), rt.call_function('__', [rt.new_string('Dismiss'), rt.new_string('woocommerce')]), rt.call_function('wc_admin_url', []rt.PhpVal{}), Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_actioned(), rt.new_bool(false)])
	return var_note.dup()
}

fn Class_Automattic_WooCommerce_Internal_Admin_Notes_UnsecuredReportFiles.possibly_add_note()  {
	mut var_note := Class_Automattic_WooCommerce_Internal_Admin_Notes_UnsecuredReportFiles.get_note()
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_UnsecuredReportFiles.note_exists() bool {
	...
	mut var_note_ids := rt.call_method(var_data_store, 'get_notes_with_name', [Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_UnsecuredReportFiles.note_name()])
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_unsecuredreportfiles_php() {
	rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('ABSPATH')])) || rt.is_true(// unsupported expression: Expr_Exit))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [Class_Automattic_WooCommerce_Admin_Notes_Note.class()]))))) {
		rt.call_function('class_alias', [Class_Automattic_WooCommerce_Internal_Admin_Notes_WC_Admin_Note.class(), Class_Automattic_WooCommerce_Admin_Notes_Note.class()])
	}
}
