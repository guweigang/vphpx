...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_PostsRedirectionController) maybe_redirect_to_orders_page() {
	mut var_post_type := if !(rt.get_superglobal('_GET').array_get('post_type')).is_null() {
		rt.get_superglobal('_GET').array_get('post_type')
	} else {
		rt.new_string('')
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_post_type))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_post_type.dup(), rt.call_function('wc_get_order_types', [rt.new_string('admin-menu')]), rt.new_bool(true)])))))))
	{
		return rt.new_null()
	}
	mut var_query_args := rt.call_function('wp_unslash', [rt.get_superglobal('_GET').dup()])
	mut var_action := if !(var_query_args.array_get('action')).is_null() {
		var_query_args.array_get('action')
	} else {
		rt.new_string('')
	}
	mut var_posts := if !(var_query_args.array_get('post')).is_null() {
		var_query_args.array_get('post')
	} else {
		rt.new_array()
	}
	...
	if var_query_args.array_isset(rt.new_string('post_status')) {
		var_query_args.array_set('status', var_query_args.array_get('post_status'))
	...
	var_new_url = rt.call_function('add_query_arg', [var_query_args.dup(),
		var_new_url.dup()])
	if rt.is_true(rt.new_bool(rt.is_true(var_action)
		&& rt.is_true(rt.call_function('in_array', [var_action.dup(), rt.create_array([rt.ArrayItem{
		key: none
		val: 'trash'
	...
	rt.call_function('wp_safe_redirect', [var_new_url.dup(), rt.new_int(301)])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_PostsRedirectionController) maybe_redirect_to_edit_order_page() {
	mut var_post_id := rt.call_function('absint', [if !(rt.get_superglobal('_GET').array_get('post')).is_null() {
		rt.get_superglobal('_GET').array_get('post')
	} else {
		rt.new_int(0)
	}])
	...
	mut var_order_type := if rt.is_true(var_post_type) {
		var_post_type
	} else {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_order_type.dup(), var_redirect_from_types.dup(), rt.new_bool(true)])))))
		|| !(rt.get_superglobal('_GET').array_isset(rt.new_string('action')))))
	{
		return rt.new_null()
	}
	mut var_query_args := rt.call_function('wp_unslash', [rt.get_superglobal('_GET').dup()])
	mut var_action := var_query_args.array_get('action')
	var_query_args.array_unset(rt.new_string('post'))
	var_query_args.array_unset(rt.new_string('_wpnonce'))
	var_query_args.array_unset(rt.new_string('_wp_http_referer'))
	var_query_args.array_unset(rt.new_string('action'))
	mut var_new_url := rt.new_string(rt.new_string(''))
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('edit'))) {
		...
		rt.call_function('check_admin_referer', [
			var_action.str() + '-post_' + var_post_id.str(),
		])
		var_new_url = rt.call_function('add_query_arg', [
			rt.create_array([rt.ArrayItem{ key: 'action', val: var_action },
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_PostsRedirectionController) maybe_update_menu_items() {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('edit.php'), var_pagenow))
		&& rt.is_true(rt.call_function('in_array', [if !(rt.get_superglobal('_GET').array_get('post_type')).is_null() { rt.get_superglobal('_GET').array_get('post_type') } else { rt.new_string('') }, rt.call_function('wc_get_order_types', [rt.new_string('admin-menu')]), rt.new_bool(true)]))))
	{
		return rt.new_null()
	}
	if rt.is_true(fn () rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Internal_Admin_Orders_WC_Admin_Menus{}
		return temp.can_view_woocommerce_menu_item()
	}())
	{
		return rt.new_null()
	}
		...
		for {
				...
				closure_7_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
					closure_6_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
						closure_5_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
							mut var_x := if args.len > 0 { args[0].dup() } else { rt.new_null() }
							return var_x.array_get(2)
						}
						mut var_x := if args.len > 0 { args[0].dup() } else { rt.new_null() }
						return var_x.array_get(2)
					}
			...
			mut var_menu_indexes := rt.call_function('array_flip', [
				rt.call_function('array_map', [rt.new_closure(closure_5_fn),
					var_post_type_menu.dup()]),
			])
			var_post_type_menu.array_get_mut(var_menu_indexes.array_get('edit.php?post_type=' +
				(rt.get_property(var_post_type, 'name')).str())).array_set(2, rt.call_method(this.page_controller,
				'get_base_page_url', [rt.get_property(var_post_type, 'name')]))
			var_post_type_menu.array_unset(var_menu_indexes.array_get(rt.concat(rt.new_string('post-new.php?post_type='), rt.get_property(var_post_type,
				'name'))))
		}
	}
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_orders_postsredirectioncontroller_php() {
}
