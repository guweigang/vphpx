...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Antom) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('function_exists', [rt.new_string('\\antom_is_active')])) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('antom_is_active', []rt.PhpVal{}))))))) {
		return false
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('\\antom_get_core_settings')])) {
		mut var_core_settings := rt.call_function('antom_get_core_settings', []rt.PhpVal{})
		if rt.has_exception() { unsafe { goto catch_label_1 } }
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_core_settings.dup().is_array()))))) {
			return false
		}
		if rt.has_exception() { unsafe { goto catch_label_1 } }
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Antom) is_antom_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('\\antom_get_core_settings')])) {
		return (rt.call_function('wc_string_to_bool', [rt.call_function('antom_get_core_settings', []rt.PhpVal{}).array_get('test_mode')])).to_bool()
	}
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_antom() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Antom {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_antom_php() {
