...
struct Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Tilopay {
	rt.PhpObjectBase
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Tilopay) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('tpay_key')])) && !(!rt.is_true(rt.get_property(var_payment_gateway, 'tpay_key'))))) && rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('tpay_user')])))) && !(!rt.is_true(rt.get_property(var_payment_gateway, 'tpay_user'))))) && rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('tpay_password')])))) && !(!rt.is_true(rt.get_property(var_payment_gateway, 'tpay_password')))
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_tilopay() &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Tilopay {
	mut obj := &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Tilopay{
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Tilopay) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
...
fn (this &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Tilopay) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Tilopay) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_settings_paymentsproviders_tilopay_php() {
