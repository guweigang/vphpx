...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_ManageOrdersOnTheGo.get_note() rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Admin_Notes_ManageOrdersOnTheGo{}; return temp.is_wc_admin_active_in_date_range(arg_0) }(rt.new_string('month-6+')))))) {
		return rt.new_null()
	}
	...
	var_note.set_title(rt.call_function('__', [rt.new_string('Manage your orders on the go'), rt.new_string('woocommerce')]))
	var_note.set_content(rt.call_function('__', [rt.new_string('Look for orders, customer info, and process refunds in one click with the Woo app.'), rt.new_string('woocommerce')]))
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_notes_manageordersonthego_php() {
