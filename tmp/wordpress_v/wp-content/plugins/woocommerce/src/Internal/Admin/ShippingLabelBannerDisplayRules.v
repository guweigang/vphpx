...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ShippingLabelBannerDisplayRules) construct(var_dotcom_connected rt.PhpVal, var_wcs_version rt.PhpVal, var_incompatible_plugins_installed rt.PhpVal)  {
	this.dotcom_connected = var_dotcom_connected.dup()
	this.wcs_version = var_wcs_version.dup()
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ShippingLabelBannerDisplayRules) should_display_banner() bool {
	return rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(this.banner_not_dismissed() && rt.is_true(this.dotcom_connected))) && rt.is_true(this.no_incompatible_plugins_installed))) && this.order_has_shippable_products())) && this.store_in_us_and_usd())) && this.wcs_not_installed()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ShippingLabelBannerDisplayRules) banner_not_dismissed() bool {
	mut var_dismissed_timestamp_ms := rt.call_function('get_option', [rt.new_string('woocommerce_shipping_dismissed_timestamp')])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_dismissed_timestamp_ms.dup().is_long() || var_dismissed_timestamp_ms.dup().is_double()))))) {
		return true
	}
	var_dismissed_timestamp_ms = rt.new_int(rt.new_int(var_dismissed_timestamp_ms.dup().to_i64()))
	mut var_dismissed_timestamp := rt.new_int(rt.new_int(rt.call_function('round', [rt.div(var_dismissed_timestamp_ms, rt.new_int(1000))]).to_i64()))
	mut var_expired_timestamp := rt.add(var_dismissed_timestamp, 24 * 60 * 60)
	mut var_dismissed_for_good := rt.identical(// unsupported expression: Expr_UnaryMinus, var_dismissed_timestamp_ms)
	mut var_dismissed_24h := rt.less(rt.call_function('time', []rt.PhpVal{}), var_expired_timestamp)
	return rt.is_true(rt.new_bool(!(rt.is_true(var_dismissed_for_good)))) && rt.is_true(rt.new_bool(!(rt.is_true(var_dismissed_24h))))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ShippingLabelBannerDisplayRules) store_in_us_and_usd() bool {
	...
	return rt.is_true(rt.call_function('in_array', [var_base_currency.dup(), this.supported_currencies, rt.new_bool(true)])) && rt.is_true(rt.call_function('in_array', [var_base_location.array_get('country'), this.supported_countries, rt.new_bool(true)]))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ShippingLabelBannerDisplayRules) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'dotcom_connected' { this.dotcom_connected = val; return true }
		'no_incompatible_plugins_installed' { this.no_incompatible_plugins_installed = (val).to_bool(); return true }
		'wcs_version' { this.wcs_version = val; return true }
		'supported_countries' { this.supported_countries = val; return true }
		'supported_currencies' { this.supported_currencies = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_internal_admin_shippinglabelbannerdisplayrules_php() {
}
