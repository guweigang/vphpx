...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableRefundDataStore) delete(var_refund rt.PhpVal, var_args rt.PhpVal) {
	...
	mut var_refund_cache_key := rt.new_string(
		(fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_WC_Cache_Helper{}
		return temp.get_cache_prefix(arg_0)
	}(rt.new_string('orders'))).str() +
		'refund_ids' + (rt.call_method(var_refund, 'get_parent_id', []rt.PhpVal{})).str())
	rt.call_function('wp_cache_delete', [var_refund_cache_key.dup(),
		rt.new_string('orders')])
	this.delete_order_data_from_custom_order_tables(var_refund_id.dup())
	...
	if rt.is_true(var_orders_table_is_authoritative) {
		...
		if rt.is_true(rt.call_method(var_data_synchronizer, 'data_sync_is_enabled', []rt.PhpVal{})) {
			rt.call_function('wp_delete_post', [var_refund_id.dup()])
		} else {
			this.handle_order_deletion_with_sync_disabled(var_refund_id.dup())
		}
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableRefundDataStore) set_order_props_from_data(var_refund rt.PhpVal, var_data rt.PhpVal) {
	this.Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStore.set_order_props_from_data(var_refund.dup(),
		var_data.dup())
	{
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableRefundDataStore) create(var_refund rt.PhpVal) {
	...
	this.persist_save(var_refund.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableRefundDataStore) update(var_refund rt.PhpVal) {
	this.persist_updates(var_refund.dup())
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableRefundDataStore) update_order_meta(var_refund rt.PhpVal) {
	this.Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStore.update_order_meta(var_refund.dup())
		...
		var_meta_key_to_props.dup())
	{
		...
		for {
			...
			rt.set_property(var_meta_object, 'key', var_meta_key.dup())
			...
			mut var_existing_meta := rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableRefundDataStore', [
				...
				var_refund.dup(), var_meta_key.dup()])
			if rt.is_true(var_existing_meta) {
				var_existing_meta = var_existing_meta.array_get(0)
				rt.set_property(var_meta_object, 'id', rt.get_property(var_existing_meta, 'id'))
				this.update_meta(var_refund.dup(), rt.new_object('WC_Meta_Data', []string{},
					var_meta_object))
			} else {
				this.add_meta(var_refund.dup(), rt.new_object('WC_Meta_Data', []string{},
					var_meta_object))
			}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableRefundDataStore) get_post_data_store_for_backfill() rt.PhpVal {
	return create_automattic_woocommerce_internal_datastores_orders_wc_order_refund_data_store_cpt()
}
...
fn create_automattic_woocommerce_internal_datastores_orders_orderstablerefunddatastore() &Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableRefundDataStore {
...
fn create_automattic_woocommerce_internal_datastores_orders_datetime() &Class_Automattic_WooCommerce_Internal_DataStores_Orders_DateTime {
...
fn create_automattic_woocommerce_internal_datastores_orders_wc_order_refund_data_store_cpt() &Class_Automattic_WooCommerce_Internal_DataStores_Orders_WC_Order_Refund_Data_Store_CPT {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_datastores_orders_orderstablerefunddatastore_php() {
}
