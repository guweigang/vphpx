...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_ProductSettingsSchema) get_item_response(var_item rt.PhpVal, mut var_request Class_WP_REST_Request, mut var_include_fields Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_array) rt.PhpVal {
	...
	mut var_current_group_id := rt.new_null()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(var_setting.array_isset(rt.new_string('id')) && rt.is_true(var_current_group))) {
				mut var_field := this.transform_setting_to_field(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_array](var_setting))
	...
	return rt.create_array([rt.ArrayItem{ key: 'id', val: 'products' }, rt.ArrayItem{ key: 'title', val: rt.call_function('__', [rt.new_string('Products'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Manage product settings including dimensions, weight units, and display options.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'values', val: var_values }, rt.ArrayItem{ key: 'groups', val: var_groups }])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_ProductSettingsSchema) get_field_options(field_id string) rt.PhpVal {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('woocommerce_weight_unit'))) {
		return rt.create_array([rt.ArrayItem{ key: 'kg', val: rt.call_function('__', [rt.new_string('kg'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'g', val: rt.call_function('__', [rt.new_string('g'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'lbs', val: rt.call_function('__', [rt.new_string('lbs'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'oz', val: rt.call_function('__', [rt.new_string('oz'), rt.new_string('woocommerce')]) }])
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('woocommerce_dimension_unit'))) {
		return rt.create_array([rt.ArrayItem{ key: 'm', val: rt.call_function('__', [rt.new_string('m'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'cm', val: rt.call_function('__', [rt.new_string('cm'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'mm', val: rt.call_function('__', [rt.new_string('mm'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'in', val: rt.call_function('__', [rt.new_string('in'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'yd', val: rt.call_function('__', [rt.new_string('yd'), rt.new_string('woocommerce')]) }])
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('woocommerce_product_type'))) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('wc_get_product_types')]))))) {
			return rt.new_array()
		}
		mut var_product_types := rt.call_function('wc_get_product_types', []rt.PhpVal{})
		return if rt.is_true(rt.new_bool(var_product_types.dup().is_array())) { var_product_types } else { rt.new_array() }
	}
	return rt.new_array()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_ProductSettingsSchema) normalize_field_type(wc_type string) string {
	mut var_type_map := rt.create_array([rt.ArrayItem{ key: 'single_select_product', val: 'select' }, rt.ArrayItem{ key: 'multi_select_product', val: 'multiselect' }])
	return (if !(var_type_map.array_get(wc_type)).is_null() { var_type_map.array_get(wc_type) } else { rt.new_string(wc_type) }).str()
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_ProductSettingsSchema) validate_field_value(var_value rt.PhpVal, type string)  {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_products_schema_productsettingsschema() &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_ProductSettingsSchema {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_ProductSettingsSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_item_response' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_array](if args.len > 2 { args[2] } else { rt.new_null() })
			return this.get_item_response(dispatch_arg_0, mut dispatch_arg_1, mut dispatch_arg_2)
		}
		'transform_setting_to_field' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_array](if args.len > 0 { args[0] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_src_internal_restapi_routes_v4_settings_products_schema_productsettingsschema_php() {
