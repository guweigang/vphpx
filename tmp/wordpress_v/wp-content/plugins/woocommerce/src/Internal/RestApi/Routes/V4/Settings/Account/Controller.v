...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Account_Controller) get_settings_account_instance() rt.PhpVal {
	if rt.is_true(rt.new_bool(this.settings_account_instance.is_null())) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [rt.new_string('WC_Admin_Settings')]))))) {
			rt.include_file((rt.get_constant('WC_ABSPATH')).str() + 'includes/admin/class-wc-admin-settings.php', '4')
		}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Account_Controller) get_item_permissions_check(var_request rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wc_rest_check_manager_permissions', [rt.new_string('settings'), rt.new_string('read')]))))) {
		return (create_wp_error(rt.new_string('rest_forbidden'), rt.call_function('__', [rt.new_string('Sorry, you are not allowed to access account settings.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code', []rt.PhpVal{}) }]))).to_bool()
	}
	return true
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Account_Controller) update_item_permissions_check(var_request rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wc_rest_check_manager_permissions', [rt.new_string('settings'), rt.new_string('edit')]))))) {
		return (create_wp_error(rt.new_string('rest_forbidden'), rt.call_function('__', [rt.new_string('Sorry, you are not allowed to edit account settings.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code', []rt.PhpVal{}) }]))).to_bool()
	}
	return true
}
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Account_Exception') {
		mut var_e := var_e_1.dup()
		return create_wp_error(rt.new_string('woocommerce_rest_account_settings_error'), rt.call_method(var_e, 'getMessage', []rt.PhpVal{}), rt.create_array([rt.ArrayItem{ key: 'status', val: 500 }]))
		unsafe { goto end_label_1 }
	}
	else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Account_Controller) sanitize_setting_value(var_setting_type rt.PhpVal, var_value rt.PhpVal)  {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_account_controller() &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Account_Controller {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Account_Controller) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Account_Schema_AccountSettingsSchema](if args.len > 0 { args[0] } else { rt.new_null() })
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Account_Controller) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'settings_account_instance' { this.settings_account_instance = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_internal_restapi_routes_v4_settings_account_controller_php() {
