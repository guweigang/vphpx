...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZones_ShippingZoneService) get_sorted_shipping_zones() rt.PhpVal {
	mut var_zones := fn () rt.PhpVal {
		mut temp := Class_WC_Shipping_Zones{}
		return temp.get_zones()
	}()
	mut var_rest_of_the_world := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal {
		mut temp := Class_WC_Shipping_Zones{}
		return temp.get_zone_by(arg_0, arg_1)
	}(rt.new_string('zone_id'), rt.new_int(0))
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZones_ShippingZoneService) create_shipping_zone(var_params rt.PhpVal) rt.PhpVal {
	...
	mut var_result := this.update_shipping_zone(var_zone.dup(), var_params_mutated.dup())
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZones_ShippingZoneService) update_shipping_zone(var_zone rt.PhpVal, var_params rt.PhpVal) rt.PhpVal {
	...
	mut var_params_mutated := var_params
	var_params_mutated = rt.call_function('wp_parse_args', [var_params_mutated.dup(),
	...
	mut var_is_rest_of_world := rt.identical(rt.new_int(0), rt.call_method(var_zone_mutated,
		'get_id', []rt.PhpVal{}))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_params_mutated.array_get('name').is_null()))))) {
		if rt.is_true(var_is_rest_of_world) {
			return create_wp_error(rt.new_string('woocommerce_rest_cannot_edit_zone'), rt.call_function('__', [
		...
		mut var_name :=
			rt.new_string(rt.new_string(var_params_mutated.array_get('name').to_string().trim_space()))
		if rt.is_true(rt.identical(rt.new_string(''), var_name)) {
			return create_wp_error(rt.new_string('woocommerce_rest_invalid_zone_name'), rt.call_function('__', [
		...
		rt.call_method(var_zone_mutated, 'set_zone_name', [var_name.dup()])
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_params_mutated.array_get('order').is_null()))))) {
		if rt.is_true(var_is_rest_of_world) {
			return create_wp_error(rt.new_string('woocommerce_rest_cannot_edit_zone'), rt.call_function('__', [
		...
		rt.call_method(var_zone_mutated, 'set_zone_order', [var_params_mutated.array_get('order')])
	}
	mut var_locations_being_cleared := rt.new_bool(rt.new_bool(false))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_params_mutated.array_get('locations').is_null()))))) {
		if rt.is_true(var_is_rest_of_world) {
			return create_wp_error(rt.new_string('woocommerce_rest_cannot_edit_zone'), rt.call_function('__', [
		...
		mut var_raw_locations := var_params_mutated.array_get('locations')
		mut var_locations := rt.new_array()
		{
			...
			for {
				...
				var_locations_being_cleared = rt.new_bool(rt.new_bool(false))
				if !rt.is_true(var_raw_location.array_get('code')) {
					continue
				}
				mut var_type := if !(!rt.is_true(var_raw_location.array_get('type'))) {
					var_raw_location.array_get('type')
				} else {
					rt.new_string('country')
				}
				if rt.is_true(rt.identical(rt.new_string('country:state'), var_type)) {
					var_type = rt.new_string(rt.new_string('state'))
				}
				if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_zone_mutated,
					'is_valid_location_type', [var_type.dup()])))))
				{
					continue
				}
				var_locations.array_push(rt.create_array([
					rt.ArrayItem{ key: 'code', val: var_raw_location.array_get('code') },
					rt.ArrayItem{ key: 'type', val: var_type },
				]))
			}
		}
		var_locations_being_cleared = rt.new_bool(!rt.is_true(var_locations))
		rt.call_method(var_zone_mutated, 'set_locations', [var_locations.dup()])
	}
	rt.call_method(var_zone_mutated, 'save', []rt.PhpVal{})
	if rt.is_true(var_locations_being_cleared) {
		var_zone_mutated = fn (arg_0 rt.PhpVal) rt.PhpVal {
			mut temp := Class_WC_Shipping_Zones{}
			return temp.get_zone(arg_0)
		}(rt.call_method(var_zone_mutated, 'get_id', []rt.PhpVal{}))
	}
	return var_zone_mutated.dup()
}
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_shippingzones_shippingzoneservice() &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZones_ShippingZoneService {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_restapi_routes_v4_shippingzones_shippingzoneservice_php() {
