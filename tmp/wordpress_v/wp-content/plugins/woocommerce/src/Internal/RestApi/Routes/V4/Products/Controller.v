...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Products_Controller) get_item_permissions_check(var_request rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_object) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		...
		if rt.is_true(rt.new_bool(rt.instance_of(var_post_type_object, 'Automattic_WooCommerce_Internal_RestApi_Routes_V4_Products_WP_Post_Type'))) {
			var_permission = rt.call_function('current_user_can', [rt.get_property(rt.get_property(var_post_type_object, 'cap'), 'read_private_posts'), var_object_id.dup()])
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_permission)))) && rt.is_true(rt.identical(rt.new_string('publish'), rt.call_method(var_object, 'get_status', []rt.PhpVal{}))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('post_password_required', [var_object_id.dup()]))))))) {
				var_permission = rt.new_bool(rt.new_bool(rt.is_true(rt.call_function('current_user_can', [rt.new_string('edit_posts')])) && rt.is_true(rt.call_function('current_user_can', [rt.get_property(rt.get_property(var_post_type_object, 'cap'), 'read'), var_object_id.dup()]))))
			}
		}
		var_permission = rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_check_permissions'), var_permission.dup(), rt.new_string('read'), var_object_id.dup(), rt.get_property(var_object, 'post_type')])
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Products_Controller) prepare_objects_query(var_request rt.PhpVal) rt.PhpVal {
	...
	if !(!rt.is_true(var_request.array_get('exclude_category'))) {
		var_tax_query.array_push(rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: 'product_cat' }, rt.ArrayItem{ key: 'field', val: 'term_id' }, rt.ArrayItem{ key: 'terms', val: var_request.array_get('exclude_category') }, rt.ArrayItem{ key: 'operator', val: 'NOT IN' }]))
	}
	...
	if !(!rt.is_true(var_request.array_get('exclude_types'))) {
		var_tax_query.array_push(rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: 'product_type' }, rt.ArrayItem{ key: 'field', val: 'slug' }, rt.ArrayItem{ key: 'terms', val: var_request.array_get('exclude_types') }, rt.ArrayItem{ key: 'operator', val: 'NOT IN' }]))
	}
	if !(!rt.is_true(var_request.array_get('attribute'))) && !(!rt.is_true(var_request.array_get('attribute_term'))) {
		if rt.is_true(rt.call_function('in_array', [var_request.array_get('attribute'), rt.call_function('wc_get_attribute_taxonomy_names', []rt.PhpVal{}), rt.new_bool(true)])) {
			var_tax_query.array_push(rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: var_request.array_get('attribute') }, rt.ArrayItem{ key: 'field', val: 'term_id' }, rt.ArrayItem{ key: 'terms', val: var_request.array_get('attribute_term') }]))
		}
	}
	...
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		...
		if !(!rt.is_true(var_request.array_get('sku'))) {
			mut var_skus := rt.call_function('explode', [rt.new_string(','), .array_get()])
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_products_controller() &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Products_Controller {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_restapi_routes_v4_products_controller_php() {
