...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) get_item_response(var_item rt.PhpVal, mut var_request Class_WP_REST_Request, mut var_include_fields Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_array) rt.PhpVal {
	...
	mut var_current_group_id := rt.new_null()
	{
		...
		for {
			...
			if rt.is_true(rt.call_function('in_array', [var_setting_type.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'title' }, rt.ArrayItem{ key: none, val: 'sectionend' }, rt.ArrayItem{ key: none, val: 'conflict_error' }, rt.ArrayItem{ key: none, val: 'add_settings_slot' }]), rt.new_bool(true)])) {
				continue
			}
			if rt.is_true(rt.new_bool(var_setting.array_isset(rt.new_string('id')) && rt.is_true(var_current_group))) {
				mut var_field := this.transform_setting_to_field(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_array](var_setting))
	...
	mut var_response := rt.create_array([rt.ArrayItem{ key: 'id', val: 'tax' }, rt.ArrayItem{ key: 'title', val: rt.call_function('__', [rt.new_string('Taxes'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Manage your store’s tax setup.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'values', val: var_values }, rt.ArrayItem{ key: 'groups', val: var_groups }])
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) transform_setting_to_field(mut var_setting Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_array) rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) normalize_field_type(wc_type string) string {
	mut var_type_map := rt.create_array([rt.ArrayItem{ key: 'single_select_country', val: 'select' }, rt.ArrayItem{ key: 'multi_select_countries', val: 'multiselect' }, rt.ArrayItem{ key: 'radio', val: 'radio' }])
	return (if !(var_type_map.array_get(wc_type)).is_null() { var_type_map.array_get(wc_type) } else { rt.new_string(wc_type) }).str()
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) validate_field_value(var_value rt.PhpVal, type string)  {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('number'))) {
		...
		return if rt.is_true(rt.new_bool(var_value.dup().is_array())) { var_value } else { rt.new_array() }
	} else {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_tax_schema_taxsettingsschema() &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema {
	mut obj := &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema{
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_item_response' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_array](if args.len > 2 { args[2] } else { rt.new_null() })
			return this.get_item_response(dispatch_arg_0, mut dispatch_arg_1, mut dispatch_arg_2)
		}
		'transform_setting_to_field' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_array](if args.len > 0 { args[0] } else { rt.new_null() })
...
fn (this &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_restapi_routes_v4_settings_tax_schema_taxsettingsschema_php() {
