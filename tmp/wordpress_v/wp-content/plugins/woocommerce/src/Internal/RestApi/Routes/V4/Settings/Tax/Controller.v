...
struct Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller {
	rt.PhpObjectBase
pub mut:
		rest_base rt.PhpVal = rt.new_string('settings/tax')
		settings_tax_instance rt.PhpVal = rt.new_null()
		item_schema rt.PhpVal = rt.new_null()
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) init(mut var_item_schema Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema)  {
	this.item_schema = var_item_schema.dup()
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) get_settings_tax_instance() rt.PhpVal {
	if rt.is_true(rt.new_bool(this.settings_tax_instance.is_null())) {
		this.settings_tax_instance = create_wc_settings_tax()
	}
	return this.settings_tax_instance
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) register_routes()  {
	rt.call_function('register_rest_route', [rt.get_property(rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller', ['Automattic_WooCommerce_Internal_RestApi_Routes_V4_AbstractController'], &this), 'namespace'), '/' + (this.rest_base).str(), rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.readable() }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller', ['Automattic_WooCommerce_Internal_RestApi_Routes_V4_AbstractController'], &this) }, rt.ArrayItem{ key: none, val: 'get_item' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller', ['Automattic_WooCommerce_Internal_RestApi_Routes_V4_AbstractController'], &this) }, rt.ArrayItem{ key: none, val: 'get_item_permissions_check' }]) }]) }, rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.editable() }, rt.ArrayItem{ key: 'callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller', ['Automattic_WooCommerce_Internal_RestApi_Routes_V4_AbstractController'], &this) }, rt.ArrayItem{ key: none, val: 'update_item' }]) }, rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller', ['Automattic_WooCommerce_Internal_RestApi_Routes_V4_AbstractController'], &this) }, rt.ArrayItem{ key: none, val: 'update_item_permissions_check' }]) }, rt.ArrayItem{ key: 'args', val: this.get_endpoint_args_for_item_schema(Class_WP_REST_Server.editable()) }]) }, rt.ArrayItem{ key: 'schema', val: rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller', ['Automattic_WooCommerce_Internal_RestApi_Routes_V4_AbstractController'], &this) }, rt.ArrayItem{ key: none, val: 'get_item_schema' }]) }])])
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) get_item_permissions_check(var_request rt.PhpVal) bool {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) update_item_permissions_check(var_request rt.PhpVal) bool {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	mut var_settings := this.get_all_settings()
	return this.prepare_item_for_response(var_settings.dup(), var_request.dup())
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) update_item(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_params.dup().is_array()))))) || !rt.is_true(var_params))) {
		return this.get_route_error_response(rt.new_string((this.get_error_prefix()).str() + 'invalid_param'), rt.call_function('__', [rt.new_string('Invalid or empty request body.'), rt.new_string('woocommerce')]), rt.new_int(400))
	}
	...
	return rt.new_bool(var_def.array_isset(rt.new_string('id')) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_type.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'title' }, rt.ArrayItem{ key: none, val: 'sectionend' }, rt.ArrayItem{ key: none, val: 'conflict_error' }, rt.ArrayItem{ key: none, val: 'add_settings_slot' }]), rt.new_bool(true)]))))))
	}
	...
	mut var_validated_settings := rt.new_array()
	{
		...
		for {
			...
			mut var_validation_result := rt.new_bool(this.validate_setting_value(var_setting_definition.dup(), var_sanitized_value.dup()))
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) validate_setting_value(var_setting rt.PhpVal, var_value rt.PhpVal) bool {
	mut var_value_mutated := var_value
	mut var_setting_id := if !(var_setting.array_get('id')).is_null() { var_setting.array_get('id') } else { rt.new_string('') }
	mut var_options := if !(var_setting.array_get('options')).is_null() { var_setting.array_get('options') } else { rt.new_array() }
	if !rt.is_true(var_options) {
		return true
	}
	mut var_allowed_values := rt.call_function('array_map', [rt.new_string('strval'), rt.func_array_keys(rt.cast_array(var_options))])
	mut var_check_values := if rt.is_true(rt.new_bool(var_value_mutated.dup().is_array())) { rt.call_function('array_map', [rt.new_string('strval'), var_value_mutated.dup()]) } else { rt.create_array([rt.ArrayItem{ key: none, val: // unsupported expression: Expr_Cast_String }]) }
	mut var_invalid_values := rt.call_function('array_diff', [var_check_values.dup(), var_allowed_values.dup()])
	if !(!rt.is_true(var_invalid_values)) {
		return (this.get_route_error_response(rt.new_string((this.get_error_prefix()).str() + 'invalid_param'), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Invalid value for "%1$s". Allowed values: %2$s.'), rt.new_string('woocommerce')]), var_setting_id.dup(), rt.call_function('implode', [rt.new_string(', '), var_allowed_values.dup()])]), rt.new_int(400))).to_bool()
	}
	return true
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) sanitize_setting_value(var_setting_type rt.PhpVal, var_value rt.PhpVal)  {
	...
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('radio'))) || rt.is_true(rt.equal(switch_val_1, rt.new_string('select'))) {
		return rt.call_function('sanitize_text_field', [var_value_mutated.dup()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) get_all_settings() rt.PhpVal {
	mut var_settings_instance := this.get_settings_tax_instance()
	mut var_settings := rt.call_method(var_settings_instance, 'get_settings_for_section', [rt.new_string('')])
	return var_settings.dup()
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) get_schema() rt.PhpVal {
	return rt.call_method(this.item_schema, 'get_item_schema', []rt.PhpVal{})
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) get_item_response(var_item rt.PhpVal, mut var_request Class_WP_REST_Request) rt.PhpVal {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_tax_controller() &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller {
	mut obj := &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller{
		PhpObjectBase: rt.PhpObjectBase{}
		rest_base: rt.new_string('settings/tax')
		settings_tax_instance: rt.new_null()
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema](if args.len > 0 { args[0] } else { rt.new_null() })
			this.init(mut dispatch_arg_0)
			return rt.new_null()
		}
		'get_settings_tax_instance' {
			return this.get_settings_tax_instance()
		}
		'register_routes' {
...
fn (this &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	match prop_name {
		'rest_base' { return this.rest_base }
		'settings_tax_instance' { return this.settings_tax_instance }
		'item_schema' { return this.item_schema }
		else { return this.PhpObjectBase.dispatch_get_prop(prop_name) }
	}
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Controller) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'rest_base' { this.rest_base = val; return true }
		'settings_tax_instance' { this.settings_tax_instance = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_internal_restapi_routes_v4_settings_tax_controller_php() {
