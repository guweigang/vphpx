...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_CodGatewaySettingsSchema) get_custom_groups_for_gateway(mut var_gateway Class_WC_Payment_Gateway) rt.PhpVal {
	...
	mut var_group := rt.create_array([rt.ArrayItem{ key: 'title', val: rt.call_function('__', [rt.new_string('Cash on delivery settings'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Manage how Cash on delivery appears at checkout and in order emails.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'order', val: 1 }, rt.ArrayItem{ key: 'fields', val: var_fields }])
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_CodGatewaySettingsSchema) load_shipping_method_options() rt.PhpVal {
	...
	mut var_zones := rt.new_array()
	{
		...
		for {
			...
			var_zones.array_push(create_wc_shipping_zone(var_raw_zone.dup()))
		}
	}
	...
	mut var_options := rt.new_array()
	{
		...
		for {
			...
			{
				mut iter_2 := var_zones.iterator()
				for {
					item_2 := iter_2.next() or { break }
					mut var_zone := item_2.val
	...
	this.shipping_method_options = var_options.dup()
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_CodGatewaySettingsSchema) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'shipping_method_options' { this.shipping_method_options = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_internal_restapi_routes_v4_settings_paymentgateways_schema_codgatewaysettingsschema_php() {
