...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductFeed_Integrations_IntegrationRegistry) register_integration(mut var_integration Class_Automattic_WooCommerce_Internal_ProductFeed_Integrations_IntegrationInterface) {
	this.integrations.array_set(var_integration.get_id(), var_integration.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductFeed_Integrations_IntegrationRegistry) get_integration(id string) rt.PhpVal {
	return if !(this.integrations.array_get(id)).is_null() {
		this.integrations.array_get(id)
	} else {
		rt.new_null()
	}
}
...
fn create_automattic_woocommerce_internal_productfeed_integrations_integrationregistry() &Class_Automattic_WooCommerce_Internal_ProductFeed_Integrations_IntegrationRegistry {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_productfeed_integrations_integrationregistry_php() {
