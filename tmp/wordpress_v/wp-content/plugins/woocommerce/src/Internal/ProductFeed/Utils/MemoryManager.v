...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductFeed_Utils_MemoryManager) get_available_memory() i64 {
	mut var_memory_limit := rt.call_function('wp_convert_hr_to_bytes', [rt.call_function('ini_get', [rt.new_string('memory_limit')])])
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductFeed_Utils_MemoryManager) flush_caches()  {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_wp_object_cache.dup().is_object()))))) {
		return rt.new_null()
	}
	...
	if rt.is_true(rt.call_function('method_exists', [var_wp_object_cache.dup(), rt.new_string('__remoteset')])) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductFeed_Utils_MemoryManager) collect_garbage()  {
	...
	// unsupported statement: Stmt_Static
	// unsupported statement: Stmt_Static
	mut var_gc_threshold_step := rt.new_int(rt.new_int(2500))
	mut var_gc_status := rt.call_function('gc_status', []rt.PhpVal{})
	if rt.is_true(rt.greater(var_gc_threshold, var_gc_status.array_get('threshold'))) {
		mut var_gc_threshold := var_gc_status.array_get('threshold')
	}
	if rt.is_true(rt.greater(var_gc_status.array_get('roots'), var_gc_threshold)) {
		...
		if rt.is_true(rt.less(var_collected, rt.new_int(100))) {
			if rt.is_true(rt.greater(var_gc_too_low_in_a_row, rt.new_int(0))) {
				mut var_gc_too_low_in_a_row := rt.new_int(rt.new_int(0))
				// unsupported expression: Expr_AssignOp_Plus
				var_gc_threshold = rt.call_function('min', [var_gc_threshold.dup(), rt.new_int(1000000000), var_gc_status.array_get('threshold')])
			} else {
				rt.pre_inc(var_gc_too_low_in_a_row)
			}
			mut var_gc_too_high_in_a_row := rt.new_int(rt.new_int(0))
		} else {
			if rt.is_true(rt.greater(var_gc_too_high_in_a_row, rt.new_int(0))) {
				var_gc_too_high_in_a_row = rt.new_int(rt.new_int(0))
				// unsupported expression: Expr_AssignOp_Minus
				var_gc_threshold = rt.call_function('max', [var_gc_threshold.dup(), rt.new_int(5000)])
			} else {
				rt.pre_inc(var_gc_too_high_in_a_row)
			}
			var_gc_too_low_in_a_row = rt.new_int(rt.new_int(0))
		}
	}
}

fn create_automattic_woocommerce_internal_productfeed_utils_memorymanager() &Class_Automattic_WooCommerce_Internal_ProductFeed_Utils_MemoryManager {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_productfeed_utils_memorymanager_php() {
