...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_StockSyncController) handle_product_stock_status_change(var_product_id rt.PhpVal, var_stock_status rt.PhpVal, var_product rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.eligibility_service,
		'is_stock_status_eligible', [var_stock_status.dup()])))))
	{
		return rt.new_null()
	}
	if rt.has_exception() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [
		var_product_mutated.dup(), rt.new_string('WC_Product')])))))
	{
		return rt.new_null()
	}
	if rt.has_exception() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.eligibility_service,
		'is_product_eligible', [var_product_mutated.dup()])))))
	{
		return rt.new_null()
	}
	if rt.has_exception() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.eligibility_service,
		'has_active_notifications', [var_product_mutated.dup()])))))
	{
		return rt.new_null()
	}
	if rt.has_exception() {
	...
	mut var_target_product_ids := rt.call_method(this.eligibility_service,
		'get_target_product_ids', [var_product_mutated.dup()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_StockSyncController) process_queue() {
	if rt.is_true(rt.new_bool(!rt.is_true(this.queue)
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.queue.is_array())))))))
	{
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_StockSyncController) output_admin_notice() {
	...
	rt.call_function('wp_admin_notice', [var_notice_message.dup(),
...
pub fn init_wp_content_plugins_woocommerce_src_internal_stocknotifications_stocksynccontroller_php() {
