...
pub fn init_wp_content_plugins_woocommerce_src_internal_stocknotifications_admin_templates_html_product_data_admin_php() {
	...
	rt.call_function('printf', [rt.new_string('(%s)'),
		rt.call_function('esc_html', [var_identifier.dup()])])
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_stock_availability.array_get('class')]))
	// unsupported statement: Stmt_InlineHTML
	if rt.is_true(rt.new_bool(!rt.is_true(var_stock_availability.array_get('availability'))
		&& rt.is_true(rt.identical(rt.new_string('in-stock'), var_stock_availability.array_get('class')))))
	{
		rt.echo_val(rt.call_function('esc_html__', [rt.new_string('In stock'),
			rt.new_string('woocommerce')]))
	} else {
		rt.echo_val(rt.call_function('esc_html', [var_stock_availability.array_get('availability')]))
	}
	// unsupported statement: Stmt_InlineHTML
}
