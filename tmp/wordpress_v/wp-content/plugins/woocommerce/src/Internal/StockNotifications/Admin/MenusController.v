...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController) init(mut var_notifications_page Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage)  {
	this.notifications_page = var_notifications_page.dup()
}

fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController) construct()  {
	rt.call_function('add_action', [rt.new_string('admin_menu'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_menu' }]), rt.new_int(10)])
	rt.call_function('add_filter', [rt.new_string('woocommerce_screen_ids'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_screen_ids' }])])
	rt.call_function('add_filter', [rt.new_string('set-screen-option'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'set_screen_option' }]), rt.new_int(10), rt.new_int(3)])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController) add_menu() bool {
	...
	mut var_dashboard_page := rt.call_function('add_submenu_page', [rt.new_string('woocommerce'), rt.call_function('__', [rt.new_string('Stock Notifications'), rt.new_string('woocommerce')]), rt.call_function('__', [rt.new_string('Notifications'), rt.new_string('woocommerce')]), rt.new_string('manage_woocommerce'), rt.new_string('wc-customer-stock-notifications'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'notifications_page' }])])
	rt.call_function('add_action', [rt.new_string("load-${var_dashboard_page.to_string()}"), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'add_screen_options' }])])
	return false
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController) add_screen_options()  {
	...
	rt.call_function('add_screen_option', [rt.new_string('per_page'), rt.create_array([rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Notifications per page'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'default', val: 10 }, rt.ArrayItem{ key: 'option', val: 'stock_notifications_per_page' }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController) notifications_page()  {
	mut var_action := if rt.get_superglobal('_GET').array_isset(rt.new_string('notification_action')) { rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('notification_action')])]) } else { rt.new_string('') }
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_action.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'create' }, rt.ArrayItem{ key: none, val: 'edit' }]), rt.new_bool(true)]))))) {
		var_action = rt.new_string(rt.new_string(''))
	}
...
fn Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController.add_screen_ids(var_screen_ids rt.PhpVal) rt.PhpVal {
	...
	return var_screen_ids_mutated.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage](if args.len > 0 { args[0] } else { rt.new_null() })
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'notifications_page' { this.notifications_page = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_internal_stocknotifications_admin_menuscontroller_php() {
