...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_JobManager) schedule_initial_job_for_product(product_id i64) bool {
	...
	if rt.is_true(rt.call_method(this.queue, 'get_next', [Class_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_JobManager.as_job_send_stock_notifications(), var_args.dup(), Class_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_JobManager.as_job_group()])) {
		return false
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_delay := // unsupported expression: Expr_Cast_Int
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	var_delay = rt.call_function('max', [rt.new_int(0), var_delay.dup()])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_action_id := rt.call_method(this.queue, 'schedule_single', [rt.add(rt.call_function('time', []rt.PhpVal{}), var_delay), Class_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_JobManager.as_job_send_stock_notifications(), var_args.dup(), Class_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_JobManager.as_job_group()])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	if rt.is_true(rt.new_bool(!(rt.is_true(var_action_id)))) {
		return false
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	rt.call_method(this.logger, 'info', [rt.call_function('sprintf', [rt.new_string('Scheduled stock notification for product %d'), rt.new_int(product_id)]), rt.create_array([rt.ArrayItem{ key: 'source', val: 'wc-customer-stock-notifications' }])])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	return true
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Exception') {
		...
		rt.call_method(this.logger, 'error', [rt.call_function('sprintf', [rt.new_string('Failed to schedule stock notification for product %d: %s'), rt.new_int(product_id), rt.call_method(var_e, 'getMessage', []rt.PhpVal{})]), rt.create_array([rt.ArrayItem{ key: 'source', val: 'wc-customer-stock-notifications' }])])
		return false
		unsafe { goto end_label_1 }
	}
	else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_JobManager) schedule_next_batch_for_product(product_id i64) bool {
	...
	if rt.is_true(rt.identical(rt.new_int(0), var_delay)) {
		mut var_action_id := rt.call_method(this.queue, 'add', [Class_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_JobManager.as_job_send_stock_notifications(), var_args.dup(), Class_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_JobManager.as_job_group()])
	} else {
		var_action_id = rt.call_method(this.queue, 'schedule_single', [rt.add(rt.call_function('time', []rt.PhpVal{}), var_delay), Class_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_JobManager.as_job_send_stock_notifications(), var_args.dup(), Class_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_Automattic_WooCommerce_Internal_StockNotifications_AsyncTasks_JobManager.as_job_group()])
	}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_stocknotifications_asynctasks_jobmanager_php() {
