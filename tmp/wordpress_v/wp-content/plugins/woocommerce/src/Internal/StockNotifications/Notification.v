...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) construct(read i64)  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.new_int(read).is_long() || rt.new_int(read).is_double())) && read > 0)) {
	...
	} else if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.new_int(read).is_array())) && !(!rt.is_true(rt.new_int(read).array_get('id'))))) {
		this.set_props(rt.new_int(read))
		this.set_object_read(rt.new_bool(true))
	} else {
		this.set_object_read(rt.new_bool(true))
	}
	this.dispatch_set_prop('data_store', fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_StockNotifications_WC_Data_Store{}; return temp.load(arg_0) }(rt.new_string('stock_notification')))
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) set_product_id(product_id i64)  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_a', [this.product, rt.new_string('WC_Product')])) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) set_status(status string)  {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string(status_mutated).dup(), fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_StockNotifications_Enums_NotificationStatus{}; return temp.get_valid_statuses() }(), rt.new_bool(true)]))))) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) set_date_created(var_date_created rt.PhpVal)  {
	this.set_date_prop(rt.new_string('date_created'), var_date_created.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) set_date_modified(var_date_modified rt.PhpVal)  {
	this.set_date_prop(rt.new_string('date_modified'), var_date_modified.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) set_date_confirmed(var_date_confirmed rt.PhpVal)  {
	this.set_date_prop(rt.new_string('date_confirmed'), var_date_confirmed.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) set_date_last_attempt(var_date_last_attempt rt.PhpVal)  {
	this.set_date_prop(rt.new_string('date_last_attempt'), var_date_last_attempt.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) set_date_notified(var_date_notified rt.PhpVal)  {
	this.set_date_prop(rt.new_string('date_notified'), var_date_notified.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) set_date_cancelled(var_date_cancelled rt.PhpVal)  {
	this.set_date_prop(rt.new_string('date_cancelled'), var_date_cancelled.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) set_cancellation_source(mut var_cancellation_source Class_Automattic_WooCommerce_Internal_StockNotifications_?string)  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_cancellation_source_mutated) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_cancellation_source_mutated.dup(), fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_StockNotifications_Enums_NotificationCancellationSource{}; return temp.get_valid_cancellation_sources() }(), rt.new_bool(true)]))))))) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) validate_props()  {
	...
	if rt.is_true(rt.new_bool(!(!rt.is_true(this.get_prop(rt.new_string('user_email')))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('filter_var', [this.get_prop(rt.new_string('user_email')), rt.get_constant('FILTER_VALIDATE_EMAIL')]))))))) {
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Internal_StockNotifications_WC_Data_Exception') {
		...
		return create_automattic_woocommerce_internal_stocknotifications_wp_error(rt.new_string('stock_notification_validation_error'), rt.call_method(var_e, 'getMessage', []rt.PhpVal{}))
		unsafe { goto end_label_1 }
	}
	else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) get_product_formatted_variation_list(flat bool) string {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_product)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_product, 'is_type', [rt.create_array([rt.ArrayItem{ key: none, val: 'variation' }])]))))))) {
		return ''
	}
	...
	mut var_attrs := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [var_key.dup(), rt.new_string('attribute_pa_')]))) {
				var_attrs.array_set(rt.call_function('str_replace', [rt.new_string('attribute_'), rt.new_string(''), var_key.dup()]), var_value.dup())
			} else {
				var_attrs.array_set(rt.call_function('wc_attribute_label', [rt.call_function('str_replace', [rt.new_string('attribute_'), rt.new_string(''), var_key.dup()]), var_product.dup()]), var_value.dup())
			}
		}
	}
	mut var_formatted_variation_list := rt.call_function('wc_get_formatted_variation', [var_attrs.dup(), rt.new_bool(flat), rt.new_bool(true), rt.new_bool(true)])
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) get_product_permalink() string {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_method(var_product, 'is_type', [rt.new_string('variation')])) && !(!rt.is_true(this.get_meta(rt.new_string('posted_attributes')))))) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) check_verification_key(key string) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_action_key.dup(), rt.new_string(':')]))))) {
		return false
	}
	// unsupported assign target: Expr_List
	mut var_threshold := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_StockNotifications_Config{}; return temp.get_verification_expiration_time_threshold() }()
	if rt.is_true(rt.greater(rt.sub(rt.call_function('time', []rt.PhpVal{}), // unsupported expression: Expr_Cast_Int), var_threshold)) {
		return false
	}
	return (fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_StockNotifications_Utilities_HasherHelper{}; return temp.wp_verify_fast_hash(arg_0, arg_1) }(rt.new_string(key_mutated), var_hash.dup())).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) get_verification_key(persist bool) string {
	...
	this.update_meta_data(rt.new_string('email_link_action_key'), rt.new_string((rt.call_function('time', []rt.PhpVal{})).str() + ':' + (fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_StockNotifications_Utilities_HasherHelper{}; return temp.wp_fast_hash(arg_0) }(var_key.dup())).str()))
	if var_persist {
		this.save()
	}
	return (var_key).str()
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) check_unsubscribe_key(key string) bool {
	mut key_mutated := key
	return (fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_StockNotifications_Utilities_HasherHelper{}; return temp.wp_verify_fast_hash(arg_0, arg_1) }(rt.new_string(key_mutated), this.get_meta(rt.new_string('email_link_action_key')))).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) get_unsubscribe_key(persist bool) string {
	mut var_key := rt.call_function('wp_generate_password', [rt.new_int(20), rt.new_bool(false)])
	mut var_hash := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_StockNotifications_Utilities_HasherHelper{}; return temp.wp_fast_hash(arg_0) }(var_key.dup())
	this.update_meta_data(rt.new_string('email_link_action_key'), var_hash.dup())
	if var_persist {
		this.save()
	}
	return (var_key).str()
...
fn create_automattic_woocommerce_internal_stocknotifications_wc_data() &Class_Automattic_WooCommerce_Internal_StockNotifications_WC_Data {
...
fn create_automattic_woocommerce_internal_stocknotifications_enums_notificationcancellationsource() &Class_Automattic_WooCommerce_Internal_StockNotifications_Enums_NotificationCancellationSource {
...
fn create_automattic_woocommerce_internal_stocknotifications_wp_error() &Class_Automattic_WooCommerce_Internal_StockNotifications_WP_Error {
...
fn create_automattic_woocommerce_internal_stocknotifications_utilities_hasherhelper() &Class_Automattic_WooCommerce_Internal_StockNotifications_Utilities_HasherHelper {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_stocknotifications_notification_php() {
