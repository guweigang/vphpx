...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage) output()  {
	mut var_table := rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_ListTable.class()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage) create()  {
	mut var_create_page := create_automattic_woocommerce_internal_stocknotifications_admin_notificationcreatepage()
	var_create_page.output()
	this.output_admin_notice()
}

fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage) edit()  {
	mut var_edit_page := create_automattic_woocommerce_internal_stocknotifications_admin_notificationeditpage()
...
fn Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage.add_notice(var_message rt.PhpVal, type string)  {
	...
	mut var_notice_data := rt.call_function('get_option', [Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage.admin_notice_option_name()])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string(type_mutated).dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'error' }, rt.ArrayItem{ key: none, val: 'warning' }, rt.ArrayItem{ key: none, val: 'success' }, rt.ArrayItem{ key: none, val: 'info' }]), rt.new_bool(true)]))))) {
		type_mutated = 'info'
	}
	var_notice_data = rt.create_array([rt.ArrayItem{ key: 'message', val: var_message }, rt.ArrayItem{ key: 'type', val: type_mutated }])
	rt.call_function('update_option', [Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage.admin_notice_option_name(), var_notice_data.dup()])
}

fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage) output_admin_notice()  {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('wp_admin_notice')]))))) {
		return rt.new_null()
	}
	mut var_notice_data := rt.call_function('get_option', [Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage.admin_notice_option_name()])
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(var_notice_data) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_notice_data.dup().is_array()))))))) || !rt.is_true(var_notice_data.array_get('message')))) {
		rt.call_function('delete_option', [Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage.admin_notice_option_name()])
		return rt.new_null()
	}
	mut var_type := if rt.is_true(rt.call_function('in_array', [var_notice_data.array_get('type'), rt.create_array([rt.ArrayItem{ key: none, val: 'error' }, rt.ArrayItem{ key: none, val: 'warning' }, rt.ArrayItem{ key: none, val: 'success' }, rt.ArrayItem{ key: none, val: 'info' }]), rt.new_bool(true)])) { var_notice_data.array_get('type') } else { rt.new_string('info') }
	rt.call_function('wp_admin_notice', [var_notice_data.array_get('message'), rt.create_array([rt.ArrayItem{ key: 'type', val: var_type }, rt.ArrayItem{ key: 'id', val: Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage.admin_notice_option_name() }, rt.ArrayItem{ key: 'dismissible', val: false }])])
	rt.call_function('delete_option', [Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage.admin_notice_option_name()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'add_notice' {
			...
			Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_NotificationsPage.add_notice(dispatch_arg_0, dispatch_arg_1)
			return rt.new_null()
		}
		'output_admin_notice' {
			this.output_admin_notice()
...
pub fn init_wp_content_plugins_woocommerce_src_internal_stocknotifications_admin_notificationspage_php() {
