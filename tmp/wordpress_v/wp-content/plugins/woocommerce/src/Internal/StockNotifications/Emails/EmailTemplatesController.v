...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Emails_EmailTemplatesController) email_product_image(var_product rt.PhpVal, var_notification rt.PhpVal, plain_text bool) {
	...
	mut var_image_src := if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(var_image.dup().is_array())) && var_image.array_isset(rt.new_int(0))))
	{
	...
	if rt.is_true(var_image_src) {
		...
		rt.echo_val(rt.call_function('esc_attr', [var_image_src.dup()]))
		// unsupported statement: Stmt_InlineHTML
	...
	rt.echo_val(rt.call_function('wp_kses_post', [var_html.dup()]))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Emails_EmailTemplatesController) email_product_attributes(var_product rt.PhpVal, var_notification rt.PhpVal, plain_text bool) {
	...
	var_formatted_variation_list = rt.call_function('strtr', [
		var_formatted_variation_list.dup(),
		rt.create_array([
	...
	rt.call_function('ob_start', []rt.PhpVal{})
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wp_kses_post', [var_formatted_variation_list.dup()]))
	// unsupported statement: Stmt_InlineHTML
	mut var_html := rt.call_function('ob_get_clean', []rt.PhpVal{})
	rt.echo_val(rt.call_function('wp_kses_post', [var_html.dup()]))
}
...
fn create_automattic_woocommerce_internal_stocknotifications_emails_emailtemplatescontroller() &Class_Automattic_WooCommerce_Internal_StockNotifications_Emails_EmailTemplatesController {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_stocknotifications_emails_emailtemplatescontroller_php() {
