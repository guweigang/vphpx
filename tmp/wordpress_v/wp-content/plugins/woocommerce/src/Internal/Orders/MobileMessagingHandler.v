...
fn Class_Automattic_WooCommerce_Internal_Orders_MobileMessagingHandler.prepare_mobile_message(mut var_order Class_WC_Order, mut var_blog_id Class_Automattic_WooCommerce_Internal_Orders_?int, mut var_now Class_DateTime, domain string) string {
	...
	mut var_used_app_in_last_month := rt.new_bool(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(rt.less_equal(rt.get_property(rt.call_method(var_last_mobile_used, 'diff', [var_now]), 'days'), Class_Automattic_WooCommerce_Internal_Orders_Automattic_WooCommerce_Internal_Orders_MobileMessagingHandler.open_order_interval_days()))))
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_has_jetpack := // unsupported expression: Expr_BinaryOp_NotIdentical
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	if rt.is_true(rt.new_bool(rt.is_true(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Orders_IppFunctions{}; return temp.is_store_in_person_payment_eligible() }()) && rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Orders_IppFunctions{}; return temp.is_order_in_person_payment_eligible(arg_0) }(rt.new_object('WC_Order', []string{}, var_order))))) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(var_used_app_in_last_month) && rt.is_true(var_has_jetpack))) {
...
fn Class_Automattic_WooCommerce_Internal_Orders_MobileMessagingHandler.get_closer_mobile_usage_date() rt.PhpVal {
	mut var_mobile_usage := fn () rt.PhpVal { mut temp := Class_WC_Tracker{}; return temp.get_woocommerce_mobile_usage() }()
	...
	return rt.call_function('max', [var_last_android_used.dup(), var_last_ios_used.dup()])
}
...
fn Class_Automattic_WooCommerce_Internal_Orders_MobileMessagingHandler.get_last_used_or_null(platform string, mut var_mobile_usage Class_Automattic_WooCommerce_Internal_Orders_array) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_mobile_usage_mutated.dup().array_isset(rt.new_string(platform)))) {
		return create_datetime(var_mobile_usage_mutated.array_get(platform).array_get('last_used'))
	} else {
		return rt.new_null()
	}
...
fn Class_Automattic_WooCommerce_Internal_Orders_MobileMessagingHandler.accept_payment_message(mut var_blog_id Class_Automattic_WooCommerce_Internal_Orders_?int, var_domain rt.PhpVal) string {
	...
	return (rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('%1$sCollect payments easily%2$s from your customers anywhere with our mobile app.'), rt.new_string('woocommerce')]), '<a href="' + (rt.call_function('esc_url', [var_deep_link_url.dup()])).str() + '">', rt.new_string('</a>')])).str()
}
...
fn Class_Automattic_WooCommerce_Internal_Orders_MobileMessagingHandler.manage_order_message(blog_id i64, order_id i64, domain string) string {
	...
	return (rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('%1$sManage the order%2$s with the app.'), rt.new_string('woocommerce')]), '<a href="' + (rt.call_function('esc_url', [var_deep_link_url.dup()])).str() + '">', rt.new_string('</a>')])).str()
}
...
fn Class_Automattic_WooCommerce_Internal_Orders_MobileMessagingHandler.no_app_message(mut var_blog_id Class_Automattic_WooCommerce_Internal_Orders_?int, domain string) string {
	...
	return (rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Process your orders on the go. %1$sGet the app%2$s.'), rt.new_string('woocommerce')]), '<a href="' + (rt.call_function('esc_url', [var_deep_link_url.dup()])).str() + '">', rt.new_string('</a>')])).str()
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_orders_mobilemessaginghandler_php() {
}
