...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_CouponsController) add_coupon_discount_via_ajax()  {
	...
	mut var_order := this.add_coupon_discount(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Orders_array](rt.get_superglobal('_POST')))
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_CouponsController) add_coupon_discount(mut var_post_variables Class_Automattic_WooCommerce_Internal_Orders_array) rt.PhpVal {
	mut var_order_id := if var_post_variables.array_isset(rt.new_string('order_id')) { rt.call_function('absint', [var_post_variables.array_get('order_id')]) } else { rt.new_int(0) }
	...
	mut var_coupon := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Utilities_ArrayUtil{}; return temp.get_value_or_default(arg_0, arg_1) }(rt.new_object('Automattic_WooCommerce_Internal_Orders_array', []string{}, var_post_variables), rt.new_string('coupon'))
	...
	mut var_user_id_arg := if var_post_variables.array_isset(rt.new_string('user_id')) { rt.call_function('absint', [var_post_variables.array_get('user_id')]) } else { rt.new_int(0) }
	mut var_user_email_arg := if var_post_variables.array_isset(rt.new_string('user_email')) { rt.call_function('sanitize_email', [rt.call_function('wp_unslash', [var_post_variables.array_get('user_email')])]) } else { rt.new_string('') }
	if rt.is_true(var_user_id_arg) {
		rt.call_method(var_order, 'set_customer_id', [var_user_id_arg.dup()])
	}
	if rt.is_true(var_user_email_arg) {
		rt.call_method(var_order, 'set_billing_email', [var_user_email_arg.dup()])
	}
	...
	mut var_code := rt.call_function('wc_format_coupon_code', [rt.call_function('wp_unslash', [var_coupon.dup()])])
	mut var_result := rt.call_method(var_order, 'apply_coupon', [var_code.dup()])
	if rt.is_true(rt.call_function('is_wp_error', [var_result.dup()])) {
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.call_function('html_entity_decode', [rt.call_function('wp_strip_all_tags', [rt.call_method(var_result, 'get_error_message', []rt.PhpVal{})])]))))
	}
	rt.call_method(var_order, 'add_order_note', [rt.call_function('esc_html', [rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Coupon applied: "%s".'), rt.new_string('woocommerce')]), var_code.dup()])]), rt.new_int(0), rt.new_bool(true), rt.create_array([rt.ArrayItem{ key: 'note_group', val: Class_Automattic_WooCommerce_Internal_Orders_OrderNoteGroup.order_update() }])])
...
fn create_automattic_woocommerce_internal_orders_couponscontroller() &Class_Automattic_WooCommerce_Internal_Orders_CouponsController {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_orders_couponscontroller_php() {
}
