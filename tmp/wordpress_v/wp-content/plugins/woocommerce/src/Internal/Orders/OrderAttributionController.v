...
pub mut:
		...
		is_stamp_html_called rt.PhpVal = rt.new_bool(false)
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) init(mut var_proxy Class_Automattic_WooCommerce_Proxies_LegacyProxy, mut var_controller Class_Automattic_WooCommerce_Internal_Features_FeaturesController, mut var_consent Class_Automattic_WooCommerce_Internal_Integrations_WPConsentAPI)  {
	...
	this.feature_controller = var_controller.dup()
	this.consent = var_consent.dup()
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) register()  {
	if rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.get_constant(arg_0) }(rt.new_string('WC_INSTALLING'))) {
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) on_init()  {
	...
	rt.call_function('add_action', [rt.new_string('wp_enqueue_scripts'), rt.new_closure(closure_1_fn)])
	...
	mut var_stamp_checkout_html_actions := rt.call_function('apply_filters', [rt.new_string('wc_order_attribution_stamp_checkout_html_actions'), rt.create_array([rt.ArrayItem{ key: none, val: 'woocommerce_checkout_billing' }, rt.ArrayItem{ key: none, val: 'woocommerce_after_checkout_billing_form' }, rt.ArrayItem{ key: none, val: 'woocommerce_checkout_shipping' }, rt.ArrayItem{ key: none, val: 'woocommerce_after_order_notes' }, rt.ArrayItem{ key: none, val: 'woocommerce_checkout_after_customer_details' }])])
	{
		...
		for {
			...
			rt.call_function('add_action', [var_action.dup(), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Orders_OrderAttributionController', ['RegisterHooksInterface'], &this) }, rt.ArrayItem{ key: none, val: 'stamp_html_element' }])])
		}
	}
	...
	mut var_order := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	if this.has_attribution(var_order.dup()) {
		return rt.new_null()
	}
	mut var_params := this.get_unprefixed_field_values(rt.get_superglobal('_POST').dup())
	rt.call_function('do_action', [rt.new_string('woocommerce_order_save_attribution_data'), var_order.dup(), var_params.dup()])
	return rt.new_null()
	}
	rt.call_function('add_action', [rt.new_string('woocommerce_checkout_order_created'), rt.new_closure(closure_3_fn)])
	closure_4_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_order := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	mut var_data := if args.len > 1 { args[1].dup() } else { rt.new_null() }
	mut var_source_data := this.get_source_values(var_data.dup())
	...
	rt.call_function('add_action', [rt.new_string('woocommerce_order_save_attribution_data'), rt.new_closure(closure_4_fn), rt.new_int(10), rt.new_int(2)])
	closure_5_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_customer_id := if args.len > 0 { args[0].dup() } else { rt.new_null() }
...
end_label_1:
	...
	rt.call_function('add_action', [rt.new_string('user_register'), rt.new_closure(closure_5_fn)])
	closure_6_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	this.register_order_origin_column()
	return rt.new_null()
	}
	rt.call_function('add_action', [rt.new_string('admin_init'), rt.new_closure(closure_6_fn)])
	closure_7_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_order_id := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	mut var_order := if args.len > 1 { args[1].dup() } else { rt.new_null() }
	this.maybe_set_admin_source(mut rt.cast_object_ptr[Class_WC_Order](var_order))
	return rt.new_null()
	}
	rt.call_function('add_action', [rt.new_string('woocommerce_new_order'), rt.new_closure(closure_7_fn), rt.new_int(2), rt.new_int(10)])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) maybe_set_admin_source(mut var_order Class_WC_Order)  {
	...
	mut var_http_referer := rt.call_function('esc_url_raw', [rt.call_function('wp_unslash', [if !(rt.get_superglobal('_SERVER').array_get('HTTP_REFERER')).is_null() { rt.get_superglobal('_SERVER').array_get('HTTP_REFERER') } else { rt.new_string('') }])])
	mut var_referer_is_admin := rt.identical(rt.new_int(0), rt.call_function('strpos', [var_http_referer.dup(), rt.call_function('get_admin_url', []rt.PhpVal{})]))
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_referer_is_admin)))) && rt.is_true(rt.call_function('wp_doing_ajax', []rt.PhpVal{})))) {
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) enqueue_scripts_and_styles()  {
	rt.call_function('wp_enqueue_script', [rt.new_string('sourcebuster-js'), rt.call_function('plugins_url', [rt.concat(rt.concat(rt.new_string('assets/js/sourcebuster/sourcebuster'), this.get_script_suffix()), rt.new_string('.js')), rt.get_constant('WC_PLUGIN_FILE')]), rt.new_array(), fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.get_constant(arg_0) }(rt.new_string('WC_VERSION')), rt.new_bool(true)])
	rt.call_function('wp_enqueue_script', [rt.new_string('wc-order-attribution'), rt.call_function('plugins_url', [rt.concat(rt.concat(rt.new_string('assets/js/frontend/order-attribution'), this.get_script_suffix()), rt.new_string('.js')), rt.get_constant('WC_PLUGIN_FILE')]), rt.create_array([rt.ArrayItem{ key: none, val: 'sourcebuster-js' }]), fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_Jetpack_Constants{}; return temp.get_constant(arg_0) }(rt.new_string('WC_VERSION')), rt.new_bool(true)])
	mut var_lifetime := // unsupported expression: Expr_Cast_Double
	mut var_session_length := // unsupported expression: Expr_Cast_Int
	...
	rt.call_function('wp_localize_script', [rt.new_string('wc-order-attribution'), rt.new_string('wc_order_attribution'), var_namespace.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) display_origin_column(var_order_id rt.PhpVal)  {
	mut var_order := this.get_hpos_order_object(var_order_id.dup())
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) output_origin_column(mut var_order Class_WC_Order)  {
	...
	mut var_origin := this.get_origin_label(var_source_type.dup(), var_source.dup())
	rt.echo_val(rt.call_function('esc_html', [var_origin.dup()]))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) stamp_html_element()  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_allow_multiple)))) && rt.is_true(// unsupported expression: Expr_StaticPropertyFetch))) {
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) set_customer_source_data(mut var_customer Class_WC_Customer)  {
	{
		mut iter_1 := this.get_source_values(this.get_unprefixed_field_values(rt.get_superglobal('_POST').dup())).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_value := item_1.val
			mut var_key := item_1.key
			var_customer.add_meta_data(this.get_meta_prefixed_field_name(var_key.dup()), var_value.dup())
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) set_order_source_data(mut var_source_data Class_Automattic_WooCommerce_Internal_Orders_array, mut var_order Class_WC_Order)  {
	...
	{
		mut iter_1 := var_source_data.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_value := item_1.val
			mut var_key := item_1.key
			rt.call_method(var_order_mutated, 'add_meta_data', [this.get_meta_prefixed_field_name(var_key.dup()), var_value.dup()])
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) send_order_tracks(mut var_source_data Class_Automattic_WooCommerce_Internal_Orders_array, mut var_order Class_WC_Order)  {
	mut var_order_mutated := var_order
	mut var_origin_label := this.get_origin_label(if !(var_source_data.array_get('source_type')).is_null() { var_source_data.array_get('source_type') } else { rt.new_string('') }, if !(var_source_data.array_get('utm_source')).is_null() { var_source_data.array_get('utm_source') } else { rt.new_string('') }, rt.new_bool(false))
	mut var_tracks_data := rt.create_array([rt.ArrayItem{ key: 'order_id', val: rt.call_method(var_order_mutated, 'get_id', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'source_type', val: if !(var_source_data.array_get('source_type')).is_null() { var_source_data.array_get('source_type') } else { rt.new_string('') } }, rt.ArrayItem{ key: 'medium', val: if !(var_source_data.array_get('utm_medium')).is_null() { var_source_data.array_get('utm_medium') } else { rt.new_string('') } }, rt.ArrayItem{ key: 'source', val: if !(var_source_data.array_get('utm_source')).is_null() { var_source_data.array_get('utm_source') } else { rt.new_string('') } }, rt.ArrayItem{ key: 'device_type', val: if !(.array_get()).is_null() { .array_get() } else { rt.new_string('unknown') }.to_string().to_lower() }, rt.ArrayItem{ key: 'origin_label', val: var_origin_label.dup().to_string().to_lower() }, rt.ArrayItem{ key: 'session_pages', val: if !(var_source_data.array_get('session_pages')).is_null() { var_source_data.array_get('session_pages') } else { rt.new_int(0) } }, rt.ArrayItem{ key: 'session_count', val: if !(var_source_data.array_get('session_count')).is_null() { var_source_data.array_get('session_count') } else { rt.new_int(0) } }, rt.ArrayItem{ key: 'order_total', val: rt.call_method(var_order_mutated, 'get_total', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'customer_registered', val: if rt.is_true(rt.call_method(var_order_mutated, 'get_customer_id', []rt.PhpVal{})) { 'yes' } else { 'no' } }])
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('wc_admin_record_tracks_event')])) {
		rt.call_function('wc_admin_record_tracks_event', [rt.new_string('order_attribution'), var_tracks_data.dup()])
	}
}

fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) get_order_screen_id() string {
	return (if rt.is_true(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Utilities_OrderUtil{}; return temp.custom_orders_table_usage_is_enabled() }()) { rt.call_function('wc_get_page_screen_id', [rt.new_string('shop-order')]) } else { rt.new_string('shop_order') }).str()
}
...
fn create_automattic_woocommerce_internal_orders_orderattributioncontroller() &Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController {
	mut obj := &Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController{
		...
		logger: rt.new_null()
		proxy: rt.new_null()
		is_stamp_html_called: rt.new_bool(false)
	}
	return obj
}
...
fn (this &Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	match prop_name {
		...
		'is_stamp_html_called' { return this.is_stamp_html_called }
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderAttributionController) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'is_stamp_html_called' { this.is_stamp_html_called = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_internal_orders_orderattributioncontroller_php() {
