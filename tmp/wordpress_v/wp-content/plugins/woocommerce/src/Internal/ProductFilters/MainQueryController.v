...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductFilters_MainQueryController) register()  {
	rt.call_function('add_filter', [rt.new_string('posts_clauses'), rt.create_array([rt.ArrayItem{ key: none, val: this.query_clauses }, rt.ArrayItem{ key: none, val: 'add_query_clauses_for_main_query' }]), rt.new_int(10), rt.new_int(2)])
	rt.call_function('add_filter', [rt.new_string('query_vars'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_ProductFilters_MainQueryController', ['RegisterHooksInterface'], &this) }, rt.ArrayItem{ key: none, val: 'add_query_vars' }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductFilters_MainQueryController) add_query_vars(mut var_query_vars Class_Automattic_WooCommerce_Internal_ProductFilters_array) rt.PhpVal {
	return rt.call_function('array_merge', [var_query_vars, rt.call_method(this.params, 'get_param_keys', []rt.PhpVal{})])
}

fn create_automattic_woocommerce_internal_productfilters_mainquerycontroller() &Class_Automattic_WooCommerce_Internal_ProductFilters_MainQueryController {
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductFilters_MainQueryController) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_ProductFilters_QueryClauses](if args.len > 0 { args[0] } else { rt.new_null() })
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_ProductFilters_Params](if args.len > 1 { args[1] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_src_internal_productfilters_mainquerycontroller_php() {
