...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Triggers_NewReviewNotificationTrigger) register()  {
	rt.call_function('add_action', [rt.new_string('comment_post'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_PushNotifications_Triggers_NewReviewNotificationTrigger', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'on_comment_post' }]), rt.new_int(10), rt.new_int(3)])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Triggers_NewReviewNotificationTrigger) on_comment_post(comment_id i64, var_comment_approved rt.PhpVal, mut var_commentdata Class_Automattic_WooCommerce_Internal_PushNotifications_Triggers_array)  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('spam'), var_comment_approved)) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return rt.new_null()
	}
	mut var_commented_on := rt.call_function('get_post_type', [// unsupported expression: Expr_Cast_Int])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		return rt.new_null()
	}
	rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_PushNotifications_Services_PendingNotificationStore.class()]), 'add', [create_automattic_woocommerce_internal_pushnotifications_notifications_newreviewnotification(rt.new_int(comment_id).dup())])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Triggers_NewReviewNotificationTrigger) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'on_comment_post' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_PushNotifications_Triggers_array](if args.len > 2 { args[2] } else { rt.new_null() })
...
pub fn init_wp_content_plugins_woocommerce_src_internal_pushnotifications_triggers_newreviewnotificationtrigger_php() {
