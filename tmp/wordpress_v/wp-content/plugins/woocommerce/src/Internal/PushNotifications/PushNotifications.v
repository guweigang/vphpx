...
pub fn Class_Automattic_WooCommerce_Internal_PushNotifications_PushNotifications.roles_with_push_notifications_enabled() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: none, val: 'administrator' }, rt.ArrayItem{ key: none, val: 'shop_manager' }])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_PushNotifications) register()  {
	rt.call_function('add_action', [rt.new_string('init'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_PushNotifications_PushNotifications', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'on_init' }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_PushNotifications) on_init()  {
	...
	rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_PushNotifications_Services_PendingNotificationStore.class()]), 'register', []rt.PhpVal{})
	rt.call_method(create_automattic_woocommerce_internal_pushnotifications_controllers_pushtokenrestcontroller(), 'register', []rt.PhpVal{})
	rt.call_method(create_automattic_woocommerce_internal_pushnotifications_controllers_pushnotificationrestcontroller(), 'register', []rt.PhpVal{})
	rt.call_method(create_automattic_woocommerce_internal_pushnotifications_triggers_newordernotificationtrigger(), 'register', []rt.PhpVal{})
	rt.call_method(create_automattic_woocommerce_internal_pushnotifications_triggers_newreviewnotificationtrigger(), 'register', []rt.PhpVal{})
	rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.class()]), 'register', []rt.PhpVal{})
}

fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_PushNotifications) register_post_types()  {
	rt.call_function('register_post_type', [Class_Automattic_WooCommerce_Internal_PushNotifications_Entities_PushToken.post_type(), rt.create_array([rt.ArrayItem{ key: 'labels', val: rt.create_array([rt.ArrayItem{ key: 'name', val: rt.call_function('__', [rt.new_string('Push Tokens'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'singular_name', val: rt.call_function('__', [rt.new_string('Push Token'), rt.new_string('woocommerce')]) }]) }, rt.ArrayItem{ key: 'public', val: false }, rt.ArrayItem{ key: 'publicly_queryable', val: false }, rt.ArrayItem{ key: 'show_ui', val: false }, rt.ArrayItem{ key: 'show_in_menu', val: false }, rt.ArrayItem{ key: 'query_var', val: false }, rt.ArrayItem{ key: 'rewrite', val: false }, rt.ArrayItem{ key: 'capability_type', val: 'post' }, rt.ArrayItem{ key: 'has_archive', val: false }, rt.ArrayItem{ key: 'hierarchical', val: false }, rt.ArrayItem{ key: 'supports', val: rt.create_array([rt.ArrayItem{ key: none, val: 'author' }]) }, rt.ArrayItem{ key: 'can_export', val: false }, rt.ArrayItem{ key: 'delete_with_user', val: true }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_PushNotifications) should_be_enabled() bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Utilities_FeaturesUtil{}; return temp.feature_is_enabled(arg_0) }(Class_Automattic_WooCommerce_Internal_PushNotifications_Automattic_WooCommerce_Internal_PushNotifications_PushNotifications.feature_name()))))) {
		this.enabled = false
		return this.enabled
	}
	...
	this.enabled = rt.is_true(rt.call_function('class_exists', [Class_Automattic_Jetpack_Connection_Manager.class()])) && rt.is_true(rt.call_method(rt.call_method(var_proxy, 'get_instance_of', [Class_Automattic_Jetpack_Connection_Manager.class()]), 'is_connected', []rt.PhpVal{}))
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Exception') {
		...
		if rt.is_true(rt.new_bool(rt.instance_of(var_logger, 'WC_Logger'))) {
			rt.call_method(var_logger, 'error', ['Error determining if PushNotifications feature should be enabled: ' + (rt.call_method(var_e, 'getMessage', []rt.PhpVal{})).str()])
		}
		this.enabled = false
		unsafe { goto end_label_1 }
	}
...
fn create_automattic_woocommerce_internal_pushnotifications_pushnotifications() &Class_Automattic_WooCommerce_Internal_PushNotifications_PushNotifications {
...
fn create_automattic_woocommerce_internal_pushnotifications_triggers_newordernotificationtrigger() &Class_Automattic_WooCommerce_Internal_PushNotifications_Triggers_NewOrderNotificationTrigger {
...
fn create_automattic_woocommerce_internal_pushnotifications_triggers_newreviewnotificationtrigger() &Class_Automattic_WooCommerce_Internal_PushNotifications_Triggers_NewReviewNotificationTrigger {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_pushnotifications_pushnotifications_php() {
