...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Services_PendingNotificationStore) add(mut var_notification Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification)  {
	if rt.is_true(rt.new_bool(!(rt.is_true(this.enabled)))) {
		return rt.new_null()
	}
	...
	this.pending.array_set(var_key, var_notification.dup())
	this.schedule_safety_net(mut var_notification)
	if rt.is_true(rt.new_bool(!(rt.is_true(this.shutdown_registered)))) {
		rt.call_function('add_action', [rt.new_string('shutdown'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_PushNotifications_Services_PendingNotificationStore', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'dispatch_all' }])])
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Services_PendingNotificationStore) schedule_safety_net(mut var_notification Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification)  {
	mut var_args := rt.create_array([rt.ArrayItem{ key: 'type', val: var_notification.get_type() }, rt.ArrayItem{ key: 'resource_id', val: var_notification.get_resource_id() }])
	if rt.is_true(rt.call_function('as_has_scheduled_action', [Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.safety_net_hook(), var_args.dup(), Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.action_scheduler_group()])) {
		return rt.new_null()
	}
	rt.call_function('as_schedule_single_action', [rt.add(rt.call_function('time', []rt.PhpVal{}), Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.safety_net_delay()), Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.safety_net_hook(), var_args.dup(), Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.action_scheduler_group()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Services_PendingNotificationStore) dispatch_all()  {
	...
	rt.call_method(this.dispatcher, 'dispatch', [rt.call_function('array_values', [this.pending])])
	this.enabled = false
...
fn create_automattic_woocommerce_internal_pushnotifications_services_pendingnotificationstore() &Class_Automattic_WooCommerce_Internal_PushNotifications_Services_PendingNotificationStore {
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Services_PendingNotificationStore) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_InternalNotificationDispatcher](if args.len > 0 { args[0] } else { rt.new_null() })
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Services_PendingNotificationStore) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'pending' { this.pending = val; return true }
		'shutdown_registered' { this.shutdown_registered = (val).to_bool(); return true }
...
pub fn init_wp_content_plugins_woocommerce_src_internal_pushnotifications_services_pendingnotificationstore_php() {
