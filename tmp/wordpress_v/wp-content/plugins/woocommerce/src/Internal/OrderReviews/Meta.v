...
fn Class_Automattic_WooCommerce_Internal_OrderReviews_Meta.parts_for_order(mut var_order Class_WC_Order) rt.PhpVal {
	...
	mut var_customer_name := rt.new_string(rt.new_string((var_order.get_billing_first_name()).str() + ' ' + (var_order.get_billing_last_name()).str().trim_space()))
	mut var_customer_email := var_order.get_billing_email()
	mut var_order_number := var_order.get_order_number()
	mut var_order_date_text := if rt.is_true(var_date_created) { rt.call_function('wc_format_datetime', [var_date_created.dup()]) } else { rt.new_string('') }
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		mut var_order_summary := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Order #%1$s (%2$s)'), rt.new_string('woocommerce')]), var_order_number.dup(), var_order_date_text.dup()])
	} else {
		var_order_summary = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Order #%s'), rt.new_string('woocommerce')]), var_order_number.dup()])
	}
	return rt.call_function('array_values', [rt.call_function('array_filter', [rt.create_array([rt.ArrayItem{ key: none, val: var_customer_name }, rt.ArrayItem{ key: none, val: var_customer_email }, rt.ArrayItem{ key: none, val: var_order_summary }])])])
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_orderreviews_meta_php() {
