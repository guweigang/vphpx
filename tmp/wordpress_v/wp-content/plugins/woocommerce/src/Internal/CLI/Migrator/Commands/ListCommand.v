...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_ListCommand) magic_invoke(mut var_args Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_array, mut var_assoc_args Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_array)  {
	...
	if !rt.is_true(var_platforms) {
		fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WP_CLI{}; return temp.line(arg_0) }(rt.new_string('No migration platforms are registered.'))
		return rt.new_null()
	}
	mut var_formatted_items := rt.new_array()
	mut var_platform_count := rt.new_int(rt.new_int(var_platforms.dup().array_count()))
	mut var_current_index := rt.new_int(rt.new_int(0))
	{
		...
		for {
			...
			var_formatted_items.array_push(rt.create_array([rt.ArrayItem{ key: 'id', val: var_id }, rt.ArrayItem{ key: 'name', val: if !(var_details.array_get('name')).is_null() { var_details.array_get('name') } else { rt.new_string('') } }, rt.ArrayItem{ key: 'fetcher', val: if !(var_details.array_get('fetcher')).is_null() { var_details.array_get('fetcher') } else { rt.new_string('') } }, rt.ArrayItem{ key: 'mapper', val: if !(var_details.array_get('mapper')).is_null() { var_details.array_get('mapper') } else { rt.new_string('') } }]))
	...
	rt.call_function('WP_CLI\Utils\format_items', [rt.new_string('table'), var_formatted_items.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'id' }, rt.ArrayItem{ key: none, val: 'name' }, rt.ArrayItem{ key: none, val: 'fetcher' }, rt.ArrayItem{ key: none, val: 'mapper' }])])
}
...
fn create_automattic_woocommerce_internal_cli_migrator_commands_listcommand() &Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_ListCommand {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_cli_migrator_commands_listcommand_php() {
