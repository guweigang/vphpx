		...
		var_name.dup()])))))
	{
		if var_function_map.array_isset(var_name) {
			...
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_Logging_Exception',
				...
				rt.call_function('esc_html', [var_name.dup()]),
			]))))
		}
	}
}
...
fn Class_Automattic_WooCommerce_Internal_Logging_SafeGlobalFunctionProxy.magic_callstatic(var_name rt.PhpVal, var_arguments rt.PhpVal) rt.PhpVal {
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			mut var_file := if args.len > 2 { args[2].dup() } else { rt.new_null() }
			mut var_line := if args.len > 3 { args[3].dup() } else { rt.new_null() }
			if rt.is_true(rt.identical(rt.new_string(@FILE), var_file)) {
				...
				var_file = if !(var_trace.array_get(2).array_get('file')).is_null() {
					var_trace.array_get(2).array_get('file')
				} else {
					var_file
				}
				var_line = if !(var_trace.array_get(2).array_get('line')).is_null() {
					var_trace.array_get(2).array_get('line')
				} else {
					var_line
				}
			}
			mut var_sanitized_message := rt.call_function('filter_var', [
				var_message.dup(), rt.get_constant('FILTER_SANITIZE_FULL_SPECIAL_CHARS')])
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_Logging_ErrorException',
				[]string{}, create_automattic_woocommerce_internal_logging_errorexception(var_sanitized_message.dup(),
				rt.new_int(0), var_type.dup(), var_file.dup(), var_line.dup())))
			return rt.new_null()
		}
		mut var_type := if args.len > 0 { args[0].dup() } else { rt.new_null() }
		mut var_message := if args.len > 1 { args[1].dup() } else { rt.new_null() }
	...
	Class_Automattic_WooCommerce_Internal_Logging_SafeGlobalFunctionProxy.maybe_load_missing_function(var_name.dup())
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	mut var_results := rt.call_function('call_user_func_array', [
		var_name.dup(), var_arguments.dup()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Internal_Logging_Throwable') {
		...
		Class_Automattic_WooCommerce_Internal_Logging_SafeGlobalFunctionProxy.log_wrapper_error(var_name.dup(), rt.call_method(var_e,
			'getMessage', []rt.PhpVal{}), var_arguments.dup())
...
fn Class_Automattic_WooCommerce_Internal_Logging_SafeGlobalFunctionProxy.log_wrapper_error(var_function_name rt.PhpVal, var_error_message rt.PhpVal, var_context rt.PhpVal) {
	...
	rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'error', [
		'[Wrapper function error] ' +(rt.call_function('sprintf', [rt.new_string('Error in %s: %s'), var_function_name.dup(), var_error_message.dup()])).str(),
...
fn create_automattic_woocommerce_internal_logging_errorexception() &Class_Automattic_WooCommerce_Internal_Logging_ErrorException {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_logging_safeglobalfunctionproxy_php() {
