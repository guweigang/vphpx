...
fn (mut this Class_Automattic_WooCommerce_Internal_Logging_OrderLogsCleanupHelper) get_max_age_in_seconds() i64 {
	return (rt.call_function('absint', [rt.call_function('apply_filters', [rt.new_string('woocommerce_cleanup_order_debug_logs_max_age'), rt.mul(rt.new_int(3), rt.get_constant('DAY_IN_SECONDS'))])])).to_i64()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Logging_OrderLogsCleanupHelper) cleanup_old_log_files(max_age i64)  {
	...
	mut var_file_controller := rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_Logging_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_FileController.class()])
	mut var_files := rt.call_method(var_file_controller, 'get_files', [rt.create_array([rt.ArrayItem{ key: 'source', val: 'place-order-debug' }, rt.ArrayItem{ key: 'date_filter', val: 'modified' }, rt.ArrayItem{ key: 'date_start', val: 1 }, rt.ArrayItem{ key: 'date_end', val: rt.sub(rt.call_function('time', []rt.PhpVal{}), rt.new_int(max_age_mutated)) }, rt.ArrayItem{ key: 'per_page', val: Class_Automattic_WooCommerce_Internal_Logging_Automattic_WooCommerce_Internal_Logging_OrderLogsCleanupHelper.max_files_per_run() }])])
	...
	if rt.is_true(rt.new_bool(rt.instance_of(var_logger, 'WC_Logger'))) {
		{
			...
			for {
				...
				rt.call_method(var_logger, 'clear', [var_source.dup()])
			}
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Logging_OrderLogsCleanupHelper) get_dangling_orders(max_age i64) rt.PhpVal {
	...
	mut var_cutoff_date := rt.call_function('gmdate', [rt.new_string('Y-m-d H:i:s'), rt.sub(rt.call_function('time', []rt.PhpVal{}), rt.new_int(max_age_mutated))])
	mut var_meta_table := if rt.is_true(this.hpos_in_use) { rt.concat(rt.get_property(var_wpdb, 'prefix'), rt.new_string('wc_orders_meta')) } else { rt.get_property(var_wpdb, 'postmeta') }
	mut var_order_table := if rt.is_true(this.hpos_in_use) { rt.concat(rt.get_property(var_wpdb, 'prefix'), rt.new_string('wc_orders')) } else { rt.get_property(var_wpdb, 'posts') }
	mut var_id_column := rt.new_string(if rt.is_true(this.hpos_in_use) { rt.new_string('order_id') } else { rt.new_string('post_id') })
	mut var_type_column := rt.new_string(if rt.is_true(this.hpos_in_use) { rt.new_string('type') } else { rt.new_string('post_type') })
	mut var_date_column := rt.new_string(if rt.is_true(this.hpos_in_use) { rt.new_string('date_created_gmt') } else { rt.new_string('post_date_gmt') })
	mut var_rows := rt.call_method(var_wpdb, 'get_results', [rt.call_method(var_wpdb, 'prepare', [rt.new_string("SELECT m.${var_id_column.to_string()} as order_id, m.meta_value\n\t\t\t\t FROM ${var_meta_table.to_string()} m\n\t\t\t\t INNER JOIN ${var_order_table.to_string()} o ON m.${var_id_column.to_string()} = o.id\n\t\t\t\t WHERE m.meta_key = %s\n\t\t\t\t AND o.${var_type_column.to_string()} = %s\n\t\t\t\t AND o.${var_date_column.to_string()} < %s\n\t\t\t\t LIMIT %d"), rt.new_string('_debug_log_source'), rt.new_string('shop_order'), var_cutoff_date.dup(), Class_Automattic_WooCommerce_Internal_Logging_Automattic_WooCommerce_Internal_Logging_OrderLogsCleanupHelper.max_orders_per_run()]), rt.get_constant('ARRAY_A')])
	return rt.call_function('array_column', [var_rows.dup(), rt.new_string('meta_value'), rt.new_string('order_id')])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Logging_OrderLogsCleanupHelper) delete_debug_log_meta_entries(mut var_order_ids Class_Automattic_WooCommerce_Internal_Logging_array)  {
	...
	// unsupported statement: Stmt_Global
	mut var_tables := rt.create_array([rt.ArrayItem{ key: none, val: rt.create_array([rt.ArrayItem{ key: 'table', val: if rt.is_true(this.hpos_in_use) { rt.concat(rt.get_property(var_wpdb, 'prefix'), rt.new_string('wc_orders_meta')) } else { rt.get_property(var_wpdb, 'postmeta') } }, rt.ArrayItem{ key: 'id_column', val: if rt.is_true(this.hpos_in_use) { 'order_id' } else { 'post_id' } }]) }])
	if rt.is_true(rt.call_method(this.data_synchronizer, 'data_sync_is_enabled', []rt.PhpVal{})) {
		var_tables.array_push(rt.create_array([rt.ArrayItem{ key: 'table', val: if rt.is_true(this.hpos_in_use) { rt.get_property(var_wpdb, 'postmeta') } else { rt.concat(rt.get_property(var_wpdb, 'prefix'), rt.new_string('wc_orders_meta')) } }, rt.ArrayItem{ key: 'id_column', val: if rt.is_true(this.hpos_in_use) { 'post_id' } else { 'order_id' } }]))
	}
	mut var_id_placeholders := rt.call_function('implode', [rt.new_string(','), rt.call_function('array_fill', [rt.new_int(0), rt.new_int(var_order_ids_mutated.dup().array_count()), rt.new_string('%d')])])
	{
		mut iter_1 := var_tables.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_table_config := item_1.val
			rt.call_method(var_wpdb, 'query', [rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('DELETE FROM '), var_table_config.array_get('table')), rt.new_string('\n\t\t\t\t\t WHERE ')), var_table_config.array_get('id_column')), rt.new_string(' IN (')), var_id_placeholders), rt.new_string(')\n\t\t\t\t\t AND meta_key IN (%s, %s)')), rt.call_function('array_merge', [var_order_ids_mutated.dup(), rt.create_array([rt.ArrayItem{ key: none, val: '_debug_log_source' }, rt.ArrayItem{ key: none, val: '_debug_log_source_pending_deletion' }])])])])
...
fn create_automattic_woocommerce_internal_logging_orderlogscleanuphelper() &Class_Automattic_WooCommerce_Internal_Logging_OrderLogsCleanupHelper {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_logging_orderlogscleanuphelper_php() {
