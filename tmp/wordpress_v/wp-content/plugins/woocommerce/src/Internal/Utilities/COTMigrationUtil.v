...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil) init(mut var_table_controller Class_Automattic_WooCommerce_Internal_DataStores_Orders_CustomOrdersTableController, mut var_data_synchronizer Class_Automattic_WooCommerce_Internal_Utilities_DataSynchronizer)  {
	this.table_controller = var_table_controller.dup()
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil) get_order_admin_screen() string {
	...
	return (if rt.is_true(rt.new_bool(this.custom_orders_table_usage_is_enabled() && rt.is_true(rt.call_function('function_exists', [rt.new_string('wc_get_page_screen_id')])))) { rt.call_function('wc_get_page_screen_id', [rt.new_string('shop-order')]) } else { rt.new_string('shop_order') }).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil) get_post_or_order_id(var_post_or_order_object rt.PhpVal) i64 {
	if rt.is_true(rt.new_bool(var_post_or_order_object.dup().is_long() || var_post_or_order_object.dup().is_double())) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil) is_order(var_order_id rt.PhpVal, mut var_types Class_Automattic_WooCommerce_Internal_Utilities_array) bool {
	...
	var_order_id_mutated = rt.new_int(this.get_post_or_order_id(var_order_id_mutated.dup()))
	mut var_order_data_store := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Utilities_WC_Data_Store{}; return temp.load(arg_0) }(rt.new_string('order'))
	return (rt.call_function('in_array', [rt.call_method(var_order_data_store, 'get_order_type', [var_order_id_mutated.dup()]), var_types, rt.new_bool(true)])).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil) get_order_type(var_order_id rt.PhpVal) rt.PhpVal {
	...
	var_order_id_mutated = rt.new_int(this.get_post_or_order_id(var_order_id_mutated.dup()))
	mut var_order_data_store := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Utilities_WC_Data_Store{}; return temp.load(arg_0) }(rt.new_string('order'))
	return rt.call_method(var_order_data_store, 'get_order_type', [var_order_id_mutated.dup()])
}

fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil) get_table_for_orders() rt.PhpVal {
	mut var_wpdb := rt.new_null()
	if this.custom_orders_table_usage_is_enabled() {
		mut var_table_name := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Utilities_OrdersTableDataStore{}; return temp.get_orders_table_name() }()
	} else {
		// unsupported statement: Stmt_Global
		var_table_name = rt.get_property(var_wpdb, 'posts')
	}
	return var_table_name.dup()
}

fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil) get_table_for_order_meta() rt.PhpVal {
	mut var_wpdb := rt.new_null()
	if this.custom_orders_table_usage_is_enabled() {
		mut var_table_name := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_Utilities_OrdersTableDataStore{}; return temp.get_meta_table_name() }()
	} else {
		// unsupported statement: Stmt_Global
...
fn create_automattic_woocommerce_internal_utilities_cotmigrationutil() &Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil {
...
fn create_automattic_woocommerce_internal_utilities_orderstabledatastore() &Class_Automattic_WooCommerce_Internal_Utilities_OrdersTableDataStore {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_utilities_cotmigrationutil_php() {
