...
fn Class_Automattic_WooCommerce_Internal_Utilities_ArrayUtil.array_is_list(mut var_arr Class_Automattic_WooCommerce_Internal_Utilities_array) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_array(), var_arr)) || rt.is_true(rt.identical(rt.call_function('array_values', [var_arr]), var_arr)))) {
		return true
	}
...
fn Class_Automattic_WooCommerce_Internal_Utilities_ArrayUtil.merge_by_key(mut var_arr1 Class_Automattic_WooCommerce_Internal_Utilities_array, mut var_arr2 Class_Automattic_WooCommerce_Internal_Utilities_array, key string) rt.PhpVal {
	...
	{
		mut iter_1 := var_arr1.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_item1 := item_1.val
			mut var_found := rt.new_bool(rt.new_bool(false))
			{
				mut iter_2 := var_arr2.iterator()
				for {
					item_2 := iter_2.next() or { break }
					mut var_item2 := item_2.val
					if rt.is_true(rt.identical(var_item1.array_get(key), var_item2.array_get(key))) {
						var_merged.array_push(rt.call_function('array_merge', [var_item1.dup(), var_item2.dup()]))
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(var_found)))) {
				var_merged.array_push(var_item1.dup())
			}
		}
	}
	{
		mut iter_1 := var_arr2.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_item2 := item_1.val
			mut var_found := rt.new_bool(rt.new_bool(false))
			{
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(var_found)))) {
				var_merged.array_push(var_item2.dup())
			}
		}
	}
	...
	rt.call_function('usort', [var_merged.dup(), rt.new_closure(closure_1_fn)])
	return rt.call_function('array_values', [var_merged.dup()])
}
	...
	mut var_filtered := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(var_value.dup().is_array())) {
				...
				var_filtered.array_set(var_key, var_value.dup())
			}
		}
	}
	return if rt.is_true(var_is_list) { rt.call_function('array_values', [var_filtered.dup()]) } else { var_filtered }
}
...
pub fn init_wp_content_plugins_woocommerce_src_internal_utilities_arrayutil_php() {
