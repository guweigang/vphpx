...
fn Class_Automattic_WooCommerce_Internal_Utilities_BlocksUtil.flatten_blocks(var_blocks rt.PhpVal) rt.PhpVal {
	...
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		var_carry.dup().array_push(rt.call_function('array_diff_key', [
			var_block.dup(),
		...
		if var_block.array_isset(rt.new_string('innerBlocks')) {
			mut var_inner_blocks :=
				Class_Automattic_WooCommerce_Internal_Utilities_BlocksUtil.flatten_blocks(var_block.array_get('innerBlocks'))
			return rt.call_function('array_merge', [var_carry.dup(),
				var_inner_blocks.dup()])
		}
		return var_carry.dup()
	}
	return rt.call_function('array_reduce', [var_blocks_mutated.dup(),
...
fn Class_Automattic_WooCommerce_Internal_Utilities_BlocksUtil.get_blocks_from_widget_area(var_block_name rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_blocks.dup().is_array())))))
		|| !rt.is_true(var_blocks)))
	{
		return rt.new_array()
	}
	closure_2_fn := fn [var_block_name] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_block := if args.len > 1 { args[1].dup() } else { rt.new_null() }
		mut var_parsed_blocks := if !(!rt.is_true(var_block.array_get('content'))) { rt.call_function('parse_blocks', [
				var_block.array_get('content'),
			]) } else { rt.new_array() }
		if rt.is_true(rt.new_bool(!(!rt.is_true(var_parsed_blocks))
			&& rt.is_true(rt.identical(var_block_name, var_parsed_blocks.array_get(0).array_get('blockName')))))
		{
			var_acc.dup().array_push(var_parsed_blocks.array_get(0))
		}
		return var_acc.dup()
	}
	return rt.call_function('array_reduce', [var_blocks.dup(),
...
fn Class_Automattic_WooCommerce_Internal_Utilities_BlocksUtil.get_block_from_template_part(var_block_name rt.PhpVal, var_template_part_slug rt.PhpVal) rt.PhpVal {
	...
	mut var_flatten_blocks :=
		Class_Automattic_WooCommerce_Internal_Utilities_BlocksUtil.flatten_blocks(var_blocks.dup())
	closure_4_fn := fn [var_block_name] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		closure_3_fn := fn [var_block_name] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			return rt.identical(var_block_name, var_block.array_get('blockName'))
		}
		mut var_block := if args.len > 0 { args[0].dup() } else { rt.new_null() }
		return rt.identical(var_block_name, var_block.array_get('blockName'))
	}
	return rt.call_function('array_values', [
		rt.call_function('array_filter', [var_flatten_blocks.dup(),
...
pub fn init_wp_content_plugins_woocommerce_src_internal_utilities_blocksutil_php() {
