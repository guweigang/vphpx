...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailApiController) init()  {
	this.post_manager = fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_EmailEditor_WCTransactionalEmails_WCTransactionalEmailPostsManager{}; return temp.get_instance() }()
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailApiController) get_email_data(var_post_data rt.PhpVal) rt.PhpVal {
	mut var_email_type := rt.call_method(this.post_manager, 'get_email_type_from_post_id', [var_post_data.array_get('id')])
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailApiController) save_email_data(mut var_data Class_Automattic_WooCommerce_Internal_EmailEditor_array, mut var_post Class_Automattic_WooCommerce_Internal_EmailEditor_WP_Post) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_data.array_isset(rt.new_string('subject'))))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_data.array_isset(rt.new_string('preheader'))))))))) {
		return rt.new_null()
	}
	...
	if rt.is_true(rt.identical(rt.new_string('customer_refunded_order'), var_email_type)) {
		if rt.is_true(rt.new_bool(var_data.array_isset(rt.new_string('subject_full')))) {
			rt.call_method(var_email, 'update_option', [rt.new_string('subject_full'), var_data.array_get('subject_full')])
		}
		if rt.is_true(rt.new_bool(var_data.array_isset(rt.new_string('subject_partial')))) {
			rt.call_method(var_email, 'update_option', [rt.new_string('subject_partial'), var_data.array_get('subject_partial')])
		}
	} else if rt.is_true(rt.new_bool(var_data.array_isset(rt.new_string('subject')))) {
		rt.call_method(var_email, 'update_option', [rt.new_string('subject'), var_data.array_get('subject')])
	}
	if rt.is_true(rt.new_bool(var_data.array_isset(rt.new_string('preheader')))) {
		rt.call_method(var_email, 'update_option', [rt.new_string('preheader'), var_data.array_get('preheader')])
	}
	if rt.is_true(rt.new_bool(var_data.array_isset(rt.new_string('enabled')))) {
		rt.call_method(var_email, 'update_option', [rt.new_string('enabled'), if rt.is_true(var_data.array_get('enabled')) { rt.new_string('yes') } else { rt.new_string('no') }])
	}
	if rt.is_true(rt.new_bool(var_data.array_isset(rt.new_string('recipient')))) {
		rt.call_method(var_email, 'update_option', [rt.new_string('recipient'), var_data.array_get('recipient')])
	}
	if rt.is_true(rt.new_bool(var_data.array_isset(rt.new_string('cc')))) {
		rt.call_method(var_email, 'update_option', [rt.new_string('cc'), var_data.array_get('cc')])
	}
	if rt.is_true(rt.new_bool(var_data.array_isset(rt.new_string('bcc')))) {
		rt.call_method(var_email, 'update_option', [rt.new_string('bcc'), var_data.array_get('bcc')])
	}
	return rt.new_null()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailApiController) validate_email_data(mut var_data Class_Automattic_WooCommerce_Internal_EmailEditor_array) rt.PhpVal {
	...
	mut var_invalid_recipients := this.filter_invalid_email_addresses(if !(var_data.array_get('recipient')).is_null() { var_data.array_get('recipient') } else { rt.new_string('') })
	if !(!rt.is_true(var_invalid_recipients)) {
		mut var_error_message := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('One or more Recipient email addresses are invalid: “%s”. Please enter valid email addresses separated by commas.'), rt.new_string('woocommerce')]), rt.call_function('implode', [rt.new_string(','), var_invalid_recipients.dup()])])
		rt.call_method(var_error, 'add', [rt.new_string('invalid_recipient_email_address'), var_error_message.dup()])
	}
	mut var_invalid_cc := this.filter_invalid_email_addresses(if !(var_data.array_get('cc')).is_null() { var_data.array_get('cc') } else { rt.new_string('') })
	if !(!rt.is_true(var_invalid_cc)) {
		var_error_message = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('One or more CC email addresses are invalid: “%s”. Please enter valid email addresses separated by commas.'), rt.new_string('woocommerce')]), rt.call_function('implode', [rt.new_string(','), var_invalid_cc.dup()])])
		rt.call_method(var_error, 'add', [rt.new_string('invalid_cc_email_address'), var_error_message.dup()])
	}
	mut var_invalid_bcc := this.filter_invalid_email_addresses(if !(var_data.array_get('bcc')).is_null() { var_data.array_get('bcc') } else { rt.new_string('') })
	if !(!rt.is_true(var_invalid_bcc)) {
		var_error_message = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('One or more BCC email addresses are invalid: “%s”. Please enter valid email addresses separated by commas.'), rt.new_string('woocommerce')]), rt.call_function('implode', [rt.new_string(','), var_invalid_bcc.dup()])])
		rt.call_method(var_error, 'add', [rt.new_string('invalid_bcc_email_address'), var_error_message.dup()])
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailApiController) filter_invalid_email_addresses(var_comma_separated_email_addresses rt.PhpVal) rt.PhpVal {
	...
	if var_comma_separated_email_addresses.dup().to_string().trim_space() == '' {
		return var_invalid_email_addresses.dup()
	}
	{
		mut iter_1 := rt.call_function('explode', [rt.new_string(','), var_comma_separated_email_addresses.dup()]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_email_address := item_1.val
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('filter_var', [rt.new_string(var_email_address.dup().to_string().trim_space()), rt.get_constant('FILTER_VALIDATE_EMAIL')]))))) {
				var_invalid_email_addresses.array_push(var_email_address.dup().to_string().trim_space())
			}
		}
	}
	return var_invalid_email_addresses.dup()
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailApiController) get_email_data_schema() rt.PhpVal {
	return rt.call_method(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.object(arg_0) }(rt.create_array([rt.ArrayItem{ key: 'subject', val: rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.string() }(), 'nullable', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'subject_full', val: rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.string() }(), 'nullable', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'subject_partial', val: rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.string() }(), 'nullable', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'preheader', val: rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.string() }(), 'nullable', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'default_subject', val: rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.string() }(), 'nullable', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'email_type', val: rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.string() }(), 'nullable', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'recipient', val: rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.string() }(), 'nullable', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'cc', val: rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.string() }(), 'nullable', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'bcc', val: rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_EmailEditor_Validator_Builder{}; return temp.string() }(), 'nullable', []rt.PhpVal{}) }])), 'to_array', []rt.PhpVal{})
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailApiController) get_email_by_type(mut var_id Class_Automattic_WooCommerce_Internal_EmailEditor_?string) rt.PhpVal {
	{
		mut iter_1 := this.get_emails().iterator()
		for {
			item_1 := iter_1.next() or { break }
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailApiController) get_default_content_response(mut var_request Class_WP_REST_Request) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.is_true(this.post_manager) && rt.is_true(this.posts_generator)))))) {
		return create_wp_error(rt.new_string('woocommerce_email_editor_not_initialized'), rt.call_function('__', [rt.new_string('Email editor is not initialized.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 500 }]))
	}
	mut var_post_id := // unsupported expression: Expr_Cast_Int
	mut var_email_type := rt.call_method(this.post_manager, 'get_email_type_from_post_id', [var_post_id.dup()])
	mut var_email := this.get_email_by_type(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_EmailEditor_?string](if !(var_email_type).is_null() { var_email_type } else { rt.new_string('') }))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_email)))) {
		return create_wp_error(rt.new_string('woocommerce_email_not_found'), rt.call_function('__', [rt.new_string('No email found for the given post ID.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	return create_wp_rest_response(rt.create_array([rt.ArrayItem{ key: 'content', val: rt.call_method(this.posts_generator, 'get_email_template', [var_email.dup()]) }]), rt.new_int(200))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailApiController) reset_response(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	mut var_canonical := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Internal_EmailEditor_WCTransactionalEmails_WCTransactionalEmailPostsGenerator{}; return temp.compute_canonical_post_content(arg_0) }(var_email.dup())
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_update_result.dup()])) {
		return create_wp_error(rt.new_string('woocommerce_email_reset_failed'), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Failed to reset email content: %s'), rt.new_string('woocommerce')]), rt.call_method(var_update_result, 'get_error_message', []rt.PhpVal{})]), rt.create_array([rt.ArrayItem{ key: 'status', val: 500 }]))
	}
	mut var_source_hash := rt.new_null()
	mut var_synced_at := rt.new_null()
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		var_source_hash = rt.new_string(rt.new_string(sha1.hexhash(var_canonical.dup().to_string())))
		var_synced_at = rt.call_function('gmdate', [rt.new_string('Y-m-d H:i:s')])
		rt.call_function('update_post_meta', [var_post_id.dup(), Class_Automattic_WooCommerce_Internal_EmailEditor_WCTransactionalEmails_WCEmailTemplateDivergenceDetector.version_meta_key(), // unsupported expression: Expr_Cast_String])
		rt.call_function('update_post_meta', [var_post_id.dup(), Class_Automattic_WooCommerce_Internal_EmailEditor_WCTransactionalEmails_WCEmailTemplateDivergenceDetector.source_hash_meta_key(), var_source_hash.dup()])
		rt.call_function('update_post_meta', [var_post_id.dup(), Class_Automattic_WooCommerce_Internal_EmailEditor_WCTransactionalEmails_WCEmailTemplateDivergenceDetector.last_synced_at_meta_key(), var_synced_at.dup()])
		rt.call_function('update_post_meta', [var_post_id.dup(), Class_Automattic_WooCommerce_Internal_EmailEditor_WCTransactionalEmails_WCEmailTemplateDivergenceDetector.status_meta_key(), Class_Automattic_WooCommerce_Internal_EmailEditor_WCTransactionalEmails_WCEmailTemplateDivergenceDetector.status_in_sync()])
	}
	return create_wp_rest_response(rt.create_array([rt.ArrayItem{ key: 'content', val: var_canonical }, rt.ArrayItem{ key: 'version', val: if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) { // unsupported expression: Expr_Cast_String } else { rt.new_null() } }, rt.ArrayItem{ key: 'source_hash', val: var_source_hash }, rt.ArrayItem{ key: 'synced_at', val: var_synced_at }, rt.ArrayItem{ key: 'status', val: if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) { Class_Automattic_WooCommerce_Internal_EmailEditor_WCTransactionalEmails_WCEmailTemplateDivergenceDetector.status_in_sync() } else { rt.new_null() } }]), rt.new_int(200))
}
...
fn create_automattic_woocommerce_internal_emaileditor_emailapicontroller() &Class_Automattic_WooCommerce_Internal_EmailEditor_EmailApiController {
...
pub fn init_wp_content_plugins_woocommerce_src_internal_emaileditor_emailapicontroller_php() {
