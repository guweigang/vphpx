...
fn (mut this Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_PostsToOrdersMigrationController) get_migrated_meta_keys() rt.PhpVal {
	mut var_migrated_meta_keys := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.call_function('method_exists', [var_migrator.dup(), rt.new_string('get_meta_column_config')])) {
				var_migrated_meta_keys = rt.call_function('array_merge', [var_migrated_meta_keys.dup(), rt.call_method(var_migrator, 'get_meta_column_config', []rt.PhpVal{})])
			}
		}
	}
	return rt.func_array_keys(var_migrated_meta_keys.dup())
}
...
fn (mut this Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_PostsToOrdersMigrationController) migrate_orders(mut var_order_post_ids Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_array)  {
	...
	mut var_data := rt.new_array()
	{
		...
		for {
			...
			if !(!rt.is_true(var_data.array_get(var_name).array_get('errors'))) {
				this.handle_migration_error(mut var_order_post_ids, mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_array](var_data.array_get(var_name).array_get('errors')), mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_?Exception](rt.new_null()), mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_?bool](rt.new_null()), (var_name).str())
				if rt.has_exception() { unsafe { goto catch_label_1 } }
				return rt.new_null()
			}
			if rt.has_exception() { unsafe { goto catch_label_1 } }
...
end_label_1:
	mut var_using_transactions := rt.new_bool(this.maybe_start_transaction())
	{
		...
		for {
			...
			mut var_errors := rt.call_function('array_unique', [var_results.array_get('errors')])
			mut var_exception := var_results.array_get('exception')
			if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_null(), var_exception)) && !rt.is_true(var_errors))) {
				continue
			}
...
fn (mut this Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_PostsToOrdersMigrationController) handle_migration_error(mut var_order_post_ids Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_array, mut var_errors Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_array, mut var_exception Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_?Exception, mut var_using_transactions Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_?bool, name string)  {
	...
	mut var_batch := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Utilities_ArrayUtil{}; return temp.to_ranges_string(arg_0) }(rt.new_object('Automattic_WooCommerce_Database_Migrations_CustomOrderTable_array', []string{}, var_order_post_ids))
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		mut var_exception_class := rt.call_function('get_class', [var_exception_mutated.dup()])
...
fn (mut this Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_PostsToOrdersMigrationController) maybe_clear_order_datastore_cache_for_ids(mut var_order_post_ids Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_array)  {
	if rt.is_true(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Utilities_OrderUtil{}; return temp.custom_orders_table_datastore_cache_enabled() }()) {
...
fn (mut this Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_PostsToOrdersMigrationController) maybe_start_transaction() bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_transaction_isolation_level.dup(), var_valid_transaction_isolation_levels.dup(), rt.new_bool(true)]))))) {
	...
	mut var_set_transaction_isolation_level_command := rt.new_string(rt.new_string("SET TRANSACTION ISOLATION LEVEL ${var_transaction_isolation_level.to_string()}"))
...
fn (mut this Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_PostsToOrdersMigrationController) db_query(query string, supress_errors bool) bool {
	...
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	if var_supress_errors {
		rt.call_method(var_wpdb, 'suppress_errors', [var_suppress.dup()])
		if rt.has_exception() { unsafe { goto catch_label_2 } }
	}
	if rt.has_exception() { unsafe { goto catch_label_2 } }
...
catch_label_2:
	...
	if rt.instance_of(var_e_2, 'Automattic_WooCommerce_Database_Migrations_CustomOrderTable_Exception') {
		...
		mut var_exception_class := rt.call_function('get_class', [var_exception.dup()])
...
fn (mut this Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_PostsToOrdersMigrationController) verify_migrated_orders(mut var_order_post_ids Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_array) rt.PhpVal {
	mut var_errors := rt.new_array()
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_migrator := item_1.val
			if rt.is_true(rt.call_function('method_exists', [var_migrator.dup(), rt.new_string('verify_migrated_data')])) {
...
fn create_automattic_woocommerce_database_migrations_customordertable_posttoorderaddresstablemigrator() &Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_PostToOrderAddressTableMigrator {
...
fn create_automattic_woocommerce_database_migrations_customordertable_posttoorderoptablemigrator() &Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_PostToOrderOpTableMigrator {
...
fn create_automattic_woocommerce_database_migrations_customordertable_postmetatoordermetamigrator() &Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_PostMetaToOrderMetaMigrator {
...
fn create_automattic_woocommerce_database_migrations_customordertable_exception() &Class_Automattic_WooCommerce_Database_Migrations_CustomOrderTable_Exception {
...
pub fn init_wp_content_plugins_woocommerce_src_database_migrations_customordertable_poststoordersmigrationcontroller_php() {
}
