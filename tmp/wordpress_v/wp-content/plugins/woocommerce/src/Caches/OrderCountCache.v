...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCache) get_saved_statuses_for_type(order_type string) rt.PhpVal {
	...
	mut var_statuses := rt.call_function('wp_cache_get', [this.get_saved_statuses_cache_key(order_type_mutated)])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_statuses.dup().is_array()))))) {
		var_statuses = rt.new_array()
	}
	return var_statuses.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCache) ensure_statuses_for_type(order_type string, mut var_order_statuses Class_Automattic_WooCommerce_Caches_array)  {
	...
	mut var_new_statuses := rt.call_function('array_diff', [var_order_statuses_mutated.dup(), var_existing.dup()])
	if !rt.is_true(var_new_statuses) {
		return rt.new_null()
	}
	mut var_merged := rt.call_function('array_unique', [rt.call_function('array_merge', [var_existing.dup(), var_new_statuses.dup()])])
	rt.call_function('wp_cache_set', [this.get_saved_statuses_cache_key(order_type_mutated), var_merged.dup(), rt.new_string(''), this.expiration])
}
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCache) is_cached(var_order_type rt.PhpVal, var_order_status rt.PhpVal) rt.PhpVal {
	mut var_order_type_mutated := var_order_type
	mut var_order_status_mutated := var_order_status
	mut var_cache_key := rt.new_string(this.get_cache_key(var_order_type_mutated.dup(), var_order_status_mutated.dup()))
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCache) set(var_order_type rt.PhpVal, var_order_status rt.PhpVal, value i64) bool {
	mut var_order_type_mutated := var_order_type
	mut var_order_status_mutated := var_order_status
	this.ensure_statuses_for_type((// unsupported expression: Expr_Cast_String).str(), mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Caches_array](rt.create_array([rt.ArrayItem{ key: none, val: // unsupported expression: Expr_Cast_String }])))
	mut var_cache_key := rt.new_string(this.get_cache_key(var_order_type_mutated.dup(), var_order_status_mutated.dup()))
	return (rt.call_function('wp_cache_set', [var_cache_key.dup(), rt.new_int(value), rt.new_string(''), this.expiration])).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCache) set_multiple(order_type string, mut var_counts Class_Automattic_WooCommerce_Caches_array) rt.PhpVal {
	...
	mut var_mapped_counts := rt.new_array()
	{
		...
		for {
			...
			var_mapped_counts.array_set(this.get_cache_key(rt.new_string(order_type_mutated), var_status.dup()), // unsupported expression: Expr_Cast_Int)
		}
	}
	return rt.call_function('wp_cache_set_multiple', [var_mapped_counts.dup(), rt.new_string(''), this.expiration])
}
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCache) get(var_order_type rt.PhpVal, var_order_statuses rt.PhpVal) rt.PhpVal {
	...
	mut var_order_statuses := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.new_string(this.get_cache_key(var_order_type_mutated.dup(), var_order_statuses_mutated.dup()))
	}
	mut var_order_statuses := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.new_string(this.get_cache_key(var_order_type_mutated.dup(), var_order_statuses_mutated.dup()))
	}
	mut var_cache_keys := rt.call_function('array_map', [rt.new_closure(closure_1_fn), var_order_statuses_mutated.dup()])
	mut var_cache_values := rt.call_function('wp_cache_get_multiple', [var_cache_keys.dup()])
	mut var_status_values := rt.new_array()
	{
		...
		for {
			...
			mut var_order_status := rt.call_function('str_replace', [this.get_cache_key(var_order_type_mutated.dup(), rt.new_string('')), rt.new_string(''), var_key.dup()])
			var_status_values.array_set(var_order_status, var_value.dup())
		}
	}
	return var_status_values.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCache) increment(var_order_type rt.PhpVal, var_order_status rt.PhpVal, offset i64) rt.PhpVal {
	...
	mut var_order_status_mutated := var_order_status
	mut var_cache_key := rt.new_string(this.get_cache_key(var_order_type_mutated.dup(), var_order_status_mutated.dup()))
	return rt.call_function('wp_cache_incr', [var_cache_key.dup(), rt.new_int(offset)])
}
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCache) decrement(var_order_type rt.PhpVal, var_order_status rt.PhpVal, offset i64) rt.PhpVal {
	...
	mut var_order_status_mutated := var_order_status
	mut var_cache_key := rt.new_string(this.get_cache_key(var_order_type_mutated.dup(), var_order_status_mutated.dup()))
	return rt.call_function('wp_cache_decr', [var_cache_key.dup(), rt.new_int(offset)])
}

fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCache) flush(order_type string, var_order_statuses rt.PhpVal)  {
	mut order_type_mutated := order_type
	mut var_order_statuses_mutated := var_order_statuses
	order_type_mutated = (// unsupported expression: Expr_Cast_String).str()
	mut var_flush_saved_statuses := rt.new_bool(rt.new_bool(false))
	if !rt.is_true(var_order_statuses_mutated) {
		var_order_statuses_mutated = this.get_saved_statuses_for_type(order_type_mutated)
		var_flush_saved_statuses = rt.new_bool(rt.new_bool(true))
	}
	...
	mut var_order_statuses := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.new_string(this.get_cache_key(rt.new_string(order_type_mutated), var_order_statuses_mutated.dup()))
	}
	mut var_order_statuses := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.new_string(this.get_cache_key(rt.new_string(order_type_mutated), var_order_statuses_mutated.dup()))
	}
	mut var_cache_keys := rt.call_function('array_map', [rt.new_closure(closure_3_fn), var_order_statuses_mutated.dup()])
	if rt.is_true(var_flush_saved_statuses) {
		var_cache_keys.array_push(this.get_saved_statuses_cache_key(order_type_mutated))
	}
	rt.call_function('wp_cache_delete_multiple', [var_cache_keys.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCache) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'is_cached' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return this.is_cached(dispatch_arg_0, dispatch_arg_1)
		}
		'set' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
...
pub fn init_wp_content_plugins_woocommerce_src_caches_ordercountcache_php() {
