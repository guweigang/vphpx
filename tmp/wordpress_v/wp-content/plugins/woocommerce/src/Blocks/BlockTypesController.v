...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypesController) get_registered_blocks_with_woocommerce_parent() rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('did_action', [rt.new_string('init')])) && !(!rt.is_true(this.registered_blocks_with_woocommerce_parents)))) {
		return this.registered_blocks_with_woocommerce_parents
	}
	mut var_registered_blocks := rt.call_method(fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_WP_Block_Type_Registry{}; return temp.get_instance() }(), 'get_all_registered', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_registered_blocks.dup().is_array()))))) {
		return rt.new_array()
	}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_property(var_block, 'parent').is_array()))))) {
		rt.set_property(var_block, 'parent', rt.create_array([rt.ArrayItem{ key: none, val: rt.get_property(var_block, 'parent') }]))
	}
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	mut var_parent_block_name := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.identical(rt.new_string('woocommerce'), rt.call_function('strtok', [var_parent_block_name.dup(), rt.new_string('/')]))
	}
	mut var_woocommerce_blocks := rt.call_function('array_filter', [rt.get_property(var_block, 'parent'), rt.new_closure(closure_2_fn)])
	return rt.new_bool(!(!rt.is_true(var_woocommerce_blocks)))
	}
	this.registered_blocks_with_woocommerce_parents = rt.call_function('array_filter', [var_registered_blocks.dup(), rt.new_closure(closure_2_fn)])
	return this.registered_blocks_with_woocommerce_parents
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypesController) register_block_metadata()  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('function_exists', [rt.new_string('wp_register_block_metadata_collection')])) && rt.is_true(rt.call_function('file_exists', [var_meta_file_path.dup()])))) {
		...
		rt.call_function('wp_register_block_metadata_collection', [(rt.get_constant('WC_ABSPATH')).str() + 'assets/client/blocks/', var_meta_file_path.dup()])
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypesController) register_block_categories(var_categories rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('array_merge', [var_categories.dup(), var_woocommerce_block_categories.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypesController) block_should_have_data_attributes(var_block_name rt.PhpVal) bool {
	...
	mut var_block_has_woo_parent := rt.call_function('in_array', [var_block_name.dup(), rt.func_array_keys(var_blocks_with_woo_parents.dup()), rt.new_bool(true)])
	mut var_in_allowed_namespace_list := rt.call_function('in_array', [var_block_namespace.dup(), var_allowed_namespaces.dup(), rt.new_bool(true)])
	mut var_in_allowed_block_list := rt.call_function('in_array', [var_block_name.dup(), var_allowed_blocks.dup(), rt.new_bool(true)])
	return rt.is_true(rt.new_bool(rt.is_true(var_block_has_woo_parent) || rt.is_true(var_in_allowed_block_list))) || rt.is_true(var_in_allowed_namespace_list)
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypesController) add_data_attributes(var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_content.dup().is_string()))))) || !(this.block_should_have_data_attributes(var_block.array_get('blockName'))))) {
		return var_content.dup()
	}
	mut var_attributes := rt.cast_array(var_block.array_get('attrs'))
	mut var_exclude_attributes := rt.create_array([rt.ArrayItem{ key: none, val: 'className' }, rt.ArrayItem{ key: none, val: 'align' }])
	mut var_processor := create_automattic_woocommerce_blocks_wp_html_tag_processor(var_content.dup())
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_bool(false), var_processor.next_tag())) || rt.is_true(var_processor.is_tag_closer()))) {
		return var_content.dup()
	}
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_key.dup().is_string()))))) || rt.is_true(rt.call_function('in_array', [var_key.dup(), var_exclude_attributes.dup(), rt.new_bool(true)])))) {
				continue
			}
			if rt.is_true(rt.new_bool(var_value.dup().is_bool())) {
				var_value = rt.new_string(if rt.is_true(var_value) { rt.new_string('true') } else { rt.new_string('false') })
			}
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_scalar', [var_value.dup()]))))) {
				var_value = rt.call_function('wp_json_encode', [var_value.dup()])
			}
			var_key = rt.new_string(rt.new_string(rt.call_function('preg_replace', [rt.new_string('/(?<!^|\\ )[A-Z]/'), rt.new_string('-$0'), var_key.dup()]).to_string().to_lower()))
			var_processor.set_attribute(rt.new_string("data-${var_key.to_string()}"), var_value.dup())
		}
	}
	var_processor.set_attribute(rt.new_string('data-block-name'), var_block.array_get('blockName'))
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypesController) redirect_to_field()  {
	if !rt.is_true(rt.get_superglobal('_GET').array_get('redirect_to')) {
		return rt.new_null()
	}
	print('<input type="hidden" name="redirect" value="' + (rt.call_function('esc_attr', [rt.call_function('esc_url_raw', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('redirect_to')])])])).str() + '" />')
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypesController) hide_legacy_widgets_with_block_equivalent(var_widget_types rt.PhpVal) rt.PhpVal {
	var_widget_types.dup().array_push(rt.new_string('woocommerce_product_search'))
	return var_widget_types.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypesController) get_block_types() rt.PhpVal {
	...
	var_block_types = rt.call_function('array_merge', [var_block_types.dup(), fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_BlockTypes_Cart{}; return temp.get_cart_block_types() }(), fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_BlockTypes_Checkout{}; return temp.get_checkout_block_types() }(), fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartContents{}; return temp.get_mini_cart_block_types() }()])
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('in_array', [var_pagenow.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'widgets.php' }, rt.ArrayItem{ key: none, val: 'themes.php' }, rt.ArrayItem{ key: none, val: 'customize.php' }]), rt.new_bool(true)])) && rt.is_true(rt.new_bool(!rt.is_true(rt.get_superglobal('_GET').array_get('page')) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))))) {
		var_block_types = rt.call_function('array_intersect', [var_block_types.dup(), this.get_widget_area_block_types()])
	}
	if rt.is_true(rt.call_function('in_array', [var_pagenow.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'post.php' }, rt.ArrayItem{ key: none, val: 'post-new.php' }]), rt.new_bool(true)])) {
		var_block_types = rt.call_function('array_diff', [var_block_types.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'Breadcrumbs' }, rt.ArrayItem{ key: none, val: 'CatalogSorting' }, rt.ArrayItem{ key: none, val: 'ClassicTemplate' }, rt.ArrayItem{ key: none, val: 'ProductResultsCount' }, rt.ArrayItem{ key: none, val: 'ProductReviews' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\Status' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\Summary' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\Totals' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\TotalsWrapper' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\Downloads' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\DownloadsWrapper' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\BillingAddress' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\ShippingAddress' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\BillingWrapper' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\ShippingWrapper' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\AdditionalInformation' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\AdditionalFieldsWrapper' }, rt.ArrayItem{ key: none, val: 'OrderConfirmation\\AdditionalFields' }])])
	}
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_get_block_types'), var_block_types.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypesController) enqueue_block_style_for_classic_themes(var_args rt.PhpVal, var_block_name rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_null(), var_should_enqueue_block_style_for_classic_themes)) {
		mut var_should_enqueue_block_style_for_classic_themes := rt.new_bool(rt.new_bool(!(rt.is_true(rt.new_bool(rt.is_true() || rt.is_true())))))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_should_enqueue_block_style_for_classic_themes)))) {
		rt.call_function('remove_filter', [rt.new_string('register_block_type_args'), rt.create_array([rt.ArrayItem{ key: none, val:  }, rt.ArrayItem{ key: none, val:  }]), rt.new_int(10)])
		return var_args_mutated.dup()
	}
	if rt.is_true(rt.new_bool(rt.is_true() || !rt.is_true() && !rt.is_true())) {
		return .dup()
	}
...
fn create_automattic_woocommerce_blocks_wp_block_type_registry() &Class_Automattic_WooCommerce_Blocks_WP_Block_Type_Registry {
...
fn create_automattic_woocommerce_blocks_wp_html_tag_processor() &Class_Automattic_WooCommerce_Blocks_WP_HTML_Tag_Processor {
...
fn create_automattic_woocommerce_blocks_blocktypes_cart() &Class_Automattic_WooCommerce_Blocks_BlockTypes_Cart {
...
fn create_automattic_woocommerce_blocks_blocktypes_minicartcontents() &Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartContents {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypescontroller_php() {
