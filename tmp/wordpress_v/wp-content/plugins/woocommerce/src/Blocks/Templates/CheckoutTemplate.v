...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_CheckoutTemplate) get_template_title() rt.PhpVal {
	return rt.call_function('_x', [rt.new_string('Page: Checkout'), rt.new_string('Template name'), rt.new_string('woocommerce')])
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_CheckoutTemplate) get_template_description() rt.PhpVal {
	return rt.call_function('__', [rt.new_string('The Checkout template guides users through the final steps of the purchase process. It enables users to enter shipping and billing information, select a payment method, and review order details.'), rt.new_string('woocommerce')])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_CheckoutTemplate) page_template_hierarchy(var_templates rt.PhpVal) rt.PhpVal {
	if this.is_active_template() {
		rt.call_function('array_unshift', [var_templates.dup(), Class_Automattic_WooCommerce_Blocks_Templates_Automattic_WooCommerce_Blocks_Templates_CheckoutTemplate.slug()])
		rt.call_function('array_unshift', [var_templates.dup(), rt.new_string('checkout')])
	}
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_templates_checkouttemplate_php() {
}
