...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_CashOnDelivery) construct(mut var_asset_api Class_Automattic_WooCommerce_Blocks_Assets_Api) {
	this.asset_api = var_asset_api.dup()
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_CashOnDelivery) initialize() {
	this.dispatch_set_prop('settings', rt.call_function('get_option', [
		rt.new_string('woocommerce_cod_settings'),
		rt.new_array(),
	]))
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_CashOnDelivery) is_active() rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_CashOnDelivery) get_enable_for_virtual() rt.PhpVal {
	return rt.call_function('filter_var', [
		this.get_setting(rt.new_string('enable_for_virtual'), rt.new_bool(false)),
		rt.get_constant('FILTER_VALIDATE_BOOLEAN'),
	])
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_CashOnDelivery) get_enable_for_methods() rt.PhpVal {
	mut var_enable_for_methods := this.get_setting(rt.new_string('enable_for_methods'),
		rt.new_array())
	if rt.is_true(rt.identical(rt.new_string(''), var_enable_for_methods)) {
		return rt.new_array()
	}
	return var_enable_for_methods.dup()
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_CashOnDelivery) get_payment_method_script_handles() rt.PhpVal {
	rt.call_method(this.asset_api, 'register_script', [
		rt.new_string('wc-payment-method-cod'),
		rt.new_string('assets/client/blocks/wc-payment-method-cod.js'),
	])
	return rt.create_array([rt.ArrayItem{ key: none, val: 'wc-payment-method-cod' }])
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_CashOnDelivery) get_payment_method_data() rt.PhpVal {
	return rt.create_array([
		...
		rt.ArrayItem{ key: 'enableForVirtual', val: this.get_enable_for_virtual() },
		rt.ArrayItem{ key: 'enableForShippingMethods', val: this.get_enable_for_methods() },
...
fn create_automattic_woocommerce_blocks_payments_integrations_cashondelivery(arg_0 rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_Payments_Integrations_CashOnDelivery {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_CashOnDelivery) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_enable_for_virtual' {
			return this.get_enable_for_virtual()
		}
		'get_enable_for_methods' {
			return this.get_enable_for_methods()
		}
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_payments_integrations_cashondelivery_php() {
}
