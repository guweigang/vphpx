...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_PaymentMethodRegistry) get_all_active_registered() rt.PhpVal {
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_payment_method := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	...
	mut var_payment_methods := this.get_all_active_registered()
	{
		...
		for {
			...
			var_script_handles = rt.call_function('array_merge', [
				var_script_handles.dup(), if rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) {
					rt.call_method(var_payment_method,
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_payments_paymentmethodregistry_php() {
}
