...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_BankTransfer) construct(mut var_asset_api Class_Automattic_WooCommerce_Blocks_Assets_Api) {
	this.asset_api = var_asset_api.dup()
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_BankTransfer) initialize() {
	this.dispatch_set_prop('settings', rt.call_function('get_option', [
		rt.new_string('woocommerce_bacs_settings'),
		rt.new_array(),
	]))
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_BankTransfer) is_active() rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_BankTransfer) get_payment_method_script_handles() rt.PhpVal {
	rt.call_method(this.asset_api, 'register_script', [
		rt.new_string('wc-payment-method-bacs'),
		rt.new_string('assets/client/blocks/wc-payment-method-bacs.js'),
	])
	return rt.create_array([rt.ArrayItem{ key: none, val: 'wc-payment-method-bacs' }])
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Integrations_BankTransfer) get_payment_method_data() rt.PhpVal {
...
fn create_automattic_woocommerce_blocks_payments_integrations_banktransfer(arg_0 rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_Payments_Integrations_BankTransfer {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_payments_integrations_banktransfer_php() {
}
