...
fn Class_Automattic_WooCommerce_Blocks_Assets.register_block_script(var_script_name rt.PhpVal, handle string, var_dependencies rt.PhpVal) {
	rt.call_function('_deprecated_function', [rt.new_string('register_block_script'),
		rt.new_string('4.5.0')])
	mut var_asset_api := rt.call_method(fn () rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Blocks_Package{}
		return temp.container()
	}(), 'get', [Class_Automattic_WooCommerce_Blocks_Assets_Api.class()])
	rt.call_method(var_asset_api, 'register_block_script', [var_script_name.dup(),
		rt.new_string(handle), var_dependencies.dup()])
}
...
fn create_automattic_woocommerce_blocks_assets() &Class_Automattic_WooCommerce_Blocks_Assets {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_assets_php() {
}
