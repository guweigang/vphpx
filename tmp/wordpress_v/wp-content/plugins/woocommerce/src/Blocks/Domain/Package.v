...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Package) construct(var_version rt.PhpVal, var_plugin_path rt.PhpVal, var_deprecated rt.PhpVal)  {
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		rt.call_function('wc_deprecated_argument', [rt.new_string('FeatureGating'), rt.new_string('9.6'), rt.new_string('FeatureGating class is deprecated, please use wp_get_environment_type() instead.')])
		this.feature_gating = create_automattic_woocommerce_blocks_domain_services_featuregating()
	}
	this.version = var_version.dup()
	this.path = var_plugin_path.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Package) get_version_stored_on_db() rt.PhpVal {
	return rt.call_function('get_option', [Class_Automattic_WooCommerce_Blocks_Options.wc_block_version(), rt.new_string('')])
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Package) set_version_stored_on_db()  {
	rt.call_function('update_option', [Class_Automattic_WooCommerce_Blocks_Options.wc_block_version(), this.get_version()])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Package) get_url(relative_url string) string {
	if rt.is_true(rt.new_bool(!(rt.is_true(this.plugin_dir_url)))) {
		this.plugin_dir_url = rt.call_function('plugin_dir_url', [(this.path).str() + '/index.php'])
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Package) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'plugin_dir_url' { this.plugin_dir_url = val; return true }
		'feature_gating' { this.feature_gating = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_domain_package_php() {
}
