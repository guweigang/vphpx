...
struct Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation {
	rt.PhpObjectBase
pub mut:
		meta_schema_json rt.PhpVal = rt.new_string('')
}
...
fn Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation.get_field_schema_with_context(var_field_id rt.PhpVal, var_field_schema rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	mut var_primary_key := rt.new_string(rt.new_string('checkout'))
	mut var_secondary_key := rt.new_string(rt.new_string('additional_fields'))
	mut switch_val_1 := var_context
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('billing_address'))) || rt.is_true(rt.equal(switch_val_1, rt.new_string('shipping_address'))) {
		var_primary_key = rt.new_string(rt.new_string('customer'))
		var_secondary_key = var_context
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('contact'))) {
		var_primary_key = rt.new_string(rt.new_string('customer'))
		var_secondary_key = rt.new_string(rt.new_string('additional_fields'))
	}
	return rt.create_array([rt.ArrayItem{ key: var_primary_key, val: rt.create_array([rt.ArrayItem{ key: 'properties', val: rt.create_array([rt.ArrayItem{ key: var_secondary_key, val: rt.create_array([rt.ArrayItem{ key: 'properties', val: rt.create_array([rt.ArrayItem{ key: var_field_id, val: var_field_schema }]) }]) }]) }]) }])
}
...
fn Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation.schema_is_unwrapped(var_schema rt.PhpVal) bool {
	return var_schema.array_isset(rt.new_string('cart')) || var_schema.array_isset(rt.new_string('checkout')) || var_schema.array_isset(rt.new_string('customer'))
}
...
fn Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation.validate_document_object(mut var_document_object Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_DocumentObject, var_rules rt.PhpVal) bool {
	...
	if rt.is_true(Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation.schema_is_unwrapped(var_rules_mutated.dup())) {
		var_rules_mutated = rt.create_array([rt.ArrayItem{ key: '$schema', val: 'http://json-schema.org/draft-07/schema#' }, rt.ArrayItem{ key: 'type', val: 'object' }, rt.ArrayItem{ key: 'properties', val: var_rules_mutated }])
	} else {
	...
	mut var_validator := create_automattic_woocommerce_blocks_domain_services_checkoutfieldsschema_validator()
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_result := var_validator.validate(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Helper{}; return temp.tojson(arg_0) }(var_document_object.get_data()), fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Helper{}; return temp.tojson(arg_0) }(var_rules_mutated.dup()))
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Exception') {
		mut var_e := var_e_1.dup()
		return (create_wp_error(rt.new_string('woocommerce_rest_checkout_validation_failed'), rt.call_function('__', [rt.new_string('Validation failed.'), rt.new_string('woocommerce')]))).to_bool()
		unsafe { goto end_label_1 }
	}
	else {
...
end_label_1:
	return (create_wp_error(rt.new_string('woocommerce_rest_checkout_invalid_field'), rt.call_function('__', [rt.new_string('Invalid field.'), rt.new_string('woocommerce')]))).to_bool()
}
...
fn Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation.has_field_schema(var_fields rt.PhpVal) rt.PhpVal {
	mut var_return := rt.new_bool(rt.new_bool(false))
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(!rt.is_true(var_field.array_get('validation'))) && rt.is_true(rt.new_bool(var_field.array_get('validation').is_array())))) || rt.is_true(rt.new_bool(!(!rt.is_true(var_field.array_get('required'))) && rt.is_true(rt.new_bool(var_field.array_get('required').is_array())))))) || rt.is_true(rt.new_bool(!(!rt.is_true(var_field.array_get('hidden'))) && rt.is_true(rt.new_bool(var_field.array_get('hidden').is_array())))))) {
...
fn Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation.is_valid_schema(var_rules rt.PhpVal) bool {
	mut var_rules_mutated := var_rules
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_rules_mutated.dup().is_array()))))) {
		return (create_wp_error(rt.new_string('woocommerce_rest_checkout_invalid_field_schema'), rt.new_string('Rules must be defined as an array.'))).to_bool()
	}
	if !rt.is_true(var_rules_mutated) {
		return true
	}
	if rt.is_true(Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation.schema_is_unwrapped(var_rules_mutated.dup())) {
		var_rules_mutated = rt.create_array([rt.ArrayItem{ key: 'type', val: 'object' }, rt.ArrayItem{ key: 'properties', val: var_rules_mutated }])
	}
	if !rt.is_true(// unsupported expression: Expr_StaticPropertyFetch) {
		// unsupported assign target: Expr_StaticPropertyFetch
	}
	mut var_validator := create_automattic_woocommerce_blocks_domain_services_checkoutfieldsschema_validator()
	mut var_result := var_validator.validate(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Helper{}; return temp.tojson(arg_0) }(rt.create_array([rt.ArrayItem{ key: '$schema', val: 'http://json-schema.org/draft-07/schema#' }, rt.ArrayItem{ key: 'type', val: 'object' }, rt.ArrayItem{ key: 'properties', val: rt.create_array([rt.ArrayItem{ key: 'test', val: var_rules_mutated }]) }, rt.ArrayItem{ key: 'required', val: rt.create_array([rt.ArrayItem{ key: none, val: 'test' }]) }])), // unsupported expression: Expr_StaticPropertyFetch)
	if rt.is_true(rt.call_method(var_result, 'hasError', []rt.PhpVal{})) {
		return (create_wp_error(rt.new_string('woocommerce_rest_checkout_invalid_field_schema'), rt.call_function('esc_html', [// unsupported expression: Expr_Cast_String]))).to_bool()
	}
	return true
}
...
fn create_automattic_woocommerce_blocks_domain_services_checkoutfieldsschema_validator() &Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validator {
...
fn create_automattic_woocommerce_blocks_domain_services_checkoutfieldsschema_helper() &Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Helper {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'get_field_schema_with_context' {
			...
			return Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation.get_field_schema_with_context(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2)
		}
		'schema_is_unwrapped' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
		...
		'validate_document_object' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_DocumentObject](if args.len > 0 { args[0] } else { rt.new_null() })
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return rt.new_bool(Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation.validate_document_object(mut dispatch_arg_0, dispatch_arg_1))
		}
		'has_field_schema' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
...
fn (this &Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	match prop_name {
		'meta_schema_json' { return this.meta_schema_json }
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFieldsSchema_Validation) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'meta_schema_json' { this.meta_schema_json = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_domain_services_checkoutfieldsschema_validation_php() {
