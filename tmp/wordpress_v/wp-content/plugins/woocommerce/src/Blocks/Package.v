...
fn Class_Automattic_WooCommerce_Blocks_Package.feature() rt.PhpVal {
	...
	return create_automattic_woocommerce_blocks_domain_services_featuregating()
}

fn Class_Automattic_WooCommerce_Blocks_Package.container(reset bool) rt.PhpVal {
	// unsupported statement: Stmt_Static
	if !(true) || var_reset {
		mut var_container := create_automattic_woocommerce_blocks_registry_container()
		closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_container := if args.len > 0 { args[0].dup() } else { rt.new_null() }
			mut var_version := rt.new_string(rt.new_string('11.8.0-dev'))
			return mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Blocks_Registry_Container](create_automattic_woocommerce_blocks_domain_package(var_version.dup(), rt.call_function('dirname', [
				rt.new_string(@DIR),
				rt.new_int(2),
			])))
		}
		...
		closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_container := if args.len > 0 { args[0].dup() } else { rt.new_null() }
			return mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Blocks_Registry_Container](create_automattic_woocommerce_blocks_domain_bootstrap(var_container.dup()))
		}
...
fn create_automattic_woocommerce_blocks_domain_services_featuregating() &Class_Automattic_WooCommerce_Blocks_Domain_Services_FeatureGating {
...
fn create_automattic_woocommerce_blocks_domain_package() &Class_Automattic_WooCommerce_Blocks_Domain_Package {
...
fn create_automattic_woocommerce_blocks_domain_bootstrap() &Class_Automattic_WooCommerce_Blocks_Domain_Bootstrap {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_package_php() {
}
