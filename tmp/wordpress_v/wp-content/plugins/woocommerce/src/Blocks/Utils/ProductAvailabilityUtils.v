...
fn Class_Automattic_WooCommerce_Blocks_Utils_ProductAvailabilityUtils.get_product_availability(var_product rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_product)))) {
		return var_product_availability.dup()
	}
	var_product_availability = rt.call_method(var_product, 'get_availability', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(var_product_availability.array_isset(rt.new_string('class'))
		&& rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('in-stock'), var_product_availability.array_get('class')))
		|| rt.is_true(rt.identical(rt.new_string('available-on-backorder'), var_product_availability.array_get('class')))))))
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_utils_productavailabilityutils_php() {
