...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Breadcrumbs) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	mut var_font_size_classes_and_styles := this.get_font_size_classes_and_styles(var_attributes.dup())
	var_classes_and_styles.array_set('classes', (var_classes_and_styles.array_get('classes')).str() + ' ' + (var_font_size_classes_and_styles.array_get('class')).str() + ' ')
	var_classes_and_styles.array_set('styles', (var_classes_and_styles.array_get('styles')).str() + ' ' + (var_font_size_classes_and_styles.array_get('style')).str() + ' ')
	return rt.call_function('sprintf', [rt.new_string('<div class="woocommerce wp-block-breadcrumbs wc-block-breadcrumbs %1$s" style="%2$s">%3$s</div>'), rt.call_function('esc_attr', [var_classes_and_styles.array_get('classes')]), rt.call_function('esc_attr', [var_classes_and_styles.array_get('styles')]), var_breadcrumb.dup()])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Breadcrumbs) get_font_size_classes_and_styles(var_attributes rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_font_size)))) && rt.is_true(rt.identical(rt.new_string(''), var_custom_font_size)))) {
		return rt.create_array([rt.ArrayItem{ key: 'class', val: rt.new_null() }, rt.ArrayItem{ key: 'style', val: rt.new_null() }])
	}
...
fn create_automattic_woocommerce_blocks_blocktypes_breadcrumbs() &Class_Automattic_WooCommerce_Blocks_BlockTypes_Breadcrumbs {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_breadcrumbs_php() {
}
