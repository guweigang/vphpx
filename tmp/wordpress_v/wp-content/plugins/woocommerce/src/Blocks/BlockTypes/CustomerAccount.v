...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount) modify_hooked_block_attributes(var_parsed_hooked_block rt.PhpVal, var_hooked_block_type rt.PhpVal, var_relative_position rt.PhpVal, var_parsed_anchor_block rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_customer_account_block_font_size.dup().is_string()))))) {
		...
		if rt.is_true(rt.new_bool(var_navigation_block_font_size.dup().is_string())) {
			var_parsed_hooked_block.array_get_mut('attrs').array_get_mut('style').array_get_mut('typography').array_set('fontSize',
				var_navigation_block_font_size.dup())
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount) should_unhook_block(var_hooked_blocks rt.PhpVal, var_position rt.PhpVal, var_anchor_block rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	...
	mut var_block_is_hooked := rt.call_function('in_array', [
		var_block_name.dup(), var_hooked_blocks.dup(), rt.new_bool(true)])
	if rt.is_true(var_block_is_hooked) {
		...
		if rt.is_true(rt.call_function('in_array', [var_active_theme.dup(),
			var_exclude_themes.dup(), rt.new_bool(true)]))
		{
			mut var_key := rt.call_function('array_search', [
				var_block_name.dup(), var_hooked_blocks.dup(),
				rt.new_bool(true)])
			var_hooked_blocks.array_unset(var_key)
		}
	}
	return var_hooked_blocks.dup()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	mut var_has_dropdown := rt.new_bool(rt.new_bool(
		rt.is_true(rt.new_bool(!(!rt.is_true(var_attributes.array_get('hasDropdownNavigation')))
	...
	mut var_aria_label := rt.new_string(if rt.is_true(rt.identical(Class_Automattic_WooCommerce_Blocks_BlockTypes_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount.icon_only(),
		var_attributes.array_get('displayStyle')))
	{
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_has_dropdown)))) {
		return this.render_link(var_attributes.dup(), var_classes_and_styles.dup(),
			var_account_link.dup(), var_aria_label.dup(), var_label_markup.dup())
	}
	return this.render_dropdown(var_attributes.dup(), var_classes_and_styles.dup(),
		var_aria_label.dup(), var_label_markup.dup())
}

fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount) render_link(var_attributes rt.PhpVal, var_classes_and_styles rt.PhpVal, var_account_link rt.PhpVal, var_aria_label rt.PhpVal, var_label_markup rt.PhpVal) rt.PhpVal {
	...
	rt.echo_val(rt.call_function('esc_attr', [var_classes_and_styles_mutated.array_get('classes')]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_classes_and_styles_mutated.array_get('styles')]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_url', [var_account_link_mutated.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(var_aria_label_mutated)
	// unsupported statement: Stmt_Nop
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wp_kses', [this.render_icon(var_attributes.dup()),
		var_allowed_svg.dup()]))
	// unsupported statement: Stmt_InlineHTML
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount) render_dropdown(var_attributes rt.PhpVal, var_classes_and_styles rt.PhpVal, var_aria_label rt.PhpVal, var_label_markup rt.PhpVal) rt.PhpVal {
	mut var_classes_and_styles_mutated := var_classes_and_styles
	mut var_aria_label_mutated := var_aria_label
	mut var_label_markup_mutated := var_label_markup
	mut var_allowed_svg := this.get_allowed_svg()
	...
	mut var_dropdown_html := this.render_dropdown_menu(var_menu_items.dup())
	rt.call_function('ob_start', []rt.PhpVal{})
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_classes_and_styles_mutated.array_get('classes')]))
	// unsupported statement: Stmt_InlineHTML
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount) render_dropdown_menu(var_menu_items rt.PhpVal) rt.PhpVal {
	...
	if var_menu_items_mutated.array_isset(rt.new_string('dashboard')) {
		var_sections.array_push(this.render_section(rt.create_array([
			rt.ArrayItem{ key: 'dashboard', val: var_menu_items_mutated.array_get('dashboard') },
		])))
	}
	mut var_nav_items := rt.call_function('array_diff_key', [
		var_menu_items_mutated.dup(),
	...
	if !(!rt.is_true(var_nav_items)) {
		var_sections.array_push(this.render_section(var_nav_items.dup()))
	}
	if var_menu_items_mutated.array_isset(rt.new_string('customer-logout')) {
		var_sections.array_push(this.render_section(rt.create_array([
			rt.ArrayItem{
				key: 'customer-logout'
				val: var_menu_items_mutated.array_get('customer-logout')
			},
		])))
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount) render_section(var_items rt.PhpVal) rt.PhpVal {
	mut var_output :=
		rt.new_string(rt.new_string('<div class="wc-block-customer-account__dropdown-section">'))
	{
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount) render_menu_item(var_endpoint rt.PhpVal, var_label rt.PhpVal) string {
	mut var_url := rt.call_function('wc_get_account_endpoint_url', [
		var_endpoint.dup()])
	return '<a href="' + (rt.call_function('esc_url', [var_url.dup()])).str() +
		'" class="wc-block-customer-account__dropdown-item">' +
		(rt.call_function('esc_html', [var_label.dup()])).str() + '</a>'
}
...
fn create_automattic_woocommerce_blocks_blocktypes_customeraccount() &Class_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CustomerAccount) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'render_link' {
			...
			dispatch_arg_3 := if args.len > 3 { args[3] } else { rt.new_null() }
			dispatch_arg_4 := if args.len > 4 { args[4] } else { rt.new_null() }
			return this.render_link(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2, dispatch_arg_3,
				dispatch_arg_4)
		}
		'render_dropdown' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_customeraccount_php() {
}
