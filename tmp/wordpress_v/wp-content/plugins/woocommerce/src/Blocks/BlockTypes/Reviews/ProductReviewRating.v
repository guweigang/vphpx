...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewRating) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_rating := rt.new_int(rt.new_int(rt.call_function('get_comment_meta', [
	...
	if rt.is_true(rt.less(rt.new_int(0), var_rating)) {
		...
		var_html = rt.call_function('sprintf', [
			...
			rt.call_function('wc_get_star_rating_html', [var_rating.dup()]),
		])
	}
...
fn create_automattic_woocommerce_blocks_blocktypes_reviews_productreviewrating() &Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewRating {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_reviews_productreviewrating_php() {
