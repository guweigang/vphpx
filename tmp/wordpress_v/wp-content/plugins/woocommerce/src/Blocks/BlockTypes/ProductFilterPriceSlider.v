...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilterPriceSlider) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	if rt.is_true(rt.new_bool(
		...
		|| !rt.is_true(rt.get_property(var_block, 'context').array_get('filterData').array_get('price'))))
	{
		return ''
	}
	mut var_price_data :=
		rt.get_property(var_block, 'context').array_get('filterData').array_get('price')
	mut var_min_price := var_price_data.array_get('minPrice')
	mut var_max_price := var_price_data.array_get('maxPrice')
	mut var_min_range := var_price_data.array_get('minRange')
	mut var_max_range := var_price_data.array_get('maxRange')
	...
	mut var_show_input_fields := if var_attributes.array_isset(rt.new_string('showInputFields')) {
		var_attributes.array_get('showInputFields')
	} else {
		rt.new_bool(false)
	}
	mut var_inline_input := if var_attributes.array_isset(rt.new_string('inlineInput')) {
		var_attributes.array_get('inlineInput')
	} else {
		rt.new_bool(false)
	}
	...
	mut var_content_class :=
		rt.new_string(rt.new_string('wc-block-product-filter-price-slider__content'))
	if rt.is_true(rt.new_bool(rt.is_true(var_inline_input) && rt.is_true(var_show_input_fields))) {
	...
	mut var___high := rt.div(rt.mul(rt.new_int(100), rt.sub(var_max_price, var_min_range)), rt.sub(var_max_range,
		var_min_range))
	mut var_range_style :=
		rt.new_string(rt.new_string('--low: ${var___low.to_string()}%; --high: ${var___high.to_string()}%'))
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_content_class.dup()]))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_min_range.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_max_range.dup()]))
	// unsupported statement: Stmt_InlineHTML
...
fn create_automattic_woocommerce_blocks_blocktypes_productfilterpriceslider() &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilterPriceSlider {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_productfilterpriceslider_php() {
