...
struct Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_ShippingAddress {
	rt.PhpObjectBase
pub mut:
	block_name rt.PhpVal = rt.new_string('order-confirmation-shipping-address')
}

fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_ShippingAddress) render_content(var_order rt.PhpVal, permission bool, var_attributes rt.PhpVal, content string) string {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!var_permission
	...
	mut var_address := rt.new_string('<address>' +
		(rt.call_function('wp_kses_post', [rt.call_method(var_order, 'get_formatted_shipping_address', []rt.PhpVal{})])).str() +
		'</address>')
	mut var_phone := rt.new_string(if rt.is_true(rt.call_method(var_order, 'get_shipping_phone',
		[]rt.PhpVal{}))
	{
		'<p class="woocommerce-customer-details--phone">' +
			(rt.call_function('esc_html', [rt.call_method(var_order, 'get_shipping_phone', []rt.PhpVal{})])).str() +
			'</p>'
	} else {
		rt.new_string('')
	})
	...
	mut var_custom := this.render_additional_fields(rt.call_method(var_controller,
		...
		rt.new_string('address'), rt.new_string('shipping'), rt.new_string('view')]))
	return var_address.str() + var_phone.str() + var_custom.str()
}

fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_ShippingAddress) enqueue_data(mut var_attributes Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_array) {
	...
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_ShippingAddress', [
...
fn create_automattic_woocommerce_blocks_blocktypes_orderconfirmation_shippingaddress() &Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_ShippingAddress {
	mut obj := &Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_ShippingAddress{
		PhpObjectBase: rt.PhpObjectBase{}
		block_name:    rt.new_string('order-confirmation-shipping-address')
	}
	return obj
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_ShippingAddress) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'render_content' {
...
fn (this &Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_ShippingAddress) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	match prop_name {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_ShippingAddress) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'block_name' {
			this.block_name = val
			return true
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_orderconfirmation_shippingaddress_php() {
}
