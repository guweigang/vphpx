...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewsTitle) get_reviews_title(var_attributes rt.PhpVal, var_product rt.PhpVal) rt.PhpVal {
	...
	mut var_show_product_title := rt.new_bool(rt.new_bool(
		!(!rt.is_true(var_attributes.array_get('showProductTitle')))
		&& rt.is_true(var_attributes.array_get('showProductTitle'))))
	mut var_show_reviews_count := rt.new_bool(rt.new_bool(
		!(!rt.is_true(var_attributes.array_get('showReviewsCount')))
		&& rt.is_true(var_attributes.array_get('showReviewsCount'))))
		...
		&& rt.is_true(var_show_product_title)))
	{
		return if rt.is_true(rt.identical(rt.new_int(1), var_reviews_count)) { rt.call_function('sprintf', [
				...
				rt.call_function('_n', [rt.new_string('%1$s review for %2$s'),
					rt.new_string('%1$s reviews for %2$s'), var_reviews_count.dup(),
					rt.new_string('woocommerce')]),
				rt.call_function('number_format_i18n', [var_reviews_count.dup()]),
				rt.call_method(var_product_mutated, 'get_title', []rt.PhpVal{}),
			]) }
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_show_reviews_count))))
		&& rt.is_true(var_show_product_title)))
	{
		return if rt.is_true(rt.identical(rt.new_int(1), var_reviews_count)) { rt.call_function('sprintf', [
		...
		&& rt.is_true(rt.new_bool(!(rt.is_true(var_show_product_title))))))
	{
		return if rt.is_true(rt.identical(rt.new_int(1), var_reviews_count)) { rt.call_function('__', [
				...
				rt.call_function('_n', [rt.new_string('%s review'),
					rt.new_string('%s reviews'), var_reviews_count.dup(),
					rt.new_string('woocommerce')]),
				rt.call_function('number_format_i18n', [var_reviews_count.dup()]),
			]) }
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewsTitle) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_align_class_name := rt.new_string(if !rt.is_true(var_attributes.array_get('textAlign')) {
		rt.new_string('')
	} else {
	...
	mut var_reviews_title := this.get_reviews_title(var_attributes.dup(), var_product.dup())
	return (rt.call_function('sprintf', [
		rt.new_string('<%1$s id="reviews" %2$s>%3$s</%1$s>'),
		var_tag_name.dup(),
		var_wrapper_attributes.dup(),
		var_reviews_title.dup(),
	])).str()
}
...
fn create_automattic_woocommerce_blocks_blocktypes_reviews_productreviewstitle() &Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewsTitle {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_reviews_productreviewstitle_php() {
