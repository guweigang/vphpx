...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CategoryDescription) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	if var_description.dup().to_string().trim_space() == '' {
		return ''
	}
	...
	return (rt.call_function('sprintf', [rt.new_string('<div %1$s>%2$s</div>'),
		...
		rt.call_function('wp_kses_post', [
			rt.call_function('wc_format_content', [var_description.dup()]),
		])])).str()
}
...
fn create_automattic_woocommerce_blocks_blocktypes_categorydescription() &Class_Automattic_WooCommerce_Blocks_BlockTypes_CategoryDescription {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_categorydescription_php() {
