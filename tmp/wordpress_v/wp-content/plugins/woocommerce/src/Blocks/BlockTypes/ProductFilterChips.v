...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilterChips) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_items := if !(rt.get_property(var_block, 'context').array_get('filterData').array_get('items')).is_null() {
		rt.get_property(var_block, 'context').array_get('filterData').array_get('items')
	} else {
		rt.new_array()
	}
	mut var_show_counts := if !(rt.get_property(var_block, 'context').array_get('filterData').array_get('showCounts')).is_null() {
		rt.get_property(var_block, 'context').array_get('filterData').array_get('showCounts')
	} else {
		rt.new_bool(false)
	}
	...
	// unsupported statement: Stmt_InlineHTML
	if !(!rt.is_true(rt.get_property(var_block, 'context').array_get('filterData').array_get('groupLabel'))) {
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_html',
			[rt.get_property(var_block, 'context').array_get('filterData').array_get('groupLabel')]))
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
	{
...
fn create_automattic_woocommerce_blocks_blocktypes_productfilterchips() &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilterChips {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_productfilterchips_php() {
