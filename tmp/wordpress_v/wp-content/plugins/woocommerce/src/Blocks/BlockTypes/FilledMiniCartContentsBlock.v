...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_FilledMiniCartContentsBlock) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	{
		return this.render_experimental_filled_mini_cart_contents(var_attributes.dup(),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_FilledMiniCartContentsBlock) render_experimental_filled_mini_cart_contents(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	mut var_consent :=
		rt.new_string(rt.new_string('I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of WooCommerce'))
	mut var_notices := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState{}
		return temp.get_cart_error_notices(arg_0)
	}(var_consent.dup())
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_dismiss_aria_label.dup()]))
	// unsupported statement: Stmt_InlineHTML
...
fn create_automattic_woocommerce_blocks_blocktypes_filledminicartcontentsblock() &Class_Automattic_WooCommerce_Blocks_BlockTypes_FilledMiniCartContentsBlock {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_filledminicartcontentsblock_php() {
}
