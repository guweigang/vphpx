...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_NoResults) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	var_content_mutated =
		rt.new_string(rt.new_string(var_content_mutated.dup().to_string().trim_space()))
	...
	mut var_updated_html_content :=
		this.modify_anchor_tag_urls(rt.new_string(var_content_mutated.dup().to_string().trim_space()))
	...
	return (rt.call_function('sprintf', [rt.new_string('<div %1$s>%2$s</div>'),
		var_wrapper_attributes.dup(), var_updated_html_content.dup()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_NoResults) modify_anchor_tag_urls(var_content rt.PhpVal) rt.PhpVal {
	mut var_content_mutated := var_content
	mut var_processor :=
		create_automattic_woocommerce_blocks_blocktypes_productcollection_wp_html_tag_processor(rt.new_string(var_content_mutated.dup().to_string().trim_space()))
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_NoResults) get_current_url_without_filters() rt.PhpVal {
	...
	mut var_http_host := if rt.get_superglobal('_SERVER').array_isset(rt.new_string('HTTP_HOST')) { rt.call_function('wp_unslash', [
			rt.get_superglobal('_SERVER').array_get('HTTP_HOST'),
		]) } else { rt.new_string('') }
	...
	var_http_host = rt.call_function('sanitize_text_field', [
		var_http_host.dup()])
	var_request_uri = rt.call_function('esc_url_raw', [var_request_uri.dup()])
	mut var_current_url := rt.new_string(var_protocol.str() + '://' + var_http_host.str() +
		var_request_uri.str())
	mut var_parsed_url := rt.call_function('wp_parse_url', [var_current_url.dup()])
	mut var_query_string := if var_parsed_url.array_isset(rt.new_string('query')) {
		var_parsed_url.array_get('query')
	} else {
		rt.new_string('')
	}
	rt.call_function('parse_str', [var_query_string.dup(), var_query_params.dup()])
			...
			mut var_key := item_1.key
			{
				...
				for {
					...
					if rt.is_true(rt.identical(rt.call_function('strpos', [
						var_key.dup(), var_param.dup()]), rt.new_int(0)))
					{
	...
	mut var_new_url := rt.new_string((var_parsed_url.array_get('scheme')).str() + '://' +
		(var_parsed_url.array_get('host')).str())
	// unsupported expression: Expr_AssignOp_Concat
	// unsupported expression: Expr_AssignOp_Concat
	return var_new_url.dup()
}
...
fn create_automattic_woocommerce_blocks_blocktypes_productcollection_noresults() &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_NoResults {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_productcollection_noresults_php() {
