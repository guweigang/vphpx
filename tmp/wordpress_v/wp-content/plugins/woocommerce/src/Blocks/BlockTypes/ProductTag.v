...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductTag) set_block_query_args(var_query_args rt.PhpVal) {
	...
	], &this), 'attributes').array_get('tags'))) {
		var_query_args.array_get_mut('tax_query').array_push(rt.create_array([
			...
			rt.ArrayItem{
				...
				val: if rt.is_true(rt.new_bool(
					...
					&& rt.is_true(rt.identical(rt.new_string('any'), rt.get_property(rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_ProductTag', ['Automattic_WooCommerce_Blocks_BlockTypes_AbstractProductGrid'], &this), 'attributes').array_get('tagOperator')))))
				{
					'IN'
				} else {
					'AND'
...
fn create_automattic_woocommerce_blocks_blocktypes_producttag() &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductTag {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_producttag_php() {
}
