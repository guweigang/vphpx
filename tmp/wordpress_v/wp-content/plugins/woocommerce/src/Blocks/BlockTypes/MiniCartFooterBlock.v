...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartFooterBlock) render_experimental_iapi_mini_cart_footer(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	mut var_formatted_subtotal := rt.new_string(rt.new_string(''))
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_subtotal_label.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_other_costs_label.dup()]))
	// unsupported statement: Stmt_InlineHTML
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartFooterBlock) get_totals_item_description() string {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_taxes_enabled)
		&& rt.is_true(var_shipping_enabled))) && rt.is_true(var_coupons_enabled)))
	{
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_shipping_enabled) && rt.is_true(var_taxes_enabled))) {
		return (rt.call_function('__', [
			rt.new_string('Shipping and taxes calculated at checkout.'),
			rt.new_string('woocommerce'),
		])).str()
	}
	if rt.is_true(rt.new_bool(rt.is_true(var_shipping_enabled) && rt.is_true(var_coupons_enabled))) {
		return (rt.call_function('__', [
			rt.new_string('Shipping and discounts calculated at checkout.'),
			rt.new_string('woocommerce'),
		])).str()
	}
	if rt.is_true(rt.new_bool(rt.is_true(var_taxes_enabled) && rt.is_true(var_coupons_enabled))) {
		return (rt.call_function('__', [
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartFooterBlock) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	{
		return this.render_experimental_iapi_mini_cart_footer(var_attributes.dup(),
...
fn create_automattic_woocommerce_blocks_blocktypes_minicartfooterblock() &Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartFooterBlock {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_minicartfooterblock_php() {
}
