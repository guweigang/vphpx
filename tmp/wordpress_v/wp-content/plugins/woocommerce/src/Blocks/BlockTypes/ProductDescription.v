...
pub mut:
		...
		seen_ids rt.PhpVal = rt.new_array()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductDescription) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_product_id := rt.get_property(var_block, 'context').array_get('postId')
	if // unsupported expression: Expr_StaticPropertyFetch.array_isset(var_product_id) {
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('WP_DEBUG')])) && rt.is_true(rt.get_constant('WP_DEBUG')))) && rt.is_true(rt.call_function('defined', [rt.new_string('WP_DEBUG_DISPLAY')])))) && rt.is_true(rt.get_constant('WP_DEBUG_DISPLAY')))) {
			return (rt.call_function('__', [rt.new_string('[product description rendering halted]'), rt.new_string('woocommerce')])).str()
		}
		return ''
	}
	// unsupported expression: Expr_StaticPropertyFetch.array_set(var_product_id, true)
	mut var_product := rt.call_function('wc_get_product', [var_product_id.dup()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_product)))) {
		// unsupported expression: Expr_StaticPropertyFetch.array_unset(var_product_id)
		return ''
	}
	mut var_description := rt.call_method(var_product, 'get_description', []rt.PhpVal{})
	var_description = rt.call_function('apply_filters', [rt.new_string('the_content'), rt.call_function('str_replace', [rt.new_string(']]>'), rt.new_string(']]&gt;'), var_description.dup()])])
	if !rt.is_true(var_description) {
		// unsupported expression: Expr_StaticPropertyFetch.array_unset(var_product_id)
		return ''
	}
	// unsupported expression: Expr_StaticPropertyFetch.array_unset(var_product_id)
	mut var_wrapper_attributes := rt.call_function('get_block_wrapper_attributes', [rt.create_array([rt.ArrayItem{ key: 'class', val: 'wc-block-product-description' }])])
	return (rt.call_function('sprintf', [rt.new_string('<div %1$s>%2$s</div>'), var_wrapper_attributes.dup(), var_description.dup()])).str()
}
...
fn create_automattic_woocommerce_blocks_blocktypes_productdescription() &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductDescription {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductDescription) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'seen_ids' { this.seen_ids = val; return true }
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_productdescription_php() {
