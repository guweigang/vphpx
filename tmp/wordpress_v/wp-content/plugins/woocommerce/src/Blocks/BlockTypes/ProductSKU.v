...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductSKU) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_is_descendant_of_product_collection :=
		rt.new_bool(rt.get_property(var_block, 'context').array_get('query').array_isset(rt.new_string('isProductCollectionBlock')))
	mut var_is_interactive := rt.new_bool(rt.new_bool(
	...
	mut var_prefix := if var_attributes.array_isset(rt.new_string('prefix')) { rt.call_function('wp_kses_post', [
			var_attributes.array_get('prefix'),
	...
	mut var_suffix := if var_attributes.array_isset(rt.new_string('suffix')) { rt.call_function('wp_kses_post', [
			var_attributes.array_get('suffix'),
		]) } else { rt.new_string('') }
	...
	mut var_interactive_attributes := rt.new_string(if rt.is_true(var_is_interactive) {
		rt.new_string('data-wp-interactive="woocommerce/products" data-wp-text="state.productInContext.sku"')
	} else {
		rt.new_string('')
	})
	return (rt.call_function('sprintf', [
		...
		var_prefix.dup(),
		var_interactive_attributes.dup(),
		var_product_sku.dup(),
		var_suffix.dup(),
	])).str()
}
...
fn create_automattic_woocommerce_blocks_blocktypes_productsku() &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductSKU {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_productsku_php() {
}
