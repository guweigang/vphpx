...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartProductsTableBlock) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_item_key := var_context.array_get('cartItem').array_get('key')
		{
			mut iter_1 := var_cart_state.array_get('cart').array_get('items').iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_item := item_1.val
				if rt.is_true(rt.identical(var_item.array_get('key'), var_item_key)) {
					return var_item.dup()
				}
			}
		}
	...
	mut var_line_item_discount_span :=
		rt.new_string(rt.new_string('<span data-wp-text="state.lineItemDiscount" class="wc-block-formatted-money-amount wc-block-components-formatted-money-amount"></span>'))
	mut var_line_item_save_badge := rt.call_function('sprintf', [
		var_save_format.dup(), var_line_item_discount_span.dup()])
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_screen_reader_text.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_head_product_label.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_head_details_label.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_head_total_label.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_available_on_backorder_label.dup()]))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_Nop
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wp_kses', [var_line_item_save_badge.dup(),
		rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartProductsTableBlock) render_product_details_markup(var_property rt.PhpVal) rt.PhpVal {
	...
	mut var_is_item_data := rt.identical(rt.new_string('item_data'),
		var_context.array_get('dataProperty'))
	rt.call_function('ob_start', []rt.PhpVal{})
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_Nop
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_property.dup()]))
	// unsupported statement: Stmt_InlineHTML
...
fn create_automattic_woocommerce_blocks_blocktypes_minicartproductstableblock() &Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartProductsTableBlock {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_minicartproductstableblock_php() {
}
