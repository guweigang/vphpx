...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductAverageRating) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_styles_and_classes := fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal {
	...
	mut var_wrapper_attributes := rt.call_function('get_block_wrapper_attributes', [
		rt.create_array([
				...
				rt.call_function('array_filter', [
					rt.create_array([
						...
						rt.ArrayItem{ key: none, val: rt.call_function('esc_attr', [
							if !(var_text_align_styles_and_classes.array_get('class')).is_null() {
			...
			rt.ArrayItem{ key: 'style', val: rt.call_function('esc_attr', [
				if !(var_styles_and_classes.array_get('styles')).is_null() {
	...
	return (rt.call_function('sprintf', [rt.new_string('<div %1$s>%2$s</div>'),
		var_wrapper_attributes.dup(), rt.call_method(var_product, 'get_average_rating',
			[]rt.PhpVal{})])).str()
}
...
fn create_automattic_woocommerce_blocks_blocktypes_productaveragerating() &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductAverageRating {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_productaveragerating_php() {
