...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ClassicTemplate) enqueue_block_assets()  {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('class_exists', [rt.new_string('WC_Frontend_Scripts')])) && rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})))) {
		mut var_frontend_scripts := create_wc_frontend_scripts()
		mut var_styles := fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_BlockTypes_{"nodeType":"Expr_Variable","line":72,"name":"frontend_scripts"}{}; return temp.get_styles() }()
		{
			...
			for {
				...
				rt.call_function('wp_enqueue_style', [var_handle.dup(), rt.call_function('set_url_scheme', [var_style.array_get('src')]), var_style.array_get('deps'), var_style.array_get('version'), var_style.array_get('media')])
			}
		}
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ClassicTemplate) enqueue_assets(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal)  {
	mut var_attributes_mutated := var_attributes
	this.Class_Automattic_WooCommerce_Blocks_BlockTypes_AbstractDynamicBlock.enqueue_assets(var_attributes_mutated.dup(), var_content.dup(), var_block.dup())
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ClassicTemplate) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('WC_Frontend_Scripts')])) {
		mut var_frontend_scripts := create_wc_frontend_scripts()
		fn () rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_BlockTypes_{"nodeType":"Expr_Variable","line":155,"name":"frontend_scripts"}{}; return temp.load_scripts() }()
	}
	if rt.is_true(rt.identical(Class_Automattic_WooCommerce_Blocks_Templates_OrderConfirmationTemplate.slug(), var_attributes_mutated.array_get('template'))) {
	...
	if rt.is_true(rt.call_function('in_array', [var_attributes_mutated.array_get('template'), var_archive_templates.dup(), rt.new_bool(true)])) {
		var_valid = rt.new_bool(rt.new_bool(true))
	}
	{
		mut iter_1 := var_archive_templates.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_template := item_1.val
			if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [var_attributes_mutated.array_get('template'), var_template.dup()]))) {
				var_valid = rt.new_bool(rt.new_bool(true))
			}
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ClassicTemplate) add_alignment_class_to_wrapper(content string, mut var_block Class_Automattic_WooCommerce_Blocks_BlockTypes_array) rt.PhpVal {
	...
	mut var_align_class_and_style := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_Utils_StyleAttributesUtils{}; return temp.get_align_class_and_style(arg_0) }(var_attributes.dup())
	if !(var_align_class_and_style.array_isset(rt.new_string('class'))) {
		return rt.new_string(content)
	}
	mut var_first_tag := rt.new_string(rt.new_string('<[^<>]+>'))
	mut var_matches := rt.new_array()
	rt.call_function('preg_match', [var_first_tag.dup(), rt.new_string(content), var_matches.dup()])
	if rt.is_true(rt.new_bool(var_matches.array_isset(rt.new_int(0)) && rt.is_true(rt.identical(rt.call_function('strpos', [var_matches.array_get(0), rt.new_string(' class=')]), rt.new_bool(false))))) {
		mut var_pattern_before_tag_closing := rt.new_string(rt.new_string('/.+?(?=>)/'))
		return rt.call_function('preg_replace', [var_pattern_before_tag_closing.dup(), '$0 class="' + (.array_get()).str() + '"', rt.new_string(content), rt.new_int(1)])
	}
	mut var_pattern_get_class := rt.new_string(rt.new_string('/(?<=class=\\"|\')[^"|\']+(?=\\"|\')/'))
	return rt.call_function('preg_replace', [var_pattern_get_class.dup(), '$0 ' + (.array_get()).str(), rt.new_string(content), rt.new_int(1)])
}
...
fn create_automattic_woocommerce_blocks_blocktypes_classictemplate() &Class_Automattic_WooCommerce_Blocks_BlockTypes_ClassicTemplate {
...
fn create_automattic_woocommerce_blocks_blocktypes_{"nodetype":"expr_variable","line":72,"name":"frontend_scripts"}() &Class_Automattic_WooCommerce_Blocks_BlockTypes_{"nodeType":"Expr_Variable","line":72,"name":"frontend_scripts"} {
...
fn create_automattic_woocommerce_blocks_blocktypes_{"nodetype":"expr_variable","line":155,"name":"frontend_scripts"}() &Class_Automattic_WooCommerce_Blocks_BlockTypes_{"nodeType":"Expr_Variable","line":155,"name":"frontend_scripts"} {
...
fn create_automattic_woocommerce_blocks_blocktypes_wp_query() &Class_Automattic_WooCommerce_Blocks_BlockTypes_WP_Query {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_classictemplate_php() {
}
