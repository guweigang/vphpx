...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartCartButtonBlock) render_experimental_iapi_markup(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	mut var_view_cart_text := if rt.is_true(var_attributes.array_get('cartButtonLabel')) {
		var_attributes.array_get('cartButtonLabel')
	} else {
	...
	mut var_classes := rt.call_function('implode', [rt.new_string(' '),
		rt.call_function('array_filter', [
			rt.create_array([
				...
				rt.ArrayItem{
					key: none
					val: if rt.is_true(rt.new_bool(
						!(var_attributes.array_isset(rt.new_string('className')))
						|| rt.is_true(rt.identical(rt.call_function('strpos', [var_attributes.array_get('className'), rt.new_string('is-style-')]), rt.new_bool(false)))))
					{
						'is-style-outline'
					} else {
						''
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_view_cart_text.dup()]))
	// unsupported statement: Stmt_InlineHTML
...
fn create_automattic_woocommerce_blocks_blocktypes_minicartcartbuttonblock() &Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartCartButtonBlock {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_minicartcartbuttonblock_php() {
}
