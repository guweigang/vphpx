...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewContent) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_show_pending_links := rt.new_bool(rt.new_bool(
	...
	if rt.is_true(rt.identical(rt.new_string('0'), rt.get_property(var_comment, 'comment_approved'))) {
		...
		var_moderation_note = rt.new_string('<p><em class="review-awaiting-moderation">' +
			(rt.call_function('esc_html', [var_moderation_note.dup()])).str() + '</em></p>')
...
fn create_automattic_woocommerce_blocks_blocktypes_reviews_productreviewcontent() &Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewContent {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_reviews_productreviewcontent_php() {
