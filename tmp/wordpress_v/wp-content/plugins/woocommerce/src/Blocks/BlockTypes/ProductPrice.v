...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductPrice) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_product) {
		mut var_styles_and_classes := fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_Automattic_WooCommerce_Blocks_Utils_StyleAttributesUtils{}; return temp.get_classes_and_styles_by_attributes(arg_0) }(var_attributes.dup())
		...
		mut var_is_interactive := rt.new_bool(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_is_descendant_of_product_collection)))) && rt.is_true(rt.new_bool(!(rt.is_true(var_is_descendant_of_grouped_product_selector)))))) && rt.is_true(rt.call_method(var_product, 'is_type', [Class_Automattic_WooCommerce_Enums_ProductType.variable()]))))
		if rt.is_true(var_is_interactive) {
			mut var_prices_vary := rt.new_bool(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical)))
			if rt.is_true(rt.new_bool(!(rt.is_true(var_prices_vary)))) {
				var_is_interactive = rt.new_bool(rt.new_bool(false))
			}
		}
		mut var_wrapper_attributes := rt.create_array([rt.ArrayItem{ key: 'style', val: if !(var_styles_and_classes.array_get('styles')).is_null() { var_styles_and_classes.array_get('styles') } else { rt.new_string('') } }, rt.ArrayItem{ key: 'class', val: if !(var_styles_and_classes.array_get('classes')).is_null() { var_styles_and_classes.array_get('classes') } else { rt.new_string('') } }])
		mut var_interactive_attributes := rt.new_string(rt.new_string(''))
		mut var_context_directive := rt.new_string(rt.new_string(''))
		if rt.is_true(var_is_interactive) {
			...
			var_context_directive = rt.call_function('wp_interactivity_data_wp_context', [rt.create_array([rt.ArrayItem{ key: 'productElementKey', val: 'price_html' }])])
			var_interactive_attributes = rt.new_string(rt.new_string('data-wp-watch="callbacks.updateValue" aria-live="polite" aria-atomic="true"'))
		}
		return rt.call_function('sprintf', [rt.new_string('<div %1$s %2$s><div class="wc-block-components-product-price wc-block-grid__product-price" %3$s>\n\t\t\t\t\t%4$s\n\t\t\t\t</div></div>'), rt.call_function('get_block_wrapper_attributes', [var_wrapper_attributes.dup()]), var_context_directive.dup(), var_interactive_attributes.dup(), rt.call_method(var_product, 'get_price_html', []rt.PhpVal{})])
	}
	return rt.new_null()
}
...
fn create_automattic_woocommerce_blocks_blocktypes_productprice() &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductPrice {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_productprice_php() {
}
