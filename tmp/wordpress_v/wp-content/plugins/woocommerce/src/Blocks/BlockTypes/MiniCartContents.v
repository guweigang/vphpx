...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartContents) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	{
		return (this.render_experimental_iapi_mini_cart_contents(var_attributes.dup(),
			var_content.dup(), var_block.dup())).str()
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_MiniCartContents) enqueue_assets(mut var_attributes Class_Automattic_WooCommerce_Blocks_BlockTypes_array, var_content rt.PhpVal, var_block rt.PhpVal) {
	...
	mut var_text_color := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Blocks_Utils_StyleAttributesUtils{}
		return temp.get_text_color_class_and_style(arg_0)
	}(rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_array', []string{}, var_attributes))
	mut var_bg_color := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Blocks_Utils_StyleAttributesUtils{}
		return temp.get_background_color_class_and_style(arg_0)
	}(rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_array', []string{}, var_attributes))
				...
				rt.ArrayItem{ key: none, val: rt.create_array([
					...
					rt.ArrayItem{
						...
						val: if rt.is_true(var_text_color) {
							var_text_color.array_get('value')
						} else {
							rt.new_bool(false)
						}
					},
	...
	mut var_parsed_style := rt.call_function('sprintf', [
		...
		rt.call_function('esc_html', [var_attributes.array_get('width')]),
	])
	{
		...
		for {
			...
			mut var_selector := if rt.is_true(rt.new_bool(var_style.array_get('selector').is_array())) { rt.call_function('implode', [
					rt.new_string(','),
					var_style.array_get('selector'),
				]) } else { var_style.array_get('selector') }
			closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
				mut var_property := if args.len > 0 { args[0].dup() } else { rt.new_null() }
				return var_property.array_get('value')
			}
			mut var_properties := rt.call_function('array_filter', [
				var_style.array_get('properties'),
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_minicartcontents_php() {
}
