...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Renderer) handle_rendering(var_block_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	return (this.enhance_product_collection_with_interactivity(var_block_content_mutated.dup(),
		var_block.dup())).str()
}

fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Renderer) should_prevent_render() bool {
	return rt.is_true(rt.new_bool(!(rt.is_true(this.render_state.array_get('has_results')))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(this.render_state.array_get('has_no_results_block')))))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Renderer) enhance_product_collection_with_interactivity(var_block_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	mut var_block_content_mutated := var_block_content
	mut var_is_product_collection_block := if !(var_block.array_get('attrs').array_get('query').array_get('isProductCollectionBlock')).is_null() {
		var_block.array_get('attrs').array_get('query').array_get('isProductCollectionBlock')
	} else {
		rt.new_bool(false)
	}
	if rt.is_true(var_is_product_collection_block) {
		...
		mut var_collection := if !(var_block.array_get('attrs').array_get('collection')).is_null() {
			var_block.array_get('attrs').array_get('collection')
		} else {
			rt.new_string('')
		}
		mut var_is_enhanced_pagination_enabled := rt.new_bool(rt.new_bool(!(rt.is_true(if !(var_block.array_get('attrs').array_get('forcePageReload')).is_null() {
			var_block.array_get('attrs').array_get('forcePageReload')
		} else {
			rt.new_bool(false)
		}))))
		...
		if rt.is_true(var_collection) {
			var_context.array_set('collection', var_collection.dup())
		}
		mut var_p := create_wp_html_tag_processor(var_block_content_mutated.dup())
		...
		])))
		{
			...
			if rt.is_true(rt.new_bool(rt.is_true(var_is_enhanced_pagination_enabled)
				&& !(this.parsed_block).is_null()))
			{
				var_p.set_attribute(rt.new_string('data-wp-router-region'), rt.new_string(
					'wc-product-collection-' +
					(this.parsed_block.array_get('attrs').array_get('queryId')).str()))
			}
		}
		...
		var_block_content_mutated = this.add_store_notices_fallback(var_block_content_mutated.dup())
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Renderer) add_store_notices_fallback(var_block_content rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('preg_replace', [rt.new_string('/(<div[^>]+>)/'),
		'$1' + (this.render_interactivity_notices_region()).str(), var_block_content_mutated.dup(),
		rt.new_int(1)])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Renderer) set_fixed_width_style(var_p rt.PhpVal, var_fixed_width rt.PhpVal) {
	...
	var_p_mutated.set_attribute(rt.new_string('style'), this.get_list_styles(var_fixed_width.dup()))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Renderer) handle_block_dimensions(var_p rt.PhpVal, var_block rt.PhpVal) {
	mut var_p_mutated := var_p
	if var_block.array_get('attrs').array_isset(rt.new_string('dimensions'))
		&& var_block.array_get('attrs').array_get('dimensions').array_isset(rt.new_string('widthType')) {
		if rt.is_true(rt.identical(rt.new_string('fixed'),
			var_block.array_get('attrs').array_get('dimensions').array_get('widthType')))
		{
			this.set_fixed_width_style(var_p_mutated,
				var_block.array_get('attrs').array_get('dimensions').array_get('fixedWidth'))
		}
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Renderer) add_navigation_link_directives(var_block_content rt.PhpVal, var_block rt.PhpVal, var_instance rt.PhpVal) rt.PhpVal {
	mut var_block_content_mutated := var_block_content
	mut var_query_context := if !(rt.get_property(var_instance, 'context').array_get('query')).is_null() {
		rt.get_property(var_instance, 'context').array_get('query')
	} else {
		rt.new_array()
	}
	mut var_is_product_collection_block := if !(var_query_context.array_get('isProductCollectionBlock')).is_null() {
		var_query_context.array_get('isProductCollectionBlock')
	} else {
		rt.new_bool(false)
	}
	mut var_query_id := if !(rt.get_property(var_instance, 'context').array_get('queryId')).is_null() {
		rt.get_property(var_instance, 'context').array_get('queryId')
	} else {
		rt.new_null()
	}
	mut var_parsed_query_id := if !(this.parsed_block.array_get('attrs').array_get('queryId')).is_null() {
		this.parsed_block.array_get('attrs').array_get('queryId')
	} else {
		rt.new_null()
	}
	mut var_is_enhanced_pagination_enabled := rt.new_bool(rt.new_bool(!(rt.is_true(if !(this.parsed_block.array_get('attrs').array_get('forcePageReload')).is_null() {
		this.parsed_block.array_get('attrs').array_get('forcePageReload')
	} else {
		rt.new_bool(false)
	}))))
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_is_product_collection_block)
		&& rt.is_true(var_is_enhanced_pagination_enabled)))
		&& rt.is_true(rt.identical(var_query_id, var_parsed_query_id))))
	{
		...
		for rt.is_true(var_p.next_tag(rt.new_string('A'))) {
			if rt.is_true(rt.new_bool(
				rt.is_true(var_p.has_class(rt.new_string('wp-block-query-pagination-next')))
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Renderer) extend_context_for_inner_blocks(var_context rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(var_context_mutated.array_isset(rt.new_string('query')))
		|| !(var_context_mutated.array_get('query').array_isset(rt.new_string('isProductCollectionBlock')))
		|| rt.is_true(rt.new_bool(!(rt.is_true(var_context_mutated.array_get('query').array_get('isProductCollectionBlock')))))))
	{
		return var_context_mutated.dup()
	}
	mut var_is_in_single_product := rt.new_bool(rt.new_bool(
		var_context_mutated.array_isset(rt.new_string('singleProduct'))
		&& !(!rt.is_true(var_context_mutated.array_get('postId')))))
	var_context_mutated.array_set('productCollectionLocation', if rt.is_true(var_is_in_single_product) { rt.create_array([
			...
			rt.ArrayItem{ key: 'sourceData', val: rt.create_array([
				rt.ArrayItem{ key: 'productId', val: rt.call_function('absint', [
					var_context_mutated.array_get('postId'),
				]) },
			]) },
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Renderer) get_location_context() rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_null(), var_location_context)) {
		mut var_location_context := fn () rt.PhpVal {
			mut temp := Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Utils{}
			return temp.parse_frontend_location_context()
		}()
	}
	return var_location_context.dup()
}
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_productcollection_renderer_php() {
