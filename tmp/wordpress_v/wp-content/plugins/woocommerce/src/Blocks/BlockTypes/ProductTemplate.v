...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductTemplate) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_query := fn (arg_0 rt.PhpVal) rt.PhpVal {
		mut temp := Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Utils{}
		return temp.prepare_and_execute_query(arg_0)
	}(var_block.dup())
	...
	mut var_classnames := rt.new_string(rt.new_string(''))
	if rt.get_property(var_block, 'context').array_isset(rt.new_string('displayLayout'))
		...
		var_classnames = rt.new_string('is-product-collection-layout-' +
			(rt.get_property(var_block, 'context').array_get('displayLayout').array_get('type')).str() +
			' ')
		if rt.is_true(rt.new_bool(
			rt.get_property(var_block, 'context').array_get('displayLayout').array_isset(rt.new_string('type'))
			&& rt.is_true(rt.identical(rt.new_string('flex'), rt.get_property(var_block, 'context').array_get('displayLayout').array_get('type')))))
		{
			if rt.is_true(rt.new_bool(
				rt.get_property(var_block, 'context').array_get('displayLayout').array_isset(rt.new_string('shrinkColumns'))
				&& rt.is_true(rt.get_property(var_block, 'context').array_get('displayLayout').array_get('shrinkColumns'))))
			{
				var_classnames = rt.new_string(rt.concat(rt.new_string('wc-block-product-template__responsive columns-'), rt.get_property(var_block,
					'context').array_get('displayLayout').array_get('columns')))
			} else {
	...
	mut var_wrapper_attributes := rt.call_function('get_block_wrapper_attributes', [
		rt.create_array([
			rt.ArrayItem{ key: 'class', val: var_classnames.dup().to_string().trim_space() },
	...
	for rt.is_true(rt.call_method(var_query, 'have_posts', []rt.PhpVal{})) {
		...
		mut var_product_id := var_block_instance.array_set('blockName', 'core/null')
		...
		mut var_block_content := rt.call_method(create_wp_block(var_block_instance.dup(),
			var_available_context.dup()), 'render', [
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductTemplate) block_core_post_template_uses_featured_image(var_inner_blocks rt.PhpVal) bool {
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.identical(rt.new_string('core/cover'), rt.get_property(var_block, 'name')))
				&& !(!rt.is_true(rt.get_property(var_block, 'attributes').array_get('useFeaturedImage')))))
			{
				return true
			}
			if rt.is_true(rt.new_bool(rt.is_true(rt.get_property(var_block, 'inner_blocks'))
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductTemplate) add_block_type_metadata_settings(var_settings rt.PhpVal, var_metadata rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(!rt.is_true(var_metadata.array_get('name')))
		&& rt.is_true(rt.identical(rt.new_string('woocommerce/product-template'), var_metadata.array_get('name')))))
	{
...
fn create_automattic_woocommerce_blocks_blocktypes_producttemplate() &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductTemplate {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_producttemplate_php() {
}
