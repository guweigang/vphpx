...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_EmailContent) render_preview(var_attributes rt.PhpVal) rt.PhpVal {
	...
	if var_attributes.array_isset(rt.new_string('postId')) {
		mut var_email_type_class_name := rt.call_method(fn () rt.PhpVal {
			mut temp :=
		...
		}(), 'get_email_type_class_name_from_post_id', [var_attributes.array_get('postId')])
		...
		var_type_param = rt.call_function('sanitize_text_field', [
			rt.call_function('wp_unslash', [var_attributes.array_get('emailType')]),
		])
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_EmailContent) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('REST_REQUEST')]))
		&& rt.is_true(rt.get_constant('REST_REQUEST'))))
		&& rt.get_superglobal('_GET').array_isset(rt.new_string('context'))))
		&& rt.is_true(rt.identical(rt.new_string('edit'), rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get('context')])])))))
	{
		return this.render_preview(var_attributes.dup())
	}
...
fn create_automattic_woocommerce_blocks_blocktypes_emailcontent() &Class_Automattic_WooCommerce_Blocks_BlockTypes_EmailContent {
...
pub fn init_wp_content_plugins_woocommerce_src_blocks_blocktypes_emailcontent_php() {
}
