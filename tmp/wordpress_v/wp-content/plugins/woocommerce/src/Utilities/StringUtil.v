...
fn Class_Automattic_WooCommerce_Utilities_StringUtil.starts_with(string string, starts_with string, case_sensitive bool) bool {
	...
	mut var_len := rt.new_int(rt.new_int(starts_with.len))
	if rt.is_true(rt.greater(var_len, rt.new_int(string_mutated.len))) {
		return false
	}
	string_mutated = (rt.call_function('substr', [rt.new_string(string_mutated).dup(), rt.new_int(0), var_len.dup()])).str()
	if var_case_sensitive {
		return (rt.identical(rt.call_function('strcmp', [rt.new_string(string_mutated).dup(), rt.new_string(starts_with)]), rt.new_int(0))).to_bool()
	}
	return (rt.identical(rt.call_function('strcasecmp', [rt.new_string(string_mutated).dup(), rt.new_string(starts_with)]), rt.new_int(0))).to_bool()
}

fn Class_Automattic_WooCommerce_Utilities_StringUtil.ends_with(string string, ends_with string, case_sensitive bool) bool {
	mut string_mutated := string
	mut var_len := rt.new_int(rt.new_int(ends_with.len))
	if rt.is_true(rt.greater(var_len, rt.new_int(string_mutated.len))) {
		return false
	}
	string_mutated = (rt.call_function('substr', [rt.new_string(string_mutated).dup(), // unsupported expression: Expr_UnaryMinus])).str()
	if var_case_sensitive {
		return (rt.identical(rt.call_function('strcmp', [rt.new_string(string_mutated).dup(), rt.new_string(ends_with)]), rt.new_int(0))).to_bool()
	}
	return (rt.identical(rt.call_function('strcasecmp', [rt.new_string(string_mutated).dup(), rt.new_string(ends_with)]), rt.new_int(0))).to_bool()
}
...
fn Class_Automattic_WooCommerce_Utilities_StringUtil.is_null_or_empty(mut var_value Class_Automattic_WooCommerce_Utilities_?string) bool {
	return rt.is_true(rt.new_bool(var_value.is_null())) || rt.is_true(rt.identical(rt.new_string(''), var_value))
}
...
fn Class_Automattic_WooCommerce_Utilities_StringUtil.is_null_or_whitespace(mut var_value Class_Automattic_WooCommerce_Utilities_?string) bool {
	return rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_value.is_null())) || rt.is_true(rt.identical(rt.new_string(''), var_value)))) || rt.is_true(rt.call_function('ctype_space', [var_value]))
}
...
fn Class_Automattic_WooCommerce_Utilities_StringUtil.normalize_local_path_slashes(mut var_path Class_Automattic_WooCommerce_Utilities_?string) rt.PhpVal {
	return if rt.is_true(rt.new_bool(var_path.is_null())) { rt.new_null() } else { rt.call_function('str_replace', [rt.create_array([rt.ArrayItem{ key: none, val: '\\' }, rt.ArrayItem{ key: none, val: '/' }]), rt.get_constant('DIRECTORY_SEPARATOR'), var_path]) }
}
...
pub fn init_wp_content_plugins_woocommerce_src_utilities_stringutil_php() {
}
