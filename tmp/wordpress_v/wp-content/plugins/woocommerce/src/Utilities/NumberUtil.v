...
fn Class_Automattic_WooCommerce_Utilities_NumberUtil.normalize(var_value rt.PhpVal, fallback i64) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_value_mutated.dup().is_long() || var_value_mutated.dup().is_double())) {
		mut var_numeric_value := if rt.is_true(rt.new_bool(var_value_mutated.dup().is_string())) { rt.new_float(var_value_mutated.dup().to_f64()) } else { var_value_mutated }
		return if rt.is_true(rt.new_bool(var_numeric_value.dup().is_long())) { var_numeric_value } else { rt.call_function('round', [var_numeric_value.dup(), rt.get_constant('WC_ROUNDING_PRECISION')]) }
	}
...
fn Class_Automattic_WooCommerce_Utilities_NumberUtil.array_sum(mut var_arr Class_Automattic_WooCommerce_Utilities_array) f64 {
	...
	return (rt.call_function('array_sum', [var_sanitized_array.dup()])).to_f64()
}
...
fn Class_Automattic_WooCommerce_Utilities_NumberUtil.sanitize_cost_in_current_locale(var_value rt.PhpVal) string {
	...
	mut var_currency_symbol_variations := rt.create_array([rt.ArrayItem{ key: none, val: var_currency_symbol_encoded }, rt.ArrayItem{ key: none, val: rt.call_function('wp_kses_normalize_entities', [var_currency_symbol_encoded.dup()]) }, rt.ArrayItem{ key: none, val: rt.call_function('html_entity_decode', [var_currency_symbol_encoded.dup(), rt.get_constant('ENT_COMPAT')]) }])
	var_value_mutated = rt.call_function('str_replace', [var_currency_symbol_variations.dup(), rt.new_string(''), var_value_mutated.dup()])
	mut var_decimal_point_count := rt.call_function('substr_count', [var_value_mutated.dup(), rt.new_string('.')])
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_int(1), var_decimal_point_count)) && rt.is_true(rt.new_bool(var_value_mutated.dup().is_long() || var_value_mutated.dup().is_double())))) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.greater(rt.call_function('substr_count', [var_value_mutated.dup(), var_decimal_separator.dup()]), rt.new_int(1))) || rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) && rt.is_true(rt.less_equal(rt.call_function('strpos', [var_value_mutated.dup(), var_decimal_separator.dup()]), rt.call_function('strpos', [var_value_mutated.dup(), var_thousand_separator.dup()]))))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Utilities_InvalidArgumentException', []string{}, create_automattic_woocommerce_utilities_invalidargumentexception(rt.call_function('esc_html', [rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('%s is not a valid numeric value: there should be one decimal separator and it has to be after the thousands separator.'), rt.new_string('woocommerce')]), var_value_mutated.dup()])]))))
	}
	var_value_mutated = rt.call_function('str_replace', [rt.call_function('wc_get_price_thousand_separator', []rt.PhpVal{}), rt.new_string(''), var_value_mutated.dup()])
	var_value_mutated = rt.call_function('str_replace', [rt.call_function('wc_get_price_decimal_separator', []rt.PhpVal{}), rt.new_string('.'), var_value_mutated.dup()])
	if rt.is_true(rt.new_bool(rt.is_true(var_value_mutated) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_value_mutated.dup().is_long() || var_value_mutated.dup().is_double()))))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Utilities_InvalidArgumentException', []string{}, create_automattic_woocommerce_utilities_invalidargumentexception(rt.call_function('esc_html', [rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('%s is not a valid numeric value.'), rt.new_string('woocommerce')]), var_value_mutated.dup()])]))))
	}
	return (var_value_mutated).str()
}
...
fn create_automattic_woocommerce_utilities_invalidargumentexception() &Class_Automattic_WooCommerce_Utilities_InvalidArgumentException {
...
pub fn init_wp_content_plugins_woocommerce_src_utilities_numberutil_php() {
}
