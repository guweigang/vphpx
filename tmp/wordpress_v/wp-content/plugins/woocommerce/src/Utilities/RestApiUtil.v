...
fn (mut this Class_Automattic_WooCommerce_Utilities_RestApiUtil) get_endpoint_data(var_endpoint rt.PhpVal, var_params rt.PhpVal) rt.PhpVal {
	mut var_request := create_automattic_woocommerce_utilities_wp_rest_request(rt.new_string('GET'), var_endpoint.dup())
	if rt.is_true(var_params) {
		var_request.set_query_params(var_params.dup())
	}
	mut var_response := rt.call_function('rest_do_request', [var_request])
	mut var_server := rt.call_function('rest_get_server', []rt.PhpVal{})
	mut var_json := rt.call_function('wp_json_encode', [rt.call_method(var_server, 'response_to_data', [var_response.dup(), rt.new_bool(false)])])
	return rt.call_function('json_decode', [var_json.dup(), rt.new_bool(true)])
}
...
fn (mut this Class_Automattic_WooCommerce_Utilities_RestApiUtil) lazy_load_namespace(route_namespace string, mut var_callback Class_Automattic_WooCommerce_Utilities_callable)  {
	...
	mut var_should_lazy_load_namespace := rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_should_lazy_load_namespace'), rt.new_bool(true), rt.new_string(route_namespace_mutated).dup()])
...
fn (mut this Class_Automattic_WooCommerce_Utilities_RestApiUtil) attach_lazy_loaded_namespace(route_namespace string, mut var_callback Class_Automattic_WooCommerce_Utilities_callable, rest_route string, callback_filter_id string)  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string(''), rt.new_string(rest_route_mutated))) && var_GLOBALS.array_isset(rt.new_string('wp')))) && rt.is_true(rt.new_bool(var_GLOBALS.array_get('wp').is_object())))) {
		rest_route_mutated = (if !(rt.get_property(var_GLOBALS.array_get('wp'), 'query_vars').array_get('rest_route')).is_null() { rt.get_property(var_GLOBALS.array_get('wp'), 'query_vars').array_get('rest_route') } else { rt.new_string('') }).str()
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		rest_route_mutated = (rt.call_function('trailingslashit', [rt.new_string(rest_route_mutated.trim_left(' \t\n\r'))])).str()
		route_namespace_mutated = (rt.call_function('trailingslashit', [rt.new_string(route_namespace_mutated).dup()])).str()
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('/'), rt.new_string(rest_route_mutated))) || rt.is_true(rt.call_function('str_starts_with', [rt.new_string(rest_route_mutated).dup(), rt.new_string(route_namespace_mutated).dup()])))) {
			if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
				rt.call_function('remove_filter', [rt.new_string('rest_pre_dispatch'), rt.new_string(callback_filter_id_mutated).dup(), rt.new_int(0)])
			}
			rt.call_function('call_user_func', [var_callback])
			return rt.new_null()
		}
	...
	mut var_filter_result := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	mut var_server := if args.len > 1 { args[1].dup() } else { rt.new_null() }
	...
	return var_filter_result.dup()
	}
		mut var_callback_filter := rt.new_closure(closure_1_fn)
		callback_filter_id_mutated = (rt.call_function('_wp_filter_build_unique_id', [rt.new_string('rest_pre_dispatch'), var_callback_filter.dup(), rt.new_int(0)])).str()
		rt.call_function('add_filter', [rt.new_string('rest_pre_dispatch'), var_callback_filter.dup(), rt.new_int(0), rt.new_int(3)])
	}
}
...
fn create_automattic_woocommerce_utilities_restapiutil() &Class_Automattic_WooCommerce_Utilities_RestApiUtil {
...
fn create_automattic_woocommerce_utilities_wp_rest_request() &Class_Automattic_WooCommerce_Utilities_WP_REST_Request {
...
pub fn init_wp_content_plugins_woocommerce_src_utilities_restapiutil_php() {
}
