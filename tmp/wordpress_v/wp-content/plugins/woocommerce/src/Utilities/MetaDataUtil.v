...
fn Class_Automattic_WooCommerce_Utilities_MetaDataUtil.update(var_meta_data rt.PhpVal, mut var_target Class_WC_Data, default_id string) {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_meta_data.dup().is_array()))))) {
		return rt.new_null()
	}
	{
		...
		for {
			...
			var_target.update_meta_data(var_meta.array_get('key'), var_meta.array_get('value'),
				var_meta.array_get('id'))
		}
	}
}
...
fn Class_Automattic_WooCommerce_Utilities_MetaDataUtil.normalize(mut var_meta_data Class_Automattic_WooCommerce_Utilities_array, default_id string) rt.PhpVal {
	mut var_normalized := rt.new_array()
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_meta := item_1.val
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_meta.dup().is_array()))
				&& var_meta.array_isset(rt.new_string('key'))))
			{
				var_normalized.array_push(rt.create_array([
					rt.ArrayItem{ key: 'key', val: var_meta.array_get('key') },
					rt.ArrayItem{
						key: 'value'
						val: if !(var_meta.array_get('value')).is_null() {
							var_meta.array_get('value')
						} else {
							rt.new_null()
						}
					},
					rt.ArrayItem{
						key: 'id'
						val: if !(var_meta.array_get('id')).is_null() {
							var_meta.array_get('id')
						} else {
...
pub fn init_wp_content_plugins_woocommerce_src_utilities_metadatautil_php() {
