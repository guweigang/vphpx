...
pub fn init_wp_content_plugins_woocommerce_templates_order_order_details_php() {
	...
	rt.call_function('do_action', [rt.new_string('woocommerce_order_details_before_order_table_items'), var_order.dup()])
	{
		...
		for {
			...
			rt.call_function('wc_get_template', [rt.new_string('order/order-details-item.php'), rt.create_array([rt.ArrayItem{ key: 'order', val: var_order }, rt.ArrayItem{ key: 'item_id', val: var_item_id }, rt.ArrayItem{ key: 'item', val: var_item }, rt.ArrayItem{ key: 'show_purchase_note', val: var_show_purchase_note }, rt.ArrayItem{ key: 'purchase_note', val: if rt.is_true(var_product) { rt.call_method(var_product, 'get_purchase_note', []rt.PhpVal{}) } else { rt.new_string('') } }, rt.ArrayItem{ key: 'product', val: var_product }])])
		}
	}
	...
	if rt.is_true(rt.call_method(var_order, 'get_customer_note', []rt.PhpVal{})) {
		...
		// unsupported statement: Stmt_InlineHTML
		mut var_customer_note := rt.call_function('wc_wptexturize_order_note', [rt.call_method(var_order, 'get_customer_note', []rt.PhpVal{})])
		rt.echo_val(rt.call_function('wp_kses', [rt.call_function('nl2br', [var_customer_note.dup()]), rt.create_array([rt.ArrayItem{ key: 'br', val: rt.new_array() }])]))
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
