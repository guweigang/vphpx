...
pub fn init_wp_content_plugins_woocommerce_templates_myaccount_form_login_php() {
	...
	rt.call_function('esc_html_e', [rt.new_string('Required'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(if rt.is_true(rt.new_bool(!(!rt.is_true(rt.get_superglobal('_POST').array_get('username'))) && rt.is_true(rt.new_bool(rt.get_superglobal('_POST').array_get('username').is_string())))) { rt.call_function('esc_attr', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_POST').array_get('username')]),
		]) } else { rt.new_string('') })
	// unsupported statement: Stmt_InlineHTML
	// unsupported statement: Stmt_Nop
	// unsupported statement: Stmt_InlineHTML
	...
	])))
	{
		...
		])))
		{
			...
			rt.call_function('esc_html_e', [rt.new_string('Required'),
				rt.new_string('woocommerce')])
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(if !(!rt.is_true(rt.get_superglobal('_POST').array_get('username'))) { rt.call_function('esc_attr', [
					rt.call_function('wp_unslash', [rt.get_superglobal('_POST').array_get('username')]),
				]) } else { rt.new_string('') })
			// unsupported statement: Stmt_InlineHTML
			// unsupported statement: Stmt_Nop
		...
		rt.call_function('esc_html_e', [rt.new_string('Required'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(if !(!rt.is_true(rt.get_superglobal('_POST').array_get('email'))) { rt.call_function('esc_attr', [
				rt.call_function('wp_unslash', [rt.get_superglobal('_POST').array_get('email')]),
			]) } else { rt.new_string('') })
		// unsupported statement: Stmt_InlineHTML
		// unsupported statement: Stmt_Nop
		// unsupported statement: Stmt_InlineHTML
