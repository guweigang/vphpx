...
pub fn init_wp_content_plugins_woocommerce_templates_brands_widgets_brand_thumbnails_php() {
	mut var_fluid_columns := rt.new_null()
	mut var_columns := rt.new_null()
	mut var_brands := rt.new_null()
	// unsupported statement: Stmt_Declare
	mut var_wrapper_class := rt.new_string(rt.new_string('fluid-columns'))
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_fluid_columns))))
		&& rt.is_true(rt.call_function('in_array', [var_columns.dup(), rt.create_array([rt.ArrayItem{
		key: none
		val: 1
	...
	rt.echo_val(rt.call_function('esc_attr', [var_wrapper_class.dup()]))
	// unsupported statement: Stmt_InlineHTML
	{
		mut iter_1 := rt.call_function('array_values', [var_brands.dup()]).iterator()
		for {
