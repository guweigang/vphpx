...
pub fn init_wp_content_plugins_woocommerce_templates_brands_shortcodes_brands_a_z_php() {
	...
	// unsupported statement: Stmt_InlineHTML
	{
		...
		for {
			...
			if var_product_brands.array_isset(var_i) {
				print('<li><a href="#brands-' +
					(rt.call_function('esc_attr', [var_i.dup()])).str() + '">' +
					(rt.call_function('esc_html', [var_i.dup()])).str() + '</a></li>')
			} else if rt.is_true(var_show_empty) {
				print('<li><span>' + (rt.call_function('esc_html', [var_i.dup()])).str() +
					'</span></li>')
			}
		}
	}
	// unsupported statement: Stmt_InlineHTML
	{
		...
		for {
			...
			if var_product_brands.array_isset(var_i) {
				...
				// unsupported statement: Stmt_InlineHTML
				rt.echo_val(rt.call_function('esc_html', [var_i.dup()]))
				// unsupported statement: Stmt_InlineHTML
				{
					mut iter_2 := var_product_brands.array_get(var_i).iterator()
					for {
