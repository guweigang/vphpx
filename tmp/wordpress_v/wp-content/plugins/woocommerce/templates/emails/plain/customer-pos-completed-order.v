...
pub fn init_wp_content_plugins_woocommerce_templates_emails_plain_customer_pos_completed_order_php() {
	...
	if !(!rt.is_true(var_pos_store_email)) || !(!rt.is_true(var_pos_store_phone_number)) || !(!rt.is_true(var_pos_store_address)) {
		if !(!rt.is_true(var_pos_store_name)) {
			print('\n' + (rt.call_function('esc_html', [var_pos_store_name.dup()])).str() + '\n\n')
		}
		if !(!rt.is_true(var_pos_store_email)) {
			print((rt.call_function('esc_html', [var_pos_store_email.dup()])).str() + '\n')
		}
		if !(!rt.is_true(var_pos_store_phone_number)) {
			print((rt.call_function('esc_html', [var_pos_store_phone_number.dup()])).str() + '\n')
		}
		if !(!rt.is_true(var_pos_store_address)) {
			print((rt.call_function('esc_html', [rt.call_function('wp_strip_all_tags', [rt.call_function('wptexturize', [var_pos_store_address.dup()])])])).str() + '\n')
		}
		print('\n----------------------------------------\n\n')
	}
	if !(!rt.is_true(var_pos_refund_returns_policy)) {
		...
		print((rt.call_function('esc_html', [rt.call_function('wp_strip_all_tags', [rt.call_function('wptexturize', [var_pos_refund_returns_policy.dup()])])])).str() + '\n')
		print('\n----------------------------------------\n\n')
	}
	rt.echo_val(rt.call_function('wp_kses_post', [rt.call_function('apply_filters', [rt.new_string('woocommerce_email_footer_text'), rt.call_function('get_option', [rt.new_string('woocommerce_email_footer_text')]), var_email.dup()])]))
}
