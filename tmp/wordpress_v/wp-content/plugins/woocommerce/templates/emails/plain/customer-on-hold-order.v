...
pub fn init_wp_content_plugins_woocommerce_templates_emails_plain_customer_on_hold_order_php() {
	...
	if rt.is_true(var_email_improvements_enabled) {
		print((rt.call_function('esc_html__', [rt.new_string('We’ve received your order and it’s currently on hold until we can confirm your payment has been processed.'), rt.new_string('woocommerce')])).str() + '\n\n')
		print((rt.call_function('esc_html__', [rt.new_string('Here’s a reminder of what you’ve ordered:'), rt.new_string('woocommerce')])).str() + '\n\n')
	} else {
		print((rt.call_function('esc_html__', [rt.new_string('Thanks for your order. It’s on-hold until we confirm that payment has been received.'), rt.new_string('woocommerce')])).str() + '\n\n')
	}
