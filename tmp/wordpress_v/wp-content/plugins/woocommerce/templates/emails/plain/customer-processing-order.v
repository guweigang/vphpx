...
pub fn init_wp_content_plugins_woocommerce_templates_emails_plain_customer_processing_order_php() {
	...
	if rt.is_true(var_email_improvements_enabled) {
		print((rt.call_function('esc_html__', [rt.new_string('Just to let you know &mdash; we’ve received your order, and it is now being processed.'), rt.new_string('woocommerce')])).str() + '\n\n')
		print((rt.call_function('esc_html__', [rt.new_string('Here’s a reminder of what you’ve ordered:'), rt.new_string('woocommerce')])).str() + '\n\n')
	} else {
		print((rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Just to let you know &mdash; we\'ve received your order #%s, and it is now being processed:'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [rt.call_method(var_order, 'get_order_number', []rt.PhpVal{})])])).str() + '\n\n')
	}
