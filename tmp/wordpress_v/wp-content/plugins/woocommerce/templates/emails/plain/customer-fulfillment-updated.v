...
pub fn init_wp_content_plugins_woocommerce_templates_emails_plain_customer_fulfillment_updated_php() {
	...
	print('\n=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n\n')
	print((rt.call_function('esc_html__', [rt.new_string('Some details of your shipment have recently been updated. This may include tracking information, item contents, or delivery status.'), rt.new_string('woocommerce')])).str() + '\n\n')
	print((rt.call_function('esc_html__', [rt.new_string('Here’s the latest info we have:'), rt.new_string('woocommerce')])).str() + '\n\n')
	...
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		print((rt.call_function('esc_html__', [rt.new_string('Note from the store:'), rt.new_string('woocommerce')])).str() + '\n')
		print((rt.call_function('esc_html', [rt.call_function('wp_strip_all_tags', [rt.call_function('wptexturize', [rt.new_string(var_customer_note_text).dup()])])])).str() + '\n\n')
	}
