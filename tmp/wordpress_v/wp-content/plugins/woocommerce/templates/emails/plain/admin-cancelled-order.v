...
pub fn init_wp_content_plugins_woocommerce_templates_emails_plain_admin_cancelled_order_php() {
	...
	print('\n=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n\n')
	mut var_text := rt.call_function('__', [rt.new_string('Notification to let you know &mdash; order #%1$s belonging to %2$s has been cancelled:'), rt.new_string('woocommerce')])
	if rt.is_true(var_email_improvements_enabled) {
		var_text = rt.call_function('__', [rt.new_string('We’re getting in touch to let you know that order #%1$s from %2$s has been cancelled.'), rt.new_string('woocommerce')])
	}
	print((rt.call_function('sprintf', [rt.call_function('esc_html', [var_text.dup()]), rt.call_function('esc_html', [rt.call_method(var_order, 'get_order_number', []rt.PhpVal{})]), rt.call_function('esc_html', [rt.call_method(var_order, 'get_formatted_billing_full_name', []rt.PhpVal{})])])).str() + '\n\n')
