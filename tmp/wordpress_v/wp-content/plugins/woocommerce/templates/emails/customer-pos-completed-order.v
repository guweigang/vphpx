...
pub fn init_wp_content_plugins_woocommerce_templates_emails_customer_pos_completed_order_php() {
	...
	if !(!rt.is_true(var_pos_store_email)) || !(!rt.is_true(var_pos_store_phone_number))
		...
		if !(!rt.is_true(var_pos_store_name)) {
			print('<h2>' + (rt.call_function('esc_html', [var_pos_store_name.dup()])).str() +
				'</h2>')
		}
		...
		if !(!rt.is_true(var_pos_store_phone_number)) {
			print('<p>' + (rt.call_function('esc_html', [var_pos_store_phone_number.dup()])).str() +
				'</p>')
		}
		if !(!rt.is_true(var_pos_store_address)) {
			rt.echo_val(rt.call_function('wp_kses_post', [
				rt.call_function('wpautop', [
					rt.call_function('wptexturize', [var_pos_store_address.dup()]),
				]),
			]))
		}
		print('</div>')
	...
	if !(!rt.is_true(var_pos_refund_returns_policy)) {
		...
		rt.echo_val(rt.call_function('wp_kses_post', [
			rt.call_function('wpautop', [
				rt.call_function('wptexturize', [var_pos_refund_returns_policy.dup()]),
			]),
		]))
		print('</div>')
	}
