...
pub fn init_wp_content_plugins_woocommerce_templates_emails_customer_refunded_order_php() {
	...
	if !(!rt.is_true(rt.call_method(var_order, 'get_billing_first_name', []rt.PhpVal{}))) {
		...
		rt.call_function('printf', [rt.call_function('esc_html__', [rt.new_string('Hi,'), rt.new_string('woocommerce')])])
	}
	// unsupported statement: Stmt_InlineHTML
	if rt.is_true(var_email_improvements_enabled) {
		if rt.is_true(var_partial_refund) {
			print((rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Your order from %s has been partially refunded.'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_blogname.dup()])])).str() + '\n\n')
		} else {
			print((rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Your order from %s has been refunded.'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_blogname.dup()])])).str() + '\n\n')
		}
		print('</p><p>')
		...
		rt.call_function('printf', [rt.call_function('esc_html__', [rt.new_string('Your order on %s has been partially refunded. There are more details below for your reference:'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_blogname.dup()])])
	} else {
		rt.call_function('printf', [rt.call_function('esc_html__', [rt.new_string('Your order on %s has been refunded. There are more details below for your reference:'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_blogname.dup()])])
	}
	// unsupported statement: Stmt_InlineHTML
