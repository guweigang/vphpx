...
pub fn init_wp_content_plugins_woocommerce_templates_emails_admin_payment_gateway_enabled_php() {
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('printf', [rt.call_function('esc_html__', [rt.new_string('Howdy %s,'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_username.dup()])])
	// unsupported statement: Stmt_InlineHTML
	print(if rt.is_true(var_email_improvements_enabled) { '</div>' } else { '' })
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('printf', [rt.call_function('esc_html__', [rt.new_string('The payment gateway "%1$s" was just enabled on this site: %2$s'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_gateway_title.dup()]), rt.call_function('esc_html', [rt.call_function('home_url', []rt.PhpVal{})])])
	// unsupported statement: Stmt_InlineHTML
	...
	rt.echo_val(rt.call_function('esc_url', [var_gateway_settings_url.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('printf', [rt.call_function('esc_html__', [rt.new_string('This email has been sent to %s'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_admin_email.dup()])])
	// unsupported statement: Stmt_InlineHTML
