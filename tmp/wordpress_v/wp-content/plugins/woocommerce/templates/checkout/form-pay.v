...
pub fn init_wp_content_plugins_woocommerce_templates_checkout_form_pay_php() {
	...
	if rt.call_method(var_order, 'get_items', []rt.PhpVal{}).array_count() > 0 {
		// unsupported statement: Stmt_InlineHTML
		{
			...
			for {
				...
				// unsupported statement: Stmt_InlineHTML
				rt.echo_val(rt.call_function('apply_filters', [rt.new_string('woocommerce_order_item_quantity_html'), ' <strong class="product-quantity">' + (rt.call_function('sprintf', [rt.new_string('&times;&nbsp;%s'), rt.call_function('esc_html', [rt.call_method(var_item, 'get_quantity', []rt.PhpVal{})])])).str() + '</strong>', var_item.dup()]))
				// unsupported statement: Stmt_InlineHTML
				// unsupported statement: Stmt_Nop
				// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(var_totals) {
		// unsupported statement: Stmt_InlineHTML
		{
			...
			for {
				...
				// unsupported statement: Stmt_InlineHTML
				rt.echo_val(var_total.array_get('label'))
				// unsupported statement: Stmt_InlineHTML
				// unsupported statement: Stmt_Nop
				// unsupported statement: Stmt_InlineHTML
				rt.echo_val(var_total.array_get('value'))
				// unsupported statement: Stmt_InlineHTML
				// unsupported statement: Stmt_Nop
				// unsupported statement: Stmt_InlineHTML
			}
	...
	if rt.is_true(rt.call_method(var_order, 'needs_payment', []rt.PhpVal{})) {
		...
		if !(!rt.is_true(var_available_gateways)) {
			...
			print('<li>')
			rt.call_function('wc_print_notice', [rt.call_function('apply_filters', [rt.new_string('woocommerce_no_available_payment_methods_message'), rt.call_function('esc_html__', [rt.new_string('Sorry, it seems that there are no available payment methods for your location. Please contact us if you require assistance or wish to make alternate arrangements.'), rt.new_string('woocommerce')])]), rt.new_string('notice')])
			print('</li>')
		}
		// unsupported statement: Stmt_InlineHTML
	}
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('apply_filters', [rt.new_string('woocommerce_pay_order_button_html'), '<button type="submit" class="button alt' + (rt.call_function('esc_attr', [if rt.is_true(rt.call_function('wc_wp_theme_get_element_class_name', [rt.new_string('button')])) { ' ' + (rt.call_function('wc_wp_theme_get_element_class_name', [rt.new_string('button')])).str() } else { rt.new_string('') }])).str() + '" id="place_order" value="' + (rt.call_function('esc_attr', [var_order_button_text.dup()])).str() + '" data-value="' + (rt.call_function('esc_attr', [var_order_button_text.dup()])).str() + '">' + (rt.call_function('esc_html', [var_order_button_text.dup()])).str() + '</button>']))
	// unsupported statement: Stmt_Nop
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [rt.new_string('woocommerce_pay_order_after_submit')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('wp_nonce_field', [rt.new_string('woocommerce-pay'), rt.new_string('woocommerce-pay-nonce')])
	// unsupported statement: Stmt_InlineHTML
}
