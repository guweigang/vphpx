...
pub fn init_wp_content_plugins_woocommerce_templates_checkout_review_order_php() {
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [rt.new_string('woocommerce_review_order_before_cart_contents')])
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var__product) && rt.is_true(rt.call_method(var__product, 'exists', []rt.PhpVal{})))) && rt.is_true(rt.greater(var_cart_item.array_get('quantity'), rt.new_int(0))))) && rt.is_true(rt.call_function('apply_filters', [rt.new_string('woocommerce_checkout_cart_item_visible'), rt.new_bool(true), var_cart_item.dup(), var_cart_item_key.dup()])))) {
				// unsupported statement: Stmt_InlineHTML
				...
				// unsupported statement: Stmt_InlineHTML
				print((rt.call_function('wp_kses_post', [rt.call_function('apply_filters', [rt.new_string('woocommerce_cart_item_name'), rt.call_method(var__product, 'get_name', []rt.PhpVal{}), var_cart_item.dup(), var_cart_item_key.dup()])])).str() + '&nbsp;')
				// unsupported statement: Stmt_InlineHTML
				rt.echo_val(rt.call_function('apply_filters', [rt.new_string('woocommerce_checkout_cart_item_quantity'), ' <strong class="product-quantity">' + (rt.call_function('sprintf', [rt.new_string('&times;&nbsp;%s'), var_cart_item.array_get('quantity')])).str() + '</strong>', var_cart_item.dup(), var_cart_item_key.dup()]))
	...
	rt.call_function('do_action', [rt.new_string('woocommerce_review_order_after_cart_contents')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Subtotal'), rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'cart'), 'needs_shipping', []rt.PhpVal{})) && rt.is_true(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'cart'), 'show_shipping', []rt.PhpVal{})))) {
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('do_action', [rt.new_string('woocommerce_review_order_before_shipping')])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('wc_cart_totals_shipping_html', []rt.PhpVal{})
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('do_action', [rt.new_string('woocommerce_review_order_after_shipping')])
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
	{
		...
		if rt.is_true(rt.identical(rt.new_string('itemized'), rt.call_function('get_option', [rt.new_string('woocommerce_tax_total_display')]))) {
			// unsupported statement: Stmt_InlineHTML
			{
				...
				for {
					...
					rt.echo_val(rt.call_function('esc_attr', [rt.call_function('sanitize_title', [var_code.dup()])]))
					// unsupported statement: Stmt_InlineHTML
					rt.echo_val(rt.call_function('esc_html', [rt.get_property(var_tax, 'label')]))
					// unsupported statement: Stmt_InlineHTML
			...
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('esc_html', [rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'tax_or_vat', []rt.PhpVal{})]))
			// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [rt.new_string('woocommerce_review_order_before_order_total')])
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [rt.new_string('woocommerce_review_order_after_order_total')])
	// unsupported statement: Stmt_InlineHTML
}
