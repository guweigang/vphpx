...
pub fn init_wp_content_plugins_woocommerce_templates_global_form_login_php() {
	...
	rt.echo_val(if rt.is_true(var_message) { rt.call_function('wpautop', [
			rt.call_function('wptexturize', [var_message.dup()]),
		]) } else { rt.new_string('') })
	// unsupported statement: Stmt_Nop
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_url', [var_redirect.dup()]))
	// unsupported statement: Stmt_InlineHTML
