...
fn Class_VHttpd_DbGateway_Client.fromenv(socketEnv string, poolEnv string, timeoutEnv string, defaultSocket string, defaultPool string) rt.PhpVal {
	...
	mut var_timeoutSeconds := if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_timeout.dup().is_string())) && rt.is_true(rt.call_function('ctype_digit', [var_timeout.dup()])))) { rt.div(rt.call_function('max', [rt.new_int(1), // unsupported expression: Expr_Cast_Int]), rt.new_int(1000)) } else { rt.new_float(5) }
	return create_vhttpd_dbgateway_self(if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_socket.dup().is_string())) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) { var_socket } else { rt.new_string(defaultSocket) }, if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_pool.dup().is_string())) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) { var_pool } else { rt.new_string(defaultPool) }, rt.new_float(1), var_timeoutSeconds.dup())
}
...
fn (mut this Class_VHttpd_DbGateway_Client) begintransaction(timeoutMs i64) string {
	...
	mut var_sessionId := // unsupported expression: Expr_Cast_String
...
fn (mut this Class_VHttpd_DbGateway_Client) call(op string, sql string, mut var_params Class_VHttpd_DbGateway_array, sessionId string, timeoutMs i64) rt.PhpVal {
	...
	mut var_value := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.call_function('base64_encode', [rt.new_string(var_value).dup()])
	}
	mut var_value := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	return rt.call_function('base64_encode', [rt.new_string(var_value).dup()])
	}
	mut var_request := rt.create_array([rt.ArrayItem{ key: 'version', val: 1 }, rt.ArrayItem{ key: 'mode', val: 'db' }, rt.ArrayItem{ key: 'op', val: op }, rt.ArrayItem{ key: 'pool', val: rt.get_property(rt.new_object('VHttpd_DbGateway_Client', []string{}, &this), 'pool') }, rt.ArrayItem{ key: 'timeout_ms', val: timeoutMs }, rt.ArrayItem{ key: 'session_id', val: sessionId_mutated }, rt.ArrayItem{ key: 'trace_id', val: Class_VHttpd_DbGateway_Client.runtimevalue('VHTTPD_TRACE_ID', 'HTTP_X_VHTTPD_TRACE_ID') }, rt.ArrayItem{ key: 'request_id', val: Class_VHttpd_DbGateway_Client.runtimevalue('VHTTPD_REQUEST_ID', 'HTTP_X_REQUEST_ID') }, rt.ArrayItem{ key: 'sql', val: sql }, rt.ArrayItem{ key: 'params', val: rt.call_function('array_map', [rt.new_closure(closure_3_fn), var_stringParams.dup()]) }, rt.ArrayItem{ key: 'params_base64', val: rt.call_function('array_map', [rt.new_closure(closure_5_fn), var_stringParams.dup()]) }])
	mut var_response := rt.call_method(this.client, 'request', [var_request.dup()])
	if rt.is_true(// unsupported expression: Expr_Cast_Bool) {
		return var_response.dup()
	}
	mut var_error := if !(var_response.array_get('error')).is_null() { var_response.array_get('error') } else { rt.new_string('db gateway call failed') }
	mut var_message := if rt.is_true(rt.new_bool(var_error.dup().is_array())) { // unsupported expression: Expr_Cast_String } else { // unsupported expression: Expr_Cast_String }
...
fn Class_VHttpd_DbGateway_Client.runtimevalue(envName string, serverName string) string {
	mut var_serverValue := if !(rt.get_superglobal('_SERVER').array_get(serverName)).is_null() { rt.get_superglobal('_SERVER').array_get(serverName) } else { if !(rt.get_superglobal('_SERVER').array_get(envName)).is_null() { rt.get_superglobal('_SERVER').array_get(envName) } else { rt.new_string('') } }
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_serverValue.dup().is_string())) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return (var_serverValue).str()
	}
	mut var_envValue := rt.call_function('getenv', [rt.new_string(envName)])
	return (if rt.is_true(rt.new_bool(var_envValue.dup().is_string())) { var_envValue } else { rt.new_string('') }).str()
}
...
fn create_vhttpd_dbgateway_self() &Class_VHttpd_DbGateway_self {
...
pub fn init_wp_content_plugins_v_profiler_src_vhttpd_dbgateway_client_php() {
