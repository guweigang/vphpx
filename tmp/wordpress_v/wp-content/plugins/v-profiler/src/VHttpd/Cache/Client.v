...
fn Class_VHttpd_Cache_Client.fromenv(socketEnv string, namespaceEnv string, defaultSocket string, defaultNamespace string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_socket.dup().is_string()))))) || rt.is_true(rt.identical(var_socket, rt.new_string(''))))) {
		var_socket = if !(rt.get_superglobal('_SERVER').array_get(socketEnv)).is_null() { rt.get_superglobal('_SERVER').array_get(socketEnv) } else { rt.new_string('') }
	}
	mut var_namespace := rt.call_function('getenv', [rt.new_string(namespaceEnv)])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_namespace.dup().is_string()))))) || rt.is_true(rt.identical(var_namespace, rt.new_string(''))))) {
		var_namespace = if !(rt.get_superglobal('_SERVER').array_get(namespaceEnv)).is_null() { rt.get_superglobal('_SERVER').array_get(namespaceEnv) } else { rt.new_string('') }
	}
	return create_vhttpd_cache_self(if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_socket.dup().is_string())) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) { var_socket } else { rt.new_string(defaultSocket) }, if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_namespace.dup().is_string())) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) { var_namespace } else { rt.new_string(defaultNamespace) })
}
...
fn (mut this Class_VHttpd_Cache_Client) get(key string, mut var_default Class_VHttpd_Cache_?string) string {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(// unsupported expression: Expr_Cast_Bool)))) {
		return var_default
	}
...
fn (mut this Class_VHttpd_Cache_Client) keys() rt.PhpVal {
	...
	mut var_keys := if !(var_response.array_get('keys')).is_null() { var_response.array_get('keys') } else { rt.new_null() }
	if rt.is_true(rt.new_bool(var_keys.dup().is_array())) {
		return rt.call_function('array_values', [rt.call_function('array_filter', [var_keys.dup(), rt.new_string('is_string')])])
	}
	mut var_raw := // unsupported expression: Expr_Cast_String
	mut var_decoded := rt.call_function('json_decode', [var_raw.dup(), rt.new_bool(true)])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_decoded.dup().is_array()))))) {
		return rt.new_array()
	}
	return rt.call_function('array_values', [rt.call_function('array_filter', [var_decoded.dup(), rt.new_string('is_string')])])
}
...
fn (mut this Class_VHttpd_Cache_Client) compareandswapset(key string, expectedFound bool, expectedValue string, value string, ttlMs i64) bool {
	...
	return rt.is_true(// unsupported expression: Expr_Cast_Bool) && rt.is_true(rt.new_bool(!(rt.is_true(// unsupported expression: Expr_Cast_Bool))))
}
...
fn (mut this Class_VHttpd_Cache_Client) remember(key string, ttlMs i64, mut var_loader Class_VHttpd_Cache_callable) string {
	...
	mut var_value := // unsupported expression: Expr_Cast_String
...
fn (mut this Class_VHttpd_Cache_Client) call(op string, key string, mut var_extra Class_VHttpd_Cache_array, throwOnConflict bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(var_throwOnConflict) && rt.is_true(// unsupported expression: Expr_Cast_Bool))) {
		return var_response.dup()
	}
	mut var_error := // unsupported expression: Expr_Cast_String
...
fn create_vhttpd_cache_self() &Class_VHttpd_Cache_self {
...
pub fn init_wp_content_plugins_v_profiler_src_vhttpd_cache_client_php() {
