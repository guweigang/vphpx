...
fn Class_VHttpd_PhpWorker_Response.fromarray(mut var_payload Class_VHttpd_PhpWorker_array) rt.PhpVal {
	return create_vhttpd_phpworker_self(// unsupported expression: Expr_Cast_String, // unsupported expression: Expr_Cast_Int, Class_VHttpd_PhpWorker_Response.normalizeheaders(mut rt.cast_object_ptr[Class_VHttpd_PhpWorker_mixed](if !(var_payload.array_get('headers')).is_null() { var_payload.array_get('headers') } else { rt.new_array() })), // unsupported expression: Expr_Cast_String)
}
...
fn Class_VHttpd_PhpWorker_Response.text(body string, status i64, mut var_headers Class_VHttpd_PhpWorker_array, id string) rt.PhpVal {
	...
	return create_vhttpd_phpworker_self(rt.new_string(id).dup(), rt.new_int(status).dup(), var_headers_mutated.dup(), rt.new_string(body).dup())
}
...
fn Class_VHttpd_PhpWorker_Response.json(mut var_data Class_VHttpd_PhpWorker_array, status i64, mut var_headers Class_VHttpd_PhpWorker_array, id string) rt.PhpVal {
	...
	return create_vhttpd_phpworker_self(rt.new_string(id).dup(), rt.new_int(status).dup(), var_headers_mutated.dup(), rt.new_string(rt.json_encode(var_data)))
}

fn (mut this Class_VHttpd_PhpWorker_Response) toarray() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: 'id', val: rt.get_property(rt.new_object('VHttpd_PhpWorker_Response', []string{}, &this), 'id') }, rt.ArrayItem{ key: 'status', val: rt.get_property(rt.new_object('VHttpd_PhpWorker_Response', []string{}, &this), 'status') }, rt.ArrayItem{ key: 'headers', val: Class_VHttpd_PhpWorker_Response.normalizeheaders(mut rt.cast_object_ptr[Class_VHttpd_PhpWorker_mixed](rt.get_property(rt.new_object('VHttpd_PhpWorker_Response', []string{}, &this), 'headers'))) }, rt.ArrayItem{ key: 'body', val: rt.get_property(rt.new_object('VHttpd_PhpWorker_Response', []string{}, &this), 'body') }])
}
...
fn Class_VHttpd_PhpWorker_Response.normalizeheaders(mut var_headers Class_VHttpd_PhpWorker_mixed) rt.PhpVal {
	...
	mut var_out := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_name.dup().is_string()))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_name.dup().is_long()))))))) {
				continue
			}
			var_out.array_set(// unsupported expression: Expr_Cast_String.to_string().to_lower(), if rt.is_true(rt.new_bool(var_value.dup().is_array())) { rt.call_function('implode', [rt.new_string(', '), rt.call_function('array_map', [rt.new_string('strval'), var_value.dup()])]) } else { // unsupported expression: Expr_Cast_String })
		}
	}
	return var_out.dup()
}
...
fn (mut this Class_VHttpd_PhpWorker_Response) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'text' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_VHttpd_PhpWorker_array](if args.len > 2 { args[2] } else { rt.new_null() })
			dispatch_arg_3 := (if args.len > 3 { args[3] } else { rt.new_null() }).str()
			return Class_VHttpd_PhpWorker_Response.text(dispatch_arg_0, dispatch_arg_1, mut dispatch_arg_2, dispatch_arg_3)
		}
		'html' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).to_i64()
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_VHttpd_PhpWorker_array](if args.len > 2 { args[2] } else { rt.new_null() })
			dispatch_arg_3 := (if args.len > 3 { args[3] } else { rt.new_null() }).str()
			return Class_VHttpd_PhpWorker_Response.html(dispatch_arg_0, dispatch_arg_1, mut dispatch_arg_2, dispatch_arg_3)
		}
		'json' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_VHttpd_PhpWorker_array](if args.len > 0 { args[0] } else { rt.new_null() })
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).to_i64()
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_VHttpd_PhpWorker_array](if args.len > 2 { args[2] } else { rt.new_null() })
			dispatch_arg_3 := (if args.len > 3 { args[3] } else { rt.new_null() }).str()
			return Class_VHttpd_PhpWorker_Response.json(mut dispatch_arg_0, dispatch_arg_1, mut dispatch_arg_2, dispatch_arg_3)
		}
		'toArray' {
			return this.toarray()
		}
...
pub fn init_wp_content_plugins_v_profiler_src_vhttpd_phpworker_response_php() {
