...
fn Class_VHttpd_PhpWorker_StreamResponse.text(mut var_chunks Class_VHttpd_PhpWorker_iterable, status i64, contentType string, mut var_headers Class_VHttpd_PhpWorker_array) rt.PhpVal {
	return create_vhttpd_phpworker_self(var_chunks.dup(), rt.new_int(status).dup(),
		var_headers.dup(), rt.new_string('text'), rt.new_string(contentType).dup())
}
...
fn Class_VHttpd_PhpWorker_StreamResponse.sse(mut var_events Class_VHttpd_PhpWorker_iterable, status i64, mut var_headers Class_VHttpd_PhpWorker_array) rt.PhpVal {
	return create_vhttpd_phpworker_self(var_events.dup(), rt.new_int(status).dup(),
		var_headers.dup(), rt.new_string('sse'), rt.new_string('text/event-stream'))
}
...
pub fn init_wp_content_plugins_v_profiler_src_vhttpd_phpworker_streamresponse_php() {
