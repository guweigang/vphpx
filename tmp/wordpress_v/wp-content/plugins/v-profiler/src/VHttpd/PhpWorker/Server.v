...
fn parseSocketFromArgv(var_argv rt.PhpVal) string {
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_arg, rt.new_string('--socket'))) && var_argv.array_isset(rt.add(var_index, rt.new_int(1))))) {
				return (// unsupported expression: Expr_Cast_String).str()
			}
			if rt.is_true(rt.call_function('str_starts_with', [// unsupported expression: Expr_Cast_String, rt.new_string('--socket=')])) {
				return (rt.call_function('substr', [// unsupported expression: Expr_Cast_String, rt.new_int('--socket='.len)])).str()
			}
		}
	}
	mut var_socket := rt.call_function('getenv', [rt.new_string('VHTTPD_WORKER_SOCKET')])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_socket.dup().is_string())) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
...
fn (mut this Class_VHttpd_PhpWorker_Server) construct(socketPath string, defaultApp string)  {
	...
	this.parentPid = if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_parentPid.dup().is_string())) && rt.is_true(rt.call_function('ctype_digit', [var_parentPid.dup()])))) { // unsupported expression: Expr_Cast_Int } else { rt.new_int(0) }
	mut var_appPath := rt.call_function('getenv', [rt.new_string('VHTTPD_APP')])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_appPath.dup().is_string()))))) || rt.is_true(rt.identical(var_appPath, rt.new_string(''))))) {
		var_appPath = rt.get_property(rt.new_object('VHttpd_PhpWorker_Server', []string{}, &this), 'defaultApp')
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_file', [var_appPath.dup()]))))) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_callable', [var_app.dup()]))))) {
...
fn (mut this Class_VHttpd_PhpWorker_Server) run()  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('file_exists', [rt.get_property(rt.new_object('VHttpd_PhpWorker_Server', []string{}, &this), 'socketPath')])) || rt.is_true(rt.call_function('is_link', [rt.get_property(rt.new_object('VHttpd_PhpWorker_Server', []string{}, &this), 'socketPath')])))) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('socket_bind', [var_server.dup(), rt.get_property(rt.new_object('VHttpd_PhpWorker_Server', []string{}, &this), 'socketPath')]))))) {
		mut var_error := rt.call_function('socket_strerror', [rt.call_function('socket_last_error', [var_server.dup()])])
		rt.call_function('socket_close', [var_server.dup()])
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('socket_listen', [var_server.dup()]))))) {
		var_error = rt.call_function('socket_strerror', [rt.call_function('socket_last_error', [var_server.dup()])])
		rt.call_function('socket_close', [var_server.dup()])
	...
	rt.call_function('socket_set_nonblock', [var_server.dup()])
	for !(this.parentprocessexited()) {
		mut var_conn := rt.call_function('socket_accept', [var_server.dup()])
		if rt.is_true(rt.new_bool(!(rt.is_true(var_conn)))) {
			if rt.is_true(Class_VHttpd_PhpWorker_Server.iswouldblock((rt.call_function('socket_last_error', [var_server.dup()])).to_i64())) {
				rt.call_function('socket_clear_error', [var_server.dup()])
				rt.call_function('usleep', [rt.new_int(200000)])
				continue
			}
			continue
		}
		rt.call_function('socket_set_block', [var_conn.dup()])
		this.handleconnection(var_conn.dup())
		rt.call_function('socket_close', [var_conn.dup()])
	}
	rt.call_function('socket_close', [var_server.dup()])
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('file_exists', [rt.get_property(rt.new_object('VHttpd_PhpWorker_Server', []string{}, &this), 'socketPath')])) || rt.is_true(rt.call_function('is_link', [rt.get_property(rt.new_object('VHttpd_PhpWorker_Server', []string{}, &this), 'socketPath')])))) {
		rt.call_function('unlink', [rt.get_property(rt.new_object('VHttpd_PhpWorker_Server', []string{}, &this), 'socketPath')])
...
fn (mut this Class_VHttpd_PhpWorker_Server) parentprocessexited() bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.less_equal(this.parentPid, rt.new_int(0))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('posix_kill')]))))))) {
		return false
	}
...
fn Class_VHttpd_PhpWorker_Server.iswouldblock(error i64) bool {
	...
	{
		mut iter_1 := rt.create_array([rt.ArrayItem{ key: none, val: 'SOCKET_EAGAIN' }, rt.ArrayItem{ key: none, val: 'SOCKET_EWOULDBLOCK' }]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_constant := item_1.val
			if rt.is_true(rt.call_function('defined', [var_constant.dup()])) {
				var_wouldBlock.array_push(rt.call_function('constant', [var_constant.dup()]))
			}
		}
	}
	return error_mutated == 0 || rt.is_true(rt.call_function('in_array', [rt.new_int(error_mutated).dup(), var_wouldBlock.dup(), rt.new_bool(true)]))
}

fn (mut this Class_VHttpd_PhpWorker_Server) handleconnection(var_conn rt.PhpVal)  {
	mut var_conn_mutated := var_conn
	mut var_raw := Class_VHttpd_PhpWorker_Server.readframe(var_conn_mutated.dup())
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_payload := rt.call_function('json_decode', [var_raw.dup(), rt.new_bool(true), rt.new_int(512), rt.get_constant('JSON_THROW_ON_ERROR')])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_payload.dup().is_array()))))) {
	...
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_result := rt.call_callable(this.app, [var_payload.dup(), var_payload.dup()])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	if rt.is_true(rt.new_bool(rt.instance_of(var_result, 'VHttpd_PhpWorker_StreamResponse'))) {
		Class_VHttpd_PhpWorker_Server.writestreamresponse(mut rt.cast_object_ptr[Class_VHttpd_PhpWorker_array](var_conn_mutated), mut rt.cast_object_ptr[Class_VHttpd_PhpWorker_StreamResponse](var_payload), var_result.dup())
		if rt.has_exception() { unsafe { goto catch_label_1 } }
		return rt.new_null()
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	Class_VHttpd_PhpWorker_Server.writeframe((var_conn_mutated).str(), rt.new_string(rt.json_encode(Class_VHttpd_PhpWorker_Server.normalizeresponse(mut rt.cast_object_ptr[Class_VHttpd_PhpWorker_array](var_payload), mut rt.cast_object_ptr[Class_VHttpd_PhpWorker_mixed](var_result)))))
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Throwable') {
		...
		Class_VHttpd_PhpWorker_Server.writeframe((var_conn_mutated).str(), rt.new_string(rt.json_encode(Class_VHttpd_PhpWorker_Server.errorresponse(mut rt.cast_object_ptr[Class_Throwable](var_e)))))
...
fn Class_VHttpd_PhpWorker_Server.readframe(var_conn rt.PhpVal) string {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.less_equal(var_size, rt.new_int(0))) || rt.is_true(rt.greater(var_size, 16 * 1024 * 1024)))) {
	...
	return (Class_VHttpd_PhpWorker_Server.readexact((var_conn_mutated).to_i64(), var_size.dup())).str()
}
...
fn Class_VHttpd_PhpWorker_Server.readexact(var_conn rt.PhpVal, size i64) string {
	...
	for var_buf.dup().to_string().len < size_mutated {
		mut var_chunk := rt.call_function('socket_read', [var_conn_mutated.dup(), size_mutated - var_buf.dup().to_string().len, rt.get_constant('PHP_BINARY_READ')])
...
fn Class_VHttpd_PhpWorker_Server.writeframe(var_conn rt.PhpVal, payload string)  {
	...
	for rt.is_true(rt.less(var_written, var_size)) {
		mut var_n := rt.call_function('socket_write', [var_conn_mutated.dup(), rt.call_function('substr', [var_data.dup(), var_written.dup()]), rt.sub(var_size, var_written)])
...
fn Class_VHttpd_PhpWorker_Server.normalizeresponse(mut var_request Class_VHttpd_PhpWorker_array, mut var_result Class_VHttpd_PhpWorker_mixed) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.instance_of(var_result_mutated, 'VHttpd_PhpWorker_Response'))) {
		...
		if rt.is_true(rt.identical(// unsupported expression: Expr_Cast_String, rt.new_string(''))) {
			var_payload.array_set('id', // unsupported expression: Expr_Cast_String)
		}
		return var_payload.dup()
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_result_mutated.dup().is_array()))))) {
		var_result_mutated = rt.create_array([rt.ArrayItem{ key: 'status', val: 200 }, rt.ArrayItem{ key: 'body', val: // unsupported expression: Expr_Cast_String }])
	}
	mut var_headers := Class_VHttpd_PhpWorker_Server.normalizeheaders(mut rt.cast_object_ptr[Class_VHttpd_PhpWorker_mixed](if !(var_result_mutated.array_get('headers')).is_null() { var_result_mutated.array_get('headers') } else { rt.new_array() }))
	mut var_contentType := // unsupported expression: Expr_Cast_String
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && !(var_headers.array_isset(rt.new_string('content-type'))))) {
		var_headers.array_set('content-type', var_contentType.dup())
	}
	return rt.create_array([rt.ArrayItem{ key: 'id', val: // unsupported expression: Expr_Cast_String }, rt.ArrayItem{ key: 'status', val: // unsupported expression: Expr_Cast_Int }, rt.ArrayItem{ key: 'headers', val: var_headers }, rt.ArrayItem{ key: 'body', val: // unsupported expression: Expr_Cast_String }])
}
...
fn Class_VHttpd_PhpWorker_Server.writestreamresponse(var_conn rt.PhpVal, mut var_request Class_VHttpd_PhpWorker_array, mut var_response Class_VHttpd_PhpWorker_StreamResponse)  {
	...
	Class_VHttpd_PhpWorker_Server.writejsonframe(mut rt.cast_object_ptr[Class_VHttpd_PhpWorker_array](var_conn_mutated), rt.create_array([rt.ArrayItem{ key: 'id', val: // unsupported expression: Expr_Cast_String }, rt.ArrayItem{ key: 'mode', val: 'stream' }, rt.ArrayItem{ key: 'event', val: 'end' }, rt.ArrayItem{ key: 'stream_type', val: rt.get_property(var_response, 'streamType') }]))
}
...
fn Class_VHttpd_PhpWorker_Server.streamchunkframe(mut var_request Class_VHttpd_PhpWorker_array, mut var_response Class_VHttpd_PhpWorker_StreamResponse, mut var_chunk Class_VHttpd_PhpWorker_mixed) rt.PhpVal {
	...
	mut var_frame := rt.create_array([rt.ArrayItem{ key: 'id', val: // unsupported expression: Expr_Cast_String }, rt.ArrayItem{ key: 'mode', val: 'stream' }, rt.ArrayItem{ key: 'event', val: 'chunk' }, rt.ArrayItem{ key: 'stream_type', val: rt.get_property(var_response, 'streamType') }])
	if rt.is_true(rt.identical(rt.get_property(var_response, 'streamType'), rt.new_string('sse'))) {
		if rt.is_true(rt.new_bool(var_chunk_mutated.dup().is_array())) {
			var_frame.array_set('data', // unsupported expression: Expr_Cast_String)
			var_frame.array_set('sse_id', // unsupported expression: Expr_Cast_String)
			var_frame.array_set('sse_event', // unsupported expression: Expr_Cast_String)
			mut var_retry := 
...
fn create_vhttpd_phpworker_runtimeexception() &Class_VHttpd_PhpWorker_RuntimeException {
...
pub fn init_wp_content_plugins_v_profiler_src_vhttpd_phpworker_server_php() {
