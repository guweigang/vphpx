...
fn main() {
	...
	mut var_cat_id := if !(!rt.is_true(rt.get_superglobal('_REQUEST').array_get('cat_id'))) { rt.call_function('absint', [
			rt.get_superglobal('_REQUEST').array_get('cat_id'),
		]) } else { rt.new_int(0) }
	mut var_link_id := if !(!rt.is_true(rt.get_superglobal('_REQUEST').array_get('link_id'))) { rt.call_function('absint', [
			rt.get_superglobal('_REQUEST').array_get('link_id'),
		]) } else { rt.new_int(0) }
