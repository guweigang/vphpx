...
fn main() {
	...
	mut var_etag := var_wp_scripts.get_etag(var_load.dup())
	...
	}
	{
		...
		for {
			...
			mut var_path := rt.new_string(rt.concat(rt.get_constant('ABSPATH'), rt.get_property(rt.get_property(var_wp_scripts,
				'registered').array_get(var_handle), 'src')))
