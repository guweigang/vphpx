...
fn main() {
	...
	rt.include_file(@DIR + '/includes/network.php', '4')
	{
		...
		for {
			...
			rt.set_property(var_wpdb, '{"nodeType":"Expr_Variable","line":37,"name":"table"}',
				var_prefixed_table.dup())
		}
	}
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('network_domain_check', []rt.PhpVal{})))))
		&& rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('WP_ALLOW_MULTISITE')])))))
	...
	if rt.is_true(rt.get_superglobal('_POST')) {
		...
		mut var_subdomain_install := if rt.is_true(rt.call_function('allow_subdomain_install',
			[]rt.PhpVal{}))
		{
			!(!rt.is_true(rt.get_superglobal('_POST').array_get('subdomain_install')))
		} else {
			false
		}
			...
			[]rt.PhpVal{})))))
		{
			mut var_result := rt.call_function('populate_network', [
				...
				rt.call_function('sanitize_email', [rt.get_superglobal('_POST').array_get('email')]),
				rt.call_function('wp_unslash', [rt.get_superglobal('_POST').array_get('sitename')]),
				var_base.dup(), rt.new_bool(var_subdomain_install).dup()])
			if rt.is_true(rt.call_function('is_wp_error', [var_result.dup()])) {
				if rt.is_true(rt.new_bool(
					1 == rt.call_method(var_result, 'get_error_codes', []rt.PhpVal{}).array_count()
					&& rt.is_true(rt.identical(rt.new_string('no_wildcard_dns'), rt.call_method(var_result, 'get_error_code', []rt.PhpVal{})))))
				{
					rt.call_function('network_step2', [var_result.dup()])
				} else {
					rt.call_function('network_step1', [var_result.dup()])
				}
			} else {
