...
fn main() {
	...
	mut var_title := rt.call_function('strip_tags', [var_title.dup()])
	if rt.is_true(rt.call_function('is_network_admin', []rt.PhpVal{})) {
		mut var_admin_title := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Network Admin: %s')]), rt.get_property(rt.call_function('get_network', []rt.PhpVal{}), 'site_name')])
	} else if rt.is_true(rt.call_function('is_user_admin', []rt.PhpVal{})) {
		var_admin_title = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('User Dashboard: %s')]), rt.get_property(rt.call_function('get_network', []rt.PhpVal{}), 'site_name')])
	} else {
		var_admin_title = rt.call_function('get_bloginfo', [rt.new_string('name')])
	}
	if rt.is_true(rt.identical(var_admin_title, var_title)) {
		var_admin_title = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('%s &#8212; WordPress')]), var_title.dup()])
	} else {
		mut var_screen_title := var_title.dup()
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('post'), rt.get_property(var_current_screen, 'base'))) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
			mut var_post_title := rt.call_function('get_the_title', []rt.PhpVal{})
			if !(!rt.is_true(var_post_title)) {
				mut var_post_type_obj := rt.call_function('get_post_type_object', [var_typenow.dup()])
				var_screen_title = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('%1$s &#8220;%2$s&#8221;')]), rt.get_property(rt.get_property(var_post_type_obj, 'labels'), 'edit_item'), var_post_title.dup()])
			}
		}
		var_admin_title = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('%1$s &lsaquo; %2$s &#8212; WordPress')]), var_screen_title.dup(), var_admin_title.dup()])
	}
	if rt.is_true(rt.call_function('wp_is_recovery_mode', []rt.PhpVal{})) {
		var_admin_title = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Recovery Mode &#8212; %s')]), var_admin_title.dup()])
	}
	var_admin_title = rt.call_function('apply_filters', [rt.new_string('admin_title'), var_admin_title.dup(), var_title.dup()])
	...
	mut var_admin_body_class := rt.call_function('preg_replace', [rt.new_string('/[^a-z0-9_-]+/i'), rt.new_string('-'), var_hook_suffix.dup()])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_js', [rt.call_function('admin_url', [rt.new_string('admin-ajax.php'), rt.new_string('relative')])]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_js', [rt.get_property(var_current_screen, 'id')]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_js', [rt.get_property(var_current_screen, 'post_type')]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_js', [var_admin_body_class.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_js', [rt.get_property(var_wp_locale, 'number_format').array_get('thousands_sep')]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_js', [rt.get_property(var_wp_locale, 'number_format').array_get('decimal_point')]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(// unsupported expression: Expr_Cast_Int)
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [rt.new_string('admin_enqueue_scripts'), var_hook_suffix.dup()])
	rt.call_function('do_action', [rt.new_string("admin_print_styles-${var_hook_suffix.to_string()}")])
	rt.call_function('do_action', [rt.new_string('admin_print_styles')])
	rt.call_function('do_action', [rt.new_string("admin_print_scripts-${var_hook_suffix.to_string()}")])
	rt.call_function('do_action', [rt.new_string('admin_print_scripts')])
	rt.call_function('do_action', [rt.new_string("admin_head-${var_hook_suffix.to_string()}")])
	rt.call_function('do_action', [rt.new_string('admin_head')])
	if rt.is_true(rt.identical(rt.new_string('f'), rt.call_function('get_user_setting', [rt.new_string('mfold')]))) {
		// unsupported expression: Expr_AssignOp_Concat
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('get_user_setting', [rt.new_string('unfold')]))))) {
		// unsupported expression: Expr_AssignOp_Concat
	}
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_error_get_last) && rt.is_true(rt.get_constant('WP_DEBUG')))) && rt.is_true(rt.get_constant('WP_DEBUG_DISPLAY')))) && rt.is_true(rt.call_function('ini_get', [rt.new_string('display_errors')])))) && rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))))) {
		// unsupported expression: Expr_AssignOp_Concat
	}
	var_error_get_last = rt.new_null()
	// unsupported statement: Stmt_InlineHTML
	mut var_admin_body_classes := rt.call_function('apply_filters', [rt.new_string('admin_body_class'), rt.new_string('')])
	var_admin_body_classes = rt.new_string(rt.new_string((var_admin_body_classes).str() + ' ' + (var_admin_body_class).str().trim_left(' \t\n\r')))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_admin_body_classes.dup()]))
	// unsupported statement: Stmt_InlineHTML
	...
	rt.call_method(var_current_screen, 'set_parentage', [var_parent_file.dup()])
	// unsupported statement: Stmt_InlineHTML
