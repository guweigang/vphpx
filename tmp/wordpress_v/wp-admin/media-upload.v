...
fn main() {
	...
	mut var_ID := if !(var_ID).is_null() { // unsupported expression: Expr_Cast_Int } else { rt.new_int(0) }
	mut var_post_id := if !(var_post_id).is_null() { // unsupported expression: Expr_Cast_Int } else { rt.new_int(0) }
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(var_action).is_null() && rt.is_true(rt.identical(rt.new_string('edit'), var_action)))) && rt.is_true(rt.new_bool(!(rt.is_true(var_ID)))))) {
		rt.call_function('wp_die', ['<h1>' + (rt.call_function('__', [rt.new_string('An error occurred during the upload process.')])).str() + '</h1>' + '<p>' + (rt.call_function('__', [rt.new_string('Invalid item ID. You can view all media items in the <a href="upload.php">Media Library</a>.')])).str() + '</p>', rt.new_int(403)])
	}
	if rt.is_true(rt.new_bool(!(!rt.is_true(rt.get_superglobal('_REQUEST').array_get('post_id'))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('edit_post'), rt.get_superglobal('_REQUEST').array_get('post_id')]))))))) {
	...
	if rt.get_superglobal('_GET').array_isset(rt.new_string('type')) {
		mut var_type := // unsupported expression: Expr_Cast_String
	} else {
	...
	if rt.get_superglobal('_GET').array_isset(rt.new_string('tab')) {
		mut var_tab := // unsupported expression: Expr_Cast_String
	} else {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('type'), var_tab)) || rt.is_true(rt.identical(rt.new_string('type_url'), var_tab)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.call_function('media_upload_tabs', []rt.PhpVal{}).array_isset(var_tab.dup())))))))) {
