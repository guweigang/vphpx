...
fn main() {
	...
	mut var_load := rt.get_superglobal('_GET').array_get('load')
	if rt.is_true(rt.new_bool(var_load.dup().is_array())) {
		rt.call_function('ksort', [var_load.dup()])
		var_load = rt.call_function('implode', [rt.new_string(''),
			var_load.dup()])
	}
	var_load = rt.call_function('preg_replace', [rt.new_string('/[^a-z0-9,_-]+/i'),
		rt.new_string(''), var_load.dup()])
	var_load = rt.call_function('array_unique', [
		rt.call_function('explode', [rt.new_string(','), var_load.dup()]),
	])
	...
	mut var_rtl := rt.get_superglobal('_GET').array_isset(rt.new_string('dir'))
		&& rt.is_true(rt.identical(rt.new_string('rtl'), rt.get_superglobal('_GET').array_get('dir')))
	mut var_expires_offset := 31536000
	mut var_out := ''
	mut var_wp_styles := create_wp_styles()
	rt.call_function('wp_default_styles', [var_wp_styles])
	mut var_etag := var_wp_styles.get_etag(var_load.dup())
	if rt.is_true(rt.new_bool(
		rt.get_superglobal('_SERVER').array_isset(rt.new_string('HTTP_IF_NONE_MATCH'))
		&& rt.is_true(rt.identical(rt.call_function('stripslashes', [rt.get_superglobal('_SERVER').array_get('HTTP_IF_NONE_MATCH')]), var_etag))))
	{
		rt.call_function('header', [
	...
	}
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_property(var_wp_styles,
				'registered').array_isset(var_handle.dup()))))))
			{
				continue
			}
			...
			mut var_path := rt.new_string(rt.concat(rt.get_constant('ABSPATH'), rt.get_property(var_style,
				'src')))
			if var_rtl && !(!rt.is_true(rt.get_property(var_style, 'extra').array_get('rtl'))) {
				var_path = rt.call_function('str_replace', [rt.new_string('.min.css'),
					rt.new_string('-rtl.min.css'), var_path.dup()])
			}
			mut var_content := rt.new_string(
				(rt.call_function('get_file', [var_path.dup()])).str() + '\n')
			if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
				rt.get_property(var_style, 'src'),
				'/' + global_const_wpinc + '/css/',
			])))
			{
				var_content = rt.call_function('str_replace', [
					rt.new_string('../images/'),
					'../' + global_const_wpinc + '/images/',
					var_content.dup(),
				])
				var_content = rt.call_function('str_replace', [
					rt.new_string('../js/tinymce/'),
					'../' + global_const_wpinc + '/js/tinymce/',
					var_content.dup(),
				])
				var_content = rt.call_function('str_replace', [
					rt.new_string('../fonts/'),
					'../' + global_const_wpinc + '/fonts/',
					var_content.dup(),
				])
