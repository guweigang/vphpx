...
fn main() {
	...
	rt.include_file((rt.call_function('dirname', [rt.new_string(@DIR)])).str() + '/wp-load.php', '3')
	...
	mut var_step := if !(rt.get_superglobal('_GET').array_get('step')).is_null() { rt.get_superglobal('_GET').array_get('step') } else { rt.new_int(0) }
	if rt.is_true(rt.identical(rt.new_string('upgrade_db'), var_step)) {
		rt.call_function('wp_upgrade', []rt.PhpVal{})
		// unsupported expression: Expr_Exit
	}
	// unsupported statement: Stmt_Global
	var_step = // unsupported expression: Expr_Cast_Int
	mut var_php_version := rt.get_constant('PHP_VERSION')
	mut var_mysql_version := rt.call_method(var_wpdb, 'db_version', []rt.PhpVal{})
	mut var_php_compat := rt.call_function('version_compare', [var_php_version.dup(), var_required_php_version.dup(), rt.new_string('>=')])
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('file_exists', [(rt.get_constant('WP_CONTENT_DIR')).str() + '/db.php'])) && !rt.is_true(rt.get_property(var_wpdb, 'is_mysql')))) {
		...
		var_mysql_compat = rt.call_function('version_compare', [var_mysql_version.dup(), var_required_mysql_version.dup(), rt.new_string('>=')])
	}
	...
	if rt.is_true(rt.new_bool(!(var_required_php_extensions).is_null() && rt.is_true(rt.new_bool(var_required_php_extensions.dup().is_array())))) {
		{
			...
			for {
				...
				var_missing_extensions << rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('You cannot upgrade because <a href="%1$s">WordPress %2$s</a> requires the %3$s PHP extension.')]), var_version_url.dup(), var_wp_version.dup(), var_extension.dup()])
			}
		}
	}
	...
	// unsupported statement: Stmt_InlineHTML
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(// unsupported expression: Expr_Cast_Int, var_wp_db_version)) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_blog_installed', []rt.PhpVal{}))))))) {
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('_e', [rt.new_string('No Update Required')])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('_e', [rt.new_string('Your WordPress database is already up to date!')])
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_url', [rt.call_function('get_option', [rt.new_string('home')])]))
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('_e', [rt.new_string('Continue')])
		// unsupported statement: Stmt_InlineHTML
	} else if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_php_compat)))) || rt.is_true(rt.new_bool(!(rt.is_true(var_mysql_compat)))))) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_mysql_compat)))) && rt.is_true(rt.new_bool(!(rt.is_true(var_php_compat)))))) {
			mut var_message := rt.new_string(rt.concat(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('You cannot update because <a href="%1$s">WordPress %2$s</a> requires PHP version %3$s or higher and MySQL version %4$s or higher. You are running PHP version %5$s and MySQL version %6$s.')]), var_version_url.dup(), var_wp_version.dup(), var_required_php_version.dup(), var_required_mysql_version.dup(), var_php_version.dup(), var_mysql_version.dup()]), var_php_update_message))
		} else if rt.is_true(rt.new_bool(!(rt.is_true(var_php_compat)))) {
			var_message = rt.new_string(rt.concat(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('You cannot update because <a href="%1$s">WordPress %2$s</a> requires PHP version %3$s or higher. You are running version %4$s.')]), var_version_url.dup(), var_wp_version.dup(), var_required_php_version.dup(), var_php_version.dup()]), var_php_update_message))
		} else if rt.is_true(rt.new_bool(!(rt.is_true(var_mysql_compat)))) {
			var_message = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('You cannot update because <a href="%1$s">WordPress %2$s</a> requires MySQL version %3$s or higher. You are running version %4$s.')]), var_version_url.dup(), var_wp_version.dup(), var_required_mysql_version.dup(), var_mysql_version.dup()])
		}
		print('<p>' + (var_message).str() + '</p>')
	} else if var_missing_extensions.len > 0 {
		print('<p>' + (rt.call_function('implode', [rt.new_string('</p><p>'), var_missing_extensions.dup()])).str() + '</p>')
	} else {
		...
		if rt.is_true(rt.equal(switch_val_1, rt.new_int(0))) {
			...
			if rt.is_true(var_goback) {
				var_goback = rt.call_function('sanitize_url', [var_goback.dup()])
				var_goback = rt.call_function('urlencode', [var_goback.dup()])
			}
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('_e', [rt.new_string('Database Update Required')])
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('_e', [rt.new_string('WordPress has been updated! Next and final step is to update your database to the newest version.')])
			// unsupported statement: Stmt_InlineHTML
			...
			rt.call_function('wp_upgrade', []rt.PhpVal{})
			mut var_backto := if !(!rt.is_true(rt.get_superglobal('_GET').array_get('backto'))) { rt.call_function('wp_unslash', [rt.call_function('urldecode', [rt.get_superglobal('_GET').array_get('backto')])]) } else { (rt.call_function('__get_option', [rt.new_string('home')])).str() + '/' }
			var_backto = rt.call_function('esc_url', [var_backto.dup()])
			var_backto = rt.call_function('wp_validate_redirect', [var_backto.dup(), (rt.call_function('__get_option', [rt.new_string('home')])).str() + '/'])
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('_e', [rt.new_string('Update Complete')])
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('_e', [rt.new_string('Your WordPress database has been successfully updated!')])
			// unsupported statement: Stmt_InlineHTML
