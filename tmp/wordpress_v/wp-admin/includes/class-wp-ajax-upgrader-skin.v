...
fn (mut this Class_WP_Ajax_Upgrader_Skin) construct(var_args rt.PhpVal) {
	this.Class_Automatic_Upgrader_Skin.construct(var_args.dup())
...
fn (mut this Class_WP_Ajax_Upgrader_Skin) get_error_messages() rt.PhpVal {
	mut var_messages := []rt.PhpVal{}
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(var_error_data)
				&& rt.is_true(rt.new_bool(var_error_data.dup().is_string()))))
			{
				var_messages <<
					(rt.call_method(this.errors, 'get_error_message', [var_error_code.dup()])).str() +
					' ' +(rt.call_function('esc_html', [rt.call_function('strip_tags', [var_error_data.dup()])])).str()
			} else {
	...
	return rt.call_function('implode', [rt.new_string(', '), var_messages.dup()])
}
...
fn (mut this Class_WP_Ajax_Upgrader_Skin) error(var_errors rt.PhpVal, var_args rt.PhpVal) {
	if rt.is_true(rt.new_bool(var_errors.dup().is_string())) {
		...
		mut var_errors_count := rt.new_int(rt.new_int(rt.call_method(this.errors,
			'get_error_codes', []rt.PhpVal{}).array_count()))
		rt.call_method(this.errors, 'add', [
			'unknown_upgrade_error_' + (rt.add(var_errors_count, rt.new_int(1))).str(),
			var_string.dup(),
		])
	} else if rt.is_true(rt.call_function('is_wp_error', [var_errors.dup()])) {
		{
			mut iter_1 := rt.call_method(var_errors, 'get_error_codes', []rt.PhpVal{}).iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_error_code := item_1.val
				rt.call_method(this.errors, 'add', [var_error_code.dup(),
	...
	this.Class_Automatic_Upgrader_Skin.error(var_errors.dup(), var_args.dup())
}
...
fn (mut this Class_WP_Ajax_Upgrader_Skin) feedback(var_feedback rt.PhpVal, var_args rt.PhpVal) {
	if rt.is_true(rt.call_function('is_wp_error', [var_feedback.dup()])) {
		{
			mut iter_1 := rt.call_method(var_feedback, 'get_error_codes', []rt.PhpVal{}).iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_error_code := item_1.val
				rt.call_method(this.errors, 'add', [var_error_code.dup(),
	...
	this.Class_Automatic_Upgrader_Skin.feedback(var_feedback.dup(), var_args.dup())
}
...
pub fn init_wp_admin_includes_class_wp_ajax_upgrader_skin_php() {
}
