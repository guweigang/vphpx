...
fn Class_WP_Internal_Pointers.enqueue_scripts(var_hook_suffix rt.PhpVal)  {
	...
	mut var_dismissed := rt.call_function('explode', [rt.new_string(','), // unsupported expression: Expr_Cast_String])
	mut var_got_pointers := rt.new_bool(rt.new_bool(false))
	{
		mut iter_1 := rt.call_function('array_diff', [var_pointers.dup(), var_dismissed.dup()]).iterator()
		for {
			...
			if var_caps_required.array_isset(var_pointer) {
				{
					...
					for {
						...
						if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [var_cap.dup()]))))) {
							continue
						}
					}
				}
			}
			rt.call_function('add_action', [rt.new_string('admin_print_footer_scripts'), rt.create_array([rt.ArrayItem{ key: none, val: 'WP_Internal_Pointers' }, rt.ArrayItem{ key: none, val: 'pointer_' + (var_pointer).str() }])])
			var_got_pointers = rt.new_bool(rt.new_bool(true))
		}
	}
...
fn Class_WP_Internal_Pointers.print_js(var_pointer_id rt.PhpVal, var_selector rt.PhpVal, var_args rt.PhpVal)  {
	if !rt.is_true(var_pointer_id) || !rt.is_true(var_selector) || !rt.is_true(var_args) || !rt.is_true(var_args.array_get('content')) {
		return rt.new_null()
	}
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wp_json_encode', [var_args.dup(), rt.bitwise_or(rt.get_constant('JSON_HEX_TAG'), rt.get_constant('JSON_UNESCAPED_SLASHES'))]))
	// unsupported statement: Stmt_InlineHTML
...
fn Class_WP_Internal_Pointers.dismiss_pointers_for_new_users(var_user_id rt.PhpVal)  {
	rt.call_function('add_user_meta', [var_user_id.dup(), rt.new_string('dismissed_wp_pointers'), rt.new_string('')])
}

fn create_wp_internal_pointers() &Class_WP_Internal_Pointers {
...
pub fn init_wp_admin_includes_class_wp_internal_pointers_php() {
}
