...
fn (mut this Class__WP_List_Table_Compat) construct(var_screen rt.PhpVal, var_columns rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(var_screen_mutated.dup().is_string())) {
		var_screen_mutated = rt.call_function('convert_to_screen', [
			var_screen_mutated.dup()])
	}
	this._screen = var_screen_mutated.dup()
	if !(!rt.is_true(var_columns_mutated)) {
		this._columns = var_columns_mutated.dup()
		rt.call_function('add_filter', [
			'manage_' + (rt.get_property(var_screen_mutated, 'id')).str() + '_columns',
			rt.create_array([
...
pub fn init_wp_admin_includes_class_wp_list_table_compat_php() {
}
