...
fn (mut this Class_WP_Application_Passwords_List_Table) display_tablenav(var_which rt.PhpVal) {
	...
	rt.echo_val(rt.call_function('esc_attr', [var_which.dup()]))
	// unsupported statement: Stmt_InlineHTML
...
fn (mut this Class_WP_Application_Passwords_List_Table) single_row(var_item rt.PhpVal) {
	print('<tr data-uuid="' + (rt.call_function('esc_attr', [var_item.array_get('uuid')])).str() +
		'">')
...
fn (mut this Class_WP_Application_Passwords_List_Table) print_js_template_row() {
	...
	print('<tr data-uuid="{{ data.uuid }}">')
	{
		...
		for {
			...
			mut var_classes :=
				rt.new_string(rt.new_string('${var_column_name.to_string()} column-${var_column_name.to_string()}'))
			if rt.is_true(var_is_primary) {
				// unsupported expression: Expr_AssignOp_Concat
			}
			if rt.is_true(rt.call_function('in_array', [var_column_name.dup(),
				var_hidden.dup(), rt.new_bool(true)]))
			{
				// unsupported expression: Expr_AssignOp_Concat
			}
			rt.call_function('printf', [
				rt.new_string('<td class="%s" data-colname="%s">'),
				rt.call_function('esc_attr', [var_classes.dup()]),
				rt.call_function('esc_attr', [
					rt.call_function('wp_strip_all_tags', [var_display_name.dup()]),
				]),
			])
...
fn create_wp_application_passwords_list_table() &Class_WP_Application_Passwords_List_Table {
...
pub fn init_wp_admin_includes_class_wp_application_passwords_list_table_php() {
}
