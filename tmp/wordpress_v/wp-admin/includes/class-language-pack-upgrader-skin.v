...
fn (mut this Class_Language_Pack_Upgrader_Skin) construct(var_args rt.PhpVal) {
	...
	var_args_mutated = rt.call_function('wp_parse_args', [var_args_mutated.dup(),
		var_defaults.dup()])
	if rt.is_true(var_args_mutated.array_get('skip_header_footer')) {
...
fn (mut this Class_Language_Pack_Upgrader_Skin) error(var_errors rt.PhpVal) {
	...
	this.Class_WP_Upgrader_Skin.error(var_errors.dup())
	print('</div>')
}
...
fn (mut this Class_Language_Pack_Upgrader_Skin) bulk_footer() {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_update_actions)
		&& rt.is_true(this.display_footer_actions)))
	{
...
pub fn init_wp_admin_includes_class_language_pack_upgrader_skin_php() {
}
