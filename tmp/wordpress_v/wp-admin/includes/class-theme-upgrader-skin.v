...
fn (mut this Class_Theme_Upgrader_Skin) construct(var_args rt.PhpVal) {
	...
	var_args_mutated = rt.call_function('wp_parse_args', [var_args_mutated.dup(),
		var_defaults.dup()])
	this.theme = var_args_mutated.array_get('theme')
...
fn (mut this Class_Theme_Upgrader_Skin) after() {
	...
	if rt.is_true(var_theme_info) {
		...
		mut var_stylesheet := rt.get_property(rt.get_property(rt.new_object('Theme_Upgrader_Skin', [
			'WP_Upgrader_Skin',
		], &this), 'upgrader'), 'result').array_get('destination_name')
		...
		var_activate_link = rt.call_function('wp_nonce_url', [
			var_activate_link.dup(), 'switch-theme_' + var_stylesheet.str()])
			...
			var_stylesheet))
		{
				...
				&& rt.is_true(rt.call_function('current_user_can', [rt.new_string('customize')]))))
			{
				var_update_actions.array_set('preview', rt.call_function('sprintf', [
					...
					rt.call_function('esc_url', [var_customize_url.dup()]),
		...
		]))
		{
			...
			var_update_actions.array_set('activate', rt.call_function('sprintf', [
				'<a href="%s" class="activatelink">' +
					'<span aria-hidden="true">%s</span><span class="screen-reader-text">%s</span></a>',
				rt.call_function('esc_url', [var_activate_link.dup()]),
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.get_property(rt.new_object('Theme_Upgrader_Skin', ['WP_Upgrader_Skin'], &this), 'result')))))
...
pub fn init_wp_admin_includes_class_theme_upgrader_skin_php() {
}
