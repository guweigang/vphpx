...
fn main() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('manage_sites')]))))) {
		rt.call_function('wp_die', [rt.call_function('__', [rt.new_string('Sorry, you are not allowed to manage themes for this site.')])])
	}
	...
	rt.call_method(rt.call_function('get_current_screen', []rt.PhpVal{}), 'set_screen_reader_content', [rt.create_array([rt.ArrayItem{ key: 'heading_views', val: rt.call_function('__', [rt.new_string('Filter site themes list')]) }, rt.ArrayItem{ key: 'heading_pagination', val: rt.call_function('__', [rt.new_string('Site themes list navigation')]) }, rt.ArrayItem{ key: 'heading_list', val: rt.call_function('__', [rt.new_string('Site themes list')]) }])])
	...
	if rt.is_true(var_action) {
		...
		mut var_allowed_themes := rt.call_function('get_option', [rt.new_string('allowedthemes')])
		...
		if rt.is_true(rt.equal(switch_val_1, rt.new_string('enable'))) {
			...
			mut var_theme := rt.get_superglobal('_GET').array_get('theme')
			var_action = rt.new_string(rt.new_string('enabled'))
			mut var_n := rt.new_int(rt.new_int(1))
			if rt.is_true(rt.new_bool(!(rt.is_true(var_allowed_themes)))) {
				var_allowed_themes = rt.create_array([rt.ArrayItem{ key: var_theme, val: true }])
			} else {
			...
			var_theme = rt.get_superglobal('_GET').array_get('theme')
			var_action = rt.new_string(rt.new_string('disabled'))
			var_n = rt.new_int(rt.new_int(1))
			if rt.is_true(rt.new_bool(!(rt.is_true(var_allowed_themes)))) {
			...
			if rt.get_superglobal('_POST').array_isset(rt.new_string('checked')) {
				mut var_themes := rt.cast_array(rt.get_superglobal('_POST').array_get('checked'))
				var_action = rt.new_string(rt.new_string('enabled'))
				var_n = rt.new_int(rt.new_int(var_themes.dup().array_count()))
				{
				...
				var_action = rt.new_string(rt.new_string('error'))
				var_n = rt.new_string(rt.new_string('none'))
			}
			...
			if rt.get_superglobal('_POST').array_isset(rt.new_string('checked')) {
				...
				var_referer = rt.call_function('apply_filters', [rt.new_string("handle_network_bulk_actions-${var_screen.to_string()}"), var_referer.dup(), var_action.dup(), var_themes.dup(), var_id.dup()])
		...
		rt.call_function('update_option', [rt.new_string('allowedthemes'), var_allowed_themes.dup(), rt.new_bool(false)])
		rt.call_function('restore_current_blog', []rt.PhpVal{})
		rt.call_function('wp_safe_redirect', [rt.call_function('add_query_arg', [rt.create_array([rt.ArrayItem{ key: 'id', val: var_id }, rt.ArrayItem{ key: var_action, val: var_n }]), var_referer.dup()])])
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('network_edit_site_nav', [rt.create_array([rt.ArrayItem{ key: 'blog_id', val: var_id }, rt.ArrayItem{ key: 'selected', val: 'site-themes' }])])
	if rt.get_superglobal('_GET').array_isset(rt.new_string('enabled')) {
		mut var_enabled := rt.call_function('absint', [rt.get_superglobal('_GET').array_get('enabled')])
		if rt.is_true(rt.identical(rt.new_int(1), var_enabled)) {
			mut var_message := rt.call_function('__', [rt.new_string('Theme enabled.')])
		} else {
			var_message = rt.call_function('_n', [rt.new_string('%s theme enabled.'), rt.new_string('%s themes enabled.'), var_enabled.dup()])
		}
		rt.call_function('wp_admin_notice', [rt.call_function('sprintf', [var_message.dup(), rt.call_function('number_format_i18n', [var_enabled.dup()])]), rt.create_array([rt.ArrayItem{ key: 'type', val: 'success' }, rt.ArrayItem{ key: 'dismissible', val: true }, rt.ArrayItem{ key: 'id', val: 'message' }])])
	} else if rt.get_superglobal('_GET').array_isset(rt.new_string('disabled')) {
		mut var_disabled := rt.call_function('absint', [rt.get_superglobal('_GET').array_get('disabled')])
		if rt.is_true(rt.identical(rt.new_int(1), var_disabled)) {
			var_message = rt.call_function('__', [rt.new_string('Theme disabled.')])
		} else {
			var_message = rt.call_function('_n', [rt.new_string('%s theme disabled.'), rt.new_string('%s themes disabled.'), var_disabled.dup()])
		}
		rt.call_function('wp_admin_notice', [rt.call_function('sprintf', [var_message.dup(), rt.call_function('number_format_i18n', [var_disabled.dup()])]), rt.create_array([rt.ArrayItem{ key: 'type', val: 'success' }, rt.ArrayItem{ key: 'dismissible', val: true }, rt.ArrayItem{ key: 'id', val: 'message' }])])
	} else if rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(rt.new_string('error')) && rt.is_true(rt.identical(rt.new_string('none'), rt.get_superglobal('_GET').array_get('error'))))) {
		rt.call_function('wp_admin_notice', [rt.call_function('__', [rt.new_string('No theme selected.')]), rt.create_array([rt.ArrayItem{ key: 'type', val: 'error' }, rt.ArrayItem{ key: 'dismissible', val: true }, rt.ArrayItem{ key: 'id', val: 'message' }])])
	}
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('_e', [rt.new_string('Network enabled themes are not shown on this screen.')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_method(var_wp_list_table, 'search_box', [rt.call_function('__', [rt.new_string('Search installed themes')]), rt.new_string('theme')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_id.dup()]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_method(, 'views', []rt.PhpVal{})
	// unsupported statement: Stmt_InlineHTML
}
