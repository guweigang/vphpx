...
fn logIO(var_io rt.PhpVal, var_msg rt.PhpVal) {
	...
	rt.call_function('_deprecated_function', [rt.new_string(@FN), rt.new_string('3.4.0'), rt.new_string('error_log()')])
	if !(!rt.is_true(var_GLOBALS.array_get('xmlrpc_logging'))) {
		rt.call_function('error_log', [(var_io).str() + ' - ' + (var_msg).str()])
	}
}
...
fn main() {
	...
	if !(!(var_HTTP_RAW_POST_DATA).is_null()) {
		mut var_HTTP_RAW_POST_DATA := rt.call_function('file_get_contents', [rt.new_string('php://input')])
	}
	var_HTTP_RAW_POST_DATA = rt.new_string(rt.new_string(var_HTTP_RAW_POST_DATA.dup().to_string().trim_space()))
	...
	if rt.get_superglobal('_GET').array_isset(rt.new_string('rsd')) {
		...
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('site_url', [rt.new_string('xmlrpc.php'), rt.new_string('rpc')]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('site_url', [rt.new_string('xmlrpc.php'), rt.new_string('rpc')]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('site_url', [rt.new_string('xmlrpc.php'), rt.new_string('rpc')]))
	...
	rt.include_file((rt.get_constant('ABSPATH')).str() + (rt.get_constant('WPINC')).str() + '/class-wp-xmlrpc-server.php', '4')
	mut var_post_default_title := ''
	mut var_wp_xmlrpc_server_class := rt.call_function('apply_filters', [rt.new_string('wp_xmlrpc_server_class'), rt.new_string('wp_xmlrpc_server')])
	mut var_wp_xmlrpc_server := rt.create_object_dynamically(var_wp_xmlrpc_server_class, []rt.PhpVal{})
