...
fn mysql2date(format string, var_date rt.PhpVal, translate bool) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('G'), rt.new_string(format))) || rt.is_true(rt.identical(rt.new_string('U'), rt.new_string(format))))) {
...
fn current_time(type string, gmt bool) rt.PhpVal {
	...
	mut var_timezone := rt.new_null()
	mut var_datetime := rt.new_null()
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('timestamp'), rt.new_string((var_type).str()))) || rt.is_true(rt.identical(rt.new_string('U'), rt.new_string((var_type).str()))))) {
...
fn date_i18n(var_format rt.PhpVal, timestamp_with_offset bool, gmt bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_timestamp.clone().is_long() || var_timestamp.clone().is_double()))))) {
	...
	} else if rt.is_true(rt.new_bool(var_gmt && rt.is_true(rt.identical(rt.new_bool(false), rt.new_bool(timestamp_with_offset))))) {
...
fn wp_date(var_format_arg rt.PhpVal, var_timestamp_arg rt.PhpVal, var_timezone_arg rt.PhpVal) bool {
	...
	} else if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_timestamp.clone().is_long() || var_timestamp.clone().is_double()))))) {
		return false
	}
...
fn wp_maybe_decline_date(var_date_arg rt.PhpVal, format string) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_string('on'), rt.call_function('_x', [rt.new_string('off'), rt.new_string('decline months names: on or off')]))) {
		...
		if rt.is_true(var_decline) {
			...
			{
				mut iter_1 := var_months_genitive.iterator()
				for {
					item_1 := iter_1.next() or { break }
					mut var_month_shadow := item_1.val
					mut var_key_shadow := item_1.key
...
fn number_format_i18n(var_number rt.PhpVal, decimals i64) rt.PhpVal {
	...
	if !(var_wp_locale).is_null() {
		var_formatted = rt.call_function('number_format', [var_number.clone(), rt.call_function('absint', [rt.new_int(decimals)]), rt.get_property(var_wp_locale, 'number_format').array_get('decimal_point'), rt.get_property(var_wp_locale, 'number_format').array_get('thousands_sep')])
	} else {
...
fn size_format(var_bytes rt.PhpVal, decimals i64) rt.PhpVal {
	...
	{
		mut iter_1 := var_quant.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_mag_shadow := item_1.val
			mut var_unit_shadow := item_1.key
...
fn human_readable_duration(duration string) bool {
	...
	if rt.is_true(rt.new_bool(var_duration == '' || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.new_string((var_duration).str()).is_string()))))))) {
		return false
	}
	...
	if 3 == var_duration_count {
		...
		mut _list_tmp_-54515 := var_duration_parts
...
fn get_weekstartend(var_mysqlstring rt.PhpVal, start_of_week string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.new_string((var_start_of_week).str()).is_long() || rt.new_string((var_start_of_week).str()).is_double()))))) {
...
fn maybe_serialize(var_data rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_data.clone().is_array())) || rt.is_true(rt.new_bool(var_data.clone().is_object())))) {
...
fn is_serialized(var_data_arg rt.PhpVal, strict bool) bool {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(':'), rt.new_string((var_data).str()).array_get(1))))) {
		return false
	}
	if var_strict {
		var_lastc = rt.call_function('substr', [rt.new_string((var_data).str()).clone(), -1])
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(';'), var_lastc)))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('}'), var_lastc)))))) {
			return false
		}
	} else {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_bool(false), var_semicolon)) && rt.is_true(rt.identical(rt.new_bool(false), var_brace)))) {
			return false
		}
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_semicolon)))) && rt.is_true(rt.less(var_semicolon, rt.new_int(3))))) {
			return false
		}
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_brace)))) && rt.is_true(rt.less(var_brace, rt.new_int(4))))) {
			return false
		}
	}
	...
	if rt.is_true(rt.equal(switch_val_2, rt.new_string('s'))) {
		if var_strict {
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('"'), rt.call_function('substr', [rt.new_string((var_data).str()).clone(), -2, rt.new_int(1)]))))) {
				return false
			}
...
fn is_serialized_string(var_data_arg rt.PhpVal) bool {
	...
	if var_data.len < 4 {
		return false
	} else if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(':'), rt.new_string((var_data).str()).array_get(1))))) {
		return false
	} else if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_ends_with', [rt.new_string((var_data).str()).clone(), rt.new_string(';')]))))) {
		return false
	} else if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('s'), rt.new_string((var_data).str()).array_get(0))))) {
		return false
	} else if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('"'), rt.call_function('substr', [rt.new_string((var_data).str()).clone(), -2, rt.new_int(1)]))))) {
		return false
	} else {
		return true
	}
...
fn wp_extract_urls(var_content rt.PhpVal) rt.PhpVal {
	...
	var_post_links = rt.call_function('array_unique', [rt.call_function('array_map', [rt.new_closure(closure_1_fn), var_post_links.array_get(2)])])
...
fn do_enclose(var_content_arg rt.PhpVal, var_post_arg rt.PhpVal) bool {
	...
	{
		mut iter_1 := var_pung.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_link_test_shadow := item_1.val
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_link_test_shadow.clone(), var_post_links_temp.clone(), rt.new_bool(true)]))))) {
				...
				{
					mut iter_2 := var_mids.iterator()
					for {
						item_2 := iter_2.next() or { break }
						mut var_mid_shadow := item_2.val
	...
	{
		mut iter_1 := rt.cast_array(var_post_links_temp).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_link_test_shadow := item_1.val
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_link_test_shadow.clone(), var_pung.clone(), rt.new_bool(true)]))))) {
				...
				} else if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_test.array_isset(rt.new_string('path')) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('/'), var_test.array_get('path'))))))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_test.array_get('path'))))))) {
					var_post_links.array_push(var_link_test_shadow.clone())
				}
			}
		}
	...
	{
		mut iter_1 := rt.cast_array(var_post_links).iterator()
		for {
			item_1 := iter_1.next() or { break }
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_url_shadow)))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_wpdb, 'get_var', [rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.new_string('SELECT post_id FROM '), rt.get_property(var_wpdb, 'postmeta')), rt.new_string(' WHERE post_id = %d AND meta_key = \'enclosure\' AND meta_value LIKE %s')), rt.get_property(var_post, 'ID'), rt.new_string((rt.call_method(var_wpdb, 'esc_like', [var_url_shadow.clone()])).str() + '%')])]))))))) {
				...
				if rt.is_true(var_headers) {
					var_len = rt.new_int((if !(var_headers.array_get('Content-Length')).is_null() { var_headers.array_get('Content-Length') } else { rt.new_int(0) }).to_i64())
					var_type = if !(var_headers.array_get('Content-Type')).is_null() { var_headers.array_get('Content-Type') } else { rt.new_string('') }
					var_allowed_types = ['video', 'audio']
					var_url_parts = rt.call_function('parse_url', [var_url_shadow.clone()])
					if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_url_parts)))) && !(!rt.is_true(var_url_parts.array_get('path'))))) {
						var_extension = rt.call_function('pathinfo', [var_url_parts.array_get('path'), rt.get_constant('PATHINFO_EXTENSION')])
						if !(!rt.is_true(var_extension)) {
							{
								mut iter_2 := wp_get_mime_types().iterator()
								for {
									item_2 := iter_2.next() or { break }
									mut var_mime_shadow := item_2.val
									mut var_exts_shadow := item_2.key
...
fn _http_build_query(var_data rt.PhpVal, var_prefix rt.PhpVal, var_sep_arg rt.PhpVal, key string, urlencode bool) rt.PhpVal {
	...
	var_ret = []rt.PhpVal{}
	{
		mut iter_1 := rt.cast_array(var_data).iterator()
		for {
			item_1 := iter_1.next() or { break }
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_k_shadow.clone().is_long())) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_prefix)))))) {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_v_shadow.clone().is_array())) || rt.is_true(rt.new_bool(var_v_shadow.clone().is_object())))) {
				var_ret.clone().array_push(_http_build_query(var_v_shadow.clone(), rt.new_string(''), var_sep.clone(), var_k_shadow.clone(), urlencode))
			} else if var_urlencode {
				var_ret.clone().array_push(rt.new_string((var_k_shadow).str() + '=' + (rt.call_function('urlencode', [var_v_shadow.clone()])).str()))
			} else {
...
fn add_query_arg(var_args_origin ...rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_args.array_get(0).is_array())) {
		if rt.is_true(rt.new_bool(var_args.clone().array_count() < 2 || rt.is_true(rt.identical(rt.new_bool(false), var_args.array_get(1))))) {
			var_uri = rt.get_superglobal('_SERVER').array_get('REQUEST_URI')
		} else {
			var_uri = var_args.array_get(1)
		}
	} else {
		if rt.is_true(rt.new_bool(var_args.clone().array_count() < 3 || rt.is_true(rt.identical(rt.new_bool(false), var_args.array_get(2))))) {
			var_uri = rt.get_superglobal('_SERVER').array_get('REQUEST_URI')
		} else {
	...
	if rt.is_true(var_frag) {
		var_uri = rt.call_function('substr', [var_uri.clone(), rt.new_int(0), -var_frag.clone().to_string().len])
	} else {
	...
	if rt.is_true(rt.call_function('str_contains', [var_uri.clone(), rt.new_string('?')])) {
		mut _list_tmp_87966 := rt.call_function('explode', [rt.new_string('?'), var_uri.clone(), rt.new_int(2)])
		var_base = rt.concat(var_base, rt.new_string('?'))
	} else if rt.is_true(rt.new_bool(var_protocol.len > 0 && var_protocol != '0' || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_uri.clone(), rt.new_string('=')]))))))) {
	...
	if rt.is_true(rt.new_bool(var_args.array_get(0).is_array())) {
		{
			mut iter_1 := var_args.array_get(0).iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_v_shadow := item_1.val
				mut var_k_shadow := item_1.key
...
fn remove_query_arg(key string, query bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.new_string(key).is_array())) {
		{
			mut iter_1 := rt.new_string(key).iterator()
			for {
				item_1 := iter_1.next() or { break }
...
fn add_magic_quotes(var_input_array rt.PhpVal) rt.PhpVal {
	...
	mut var_k := rt.new_null()
	{
		mut iter_1 := rt.cast_array(var_input_array).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_v_shadow := item_1.val
			mut var_k_shadow := item_1.key
...
fn wp_remote_fopen(var_uri rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_parsed_url)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_parsed_url.clone().is_array()))))))) {
		return false
	}
...
fn nocache_headers() {
	...
	rt.call_function('header_remove', [rt.new_string('Last-Modified')])
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_field_value_shadow := item_1.val
...
fn do_feed() {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string(''), var_feed)) || rt.is_true(rt.identical(rt.new_string('feed'), var_feed)))) {
...
fn do_robots() {
	...
	var_path = if !(!rt.is_true(var_site_url.array_get('path'))) { var_site_url.array_get('path') } else { rt.new_string('') }
...
fn is_blog_installed() bool {
	...
	{
		mut iter_1 := var_wp_tables.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_table_shadow := item_1.val
			if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('CUSTOM_USER_TABLE')])) && rt.is_true(rt.identical(rt.get_constant('CUSTOM_USER_TABLE'), var_table_shadow)))) {
				continue
			}
			if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('CUSTOM_USER_META_TABLE')])) && rt.is_true(rt.identical(rt.get_constant('CUSTOM_USER_META_TABLE'), var_table_shadow)))) {
				continue
			}
			var_described_table = rt.call_method(var_wpdb, 'get_results', [rt.new_string("DESCRIBE ${var_table.to_string()};")])
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_described_table)))) && !rt.is_true(rt.get_property(var_wpdb, 'last_error')))) || rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_described_table.clone().is_array())) && 0 == var_described_table.clone().array_count())))) {
				continue
			}
...
fn wp_original_referer_field(display bool, jump_back_to string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_ref)))) {
		var_ref = if rt.is_true(rt.identical(rt.new_string('previous'), rt.new_string(jump_back_to))) { rt.new_bool(wp_get_referer()) } else { rt.call_function('wp_unslash', [rt.get_superglobal('_SERVER').array_get('REQUEST_URI')]) }
	}
...
fn wp_get_referer() bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_ref) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('wp_unslash', [rt.get_superglobal('_SERVER').array_get('REQUEST_URI')]), var_ref)))))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical((rt.call_function('home_url', []rt.PhpVal{})).str() + (rt.call_function('wp_unslash', [rt.get_superglobal('_SERVER').array_get('REQUEST_URI')])).str(), var_ref)))))) {
...
fn wp_get_raw_referer() bool {
	if rt.is_true(rt.new_bool(!(!rt.is_true(rt.get_superglobal('_REQUEST').array_get('_wp_http_referer'))) && rt.is_true(rt.new_bool(rt.get_superglobal('_REQUEST').array_get('_wp_http_referer').is_string())))) {
...
fn wp_get_original_referer() bool {
	...
	if !(!rt.is_true(rt.get_superglobal('_REQUEST').array_get('_wp_original_http_referer'))) {
		return (rt.call_function('wp_validate_redirect', [rt.call_function('wp_unslash', [rt.get_superglobal('_REQUEST').array_get('_wp_original_http_referer')]), rt.new_bool(false)])).to_bool()
	}
	return false
}
...
fn wp_mkdir_p(var_target_arg rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(wp_is_stream(var_target.clone()))) {
		mut _list_tmp_-68560 := rt.call_function('explode', [rt.new_string('://'), var_target.clone(), rt.new_int(2)])
	}
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('str_contains', [var_target.clone(), rt.new_string('../')])) || rt.is_true(rt.call_function('str_contains', [var_target.clone(), rt.new_string('..' + (rt.get_constant('DIRECTORY_SEPARATOR')).str())])))) {
		return false
	}
	var_target_parent = rt.call_function('dirname', [var_target.clone()])
	for rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('.'), var_target_parent)))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_dir', [var_target_parent.clone()]))))))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('dirname', [var_target_parent.clone()]), var_target_parent)))))) {
	...
	if rt.is_true(rt.call_function('mkdir', [var_target.clone(), var_dir_perms.clone(), rt.new_bool(true)])) {
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.bitwise_and(var_dir_perms, rt.bitwise_not(rt.call_function('umask', []rt.PhpVal{}))), var_dir_perms)))) {
			var_folder_parts = rt.call_function('explode', [rt.new_string('/'), rt.call_function('substr', [var_target.clone(), var_target_parent.clone().to_string().len + 1])])
			{
				var_i = 1
...
fn path_is_absolute(var_path rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(wp_is_stream(var_path.clone()) && rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_dir', [var_path.clone()])) || rt.is_true(rt.call_function('is_file', [var_path.clone()])))))) {
		return true
	}
	if rt.is_true(rt.identical(rt.call_function('realpath', [var_path.clone()]), var_path)) {
		return true
	}
	if rt.is_true(rt.new_bool(var_path.clone().to_string().len == 0 || rt.is_true(rt.identical(rt.new_string('.'), var_path.array_get(0))))) {
		return false
	}
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('#^[a-zA-Z]:\\\\#'), var_path.clone()])) {
		return true
	}
	return rt.is_true(rt.identical(rt.new_string('/'), var_path.array_get(0))) || rt.is_true(rt.identical(rt.new_string('\\'), var_path.array_get(0)))
}
...
fn wp_normalize_path(var_path_arg rt.PhpVal) string {
	...
	if rt.is_true(rt.new_bool(wp_is_stream(var_path.clone()))) {
		mut _list_tmp_30044 := rt.call_function('explode', [rt.new_string('://'), var_path.clone(), rt.new_int(2)])
...
fn get_temp_dir() string {
	...
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('sys_get_temp_dir')])) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_dir', [var_temp.clone()])) && rt.is_true(wp_is_writable(var_temp.clone())))) {
			return (rt.call_function('trailingslashit', [var_temp.clone()])).str()
		}
	}
...
fn win_is_writable(var_path rt.PhpVal) bool {
	...
	if rt.is_true(rt.identical(rt.new_string('/'), var_path.array_get(var_path.clone().to_string().len - 1))) {
	...
	if var_create_dir {
		...
		if rt.is_true(rt.new_bool(var_tested_paths.clone().array_isset(var_path.clone()))) {
			...
			if !(wp_mkdir_p(var_path.clone())) {
				if rt.is_true(rt.call_function('str_starts_with', [var_uploads.array_get('basedir'), rt.get_constant('ABSPATH')])) {
					var_error_path = rt.new_string((rt.call_function('str_replace', [rt.get_constant('ABSPATH'), rt.new_string(''), var_uploads.array_get('basedir')])).str() + (var_uploads.array_get('subdir')).str())
				} else {
					var_error_path = rt.new_string((rt.call_function('wp_basename', [var_uploads.array_get('basedir')])).str() + (var_uploads.array_get('subdir')).str())
				}
...
fn _wp_upload_dir(var_time_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_upload_path == '' || rt.is_true(rt.identical(rt.new_string('wp-content/uploads'), rt.new_string((var_upload_path).str()))))) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_url)))) {
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_upload_path == '' || rt.is_true(rt.identical(rt.new_string('wp-content/uploads'), rt.new_string((var_upload_path).str()))))) || rt.is_true(rt.identical(rt.new_string((var_upload_path).str()), var_dir)))) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('UPLOADS')])) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{})) && rt.is_true(rt.call_function('get_site_option', [rt.new_string('ms_files_rewriting')]))))))))) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{})) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(is_main_network() && is_main_site())) && rt.is_true(rt.call_function('defined', [rt.new_string('MULTISITE')]))))))))) {
		...
		} else if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('UPLOADS')])) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('ms_is_switched', []rt.PhpVal{}))))))) {
...
fn wp_unique_filename(var_dir rt.PhpVal, var_filename_arg rt.PhpVal, var_unique_filename_callback rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_unique_filename_callback) && rt.is_true(rt.call_function('is_callable', [var_unique_filename_callback.clone()])))) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(var_fname) && rt.is_true(rt.call_function('preg_match', [rt.new_string('/-(?:\\d+x\\d+|scaled|rotated)$/'), var_fname.clone()])))) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(var_ext) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string((var_lc_ext).str()), var_ext)))))) {
		...
		for rt.is_true(rt.new_bool(rt.is_true(rt.call_function('file_exists', [rt.new_string((var__dir).str() + (var_filename).str())])) || rt.is_true(rt.new_bool(rt.is_true(var_lc_filename) && rt.is_true(rt.call_function('file_exists', [rt.new_string((var__dir).str() + (var_lc_filename).str())])))))) {
		...
		var_files = []rt.PhpVal{}
		var_count = 10000
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_name) && rt.is_true(var_ext))) && rt.is_true(rt.call_function('is_dir', [var_dir.clone()])))) && rt.is_true(rt.call_function('str_contains', [var_dir.clone(), var_upload_dir.array_get('basedir')])))) {
			...
			if !(!rt.is_true(var_files)) {
				...
				var_i = 0
				for rt.is_true(rt.new_bool(var_i <= var_count && _wp_check_existing_file_names(var_filename.clone(), var_files.clone()))) {
					var_new_number = rt.new_int((var_number).to_i64()) + 1
		...
		if var_is_image {
			...
			{
				mut iter_1 := var_alt_types.iterator()
				for {
					item_1 := iter_1.next() or { break }
					mut var_alt_type_shadow := item_1.val
			...
			if !(!rt.is_true(var_alt_filenames)) {
				...
				var_i = 0
				for rt.is_true(rt.new_bool(var_i <= var_count && _wp_check_alternate_file_names(var_alt_filenames.clone(), var__dir.clone(), var_files.clone()))) {
					var_new_number = rt.new_int((var_number).to_i64()) + 1
					{
						mut iter_1 := var_alt_filenames.iterator()
						for {
							item_1 := iter_1.next() or { break }
							mut var_alt_filename_shadow := item_1.val
							mut var_alt_ext_shadow := item_1.key
...
fn _wp_check_alternate_file_names(var_filenames rt.PhpVal, var_dir rt.PhpVal, var_files rt.PhpVal) bool {
	...
	{
		mut iter_1 := var_filenames.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_filename_shadow := item_1.val
			if rt.is_true(rt.call_function('file_exists', [rt.new_string((var_dir).str() + (var_filename_shadow).str())])) {
				return true
			}
			if rt.is_true(rt.new_bool(!(!rt.is_true(var_files)) && _wp_check_existing_file_names(var_filename_shadow.clone(), var_files.clone()))) {
				return true
			}
		}
	}
...
fn _wp_check_existing_file_names(var_filename rt.PhpVal, var_files rt.PhpVal) bool {
	...
	var_regex = rt.new_string('/^' + (rt.call_function('preg_quote', [var_fname.clone()])).str() + '-(?:\\d+x\\d+|scaled|rotated)' + (rt.call_function('preg_quote', [var_ext.clone()])).str() + '$/i')
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_file_shadow := item_1.val
...
fn wp_upload_bits(var_name rt.PhpVal, var_deprecated rt.PhpVal, var_bits rt.PhpVal, var_time rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_wp_filetype.array_get('ext'))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('unfiltered_upload')]))))))) {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_upload.array_get('error'))))) {
	...
	var_new_file = rt.new_string((var_upload.array_get('path')).str() + "/${var_filename.to_string()}")
	if !(wp_mkdir_p(rt.call_function('dirname', [var_new_file.clone()]))) {
		if rt.is_true(rt.call_function('str_starts_with', [var_upload.array_get('basedir'), rt.get_constant('ABSPATH')])) {
			var_error_path = rt.new_string((rt.call_function('str_replace', [rt.get_constant('ABSPATH'), rt.new_string(''), var_upload.array_get('basedir')])).str() + (var_upload.array_get('subdir')).str())
		} else {
			var_error_path = rt.new_string((rt.call_function('wp_basename', [var_upload.array_get('basedir')])).str() + (var_upload.array_get('subdir')).str())
		}
	...
	return rt.call_function('apply_filters', [rt.new_string('wp_handle_upload'), rt.create_array([rt.ArrayItem{ key: 'file', val: var_new_file }, rt.ArrayItem{ key: 'url', val: var_url }, rt.ArrayItem{ key: 'type', val: var_wp_filetype.array_get('type') }, rt.ArrayItem{ key: 'error', val: false }]), rt.new_string('sideload')])
}
...
fn wp_ext2type(var_ext_arg rt.PhpVal) rt.PhpVal {
	...
	{
		mut iter_1 := var_ext2type.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_exts_shadow := item_1.val
...
fn wp_check_filetype(var_filename rt.PhpVal, var_mimes_arg rt.PhpVal) rt.PhpVal {
	...
	{
		mut iter_1 := var_mimes.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_mime_match_shadow := item_1.val
			mut var_ext_preg_shadow := item_1.key
...
fn wp_check_filetype_and_ext(var_file rt.PhpVal, var_filename rt.PhpVal, var_mimes rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_type) && rt.is_true(rt.call_function('str_starts_with', [var_type.clone(), rt.new_string('image/')])))) {
		var_real_mime = rt.new_bool(wp_get_image_mime(var_file.clone()))
		var_heic_images_extensions = ['heif', 'heics', 'heifs']
		if rt.is_true(rt.new_bool(rt.is_true(var_real_mime) && rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_real_mime, var_type)))) || rt.is_true(rt.call_function('in_array', [var_ext.clone(), rt.create_array_from_list(var_heic_images_extensions), rt.new_bool(true)])))))) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_type) && rt.is_true(rt.new_bool(!(rt.is_true(var_real_mime)))))) && rt.is_true(rt.call_function('extension_loaded', [rt.new_string('fileinfo')])))) {
		...
		} else if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('str_starts_with', [var_real_mime.clone(), rt.new_string('video/')])) || rt.is_true(rt.call_function('str_starts_with', [var_real_mime.clone(), rt.new_string('audio/')])))) {
...
fn wp_get_image_mime(var_file rt.PhpVal) bool {
	...
	if rt.is_true(rt.call_function('is_callable', [rt.new_string('exif_imagetype')])) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('WP_DEBUG')])) && rt.is_true(rt.get_constant('WP_DEBUG')))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('WP_RUN_CORE_TESTS')]))))))) {
		...
		if rt.has_exception() { unsafe { goto catch_label_1 } }
		var_mime = if !(var_imagesize.array_get('mime')).is_null() { var_imagesize.array_get('mime') } else { rt.new_bool(false) }
		if rt.has_exception() { unsafe { goto catch_label_1 } }
	} else {
	...
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('str_starts_with', [var_magic.clone(), rt.new_string('52494646')])) && rt.is_true(rt.identical(rt.new_int(16), rt.call_function('strpos', [var_magic.clone(), rt.new_string('57454250')]))))) {
	...
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	if rt.is_true(rt.new_bool(var_magic.array_isset(rt.new_int(1)) && var_magic.array_isset(rt.new_int(2)) && rt.is_true(rt.identical(rt.new_string('ftyp'), rt.call_function('hex2bin', [var_magic.array_get(1)]))))) {
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('avif'), rt.call_function('hex2bin', [var_magic.array_get(2)]))) || rt.is_true(rt.identical(rt.new_string('avis'), rt.call_function('hex2bin', [var_magic.array_get(2)]))))) {
			var_mime = rt.new_string('image/avif')
			if rt.has_exception() { unsafe { goto catch_label_1 } }
		} else if rt.is_true(rt.identical(rt.new_string('heic'), rt.call_function('hex2bin', [var_magic.array_get(2)]))) {
			var_mime = rt.new_string('image/heic')
			if rt.has_exception() { unsafe { goto catch_label_1 } }
		} else if rt.is_true(rt.identical(rt.new_string('heif'), rt.call_function('hex2bin', [var_magic.array_get(2)]))) {
...
fn wp_nonce_ays(var_action rt.PhpVal) {
	...
	if rt.is_true(rt.identical(rt.new_string('log-out'), var_action)) {
		...
		var_redirect_to = if !(rt.get_superglobal('_REQUEST').array_get('redirect_to')).is_null() { rt.get_superglobal('_REQUEST').array_get('redirect_to') } else { rt.new_string('') }
...
fn _default_wp_die_handler(var_message_arg rt.PhpVal, title string, var_args rt.PhpVal) {
	...
	mut _list_tmp_46415 := _wp_die_process_input(var_message.clone(), var_title, var_args.clone())
	if rt.is_true(rt.new_bool(var_message.clone().is_string())) {
		if !(!rt.is_true(var_parsed_args.array_get('additional_errors'))) {
			var_message = rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: none, val: var_message }]), wp_list_pluck(var_parsed_args.array_get('additional_errors'), 'message', rt.new_null())])
	...
	if !(!rt.is_true(var_parsed_args.array_get('link_url'))) && !(!rt.is_true(var_parsed_args.array_get('link_text'))) {
	...
	if rt.is_true(rt.new_bool(var_parsed_args.array_isset(rt.new_string('back_link')) && rt.is_true(var_parsed_args.array_get('back_link')))) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('did_action', [rt.new_string('admin_head')]))))) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('headers_sent', []rt.PhpVal{}))))) {
			rt.call_function('header', [rt.concat(rt.new_string('Content-Type: text/html; charset='), var_parsed_args.array_get('charset'))])
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(var_args.array_get('text_direction')) && rt.is_true(rt.call_function('function_exists', [rt.new_string('language_attributes')])))) && rt.is_true(rt.call_function('function_exists', [rt.new_string('is_rtl')])))) {
		...
		// unsupported statement: Stmt_InlineHTML
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('function_exists', [rt.new_string('wp_robots')])) && rt.is_true(rt.call_function('function_exists', [rt.new_string('wp_robots_no_robots')])))) && rt.is_true(rt.call_function('function_exists', [rt.new_string('add_filter')])))) {
...
fn _ajax_wp_die_handler(var_message_arg rt.PhpVal, title string, var_args_arg rt.PhpVal) {
	...
	mut _list_tmp_46415 := _wp_die_process_input(var_message.clone(), var_title, var_args.clone())
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('headers_sent', []rt.PhpVal{}))))) {
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_args.array_get('response'))))) {
			status_header(var_parsed_args.array_get('response'), '')
		}
		nocache_headers()
	}
...
fn _json_wp_die_handler(var_message rt.PhpVal, title string, var_args rt.PhpVal) {
	...
	mut _list_tmp_46415 := _wp_die_process_input(var_message.clone(), var_title, var_args.clone())
	var_data = rt.create_array([rt.ArrayItem{ key: 'code', val: var_parsed_args.array_get('code') }, rt.ArrayItem{ key: 'message', val: var_message }, rt.ArrayItem{ key: 'data', val: rt.create_array([rt.ArrayItem{ key: 'status', val: var_parsed_args.array_get('response') }]) }, rt.ArrayItem{ key: 'additional_errors', val: var_parsed_args.array_get('additional_errors') }])
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('headers_sent', []rt.PhpVal{}))))) {
		rt.call_function('header', [rt.concat(rt.new_string('Content-Type: application/json; charset='), var_parsed_args.array_get('charset'))])
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_parsed_args.array_get('response'))))) {
			status_header(var_parsed_args.array_get('response'), '')
		}
		nocache_headers()
	}
	print(wp_json_encode(var_data.clone()))
	if rt.is_true(var_parsed_args.array_get('exit')) {
		exit(0)
	}
}
...
fn _jsonp_wp_die_handler(var_message rt.PhpVal, title string, var_args rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('headers_sent', []rt.PhpVal{}))))) {
		rt.call_function('header', [rt.concat(rt.new_string('Content-Type: application/javascript; charset='), var_parsed_args.array_get('charset'))])
	...
	var_result = rt.new_string(wp_json_encode(var_data.clone(), 0, 0))
...
fn _xml_wp_die_handler(var_message_arg rt.PhpVal, title string, var_args rt.PhpVal) {
	...
	var_xml = rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('<error>\n    <code>'), var_parsed_args.array_get('code')), rt.new_string('</code>\n    <title><![CDATA[')), rt.new_string((var_title).str())), rt.new_string(']]></title>\n    <message><![CDATA[')), var_message), rt.new_string(']]></message>\n    <data>\n        <status>')), var_parsed_args.array_get('response')), rt.new_string('</status>\n    </data>\n</error>\n'))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('headers_sent', []rt.PhpVal{}))))) {
		rt.call_function('header', [rt.concat(rt.new_string('Content-Type: text/xml; charset='), var_parsed_args.array_get('charset'))])
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_parsed_args.array_get('response'))))) {
			status_header(var_parsed_args.array_get('response'), '')
		}
		nocache_headers()
...
fn _scalar_wp_die_handler(message string, title string, var_args rt.PhpVal) {
	...
	mut var_title := title
	mut var_parsed_args := rt.new_null()
	mut _list_tmp_14022 := _wp_die_process_input(rt.new_string(message), var_title, var_args.clone())
	if rt.is_true(var_parsed_args.array_get('exit')) {
...
fn _wp_die_process_input(var_message_arg rt.PhpVal, title string, var_args_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('function_exists', [rt.new_string('is_wp_error')])) && rt.is_true(rt.call_function('is_wp_error', [var_message.clone()])))) {
		if !(!rt.is_true(rt.get_property(var_message, 'errors'))) {
			var_errors = []rt.PhpVal{}
			{
				mut iter_1 := rt.cast_array(rt.get_property(var_message, 'errors')).iterator()
				for {
					item_1 := iter_1.next() or { break }
					mut var_error_messages_shadow := item_1.val
					mut var_error_code_shadow := item_1.key
					{
						mut iter_2 := rt.cast_array(var_error_messages_shadow).iterator()
						for {
							item_2 := iter_2.next() or { break }
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(var_args.array_get('response')) && rt.is_true(rt.new_bool(var_errors.array_get(0).array_get('data').is_array())))) && !(!rt.is_true(var_errors.array_get(0).array_get('data').array_get('status'))))) {
				var_args.array_set('response', var_errors.array_get(0).array_get('data').array_get('status'))
			}
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_title == '' && rt.is_true(rt.new_bool(var_errors.array_get(0).array_get('data').is_array())))) && !(!rt.is_true(var_errors.array_get(0).array_get('data').array_get('title'))))) {
				var_title = (var_errors.array_get(0).array_get('data').array_get('title')).str()
			}
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.get_constant('WP_DEBUG_DISPLAY')) && rt.is_true(rt.new_bool(var_errors.array_get(0).array_get('data').is_array())))) && !(!rt.is_true(var_errors.array_get(0).array_get('data').array_get('error'))))) {
				var_args.array_set('error_data', var_errors.array_get(0).array_get('data').array_get('error'))
			}
	...
	if rt.is_true(rt.new_bool(!rt.is_true(var_args.array_get('text_direction')) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_args.array_get('text_direction'), rt.create_array([rt.ArrayItem{ key: none, val: 'ltr' }, rt.ArrayItem{ key: none, val: 'rtl' }]), rt.new_bool(true)]))))))) {
		var_args.array_set('text_direction', 'ltr')
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('function_exists', [rt.new_string('is_rtl')])) && rt.is_true(rt.call_function('is_rtl', []rt.PhpVal{})))) {
...
fn wp_json_encode(var_value_arg rt.PhpVal, flags i64, depth i64) string {
	mut var_flags := flags
	mut var_depth := depth
...
end_label_2:
	return rt.json_encode(var_value.clone())
}
	...
	if rt.is_true(rt.new_bool(var_value.clone().is_array())) {
		var_output = []rt.PhpVal{}
		{
			...
			for {
				...
				if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_el_shadow.clone().is_array())) || rt.is_true(rt.new_bool(var_el_shadow.clone().is_object())))) {
...
fn wp_send_json(var_response rt.PhpVal, var_status_code rt.PhpVal, flags i64) {
	...
	print(wp_json_encode(var_response.clone(), flags))
	if rt.is_true(rt.call_function('wp_doing_ajax', []rt.PhpVal{})) {
...
fn wp_send_json_error(var_value rt.PhpVal, var_status_code rt.PhpVal, flags i64) {
	...
	if !(var_value).is_null() {
		if rt.is_true(rt.call_function('is_wp_error', [var_value.clone()])) {
			...
			{
				mut iter_1 := rt.get_property(var_value, 'errors').iterator()
				for {
					item_1 := iter_1.next() or { break }
					mut var_messages_shadow := item_1.val
					mut var_code_shadow := item_1.key
					{
						mut iter_2 := var_messages_shadow.iterator()
						for {
							item_2 := iter_2.next() or { break }
...
fn wp_check_jsonp_callback(var_callback rt.PhpVal) bool {
	...
	rt.call_function('preg_replace', [rt.new_string('/[^\\w\\.]/'), rt.new_string(''), var_callback.clone(), -1, var_illegal_char_count.clone()])
...
fn wp_json_file_decode(var_filename_arg rt.PhpVal, var_options_arg rt.PhpVal) rt.PhpVal {
	...
	var_decoded_file = rt.call_function('json_decode', [rt.call_function('file_get_contents', [rt.new_string((var_filename).str()).clone()]), var_options.array_get('associative')])
...
fn _mce_set_direction(var_mce_init rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.call_function('is_rtl', []rt.PhpVal{})) {
		...
		if rt.is_true(rt.new_bool(!(!rt.is_true(var_mce_init.array_get('plugins'))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_mce_init.array_get('plugins'), rt.new_string('directionality')]))))))) {
			var_mce_init.array_get('plugins') = rt.concat(var_mce_init.array_get('plugins'), rt.new_string(',directionality'))
		}
		if rt.is_true(rt.new_bool(!(!rt.is_true(var_mce_init.array_get('toolbar1'))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('/\\bltr\\b/'), var_mce_init.array_get('toolbar1')]))))))) {
			var_mce_init.array_get('toolbar1') = rt.concat(var_mce_init.array_get('toolbar1'), rt.new_string(',ltr'))
		}
	}
...
fn smilies_init() {
	...
	{
		mut iter_1 := rt.cast_array(var_wpsmiliestrans).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_img_shadow := item_1.val
			mut var_smiley_shadow := item_1.key
...
fn wp_parse_args(var_args rt.PhpVal, var_defaults rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.create_array_from_native_map(var_defaults).is_array())) && rt.is_true(var_defaults))) {
...
fn wp_parse_list(var_input_list_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_input_list.clone().is_array()))))) {
		return rt.call_function('preg_split', [rt.new_string('/[\\s,]+/'), var_input_list.clone(), -1, rt.get_constant('PREG_SPLIT_NO_EMPTY')])
	}
...
fn wp_recursive_ksort(var_input_array rt.PhpVal) {
	...
	{
		mut iter_1 := var_input_array.iterator()
		for {
			item_1 := iter_1.next() or { break }
...
fn _wp_array_get(var_input_array_arg rt.PhpVal, var_path rt.PhpVal, var_default_value rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_path.clone().is_array()))))) || 0 == var_path.clone().array_count())) {
	...
	}
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_path_element_shadow := item_1.val
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_input_array.clone().is_array()))))) {
				return var_default_value.clone()
			}
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_path_element_shadow.clone().is_string())) || rt.is_true(rt.new_bool(var_path_element_shadow.clone().is_long())))) || rt.is_true(rt.identical(rt.new_null(), var_path_element_shadow)))) {
				...
				if rt.is_true(rt.new_bool(!(var_path_element_shadow).is_null() && rt.is_true(rt.new_bool(var_input_array.clone().array_isset(var_path_element_shadow.clone()))))) {
					var_input_array = var_input_array.array_get(var_path_element_shadow)
					continue
				}
			}
...
fn _wp_array_set(var_input_array rt.PhpVal, var_path rt.PhpVal, var_value rt.PhpVal) {
	...
	}
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_path_element_shadow := item_1.val
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_path_element_shadow.clone().is_string()))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_path_element_shadow.clone().is_long()))))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_path_element_shadow.clone().is_null()))))))) {
				return
			}
		}
	}
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_input_array.clone().array_isset(var_path_element.clone())))))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_input_array.array_get(var_path_element).is_array()))))))) {
...
fn wp_widgets_add_menu() {
	...
	rt.call_function('ksort', [var_submenu.array_get('themes.php'), rt.get_constant('SORT_NUMERIC')])
}
...
fn dead_db() {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('wp_installing', []rt.PhpVal{})) || rt.is_true(rt.call_function('defined', [rt.new_string('WP_ADMIN')])))) {
...
fn _deprecated_function(var_function_name rt.PhpVal, version string, replacement string) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.get_constant('WP_DEBUG')) && rt.is_true(rt.call_function('apply_filters', [rt.new_string('deprecated_function_trigger_error'), rt.new_bool(true)])))) {
...
fn _deprecated_constructor(var_class_name rt.PhpVal, var_version rt.PhpVal, parent_class string) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.get_constant('WP_DEBUG')) && rt.is_true(rt.call_function('apply_filters', [rt.new_string('deprecated_constructor_trigger_error'), rt.new_bool(true)])))) {
		if rt.is_true(rt.call_function('function_exists', [rt.new_string('__')])) {
...
fn _deprecated_class(var_class_name rt.PhpVal, var_version rt.PhpVal, replacement string) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.get_constant('WP_DEBUG')) && rt.is_true(rt.call_function('apply_filters', [rt.new_string('deprecated_class_trigger_error'), rt.new_bool(true)])))) {
		if rt.is_true(rt.call_function('function_exists', [rt.new_string('__')])) {
			if var_replacement.len > 0 && var_replacement != '0' {
...
fn _deprecated_file(var_file rt.PhpVal, var_version rt.PhpVal, replacement string, message string) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.get_constant('WP_DEBUG')) && rt.is_true(rt.call_function('apply_filters', [rt.new_string('deprecated_file_trigger_error'), rt.new_bool(true)])))) {
...
fn _deprecated_argument(var_function_name rt.PhpVal, version string, message string) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.get_constant('WP_DEBUG')) && rt.is_true(rt.call_function('apply_filters', [rt.new_string('deprecated_argument_trigger_error'), rt.new_bool(true)])))) {
		if rt.is_true(rt.call_function('function_exists', [rt.new_string('__')])) {
...
fn _deprecated_hook(var_hook rt.PhpVal, var_version rt.PhpVal, replacement string, message string) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.get_constant('WP_DEBUG')) && rt.is_true(rt.call_function('apply_filters', [rt.new_string('deprecated_hook_trigger_error'), rt.new_bool(true)])))) {
		var_message = if var_message == '' { '' } else { ' ' + var_message }
		if var_replacement.len > 0 && var_replacement != '0' {
...
fn _doing_it_wrong(var_function_name rt.PhpVal, var_message_arg rt.PhpVal, version string) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.get_constant('WP_DEBUG')) && rt.is_true(rt.call_function('apply_filters', [rt.new_string('doing_it_wrong_trigger_error'), rt.new_bool(true), var_function_name.clone(), var_message.clone(), rt.new_string((var_version).str())])))) {
		if rt.is_true(rt.call_function('function_exists', [rt.new_string('__')])) {
...
fn is_lighttpd_before_150() bool {
	...
	var_server_parts = rt.call_function('explode', [rt.new_string('/'), if !(rt.get_superglobal('_SERVER').array_get('SERVER_SOFTWARE')).is_null() { rt.get_superglobal('_SERVER').array_get('SERVER_SOFTWARE') } else { rt.new_string('') }])
	var_server_parts.array_set(1, if !(var_server_parts.array_get(1)).is_null() { var_server_parts.array_get(1) } else { rt.new_string('') })
	return rt.is_true(rt.identical(rt.new_string('lighttpd'), var_server_parts.array_get(0))) && rt.is_true(rt.identical(-1, rt.call_function('version_compare', [var_server_parts.array_get(1), rt.new_string('1.5.0')])))
}
...
fn apache_mod_loaded(var_mod rt.PhpVal, default_value bool) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(var_loaded_mods) && rt.is_true(rt.call_function('function_exists', [rt.new_string('phpinfo')])))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [rt.call_function('ini_get', [rt.new_string('disable_functions')]), rt.new_string('phpinfo')]))))))) {
...
fn iis7_supports_permalinks() rt.PhpVal {
	...
	if rt.is_true(var_is_iis7) {
		var_supports_permalinks = rt.is_true(rt.new_bool(rt.is_true(rt.call_function('class_exists', [rt.new_string('DOMDocument'), rt.new_bool(false)])) && rt.get_superglobal('_SERVER').array_isset(rt.new_string('IIS_UrlRewriteModule')))) && rt.is_true(rt.identical(rt.new_string('cgi-fcgi'), rt.get_constant('PHP_SAPI')))
	}
...
fn validate_file(var_file_arg rt.PhpVal, var_allowed_files_arg rt.PhpVal) i64 {
	...
	mut var_matches := []rt.PhpVal{}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_scalar', [rt.new_string((var_file).str()).clone()]))))) || rt.is_true(rt.identical(rt.new_string(''), rt.new_string((var_file).str()))))) {
		return 0
	}
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('preg_match_all', [rt.new_string('#\\.\\./#'), rt.new_string((var_file).str()).clone(), rt.create_array_from_list(var_matches), rt.get_constant('PREG_SET_ORDER')])) && var_matches.len > 1)) {
		return 1
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('str_contains', [rt.new_string((var_file).str()).clone(), rt.new_string('../')])) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('../'), rt.call_function('mb_substr', [rt.new_string((var_file).str()).clone(), -3, rt.new_int(3)]))))))) {
		return 1
	}
	if rt.is_true(rt.new_bool(!(!rt.is_true(var_allowed_files)) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string((var_file).str()).clone(), var_allowed_files.clone(), rt.new_bool(true)]))))))) {
		return 3
	}
...
fn wp_guess_url() string {
	...
	mut var_schema := ''
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('WP_SITEURL')])) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), rt.get_constant('WP_SITEURL'))))))) {
		...
		var_script_filename_dir = rt.call_function('dirname', [rt.get_superglobal('_SERVER').array_get('SCRIPT_FILENAME')])
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('str_contains', [rt.get_superglobal('_SERVER').array_get('REQUEST_URI'), rt.new_string('wp-admin')])) || rt.is_true(rt.call_function('str_contains', [rt.get_superglobal('_SERVER').array_get('REQUEST_URI'), rt.new_string('wp-login.php')])))) {
			var_path = rt.call_function('preg_replace', [rt.new_string('#/(wp-admin/?.*|wp-login\\.php.*)#i'), rt.new_string(''), rt.get_superglobal('_SERVER').array_get('REQUEST_URI')])
		} else if rt.is_true(rt.identical((var_script_filename_dir).str() + '/', var_abspath_fix)) {
			var_path = rt.call_function('preg_replace', [rt.new_string('#/[^/]*$#i'), rt.new_string(''), rt.get_superglobal('_SERVER').array_get('PHP_SELF')])
		} else {
			if rt.is_true(rt.call_function('str_contains', [rt.get_superglobal('_SERVER').array_get('SCRIPT_FILENAME'), var_abspath_fix.clone()])) {
				var_directory = rt.call_function('str_replace', [rt.get_constant('ABSPATH'), rt.new_string(''), var_script_filename_dir.clone()])
				var_path = rt.call_function('preg_replace', [rt.new_string('#/' + (rt.call_function('preg_quote', [var_directory.clone(), rt.new_string('#')])).str() + '/[^/]*$#i'), rt.new_string(''), rt.get_superglobal('_SERVER').array_get('REQUEST_URI')])
				...
				var_path = rt.new_string((rt.call_function('preg_replace', [rt.new_string('#/[^/]*$#i'), rt.new_string(''), rt.get_superglobal('_SERVER').array_get('REQUEST_URI')])).str() + (var_subdirectory).str())
			} else {
		...
		var_url = rt.new_string((var_schema + (rt.get_superglobal('_SERVER').array_get('HTTP_HOST')).str() + (var_path).str()).str())
	}
...
fn wp_timezone_override_offset() bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_bool(false), var_timezone_object)) || rt.is_true(rt.identical(rt.new_bool(false), var_datetime_object)))) {
		return false
	}
...
fn _wp_timezone_choice_usort_callback(var_a rt.PhpVal, var_b rt.PhpVal) i64 {
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('Etc'), var_a.array_get('continent'))) && rt.is_true(rt.identical(rt.new_string('Etc'), var_b.array_get('continent'))))) {
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('str_starts_with', [var_a.array_get('city'), rt.new_string('GMT+')])) && rt.is_true(rt.call_function('str_starts_with', [var_b.array_get('city'), rt.new_string('GMT+')])))) {
			return (rt.mul(-1, rt.call_function('strnatcasecmp', [var_a.array_get('city'), var_b.array_get('city')]))).to_i64()
		}
		if rt.is_true(rt.identical(rt.new_string('UTC'), var_a.array_get('city'))) {
			if rt.is_true(rt.call_function('str_starts_with', [var_b.array_get('city'), rt.new_string('GMT+')])) {
				return 1
			}
			return -1
		}
		if rt.is_true(rt.identical(rt.new_string('UTC'), var_b.array_get('city'))) {
			if rt.is_true(rt.call_function('str_starts_with', [var_a.array_get('city'), rt.new_string('GMT+')])) {
				return -1
			}
			return 1
		}
		return (rt.call_function('strnatcasecmp', [var_a.array_get('city'), var_b.array_get('city')])).to_i64()
	}
	if rt.is_true(rt.identical(var_a.array_get('t_continent'), var_b.array_get('t_continent'))) {
		if rt.is_true(rt.identical(var_a.array_get('t_city'), var_b.array_get('t_city'))) {
			return (rt.call_function('strnatcasecmp', [var_a.array_get('t_subcity'), var_b.array_get('t_subcity')])).to_i64()
		}
		return (rt.call_function('strnatcasecmp', [var_a.array_get('t_city'), var_b.array_get('t_city')])).to_i64()
	} else {
		...
		return (rt.call_function('strnatcasecmp', [var_a.array_get('t_continent'), var_b.array_get('t_continent')])).to_i64()
	}
	return 0
}
...
fn wp_timezone_choice(var_selected_zone rt.PhpVal, var_locale rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(var_mo_loaded) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_locale, var_locale_loaded)))))) {
	...
	var_zonen = []rt.PhpVal{}
	{
		mut iter_1 := var_tz_identifiers.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_zone_shadow := item_1.val
			var_zone_shadow = rt.call_function('explode', [rt.new_string('/'), var_zone_shadow.clone()])
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_zone_shadow.array_get(0), rt.create_array_from_list(var_continents), rt.new_bool(true)]))))) {
				continue
			}
			var_exists = [var_zone_shadow.array_isset(rt.new_int(0)) && rt.is_true(var_zone_shadow.array_get(0)), var_zone_shadow.array_isset(rt.new_int(1)) && rt.is_true(var_zone_shadow.array_get(1)), var_zone_shadow.array_isset(rt.new_int(2)) && rt.is_true(var_zone_shadow.array_get(2))]
			var_exists[3] = rt.is_true(var_exists.array_get(0)) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('Etc'), var_zone_shadow.array_get(0)))))
			...
			var_zonen.array_push(rt.create_array([rt.ArrayItem{ key: 'continent', val: if rt.is_true(var_exists.array_get(0)) { var_zone_shadow.array_get(0) } else { rt.new_string('') } }, rt.ArrayItem{ key: 'city', val: if rt.is_true(var_exists.array_get(1)) { var_zone_shadow.array_get(1) } else { rt.new_string('') } }, rt.ArrayItem{ key: 'subcity', val: if rt.is_true(var_exists.array_get(2)) { var_zone_shadow.array_get(2) } else { rt.new_string('') } }, rt.ArrayItem{ key: 't_continent', val: if rt.is_true(var_exists.array_get(3)) { rt.call_function('translate', [rt.call_function('str_replace', [rt.new_string('_'), rt.new_string(' '), var_zone_shadow.array_get(0)]), rt.new_string('continents-cities')]) } else { rt.new_string('') } }, rt.ArrayItem{ key: 't_city', val: if rt.is_true(var_exists.array_get(4)) { rt.call_function('translate', [rt.call_function('str_replace', [rt.new_string('_'), rt.new_string(' '), var_zone_shadow.array_get(1)]), rt.new_string('continents-cities')]) } else { rt.new_string('') } }, rt.ArrayItem{ key: 't_subcity', val: if rt.is_true(var_exists.array_get(5)) { rt.call_function('translate', [rt.call_function('str_replace', [rt.new_string('_'), rt.new_string(' '), var_zone_shadow.array_get(2)]), rt.new_string('continents-cities')]) } else { rt.new_string('') } }]))
		}
	}
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.call_function('in_array', [var_selected_zone.clone(), var_tz_identifiers.clone(), rt.new_bool(true)]), rt.new_bool(false))) && rt.is_true(rt.call_function('in_array', [var_selected_zone.clone(), rt.call_function('timezone_identifiers_list', [Class_DateTimeZone.all_with_bc()]), rt.new_bool(true)])))) {
	...
	{
		mut iter_1 := var_zonen.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_zone_shadow := item_1.val
			mut var_key_shadow := item_1.key
			var_value = rt.create_array([rt.ArrayItem{ key: none, val: var_zone_shadow.array_get('continent') }])
			if !rt.is_true(var_zone_shadow.array_get('city')) {
				var_display = var_zone_shadow.array_get('t_continent')
			} else {
				if rt.is_true(rt.new_bool(!(var_zonen.array_isset(rt.sub(var_key_shadow, rt.new_int(1)))) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_zonen.array_get(rt.sub(var_key_shadow, rt.new_int(1))).array_get('continent'), var_zone_shadow.array_get('continent'))))))) {
				...
				if !(!rt.is_true(var_zone_shadow.array_get('subcity'))) {
					...
					var_display = rt.concat(var_display, rt.new_string(' - ' + (var_zone_shadow.array_get('t_subcity')).str()))
				}
			}
			...
			if rt.is_true(rt.new_bool(!(!rt.is_true(var_zone_shadow.array_get('city'))) && rt.is_true(rt.new_bool(!(var_zonen.array_isset(rt.add(var_key_shadow, rt.new_int(1)))) || rt.is_true(rt.new_bool(var_zonen.array_isset(rt.add(var_key_shadow, rt.new_int(1))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_zonen.array_get(rt.add(var_key_shadow, rt.new_int(1))).array_get('continent'), var_zone_shadow.array_get('continent'))))))))))) {
...
fn wp_scheduled_delete() {
	...
	{
		mut iter_1 := rt.cast_array(var_posts_to_delete).iterator()
		for {
			item_1 := iter_1.next() or { break }
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_del_post)))) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('trash'), rt.get_property(var_del_post, 'post_status'))))))) {
	...
	{
		mut iter_1 := rt.cast_array(var_comments_to_delete).iterator()
		for {
			item_1 := iter_1.next() or { break }
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_del_comment)))) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('trash'), rt.get_property(var_del_comment, 'comment_approved'))))))) {
...
fn get_file_data(var_file rt.PhpVal, var_default_headers rt.PhpVal, context string) rt.PhpVal {
	...
	{
		mut iter_1 := var_all_headers.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_regex_shadow := item_1.val
			mut var_field_shadow := item_1.key
			if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('preg_match', [rt.new_string('/^(?:[ \\t]*<\\?php)?[ \\t\\/*#@]*' + (rt.call_function('preg_quote', [var_regex_shadow.clone(), rt.new_string('/')])).str() + ':(.*)$/mi'), var_file_data.clone(), rt.create_array_from_list(var_match)])) && rt.is_true(var_match.array_get(1)))) {
				var_all_headers.array_set(var_field_shadow, _cleanup_header_comment(var_match.array_get(1)))
			} else {
...
fn _wp_mysql_week(var_column rt.PhpVal) {
	...
	if rt.is_true(rt.equal(switch_val_3, rt.new_int(1))) {
		...
		rt.new_string("WEEK( DATE_SUB( ${var_column.to_string()}, INTERVAL ${var_start_of_week.to_string()} DAY ), 0 )")
		return
	} else {
		rt.new_string("WEEK( ${var_column.to_string()}, 0 )")
		return
	}
}
...
fn wp_find_hierarchy_loop(var_callback rt.PhpVal, var_start rt.PhpVal, var_start_parent rt.PhpVal, var_callback_args rt.PhpVal) rt.PhpVal {
	...
	var_override = if rt.is_true(rt.new_bool(var_start_parent.clone().is_null())) { []rt.PhpVal{} } else { rt.create_array([rt.ArrayItem{ key: var_start, val: var_start_parent }]) }
...
fn wp_find_hierarchy_loop_tortoise_hare(var_callback rt.PhpVal, var_start rt.PhpVal, var_override rt.PhpVal, var_callback_args rt.PhpVal, _return_loop bool) bool {
	...
	for rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_tortoise) && rt.is_true(var_evanescent_hare = if !(var_override.array_get(var_hare)).is_null() { var_override.array_get(var_hare) } else { rt.call_function('call_user_func_array', [var_callback.clone(), rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: none, val: var_hare }]), var_callback_args.clone()])]) }))) && rt.is_true(var_hare = if !(var_override.array_get(var_evanescent_hare)).is_null() { var_override.array_get(var_evanescent_hare) } else { rt.call_function('call_user_func_array', [var_callback.clone(), rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: none, val: var_evanescent_hare }]), var_callback_args.clone()])]) }))) {
		if var__return_loop {
			var_return.array_set(var_tortoise, true)
			var_return.array_set(var_evanescent_hare, true)
			var_return.array_set(var_hare, true)
		}
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_tortoise, var_evanescent_hare)) || rt.is_true(rt.identical(var_tortoise, var_hare)))) {
...
fn wp_debug_backtrace_summary(var_ignore_class rt.PhpVal, skip_frames i64, pretty bool) rt.PhpVal {
	...
	}
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_call_shadow := item_1.val
			if skip_frames > 0 {
				var_skip_frames -= 1
			} else if var_call_shadow.array_isset(rt.new_string('class')) {
				if rt.is_true(rt.new_bool(var_check_class && rt.is_true(rt.identical(var_ignore_class, var_call_shadow.array_get('class'))))) {
					continue
				}
				var_caller << rt.concat(rt.concat(var_call_shadow.array_get('class'), var_call_shadow.array_get('type')), var_call_shadow.array_get('function'))
			} else {
				if rt.is_true(rt.call_function('in_array', [var_call_shadow.array_get('function'), rt.create_array([rt.ArrayItem{ key: none, val: 'do_action' }, rt.ArrayItem{ key: none, val: 'apply_filters' }, rt.ArrayItem{ key: none, val: 'do_action_ref_array' }, rt.ArrayItem{ key: none, val: 'apply_filters_ref_array' }]), rt.new_bool(true)])) {
					var_caller << rt.concat(rt.concat(rt.concat(var_call_shadow.array_get('function'), rt.new_string('(\'')), var_call_shadow.array_get('args').array_get(0)), rt.new_string('\')'))
				} else if rt.is_true(rt.call_function('in_array', [var_call_shadow.array_get('function'), rt.create_array([rt.ArrayItem{ key: none, val: 'include' }, rt.ArrayItem{ key: none, val: 'include_once' }, rt.ArrayItem{ key: none, val: 'require' }, rt.ArrayItem{ key: none, val: 'require_once' }]), rt.new_bool(true)])) {
					var_filename = if !(var_call_shadow.array_get('args').array_get(0)).is_null() { var_call_shadow.array_get('args').array_get(0) } else { rt.new_string('') }
					var_caller << (var_call_shadow.array_get('function')).str() + '(\'' + (rt.call_function('str_replace', [rt.create_array_from_list(var_truncate_paths), rt.new_string(''), rt.new_string(wp_normalize_path(var_filename.clone()))])).str() + '\')'
				} else {
...
fn _validate_cache_id(var_object_id rt.PhpVal) bool {
	mut var_message := rt.new_null()
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_object_id.clone().is_long())) || rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_object_id.clone().is_string())) && rt.is_true(rt.identical(rt.new_int((var_object_id).to_i64()).str(), var_object_id)))))) {
		return true
	}
...
fn _device_can_upload() bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('str_contains', [var_ua.clone(), rt.new_string('iPhone')])) || rt.is_true(rt.call_function('str_contains', [var_ua.clone(), rt.new_string('iPad')])))) || rt.is_true(rt.call_function('str_contains', [var_ua.clone(), rt.new_string('iPod')])))) {
		return rt.is_true(rt.call_function('preg_match', [rt.new_string('#OS ([\\d_]+) like Mac OS X#'), var_ua.clone(), var_version.clone()])) && rt.is_true(rt.call_function('version_compare', [var_version.array_get(1), rt.new_string('6'), rt.new_string('>=')]))
	}
	return true
}
...
fn wp_checkdate(var_month rt.PhpVal, var_day rt.PhpVal, var_year rt.PhpVal, var_source_date rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_month.clone().is_long() || var_month.clone().is_double())) && rt.is_true(rt.new_bool(var_day.clone().is_long() || var_day.clone().is_double())))) && rt.is_true(rt.new_bool(var_year.clone().is_long() || var_year.clone().is_double())))) {
...
fn wp_auth_check_load() {
	...
	mut var_show := false
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_admin', []rt.PhpVal{}))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{}))))))) {
		return
	}
...
fn wp_auth_check_html() {
	...
	var_current_domain = rt.new_string((if rt.is_true(rt.call_function('is_ssl', []rt.PhpVal{})) { 'https://' } else { 'http://' } + (rt.get_superglobal('_SERVER').array_get('HTTP_HOST')).str()).str())
...
fn wp_auth_check(var_response rt.PhpVal) rt.PhpVal {
	...
	var_response.array_set('wp-auth-check', rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})) && !rt.is_true(var_GLOBALS.array_get('login_grace_period')))
...
fn _canonical_charset(var_charset rt.PhpVal) string {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strcasecmp', [rt.new_string('iso-8859-1'), var_charset.clone()]))) || rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strcasecmp', [rt.new_string('iso8859-1'), var_charset.clone()]))))) {
		return 'ISO-8859-1'
	}
...
fn mbstring_binary_safe_encoding(reset bool) {
	...
	if rt.is_true(rt.new_bool(rt.new_bool(var_overloaded).clone().is_null())) {
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('function_exists', [rt.new_string('mb_internal_encoding')])) && rt.is_true(rt.new_int((rt.call_function('ini_get', [rt.new_string('mbstring.func_overload')])).to_i64()) & 2))) {
			var_overloaded = true
		} else {
...
fn wp_validate_boolean(var_value rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_value.clone().is_string())) && rt.is_true(rt.identical(rt.new_string('false'), rt.new_string(var_value.clone().to_string().to_lower()))))) {
		return false
	}
	return (var_value).to_bool()
}
...
fn wp_delete_file_from_directory(var_file rt.PhpVal, var_directory rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_bool(false), var_real_file)) || rt.is_true(rt.identical(rt.new_bool(false), var_real_directory)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_starts_with', [var_real_file.clone(), rt.call_function('trailingslashit', [var_real_directory.clone()])]))))))) {
		return false
	}
...
fn wp_post_preview_js() {
	mut var_post := rt.new_null()
	mut var_name := rt.new_null()
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_preview', []rt.PhpVal{}))))) || !rt.is_true(var_post))) {
		return
	}
...
fn wp_raise_memory_limit(context string) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(-1, var_filtered_limit_int)) || rt.is_true(rt.new_bool(rt.is_true(rt.greater(var_filtered_limit_int, var_wp_max_limit_int)) && rt.is_true(rt.greater(var_filtered_limit_int, var_current_limit_int)))))) {
	...
	} else if rt.is_true(rt.new_bool(rt.is_true(rt.identical(-1, var_wp_max_limit_int)) || rt.is_true(rt.greater(var_wp_max_limit_int, var_current_limit_int)))) {
...
fn wp_unique_id_from_values(var_data rt.PhpVal, prefix string) string {
	...
	var_serialized = wp_json_encode(var_data.clone())
	var_hash = rt.call_function('substr', [rt.new_string(md5.hexhash(var_serialized)), rt.new_int(0), rt.new_int(8)])
...
fn wp_site_admin_email_change_notification(var_old_email rt.PhpVal, var_new_email rt.PhpVal, var_option_name rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(var_old_email) || rt.is_true(rt.identical(rt.new_string('you@example.com'), var_old_email)))) {
	...
	var_email_change_email.array_set('message', rt.call_function('str_replace', [rt.new_string('###OLD_EMAIL###'), var_old_email.clone(), var_email_change_email.array_get('message')]))
	var_email_change_email.array_set('message', rt.call_function('str_replace', [rt.new_string('###NEW_EMAIL###'), var_new_email.clone(), var_email_change_email.array_get('message')]))
	var_email_change_email.array_set('message', rt.call_function('str_replace', [rt.new_string('###SITENAME###'), var_site_name.clone(), var_email_change_email.array_get('message')]))
	var_email_change_email.array_set('message', rt.call_function('str_replace', [rt.new_string('###SITEURL###'), rt.call_function('home_url', []rt.PhpVal{}), var_email_change_email.array_get('message')]))
	rt.call_function('wp_mail', [var_email_change_email.array_get('to'), rt.call_function('sprintf', [var_email_change_email.array_get('subject'), var_site_name.clone()]), var_email_change_email.array_get('message'), var_email_change_email.array_get('headers')])
}
...
fn wp_privacy_anonymize_ip(var_ip_addr_arg rt.PhpVal, ipv6_fallback bool) string {
	...
	if var_is_ipv6 {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_left_bracket)))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_right_bracket)))))) {
		...
		} else if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_left_bracket)))) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_right_bracket)))))) {
			return '::'
		}
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('function_exists', [rt.new_string('inet_pton')])) && rt.is_true(rt.call_function('function_exists', [rt.new_string('inet_ntop')])))) {
...
fn wp_privacy_exports_dir() rt.PhpVal {
	...
	var_exports_dir = rt.new_string((rt.call_function('trailingslashit', [var_upload_dir.array_get('basedir')])).str() + 'wp-personal-data-exports/')
...
fn wp_privacy_exports_url() rt.PhpVal {
	...
	var_upload_dir = wp_upload_dir(rt.new_null(), false, false)
	var_exports_url = rt.new_string((rt.call_function('trailingslashit', [var_upload_dir.array_get('baseurl')])).str() + 'wp-personal-data-exports/')
...
fn wp_privacy_delete_old_export_files() {
	...
	{
		mut iter_1 := rt.cast_array(var_export_files).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_export_file_shadow := item_1.val
...
fn get_dirsize(var_directory rt.PhpVal, var_max_execution_time rt.PhpVal) rt.PhpVal {
	mut var_size := rt.new_null()
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{})) && is_main_site())) {
...
fn recurse_dirsize(var_directory_arg rt.PhpVal, var_exclude rt.PhpVal, var_max_execution_time_arg rt.PhpVal, var_directory_cache_arg rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(var_directory_cache.array_isset(var_directory) && rt.is_true(rt.new_bool(var_directory_cache.array_get(var_directory).is_long())))) {
		return (var_directory_cache.array_get(var_directory)).to_bool()
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('file_exists', [var_directory.clone()]))))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_dir', [var_directory.clone()]))))))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_readable', [var_directory.clone()]))))))) {
		return false
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_exclude.clone().is_string())) && rt.is_true(rt.identical(var_directory, var_exclude)))) || rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_exclude.clone().is_array())) && rt.is_true(rt.call_function('in_array', [var_directory.clone(), var_exclude.clone(), rt.new_bool(true)])))))) {
		return false
	}
	...
	if rt.is_true(rt.identical(rt.new_bool(false), var_size)) {
		...
		if rt.is_true(var_handle) {
			for rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_file = rt.call_function('readdir', [var_handle.clone()]), rt.new_bool(false))))) {
				var_path = rt.new_string((var_directory).str() + '/' + (var_file).str())
				if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('.'), var_file)))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('..'), var_file)))))) {
					...
					if rt.is_true(rt.new_bool(rt.is_true(rt.greater(var_max_execution_time, rt.new_int(0))) && rt.is_true(rt.greater(rt.sub(rt.call_function('microtime', [rt.new_bool(true)]), rt.get_constant('WP_START_TIMESTAMP')), var_max_execution_time)))) {
...
fn clean_dirsize_cache(var_path_arg rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_path.clone().is_string()))))) || !rt.is_true(var_path))) {
	...
	var_expiration = if rt.is_true(rt.call_function('wp_using_ext_object_cache', []rt.PhpVal{})) { rt.new_int(0) } else { rt.mul(rt.new_int(10), rt.get_constant('YEAR_IN_SECONDS')) }
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_path.clone(), rt.new_string('/')]))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_path.clone(), rt.new_string('\\')]))))))) {
...
fn is_wp_version_compatible(var_required_arg rt.PhpVal) bool {
	...
	mut var_wp_version := rt.new_null()
	mut var_trimmed := ''
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('WP_RUN_CORE_TESTS')])) && rt.is_true(rt.get_constant('WP_RUN_CORE_TESTS')))) && var_GLOBALS.array_isset(rt.new_string('_wp_tests_wp_version')))) {
		var_wp_version = var_GLOBALS.array_get('_wp_tests_wp_version')
	} else {
		var_wp_version = wp_get_wp_version()
	}
	mut _list_tmp_-58077 := rt.call_function('explode', [rt.new_string('-'), var_wp_version.clone()])
	if rt.is_true(rt.new_bool(var_required.clone().is_string())) {
		var_trimmed = var_required.clone().to_string().trim_space()
		if rt.is_true(rt.new_bool(rt.is_true(rt.greater(rt.call_function('substr_count', [rt.new_string((var_trimmed).str()).clone(), rt.new_string('.')]), rt.new_int(1))) && rt.is_true(rt.call_function('str_ends_with', [rt.new_string((var_trimmed).str()).clone(), rt.new_string('.0')])))) {
			var_required = rt.call_function('substr', [rt.new_string((var_trimmed).str()).clone(), rt.new_int(0), -2])
		}
	}
...
fn wp_get_admin_notice(var_message_arg rt.PhpVal, var_args_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_args.array_get('additional_classes').is_array())) && !(!rt.is_true(var_args.array_get('additional_classes'))))) {
		var_classes = var_classes + ' ' + (rt.call_function('implode', [rt.new_string(' '), var_args.array_get('additional_classes')])).str()
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_args.array_get('attributes').is_array())) && !(!rt.is_true(var_args.array_get('attributes'))))) {
		var_attributes = ''
		{
			mut iter_1 := var_args.array_get('attributes').iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_val_shadow := item_1.val
				mut var_attr_shadow := item_1.key
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_args.array_get('paragraph_wrap'))))) {
...
fn init_registry() {
	...
	rt.register_func('wp_json_encode', fn(args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_string(wp_json_encode(arg_0, arg_1, arg_2))
	})
...
pub fn init_wp_includes_functions_php() {
