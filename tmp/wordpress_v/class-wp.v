...
fn (mut this Class_WP) parse_request(extra_query_vars string) bool {
	...
	if !(!rt.is_true(var_rewrite)) {
		...
		mut _list_tmp_-45738 := rt.call_function('explode', [rt.new_string('?'), var_pathinfo.clone()])
		var_pathinfo = rt.call_function('str_replace', [rt.new_string('%'), rt.new_string('%25'), var_pathinfo.clone()])
		mut _list_tmp_-11620 := rt.call_function('explode', [rt.new_string('?'), rt.get_superglobal('_SERVER').array_get('REQUEST_URI')])
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_home_path.clone().is_string())) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_home_path)))))) {
		...
		mut var_req_uri := rt.call_function('str_replace', [var_pathinfo.clone(), rt.new_string(''), var_req_uri.clone()])
		...
		if rt.is_true(rt.new_bool(!(!rt.is_true(var_pathinfo)) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('|^.*' + (rt.get_property(var_wp_rewrite, 'index')).str() + '$|'), var_pathinfo.clone()]))))))) {
		...
		if !rt.is_true(var_request_match) {
		...
		} else {
			{
				...
				for {
					...
					if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(!rt.is_true(var_requested_file)) && rt.is_true(rt.call_function('str_starts_with', [var_match.clone(), var_requested_file.clone()])))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_requested_file, var_requested_path)))))) {
						var_request_match = rt.new_string((var_requested_file).str() + '/' + (var_requested_path).str())
					}
					if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('preg_match', [rt.new_string("#^${var_match.to_string()}#"), var_request_match.clone(), var_matches.clone()])) || rt.is_true(rt.call_function('preg_match', [rt.new_string("#^${var_match.to_string()}#"), rt.call_function('urldecode', [var_request_match.clone()]), var_matches.clone()])))) {
						if rt.is_true(rt.new_bool(rt.is_true(rt.get_property(var_wp_rewrite, 'use_verbose_page_rules')) && rt.is_true(rt.call_function('preg_match', [rt.new_string('/pagename=\\$matches\\[([0-9]+)\\]/'), var_query_shadow.clone(), rt.create_array_from_list(var_varmatch)])))) {
		...
		if !(!rt.is_true(this.matched_rule)) {
			...
			var_query = rt.call_function('addslashes', [fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_WP_MatchesMapRegex{}; return temp.apply(arg_0, arg_1) }(var_query.clone(), var_matches.clone())])
	...
	this.public_query_vars = rt.call_function('apply_filters', [rt.new_string('query_vars'), this.public_query_vars])
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_post_type_viewable', [var_t.clone()])) && rt.is_true(rt.get_property(var_t, 'query_var')))) {
	...
	}
	{
		...
		for {
			...
			} else if rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(var_wpvar) && rt.get_superglobal('_POST').array_isset(var_wpvar) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_superglobal('_GET').array_get(var_wpvar), rt.get_superglobal('_POST').array_get(var_wpvar))))))) {
	...
	if this.query_vars.array_isset(rt.new_string('post_type')) {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.query_vars.array_get('post_type').is_array()))))) {
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [this.query_vars.array_get('post_type'), var_queryable_post_types.clone(), rt.new_bool(true)]))))) {
				this.query_vars.array_unset(rt.new_string('post_type'))
			}
		} else {
			this.query_vars.array_set('post_type', rt.call_function('array_intersect', [this.query_vars.array_get('post_type'), var_queryable_post_types.clone()]))
		}
	}
...
fn (mut this Class_WP) send_headers()  {
	...
	if !(!rt.is_true(this.query_vars.array_get('error'))) {
		var_status = rt.new_int((this.query_vars.array_get('error')).to_i64())
	...
	} else if !rt.is_true(this.query_vars.array_get('feed')) {
		var_headers.array_set('Content-Type', (rt.call_function('get_option', [rt.new_string('html_type')])).str() + '; charset=' + (rt.call_function('get_option', [rt.new_string('blog_charset')])).str())
	} else {
		mut var_type := this.query_vars.array_get('feed')
		if rt.is_true(rt.identical(rt.new_string('feed'), this.query_vars.array_get('feed'))) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(!rt.is_true(this.query_vars.array_get('withcomments'))) || rt.is_true(rt.call_function('str_contains', [this.query_vars.array_get('feed'), rt.new_string('comments-')])))) || !rt.is_true(this.query_vars.array_get('withoutcomments')) && !(!rt.is_true(this.query_vars.array_get('p'))) || !(!rt.is_true(this.query_vars.array_get('name'))) || !(!rt.is_true(this.query_vars.array_get('page_id'))) || !(!rt.is_true(this.query_vars.array_get('pagename'))) || !(!rt.is_true(this.query_vars.array_get('attachment'))) || !(!rt.is_true(this.query_vars.array_get('attachment_id'))))) {
		...
		if rt.get_superglobal('_SERVER').array_isset(rt.new_string('HTTP_IF_NONE_MATCH')) {
			mut var_client_etag := rt.call_function('wp_unslash', [rt.get_superglobal('_SERVER').array_get('HTTP_IF_NONE_MATCH')])
		} else {
			var_client_etag = rt.new_string('')
		}
		if rt.get_superglobal('_SERVER').array_isset(rt.new_string('HTTP_IF_MODIFIED_SINCE')) {
			mut var_client_last_modified := rt.new_string(rt.get_superglobal('_SERVER').array_get('HTTP_IF_MODIFIED_SINCE').to_string().trim_space())
		} else {
	...
	if rt.is_true(rt.call_function('is_singular', []rt.PhpVal{})) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(var_post) && rt.is_true(rt.call_function('pings_open', [var_post.clone()])))) {
	...
	if rt.is_true(rt.new_bool(var_headers.array_isset(rt.new_string('Last-Modified')) && rt.is_true(rt.identical(rt.new_bool(false), var_headers.array_get('Last-Modified'))))) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('headers_sent', []rt.PhpVal{}))))) {
		{
			mut iter_1 := rt.cast_array(var_headers).iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_field_value := item_1.val
...
fn (mut this Class_WP) build_query_string()  {
	...
	{
		mut iter_1 := rt.cast_array(rt.func_array_keys(this.query_vars)).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_wpvar := item_1.val
...
fn (mut this Class_WP) register_globals()  {
	...
	{
		mut iter_1 := rt.cast_array(rt.get_property(var_wp_query, 'query_vars')).iterator()
		for {
			item_1 := iter_1.next() or { break }
	...
	var_GLOBALS.array_get('posts') = rt.get_property(var_wp_query, 'posts')
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_method(var_wp_query, 'is_single', []rt.PhpVal{})) || rt.is_true(rt.call_method(var_wp_query, 'is_page', []rt.PhpVal{})))) {
...
fn (mut this Class_WP) handle_404()  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) || rt.is_true(rt.call_function('is_robots', []rt.PhpVal{})))) || rt.is_true(rt.call_function('is_favicon', []rt.PhpVal{})))) {
		...
		if rt.is_true(rt.call_function('is_singular', []rt.PhpVal{})) {
			...
			if rt.is_true(rt.new_bool(rt.is_true(var_post) && !(!rt.is_true(this.query_vars.array_get('page'))))) {
				if rt.is_true(rt.call_function('str_contains', [rt.get_property(var_post, 'post_content'), var_next.clone()])) {
					mut var_page := rt.new_string(this.query_vars.array_get('page').to_string().trim_space())
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.get_property(var_wp_query, 'is_posts_page')) && !(!rt.is_true(this.query_vars.array_get('page'))))) {
			var_content_found = rt.new_bool(false)
		}
