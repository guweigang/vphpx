...
fn (mut this Class_WP_Abilities_Registry) register(name string, mut var_args Class_array) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('/^[a-z0-9-]+\\/[a-z0-9-]+$/'), rt.new_string(name)]))))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD), rt.call_function('__', [rt.new_string('Ability name must be a string containing a namespace prefix, i.e. "my-plugin/my-ability". It can only contain lowercase alphanumeric characters, dashes and the forward slash.')]), rt.new_string('6.9.0')])
		return rt.new_null()
	}
	if this.is_registered(name) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Ability "%s" is already registered.')]), rt.call_function('esc_html', [rt.new_string(name)])]), rt.new_string('6.9.0')])
		return rt.new_null()
	}
	var_args_mutated = rt.call_function('apply_filters', [rt.new_string('wp_register_ability_args'), var_args_mutated.dup(), rt.new_string(name)])
	if var_args_mutated.array_isset(rt.new_string('category')) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_has_ability_category', [var_args_mutated.array_get('category')]))))) {
			rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Ability category "%1$s" is not registered. Please register the ability category before assigning it to ability "%2$s".')]), rt.call_function('esc_html', [var_args_mutated.array_get('category')]), rt.call_function('esc_html', [rt.new_string(name)])]), rt.new_string('6.9.0')])
			return rt.new_null()
		}
	}
	if rt.is_true(rt.new_bool(var_args_mutated.array_isset(rt.new_string('ability_class')) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [var_args_mutated.array_get('ability_class'), Class_WP_Ability.class(), rt.new_bool(true)]))))))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD), rt.call_function('__', [rt.new_string('The ability args should provide a valid `ability_class` that extends WP_Ability.')]), rt.new_string('6.9.0')])
		return rt.new_null()
	}
	mut var_ability_class := if !(var_args_mutated.array_get('ability_class')).is_null() { var_args_mutated.array_get('ability_class') } else { Class_WP_Ability.class() }
	var_args_mutated.array_unset(rt.new_string('ability_class'))
	mut var_ability := rt.create_object_dynamically(var_ability_class, [rt.new_string(name), var_args_mutated.dup()])
...
end_label_1:
	this.registered_abilities.array_set(name, var_ability.dup())
	return var_ability.dup()
}

fn (mut this Class_WP_Abilities_Registry) unregister(name string) rt.PhpVal {
	if !(this.is_registered(name)) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Ability "%s" not found.')]), rt.call_function('esc_html', [rt.new_string(name)])]), rt.new_string('6.9.0')])
		return rt.new_null()
	}
	mut var_unregistered_ability := this.registered_abilities.array_get(name)
	this.registered_abilities.array_unset(rt.new_string(name))
	return var_unregistered_ability.dup()
}
...
fn (mut this Class_WP_Abilities_Registry) get_registered(name string) rt.PhpVal {
	...
	return this.registered_abilities.array_get(name)
}
...
fn Class_WP_Abilities_Registry.get_instance() rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_null(), // unsupported expression: Expr_StaticPropertyFetch)) {
		...
		fn () rt.PhpVal { mut temp := Class_WP_Ability_Categories_Registry{}; return temp.get_instance() }()
		rt.call_function('do_action', [rt.new_string('wp_abilities_api_init'), // unsupported expression: Expr_StaticPropertyFetch])
	}
...
fn (mut this Class_WP_Abilities_Registry) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'registered_abilities' { this.registered_abilities = val; return true }
...
pub fn init_wp_includes_abilities_api_class_wp_abilities_registry_php() {
