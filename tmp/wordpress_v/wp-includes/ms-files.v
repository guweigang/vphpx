...
pub fn init_wp_includes_ms_files_php() {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('1'), rt.get_property(var_current_blog, 'archived')))
	...
	mut var_file := rt.new_string(
		rt.get_constant('BLOGUPLOADDIR').to_string().trim_right(' \t\n\r') + '/' +(rt.call_function('str_replace', [rt.new_string('..'), rt.new_string(''), rt.get_superglobal('_GET').array_get('file')])).str())
	...
	mut var_mime := rt.call_function('wp_check_filetype', [var_file.dup()])
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.identical(rt.new_bool(false), var_mime.array_get('type')))
	...
	if rt.is_true(var_mime.array_get('type')) {
		mut var_mimetype := var_mime.array_get('type')
	} else {
		var_mimetype =
			rt.new_string('image/' +(rt.call_function('substr', [var_file.dup(), rt.add(rt.call_function('strrpos', [var_file.dup(), rt.new_string('.')]), rt.new_int(1))])).str())
	}
	rt.call_function('header', ['Content-Type: ' + var_mimetype.str()])
	...
	])))))
	{
		rt.call_function('header', [
			'Content-Length: ' + (rt.call_function('filesize', [var_file.dup()])).str(),
		])
	}
	if rt.is_true(rt.get_constant('WPMU_ACCEL_REDIRECT')) {
		rt.call_function('header', [
			'X-Accel-Redirect: ' +(rt.call_function('str_replace', [rt.get_constant('WP_CONTENT_DIR'), rt.new_string(''), var_file.dup()])).str(),
		])
		// unsupported expression: Expr_Exit
	} else if rt.is_true(rt.get_constant('WPMU_SENDFILE')) {
		rt.call_function('header', ['X-Sendfile: ' + var_file.str()])
		// unsupported expression: Expr_Exit
	}
	mut var_wp_last_modified := rt.call_function('gmdate', [
		rt.new_string('D, d M Y H:i:s'),
		rt.call_function('filemtime', [var_file.dup()]),
	])
	mut var_wp_etag :=
		rt.new_string('"' + md5.hexhash(var_wp_last_modified.dup().to_string()) + '"')
	rt.call_function('header', [
		rt.new_string('Last-Modified: ${var_wp_last_modified.to_string()} GMT'),
	])
	rt.call_function('header', ['ETag: ' + var_wp_etag.str()])
	rt.call_function('header', [
		'Expires: ' +
	...
	if rt.get_superglobal('_SERVER').array_isset(rt.new_string('HTTP_IF_NONE_MATCH')) {
		mut var_client_etag := rt.call_function('stripslashes', [
			rt.get_superglobal('_SERVER').array_get('HTTP_IF_NONE_MATCH'),
		])
	} else {
		var_client_etag = rt.new_string(rt.new_string(''))
	}
	if rt.get_superglobal('_SERVER').array_isset(rt.new_string('HTTP_IF_MODIFIED_SINCE')) {
		mut var_client_last_modified :=
			rt.get_superglobal('_SERVER').array_get('HTTP_IF_MODIFIED_SINCE').to_string().trim_space()
	} else {
	...
	mut var_client_modified_timestamp := if var_client_last_modified.len > 0 && var_client_last_modified != '0' { rt.call_function('strtotime', [
			rt.new_string(var_client_last_modified).dup(),
		]) } else { rt.new_int(0) }
	mut var_wp_modified_timestamp := rt.call_function('strtotime', [
		var_wp_last_modified.dup()])
	if rt.is_true(if rt.is_true(rt.new_bool(var_client_last_modified.len > 0
		&& var_client_last_modified != '0' && rt.is_true(var_client_etag)))
	{
		rt.new_bool(
	...
	rt.call_function('readfile', [var_file.dup()])
