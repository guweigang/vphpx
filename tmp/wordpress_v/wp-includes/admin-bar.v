...
fn wp_admin_bar_render() {
	...
	if rt.is_true(rt.new_bool(!(is_admin_bar_showing())
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_wp_admin_bar.clone().is_object())))))))
	{
		return
	}
...
fn wp_admin_bar_site_menu(var_wp_admin_bar rt.PhpVal) {
	...
	rt.call_method(var_wp_admin_bar, 'add_node', [
		rt.create_array([rt.ArrayItem{ key: 'id', val: 'site-name' },
			rt.ArrayItem{ key: 'title', val: var_title }, rt.ArrayItem{
				key: 'href'
				val: if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('read')]))))))) { rt.call_function('home_url', [
						rt.new_string('/'),
	...
	if rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_blog_admin', []rt.PhpVal{}))
...
fn wp_admin_bar_customize_menu(var_wp_admin_bar rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('customize')])))))
		|| rt.is_true(rt.call_function('is_admin', []rt.PhpVal{}))))
	{
		return
	}
	...
	var_current_url = rt.new_string((
		...
		(rt.get_superglobal('_SERVER').array_get('REQUEST_URI')).str()).str())
...
fn wp_admin_bar_my_sites_menu(var_wp_admin_bar rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(
		rt.get_property(rt.get_property(var_wp_admin_bar, 'user'), 'blogs').array_count() < 1
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('manage_network')])))))))
	{
		return
	}
	...
	{
		mut iter_1 := rt.cast_array(rt.get_property(rt.get_property(var_wp_admin_bar, 'user'),
			'blogs')).iterator()
		for {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.identical(rt.new_bool(true), var_show_site_icons))
				&& rt.is_true(rt.call_function('has_site_icon', []rt.PhpVal{}))))
			{
				var_blavatar = rt.call_function('sprintf', [
					...
					rt.call_function('esc_url', [
						rt.call_function('get_site_icon_url', [
							rt.new_int(16)]),
					]),
					rt.call_function('esc_url', [
						rt.call_function('get_site_icon_url', [
					...
					if rt.is_true(rt.call_function('wp_lazy_loading_enabled', [
						rt.new_string('img'),
...
fn wp_admin_bar_edit_menu(var_wp_admin_bar rt.PhpVal) {
	...
	if rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('post'), rt.get_property(var_current_screen, 'base')))
			|| rt.is_true(rt.identical(rt.new_string('edit-comments'), rt.get_property(var_current_screen, 'base')))))
			...
			&& rt.is_true(rt.get_property(var_post_type_object, 'show_in_admin_bar'))))
		{
			...
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('edit'), rt.get_property(var_current_screen, 'base')))
			&& rt.is_true(var_post_type_object)))
			&& rt.is_true(rt.get_property(var_post_type_object, 'public'))))
			&& rt.is_true(rt.get_property(var_post_type_object, 'show_in_admin_bar'))))
			&& rt.is_true(rt.call_function('get_post_type_archive_link', [rt.get_property(var_post_type_object, 'name')]))))
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('post'), rt.get_property(var_post_type_object, 'name')))
			&& rt.is_true(rt.identical(rt.new_string('posts'), rt.call_function('get_option', [rt.new_string('show_on_front')]))))))))))
		{
			...
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('term'), rt.get_property(var_current_screen, 'base')))
			&& !var_tag.is_null())) && rt.is_true(rt.new_bool(var_tag.clone().is_object()))))
			...
			&& !var_user_id.is_null()))
		{
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.call_method(var_user_object, 'exists', []rt.PhpVal{}))
		...
		if !(!rt.is_true(rt.get_property(var_current_object, 'post_type'))) {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_post_type_object)
				&& rt.is_true(var_edit_post_link)))
			...
			rt.is_true(rt.new_bool(rt.instance_of(var_current_object, 'WP_User')))
...
fn wp_admin_bar_command_palette_menu(var_wp_admin_bar rt.PhpVal) {
	...
	var_is_apple_os = rt.new_bool((rt.call_function('preg_match', [
		...
		if !(rt.get_superglobal('_SERVER').array_get('HTTP_USER_AGENT')).is_null() {
	...
	var_shortcut_label = if rt.is_true(var_is_apple_os) {
		var_shortcut_labels.array_get('appleOS')
	} else {
		var_shortcut_labels.array_get('default')
	}
...
fn wp_admin_bar_new_content_menu(var_wp_admin_bar rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(var_cpts.array_isset(rt.new_string('post'))
		&& rt.is_true(rt.call_function('current_user_can', [rt.get_property(rt.get_property(var_cpts.array_get('post'), 'cap'), 'create_posts')]))))
	{
		var_actions.array_set('post-new.php', rt.create_array([
			rt.ArrayItem{ key: none, val: rt.get_property(rt.get_property(var_cpts.array_get('post'),
				'labels'), 'name_admin_bar') },
			rt.ArrayItem{ key: none, val: 'new-post' },
		]))
	}
	if rt.is_true(rt.new_bool(var_cpts.array_isset(rt.new_string('attachment'))
		&& rt.is_true(rt.call_function('current_user_can', [rt.new_string('upload_files')]))))
	{
		var_actions.array_set('media-new.php', rt.create_array([
			rt.ArrayItem{ key: none, val: rt.get_property(rt.get_property(var_cpts.array_get('attachment'),
				'labels'), 'name_admin_bar') },
	...
	if rt.is_true(rt.new_bool(var_cpts.array_isset(rt.new_string('page'))
		&& rt.is_true(rt.call_function('current_user_can', [rt.get_property(rt.get_property(var_cpts.array_get('page'), 'cap'), 'create_posts')]))))
	{
		var_actions.array_set('post-new.php?post_type=page', rt.create_array([
			rt.ArrayItem{ key: none, val: rt.get_property(rt.get_property(var_cpts.array_get('page'),
				'labels'), 'name_admin_bar') },
	...
	{
		mut iter_1 := var_cpts.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_cpt_shadow := item_1.val
			...
			var_actions.array_set(var_key, rt.create_array([
				rt.ArrayItem{ key: none, val: rt.get_property(rt.get_property(var_cpt_shadow,
					'labels'), 'name_admin_bar') },
	...
	])
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_action_shadow := item_1.val
			mut var_link_shadow := item_1.key
			// unsupported assign target: Expr_List
			rt.call_method(var_wp_admin_bar, 'add_node', [
				rt.create_array([rt.ArrayItem{ key: 'parent', val: 'new-content' },
					rt.ArrayItem{ key: 'id', val: var_id }, rt.ArrayItem{
						key: 'title'
						val: var_title
					}, rt.ArrayItem{ key: 'href', val: rt.call_function('admin_url', [
...
fn wp_admin_bar_updates_menu(var_wp_admin_bar rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_update_data.array_get('counts').array_get('total'))))) {
		return
	}
	var_updates_text = rt.call_function('sprintf', [
		rt.call_function('_n', [rt.new_string('%s update available'),
			rt.new_string('%s updates available'), var_update_data.array_get('counts').array_get('total')]),
	...
	var_title = rt.new_string('<span class="ab-label" aria-hidden="true">' +
		(rt.call_function('number_format_i18n', [var_update_data.array_get('counts').array_get('total')])).str() +
		'</span>')
	var_title = rt.concat(var_title, rt.new_string(
...
fn wp_enqueue_admin_bar_bump_styles() {
		...
		rt.new_string('admin-bar')]))
	{
		...
		var_header_callback = var_admin_bar_args.array_get(0).array_get('callback')
	}
...
fn is_admin_bar_showing() bool {
	...
	mut var_show_admin_bar := rt.new_null()
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('XMLRPC_REQUEST')]))
...
pub fn init_wp_includes_admin_bar_php() {
}
