import rt

struct Class_WP_Font_Utils {
	rt.PhpObjectBase
}

fn Class_WP_Font_Utils.maybe_add_quotes(var_item rt.PhpVal) string {
	mut var_item_mutated := var_item
	mut var_regex :=
		rt.new_string(rt.new_string('/^(?!generic\\([a-zA-Z\\-]+\\)$)(?!^[a-zA-Z\\-]+$).+/'))
	var_item_mutated = rt.new_string(rt.new_string(var_item_mutated.dup().to_string().trim_space()))
	if rt.is_true(rt.call_function('preg_match', [var_regex.dup(),
		var_item_mutated.dup()]))
	{
		var_item_mutated =
			rt.new_string(rt.new_string(var_item_mutated.dup().to_string().trim_space()))
		return '"' + var_item_mutated.str() + '"'
	}
	return var_item_mutated.str()
}

fn Class_WP_Font_Utils.sanitize_font_family(var_font_family rt.PhpVal) string {
	mut var_font_family_mutated := var_font_family
	if rt.is_true(rt.new_bool(!(rt.is_true(var_font_family_mutated)))) {
		return ''
	}
	mut var_output := rt.call_function('sanitize_text_field', [
		var_font_family_mutated.dup()])
	mut var_formatted_items := []rt.PhpVal{}
	if rt.is_true(rt.call_function('str_contains', [var_output.dup(),
		rt.new_string(',')]))
	{
		mut var_items := rt.call_function('explode', [rt.new_string(','),
			var_output.dup()])
		{
			mut iter_1 := var_items.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_item := item_1.val
				mut var_formatted_item := Class_WP_Font_Utils.maybe_add_quotes(var_item.dup())
				if !(!rt.is_true(var_formatted_item)) {
					var_formatted_items << var_formatted_item.dup()
				}
			}
		}
		return (rt.call_function('implode', [rt.new_string(', '),
			var_formatted_items.dup()])).str()
	}
	return (Class_WP_Font_Utils.maybe_add_quotes(var_output.dup())).str()
}

fn Class_WP_Font_Utils.get_font_face_slug(var_settings rt.PhpVal) rt.PhpVal {
	mut var_settings_mutated := var_settings
	mut var_defaults := {
		'fontFamily':   ''
		'fontStyle':    'normal'
		'fontWeight':   '400'
		'fontStretch':  '100%'
		'unicodeRange': 'U+0-10FFFF'
	}
	var_settings_mutated = rt.call_function('wp_parse_args', [
		var_settings_mutated.dup(), var_defaults.dup()])
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('mb_strtolower')])) {
		mut var_font_family := rt.call_function('mb_strtolower', [
			var_settings_mutated.array_get('fontFamily'),
		])
	} else {
		var_font_family =
			rt.new_string(rt.new_string(var_settings_mutated.array_get('fontFamily').to_string().to_lower()))
	}
	mut var_font_style :=
		rt.new_string(rt.new_string(var_settings_mutated.array_get('fontStyle').to_string().to_lower()))
	mut var_font_weight :=
		rt.new_string(rt.new_string(var_settings_mutated.array_get('fontWeight').to_string().to_lower()))
	mut var_font_stretch :=
		rt.new_string(rt.new_string(var_settings_mutated.array_get('fontStretch').to_string().to_lower()))
	mut var_unicode_range :=
		rt.new_string(rt.new_string(var_settings_mutated.array_get('unicodeRange').to_string().to_upper()))
	var_font_weight = rt.call_function('str_replace', [
		rt.create_array([rt.ArrayItem{ key: none, val: 'normal' },
			rt.ArrayItem{ key: none, val: 'bold' }]),
		rt.create_array([rt.ArrayItem{ key: none, val: '400' },
			rt.ArrayItem{ key: none, val: '700' }]),
		var_font_weight.dup(),
	])
	mut var_font_stretch_map := {
		'ultra-condensed': '50%'
		'extra-condensed': '62.5%'
		'condensed':       '75%'
		'semi-condensed':  '87.5%'
		'normal':          '100%'
		'semi-expanded':   '112.5%'
		'expanded':        '125%'
		'extra-expanded':  '150%'
		'ultra-expanded':  '200%'
	}
	var_font_stretch = rt.call_function('str_replace', [
		rt.func_array_keys(var_font_stretch_map.dup()),
		rt.call_function('array_values', [var_font_stretch_map.dup()]),
		var_font_stretch.dup(),
	])
	mut var_slug_elements := rt.create_array([
		rt.ArrayItem{ key: none, val: var_font_family },
		rt.ArrayItem{ key: none, val: var_font_style },
		rt.ArrayItem{ key: none, val: var_font_weight },
		rt.ArrayItem{ key: none, val: var_font_stretch },
		rt.ArrayItem{ key: none, val: var_unicode_range },
	])
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_elem := if args.len > 0 { args[0].dup() } else { rt.new_null() }
			var_elem = rt.new_string(rt.new_string(rt.call_function('str_replace', [
				rt.create_array([rt.ArrayItem{ key: none, val: '"' },
					rt.ArrayItem{ key: none, val: "'" }, rt.ArrayItem{ key: none, val: ';' }]),
				rt.new_string(''),
				var_elem.dup(),
			]).to_string().trim_space()))
			return rt.call_function('preg_replace', [rt.new_string('/,\\s+/'),
				rt.new_string(','), var_elem.dup()])
		}
		mut var_elem := if args.len > 0 { args[0].dup() } else { rt.new_null() }
		var_elem = rt.new_string(rt.new_string(rt.call_function('str_replace', [
			rt.create_array([rt.ArrayItem{ key: none, val: '"' },
				rt.ArrayItem{ key: none, val: "'" }, rt.ArrayItem{ key: none, val: ';' }]),
			rt.new_string(''),
			var_elem.dup(),
		]).to_string().trim_space()))
		return rt.call_function('preg_replace', [rt.new_string('/,\\s+/'),
			rt.new_string(','), var_elem.dup()])
	}
	var_slug_elements = rt.call_function('array_map', [rt.new_closure(closure_1_fn),
		var_slug_elements.dup()])
	return rt.call_function('sanitize_text_field', [
		rt.call_function('implode', [rt.new_string(';'), var_slug_elements.dup()]),
	])
}

fn Class_WP_Font_Utils.sanitize_from_schema(var_tree rt.PhpVal, var_schema rt.PhpVal) rt.PhpVal {
	mut var_tree_mutated := var_tree
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_tree_mutated.dup().is_array())))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_schema.dup().is_array())))))))
	{
		return []rt.PhpVal{}
	}
	{
		mut iter_1 := var_tree_mutated.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_value := item_1.val
			mut var_key := item_1.key
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_schema.dup().array_isset(var_key.dup())))))) {
				var_tree_mutated.array_unset(var_key)
				continue
			}
			mut var_is_value_array := rt.new_bool(rt.new_bool(var_value.dup().is_array()))
			mut var_is_schema_array := rt.new_bool(rt.new_bool(
				rt.is_true(rt.new_bool(var_schema.array_get(var_key).is_array()))
				&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_callable', [var_schema.array_get(var_key)])))))))
			if rt.is_true(rt.new_bool(rt.is_true(var_is_value_array)
				&& rt.is_true(var_is_schema_array)))
			{
				if rt.is_true(rt.call_function('wp_is_numeric_array', [
					var_value.dup()]))
				{
					{
						mut iter_2 := var_value.iterator()
						for {
							item_2 := iter_2.next() or { break }
							mut var_item_value := item_2.val
							mut var_item_key := item_2.key
							var_tree_mutated.array_get_mut(var_key).array_set(var_item_key, if rt.is_true(rt.new_bool(
								var_schema.array_get(var_key).array_isset(rt.new_int(0))
								&& rt.is_true(rt.new_bool(var_schema.array_get(var_key).array_get(0).is_array()))))
							{
								Class_WP_Font_Utils.sanitize_from_schema(var_item_value.dup(),
									var_schema.array_get(var_key).array_get(0))
							} else {
								Class_WP_Font_Utils.apply_sanitizer(var_item_value.dup(),
									var_schema.array_get(var_key).array_get(0))
							})
						}
					}
				} else {
					var_tree_mutated.array_set(var_key, Class_WP_Font_Utils.sanitize_from_schema(var_value.dup(),
						var_schema.array_get(var_key)))
				}
			} else if rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(!(rt.is_true(var_is_value_array))))
				&& rt.is_true(var_is_schema_array)))
			{
				var_tree_mutated.array_unset(var_key)
			} else if rt.is_true(rt.new_bool(!(rt.is_true(var_is_schema_array)))) {
				var_tree_mutated.array_set(var_key, Class_WP_Font_Utils.apply_sanitizer(var_value.dup(),
					var_schema.array_get(var_key)))
			}
			if !rt.is_true(var_tree_mutated.array_get(var_key)) {
				var_tree_mutated.array_unset(var_key)
			}
		}
	}
	return var_tree_mutated.dup()
}

fn Class_WP_Font_Utils.apply_sanitizer(var_value rt.PhpVal, var_sanitizer rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_null(), var_sanitizer)) {
		return var_value.dup()
	}
	return rt.call_function('call_user_func', [var_sanitizer.dup(),
		var_value.dup()])
}

fn Class_WP_Font_Utils.get_allowed_font_mime_types() rt.PhpVal {
	mut var_php_7_ttf_mime_type := rt.new_string(if rt.is_true(rt.greater_equal(rt.get_constant('PHP_VERSION_ID'),
		rt.new_int(70300)))
	{
		rt.new_string('application/font-sfnt')
	} else {
		rt.new_string('application/x-font-ttf')
	})
	return rt.create_array([
		rt.ArrayItem{ key: 'otf', val: 'application/vnd.ms-opentype' },
		rt.ArrayItem{
			key: 'ttf'
			val: if rt.is_true(rt.greater_equal(rt.get_constant('PHP_VERSION_ID'),
				rt.new_int(70400)))
			{
				rt.new_string('font/sfnt')
			} else {
				var_php_7_ttf_mime_type
			}
		},
		rt.ArrayItem{
			key: 'woff'
			val: if rt.is_true(rt.greater_equal(rt.get_constant('PHP_VERSION_ID'),
				rt.new_int(80112)))
			{
				'font/woff'
			} else {
				'application/font-woff'
			}
		},
		rt.ArrayItem{
			key: 'woff2'
			val: if rt.is_true(rt.greater_equal(rt.get_constant('PHP_VERSION_ID'),
				rt.new_int(80112)))
			{
				'font/woff2'
			} else {
				'application/font-woff2'
			}
		},
	])
}

fn create_wp_font_utils() &Class_WP_Font_Utils {
	mut obj := &Class_WP_Font_Utils{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
}

fn (mut this Class_WP_Font_Utils) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'maybe_add_quotes' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			return rt.new_string(Class_WP_Font_Utils.maybe_add_quotes(dispatch_arg_0))
		}
		'sanitize_font_family' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			return rt.new_string(Class_WP_Font_Utils.sanitize_font_family(dispatch_arg_0))
		}
		'get_font_face_slug' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			return Class_WP_Font_Utils.get_font_face_slug(dispatch_arg_0)
		}
		'sanitize_from_schema' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return Class_WP_Font_Utils.sanitize_from_schema(dispatch_arg_0, dispatch_arg_1)
		}
		'apply_sanitizer' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return Class_WP_Font_Utils.apply_sanitizer(dispatch_arg_0, dispatch_arg_1)
		}
		'get_allowed_font_mime_types' {
			return Class_WP_Font_Utils.get_allowed_font_mime_types()
		}
		else {
			return none
		}
	}
}

fn (this &Class_WP_Font_Utils) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_WP_Font_Utils) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	return this.PhpObjectBase.dispatch_set_prop(prop_name, val)
}

pub fn init_wp_includes_fonts_class_wp_font_utils_php() {
}
