...
fn Class_PHPMailer_PHPMailer_POP3.popbeforesmtp(var_host rt.PhpVal, port bool, timeout bool, username string, password string, debug_level i64) rt.PhpVal {
	...
	return var_pop.authorise(var_host.dup(), rt.new_bool(port_mutated), rt.new_bool(timeout), rt.new_string(username_mutated), rt.new_string(password_mutated), rt.new_int(debug_level))
}
...
fn (mut this Class_PHPMailer_PHPMailer_POP3) authorise(var_host rt.PhpVal, port bool, timeout bool, username string, password string, debug_level i64) bool {
	...
	if rt.is_true(rt.identical(rt.new_bool(false), rt.new_bool(port_mutated))) {
		...
		this.port = // unsupported expression: Expr_Cast_Int
	}
	if rt.is_true(rt.identical(rt.new_bool(false), rt.new_bool(timeout))) {
		...
		this.tval = // unsupported expression: Expr_Cast_Int
	}
	this.do_debug = rt.new_int(debug_level).dup()
	this.username = rt.new_string(username_mutated).dup()
	this.password = rt.new_string(password_mutated).dup()
	this.errors = rt.new_array()
	mut var_result := rt.new_bool(this.connect(this.host, (this.port).to_bool(), (this.tval).to_i64()))
	if rt.is_true(var_result) {
		mut var_login_result := rt.new_bool(this.login((this.username).str(), (this.password).str()))
...
fn (mut this Class_PHPMailer_PHPMailer_POP3) connect(var_host rt.PhpVal, port bool, tval i64) bool {
	mut port_mutated := port
	if rt.is_true(this.connected) {
		return true
	}
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	rt.call_function('call_user_func_array', [rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('PHPMailer_PHPMailer_POP3', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'catchWarning' }]), rt.call_function('func_get_args', []rt.PhpVal{})])
	return rt.new_null()
	}
	rt.call_function('call_user_func_array', [rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('PHPMailer_PHPMailer_POP3', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'catchWarning' }]), rt.call_function('func_get_args', []rt.PhpVal{})])
	return rt.new_null()
	...
	this.pop_conn = rt.call_function('fsockopen', [var_host.dup(), rt.new_bool(port_mutated).dup(), var_errno.dup(), var_errstr.dup(), rt.new_int(tval)])
	...
	rt.call_function('stream_set_timeout', [this.pop_conn, rt.new_int(tval), rt.new_int(0)])
	mut var_pop3_response := this.getresponse(0)
	if this.checkresponse(var_pop3_response.dup()) {
		this.connected = true
		return true
	}
	return false
...
fn (mut this Class_PHPMailer_PHPMailer_POP3) login(username string, password string) bool {
	mut username_mutated := username
	mut password_mutated := password
	if rt.is_true(rt.new_bool(!(rt.is_true(this.connected)))) {
	...
	this.sendstring(rt.new_string("USER ${var_username.to_string()}" + (Class_PHPMailer_PHPMailer_static.le()).str()))
	mut var_pop3_response := this.getresponse(0)
	if this.checkresponse(var_pop3_response.dup()) {
		this.sendstring(rt.new_string("PASS ${var_password.to_string()}" + (Class_PHPMailer_PHPMailer_static.le()).str()))
...
fn (mut this Class_PHPMailer_PHPMailer_POP3) getresponse(size i64) rt.PhpVal {
	...
	if rt.is_true(rt.greater_equal(this.do_debug, Class_PHPMailer_PHPMailer_PHPMailer_PHPMailer_POP3.debug_server())) {
		print('Server -> Client: ')
...
fn (mut this Class_PHPMailer_PHPMailer_POP3) sendstring(var_string rt.PhpVal) i64 {
	if rt.is_true(this.pop_conn) {
		if rt.is_true(rt.greater_equal(this.do_debug, Class_PHPMailer_PHPMailer_PHPMailer_PHPMailer_POP3.debug_client())) {
			print('Client -> Server: ')
			rt.echo_val(var_string)
		}
		return (rt.call_function('fwrite', [this.pop_conn, var_string.dup(), rt.new_int(var_string.dup().to_string().len)])).to_i64()
	}
	return 0
}
...
fn (mut this Class_PHPMailer_PHPMailer_POP3) catchwarning(var_errno rt.PhpVal, var_errstr rt.PhpVal, var_errfile rt.PhpVal, var_errline rt.PhpVal)  {
	...
	this.seterror(rt.new_string('Connecting to the POP3 server raised a PHP warning:' + "errno: ${var_errno.to_string()} errstr: ${var_errstr.to_string()}; errfile: ${var_errfile.to_string()}; errline: ${var_errline.to_string()}"))
}
...
fn create_phpmailer_phpmailer_pop3() &Class_PHPMailer_PHPMailer_POP3 {
...
fn create_phpmailer_phpmailer_self() &Class_PHPMailer_PHPMailer_self {
...
fn (mut this Class_PHPMailer_PHPMailer_POP3) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'popBeforeSmtp' {
			...
			return Class_PHPMailer_PHPMailer_POP3.popbeforesmtp(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2, dispatch_arg_3, dispatch_arg_4, dispatch_arg_5)
		}
		'authorise' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).to_bool()
			...
			dispatch_arg_4 := (if args.len > 4 { args[4] } else { rt.new_null() }).str()
			dispatch_arg_5 := (if args.len > 5 { args[5] } else { rt.new_null() }).to_i64()
			return rt.new_bool(this.authorise(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2, dispatch_arg_3, dispatch_arg_4, dispatch_arg_5))
		}
		'connect' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).to_bool()
...
fn (mut this Class_PHPMailer_PHPMailer_POP3) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'tval' { this.tval = val; return true }
		'username' { this.username = val; return true }
		'password' { this.password = val; return true }
		'pop_conn' { this.pop_conn = val; return true }
		'connected' { this.connected = (val).to_bool(); return true }
...
pub fn init_wp_includes_phpmailer_pop3_php() {
}
