...
fn (mut this Class_PHPMailer_PHPMailer_OAuth) construct(var_options rt.PhpVal) {
	this.provider = var_options.array_get('provider')
	this.oauthUserEmail = var_options.array_get('userName')
	this.oauthClientSecret = var_options.array_get('clientSecret')
	this.oauthClientId = var_options.array_get('clientId')
	this.oauthRefreshToken = var_options.array_get('refreshToken')
}

fn (mut this Class_PHPMailer_PHPMailer_OAuth) getgrant() rt.PhpVal {
	return create_league_oauth2_client_grant_refreshtoken()
}
...
fn (mut this Class_PHPMailer_PHPMailer_OAuth) getoauth64() rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_null(), this.oauthToken))
		|| rt.is_true(rt.call_method(this.oauthToken, 'hasExpired', []rt.PhpVal{}))))
	{
		this.oauthToken = this.gettoken()
	}
	return rt.call_function('base64_encode', [
		'user=' + (this.oauthUserEmail).str() + '\\auth=Bearer ' + (this.oauthToken).str() + '\\\\',
	])
}
...
fn create_league_oauth2_client_grant_refreshtoken() &Class_League_OAuth2_Client_Grant_RefreshToken {
...
pub fn init_wp_includes_phpmailer_oauth_php() {
}
