...
fn Class_WpOrg_Requests_Utility_InputValidator.is_string_or_stringable(var_input rt.PhpVal) bool {
	return rt.is_true(rt.new_bool(var_input.dup().is_string())) || rt.is_true(Class_WpOrg_Requests_Utility_InputValidator.is_stringable_object(var_input.dup()))
}

fn Class_WpOrg_Requests_Utility_InputValidator.is_numeric_array_key(var_input rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(var_input.dup().is_long())) {
		return true
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_input.dup().is_string()))))) {
		return false
	}
...
fn Class_WpOrg_Requests_Utility_InputValidator.is_stringable_object(var_input rt.PhpVal) bool {
	return rt.is_true(rt.new_bool(var_input.dup().is_object())) && rt.is_true(rt.call_function('method_exists', [var_input.dup(), rt.new_string('__toString')]))
}

fn Class_WpOrg_Requests_Utility_InputValidator.has_array_access(var_input rt.PhpVal) bool {
	return rt.is_true(rt.new_bool(var_input.dup().is_array())) || rt.is_true(rt.new_bool(rt.instance_of(var_input, 'ArrayAccess')))
}

fn Class_WpOrg_Requests_Utility_InputValidator.is_iterable(var_input rt.PhpVal) bool {
	return rt.is_true(rt.new_bool(var_input.dup().is_array())) || rt.is_true(rt.new_bool(rt.instance_of(var_input, 'Traversable')))
}

fn Class_WpOrg_Requests_Utility_InputValidator.is_curl_handle(var_input rt.PhpVal) bool {
	if rt.is_true(rt.call_function('is_resource', [var_input.dup()])) {
		return (rt.identical(rt.call_function('get_resource_type', [var_input.dup()]), rt.new_string('curl'))).to_bool()
	}
	if rt.is_true(rt.new_bool(var_input.dup().is_object())) {
...
pub fn init_wp_includes_requests_src_utility_inputvalidator_php() {
}
