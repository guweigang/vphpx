...
fn Class_WpOrg_Requests_Exception_InvalidArgument.create(var_position rt.PhpVal, var_name rt.PhpVal, var_expected rt.PhpVal, var_received rt.PhpVal) rt.PhpVal {
	...
	return create_wporg_requests_exception_self(rt.call_function('sprintf', [
		rt.new_string('%s::%s(): Argument #%d (%s) must be of type %s, %s given'),
		var_stack.array_get(1).array_get('class'),
		var_stack.array_get(1).array_get('function'),
		var_position.dup(),
		var_name.dup(),
		var_expected.dup(),
		var_received.dup(),
...
fn create_wporg_requests_exception_self() &Class_WpOrg_Requests_Exception_self {
...
pub fn init_wp_includes_requests_src_exception_invalidargument_php() {
}
