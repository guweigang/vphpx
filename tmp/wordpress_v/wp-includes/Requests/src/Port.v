...
fn Class_WpOrg_Requests_Port.get(var_type rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_type_mutated.dup().is_string()))))) {
		...
		}(rt.new_int(1), rt.new_string('$type'), rt.new_string('string'), rt.call_function('gettype', [
			var_type_mutated.dup(),
		])))
	}
	var_type_mutated = rt.new_string(rt.new_string(var_type_mutated.dup().to_string().to_upper()))
	...
	])))))
	{
		...
		rt.throw_exception(rt.new_object('WpOrg_Requests_Exception', []string{}, create_wporg_requests_exception(var_message.dup(),
...
fn create_wporg_requests_port() &Class_WpOrg_Requests_Port {
...
pub fn init_wp_includes_requests_src_port_php() {
}
