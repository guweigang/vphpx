...
fn (mut this Class_SimplePie_Registry) register(type string, var_class rt.PhpVal, legacy bool) bool {
	...
	if rt.is_true(rt.new_bool(this.legacyTypes.array_isset(rt.new_string(type_mutated).dup()))) {
		type_mutated = (this.legacyTypes.array_get(type_mutated)).str()
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.default.array_isset(rt.new_string(type_mutated).dup())))))) {
		return false
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		var_class_mutated.dup()])))))
	{
		return false
	}
	mut var_base_class := this.default.array_get(type_mutated)
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_subclass_of', [
		var_class_mutated.dup(), var_base_class.dup()])))))
	{
		return false
	}
	this.classes.array_set(type_mutated, var_class_mutated.dup())
	if var_legacy {
		this.legacy.array_push(var_class_mutated.dup())
	}
	return true
}
...
fn (mut this Class_SimplePie_Registry) get_class(var_type rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(this.legacyTypes.array_isset(var_type_mutated.dup()))) {
		var_type_mutated = this.legacyTypes.array_get(var_type_mutated)
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.default.array_isset(var_type_mutated.dup())))))) {
		return rt.new_null()
	}
	mut var_class := this.default.array_get(var_type_mutated)
	if rt.is_true(rt.new_bool(this.classes.array_isset(var_type_mutated.dup()))) {
...
fn (mut this Class_SimplePie_Registry) create(var_type rt.PhpVal, mut var_parameters Class_SimplePie_array) rt.PhpVal {
	mut var_type_mutated := var_type
	mut var_class := this.get_class(var_type_mutated.dup())
	...
	} else if rt.is_true(rt.call_function('method_exists', [var_instance.dup(),
...
fn (mut this Class_SimplePie_Registry) call(var_type rt.PhpVal, method string, mut var_parameters Class_SimplePie_array) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('in_array', [var_class.dup(), this.legacy])) {
	...
	var_result = rt.call_function('call_user_func_array', [var_callable.dup(), var_parameters])
	return var_result.dup()
}
...
fn create_simplepie_reflectionclass() &Class_SimplePie_ReflectionClass {
...
pub fn init_wp_includes_simplepie_src_registry_php() {
