...
fn (mut this Class_SimplePie_Cache_Psr16) get_data(key string, var_default rt.PhpVal) rt.PhpVal {
	mut var_data := rt.call_method(this.cache, 'get', [rt.new_string(key), var_default.dup()])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_data.dup().is_array()))))) || rt.is_true(rt.identical(var_data, var_default)))) {
...
pub fn init_wp_includes_simplepie_src_cache_psr16_php() {
