...
fn (mut this Class_SimplePie_HTTP_Psr7Response) get_headers() rt.PhpVal {
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_bool(rt.new_bool(var_header.dup().array_count() >= 1))
	}
...
fn (mut this Class_SimplePie_HTTP_Psr7Response) with_header(name string, var_value rt.PhpVal) rt.PhpVal {
	return create_simplepie_http_self(rt.call_method(this.response, 'withHeader', [
		rt.new_string(name),
		var_value.dup(),
	]), this.permanent_url, this.requested_url)
}
...
fn create_simplepie_http_self() &Class_SimplePie_HTTP_self {
...
pub fn init_wp_includes_simplepie_src_http_psr7response_php() {
