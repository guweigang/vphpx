...
fn (mut this Class_SimplePie_HTTP_RawTextResponse) get_header(name string) rt.PhpVal {
	return if this.headers.array_isset(rt.new_string(name.to_lower())) { this.headers.array_get(name) } else { rt.new_array() }
}
...
fn (mut this Class_SimplePie_HTTP_RawTextResponse) get_header_line(name string) string {
	return (if this.headers.array_isset(rt.new_string(name.to_lower())) { rt.call_function('implode', [rt.new_string(', '), this.headers.array_get(name)]) } else { rt.new_string('') }).str()
}
...
fn (mut this Class_SimplePie_HTTP_RawTextResponse) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'raw_text' { this.raw_text = (val).str(); return true }
		'permanent_url' { this.permanent_url = (val).str(); return true }
		'headers' { this.headers = val; return true }
		'requested_url' { this.requested_url = (val).str(); return true }
...
pub fn init_wp_includes_simplepie_src_http_rawtextresponse_php() {
