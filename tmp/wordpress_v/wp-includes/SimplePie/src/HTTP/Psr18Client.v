...
fn (mut this Class_SimplePie_HTTP_Psr18Client) construct(mut var_httpClient Class_Psr_Http_Client_ClientInterface, mut var_requestFactory Class_Psr_Http_Message_RequestFactoryInterface, mut var_uriFactory Class_Psr_Http_Message_UriFactoryInterface)  {
	this.httpClient = var_httpClient.dup()
	this.requestFactory = var_requestFactory.dup()
	this.uriFactory = var_uriFactory.dup()
}
...
fn (mut this Class_SimplePie_HTTP_Psr18Client) request(method string, url string, mut var_headers Class_SimplePie_HTTP_array) rt.PhpVal {
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.call_function('sprintf', [rt.new_string('%s(): Argument #1 ($method) only supports method "%s".'), rt.new_string(@METHOD), Class_SimplePie_HTTP_SimplePie_HTTP_Psr18Client.method_get()]), rt.new_int(1))))
	}
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^http(s)?:\\/\\//i'), rt.new_string(url)])) {
...
fn (mut this Class_SimplePie_HTTP_Psr18Client) requesturl(method string, url string, mut var_headers Class_SimplePie_HTTP_array) rt.PhpVal {
	mut var_permanentUrl := rt.new_string(rt.new_string(url))
	mut var_requestedUrl := rt.new_string(rt.new_string(url))
	mut var_remainingRedirects := this.allowedRedirects
	mut var_request := rt.call_method(this.requestFactory, 'createRequest', [rt.new_string(method), rt.call_method(this.uriFactory, 'createUri', [var_requestedUrl.dup()])])
	{
		...
		for {
			...
			var_request = rt.call_method(var_request, 'withHeader', [var_name.dup(), var_value.dup()])
		}
	}
	for {
		mut var_followRedirect := rt.new_bool(rt.new_bool(false))
		mut var_response := rt.call_method(this.httpClient, 'sendRequest', [var_request.dup()])
...
end_label_1:
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('in_array', [var_statusCode.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 300 }, rt.ArrayItem{ key: none, val: 301 }, rt.ArrayItem{ key: none, val: 302 }, rt.ArrayItem{ key: none, val: 303 }, rt.ArrayItem{ key: none, val: 307 }])])) && rt.is_true(rt.call_method(var_response, 'hasHeader', [rt.new_string('Location')])))) {
			if rt.is_true(rt.less_equal(var_remainingRedirects, rt.new_int(0))) {
				break
			}
			rt.post_dec(var_remainingRedirects)
			var_followRedirect = rt.new_bool(rt.new_bool(true))
			var_requestedUrl = rt.call_method(var_response, 'getHeaderLine', [rt.new_string('Location')])
			if rt.is_true(rt.identical(var_statusCode, rt.new_int(301))) {
				var_permanentUrl = var_requestedUrl.dup()
			}
			var_request = rt.call_method(var_request, 'withUri', [rt.call_method(this.uriFactory, 'createUri', [var_requestedUrl.dup()])])
		}
		if !(rt.is_true(var_followRedirect)) {
			break
		}
	}
	return create_simplepie_http_psr7response(var_response.dup(), var_permanentUrl.dup(), var_requestedUrl.dup())
}

fn (mut this Class_SimplePie_HTTP_Psr18Client) requestlocalfile(path string) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_readable', [rt.new_string(path)]))))) {
		rt.throw_exception(rt.new_object('SimplePie_HTTP_ClientException', []string{}, create_simplepie_http_clientexception(rt.call_function('sprintf', [rt.new_string('file "%s" is not readable'), rt.new_string(path)]))))
	}
...
end_label_2:
	if rt.is_true(rt.identical(var_raw, rt.new_bool(false))) {
		rt.throw_exception(rt.new_object('SimplePie_HTTP_ClientException', []string{}, create_simplepie_http_clientexception(rt.new_string('file_get_contents() could not read the file'), rt.new_int(1))))
	}
	return create_simplepie_http_rawtextresponse(var_raw.dup(), rt.new_string(path).dup())
}
...
fn create_simplepie_http_psr7response() &Class_SimplePie_HTTP_Psr7Response {
...
fn create_simplepie_http_rawtextresponse() &Class_SimplePie_HTTP_RawTextResponse {
...
fn (mut this Class_SimplePie_HTTP_Psr18Client) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'httpClient' { this.httpClient = val; return true }
		'requestFactory' { this.requestFactory = val; return true }
		'uriFactory' { this.uriFactory = val; return true }
		'allowedRedirects' { this.allowedRedirects = val; return true }
...
pub fn init_wp_includes_simplepie_src_http_psr18client_php() {
