...
fn (mut this Class_WP_Customize_Section) construct(var_manager rt.PhpVal, var_id rt.PhpVal, var_args rt.PhpVal)  {
	mut var_keys := rt.func_array_keys(rt.call_function('get_object_vars', [rt.new_object('WP_Customize_Section', []string{}, &this)]))
	{
		...
		for {
			...
			if var_args.array_isset(var_key) {
				this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":177,"name":"key"}', var_args.array_get(var_key))
			}
		}
	}
	...
	if !rt.is_true(this.active_callback) {
		this.active_callback = rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WP_Customize_Section', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'active_callback' }])
	}
	// unsupported expression: Expr_AssignOp_Plus
	this.instance_number = // unsupported expression: Expr_StaticPropertyFetch
	this.controls = rt.new_array()
	// unsupported statement: Stmt_Nop
}

fn (mut this Class_WP_Customize_Section) active() rt.PhpVal {
	mut var_section := rt.new_object('WP_Customize_Section', []string{}, &this).dup()
	mut var_active := rt.call_function('call_user_func', [this.active_callback, rt.new_object('WP_Customize_Section', []string{}, &this)])
	var_active = rt.call_function('apply_filters', [rt.new_string('customize_section_active'), var_active.dup(), var_section.dup()])
...
fn (mut this Class_WP_Customize_Section) json() rt.PhpVal {
	mut var_array := rt.call_function('wp_array_slice_assoc', [rt.cast_array(rt.new_object('WP_Customize_Section', []string{}, &this)), rt.create_array([rt.ArrayItem{ key: none, val: 'id' }, rt.ArrayItem{ key: none, val: 'description' }, rt.ArrayItem{ key: none, val: 'priority' }, rt.ArrayItem{ key: none, val: 'panel' }, rt.ArrayItem{ key: none, val: 'type' }, rt.ArrayItem{ key: none, val: 'description_hidden' }])])
	var_array.array_set('title', rt.call_function('html_entity_decode', [this.title, rt.get_constant('ENT_QUOTES'), rt.call_function('get_bloginfo', [rt.new_string('charset')])]))
	...
	if rt.is_true(this.panel) {
		var_array.array_set('customizeAction', rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Customizing &#9656; %s')]), rt.call_function('esc_html', [rt.get_property(rt.call_method(this.manager, 'get_panel', [this.panel]), 'title')])]))
	} else {
		var_array.array_set('customizeAction', rt.call_function('__', [rt.new_string('Customizing')]))
	}
...
fn (mut this Class_WP_Customize_Section) check_capabilities() bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(this.theme_supports) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_theme_supports', [rt.cast_array(this.theme_supports)]))))))) {
		return false
	}
	return true
}
...
fn (mut this Class_WP_Customize_Section) maybe_render()  {
	...
	rt.call_function('do_action', [rt.new_string('customize_render_section'), rt.new_object('WP_Customize_Section', []string{}, &this)])
	rt.call_function('do_action', [rt.concat(rt.new_string('customize_render_section_'), this.id)])
	this.render()
}
...
fn (mut this Class_WP_Customize_Section) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'instance_count' { this.instance_count = val; return true }
		'instance_number' { this.instance_number = val; return true }
		'manager' { this.manager = val; return true }
		'id' { this.id = val; return true }
		'priority' { this.priority = val; return true }
		'panel' { this.panel = val; return true }
		...
		'active_callback' { this.active_callback = val; return true }
		'description_hidden' { this.description_hidden = val; return true }
...
pub fn init_wp_includes_class_wp_customize_section_php() {
