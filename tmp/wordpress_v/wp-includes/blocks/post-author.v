...
fn render_block_core_post_author(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	if !(rt.get_property(var_block, 'context').array_isset(rt.new_string('postId'))) {
		mut var_author_id := rt.call_function('get_query_var', [
	...
	mut var_avatar := if !(!rt.is_true(var_attributes.array_get('avatarSize'))) { rt.call_function('get_avatar', [
			var_author_id.dup(),
			var_attributes.array_get('avatarSize'),
		]) } else { rt.new_null() }
	mut var_link := rt.call_function('get_author_posts_url', [
		var_author_id.dup()])
	...
	mut var_byline := if !(!rt.is_true(var_attributes.array_get('byline'))) {
		var_attributes.array_get('byline')
	} else {
		rt.new_bool(false)
	}
	mut var_classes := []rt.PhpVal{}
	if var_attributes.array_isset(rt.new_string('itemsJustification')) {
		var_classes << 'items-justified-' + (var_attributes.array_get('itemsJustification')).str()
	}
	...
	return (rt.call_function('sprintf', [rt.new_string('<div %1$s>'), var_wrapper_attributes.dup()])).str() + if !(!rt.is_true(var_attributes.array_get('showAvatar'))) {
		...
		'<p class="wp-block-post-author__byline">' + (rt.call_function('wp_kses_post', [var_byline.dup()])).str() + '</p>'
	} else {
		''
	} + '<p class="wp-block-post-author__name">' + var_author_name.str() + '</p>' + if !(!rt.is_true(var_attributes.array_get('showBio'))) {
		'<p class="wp-block-post-author__bio">' + (rt.call_function('get_the_author_meta', [rt.new_string('user_description'), var_author_id.dup()])).str() + '</p>'
	} else {
		''
	} + '</div>' + '</div>'
}
...
fn register_block_core_post_author() {
	rt.call_function('register_block_type_from_metadata', [@DIR + '/post-author',
		rt.create_array([
			rt.ArrayItem{ key: 'render_callback', val: 'render_block_core_post_author' },
		])])
}

pub fn init_wp_includes_blocks_post_author_php() {
