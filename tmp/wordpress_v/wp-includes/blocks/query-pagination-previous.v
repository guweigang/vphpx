...
fn render_block_core_query_pagination_previous(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	mut var_default_label := rt.call_function('__', [rt.new_string('Previous Page')])
	...
	mut var_pagination_arrow := rt.call_function('get_query_pagination_arrow', [var_block.dup(), rt.new_bool(false)])
	...
	return var_wrapper_attributes.dup()
		...
		rt.call_function('add_filter', [rt.new_string('previous_posts_link_attributes'), var_filter_link_attributes.dup()])
		var_content = rt.call_function('get_previous_posts_link', [var_label.dup()])
		rt.call_function('remove_filter', [rt.new_string('previous_posts_link_attributes'), var_filter_link_attributes.dup()])
	} else {
		mut var_block_query := create_wp_query(rt.call_function('build_query_vars_from_query_block', [var_block.dup(), var_page.dup()]))
		mut var_block_max_pages := rt.get_property(var_block_query, 'max_num_pages')
		mut var_total := if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_max_page)))) || rt.is_true(rt.greater(var_max_page, var_block_max_pages)))) { var_block_max_pages } else { var_max_page }
		rt.call_function('wp_reset_postdata', []rt.PhpVal{})
		if rt.is_true(rt.new_bool(rt.is_true(rt.less(rt.new_int(1), var_page)) && rt.is_true(rt.less_equal(var_page, var_total)))) {
			var_content = rt.call_function('sprintf', [rt.new_string('<a href="%1$s" %2$s>%3$s</a>'), rt.call_function('esc_url', [rt.call_function('add_query_arg', [var_page_key.dup(), rt.sub(var_page, rt.new_int(1))])]), var_wrapper_attributes.dup(), var_label.dup()])
		}
	}
	if rt.is_true(rt.new_bool(rt.is_true(var_enhanced_pagination) && !(var_content).is_null())) {
		...
		if rt.is_true(var_p.next_tag(rt.create_array([rt.ArrayItem{ key: 'tag_name', val: 'a' }, rt.ArrayItem{ key: 'class_name', val: 'wp-block-query-pagination-previous' }]))) {
			var_p.set_attribute(rt.new_string('data-wp-key'), rt.new_string('query-pagination-previous'))
...
fn register_block_core_query_pagination_previous() {
	rt.call_function('register_block_type_from_metadata', [@DIR + '/query-pagination-previous', rt.create_array([rt.ArrayItem{ key: 'render_callback', val: 'render_block_core_query_pagination_previous' }])])
}
...
pub fn init_wp_includes_blocks_query_pagination_previous_php() {
	rt.call_function('add_action', [rt.new_string('init'), rt.new_string('register_block_core_query_pagination_previous')])
}
