...
fn block_core_accordion_item_render(var_attributes rt.PhpVal, content string) string {
	...
	{
		mut var_open_by_default := if rt.is_true(var_attributes.array_get('openByDefault')) {
			'true'
		} else {
			'false'
		}
		...
		])))
		{
			...
			])))
			{
				...
				var_p.set_attribute(rt.new_string('data-wp-bind--inert'),
					rt.new_string('!state.isOpen'))
				content = (var_p.get_updated_html()).str()
			}
		}
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_attributes.array_get('openByDefault'))))) {
		mut var_processor := create_wp_html_tag_processor(rt.new_string(content).dup())
		...
		content = (var_processor.get_updated_html()).str()
	}
	return content
}

fn register_block_core_accordion_item() {
	rt.call_function('register_block_type_from_metadata', [@DIR + '/accordion-item',
		rt.create_array([
...
pub fn init_wp_includes_blocks_accordion_item_php() {
