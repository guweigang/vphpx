...
fn render_block_core_footnotes(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_footnotes := rt.call_function('get_post_meta', [rt.get_property(var_block, 'context').array_get('postId'),
		rt.new_string('footnotes'), rt.new_bool(true)])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_footnotes)))) {
		return ''
	}
	var_footnotes = rt.call_function('json_decode', [var_footnotes.dup(),
		rt.new_bool(true)])
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_footnotes.dup().is_array())))))
		|| var_footnotes.dup().array_count() == 0))
	{
		return ''
	}
	mut var_wrapper_attributes := rt.call_function('get_block_wrapper_attributes', []rt.PhpVal{})
	mut var_footnote_index := 1
	mut var_block_content := ''
	{
		mut iter_1 := var_footnotes.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_footnote := item_1.val
			mut var_aria_label := rt.call_function('sprintf', [
				rt.call_function('__', [rt.new_string('Jump to footnote reference %1$d')]),
				rt.new_int(var_footnote_index).dup(),
			])
	...
	return (rt.call_function('sprintf', [rt.new_string('<ol %1$s>%2$s</ol>'),
		var_wrapper_attributes.dup(), rt.new_string(var_block_content).dup()])).str()
}

fn register_block_core_footnotes() {
	rt.call_function('register_block_type_from_metadata', [@DIR + '/footnotes',
		rt.create_array([
...
fn register_block_core_footnotes_post_meta() {
	...
	])
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(rt.is_true(rt.call_function('post_type_supports', [var_post_type.dup(), rt.new_string('editor')]))
				&& rt.is_true(rt.call_function('post_type_supports', [var_post_type.dup(), rt.new_string('custom-fields')]))))
				&& rt.is_true(rt.call_function('post_type_supports', [var_post_type.dup(), rt.new_string('revisions')]))))
			{
				rt.call_function('register_post_meta', [var_post_type.dup(),
...
fn wp_get_footnotes_from_revision(var_revision_field rt.PhpVal, var_field rt.PhpVal, var_revision rt.PhpVal) rt.PhpVal {
	return rt.call_function('get_metadata', [rt.new_string('post'),
		rt.get_property(var_revision, 'ID'), var_field.dup(),
		rt.new_bool(true)])
}

pub fn init_wp_includes_blocks_footnotes_php() {
