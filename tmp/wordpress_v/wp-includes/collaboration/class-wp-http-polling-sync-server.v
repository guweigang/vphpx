...
fn (mut this Class_WP_HTTP_Polling_Sync_Server) check_permissions(mut var_request Class_WP_REST_Request) bool {
	...
	mut var_rooms := var_request.array_get('rooms')
	mut var_wp_user_id := rt.call_function('get_current_user_id', []rt.PhpVal{})
	{
		...
		for {
			...
			mut var_client_id := var_room.array_get('client_id')
			var_room = var_room.array_get('room')
			mut var_existing_awareness := rt.call_method(this.storage, 'get_awareness_state', [var_room.dup()])
			{
				mut iter_2 := var_existing_awareness.iterator()
				for {
					item_2 := iter_2.next() or { break }
					mut var_entry := item_2.val
					if rt.is_true(rt.new_bool(rt.is_true(rt.identical(var_client_id, var_entry.array_get('client_id'))) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
			...
			mut var_type_parts := rt.call_function('explode', [rt.new_string('/'), var_room.dup(), rt.new_int(2)])
			mut var_object_parts := rt.call_function('explode', [rt.new_string(':'), if !(var_type_parts.array_get(1)).is_null() { var_type_parts.array_get(1) } else { rt.new_string('') }, rt.new_int(2)])
			mut var_entity_kind := var_type_parts.array_get(0)
			mut var_entity_name := var_object_parts.array_get(0)
			mut var_object_id := if !(var_object_parts.array_get(1)).is_null() { var_object_parts.array_get(1) } else { rt.new_null() }
			if !(this.can_user_sync_entity_type((var_entity_kind).str(), (var_entity_name).str(), mut rt.cast_object_ptr[Class_?string](var_object_id))) {
				return (create_wp_error(rt.new_string('rest_cannot_edit'), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('You do not have permission to sync this entity: %s.')]), var_room.dup()]), rt.create_array([rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code', []rt.PhpVal{}) }]))).to_bool()
			}
		}
	}
	return true
...
fn (mut this Class_WP_HTTP_Polling_Sync_Server) validate_request(mut var_request Class_WP_REST_Request) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_body.dup().is_string())) && rt.is_true(rt.greater(rt.new_int(var_body.dup().to_string().len), Class_WP_HTTP_Polling_Sync_Server.max_body_size())))) {
...
fn (mut this Class_WP_HTTP_Polling_Sync_Server) handle_request(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	mut var_response := rt.create_array([rt.ArrayItem{ key: 'rooms', val: rt.new_array() }])
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_room_request := item_1.val
			mut var_awareness := var_room_request.array_get('awareness')
			mut var_client_id := var_room_request.array_get('client_id')
			mut var_cursor := var_room_request.array_get('after')
			mut var_room := var_room_request.array_get('room')
			...
			mut var_is_compactor := rt.new_bool(rt.new_bool(false))
			if var_merged_awareness.dup().array_count() > 0 {
				var_is_compactor = rt.identical(rt.call_function('min', [rt.func_array_keys(var_merged_awareness.dup())]), var_client_id)
			}
			{
				mut iter_2 := var_room_request.array_get('updates').iterator()
				for {
					item_2 := iter_2.next() or { break }
			...
			var_room_response.array_set('awareness', var_merged_awareness.dup())
			var_response.array_get_mut('rooms').array_push(var_room_response.dup())
		}
	}
...
fn (mut this Class_WP_HTTP_Polling_Sync_Server) can_user_sync_entity_type(entity_kind string, entity_name string, mut var_object_id Class_?string) bool {
	...
	if rt.is_true(rt.new_bool(var_object_id_mutated.dup().is_string())) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('ctype_digit', [var_object_id_mutated.dup()]))))) {
			return false
		}
		var_object_id_mutated = // unsupported expression: Expr_Cast_Int
	}
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(rt.less_equal(var_object_id_mutated, rt.new_int(0))))) {
		return false
	}
	if rt.is_true(rt.new_bool(var_object_id_mutated.dup().is_long())) {
		if rt.is_true(rt.identical(rt.new_string('postType'), rt.new_string(entity_kind_mutated))) {
			if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
				return false
			}
			return (rt.call_function('current_user_can', [rt.new_string('edit_post'), var_object_id_mutated.dup()])).to_bool()
		}
		if rt.is_true(rt.identical(rt.new_string('taxonomy'), rt.new_string(entity_kind_mutated))) {
			mut var_term_exists := rt.call_function('term_exists', [var_object_id_mutated.dup(), rt.new_string(entity_name_mutated).dup()])
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_term_exists.dup().is_array()))))) || !(var_term_exists.array_isset(rt.new_string('term_id'))))) {
				return false
			}
			return (rt.call_function('current_user_can', [rt.new_string('edit_term'), var_object_id_mutated.dup()])).to_bool()
		}
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('root'), rt.new_string(entity_kind_mutated))) && rt.is_true(rt.identical(rt.new_string('comment'), rt.new_string(entity_name_mutated))))) {
			return (rt.call_function('current_user_can', [rt.new_string('edit_comment'), var_object_id_mutated.dup()])).to_bool()
		}
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		return false
	...
	if rt.is_true(rt.identical(rt.new_string('postType'), rt.new_string(entity_kind_mutated))) {
		mut var_post_type_object := rt.call_function('get_post_type_object', [rt.new_string(entity_name_mutated).dup()])
	...
	return (rt.call_function('in_array', [rt.new_string(entity_kind_mutated).dup(), var_allowed_collection_entity_kinds.dup(), rt.new_bool(true)])).to_bool()
}
...
fn (mut this Class_WP_HTTP_Polling_Sync_Server) process_awareness_update(room string, client_id i64, mut var_awareness_update Class_?array) rt.PhpVal {
	...
	mut var_existing_awareness := rt.call_method(this.storage, 'get_awareness_state', [rt.new_string(room_mutated).dup()])
	mut var_updated_awareness := rt.new_array()
	mut var_current_time := rt.call_function('time', []rt.PhpVal{})
	{
		mut iter_1 := var_existing_awareness.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_entry := item_1.val
			if rt.is_true(rt.identical(rt.new_int(client_id_mutated), var_entry.array_get('client_id'))) {
				continue
			}
			if rt.is_true(rt.greater_equal(rt.sub(var_current_time, var_entry.array_get('updated_at')), Class_WP_HTTP_Polling_Sync_Server.awareness_timeout())) {
				continue
			}
			var_updated_awareness << var_entry.dup()
		}
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
	...
	rt.call_method(this.storage, 'set_awareness_state', [rt.new_string(room_mutated).dup(), var_updated_awareness.dup()])
	mut var_response := rt.new_array()
	for var_entry in var_updated_awareness {
		var_response.array_set(var_entry.array_get('client_id'), var_entry.array_get('state'))
	}
...
fn (mut this Class_WP_HTTP_Polling_Sync_Server) process_sync_update(room string, client_id i64, cursor i64, mut var_update Class_array) rt.PhpVal {
	...
	mut var_data := var_update_mutated.array_get('data')
	mut var_type := var_update_mutated.array_get('type')
	mut switch_val_1 := var_type
	if rt.is_true(rt.equal(switch_val_1, Class_WP_HTTP_Polling_Sync_Server.update_type_compaction())) {
		mut var_updates_after_cursor := rt.call_method(this.storage, 'get_updates_after_cursor', [rt.new_string(room_mutated).dup(), rt.new_int(cursor_mutated).dup()])
		mut var_has_newer_compaction := rt.new_bool(rt.new_bool(false))
		{
			mut iter_1 := var_updates_after_cursor.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_existing := item_1.val
				if rt.is_true(rt.identical(Class_WP_HTTP_Polling_Sync_Server.update_type_compaction(), var_existing.array_get('type'))) {
					var_has_newer_compaction = rt.new_bool(rt.new_bool(true))
					break
				}
			}
		}
		if rt.is_true(rt.new_bool(!(rt.is_true(var_has_newer_compaction)))) {
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.storage, 'remove_updates_before_cursor', [rt.new_string(room_mutated).dup(), rt.new_int(cursor_mutated).dup()]))))) {
				return create_wp_error(rt.new_string('rest_sync_storage_error'), rt.call_function('__', [rt.new_string('Failed to remove updates during compaction.')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 500 }]))
			}
	...
	return create_wp_error(rt.new_string('rest_invalid_update_type'), rt.call_function('__', [rt.new_string('Invalid sync update type.')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))
}
...
fn (mut this Class_WP_HTTP_Polling_Sync_Server) add_update(room string, client_id i64, type string, data string) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.storage, 'add_update', [rt.new_string(room_mutated).dup(), var_update.dup()]))))) {
...
fn (mut this Class_WP_HTTP_Polling_Sync_Server) get_updates(room string, client_id i64, cursor i64, is_compactor bool) rt.PhpVal {
	...
	mut var_updates_after_cursor := rt.call_method(this.storage, 'get_updates_after_cursor', [rt.new_string(room_mutated).dup(), rt.new_int(cursor_mutated).dup()])
	mut var_total_updates := rt.call_method(this.storage, 'get_update_count', [rt.new_string(room_mutated).dup()])
	mut var_typed_updates := rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_int(client_id_mutated), var_update.array_get('client_id'))) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
				continue
			}
			var_typed_updates << rt.create_array([rt.ArrayItem{ key: 'data', val: var_update.array_get('data') }, rt.ArrayItem{ key: 'type', val: var_update.array_get('type') }])
		}
	}
	mut var_should_compact := rt.new_bool(rt.new_bool(rt.is_true(rt.new_bool(is_compactor_mutated)) && rt.is_true(rt.greater(var_total_updates, Class_WP_HTTP_Polling_Sync_Server.compaction_threshold()))))
	return rt.create_array([rt.ArrayItem{ key: 'end_cursor', val: rt.call_method(this.storage, 'get_cursor', [rt.new_string(room_mutated).dup()]) }, rt.ArrayItem{ key: 'room', val: room_mutated }, rt.ArrayItem{ key: 'should_compact', val: var_should_compact }, rt.ArrayItem{ key: 'total_updates', val: var_total_updates }, rt.ArrayItem{ key: 'updates', val: var_typed_updates }])
}
...
pub fn init_wp_includes_collaboration_class_wp_http_polling_sync_server_php() {
}
