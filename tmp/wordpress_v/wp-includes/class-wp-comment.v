...
fn Class_WP_Comment.get_instance(var_id rt.PhpVal) bool {
	...
	mut var_comment_id := // unsupported expression: Expr_Cast_Int
	if rt.is_true(rt.new_bool(!(rt.is_true(var_comment_id)))) {
		return false
	}
	mut var__comment := rt.call_function('wp_cache_get', [var_comment_id.dup(), rt.new_string('comment')])
	if rt.is_true(rt.new_bool(!(rt.is_true(var__comment)))) {
		var__comment = rt.call_method(var_wpdb, 'get_row', [rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.new_string('SELECT * FROM '), rt.get_property(var_wpdb, 'comments')), rt.new_string(' WHERE comment_ID = %d LIMIT 1')), var_comment_id.dup()])])
		if rt.is_true(rt.new_bool(!(rt.is_true(var__comment)))) {
			return false
		}
		rt.call_function('wp_cache_add', [rt.get_property(var__comment, 'comment_ID'), var__comment.dup(), rt.new_string('comment')])
	}
	return (create_wp_comment(var__comment.dup())).to_bool()
}

fn (mut this Class_WP_Comment) construct(var_comment rt.PhpVal)  {
	{
		mut iter_1 := rt.call_function('get_object_vars', [var_comment.dup()]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_value := item_1.val
			mut var_key := item_1.key
			this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":217,"name":"key"}', var_value.dup())
		}
	}
}

fn (mut this Class_WP_Comment) to_array() rt.PhpVal {
	return rt.call_function('get_object_vars', [rt.new_object('WP_Comment', []string{}, &this)])
}

fn (mut this Class_WP_Comment) get_children(var_args rt.PhpVal) rt.PhpVal {
	mut var_defaults := { 'format': 'tree', 'status': 'all', 'hierarchical': 'threaded', 'orderby': '' }
	mut var__args := rt.call_function('wp_parse_args', [var_args.dup(), var_defaults.dup()])
	...
	if rt.is_true(rt.new_bool(this.children.is_null())) {
		if rt.is_true(this.populated_children) {
			...
			this.children = rt.call_function('get_comments', [var__args.dup()])
		}
	}
	if rt.is_true(rt.identical(rt.new_string('flat'), var__args.array_get('format'))) {
		mut var_children := rt.new_array()
		{
			mut iter_1 := this.children.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_child := item_1.val
				mut var_child_args := var__args.dup()
				var_child_args.array_set('format', 'flat')
				var_child_args.array_unset(rt.new_string('parent'))
				var_children = rt.call_function('array_merge', [var_children.dup(), rt.create_array([rt.ArrayItem{ key: none, val: var_child }]), rt.call_method(var_child, 'get_children', [var_child_args.dup()])])
			}
		}
	} else {
...
fn (mut this Class_WP_Comment) add_child(mut var_child Class_WP_Comment)  {
	this.children.array_set(var_child.comment_ID, var_child.dup())
}

fn (mut this Class_WP_Comment) get_child(var_child_id rt.PhpVal) rt.PhpVal {
	return if !(this.children.array_get(var_child_id)).is_null() { this.children.array_get(var_child_id) } else { rt.new_bool(false) }
}

fn (mut this Class_WP_Comment) populated_children(var_set rt.PhpVal)  {
	this.populated_children = // unsupported expression: Expr_Cast_Bool
}

fn (mut this Class_WP_Comment) magic_isset(var_name rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('in_array', [var_name.dup(), this.post_fields, rt.new_bool(true)])) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		mut var_post := rt.call_function('get_post', [this.comment_post_ID])
		return (rt.call_function('property_exists', [var_post.dup(), var_name.dup()])).to_bool()
	}
	return false
}

fn (mut this Class_WP_Comment) magic_get(var_name rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.call_function('in_array', [var_name.dup(), this.post_fields, rt.new_bool(true)])) {
		mut var_post := rt.call_function('get_post', [this.comment_post_ID])
...
fn (mut this Class_WP_Comment) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'add_child' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WP_Comment](if args.len > 0 { args[0] } else { rt.new_null() })
...
fn (mut this Class_WP_Comment) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'comment_ID' { this.comment_ID = val; return true }
		'comment_post_ID' { this.comment_post_ID = val; return true }
		'comment_author' { this.comment_author = val; return true }
		'comment_author_email' { this.comment_author_email = val; return true }
		'comment_author_url' { this.comment_author_url = val; return true }
		'comment_author_IP' { this.comment_author_IP = val; return true }
		'comment_date' { this.comment_date = val; return true }
		'comment_date_gmt' { this.comment_date_gmt = val; return true }
		'comment_content' { this.comment_content = val; return true }
		'comment_karma' { this.comment_karma = val; return true }
		'comment_approved' { this.comment_approved = val; return true }
		'comment_agent' { this.comment_agent = val; return true }
		'comment_type' { this.comment_type = val; return true }
		'comment_parent' { this.comment_parent = val; return true }
		'user_id' { this.user_id = val; return true }
		'children' { this.children = val; return true }
		'populated_children' { this.populated_children = val; return true }
		'post_fields' { this.post_fields = val; return true }
...
pub fn init_wp_includes_class_wp_comment_php() {
}
