...
pub mut:
		...
		back_compat_keys rt.PhpVal = rt.new_null()
}
...
fn (mut this Class_WP_User) construct(id i64, name string, site_id i64)  {
	...
	if rt.is_true(rt.new_bool(rt.instance_of(rt.new_int(id_mutated), 'WP_User'))) {
		...
		return
	} else if rt.is_true(rt.new_bool(rt.new_int(id_mutated).dup().is_object())) {
		this.init(rt.new_int(id_mutated), site_id)
		return
	}
	if rt.is_true(rt.new_bool(!(id_mutated == 0) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.new_int(id_mutated).dup().is_long() || rt.new_int(id_mutated).dup().is_double()))))))) {
	...
	if rt.is_true(var_data) {
		this.init(var_data.dup(), site_id)
	} else {
...
fn Class_WP_User.get_data_by(var_field rt.PhpVal, var_value rt.PhpVal) bool {
	...
	if rt.is_true(rt.identical(rt.new_string('ID'), var_field_mutated)) {
		var_field_mutated = rt.new_string(rt.new_string('id'))
	}
	if rt.is_true(rt.identical(rt.new_string('id'), var_field_mutated)) {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('id'))) {
		mut var_user_id := var_value_mutated.dup()
		mut var_db_field := rt.new_string(rt.new_string('ID'))
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('slug'))) {
		var_user_id = rt.call_function('wp_cache_get', [var_value_mutated.dup(), rt.new_string('userslugs')])
		var_db_field = rt.new_string(rt.new_string('user_nicename'))
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('email'))) {
		var_user_id = rt.call_function('wp_cache_get', [var_value_mutated.dup(), rt.new_string('useremail')])
		var_db_field = rt.new_string(rt.new_string('user_email'))
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('login'))) {
		var_value_mutated = rt.call_function('sanitize_user', [var_value_mutated.dup()])
		var_user_id = rt.call_function('wp_cache_get', [var_value_mutated.dup(), rt.new_string('userlogins')])
		var_db_field = rt.new_string(rt.new_string('user_login'))
	} else {
		return false
	}
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		mut var_user := rt.call_function('wp_cache_get', [var_user_id.dup(), rt.new_string('users')])
		if rt.is_true(var_user) {
			return (var_user).to_bool()
		}
	}
	var_user = rt.call_method(var_wpdb, 'get_row', [rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('SELECT * FROM '), rt.get_property(var_wpdb, 'users')), rt.new_string(' WHERE ')), var_db_field), rt.new_string(' = %s LIMIT 1')), var_value_mutated.dup()])])
...
fn (mut this Class_WP_User) magic_isset(var_key rt.PhpVal) bool {
	...
	if rt.is_true(rt.identical(rt.new_string('id'), var_key_mutated)) {
		rt.call_function('_deprecated_argument', [rt.new_string('WP_User->id'), rt.new_string('2.1.0'), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Use %s instead.')]), rt.new_string('<code>WP_User->ID</code>')])])
		var_key_mutated = rt.new_string(rt.new_string('ID'))
	}
	if !(rt.get_property(this.data, '{"nodeType":"Expr_Variable","line":291,"name":"key"}')).is_null() {
		return true
	}
	if // unsupported expression: Expr_StaticPropertyFetch.array_isset(var_key_mutated) {
		var_key_mutated = // unsupported expression: Expr_StaticPropertyFetch.array_get(var_key_mutated)
	}
	return (rt.call_function('metadata_exists', [rt.new_string('user'), this.ID, var_key_mutated.dup()])).to_bool()
}
...
fn (mut this Class_WP_User) magic_get(var_key rt.PhpVal) rt.PhpVal {
	...
	if !(rt.get_property(this.data, '{"nodeType":"Expr_Variable","line":324,"name":"key"}')).is_null() {
		mut var_value := rt.get_property(this.data, '{"nodeType":"Expr_Variable","line":325,"name":"key"}')
	} else {
		if // unsupported expression: Expr_StaticPropertyFetch.array_isset(var_key_mutated) {
			var_key_mutated = // unsupported expression: Expr_StaticPropertyFetch.array_get(var_key_mutated)
		}
		var_value = rt.call_function('get_user_meta', [this.ID, var_key_mutated.dup(), rt.new_bool(true)])
	}
	if rt.is_true(this.filter) {
		var_value = rt.call_function('sanitize_user_field', [var_key_mutated.dup(), var_value.dup(), this.ID, this.filter])
	}
...
fn (mut this Class_WP_User) magic_set(var_key rt.PhpVal, var_value rt.PhpVal)  {
	...
	if rt.is_true(rt.identical(rt.new_string('id'), var_key_mutated)) {
		rt.call_function('_deprecated_argument', [rt.new_string('WP_User->id'), rt.new_string('2.1.0'), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Use %s instead.')]), rt.new_string('<code>WP_User->ID</code>')])])
		this.ID = var_value_mutated.dup()
		return rt.new_null()
	}
	rt.set_property(this.data, '{"nodeType":"Expr_Variable","line":366,"name":"key"}', var_value_mutated.dup())
}
...
fn (mut this Class_WP_User) magic_unset(var_key rt.PhpVal)  {
	...
	if !(rt.get_property(this.data, '{"nodeType":"Expr_Variable","line":389,"name":"key"}')).is_null() {
		rt.get_property(this.data, '{"nodeType":"Expr_Variable","line":390,"name":"key"}') = rt.new_null()
	}
	if // unsupported expression: Expr_StaticPropertyFetch.array_isset(var_key_mutated) {
		// unsupported expression: Expr_StaticPropertyFetch.array_unset(var_key_mutated)
	}
}
...
fn (mut this Class_WP_User) get(var_key rt.PhpVal) rt.PhpVal {
	mut var_key_mutated := var_key
	return this.magic_get(var_key_mutated.dup())
}

fn (mut this Class_WP_User) has_prop(var_key rt.PhpVal) rt.PhpVal {
	mut var_key_mutated := var_key
	return rt.new_bool(this.magic_isset(var_key_mutated.dup()))
}
...
fn (mut this Class_WP_User) _init_caps(cap_key string)  {
	mut var_wpdb := rt.new_null()
	// unsupported statement: Stmt_Global
	rt.call_function('_deprecated_function', [rt.new_string(@METHOD), rt.new_string('4.9.0'), rt.new_string('WP_User::for_site()')])
	if cap_key == '' {
		this.cap_key = (rt.call_method(var_wpdb, 'get_blog_prefix', [this.site_id])).str() + 'capabilities'
	} else {
		this.cap_key = rt.new_string(cap_key).dup()
	}
...
fn (mut this Class_WP_User) get_role_caps() rt.PhpVal {
	mut var_switch_site := rt.new_bool(rt.new_bool(false))
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{})) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		var_switch_site = rt.new_bool(rt.new_bool(true))
	...
	if rt.is_true(rt.new_bool(this.caps.is_array())) {
		this.roles = rt.new_array()
		{
			...
			for {
				...
				if rt.is_true(rt.call_method(var_wp_roles, 'is_role', [var_key.dup()])) {
					this.roles.array_push(var_key.dup())
				}
			}
		}
	}
	...
	{
		mut iter_1 := rt.cast_array(this.roles).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_role := item_1.val
			mut var_the_role := rt.call_method(var_wp_roles, 'get_role', [var_role.dup()])
			this.allcaps = rt.call_function('array_merge', [rt.cast_array(this.allcaps), rt.cast_array(rt.get_property(var_the_role, 'capabilities'))])
		}
	}
	this.allcaps = rt.call_function('array_merge', [rt.cast_array(this.allcaps), rt.cast_array(this.caps)])
...
fn (mut this Class_WP_User) add_role(var_role rt.PhpVal)  {
	...
	if rt.is_true(rt.call_function('in_array', [var_role.dup(), this.roles, rt.new_bool(true)])) {
		return rt.new_null()
	}
	...
	rt.call_function('do_action', [rt.new_string('add_user_role'), this.ID, var_role.dup()])
}

fn (mut this Class_WP_User) remove_role(var_role rt.PhpVal)  {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_role.dup(), this.roles, rt.new_bool(true)]))))) {
		return rt.new_null()
	}
...
fn (this &Class_WP_User) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	match prop_name {
		...
		'back_compat_keys' { return this.back_compat_keys }
...
fn (mut this Class_WP_User) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'caps' { this.caps = val; return true }
		'cap_key' { this.cap_key = val; return true }
		'roles' { this.roles = val; return true }
		'allcaps' { this.allcaps = val; return true }
		'filter' { this.filter = val; return true }
		'site_id' { this.site_id = val; return true }
		'back_compat_keys' { this.back_compat_keys = val; return true }
...
pub fn init_wp_includes_class_wp_user_php() {
}
