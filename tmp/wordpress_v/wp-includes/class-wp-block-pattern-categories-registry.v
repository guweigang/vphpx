...
fn (mut this Class_WP_Block_Pattern_Categories_Registry) register(var_category_name rt.PhpVal, var_category_properties rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(!(!(var_category_name).is_null()) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_category_name.dup().is_string()))))))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD), rt.call_function('__', [rt.new_string('Block pattern category name must be a string.')]), rt.new_string('5.5.0')])
		return false
	}
	mut var_category := rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: 'name', val: var_category_name }]), var_category_properties.dup()])
	this.registered_categories.array_set(var_category_name, var_category.dup())
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('current_action', []rt.PhpVal{})) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		this.registered_categories_outside_init.array_set(var_category_name, var_category.dup())
	}
	return true
}

fn (mut this Class_WP_Block_Pattern_Categories_Registry) unregister(var_category_name rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(this.is_registered(var_category_name.dup()))))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Block pattern category "%s" not found.')]), var_category_name.dup()]), rt.new_string('5.5.0')])
		return false
	}
...
fn (mut this Class_WP_Block_Pattern_Categories_Registry) get_all_registered(outside_init_only bool) rt.PhpVal {
	return rt.call_function('array_values', [if var_outside_init_only { this.registered_categories_outside_init } else { this.registered_categories }])
}

fn (mut this Class_WP_Block_Pattern_Categories_Registry) is_registered(var_category_name rt.PhpVal) rt.PhpVal {
	return rt.new_bool(!(var_category_name).is_null() && this.registered_categories.array_isset(var_category_name))
}
...
fn (mut this Class_WP_Block_Pattern_Categories_Registry) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'registered_categories_outside_init' { this.registered_categories_outside_init = val; return true }
...
pub fn init_wp_includes_class_wp_block_pattern_categories_registry_php() {
}
