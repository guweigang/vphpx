...
fn get_user_locale(user i64) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(0 == user
	...
	} else if rt.is_true(rt.new_bool(var_user != 0
		&& rt.is_true(rt.new_bool(rt.new_int(user).is_long()
		|| rt.new_int(user).is_double()))))
	{
...
fn determine_locale() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_determined_locale)
		&& rt.is_true(rt.new_bool(var_determined_locale.clone().is_string()))))
	{
		return var_determined_locale.clone()
	}
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(var_GLOBALS.array_isset(rt.new_string('pagenow'))
		&& rt.is_true(rt.identical(rt.new_string('wp-login.php'), var_GLOBALS.array_get('pagenow')))))
		&& !(!rt.is_true(rt.get_superglobal('_GET').array_get('wp_lang')))
		|| !(!rt.is_true(rt.get_superglobal('_COOKIE').array_get('wp_lang')))))
	{
		if !(!rt.is_true(rt.get_superglobal('_GET').array_get('wp_lang'))) {
			var_determined_locale = rt.call_function('sanitize_locale_name', [
				rt.get_superglobal('_GET').array_get('wp_lang'),
			])
		} else {
			var_determined_locale = rt.call_function('sanitize_locale_name', [
				rt.get_superglobal('_COOKIE').array_get('wp_lang'),
			])
		}
	} else if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_admin', []rt.PhpVal{}))
		|| rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(rt.new_string('_locale'))
		&& rt.is_true(rt.identical(rt.new_string('user'), rt.get_superglobal('_GET').array_get('_locale')))))
		&& rt.is_true(rt.call_function('wp_is_json_request', []rt.PhpVal{}))))))
	{
		...
		!(!rt.is_true(rt.get_superglobal('_REQUEST').array_get('language')))
		...
		&& rt.is_true(rt.call_function('wp_installing', []rt.PhpVal{}))))
	{
		if !(!rt.is_true(rt.get_superglobal('_REQUEST').array_get('language'))) {
			var_determined_locale = rt.call_function('sanitize_locale_name', [
				rt.get_superglobal('_REQUEST').array_get('language'),
			])
		} else {
			var_determined_locale = var_GLOBALS.array_get('wp_local_package')
		}
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_determined_locale)))) {
...
fn translate_nooped_plural(var_nooped_plural rt.PhpVal, var_count rt.PhpVal, domain string) rt.PhpVal {
	mut var_domain := domain
	if rt.is_true(var_nooped_plural.array_get('domain')) {
		var_domain = (var_nooped_plural['domain']).str()
	}
	if rt.is_true(var_nooped_plural.array_get('context')) {
		return _nx(var_nooped_plural.array_get('singular'), var_nooped_plural.array_get('plural'),
			var_count.clone(), var_nooped_plural.array_get('context'), var_domain)
	} else {
		return _n(var_nooped_plural.array_get('singular'), var_nooped_plural.array_get('plural'),
			var_count.clone(), var_domain)
	}
	return rt.new_null()
}
...
fn load_textdomain(var_domain rt.PhpVal, var_mofile_arg rt.PhpVal, var_locale_arg rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_domain.clone().is_string()))))) {
		return false
	}
	...
	for var_file_shadow in var_translation_files {
		...
		if rt.is_true(var_success) {
			if rt.is_true(rt.new_bool(var_l10n.array_isset(var_domain)
...
fn unload_textdomain(domain string, reloadable bool) bool {
	...
	if var_l10n.array_isset(rt.new_string(var_domain.str())) {
		if rt.is_true(rt.new_bool(rt.instance_of(var_l10n.array_get(var_domain),
			'NOOP_Translations')))
		{
...
fn load_default_textdomain(var_locale_arg rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(
		...
		|| rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('WP_INSTALLING_NETWORK')]))
	...
	if rt.is_true(rt.new_bool(
		...
		|| rt.is_true(rt.call_function('wp_installing', []rt.PhpVal{}))))
		|| rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('WP_REPAIRING')]))
...
fn load_script_textdomain(var_handle rt.PhpVal, domain string, path string) bool {
	...
	var_src = rt.get_property(rt.get_property(var_wp_scripts, 'registered').array_get(var_handle),
		'src')
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('|^(https?:)?//|'), var_src.clone()])))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.is_true(rt.get_property(var_wp_scripts, 'content_url'))
...
fn load_script_module_textdomain(id string, domain string, path string) bool {
	...
	var_src = var_module.array_get('src')
...
fn _load_script_textdomain_from_src(handle string, src string, domain string, path string, is_module bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(var_content_url.array_isset(rt.new_string('path')))
		|| rt.is_true(rt.call_function('str_starts_with', [var_src_url.array_get('path'), var_content_url.array_get('path')]))))
		&& rt.is_true(rt.new_bool(!(var_src_url.array_isset(rt.new_string('host')))
		|| !(var_content_url.array_isset(rt.new_string('host')))
		|| rt.is_true(rt.identical(var_src_url.array_get('host'), var_content_url.array_get('host')))))))
	{
		if var_content_url.array_isset(rt.new_string('path')) {
			var_relative = rt.call_function('substr', [var_src_url.array_get('path'),
				rt.new_int(var_content_url.array_get('path').to_string().len)])
		} else {
			var_relative = var_src_url.array_get('path')
		}
		...
		var_dirname = if rt.is_true(rt.identical(var_theme_dir.array_get(0),
			var_relative.array_get(0)))
		{
			'themes'
		} else {
			'plugins'
		...
		rt.is_true(rt.new_bool(!(var_plugins_url.array_isset(rt.new_string('path')))
		|| rt.is_true(rt.call_function('str_starts_with', [var_src_url.array_get('path'), var_plugins_url.array_get('path')]))))
		&& rt.is_true(rt.new_bool(!(var_src_url.array_isset(rt.new_string('host')))
		|| !(var_plugins_url.array_isset(rt.new_string('host')))
		|| rt.is_true(rt.identical(var_src_url.array_get('host'), var_plugins_url.array_get('host')))))))
	{
		if var_plugins_url.array_isset(rt.new_string('path')) {
			var_relative = rt.call_function('substr', [var_src_url.array_get('path'),
				rt.new_int(var_plugins_url.array_get('path').to_string().len)])
		} else {
			var_relative = var_src_url.array_get('path')
		}
		var_relative = rt.new_string(var_relative.clone().to_string().trim_space())
		...
		var_relative = rt.call_function('implode', [rt.new_string('/'),
			var_relative.clone()])
	} else if rt.is_true(rt.new_bool(!(var_src_url.array_isset(rt.new_string('host')))
		|| !(var_site_url.array_isset(rt.new_string('host')))
		|| rt.is_true(rt.identical(var_src_url.array_get('host'), var_site_url.array_get('host')))))
	{
		if !(var_site_url.array_isset(rt.new_string('path'))) {
			var_relative = rt.new_string(var_src_url.array_get('path').to_string().trim_space())
		} else if rt.is_true(rt.call_function('str_starts_with', [
			var_src_url.array_get('path'),
			rt.call_function('trailingslashit', [
				var_site_url.array_get('path'),
			])]))
		{
			var_relative = rt.call_function('substr', [var_src_url.array_get('path'),
				rt.new_int(var_site_url.array_get('path').to_string().len)])
			var_relative = rt.new_string(var_relative.clone().to_string().trim_space())
		}
	}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_relative.clone().is_string()))))) {
		return rt.new_bool(load_script_translations(false, handle, var_domain))
	}
		...
		rt.new_string('.min.js')]))
	{
		var_relative = rt.new_string(
			(rt.call_function('substr', [var_relative.clone(), rt.new_int(0), -7])).str() + '.js')
	}
...
fn load_script_translations(var_file_arg rt.PhpVal, var_handle rt.PhpVal, var_domain rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_file))))
...
fn _load_textdomain_just_in_time(var_domain rt.PhpVal) bool {
	...
	mut var_mofile := ''
	var_l10n_unloaded = rt.cast_array(var_l10n_unloaded)
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('default'), var_domain))
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('doing_action', [rt.new_string('after_setup_theme')])))))
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.call_function('str_starts_with', [var_path.clone(), var_template_directory.clone()]))
...
fn get_translations_for_domain(var_domain rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_l10n.array_isset(var_domain)
		|| rt.is_true(rt.new_bool(_load_textdomain_just_in_time(var_domain.clone())
		&& var_l10n.array_isset(var_domain)))))
	{
...
fn get_available_languages(var_dir rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_lang_files) {
		...
		for {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_starts_with', [var_lang_file_shadow.clone(), rt.new_string('continents-cities')])))))
...
fn wp_get_installed_translations(var_type rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('themes'), var_type))))
	...
	for {
			...
			rt.new_string('.mo')]))
		{
			var_pofile = rt.call_function('substr_replace', [
				var_file_shadow.clone(), rt.new_string('.po'), -'.mo'.len])
			...
			var_pofile = rt.call_function('substr_replace', [
				var_file_shadow.clone(), rt.new_string('.po'), -'.l10n.php'.len])
...
fn wp_dropdown_languages(var_args rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(var_parsed_args.array_get('id')))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(var_parsed_args.array_get('name')))))))
	{
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.identical(rt.new_string('en_US'), var_parsed_args.array_get('selected')))
		&& rt.is_true(rt.new_bool(!(rt.is_true(var_parsed_args.array_get('explicit_option_en_us')))))))
	{
		var_parsed_args.array_set('selected', '')
	}
	var_translations = var_parsed_args.array_get('translations')
	if !rt.is_true(var_translations) {
		rt.include_file(
	...
	var_languages = []rt.PhpVal{}
	mut iter_5 := var_parsed_args.array_get('languages').iterator()
	for {
		...
		if var_translations.array_isset(var_locale_shadow) {
			...
			var_languages << rt.create_array([
				rt.ArrayItem{ key: 'language', val: var_translation.array_get('language') },
				rt.ArrayItem{ key: 'native_name', val: var_translation.array_get('native_name') },
				rt.ArrayItem{ key: 'lang', val: rt.call_function('current', [
					var_translation.array_get('iso'),
				]) },
			])
	...
	var_translations_available = !(!rt.is_true(var_translations))
		&& rt.is_true(var_parsed_args.array_get('show_available_translations'))
	var_structure = []rt.PhpVal{}
	if var_translations_available {
		var_structure << '<optgroup label="' + (esc_attr_x('Installed', 'translations', '')).str() +
			'">'
	}
	if rt.is_true(var_parsed_args.array_get('show_option_site_default')) {
		var_structure << rt.call_function('sprintf', [
			...
			rt.call_function('selected', [rt.new_string('site-default'),
				var_parsed_args.array_get('selected'), rt.new_bool(false)]),
			_x('Site Default', 'default site language', ''),
		])
	}
	if rt.is_true(var_parsed_args.array_get('show_option_en_us')) {
		var_value = if rt.is_true(var_parsed_args.array_get('explicit_option_en_us')) {
			'en_US'
		} else {
			''
		}
		var_structure << rt.call_function('sprintf', [
			...
			rt.call_function('selected', [rt.new_string(''), var_parsed_args.array_get('selected'),
				rt.new_bool(false)]),
		])
	}
	for var_language_shadow in var_languages {
		var_structure << rt.call_function('sprintf', [
			...
			rt.call_function('esc_attr', [var_language_shadow.array_get('language')]),
			rt.call_function('esc_attr', [var_language_shadow.array_get('lang')]),
			rt.call_function('selected', [var_language_shadow.array_get('language'),
				var_parsed_args.array_get('selected'), rt.new_bool(false)]),
			rt.call_function('esc_html', [var_language_shadow.array_get('native_name')]),
		])
	}
	if var_translations_available {
	...
	if var_translations_available {
		...
		for {
			...
			var_structure << rt.call_function('sprintf', [
				...
				rt.call_function('esc_attr', [var_translation_shadow.array_get('language')]),
				rt.call_function('esc_attr', [
					rt.call_function('current', [var_translation_shadow.array_get('iso')]),
				]),
				rt.call_function('selected', [
					var_translation_shadow.array_get('language'),
					var_parsed_args.array_get('selected'),
					rt.new_bool(false),
				]),
				rt.call_function('esc_html', [
					var_translation_shadow.array_get('native_name'),
				]),
			])
		}
		var_structure << rt.new_string('</optgroup>')
	...
	var_output = rt.call_function('sprintf', [
		...
		rt.call_function('esc_attr', [var_parsed_args.array_get('name')]),
		rt.call_function('esc_attr', [var_parsed_args.array_get('id')]),
	])
...
fn translate_settings_using_i18n_schema(var_i18n_schema rt.PhpVal, var_settings rt.PhpVal, var_textdomain rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.create_array_from_list(var_i18n_schema).is_string()))
		&& rt.is_true(rt.new_bool(var_settings.clone().is_string()))))
	{
		return translate_with_gettext_context(var_settings.clone(),
			rt.create_array_from_list(var_i18n_schema), var_textdomain.clone())
	}
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.create_array_from_list(var_i18n_schema).is_array()))
		&& rt.is_true(rt.new_bool(var_settings.clone().is_array()))))
	{
		...
		for {
			...
			var_translated_settings.array_push(translate_settings_using_i18n_schema(var_i18n_schema.array_get(0),
				var_value_shadow.clone(), var_textdomain.clone()))
		}
		return var_translated_settings.clone()
	}
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.create_array_from_list(var_i18n_schema).is_object()))
		&& rt.is_true(rt.new_bool(var_settings.clone().is_array()))))
	{
		var_group_key = '*'
		var_translated_settings = []rt.PhpVal{}
...
fn create_wp_translations() &Class_WP_Translations {
