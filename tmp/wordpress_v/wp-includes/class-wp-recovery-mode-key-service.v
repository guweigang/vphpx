...
fn (mut this Class_WP_Recovery_Mode_Key_Service) validate_recovery_mode_key(var_token rt.PhpVal, var_key rt.PhpVal, var_ttl rt.PhpVal) bool {
	...
	this.remove_key(var_token.dup())
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_record.dup().is_array())))))
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_verify_fast_hash', [
		var_key_mutated.dup(),
		var_record.array_get('hashed_key'),
	])))))
	{
	...
	if rt.is_true(rt.greater(rt.call_function('time', []rt.PhpVal{}), rt.add(var_record.array_get('created_at'),
		var_ttl)))
	{
...
fn (mut this Class_WP_Recovery_Mode_Key_Service) clean_expired_keys(var_ttl rt.PhpVal) {
	mut var_records := this.get_keys()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(!(var_record.array_isset(rt.new_string('created_at')))
				|| rt.is_true(rt.greater(rt.call_function('time', []rt.PhpVal{}), rt.add(var_record.array_get('created_at'), var_ttl)))))
			{
...
pub fn init_wp_includes_class_wp_recovery_mode_key_service_php() {
}
