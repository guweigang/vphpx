...
fn wp_remote_retrieve_headers(var_response rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_wp_error', [rt.create_array_from_native_map(var_response)])) || !(var_response.array_isset(rt.new_string('headers'))))) {
		return rt.new_array()
	}
	return var_response.array_get('headers')
}
...
fn wp_remote_retrieve_header(var_response rt.PhpVal, var_header rt.PhpVal) string {
	...
	return (if !(var_response.array_get('headers').array_get(var_header)).is_null() { var_response.array_get('headers').array_get(var_header) } else { rt.new_string('') }).str()
}

fn wp_remote_retrieve_response_code(var_response rt.PhpVal) string {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_wp_error', [rt.create_array_from_native_map(var_response)])) || !(var_response.array_isset(rt.new_string('response'))))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_response.array_get('response').is_array()))))))) {
		return ''
	}
	return (var_response.array_get('response').array_get('code')).str()
}
...
fn wp_remote_retrieve_response_message(var_response rt.PhpVal) string {
	...
	return (var_response.array_get('response').array_get('message')).str()
}

fn wp_remote_retrieve_body(var_response rt.PhpVal) string {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_wp_error', [rt.create_array_from_native_map(var_response)])) || !(var_response.array_isset(rt.new_string('body'))))) {
		return ''
	}
	return (var_response.array_get('body')).str()
}

fn wp_remote_retrieve_cookies(var_response rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_wp_error', [rt.create_array_from_native_map(var_response)])) || !rt.is_true(var_response.array_get('cookies')))) {
		return rt.new_array()
	}
	return var_response.array_get('cookies')
}
...
fn wp_http_supports(var_capabilities_arg rt.PhpVal, var_url rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_url) && !(var_capabilities.array_isset(rt.new_string('ssl'))))) {
		var_scheme = rt.call_function('parse_url', [var_url.clone(), rt.get_constant('PHP_URL_SCHEME')])
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('https'), var_scheme)) || rt.is_true(rt.identical(rt.new_string('ssl'), var_scheme)))) {
...
fn get_http_origin() rt.PhpVal {
	...
	if !(!rt.is_true(rt.get_superglobal('_SERVER').array_get('HTTP_ORIGIN'))) {
	var_origin = rt.get_superglobal('_SERVER').array_get('HTTP_ORIGIN')
	}
...
fn get_allowed_http_origins() rt.PhpVal {
	...
	var_allowed_origins = rt.call_function('array_unique', [rt.create_array([rt.ArrayItem{ key: none, val: 'http://' + (var_admin_origin.array_get('host')).str() }, rt.ArrayItem{ key: none, val: 'https://' + (var_admin_origin.array_get('host')).str() }, rt.ArrayItem{ key: none, val: 'http://' + (var_home_origin.array_get('host')).str() }, rt.ArrayItem{ key: none, val: 'https://' + (var_home_origin.array_get('host')).str() }])])
...
fn is_allowed_http_origin(var_origin_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_origin) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_origin.clone(), get_allowed_http_origins(), rt.new_bool(true)]))))))) {
	var_origin = rt.new_string('')
	}
...
fn send_origin_headers() bool {
	...
	if rt.is_true(is_allowed_http_origin(var_origin.clone())) {
		...
		if rt.is_true(rt.identical(rt.new_string('OPTIONS'), rt.get_superglobal('_SERVER').array_get('REQUEST_METHOD'))) {
			exit(0)
		}
...
fn wp_http_validate_url(var_url_arg rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_url.clone().is_string()))))) || rt.is_true(rt.identical(rt.new_string(''), var_url)))) || rt.is_true(rt.new_bool(var_url.clone().is_long() || var_url.clone().is_double())))) {
		return false
	}
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_url)))) || rt.is_true(rt.new_bool(var_url.clone().to_string().to_lower() != var_original_url.clone().to_string().to_lower())))) {
		return false
	}
	var_parsed_url = rt.call_function('parse_url', [var_url.clone()])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_parsed_url)))) || !rt.is_true(var_parsed_url.array_get('host')))) {
		return false
	}
	if var_parsed_url.array_isset(rt.new_string('user')) || var_parsed_url.array_isset(rt.new_string('pass')) {
		return false
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('strpbrk', [var_parsed_url.array_get('host'), rt.new_string(':#?[]')]))))) {
		return false
	}
	var_parsed_home = rt.call_function('parse_url', [rt.call_function('get_option', [rt.new_string('home')])])
	var_same_host = var_parsed_home.array_isset(rt.new_string('host')) && rt.is_true(rt.identical(rt.new_string(var_parsed_home.array_get('host').to_string().to_lower()), rt.new_string(var_parsed_url.array_get('host').to_string().to_lower())))
	var_host = var_parsed_url.array_get('host').to_string().trim_space()
	if !(var_same_host) {
		...
		if rt.is_true(var_ip) {
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_int(127), var_parts.array_get(0))) || rt.is_true(rt.identical(rt.new_int(10), var_parts.array_get(0))))) || rt.is_true(rt.identical(rt.new_int(0), var_parts.array_get(0))))) || rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_int(172), var_parts.array_get(0))) && rt.is_true(rt.less_equal(rt.new_int(16), var_parts.array_get(1))))) && rt.is_true(rt.greater_equal(rt.new_int(31), var_parts.array_get(1))))))) || rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_int(192), var_parts.array_get(0))) && rt.is_true(rt.identical(rt.new_int(168), var_parts.array_get(1))))))) {
	...
	var_port = var_parsed_url.array_get('port')
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_allowed_ports.clone().is_array())) && rt.is_true(rt.call_function('in_array', [var_port.clone(), var_allowed_ports.clone(), rt.new_bool(true)])))) {
		return (var_url).to_bool()
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_parsed_home) && var_same_host)) && var_parsed_home.array_isset(rt.new_string('port')))) && rt.is_true(rt.identical(var_parsed_home.array_get('port'), var_port)))) {
		return (var_url).to_bool()
	}
	return false
}
...
fn allowed_http_request_hosts(var_is_external_arg rt.PhpVal, var_host rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(!(var_is_external) && rt.is_true(rt.call_function('wp_validate_redirect', [rt.new_string('http://' + (var_host).str())])))) {
...
fn _get_component_from_parsed_url_array(var_url_parts rt.PhpVal, var_component rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_key)))) && rt.is_true(rt.new_bool(var_url_parts.clone().is_array())))) && var_url_parts.array_isset(var_key))) {
