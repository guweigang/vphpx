...
fn image_constrain_size_for_editor(var_width rt.PhpVal, var_height rt.PhpVal, size string, var_context_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.new_string(size).is_array())) {
		var_max_width = rt.new_string(size).array_get(0)
		var_max_height = rt.new_string(size).array_get(1)
	} else if rt.is_true(rt.new_bool(
		rt.is_true(rt.identical(rt.new_string('thumb'), rt.new_string(size)))
		|| rt.is_true(rt.identical(rt.new_string('thumbnail'), rt.new_string(size)))))
	{
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_max_width))))
	...
	} else if rt.is_true(rt.new_bool(!(!rt.is_true(var__wp_additional_image_sizes))
	...
	{
		var_max_width =
			rt.new_int((var__wp_additional_image_sizes.array_get(size).array_get('width')).to_i64())
		var_max_height =
			rt.new_int((var__wp_additional_image_sizes.array_get(size).array_get('height')).to_i64())
		if rt.is_true(rt.new_bool(rt.new_int(var_content_width.to_i64()) > 0
...
fn image_downsize(var_id rt.PhpVal, size string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_is_image)))) {
		if !(!rt.is_true(var_meta.array_get('sizes').array_get('full'))) {
			var_img_url = rt.call_function('str_replace', [var_img_url_basename.clone(),
				var_meta.array_get('sizes').array_get('full').array_get('file'),
				var_img_url.clone()])
			var_img_url_basename = var_meta.array_get('sizes').array_get('full').array_get('file')
			var_width = var_meta.array_get('sizes').array_get('full').array_get('width')
			var_height = var_meta.array_get('sizes').array_get('full').array_get('height')
		} else {
	...
	if rt.is_true(var_intermediate) {
		var_img_url = rt.call_function('str_replace', [var_img_url_basename.clone(),
			var_intermediate.array_get('file'), var_img_url.clone()])
		var_width = var_intermediate.array_get('width')
		var_height = var_intermediate.array_get('height')
		var_is_intermediate = true
	} else if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('thumbnail'), rt.new_string(size)))
		&& !(!rt.is_true(var_meta.array_get('thumb')))))
		&& rt.is_true(rt.new_bool(var_meta.array_get('thumb').is_string()))))
	{
		...
		var_thumbfile = rt.call_function('str_replace', [
			...
			rt.call_function('wp_basename', [var_meta.array_get('thumb')]),
			var_imagefile.clone(),
		])
		if rt.is_true(rt.call_function('file_exists', [var_thumbfile.clone()])) {
			...
			if rt.is_true(var_info) {
				var_img_url = rt.call_function('str_replace', [
					...
					var_img_url.clone()])
				var_width = var_info.array_get(0)
				var_height = var_info.array_get(1)
				var_is_intermediate = true
			}
		}
	}
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_width))))
	...
	{
		var_width = var_meta.array_get('width')
		var_height = var_meta.array_get('height')
	}
...
fn get_image_tag(var_id rt.PhpVal, var_alt rt.PhpVal, var_title_arg rt.PhpVal, var_align rt.PhpVal, size string) rt.PhpVal {
	...
	var_size_class = if rt.is_true(rt.new_bool(rt.new_string(size).is_array())) { rt.call_function('implode', [
			rt.new_string('x'),
			rt.new_string(size),
...
fn wp_constrain_dimensions(var_current_width rt.PhpVal, var_current_height rt.PhpVal, max_width i64, max_height i64) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(max_width > 0
		&& rt.is_true(rt.greater(var_current_width, rt.new_int(0)))))
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(max_height > 0
		&& rt.is_true(rt.greater(var_current_height, rt.new_int(0)))))
	...
	if rt.is_true(rt.new_bool(var_did_width && rt.is_true(rt.identical(var_w, max_width - 1)))) {
		var_w = rt.new_int(max_width)
	}
	if rt.is_true(rt.new_bool(var_did_height && rt.is_true(rt.identical(var_h, max_height - 1)))) {
...
fn image_resize_dimensions(var_orig_w rt.PhpVal, var_orig_h rt.PhpVal, var_dest_w rt.PhpVal, var_dest_h rt.PhpVal, crop bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.less_equal(var_orig_w, rt.new_int(0)))
		|| rt.is_true(rt.less_equal(var_orig_h, rt.new_int(0)))))
	{
		return rt.new_bool(false)
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.less_equal(var_dest_w, rt.new_int(0)))
	...
	if !rt.is_true(var_dest_h) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.less(var_orig_w, var_dest_w))
	...
	if var_crop {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.new_bool(var_crop).is_array())))))
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.call_function('wp_fuzzy_number_match', [var_new_w.clone(), var_orig_w.clone()]))
...
fn image_make_intermediate_size(var_file rt.PhpVal, var_width rt.PhpVal, var_height rt.PhpVal, crop bool) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_width) || rt.is_true(var_height))) {
		var_editor = wp_get_image_editor(var_file.clone(), rt.new_null())
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_wp_error', [var_editor.clone()]))
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_wp_error', [var_resized_file.clone()])))))
...
fn wp_image_matches_ratio(var_source_width rt.PhpVal, var_source_height rt.PhpVal, var_target_width rt.PhpVal, var_target_height rt.PhpVal) bool {
	...
	var_matched =
		rt.is_true(rt.call_function('wp_fuzzy_number_match', [var_constrained_size.array_get(0), var_expected_size.array_get(0)]))
		&& rt.is_true(rt.call_function('wp_fuzzy_number_match', [var_constrained_size.array_get(1), var_expected_size.array_get(1)]))
	return var_matched
}
...
fn image_get_intermediate_size(var_post_id rt.PhpVal, size string) bool {
	...
	var_imagedata = rt.call_function('wp_get_attachment_metadata', [
		var_post_id.clone()])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(var_size.len > 0 && var_size != '0')
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_imagedata.clone().is_array())))))))
		|| !rt.is_true(var_imagedata.array_get('sizes'))))
	{
		return false
	}
	...
	if rt.is_true(rt.new_bool(rt.new_string(size).is_array())) {
		...
		if !(var_imagedata.array_isset(rt.new_string('file')))
			&& var_imagedata.array_get('sizes').array_isset(rt.new_string('full')) {
			var_imagedata.array_set('height',
				var_imagedata.array_get('sizes').array_get('full').array_get('height'))
			var_imagedata.array_set('width',
				var_imagedata.array_get('sizes').array_get('full').array_get('width'))
		}
		mut iter_1 := var_imagedata.array_get('sizes').iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_data_shadow := item_1.val
			mut var__size_shadow := item_1.key
			if rt.new_int((var_data_shadow.array_get('width')).to_i64()) == rt.new_int((rt.new_string(size).array_get(0)).to_i64())
				&& rt.new_int((var_data_shadow.array_get('height')).to_i64()) == rt.new_int((rt.new_string(size).array_get(1)).to_i64()) {
				var_candidates.array_set(rt.mul(var_data_shadow.array_get('width'),
					var_data_shadow.array_get('height')), var_data_shadow.clone())
				break
			}
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.greater_equal(var_data_shadow.array_get('width'), rt.new_string(size).array_get(0)))
				&& rt.is_true(rt.greater_equal(var_data_shadow.array_get('height'), rt.new_string(size).array_get(1)))))
			{
				if rt.is_true(rt.new_bool(
					rt.is_true(rt.identical(rt.new_int(0), rt.new_string(size).array_get(0)))
					|| rt.is_true(rt.identical(rt.new_int(0), rt.new_string(size).array_get(1)))))
				{
					var_same_ratio = wp_image_matches_ratio(var_data_shadow.array_get('width'),
						var_data_shadow.array_get('height'), var_imagedata.array_get('width'),
						var_imagedata.array_get('height'))
				} else {
					var_same_ratio = wp_image_matches_ratio(var_data_shadow.array_get('width'),
						var_data_shadow.array_get('height'), rt.new_string(size).array_get(0),
						rt.new_string(size).array_get(1))
				}
				if var_same_ratio {
					var_candidates.array_set(rt.mul(var_data_shadow.array_get('width'),
						var_data_shadow.array_get('height')), var_data_shadow.clone())
		...
		if !(!rt.is_true(var_candidates)) {
			...
			rt.is_true(rt.new_bool(!(!rt.is_true(var_imagedata.array_get('sizes').array_get('thumbnail')))
			&& rt.is_true(rt.less_equal(rt.new_string(size).array_get(0), var_imagedata.array_get('sizes').array_get('thumbnail').array_get('width')))))
			&& rt.is_true(rt.less_equal(rt.new_string(size).array_get(1), var_imagedata.array_get('sizes').array_get('thumbnail').array_get('width')))))
		{
			var_data = var_imagedata.array_get('sizes').array_get('thumbnail')
		} else {
			return false
		}
		mut list_tmp_6 := image_constrain_size_for_editor(var_data.array_get('width'),
			var_data.array_get('height'), size, rt.new_null())
		var_data.array_get_mut('width') = list_tmp_6.array_get(0)
		var_data.array_get_mut('height') = list_tmp_6.array_get(1)
	} else if !(!rt.is_true(var_imagedata.array_get('sizes').array_get(size))) {
		var_data = var_imagedata.array_get('sizes').array_get(size)
	}
	if !rt.is_true(var_data) {
		return false
	}
	if !rt.is_true(var_data.array_get('path')) && !(!rt.is_true(var_data.array_get('file')))
		&& !(!rt.is_true(var_imagedata.array_get('file'))) {
		var_file_url = rt.call_function('wp_get_attachment_url', [
			var_post_id.clone()])
		var_data.array_set('path', rt.call_function('path_join', [
			rt.call_function('dirname', [var_imagedata.array_get('file')]),
			var_data.array_get('file'),
		]))
...
fn wp_get_registered_image_subsizes() rt.PhpVal {
	...
	for {
		...
		if var_additional_sizes.array_get(var_size_name_shadow).array_isset(rt.new_string('width')) {
			var_size_data['width'] =
				rt.new_int((var_additional_sizes.array_get(var_size_name_shadow).array_get('width')).to_i64())
		} else {
		...
		if var_additional_sizes.array_get(var_size_name_shadow).array_isset(rt.new_string('height')) {
			var_size_data['height'] =
				rt.new_int((var_additional_sizes.array_get(var_size_name_shadow).array_get('height')).to_i64())
		} else {
			var_size_data['height'] = rt.new_int((rt.call_function('get_option', [
				rt.new_string('${var_size_name.to_string()}_size_h'),
			])).to_i64())
		}
		if !rt.is_true(var_size_data.array_get('width'))
			&& !rt.is_true(var_size_data.array_get('height')) {
			continue
		}
		if var_additional_sizes.array_get(var_size_name_shadow).array_isset(rt.new_string('crop')) {
			var_size_data['crop'] =
				var_additional_sizes.array_get(var_size_name_shadow).array_get('crop')
		} else {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_size_data.array_get('crop').is_array())))))
			|| !rt.is_true(var_size_data.array_get('crop'))))
		{
			var_size_data['crop'] = (var_size_data.array_get('crop')).to_bool()
		}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_image)))) {
		...
		if var_icon {
			...
			if rt.is_true(var_src) {
				...
				var_ext =
					rt.call_function('substr', [var_src_file.clone(), -4]).to_string().to_lower()
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_src) && var_width != 0))
			&& var_height != 0))
		{
...
fn wp_get_attachment_image(var_attachment_id rt.PhpVal, size string, icon bool, attr string) rt.PhpVal {
	...
	if rt.is_true(var_image) {
		...
		if rt.is_true(rt.new_bool(
			!(rt.new_string(var_attr.str()).array_isset(rt.new_string('width')))
			|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.new_string(var_attr.str()).array_get('width').is_long()
			|| rt.new_string(var_attr.str()).array_get('width').is_double())))))))
		{
			rt.new_string(var_attr.str()).array_set('width', var_width.clone())
		}
		if rt.is_true(rt.new_bool(
			!(rt.new_string(var_attr.str()).array_isset(rt.new_string('height')))
			|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.new_string(var_attr.str()).array_get('height').is_long()
			|| rt.new_string(var_attr.str()).array_get('height').is_double())))))))
		{
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.new_string(var_attr.str()).array_get('decoding'))
			|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string(var_attr.str()).array_get('decoding'), rt.create_array([rt.ArrayItem{
			key: none
			val: 'async'
		...
		if rt.is_true(rt.new_bool(
			...
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.new_string(var_attr.str()).array_get('loading')))))))
		{
			rt.new_string(var_attr.str()).array_unset(rt.new_string('loading'))
		}
		if rt.is_true(rt.new_bool(
			rt.new_string(var_attr.str()).array_isset(rt.new_string('fetchpriority'))
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.new_string(var_attr.str()).array_get('fetchpriority')))))))
		{
			rt.new_string(var_attr.str()).array_unset(rt.new_string('fetchpriority'))
		}
		if !rt.is_true(rt.new_string(var_attr.str()).array_get('srcset')) {
			...
			if rt.is_true(rt.new_bool(var_image_meta.clone().is_array())) {
				...
				if rt.is_true(rt.new_bool(rt.is_true(var_srcset)
					&& rt.is_true(rt.new_bool(rt.is_true(var_sizes)
					|| !(!rt.is_true(rt.new_string(var_attr.str()).array_get('sizes')))))))
				{
					rt.new_string(var_attr.str()).array_set('srcset', var_srcset.clone())
					if !rt.is_true(rt.new_string(var_attr.str()).array_get('sizes')) {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_add_auto_sizes)
			&& rt.new_string(var_attr.str()).array_isset(rt.new_string('loading'))))
			&& rt.is_true(rt.identical(rt.new_string('lazy'), rt.new_string(var_attr.str()).array_get('loading')))))
			&& rt.new_string(var_attr.str()).array_isset(rt.new_string('sizes'))))
			&& !(wp_sizes_attribute_includes_valid_auto(rt.new_string(var_attr.str()).array_get('sizes')))))
		{
			rt.new_string(var_attr.str()).array_set('sizes', 'auto, ' +
				(rt.new_string(var_attr.str()).array_get('sizes')).str())
		}
		...
		if rt.is_true(rt.new_bool(rt.new_string(var_attr.str()).array_isset(rt.new_string('width'))
			&& rt.is_true(rt.new_bool(rt.new_string(var_attr.str()).array_get('width').is_long()
			|| rt.new_string(var_attr.str()).array_get('width').is_double()))))
		{
			var_width = rt.call_function('absint',
				[rt.new_string(var_attr.str()).array_get('width')])
		}
		if rt.is_true(rt.new_bool(
			rt.new_string(var_attr.str()).array_isset(rt.new_string('height'))
			&& rt.is_true(rt.new_bool(rt.new_string(var_attr.str()).array_get('height').is_long()
			|| rt.new_string(var_attr.str()).array_get('height').is_double()))))
		{
			var_height = rt.call_function('absint',
				[rt.new_string(var_attr.str()).array_get('height')])
		}
...
fn wp_get_attachment_image_url(var_attachment_id rt.PhpVal, size string, icon bool) rt.PhpVal {
	...
	mut var_image := rt.new_null()
	var_image = wp_get_attachment_image_src(var_attachment_id.clone(), size, icon)
	return if !(var_image.array_get(0)).is_null() {
		var_image.array_get(0)
	} else {
		rt.new_bool(false)
	}
}
...
fn _wp_get_image_size_from_meta(var_size_name rt.PhpVal, var_image_meta rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_string('full'), var_size_name)) {
		return rt.create_array([
			rt.ArrayItem{ key: none, val: rt.call_function('absint', [
				var_image_meta.array_get('width'),
			]) },
			rt.ArrayItem{ key: none, val: rt.call_function('absint', [
				var_image_meta.array_get('height'),
			]) },
		])
	} else if !(!rt.is_true(var_image_meta.array_get('sizes').array_get(var_size_name))) {
		return rt.create_array([
			rt.ArrayItem{ key: none, val: rt.call_function('absint', [
				var_image_meta.array_get('sizes').array_get(var_size_name).array_get('width'),
			]) },
			rt.ArrayItem{ key: none, val: rt.call_function('absint', [
				var_image_meta.array_get('sizes').array_get(var_size_name).array_get('height'),
			]) },
		])
	}
	return rt.new_bool(false)
...
fn wp_get_attachment_image_srcset(var_attachment_id rt.PhpVal, size string, var_image_meta_arg rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_image_meta.clone().is_array()))))) {
		var_image_meta = rt.call_function('wp_get_attachment_metadata', [
			var_attachment_id.clone()])
	}
	var_image_src = var_image.array_get(0)
	var_size_array = rt.create_array([
		rt.ArrayItem{ key: none, val: rt.call_function('absint', [
			var_image.array_get(1)]) },
		rt.ArrayItem{ key: none, val: rt.call_function('absint', [
			var_image.array_get(2)]) },
	])
...
fn wp_calculate_image_srcset(var_size_array rt.PhpVal, var_image_src rt.PhpVal, var_image_meta_arg rt.PhpVal, attachment_id i64) rt.PhpVal {
	...
	if !rt.is_true(var_image_meta.array_get('sizes'))
		...
		|| var_image_meta.array_get('file').to_string().len < 4 {
		return rt.new_bool(false)
	}
	var_image_sizes = var_image_meta.array_get('sizes')
	var_image_width = rt.new_int((var_size_array.array_get(0)).to_i64())
	var_image_height = rt.new_int((var_size_array.array_get(1)).to_i64())
	if rt.is_true(rt.less(var_image_width, rt.new_int(1))) {
		return rt.new_bool(false)
	}
	var_image_basename = rt.call_function('wp_basename', [var_image_meta.array_get('file')])
	if rt.is_true(rt.new_bool(
		!(var_image_sizes.array_get('thumbnail').array_isset(rt.new_string('mime-type')))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('image/gif'), var_image_sizes.array_get('thumbnail').array_get('mime-type')))))))
	{
		var_image_sizes.array_push(rt.create_array([
			rt.ArrayItem{ key: 'width', val: var_image_meta.array_get('width') },
			rt.ArrayItem{ key: 'height', val: var_image_meta.array_get('height') },
			rt.ArrayItem{ key: 'file', val: var_image_basename },
		]))
	} else if rt.is_true(rt.call_function('str_contains', [var_image_src.clone(),
		var_image_meta.array_get('file')]))
	{
		return rt.new_bool(false)
	}
	var_dirname = rt.new_string(_wp_get_attachment_relative_path(var_image_meta.array_get('file')))
		...
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_starts_with', [var_image_baseurl.clone(), rt.new_string('https')])))))))
	{
		...
		var_domain = if !(var_parsed.array_get('host')).is_null() {
			var_parsed.array_get('host')
		} else {
			rt.new_string('')
		}
		if var_parsed.array_isset(rt.new_string('port')) {
			var_domain = rt.concat(var_domain, rt.new_string(':' +
				(var_parsed.array_get('port')).str()))
		}
		if rt.is_true(rt.identical(rt.get_superglobal('_SERVER').array_get('HTTP_HOST'), var_domain)) {
	...
	for {
		...
		var_is_src = false
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_image_shadow.clone().is_array()))))) {
			continue
		}
		if rt.is_true(rt.new_bool(!var_src_matched
			&& rt.is_true(rt.call_function('str_contains', [var_image_src.clone(), rt.new_string(var_dirname.str() + (var_image_shadow.array_get('file')).str())]))))
		{
			var_src_matched = true
			var_is_src = true
		}
		if rt.is_true(rt.new_bool(rt.is_true(var_image_edited)
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('strpos', [var_image_shadow.array_get('file'), var_image_edit_hash.array_get(0)])))))))
		{
			continue
		}
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_max_srcset_image_width)
			&& rt.is_true(rt.greater(var_image_shadow.array_get('width'), var_max_srcset_image_width))))
			&& !var_is_src))
		{
			continue
		}
		if rt.is_true(rt.new_bool(wp_image_matches_ratio(var_image_width.clone(),
			var_image_height.clone(), var_image_shadow.array_get('width'),
			var_image_shadow.array_get('height'))))
		{
			var_source = rt.create_array([
				rt.ArrayItem{ key: 'url', val: var_image_baseurl.str() +
					(var_image_shadow.array_get('file')).str() },
				rt.ArrayItem{ key: 'descriptor', val: 'w' },
				rt.ArrayItem{ key: 'value', val: var_image_shadow.array_get('width') },
			])
			if var_is_src {
				var_sources = rt.add(rt.create_array([
					rt.ArrayItem{ key: var_image_shadow.array_get('width'), val: var_source },
				]), var_sources)
			} else {
				var_sources.array_set(var_image_shadow.array_get('width'), var_source.clone())
			}
		}
	}
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!var_src_matched
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_sources.clone().is_array())))))))
	...
	for {
		...
		var_srcset = var_srcset +
			(rt.call_function('str_replace', [rt.new_string(' '), rt.new_string('%20'), var_source_shadow.array_get('url')])).str() +
			' ' +
			(var_source_shadow.array_get('value')).str() + (var_source_shadow.array_get('descriptor')).str() + ', '
	}
...
fn wp_calculate_image_sizes(var_size rt.PhpVal, var_image_src rt.PhpVal, var_image_meta_arg rt.PhpVal, attachment_id i64) bool {
	...
	if rt.is_true(rt.new_bool(var_size.clone().is_array())) {
		var_width = rt.call_function('absint', [var_size.array_get(0)])
	} else if rt.is_true(rt.new_bool(var_size.clone().is_string())) {
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_image_meta))))
			&& var_attachment_id != 0))
		{
			var_image_meta = rt.call_function('wp_get_attachment_metadata', [
				rt.new_int(attachment_id),
		...
		if rt.is_true(rt.new_bool(var_image_meta.clone().is_array())) {
			...
			if rt.is_true(var_size_array) {
				var_width = rt.call_function('absint', [var_size_array.array_get(0)])
			}
		}
	}
...
fn wp_image_file_matches_image_meta(var_image_location rt.PhpVal, var_image_meta rt.PhpVal, attachment_id i64) rt.PhpVal {
	...
	if var_image_meta.array_isset(rt.new_string('file'))
		...
		if rt.is_true(rt.identical(rt.call_function('strrpos', [
			var_image_location.clone(), var_image_meta.array_get('file')]),
			var_image_location.clone().to_string().len - var_image_meta.array_get('file').to_string().len))
		{
			var_match = true
		} else {
			var_dirname =
				rt.new_string(_wp_get_attachment_relative_path(var_image_meta.array_get('file')))
			if rt.is_true(var_dirname) {
			...
			if !(!rt.is_true(var_image_meta.array_get('original_image'))) {
				var_relative_path = rt.new_string(var_dirname.str() +
					(var_image_meta.array_get('original_image')).str())
				if rt.is_true(rt.identical(rt.call_function('strrpos', [
			...
			if !var_match && !(!rt.is_true(var_image_meta.array_get('sizes'))) {
				mut iter_6 := var_image_meta.array_get('sizes').iterator()
				for {
					item_6 := iter_6.next() or { break }
					mut var_image_size_data_shadow := item_6.val
					var_relative_path = rt.new_string(var_dirname.str() +
						(var_image_size_data_shadow.array_get('file')).str())
					if rt.is_true(rt.identical(rt.call_function('strrpos', [
						var_image_location.clone(), var_relative_path.clone()]),
						var_image_location.clone().to_string().len - var_relative_path.clone().to_string().len))
					{
...
fn wp_image_src_get_dimensions(var_image_src rt.PhpVal, var_image_meta rt.PhpVal, attachment_id i64) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_image_meta.array_isset(rt.new_string('file'))
		&& rt.is_true(rt.call_function('str_contains', [var_image_src.clone(), rt.call_function('wp_basename', [var_image_meta.array_get('file')])]))))
	{
		var_dimensions = rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_int((var_image_meta.array_get('width')).to_i64()) },
			rt.ArrayItem{ key: none, val: rt.new_int((var_image_meta.array_get('height')).to_i64()) },
		])
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_dimensions))))
		&& !(!rt.is_true(var_image_meta.array_get('sizes')))))
	{
		var_src_filename = rt.call_function('wp_basename', [var_image_src.clone()])
		mut iter_7 := var_image_meta.array_get('sizes').iterator()
		for {
			item_7 := iter_7.next() or { break }
			mut var_image_size_data_shadow := item_7.val
			if rt.is_true(rt.identical(var_src_filename,
				var_image_size_data_shadow.array_get('file')))
			{
				var_dimensions = rt.create_array([
					rt.ArrayItem{
						key: none
						val: rt.new_int((var_image_size_data_shadow.array_get('width')).to_i64())
					},
					rt.ArrayItem{
						key: none
						val: rt.new_int((var_image_size_data_shadow.array_get('height')).to_i64())
					},
				])
				break
			}
...
fn wp_image_add_srcset_and_sizes(var_image rt.PhpVal, var_image_meta rt.PhpVal, var_attachment_id rt.PhpVal) rt.PhpVal {
	...
	{ var_match_src.array_get(1) } else { rt.new_string('') }
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(var_image_meta.array_isset(rt.new_string('file'))
		&& rt.is_true(rt.call_function('preg_match', [rt.new_string('/-e[0-9]{13}/'), var_image_meta.array_get('file'), rt.create_array_from_list(var_img_edit_hash)]))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [rt.call_function('wp_basename', [var_image_src.clone()]), var_img_edit_hash.array_get(0)])))))))
	{
		return var_image.clone()
	}
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_srcset) && rt.is_true(var_sizes))) {
...
fn wp_filter_content_tags(var_content_arg rt.PhpVal, var_context_arg rt.PhpVal) rt.PhpVal {
	...
	for var_match_shadow in var_matches {
		if var_images.array_isset(var_match_shadow.array_get(0)) {
			var_filtered_image = var_match_shadow.array_get(0)
			var_attachment_id = var_images.array_get(var_match_shadow.array_get(0))
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(rt.is_true(rt.greater(var_attachment_id, rt.new_int(0)))
			...
			if rt.is_true(rt.new_bool(rt.is_true(rt.greater(var_attachment_id, rt.new_int(0)))
			...
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_filtered_image,
				var_match_shadow.array_get(0)))))
			{
				var_content = rt.call_function('str_replace', [
					var_match_shadow.array_get(0), var_filtered_image.clone(),
					var_content.clone()])
			}
			var_images.array_unset(var_match_shadow.array_get(0))
		}
		if var_iframes.array_isset(var_match_shadow.array_get(0)) {
			var_filtered_iframe = var_match_shadow.array_get(0)
			if rt.is_true(rt.new_bool(var_add_iframe_loading_attr
			...
			{
				var_content = rt.call_function('str_replace', [
					var_match_shadow.array_get(0), var_filtered_iframe.clone(),
					var_content.clone()])
			}
			var_iframes.array_unset(var_match_shadow.array_get(0))
		}
	}
	return var_content.clone()
}
...
fn wp_img_tag_add_auto_sizes(image string) string {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_loading.clone().is_string())))))
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_width.clone().is_string())))))
		|| rt.is_true(rt.identical(rt.new_string(''), var_width))))
	{
		return image
	}
	var_sizes = var_processor.get_attribute(rt.new_string('sizes'))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_sizes.clone().is_string()))))) {
		return image
	}
...
fn wp_img_tag_add_loading_optimization_attrs(var_image_arg rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	...
	{ var_matche_src.array_get(1) } else { rt.new_null() }
	var_width = if rt.is_true(rt.call_function('preg_match', [
		rt.new_string('/ width=["\']([0-9]+)["\']/'),
		var_image.clone(),
		var_match_width.clone(),
	]))
	{ rt.new_int((var_match_width.array_get(1)).to_i64()) } else { rt.new_null() }
	var_height = if rt.is_true(rt.call_function('preg_match', [
		rt.new_string('/ height=["\']([0-9]+)["\']/'),
		var_image.clone(),
		var_match_height.clone(),
	]))
	{ rt.new_int((var_match_height.array_get(1)).to_i64()) } else { rt.new_null() }
	var_loading_val = if rt.is_true(rt.call_function('preg_match', [
		rt.new_string('/ loading=["\']([A-Za-z]+)["\']/'),
		var_image.clone(),
		var_match_loading.clone(),
	]))
	{ var_match_loading.array_get(1) } else { rt.new_null() }
	...
	if !rt.is_true(var_decoding_val) {
		var_filtered_decoding_attr = rt.call_function('apply_filters', [
			...
			if !(var_optimization_attrs.array_get('decoding')).is_null() {
				var_optimization_attrs.array_get('decoding')
			} else {
				rt.new_bool(false)
			},
			var_image.clone(),
		...
		if rt.is_true(rt.new_bool(var_optimization_attrs.array_isset(rt.new_string('decoding'))
		...
		if !(!rt.is_true(var_optimization_attrs.array_get('decoding'))) {
			var_image = rt.call_function('str_replace', [rt.new_string('<img'),
				rt.new_string('<img decoding="' +
					(rt.call_function('esc_attr', [var_optimization_attrs.array_get('decoding')])).str() +
					'"'),
				var_image.clone()])
		}
	}
	...
	if !rt.is_true(var_loading_val) && var_loading_attrs_enabled {
		var_filtered_loading_attr = rt.call_function('apply_filters', [
			...
			if !(var_optimization_attrs.array_get('loading')).is_null() {
				var_optimization_attrs.array_get('loading')
			} else {
				rt.new_bool(false)
			},
			var_image.clone(),
		...
		if rt.is_true(rt.new_bool(var_optimization_attrs.array_isset(rt.new_string('loading'))
		...
		{
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_optimization_attrs.array_isset(rt.new_string('fetchpriority'))
				&& rt.is_true(rt.identical(rt.new_string('high'), var_optimization_attrs.array_get('fetchpriority')))))
				&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(if !(var_optimization_attrs.array_get('loading')).is_null() { var_optimization_attrs.array_get('loading') } else { rt.new_bool(false) }, var_filtered_loading_attr))))))
		...
		if !(!rt.is_true(var_optimization_attrs.array_get('loading'))) {
			var_image = rt.call_function('str_replace', [rt.new_string('<img'),
				rt.new_string('<img loading="' +
					(rt.call_function('esc_attr', [var_optimization_attrs.array_get('loading')])).str() +
					'"'),
				var_image.clone()])
		}
	}
	if !rt.is_true(var_fetchpriority_val)
		&& !(!rt.is_true(var_optimization_attrs.array_get('fetchpriority'))) {
		var_image = rt.call_function('str_replace', [rt.new_string('<img'),
			rt.new_string('<img fetchpriority="' +
				(rt.call_function('esc_attr', [var_optimization_attrs.array_get('fetchpriority')])).str() +
				'"'),
			var_image.clone()])
	}
	return var_image.clone()
...
fn wp_img_tag_add_width_and_height_attr(var_image rt.PhpVal, var_context rt.PhpVal, var_attachment_id rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_bool(true), var_add)) {
		...
		var_size_array = wp_image_src_get_dimensions(var_image_src.clone(), var_image_meta.clone(),
			var_attachment_id.clone())
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_size_array)
			&& rt.is_true(var_size_array.array_get(0)))) && rt.is_true(var_size_array.array_get(1))))
		{
			...
			if rt.is_true(var_style_width) {
				var_size_array.array_set(1, rt.new_int((rt.call_function('round', [
					rt.div(rt.mul(var_size_array.array_get(1), var_style_width),
						var_size_array.array_get(0)),
				])).to_i64()))
				var_size_array.array_set(0, var_style_width.clone())
			}
			var_hw =
				image_hwstring(var_size_array.array_get(0), var_size_array.array_get(1)).trim_space()
...
fn wp_iframe_tag_add_loading_attr(var_iframe rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_iframe.clone(), rt.new_string(' src="')])))))
	...
	var_value = if !(var_optimization_attrs.array_get('loading')).is_null() {
		var_optimization_attrs.array_get('loading')
	} else {
		rt.new_bool(false)
	}
...
fn _wp_post_thumbnail_class_filter(var_attr rt.PhpVal) rt.PhpVal {
	var_attr.array_get('class') = rt.concat(var_attr.array_get('class'),
...
fn img_caption_shortcode(var_attr rt.PhpVal, content string) rt.PhpVal {
	...
	if !(var_attr.array_isset(rt.new_string('caption'))) {
		...
		]))
		{
			...
			var_attr.array_set('caption', var_matches.array_get(2).to_string().trim_space())
		}
	} else if rt.is_true(rt.call_function('str_contains', [var_attr.array_get('caption'),
		rt.new_string('<')]))
	{
		var_attr.array_set('caption', rt.call_function('wp_kses', [
			var_attr.array_get('caption'),
			rt.new_string('post'),
		]))
	}
	var_output = rt.call_function('apply_filters', [
	...
	var_atts.array_set('width', rt.new_int((var_atts.array_get('width')).to_i64()))
	if rt.is_true(rt.new_bool(rt.is_true(rt.less(var_atts.array_get('width'), rt.new_int(1)))
		|| !rt.is_true(var_atts.array_get('caption'))))
	{
	...
	if rt.is_true(var_atts.array_get('id')) {
		var_atts.array_set('id', rt.call_function('sanitize_html_class', [
			var_atts.array_get('id'),
		]))
		var_id = rt.new_string('id="' +
			(rt.call_function('esc_attr', [var_atts.array_get('id')])).str() + '" ')
	}
	if rt.is_true(var_atts.array_get('caption_id')) {
		var_atts.array_set('caption_id', rt.call_function('sanitize_html_class', [
			var_atts.array_get('caption_id'),
		]))
	} else if rt.is_true(var_atts.array_get('id')) {
		var_atts.array_set('caption_id',
			'caption-' +(rt.call_function('str_replace', [rt.new_string('_'), rt.new_string('-'), var_atts.array_get('id')])).str())
	}
	if rt.is_true(var_atts.array_get('caption_id')) {
		var_caption_id = rt.new_string('id="' +
			(rt.call_function('esc_attr', [var_atts.array_get('caption_id')])).str() + '" ')
		var_describedby = rt.new_string('aria-describedby="' +
			(rt.call_function('esc_attr', [var_atts.array_get('caption_id')])).str() + '" ')
	}
	var_class = 'wp-caption ' +
		(var_atts.array_get('align')).str() + ' ' + (var_atts.array_get('class')).str().trim_space()
	var_html5 = rt.call_function('current_theme_supports', [rt.new_string('html5'),
		rt.new_string('caption')])
	var_width = if rt.is_true(var_html5) {
		var_atts.array_get('width')
	} else {
		rt.add(rt.new_int(10), var_atts.array_get('width'))
	}
	...
	if rt.is_true(var_html5) {
		...
		var_html = rt.call_function('sprintf', [
			...
			rt.call_function('sprintf', [rt.new_string('<p %sclass="wp-caption-text">%s</p>'),
				var_caption_id.clone(), var_atts.array_get('caption')]),
		])
	}
...
fn gallery_shortcode(var_attr_arg rt.PhpVal) string {
	...
	if !(!rt.is_true(var_attr.array_get('ids'))) {
		if !rt.is_true(var_attr.array_get('orderby')) {
			var_attr.array_set('orderby', 'post__in')
		}
		var_attr.array_set('include', var_attr.array_get('ids'))
	}
	...
	var_id = rt.new_int((var_atts.array_get('id')).to_i64())
	if !(!rt.is_true(var_atts.array_get('include'))) {
		var__attachments = rt.call_function('get_posts', [
			rt.create_array([
				rt.ArrayItem{ key: 'include', val: var_atts.array_get('include') },
				...
				rt.ArrayItem{ key: 'order', val: var_atts.array_get('order') },
				rt.ArrayItem{ key: 'orderby', val: var_atts.array_get('orderby') },
			]),
		])
	...
	} else if !(!rt.is_true(var_atts.array_get('exclude'))) {
		...
		var_attachments = rt.call_function('get_children', [
			rt.create_array([rt.ArrayItem{ key: 'post_parent', val: var_id },
				rt.ArrayItem{ key: 'exclude', val: var_atts.array_get('exclude') },
				rt.ArrayItem{ key: 'post_status', val: 'inherit' },
				rt.ArrayItem{ key: 'post_type', val: 'attachment' },
				rt.ArrayItem{ key: 'post_mime_type', val: 'image' },
				rt.ArrayItem{ key: 'order', val: var_atts.array_get('order') },
	...
	if !(!rt.is_true(var_post_parent_id)) {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_post_publicly_viewable', [rt.get_property(var_post_parent, 'ID')])))))
	...
	if rt.is_true(rt.call_function('is_feed', []rt.PhpVal{})) {
		...
		for {
			...
			if !(!rt.is_true(var_atts.array_get('link'))) {
				if rt.is_true(rt.identical(rt.new_string('none'), var_atts.array_get('link'))) {
					var_output = rt.concat(var_output, wp_get_attachment_image(var_att_id_shadow.clone(),
						var_atts.array_get('size'), false, var_attr.clone()))
				} else {
					var_output = rt.concat(var_output, rt.call_function('wp_get_attachment_link', [
						var_att_id_shadow.clone(),
						var_atts.array_get('size'),
						rt.new_bool(false),
					]))
				}
			} else {
	...
	var_itemtag = rt.call_function('tag_escape', [var_atts.array_get('itemtag')])
	var_captiontag = rt.call_function('tag_escape', [var_atts.array_get('captiontag')])
	var_icontag = rt.call_function('tag_escape', [var_atts.array_get('icontag')])
	...
	var_columns = rt.new_int((var_atts.array_get('columns')).to_i64())
	...
	var_size_class = rt.call_function('sanitize_html_class', [if rt.is_true(rt.new_bool(var_atts.array_get('size').is_array())) { rt.call_function('implode', [
			rt.new_string('x'),
			var_atts.array_get('size'),
		]) } else { var_atts.array_get('size') }])
	...
	for {
		...
		if rt.is_true(rt.new_bool(!(!rt.is_true(var_atts.array_get('link')))
			&& rt.is_true(rt.identical(rt.new_string('file'), var_atts.array_get('link')))))
		{
			var_image_output = rt.call_function('wp_get_attachment_link', [
				var_id_shadow.clone(), var_atts.array_get('size'),
				rt.new_bool(false), rt.new_bool(false), rt.new_bool(false),
				var_attr.clone()])
		} else if rt.is_true(rt.new_bool(!(!rt.is_true(var_atts.array_get('link')))
			&& rt.is_true(rt.identical(rt.new_string('none'), var_atts.array_get('link')))))
		{
		...
		if var_image_meta.array_isset(rt.new_string('height'))
			...
			var_orientation = if rt.is_true(rt.greater(var_image_meta.array_get('height'),
				var_image_meta.array_get('width')))
			{
				'portrait'
			} else {
		...
		if rt.is_true(rt.new_bool(rt.is_true(var_captiontag)
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_html5))))
...
fn wp_playlist_shortcode(var_attr rt.PhpVal) string {
	...
	var_args = rt.create_array([rt.ArrayItem{ key: 'post_status', val: 'inherit' },
		rt.ArrayItem{ key: 'post_type', val: 'attachment' }, rt.ArrayItem{
			key: 'post_mime_type'
			val: var_atts.array_get('type')
		}, rt.ArrayItem{ key: 'order', val: var_atts.array_get('order') },
		rt.ArrayItem{ key: 'orderby', val: var_atts.array_get('orderby') }])
	if !(!rt.is_true(var_atts.array_get('include'))) {
		var_args.array_set('include', var_atts.array_get('include'))
		...
		var_args.array_set('exclude', var_atts.array_get('exclude'))
		var_attachments = rt.call_function('get_children', [var_args.clone()])
	} else {
		var_args.array_set('post_parent', var_id.clone())
		var_attachments = rt.call_function('get_children', [var_args.clone()])
	}
	if !(!rt.is_true(var_args.array_get('post_parent'))) {
	...
	for {
		...
		if !(!rt.is_true(var_meta)) {
			...
			if rt.is_true(rt.identical(rt.new_string('video'), var_atts.array_get('type'))) {
				if !(!rt.is_true(var_meta.array_get('width')))
					&& !(!rt.is_true(var_meta.array_get('height'))) {
					var_width = var_meta.array_get('width')
					var_height = var_meta.array_get('height')
		...
		if rt.is_true(var_atts.array_get('images')) {
	...
	var_safe_type = rt.call_function('esc_attr', [var_atts.array_get('type')])
	var_safe_style = rt.call_function('esc_attr', [var_atts.array_get('style')])
	rt.call_function('ob_start', []rt.PhpVal{})
	if !var_is_loaded {
		rt.call_function('do_action', [rt.new_string('wp_playlist_scripts'),
			var_atts.array_get('type'), var_atts.array_get('style')])
		var_is_loaded = true
	}
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	if rt.is_true(rt.identical(rt.new_string('audio'), var_atts.array_get('type'))) {
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(var_safe_type)
...
fn wp_audio_shortcode(var_attr rt.PhpVal, content string) rt.PhpVal {
	...
	var_primary = false
	if !(!rt.is_true(var_atts.array_get('src'))) {
		var_type = rt.call_function('wp_check_filetype', [var_atts.array_get('src'),
			rt.call_function('wp_get_mime_types', []rt.PhpVal{})])
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
			rt.new_string(var_type.array_get('ext').to_string().to_lower()),
		...
		])))))
		{
			return rt.call_function('sprintf', [
				...
				rt.call_function('esc_url', [var_atts.array_get('src')]),
				rt.call_function('esc_html', [var_atts.array_get('src')]),
			])
		}
		var_primary = true
		...
		for {
			...
			if !(!rt.is_true(var_atts.array_get(var_ext_shadow))) {
				...
				if rt.is_true(rt.identical(rt.new_string(var_type.array_get('ext').to_string().to_lower()),
					var_ext_shadow))
				{
					var_primary = true
				}
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('mediaelement'), var_library))
	...
	var_atts.array_set('class', rt.call_function('apply_filters', [
		...
		var_atts.array_get('class'),
		var_atts.clone(),
	]))
	var_html_atts = rt.create_array([
		rt.ArrayItem{ key: 'class', val: var_atts.array_get('class') },
		...
		rt.ArrayItem{ key: 'loop', val: rt.call_function('wp_validate_boolean', [
			var_atts.array_get('loop'),
		]) },
		rt.ArrayItem{ key: 'autoplay', val: rt.call_function('wp_validate_boolean', [
			var_atts.array_get('autoplay'),
		]) },
		rt.ArrayItem{ key: 'muted', val: rt.call_function('wp_validate_boolean', [
			var_atts.array_get('muted'),
		]) },
		rt.ArrayItem{ key: 'preload', val: var_atts.array_get('preload') },
		rt.ArrayItem{ key: 'style', val: var_atts.array_get('style') },
	])
	...
	for {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.call_function('in_array', [var_attribute_name_shadow.clone(), rt.create_array([rt.ArrayItem{
			key: none
			val: 'loop'
			...
			&& rt.is_true(rt.identical(rt.new_bool(true), var_attribute_value_shadow))))
		{
			...
			rt.is_true(rt.identical(rt.new_string('preload'), var_attribute_name_shadow))
	...
	for {
		...
		if !(!rt.is_true(var_atts.array_get(var_fallback_shadow))) {
			...
			var_html = rt.concat(var_html, rt.call_function('sprintf', [
				rt.new_string(var_source.str()).clone(), var_type.array_get('type'),
				rt.call_function('esc_url', [var_url.clone()])]))
		}
	}
...
fn wp_video_shortcode(var_attr rt.PhpVal, content string) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) {
		if rt.is_true(rt.greater(var_atts.array_get('width'), var_defaults_atts.array_get('width'))) {
			var_atts.array_set('height', rt.call_function('round', [
				rt.div(rt.mul(var_atts.array_get('height'), var_defaults_atts.array_get('width')),
					var_atts.array_get('width')),
			]))
			var_atts.array_set('width', var_defaults_atts.array_get('width'))
		}
	} else {
		if rt.is_true(rt.new_bool(!(!rt.is_true(var_content_width))
			&& rt.is_true(rt.greater(var_atts.array_get('width'), var_content_width))))
		{
			var_atts.array_set('height', rt.call_function('round', [
				rt.div(rt.mul(var_atts.array_get('height'), var_content_width),
					var_atts.array_get('width')),
			]))
	...
	if !(!rt.is_true(var_atts.array_get('src'))) {
		var_is_vimeo = rt.call_function('preg_match', [rt.new_string(var_vimeo_pattern.str()).clone(),
			var_atts.array_get('src')])
		var_is_youtube = rt.call_function('preg_match', [rt.new_string(var_yt_pattern.str()).clone(),
			var_atts.array_get('src')])
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_is_youtube))))
			&& rt.is_true(rt.new_bool(!(rt.is_true(var_is_vimeo))))))
		{
			var_type = rt.call_function('wp_check_filetype', [
				var_atts.array_get('src'), rt.call_function('wp_get_mime_types', []rt.PhpVal{})])
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
				rt.new_string(var_type.array_get('ext').to_string().to_lower()),
				var_default_types.clone(),
				rt.new_bool(true),
	...
	if rt.is_true(rt.identical(rt.new_string('mediaelement'), var_library)) {
		if rt.is_true(var_is_youtube) {
			...
			var_vimeo_src = rt.new_string('https://' +
				(var_parsed_vimeo_url.array_get('host')).str() +
				(var_parsed_vimeo_url.array_get('path')).str())
			var_loop = if rt.is_true(var_atts.array_get('loop')) { '1' } else { '0' }
	...
	for {
		...
		if !(!rt.is_true(var_atts.array_get(var_fallback_shadow))) {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.identical(rt.new_string('src'), var_fallback_shadow))
	...
	if !(!rt.is_true(var_atts.array_get('width'))) {
...
fn get_attachment_taxonomies(var_attachment_arg rt.PhpVal, output string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_attachment.clone().is_object()))))) {
		return rt.new_array()
	}
...
fn get_taxonomies_for_attachments(output string) rt.PhpVal {
	...
	for {
		...
		for {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.identical(rt.new_string('attachment'), var_object_type_shadow))
...
fn is_gd_image(var_image rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.instance_of(var_image, 'GdImage')))
		|| rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_resource', [var_image.clone()]))
...
fn wp_imagecreatetruecolor(var_width rt.PhpVal, var_height rt.PhpVal) rt.PhpVal {
	...
	var_img = rt.call_function('imagecreatetruecolor', [var_width.clone(),
		var_height.clone()])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(is_gd_image(var_img.clone())
...
fn wp_get_image_editor(var_path rt.PhpVal, var_args rt.PhpVal) rt.PhpVal {
	...
	if !(var_args.array_isset(rt.new_string('mime_type'))) {
		var_file_info = rt.call_function('wp_check_filetype', [
			var_args.array_get('path')])
		if rt.is_true(rt.new_bool(!var_file_info.is_null()
			&& rt.is_true(var_file_info.array_get('type'))))
		{
			var_args.array_set('mime_type', var_file_info.array_get('type'))
		}
	}
	if var_args.array_isset(rt.new_string('mime_type')) {
		var_output_format = wp_get_image_editor_output_format(var_path.clone(),
			var_args.array_get('mime_type'))
		if var_output_format.array_isset(var_args.array_get('mime_type')) {
			var_args.array_set('output_mime_type',
				var_output_format.array_get(var_args.array_get('mime_type')))
		}
	}
	...
	return create_wp_error(rt.new_string('image_no_editor'), rt.call_function('__', [
...
fn _wp_image_editor_choose(var_args rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_editors.clone().is_array()))))) {
	...
	if var_editors.array_isset(rt.new_string(var_cache_key.str())) {
		return var_editors.array_get(var_cache_key)
	}
	...
	for {
		...
		if rt.is_true(rt.new_bool(var_args.array_isset(rt.new_string('mime_type'))
		...
		}, rt.ArrayItem{ key: none, val: 'supports_mime_type' }]), var_args.array_get('mime_type')])))))))
		{
			continue
		}
		if rt.is_true(rt.new_bool(var_args.array_isset(rt.new_string('methods'))
			&& rt.is_true(rt.call_function('array_diff', [var_args.array_get('methods'), rt.call_function('get_class_methods', [var_implementation_shadow.clone()])]))))
		{
			continue
		}
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(var_args.array_isset(rt.new_string('mime_type'))
			&& var_args.array_isset(rt.new_string('output_mime_type'))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_args.array_get('mime_type'), var_args.array_get('output_mime_type')))))))
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('call_user_func', [rt.create_array([rt.ArrayItem{
			key: none
			val: var_implementation_shadow
		}, rt.ArrayItem{ key: none, val: 'supports_mime_type' }]), var_args.array_get('output_mime_type')])))))))
		{
...
fn wp_plupload_default_settings() {
	...
	if rt.is_true(rt.new_bool(
		...
		&& rt.is_true(rt.call_function('str_contains', [rt.get_superglobal('_SERVER').array_get('HTTP_USER_AGENT'), rt.new_string('OS 7_')]))))
		&& rt.is_true(rt.call_function('str_contains', [rt.get_superglobal('_SERVER').array_get('HTTP_USER_AGENT'), rt.new_string('like Mac OS X')]))))
	{
...
fn wp_prepare_attachment_for_js(var_attachment_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.get_property(var_attachment, 'post_parent')) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(var_post_parent)
	...
	if var_meta.array_isset(rt.new_string('filesize')) {
		var_bytes = var_meta.array_get('filesize')
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_meta)
		&& rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('image'), var_type))
		|| !(!rt.is_true(var_meta.array_get('sizes')))))))
	{
		...
		for {
			...
			if rt.is_true(var_downsize) {
				if !rt.is_true(var_downsize.array_get(3)) {
					continue
				}
				var_sizes.array_set(var_size_shadow, rt.create_array([
					rt.ArrayItem{ key: 'height', val: var_downsize.array_get(2) },
					rt.ArrayItem{ key: 'width', val: var_downsize.array_get(1) },
					rt.ArrayItem{ key: 'url', val: var_downsize.array_get(0) },
					rt.ArrayItem{
						key: 'orientation'
						val: if rt.is_true(rt.greater(var_downsize.array_get(2),
							var_downsize.array_get(1)))
						{
							'portrait'
						} else {
							'landscape'
			...
			} else if var_meta.array_get('sizes').array_isset(var_size_shadow) {
				var_size_meta = var_meta.array_get('sizes').array_get(var_size_shadow)
				mut list_tmp_19 := image_constrain_size_for_editor(var_size_meta.array_get('width'),
					var_size_meta.array_get('height'), var_size_shadow.clone(),
					rt.new_string('edit'))
				...
				var_sizes.array_set(var_size_shadow, rt.create_array([
					...
					rt.ArrayItem{ key: 'url', val: var_base_url.str() +
						(var_size_meta.array_get('file')).str() },
					rt.ArrayItem{
						key: 'orientation'
		...
		if rt.is_true(rt.identical(rt.new_string('image'), var_type)) {
			if !(!rt.is_true(var_meta.array_get('original_image'))) {
			...
			if var_meta.array_isset(rt.new_string('height'))
				&& var_meta.array_isset(rt.new_string('width')) {
				var_sizes.array_get_mut('full').array_set('height', var_meta.array_get('height'))
				var_sizes.array_get_mut('full').array_set('width', var_meta.array_get('width'))
				var_sizes.array_get_mut('full').array_set('orientation', if rt.is_true(rt.greater(var_meta.array_get('height'),
					var_meta.array_get('width')))
				{
					'portrait'
				} else {
					'landscape'
			...
			var_response = rt.call_function('array_merge', [var_response.clone(),
				var_sizes.array_get('full')])
		} else if rt.is_true(var_meta.array_get('sizes').array_get('full').array_get('file')) {
			var_sizes.array_set('full', rt.create_array([
				rt.ArrayItem{ key: 'url', val: var_base_url.str() +
					(var_meta.array_get('sizes').array_get('full').array_get('file')).str() },
				rt.ArrayItem{
					key: 'height'
					val: var_meta.array_get('sizes').array_get('full').array_get('height')
				},
				rt.ArrayItem{
					key: 'width'
					val: var_meta.array_get('sizes').array_get('full').array_get('width')
				},
				rt.ArrayItem{
					key: 'orientation'
					val: if rt.is_true(rt.greater(var_meta.array_get('sizes').array_get('full').array_get('height'),
						var_meta.array_get('sizes').array_get('full').array_get('width')))
					{
						'portrait'
					} else {
						'landscape'
		...
		&& rt.is_true(rt.identical(rt.new_string('video'), var_type))))
	{
		if var_meta.array_isset(rt.new_string('width')) {
			var_response.array_set('width', rt.new_int((var_meta.array_get('width')).to_i64()))
		}
		if var_meta.array_isset(rt.new_string('height')) {
			var_response.array_set('height', rt.new_int((var_meta.array_get('height')).to_i64()))
		}
	}
	if rt.is_true(rt.new_bool(rt.is_true(var_meta)
		&& rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('audio'), var_type))
		|| rt.is_true(rt.identical(rt.new_string('video'), var_type))))))
	{
		if var_meta.array_isset(rt.new_string('length_formatted')) {
			var_response.array_set('fileLength', var_meta.array_get('length_formatted'))
...
fn wp_enqueue_media(var_args_arg rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_months.clone().is_array()))))) {
	...
	if var_args.array_isset(rt.new_string('post')) {
		var_post = rt.call_function('get_post', [var_args.array_get('post')])
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!var_thumbnail_support
...
fn get_media_embedded_in_content(var_content rt.PhpVal, var_types_arg rt.PhpVal) rt.PhpVal {
	...
	if !(!rt.is_true(var_types)) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.create_array_from_list(var_types).is_array()))))) {
			var_types = [var_types]
		}
	...
	{
		mut iter_37 := var_matches.array_get(0).iterator()
		for {
...
fn get_post_galleries(var_post_arg rt.PhpVal, html bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('has_shortcode', [rt.get_property(var_post, 'post_content'), rt.new_string('gallery')])))))
	...
	]))
	{
		for var_shortcode_shadow in var_matches {
			if rt.is_true(rt.identical(rt.new_string('gallery'), var_shortcode_shadow.array_get(2))) {
				var_srcs = rt.new_array()
				var_shortcode_attrs = rt.call_function('shortcode_parse_atts', [
					var_shortcode_shadow.array_get(3),
				])
				if !(var_shortcode_attrs.array_isset(rt.new_string('id'))) {
					var_shortcode_shadow.array_get(3) = rt.concat(var_shortcode_shadow.array_get(3), rt.new_string(
				...
				if var_html {
					...
					if !(!rt.is_true(var_src)) {
						...
						for {
							...
							var_srcs << var_s_shadow.array_get(2)
						}
					}
		...
		rt.get_property(var_post, 'post_content')]))
	{
		...
		for rt.is_true(var_block) {
			var_has_inner_blocks = !(!rt.is_true(var_block.array_get('innerBlocks')))
			if rt.is_true(rt.new_bool(!(rt.is_true(var_block.array_get('blockName'))))) {
				continue
			}
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('core/gallery'),
				var_block.array_get('blockName')))))
			{
				if var_has_inner_blocks {
					var_post_blocks.clone().array_push(var_block.array_get('innerBlocks'))
				}
				continue
			}
			if var_has_inner_blocks && var_html {
				var_block_html = rt.call_function('wp_list_pluck', [
					var_block.array_get('innerBlocks'),
			...
			if var_has_inner_blocks {
				...
				for {
					...
					var_url = rt.call_function('wp_get_attachment_url', [
						var_id_shadow.clone()])
					if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_url.clone().is_string()))
			...
			if var_html {
				var_galleries.array_push(var_block.array_get('innerHTML'))
				continue
			}
			var_ids = if !(!rt.is_true(var_block.array_get('attrs').array_get('ids'))) {
				var_block.array_get('attrs').array_get('ids')
			} else {
				rt.new_array()
			}
			...
			rt.call_function('preg_match_all', [rt.new_string('#src=([\'"])(.+?)\\1#is'),
				var_block.array_get('innerHTML'), rt.create_array_from_list(var_found_srcs),
				rt.get_constant('PREG_SET_ORDER')])
			if !(!rt.is_true(var_found_srcs.array_get(0))) {
				for var_src_shadow in var_found_srcs {
					if rt.is_true(rt.new_bool(var_src_shadow.array_isset(rt.new_int(2))
						&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_src_shadow.array_get(2), rt.create_array_from_list(var_srcs), rt.new_bool(true)])))))))
					{
						var_srcs << var_src_shadow.array_get(2)
					}
				}
			}
			var_galleries.array_push(rt.create_array([
...
fn get_post_gallery_images(post i64) rt.PhpVal {
	...
	return if !rt.is_true(var_gallery.array_get('src')) {
		rt.new_array()
	} else {
		var_gallery.array_get('src')
	}
}
...
fn wp_maybe_generate_attachment_metadata(var_attachment rt.PhpVal) {
		...
		&& rt.is_true(rt.call_function('file_exists', [var_file.clone()]))))
	{
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var__meta.clone().array_isset(rt.new_string('_wp_attachment_metadata')))))))
...
fn attachment_url_to_postid(var_url rt.PhpVal) i64 {
	...
	var_site_url = rt.call_function('parse_url', [var_dir.array_get('url')])
	var_image_path = rt.call_function('parse_url', [var_path.clone()])
	if rt.is_true(rt.new_bool(var_image_path.array_isset(rt.new_string('scheme'))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_image_path.array_get('scheme'), var_site_url.array_get('scheme')))))))
	{
		var_path = rt.call_function('str_replace', [var_image_path.array_get('scheme'),
			var_site_url.array_get('scheme'), var_path.clone()])
	}
	if rt.is_true(rt.call_function('str_starts_with', [var_path.clone(),
		rt.new_string((var_dir.array_get('baseurl')).str() + '/')]))
	{
		var_path = rt.call_function('substr', [var_path.clone(),
			rt.new_int((var_dir.array_get('baseurl')).str() + '/'.len)])
	}
...
fn wp_getimagesize(var_filename rt.PhpVal, var_image_info rt.PhpVal) rt.PhpVal {
	...
	if !(!rt.is_true(var_info)) && !(!rt.is_true(var_info.array_get(0))
		&& !rt.is_true(var_info.array_get(1))) {
	...
	if rt.is_true(rt.identical(rt.new_string('image/webp'), var_image_mime_type)) {
		...
		var_width = var_webp_info.array_get('width')
		var_height = var_webp_info.array_get('height')
		if rt.is_true(rt.new_bool(rt.is_true(var_width) && rt.is_true(var_height))) {
	...
	if rt.is_true(rt.identical(rt.new_string('image/avif'), var_image_mime_type)) {
		...
		var_width = var_avif_info.array_get('width')
		var_height = var_avif_info.array_get('height')
		if rt.is_true(rt.new_bool(rt.is_true(var_width) && rt.is_true(var_height))) {
			return rt.create_array([rt.ArrayItem{ key: none, val: var_width },
				rt.ArrayItem{ key: none, val: var_height }, rt.ArrayItem{
					key: none
		...
		var_image_mime_type.clone()]))
	{
		...
		if rt.is_true(rt.new_bool(rt.instance_of(var_editor, 'WP_Image_Editor_Imagick'))) {
			...
			return rt.create_array([
				rt.ArrayItem{ key: none, val: var_size.array_get('width') },
				rt.ArrayItem{ key: none, val: var_size.array_get('height') },
				rt.ArrayItem{ key: none, val: rt.get_constant('IMAGETYPE_HEIF') },
				rt.ArrayItem{ key: none, val: rt.call_function('sprintf', [
					rt.new_string('width="%d" height="%d"'),
					var_size.array_get('width'),
					var_size.array_get('height'),
				]) },
	...
	if rt.is_true(rt.equal(switch_val_2, rt.new_string('VP8 '))) {
		var_parts = rt.call_function('unpack', [rt.new_string('v2'),
			rt.call_function('substr', [var_magic.clone(), rt.new_int(26),
				rt.new_int(4)])])
		var_width = rt.bitwise_and(var_parts.array_get(1), rt.new_int(16383))
		var_height = rt.bitwise_and(var_parts.array_get(2), rt.new_int(16383))
		var_type = 'lossy'
		...
		var_parts = rt.call_function('unpack', [rt.new_string('C4'),
			rt.call_function('substr', [var_magic.clone(), rt.new_int(21),
				rt.new_int(4)])])
		var_width =
			rt.bitwise_or(var_parts.array_get(1), rt.bitwise_and(var_parts.array_get(2), rt.new_int(63)) << 8) +
			1
		var_height =
			rt.bitwise_and(var_parts.array_get(2), rt.new_int(192)) >> 6 | rt.shift_left(var_parts.array_get(3), rt.new_int(2)) | rt.bitwise_and(var_parts.array_get(4), rt.new_int(3)) << 10 +
			1
		var_type = 'lossless'
		...
		var_width = (rt.call_function('unpack', [rt.new_string('V'),
			rt.new_string(
				...
				'')])).to_bool()
		var_width = rt.bitwise_and(rt.new_bool(var_width).array_get(1), rt.new_int(16777215)) + 1
		var_height = (rt.call_function('unpack', [rt.new_string('V'),
			rt.new_string(
				(rt.call_function('substr', [var_magic.clone(), rt.new_int(27), rt.new_int(3)])).str() +
				'')])).to_bool()
		var_height = rt.bitwise_and(rt.new_bool(var_height).array_get(1), rt.new_int(16777215)) + 1
...
fn wp_get_loading_optimization_attributes(tag_name string, var_attr rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool('img' != tag_name))
		&& rt.is_true(rt.new_bool('iframe' != tag_name))))
	{
		return rt.call_function('apply_filters', [
			rt.new_string('wp_get_loading_optimization_attributes'),
			var_loading_attrs.clone(),
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('the_content'), var_context))))
		&& rt.is_true(rt.call_function('doing_filter', [rt.new_string('the_content')]))))
		|| rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('widget_text_content'), var_context))))
		&& rt.is_true(rt.call_function('doing_filter', [rt.new_string('widget_text_content')]))))))
		|| rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('widget_block_content'), var_context))))
	...
	if rt.is_true(rt.identical(rt.new_string('img'), rt.new_string(tag_name))) {
		var_loading_attrs.array_set('decoding', if !(var_attr.array_get('decoding')).is_null() {
			var_attr.array_get('decoding')
		} else {
			rt.new_string('async')
		})
	}
	...
	if var_attr.array_isset(rt.new_string('loading')) {
		if rt.is_true(rt.identical(rt.new_string('lazy'), var_attr.array_get('loading'))) {
			var_maybe_in_viewport = rt.new_bool(false)
		} else {
			var_maybe_in_viewport = rt.new_bool(true)
		}
	}
	var_existing_fetchpriority = if !(var_attr.array_get('fetchpriority')).is_null() {
		var_attr.array_get('fetchpriority')
	} else {
		rt.new_null()
	}
	...
	if rt.is_true(rt.identical(rt.new_null(), var_maybe_in_viewport)) {
		...
		if var_header_enforced_contexts.array_isset(var_context) {
			...
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})))))
			...
			&& rt.is_true(rt.call_function('is_main_query', []rt.PhpVal{}))))
		{
			...
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.get_property(var_wp_query, 'before_loop'))
		...
		var_existing_fetchpriority))))
	{
		if var_increase_count {
			...
			if rt.is_true(rt.less_equal(var_wp_min_priority_img_pixels, rt.mul(var_attr.array_get('width'),
				var_attr.array_get('height'))))
			{
				wp_increase_content_media_count(0)
			}
		}
...
fn wp_maybe_add_fetchpriority_high_attr(var_loading_attrs rt.PhpVal, var_tag_name rt.PhpVal, var_attr rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_existing_fetchpriority))))
	...
	if rt.is_true(rt.new_bool(var_loading_attrs.array_isset(rt.new_string('loading'))
		&& rt.is_true(rt.identical(rt.new_string('lazy'), var_loading_attrs.array_get('loading')))))
	{
		return var_loading_attrs.clone()
	}
...
fn create_avifinfo_parser() &Class_Avifinfo_Parser {
