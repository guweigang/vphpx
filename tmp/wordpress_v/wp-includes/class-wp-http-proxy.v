...
fn (mut this Class_WP_HTTP_Proxy) is_enabled() bool {
	return rt.is_true(rt.call_function('defined', [rt.new_string('WP_PROXY_HOST')]))
		&& rt.is_true(rt.call_function('defined', [rt.new_string('WP_PROXY_PORT')]))
}

fn (mut this Class_WP_HTTP_Proxy) use_authentication() bool {
	return rt.is_true(rt.call_function('defined', [rt.new_string('WP_PROXY_USERNAME')]))
		&& rt.is_true(rt.call_function('defined', [rt.new_string('WP_PROXY_PASSWORD')]))
}
...
fn (mut this Class_WP_HTTP_Proxy) send_through_proxy(var_uri rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.identical(rt.new_string('localhost'), var_check.array_get('host')))
		|| rt.is_true(rt.new_bool(var_home.array_isset(rt.new_string('host'))
		&& rt.is_true(rt.identical(var_home.array_get('host'), var_check.array_get('host')))))))
	{
		return false
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [
		rt.new_string('WP_PROXY_BYPASS_HOSTS'),
	])))))
	{
		return true
	}
	...
	if rt.is_true(rt.identical(rt.new_null(), var_bypass_hosts)) {
		mut var_bypass_hosts := rt.call_function('preg_split', [
			rt.new_string('|,\\s*|'), rt.get_constant('WP_PROXY_BYPASS_HOSTS')])
		if rt.is_true(rt.call_function('str_contains', [
			rt.get_constant('WP_PROXY_BYPASS_HOSTS'),
			rt.new_string('*'),
		]))
		{
			mut var_wildcard_regex := rt.new_array()
			{
				...
				for {
					...
					var_wildcard_regex.array_push(rt.call_function('str_replace', [
						rt.new_string('\\*'),
						rt.new_string('.+'),
						rt.call_function('preg_quote', [var_host.dup(),
							rt.new_string('/')]),
					]))
				}
			}
			var_wildcard_regex = rt.new_string('/^(' +
				(rt.call_function('implode', [rt.new_string('|'), var_wildcard_regex.dup()])).str() +
				')$/i')
		}
	}
	if !(!rt.is_true(var_wildcard_regex)) {
		return !(rt.is_true(rt.call_function('preg_match', [var_wildcard_regex.dup(),
			var_check.array_get('host')])))
	} else {
		return !(rt.is_true(rt.call_function('in_array', [var_check.array_get('host'),
			var_bypass_hosts.dup(), rt.new_bool(true)])))
	}
	return false
}
...
pub fn init_wp_includes_class_wp_http_proxy_php() {
}
