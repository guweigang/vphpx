...
fn (mut this Class_WP_SimplePie_Sanitize_KSES) sanitize(var_data rt.PhpVal, var_type rt.PhpVal, base string) rt.PhpVal {
	...
	var_data_mutated = rt.new_string(rt.new_string(var_data_mutated.dup().to_string().trim_space()))
	if rt.is_true(rt.bitwise_and(var_type, Class_SimplePie_SimplePie.construct_maybe_html())) {
		if rt.is_true(rt.call_function('preg_match', ['/(&(#(x[0-9a-fA-F]+|[0-9]+)|[a-zA-Z0-9]+)|<\\/[A-Za-z][^\\x09\\x0A\\x0B\\x0C\\x0D\\x20\\x2F\\x3E]*' + (rt.get_constant('SIMPLEPIE_PCRE_HTML_ATTRIBUTE')).str() + '>)/', var_data_mutated.dup()])) {
	...
	if rt.is_true(rt.bitwise_and(var_type, Class_SimplePie_SimplePie.construct_base64())) {
		var_data_mutated = rt.call_function('base64_decode', [var_data_mutated.dup()])
	}
	if rt.is_true(rt.bitwise_and(var_type, rt.bitwise_or(Class_SimplePie_SimplePie.construct_html(), Class_SimplePie_SimplePie.construct_xhtml()))) {
		var_data_mutated = rt.call_function('wp_kses_post', [var_data_mutated.dup()])
		if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
			var_data_mutated = rt.call_method(rt.get_property(rt.new_object('WP_SimplePie_Sanitize_KSES', ['SimplePie_Sanitize'], &this), 'registry'), 'call', [rt.new_string('Misc'), rt.new_string('change_encoding'), rt.create_array([rt.ArrayItem{ key: none, val: var_data_mutated }, rt.ArrayItem{ key: none, val: 'UTF-8' }, rt.ArrayItem{ key: none, val: rt.get_property(rt.new_object('WP_SimplePie_Sanitize_KSES', ['SimplePie_Sanitize'], &this), 'output_encoding') }])])
		}
		return var_data_mutated.dup()
	} else {
		return this.Class_SimplePie_Sanitize.sanitize(var_data_mutated.dup(), var_type.dup(), rt.new_string(base))
	}
	return rt.new_null()
}
...
fn create_wp_simplepie_sanitize_kses() &Class_WP_SimplePie_Sanitize_KSES {
...
pub fn init_wp_includes_class_wp_simplepie_sanitize_kses_php() {
