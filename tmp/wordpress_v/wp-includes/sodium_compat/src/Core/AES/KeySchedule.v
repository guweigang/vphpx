...
fn (mut this Class_ParagonIE_Sodium_Core_AES_KeySchedule) construct(mut var_skey Class_array, numRounds i64)  {
	this.skey = var_skey.dup()
	this.numRounds = rt.new_int(numRounds).dup()
}
...
fn (mut this Class_ParagonIE_Sodium_Core_AES_KeySchedule) getroundkey(var_offset rt.PhpVal) rt.PhpVal {
	return fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_ParagonIE_Sodium_Core_AES_Block{}; return temp.fromarray(arg_0) }(rt.call_function('array_slice', [this.skey, var_offset.dup(), rt.new_int(8)]))
}

fn (mut this Class_ParagonIE_Sodium_Core_AES_KeySchedule) expand() rt.PhpVal {
	mut var_exp := create_paragonie_sodium_core_aes_expanded(rt.call_function('array_fill', [rt.new_int(0), rt.new_int(120), rt.new_int(0)]), this.numRounds)
	mut var_n := rt.new_int(rt.shift_left(rt.add(rt.get_property(var_exp, 'numRounds'), rt.new_int(1)), rt.new_int(2)))
	{
		mut var_u := rt.new_int(rt.new_int(0))
		mut var_v := rt.new_int(rt.new_int(0))
		for {
			if !(rt.is_true(rt.less(var_u, var_n))) { break }
			mut var_x := mut var_y := this.skey.array_get(var_u)
			// unsupported expression: Expr_AssignOp_BitwiseAnd
			rt.get_property(var_exp, 'skey').array_set(var_v, rt.bitwise_and(rt.bitwise_or(var_x, rt.shift_left(var_x, rt.new_int(1))), Class_ParagonIE_Sodium_Core_Util.u32_max()))
			// unsupported expression: Expr_AssignOp_BitwiseAnd
			rt.get_property(var_exp, 'skey').array_set(rt.add(var_v, rt.new_int(1)), rt.bitwise_and(rt.bitwise_or(var_y, rt.shift_right(var_y, rt.new_int(1))), Class_ParagonIE_Sodium_Core_Util.u32_max()))
			rt.pre_inc(var_u)
...
fn create_paragonie_sodium_core_aes_block() &Class_ParagonIE_Sodium_Core_AES_Block {
...
fn (mut this Class_ParagonIE_Sodium_Core_AES_KeySchedule) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'skey' { this.skey = val; return true }
		'expanded' { this.expanded = val; return true }
		'numRounds' { this.numRounds = val; return true }
...
pub fn init_wp_includes_sodium_compat_src_core_aes_keyschedule_php() {
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('ParagonIE_Sodium_Core_AES_KeySchedule'), rt.new_bool(false)])) {
		return rt.new_null()
	}
}
