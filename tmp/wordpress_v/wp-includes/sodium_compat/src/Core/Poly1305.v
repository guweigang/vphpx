...
pub fn Class_ParagonIE_Sodium_Core_Poly1305.block_size() i64 {
	return 16
}
...
fn Class_ParagonIE_Sodium_Core_Poly1305.onetimeauth(var_m rt.PhpVal, var_key rt.PhpVal) rt.PhpVal {
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.new_string('Key must be 32 bytes long.'))))
	}
	mut var_state := create_paragonie_sodium_core_poly1305_state(fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_ParagonIE_Sodium_Core_Poly1305{}; return temp.substr(arg_0, arg_1, arg_2) }(var_key.dup(), rt.new_int(0), rt.new_int(32)))
	return rt.call_method(var_state.update(var_m.dup()), 'finish', []rt.PhpVal{})
}

fn Class_ParagonIE_Sodium_Core_Poly1305.onetimeauth_verify(var_mac rt.PhpVal, var_m rt.PhpVal, var_key rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.less(fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_ParagonIE_Sodium_Core_Poly1305{}; return temp.strlen(arg_0) }(var_key.dup()), rt.new_int(32))) {
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.new_string('Key must be 32 bytes long.'))))
	}
	mut var_state := create_paragonie_sodium_core_poly1305_state(fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal, arg_2 rt.PhpVal) rt.PhpVal { mut temp := Class_ParagonIE_Sodium_Core_Poly1305{}; return temp.substr(arg_0, arg_1, arg_2) }(var_key.dup(), rt.new_int(0), rt.new_int(32)))
	mut var_calc := rt.call_method(var_state.update(var_m.dup()), 'finish', []rt.PhpVal{})
	return fn (arg_0 rt.PhpVal, arg_1 rt.PhpVal) rt.PhpVal { mut temp := Class_ParagonIE_Sodium_Core_Poly1305{}; return temp.verify_16(arg_0, arg_1) }(var_calc.dup(), var_mac.dup())
}
...
fn (mut this Class_ParagonIE_Sodium_Core_Poly1305) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'onetimeauth' {
			...
			return Class_ParagonIE_Sodium_Core_Poly1305.onetimeauth(dispatch_arg_0, dispatch_arg_1)
		}
		'onetimeauth_verify' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			dispatch_arg_2 := if args.len > 2 { args[2] } else { rt.new_null() }
			return Class_ParagonIE_Sodium_Core_Poly1305.onetimeauth_verify(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2)
		}
		else { return none }
	}
}
...
pub fn init_wp_includes_sodium_compat_src_core_poly1305_php() {
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('ParagonIE_Sodium_Core_Poly1305'), rt.new_bool(false)])) {
		return rt.new_null()
	}
}
