...
fn Class_ParagonIE_Sodium_Core_Base64_UrlSafe.encode(var_src rt.PhpVal) rt.PhpVal {
	...
	return Class_ParagonIE_Sodium_Core_Base64_UrlSafe.doencode((var_src_mutated).to_bool(), rt.new_bool(true))
}

fn Class_ParagonIE_Sodium_Core_Base64_UrlSafe.encodeunpadded(var_src rt.PhpVal) rt.PhpVal {
	mut var_src_mutated := var_src
	return Class_ParagonIE_Sodium_Core_Base64_UrlSafe.doencode((var_src_mutated).to_bool(), rt.new_bool(false))
}
...
fn Class_ParagonIE_Sodium_Core_Base64_UrlSafe.decode(var_src rt.PhpVal, strictPadding bool) string {
	...
	mut var_dest := rt.new_string(rt.new_string(''))
	{
		...
		for {
			...
			mut var_c0 := Class_ParagonIE_Sodium_Core_Base64_UrlSafe.decode6bits(var_chunk.array_get(1))
			mut var_c1 := Class_ParagonIE_Sodium_Core_Base64_UrlSafe.decode6bits(var_chunk.array_get(2))
			mut var_c2 := Class_ParagonIE_Sodium_Core_Base64_UrlSafe.decode6bits(var_chunk.array_get(3))
			mut var_c3 := Class_ParagonIE_Sodium_Core_Base64_UrlSafe.decode6bits(var_chunk.array_get(4))
			// unsupported expression: Expr_AssignOp_Concat
	...
	if rt.is_true(rt.less(var_i, var_srcLen)) {
		...
		if rt.is_true(rt.less(rt.add(var_i, rt.new_int(2)), var_srcLen)) {
			...
			var_c1 = Class_ParagonIE_Sodium_Core_Base64_UrlSafe.decode6bits(var_chunk.array_get(2))
			// unsupported expression: Expr_AssignOp_Concat
			// unsupported expression: Expr_AssignOp_BitwiseOr
...
fn Class_ParagonIE_Sodium_Core_Base64_UrlSafe.decodenopadding(var_encodedString rt.PhpVal) string {
	...
	return (Class_ParagonIE_Sodium_Core_Base64_UrlSafe.decode((var_encodedString).to_bool(), rt.new_bool(true))).str()
}
...
fn create_paragonie_sodium_core_base64_urlsafe() &Class_ParagonIE_Sodium_Core_Base64_UrlSafe {
...
fn (mut this Class_ParagonIE_Sodium_Core_Base64_UrlSafe) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'doEncode' {
			...
			return Class_ParagonIE_Sodium_Core_Base64_UrlSafe.doencode(dispatch_arg_0, dispatch_arg_1)
		}
		'decode' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).to_bool()
			return rt.new_string(Class_ParagonIE_Sodium_Core_Base64_UrlSafe.decode(dispatch_arg_0, dispatch_arg_1))
		}
		'decodeNoPadding' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
...
pub fn init_wp_includes_sodium_compat_src_core_base64_urlsafe_php() {
}
