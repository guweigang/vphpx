...
fn (mut this Class_WP_Block_List) offsetget(var_offset rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!var_block.is_null()
		&& rt.is_true(rt.new_bool(var_block.dup().is_array()))))
	{
		var_block = create_wp_block(var_block.dup(), this.available_context, this.registry)
		this.blocks.array_set(var_offset, var_block.dup())
	}
...
fn (mut this Class_WP_Block_List) offsetset(var_offset rt.PhpVal, var_value rt.PhpVal) {
	if rt.is_true(rt.new_bool(var_offset.dup().is_null())) {
		this.blocks.array_push(var_value.dup())
	} else {
		this.blocks.array_set(var_offset, var_value.dup())
	}
}
...
fn (mut this Class_WP_Block_List) valid() rt.PhpVal {
	return
}
...
pub fn init_wp_includes_class_wp_block_list_php() {
}
