...
fn (mut this Class_WP_User_Request) construct(var_post rt.PhpVal)  {
	...
	this.created_timestamp = rt.call_function('strtotime', [rt.get_property(var_post, 'post_date_gmt')])
	this.modified_timestamp = rt.call_function('strtotime', [rt.get_property(var_post, 'post_modified_gmt')])
	this.confirmed_timestamp = // unsupported expression: Expr_Cast_Int
	this.completed_timestamp = // unsupported expression: Expr_Cast_Int
	this.request_data = rt.call_function('json_decode', [rt.get_property(var_post, 'post_content'), rt.new_bool(true)])
...
fn (mut this Class_WP_User_Request) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'action_name' { this.action_name = val; return true }
		'status' { this.status = val; return true }
		'created_timestamp' { this.created_timestamp = val; return true }
		'modified_timestamp' { this.modified_timestamp = val; return true }
		'confirmed_timestamp' { this.confirmed_timestamp = val; return true }
		'completed_timestamp' { this.completed_timestamp = val; return true }
		'request_data' { this.request_data = val; return true }
		'confirm_key' { this.confirm_key = val; return true }
...
pub fn init_wp_includes_class_wp_user_request_php() {
}
