...
fn (mut this Class_WP_Text_Diff_Renderer_inline) _splitonwords(var_string rt.PhpVal, newlineEscape string) rt.PhpVal {
	...
	mut var_words := rt.call_function('preg_split', [rt.new_string('/([^\\w])/u'), var_string_mutated.dup(), // unsupported expression: Expr_UnaryMinus, rt.get_constant('PREG_SPLIT_DELIM_CAPTURE')])
	var_words = rt.call_function('str_replace', [rt.new_string('\n'), rt.new_string(newlineEscape), var_words.dup()])
...
fn create_wp_text_diff_renderer_inline() &Class_WP_Text_Diff_Renderer_inline {
...
pub fn init_wp_includes_class_wp_text_diff_renderer_inline_php() {
}
