...
fn wp_robots() {
	mut var_robots := rt.call_function('apply_filters', [rt.new_string('wp_robots'),
		rt.new_array()])
	mut var_robots_strings := rt.new_array()
	{
		mut iter_1 := var_robots.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_value := item_1.val
			mut var_directive := item_1.key
			if rt.is_true(rt.new_bool(var_value.dup().is_string())) {
				...
				var_robots_strings << var_directive.dup()
			}
		}
	}
	...
	print("<meta name='robots' content='" +
		(rt.call_function('esc_attr', [rt.call_function('implode', [rt.new_string(', '), var_robots_strings.dup()])])).str() +
		"' />\n")
}
...
fn wp_robots_noindex(var_robots rt.PhpVal) rt.PhpVal {
	...
	{
		return wp_robots_no_robots(var_robots.dup())
	}
	return var_robots.dup()
}
...
pub fn init_wp_includes_robots_template_php() {
}
