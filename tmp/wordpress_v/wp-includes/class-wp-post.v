...
fn Class_WP_Post.get_instance(var_post_id rt.PhpVal) bool {
	...
	mut var__post := rt.call_function('wp_cache_get', [var_post_id_mutated.dup(), rt.new_string('posts')])
	if rt.is_true(rt.new_bool(!(rt.is_true(var__post)))) {
		var__post = rt.call_method(var_wpdb, 'get_row', [rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.new_string('SELECT * FROM '), rt.get_property(var_wpdb, 'posts')), rt.new_string(' WHERE ID = %d LIMIT 1')), var_post_id_mutated.dup()])])
		if rt.is_true(rt.new_bool(!(rt.is_true(var__post)))) {
			return false
		}
		var__post = rt.call_function('sanitize_post', [var__post.dup(), rt.new_string('raw')])
		rt.call_function('wp_cache_add', [rt.get_property(var__post, 'ID'), var__post.dup(), rt.new_string('posts')])
	} else if rt.is_true(rt.new_bool(!rt.is_true(rt.get_property(var__post, 'filter')) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		var__post = rt.call_function('sanitize_post', [var__post.dup(), rt.new_string('raw')])
	}
	return (create_wp_post(var__post.dup())).to_bool()
}

fn (mut this Class_WP_Post) construct(var_post rt.PhpVal)  {
	mut var_post_mutated := var_post
	{
		mut iter_1 := rt.call_function('get_object_vars', [var_post_mutated.dup()]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_value := item_1.val
			mut var_key := item_1.key
			this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":267,"name":"key"}', var_value.dup())
		}
	}
}
...
fn (mut this Class_WP_Post) magic_isset(var_key rt.PhpVal) bool {
	...
	return (rt.call_function('metadata_exists', [rt.new_string('post'), this.ID, var_key.dup()])).to_bool()
}

fn (mut this Class_WP_Post) magic_get(var_key rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('page_template'), var_key)) && this.magic_isset(var_key.dup()))) {
		return rt.call_function('get_post_meta', [this.ID, rt.new_string('_wp_page_template'), rt.new_bool(true)])
	}
	if rt.is_true(rt.identical(rt.new_string('post_category'), var_key)) {
		if rt.is_true(rt.call_function('is_object_in_taxonomy', [this.post_type, rt.new_string('category')])) {
			mut var_terms := rt.call_function('get_the_terms', [rt.new_object('WP_Post', []string{}, &this), rt.new_string('category')])
		}
	...
	if rt.is_true(rt.identical(rt.new_string('tags_input'), var_key)) {
		if rt.is_true(rt.call_function('is_object_in_taxonomy', [this.post_type, rt.new_string('post_tag')])) {
			var_terms = rt.call_function('get_the_terms', [rt.new_object('WP_Post', []string{}, &this), rt.new_string('post_tag')])
		}
		if !rt.is_true(var_terms) {
			return rt.new_array()
		}
		return rt.call_function('wp_list_pluck', [var_terms.dup(), rt.new_string('name')])
	}
	if rt.is_true(rt.identical(rt.new_string('ancestors'), var_key)) {
		mut var_value := rt.call_function('get_post_ancestors', [rt.new_object('WP_Post', []string{}, &this)])
	} else {
		var_value = rt.call_function('get_post_meta', [this.ID, var_key.dup(), rt.new_bool(true)])
	}
	if rt.is_true(this.filter) {
		var_value = rt.call_function('sanitize_post_field', [var_key.dup(), var_value.dup(), this.ID, this.filter])
	}
...
fn (mut this Class_WP_Post) filter(var_filter rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('sanitize_post', [rt.new_object('WP_Post', []string{}, &this), var_filter.dup()])
}

fn (mut this Class_WP_Post) to_array() rt.PhpVal {
	mut var_post := rt.call_function('get_object_vars', [rt.new_object('WP_Post', []string{}, &this)])
	{
		mut iter_1 := rt.create_array([rt.ArrayItem{ key: none, val: 'ancestors' }, rt.ArrayItem{ key: none, val: 'page_template' }, rt.ArrayItem{ key: none, val: 'post_category' }, rt.ArrayItem{ key: none, val: 'tags_input' }]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_key := item_1.val
			if this.magic_isset(var_key.dup()) {
				var_post.array_set(var_key, this.magic_get(var_key.dup()))
			}
		}
	}
	return var_post.dup()
...
fn (mut this Class_WP_Post) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'post_author' { this.post_author = val; return true }
		'post_date' { this.post_date = val; return true }
		'post_date_gmt' { this.post_date_gmt = val; return true }
		'post_content' { this.post_content = val; return true }
		'post_title' { this.post_title = val; return true }
		'post_excerpt' { this.post_excerpt = val; return true }
		'post_status' { this.post_status = val; return true }
		'comment_status' { this.comment_status = val; return true }
		'ping_status' { this.ping_status = val; return true }
		'post_password' { this.post_password = val; return true }
		'post_name' { this.post_name = val; return true }
		'to_ping' { this.to_ping = val; return true }
		'pinged' { this.pinged = val; return true }
		'post_modified' { this.post_modified = val; return true }
		'post_modified_gmt' { this.post_modified_gmt = val; return true }
		'post_content_filtered' { this.post_content_filtered = val; return true }
		'post_parent' { this.post_parent = val; return true }
		'guid' { this.guid = val; return true }
		'menu_order' { this.menu_order = val; return true }
		'post_type' { this.post_type = val; return true }
		'post_mime_type' { this.post_mime_type = val; return true }
		'comment_count' { this.comment_count = val; return true }
...
pub fn init_wp_includes_class_wp_post_php() {
}
