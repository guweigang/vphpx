...
fn (mut this Class_WP_Roles) _init()  {
	rt.call_function('_deprecated_function', [rt.new_string(@METHOD), rt.new_string('4.9.0'), rt.new_string('WP_Roles::for_site()')])
	this.for_site(rt.new_null())
}

fn (mut this Class_WP_Roles) reinit()  {
	rt.call_function('_deprecated_function', [rt.new_string(@METHOD), rt.new_string('4.7.0'), rt.new_string('WP_Roles::for_site()')])
	this.for_site(rt.new_null())
}
...
fn (mut this Class_WP_Roles) add_role(var_role rt.PhpVal, var_display_name rt.PhpVal, var_capabilities rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('wp_is_numeric_array', [var_capabilities_mutated.dup()])) {
		var_capabilities_mutated = rt.call_function('array_fill_keys', [var_capabilities_mutated.dup(), rt.new_bool(true)])
	}
	this.roles.array_set(var_role, rt.create_array([rt.ArrayItem{ key: 'name', val: var_display_name }, rt.ArrayItem{ key: 'capabilities', val: var_capabilities_mutated }]))
	if rt.is_true(this.use_db) {
		rt.call_function('update_option', [this.role_key, this.roles, rt.new_bool(true)])
	}
	this.role_objects.array_set(var_role, create_wp_role(var_role.dup(), var_capabilities_mutated.dup()))
	this.role_names.array_set(var_role, var_display_name.dup())
...
fn (mut this Class_WP_Roles) remove_role(var_role rt.PhpVal)  {
	...
	if rt.is_true(rt.identical(rt.call_function('get_option', [rt.new_string('default_role')]), var_role)) {
		rt.call_function('update_option', [rt.new_string('default_role'), rt.new_string('subscriber')])
	}
}
...
fn (mut this Class_WP_Roles) remove_cap(var_role rt.PhpVal, var_cap rt.PhpVal)  {
	...
	this.roles.array_get(var_role).array_get('capabilities').array_unset(var_cap)
	if rt.is_true(this.use_db) {
		rt.call_function('update_option', [this.role_key, this.roles])
	}
}
...
fn (mut this Class_WP_Roles) get_role(var_role rt.PhpVal) rt.PhpVal {
	return if !(this.role_objects.array_get(var_role)).is_null() { this.role_objects.array_get(var_role) } else { rt.new_null() }
}
...
fn (mut this Class_WP_Roles) init_roles()  {
	...
	this.role_names = rt.new_array()
	{
		...
		for {
			...
			this.role_objects.array_set(var_role, create_wp_role(var_role.dup(), this.roles.array_get(var_role).array_get('capabilities')))
			this.role_names.array_set(var_role, this.roles.array_get(var_role).array_get('name'))
		}
	}
	rt.call_function('do_action', [rt.new_string('wp_roles_init'), rt.new_object('WP_Roles', []string{}, &this)])
}
...
fn (mut this Class_WP_Roles) for_site(var_site_id rt.PhpVal)  {
	...
	if !(!rt.is_true(var_site_id)) {
		this.site_id = rt.call_function('absint', [var_site_id.dup()])
	} else {
		this.site_id = rt.call_function('get_current_blog_id', []rt.PhpVal{})
	}
	this.role_key = (rt.call_method(var_wpdb, 'get_blog_prefix', [this.site_id])).str() + 'user_roles'
	if rt.is_true(rt.new_bool(!(!rt.is_true(this.roles)) && rt.is_true(rt.new_bool(!(rt.is_true(this.use_db)))))) {
		return rt.new_null()
	}
...
fn (mut this Class_WP_Roles) get_roles_data() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{})) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		rt.call_function('remove_action', [rt.new_string('switch_blog'), rt.new_string('wp_switch_roles_and_user'), rt.new_int(1)])
		mut var_roles := rt.call_function('get_blog_option', [this.site_id, this.role_key, rt.new_array()])
		rt.call_function('add_action', [rt.new_string('switch_blog'), rt.new_string('wp_switch_roles_and_user'), rt.new_int(1), rt.new_int(2)])
		return var_roles.dup()
	}
...
fn create_wp_role() &Class_WP_Role {
...
fn (mut this Class_WP_Roles) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'role_objects' { this.role_objects = val; return true }
		'role_names' { this.role_names = val; return true }
		'role_key' { this.role_key = val; return true }
		'use_db' { this.use_db = val; return true }
...
pub fn init_wp_includes_class_wp_roles_php() {
}
