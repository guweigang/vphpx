...
fn the_title_attribute(args string) rt.PhpVal {
	...
	var_title = get_the_title(var_parsed_args.array_get('post'))
	if var_title.clone().to_string().len == 0 {
		return rt.new_null()
	}
	var_title = rt.new_string((var_parsed_args.array_get('before')).str() + var_title.str() +
		(var_parsed_args.array_get('after')).str())
...
fn get_the_content(var_more_link_text_arg rt.PhpVal, strip_teaser bool, var_post rt.PhpVal) string {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_null(), var_post))
	...
	if rt.is_true(rt.greater(var_elements.array_get('page'),
		rt.new_int(var_elements.array_get('pages').array_count())))
	{
		var_elements.array_set('page', var_elements.array_get('pages').array_count())
	}
	var_page_no = var_elements.array_get('page')
	var_content = var_elements.array_get('pages').array_get(rt.sub(var_page_no, rt.new_int(1)))
		...
		var_content.clone(), rt.create_array_from_list(var_matches)]))
	{
		...
		var_content = rt.call_function('explode', [var_matches.array_get(0),
			var_content.clone(), rt.new_int(2)])
		if !(!rt.is_true(var_matches.array_get(1))) && !(!rt.is_true(var_more_link_text)) {
			var_more_link_text = rt.call_function('strip_tags', [
				rt.call_function('wp_kses_no_null', [
					rt.new_string(var_matches.array_get(1).to_string().trim_space()),
				]),
			])
		}
		var_has_teaser = true
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.call_function('str_contains', [rt.get_property(var__post, 'post_content'), rt.new_string('<!--noteaser-->')]))
		&& rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_elements.array_get('multipage')))))
		|| rt.is_true(rt.identical(rt.new_int(1), var_elements.array_get('page')))))))
	{
		var_strip_teaser = true
	}
	var_teaser = var_content.array_get(0)
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_elements.array_get('more'))
		&& var_strip_teaser)) && var_has_teaser))
	{
		var_teaser = rt.new_string('')
	}
	var_output = rt.concat(var_output, var_teaser)
	if var_content.clone().array_count() > 1 {
		if rt.is_true(var_elements.array_get('more')) {
			var_output = rt.concat(var_output, rt.new_string('<span id="more-' +
				(rt.get_property(var__post, 'ID')).str() + '"></span>' +
				(var_content.array_get(1)).str()))
		} else {
...
fn get_post_class(css_class string, var_post_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('current_theme_supports', [rt.new_string('post-thumbnails')]))
	...
	if rt.is_true(rt.call_function('is_sticky', [rt.get_property(var_post, 'ID')])) {
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_home', []rt.PhpVal{}))
			...
			{
				{
					...
					for {
						...
						if rt.is_true(rt.new_bool(
							rt.is_true(rt.new_bool(var_term_class.clone().is_long()
							|| var_term_class.clone().is_double()))
...
fn get_body_class(css_class string) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_singular', []rt.PhpVal{})) {
		...
		if rt.is_true(rt.new_bool(is_page_template(''))) {
				...
				rt.new_string('/'), var_template_slug.clone()])
			{
				...
				for {
					item_1 := iter_1.next() or { break }
					mut var_part_shadow := item_1.val
		...
		if rt.is_true(rt.call_function('is_post_type_archive', []rt.PhpVal{})) {
			...
			if !(rt.get_property(var_cat, 'term_id')).is_null() {
				...
				if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_cat_class.clone().is_long()
					|| var_cat_class.clone().is_double()))
			...
			if !(rt.get_property(var_tag, 'term_id')).is_null() {
				...
				if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_tag_class.clone().is_long()
					|| var_tag_class.clone().is_double()))
			...
			if !(rt.get_property(var_term, 'term_id')).is_null() {
				...
				if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_term_class.clone().is_long()
					|| var_term_class.clone().is_double()))
					|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_string(var_term_class.clone().to_string().trim_space())))))))
				{
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.call_function('current_theme_supports', [rt.new_string('custom-background')]))
		&& rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_background_color', []rt.PhpVal{}), rt.call_function('get_theme_support', [rt.new_string('custom-background'), rt.new_string('default-color')])))))
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_page))))
...
fn post_password_required(var_post_arg rt.PhpVal) rt.PhpVal {
	...
	var_hash = rt.call_function('wp_unslash', [
		rt.get_superglobal('_COOKIE').array_get('wp-postpass_' +
			(rt.get_constant('COOKIEHASH')).str()),
	])
...
fn wp_link_pages(args string) rt.PhpVal {
	...
	if rt.is_true(var_multipage) {
		if rt.is_true(rt.identical(rt.new_string('number'),
			var_parsed_args.array_get('next_or_number')))
		{
			var_output = var_output + (var_parsed_args.array_get('before')).str()
			{
				...
				for {
					...
					var_link = rt.new_string(
						(var_parsed_args.array_get('link_before')).str() + (rt.call_function('str_replace', [rt.new_string('%'), rt.new_int(var_i).clone(), var_parsed_args.array_get('pagelink')])).str() +
						(var_parsed_args.array_get('link_after')).str())
					if rt.is_true(rt.new_bool(
						rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int(var_i), var_page))))
						|| rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_more))))
						&& rt.is_true(rt.identical(rt.new_int(1), var_page))))))
					{
						...
						var_link = rt.new_string(
							...
							(rt.call_function('esc_attr', [var_parsed_args.array_get('aria_current')])).str() +
							'">' + var_link.str() + '</span>')
					}
					...
					var_output = var_output +(if 1 == var_i { rt.new_string(' ') } else { var_parsed_args.array_get('separator') }).str()
			...
			if rt.is_true(rt.greater(var_prev, rt.new_int(0))) {
				var_link = rt.new_string(_wp_link_page(var_prev.clone()) +
					(var_parsed_args.array_get('link_before')).str() + (var_parsed_args.array_get('previouspagelink')).str() + (var_parsed_args.array_get('link_after')).str() + '</a>')
			...
			if rt.is_true(rt.less_equal(var_next, var_numpages)) {
				if rt.is_true(var_prev) {
					var_output = var_output + (var_parsed_args.array_get('separator')).str()
				}
				var_link = rt.new_string(_wp_link_page(var_next.clone()) +
					(var_parsed_args.array_get('link_before')).str() + (var_parsed_args.array_get('nextpagelink')).str() + (var_parsed_args.array_get('link_after')).str() + '</a>')
...
fn _wp_link_page(var_i rt.PhpVal) string {
	...
	if rt.is_true(rt.identical(rt.new_int(1), var_i)) {
			...
			&& rt.is_true(rt.identical(rt.new_int((rt.call_function('get_option', [rt.new_string('page_on_front')])).to_i64()), rt.get_property(var_post, 'ID')))))
		{
			var_url = rt.new_string(
				...
				var_i.str()), rt.new_string('single_paged')])).str())
		} else {
			var_url = rt.new_string(
	...
	if rt.is_true(rt.call_function('is_preview', []rt.PhpVal{})) {
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('draft'), rt.get_property(var_post, 'post_status')))))
			...
			&& rt.get_superglobal('_GET').array_isset(rt.new_string('preview_nonce'))))
		{
			var_query_args['preview_id'] = rt.call_function('wp_unslash', [
				rt.get_superglobal('_GET').array_get('preview_id'),
			])
			var_query_args['preview_nonce'] = rt.call_function('wp_unslash', [
				rt.get_superglobal('_GET').array_get('preview_nonce'),
			])
		}
...
fn post_custom(key string) bool {
	...
	if !(var_custom.array_isset(rt.new_string(key))) {
		return false
	} else if 1 == var_custom.array_get(key).array_count() {
		return (var_custom.array_get(key).array_get(0)).to_bool()
	} else {
		return (var_custom.array_get(key)).to_bool()
	}
	return false
}
...
fn the_meta() {
	...
	if rt.is_true(var_keys) {
		var_li_html = ''
		{
			mut iter_1 := rt.cast_array(var_keys).iterator()
			for {
				item_1 := iter_1.next() or { break }
				...
				var_values = rt.call_function('array_map', [rt.new_string('trim'),
					rt.call_function('get_post_custom_values', [
...
fn wp_dropdown_pages(args string) rt.PhpVal {
	...
	var_output = rt.new_string('')
	if !rt.is_true(var_parsed_args.array_get('id')) {
		var_parsed_args.array_set('id', var_parsed_args.array_get('name'))
	}
	if !(!rt.is_true(var_pages)) {
		var_class = rt.new_string('')
		if !(!rt.is_true(var_parsed_args.array_get('class'))) {
			var_class = rt.new_string(" class='" +
				(rt.call_function('esc_attr', [var_parsed_args.array_get('class')])).str() + "'")
		}
		var_output = rt.new_string("<select name='" +
			(rt.call_function('esc_attr', [var_parsed_args.array_get('name')])).str() + "'" +
			var_class.str() + " id='" +
			(rt.call_function('esc_attr', [var_parsed_args.array_get('id')])).str() + "'>\n")
		if rt.is_true(var_parsed_args.array_get('show_option_no_change')) {
			var_output = rt.concat(var_output, rt.new_string('\t<option value="-1">' +
				(var_parsed_args.array_get('show_option_no_change')).str() + '</option>\n'))
		}
		if rt.is_true(var_parsed_args.array_get('show_option_none')) {
			var_output = rt.concat(var_output, rt.new_string('\t<option value="' +
				(rt.call_function('esc_attr', [var_parsed_args.array_get('option_none_value')])).str() +
				'">' + (var_parsed_args.array_get('show_option_none')).str() + '</option>\n'))
		}
		var_output = rt.concat(var_output, walk_page_dropdown_tree(var_pages.clone(),
			var_parsed_args.array_get('depth'), var_parsed_args.clone()))
...
fn wp_list_pages(args string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_parsed_args.array_get('item_spacing'),
		rt.create_array([rt.ArrayItem{ key: none, val: 'preserve' },
			rt.ArrayItem{ key: none, val: 'discard' }]),
		rt.new_bool(true),
	])))))
	{
		var_parsed_args.array_set('item_spacing', var_defaults.array_get('item_spacing'))
	}
	var_output = ''
	...
	var_parsed_args.array_set('exclude', rt.call_function('preg_replace', [
		rt.new_string('/[^0-9,]/'),
		rt.new_string(''),
		var_parsed_args.array_get('exclude'),
	]))
	var_exclude_array = if rt.is_true(var_parsed_args.array_get('exclude')) { rt.call_function('explode', [
			rt.new_string(','),
			var_parsed_args.array_get('exclude'),
		]) } else { rt.new_array() }
	...
	if !(!rt.is_true(var_pages)) {
		if rt.is_true(var_parsed_args.array_get('title_li')) {
			var_output = var_output + '<li class="pagenav">' +
				(var_parsed_args.array_get('title_li')).str() + '<ul>'
		}
		if rt.is_true(rt.new_bool(
		...
		var_output = var_output +(walk_page_tree(var_pages.clone(), var_parsed_args.array_get('depth'), var_current_page.clone(), var_parsed_args.clone())).str()
		if rt.is_true(var_parsed_args.array_get('title_li')) {
...
fn wp_page_menu(var_args_arg rt.PhpVal) rt.PhpVal {
	...
	var_args = rt.call_function('wp_parse_args', [var_args.clone(),
		rt.create_array_from_native_map(var_defaults)])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_args.array_get('item_spacing'),
		rt.create_array([rt.ArrayItem{ key: none, val: 'preserve' },
			rt.ArrayItem{ key: none, val: 'discard' }]),
		rt.new_bool(true),
	])))))
	...
	if rt.is_true(rt.identical(rt.new_string('preserve'), var_args.array_get('item_spacing'))) {
		var_t = '\t'
		var_n = '\n'
	} else {
		var_t = ''
	...
	if !(!rt.is_true(var_args.array_get('show_home'))) {
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_bool(true), var_args.array_get('show_home')))
			|| rt.is_true(rt.identical(rt.new_string('1'), var_args.array_get('show_home')))))
			|| rt.is_true(rt.identical(rt.new_int(1), var_args.array_get('show_home')))))
		{
			var_text = rt.call_function('__', [rt.new_string('Home')])
		} else {
			var_text = var_args.array_get('show_home')
		}
		var_class = ''
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_front_page', []rt.PhpVal{}))
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_paged', []rt.PhpVal{})))))))
		{
		...
		var_menu = rt.concat(var_menu, rt.new_string('<li ' + var_class + '><a href="' +
			...
			'">' + (var_args.array_get('link_before')).str() + var_text.str() +
			(var_args.array_get('link_after')).str() + '</a></li>'))
		if rt.is_true(rt.identical(rt.new_string('page'), rt.call_function('get_option', [
			rt.new_string('show_on_front'),
		])))
		{
			if !(!rt.is_true(var_list_args.array_get('exclude'))) {
				var_list_args.array_get('exclude') = rt.concat(var_list_args.array_get('exclude'),
					rt.new_string(','))
			} else {
				var_list_args.array_set('exclude', '')
			}
			var_list_args.array_get('exclude') = rt.concat(var_list_args.array_get('exclude'), rt.call_function('get_option', [
	...
	var_container = rt.call_function('sanitize_text_field', [
		var_args.array_get('container')])
	if !rt.is_true(var_container) {
		var_container = rt.new_string('div')
	}
	if rt.is_true(var_menu) {
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(var_args.array_isset(rt.new_string('fallback_cb'))
			&& rt.is_true(rt.identical(rt.new_string('wp_page_menu'), var_args.array_get('fallback_cb')))))
		...
		var_menu = rt.new_string((var_args.array_get('before')).str() + var_menu.str() +
			(var_args.array_get('after')).str())
	}
	var_attrs = ''
	if !(!rt.is_true(var_args.array_get('menu_id'))) {
		var_attrs = var_attrs + ' id="' +
			(rt.call_function('esc_attr', [var_args.array_get('menu_id')])).str() + '"'
	}
	if !(!rt.is_true(var_args.array_get('menu_class'))) {
		var_attrs = var_attrs + ' class="' +
			(rt.call_function('esc_attr', [var_args.array_get('menu_class')])).str() + '"'
	}
...
fn walk_page_tree(var_pages rt.PhpVal, var_depth rt.PhpVal, var_current_page rt.PhpVal, var_args rt.PhpVal) rt.PhpVal {
	...
	mut var_page := rt.new_null()
	if !rt.is_true(var_args.array_get('walker')) {
		var_walker = create_walker_page()
	} else {
		var_walker = var_args.array_get('walker')
	}
	{
		mut iter_1 := rt.cast_array(var_pages).iterator()
		for {
			item_1 := iter_1.next() or { break }
...
fn walk_page_dropdown_tree(var_args_origin ...rt.PhpVal) rt.PhpVal {
	...
	mut var_walker := rt.new_null()
	if !rt.is_true(var_args.array_get(2).array_get('walker')) {
...
fn wp_get_attachment_link(post i64, size string, permalink bool, icon bool, text bool, attr string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(var__post)
	...
	} else if rt.is_true(rt.new_bool(var_size.len > 0 && var_size != '0'
		&& rt.is_true(rt.new_bool('none' != size))))
	{
...
fn prepend_attachment(var_content rt.PhpVal) string {
	...
	var_post = rt.call_function('get_post', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!rt.is_true(rt.get_property(var_post, 'post_type'))
		...
		var_post.clone()]))
	{
		...
		if !(!rt.is_true(var_meta.array_get('width')))
			...
			var_atts['width'] = rt.new_int((var_meta.array_get('width')).to_i64())
			var_atts['height'] = rt.new_int((var_meta.array_get('height')).to_i64())
		}
...
fn get_the_password_form(post i64) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(!rt.is_true(rt.get_property(rt.new_int(var_post), 'ID')))
...
fn is_page_template(template string) bool {
	...
	if rt.is_true(rt.new_bool(rt.new_string(template).is_array())) {
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.call_function('in_array', [rt.new_string('default'), rt.new_string(template), rt.new_bool(true)]))
...
fn get_page_template_slug(var_post_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_template))))
...
fn wp_post_revision_title(var_revision_arg rt.PhpVal, link bool) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_link
...
fn wp_list_post_revisions(post i64, type string) {
	...
	if rt.is_true(rt.new_bool(rt.new_string(var_type.str()).is_array())) {
		var_type = (if !(!rt.is_true(rt.new_string(var_type.str()).array_get('type'))) {
			rt.new_string(var_type.str()).array_get('type')
		} else {
	...
	var_rows = ''
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_revision_shadow := item_1.val
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('revision'), rt.new_string(var_type.str())))
				&& rt.is_true(var_is_autosave)))
				|| rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('autosave'), rt.new_string(var_type.str())))
...
pub fn init_wp_includes_post_template_php() {
}
