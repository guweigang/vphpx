...
fn (mut this Class_WP_Widget_Categories) construct()  {
	mut var_widget_ops := { 'classname': rt.new_string('widget_categories'), 'description': rt.call_function('__', [rt.new_string('A list or dropdown of categories.')]), 'customize_selective_refresh': rt.new_bool(true), 'show_instance_in_rest': rt.new_bool(true) }
	this.Class_WP_Widget.construct(rt.new_string('categories'), rt.call_function('__', [rt.new_string('Categories')]), var_widget_ops.dup())
}
...
fn (mut this Class_WP_Widget_Categories) widget(var_args rt.PhpVal, var_instance rt.PhpVal)  {
	...
	var_title = rt.call_function('apply_filters', [rt.new_string('widget_title'), var_title.dup(), var_instance_mutated.dup(), rt.get_property(rt.new_object('WP_Widget_Categories', ['WP_Widget'], &this), 'id_base')])
	mut var_count := rt.new_string(if !(!rt.is_true(var_instance_mutated.array_get('count'))) { rt.new_string('1') } else { rt.new_string('0') })
	mut var_hierarchical := rt.new_string(if !(!rt.is_true(var_instance_mutated.array_get('hierarchical'))) { rt.new_string('1') } else { rt.new_string('0') })
	mut var_dropdown := rt.new_string(if !(!rt.is_true(var_instance_mutated.array_get('dropdown'))) { rt.new_string('1') } else { rt.new_string('0') })
	...
	mut var_cat_args := { 'orderby': rt.new_string('name'), 'show_count': var_count, 'hierarchical': var_hierarchical }
	if rt.is_true(var_dropdown) {
		rt.call_function('printf', [rt.new_string('<form action="%s" method="get">'), rt.call_function('esc_url', [rt.call_function('home_url', []rt.PhpVal{})])])
		mut var_dropdown_id := rt.new_string(if rt.is_true(var_first_dropdown) { rt.new_string('cat') } else { rt.concat(rt.concat(rt.get_property(rt.new_object('WP_Widget_Categories', ['WP_Widget'], &this), 'id_base'), rt.new_string('-dropdown-')), rt.get_property(rt.new_object('WP_Widget_Categories', ['WP_Widget'], &this), 'number')) })
		mut var_first_dropdown := rt.new_bool(rt.new_bool(false))
		print('<label class="screen-reader-text" for="' + (rt.call_function('esc_attr', [var_dropdown_id.dup()])).str() + '">' + (var_title).str() + '</label>')
		var_cat_args['show_option_none'] = rt.call_function('__', [rt.new_string('Select Category')])
		var_cat_args['id'] = var_dropdown_id.dup()
		rt.call_function('wp_dropdown_categories', [rt.call_function('apply_filters', [rt.new_string('widget_categories_dropdown_args'), var_cat_args.dup(), var_instance_mutated.dup()])])
		print('</form>')
		rt.call_function('ob_start', []rt.PhpVal{})
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('wp_json_encode', [var_dropdown_id.dup(), rt.bitwise_or(rt.get_constant('JSON_HEX_TAG'), rt.get_constant('JSON_UNESCAPED_SLASHES'))]))
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('wp_print_inline_script_tag', [(rt.call_function('wp_remove_surrounding_empty_script_tags', [rt.call_function('ob_get_clean', []rt.PhpVal{})])).str() + '\n//# sourceURL=' + (rt.call_function('rawurlencode', [rt.new_string(@METHOD)])).str()])
	} else {
		...
		rt.call_function('wp_list_categories', [rt.call_function('apply_filters', [rt.new_string('widget_categories_args'), var_cat_args.dup(), var_instance_mutated.dup()])])
		// unsupported statement: Stmt_InlineHTML
		if rt.is_true(rt.identical(rt.new_string('html5'), var_format)) {
			print('</nav>')
		}
...
fn (mut this Class_WP_Widget_Categories) update(var_new_instance rt.PhpVal, var_old_instance rt.PhpVal) rt.PhpVal {
	...
	var_instance.array_set('count', if !(!rt.is_true(var_new_instance.array_get('count'))) { 1 } else { 0 })
	var_instance.array_set('hierarchical', if !(!rt.is_true(var_new_instance.array_get('hierarchical'))) { 1 } else { 0 })
	var_instance.array_set('dropdown', if !(!rt.is_true(var_new_instance.array_get('dropdown'))) { 1 } else { 0 })
...
fn (mut this Class_WP_Widget_Categories) form(var_instance rt.PhpVal)  {
	...
	mut var_hierarchical := if var_instance_mutated.array_isset(rt.new_string('hierarchical')) { // unsupported expression: Expr_Cast_Bool } else { rt.new_bool(false) }
	mut var_dropdown := if var_instance_mutated.array_isset(rt.new_string('dropdown')) { // unsupported expression: Expr_Cast_Bool } else { rt.new_bool(false) }
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('checked', [var_dropdown.dup()])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(this.get_field_id(rt.new_string('dropdown')))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('checked', [var_count.dup()])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(this.get_field_id(rt.new_string('count')))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('checked', [var_hierarchical.dup()])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(this.get_field_id(rt.new_string('hierarchical')))
	// unsupported statement: Stmt_InlineHTML
...
pub fn init_wp_includes_widgets_class_wp_widget_categories_php() {
}
