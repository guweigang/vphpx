...
fn (mut this Class_WP_Widget_Media_Image) render_media(var_instance rt.PhpVal) {
	...
	var_instance_mutated = rt.call_function('wp_parse_args', [
		var_instance_mutated.dup(), rt.create_array([
			rt.ArrayItem{ key: 'size', val: 'thumbnail' },
		])])
	mut var_attachment := rt.new_null()
	if rt.is_true(this.is_attachment_with_mime_type(var_instance_mutated.array_get('attachment_id'), rt.get_property(rt.new_object('WP_Widget_Media_Image', [
		'WP_Widget_Media',
	...
	if rt.is_true(var_attachment) {
		mut var_caption := rt.new_string(rt.new_string(''))
		if !(var_instance_mutated.array_isset(rt.new_string('caption'))) {
			var_caption = rt.get_property(var_attachment, 'post_excerpt')
		} else if rt.is_true(rt.new_string(var_instance_mutated.array_get('caption').to_string().trim_space())) {
			var_caption = var_instance_mutated.array_get('caption')
		}
		mut var_image_attributes := {
			'class': rt.call_function('sprintf', [rt.new_string('image wp-image-%d %s'),
				rt.get_property(var_attachment, 'ID'), var_instance_mutated.array_get('image_classes')])
			'style': rt.new_string('max-width: 100%; height: auto;')
		}
		if !(!rt.is_true(var_instance_mutated.array_get('image_title'))) {
			var_image_attributes['title'] = var_instance_mutated.array_get('image_title')
		}
		if rt.is_true(var_instance_mutated.array_get('alt')) {
			var_image_attributes['alt'] = var_instance_mutated.array_get('alt')
		}
		mut var_size := var_instance_mutated.array_get('size')
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('custom'), var_size))
			|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_size.dup(), rt.call_function('array_merge', [rt.call_function('get_intermediate_image_sizes', []rt.PhpVal{}), rt.create_array([rt.ArrayItem{
			key: none
			val: 'full'
		}])]), rt.new_bool(true)])))))))
		{
			var_size = rt.create_array([
				rt.ArrayItem{ key: none, val: var_instance_mutated.array_get('width') },
				rt.ArrayItem{ key: none, val: var_instance_mutated.array_get('height') },
			])
			mut var_width := var_instance_mutated.array_get('width')
		} else {
			mut var_caption_size := rt.call_function('_wp_get_image_size_from_meta', [
				var_instance_mutated.array_get('size'),
				rt.call_function('wp_get_attachment_metadata', [
					rt.get_property(var_attachment, 'ID'),
				]),
			])
			var_width = if !rt.is_true(var_caption_size.array_get(0)) {
				rt.new_int(0)
			} else {
				var_caption_size.array_get(0)
			}
		}
		...
		mut var_image := rt.call_function('wp_get_attachment_image', [
			...
			var_size.dup(),
			rt.new_bool(false),
			var_image_attributes.dup(),
		])
	} else {
		if !rt.is_true(var_instance_mutated.array_get('url')) {
			return rt.new_null()
		}
		var_instance_mutated.array_set('size', 'custom')
		var_caption = var_instance_mutated.array_get('caption')
		var_width = var_instance_mutated.array_get('width')
		mut var_classes := rt.new_string('image ' +
			(var_instance_mutated.array_get('image_classes')).str())
		if rt.is_true(rt.identical(rt.new_int(0), var_instance_mutated.array_get('width'))) {
			var_instance_mutated.array_set('width', '')
		}
		if rt.is_true(rt.identical(rt.new_int(0), var_instance_mutated.array_get('height'))) {
			var_instance_mutated.array_set('height', '')
		}
		mut var_attr := rt.create_array([rt.ArrayItem{ key: 'class', val: var_classes },
			rt.ArrayItem{ key: 'src', val: var_instance_mutated.array_get('url') },
			rt.ArrayItem{ key: 'alt', val: var_instance_mutated.array_get('alt') },
			rt.ArrayItem{ key: 'width', val: var_instance_mutated.array_get('width') },
			rt.ArrayItem{ key: 'height', val: var_instance_mutated.array_get('height') }])
		...
		var_attr = rt.call_function('array_merge', [var_attr.dup(),
			var_loading_optimization_attr.dup()])
		var_attr = rt.call_function('array_map', [rt.new_string('esc_attr'),
			var_attr.dup()])
		var_image = rt.new_string(rt.new_string('<img'))
		{
	...
	if rt.is_true(rt.identical(rt.new_string('file'), var_instance_mutated.array_get('link_type'))) {
		var_url = if rt.is_true(var_attachment) { rt.call_function('wp_get_attachment_url', [
				rt.get_property(var_attachment, 'ID'),
			]) } else { var_instance_mutated.array_get('url') }
	} else if rt.is_true(rt.new_bool(rt.is_true(var_attachment)
		&& rt.is_true(rt.identical(rt.new_string('post'), var_instance_mutated.array_get('link_type')))))
	{
		var_url = rt.call_function('get_attachment_link', [
			rt.get_property(var_attachment, 'ID'),
		])
	} else if rt.is_true(rt.new_bool(
		rt.is_true(rt.identical(rt.new_string('custom'), var_instance_mutated.array_get('link_type')))
		&& !(!rt.is_true(var_instance_mutated.array_get('link_url')))))
	{
		var_url = var_instance_mutated.array_get('link_url')
	}
	if rt.is_true(var_url) {
		mut var_link := rt.call_function('sprintf', [rt.new_string('<a href="%s"'),
			rt.call_function('esc_url', [var_url.dup()])])
		if !(!rt.is_true(var_instance_mutated.array_get('link_classes'))) {
			// unsupported expression: Expr_AssignOp_Concat
		}
		if !(!rt.is_true(var_instance_mutated.array_get('link_rel'))) {
			// unsupported expression: Expr_AssignOp_Concat
		}
		if !(!rt.is_true(var_instance_mutated.array_get('link_target_blank'))) {
			// unsupported expression: Expr_AssignOp_Concat
		}
		// unsupported expression: Expr_AssignOp_Concat
		// unsupported expression: Expr_AssignOp_Concat
		// unsupported expression: Expr_AssignOp_Concat
		var_image = var_link.dup()
	}
...
fn (mut this Class_WP_Widget_Media_Image) enqueue_admin_scripts() {
	...
	mut var_handle := rt.new_string(rt.new_string('media-image-widget'))
...
fn (mut this Class_WP_Widget_Media_Image) render_control_template_scripts() {
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('wp_admin_notice', [rt.get_property(rt.new_object('WP_Widget_Media_Image', [
		'WP_Widget_Media',
...
pub fn init_wp_includes_widgets_class_wp_widget_media_image_php() {
}
