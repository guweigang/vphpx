...
fn (mut this Class_WP_Widget_Archives) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	mut var_count := rt.new_string(if !(!rt.is_true(var_instance_mutated.array_get('count'))) {
		rt.new_string('1')
	} else {
		rt.new_string('0')
	})
	mut var_dropdown := rt.new_string(if !(!rt.is_true(var_instance_mutated.array_get('dropdown'))) {
		rt.new_string('1')
	} else {
		rt.new_string('0')
	})
	...
	if rt.is_true(var_dropdown) {
		...
		rt.echo_val(rt.call_function('esc_attr', [var_dropdown_id.dup()]))
		// unsupported statement: Stmt_InlineHTML
		...
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('wp_get_archives', [var_dropdown_args.dup()])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('ob_start', []rt.PhpVal{})
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('wp_json_encode', [var_dropdown_id.dup(),
...
fn (mut this Class_WP_Widget_Archives) update(var_new_instance rt.PhpVal, var_old_instance rt.PhpVal) rt.PhpVal {
	...
	var_instance.array_set('count', if rt.is_true(var_new_instance_mutated.array_get('count')) {
		1
	} else {
		0
	})
	var_instance.array_set('dropdown', if rt.is_true(var_new_instance_mutated.array_get('dropdown')) {
		1
	} else {
		0
	})
...
fn (mut this Class_WP_Widget_Archives) form(var_instance rt.PhpVal) {
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('checked', [var_instance_mutated.array_get('dropdown')])
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('checked', [var_instance_mutated.array_get('count')])
	// unsupported statement: Stmt_InlineHTML
...
pub fn init_wp_includes_widgets_class_wp_widget_archives_php() {
}
