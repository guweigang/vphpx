...
fn (mut this Class_WP_Nav_Menu_Widget) construct()  {
	mut var_widget_ops := { 'description': rt.call_function('__', [rt.new_string('Add a navigation menu to your sidebar.')]), 'customize_selective_refresh': rt.new_bool(true), 'show_instance_in_rest': rt.new_bool(true) }
	this.Class_WP_Widget.construct(rt.new_string('nav_menu'), rt.call_function('__', [rt.new_string('Navigation Menu')]), var_widget_ops.dup())
}
...
fn (mut this Class_WP_Nav_Menu_Widget) widget(var_args rt.PhpVal, var_instance rt.PhpVal)  {
	...
	mut var_nav_menu := if !(!rt.is_true(var_instance_mutated.array_get('nav_menu'))) { rt.call_function('wp_get_nav_menu_object', [var_instance_mutated.array_get('nav_menu')]) } else { rt.new_bool(false) }
	...
	var_title = rt.call_function('apply_filters', [rt.new_string('widget_title'), var_title.dup(), var_instance_mutated.dup(), rt.get_property(rt.new_object('WP_Nav_Menu_Widget', ['WP_Widget'], &this), 'id_base')])
	...
	if rt.is_true(rt.identical(rt.new_string('html5'), var_format)) {
		...
		mut var_nav_menu_args := { 'fallback_cb': rt.new_string(''), 'menu': var_nav_menu, 'container': rt.new_string('nav'), 'container_aria_label': var_aria_label, 'items_wrap': rt.new_string('<ul id="%1$s" class="%2$s">%3$s</ul>') }
	} else {
		var_nav_menu_args = { 'fallback_cb': rt.new_string(''), 'menu': var_nav_menu }
	}
	rt.call_function('wp_nav_menu', [rt.call_function('apply_filters', [rt.new_string('widget_nav_menu_args'), var_nav_menu_args.dup(), var_nav_menu.dup(), var_args.dup(), var_instance_mutated.dup()])])
...
fn (mut this Class_WP_Nav_Menu_Widget) update(var_new_instance rt.PhpVal, var_old_instance rt.PhpVal) rt.PhpVal {
	...
	if !(!rt.is_true(var_new_instance.array_get('title'))) {
		var_instance['title'] = rt.call_function('sanitize_text_field', [var_new_instance.array_get('title')])
	}
	if !(!rt.is_true(var_new_instance.array_get('nav_menu'))) {
		var_instance['nav_menu'] = // unsupported expression: Expr_Cast_Int
	}
...
fn (mut this Class_WP_Nav_Menu_Widget) form(var_instance rt.PhpVal)  {
	...
	mut var_title := if !(var_instance_mutated.array_get('title')).is_null() { var_instance_mutated.array_get('title') } else { rt.new_string('') }
	mut var_nav_menu := if !(var_instance_mutated.array_get('nav_menu')).is_null() { var_instance_mutated.array_get('nav_menu') } else { rt.new_string('') }
	mut var_menus := rt.call_function('wp_get_nav_menus', []rt.PhpVal{})
	mut var_empty_menus_style := rt.new_string(rt.new_string(''))
	mut var_not_empty_menus_style := rt.new_string(rt.new_string(''))
	if !rt.is_true(var_menus) {
		var_empty_menus_style = rt.new_string(rt.new_string(' style="display:none" '))
	} else {
		var_not_empty_menus_style = rt.new_string(rt.new_string(' style="display:none" '))
	}
	mut var_nav_menu_style := rt.new_string(rt.new_string(''))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_nav_menu)))) {
		var_nav_menu_style = rt.new_string(rt.new_string('display: none;'))
	}
	...
	if rt.is_true(rt.new_bool(rt.instance_of(var_wp_customize, 'WP_Customize_Manager'))) {
		mut var_url := rt.new_string(rt.new_string('javascript: wp.customize.panel( "nav_menus" ).focus();'))
	} else {
		var_url = rt.call_function('admin_url', [rt.new_string('nav-menus.php')])
	}
	rt.call_function('printf', [rt.call_function('__', [rt.new_string('No menus have been created yet. <a href="%s">Create some</a>.')]), rt.call_function('esc_attr', [var_url.dup()])])
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	{
		...
		for {
			...
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('selected', [var_nav_menu.dup(), rt.get_property(var_menu, 'term_id')])
			// unsupported statement: Stmt_InlineHTML
...
pub fn init_wp_includes_widgets_class_wp_nav_menu_widget_php() {
}
