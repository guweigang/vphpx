...
pub fn init_wp_includes_theme_compat_embed_content_php() {
	// unsupported statement: Stmt_Nop
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('post_class', [rt.new_string('wp-embed')])
	// unsupported statement: Stmt_InlineHTML
	mut var_thumbnail_id := rt.new_int(rt.new_int(0))
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.identical(rt.new_string('attachment'), rt.call_function('get_post_type', []rt.PhpVal{})))
	...
	var_thumbnail_id = rt.call_function('apply_filters', [
		...
		var_thumbnail_id.dup(),
	])
	if rt.is_true(var_thumbnail_id) {
		mut var_aspect_ratio := rt.new_int(rt.new_int(1))
		mut var_measurements := [rt.new_int(1), rt.new_int(1)]
		mut var_image_size := rt.new_string(rt.new_string('full'))
		mut var_meta := rt.call_function('wp_get_attachment_metadata', [
			var_thumbnail_id.dup()])
		if !(!rt.is_true(var_meta.array_get('sizes'))) {
			{
				mut iter_1 := var_meta.array_get('sizes').iterator()
				for {
					item_1 := iter_1.next() or { break }
					mut var_data := item_1.val
					mut var_size := item_1.key
					if rt.is_true(rt.new_bool(
						rt.is_true(rt.greater(var_data.array_get('height'), rt.new_int(0)))
						&& rt.is_true(rt.greater(rt.div(var_data.array_get('width'), var_data.array_get('height')), var_aspect_ratio))))
					{
						var_aspect_ratio = rt.div(var_data.array_get('width'),
							var_data.array_get('height'))
						var_measurements = [var_data.array_get('width'),
							var_data.array_get('height')]
		...
		mut var_shape := rt.new_string(if rt.is_true(rt.greater_equal(rt.div(var_measurements.array_get(0),
			var_measurements.array_get(1)), rt.new_float(1.75)))
		{
			rt.new_string('rectangular')
		} else {
	...
	if rt.is_true(rt.new_bool(rt.is_true(var_thumbnail_id)
		&& rt.is_true(rt.identical(rt.new_string('rectangular'), var_shape))))
	{
		...
		rt.echo_val(rt.call_function('wp_get_attachment_image', [
			var_thumbnail_id.dup(), var_image_size.dup()]))
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('the_permalink', []rt.PhpVal{})
	...
	// unsupported statement: Stmt_InlineHTML
	if rt.is_true(rt.new_bool(rt.is_true(var_thumbnail_id)
		&& rt.is_true(rt.identical(rt.new_string('square'), var_shape))))
	{
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('the_permalink', []rt.PhpVal{})
		// unsupported statement: Stmt_InlineHTML
