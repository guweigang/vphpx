...
pub fn init_wp_includes_theme_compat_sidebar_php() {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('dynamic_sidebar')])))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('dynamic_sidebar', []rt.PhpVal{})))))))
	{
		...
		// unsupported statement: Stmt_InlineHTML
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_404', []rt.PhpVal{}))
			...
			|| rt.is_true(rt.call_function('is_paged', []rt.PhpVal{}))))
		{
			...
			if rt.is_true(rt.call_function('is_404', []rt.PhpVal{})) {
				...
				&& !(!rt.is_true(rt.get_superglobal('_GET').array_get('paged'))) {
				// unsupported statement: Stmt_Nop
				// unsupported statement: Stmt_InlineHTML
				rt.call_function('printf', [
					rt.call_function('__', [
		...
		rt.call_function('wp_list_pages', [
			'title_li=<h2>' + (rt.call_function('__', [rt.new_string('Pages')])).str() + '</h2>',
		])
		// unsupported statement: Stmt_InlineHTML
