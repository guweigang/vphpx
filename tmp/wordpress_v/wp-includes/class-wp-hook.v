...
fn (mut this Class_WP_Hook) add_filter(var_hook_name rt.PhpVal, var_callback rt.PhpVal, var_priority rt.PhpVal, var_accepted_args rt.PhpVal)  {
	...
	if rt.is_true(rt.identical(rt.new_null(), var_priority_mutated)) {
		var_priority_mutated = rt.new_int(rt.new_int(0))
	}
	mut var_idx := rt.call_function('_wp_filter_build_unique_id', [var_hook_name.dup(), var_callback.dup(), var_priority_mutated.dup()])
	mut var_priority_existed := rt.new_bool(this.callbacks.array_isset(var_priority_mutated))
	this.callbacks.array_get_mut(var_priority_mutated).array_set(var_idx, rt.create_array([rt.ArrayItem{ key: 'function', val: var_callback }, rt.ArrayItem{ key: 'accepted_args', val: // unsupported expression: Expr_Cast_Int }]))
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_priority_existed)))) && this.callbacks.array_count() > 1)) {
	...
	if rt.is_true(rt.greater(this.nesting_level, rt.new_int(0))) {
		this.resort_active_iterations((var_priority_mutated).to_bool(), (var_priority_existed).to_bool())
	}
}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_new_priorities)))) {
		{
			...
			for {
				...
				this.iterations.array_set(var_index, var_new_priorities.dup())
			}
		}
		return rt.new_null()
	}
	mut var_min := rt.call_function('min', [var_new_priorities.dup()])
	{
		mut iter_1 := this.iterations.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_iteration := item_1.val
			mut var_index := item_1.key
			mut var_current := rt.call_function('current', [var_iteration.dup()])
			if rt.is_true(rt.identical(rt.new_bool(false), var_current)) {
				continue
			}
			var_iteration = var_new_priorities.dup()
			if rt.is_true(rt.less(var_current, var_min)) {
				rt.call_function('array_unshift', [var_iteration.dup(), var_current.dup()])
				continue
			}
			for rt.is_true(rt.less(rt.call_function('current', [var_iteration.dup()]), var_current)) {
				if rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('next', [var_iteration.dup()]))) {
					break
				}
			}
			if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_bool(new_priority), this.current_priority.array_get(var_index))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(priority_existed_mutated))))))) {
				if rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('current', [var_iteration.dup()]))) {
					mut var_prev := rt.call_function('end', [var_iteration.dup()])
				} else {
					var_prev = rt.call_function('prev', [var_iteration.dup()])
				}
				if rt.is_true(rt.identical(rt.new_bool(false), var_prev)) {
					rt.call_function('reset', [var_iteration.dup()])
				} else if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
					rt.call_function('next', [var_iteration.dup()])
				}
			}
		}
	}
...
fn (mut this Class_WP_Hook) remove_filter(var_hook_name rt.PhpVal, var_callback rt.PhpVal, var_priority rt.PhpVal) rt.PhpVal {
	...
	mut var_function_key := rt.call_function('_wp_filter_build_unique_id', [var_hook_name.dup(), var_callback.dup(), var_priority_mutated.dup()])
	mut var_exists := rt.new_bool(!(var_function_key).is_null() && this.callbacks.array_get(var_priority_mutated).array_isset(var_function_key))
...
fn (mut this Class_WP_Hook) has_filter(hook_name string, callback bool, priority bool) bool {
	...
	mut var_function_key := rt.call_function('_wp_filter_build_unique_id', [rt.new_string(hook_name), rt.new_bool(callback), rt.new_bool(false)])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_function_key)))) {
		return false
	}
	if rt.is_true(rt.new_bool(rt.new_bool(priority_mutated).dup().is_long())) {
		return (rt.new_bool(this.callbacks.array_get(priority_mutated).array_isset(var_function_key))).to_bool()
	}
	{
		mut iter_1 := this.callbacks.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_callbacks := item_1.val
			mut var_callback_priority := item_1.key
...
fn (mut this Class_WP_Hook) apply_filters(var_value rt.PhpVal, var_args rt.PhpVal) rt.PhpVal {
	...
	mut var_num_args := rt.new_int(rt.new_int(var_args_mutated.dup().array_count()))
	for {
		this.current_priority.array_set(var_nesting_level, rt.call_function('current', [this.iterations.array_get(var_nesting_level)]))
		mut var_priority := this.current_priority.array_get(var_nesting_level)
		{
			mut iter_1 := this.callbacks.array_get(var_priority).iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_the_ := item_1.val
				if rt.is_true(rt.new_bool(!(rt.is_true(this.doing_action)))) {
					var_args_mutated.array_set(0, var_value_mutated.dup())
				}
				if rt.is_true(rt.identical(rt.new_int(0), var_the_.array_get('accepted_args'))) {
					var_value_mutated = rt.call_function('call_user_func', [var_the_.array_get('function')])
				} else if rt.is_true(rt.greater_equal(var_the_.array_get('accepted_args'), var_num_args)) {
					var_value_mutated = rt.call_function('call_user_func_array', [var_the_.array_get('function'), var_args_mutated.dup()])
				} else {
					var_value_mutated = rt.call_function('call_user_func_array', [var_the_.array_get('function'), rt.call_function('array_slice', [var_args_mutated.dup(), rt.new_int(0), var_the_.array_get('accepted_args')])])
				}
			}
		}
...
fn (mut this Class_WP_Hook) do_action(var_args rt.PhpVal)  {
	...
	this.apply_filters(rt.new_string(''), var_args_mutated.dup())
...
fn (mut this Class_WP_Hook) do_all_hook(var_args rt.PhpVal)  {
	...
	this.iterations.array_set(var_nesting_level, this.priorities)
	for {
		mut var_priority := rt.call_function('current', [this.iterations.array_get(var_nesting_level)])
		{
			mut iter_1 := this.callbacks.array_get(var_priority).iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_the_ := item_1.val
				rt.call_function('call_user_func_array', [var_the_.array_get('function'), var_args_mutated.dup()])
			}
		}
		if !(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical)) {
			break
...
fn (mut this Class_WP_Hook) current_priority() bool {
	if rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('current', [this.iterations]))) {
		return false
	}
...
fn Class_WP_Hook.build_preinitialized_hooks(var_filters rt.PhpVal) rt.PhpVal {
	mut var_normalized := rt.new_array()
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_callback_groups := item_1.val
			mut var_hook_name := item_1.key
			if rt.is_true(rt.new_bool(rt.instance_of(var_callback_groups, 'WP_Hook'))) {
				var_normalized.array_set(var_hook_name, var_callback_groups.dup())
				continue
			}
			mut var_hook := create_wp_hook()
			{
				mut iter_2 := var_callback_groups.iterator()
				for {
					item_2 := iter_2.next() or { break }
					mut var_callbacks := item_2.val
					mut var_priority := item_2.key
					{
						mut iter_3 := var_callbacks.iterator()
						for {
							item_3 := iter_3.next() or { break }
							mut var_cb := item_3.val
							var_hook.add_filter(var_hook_name.dup(), var_cb.array_get('function'), var_priority.dup(), var_cb.array_get('accepted_args'))
						}
					}
				}
			}
			var_normalized.array_set(var_hook_name, var_hook.dup())
		}
	}
...
fn (mut this Class_WP_Hook) offsetget(var_offset rt.PhpVal) rt.PhpVal {
	return if !(this.callbacks.array_get(var_offset)).is_null() { this.callbacks.array_get(var_offset) } else { rt.new_null() }
}
...
fn (mut this Class_WP_Hook) offsetset(var_offset rt.PhpVal, var_value rt.PhpVal)  {
	...
	if rt.is_true(rt.new_bool(var_offset.dup().is_null())) {
		this.callbacks.array_push(var_value_mutated.dup())
	} else {
		this.callbacks.array_set(var_offset, var_value_mutated.dup())
	}
	this.priorities = rt.func_array_keys(this.callbacks)
}
...
fn (mut this Class_WP_Hook) key() rt.PhpVal {
	return rt.call_function('key', [])
}

fn (mut this Class_WP_Hook) valid() rt.PhpVal {
	return 
}
...
fn (mut this Class_WP_Hook) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'iterations' { this.iterations = val; return true }
		'current_priority' { this.current_priority = val; return true }
		'nesting_level' { this.nesting_level = val; return true }
		'doing_action' { this.doing_action = (val).to_bool(); return true }
...
pub fn init_wp_includes_class_wp_hook_php() {
}
