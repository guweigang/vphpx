...
fn wp_register_options_connectors_route(var_path rt.PhpVal, var_content_module rt.PhpVal, var_route_module rt.PhpVal) {
	...
	var_wp_options_connectors_routes << var_route.dup()
}
...
fn wp_register_options_connectors_menu_item(var_id rt.PhpVal, var_label rt.PhpVal, var_to rt.PhpVal, parent_id string, parent_type string) {
	...
	var_wp_options_connectors_menu_items << var_menu_item.dup()
}
...
fn wp_options_connectors_render_page() {
	...
	mut var_menu_items := wp_get_options_connectors_menu_items()
	mut var_routes := wp_get_options_connectors_routes()
	...
	// unsupported statement: Stmt_InlineHTML
	// unsupported statement: Stmt_Global
	mut var_hook_suffix := 'options-connectors'
...
fn wp_options_connectors_intercept_render() {
	if rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_isset(rt.new_string('page'))
		&& rt.is_true(rt.identical(rt.new_string('options-connectors'), rt.get_superglobal('_GET').array_get('page')))))
	{
...
pub fn init_wp_includes_build_pages_options_connectors_page_php() {
	// unsupported statement: Stmt_Global
	mut var_wp_options_connectors_routes := []rt.PhpVal{}
	mut var_wp_options_connectors_menu_items := []rt.PhpVal{}
