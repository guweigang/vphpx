...
fn (mut this Class_WP_Error) get_error_code() string {
	...
	return (var_codes.array_get(0)).str()
}
...
fn (mut this Class_WP_Error) get_error_messages(code string) rt.PhpVal {
	...
	if this.errors.array_isset(rt.new_string(code_mutated)) {
		return this.errors.array_get(code_mutated)
	} else {
		return rt.new_array()
	}
	return rt.new_null()
...
fn (mut this Class_WP_Error) get_error_message(code string) string {
	...
	return (var_messages.array_get(0)).str()
}
...
fn (mut this Class_WP_Error) get_error_data(code string) rt.PhpVal {
	...
	if this.error_data.array_isset(rt.new_string(code_mutated)) {
		return this.error_data.array_get(code_mutated)
	}
	return rt.new_null()
}
...
fn (mut this Class_WP_Error) add(var_code rt.PhpVal, var_message rt.PhpVal, data string) {
	...
	mut data_mutated := data
	this.errors.array_get_mut(var_code_mutated).array_push(var_message.dup())
	if !(data_mutated == '') {
		this.add_data(rt.new_string(data_mutated), var_code_mutated.str())
	}
	rt.call_function('do_action', [rt.new_string('wp_error_added'),
		var_code_mutated.dup(), var_message.dup(), rt.new_string(data_mutated).dup(),
...
fn (mut this Class_WP_Error) add_data(var_data rt.PhpVal, code string) {
	...
	if this.error_data.array_isset(rt.new_string(code_mutated)) {
		this.additional_data.array_get_mut(code_mutated).array_push(this.error_data.array_get(code_mutated))
	}
	this.error_data.array_set(code_mutated, var_data_mutated.dup())
}
...
fn (mut this Class_WP_Error) get_all_error_data(code string) rt.PhpVal {
	...
	if this.additional_data.array_isset(rt.new_string(code_mutated)) {
		var_data = this.additional_data.array_get(code_mutated)
	}
	if this.error_data.array_isset(rt.new_string(code_mutated)) {
		var_data.array_push(this.error_data.array_get(code_mutated))
	}
	return var_data.dup()
}
...
fn Class_WP_Error.copy_errors(mut var_from Class_WP_Error, mut var_to Class_WP_Error) {
	{
		mut iter_1 := var_from.get_error_codes().iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_code := item_1.val
			{
				mut iter_2 := var_from.get_error_messages(var_code.str()).iterator()
				for {
					item_2 := iter_2.next() or { break }
					mut var_error_message := item_2.val
					var_to.add(var_code.dup(), var_error_message.dup(), '')
				}
			}
			{
				mut iter_2 := var_from.get_all_error_data(var_code.str()).iterator()
				for {
					item_2 := iter_2.next() or { break }
					mut var_data := item_2.val
					var_to.add_data(var_data.dup(), var_code.str())
				}
			}
		}
	}
...
pub fn init_wp_includes_class_wp_error_php() {
}
