...
fn (mut this Class_WP_Recovery_Mode_Link_Service) construct(mut var_cookie_service Class_WP_Recovery_Mode_Cookie_Service, mut var_key_service Class_WP_Recovery_Mode_Key_Service)  {
	this.cookie_service = var_cookie_service.dup()
	this.key_service = var_key_service.dup()
}
...
fn (mut this Class_WP_Recovery_Mode_Link_Service) generate_url() rt.PhpVal {
	...
	mut var_key := rt.call_method(this.key_service, 'generate_and_store_recovery_mode_key', [var_token.dup()])
	return this.get_recovery_mode_begin_url(var_token.dup(), var_key.dup())
}

fn (mut this Class_WP_Recovery_Mode_Link_Service) handle_begin_link(var_ttl rt.PhpVal)  {
	mut var_GLOBALS := rt.new_null()
	if rt.is_true(rt.new_bool(!(var_GLOBALS.array_isset(rt.new_string('pagenow'))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(!(rt.get_superglobal('_GET').array_isset(rt.new_string('action')) && rt.get_superglobal('_GET').array_isset(rt.new_string('rm_token')) && rt.get_superglobal('_GET').array_isset(rt.new_string('rm_key'))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return rt.new_null()
	}
	...
	mut var_validated := rt.call_method(this.key_service, 'validate_recovery_mode_key', [rt.get_superglobal('_GET').array_get('rm_token'), rt.get_superglobal('_GET').array_get('rm_key'), var_ttl.dup()])
	if rt.is_true(rt.call_function('is_wp_error', [var_validated.dup()])) {
		rt.call_function('wp_die', [var_validated.dup(), rt.new_string('')])
	}
	rt.call_method(this.cookie_service, 'set_cookie', []rt.PhpVal{})
	mut var_url := rt.call_function('add_query_arg', [rt.new_string('action'), Class_WP_Recovery_Mode_Link_Service.login_action_entered(), rt.call_function('wp_login_url', []rt.PhpVal{})])
...
fn (mut this Class_WP_Recovery_Mode_Link_Service) get_recovery_mode_begin_url(var_token rt.PhpVal, var_key rt.PhpVal) rt.PhpVal {
	...
	mut var_url := rt.call_function('add_query_arg', [rt.create_array([rt.ArrayItem{ key: 'action', val: Class_WP_Recovery_Mode_Link_Service.login_action_enter() }, rt.ArrayItem{ key: 'rm_token', val: var_token_mutated }, rt.ArrayItem{ key: 'rm_key', val: var_key_mutated }]), rt.call_function('wp_login_url', []rt.PhpVal{})])
	return rt.call_function('apply_filters', [rt.new_string('recovery_mode_begin_url'), var_url.dup(), var_token_mutated.dup(), var_key_mutated.dup()])
}
...
fn (mut this Class_WP_Recovery_Mode_Link_Service) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WP_Recovery_Mode_Cookie_Service](if args.len > 0 { args[0] } else { rt.new_null() })
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_WP_Recovery_Mode_Key_Service](if args.len > 1 { args[1] } else { rt.new_null() })
...
pub fn init_wp_includes_class_wp_recovery_mode_link_service_php() {
}
