...
fn get_comment_author_email_link(link_text string, before string, after string, var_comment_arg rt.PhpVal) string {
	...
	if rt.is_true(rt.new_bool(!(!rt.is_true(var_comment_author_email))
...
fn get_comment_author_link(comment_id i64) rt.PhpVal {
	...
	var_comment_author = get_comment_author(var_comment.clone())
	if rt.is_true(rt.new_bool(!rt.is_true(var_comment_author_url)
...
fn get_comment_class(css_class string, var_comment_id rt.PhpVal, var_post rt.PhpVal) rt.PhpVal {
	...
	if !(var_css_class == '') {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.new_string(var_css_class.str()).is_array()))))) {
...
fn get_comment_link(var_comment_arg rt.PhpVal, var_args_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_args.array_get('cpage').is_null()))))) {
		var_cpage = var_args.array_get('cpage')
	} else {
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.identical(rt.new_string(''), var_args.array_get('per_page')))
		...
		if !rt.is_true(var_args.array_get('per_page')) {
			var_args.array_set('per_page', 0)
			var_args.array_set('page', 0)
		}
		var_cpage = var_args.array_get('page')
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.identical(rt.new_string('oldest'), rt.call_function('get_option', [rt.new_string('default_comments_page')])))
			&& rt.is_true(rt.identical(rt.new_int(1), var_cpage))))
		{
			var_cpage = rt.new_string('')
		}
	}
	if rt.is_true(rt.new_bool(rt.is_true(var_cpage)
		&& rt.is_true(rt.call_function('get_option', [rt.new_string('page_comments')]))))
	{
	...
	if rt.is_true(rt.greater(var_comments_number, rt.new_int(1))) {
		if rt.is_true(rt.identical(rt.new_bool(false), rt.new_bool(var_more))) {
			...
			])))
			{
				...
				if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(var_text)
...
fn get_comment_text(comment_id i64, var_args rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_comment_feed', []rt.PhpVal{}))
...
fn trackback_rdf(deprecated string) {
	...
	if rt.is_true(rt.new_bool(
		rt.get_superglobal('_SERVER').array_isset(rt.new_string('HTTP_USER_AGENT'))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('stripos', [rt.get_superglobal('_SERVER').array_get('HTTP_USER_AGENT'), rt.new_string('W3C_Validator')])))))))
	{
		return
	}
...
fn comments_template(file string, separate_comments bool) {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_single', []rt.PhpVal{}))
		|| rt.is_true(rt.call_function('is_page', []rt.PhpVal{}))))
		|| rt.is_true(var_withcomments)))))) || !rt.is_true(var_post)))
	{
		return
	}
	if var_file == '' {
	...
	var_comment_author = var_commenter.array_get('comment_author')
	var_comment_author_email = var_commenter.array_get('comment_author_email')
	var_comment_author_url = rt.call_function('esc_url', [
		var_commenter.array_get('comment_author_url'),
	])
	...
	if rt.is_true(rt.call_function('get_option', [rt.new_string('page_comments')])) {
		...
		])))
		{
			...
			if rt.is_true(var_comment_args.array_get('hierarchical')) {
			...
			if var_comment_args.array_isset(rt.new_string('include_unapproved')) {
				var_top_level_args.array_set('include_unapproved',
					var_comment_args.array_get('include_unapproved'))
			}
			...
			for {
				...
				var_comment_children = rt.call_method(var__comment_shadow, 'get_children', [
					rt.create_array([rt.ArrayItem{ key: 'format', val: 'flat' },
						rt.ArrayItem{ key: 'status', val: var_comment_args.array_get('status') },
						rt.ArrayItem{ key: 'orderby', val: var_comment_args.array_get('orderby') }]),
				])
				{
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.identical(rt.new_string(''), rt.call_function('get_query_var', [rt.new_string('cpage')])))
...
fn comments_popup_link(zero bool, one bool, more bool, css_class string, none bool) {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_int(0), var_comments_number))
		&& rt.is_true(rt.new_bool(!(rt.is_true(comments_open(rt.new_null())))))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(pings_open(rt.new_null())))))))
	{
		rt.call_function('printf', [rt.new_string('<span%1$s>%2$s</span>'), if !(var_css_class == '') {
			' class="' +
	...
	rt.call_function('printf', [rt.new_string('<a href="%1$s"%2$s%3$s>%4$s</a>'),
		rt.call_function('esc_url', [var_comments_link.clone()]), if !(var_css_class == '') {
			' class="' + var_css_class + '" '
		} else {
			''
...
fn get_comment_reply_link(var_args_arg rt.PhpVal, var_comment_arg rt.PhpVal, var_post_arg rt.PhpVal) rt.PhpVal {
	...
	var_args = rt.call_function('wp_parse_args', [var_args.clone(),
		rt.create_array_from_native_map(var_defaults)])
	var_args.array_set('max_depth', rt.new_int((var_args.array_get('max_depth')).to_i64()))
	var_args.array_set('depth', rt.new_int((var_args.array_get('depth')).to_i64()))
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_int(0), var_args.array_get('depth')))
		|| rt.is_true(rt.less_equal(var_args.array_get('max_depth'), var_args.array_get('depth')))))
	{
		return rt.new_null()
	}
	var_comment = rt.call_function('get_comment', [var_comment.clone()])
		...
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})))))))
	{
		var_link = rt.call_function('sprintf', [
			...
			var_args.array_get('login_text'),
		])
	} else {
		var_data_attributes = {
			'commentid':      rt.get_property(var_comment, 'comment_ID')
			'postid':         rt.get_property(var_post, 'ID')
			'belowelement':   (var_args.array_get('add_below')).str() + '-' +
				(rt.get_property(var_comment, 'comment_ID')).str()
			'respondelement': var_args.array_get('respond_id')
			'replyto':        rt.call_function('sprintf', [var_args.array_get('reply_to_text'),
		...
		var_reply_text = if rt.is_true(var_args.array_get('show_reply_to_text')) { rt.call_function('sprintf', [
				var_args.array_get('reply_to_text'),
				get_comment_author(var_comment.clone()),
			]) } else { var_args.array_get('reply_text') }
		var_aria_label = if rt.is_true(var_args.array_get('show_reply_to_text')) { rt.new_string('') } else { rt.call_function('sprintf', [
				var_args.array_get('reply_to_text'),
				get_comment_author(var_comment.clone()),
			]) }
		var_link = rt.call_function('sprintf', [
			...
			rt.new_string(
				...
				'#' + (var_args.array_get('respond_id')).str()),
			rt.new_string(var_data_attribute_string.str()).clone(),
			if rt.is_true(var_aria_label) {
	...
	var_comment_reply_link = rt.new_string((var_args.array_get('before')).str() + var_link.str() +
		(var_args.array_get('after')).str())
...
fn get_post_reply_link(var_args_arg rt.PhpVal, var_post_arg rt.PhpVal) bool {
		...
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})))))))
	{
		...
		var_onclick = rt.call_function('sprintf', [
			...
			var_args.array_get('add_below'),
			rt.get_property(var_post, 'ID'),
			var_args.array_get('respond_id'),
		])
		var_link = rt.call_function('sprintf', [
			...
			rt.new_string(
				...
				(var_args.array_get('respond_id')).str()),
			var_onclick.clone(),
			var_args.array_get('reply_text'),
		])
	}
	var_post_reply_link = rt.new_string((var_args.array_get('before')).str() + var_link.str() +
		(var_args.array_get('after')).str())
...
fn _get_comment_reply_id(var_post_arg rt.PhpVal) i64 {
	...
	var_post = rt.call_function('get_post', [var_post.clone()])
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_post))))
		|| !(rt.get_superglobal('_GET').array_isset(rt.new_string('replytocom')))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_superglobal('_GET').array_get('replytocom').is_long()
		|| rt.get_superglobal('_GET').array_get('replytocom').is_double())))))))
	{
		return 0
	}
	var_reply_to_id = rt.new_int((rt.get_superglobal('_GET').array_get('replytocom')).to_i64())
	var_comment = rt.call_function('get_comment', [var_reply_to_id.clone()])
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_comment, 'WP_Comment'))))))
...
fn wp_list_comments(var_args rt.PhpVal, var_comments_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_comments)))) {
			...
			var_parsed_args.array_get('type')))))
		{
			var_comments_by_type = rt.call_function('separate_comments', [
				var_comments.clone()])
			if !rt.is_true(var_comments_by_type.array_get(var_parsed_args.array_get('type'))) {
				return rt.new_null()
			}
			var__comments = var_comments_by_type.array_get(var_parsed_args.array_get('type'))
		} else {
			var__comments = var_comments.clone()
		}
	} else {
		if rt.is_true(rt.new_bool(rt.is_true(var_parsed_args.array_get('page'))
			|| rt.is_true(var_parsed_args.array_get('per_page'))))
		{
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int((var_parsed_args.array_get('page')).to_i64()), var_current_cpage))))
				|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int((var_parsed_args.array_get('per_page')).to_i64()), var_current_per_page))))))
			{
				var_comment_args = rt.create_array([
					...
					var_parsed_args.array_get('type')))))
				{
					...
					var__comments =
						var_comments_by_type.array_get(var_parsed_args.array_get('type'))
				} else {
					var__comments = var_comments.clone()
				}
			}
				...
				var_parsed_args.array_get('type')))))
			{
				...
				if !rt.is_true(rt.get_property(var_wp_query, 'comments_by_type').array_get(var_parsed_args.array_get('type'))) {
					return rt.new_null()
				}
				var__comments =
					rt.get_property(var_wp_query, 'comments_by_type').array_get(var_parsed_args.array_get('type'))
			} else {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.identical(rt.new_string(''), var_parsed_args.array_get('per_page')))
		&& rt.is_true(rt.call_function('get_option', [rt.new_string('page_comments')]))))
	{
	...
	if !rt.is_true(var_parsed_args.array_get('per_page')) {
		var_parsed_args.array_set('per_page', 0)
		var_parsed_args.array_set('page', 0)
	}
	if rt.is_true(rt.identical(rt.new_string(''), var_parsed_args.array_get('max_depth'))) {
		if rt.is_true(rt.call_function('get_option', [rt.new_string('thread_comments')])) {
	...
	if rt.is_true(rt.identical(rt.new_string(''), var_parsed_args.array_get('page'))) {
		if !rt.is_true(var_overridden_cpage) {
			...
			var_threaded =
				rt.new_bool(-1 != rt.new_int((var_parsed_args.array_get('max_depth')).to_i64()))
			var_parsed_args.array_set('page', if rt.is_true(rt.identical(rt.new_string('newest'), rt.call_function('get_option', [
				rt.new_string('default_comments_page'),
			])))
			{
				rt.call_function('get_comment_pages_count', [
					var__comments.clone(), var_parsed_args.array_get('per_page'),
					var_threaded.clone()])
			} else {
				rt.new_int(1)
			})
			rt.call_function('set_query_var', [rt.new_string('cpage'),
				var_parsed_args.array_get('page')])
		}
	}
	var_parsed_args.array_set('page', rt.new_int((var_parsed_args.array_get('page')).to_i64()))
	var_parsed_args.array_set('per_page',
		rt.new_int((var_parsed_args.array_get('per_page')).to_i64()))
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.identical(rt.new_int(0), var_parsed_args.array_get('page')))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int(0), var_parsed_args.array_get('per_page')))))))
	{
		var_parsed_args.array_set('page', 1)
	}
	if rt.is_true(rt.identical(rt.new_null(), var_parsed_args.array_get('reverse_top_level'))) {
	...
	if !rt.is_true(var_parsed_args.array_get('walker')) {
	...
	var_output = rt.call_method(var_walker, 'paged_walk', [var__comments.clone(),
		var_parsed_args.array_get('max_depth'), var_parsed_args.array_get('page'),
		var_parsed_args.array_get('per_page'), var_parsed_args.clone()])
...
fn comment_form(var_args_arg rt.PhpVal, var_post_arg rt.PhpVal) {
	...
	var_req = rt.call_function('get_option', [rt.new_string('require_name_email')])
	var_html5 = (rt.identical(rt.new_string('html5'), var_args.array_get('format'))).to_bool()
	...
	var_fields = rt.create_array([
		rt.ArrayItem{ key: 'author', val: rt.call_function('sprintf', [
			...
			rt.call_function('sprintf', [
				...
				rt.call_function('esc_attr', [var_commenter.array_get('comment_author')]),
				if rt.is_true(var_req) { var_required_attribute } else { '' },
			]),
		]) },
		rt.ArrayItem{ key: 'email', val: rt.call_function('sprintf', [
			...
			rt.call_function('sprintf', [
				...
				if var_html5 { 'type="email"' } else { 'type="text"' },
				rt.call_function('esc_attr', [var_commenter.array_get('comment_author_email')]),
				if rt.is_true(var_req) { var_required_attribute } else { '' },
			]),
		]) },
		rt.ArrayItem{ key: 'url', val: rt.call_function('sprintf', [
			...
			rt.call_function('sprintf', [
				...
				if var_html5 { 'type="url"' } else { 'type="text"' },
				rt.call_function('esc_attr', [var_commenter.array_get('comment_author_url')]),
			]),
		]) },
	])
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.call_function('has_action', [rt.new_string('set_comment_cookies'), rt.new_string('wp_set_comment_cookies')]))
		&& rt.is_true(rt.call_function('get_option', [rt.new_string('show_comments_cookies_opt_in')]))))
	{
		var_consent = if !rt.is_true(var_commenter.array_get('comment_author_email')) {
			''
		} else {
		...
		if var_args.array_isset(rt.new_string('fields'))
			&& !(var_args.array_get('fields').array_isset(rt.new_string('cookies'))) {
			var_args.array_get_mut('fields').array_set('cookies', var_fields.array_get('cookies'))
		}
	}
	...
	if rt.is_true(rt.new_bool(var_args.array_get('fields').array_isset(rt.new_string('email'))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_args.array_get('comment_notes_before'), rt.new_string('id="email-notes"')])))))))
	{
		var_args.array_get_mut('fields').array_set('email', rt.call_function('str_replace', [
			rt.new_string(' aria-describedby="email-notes"'),
			rt.new_string(''),
			var_args.array_get('fields').array_get('email'),
		]))
	}
	rt.call_function('do_action', [rt.new_string('comment_form_before')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_args.array_get('class_container')]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(var_args.array_get('title_reply_before'))
	comment_form_title(var_args.array_get('title_reply'), var_args.array_get('title_reply_to'),
		true, var_post_id.clone())
	if rt.is_true(rt.call_function('get_option', [rt.new_string('thread_comments')])) {
		rt.echo_val(var_args.array_get('cancel_reply_before'))
		cancel_comment_reply_link(var_args.array_get('cancel_reply_link'))
		rt.echo_val(var_args.array_get('cancel_reply_after'))
	}
	rt.echo_val(var_args.array_get('title_reply_after'))
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.call_function('get_option', [rt.new_string('comment_registration')]))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})))))))
	{
		rt.echo_val(var_args.array_get('must_log_in'))
		...
		rt.call_function('printf', [
			...
			rt.call_function('esc_url', [var_args.array_get('action')]),
			rt.call_function('esc_attr', [var_args.array_get('id_form')]),
			rt.call_function('esc_attr', [var_args.array_get('class_form')]),
			if rt.is_true(var_args.array_get('novalidate')) { ' novalidate' } else { '' },
		])
		...
		if rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})) {
			rt.echo_val(rt.call_function('apply_filters', [
				...
				var_args.array_get('logged_in_as'),
				var_commenter.clone(),
				var_user_identity.clone(),
			]))
			...
			rt.echo_val(var_args.array_get('comment_notes_before'))
		}
		var_comment_fields = rt.add(rt.create_array([
			rt.ArrayItem{ key: 'comment', val: var_args.array_get('comment_field') },
		]), rt.cast_array(var_args.array_get('fields')))
		...
		{
			mut iter_1 := var_comment_fields.iterator()
			for {
				item_1 := iter_1.next() or { break }
				...
				if rt.is_true(rt.identical(rt.new_string('comment'), var_name_shadow)) {
					...
					rt.echo_val(var_args.array_get('comment_notes_after'))
		...
		var_submit_button = rt.call_function('sprintf', [var_args.array_get('submit_button'),
			rt.call_function('esc_attr', [var_args.array_get('name_submit')]),
			rt.call_function('esc_attr', [var_args.array_get('id_submit')]),
			rt.call_function('esc_attr', [var_args.array_get('class_submit')]),
			rt.call_function('esc_attr', [var_args.array_get('label_submit')])])
		var_submit_button = rt.call_function('apply_filters', [
			rt.new_string('comment_form_submit_button'),
			var_submit_button.clone(),
			var_args.clone(),
		])
		var_submit_field = rt.call_function('sprintf', [var_args.array_get('submit_field'),
			var_submit_button.clone(), rt.new_string(get_comment_id_fields(var_post_id.clone()))])
		rt.echo_val(rt.call_function('apply_filters', [
...
pub fn init_wp_includes_comment_template_php() {
}
