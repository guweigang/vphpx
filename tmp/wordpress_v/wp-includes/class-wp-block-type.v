...
pub fn Class_WP_Block_Type.global_attributes() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: 'lock', val: rt.create_array([rt.ArrayItem{ key: 'type', val: 'object' }]) }, rt.ArrayItem{ key: 'metadata', val: rt.create_array([rt.ArrayItem{ key: 'type', val: 'object' }]) }])
}
...
fn (mut this Class_WP_Block_Type) magic_get(var_name rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_name.dup(), this.deprecated_properties, rt.new_bool(true)]))))) {
		return rt.new_null()
	}
	mut var_new_name := rt.new_string((var_name).str() + '_handles')
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('property_exists', [rt.new_object('WP_Block_Type', []string{}, &this), var_new_name.dup()]))))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_property(rt.new_object('WP_Block_Type', []string{}, &this), '{"nodeType":"Expr_Variable","line":379,"name":"new_name"}').is_array()))))))) {
		return rt.new_null()
	}
	if rt.get_property(rt.new_object('WP_Block_Type', []string{}, &this), '{"nodeType":"Expr_Variable","line":383,"name":"new_name"}').array_count() > 1 {
		return rt.get_property(rt.new_object('WP_Block_Type', []string{}, &this), '{"nodeType":"Expr_Variable","line":384,"name":"new_name"}')
	}
	return if !(rt.get_property(rt.new_object('WP_Block_Type', []string{}, &this), '{"nodeType":"Expr_Variable","line":386,"name":"new_name"}').array_get(0)).is_null() { rt.get_property(rt.new_object('WP_Block_Type', []string{}, &this), '{"nodeType":"Expr_Variable","line":386,"name":"new_name"}').array_get(0) } else { rt.new_null() }
}

fn (mut this Class_WP_Block_Type) magic_isset(var_name rt.PhpVal) bool {
	if rt.is_true(rt.call_function('in_array', [var_name.dup(), rt.create_array([rt.ArrayItem{ key: none, val: 'variations' }, rt.ArrayItem{ key: none, val: 'uses_context' }]), rt.new_bool(true)])) {
		return true
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_name.dup(), this.deprecated_properties, rt.new_bool(true)]))))) {
		return false
	...
	mut var_new_name := rt.new_string((var_name).str() + '_handles')
	return (rt.new_bool(rt.get_property(rt.new_object('WP_Block_Type', []string{}, &this), '{"nodeType":"Expr_Variable","line":410,"name":"new_name"}').array_isset(rt.new_int(0)))).to_bool()
}

fn (mut this Class_WP_Block_Type) magic_set(var_name rt.PhpVal, var_value rt.PhpVal)  {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_name.dup(), this.deprecated_properties, rt.new_bool(true)]))))) {
		this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":425,"name":"name"}', var_value.dup())
		return rt.new_null()
	}
	mut var_new_name := rt.new_string((var_name).str() + '_handles')
	if rt.is_true(rt.new_bool(var_value.dup().is_array())) {
		mut var_filtered := rt.call_function('array_filter', [var_value.dup(), rt.new_string('is_string')])
		if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
			rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('The %s argument must be a string or a string array.')]), rt.new_string('<code>$value</code>')]), rt.new_string('6.1.0')])
		}
		this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":446,"name":"new_name"}', rt.call_function('array_values', [var_filtered.dup()]))
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_value.dup().is_string()))))) {
		return rt.new_null()
	}
	this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":454,"name":"new_name"}', rt.create_array([rt.ArrayItem{ key: none, val: var_value }]))
}
...
fn (mut this Class_WP_Block_Type) render(var_attributes rt.PhpVal, content string) string {
	...
	var_attributes_mutated = this.prepare_attributes_for_render(var_attributes_mutated.dup())
...
fn (mut this Class_WP_Block_Type) prepare_attributes_for_render(var_attributes rt.PhpVal) rt.PhpVal {
	...
	}
	{
		...
		for {
			...
			mut var_is_valid := rt.call_function('rest_validate_value_from_schema', [var_value.dup(), var_schema.dup(), var_attribute_name.dup()])
			if rt.is_true(rt.call_function('is_wp_error', [var_is_valid.dup()])) {
				var_attributes_mutated.array_unset(var_attribute_name)
			}
		}
	}
	mut var_missing_schema_attributes := rt.call_function('array_diff_key', [this.attributes, var_attributes_mutated.dup()])
	{
		mut iter_1 := var_missing_schema_attributes.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_schema := item_1.val
			mut var_attribute_name := item_1.key
			if var_schema.array_isset(rt.new_string('default')) {
				var_attributes_mutated.array_set(var_attribute_name, var_schema.array_get('default'))
			}
		}
	}
	return var_attributes_mutated.dup()
...
fn (mut this Class_WP_Block_Type) set_props(var_args rt.PhpVal)  {
	mut var_args_mutated := var_args
	var_args_mutated = rt.call_function('wp_parse_args', [var_args_mutated.dup(), rt.create_array([rt.ArrayItem{ key: 'render_callback', val: rt.new_null() }])])
	var_args_mutated.array_set('name', this.name)
	if rt.is_true(rt.new_bool(!(var_args_mutated.array_isset(rt.new_string('attributes'))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_args_mutated.array_get('attributes').is_array()))))))) {
		var_args_mutated.array_set('attributes', rt.new_array())
	}
	{
		mut iter_1 := Class_static.global_attributes().iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_attr_schema := item_1.val
			mut var_attr_key := item_1.key
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_args_mutated.array_get('attributes').array_isset(var_attr_key.dup())))))) {
				var_args_mutated.array_get_mut('attributes').array_set(var_attr_key, var_attr_schema.dup())
			}
		}
	}
	var_args_mutated = rt.call_function('apply_filters', [rt.new_string('register_block_type_args'), var_args_mutated.dup(), this.name])
	{
		...
		for {
			...
			this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":576,"name":"property_name"}', var_property_value.dup())
		}
	}
}

fn (mut this Class_WP_Block_Type) get_attributes() rt.PhpVal {
	return if rt.is_true(rt.new_bool(this.attributes.is_array())) { this.attributes } else { rt.new_array() }
}
...
fn (mut this Class_WP_Block_Type) get_variations() rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('get_block_type_variations'), this.variations, rt.new_object('WP_Block_Type', []string{}, &this)])
}

fn (mut this Class_WP_Block_Type) get_uses_context() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('get_block_type_uses_context'), this.uses_context, rt.new_object('WP_Block_Type', []string{}, &this)])
}
...
fn (mut this Class_WP_Block_Type) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'ancestor' { this.ancestor = val; return true }
		'allowed_blocks' { this.allowed_blocks = val; return true }
		'icon' { this.icon = val; return true }
		'description' { this.description = val; return true }
		'keywords' { this.keywords = val; return true }
		'textdomain' { this.textdomain = val; return true }
		'styles' { this.styles = val; return true }
		'variations' { this.variations = val; return true }
		'variation_callback' { this.variation_callback = val; return true }
		'selectors' { this.selectors = val; return true }
		'supports' { this.supports = val; return true }
		'example' { this.example = val; return true }
		'render_callback' { this.render_callback = val; return true }
		'attributes' { this.attributes = val; return true }
		'uses_context' { this.uses_context = val; return true }
		'provides_context' { this.provides_context = val; return true }
		'block_hooks' { this.block_hooks = val; return true }
		'editor_script_handles' { this.editor_script_handles = val; return true }
		'script_handles' { this.script_handles = val; return true }
		'view_script_handles' { this.view_script_handles = val; return true }
		'view_script_module_ids' { this.view_script_module_ids = val; return true }
		'editor_style_handles' { this.editor_style_handles = val; return true }
		'style_handles' { this.style_handles = val; return true }
		'view_style_handles' { this.view_style_handles = val; return true }
		'deprecated_properties' { this.deprecated_properties = val; return true }
...
pub fn init_wp_includes_class_wp_block_type_php() {
}
