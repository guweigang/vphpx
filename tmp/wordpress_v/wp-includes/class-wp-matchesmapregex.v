...
fn (mut this Class_WP_MatchesMapRegex) construct(var_subject rt.PhpVal, var_matches rt.PhpVal)  {
	this._subject = var_subject.dup()
	this._matches = var_matches.dup()
	this.output = this._map()
}

fn Class_WP_MatchesMapRegex.apply(var_subject rt.PhpVal, var_matches rt.PhpVal) rt.PhpVal {
	mut var_result := create_wp_matchesmapregex(var_subject.dup(), var_matches.dup())
	return var_result.output
}

fn (mut this Class_WP_MatchesMapRegex) _map() rt.PhpVal {
	mut var_callback := [rt.new_object('WP_MatchesMapRegex', []string{}, &this), rt.new_string('callback')]
	return rt.call_function('preg_replace_callback', [this._pattern, var_callback.dup(), this._subject])
}

fn (mut this Class_WP_MatchesMapRegex) callback(var_matches rt.PhpVal) rt.PhpVal {
	mut var_index := // unsupported expression: Expr_Cast_Int
	return if this._matches.array_isset(var_index) { rt.call_function('urlencode', [this._matches.array_get(var_index)]) } else { rt.new_string('') }
}
...
fn (mut this Class_WP_MatchesMapRegex) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'_matches' { this._matches = val; return true }
		'output' { this.output = val; return true }
		'_subject' { this._subject = val; return true }
		'_pattern' { this._pattern = val; return true }
...
pub fn init_wp_includes_class_wp_matchesmapregex_php() {
}
