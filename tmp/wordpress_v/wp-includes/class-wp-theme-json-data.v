...
fn (mut this Class_WP_Theme_JSON_Data) construct(var_data rt.PhpVal, origin string) {
	this.origin = rt.new_string(origin).dup()
	this.theme_json = create_wp_theme_json(var_data.dup(), this.origin)
}

fn (mut this Class_WP_Theme_JSON_Data) update_with(var_new_data rt.PhpVal) rt.PhpVal {
	rt.call_method(this.theme_json, 'merge', [
		create_wp_theme_json(var_new_data.dup(), this.origin),
	])
...
pub fn init_wp_includes_class_wp_theme_json_data_php() {
}
