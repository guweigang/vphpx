...
fn get_categories(args string) rt.PhpVal {
	...
	rt.new_string(var_args.str()).array_set('taxonomy', rt.call_function('apply_filters', [
		...
		rt.new_string(var_args.str()).array_get('taxonomy'),
		rt.new_string(var_args.str()),
	]))
	if rt.is_true(rt.new_bool(rt.new_string(var_args.str()).array_isset(rt.new_string('type'))
		&& rt.is_true(rt.identical(rt.new_string('link'), rt.new_string(var_args.str()).array_get('type')))))
	{
...
fn get_category_by_path(var_category_path_arg rt.PhpVal, full_match bool, var_output rt.PhpVal) rt.PhpVal {
	...
	var_full_path = ''
	{
		mut iter_1 := rt.cast_array(var_category_paths).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_pathdir_shadow := item_1.val
	...
	}
	{
		...
		for {
			...
			for rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int(0), rt.get_property(var_curcategory, 'parent')))))
...
fn get_cat_name(var_cat_id_arg rt.PhpVal) string {
	...
	var_category = rt.call_function('get_term', [var_cat_id.clone(),
		rt.new_string('category')])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_category))))
...
fn _make_cat_compat(var_category rt.PhpVal) {
	...
	} else if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_category.clone().is_array()))
		&& var_category.array_isset(rt.new_string('term_id'))))
	{
		var_category.array_get('cat_ID') = var_category.array_get('term_id')
		var_category.array_get('category_count') = var_category.array_get('count')
		var_category.array_get('category_description') = var_category.array_get('description')
		var_category.array_get('cat_name') = var_category.array_get('name')
		var_category.array_get('category_nicename') = var_category.array_get('slug')
		var_category.array_get('category_parent') = var_category.array_get('parent')
	}
}

pub fn init_wp_includes_category_php() {
}
