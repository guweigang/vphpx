...
fn (mut this Class_WP_HTML_Open_Elements) contains(node_name string) bool {
	{
		mut iter_1 := this.walk_up(rt.new_null()).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_item := item_1.val
...
fn (mut this Class_WP_HTML_Open_Elements) push(mut var_stack_item Class_WP_HTML_Token)  {
	this.stack.array_push(var_stack_item.dup())
...
fn (mut this Class_WP_HTML_Open_Elements) remove_node(mut var_token Class_WP_HTML_Token) bool {
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_item := item_1.val
			mut var_position_from_end := item_1.key
			if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
				continue
			}
			mut var_position_from_start := rt.sub(rt.sub(this.count(), var_position_from_end), rt.new_int(1))
			rt.call_function('array_splice', [this.stack, var_position_from_start.dup(), rt.new_int(1)])
			this.after_element_pop(mut rt.cast_object_ptr[Class_WP_HTML_Token](var_item))
			return true
		}
	}
...
fn (mut this Class_WP_HTML_Open_Elements) walk_down()  {
	mut var_count := rt.new_int(rt.new_int(this.stack.array_count()))
	{
...
fn (mut this Class_WP_HTML_Open_Elements) after_element_push(mut var_item Class_WP_HTML_Token)  {
	...
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		rt.call_function('call_user_func', [this.push_handler, var_item_mutated.dup()])
	}
}
...
fn (mut this Class_WP_HTML_Open_Elements) after_element_pop(mut var_item Class_WP_HTML_Token)  {
	...
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		rt.call_function('call_user_func', [this.pop_handler, var_item_mutated.dup()])
	}
}
...
fn (mut this Class_WP_HTML_Open_Elements) clear_to_table_context()  {
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_item := item_1.val
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('TABLE'), rt.get_property(var_item, 'node_name'))) || rt.is_true(rt.identical(rt.new_string('TEMPLATE'), rt.get_property(var_item, 'node_name'))))) || rt.is_true(rt.identical(rt.new_string('HTML'), rt.get_property(var_item, 'node_name'))))) {
				break
			}
			this.pop()
		}
...
fn (mut this Class_WP_HTML_Open_Elements) clear_to_table_row_context()  {
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_item := item_1.val
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('TR'), rt.get_property(var_item, 'node_name'))) || rt.is_true(rt.identical(rt.new_string('TEMPLATE'), rt.get_property(var_item, 'node_name'))))) || rt.is_true(rt.identical(rt.new_string('HTML'), rt.get_property(var_item, 'node_name'))))) {
				break
			}
			this.pop()
		}
...
fn create_wp_html_open_elements() &Class_WP_HTML_Open_Elements {
...
pub fn init_wp_includes_html_api_class_wp_html_open_elements_php() {
}
