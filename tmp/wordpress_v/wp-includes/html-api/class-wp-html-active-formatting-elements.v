...
fn (mut this Class_WP_HTML_Active_Formatting_Elements) contains_node(mut var_token Class_WP_HTML_Token) bool {
	{
		...
		for {
			...
			if rt.is_true(rt.identical(rt.get_property(var_token, 'bookmark_name'), rt.get_property(var_item, 'bookmark_name'))) {
				return true
			}
		}
	}
...
fn (mut this Class_WP_HTML_Active_Formatting_Elements) insert_marker()  {
	this.push(mut rt.cast_object_ptr[Class_WP_HTML_Token](create_wp_html_token(rt.new_null(), rt.new_string('marker'), rt.new_bool(false))))
}

fn (mut this Class_WP_HTML_Active_Formatting_Elements) push(mut var_token Class_WP_HTML_Token)  {
	this.stack.array_push(var_token.dup())
}
...
fn create_wp_html_active_formatting_elements() &Class_WP_HTML_Active_Formatting_Elements {
...
fn create_wp_html_token() &Class_WP_HTML_Token {
...
pub fn init_wp_includes_html_api_class_wp_html_active_formatting_elements_php() {
}
