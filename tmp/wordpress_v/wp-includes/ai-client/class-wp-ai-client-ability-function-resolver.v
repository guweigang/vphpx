	...
	this.allowed_abilities = rt.new_array()
	{
		...
		for {
			...
			if rt.is_true(rt.new_bool(rt.instance_of(var_ability, 'WP_Ability'))) {
				this.allowed_abilities.array_set(rt.call_method(var_ability, 'get_name',
					[]rt.PhpVal{}), true)
			} else if rt.is_true(rt.new_bool(var_ability.dup().is_string())) {
...
fn (mut this Class_WP_AI_Client_Ability_Function_Resolver) is_ability_call(mut var_call Class_WordPress_AiClient_Tools_DTO_FunctionCall) bool {
	...
	return (rt.call_function('str_starts_with', [var_name.dup(),
		Class_WP_AI_Client_Ability_Function_Resolver.ability_prefix()])).to_bool()
}
...
fn (mut this Class_WP_AI_Client_Ability_Function_Resolver) execute_ability(mut var_call Class_WordPress_AiClient_Tools_DTO_FunctionCall) rt.PhpVal {
	...
	if !(this.is_ability_call(mut var_call)) {
		return create_wordpress_aiclient_tools_dto_functionresponse(var_function_id.dup(),
			var_function_name.dup(), rt.create_array([
	...
	if !(this.allowed_abilities.array_isset(var_ability_name)) {
		return create_wordpress_aiclient_tools_dto_functionresponse(var_function_id.dup(),
			...
			rt.ArrayItem{ key: 'error', val: rt.call_function('sprintf', [
				...
				var_ability_name.dup(),
			]) },
			rt.ArrayItem{ key: 'code', val: 'ability_not_allowed' },
		]))
	}
	mut var_ability := rt.call_function('wp_get_ability', [var_ability_name.dup()])
	...
	return create_wordpress_aiclient_tools_dto_functionresponse(var_function_id.dup(),
		var_function_name.dup(), var_result.dup())
}
...
fn (mut this Class_WP_AI_Client_Ability_Function_Resolver) has_ability_calls(mut var_message Class_WordPress_AiClient_Messages_DTO_Message) bool {
	{
		mut iter_1 := var_message.getparts().iterator()
		for {
			item_1 := iter_1.next() or { break }
				...
				'isFunctionCall', []rt.PhpVal{}))
			{
				...
				if rt.is_true(rt.new_bool(
					rt.is_true(rt.new_bool(rt.instance_of(var_function_call, 'WordPress_AiClient_Tools_DTO_FunctionCall')))
		...
		for {
				...
				'isFunctionCall', []rt.PhpVal{}))
			{
					...
					'WordPress_AiClient_Tools_DTO_FunctionCall')))
				{
					...
					var_response_parts << create_wordpress_aiclient_messages_dto_messagepart(var_function_response.dup())
				}
			}
		}
	}
	return create_wordpress_aiclient_messages_dto_usermessage(var_response_parts.dup())
}
...
fn Class_WP_AI_Client_Ability_Function_Resolver.ability_name_to_function_name(ability_name string) string {
	...
	return Class_WP_AI_Client_Ability_Function_Resolver.ability_prefix() +(rt.call_function('str_replace', [rt.new_string('/'), rt.new_string('__'), rt.new_string(ability_name_mutated).dup()])).str()
}
...
fn Class_WP_AI_Client_Ability_Function_Resolver.function_name_to_ability_name(function_name string) string {
	...
	mut var_without_prefix := rt.call_function('substr', [rt.new_string(function_name_mutated).dup(),
	...
	return (rt.call_function('str_replace', [rt.new_string('__'),
		rt.new_string('/'), var_without_prefix.dup()])).str()
}
...
fn create_wordpress_aiclient_messages_dto_usermessage() &Class_WordPress_AiClient_Messages_DTO_UserMessage {
...
pub fn init_wp_includes_ai_client_class_wp_ai_client_ability_function_resolver_php() {
}
