...
fn (mut this Class_WP_AI_Client_Event_Dispatcher) dispatch(mut var_event Class_object) rt.PhpVal {
	...
	rt.call_function('do_action', [rt.new_string("wp_ai_client_${var_event_name.to_string()}"), var_event])
...
fn (mut this Class_WP_AI_Client_Event_Dispatcher) get_hook_name_portion_for_event(mut var_event Class_object) string {
	...
	mut var_pos := rt.call_function('strrpos', [var_class_name.dup(), rt.new_string('\\')])
	mut var_short_name := if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) { rt.call_function('substr', [var_class_name.dup(), rt.add(var_pos, rt.new_int(1))]) } else { var_class_name }
	mut var_snake_case := rt.new_string(rt.new_string(// unsupported expression: Expr_Cast_String.to_string().to_lower()))
	if rt.is_true(rt.call_function('str_ends_with', [var_snake_case.dup(), rt.new_string('_event')])) {
		var_snake_case = // unsupported expression: Expr_Cast_String
	}
	return (var_snake_case).str()
}

fn create_wp_ai_client_event_dispatcher() &Class_WP_AI_Client_Event_Dispatcher {
...
fn (mut this Class_WP_AI_Client_Event_Dispatcher) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'dispatch' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_object](if args.len > 0 { args[0] } else { rt.new_null() })
...
pub fn init_wp_includes_ai_client_adapters_class_wp_ai_client_event_dispatcher_php() {
}
