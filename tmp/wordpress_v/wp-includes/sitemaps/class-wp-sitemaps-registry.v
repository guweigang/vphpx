...
fn (mut this Class_WP_Sitemaps_Registry) add_provider(var_name rt.PhpVal, mut var_provider Class_WP_Sitemaps_Provider) bool {
	...
	this.providers.array_set(var_name, var_provider_mutated.dup())
	return true
}

fn (mut this Class_WP_Sitemaps_Registry) get_provider(var_name rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_name.dup().is_string())))))
		|| !(this.providers.array_isset(var_name))))
	{
		return rt.new_null()
	}
...
pub fn init_wp_includes_sitemaps_class_wp_sitemaps_registry_php() {
}
