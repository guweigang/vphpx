...
fn (mut this Class_WP_Sitemaps_Users) get_url_list(var_page_num rt.PhpVal, object_subtype string) rt.PhpVal {
	mut var_url_list := rt.call_function('apply_filters', [rt.new_string('wp_sitemaps_users_pre_url_list'), rt.new_null(), var_page_num.dup()])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		return var_url_list.dup()
	}
	mut var_args := this.get_users_query_args()
	var_args.array_set('paged', var_page_num.dup())
	mut var_query := create_wp_user_query(var_args.dup())
	...
	var_url_list = rt.new_array()
	{
		...
		for {
			...
			mut var_sitemap_entry := rt.create_array([rt.ArrayItem{ key: 'loc', val: rt.call_function('get_author_posts_url', [rt.get_property(var_user, 'ID')]) }])
			var_sitemap_entry = rt.call_function('apply_filters', [rt.new_string('wp_sitemaps_users_entry'), var_sitemap_entry.dup(), var_user.dup()])
...
fn (mut this Class_WP_Sitemaps_Users) get_max_num_pages(object_subtype string) rt.PhpVal {
	mut var_max_num_pages := rt.call_function('apply_filters', [rt.new_string('wp_sitemaps_users_pre_max_num_pages'), rt.new_null()])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		return var_max_num_pages.dup()
	}
	mut var_args := this.get_users_query_args()
	mut var_query := create_wp_user_query(var_args.dup())
...
fn (mut this Class_WP_Sitemaps_Users) get_users_query_args() rt.PhpVal {
	mut var_public_post_types := rt.call_function('get_post_types', [rt.create_array([rt.ArrayItem{ key: 'public', val: true }])])
	var_public_post_types.array_unset(rt.new_string('attachment'))
	var_public_post_types.array_unset(rt.new_string('page'))
	mut var_args := rt.call_function('apply_filters', [rt.new_string('wp_sitemaps_users_query_args'), rt.create_array([rt.ArrayItem{ key: 'has_published_posts', val: rt.func_array_keys(var_public_post_types.dup()) }, rt.ArrayItem{ key: 'number', val: rt.call_function('wp_sitemaps_get_max_urls', [rt.get_property(rt.new_object('WP_Sitemaps_Users', ['WP_Sitemaps_Provider'], &this), 'object_type')]) }])])
	return var_args.dup()
}
...
pub fn init_wp_includes_sitemaps_providers_class_wp_sitemaps_users_php() {
}
