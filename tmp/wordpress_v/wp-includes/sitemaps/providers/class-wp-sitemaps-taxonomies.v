...
fn (mut this Class_WP_Sitemaps_Taxonomies) get_object_subtypes() rt.PhpVal {
	mut var_taxonomies := rt.call_function('get_taxonomies', [rt.create_array([rt.ArrayItem{ key: 'public', val: true }]), rt.new_string('objects')])
	var_taxonomies = rt.call_function('array_filter', [var_taxonomies.dup(), rt.new_string('is_taxonomy_viewable')])
	return rt.call_function('apply_filters', [rt.new_string('wp_sitemaps_taxonomies'), var_taxonomies.dup()])
}

fn (mut this Class_WP_Sitemaps_Taxonomies) get_url_list(var_page_num rt.PhpVal, object_subtype string) rt.PhpVal {
	mut var_taxonomy := rt.new_string(rt.new_string(object_subtype))
	mut var_supported_types := this.get_object_subtypes()
	if !(var_supported_types.array_isset(var_taxonomy)) {
		return rt.new_array()
	}
	mut var_url_list := rt.call_function('apply_filters', [rt.new_string('wp_sitemaps_taxonomies_pre_url_list'), rt.new_null(), var_taxonomy.dup(), var_page_num.dup()])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		return var_url_list.dup()
	}
	var_url_list = rt.new_array()
	mut var_offset := rt.mul(rt.sub(var_page_num, rt.new_int(1)), rt.call_function('wp_sitemaps_get_max_urls', [rt.get_property(rt.new_object('WP_Sitemaps_Taxonomies', ['WP_Sitemaps_Provider'], &this), 'object_type')]))
	mut var_args := this.get_taxonomies_query_args(var_taxonomy.dup())
	var_args.array_set('fields', 'all')
	var_args.array_set('offset', var_offset.dup())
	mut var_taxonomy_terms := create_wp_term_query(var_args.dup())
	if !(!rt.is_true(rt.get_property(var_taxonomy_terms, 'terms'))) {
		{
			...
			for {
				...
				mut var_term_link := rt.call_function('get_term_link', [var_term.dup(), var_taxonomy.dup()])
				if rt.is_true(rt.call_function('is_wp_error', [var_term_link.dup()])) {
					continue
				}
				mut var_sitemap_entry := rt.create_array([rt.ArrayItem{ key: 'loc', val: var_term_link }])
				var_sitemap_entry = rt.call_function('apply_filters', [rt.new_string('wp_sitemaps_taxonomies_entry'), var_sitemap_entry.dup(), rt.get_property(var_term, 'term_id'), var_taxonomy.dup(), var_term.dup()])
...
fn (mut this Class_WP_Sitemaps_Taxonomies) get_max_num_pages(object_subtype string) i64 {
	...
	mut var_taxonomy := rt.new_string(rt.new_string(object_subtype))
	mut var_max_num_pages := rt.call_function('apply_filters', [rt.new_string('wp_sitemaps_taxonomies_pre_max_num_pages'), rt.new_null(), var_taxonomy.dup()])
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		return (var_max_num_pages).to_i64()
	}
	mut var_term_count := rt.call_function('wp_count_terms', [this.get_taxonomies_query_args(var_taxonomy.dup())])
...
fn (mut this Class_WP_Sitemaps_Taxonomies) get_taxonomies_query_args(var_taxonomy rt.PhpVal) rt.PhpVal {
	...
	mut var_args := rt.call_function('apply_filters', [rt.new_string('wp_sitemaps_taxonomies_query_args'), rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: var_taxonomy_mutated }, rt.ArrayItem{ key: 'orderby', val: 'term_order' }, rt.ArrayItem{ key: 'number', val: rt.call_function('wp_sitemaps_get_max_urls', [rt.get_property(rt.new_object('WP_Sitemaps_Taxonomies', ['WP_Sitemaps_Provider'], &this), 'object_type')]) }, rt.ArrayItem{ key: 'hide_empty', val: true }, rt.ArrayItem{ key: 'hierarchical', val: false }, rt.ArrayItem{ key: 'update_term_meta_cache', val: false }]), var_taxonomy_mutated.dup()])
	return var_args.dup()
}
...
pub fn init_wp_includes_sitemaps_providers_class_wp_sitemaps_taxonomies_php() {
}
