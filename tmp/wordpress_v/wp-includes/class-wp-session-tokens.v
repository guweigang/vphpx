...
fn Class_WP_Session_Tokens.get_instance(var_user_id rt.PhpVal) rt.PhpVal {
	...
	return rt.create_object_dynamically(var_manager, [var_user_id.dup()])
}
...
fn (mut this Class_WP_Session_Tokens) hash_token(var_token rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('hash', [rt.new_string('sha256'),
		var_token_mutated.dup()])
}

fn (mut this Class_WP_Session_Tokens) get(var_token rt.PhpVal) rt.PhpVal {
	mut var_token_mutated := var_token
	mut var_verifier := this.hash_token(var_token_mutated.dup())
	this.get_session(var_verifier.dup())
	return rt.new_null()
}

fn (mut this Class_WP_Session_Tokens) verify(var_token rt.PhpVal) rt.PhpVal {
	mut var_token_mutated := var_token
	mut var_verifier := this.hash_token(var_token_mutated.dup())
	return
}
...
fn (mut this Class_WP_Session_Tokens) create(var_expiration rt.PhpVal) rt.PhpVal {
	...
	var_session.array_set('expiration', var_expiration.dup())
	if !(!rt.is_true(rt.get_superglobal('_SERVER').array_get('REMOTE_ADDR'))) {
		var_session.array_set('ip', rt.get_superglobal('_SERVER').array_get('REMOTE_ADDR'))
	}
	if !(!rt.is_true(rt.get_superglobal('_SERVER').array_get('HTTP_USER_AGENT'))) {
	...
	this.update(var_token.dup(), var_session.dup())
...
fn (mut this Class_WP_Session_Tokens) update(var_token rt.PhpVal, var_session rt.PhpVal) {
	...
	mut var_verifier := this.hash_token(var_token_mutated.dup())
	this.update_session(var_verifier.dup(), var_session_mutated.dup())
}

fn (mut this Class_WP_Session_Tokens) destroy(var_token rt.PhpVal) {
	mut var_token_mutated := var_token
	mut var_verifier := this.hash_token(var_token_mutated.dup())
	this.update_session(var_verifier.dup(), rt.new_null())
}

fn (mut this Class_WP_Session_Tokens) destroy_others(var_token_to_keep rt.PhpVal) {
	mut var_verifier := this.hash_token(var_token_to_keep.dup())
	mut var_session := this.get_session(var_verifier.dup())
	if rt.is_true(var_session) {
		this.destroy_other_sessions(var_verifier.dup())
	} else {
...
fn (mut this Class_WP_Session_Tokens) is_still_valid(var_session rt.PhpVal) rt.PhpVal {
	mut var_session_mutated := var_session
	return rt.greater_equal(var_session_mutated.array_get('expiration'), rt.call_function('time',
		[]rt.PhpVal{}))
}
...
pub fn init_wp_includes_class_wp_session_tokens_php() {
}
