...
fn (mut this Class_WP_REST_Autosaves_Controller) get_parent(var_parent_id rt.PhpVal) rt.PhpVal {
	...
	return rt.call_method(this.revisions_controller, 'get_parent', [var_parent_id_mutated.dup()])
}

fn (mut this Class_WP_REST_Autosaves_Controller) get_items_permissions_check(var_request rt.PhpVal) bool {
	mut var_parent := this.get_parent(var_request.array_get('id'))
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('edit_post'), rt.get_property(var_parent, 'ID')]))))) {
		return (create_wp_error(rt.new_string('rest_cannot_read'), rt.call_function('__', [rt.new_string('Sorry, you are not allowed to view autosaves of this post.')]), rt.create_array([rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code', []rt.PhpVal{}) }]))).to_bool()
	}
	return true
}
...
fn (mut this Class_WP_REST_Autosaves_Controller) create_item_permissions_check(var_request rt.PhpVal) rt.PhpVal {
	...
	if !rt.is_true(var_id) {
		return create_wp_error(rt.new_string('rest_post_invalid_id'), rt.call_function('__', [rt.new_string('Invalid item ID.')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	return rt.call_method(this.parent_controller, 'update_item_permissions_check', [var_request.dup()])
}

fn (mut this Class_WP_REST_Autosaves_Controller) create_item(var_request rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('WP_RUN_CORE_TESTS')]))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('DOING_AUTOSAVE')]))))))) {
		rt.call_function('define', [rt.new_string('DOING_AUTOSAVE'), rt.new_bool(true)])
	}
	mut var_post := this.get_parent(var_request.array_get('id'))
	if rt.is_true(rt.call_function('is_wp_error', [var_post.dup()])) {
		return var_post.dup()
	}
	mut var_prepared_post := rt.call_method(this.parent_controller, 'prepare_item_for_database', [var_request.dup()])
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('wp_check_post_lock')]))))) {
		rt.include_file((rt.get_constant('ABSPATH')).str() + 'wp-admin/includes/post.php', '4')
	}
	mut var_post_lock_is_active := rt.call_function('wp_check_post_lock', [rt.get_property(var_post, 'ID')])
	mut var_is_draft := rt.new_bool(rt.new_bool(rt.is_true(rt.identical(rt.new_string('draft'), rt.get_property(var_post, 'post_status'))) || rt.is_true(rt.identical(rt.new_string('auto-draft'), rt.get_property(var_post, 'post_status')))))
	mut var_can_update_author_draft_post := rt.new_bool(rt.new_bool(rt.is_true(var_is_draft) && rt.is_true(rt.identical(// unsupported expression: Expr_Cast_Int, var_user_id))))
	mut var_should_update_parent_draft_post := rt.new_bool(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_post_lock_is_active)))) && rt.is_true(var_can_update_author_draft_post)))
	if rt.is_true(var_should_update_parent_draft_post) {
		mut var_autosave_id := rt.call_function('wp_update_post', [rt.call_function('wp_slash', [rt.cast_array(var_prepared_post)]), rt.new_bool(true)])
	} else {
		var_autosave_id = this.create_post_autosave(rt.cast_array(var_prepared_post), mut rt.cast_object_ptr[Class_array](rt.cast_array(rt.call_method(var_request, 'get_param', [rt.new_string('meta')]))))
	}
	if rt.is_true(rt.call_function('is_wp_error', [var_autosave_id.dup()])) {
		return var_autosave_id.dup()
	}
	mut var_autosave := rt.call_function('get_post', [var_autosave_id.dup()])
...
fn (mut this Class_WP_REST_Autosaves_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.less_equal(var_parent_id, rt.new_int(0))) {
		return create_wp_error(rt.new_string('rest_post_invalid_id'), rt.call_function('__', [rt.new_string('Invalid post parent ID.')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	mut var_autosave := rt.call_function('wp_get_post_autosave', [var_parent_id.dup()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_autosave)))) {
		return create_wp_error(rt.new_string('rest_post_no_autosave'), rt.call_function('__', [rt.new_string('There is no autosave revision for this post.')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	mut var_response := this.prepare_item_for_response(var_autosave.dup(), var_request.dup())
	return var_response.dup()
}
...
fn (mut this Class_WP_REST_Autosaves_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_revisions := rt.call_function('wp_get_post_revisions', [var_parent_id.dup(), rt.create_array([rt.ArrayItem{ key: 'check_enabled', val: false }])])
	{
		mut iter_1 := var_revisions.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_revision := item_1.val
			if rt.is_true(rt.call_function('str_contains', [rt.get_property(var_revision, 'post_name'), rt.new_string("${var_parent_id.to_string()}-autosave")])) {
...
fn (mut this Class_WP_REST_Autosaves_Controller) get_item_schema() rt.PhpVal {
	if rt.is_true(rt.get_property(rt.new_object('WP_REST_Autosaves_Controller', ['WP_REST_Revisions_Controller'], &this), 'schema')) {
		return this.add_additional_fields_schema(rt.get_property(rt.new_object('WP_REST_Autosaves_Controller', ['WP_REST_Revisions_Controller'], &this), 'schema'))
	}
	mut var_schema := rt.call_method(this.revisions_controller, 'get_item_schema', []rt.PhpVal{})
	var_schema.array_get_mut('properties').array_set('preview_link', rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Preview link for the post.')]) }, rt.ArrayItem{ key: 'type', val: 'string' }, rt.ArrayItem{ key: 'format', val: 'uri' }, rt.ArrayItem{ key: 'context', val: rt.create_array([rt.ArrayItem{ key: none, val: 'edit' }]) }, rt.ArrayItem{ key: 'readonly', val: true }]))
...
fn (mut this Class_WP_REST_Autosaves_Controller) create_post_autosave(var_post_data rt.PhpVal, mut var_meta Class_array) rt.PhpVal {
	...
	mut var_autosave_is_different := rt.new_bool(rt.new_bool(false))
	mut var_new_autosave := rt.call_function('_wp_post_revision_data', [var_post_data.dup(), rt.new_bool(true)])
	{
		mut iter_1 := rt.call_function('array_intersect', [rt.func_array_keys(var_new_autosave.dup()), rt.func_array_keys(rt.call_function('_wp_post_revision_fields', [var_post.dup()]))]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_field := item_1.val
			if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
				var_autosave_is_different = rt.new_bool(rt.new_bool(true))
				break
			}
		}
	}
	if !(!rt.is_true(var_meta)) {
		mut var_revisioned_meta_keys := rt.call_function('wp_post_revision_meta_keys', [rt.get_property(var_post, 'post_type')])
		{
			mut iter_1 := var_revisioned_meta_keys.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_meta_key := item_1.val
				mut var_old_meta := rt.call_function('get_metadata_raw', [rt.new_string('post'), var_post_id.dup(), var_meta_key.dup(), rt.new_bool(true)])
				mut var_new_meta := if !(var_meta.array_get(var_meta_key)).is_null() { var_meta.array_get(var_meta_key) } else { rt.new_string('') }
				if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
					var_autosave_is_different = rt.new_bool(rt.new_bool(true))
					break
				}
	...
	mut var_user_id := rt.call_function('get_current_user_id', []rt.PhpVal{})
	mut var_old_autosave := rt.call_function('wp_get_post_autosave', [var_post_id.dup(), var_user_id.dup()])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_autosave_is_different)))) && rt.is_true(var_old_autosave))) {
	...
	if rt.is_true(var_old_autosave) {
		...
		var_new_autosave.array_set('post_author', var_user_id.dup())
		rt.call_function('do_action', [rt.new_string('wp_creating_autosave'), var_new_autosave.dup()])
		mut var_revision_id := rt.call_function('wp_update_post', [rt.call_function('wp_slash', [var_new_autosave.dup()])])
	} else {
		var_revision_id = rt.call_function('_wp_put_post_revision', [var_post_data.dup(), rt.new_bool(true)])
	}
	if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_wp_error', [var_revision_id.dup()])) || rt.is_true(rt.identical(rt.new_int(0), var_revision_id)))) {
	...
	if !(!rt.is_true(var_meta)) {
		{
			...
			for {
				...
				if var_meta.array_isset(var_meta_key) {
					rt.call_function('update_metadata', [rt.new_string('post'), var_revision_id.dup(), var_meta_key.dup(), rt.call_function('wp_slash', [var_meta.array_get(var_meta_key)])])
				}
			}
		}
	}
...
fn (mut this Class_WP_REST_Autosaves_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_method(var_request, 'is_method', [rt.new_string('HEAD')])) {
		return rt.call_function('apply_filters', [rt.new_string('rest_prepare_autosave'), create_wp_rest_response(rt.new_array()), var_post.dup(), var_request.dup()])
	}
	mut var_response := rt.call_method(this.revisions_controller, 'prepare_item_for_response', [var_post.dup(), var_request.dup()])
	mut var_fields := this.get_fields_for_response(var_request.dup())
	if rt.is_true(rt.call_function('in_array', [rt.new_string('preview_link'), var_fields.dup(), rt.new_bool(true)])) {
		mut var_parent_id := rt.call_function('wp_is_post_autosave', [var_post.dup()])
		mut var_preview_post_id := if rt.is_true(rt.identical(rt.new_bool(false), var_parent_id)) { rt.get_property(var_post, 'ID') } else { var_parent_id }
		mut var_preview_query_args := rt.new_array()
		if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
			var_preview_query_args['preview_id'] = var_parent_id.dup()
			[] = 
		}
		.array_set(, )
	}
...
pub fn init_wp_includes_rest_api_endpoints_class_wp_rest_autosaves_controller_php() {
}
