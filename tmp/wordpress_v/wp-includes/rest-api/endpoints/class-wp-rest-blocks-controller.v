...
fn (mut this Class_WP_REST_Blocks_Controller) check_read_permission(var_post rt.PhpVal) bool {
	...
	return (this.Class_WP_REST_Posts_Controller.check_read_permission(var_post.dup())).to_bool()
}
...
fn (mut this Class_WP_REST_Blocks_Controller) filter_response_by_context(var_data rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	...
	var_data_mutated = this.Class_WP_REST_Posts_Controller.filter_response_by_context(var_data_mutated.dup(),
		var_context.dup())
	var_data_mutated.array_get('title').array_unset(rt.new_string('rendered'))
	var_data_mutated.array_get('content').array_unset(rt.new_string('rendered'))
	var_data_mutated.array_set('wp_pattern_sync_status', if !(var_data_mutated.array_get('meta').array_get('wp_pattern_sync_status')).is_null() {
		var_data_mutated.array_get('meta').array_get('wp_pattern_sync_status')
	} else {
		rt.new_string('')
	})
	var_data_mutated.array_get('meta').array_unset(rt.new_string('wp_pattern_sync_status'))
...
fn (mut this Class_WP_REST_Blocks_Controller) get_item_schema() rt.PhpVal {
	...
	var_schema.array_get('properties').array_get('title').array_get('properties').array_unset(rt.new_string('rendered'))
	var_schema.array_get('properties').array_get('content').array_get('properties').array_unset(rt.new_string('rendered'))
...
fn create_wp_rest_blocks_controller() &Class_WP_REST_Blocks_Controller {
...
pub fn init_wp_includes_rest_api_endpoints_class_wp_rest_blocks_controller_php() {
}
