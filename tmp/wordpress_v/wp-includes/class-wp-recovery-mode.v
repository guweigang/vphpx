...
fn (mut this Class_WP_Recovery_Mode) initialize()  {
	...
	rt.call_function('add_action', [rt.new_string('wp_logout'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WP_Recovery_Mode', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'exit_recovery_mode' }])])
	rt.call_function('add_action', ['login_form_' + Class_WP_Recovery_Mode.exit_action(), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WP_Recovery_Mode', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'handle_exit_recovery_mode' }])])
	rt.call_function('add_action', [rt.new_string('recovery_mode_clean_expired_keys'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WP_Recovery_Mode', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'clean_expired_keys' }])])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_next_scheduled', [rt.new_string('recovery_mode_clean_expired_keys')]))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_installing', []rt.PhpVal{}))))))) {
		rt.call_function('wp_schedule_event', [rt.call_function('time', []rt.PhpVal{}), rt.new_string('daily'), rt.new_string('recovery_mode_clean_expired_keys')])
	}
	if rt.is_true(rt.call_function('defined', [rt.new_string('WP_RECOVERY_MODE_SESSION_ID')])) {
		this.is_active = true
...
fn (mut this Class_WP_Recovery_Mode) handle_error(mut var_error Class_array) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_extension)))) || this.is_network_plugin(var_extension.dup()))) {
		return (create_wp_error(rt.new_string('invalid_source'), rt.call_function('__', [rt.new_string('Error not caused by a plugin or theme.')]))).to_bool()
	}
	if !(this.is_active()) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_protected_endpoint', []rt.PhpVal{}))))) {
			return (create_wp_error(rt.new_string('non_protected_endpoint'), rt.call_function('__', [rt.new_string('Error occurred on a non-protected endpoint.')]))).to_bool()
		}
		...
		return (rt.call_method(this.email_service, 'maybe_send_recovery_mode_email', [this.get_email_rate_limit(), var_error, var_extension.dup()])).to_bool()
	}
	if !(this.store_error(rt.new_object('array', []string{}, var_error))) {
		return (create_wp_error(rt.new_string('storage_error'), rt.call_function('__', [rt.new_string('Failed to store the error.')]))).to_bool()
	}
...
fn (mut this Class_WP_Recovery_Mode) exit_recovery_mode() bool {
	...
	rt.call_method(rt.call_function('wp_paused_plugins', []rt.PhpVal{}), 'delete_all', []rt.PhpVal{})
...
fn (mut this Class_WP_Recovery_Mode) handle_exit_recovery_mode()  {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_redirect_to)))) {
		var_redirect_to = if rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})) { rt.call_function('admin_url', []rt.PhpVal{}) } else { rt.call_function('home_url', []rt.PhpVal{}) }
	}
	if !(this.is_active()) {
		rt.call_function('wp_safe_redirect', [var_redirect_to.dup()])
		// unsupported expression: Expr_Exit
	}
	if rt.is_true(rt.new_bool(!(rt.get_superglobal('_GET').array_isset(rt.new_string('action'))) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
		return rt.new_null()
	}
	if rt.is_true(rt.new_bool(!(rt.get_superglobal('_GET').array_isset(rt.new_string('_wpnonce'))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_verify_nonce', [rt.get_superglobal('_GET').array_get('_wpnonce'), Class_WP_Recovery_Mode.exit_action()]))))))) {
		rt.call_function('wp_die', [rt.call_function('__', [rt.new_string('Exit recovery mode link expired.')]), rt.new_int(403)])
	}
	if !(this.exit_recovery_mode()) {
		rt.call_function('wp_die', [rt.call_function('__', [rt.new_string('Failed to exit recovery mode. Please try again later.')])])
	}
	rt.call_function('wp_safe_redirect', [var_redirect_to.dup()])
	// unsupported expression: Expr_Exit
}
...
fn (mut this Class_WP_Recovery_Mode) handle_cookie()  {
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_validated.dup()])) {
		rt.call_method(this.cookie_service, 'clear_cookie', []rt.PhpVal{})
		rt.call_method(var_validated, 'add_data', [rt.create_array([rt.ArrayItem{ key: 'status', val: 403 }])])
		rt.call_function('wp_die', [var_validated.dup()])
	}
	mut var_session_id := rt.call_method(this.cookie_service, 'get_session_id_from_cookie', []rt.PhpVal{})
	if rt.is_true(rt.call_function('is_wp_error', [var_session_id.dup()])) {
		rt.call_method(this.cookie_service, 'clear_cookie', []rt.PhpVal{})
		rt.call_method(var_session_id, 'add_data', [rt.create_array([rt.ArrayItem{ key: 'status', val: 403 }])])
		rt.call_function('wp_die', [var_session_id.dup()])
	}
	this.is_active = true
	this.session_id = var_session_id.dup()
}

fn (mut this Class_WP_Recovery_Mode) get_email_rate_limit() rt.PhpVal {
	return rt.call_function('apply_filters', [rt.new_string('recovery_mode_email_rate_limit'), rt.get_constant('DAY_IN_SECONDS')])
}

fn (mut this Class_WP_Recovery_Mode) get_link_ttl() rt.PhpVal {
	mut var_rate_limit := this.get_email_rate_limit()
	mut var_valid_for := var_rate_limit.dup()
	var_valid_for = rt.call_function('apply_filters', [rt.new_string('recovery_mode_email_link_ttl'), var_valid_for.dup()])
	return rt.call_function('max', [var_valid_for.dup(), var_rate_limit.dup()])
}
...
fn (mut this Class_WP_Recovery_Mode) get_extension_for_error(var_error rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('WP_PLUGIN_DIR')]))))) {
		return rt.new_bool(false)
	}
	mut var_error_file := rt.call_function('wp_normalize_path', [var_error.array_get('file')])
	mut var_wp_plugin_dir := rt.call_function('wp_normalize_path', [rt.get_constant('WP_PLUGIN_DIR')])
	if rt.is_true(rt.call_function('str_starts_with', [var_error_file.dup(), var_wp_plugin_dir.dup()])) {
		mut var_path := rt.call_function('str_replace', [(var_wp_plugin_dir).str() + '/', rt.new_string(''), var_error_file.dup()])
		mut var_parts := rt.call_function('explode', [rt.new_string('/'), var_path.dup()])
		return rt.create_array([rt.ArrayItem{ key: 'type', val: 'plugin' }, rt.ArrayItem{ key: 'slug', val: var_parts.array_get(0) }])
	}
	...
	}
	{
		...
		for {
			...
			var_theme_directory = rt.call_function('wp_normalize_path', [var_theme_directory.dup()])
			if rt.is_true(rt.call_function('str_starts_with', [var_error_file.dup(), var_theme_directory.dup()])) {
				var_path = rt.call_function('str_replace', [(var_theme_directory).str() + '/', rt.new_string(''), var_error_file.dup()])
				var_parts = rt.call_function('explode', [rt.new_string('/'), var_path.dup()])
				return rt.create_array([rt.ArrayItem{ key: 'type', val: 'theme' }, rt.ArrayItem{ key: 'slug', val: var_parts.array_get(0) }])
			}
		}
	}
	return rt.new_bool(false)
...
fn (mut this Class_WP_Recovery_Mode) is_network_plugin(var_extension rt.PhpVal) bool {
	...
	{
		mut iter_1 := var_network_plugins.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_plugin := item_1.val
			if rt.is_true(rt.call_function('str_starts_with', [var_plugin.dup(), (var_extension_mutated.array_get('slug')).str() + '/'])) {
				return true
			}
		}
	}
...
fn (mut this Class_WP_Recovery_Mode) store_error(var_error rt.PhpVal) bool {
	mut var_extension := this.get_extension_for_error(var_error.dup())
	if rt.is_true(rt.new_bool(!(rt.is_true(var_extension)))) {
		return false
	}
	mut switch_val_1 := var_extension.array_get('type')
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('plugin'))) {
		return (rt.call_method(rt.call_function('wp_paused_plugins', []rt.PhpVal{}), 'set', [var_extension.array_get('slug'), var_error.dup()])).to_bool()
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('theme'))) {
		return (rt.call_method(rt.call_function('wp_paused_themes', []rt.PhpVal{}), 'set', [var_extension.array_get('slug'), var_error.dup()])).to_bool()
	} else {
		return false
	}
	return false
...
fn (mut this Class_WP_Recovery_Mode) redirect_protected()  {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('wp_safe_redirect')]))))) {
		rt.include_file((rt.get_constant('ABSPATH')).str() + (rt.get_constant('WPINC')).str() + '/pluggable.php', '4')
	}
	mut var_scheme := rt.new_string(if rt.is_true(rt.call_function('is_ssl', []rt.PhpVal{})) { rt.new_string('https://') } else { rt.new_string('http://') })
	mut var_url := rt.new_string(rt.concat(rt.concat(var_scheme, rt.get_superglobal('_SERVER').array_get('HTTP_HOST')), rt.get_superglobal('_SERVER').array_get('REQUEST_URI')))
...
fn create_wp_recovery_mode_key_service() &Class_WP_Recovery_Mode_Key_Service {
...
fn create_wp_recovery_mode_link_service() &Class_WP_Recovery_Mode_Link_Service {
...
fn create_wp_recovery_mode_email_service() &Class_WP_Recovery_Mode_Email_Service {
...
fn (mut this Class_WP_Recovery_Mode) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'cookie_service' { this.cookie_service = val; return true }
		'key_service' { this.key_service = val; return true }
		'link_service' { this.link_service = val; return true }
		'email_service' { this.email_service = val; return true }
		'is_initialized' { this.is_initialized = (val).to_bool(); return true }
		'is_active' { this.is_active = (val).to_bool(); return true }
		'session_id' { this.session_id = val; return true }
...
pub fn init_wp_includes_class_wp_recovery_mode_php() {
}
