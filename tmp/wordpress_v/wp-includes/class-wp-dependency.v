...
fn (mut this Class__WP_Dependency) construct(var_args rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.deps.is_array()))))) {
...
fn (mut this Class__WP_Dependency) add_data(var_name rt.PhpVal, var_data rt.PhpVal) bool {
	...
	this.extra.array_set(var_name, var_data.dup())
	return true
}

fn (mut this Class__WP_Dependency) set_translations(var_domain rt.PhpVal, path string) bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_domain.dup().is_string()))))) {
		return false
	}
	this.textdomain = var_domain.dup()
	this.translations_path = rt.new_string(path).dup()
	return true
}
...
pub fn init_wp_includes_class_wp_dependency_php() {
}
