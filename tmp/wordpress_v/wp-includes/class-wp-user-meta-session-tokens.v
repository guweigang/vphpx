...
fn (mut this Class_WP_User_Meta_Session_Tokens) get_sessions() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_sessions.dup().is_array()))))) {
		return rt.new_array()
	}
	...
	return rt.call_function('array_filter', [var_sessions.dup(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WP_User_Meta_Session_Tokens', [
				'WP_Session_Tokens',
			], &this) },
...
fn (mut this Class_WP_User_Meta_Session_Tokens) prepare_session(var_session rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_session_mutated.dup().is_long())) {
		return rt.create_array([
			rt.ArrayItem{ key: 'expiration', val: var_session_mutated },
		])
	}
	return var_session_mutated.dup()
}
...
fn (mut this Class_WP_User_Meta_Session_Tokens) update_session(var_verifier rt.PhpVal, var_session rt.PhpVal) {
	...
	if rt.is_true(var_session_mutated) {
		var_sessions.array_set(var_verifier, var_session_mutated.dup())
	} else {
		var_sessions.array_unset(var_verifier)
	}
	this.update_sessions(var_sessions.dup())
}
...
fn create_wp_user_meta_session_tokens() &Class_WP_User_Meta_Session_Tokens {
...
pub fn init_wp_includes_class_wp_user_meta_session_tokens_php() {
}
