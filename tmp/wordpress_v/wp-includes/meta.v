...
fn add_metadata(var_meta_type rt.PhpVal, var_object_id_arg rt.PhpVal, var_meta_key_arg rt.PhpVal, var_meta_value_arg rt.PhpVal, unique bool) bool {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_meta_type))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(var_meta_key))))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_object_id.clone().is_long()
	...
	if rt.is_true(rt.new_bool(var_unique
...
fn update_metadata(var_meta_type rt.PhpVal, var_object_id_arg rt.PhpVal, var_meta_key_arg rt.PhpVal, var_meta_value_arg rt.PhpVal, prev_value string) bool {
	...
	if var_prev_value == '' {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.call_function('is_countable', [var_old_value.clone()]))
			&& var_old_value.clone().array_count() == 1))
		{
			if rt.is_true(rt.identical(var_old_value.array_get(0), var_meta_value)) {
				return false
			}
		}
	}
...
fn delete_metadata(var_meta_type rt.PhpVal, var_object_id_arg rt.PhpVal, var_meta_key_arg rt.PhpVal, meta_value string, delete_all bool) bool {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_meta_type))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(var_meta_key))))))
		|| rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_object_id.clone().is_long()
		|| var_object_id.clone().is_double()))))) && !var_delete_all))))
	{
		return false
	}
	var_object_id = rt.call_function('absint', [var_object_id.clone()])
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_object_id))))
		&& !var_delete_all))
	{
		return false
	}
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool('' != var_meta_value))
	...
	if var_delete_all {
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.new_bool('' != var_meta_value))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), rt.new_string(var_meta_value.str())))))))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), rt.new_string(var_meta_value.str())))))))
		{
...
fn get_metadata(var_meta_type rt.PhpVal, var_object_id rt.PhpVal, meta_key string, single bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_value.clone().is_null()))))) {
...
fn get_metadata_raw(var_meta_type rt.PhpVal, var_object_id_arg rt.PhpVal, meta_key string, single bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_meta_type))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_object_id.clone().is_long()
		|| var_object_id.clone().is_double())))))))
	{
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_check)))) {
		if rt.is_true(rt.new_bool(var_single
			&& rt.is_true(rt.new_bool(var_check.clone().is_array()))))
		{
			return var_check.array_get(0)
		} else {
	...
	if var_meta_cache.array_isset(rt.new_string(meta_key)) {
		if var_single {
			return rt.call_function('maybe_unserialize',
				[var_meta_cache.array_get(meta_key).array_get(0)])
		} else {
			return rt.call_function('array_map', [rt.new_string('maybe_unserialize'),
				var_meta_cache.array_get(meta_key)])
		}
	}
	return rt.new_null()
}
...
fn get_metadata_by_mid(var_meta_type rt.PhpVal, var_meta_id_arg rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_meta_type))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_meta_id.clone().is_long()
...
fn update_metadata_by_mid(var_meta_type rt.PhpVal, var_meta_id_arg rt.PhpVal, var_meta_value_arg rt.PhpVal, meta_key bool) bool {
	...
	if var_meta {
		...
		} else if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.new_bool(var_meta_key).is_string()))))) {
			return false
		}
		var_meta_subtype = get_object_subtype(var_meta_type.clone(), var_object_id.clone())
		var__meta_value = var_meta_value.clone()
...
fn delete_metadata_by_mid(var_meta_type rt.PhpVal, var_meta_id_arg rt.PhpVal) bool {
	...
	if var_meta {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('post'), var_meta_type))
...
fn update_meta_cache(var_meta_type rt.PhpVal, var_object_ids_arg rt.PhpVal) bool {
		...
		var_object_ids.clone(), var_cache_group.clone()])
	{
		...
		for {
			item_1 := iter_1.next() or { break }
			mut var_cached_object_shadow := item_1.val
	...
	if !(!rt.is_true(var_meta_list)) {
		{
			mut iter_1 := var_meta_list.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_metarow_shadow := item_1.val
				var_mpid = rt.new_int((var_metarow_shadow.array_get(var_column)).to_i64())
				var_mkey = var_metarow_shadow.array_get('meta_key')
				var_mval = var_metarow_shadow.array_get('meta_value')
				if rt.is_true(rt.new_bool(!(var_cache.array_isset(var_mpid))
					|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_cache.array_get(var_mpid).is_array())))))))
				{
					var_cache.array_set(var_mpid, rt.new_array())
				}
				if rt.is_true(rt.new_bool(!(var_cache.array_get(var_mpid).array_isset(var_mkey))
					|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_cache.array_get(var_mpid).array_get(var_mkey).is_array())))))))
				{
...
fn is_protected_meta(var_meta_key rt.PhpVal, meta_type string) rt.PhpVal {
	...
	var_protected = var_sanitized_key.clone().to_string().len > 0
		&& rt.is_true(rt.identical(rt.new_string('_'), var_sanitized_key.array_get(0)))
...
fn sanitize_meta(var_meta_key rt.PhpVal, var_meta_value rt.PhpVal, var_object_type rt.PhpVal, object_subtype string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(object_subtype == '')
...
fn register_meta(var_object_type rt.PhpVal, var_meta_key rt.PhpVal, var_args_arg rt.PhpVal, var_deprecated rt.PhpVal) bool {
	...
	mut var_check := rt.new_null()
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_wp_meta_keys.clone().is_array()))))) {
	...
	var_args = rt.call_function('wp_parse_args', [var_args.clone(),
		var_defaults.clone()])
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_args.array_get('show_in_rest')))))
		&& rt.is_true(rt.identical(rt.new_string('array'), var_args.array_get('type')))))
	{
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_args.array_get('show_in_rest').is_array())))))
	...
	var_object_subtype = if !(!rt.is_true(var_args.array_get('object_subtype'))) {
		var_args.array_get('object_subtype')
	} else {
		rt.new_string('')
	}
	if rt.is_true(var_args.array_get('revisions_enabled')) {
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('post'), var_object_type)))) {
			rt.call_function('_doing_it_wrong', [rt.new_string(@FN),
				...
				rt.new_string('6.4.0')])
			return false
		} else if rt.is_true(rt.new_bool(!(!rt.is_true(var_object_subtype))
	...
	if !rt.is_true(var_args.array_get('auth_callback')) {
	...
	if rt.is_true(rt.call_function('is_callable', [var_args.array_get('sanitize_callback')])) {
	...
	if rt.is_true(rt.new_bool(var_args.clone().array_isset(rt.new_string('default')))) {
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(var_args.array_get('show_in_rest').is_array()))
			&& var_args.array_get('show_in_rest').array_isset(rt.new_string('schema'))))
		{
			var_schema = rt.call_function('array_merge', [var_schema.clone(),
				var_args.array_get('show_in_rest').array_get('schema')])
		}
...
fn filter_default_metadata(var_value_arg rt.PhpVal, var_object_id rt.PhpVal, var_meta_key rt.PhpVal, var_single rt.PhpVal, var_meta_type rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_wp_meta_keys.clone().is_array())))))
		|| !(var_wp_meta_keys.array_isset(var_meta_type))))
	{
		return var_value.clone()
	}
	var_defaults = rt.new_array()
	{
		mut iter_1 := var_wp_meta_keys.array_get(var_meta_type).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_meta_data_shadow := item_1.val
			mut var_sub_type_shadow := item_1.key
			{
				mut iter_2 := var_meta_data_shadow.iterator()
				for {
					item_2 := iter_2.next() or { break }
					mut var_args_shadow := item_2.val
					mut var__meta_key_shadow := item_2.key
					if rt.is_true(rt.new_bool(
						rt.is_true(rt.identical(var__meta_key_shadow, var_meta_key))
	...
	if var_defaults.array_isset(rt.new_string('')) {
		var_metadata = var_defaults.array_get('')
	} else {
	...
	if rt.is_true(var_single) {
		var_value = var_metadata.array_get('default')
	} else {
		var_value = rt.create_array([
			rt.ArrayItem{ key: none, val: var_metadata.array_get('default') },
		])
	}
	return var_value.clone()
}
...
fn unregister_meta_key(var_object_type rt.PhpVal, var_meta_key rt.PhpVal, object_subtype string) bool {
	...
	var_args =
		var_wp_meta_keys.array_get(var_object_type).array_get(object_subtype).array_get(var_meta_key)
	if rt.is_true(rt.new_bool(var_args.array_isset(rt.new_string('sanitize_callback'))
		&& rt.is_true(rt.call_function('is_callable', [var_args.array_get('sanitize_callback')]))))
	{
		if !(object_subtype == '') {
	...
	if rt.is_true(rt.new_bool(var_args.array_isset(rt.new_string('auth_callback'))
		&& rt.is_true(rt.call_function('is_callable', [var_args.array_get('auth_callback')]))))
	{
		if !(object_subtype == '') {
			rt.call_function('remove_filter', [
	...
	var_wp_meta_keys.array_get(var_object_type).array_get(object_subtype).array_unset(var_meta_key)
	if !rt.is_true(var_wp_meta_keys.array_get(var_object_type).array_get(object_subtype)) {
...
fn get_registered_meta_keys(var_object_type rt.PhpVal, object_subtype string) rt.PhpVal {
	...
	mut var_wp_meta_keys := rt.new_null()
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_wp_meta_keys.clone().is_array())))))
		|| !(var_wp_meta_keys.array_isset(var_object_type))))
		|| !(var_wp_meta_keys.array_get(var_object_type).array_isset(rt.new_string(object_subtype)))))
	{
		return rt.new_array()
	}
	return var_wp_meta_keys.array_get(var_object_type).array_get(object_subtype)
}
...
fn get_registered_metadata(var_object_type rt.PhpVal, var_object_id rt.PhpVal, meta_key string) rt.PhpVal {
	...
	if !(var_meta_key == '') {
		if rt.is_true(rt.new_bool(!(!rt.is_true(var_object_subtype))
		...
		var_meta_key_data = var_meta_keys.array_get(var_meta_key)
		var_data = get_metadata(var_object_type.clone(), var_object_id.clone(), var_meta_key,
			var_meta_key_data.array_get('single'))
...
pub fn init_wp_includes_meta_php() {
	rt.include_file(
