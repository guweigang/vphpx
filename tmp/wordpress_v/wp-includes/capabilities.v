...
fn map_meta_cap(var_cap_arg rt.PhpVal, var_user_id rt.PhpVal, var_args_origin ...rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('remove_user'))) {
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_args.array_isset(rt.new_int(0))
			&& rt.is_true(rt.identical(var_user_id, rt.new_int((var_args.array_get(0)).to_i64())))))
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('edit_user'), var_cap))
			&& var_args.array_isset(rt.new_int(0))))
			&& rt.is_true(rt.identical(var_user_id, rt.new_int((var_args.array_get(0)).to_i64())))))
		{
		}
		if rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{}))
			&& rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(is_super_admin(var_user_id.clone()))
			&& rt.is_true(rt.identical(rt.new_string('edit_user'), var_cap))))
			&& is_super_admin(var_args.array_get(0))))
		...
		var_post = rt.call_function('get_post', [var_args.array_get(0)])
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.identical(rt.new_int((rt.call_function('get_option', [rt.new_string('page_for_posts')])).to_i64()), rt.get_property(var_post, 'ID')))
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.get_property(var_post, 'post_author'))
		...
		var_object_id = rt.new_int((var_args.array_get(0)).to_i64())
		...
		var_meta_key = if !(var_args.array_get(1)).is_null() {
			var_args.array_get(1)
		} else {
			rt.new_bool(false)
		}
		...
		var_comment = rt.call_function('get_comment', [var_args.array_get(0)])
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('ALLOW_UNFILTERED_UPLOADS')]))
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.call_function('defined', [rt.new_string('DISALLOW_UNFILTERED_HTML')]))
		...
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.call_function('defined', [rt.new_string('DISALLOW_FILE_EDIT')]))
		...
		if rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{})) {
			...
			if !rt.is_true(var_menu_perms.array_get('plugins')) {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{}))))) {
			var_caps.array_push(var_cap.clone())
		} else if rt.is_true(rt.new_bool(is_super_admin(var_user_id.clone())
		...
		var_term_id = rt.new_int((var_args.array_get(0)).to_i64())
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string('delete_term'), var_cap))
			&& rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_int((rt.call_function('get_option', [rt.new_string('default_' + (rt.get_property(var_term, 'taxonomy')).str())])).to_i64()), rt.get_property(var_term, 'term_id')))
		...
		var_block_editor_context = var_args.array_get(0)
		...
		if !(!var_object_id.is_null()) {
			if rt.is_true(rt.new_bool(
				!(!(rt.get_property(var_block_editor_context, 'name')).is_null())
...
fn user_can(var_user_arg rt.PhpVal, var_capability rt.PhpVal, var_args_origin ...rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_user.clone().is_object()))))) {
...
fn user_can_for_site(var_user_arg rt.PhpVal, var_site_id rt.PhpVal, var_capability rt.PhpVal, var_args_origin ...rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_site_id.clone().is_long()
		|| var_site_id.clone().is_double())))))
...
fn is_super_admin(user_id bool) bool {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_user))))
	...
	if rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{})) {
		...
		if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_super_admins.clone().is_array()))
...
fn grant_super_admin(var_user_id rt.PhpVal) bool {
	...
	mut var_super_admins := rt.new_null()
	mut var_user := rt.new_null()
	if rt.is_true(rt.new_bool(var_GLOBALS.array_isset(rt.new_string('super_admins'))
...
fn wp_maybe_grant_site_health_caps(var_allcaps rt.PhpVal, var_caps rt.PhpVal, var_args rt.PhpVal, var_user rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(!(var_allcaps['install_plugins']))
		&& rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{})))))
...
fn create_wp_roles() &Class_WP_Roles {
...
pub fn init_wp_includes_capabilities_php() {
	return rt.new_null()
