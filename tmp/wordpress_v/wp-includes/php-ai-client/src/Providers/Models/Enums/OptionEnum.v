	...
	mut var_modelConfigConstants := var_modelConfigReflection.getconstants()
	{
		...
		for {
			...
			if rt.is_true(rt.call_function('str_starts_with', [
				var_constantName.dup(), rt.new_string('KEY_')]))
			{
				mut var_enumConstantName := rt.call_function('substr', [
					var_constantName.dup(), rt.new_int(4)])
				if rt.is_true(rt.new_bool(var_constantValue.dup().is_string())) {
					var_constants.array_set(var_enumConstantName, var_constantValue.dup())
				}
			}
		}
	}
	return var_constants.dup()
}
...
pub fn init_wp_includes_php_ai_client_src_providers_models_enums_optionenum_php() {
