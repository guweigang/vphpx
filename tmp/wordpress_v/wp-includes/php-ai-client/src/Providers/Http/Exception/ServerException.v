...
fn Class_WordPress_AiClient_Providers_Http_Exception_ServerException.fromservererrorresponse(mut var_response Class_WordPress_AiClient_Providers_Http_DTO_Response) rt.PhpVal {
	...
	mut var_statusTexts := rt.create_array([rt.ArrayItem{ key: 500, val: 'Internal Server Error' }, rt.ArrayItem{ key: 502, val: 'Bad Gateway' }, rt.ArrayItem{ key: 503, val: 'Service Unavailable' }, rt.ArrayItem{ key: 504, val: 'Gateway Timeout' }, rt.ArrayItem{ key: 507, val: 'Insufficient Storage' }, rt.ArrayItem{ key: 529, val: 'Overloaded' }])
	if var_statusTexts.array_isset(var_statusCode) {
		...
		var_errorMessage = rt.call_function('sprintf', [rt.new_string('Server error (%d): Request was rejected due to server-side issue'), var_statusCode.dup()])
	}
	...
	return create_wordpress_aiclient_providers_http_exception_self(var_errorMessage.dup(), var_response.getstatuscode())
}
...
pub fn init_wp_includes_php_ai_client_src_providers_http_exception_serverexception_php() {
