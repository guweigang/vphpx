...
fn (mut this Class_WordPress_AiClient_Common_AbstractEnum) magic_get(property string) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_string(property), rt.new_string('value'))) || rt.is_true(rt.identical(rt.new_string(property), rt.new_string('name'))))) {
		return rt.get_property(rt.new_object('WordPress_AiClient_Common_AbstractEnum', ['JsonSerializable'], &this), '{"nodeType":"Expr_Variable","line":81,"name":"property"}')
	}
	rt.throw_exception(rt.new_object('BadMethodCallException', []string{}, create_badmethodcallexception(rt.call_function('sprintf', [rt.new_string('Property %s::%s does not exist'), Class_WordPress_AiClient_Common_static.class(), rt.new_string(property)]))))
	return rt.new_null()
}

fn (mut this Class_WordPress_AiClient_Common_AbstractEnum) magic_set(property string, var_value rt.PhpVal)  {
	rt.throw_exception(rt.new_object('BadMethodCallException', []string{}, create_badmethodcallexception(rt.call_function('sprintf', [rt.new_string('Cannot modify property %s::%s - enum properties are read-only'), Class_WordPress_AiClient_Common_static.class(), rt.new_string(property)]))))
}
...
fn Class_WordPress_AiClient_Common_AbstractEnum.from(value string) rt.PhpVal {
	...
	if rt.is_true(rt.identical(var_instance, rt.new_null())) {
		rt.throw_exception(rt.new_object('WordPress_AiClient_Common_Exception_InvalidArgumentException', []string{}, create_wordpress_aiclient_common_exception_invalidargumentexception(rt.call_function('sprintf', [rt.new_string('%s is not a valid backing value for enum %s'), rt.new_string(value), Class_WordPress_AiClient_Common_static.class()]))))
	}
	...
	mut var_constants := Class_WordPress_AiClient_Common_AbstractEnum.getconstants()
	{
		...
		for {
			...
			if rt.is_true(rt.identical(var_constantValue, rt.new_string(value))) {
				return Class_WordPress_AiClient_Common_AbstractEnum.getinstance((var_constantValue).str(), (var_name).str())
			}
		}
	}
	return rt.new_null()
...
fn Class_WordPress_AiClient_Common_AbstractEnum.cases() rt.PhpVal {
	...
	mut var_constants := Class_WordPress_AiClient_Common_AbstractEnum.getconstants()
	{
		...
		for {
			...
			mut var_name := item_1.key
			var_cases.array_push(Class_WordPress_AiClient_Common_AbstractEnum.getinstance((var_value).str(), (var_name).str()))
		}
	}
	return var_cases.dup()
}
...
fn (mut this Class_WordPress_AiClient_Common_AbstractEnum) is(mut var_other Class_WordPress_AiClient_Common_self) bool {
	return (rt.identical(rt.new_object('WordPress_AiClient_Common_AbstractEnum', ['JsonSerializable'], &this), var_other)).to_bool()
	// unsupported statement: Stmt_Nop
	return false
}

fn Class_WordPress_AiClient_Common_AbstractEnum.getvalues() rt.PhpVal {
	return rt.call_function('array_values', [Class_WordPress_AiClient_Common_AbstractEnum.getconstants()])
}

fn Class_WordPress_AiClient_Common_AbstractEnum.isvalidvalue(value string) bool {
	return (rt.call_function('in_array', [rt.new_string(value), Class_WordPress_AiClient_Common_AbstractEnum.getvalues(), rt.new_bool(true)])).to_bool()
}
...
fn Class_WordPress_AiClient_Common_AbstractEnum.getinstance(value string, name string) rt.PhpVal {
	...
	if !(// unsupported expression: Expr_StaticPropertyFetch.array_isset(var_className)) {
		// unsupported expression: Expr_StaticPropertyFetch.array_set(var_className, rt.new_array())
	}
	if !(// unsupported expression: Expr_StaticPropertyFetch.array_get(var_className).array_isset(rt.new_string(name))) {
		mut var_instance := rt.create_object_dynamically(var_className, [rt.new_string(value), rt.new_string(name)])
		// unsupported expression: Expr_StaticPropertyFetch.array_get_mut(var_className).array_set(name, var_instance.dup())
	}
	return // unsupported expression: Expr_StaticPropertyFetch.array_get(var_className).array_get(name)
}

fn Class_WordPress_AiClient_Common_AbstractEnum.getconstants() rt.PhpVal {
	mut var_className := Class_WordPress_AiClient_Common_static.class()
	if !(// unsupported expression: Expr_StaticPropertyFetch.array_isset(var_className)) {
		// unsupported expression: Expr_StaticPropertyFetch.array_set(var_className, Class_WordPress_AiClient_Common_AbstractEnum.determineclassenumerations((var_className).str()))
	}
	return // unsupported expression: Expr_StaticPropertyFetch.array_get(var_className)
}
...
fn Class_WordPress_AiClient_Common_AbstractEnum.determineclassenumerations(className string) rt.PhpVal {
	...
	mut var_reflection := create_reflectionclass(rt.new_string(className_mutated).dup())
	...
	mut var_enumConstants := rt.new_array()
	{
		...
		for {
			...
			mut var_value := item_1.val
			mut var_name := item_1.key
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('/^[A-Z][A-Z0-9_]*$/'), var_name.dup()]))))) {
				rt.throw_exception(rt.new_object('WordPress_AiClient_Common_Exception_RuntimeException', []string{}, create_wordpress_aiclient_common_exception_runtimeexception(rt.call_function('sprintf', [rt.new_string('Invalid enum constant name "%s" in %s. Constants must be UPPER_SNAKE_CASE.'), var_name.dup(), rt.new_string(className_mutated).dup()]))))
			}
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_value.dup().is_string()))))) {
				rt.throw_exception(rt.new_object('WordPress_AiClient_Common_Exception_RuntimeException', []string{}, create_wordpress_aiclient_common_exception_runtimeexception(rt.call_function('sprintf', ['Invalid enum value type for constant %s::%s. ' + 'Only string values are allowed, %s given.', rt.new_string(className_mutated).dup(), var_name.dup(), rt.call_function('gettype', [var_value.dup()])]))))
			}
			var_enumConstants.array_set(var_name, var_value.dup())
		}
	}
	return var_enumConstants.dup()
}
...
fn (mut this Class_WordPress_AiClient_Common_AbstractEnum) magic_call(name string, mut var_arguments Class_WordPress_AiClient_Common_array) bool {
	if rt.is_true(rt.call_function('str_starts_with', [rt.new_string(name), rt.new_string('is')])) {
		mut var_constantName := Class_WordPress_AiClient_Common_AbstractEnum.camelcasetoconstant((rt.call_function('substr', [rt.new_string(name), rt.new_int(2)])).str())
		mut var_constants := Class_WordPress_AiClient_Common_AbstractEnum.getconstants()
	...
	rt.throw_exception(rt.new_object('BadMethodCallException', []string{}, create_badmethodcallexception(rt.call_function('sprintf', [rt.new_string('Method %s::%s does not exist'), Class_WordPress_AiClient_Common_static.class(), rt.new_string(name)]))))
	return false
}
...
fn Class_WordPress_AiClient_Common_AbstractEnum.magic_callstatic(name string, mut var_arguments Class_WordPress_AiClient_Common_array) rt.PhpVal {
	...
	mut var_constants := Class_WordPress_AiClient_Common_AbstractEnum.getconstants()
	if var_constants.array_isset(var_constantName) {
		return Class_WordPress_AiClient_Common_AbstractEnum.getinstance((var_constants.array_get(var_constantName)).str(), (var_constantName).str())
	}
	rt.throw_exception(rt.new_object('BadMethodCallException', []string{}, create_badmethodcallexception(rt.call_function('sprintf', [rt.new_string('Method %s::%s does not exist'), Class_WordPress_AiClient_Common_static.class(), rt.new_string(name)]))))
	return rt.new_null()
}
...
fn Class_WordPress_AiClient_Common_AbstractEnum.camelcasetoconstant(camelCase string) string {
	mut var_snakeCase := rt.call_function('preg_replace', [rt.new_string('/([a-z])([A-Z])/'), rt.new_string('$1_$2'), rt.new_string(camelCase)])
	if rt.is_true(rt.identical(var_snakeCase, rt.new_null())) {
		return camelCase.to_upper()
	}
	return var_snakeCase.dup().to_string().to_upper()
}
...
fn (mut this Class_WordPress_AiClient_Common_AbstractEnum) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'is' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WordPress_AiClient_Common_self](if args.len > 0 { args[0] } else { rt.new_null() })
		...
		'getInstance' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).str()
			return Class_WordPress_AiClient_Common_AbstractEnum.getinstance(dispatch_arg_0, dispatch_arg_1)
		}
		'getConstants' {
		...
		'__call' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_WordPress_AiClient_Common_array](if args.len > 1 { args[1] } else { rt.new_null() })
			return rt.new_bool(this.magic_call(dispatch_arg_0, mut dispatch_arg_1))
		}
		'__callStatic' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_WordPress_AiClient_Common_array](if args.len > 1 { args[1] } else { rt.new_null() })
			return Class_WordPress_AiClient_Common_AbstractEnum.magic_callstatic(dispatch_arg_0, mut dispatch_arg_1)
		}
		'camelCaseToConstant' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
...
fn init_registry() {
	rt.register_class_factory('WordPress_AiClient_Common_AbstractEnum', fn(args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_object('WordPress_AiClient_Common_AbstractEnum', ['JsonSerializable'], obj)
	})
	...
	rt.register_class_factory('WordPress_AiClient_Common_Exception_InvalidArgumentException', fn(args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_object('WordPress_AiClient_Common_Exception_InvalidArgumentException', []string{}, obj)
	})
	...
	rt.register_class_factory('WordPress_AiClient_Common_Exception_RuntimeException', fn(args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_object('WordPress_AiClient_Common_Exception_RuntimeException', []string{}, obj)
	})
}
...
pub fn init_wp_includes_php_ai_client_src_common_abstractenum_php() {
