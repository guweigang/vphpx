...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Response) construct(status i64, mut var_headers Class_WordPress_AiClientDependencies_Nyholm_Psr7_array, var_body rt.PhpVal, version string, mut var_reason Class_WordPress_AiClientDependencies_Nyholm_Psr7_?string)  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.identical(rt.new_null(), var_reason)) && Class_WordPress_AiClientDependencies_Nyholm_Psr7_WordPress_AiClientDependencies_Nyholm_Psr7_Response.phrases().array_isset(this.statusCode))) {
		this.reasonPhrase = Class_WordPress_AiClientDependencies_Nyholm_Psr7_WordPress_AiClientDependencies_Nyholm_Psr7_Response.phrases().array_get(status)
	} else {
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Response) withstatus(var_code rt.PhpVal, reasonPhrase string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_code_mutated.dup().is_long()))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_code_mutated.dup().is_string()))))))) {
	...
	var_code_mutated = // unsupported expression: Expr_Cast_Int
	if rt.is_true(rt.new_bool(rt.is_true(rt.less(var_code_mutated, rt.new_int(100))) || rt.is_true(rt.greater(var_code_mutated, rt.new_int(599))))) {
		rt.throw_exception(rt.new_object('WordPress_AiClientDependencies_Nyholm_Psr7_InvalidArgumentException', []string{}, create_wordpress_aiclientdependencies_nyholm_psr7_invalidargumentexception(rt.call_function('sprintf', [rt.new_string('Status code has to be an integer between 100 and 599. A status code of %d was given'), var_code_mutated.dup()]))))
	}
	mut var_new := // unsupported expression: Expr_Clone
	rt.set_property(var_new, 'statusCode', var_code_mutated.dup())
	...
	rt.set_property(var_new, 'reasonPhrase', rt.new_string(reasonPhrase_mutated).dup())
	return var_new.dup()
}
...
pub fn init_wp_includes_php_ai_client_third_party_nyholm_psr7_response_php() {
