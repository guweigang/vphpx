...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Factory_HttplugFactory) createrequest(var_method rt.PhpVal, var_uri rt.PhpVal, mut var_headers Class_WordPress_AiClientDependencies_Nyholm_Psr7_Factory_array, var_body rt.PhpVal, protocolVersion string) rt.PhpVal {
	return create_wordpress_aiclientdependencies_nyholm_psr7_request(var_method.dup(), var_uri.dup(), var_headers.dup(), var_body.dup(), rt.new_string(protocolVersion).dup())
}
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Factory_HttplugFactory) createresponse(statusCode i64, var_reasonPhrase rt.PhpVal, mut var_headers Class_WordPress_AiClientDependencies_Nyholm_Psr7_Factory_array, var_body rt.PhpVal, version string) rt.PhpVal {
	return create_wordpress_aiclientdependencies_nyholm_psr7_response(// unsupported expression: Expr_Cast_Int, var_headers.dup(), var_body.dup(), rt.new_string(version).dup(), var_reasonPhrase.dup())
}
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Factory_HttplugFactory) createstream(var_body rt.PhpVal) rt.PhpVal {
	return fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream{}; return temp.create(arg_0) }(if !(var_body).is_null() { var_body } else { rt.new_string('') })
}

fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Factory_HttplugFactory) createuri(uri string) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.instance_of(rt.new_string(uri), 'WordPress_AiClientDependencies_Psr_Http_Message_UriInterface'))) {
...
fn create_wordpress_aiclientdependencies_nyholm_psr7_factory_httplugfactory() &Class_WordPress_AiClientDependencies_Nyholm_Psr7_Factory_HttplugFactory {
...
fn create_wordpress_aiclientdependencies_nyholm_psr7_factory_logicexception() &Class_WordPress_AiClientDependencies_Nyholm_Psr7_Factory_LogicException {
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Factory_HttplugFactory) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'createRequest' {
			...
			return this.createrequest(dispatch_arg_0, dispatch_arg_1, mut dispatch_arg_2, dispatch_arg_3, dispatch_arg_4)
		}
		'createResponse' {
			...
			dispatch_arg_3 := if args.len > 3 { args[3] } else { rt.new_null() }
			dispatch_arg_4 := (if args.len > 4 { args[4] } else { rt.new_null() }).str()
			return this.createresponse(dispatch_arg_0, dispatch_arg_1, mut dispatch_arg_2, dispatch_arg_3, dispatch_arg_4)
		}
		'createStream' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
...
pub fn init_wp_includes_php_ai_client_third_party_nyholm_psr7_factory_httplugfactory_php() {
	// unsupported statement: Stmt_Declare
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('interface_exists', [Class_WordPress_AiClientDependencies_Http_Message_MessageFactory.class()]))))) {
		rt.throw_exception(rt.new_object('WordPress_AiClientDependencies_Nyholm_Psr7_Factory_LogicException', []string{}, create_wordpress_aiclientdependencies_nyholm_psr7_factory_logicexception(rt.new_string('You cannot use "Nyholm\\Psr7\\Factory\\HttplugFactory" as the "php-http/message-factory" package is not installed. Try running "composer require php-http/message-factory". Note that this package is deprecated, use "psr/http-factory" instead'))))
	}
	rt.call_function('trigger_error', [rt.new_string('Class "Nyholm\\Psr7\\Factory\\HttplugFactory" is deprecated since version 1.8, use "Nyholm\\Psr7\\Factory\\Psr17Factory" instead.'), rt.get_constant('E_USER_DEPRECATED')])
}
