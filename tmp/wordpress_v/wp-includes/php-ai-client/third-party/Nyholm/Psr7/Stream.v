...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream) construct(var_body rt.PhpVal)  {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_resource', [var_body_mutated.dup()]))))) {
	...
	this.stream = var_body_mutated.dup()
	mut var_meta := rt.call_function('stream_get_meta_data', [this.stream])
	this.seekable = rt.is_true(var_meta.array_get('seekable')) && rt.is_true(rt.identical(rt.new_int(0), rt.call_function('fseek', [this.stream, rt.new_int(0), rt.get_constant('SEEK_CUR')])))
	this.readable = rt.new_bool(Class_WordPress_AiClientDependencies_Nyholm_Psr7_WordPress_AiClientDependencies_Nyholm_Psr7_Stream.read_write_hash().array_get('read').array_isset(var_meta.array_get('mode')))
	this.writable = rt.new_bool(Class_WordPress_AiClientDependencies_Nyholm_Psr7_WordPress_AiClientDependencies_Nyholm_Psr7_Stream.read_write_hash().array_get('write').array_isset(var_meta.array_get('mode')))
}
...
fn Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream.create(body string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.new_string(body_mutated).dup().is_string())) {
		if 200000 <= body_mutated.len {
			...
			rt.call_function('fwrite', [var_resource.dup(), rt.new_string(body_mutated).dup()])
			rt.call_function('fseek', [var_resource.dup(), rt.new_int(0)])
			body_mutated = (var_resource).str()
		}
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_resource', [rt.new_string(body_mutated).dup()]))))) {
	...
	return create_wordpress_aiclientdependencies_nyholm_psr7_self(rt.new_string(body_mutated).dup())
}
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream) getsize() i64 {
	...
	if rt.is_true(mut var_uri := this.geturi()) {
		rt.call_function('clearstatcache', [rt.new_bool(true), var_uri.dup()])
	}
	mut var_stats := rt.call_function('fstat', [this.stream])
	if var_stats.array_isset(rt.new_string('size')) {
		this.size = var_stats.array_get('size')
		return (this.size).to_i64()
	}
	return (rt.new_null()).to_i64()
}
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream) tell() i64 {
	...
	if rt.is_true(rt.identical(rt.new_bool(false), mut var_result := rt.call_function('ftell', [this.stream]))) {
		rt.throw_exception(rt.new_object('WordPress_AiClientDependencies_Nyholm_Psr7_RuntimeException', []string{}, create_wordpress_aiclientdependencies_nyholm_psr7_runtimeexception('Unable to determine stream position: ' + (if !(rt.call_function('error_get_last', []rt.PhpVal{}).array_get('message')).is_null() { rt.call_function('error_get_last', []rt.PhpVal{}).array_get('message') } else { rt.new_string('') }).str())))
	}
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream) seek(var_offset rt.PhpVal, var_whence rt.PhpVal)  {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(this.seekable)))) {
	...
	if rt.is_true(rt.identical(// unsupported expression: Expr_UnaryMinus, rt.call_function('fseek', [this.stream, var_offset.dup(), var_whence.dup()]))) {
		rt.throw_exception(rt.new_object('WordPress_AiClientDependencies_Nyholm_Psr7_RuntimeException', []string{}, create_wordpress_aiclientdependencies_nyholm_psr7_runtimeexception('Unable to seek to stream position "' + (var_offset).str() + '" with whence ' + (rt.call_function('var_export', [var_whence.dup(), rt.new_bool(true)])).str())))
	}
}
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream) write(var_string rt.PhpVal) i64 {
	...
	if rt.is_true(rt.identical(rt.new_bool(false), mut var_result := rt.call_function('fwrite', [this.stream, var_string.dup()]))) {
		rt.throw_exception(rt.new_object('WordPress_AiClientDependencies_Nyholm_Psr7_RuntimeException', []string{}, create_wordpress_aiclientdependencies_nyholm_psr7_runtimeexception('Unable to write to stream: ' + (if !(rt.call_function('error_get_last', []rt.PhpVal{}).array_get('message')).is_null() { rt.call_function('error_get_last', []rt.PhpVal{}).array_get('message') } else { rt.new_string('') }).str())))
	}
	return (var_result).to_i64()
}
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream) read(var_length rt.PhpVal) string {
	...
	if rt.is_true(rt.identical(rt.new_bool(false), mut var_result := rt.call_function('fread', [this.stream, var_length.dup()]))) {
		rt.throw_exception(rt.new_object('WordPress_AiClientDependencies_Nyholm_Psr7_RuntimeException', []string{}, create_wordpress_aiclientdependencies_nyholm_psr7_runtimeexception('Unable to read from stream: ' + (if !(rt.call_function('error_get_last', []rt.PhpVal{}).array_get('message')).is_null() { rt.call_function('error_get_last', []rt.PhpVal{}).array_get('message') } else { rt.new_string('') }).str())))
	}
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream) getcontents() string {
	...
	rt.throw_exception(var_exception = create_wordpress_aiclientdependencies_nyholm_psr7_runtimeexception('Unable to read stream contents: ' + (var_message).str()))
	return rt.new_null()
	}
	mut var_type := if args.len > 0 { args[0].dup() } else { rt.new_null() }
	mut var_message := if args.len > 1 { args[1].dup() } else { rt.new_null() }
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'WordPress_AiClientDependencies_Nyholm_Psr7_Throwable') {
		...
		rt.throw_exception(if rt.is_true(rt.identical(var_e, var_exception)) { var_e } else { create_wordpress_aiclientdependencies_nyholm_psr7_runtimeexception('Unable to read stream contents: ' + (rt.call_method(var_e, 'getMessage', []rt.PhpVal{})).str(), rt.new_int(0), var_e.dup()) })
		unsafe { goto finally_label_1 }
	}
	else {
...
fn (mut this Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream) getmetadata(var_key rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_key.dup().is_string()))))))) {
...
fn Class_WordPress_AiClientDependencies_Nyholm_Psr7_Stream.openzvalstream(body string) rt.PhpVal {
	...
	if !(var_wrapper).is_null() { var_wrapper } else { rt.call_function('stream_wrapper_register', [rt.new_string('Nyholm-Psr7-Zval'), mut var_wrapper := rt.call_function('get_class', [rt.create_object_dynamically(// unsupported expression: Stmt_Class, []rt.PhpVal{})])]) }
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(mut var_stream := rt.call_function('fopen', [rt.new_string('Nyholm-Psr7-Zval://'), rt.new_string('r+'), rt.new_bool(false), var_context.dup()]))))) {
		rt.call_function('stream_wrapper_register', [rt.new_string('Nyholm-Psr7-Zval'), var_wrapper.dup()])
		var_stream = rt.call_function('fopen', [rt.new_string('Nyholm-Psr7-Zval://'), rt.new_string('r+'), rt.new_bool(false), var_context.dup()])
	}
	return var_stream.dup()
}
...
fn create_wordpress_aiclientdependencies_nyholm_psr7_self() &Class_WordPress_AiClientDependencies_Nyholm_Psr7_self {
...
pub fn init_wp_includes_php_ai_client_third_party_nyholm_psr7_stream_php() {
