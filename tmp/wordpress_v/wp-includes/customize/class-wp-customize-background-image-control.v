...
fn (mut this Class_WP_Customize_Background_Image_Control) enqueue() {
	...
	rt.call_function('wp_localize_script', [rt.new_string('customize-controls'),
		...
		rt.create_array([
			rt.ArrayItem{
				key: 'defaults'
				val: if !(!rt.is_true(var_custom_background.array_get(0))) {
					var_custom_background.array_get(0)
				} else {
					rt.new_array()
				}
			},
...
pub fn init_wp_includes_customize_class_wp_customize_background_image_control_php() {
}
