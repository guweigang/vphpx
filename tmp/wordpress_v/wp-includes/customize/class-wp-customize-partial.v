...
fn (mut this Class_WP_Customize_Partial) construct(mut var_component Class_WP_Customize_Selective_Refresh, var_id rt.PhpVal, var_args rt.PhpVal)  {
	mut var_keys := rt.func_array_keys(rt.call_function('get_object_vars', [rt.new_object('WP_Customize_Partial', []string{}, &this)]))
	{
		...
		for {
			...
			if var_args.array_isset(var_key) {
				this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":165,"name":"key"}', var_args.array_get(var_key))
			}
		}
	}
	this.component = var_component.dup()
	this.id = var_id.dup()
	...
	if !rt.is_true(this.render_callback) {
		this.render_callback = rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WP_Customize_Partial', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'render_callback' }])
	}
...
fn (mut this Class_WP_Customize_Partial) render(var_container_context rt.PhpVal) rt.PhpVal {
	mut var_partial := rt.new_object('WP_Customize_Partial', []string{}, &this).dup()
	mut var_rendered := rt.new_bool(rt.new_bool(false))
	if !(!rt.is_true(this.render_callback)) {
		rt.call_function('ob_start', []rt.PhpVal{})
		mut var_return_render := rt.call_function('call_user_func', [this.render_callback, rt.new_object('WP_Customize_Partial', []string{}, &this), var_container_context.dup()])
		...
		if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))) {
			rt.call_function('_doing_it_wrong', [rt.new_string(@FN), rt.call_function('__', [rt.new_string('Partial render must echo the content or return the content string (or array), but not both.')]), rt.new_string('4.5.0')])
		}
		var_rendered = if !(var_return_render).is_null() { var_return_render } else { var_ob_render }
	}
	var_rendered = rt.call_function('apply_filters', [rt.new_string('customize_partial_render'), var_rendered.dup(), var_partial.dup(), var_container_context.dup()])
	var_rendered = rt.call_function('apply_filters', [rt.concat(rt.new_string('customize_partial_render_'), rt.get_property(var_partial, 'id')), var_rendered.dup(), var_partial.dup(), var_container_context.dup()])
	return var_rendered.dup()
}
...
fn (mut this Class_WP_Customize_Partial) json() rt.PhpVal {
	mut var_exports := { 'settings': this.settings, 'primarySetting': this.primary_setting, 'selector': this.selector, 'type': this.prop_type, 'fallbackRefresh': this.fallback_refresh, 'containerInclusive': this.container_inclusive }
	return var_exports.dup()
}

fn (mut this Class_WP_Customize_Partial) check_capabilities() bool {
	if rt.is_true(rt.new_bool(!(!rt.is_true(this.capability)) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [this.capability]))))))) {
		return false
	}
	{
		...
		for {
			...
			mut var_setting := rt.call_method(rt.get_property(this.component, 'manager'), 'get_setting', [var_setting_id.dup()])
			if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_setting)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_setting, 'check_capabilities', []rt.PhpVal{}))))))) {
				return false
			}
		}
	}
...
fn (mut this Class_WP_Customize_Partial) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WP_Customize_Selective_Refresh](if args.len > 0 { args[0] } else { rt.new_null() })
		...
		'render_callback' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WP_Customize_Partial](if args.len > 0 { args[0] } else { rt.new_null() })
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
...
fn (mut this Class_WP_Customize_Partial) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'component' { this.component = val; return true }
		...
		'primary_setting' { this.primary_setting = val; return true }
		'capability' { this.capability = val; return true }
		'render_callback' { this.render_callback = val; return true }
		'container_inclusive' { this.container_inclusive = val; return true }
		'fallback_refresh' { this.fallback_refresh = val; return true }
...
pub fn init_wp_includes_customize_class_wp_customize_partial_php() {
}
