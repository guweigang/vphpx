		...
		rt.new_string('menus')]))
	{
		...
		// unsupported statement: Stmt_InlineHTML
		{
			...
			for {
				...
				// unsupported statement: Stmt_InlineHTML
				rt.echo_val(rt.call_function('esc_attr', [var_location.dup()]))
				// unsupported statement: Stmt_InlineHTML
				...
				rt.call_function('printf', [
					...
					'<span class="current-menu-location-name-' +
						(rt.call_function('esc_attr', [var_location.dup()])).str() + '"></span>',
				])
				// unsupported statement: Stmt_InlineHTML
			}
		}
...
fn create_wp_customize_nav_menu_locations_control() &Class_WP_Customize_Nav_Menu_Locations_Control {
...
pub fn init_wp_includes_customize_class_wp_customize_nav_menu_locations_control_php() {
}
