...
fn (mut this Class_WP_Customize_Header_Image_Control) construct(var_manager rt.PhpVal) {
	this.Class_WP_Customize_Image_Control.construct(var_manager.dup(),
...
fn (mut this Class_WP_Customize_Header_Image_Control) render_content() {
	mut var_visibility := rt.new_string(if rt.is_true(this.get_current_image_src()) {
		...
		rt.new_string(' style="display:none" ')
	})
	...
	} else if rt.is_true(rt.new_bool(rt.is_true(var_width) && rt.is_true(var_height))) {
...
fn create_wp_customize_image_control() &Class_WP_Customize_Image_Control {
...
pub fn init_wp_includes_customize_class_wp_customize_header_image_control_php() {
}
