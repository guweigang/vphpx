...
fn (mut this Class_WP_Customize_Custom_CSS_Setting) construct(var_manager rt.PhpVal, var_id rt.PhpVal, var_args rt.PhpVal)  {
	this.Class_WP_Customize_Setting.construct(var_manager.dup(), var_id.dup(), var_args.dup())
	if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.new_string('Expected custom_css id_base.'))))
	}
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) || !rt.is_true(rt.get_property(rt.new_object('WP_Customize_Custom_CSS_Setting', ['WP_Customize_Setting'], &this), 'id_data').array_get('keys').array_get(0)))) {
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.new_string('Expected single stylesheet key.'))))
	}
	this.stylesheet = rt.get_property(rt.new_object('WP_Customize_Custom_CSS_Setting', ['WP_Customize_Setting'], &this), 'id_data').array_get('keys').array_get(0)
}

fn (mut this Class_WP_Customize_Custom_CSS_Setting) preview() bool {
	if rt.is_true(rt.get_property(rt.new_object('WP_Customize_Custom_CSS_Setting', ['WP_Customize_Setting'], &this), 'is_previewed')) {
		return false
	}
	this.dispatch_set_prop('is_previewed', rt.new_bool(true))
	rt.call_function('add_filter', [rt.new_string('wp_get_custom_css'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WP_Customize_Custom_CSS_Setting', ['WP_Customize_Setting'], &this) }, rt.ArrayItem{ key: none, val: 'filter_previewed_wp_get_custom_css' }]), rt.new_int(9), rt.new_int(2)])
	return true
}
...
fn (mut this Class_WP_Customize_Custom_CSS_Setting) filter_previewed_wp_get_custom_css(var_css rt.PhpVal, var_stylesheet rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(var_stylesheet, this.stylesheet)) {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_customized_value.dup().is_null()))))) {
			var_css_mutated = var_customized_value.dup()
		}
	}
	return var_css_mutated.dup()
}
...
fn (mut this Class_WP_Customize_Custom_CSS_Setting) value() rt.PhpVal {
	if rt.is_true(rt.get_property(rt.new_object('WP_Customize_Custom_CSS_Setting', ['WP_Customize_Setting'], &this), 'is_previewed')) {
		...
		if rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) {
			return var_post_value.dup()
		}
	}
	mut var_id_base := rt.get_property(rt.new_object('WP_Customize_Custom_CSS_Setting', ['WP_Customize_Setting'], &this), 'id_data').array_get('base')
	...
	if !rt.is_true(var_value) {
		var_value = rt.get_property(rt.new_object('WP_Customize_Custom_CSS_Setting', ['WP_Customize_Setting'], &this), 'default')
	}
	var_value = rt.call_function('apply_filters', [rt.new_string("customize_value_${var_id_base.to_string()}"), var_value.dup(), rt.new_object('WP_Customize_Custom_CSS_Setting', ['WP_Customize_Setting'], &this)])
	return var_value.dup()
}

fn (mut this Class_WP_Customize_Custom_CSS_Setting) validate(var_value rt.PhpVal) rt.PhpVal {
	mut var_value_mutated := var_value
	mut var_css := var_value_mutated.dup()
	mut var_validity := create_wp_error()
	mut var_length := rt.new_int(rt.new_int(var_css.dup().to_string().len))
	{
		mut var_at := rt.call_function('strcspn', [var_css.dup(), rt.new_string('<')])
		for {
			if !(rt.is_true(rt.less(var_at, var_length))) { break }
			mut var_remaining_strlen := rt.sub(var_length, var_at)
			mut var_possible_style_close_tag := rt.identical(rt.new_int(0), rt.call_function('substr_compare', [var_css.dup(), rt.new_string('</style'), var_at.dup(), rt.call_function('min', [rt.new_int(7), var_remaining_strlen.dup()]), rt.new_bool(true)]))
			if rt.is_true(var_possible_style_close_tag) {
				if rt.is_true(rt.less(var_remaining_strlen, rt.new_int(8))) {
					rt.call_method(var_validity, 'add', [rt.new_string('illegal_markup'), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('The CSS must not end in "%s".')]), rt.call_function('esc_html', [rt.call_function('substr', [var_css.dup(), var_at.dup()])])])])
					break
				}
				if rt.is_true(rt.identical(rt.new_int(1), rt.call_function('strspn', [var_css.dup(), rt.new_string(' \t\r\n/>'), rt.add(var_at, rt.new_int(7)), rt.new_int(1)]))) {
					rt.call_method(var_validity, 'add', [rt.new_string('illegal_markup'), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('The CSS must not contain "%s".')]), rt.call_function('esc_html', [rt.call_function('substr', [var_css.dup(), var_at.dup(), rt.new_int(8)])])])])
					break
				}
			}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_validity, 'has_errors', []rt.PhpVal{}))))) {
		var_validity = this.Class_WP_Customize_Setting.validate(var_css.dup())
	}
...
fn (mut this Class_WP_Customize_Custom_CSS_Setting) update(var_value rt.PhpVal) bool {
	...
	if !rt.is_true(var_css) {
		var_css = rt.new_string(rt.new_string(''))
	}
	mut var_r := rt.call_function('wp_update_custom_css_post', [var_css.dup(), rt.create_array([rt.ArrayItem{ key: 'stylesheet', val: this.stylesheet }])])
	if rt.is_true(rt.call_function('is_wp_error', [var_r.dup()])) {
		return false
	}
	mut var_post_id := rt.get_property(var_r, 'ID')
	if rt.is_true(rt.identical(rt.call_method(rt.get_property(rt.new_object('WP_Customize_Custom_CSS_Setting', ['WP_Customize_Setting'], &this), 'manager'), 'get_stylesheet', []rt.PhpVal{}), this.stylesheet)) {
		rt.call_function('set_theme_mod', [rt.new_string('custom_css_post_id'), var_post_id.dup()])
	}
...
pub fn init_wp_includes_customize_class_wp_customize_custom_css_setting_php() {
}
