...
fn (mut this Class_WP_Taxonomy) construct(var_taxonomy rt.PhpVal, var_object_type rt.PhpVal, var_args rt.PhpVal)  {
	...
	this.name = var_taxonomy_mutated.dup()
	this.set_props(var_object_type.dup(), var_args_mutated.dup())
}
...
fn (mut this Class_WP_Taxonomy) set_props(var_object_type rt.PhpVal, var_args rt.PhpVal)  {
	...
	var_args_mutated = rt.call_function('apply_filters', [rt.new_string('register_taxonomy_args'), var_args_mutated.dup(), this.name, rt.cast_array(var_object_type)])
	mut var_taxonomy := this.name
	var_args_mutated = rt.call_function('apply_filters', [rt.new_string("register_${var_taxonomy.to_string()}_taxonomy_args"), var_args_mutated.dup(), this.name, rt.cast_array(var_object_type)])
	mut var_defaults := { 'labels': map[string]rt.PhpVal{}, 'description': rt.new_string(''), 'public': rt.new_bool(true), 'publicly_queryable': rt.new_null(), 'hierarchical': rt.new_bool(false), 'show_ui': rt.new_null(), 'show_in_menu': rt.new_null(), 'show_in_nav_menus': rt.new_null(), 'show_tagcloud': rt.new_null(), 'show_in_quick_edit': rt.new_null(), 'show_admin_column': rt.new_bool(false), 'meta_box_cb': rt.new_null(), 'meta_box_sanitize_cb': rt.new_null(), 'capabilities': map[string]rt.PhpVal{}, 'rewrite': rt.new_bool(true), 'query_var': this.name, 'update_count_callback': rt.new_string(''), 'show_in_rest': rt.new_bool(false), 'rest_base': rt.new_bool(false), 'rest_namespace': rt.new_bool(false), 'rest_controller_class': rt.new_bool(false), 'default_term': rt.new_null(), 'sort': rt.new_null(), 'args': rt.new_null(), '_builtin': rt.new_bool(false) }
	...
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) || rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical))))) {
	...
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) || rt.is_true(rt.call_function('get_option', [rt.new_string('permalink_structure')])))))) {
		var_args_mutated.array_set('rewrite', rt.call_function('wp_parse_args', [var_args_mutated.array_get('rewrite'), rt.create_array([rt.ArrayItem{ key: 'with_front', val: true }, rt.ArrayItem{ key: 'hierarchical', val: false }, rt.ArrayItem{ key: 'ep_mask', val: rt.get_constant('EP_NONE') }])]))
		if !rt.is_true(var_args_mutated.array_get('rewrite').array_get('slug')) {
			var_args_mutated.array_get_mut('rewrite').array_set('slug', rt.call_function('sanitize_title_with_dashes', [this.name]))
		}
	}
	...
	if rt.is_true(rt.identical(rt.new_null(), var_args_mutated.array_get('show_tagcloud'))) {
		var_args_mutated.array_set('show_tagcloud', var_args_mutated.array_get('show_ui'))
	}
	if rt.is_true(rt.identical(rt.new_null(), var_args_mutated.array_get('show_in_quick_edit'))) {
		var_args_mutated.array_set('show_in_quick_edit', var_args_mutated.array_get('show_ui'))
	}
	...
	mut var_default_caps := { 'manage_terms': 'manage_categories', 'edit_terms': 'manage_categories', 'delete_terms': 'manage_categories', 'assign_terms': 'edit_posts' }
	var_args_mutated.array_set('cap', // unsupported expression: Expr_Cast_Object)
	var_args_mutated.array_unset(rt.new_string('capabilities'))
	var_args_mutated.array_set('object_type', rt.call_function('array_unique', [rt.cast_array(var_object_type)]))
	if rt.is_true(rt.identical(rt.new_null(), var_args_mutated.array_get('meta_box_cb'))) {
		if rt.is_true(var_args_mutated.array_get('hierarchical')) {
	...
	if rt.is_true(rt.identical(rt.new_null(), var_args_mutated.array_get('meta_box_sanitize_cb'))) {
		mut switch_val_1 := var_args_mutated.array_get('meta_box_cb')
		if rt.is_true(rt.equal(switch_val_1, rt.new_string('post_categories_meta_box'))) {
			var_args_mutated.array_set('meta_box_sanitize_cb', 'taxonomy_meta_box_sanitize_cb_checkboxes')
		} else {
			var_args_mutated.array_set('meta_box_sanitize_cb', 'taxonomy_meta_box_sanitize_cb_input')
		}
	}
	if !(!rt.is_true(var_args_mutated.array_get('default_term'))) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_args_mutated.array_get('default_term').is_array()))))) {
			var_args_mutated.array_set('default_term', rt.create_array([rt.ArrayItem{ key: 'name', val: var_args_mutated.array_get('default_term') }]))
		}
		var_args_mutated.array_set('default_term', rt.call_function('wp_parse_args', [var_args_mutated.array_get('default_term'), rt.create_array([rt.ArrayItem{ key: 'name', val: '' }, rt.ArrayItem{ key: 'slug', val: '' }, rt.ArrayItem{ key: 'description', val: '' }])]))
	}
	{
		...
		for {
			...
			this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":483,"name":"property_name"}', var_property_value.dup())
		}
	}
	this.labels = rt.call_function('get_taxonomy_labels', [rt.new_object('WP_Taxonomy', []string{}, &this)])
...
fn (mut this Class_WP_Taxonomy) add_rewrite_rules()  {
	...
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(var_wp))) {
		rt.call_method(var_wp, 'add_query_var', [this.query_var])
	}
	if rt.is_true(rt.new_bool(rt.is_true(// unsupported expression: Expr_BinaryOp_NotIdentical) && rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) || rt.is_true(rt.call_function('get_option', [rt.new_string('permalink_structure')])))))) {
		if rt.is_true(rt.new_bool(rt.is_true(this.hierarchical) && rt.is_true(this.rewrite.array_get('hierarchical')))) {
			mut var_tag := rt.new_string(rt.new_string('(.+?)'))
		} else {
			var_tag = rt.new_string(rt.new_string('([^/]+)'))
		}
		rt.call_function('add_rewrite_tag', [rt.concat(rt.concat(rt.new_string('%'), this.name), rt.new_string('%')), var_tag.dup(), if rt.is_true(this.query_var) { rt.concat(this.query_var, rt.new_string('=')) } else { rt.concat(rt.concat(rt.new_string('taxonomy='), this.name), rt.new_string('&term=')) }])
		rt.call_function('add_permastruct', [this.name, rt.concat(rt.concat(rt.concat(this.rewrite.array_get('slug'), rt.new_string('/%')), this.name), rt.new_string('%')), this.rewrite])
	}
}
...
fn (mut this Class_WP_Taxonomy) add_hooks()  {
	rt.call_function('add_filter', ['wp_ajax_add-' + (this.name).str(), rt.new_string('_wp_ajax_add_hierarchical_term')])
}

fn (mut this Class_WP_Taxonomy) remove_hooks()  {
	rt.call_function('remove_filter', ['wp_ajax_add-' + (this.name).str(), rt.new_string('_wp_ajax_add_hierarchical_term')])
}
...
fn (mut this Class_WP_Taxonomy) get_rest_controller() rt.PhpVal {
	...
	mut var_class := if rt.is_true(this.rest_controller_class) { this.rest_controller_class } else { Class_WP_REST_Terms_Controller.class() }
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(this.rest_controller, '{"nodeType":"Expr_Variable","line":588,"name":"class"}')))))) {
		return rt.new_null()
	}
...
fn Class_WP_Taxonomy.get_default_labels() rt.PhpVal {
	...
	mut var_name_field_description := rt.call_function('__', [rt.new_string('The name is how it appears on your site.')])
	mut var_slug_field_description := rt.call_function('__', [rt.new_string('The &#8220;slug&#8221; is the URL-friendly version of the name. It is usually all lowercase and contains only letters, numbers, and hyphens.')])
	mut var_parent_field_description := rt.call_function('__', [rt.new_string('Assign a parent term to create a hierarchy. The term Jazz, for example, would be the parent of Bebop and Big Band.')])
	mut var_desc_field_description := rt.call_function('__', [rt.new_string('The description is not prominent by default; however, some themes may show it.')])
...
fn (mut this Class_WP_Taxonomy) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'show_tagcloud' { this.show_tagcloud = val; return true }
		'show_in_quick_edit' { this.show_in_quick_edit = val; return true }
		'show_admin_column' { this.show_admin_column = val; return true }
		'meta_box_cb' { this.meta_box_cb = val; return true }
		'meta_box_sanitize_cb' { this.meta_box_sanitize_cb = val; return true }
		...
		'update_count_callback' { this.update_count_callback = val; return true }
		...
		'default_term' { this.default_term = val; return true }
...
pub fn init_wp_includes_class_wp_taxonomy_php() {
}
