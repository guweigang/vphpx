...
fn get_category_link(var_category_arg rt.PhpVal) string {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_category.clone().is_object()))))) {
...
fn get_the_category(post_id bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_categories))))
...
fn get_the_category_list(separator string, parents string, post_id bool) rt.PhpVal {
	...
	var_rel = if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(var_wp_rewrite.clone().is_object()))
			...
			for {
				...
				if rt.is_true(rt.equal(switch_val_1, rt.new_string('multiple'))) {
					...
					var_thelist = var_thelist + '<a href="' +
						...
						'" ' + var_rel + '>' +
						(rt.get_property(var_category_shadow, 'name')).str() + '</a></li>'
			...
			for {
				...
				if rt.is_true(rt.equal(switch_val_2, rt.new_string('multiple'))) {
					...
					var_thelist = var_thelist + '<a href="' +
						(rt.call_function('esc_url', [rt.new_string(get_category_link(rt.get_property(var_category_shadow, 'term_id')))])).str() +
						'" ' + var_rel + '>' +
						(rt.get_property(var_category_shadow, 'name')).str() + '</a>'
...
fn wp_dropdown_categories(args string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.new_string(args).array_isset(rt.new_string('type'))
		&& rt.is_true(rt.identical(rt.new_string('link'), rt.new_string(args).array_get('type')))))
	{
		rt.call_function('_deprecated_argument', [rt.new_string(@FN),
			rt.new_string('3.0.0'),
	...
	var_option_none_value = var_parsed_args.array_get('option_none_value')
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(var_parsed_args.array_isset(rt.new_string('pad_counts')))
		&& rt.is_true(var_parsed_args.array_get('show_count'))))
		&& rt.is_true(var_parsed_args.array_get('hierarchical'))))
	{
		var_parsed_args.array_set('pad_counts', true)
	}
	var_tab_index = var_parsed_args.array_get('tab_index')
	...
	var_name = rt.call_function('esc_attr', [var_parsed_args.array_get('name')])
	var_class = rt.call_function('esc_attr', [var_parsed_args.array_get('class')])
	var_id = if rt.is_true(var_parsed_args.array_get('id')) { rt.call_function('esc_attr', [
			var_parsed_args.array_get('id'),
		]) } else { var_name }
	var_required = if rt.is_true(var_parsed_args.array_get('required')) { 'required' } else { '' }
	var_aria_describedby_attribute = rt.new_string((if rt.is_true(var_parsed_args.array_get('aria_describedby')) {
		' aria-describedby="' +
			(rt.call_function('esc_attr', [var_parsed_args.array_get('aria_describedby')])).str() +
			'"'
	} else {
		''
	}).str())
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(var_parsed_args.array_get('hide_if_empty')))))
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(!rt.is_true(var_categories)
		&& rt.is_true(rt.new_bool(!(rt.is_true(var_parsed_args.array_get('hide_if_empty')))))))
		&& !(!rt.is_true(var_parsed_args.array_get('show_option_none')))))
	{
		var_show_option_none = rt.call_function('apply_filters', [
			rt.new_string('list_cats'),
			var_parsed_args.array_get('show_option_none'),
			rt.new_null(),
		])
	...
	if !(!rt.is_true(var_categories)) {
		if rt.is_true(var_parsed_args.array_get('show_option_all')) {
			var_show_option_all = rt.call_function('apply_filters', [
				rt.new_string('list_cats'),
				var_parsed_args.array_get('show_option_all'),
				rt.new_null(),
			])
			var_selected = rt.new_string((if rt.is_true(rt.identical(rt.new_string('0'),
				(var_parsed_args.array_get('selected')).str()))
			{
				" selected='selected'"
			} else {
				''
		...
		if rt.is_true(var_parsed_args.array_get('hierarchical')) {
			var_depth = var_parsed_args.array_get('depth')
		} else {
...
fn wp_list_categories(args string) bool {
	...
	if !(var_parsed_args.array_isset(rt.new_string('class'))) {
		var_parsed_args.array_set('class', if rt.is_true(rt.identical(rt.new_string('category'),
			var_parsed_args.array_get('taxonomy')))
		{
	...
	var_show_option_all = var_parsed_args.array_get('show_option_all')
	var_show_option_none = var_parsed_args.array_get('show_option_none')
	var_categories = rt.call_function('get_categories', [var_parsed_args.clone()])
	var_output = rt.new_string('')
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(rt.is_true(var_parsed_args.array_get('title_li'))
		&& rt.is_true(rt.identical(rt.new_string('list'), var_parsed_args.array_get('style')))))
		&& rt.is_true(rt.new_bool(!(!rt.is_true(var_categories))
		|| rt.is_true(rt.new_bool(!(rt.is_true(var_parsed_args.array_get('hide_title_if_empty')))))))))
	{
	...
	if !rt.is_true(var_categories) {
		...
		if !(!rt.is_true(var_show_option_all)) {
			...
			if rt.is_true(rt.new_bool(
				rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string('post'), rt.get_property(var_taxonomy_object, 'object_type'), rt.new_bool(true)])))))
			...
			{
				{
					mut iter_1 := rt.get_property(var_taxonomy_object, 'object_type').iterator()
					for {
						item_1 := iter_1.next() or { break }
		...
		if rt.is_true(rt.new_bool(!rt.is_true(var_parsed_args.array_get('current_category'))
			&& rt.is_true(rt.new_bool(rt.is_true(rt.new_bool(rt.is_true(rt.call_function('is_category', []rt.PhpVal{}))
			...
			|| rt.is_true(rt.call_function('is_tag', []rt.PhpVal{}))))))
		{
			...
			if rt.is_true(rt.new_bool(rt.is_true(var_current_term_object)
				&& rt.is_true(rt.identical(var_parsed_args.array_get('taxonomy'), rt.get_property(var_current_term_object, 'taxonomy')))))
			{
...
fn wp_tag_cloud(args string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(var_tags)
		|| rt.is_true(rt.call_function('is_wp_error', [var_tags.clone()]))))
	{
		return rt.new_null()
	}
	{
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.identical(rt.new_string('array'), rt.new_string(var_args.str()).array_get('format')))
		|| !rt.is_true(rt.new_string(var_args.str()).array_get('echo'))))
	{
...
fn wp_generate_tag_cloud(var_tags_arg rt.PhpVal, args string) rt.PhpVal {
	...
	if rt.new_string(var_args.str()).array_isset(rt.new_string('topic_count_text')) {
		var_translate_nooped_plural = rt.new_string(var_args.str()).array_get('topic_count_text')
	} else if !(!rt.is_true(rt.new_string(var_args.str()).array_get('topic_count_text_callback'))) {
		if rt.is_true(rt.identical(rt.new_string('default_topic_count_text'),
			rt.new_string(var_args.str()).array_get('topic_count_text_callback')))
		{
		...
		var_translate_nooped_plural = rt.call_function('_n_noop', [
			rt.new_string(var_args.str()).array_get('single_text'),
			rt.new_string(var_args.str()).array_get('multiple_text'),
		])
	} else {
		var_translate_nooped_plural = rt.call_function('_n_noop', [
			rt.new_string('%s item'),
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_tags_sorted, var_tags)))) {
		...
		if rt.is_true(rt.identical(rt.new_string('RAND'),
			rt.new_string(var_args.str()).array_get('order')))
		{
			rt.call_function('shuffle', [var_tags.clone()])
		} else {
			if rt.is_true(rt.identical(rt.new_string('name'),
				rt.new_string(var_args.str()).array_get('orderby')))
			{
	...
	if rt.is_true(rt.greater(rt.new_string(var_args.str()).array_get('number'), rt.new_int(0))) {
		var_tags = rt.call_function('array_slice', [var_tags.clone(),
			rt.new_int(0), rt.new_string(var_args.str()).array_get('number')])
	}
	...
	var_real_counts = rt.new_array()
	{
		...
		for {
			...
			var_counts.array_set(var_key_shadow, rt.call_function('call_user_func', [
				rt.new_string(var_args.str()).array_get('topic_count_scale_callback'),
	...
	var_font_spread = rt.sub(rt.new_string(var_args.str()).array_get('largest'),
		rt.new_string(var_args.str()).array_get('smallest'))
	...
	if rt.is_true(rt.new_bool(rt.is_true(rt.new_string(var_args.str()).array_get('show_count'))
	...
	var_tags_data = rt.new_array()
	{
		...
		for {
			...
			var_tags_data.array_push(rt.create_array([
				...
				rt.ArrayItem{ key: 'font_size', val: rt.add(rt.new_string(var_args.str()).array_get('smallest'), rt.mul(rt.sub(var_count,
				...
				rt.ArrayItem{
					key: 'show_count'
					val: if rt.is_true(rt.new_string(var_args.str()).array_get('show_count')) {
	...
	var_a = rt.new_array()
	{
		mut iter_1 := var_tags_data.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_tag_data_shadow := item_1.val
			mut var_key_shadow := item_1.key
			var_class = rt.new_string((var_tag_data_shadow.array_get('class')).str() +
				' tag-link-position-' + (rt.add(var_key_shadow, rt.new_int(1))).str())
			var_a << rt.call_function('sprintf', [
				rt.new_string('<a href="%1$s"%2$s class="%3$s" style="font-size: %4$s;"%5$s>%6$s%7$s</a>'),
				rt.call_function('esc_url', [var_tag_data_shadow.array_get('url')]),
				var_tag_data_shadow.array_get('role'),
				rt.call_function('esc_attr', [var_class.clone()]),
				rt.call_function('esc_attr', [
					rt.new_string(
						(rt.call_function('str_replace', [rt.new_string(','), rt.new_string('.'), var_tag_data_shadow.array_get('font_size')])).str() +
						(rt.new_string(var_args.str()).array_get('unit')).str()),
				]),
				var_tag_data_shadow.array_get('aria_label'),
				rt.call_function('esc_html', [
					var_tag_data_shadow.array_get('name'),
				]),
				var_tag_data_shadow.array_get('show_count'),
			])
		}
	}
	mut switch_val_3 := rt.new_string(var_args.str()).array_get('format')
	if rt.is_true(rt.equal(switch_val_3, rt.new_string('array'))) {
		...
		var_return = rt.call_function('implode', [rt.new_string(var_args.str()).array_get('separator'),
			rt.create_array_from_list(var_a)])
	}
	if rt.is_true(rt.new_string(var_args.str()).array_get('filter')) {
...
fn walk_category_tree(var_args_origin ...rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(var_args.array_get(2).array_get('walker'))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_args.array_get(2).array_get('walker'), 'Walker'))))))))
	{
...
fn term_description(term i64, var_deprecated rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(var_term != 0)
...
fn get_the_term_list(var_post_id rt.PhpVal, var_taxonomy rt.PhpVal, before string, sep string, after string) rt.PhpVal {
	...
	{
		mut iter_1 := rt.new_bool(var_terms).iterator()
		for {
			item_1 := iter_1.next() or { break }
...
fn get_term_parents_list(var_term_id_arg rt.PhpVal, taxonomy string, var_args_arg rt.PhpVal) string {
	...
	var_args = rt.call_function('wp_parse_args', [var_args.clone(),
		rt.create_array_from_native_map(var_defaults)])
	{
		mut iter_1 := rt.create_array([rt.ArrayItem{ key: none, val: 'link' },
			rt.ArrayItem{ key: none, val: 'inclusive' }]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_bool_shadow := item_1.val
	...
	if rt.is_true(var_args.array_get('inclusive')) {
		rt.call_function('array_unshift', [var_parents.clone(),
			var_term_id.clone()])
	}
	{
		mut iter_1 := rt.call_function('array_reverse', [var_parents.clone()]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			...
			var_name = if rt.is_true(rt.identical(rt.new_string('slug'),
				var_args.array_get('format')))
			{
				rt.get_property(var_parent, 'slug')
			} else {
				rt.get_property(var_parent, 'name')
			}
			if rt.is_true(var_args.array_get('link')) {
				var_list = var_list + '<a href="' +
					...
					'">' + var_name.str() + '</a>' + (var_args.array_get('separator')).str()
			} else {
				var_list = var_list + var_name.str() + (var_args.array_get('separator')).str()
			}
		}
	}
	return var_list
...
pub fn init_wp_includes_category_template_php() {
}
