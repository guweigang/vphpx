...
fn (mut this Class_WP_Style_Engine_CSS_Rule) set_selector(var_selector rt.PhpVal) rt.PhpVal {
	...
	this.selector = var_selector_mutated.dup()
...
fn (mut this Class_WP_Style_Engine_CSS_Rule) add_declarations(var_declarations rt.PhpVal) rt.PhpVal {
	mut var_is_declarations_object :=
		rt.new_bool(rt.new_bool(!(rt.is_true(rt.new_bool(var_declarations.dup().is_array())))))
	...
	if rt.is_true(rt.identical(rt.new_null(), this.declarations)) {
		if rt.is_true(var_is_declarations_object) {
			this.declarations = var_declarations.dup()
			return rt.new_object('WP_Style_Engine_CSS_Rule', []string{}, this)
		}
		this.declarations = create_wp_style_engine_css_declarations(var_declarations_array.dup())
	}
	rt.call_method(this.declarations, 'add_declarations', [var_declarations_array.dup()])
	return rt.new_object('WP_Style_Engine_CSS_Rule', []string{}, this)
}
...
fn (mut this Class_WP_Style_Engine_CSS_Rule) set_rules_group(var_rules_group rt.PhpVal) rt.PhpVal {
	...
	this.rules_group = var_rules_group_mutated.dup()
	return rt.new_object('WP_Style_Engine_CSS_Rule', []string{}, this)
}
...
fn (mut this Class_WP_Style_Engine_CSS_Rule) get_css(should_prettify bool, indent_count i64) string {
	...
	mut var_declarations_indent := rt.new_int(if var_should_prettify {
		indent_count + 1
	} else {
		rt.new_int(0)
	})
	...
	mut var_spacer := rt.new_string(if var_should_prettify {
		rt.new_string(' ')
	} else {
		rt.new_string('')
	})
	...
	mut var_has_rules_group := rt.new_bool(rt.new_bool(!(!rt.is_true(var_rules_group))))
...
pub fn init_wp_includes_style_engine_class_wp_style_engine_css_rule_php() {
}
