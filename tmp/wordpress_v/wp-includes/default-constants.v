...
fn wp_initial_constants() {
	...
	])))))
	{
		...
		])))
		{
			rt.call_function('define', [rt.new_string('WP_MAX_MEMORY_LIMIT'),
				var_current_limit.clone()])
		} else if rt.is_true(rt.new_bool(rt.is_true(rt.identical(-1, var_current_limit_int))
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!rt.is_true(rt.identical(-1, var_current_limit_int))))
		&& rt.is_true(rt.new_bool(rt.is_true(rt.identical(-1, var_wp_limit_int))
	...
	{
		if rt.is_true(rt.new_bool(
			rt.is_true(rt.call_function('wp_get_development_mode', []rt.PhpVal{}))
...
fn wp_ssl_constants() {
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.call_function('defined', [rt.new_string('FORCE_SSL_LOGIN')]))
...
pub fn init_wp_includes_default_constants_php() {
}
