...
fn (mut this Class_IXR_Message) ixr_message(var_message rt.PhpVal)  {
	fn (arg_0 rt.PhpVal) rt.PhpVal { mut temp := Class_IXR_Message{}; temp.construct(arg_0); return rt.new_null() }(var_message.dup())
}
...
fn (mut this Class_IXR_Message) parse() bool {
	...
	mut var_header := rt.call_function('preg_replace', ['/<\\?xml.*?\\?' + '>/s', rt.new_string(''), rt.call_function('substr', [this.message, rt.new_int(0), rt.new_int(100)]), rt.new_int(1)])
	this.message = rt.new_string(rt.call_function('substr_replace', [this.message, var_header.dup(), rt.new_int(0), rt.new_int(100)]).to_string().trim_space())
	if rt.is_true(rt.equal(rt.new_string(''), this.message)) {
		return false
	}
	var_header = rt.call_function('preg_replace', [rt.new_string('/^<!DOCTYPE[^>]*+>/i'), rt.new_string(''), rt.call_function('substr', [this.message, rt.new_int(0), rt.new_int(200)]), rt.new_int(1)])
	this.message = rt.new_string(rt.call_function('substr_replace', [this.message, var_header.dup(), rt.new_int(0), rt.new_int(200)]).to_string().trim_space())
	if rt.is_true(rt.equal(rt.new_string(''), this.message)) {
		return false
	}
	mut var_root_tag := rt.call_function('substr', [this.message, rt.new_int(0), rt.call_function('strcspn', [rt.call_function('substr', [this.message, rt.new_int(0), rt.new_int(20)]), rt.new_string('> \t\r\n')])])
	if rt.is_true(rt.identical(rt.new_string('<!DOCTYPE'), rt.new_string(var_root_tag.dup().to_string().to_upper()))) {
		return false
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_root_tag.dup(), rt.create_array([rt.ArrayItem{ key: none, val: '<methodCall' }, rt.ArrayItem{ key: none, val: '<methodResponse' }, rt.ArrayItem{ key: none, val: '<fault' }])]))))) {
		return false
	}
	mut var_element_limit := rt.new_int(rt.new_int(30000))
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('apply_filters')])) {
		var_element_limit = rt.call_function('apply_filters', [rt.new_string('xmlrpc_element_limit'), var_element_limit.dup()])
	}
	if rt.is_true(rt.new_bool(rt.is_true(var_element_limit) && rt.is_true(rt.less(rt.mul(rt.new_int(2), var_element_limit), rt.call_function('substr_count', [this.message, rt.new_string('<')]))))) {
		return false
	}
	this._parser = rt.call_function('xml_parser_create', []rt.PhpVal{})
	rt.call_function('xml_parser_set_option', [this._parser, rt.get_constant('XML_OPTION_CASE_FOLDING'), rt.new_bool(false)])
	rt.call_function('xml_set_element_handler', [this._parser, rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('IXR_Message', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'tag_open' }]), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('IXR_Message', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'tag_close' }])])
	rt.call_function('xml_set_character_data_handler', [this._parser, rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('IXR_Message', []string{}, &this) }, rt.ArrayItem{ key: none, val: 'cdata' }])])
	mut var_chunk_size := rt.new_int(rt.new_int(262144))
	var_chunk_size = rt.call_function('apply_filters', [rt.new_string('xmlrpc_chunk_parsing_size'), var_chunk_size.dup()])
	mut var_final := rt.new_bool(rt.new_bool(false))
	for {
		if rt.is_true(rt.less_equal(rt.new_int(this.message.to_string().len), var_chunk_size)) {
			var_final = rt.new_bool(rt.new_bool(true))
		}
		mut var_part := rt.call_function('substr', [this.message, rt.new_int(0), var_chunk_size.dup()])
		this.message = rt.call_function('substr', [this.message, var_chunk_size.dup()])
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('xml_parse', [this._parser, var_part.dup(), var_final.dup()]))))) {
	...
	if rt.is_true(rt.equal(this.messageType, rt.new_string('fault'))) {
		this.faultCode = this.params.array_get(0).array_get('faultCode')
		this.faultString = this.params.array_get(0).array_get('faultString')
	}
	return true
}
...
fn (mut this Class_IXR_Message) tag_open(var_parser rt.PhpVal, var_tag rt.PhpVal, var_attr rt.PhpVal)  {
	...
	this._currentTag = var_tag.dup()
	mut switch_val_1 := var_tag
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('methodCall'))) || rt.is_true(rt.equal(switch_val_1, rt.new_string('methodResponse'))) || rt.is_true(rt.equal(switch_val_1, rt.new_string('fault'))) {
		this.messageType = var_tag.dup()
...
fn (mut this Class_IXR_Message) tag_close(var_parser rt.PhpVal, var_tag rt.PhpVal)  {
	mut var_valueFlag := rt.new_bool(rt.new_bool(false))
	mut switch_val_2 := var_tag
	if rt.is_true(rt.equal(switch_val_2, rt.new_string('int'))) || rt.is_true(rt.equal(switch_val_2, rt.new_string('i4'))) {
		mut var_value := // unsupported expression: Expr_Cast_Int
		var_valueFlag = rt.new_bool(rt.new_bool(true))
		...
		var_value = // unsupported expression: Expr_Cast_Bool
		var_valueFlag = rt.new_bool(rt.new_bool(true))
	} else if rt.is_true(rt.equal(switch_val_2, rt.new_string('base64'))) {
		var_value = rt.call_function('base64_decode', [this._currentTagContents])
		var_valueFlag = rt.new_bool(rt.new_bool(true))
	} else if rt.is_true(rt.equal(switch_val_2, rt.new_string('data'))) || rt.is_true(rt.equal(switch_val_2, rt.new_string('struct'))) {
	...
	if rt.is_true(var_valueFlag) {
		if this._arraystructs.array_count() > 0 {
			if rt.is_true(rt.equal(this._arraystructstypes.array_get(this._arraystructstypes.array_count() - 1), rt.new_string('struct'))) {
				this._arraystructs.array_get_mut(this._arraystructs.array_count() - 1).array_set(this._currentStructName.array_get(this._currentStructName.array_count() - 1), var_value.dup())
			} else {
				this._arraystructs.array_get_mut(this._arraystructs.array_count() - 1).array_push(var_value.dup())
			}
		} else {
			this.params.array_push(var_value.dup())
		}
	}
	this._currentTagContents = ''
}
...
fn (mut this Class_IXR_Message) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'messageType' { this.messageType = val; return true }
		'faultCode' { this.faultCode = val; return true }
		'faultString' { this.faultString = val; return true }
		'methodName' { this.methodName = val; return true }
		'params' { this.params = val; return true }
		'_arraystructs' { this._arraystructs = val; return true }
		'_arraystructstypes' { this._arraystructstypes = val; return true }
		'_currentStructName' { this._currentStructName = val; return true }
		'_param' { this._param = val; return true }
		'_value' { this._value = val; return true }
		'_currentTag' { this._currentTag = val; return true }
		'_currentTagContents' { this._currentTagContents = (val).str(); return true }
		'_parser' { this._parser = val; return true }
...
pub fn init_wp_includes_ixr_class_ixr_message_php() {
}
