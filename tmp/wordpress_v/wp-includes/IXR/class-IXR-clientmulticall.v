...
fn (mut this Class_IXR_ClientMulticall) construct(var_server rt.PhpVal, path bool, port i64) {
	this.Class_IXR_Client.ixr_client(var_server.dup(), rt.new_bool(path), rt.new_int(port))
...
fn (mut this Class_IXR_ClientMulticall) ixr_clientmulticall(var_server rt.PhpVal, path bool, port i64) {
	fn (arg_0 bool, arg_1 i64, arg_2 rt.PhpVal) rt.PhpVal {
		mut temp := Class_IXR_ClientMulticall{}
		temp.construct(arg_0, arg_1, arg_2)
		return rt.new_null()
	}(var_server.to_bool(), path, rt.new_int(port))
}

fn (mut this Class_IXR_ClientMulticall) addcall(var_args rt.PhpVal) {
	mut var_methodName := rt.call_function('array_shift', [var_args.dup()])
	mut var_struct := {
		'methodName': var_methodName
		'params':     var_args
	}
	this.calls.array_push(var_struct.dup())
}
...
pub fn init_wp_includes_ixr_class_ixr_clientmulticall_php() {
}
