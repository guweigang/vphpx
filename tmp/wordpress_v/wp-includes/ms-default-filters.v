...
pub fn init_wp_includes_ms_default_filters_php() {
		...
		rt.new_int(0)])
	{
		mut iter_1 := rt.create_array([rt.ArrayItem{ key: none, val: 'wpmu_new_user' },
			rt.ArrayItem{ key: none, val: 'make_spam_user' },
			rt.ArrayItem{ key: none, val: 'make_ham_user' }]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_action := item_1.val
			rt.call_function('add_action', [var_action.dup(),
	...
	rt.call_function('remove_action', [rt.new_string('wp_update_user_counts'),
		rt.new_string('wp_schedule_update_user_counts')])
	{
		mut iter_1 := rt.create_array([rt.ArrayItem{ key: none, val: 'make_spam_blog' },
			rt.ArrayItem{ key: none, val: 'make_ham_blog' }, rt.ArrayItem{
				key: none
				val: 'archive_blog'
			}, rt.ArrayItem{ key: none, val: 'unarchive_blog' },
			rt.ArrayItem{ key: none, val: 'make_delete_blog' },
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('POST_BY_EMAIL')])))))
	...
	if rt.is_true(rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('EDIT_ANY_USER')])))))
