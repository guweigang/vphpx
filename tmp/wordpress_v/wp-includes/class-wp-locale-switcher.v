...
fn (mut this Class_WP_Locale_Switcher) switch_to_locale(var_locale rt.PhpVal, user_id bool) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_locale_mutated.dup(), this.available_languages, rt.new_bool(true)])))))
	{
		return false
	}
	...
	this.change_locale(var_locale_mutated.dup())
	rt.call_function('do_action', [rt.new_string('switch_locale'),
		var_locale_mutated.dup(), rt.new_bool(user_id)])
	return true
}

fn (mut this Class_WP_Locale_Switcher) switch_to_user_locale(var_user_id rt.PhpVal) rt.PhpVal {
	mut var_locale := rt.call_function('get_user_locale', [var_user_id.dup()])
	return rt.new_bool(this.switch_to_locale(var_locale.dup(), var_user_id.to_bool()))
}
...
fn (mut this Class_WP_Locale_Switcher) restore_previous_locale() bool {
	...
	mut var_locale := if rt.is_true(rt.new_bool(var_entry.dup().is_array())) {
		var_entry.array_get(0)
	} else {
		rt.new_bool(false)
	}
	...
	this.change_locale(var_locale.dup())
	rt.call_function('do_action', [rt.new_string('restore_previous_locale'),
		var_locale.dup(), var_previous_locale.array_get(0)])
...
fn (mut this Class_WP_Locale_Switcher) get_switched_locale() bool {
	...
	if rt.is_true(var_entry) {
		return (var_entry.array_get(0)).to_bool()
	}
	return false
}
...
fn (mut this Class_WP_Locale_Switcher) get_switched_user_id() bool {
	mut var_entry := rt.call_function('end', [this.stack])
	if rt.is_true(var_entry) {
		return (var_entry.array_get(1)).to_bool()
	}
	return false
}
...
fn (mut this Class_WP_Locale_Switcher) filter_locale(var_locale rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_switched_locale) {
		return var_switched_locale.dup()
	}
	return var_locale_mutated.dup()
}
...
fn (mut this Class_WP_Locale_Switcher) load_translations(var_locale rt.PhpVal) {
	...
	mut var_domains := if rt.is_true(var_l10n) {
		rt.func_array_keys(var_l10n.dup())
	} else {
		rt.new_array()
	}
	rt.call_function('load_default_textdomain', [var_locale_mutated.dup()])
	{
		...
		for {
			...
			rt.call_function('unload_textdomain', [var_domain.dup(),
				rt.new_bool(true)])
			rt.call_function('get_translations_for_domain', [
				var_domain.dup()])
		}
	}
}
...
fn (mut this Class_WP_Locale_Switcher) change_locale(var_locale rt.PhpVal) {
	...
	mut var_locale_mutated := var_locale
	// unsupported statement: Stmt_Global
	this.load_translations(var_locale_mutated.dup())
	mut var_wp_locale := create_wp_locale()
	rt.call_method(fn () rt.PhpVal {
		mut temp := Class_WP_Translation_Controller{}
		return temp.get_instance()
	}(), 'set_locale', [var_locale_mutated.dup()])
...
pub fn init_wp_includes_class_wp_locale_switcher_php() {
}
