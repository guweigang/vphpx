...
fn _get_cron_lock() rt.PhpVal {
	...
	if rt.is_true(rt.call_function('wp_using_ext_object_cache', []rt.PhpVal{})) {
		var_value = rt.call_function('wp_cache_get', [rt.new_string('doing_cron'),
			rt.new_string('transient'), rt.new_bool(true)])
	} else {
		var_row = rt.call_method(var_wpdb, 'get_row', [
			rt.call_method(var_wpdb, 'prepare', [
				...
				rt.new_string('_transient_doing_cron'),
			]),
		])
...
fn main() {
	...
	]))
	{
		...
		rt.new_string('litespeed_finish_request'),
	]))
	{
	...
	if !(!rt.is_true(rt.get_superglobal('_POST')))
		...
		|| rt.is_true(rt.call_function('defined', [rt.new_string('DOING_CRON')])) {
		exit(0)
	}
	...
	mut var_doing_cron_transient := rt.call_function('get_transient', [
		rt.new_string('doing_cron'),
	])
	if !rt.is_true(var_doing_wp_cron) {
		if !rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('doing_wp_cron'))) {
			if rt.is_true(var_doing_cron_transient)
				&& rt.is_true(rt.greater(rt.add(var_doing_cron_transient, rt.get_constant('WP_CRON_LOCK_TIMEOUT')), var_gmt_time)) {
				return rt.new_null()
			}
			mut var_doing_wp_cron := rt.call_function('sprintf', [
				rt.new_string('%.22F'), rt.call_function('microtime', [
					rt.new_bool(true)])])
			var_doing_cron_transient = var_doing_wp_cron.clone()
			rt.call_function('set_transient', [rt.new_string('doing_cron'),
				var_doing_wp_cron.clone()])
		} else {
			var_doing_wp_cron = rt.get_superglobal('_GET').array_get(rt.new_string('doing_wp_cron'))
		}
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_doing_cron_transient, var_doing_wp_cron)))) {
		return rt.new_null()
	}
	...
	for {
		...
		for {
			...
			for {
				...
				mut var_schedule := var_v.array_get(rt.new_string('schedule'))
				if rt.is_true(var_schedule) {
					mut var_result := rt.call_function('wp_reschedule_event', [
						var_timestamp.clone(),
						var_schedule.clone(),
						var_hook.clone(),
						var_v.array_get(rt.new_string('args')),
						rt.new_bool(true),
					])
						...
						var_result.clone()]))
					{
						rt.call_function('error_log', [
							rt.call_function('sprintf', [
								rt.call_function('__', [
									rt.new_string('Cron reschedule event error for hook: %1$s, Error code: %2$s, Error message: %3$s, Data: %4$s'),
								]),
								var_hook.clone(),
								rt.call_method(var_result, 'get_error_code', []rt.PhpVal{}),
						...
						rt.call_function('do_action', [
							rt.new_string('cron_reschedule_event_error'),
							var_result.clone(),
							var_hook.clone(),
							var_v.clone(),
						])
				...
				var_result = rt.call_function('wp_unschedule_event', [
					var_timestamp.clone(), var_hook.clone(), var_v.array_get(rt.new_string('args')),
					rt.new_bool(true)])
				if rt.is_true(rt.call_function('is_wp_error', [
					var_result.clone()]))
				{
					rt.call_function('error_log', [
						rt.call_function('sprintf', [
							rt.call_function('__', [
								rt.new_string('Cron unschedule event error for hook: %1$s, Error code: %2$s, Error message: %3$s, Data: %4$s'),
							]),
							var_hook.clone(),
							rt.call_method(var_result, 'get_error_code', []rt.PhpVal{}),
							rt.call_method(var_result, 'get_error_message', []rt.PhpVal{}),
					...
					rt.call_function('do_action', [
						rt.new_string('cron_unschedule_event_error'),
						var_result.clone(),
						var_hook.clone(),
						var_v.clone(),
					])
				...
				rt.call_function('do_action_ref_array', [var_hook.clone(),
					var_v.array_get(rt.new_string('args'))])
				if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(_get_cron_lock(),
					var_doing_wp_cron))))
				{
					return rt.new_null()
				}
