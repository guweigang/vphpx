...
fn main() {
	...
	{
		var_post_type = rt.get_superglobal('_GET').array_get(rt.new_string('post_type'))
	} else {
	...
	if rt.is_true(rt.identical(rt.new_string('post'), var_post_type)) {
		...
		mut var_submenu_file := rt.new_string('post-new.php')
		...
		var_submenu_file = rt.new_string('post-new.php?post_type=${var_post_type.to_string()}')
		if !var_post_type_object.is_null()
			&& rt.is_true(rt.get_property(var_post_type_object, 'show_in_menu'))
			...
			if !(var__registered_pages.array_isset(rt.call_function('get_plugin_page_hookname', [
				rt.new_string('post-new.php?post_type=${var_post_type.to_string()}'),
				rt.get_property(var_post_type_object, 'show_in_menu'),
			]))) {
				if var__registered_pages.array_isset(rt.call_function('get_plugin_page_hookname', [
					rt.new_string('edit.php?post_type=${var_post_type.to_string()}'),
					rt.get_property(var_post_type_object, 'show_in_menu'),
				]))
				{
					var_submenu_file =
					...
					var_submenu_file = var_parent_file.clone()
				}
			}
		} else {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.get_property(rt.get_property(var_post_type_object, 'cap'), 'edit_posts')])))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.get_property(rt.get_property(var_post_type_object, 'cap'), 'create_posts')]))))) {
		rt.call_function('wp_die', [
			rt.new_string('<h1>' +
				(rt.call_function('__', [rt.new_string('You need a higher level of permission.')])).str() +
				'</h1>' + '<p>' +
				(rt.call_function('__', [rt.new_string('Sorry, you are not allowed to create posts as this user.')])).str() +
				'</p>'),
			rt.new_int(403),
		])
	}
	...
	]), rt.new_bool(true)))))
	{
			...
			var_post.clone()]))
		{
			...
			rt.include_file((rt.get_constant('ABSPATH')).str() + 'wp-admin/edit-form-advanced.php',
				'3')
		}
	} else {
		mut var_current_screen := rt.call_function('get_current_screen', []rt.PhpVal{})
		rt.call_method(var_current_screen, 'is_block_editor', [
			rt.new_bool(false)])
	}
