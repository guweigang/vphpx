...
fn main() {
	...
	if !rt.is_true(rt.get_superglobal('_REQUEST').array_get(rt.new_string('tag_ID'))) {
		mut var_sendback := rt.call_function('admin_url', [
			rt.new_string('edit-tags.php'),
		])
		if !(!rt.is_true(var_taxnow)) {
			var_sendback = rt.call_function('add_query_arg', [
				rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: var_taxnow }]),
				var_sendback.clone(),
			])
		}
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('post'), rt.get_property(rt.call_function('get_current_screen',
			[]rt.PhpVal{}), 'post_type')))))
		{
			var_sendback = rt.call_function('add_query_arg', [
				rt.new_string('post_type'),
				rt.get_property(rt.call_function('get_current_screen',
					[]rt.PhpVal{}), 'post_type'),
				var_sendback.clone()])
		}
		rt.call_function('wp_redirect', [
			rt.call_function('sanitize_url', [var_sendback.clone()]),
		])
		exit(0)
	}
	mut var_tag_ID := rt.call_function('absint',
		[rt.get_superglobal('_REQUEST').array_get(rt.new_string('tag_ID'))])
	mut var_tag := rt.call_function('get_term', [var_tag_ID.clone(),
		var_taxnow.clone(), rt.get_constant('OBJECT'), rt.new_string('edit')])
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_taxonomy.clone(), rt.call_function('get_taxonomies', [rt.create_array([rt.ArrayItem{
		key: 'show_ui'
		val: true
	}])]), rt.new_bool(true)])))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('edit_term'), rt.get_property(var_tag, 'term_id')]))))) {
		rt.call_function('wp_die', [
			rt.new_string('<h1>' +
	...
	if !rt.is_true(var_post_type) {
		var_post_type = rt.call_function('reset', [
