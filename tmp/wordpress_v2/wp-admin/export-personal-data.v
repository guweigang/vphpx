...
fn main() {
	...
	])))))
	{
		rt.call_function('wp_die', [
			rt.call_function('__', [
				rt.new_string('Sorry, you are not allowed to export personal data on this site.'),
			]),
		])
	}
	mut var_title := rt.call_function('__', [rt.new_string('Export Personal Data')])
	rt.call_method(rt.call_function('get_current_screen', []rt.PhpVal{}), 'add_help_tab', [
		rt.create_array([rt.ArrayItem{ key: 'id', val: 'overview' },
			rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
				...
				(rt.call_function('__', [rt.new_string('This screen is where you manage requests for an export of personal data.')])).str() +
				'</p>' + '<p>' +
				(rt.call_function('__', [rt.new_string('Privacy Laws around the world require businesses and online services to provide an export of some of the data they collect about an individual, and to deliver that export on request. The rights those laws enshrine are sometimes called the "Right of Data Portability". It allows individuals to obtain and reuse their personal data for their own purposes across different services. It allows them to move, copy or transfer personal data easily from one IT environment to another.')])).str() +
				'</p>' + '<p>' +
				...
				'</p>' + '<p><strong>' +
				(rt.call_function('__', [rt.new_string('Note: Since this tool only gathers data from WordPress and participating plugins, you may need to do more to comply with export requests. For example, you should also send the requester some of the data collected from or stored with the 3rd party services your organization uses.')])).str() +
				'</strong></p>' }]),
	])
	rt.call_method(rt.call_function('get_current_screen', []rt.PhpVal{}), 'add_help_tab', [
		rt.create_array([rt.ArrayItem{ key: 'id', val: 'default-data' },
			rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
				...
				(rt.call_function('__', [rt.new_string('WordPress collects (but <em>never</em> publishes) a limited amount of data from registered users who have logged in to the site. Generally, these users are people who contribute to the site in some way -- content, store management, and so on. With rare exceptions, these users do not include occasional visitors who might have registered to comment on articles or buy products. The data WordPress retains can include:')])).str() +
				'</p>' + '<p>' +
				...
				'</p>' + '<p>' +
				(rt.call_function('__', [rt.new_string('<strong>Community Events Location</strong> &mdash; The IP Address of the user, which populates the Upcoming Community Events dashboard widget with relevant information.')])).str() +
				'</p>' + '<p>' +
				...
				'</p>' + '<p>' +
				(rt.call_function('__', [rt.new_string('<strong>Comments</strong> &mdash; For user comments, Email Address, IP Address, User Agent (Browser/OS), Date/Time, Comment Content, and Content URL.')])).str() +
				'</p>' + '<p>' +
				(rt.call_function('__', [rt.new_string('<strong>Media</strong> &mdash; A list of URLs for media files the user uploads.')])).str() +
				'</p>' }]),
	])
	...
	rt.call_method(rt.call_function('get_current_screen', []rt.PhpVal{}), 'add_help_tab', [
		rt.create_array([rt.ArrayItem{ key: 'id', val: 'plugin-data' },
			rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
				...
				(rt.call_function('__', [rt.new_string('Many plugins may collect or store personal data either in the WordPress database or remotely. Any Export Personal Data request should include data from plugins as well.')])).str() +
				'</p>' + var_privacy_policy_guide.str() + '<p>' +
				(rt.call_function('__', [rt.new_string('If you are a plugin author, you can learn more about <a href="https://developer.wordpress.org/plugins/privacy/adding-the-personal-data-exporter-to-your-plugin/">how to add the Personal Data Exporter to a plugin</a>.')])).str() +
				'</p>' }]),
	])
	rt.call_method(rt.call_function('get_current_screen', []rt.PhpVal{}), 'set_help_sidebar', [
		rt.new_string('<p><strong>' +
			...
			'</strong></p>' + '<p>' +
			(rt.call_function('__', [rt.new_string('<a href="https://wordpress.org/documentation/article/tools-export-personal-data-screen/">Documentation on Export Personal Data</a>')])).str() +
			'</p>' + '<p>' +
	...
	rt.call_function('add_screen_option', [rt.new_string('per_page'),
		rt.create_array([rt.ArrayItem{ key: 'default', val: 20 },
			rt.ArrayItem{ key: 'option', val: 'export_personal_data_requests_per_page' }])])
	mut var__list_table_args := {
		'plural':   'privacy_requests'
		'singular': 'privacy_request'
	}
	mut var_requests_table := rt.call_function('_get_list_table', [
		rt.new_string('WP_Privacy_Data_Export_Requests_List_Table'),
		rt.create_array_from_native_map(var__list_table_args),
	])
	rt.call_method(rt.get_property(var_requests_table, 'screen'), 'set_screen_reader_content', [
		rt.create_array([
			rt.ArrayItem{ key: 'heading_views', val: rt.call_function('__', [
				rt.new_string('Filter export personal data list'),
			]) },
			rt.ArrayItem{ key: 'heading_pagination', val: rt.call_function('__', [
				rt.new_string('Export personal data list navigation'),
			]) },
			rt.ArrayItem{ key: 'heading_list', val: rt.call_function('__', [
				rt.new_string('Export personal data list'),
			]) },
		]),
	])
	...
	rt.call_function('esc_html_e', [rt.new_string('Export Personal Data')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('_e', [
		rt.new_string('This tool helps site owners comply with local laws and regulations by exporting known data for a given user in a .zip file.'),
	])
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Add Data Export Request')])
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('_e', [
		rt.new_string('Send personal data export confirmation email.'),
	])
	// unsupported statement: Stmt_InlineHTML
