...
fn main() {
	...
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get(rt.new_string('deletebookmarks')))) {
		var_action = rt.new_string('deletebookmarks')
	}
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get(rt.new_string('move')))) {
		var_action = rt.new_string('move')
	}
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get(rt.new_string('linkcheck')))) {
		mut var_linkcheck := rt.get_superglobal('_POST').array_get(rt.new_string('linkcheck'))
	}
	mut var_this_file := rt.call_function('admin_url', [
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('deletebookmarks'))) {
		...
		if var_linkcheck.clone().array_count() == 0 {
			rt.call_function('wp_redirect', [var_this_file.clone()])
			exit(0)
		}
		mut var_deleted := 0
		...
		for {
			...
			var_link_id_shadow = rt.new_int(var_link_id_shadow.to_i64())
			if rt.is_true(rt.call_function('wp_delete_link', [
				var_link_id_shadow.clone()]))
			{
				var_deleted += 1
			}
		}
		rt.call_function('wp_redirect', [
			rt.new_string('${var_this_file.to_string()}?deleted=${var_deleted.str()}'),
		])
		exit(0)
		...
		mut var_all_links := rt.call_function('implode', [rt.new_string(','),
			var_linkcheck.clone()])
		rt.call_function('wp_redirect', [var_this_file.clone()])
		exit(0)
		...
		if rt.is_true(rt.call_function('add_link', []rt.PhpVal{})) {
			var_redir = rt.call_function('add_query_arg', [rt.new_string('added'),
				rt.new_string('true'), var_redir.clone()])
		}
		rt.call_function('wp_redirect', [var_redir.clone()])
		exit(0)
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('save'))) {
		var_link_id =
			rt.new_int((rt.get_superglobal('_POST').array_get(rt.new_string('link_id'))).to_i64())
		rt.call_function('check_admin_referer', [
			rt.new_string('update-bookmark_' + var_link_id.str()),
		])
		rt.call_function('edit_link', [var_link_id.clone()])
		rt.call_function('wp_redirect', [var_this_file.clone()])
		exit(0)
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('delete'))) {
		var_link_id =
			rt.new_int((rt.get_superglobal('_GET').array_get(rt.new_string('link_id'))).to_i64())
		rt.call_function('check_admin_referer', [
			rt.new_string('delete-bookmark_' + var_link_id.str()),
		])
		...
		var_link_id =
			rt.new_int((rt.get_superglobal('_GET').array_get(rt.new_string('link_id'))).to_i64())
		mut var_link := rt.call_function('get_link_to_edit', [
			var_link_id.clone()])
		if rt.is_true(rt.new_bool(!(rt.is_true(var_link)))) {
			rt.call_function('wp_die', [
				rt.call_function('__', [rt.new_string('Link not found.')]),
			])
		}
