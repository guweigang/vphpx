...
fn main() {
	...
	mut var_media_options_help := rt.new_string('<p>' +
		(rt.call_function('__', [rt.new_string('You can set maximum sizes for images inserted into your written content; you can also insert an image as Full Size.')])).str() +
		'</p>')
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{})))))
		&& rt.is_true(rt.call_function('get_option', [rt.new_string('upload_url_path')]))
		...
		var_media_options_help = rt.concat(var_media_options_help, rt.new_string('<p>' +
			(rt.call_function('__', [rt.new_string('Uploading Files allows you to choose the folder and path for storing your uploaded files.')])).str() +
			'</p>'))
	}
	var_media_options_help = rt.concat(var_media_options_help, rt.new_string('<p>' +
	...
	rt.call_method(rt.call_function('get_current_screen', []rt.PhpVal{}), 'add_help_tab', [
		rt.create_array([rt.ArrayItem{ key: 'id', val: 'overview' },
			...
			]) }, rt.ArrayItem{ key: 'content', val: var_media_options_help }]),
	])
	rt.call_method(rt.call_function('get_current_screen', []rt.PhpVal{}), 'set_help_sidebar', [
		rt.new_string('<p><strong>' +
			...
			'</strong></p>' + '<p>' +
			(rt.call_function('__', [rt.new_string('<a href="https://wordpress.org/documentation/article/settings-media-screen/">Documentation on Media Settings</a>')])).str() +
			'</p>' + '<p>' +
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('_e', [
		rt.new_string('The sizes listed below determine the maximum dimensions in pixels to use when adding an image to the Media Library.'),
	])
	// unsupported statement: Stmt_InlineHTML
	mut var_thumbnail_size_title := rt.call_function('__', [
		rt.new_string('Thumbnail size'),
	])
	// unsupported statement: Stmt_InlineHTML
	...
	rt.call_function('checked', [rt.new_string('1'),
		rt.call_function('get_option', [rt.new_string('thumbnail_crop')])])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('_e', [
		rt.new_string('Crop thumbnail to exact dimensions (normally thumbnails are proportional)'),
	])
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_settings_fields', [rt.new_string('media'),
		rt.new_string('default')])
	// unsupported statement: Stmt_InlineHTML
	if var_GLOBALS.array_get(rt.new_string('wp_settings')).array_get(rt.new_string('media')).array_isset(rt.new_string('embeds')) {
		// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{}))))) {
		...
		if rt.is_true(rt.call_function('get_option', [rt.new_string('upload_url_path')]))
			...
			rt.echo_val(rt.call_function('esc_attr', [
				rt.call_function('get_option', [rt.new_string('upload_path')]),
			]))
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('printf', [
				rt.call_function('__', [rt.new_string('Default is %s')]),
				rt.new_string('<code>wp-content/uploads</code>'),
			])
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('_e', [rt.new_string('Full URL path to files')])
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('esc_attr', [
				rt.call_function('get_option', [rt.new_string('upload_url_path')]),
			]))
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('_e', [
				rt.new_string('Configuring this is optional. By default, it should be blank.'),
			])
			// unsupported statement: Stmt_InlineHTML
		} else {
			// unsupported statement: Stmt_InlineHTML
		...
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('_e', [
			rt.new_string('Organize my uploads into month- and year-based folders'),
		])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('do_settings_fields', [rt.new_string('media'),
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_settings_sections', [rt.new_string('media')])
	// unsupported statement: Stmt_InlineHTML
