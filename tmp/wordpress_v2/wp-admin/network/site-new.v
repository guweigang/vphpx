...
fn main() {
	...
	if rt.get_superglobal('_REQUEST').array_isset(rt.new_string('action'))
		&& rt.is_true(rt.identical(rt.new_string('add-site'), rt.get_superglobal('_REQUEST').array_get(rt.new_string('action')))) {
		rt.call_function('check_admin_referer', [rt.new_string('add-blog'),
			rt.new_string('_wpnonce_add-blog')])
		if !(rt.get_superglobal('_POST').array_get(rt.new_string('blog')).is_array()) {
			rt.call_function('wp_die', [
				rt.call_function('__', [rt.new_string('Cannot create an empty site.')]),
			])
		}
		mut var_blog := rt.get_superglobal('_POST').array_get(rt.new_string('blog'))
		mut var_domain := ''
		var_blog.array_set('domain',
			var_blog.array_get(rt.new_string('domain')).to_string().trim_space())
		if rt.is_true(rt.call_function('preg_match', [
			rt.new_string('|^([a-zA-Z0-9-])+$|'),
			var_blog.array_get(rt.new_string('domain')),
		]))
		{
			var_domain = var_blog.array_get(rt.new_string('domain')).to_string().to_lower()
		}
			...
			[]rt.PhpVal{})))))
		{
			mut var_subdirectory_reserved_names := rt.call_function('get_subdirectory_reserved_names',
				[]rt.PhpVal{})
			if rt.is_true(rt.call_function('in_array', [rt.new_string(var_domain.str()).clone(),
				var_subdirectory_reserved_names.clone(), rt.new_bool(true)]))
			{
				rt.call_function('wp_die', [
					rt.call_function('sprintf', [
						...
						rt.new_string('<code>' +
							(rt.call_function('implode', [rt.new_string('</code>, <code>'), var_subdirectory_reserved_names.clone()])).str() +
							'</code>'),
					]),
				])
			}
		}
		mut var_title := var_blog.array_get(rt.new_string('title'))
		mut var_meta := {
			'public': rt.new_int(1)
		}
		if rt.get_superglobal('_POST').array_isset(rt.new_string('WPLANG')) {
			...
			{
				var_meta['WPLANG'] = rt.get_superglobal('_POST').array_get(rt.new_string('WPLANG'))
			} else if
				...
				if rt.is_true(var_language) {
					var_meta['WPLANG'] = var_language.clone()
				}
			}
		}
		...
		if var_blog.array_isset(rt.new_string('email'))
			&& rt.is_true(rt.identical(rt.new_string(''), rt.new_string(var_blog.array_get(rt.new_string('email')).to_string().trim_space()))) {
			rt.call_function('wp_die', [
				rt.call_function('__', [rt.new_string('Missing email address.')]),
			])
		}
		mut var_email := rt.call_function('sanitize_email', [
			var_blog.array_get(rt.new_string('email')),
		])
		...
		if rt.is_true(rt.call_function('is_subdomain_install', []rt.PhpVal{})) {
			mut var_newdomain :=
				rt.new_string((var_domain + '.' +(rt.call_function('preg_replace', [rt.new_string('|^www\\.|'), rt.new_string(''), rt.get_property(rt.call_function('get_network', []rt.PhpVal{}), 'domain')])).str()).str())
		...
		mut var_password := rt.new_string('N/A')
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(var_user_id)))) {
			...
			var_user_id = rt.call_function('username_exists', [
				rt.new_string(var_domain.str()).clone()])
			...
			var_user_id = rt.call_function('wpmu_create_user', [
				rt.new_string(var_domain.str()).clone(), var_password.clone(),
				var_email.clone()])
		...
		mut var_id := rt.call_function('wpmu_create_blog', [var_newdomain.clone(),
			var_path.clone(), var_title.clone(), var_user_id.clone(),
			rt.create_array_from_native_map(var_meta),
			rt.call_function('get_current_network_id',
				[]rt.PhpVal{})])
		...
		])))))
		{
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_super_admin', [var_user_id.clone()])))))
				&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('get_user_option', [rt.new_string('primary_blog'), var_user_id.clone()]))))) {
				rt.call_function('update_user_option', [var_user_id.clone(),
					rt.new_string('primary_blog'), var_id.clone(),
					rt.new_bool(true)])
			}
			rt.call_function('wpmu_new_site_admin_notification', [
				var_id.clone(), var_user_id.clone()])
			rt.call_function('wpmu_welcome_notification', [var_id.clone(),
				var_user_id.clone(), var_password.clone(), var_title.clone(),
			...
			rt.get_superglobal('_GET').array_get(rt.new_string('update'))))
		{
			var_messages << rt.call_function('sprintf', [
				...
				rt.call_function('network_admin_url', [
					rt.new_string('site-info.php?id=' +(rt.call_function('absint', [rt.get_superglobal('_GET').array_get(rt.new_string('id'))])).str()),
				]),
			])
		}
	}
	...
	if rt.is_true(rt.call_function('is_subdomain_install', []rt.PhpVal{})) {
		...
		rt.echo_val(rt.call_function('esc_html', [
			rt.new_string('.' +(rt.call_function('preg_replace', [rt.new_string('|^www\\.|'), rt.new_string(''), rt.get_property(rt.call_function('get_network', []rt.PhpVal{}), 'domain')])).str()),
		]))
		// unsupported statement: Stmt_InlineHTML
	} else {
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_html', [
			rt.new_string(
				(rt.get_property(rt.call_function('get_network', []rt.PhpVal{}), 'domain')).str() +
				(rt.get_property(rt.call_function('get_network', []rt.PhpVal{}), 'path')).str()),
		]))
		// unsupported statement: Stmt_InlineHTML
	}
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [rt.new_string('network_site_new_form')])
	rt.call_function('submit_button', [
		rt.call_function('__', [rt.new_string('Add Site')]),
