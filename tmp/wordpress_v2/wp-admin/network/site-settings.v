...
fn main() {
	...
	if rt.get_superglobal('_REQUEST').array_isset(rt.new_string('action'))
		...
		&& rt.get_superglobal('_POST').array_get(rt.new_string('option')).is_array() {
		...
		mut iter_1 :=
			rt.cast_array(rt.get_superglobal('_POST').array_get(rt.new_string('option'))).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_val := item_1.val
			mut var_key := item_1.key
			var_key = rt.call_function('wp_unslash', [var_key.clone()])
			var_val = rt.call_function('wp_unslash', [var_val.clone()])
			if rt.is_true(rt.identical(rt.new_int(0), var_key)) || var_val.clone().is_array()
				|| rt.is_true(rt.call_function('in_array', [var_key.clone(), rt.create_array_from_list(var_skip_options), rt.new_bool(true)])) {
				continue
			}
			rt.call_function('update_option', [var_key.clone(),
				var_val.clone()])
		}
		rt.call_function('do_action', [rt.new_string('wpmu_update_blog_options'),
			var_id.clone()])
		...
		rt.call_function('wp_redirect', [
			rt.call_function('add_query_arg', [
				...
				rt.new_string('site-settings.php'),
			]),
		])
		exit(0)
	}
	if rt.get_superglobal('_GET').array_isset(rt.new_string('update')) {
			...
			rt.get_superglobal('_GET').array_get(rt.new_string('update'))))
		{
			var_messages << rt.call_function('__', [
				rt.new_string('Site options updated.'),
			])
		}
	}
	...
	rt.call_function('network_edit_site_nav', [
		rt.create_array([rt.ArrayItem{ key: 'blog_id', val: var_id },
			rt.ArrayItem{ key: 'selected', val: 'site-settings' }]),
	])
	...
	// unsupported statement: Stmt_InlineHTML
	mut var_blog_prefix := rt.call_method(var_wpdb, 'get_blog_prefix', [
		var_id.clone()])
	mut var_options := rt.call_method(var_wpdb, 'get_results', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.new_string('SELECT * FROM %i\n\t\t\t\tWHERE option_name NOT LIKE %s\n\t\t\t\tAND option_name NOT LIKE %s'),
			rt.new_string('${var_blog_prefix.to_string()}options'),
			rt.new_string((rt.call_method(var_wpdb, 'esc_like', [rt.new_string('_')])).str() + '%'),
			rt.new_string('%' +
				(rt.call_method(var_wpdb, 'esc_like', [rt.new_string('user_roles')])).str()),
		]),
	])
	...
	for {
		...
		if rt.is_true(rt.identical(rt.new_string('default_role'), rt.get_property(var_option,
			'option_name')))
		{
		...
		]))
		{
			...
			{
				rt.set_property(var_option, 'option_value', rt.call_function('esc_html', [
					rt.call_function('maybe_unserialize', [
						rt.get_property(var_option, 'option_value'),
					]),
				]))
			} else {
		...
		mut var_ltr_fields := ['siteurl', 'home', 'admin_email', 'new_admin_email', 'mailserver_url',
			'mailserver_login', 'mailserver_pass', 'ping_sites', 'permalink_structure',
			'category_base', 'tag_base', 'upload_path', 'upload_url_path']
		if rt.is_true(rt.call_function('in_array', [
			rt.get_property(var_option, 'option_name'),
			rt.create_array_from_list(var_ltr_fields),
			rt.new_bool(true),
		]))
		{
			var_class = var_class + ' ltr'
		}
		...
		]))
		{
			...
			if rt.is_true(var_is_main_site)
				&& rt.is_true(rt.call_function('in_array', [rt.get_property(var_option, 'option_name'), rt.create_array([rt.ArrayItem{
				key: none
				val: 'siteurl'
			}, rt.ArrayItem{ key: none, val: 'home' }]), rt.new_bool(true)])) {
				// unsupported statement: Stmt_InlineHTML
				rt.echo_val(rt.call_function('esc_html', [
					rt.get_property(var_option, 'option_value'),
				]))
	...
	rt.call_function('do_action', [rt.new_string('wpmueditblogaction'),
		var_id.clone()])
	// unsupported statement: Stmt_InlineHTML
