...
fn main() {
	...
	rt.include_file(
		(rt.get_constant('ABSPATH')).str() + (rt.get_constant('WPINC')).str() + '/http.php', '4')
	...
	rt.call_method(rt.call_function('get_current_screen', []rt.PhpVal{}), 'add_help_tab', [
		rt.create_array([rt.ArrayItem{ key: 'id', val: 'overview' },
			rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
				...
				(rt.call_function('__', [rt.new_string('Only use this screen once you have updated to a new version of WordPress through Updates/Available Updates (via the Network Administration navigation menu or the Toolbar). Clicking the Upgrade Network button will step through each site in the network, five at a time, and make sure any database updates are applied.')])).str() +
				'</p>' + '<p>' +
				(rt.call_function('__', [rt.new_string('If a version update to core has not happened, clicking this button will not affect anything.')])).str() +
				'</p>' + '<p>' +
				(rt.call_function('__', [rt.new_string('If this process fails for any reason, users logging in to their sites will force the same update.')])).str() +
				'</p>' }]),
	])
	rt.call_method(rt.call_function('get_current_screen', []rt.PhpVal{}), 'set_help_sidebar', [
		rt.new_string('<p><strong>' +
			...
			'</strong></p>' + '<p>' +
			(rt.call_function('__', [rt.new_string('<a href="https://developer.wordpress.org/advanced-administration/multisite/admin/#network-admin-updates-screen">Documentation on Upgrade Network</a>')])).str() +
			'</p>' + '<p>' +
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('upgrade'))) {
		mut var_n := rt.new_int(if rt.get_superglobal('_GET').array_isset(rt.new_string('n')) {
			rt.new_int((rt.get_superglobal('_GET').array_get(rt.new_string('n'))).to_i64())
		} else {
			0
		})
		if rt.is_true(rt.less(var_n, rt.new_int(5))) {
			rt.call_function('update_site_option', [rt.new_string('wpmu_upgrade_site'),
				var_wp_db_version.clone()])
		}
		mut var_site_ids := rt.call_function('get_sites', [
			rt.create_array([rt.ArrayItem{ key: 'spam', val: 0 },
				rt.ArrayItem{ key: 'deleted', val: 0 }, rt.ArrayItem{ key: 'archived', val: 0 },
				rt.ArrayItem{ key: 'network_id', val: rt.call_function('get_current_network_id',
					[]rt.PhpVal{}) }, rt.ArrayItem{ key: 'number', val: 5 },
				rt.ArrayItem{ key: 'offset', val: var_n }, rt.ArrayItem{ key: 'fields', val: 'ids' },
				rt.ArrayItem{ key: 'order', val: 'DESC' }, rt.ArrayItem{ key: 'orderby', val: 'id' },
		...
		for {
			...
			mut var_upgrade_url := rt.call_function('admin_url', [
			...
			mut var_response := rt.call_function('wp_remote_get', [
				var_upgrade_url.clone(),
				rt.create_array([
					rt.ArrayItem{ key: 'timeout', val: 120 },
					rt.ArrayItem{ key: 'httpversion', val: '1.1' },
					rt.ArrayItem{ key: 'sslverify', val: false },
				])])
			if rt.is_true(rt.call_function('is_wp_error', [var_response.clone()])) {
				rt.call_function('wp_die', [
					rt.call_function('sprintf', [
						rt.call_function('__', [
							rt.new_string('Warning! Problem updating %1$s. Your server may not be able to connect to sites running on it. Error message: %2$s'),
						]),
						var_siteurl.clone(),
						rt.new_string('<em>' +
			...
			rt.call_function('do_action', [rt.new_string('after_mu_upgrade'),
				var_response.clone()])
			rt.call_function('do_action', [rt.new_string('wpmu_upgrade_site'),
				var_site_id.clone()])
		}
		print('</ul>')
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('_e', [
			rt.new_string('If your browser does not start loading the next page automatically, click this link:'),
		])
		// unsupported statement: Stmt_InlineHTML
		...
		])).to_i64()), var_wp_db_version))))
		{
			...
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('_e', [
				rt.new_string('WordPress has been updated! Next and final step is to individually upgrade the sites in your network.'),
			])
			// unsupported statement: Stmt_InlineHTML
		}
		// unsupported statement: Stmt_InlineHTML
