...
fn (mut this Class_Bulk_Upgrader_Skin) feedback(var_feedback rt.PhpVal, var_args rt.PhpVal) {
	...
	if rt.is_true(rt.call_function('str_contains', [var_feedback_mutated.clone(),
		rt.new_string('%')]))
	{
		if rt.is_true(var_args_mutated) {
			...
			var_args_mutated = rt.call_function('array_map', [
				rt.new_string('esc_html'), var_args_mutated.clone()])
			var_feedback_mutated = rt.call_function('vsprintf', [
				var_feedback_mutated.clone(), var_args_mutated.clone()])
		}
	}
...
fn (mut this Class_Bulk_Upgrader_Skin) error(var_errors rt.PhpVal) {
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_errors.clone()])) {
		...
		for {
			...
			if rt.is_true(rt.call_method(var_errors, 'get_error_data', []rt.PhpVal{}))
				&& rt.call_method(var_errors, 'get_error_data', []rt.PhpVal{}).is_string() {
				...
				var_messages << var_emessage.clone()
			}
		}
...
fn (mut this Class_Bulk_Upgrader_Skin) before(title string) {
	...
	rt.call_function('printf', [
		rt.new_string('<h2>' +
			(rt.get_property(rt.get_property(rt.new_object('Bulk_Upgrader_Skin', ['WP_Upgrader_Skin'], &this), 'upgrader'), 'strings').array_get(rt.new_string('skin_before_update_header'))).str() +
...
fn (mut this Class_Bulk_Upgrader_Skin) after(title string) {
	...
	if rt.is_true(this.error)
		...
		if rt.is_true(this.error) {
			mut var_after_error_message := rt.call_function('sprintf', [
				rt.get_property(rt.get_property(rt.new_object('Bulk_Upgrader_Skin', [
					'WP_Upgrader_Skin',
				], &this), 'upgrader'), 'strings').array_get(rt.new_string('skin_update_failed_error')),
				rt.new_string(title),
				rt.new_string('<strong>' + (this.error).str() + '</strong>'),
			])
		} else {
			var_after_error_message = rt.call_function('sprintf', [
				rt.get_property(rt.get_property(rt.new_object('Bulk_Upgrader_Skin', [
					'WP_Upgrader_Skin',
				], &this), 'upgrader'), 'strings').array_get(rt.new_string('skin_update_failed')),
				rt.new_string(title),
			])
		}
		rt.call_function('wp_admin_notice', [var_after_error_message.clone(),
			rt.create_array([
	...
	if rt.is_true(rt.get_property(rt.new_object('Bulk_Upgrader_Skin', ['WP_Upgrader_Skin'], &this), 'result'))
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(this.error)))) {
			print('<div class="updated js-update-details" data-update-details="progress-' +
				(rt.call_function('esc_attr', [rt.get_property(rt.get_property(rt.new_object('Bulk_Upgrader_Skin', ['WP_Upgrader_Skin'], &this), 'upgrader'), 'update_current')])).str() +
				'">' + '<p>' +
				(rt.call_function('sprintf', [rt.get_property(rt.get_property(rt.new_object('Bulk_Upgrader_Skin', ['WP_Upgrader_Skin'], &this), 'upgrader'), 'strings').array_get(rt.new_string('skin_update_successful')), rt.new_string(title)])).str() +
