...
fn (mut this Class_Plugin_Upgrader_Skin) construct(var_args rt.PhpVal) {
	...
	this.plugin = var_args_mutated.array_get(rt.new_string('plugin'))
...
fn (mut this Class_Plugin_Upgrader_Skin) after() {
	...
	if !(!rt.is_true(this.plugin))
		...
		rt.call_function('printf', [
			...
			rt.call_function('wp_nonce_url', [
				rt.new_string('update.php?action=activate-plugin&networkwide=' +
					(this.plugin_network_active).str() + '&plugin=' +
					(rt.call_function('urlencode', [this.plugin])).str()),
				rt.new_string('activate-plugin_' + (this.plugin).str()),
			]),
		])
	}
	...
	if rt.is_true(this.plugin_active)
