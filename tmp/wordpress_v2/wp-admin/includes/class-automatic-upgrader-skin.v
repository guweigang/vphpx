...
fn (mut this Class_Automatic_Upgrader_Skin) feedback(var_feedback rt.PhpVal, var_args rt.PhpVal) {
	...
	} else if rt.is_true(rt.new_bool(var_feedback.clone().is_array())) {
		return
	} else {
	...
	var_string = rt.new_string(var_string.clone().to_string().trim_space())
	var_string = rt.call_function('wp_kses', [var_string.clone(),
		rt.create_array([
	...
	this.messages.array_push(var_string.clone())
}
...
fn (mut this Class_Automatic_Upgrader_Skin) footer() {
	...
	if !(!rt.is_true(var_output)) {
		this.feedback(var_output.clone(), rt.new_null())
	}
}
