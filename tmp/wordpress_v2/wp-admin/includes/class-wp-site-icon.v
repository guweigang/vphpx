...
fn (mut this Class_WP_Site_Icon) construct() {
	rt.call_function('add_action', [rt.new_string('delete_attachment'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WP_Site_Icon', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'delete_attachment_data' },
		])])
	rt.call_function('add_filter', [rt.new_string('get_post_metadata'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WP_Site_Icon', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'get_post_metadata' },
		]),
...
fn (mut this Class_WP_Site_Icon) create_attachment_object(var_cropped rt.PhpVal, var_parent_attachment_id rt.PhpVal) rt.PhpVal {
	...
	mut var_size := rt.call_function('wp_getimagesize', [var_cropped.clone()])
	...
	mut var_attachment := {
		'ID':             var_parent_attachment_id
		'post_title':     rt.call_function('wp_basename', [var_cropped.clone()])
		'post_content':   var_url
		'post_mime_type': var_image_type
		'guid':           var_url
		'context':        rt.new_string('site-icon')
	}
...
fn (mut this Class_WP_Site_Icon) insert_attachment(var_attachment rt.PhpVal, var_file rt.PhpVal) rt.PhpVal {
	...
	mut var_attachment_id := rt.call_function('wp_insert_attachment', [
		var_attachment_mutated.clone(), var_file.clone()])
	...
	var_metadata = rt.call_function('apply_filters', [
		rt.new_string('site_icon_attachment_metadata'),
		var_metadata.clone(),
	])
...
fn (mut this Class_WP_Site_Icon) additional_sizes(var_sizes rt.PhpVal) rt.PhpVal {
	...
	this.site_icon_sizes = rt.call_function('apply_filters', [
		rt.new_string('site_icon_image_sizes'),
	...
	for {
		...
		if var_size_array.array_isset(rt.new_string('crop')) {
			var_only_crop_sizes.array_set(var_name, var_size_array.clone())
		}
	}
	mut iter_2 := this.site_icon_sizes.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_size := item_2.val
		if rt.is_true(rt.less(var_size, this.min_size)) {
			var_only_crop_sizes.array_set('site_icon-' + var_size.str(), rt.create_array([
				rt.ArrayItem{ key: 'width ', val: var_size },
				rt.ArrayItem{ key: 'height', val: var_size },
				rt.ArrayItem{ key: 'crop', val: true },
			]))
		}
	}
	return var_only_crop_sizes.clone()
}
...
fn (mut this Class_WP_Site_Icon) delete_attachment_data(var_post_id rt.PhpVal) {
	...
	if rt.is_true(var_site_icon_id) && rt.is_true(rt.identical(var_post_id, var_site_icon_id)) {
...
fn (mut this Class_WP_Site_Icon) get_post_metadata(var_value rt.PhpVal, var_post_id rt.PhpVal, var_meta_key rt.PhpVal, var_single rt.PhpVal) rt.PhpVal {
	if rt.is_true(var_single)
		&& rt.is_true(rt.identical(rt.new_string('_wp_attachment_backup_sizes'), var_meta_key)) {
		mut var_site_icon_id := rt.new_int((rt.call_function('get_option', [
			rt.new_string('site_icon'),
		])).to_i64())
		if rt.is_true(rt.identical(var_post_id, var_site_icon_id)) {
			rt.call_function('add_filter', [rt.new_string('intermediate_image_sizes'),
				rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_Site_Icon', []string{}, &this) },
					rt.ArrayItem{ key: none, val: 'intermediate_image_sizes' },
				])])
		}
	}
...
fn (mut this Class_WP_Site_Icon) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_post_metadata' {
			...
			return this.get_post_metadata(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2,
				dispatch_arg_3)
		}
		else {
			return none
