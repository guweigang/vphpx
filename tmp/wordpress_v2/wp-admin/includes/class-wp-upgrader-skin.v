...
fn (mut this Class_WP_Upgrader_Skin) construct(var_args rt.PhpVal) {
	...
	this.options = rt.call_function('wp_parse_args', [var_args_mutated.clone(),
		rt.create_array_from_native_map(var_defaults)])
}

fn (mut this Class_WP_Upgrader_Skin) set_upgrader(var_upgrader rt.PhpVal) {
	if rt.is_true(rt.new_bool(var_upgrader.clone().is_object())) {
		this.upgrader = var_upgrader
	}
	this.add_strings()
}
...
fn (mut this Class_WP_Upgrader_Skin) request_filesystem_credentials(error bool, context string, allow_relaxed_file_ownership bool) rt.PhpVal {
	...
	mut var_url := this.options.array_get(rt.new_string('url'))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_string(context_mutated))))) {
		context_mutated = (this.options.array_get(rt.new_string('context'))).str()
	}
	if !(!rt.is_true(this.options.array_get(rt.new_string('nonce')))) {
		var_url = rt.call_function('wp_nonce_url', [var_url.clone(),
			this.options.array_get(rt.new_string('nonce'))])
	}
	mut var_extra_fields := rt.new_array()
	return rt.call_function('request_filesystem_credentials', [
		var_url.clone(), rt.new_string(''), rt.new_bool(error),
		rt.new_string(context_mutated).clone(), var_extra_fields.clone(),
...
fn (mut this Class_WP_Upgrader_Skin) header() {
	...
	print('<h1>' + (this.options.array_get(rt.new_string('title'))).str() + '</h1>')
}
...
fn (mut this Class_WP_Upgrader_Skin) error(var_errors rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(var_errors.clone().is_string())) {
		this.feedback(var_errors.clone(), rt.new_null())
		...
		for {
			...
			if rt.is_true(rt.call_method(var_errors, 'get_error_data', []rt.PhpVal{}))
				...
				this.feedback(var_message.clone(), rt.new_null())
			}
		}
	}
}
...
fn (mut this Class_WP_Upgrader_Skin) feedback(var_feedback rt.PhpVal, var_args rt.PhpVal) {
	...
	rt.call_function('show_message', [var_feedback_mutated.clone()])
}
