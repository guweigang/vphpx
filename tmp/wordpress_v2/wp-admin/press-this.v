...
fn wp_load_press_this() {
	...
	var_plugin_slug = 'press-this'
	var_plugin_file = 'press-this/press-this-plugin.php'
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('edit_posts')])))))
		...
		rt.new_string(var_plugin_file.str()).clone()]))
	{
		rt.include_file(
	...
	]))
	{
		if rt.is_true(rt.call_function('file_exists', [
			rt.new_string((rt.get_constant('WP_PLUGIN_DIR')).str() + '/' + var_plugin_file),
		]))
		{
			var_url = rt.call_function('wp_nonce_url', [
				...
				rt.new_string('activate-plugin_' + var_plugin_file),
			])
			...
			if rt.is_true(rt.call_function('is_main_site', []rt.PhpVal{})) {
				var_url = rt.call_function('wp_nonce_url', [
					...
					rt.new_string('install-plugin_' + var_plugin_slug),
				])
				var_action = rt.call_function('sprintf', [
					rt.new_string('<a href="%1$s" class="install-now" data-slug="%2$s" data-name="%2$s" aria-label="%3$s">%3$s</a>'),
					rt.call_function('esc_url', [var_url.clone()]),
					rt.call_function('esc_attr', [rt.new_string(var_plugin_slug.str()).clone()]),
