...
fn main() {
	...
	rt.call_function('header', [
		rt.new_string('Content-Type: text/html; charset=' +
	...
	if !rt.is_true(rt.get_superglobal('_REQUEST').array_get(rt.new_string('action')))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_scalar', [rt.get_superglobal('_REQUEST').array_get(rt.new_string('action'))]))))) {
	...
	var_core_actions_post = rt.call_function('array_merge', [
		var_core_actions_post.clone(), rt.create_array_from_list(var_core_actions_post_deprecated)])
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('action'))))
		&& rt.is_true(rt.call_function('in_array', [rt.get_superglobal('_GET').array_get(rt.new_string('action')), rt.create_array_from_list(var_core_actions_get), rt.new_bool(true)])) {
		rt.call_function('add_action', [
			rt.new_string('wp_ajax_' +
				(rt.get_superglobal('_GET').array_get(rt.new_string('action'))).str()),
			rt.new_string('wp_ajax_' +(rt.call_function('str_replace', [rt.new_string('-'), rt.new_string('_'), rt.get_superglobal('_GET').array_get(rt.new_string('action'))])).str()),
			rt.new_int(1),
		])
	}
	if !(!rt.is_true(rt.get_superglobal('_POST').array_get(rt.new_string('action'))))
		&& rt.is_true(rt.call_function('in_array', [rt.get_superglobal('_POST').array_get(rt.new_string('action')), var_core_actions_post.clone(), rt.new_bool(true)])) {
		rt.call_function('add_action', [
			rt.new_string('wp_ajax_' +
				(rt.get_superglobal('_POST').array_get(rt.new_string('action'))).str()),
			rt.new_string('wp_ajax_' +(rt.call_function('str_replace', [rt.new_string('-'), rt.new_string('_'), rt.get_superglobal('_POST').array_get(rt.new_string('action'))])).str()),
			rt.new_int(1),
		])
	}
