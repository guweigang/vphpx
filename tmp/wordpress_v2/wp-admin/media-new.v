...
fn main() {
	...
	if rt.is_true(rt.get_superglobal('_POST')) {
		if rt.get_superglobal('_POST').array_isset(rt.new_string('html-upload'))
			...
			if rt.is_true(rt.call_function('is_wp_error', [var_upload_id.clone()])) {
				rt.call_function('wp_die', [var_upload_id.clone()])
			}
		}
	...
	if rt.is_true(rt.call_function('get_user_setting', [rt.new_string('uploader')]))
		|| rt.get_superglobal('_GET').array_isset(rt.new_string('browser-uploader')) {
		var_form_class = var_form_class + ' html-uploader'
	}
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [rt.new_string(var_form_class.str()).clone()]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('media_upload_form', []rt.PhpVal{})
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('absint', [var_post_id.clone()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('absint', [var_post_id.clone()]))
	// unsupported statement: Stmt_InlineHTML
