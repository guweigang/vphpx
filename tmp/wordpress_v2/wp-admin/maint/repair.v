...
fn main() {
	...
	// unsupported statement: Stmt_InlineHTML
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('WP_ALLOW_REPAIR')])))))
		...
		mut iter_1 := rt.create_array([rt.ArrayItem{ key: none, val: 'AUTH_KEY' },
			...
			rt.ArrayItem{ key: none, val: 'LOGGED_IN_KEY' }, rt.ArrayItem{
				key: none
				val: 'NONCE_KEY'
			}, rt.ArrayItem{ key: none, val: 'AUTH_SALT' }, rt.ArrayItem{
				key: none
				val: 'SECURE_AUTH_SALT'
			}, rt.ArrayItem{ key: none, val: 'LOGGED_IN_SALT' },
			rt.ArrayItem{ key: none, val: 'NONCE_SALT' }]).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_key := item_1.val
			if rt.is_true(rt.call_function('defined', [var_key.clone()])) {
				var_duplicated_keys.array_set(rt.call_function('constant', [
					var_key.clone()]), rt.new_bool(var_duplicated_keys.array_isset(rt.call_function('constant', [
					var_key.clone(),
				]))))
			} else {
		...
		mut iter_2 := var_default_keys.iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_default_key := item_2.val
		...
		var_duplicated_keys = rt.call_function('array_filter', [
			var_duplicated_keys.clone()])
		if rt.is_true(var_duplicated_keys) || var_missing_key {
		...
		mut var_optimize := rt.identical(rt.new_string('2'),
			rt.get_superglobal('_GET').array_get(rt.new_string('repair')))
		mut var_okay := true
		mut var_problems := rt.new_array()
		mut var_tables := rt.call_method(var_wpdb, 'tables', []rt.PhpVal{})
		var_tables = rt.call_function('array_merge', [var_tables.clone(),
		...
		for {
			...
			print('<p>')
			if rt.is_true(rt.identical(rt.new_string('OK'), rt.get_property(var_check, 'Msg_text'))) {
				rt.call_function('printf', [
			...
			if var_okay && rt.is_true(var_optimize) {
					...
					'Msg_text')))
				{
					...
					print('<br />&nbsp;&nbsp;&nbsp;&nbsp;')
					if rt.is_true(rt.identical(rt.new_string('OK'), rt.get_property(var_optimize, 'Msg_text')))
		...
		if rt.is_true(var_problems) {
			...
			mut iter_4 := var_problems.iterator()
			for {
				item_4 := iter_4.next() or { break }
				mut var_problem := item_4.val
				mut var_table := item_4.key
				var_problem_output = var_problem_output +
					'${var_table.to_string()}: ${var_problem.to_string()}\n'
			}
			print('<p><textarea name="errors" id="errors" rows="20" cols="60">' +
				(rt.call_function('esc_textarea', [rt.new_string(var_problem_output.str()).clone()])).str() +
				'</textarea></p>')
		} else {
			print('<p>' +
		...
		if rt.get_superglobal('_GET').array_isset(rt.new_string('referrer'))
			&& rt.is_true(rt.identical(rt.new_string('is_blog_installed'), rt.get_superglobal('_GET').array_get(rt.new_string('referrer')))) {
			print('<p>' +
