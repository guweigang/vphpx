...
fn main() {
	...
	mut var_redirect_user_admin_request := rt.new_bool(
	...
	var_redirect_user_admin_request = rt.call_function('apply_filters', [
		rt.new_string('redirect_user_admin_request'),
		var_redirect_user_admin_request.clone(),
	])
	if rt.is_true(var_redirect_user_admin_request) {
		rt.call_function('wp_redirect', [
			rt.call_function('user_admin_url', []rt.PhpVal{}),
		])
		exit(0)
	}
