...
fn main() {
	...
	mut var_vhttpdVendor := rt.call_function('getenv', [rt.new_string('VHTTPD_VENDOR')])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_vhttpdVendor, rt.new_bool(false)))))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_vhttpdVendor, rt.new_string(''))))) {
		rt.include_file(var_vhttpdVendor.clone().to_string().trim_right(' \t\n\r') +
			'/wordpress/vhttpd-db.php', '4')
	}
