...
fn Class_VHttpd_Wire_FrameCodec.read(var_conn rt.PhpVal, maxBytes i64) string {
	...
	mut var_unpacked := rt.call_function('unpack', [rt.new_string('Nsize'),
		var_header.clone()])
	mut var_size := rt.new_int((if !(var_unpacked.array_get(rt.new_string('size'))).is_null() {
		var_unpacked.array_get(rt.new_string('size'))
	} else {
		rt.new_int(0)
	}).to_i64())
	if rt.is_true(rt.less_equal(var_size, rt.new_int(0)))
		|| rt.is_true(rt.greater(var_size, rt.new_int(maxBytes))) {
		rt.throw_exception(rt.new_object('RuntimeException', []string{}, create_runtimeexception(
			'invalid frame size: ' + var_size.str())))
	}
	return (Class_VHttpd_Wire_FrameCodec.readexact(var_conn.to_i64(), var_size.clone())).str()
}

fn Class_VHttpd_Wire_FrameCodec.write(var_conn rt.PhpVal, payload string) {
	mut var_data := rt.new_string(
		(rt.call_function('pack', [rt.new_string('N'), rt.new_int(payload.len)])).str() + payload)
	...
	for rt.is_true(rt.less(var_written, var_size)) {
		mut var_n := rt.call_function('fwrite', [var_conn.clone(),
			rt.call_function('substr', [var_data.clone(), var_written.clone()])])
		if rt.is_true(rt.identical(var_n, rt.new_bool(false)))
			|| rt.is_true(rt.identical(var_n, rt.new_int(0))) {
			rt.throw_exception(rt.new_object('RuntimeException', []string{},
				create_runtimeexception(rt.new_string('failed to write frame'))))
		}
...
fn Class_VHttpd_Wire_FrameCodec.readexact(var_conn rt.PhpVal, size i64) string {
	...
	for var_buf.clone().to_string().len < size_mutated {
		mut var_chunk := rt.call_function('fread', [var_conn.clone(),
			rt.new_int(size_mutated - var_buf.clone().to_string().len)])
		if rt.is_true(rt.identical(var_chunk, rt.new_bool(false)))
			|| rt.is_true(rt.identical(var_chunk, rt.new_string(''))) {
			rt.throw_exception(rt.new_object('RuntimeException', []string{},
				create_runtimeexception(rt.new_string('unexpected EOF while reading frame'))))
		}
...
fn (mut this Class_VHttpd_Wire_FrameCodec) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'readExact' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).to_i64()
			return rt.new_string(Class_VHttpd_Wire_FrameCodec.readexact(dispatch_arg_0,
				dispatch_arg_1))
		}
		else {
			return none
