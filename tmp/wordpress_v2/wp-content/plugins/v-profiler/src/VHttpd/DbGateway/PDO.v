...
fn (mut this Class_VHttpd_DbGateway_PDO) intransaction() bool {
	return rt.new_bool(!rt.is_true(rt.identical(this.sessionId, rt.new_null())))
}

fn (mut this Class_VHttpd_DbGateway_PDO) begintransaction() bool {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(this.sessionId, rt.new_null())))) {
...
fn (mut this Class_VHttpd_DbGateway_PDO) prepare(sql string) rt.PhpVal {
	return rt.new_object('VHttpd_DbGateway_PDOStatement', []string{}, create_vhttpd_dbgateway_pdostatement(rt.new_object('VHttpd_DbGateway_PDO', []string{}, &this), rt.new_string(sql)))
}
...
fn (mut this Class_VHttpd_DbGateway_PDO) execute(sql string, mut var_bindings Class_VHttpd_DbGateway_array, timeoutMs i64) i64 {
	...
	return rt.new_int((if !(var_result.array_get(rt.new_string('affected_rows'))).is_null() { var_result.array_get(rt.new_string('affected_rows')) } else { rt.new_int(0) }).to_i64())
}
...
fn (mut this Class_VHttpd_DbGateway_PDO) isquerysql(sql string) bool {
	mut var_prefix := rt.new_string(if rt.is_true(rt.call_function('strtok', [rt.new_string(sql.trim_left(' \t\n\r')), rt.new_string(' \t\r\n(')])) { rt.call_function('strtok', [rt.new_string(sql.trim_left(' \t\n\r')), rt.new_string(' \t\r\n(')]) } else { rt.new_string('') }.to_string().to_upper())
	return (rt.call_function('in_array', [var_prefix.clone(), rt.create_array([rt.ArrayItem{ key: none, val: 'SELECT' }, rt.ArrayItem{ key: none, val: 'SHOW' }, rt.ArrayItem{ key: none, val: 'DESCRIBE' }, rt.ArrayItem{ key: none, val: 'EXPLAIN' }, rt.ArrayItem{ key: none, val: 'WITH' }, rt.ArrayItem{ key: none, val: 'PRAGMA' }]), rt.new_bool(true)])).to_bool()
}
...
fn create_vhttpd_dbgateway_client(_args ...rt.PhpVal) &Class_VHttpd_DbGateway_Client {
...
fn create_vhttpd_dbgateway_pdostatement(_args ...rt.PhpVal) &Class_VHttpd_DbGateway_PDOStatement {
