...
fn (mut this Class_VHttpd_PhpWorker_Client) construct(socketPath string, connectTimeoutSeconds f64, readTimeoutSeconds f64) {
	this.wire = create_vhttpd_wire_jsonclient(rt.new_string(socketPath), rt.new_string('worker'),
		rt.new_float(connectTimeoutSeconds), rt.new_float(readTimeoutSeconds))
}
...
fn (mut this Class_VHttpd_PhpWorker_Client) request(mut var_request Class_VHttpd_PhpWorker_Request) rt.PhpVal {
	mut iife_temp_0 := Class_VHttpd_PhpWorker_Response{}
	mut iife_result_0 := iife_temp_0.fromarray(rt.call_method(this.wire, 'request', [
		var_request.toarray(),
	]))
...
fn create_vhttpd_phpworker_response(_args ...rt.PhpVal) &Class_VHttpd_PhpWorker_Response {
