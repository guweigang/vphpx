...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCacheController) orders_cache_usage_is_temporarly_disabled() bool {
	return rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), this.orders_cache_usage_backup)))
}
...
fn create_automattic_woocommerce_caches_ordercachecontroller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Caches_OrderCacheController {
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCacheController) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Caches_OrderCache](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCacheController) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'orders_cache_usage_backup' {
			this.orders_cache_usage_backup = val
			return true
		}
		else {
