...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCacheService) refresh_cache(var_order_type rt.PhpVal) {
	...
	rt.call_method(this.order_count_cache, 'flush', [var_order_type_mutated.clone()])
	mut iife_temp_0 := Class_Automattic_WooCommerce_Utilities_OrderUtil{}
	mut iife_result_0 := iife_temp_0.get_count_for_type(var_order_type_mutated.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCacheService) prime_cache_if_cold(var_order_type rt.PhpVal) {
	...
	if rt.is_true(rt.call_function('wp_using_ext_object_cache', []rt.PhpVal{}))
		&& rt.is_true(rt.identical(rt.new_null(), rt.call_method(this.order_count_cache, 'get', [var_order_type_mutated.clone()]))) {
		rt.call_method(this.order_count_cache, 'flush', [var_order_type_mutated.clone()])
		mut iife_temp_1 := Class_Automattic_WooCommerce_Utilities_OrderUtil{}
		mut iife_result_1 := iife_temp_1.get_count_for_type(var_order_type_mutated.clone())
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCacheService) schedule_background_actions() {
	...
	for {
		...
		rt.call_function('as_schedule_recurring_action', [var_timestamp.clone(),
			var_frequency.clone(),
			Class_Automattic_WooCommerce_Caches_Automattic_WooCommerce_Caches_OrderCountCacheService.background_event_hook(),
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCacheService) update_on_new_order(var_order_id rt.PhpVal, var_order rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.order_count_cache, 'is_cached', [
		...
		this.get_prefixed_status(var_order_status.clone()),
	])))))
	{
		return
	}
	...
	if this.order_statuses.array_isset(var_order_id)
		&& rt.is_true(rt.identical(this.order_statuses.array_get(var_order_id), var_order_status)) {
		return
	}
	this.order_statuses.array_set(var_order_id, var_order_status.clone())
	rt.call_method(this.order_count_cache, 'increment', [var_order_type.clone(),
		this.get_prefixed_status(var_order_status.clone())])
}
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCacheService) update_on_order_trashed(var_order_id rt.PhpVal, var_order rt.PhpVal) {
	mut var_order_type := rt.call_method(var_order, 'get_type', []rt.PhpVal{})
	mut var_order_status := rt.call_method(var_order, 'get_status', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.order_count_cache, 'is_cached', [var_order_type.clone(), this.get_prefixed_status(var_order_status.clone())])))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.order_count_cache, 'is_cached', [var_order_type.clone(), Class_Automattic_WooCommerce_Enums_OrderStatus.trash()]))))) {
		return
	}
	rt.call_method(this.order_count_cache, 'decrement', [var_order_type.clone(),
		this.get_prefixed_status(var_order_status.clone())])
	rt.call_method(this.order_count_cache, 'increment', [var_order_type.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Caches_OrderCountCacheService) update_on_order_status_changed(var_order_id rt.PhpVal, var_previous_status rt.PhpVal, var_next_status rt.PhpVal, var_order rt.PhpVal) {
	mut var_order_type := rt.call_method(var_order, 'get_type', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.order_count_cache, 'is_cached', [var_order_type.clone(), this.get_prefixed_status(var_next_status.clone())])))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.order_count_cache, 'is_cached', [var_order_type.clone(), this.get_prefixed_status(var_previous_status.clone())]))))) {
		return
	}
	if this.order_statuses.array_isset(var_order_id)
	...
	this.order_statuses.array_set(var_order_id, var_next_status.clone())
	mut var_was_decremented := rt.call_method(this.order_count_cache, 'decrement', [
		var_order_type.clone(),
		this.get_prefixed_status(var_previous_status.clone()),
	])
	rt.call_method(this.order_count_cache, 'increment', [var_order_type.clone(),
		this.get_prefixed_status(var_next_status.clone())])
	if !(this.initial_order_statuses.array_isset(var_order_id)) && rt.is_true(var_was_decremented) {
		this.initial_order_statuses.array_set(var_order_id, var_previous_status.clone())
	}
}
...
fn create_automattic_woocommerce_caches_ordercountcacheservice(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Caches_OrderCountCacheService {
