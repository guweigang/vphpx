...
pub fn Class_Automattic_WooCommerce_Caching_ObjectCache.default_expiration() i64 {
	return -1
}
...
fn (mut this Class_Automattic_WooCommerce_Caching_ObjectCache) get_cache_engine() rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_null(), this.cache_engine)) {
		...
		this.cache_engine = rt.call_function('apply_filters', [rt.new_string('wc_object_cache_get_engine'), var_engine.clone(), rt.new_object('Automattic_WooCommerce_Caching_ObjectCache', []string{}, &this)])
	}
...
fn (mut this Class_Automattic_WooCommerce_Caching_ObjectCache) set(var_object rt.PhpVal, var_id rt.PhpVal, expiration i64) bool {
	...
	if rt.is_true(rt.identical(rt.new_null(), var_object_mutated)) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Caching_CacheException', []string{}, create_automattic_woocommerce_caching_cacheexception(rt.new_string('Can\'t cache a null value'), rt.new_object('Automattic_WooCommerce_Caching_ObjectCache', []string{}, &this), var_id_mutated.clone())))
	}
	if !(var_object_mutated.clone().is_array()) && !(var_object_mutated.clone().is_object()) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Caching_CacheException', []string{}, create_automattic_woocommerce_caching_cacheexception(rt.new_string('Can\'t cache a non-object, non-array value'), rt.new_object('Automattic_WooCommerce_Caching_ObjectCache', []string{}, &this), var_id_mutated.clone())))
	}
	if !(var_id_mutated.clone().is_string()) && !(var_id_mutated.clone().is_long()) && !(var_id_mutated.clone().is_null()) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Caching_CacheException', []string{}, create_automattic_woocommerce_caching_cacheexception(rt.new_string('Object id must be an int, a string, or null for \'set\''), rt.new_object('Automattic_WooCommerce_Caching_ObjectCache', []string{}, &this), var_id_mutated.clone())))
	}
	this.verify_expiration_value(expiration)
	mut var_errors := this.validate(var_object_mutated.clone())
	if !(var_errors.clone().is_null()) {
		var_id_mutated = this.get_id_from_object_if_null(var_object_mutated.clone(), var_id_mutated.clone())
...
end_label_1:
		if var_errors.clone().array_count() == 1 {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Caching_CacheException', []string{}, create_automattic_woocommerce_caching_cacheexception('Object validation/serialization failed: ' + (var_errors.array_get(rt.new_int(0))).str(), rt.new_object('Automattic_WooCommerce_Caching_ObjectCache', []string{}, &this), var_id_mutated.clone(), var_errors.clone())))
		} else if !(!rt.is_true(var_errors)) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Caching_CacheException', []string{}, create_automattic_woocommerce_caching_cacheexception(rt.new_string('Object validation/serialization failed'), rt.new_object('Automattic_WooCommerce_Caching_ObjectCache', []string{}, &this), var_id_mutated.clone(), var_errors.clone())))
		}
	}
	var_id_mutated = this.get_id_from_object_if_null(var_object_mutated.clone(), var_id_mutated.clone())
	this.last_cached_data = var_object_mutated.clone()
	return (rt.call_method(this.get_cache_engine(), 'cache_object', [var_id_mutated.clone(), var_object_mutated.clone(), if rt.is_true(rt.identical(Class_Automattic_WooCommerce_Caching_Automattic_WooCommerce_Caching_ObjectCache.default_expiration(), rt.new_int(expiration))) { this.default_expiration } else { rt.new_int(expiration) }, rt.new_string(this.get_object_type())])).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Caching_ObjectCache) update_if_cached(var_object rt.PhpVal, var_id rt.PhpVal, expiration i64) bool {
	...
	mut var_id_mutated := var_id
	var_id_mutated = this.get_id_from_object_if_null(var_object_mutated.clone(), var_id_mutated.clone())
	if !(this.is_cached(var_id_mutated.clone())) {
		return false
	}
	return this.set(var_object_mutated.clone(), var_id_mutated.clone(), expiration)
}
...
fn (mut this Class_Automattic_WooCommerce_Caching_ObjectCache) get_id_from_object_if_null(var_object rt.PhpVal, var_id rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_null(), var_id_mutated)) {
		var_id_mutated = this.get_object_id(var_object_mutated.clone())
		if rt.is_true(rt.identical(rt.new_null(), var_id_mutated)) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Caching_CacheException', []string{}, create_automattic_woocommerce_caching_cacheexception(rt.new_string('Null id supplied and the cache class doesn\'t implement get_object_id'), rt.new_object('Automattic_WooCommerce_Caching_ObjectCache', []string{}, &this))))
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Caching_ObjectCache) verify_expiration_value(expiration i64) {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Caching_Automattic_WooCommerce_Caching_ObjectCache.default_expiration(), rt.new_int(expiration))))) && expiration < 1 || rt.is_true(rt.greater(rt.new_int(expiration), Class_Automattic_WooCommerce_Caching_Automattic_WooCommerce_Caching_ObjectCache.max_expiration())) {
...
fn (mut this Class_Automattic_WooCommerce_Caching_ObjectCache) get(var_id rt.PhpVal, expiration i64, mut var_get_from_datastore_callback Class_Automattic_WooCommerce_Caching_?callable) rt.PhpVal {
	mut var_id_mutated := var_id
	if !(var_id_mutated.clone().is_string()) && !(var_id_mutated.clone().is_long()) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Caching_CacheException', []string{}, create_automattic_woocommerce_caching_cacheexception(rt.new_string('Object id must be an int or a string for \'get\''), rt.new_object('Automattic_WooCommerce_Caching_ObjectCache', []string{}, &this))))
	}
	this.verify_expiration_value(expiration)
	mut var_data := rt.call_method(this.get_cache_engine(), 'get_cached_object', [var_id_mutated.clone(), rt.new_string(this.get_object_type())])
	if rt.is_true(rt.identical(rt.new_null(), var_data)) {
		mut var_object := rt.new_null()
		if rt.is_true(var_get_from_datastore_callback) {
		var_object = rt.call_callable(var_get_from_datastore_callback, [var_id_mutated.clone()])
		}
		if rt.is_true(rt.identical(rt.new_null(), var_object)) {
			return rt.new_null()
		}
		this.set(var_object.clone(), var_id_mutated.clone(), expiration)
...
fn (mut this Class_Automattic_WooCommerce_Caching_ObjectCache) remove(var_id rt.PhpVal) bool {
	mut var_id_mutated := var_id
	return (rt.call_method(this.get_cache_engine(), 'delete_cached_object', [var_id_mutated.clone(), rt.new_string(this.get_object_type())])).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Caching_ObjectCache) flush() bool {
	return (rt.call_method(this.get_cache_engine(), 'delete_cache_group', [rt.new_string(this.get_object_type())])).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Caching_ObjectCache) is_cached(var_id rt.PhpVal) bool {
	mut var_id_mutated := var_id
	return (rt.call_method(this.get_cache_engine(), 'is_cached', [var_id_mutated.clone(), rt.new_string(this.get_object_type())])).to_bool()
}
...
fn create_automattic_woocommerce_caching_cacheexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Caching_CacheException {
