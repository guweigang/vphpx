...
fn (mut this Class_Automattic_WooCommerce_Blocks_AIContent_UpdateProducts) fetch_dummy_products_to_update() rt.PhpVal {
	...
	mut var_real_products_count := rt.new_int(var_real_products.clone().array_count())
	if var_real_products.clone().is_array()
	...
	mut var_dummy_products_count := rt.new_int(var_dummy_products.clone().array_count())
	...
	if !(var_dummy_products_ids.clone().is_array()) {
	...
	for {
		...
		mut var_should_update_dummy_product :=
			rt.new_bool(this.should_update_dummy_product(var_dummy_product.clone()))
		if rt.is_true(var_should_update_dummy_product) {
			var_dummy_products_to_update.array_push(var_dummy_product.clone())
		}
	}
	return var_dummy_products_to_update.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_AIContent_UpdateProducts) should_update_dummy_product(var_dummy_product rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_date_created, 'Automattic_WooCommerce_Blocks_AIContent_WC_DateTime'))))))
	...
	mut var_timestamp_created := rt.call_function('strtotime', [
		var_formatted_date_created.clone()])
	mut var_timestamp_modified := rt.call_function('strtotime', [
		var_formatted_date_modified.clone()])
	...
	if rt.is_true(var_dummy_product_not_modified) || rt.is_true(var_dummy_product_recently_modified) {
		return true
	}
	return false
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_AIContent_UpdateProducts) create_new_product(var_product_data rt.PhpVal) rt.PhpVal {
	...
	mut var_image_src := rt.call_function('plugins_url', [
		var_product_data.array_get(rt.new_string('image')),
		rt.call_function('dirname', [rt.new_string(@DIR), rt.new_int(2)]),
	])
	mut var_image_alt := var_product_data.array_get(rt.new_string('title'))
	mut var_product_image_id := this.product_image_upload(var_product.get_id(),
		var_image_src.clone(), var_image_alt.clone())
	mut var_saved_product := this.product_update(rt.new_object('Automattic_WooCommerce_Blocks_AIContent_WC_Product',
		[]string{}, var_product), var_product_image_id.clone(),
		var_product_data.array_get(rt.new_string('title')),
		var_product_data.array_get(rt.new_string('description')),
		var_product_data.array_get(rt.new_string('price')))
	if rt.is_true(rt.call_function('is_wp_error', [var_saved_product.clone()])) {
		return var_saved_product.clone()
	}
	return rt.call_function('update_post_meta', [var_saved_product.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_AIContent_UpdateProducts) product_image_upload(var_product_id rt.PhpVal, var_image_src rt.PhpVal, var_image_alt rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('media_sideload_image', [var_image_src_mutated.clone(),
		var_product_id.clone(), var_image_alt_mutated.clone(),
		rt.new_string('id')])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_AIContent_UpdateProducts) product_update(var_product rt.PhpVal, var_product_image_id rt.PhpVal, var_product_title rt.PhpVal, var_product_description rt.PhpVal, var_product_price rt.PhpVal) rt.PhpVal {
	...
	if !(true) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('invalid_product'), rt.call_function('__', [
			rt.new_string('Invalid product.'),
			rt.new_string('woocommerce'),
		])))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_wp_error', [
		var_product_image_id_mutated.clone()])))))
	{
		var_product_mutated.set_image_id(var_product_image_id_mutated.clone())
	} else {
	...
	var_product_mutated.set_name(var_product_title.clone())
	var_product_mutated.set_description(var_product_description.clone())
	var_product_mutated.set_price(var_product_price.clone())
	var_product_mutated.set_regular_price(var_product_price.clone())
	var_product_mutated.set_slug(rt.call_function('sanitize_title', [
		var_product_title.clone()]))
...
fn create_automattic_woocommerce_blocks_aicontent_wc_product(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_AIContent_WC_Product {
