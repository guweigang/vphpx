...
fn (mut this Class_Automattic_WooCommerce_Blocks_Shipping_PickupLocation) has_valid_pickup_location(var_address rt.PhpVal) bool {
	...
	if !rt.is_true(var_address_fields.array_get(rt.new_string('country'))) {
		return false
	}
	if !(!rt.is_true(var_address_fields.array_get(rt.new_string('city'))))
		&& !(!rt.is_true(var_address_fields.array_get(rt.new_string('postcode'))))
		&& !(!rt.is_true(var_address_fields.array_get(rt.new_string('state')))) {
		return true
	}
	mut var_country_address_fields := rt.call_method(rt.get_property(rt.call_function('wc',
		[]rt.PhpVal{}), 'countries'), 'get_address_fields', [
		var_address_fields.array_get(rt.new_string('country')),
		rt.new_string('shipping_'),
	])
	...
	for {
		...
		mut var_key := rt.call_function('str_replace', [rt.new_string('shipping_'),
			rt.new_string(''), var_field_name.clone()])
		if var_address_fields.array_isset(var_key)
			&& rt.is_true(rt.identical(rt.new_bool(true), var_field.array_get(rt.new_string('required'))))
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Shipping_PickupLocation) calculate_shipping(var_package rt.PhpVal) {
	if rt.is_true(this.pickup_locations) {
		mut iter_2 := this.pickup_locations.iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_location := item_2.val
			mut var_index := item_2.key
			if rt.is_true(rt.new_bool(!(rt.is_true(var_location.array_get(rt.new_string('enabled')))))) {
				continue
			}
			this.add_rate(rt.create_array([
				...
				rt.ArrayItem{ key: 'label', val: rt.call_function('wp_kses_post', [
					rt.new_string(
						...
						' (' + (var_location.array_get(rt.new_string('name'))).str() + ')'),
				]) },
				...
				rt.ArrayItem{ key: 'meta_data', val: rt.create_array([
					rt.ArrayItem{ key: 'pickup_location', val: rt.call_function('wp_kses_post', [
						var_location.array_get(rt.new_string('name')),
					]) },
					rt.ArrayItem{
						key: 'pickup_address'
						val: if this.has_valid_pickup_location(var_location.array_get(rt.new_string('address'))) { rt.call_method(rt.get_property(rt.call_function('wc', []rt.PhpVal{}), 'countries'), 'get_formatted_address', [
								var_location.array_get(rt.new_string('address')),
								rt.new_string(', '),
							]) } else { rt.new_string('') }
					},
					rt.ArrayItem{ key: 'pickup_details', val: rt.call_function('wp_kses_post', [
						var_location.array_get(rt.new_string('details')),
					]) },
				]) },
			]))
		}
