...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ArchiveProductTemplatesCompatibility) update_render_block_data(var_parsed_block rt.PhpVal, var_source_block rt.PhpVal, var_parent_block rt.PhpVal) rt.PhpVal {
	...
	this.inner_blocks_walker(var_parsed_block.clone())
	return var_parsed_block.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ArchiveProductTemplatesCompatibility) inject_hooks(var_block_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	if !rt.is_true(var_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('isInherited'))) {
		return var_block_content.clone()
	}
	mut var_block_name := var_block.array_get(rt.new_string('blockName'))
	if this.is_null_post_template(var_block.clone()) {
		var_block_name =
	...
	if this.is_post_or_product_template(var_block_name.clone()) && !(!rt.is_true(var_block_content)) {
		...
		mut var_content := rt.call_function('sprintf', [rt.new_string('%1$s%2$s%3$s'),
			...
			this.get_hooks_buffer(var_block_hooks.clone(),
				rt.new_string('after'))])
	...
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_hook := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return var_hook.array_get(rt.new_string('block_names'))
	}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_block_name.clone(), var_supported_blocks.clone(),
		rt.new_bool(true)])))))
	{
		return var_block_content.clone()
	}
	if rt.is_true(rt.identical(rt.new_string('core/query-no-results'), var_block_name)) {
		if var_block_content.clone().to_string().trim_space() == '' {
			return var_block_content.clone()
		}
		this.restore_default_hooks()
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ArchiveProductTemplatesCompatibility) inner_blocks_walker(var_block rt.PhpVal) {
	if this.is_products_block_with_inherit_query(var_block.clone())
		|| this.is_product_collection_block_with_inherit_query(var_block.clone()) {
		this.inject_attribute(var_block.clone())
		this.remove_default_hooks()
	}
	if !(!rt.is_true(var_block.array_get(rt.new_string('innerBlocks')))) {
		rt.call_function('array_walk', [var_block.array_get(rt.new_string('innerBlocks')),
			rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ArchiveProductTemplatesCompatibility) restore_default_hooks() {
	...
	for {
		...
		mut iter_2 := var_data.array_get(rt.new_string('hooked')).iterator()
		for {
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
				var_callback.clone(),
				if !(var_data.array_get(rt.new_string('permanently_removed_actions'))).is_null() {
					var_data.array_get(rt.new_string('permanently_removed_actions'))
				} else {
					rt.new_array()
				},
				rt.new_bool(true),
			])))))
			{
				rt.call_function('add_action', [var_hook.clone(),
					var_callback.clone(), var_priority.clone()])
			}
		}
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ArchiveProductTemplatesCompatibility) is_block_within_namespace(var_block rt.PhpVal) bool {
	mut var_attributes := var_block.array_get(rt.new_string('attrs'))
	return var_attributes.array_isset(rt.new_string('__woocommerceNamespace'))
		&& rt.is_true(rt.identical(rt.new_string('woocommerce/product-query/product-template'), var_attributes.array_get(rt.new_string('__woocommerceNamespace'))))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ArchiveProductTemplatesCompatibility) is_block_inherited(var_block rt.PhpVal) rt.PhpVal {
	mut var_attributes := var_block.array_get(rt.new_string('attrs'))
	mut var_outcome := rt.new_bool(var_attributes.array_isset(rt.new_string('isInherited'))
		&& rt.is_true(rt.identical(rt.new_int(1), var_attributes.array_get(rt.new_string('isInherited')))))
	return var_outcome.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ArchiveProductTemplatesCompatibility) is_null_post_template(var_block rt.PhpVal) bool {
	...
	return rt.is_true(rt.identical(rt.new_string('core/null'), var_block_name))
		&& rt.is_true(this.is_block_inherited(var_block.clone()))
		|| this.is_block_within_namespace(var_block.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ArchiveProductTemplatesCompatibility) is_post_or_product_template(var_block_name rt.PhpVal) bool {
	mut var_block_name_mutated := var_block_name
	return rt.is_true(this.is_post_template(var_block_name_mutated.clone()))
		|| rt.is_true(this.is_product_template(var_block_name_mutated.clone()))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ArchiveProductTemplatesCompatibility) is_products_block_with_inherit_query(var_block rt.PhpVal) bool {
	return
		rt.is_true(rt.identical(rt.new_string('core/query'), var_block.array_get(rt.new_string('blockName'))))
		&& var_block.array_get(rt.new_string('attrs')).array_isset(rt.new_string('namespace'))
		&& rt.is_true(rt.identical(rt.new_string('woocommerce/product-query'), var_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('namespace'))))
		&& var_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('query')).array_isset(rt.new_string('inherit'))
		&& rt.is_true(var_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('query')).array_get(rt.new_string('inherit')))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ArchiveProductTemplatesCompatibility) is_product_collection_block_with_inherit_query(var_block rt.PhpVal) bool {
	return
		rt.is_true(rt.identical(rt.new_string('woocommerce/product-collection'), var_block.array_get(rt.new_string('blockName'))))
		&& var_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('query')).array_isset(rt.new_string('inherit'))
		&& rt.is_true(var_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('query')).array_get(rt.new_string('inherit')))
}
