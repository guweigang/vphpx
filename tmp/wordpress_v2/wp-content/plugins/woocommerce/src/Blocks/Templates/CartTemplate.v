...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_CartTemplate) get_template_title() rt.PhpVal {
	return rt.call_function('_x', [rt.new_string('Page: Cart'),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_CartTemplate) get_template_description() rt.PhpVal {
	return rt.call_function('__', [
		rt.new_string('The Cart template displays the items selected by the user for purchase, including quantities, prices, and discounts. It allows users to review their choices before proceeding to checkout.'),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_CartTemplate) page_template_hierarchy(var_templates rt.PhpVal) rt.PhpVal {
	if this.is_active_template() {
		rt.call_function('array_unshift', [var_templates.clone(),
			Class_Automattic_WooCommerce_Blocks_Templates_Automattic_WooCommerce_Blocks_Templates_CartTemplate.slug()])
		rt.call_function('array_unshift', [var_templates.clone(),
			rt.new_string('cart')])
	}
