...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_AbstractTemplateCompatibility) init() {
	...
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_parsed_block := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_source_block := if args.len > 1 { args[1].clone() } else { rt.new_null() }
		mut var_parent_block := if args.len > 2 { args[2].clone() } else { rt.new_null() }
	...
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_block_content := if args.len > 0 { args[0].clone() } else { rt.new_null() }
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_AbstractTemplateCompatibility) remove_default_hooks() {
	...
	for {
		...
		for {
			...
			rt.call_function('remove_action', [var_hook.clone(),
	...
	if !rt.is_true(var_additional_hook_data) || !(var_additional_hook_data.clone().is_array()) {
		return
	}
	mut iter_3 := var_additional_hook_data.iterator()
	for {
		...
		rt.call_function('remove_action', [var_data.array_get(rt.new_string('hook')),
			var_data.array_get(rt.new_string('function')), var_data.array_get(rt.new_string('priority'))])
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_AbstractTemplateCompatibility) get_hooks_buffer(var_hooks rt.PhpVal, var_position rt.PhpVal) rt.PhpVal {
	...
	mut iter_4 := var_hooks.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_data := item_4.val
		mut var_hook := item_4.key
		if rt.is_true(rt.identical(var_data.array_get(rt.new_string('position')), var_position)) {
			rt.call_function('do_action', [var_hook.clone()])
		}
	}
