...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_AbstractTemplateWithFallback) init() {
	rt.call_function('add_filter', [rt.new_string('taxonomy_template_hierarchy'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Blocks_Templates_AbstractTemplateWithFallback', [
				'Automattic_WooCommerce_Blocks_Templates_AbstractTemplate',
			], &this) },
			rt.ArrayItem{ key: none, val: 'template_hierarchy' },
		]),
		rt.new_int(1)])
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_AbstractTemplateWithFallback) template_hierarchy(var_templates rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_bool(false), var_index)) {
		var_index = rt.call_function('array_search', [
			rt.new_string(
				(Class_Automattic_WooCommerce_Blocks_Templates_static.slug()).str() + '.php'),
			var_templates.clone(),
			rt.new_bool(true),
		])
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_index))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_templates.clone().array_isset(rt.add(var_index, rt.new_int(1))))))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_templates.array_get(rt.add(var_index, rt.new_int(1))), this.fallback_template)))) {
		rt.call_function('array_splice', [var_templates.clone(),
			rt.add(var_index, rt.new_int(1)), rt.new_int(0), this.fallback_template])
	}
