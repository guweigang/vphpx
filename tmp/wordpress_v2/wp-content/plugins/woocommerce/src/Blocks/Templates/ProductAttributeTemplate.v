...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ProductAttributeTemplate) render_block_template() {
	...
	if !(rt.get_property(var_queried_object, 'taxonomy')).is_null()
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Templates_ProductAttributeTemplate) template_hierarchy(var_templates rt.PhpVal) rt.PhpVal {
	...
	mut var_queried_object := rt.call_function('get_queried_object', []rt.PhpVal{})
	if !(var_queried_object.clone().is_null())
		&& rt.is_true(rt.call_function('taxonomy_is_product_attribute', [rt.get_property(var_queried_object, 'taxonomy')]))
		...
		mut iife_result_1 :=
			iife_temp_1.theme_has_template(Class_Automattic_WooCommerce_Blocks_Templates_Automattic_WooCommerce_Blocks_Templates_ProductAttributeTemplate.slug())
		mut iife_temp_2 := Class_Automattic_WooCommerce_Blocks_Utils_BlockTemplateUtils{}
		mut iife_result_2 := iife_temp_2.get_block_templates_from_db(rt.create_array([
			rt.ArrayItem{
				key: none
				val: Class_Automattic_WooCommerce_Blocks_Templates_Automattic_WooCommerce_Blocks_Templates_ProductAttributeTemplate.slug()
			},
		]))
		if rt.is_true(iife_result_1) || rt.is_true(iife_result_2) {
		...
		rt.call_function('array_splice', [var_templates_mutated.clone(),
			rt.new_int(var_templates_mutated.clone().array_count() - 1),
			rt.new_int(0), var_slugs.clone()])
	}
