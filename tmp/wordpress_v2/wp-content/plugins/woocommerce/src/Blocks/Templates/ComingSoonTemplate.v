...
fn Class_Automattic_WooCommerce_Blocks_Templates_ComingSoonTemplate.get_font_families() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_is_block_theme', []rt.PhpVal{}))))) {
		return var_default_fonts.clone()
	}
	...
	if !(var_settings.array_get(rt.new_string('typography')).array_get(rt.new_string('fontFamilies')).array_isset(rt.new_string('theme')))
		|| !(var_settings.array_get(rt.new_string('typography')).array_get(rt.new_string('fontFamilies')).array_get(rt.new_string('theme')).is_array()) {
		return var_default_fonts.clone()
	}
	mut var_theme_fonts :=
		var_settings.array_get(rt.new_string('typography')).array_get(rt.new_string('fontFamilies')).array_get(rt.new_string('theme'))
	if var_theme_fonts.array_get(rt.new_int(0)).array_isset(rt.new_string('slug'))
		&& !(!rt.is_true(var_theme_fonts.array_get(rt.new_int(0)).array_get(rt.new_string('slug')))) {
		var_default_fonts.array_set('heading', rt.call_function('str_replace', [
			rt.new_string(' '),
			rt.new_string('-'),
			var_theme_fonts.array_get(rt.new_int(0)).array_get(rt.new_string('slug')),
		]).to_string().to_lower())
	}
	if var_theme_fonts.array_get(rt.new_int(1)).array_isset(rt.new_string('slug'))
		&& !(!rt.is_true(var_theme_fonts.array_get(rt.new_int(1)).array_get(rt.new_string('slug')))) {
		var_default_fonts.array_set('body', rt.call_function('str_replace', [
			rt.new_string(' '),
			rt.new_string('-'),
			var_theme_fonts.array_get(rt.new_int(1)).array_get(rt.new_string('slug')),
		]).to_string().to_lower())
		var_default_fonts.array_set('paragraph', var_default_fonts.array_get(rt.new_string('body')))
	}
	return var_default_fonts.clone()
}
