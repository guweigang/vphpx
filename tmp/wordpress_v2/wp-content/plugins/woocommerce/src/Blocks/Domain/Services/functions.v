...
fn woocommerce_register_additional_checkout_field(var_options rt.PhpVal) {
	mut var_woocommerce_blocks_loaded_ran := rt.new_null()
	mut var_checkout_fields := rt.new_null()
	mut var_result := rt.new_null()
	var_woocommerce_blocks_loaded_ran = rt.call_function('did_action', [
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_woocommerce_blocks_loaded_ran)))) {
		closure_1_fn := fn [var_options] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			woocommerce_register_additional_checkout_field(var_options.clone())
			return rt.new_null()
		}
	...
	var_checkout_fields = rt.call_method(iife_result_1, 'get', [
		Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFields.class(),
	])
	var_result = rt.call_method(var_checkout_fields, 'register_checkout_field', [
		var_options.clone(),
	])
...
fn __internal_woocommerce_blocks_deregister_checkout_field(var_field_id rt.PhpVal) {
	...
	var_checkout_fields = rt.call_method(iife_result_2, 'get', [
		Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutFields.class(),
	])
	var_result = rt.call_method(var_checkout_fields, 'deregister_checkout_field', [
		var_field_id.clone(),
	])
	if rt.is_true(rt.call_function('is_wp_error', [var_result.clone()])) {
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.call_function('esc_attr', [
