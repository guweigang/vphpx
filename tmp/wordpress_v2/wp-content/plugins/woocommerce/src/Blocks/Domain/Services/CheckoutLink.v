...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutLink) init() {
	rt.call_function('add_action', [rt.new_string('init'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Blocks_Domain_Services_CheckoutLink',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'add_checkout_link_endpoint' },
		])])
	...
	rt.call_function('add_action', [rt.new_string('template_redirect'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Blocks_Domain_Services_CheckoutLink',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'handle_checkout_link_endpoint' },
		])])
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutLink) add_checkout_link_endpoint() {
	mut var_rules := rt.call_function('get_option', [rt.new_string('rewrite_rules'),
		rt.new_array()])
	mut var_regex := rt.new_string('^checkout-link$')
	rt.call_function('add_rewrite_rule', [var_regex.clone(),
		rt.new_string('index.php?checkout-link=true'), rt.new_string('top')])
	...
	if !(this.validate_checkout_link()) {
		mut var_redirect := rt.call_function('add_query_arg', [
			...
			rt.call_function('rawurlencode', [
				rt.call_function('__', [
					rt.new_string('The provided checkout link was out of date or invalid. No products were added to the cart.'),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutLink) get_products_from_checkout_link() rt.PhpVal {
	mut var_raw_products := rt.call_function('array_filter', [
		rt.call_function('explode', [rt.new_string(','),
			rt.call_function('wc_clean', [
				rt.call_function('wp_unslash', [if !(rt.get_superglobal('_GET').array_get(rt.new_string('products'))).is_null() {
					rt.get_superglobal('_GET').array_get(rt.new_string('products'))
				} else {
					rt.new_string('')
				}]),
			])]),
	...
	for {
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('strpos', [
			var_product_id_qty.clone(),
			rt.new_string(':'),
		]), rt.new_bool(false)))))
		{
			mut list_tmp_1 := rt.call_function('explode', [rt.new_string(':'),
				var_product_id_qty.clone()])
			mut var_product_id := list_tmp_1.array_get(0)
			mut var_qty := list_tmp_1.array_get(1)
		} else {
			var_product_id = var_product_id_qty
			var_qty = rt.new_int(1)
		}
		var_product_id = rt.call_function('absint', [var_product_id.clone()])
		var_qty = rt.call_function('absint', [var_qty.clone()])
		if rt.is_true(rt.new_bool(!(rt.is_true(var_product_id))))
			|| rt.is_true(rt.new_bool(!(rt.is_true(var_qty)))) {
			continue
		}
		var_products.array_set(var_product_id, var_qty.clone())
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutLink) add_error_notices(mut var_errors Class_Automattic_WooCommerce_Blocks_Domain_Services_WP_Error) {
	...
	mut iter_2 := var_errors_mutated.get_error_messages().iterator()
	for {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutLink) get_checkout_link() rt.PhpVal {
	...
	mut iter_3 := var_products.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_qty := item_3.val
		mut var_product_id := item_3.key
		var_controller.add_to_cart(rt.create_array([
			rt.ArrayItem{ key: 'id', val: var_product_id },
			rt.ArrayItem{ key: 'quantity', val: var_qty },
		]))
		if rt.has_exception() {
			unsafe {
				goto catch_label_1
		...
		if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Blocks_Domain_Services_Exception') {
			...
			var_errors.add(rt.new_string('error'), rt.call_method(var_e, 'getMessage',
				[]rt.PhpVal{}))
			unsafe {
				goto end_label_1
			}
	...
	if rt.is_true(rt.call_method(rt.get_property(rt.call_function('wc', []rt.PhpVal{}), 'cart'),
		'is_empty', []rt.PhpVal{}))
	{
		var_errors.add(rt.new_string('error'), rt.call_function('__', [
			rt.new_string('The provided checkout link was out of date or invalid. No products were added to the cart.'),
			rt.new_string('woocommerce'),
		]))
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(rt.get_property(rt.call_function('wc',
			[]rt.PhpVal{}), 'session'), 'has_session', []rt.PhpVal{})))))
		{
			return rt.call_function('add_query_arg', [rt.new_string('wc_error'),
				rt.call_function('rawurlencode', [var_errors.get_error_message()]),
				rt.call_function('wc_get_cart_url', []rt.PhpVal{})])
		} else {
	...
	mut var_coupon := rt.call_function('wc_format_coupon_code', [
		rt.call_function('wp_unslash', [if !(rt.get_superglobal('_GET').array_get(rt.new_string('coupon'))).is_null() {
			rt.get_superglobal('_GET').array_get(rt.new_string('coupon'))
		} else {
			rt.new_string('')
		}]),
	])
	if rt.is_true(rt.call_function('wc_coupons_enabled', []rt.PhpVal{}))
		&& !(!rt.is_true(var_coupon)) {
		var_controller.apply_coupon(var_coupon.clone())
		if rt.has_exception() {
			unsafe {
				goto catch_label_2
			}
	...
	if !(!rt.is_true(rt.get_superglobal('_SERVER').array_get(rt.new_string('QUERY_STRING')))) {
		var_redirect_url = rt.call_function('remove_query_arg', [
			rt.create_array([rt.ArrayItem{ key: none, val: 'products' },
				rt.ArrayItem{ key: none, val: 'coupon' }, rt.ArrayItem{
					key: none
					val: 'checkout-link'
				}]),
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{}))))) {
		...
		mut iife_result_0 := iife_temp_0.get_cart_token(rt.new_string((rt.call_method(rt.get_property(rt.call_function('wc',
			[]rt.PhpVal{}), 'session'), 'get_customer_id', []rt.PhpVal{})).str()))
		mut var_session_token := iife_result_0
		var_redirect_url = rt.call_function('add_query_arg', [
			rt.new_string('session'), var_session_token.clone(),
			var_redirect_url.clone()])
	}
	return var_redirect_url.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Domain_Services_CheckoutLink) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'add_error_notices' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Blocks_Domain_Services_WP_Error](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
