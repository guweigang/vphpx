...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Api) construct(mut var_payment_method_registry Class_Automattic_WooCommerce_Blocks_Payments_PaymentMethodRegistry, mut var_asset_registry Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) {
	this.payment_method_registry = var_payment_method_registry
	this.asset_registry = var_asset_registry
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Api) add_payment_method_script_dependencies(var_dependencies rt.PhpVal, var_handle rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		...
		rt.new_bool(true)])))))
	{
		return var_dependencies.clone()
	}
	return rt.call_function('array_merge', [var_dependencies.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Api) add_payment_method_script_data() {
	...
	])))))
	{
		...
		rt.call_method(this.asset_registry, 'add', [
			rt.new_string('paymentMethodSortOrder'),
			rt.func_array_keys(var_enabled_gateways.clone()),
		])
	}
	...
	for {
		...
		{
			rt.call_method(this.asset_registry, 'add', [var_asset_data_key.clone(),
				var_asset_data_value.clone()])
		}
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Api) register_payment_method_integrations(mut var_payment_method_registry Class_Automattic_WooCommerce_Blocks_Payments_PaymentMethodRegistry) {
	...
	var_payment_method_registry.register(rt.call_method(iife_result_0, 'get', [
	...
	var_payment_method_registry.register(rt.call_method(iife_result_1, 'get', [
	...
	var_payment_method_registry.register(rt.call_method(iife_result_2, 'get', [
	...
	var_payment_method_registry.register(rt.call_method(iife_result_3, 'get', [
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Payments_Api) verify_payment_methods_dependencies() {
	...
	mut var_payment_method_scripts := rt.call_method(this.payment_method_registry,
		'get_all_active_payment_method_script_dependencies', []rt.PhpVal{})
	mut iter_2 := var_payment_method_scripts.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_payment_method_script := item_2.val
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_property(var_wp_scripts, 'registered').array_isset(var_payment_method_script.clone()))))))
		...
		mut var_deps := rt.get_property(rt.get_property(var_wp_scripts, 'registered').array_get(var_payment_method_script),
			'deps')
		mut iter_3 := var_deps.iterator()
		for {
			item_3 := iter_3.next() or { break }
			mut var_dep := item_3.val
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_script_is', [
				var_dep.clone(),
				rt.new_string('registered'),
			])))))
			{
				...
				mut var_error_message := rt.call_function('sprintf', [
					...
					rt.call_function('esc_html', [var_payment_method_script.clone()]),
					rt.call_function('esc_html', [var_dep.clone()]),
				])
				...
				rt.call_function('wp_add_inline_script', [var_error_handle.clone(),
					rt.call_function('sprintf', [rt.new_string('console.error( "%s" );'),
				...
				mut iter_4 := var_cart_checkout_scripts.iterator()
				for {
					item_4 := iter_4.next() or { break }
					mut var_script_handle := item_4.val
					if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.get_property(var_wp_scripts, 'registered').array_isset(var_script_handle.clone()))))))
