	...
	rt.call_function('add_action', [rt.new_string('init'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'register_data_script' },
		])])
	rt.call_function('add_action', [
		rt.new_string((if rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) {
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'enqueue_asset_data' },
		]),
		rt.new_int(1),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) get_core_data() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'adminUrl', val: rt.call_function('admin_url', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'countries', val: rt.call_method(rt.get_property(rt.call_function('WC',
			[]rt.PhpVal{}), 'countries'), 'get_countries', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'currency', val: this.get_currency_data() },
		rt.ArrayItem{ key: 'currentUserId', val: rt.call_function('get_current_user_id',
			[]rt.PhpVal{}) },
		rt.ArrayItem{ key: 'currentUserIsAdmin', val: rt.call_function('current_user_can', [
			rt.new_string('manage_woocommerce'),
		]) },
		rt.ArrayItem{ key: 'currentThemeIsFSETheme', val: rt.call_function('wp_is_block_theme',
			[]rt.PhpVal{}) },
		rt.ArrayItem{ key: 'dateFormat', val: rt.call_function('wc_date_format', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'homeUrl', val: rt.call_function('esc_url', [
			rt.call_function('home_url', [rt.new_string('/')]),
		]) },
		rt.ArrayItem{ key: 'locale', val: this.get_locale_data() },
		rt.ArrayItem{ key: 'isRemoteLoggingEnabled', val: rt.call_method(rt.call_method(rt.call_function('wc_get_container',
			[]rt.PhpVal{}), 'get', [
			Class_Automattic_WooCommerce_Internal_Logging_RemoteLogger.class(),
		]), 'is_remote_logging_allowed', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'dashboardUrl', val: rt.call_function('wc_get_account_endpoint_url', [
			rt.new_string('dashboard'),
		]) },
		rt.ArrayItem{ key: 'orderStatuses', val: this.get_order_statuses() },
		rt.ArrayItem{ key: 'placeholderImgSrc', val: rt.call_function('wc_placeholder_img_src',
			[]rt.PhpVal{}) },
		rt.ArrayItem{ key: 'productsSettings', val: this.get_products_settings() },
		rt.ArrayItem{ key: 'siteTitle', val: rt.call_function('wp_specialchars_decode', [
			rt.call_function('get_bloginfo', [rt.new_string('name')]),
			rt.get_constant('ENT_QUOTES'),
		]) },
		rt.ArrayItem{ key: 'storePages', val: this.get_store_pages() },
		rt.ArrayItem{ key: 'wcAssetUrl', val: rt.call_function('plugins_url', [
			rt.new_string('assets/'),
			rt.get_constant('WC_PLUGIN_FILE'),
		]) },
		rt.ArrayItem{
			key: 'wcVersion'
			val: if rt.is_true(rt.call_function('defined', [
				rt.new_string('WC_VERSION'),
			]))
			{ rt.get_constant('WC_VERSION') } else { rt.new_string('') }
		},
		rt.ArrayItem{ key: 'wpLoginUrl', val: rt.call_function('wp_login_url', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'wpVersion', val: rt.call_function('get_bloginfo', [
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) get_currency_data() rt.PhpVal {
	...
	return rt.create_array([rt.ArrayItem{ key: 'code', val: var_currency },
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) get_locale_data() rt.PhpVal {
	...
	return rt.create_array([
		rt.ArrayItem{ key: 'siteLocale', val: rt.call_function('get_locale', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'userLocale', val: rt.call_function('get_user_locale', []rt.PhpVal{}) },
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) get_store_pages() rt.PhpVal {
	mut var_store_pages := rt.create_array([
		rt.ArrayItem{ key: 'myaccount', val: rt.call_function('wc_get_page_id', [
			rt.new_string('myaccount'),
		]) },
		rt.ArrayItem{ key: 'shop', val: rt.call_function('wc_get_page_id', [
			rt.new_string('shop'),
		]) },
		rt.ArrayItem{ key: 'cart', val: rt.call_function('wc_get_page_id', [
			rt.new_string('cart'),
		]) },
		rt.ArrayItem{ key: 'checkout', val: rt.call_function('wc_get_page_id', [
			rt.new_string('checkout'),
		]) },
		rt.ArrayItem{ key: 'privacy', val: rt.call_function('wc_privacy_policy_page_id',
			[]rt.PhpVal{}) },
		rt.ArrayItem{ key: 'terms', val: rt.call_function('wc_terms_and_conditions_page_id',
			[]rt.PhpVal{}) },
	])
	rt.call_function('_prime_post_caches', [
		rt.call_function('array_values', [var_store_pages.clone()]),
		rt.new_bool(false),
		rt.new_bool(false),
	])
	return rt.call_function('array_map', [
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'format_page_resource' },
		]),
		var_store_pages.clone(),
	])
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) get_products_settings() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'cartRedirectAfterAdd', val: rt.identical(rt.call_function('get_option', [
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) format_page_resource(var_page rt.PhpVal) rt.PhpVal {
	...
	if var_page_mutated.clone().is_long() || var_page_mutated.clone().is_double()
		&& rt.is_true(rt.greater(var_page_mutated, rt.new_int(0))) {
		var_page_mutated = rt.call_function('get_post', [var_page_mutated.clone()])
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [var_page_mutated.clone(), rt.new_string('\\WP_Post')])))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('publish'), rt.get_property(var_page_mutated, 'post_status'))))) {
		return rt.create_array([rt.ArrayItem{ key: 'id', val: 0 },
			rt.ArrayItem{ key: 'title', val: '' }, rt.ArrayItem{ key: 'permalink', val: false }])
	}
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.get_property(var_page_mutated, 'ID') },
		rt.ArrayItem{ key: 'title', val: rt.get_property(var_page_mutated, 'post_title') },
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) initialize_core_data() {
	mut var_settings := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_shared_settings'),
		this.data,
	])
	...
	{
		mut var_error_handle := rt.new_string('deprecated-shared-settings-error')
		mut var_error_message :=
			rt.new_string('`woocommerce_shared_settings` filter in Blocks is deprecated. See https://github.com/woocommerce/woocommerce-gutenberg-products-block/blob/trunk/docs/contributors/block-assets.md')
	...
	mut iife_result_0 := iife_temp_0.is_enabled(rt.new_string('rest-api-v4'))
	var_core_data.array_set('experimentalWcRestApiV4', iife_result_0)
	this.data = rt.call_function('array_replace_recursive', [
		var_settings.clone(), var_core_data.clone()])
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) execute_lazy_data() {
	mut iter_2 := this.lazy_data.iterator()
	for {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) exists(var_key rt.PhpVal) bool {
	return this.data.array_isset(var_key.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) add(var_key rt.PhpVal, var_data rt.PhpVal, check_key_exists bool) {
	...
	if var_check_key_exists {
		rt.call_function('wc_deprecated_argument', [
			rt.new_string('Automattic\\WooCommerce\\Blocks\\Assets\\AssetDataRegistry::add()'),
			rt.new_string('8.9'),
			rt.new_string('The $check_key_exists parameter is no longer used: all duplicate data will be ignored if the key exists by default'),
		])
	}
	this.add_data(var_key.clone(), var_data_mutated.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) hydrate_api_request(var_path rt.PhpVal) {
	if !(this.preloaded_api_requests.array_isset(var_path)) {
		...
		this.preloaded_api_requests.array_set(var_path, rt.call_method(rt.call_method(iife_result_1,
			'get', [
			Class_Automattic_WooCommerce_Blocks_Domain_Services_Hydration.class(),
		]), 'get_rest_api_response_data', [var_path.clone()]))
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) hydrate_data_from_api_request(var_key rt.PhpVal, var_path rt.PhpVal, check_key_exists bool) {
	closure_4_fn := fn [var_path] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		if this.preloaded_api_requests.array_isset(var_path)
			&& this.preloaded_api_requests.array_get(var_path).array_isset(rt.new_string('body')) {
			return
		}
		mut iife_temp_3 := Class_Automattic_WooCommerce_Blocks_Package{}
		mut iife_result_3 := iife_temp_3.container()
		mut var_response := rt.call_method(rt.call_method(iife_result_3, 'get', [
			Class_Automattic_WooCommerce_Blocks_Domain_Services_Hydration.class(),
		]), 'get_rest_api_response_data', [var_path.clone()])
		return
	}
	this.add(var_key.clone(), rt.new_closure(closure_4_fn), check_key_exists)
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) register_page_id(var_page_id rt.PhpVal) {
	mut var_permalink := if rt.is_true(var_page_id) { rt.call_function('get_permalink', [
			var_page_id.clone(),
		]) } else { rt.new_bool(false) }
	if rt.is_true(var_permalink) {
		this.data.array_set('page-' + var_page_id.str(), var_permalink.clone())
	}
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) register_data_script() {
	rt.call_method(this.api, 'register_script', [this.handle,
		rt.new_string('assets/client/blocks/wc-settings.js'),
		rt.create_array([rt.ArrayItem{ key: none, val: 'wp-api-fetch' }]),
		rt.new_bool(true)])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) enqueue_asset_data() {
	if rt.is_true(rt.call_function('wp_script_is', [this.handle, rt.new_string('enqueued')])) {
		...
		mut var_data := rt.call_function('rawurlencode', [
			rt.call_function('wp_json_encode', [this.data]),
		])
		mut var_wc_settings_script := rt.new_string(
			"var wcSettings = JSON.parse( decodeURIComponent( '" +
			(rt.call_function('esc_js', [var_data.clone()])).str() + "' ) );")
		mut var_preloaded_api_requests_script := rt.new_string('')
		if this.preloaded_api_requests.array_count() > 0 {
			mut var_preloaded_api_requests := rt.call_function('rawurlencode', [
				rt.call_function('wp_json_encode', [this.preloaded_api_requests]),
			])
			var_preloaded_api_requests_script = rt.new_string(
				"wp.apiFetch.use( wp.apiFetch.createPreloadingMiddleware( JSON.parse( decodeURIComponent( '" +
				(rt.call_function('esc_js', [var_preloaded_api_requests.clone()])).str() +
				"' ) ) ) );")
		}
		rt.call_function('wp_add_inline_script', [this.handle,
			rt.new_string(var_wc_settings_script.str() + var_preloaded_api_requests_script.str()),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) add_data(var_key rt.PhpVal, var_data rt.PhpVal) {
	mut var_data_mutated := var_data
	if !(var_key.clone().is_string()) {
		rt.call_function('trigger_error', [
			rt.call_function('esc_html__', [
				rt.new_string('Key for the data being registered must be a string'),
	...
	if this.exists(var_key.clone()) {
		return
	}
	if this.data.array_isset(var_key) {
		rt.call_function('trigger_error', [
			rt.call_function('esc_html__', [
				rt.new_string('Overriding existing data with an already registered key is not allowed'),
				rt.new_string('woocommerce'),
			]),
			rt.get_constant('E_USER_WARNING'),
		])
	...
	if rt.is_true(rt.call_function('is_callable', [var_data_mutated.clone()])) {
		this.lazy_data.array_set(var_key, var_data_mutated.clone())
		return
	}
	this.data.array_set(var_key, var_data_mutated.clone())
}

fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) debug() bool {
	return rt.is_true(rt.call_function('defined', [rt.new_string('WP_DEBUG')]))
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Assets_AssetDataRegistry) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'preloaded_api_requests' {
			this.preloaded_api_requests = val
			return true
		}
		'lazy_data' {
			this.lazy_data = val
