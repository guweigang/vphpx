...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_Totals) render_content(var_order rt.PhpVal, permission bool, var_attributes rt.PhpVal, content string) string {
	...
	return (this.get_hook_content(rt.new_string('woocommerce_order_details_before_order_table'), rt.create_array([rt.ArrayItem{
		key: none
		val: var_order
	}]))).str() + '\n\t\t\t<table cellspacing="0" class="wc-block-order-confirmation-totals__table ' + (rt.call_function('esc_attr', [var_classes_and_styles.array_get(rt.new_string('classes'))])).str() + '" style="' + (rt.call_function('esc_attr', [var_classes_and_styles.array_get(rt.new_string('styles'))])).str() + '">\n\t\t\t\t<thead>\n\t\t\t\t\t<tr>\n\t\t\t\t\t\t<th class="wc-block-order-confirmation-totals__product">' + (rt.call_function('esc_html__', [rt.new_string('Product'), rt.new_string('woocommerce')])).str() + '</th>\n\t\t\t\t\t\t<th class="wc-block-order-confirmation-totals__total">' + (rt.call_function('esc_html__', [rt.new_string('Total'), rt.new_string('woocommerce')])).str() + '</th>\n\t\t\t\t\t</tr>\n\t\t\t\t</thead>\n\t\t\t\t<tbody>\n\t\t\t\t\t' + (this.get_hook_content(rt.new_string('woocommerce_order_details_before_order_table_items'), rt.create_array([rt.ArrayItem{
		key: none
		val: var_order
	}]))).str() + '\n\t\t\t\t\t' + (this.render_order_details_table_items(var_order.clone())).str() + '\n\t\t\t\t\t' + (this.get_hook_content(rt.new_string('woocommerce_order_details_after_order_table_items'), rt.create_array([rt.ArrayItem{
		key: none
		val: var_order
	}]))).str() + '\n\t\t\t\t</tbody>\n\t\t\t\t<tfoot>\n\t\t\t\t\t' + (this.render_order_details_table_totals(var_order.clone())).str() + '\n\t\t\t\t\t</tfoot>\n\t\t\t\t\t</table>\n\t\t\t' + this.render_order_details_customer_note(var_order.clone()) + '\n\t\t\t' + (this.get_hook_content(rt.new_string('woocommerce_order_details_after_order_table'), rt.create_array([rt.ArrayItem{
		key: none
		val: var_order
	}]))).str() + '\n\t\t\t' + (this.get_hook_content(rt.new_string('woocommerce_after_order_details'), rt.create_array([rt.ArrayItem{
		key: none
		val: var_order
	}]))).str() + '\n\t\t'
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_Totals) get_inline_styles(mut var_attributes Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_array) string {
	...
	return '\n\t\t\t.wc-block-order-confirmation-totals__table a {' +
		(var_link_classes_and_styles.array_get(rt.new_string('style'))).str() + '}\n\t\t\t.wc-block-order-confirmation-totals__table a:hover, .wc-block-order-confirmation-totals__table a:focus {' + (var_link_hover_classes_and_styles.array_get(rt.new_string('style'))).str() + '}\n\t\t\t.wc-block-order-confirmation-totals__table {' + (var_border_classes_and_styles.array_get(rt.new_string('styles'))).str() + '}\n\t\t\t.wc-block-order-confirmation-totals__table th, .wc-block-order-confirmation-totals__table td {' + (var_border_classes_and_styles.array_get(rt.new_string('styles'))).str() + '}\n\t\t'
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_Totals) render_order_details_table_items(var_order rt.PhpVal) rt.PhpVal {
	...
	mut var_order_items := rt.call_function('array_filter', [
	...
	for {
		...
		var_return = rt.concat(var_return, this.render_order_details_table_item(var_order.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_Totals) render_order_details_table_item(var_order rt.PhpVal, var_item_id rt.PhpVal, var_item rt.PhpVal, var_product rt.PhpVal) string {
	...
	mut var_is_visible := rt.new_bool(rt.is_true(var_product_mutated)
		&& rt.is_true(rt.call_method(var_product_mutated, 'is_visible', []rt.PhpVal{})))
	mut var_row_class := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_order_item_class'),
		rt.new_string('woocommerce-table__line-item order_item'),
		var_item.clone(),
		var_order.clone(),
	])
	mut var_product_permalink := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_order_item_permalink'),
		if rt.is_true(var_is_visible) { rt.call_method(var_product_mutated, 'get_permalink', [
				var_item.clone(),
			]) } else { rt.new_string('') },
		var_item.clone(),
		var_order.clone(),
	...
	mut var_item_name := rt.call_function('apply_filters', [
		...
		if rt.is_true(var_product_permalink) { rt.call_function('sprintf', [
				rt.new_string('<a href="%s">%s</a>'),
				var_product_permalink.clone(),
				rt.call_method(var_item, 'get_name', []rt.PhpVal{}),
			]) } else { rt.call_method(var_item, 'get_name', []rt.PhpVal{}) },
		var_item.clone(),
	...
	mut var_qty_display := if rt.is_true(var_refunded_qty) { '<del>' + (rt.call_function('esc_html', [var_qty.clone()])).str() + '</del> <ins>' + (rt.call_function('esc_html', [rt.sub(var_qty, rt.mul(var_refunded_qty, -1))])).str() + '</ins>' } else { rt.call_function('esc_html', [
			var_qty.clone(),
		]) }
	mut var_item_qty := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_order_item_quantity_html'),
		rt.new_string('<strong class="product-quantity">' +
			(rt.call_function('sprintf', [rt.new_string('&times;&nbsp;%s'), var_qty_display.clone()])).str() +
			'</strong>'),
		var_item.clone(),
	])
	return '\n\t\t\t<tr class="' + (rt.call_function('esc_attr', [var_row_class.clone()])).str() +
		'">\n\t\t\t\t<td class="wc-block-order-confirmation-totals__product">\n\t\t\t\t\t' +
		(rt.call_function('wp_kses_post', [var_item_name.clone()])).str() + '&nbsp;\n\t\t\t\t\t' +
		(rt.call_function('wp_kses_post', [var_item_qty.clone()])).str() + '\n\t\t\t\t\t' +
		(this.get_hook_content(rt.new_string('woocommerce_order_item_meta_start'), rt.create_array([rt.ArrayItem{
		key: none
		val: var_item_id
	}, rt.ArrayItem{ key: none, val: var_item }, rt.ArrayItem{ key: none, val: var_order }, rt.ArrayItem{
		key: none
		val: false
	}]))).str() +
		'\n\t\t\t\t\t' +
		(rt.call_function('wc_display_item_meta', [var_item.clone(), rt.create_array([rt.ArrayItem{
		key: 'echo'
		val: false
	}])])).str() +
		'\n\t\t\t\t\t' +
		(this.get_hook_content(rt.new_string('woocommerce_order_item_meta_end'), rt.create_array([rt.ArrayItem{
		key: none
		val: var_item_id
	}, rt.ArrayItem{ key: none, val: var_item }, rt.ArrayItem{ key: none, val: var_order }, rt.ArrayItem{
		key: none
		...
		'\n\t\t\t\t\t' +
		this.render_order_details_table_item_purchase_note(var_order.clone(), var_product_mutated.clone()) +
		'\n\t\t\t\t</td>\n\t\t\t\t<td class="wc-block-order-confirmation-totals__total">\n\t\t\t\t\t' +
		(rt.call_function('wp_kses_post', [rt.call_method(var_order, 'get_formatted_line_subtotal', [var_item.clone()])])).str() +
		'\n\t\t\t\t</td>\n\t\t\t</tr>\n\t\t'
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_Totals) render_order_details_table_item_purchase_note(var_order rt.PhpVal, var_product rt.PhpVal) string {
	...
	mut var_purchase_note := if rt.is_true(var_product_mutated) {
		rt.call_method(var_product_mutated, 'get_purchase_note', []rt.PhpVal{})
	} else {
		rt.new_string('')
	}
	return if rt.is_true(var_show_purchase_note) && rt.is_true(var_purchase_note) {
		'<div class="product-purchase-note">' +
			(rt.call_function('wp_kses_post', [var_purchase_note.clone()])).str() + '</div>'
	} else {
		''
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_Totals) render_order_details_table_totals(var_order rt.PhpVal) rt.PhpVal {
	...
	mut var_return := rt.new_string('')
	mut var_total_rows := rt.call_function('array_diff_key', [
		rt.call_method(var_order, 'get_order_item_totals', []rt.PhpVal{}),
		rt.create_array([rt.ArrayItem{ key: 'cart_subtotal', val: '' },
			rt.ArrayItem{ key: 'payment_method', val: '' }]),
	])
	mut iter_2 := var_total_rows.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_total := item_2.val
		var_return = rt.concat(var_return, rt.new_string(
			'\n\t\t\t\t<tr>\n\t\t\t\t\t<th class="wc-block-order-confirmation-totals__label" scope="row">' +
			(rt.call_function('esc_html', [var_total.array_get(rt.new_string('label'))])).str() +
			'</th>\n\t\t\t\t\t<td class="wc-block-order-confirmation-totals__total">' +
			(rt.call_function('wp_kses_post', [var_total.array_get(rt.new_string('value'))])).str() +
			'</td>\n\t\t\t\t</tr>\n\t\t\t'))
	}
	return var_return.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_Totals) render_order_details_customer_note(var_order rt.PhpVal) string {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_order, 'get_customer_note',
		[]rt.PhpVal{})))))
	{
		return ''
	}
	return '<div class="wc-block-order-confirmation-order-note">' +
		...
		'</p>' + '<p>' +
		(rt.call_function('wp_kses', [rt.call_function('nl2br', [rt.call_function('wptexturize', [rt.call_method(var_order, 'get_customer_note', []rt.PhpVal{})])]), rt.create_array([rt.ArrayItem{
		key: 'br'
		val: rt.new_array()
	}])])).str() +
		'</p>' + '</div>'
...
fn create_automattic_woocommerce_blocks_blocktypes_orderconfirmation_totals(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_Totals {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_Totals) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'render_order_details_table_item' {
			...
			return rt.new_string(this.render_order_details_table_item(dispatch_arg_0,
				dispatch_arg_1, dispatch_arg_2, dispatch_arg_3))
		}
		'render_order_details_table_item_purchase_note' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return rt.new_string(this.render_order_details_table_item_purchase_note(dispatch_arg_0,
				dispatch_arg_1))
		}
