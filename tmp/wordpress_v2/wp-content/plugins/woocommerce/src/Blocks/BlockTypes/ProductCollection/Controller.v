...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) initialize() {
	...
	this.query_builder =
		create_automattic_woocommerce_blocks_blocktypes_productcollection_querybuilder()
	this.renderer = create_automattic_woocommerce_blocks_blocktypes_productcollection_renderer()
	this.collection_handler_registry =
		create_automattic_woocommerce_blocks_blocktypes_productcollection_handlerregistry()
	rt.call_function('add_filter', [rt.new_string('query_loop_block_query_vars'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'build_frontend_query' },
		]),
	...
	rt.call_function('add_filter', [rt.new_string('pre_render_block'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'add_support_for_filter_blocks' },
		]),
	...
	rt.call_function('add_filter', [rt.new_string('rest_product_query'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'update_rest_query_in_editor' },
		]),
		rt.new_int(10), rt.new_int(2)])
	...
	rt.call_function('add_filter', [rt.new_string('render_block_core/post-title'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'add_product_title_click_event_directives' },
		]),
		rt.new_int(10), rt.new_int(3)])
	rt.call_function('add_filter', [rt.new_string('render_block_data'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'disable_enhanced_pagination' },
		]),
		rt.new_int(10), rt.new_int(1)])
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) add_product_title_click_event_directives(var_block_content rt.PhpVal, var_block rt.PhpVal, var_instance rt.PhpVal) rt.PhpVal {
	...
	mut var_namespace := if !(rt.get_property(var_instance, 'attributes').array_get(rt.new_string('__woocommerceNamespace'))).is_null() {
		rt.get_property(var_instance, 'attributes').array_get(rt.new_string('__woocommerceNamespace'))
	} else {
		rt.new_string('')
	}
	mut var_is_product_title_block := rt.identical(rt.new_string('woocommerce/product-collection/product-title'),
		var_namespace)
	mut var_is_link := if !(rt.get_property(var_instance, 'attributes').array_get(rt.new_string('isLink'))).is_null() {
		rt.get_property(var_instance, 'attributes').array_get(rt.new_string('isLink'))
	} else {
		rt.new_bool(false)
	}
	if rt.is_true(var_is_product_title_block) && rt.is_true(var_is_link) {
		mut var_p :=
			create_automattic_woocommerce_blocks_blocktypes_productcollection_wp_html_tag_processor(var_block_content_mutated.clone())
		var_p.next_tag(rt.create_array([
			rt.ArrayItem{ key: 'class_name', val: 'wp-block-post-title' },
		]))
		mut var_is_anchor := var_p.next_tag(rt.create_array([
			rt.ArrayItem{ key: 'tag_name', val: 'a' },
		]))
		if rt.is_true(var_is_anchor) {
			var_p.set_attribute(rt.new_string('data-wp-on--click'),
				rt.new_string('woocommerce/product-collection::actions.viewProduct'))
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) is_block_compatible(var_block_name rt.PhpVal) bool {
	...
	mut iife_temp_0 :=
		Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_WP_Block_Type_Registry{}
	...
	mut var_supports_interactivity := rt.new_bool(
	...
	mut var_supports_client_navigation := rt.new_bool(
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) disable_enhanced_pagination(var_parsed_block rt.PhpVal) rt.PhpVal {
	...
	mut var_render_product_collection_callback := rt.new_null()
	mut var_block_name := var_parsed_block.array_get(rt.new_string('blockName'))
	mut var_is_product_collection_block := if !(var_parsed_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('query')).array_get(rt.new_string('isProductCollectionBlock'))).is_null() {
		var_parsed_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('query')).array_get(rt.new_string('isProductCollectionBlock'))
	} else {
		rt.new_bool(false)
	}
	mut var_force_page_reload_global := rt.new_bool(
		rt.is_true(if !(var_parsed_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('forcePageReload'))).is_null() { var_parsed_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('forcePageReload')) } else { rt.new_bool(false) })
	...
	if rt.is_true(var_is_product_collection_block)
		&& rt.is_true(rt.identical(rt.new_string('woocommerce/product-collection'), var_block_name))
		&& rt.is_true(rt.new_bool(!(rt.is_true(var_force_page_reload_global))))
		&& var_parsed_block.array_get(rt.new_string('attrs')).array_isset(rt.new_string('queryId')) {
		var_enhanced_query_stack.array_push(var_parsed_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('queryId')))
		if !(!var_render_product_collection_callback.is_null()) {
			closure_2_fn := fn [mut var_enhanced_query_stack, mut var_dirty_enhanced_queries, mut var_render_product_collection_callback] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
				mut var_content := if args.len > 0 { args[0].clone() } else { rt.new_null() }
				mut var_block := if args.len > 1 { args[1].clone() } else { rt.new_null() }
				mut var_force_page_reload := rt.new_bool(
					rt.is_true(if !(var_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('forcePageReload'))).is_null() { var_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('forcePageReload')) } else { rt.new_bool(false) })
				...
				if var_block.array_get(rt.new_string('attrs')).array_isset(rt.new_string('queryId'))
					&& var_dirty_enhanced_queries.array_isset(var_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('queryId'))) {
				...
				rt.call_function('array_pop', [var_enhanced_query_stack.clone()])
				if !rt.is_true(var_enhanced_query_stack) {
					rt.call_function('remove_filter', [
						...
						var_render_product_collection_callback.clone(),
						rt.new_int(5),
					])
			...
			var_render_product_collection_callback = rt.new_closure(closure_2_fn)
		...
		&& !(this.is_block_compatible(var_block_name.clone())) {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) enqueue_data(mut var_attributes Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_array) {
	this.Class_Automattic_WooCommerce_Blocks_BlockTypes_AbstractBlock.enqueue_data(rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_array',
	...
	rt.call_function('register_setting', [rt.new_string('options'),
		...
		rt.create_array([rt.ArrayItem{ key: 'type', val: 'object' },
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				...
				rt.ArrayItem{ key: 'name', val: 'woocommerce_default_catalog_orderby' },
				rt.ArrayItem{ key: 'schema', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'enum', val: rt.create_array([
						...
						rt.ArrayItem{ key: none, val: 'price-desc' },
					]) },
				]) },
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) update_rest_query_in_editor(var_query rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_is_product_collection_block := rt.call_method(var_request, 'get_param', [
		rt.new_string('isProductCollectionBlock'),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_is_product_collection_block)))) {
		return var_query_mutated.clone()
	}
	mut var_product_collection_query_context := rt.call_method(var_request, 'get_param', [
		rt.new_string('productCollectionQueryContext'),
	])
	mut var_collection_args := rt.create_array([
		rt.ArrayItem{
			key: 'name'
			val: if !(var_product_collection_query_context.array_get(rt.new_string('collection'))).is_null() {
				var_product_collection_query_context.array_get(rt.new_string('collection'))
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{ key: 'productCollectionLocation', val: rt.call_method(var_request,
			'get_param', [rt.new_string('productCollectionLocation')]) },
	])
	mut var_handlers := rt.call_method(this.collection_handler_registry, 'get_collection_handler', [
		var_collection_args.array_get(rt.new_string('name')),
	])
	if var_handlers.array_isset(rt.new_string('editor_args')) {
		var_collection_args = rt.call_function('call_user_func', [
			var_handlers.array_get(rt.new_string('editor_args')),
			var_collection_args.clone(),
	...
	mut var_preview_state := rt.call_method(var_request, 'get_param', [
		rt.new_string('previewState'),
	])
	if var_preview_state.array_isset(rt.new_string('isPreview'))
		&& rt.is_true(rt.identical(rt.new_string('true'), var_preview_state.array_get(rt.new_string('isPreview')))) {
		return rt.call_method(this.query_builder, 'get_preview_query_args', [
			var_collection_args.clone(),
	...
	mut var_on_sale := rt.identical(rt.call_method(var_request, 'get_param', [
		rt.new_string('woocommerceOnSale'),
	]), rt.new_string('true'))
	mut var_stock_status := rt.call_method(var_request, 'get_param', [
		rt.new_string('woocommerceStockStatus'),
	])
	mut var_product_attributes := rt.call_method(var_request, 'get_param', [
		rt.new_string('woocommerceAttributes'),
	])
	mut var_handpicked_products := rt.call_method(var_request, 'get_param', [
		rt.new_string('woocommerceHandPickedProducts'),
	])
	mut var_featured := rt.call_method(var_request, 'get_param', [
		rt.new_string('featured'),
	])
	mut var_time_frame := rt.call_method(var_request, 'get_param', [
		rt.new_string('timeFrame'),
	])
	mut var_price_range := rt.call_method(var_request, 'get_param', [
		rt.new_string('priceRange'),
	])
	mut var_raw_tax_query_from_rest_params := if !(var_query_mutated.array_get(rt.new_string('tax_query'))).is_null() {
		var_query_mutated.array_get(rt.new_string('tax_query'))
	} else {
		rt.new_array()
	}
	var_query_mutated.array_set('author', '')
	return rt.call_method(this.query_builder, 'get_final_query_args', [
		var_collection_args.clone(), var_query_mutated.clone(),
		rt.create_array([rt.ArrayItem{ key: 'orderby', val: var_orderby },
			rt.ArrayItem{ key: 'on_sale', val: var_on_sale },
			...
			rt.ArrayItem{ key: 'taxonomies_query', val: var_raw_tax_query_from_rest_params }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) add_support_for_filter_blocks(var_pre_render rt.PhpVal, var_parsed_block rt.PhpVal) rt.PhpVal {
	...
	rt.call_method(this.renderer, 'set_parsed_block', [var_parsed_block.clone()])
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller', [
		'Automattic_WooCommerce_Blocks_BlockTypes_AbstractBlock',
	...
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller', [
		'Automattic_WooCommerce_Blocks_BlockTypes_AbstractBlock',
	], &this), 'asset_data_registry'), 'add', [rt.new_string('isRenderingPhpTemplate'),
		rt.new_bool(true)])
	return var_pre_render.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) build_frontend_query(var_query rt.PhpVal, var_block rt.PhpVal, var_page rt.PhpVal) rt.PhpVal {
	...
	mut var_block_context_query :=
		rt.get_property(var_block, 'context').array_get(rt.new_string('query'))
	var_block_context_query.array_set('tax_query', if !(!rt.is_true(var_query_mutated.array_get(rt.new_string('tax_query')))) {
		var_query_mutated.array_get(rt.new_string('tax_query'))
	} else {
		rt.new_array()
	})
	mut var_inherit := if !(rt.get_property(var_block, 'context').array_get(rt.new_string('query')).array_get(rt.new_string('inherit'))).is_null() {
		rt.get_property(var_block, 'context').array_get(rt.new_string('query')).array_get(rt.new_string('inherit'))
	} else {
		rt.new_bool(false)
	}
	mut var_filterable := if !(rt.get_property(var_block, 'context').array_get(rt.new_string('query')).array_get(rt.new_string('filterable'))).is_null() {
		rt.get_property(var_block, 'context').array_get(rt.new_string('query')).array_get(rt.new_string('filterable'))
	} else {
		rt.new_bool(false)
	}
	mut var_is_exclude_applied_filters := rt.new_bool(!(rt.is_true(var_inherit)
		|| rt.is_true(var_filterable)))
	mut var_collection_args := rt.create_array([
		rt.ArrayItem{
			key: 'name'
			val: if !(rt.get_property(var_block, 'context').array_get(rt.new_string('collection'))).is_null() {
				rt.get_property(var_block, 'context').array_get(rt.new_string('collection'))
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{
			key: 'productCollectionLocation'
			val: if !(rt.get_property(var_block, 'context').array_get(rt.new_string('productCollectionLocation'))).is_null() {
				rt.get_property(var_block, 'context').array_get(rt.new_string('productCollectionLocation'))
			} else {
				rt.new_null()
			}
		},
	])
	return rt.call_method(this.query_builder, 'get_final_frontend_query', [
		var_collection_args.clone(), var_block_context_query.clone(),
		var_page.clone(), var_is_exclude_applied_filters.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) extend_rest_query_allowed_params(var_params rt.PhpVal) rt.PhpVal {
	...
	var_params.array_get_mut('orderby').array_set('enum', rt.call_function('array_unique', [
		rt.call_function('array_merge', [var_original_enum.clone(),
			rt.call_method(this.query_builder, 'get_custom_order_opts', []rt.PhpVal{})]),
	]))
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) register_core_collections_and_set_handler_store() {
	mut var_collection_handler_store := rt.call_method(this.collection_handler_registry,
		'register_core_collections', []rt.PhpVal{})
	rt.call_method(this.query_builder, 'set_collection_handler_store', [
		var_collection_handler_store.clone()])
}
...
fn create_automattic_woocommerce_blocks_blocktypes_productcollection_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller {
...
fn create_automattic_woocommerce_blocks_blocktypes_productcollection_querybuilder(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_QueryBuilder {
...
fn create_automattic_woocommerce_blocks_blocktypes_productcollection_renderer(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Renderer {
...
fn create_automattic_woocommerce_blocks_blocktypes_productcollection_wp_block_type_registry(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_WP_Block_Type_Registry {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'add_product_title_click_event_directives' {
			...
			return this.add_product_title_click_event_directives(dispatch_arg_0, dispatch_arg_1,
				dispatch_arg_2)
		}
		'is_block_compatible' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductCollection_Controller) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'collection_handler_registry' {
			this.collection_handler_registry = val
			return true
		}
		'query_builder' {
			this.query_builder = val
			return true
		}
		'renderer' {
			this.renderer = val
