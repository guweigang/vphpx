...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilters) enqueue_data(mut var_attributes Class_Automattic_WooCommerce_Blocks_BlockTypes_array) {
	...
	mut iife_temp_0 := Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState{}
	mut iife_result_0 :=
		iife_temp_0.load_store_config(rt.new_string('I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of WooCommerce'))
	mut var_is_product_archive := rt.new_bool(
		rt.is_true(rt.call_function('is_shop', []rt.PhpVal{}))
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilters) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	mut var_query_id := if !(rt.get_property(var_block, 'context').array_get(rt.new_string('queryId'))).is_null() {
		rt.get_property(var_block, 'context').array_get(rt.new_string('queryId'))
	} else {
		rt.new_int(0)
	}
	mut var_filter_params := this.get_filter_params(var_query_id.clone())
	rt.call_function('wp_interactivity_config', [this.get_full_block_name(),
		rt.create_array([
			rt.ArrayItem{
				key: 'canonicalUrl'
				val: this.get_canonical_url_no_pagination(var_filter_params.clone())
			},
		])])
	mut var_active_filters := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_blocks_product_filters_selected_items'),
		rt.new_array(),
		var_filter_params.clone(),
	])
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_a := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_b := if args.len > 1 { args[1].clone() } else { rt.new_null() }
		return rt.call_function('strnatcmp', [var_a.array_get(rt.new_string('activeLabel')),
			var_b.array_get(rt.new_string('activeLabel'))])
	}
	rt.call_function('usort', [var_active_filters.clone(), rt.new_closure(closure_2_fn)])
	...
	closure_3_fn := fn [var_block_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		var_carry = rt.concat(var_carry, rt.call_method(create_automattic_woocommerce_blocks_blocktypes_wp_block(var_parsed_block.clone(),
			var_block_context.clone()), 'render', []rt.PhpVal{}))
		return var_carry.clone()
	}
	mut var_inner_blocks := rt.call_function('array_reduce', [
		rt.get_property(var_block, 'parsed_block').array_get(rt.new_string('innerBlocks')),
	...
	mut var_wrapper_attributes := rt.create_array([
		...
		rt.ArrayItem{ key: 'data-wp-context', val: rt.call_function('wp_json_encode', [
			var_interactivity_context.clone(),
	...
	if !(rt.get_property(var_block, 'context').array_isset(rt.new_string('productCollectionLocation'))) {
		var_wrapper_attributes.array_set('data-wp-router-region',
			this.generate_navigation_id(var_block.clone()))
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilters) get_svg_icon(name string) string {
	...
	return (rt.call_function('sprintf', [
		...
		var_icons.array_get(rt.new_string(name)),
	])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilters) get_filter_params(var_query_id rt.PhpVal) rt.PhpVal {
	...
	mut var_parsed_url := rt.call_function('wp_parse_url', [
		rt.call_function('esc_url_raw', [var_request_uri.clone()]),
	])
	...
	closure_4_fn := fn [var_filter_param_keys] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_key := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return rt.call_function('in_array', [var_key.clone(),
			var_filter_param_keys.clone(), rt.new_bool(true)])
	}
	return rt.call_function('array_filter', [var_url_query_params.clone(),
		rt.new_closure(closure_4_fn), rt.get_constant('ARRAY_FILTER_USE_KEY')])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilters) get_canonical_url_no_pagination(var_filter_params rt.PhpVal) rt.PhpVal {
	...
	mut var_decoded_url := rt.call_function('html_entity_decode', [
		var_canonical_url_no_pagination.clone(), rt.get_constant('ENT_QUOTES'),
		rt.call_function('get_bloginfo', [rt.new_string('charset')])])
	mut var_parsed_url := rt.call_function('wp_parse_url', [var_decoded_url.clone()])
	if !rt.is_true(var_filter_params_mutated)
		|| !rt.is_true(var_parsed_url.array_get(rt.new_string('query'))) {
		return var_decoded_url.clone()
	}
	mut iter_1 := rt.func_array_keys(var_filter_params_mutated.clone()).iterator()
	for {
	...
	if var_parsed_url.array_isset(rt.new_string('host')) {
		var_url = rt.concat(var_url, var_parsed_url.array_get(rt.new_string('host')))
	}
	...
	if !(!rt.is_true(var_parsed_url.array_get(rt.new_string('query')))) {
		var_url = rt.concat(var_url, rt.new_string('?' +
			(var_parsed_url.array_get(rt.new_string('query'))).str()))
	}
	if var_parsed_url.array_isset(rt.new_string('fragment')) {
		var_url = rt.concat(var_url, rt.new_string('#' +
			(var_parsed_url.array_get(rt.new_string('fragment'))).str()))
	}
...
fn create_automattic_woocommerce_blocks_blocktypes_productfilters(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilters {
