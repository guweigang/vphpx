...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_GroupedProductItemSelector) get_quantity_selector_markup(var_product rt.PhpVal) rt.PhpVal {
	...
	mut var_max_value := rt.call_method(var_product_mutated, 'get_max_purchase_quantity',
		[]rt.PhpVal{})
	if rt.is_true(rt.identical(var_min_value, var_max_value))
	...
	rt.call_function('woocommerce_quantity_input', [
		rt.create_array([
			...
			rt.ArrayItem{
				key: 'input_value'
				val: if rt.get_superglobal('_POST').array_get(rt.new_string('quantity')).array_isset(rt.call_method(var_product_mutated, 'get_id', []rt.PhpVal{})) { rt.call_function('wc_stock_amount', [
						rt.call_function('wc_clean', [
							rt.call_function('wp_unslash', [
								rt.get_superglobal('_POST').array_get(rt.new_string('quantity')).array_get(rt.call_method(var_product_mutated, 'get_id', []rt.PhpVal{})),
							]),
						]),
					]) } else { rt.new_string('') }
			},
	...
	var_quantity_html = this.remove_quantity_label(var_quantity_html.clone())
	...
	mut iife_result_0 := iife_temp_0.add_quantity_steppers(var_quantity_html.clone(),
		var_product_name.clone())
	var_quantity_html = iife_result_0
	mut iife_temp_1 := Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils{}
	mut iife_result_1 := iife_temp_1.add_quantity_stepper_classes(var_quantity_html.clone())
	var_quantity_html = iife_result_1
	...
	mut iife_result_2 := iife_temp_2.make_quantity_input_interactive(var_quantity_html.clone(),
		rt.new_array(), rt.new_array(), var_context.clone(), rt.new_bool(true))
	var_quantity_html = iife_result_2
	return var_quantity_html.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_GroupedProductItemSelector) remove_quantity_label(var_quantity_html rt.PhpVal) rt.PhpVal {
	...
	var_quantity_html_mutated = rt.call_function('preg_replace', [
		...
		rt.new_string(''),
		var_quantity_html_mutated.clone(),
	])
	return rt.call_function('preg_replace', [rt.new_string('/\\s*aria-label="[^"]*"/'),
		rt.new_string(''), var_quantity_html_mutated.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_GroupedProductItemSelector) get_checkbox_markup(var_product rt.PhpVal) string {
	...
	return '<input type="checkbox" name="' +
		(rt.call_function('esc_attr', [rt.new_string('quantity[' + (rt.call_method(var_product_mutated, 'get_id', []rt.PhpVal{})).str() +
		']')])).str() + '" value="1" class="wc-grouped-product-add-to-cart-checkbox" id="' +
		(rt.call_function('esc_attr', [rt.new_string('quantity_' + (rt.call_method(var_product_mutated, 'get_id', []rt.PhpVal{})).str())])).str() +
		...
		(rt.call_function('esc_attr', [var_label.clone()])).str() + '"/>'
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_GroupedProductItemSelector) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut iife_result_3 := iife_temp_3.get_product_from_context(var_block.clone(),
		var_previous_product.clone())
	var_product = iife_result_3
	mut var_markup := rt.new_string('')
	if rt.is_true(var_product) {
		mut var_is_interactive := rt.new_bool(false)
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_product, 'is_purchasable', []rt.PhpVal{})))))
			...
			var_markup = this.get_button_markup(var_product.clone())
		} else if rt.is_true(rt.call_method(var_product, 'is_sold_individually', []rt.PhpVal{})) {
			var_is_interactive = rt.new_bool(true)
			var_markup = rt.new_string(this.get_checkbox_markup(var_product.clone()))
		} else {
			var_is_interactive = rt.new_bool(true)
			var_markup = this.get_quantity_selector_markup(var_product.clone())
		}
...
fn create_automattic_woocommerce_blocks_blocktypes_addtocartwithoptions_groupedproductitemselector(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_GroupedProductItemSelector {
