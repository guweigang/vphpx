...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ClassicShortcode) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut iife_temp_0 := Class_Automattic_WooCommerce_Blocks_BlockTypes_{"nodeType":"Expr_Variable","line":49,"name":"frontend_scripts"}{}
	mut iife_result_0 := iife_temp_0.load_scripts()
	}
	if rt.is_true(rt.identical(rt.new_string('cart'), var_attributes.array_get(rt.new_string('shortcode')))) {
		return this.render_cart(var_attributes.clone())
	}
	if rt.is_true(rt.identical(rt.new_string('checkout'), var_attributes.array_get(rt.new_string('shortcode')))) {
		return this.render_checkout(var_attributes.clone())
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ClassicShortcode) get_container_classes(var_attributes rt.PhpVal) rt.PhpVal {
	...
	if var_attributes.array_isset(rt.new_string('align')) {
		var_classes.array_push(rt.concat(rt.new_string('align'), var_attributes.array_get(rt.new_string('align'))))
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ClassicShortcode) render_cart(var_attributes rt.PhpVal) string {
	...
	print('<div class="' + (rt.call_function('esc_attr', [this.get_container_classes(var_attributes.clone())])).str() + '">')
	mut iife_temp_1 := Class_WC_Shortcode_Cart{}
	mut iife_result_1 := iife_temp_1.output(rt.new_array())
	print('</div>')
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ClassicShortcode) render_checkout(var_attributes rt.PhpVal) string {
	...
	rt.call_function('ob_start', []rt.PhpVal{})
	print('<div class="' + (rt.call_function('esc_attr', [this.get_container_classes(var_attributes.clone())])).str() + '">')
	mut iife_temp_2 := Class_WC_Shortcode_Checkout{}
	mut iife_result_2 := iife_temp_2.output(rt.new_array())
	print('</div>')
	return (rt.call_function('ob_get_clean', []rt.PhpVal{})).str()
}
...
fn create_automattic_woocommerce_blocks_blocktypes_classicshortcode(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ClassicShortcode {
...
fn create_automattic_woocommerce_blocks_blocktypes_{"nodetype":"expr_variable","line":49,"name":"frontend_scripts"}(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_{"nodeType":"Expr_Variable","line":49,"name":"frontend_scripts"} {
