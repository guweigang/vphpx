...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_CategoryTitle) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_level := if var_attributes.array_isset(rt.new_string('level')) { rt.call_function('max', [
			...
			rt.call_function('min', [rt.new_int(6),
				rt.new_int(var_attributes.array_get(rt.new_string('level')).to_i64())]),
		]) } else { rt.new_int(2) }
	...
	mut var_is_link :=
		rt.new_bool(!(!rt.is_true(var_attributes.array_get(rt.new_string('isLink')))))
	mut var_rel := if var_attributes.array_isset(rt.new_string('rel')) { rt.call_function('esc_attr', [
			var_attributes.array_get(rt.new_string('rel')),
		]) } else { rt.new_string('') }
	...
	mut var_title_html := rt.new_string('')
	if rt.is_true(var_is_link) {
		...
		])))))
		{
			var_title_html = rt.call_function('sprintf', [
				...
				rt.call_function('esc_attr', [var_tag_name.clone()]),
				var_wrapper_attributes.clone(),
				rt.call_function('esc_url', [var_link.clone()]),
				rt.call_function('esc_attr', [var_target.clone()]),
				var_rel.clone(),
...
fn create_automattic_woocommerce_blocks_blocktypes_categorytitle(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_CategoryTitle {
