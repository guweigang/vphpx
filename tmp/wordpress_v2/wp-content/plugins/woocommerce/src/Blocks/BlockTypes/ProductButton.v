...
fn init_static_automattic_woocommerce_blocks_blocktypes_productbutton() {
	rt.init_static_prop('Automattic_WooCommerce_Blocks_BlockTypes_ProductButton', 'cart',
		rt.new_null())
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductButton) enqueue_assets(mut var_attributes Class_Automattic_WooCommerce_Blocks_BlockTypes_array, var_content rt.PhpVal, var_block rt.PhpVal) {
	...
	this.Class_Automattic_WooCommerce_Blocks_BlockTypes_AbstractBlock.enqueue_assets(rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_array',
		[]string{}, var_attributes_mutated), var_content.clone(), var_block.clone())
	if rt.is_true(rt.call_function('wp_is_block_theme', []rt.PhpVal{})) {
		rt.call_function('add_action', [rt.new_string('wp_enqueue_scripts'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_ProductButton', [
					'Automattic_WooCommerce_Blocks_BlockTypes_AbstractBlock',
				], &this) },
				rt.ArrayItem{ key: none, val: 'dequeue_add_to_cart_scripts' },
			])])
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductButton) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_post := if rt.is_true(var_post_id) { rt.call_function('wc_get_product', [
			var_post_id.clone(),
		]) } else { rt.new_null() }
	if rt.is_true(rt.new_bool(rt.instance_of(var_post,
		'Automattic_WooCommerce_Blocks_BlockTypes_WC_Product')))
	{
		var_product = var_post.clone()
	} else if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_product,
		'Automattic_WooCommerce_Blocks_BlockTypes_WC_Product'))))))
	{
		return ''
	}
	mut var_is_descendant_of_add_to_cart_form := if rt.get_property(var_block, 'context').array_isset(rt.new_string('woocommerce/isDescendantOfAddToCartWithOptions')) {
		rt.get_property(var_block, 'context').array_get(rt.new_string('woocommerce/isDescendantOfAddToCartWithOptions'))
	} else {
		rt.new_bool(false)
	}
	mut iife_temp_0 := Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils{}
	mut iife_result_0 := iife_temp_0.is_not_purchasable_product(var_product.clone())
	if rt.is_true(var_is_descendant_of_add_to_cart_form) && rt.is_true(iife_result_0) {
		var_product = var_previous_product.clone()
		return ''
	}
	mut iife_temp_1 := Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState{}
	mut iife_result_1 :=
		iife_temp_1.load_cart_state(rt.new_string('I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of WooCommerce'))
	mut var_number_of_items_in_cart := rt.new_int(this.get_cart_item_quantities_by_product_id(rt.call_method(var_product,
		'get_id', []rt.PhpVal{})))
	mut var_is_product_purchasable := rt.new_bool(this.is_product_purchasable(var_product.clone()))
	mut var_cart_redirect_after_add := rt.identical(rt.call_function('get_option', [
		rt.new_string('woocommerce_cart_redirect_after_add'),
	]), rt.new_string('yes'))
	mut var_ajax_add_to_cart_enabled := rt.identical(rt.call_function('get_option', [
		rt.new_string('woocommerce_enable_ajax_add_to_cart'),
	]), rt.new_string('yes'))
	mut var_is_ajax_button := rt.new_bool((rt.is_true(var_ajax_add_to_cart_enabled)
		&& rt.is_true(rt.call_method(var_product, 'supports', [rt.new_string('ajax_add_to_cart')])))
		|| rt.is_true(var_is_descendant_of_add_to_cart_form)
		&& rt.is_true(var_is_product_purchasable)
		&& rt.is_true(rt.new_bool(!(rt.is_true(var_cart_redirect_after_add)))))
	mut var_html_element := rt.new_string((if rt.is_true(var_is_ajax_button)
		|| (rt.is_true(var_is_descendant_of_add_to_cart_form)
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_product, 'is_type', [Class_Automattic_WooCommerce_Enums_ProductType.external()])))))) {
		'button'
	} else {
		'a'
	}).str())
	mut iife_temp_2 := Class_Automattic_WooCommerce_Blocks_Utils_StyleAttributesUtils{}
	mut iife_result_2 := iife_temp_2.get_classes_and_styles_by_attributes(var_attributes_mutated.clone(),
		rt.new_array(), rt.create_array([rt.ArrayItem{ key: none, val: 'extra_classes' }]))
	mut var_styles_and_classes := iife_result_2
	mut iife_temp_3 := Class_Automattic_WooCommerce_Blocks_Utils_StyleAttributesUtils{}
	mut iife_result_3 := iife_temp_3.get_classes_by_attributes(var_attributes_mutated.clone(), rt.create_array([
		rt.ArrayItem{ key: none, val: 'extra_classes' },
	]))
	mut var_classname := iife_result_3
	mut var_custom_width_classes := rt.new_string((if var_attributes_mutated.array_isset(rt.new_string('width')) {
		'has-custom-width wp-block-button__width-' +
			(var_attributes_mutated.array_get(rt.new_string('width'))).str()
	} else {
		''
	}).str())
	mut var_custom_align_classes := rt.new_string((if var_attributes_mutated.array_isset(rt.new_string('textAlign')) {
		'align-' + (var_attributes_mutated.array_get(rt.new_string('textAlign'))).str()
	} else {
		''
	}).str())
	mut var_html_classes := rt.call_function('implode', [rt.new_string(' '),
		rt.call_function('array_filter', [
			rt.create_array([rt.ArrayItem{ key: none, val: 'wp-block-button__link' },
				rt.ArrayItem{ key: none, val: 'wp-element-button' },
				rt.ArrayItem{ key: none, val: 'wc-block-components-product-button__button' },
				rt.ArrayItem{
					key: none
					val: if
					...
					key: none
					val: if rt.is_true(var_is_ajax_button) { 'ajax_add_to_cart' } else { '' }
				}, rt.ArrayItem{ key: none, val: 'product_type_' +
					(rt.call_method(var_product, 'get_type', []rt.PhpVal{})).str() },
				rt.ArrayItem{ key: none, val: rt.call_function('esc_attr', [
					var_styles_and_classes.array_get(rt.new_string('classes')),
				]) }]),
		])])
	mut var_default_quantity := rt.new_int(1)
	if rt.is_true(rt.new_bool(!(rt.is_true(var_is_descendant_of_add_to_cart_form)))) {
		var_default_quantity = rt.call_function('apply_filters', [
			rt.new_string('woocommerce_add_to_cart_quantity'),
			var_default_quantity.clone(),
			rt.call_method(var_product, 'get_id', []rt.PhpVal{}),
		])
	}
	mut var_add_to_cart_text := if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), rt.call_method(var_product, 'add_to_cart_text', []rt.PhpVal{}))))) { rt.call_method(var_product, 'add_to_cart_text', []rt.PhpVal{}) } else { rt.call_function('__', [
			rt.new_string('Add to cart'),
			rt.new_string('woocommerce'),
		]) }
	if rt.is_true(var_is_descendant_of_add_to_cart_form)
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), rt.call_method(var_product, 'single_add_to_cart_text', []rt.PhpVal{}))))) {
		var_add_to_cart_text = rt.call_method(var_product, 'single_add_to_cart_text', []rt.PhpVal{})
	}
	mut var_context := rt.create_array([
		rt.ArrayItem{ key: 'quantityToAdd', val: var_default_quantity },
		rt.ArrayItem{ key: 'addToCartText', val: var_add_to_cart_text },
		rt.ArrayItem{ key: 'tempQuantity', val: var_number_of_items_in_cart },
		rt.ArrayItem{ key: 'animationStatus', val: 'IDLE' },
		rt.ArrayItem{ key: 'inTheCartText', val: this.get_in_the_cart_text(var_product.clone()) },
		rt.ArrayItem{ key: 'noticeId', val: '' },
		rt.ArrayItem{ key: 'hasPressedButton', val: false },
	])
	if rt.is_true(rt.call_method(var_product, 'is_type', [
		Class_Automattic_WooCommerce_Enums_ProductType.grouped(),
	]))
	{
		var_context.array_set('groupedProductIds', rt.call_method(var_product, 'get_children',
			[]rt.PhpVal{}))
	}
	var_attributes_mutated = rt.create_array([
		rt.ArrayItem{
			key: 'type'
			val: if rt.is_true(var_is_descendant_of_add_to_cart_form) { 'submit' } else { 'button' }
		},
	])
	...
	rt.call_function('wp_interactivity_config', [rt.new_string('woocommerce'),
		rt.create_array([
			rt.ArrayItem{ key: 'messages', val: rt.create_array([
				rt.ArrayItem{ key: 'addedToCartText', val: rt.call_function('__', [
	...
	closure_5_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_quantity := var_context.array_get(rt.new_string('tempQuantity'))
		mut var_add_to_cart_text := var_context.array_get(rt.new_string('addToCartText'))
		return (if rt.is_true(rt.greater(var_quantity, rt.new_int(0))) {
			rt.call_function('sprintf', [
				rt.call_function('__', [rt.new_string('%s in cart'),
					rt.new_string('woocommerce')]),
				var_quantity.clone(),
			])
		} else {
			var_add_to_cart_text
		}).str()
	}
	rt.call_function('wp_interactivity_state', [
		rt.new_string('woocommerce/product-button'),
		rt.create_array([
			rt.ArrayItem{ key: 'addToCartText', val: rt.new_closure(closure_5_fn) },
		]),
	])
	mut var_args := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_loop_add_to_cart_args'),
		rt.create_array([rt.ArrayItem{ key: 'class', val: var_html_classes },
			rt.ArrayItem{ key: 'attributes', val: rt.call_function('array_merge', [
				var_attributes_mutated.clone(),
				rt.create_array([
					rt.ArrayItem{ key: 'data-product_id', val: rt.call_method(var_product,
						'get_id', []rt.PhpVal{}) },
					rt.ArrayItem{ key: 'data-product_sku', val: rt.call_method(var_product,
						'get_sku', []rt.PhpVal{}) },
					rt.ArrayItem{
						key: 'aria-label'
						val: if
							rt.is_true(rt.new_bool(!(rt.is_true(var_is_descendant_of_add_to_cart_form))))
							|| rt.is_true(rt.identical(rt.new_string('simple'), rt.call_method(var_product, 'get_type', []rt.PhpVal{}))) {
							rt.call_method(var_product, 'add_to_cart_description', []rt.PhpVal{})
						} else {
							rt.new_null()
						}
					},
	...
	if var_args.array_get(rt.new_string('attributes')).array_isset(rt.new_string('aria-label')) {
		var_args.array_get_mut('attributes').array_set('aria-label', rt.call_function('wp_strip_all_tags', [
			var_args.array_get(rt.new_string('attributes')).array_get(rt.new_string('aria-label')),
		]))
	}
	mut var_div_directives :=
		rt.new_string('\n\t\t\tdata-wp-interactive="woocommerce/product-button"\n\t\t')
	mut var_context_directives := rt.call_function('wp_interactivity_data_wp_context', [
		var_context.clone(),
	])
	mut var_button_directives := rt.new_string((if rt.is_true(var_is_descendant_of_add_to_cart_form) {
	...
	mut var_anchor_directive := rt.new_string((if rt.is_true(var_is_descendant_of_add_to_cart_form) {
		''
	} else {
		'data-wp-on--click="woocommerce/product-collection::actions.viewProduct"'
	}).str())
	mut var_span_button_directives :=
			...
			rt.ArrayItem{ key: 'class', val: rt.call_function('implode', [
				...
				rt.call_function('array_filter', [
					rt.create_array([
						rt.ArrayItem{
							key: none
							val: 'wp-block-button wc-block-components-product-button'
						},
						rt.ArrayItem{ key: none, val: rt.call_function('esc_attr', [
							rt.new_string(var_classname.str() + ' ' +
								var_custom_width_classes.str() + ' ' +
								var_custom_align_classes.str()),
						]) },
					]),
				]),
			]) },
	...
	mut var_button_classes := if var_args.array_isset(rt.new_string('class')) { rt.call_function('esc_attr', [
			rt.new_string((var_args.array_get(rt.new_string('class'))).str() + ' wc-interactive'),
		]) } else { rt.new_string('wc-interactive') }
	if rt.is_true(var_is_descendant_of_add_to_cart_form) {
		var_button_classes = rt.concat(var_button_classes,
			rt.new_string(' single_add_to_cart_button'))
		var_args.array_get_mut('attributes').array_set('value', rt.call_method(var_product,
			'get_id', []rt.PhpVal{}))
	}
	mut iife_temp_5 := Class_Automattic_WooCommerce_Blocks_Utils_CartCheckoutUtils{}
	mut iife_result_5 := iife_temp_5.has_cart_page()
	mut var_html := rt.call_function('strtr', [
		rt.new_string('<div {wrapper_attributes}\n\t\t\t\t\t{div_directives}\n\t\t\t\t\t{context_directives}\n\t\t\t\t>\n\t\t\t\t\t<{html_element}\n\t\t\t\t\t\tclass="{button_classes}"\n\t\t\t\t\t\tstyle="{button_styles}"\n\t\t\t\t\t\t{attributes}\n\t\t\t\t\t\t{button_directives}\n\t\t\t\t\t>\n\t\t\t\t\t<span {span_button_directives}>{add_to_cart_text}</span>\n\t\t\t\t\t</{html_element}>\n\t\t\t\t\t{view_cart_html}\n\t\t\t\t</div>'),
		rt.create_array([
			rt.ArrayItem{ key: '{wrapper_attributes}', val: var_wrapper_attributes },
			rt.ArrayItem{ key: '{html_element}', val: var_html_element },
			rt.ArrayItem{ key: '{button_classes}', val: var_button_classes },
			rt.ArrayItem{ key: '{context_directives}', val: var_context_directives },
			rt.ArrayItem{ key: '{button_styles}', val: rt.call_function('esc_attr', [
				var_styles_and_classes.array_get(rt.new_string('styles')),
			]) },
			rt.ArrayItem{
				key: '{attributes}'
				val: if var_args.array_isset(rt.new_string('attributes')) { rt.call_function('wc_implode_html_attributes', [
						var_args.array_get(rt.new_string('attributes')),
					]) } else { rt.new_string('') }
			},
			rt.ArrayItem{
				key: '{add_to_cart_text}'
				val: if rt.is_true(var_is_ajax_button) {
					rt.new_string('')
				} else {
					var_add_to_cart_text
				}
			...
			rt.ArrayItem{
				key: '{button_directives}'
				val: if rt.is_true(var_is_ajax_button) {
					var_button_directives
				} else {
					var_anchor_directive
				}
			},
			rt.ArrayItem{
				key: '{span_button_directives}'
				val: if rt.is_true(var_is_ajax_button) {
					var_span_button_directives
				} else {
					rt.new_string('')
				}
			},
			rt.ArrayItem{
				key: '{view_cart_html}'
				val: if rt.is_true(var_is_ajax_button) && rt.is_true(iife_result_5)
					&& rt.is_true(rt.new_bool(!(rt.is_true(var_is_descendant_of_add_to_cart_form)))) {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductButton) get_cart_item_quantities_by_product_id(var_product_id rt.PhpVal) i64 {
	...
	mut var_cart := rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'cart'),
		'get_cart_item_quantities', []rt.PhpVal{})
	return (if var_cart.array_isset(var_product_id) {
		var_cart.array_get(var_product_id)
	} else {
		rt.new_int(0)
	}).to_i64()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductButton) is_product_purchasable(var_product rt.PhpVal) bool {
	...
	{
		mut var_grouped_product_ids := rt.call_method(var_product_mutated, 'get_children',
			[]rt.PhpVal{})
		if !(!rt.is_true(var_grouped_product_ids)) {
			rt.call_function('_prime_post_caches', [var_grouped_product_ids.clone()])
		}
		mut iter_1 := var_grouped_product_ids.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_child := item_1.val
			mut var_child_product := rt.call_function('wc_get_product', [
				var_child.clone()])
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_child_product,
				'Automattic_WooCommerce_Blocks_BlockTypes_WC_Product'))))))
			{
				continue
			}
			if rt.is_true(rt.call_method(var_child_product, 'is_purchasable', []rt.PhpVal{}))
				&& rt.is_true(rt.call_method(var_child_product, 'is_in_stock', []rt.PhpVal{})) {
				return true
			}
		}
		return false
	}
	return rt.is_true(rt.call_method(var_product_mutated, 'is_purchasable', []rt.PhpVal{}))
		&& rt.is_true(rt.call_method(var_product_mutated, 'is_in_stock', []rt.PhpVal{}))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductButton) get_in_the_cart_text(var_product rt.PhpVal) rt.PhpVal {
	...
	{
		return rt.call_function('__', [rt.new_string('Added to cart'),
			rt.new_string('woocommerce')])
	}
	return rt.call_function('sprintf', [
		rt.call_function('__', [rt.new_string('%s in cart'), rt.new_string('woocommerce')]),
		rt.new_string('###'),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductButton) get_view_cart_html() rt.PhpVal {
	return rt.call_function('sprintf', [
		...
		rt.call_function('esc_url', [rt.call_function('wc_get_cart_url', []rt.PhpVal{})]),
		rt.call_function('esc_attr__', [rt.new_string('View cart'),
			rt.new_string('woocommerce')]),
		rt.call_function('esc_html__', [rt.new_string('View cart'),
			rt.new_string('woocommerce')]),
	])
}
...
fn create_automattic_woocommerce_blocks_blocktypes_productbutton(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductButton {
