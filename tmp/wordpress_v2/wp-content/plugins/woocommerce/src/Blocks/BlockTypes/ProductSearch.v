...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductSearch) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
			...
			rt.ArrayItem{ key: 'class', val: rt.call_function('implode', [
				...
				rt.call_function('array_filter', [
					rt.create_array([
						...
						rt.ArrayItem{
							key: none
							val: if rt.is_true(var_attributes_mutated.array_get(rt.new_string('align'))) {
								'align' +
									(var_attributes_mutated.array_get(rt.new_string('align'))).str()
							} else {
								''
							}
						},
	...
	mut var_label_markup := if rt.is_true(var_attributes_mutated.array_get(rt.new_string('hasLabel'))) { rt.call_function('sprintf', [
			rt.new_string('<label for="%s" class="wc-block-product-search__label">%s</label>'),
			rt.call_function('esc_attr', [var_input_id.clone()]),
			rt.call_function('esc_html', [var_attributes_mutated.array_get(rt.new_string('label'))]),
	...
	mut var_input_markup := rt.call_function('sprintf', [
		...
		rt.call_function('esc_attr',
			[var_attributes_mutated.array_get(rt.new_string('placeholder'))]),
	])
	...
	return rt.call_function('sprintf', [
		...
		rt.call_function('esc_url', [rt.call_function('home_url', [
			rt.new_string('/')])]),
		rt.new_string(var_label_markup.str() + var_field_markup.str()),
	])
}
...
fn create_automattic_woocommerce_blocks_blocktypes_productsearch(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductSearch {
