...
fn Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.add_quantity_steppers(var_quantity_html rt.PhpVal, var_product_name rt.PhpVal) rt.PhpVal {
	...
	closure_1_fn := fn [var_product_name] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_string(
			...
			'" type="button" data-wp-on--click="actions.increaseQuantity" data-wp-bind--disabled="!state.allowsIncrease" class="wc-block-components-quantity-selector__button wc-block-components-quantity-selector__button--plus">+</button>')
	}
	mut var_new_html := rt.call_function('preg_replace_callback', [
		var_pattern.clone(), rt.new_closure(closure_1_fn), if !var_quantity_html_mutated.is_null() {
			var_quantity_html_mutated
		} else {
			rt.new_string('')
		}])
	closure_2_fn := fn [var_product_name] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_string(
			...
			'" type="button" data-wp-on--click="actions.decreaseQuantity" data-wp-bind--disabled="!state.allowsDecrease" class="wc-block-components-quantity-selector__button wc-block-components-quantity-selector__button--minus">−</button>')
	}
...
fn Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.add_quantity_stepper_classes(var_quantity_html rt.PhpVal) rt.PhpVal {
	mut var_quantity_html_mutated := var_quantity_html
	mut var_processor :=
		create_automattic_woocommerce_blocks_blocktypes_addtocartwithoptions_wp_html_tag_processor(var_quantity_html_mutated.clone())
	for rt.is_true(var_processor.next_tag()) {
...
fn Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.make_quantity_input_interactive(var_quantity_html rt.PhpVal, var_wrapper_attributes rt.PhpVal, var_input_attributes rt.PhpVal, var_context rt.PhpVal, set_product_context bool) rt.PhpVal {
	...
	mut var_processor :=
		create_automattic_woocommerce_blocks_blocktypes_addtocartwithoptions_wp_html_tag_processor(var_quantity_html_mutated.clone())
	if var_set_product_context
		&& rt.is_true(rt.new_bool(rt.instance_of(var_product, 'Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_WC_Product'))) {
		mut var_product_context := rt.create_array([
			rt.ArrayItem{ key: 'productId', val: rt.call_method(var_product, 'get_id',
				[]rt.PhpVal{}) },
			rt.ArrayItem{ key: 'variationId', val: rt.new_null() },
		])
		mut var_products_context :=
			rt.new_string('woocommerce/products::' +(rt.call_function('wp_json_encode', [var_product_context.clone(), rt.bitwise_or(rt.bitwise_or(rt.bitwise_or(rt.get_constant('JSON_HEX_TAG'), rt.get_constant('JSON_HEX_APOS')), rt.get_constant('JSON_HEX_QUOT')), rt.get_constant('JSON_HEX_AMP'))])).str())
		if rt.is_true(var_processor.next_tag(rt.create_array([
			rt.ArrayItem{ key: 'tag_name', val: 'div' },
			rt.ArrayItem{ key: 'class_name', val: 'quantity' },
		])))
		{
			var_processor.set_attribute(rt.new_string('data-wp-context'),
				var_products_context.clone())
		} else {
			var_processor =
				create_automattic_woocommerce_blocks_blocktypes_addtocartwithoptions_wp_html_tag_processor(var_quantity_html_mutated.clone())
		}
	}
	if rt.is_true(var_processor.next_tag(rt.new_string('input')))
		&& rt.is_true(rt.identical(var_processor.get_attribute(rt.new_string('type')), rt.new_string('number')))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('strpos', [var_processor.get_attribute(rt.new_string('name')), rt.new_string('quantity')]), rt.new_bool(false))))) {
		mut var_default_quantity := if rt.is_true(rt.new_bool(rt.instance_of(var_product,
		...
		mut var_input_quantity := if var_context.array_isset(rt.new_string('allowZero'))
			&& rt.is_true(rt.identical(rt.new_bool(true), var_context.array_get(rt.new_string('allowZero')))) {
			rt.new_int(0)
		} else {
		...
		rt.call_function('wp_interactivity_state', [
			...
			rt.create_array([
				rt.ArrayItem{ key: 'inputQuantity', val: var_input_quantity },
			]),
		])
		var_processor.set_attribute(rt.new_string('data-wp-on--blur'),
			rt.new_string('actions.handleQuantityBlur'))
		var_processor.set_attribute(rt.new_string('data-wp-bind--value'),
			rt.new_string('state.inputQuantity'))
		mut iter_1 := var_input_attributes.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_value := item_1.val
			mut var_attribute := item_1.key
			var_processor.set_attribute(var_attribute.clone(), var_value.clone())
		}
	}
	var_quantity_html_mutated = var_processor.get_updated_html()
	var_wrapper_attributes_mutated = rt.call_function('array_merge', [
		rt.create_array([
			rt.ArrayItem{
				key: 'data-wp-interactive'
				val: 'woocommerce/add-to-cart-with-options-quantity-selector'
			},
			rt.ArrayItem{ key: 'data-wp-init', val: 'callbacks.storeInputElementRef' },
		]),
		var_wrapper_attributes_mutated.clone(),
	])
	mut var_context_attribute := rt.call_function('wp_interactivity_data_wp_context', [
		var_context.clone(),
	])
	return rt.call_function('sprintf', [rt.new_string('<div %1$s %2$s>%3$s</div>'),
		rt.call_function('get_block_wrapper_attributes', [var_wrapper_attributes_mutated.clone()]),
		var_context_attribute.clone(), var_quantity_html_mutated.clone()])
}
...
fn Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.get_product_from_context(var_block rt.PhpVal, var_previous_product rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_product, 'Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_WC_Product'))))))
		&& rt.is_true(rt.new_bool(rt.instance_of(var_previous_product, 'Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_WC_Product'))) {
...
fn Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.is_not_purchasable_product(var_product rt.PhpVal) bool {
	...
	{
		return
			rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_product_mutated, 'is_in_stock', []rt.PhpVal{})))))
			|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_product_mutated, 'is_purchasable', []rt.PhpVal{})))))
	...
	]))
	{
		return
			rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_product_mutated, 'is_in_stock', []rt.PhpVal{})))))
			|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_product_mutated, 'has_purchasable_variations', []rt.PhpVal{})))))
	}
	return false
}
...
fn Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.render_block_with_context(var_block rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	...
	mut var_new_block := create_wp_block(var_block_instance.clone(), var_context.clone())
	return var_new_block.render(rt.create_array([
		rt.ArrayItem{ key: 'dynamic', val: false },
	]))
}
...
fn Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.is_min_max_quantity_same(var_product rt.PhpVal) rt.PhpVal {
	mut var_product_mutated := var_product
	mut var_min_purchase_quantity := rt.call_method(var_product_mutated,
		'get_min_purchase_quantity', []rt.PhpVal{})
	mut var_max_purchase_quantity := rt.call_method(var_product_mutated,
...
fn Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.get_product_quantity_constraints(var_product rt.PhpVal) rt.PhpVal {
	mut var_product_mutated := var_product
	mut var_min := if
		rt.call_method(var_product_mutated, 'get_min_purchase_quantity', []rt.PhpVal{}).is_long()
		|| rt.call_method(var_product_mutated, 'get_min_purchase_quantity', []rt.PhpVal{}).is_double() {
		rt.call_method(var_product_mutated, 'get_min_purchase_quantity', []rt.PhpVal{})
	} else {
		rt.new_int(1)
	}
	mut var_max_quantity := rt.call_method(var_product_mutated, 'get_max_purchase_quantity',
		[]rt.PhpVal{})
	mut var_max := if var_max_quantity.clone().is_long()
		|| var_max_quantity.clone().is_double()
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(-1, var_max_quantity)))) {
		var_max_quantity
	} else {
		rt.new_null()
	}
	mut var_step := if
		rt.call_method(var_product_mutated, 'get_purchase_quantity_step', []rt.PhpVal{}).is_long()
		|| rt.call_method(var_product_mutated, 'get_purchase_quantity_step', []rt.PhpVal{}).is_double() {
		rt.call_method(var_product_mutated, 'get_purchase_quantity_step', []rt.PhpVal{})
	} else {
		rt.new_int(1)
	}
	return rt.create_array([rt.ArrayItem{ key: 'min', val: var_min },
		rt.ArrayItem{ key: 'max', val: var_max }, rt.ArrayItem{ key: 'step', val: var_step }])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'add_quantity_steppers' {
			...
			return Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.add_quantity_steppers(dispatch_arg_0,
				dispatch_arg_1)
		}
		...
		'make_quantity_input_interactive' {
			...
			return Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.make_quantity_input_interactive(dispatch_arg_0,
				dispatch_arg_1, dispatch_arg_2, dispatch_arg_3, dispatch_arg_4)
		}
		'get_product_from_context' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.get_product_from_context(dispatch_arg_0,
				dispatch_arg_1)
		}
		...
		'render_block_with_context' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return Class_Automattic_WooCommerce_Blocks_BlockTypes_AddToCartWithOptions_Utils.render_block_with_context(dispatch_arg_0,
				dispatch_arg_1)
		}
