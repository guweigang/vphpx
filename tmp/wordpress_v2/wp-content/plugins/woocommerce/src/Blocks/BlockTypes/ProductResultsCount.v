...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductResultsCount) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	mut var_p :=
		create_automattic_woocommerce_blocks_blocktypes_wp_html_tag_processor(var_product_results_count.clone())
	...
	mut var_parsed_style_attributes := iife_result_0
	mut var_classes := rt.call_function('array_merge', [
		rt.call_function('explode',
			[rt.new_string(' '), var_parsed_style_attributes.array_get(rt.new_string('classes'))]),
	...
	var_p.set_attribute(rt.new_string('style'),
		var_parsed_style_attributes.array_get(rt.new_string('styles')))
	...
	var_p.set_attribute(rt.new_string('data-wp-router-region'), rt.new_string(
		'wc-product-results-count-' +(if rt.get_property(var_block, 'context').array_isset(rt.new_string('queryId')) { rt.get_property(var_block, 'context').array_get(rt.new_string('queryId')) } else { rt.new_int(0) }).str()))
...
fn create_automattic_woocommerce_blocks_blocktypes_productresultscount(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductResultsCount {
