...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductGallery) render_dialog(var_images rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		mut var_id := var_image.array_get(rt.new_string('id'))
		mut var_src := var_image.array_get(rt.new_string('src'))
		mut var_srcset := var_image.array_get(rt.new_string('srcset'))
		mut var_sizes := var_image.array_get(rt.new_string('sizes'))
		mut var_alt := var_image.array_get(rt.new_string('alt'))
		mut var_loading := rt.new_string((if rt.is_true(rt.identical(rt.new_int(0), var_index)) {
			'fetchpriority="high"'
		} else {
			'loading="lazy"'
		}).str())
		var_images_html = rt.concat(var_images_html,
			rt.new_string("<img data-image-id='${var_id.to_string()}' src='${var_src.to_string()}' srcset='${var_srcset.to_string()}' sizes='${var_sizes.to_string()}' loading='${var_loading.to_string()}' decoding='async' alt='${var_alt.to_string()}' />"))
	}
	rt.call_function('ob_start', []rt.PhpVal{})
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr__', [rt.new_string('Close dialog'),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductGallery) inject_dialog(var_gallery_html rt.PhpVal, var_dialog_html rt.PhpVal) rt.PhpVal {
	mut var_pos := rt.call_function('strrpos', [var_gallery_html.clone(),
		rt.new_string('</div>')])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_pos)))) {
		mut var_html := rt.call_function('substr_replace', [var_gallery_html.clone(),
			var_dialog_html.clone(), var_pos.clone(), rt.new_int(0)])
		return var_html.clone()
	}
	return var_gallery_html.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductGallery) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut iife_result_0 := iife_temp_0.get_all_image_ids(var_product.clone())
	mut var_image_ids := iife_result_0
	mut var_number_of_images := rt.new_int(var_image_ids.clone().array_count())
	...
	mut var_initial_image_id := if rt.is_true(rt.greater(var_number_of_images, rt.new_int(0))) {
		var_image_ids.array_get(rt.new_int(0))
	} else {
		-1
	}
	mut var_classname_single_image := rt.new_string((if rt.is_true(rt.less(var_number_of_images,
		rt.new_int(2)))
	{
		'is-single-product-gallery-image'
	} else {
		''
	}).str())
	mut var_product_id :=
		rt.new_string(rt.call_method(var_product, 'get_id', []rt.PhpVal{}).to_string())
	mut iife_temp_2 := Class_Automattic_WooCommerce_Blocks_Utils_ProductGalleryUtils{}
	mut iife_result_2 := iife_temp_2.get_image_src_data(var_image_ids.clone(),
		rt.new_string('full'), rt.call_method(var_product, 'get_title', []rt.PhpVal{}))
	mut var_fullsize_image_data := iife_result_2
	mut var_gallery_with_dialog := this.inject_dialog(var_content.clone(),
		this.render_dialog(var_fullsize_image_data.clone()))
	mut var_p :=
		create_automattic_woocommerce_blocks_blocktypes_wp_html_tag_processor(var_gallery_with_dialog.clone())
	mut var_html := var_gallery_with_dialog.clone()
	if rt.is_true(var_p.next_tag()) {
		...
		var_p.set_attribute(rt.new_string('data-wp-context'), rt.call_function('wp_json_encode', [
			rt.create_array([rt.ArrayItem{ key: 'imageData', val: var_image_ids },
				rt.ArrayItem{ key: 'isDialogOpen', val: false },
				rt.ArrayItem{ key: 'isDragging', val: false },
				rt.ArrayItem{ key: 'touchStartX', val: 0 }, rt.ArrayItem{
					key: 'touchCurrentX'
					val: 0
				}, rt.ArrayItem{ key: 'productId', val: var_product_id },
				rt.ArrayItem{ key: 'selectedImageId', val: var_initial_image_id },
				rt.ArrayItem{ key: 'thumbnailsOverflow', val: rt.create_array([
					rt.ArrayItem{ key: 'top', val: false },
					rt.ArrayItem{ key: 'bottom', val: false },
					rt.ArrayItem{ key: 'left', val: false },
					rt.ArrayItem{ key: 'right', val: false },
				]) }, rt.ArrayItem{ key: 'hideNextPreviousButtons', val: rt.less_equal(var_number_of_images,
					rt.new_int(1)) }, rt.ArrayItem{ key: 'isDisabledPrevious', val: true },
				rt.ArrayItem{ key: 'isDisabledNext', val: false },
				rt.ArrayItem{ key: 'ariaLabelPrevious', val: rt.call_function('__', [
					rt.new_string('Previous image'),
					rt.new_string('woocommerce'),
				]) }, rt.ArrayItem{ key: 'ariaLabelNext', val: rt.call_function('__', [
					rt.new_string('Next image'),
					rt.new_string('woocommerce'),
				]) }]),
		...
		{
			mut var_has_variation_images := rt.new_bool(false)
			mut iter_2 := rt.call_method(var_product, 'get_available_variations', [
				rt.new_string('objects'),
			]).iterator()
			for {
				item_2 := iter_2.next() or { break }
				mut var_variation := item_2.val
				if rt.is_true(rt.new_int((rt.call_method(var_variation, 'get_image_id',
					[]rt.PhpVal{})).to_i64()))
				{
					var_has_variation_images = rt.new_bool(true)
					break
				}
			}
			if rt.is_true(var_has_variation_images) {
				var_p.set_attribute(rt.new_string('data-wp-init--watch-changes-on-add-to-cart-form'),
					rt.new_string('callbacks.watchForChangesOnAddToCartForm'))
				var_p.set_attribute(rt.new_string('data-wp-watch'),
					rt.new_string('callbacks.listenToProductDataChanges'))
			}
		}
		var_p.add_class(var_classname.clone())
		var_p.add_class(var_classname_single_image.clone())
...
fn create_automattic_woocommerce_blocks_blocktypes_productgallery(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductGallery {
