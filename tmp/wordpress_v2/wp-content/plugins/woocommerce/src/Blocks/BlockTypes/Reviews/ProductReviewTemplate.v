...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewTemplate) block_product_review_template_render_comments(mut var_comments Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_array, mut var_block Class_WP_Block) string {
	...
	for {
		...
		mut var_comment_classes := rt.call_function('comment_class', [
			rt.new_string(''), rt.new_int((rt.get_property(var_comment, 'comment_ID')).to_i64()),
			rt.new_int((rt.get_property(var_comment, 'comment_post_ID')).to_i64()),
			rt.new_bool(false)])
		if !(!rt.is_true(var_children)) {
			mut var_inner_content := rt.new_string(this.block_product_review_template_render_comments(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_array](var_children), mut
				var_block))
		...
		var_content = rt.concat(var_content, rt.call_function('sprintf', [
			rt.new_string('<li id="comment-%1$s" %2$s>%3$s</li>'),
			rt.get_property(var_comment, 'comment_ID'),
			var_comment_classes.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewTemplate) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	return (rt.call_function('sprintf', [rt.new_string('<ol %1$s>%2$s</ol>'),
		...
		rt.new_string(this.block_product_review_template_render_comments(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_array](var_comments), mut
			rt.cast_object_ptr[Class_WP_Block](var_block)))])).str()
}
...
fn create_automattic_woocommerce_blocks_blocktypes_reviews_productreviewtemplate(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewTemplate {
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewTemplate) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'block_product_review_template_render_comments' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_WP_Block](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
			return rt.new_string(this.block_product_review_template_render_comments(mut dispatch_arg_0, mut
				dispatch_arg_1))
		}
		'render' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
