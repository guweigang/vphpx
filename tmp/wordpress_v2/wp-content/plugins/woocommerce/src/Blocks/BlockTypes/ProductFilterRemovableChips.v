...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilterRemovableChips) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_filter_items := if !(rt.get_property(var_block, 'context').array_get(rt.new_string('filterData')).array_get(rt.new_string('items'))).is_null() {
	...
	for {
		...
		mut var_remove_label := rt.call_function('sprintf', [
			...
			var_item.array_get(rt.new_string('activeLabel')),
		])
		// unsupported statement: Stmt_InlineHTML
		...
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_remove_label.clone()]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('wp_interactivity_data_wp_context', [
			rt.create_array([rt.ArrayItem{ key: 'item', val: var_item }]),
		]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_html', [var_remove_label.clone()]))
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
...
fn create_automattic_woocommerce_blocks_blocktypes_productfilterremovablechips(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilterRemovableChips {
