...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_FeaturedProduct) get_item(var_attributes rt.PhpVal) rt.PhpVal {
	mut var_id := rt.call_function('absint', [if !(var_attributes.array_get(rt.new_string('productId'))).is_null() {
		var_attributes.array_get(rt.new_string('productId'))
	} else {
		rt.new_int(0)
	}])
	mut var_product := rt.call_function('wc_get_product', [var_id.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_product))))|| (rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Enums_ProductStatus.publish(), rt.call_method(var_product, 'get_status', []rt.PhpVal{})))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('read_product'), var_id.clone()])))))) {
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_FeaturedProduct) get_item_image(var_product rt.PhpVal, size string) rt.PhpVal {
	...
	if rt.is_true(rt.call_method(var_product_mutated, 'get_image_id', []rt.PhpVal{})) {
		var_image = rt.call_function('wp_get_attachment_image_url', [
		...
		if rt.is_true(var_parent_product) {
			var_image = rt.call_function('wp_get_attachment_image_url', [
				rt.call_method(var_parent_product, 'get_image_id', []rt.PhpVal{}),
				rt.new_string(size),
			])
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_FeaturedProduct) render_attributes(var_product rt.PhpVal, var_attributes rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_attributes.clone().array_isset(rt.new_string('editMode'))))
		&& var_attributes.array_get(rt.new_string('editMode')).is_bool() {
		mut var_legacy_title := rt.call_function('sprintf', [
			rt.new_string('<h2 class="wc-block-featured-product__title">%s</h2>'),
		...
		{
			var_legacy_title = rt.concat(var_legacy_title, rt.call_function('sprintf', [
				rt.new_string('<h3 class="wc-block-featured-product__variation">%s</h3>'),
				rt.call_function('wp_kses_post', [
					rt.call_function('wc_get_formatted_variation', [
						var_product_mutated.clone(), rt.new_bool(true),
						rt.new_bool(true), rt.new_bool(false)]),
				]),
			]))
		}
		var_output = rt.concat(var_output, var_legacy_title)
		if !(var_attributes.array_isset(rt.new_string('showDesc')))
			|| (var_attributes.array_isset(rt.new_string('showDesc'))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_attributes.array_get(rt.new_string('showDesc'))))))) {
			mut var_desc_str := rt.call_function('sprintf', [
				rt.new_string('<div class="wc-block-featured-product__description">%s</div>'),
				rt.call_function('wc_format_content', [
					rt.call_function('wp_kses_post', [if rt.is_true(rt.call_method(var_product_mutated, 'get_short_description', []rt.PhpVal{})) { rt.call_method(var_product_mutated, 'get_short_description', []rt.PhpVal{}) } else { rt.call_function('wc_trim_string', [
							rt.call_method(var_product_mutated, 'get_description', []rt.PhpVal{}),
							rt.new_int(400),
						]) }]),
				]),
			])
			var_output = rt.concat(var_output, var_desc_str)
		}
		if !(var_attributes.array_isset(rt.new_string('showPrice')))
			|| (var_attributes.array_isset(rt.new_string('showPrice'))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_attributes.array_get(rt.new_string('showPrice'))))))) {
			mut var_price_str := rt.call_function('sprintf', [
				rt.new_string('<div class="wc-block-featured-product__price">%s</div>'),
				rt.call_function('wp_kses_post', [
					rt.call_method(var_product_mutated, 'get_price_html', []rt.PhpVal{}),
				]),
			])
			var_output = rt.concat(var_output, var_price_str)
		}
	}
...
fn create_automattic_woocommerce_blocks_blocktypes_featuredproduct(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_FeaturedProduct {
