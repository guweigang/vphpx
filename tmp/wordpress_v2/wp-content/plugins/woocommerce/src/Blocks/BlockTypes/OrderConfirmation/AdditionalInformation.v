...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AdditionalInformation) render_content(var_order rt.PhpVal, permission bool, var_attributes rt.PhpVal, content string) rt.PhpVal {
	...
	content = content + (this.get_hook_content(rt.new_string('woocommerce_thankyou_' + (rt.call_method(var_order, 'get_payment_method', []rt.PhpVal{})).str()), rt.create_array([rt.ArrayItem{
		key: none
		val: rt.call_method(var_order, 'get_id', []rt.PhpVal{})
	}]))).str()
	content = content + (this.get_hook_content(rt.new_string('woocommerce_thankyou'), rt.create_array([rt.ArrayItem{
		key: none
		val: rt.call_method(var_order, 'get_id', []rt.PhpVal{})
	}]))).str()
...
fn create_automattic_woocommerce_blocks_blocktypes_orderconfirmation_additionalinformation(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AdditionalInformation {
