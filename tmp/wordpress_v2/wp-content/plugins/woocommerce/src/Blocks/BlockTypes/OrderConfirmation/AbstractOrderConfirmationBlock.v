...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) get_hook_content(var_hook rt.PhpVal, var_args rt.PhpVal) rt.PhpVal {
	...
	rt.call_function('do_action_ref_array', [var_hook.clone(),
		var_args.clone()])
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	mut var_permission := this.get_view_order_permissions(var_order.clone())
	mut var_block_content := if rt.is_true(var_order) {
		this.render_content(var_order.clone(), var_permission.to_bool(), var_attributes.clone(),
			var_content.str())
	} else {
		this.render_content_fallback()
	}
	...
	return if rt.is_true(var_block_content) { rt.call_function('sprintf', [
			rt.new_string('<div class="wp-block-%5$s-%4$s wc-block-%4$s %1$s" style="%2$s">%3$s</div>'),
			...
			rt.call_function('esc_attr', [
				rt.get_property(rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock', [
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) get_order() rt.PhpVal {
	mut var_order_id := rt.call_function('absint', [
		rt.call_function('get_query_var', [rt.new_string('order-received')]),
	])
	if rt.is_true(var_order_id) {
		return rt.call_function('wc_get_order', [var_order_id.clone()])
	}
	return rt.new_null()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) get_view_order_permissions(var_order rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order_mutated))))
		|| !(this.has_valid_order_key(var_order_mutated.clone())) {
		return rt.new_bool(false)
	}
	if rt.is_true(this.is_customer_order(var_order_mutated.clone())) {
		...
		return if this.is_current_customer_order(var_order_mutated.clone()) {
			rt.new_string('full')
		} else {
			rt.new_bool(false)
		}
	}
	return if this.email_verification_required(var_order_mutated.clone()) {
		rt.new_bool(false)
	} else {
		rt.new_string('full')
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) email_verification_permitted(var_order rt.PhpVal) bool {
	mut var_order_mutated := var_order
	return rt.is_true(this.allow_guest_checkout())
		&& this.has_valid_order_key(var_order_mutated.clone())
		&& rt.is_true(rt.new_bool(!(rt.is_true(this.is_customer_order(var_order_mutated.clone())))))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) is_within_grace_period(var_order rt.PhpVal) bool {
	mut var_order_mutated := var_order
	mut var_verification_grace_period := rt.new_int((rt.call_function('apply_filters', [
		rt.new_string('woocommerce_order_email_verification_grace_period'),
	...
	return
		rt.is_true(rt.call_function('is_a', [var_date_created.clone(), Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_WC_DateTime.class()]))
		&& rt.is_true(rt.less_equal(rt.sub(rt.call_function('time', []rt.PhpVal{}), rt.call_method(var_date_created, 'getTimestamp', []rt.PhpVal{})), var_verification_grace_period))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) is_email_verified(var_order rt.PhpVal) bool {
	...
	mut var_nonce_value := rt.call_function('sanitize_key', [
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_verify_nonce', [var_nonce_value.clone(), rt.new_string('wc_verify_email')])))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_verify_nonce', [var_nonce_value.clone(), rt.new_string('wc_create_account')]))))) {
		return false
	}
	return rt.is_true(rt.call_method(var_order_mutated, 'get_billing_email', []rt.PhpVal{}))
		&& rt.is_true(rt.identical(rt.call_function('sanitize_email', [rt.call_function('wp_unslash', [if !(rt.get_superglobal('_POST').array_get(rt.new_string('email'))).is_null() { rt.get_superglobal('_POST').array_get(rt.new_string('email')) } else { rt.new_string('') }])]), rt.call_method(var_order_mutated, 'get_billing_email', []rt.PhpVal{})))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) email_verification_required(var_order rt.PhpVal) bool {
	...
	if rt.is_true(rt.call_function('is_a', [var_session.clone(), Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_WC_Session.class()]))
		&& rt.is_true(rt.identical(rt.call_method(var_order_mutated, 'get_id', []rt.PhpVal{}), rt.new_int((rt.call_method(var_session, 'get', [rt.new_string('store_api_draft_order')])).to_i64()))) {
		return false
	}
	if this.is_within_grace_period(var_order_mutated.clone()) {
		return false
	}
	if this.is_email_verified(var_order_mutated.clone()) {
		return false
	}
	return (rt.call_function('apply_filters', [
		rt.new_string('woocommerce_order_email_verification_required'),
		rt.new_bool(true),
		var_order_mutated.clone(),
		rt.new_string('order-received'),
	])).to_bool()
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) has_valid_order_key(var_order rt.PhpVal) bool {
	mut var_order_mutated := var_order
	return !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('key'))))
		&& rt.is_true(rt.call_method(var_order_mutated, 'key_is_valid', [rt.call_function('wc_clean', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('key'))])])]))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) is_current_customer_order(var_order rt.PhpVal) bool {
	mut var_order_mutated := var_order
	return rt.is_true(this.is_customer_order(var_order_mutated.clone()))
		&& rt.is_true(rt.identical(rt.call_method(var_order_mutated, 'get_user_id', []rt.PhpVal{}), rt.call_function('get_current_user_id', []rt.PhpVal{})))
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock) render_additional_fields(var_fields rt.PhpVal) string {
	...
	return '<dl class="wc-block-components-additional-fields-list">' +
		(rt.call_function('implode', [rt.new_string(''), rt.call_function('array_map', [rt.create_array([rt.ArrayItem{
		key: none
		val: rt.new_object('Automattic_WooCommerce_Blocks_BlockTypes_OrderConfirmation_AbstractOrderConfirmationBlock', ['Automattic_WooCommerce_Blocks_BlockTypes_AbstractBlock'], &this)
	}, rt.ArrayItem{ key: none, val: 'render_additional_field' }]), var_fields.clone()])])).str() +
		'</dl>'
}
