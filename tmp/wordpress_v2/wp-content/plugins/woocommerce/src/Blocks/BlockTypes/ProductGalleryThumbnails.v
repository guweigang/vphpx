...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductGalleryThumbnails) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut iife_result_1 := iife_temp_1.get_product_gallery_image_data(var_product.clone(),
		var_image_size.clone())
	mut var_product_gallery_images := iife_result_1
	if var_product_gallery_images.clone().array_count() <= 1 {
		return ''
	}
	mut var_thumbnail_size := rt.call_function('str_replace', [
		rt.new_string('%'), rt.new_string(''), if !(var_attributes.array_get(rt.new_string('thumbnailSize'))).is_null() {
			var_attributes.array_get(rt.new_string('thumbnailSize'))
		} else {
			rt.new_string('25%')
		}])
	mut var_active_thumbnail_style := if !(var_attributes.array_get(rt.new_string('activeThumbnailStyle'))).is_null() {
		var_attributes.array_get(rt.new_string('activeThumbnailStyle'))
	} else {
		rt.new_string('overlay')
	}
	mut var_img_class := rt.new_string('wc-block-product-gallery-thumbnails__thumbnail__image')
	rt.call_function('ob_start', []rt.PhpVal{})
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_active_thumbnail_style.clone()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr',
		[var_classes_and_styles.array_get(rt.new_string('classes'))]))
	// unsupported statement: Stmt_InlineHTML
	print('--wc-block-product-gallery-thumbnails-size:' +
		(rt.call_function('absint', [var_thumbnail_size.clone()])).str() + ';' +(rt.call_function('esc_attr', [var_classes_and_styles.array_get(rt.new_string('styles'))])).str())
	// unsupported statement: Stmt_InlineHTML
	...
	for {
		...
		rt.echo_val(if rt.is_true(rt.identical(rt.new_int(0), var_index)) { rt.call_function('esc_attr', [
				rt.new_string(var_img_class.str() + ' wc-block-product-gallery-thumbnails__thumbnail__image--is-active'),
			]) } else { rt.call_function('esc_attr', [var_img_class.clone()]) })
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_image.array_get(rt.new_string('id'))]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_image.array_get(rt.new_string('src'))]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_image.array_get(rt.new_string('srcset'))]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_image.array_get(rt.new_string('sizes'))]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_image.array_get(rt.new_string('alt'))]))
		// unsupported statement: Stmt_InlineHTML
...
fn create_automattic_woocommerce_blocks_blocktypes_productgallerythumbnails(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductGalleryThumbnails {
