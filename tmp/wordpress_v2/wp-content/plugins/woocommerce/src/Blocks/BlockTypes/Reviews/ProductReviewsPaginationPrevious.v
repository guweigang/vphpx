...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewsPaginationPrevious) get_pagination_arrow(var_block rt.PhpVal) rt.PhpVal {
	...
	if !(!rt.is_true(rt.get_property(var_block, 'context').array_get(rt.new_string('reviews/paginationArrow'))))
		...
		mut var_arrow_classes :=
			rt.new_string('wp-block-woocommerce-product-reviews-pagination-previous-arrow is-arrow-${var_arrow_attribute.to_string()}')
...
fn create_automattic_woocommerce_blocks_blocktypes_reviews_productreviewspaginationprevious(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewsPaginationPrevious {
