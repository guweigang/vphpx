...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewsPaginationNumbers) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_total := rt.get_property(create_automattic_woocommerce_blocks_blocktypes_reviews_wp_comment_query(var_comment_vars.clone()),
		'max_num_pages')
	mut var_current := if !(!rt.is_true(var_comment_vars.array_get(rt.new_string('paged')))) {
...
fn create_automattic_woocommerce_blocks_blocktypes_reviews_productreviewspaginationnumbers(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_Reviews_ProductReviewsPaginationNumbers {
