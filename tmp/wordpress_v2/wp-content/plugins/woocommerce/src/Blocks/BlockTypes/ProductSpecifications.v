...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductSpecifications) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_show_weight := if var_attributes.array_isset(rt.new_string('showWeight')) {
		var_attributes.array_get(rt.new_string('showWeight'))
	} else {
		rt.new_bool(true)
	}
	mut var_show_dimensions := if var_attributes.array_isset(rt.new_string('showDimensions')) {
		var_attributes.array_get(rt.new_string('showDimensions'))
	} else {
		rt.new_bool(true)
	}
	mut var_show_attributes := if var_attributes.array_isset(rt.new_string('showAttributes')) {
		var_attributes.array_get(rt.new_string('showAttributes'))
	} else {
		rt.new_bool(true)
	}
	if rt.is_true(var_show_weight)
	...
	if rt.is_true(var_show_dimensions)
	...
	// unsupported statement: Stmt_InlineHTML
	mut iter_4 := var_product_data.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_product_attribute := item_4.val
		mut var_product_attribute_key := item_4.key
		// unsupported statement: Stmt_InlineHTML
		...
		if rt.is_true(var_is_interactive)
			...
			rt.echo_val(rt.call_function('esc_attr', [
				var_product_attribute.array_get(rt.new_string('api_field')),
			]))
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('wp_kses_post', [
...
fn create_automattic_woocommerce_blocks_blocktypes_productspecifications(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductSpecifications {
