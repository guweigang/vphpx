...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_FeaturedCategory) get_block_type_attributes() rt.PhpVal {
	return rt.call_function('array_merge', [this.Class_Automattic_WooCommerce_Blocks_BlockTypes_FeaturedItem.get_block_type_attributes(),
		rt.create_array([rt.ArrayItem{ key: 'textColor', val: this.get_schema_string() },
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_FeaturedCategory) get_item(var_attributes rt.PhpVal) rt.PhpVal {
	mut var_id := rt.call_function('absint', [if !(var_attributes.array_get(rt.new_string('categoryId'))).is_null() {
		var_attributes.array_get(rt.new_string('categoryId'))
	} else {
		rt.new_int(0)
	}])
	mut var_category := rt.call_function('get_term', [var_id.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_FeaturedCategory) get_item_image(var_category rt.PhpVal, size string) rt.PhpVal {
	...
	if rt.is_true(var_image_id) {
		var_image = rt.call_function('wp_get_attachment_image_url', [
			var_image_id.clone(), rt.new_string(size)])
	}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_FeaturedCategory) render_attributes(var_category rt.PhpVal, var_attributes rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_attributes.clone().array_isset(rt.new_string('editMode'))))
		...
		mut var_legacy_title := rt.call_function('sprintf', [
			rt.new_string('<h2 class="wc-block-featured-category__title">%s</h2>'),
			rt.call_function('wp_kses_post', [
				rt.get_property(var_category_mutated, 'name'),
			]),
		])
		...
		if !(var_attributes.array_isset(rt.new_string('showDesc')))
			...
			mut var_desc_str := rt.call_function('sprintf', [
				rt.new_string('<div class="wc-block-featured-category__description">%s</div>'),
				rt.call_function('wc_format_content', [
					rt.call_function('wp_kses_post', [
						rt.get_property(var_category_mutated, 'description'),
					]),
				]),
			])
...
fn create_automattic_woocommerce_blocks_blocktypes_featuredcategory(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_FeaturedCategory {
