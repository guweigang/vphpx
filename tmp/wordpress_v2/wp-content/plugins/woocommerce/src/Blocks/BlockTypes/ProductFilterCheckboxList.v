...
fn (mut this Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilterCheckboxList) render(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_block_context :=
		rt.get_property(var_block, 'context').array_get(rt.new_string('filterData'))
	mut var_items := if !(var_block_context.array_get(rt.new_string('items'))).is_null() {
		var_block_context.array_get(rt.new_string('items'))
	} else {
		rt.new_array()
	}
	mut var_show_counts := if !(var_block_context.array_get(rt.new_string('showCounts'))).is_null() {
		var_block_context.array_get(rt.new_string('showCounts'))
	} else {
		rt.new_bool(false)
	}
	...
	// unsupported statement: Stmt_InlineHTML
	if !(!rt.is_true(var_block_context.array_get(rt.new_string('groupLabel')))) {
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_html', [
			var_block_context.array_get(rt.new_string('groupLabel')),
		]))
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
	...
	for {
		...
		rt.echo_val(if var_item.array_isset(rt.new_string('depth')) { rt.call_function('esc_attr', [
				rt.new_string('has-depth-' + (var_item.array_get(rt.new_string('depth'))).str()),
			]) } else { rt.new_string('') })
		// unsupported statement: Stmt_InlineHTML
...
fn create_automattic_woocommerce_blocks_blocktypes_productfiltercheckboxlist(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_BlockTypes_ProductFilterCheckboxList {
