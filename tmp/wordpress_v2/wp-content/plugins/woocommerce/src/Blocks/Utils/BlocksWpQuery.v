...
fn (mut this Class_Automattic_WooCommerce_Blocks_Utils_BlocksWpQuery) get_cached_posts(transient_version string) rt.PhpVal {
	...
	if !var_transient_value.is_null() && var_transient_value.array_isset(rt.new_string('version'))
		&& var_transient_value.array_isset(rt.new_string('value'))
		&& rt.is_true(rt.identical(var_transient_value.array_get(rt.new_string('version')), rt.new_string(transient_version))) {
		return var_transient_value.array_get(rt.new_string('value'))
	}
