...
fn init_static_automattic_woocommerce_blocks_utils_blockssharedstate() {
	rt.init_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
		'consent_statement',
...
fn Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState.prevent_cache() {
	mut iife_temp_0 := Class_Automattic_WooCommerce_Blocks_Utils_WC_Cache_Helper{}
	mut iife_result_0 := iife_temp_0.set_nocache_constants()
...
fn Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState.check_consent(consent_statement string) bool {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(consent_statement), rt.get_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
		'consent_statement')))))
	{
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{},
			create_invalidargumentexception(rt.new_string('This method cannot be called without consenting the API may change.'))))
	}
	return true
}
...
fn Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState.load_store_config(consent_statement string) {
	...
	if rt.is_true(rt.get_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
		'core_config_registered'))
	{
		return
	}
	rt.set_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
		'core_config_registered', rt.new_bool(true))
	rt.call_function('wp_interactivity_config', [
		rt.get_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
			'settings_namespace'),
		Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState.get_currency_data(),
	])
	rt.call_function('wp_interactivity_config', [
		rt.get_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
			'settings_namespace'),
		Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState.get_locale_data(),
	])
}

fn Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState.load_cart_state(consent_statement string) {
	Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState.check_consent(consent_statement)
	if rt.is_true(rt.identical(rt.new_null(), rt.get_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
		'blocks_shared_cart_state')))
	{
		mut var_cart_exists := rt.new_bool(!(rt.get_property(rt.call_function('WC', []rt.PhpVal{}),
			'cart')).is_null())
		mut var_cart_has_contents := rt.new_bool(rt.is_true(var_cart_exists)
		...
		if rt.is_true(var_cart_exists) {
			...
			mut var_cart_response := rt.call_method(rt.call_method(iife_result_1, 'get', [
				Class_Automattic_WooCommerce_Blocks_Domain_Services_Hydration.class(),
			]), 'get_rest_api_response_data', [rt.new_string('/wc/store/v1/cart')])
			rt.set_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
				'blocks_shared_cart_state', if !(var_cart_response.array_get(rt.new_string('body'))).is_null() {
				var_cart_response.array_get(rt.new_string('body'))
			} else {
				rt.new_array()
			})
		} else {
			rt.set_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
				'blocks_shared_cart_state', rt.new_array())
		}
		...
		rt.call_function('wp_interactivity_config', [
			...
			rt.create_array([
				rt.ArrayItem{
					key: 'nonOptimisticProperties'
					val: Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState.get_non_optimistic_properties()
				},
			]),
		])
		rt.call_function('wp_interactivity_state', [
			rt.get_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
				'settings_namespace'),
			rt.create_array([
				rt.ArrayItem{ key: 'cart', val: rt.get_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
					'blocks_shared_cart_state') },
				rt.ArrayItem{ key: 'noticeId', val: '' },
				rt.ArrayItem{ key: 'restUrl', val: rt.call_function('get_rest_url', []rt.PhpVal{}) },
			]),
		])
	}
}
...
fn Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState.get_currency_data() rt.PhpVal {
	...
	return rt.create_array([
		rt.ArrayItem{ key: 'currency', val: rt.create_array([
			rt.ArrayItem{ key: 'code', val: var_currency },
			rt.ArrayItem{ key: 'precision', val: rt.call_function('wc_get_price_decimals',
				[]rt.PhpVal{}) },
			rt.ArrayItem{ key: 'symbol', val: rt.call_function('html_entity_decode', [
			...
			rt.ArrayItem{ key: 'decimalSeparator', val: rt.call_function('wc_get_price_decimal_separator',
				[]rt.PhpVal{}) },
			rt.ArrayItem{ key: 'thousandSeparator', val: rt.call_function('wc_get_price_thousand_separator',
				[]rt.PhpVal{}) },
			rt.ArrayItem{ key: 'priceFormat', val: rt.call_function('html_entity_decode', [
...
fn Class_Automattic_WooCommerce_Blocks_Utils_BlocksSharedState.get_cart_error_notices(consent_statement string) rt.PhpVal {
	...
	mut var_errors := if !(rt.get_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
		'blocks_shared_cart_state').array_get(rt.new_string('errors'))).is_null() {
		rt.get_static_prop('Automattic_WooCommerce_Blocks_Utils_BlocksSharedState',
			'blocks_shared_cart_state').array_get(rt.new_string('errors'))
	} else {
		rt.new_array()
	}
	...
	for {
		...
		var_notices.array_push(rt.create_array([
			rt.ArrayItem{ key: 'id', val: rt.call_function('wp_unique_id', [
				rt.new_string('store-notice-'),
			]) },
			rt.ArrayItem{
				key: 'notice'
				val: if !(var_error.array_get(rt.new_string('message'))).is_null() {
...
fn create_automattic_woocommerce_blocks_utils_wc_cache_helper(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blocks_Utils_WC_Cache_Helper {
