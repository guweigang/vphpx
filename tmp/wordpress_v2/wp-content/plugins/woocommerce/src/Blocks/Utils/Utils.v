...
fn Class_Automattic_WooCommerce_Blocks_Utils_Utils.wp_version_compare(var_version rt.PhpVal, var_operator rt.PhpVal) rt.PhpVal {
	...
	mut var_current_wp_version := rt.call_function('get_bloginfo', [
		rt.new_string('version'),
	])
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^([0-9]+\\.[0-9]+)/'),
		var_current_wp_version.clone(), var_matches.clone()]))
	{
		var_current_wp_version = rt.new_float((var_matches.array_get(rt.new_int(1))).to_f64())
	}
	var_current_wp_version = rt.call_function('preg_replace', [
		rt.new_string('/[^0-9a-zA-Z\\.]+/i'),
		rt.new_string('.'),
		var_current_wp_version.clone(),
	])
	var_version_mutated = rt.call_function('preg_replace', [
		rt.new_string('/[^0-9a-zA-Z\\.]+/i'),
		rt.new_string('.'),
		var_version_mutated.clone(),
	])
	return rt.call_function('version_compare', [var_current_wp_version.clone(),
		var_version_mutated.clone(), var_operator.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Utils_Utils) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'wp_version_compare' {
			...
			return Class_Automattic_WooCommerce_Blocks_Utils_Utils.wp_version_compare(dispatch_arg_0,
				dispatch_arg_1)
		}
		else {
			return none
