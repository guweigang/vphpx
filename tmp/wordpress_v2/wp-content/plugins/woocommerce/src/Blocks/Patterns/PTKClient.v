...
fn (mut this Class_Automattic_WooCommerce_Blocks_Patterns_PTKClient) construct() {
	this.schema = rt.create_array([rt.ArrayItem{ key: 'type', val: 'array' },
		rt.ArrayItem{ key: 'items', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'properties', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'site_id', val: rt.create_array([
					rt.ArrayItem{ key: 'type', val: 'integer' },
					rt.ArrayItem{ key: 'required', val: true },
				]) },
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Patterns_PTKClient) fetch_patterns(mut var_options Class_Automattic_WooCommerce_Blocks_Patterns_array) rt.PhpVal {
	...
	mut var_lang := rt.call_function('preg_replace', [rt.new_string('/(_.*)$/'),
		rt.new_string(''), var_locale.clone()])
	mut var_ptk_url := rt.new_string(
		(Class_Automattic_WooCommerce_Blocks_Patterns_Automattic_WooCommerce_Blocks_Patterns_PTKClient.patterns_toolkit_url()).str() +
		var_lang.str())
	if var_options.array_isset(rt.new_string('site')) {
		var_ptk_url = rt.call_function('add_query_arg', [rt.new_string('site'),
			var_options.array_get(rt.new_string('site')), var_ptk_url.clone()])
	}
	if var_options.array_isset(rt.new_string('categories')) {
		var_ptk_url = rt.call_function('add_query_arg', [rt.new_string('categories'),
			rt.call_function('implode',
				[rt.new_string(','), var_options.array_get(rt.new_string('categories'))]),
			var_ptk_url.clone()])
	}
	if var_options.array_isset(rt.new_string('per_page')) {
		var_ptk_url = rt.call_function('add_query_arg', [rt.new_string('per_page'),
			var_options.array_get(rt.new_string('per_page')),
			var_ptk_url.clone()])
	}
	mut var_patterns := rt.call_function('wp_safe_remote_get', [
		var_ptk_url.clone()])
	if rt.is_true(rt.call_function('is_wp_error', [var_patterns.clone()]))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int(200), rt.call_function('wp_remote_retrieve_response_code', [var_patterns.clone()]))))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('patterns_toolkit_api_error'), rt.call_function('__', [
			rt.new_string('Failed to connect with the Patterns Toolkit API: try again later.'),
	...
	if !rt.is_true(var_body) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('patterns_toolkit_api_error'), rt.call_function('__', [
			rt.new_string('Empty response received from the Patterns Toolkit API.'),
			rt.new_string('woocommerce'),
		])))
	}
	mut var_decoded_body := rt.call_function('json_decode', [
		var_body.clone(), rt.new_bool(true)])
	mut var_is_pattern_payload_valid := rt.new_bool(this.is_valid_schema(var_decoded_body.clone()))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_is_pattern_payload_valid)))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('patterns_toolkit_api_error'), rt.call_function('__', [
			rt.new_string('Wrong response received from the Patterns Toolkit API: try again later.'),
			rt.new_string('woocommerce'),
		])))
	}
	return var_decoded_body.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blocks_Patterns_PTKClient) is_valid_schema(var_patterns rt.PhpVal) bool {
	...
	mut var_is_pattern_payload_valid := rt.call_function('rest_validate_value_from_schema', [
		var_patterns_mutated.clone(),
		this.schema,
	])
	return !(rt.is_true(rt.call_function('is_wp_error', [var_is_pattern_payload_valid.clone()])))
}
