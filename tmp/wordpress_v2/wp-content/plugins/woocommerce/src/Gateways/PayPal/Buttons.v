...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Buttons) get_options() rt.PhpVal {
	...
	return rt.call_function('array_merge', [var_common_options.clone(),
		var_options.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Buttons) get_client_id() string {
	...
	mut var_client_id := rt.call_function('get_option', [var_option_key.clone(),
		rt.new_null()])
	if !rt.is_true(var_client_id) {
		...
		rt.call_function('update_option', [var_option_key.clone(),
			var_client_id.clone()])
	}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_Buttons) get_current_page_for_app_switch() string {
	if rt.is_true(rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Proxies_LegacyProxy.class()]), 'call_function', [rt.new_string('is_checkout')]))
