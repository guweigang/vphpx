...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) do_onboarding() {
	...
	mut var_jetpack_connection_manager := rt.call_method(this.gateway,
		'get_jetpack_connection_manager', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(var_jetpack_connection_manager)))) {
		...
		mut iife_result_0 := iife_temp_0.log(rt.new_string('Jetpack connection manager not found.'),
			rt.new_string('error'))
		return
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_jetpack_connection_manager,
		'is_connected', []rt.PhpVal{})))))
	{
		mut var_result := rt.call_method(var_jetpack_connection_manager, 'try_registration',
			[]rt.PhpVal{})
		if rt.is_true(rt.call_function('is_wp_error', [var_result.clone()])) {
			mut iife_temp_1 := Class_Automattic_WooCommerce_Gateways_PayPal_WC_Gateway_Paypal{}
			mut iife_result_1 := iife_temp_1.log(rt.new_string('Jetpack registration failed: ' +
	...
	if !rt.is_true(var_merchant_account_data) {
		...
		if !rt.is_true(var_merchant_account) {
			...
			mut iife_result_2 := iife_temp_2.log(rt.new_string('Transact merchant onboarding failed.'),
				rt.new_string('error'))
			return
		}
		this.update_transact_account_cache(this.get_cache_key('merchant'),
			var_merchant_account.clone())
	}
	...
	if !rt.is_true(var_provider_account_data) {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(var_provider_account)))) {
			...
			mut iife_result_3 := iife_temp_3.log(rt.new_string('Transact provider onboarding failed.'),
				rt.new_string('error'))
			return
		}
		this.update_transact_account_cache(this.get_cache_key('provider'),
			var_provider_account.clone())
	}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) get_transact_account_data(account_type string) rt.PhpVal {
	...
	if !rt.is_true(var_transact_account) {
		var_transact_account = if rt.is_true(rt.identical(rt.new_string('merchant'),
			rt.new_string(account_type)))
		{
			this.fetch_merchant_account()
		} else {
			this.fetch_provider_account()
		}
		if !rt.is_true(var_transact_account) {
			return rt.new_null()
		}
		this.update_transact_account_cache(var_cache_key.str(), var_transact_account.clone())
	}
	return var_transact_account.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) get_cache_key(account_type string) string {
	if rt.is_true(rt.identical(rt.new_string('merchant'), rt.new_string(account_type))) {
		return (if rt.is_true(rt.get_property(this.gateway, 'testmode')) {
			Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager.transact_merchant_account_cache_key_test()
		} else {
			Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager.transact_merchant_account_cache_key_live()
		}).str()
	}
	if rt.is_true(rt.identical(rt.new_string('provider'), rt.new_string(account_type))) {
		return (if rt.is_true(rt.get_property(this.gateway, 'testmode')) {
			Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager.transact_provider_account_cache_key_test()
		} else {
			Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager.transact_provider_account_cache_key_live()
		}).str()
	}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) fetch_merchant_account() rt.PhpVal {
	mut iife_temp_4 := Class_Automattic_WooCommerce_Gateways_PayPal_Jetpack_Options{}
	mut iife_result_4 := iife_temp_4.get_option(rt.new_string('id'))
	mut var_site_id := iife_result_4
	if rt.is_true(rt.new_bool(!(rt.is_true(var_site_id)))) {
		return rt.new_null()
	}
	mut var_request_body := rt.create_array([
		rt.ArrayItem{ key: 'test_mode', val: rt.get_property(this.gateway, 'testmode') },
	])
	mut var_response := this.send_transact_api_request('GET', (rt.call_function('sprintf', [
		rt.new_string('/sites/%d/transact/account'),
		var_site_id.clone(),
	])).str(), mut
		rt.cast_object_ptr[Class_Automattic_WooCommerce_Gateways_PayPal_array](var_request_body))
	...
	mut var_response_data := rt.call_function('json_decode', [
		rt.call_function('wp_remote_retrieve_body', [var_response.clone()]),
		rt.new_bool(true),
	])
	if !rt.is_true(var_response_data.array_get(rt.new_string('public_id'))) {
		return rt.new_null()
	}
	return rt.create_array([
		rt.ArrayItem{ key: 'public_id', val: var_response_data.array_get(rt.new_string('public_id')) },
	])
}

fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) fetch_provider_account() bool {
	mut iife_temp_5 := Class_Automattic_WooCommerce_Gateways_PayPal_Jetpack_Options{}
	mut iife_result_5 := iife_temp_5.get_option(rt.new_string('id'))
	mut var_site_id := iife_result_5
	if rt.is_true(rt.new_bool(!(rt.is_true(var_site_id)))) {
		return false
	}
	mut var_request_body := rt.create_array([
		...
		rt.ArrayItem{
			key: 'provider_type'
			val: Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager.transact_provider_type()
		},
	])
	mut var_response := this.send_transact_api_request('GET', (rt.call_function('sprintf', [
		rt.new_string('/sites/%d/transact/account/%s'),
		var_site_id.clone(),
		Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager.transact_provider_type(),
	])).str(), mut
		rt.cast_object_ptr[Class_Automattic_WooCommerce_Gateways_PayPal_array](var_request_body))
	if rt.is_true(rt.call_function('is_wp_error', [var_response.clone()]))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int(200), rt.call_function('wp_remote_retrieve_response_code', [var_response.clone()]))))) {
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) create_merchant_account() rt.PhpVal {
	mut iife_temp_6 := Class_Automattic_WooCommerce_Gateways_PayPal_Jetpack_Options{}
	mut iife_result_6 := iife_temp_6.get_option(rt.new_string('id'))
	mut var_site_id := iife_result_6
	if rt.is_true(rt.new_bool(!(rt.is_true(var_site_id)))) {
		return rt.new_null()
	}
	mut var_request_body := rt.create_array([
	...
	mut var_response := this.send_transact_api_request('POST', (rt.call_function('sprintf', [
		rt.new_string('/sites/%d/transact/account'),
		var_site_id.clone(),
	])).str(), mut
		rt.cast_object_ptr[Class_Automattic_WooCommerce_Gateways_PayPal_array](var_request_body))
	...
	if !rt.is_true(var_response_data.array_get(rt.new_string('public_id'))) {
		...
		mut iife_result_7 := iife_temp_7.log(rt.new_string(
			'Transact merchant account creation failed. Response body: ' +
			(rt.call_function('wc_print_r', [var_response_data.clone(), rt.new_bool(true)])).str()))
		return rt.new_null()
	}
	return rt.create_array([
		rt.ArrayItem{ key: 'public_id', val: var_response_data.array_get(rt.new_string('public_id')) },
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) create_provider_account() bool {
	mut iife_temp_8 := Class_Automattic_WooCommerce_Gateways_PayPal_Jetpack_Options{}
	mut iife_result_8 := iife_temp_8.get_option(rt.new_string('id'))
	mut var_site_id := iife_result_8
	if rt.is_true(rt.new_bool(!(rt.is_true(var_site_id)))) {
		return false
	}
	mut var_request_body := rt.create_array([
	...
	mut var_response := this.send_transact_api_request('POST', (rt.call_function('sprintf', [
		rt.new_string('/sites/%d/transact/account/%s/onboard'),
		var_site_id.clone(),
		Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager.transact_provider_type(),
	])).str(), mut
		rt.cast_object_ptr[Class_Automattic_WooCommerce_Gateways_PayPal_array](var_request_body))
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) update_transact_account_cache(cache_key string, var_account_data rt.PhpVal) {
	...
	mut var_expires := rt.add(rt.call_function('time', []rt.PhpVal{}),
		Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager.transact_account_cache_expiry())
	rt.call_function('update_option', [rt.new_string(cache_key_mutated).clone(),
		rt.create_array([rt.ArrayItem{ key: 'account', val: var_account_data },
			rt.ArrayItem{ key: 'expiry', val: var_expires }])])
}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) get_transact_account_from_cache(cache_key string) rt.PhpVal {
	mut cache_key_mutated := cache_key
	mut var_transact_account := rt.call_function('get_option', [
		rt.new_string(cache_key_mutated).clone(), rt.new_null()])
	if !rt.is_true(var_transact_account)
		|| (var_transact_account.array_isset(rt.new_string('expiry'))
		&& rt.is_true(rt.less(var_transact_account.array_get(rt.new_string('expiry')), rt.call_function('time', []rt.PhpVal{})))) {
		return rt.new_null()
	}
	return if !(var_transact_account.array_get(rt.new_string('account'))).is_null() {
		var_transact_account.array_get(rt.new_string('account'))
	} else {
		rt.new_null()
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) send_transact_api_request(method string, endpoint string, mut var_request_body Class_Automattic_WooCommerce_Gateways_PayPal_array) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_string('GET'), rt.new_string(method))) {
		endpoint = endpoint + '?' +
			(rt.call_function('http_build_query', [var_request_body_mutated])).str()
	}
	mut iife_temp_9 := Class_Automattic_Jetpack_Connection_Client{}
	mut iife_result_9 := iife_temp_9.wpcom_json_api_request_as_blog(rt.new_string(endpoint),
		rt.new_string((Class_Automattic_WooCommerce_Gateways_PayPal_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager.wpcom_proxy_endpoint_api_version()).str()), rt.create_array([
		rt.ArrayItem{ key: 'headers', val: rt.create_array([
			rt.ArrayItem{ key: 'Content-Type', val: 'application/json' },
		]) },
		rt.ArrayItem{ key: 'method', val: method },
		rt.ArrayItem{
			key: 'timeout'
			val: Class_Automattic_WooCommerce_Gateways_PayPal_Constants.wpcom_proxy_request_timeout()
		},
	]), if rt.is_true(rt.identical(rt.new_string('GET'), rt.new_string(method))) { rt.new_null() } else { rt.call_function('wp_json_encode', [
			var_request_body_mutated,
		]) }, rt.new_string('wpcom'))
	mut var_response := iife_result_9
...
fn (mut this Class_Automattic_WooCommerce_Gateways_PayPal_TransactAccountManager) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'send_transact_api_request' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Gateways_PayPal_array](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
			return this.send_transact_api_request(dispatch_arg_0, dispatch_arg_1, mut
				dispatch_arg_2)
		}
		else {
			return none
