...
fn init_static_automattic_woocommerce_layouttemplates_layouttemplateregistry() {
	rt.init_static_prop('Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry',
		'instance', rt.new_null())
}

fn Class_Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry.get_instance() rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_null(), rt.get_static_prop('Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry',
		'instance')))
	{
		rt.set_static_prop('Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry',
			'instance', rt.new_object('Automattic_WooCommerce_LayoutTemplates_self', []string{},
			create_automattic_woocommerce_layouttemplates_self()))
	}
	return rt.get_static_prop('Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry',
		'instance')
}
...
fn (mut this Class_Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry) register(var_layout_template_id rt.PhpVal, var_layout_template_area rt.PhpVal, var_layout_template_class_name rt.PhpVal) {
	mut var_layout_template_id_mutated := var_layout_template_id
	if this.is_registered(var_layout_template_id_mutated.clone()) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_LayoutTemplates_ValueError',
			[]string{},
			create_automattic_woocommerce_layouttemplates_valueerror(rt.new_string('A layout template with the specified ID already exists in the registry.'))))
	}
	if !rt.is_true(var_layout_template_area) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_LayoutTemplates_ValueError',
			[]string{},
			create_automattic_woocommerce_layouttemplates_valueerror(rt.new_string('The specified layout template area is empty.'))))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		var_layout_template_class_name.clone()])))))
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_LayoutTemplates_ValueError',
			[]string{},
			create_automattic_woocommerce_layouttemplates_valueerror(rt.new_string('The specified layout template class does not exist.'))))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_subclass_of', [
		var_layout_template_class_name.clone(),
		Class_Automattic_WooCommerce_Admin_BlockTemplates_BlockTemplateInterface.class(),
	])))))
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_LayoutTemplates_ValueError',
			[]string{},
			create_automattic_woocommerce_layouttemplates_valueerror(rt.new_string('The specified layout template class does not implement the BlockTemplateInterface.'))))
	}
	this.layout_templates_info.array_set(var_layout_template_id_mutated, rt.create_array([
		rt.ArrayItem{ key: 'id', val: var_layout_template_id_mutated },
		rt.ArrayItem{ key: 'area', val: var_layout_template_area },
		rt.ArrayItem{ key: 'class_name', val: var_layout_template_class_name },
	]))
}
...
fn (mut this Class_Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry) instantiate_layout_templates(mut var_query_params Class_Automattic_WooCommerce_LayoutTemplates_array) rt.PhpVal {
	mut iife_temp_0 :=
		Class_Automattic_WooCommerce_Internal_Admin_BlockTemplates_BlockTemplateLogger{}
	...
	for {
		...
		mut var_layout_template :=
			this.get_layout_template_instance(var_layout_template_info.clone())
		mut var_layout_template_id := rt.call_method(var_layout_template, 'get_id', []rt.PhpVal{})
		var_layout_templates.array_set(var_layout_template_id, var_layout_template.clone())
		rt.call_method(var_logger, 'log_template_events_to_file', [
			var_layout_template_id.clone()])
	}
	return var_layout_templates.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry) get_layout_template_instance(var_layout_template_info rt.PhpVal) rt.PhpVal {
	mut var_class_name := var_layout_template_info.array_get(rt.new_string('class_name'))
	mut var_layout_template_instance := if this.layout_template_instances.array_isset(var_class_name) {
		this.layout_template_instances.array_get(var_class_name)
	} else {
		rt.new_null()
	}
	if !(!rt.is_true(var_layout_template_instance)) {
		return var_layout_template_instance.clone()
	}
	var_layout_template_instance = rt.create_object_dynamically(var_class_name, []rt.PhpVal{})
	this.layout_template_instances.array_set(var_class_name, var_layout_template_instance.clone())
	rt.call_function('do_action', [
		rt.new_string('woocommerce_layout_template_after_instantiation'),
		var_layout_template_info.array_get(rt.new_string('id')),
		var_layout_template_info.array_get(rt.new_string('area')),
		var_layout_template_instance.clone(),
	])
	rt.call_function('wc_do_deprecated_action', [
		rt.new_string('woocommerce_block_template_register'),
		rt.create_array([rt.ArrayItem{ key: none, val: var_layout_template_instance }]),
		rt.new_string('8.6.0'),
		rt.new_string('woocommerce_layout_template_after_instantiation'),
	])
	return var_layout_template_instance.clone()
...
fn (mut this Class_Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry) get_matching_layout_templates_info(mut var_query_params Class_Automattic_WooCommerce_LayoutTemplates_array) rt.PhpVal {
	mut var_area_to_match := if var_query_params.array_isset(rt.new_string('area')) {
		var_query_params.array_get(rt.new_string('area'))
	} else {
		rt.new_null()
	}
	mut var_id_to_match := if var_query_params.array_isset(rt.new_string('id')) {
		var_query_params.array_get(rt.new_string('id'))
	} else {
		rt.new_null()
	}
	mut var_matching_layout_templates_info := rt.new_array()
	mut iter_2 := this.layout_templates_info.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_layout_template_info := item_2.val
		if !(!rt.is_true(var_area_to_match))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_layout_template_info.array_get(rt.new_string('area')), var_area_to_match)))) {
			continue
		}
		if !(!rt.is_true(var_id_to_match))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_layout_template_info.array_get(rt.new_string('id')), var_id_to_match)))) {
			continue
		}
		var_matching_layout_templates_info.array_push(var_layout_template_info.clone())
	}
	return var_matching_layout_templates_info.clone()
}

struct Class_Automattic_WooCommerce_LayoutTemplates_self {
	rt.PhpObjectBase
}
...
fn create_automattic_woocommerce_layouttemplates_layouttemplateregistry(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry {
...
fn create_automattic_woocommerce_layouttemplates_self(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_LayoutTemplates_self {
	mut obj := &Class_Automattic_WooCommerce_LayoutTemplates_self{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
}

fn create_automattic_woocommerce_layouttemplates_valueerror(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_LayoutTemplates_ValueError {
...
fn create_automattic_woocommerce_internal_admin_blocktemplates_blocktemplatelogger(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_BlockTemplates_BlockTemplateLogger {
...
fn (mut this Class_Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'instantiate_layout_templates' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_LayoutTemplates_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
...
fn (mut this Class_Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'layout_templates_info' {
			this.layout_templates_info = val
			return true
		}
		'layout_template_instances' {
			this.layout_template_instances = val
			return true
		}
		else {
...
fn (mut this Class_Automattic_WooCommerce_LayoutTemplates_self) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	return none
}

fn (this &Class_Automattic_WooCommerce_LayoutTemplates_self) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_LayoutTemplates_self) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	return this.PhpObjectBase.dispatch_set_prop(prop_name, val)
}
...
fn init_registry() {
	rt.register_class_factory('Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry', fn (args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_object('Automattic_WooCommerce_LayoutTemplates_LayoutTemplateRegistry',
			[]string{}, obj)
	})
	rt.register_class_factory('Automattic_WooCommerce_LayoutTemplates_self', fn (args []rt.PhpVal) rt.PhpVal {
		obj := create_automattic_woocommerce_layouttemplates_self()
		return rt.new_object('Automattic_WooCommerce_LayoutTemplates_self', []string{}, obj)
	})
	...
	rt.register_class_factory('Automattic_WooCommerce_Internal_Admin_BlockTemplates_BlockTemplateLogger', fn (args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_object('Automattic_WooCommerce_Internal_Admin_BlockTemplates_BlockTemplateLogger',
			[]string{}, obj)
	})
}
