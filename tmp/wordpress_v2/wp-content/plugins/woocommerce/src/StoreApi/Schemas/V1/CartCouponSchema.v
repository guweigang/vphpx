	...
	return rt.create_array([
		rt.ArrayItem{ key: 'code', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string("The coupon's unique code."),
			...
			rt.ArrayItem{ key: 'arg_options', val: rt.create_array([
				rt.ArrayItem{ key: 'sanitize_callback', val: 'wc_format_coupon_code' },
				rt.ArrayItem{ key: 'validate_callback', val: rt.create_array([
					...
					rt.ArrayItem{ key: none, val: 'coupon_exists' },
				]) },
			]) },
		]) },
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartCouponSchema) coupon_exists(var_coupon_code rt.PhpVal) bool {
	mut var_coupon :=
		create_automattic_woocommerce_storeapi_schemas_v1_wc_coupon(var_coupon_code.clone())
	return rt.is_true((var_coupon.get_id()).to_bool()) || rt.is_true(var_coupon.get_virtual())
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartCouponSchema) get_item_response(var_coupon_code rt.PhpVal) rt.PhpVal {
	...
	mut var_coupon :=
		create_automattic_woocommerce_storeapi_schemas_v1_wc_coupon(var_coupon_code.clone())
	return rt.create_array([rt.ArrayItem{ key: 'code', val: var_coupon.get_code() },
		...
		rt.ArrayItem{ key: 'totals', val: rt.array_to_object(this.prepare_currency_response(rt.create_array([
			rt.ArrayItem{ key: 'total_discount', val: this.prepare_money_response(rt.call_method(var_cart,
				'get_coupon_discount_amount', [var_coupon_code.clone()]), rt.call_function('wc_get_price_decimals',
				[]rt.PhpVal{})) },
			rt.ArrayItem{ key: 'total_discount_tax', val: this.prepare_money_response(rt.call_method(var_cart,
				'get_coupon_discount_tax_amount', [var_coupon_code.clone()]), rt.call_function('wc_get_price_decimals',
...
fn create_automattic_woocommerce_storeapi_schemas_v1_cartcouponschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartCouponSchema {
