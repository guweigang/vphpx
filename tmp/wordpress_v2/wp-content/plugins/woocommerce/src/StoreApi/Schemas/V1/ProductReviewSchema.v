...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductReviewSchema) construct(mut var_extend Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema, mut var_controller Class_Automattic_WooCommerce_StoreApi_SchemaController) {
	...
	this.image_attachment_schema = rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Schemas_V1_ProductReviewSchema', [
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductReviewSchema) get_properties() rt.PhpVal {
	mut var_properties := rt.create_array([
		...
		rt.ArrayItem{ key: 'formatted_date_created', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string("The date the review was created, in the site's timezone in human-readable format."),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{ key: 'context', val: rt.create_array([
		...
		rt.ArrayItem{ key: 'product_name', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Name of the product that the review belongs to.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{ key: 'context', val: rt.create_array([
		...
		rt.ArrayItem{ key: 'product_permalink', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Permalink of the product that the review belongs to.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{ key: 'context', val: rt.create_array([
		...
		rt.ArrayItem{ key: 'product_image', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Image of the product that the review belongs to.'),
				rt.new_string('woocommerce'),
			]) },
		...
		rt.ArrayItem{ key: 'review', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'arg_options', val: rt.create_array([
				rt.ArrayItem{ key: 'sanitize_callback', val: 'wp_filter_post_kses' },
			]) },
			rt.ArrayItem{ key: 'readonly', val: true },
		]) },
		...
		rt.ArrayItem{ key: 'verified', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
	...
	if rt.is_true(rt.call_function('get_option', [rt.new_string('show_avatars')])) {
		...
		var_properties.array_set('reviewer_avatar_urls', rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductReviewSchema) get_item_response(var_review rt.PhpVal) rt.PhpVal {
	mut var_rating := if rt.is_true(rt.identical(rt.call_function('get_comment_meta', [
		rt.get_property(var_review, 'comment_ID'),
		rt.new_string('rating'),
		rt.new_bool(true),
	]), rt.new_string('')))
	{ rt.new_null() } else { rt.new_int((rt.call_function('get_comment_meta', [
			rt.get_property(var_review, 'comment_ID'),
			rt.new_string('rating'),
			rt.new_bool(true),
		])).to_i64()) }
	return rt.create_array([
		...
		rt.ArrayItem{ key: 'formatted_date_created', val: rt.call_function('get_comment_date', [
			rt.new_string('F j, Y'),
			rt.get_property(var_review, 'comment_ID'),
		]) },
		...
		rt.ArrayItem{ key: 'product_id', val: rt.new_int((rt.get_property(var_review,
			'comment_post_ID')).to_i64()) },
		rt.ArrayItem{ key: 'product_name', val: rt.call_function('get_the_title', [
			rt.new_int((rt.get_property(var_review, 'comment_post_ID')).to_i64()),
		]) },
		rt.ArrayItem{ key: 'product_permalink', val: rt.call_function('get_permalink', [
			rt.new_int((rt.get_property(var_review, 'comment_post_ID')).to_i64()),
		]) },
		rt.ArrayItem{ key: 'product_image', val: rt.call_method(this.image_attachment_schema,
			'get_item_response', [
			rt.call_function('get_post_thumbnail_id', [
				rt.new_int((rt.get_property(var_review, 'comment_post_ID')).to_i64()),
			]),
		]) },
		rt.ArrayItem{ key: 'reviewer', val: rt.get_property(var_review, 'comment_author') },
		rt.ArrayItem{ key: 'review', val: rt.call_function('wpautop', [
			rt.get_property(var_review, 'comment_content'),
		]) },
		rt.ArrayItem{ key: 'rating', val: var_rating },
		rt.ArrayItem{ key: 'verified', val: rt.call_function('wc_review_is_from_verified_owner', [
			rt.get_property(var_review, 'comment_ID'),
		]) },
		rt.ArrayItem{ key: 'reviewer_avatar_urls', val: rt.call_function('rest_get_avatar_urls', [
			rt.get_property(var_review, 'comment_author_email'),
		]) },
	])
}
