...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ShippingAddressSchema) get_item_response(var_address rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.instance_of(var_address, 'Automattic_WooCommerce_StoreApi_Schemas_V1_WC_Customer')))
		|| rt.is_true(rt.new_bool(rt.instance_of(var_address, 'Automattic_WooCommerce_StoreApi_Schemas_V1_WC_Order'))) {
		mut var_shipping_country := rt.call_method(var_address, 'get_shipping_country',
			[]rt.PhpVal{})
		mut var_shipping_state := rt.call_method(var_address, 'get_shipping_state', []rt.PhpVal{})
		if rt.is_true(rt.new_bool(!(rt.is_true(var_validation_util.validate_state(var_shipping_state.clone(),
			var_shipping_country.clone())))))
		{
			var_shipping_state = rt.new_string('')
		}
		...
		mut var_address_object := rt.call_function('array_merge', [
			rt.create_array([
				rt.ArrayItem{ key: 'first_name', val: rt.call_method(var_address,
					'get_shipping_first_name', []rt.PhpVal{}) },
				rt.ArrayItem{ key: 'last_name', val: rt.call_method(var_address,
					'get_shipping_last_name', []rt.PhpVal{}) },
				rt.ArrayItem{ key: 'company', val: rt.call_method(var_address,
					'get_shipping_company', []rt.PhpVal{}) },
				rt.ArrayItem{ key: 'address_1', val: rt.call_method(var_address,
					'get_shipping_address_1', []rt.PhpVal{}) },
				rt.ArrayItem{ key: 'address_2', val: rt.call_method(var_address,
					'get_shipping_address_2', []rt.PhpVal{}) },
				rt.ArrayItem{ key: 'city', val: rt.call_method(var_address, 'get_shipping_city',
					[]rt.PhpVal{}) },
				rt.ArrayItem{ key: 'state', val: var_shipping_state },
				rt.ArrayItem{ key: 'postcode', val: rt.call_method(var_address,
					'get_shipping_postcode', []rt.PhpVal{}) },
				rt.ArrayItem{ key: 'country', val: var_shipping_country },
				rt.ArrayItem{ key: 'phone', val: rt.call_method(var_address, 'get_shipping_phone',
					[]rt.PhpVal{}) },
			]),
			var_additional_address_fields.clone(),
		])
		mut iter_1 := rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Schemas_V1_ShippingAddressSchema', [
			'Automattic_WooCommerce_StoreApi_Schemas_V1_AbstractAddressSchema',
	...
	rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Exceptions_RouteException',
		...
		rt.new_string('ShippingAddressSchema::get_item_response'),
...
fn create_automattic_woocommerce_storeapi_schemas_v1_shippingaddressschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ShippingAddressSchema {
