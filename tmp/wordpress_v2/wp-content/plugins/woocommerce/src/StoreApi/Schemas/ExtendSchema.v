...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema) construct(mut var_formatters Class_Automattic_WooCommerce_StoreApi_Formatters) {
	this.formatters = var_formatters
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema) register_endpoint_data(var_args rt.PhpVal) {
	...
	if !(var_args_mutated.array_get(rt.new_string('namespace')).is_string())
		|| !rt.is_true(var_args_mutated.array_get(rt.new_string('namespace'))) {
		this.throw_exception(rt.new_string('You must provide a plugin namespace when extending a Store REST endpoint.'))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_args_mutated.array_get(rt.new_string('endpoint')),
		this.endpoints,
		rt.new_bool(true),
	])))))
	{
	...
	if !(var_args_mutated.array_get(rt.new_string('schema_callback')).is_null())
		&& !(rt.call_function('is_callable', [var_args_mutated.array_get(rt.new_string('schema_callback'))])) {
		this.throw_exception(rt.new_string('$schema_callback must be a callable function.'))
	}
	if !(var_args_mutated.array_get(rt.new_string('data_callback')).is_null())
		&& !(rt.call_function('is_callable', [var_args_mutated.array_get(rt.new_string('data_callback'))])) {
		this.throw_exception(rt.new_string('$data_callback must be a callable function.'))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_args_mutated.array_get(rt.new_string('schema_type')),
	...
	this.extend_data.array_get_mut(var_args_mutated.array_get(rt.new_string('endpoint'))).array_set(var_args_mutated.array_get(rt.new_string('namespace')), rt.create_array([
		rt.ArrayItem{
			key: 'schema_callback'
			val: var_args_mutated.array_get(rt.new_string('schema_callback'))
		},
		rt.ArrayItem{
			key: 'data_callback'
			val: var_args_mutated.array_get(rt.new_string('data_callback'))
		},
		rt.ArrayItem{
			key: 'schema_type'
			val: var_args_mutated.array_get(rt.new_string('schema_type'))
		},
	]))
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema) register_update_callback(var_args rt.PhpVal) {
	...
	if !(rt.call_function('is_callable', [var_args_mutated.array_get(rt.new_string('callback'))])) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Schemas_Exception',
			[]string{},
	...
	this.callback_methods.array_set(var_args_mutated.array_get(rt.new_string('namespace')),
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema) register_payment_requirements(var_args rt.PhpVal) {
	mut var_args_mutated := var_args
	if !rt.is_true(var_args_mutated.array_get(rt.new_string('data_callback')))
		|| !(rt.call_function('is_callable', [var_args_mutated.array_get(rt.new_string('data_callback'))])) {
		this.throw_exception(rt.new_string('$data_callback must be a callable function.'))
	}
	this.payment_requirements.array_push(var_args_mutated.array_get(rt.new_string('data_callback')))
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema) get_update_callback(var_namespace rt.PhpVal) rt.PhpVal {
	if !(var_namespace.clone().is_string()) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Schemas_Exception',
			[]string{},
			create_automattic_woocommerce_storeapi_schemas_exception(rt.new_string('You must provide a plugin namespace when extending a Store REST endpoint.'))))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.callback_methods.array_isset(var_namespace.clone())))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Schemas_Exception',
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.callback_methods.array_get(var_namespace).array_isset(rt.new_string('callback')))))))
		|| !(rt.call_function('is_callable', [this.callback_methods.array_get(var_namespace).array_get(rt.new_string('callback'))])) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Schemas_Exception',
			[]string{}, create_automattic_woocommerce_storeapi_schemas_exception(rt.call_function('sprintf', [
	...
	return this.callback_methods.array_get(var_namespace).array_get(rt.new_string('callback'))
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema) get_endpoint_data(var_endpoint rt.PhpVal, mut var_passed_args Class_Automattic_WooCommerce_StoreApi_Schemas_array) rt.PhpVal {
	...
	if this.extend_data.array_isset(var_endpoint) {
		...
		for {
			...
			if rt.is_true(rt.new_bool(var_callbacks.array_get(rt.new_string('data_callback')).is_null())) {
				continue
			}
			mut var_data := rt.call_callable(var_callbacks.array_get(rt.new_string('data_callback')), [
				var_passed_args,
			])
			if rt.has_exception() {
				unsafe {
			...
			if rt.instance_of(var_e_1, 'Automattic_WooCommerce_StoreApi_Schemas_Throwable') {
				...
				this.throw_exception(var_e.clone())
				unsafe {
					goto end_label_1
				}
			} else {
			...
			end_label_1:
			var_registered_data.array_set(var_namespace, var_data.clone())
		}
	}
	return mut rt.array_to_object(var_registered_data)
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema) get_endpoint_schema(var_endpoint rt.PhpVal, mut var_passed_args Class_Automattic_WooCommerce_StoreApi_Schemas_array) rt.PhpVal {
	...
	if this.extend_data.array_isset(var_endpoint) {
		mut iter_2 := this.extend_data.array_get(var_endpoint).iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_callbacks := item_2.val
			mut var_namespace := item_2.key
			if rt.is_true(rt.new_bool(var_callbacks.array_get(rt.new_string('schema_callback')).is_null())) {
				continue
			}
			mut var_schema := rt.call_callable(var_callbacks.array_get(rt.new_string('schema_callback')), [
				var_passed_args,
			])
			if rt.has_exception() {
				unsafe {
			...
			end_label_2:
			var_registered_schema.array_set(var_namespace, this.format_extensions_properties(var_namespace.clone(),
				var_schema.clone(), var_callbacks.array_get(rt.new_string('schema_type'))))
		}
	}
	return mut rt.array_to_object(var_registered_schema)
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema) get_payment_requirements(mut var_requirements Class_Automattic_WooCommerce_StoreApi_Schemas_array) rt.PhpVal {
	...
	if !(!rt.is_true(this.payment_requirements)) {
		mut iter_3 := this.payment_requirements.iterator()
		for {
			item_3 := iter_3.next() or { break }
			mut var_callback := item_3.val
			...
			var_requirements_mutated = rt.call_function('array_unique', [
				rt.call_function('array_merge', [var_requirements_mutated, var_data.clone()]),
			])
			if rt.has_exception() {
				unsafe {
					goto catch_label_3
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema) throw_exception(var_exception_or_error rt.PhpVal) {
	mut var_exception := if var_exception_or_error.clone().is_string() {
		create_automattic_woocommerce_storeapi_schemas_exception(var_exception_or_error.clone())
	} else {
		var_exception_or_error
	}
	rt.call_function('wc_caught_exception', [var_exception.clone()])
...
fn create_automattic_woocommerce_storeapi_schemas_exception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Schemas_Exception {
