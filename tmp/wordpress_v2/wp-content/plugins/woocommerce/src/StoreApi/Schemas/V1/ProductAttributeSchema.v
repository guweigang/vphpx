...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductAttributeSchema) get_properties() rt.PhpVal {
	return rt.create_array([
		...
		rt.ArrayItem{ key: 'taxonomy', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('The attribute taxonomy name.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{ key: 'context', val: rt.create_array([
		...
		rt.ArrayItem{ key: 'order', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('How terms in this attribute are sorted by default.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{ key: 'context', val: rt.create_array([
		...
		rt.ArrayItem{ key: 'has_archives', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('If this attribute has term archive pages.'),
				rt.new_string('woocommerce'),
			]) },
		...
		rt.ArrayItem{ key: 'count', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Number of terms in the attribute taxonomy.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'integer' },
			rt.ArrayItem{ key: 'context', val: rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductAttributeSchema) get_item_response(var_attribute rt.PhpVal) rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.new_int((rt.get_property(var_attribute, 'id')).to_i64()) },
		rt.ArrayItem{ key: 'name', val: this.prepare_html_response(rt.get_property(var_attribute,
			'name')) },
		rt.ArrayItem{ key: 'taxonomy', val: rt.get_property(var_attribute, 'slug') },
		rt.ArrayItem{ key: 'type', val: rt.get_property(var_attribute, 'type') },
		rt.ArrayItem{ key: 'order', val: rt.get_property(var_attribute, 'order_by') },
		rt.ArrayItem{ key: 'has_archives', val: rt.get_property(var_attribute, 'has_archives') },
		rt.ArrayItem{ key: 'count', val: rt.new_int((rt.call_function('wp_count_terms', [
			rt.get_property(var_attribute, 'slug'),
		])).to_i64()) },
	])
}
...
fn create_automattic_woocommerce_storeapi_schemas_v1_productattributeschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductAttributeSchema {
