...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductCategorySchema) construct(mut var_extend Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema, mut var_controller Class_Automattic_WooCommerce_StoreApi_SchemaController) {
	...
	this.image_attachment_schema = rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Schemas_V1_ProductCategorySchema', [
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductCategorySchema) get_properties() rt.PhpVal {
	...
	var_schema.array_set('review_count', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Number of reviews for products in this category.'),
			rt.new_string('woocommerce'),
		]) },
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductCategorySchema) get_item_response(var_term rt.PhpVal) rt.PhpVal {
	...
	mut var_count := rt.call_function('get_term_meta', [
		...
		rt.new_string('product_count_product_cat'),
		rt.new_bool(true),
	])
	...
	var_response.array_set('review_count', this.get_category_review_count(var_term.clone()))
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductCategorySchema) get_category_review_count(var_term rt.PhpVal) i64 {
	...
	mut var_products_of_category_sql := rt.new_string((
	...
	mut var_review_count := rt.call_method(var_wpdb, 'get_var', [
		var_products_of_category_sql.clone()])
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_ProductCategorySchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_category_review_count' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			return rt.new_int(this.get_category_review_count(dispatch_arg_0))
		}
		else {
			return none
		}
