	...
	return rt.create_array([
		rt.ArrayItem{ key: 'code', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('The coupons unique code.'),
		...
		rt.ArrayItem{ key: 'discount_type', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('The discount type for the coupon (e.g. percentage or fixed amount)'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{ key: 'context', val: rt.create_array([
		...
		rt.ArrayItem{ key: 'totals', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Total amounts provided using the smallest unit of the currency.'),
				rt.new_string('woocommerce'),
			]) },
			...
			rt.ArrayItem{ key: 'properties', val: rt.call_function('array_merge', [
				...
				rt.create_array([
					rt.ArrayItem{ key: 'total_discount', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('Total discount applied by this coupon.'),
							rt.new_string('woocommerce'),
						]) },
						rt.ArrayItem{ key: 'type', val: 'string' },
						rt.ArrayItem{ key: 'context', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'total_discount_tax', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('Total tax removed due to discount applied by this coupon.'),
							rt.new_string('woocommerce'),
						]) },
						rt.ArrayItem{ key: 'type', val: 'string' },
						rt.ArrayItem{ key: 'context', val: rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_OrderCouponSchema) get_item_response(var_coupon rt.PhpVal) rt.PhpVal {
	mut var_coupon_object := create_automattic_woocommerce_storeapi_schemas_v1_wc_coupon(rt.call_method(var_coupon,
		'get_code', []rt.PhpVal{}))
	return rt.create_array([
		rt.ArrayItem{ key: 'code', val: rt.call_method(var_coupon, 'get_code', []rt.PhpVal{}) },
		rt.ArrayItem{
			key: 'discount_type'
			val: if rt.is_true(var_coupon_object) {
				var_coupon_object.get_discount_type()
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{ key: 'totals', val: rt.array_to_object(this.prepare_currency_response(rt.create_array([
			rt.ArrayItem{ key: 'total_discount', val: this.prepare_money_response(rt.call_method(var_coupon,
				'get_discount', []rt.PhpVal{}), rt.call_function('wc_get_price_decimals',
				[]rt.PhpVal{})) },
			rt.ArrayItem{ key: 'total_discount_tax', val: this.prepare_money_response(rt.call_method(var_coupon,
				'get_discount_tax', []rt.PhpVal{}), rt.call_function('wc_get_price_decimals',
...
fn create_automattic_woocommerce_storeapi_schemas_v1_ordercouponschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Schemas_V1_OrderCouponSchema {
...
fn create_automattic_woocommerce_storeapi_schemas_v1_wc_coupon(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Schemas_V1_WC_Coupon {
