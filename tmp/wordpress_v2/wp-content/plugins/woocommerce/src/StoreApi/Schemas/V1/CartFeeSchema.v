...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartFeeSchema) get_item_response(var_fee rt.PhpVal) rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'key', val: rt.get_property(var_fee, 'id') },
		rt.ArrayItem{ key: 'name', val: this.prepare_html_response(rt.get_property(var_fee, 'name')) },
		rt.ArrayItem{ key: 'totals', val: rt.array_to_object(this.prepare_currency_response(rt.create_array([
			rt.ArrayItem{ key: 'total', val: this.prepare_money_response(rt.get_property(var_fee,
				'total'), rt.call_function('wc_get_price_decimals', []rt.PhpVal{})) },
			rt.ArrayItem{ key: 'total_tax', val: this.prepare_money_response(rt.get_property(var_fee,
				'tax'), rt.call_function('wc_get_price_decimals', []rt.PhpVal{}),
				rt.get_constant('PHP_ROUND_HALF_DOWN')) },
		]))) },
	])
}
...
fn create_automattic_woocommerce_storeapi_schemas_v1_cartfeeschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Schemas_V1_CartFeeSchema {
