...
fn woocommerce_store_api_register_endpoint_data(var_args rt.PhpVal) bool {
	...
	var_extend = rt.call_method(iife_result_0, 'get', [
	...
	rt.call_method(var_extend, 'register_endpoint_data', [var_args.clone()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
...
fn woocommerce_store_api_register_update_callback(var_args rt.PhpVal) bool {
	...
	var_extend = rt.call_method(iife_result_1, 'get', [
		Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema.class(),
	])
	if rt.has_exception() {
		unsafe {
	...
	rt.call_method(var_extend, 'register_update_callback', [var_args.clone()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_2
		}
...
fn woocommerce_store_api_register_payment_requirements(var_args rt.PhpVal) bool {
	mut var_extend := rt.new_null()
	mut var_error := rt.new_null()
	mut iife_temp_2 := Class_Automattic_WooCommerce_StoreApi_StoreApi{}
	mut iife_result_2 := iife_temp_2.container()
	var_extend = rt.call_method(iife_result_2, 'get', [
		Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema.class(),
	])
	if rt.has_exception() {
		unsafe {
...
fn woocommerce_store_api_get_formatter(var_name rt.PhpVal) rt.PhpVal {
	mut iife_temp_3 := Class_Automattic_WooCommerce_StoreApi_StoreApi{}
	mut iife_result_3 := iife_temp_3.container()
	return rt.call_method(rt.call_method(iife_result_3, 'get', [
		Class_Automattic_WooCommerce_StoreApi_Schemas_ExtendSchema.class(),
	]), 'get_formatter', [var_name.clone()])
}
