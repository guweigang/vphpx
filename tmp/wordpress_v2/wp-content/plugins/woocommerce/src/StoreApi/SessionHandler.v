...
fn (mut this Class_Automattic_WooCommerce_StoreApi_SessionHandler) construct() {
	...
	this.token = rt.call_function('wc_clean', [
	...
	this.table = (rt.get_property(var_GLOBALS.array_get(rt.new_string('wpdb')), 'prefix')).str() +
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_SessionHandler) init() {
	...
	rt.call_function('add_action', [rt.new_string('shutdown'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_StoreApi_SessionHandler', [
				'WC_Session',
			], &this) },
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_SessionHandler) init_session_from_token() {
	...
	mut iife_result_0 := iife_temp_0.get_cart_token_payload(this.token)
	mut var_payload := iife_result_0
	this.dispatch_set_prop('_customer_id', var_payload.array_get(rt.new_string('user_id')))
	this.session_expiration = var_payload.array_get(rt.new_string('exp'))
	this.dispatch_set_prop('_data', rt.cast_array(this.get_session(this.get_customer_id(),
		(rt.new_array()).to_bool())))
}

fn (mut this Class_Automattic_WooCommerce_StoreApi_SessionHandler) has_session() bool {
	return !(!rt.is_true(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_SessionHandler', [
		'WC_Session',
	], &this), '_customer_id')))
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_SessionHandler) get_customer_unique_id() string {
	if this.has_session() && rt.is_true(this.get_customer_id()) {
		return (this.get_customer_id()).str()
	}
	return if rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})) {
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_SessionHandler) get_session(var_customer_id rt.PhpVal, default_value bool) rt.PhpVal {
	...
	mut iife_result_1 := iife_temp_1.is_defined(rt.new_string('WP_SETUP_CONFIG'))
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_SessionHandler) delete_session(var_customer_id rt.PhpVal) {
	...
	rt.call_method(var_GLOBALS.array_get(rt.new_string('wpdb')), 'delete', [this.table,
		rt.create_array([rt.ArrayItem{ key: 'session_key', val: var_customer_id }])])
}

fn (mut this Class_Automattic_WooCommerce_StoreApi_SessionHandler) save_data() {
	mut var_wpdb := rt.new_null()
	if rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_SessionHandler', [
		'WC_Session',
	], &this), '_dirty')
	{
		rt.call_method(var_wpdb, 'query', [
			rt.call_method(var_wpdb, 'prepare', [
				rt.new_string('INSERT INTO %i (`session_key`, `session_value`, `session_expiry`) VALUES (%s, %s, %d) ON DUPLICATE KEY UPDATE `session_value` = VALUES(`session_value`), `session_expiry` = VALUES(`session_expiry`)'),
				this.table,
				this.get_customer_id(),
				rt.call_function('maybe_serialize', [
					rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_SessionHandler', [
						'WC_Session',
					], &this), '_data'),
				]),
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_SessionHandler) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_customer_unique_id' {
			return rt.new_string(this.get_customer_unique_id())
		}
		'get_session_data' {
