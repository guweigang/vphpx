...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_cache_key(var_action_id rt.PhpVal) string {
	...
	mut iife_result_0 := iife_temp_0.get_cache_prefix(rt.new_string('store_api_rate_limit' +
		var_action_id.str()))
...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_rate_limit_row(action_id string) rt.PhpVal {
	...
	mut var_row := rt.call_method(var_wpdb, 'get_row', [
	...
	if !rt.is_true(var_row) {
		...
		return mut rt.array_to_object(rt.create_array([
			rt.ArrayItem{ key: 'reset', val: rt.add(rt.new_int((rt.get_property(var_options,
				'seconds')).to_i64()), var_time) },
			rt.ArrayItem{
				key: 'remaining'
				val: rt.new_int((rt.get_property(var_options, 'limit')).to_i64())
			},
		]))
	}
	return mut rt.array_to_object(rt.create_array([
		rt.ArrayItem{ key: 'reset', val: rt.new_int((rt.get_property(var_row, 'reset')).to_i64()) },
		rt.ArrayItem{
			key: 'remaining'
			val: rt.new_int((rt.get_property(var_row, 'remaining')).to_i64())
		},
	]))
}

fn Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_rate_limit(action_id string) rt.PhpVal {
	mut var_current_limit :=
		Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_cached(rt.new_string(action_id))
	if rt.is_true(rt.identical(rt.new_bool(false), var_current_limit)) {
		var_current_limit =
			Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_rate_limit_row(action_id)
		Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.set_cache(rt.new_string(action_id),
			var_current_limit.clone())
	}
	return var_current_limit.clone()
}

fn Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.is_exceeded_retry_after(action_id string) bool {
	mut var_current_limit :=
		Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_rate_limit(action_id)
	mut var_time := rt.call_function('time', []rt.PhpVal{})
	if rt.is_true(rt.less_equal(var_time, rt.new_int((rt.get_property(var_current_limit, 'reset')).to_i64())))
		&& 0 == rt.new_int((rt.get_property(var_current_limit, 'remaining')).to_i64()) {
		return (rt.sub(rt.new_int((rt.get_property(var_current_limit, 'reset')).to_i64()), var_time)).to_bool()
	}
	return false
}
...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.update_rate_limit(action_id string) rt.PhpVal {
	...
	mut var_options := Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_options()
	mut var_time := rt.call_function('time', []rt.PhpVal{})
	mut var_rate_limit_expiry := rt.add(var_time, rt.new_int((rt.get_property(var_options,
		'seconds')).to_i64()))
	rt.call_method(var_wpdb, 'query', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.concat(rt.concat(rt.new_string('INSERT INTO '), rt.get_property(var_wpdb, 'prefix')),
			...
			rt.new_string(action_id),
			var_rate_limit_expiry.clone(),
			rt.new_int((rt.get_property(var_options, 'limit')).to_i64()) - 1,
			var_time.clone(),
			var_time.clone(),
		]),
	])
...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_cached(var_action_id rt.PhpVal) rt.PhpVal {
	return rt.call_function('wp_cache_get', [
		Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_cache_key(var_action_id.clone()),
		Class_Automattic_WooCommerce_StoreApi_Utilities_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.cache_group(),
	])
}
...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.set_cache(var_action_id rt.PhpVal, var_current_limit rt.PhpVal) bool {
	...
	return (rt.call_function('wp_cache_set', [
		Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_cache_key(var_action_id.clone()),
		var_current_limit_mutated.clone(),
		Class_Automattic_WooCommerce_StoreApi_Utilities_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.cache_group(),
	])).to_bool()
}
...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_options() rt.PhpVal {
	mut var_default_options := rt.create_array([
		rt.ArrayItem{
			key: 'enabled'
			val: Class_Automattic_WooCommerce_StoreApi_Utilities_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.enabled()
		},
		rt.ArrayItem{
			key: 'proxy_support'
			val: Class_Automattic_WooCommerce_StoreApi_Utilities_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.proxy_support()
		},
		rt.ArrayItem{
			key: 'limit'
			val: Class_Automattic_WooCommerce_StoreApi_Utilities_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.limit()
		},
		rt.ArrayItem{
			key: 'seconds'
			val: Class_Automattic_WooCommerce_StoreApi_Utilities_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.seconds()
		},
	])
	return mut rt.array_to_object(rt.call_function('array_merge', [
		var_default_options.clone(),
...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.get_option(var_option rt.PhpVal) rt.PhpVal {
	if !(var_option.clone().is_string())
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('RateLimits::' + var_option.clone().to_string().to_upper())]))))) {
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'set_cache' {
			...
			return rt.new_bool(Class_Automattic_WooCommerce_StoreApi_Utilities_RateLimits.set_cache(dispatch_arg_0,
				dispatch_arg_1))
		}
		'get_options' {
