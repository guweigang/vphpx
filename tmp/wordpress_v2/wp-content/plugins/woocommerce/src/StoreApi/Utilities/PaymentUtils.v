...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_PaymentUtils.include_token_id_with_payment_methods(var_list_item rt.PhpVal, var_token rt.PhpVal) rt.PhpVal {
	...
	mut var_brand := rt.new_string((if !(!rt.is_true(var_list_item_mutated.array_get(rt.new_string('method')).array_get(rt.new_string('brand')))) {
		var_list_item_mutated.array_get(rt.new_string('method')).array_get(rt.new_string('brand')).to_string().to_lower()
	} else {
		''
	}).str())
	if !(!rt.is_true(var_brand))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('esc_html__', [rt.new_string('Credit card'), rt.new_string('woocommerce')]), var_brand)))) {
		var_list_item_mutated.array_get_mut('method').array_set('brand', rt.call_function('wc_get_credit_card_type_label', [
			var_brand.clone(),
		]))
	}
	return var_list_item_mutated.clone()
}
...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_PaymentUtils.get_enabled_payment_gateways() rt.PhpVal {
	...
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_payment_gateway := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return rt.identical(rt.new_string('yes'), rt.get_property(var_payment_gateway, 'enabled'))
	}
	return rt.call_function('array_filter', [var_payment_gateways.clone(),
...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_PaymentUtils.get_saved_payment_methods() rt.PhpVal {
	...
	rt.call_function('add_filter', [
		...
		rt.create_array([
			rt.ArrayItem{
				key: none
				val: Class_Automattic_WooCommerce_StoreApi_Utilities_Automattic_WooCommerce_StoreApi_Utilities_PaymentUtils.class()
			},
			rt.ArrayItem{ key: none, val: 'include_token_id_with_payment_methods' },
		]),
		rt.new_int(10),
		rt.new_int(2),
	])
	mut var_enabled_payment_gateways :=
		Class_Automattic_WooCommerce_StoreApi_Utilities_PaymentUtils.get_enabled_payment_gateways()
	mut var_saved_payment_methods := rt.call_function('wc_get_customer_saved_methods_list', [
		rt.call_function('get_current_user_id', []rt.PhpVal{}),
	])
	mut var_payment_methods := rt.create_array([
		rt.ArrayItem{ key: 'enabled', val: rt.new_array() },
	...
	for {
		...
		closure_2_fn := fn [var_enabled_payment_gateways, mut var_payment_methods] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_saved_payment_method := if args.len > 0 {
				args[0].clone()
			} else {
				rt.new_null()
			}
			if rt.is_true(rt.identical(rt.new_bool(true), var_saved_payment_method.array_get(rt.new_string('is_default'))))
				&& rt.is_true(rt.identical(rt.new_null(), var_payment_methods.array_get(rt.new_string('default')))) {
				var_payment_methods.array_set('default', var_saved_payment_method.clone())
			}
			return rt.call_function('in_array', [var_saved_payment_method.array_get(rt.new_string('method')).array_get(rt.new_string('gateway')),
				rt.func_array_keys(var_enabled_payment_gateways.clone()),
				rt.new_bool(true)])
		}
		...
		var_payment_methods.array_get_mut('enabled').array_set(var_payment_method_group, rt.call_function('array_values', [
			rt.call_function('array_filter', [var_saved_payment_methods_shadow.clone(),
...
fn Class_Automattic_WooCommerce_StoreApi_Utilities_PaymentUtils.get_default_payment_method() string {
	mut var_saved_payment_methods :=
		Class_Automattic_WooCommerce_StoreApi_Utilities_PaymentUtils.get_saved_payment_methods()
	if rt.is_true(var_saved_payment_methods)
		&& !(!rt.is_true(var_saved_payment_methods.array_get(rt.new_string('default')))) {
		return (if !(var_saved_payment_methods.array_get(rt.new_string('default')).array_get(rt.new_string('method')).array_get(rt.new_string('gateway'))).is_null() {
			var_saved_payment_methods.array_get(rt.new_string('default')).array_get(rt.new_string('method')).array_get(rt.new_string('gateway'))
		} else {
			rt.new_string('')
		}).str()
	}
	mut var_chosen_payment_method := rt.call_method(rt.get_property(rt.call_function('WC',
		[]rt.PhpVal{}), 'session'), 'get', [rt.new_string('chosen_payment_method')])
	...
	mut var_first_key := rt.call_function('array_key_first', [
		var_enabled_payment_gateways.clone()])
	mut var_first_payment_method := var_enabled_payment_gateways.array_get(var_first_key)
	return (if !(rt.get_property(var_first_payment_method, 'id')).is_null() {
		rt.get_property(var_first_payment_method, 'id')
	} else {
		rt.new_string('')
	}).str()
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Utilities_PaymentUtils) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'include_token_id_with_payment_methods' {
			...
			return Class_Automattic_WooCommerce_StoreApi_Utilities_PaymentUtils.include_token_id_with_payment_methods(dispatch_arg_0,
				dispatch_arg_1)
		}
