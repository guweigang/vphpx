...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute) set_namespace(var_namespace rt.PhpVal) {
	this.namespace = var_namespace.clone()
}
...
end_label_1:
	return if rt.is_true(rt.call_function('is_wp_error', [var_response.clone()])) { this.error_to_response(var_response.clone()) } else { var_response }
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute) error_to_response(var_error rt.PhpVal) rt.PhpVal {
	...
	mut var_status := if !(var_error_data).is_null() && var_error_data.array_isset(rt.new_string('status')) { var_error_data.array_get(rt.new_string('status')) } else { rt.new_int(500) }
	...
	for {
		...
		for {
			...
			var_errors.array_push(rt.create_array([rt.ArrayItem{ key: 'code', val: var_code }, rt.ArrayItem{ key: 'message', val: var_message }, rt.ArrayItem{ key: 'data', val: rt.call_method(var_error, 'get_error_data', [var_code.clone()]) }]))
		}
	}
	mut var_data := rt.call_function('array_shift', [var_errors.clone()])
	if rt.is_true(rt.new_int(var_errors.clone().array_count())) {
		var_data.array_set('additional_errors', var_errors.clone())
	}
	return rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Response', []string{}, create_automattic_woocommerce_storeapi_routes_v1_wp_rest_response(var_data.clone(), var_status.clone()))
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute) get_route_error_response(var_error_code rt.PhpVal, var_error_message rt.PhpVal, http_status_code i64, var_additional_data rt.PhpVal) rt.PhpVal {
	return rt.new_object('WP_Error', []string{}, create_wp_error(var_error_code.clone(), var_error_message.clone(), rt.call_function('array_merge', [var_additional_data.clone(), rt.create_array([rt.ArrayItem{ key: 'status', val: http_status_code }])])))
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute) get_route_error_response_from_object(var_error_object rt.PhpVal, http_status_code i64, var_additional_data rt.PhpVal) rt.PhpVal {
	rt.call_method(var_error_object, 'add_data', [rt.call_function('array_merge', [var_additional_data.clone(), rt.create_array([rt.ArrayItem{ key: 'status', val: http_status_code }])])])
	return var_error_object.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute) prepare_item_for_response(var_item rt.PhpVal, mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	mut var_response := rt.call_function('rest_ensure_response', [rt.call_method(this.schema, 'get_item_response', [var_item.clone()])])
	if rt.is_true(rt.call_function('is_wp_error', [var_response.clone()])) {
		return rt.call_function('rest_convert_error_to_response', [var_response.clone()])
	}
	rt.call_method(var_response, 'add_links', [this.prepare_links(var_item.clone(), rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request', []string{}, var_request))])
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute) get_context_param(var_args rt.PhpVal) rt.PhpVal {
	...
	if !rt.is_true(var_schema.array_get(rt.new_string('properties'))) {
		return rt.call_function('array_merge', [var_param_details.clone(), var_args.clone()])
	}
	mut var_contexts := rt.new_array()
	mut iter_3 := var_schema.array_get(rt.new_string('properties')).iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_attributes := item_3.val
	...
	if !(!rt.is_true(var_contexts)) {
		var_param_details.array_set('enum', rt.call_function('array_unique', [var_contexts.clone()]))
		rt.call_function('rsort', [var_param_details.array_get(rt.new_string('enum'))])
	}
	return rt.call_function('array_merge', [var_param_details.clone(), var_args.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute) prepare_response_for_collection(mut var_response Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Response) rt.PhpVal {
	...
	mut iife_temp_0 := Class_Automattic_WooCommerce_StoreApi_Routes_V1_{"nodeType":"Expr_Variable","line":305,"name":"server"}{}
	mut iife_result_0 := iife_temp_0.get_compact_response_links(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Response', []string{}, var_response_mutated))
...
fn create_automattic_woocommerce_storeapi_routes_v1_{"nodetype":"expr_variable","line":305,"name":"server"}(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Routes_V1_{"nodeType":"Expr_Variable","line":305,"name":"server"} {
