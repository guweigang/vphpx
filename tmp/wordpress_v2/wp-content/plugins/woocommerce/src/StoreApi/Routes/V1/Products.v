...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_Products) get_route_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
		...
		var_request.get_method()))
	{
		...
		rt.call_method(var_response, 'set_data', [var_response_objects.clone()])
	} else {
	...
	var_response = rt.call_method(create_automattic_woocommerce_storeapi_utilities_pagination(),
		'add_headers', [var_response.clone(), var_request, var_query_results.array_get(rt.new_string('total')),
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_Products) get_collection_params() rt.PhpVal {
	...
	mut iter_2 := rt.get_superglobal('_REQUEST').iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_value := item_2.val
		mut var_param := item_2.key
		if !(var_param.clone().is_string()) {
			continue
		}
		if rt.is_true(rt.call_function('str_starts_with', [var_param.clone(), rt.new_string('_unstable_tax_')]))
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_ends_with', [var_param.clone(), rt.new_string('_operator')]))))) {
		...
		if rt.is_true(rt.call_function('str_starts_with', [var_param.clone(), rt.new_string('_unstable_tax_')]))
			&& rt.is_true(rt.call_function('str_ends_with', [var_param.clone(), rt.new_string('_operator')])) {
			var_params.array_set(var_param, rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
