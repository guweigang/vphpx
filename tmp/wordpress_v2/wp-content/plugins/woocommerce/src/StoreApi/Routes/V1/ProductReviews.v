...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductReviews) get_route_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	mut var_prepared_args := rt.create_array([rt.ArrayItem{ key: 'type', val: 'review' },
		rt.ArrayItem{ key: 'status', val: 'approve' }, rt.ArrayItem{
			key: 'no_found_rows'
			val: false
		...
		rt.ArrayItem{ key: 'post__in', val: var_request.array_get(rt.new_string('product_id')) }])
	if !(!rt.is_true(var_request.array_get(rt.new_string('category_id')))) {
		mut var_category_ids := var_request.array_get(rt.new_string('category_id'))
		mut var_child_ids := rt.new_array()
		mut iter_1 := var_category_ids.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_category_id := item_1.val
			var_child_ids = rt.call_function('array_merge', [
				var_child_ids.clone(),
				rt.call_function('get_term_children', [
					var_category_id.clone(), rt.new_string('product_cat')])])
		}
		var_category_ids = rt.call_function('array_unique', [
			rt.call_function('array_merge', [var_category_ids.clone(),
				var_child_ids.clone()]),
		])
		mut var_product_ids := rt.call_function('get_objects_in_term', [
			var_category_ids.clone(), rt.new_string('product_cat')])
		var_prepared_args.array_set('post__in', if var_prepared_args.array_isset(rt.new_string('post__in')) { rt.call_function('array_merge', [
				var_prepared_args.array_get(rt.new_string('post__in')),
				var_product_ids.clone(),
			]) } else { var_product_ids })
	}
	if rt.is_true(rt.identical(rt.new_string('rating'),
		var_request.array_get(rt.new_string('orderby'))))
	{
	...
	for {
		...
		mut var_data := this.prepare_item_for_response(var_review.clone(), rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request',
			[]string{}, var_request))
		var_response_objects.array_push(this.prepare_response_for_collection(var_data.clone()))
	}
	...
	if rt.is_true(rt.less(var_total_reviews, rt.new_int(1))) {
		...
		var_max_pages = if rt.is_true(var_request.array_get(rt.new_string('per_page'))) { rt.call_function('ceil', [
				rt.div(var_total_reviews, var_request.array_get(rt.new_string('per_page'))),
			]) } else { rt.new_int(1) }
	}
	mut var_response := rt.call_function('rest_ensure_response', [
		var_response_objects.clone()])
	var_response = rt.call_method(create_automattic_woocommerce_storeapi_utilities_pagination(),
		'add_headers', [var_response.clone(), var_request, var_total_reviews.clone(),
		var_max_pages.clone()])
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductReviews) normalize_query_param(var_query_param rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('id'))) {
		...
		var_normalized = rt.new_string('meta_value_num')
	} else {
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductReviews) get_collection_params() rt.PhpVal {
	...
	var_params.array_set('category_id', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit result set to reviews from specific category IDs.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'string' },
	...
	var_params.array_set('product_id', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit result set to reviews from specific product IDs.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'string' },
		rt.ArrayItem{ key: 'sanitize_callback', val: 'wp_parse_id_list' },
...
fn create_automattic_woocommerce_storeapi_routes_v1_productreviews(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductReviews {
