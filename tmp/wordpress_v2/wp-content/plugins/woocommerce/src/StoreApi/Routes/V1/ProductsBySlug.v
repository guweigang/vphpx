	...
	return rt.create_array([
		rt.ArrayItem{ key: 'args', val: rt.create_array([
			rt.ArrayItem{ key: 'slug', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Slug of the resource.'),
		...
		rt.ArrayItem{ key: none, val: rt.create_array([
			...
			rt.ArrayItem{ key: 'callback', val: rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_ProductsBySlug', [
		...
		rt.ArrayItem{ key: 'schema', val: rt.create_array([
			rt.ArrayItem{ key: none, val: rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_ProductsBySlug', [
				'Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute',
			], &this), 'schema') },
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductsBySlug) get_route_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	...
	mut var_object := this.get_product_by_slug(var_slug.clone())
	if rt.is_true(rt.new_bool(!(rt.is_true(var_object)))) {
		var_object = this.get_product_variation_by_slug(var_slug.clone())
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_object))))
		|| rt.is_true(rt.identical(rt.new_int(0), rt.call_method(var_object, 'get_id', []rt.PhpVal{})))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Enums_ProductStatus.publish(), rt.call_method(var_object, 'get_status', []rt.PhpVal{}))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Exceptions_RouteException',
			[]string{}, create_automattic_woocommerce_storeapi_exceptions_routeexception(rt.new_string('woocommerce_rest_product_invalid_slug'), rt.call_function('__', [
			rt.new_string('Invalid product slug.'),
			rt.new_string('woocommerce'),
		]), rt.new_int(404))))
	}
	return this.prepare_item_for_response(var_object.clone(), rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request',
		[]string{}, var_request))
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductsBySlug) get_product_by_slug(var_slug rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('wc_get_product', [
		rt.call_function('get_page_by_path', [var_slug_mutated.clone(),
			rt.get_constant('OBJECT'), rt.new_string('product')]),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductsBySlug) get_product_variation_by_slug(var_slug rt.PhpVal) rt.PhpVal {
	...
	mut var_result := rt.call_method(var_wpdb, 'get_results', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.concat(rt.concat(rt.new_string('SELECT ID, post_name, post_parent, post_type\n\t\t\t\tFROM '), rt.get_property(var_wpdb,
				'posts')),
				rt.new_string("\n\t\t\t\tWHERE post_name = %s\n\t\t\t\tAND post_type = 'product_variation'")),
			var_slug_mutated.clone(),
		]),
	])
	...
	return rt.call_function('wc_get_product', [
		rt.get_property(var_result.array_get(rt.new_int(0)), 'ID'),
	])
}
...
fn create_automattic_woocommerce_storeapi_routes_v1_productsbyslug(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductsBySlug {
