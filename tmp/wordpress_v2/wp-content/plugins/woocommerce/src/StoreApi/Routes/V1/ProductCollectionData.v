...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductCollectionData) get_route_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	mut var_data := rt.create_array([
		rt.ArrayItem{ key: 'min_price', val: rt.new_null() },
		rt.ArrayItem{ key: 'max_price', val: rt.new_null() },
		rt.ArrayItem{ key: 'attribute_counts', val: rt.new_null() },
		rt.ArrayItem{ key: 'stock_status_counts', val: rt.new_null() },
		rt.ArrayItem{ key: 'rating_counts', val: rt.new_null() },
		rt.ArrayItem{ key: 'taxonomy_counts', val: rt.new_null() },
	])
	mut var_filters := create_automattic_woocommerce_storeapi_utilities_productqueryfilters()
	if !(!rt.is_true(var_request.array_get(rt.new_string('calculate_price_range')))) {
		mut var_filter_request := var_request.dup()
		rt.call_method(var_filter_request, 'set_param', [rt.new_string('min_price'),
			rt.new_null()])
		rt.call_method(var_filter_request, 'set_param', [rt.new_string('max_price'),
			rt.new_null()])
		mut var_price_results := var_filters.get_filtered_price(var_filter_request.clone())
	...
	if !(!rt.is_true(var_request.array_get(rt.new_string('calculate_stock_status_counts')))) {
		var_filter_request = var_request.dup()
		mut var_counts := var_filters.get_stock_status_counts(var_filter_request.clone())
		...
		for {
			...
			var_data.array_get_mut('stock_status_counts').array_push(rt.array_to_object(rt.create_array([
				rt.ArrayItem{ key: 'status', val: var_key },
				rt.ArrayItem{ key: 'count', val: var_value },
			])))
		}
	}
	if !(!rt.is_true(var_request.array_get(rt.new_string('calculate_attribute_counts')))) {
		mut var_taxonomy__or_queries := rt.new_array()
		mut var_taxonomy__and_queries := rt.new_array()
		mut iter_2 := var_request.array_get(rt.new_string('calculate_attribute_counts')).iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_attributes_to_count := item_2.val
			if !(!rt.is_true(var_attributes_to_count.array_get(rt.new_string('taxonomy')))) {
				if !rt.is_true(var_attributes_to_count.array_get(rt.new_string('query_type')))
					|| rt.is_true(rt.identical(rt.new_string('or'), var_attributes_to_count.array_get(rt.new_string('query_type')))) {
					var_taxonomy__or_queries.array_push(var_attributes_to_count.array_get(rt.new_string('taxonomy')))
				} else {
					var_taxonomy__and_queries.array_push(var_attributes_to_count.array_get(rt.new_string('taxonomy')))
				}
			}
		}
		var_data.array_set('attribute_counts', rt.new_array())
		if rt.is_true(var_taxonomy__or_queries) {
			mut iter_3 := var_taxonomy__or_queries.iterator()
			for {
				item_3 := iter_3.next() or { break }
				mut var_taxonomy := item_3.val
				var_filter_request = var_request.dup()
				mut var_filter_attributes := rt.call_method(var_filter_request, 'get_param', [
				...
				if !(!rt.is_true(var_filter_attributes)) {
					closure_1_fn := fn [var_taxonomy] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
						...
						return rt.new_bool(!rt.is_true(rt.identical(var_query.array_get(rt.new_string('attribute')),
							var_taxonomy)))
					}
					var_filter_attributes = rt.call_function('array_filter', [
						var_filter_attributes.clone(), rt.new_closure(closure_1_fn)])
				}
				rt.call_method(var_filter_request, 'set_param', [
					rt.new_string('attributes'),
					var_filter_attributes.clone(),
				])
				var_counts = var_filters.get_attribute_counts(var_filter_request.clone(), rt.create_array([
					rt.ArrayItem{ key: none, val: var_taxonomy },
				]))
				mut iter_4 := var_counts.iterator()
				for {
					item_4 := iter_4.next() or { break }
					mut var_value := item_4.val
					mut var_key := item_4.key
					var_data.array_get_mut('attribute_counts').array_push(rt.array_to_object(rt.create_array([
		...
		if rt.is_true(var_taxonomy__and_queries) {
			var_counts = var_filters.get_attribute_counts(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request',
				[]string{}, var_request), var_taxonomy__and_queries.clone())
			mut iter_5 := var_counts.iterator()
			for {
	...
	if !(!rt.is_true(var_request.array_get(rt.new_string('calculate_rating_counts')))) {
		var_filter_request = var_request.dup()
		var_counts = var_filters.get_rating_counts(var_filter_request.clone())
		var_data.array_set('rating_counts', rt.new_array())
		mut iter_6 := var_counts.iterator()
		for {
			item_6 := iter_6.next() or { break }
			mut var_value := item_6.val
			mut var_key := item_6.key
			var_data.array_get_mut('rating_counts').array_push(rt.array_to_object(rt.create_array([
				rt.ArrayItem{ key: 'rating', val: var_key },
				rt.ArrayItem{ key: 'count', val: var_value },
			])))
		}
	}
	if !(!rt.is_true(var_request.array_get(rt.new_string('calculate_taxonomy_counts')))) {
		mut var_taxonomies := var_request.array_get(rt.new_string('calculate_taxonomy_counts'))
		var_data.array_set('taxonomy_counts', rt.new_array())
		if rt.is_true(var_taxonomies) {
			var_counts = var_filters.get_taxonomy_counts(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request',
				[]string{}, var_request), var_taxonomies.clone())
			mut iter_7 := var_counts.iterator()
			for {
				item_7 := iter_7.next() or { break }
				mut var_value := item_7.val
				mut var_key := item_7.key
				var_data.array_get_mut('taxonomy_counts').array_push(rt.array_to_object(rt.create_array([
					rt.ArrayItem{ key: 'term', val: var_key },
					rt.ArrayItem{ key: 'count', val: var_value },
				])))
			}
	...
	return rt.call_function('rest_ensure_response', [
		rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_ProductCollectionData', [
			'Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute',
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductCollectionData) get_collection_params() rt.PhpVal {
	mut var_params := rt.call_method(create_automattic_woocommerce_storeapi_routes_v1_products(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_ProductCollectionData', [
		'Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute',
	], &this), 'schema_controller'), rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_ProductCollectionData', [
		'Automattic_WooCommerce_StoreApi_Routes_V1_AbstractRoute',
	], &this), 'schema')), 'get_collection_params', []rt.PhpVal{})
	var_params.array_set('calculate_price_range', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('If true, calculates the minimum and maximum product prices for the collection.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'boolean' },
		rt.ArrayItem{ key: 'default', val: false },
	]))
	var_params.array_set('calculate_stock_status_counts', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('If true, calculates stock counts for products in the collection.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'boolean' },
		rt.ArrayItem{ key: 'default', val: false },
	...
	var_params.array_set('calculate_attribute_counts', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('If requested, calculates attribute term counts for products in the collection.'),
			rt.new_string('woocommerce'),
		]) },
		...
		rt.ArrayItem{ key: 'items', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'properties', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'query_type', val: rt.create_array([
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Filter condition\t being performed which may affect counts. Valid values include "and" and "or".'),
						rt.new_string('woocommerce'),
					]) },
					rt.ArrayItem{ key: 'type', val: 'string' },
	...
	var_params.array_set('calculate_rating_counts', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('If true, calculates rating counts for products in the collection.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'boolean' },
		rt.ArrayItem{ key: 'default', val: false },
	]))
	var_params.array_set('calculate_taxonomy_counts', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('If requested, calculates taxonomy term counts for products in the collection.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'array' },
		rt.ArrayItem{ key: 'items', val: rt.create_array([
...
fn create_automattic_woocommerce_storeapi_routes_v1_productcollectiondata(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Routes_V1_ProductCollectionData {
...
fn create_automattic_woocommerce_storeapi_routes_v1_products(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Routes_V1_Products {
