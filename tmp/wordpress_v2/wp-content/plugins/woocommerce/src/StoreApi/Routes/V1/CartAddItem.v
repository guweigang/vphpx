...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartAddItem) get_route_post_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	...
	mut var_item_id := rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CartAddItem', [
		'Automattic_WooCommerce_StoreApi_Routes_V1_AbstractCartRoute',
	], &this), 'cart_controller'), 'add_to_cart', [var_add_to_cart_data.clone()])
	...
	if !(!rt.is_true(var_cart_item)) {
		mut var_product_id := if rt.is_true(var_cart_item.array_get(rt.new_string('variation_id'))) {
			var_cart_item.array_get(rt.new_string('variation_id'))
		} else {
			var_cart_item.array_get(rt.new_string('product_id'))
		}
		mut var_quantity := if !(var_add_to_cart_data.array_get(rt.new_string('quantity'))).is_null() {
			var_add_to_cart_data.array_get(rt.new_string('quantity'))
		} else {
...
fn create_automattic_woocommerce_storeapi_routes_v1_cartadditem(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Routes_V1_CartAddItem {
