	...
	return rt.create_array([
		rt.ArrayItem{ key: none, val: rt.create_array([
			...
			rt.ArrayItem{ key: 'callback', val: rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CheckoutOrder', [
			...
			rt.ArrayItem{ key: 'args', val: rt.call_function('array_merge', [
				...
				rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CheckoutOrder', [
					'Automattic_WooCommerce_StoreApi_Routes_V1_AbstractCartRoute',
		...
		rt.ArrayItem{ key: 'schema', val: rt.create_array([
			rt.ArrayItem{ key: none, val: rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_CheckoutOrder', [
				'Automattic_WooCommerce_StoreApi_Routes_V1_AbstractCartRoute',
			], &this), 'schema') },
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CheckoutOrder) get_route_post_response(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	mut var_order_id := rt.call_function('absint', [var_request.array_get(rt.new_string('id'))])
	this.order = rt.call_function('wc_get_order', [var_order_id.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(this.order))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.order, 'needs_payment', []rt.PhpVal{}))))) {
		return rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_WP_Error', []string{}, create_automattic_woocommerce_storeapi_routes_v1_wp_error(rt.new_string('invalid_order_update_status'), rt.call_function('__', [
			rt.new_string('This order cannot be paid for.'),
			rt.new_string('woocommerce'),
		])))
	}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CheckoutOrder) update_billing_address(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) {
	...
	mut var_billing := var_request.array_get(rt.new_string('billing_address'))
	mut var_shipping := var_request.array_get(rt.new_string('shipping_address'))
	...
	for {
		...
		if rt.is_true(rt.call_function('is_callable', [
			rt.create_array([rt.ArrayItem{ key: none, val: var_customer },
				rt.ArrayItem{ key: none, val: 'set_billing_${var_key.to_string()}' }]),
		]))
		{
			rt.call_method(var_customer, 'set_billing_${var_key.to_string()}', [
				var_value.clone(),
			])
		}
	}
	mut var_shipping_address_values := if !var_shipping.is_null() {
		var_shipping
	} else {
		var_billing
	}
	mut iter_2 := var_shipping_address_values.iterator()
	for {
		...
		if rt.is_true(rt.call_function('is_callable', [
			rt.create_array([rt.ArrayItem{ key: none, val: var_customer },
				rt.ArrayItem{ key: none, val: 'set_shipping_${var_key.to_string()}' }]),
		]))
		{
			rt.call_method(var_customer, 'set_shipping_${var_key.to_string()}', [
				var_value.clone(),
			])
		} else if rt.is_true(rt.identical(rt.new_string('phone'), var_key)) {
			rt.call_method(var_customer, 'update_meta_data', [
				rt.new_string('shipping_phone'),
				var_value.clone(),
			])
		}
	}
	...
	rt.call_method(this.order, 'set_billing_address', [var_billing.clone()])
	rt.call_method(this.order, 'set_shipping_address', [var_shipping.clone()])
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_CheckoutOrder) get_request_payment_method(mut var_request Class_Automattic_WooCommerce_StoreApi_Routes_V1_WP_REST_Request) rt.PhpVal {
	...
	if !rt.is_true(var_request_payment_method) {
		if rt.is_true(var_requires_payment_method) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Exceptions_RouteException',
				[]string{}, create_automattic_woocommerce_storeapi_exceptions_routeexception(rt.new_string('woocommerce_rest_checkout_missing_payment_method'), rt.call_function('__', [
	...
	if !(var_available_gateways.array_isset(var_request_payment_method)) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Exceptions_RouteException',
			...
			rt.call_function('__', [
				rt.new_string('The %s payment gateway is not available.'),
				rt.new_string('woocommerce'),
			]),
			rt.call_function('esc_html', [
				var_request_payment_method.clone(),
			]),
		]), rt.new_int(400))))
	}
...
fn create_automattic_woocommerce_storeapi_routes_v1_checkoutorder(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Routes_V1_CheckoutOrder {
