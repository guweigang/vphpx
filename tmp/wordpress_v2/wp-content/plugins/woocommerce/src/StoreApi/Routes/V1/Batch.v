...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_Batch) get_args() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'callback', val: rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_StoreApi_Routes_V1_Batch', [
		...
		rt.ArrayItem{ key: 'args', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'requests', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'maxItems', val: 25 },
				rt.ArrayItem{ key: 'items', val: rt.create_array([
					rt.ArrayItem{ key: 'type', val: 'object' },
					rt.ArrayItem{ key: 'properties', val: rt.create_array([
						rt.ArrayItem{ key: 'method', val: rt.create_array([
							rt.ArrayItem{ key: 'type', val: 'string' },
							rt.ArrayItem{ key: 'enum', val: rt.call_function('apply_filters', [
								rt.new_string('__experimental_woocommerce_store_api_batch_request_methods'),
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Routes_V1_Batch) get_response(mut var_request Class_WP_REST_Request) rt.PhpVal {
	mut iter_1 := var_request.array_get(rt.new_string('requests')).iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_args := item_1.val
		mut var_parsed_path := rt.call_function('wp_parse_url', [
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(var_parsed_path))))
			|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('strpos', [var_parsed_path.clone(), rt.new_string('/wc/store')]), rt.new_int(0))))) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Exceptions_RouteException',
				[]string{}, create_automattic_woocommerce_storeapi_exceptions_routeexception(rt.new_string('woocommerce_rest_invalid_path'), rt.call_function('__', [
				rt.new_string('Invalid path provided.'),
	...
	mut var_response := rt.call_method(rt.call_function('rest_get_server', []rt.PhpVal{}),
		'serve_batch_request_v1', [var_request])
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_StoreApi_Exceptions_RouteException') {
		...
		var_response = this.get_route_error_response(rt.new_string('woocommerce_rest_unknown_server_error'), rt.call_method(var_error,
			'getMessage', []rt.PhpVal{}), rt.new_int(500))
		unsafe {
			goto end_label_1
		}
	} else {
	...
	rt.call_method(var_response, 'header', [rt.new_string('Nonce'),
		var_nonce.clone()])
	rt.call_method(var_response, 'header', [rt.new_string('Nonce-Timestamp'),
		rt.call_function('time', []rt.PhpVal{})])
	rt.call_method(var_response, 'header', [rt.new_string('User-ID'),
...
fn create_automattic_woocommerce_storeapi_routes_v1_batch(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Routes_V1_Batch {
