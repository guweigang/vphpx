...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Payments_PaymentContext) magic_get(var_name rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.call_function('in_array', [var_name.clone(),
		rt.create_array([rt.ArrayItem{ key: none, val: 'payment_method' },
			rt.ArrayItem{ key: none, val: 'order' }, rt.ArrayItem{ key: none, val: 'payment_data' }]),
		rt.new_bool(true)]))
	{
		return rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Payments_PaymentContext',
			[]string{}, &this), '{"nodeType":"Expr_Variable","line":36,"name":"name"}')
	}
	return rt.new_null()
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Payments_PaymentContext) set_payment_method(var_payment_method rt.PhpVal) {
	this.payment_method = var_payment_method.str()
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Payments_PaymentContext) set_payment_data(var_payment_data rt.PhpVal) {
	...
	for {
		...
		this.payment_data.array_set(var_key.str(), var_value.str())
	}
}

fn create_automattic_woocommerce_storeapi_payments_paymentcontext(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Payments_PaymentContext {
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Payments_PaymentContext) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'set_order' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_StoreApi_Payments_WC_Order](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
