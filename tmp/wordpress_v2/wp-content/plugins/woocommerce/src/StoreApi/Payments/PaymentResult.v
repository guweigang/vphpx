...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Payments_PaymentResult) magic_get(var_name rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.call_function('in_array', [var_name.clone(),
		rt.create_array([rt.ArrayItem{ key: none, val: 'status' },
			rt.ArrayItem{ key: none, val: 'payment_details' },
			rt.ArrayItem{ key: none, val: 'redirect_url' }]),
		rt.new_bool(true)]))
	{
		return rt.get_property(rt.new_object('Automattic_WooCommerce_StoreApi_Payments_PaymentResult',
			[]string{}, &this), '{"nodeType":"Expr_Variable","line":54,"name":"name"}')
	}
	return rt.new_null()
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Payments_PaymentResult) set_status(var_payment_status rt.PhpVal) {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_payment_status.clone(), this.valid_statuses, rt.new_bool(true)])))))
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Payments_Exception',
			[]string{}, create_automattic_woocommerce_storeapi_payments_exception(rt.call_function('sprintf', [
			rt.new_string('Invalid payment status %s. Use one of %s'),
			var_payment_status.clone(),
			rt.call_function('implode', [rt.new_string(', '), this.valid_statuses]),
		]))))
	}
	this.status = var_payment_status.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Payments_PaymentResult) set_payment_details(var_payment_details rt.PhpVal) {
	...
	for {
		...
		this.payment_details.array_set(var_key.str(), var_value.str())
	}
}

fn (mut this Class_Automattic_WooCommerce_StoreApi_Payments_PaymentResult) set_redirect_url(var_redirect_url rt.PhpVal) {
	this.redirect_url = rt.call_function('esc_url_raw', [var_redirect_url.clone()])
}
...
fn create_automattic_woocommerce_storeapi_payments_exception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Payments_Exception {
