...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Formatters_MoneyFormatter) format(var_value rt.PhpVal, mut var_options Class_Automattic_WooCommerce_StoreApi_Formatters_array) string {
	...
	if !(var_value_mutated.clone().is_long()) && !(var_value_mutated.clone().is_string())
	...
	var_options_mutated = rt.call_function('wp_parse_args', [var_options_mutated,
		rt.create_array([
	...
	var_options_mutated.array_set('rounding_mode', rt.call_function('absint', [
		var_options_mutated.array_get(rt.new_string('rounding_mode')),
	]))
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_options_mutated.array_get(rt.new_string('rounding_mode')),
		var_rounding_modes.clone(),
		rt.new_bool(true),
	])))))
	{
		var_options_mutated.array_set('rounding_mode', rt.get_constant('PHP_ROUND_HALF_UP'))
	}
	var_value_mutated = rt.new_float(var_value_mutated.clone().to_f64())
	var_value_mutated = rt.mul(var_value_mutated, rt.call_function('pow', [
		rt.new_int(10),
		rt.call_function('absint', [var_options_mutated.array_get(rt.new_string('decimals'))]),
	]))
	var_value_mutated = rt.call_function('round', [var_value_mutated.clone(),
		rt.new_int(0), var_options_mutated.array_get(rt.new_string('rounding_mode'))])
	return (rt.call_function('wc_format_decimal', [var_value_mutated.clone(),
