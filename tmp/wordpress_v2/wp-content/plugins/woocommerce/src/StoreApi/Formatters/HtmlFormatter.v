...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Formatters_HtmlFormatter) format(var_value rt.PhpVal, mut var_options Class_Automattic_WooCommerce_StoreApi_Formatters_array) rt.PhpVal {
	...
	return if rt.is_true(rt.call_function('is_scalar', [var_value.clone()])) { rt.call_function('wp_kses_post', [
			rt.new_string(rt.call_function('convert_chars', [
				rt.call_function('wptexturize', [var_value.clone()]),
			]).to_string().trim_space()),
		]) } else { var_value }
}

fn create_automattic_woocommerce_storeapi_formatters_htmlformatter(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Formatters_HtmlFormatter {
