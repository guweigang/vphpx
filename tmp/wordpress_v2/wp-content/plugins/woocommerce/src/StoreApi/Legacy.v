...
fn (mut this Class_Automattic_WooCommerce_StoreApi_Legacy) process_legacy_payment(mut var_context Class_Automattic_WooCommerce_StoreApi_Payments_PaymentContext, mut var_result Class_Automattic_WooCommerce_StoreApi_Payments_PaymentResult) {
	...
	mut var_post_data := rt.get_superglobal('_POST').clone()
	...
	mut iife_result_0 :=
		iife_temp_0.convert_notices_to_exceptions(rt.new_string('woocommerce_rest_payment_error'))
	...
	var__POST = var_post_data.clone()
	if var_gateway_result.array_isset(rt.new_string('result'))
		&& rt.is_true(rt.identical(rt.new_string('failure'), var_gateway_result.array_get(rt.new_string('result')))) {
		if var_gateway_result.array_isset(rt.new_string('message')) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_StoreApi_Exceptions_RouteException',
				...
				rt.call_function('wp_strip_all_tags', [
					var_gateway_result.array_get(rt.new_string('message')),
				]),
			]), rt.new_int(400))))
		} else {
			mut iife_temp_1 := Class_Automattic_WooCommerce_StoreApi_Utilities_NoticeHandler{}
			mut iife_result_1 :=
				iife_temp_1.convert_notices_to_exceptions(rt.new_string('woocommerce_rest_payment_error'))
		}
	}
	mut var_result_status := if !(var_gateway_result.array_get(rt.new_string('result'))).is_null() {
		var_gateway_result.array_get(rt.new_string('result'))
	} else {
	...
	var_result.set_status(if rt.is_true(rt.call_function('in_array', [
		var_result_status.clone(), var_valid_status.clone(), rt.new_bool(true)]))
	...
	var_result.set_payment_details(rt.call_function('array_merge', [
		...
		var_gateway_result.clone(),
	]))
	var_result.set_redirect_url(if !(var_gateway_result.array_get(rt.new_string('redirect'))).is_null() {
		var_gateway_result.array_get(rt.new_string('redirect'))
	} else {
		rt.new_string('')
	})
}
