...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_PersonalizationTags_StoreTagsProvider) register_tags(mut var_registry Class_Automattic_WooCommerce_EmailEditor_Engine_PersonalizationTags_Personalization_Tags_Registry) {
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_wc_email := if !(var_context.array_get(rt.new_string('wc_email'))).is_null() {
			var_context.array_get(rt.new_string('wc_email'))
		} else {
			rt.new_null()
		}
	...
	closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		if var_context.array_isset(rt.new_string('wc_email'))
			&& !(!rt.is_true(rt.call_method(var_context.array_get(rt.new_string('wc_email')), 'get_from_name', []rt.PhpVal{}))) {
			return
		}
		return
	}
	...
	closure_6_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_context := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		if var_context.array_isset(rt.new_string('wc_email'))
			&& !(rt.get_property(var_context.array_get(rt.new_string('wc_email')), 'customer_note')).is_null() {
			return
		}
		return
	}
