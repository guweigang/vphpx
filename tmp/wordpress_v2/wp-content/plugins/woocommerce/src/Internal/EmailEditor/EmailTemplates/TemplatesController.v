...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailTemplates_TemplatesController) init() {
	rt.call_function('add_filter', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_EmailEditor_EmailTemplates_TemplatesController',
				[]string{}, &this) },
	...
	rt.call_function('add_filter', [rt.new_string('get_block_templates'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_EmailEditor_EmailTemplates_TemplatesController',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'filter_email_templates' },
		]),
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailTemplates_TemplatesController) filter_email_templates(var_templates rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_current_screen)
		&& rt.is_true(rt.identical(rt.new_string('site-editor'), rt.get_property(var_current_screen, 'id'))) {
		closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_template := if args.len > 0 { args[0].clone() } else { rt.new_null() }
			return rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_EmailEditor_EmailTemplates_WooEmailTemplate.template_slug(), rt.get_property(var_template,
				'slug'))))
		}
		var_templates_mutated = rt.call_function('array_filter', [
			var_templates_mutated.clone(), rt.new_closure(closure_1_fn)])
	}
	return var_templates_mutated.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_EmailTemplates_TemplatesController) register_templates(mut var_templates_registry Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates_Registry) rt.PhpVal {
	...
	for {
		...
		mut var_the_template := create_automattic_woocommerce_emaileditor_engine_templates_template(this.template_prefix, rt.call_method(var_template,
			'get_slug', []rt.PhpVal{}), rt.call_method(var_template, 'get_title', []rt.PhpVal{}), rt.call_method(var_template,
			'get_description', []rt.PhpVal{}), rt.call_method(var_template, 'get_content',
...
fn create_automattic_woocommerce_internal_emaileditor_emailtemplates_templatescontroller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_EmailEditor_EmailTemplates_TemplatesController {
