...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_WooContentProcessor) construct() {
	...
	this.theme_controller = rt.call_method(iife_result_0, 'get', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_WooContentProcessor) prepare_css(css string) string {
	mut css_mutated := css
	rt.call_function('remove_filter', [rt.new_string('woocommerce_email_styles'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_EmailEditor_WooContentProcessor',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'prepare_css' },
		])])
	return (rt.call_function('preg_replace', [
		rt.create_array([rt.ArrayItem{ key: none, val: '/color\\s*:\\s*[^;]+;/' },
			rt.ArrayItem{ key: none, val: '/font-family\\s*:\\s*[^;]+;/' },
			rt.ArrayItem{ key: none, val: '/font-size\\s*:\\s*[^;]+;/' },
			rt.ArrayItem{ key: none, val: '/font-weight\\s*:\\s*[^;]+;/' },
			rt.ArrayItem{ key: none, val: '/line-height\\s*:\\s*[^;]+;/' },
			rt.ArrayItem{ key: none, val: '/letter-spacing\\s*:\\s*[^;]+;/' }]),
		rt.new_string(''),
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_WooContentProcessor) get_html_body_content(html string) string {
	...
	if rt.is_true(rt.call_function('preg_match', [
		rt.new_string('/<body[^>]*>(.*?)<\\/body>/is'),
		rt.new_string(html),
		var_matches.clone(),
	]))
	{
		return (var_matches.array_get(rt.new_int(1))).str()
	}
	return html
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_EmailEditor_WooContentProcessor) inline_css(woo_content string) string {
	...
	mut var_css := rt.call_method(this.theme_controller, 'get_stylesheet_for_rendering',
		[]rt.PhpVal{})
	var_css = rt.concat(var_css, this.get_woo_content_styles())
	return (rt.call_method(rt.call_method(rt.call_method(this.css_inliner, 'from_html', [
		rt.new_string(woo_content_mutated).clone(),
	]), 'inline_css', [var_css.clone()]), 'render', []rt.PhpVal{})).str()
}
