...
fn Class_Automattic_WooCommerce_Internal_Brands.init() {
	...
	rt.include_file((rt.get_constant('WC_ABSPATH')).str() + 'includes/class-wc-brands-coupons.php',
		'2')
	rt.include_file(
		(rt.get_constant('WC_ABSPATH')).str() + 'includes/class-wc-brands-brand-settings-manager.php',
		'2')
	...
	if rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) {
		rt.include_file(
			(rt.get_constant('WC_ABSPATH')).str() + 'includes/admin/class-wc-admin-brands.php', '2')
	}
}
...
fn Class_Automattic_WooCommerce_Internal_Brands.prepare() {
	...
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('wc_brands_init')])) {
		rt.call_function('remove_action', [rt.new_string('plugins_loaded'),
			rt.new_string('wc_brands_init'), rt.new_int(1)])
	}
}

fn create_automattic_woocommerce_internal_brands(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Brands {
