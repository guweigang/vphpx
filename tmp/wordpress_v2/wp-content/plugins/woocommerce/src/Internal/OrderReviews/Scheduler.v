...
fn (mut this Class_Automattic_WooCommerce_Internal_OrderReviews_Scheduler) init() {
	rt.call_function('add_action', [rt.new_string('woocommerce_order_status_completed'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_OrderReviews_Scheduler',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'handle_woocommerce_order_status_completed' },
		]),
	...
	rt.call_function('add_action', [rt.new_string('woocommerce_order_status_changed'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_OrderReviews_Scheduler',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'handle_status_changed' },
		]),
	...
	rt.call_function('add_action', [rt.new_string('woocommerce_trash_order'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_OrderReviews_Scheduler',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'handle_cancellation' },
		]),
		rt.new_int(10), rt.new_int(1)])
...
fn (mut this Class_Automattic_WooCommerce_Internal_OrderReviews_Scheduler) handle_status_changed(order_id i64, old_status string, new_status string) {
	...
	mut var_was_eligible := rt.call_function('in_array', [rt.new_string(old_status),
		var_eligible_statuses.clone(), rt.new_bool(true)])
	mut var_is_eligible := rt.call_function('in_array', [rt.new_string(new_status),
		var_eligible_statuses.clone(), rt.new_bool(true)])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_was_eligible)))) || rt.is_true(var_is_eligible) {
		return
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_OrderReviews_Scheduler) handle_woocommerce_order_status_completed(order_id i64) {
	...
	if rt.is_true(rt.call_method(var_order, 'get_meta', [
		Class_Automattic_WooCommerce_Internal_OrderReviews_Automattic_WooCommerce_Internal_OrderReviews_Scheduler.scheduled_meta_key(),
	]))
	{
		this.log_skip(order_id, 'already scheduled')
		return
	}
	mut var_should_send := rt.new_bool((rt.call_function('apply_filters', [
		rt.new_string('woocommerce_should_send_review_request'),
		rt.new_bool(true),
		var_order.clone(),
	])).to_bool())
	...
	mut var_when := rt.add(rt.call_function('time', []rt.PhpVal{}), rt.call_method(var_email,
		'get_delay_seconds', []rt.PhpVal{}))
	rt.call_function('as_schedule_single_action', [var_when.clone(),
		Class_Automattic_WooCommerce_Internal_OrderReviews_Automattic_WooCommerce_Internal_OrderReviews_Scheduler.action_hook(),
		rt.create_array([rt.ArrayItem{ key: none, val: order_id }])])
	rt.call_method(var_order, 'update_meta_data', [
		Class_Automattic_WooCommerce_Internal_OrderReviews_Automattic_WooCommerce_Internal_OrderReviews_Scheduler.scheduled_meta_key(),
		rt.new_string(var_when.str()),
	])
...
fn (mut this Class_Automattic_WooCommerce_Internal_OrderReviews_Scheduler) handle_cancellation(order_id i64) {
	...
	mut var_order := rt.call_function('wc_get_order', [rt.new_int(order_id)])
	if rt.is_true(rt.new_bool(rt.instance_of(var_order, 'WC_Order')))
		&& rt.is_true(rt.call_method(var_order, 'get_meta', [Class_Automattic_WooCommerce_Internal_OrderReviews_Automattic_WooCommerce_Internal_OrderReviews_Scheduler.scheduled_meta_key()])) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_OrderReviews_Scheduler) get_email() rt.PhpVal {
	...
	mut var_email := if !(var_emails.array_get(rt.new_string('WC_Email_Customer_Review_Request'))).is_null() {
		var_emails.array_get(rt.new_string('WC_Email_Customer_Review_Request'))
	} else {
		rt.new_null()
	}
	return if rt.is_true(rt.new_bool(rt.instance_of(var_email, 'WC_Email_Customer_Review_Request'))) {
		var_email
	} else {
		rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_OrderReviews_Scheduler) log_skip(order_id i64, reason string) {
	rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'info', [
		rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('Skipped scheduling review-request email for order %1$d: %2$s.'),
		...
		rt.create_array([
			rt.ArrayItem{
				key: 'source'
				val: Class_Automattic_WooCommerce_Internal_OrderReviews_Automattic_WooCommerce_Internal_OrderReviews_Scheduler.log_source()
			},
		]),
	])
}

fn create_automattic_woocommerce_internal_orderreviews_scheduler(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_OrderReviews_Scheduler {
