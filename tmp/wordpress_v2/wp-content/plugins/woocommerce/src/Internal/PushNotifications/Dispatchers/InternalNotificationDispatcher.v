...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_InternalNotificationDispatcher) dispatch(mut var_notifications Class_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_array) {
	...
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_notification := if args.len > 0 { args[0].clone() } else { rt.new_null() }
	...
	mut var_encoded := rt.call_function('array_map',
		[rt.new_closure(closure_1_fn), var_notifications])
	mut var_body := rt.call_function('wp_json_encode', [
		rt.create_array([rt.ArrayItem{ key: 'notifications', val: var_encoded }]),
	])
	if rt.is_true(rt.identical(rt.new_bool(false), var_body)) {
		rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'error', [
			rt.new_string('Failed to JSON-encode push notification payload.'),
			rt.create_array([
				rt.ArrayItem{
					key: 'source'
	...
	mut iife_result_2 := iife_temp_2.create(rt.create_array([
		rt.ArrayItem{ key: 'iss', val: rt.call_function('get_site_url', []rt.PhpVal{}) },
		rt.ArrayItem{
			key: 'exp'
			val: rt.add(rt.call_function('time', []rt.PhpVal{}),
				Class_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_InternalNotificationDispatcher.jwt_expiry_seconds())
		},
		rt.ArrayItem{ key: 'body_hash', val: rt.call_function('hash', [
			rt.new_string('sha256'),
			var_body.clone(),
		]) },
	]), rt.call_function('wp_salt', [rt.new_string('auth')]))
	mut var_token := iife_result_2
	rt.call_function('wp_remote_post', [
		rt.call_function('rest_url', [
			Class_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_InternalNotificationDispatcher.send_endpoint(),
		]),
		rt.create_array([
			...
			rt.ArrayItem{ key: 'headers', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'Authorization', val: 'Bearer ' + var_token.str() },
			]) },
...
fn create_automattic_woocommerce_internal_pushnotifications_dispatchers_internalnotificationdispatcher(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_InternalNotificationDispatcher {
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_InternalNotificationDispatcher) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'dispatch' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
