...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor) register() {
	rt.call_function('add_action', [
		Class_Automattic_WooCommerce_Internal_PushNotifications_Services_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.safety_net_hook(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'handle_safety_net' },
		]),
		rt.new_int(10),
		rt.new_int(2),
	])
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor) process(mut var_notification Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification, is_retry bool) bool {
	...
	if rt.is_true(rt.call_method(var_notification_mutated, 'has_meta', [
		Class_Automattic_WooCommerce_Internal_PushNotifications_Services_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.sent_meta_key(),
	]))
	{
		return true
	}
	if !var_is_retry {
		if rt.is_true(rt.call_method(var_notification_mutated, 'has_meta', [
			Class_Automattic_WooCommerce_Internal_PushNotifications_Services_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.claimed_meta_key(),
		]))
		{
			return true
		}
		rt.call_method(var_notification_mutated, 'write_meta', [
			Class_Automattic_WooCommerce_Internal_PushNotifications_Services_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.claimed_meta_key(),
		])
	}
	mut var_tokens := rt.call_method(this.data_store, 'get_tokens_for_roles', [
		Class_Automattic_WooCommerce_Internal_PushNotifications_PushNotifications.roles_with_push_notifications_enabled(),
	])
	...
	mut var_result := rt.call_method(this.dispatcher, 'dispatch', [
		var_notification_mutated,
		var_tokens.clone(),
	])
	if !(!rt.is_true(var_result.array_get(rt.new_string('success')))) {
		rt.call_method(var_notification_mutated, 'write_meta', [
			Class_Automattic_WooCommerce_Internal_PushNotifications_Services_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.sent_meta_key(),
		])
		rt.call_method(var_notification_mutated, 'delete_meta', [
			Class_Automattic_WooCommerce_Internal_PushNotifications_Services_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor.claimed_meta_key(),
		])
		return true
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor) handle_safety_net(type string, resource_id i64) {
	...
	mut iife_result_0 := iife_temp_0.from_array(rt.create_array([
		rt.ArrayItem{ key: 'type', val: type },
		rt.ArrayItem{ key: 'resource_id', val: resource_id },
	]))
	...
	this.process(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification](var_notification),
		true)
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
	...
	if rt.instance_of(var_e_1, 'Exception') {
		...
		rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'error', [
			rt.call_function('sprintf', [rt.new_string('Safety net failed: %s'),
...
fn create_automattic_woocommerce_internal_pushnotifications_services_notificationprocessor(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor {
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Services_NotificationProcessor) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_PushNotifications_Dispatchers_WpcomNotificationDispatcher](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_PushNotifications_DataStores_PushTokensDataStore](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
