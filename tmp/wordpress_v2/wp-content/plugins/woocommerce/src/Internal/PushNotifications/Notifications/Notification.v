...
pub fn Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification.notification_classes() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{
			key: 'store_order'
			val: Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_NewOrderNotification.class()
		},
		rt.ArrayItem{
			key: 'store_review'
			val: Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_NewReviewNotification.class()
		},
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification) construct(resource_id i64) {
	...
	if resource_id_mutated <= 0 {
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{},
			create_invalidargumentexception(rt.new_string('Notification resource_id must be positive.'))))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification) to_array() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: 'type', val: this.get_type() },
		rt.ArrayItem{ key: 'resource_id', val: this.resource_id }])
}
...
fn Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification.from_array(mut var_data Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_array) rt.PhpVal {
	mut var_type := if !(var_data.array_get(rt.new_string('type'))).is_null() {
		var_data.array_get(rt.new_string('type'))
	} else {
		rt.new_string('')
	}
	mut var_resource_id := rt.new_int((if !(var_data.array_get(rt.new_string('resource_id'))).is_null() {
		var_data.array_get(rt.new_string('resource_id'))
	} else {
		rt.new_int(0)
	}).to_i64())
	mut var_class := if !(Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification.notification_classes().array_get(var_type)).is_null() {
		Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification.notification_classes().array_get(var_type)
	} else {
		rt.new_null()
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_class)))) {
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.call_function('sprintf', [
			rt.new_string('Unknown notification type: %s'),
			var_type.clone(),
		]))))
	}
	return rt.new_object('', []string{}, rt.create_object_dynamically(var_class, [
		var_resource_id.clone(),
	]))
}

fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification) get_identifier() string {
	return (rt.call_function('sprintf', [rt.new_string('%s_%s_%s'),
		rt.call_function('get_current_blog_id', []rt.PhpVal{}),
		rt.new_string(this.get_type()), rt.new_int(this.resource_id)])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'from_array' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_PushNotifications_Notifications_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
...
fn init_registry() {
	rt.register_class_factory('Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification', fn (args []rt.PhpVal) rt.PhpVal {
		...
		obj :=
			create_automattic_woocommerce_internal_pushnotifications_notifications_notification(c_arg_0)
		return rt.new_object('Automattic_WooCommerce_Internal_PushNotifications_Notifications_Notification',
			[]string{}, obj)
	})
