...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_PlatformRegistry) load_platforms() {
	...
	if !(var_platforms.clone().is_array()) {
		return
	}
	...
	for {
		...
		if var_config.array_isset(rt.new_string('fetcher'))
			&& var_config.array_isset(rt.new_string('mapper'))
			&& var_config.array_get(rt.new_string('fetcher')).is_string()
			&& !(!rt.is_true(var_config.array_get(rt.new_string('fetcher'))))
			&& var_config.array_get(rt.new_string('mapper')).is_string()
			&& !(!rt.is_true(var_config.array_get(rt.new_string('mapper')))) {
			this.platforms.array_set(var_platform_id, var_config.clone())
		}
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_PlatformRegistry) get_platform(platform_id string) rt.PhpVal {
	return if !(this.platforms.array_get(rt.new_string(platform_id))).is_null() {
		this.platforms.array_get(rt.new_string(platform_id))
	} else {
		rt.new_null()
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_PlatformRegistry) get_fetcher(platform_id string) rt.PhpVal {
	...
	mut var_fetcher_class := var_platform.array_get(rt.new_string('fetcher'))
	if !(var_fetcher_class.clone().is_string()) || !rt.is_true(var_fetcher_class) {
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.call_function('sprintf', [
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		...
		rt.call_function('class_implements', [var_fetcher_class.clone()]),
		rt.new_bool(true),
	])))))
	{
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.call_function('sprintf', [
	...
	if !(var_credentials.clone().is_array()) {
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.call_function('sprintf', [
			rt.new_string('No credentials found for platform "%s". Please configure credentials using: wp wc migrate setup'),
			rt.call_function('esc_html', [rt.new_string(platform_id)]),
		]))))
	}
	return rt.new_object('', []string{}, rt.create_object_dynamically(var_fetcher_class, [
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_PlatformRegistry) get_mapper(platform_id string, mut var_args Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_array) rt.PhpVal {
	...
	mut var_mapper_class := var_platform.array_get(rt.new_string('mapper'))
	if !(var_mapper_class.clone().is_string()) || !rt.is_true(var_mapper_class) {
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.call_function('sprintf', [
			rt.call_function('esc_html__', [
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		...
		rt.call_function('class_implements', [var_mapper_class.clone()]),
		rt.new_bool(true),
	])))))
	{
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.call_function('sprintf', [
	...
	if !(!rt.is_true(var_args)) {
		return rt.new_object('', []string{}, rt.create_object_dynamically(var_mapper_class, [
			var_args,
		]))
	} else {
		mut var_container := rt.call_function('wc_get_container', []rt.PhpVal{})
		return rt.call_method(var_container, 'get', [var_mapper_class.clone()])
	}
	return rt.new_null()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_PlatformRegistry) resolve_platform(mut var_assoc_args Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_array, default_platform string) string {
	mut var_platform := if !(var_assoc_args.array_get(rt.new_string('platform'))).is_null() {
		var_assoc_args.array_get(rt.new_string('platform'))
	} else {
		rt.new_null()
	}
	if !rt.is_true(var_platform) {
		var_platform = rt.new_string(default_platform)
		mut var_platform_display_name :=
			rt.new_string(this.get_platform_display_name(var_platform.str()))
		mut iife_temp_0 := Class_WP_CLI{}
		mut iife_result_0 :=
			iife_temp_0.log(rt.new_string("Platform not specified, using default: '${var_platform_display_name.to_string()}'."))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(this.get_platform(var_platform.str()))))) {
		...
		if !rt.is_true(var_available_platforms) {
			...
			mut iife_result_1 :=
				iife_temp_1.error(rt.new_string('No platforms are currently registered. Please ensure platform plugins are installed and activated.'))
		} else {
			mut iife_temp_2 := Class_WP_CLI{}
			mut iife_result_2 := iife_temp_2.error(rt.call_function('sprintf', [
				rt.new_string("Platform '%s' is not registered. Available platforms: %s"),
				var_platform.clone(),
				rt.call_function('implode', [rt.new_string(', '),
					var_available_platforms.clone()]),
			]))
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_PlatformRegistry) get_platform_credential_fields(platform_slug string) rt.PhpVal {
	...
	if !(var_platform.clone().is_array()) {
		return rt.new_array()
	}
	mut var_credentials := if !(var_platform.array_get(rt.new_string('credentials'))).is_null() {
		var_platform.array_get(rt.new_string('credentials'))
	} else {
		rt.new_array()
	}
	return if var_credentials.clone().is_array() { var_credentials } else { rt.new_array() }
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_PlatformRegistry) get_platform_display_name(platform_slug string) string {
	mut var_platform := this.get_platform(platform_slug)
	if var_platform.clone().is_array() && var_platform.array_isset(rt.new_string('name')) {
		return (var_platform.array_get(rt.new_string('name'))).str()
	}
