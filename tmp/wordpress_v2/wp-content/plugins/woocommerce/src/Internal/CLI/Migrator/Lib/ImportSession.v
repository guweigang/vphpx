...
pub fn Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.stages_in_order() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.stage_initial()
		},
		rt.ArrayItem{ key: none, val: 'indexing' },
		rt.ArrayItem{ key: none, val: 'preparing' },
		rt.ArrayItem{ key: none, val: 'importing' },
		rt.ArrayItem{ key: none, val: 'finalizing' },
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.stage_finished()
		},
	])
}
...
pub fn Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.progress_entities() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: none, val: 'site_option' },
		rt.ArrayItem{ key: none, val: 'user' }, rt.ArrayItem{ key: none, val: 'category' },
		rt.ArrayItem{ key: none, val: 'tag' }, rt.ArrayItem{ key: none, val: 'term' },
		rt.ArrayItem{ key: none, val: 'post' }, rt.ArrayItem{ key: none, val: 'post_meta' },
		rt.ArrayItem{ key: none, val: 'comment' }, rt.ArrayItem{ key: none, val: 'comment_meta' }])
}
...
fn Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.create(var_args rt.PhpVal) rt.PhpVal {
	...
	mut switch_val_1 := var_args_mutated.array_get(rt.new_string('data_source'))
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('wxr_file'))) {
		if !rt.is_true(var_args_mutated.array_get(rt.new_string('file_name'))) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Exception',
				[]string{},
				create_automattic_woocommerce_internal_cli_migrator_lib_exception(rt.new_string('File name is required for WXR file imports'))))
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('wxr_url'))) {
		if !rt.is_true(var_args_mutated.array_get(rt.new_string('source_url'))) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Exception',
				[]string{},
				create_automattic_woocommerce_internal_cli_migrator_lib_exception(rt.new_string('Source URL is required for remote imports'))))
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('markdown_zip'))) {
		if !rt.is_true(var_args_mutated.array_get(rt.new_string('file_name'))) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Exception',
				[]string{},
				create_automattic_woocommerce_internal_cli_migrator_lib_exception(rt.new_string('File name is required for Markdown ZIP imports'))))
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('local_directory'))) {
		if !rt.is_true(var_args_mutated.array_get(rt.new_string('file_name'))) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Exception',
				[]string{},
				create_automattic_woocommerce_internal_cli_migrator_lib_exception(rt.new_string('Directory path is required for local directory imports'))))
		}
	}
	mut var_post_id := rt.call_function('wp_insert_post', [
		rt.create_array([
			rt.ArrayItem{
				key: 'post_type'
				val: Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.post_type()
			},
			rt.ArrayItem{ key: 'post_status', val: 'publish' },
			rt.ArrayItem{ key: 'post_title', val: rt.call_function('sprintf', [
				rt.new_string('Import from %s - %s'),
				var_args_mutated.array_get(rt.new_string('data_source')),
				if !(var_args_mutated.array_get(rt.new_string('file_name'))).is_null() {
					var_args_mutated.array_get(rt.new_string('file_name'))
				} else {
					if !(var_args_mutated.array_get(rt.new_string('source_url'))).is_null() {
						var_args_mutated.array_get(rt.new_string('source_url'))
					} else {
						rt.new_string('Unknown source')
					}
				},
			]) },
			rt.ArrayItem{ key: 'meta_input', val: rt.create_array([
				rt.ArrayItem{
					key: 'data_source'
					val: var_args_mutated.array_get(rt.new_string('data_source'))
				},
				rt.ArrayItem{ key: 'started_at', val: rt.call_function('time', []rt.PhpVal{}) },
				rt.ArrayItem{
					key: 'file_name'
					val: if !(var_args_mutated.array_get(rt.new_string('file_name'))).is_null() {
						var_args_mutated.array_get(rt.new_string('file_name'))
					} else {
						rt.new_null()
					}
				...
				rt.ArrayItem{
					key: 'source_url'
					val: if !(var_args_mutated.array_get(rt.new_string('source_url'))).is_null() {
						var_args_mutated.array_get(rt.new_string('source_url'))
					} else {
						rt.new_null()
					}
				...
				rt.ArrayItem{
					key: 'attachment_id'
					val: if !(var_args_mutated.array_get(rt.new_string('attachment_id'))).is_null() {
						var_args_mutated.array_get(rt.new_string('attachment_id'))
					} else {
						rt.new_null()
					}
				},
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_post_id.clone()])) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Exception',
			[]string{}, create_automattic_woocommerce_internal_cli_migrator_lib_exception(
			'Error creating an import session: ' +
			(rt.call_method(var_post_id, 'get_error_message', []rt.PhpVal{})).str())))
	}
	if !(!rt.is_true(var_args_mutated.array_get(rt.new_string('attachment_id')))) {
		rt.call_function('wp_update_post', [
			rt.create_array([rt.ArrayItem{ key: 'ID', val: var_post_id },
				rt.ArrayItem{
					key: 'post_parent'
					val: var_args_mutated.array_get(rt.new_string('attachment_id'))
				}]),
		])
	}
	return rt.new_object('Automattic_WooCommerce_Internal_CLI_Migrator_Lib_self', []string{},
		create_automattic_woocommerce_internal_cli_migrator_lib_self(var_post_id.clone()))
}
...
fn Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.by_id(var_post_id rt.PhpVal) bool {
	...
	mut var_post := rt.call_function('get_post', [var_post_id_mutated.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_post))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(var_post, 'post_type'), Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.post_type())))) {
		return false
	}
	return (create_automattic_woocommerce_internal_cli_migrator_lib_self(var_post_id_mutated.clone())).to_bool()
}
...
fn Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.get_active() bool {
	...
	return (create_automattic_woocommerce_internal_cli_migrator_lib_self(rt.get_property(var_posts.array_get(rt.new_int(0)),
		'ID'))).to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) construct(var_post_id rt.PhpVal) {
	mut var_post_id_mutated := var_post_id
	this.post_id = var_post_id_mutated.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_metadata() rt.PhpVal {
	...
	return rt.create_array([rt.ArrayItem{ key: 'post_id', val: this.post_id },
		rt.ArrayItem{
			key: 'cursor'
			val: if rt.is_true(var_cursor) { var_cursor } else { rt.new_null() }
		}, rt.ArrayItem{ key: 'data_source', val: rt.call_function('get_post_meta', [
			this.post_id,
			rt.new_string('data_source'),
			rt.new_bool(true),
		]) }, rt.ArrayItem{ key: 'source_url', val: rt.call_function('get_post_meta', [
			this.post_id,
			rt.new_string('source_url'),
			rt.new_bool(true),
		]) }, rt.ArrayItem{ key: 'attachment_id', val: rt.call_function('get_post_meta', [
			this.post_id,
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_data_source() rt.PhpVal {
	return rt.call_function('get_post_meta', [this.post_id, rt.new_string('data_source'),
		rt.new_bool(true)])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_human_readable_file_reference() string {
	...
	if rt.is_true(rt.equal(switch_val_2, rt.new_string('wxr_file')))
		|| rt.is_true(rt.equal(switch_val_2, rt.new_string('markdown_zip'))) {
		return (rt.call_function('get_post_meta', [this.post_id, rt.new_string('file_name'),
			rt.new_bool(true)])).str()
	} else if rt.is_true(rt.equal(switch_val_2, rt.new_string('wxr_url'))) {
		return (rt.call_function('get_post_meta', [this.post_id, rt.new_string('source_url'),
			rt.new_bool(true)])).str()
	}
	return ''
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) archive() {
	rt.call_function('wp_update_post', [
		rt.create_array([rt.ArrayItem{ key: 'ID', val: this.post_id },
			rt.ArrayItem{ key: 'post_status', val: 'archived' }]),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) count_imported_entities() rt.PhpVal {
	...
	mut iter_1 :=
		Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.progress_entities().iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_entity := item_1.val
		var_progress.array_push(rt.create_array([
			rt.ArrayItem{ key: 'label', val: var_entity },
			rt.ArrayItem{ key: 'imported', val: rt.new_int((rt.call_function('get_post_meta', [
				this.post_id,
				rt.new_string('imported_' + var_entity.str()),
				rt.new_bool(true),
			])).to_i64()) },
			rt.ArrayItem{ key: 'total', val: rt.new_int((rt.call_function('get_post_meta', [
				this.post_id,
				rt.new_string('total_' + var_entity.str()),
				rt.new_bool(true),
			])).to_i64()) },
		]))
	}
	return var_progress.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) count_all_imported_entities() rt.PhpVal {
	...
	return rt.call_function('array_sum', [
		rt.call_function('array_column', [var_counts.clone(),
			rt.new_string('imported')]),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) count_remaining_entities() rt.PhpVal {
	mut var_counts := this.count_imported_entities()
	return rt.sub(rt.call_function('array_sum', [
		rt.call_function('array_column', [var_counts.clone(),
			rt.new_string('total')]),
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) bump_imported_entities_counts(var_newly_imported_entities rt.PhpVal) {
	mut iter_2 := var_newly_imported_entities.iterator()
	for {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
			var_field.clone(),
			Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_static.progress_entities(),
			rt.new_bool(true)])))))
		{
			rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
				rt.new_string('Cannot bump imported entities count for unknown entity type: ' +
					var_field.str()),
				rt.new_string('1.0.0')])
			continue
		}
		if !(this.cached_imported_counts.array_isset(var_field)) {
			this.cached_imported_counts.array_set(var_field, rt.new_int((rt.call_function('get_post_meta', [
				this.post_id,
				rt.new_string('imported_' + var_field.str()),
				rt.new_bool(true),
			])).to_i64()))
		}
		mut var_new_count := rt.add(this.cached_imported_counts.array_get(var_field), var_count)
		rt.call_function('update_post_meta', [this.post_id,
			rt.new_string('imported_' + var_field.str()), var_new_count.clone()])
		this.cached_imported_counts.array_set(var_field, var_new_count.clone())
	}
}

fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) count_awaiting_frontloading_stubs() i64 {
	...
	return rt.new_int((rt.call_method(var_wpdb, 'get_var', [
		rt.call_method(var_wpdb, 'prepare', [
			...
			this.post_id,
			Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.frontload_status_awaiting_download(),
		]),
	])).to_i64())
}

fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) count_unfinished_frontloading_stubs() i64 {
	mut var_wpdb := rt.new_null()
	return rt.new_int((rt.call_method(var_wpdb, 'get_var', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.concat(rt.concat(rt.new_string('SELECT COUNT(*) FROM '), rt.get_property(var_wpdb,
				...
				rt.new_string("\n\t\t\t\t WHERE post_type = 'frontloading_stub'\n\t\t\t\t AND post_parent = %d\n\t\t\t\t AND post_status != %s\n\t\t\t\t AND post_status != %s")),
			this.post_id,
			Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.frontload_status_succeeded(),
			Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.frontload_status_ignored(),
		]),
	])).to_i64())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) mark_frontloading_errors_as_ignored() {
	...
	rt.call_method(var_wpdb, 'update', [rt.get_property(var_wpdb, 'posts'),
		rt.create_array([
			rt.ArrayItem{
				key: 'post_status'
				val: Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.frontload_status_ignored()
			},
		]),
		rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_frontloading_stubs(var_options rt.PhpVal) rt.PhpVal {
	mut var_query := create_wp_query(rt.create_array([
		...
		rt.ArrayItem{ key: 'post_parent', val: this.post_id },
		rt.ArrayItem{
			key: 'posts_per_page'
			val: if !(var_options.array_get(rt.new_string('per_page'))).is_null() {
				var_options.array_get(rt.new_string('per_page'))
			} else {
				rt.new_int(25)
			}
		},
		rt.ArrayItem{
			key: 'paged'
			val: if !(var_options.array_get(rt.new_string('page'))).is_null() {
				var_options.array_get(rt.new_string('page'))
			} else {
				rt.new_int(1)
			}
		},
		rt.ArrayItem{ key: 'orderby', val: rt.create_array([
			rt.ArrayItem{ key: 'post_status', val: rt.create_array([
				rt.ArrayItem{
					key: Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.frontload_status_error()
					val: 0
				},
				rt.ArrayItem{
					key: Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.frontload_status_awaiting_download()
					val: 1
				},
				rt.ArrayItem{ key: 'any', val: 2 },
			]) },
	...
	mut var_ids := rt.call_function('array_map', [rt.new_closure(closure_1_fn),
		var_posts.clone()])
	rt.call_function('update_meta_cache', [rt.new_string('post'),
		var_ids.clone()])
	mut iter_3 := var_posts.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_post := item_3.val
		rt.set_property(var_post, 'meta', rt.call_function('get_all_post_meta_flat', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_total_number_of_entities() rt.PhpVal {
	...
	mut iter_4 :=
		Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_static.progress_entities().iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_field := item_4.val
		var_totals.array_set(var_field, rt.new_int((rt.call_function('get_post_meta', [
			this.post_id,
			rt.new_string('total_' + var_field.str()),
			rt.new_bool(true),
		])).to_i64()))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_total_number_of_assets() i64 {
	mut var_wpdb := rt.new_null()
	return rt.new_int((rt.call_method(var_wpdb, 'get_var', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.concat(rt.concat(rt.new_string('SELECT COUNT(*) FROM '), rt.get_property(var_wpdb,
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) create_frontloading_stubs(var_urls rt.PhpVal) {
	mut var_wpdb := rt.new_null()
	mut iter_5 := var_urls.iterator()
	for {
		item_5 := iter_5.next() or { break }
		mut var__ := item_5.val
		mut var_url := item_5.key
		mut var_exists := rt.call_method(var_wpdb, 'get_var', [
			rt.call_method(var_wpdb, 'prepare', [
		...
		mut var_post_data := rt.create_array([
			rt.ArrayItem{ key: 'post_type', val: 'frontloading_stub' },
			rt.ArrayItem{ key: 'post_parent', val: this.post_id },
			rt.ArrayItem{ key: 'post_title', val: rt.call_function('basename', [
				var_url.clone(),
			]) },
			rt.ArrayItem{
				key: 'post_status'
				val: Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.frontload_status_awaiting_download()
			},
			rt.ArrayItem{ key: 'guid', val: var_url },
			rt.ArrayItem{ key: 'meta_input', val: rt.create_array([
				rt.ArrayItem{ key: 'original_url', val: var_url },
				rt.ArrayItem{ key: 'current_url', val: var_url },
				rt.ArrayItem{ key: 'attempts', val: 0 },
				rt.ArrayItem{ key: 'last_error', val: rt.new_null() },
				rt.ArrayItem{ key: 'target_path', val: '' },
			]) },
		])
		mut var_insertion_result := rt.call_function('wp_insert_post', [
			var_post_data.clone()])
		if rt.is_true(rt.call_function('is_wp_error', [var_insertion_result.clone()])) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Exception',
				[]string{},
				create_automattic_woocommerce_internal_cli_migrator_lib_exception(rt.new_string('Failed to insert frontloading placeholder'))))
		}
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) bump_total_number_of_entities(var_newly_indexed_entities rt.PhpVal) {
	mut iter_6 := var_newly_indexed_entities.iterator()
	for {
		...
		{
			rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
				rt.new_string('Cannot set total number of entities for unknown entity type: ' +
					var_field.str()),
				rt.new_string('1.0.0')])
			continue
		}
		if !(this.cached_totals.array_isset(var_field)) {
			this.cached_totals.array_set(var_field, rt.new_int((rt.call_function('get_post_meta', [
				this.post_id,
				rt.new_string('total_' + var_field.str()),
				rt.new_bool(true),
			])).to_i64()))
		...
		rt.call_function('update_post_meta', [this.post_id, rt.new_string('total_' + var_field.str()),
			var_new_total.clone()])
		this.cached_totals.array_set(var_field, var_new_total.clone())
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) bump_frontloading_progress(var_frontloading_progress rt.PhpVal, var_events rt.PhpVal) {
	rt.call_function('update_post_meta', [this.post_id, rt.new_string('frontloading_progress'),
		var_frontloading_progress.clone()])
	mut iter_7 := var_events.iterator()
	for {
		item_7 := iter_7.next() or { break }
		mut var_event := item_7.val
		mut var_url := rt.get_property(var_event, 'resource_id')
		mut var_placeholder := this.get_frontloading_stub(var_url.clone())
		if rt.is_true(rt.new_bool(!(rt.is_true(var_placeholder)))) {
			rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
				rt.new_string('Frontloading placeholder post not found for URL: ' + var_url.str()),
				rt.new_string('1.0.0')])
			continue
		}
		rt.call_function('update_post_meta', [rt.get_property(var_placeholder, 'ID'),
			rt.new_string('last_error'), rt.get_property(var_event, 'error')])
		mut var_attempts := rt.call_function('get_post_meta', [
			rt.get_property(var_placeholder, 'ID'),
			rt.new_string('attempts'),
			rt.new_bool(true),
		])
		mut var_new_attempts := var_attempts.clone()
		mut var_new_status := rt.get_property(var_placeholder, 'post_status')
		mut switch_val_3 := rt.get_property(var_event, 'type')
		if rt.is_true(rt.equal(switch_val_3,
			Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.event_success()))
		{
			var_new_status =
				Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.frontload_status_succeeded()
			var_new_attempts = rt.add(var_attempts, rt.new_int(1))
		} else if rt.is_true(rt.equal(switch_val_3,
			Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.event_already_exists()))
		{
			var_new_status =
				Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.frontload_status_succeeded()
		} else if rt.is_true(rt.equal(switch_val_3,
			Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.event_failure()))
		{
			var_new_status =
				Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.frontload_status_error()
			var_new_attempts = rt.add(var_attempts, rt.new_int(1))
		}
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_new_attempts, var_attempts)))) {
			rt.call_function('update_post_meta', [rt.get_property(var_placeholder, 'ID'),
				rt.new_string('attempts'), var_new_attempts.clone()])
		}
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_new_status, rt.get_property(var_placeholder,
			'post_status')))))
		{
			rt.call_function('wp_update_post', [
				rt.create_array([
					rt.ArrayItem{ key: 'ID', val: rt.get_property(var_placeholder, 'ID') },
					rt.ArrayItem{ key: 'post_status', val: var_new_status },
				]),
			])
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_frontloading_progress() rt.PhpVal {
	mut var_meta := rt.call_function('get_post_meta', [this.post_id,
		rt.new_string('frontloading_progress'), rt.new_bool(true)])
	return if rt.is_true(var_meta) { var_meta } else { rt.new_array() }
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) is_stage_completed(var_stage rt.PhpVal) rt.PhpVal {
	mut var_current_stage := this.get_stage()
	mut var_stage_index := rt.call_function('array_search', [
		var_stage.clone(),
		Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.stages_in_order(),
		rt.new_bool(true)])
	mut var_current_stage_index := rt.call_function('array_search', [
		var_current_stage.clone(),
		Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.stages_in_order(),
		rt.new_bool(true)])
	return rt.greater(var_current_stage_index, var_stage_index)
}

fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_stage() rt.PhpVal {
	if !(!(this.cached_stage).is_null()) {
		mut var_meta := rt.call_function('get_post_meta', [this.post_id, rt.new_string('current_stage'),
			rt.new_bool(true)])
		this.cached_stage = if rt.is_true(var_meta) {
			var_meta
		} else {
			Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.stage_initial()
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) set_stage(var_stage rt.PhpVal) {
	if rt.is_true(rt.identical(var_stage, this.get_stage())) {
		return
	}
	if rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession.stage_finished(),
		var_stage))
	{
		rt.call_function('update_post_meta', [this.post_id, rt.new_string('finished_at'),
			rt.call_function('time', []rt.PhpVal{})])
	}
	rt.call_function('update_post_meta', [this.post_id, rt.new_string('current_stage'),
		var_stage.clone()])
	this.cached_stage = var_stage.clone()
}

fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_started_at() rt.PhpVal {
	return rt.call_function('get_post_meta', [this.post_id, rt.new_string('started_at'),
		rt.new_bool(true)])
}

fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_finished_at() rt.PhpVal {
	return rt.call_function('get_post_meta', [this.post_id, rt.new_string('finished_at'),
		rt.new_bool(true)])
}

fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) is_finished() bool {
	return !(!rt.is_true(rt.call_function('get_post_meta', [this.post_id, rt.new_string('finished_at'),
		rt.new_bool(true)])))
}

fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_reentrancy_cursor() rt.PhpVal {
	return rt.call_function('get_post_meta', [this.post_id, rt.new_string('importer_cursor'),
		rt.new_bool(true)])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) set_reentrancy_cursor(var_cursor rt.PhpVal) {
	...
	rt.call_function('update_post_meta', [this.post_id, rt.new_string('importer_cursor'),
		rt.call_function('addslashes', [var_cursor_mutated.clone()])])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) set_original_arguments(mut var_args Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_array) {
	mut var_args_mutated := var_args
	rt.call_function('update_post_meta', [this.post_id, rt.new_string('original_arguments'),
		var_args_mutated])
}

fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) get_original_arguments() rt.PhpVal {
	mut var_args := rt.call_function('get_post_meta', [this.post_id, rt.new_string('original_arguments'),
		rt.new_bool(true)])
	return if var_args.clone().is_array() && !(!rt.is_true(var_args)) {
		var_args
	} else {
		rt.new_null()
	}
...
fn create_automattic_woocommerce_internal_cli_migrator_lib_exception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_Exception {
...
fn create_automattic_woocommerce_internal_cli_migrator_lib_self(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_self {
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'count_awaiting_frontloading_stubs' {
			return rt.new_int(this.count_awaiting_frontloading_stubs())
		}
		'count_unfinished_frontloading_stubs' {
			return rt.new_int(this.count_unfinished_frontloading_stubs())
		}
		...
		'get_total_number_of_assets' {
			return rt.new_int(this.get_total_number_of_assets())
		}
		...
		'set_original_arguments' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Lib_ImportSession) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'cached_stage' {
			this.cached_stage = val
			return true
		}
		'cached_imported_counts' {
			this.cached_imported_counts = val
			return true
		}
		'cached_totals' {
			this.cached_totals = val
			return true
		}
		else {
