...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_ProductsCommand) init(mut var_credential_manager Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_CredentialManager, mut var_platform_registry Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_PlatformRegistry, mut var_products_controller Class_Automattic_WooCommerce_Internal_CLI_Migrator_Core_ProductsController) {
	...
	this.products_controller = var_products_controller
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_ProductsCommand) magic_invoke(mut var_args Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_array, mut var_assoc_args Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_array) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.credential_manager,
		'has_credentials', [var_platform.clone()])))))
	{
		mut iife_temp_0 := Class_WP_CLI{}
		mut iife_result_0 :=
			iife_temp_0.log(rt.new_string("Credentials for '${var_platform_display_name.to_string()}' not found. Let's set them up."))
		...
		if !rt.is_true(var_required_fields) {
			...
			mut iife_result_1 :=
				iife_temp_1.error(rt.new_string("The platform '${var_platform_display_name.to_string()}' does not have configured credential fields."))
			return
		}
		...
		mut iife_result_2 :=
			iife_temp_2.success(rt.new_string('Credentials saved successfully. Please run the command again to begin the migration.'))
		return
	}
	...
	rt.call_method(this.products_controller, 'migrate_products',
		[var_assoc_args, var_platform.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_ProductsCommand) handle_count_request(platform string, platform_display_name string, mut var_assoc_args Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_array) {
	...
	mut iife_result_3 :=
		iife_temp_3.log(rt.new_string('Fetching product count from ${var_platform_display_name.to_string()}...'))
	mut var_fetcher := rt.call_method(this.platform_registry, 'get_fetcher', [
		rt.new_string(platform_mutated).clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_fetcher)))) {
		mut iife_temp_4 := Class_WP_CLI{}
		mut iife_result_4 :=
			iife_temp_4.error(rt.new_string("Could not get fetcher for platform '${var_platform_display_name.to_string()}'"))
		return
	}
	mut var_filter_args := rt.new_array()
	if var_assoc_args.array_isset(rt.new_string('status')) {
		var_filter_args.array_set('status', var_assoc_args.array_get(rt.new_string('status')))
	}
	if var_assoc_args.array_isset(rt.new_string('product-type')) {
		var_filter_args.array_set('product_type',
			var_assoc_args.array_get(rt.new_string('product-type')))
	}
	if var_assoc_args.array_isset(rt.new_string('vendor')) {
		var_filter_args.array_set('vendor', var_assoc_args.array_get(rt.new_string('vendor')))
	}
	if var_assoc_args.array_isset(rt.new_string('ids')) {
		var_filter_args.array_set('ids', var_assoc_args.array_get(rt.new_string('ids')))
	}
	mut var_count := rt.call_method(var_fetcher, 'fetch_total_count', [
		var_filter_args.clone()])
	if rt.is_true(rt.identical(rt.new_int(0), var_count)) {
		mut iife_temp_5 := Class_WP_CLI{}
		mut iife_result_5 :=
			iife_temp_5.log(rt.new_string('No products found or unable to fetch count.'))
	} else {
		mut var_filters := rt.new_array()
		if var_assoc_args.array_isset(rt.new_string('status')) {
			var_filters.array_push(rt.concat(rt.concat(rt.new_string("status '"),
				var_assoc_args.array_get(rt.new_string('status'))), rt.new_string("'")))
		}
		if var_assoc_args.array_isset(rt.new_string('product-type')) {
			var_filters.array_push(rt.concat(rt.concat(rt.new_string("type '"),
				var_assoc_args.array_get(rt.new_string('product-type'))), rt.new_string("'")))
		}
		if var_assoc_args.array_isset(rt.new_string('vendor')) {
			var_filters.array_push(rt.concat(rt.concat(rt.new_string("vendor '"),
				var_assoc_args.array_get(rt.new_string('vendor'))), rt.new_string("'")))
		}
		if var_assoc_args.array_isset(rt.new_string('ids')) {
			var_filters.array_push(rt.concat(rt.concat(rt.new_string("IDs '"),
				var_assoc_args.array_get(rt.new_string('ids'))), rt.new_string("'")))
		}
		mut var_filter_description := rt.new_string((if !rt.is_true(var_filters) {
			''
		} else {
			' with ' +
				(rt.call_function('implode', [rt.new_string(', '), var_filters.clone()])).str()
		}).str())
		mut iife_temp_6 := Class_WP_CLI{}
		mut iife_result_6 :=
			iife_temp_6.success(rt.new_string('Found ${var_count.to_string()} products${var_filter_description.to_string()} on ${var_platform_display_name.to_string()}.'))
	}
}
...
fn create_automattic_woocommerce_internal_cli_migrator_commands_productscommand(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_CLI_Migrator_Commands_ProductsCommand {
