...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) construct(mut var_query Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableQuery) {
	this.query = var_query
	this.search_term = var_query.get(rt.new_string('s'))
	this.search_filters = this.sanitize_search_filters((if !(var_query.get(rt.new_string('search_filter'))).is_null() {
		var_query.get(rt.new_string('search_filter'))
	} else {
		rt.new_string('')
	}).str())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) sanitize_search_filters(search_filter string) rt.PhpVal {
	mut var_core_filters := rt.create_array([rt.ArrayItem{ key: none, val: 'order_id' },
		rt.ArrayItem{ key: none, val: 'transaction_id' }, rt.ArrayItem{
			key: none
			val: 'customer_email'
		}, rt.ArrayItem{ key: none, val: 'customers' }, rt.ArrayItem{ key: none, val: 'products' }])
	if rt.is_true(rt.identical(rt.new_string('all'), rt.new_string(search_filter)))
		|| rt.is_true(rt.identical(rt.new_string(''), rt.new_string(search_filter))) {
		return var_core_filters.clone()
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) get_sql_clauses() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'join', val: rt.create_array([
			rt.ArrayItem{ key: none, val: this.generate_join() },
		]) },
		rt.ArrayItem{ key: 'where', val: rt.create_array([
			rt.ArrayItem{ key: none, val: this.generate_where() },
		]) },
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) generate_join() string {
	...
	for {
		...
		var_join.array_push(this.generate_join_for_search_filter(var_search_filter.clone()))
	}
	return (rt.call_function('implode', [rt.new_string(' '), var_join.clone()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) generate_join_for_search_filter(var_search_filter rt.PhpVal) string {
	...
	return (rt.call_function('apply_filters', [
		rt.new_string('woocommerce_hpos_generate_join_for_search_filter'),
		var_join.clone(),
		this.search_term,
		var_search_filter.clone(),
		this.query,
	])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) maybe_get_join_for_products() string {
	...
	mut var_items_table := rt.call_method(this.query, 'get_table_name', [
		rt.new_string('items'),
	])
	mut var_orders_table := rt.call_method(this.query, 'get_table_name', [
		rt.new_string('orders'),
	])
	mut var_fts_enabled := rt.new_bool(
		rt.is_true(rt.identical(rt.call_function('get_option', [Class_Automattic_WooCommerce_Internal_DataStores_Orders_CustomOrdersTableController.hpos_fts_index_option()]), rt.new_string('yes')))
		&& rt.is_true(rt.identical(rt.call_function('get_option', [Class_Automattic_WooCommerce_Internal_DataStores_Orders_CustomOrdersTableController.hpos_fts_order_item_index_created_option()]), rt.new_string('yes'))))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_fts_enabled)))) {
		return ''
	}
	mut var_search_pattern := rt.call_method(var_wpdb, 'esc_like', [
		rt.call_method(var_db_util, 'sanitise_boolean_fts_search_term', [this.search_term]),
	])
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) maybe_get_join_for_customers() string {
	...
	mut var_address_table := rt.call_method(this.query, 'get_table_name', [
	...
	mut var_fts_enabled := rt.new_bool(
		rt.is_true(rt.identical(rt.call_function('get_option', [Class_Automattic_WooCommerce_Internal_DataStores_Orders_CustomOrdersTableController.hpos_fts_index_option()]), rt.new_string('yes')))
		&& rt.is_true(rt.identical(rt.call_function('get_option', [Class_Automattic_WooCommerce_Internal_DataStores_Orders_CustomOrdersTableController.hpos_fts_address_index_created_option()]), rt.new_string('yes'))))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_fts_enabled)))) {
		return ''
	}
	mut var_search_pattern := rt.call_method(var_wpdb, 'esc_like', [
	...
	mut var_maybe_phone_field := rt.new_string('')
	...
	{
		var_maybe_phone_field = rt.new_string(', phone')
	}
	return (rt.call_method(var_wpdb, 'prepare', [
		rt.new_string(
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) generate_where() string {
	...
	mut var_possible_order_id := rt.new_string((rt.call_function('absint', [
		this.search_term,
	])).str())
	mut var_order_table := rt.call_method(this.query, 'get_table_name', [
		rt.new_string('orders'),
	])
	if rt.is_true(rt.identical((rt.call_method(this.query, 'get', [
		rt.new_string('s')])).str(), var_possible_order_id))
	{
		var_where.array_push('`${var_order_table.to_string()}`.id = ${var_possible_order_id.to_string()}')
	}
	mut iter_2 := this.search_filters.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_search_filter := item_2.val
		mut var_search_where :=
			rt.new_string(this.generate_where_for_search_filter(var_search_filter.str()).trim_space())
		if var_search_where.clone().to_string().len > 0 {
			var_where.array_push(var_search_where.clone())
		}
	}
	mut var_where_statement := rt.call_function('implode', [rt.new_string(' OR '),
		var_where.clone()])
	return if var_where_statement.clone().to_string().len > 0 {
		' ( ${var_where_statement.to_string()} ) '
	} else {
		''
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) generate_where_for_search_filter(search_filter string) string {
	...
	if rt.is_true(rt.identical(rt.new_string('customer_email'), rt.new_string(search_filter))) {
		return (rt.call_method(var_wpdb, 'prepare', [
			rt.new_string('`${var_order_table.to_string()}`.billing_email LIKE %s'),
			rt.new_string((rt.call_method(var_wpdb, 'esc_like', [this.search_term])).str() + '%'),
		])).str()
	}
	if rt.is_true(rt.identical(rt.new_string('order_id'), rt.new_string(search_filter)))
		&& this.search_term.is_long() || this.search_term.is_double() {
		return (rt.call_method(var_wpdb, 'prepare', [
			rt.new_string('`${var_order_table.to_string()}`.id = %d'),
			rt.call_function('absint', [this.search_term]),
		])).str()
	}
	if rt.is_true(rt.identical(rt.new_string('transaction_id'), rt.new_string(search_filter))) {
		return (rt.call_method(var_wpdb, 'prepare', [
			rt.new_string('`${var_order_table.to_string()}`.transaction_id LIKE %s'),
			rt.new_string('%' + (rt.call_method(var_wpdb, 'esc_like', [this.search_term])).str() +
				'%'),
		])).str()
	}
	...
	return (rt.call_function('apply_filters', [
		rt.new_string('woocommerce_hpos_generate_where_for_search_filter'),
		rt.new_string(''),
		this.search_term,
		rt.new_string(search_filter),
		this.query,
	])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) get_where_for_products() string {
	...
	if rt.is_true(var_fts_enabled) {
		return
			(Class_Automattic_WooCommerce_Internal_DataStores_Orders_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery.products_join_alias()).str() + '.order_id IS NOT NULL'
	}
	return (rt.call_method(var_wpdb, 'prepare', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) get_where_for_customers() string {
	...
	if rt.is_true(var_fts_enabled) {
		return
			(Class_Automattic_WooCommerce_Internal_DataStores_Orders_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery.customers_join_alias()).str() + '.order_id IS NOT NULL'
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) generate_where_for_meta_table() string {
	mut var_wpdb := rt.new_null()
	mut var_meta_table := rt.call_method(this.query, 'get_table_name', [
		rt.new_string('meta'),
	])
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableSearchQuery) get_meta_fields_to_be_searched() string {
	mut var_meta_fields_to_search := rt.create_array([
		rt.ArrayItem{ key: none, val: '_billing_address_index' },
		rt.ArrayItem{ key: none, val: '_shipping_address_index' },
	])
	mut var_meta_keys := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_order_table_search_query_meta_keys'),
		var_meta_fields_to_search.clone(),
	])
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_meta_key := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return "'" +
			(rt.call_function('esc_sql', [rt.call_function('wc_clean', [var_meta_key.clone()])])).str() +
			"'"
	}
	...
	var_meta_keys = rt.cast_array(rt.call_function('array_map', [
		rt.new_closure(closure_1_fn),
		var_meta_keys.clone(),
	]))
	return (rt.call_function('implode', [rt.new_string(','), var_meta_keys.clone()])).str()
}
