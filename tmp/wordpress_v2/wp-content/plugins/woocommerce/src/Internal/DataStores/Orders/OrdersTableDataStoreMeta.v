...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStoreMeta) get_table_name() rt.PhpVal {
	...
	mut iife_result_0 := iife_temp_0.get_meta_table_name()
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStoreMeta) delete_meta(var_object rt.PhpVal, var_meta rt.PhpVal) bool {
	mut var_successful := this.Class_Automattic_WooCommerce_Internal_DataStores_CustomMetaDataStore.delete_meta(var_object.clone(),
		var_meta.clone())
	if rt.is_true(var_successful) {
		this.clear_cached_data(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_DataStores_Orders_array](rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStoreMeta) add_meta(var_object rt.PhpVal, var_meta rt.PhpVal) rt.PhpVal {
	mut var_insert_id := this.Class_Automattic_WooCommerce_Internal_DataStores_CustomMetaDataStore.add_meta(var_object.clone(),
		var_meta.clone())
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_insert_id)))) {
		this.clear_cached_data(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_DataStores_Orders_array](rt.create_array([
			rt.ArrayItem{ key: none, val: rt.call_method(var_object, 'get_id', []rt.PhpVal{}) },
		])))
	}
	return var_insert_id.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStoreMeta) update_meta(var_object rt.PhpVal, var_meta rt.PhpVal) bool {
	mut var_is_successful := this.Class_Automattic_WooCommerce_Internal_DataStores_CustomMetaDataStore.update_meta(var_object.clone(),
		var_meta.clone())
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStoreMeta) get_meta_data_for_object_ids(mut var_object_ids Class_Automattic_WooCommerce_Internal_DataStores_Orders_array) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(iife_result_1)))) {
		return this.Class_Automattic_WooCommerce_Internal_DataStores_CustomMetaDataStore.get_meta_data_for_object_ids(rt.new_object('Automattic_WooCommerce_Internal_DataStores_Orders_array',
			[]string{}, var_object_ids_mutated))
	}
	mut var_meta_data := this.get_meta_data_for_object_ids_from_cache(mut var_object_ids_mutated)
	var_object_ids_mutated = rt.call_function('array_diff', [var_object_ids_mutated,
		rt.func_array_keys(var_meta_data.clone())])
	if !rt.is_true(var_object_ids_mutated) {
		return var_meta_data.clone()
	}
	mut var_db_meta_data := this.Class_Automattic_WooCommerce_Internal_DataStores_CustomMetaDataStore.get_meta_data_for_object_ids(rt.new_object('Automattic_WooCommerce_Internal_DataStores_Orders_array',
		[]string{}, var_object_ids_mutated))
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStoreMeta) get_meta_data_for_object_ids_from_cache(mut var_object_ids Class_Automattic_WooCommerce_Internal_DataStores_Orders_array) rt.PhpVal {
	mut var_object_ids_mutated := var_object_ids
	mut var_cache_engine := rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}),
		'get', [Class_Automattic_WooCommerce_Caching_WPCacheEngine.class()])
	mut var_meta_data := rt.call_method(var_cache_engine, 'get_cached_objects', [
		var_object_ids_mutated,
		rt.new_string(this.get_cache_group()),
	])
	return rt.call_function('array_filter', [var_meta_data.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStoreMeta) set_meta_data_for_objects_in_cache(mut var_meta_data Class_Automattic_WooCommerce_Internal_DataStores_Orders_array) {
	...
	mut var_cache_engine := rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}),
		'get', [Class_Automattic_WooCommerce_Caching_WPCacheEngine.class()])
	rt.call_method(var_cache_engine, 'cache_objects', [var_meta_data_mutated, rt.new_int(0),
		rt.new_string(this.get_cache_group())])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStoreMeta) clear_cached_data(mut var_object_ids Class_Automattic_WooCommerce_Internal_DataStores_Orders_array) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(iife_result_2)))) {
		return rt.call_function('array_fill_keys', [var_object_ids_mutated, rt.new_bool(true)])
	}
	mut var_cache_engine := rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}),
		'get', [Class_Automattic_WooCommerce_Caching_WPCacheEngine.class()])
	...
	for {
		...
		var_return_values.array_set(var_object_id, rt.call_method(var_cache_engine,
			'delete_cached_object', [var_object_id.clone(), rt.new_string(this.get_cache_group())]))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStoreMeta) clear_all_cached_data() bool {
	...
	mut iife_result_3 := iife_temp_3.custom_orders_table_datastore_cache_enabled()
	if rt.is_true(rt.new_bool(!(rt.is_true(iife_result_3)))) {
		return true
	}
	mut var_cache_engine := rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}),
		'get', [Class_Automattic_WooCommerce_Caching_WPCacheEngine.class()])
	return (rt.call_method(var_cache_engine, 'delete_cache_group', [
		rt.new_string(this.get_cache_group()),
	])).to_bool()
}
...
fn create_automattic_woocommerce_internal_datastores_orders_orderstabledatastoremeta(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_DataStores_Orders_OrdersTableDataStoreMeta {
