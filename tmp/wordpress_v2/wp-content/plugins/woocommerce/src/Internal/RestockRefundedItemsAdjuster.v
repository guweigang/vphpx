...
fn (mut this Class_Automattic_WooCommerce_Internal_RestockRefundedItemsAdjuster) initialize_restock_refunded_items(var_order_id rt.PhpVal, var_items rt.PhpVal) {
	...
	if rt.is_true(rt.call_function('version_compare', [var_order_version.clone(), rt.new_string('5.5'), rt.new_string('>=')])) {
		return
	}
	...
	if var_items.array_isset(rt.new_string('order_item_id')) {
		mut iter_1 := var_items.array_get(rt.new_string('order_item_id')).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_item_id := item_1.val
			mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_{"nodeType":"Expr_PropertyFetch","line":57,"var":{"nodeType":"Expr_Variable","line":57,"name":"this"},"name":"order_factory"}{}
			mut iife_result_0 := iife_temp_0.get_order_item(rt.call_function('absint', [var_item_id.clone()]))
			mut var_item := iife_result_0
			if rt.is_true(rt.new_bool(!(rt.is_true(var_item)))) {
				continue
			}
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Enums_OrderItemType.line_item(), rt.call_method(var_item, 'get_type', []rt.PhpVal{}))))) {
				continue
			}
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), rt.call_method(var_item, 'get_meta', [rt.new_string('_restock_refunded_items'), rt.new_bool(true)]))))) {
				continue
			}
			...
			rt.call_method(var_item, 'add_meta_data', [rt.new_string('_restock_refunded_items'), var_refunded_item_quantity.clone(), rt.new_bool(false)])
...
fn create_automattic_woocommerce_internal_restockrefundeditemsadjuster(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestockRefundedItemsAdjuster {
...
fn create_automattic_woocommerce_internal_{"nodetype":"expr_propertyfetch","line":57,"var":{"nodetype":"expr_variable","line":57,"name":"this"},"name":"order_factory"}(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_{"nodeType":"Expr_PropertyFetch","line":57,"var":{"nodeType":"Expr_Variable","line":57,"name":"this"},"name":"order_factory"} {
