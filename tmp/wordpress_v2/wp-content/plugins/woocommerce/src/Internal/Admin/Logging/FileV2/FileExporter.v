...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_FileExporter) emit_file() rt.PhpVal {
	...
	mut iife_result_0 := iife_temp_0.get_wp_filesystem()
	mut var_filesystem := iife_result_0
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	}
	mut var_is_readable := rt.new_bool(
		rt.is_true(rt.call_method(var_filesystem, 'is_file', [rt.new_string(this.path)]))
		&& rt.is_true(rt.call_method(var_filesystem, 'is_readable', [rt.new_string(this.path)])))
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	if rt.instance_of(var_e_1, 'Exception') {
		...
		var_is_readable = rt.new_bool(false)
		unsafe {
			goto end_label_1
		}
	} else {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_is_readable)))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('wc_logs_invalid_file'), rt.call_function('__', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_FileExporter) send_headers() {
	...
	rt.call_function('header', [
		rt.new_string('Content-Disposition: attachment; filename=' + this.get_filename()),
	])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_FileExporter) send_contents() {
	mut var_stream := rt.call_function('fopen', [rt.new_string(this.path),
		rt.new_string('rb')])
	for rt.is_true(rt.call_function('is_resource', [var_stream.clone()]))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('feof', [var_stream.clone()]))))) {
		mut var_chunk := rt.call_function('fread', [var_stream.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Logging_FileV2_FileExporter) get_filename() string {
	...
	return (rt.call_function('basename', [rt.new_string(this.path)])).str()
}
