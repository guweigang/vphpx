...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder) build_payload(event string, mut var_order Class_WC_Order) rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: 'type', val: event },
		rt.ArrayItem{ key: 'data', val: this.build_order_data(mut var_order) }])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder) build_order_data(mut var_order Class_WC_Order) rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: 'type', val: 'order' },
		rt.ArrayItem{
			key: 'checkout_session_id'
			val: var_order.get_meta(Class_Automattic_WooCommerce_StoreApi_Routes_V1_Agentic_Enums_OrderMetaKey.agentic_checkout_session_id())
		}, rt.ArrayItem{ key: 'permalink_url', val: var_order.get_checkout_order_received_url() },
		rt.ArrayItem{ key: 'status', val: this.map_order_status((var_order.get_status()).str()) },
		rt.ArrayItem{ key: 'refunds', val: this.build_refunds_data(mut var_order) }])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder) map_order_status(wc_status string) string {
	mut var_status_map := rt.create_array([
		rt.ArrayItem{
			...
			val: Class_Automattic_WooCommerce_Internal_Agentic_Enums_Specs_OrderStatus.created()
		},
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Enums_OrderStatus.processing()
			val: Class_Automattic_WooCommerce_Internal_Agentic_Enums_Specs_OrderStatus.confirmed()
		},
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Enums_OrderStatus.on_hold()
			val: Class_Automattic_WooCommerce_Internal_Agentic_Enums_Specs_OrderStatus.manual_review()
		},
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Enums_OrderStatus.completed()
			val: Class_Automattic_WooCommerce_Internal_Agentic_Enums_Specs_OrderStatus.fulfilled()
		},
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Enums_OrderStatus.cancelled()
			val: Class_Automattic_WooCommerce_Internal_Agentic_Enums_Specs_OrderStatus.canceled()
		},
		rt.ArrayItem{
	...
	var_status_map = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_agentic_webhook_order_status_map'),
		var_status_map.clone(),
		rt.new_string(wc_status),
	])
	mut var_mapped_status := if var_status_map.array_isset(rt.new_string(wc_status)) {
		var_status_map.array_get(rt.new_string(wc_status))
	} else {
		Class_Automattic_WooCommerce_Internal_Agentic_Enums_Specs_OrderStatus.created()
	}
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Agentic_Enums_Specs_OrderStatus{}
	mut iife_result_0 := iife_temp_0.is_valid(var_mapped_status.clone())
	if rt.is_true(rt.new_bool(!(rt.is_true(iife_result_0)))) {
		rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'warning', [
			rt.call_function('sprintf', [
				rt.new_string('Invalid ACP order status "%s" returned by woocommerce_agentic_webhook_order_status_map filter for WooCommerce status "%s". Using "created" as fallback.'),
				var_mapped_status.clone(),
				rt.new_string(wc_status),
			]),
			rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder) build_refunds_data(mut var_order Class_WC_Order) rt.PhpVal {
	return rt.call_function('array_map', [
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'build_single_refund_data' },
		]),
		var_order.get_refunds(),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder) build_single_refund_data(mut var_refund Class_WC_Order_Refund) rt.PhpVal {
	...
	mut var_amount := rt.call_function('abs', [
		rt.new_float((var_refund.get_total()).to_f64()),
	])
	mut var_amount_in_minor_units := rt.new_int((rt.call_method(this.money_formatter, 'format', [
		var_amount.clone(),
	])).to_i64())
	return rt.create_array([rt.ArrayItem{ key: 'type', val: var_refund_type },
		rt.ArrayItem{ key: 'amount', val: var_amount_in_minor_units }])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder) determine_refund_type(mut var_refund Class_WC_Order_Refund) string {
	mut var_refund_type :=
		Class_Automattic_WooCommerce_Internal_Agentic_Enums_Specs_RefundType.original_payment()
	return (rt.call_function('apply_filters', [
		rt.new_string('woocommerce_agentic_webhook_refund_type'),
...
fn create_automattic_woocommerce_internal_admin_agentic_agenticwebhookpayloadbuilder(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder {
...
fn create_automattic_woocommerce_storeapi_formatters_moneyformatter(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_StoreApi_Formatters_MoneyFormatter {
...
fn create_automattic_woocommerce_internal_agentic_enums_specs_orderstatus(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Agentic_Enums_Specs_OrderStatus {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'build_single_refund_data' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WC_Order_Refund](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
