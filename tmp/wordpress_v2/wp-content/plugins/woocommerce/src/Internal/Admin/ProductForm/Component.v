...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component) construct(var_id rt.PhpVal, var_plugin_id rt.PhpVal, var_additional_args rt.PhpVal) {
	this.dispatch_set_prop('id', var_id.clone())
	this.dispatch_set_prop('plugin_id', var_plugin_id.clone())
	this.additional_args = var_additional_args.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component) get_additional_argument(var_key rt.PhpVal) rt.PhpVal {
	...
	return Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component.get_argument_from_path((this.additional_args).str(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component) get_json() rt.PhpVal {
	return rt.call_function('array_merge', [
		rt.create_array([rt.ArrayItem{ key: 'id', val: this.get_id() },
			rt.ArrayItem{ key: 'plugin_id', val: this.get_plugin_id() }]),
...
fn Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component.sort(var_a rt.PhpVal, var_b rt.PhpVal, var_sort_by rt.PhpVal) rt.PhpVal {
	mut var_key := var_sort_by.array_get(rt.new_string('key'))
	mut var_a_val := rt.call_method(var_a, 'get_additional_argument', [
		var_key.clone()])
	mut var_b_val := rt.call_method(var_b, 'get_additional_argument', [
		var_key.clone()])
	if rt.is_true(rt.identical(rt.new_string('asc'), var_sort_by.array_get(rt.new_string('order')))) {
		return rt.new_null()
	} else {
		return rt.new_null()
	}
...
fn Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component.get_argument_from_path(var_arguments rt.PhpVal, var_path rt.PhpVal, delimiter string) rt.PhpVal {
	mut var_path_keys := rt.call_function('explode', [rt.new_string(delimiter),
		var_path.clone()])
	mut var_num_keys := rt.new_int(if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false),
		var_path_keys))))
	{
		var_path_keys.clone().array_count()
	} else {
		0
	})
	...
	for {
		...
		if rt.is_true(rt.new_bool(var_val.clone().array_isset(var_key.clone()))) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component) get_missing_arguments(var_args rt.PhpVal) rt.PhpVal {
	closure_1_fn := fn [var_args] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_arg_key := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return rt.identical(rt.new_null(), Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component.get_argument_from_path(var_args.str(),
			var_arg_key.clone()))
	}
	...
	return rt.call_function('array_values', [
		rt.call_function('array_filter', [this.required_arguments, rt.new_closure(closure_1_fn)]),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'sort' {
			...
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			dispatch_arg_2 := if args.len > 2 { args[2] } else { rt.new_null() }
			return Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component.sort(dispatch_arg_0,
		...
		'get_argument_from_path' {
			...
			return Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component.get_argument_from_path(dispatch_arg_0,
				dispatch_arg_1, dispatch_arg_2)
		}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductForm_Component) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'additional_args' {
			this.additional_args = val
			return true
		}
		'required_arguments' {
			this.required_arguments = val
			return true
		}
		else {
