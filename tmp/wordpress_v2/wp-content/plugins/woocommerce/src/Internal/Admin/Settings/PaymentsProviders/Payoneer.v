...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return !(rt.is_true(rt.call_function('wc_string_to_bool', [
		var_payment_gateway.get_option(rt.new_string('live_mode')),
	])))
	unsafe {
		goto end_label_1
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	mut var_sandbox_prefix := rt.new_string((if this.is_in_test_mode(mut var_payment_gateway) {
		'sandbox_'
	} else {
		''
	}).str())
	...
	return !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('merchant_code'))))
		&& !(!rt.is_true(var_payment_gateway.get_option(rt.new_string(var_sandbox_prefix.str() + 'merchant_token'))))
		&& !(!rt.is_true(var_payment_gateway.get_option(rt.new_string(var_sandbox_prefix.str() + 'store_code'))))
	unsafe {
		goto end_label_2
	}
	catch_label_2:
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_payoneer(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payoneer {
