...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_NexiCheckout) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	...
	return if rt.is_true(var_sandbox) {
		!(!rt.is_true(var_payment_gateway.get_option(rt.new_string('dibs_test_key'))))
			&& !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('dibs_test_checkout_key'))))
	} else {
		!(!rt.is_true(var_payment_gateway.get_option(rt.new_string('dibs_live_key'))))
			&& !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('dibs_checkout_key'))))
	}
	unsafe {
		goto end_label_1
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_NexiCheckout) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_nexi_in_sandbox_mode(mut var_payment_gateway)).is_null() {
		this.is_nexi_in_sandbox_mode(mut var_payment_gateway)
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_NexiCheckout) is_nexi_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (rt.call_function('wc_string_to_bool', [
		var_payment_gateway.get_option(rt.new_string('test_mode')),
	])).to_bool()
	unsafe {
		goto end_label_2
	}
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_nexicheckout(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_NexiCheckout {
