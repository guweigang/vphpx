...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paystack) needs_setup(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	mut var_is_valid_for_use := rt.new_bool(true)
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	if rt.is_true(rt.call_function('is_callable', [
		rt.create_array([rt.ArrayItem{ key: none, val: var_payment_gateway },
			rt.ArrayItem{ key: none, val: 'is_valid_for_use' }]),
	]))
	{
		var_is_valid_for_use = rt.call_function('wc_string_to_bool', [
			var_payment_gateway.is_valid_for_use(),
		])
		if rt.has_exception() {
			unsafe {
				goto catch_label_1
	...
	return rt.is_true(rt.new_bool(!(rt.is_true(var_is_valid_for_use))))
		|| !(this.is_account_connected(mut var_payment_gateway))
	unsafe {
		goto end_label_1
	}
	catch_label_1:
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paystack) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return
		rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('public_key')]))
		&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'public_key')))
		&& rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('secret_key')]))
		&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'secret_key')))
	unsafe {
		goto end_label_2
	}
	catch_label_2:
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_paystack(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paystack {
