...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_CustomizeStoreWithBlocks.get_note() rt.PhpVal {
	...
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Admin_Notes_CustomizeStoreWithBlocks{}
	mut iife_result_0 := iife_temp_0.is_wc_admin_active_in_date_range(rt.new_string('week-1-4'), rt.mul(rt.new_int(14),
	...
	if 0 == var_products.clone().array_count() {
		return rt.new_null()
	}
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [
		rt.new_string('Customize your online store with WooCommerce blocks'),
		rt.new_string('woocommerce'),
	]))
	var_note.set_content(rt.call_function('__', [
		rt.new_string('With our blocks, you can select and display products, categories, filters, and more virtually anywhere on your site — no need to use shortcodes or edit lines of code. Learn more about how to use each one of them.'),
		rt.new_string('woocommerce'),
	]))
	...
	var_note.add_action(rt.new_string('customize-store-with-blocks'), rt.call_function('__', [
		rt.new_string('Learn more'),
		rt.new_string('woocommerce'),
	]),
		rt.new_string('https://woocommerce.com/posts/how-to-customize-your-online-store-with-woocommerce-blocks/?utm_source=inbox&utm_medium=product'),
...
fn create_automattic_woocommerce_internal_admin_notes_customizestorewithblocks(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Notes_CustomizeStoreWithBlocks {
