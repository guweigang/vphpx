...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Mollie) get_settings_url(mut var_payment_gateway Class_WC_Payment_Gateway) string {
	if rt.is_true(rt.identical(rt.new_string('mollie_stand_in'), rt.get_property(var_payment_gateway,
		'id')))
	{
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Mollie) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	...
	if rt.is_true(rt.identical(rt.new_bool(true), var_sandbox_mode)) {
		return !(!rt.is_true(rt.call_function('get_option', [
			rt.new_string('mollie-payments-for-woocommerce_test_api_key'),
			rt.new_string(''),
		])))
	} else if rt.is_true(rt.identical(rt.new_bool(false), var_sandbox_mode)) {
		return !(!rt.is_true(rt.call_function('get_option', [
			rt.new_string('mollie-payments-for-woocommerce_live_api_key'),
			rt.new_string(''),
		])))
	}
	if rt.has_exception() {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Mollie) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_mollie_in_sandbox_mode(mut var_payment_gateway)).is_null() {
		this.is_mollie_in_sandbox_mode(mut var_payment_gateway)
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Mollie) is_gateway_registered(mut var_payment_gateways Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_array) bool {
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return (rt.call_function('str_starts_with', [rt.get_property(var_gateway, 'id'),
			rt.new_string('mollie_wc_gateway_')])).to_bool()
	}
	mut var_mollie_gateways := rt.call_function('array_filter', [var_payment_gateways,
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Mollie) get_pseudo_gateway(mut var_suggestion Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_array) rt.PhpVal {
	return rt.new_object('Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PseudoWCPaymentGateway',
		[]string{}, create_automattic_woocommerce_internal_admin_settings_paymentsproviders_pseudowcpaymentgateway(rt.new_string('mollie_stand_in'), rt.create_array([
		rt.ArrayItem{ key: 'method_title', val: var_suggestion.array_get(rt.new_string('title')) },
		rt.ArrayItem{
			key: 'method_description'
			val: var_suggestion.array_get(rt.new_string('description'))
		},
		rt.ArrayItem{ key: 'enabled', val: false },
		rt.ArrayItem{ key: 'needs_setup', val: true },
		rt.ArrayItem{ key: 'test_mode', val: false },
		rt.ArrayItem{ key: 'dev_mode', val: false },
		rt.ArrayItem{ key: 'account_connected', val: false },
		rt.ArrayItem{ key: 'onboarding_started', val: false },
		rt.ArrayItem{ key: 'onboarding_completed', val: false },
		rt.ArrayItem{ key: 'settings_url', val: this.get_custom_settings_url('') },
		rt.ArrayItem{
			key: 'plugin_slug'
			val: var_suggestion.array_get(rt.new_string('plugin')).array_get(rt.new_string('slug'))
		},
		rt.ArrayItem{
			key: 'plugin_file'
			val: var_suggestion.array_get(rt.new_string('plugin')).array_get(rt.new_string('file'))
		},
	])))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Mollie) get_custom_settings_url(section string) string {
	mut var_settings_url := rt.call_function('admin_url', [
		rt.new_string('admin.php?page=wc-settings&tab=mollie_settings'),
	])
	if !(section == '') {
		var_settings_url = rt.call_function('add_query_arg', [
			rt.new_string('section'), rt.new_string(section),
			var_settings_url.clone()])
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Mollie) is_mollie_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (rt.call_function('filter_var', [
		rt.call_function('get_option', [
			rt.new_string('mollie-payments-for-woocommerce_test_mode_enabled'),
			rt.new_string('yes'),
		]),
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_mollie(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Mollie {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_pseudowcpaymentgateway(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PseudoWCPaymentGateway {
