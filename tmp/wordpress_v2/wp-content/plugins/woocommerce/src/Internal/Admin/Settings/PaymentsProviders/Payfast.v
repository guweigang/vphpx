...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payfast) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('merchant_id'))))
		&& !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('merchant_key'))))
		&& !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('pass_phrase'))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wc_string_to_bool', [rt.call_function('get_option', [rt.new_string('woocommerce_payfast_invalid_credentials'), rt.new_string('no')])])))))
	unsafe {
		goto end_label_1
	}
	catch_label_1:
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_payfast(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Payfast {
