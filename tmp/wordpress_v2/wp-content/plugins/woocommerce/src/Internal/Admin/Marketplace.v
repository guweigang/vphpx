...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Marketplace) init() {
	rt.call_function('add_action', [rt.new_string('init'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Marketplace',
				[]string{}, &this) },
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Marketplace) on_init() {
	...
	rt.call_function('add_filter', [rt.new_string('install_plugins_tabs'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Marketplace',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'add_woo_plugin_install_action_link' },
		])])
	rt.call_function('add_action', [rt.new_string('install_plugins_pre_woo'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Marketplace',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'maybe_open_woo_tab' },
		])])
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Marketplace) register_pages() {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [
		rt.new_string('wc_admin_register_page'),
	])))))
	{
		return
	}
	...
	for {
		...
		if !(var_marketplace_page.clone().is_null()) {
			rt.call_function('wc_admin_register_page', [var_marketplace_page.clone()])
		}
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Marketplace) get_marketplace_pages() rt.PhpVal {
	mut var_marketplace_pages := rt.create_array([
		rt.ArrayItem{ key: none, val: rt.create_array([
			rt.ArrayItem{ key: 'id', val: 'woocommerce-marketplace' },
			rt.ArrayItem{ key: 'parent', val: 'woocommerce' },
			rt.ArrayItem{ key: 'title', val:
				(rt.call_function('__', [rt.new_string('Extensions'), rt.new_string('woocommerce')])).str() +
				this.badge() },
			rt.ArrayItem{ key: 'page_title', val: rt.call_function('__', [
				rt.new_string('Extensions'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'path', val: '/extensions' },
		]) },
	])
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_marketplace_menu_items'),
		var_marketplace_pages.clone(),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Marketplace) enqueue_scripts(var_hook_suffix rt.PhpVal) {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('woocommerce_page_wc-admin'),
		var_hook_suffix))))
	{
		return
	}
	if !(rt.get_superglobal('_GET').array_isset(rt.new_string('path')))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('/extensions'), rt.get_superglobal('_GET').array_get(rt.new_string('path')))))) {
		return
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Marketplace) add_woo_plugin_install_action_link(var_tabs rt.PhpVal) rt.PhpVal {
	...
	var_tabs_mutated.array_set(Class_Automattic_WooCommerce_Internal_Admin_Automattic_WooCommerce_Internal_Admin_Marketplace.marketplace_tab_slug(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Marketplace) maybe_open_woo_tab() {
	if !(rt.get_superglobal('_GET').array_isset(rt.new_string('tab')))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Automattic_WooCommerce_Internal_Admin_Marketplace.marketplace_tab_slug(), rt.get_superglobal('_GET').array_get(rt.new_string('tab')))))) {
		return
	}
	mut var_woo_url := rt.call_function('add_query_arg', [
		rt.create_array([rt.ArrayItem{ key: 'page', val: 'wc-admin' },
			rt.ArrayItem{ key: 'path', val: '/extensions' }, rt.ArrayItem{
				key: 'tab'
				val: 'extensions'
			}, rt.ArrayItem{ key: 'ref', val: 'plugins' }]),
		rt.call_function('admin_url', [rt.new_string('admin.php')]),
	])
	rt.call_function('wc_admin_record_tracks_event', [
		rt.new_string('marketplace_plugin_install_woo_clicked'),
	])
	rt.call_function('wp_safe_redirect', [var_woo_url.clone()])
	exit(0)
}
...
fn create_automattic_woocommerce_internal_admin_marketplace(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Marketplace {
