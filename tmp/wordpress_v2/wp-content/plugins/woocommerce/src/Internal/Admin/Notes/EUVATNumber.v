...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_EUVATNumber.get_note() rt.PhpVal {
	...
	mut var_eu_countries := rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}),
		'countries'), 'get_european_union_countries', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_country_code.clone(), var_eu_countries.clone(), rt.new_bool(true)])))))
	{
		return rt.new_null()
	}
	mut var_content := rt.call_function('__', [
		rt.new_string("If your store is based in the EU, we recommend using the EU VAT Number extension in addition to automated taxes. It provides your checkout with a field to collect and validate a customer's EU VAT number, if they have one."),
		rt.new_string('woocommerce'),
	])
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [
		rt.new_string('Collect and validate EU VAT numbers at checkout'),
		rt.new_string('woocommerce'),
	]))
	...
	var_note.add_action(rt.new_string('learn-more'), rt.call_function('__', [
		...
		rt.new_string('woocommerce'),
	]), rt.new_string('https://woocommerce.com/products/eu-vat-number/?utm_medium=product'),
...
fn create_automattic_woocommerce_internal_admin_notes_euvatnumber(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Notes_EUVATNumber {
