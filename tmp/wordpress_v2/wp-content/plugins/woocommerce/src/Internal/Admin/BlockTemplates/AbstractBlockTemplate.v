...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_BlockTemplates_AbstractBlockTemplate) get_block(block_id string) rt.PhpVal {
	...
	return if !(this.block_cache.array_get(rt.new_string(block_id_mutated))).is_null() {
		this.block_cache.array_get(rt.new_string(block_id_mutated))
	} else {
		rt.new_null()
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_BlockTemplates_AbstractBlockTemplate) cache_block(mut var_block Class_Automattic_WooCommerce_Admin_BlockTemplates_BlockInterface) {
	...
	if this.block_cache.array_isset(var_id) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_Admin_BlockTemplates_ValueError',
			[]string{},
			create_automattic_woocommerce_internal_admin_blocktemplates_valueerror(rt.new_string('A block with the specified ID already exists in the template.'))))
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_block.get_root_template(), rt.new_object('Automattic_WooCommerce_Internal_Admin_BlockTemplates_AbstractBlockTemplate', [
		'BlockTemplateInterface',
	], &this)))))
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_Admin_BlockTemplates_ValueError',
			[]string{},
			create_automattic_woocommerce_internal_admin_blocktemplates_valueerror(rt.new_string('The block template that the block belongs to must be the same as this template.'))))
	}
	this.block_cache.array_set(var_id, var_block)
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_BlockTemplates_AbstractBlockTemplate) generate_block_id(id_base string) string {
	mut var_instance_count := rt.new_int(0)
	for {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_BlockTemplates_AbstractBlockTemplate) get_root_template() rt.PhpVal {
	return rt.new_object('Automattic_WooCommerce_Internal_Admin_BlockTemplates_AbstractBlockTemplate',
		[]string{}, this)
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_BlockTemplates_AbstractBlockTemplate) get_formatted_template() rt.PhpVal {
	...
	mut var_inner_blocks_formatted_template := rt.call_function('array_map', [
		rt.new_closure(closure_1_fn),
		var_inner_blocks.clone(),
	])
	return var_inner_blocks_formatted_template.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_BlockTemplates_AbstractBlockTemplate) to_json() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: 'id', val: this.get_id() },
		...
		}, rt.ArrayItem{ key: 'area', val: this.get_area() },
		rt.ArrayItem{ key: 'blockTemplates', val: this.get_formatted_template() }])
}
