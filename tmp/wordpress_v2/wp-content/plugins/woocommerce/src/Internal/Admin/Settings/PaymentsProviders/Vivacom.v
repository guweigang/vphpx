...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Vivacom) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(this.is_in_test_mode(rt.new_object('WC_Payment_Gateway', []string{},
		var_payment_gateway)))
	{
		return
			rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('test_client_id')]))
			&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'test_client_id')))
			&& rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('test_client_secret')]))
			&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'test_client_secret')))
			&& rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('test_source_code')]))
			&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'test_source_code')))
	} else {
		return
			rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('client_id')]))
			&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'client_id')))
			&& rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('client_secret')]))
			&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'client_secret')))
			&& rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('source_code')]))
			&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'source_code')))
	}
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_vivacom(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Vivacom {
