...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_MetaBoxes_OrderAttribution) format_meta_data(mut var_meta Class_Automattic_WooCommerce_Internal_Admin_Orders_MetaBoxes_array) {
	...
	if rt.is_true(rt.new_bool(var_meta_mutated.array_isset(rt.new_string('device_type')))) {
		mut switch_val_1 := var_meta_mutated.array_get(rt.new_string('device_type'))
		if rt.is_true(rt.equal(switch_val_1, rt.new_string('Mobile'))) {
			var_meta_mutated.array_set('device_type', rt.call_function('__', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_MetaBoxes_OrderAttribution) output(mut var_order Class_WC_Order) {
	...
	mut var_has_more_details := rt.new_bool(!rt.is_true(rt.identical(rt.create_array([
		rt.ArrayItem{ key: none, val: 'origin' },
	]), rt.func_array_keys(var_meta.clone()))))
	mut var_simple_sources := rt.create_array([rt.ArrayItem{ key: none, val: 'typein' },
		rt.ArrayItem{ key: none, val: 'admin' }, rt.ArrayItem{ key: none, val: 'mobile_app' },
		rt.ArrayItem{ key: none, val: 'pos' }])
	if var_meta.array_isset(rt.new_string('source_type'))
		&& rt.is_true(rt.call_function('in_array', [var_meta.array_get(rt.new_string('source_type')), var_simple_sources.clone(), rt.new_bool(true)])) {
		var_has_more_details = rt.new_bool(false)
	}
	mut var_template_data := rt.create_array([rt.ArrayItem{ key: 'meta', val: var_meta },
		rt.ArrayItem{ key: 'has_more_details', val: var_has_more_details }])
	rt.call_function('wc_get_template', [rt.new_string('order/attribution-details.php'),
		var_template_data.clone()])
}
