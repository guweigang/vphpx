...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_LaunchChecklist.get_note() rt.PhpVal {
	mut var_completed_tasks := rt.call_function('get_option', [
		rt.new_string('woocommerce_task_list_tracked_completed_tasks'),
		rt.new_array(),
	])
	mut var_ten_days_in_seconds := rt.mul(rt.new_int(10), rt.get_constant('DAY_IN_SECONDS'))
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Admin_Notes_LaunchChecklist{}
	mut iife_result_0 := iife_temp_0.is_wc_admin_active_in_date_range(rt.new_string('week-1-4'),
		var_ten_days_in_seconds.clone())
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('get_option', [rt.new_string('woocommerce_task_list_complete')])))))
		&& var_completed_tasks.clone().array_count() < 3 || rt.is_true(iife_result_0) {
		return rt.new_null()
	}
	mut var_content := rt.call_function('__', [
		rt.new_string('To make sure you never get that sinking "what did I forget" feeling, we\'ve put together the essential pre-launch checklist.'),
		rt.new_string('woocommerce'),
	])
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [
		rt.new_string('Ready to launch your store?'),
		rt.new_string('woocommerce'),
	]))
	...
	var_note.add_action(rt.new_string('learn-more'), rt.call_function('__', [
		...
		rt.new_string('https://woocommerce.com/posts/pre-launch-checklist-the-essentials/?utm_source=inbox&utm_medium=product'))
	return mut var_note
}
...
fn create_automattic_woocommerce_internal_admin_notes_launchchecklist(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Notes_LaunchChecklist {
