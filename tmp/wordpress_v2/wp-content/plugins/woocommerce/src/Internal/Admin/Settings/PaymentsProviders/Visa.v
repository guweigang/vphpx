...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
	...
	}, rt.ArrayItem{ key: none, val: 'get_config_settings' }])])
		&& rt.is_true(rt.call_function('defined', [rt.new_string('VISA_ACCEPTANCE_ENVIRONMENT_TEST')]))
		&& rt.is_true(rt.call_function('defined', [rt.new_string('VISA_ACCEPTANCE_ENVIRONMENT_PRODUCTION')])) {
		...
		return var_settings.clone().is_array()
			&& var_settings.array_isset(rt.new_string('environment'))&& (rt.is_true(rt.identical(rt.get_constant('VISA_ACCEPTANCE_ENVIRONMENT_TEST'), var_settings.array_get(rt.new_string('environment'))))
			&& !(!rt.is_true(var_settings.array_get(rt.new_string('test_merchant_id'))))
			&& !(!rt.is_true(var_settings.array_get(rt.new_string('test_api_key'))))
			&& !(!rt.is_true(var_settings.array_get(rt.new_string('test_api_shared_secret')))))|| (rt.is_true(rt.identical(rt.get_constant('VISA_ACCEPTANCE_ENVIRONMENT_PRODUCTION'), var_settings.array_get(rt.new_string('environment'))))
			&& !(!rt.is_true(var_settings.array_get(rt.new_string('merchant_id'))))
			&& !(!rt.is_true(var_settings.array_get(rt.new_string('api_key'))))
			&& !(!rt.is_true(var_settings.array_get(rt.new_string('api_shared_secret')))))
	}
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_visa_in_sandbox_mode(mut var_payment_gateway)).is_null() {
		this.is_visa_in_sandbox_mode(mut var_payment_gateway)
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa) is_visa_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
		...
		if var_settings.clone().is_array() && var_settings.array_isset(rt.new_string('environment')) {
			if rt.is_true(rt.identical(rt.get_constant('VISA_ACCEPTANCE_ENVIRONMENT_TEST'),
				var_settings.array_get(rt.new_string('environment'))))
			{
				return true
			}
			if rt.has_exception() {
			...
			if rt.is_true(rt.identical(rt.get_constant('VISA_ACCEPTANCE_ENVIRONMENT_PRODUCTION'),
				var_settings.array_get(rt.new_string('environment'))))
			{
				return false
			}
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_visa(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Visa {
