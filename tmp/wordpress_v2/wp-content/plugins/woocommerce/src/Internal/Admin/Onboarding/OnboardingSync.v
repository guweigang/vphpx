...
fn init_static_automattic_woocommerce_internal_admin_onboarding_onboardingsync() {
	rt.init_static_prop('Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync',
		'instance', rt.new_null())
}

fn Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync.instance() rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.get_static_prop('Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync',
		'instance')))))
	{
		rt.set_static_prop('Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync',
	...
	return rt.get_static_prop('Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync',
		'instance')
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync) init() {
	rt.call_function('add_action', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'send_profile_data_on_update' },
		]),
		rt.new_int(10),
		rt.new_int(2),
	])
	rt.call_function('add_action', [rt.new_string('woocommerce_helper_connected'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'send_profile_data_on_connect' },
		])])
	...
	rt.call_function('add_action', [rt.new_string('current_screen'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'redirect_wccom_install' },
		])])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync) send_profile_data() bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [rt.new_string('\\WC_Helper_API')])))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('method_exists', [rt.new_string('\\WC_Helper_API'), rt.new_string('put')]))))) {
		return false
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('\\WC_Helper_Options'),
	])))))
	{
		return false
	}
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Admin_Onboarding_WC_Helper_Options{}
	...
	mut var_defaults := rt.create_array([rt.ArrayItem{ key: 'plugins', val: 'skipped' },
		rt.ArrayItem{ key: 'industry', val: rt.new_array() },
		rt.ArrayItem{ key: 'product_types', val: rt.new_array() },
		rt.ArrayItem{ key: 'product_count', val: '0' }, rt.ArrayItem{
			key: 'selling_venues'
			val: 'no'
		}, rt.ArrayItem{ key: 'number_employees', val: '1' },
		rt.ArrayItem{ key: 'revenue', val: 'none' }, rt.ArrayItem{
			key: 'other_platform'
			val: 'none'
		}, rt.ArrayItem{ key: 'business_extensions', val: rt.new_array() },
		rt.ArrayItem{ key: 'theme', val: rt.call_function('get_stylesheet', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'setup_client', val: false }, rt.ArrayItem{
			key: 'store_location'
			val: var_base_location.array_get(rt.new_string('country'))
		}, rt.ArrayItem{ key: 'default_currency', val: rt.call_function('get_woocommerce_currency',
			[]rt.PhpVal{}) }])
	if var_profile.array_isset(rt.new_string('industry'))
		&& var_profile.array_get(rt.new_string('industry')).is_array() {
		mut var_industry_slugs := rt.new_array()
		mut iter_1 := var_profile.array_get(rt.new_string('industry')).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_industry := item_1.val
			var_industry_slugs.array_push(if var_industry.clone().is_array() {
				var_industry.array_get(rt.new_string('slug'))
			} else {
				var_industry
			})
		}
		var_profile.array_set('industry', var_industry_slugs.clone())
	}
	mut var_body := rt.call_function('wp_parse_args', [var_profile.clone(),
		var_defaults.clone()])
	mut iife_temp_1 := Class_Automattic_WooCommerce_Internal_Admin_Onboarding_WC_Helper_API{}
	mut iife_result_1 := iife_temp_1.put(rt.new_string('profile'), rt.create_array([
		rt.ArrayItem{ key: 'authenticated', val: true },
		rt.ArrayItem{ key: 'body', val: rt.call_function('wp_json_encode', [
			var_body.clone()]) },
		rt.ArrayItem{ key: 'headers', val: rt.create_array([rt.ArrayItem{
			key: 'Content-Type'
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync) send_profile_data_on_connect() {
	...
	if !(var_profile.array_isset(rt.new_string('completed')))
		|| rt.is_true(rt.new_bool(!(rt.is_true(var_profile.array_get(rt.new_string('completed')))))) {
		return
	}
	this.send_profile_data()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Onboarding_OnboardingSync) redirect_wccom_install() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_task_list))))
		|| rt.is_true(rt.call_method(var_task_list, 'is_hidden', []rt.PhpVal{}))
		|| !(rt.get_superglobal('_SERVER').array_isset(rt.new_string('HTTP_REFERER')))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [rt.call_function('wp_unslash', [rt.get_superglobal('_SERVER').array_get(rt.new_string('HTTP_REFERER'))]), rt.new_string('https://woocommerce.com/checkout?utm_medium=product')]))))) {
		return
	}
...
fn create_automattic_woocommerce_internal_admin_onboarding_wc_helper_api(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Onboarding_WC_Helper_API {
