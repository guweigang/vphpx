...
fn init_static_automattic_woocommerce_internal_admin_wcadminassets() {
		rt.init_static_prop('Automattic_WooCommerce_Internal_Admin_WCAdminAssets', 'instance', rt.new_null())
}

fn Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_instance() rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.get_static_prop('Automattic_WooCommerce_Internal_Admin_WCAdminAssets', 'instance'))))) {
		rt.set_static_prop('Automattic_WooCommerce_Internal_Admin_WCAdminAssets', 'instance', rt.new_object('Automattic_WooCommerce_Internal_Admin_self', []string{}, create_automattic_woocommerce_internal_admin_self()))
	}
	return rt.get_static_prop('Automattic_WooCommerce_Internal_Admin_WCAdminAssets', 'instance')
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.should_use_minified_js_file(var_script_debug rt.PhpVal) bool {
	...
	mut iife_result_1 := iife_temp_1.exists(rt.new_string('minified-js'))
...
fn Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_url(var_file rt.PhpVal, var_ext rt.PhpVal) rt.PhpVal {
	...
	mut var_script_debug := rt.new_bool(rt.is_true(rt.call_function('defined', [rt.new_string('SCRIPT_DEBUG')])) && rt.is_true(rt.get_constant('SCRIPT_DEBUG')))
	var_suffix = rt.new_string((if rt.is_true(Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.should_use_minified_js_file(var_script_debug.clone())) { '.min' } else { '' }).str())
	}
	return rt.call_function('plugins_url', [rt.new_string((Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_path(var_ext.clone())).str() + (var_file).str() + (var_suffix).str() + '.' + (var_ext).str()), rt.get_constant('WC_ADMIN_PLUGIN_FILE')])
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_file_version(var_ext rt.PhpVal, var_asset_version rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.call_function('defined', [rt.new_string('SCRIPT_DEBUG')])) && rt.is_true(rt.get_constant('SCRIPT_DEBUG')) {
		return rt.call_function('filemtime', [rt.new_string((rt.get_constant('WC_ADMIN_ABSPATH')).str() + (Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_path(var_ext.clone())).str())])
	}
	if !(!rt.is_true(var_asset_version)) {
		return var_asset_version.clone()
	}
...
fn Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_script_asset_filename(var_script_path_name rt.PhpVal, var_file rt.PhpVal) rt.PhpVal {
	...
	mut iife_result_2 := iife_temp_2.exists(rt.new_string('minified-js'))
	mut var_minification_supported := iife_result_2
	...
	if rt.is_true(var_minification_supported) && rt.is_true(rt.call_function('is_readable', [rt.new_string((var_script_asset_path).str() + (var_script_min_filename).str())])) {
		return var_script_min_filename.clone()
	} else if rt.is_true(rt.call_function('is_readable', [rt.new_string((var_script_asset_path).str() + (var_script_nonmin_filename).str())])) {
		return var_script_nonmin_filename.clone()
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets) maybe_output_preload_link_tag(var_dependency rt.PhpVal, var_type rt.PhpVal, var_allowlist rt.PhpVal) {
	...
	rt.echo_val(rt.call_function('esc_url', [var_source.clone()]))
	print('" as="')
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets) output_header_preload_tags_for_type(var_type rt.PhpVal, var_allowlist rt.PhpVal) {
	...
	for {
		...
		mut var_dependency := rt.call_method(var_dependencies_of_type, 'query', [var_dependency_handle.clone(), rt.new_string('registered')])
		...
		for {
			...
			mut var_sub_dependency := rt.call_method(var_dependencies_of_type, 'query', [var_sub_dependency_handle.clone(), rt.new_string('registered')])
			if rt.is_true(var_sub_dependency) {
				this.maybe_output_preload_link_tag(var_sub_dependency.clone(), var_type.clone(), var_allowlist.clone())
			}
		}
		this.maybe_output_preload_link_tag(var_dependency.clone(), var_type.clone(), var_allowlist.clone())
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets) output_header_preload_tags() {
	...
	this.output_header_preload_tags_for_type(rt.new_string('style'), var_wc_admin_styles.clone())
	this.output_header_preload_tags_for_type(rt.new_string('script'), var_wc_admin_scripts.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets) enqueue_assets() {
	...
	mut iife_result_4 := iife_temp_4.is_modern_settings_page()
	...
	mut iife_result_5 := iife_temp_5.is_settings_page()
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets) modify_script_dependencies(var_dependencies rt.PhpVal, var_script rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.get_constant('WC_ADMIN_APP'))) {
		mut iife_temp_6 := Class_Automattic_WooCommerce_Admin_PageController{}
		mut iife_result_6 := iife_temp_6.is_admin_page()
		mut var_is_customize_store_page := rt.new_bool(rt.is_true(iife_result_6) && rt.get_superglobal('_GET').array_isset(rt.new_string('path')) && rt.is_true(rt.call_function('str_starts_with', [rt.call_function('wc_clean', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('path'))])]), rt.new_string('/customize-store')])))
		if rt.is_true(rt.new_bool(!(rt.is_true(var_is_customize_store_page)))) {
		var_dependencies_mutated = rt.call_function('array_diff', [var_dependencies_mutated.clone(), rt.create_array([rt.ArrayItem{ key: none, val: 'wp-editor' }])])
		}
		mut iife_temp_7 := Class_Automattic_WooCommerce_Utilities_FeaturesUtil{}
		mut iife_result_7 := iife_temp_7.feature_is_enabled(rt.new_string('product_block_editor'))
		if rt.is_true(rt.new_bool(!(rt.is_true(iife_result_7)))) {
		var_dependencies_mutated = rt.call_function('array_diff', [var_dependencies_mutated.clone(), rt.create_array([rt.ArrayItem{ key: none, val: 'wc-product-editor' }])])
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('wc-product-editor'))) {
		mut iife_temp_8 := Class_Automattic_WooCommerce_Internal_Admin_Automattic_WooCommerce_Admin_Features_ProductDataViews_Init{}
		mut iife_result_8 := iife_temp_8.is_product_data_view_page()
		mut var_is_product_data_view_page := iife_result_8
		...
		if !(rt.is_true(iife_result_9) || rt.is_true(var_is_product_data_view_page)) {
		var_dependencies_mutated = rt.call_function('array_diff', [var_dependencies_mutated.clone(), rt.create_array([rt.ArrayItem{ key: none, val: 'wp-editor' }])])
		}
	}
	return var_dependencies_mutated.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets) register_scripts() {
	...
	mut iife_result_10 := iife_temp_10.is_embed_page()
	mut var_scripts_map := rt.create_array([rt.ArrayItem{ key: rt.get_constant('WC_ADMIN_APP'), val: if rt.is_true(iife_result_10) { 'embed' } else { 'app' } }, rt.ArrayItem{ key: 'wc-csv', val: 'csv-export' }, rt.ArrayItem{ key: 'wc-store-data', val: 'data' }])
	...
	mut iter_3 := var_scripts.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_script := item_3.val
		mut var_script_path_name := if var_scripts_map.array_isset(var_script) { var_scripts_map.array_get(var_script) } else { rt.call_function('str_replace', [rt.new_string('wc-'), rt.new_string(''), var_script.clone()]) }
		mut var_script_assets_filename := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_script_asset_filename(var_script_path_name.clone(), rt.new_string('index'))
		...
		if rt.has_exception() { unsafe { goto catch_label_1 } }
		mut var_script_version := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_file_version(rt.new_string('js'), var_script_assets.array_get(rt.new_string('version')))
		if rt.has_exception() { unsafe { goto catch_label_1 } }
		mut var_script_dependencies := this.modify_script_dependencies(var_script_assets.array_get(rt.new_string('dependencies')), var_script.clone(), var_script_path_name.clone())
		if rt.has_exception() { unsafe { goto catch_label_1 } }
		rt.call_function('wp_register_script', [var_script.clone(), Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_url(rt.new_string((var_script_path_name).str() + '/index'), rt.new_string('js')), var_script_dependencies.clone(), var_script_version.clone(), rt.new_bool(true)])
		if rt.has_exception() { unsafe { goto catch_label_1 } }
		if rt.is_true(rt.call_function('in_array', [var_script.clone(), var_translated_scripts.clone(), rt.new_bool(true)])) {
			rt.call_function('wp_set_script_translations', [var_script.clone(), rt.new_string('woocommerce')])
			if rt.has_exception() { unsafe { goto catch_label_1 } }
		}
		if rt.has_exception() { unsafe { goto catch_label_1 } }
...
catch_label_1:
		...
		if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Internal_Admin_Exception') {
			...
			rt.call_function('wc_caught_exception', [var_e.clone(), rt.new_string(@STRUCT + '::' + @FN), var_script_path_name.clone()])
			unsafe { goto end_label_1 }
		}
		else {
...
end_label_1:
	...
	mut iter_4 := var_styles.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_style := item_4.val
		mut var_handle := var_style.array_get(rt.new_string('handle'))
		mut var_style_path_name := if var_scripts_map.array_isset(var_handle) { var_scripts_map.array_get(var_handle) } else { rt.call_function('str_replace', [rt.new_string('wc-'), rt.new_string(''), var_handle.clone()]) }
		mut var_style_assets_filename := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_script_asset_filename(var_style_path_name.clone(), rt.new_string('style'))
		...
		if rt.has_exception() { unsafe { goto catch_label_2 } }
		mut var_version := var_style_assets.array_get(rt.new_string('version'))
		if rt.has_exception() { unsafe { goto catch_label_2 } }
...
catch_label_2:
		...
		if rt.instance_of(var_e_2, 'Automattic_WooCommerce_Internal_Admin_Throwable') {
			...
			var_version = var_css_file_version.clone()
			unsafe { goto end_label_2 }
		}
		else {
...
end_label_2:
		mut var_dependencies := if var_style.array_isset(rt.new_string('dependencies')) { var_style.array_get(rt.new_string('dependencies')) } else { rt.new_array() }
		rt.call_function('wp_register_style', [var_handle.clone(), Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_url(rt.new_string((var_style_path_name).str() + '/style'), rt.new_string('css')), var_dependencies.clone(), Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_file_version(rt.new_string('css'), var_version.clone())])
		rt.call_function('wp_style_add_data', [var_handle.clone(), rt.new_string('rtl'), rt.new_string('replace')])
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets) inject_wc_settings_dependencies() {
	...
	if rt.is_true(rt.call_function('wp_script_is', [rt.new_string('wc-settings'), rt.new_string('registered')])) {
		...
		mut iter_5 := var_handles_for_injection.iterator()
		for {
			item_5 := iter_5.next() or { break }
			mut var_handle := item_5.val
			mut var_script := rt.call_method(var_wp_scripts, 'query', [var_handle.clone(), rt.new_string('registered')])
			if rt.is_true(rt.new_bool(rt.instance_of(var_script, '_WP_Dependency'))) {
				rt.get_property(var_script, 'deps').array_push('wc-settings')
				rt.call_method(var_wp_scripts, 'add_data', [var_handle.clone(), rt.new_string('group'), rt.new_int(1)])
			}
		}
		mut iter_6 := rt.get_property(var_wp_scripts, 'registered').iterator()
		for {
			item_6 := iter_6.next() or { break }
			mut var_script := item_6.val
			mut var_handle := item_6.key
			if rt.is_true(rt.call_function('array_intersect', [var_handles_for_injection.clone(), rt.get_property(var_script, 'deps')])) && !(rt.get_property(var_script, 'extra').array_isset(rt.new_string('group'))) {
				rt.call_method(var_wp_scripts, 'add_data', [var_handle.clone(), rt.new_string('group'), rt.new_int(1)])
				mut var_error_handle := rt.new_string('wc-settings-dep-in-header')
				mut var_used_deps := rt.call_function('implode', [rt.new_string(', '), rt.call_function('array_intersect', [var_handles_for_injection.clone(), rt.get_property(var_script, 'deps')])])
				mut var_error_message := rt.new_string("Scripts that have a dependency on [${var_used_deps.to_string()}] must be loaded in the footer, ${var_handle.to_string()} was registered to load in the header, but has been switched to load in the footer instead. See https://github.com/woocommerce/woocommerce-gutenberg-products-block/pull/5059")
				rt.call_function('wp_register_script', [var_error_handle.clone(), rt.new_string('')])
				rt.call_function('wp_enqueue_script', [var_error_handle.clone()])
				rt.call_function('wp_add_inline_script', [var_error_handle.clone(), rt.call_function('sprintf', [rt.new_string('console.warn( "%s" );'), var_error_message.clone()])])
			}
		}
	}
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.register_script(var_script_path_name rt.PhpVal, var_script_name rt.PhpVal, need_translation bool, var_dependencies rt.PhpVal) {
	mut var_script_path_name_mutated := var_script_path_name
	mut var_dependencies_mutated := var_dependencies
	mut var_script_assets_filename := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_script_asset_filename(var_script_path_name_mutated.clone(), var_script_name.clone())
	mut var_script_assets := rt.include_file((rt.get_constant('WC_ADMIN_ABSPATH')).str() + (rt.get_constant('WC_ADMIN_DIST_JS_FOLDER')).str() + (var_script_path_name_mutated).str() + '/' + (var_script_assets_filename).str(), '3')
	rt.call_function('wp_enqueue_script', [rt.new_string('wc-admin-' + (var_script_name).str()), Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_url(rt.new_string((var_script_path_name_mutated).str() + '/' + (var_script_name).str()), rt.new_string('js')), rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: none, val: rt.get_constant('WC_ADMIN_APP') }]), var_script_assets.array_get(rt.new_string('dependencies')), var_dependencies_mutated.clone()]), Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_file_version(rt.new_string('js'), var_script_assets.array_get(rt.new_string('version'))), rt.new_bool(true)])
	if var_need_translation {
		rt.call_function('wp_set_script_translations', [rt.new_string('wc-admin-' + (var_script_name).str()), rt.new_string('woocommerce')])
	}
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.register_style(var_style_path_name rt.PhpVal, var_style_name rt.PhpVal, var_dependencies rt.PhpVal) {
	...
	mut var_dependencies_mutated := var_dependencies
	mut var_style_assets_filename := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_script_asset_filename(var_style_path_name_mutated.clone(), var_style_name.clone())
	mut var_style_assets := rt.include_file((rt.get_constant('WC_ADMIN_ABSPATH')).str() + (rt.get_constant('WC_ADMIN_DIST_CSS_FOLDER')).str() + (var_style_path_name_mutated).str() + '/' + (var_style_assets_filename).str(), '3')
	mut var_handle := rt.new_string('wc-admin-' + (var_style_name).str())
	rt.call_function('wp_enqueue_style', [var_handle.clone(), Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_url(rt.new_string((var_style_path_name_mutated).str() + '/' + (var_style_name).str()), rt.new_string('css')), var_dependencies_mutated.clone(), Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets.get_file_version(rt.new_string('css'), var_style_assets.array_get(rt.new_string('version')))])
	rt.call_function('wp_style_add_data', [var_handle.clone(), rt.new_string('rtl'), rt.new_string('replace')])
}
...
fn create_automattic_woocommerce_internal_admin_exception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Exception {
...
fn create_automattic_woocommerce_internal_admin_automattic_woocommerce_admin_features_productdataviews_init(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Automattic_WooCommerce_Admin_Features_ProductDataViews_Init {
