...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_OnlineClothingStore.is_in_fashion_industry(var_industries rt.PhpVal) bool {
	...
	for {
		...
		if rt.is_true(rt.identical(rt.new_string('fashion-apparel-accessories'),
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_OnlineClothingStore.get_note() rt.PhpVal {
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Admin_Notes_OnlineClothingStore{}
	mut iife_result_0 := iife_temp_0.is_wc_admin_active_in_date_range(rt.new_string('week-1'), rt.mul(rt.new_int(2),
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(Class_Automattic_WooCommerce_Internal_Admin_Notes_OnlineClothingStore.is_in_fashion_industry(var_onboarding_profile.array_get(rt.new_string('industry'))))))) {
		return rt.new_null()
	}
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [
		rt.new_string('Start your online clothing store'),
		rt.new_string('woocommerce'),
	]))
	var_note.set_content(rt.call_function('__', [
		rt.new_string("Starting a fashion website is exciting but it may seem overwhelming as well. In this article, we'll walk you through the setup process, teach you to create successful product listings, and show you how to market to your ideal audience."),
		rt.new_string('woocommerce'),
	]))
	...
	var_note.add_action(rt.new_string('online-clothing-store'), rt.call_function('__', [
		rt.new_string('Learn more'),
		rt.new_string('woocommerce'),
	]),
		rt.new_string('https://woocommerce.com/posts/starting-an-online-clothing-store/?utm_source=inbox&utm_medium=product'),
...
fn create_automattic_woocommerce_internal_admin_notes_onlineclothingstore(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Notes_OnlineClothingStore {
