...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_RemoteFreeExtensions_ProcessCoreProfilerPluginInstallOptions) process_install_options() {
	...
	mut iter_2 := var_install_options.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_install_option := item_2.val
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_RemoteFreeExtensions_ProcessCoreProfilerPluginInstallOptions) add_install_option(mut var_install_option Class_Automattic_WooCommerce_Internal_Admin_RemoteFreeExtensions_object) {
	...
	mut var_options := if !(rt.get_property(var_install_option_mutated, 'options')).is_null() { rt.array_to_object(rt.get_property(var_install_option_mutated, 'options')) } else { create_automattic_woocommerce_internal_admin_remotefreeextensions_stdclass() }
	mut iter_3 := var_default_options.iterator()
	for {
		...
		if !(!(rt.get_property(var_options, '{"nodeType":"Expr_Variable","line":118,"name":"key"}')).is_null()) {
			rt.set_property(var_options, '{"nodeType":"Expr_Variable","line":119,"name":"key"}', var_value.clone())
		}
	}
	if rt.is_true(rt.get_property(var_options, 'force_array')) {
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('json_last_error', []rt.PhpVal{}), rt.get_constant('JSON_ERROR_NONE'))))) {
			rt.new_bool(rt.is_true(this.logger) && rt.is_true(rt.call_method(this.logger, 'error', [rt.new_string('Failed to decode JSON for install option value for ' + (rt.get_property(var_install_option_mutated, 'name')).str() + ': ' + (rt.call_function('json_last_error_msg', []rt.PhpVal{})).str())])))
			return
		}
	}
	...
	if !(rt.get_property(var_options, 'autoload')).is_null() {
		...
		var_autoload = rt.new_bool(true)
		} else if rt.is_true(rt.identical(rt.new_string('no'), rt.get_property(var_options, 'autoload'))) {
		var_autoload = rt.new_bool(false)
		} else if rt.is_true(rt.identical(rt.new_bool(true), rt.get_property(var_options, 'autoload'))) || rt.is_true(rt.identical(rt.new_bool(false), rt.get_property(var_options, 'autoload'))) {
		var_autoload = rt.get_property(var_options, 'autoload')
		}
	}
	this.add_option((rt.get_property(var_install_option_mutated, 'name')).str(), rt.get_property(var_install_option_mutated, 'value'), var_autoload.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_RemoteFreeExtensions_ProcessCoreProfilerPluginInstallOptions) add_option(name string, var_value rt.PhpVal, var_autoload rt.PhpVal) {
	...
	if rt.is_true(rt.call_function('in_array', [rt.new_string(name), Class_Automattic_WooCommerce_Internal_Admin_RemoteFreeExtensions_Automattic_WooCommerce_Internal_Admin_RemoteFreeExtensions_ProcessCoreProfilerPluginInstallOptions.disallowed_options(), rt.new_bool(true)])) {
		rt.new_bool(rt.is_true(this.logger) && rt.is_true(rt.call_method(this.logger, 'error', [rt.new_string('Disallowed option: ' + name)])))
		return
	}
	rt.call_function('add_option', [rt.new_string(name), var_value.clone(), rt.new_string(''), var_autoload_mutated.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_RemoteFreeExtensions_ProcessCoreProfilerPluginInstallOptions) matches_plugin_slug(mut var_plugin Class_Automattic_WooCommerce_Internal_Admin_RemoteFreeExtensions_object, plugin_slug string) bool {
	return (rt.identical(rt.call_function('explode', [rt.new_string(':'), rt.get_property(var_plugin, 'key')]).array_get(rt.new_int(0)), rt.new_string(plugin_slug))).to_bool()
}
...
fn create_automattic_woocommerce_internal_admin_remotefreeextensions_stdclass(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_RemoteFreeExtensions_stdClass {
