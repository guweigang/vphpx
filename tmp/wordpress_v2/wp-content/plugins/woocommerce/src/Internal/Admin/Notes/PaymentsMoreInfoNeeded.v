...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_PaymentsMoreInfoNeeded.should_display_note() bool {
	...
	if rt.is_true(rt.call_method(iife_result_0, 'has_incentive', []rt.PhpVal{})) {
		return false
	}
	mut var_exit_survey_timestamp := rt.call_function('get_option', [
		rt.new_string('wcpay_welcome_page_exit_survey_more_info_needed_timestamp'),
		rt.new_bool(false),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_exit_survey_timestamp))))
		|| rt.is_true(rt.less(rt.sub(rt.call_function('time', []rt.PhpVal{}), var_exit_survey_timestamp), rt.mul(rt.new_int(30), rt.get_constant('DAY_IN_SECONDS')))) {
		return false
	}
	return true
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_PaymentsMoreInfoNeeded.get_note() rt.PhpVal {
	...
	mut var_content := rt.call_function('sprintf', [
		rt.call_function('__', [
			rt.new_string('We recently asked you if you wanted more information about %s. Run your business and manage your payments in one place with the solution built and supported by WooCommerce.'),
	...
	var_note.set_title(rt.call_function('sprintf', [
		rt.call_function('__', [rt.new_string('Payments made simple with %s'),
			rt.new_string('woocommerce')]),
		rt.new_string('WooPayments'),
	]))
	...
	var_note.add_action(rt.new_string('learn-more'), rt.call_function('__', [
		rt.new_string('Learn more here'),
		rt.new_string('woocommerce'),
	]), rt.new_string('https://woocommerce.com/payments/'))
	return mut var_note
}
