...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_EditLock) get_lock(mut var_order Class_Automattic_WooCommerce_Internal_Admin_Orders_WC_Order) bool {
	...
	if rt.is_true(rt.new_bool(2 != var_lock.clone().array_count())) {
		return false
	}
	mut var_time := rt.call_function('absint', [var_lock.array_get(rt.new_int(0))])
	mut var_user_id := if var_lock.array_isset(rt.new_int(1)) { rt.call_function('absint', [var_lock.array_get(rt.new_int(1))]) } else { rt.new_int(0) }
	if rt.is_true(rt.new_bool(!(rt.is_true(var_time)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('get_user_by', [rt.new_string('id'), var_user_id.clone()]))))) {
		return false
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_EditLock) is_locked_by_another_user(mut var_order Class_Automattic_WooCommerce_Internal_Admin_Orders_WC_Order) bool {
	...
	return rt.is_true(var_lock) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_current_user_id', []rt.PhpVal{}), var_lock.array_get(rt.new_string('user_id'))))))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_EditLock) is_locked(mut var_order Class_Automattic_WooCommerce_Internal_Admin_Orders_WC_Order) bool {
	mut var_order_mutated := var_order
	return this.get_lock(mut var_order_mutated)
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_EditLock) lock(mut var_order Class_Automattic_WooCommerce_Internal_Admin_Orders_WC_Order) bool {
	...
	rt.call_method(var_order_mutated, 'update_meta_data', [Class_Automattic_WooCommerce_Internal_Admin_Orders_Automattic_WooCommerce_Internal_Admin_Orders_EditLock.meta_key_name(), rt.new_string((rt.call_function('time', []rt.PhpVal{})).str() + ':' + (var_user_id).str())])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_EditLock) refresh_lock_ajax(var_response rt.PhpVal, var_data rt.PhpVal) rt.PhpVal {
	...
	mut var_order_id := rt.call_function('absint', [if !(var_data.array_get(rt.new_string('wc-refresh-order-lock'))).is_null() { var_data.array_get(rt.new_string('wc-refresh-order-lock')) } else { rt.new_int(0) }])
	...
	if !(this.is_locked_by_another_user(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Admin_Orders_WC_Order](var_order))) {
		...
		mut var_user := rt.call_function('get_user_by', [rt.new_string('id'), var_current_lock.array_get(rt.new_string('user_id'))])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_EditLock) check_locked_orders_ajax(var_response rt.PhpVal, var_data rt.PhpVal) rt.PhpVal {
	mut var_response_mutated := var_response
	if !rt.is_true(var_data.array_get(rt.new_string('wc-check-locked-orders'))) || !(var_data.array_get(rt.new_string('wc-check-locked-orders')).is_array()) {
		return var_response_mutated.clone()
	}
	var_response_mutated.array_set('wc-check-locked-orders', rt.new_array())
	mut var_order_ids := rt.call_function('array_unique', [rt.call_function('array_map', [rt.new_string('absint'), var_data.array_get(rt.new_string('wc-check-locked-orders'))])])
	mut iter_1 := var_order_ids.iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_order_id := item_1.val
		mut var_order := rt.call_function('wc_get_order', [var_order_id.clone()])
		if rt.is_true(rt.new_bool(!(rt.is_true(var_order)))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [var_order.clone(), Class_Automattic_WooCommerce_Internal_Admin_Orders_WC_Order.class()]))))) {
			continue
		}
		if !(this.is_locked_by_another_user(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Admin_Orders_WC_Order](var_order))) || (rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.get_property(rt.get_property(rt.call_function('get_post_type_object', [rt.call_method(var_order, 'get_type', []rt.PhpVal{})]), 'cap'), 'edit_post'), rt.call_method(var_order, 'get_id', []rt.PhpVal{})]))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('manage_woocommerce')])))))) {
			continue
		}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Orders_EditLock) render_dialog(var_order rt.PhpVal) {
	...
	mut var_user := if rt.is_true(var_lock) { rt.call_function('get_user_by', [rt.new_string('id'), var_lock.array_get(rt.new_string('user_id'))]) } else { rt.new_bool(false) }
	mut var_locked := rt.new_bool(rt.is_true(var_user) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_current_user_id', []rt.PhpVal{}), rt.get_property(var_user, 'ID'))))))
	...
	if rt.is_true(var_locked) {
		...
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_url', [var_sendback_url.clone()]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_html', [var_sendback_text.clone()]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_url', [rt.call_function('add_query_arg', [rt.new_string('claim-lock'), rt.new_string('1'), rt.call_function('wp_nonce_url', [var_edit_url.clone(), rt.new_string('claim-lock-' + (rt.call_method(var_order_mutated, 'get_id', []rt.PhpVal{})).str())])])]))
		// unsupported statement: Stmt_InlineHTML
...
fn create_automattic_woocommerce_internal_admin_orders_editlock(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Orders_EditLock {
