...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout) needs_setup(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return !(!rt.is_true(rt.call_function('get_option', [
		rt.new_string('kco_credentials_error'),
	])))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('KCO_WC')])) {
		mut var_credentials := rt.get_property(rt.call_function('KCO_WC', []rt.PhpVal{}),
			'credentials')
		if rt.has_exception() {
			unsafe {
				goto catch_label_1
		...
		if var_credentials.clone().is_object()
			&& rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
			key: none
			val: var_credentials
		}, rt.ArrayItem{ key: none, val: 'get_credentials_from_session' }])]) {
			return !(!rt.is_true(rt.call_method(var_credentials, 'get_credentials_from_session',
				[]rt.PhpVal{})))
		}
		if rt.has_exception() {
			unsafe {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_klarnacheckout(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_KlarnaCheckout {
