...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins) construct() {
	rt.call_function('add_action', [
		rt.new_string('woocommerce_note_action_install-jp-and-wcs-plugins'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'install_jp_and_wcs_plugins' },
		]),
	])
	rt.call_function('add_action', [rt.new_string('activated_plugin'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'action_note' },
		])])
	rt.call_function('add_action', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'on_install_error' },
		]),
	])
	...
	rt.call_function('add_action', [rt.new_string('woocommerce_plugins_activate_error'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'on_install_error' },
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins.get_note() rt.PhpVal {
	mut var_content := rt.call_function('__', [
		rt.new_string('We noticed that there was a problem during the Jetpack and WooCommerce Shipping & Tax install. Please try again and enjoy all the advantages of having the plugins connected to your store! Sorry for the inconvenience. The "Jetpack" and "WooCommerce Shipping & Tax" plugins will be installed & activated for free.'),
		rt.new_string('woocommerce'),
	])
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	rt.call_method(var_note, 'set_title', [
		rt.call_function('__', [
			rt.new_string('Uh oh... There was a problem during the Jetpack and WooCommerce Shipping & Tax install. Please try again.'),
			rt.new_string('woocommerce'),
		]),
	])
	...
	rt.call_method(var_note, 'set_name', [
		Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins.note_name(),
	])
	rt.call_method(var_note, 'set_source', [rt.new_string('woocommerce-admin')])
	rt.call_method(var_note, 'add_action', [rt.new_string('install-jp-and-wcs-plugins'),
		rt.call_function('__', [rt.new_string('Install plugins'),
			rt.new_string('woocommerce')]),
		rt.new_bool(false), Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_actioned()])
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins.action_note() {
	...
	mut var_active_plugin_slugs := iife_result_0
	mut var_jp_active := rt.call_function('in_array', [rt.new_string('jetpack'),
		var_active_plugin_slugs.clone(), rt.new_bool(true)])
	mut var_wcs_active := rt.call_function('in_array', [
		rt.new_string('woocommerce-services'),
		var_active_plugin_slugs.clone(),
		rt.new_bool(true),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_jp_active))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(var_wcs_active)))) {
		return
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins) install_jp_and_wcs_plugins(var_note rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins.note_name(), rt.call_method(var_note_mutated,
		'get_name', []rt.PhpVal{})))))
	{
		return
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins) install_and_activate_plugin(var_plugin rt.PhpVal) {
	...
	mut var_installer :=
		create_automattic_woocommerce_internal_admin_notes_automattic_woocommerce_admin_api_onboardingplugins()
	mut var_result := var_installer.install_plugin(var_install_request.clone())
	if rt.is_true(rt.call_function('is_wp_error', [var_result.clone()])) {
		return
	}
	mut var_activate_request := rt.create_array([
		rt.ArrayItem{ key: 'plugins', val: var_plugin },
	])
	var_installer.activate_plugins(var_activate_request.clone())
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins) on_install_error(var_slug rt.PhpVal) {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('jetpack'), var_slug))))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('woocommerce-services'), var_slug)))) {
		return
	}
	mut iife_temp_3 := Class_Automattic_WooCommerce_Internal_Admin_Notes_InstallJPAndWCSPlugins{}
	mut iife_result_3 := iife_temp_3.possibly_add_note()
}
...
fn create_automattic_woocommerce_internal_admin_notes_automattic_woocommerce_admin_api_onboardingplugins(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Admin_API_OnboardingPlugins {
