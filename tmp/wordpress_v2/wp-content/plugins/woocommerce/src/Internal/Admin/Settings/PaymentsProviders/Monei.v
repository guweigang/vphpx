...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Monei) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	...
	}, rt.ArrayItem{ key: none, val: 'getAccountId' }])])
		&& rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
		key: none
		val: var_payment_gateway
	}, rt.ArrayItem{ key: none, val: 'getApiKey' }])]) {
		return !(!rt.is_true(var_payment_gateway.getaccountid()))
			&& !(!rt.is_true(var_payment_gateway.getapikey()))
	}
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_monei(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Monei {
