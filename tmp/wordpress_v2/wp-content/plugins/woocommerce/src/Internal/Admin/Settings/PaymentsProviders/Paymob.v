...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paymob) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_paymob_in_sandbox_mode(mut var_payment_gateway)).is_null() {
		this.is_paymob_in_sandbox_mode(mut var_payment_gateway)
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paymob) is_paymob_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	mut var_options := rt.call_function('get_option', [
		rt.new_string('woocommerce_paymob-main_settings'),
		rt.new_array(),
	])
	if rt.has_exception() {
		unsafe {
	...
	return (rt.identical(rt.new_string('test'), if !(var_options.array_get(rt.new_string('mode'))).is_null() {
		var_options.array_get(rt.new_string('mode'))
	} else {
		rt.new_string('test')
	})).to_bool()
	unsafe {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_paymob(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paymob {
