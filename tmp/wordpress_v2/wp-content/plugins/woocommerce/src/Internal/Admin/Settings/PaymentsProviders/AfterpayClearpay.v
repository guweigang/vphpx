...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	...
	}, rt.ArrayItem{ key: none, val: 'get_merchant_id' }])])
		&& rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
		key: none
		val: var_payment_gateway
	}, rt.ArrayItem{ key: none, val: 'get_secret_key' }])]) {
		return !(!rt.is_true(var_payment_gateway.get_merchant_id()))
			&& !(!rt.is_true(var_payment_gateway.get_secret_key()))
	}
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_afterpay_clearpay_in_sandbox_mode(mut var_payment_gateway)).is_null() {
		this.is_afterpay_clearpay_in_sandbox_mode(mut var_payment_gateway)
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay) is_afterpay_clearpay_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('is_callable', [
		rt.create_array([rt.ArrayItem{ key: none, val: var_payment_gateway },
			rt.ArrayItem{ key: none, val: 'get_api_env' }]),
	]))
	{
		return rt.new_bool(!rt.is_true(rt.identical(rt.new_string('production'),
			var_payment_gateway.get_api_env())))
	}
	if rt.has_exception() {
		unsafe {
			goto catch_label_2
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_afterpayclearpay(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_AfterpayClearpay {
