...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('defined', [rt.new_string('HELIO_DEVNET_ENABLED')])) {
		return (rt.call_function('wc_string_to_bool', [
			var_payment_gateway.get_option(rt.get_constant('HELIO_DEVNET_ENABLED')),
		])).to_bool()
	}
	if rt.has_exception() {
		unsafe {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if this.is_in_test_mode(mut var_payment_gateway) {
		if rt.is_true(rt.call_function('defined', [rt.new_string('HELIO_API_KEY_DEVNET')]))
			&& rt.is_true(rt.call_function('defined', [rt.new_string('HELIO_API_SECRET_DEVNET')])) {
			return
				!(!rt.is_true(var_payment_gateway.get_option(rt.get_constant('HELIO_API_KEY_DEVNET'))))
				&& !(!rt.is_true(var_payment_gateway.get_option(rt.get_constant('HELIO_API_SECRET_DEVNET'))))
		}
		if rt.has_exception() {
			unsafe {
				goto catch_label_2
	...
	} else if rt.is_true(rt.call_function('defined', [rt.new_string('HELIO_API_KEY_MAINNET')]))
		&& rt.is_true(rt.call_function('defined', [rt.new_string('HELIO_API_SECRET_MAINNET')])) {
		return
			!(!rt.is_true(var_payment_gateway.get_option(rt.get_constant('HELIO_API_KEY_MAINNET'))))
			&& !(!rt.is_true(var_payment_gateway.get_option(rt.get_constant('HELIO_API_SECRET_MAINNET'))))
	}
	if rt.has_exception() {
		unsafe {
			goto catch_label_2
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_heliopay(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_HelioPay {
