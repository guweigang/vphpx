...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WCCore) get_icon(mut var_payment_gateway Class_WC_Payment_Gateway) string {
	...
	if rt.is_true(rt.equal(switch_val_1, Class_WC_Gateway_BACS.id())) {
		return (rt.call_function('plugins_url', [
			rt.new_string('assets/images/payment_methods/bacs.svg'),
			rt.get_constant('WC_PLUGIN_FILE'),
		])).str()
	} else if rt.is_true(rt.equal(switch_val_1, Class_WC_Gateway_Cheque.id())) {
		return (rt.call_function('plugins_url', [
			rt.new_string('assets/images/payment_methods/cheque.svg'),
			rt.get_constant('WC_PLUGIN_FILE'),
		])).str()
		...
		return (rt.call_function('plugins_url', [
			rt.new_string('assets/images/payment_methods/72x72/paypal.png'),
			rt.get_constant('WC_PLUGIN_FILE'),
		])).str()
	}
	return (this.Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PaymentGateway.get_icon(rt.new_object('WC_Payment_Gateway',
		[]string{}, var_payment_gateway))).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WCCore) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	...
	if rt.is_true(rt.equal(switch_val_2, Class_WC_Gateway_BACS.id())) {
		return
			rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('account_details')]))
			&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'account_details')))
	} else if rt.is_true(rt.equal(switch_val_2, Class_WC_Gateway_Cheque.id()))
		|| rt.is_true(rt.equal(switch_val_2, Class_WC_Gateway_COD.id())) {
		return true
	} else if rt.is_true(rt.equal(switch_val_2, Class_WC_Gateway_Paypal.id())) {
		return
			rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('email')]))
			&& rt.is_true(rt.call_function('is_email', [rt.get_property(var_payment_gateway, 'email')]))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WCCore) is_in_test_mode_onboarding(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	...
	if rt.is_true(rt.equal(switch_val_3, Class_WC_Gateway_BACS.id()))
		|| rt.is_true(rt.equal(switch_val_3, Class_WC_Gateway_Cheque.id()))
		|| rt.is_true(rt.equal(switch_val_3, Class_WC_Gateway_COD.id())) {
		return false
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WCCore) get_plugin_details(mut var_payment_gateway Class_WC_Payment_Gateway) rt.PhpVal {
	mut var_plugin_details := this.Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PaymentGateway.get_plugin_details(rt.new_object('WC_Payment_Gateway',
		[]string{}, var_payment_gateway))
	var_plugin_details.array_set('file', '')
	return var_plugin_details.clone()
}
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_wccore(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WCCore {
