...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_MagentoMigration) construct() {
	...
	rt.call_function('add_action', [
		rt.new_string('woocommerce_admin_magento_migration_note'),
		rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT },
			rt.ArrayItem{ key: none, val: 'save_note' }]),
	])
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_MagentoMigration.possibly_add_note() {
	...
	if !(var_onboarding_profile.array_isset(rt.new_string('other_platform')))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('magento'), var_onboarding_profile.array_get(rt.new_string('other_platform')))))) {
		return
	}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_MagentoMigration.save_note() {
	...
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Admin_Notes_MagentoMigration{}
	mut iife_result_0 := iife_temp_0.note_exists()
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_MagentoMigration.get_note() rt.PhpVal {
	...
	rt.call_method(var_note, 'set_title', [
		rt.call_function('__', [
			rt.new_string('How to Migrate from Magento to WooCommerce'),
			rt.new_string('woocommerce'),
		]),
	])
	rt.call_method(var_note, 'set_content', [
		rt.call_function('__', [
			rt.new_string('Changing platforms might seem like a big hurdle to overcome, but it is easier than you might think to move your products, customers, and orders to WooCommerce. This article will help you with going through this process.'),
			rt.new_string('woocommerce'),
		]),
	])
	...
	rt.call_method(var_note, 'set_name', [
		Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_MagentoMigration.note_name(),
	])
	...
	rt.call_method(var_note, 'add_action', [rt.new_string('learn-more'),
		...
		rt.new_string('https://woocommerce.com/posts/how-migrate-from-magento-to-woocommerce/?utm_source=inbox')])
