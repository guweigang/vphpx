...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PayPal) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_paypal_in_sandbox_mode(mut var_payment_gateway)).is_null() {
		this.is_paypal_in_sandbox_mode(mut var_payment_gateway)
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PayPal) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_paypal_onboarded(mut var_payment_gateway)).is_null() {
		this.is_paypal_onboarded(mut var_payment_gateway)
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PayPal) is_paypal_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('\\WooCommerce\\PayPalCommerce\\PPCP')]))
		&& rt.call_function('is_callable', [rt.new_string('\\WooCommerce\\PayPalCommerce\\PPCP::container')]) {
		mut iife_temp_0 :=
			Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooCommerce_PayPalCommerce_PPCP{}
		mut iife_result_0 := iife_temp_0.container()
		mut var_container := iife_result_0
		if rt.has_exception() {
			unsafe {
				goto catch_label_1
			}
		}
		if rt.is_true(rt.call_method(var_container, 'has', [
			rt.new_string('settings.connection-state'),
		]))
		{
			mut var_state := rt.call_method(var_container, 'get', [
				rt.new_string('settings.connection-state'),
			])
			if rt.has_exception() {
				unsafe {
		...
		if rt.is_true(rt.call_method(var_container, 'has', [rt.new_string('onboarding.environment')]))
			&& rt.is_true(rt.call_function('defined', [rt.new_string('\\WooCommerce\\PayPalCommerce\\Onboarding\\Environment::SANDBOX')])) {
			mut var_environment := rt.call_method(var_container, 'get', [
				rt.new_string('onboarding.environment'),
			])
			if rt.has_exception() {
				unsafe {
					goto catch_label_1
			...
			mut var_current_environment := rt.call_method(var_environment, 'current_environment',
				[]rt.PhpVal{})
			if rt.has_exception() {
				unsafe {
					goto catch_label_1
			...
			return (rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooCommerce_PayPalCommerce_Onboarding_Environment.sandbox(),
				var_current_environment)).to_bool()
		}
		if rt.has_exception() {
			unsafe {
				goto catch_label_1
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PayPal) is_paypal_onboarded(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('\\WooCommerce\\PayPalCommerce\\PPCP')]))
		...
		mut var_container := iife_result_2
		if rt.has_exception() {
			unsafe {
				goto catch_label_2
			}
		...
		if rt.is_true(rt.call_method(var_container, 'has', [rt.new_string('onboarding.state')]))
			&& rt.is_true(rt.call_function('defined', [rt.new_string('\\WooCommerce\\PayPalCommerce\\Onboarding\\State::STATE_ONBOARDED')])) {
			var_state = rt.call_method(var_container, 'get', [
				rt.new_string('onboarding.state'),
			])
			if rt.has_exception() {
				unsafe {
					goto catch_label_2
			...
			return (rt.greater_equal(rt.call_method(var_state, 'current_state', []rt.PhpVal{}),
				Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooCommerce_PayPalCommerce_Onboarding_State.state_onboarded())).to_bool()
		}
		if rt.has_exception() {
			unsafe {
				goto catch_label_2
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_paypal(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_PayPal {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_woocommerce_paypalcommerce_ppcp(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooCommerce_PayPalCommerce_PPCP {
