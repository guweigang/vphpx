...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreviewRestController) register_routes() {
	...
	rt.call_function('register_rest_route', [this.route_namespace,
		rt.new_string('/' + (this.rest_base).str() + '/send-preview'),
		rt.create_array([
	...
	closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		return rt.create_array([
			rt.ArrayItem{ key: 'subject', val: rt.call_method(this.email_preview, 'get_subject',
				[]rt.PhpVal{}) },
		])
	}
	...
	rt.call_function('register_rest_route', [this.route_namespace,
		rt.new_string('/' + (this.rest_base).str() + '/preview-subject'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{
					key: 'methods'
	...
	rt.call_function('register_rest_route', [this.route_namespace,
		rt.new_string('/' + (this.rest_base).str() + '/save-transient'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{
					key: 'methods'
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreviewRestController) get_args_for_save_transient() rt.PhpVal {
	closure_10_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut iife_result_9 := iife_temp_9.get_all_email_setting_ids()
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
			var_key.clone(), iife_result_9, rt.new_bool(true)])))))
		{
	...
	closure_11_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_key := rt.call_method(var_request, 'get_param', [
			rt.new_string('key')])
		if rt.is_true(rt.identical(rt.new_string('woocommerce_email_footer_text'), var_key))
			|| rt.is_true(rt.call_function('preg_match', [rt.new_string('/_additional_content$/'), var_key.clone()])) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreviewRestController) send_email_preview(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	mut var_sent := var_email.send(var_email_address.clone(), var_email_subject.clone(),
		var_email_content.clone())
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_EmailPreviewRestController) save_transient(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	mut var_is_set := rt.call_function('set_transient', [var_key.clone(),
		var_value.clone(), rt.get_constant('HOUR_IN_SECONDS')])
...
fn create_automattic_woocommerce_internal_admin_emailpreview_wc_emails(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_EmailPreview_WC_Emails {
