...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_EmailImprovements.get_note() rt.PhpVal {
	mut iife_temp_0 :=
		Class_Automattic_WooCommerce_Internal_Admin_EmailImprovements_EmailImprovements{}
	mut iife_result_0 := iife_temp_0.is_email_improvements_enabled_for_existing_stores()
	...
	mut iife_temp_1 :=
		Class_Automattic_WooCommerce_Internal_Admin_EmailImprovements_EmailImprovements{}
	mut iife_result_1 := iife_temp_1.should_notify_merchant_about_email_improvements()
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_EmailImprovements.get_email_improvements_enabled_note() rt.PhpVal {
	...
	var_note.set_title(rt.call_function('__', [
		rt.new_string('Your store emails have had an upgrade!'),
		rt.new_string('woocommerce'),
	]))
	var_note.set_content(rt.call_function('__', [
		rt.new_string('We’ve made some exciting improvements to your email templates, including modern, shopper-friendly designs and new customization options. And if you’re using a block theme, you can automatically sync your theme styles! Head to your email settings to explore the new changes.'),
		rt.new_string('woocommerce'),
	]))
	...
	var_note.add_action(rt.new_string('customize-your-emails'), rt.call_function('__', [
		rt.new_string('Customize your emails'),
		rt.new_string('woocommerce'),
	]), rt.new_string('?page=wc-settings&tab=email'))
	return mut var_note
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_EmailImprovements.get_try_email_improvements_note() rt.PhpVal {
	mut var_note := create_automattic_woocommerce_admin_notes_note()
	var_note.set_title(rt.call_function('__', [
		rt.new_string('Store emails have had an upgrade!'),
		rt.new_string('woocommerce'),
	]))
	var_note.set_content(rt.call_function('__', [
		rt.new_string('We’ve made some exciting improvements to our email templates, including modern, shopper-friendly designs and new customization options. And if you’re using a block theme, you can automatically sync your theme styles! Head to your email settings to explore the new features.'),
		rt.new_string('woocommerce'),
	]))
	var_note.set_type(Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_informational())
	var_note.set_name(Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_EmailImprovements.note_name())
	var_note.set_source(rt.new_string('woocommerce-admin'))
	var_note.add_action(rt.new_string('try-the-new-templates'), rt.call_function('__', [
		rt.new_string('Try the new templates'),
		rt.new_string('woocommerce'),
	]), rt.new_string('?page=wc-settings&tab=email&try-new-templates'))
	return mut var_note
}
...
fn create_automattic_woocommerce_internal_admin_notes_emailimprovements(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Notes_EmailImprovements {
