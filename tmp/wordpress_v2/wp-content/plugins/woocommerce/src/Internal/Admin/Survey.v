...
fn Class_Automattic_WooCommerce_Internal_Admin_Survey.get_url(var_path rt.PhpVal, var_query rt.PhpVal) rt.PhpVal {
	mut var_url := rt.new_string(
		(Class_Automattic_WooCommerce_Internal_Admin_Automattic_WooCommerce_Internal_Admin_Survey.survey_url()).str() +
		var_path.str())
	mut var_query_args := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_admin_survey_query'),
		var_query.clone(),
	])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Survey) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'get_url' {
			...
			return Class_Automattic_WooCommerce_Internal_Admin_Survey.get_url(dispatch_arg_0,
				dispatch_arg_1)
		}
		else {
			return none
