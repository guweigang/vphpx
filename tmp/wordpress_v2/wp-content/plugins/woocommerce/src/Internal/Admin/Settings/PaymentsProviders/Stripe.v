...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Stripe) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('\\WC_Stripe_Mode')]))
		&& rt.call_function('is_callable', [rt.new_string('\\WC_Stripe_Mode::is_test')]) {
		mut iife_temp_0 :=
			Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WC_Stripe_Mode{}
		mut iife_result_0 := iife_temp_0.is_test()
		mut iife_temp_1 :=
			Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WC_Stripe_Mode{}
		mut iife_result_1 := iife_temp_1.is_test()
		return (rt.call_function('wc_string_to_bool', [iife_result_0])).to_bool()
	}
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Stripe) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('\\WC_Stripe')]))
		&& rt.call_function('is_callable', [rt.new_string('\\WC_Stripe::get_instance')]) {
		mut iife_temp_3 :=
			Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WC_Stripe{}
		mut iife_result_3 := iife_temp_3.get_instance()
		mut var_stripe := iife_result_3
		if rt.has_exception() {
			unsafe {
				goto catch_label_2
			}
		}
		if var_stripe.clone().is_object() && !(rt.get_property(var_stripe, 'account')).is_null()
			&& rt.is_true(rt.call_function('class_exists', [rt.new_string('\\WC_Stripe_Account')]))
			&& rt.is_true(rt.call_function('defined', [rt.new_string('\\WC_Stripe_Account::STATUS_NO_ACCOUNT')]))
			&& rt.is_true(rt.new_bool(rt.instance_of(rt.get_property(var_stripe, 'account'), 'Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WC_Stripe_Account')))
			&& rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
			key: none
			val: rt.get_property(var_stripe, 'account')
		}, rt.ArrayItem{ key: none, val: 'get_account_status' }])]) {
			return rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WC_Stripe_Account.status_no_account(), rt.call_method(rt.get_property(var_stripe,
				'account'), 'get_account_status', []rt.PhpVal{}))))
		}
		if rt.has_exception() {
			unsafe {
				goto catch_label_2
	...
	if rt.instance_of(var_e_2, 'Throwable') {
		...
		rt.call_method(iife_result_4, 'debug', [
			rt.new_string('Failed to determine if gateway has account connected: ' +
				(rt.call_method(var_e, 'getMessage', []rt.PhpVal{})).str()),
			rt.create_array([
				rt.ArrayItem{ key: 'gateway', val: rt.get_property(var_payment_gateway, 'id') },
				rt.ArrayItem{ key: 'source', val: 'settings-payments' },
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Stripe) is_in_test_mode_onboarding(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('\\WC_Stripe')]))
		...
		mut var_stripe := iife_result_5
		if rt.has_exception() {
			unsafe {
				goto catch_label_3
			}
		}
		if var_stripe.clone().is_object() && !(rt.get_property(var_stripe, 'connect')).is_null()
			&& rt.is_true(rt.call_function('class_exists', [rt.new_string('\\WC_Stripe_Connect')]))
			&& rt.is_true(rt.new_bool(rt.instance_of(rt.get_property(var_stripe, 'connect'), 'Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WC_Stripe_Connect')))
			&& rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
			key: none
			val: rt.get_property(var_stripe, 'connect')
		}, rt.ArrayItem{ key: none, val: 'is_connected' }])]) {
			return
				rt.is_true(rt.call_method(rt.get_property(var_stripe, 'connect'), 'is_connected', [rt.new_string('test')]))
				&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(rt.get_property(var_stripe, 'connect'), 'is_connected', [rt.new_string('live')])))))
		}
		if rt.has_exception() {
			unsafe {
				goto catch_label_3
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Stripe) get_settings_url(mut var_payment_gateway Class_WC_Payment_Gateway) string {
	mut iife_temp_7 := Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils{}
	mut iife_result_7 := iife_temp_7.wc_payments_settings_url(rt.new_null(), rt.create_array([
		rt.ArrayItem{
			key: 'section'
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_stripe(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Stripe {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_wc_stripe_mode(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WC_Stripe_Mode {
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_wc_stripe(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WC_Stripe {
