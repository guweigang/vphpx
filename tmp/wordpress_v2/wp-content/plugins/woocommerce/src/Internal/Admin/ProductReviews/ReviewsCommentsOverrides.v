...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductReviews_ReviewsCommentsOverrides) construct() {
	...
	rt.call_function('add_filter', [
		rt.new_string('woocommerce_dismiss_admin_notice_capability'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_ProductReviews_ReviewsCommentsOverrides',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'get_dismiss_capability' },
		]),
		rt.new_int(10),
		rt.new_int(2),
	])
	rt.call_function('add_filter', [rt.new_string('comments_list_table_query_args'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_ProductReviews_ReviewsCommentsOverrides',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'exclude_reviews_from_comments' },
		])])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductReviews_ReviewsCommentsOverrides) display_notices() {
	...
	if !rt.is_true(var_screen)
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(var_screen, 'base'), rt.new_string('edit-comments'))))) {
		return
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductReviews_ReviewsCommentsOverrides) should_display_reviews_moved_notice() bool {
	...
	mut iife_result_0 := iife_temp_0.get_capability()
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(rt.call_function('WC', []rt.PhpVal{}),
		'call_function', [rt.new_string('current_user_can'), iife_result_0])))))
	{
		return false
	}
	if rt.is_true(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'call_function', [
		rt.new_string('get_user_meta'),
		rt.call_function('get_current_user_id', []rt.PhpVal{}),
		rt.new_string('dismissed_' +
			(Class_Automattic_WooCommerce_Internal_Admin_ProductReviews_static.reviews_moved_notice_id()).str() + '_notice'),
		rt.new_bool(true),
	]))
	{
		return false
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductReviews_ReviewsCommentsOverrides) display_reviews_moved_notice() {
	...
	rt.call_function('esc_html_e', [rt.new_string('Product reviews have moved!'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [
		rt.new_string('Product reviews can now be managed from Products > Reviews.'),
		rt.new_string('woocommerce'),
	])
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Visit new location'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_url', [
	...
	rt.echo_val(rt.call_function('esc_attr', [
		Class_Automattic_WooCommerce_Internal_Admin_ProductReviews_static.reviews_moved_notice_id(),
	]))
	// unsupported statement: Stmt_InlineHTML
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('comment_status')))) {
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [
			rt.get_superglobal('_GET').array_get(rt.new_string('comment_status')),
		]))
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('paged')))) {
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr',
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('wp_nonce_field', [rt.new_string('woocommerce_hide_notices_nonce'),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductReviews_ReviewsCommentsOverrides) get_dismiss_capability(var_default_capability rt.PhpVal, var_notice_name rt.PhpVal) rt.PhpVal {
	...
	mut iife_result_1 := iife_temp_1.get_capability()
	return if rt.is_true(rt.identical(var_notice_name,
		Class_Automattic_WooCommerce_Internal_Admin_ProductReviews_Automattic_WooCommerce_Internal_Admin_ProductReviews_ReviewsCommentsOverrides.reviews_moved_notice_id()))
	{
		iife_result_1
	} else {
		var_default_capability
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_ProductReviews_ReviewsCommentsOverrides) exclude_reviews_from_comments(var_args rt.PhpVal) rt.PhpVal {
	...
	mut var_screen := rt.call_function('get_current_screen', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_screen, 'WP_Screen'))))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('edit-comments'), rt.get_property(var_screen, 'id'))))) {
		return var_args_mutated.clone()
	}
	if !(!rt.is_true(var_args_mutated.array_get(rt.new_string('post_type'))))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_args_mutated.array_get(rt.new_string('post_type')), rt.new_string('any'))))) {
		mut var_post_types := rt.cast_array(var_args_mutated.array_get(rt.new_string('post_type')))
	} else {
		var_post_types = rt.call_function('get_post_types', []rt.PhpVal{})
	}
	mut var_index := rt.call_function('array_search', [rt.new_string('product'),
	...
	var_args_mutated.array_set('post_type', var_post_types.clone())
	return var_args_mutated.clone()
}
