...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_MercadoPago) needs_setup(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	...
	if !(var_is_onboarded.clone().is_null()) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_MercadoPago) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_mercado_pago_in_sandbox_mode(mut var_payment_gateway)).is_null() {
		this.is_mercado_pago_in_sandbox_mode(mut var_payment_gateway)
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_MercadoPago) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (if !(this.is_mercado_pago_onboarded(mut var_payment_gateway)).is_null() {
		this.is_mercado_pago_onboarded(mut var_payment_gateway)
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_MercadoPago) is_mercado_pago_in_sandbox_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	...
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('\\MercadoPago\\Woocommerce\\WoocommerceMercadoPago')]))
		&& rt.is_true(rt.call_function('class_exists', [rt.new_string('\\MercadoPago\\Woocommerce\\Configs\\Store')]))
		&& rt.is_true(rt.new_bool(rt.instance_of(var_mercadopago, 'Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_MercadoPago_Woocommerce_WoocommerceMercadoPago')))
		&& !(rt.get_property(var_mercadopago, 'storeConfig').is_null())
		&& rt.is_true(rt.new_bool(rt.instance_of(rt.get_property(var_mercadopago, 'storeConfig'), 'Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_MercadoPago_Woocommerce_Configs_Store')))
		&& rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
		key: none
		val: rt.get_property(var_mercadopago, 'storeConfig')
	}, rt.ArrayItem{ key: none, val: 'isTestMode' }])]) {
		return (rt.call_function('wc_string_to_bool', [
			rt.call_method(rt.get_property(var_mercadopago, 'storeConfig'), 'isTestMode',
				[]rt.PhpVal{}),
		])).to_bool()
	}
	if rt.has_exception() {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_MercadoPago) is_mercado_pago_onboarded(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	mut var_mercadopago := rt.new_null()
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('\\MercadoPago\\Woocommerce\\WoocommerceMercadoPago')]))
		&& rt.is_true(rt.call_function('class_exists', [rt.new_string('\\MercadoPago\\Woocommerce\\Configs\\Seller')]))
		&& rt.is_true(rt.new_bool(rt.instance_of(var_mercadopago, 'Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_MercadoPago_Woocommerce_WoocommerceMercadoPago')))
		&& !(rt.get_property(var_mercadopago, 'sellerConfig').is_null())
		&& rt.is_true(rt.new_bool(rt.instance_of(rt.get_property(var_mercadopago, 'sellerConfig'), 'Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_MercadoPago_Woocommerce_Configs_Seller')))
		&& rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
		key: none
		val: rt.get_property(var_mercadopago, 'sellerConfig')
	}, rt.ArrayItem{ key: none, val: 'getCredentialsPublicKey' }])])
		&& rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
		key: none
		val: rt.get_property(var_mercadopago, 'sellerConfig')
	}, rt.ArrayItem{ key: none, val: 'getCredentialsAccessToken' }])]) {
		return
			!(!rt.is_true(rt.call_method(rt.get_property(var_mercadopago, 'sellerConfig'), 'getCredentialsPublicKey', []rt.PhpVal{})))
			&& !(!rt.is_true(rt.call_method(rt.get_property(var_mercadopago, 'sellerConfig'), 'getCredentialsAccessToken', []rt.PhpVal{})))
	}
	if rt.has_exception() {
		unsafe {
			goto catch_label_2
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_mercadopago(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_MercadoPago {
