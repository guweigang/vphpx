...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_ScheduledUpdatesPromotion) construct() {
	rt.call_function('add_action', [
		rt.new_string('woocommerce_note_action_scheduled-updates-enable'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Notes_ScheduledUpdatesPromotion',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'enable_scheduled_updates' },
		]),
	])
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_ScheduledUpdatesPromotion.is_applicable() bool {
	...
	mut iife_result_0 := iife_temp_0.is_enabled(rt.new_string('analytics-scheduled-import'))
	if rt.is_true(rt.new_bool(!(rt.is_true(iife_result_0)))) {
		return false
	}
	mut var_immediate_import := rt.call_function('get_option', [
		Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_ScheduledUpdatesPromotion.option_name(),
		rt.new_bool(false),
	])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_immediate_import)))) {
		return false
	}
	return true
}
...
fn Class_Automattic_WooCommerce_Internal_Admin_Notes_ScheduledUpdatesPromotion.get_note() rt.PhpVal {
	...
	var_note.set_title(rt.call_function('__', [
		rt.new_string('Analytics now supports scheduled updates'),
		rt.new_string('woocommerce'),
	]))
	var_note.set_content(rt.call_function('__', [
		rt.new_string('This provides improved performance to your store, enable it in Analytics > Settings.'),
		rt.new_string('woocommerce'),
	]))
	...
	var_note.add_action(rt.new_string('scheduled-updates-enable'), rt.call_function('__', [
		...
		rt.new_string('Scheduled updates enabled'),
		rt.new_string('woocommerce'),
	]))
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Notes_ScheduledUpdatesPromotion) enable_scheduled_updates(var_note rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Notes_Automattic_WooCommerce_Internal_Admin_Notes_ScheduledUpdatesPromotion.note_name(),
