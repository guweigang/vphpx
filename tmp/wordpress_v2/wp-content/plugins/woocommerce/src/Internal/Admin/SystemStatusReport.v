...
fn init_static_automattic_woocommerce_internal_admin_systemstatusreport() {
	rt.init_static_prop('Automattic_WooCommerce_Internal_Admin_SystemStatusReport', 'instance',
		rt.new_null())
}

fn Class_Automattic_WooCommerce_Internal_Admin_SystemStatusReport.get_instance() rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.get_static_prop('Automattic_WooCommerce_Internal_Admin_SystemStatusReport',
		'instance')))))
	{
		rt.set_static_prop('Automattic_WooCommerce_Internal_Admin_SystemStatusReport', 'instance', rt.new_object('Automattic_WooCommerce_Internal_Admin_self',
			[]string{}, create_automattic_woocommerce_internal_admin_self()))
	}
	return rt.get_static_prop('Automattic_WooCommerce_Internal_Admin_SystemStatusReport',
		'instance')
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_SystemStatusReport) construct() {
	rt.call_function('add_action', [rt.new_string('woocommerce_system_status_report'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_SystemStatusReport',
				[]string{}, &this) },
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_SystemStatusReport) system_status_report() {
	...
	rt.echo_val(rt.call_function('wc_help_tip', [
		rt.call_function('esc_html__', [
			rt.new_string('This section shows details of WC Admin.'),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_SystemStatusReport) render_features() {
	mut var_features := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_admin_get_feature_config'),
		rt.call_function('wc_admin_get_feature_config', []rt.PhpVal{}),
	])
	mut var_enabled_features := rt.call_function('array_filter', [
		var_features.clone()])
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_feature := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return
	}
	mut var_disabled_features := rt.call_function('array_filter', [
		var_features.clone(), rt.new_closure(closure_1_fn)])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Enabled Features'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wc_help_tip', [
		rt.call_function('esc_html__', [rt.new_string('Which features are enabled?'),
			rt.new_string('woocommerce')]),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [
		rt.call_function('implode', [rt.new_string(', '),
			rt.func_array_keys(var_enabled_features.clone())]),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Disabled Features'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wc_help_tip', [
		rt.call_function('esc_html__', [rt.new_string('Which features are disabled?'),
			rt.new_string('woocommerce')]),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [
		rt.call_function('implode', [rt.new_string(', '),
			rt.func_array_keys(var_disabled_features.clone())]),
	]))
	// unsupported statement: Stmt_InlineHTML
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_SystemStatusReport) render_daily_cron() {
	mut var_next_daily_cron := rt.call_function('wp_next_scheduled', [
		rt.new_string('wc_admin_daily'),
	])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Daily Cron'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wc_help_tip', [
		rt.call_function('esc_html__', [
			rt.new_string('Is the daily cron job active, when does it next run?'),
			rt.new_string('woocommerce'),
		]),
	]))
	// unsupported statement: Stmt_InlineHTML
	if !rt.is_true(var_next_daily_cron) {
		print('<mark class="error"><span class="dashicons dashicons-warning"></span> ' +
			(rt.call_function('esc_html__', [rt.new_string('Not scheduled'), rt.new_string('woocommerce')])).str() +
			'</mark>')
	} else {
		print('<mark class="yes"><span class="dashicons dashicons-yes"></span> Next scheduled: ' +
			(rt.call_function('esc_html', [rt.call_function('date_i18n', [rt.new_string('Y-m-d H:i:s P'), var_next_daily_cron.clone()])])).str() +
			'</mark>')
	}
	// unsupported statement: Stmt_InlineHTML
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_SystemStatusReport) render_options() {
	mut var_woocommerce_admin_install_timestamp := rt.call_function('get_option', [
		rt.new_string('woocommerce_admin_install_timestamp'),
	])
	mut var_all_options_expected := rt.new_bool(
		var_woocommerce_admin_install_timestamp.clone().is_long()
		|| var_woocommerce_admin_install_timestamp.clone().is_double()
		&& 0 < rt.new_int(var_woocommerce_admin_install_timestamp.to_i64())
		&& rt.call_function('get_option', [rt.new_string('woocommerce_onboarding_profile'), rt.new_array()]).is_array())
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Options'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wc_help_tip', [
		rt.call_function('esc_html__', [
			rt.new_string('Do the important options return expected values?'),
			rt.new_string('woocommerce'),
		]),
	]))
	// unsupported statement: Stmt_InlineHTML
	if rt.is_true(var_all_options_expected) {
		...
		print('<mark class="error"><span class="dashicons dashicons-warning"></span> ' +
			(rt.call_function('esc_html__', [rt.new_string('Not all expected'), rt.new_string('woocommerce')])).str() +
			'</mark>')
	}
	// unsupported statement: Stmt_InlineHTML
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_SystemStatusReport) render_notes() {
	...
	mut iife_result_1 := iife_temp_1.get_notes_count()
	mut var_notes_count := iife_result_1
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Notes'), rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wc_help_tip', [
		rt.call_function('esc_html__', [rt.new_string('How many notes in the database?'),
			rt.new_string('woocommerce')]),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_notes_count.clone()]))
	// unsupported statement: Stmt_InlineHTML
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_SystemStatusReport) render_onboarding_state() {
	...
	mut var_onboarding_state := rt.new_string('-')
	if var_onboarding_profile.array_isset(rt.new_string('skipped'))
		&& rt.is_true(var_onboarding_profile.array_get(rt.new_string('skipped'))) {
		var_onboarding_state = rt.new_string('skipped')
	}
	if var_onboarding_profile.array_isset(rt.new_string('completed'))
		&& rt.is_true(var_onboarding_profile.array_get(rt.new_string('completed'))) {
		var_onboarding_state = rt.new_string('completed')
	}
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Onboarding'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wc_help_tip', [
		rt.call_function('esc_html__', [
			rt.new_string('Was onboarding completed or skipped?'),
			rt.new_string('woocommerce'),
		]),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_onboarding_state.clone()]))
	// unsupported statement: Stmt_InlineHTML
}
