...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager) init(mut var_payload_builder Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder) {
	this.payload_builder = var_payload_builder
}

fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager) register() {
	rt.call_function('add_filter', [rt.new_string('woocommerce_webhook_topics'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager', [
				'RegisterHooksInterface',
			], &this) },
			rt.ArrayItem{ key: none, val: 'register_webhook_topic_names' },
		])])
	rt.call_function('add_action', [rt.new_string('woocommerce_new_order'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'handle_order_created' },
		]),
		rt.new_int(999), rt.new_int(2)])
	rt.call_function('add_action', [rt.new_string('woocommerce_order_status_changed'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager', [
				'RegisterHooksInterface',
			], &this) },
			rt.ArrayItem{ key: none, val: 'handle_order_status_changed' },
		]),
	...
	rt.call_function('add_action', [rt.new_string('woocommerce_order_refunded'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'handle_order_refunded' },
		]),
		rt.new_int(10), rt.new_int(1)])
	rt.call_function('add_filter', [rt.new_string('woocommerce_webhook_payload'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager', [
				'RegisterHooksInterface',
			], &this) },
			rt.ArrayItem{ key: none, val: 'customize_webhook_payload' },
		]),
		rt.new_int(10), rt.new_int(4)])
	rt.call_function('add_filter', [rt.new_string('woocommerce_webhook_http_args'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'customize_webhook_http_args' },
		]),
		rt.new_int(10), rt.new_int(3)])
	rt.call_function('add_action', [rt.new_string('woocommerce_webhook_delivery'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager', [
				'RegisterHooksInterface',
			], &this) },
			rt.ArrayItem{ key: none, val: 'mark_first_event_delivered' },
		]),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager) register_webhook_topic_names(var_topics rt.PhpVal) rt.PhpVal {
	...
	var_topics_mutated.array_set(Class_Automattic_WooCommerce_Internal_Admin_Agentic_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager.webhook_topic(), rt.call_function('__', [
		rt.new_string('Agentic Commerce Protocol: Order created or updated'),
		rt.new_string('woocommerce'),
	]))
	return var_topics_mutated.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager) handle_order_created(var_order_id rt.PhpVal, var_order rt.PhpVal) {
	...
	if !(this.should_trigger_webhook(var_order_mutated.clone())) {
		return
	}
	rt.call_function('do_action', [
		Class_Automattic_WooCommerce_Internal_Admin_Agentic_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager.webhook_action(),
		var_order_id.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager) handle_order_refunded(var_order_id rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order))))
		|| !(this.should_trigger_webhook(var_order.clone())) {
		return
	}
	rt.call_function('do_action', [
		Class_Automattic_WooCommerce_Internal_Admin_Agentic_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager.webhook_action(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager) should_trigger_webhook(var_order rt.PhpVal) bool {
	mut var_order_mutated := var_order
	mut var_checkout_session_id := rt.call_method(var_order_mutated, 'get_meta', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager) customize_webhook_payload(var_payload rt.PhpVal, var_resource_type rt.PhpVal, var_resource_id rt.PhpVal, var_webhook_id rt.PhpVal) rt.PhpVal {
	mut var_webhook := rt.call_function('wc_get_webhook', [var_webhook_id.clone()])
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Agentic_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager.webhook_topic(),
		var_topic))))
	{
		return var_payload.clone()
	}
	mut var_order := rt.call_function('wc_get_order', [var_resource_id.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order)))) {
		return var_payload.clone()
	}
	mut var_is_first_event := rt.new_bool(!rt.is_true(rt.identical(rt.new_string('sent'), rt.call_method(var_order,
		'get_meta', [
		Class_Automattic_WooCommerce_Internal_Admin_Agentic_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager.first_event_delivered_meta_key(),
	]))))
	mut var_event := rt.new_string((if rt.is_true(var_is_first_event) {
		'order_create'
	} else {
		'order_update'
	}).str())
	return rt.call_method(this.payload_builder, 'build_payload', [
		var_event.clone(), var_order.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager) customize_webhook_http_args(var_http_args rt.PhpVal, var_arg rt.PhpVal, var_webhook_id rt.PhpVal) rt.PhpVal {
	...
	if var_http_args.array_isset(rt.new_string('body'))
		&& !(!rt.is_true(rt.call_method(var_webhook, 'get_secret', []rt.PhpVal{}))) {
		mut var_signature := rt.call_method(var_webhook, 'generate_signature', [
			var_http_args.array_get(rt.new_string('body')),
		])
		var_http_args.array_get_mut('headers').array_set('Merchant-Signature',
			var_signature.clone())
	}
	return var_http_args.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager) mark_first_event_delivered(var_http_args rt.PhpVal, var_response rt.PhpVal, var_duration rt.PhpVal, var_arg rt.PhpVal, var_webhook_id rt.PhpVal) {
	...
	mut var_code := rt.call_function('wp_remote_retrieve_response_code', [
		var_response.clone()])
	if rt.is_true(rt.less(var_code, rt.new_int(200)))
		|| rt.is_true(rt.greater_equal(var_code, rt.new_int(300))) {
		return
	}
	mut var_webhook := rt.call_function('wc_get_webhook', [var_webhook_id.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_webhook))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Agentic_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager.webhook_topic(), rt.call_method(var_webhook, 'get_topic', []rt.PhpVal{}))))) {
		return
	}
	mut var_order := rt.call_function('wc_get_order', [var_arg.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order)))) {
		return
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('sent'), rt.call_method(var_order,
		'get_meta', [
		Class_Automattic_WooCommerce_Internal_Admin_Agentic_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager.first_event_delivered_meta_key(),
	])))))
	{
...
fn create_automattic_woocommerce_internal_admin_agentic_agenticwebhookmanager(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookManager) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Admin_Agentic_AgenticWebhookPayloadBuilder](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
		...
		'handle_order_status_changed' {
			...
			this.handle_order_status_changed(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2,
				dispatch_arg_3)
			return rt.new_null()
		}
		...
		'customize_webhook_payload' {
			...
			dispatch_arg_2 := if args.len > 2 { args[2] } else { rt.new_null() }
			dispatch_arg_3 := if args.len > 3 { args[3] } else { rt.new_null() }
			return this.customize_webhook_payload(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2,
				dispatch_arg_3)
		}
		...
		'mark_first_event_delivered' {
			...
			this.mark_first_event_delivered(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2,
