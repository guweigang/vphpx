...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail) is_in_test_mode(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return (rt.call_function('wc_string_to_bool', [
		var_payment_gateway.get_option(rt.new_string('enable_test_mode'), rt.new_string('no')),
	])).to_bool()
	unsafe {
		goto end_label_1
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	...
	return !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('merchant_id'))))
		&& !(!rt.is_true(var_payment_gateway.get_option(rt.new_string('secret_key'))))
	unsafe {
		goto end_label_2
	}
	catch_label_2:
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_paytrail(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Paytrail {
