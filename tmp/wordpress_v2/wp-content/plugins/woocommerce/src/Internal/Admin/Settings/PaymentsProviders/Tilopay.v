...
fn (mut this Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Tilopay) is_account_connected(mut var_payment_gateway Class_WC_Payment_Gateway) bool {
	return
		rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('tpay_key')]))
		&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'tpay_key')))
		&& rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('tpay_user')]))
		&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'tpay_user')))
		&& rt.is_true(rt.call_function('property_exists', [var_payment_gateway, rt.new_string('tpay_password')]))
		&& !(!rt.is_true(rt.get_property(var_payment_gateway, 'tpay_password')))
	unsafe {
		goto end_label_1
	}
	catch_label_1:
...
fn create_automattic_woocommerce_internal_admin_settings_paymentsproviders_tilopay(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_Tilopay {
