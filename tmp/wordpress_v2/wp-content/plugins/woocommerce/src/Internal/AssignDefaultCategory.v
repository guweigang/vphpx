...
fn (mut this Class_Automattic_WooCommerce_Internal_AssignDefaultCategory) init() {
	rt.call_function('add_action', [
		rt.new_string('wc_schedule_update_product_default_cat'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_AssignDefaultCategory',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'maybe_assign_default_product_cat' },
		]),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_AssignDefaultCategory) schedule_action() {
	rt.call_method(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'queue', []rt.PhpVal{}),
		'schedule_single', [rt.call_function('time', []rt.PhpVal{}),
		rt.new_string('wc_schedule_update_product_default_cat'),
		rt.new_array(), rt.new_string('wc_update_product_default_cat')])
}

fn (mut this Class_Automattic_WooCommerce_Internal_AssignDefaultCategory) maybe_assign_default_product_cat() {
	mut var_wpdb := rt.new_null()
	mut var_default_category := rt.call_function('get_option', [
		rt.new_string('default_product_cat'),
		rt.new_int(0),
	])
	if rt.is_true(var_default_category) {
		mut var_affected_rows := rt.call_method(var_wpdb, 'query', [
			rt.call_method(var_wpdb, 'prepare', [
				rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('INSERT INTO '), rt.get_property(var_wpdb,
					'term_relationships')),
					rt.new_string(' (object_id, term_taxonomy_id)\n\t\t\t\t\tSELECT DISTINCT posts.ID, %s FROM ')), rt.get_property(var_wpdb,
					'posts')),
					rt.new_string(' posts\n\t\t\t\t\tLEFT JOIN\n\t\t\t\t\t\t(\n\t\t\t\t\t\t\tSELECT object_id FROM ')), rt.get_property(var_wpdb,
					'term_relationships')),
					rt.new_string(' term_relationships\n\t\t\t\t\t\t\tLEFT JOIN ')), rt.get_property(var_wpdb,
					'term_taxonomy')),
					rt.new_string(" term_taxonomy ON term_relationships.term_taxonomy_id = term_taxonomy.term_taxonomy_id\n\t\t\t\t\t\t\tWHERE term_taxonomy.taxonomy = 'product_cat'\n\t\t\t\t\t\t) AS tax_query\n\t\t\t\t\tON posts.ID = tax_query.object_id\n\t\t\t\t\tWHERE posts.post_type = 'product'\n\t\t\t\t\tAND tax_query.object_id IS NULL")),
				var_default_category.clone(),
			]),
		])
		if rt.is_true(rt.greater(var_affected_rows, rt.new_int(0))) {
			...
			rt.call_function('wp_update_term_count_now', [
				rt.create_array([rt.ArrayItem{ key: none, val: var_default_category }]),
...
fn create_automattic_woocommerce_internal_assigndefaultcategory(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_AssignDefaultCategory {
