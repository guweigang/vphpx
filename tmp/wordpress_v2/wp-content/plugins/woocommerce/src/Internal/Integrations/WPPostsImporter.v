...
fn (mut this Class_Automattic_WooCommerce_Internal_Integrations_WPPostsImporter) register() {
	rt.call_function('add_action', [rt.new_string('wp_import_posts'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Integrations_WPPostsImporter',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'register_product_attribute_taxonomies' },
		]),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Integrations_WPPostsImporter) register_product_attribute_taxonomies(var_posts rt.PhpVal) rt.PhpVal {
	if !(var_posts.clone().is_array()) || !rt.is_true(var_posts) {
	...
	for {
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('product'), var_post.array_get(rt.new_string('post_type'))))))
			|| !rt.is_true(var_post.array_get(rt.new_string('terms'))) {
			continue
		}
		mut iter_2 := var_post.array_get(rt.new_string('terms')).iterator()
		for {
			...
			rt.call_function('register_taxonomy', [var_term.array_get(rt.new_string('domain')),
...
fn create_automattic_woocommerce_internal_integrations_wppostsimporter(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Integrations_WPPostsImporter {
