...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_Settings) register() {
	rt.call_function('add_filter', [rt.new_string('woocommerce_get_sections_advanced'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Api_Settings',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'add_section' },
		])])
	rt.call_function('add_filter', [rt.new_string('woocommerce_get_settings_advanced'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Api_Settings',
				[]string{}, &this) },
...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_Settings) add_section(mut var_sections Class_Automattic_WooCommerce_Internal_Api_array) rt.PhpVal {
	...
	mut iife_result_0 := iife_temp_0.is_enabled()
	if rt.is_true(iife_result_0) {
		var_sections_mutated.array_set(Class_Automattic_WooCommerce_Internal_Api_Automattic_WooCommerce_Internal_Api_Settings.section_id(), rt.call_function('__', [
			rt.new_string('GraphQL'),
			rt.new_string('woocommerce'),
		]))
	}
	return rt.new_object('Automattic_WooCommerce_Internal_Api_array', []string{},
		var_sections_mutated)
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Api_Settings) add_settings(mut var_settings Class_Automattic_WooCommerce_Internal_Api_array, section_id string) rt.PhpVal {
	...
	mut iife_result_1 := iife_temp_1.is_enabled()
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Api_Automattic_WooCommerce_Internal_Api_Settings.section_id(), rt.new_string(section_id)))))
	...
	return rt.create_array([
		rt.ArrayItem{ key: none, val: rt.create_array([
			...
			rt.ArrayItem{ key: 'desc', val: rt.call_function('__', [
				rt.new_string('Configure the WooCommerce GraphQL API.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'title' },
			rt.ArrayItem{ key: 'id', val: 'woocommerce_graphql_options' },
		]) },
		rt.ArrayItem{ key: none, val: rt.create_array([
			rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
				rt.new_string('Enable GET endpoint'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'desc', val: rt.call_function('__', [
				rt.new_string('Allow GraphQL queries over GET in addition to POST'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{
				key: 'id'
				val: Class_Automattic_WooCommerce_Internal_Api_Main.option_get_endpoint_enabled()
			},
...
fn create_automattic_woocommerce_internal_api_settings(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Api_Settings {
