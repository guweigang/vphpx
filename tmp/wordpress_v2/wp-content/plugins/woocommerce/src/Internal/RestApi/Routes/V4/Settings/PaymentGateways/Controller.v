...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller) get_schema() rt.PhpVal {
	mut var_schema :=
		create_automattic_woocommerce_internal_restapi_routes_v4_settings_paymentgateways_schema_paymentgatewaysettingsschema()
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller) register_routes() {
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller', [
						'Automattic_WooCommerce_Internal_RestApi_Routes_V4_AbstractController',
					], &this) },
				...
				rt.ArrayItem{ key: 'args', val: rt.create_array([
					rt.ArrayItem{ key: 'enabled', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('Gateway enabled status.'),
					...
					rt.ArrayItem{ key: 'description', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('Gateway description.'),
							rt.new_string('woocommerce'),
						]) },
						rt.ArrayItem{ key: 'type', val: 'string' },
						rt.ArrayItem{ key: 'required', val: false },
					...
					rt.ArrayItem{ key: 'order', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('Gateway sort order.'),
							rt.new_string('woocommerce'),
						]) },
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	...
	if !(var_payment_gateways.array_isset(var_id)) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_payment_gateway_invalid_id'), rt.call_function('__', [
			rt.new_string('Invalid payment gateway ID.'),
			rt.new_string('woocommerce'),
	...
	mut var_data := rt.call_method(var_schema, 'get_item_response', [
		var_gateway.clone(), var_request.clone()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller) get_payment_gateway(var_id rt.PhpVal) rt.PhpVal {
	...
	mut var_payment_gateways := rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}),
		'payment_gateways'), 'payment_gateways', []rt.PhpVal{})
	return if !(var_payment_gateways.array_get(var_id_mutated)).is_null() {
		var_payment_gateways.array_get(var_id_mutated)
	} else {
		rt.new_null()
	}
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller) get_schema_for_gateway(gateway_id string) rt.PhpVal {
	mut switch_val_1 := rt.new_string(gateway_id)
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('bacs'))) {
		return rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_BacsGatewaySettingsSchema',
			[]string{},
			create_automattic_woocommerce_internal_restapi_routes_v4_settings_paymentgateways_schema_bacsgatewaysettingsschema())
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('cheque'))) {
		return rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_ChequeGatewaySettingsSchema',
			[]string{},
			create_automattic_woocommerce_internal_restapi_routes_v4_settings_paymentgateways_schema_chequegatewaysettingsschema())
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('cod'))) {
		return rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_CodGatewaySettingsSchema',
			[]string{},
			create_automattic_woocommerce_internal_restapi_routes_v4_settings_paymentgateways_schema_codgatewaysettingsschema())
	} else {
		return rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_PaymentGatewaySettingsSchema',
			[]string{},
			create_automattic_woocommerce_internal_restapi_routes_v4_settings_paymentgateways_schema_paymentgatewaysettingsschema())
	}
	return rt.new_null()
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller) update_item(var_request rt.PhpVal) rt.PhpVal {
	mut var_id := var_request.array_get(rt.new_string('id'))
	mut var_gateway := this.get_payment_gateway(var_id.clone())
	...
	mut var_values_to_update := if !(var_params.array_get(rt.new_string('values'))).is_null() {
		var_params.array_get(rt.new_string('values'))
	} else {
		rt.new_array()
	}
	...
	mut var_enabled := if !(var_params.array_get(rt.new_string('enabled'))).is_null() {
		var_params.array_get(rt.new_string('enabled'))
	} else {
		if !(var_values_to_update.array_get(rt.new_string('enabled'))).is_null() {
			var_values_to_update.array_get(rt.new_string('enabled'))
		} else {
			rt.new_null()
		}
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_enabled)))) {
		rt.set_property(var_gateway, 'enabled', rt.call_function('wc_bool_to_string', [
			var_enabled.clone(),
		]))
		rt.get_property(var_gateway, 'settings').array_set('enabled', rt.get_property(var_gateway,
			'enabled'))
		var_values_to_update.array_unset(rt.new_string('enabled'))
	}
	mut var_title := if !(var_params.array_get(rt.new_string('title'))).is_null() {
		var_params.array_get(rt.new_string('title'))
	} else {
		if !(var_values_to_update.array_get(rt.new_string('title'))).is_null() {
			var_values_to_update.array_get(rt.new_string('title'))
		} else {
			rt.new_null()
		}
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_title)))) {
		rt.set_property(var_gateway, 'title', rt.call_function('sanitize_text_field', [
			var_title.clone(),
		]))
		rt.get_property(var_gateway, 'settings').array_set('title', rt.get_property(var_gateway,
			'title'))
		var_values_to_update.array_unset(rt.new_string('title'))
	}
	mut var_description := if !(var_params.array_get(rt.new_string('description'))).is_null() {
		var_params.array_get(rt.new_string('description'))
	} else {
		if !(var_values_to_update.array_get(rt.new_string('description'))).is_null() {
			var_values_to_update.array_get(rt.new_string('description'))
		} else {
			rt.new_null()
		}
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_description)))) {
		rt.set_property(var_gateway, 'description', rt.call_function('wp_kses_post', [
			var_description.clone(),
		]))
		rt.get_property(var_gateway, 'settings').array_set('description', rt.get_property(var_gateway,
			'description'))
		var_values_to_update.array_unset(rt.new_string('description'))
	}
	mut var_order_to_update := rt.new_null()
	mut var_order_value := if !(var_params.array_get(rt.new_string('order'))).is_null() {
		var_params.array_get(rt.new_string('order'))
	} else {
		if !(var_values_to_update.array_get(rt.new_string('order'))).is_null() {
			var_values_to_update.array_get(rt.new_string('order'))
		} else {
			rt.new_null()
		}
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_order_value)))) {
		var_order_to_update = rt.call_function('absint', [var_order_value.clone()])
	...
	for {
		...
		if rt.is_true(rt.call_method(var_schema, 'is_special_field', [
			var_key.clone()]))
		{
			var_special_values.array_set(var_key, var_value.clone())
		} else if rt.get_property(var_gateway, 'form_fields').array_isset(var_key) {
			var_standard_values.array_set(var_key, var_value.clone())
		}
	}
	mut var_validated_settings := rt.call_method(var_schema, 'validate_and_sanitize_settings', [
		var_gateway.clone(),
		var_standard_values.clone(),
	])
	if rt.is_true(rt.call_function('is_wp_error', [var_validated_settings.clone()])) {
		return var_validated_settings.clone()
	}
	mut var_validated_special := rt.call_method(var_schema, 'validate_and_sanitize_special_fields', [
		var_gateway.clone(),
		var_special_values.clone(),
	])
	if rt.is_true(rt.call_function('is_wp_error', [var_validated_special.clone()])) {
		return var_validated_special.clone()
	}
	...
	for {
		...
		rt.get_property(var_gateway, 'settings').array_set(var_key, var_value.clone())
	}
	rt.call_function('update_option', [
		rt.call_method(var_gateway, 'get_option_key', []rt.PhpVal{}),
		rt.get_property(var_gateway, 'settings'),
	])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_order_to_update)))) {
		mut var_gateway_order := rt.cast_array(rt.call_function('get_option', [
			rt.new_string('woocommerce_gateway_order'),
			rt.new_array(),
		]))
		var_gateway_order.array_set(var_id, var_order_to_update.clone())
		rt.call_function('update_option', [rt.new_string('woocommerce_gateway_order'),
			var_gateway_order.clone()])
	}
	rt.call_method(var_schema, 'update_special_fields', [var_gateway.clone(),
		var_validated_special.clone()])
	mut var_data := rt.call_method(var_schema, 'get_item_response', [
		var_gateway.clone(), var_request.clone()])
	return rt.call_function('rest_ensure_response', [var_data.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller) get_item_response(var_item rt.PhpVal, mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	return rt.call_method(var_schema, 'get_item_response', [var_item.clone(), var_request])
}
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_paymentgateways_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_paymentgateways_schema_paymentgatewaysettingsschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_PaymentGatewaySettingsSchema {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_paymentgateways_schema_bacsgatewaysettingsschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_BacsGatewaySettingsSchema {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_paymentgateways_schema_chequegatewaysettingsschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_ChequeGatewaySettingsSchema {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Controller) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_schema_for_gateway' {
			...
			return this.get_schema_for_gateway(dispatch_arg_0)
		}
		'update_item' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
