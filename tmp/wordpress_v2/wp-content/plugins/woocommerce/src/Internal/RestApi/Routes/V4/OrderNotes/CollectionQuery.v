	...
	return rt.create_array([
		rt.ArrayItem{ key: 'note_type', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Limit result to customer notes or private notes.'),
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_CollectionQuery) get_query_args(mut var_request Class_WP_REST_Request) rt.PhpVal {
	mut var_args := rt.create_array([
		rt.ArrayItem{
			key: 'post_id'
			val: if !(var_request.array_get(rt.new_string('order_id'))).is_null() {
		...
		rt.ArrayItem{ key: 'type', val: 'order_note' },
	])
	if rt.is_true(rt.identical(rt.new_string('customer'),
		var_request.array_get(rt.new_string('note_type'))))
	{
		var_args.array_set('meta_query', rt.create_array([
	...
	} else if rt.is_true(rt.identical(rt.new_string('private'),
		var_request.array_get(rt.new_string('note_type'))))
	{
		var_args.array_set('meta_query', rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_ordernotes_collectionquery(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_CollectionQuery {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_CollectionQuery) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_query_results' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
