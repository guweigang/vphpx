...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_OfflinePaymentMethods_Controller) register_routes() {
	...
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_OfflinePaymentMethods_Controller', [
					...
					rt.create_array([
						rt.ArrayItem{ key: 'location', val: rt.create_array([
							rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
								rt.new_string('Country code to retrieve offline payment methods for.'),
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_OfflinePaymentMethods_Controller) get_items_permissions_check(var_request rt.PhpVal) bool {
	...
	{
		return (create_wp_error(rt.new_string('woocommerce_rest_cannot_read'), rt.call_function('__', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_OfflinePaymentMethods_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	mut var_offline_methods := this.get_offline_payment_methods_data(var_request.clone())
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	if rt.instance_of(var_e_1,
		'Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_OfflinePaymentMethods_Exception')
	{
		mut var_e := var_e_1.clone()
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_offline_payment_methods_error'), rt.call_method(var_e,
	...
	end_label_1:
	if rt.is_true(rt.call_function('is_wp_error', [var_offline_methods.clone()])) {
		return var_offline_methods.clone()
	}
	mut var_response_data := rt.create_array([
		rt.ArrayItem{ key: 'id', val: 'payments/offline-methods' },
		rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
			rt.new_string('Offline Payment Methods'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Manage offline payment methods available for your store.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'values', val: rt.new_array() },
		rt.ArrayItem{ key: 'groups', val: rt.create_array([
			rt.ArrayItem{ key: 'payment_methods', val: rt.new_array() },
		]) },
	])
	if !(var_offline_methods.clone().is_array()) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_invalid_data'), rt.call_function('__', [
			rt.new_string('Invalid payment methods data received.'),
			rt.new_string('woocommerce'),
	...
	for {
		...
		if !(var_method.clone().is_array()) {
			continue
		}
		mut var_method_id := if !(var_method.array_get(rt.new_string('id'))).is_null() {
			var_method.array_get(rt.new_string('id'))
		} else {
			rt.new_string('')
		}
		if !rt.is_true(var_method_id) || !(var_method_id.clone().is_string()) {
			continue
		}
		mut var_enabled_state := rt.new_bool(false)
		if var_method.array_isset(rt.new_string('state'))
			&& var_method.array_get(rt.new_string('state')).is_array() {
			var_enabled_state = if !(var_method.array_get(rt.new_string('state')).array_get(rt.new_string('enabled'))).is_null() {
				var_method.array_get(rt.new_string('state')).array_get(rt.new_string('enabled'))
			} else {
				rt.new_bool(false)
			}
		}
		if rt.is_true(rt.new_bool(var_enabled_state.clone().is_array())) {
			var_enabled_state = if !(var_enabled_state.array_get(rt.new_string('value'))).is_null() {
				var_enabled_state.array_get(rt.new_string('value'))
			} else {
				rt.new_bool(false)
			}
		}
		if rt.is_true(rt.new_bool(var_enabled_state.clone().is_string())) {
			var_enabled_state = rt.call_function('wc_string_to_bool', [
				var_enabled_state.clone()])
		} else if !(var_enabled_state.clone().is_bool()) {
			var_enabled_state = rt.new_bool(var_enabled_state.to_bool())
		}
		var_response_data.array_get_mut('values').array_set(var_method_id,
			var_enabled_state.clone())
		var_response_data.array_get_mut('groups').array_get_mut('payment_methods').array_set(var_method_id, rt.create_array([
			rt.ArrayItem{ key: 'id', val: var_method_id },
			rt.ArrayItem{
				key: '_order'
				val: if var_method.array_isset(rt.new_string('_order')) { rt.call_function('absint', [
						var_method.array_get(rt.new_string('_order')),
					]) } else { rt.new_int(0) }
			},
			rt.ArrayItem{ key: 'title', val: rt.call_function('sanitize_text_field', [
				if !(var_method.array_get(rt.new_string('title'))).is_null() {
					var_method.array_get(rt.new_string('title'))
				} else {
					rt.new_string('')
				},
			]) },
			rt.ArrayItem{ key: 'description', val: rt.call_function('wp_kses_post', [
				if !(var_method.array_get(rt.new_string('description'))).is_null() {
					var_method.array_get(rt.new_string('description'))
				} else {
					rt.new_string('')
				},
			]) },
			rt.ArrayItem{ key: 'icon', val: rt.call_function('esc_url_raw', [
				if !(var_method.array_get(rt.new_string('icon'))).is_null() {
					var_method.array_get(rt.new_string('icon'))
				} else {
					rt.new_string('')
				},
			]) },
			rt.ArrayItem{ key: 'state', val: rt.call_function('array_map', [
				rt.new_string('rest_sanitize_boolean'),
				rt.call_function('wp_parse_args', [if if !(var_method.array_get(rt.new_string('state'))).is_null() {
					var_method.array_get(rt.new_string('state'))
				} else {
					rt.new_null()
				}.is_array()
				{ var_method.array_get(rt.new_string('state'))
					...
					rt.create_array([rt.ArrayItem{ key: 'enabled', val: false },
						rt.ArrayItem{ key: 'account_connected', val: false },
						rt.ArrayItem{ key: 'needs_setup', val: false },
						rt.ArrayItem{ key: 'test_mode', val: false },
						rt.ArrayItem{ key: 'dev_mode', val: false }])]),
			]) },
			rt.ArrayItem{ key: 'management', val: this.sanitize_management_field(if !(var_method.array_get(rt.new_string('management'))).is_null() {
				var_method.array_get(rt.new_string('management'))
			} else {
				rt.new_array()
			}) },
		]))
	}
	return rt.call_function('rest_ensure_response', [var_response_data.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_OfflinePaymentMethods_Controller) get_offline_payment_methods_data(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_location := rt.call_function('sanitize_text_field', [
		rt.call_method(var_request, 'get_param', [rt.new_string('location')]),
	])
	...
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_bool(var_provider.array_isset(rt.new_string('_type'))
			&& rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders.type_offline_pm(), var_provider.array_get(rt.new_string('_type')))))
	}
	...
	return var_offline_payment_providers.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_OfflinePaymentMethods_Controller) sanitize_management_field(var_management rt.PhpVal) rt.PhpVal {
	if !(var_management.clone().is_array()) {
		return rt.create_array([rt.ArrayItem{ key: '_links', val: rt.new_array() }])
	}
	mut var_sanitized := rt.create_array([
		rt.ArrayItem{ key: '_links', val: rt.new_array() },
	])
	if var_management.array_isset(rt.new_string('_links'))
		&& var_management.array_get(rt.new_string('_links')).is_array() {
		mut iter_2 := var_management.array_get(rt.new_string('_links')).iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_link := item_2.val
			mut var_key := item_2.key
			mut var_sanitized_key := rt.call_function('sanitize_key', [
				var_key.clone()])
			if var_link.clone().is_array() && var_link.array_isset(rt.new_string('href')) {
				var_sanitized.array_get_mut('_links').array_set(var_sanitized_key, rt.create_array([
					rt.ArrayItem{ key: 'href', val: rt.call_function('esc_url_raw', [
						var_link.array_get(rt.new_string('href')),
					]) },
				]))
			} else if rt.is_true(rt.new_bool(var_link.clone().is_string())) {
				var_sanitized.array_get_mut('_links').array_set(var_sanitized_key, rt.create_array([
					rt.ArrayItem{ key: 'href', val: rt.call_function('esc_url_raw', [
						var_link.clone(),
					]) },
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_offlinepaymentmethods_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_OfflinePaymentMethods_Controller {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_OfflinePaymentMethods_Controller) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_OfflinePaymentMethods_Schema_OfflinePaymentMethodSchema](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
