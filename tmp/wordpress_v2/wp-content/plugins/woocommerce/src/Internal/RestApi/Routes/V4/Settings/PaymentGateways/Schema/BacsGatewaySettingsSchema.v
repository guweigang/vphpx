...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_BacsGatewaySettingsSchema) get_custom_groups_for_gateway(mut var_gateway Class_WC_Payment_Gateway) rt.PhpVal {
	mut var_core_field_overrides := rt.create_array([
		rt.ArrayItem{ key: 'enabled', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'desc', val: rt.call_function('__', [
				rt.new_string('Enable Direct bank transfer at checkout'),
				rt.new_string('woocommerce'),
			]) },
		]) },
	...
	mut var_fields := this.build_fields_from_form_fields(rt.new_object('WC_Payment_Gateway',
		[]string{}, var_gateway), var_core_field_overrides.clone(), rt.create_array([
		rt.ArrayItem{ key: none, val: 'account_details' },
	]))
	mut var_settings_group := rt.create_array([
		rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
			rt.new_string('Direct bank transfer settings'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Manage how Direct bank transfer appears at checkout and in order emails.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'order', val: 1 },
		rt.ArrayItem{ key: 'fields', val: var_fields },
	])
	mut var_field := if !(rt.get_property(var_gateway, 'form_fields').array_get(rt.new_string('account_details'))).is_null() {
		rt.get_property(var_gateway, 'form_fields').array_get(rt.new_string('account_details'))
	} else {
		rt.new_array()
	}
	mut var_account_details_group := rt.create_array([
		rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
			rt.new_string('Bank account details'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Manage the bank accounts customers can use to pay by bank transfer.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'order', val: 2 },
		rt.ArrayItem{ key: 'fields', val: rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'id', val: 'account_details' },
				rt.ArrayItem{
					key: 'label'
					val: if !(var_field.array_get(rt.new_string('title'))).is_null() { var_field.array_get(rt.new_string('title')) } else { rt.call_function('__', [
				...
				rt.ArrayItem{
					key: 'desc'
					val: if !(var_field.array_get(rt.new_string('description'))).is_null() { var_field.array_get(rt.new_string('description')) } else { rt.call_function('__', [
							rt.new_string('Bank account details for direct bank transfer.'),
							rt.new_string('woocommerce'),
						]) }
				},
			]) },
	...
	return rt.create_array([rt.ArrayItem{ key: 'settings', val: var_settings_group },
		rt.ArrayItem{ key: 'account_details', val: var_account_details_group }])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_BacsGatewaySettingsSchema) get_special_field_values(mut var_gateway Class_WC_Payment_Gateway) rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'account_details', val: rt.call_function('get_option', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_BacsGatewaySettingsSchema) validate_and_sanitize_special_fields(mut var_gateway Class_WC_Payment_Gateway, mut var_values Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.identical(rt.new_string('account_details'), var_field_id)) {
			var_validated.array_set(var_field_id, this.validate_bacs_accounts(var_value.clone()))
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_BacsGatewaySettingsSchema) validate_bacs_accounts(var_value rt.PhpVal) rt.PhpVal {
	if !(var_value.clone().is_array()) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('__', [
			rt.new_string('Account details must be an array.'),
			rt.new_string('woocommerce'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }])))
	}
	mut var_validated_accounts := rt.new_array()
	mut var_valid_fields := rt.create_array([
		rt.ArrayItem{ key: none, val: 'account_name' },
		rt.ArrayItem{ key: none, val: 'account_number' },
		rt.ArrayItem{ key: none, val: 'sort_code' },
		rt.ArrayItem{ key: none, val: 'bank_name' },
		rt.ArrayItem{ key: none, val: 'iban' },
		rt.ArrayItem{ key: none, val: 'bic' },
	])
	mut iter_3 := var_value.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_account := item_3.val
		mut var_index := item_3.key
		if !(var_account.clone().is_array()) {
			return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('Account at index %d must be an object.'),
					rt.new_string('woocommerce'),
				]),
				var_index.clone(),
			]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }])))
		...
		mut iter_4 := var_valid_fields.iterator()
		for {
			item_4 := iter_4.next() or { break }
			mut var_field := item_4.val
			var_validated_account.array_set(var_field, if var_account.array_isset(var_field) { rt.call_function('sanitize_text_field', [
					var_account.array_get(var_field),
				]) } else { rt.new_string('') })
		}
		if rt.is_true(rt.call_function('array_filter', [var_validated_account.clone()])) {
			var_validated_accounts.array_push(var_validated_account.clone())
		}
	}
	return var_validated_accounts.clone()
}
