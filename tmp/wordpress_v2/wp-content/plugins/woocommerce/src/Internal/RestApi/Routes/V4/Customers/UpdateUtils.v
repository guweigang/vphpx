...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_UpdateUtils) update_customer_from_request(mut var_customer Class_WC_Customer, mut var_request Class_WP_REST_Request, creating bool) {
	if var_request.array_isset(rt.new_string('email')) {
		var_customer.set_email(rt.call_function('sanitize_email', [
			var_request.array_get(rt.new_string('email')),
		]))
	}
	if var_request.array_isset(rt.new_string('password')) {
		var_customer.set_password(var_request.array_get(rt.new_string('password')))
	}
	if var_request.array_isset(rt.new_string('first_name')) {
		var_customer.set_first_name(rt.call_function('wc_clean', [
			var_request.array_get(rt.new_string('first_name')),
		]))
	}
	if var_request.array_isset(rt.new_string('last_name')) {
		var_customer.set_last_name(rt.call_function('wc_clean', [
			var_request.array_get(rt.new_string('last_name')),
		]))
	}
	if var_request.array_isset(rt.new_string('billing'))
		&& var_request.array_get(rt.new_string('billing')).is_array() {
		this.update_customer_address(mut var_customer, mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_array](var_request.array_get(rt.new_string('billing'))),
			'billing')
	}
	if var_request.array_isset(rt.new_string('shipping'))
		&& var_request.array_get(rt.new_string('shipping')).is_array() {
		this.update_customer_address(mut var_customer, mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_array](var_request.array_get(rt.new_string('shipping'))),
			'shipping')
	}
	var_customer.save()
	mut var_user_data := rt.call_function('get_userdata', [var_customer.get_id()])
	if rt.is_true(var_user_data) {
		this.update_additional_fields_for_object(var_user_data.clone(), mut var_request)
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_UpdateUtils) update_customer_address(mut var_customer Class_WC_Customer, mut var_address Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_array, type string) {
	...
	var_address_mutated = rt.call_function('wc_clean', [var_address_mutated])
	mut var_address_fields := rt.create_array([
		rt.ArrayItem{ key: none, val: 'first_name' },
		rt.ArrayItem{ key: none, val: 'last_name' },
		rt.ArrayItem{ key: none, val: 'company' },
		rt.ArrayItem{ key: none, val: 'address_1' },
		rt.ArrayItem{ key: none, val: 'address_2' },
	...
	for {
		...
		if var_address_mutated.array_isset(var_field)
			&& rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
			key: none
			val: var_customer
		}, rt.ArrayItem{ key: none, val: 'set_${var_type}_${var_field.to_string()}' }])]) {
			mut var_value := if rt.is_true(rt.identical(rt.new_string('email'), var_field)) { rt.call_function('sanitize_email', [
					var_address_mutated.array_get(var_field),
				]) } else { var_address_mutated.array_get(var_field) }
			rt.call_method(var_customer, 'set_${var_type}_${var_field.to_string()}', [
				var_value.clone(),
			])
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_UpdateUtils) update_additional_fields_for_object(var_item rt.PhpVal, mut var_request Class_WP_REST_Request) {
	...
	for {
		...
		mut var_field_options := item_2.val
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(var_field_options.array_get(rt.new_string('update_callback'))))))
			|| !(rt.call_function('is_callable', [var_field_options.array_get(rt.new_string('update_callback'))])) {
			continue
		}
		...
		mut var_result := rt.call_function('call_user_func', [
			var_field_options.array_get(rt.new_string('update_callback')),
			var_request.array_get(var_field_name),
			var_item.clone(),
			var_field_name.clone(),
			var_request,
		])
		if rt.is_true(rt.call_function('is_wp_error', [var_result.clone()])) {
			rt.throw_exception(rt.new_object('WC_REST_Exception', []string{}, create_wc_rest_exception(rt.new_string('woocommerce_rest_cannot_update'), rt.call_function('esc_html', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_UpdateUtils) get_additional_fields() rt.PhpVal {
	...
	var_fields = rt.call_function('apply_filters', [
		rt.new_string('rest_additional_fields'),
		var_fields.clone(),
		rt.new_string('user'),
	])
	var_fields = rt.call_function('apply_filters', [
		rt.concat(rt.concat(rt.new_string('rest_'), this.get_object_type()),
			rt.new_string('_additional_fields')),
		var_fields.clone(),
		rt.new_string('user'),
	])
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_customers_updateutils(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_UpdateUtils {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_UpdateUtils) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'update_customer_from_request' {
			...
			this.update_customer_from_request(mut dispatch_arg_0, mut dispatch_arg_1,
				dispatch_arg_2)
			return rt.new_null()
		}
		'update_customer_address' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
