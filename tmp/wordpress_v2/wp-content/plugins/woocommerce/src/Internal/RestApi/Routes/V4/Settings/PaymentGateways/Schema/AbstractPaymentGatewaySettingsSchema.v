...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) get_item_schema_properties() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.create_array([
			...
			rt.ArrayItem{
				key: 'context'
				val: Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema.view_edit_context()
			},
		...
		rt.ArrayItem{ key: 'method_description', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
		...
		rt.ArrayItem{ key: 'method_supports', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) get_values(mut var_gateway Class_WC_Payment_Gateway) rt.PhpVal {
	...
	for {
		...
		var_values.array_set(var_id, if !(rt.get_property(var_gateway, 'settings').array_get(var_id)).is_null() {
			rt.get_property(var_gateway, 'settings').array_get(var_id)
		} else {
			if !(var_field.array_get(rt.new_string('default'))).is_null() {
	...
	var_values = rt.call_function('array_merge', [var_values.clone(),
		var_special_fields.clone()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) get_groups(mut var_gateway Class_WC_Payment_Gateway) rt.PhpVal {
	...
	if !(!rt.is_true(var_custom_groups)) {
		return var_custom_groups.clone()
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) get_default_group(mut var_gateway Class_WC_Payment_Gateway) rt.PhpVal {
	...
	var_group.array_get_mut('fields').array_push(rt.create_array([
		rt.ArrayItem{ key: 'id', val: 'enabled' },
		...
		rt.ArrayItem{ key: 'desc', val: rt.call_function('__', [
			rt.new_string('Enable this payment gateway'),
			rt.new_string('woocommerce'),
		]) },
	]))
	var_group.array_get_mut('fields').array_push(rt.create_array([
		rt.ArrayItem{ key: 'id', val: 'title' },
		rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
			rt.new_string('Title'), rt.new_string('woocommerce')]) },
	...
	var_group.array_get_mut('fields').array_push(rt.create_array([
		rt.ArrayItem{ key: 'id', val: 'description' },
		rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
	...
	for {
		...
		if rt.is_true(rt.call_function('in_array', [var_field_type.clone(), rt.create_array([rt.ArrayItem{
			key: none
			val: 'title'
		}, rt.ArrayItem{ key: none, val: 'sectionend' }]), rt.new_bool(true)]))
			|| rt.is_true(rt.call_function('in_array', [var_id.clone(), rt.create_array([rt.ArrayItem{
			key: none
			val: 'enabled'
		}, rt.ArrayItem{ key: none, val: 'description' }, rt.ArrayItem{ key: none, val: 'title' }]), rt.new_bool(true)]))
			|| this.is_special_field(var_id.str()) {
			continue
		}
		var_group.array_get_mut('fields').array_push(this.transform_field_to_schema(var_id.str(), mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array](var_field), mut
			var_gateway))
	}
	mut var_special_fields := this.get_special_field_schemas(mut var_gateway)
	var_group.array_set('fields', rt.call_function('array_merge', [
		var_group.array_get(rt.new_string('fields')),
		var_special_fields.clone(),
	]))
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) transform_field_to_schema(id string, mut var_field Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array, mut var_gateway Class_WC_Payment_Gateway) rt.PhpVal {
	...
	mut var_schema_field := rt.create_array([rt.ArrayItem{ key: 'id', val: id },
		rt.ArrayItem{
			key: 'label'
			val: if !(var_field_mutated.array_get(rt.new_string('title'))).is_null() {
				var_field_mutated.array_get(rt.new_string('title'))
			} else {
				if !(var_field_mutated.array_get(rt.new_string('label'))).is_null() {
					var_field_mutated.array_get(rt.new_string('label'))
				} else {
					rt.new_string('')
				}
			}
		}, rt.ArrayItem{ key: 'type', val: this.normalize_field_type(var_field_type.str()) },
		rt.ArrayItem{
			key: 'desc'
			val: if !(var_field_mutated.array_get(rt.new_string('description'))).is_null() {
				var_field_mutated.array_get(rt.new_string('description'))
			} else {
				rt.new_string('')
			}
		}])
	if rt.is_true(rt.identical(rt.new_string('checkbox'), var_field_type))
		&& !rt.is_true(var_schema_field.array_get(rt.new_string('desc')))
		&& !(!rt.is_true(var_field_mutated.array_get(rt.new_string('label')))) {
		var_schema_field.array_set('desc', var_field_mutated.array_get(rt.new_string('label')))
	}
	if rt.is_true(rt.call_function('in_array', [var_schema_field.array_get(rt.new_string('type')),
		rt.create_array([rt.ArrayItem{ key: none, val: 'select' },
			rt.ArrayItem{ key: none, val: 'multiselect' }]),
		rt.new_bool(true)]))
	{
		if !(!rt.is_true(var_field_mutated.array_get(rt.new_string('options')))) {
			var_schema_field.array_set('options',
				var_field_mutated.array_get(rt.new_string('options')))
		} else {
			var_schema_field.array_set('options', this.get_field_options(id))
		}
	}
	return var_schema_field.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) build_fields_from_form_fields(mut var_gateway Class_WC_Payment_Gateway, mut var_core_field_overrides Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array, mut var_skip_field_ids Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array) rt.PhpVal {
	...
	mut iter_3 := var_core_field_overrides.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_override := item_3.val
		mut var_field_id := item_3.key
		if rt.is_true(rt.identical(rt.new_string('order'), var_field_id)) {
			var_fields.array_push(rt.call_function('array_merge', [
				rt.create_array([rt.ArrayItem{ key: 'id', val: var_field_id }]),
				var_override.clone(),
			]))
			continue
		}
		...
		mut var_schema_field := rt.call_function('array_merge', [
			rt.create_array([rt.ArrayItem{ key: 'id', val: var_field_id }]),
			var_override.clone(),
		])
		if rt.is_true(rt.call_function('in_array', [if !(var_schema_field.array_get(rt.new_string('type'))).is_null() { var_schema_field.array_get(rt.new_string('type')) } else { rt.new_string('') }, rt.create_array([rt.ArrayItem{
			key: none
			val: 'select'
		}, rt.ArrayItem{ key: none, val: 'multiselect' }]), rt.new_bool(true)]))
			&& !(var_schema_field.array_isset(rt.new_string('options'))) {
			if !(!rt.is_true(var_field.array_get(rt.new_string('options')))) {
				var_schema_field.array_set('options', var_field.array_get(rt.new_string('options')))
			}
		}
		var_fields.array_push(var_schema_field.clone())
	}
	mut iter_4 := rt.get_property(var_gateway, 'form_fields').iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_field := item_4.val
		mut var_field_id := item_4.key
		mut var_field_type := if !(var_field.array_get(rt.new_string('type'))).is_null() {
			var_field.array_get(rt.new_string('type'))
		} else {
			rt.new_string('')
		...
		if rt.is_true(rt.call_function('in_array', [var_field_type.clone(), rt.create_array([rt.ArrayItem{
			...
			|| var_core_field_overrides.array_isset(var_field_id)
			|| rt.is_true(rt.call_function('in_array', [var_field_id.clone(), var_skip_field_ids, rt.new_bool(true)]))
			|| this.is_special_field(var_field_id.str()) {
			continue
		}
		var_fields.array_push(this.transform_field_to_schema(var_field_id.str(), mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array](var_field), mut
			var_gateway))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) normalize_field_type(wc_type string) string {
	mut var_type_map := rt.create_array([rt.ArrayItem{ key: 'email', val: 'text' },
		rt.ArrayItem{ key: 'password', val: 'text' }, rt.ArrayItem{ key: 'textarea', val: 'text' },
		rt.ArrayItem{ key: 'safe_text', val: 'text' }, rt.ArrayItem{ key: 'color', val: 'text' },
		rt.ArrayItem{ key: 'image_width', val: 'text' }, rt.ArrayItem{ key: 'radio', val: 'select' }])
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) get_settings(mut var_gateway Class_WC_Payment_Gateway) rt.PhpVal {
	...
	var_gateway.init_form_fields()
	mut iter_5 := rt.get_property(var_gateway, 'form_fields').iterator()
	for {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) get_item_response(var_gateway rt.PhpVal, mut var_request Class_WP_REST_Request, mut var_include_fields Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array) rt.PhpVal {
	...
	return rt.create_array([
		...
		rt.ArrayItem{ key: 'title', val: rt.get_property(var_gateway, 'title') },
		rt.ArrayItem{ key: 'description', val: rt.get_property(var_gateway, 'description') },
		rt.ArrayItem{
			key: 'order'
			val: if !(var_order.array_get(rt.get_property(var_gateway, 'id'))).is_null() {
				var_order.array_get(rt.get_property(var_gateway, 'id'))
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{ key: 'enabled', val: rt.identical(rt.new_string('yes'), rt.get_property(var_gateway,
			'enabled')) },
		rt.ArrayItem{ key: 'method_title', val: rt.call_method(var_gateway, 'get_method_title',
			[]rt.PhpVal{}) },
		rt.ArrayItem{ key: 'method_description', val: rt.call_method(var_gateway,
			'get_method_description', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'method_supports', val: rt.get_property(var_gateway, 'supports') },
		rt.ArrayItem{
			key: 'values'
			val: this.get_values(mut rt.cast_object_ptr[Class_WC_Payment_Gateway](var_gateway))
		},
		rt.ArrayItem{
			key: 'groups'
			val: this.get_groups(mut rt.cast_object_ptr[Class_WC_Payment_Gateway](var_gateway))
		},
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) validate_and_sanitize_settings(mut var_gateway Class_WC_Payment_Gateway, mut var_values Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array) rt.PhpVal {
	...
	mut iter_6 := var_values_mutated.iterator()
	for {
		...
		mut var_validation := rt.new_bool(this.validate_field_value(var_key.str(),
		...
		var_validated.array_set(var_key, var_sanitized.clone())
	}
	return var_validated.clone()
}

fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) sanitize_field_value(type string, var_value rt.PhpVal) rt.PhpVal {
	mut switch_val_1 := rt.new_string(type)
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('checkbox'))) {
		return rt.call_function('wc_bool_to_string', [var_value.clone()])
		...
		return if var_value.clone().is_string() { rt.create_array([
		...
		return rt.call_function('sanitize_email', [var_value.clone()])
		...
		return rt.new_string((if rt.is_true(rt.call_function('is_scalar', [
			var_value.clone()]))
		{ var_value.str().trim_space() } else { '' }).str())
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) validate_field_value(key string, var_value rt.PhpVal, mut var_field Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array, mut var_gateway Class_WC_Payment_Gateway) bool {
	mut var_field_mutated := var_field
	mut var_field_type := rt.new_string(this.normalize_field_type((if !(var_field_mutated.array_get(rt.new_string('type'))).is_null() {
		var_field_mutated.array_get(rt.new_string('type'))
	} else {
		rt.new_string('text')
	}).str()))
	if rt.is_true(rt.call_function('in_array', [var_field_type.clone(), rt.create_array([rt.ArrayItem{
		key: none
		val: 'select'
	}, rt.ArrayItem{ key: none, val: 'radio' }]), rt.new_bool(true)]))
		&& !(!rt.is_true(var_field_mutated.array_get(rt.new_string('options')))) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_field_mutated.array_get(rt.new_string('options')).array_isset(var_value.clone()))))))
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_AbstractPaymentGatewaySettingsSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'transform_field_to_schema' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_WC_Payment_Gateway](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
			return this.transform_field_to_schema(dispatch_arg_0, mut dispatch_arg_1, mut
				dispatch_arg_2)
		}
		'get_field_options' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
		...
		'build_fields_from_form_fields' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_array](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
			return this.build_fields_from_form_fields(mut dispatch_arg_0, mut dispatch_arg_1, mut
				dispatch_arg_2)
		}
		...
		'validate_field_value' {
			...
			mut dispatch_arg_3 := rt.cast_object_ptr[Class_WC_Payment_Gateway](if args.len > 3 {
				args[3]
			} else {
				rt.new_null()
			})
