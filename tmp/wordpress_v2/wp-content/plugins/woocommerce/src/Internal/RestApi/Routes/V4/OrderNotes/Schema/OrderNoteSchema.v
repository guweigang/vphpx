...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_Schema_OrderNoteSchema) get_item_schema_properties() rt.PhpVal {
	mut var_schema := rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.create_array([
			...
			rt.ArrayItem{
				key: 'context'
				val: Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_Schema_OrderNoteSchema.view_edit_embed_context()
			},
			rt.ArrayItem{ key: 'readonly', val: true },
		]) },
		rt.ArrayItem{ key: 'order_id', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Order ID the note belongs to.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'integer' },
			rt.ArrayItem{
				key: 'context'
				val: Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_Schema_OrderNoteSchema.view_edit_context()
			},
			rt.ArrayItem{ key: 'readonly', val: true },
		]) },
		...
		rt.ArrayItem{ key: 'title', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('The title of the order note group.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{
		...
		rt.ArrayItem{ key: 'group', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('The group of order note.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{
		...
		rt.ArrayItem{ key: 'is_customer_note', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('If true, the note will be shown to customers. If false, the note will be for admin reference only.'),
				rt.new_string('woocommerce'),
			]) },
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_Schema_OrderNoteSchema) get_item_response(var_note rt.PhpVal, mut var_request Class_WP_REST_Request, mut var_include_fields Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_Schema_array) rt.PhpVal {
	mut var_group := rt.call_function('get_comment_meta', [
		rt.get_property(var_note, 'comment_ID'),
		rt.new_string('note_group'),
		rt.new_bool(true),
	])
	mut var_title := rt.call_function('get_comment_meta', [
		rt.get_property(var_note, 'comment_ID'),
		rt.new_string('note_title'),
		rt.new_bool(true),
	])
	mut var_is_customer_note := rt.call_function('wc_string_to_bool', [
		rt.call_function('get_comment_meta', [rt.get_property(var_note, 'comment_ID'),
			rt.new_string('is_customer_note'), rt.new_bool(true)]),
	])
	if rt.is_true(var_group) && rt.is_true(rt.new_bool(!(rt.is_true(var_title)))) {
		mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Orders_OrderNoteGroup{}
		mut iife_result_0 := iife_temp_0.get_default_group_title(var_group.clone())
		var_title = iife_result_0
	}
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.new_int((rt.get_property(var_note, 'comment_ID')).to_i64()) },
		rt.ArrayItem{
			key: 'order_id'
			val: rt.new_int((rt.get_property(var_note, 'comment_post_ID')).to_i64())
		},
		rt.ArrayItem{ key: 'author', val: rt.get_property(var_note, 'comment_author') },
		...
		rt.ArrayItem{ key: 'is_customer_note', val: var_is_customer_note },
	])
}
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_ordernotes_schema_ordernoteschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_Schema_OrderNoteSchema {
...
fn create_automattic_woocommerce_internal_orders_ordernotegroup(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Orders_OrderNoteGroup {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_Schema_OrderNoteSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_item_response' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_OrderNotes_Schema_array](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
