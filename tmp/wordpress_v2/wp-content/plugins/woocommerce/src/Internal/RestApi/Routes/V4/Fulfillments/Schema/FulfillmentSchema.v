...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Fulfillments_Schema_FulfillmentSchema) get_item_schema_properties() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.create_array([
			...
			rt.ArrayItem{
				key: 'context'
				val: Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Fulfillments_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Fulfillments_Schema_FulfillmentSchema.view_edit_context()
			},
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Fulfillments_Schema_FulfillmentSchema) get_item_response(var_fulfillment rt.PhpVal, mut var_request Class_WP_REST_Request, mut var_include_fields Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Fulfillments_Schema_array) rt.PhpVal {
	...
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.call_method(var_fulfillment, 'get_id', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'entity_type', val: rt.call_method(var_fulfillment, 'get_entity_type',
			[]rt.PhpVal{}) },
		rt.ArrayItem{ key: 'entity_id', val: (rt.call_method(var_fulfillment, 'get_entity_id',
			[]rt.PhpVal{})).str() },
		rt.ArrayItem{ key: 'status', val: rt.call_method(var_fulfillment, 'get_status',
			[]rt.PhpVal{}) },
		rt.ArrayItem{ key: 'is_fulfilled', val: rt.call_method(var_fulfillment, 'get_is_fulfilled',
			[]rt.PhpVal{}) },
		rt.ArrayItem{ key: 'date_updated', val: rt.call_function('wc_rest_prepare_date_response', [
			rt.call_method(var_fulfillment, 'get_date_updated', []rt.PhpVal{}),
		]) },
		rt.ArrayItem{
			key: 'date_deleted'
			val: if rt.is_true(var_date_deleted) { rt.call_function('wc_rest_prepare_date_response', [
					var_date_deleted.clone(),
				]) } else { rt.new_null() }
		},
		rt.ArrayItem{ key: 'meta_data', val: rt.call_method(var_fulfillment, 'get_meta_data',
			[]rt.PhpVal{}) },
	])
}
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_fulfillments_schema_fulfillmentschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Fulfillments_Schema_FulfillmentSchema {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Fulfillments_Schema_FulfillmentSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_item_response' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Fulfillments_Schema_array](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
