	...
	return rt.create_array([
		rt.ArrayItem{ key: 'order_id', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Filter refunds by order ID.'),
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Refunds_CollectionQuery) get_query_args(mut var_request Class_WP_REST_Request) rt.PhpVal {
	mut var_args := rt.create_array([
		...
		rt.ArrayItem{ key: 'posts_per_page', val: var_request.array_get(rt.new_string('per_page')) },
	])
	...
	mut var_order_id := rt.call_function('absint', [if !(var_request.array_get(rt.new_string('order_id'))).is_null() {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_refunds_collectionquery(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Refunds_CollectionQuery {
