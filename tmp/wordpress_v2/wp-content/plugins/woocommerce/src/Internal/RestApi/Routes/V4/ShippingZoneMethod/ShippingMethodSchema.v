...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZoneMethod_ShippingMethodSchema) get_item_schema_properties() rt.PhpVal {
	return rt.create_array([
		...
		rt.ArrayItem{ key: 'zone_id', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Shipping zone ID.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'integer' },
			rt.ArrayItem{ key: 'context', val: rt.create_array([
		...
		rt.ArrayItem{ key: 'settings', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Shipping method settings including title and configuration.'),
				rt.new_string('woocommerce'),
			]) },
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZoneMethod_ShippingMethodSchema) get_item_response(var_method rt.PhpVal, mut var_request Class_WP_REST_Request, mut var_include_fields Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZoneMethod_array) rt.PhpVal {
	if var_request.array_isset(rt.new_string('zone_id')) {
		mut var_zone_id := rt.new_int((var_request.array_get(rt.new_string('zone_id'))).to_i64())
	} else {
		mut iife_temp_0 :=
			Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZoneMethod_WC_Data_Store{}
		mut iife_result_0 := iife_temp_0.load(rt.new_string('shipping-zone'))
		mut var_data_store := iife_result_0
		var_zone_id = rt.call_method(var_data_store, 'get_zone_id_by_instance_id', [
			rt.get_property(var_method, 'instance_id'),
		])
	}
	return rt.create_array([
		rt.ArrayItem{ key: 'instance_id', val: rt.new_int((rt.get_property(var_method,
			'instance_id')).to_i64()) },
		rt.ArrayItem{ key: 'zone_id', val: rt.new_int(var_zone_id.to_i64()) },
		rt.ArrayItem{ key: 'enabled', val: rt.call_function('wc_string_to_bool', [
			rt.get_property(var_method, 'enabled'),
		]) },
		rt.ArrayItem{
			key: 'order'
			val: rt.new_int((rt.get_property(var_method, 'method_order')).to_i64())
		},
	...
	for {
		...
		if !(rt.get_property(var_method, '{"nodeType":"Expr_Variable","line":126,"name":"field"}')).is_null() {
			var_settings.array_set(var_field, rt.get_property(var_method,
				'{"nodeType":"Expr_Variable","line":127,"name":"field"}'))
		}
	}
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_shippingzonemethod_shippingmethodschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZoneMethod_ShippingMethodSchema {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_shippingzonemethod_wc_data_store(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZoneMethod_WC_Data_Store {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZoneMethod_ShippingMethodSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_item_response' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_ShippingZoneMethod_array](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
