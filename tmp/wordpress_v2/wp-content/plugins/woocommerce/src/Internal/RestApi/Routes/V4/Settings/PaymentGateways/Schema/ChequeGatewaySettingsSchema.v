...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_PaymentGateways_Schema_ChequeGatewaySettingsSchema) get_custom_groups_for_gateway(mut var_gateway Class_WC_Payment_Gateway) rt.PhpVal {
	mut var_core_field_overrides := rt.create_array([
		rt.ArrayItem{ key: 'enabled', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'desc', val: rt.call_function('__', [
				rt.new_string('Enable check payments at checkout'),
				rt.new_string('woocommerce'),
			]) },
		]) },
		...
		rt.ArrayItem{ key: 'instructions', val: rt.create_array([
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Instructions shown after checkout'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'text' },
			rt.ArrayItem{ key: 'desc', val: rt.call_function('__', [
	...
	mut var_group := rt.create_array([
		rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
			rt.new_string('Check payment settings'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Manage how check payments appear at checkout and in order emails.'),
			rt.new_string('woocommerce'),
		]) },
