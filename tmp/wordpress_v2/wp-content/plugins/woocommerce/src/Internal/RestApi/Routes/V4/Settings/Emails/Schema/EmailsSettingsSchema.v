...
pub fn Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema.fields_supporting_personalization_tags() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: none, val: 'subject' },
		rt.ArrayItem{ key: none, val: 'preheader' }, rt.ArrayItem{ key: none, val: 'subject_full' },
		rt.ArrayItem{ key: none, val: 'subject_partial' }])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) init() {
	...
	this.personalization_tags_registry = rt.call_method(iife_result_0, 'get', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) get_item_schema_properties() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.create_array([
			...
			rt.ArrayItem{
				key: 'context'
				val: Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema.view_edit_context()
			},
			rt.ArrayItem{ key: 'readonly', val: true },
		]) },
		rt.ArrayItem{ key: 'title', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Email title.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{
		...
		rt.ArrayItem{ key: 'description', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Email description.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{
		...
		rt.ArrayItem{ key: 'post_id', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Template post ID.'),
				rt.new_string('woocommerce'),
			]) },
		...
		rt.ArrayItem{ key: 'link', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Link to template editor.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
		...
		rt.ArrayItem{ key: 'email_group', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Email group identifier.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{
		...
		rt.ArrayItem{ key: 'email_group_title', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Email group title.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'string' },
			rt.ArrayItem{
		...
		rt.ArrayItem{ key: 'is_customer_email', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Whether this is a customer email.'),
				rt.new_string('woocommerce'),
			]) },
		...
		rt.ArrayItem{ key: 'is_manual', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Whether this is sent only manually.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'boolean' },
			rt.ArrayItem{
		...
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'type', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'enum', val: rt.create_array([
					...
					rt.ArrayItem{ key: none, val: 'textarea' },
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) get_item_response(var_email rt.PhpVal, mut var_request Class_WP_REST_Request, mut var_include_fields Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_array) rt.PhpVal {
	...
	mut var_email_post_manager := iife_result_1
	mut var_post_id := rt.call_method(var_email_post_manager, 'get_email_template_post_id', [
		if !(rt.get_property(var_email_mutated, 'id')).is_null() {
			rt.get_property(var_email_mutated, 'id')
		} else {
			rt.new_string('')
		},
	])
	var_post_id = if rt.is_true(var_post_id) {
	...
	if rt.is_true(var_post_id) {
		mut var_permalink := rt.call_function('get_permalink', [
			var_post_id.clone()])
		var_link = if var_permalink.clone().is_string() { var_permalink } else { rt.new_string('') }
	}
	rt.call_method(var_email_mutated, 'init_form_fields', []rt.PhpVal{})
	mut var_response := rt.create_array([
		rt.ArrayItem{
			key: 'id'
			val: if !(rt.get_property(var_email_mutated, 'id')).is_null() {
				rt.get_property(var_email_mutated, 'id')
			} else {
				rt.new_string('')
			}
		...
		rt.ArrayItem{
			key: 'title'
			val: if !(rt.get_property(var_email_mutated, 'title')).is_null() {
				rt.get_property(var_email_mutated, 'title')
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{
			key: 'description'
			val: if !(rt.get_property(var_email_mutated, 'description')).is_null() {
				rt.get_property(var_email_mutated, 'description')
			} else {
				rt.new_string('')
			}
		},
		...
		rt.ArrayItem{
			key: 'email_group'
			val: if !(rt.get_property(var_email_mutated, 'email_group')).is_null() {
				rt.get_property(var_email_mutated, 'email_group')
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{
			key: 'email_group_title'
			val: if rt.is_true(rt.call_function('method_exists', [
				var_email_mutated.clone(),
				rt.new_string('get_email_group_title'),
			]))
			{
				rt.call_method(var_email_mutated, 'get_email_group_title', []rt.PhpVal{})
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{
			...
			val: if rt.is_true(rt.call_function('method_exists', [
				var_email_mutated.clone(),
				rt.new_string('is_customer_email'),
			]))
			{
				rt.call_method(var_email_mutated, 'is_customer_email', []rt.PhpVal{})
			} else {
				rt.new_bool(false)
			}
		},
		rt.ArrayItem{
			...
			val: if rt.is_true(rt.call_function('method_exists', [
				var_email_mutated.clone(),
				rt.new_string('is_manual'),
			]))
			{
				rt.call_method(var_email_mutated, 'is_manual', []rt.PhpVal{})
			} else {
				rt.new_bool(false)
			}
		},
		rt.ArrayItem{
			key: 'values'
			val: this.get_values(mut rt.cast_object_ptr[Class_WC_Email](var_email_mutated))
		},
		rt.ArrayItem{
			key: 'groups'
			val: this.get_groups(mut rt.cast_object_ptr[Class_WC_Email](var_email_mutated))
		},
	])
	if !(!rt.is_true(var_include_fields)) {
		var_response = rt.call_function('array_intersect_key', [
			var_response.clone(), rt.call_function('array_flip', [var_include_fields])])
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) get_values(mut var_email Class_WC_Email) rt.PhpVal {
	...
	if !(var_form_fields.clone().is_array()) {
		return var_values.clone()
	}
	rt.set_property(var_email_mutated, 'object',
		create_automattic_woocommerce_internal_restapi_routes_v4_settings_emails_schema_wc_order())
	mut iter_1 := var_form_fields.iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_field := item_1.val
		mut var_id := item_1.key
		mut var_field_type := if !(var_field.array_get(rt.new_string('type'))).is_null() {
			var_field.array_get(rt.new_string('type'))
		} else {
			rt.new_string('text')
		}
		if rt.is_true(rt.call_function('in_array', [var_field_type.clone(),
		...
		mut var_default := this.get_field_default_value(mut var_email_mutated, var_id.str(), mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_array](var_field))
		mut var_value := rt.call_method(var_email_mutated, 'get_option', [
			var_id.clone(), var_default.clone()])
		if rt.is_true(rt.call_function('in_array', [var_id.clone(),
			Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema.fields_supporting_personalization_tags(),
			rt.new_bool(true)]))
		{
			var_value = this.unwrap_woocommerce_tags(var_value.clone())
		}
		if rt.is_true(rt.identical(rt.new_string('checkbox'), var_field_type)) {
			var_value = rt.identical(rt.new_string('yes'), var_value)
		}
		var_values.array_set(var_id, var_value.clone())
		if rt.is_true(rt.identical(rt.new_string('customer_refunded_order'), rt.get_property(var_email_mutated, 'id')))
			&& rt.is_true(rt.identical(rt.new_string('subject_full'), var_id)) {
			if !(var_values.array_isset(rt.new_string('subject'))) {
				var_values.array_set('subject', var_value.clone())
			}
		}
		if rt.is_true(rt.identical(rt.new_string('customer_partially_refunded_order'), rt.get_property(var_email_mutated, 'id')))
			&& rt.is_true(rt.identical(rt.new_string('subject_partial'), var_id)) {
			if !(var_values.array_isset(rt.new_string('subject'))) {
				var_values.array_set('subject', var_value.clone())
			}
		}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) get_field_default_value(mut var_email Class_WC_Email, id string, mut var_field Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_array) rt.PhpVal {
	mut var_email_mutated := var_email
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('enabled'))) {
		return if rt.is_true(rt.call_function('method_exists', [var_email_mutated,
			rt.new_string('is_enabled')]))
		{
			rt.call_method(var_email_mutated, 'is_enabled', []rt.PhpVal{})
		} else {
			rt.new_bool(false)
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('recipient'))) {
		return if rt.is_true(rt.call_function('method_exists', [var_email_mutated,
			rt.new_string('get_recipient')]))
		{
			rt.call_method(var_email_mutated, 'get_recipient', []rt.PhpVal{})
		} else {
			rt.new_string('')
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('subject'))) {
		return if rt.is_true(rt.call_function('method_exists', [var_email_mutated,
			rt.new_string('get_subject')]))
		{
			rt.call_method(var_email_mutated, 'get_subject', []rt.PhpVal{})
		} else {
			rt.new_string('')
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('heading'))) {
		return if rt.is_true(rt.call_function('method_exists', [var_email_mutated,
			rt.new_string('get_heading')]))
		{
			rt.call_method(var_email_mutated, 'get_heading', []rt.PhpVal{})
		} else {
			rt.new_string('')
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('preheader'))) {
		return if rt.is_true(rt.call_function('method_exists', [var_email_mutated,
			rt.new_string('get_preheader')]))
		{
			rt.call_method(var_email_mutated, 'get_preheader', []rt.PhpVal{})
		} else {
			rt.new_string('')
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('additional_content'))) {
		return if rt.is_true(rt.call_function('method_exists', [var_email_mutated,
			rt.new_string('get_additional_content')]))
		{
			rt.call_method(var_email_mutated, 'get_additional_content', []rt.PhpVal{})
		} else {
			rt.new_string('')
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('cc'))) {
		return if !(rt.get_property(var_email_mutated, 'cc')).is_null() {
			rt.get_property(var_email_mutated, 'cc')
		} else {
			rt.new_string('')
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('bcc'))) {
		return if !(rt.get_property(var_email_mutated, 'bcc')).is_null() {
			rt.get_property(var_email_mutated, 'bcc')
		} else {
			rt.new_string('')
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('email_type'))) {
		return if !(rt.get_property(var_email_mutated, 'email_type')).is_null() {
			rt.get_property(var_email_mutated, 'email_type')
		} else {
			rt.new_string('')
		}
	} else {
		return if !(var_field_mutated.array_get(rt.new_string('default'))).is_null() {
			var_field_mutated.array_get(rt.new_string('default'))
		} else {
			if !(var_field_mutated.array_get(rt.new_string('placeholder'))).is_null() {
				var_field_mutated.array_get(rt.new_string('placeholder'))
			} else {
				rt.new_string('')
			}
		}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) unwrap_woocommerce_tags(var_value rt.PhpVal) rt.PhpVal {
	...
	mut var_escaped_prefixes := rt.call_function('array_map', [
		rt.new_string('preg_quote'),
		var_prefixes.clone(),
	])
	mut var_prefixes_pattern := rt.call_function('implode', [
		rt.new_string('|'), var_escaped_prefixes.clone()])
	mut var_unwrapped_value := rt.call_function('preg_replace', [
		rt.new_string('/<!--(\\[(?:' + var_prefixes_pattern.str() + ')\\/[^\\]]+\\])-->/i'),
		rt.new_string('$1'),
		var_value_mutated.clone(),
	])
	return var_unwrapped_value.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) get_groups(mut var_email Class_WC_Email) rt.PhpVal {
	...
	mut var_form_fields := rt.call_method(var_email_mutated, 'get_form_fields', []rt.PhpVal{})
	mut iter_2 := var_form_fields.iterator()
	for {
		...
		mut var_field_schema := rt.create_array([rt.ArrayItem{ key: 'id', val: var_id },
			rt.ArrayItem{
				key: 'label'
				val: if !(var_field.array_get(rt.new_string('title'))).is_null() {
					var_field.array_get(rt.new_string('title'))
				} else {
					var_id
				}
			}, rt.ArrayItem{ key: 'type', val: var_field_type },
			rt.ArrayItem{
				key: 'desc'
				val: if !(var_field.array_get(rt.new_string('description'))).is_null() {
					var_field.array_get(rt.new_string('description'))
				} else {
					rt.new_string('')
				}
			}])
		if var_field.array_isset(rt.new_string('options'))
			&& var_field.array_get(rt.new_string('options')).is_array() {
			var_field_schema.array_set('options', var_field.array_get(rt.new_string('options')))
		}
		var_group.array_get_mut('fields').array_push(var_field_schema.clone())
	}
	if !rt.is_true(var_group.array_get(rt.new_string('fields'))) {
		return rt.new_array()
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) validate_and_sanitize_settings(mut var_email Class_WC_Email, mut var_values Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_array) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.call_function('in_array', [var_field_id.clone(),
			Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema.fields_supporting_personalization_tags(),
			rt.new_bool(true)]))
		{
			var_value = this.unwrap_woocommerce_tags(var_value.clone())
		}
		mut var_sanitized := this.sanitize_field_value(var_field_type.str(), var_value.clone())
		if rt.is_true(rt.call_function('in_array', [var_field_id.clone(),
			Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema.fields_supporting_personalization_tags(),
			rt.new_bool(true)]))
		{
			var_sanitized = this.wrap_woocommerce_tags(var_sanitized.clone())
		}
		mut var_validation := rt.new_bool(this.validate_field_value(var_field_id.str(),
			var_sanitized.clone(), mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_array](var_field)))
		if rt.is_true(rt.call_function('is_wp_error', [var_validation.clone()])) {
			return var_validation.clone()
		}
		var_validated.array_set(var_field_id, var_sanitized.clone())
	}
	var_validated = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_emails_settings_schema_validate_and_sanitize_settings'),
		var_validated.clone(),
		var_email_mutated,
		var_values_mutated,
	])
	...
	if !(var_validated.clone().is_array()) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) sanitize_field_value(type string, var_value rt.PhpVal) rt.PhpVal {
	mut var_value_mutated := var_value
	...
	if rt.is_true(rt.equal(switch_val_2, rt.new_string('checkbox'))) {
		...
		return rt.call_function('sanitize_email', [var_value_mutated.clone()])
		...
		mut var_int_value := rt.call_function('filter_var', [
			var_value_mutated.clone(), rt.get_constant('FILTER_VALIDATE_INT'),
			rt.get_constant('FILTER_NULL_ON_FAILURE')])
		return if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_int_value)))) {
			var_int_value
		} else {
		...
		return if var_value_mutated.clone().is_string() { rt.create_array([
		...
		return rt.new_string((if rt.is_true(rt.call_function('is_scalar', [
			var_value_mutated.clone()]))
		{ var_value_mutated.str().trim_space() } else { '' }).str())
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) validate_field_value(key string, var_value rt.PhpVal, mut var_field Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_array) bool {
	mut var_value_mutated := var_value
	mut var_field_mutated := var_field
	mut var_field_type := if !(var_field_mutated.array_get(rt.new_string('type'))).is_null() {
		var_field_mutated.array_get(rt.new_string('type'))
	} else {
		rt.new_string('text')
	}
	if rt.is_true(rt.identical(rt.new_string('email'), var_field_type))
		&& !(!rt.is_true(var_value_mutated))
		...
		return (create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('sprintf', [
			rt.call_function('__', [rt.new_string('Invalid email format for %s.'),
				rt.new_string('woocommerce')]),
			rt.new_string(key),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))).to_bool()
	}
	if rt.is_true(rt.identical(rt.new_string('select'), var_field_type))
		&& !(!rt.is_true(var_field_mutated.array_get(rt.new_string('options')))) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_field_mutated.array_get(rt.new_string('options')).array_isset(var_value_mutated.clone()))))))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_value_mutated)))) {
			return (create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('Invalid value for %1$s. Valid options: %2$s'),
					rt.new_string('woocommerce'),
				]),
				rt.new_string(key),
				rt.call_function('implode', [
					rt.new_string(', '),
					rt.func_array_keys(var_field_mutated.array_get(rt.new_string('options'))),
				]),
			]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))).to_bool()
		}
	}
	if rt.is_true(rt.identical(rt.new_string('multiselect'), var_field_type))
		&& !(!rt.is_true(var_field_mutated.array_get(rt.new_string('options')))) {
		if rt.is_true(rt.new_bool(var_value_mutated.clone().is_array())) {
			...
			for {
				...
				if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_field_mutated.array_get(rt.new_string('options')).array_isset(var_v.clone())))))) {
					return (create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('sprintf', [
						rt.call_function('__', [
							rt.new_string('Invalid option "%2$s" for %1$s.'),
							rt.new_string('woocommerce'),
						]),
						rt.new_string(key),
						var_v.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) get_personalization_tag_prefixes() rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_null(), this.personalization_tags_registry)) {
		return rt.new_array()
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), this.cached_prefixes)))) {
		return this.cached_prefixes
	}
	mut var_prefixes := rt.new_array()
	...
	for {
		...
		{
			var_prefixes.array_set(var_matches.array_get(rt.new_int(1)), true)
		}
	}
	this.cached_prefixes = rt.func_array_keys(var_prefixes.clone())
	return this.cached_prefixes
}
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_emails_schema_emailssettingsschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_emails_schema_wc_order(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_WC_Order {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_EmailsSettingsSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_item_response' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_array](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
		...
		'get_values' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WC_Email](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
		...
		'get_field_default_value' {
			...
			return this.get_field_default_value(mut dispatch_arg_0, dispatch_arg_1, mut
				dispatch_arg_2)
		}
		...
		'validate_and_sanitize_settings' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Emails_Schema_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
		...
		'sanitize_field_value' {
			...
			return this.sanitize_field_value(dispatch_arg_0, dispatch_arg_1)
		}
		'validate_field_value' {
			...
			return rt.new_bool(this.validate_field_value(dispatch_arg_0, dispatch_arg_1, mut
				dispatch_arg_2))
		}
