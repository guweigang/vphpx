...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) get_item_schema_properties() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.create_array([
			...
			rt.ArrayItem{
				key: 'context'
				val: Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema.view_edit_context()
			},
		...
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'type', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'enum', val: rt.create_array([
					...
					rt.ArrayItem{ key: none, val: 'radio' },
			...
			rt.ArrayItem{ key: 'options', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Available options for select/radio fields.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'object' },
				rt.ArrayItem{
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) get_item_response(var_item rt.PhpVal, mut var_request Class_WP_REST_Request, mut var_include_fields Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_array) rt.PhpVal {
	...
	for {
		...
		if var_setting.array_isset(rt.new_string('id')) && rt.is_true(var_current_group) {
			mut var_field :=
				this.transform_setting_to_field(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_array](var_setting))
	...
	mut var_response := rt.create_array([rt.ArrayItem{ key: 'id', val: 'tax' },
		rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
			rt.new_string('Taxes'), rt.new_string('woocommerce')]) },
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Manage your store’s tax setup.'),
			rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'values', val: var_values },
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) normalize_field_type(wc_type string) string {
	mut var_type_map := rt.create_array([
		...
		rt.ArrayItem{ key: 'radio', val: 'radio' },
	])
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) validate_field_value(var_value rt.PhpVal, type string) rt.PhpVal {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_tax_schema_taxsettingsschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_TaxSettingsSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_item_response' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_array](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
		...
		'transform_setting_to_field' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Tax_Schema_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
