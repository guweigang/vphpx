...
pub fn Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_EmailSettingsSchema.non_editable_types() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: none, val: 'title' },
		rt.ArrayItem{ key: none, val: 'sectionend' }, rt.ArrayItem{
			key: none
			val: 'email_color_palette'
		}, rt.ArrayItem{ key: none, val: 'previewing_new_templates' },
		rt.ArrayItem{ key: none, val: 'email_improvements_button' },
		rt.ArrayItem{ key: none, val: 'email_notification' },
		rt.ArrayItem{ key: none, val: 'email_notification_block_emails' },
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_EmailSettingsSchema) get_item_schema_properties() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.create_array([
			...
			rt.ArrayItem{
				key: 'context'
				val: Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_EmailSettingsSchema.view_edit_context()
			},
	...
	return rt.create_array([rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'desc', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Setting field description.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{
			...
			rt.ArrayItem{ key: 'options', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Available options for selectable fields.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'object' },
				rt.ArrayItem{
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_EmailSettingsSchema) get_item_response(var_item rt.PhpVal, mut var_request Class_WP_REST_Request, mut var_include_fields Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_array) rt.PhpVal {
	mut var_settings := if var_item.clone().is_array() { var_item } else { rt.new_array() }
	...
	for {
		...
		if rt.is_true(rt.identical(rt.new_string('title'), var_setting_type))
			|| rt.is_true(rt.identical(rt.new_string('email_color_palette'), var_setting_type)) {
		...
		if rt.is_true(rt.call_function('in_array', [var_setting_type.clone(),
			Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_EmailSettingsSchema.non_editable_types(),
			rt.new_bool(true)]))
		{
			continue
		}
		if var_setting.array_isset(rt.new_string('id')) && rt.is_true(var_current_group) {
			mut var_setting_id := var_setting.array_get(rt.new_string('id'))
			var_setting_type = if !(var_setting.array_get(rt.new_string('type'))).is_null() {
				var_setting.array_get(rt.new_string('type'))
			} else {
				rt.new_string('text')
			}
			mut var_api_type :=
				rt.new_string(this.map_setting_type_to_api_type(var_setting_type.str()))
			mut var_field := rt.create_array([
				rt.ArrayItem{ key: 'id', val: var_setting_id },
				rt.ArrayItem{
					key: 'label'
					val: if !(var_setting.array_get(rt.new_string('title'))).is_null() {
						var_setting.array_get(rt.new_string('title'))
				...
				rt.ArrayItem{ key: 'type', val: var_api_type },
			])
			if !(!rt.is_true(var_setting.array_get(rt.new_string('desc')))) {
				var_field.array_set('desc', var_setting.array_get(rt.new_string('desc')))
			}
			...
			mut var_current_value := rt.call_function('get_option', [
				var_setting_id.clone(), var_default_value.clone()])
			...
			var_values.array_set(var_setting_id, var_current_value.clone())
		}
	}
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_group := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return rt.new_bool(!(!rt.is_true(var_group.array_get(rt.new_string('fields')))))
	}
	var_groups = rt.call_function('array_filter', [var_groups.clone(),
		rt.new_closure(closure_1_fn)])
	mut var_response := rt.create_array([rt.ArrayItem{ key: 'id', val: 'email' },
		rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
			rt.new_string('Email design'),
			rt.new_string('woocommerce'),
		]) }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Customize the look and feel of all you notification emails.'),
			rt.new_string('woocommerce'),
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_EmailSettingsSchema) map_setting_type_to_api_type(setting_type string) string {
	...
	mut var_type_map := rt.create_array([rt.ArrayItem{ key: 'text', val: 'text' },
		rt.ArrayItem{ key: 'email', val: 'email' }, rt.ArrayItem{ key: 'checkbox', val: 'checkbox' },
		rt.ArrayItem{ key: 'number', val: 'number' }, rt.ArrayItem{ key: 'color', val: 'color' },
		rt.ArrayItem{ key: 'select', val: 'select' }])
	return (if !(var_type_map.array_get(rt.new_string(setting_type_mutated))).is_null() {
		var_type_map.array_get(rt.new_string(setting_type_mutated))
	} else {
		rt.new_string('text')
	}).str()
}
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_email_schema_emailsettingsschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_EmailSettingsSchema {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_EmailSettingsSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_item_response' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Email_Schema_array](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
