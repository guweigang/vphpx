...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_GeneralSettingsSchema) get_item_schema_properties() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.create_array([
			...
			rt.ArrayItem{
				key: 'context'
				val: Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_GeneralSettingsSchema.view_edit_context()
			},
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_GeneralSettingsSchema) get_item_response(var_item rt.PhpVal, mut var_request Class_WP_REST_Request, mut var_include_fields Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_array) rt.PhpVal {
	...
	for {
		...
		if var_setting.array_isset(rt.new_string('id')) && rt.is_true(var_current_group) {
			mut var_field :=
				this.transform_setting_to_field(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_array](var_setting))
	...
	mut var_response := rt.create_array([rt.ArrayItem{ key: 'id', val: 'general' },
		rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
			rt.new_string('General'),
			rt.new_string('woocommerce'),
		]) }, rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string("Set your store's address, visibility, currency, language, and timezone."),
			rt.new_string('woocommerce'),
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_GeneralSettingsSchema) get_field_options(field_id string) rt.PhpVal {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('woocommerce_currency'))) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('get_woocommerce_currencies')])))))
			|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('get_woocommerce_currency_symbol')]))))) {
			return rt.new_array()
		}
		mut var_currencies := rt.call_function('get_woocommerce_currencies', []rt.PhpVal{})
		mut var_options := rt.new_array()
		mut iter_2 := var_currencies.iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_name := item_2.val
			mut var_code := item_2.key
			mut var_label := rt.call_function('wp_specialchars_decode', [
				rt.new_string(var_name.str()),
			])
			mut var_symbol := rt.call_function('wp_specialchars_decode', [
				rt.new_string((rt.call_function('get_woocommerce_currency_symbol', [
					var_code.clone(),
				])).str()),
			])
			var_options.array_set(var_code, var_label.str() + ' (' + var_symbol.str() + ') — ' +
				var_code.str())
		}
		...
		for {
			...
			mut var_country_name := item_3.val
			mut var_country_code := item_3.key
			mut var_country_states := if !(var_states.array_get(var_country_code)).is_null() {
				var_states.array_get(var_country_code)
			} else {
				rt.new_array()
			}
			if !rt.is_true(var_country_states) {
				var_options.array_set(var_country_code, var_country_name.clone())
			} else {
				mut iter_4 := var_country_states.iterator()
				for {
					item_4 := iter_4.next() or { break }
					mut var_state_name := item_4.val
					mut var_state_code := item_4.key
					var_options.array_set(var_country_code.str() + ':' + var_state_code.str(),

						var_country_name.str() + ' — ' + var_state_name.str())
				}
			}
		}
		return var_options.clone()
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_GeneralSettingsSchema) validate_field_value(var_value rt.PhpVal, type string) rt.PhpVal {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_general_schema_generalsettingsschema(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_GeneralSettingsSchema {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_GeneralSettingsSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_item_response' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_array](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
		...
		'transform_setting_to_field' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_General_Schema_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
