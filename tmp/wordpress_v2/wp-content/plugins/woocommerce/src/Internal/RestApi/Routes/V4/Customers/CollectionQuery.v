	...
	return rt.create_array([
		...
		rt.ArrayItem{ key: 'orderby', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'enum', val: rt.create_array([
				...
				rt.ArrayItem{ key: none, val: 'order_count' },
				rt.ArrayItem{ key: none, val: 'total_spent' },
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_CollectionQuery) get_query_args(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	var_prepared_args.array_set('exclude', var_request.array_get(rt.new_string('exclude')))
	...
	var_prepared_args.array_set('page', rt.call_function('max', [
		rt.new_int(1), rt.new_int(var_request.array_get(rt.new_string('page')).to_i64())]))
	mut var_orderby_possibles := rt.create_array([rt.ArrayItem{ key: 'id', val: 'ID' },
		rt.ArrayItem{ key: 'name', val: 'display_name' }, rt.ArrayItem{
			key: 'registered_date'
			val: 'user_registered'
		}, rt.ArrayItem{ key: 'order_count', val: 'wc_order_count' },
		rt.ArrayItem{ key: 'total_spent', val: 'wc_money_spent' },
		rt.ArrayItem{ key: 'last_active', val: 'wc_last_active' }])
	...
	var_prepared_args.array_set('role', var_request.array_get(rt.new_string('role')))
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_CollectionQuery) get_query_results(mut var_query_args Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_array, mut var_request Class_WP_REST_Request) rt.PhpVal {
	mut var_method_args := rt.create_array([
		rt.ArrayItem{
			key: 'order'
			val: if !(var_query_args.array_get(rt.new_string('order'))).is_null() {
		...
		rt.ArrayItem{
			key: 'orderby'
			val: if !(var_query_args.array_get(rt.new_string('orderby'))).is_null() {
		...
		rt.ArrayItem{
			key: 'per_page'
			val: if !(var_query_args.array_get(rt.new_string('number'))).is_null() {
		...
		rt.ArrayItem{
			key: 'search'
			val: if !(var_query_args.array_get(rt.new_string('search'))).is_null() {
				var_query_args.array_get(rt.new_string('search'))
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{
			key: 'role'
			val: if !(var_query_args.array_get(rt.new_string('role'))).is_null() {
				var_query_args.array_get(rt.new_string('role'))
			} else {
		...
		rt.ArrayItem{
			key: 'include'
			val: if !(var_query_args.array_get(rt.new_string('include'))).is_null() {
				var_query_args.array_get(rt.new_string('include'))
			} else {
				rt.new_array()
			}
		},
		rt.ArrayItem{
			key: 'exclude'
			val: if !(var_query_args.array_get(rt.new_string('exclude'))).is_null() {
				var_query_args.array_get(rt.new_string('exclude'))
			} else {
				rt.new_array()
			}
		},
		rt.ArrayItem{
			key: 'page'
			val: if !(var_query_args.array_get(rt.new_string('page'))).is_null() {
	...
	mut iife_temp_0 :=
		Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_WC_Data_Store{}
	mut iife_result_0 := iife_temp_0.load(rt.new_string('customer'))
	mut var_data_store := iife_result_0
	mut var_customer_query_results := rt.call_method(var_data_store, 'query_customers', [
		var_method_args.clone(),
	])
	...
	return rt.create_array([rt.ArrayItem{ key: 'results', val: var_users },
		rt.ArrayItem{ key: 'total', val: var_total_users }, rt.ArrayItem{
			key: 'pages'
			val: var_max_pages
		}])
}
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_customers_collectionquery(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_CollectionQuery {
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_customers_wc_data_store(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_WC_Data_Store {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_CollectionQuery) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_query_results' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Customers_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
