...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller) register_routes() {
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller) get_item_permissions_check(var_request rt.PhpVal) bool {
	...
	])))))
	{
		return (create_wp_error(rt.new_string('rest_forbidden'), rt.call_function('__', [
			rt.new_string('Sorry, you are not allowed to access product settings.'),
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller) update_item_permissions_check(var_request rt.PhpVal) bool {
	...
	{
		return (create_wp_error(rt.new_string('rest_forbidden'), rt.call_function('__', [
			rt.new_string('Sorry, you are not allowed to edit product settings.'),
			rt.new_string('woocommerce'),
		]), rt.create_array([
			rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code',
				[]rt.PhpVal{}) },
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_response := rt.call_method(this.schema, 'get_item_response', [
		var_settings.clone(), var_request.clone()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller) update_item(var_request rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		mut var_sanitized_value := this.sanitize_setting_value(var_setting_type.str(),
			var_setting_value.clone())
		mut var_validation_result := rt.new_bool(this.validate_setting_value(var_setting_id.str(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller) validate_setting_value(setting_id string, var_value rt.PhpVal) bool {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('woocommerce_weight_unit'))) {
		mut var_valid_units := rt.call_function('apply_filters', [
			rt.new_string('woocommerce_weight_units'),
			rt.create_array([rt.ArrayItem{ key: none, val: 'kg' },
				rt.ArrayItem{ key: none, val: 'g' }, rt.ArrayItem{ key: none, val: 'lbs' },
				rt.ArrayItem{ key: none, val: 'oz' }]),
		])
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
			var_value_mutated.clone(), var_valid_units.clone(),
			rt.new_bool(true)])))))
		{
			return (create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('__', [
				rt.new_string('Invalid weight unit. Valid units are: kg, g, lbs, oz.'),
				rt.new_string('woocommerce'),
		...
		var_valid_units = rt.call_function('apply_filters', [
			rt.new_string('woocommerce_dimension_units'),
			rt.create_array([rt.ArrayItem{ key: none, val: 'm' },
				rt.ArrayItem{ key: none, val: 'cm' }, rt.ArrayItem{ key: none, val: 'mm' },
				rt.ArrayItem{ key: none, val: 'in' }, rt.ArrayItem{ key: none, val: 'yd' }]),
		])
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
			var_value_mutated.clone(), var_valid_units.clone(),
			rt.new_bool(true)])))))
		{
			return (create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('__', [
				rt.new_string('Invalid dimension unit. Valid units are: m, cm, mm, in, yd.'),
				rt.new_string('woocommerce'),
			]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))).to_bool()
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('woocommerce_product_type'))) {
		mut var_valid_types := rt.func_array_keys(rt.call_function('wc_get_product_types',
			[]rt.PhpVal{}))
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
			var_value_mutated.clone(), var_valid_types.clone(),
			rt.new_bool(true)])))))
		{
			return (create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('__', [
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller) sanitize_setting_value(setting_type string, var_value rt.PhpVal) rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller) get_all_settings() rt.PhpVal {
	...
	mut iter_3 := rt.func_array_keys(var_sections.clone()).iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_section := item_3.val
...
fn create_automattic_woocommerce_internal_restapi_routes_v4_settings_products_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller {
...
fn create_wc_settings_products(_args ...rt.PhpVal) &Class_WC_Settings_Products {
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Schema_ProductSettingsSchema](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
...
fn (mut this Class_Automattic_WooCommerce_Internal_RestApi_Routes_V4_Settings_Products_Controller) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'settings_products_instance' {
			this.settings_products_instance = val
			return true
		}
		else {
