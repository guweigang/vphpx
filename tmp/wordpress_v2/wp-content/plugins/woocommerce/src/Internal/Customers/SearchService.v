...
fn (mut this Class_Automattic_WooCommerce_Internal_Customers_SearchService) find_user_ids_by_billing_email_for_coupons_usage_lookup(mut var_emails Class_Automattic_WooCommerce_Internal_Customers_array) rt.PhpVal {
	...
	var_emails_mutated = rt.call_function('array_unique', [
		rt.call_function('array_map', [rt.new_string('strtolower'),
			rt.call_function('array_map', [rt.new_string('sanitize_email'), var_emails_mutated])]),
	])
	...
	if rt.is_true(iife_result_0) {
		mut var_placeholders := rt.call_function('implode', [
			...
			rt.call_function('array_fill', [
				rt.new_int(0), rt.new_int(var_emails_mutated.array_count()),
				rt.new_string('%s')])])
		mut iife_temp_1 :=
...
fn create_automattic_woocommerce_internal_customers_searchservice(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Customers_SearchService {
...
fn create_automattic_woocommerce_internal_customers_wp_user_query(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Customers_WP_User_Query {
