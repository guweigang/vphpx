...
fn Class_Automattic_WooCommerce_Internal_StockNotifications_Factory.get_notification(notification_id i64) rt.PhpVal {
	...
	mut var_notification :=
		create_automattic_woocommerce_internal_stocknotifications_notification(rt.new_int(notification_id))
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Internal_StockNotifications_Exception') {
		...
		rt.call_function('wc_caught_exception', [var_e.clone(),
			rt.new_string(@FN), rt.create_array([
				rt.ArrayItem{ key: none, val: notification_id },
			])])
		return rt.new_bool(false)
		unsafe {
			goto end_label_1
...
fn Class_Automattic_WooCommerce_Internal_StockNotifications_Factory.create_dummy_notification() rt.PhpVal {
	...
	var_product.set_name(rt.call_function('__', [rt.new_string('Dummy Product'),
		rt.new_string('woocommerce')]))
	var_product.set_price(rt.new_int(25))
	var_product.set_image_id(rt.call_function('get_option', [
	...
	rt.set_property(var_notification, 'product', var_product)
...
fn create_automattic_woocommerce_internal_stocknotifications_wc_product(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_StockNotifications_WC_Product {
