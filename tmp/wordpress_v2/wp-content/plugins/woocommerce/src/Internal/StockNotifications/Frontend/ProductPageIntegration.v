...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration) init(mut var_eligibility_service Class_Automattic_WooCommerce_Internal_StockNotifications_Utilities_EligibilityService, mut var_signup_service Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_SignupService) {
	...
	this.signup_service = var_signup_service
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration) construct() {
	rt.call_function('add_action', [rt.new_string('woocommerce_simple_add_to_cart'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'maybe_render_form' },
		]),
		rt.new_int(30)])
	rt.call_function('add_action', [rt.new_string('woocommerce_after_add_to_cart_form'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'maybe_render_form' },
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration) maybe_render_form() {
	...
	mut var_is_variable := rt.call_method(var_product, 'is_type', [
		rt.new_string('variable'),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_is_variable))))
		&& rt.is_true(rt.call_method(this.eligibility_service, 'is_stock_status_eligible', [rt.call_method(var_product, 'get_stock_status', []rt.PhpVal{})])) {
		return
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration) render_form(mut var_product Class_WC_Product) {
	...
	mut iife_result_1 := iife_temp_1.requires_account()
	...
	if this.is_personalization_enabled()
		...
		mut var_notification := rt.call_method(this.signup_service, 'is_already_signed_up', [
			var_product.get_id(),
		...
		{
			this.display_already_signed_up(mut var_product, mut
				rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_StockNotifications_Notification](var_notification))
			return
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration) display_account_required(mut var_product Class_WC_Product) {
	mut var_pre := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_customer_stock_notifications_account_required_message_html'),
		rt.new_null(),
		var_product,
	])
	if !(var_pre.clone().is_null()) {
		rt.echo_val(rt.call_function('wp_kses_post', [var_pre.clone()]))
		return
	}
	mut var_text := rt.call_function('__', [
		rt.new_string('Please {login_link} to sign up for stock notifications.'),
		rt.new_string('woocommerce'),
	])
	var_text = rt.call_function('str_replace', [rt.new_string('{login_link}'),
		rt.new_string('<a href="' +
			(rt.call_function('wc_get_account_endpoint_url', [rt.new_string('my-account')])).str() +
			'">' +
			(rt.call_function('_x', [rt.new_string('log in'), rt.new_string('back in stock form'), rt.new_string('woocommerce')])).str() +
			'</a>'),
		var_text.clone()])
	rt.call_function('wc_print_notice', [var_text.clone(), rt.new_string('notice')])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration) display_already_signed_up(mut var_product Class_WC_Product, mut var_notification Class_Automattic_WooCommerce_Internal_StockNotifications_Notification) {
	...
	mut var_pre := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_customer_stock_notifications_already_signed_up_message_html'),
		rt.new_null(),
		var_product,
		var_notification_mutated,
	])
	if !(var_pre.clone().is_null()) {
		rt.echo_val(rt.call_function('wp_kses_post', [var_pre.clone()]))
		return
	...
	mut var_text := rt.call_function('__', [
		rt.new_string('You have already joined the waitlist! Click {manage_account_link} to manage your notifications.'),
		rt.new_string('woocommerce'),
	])
	var_text = rt.call_function('str_replace', [rt.new_string('{manage_account_link}'),
		rt.new_string('<a href="' +
			(rt.call_function('wc_get_account_endpoint_url', [rt.new_string('stock-notifications')])).str() +
			'">' +
			(rt.call_function('_x', [rt.new_string('here'), rt.new_string('back in stock form'), rt.new_string('woocommerce')])).str() +
			'</a>'),
		var_text.clone()])
	rt.call_function('wc_print_notice', [var_text.clone(), rt.new_string('notice')])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration) display_form(mut var_product Class_WC_Product) {
	mut var_button_class := rt.call_function('implode', [rt.new_string(' '),
		rt.call_function('array_filter', [
			rt.create_array([rt.ArrayItem{ key: none, val: 'button' },
				rt.ArrayItem{ key: none, val: rt.call_function('wc_wp_theme_get_element_class_name', [
					rt.new_string('button'),
				]) }, rt.ArrayItem{ key: none, val: 'wc_bis_form__button' }]),
		])])
	mut var_is_visible := rt.new_bool(
		rt.is_true(rt.new_bool(!(rt.is_true(var_product.is_type(rt.new_string('variable'))))))
		|| rt.is_true(rt.identical(rt.new_string('yes'), rt.call_function('get_option', [rt.new_string('woocommerce_hide_out_of_stock_items')])))
		&& rt.is_true(rt.new_bool(!(rt.is_true(var_product.has_purchasable_variations())))))
	...
	mut iife_result_3 := iife_temp_3.requires_account()
	mut iife_temp_4 := Class_Automattic_WooCommerce_Internal_StockNotifications_Config{}
	mut iife_result_4 := iife_temp_4.requires_account()
	rt.call_function('wc_get_template', [
		rt.new_string('single-product/back-in-stock-form.php'),
		rt.create_array([
			rt.ArrayItem{
				key: 'product_id'
				val: if rt.is_true(var_product.get_parent_id()) {
					var_product.get_parent_id()
				} else {
					var_product.get_id()
				}
			},
			rt.ArrayItem{ key: 'show_checkbox', val:
				rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})))))
				&& rt.is_true(iife_result_2)
				&& rt.is_true(rt.new_bool(!(rt.is_true(iife_result_3)))) },
			rt.ArrayItem{ key: 'show_email_field', val:
				rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})))))
				&& rt.is_true(rt.new_bool(!(rt.is_true(iife_result_4)))) },
			rt.ArrayItem{ key: 'button_class', val: var_button_class },
			rt.ArrayItem{ key: 'is_visible', val: var_is_visible },
		]),
	])
}
...
fn Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration.is_personalization_enabled() bool {
	return (rt.call_function('apply_filters', [
		rt.new_string('woocommerce_customer_stock_notifications_personalization_enabled'),
		rt.new_bool(false),
	])).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_SignupService](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
