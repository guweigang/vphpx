...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_FormHandlerService) handle_signup() {
	...
	if rt.is_true(Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_FormHandlerService.requires_nonce_check()) {
		if !(rt.get_superglobal('_POST').array_isset(rt.new_string('wc_bis_nonce'))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_verify_nonce', [rt.call_function('wp_unslash', [rt.get_superglobal('_POST').array_get(rt.new_string('wc_bis_nonce'))]), rt.new_string('wc_bis_signup')]))))) {
	...
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_data := rt.call_method(this.signup_service, 'parse', [rt.get_superglobal('_POST').clone()])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	...
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_result := rt.call_method(this.signup_service, 'signup', [var_data.array_get(rt.new_string('product_id')), var_data.array_get(rt.new_string('user_id')), var_data.array_get(rt.new_string('user_email')), if !(var_data.array_get(rt.new_string('posted_attributes'))).is_null() { var_data.array_get(rt.new_string('posted_attributes')) } else { rt.new_array() }])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
fn Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_FormHandlerService.requires_nonce_check() bool {
	mut iife_temp_1 := Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration{}
	mut iife_result_1 := iife_temp_1.is_personalization_enabled()
	mut iife_temp_2 := Class_Automattic_WooCommerce_Internal_StockNotifications_Config{}
	mut iife_result_2 := iife_temp_2.requires_account()
	mut var_requires_account := rt.new_bool(rt.is_true(iife_result_1) && rt.is_true(iife_result_2) || rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})))
	return (rt.call_function('apply_filters', [rt.new_string('woocommerce_customer_stock_notifications_requires_nonce_check'), var_requires_account.clone()])).to_bool()
}
...
fn create_automattic_woocommerce_internal_stocknotifications_frontend_productpageintegration(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_StockNotifications_Frontend_ProductPageIntegration {
