...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_AdminManager) construct() {
	rt.call_function('add_action', [rt.new_string('admin_enqueue_scripts'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_StockNotifications_Admin_AdminManager',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'admin_resources' },
		]),
		rt.new_int(11)])
	mut var_container := rt.call_function('wc_get_container', []rt.PhpVal{})
	rt.call_method(var_container, 'get', [
		Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_MenusController.class(),
	])
	rt.call_method(var_container, 'get', [
		Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_SettingsController.class(),
	])
}
...
fn Class_Automattic_WooCommerce_Internal_StockNotifications_Admin_AdminManager.admin_resources() {
	...
	rt.call_function('wp_register_script', [
		rt.new_string('wc-admin-customer-stock-notifications'),
		rt.new_string(
			(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'plugin_url', []rt.PhpVal{})).str() +
			'/assets/js/admin/wc-customer-stock-notifications' + var_suffix.str() + '.js'),
		rt.create_array([rt.ArrayItem{ key: none, val: 'jquery' }]),
		var_version.clone(),
		rt.new_bool(true),
	])
	mut var_params := rt.create_array([
		rt.ArrayItem{ key: 'i18n_wc_delete_notification_warning', val: rt.call_function('__', [
			rt.new_string('Delete this notification permanently?'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'i18n_wc_bulk_delete_notifications_warning', val: rt.call_function('__', [
			rt.new_string('Delete the selected notifications permanently?'),
			rt.new_string('woocommerce'),
		]) },
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_screen_id.clone(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: 'woocommerce_page_wc-customer-stock-notifications' },
			rt.ArrayItem{ key: none, val: 'woocommerce_page_wc-settings' },
		]),
		rt.new_bool(true)])))))
	{
		return
	}
	if rt.is_true(rt.identical(rt.new_string('woocommerce_page_wc-settings'), var_screen_id))
	...
	rt.call_function('wp_localize_script', [
		rt.new_string('wc-admin-customer-stock-notifications'),
		rt.new_string('wc_admin_customer_stock_notifications_params'),
		var_params.clone(),
	])
}
