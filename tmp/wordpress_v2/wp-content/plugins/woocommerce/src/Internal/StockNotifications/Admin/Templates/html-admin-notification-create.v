...
fn main() {
	...
	mut var_user_id := rt.new_int(0)
	if !(!rt.is_true(rt.get_superglobal('_REQUEST').array_get(rt.new_string('user_id')))) {
	...
	mut var_email := if rt.get_superglobal('_REQUEST').array_isset(rt.new_string('user_email')) { rt.call_function('sanitize_text_field', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_REQUEST').array_get(rt.new_string('user_email'))]),
		]) } else { rt.new_string('') }
	// unsupported statement: Stmt_InlineHTML
	...
	rt.call_function('esc_html_e', [rt.new_string('Enter customer e-mail&hellip;'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_email.clone()]))
	// unsupported statement: Stmt_InlineHTML
	...
	if !(!rt.is_true(rt.get_superglobal('_REQUEST').array_get(rt.new_string('product_id')))) {
		var_product_id = rt.call_function('absint', [
			rt.call_function('wp_unslash',
				[rt.get_superglobal('_REQUEST').array_get(rt.new_string('product_id'))]),
		])
	...
	rt.echo_val(rt.call_function('esc_attr', [
		rt.call_function('implode', [rt.new_string(','), var_excluded_product_types.clone()]),
	]))
	// unsupported statement: Stmt_InlineHTML
