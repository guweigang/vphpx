...
fn (mut this Class_Automattic_WooCommerce_Internal_StockNotifications_Utilities_StockManagementHelper) get_managed_variations(mut var_product Class_WC_Product) rt.PhpVal {
	...
	mut var_managed_children := rt.call_method(var_wpdb, 'get_col', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.concat(rt.concat(rt.concat(rt.new_string('SELECT DISTINCT post_id FROM '), rt.get_property(var_wpdb,
				'postmeta')),
	...
	this.managed_variations.array_set(var_product_id, rt.call_function('array_map', [
		rt.new_string('intval'),
		var_managed_children.clone(),
	]))
	return this.managed_variations.array_get(var_product_id)
}

fn create_automattic_woocommerce_internal_stocknotifications_utilities_stockmanagementhelper(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_StockNotifications_Utilities_StockManagementHelper {
