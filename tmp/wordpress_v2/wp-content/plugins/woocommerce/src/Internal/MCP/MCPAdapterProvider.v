...
fn (mut this Class_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider) construct() {
	rt.call_function('add_action', [rt.new_string('rest_api_init'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'maybe_initialize' },
		]),
		rt.new_int(10)])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider) maybe_initialize() {
	...
	mut iife_result_0 := iife_temp_0.feature_is_enabled(rt.new_string('mcp_integration'))
...
fn (mut this Class_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider) initialize_mcp_adapter() {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('WP\\MCP\\Core\\McpAdapter'),
	])))))
	{
		...
		]))
		{
			rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'warning', [
				rt.new_string('MCP adapter class not found. Skipping MCP initialization.'),
				rt.create_array([rt.ArrayItem{ key: 'source', val: 'woocommerce-mcp' }]),
			])
		}
		return
	}
	mut iife_temp_1 := Class_Automattic_WooCommerce_Internal_MCP_WP_MCP_Core_McpAdapter{}
...
fn (mut this Class_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider) register_hooks() {
	rt.call_function('add_action', [rt.new_string('mcp_adapter_init'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider',
				[]string{}, &this) },
			rt.ArrayItem{ key: none, val: 'initialize_mcp_server' },
		])])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider) initialize_mcp_server(var_adapter rt.PhpVal) {
	...
	rt.call_function('add_filter', [rt.new_string('mcp_validation_enabled'),
		rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT },
			rt.ArrayItem{ key: none, val: 'disable_mcp_validation' }]),
		rt.new_int(999)])
	rt.call_method(var_adapter, 'create_server', [rt.new_string('woocommerce-mcp'),
		Class_Automattic_WooCommerce_Internal_MCP_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider.mcp_namespace(),
		Class_Automattic_WooCommerce_Internal_MCP_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider.mcp_route(),
		rt.call_function('__', [rt.new_string('WooCommerce MCP Server'),
			rt.new_string('woocommerce')]),
		rt.call_function('__', [rt.new_string('AI-accessible WooCommerce operations via MCP'),
			rt.new_string('woocommerce')]),
		rt.new_string('1.0.0'),
		rt.create_array([rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Internal_MCP_Transport_WooCommerceRestTransport.class()
		}]),
		Class_Automattic_WooCommerce_Internal_MCP_WP_MCP_Infrastructure_ErrorHandling_ErrorLogMcpErrorHandler.class(),
		Class_Automattic_WooCommerce_Internal_MCP_WP_MCP_Infrastructure_Observability_NullMcpObservabilityHandler.class(),
		var_abilities_ids.clone()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Internal_MCP_Throwable') {
		...
		]))
		{
			rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'error', [
				rt.new_string('MCP server initialization failed: ' +
	...
	finally_label_1:
	rt.call_function('remove_filter', [rt.new_string('mcp_validation_enabled'),
		rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT },
			rt.ArrayItem{ key: none, val: 'disable_mcp_validation' }]),
		rt.new_int(999)])
...
fn (mut this Class_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider) get_woocommerce_mcp_abilities() rt.PhpVal {
	mut var_abilities_registry := rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}),
		'get', [
		Class_Automattic_WooCommerce_Internal_Abilities_AbilitiesRegistry.class(),
	])
	mut var_all_abilities_ids := rt.call_method(var_abilities_registry, 'get_abilities_ids',
		[]rt.PhpVal{})
	closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_ability_id := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_include := rt.call_function('str_starts_with', [
			var_ability_id.clone(), rt.new_string('woocommerce/')])
		return rt.call_function('apply_filters', [
			rt.new_string('woocommerce_mcp_include_ability'),
			var_include.clone(),
			var_ability_id.clone(),
		])
	}
	mut var_mcp_abilities := rt.call_function('array_filter', [
		var_all_abilities_ids.clone(), rt.new_closure(closure_3_fn)])
	return rt.call_function('array_values', [var_mcp_abilities.clone()])
}
...
fn Class_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider.is_mcp_request() bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_is_serving_rest_request',
		[]rt.PhpVal{})))))
	{
		return false
	}
	mut var_request_uri := if rt.get_superglobal('_SERVER').array_isset(rt.new_string('REQUEST_URI')) { rt.call_function('sanitize_text_field', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_SERVER').array_get(rt.new_string('REQUEST_URI'))]),
		]) } else { rt.new_string('') }
	mut var_mcp_endpoint := rt.new_string('/' +
		(Class_Automattic_WooCommerce_Internal_MCP_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider.mcp_namespace()).str() +
		'/' +(Class_Automattic_WooCommerce_Internal_MCP_Automattic_WooCommerce_Internal_MCP_MCPAdapterProvider.mcp_route()).str())
	return rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('strpos', [
		var_request_uri.clone(),
		var_mcp_endpoint.clone(),
	]))))
}
...
fn create_automattic_woocommerce_internal_mcp_wp_mcp_core_mcpadapter(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_MCP_WP_MCP_Core_McpAdapter {
