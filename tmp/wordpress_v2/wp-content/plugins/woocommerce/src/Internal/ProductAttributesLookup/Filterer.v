...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_Filterer) filtering_via_lookup_table_is_active() rt.PhpVal {
	return rt.identical(rt.new_string('yes'), rt.call_function('get_option', [
		rt.new_string('woocommerce_attribute_lookup_enabled'),
	]))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_Filterer) filter_by_attribute_post_clauses(mut var_args Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_array, mut var_wp_query Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_WP_Query, mut var_attributes_to_filter_by Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_array) rt.PhpVal {
	...
	mut var_enable_filtering := rt.call_function('apply_filters', [
		...
		var_wp_query.is_main_query(),
		var_wp_query,
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_enable_filtering))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(this.filtering_via_lookup_table_is_active())))) {
	...
	mut var_hide_out_of_stock := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_product_attributes_filterer_hide_out_of_stock'),
	...
	for {
		...
		mut var_all_terms := rt.call_function('get_terms', [var_taxonomy.clone(),
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int(0), var_count)))) {
			if rt.is_true(var_is_and_query) && rt.is_true(rt.greater(var_count, rt.new_int(1))) {
				...
				var_clauses.array_push(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('\n\t\t\t\t\t\t\t'),
					...
					this.lookup_table_name), rt.new_string(' lt\n\t\t\t\t\t\t\tWHERE term_id in ')),
	...
	if !(!rt.is_true(var_clauses)) {
		var_args.array_get(rt.new_string('where')) = rt.concat(var_args.array_get(rt.new_string('where')), rt.new_string(
		...
		var_args.array_get(rt.new_string('where')) = rt.concat(var_args.array_get(rt.new_string('where')),
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_Filterer) get_filtered_term_product_counts(var_term_ids rt.PhpVal, var_taxonomy rt.PhpVal, var_query_type rt.PhpVal) rt.PhpVal {
	...
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_WC_Query{}
	mut iife_result_0 := iife_temp_0.get_main_tax_query()
	mut var_tax_query := iife_result_0
	mut iife_temp_1 := Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_WC_Query{}
	mut iife_result_1 := iife_temp_1.get_main_meta_query()
	mut var_meta_query := iife_result_1
	if rt.is_true(rt.identical(rt.new_string('or'), var_query_type)) {
		mut iter_2 := var_tax_query.iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_query := item_2.val
			mut var_key := item_2.key
			if var_query.clone().is_array()
				&& rt.is_true(rt.identical(var_taxonomy, var_query.array_get(rt.new_string('taxonomy')))) {
				var_tax_query.array_unset(var_key)
			}
		}
	}
	var_meta_query =
		create_automattic_woocommerce_internal_productattributeslookup_wp_meta_query(var_meta_query.clone())
	var_tax_query =
		create_automattic_woocommerce_internal_productattributeslookup_wp_tax_query(var_tax_query.clone())
	if rt.is_true(var_use_lookup_table) {
		mut var_query := this.get_product_counts_query_using_lookup_table(var_tax_query.clone(),
			var_meta_query.clone(), var_taxonomy.clone(), var_term_ids.clone())
	} else {
		var_query = this.get_product_counts_query_not_using_lookup_table(var_tax_query.clone(),
			var_meta_query.clone(), var_term_ids.clone())
	}
	var_query = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_get_filtered_term_product_counts_query'),
		var_query.clone(),
	])
	mut var_query_sql := rt.call_function('implode', [rt.new_string(' '),
		var_query.clone()])
	mut var_query_hash := rt.new_string(md5.hexhash(var_query_sql.clone().to_string()))
	...
	if !(var_cached_counts.array_isset(var_query_hash)) {
		mut var_results := rt.call_method(var_wpdb, 'get_results', [
			var_query_sql.clone(), rt.get_constant('ARRAY_A')])
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_Filterer) get_product_counts_query_using_lookup_table(var_tax_query rt.PhpVal, var_meta_query rt.PhpVal, var_taxonomy rt.PhpVal, var_term_ids rt.PhpVal) rt.PhpVal {
	...
	mut var_meta_query_sql := rt.call_method(var_meta_query_mutated, 'get_sql', [
		rt.new_string('post'),
		this.lookup_table_name,
		rt.new_string('product_or_parent_id'),
	])
	mut var_tax_query_sql := rt.call_method(var_tax_query_mutated, 'get_sql', [
		this.lookup_table_name,
		rt.new_string('product_or_parent_id'),
	])
	mut var_hide_out_of_stock := rt.call_function('apply_filters', [
	...
	mut var_in_stock_clause := rt.new_string((if rt.is_true(var_hide_out_of_stock) {
		' AND in_stock = 1'
	} else {
		''
	}).str())
	mut var_query := rt.new_array()
	var_query.array_set('select',
	...
	var_query.array_set('join', rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('\n\t\t\t'),
		var_tax_query_sql.array_get(rt.new_string('join'))), rt.new_string(' ')),
		var_meta_query_sql.array_get(rt.new_string('join'))), rt.new_string('\n\t\t\tINNER JOIN ')), rt.get_property(var_wpdb,
		'posts')), rt.new_string(' ON ')), rt.get_property(var_wpdb, 'posts')),
		rt.new_string('.ID = ')), this.lookup_table_name), rt.new_string('.product_or_parent_id')))
	mut var_encoded_taxonomy := rt.call_function('sanitize_title', [
		var_taxonomy.clone()])
	mut var_term_ids_sql := rt.new_string(this.get_term_ids_sql(var_term_ids.clone()))
	var_query.array_set('where', rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('\n\t\t\tWHERE '), rt.get_property(var_wpdb,
		'posts')), rt.new_string(".post_type IN ( 'product' )\n\t\t\tAND ")), rt.get_property(var_wpdb,
		'posts')), rt.new_string(".post_status = 'publish'\n\t\t\t")),
		var_tax_query_sql.array_get(rt.new_string('where'))), rt.new_string(' ')),
		var_meta_query_sql.array_get(rt.new_string('where'))), rt.new_string('\n\t\t\tAND ')),
		this.lookup_table_name), rt.new_string(".taxonomy='")), var_encoded_taxonomy),
		rt.new_string("'\n\t\t\tAND ")), this.lookup_table_name), rt.new_string('.term_id IN ')),
		var_term_ids_sql), rt.new_string('\n\t\t\t')), var_in_stock_clause))
	if !(!rt.is_true(var_term_ids)) {
		mut iife_temp_2 := Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_WC_Query{}
		mut iife_result_2 := iife_temp_2.get_layered_nav_chosen_attributes()
		mut var_attributes_to_filter_by := iife_result_2
		if !(!rt.is_true(var_attributes_to_filter_by)) {
			mut var_and_term_ids := rt.new_array()
			mut iter_3 := var_attributes_to_filter_by.iterator()
			for {
				item_3 := iter_3.next() or { break }
				mut var_data := item_3.val
				mut var_taxonomy_shadow := item_3.key
				if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('and'),
					var_data.array_get(rt.new_string('query_type'))))))
				{
					continue
				}
				...
				mut var_term_ids_to_filter_by := rt.call_function('array_values', [
					rt.call_function('array_intersect_key', [
						var_term_ids_by_slug.clone(),
						rt.call_function('array_flip', [
							var_data.array_get(rt.new_string('terms')),
						])]),
				])
				var_and_term_ids = rt.call_function('array_merge', [
					var_and_term_ids.clone(), var_term_ids_to_filter_by.clone()])
			}
			if !(!rt.is_true(var_and_term_ids)) {
				mut var_terms_count := rt.new_int(var_and_term_ids.clone().array_count())
				mut var_term_ids_list := rt.new_string('(' +
					(rt.call_function('join', [rt.new_string(','), var_and_term_ids.clone()])).str() +
					')')
				var_query.array_get(rt.new_string('where')) = rt.concat(var_query.array_get(rt.new_string('where')), rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('\n\t\t\t\t\t\tAND product_or_parent_id IN ( SELECT product_or_parent_id FROM (\n\t\t\t\t\t\t\tSELECT product_or_parent_id\n\t\t\t\t\t\t\tFROM '),
					this.lookup_table_name),
					rt.new_string(' lt\n\t\t\t\t\t\t\tWHERE is_variation_attribute=0\n\t\t\t\t\t\t\t')),
					var_in_stock_clause), rt.new_string('\n\t\t\t\t\t\t\tAND term_id in ')),
					var_term_ids_list),
		...
		var_query.array_get(rt.new_string('where')) = rt.concat(var_query.array_get(rt.new_string('where')), rt.concat(rt.concat(rt.new_string(' AND '),
			this.lookup_table_name), rt.new_string('.in_stock=1')))
	}
	mut iife_temp_3 := Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_WC_Query{}
	mut iife_result_3 := iife_temp_3.get_main_search_query_sql()
	mut var_search_query_sql := iife_result_3
	if rt.is_true(var_search_query_sql) {
		var_query.array_get(rt.new_string('where')) = rt.concat(var_query.array_get(rt.new_string('where')), rt.new_string(
			' AND ' + var_search_query_sql.str()))
	}
	var_query.array_set('group_by', 'GROUP BY terms.term_id')
	var_query.array_set('group_by', rt.concat(rt.concat(rt.new_string('GROUP BY '),
		this.lookup_table_name), rt.new_string('.term_id')))
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_Filterer) get_product_counts_query_not_using_lookup_table(var_tax_query rt.PhpVal, var_meta_query rt.PhpVal, var_term_ids rt.PhpVal) rt.PhpVal {
	...
	var_query.array_set('where', rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('\n\t\t\tWHERE '), rt.get_property(var_wpdb,
		...
		'posts')), rt.new_string(".post_status = 'publish'\n\t\t\t")),
		var_tax_query_sql.array_get(rt.new_string('where'))), rt.new_string(' ')),
		var_meta_query_sql.array_get(rt.new_string('where'))),
		rt.new_string('\n\t\t\tAND terms.term_id IN ')), var_term_ids_sql))
	mut iife_temp_4 := Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_WC_Query{}
	mut iife_result_4 := iife_temp_4.get_main_search_query_sql()
	mut var_search_query_sql := iife_result_4
	if rt.is_true(var_search_query_sql) {
		var_query.array_get(rt.new_string('where')) = rt.concat(var_query.array_get(rt.new_string('where')), rt.new_string(
			' AND ' + var_search_query_sql.str()))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_Filterer) get_term_ids_sql(var_term_ids rt.PhpVal) string {
	return '(' +
		(rt.call_function('implode', [rt.new_string(','), rt.call_function('array_map', [rt.new_string('absint'), var_term_ids.clone()])])).str() +
		')'
}
...
fn create_automattic_woocommerce_internal_productattributeslookup_filterer(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_Filterer {
...
fn create_automattic_woocommerce_internal_productattributeslookup_wc_query(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_WC_Query {
...
fn create_automattic_woocommerce_internal_productattributeslookup_wp_meta_query(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_WP_Meta_Query {
...
fn create_automattic_woocommerce_internal_productattributeslookup_wp_tax_query(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_WP_Tax_Query {
...
fn (mut this Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_Filterer) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'init' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_LookupDataStore](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
		...
		'filter_by_attribute_post_clauses' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_ProductAttributesLookup_WP_Query](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
			...
			return this.filter_by_attribute_post_clauses(mut dispatch_arg_0, mut dispatch_arg_1, mut
				dispatch_arg_2)
		}
		...
		'get_product_counts_query_using_lookup_table' {
			...
			return this.get_product_counts_query_using_lookup_table(dispatch_arg_0, dispatch_arg_1,
		...
		'get_product_counts_query_not_using_lookup_table' {
			...
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			dispatch_arg_2 := if args.len > 2 { args[2] } else { rt.new_null() }
			return this.get_product_counts_query_not_using_lookup_table(dispatch_arg_0,
