...
fn (mut this Class_Automattic_WooCommerce_Internal_Caches_ProductCache) set(var_product rt.PhpVal, var_id rt.PhpVal, expiration i64) bool {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_id_mutated)))) {
	...
	mut var_result := this.Class_Automattic_WooCommerce_Caching_ObjectCache.set(var_product_mutated.clone(), var_id_mutated.clone(), rt.new_int(expiration))
	rt.call_method(var_product_mutated, 'set_clone_mode', [var_original_mode.clone()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Caches_ProductCache) remove(var_id rt.PhpVal) bool {
	mut var_id_mutated := var_id
	return (this.Class_Automattic_WooCommerce_Caching_ObjectCache.remove(rt.new_int((var_id_mutated).to_i64()))).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Caches_ProductCache) get(var_id rt.PhpVal, expiration i64, mut var_get_from_datastore_callback Class_Automattic_WooCommerce_Internal_Caches_?callable) rt.PhpVal {
	mut var_id_mutated := var_id
	var_id_mutated = rt.new_int((var_id_mutated).to_i64())
	mut var_product := this.Class_Automattic_WooCommerce_Caching_ObjectCache.get(var_id_mutated.clone(), rt.new_int(expiration), rt.new_object('Automattic_WooCommerce_Internal_Caches_?callable', []string{}, var_get_from_datastore_callback))
...
fn create_automattic_woocommerce_internal_caches_productcache(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Caches_ProductCache {
...
fn create_automattic_woocommerce_caching_objectcache(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Caching_ObjectCache {
