...
fn (mut this Class_Automattic_WooCommerce_Internal_Caches_VersionStringGenerator) can_use() bool {
	if !(this.can_use.is_null()) {
		return (this.can_use).to_bool()
	}
	this.can_use = if !(rt.call_method(this.legacy_proxy, 'call_function', [
		rt.new_string('wp_using_ext_object_cache'),
	])).is_null() { rt.call_method(this.legacy_proxy, 'call_function', [
			rt.new_string('wp_using_ext_object_cache'),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Caches_VersionStringGenerator) get_version(id string, generate bool) string {
	...
	mut var_version := rt.call_function('wp_cache_get', [var_cache_key.clone(),
		Class_Automattic_WooCommerce_Internal_Caches_Automattic_WooCommerce_Internal_Caches_VersionStringGenerator.cache_group()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Caches_VersionStringGenerator) store_version(id string, version string) bool {
	...
	mut var_cache_key := rt.new_string(this.get_cache_key(id))
	mut var_ttl := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_version_string_generator_ttl'),
		rt.get_constant('DAY_IN_SECONDS'),
		rt.new_string(id),
	])
	var_ttl = rt.call_function('max', [rt.new_int(0), rt.new_int(var_ttl.to_i64())])
	mut var_result := rt.call_function('wp_cache_set', [var_cache_key.clone(),
		rt.new_string(version_mutated).clone(),
		Class_Automattic_WooCommerce_Internal_Caches_Automattic_WooCommerce_Internal_Caches_VersionStringGenerator.cache_group(),
		var_ttl.clone()])
	if rt.is_true(rt.new_bool(var_result.clone().is_bool())) {
		return var_result.to_bool()
	}
	mut var_stored_value := rt.call_function('wp_cache_get', [
		var_cache_key.clone(),
		Class_Automattic_WooCommerce_Internal_Caches_Automattic_WooCommerce_Internal_Caches_VersionStringGenerator.cache_group()])
	if rt.is_true(rt.identical(var_stored_value, rt.new_string(version_mutated))) {
		return true
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_stored_value)))) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Caches_VersionStringGenerator) delete_version(id string) bool {
	this.validate_input(id)
	mut var_cache_key := rt.new_string(this.get_cache_key(id))
	mut var_result := rt.call_function('wp_cache_delete', [var_cache_key.clone(),
		Class_Automattic_WooCommerce_Internal_Caches_Automattic_WooCommerce_Internal_Caches_VersionStringGenerator.cache_group()])
	return !(var_result.clone().is_bool()) || rt.is_true(var_result)
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Caches_VersionStringGenerator) validate_input(id string) {
	if rt.is_true(rt.identical(rt.new_string(''), rt.new_string(id))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_Caches_InvalidArgumentException',
			[]string{},
			create_automattic_woocommerce_internal_caches_invalidargumentexception(rt.new_string('ID cannot be empty.'))))
	}
}
...
fn create_automattic_woocommerce_internal_caches_versionstringgenerator(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Caches_VersionStringGenerator {
...
fn create_automattic_woocommerce_internal_caches_invalidargumentexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Caches_InvalidArgumentException {
