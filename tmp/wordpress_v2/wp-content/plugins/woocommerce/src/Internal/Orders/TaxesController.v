...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_TaxesController) calc_line_taxes(mut var_post_variables Class_Automattic_WooCommerce_Internal_Orders_array) rt.PhpVal {
	mut var_order_id := rt.call_function('absint', [var_post_variables.array_get(rt.new_string('order_id'))])
	mut var_calculate_tax_args := rt.create_array([rt.ArrayItem{ key: 'country', val: if var_post_variables.array_isset(rt.new_string('country')) { rt.call_function('wc_strtoupper', [rt.call_function('wc_clean', [rt.call_function('wp_unslash', [var_post_variables.array_get(rt.new_string('country'))])])]) } else { rt.new_string('') } }, rt.ArrayItem{ key: 'state', val: if var_post_variables.array_isset(rt.new_string('state')) { rt.call_function('wc_strtoupper', [rt.call_function('wc_clean', [rt.call_function('wp_unslash', [var_post_variables.array_get(rt.new_string('state'))])])]) } else { rt.new_string('') } }, rt.ArrayItem{ key: 'postcode', val: if var_post_variables.array_isset(rt.new_string('postcode')) { rt.call_function('wc_strtoupper', [rt.call_function('wc_clean', [rt.call_function('wp_unslash', [var_post_variables.array_get(rt.new_string('postcode'))])])]) } else { rt.new_string('') } }, rt.ArrayItem{ key: 'city', val: if var_post_variables.array_isset(rt.new_string('city')) { rt.call_function('wc_strtoupper', [rt.call_function('wc_clean', [rt.call_function('wp_unslash', [var_post_variables.array_get(rt.new_string('city'))])])]) } else { rt.new_string('') } }])
	mut var_items := rt.new_array()
	rt.call_function('parse_str', [rt.call_function('wp_unslash', [var_post_variables.array_get(rt.new_string('items'))]), var_items.clone()])
...
fn create_automattic_woocommerce_internal_orders_taxescontroller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Orders_TaxesController {
