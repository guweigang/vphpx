...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderStatusRestController) register_routes() {
	rt.call_function('register_rest_route', [
		rt.new_string(this.get_rest_api_namespace()),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Orders_OrderStatusRestController) get_items(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	for {
		...
		mut var_slug := rt.call_function('str_replace', [rt.new_string('wc-'),
			rt.new_string(''), var_status_slug.clone()])
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_formatted_statuses)))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_not_found'), rt.call_function('__', [
			rt.new_string('Order statuses not found'),
			rt.new_string('woocommerce'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }])))
	}
	return rt.call_function('rest_ensure_response', [var_formatted_statuses.clone()])
}
...
fn create_automattic_woocommerce_internal_orders_orderstatusrestcontroller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Orders_OrderStatusRestController {
