...
fn Class_Automattic_WooCommerce_Internal_Orders_PointOfSaleOrderUtil.is_order_paid_at_pos(mut var_order Class_WC_Abstract_Order) bool {
	...
	if rt.is_true(rt.identical(rt.new_string('mobile_pos'),
		var_order.get_meta(rt.new_string('_wcpay_ipp_channel'))))
	{
		return true
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''),
		var_order.get_meta(rt.new_string('_cash_change_amount'))))))
	{
		return true
	}
	return false
