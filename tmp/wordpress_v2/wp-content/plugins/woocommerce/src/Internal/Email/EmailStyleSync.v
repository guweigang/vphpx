...
fn (mut this Class_Automattic_WooCommerce_Internal_Email_EmailStyleSync) register() {
	rt.call_function('add_action', [rt.new_string('after_switch_theme'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Email_EmailStyleSync', [
				'RegisterHooksInterface',
			], &this) },
			rt.ArrayItem{ key: none, val: 'sync_email_styles_with_theme' },
		])])
	...
	rt.call_function('add_action', [rt.new_string('wp_theme_json_data_updated'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Email_EmailStyleSync', [
				'RegisterHooksInterface',
			], &this) },
	...
	rt.call_function('add_action', [rt.new_string('rest_after_insert_global_styles'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Email_EmailStyleSync', [
				'RegisterHooksInterface',
			], &this) },
	...
	rt.call_function('add_action', [rt.new_string('update_option_wp_global_styles'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Email_EmailStyleSync', [
				'RegisterHooksInterface',
			], &this) },
	...
	rt.call_function('add_action', [rt.new_string('wp_ajax_wp_save_styles'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Email_EmailStyleSync', [
				'RegisterHooksInterface',
			], &this) },
	...
	rt.call_function('add_action', [
		rt.new_string('update_option_' +(Class_Automattic_WooCommerce_Internal_Email_Automattic_WooCommerce_Internal_Email_EmailStyleSync.auto_sync_option()).str()),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Internal_Email_EmailStyleSync', [
				'RegisterHooksInterface',
			], &this) },
			rt.ArrayItem{ key: none, val: 'maybe_sync_on_option_update' },
		]),
		rt.new_int(10),
		rt.new_int(3),
	])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Email_EmailStyleSync) maybe_sync_on_option_update(var_old_value rt.PhpVal, var_new_value rt.PhpVal, var_option rt.PhpVal) {
	if rt.is_true(rt.identical(rt.new_string('yes'), var_new_value))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('yes'), var_old_value)))) {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Email_EmailStyleSync) is_auto_sync_enabled() rt.PhpVal {
	return rt.identical(rt.new_string('yes'), rt.call_function('get_option', [
		Class_Automattic_WooCommerce_Internal_Email_Automattic_WooCommerce_Internal_Email_EmailStyleSync.auto_sync_option(),
		rt.new_string('no'),
	]))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Email_EmailStyleSync) set_auto_sync(enabled bool) rt.PhpVal {
	return rt.call_function('update_option', [
		Class_Automattic_WooCommerce_Internal_Email_Automattic_WooCommerce_Internal_Email_EmailStyleSync.auto_sync_option(),
		rt.new_string((if var_enabled { 'yes' } else { 'no' }).str()),
	])
}

fn (mut this Class_Automattic_WooCommerce_Internal_Email_EmailStyleSync) sync_email_styles_with_theme() {
	if this.is_syncing || rt.is_true(rt.new_bool(!(rt.is_true(this.is_auto_sync_enabled()))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_theme_has_theme_json', []rt.PhpVal{}))))) {
		return
	}
	this.is_syncing = true
	this.update_email_colors()
...
fn (mut this Class_Automattic_WooCommerce_Internal_Email_EmailStyleSync) update_email_colors() {
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Email_EmailColors{}
	mut iife_result_0 := iife_temp_0.get_default_colors()
	mut var_colors := iife_result_0
	if !rt.is_true(var_colors) {
		return
	}
	if !(!rt.is_true(var_colors.array_get(rt.new_string('base')))) {
		rt.call_function('update_option', [rt.new_string('woocommerce_email_base_color'),
			var_colors.array_get(rt.new_string('base'))])
	}
	if !(!rt.is_true(var_colors.array_get(rt.new_string('bg')))) {
		rt.call_function('update_option', [
			rt.new_string('woocommerce_email_background_color'),
			var_colors.array_get(rt.new_string('bg')),
		])
	}
	if !(!rt.is_true(var_colors.array_get(rt.new_string('body_bg')))) {
		rt.call_function('update_option', [
			rt.new_string('woocommerce_email_body_background_color'),
			var_colors.array_get(rt.new_string('body_bg')),
		])
	}
	if !(!rt.is_true(var_colors.array_get(rt.new_string('body_text')))) {
		rt.call_function('update_option', [rt.new_string('woocommerce_email_text_color'),
			var_colors.array_get(rt.new_string('body_text'))])
	}
	if !(!rt.is_true(var_colors.array_get(rt.new_string('footer_text')))) {
		rt.call_function('update_option', [
			rt.new_string('woocommerce_email_footer_text_color'),
			var_colors.array_get(rt.new_string('footer_text')),
		])
	}
}
...
fn create_automattic_woocommerce_internal_email_emailstylesync(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Email_EmailStyleSync {
