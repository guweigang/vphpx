...
pub fn Class_Automattic_WooCommerce_Internal_Utilities_HtmlSanitizer.low_html_balanced_tags_no_links() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'pre_processors', val: rt.create_array([
			rt.ArrayItem{ key: none, val: 'stripslashes' },
			rt.ArrayItem{ key: none, val: 'force_balance_tags' },
		]) },
		rt.ArrayItem{ key: 'wp_kses_rules', val: rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_HtmlSanitizer) styled_post_content(html string) string {
	...
	mut var_rules := rt.call_function('wp_kses_allowed_html', [
		rt.new_string('post')])
	var_rules.array_set('style', true)
	return (rt.call_function('wp_kses', [rt.new_string(html_mutated).clone(),
		var_rules.clone()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_HtmlSanitizer) sanitize(html string, mut var_sanitizer_rules Class_Automattic_WooCommerce_Internal_Utilities_array) string {
	mut html_mutated := html
	if var_sanitizer_rules.array_isset(rt.new_string('pre_processors'))
		&& var_sanitizer_rules.array_get(rt.new_string('pre_processors')).is_array() {
		html_mutated = this.apply_string_callbacks(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Utilities_array](var_sanitizer_rules.array_get(rt.new_string('pre_processors'))),
			html_mutated)
	}
	mut var_kses_rules := if var_sanitizer_rules.array_isset(rt.new_string('wp_kses_rules'))
		&& var_sanitizer_rules.array_get(rt.new_string('wp_kses_rules')).is_array() {
		var_sanitizer_rules.array_get(rt.new_string('wp_kses_rules'))
	} else {
		rt.new_array()
	}
	return (rt.call_function('wp_kses', [rt.new_string(html_mutated).clone(),
		var_kses_rules.clone()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_HtmlSanitizer) apply_string_callbacks(mut var_callbacks Class_Automattic_WooCommerce_Internal_Utilities_array, string string) string {
	...
	for {
		...
		if !(rt.call_function('is_callable', [var_callback.clone()])) {
			rt.call_function('_doing_it_wrong', [rt.new_string(@STRUCT + '::apply'),
				rt.call_function('esc_html__', [
					rt.new_string('String processors must be an array of valid callbacks.'),
		...
		string_mutated =
			(rt.call_callable(var_callback, [rt.new_string(string_mutated).clone()])).str()
	}
	return string_mutated
}

fn create_automattic_woocommerce_internal_utilities_htmlsanitizer(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Utilities_HtmlSanitizer {
