...
fn Class_Automattic_WooCommerce_Internal_Utilities_Types.ensure_instance_of(var_thing rt.PhpVal, desired_type string, mut var_on_failure Class_Automattic_WooCommerce_Internal_Utilities_?callable) rt.PhpVal {
	...
	if rt.is_true(var_logger) {
		rt.call_method(var_logger, 'error', [var_summary.clone(), rt.create_array([rt.ArrayItem{ key: 'source', val: 'wc-type-check-utility' }, rt.ArrayItem{ key: 'backtrace', val: true }])])
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_on_failure)))) {
		return rt.call_callable(var_on_failure, [var_thing.clone(), rt.new_string(desired_type)])
	}
	rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.call_function('esc_html', [var_summary.clone()]))))
	return rt.new_null()
}
