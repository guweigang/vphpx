...
fn Class_Automattic_WooCommerce_Internal_Utilities_Users.is_site_administrator(user_id i64) bool {
	...
	mut var_user := if 0 == user_id_mutated { rt.call_function('wp_get_current_user', []rt.PhpVal{}) } else { rt.call_function('get_user_by', [rt.new_string('id'), rt.new_int(user_id_mutated).clone()]) }
...
fn Class_Automattic_WooCommerce_Internal_Utilities_Users.get_user_in_current_site(var_user_id rt.PhpVal, mut var_requesting_user_id Class_Automattic_WooCommerce_Internal_Utilities_?int) rt.PhpVal {
	...
	var_user_id_mutated = rt.new_int(if var_user_id_mutated.clone().is_long() || var_user_id_mutated.clone().is_double() { rt.new_int((var_user_id_mutated).to_i64()) } else { 0 })
	...
	mut var_user := rt.call_function('get_userdata', [var_user_id_mutated.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_user, 'WP_User')))))) || rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_user, 'exists', []rt.PhpVal{}))))) {
		return mut var_error
	}
	if rt.is_true(rt.call_method(var_legacy_proxy, 'call_function', [rt.new_string('is_multisite')])) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_legacy_proxy, 'call_function', [rt.new_string('is_user_member_of_blog'), rt.get_property(var_user, 'ID')]))))) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_legacy_proxy, 'call_function', [rt.new_string('user_can'), var_requesting_user_id_mutated, rt.new_string('manage_network_users')]))))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_site_option', [rt.new_string('woocommerce_network_wide_customers'), rt.new_string('no')]), rt.new_string('yes'))))) {
		return mut var_error
	}
...
fn Class_Automattic_WooCommerce_Internal_Utilities_Users.should_user_verify_order_email(var_order_id rt.PhpVal, var_supplied_email rt.PhpVal, context string) bool {
	...
	if rt.is_true(var_customer_id) && rt.is_true(rt.identical(rt.call_function('get_current_user_id', []rt.PhpVal{}), var_customer_id)) {
		return false
	}
	mut var_verification_grace_period := rt.new_int((rt.call_function('apply_filters', [rt.new_string('woocommerce_order_email_verification_grace_period'), rt.mul(rt.new_int(10), rt.get_constant('MINUTE_IN_SECONDS')), var_order.clone(), rt.new_string(context)])).to_i64())
	mut var_date_created := rt.call_method(var_order, 'get_date_created', []rt.PhpVal{})
	if rt.is_true(rt.call_function('is_a', [var_date_created.clone(), Class_Automattic_WooCommerce_Internal_Utilities_WC_DateTime.class(), rt.new_bool(true)])) && rt.is_true(rt.less_equal(rt.sub(rt.call_function('time', []rt.PhpVal{}), rt.call_method(var_date_created, 'getTimestamp', []rt.PhpVal{})), var_verification_grace_period)) {
		return false
	}
	mut var_session := rt.get_property(rt.call_function('wc', []rt.PhpVal{}), 'session')
	mut var_session_email := rt.new_string('')
	if rt.is_true(rt.call_function('is_a', [var_session.clone(), Class_Automattic_WooCommerce_Internal_Utilities_WC_Session.class()])) {
	mut var_customer := rt.call_method(var_session, 'get', [rt.new_string('customer')])
	var_session_email = if var_customer.clone().is_array() && var_customer.array_isset(rt.new_string('email')) { var_customer.array_get(rt.new_string('email')) } else { rt.new_string('') }
	}
	mut var_can_view_orders := rt.call_function('current_user_can', [rt.new_string('read_private_shop_orders')])
	mut var_session_email_match := rt.new_bool(!(!rt.is_true(var_session_email)) && rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strcasecmp', [var_session_email.clone(), var_billing_email.clone()]))))
	mut var_supplied_email_match := rt.new_bool(!(!rt.is_true(var_supplied_email)) && rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strcasecmp', [var_supplied_email.clone(), var_billing_email.clone()]))))
	mut var_email_verification_required := rt.new_bool(rt.is_true(rt.new_bool(!(rt.is_true(var_session_email_match)))) && rt.is_true(rt.new_bool(!(rt.is_true(var_supplied_email_match)))) && rt.is_true(rt.new_bool(!(rt.is_true(var_can_view_orders)))))
	return (rt.call_function('apply_filters', [rt.new_string('woocommerce_order_email_verification_required'), var_email_verification_required.clone(), var_order.clone(), rt.new_string(context)])).to_bool()
}
...
fn Class_Automattic_WooCommerce_Internal_Utilities_Users.get_site_user_meta(user_id i64, key string, single bool) rt.PhpVal {
	...
	return rt.call_function('get_user_meta', [rt.new_int(user_id_mutated).clone(), var_site_specific_key.clone(), rt.new_bool(single)])
}
...
fn Class_Automattic_WooCommerce_Internal_Utilities_Users.update_site_user_meta(user_id i64, meta_key string, var_meta_value rt.PhpVal, prev_value string) rt.PhpVal {
	...
	return rt.call_function('update_user_meta', [rt.new_int(user_id_mutated).clone(), var_site_specific_key.clone(), var_meta_value.clone(), rt.new_string(prev_value)])
}
...
fn Class_Automattic_WooCommerce_Internal_Utilities_Users.delete_site_user_meta(var_user_id rt.PhpVal, var_meta_key rt.PhpVal, meta_value string) rt.PhpVal {
	...
	return rt.call_function('delete_user_meta', [var_user_id_mutated.clone(), var_site_specific_key.clone(), rt.new_string(meta_value)])
}
