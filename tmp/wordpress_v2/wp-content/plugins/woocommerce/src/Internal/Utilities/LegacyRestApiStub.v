...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_LegacyRestApiStub) init(mut var_rest_api_util Class_Automattic_WooCommerce_Utilities_RestApiUtil) {
	this.rest_api_util = var_rest_api_util
}
...
fn Class_Automattic_WooCommerce_Internal_Utilities_LegacyRestApiStub.parse_legacy_rest_api_request() {
	...
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('wc-api-version')))) {
		rt.get_property(var_wp, 'query_vars').array_set('wc-api-version',
			rt.get_superglobal('_GET').array_get(rt.new_string('wc-api-version')))
	}
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('wc-api-route')))) {
		rt.get_property(var_wp, 'query_vars').array_set('wc-api-route',
			rt.get_superglobal('_GET').array_get(rt.new_string('wc-api-route')))
	}
	if !(!rt.is_true(rt.get_property(var_wp, 'query_vars').array_get(rt.new_string('wc-api-version'))))
		&& !(!rt.is_true(rt.get_property(var_wp, 'query_vars').array_get(rt.new_string('wc-api-route')))) {
		rt.call_function('header', [
			rt.call_function('sprintf', [rt.new_string('Content-Type: %s; charset=%s'),
				rt.new_string((if rt.get_superglobal('_GET').array_isset(rt.new_string('_jsonp')) {
...
fn Class_Automattic_WooCommerce_Internal_Utilities_LegacyRestApiStub.maybe_process_wc_api_query_var() {
	mut var_wp := rt.new_null()
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('wc-api')))) {
		rt.get_property(var_wp, 'query_vars').array_set('wc-api', rt.call_function('sanitize_key', [
			rt.call_function('wp_unslash',
				[rt.get_superglobal('_GET').array_get(rt.new_string('wc-api'))]),
		]))
	}
	if !(!rt.is_true(rt.get_property(var_wp, 'query_vars').array_get(rt.new_string('wc-api')))) {
		rt.call_function('ob_start', []rt.PhpVal{})
		rt.call_function('wc_nocache_headers', []rt.PhpVal{})
		mut var_api_request := rt.new_string(rt.call_function('wc_clean', [
			rt.get_property(var_wp, 'query_vars').array_get(rt.new_string('wc-api')),
		]).to_string().to_lower())
		...
		rt.call_function('do_action', [rt.new_string('woocommerce_api_request'),
			var_api_request.clone()])
		rt.call_function('status_header', [
			rt.new_int(if rt.is_true(rt.call_function('has_action', [
				rt.new_string('woocommerce_api_' + var_api_request.str()),
			]))
			{ 200 } else { 400 }),
		])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_LegacyRestApiStub) get_endpoint_data(var_endpoint rt.PhpVal, var_params rt.PhpVal) rt.PhpVal {
	...
	return rt.call_method(this.rest_api_util, 'get_endpoint_data', [
		var_endpoint.clone(), var_params.clone()])
}

fn create_automattic_woocommerce_internal_utilities_legacyrestapistub(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Utilities_LegacyRestApiStub {
