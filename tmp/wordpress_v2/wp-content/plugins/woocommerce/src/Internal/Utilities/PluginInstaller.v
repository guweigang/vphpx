...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_PluginInstaller) install_plugin(plugin_url string, mut var_metadata Class_Automattic_WooCommerce_Internal_Utilities_array) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('in_array', [rt.new_string(plugin_url), var_plugins_being_installed.clone(), rt.new_bool(true)])) {
		return rt.create_array([rt.ArrayItem{ key: 'already_installing', val: true }])
	}
	var_plugins_being_installed.array_push(plugin_url)
	rt.call_function('update_site_option', [rt.new_string('woocommerce_autoinstalling_plugins'), var_plugins_being_installed.clone()])
...
finally_label_1:
	var_plugins_being_installed = rt.call_function('array_diff', [var_plugins_being_installed.clone(), rt.create_array([rt.ArrayItem{ key: none, val: plugin_url }])])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_PluginInstaller) install_plugin_core(plugin_url string, mut var_metadata Class_Automattic_WooCommerce_Internal_Utilities_array) rt.PhpVal {
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Utilities_StringUtil{}
	mut iife_result_0 := iife_temp_0.starts_with(rt.new_string(plugin_url), rt.new_string('https://downloads.wordpress.org/'), rt.new_bool(false))
	...
	mut var_installed_by := if !(var_metadata.array_get(rt.new_string('installed_by'))).is_null() { var_metadata.array_get(rt.new_string('installed_by')) } else { rt.new_string('WooCommerce') }
	if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strcasecmp', [rt.new_string('WooCommerce'), var_installed_by.clone()]))) {
		mut iife_temp_1 := Class_Automattic_WooCommerce_Internal_Utilities_StringUtil{}
		mut iife_result_1 := iife_temp_1.normalize_local_path_slashes(if !(rt.call_function('debug_backtrace', []rt.PhpVal{}).array_get(rt.new_int(1)).array_get(rt.new_string('file'))).is_null() { rt.call_function('debug_backtrace', []rt.PhpVal{}).array_get(rt.new_int(1)).array_get(rt.new_string('file')) } else { rt.new_string('') })
		mut var_calling_file := iife_result_1
		mut iife_temp_2 := Class_Automattic_WooCommerce_Internal_Utilities_StringUtil{}
		mut iife_result_2 := iife_temp_2.normalize_local_path_slashes(rt.new_string((rt.get_constant('WC_ABSPATH')).str() + 'includes/'))
		mut iife_temp_3 := Class_Automattic_WooCommerce_Internal_Utilities_StringUtil{}
		mut iife_result_3 := iife_temp_3.starts_with(var_calling_file.clone(), iife_result_2)
		mut iife_temp_4 := Class_Automattic_WooCommerce_Internal_Utilities_StringUtil{}
		mut iife_result_4 := iife_temp_4.normalize_local_path_slashes(rt.new_string((rt.get_constant('WC_ABSPATH')).str() + 'src/'))
		mut iife_temp_5 := Class_Automattic_WooCommerce_Internal_Utilities_StringUtil{}
		mut iife_result_5 := iife_temp_5.starts_with(var_calling_file.clone(), iife_result_4)
		if rt.is_true(rt.new_bool(!(rt.is_true(iife_result_3)))) && rt.is_true(rt.new_bool(!(rt.is_true(iife_result_5)))) {
	...
	mut var_upgrader := create_automattic_woocommerce_internal_utilities_plugin_upgrader(var_skin)
	...
	if rt.is_true(var_install_ok) {
		...
		mut var_plugin_version := rt.call_function('get_plugins', []rt.PhpVal{}).array_get(var_plugin_name).array_get(rt.new_string('Version'))
		var_result.array_set('plugin_name', var_plugin_name.clone())
		...
		if !(!rt.is_true(var_metadata)) {
			var_plugin_data.array_set('metadata', var_metadata)
		}
		...
		var_auto_installed_plugins.array_set(var_plugin_name, var_plugin_data.clone())
		rt.call_function('update_site_option', [rt.new_string('woocommerce_autoinstalled_plugins'), var_auto_installed_plugins.clone()])
		...
		if !(var_auto_installed_plugins_history.array_isset(var_plugin_name)) {
			var_auto_installed_plugins_history.array_set(var_plugin_name, var_plugin_data.clone())
			rt.call_function('update_site_option', [rt.new_string('woocommerce_history_of_autoinstalled_plugins'), var_auto_installed_plugins_history.clone()])
		}
	closure_7_fn := fn [var_plugin_name, var_plugin_version, var_installed_by, var_plugin_url, var_plugin_data] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'info', [rt.new_string("Plugin ${var_plugin_name.to_string()} v${var_plugin_version.to_string()} installed by ${var_installed_by.to_string()}, source: ${var_plugin_url}"), var_log_context.clone()])
		return rt.new_null()
		}
	mut var_post_install := rt.new_closure(closure_7_fn)
	} else {
	mut var_messages := var_skin.get_upgrade_messages()
	closure_8_fn := fn [var_plugin_url, var_installed_by, var_messages] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'error', [rt.new_string("${var_installed_by.to_string()} failed to install plugin from source: ${var_plugin_url}"), var_log_context.clone()])
		return rt.new_null()
		}
	var_post_install = rt.new_closure(closure_8_fn)
	}
...
fn Class_Automattic_WooCommerce_Internal_Utilities_PluginInstaller.woocommerce_is_active_in_current_site() bool {
	...
	closure_9_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return rt.identical(rt.call_function('substr_compare', [var_plugin.clone(), rt.new_string('/woocommerce.php'), rt.new_int(-'/woocommerce.php'.len)]), rt.new_int(0))
		}
	return !(!rt.is_true(rt.call_function('array_filter', [var_active_valid_plugins.clone(), rt.new_closure(closure_9_fn)])))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_PluginInstaller) handle_plugin_list_rows(var_plugin_file rt.PhpVal, var_plugin_data rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(var_wp_list_table.clone().is_null())) {
		return
	}
	...
	if var_current_plugin_info.clone().is_null() || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_current_plugin_info.array_get(rt.new_string('version')), var_plugin_data_mutated.array_get(rt.new_string('Version')))))) {
		return
	}
	mut var_installed_by := if !(var_current_plugin_info.array_get(rt.new_string('metadata')).array_get(rt.new_string('installed_by'))).is_null() { var_current_plugin_info.array_get(rt.new_string('metadata')).array_get(rt.new_string('installed_by')) } else { rt.new_string('WooCommerce') }
	mut var_info_link := if !(var_current_plugin_info.array_get(rt.new_string('metadata')).array_get(rt.new_string('info_link'))).is_null() { var_current_plugin_info.array_get(rt.new_string('metadata')).array_get(rt.new_string('info_link')) } else { rt.new_null() }
	if rt.is_true(var_info_link) {
	mut var_message := rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Plugin installed by %1$s on %2$s. <a target="_blank" href="%3$s">More information</a>'), rt.new_string('woocommerce')]), var_installed_by.clone(), var_current_plugin_info.array_get(rt.new_string('date')), var_info_link.clone()])
	} else {
	var_message = rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Plugin installed by %1$s on %2$s.'), rt.new_string('woocommerce')]), var_installed_by.clone(), var_current_plugin_info.array_get(rt.new_string('date'))])
	}
	mut var_columns_count := rt.call_method(var_wp_list_table, 'get_column_count', []rt.PhpVal{})
	mut var_is_active := rt.call_function('is_plugin_active', [var_plugin_file.clone()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_PluginInstaller) handle_upgrader_process_complete(mut var_upgrader Class_Automattic_WooCommerce_Internal_Utilities_WP_Upgrader, mut var_hook_extra Class_Automattic_WooCommerce_Internal_Utilities_array) {
	...
	if this.installing_plugin || !(true) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('plugin'), if !(var_hook_extra.array_get(rt.new_string('type'))).is_null() { var_hook_extra.array_get(rt.new_string('type')) } else { rt.new_null() })))) {
		return
	}
	...
	if rt.is_true(if !(var_hook_extra.array_get(rt.new_string('bulk'))).is_null() { var_hook_extra.array_get(rt.new_string('bulk')) } else { rt.new_bool(false) }) {
	mut var_updated_plugin_names := if !(var_hook_extra.array_get(rt.new_string('plugins'))).is_null() { var_hook_extra.array_get(rt.new_string('plugins')) } else { rt.new_array() }
	} else {
	var_updated_plugin_names = rt.create_array([rt.ArrayItem{ key: none, val: var_upgrader_mutated.plugin_info() }])
	}
	mut var_auto_installed_plugin_names := rt.func_array_keys(var_auto_installed_plugins.clone())
	mut var_updated_auto_installed_plugin_names := rt.call_function('array_intersect', [var_auto_installed_plugin_names.clone(), var_updated_plugin_names.clone()])
	if !rt.is_true(var_updated_auto_installed_plugin_names) {
		return
	}
	mut var_new_auto_installed_plugins := rt.call_function('array_diff_key', [var_auto_installed_plugins.clone(), rt.call_function('array_flip', [var_updated_auto_installed_plugin_names.clone()])])
	if !rt.is_true(var_new_auto_installed_plugins) {
		...
		rt.call_function('update_site_option', [rt.new_string('woocommerce_autoinstalled_plugins'), var_new_auto_installed_plugins.clone()])
	}
}
...
fn create_automattic_woocommerce_internal_utilities_plugininstaller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Utilities_PluginInstaller {
...
fn create_automattic_woocommerce_internal_utilities_stringutil(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Utilities_StringUtil {
...
fn create_automattic_woocommerce_internal_utilities_invalidargumentexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Utilities_InvalidArgumentException {
...
fn create_automattic_woocommerce_internal_utilities_automatic_upgrader_skin(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Utilities_Automatic_Upgrader_Skin {
...
fn create_automattic_woocommerce_internal_utilities_plugin_upgrader(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Utilities_Plugin_Upgrader {
