...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_ProductUtil) delete_product_specific_transients(var_product_or_id rt.PhpVal) {
	mut var_parent_id := rt.new_int(0)
	...
	for {
		...
		rt.call_function('delete_transient', [
			rt.new_string(var_transient.str() + var_product_id.str()),
		])
		if rt.is_true(var_parent_id) {
			rt.call_function('delete_transient', [
				rt.new_string(var_transient.str() + var_parent_id.str()),
			])
		}
	}
}

fn create_automattic_woocommerce_internal_utilities_productutil(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Utilities_ProductUtil {
