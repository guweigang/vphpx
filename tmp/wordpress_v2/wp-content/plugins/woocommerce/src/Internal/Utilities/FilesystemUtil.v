...
fn Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.get_wp_filesystem() rt.PhpVal {
	...
	mut var_initialized := rt.new_bool(
		rt.is_true(rt.new_bool(rt.instance_of(var_wp_filesystem, 'WP_Filesystem_Base')))
		|| rt.is_true(Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.initialize_wp_filesystem()))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_initialized))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.is_usable_ftp_filesystem(mut rt.cast_object_ptr[Class_WP_Filesystem_Base](var_wp_filesystem)))))) {
		rt.throw_exception(rt.new_object('Exception', []string{},
			create_exception(rt.new_string('The WordPress filesystem could not be initialized.'))))
	}
	return var_wp_filesystem.clone()
}
...
fn Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.get_wp_filesystem_method_or_direct() string {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.constant_exists('FS_METHOD')))))
		&& rt.is_true(rt.identical(rt.new_bool(false), rt.call_method(var_proxy, 'call_function', [rt.new_string('get_option'), rt.new_string('ftp_credentials')])))
		&& rt.is_true(rt.new_bool(!(rt.is_true(Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.constant_exists('FTP_HOST'))))) {
		return 'direct'
	}
	mut var_method := rt.call_method(var_proxy, 'call_function', [
...
fn Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.constant_exists(name string) bool {
	...
	mut iife_result_0 := iife_temp_0.is_defined(rt.new_string(name))
	mut iife_temp_1 := Class_Automattic_Jetpack_Constants{}
	mut iife_result_1 := iife_temp_1.get_constant(rt.new_string(name))
	mut iife_temp_2 := Class_Automattic_Jetpack_Constants{}
	mut iife_result_2 := iife_temp_2.get_constant(rt.new_string(name))
	return rt.is_true(iife_result_0) && !(iife_result_1.is_null())
}
...
fn Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.mkdir_p_not_indexable(path string, allow_file_access bool) {
	mut var_wp_fs :=
		Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.get_wp_filesystem()
	...
	])))))
	{
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.call_function('esc_html', [
			rt.call_function('sprintf', [
				rt.new_string('Could not create directory: %s.'),
				rt.call_function('wp_basename', [rt.new_string(path)]),
			]),
		]))))
	}
	mut var_htaccess_content := rt.new_string((if var_allow_file_access {
		'Options -Indexes'
	} else {
		'deny from all'
	}).str())
	mut var_files := rt.create_array([
		rt.ArrayItem{ key: '.htaccess', val: var_htaccess_content },
		rt.ArrayItem{ key: 'index.html', val: '' },
	])
	...
	for {
		...
		rt.call_method(var_wp_fs, 'put_contents', [
			rt.new_string((rt.call_function('trailingslashit', [rt.new_string(path)])).str() +
				var_name.str()),
			var_content.clone(),
		])
	}
...
fn Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.initialize_wp_filesystem() bool {
	...
	mut var_method :=
		Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.get_wp_filesystem_method_or_direct()
	mut var_initialized := rt.new_bool(false)
	if rt.is_true(rt.identical(rt.new_string('direct'), var_method)) {
		var_initialized = rt.call_function('WP_Filesystem', []rt.PhpVal{})
	} else if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_method)))) {
		mut var_is_ftp := rt.call_function('in_array', [var_method.clone(),
			rt.create_array([rt.ArrayItem{ key: none, val: 'ftpext' },
				rt.ArrayItem{ key: none, val: 'ftpsockets' }]),
			rt.new_bool(true)])
		if rt.is_true(var_is_ftp)
			&& rt.is_true(rt.call_function('get_transient', [Class_Automattic_WooCommerce_Internal_Utilities_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.ftp_init_failure_transient()])) {
			return false
		}
		...
		var_initialized = rt.new_bool(rt.is_true(var_credentials)
			&& rt.is_true(rt.call_function('WP_Filesystem', [var_credentials.clone()])))
		if rt.is_true(var_is_ftp) {
			if rt.is_true(rt.new_bool(!(rt.is_true(var_initialized)))) {
				rt.call_function('set_transient', [
					Class_Automattic_WooCommerce_Internal_Utilities_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.ftp_init_failure_transient(),
					rt.new_bool(true),
					rt.mul(Class_Automattic_WooCommerce_Internal_Utilities_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.ftp_init_cooldown_minutes(),
						rt.get_constant('MINUTE_IN_SECONDS')),
				])
				rt.call_function('error_log', [
					rt.call_function('sprintf', [
						rt.new_string('WooCommerce: FTP filesystem connection failed. Please check your FTP credentials. Retrying in %d minutes.'),
						Class_Automattic_WooCommerce_Internal_Utilities_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.ftp_init_cooldown_minutes(),
					]),
				])
			} else {
	...
	return (if var_initialized.clone().is_null() {
		rt.new_bool(false)
	} else {
		var_initialized
	}).to_bool()
...
fn Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.is_usable_ftp_filesystem(mut var_wp_filesystem Class_WP_Filesystem_Base) bool {
	...
	mut var_has_broken_state := rt.new_bool(false)
	mut var_has_errors := rt.new_bool(false)
	if rt.is_true(rt.identical(rt.new_string('ftpext'), rt.get_property(var_wp_filesystem_mutated,
		'method')))
	{
		var_has_broken_state = rt.new_bool(!rt.is_true(rt.get_property(var_wp_filesystem_mutated,
			'link')))
		var_has_errors = rt.new_bool(
			rt.is_true(rt.call_function('is_wp_error', [rt.get_property(var_wp_filesystem_mutated, 'errors')]))
			&& rt.is_true(rt.call_method(rt.get_property(var_wp_filesystem_mutated, 'errors'), 'has_errors', []rt.PhpVal{})))
	}
	if rt.is_true(rt.identical(rt.new_string('ftpsockets'), rt.get_property(var_wp_filesystem_mutated,
		'method')))
	{
		var_has_broken_state = rt.new_bool(!rt.is_true(rt.get_property(var_wp_filesystem_mutated,
			'ftp')))
	...
	return rt.is_true(rt.new_bool(!(rt.is_true(var_has_broken_state))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(var_has_errors))))
}

fn Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.validate_upload_file_path(path string) {
	mut var_wp_filesystem :=
		Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.get_wp_filesystem()
	mut var_is_valid_file := rt.call_method(var_wp_filesystem, 'is_readable', [
		rt.new_string(path),
	])
	if rt.is_true(var_is_valid_file) {
		var_is_valid_file = Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.file_is_in_directory(path, (rt.call_method(var_wp_filesystem,
			'abspath', []rt.PhpVal{})).str())
		if rt.is_true(rt.new_bool(!(rt.is_true(var_is_valid_file)))) {
			mut var_upload_dir := rt.call_function('wp_get_upload_dir', []rt.PhpVal{})
			var_is_valid_file = rt.new_bool(
				rt.is_true(rt.identical(rt.new_bool(false), var_upload_dir.array_get(rt.new_string('error'))))
				&& rt.is_true(Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.file_is_in_directory(path, (var_upload_dir.array_get(rt.new_string('basedir'))).str())))
		}
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_is_valid_file)))) {
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.call_function('esc_html__', [
			rt.new_string('File path is not a valid upload path.'),
...
fn Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.file_is_in_directory(file_path string, directory string) bool {
	...
	mut var_protocol := rt.new_string('')
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('#^([a-z0-9]+://)#i'),
		rt.new_string(file_path_mutated).clone(), var_matches.clone()]))
	{
		var_protocol = var_matches.array_get(rt.new_int(1))
		file_path_mutated = (rt.call_function('preg_replace', [
	...
	file_path_mutated =
		(create_automattic_woocommerce_internal_utilities_url(rt.new_string(file_path_mutated).clone())).str()
	file_path_mutated = (rt.call_function('preg_replace', [
		rt.new_string('/^file:\\/\\//'),
		var_protocol.clone(),
		rt.new_string(file_path_mutated).clone(),
	...
	return (rt.identical(rt.new_int(0), rt.call_function('stripos', [
		rt.call_function('wp_normalize_path', [rt.new_string(file_path_mutated).clone()]),
		rt.call_function('trailingslashit', [
			rt.call_function('wp_normalize_path', [rt.new_string(directory)]),
		]),
	]))).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'mkdir_p_not_indexable' {
			...
			Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.mkdir_p_not_indexable(dispatch_arg_0,
				dispatch_arg_1)
			return rt.new_null()
		}
		...
		'is_usable_ftp_filesystem' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WP_Filesystem_Base](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
		...
		'file_is_in_directory' {
			...
			return rt.new_bool(Class_Automattic_WooCommerce_Internal_Utilities_FilesystemUtil.file_is_in_directory(dispatch_arg_0,
				dispatch_arg_1))
		}
		else {
			return none
