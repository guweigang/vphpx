...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) dbdelta(queries string, execute bool) rt.PhpVal {
	...
	return rt.call_function('dbDelta', [rt.new_string(queries),
		rt.new_bool(execute)])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) get_missing_tables(creation_queries string) rt.PhpVal {
	...
	mut var_suppress_errors := rt.call_method(var_wpdb, 'suppress_errors', [
		rt.new_bool(true),
	])
	...
	mut var_parsed_output :=
		this.parse_dbdelta_output(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Utilities_array](var_dbdelta_output))
	return var_parsed_output.array_get(rt.new_string('created_tables'))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) parse_dbdelta_output(mut var_dbdelta_output Class_Automattic_WooCommerce_Internal_Utilities_array) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.identical(rt.new_string('Created table ${var_table_name.to_string()}'),
			var_result))
		{
			var_created_tables.array_push(rt.call_function('str_replace', [
				rt.new_string('('),
				rt.new_string(''),
	...
	return rt.create_array([
		rt.ArrayItem{ key: 'created_tables', val: var_created_tables },
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) drop_database_table(table_name string, add_prefix bool) rt.PhpVal {
	...
	return rt.call_method(var_wpdb, 'query', [
		rt.new_string('DROP TABLE IF EXISTS `${var_table_name.to_string()}`'),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) drop_table_index(table_name string, index_name string) bool {
	...
	rt.call_method(var_wpdb, 'query', [
		rt.new_string('ALTER TABLE ${var_table_name.to_string()} DROP INDEX ${var_index_name.to_string()}'),
	])
	return true
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) create_primary_key(table_name string, mut var_columns Class_Automattic_WooCommerce_Internal_Utilities_array) bool {
	...
	rt.call_method(var_wpdb, 'query', [
		rt.new_string('ALTER TABLE ${var_table_name.to_string()} ADD PRIMARY KEY(`' +
			(rt.call_function('join', [rt.new_string('`,`'), var_columns_mutated])).str() + '`)'),
	])
	return true
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) get_index_columns(table_name string, index_name string) rt.PhpVal {
	...
	mut var_results := rt.call_method(var_wpdb, 'get_results', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.new_string('SHOW INDEX FROM ${var_table_name.to_string()} WHERE Key_name = %s'),
			rt.new_string(index_name_mutated).clone(),
		]),
	])
	if !rt.is_true(var_results) {
		return rt.new_array()
	}
	return rt.call_function('array_column', [var_results.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) format_object_value_for_db(var_value rt.PhpVal, type string) rt.PhpVal {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('decimal'))) {
		var_value_mutated = rt.call_function('wc_format_decimal', [
			var_value_mutated.clone(), rt.new_bool(false), rt.new_bool(true)])
		...
		var_value_mutated = rt.new_string(var_value_mutated.clone().to_string())
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('date'))) {
		var_value_mutated = if rt.is_true(var_value_mutated) { rt.call_method(rt.call_method(create_datetime(var_value_mutated.clone()), 'setTimezone', [
				create_datetimezone(rt.new_string('+00:00')),
			]), 'format', [rt.new_string('Y-m-d H:i:s')]) } else { rt.new_null() }
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('date_epoch'))) {
		var_value_mutated = if rt.is_true(var_value_mutated) { rt.call_method(create_datetime(rt.new_string('@${var_value.to_string()}')), 'format', [
				rt.new_string('Y-m-d H:i:s'),
			]) } else { rt.new_null() }
	} else {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_Utilities_Exception',
			[]string{}, create_automattic_woocommerce_internal_utilities_exception(rt.call_function('esc_html', [
			rt.new_string('Invalid type received: ' + type),
		]))))
	}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) get_wpdb_format_for_type(type string) rt.PhpVal {
	...
	if !(var_wpdb_placeholder_for_type.array_isset(rt.new_string(type))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Internal_Utilities_Exception',
			[]string{}, create_automattic_woocommerce_internal_utilities_exception(rt.call_function('esc_html', [
			rt.new_string('Invalid column type: ' + type),
		]))))
	}
	return var_wpdb_placeholder_for_type.array_get(rt.new_string(type))
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) generate_on_duplicate_statement_clause(mut var_columns Class_Automattic_WooCommerce_Internal_Utilities_array) string {
	...
	mut iter_2 := var_columns_mutated.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_column := item_2.val
		var_update_value_statements.array_push('`${var_column.to_string()}` = VALUES( `${var_column.to_string()}` )')
	}
	mut var_update_value_clause := rt.call_function('implode', [
		rt.new_string(', '), var_update_value_statements.clone()])
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) insert_on_duplicate_key_update(var_table_name rt.PhpVal, var_data rt.PhpVal, var_format rt.PhpVal) i64 {
	...
	mut var_columns := rt.func_array_keys(var_data.clone())
	...
	mut var_column_clause := rt.new_string('`' +
		(rt.call_function('implode', [rt.new_string('`, `'), var_columns.clone()])).str() + '`')
	mut var_value_format_clause := rt.call_function('implode', [
		rt.new_string(', '), var_value_format.clone()])
	mut var_on_duplicate_clause :=
		rt.new_string(this.generate_on_duplicate_statement_clause(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Utilities_array](var_columns)))
	...
	return (rt.call_method(var_wpdb, 'query', [var_sql.clone()])).to_i64()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) insert_or_update(var_table_name rt.PhpVal, var_data rt.PhpVal, var_where rt.PhpVal, var_format rt.PhpVal, var_where_format rt.PhpVal, primary_key_column string, primary_key_format string) i64 {
	...
	mut iter_4 := var_where.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_value := item_4.val
		mut var_column := item_4.key
		if rt.is_true(rt.new_bool(var_value.clone().is_null())) {
			var_conditions.array_push('`${var_column.to_string()}` IS NULL')
			continue
		}
		var_conditions.array_push('`${var_column.to_string()}` = ' +
			(var_where_format.array_get(var_index)).str())
		var_values.array_push(var_value.clone())
		rt.pre_inc(var_index)
	}
	var_conditions = rt.call_function('implode', [rt.new_string(' AND '),
		var_conditions.clone()])
	mut var_query := rt.call_method(var_wpdb, 'prepare', [
		rt.new_string('SELECT `${var_primary_key_column}` FROM `${var_table_name.to_string()}` WHERE ${var_conditions.to_string()} LIMIT 1'),
		var_values.clone(),
	])
	mut var_row_id := rt.call_method(var_wpdb, 'get_var', [var_query.clone()])
	if rt.is_true(var_row_id) {
		mut var_result := rt.call_method(var_wpdb, 'update', [
			var_table_name_mutated.clone(), var_data.clone(),
			rt.create_array([rt.ArrayItem{ key: primary_key_column, val: var_row_id }]),
			var_format.clone(), rt.create_array([rt.ArrayItem{ key: none, val: primary_key_format }])])
	} else {
		var_result = rt.call_method(var_wpdb, 'insert', [var_table_name_mutated.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) get_max_index_length() i64 {
	mut var_max_index_length := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_database_max_index_length'),
		rt.new_int(191),
	])
	return (rt.call_function('min', [
		rt.call_function('absint', [var_max_index_length.clone()]),
		rt.new_int(767),
	])).to_i64()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) create_fts_index_order_address_table() {
	mut var_wpdb := rt.new_null()
	mut var_address_table := rt.new_string((rt.get_property(var_wpdb, 'prefix')).str() +
		'wc_order_addresses')
	rt.call_method(var_wpdb, 'query', [
		rt.new_string('CREATE FULLTEXT INDEX order_addresses_fts ON ${var_address_table.to_string()} (first_name, last_name, company, address_1, address_2, city, state, postcode, country, email, phone)'),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) drop_fts_index_order_address_table() {
	...
	mut var_address_table := rt.new_string((rt.get_property(var_wpdb, 'prefix')).str() +
		'wc_order_addresses')
	rt.call_method(var_wpdb, 'query', [
		rt.new_string('ALTER TABLE ${var_address_table.to_string()} DROP INDEX order_addresses_fts;'),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) sanitise_boolean_fts_search_term(param string) string {
	...
	mut var_sanitized_param := rt.call_function('preg_replace', [
		rt.new_string('/[^\\p{L}\\p{N}_]+/u'),
		rt.new_string(' '),
		rt.new_string(param_mutated).clone(),
	])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_sanitized_param,
		rt.new_string(param_mutated)))))
	{
		param_mutated = (rt.call_function('str_replace', [rt.new_string('"'),
			rt.new_string(''), rt.new_string(param_mutated).clone()])).str()
		return '"' + param_mutated + '"'
	}
	mut var_words := rt.call_function('explode', [rt.new_string(' '),
		rt.new_string(param_mutated).clone()])
	mut var_sanitized_words := rt.new_array()
	mut iter_5 := var_words.iterator()
	for {
		item_5 := iter_5.next() or { break }
		mut var_word := item_5.val
		var_word = rt.new_string(var_word.str() + '*')
		var_sanitized_words.array_push(var_word.clone())
	}
	return (rt.call_function('implode', [rt.new_string(' '), var_sanitized_words.clone()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) fts_index_on_order_address_table_exists() bool {
	...
	mut var_address_table := rt.new_string((rt.get_property(var_wpdb, 'prefix')).str() +
		'wc_order_addresses')
	return !(!rt.is_true(rt.call_method(var_wpdb, 'get_results', [
		rt.new_string("SHOW INDEX FROM ${var_address_table.to_string()} WHERE Key_name = 'order_addresses_fts'"),
	])))
}

fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) create_fts_index_order_item_table() {
	mut var_wpdb := rt.new_null()
	mut var_order_item_table := rt.new_string((rt.get_property(var_wpdb, 'prefix')).str() +
		'woocommerce_order_items')
	rt.call_method(var_wpdb, 'query', [
		rt.new_string('CREATE FULLTEXT INDEX order_item_fts ON ${var_order_item_table.to_string()} (order_item_name)'),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) fts_index_on_order_item_table_exists() bool {
	...
	mut var_order_item_table := rt.new_string((rt.get_property(var_wpdb, 'prefix')).str() +
		'woocommerce_order_items')
	return !(!rt.is_true(rt.call_method(var_wpdb, 'get_results', [
		rt.new_string("SHOW INDEX FROM ${var_order_item_table.to_string()} WHERE Key_name = 'order_item_fts'"),
	])))
}
...
fn create_automattic_woocommerce_internal_utilities_databaseutil(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil {
...
fn (mut this Class_Automattic_WooCommerce_Internal_Utilities_DatabaseUtil) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'parse_dbdelta_output' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Utilities_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
		...
		'create_primary_key' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Internal_Utilities_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
		...
		'insert_on_duplicate_key_update' {
			...
			return rt.new_int(this.insert_on_duplicate_key_update(dispatch_arg_0, dispatch_arg_1,
				dispatch_arg_2))
		}
		'insert_or_update' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			...
			return rt.new_int(this.insert_or_update(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2,
