...
fn init_static_automattic_woocommerce_admin_deprecatedclassfacade() {
	rt.init_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade',
		'facade_over_classname', rt.new_string(''))
	rt.init_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade',
		'deprecated_in_version', rt.new_string(''))
	rt.init_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade', 'logged_messages',
		rt.new_array())
}

fn (mut this Class_Automattic_WooCommerce_Admin_DeprecatedClassFacade) construct() {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), rt.get_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade',
		'facade_over_classname')))))
	{
		this.instance = rt.create_object_dynamically(rt.get_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade',
			'facade_over_classname'), []rt.PhpVal{})
	}
}
...
fn Class_Automattic_WooCommerce_Admin_DeprecatedClassFacade.log_deprecation(var_function rt.PhpVal) {
	mut var_message := rt.call_function('sprintf', [
		rt.new_string('%1$s is deprecated since version %2$s! Use %3$s instead.'),
		rt.new_string(
			(Class_Automattic_WooCommerce_Admin_static.class()).str() + '::' + var_function.str()),
		rt.get_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade',
			'deprecated_in_version'),
		rt.new_string(
			(rt.get_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade', 'facade_over_classname')).str() +
			'::' + var_function.str()),
	])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), rt.get_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade',
		'facade_over_classname')))))
	{
		var_message = rt.new_string(var_message.str() +
			(rt.call_function('sprintf', [rt.new_string(' Use %s instead.'), rt.new_string((rt.get_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade', 'facade_over_classname')).str() +
			'::' + var_function.str())])).str())
	}
		...
		rt.new_bool(true)])))))
	{
		...
		rt.get_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade', 'logged_messages').array_push(var_message.clone())
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_DeprecatedClassFacade) magic_call(var_name rt.PhpVal, var_arguments rt.PhpVal) rt.PhpVal {
	Class_Automattic_WooCommerce_Admin_DeprecatedClassFacade.log_deprecation(var_name.clone())
...
fn Class_Automattic_WooCommerce_Admin_DeprecatedClassFacade.magic_callstatic(var_name rt.PhpVal, var_arguments rt.PhpVal) rt.PhpVal {
	Class_Automattic_WooCommerce_Admin_DeprecatedClassFacade.log_deprecation(var_name.clone())
	if rt.is_true(rt.identical(rt.new_string(''), rt.get_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade',
		'facade_over_classname')))
	{
		return rt.new_null()
	}
	return rt.call_function('call_user_func_array', [
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.get_static_prop('Automattic_WooCommerce_Admin_DeprecatedClassFacade',
				'facade_over_classname') },
			rt.ArrayItem{ key: none, val: var_name },
		]),
		var_arguments.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Admin_DeprecatedClassFacade) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'__callStatic' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return Class_Automattic_WooCommerce_Admin_DeprecatedClassFacade.magic_callstatic(dispatch_arg_0,
				dispatch_arg_1)
		}
		else {
			return none
