...
pub fn Class_Automattic_WooCommerce_Admin_WCAdminHelper.wc_admin_store_age_ranges() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'week-1', val: rt.create_array([
			rt.ArrayItem{ key: 'start', val: 0 },
			rt.ArrayItem{ key: 'end', val: rt.get_constant('WEEK_IN_SECONDS') },
		]) },
		rt.ArrayItem{ key: 'week-1-4', val: rt.create_array([
			rt.ArrayItem{ key: 'start', val: rt.get_constant('WEEK_IN_SECONDS') },
			rt.ArrayItem{ key: 'end', val: rt.mul(rt.get_constant('WEEK_IN_SECONDS'), rt.new_int(4)) },
		]) },
		rt.ArrayItem{ key: 'month-1-3', val: rt.create_array([
		...
		rt.ArrayItem{ key: 'month-3-6', val: rt.create_array([
			rt.ArrayItem{ key: 'start', val: rt.mul(rt.get_constant('MONTH_IN_SECONDS'),
				rt.new_int(3)) },
			rt.ArrayItem{ key: 'end', val: rt.mul(rt.get_constant('MONTH_IN_SECONDS'),
				rt.new_int(6)) },
		]) },
		rt.ArrayItem{ key: 'month-6+', val: rt.create_array([
			rt.ArrayItem{ key: 'start', val: rt.mul(rt.get_constant('MONTH_IN_SECONDS'),
				rt.new_int(6)) },
		]) },
	])
...
fn Class_Automattic_WooCommerce_Admin_WCAdminHelper.get_wcadmin_active_for_in_seconds() rt.PhpVal {
	mut var_install_timestamp := rt.call_function('get_option', [
		Class_Automattic_WooCommerce_Admin_Automattic_WooCommerce_Admin_WCAdminHelper.wc_admin_timestamp_option(),
	])
	if !(var_install_timestamp.clone().is_long() || var_install_timestamp.clone().is_double()) {
		var_install_timestamp = rt.call_function('time', []rt.PhpVal{})
		rt.call_function('update_option', [
			Class_Automattic_WooCommerce_Admin_Automattic_WooCommerce_Admin_WCAdminHelper.wc_admin_timestamp_option(),
			var_install_timestamp.clone(),
		])
	}
...
fn Class_Automattic_WooCommerce_Admin_WCAdminHelper.is_wc_admin_active_for(var_seconds rt.PhpVal) rt.PhpVal {
	mut var_wc_admin_active_for :=
		Class_Automattic_WooCommerce_Admin_WCAdminHelper.get_wcadmin_active_for_in_seconds()
...
fn Class_Automattic_WooCommerce_Admin_WCAdminHelper.is_wc_admin_active_in_date_range(var_range rt.PhpVal, var_custom_start rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(Class_Automattic_WooCommerce_Admin_Automattic_WooCommerce_Admin_WCAdminHelper.wc_admin_store_age_ranges().array_isset(var_range.clone())))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Admin_InvalidArgumentException',
			[]string{}, create_automattic_woocommerce_admin_invalidargumentexception(rt.call_function('sprintf', [
			rt.new_string('"%s" range is not supported, use one of: %s'),
			var_range.clone(),
			rt.call_function('implode', [rt.new_string(', '),
				rt.func_array_keys(Class_Automattic_WooCommerce_Admin_Automattic_WooCommerce_Admin_WCAdminHelper.wc_admin_store_age_ranges())]),
		]))))
	}
	mut var_wc_admin_active_for :=
		Class_Automattic_WooCommerce_Admin_WCAdminHelper.get_wcadmin_active_for_in_seconds()
	mut var_range_data :=
		Class_Automattic_WooCommerce_Admin_Automattic_WooCommerce_Admin_WCAdminHelper.wc_admin_store_age_ranges().array_get(var_range)
	mut var_start := if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(),
		var_custom_start))))
	{
		var_custom_start
	} else {
		var_range_data.array_get(rt.new_string('start'))
	}
	if rt.is_true(var_range_data)
		&& rt.is_true(rt.greater_equal(var_wc_admin_active_for, var_start)) {
		return (if var_range_data.array_isset(rt.new_string('end')) {
			rt.less(var_wc_admin_active_for, var_range_data.array_get(rt.new_string('end')))
		} else {
			rt.new_bool(true)
		}).to_bool()
	}
...
fn Class_Automattic_WooCommerce_Admin_WCAdminHelper.is_site_fresh() bool {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('1'), var_fresh_site)))) {
		return false
	}
	mut var_current_userdata := rt.call_function('get_userdata', [
		rt.call_function('get_current_user_id', []rt.PhpVal{}),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_current_userdata)))) {
		return false
	}
	mut var_date := create_automattic_woocommerce_admin_datetime(rt.get_property(var_current_userdata,
		'user_registered'))
...
fn Class_Automattic_WooCommerce_Admin_WCAdminHelper.is_current_page_store_page() bool {
	...
	mut var_store_pages := rt.create_array([
		...
		rt.ArrayItem{ key: 'terms', val: rt.call_function('wc_terms_and_conditions_page_id',
			[]rt.PhpVal{}) },
		rt.ArrayItem{ key: 'coming_soon', val: rt.call_function('wc_get_page_id', [
			rt.new_string('coming_soon'),
		]) },
	])
	var_store_pages = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_store_pages'),
	...
	mut var_url :=
		Class_Automattic_WooCommerce_Admin_WCAdminHelper.get_url_from_wp(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_WP](var_wp))
	mut var_is_store_page := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_is_extension_store_page'),
		rt.new_bool(false),
		var_url.clone(),
	])
	return (rt.call_function('filter_var', [var_is_store_page.clone(),
		rt.get_constant('FILTER_VALIDATE_BOOL')])).to_bool()
}
...
fn Class_Automattic_WooCommerce_Admin_WCAdminHelper.is_store_page(url string) rt.PhpVal {
	...
	rt.call_function('_deprecated_function', [rt.new_string(@METHOD),
		rt.new_string('9.8.0'), rt.new_string('is_current_page_store_page')])
...
fn Class_Automattic_WooCommerce_Admin_WCAdminHelper.get_normalized_url_path(var_url rt.PhpVal) rt.PhpVal {
	...
	mut var_query := rt.call_function('wp_parse_url', [var_url_mutated.clone(),
		rt.get_constant('PHP_URL_QUERY')])
	mut var_path := rt.new_string(
		(rt.call_function('wp_parse_url', [var_url_mutated.clone(), rt.get_constant('PHP_URL_PATH')])).str() +
		if rt.is_true(var_query) { '?' + var_query.str() } else { '' })
	mut var_home_path := if !(rt.call_function('wp_parse_url', [
	...
	mut var_normalized_path := rt.new_string(rt.call_function('substr', [
		var_path.clone(), rt.new_int(var_home_path.clone().to_string().len)]).to_string().trim_space())
	return var_normalized_path.clone()
}
...
fn Class_Automattic_WooCommerce_Admin_WCAdminHelper.get_url_from_wp(mut var_wp Class_Automattic_WooCommerce_Admin_WP) rt.PhpVal {
	if !rt.is_true(rt.get_property(var_wp, 'query_vars'))
		|| !rt.is_true(rt.get_property(var_wp, 'request')) {
		var_wp.parse_request()
	}
	return rt.call_function('home_url', [
		rt.call_function('add_query_arg', [rt.get_property(var_wp, 'query_vars'),
...
fn create_automattic_woocommerce_admin_invalidargumentexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_InvalidArgumentException {
...
fn (mut this Class_Automattic_WooCommerce_Admin_WCAdminHelper) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'is_wc_admin_active_in_date_range' {
			...
			return rt.new_bool(Class_Automattic_WooCommerce_Admin_WCAdminHelper.is_wc_admin_active_in_date_range(dispatch_arg_0,
				dispatch_arg_1))
		}
		'is_site_fresh' {
		...
		'get_url_from_wp' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_WP](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
