...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) construct(data string) {
	this.dispatch_set_prop('data', rt.create_array([
		rt.ArrayItem{ key: 'name', val: '-' },
		rt.ArrayItem{
			key: 'type'
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_informational()
		},
		rt.ArrayItem{ key: 'locale', val: 'en_US' },
		rt.ArrayItem{ key: 'title', val: '-' },
		rt.ArrayItem{ key: 'content', val: '-' },
		rt.ArrayItem{ key: 'content_data', val: create_automattic_woocommerce_admin_notes_stdclass() },
		rt.ArrayItem{
			key: 'status'
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_unactioned()
		},
		rt.ArrayItem{ key: 'source', val: 'woocommerce' },
		rt.ArrayItem{ key: 'date_created', val: '0000-00-00 00:00:00' },
		rt.ArrayItem{ key: 'date_reminder', val: rt.new_null() },
		rt.ArrayItem{ key: 'is_snoozable', val: false },
		rt.ArrayItem{ key: 'actions', val: rt.new_array() },
		rt.ArrayItem{ key: 'layout', val: 'plain' },
		rt.ArrayItem{ key: 'image', val: '' },
		rt.ArrayItem{ key: 'is_deleted', val: false },
		rt.ArrayItem{ key: 'is_read', val: false },
	]))
	this.Class_Automattic_WooCommerce_Admin_Notes_WC_Data.construct(rt.new_string(data))
	if rt.is_true(rt.new_bool(rt.instance_of(rt.new_string(data),
		'Automattic_WooCommerce_Admin_Notes_Note')))
	{
		this.set_id(rt.call_function('absint', [
			rt.call_method(rt.new_string(data), 'get_id', []rt.PhpVal{}),
		]))
	} else if rt.is_true(rt.new_bool(rt.new_string(data).is_long()
		|| rt.new_string(data).is_double()))
	{
		this.set_id(rt.new_string(data))
	} else if rt.new_string(data).is_object()
		&& !(!rt.is_true(rt.get_property(rt.new_string(data), 'note_id'))) {
	...
	if rt.is_true(rt.greater(this.get_id(), rt.new_int(0))) {
		rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_Notes_Note', [
			'Automattic_WooCommerce_Admin_Notes_WC_Data',
		], &this), 'data_store'), 'read', [
			rt.new_object('Automattic_WooCommerce_Admin_Notes_Note', [
				'Automattic_WooCommerce_Admin_Notes_WC_Data',
			], &this),
		])
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) apply_changes() {
	this.dispatch_set_prop('data', rt.call_function('array_replace_recursive', [
		rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_Notes_Note', [
			'Automattic_WooCommerce_Admin_Notes_WC_Data',
		], &this), 'data'),
		rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_Notes_Note', [
			'Automattic_WooCommerce_Admin_Notes_WC_Data',
	...
	if rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_Notes_Note', [
		'Automattic_WooCommerce_Admin_Notes_WC_Data',
	], &this), 'changes').array_isset(rt.new_string('actions'))
	{
		rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_Notes_Note', [
			'Automattic_WooCommerce_Admin_Notes_WC_Data',
		], &this), 'data').array_set('actions', rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_Notes_Note', [
			'Automattic_WooCommerce_Admin_Notes_WC_Data',
		], &this), 'changes').array_get(rt.new_string('actions')))
	}
...
fn Class_Automattic_WooCommerce_Admin_Notes_Note.get_deprecated_types() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_email()
		},
	])
}
...
fn Class_Automattic_WooCommerce_Admin_Notes_Note.get_allowed_types() rt.PhpVal {
	mut var_allowed_types := rt.create_array([
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_error()
		},
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_warning()
		},
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_update()
		},
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_informational()
		...
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_marketing()
		},
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_survey()
		},
	])
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_note_types'),
...
fn Class_Automattic_WooCommerce_Admin_Notes_Note.get_allowed_statuses() rt.PhpVal {
	mut var_allowed_statuses := rt.create_array([
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_pending()
		},
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_actioned()
		},
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_unactioned()
		...
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_snoozed()
		},
		rt.ArrayItem{
			key: none
			val: Class_Automattic_WooCommerce_Admin_Notes_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_sent()
		},
	])
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_note_statuses'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) get_action(var_action_name rt.PhpVal, context string) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.identical(rt.get_property(var_action, 'name'), var_action_name)) {
			var_matching_action = var_actions.array_get(var_i)
			break
		}
	}
	return var_matching_action.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_name(var_name rt.PhpVal) {
	...
	if !rt.is_true(var_name_mutated) {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note name prop cannot be empty.'),
			rt.new_string('woocommerce'),
		]))
	}
	this.set_prop(rt.new_string('name'), var_name_mutated.clone())
}

fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_type(var_type rt.PhpVal) {
	if !rt.is_true(var_type) {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note type prop cannot be empty.'),
			rt.new_string('woocommerce'),
		]))
	}
	if rt.is_true(rt.call_function('in_array', [var_type.clone(),
		Class_Automattic_WooCommerce_Admin_Notes_Note.get_deprecated_types(),
		rt.new_bool(true)]))
	{
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note type prop is deprecated.'),
			rt.new_string('woocommerce'),
		]))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_type.clone(), Class_Automattic_WooCommerce_Admin_Notes_Note.get_allowed_types(),
		rt.new_bool(true)])))))
	{
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('The admin note type prop (%s) is not one of the supported types.'),
				rt.new_string('woocommerce'),
			]),
			var_type.clone(),
		]))
	}
	this.set_prop(rt.new_string('type'), var_type.clone())
}

fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_locale(var_locale rt.PhpVal) {
	if !rt.is_true(var_locale) {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note locale prop cannot be empty.'),
			rt.new_string('woocommerce'),
		]))
	}
	this.set_prop(rt.new_string('locale'), var_locale.clone())
}

fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_title(var_title rt.PhpVal) {
	if !rt.is_true(var_title) {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note title prop cannot be empty.'),
			rt.new_string('woocommerce'),
		]))
	}
	this.set_prop(rt.new_string('title'), var_title.clone())
}

fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_icon(var_icon rt.PhpVal) {
	rt.call_function('wc_deprecated_function', [rt.new_string('set_icon'),
		rt.new_string('4.3')])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_content(var_content rt.PhpVal) {
	...
	var_content_mutated = rt.call_function('wp_kses', [var_content_mutated.clone(),
		var_allowed_html.clone()])
	if !rt.is_true(var_content_mutated) {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note content prop cannot be empty.'),
			rt.new_string('woocommerce'),
		]))
	}
	this.set_prop(rt.new_string('content'), var_content_mutated.clone())
}

fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_content_data(var_content_data rt.PhpVal) {
	mut var_allowed_type := rt.new_bool(false)
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_content_data,
		'Automattic_WooCommerce_Admin_Notes_stdClass'))))))
	{
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note content_data prop must be an instance of stdClass.'),
			rt.new_string('woocommerce'),
		]))
	}
	this.set_prop(rt.new_string('content_data'), var_content_data.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_status(var_status rt.PhpVal) {
	...
	if !rt.is_true(var_status_mutated) {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note status prop cannot be empty.'),
			rt.new_string('woocommerce'),
		]))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_status_mutated.clone(),
		Class_Automattic_WooCommerce_Admin_Notes_Note.get_allowed_statuses(),
		rt.new_bool(true)])))))
	{
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('The admin note status prop (%s) is not one of the supported statuses.'),
				rt.new_string('woocommerce'),
			]),
			var_status_mutated.clone(),
		]))
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_source(var_source rt.PhpVal) {
	if !rt.is_true(var_source) {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note source prop cannot be empty.'),
			rt.new_string('woocommerce'),
		]))
	}
	this.set_prop(rt.new_string('source'), var_source.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_date_created(var_date rt.PhpVal) {
	...
	if !rt.is_true(var_date_mutated) {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note date prop cannot be empty.'),
			rt.new_string('woocommerce'),
		]))
	}
	if var_date_mutated.clone().is_string() && !(var_date_mutated.clone().is_long()
		|| var_date_mutated.clone().is_double()) {
		var_date_mutated = rt.call_function('wc_string_to_timestamp', [
			var_date_mutated.clone()])
	}
	this.set_date_prop(rt.new_string('date_created'), var_date_mutated.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_date_reminder(var_date rt.PhpVal) {
	...
	this.set_date_prop(rt.new_string('date_reminder'), var_date_mutated.clone())
}

fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_is_snoozable(var_is_snoozable rt.PhpVal) rt.PhpVal {
	return this.set_prop(rt.new_string('is_snoozable'), var_is_snoozable.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_layout(var_layout rt.PhpVal) {
	...
	if !rt.is_true(var_layout_mutated) {
		var_layout_mutated = rt.new_string('plain')
	}
	mut var_valid_layouts := rt.create_array([rt.ArrayItem{ key: none, val: 'plain' },
		rt.ArrayItem{ key: none, val: 'thumbnail' }])
	if rt.is_true(rt.call_function('in_array', [var_layout_mutated.clone(),
		var_valid_layouts.clone(), rt.new_bool(true)]))
	{
		this.set_prop(rt.new_string('layout'), var_layout_mutated.clone())
	} else {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note layout has a wrong prop value.'),
			rt.new_string('woocommerce'),
		]))
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) add_action(var_name rt.PhpVal, var_label rt.PhpVal, url string, var_status rt.PhpVal, primary bool, actioned_text string) {
	...
	var_label_mutated = rt.call_function('wc_clean', [var_label_mutated.clone()])
	mut var_query := rt.call_function('esc_url_raw', [rt.new_string(url)])
	var_status_mutated = rt.call_function('wc_clean', [var_status_mutated.clone()])
	actioned_text_mutated = (rt.call_function('wc_clean',
		[rt.new_string(actioned_text_mutated).clone()])).str()
	if !rt.is_true(var_name_mutated) {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note action name prop cannot be empty.'),
			rt.new_string('woocommerce'),
		]))
	}
	if !rt.is_true(var_label_mutated) {
		this.error(rt.new_string('admin_note_invalid_data'), rt.call_function('__', [
			rt.new_string('The admin note action label prop cannot be empty.'),
			rt.new_string('woocommerce'),
		]))
	}
	mut var_action := rt.create_array([
		rt.ArrayItem{ key: 'name', val: var_name_mutated },
		rt.ArrayItem{ key: 'label', val: var_label_mutated },
		rt.ArrayItem{ key: 'query', val: var_query },
		rt.ArrayItem{ key: 'status', val: var_status_mutated },
		rt.ArrayItem{ key: 'actioned_text', val: actioned_text_mutated },
		rt.ArrayItem{ key: 'nonce_name', val: rt.new_null() },
		rt.ArrayItem{ key: 'nonce_action', val: rt.new_null() },
	])
	mut var_note_actions := this.get_prop(rt.new_string('actions'), rt.new_string('edit'))
	var_note_actions.array_push(rt.array_to_object(var_action))
	this.set_prop(rt.new_string('actions'), var_note_actions.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) set_actions(var_actions rt.PhpVal) {
	...
	this.set_prop(rt.new_string('actions'), var_actions_mutated.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) add_nonce_to_action(note_action_name string, nonce_action string, nonce_name string) {
	...
	for {
		...
		if rt.is_true(rt.identical(rt.get_property(var_action, 'name'),
			rt.new_string(note_action_name)))
		{
			var_matching_action = var_actions.array_get(var_i)
		}
	}
	if !rt.is_true(var_matching_action) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Admin_Notes_Exception',
			[]string{}, create_automattic_woocommerce_admin_notes_exception(rt.call_function('sprintf', [
			rt.new_string('Could not find action %s in note %s'),
			rt.new_string(note_action_name),
			this.get_name(''),
		]))))
	}
	rt.set_property(var_matching_action, 'nonce_action', rt.new_string(nonce_action))
	rt.set_property(var_matching_action, 'nonce_name', rt.new_string(nonce_name))
	this.set_actions(var_actions.clone())
}
...
fn create_automattic_woocommerce_admin_notes_wc_data(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Notes_WC_Data {
...
fn (mut this Class_Automattic_WooCommerce_Admin_Notes_Note) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'add_action' {
			...
			this.add_action(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2, dispatch_arg_3,
