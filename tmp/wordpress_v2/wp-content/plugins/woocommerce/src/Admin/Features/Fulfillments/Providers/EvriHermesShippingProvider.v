...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_EvriHermesShippingProvider) try_parse_tracking_number(tracking_number string, shipping_from string, shipping_to string) rt.PhpVal {
	...
	for {
		...
		{
			mut var_confidence := rt.new_int(90)
	...
	for {
		...
		if rt.is_true(rt.call_function('preg_match', [var_pattern.clone(),
			var_normalized.clone()]))
		{
			mut var_confidence := rt.new_int(75)
			if rt.is_true(rt.identical(rt.new_string('GB'), rt.new_string(shipping_from))) {
				var_confidence = rt.call_function('min', [rt.new_int(95),
					rt.add(var_confidence, rt.new_int(15))])
			}
			return rt.create_array([
				rt.ArrayItem{ key: 'url', val: this.get_tracking_url(var_normalized.str()) },
				rt.ArrayItem{ key: 'ambiguity_score', val: var_confidence },
...
fn create_automattic_woocommerce_admin_features_fulfillments_providers_evrihermesshippingprovider(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_EvriHermesShippingProvider {
