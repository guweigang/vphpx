...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax) construct(var_task_list rt.PhpVal) {
	...
	rt.call_function('add_action', [rt.new_string('admin_enqueue_scripts'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'possibly_add_return_notice_script' },
		])])
	rt.call_function('add_action', [rt.new_string('woocommerce_tax_rate_added'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax', [
				'Automattic_WooCommerce_Admin_Features_OnboardingTasks_Task',
			], &this) },
			rt.ArrayItem{ key: none, val: 'on_tax_rate_added' },
		])])
	rt.call_function('add_action', [rt.new_string('woocommerce_tax_rate_deleted'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax', [
				'Automattic_WooCommerce_Admin_Features_OnboardingTasks_Task',
			], &this) },
			rt.ArrayItem{ key: none, val: 'on_tax_rate_deleted' },
		])])
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax) possibly_add_return_notice_script() {
	mut var_page := if rt.get_superglobal('_GET').array_isset(rt.new_string('page')) {
		rt.get_superglobal('_GET').array_get(rt.new_string('page'))
	} else {
		rt.new_string('')
	}
	mut var_tab := if rt.get_superglobal('_GET').array_isset(rt.new_string('tab')) {
		rt.get_superglobal('_GET').array_get(rt.new_string('tab'))
	} else {
		rt.new_string('')
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_page, rt.new_string('wc-settings')))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_tab, rt.new_string('tax'))))) {
		return
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(this.is_active())))) || this.is_complete() {
		return
	}
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Admin_WCAdminAssets{}
	mut iife_result_0 := iife_temp_0.register_script(rt.new_string('wp-admin-scripts'),
		rt.new_string('onboarding-tax-notice'), rt.new_bool(true))
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax) get_title() rt.PhpVal {
	return rt.call_function('__', [rt.new_string('Collect sales tax'),
		rt.new_string('woocommerce')])
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax) get_content() rt.PhpVal {
	return if rt.is_true(Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax.can_use_automated_taxes()) { rt.call_function('__', [
			rt.new_string('Good news! WooCommerce Tax can automate your sales tax calculations for you.'),
			rt.new_string('woocommerce'),
		]) } else { rt.call_function('__', [
			rt.new_string('Set your store location and configure tax rate settings.'),
			rt.new_string('woocommerce'),
		]) }
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax) get_action_label() rt.PhpVal {
	return if rt.is_true(Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax.can_use_automated_taxes()) { rt.call_function('__', [
			rt.new_string('Yes please'),
			rt.new_string('woocommerce'),
		]) } else { rt.call_function('__', [rt.new_string("Let's go"),
			rt.new_string('woocommerce')]) }
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax) is_complete() bool {
	if rt.is_true(rt.identical(this.is_complete_result, rt.new_null())) {
		mut var_wc_connect_taxes_enabled := rt.call_function('get_option', [
			rt.new_string('wc_connect_taxes_enabled'),
		])
		mut var_is_wc_connect_taxes_enabled := rt.new_bool(
			rt.is_true(rt.identical(var_wc_connect_taxes_enabled, rt.new_string('yes')))
			|| rt.is_true(rt.identical(var_wc_connect_taxes_enabled, rt.new_bool(true))))
		mut var_third_party_complete := rt.call_function('apply_filters', [
			rt.new_string('woocommerce_admin_third_party_tax_setup_complete'),
			rt.new_bool(false),
		])
		this.is_complete_result = rt.is_true(var_is_wc_connect_taxes_enabled)
			|| rt.is_true(var_third_party_complete)
			|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('get_option', [rt.new_string('woocommerce_no_sales_tax')])))))
			|| rt.is_true(this.has_existing_tax_rates())
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax) has_existing_tax_rates() rt.PhpVal {
	...
	mut var_has_existing_tax_rates := rt.call_function('wp_cache_get', [
		Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax.tax_rate_exists_cache_key(),
	])
	if rt.is_true(rt.identical(rt.new_bool(false), var_has_existing_tax_rates)) {
		mut var_rate_exists := rt.new_bool((rt.call_method(var_wpdb, 'get_var', [
			rt.concat(rt.concat(rt.new_string('SELECT 1 FROM '),
				rt.get_property(var_wpdb, 'prefix')),
				rt.new_string('woocommerce_tax_rates limit 1')),
		])).to_bool())
		var_has_existing_tax_rates = rt.new_string((if rt.is_true(var_rate_exists) {
			'yes'
		} else {
			'no'
		}).str())
		rt.call_function('wp_cache_set', [
			Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax.tax_rate_exists_cache_key(),
			var_has_existing_tax_rates.clone(),
		])
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax) get_additional_data() rt.PhpVal {
	...
	mut iife_result_1 := iife_temp_1.is_plugin_active(rt.new_string('woocommerce-avatax'))
	mut iife_temp_2 := Class_Automattic_WooCommerce_Admin_PluginsHelper{}
	mut iife_result_2 := iife_temp_2.is_plugin_active(rt.new_string('stripe-tax-for-woocommerce'))
	mut iife_temp_3 := Class_Automattic_WooCommerce_Admin_PluginsHelper{}
	mut iife_result_3 := iife_temp_3.is_plugin_active(rt.new_string('woocommerce-tax'))
	mut iife_temp_4 := Class_Automattic_WooCommerce_Admin_PluginsHelper{}
	mut iife_result_4 := iife_temp_4.is_plugin_active(rt.new_string('woocommerce-shipping'))
	return rt.create_array([rt.ArrayItem{ key: 'avalara_activated', val: iife_result_1 },
		rt.ArrayItem{ key: 'tax_jar_activated', val: rt.call_function('class_exists', [
			rt.new_string('WC_Taxjar'),
		]) }, rt.ArrayItem{ key: 'stripe_tax_activated', val: iife_result_2 },
		rt.ArrayItem{ key: 'woocommerce_tax_activated', val: iife_result_3 },
		rt.ArrayItem{ key: 'woocommerce_shipping_activated', val: iife_result_4 },
		rt.ArrayItem{
			key: 'woocommerce_tax_countries'
			val: Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax.get_automated_support_countries()
		}, rt.ArrayItem{
			key: 'stripe_tax_countries'
			val: Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax.get_stripe_tax_support_countries()
		}])
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax.can_use_automated_taxes() bool {
	...
	return (rt.call_function('in_array', [
		...
		Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax.get_automated_support_countries(),
		rt.new_bool(true),
	])).to_bool()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax.get_automated_support_countries() rt.PhpVal {
	...
	return var_tax_supported_countries.clone()
}

fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax.get_stripe_tax_support_countries() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: none, val: 'AU' },
		rt.ArrayItem{ key: none, val: 'AT' }, rt.ArrayItem{ key: none, val: 'BE' },
		rt.ArrayItem{ key: none, val: 'BG' }, rt.ArrayItem{ key: none, val: 'CA' },
		rt.ArrayItem{ key: none, val: 'HR' }, rt.ArrayItem{ key: none, val: 'CY' },
		rt.ArrayItem{ key: none, val: 'CZ' }, rt.ArrayItem{ key: none, val: 'DK' },
		rt.ArrayItem{ key: none, val: 'EE' }, rt.ArrayItem{ key: none, val: 'FI' },
		rt.ArrayItem{ key: none, val: 'FR' }, rt.ArrayItem{ key: none, val: 'DE' },
		rt.ArrayItem{ key: none, val: 'GR' }, rt.ArrayItem{ key: none, val: 'HK' },
		rt.ArrayItem{ key: none, val: 'HU' }, rt.ArrayItem{ key: none, val: 'IE' },
		rt.ArrayItem{ key: none, val: 'IT' }, rt.ArrayItem{ key: none, val: 'JP' },
		rt.ArrayItem{ key: none, val: 'LV' }, rt.ArrayItem{ key: none, val: 'LT' },
		rt.ArrayItem{ key: none, val: 'LU' }, rt.ArrayItem{ key: none, val: 'MT' },
		rt.ArrayItem{ key: none, val: 'NL' }, rt.ArrayItem{ key: none, val: 'NZ' },
		rt.ArrayItem{ key: none, val: 'NO' }, rt.ArrayItem{ key: none, val: 'PL' },
		rt.ArrayItem{ key: none, val: 'PT' }, rt.ArrayItem{ key: none, val: 'RO' },
		rt.ArrayItem{ key: none, val: 'SG' }, rt.ArrayItem{ key: none, val: 'SK' },
		rt.ArrayItem{ key: none, val: 'SI' }, rt.ArrayItem{ key: none, val: 'ES' },
		rt.ArrayItem{ key: none, val: 'SE' }, rt.ArrayItem{ key: none, val: 'CH' },
		rt.ArrayItem{ key: none, val: 'AE' }, rt.ArrayItem{ key: none, val: 'GB' },
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Tax) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'is_complete_result' {
			this.is_complete_result = val.to_bool()
			return true
		}
		else {
