...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_DHLShippingProvider) get_tracking_url(tracking_number string) string {
	...
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^(GM|LX|RX|CN|SG|MY|HK|AU|TH|420)/'), rt.new_string(tracking_number_mutated).clone()])) {
		return 'https://webtrack.dhlglobalmail.com/?trackingnumber=' + (rt.call_function('rawurlencode', [rt.new_string(tracking_number_mutated).clone()])).str()
	}
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^3S[A-Z0-9]{8,12}$/'), rt.new_string(tracking_number_mutated).clone()])) {
		return 'https://www.dhl.de/en/privatkunden/dhl-sendungsverfolgung.html?piececode=' + (rt.call_function('rawurlencode', [rt.new_string(tracking_number_mutated).clone()])).str()
	}
	return 'https://www.dhl.com/en/express/tracking.html?AWB=' + (rt.call_function('rawurlencode', [rt.new_string(tracking_number_mutated).clone()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_DHLShippingProvider) try_parse_tracking_number(tracking_number string, shipping_from string, shipping_to string) rt.PhpVal {
	...
	closure_2_fn := fn [var_tracking_number] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut iife_result_1 := iife_temp_1.validate_mod11_check_digit(rt.new_string(tracking_number_mutated))
		return rt.new_int(if rt.is_true(iife_result_1) { 98 } else { 90 })
		}
	closure_4_fn := fn [var_tracking_number] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut iife_result_3 := iife_temp_3.validate_mod11_check_digit(rt.new_string(tracking_number_mutated))
		return rt.new_int(if rt.is_true(iife_result_3) { 98 } else { 90 })
		}
	closure_5_fn := fn [var_shipping_from, var_shipping_to] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		return rt.new_int(if rt.is_true(rt.identical(rt.new_string('DE'), rt.new_string(shipping_from))) && rt.is_true(rt.identical(rt.new_string('DE'), rt.new_string(shipping_to))) { 92 } else { 60 })
		}
	closure_6_fn := fn [var_shipping_from, var_shipping_to] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		return rt.new_int(if rt.is_true(rt.identical(rt.new_string('DE'), rt.new_string(shipping_from))) && rt.is_true(rt.identical(rt.new_string('DE'), rt.new_string(shipping_to))) { 92 } else { 60 })
		}
	closure_7_fn := fn [var_shipping_from] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	...
	closure_9_fn := fn [var_tracking_number] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut iife_temp_8 := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils{}
		mut iife_result_8 := iife_temp_8.check_s10_upu_format(rt.new_string(tracking_number_mutated))
		return rt.new_int(if rt.is_true(iife_result_8) { 88 } else { 75 })
		}
	mut var_patterns := rt.create_array([rt.ArrayItem{ key: '/^\\d{10}$/', val: rt.new_closure(closure_2_fn) }, rt.ArrayItem{ key: '/^\\d{11}$/', val: rt.new_closure(closure_4_fn) }, rt.ArrayItem{ key: '/^JJD\\d{10}$/', val: 98 }, rt.ArrayItem{ key: '/^JVGL\\d{10}$/', val: 98 }, rt.ArrayItem{ key: '/^\\d{12}$/', val: rt.new_closure(closure_5_fn) }, rt.ArrayItem{ key: '/^\\d{14}$/', val: rt.new_closure(closure_6_fn) }, rt.ArrayItem{ key: '/^\\d{20}$/', val: 90 }, rt.ArrayItem{ key: '/^3S[A-Z0-9]{8,12}$/', val: 95 }, rt.ArrayItem{ key: '/^GM\\d{16,20}$/', val: rt.new_closure(closure_7_fn) }, rt.ArrayItem{ key: '/^(LX|RX|CN|SG|MY|HK|AU|TH)\\d{9}[A-Z]{2}$/', val: 92 }, rt.ArrayItem{ key: '/^420\\d{23,31}$/', val: 90 }, rt.ArrayItem{ key: '/^\\d{7,9}$/', val: 88 }, rt.ArrayItem{ key: '/^\\d[A-Z]{2}\\d{4,6}$/', val: 90 }, rt.ArrayItem{ key: '/^[A-Z]{3,4}\\d{4,8}$/', val: 88 }, rt.ArrayItem{ key: '/^DSD\\d{8,12}$/', val: 92 }, rt.ArrayItem{ key: '/^JD\\d{11}$/', val: 90 }, rt.ArrayItem{ key: '/^DSC\\d{10,15}$/', val: 85 }, rt.ArrayItem{ key: '/^[A-Z]{2}\\d{9}[A-Z]{2}$/', val: rt.new_closure(closure_9_fn) }, rt.ArrayItem{ key: '/^\\d{22}$/', val: 70 }])
	...
	for {
		...
		if rt.is_true(rt.call_function('preg_match', [var_pattern.clone(), rt.new_string(tracking_number_mutated).clone()])) {
			mut var_score := if rt.call_function('is_callable', [var_base_score.clone()]) { rt.call_callable(var_base_score, []rt.PhpVal{}) } else { var_base_score }
...
fn create_automattic_woocommerce_admin_features_fulfillments_providers_dhlshippingprovider(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_DHLShippingProvider {
