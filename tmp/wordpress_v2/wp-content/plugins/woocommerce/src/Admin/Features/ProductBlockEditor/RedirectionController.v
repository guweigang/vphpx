...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController) construct() {
	...
	if rt.is_true(iife_result_0) {
		rt.call_function('add_action', [rt.new_string('current_screen'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'maybe_redirect_to_new_editor' },
			]),
		...
		rt.call_function('add_action', [rt.new_string('current_screen'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'redirect_non_supported_product_types' },
			]),
			rt.new_int(30), rt.new_int(0)])
	} else {
		rt.call_function('add_action', [rt.new_string('current_screen'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'maybe_redirect_to_old_editor' },
			]),
			rt.new_int(30), rt.new_int(0)])
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController) is_legacy_add_new_screen() bool {
	...
	return rt.is_true(rt.identical(rt.new_string('post'), rt.get_property(var_screen, 'base')))
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController) is_product_supported(var_product_id rt.PhpVal) bool {
	...
	mut var_product := if rt.is_true(var_product_id_mutated) { rt.call_function('wc_get_product', [
			var_product_id_mutated.clone(),
		]) } else { rt.new_null() }
	if rt.is_true(rt.new_bool(var_product.clone().is_null())) {
		return false
	}
	mut var_digital_product := rt.new_bool(
		rt.is_true(rt.call_method(var_product, 'is_downloadable', []rt.PhpVal{}))
		|| rt.is_true(rt.call_method(var_product, 'is_virtual', []rt.PhpVal{})))
	mut var_product_template_id := rt.call_method(var_product, 'get_meta', [
	...
	for {
		...
		if rt.is_true(rt.new_bool(rt.call_method(var_product_template, 'get_layout_template_id',
			[]rt.PhpVal{}).is_null()))
		{
			continue
		}
		mut var_product_data := rt.call_method(var_product_template, 'get_product_data',
			[]rt.PhpVal{})
		mut var_product_data_type := var_product_data.array_get(rt.new_string('type'))
		mut var_product_type := if rt.is_true(rt.identical(rt.call_method(var_product, 'get_type',
			[]rt.PhpVal{}), Class_Automattic_WooCommerce_Enums_ProductType.variable()))
		{
			Class_Automattic_WooCommerce_Enums_ProductType.simple()
		} else {
			rt.call_method(var_product, 'get_type', []rt.PhpVal{})
		}
		if !var_product_data_type.is_null()
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_product_data_type, var_product_type)))) {
			continue
		}
		if !var_product_template_id.is_null()
			&& rt.is_true(rt.identical(var_product_template_id, rt.call_method(var_product_template, 'get_id', []rt.PhpVal{}))) {
			return true
		}
		if !var_product_data_type.is_null() {
			return true
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController) set_product_templates(mut var_product_templates Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_array) {
	this.product_templates = var_product_templates
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController) maybe_redirect_to_new_editor() {
	...
	if this.is_legacy_edit_screen() {
		mut var_product_id := if rt.get_superglobal('_GET').array_isset(rt.new_string('post')) { rt.call_function('absint', [
				rt.get_superglobal('_GET').array_get(rt.new_string('post')),
			]) } else { rt.new_null() }
		if !(this.is_product_supported(var_product_id.clone())) {
			return
		}
		rt.call_function('wp_safe_redirect', [
			rt.call_function('admin_url', [
				rt.new_string('admin.php?page=wc-admin&path=/product/' + var_product_id.str()),
			]),
		])
		exit(0)
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController) maybe_redirect_to_old_editor() {
	...
	if rt.is_true(rt.identical(rt.new_string('add-product'),
		var_route.array_get(rt.new_string('page'))))
	{
		rt.call_function('wp_safe_redirect', [
			rt.call_function('admin_url', [
				rt.new_string('post-new.php?post_type=product'),
			]),
		])
		exit(0)
	}
	if rt.is_true(rt.identical(rt.new_string('product'), var_route.array_get(rt.new_string('page')))) {
		rt.call_function('wp_safe_redirect', [
			rt.call_function('admin_url', [
				rt.new_string('post.php?post=' +
					(var_route.array_get(rt.new_string('product_id'))).str() + '&action=edit'),
			]),
		])
		exit(0)
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController) get_parsed_route() rt.PhpVal {
	mut iife_temp_1 :=
		Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Automattic_WooCommerce_Admin_PageController{}
	mut iife_result_1 := iife_temp_1.is_admin_page()
	if rt.is_true(rt.new_bool(!(rt.is_true(iife_result_1))))
		|| !(rt.get_superglobal('_GET').array_isset(rt.new_string('path'))) {
		return rt.create_array([rt.ArrayItem{ key: 'page', val: rt.new_null() },
			rt.ArrayItem{ key: 'product_id', val: rt.new_null() }])
	}
	mut var_path := rt.call_function('esc_url_raw', [
		rt.call_function('wp_unslash',
			[rt.get_superglobal('_GET').array_get(rt.new_string('path'))]),
	])
	mut var_path_pieces := rt.call_function('explode', [rt.new_string('/'),
		rt.call_function('wp_parse_url', [var_path.clone(), rt.get_constant('PHP_URL_PATH')])])
	return rt.create_array([
		rt.ArrayItem{
			key: 'page'
			val: if !(var_path_pieces.array_get(rt.new_int(1))).is_null() {
				var_path_pieces.array_get(rt.new_int(1))
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{
			key: 'product_id'
			val: if rt.is_true(rt.identical(rt.new_string('product'), if !(var_path_pieces.array_get(rt.new_int(1))).is_null() {
				var_path_pieces.array_get(rt.new_int(1))
			} else {
				rt.new_string('')
			}))
			{ rt.call_function('absint', [if !(var_path_pieces.array_get(rt.new_int(2))).is_null() {
					var_path_pieces.array_get(rt.new_int(2))
				} else {
					rt.new_int(0)
				}])
			 } else { rt.new_null()
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController) redirect_non_supported_product_types() {
	mut var_route := this.get_parsed_route()
	mut var_product_id := var_route.array_get(rt.new_string('product_id'))
	if rt.is_true(rt.identical(rt.new_string('product'), var_route.array_get(rt.new_string('page'))))
		&& !(this.is_product_supported(var_product_id.clone())) {
		rt.call_function('wp_safe_redirect', [
			rt.call_function('admin_url', [
				rt.new_string('post.php?post=' +
					(var_route.array_get(rt.new_string('product_id'))).str() + '&action=edit'),
...
fn create_automattic_woocommerce_admin_features_productblockeditor_automattic_woocommerce_admin_pagecontroller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Automattic_WooCommerce_Admin_PageController {
