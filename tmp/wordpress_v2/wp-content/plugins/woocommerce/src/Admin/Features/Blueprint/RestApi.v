...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_RestApi) export(var_request rt.PhpVal) rt.PhpVal {
	mut var_payload := rt.call_method(var_request, 'get_param', [
		rt.new_string('steps')])
	mut var_steps := this.steps_payload_to_blueprint_steps(var_payload.clone())
	...
	if var_payload.array_isset(rt.new_string('plugins')) {
		closure_3_fn := fn [var_payload] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			closure_3_fn := fn [var_payload] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
				mut var_plugins := if args.len > 0 { args[0].clone() } else { rt.new_null() }
				return rt.call_function('array_intersect_key', [
					var_plugins.clone(),
					rt.call_function('array_flip', [
						var_payload.array_get(rt.new_string('plugins')),
					])])
			}
		...
		closure_5_fn := fn [var_payload] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			closure_5_fn := fn [var_payload] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
				...
				return rt.call_function('array_intersect_key', [
					var_plugins.clone(),
					rt.call_function('array_flip', [
						var_payload.array_get(rt.new_string('themes')),
					])])
			}
	...
	mut var_data := var_exporter.export(var_steps.clone())
	if rt.is_true(rt.call_function('is_wp_error', [var_data.clone()])) {
		return rt.new_object('Automattic_WooCommerce_Admin_Features_Blueprint_WP_REST_Response',
			[]string{}, create_automattic_woocommerce_admin_features_blueprint_wp_rest_response(var_data.clone(),
			rt.new_int(400)))
	}
	return rt.new_object('Automattic_WooCommerce_Admin_Features_Blueprint_WP_HTTP_Response',
		[]string{}, create_automattic_woocommerce_admin_features_blueprint_wp_http_response(rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_RestApi) steps_payload_to_blueprint_steps(var_steps rt.PhpVal) rt.PhpVal {
	...
	if var_steps_mutated.array_isset(rt.new_string('settings'))
		&& var_steps_mutated.array_get(rt.new_string('settings')).array_count() > 0 {
		var_blueprint_steps = rt.call_function('array_merge', [
			var_blueprint_steps.clone(), var_steps_mutated.array_get(rt.new_string('settings'))])
	}
	if var_steps_mutated.array_isset(rt.new_string('plugins'))
		&& var_steps_mutated.array_get(rt.new_string('plugins')).array_count() > 0 {
		var_blueprint_steps.array_push('installPlugin')
	}
	if var_steps_mutated.array_isset(rt.new_string('themes'))
		&& var_steps_mutated.array_get(rt.new_string('themes')).array_count() > 0 {
		var_blueprint_steps.array_push('installTheme')
	}
	return var_blueprint_steps.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_RestApi) import_step(mut var_request Class_Automattic_WooCommerce_Admin_Features_Blueprint_WP_REST_Request) rt.PhpVal {
	...
	if !(this.can_import_blueprint(var_session_token.clone())) {
	...
	if rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('get_transient', [
		rt.new_string('blueprint_import_session_' + var_session_token.str()),
	])))
	{
	...
	mut var_body_size := rt.new_int(var_request.get_body().to_string().len)
	...
	mut var_step_importer :=
		create_automattic_woocommerce_blueprint_importstep(var_step_definition.clone())
	...
	var_response.header(rt.new_string('X-Blueprint-Import-Session'), var_session_token.clone())
	return rt.new_object('Automattic_WooCommerce_Admin_Features_Blueprint_WP_REST_Response',
		[]string{}, var_response)
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_RestApi) can_import_blueprint(var_session_token rt.PhpVal) bool {
	...
	if rt.is_true(var_session_token_mutated)
		&& rt.is_true(rt.call_function('get_transient', [rt.new_string('blueprint_import_session_' + var_session_token_mutated.str())])) {
		return true
	}
	if rt.is_true(rt.call_function('defined', [rt.new_string('ALLOW_BLUEPRINT_IMPORT_IN_LIVE_MODE')]))
...
fn create_automattic_woocommerce_admin_features_blueprint_wp_rest_response(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_Blueprint_WP_REST_Response {
...
fn create_automattic_woocommerce_admin_features_blueprint_wp_http_response(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_Blueprint_WP_HTTP_Response {
