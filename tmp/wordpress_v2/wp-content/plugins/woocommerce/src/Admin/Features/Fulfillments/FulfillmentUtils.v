	...
	if !(!rt.is_true(var_items_in_fulfillments)) {
		...
		for {
			...
			if var_items_in_fulfillments.array_isset(var_item_id) {
				var_item.array_set('qty', rt.sub(var_item.array_get(rt.new_string('qty')), var_items_in_fulfillments.array_get(var_item_id)))
			}
		}
	}
	closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_item := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return rt.greater(var_item.array_get(rt.new_string('qty')), rt.new_int(0))
		return rt.new_null()
		}
	return rt.call_function('array_filter', [var_order_items.clone(), rt.new_closure(closure_3_fn)])
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.get_refunded_items(mut var_order Class_WC_Order) rt.PhpVal {
	...
	mut iter_2 := var_order.get_items().iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_item := item_2.val
		var_items_refunded.array_set(rt.call_method(var_item, 'get_id', []rt.PhpVal{}), rt.mul(-1, var_order.get_qty_refunded_for_item(rt.call_method(var_item, 'get_id', []rt.PhpVal{}))))
	}
	closure_4_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_qty := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return rt.greater(var_qty, rt.new_int(0))
		return rt.new_null()
		}
	return rt.call_function('array_filter', [var_items_refunded.clone(), rt.new_closure(closure_4_fn)])
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.get_fulfillment_items(mut var_order Class_WC_Order, mut var_fulfillment Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) rt.PhpVal {
	...
	closure_5_fn := fn [var_order] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_item := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return rt.create_array([rt.ArrayItem{ key: 'item_id', val: rt.call_method(var_item, 'get_id', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'item', val: var_item }, rt.ArrayItem{ key: 'qty', val: rt.sub(rt.call_method(var_item, 'get_quantity', []rt.PhpVal{}), var_order.get_qty_refunded_for_item(var_item.clone())) }])
		}
	...
	closure_7_fn := fn [var_fulfillment_items] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_item := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		var_item.array_set('qty', var_fulfillment_items.array_get(var_item.array_get(rt.new_string('item_id'))))
		return var_item.clone()
		}
	closure_8_fn := fn [var_fulfillment_items] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_item := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return rt.new_bool(var_fulfillment_items.array_isset(var_item.array_get(rt.new_string('item_id'))))
		}
	...
	return rt.call_function('array_map', [rt.new_closure(closure_7_fn), rt.call_function('array_filter', [var_order_items.clone(), rt.new_closure(closure_8_fn)])])
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.calculate_order_fulfillment_status(mut var_order Class_WC_Order, var_fulfillments rt.PhpVal) string {
	mut var_has_fulfillments := rt.new_bool(!(!rt.is_true(var_fulfillments)))
	if rt.is_true(var_has_fulfillments) {
		...
		mut var_all_fulfilled := rt.new_bool(true)
		mut var_some_fulfilled := rt.new_bool(false)
		mut iter_3 := var_fulfillments.iterator()
		for {
			...
			var_all_fulfilled = rt.new_bool(false)
			} else {
			var_some_fulfilled = rt.new_bool(true)
			}
		}
		if rt.is_true(var_all_fulfilled) && !rt.is_true(var_pending_items) {
		mut var_status := rt.new_string('fulfilled')
		} else if rt.is_true(var_some_fulfilled) {
		var_status = rt.new_string('partially_fulfilled')
		} else {
		var_status = rt.new_string('unfulfilled')
		}
	} else {
	var_status = rt.new_string('no_fulfillments')
	}
	return (rt.call_function('apply_filters', [rt.new_string('woocommerce_fulfillment_calculate_order_fulfillment_status'), var_status.clone(), var_order, var_fulfillments.clone()])).str()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.get_all_items_of_fulfillments(mut var_fulfillments Class_Automattic_WooCommerce_Admin_Features_Fulfillments_array) rt.PhpVal {
	...
	for {
		...
		mut iter_5 := var_fulfillment_items.iterator()
		for {
			item_5 := iter_5.next() or { break }
			mut var_item := item_5.val
			if !(var_items.array_isset(var_item.array_get(rt.new_string('item_id')))) {
				var_items.array_set(var_item.array_get(rt.new_string('item_id')), 0)
			}
			var_items.array_get(var_item.array_get(rt.new_string('item_id'))) = rt.add(var_items.array_get(var_item.array_get(rt.new_string('item_id'))), var_item.array_get(rt.new_string('qty')))
		}
	}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.get_tracking_info_html(mut var_fulfillment Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) string {
	mut var_tracking_html := rt.new_string('')
	mut var_tracking_url := var_fulfillment.get_tracking_url()
	mut var_tracking_number := var_fulfillment.get_tracking_number()
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_tracking_url)))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_tracking_number)))) {
		var_tracking_html = rt.concat(var_tracking_html, rt.new_string('<a href="' + (rt.call_function('esc_url', [var_tracking_url.clone()])).str() + '" target="_blank" rel="noopener noreferrer">'))
		var_tracking_html = rt.concat(var_tracking_html, rt.call_function('esc_html', [var_tracking_number.clone()]))
		var_tracking_html = rt.concat(var_tracking_html, rt.new_string('</a>'))
	} else if !(!rt.is_true(var_tracking_number)) {
		var_tracking_html = rt.concat(var_tracking_html, rt.call_function('esc_html', [var_tracking_number.clone()]))
	} else {
		var_tracking_html = rt.concat(var_tracking_html, rt.new_string('<span class="no-tracking">' + (rt.call_function('esc_html__', [rt.new_string('No tracking number available'), rt.new_string('woocommerce')])).str() + '</span>'))
	}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.get_order_fulfillment_status_text(mut var_order Class_WC_Order) string {
	...
	mut var_fulfillment_status_text := rt.new_string('')
	...
	return (rt.call_function('apply_filters', [rt.new_string('woocommerce_fulfillment_order_fulfillment_status_text'), var_fulfillment_status_text.clone(), var_fulfillment_status.clone(), var_order])).str()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.is_valid_order_fulfillment_status(mut var_status Class_Automattic_WooCommerce_Admin_Features_Fulfillments_?string) bool {
	...
	if rt.is_true(rt.new_bool(var_status_mutated.is_null())) {
		return false
	}
	...
	return (rt.call_function('in_array', [var_status_mutated, rt.func_array_keys(var_order_fulfillment_statuses.clone()), rt.new_bool(true)])).to_bool()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.is_valid_fulfillment_status(mut var_status Class_Automattic_WooCommerce_Admin_Features_Fulfillments_?string) bool {
	...
	return (rt.call_function('in_array', [var_status_mutated, rt.func_array_keys(var_fulfillment_statuses.clone()), rt.new_bool(true)])).to_bool()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.get_shipping_providers() rt.PhpVal {
	...
	if !(var_raw_providers.clone().is_array()) {
		return rt.new_array()
	}
	mut var_resolved := rt.new_array()
	mut iter_6 := var_raw_providers.iterator()
	for {
		...
		if rt.is_true(rt.new_bool(rt.instance_of(var_provider, 'Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_AbstractShippingProvider'))) {
			var_resolved.array_set(rt.call_method(var_provider, 'get_key', []rt.PhpVal{}), var_provider.clone())
		} else if var_provider.clone().is_string() && rt.is_true(rt.call_function('class_exists', [var_provider.clone()])) && rt.is_true(rt.call_function('is_subclass_of', [var_provider.clone(), Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_AbstractShippingProvider.class()])) {
			mut var_instance := rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [var_provider.clone()])
...
end_label_1:
			var_resolved.array_set(rt.call_method(var_instance, 'get_key', []rt.PhpVal{}), var_instance.clone())
		}
	}
	return var_resolved.clone()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.get_default_fulfillment_statuses() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: 'fulfilled', val: rt.create_array([rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Fulfilled'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'is_fulfilled', val: true }, rt.ArrayItem{ key: 'background_color', val: '#C6E1C6' }, rt.ArrayItem{ key: 'text_color', val: '#13550F' }]) }, rt.ArrayItem{ key: 'unfulfilled', val: rt.create_array([rt.ArrayItem{ key: 'label', val: rt.call_function('__', [rt.new_string('Unfulfilled'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'is_fulfilled', val: false }, rt.ArrayItem{ key: 'background_color', val: '#FBE5E5' }, rt.ArrayItem{ key: 'text_color', val: '#CC1818' }]) }])
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.check_s10_upu_format(tracking_number string) bool {
	...
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^[A-Z]{2}\\d{9}[A-Z]{2}$/'), rt.new_string(tracking_number_mutated).clone()])) {
	tracking_number_mutated = (rt.call_function('substr', [rt.new_string(tracking_number_mutated).clone(), rt.new_int(2), rt.new_int(-2)])).str()
	} else if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('/^\\d{9}$/'), rt.new_string(tracking_number_mutated).clone()]))))) {
		return false
	}
	mut var_weights := rt.create_array([rt.ArrayItem{ key: none, val: 8 }, rt.ArrayItem{ key: none, val: 6 }, rt.ArrayItem{ key: none, val: 4 }, rt.ArrayItem{ key: none, val: 2 }, rt.ArrayItem{ key: none, val: 3 }, rt.ArrayItem{ key: none, val: 5 }, rt.ArrayItem{ key: none, val: 9 }, rt.ArrayItem{ key: none, val: 7 }])
	mut var_sum := rt.new_int(0)
	mut var_i := rt.new_int(0)
	for {
		if !(rt.is_true(rt.less(var_i, rt.new_int(8)))) { break }
		var_sum = rt.add(var_sum, rt.mul(var_weights.array_get(var_i), rt.new_int((rt.new_string(tracking_number_mutated).array_get(var_i)).to_i64())))
		rt.post_inc(var_i)
	}
	mut var_check_digit := rt.sub(rt.new_int(11), rt.mod_(var_sum, rt.new_int(11)))
	if rt.is_true(rt.identical(rt.new_int(10), var_check_digit)) {
	var_check_digit = rt.new_int(0)
	} else if rt.is_true(rt.identical(rt.new_int(11), var_check_digit)) {
	var_check_digit = rt.new_int(5)
	}
	return (rt.identical(rt.new_int((rt.new_string(tracking_number_mutated).array_get(rt.new_int(8))).to_i64()), var_check_digit)).to_bool()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.validate_ups_1z_check_digit(tracking_number string) bool {
	mut tracking_number_mutated := tracking_number
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('/^1Z[0-9A-Z]{15,16}$/'), rt.new_string(tracking_number_mutated).clone()]))))) {
		return false
	}
	mut var_trackable := rt.call_function('substr', [rt.new_string(tracking_number_mutated).clone(), rt.new_int(2)])
	mut var_check_digit := rt.new_int((rt.call_function('substr', [var_trackable.clone(), rt.new_int(-1)])).to_i64())
	var_trackable = rt.call_function('substr', [var_trackable.clone(), rt.new_int(0), rt.new_int(-1)])
	mut var_sum := rt.new_int(0)
	mut var_odd_position := rt.new_bool(true)
	mut var_i := rt.new_int(var_trackable.clone().to_string().len - 1)
	for {
		if !(rt.is_true(rt.greater_equal(var_i, rt.new_int(0)))) { break }
		mut var_char := var_trackable.array_get(var_i)
		mut var_value := if var_char.clone().is_long() || var_char.clone().is_double() { rt.new_int((var_char).to_i64()) } else { rt.sub(rt.call_function('ord', [var_char.clone()]), rt.new_int(55)) }
		if rt.is_true(var_odd_position) {
			var_value = rt.mul(var_value, rt.new_int(2))
			if rt.is_true(rt.greater(var_value, rt.new_int(9))) {
			var_value = rt.add(rt.new_int((rt.div(var_value, rt.new_int(10))).to_i64()), rt.mod_(var_value, rt.new_int(10)))
			}
		}
		var_sum = rt.add(var_sum, var_value)
		var_odd_position = rt.new_bool(!(rt.is_true(var_odd_position)))
		rt.post_dec(var_i)
	}
	mut var_calculated_check := rt.mod_(rt.sub(rt.new_int(10), rt.mod_(var_sum, rt.new_int(10))), rt.new_int(10))
	return (rt.identical(var_calculated_check, var_check_digit)).to_bool()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.validate_mod7_check_digit(tracking_number string) bool {
	mut tracking_number_mutated := tracking_number
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('/^\\d+$/'), rt.new_string(tracking_number_mutated).clone()]))))) || tracking_number_mutated.len < 2 {
		return false
	}
	mut var_check_digit := rt.new_int((rt.call_function('substr', [rt.new_string(tracking_number_mutated).clone(), rt.new_int(-1)])).to_i64())
	mut var_number := rt.call_function('substr', [rt.new_string(tracking_number_mutated).clone(), rt.new_int(0), rt.new_int(-1)])
	mut var_sum := rt.new_int(0)
	mut var_weights := rt.create_array([rt.ArrayItem{ key: none, val: 3 }, rt.ArrayItem{ key: none, val: 1 }, rt.ArrayItem{ key: none, val: 3 }, rt.ArrayItem{ key: none, val: 1 }, rt.ArrayItem{ key: none, val: 3 }, rt.ArrayItem{ key: none, val: 1 }, rt.ArrayItem{ key: none, val: 3 }])
	mut var_weight_index := rt.new_int(0)
	mut var_i := rt.new_int(var_number.clone().to_string().len - 1)
	for {
		if !(rt.is_true(rt.greater_equal(var_i, rt.new_int(0)))) { break }
		mut var_digit := rt.new_int((var_number.array_get(var_i)).to_i64())
		var_sum = rt.add(var_sum, rt.mul(var_digit, var_weights.array_get(rt.mod_(var_weight_index, rt.new_int(var_weights.clone().array_count())))))
		rt.pre_inc(var_weight_index)
		rt.post_dec(var_i)
	}
	mut var_calculated_check := rt.mod_(var_sum, rt.new_int(7))
	if rt.is_true(rt.identical(rt.new_int(0), var_calculated_check)) {
	var_calculated_check = rt.new_int(7)
	}
	return (rt.identical(var_calculated_check, var_check_digit)).to_bool()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.validate_mod10_check_digit(tracking_number string) bool {
	...
	for {
		...
		mut var_digit := rt.new_int((var_number.array_get(var_i)).to_i64())
		if rt.is_true(var_odd_position) {
			var_digit = rt.mul(var_digit, rt.new_int(2))
			if rt.is_true(rt.greater(var_digit, rt.new_int(9))) {
			var_digit = rt.add(rt.new_int((rt.div(var_digit, rt.new_int(10))).to_i64()), rt.mod_(var_digit, rt.new_int(10)))
			}
		}
		var_sum = rt.add(var_sum, var_digit)
		var_odd_position = rt.new_bool(!(rt.is_true(var_odd_position)))
		rt.post_dec(var_i)
	}
	mut var_calculated_check := rt.mod_(rt.sub(rt.new_int(10), rt.mod_(var_sum, rt.new_int(10))), rt.new_int(10))
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.validate_mod11_check_digit(tracking_number string) bool {
	...
	mut var_check_digit := rt.new_int((rt.call_function('substr', [rt.new_string(tracking_number_mutated).clone(), rt.new_int(-1)])).to_i64())
	mut var_number := rt.call_function('substr', [rt.new_string(tracking_number_mutated).clone(), rt.new_int(0), rt.new_int(-1)])
	mut var_weights := rt.create_array([rt.ArrayItem{ key: none, val: 2 }, rt.ArrayItem{ key: none, val: 3 }, rt.ArrayItem{ key: none, val: 4 }, rt.ArrayItem{ key: none, val: 5 }, rt.ArrayItem{ key: none, val: 6 }, rt.ArrayItem{ key: none, val: 7 }, rt.ArrayItem{ key: none, val: 8 }, rt.ArrayItem{ key: none, val: 9 }, rt.ArrayItem{ key: none, val: 10 }, rt.ArrayItem{ key: none, val: 11 }])
	mut var_sum := rt.new_int(0)
	mut var_weight_index := rt.new_int(0)
	mut var_i := rt.new_int(var_number.clone().to_string().len - 1)
	for {
	...
	mut var_calculated_check := rt.sub(rt.new_int(11), rt.mod_(var_sum, rt.new_int(11)))
	if rt.is_true(rt.identical(rt.new_int(10), var_calculated_check)) {
	var_calculated_check = rt.new_int(0)
	} else if rt.is_true(rt.identical(rt.new_int(11), var_calculated_check)) {
	var_calculated_check = rt.new_int(5)
	}
	return (rt.identical(var_calculated_check, var_check_digit)).to_bool()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.validate_fedex_check_digit(tracking_number string) bool {
	mut tracking_number_mutated := tracking_number
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [rt.new_string('/^\\d{12}$/'), rt.new_string(tracking_number_mutated).clone()]))))) {
		return false
	}
	mut var_digits := rt.call_function('str_split', [rt.call_function('substr', [rt.new_string(tracking_number_mutated).clone(), rt.new_int(0), rt.new_int(11)])])
	mut var_multipliers := rt.create_array([rt.ArrayItem{ key: none, val: 3 }, rt.ArrayItem{ key: none, val: 1 }, rt.ArrayItem{ key: none, val: 7 }])
	mut var_sum := rt.new_int(0)
	mut var_multiplier_index := rt.new_int(0)
	mut var_i := rt.new_int(10)
	for {
		if !(rt.is_true(rt.greater_equal(var_i, rt.new_int(0)))) { break }
		var_sum = rt.add(var_sum, rt.mul(var_digits.array_get(var_i), var_multipliers.array_get(var_multiplier_index)))
		var_multiplier_index = rt.mod_(rt.pre_inc(var_multiplier_index), rt.new_int(3))
		rt.post_dec(var_i)
	}
	mut var_check := rt.mod_(var_sum, rt.new_int(11))
	if rt.is_true(rt.identical(rt.new_int(10), var_check)) {
	var_check = rt.new_int(0)
	}
	return (rt.identical(rt.new_int(rt.new_string(tracking_number_mutated).array_get(rt.new_int(11)).to_i64()), var_check)).to_bool()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils.resolve_provider_name(mut var_fulfillment Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) string {
	mut var_shipment_provider := if !(var_fulfillment.get_shipment_provider()).is_null() { var_fulfillment.get_shipment_provider() } else { rt.new_string('') }
	mut var_provider_name := var_fulfillment.get_meta(rt.new_string('_provider_name'), rt.new_bool(true))
	var_provider_name = rt.new_string((if !(!rt.is_true(var_provider_name)) { (var_provider_name).str() } else { '' }).str())
	if rt.is_true(rt.identical(rt.new_string('other'), var_shipment_provider)) {
		return (var_provider_name).str()
	}
	return (if !(!rt.is_true(var_shipment_provider)) { var_shipment_provider } else { var_provider_name }).str()
}
