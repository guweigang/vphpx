...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Init) get_woo_exporters() rt.PhpVal {
	...
	for {
		...
		this.initialized_exporters.array_set(var_classname, var_exporters.array_get(var_classname))
	}
	return rt.call_function('array_values', [var_exporters.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Init) add_woo_exporters(mut var_exporters Class_Automattic_WooCommerce_Admin_Features_Blueprint_array) rt.PhpVal {
	...
	return rt.call_function('array_merge', [var_exporters_mutated, this.get_woo_exporters()])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Init) get_plugins_for_export_group() rt.PhpVal {
	...
	closure_2_fn := fn [var_active_plugins] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_plugin := if args.len > 1 { args[1].clone() } else { rt.new_null() }
		return rt.create_array([rt.ArrayItem{ key: 'id', val: var_key },
			...
			rt.ArrayItem{ key: 'checked', val: rt.call_function('in_array', [
				var_key.clone(), var_active_plugins.clone(), rt.new_bool(true)]) }])
	}
	...
	var_plugins = rt.call_function('array_map', [rt.new_closure(closure_2_fn),
		rt.func_array_keys(var_plugins.clone()), var_plugins.clone()])
	closure_4_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_a := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_b := if args.len > 1 { args[1].clone() } else { rt.new_null() }
		return rt.new_null()
	}
	rt.call_function('usort', [var_plugins.clone(), rt.new_closure(closure_4_fn)])
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Init) get_themes_for_export_group() rt.PhpVal {
	...
	rt.call_function('usort', [var_themes.clone(), rt.new_closure(closure_7_fn)])
	return rt.call_function('array_values', [var_themes.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Init) add_js_vars(var_settings rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_string('woocommerce_page_wc-settings-advanced-blueprint'), rt.call_method(iife_result_9,
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Init) get_installed_wp_org_plugins() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_wp_org_plugins.clone().is_array())) {
		return var_wp_org_plugins.clone()
	}
	mut var_all_plugins := this.wp_get_plugins()
	mut var_plugin_slugs := rt.new_array()
	mut iter_2 := var_all_plugins.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_plugin := item_2.val
		mut var_key := item_2.key
		mut var_slug := rt.call_function('dirname', [var_key.clone()])
		var_slug = rt.call_function('apply_filters', [
			rt.new_string('wp_plugin_dependencies_slug'),
			var_slug.clone(),
		])
		var_plugin_slugs.array_push(var_slug.clone())
		var_all_plugins.array_get_mut(var_key).array_set('slug', var_slug.clone())
	}
	...
	closure_11_fn := fn [var_api_response] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_slug := var_plugin.array_get(rt.new_string('slug'))
		return rt.new_bool(
	...
	var_wp_org_plugins = rt.call_function('array_filter', [var_all_plugins.clone(),
		rt.new_closure(closure_11_fn)])
	rt.call_function('set_transient', [
		Class_Automattic_WooCommerce_Admin_Features_Blueprint_Automattic_WooCommerce_Admin_Features_Blueprint_Init.installed_wp_org_plugins_transient(),
		var_wp_org_plugins.clone(),
	])
	return var_wp_org_plugins.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Init) get_installed_wp_org_themes() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_wp_org_themes.clone().is_array())) {
		return var_wp_org_themes.clone()
	}
	mut var_all_themes := this.wp_get_themes()
	mut var_theme_slugs := rt.new_array()
	mut iter_3 := var_all_themes.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_theme := item_3.val
		mut var_key := item_3.key
		if rt.is_true(rt.new_bool(var_key.clone().is_string())) {
			var_theme_slugs.array_push(var_key.clone().to_string().to_lower())
		}
	}
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_api_response.clone()])) {
		return var_all_themes.clone()
	}
	closure_12_fn := fn [var_api_response] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_theme := if args.len > 0 { args[0].clone() } else { rt.new_null() }
	...
	var_wp_org_themes = rt.call_function('array_filter', [var_all_themes.clone(),
		rt.new_closure(closure_12_fn)])
	rt.call_function('set_transient', [
		Class_Automattic_WooCommerce_Admin_Features_Blueprint_Automattic_WooCommerce_Admin_Features_Blueprint_Init.installed_wp_org_themes_transient(),
		var_wp_org_themes.clone(),
	])
	return var_wp_org_themes.clone()
}
...
fn create_automattic_woocommerce_admin_features_blueprint_restapi(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_Blueprint_RestApi {
