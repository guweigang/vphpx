...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentsSettings) add_auto_fulfill_settings(mut var_settings Class_Automattic_WooCommerce_Admin_Features_Fulfillments_array, var_current_section rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		if var_setting.array_isset(rt.new_string('type')) && var_setting.array_isset(rt.new_string('id')) && rt.is_true(rt.identical(rt.new_string('sectionend'), var_setting.array_get(rt.new_string('type')))) && rt.is_true(rt.identical(rt.new_string('catalog_options'), var_setting.array_get(rt.new_string('id')))) {
			var_insertion_index = rt.add(var_index, rt.new_int(1))
			break
		}
	}
	if rt.is_true(rt.new_bool(var_insertion_index.clone().is_null())) {
		return rt.new_object('Automattic_WooCommerce_Admin_Features_Fulfillments_array', []string{}, var_settings)
	}
	...
	rt.call_function('array_splice', [var_settings, var_insertion_index.clone(), rt.new_int(0), var_auto_fulfill_settings.clone()])
	return rt.new_object('Automattic_WooCommerce_Admin_Features_Fulfillments_array', []string{}, var_settings)
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentsSettings) auto_fulfill_items_on_processing(order_id i64, var_order rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order_mutated)))) || !rt.is_true(rt.call_method(var_order_mutated, 'get_items', []rt.PhpVal{})) {
		return
	}
	...
	mut var_auto_fulfill_product_ids := rt.call_function('apply_filters', [rt.new_string('woocommerce_fulfillments_auto_fulfill_products'), rt.new_array(), var_order_mutated.clone()])
	mut var_auto_fulfill_items := rt.new_array()
	mut iter_2 := rt.call_method(var_order_mutated, 'get_items', []rt.PhpVal{}).iterator()
	for {
	...
	if !(!rt.is_true(var_auto_fulfill_items)) {
		...
		var_fulfillment.set_entity_id(rt.new_string(order_id.str()))
		...
		var_fulfillment.set_items(rt.call_function('array_map', [rt.new_closure(closure_1_fn), var_auto_fulfill_items.clone()]))
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentsSettings) auto_fulfill_items_on_completed(order_id i64, var_order rt.PhpVal) {
	...
	mut iife_temp_2 := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Data_Store{}
	mut iife_result_2 := iife_temp_2.load(rt.new_string('order-fulfillment'))
	mut var_fulfillment_data_store := iife_result_2
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	mut var_fulfillments := rt.call_method(var_fulfillment_data_store, 'read_fulfillments', [Class_WC_Order.class(), rt.new_string(order_id.str())])
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
end_label_1:
	this.auto_fulfill_items_on_processing(order_id, var_order_mutated.clone())
}
...
fn create_automattic_woocommerce_admin_features_fulfillments_fulfillmentssettings(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentsSettings {
