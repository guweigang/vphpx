...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) construct(data string) {
	...
	} else if rt.new_string(data).is_array() && rt.new_string(data).array_isset(rt.new_string('id')) {
		this.set_id(rt.call_function('absint', [rt.new_string(data).array_get(rt.new_string('id'))]))
	} else if rt.new_string(data).is_string() && !(data == '') {
		this.set_id(rt.call_function('absint', [rt.new_string(data)]))
	} else if rt.new_string(data).is_object() && !(rt.get_property(rt.new_string(data), 'id')).is_null() {
		this.set_id(rt.call_function('absint', [rt.get_property(rt.new_string(data), 'id')]))
	} else {
		this.set_object_read(rt.new_bool(true))
	}
	mut iife_temp_0 := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Data_Store{}
	mut iife_result_0 := iife_temp_0.load(rt.new_string('order-fulfillment'))
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) get_changes() rt.PhpVal {
	...
	mut iter_2 := this.get_meta_data().iterator()
	for {
	...
	mut iter_3 := var_current_meta.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_value := item_3.val
		mut var_key := item_3.key
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.meta_snapshot.array_isset(var_key.clone())))))) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(this.meta_snapshot.array_get(var_key), var_value)))) {
			var_meta_changes.array_set(var_key, var_value.clone())
		}
	}
	mut iter_4 := this.meta_snapshot.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_value := item_4.val
		mut var_key := item_4.key
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_current_meta.clone().array_isset(var_key.clone())))))) {
			var_meta_changes.array_set(var_key, rt.new_null())
		}
	}
	if !(!rt.is_true(var_meta_changes)) {
		var_changes.array_set('meta_data', var_meta_changes.clone())
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) get_id() i64 {
	return (if !(this.data.array_get(rt.new_string('id'))).is_null() { this.data.array_get(rt.new_string('id')) } else { rt.new_int(0) }).to_i64()
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) set_id(var_id rt.PhpVal) {
	this.data.array_set('id', if var_id.clone().is_long() || var_id.clone().is_double() { rt.call_function('absint', [var_id.clone()]) } else { rt.new_int(0) })
	this.Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Data.set_id(this.data.array_get(rt.new_string('id')))
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) set_status(mut var_status Class_Automattic_WooCommerce_Admin_Features_Fulfillments_?string) {
	...
	mut iife_result_1 := iife_temp_1.get_fulfillment_statuses()
	mut var_statuses := iife_result_1
	if !(var_statuses.array_isset(var_status_mutated)) {
	var_status_mutated = rt.new_string((if this.get_is_fulfilled() { 'fulfilled' } else { 'unfulfilled' }).str())
	}
	this.set_is_fulfilled((if !(var_statuses.array_get(var_status_mutated).array_get(rt.new_string('is_fulfilled'))).is_null() { var_statuses.array_get(var_status_mutated).array_get(rt.new_string('is_fulfilled')) } else { rt.new_bool(false) }).to_bool())
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) get_is_fulfilled() bool {
	return (this.get_prop(rt.new_string('is_fulfilled'))).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) set_items(mut var_items Class_Automattic_WooCommerce_Admin_Features_Fulfillments_array) {
	...
	this.update_meta_data(rt.new_string('_items'), rt.call_function('array_values', [var_items_mutated]))
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) get_item_count() i64 {
	closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return (rt.add(var_carry, rt.new_int((var_item.array_get(rt.new_string('qty'))).to_i64()))).to_i64()
		}
	return (rt.call_function('array_reduce', [this.get_items(), rt.new_closure(closure_3_fn), rt.new_int(0)])).to_i64()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) get_tracking_number() string {
	...
	return (if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_value)))) { var_value } else { rt.new_null() }).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) get_order() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_entity_type)))) || rt.is_true(rt.new_bool(!(rt.is_true(var_entity_id)))) {
		return rt.new_null()
	}
	if rt.is_true(rt.identical(Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Order.class(), var_entity_type)) {
		mut var_order := rt.call_function('wc_get_order', [rt.new_int((var_entity_id).to_i64())])
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Fulfillment) get_raw_meta_data() rt.PhpVal {
	...
	return rt.call_function('array_map', [rt.new_closure(closure_4_fn), this.get_meta_data()])
}
...
fn create_automattic_woocommerce_admin_features_fulfillments_wc_data(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_Fulfillments_WC_Data {
