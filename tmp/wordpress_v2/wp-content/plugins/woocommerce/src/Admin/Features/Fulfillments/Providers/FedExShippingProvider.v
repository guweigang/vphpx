...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_FedExShippingProvider) get_tracking_url(tracking_number string) string {
	...
	return 'https://www.fedex.com/fedextrack/?tracknumbers=' + (rt.call_function('rawurlencode', [rt.new_string(tracking_number_mutated).clone()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_FedExShippingProvider) try_parse_tracking_number(tracking_number string, shipping_from string, shipping_to string) rt.PhpVal {
	...
	mut var_is_us_domestic := rt.new_bool(rt.is_true(rt.identical(rt.new_string('US'), rt.new_string(shipping_from))) && rt.is_true(rt.identical(rt.new_string('US'), rt.new_string(shipping_to))))
	closure_2_fn := fn [var_tracking_number, var_is_north_america, var_is_us_domestic] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut iife_temp_1 := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils{}
		mut iife_result_1 := iife_temp_1.validate_fedex_check_digit(rt.new_string(tracking_number_mutated))
		if rt.is_true(iife_result_1) {
			return rt.new_int(if rt.is_true(var_is_north_america) || rt.is_true(var_is_us_domestic) { 98 } else { 85 })
		}
		return rt.new_int(if rt.is_true(var_is_north_america) { if rt.is_true(var_is_us_domestic) { 98 } else { 85 } } else { 70 })
		return rt.new_null()
		}
	closure_4_fn := fn [var_tracking_number, var_is_north_america] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut iife_temp_3 := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils{}
		mut iife_result_3 := iife_temp_3.validate_fedex_check_digit(rt.new_string(tracking_number_mutated))
	...
	closure_6_fn := fn [var_tracking_number, var_is_north_america] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut iife_temp_5 := Class_Automattic_WooCommerce_Admin_Features_Fulfillments_FulfillmentUtils{}
		mut iife_result_5 := iife_temp_5.validate_fedex_check_digit(rt.new_string(tracking_number_mutated))
	...
	mut var_patterns := rt.create_array([rt.ArrayItem{ key: '/^DT\\d{12}$/', val: if rt.is_true(var_is_north_america) { 90 } else { 0 } }, rt.ArrayItem{ key: '/^0[01]\\d{13,23}$/', val: 98 }, rt.ArrayItem{ key: '/^023\\d{17}$/', val: 97 }, rt.ArrayItem{ key: '/^58\\d{17,19}$/', val: 96 }, rt.ArrayItem{ key: '/^\\d{12}$/', val: rt.new_closure(closure_2_fn) }, rt.ArrayItem{ key: '/^\\d{15}$/', val: rt.new_closure(closure_4_fn) }, rt.ArrayItem{ key: '/^\\d{14}$/', val: rt.new_closure(closure_6_fn) }, rt.ArrayItem{ key: '/^\\d{34}$/', val: 90 }, rt.ArrayItem{ key: '/^96\\d{18,20}$/', val: if rt.is_true(var_is_north_america) { 95 } else { 60 } }, rt.ArrayItem{ key: '/^7\\d{11,20}$/', val: if rt.is_true(var_is_north_america) { 90 } else { 75 } }, rt.ArrayItem{ key: '/^97\\d{13,23}$/', val: 93 }, rt.ArrayItem{ key: '/^3\\d{10,14}$/', val: 92 }, rt.ArrayItem{ key: '/^8\\d{8,14}$/', val: rt.new_closure(closure_7_fn) }, rt.ArrayItem{ key: '/^NFO\\d{10,15}$/', val: 92 }, rt.ArrayItem{ key: '/^SD\\d{10,15}$/', val: 90 }, rt.ArrayItem{ key: '/^\\d{20}$/', val: 70 }, rt.ArrayItem{ key: '/^\\d{22}$/', val: 65 }])
...
fn create_automattic_woocommerce_admin_features_fulfillments_providers_fedexshippingprovider(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_FedExShippingProvider {
