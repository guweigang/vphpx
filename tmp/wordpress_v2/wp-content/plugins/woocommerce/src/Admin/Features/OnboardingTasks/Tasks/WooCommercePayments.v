...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments) get_badge() rt.PhpVal {
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_admin_woopayments_onboarding_task_badge'),
		rt.new_string(''),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments) get_content() rt.PhpVal {
	return rt.call_function('__', [
		rt.new_string("You're only one step away from getting paid. Verify your business details to start managing transactions with WooPayments."),
		rt.new_string('woocommerce'),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments) get_additional_data() rt.PhpVal {
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_admin_woopayments_onboarding_task_additional_data'),
		rt.new_null(),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments) get_action_label() rt.PhpVal {
	return rt.call_function('__', [rt.new_string('Finish setup'),
		rt.new_string('woocommerce')])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments) is_complete() bool {
	if rt.is_true(rt.identical(rt.new_null(), this.is_complete_result)) {
		this.is_complete_result =
			rt.is_true(Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.has_other_ecommerce_gateways())
			|| rt.is_true(Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.is_connected())
			&& rt.is_true(rt.new_bool(!(rt.is_true(Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.is_account_partially_onboarded()))))
	}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.is_requested() bool {
	...
	mut var_business_extensions := if var_profiler_data.array_isset(rt.new_string('business_extensions')) {
		var_profiler_data.array_get(rt.new_string('business_extensions'))
	} else {
		rt.new_array()
	}
	mut var_subscriptions_and_us := rt.new_bool(
		rt.is_true(rt.call_function('in_array', [rt.new_string('subscriptions'), var_product_types.clone(), rt.new_bool(true)]))
		&& rt.is_true(rt.identical(rt.new_string('US'), rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'get_base_country', []rt.PhpVal{}))))
	return
		rt.is_true(rt.call_function('in_array', [rt.new_string('woocommerce-payments'), var_business_extensions.clone(), rt.new_bool(true)]))
		|| rt.is_true(var_subscriptions_and_us)
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.is_installed() rt.PhpVal {
	...
	return rt.call_function('in_array', [rt.new_string('woocommerce-payments'),
		var_installed_plugins.clone(), rt.new_bool(true)])
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.is_connected() bool {
	...
	mut var_wc_payments_gateway :=
		Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.get_gateway()
	if rt.is_true(var_wc_payments_gateway)
		&& rt.is_true(rt.call_function('method_exists', [var_wc_payments_gateway.clone(), rt.new_string('is_connected')])) {
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.is_account_partially_onboarded() bool {
	...
	mut var_wc_payments_gateway :=
		Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.get_gateway()
	if rt.is_true(var_wc_payments_gateway)
		&& rt.is_true(rt.call_function('method_exists', [var_wc_payments_gateway.clone(), rt.new_string('is_account_partially_onboarded')])) {
		return (rt.call_method(var_wc_payments_gateway, 'is_account_partially_onboarded',
			[]rt.PhpVal{})).to_bool()
	}
	return false
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.get_suggestion() rt.PhpVal {
	...
	mut iife_temp_2 := Class_Automattic_WooCommerce_Admin_Features_PaymentGatewaySuggestions_Init{}
	mut iife_result_2 := iife_temp_2.get_suggestions(iife_result_1)
	mut var_suggestions := iife_result_2
	closure_4_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_suggestion := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		if !rt.is_true(rt.get_property(var_suggestion, 'plugins'))
			|| !(rt.get_property(var_suggestion, 'plugins').is_array()) {
			return rt.new_bool(false)
		}
		return rt.call_function('in_array', [rt.new_string('woocommerce-payments'),
			rt.get_property(var_suggestion, 'plugins'), rt.new_bool(true)])
	}
	mut var_wcpay_suggestions := rt.call_function('array_filter', [
		var_suggestions.clone(), rt.new_closure(closure_4_fn)])
	if !rt.is_true(var_wcpay_suggestions) {
		return rt.new_null()
	}
	return rt.call_function('reset', [var_wcpay_suggestions.clone()])
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.get_gateway() rt.PhpVal {
	...
	if var_payment_gateways.array_isset(rt.new_string('woocommerce_payments')) {
		return var_payment_gateways.array_get(rt.new_string('woocommerce_payments'))
	}
	return rt.new_null()
}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.has_other_ecommerce_gateways() bool {
	...
	closure_5_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return
			...
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [rt.get_property(var_gateway, 'id'), rt.new_string('woocommerce_payments')])))))
	...
	mut var_enabled_gateways := rt.call_function('array_filter', [
		var_gateways.clone(), rt.new_closure(closure_5_fn)])
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments) get_action_url() rt.PhpVal {
	if rt.is_true(Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.is_supported()) {
		...
		mut iife_temp_5 := Class_Automattic_WooCommerce_Internal_Admin_WcPayWelcomePage{}
		mut iife_result_5 := iife_temp_5.instance()
		if rt.is_true(rt.call_method(iife_result_5, 'has_incentive', []rt.PhpVal{})) {
			return rt.call_function('add_query_arg', [rt.new_string('from'),
				rt.new_string('WCADMIN_PAYMENT_TASK'),
				rt.call_function('admin_url', [
					rt.new_string('admin.php?page=wc-admin&path=/wc-pay-welcome-page'),
				])])
		}
		return rt.call_function('add_query_arg', [
			rt.create_array([rt.ArrayItem{ key: 'task', val: this.get_id() },
				rt.ArrayItem{ key: 'id', val: rt.get_property(Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments.get_suggestion(),
					'id') }]),
			rt.call_function('admin_url', [rt.new_string('admin.php?page=wc-admin')]),
		])
	}
	return rt.call_function('add_query_arg', [rt.new_string('task'),
		rt.new_string(this.get_id()),
		rt.call_function('admin_url', [
			rt.new_string('admin.php?page=wc-admin'),
		])])
}
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_tasks_woocommercepayments(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WooCommercePayments {
