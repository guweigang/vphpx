...
fn init_static_automattic_woocommerce_admin_features_onboardingtasks_tasks_products() {
	rt.init_static_prop('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products',
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) construct(var_task_list rt.PhpVal) {
	...
	rt.call_function('add_action', [rt.new_string('admin_enqueue_scripts'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'possibly_add_import_return_notice_script' },
		])])
	rt.call_function('add_action', [rt.new_string('admin_enqueue_scripts'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products', [
			...
			rt.ArrayItem{ key: none, val: 'possibly_add_load_sample_return_notice_script' },
		])])
	rt.call_function('add_action', [rt.new_string('woocommerce_update_product'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'maybe_set_has_product_transient' },
		]),
	...
	rt.call_function('add_action', [rt.new_string('untrashed_post'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'maybe_set_has_product_transient_on_untrashed_post' },
		])])
	rt.call_function('add_action', [rt.new_string('current_screen'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'maybe_redirect_to_add_product_tasklist' },
		]),
		rt.new_int(30), rt.new_int(0)])
	rt.call_function('add_action', [rt.new_string('trashed_post'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products', [
				'Automattic_WooCommerce_Admin_Features_OnboardingTasks_Task',
			], &this) },
			rt.ArrayItem{ key: none, val: 'on_product_trashed' },
		])])
	rt.call_function('add_action', [rt.new_string('deleted_post_product'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products', [
				'Automattic_WooCommerce_Admin_Features_OnboardingTasks_Task',
			], &this) },
			rt.ArrayItem{ key: none, val: 'on_product_deleted' },
		])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) get_title() rt.PhpVal {
	...
	if var_onboarding_profile.array_isset(rt.new_string('business_choice'))
		...
		return rt.call_function('__', [rt.new_string('Import your products'),
			rt.new_string('woocommerce')])
	}
	return rt.call_function('__', [rt.new_string('Add your products'),
		rt.new_string('woocommerce')])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) get_content() rt.PhpVal {
	return rt.call_function('__', [
		rt.new_string('Start by adding the first product to your store. You can add your products manually, via CSV, or import them from another service.'),
		rt.new_string('woocommerce'),
	])
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) get_time() rt.PhpVal {
	return rt.call_function('__', [rt.new_string('1 minute per product'),
		rt.new_string('woocommerce')])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) get_additional_data() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{
			key: 'has_products'
			val: Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products.has_products()
		},
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) possibly_add_import_return_notice_script(var_hook rt.PhpVal) {
	mut var_step := if rt.get_superglobal('_GET').array_isset(rt.new_string('step')) {
		rt.get_superglobal('_GET').array_get(rt.new_string('step'))
	} else {
		rt.new_string('')
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_hook, rt.new_string('product_page_product_importer')))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_step, rt.new_string('done'))))) {
		return
	}
	...
	mut iife_result_0 := iife_temp_0.register_script(rt.new_string('wp-admin-scripts'),
		rt.new_string('onboarding-product-import-notice'), rt.new_bool(true))
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) possibly_add_load_sample_return_notice_script(var_hook rt.PhpVal) {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_hook, rt.new_string('edit.php')))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_query_var', [rt.new_string('post_type')]), rt.new_string('product'))))) {
		return
	}
	mut var_referer := rt.call_function('wp_get_referer', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(var_referer))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('strpos', [var_referer.clone(), rt.call_function('wc_admin_url', []rt.PhpVal{})]), rt.new_int(0))))) {
		return
	}
	...
	mut var_task_id := rt.call_function('sanitize_title_with_dashes', [
		rt.call_function('wp_unslash', [
			rt.get_superglobal('_GET').array_get(Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Task.active_task_transient()),
		]),
	])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_task_id, this.get_id()))))
		|| !(this.is_complete()) {
		return
	}
	...
	mut iife_result_1 := iife_temp_1.register_script(rt.new_string('wp-admin-scripts'),
		rt.new_string('onboarding-load-sample-products-notice'), rt.new_bool(true))
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) maybe_set_has_product_transient_on_untrashed_post(var_post_id rt.PhpVal) {
	...
	this.maybe_set_has_product_transient(var_post_id.clone(), rt.call_function('wc_get_product', [
		var_post_id.clone(),
	]))
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) maybe_set_has_product_transient(var_product_id rt.PhpVal, var_product rt.PhpVal) {
	if rt.is_true(rt.new_bool(!(rt.is_true(this.has_previously_completed()))))
		&& this.is_valid_product(var_product.clone()) {
		rt.call_function('set_transient', [
			Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products.has_product_transient(),
			rt.new_string('yes'),
		])
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) revert_task_completion() {
	...
	if rt.is_true(rt.get_static_prop('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products',
		'revert_scheduled'))
	{
		return
	}
	rt.set_static_prop('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products',
		'revert_scheduled', rt.new_bool(true))
	rt.call_function('add_action', [rt.new_string('shutdown'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products', [
				'Automattic_WooCommerce_Admin_Features_OnboardingTasks_Task',
			], &this) },
			rt.ArrayItem{ key: none, val: 'maybe_revert_on_shutdown' },
		])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) maybe_revert_on_shutdown() {
	...
	mut var_completed_tasks := rt.call_function('get_option', [
		Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products.completed_option(),
		rt.new_array(),
	])
	mut var_task_id := rt.new_string(this.get_id())
	if rt.is_true(rt.call_function('in_array', [var_task_id.clone(),
		var_completed_tasks.clone(), rt.new_bool(true)]))
	{
		var_completed_tasks = rt.call_function('array_values', [
			rt.call_function('array_diff', [var_completed_tasks.clone(),
				rt.create_array([rt.ArrayItem{ key: none, val: var_task_id }])]),
		])
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) is_valid_product(var_product rt.PhpVal) bool {
	return
		...
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_product, 'get_meta', [rt.new_string('_headstart_post')])))))
		|| rt.is_true(rt.call_function('get_post_meta', [rt.call_method(var_product, 'get_id', []rt.PhpVal{}), rt.new_string('_edit_last'), rt.new_bool(true)]))
}

fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products.has_products() rt.PhpVal {
	mut var_wpdb := rt.new_null()
	mut var_product_exists := rt.call_function('get_transient', [
		Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products.has_product_transient(),
	])
	if rt.is_true(var_product_exists) {
		return rt.identical(rt.new_string('yes'), var_product_exists)
	}
	mut var_value := rt.call_method(var_wpdb, 'get_var', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('SELECT IF(\n\t\t\t\t\tEXISTS (\n\t\t\t\t\t\tSELECT 1 FROM '), rt.get_property(var_wpdb,
				'posts')),
				rt.new_string(' p\n\t\t\t\t\t\tWHERE p.post_type = %s\n\t\t\t\t\t\tAND p.post_status = %s\n\t\t\t\t\t\tAND (\n\t\t\t\t\t\t\tEXISTS (\n\t\t\t\t\t\t\t\tSELECT 1 FROM ')), rt.get_property(var_wpdb,
				'postmeta')),
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Products) maybe_redirect_to_add_product_tasklist() {
	...
	if rt.is_true(var_screen)
		&& rt.is_true(rt.identical(rt.new_string('edit'), rt.get_property(var_screen, 'base')))
		&& rt.is_true(rt.identical(rt.new_string('product'), rt.get_property(var_screen, 'post_type'))) {
		...
		mut var_count := rt.call_function('array_sum', [var_counts.clone()])
		if rt.is_true(rt.greater(var_count, rt.new_int(0))) {
			return
		}
		rt.call_function('wp_safe_redirect', [
			rt.call_function('admin_url', [
				rt.new_string('admin.php?page=wc-admin&task=products'),
			]),
		])
		exit(0)
	}
