...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Settings_Transformer) transform(mut var_raw_settings Class_Automattic_WooCommerce_Admin_Features_Settings_array) rt.PhpVal {
	...
	for {
		...
		if !(var_tab.array_isset(rt.new_string('sections'))) || !(var_tab.array_get(rt.new_string('sections')).is_array()) {
			var_transformed.array_set(var_tab_id, var_tab.clone())
			continue
		}
		var_transformed.array_set(var_tab_id, var_tab.clone())
		var_transformed.array_get_mut(var_tab_id).array_set('sections', this.transform_sections(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_Settings_array](var_tab.array_get(rt.new_string('sections')))))
	}
	return var_transformed.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Settings_Transformer) transform_sections(mut var_sections Class_Automattic_WooCommerce_Admin_Features_Settings_array) rt.PhpVal {
	...
	for {
		...
		mut var_section := item_2.val
		mut var_section_id := item_2.key
		if !(var_section.array_isset(rt.new_string('settings'))) || !(var_section.array_get(rt.new_string('settings')).is_array()) {
			var_transformed_sections.array_set(var_section_id, var_section.clone())
			continue
		}
		var_transformed_sections.array_set(var_section_id, var_section.clone())
		var_transformed_sections.array_get_mut(var_section_id).array_set('settings', this.transform_section_settings(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_Settings_array](var_section.array_get(rt.new_string('settings')))))
	}
	return var_transformed_sections.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Settings_Transformer) transform_section_settings(mut var_settings Class_Automattic_WooCommerce_Admin_Features_Settings_array) rt.PhpVal {
	...
	mut iter_3 := var_settings.iterator()
	for {
	...
	return var_transformed_settings.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Settings_Transformer) process_setting(mut var_setting Class_Automattic_WooCommerce_Admin_Features_Settings_?array, mut var_transformed_settings Class_Automattic_WooCommerce_Admin_Features_Settings_array) {
	...
	mut var_type := if !(var_setting_mutated.array_get(rt.new_string('type'))).is_null() { var_setting_mutated.array_get(rt.new_string('type')) } else { rt.new_string('') }
	if rt.is_true(this.current_checkbox_group) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('checkbox'), var_type)))) {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('title'))) {
		...
		if !(!rt.is_true(var_setting_mutated.array_get(rt.new_string('text')))) {
			var_setting_mutated.array_set('text', rt.call_function('wp_kses_post', [rt.call_function('wpautop', [rt.call_function('wptexturize', [var_setting_mutated.array_get(rt.new_string('text'))])])]))
		}
		if !(!rt.is_true(var_setting_mutated.array_get(rt.new_string('row_class')))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('substr', [var_setting_mutated.array_get(rt.new_string('row_class')), rt.new_int(0), rt.new_int(16)]), rt.new_string('wc-settings-row-'))))) {
			var_setting_mutated.array_set('row_class', 'wc-settings-row-' + (var_setting_mutated.array_get(rt.new_string('row_class'))).str())
		}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Settings_Transformer) handle_group_end(mut var_setting Class_Automattic_WooCommerce_Admin_Features_Settings_array, mut var_transformed_settings Class_Automattic_WooCommerce_Admin_Features_Settings_array) {
	mut var_setting_mutated := var_setting
	mut var_transformed_settings_mutated := var_transformed_settings
	mut var_ids_match := rt.new_bool(rt.is_true(this.current_group) && this.current_group.array_get(rt.new_int(0)).array_isset(rt.new_string('id')) && var_setting_mutated.array_isset(rt.new_string('id')) && rt.is_true(rt.identical(this.current_group.array_get(rt.new_int(0)).array_get(rt.new_string('id')), var_setting_mutated.array_get(rt.new_string('id')))))
	mut var_ids_match_undefined := rt.new_bool(rt.is_true(this.current_group) && !(this.current_group.array_get(rt.new_int(0)).array_isset(rt.new_string('id'))) && !(var_setting_mutated.array_isset(rt.new_string('id'))))
	if rt.is_true(var_ids_match) || rt.is_true(var_ids_match_undefined) {
		mut var_title_setting := rt.call_function('array_shift', [this.current_group])
		var_title_setting.array_set('id', if !(var_title_setting.array_get(rt.new_string('id'))).is_null() { var_title_setting.array_get(rt.new_string('id')) } else { rt.call_function('wp_unique_prefixed_id', [rt.new_string('setting_group')]) })
		var_transformed_settings_mutated.array_push(rt.call_function('array_merge', [var_title_setting.clone(), rt.create_array([rt.ArrayItem{ key: 'type', val: 'group' }, rt.ArrayItem{ key: 'settings', val: this.current_group }])]))
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Settings_Transformer) flush_current_group(mut var_transformed_settings Class_Automattic_WooCommerce_Admin_Features_Settings_array) {
	mut var_transformed_settings_mutated := var_transformed_settings
	if this.current_group.is_array() && !(!rt.is_true(this.current_group)) {
		this.current_group.array_get_mut(0).array_set('id', if !(this.current_group.array_get(rt.new_int(0)).array_get(rt.new_string('id'))).is_null() { this.current_group.array_get(rt.new_int(0)).array_get(rt.new_string('id')) } else { rt.call_function('wp_unique_prefixed_id', [rt.new_string('setting_title')]) })
	var_transformed_settings_mutated = rt.call_function('array_merge', [var_transformed_settings_mutated, this.current_group])
	}
	this.current_group = rt.new_null()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Settings_Transformer) handle_checkbox_setting(mut var_setting Class_Automattic_WooCommerce_Admin_Features_Settings_array, mut var_transformed_settings Class_Automattic_WooCommerce_Admin_Features_Settings_array) {
	mut var_setting_mutated := var_setting
	mut var_transformed_settings_mutated := var_transformed_settings
	mut var_checkboxgroup := if !(var_setting_mutated.array_get(rt.new_string('checkboxgroup'))).is_null() { var_setting_mutated.array_get(rt.new_string('checkboxgroup')) } else { rt.new_string('') }
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Settings_Transformer) end_checkbox_group(mut var_setting Class_Automattic_WooCommerce_Admin_Features_Settings_array, mut var_transformed_settings Class_Automattic_WooCommerce_Admin_Features_Settings_array) {
	...
	this.current_checkbox_group.array_push(var_setting_mutated)
	mut var_first_setting := this.current_checkbox_group.array_get(rt.new_int(0))
	mut var_checkbox_group_setting := rt.create_array([rt.ArrayItem{ key: 'id', val: rt.call_function('wp_unique_prefixed_id', [rt.new_string('setting_checkboxgroup')]) }, rt.ArrayItem{ key: 'type', val: 'checkboxgroup' }, rt.ArrayItem{ key: 'title', val: if !(var_first_setting.array_get(rt.new_string('title'))).is_null() { var_first_setting.array_get(rt.new_string('title')) } else { rt.new_string('') } }, rt.ArrayItem{ key: 'settings', val: this.current_checkbox_group }])
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Settings_Transformer) add_setting(mut var_setting Class_Automattic_WooCommerce_Admin_Features_Settings_array, mut var_transformed_settings Class_Automattic_WooCommerce_Admin_Features_Settings_array) {
	mut var_setting_mutated := var_setting
	mut var_transformed_settings_mutated := var_transformed_settings
	var_setting_mutated.array_set('id', if !(var_setting_mutated.array_get(rt.new_string('id'))).is_null() { var_setting_mutated.array_get(rt.new_string('id')) } else { rt.call_function('wp_unique_prefixed_id', [rt.new_string('setting_field')]) })
	if rt.is_true(rt.new_bool(this.current_group.is_array())) {
		this.current_group.array_push(var_setting_mutated)
		return
	}
	var_transformed_settings_mutated.array_push(var_setting_mutated)
}
