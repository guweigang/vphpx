...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_DPDShippingProvider) validate_country_pattern(tracking_number string, country_code string) rt.PhpVal {
	...
	mut var_country_data :=
		Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_DPDShippingProvider.tracking_patterns().array_get(rt.new_string(country_code))
	mut var_detected_service := rt.new_null()
	mut var_confidence_boost := rt.new_int(0)
	if var_country_data.array_isset(rt.new_string('services')) {
		...
		{
			var_detected_service = rt.new_string('classic')
			var_confidence_boost = if !(var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('classic'))).is_null() {
				var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('classic'))
			} else {
				rt.new_int(0)
			}
		...
		{
			var_detected_service = rt.new_string('express')
			var_confidence_boost = if !(var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('express'))).is_null() {
				var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('express'))
			} else {
				rt.new_int(0)
			}
		} else if rt.is_true(rt.call_function('preg_match', [
		...
		{
			var_detected_service = rt.new_string('predict')
			var_confidence_boost = if !(var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('predict'))).is_null() {
				var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('predict'))
			} else {
				rt.new_int(0)
			}
		} else if
			...
			var_detected_service = rt.new_string('next_day')
			var_confidence_boost = if !(var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('next_day'))).is_null() {
				var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('next_day'))
			} else {
				rt.new_int(0)
			}
		} else if
			...
			var_detected_service = rt.new_string('relais')
			var_confidence_boost = if !(var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('relais'))).is_null() {
				var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('relais'))
			} else {
				rt.new_int(0)
			}
		} else if rt.is_true(rt.call_function('preg_match', [
		...
		{
			var_detected_service = rt.new_string('s10')
			var_confidence_boost = if !(var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('s10'))).is_null() {
				var_country_data.array_get(rt.new_string('services')).array_get(rt.new_string('s10'))
			} else {
				rt.new_int(90)
			}
		}
	}
	mut iter_1 := var_country_data.array_get(rt.new_string('patterns')).iterator()
	for {
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_DPDShippingProvider) try_parse_tracking_number(tracking_number string, shipping_from string, shipping_to string) rt.PhpVal {
	...
	{
		if rt.is_true(rt.call_function('in_array', [rt.new_string(shipping_from_mutated).clone(), this.get_shipping_from_countries(), rt.new_bool(true)]))
			&& rt.is_true(rt.call_function('in_array', [rt.new_string(shipping_to_mutated).clone(), this.get_shipping_to_countries(), rt.new_bool(true)])) {
			return rt.create_array([
	...
	if rt.is_true(var_validation_result) && var_validation_result.clone().is_array() {
		mut var_confidence :=
			Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_DPDShippingProvider.tracking_patterns().array_get(rt.new_string(shipping_from_mutated)).array_get(rt.new_string('confidence'))
		if rt.is_true(rt.greater(var_validation_result.array_get(rt.new_string('confidence_boost')),
			rt.new_int(0)))
		{
			var_confidence = rt.call_function('min', [rt.new_int(95),
				var_validation_result.array_get(rt.new_string('confidence_boost'))])
		}
		...
		if rt.is_true(rt.identical(rt.new_string('express'),
			var_validation_result.array_get(rt.new_string('service'))))
		{
			var_confidence = rt.call_function('min', [rt.new_int(98),
...
fn create_automattic_woocommerce_admin_features_fulfillments_providers_dpdshippingprovider(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_Fulfillments_Providers_DPDShippingProvider {
