...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exporters_ExportWCSettingsShipping) get_terms(mut var_classes Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exporters_array) rt.PhpVal {
	...
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_int((var_term.array_get(rt.new_string('term_id'))).to_i64())
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exporters_ExportWCSettingsShipping) get_steps_for_classes_and_terms() rt.PhpVal {
	...
	closure_4_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_class_row := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_3 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_3 := iife_temp_3.array_to_insert_sql(var_class_row.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'term_taxonomy'),
			rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_3)
	}
	closure_6_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_class_row := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_5 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_5 := iife_temp_5.array_to_insert_sql(var_class_row.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'term_taxonomy'),
			rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_5)
	}
	mut var_classes_steps := rt.call_function('array_map', [rt.new_closure(closure_4_fn),
		var_classes.clone()])
	closure_8_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_term := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_7 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_7 := iife_temp_7.array_to_insert_sql(var_term.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'terms'), rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_7)
	}
	closure_10_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_term := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_9 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_9 := iife_temp_9.array_to_insert_sql(var_term.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'terms'), rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_9)
	}
	mut var_terms := rt.call_function('array_map', [rt.new_closure(closure_8_fn),
		this.get_terms(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exporters_array](var_classes))])
	return rt.call_function('array_merge', [var_classes_steps.clone(),
		var_terms.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exporters_ExportWCSettingsShipping) get_step_name() string {
	mut iife_temp_10 := Class_Automattic_WooCommerce_Blueprint_Steps_RunSql{}
	mut iife_result_10 := iife_temp_10.get_step_name()
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exporters_ExportWCSettingsShipping) get_steps_for_zones() rt.PhpVal {
	...
	closure_13_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_zone := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_12 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_12 := iife_temp_12.array_to_insert_sql(var_zone.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'woocommerce_shipping_zones'),
			rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_12)
	}
	closure_15_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_zone := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_14 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_14 := iife_temp_14.array_to_insert_sql(var_zone.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'woocommerce_shipping_zones'),
			rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_14)
	}
	return rt.call_function('array_map', [rt.new_closure(closure_13_fn),
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exporters_ExportWCSettingsShipping) get_steps_for_locations() rt.PhpVal {
	...
	closure_17_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut iife_temp_16 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_16 := iife_temp_16.array_to_insert_sql(var_location.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'woocommerce_shipping_zone_locations'),
			rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_16)
	}
	closure_19_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_location := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_18 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_18 := iife_temp_18.array_to_insert_sql(var_location.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'woocommerce_shipping_zone_locations'),
			rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_18)
	}
	return rt.call_function('array_map', [rt.new_closure(closure_17_fn),
		rt.call_method(var_wpdb, 'get_results', [
			rt.concat(rt.concat(rt.new_string('SELECT * FROM '),
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exporters_ExportWCSettingsShipping) get_steps_for_methods_and_options() rt.PhpVal {
	...
	closure_21_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut iife_temp_20 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_20 := iife_temp_20.array_to_insert_sql(var_method.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'woocommerce_shipping_zone_methods'),
			rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_20)
	}
	closure_23_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_method := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_22 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_22 := iife_temp_22.array_to_insert_sql(var_method.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'woocommerce_shipping_zone_methods'),
			rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_22)
	}
	closure_25_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_option := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_24 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_24 := iife_temp_24.array_to_insert_sql(var_option.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'options'), rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_24)
	}
	closure_27_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_option := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_26 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_26 := iife_temp_26.array_to_insert_sql(var_option.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'options'), rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_26)
	}
	closure_29_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_method := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_28 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_28 := iife_temp_28.array_to_insert_sql(var_method.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'woocommerce_shipping_zone_methods'),
			rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_28)
	}
	closure_31_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_method := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut iife_temp_30 := Class_Automattic_WooCommerce_Blueprint_Util{}
		mut iife_result_30 := iife_temp_30.array_to_insert_sql(var_method.clone(), rt.new_string(
			(rt.get_property(var_wpdb, 'prefix')).str() + 'woocommerce_shipping_zone_methods'),
			rt.new_string('replace into'))
		return create_automattic_woocommerce_blueprint_steps_runsql(iife_result_30)
	}
	return rt.call_function('array_merge', [
		rt.call_function('array_map', [rt.new_closure(closure_21_fn),
			var_methods.clone()]),
		rt.call_function('array_map', [rt.new_closure(closure_25_fn),
			var_method_options.clone()]),
	])
}
...
fn create_automattic_woocommerce_admin_features_blueprint_exporters_exportwcsettingsshipping(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_Blueprint_Exporters_ExportWCSettingsShipping {
