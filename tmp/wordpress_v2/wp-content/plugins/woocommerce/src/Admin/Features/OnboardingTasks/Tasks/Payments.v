...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) get_title() rt.PhpVal {
	return rt.call_function('__', [rt.new_string('Set up payments'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) get_time() rt.PhpVal {
	return rt.call_function('__', [rt.new_string('5 minutes'),
		rt.new_string('woocommerce')])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) is_complete() rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_null(), this.is_complete_result)) {
		if this.is_woopayments_active() {
			this.is_complete_result = rt.new_bool(this.is_woopayments_onboarded()
				&& !(this.has_woopayments_test_account()))
		} else {
			this.is_complete_result =
				Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments.has_gateways()
		}
	}
...
fn Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments.has_gateways() bool {
	...
	mut var_enabled_gateways := rt.call_function('array_filter', [
		var_gateways.clone(), rt.new_closure(closure_1_fn)])
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) in_progress_label() rt.PhpVal {
	if this.has_woopayments_live_account_in_progress() {
		return rt.call_function('esc_html__', [rt.new_string('Action needed'),
			rt.new_string('woocommerce')])
	}
	return rt.call_function('esc_html__', [rt.new_string('Test account'),
		rt.new_string('woocommerce')])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) get_action_url() rt.PhpVal {
	return rt.call_function('admin_url', [
		rt.new_string('admin.php?page=wc-settings&tab=checkout&from=' +(Class_Automattic_WooCommerce_Internal_Admin_Settings_Payments.from_payments_task()).str()),
	])
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) get_additional_data() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: 'wooPaymentsIsActive', val: this.is_woopayments_active() },
		rt.ArrayItem{ key: 'wooPaymentsIsInstalled', val: this.is_woopayments_installed() },
		rt.ArrayItem{
			key: 'wooPaymentsSettingsCountryIsSupported'
			val: this.is_woopayments_supported_country(this.get_payments_settings_country())
		},
		rt.ArrayItem{ key: 'wooPaymentsIsOnboarded', val: this.is_woopayments_onboarded() },
		rt.ArrayItem{ key: 'wooPaymentsHasTestAccount', val: this.has_woopayments_test_account() },
		rt.ArrayItem{
			key: 'wooPaymentsHasOtherProvidersEnabled'
			val: this.has_providers_enabled_other_than_woopayments()
		},
		rt.ArrayItem{
			key: 'wooPaymentsHasOtherProvidersNeedSetup'
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) is_woopayments_installed() bool {
	...
	if !(!rt.is_true(var_woopayments_suggestion.array_get(rt.new_string('plugin')).array_get(rt.new_string('status'))))
		&& rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders.extension_installed(), var_woopayments_suggestion.array_get(rt.new_string('plugin')).array_get(rt.new_string('status')))) {
		return true
	}
	return false
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) is_woopayments_onboarded() bool {
	...
	if !(!rt.is_true(var_woopayments_provider.array_get(rt.new_string('onboarding')).array_get(rt.new_string('state')).array_get(rt.new_string('completed')))) {
		return true
	}
	return false
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) has_woopayments_live_account_in_progress() bool {
	...
	if !(!rt.is_true(var_woopayments_provider.array_get(rt.new_string('onboarding')).array_get(rt.new_string('state')).array_get(rt.new_string('started')))) {
		return true
	}
	return false
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) has_woopayments_test_account() bool {
	...
	if !(!rt.is_true(var_woopayments_provider.array_get(rt.new_string('onboarding')).array_get(rt.new_string('state')).array_get(rt.new_string('test_drive_account')))) {
		return true
	}
	return false
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) is_woopayments_supported_country(country_code string) bool {
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('\\WC_Payments_Utils')]))
		&& rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{
		key: none
		val: '\\WC_Payments_Utils'
	}, rt.ArrayItem{ key: none, val: 'supported_countries' }])]) {
		mut iife_temp_1 :=
			Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WC_Payments_Utils{}
		mut iife_result_1 := iife_temp_1.supported_countries()
		mut var_supported_countries := rt.func_array_keys(iife_result_1)
		return (rt.call_function('in_array', [rt.new_string(country_code),
			var_supported_countries.clone(), rt.new_bool(true)])).to_bool()
	}
	mut iife_temp_2 :=
		Class_Automattic_WooCommerce_Admin_Features_PaymentGatewaySuggestions_DefaultPaymentGateways{}
	mut iife_result_2 := iife_temp_2.get_wcpay_countries()
	var_supported_countries = iife_result_2
	return (rt.call_function('in_array', [rt.new_string(country_code),
		var_supported_countries.clone(), rt.new_bool(true)])).to_bool()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) has_providers_enabled_other_than_woopayments() bool {
	...
	for {
		...
		if !(!rt.is_true(var_provider.array_get(rt.new_string('state')).array_get(rt.new_string('enabled'))))
			...
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('woocommerce_payments'), var_provider.array_get(rt.new_string('id')))))) {
			return true
		}
	}
	return false
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) has_providers_needing_setup_other_than_woopayments() bool {
	...
	for {
		...
		if !(!rt.is_true(var_provider.array_get(rt.new_string('state')).array_get(rt.new_string('needs_setup'))))
			&& !(!rt.is_true(var_provider.array_get(rt.new_string('id'))))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('woocommerce_payments'), var_provider.array_get(rt.new_string('id')))))) {
			return true
		}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) has_online_gateways() bool {
	...
	for {
		...
		if !(!rt.is_true(var_provider.array_get(rt.new_string('state')).array_get(rt.new_string('enabled'))))
			&& !(!rt.is_true(var_provider.array_get(rt.new_string('id'))))
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_provider.array_get(rt.new_string('id')), rt.create_array([rt.ArrayItem{
			key: none
			val: Class_WC_Gateway_BACS.id()
		}, rt.ArrayItem{ key: none, val: Class_WC_Gateway_Cheque.id() }, rt.ArrayItem{
			key: none
			val: Class_WC_Gateway_COD.id()
		}]), rt.new_bool(true)]))))) {
			return true
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) get_payments_providers() rt.PhpVal {
	...
	return rt.call_method(var_settings_payments_service, 'get_payment_providers', [
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) get_payments_extension_suggestions() rt.PhpVal {
	...
	return rt.call_method(var_settings_payments_service, 'get_payment_extension_suggestions', [
		rt.call_method(var_settings_payments_service, 'get_country', []rt.PhpVal{}),
	])
	unsafe {
		goto end_label_3
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) get_woopayments_provider() rt.PhpVal {
	mut var_providers := this.get_payments_providers()
	mut iter_4 := var_providers.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_provider := item_4.val
		if !(!rt.is_true(var_provider.array_get(rt.new_string('id'))))
			&& rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Settings_PaymentsProviders_WooPayments_WooPaymentsService.gateway_id(), var_provider.array_get(rt.new_string('id')))) {
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_Payments) get_woopayments_suggestion() rt.PhpVal {
	mut var_providers := this.get_payments_providers()
	mut iter_5 := var_providers.iterator()
	for {
		item_5 := iter_5.next() or { break }
		mut var_provider := item_5.val
		if !(!rt.is_true(var_provider.array_get(rt.new_string('_type'))))
			...
			&& rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Suggestions_PaymentsExtensionSuggestions.woopayments(), var_provider.array_get(rt.new_string('_suggestion_id')))) {
			return var_provider.clone()
		}
	}
	mut var_suggestions := this.get_payments_extension_suggestions()
	mut iter_6 := var_suggestions.iterator()
	for {
		item_6 := iter_6.next() or { break }
		mut var_suggestion := item_6.val
		if !(!rt.is_true(var_suggestion.array_get(rt.new_string('id'))))
			&& rt.is_true(rt.identical(Class_Automattic_WooCommerce_Internal_Admin_Suggestions_PaymentsExtensionSuggestions.woopayments(), var_suggestion.array_get(rt.new_string('id')))) {
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_tasks_wc_payments_utils(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_WC_Payments_Utils {
