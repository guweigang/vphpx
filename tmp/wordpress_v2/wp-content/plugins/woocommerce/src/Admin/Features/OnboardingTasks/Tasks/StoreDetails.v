...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_StoreDetails) get_title() rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_bool(true),
		this.get_parent_option(rt.new_string('use_completed_title'))))
	{
		if this.is_complete() {
			return rt.call_function('__', [rt.new_string('You added store details'),
				rt.new_string('woocommerce')])
		}
		return rt.call_function('__', [rt.new_string('Add store details'),
			rt.new_string('woocommerce')])
	}
	return rt.call_function('__', [rt.new_string('Store details'),
		rt.new_string('woocommerce')])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_StoreDetails) get_content() rt.PhpVal {
	return rt.call_function('__', [
		rt.new_string('Your store address is required to set the origin country for shipping, currencies, and payment options.'),
		rt.new_string('woocommerce'),
	])
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_StoreDetails) get_time() rt.PhpVal {
	return rt.call_function('__', [rt.new_string('4 minutes'),
		rt.new_string('woocommerce')])
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_StoreDetails) get_action_url() rt.PhpVal {
	return if !(this.is_complete()) { rt.call_function('admin_url', [
			rt.new_string('admin.php?page=wc-settings&tab=general&tutorial=true'),
		]) } else { rt.call_function('admin_url', [
			rt.new_string('admin.php?page=wc-settings&tab=general'),
		]) }
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_StoreDetails) is_complete() bool {
	...
	mut var_country_locale := rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}),
		'countries'), 'get_country_locale', []rt.PhpVal{})
	mut var_locale := if !(var_country_locale.array_get(var_country)).is_null() {
		var_country_locale.array_get(var_country)
	} else {
		rt.new_array()
	}
	mut var_hide_postcode := if !(var_locale.array_get(rt.new_string('postcode')).array_get(rt.new_string('hidden'))).is_null() {
		var_locale.array_get(rt.new_string('postcode')).array_get(rt.new_string('hidden'))
	} else {
		rt.new_bool(false)
	}
	if rt.is_true(var_hide_postcode) {
		return
			rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_option', [rt.new_string('woocommerce_store_address'), rt.new_string('')]), rt.new_string('')))))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_option', [rt.new_string('woocommerce_store_city'), rt.new_string('')]), rt.new_string('')))))
	}
	return
		rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_option', [rt.new_string('woocommerce_store_address'), rt.new_string('')]), rt.new_string('')))))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_option', [rt.new_string('woocommerce_store_city'), rt.new_string('')]), rt.new_string('')))))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_option', [rt.new_string('woocommerce_store_postcode'), rt.new_string('')]), rt.new_string('')))))
}
...
fn create_automattic_woocommerce_admin_features_onboardingtasks_tasks_storedetails(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_OnboardingTasks_Tasks_StoreDetails {
