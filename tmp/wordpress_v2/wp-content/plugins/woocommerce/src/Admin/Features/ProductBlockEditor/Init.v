...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) construct() {
	...
	this.redirection_controller =
		create_automattic_woocommerce_admin_features_productblockeditor_redirectioncontroller()
	mut iife_temp_0 :=
		Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Automattic_WooCommerce_Utilities_FeaturesUtil{}
	mut iife_result_0 := iife_temp_0.feature_is_enabled(rt.new_string('product_block_editor'))
	if rt.is_true(iife_result_0) {
		rt.call_function('add_action', [rt.new_string('admin_enqueue_scripts'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
		...
		rt.call_function('add_action', [rt.new_string('admin_enqueue_scripts'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'dequeue_conflicting_styles' },
			]),
			rt.new_int(100)])
		rt.call_function('add_action', [rt.new_string('get_edit_post_link'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'update_edit_product_link' },
			]),
		...
		rt.call_function('add_filter', [
			...
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'enable_rest_api_for_product_variation' },
			]),
		])
		rt.call_function('add_action', [rt.new_string('current_screen'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'set_current_screen_to_block_editor_if_wc_admin' },
			])])
		rt.call_function('add_action', [rt.new_string('rest_api_init'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'register_layout_templates' },
			])])
		rt.call_function('add_action', [rt.new_string('rest_api_init'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'register_user_metas' },
			])])
		rt.call_function('add_filter', [rt.new_string('register_block_type_args'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'register_metadata_attribute' },
			])])
		rt.call_function('add_filter', [rt.new_string('woocommerce_get_block_types'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'get_block_types' },
			]),
		...
		rt.call_function('add_filter', [
			...
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'possibly_add_template_id' },
			]),
			rt.new_int(10),
			rt.new_int(2),
		])
		rt.call_function('add_filter', [
			rt.new_string('woocommerce_rest_prepare_product_variation_object'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init',
					[]string{}, &this) },
				rt.ArrayItem{ key: none, val: 'possibly_add_template_id' },
		...
		mut iife_temp_1 :=
			Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_BlockRegistry{}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) possibly_add_template_id(var_response rt.PhpVal, var_product rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_product_mutated, 'meta_exists', [
		rt.new_string('_product_template_id'),
	])))))
	{
		mut var_product_template_id := rt.call_function('apply_filters', [
			rt.new_string('experimental_woocommerce_product_editor_product_template_id_for_product'),
			rt.new_string(''),
			var_product_mutated.clone(),
		])
		if rt.is_true(var_product_template_id) {
			rt.get_property(var_response, 'data').array_get_mut('meta_data').array_push(create_wc_meta_data(rt.create_array([
				rt.ArrayItem{ key: 'key', val: '_product_template_id' },
				rt.ArrayItem{ key: 'value', val: var_product_template_id },
			])))
		}
	}
	return var_response.clone()
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) enqueue_scripts() {
	...
	mut iife_result_2 := iife_temp_2.is_admin_or_embed_page()
	if rt.is_true(rt.new_bool(!(rt.is_true(iife_result_2)))) {
		return
	}
	mut var_editor_settings := this.get_product_editor_settings()
	mut var_script_handle := rt.new_string('wc-admin-edit-product')
	rt.call_function('wp_register_script', [var_script_handle.clone(),
	...
	rt.call_function('wp_add_inline_script', [var_script_handle.clone(),
		rt.new_string('var productBlockEditorSettings = productBlockEditorSettings || ' +
			(rt.call_function('wp_json_encode', [var_editor_settings.clone(), rt.bitwise_or(rt.get_constant('JSON_HEX_TAG'), rt.get_constant('JSON_UNESCAPED_SLASHES'))])).str() +
			';'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) enqueue_styles() {
	...
	mut iife_result_3 := iife_temp_3.is_admin_page()
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) update_edit_product_link(var_link rt.PhpVal, var_post_id rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.call_method(var_product, 'get_type', []rt.PhpVal{}),
		Class_Automattic_WooCommerce_Enums_ProductType.simple()))
	{
		return rt.call_function('admin_url', [
			rt.new_string('admin.php?page=wc-admin&path=/product/' +
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) add_user_data_fields(var_user_data_fields rt.PhpVal) rt.PhpVal {
	return rt.call_function('array_merge', [var_user_data_fields.clone(),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'local_attributes_notice_dismissed_ids' },
			rt.ArrayItem{ key: none, val: 'variable_items_without_price_notice_dismissed' },
			rt.ArrayItem{ key: none, val: 'product_advice_card_dismissed' },
		])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) set_current_screen_to_block_editor_if_wc_admin() {
	...
	if rt.is_true(iife_result_5) {
		...
		rt.call_function('wp_add_inline_script', [rt.new_string('wp-blocks'),
			rt.new_string(
				'wp.blocks && wp.blocks.unstable__bootstrapServerSideBlockDefinitions && wp.blocks.unstable__bootstrapServerSideBlockDefinitions(' +
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) get_product_editor_settings() rt.PhpVal {
	...
	closure_7_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_product_template := if args.len > 0 { args[0].clone() } else { rt.new_null() }
	...
	var_editor_settings.array_set('productTemplates', rt.call_function('array_map', [
	...
	mut var_block_editor_context := create_wp_block_editor_context(rt.create_array([
		rt.ArrayItem{
			key: 'name'
			val: Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init.editor_context_name()
		},
	]))
	return rt.call_function('get_block_editor_settings', [var_editor_settings.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) get_default_product_templates() rt.PhpVal {
	...
	var_templates.array_push(create_automattic_woocommerce_admin_features_productblockeditor_producttemplate(rt.create_array([
		rt.ArrayItem{ key: 'id', val: 'standard-product-template' },
		rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
			rt.new_string('Standard product'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('A single physical or virtual product, e.g. a t-shirt or an eBook.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'order', val: 10 },
		rt.ArrayItem{ key: 'icon', val: 'shipping' },
		rt.ArrayItem{ key: 'layout_template_id', val: 'simple-product' },
		rt.ArrayItem{ key: 'product_data', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: Class_Automattic_WooCommerce_Enums_ProductType.simple() },
		]) },
	])))
	var_templates.array_push(create_automattic_woocommerce_admin_features_productblockeditor_producttemplate(rt.create_array([
		rt.ArrayItem{ key: 'id', val: 'grouped-product-template' },
		rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
			rt.new_string('Grouped product'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('A set of products that go well together, e.g. camera kit.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'order', val: 20 },
		rt.ArrayItem{ key: 'icon', val: 'group' },
		rt.ArrayItem{ key: 'layout_template_id', val: 'simple-product' },
		rt.ArrayItem{ key: 'product_data', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: Class_Automattic_WooCommerce_Enums_ProductType.grouped() },
		]) },
	])))
	var_templates.array_push(create_automattic_woocommerce_admin_features_productblockeditor_producttemplate(rt.create_array([
		rt.ArrayItem{ key: 'id', val: 'affiliate-product-template' },
		rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
			rt.new_string('Affiliate product'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('A link to a product sold on a different website, e.g. brand collab.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'order', val: 30 },
		rt.ArrayItem{ key: 'icon', val: 'link' },
		rt.ArrayItem{ key: 'layout_template_id', val: 'simple-product' },
		rt.ArrayItem{ key: 'product_data', val: rt.create_array([
			rt.ArrayItem{
				key: 'type'
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) create_default_product_template_by_custom_product_type(mut var_templates Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_array) rt.PhpVal {
	...
	closure_9_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_product_type := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
			var_product_type.clone(), this.supported_product_types, rt.new_bool(true)]))))
	}
	mut var_custom_product_types := rt.call_function('array_filter', [
		var_registered_product_types.clone(), rt.new_closure(closure_9_fn),
	...
	closure_10_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_bool(!(var_product_data.clone().is_null())
			&& rt.is_true(rt.new_bool(var_product_data.clone().array_isset(rt.new_string('type')))))
	}
	mut var_templates_with_product_type := rt.call_function('array_filter', [
		var_templates_mutated,
		rt.new_closure(closure_10_fn),
	])
	closure_11_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_template := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_product_data := rt.call_method(var_template, 'get_product_data', []rt.PhpVal{})
		return var_product_data.array_get(rt.new_string('type'))
	}
	...
	mut var_custom_product_types_on_templates := rt.call_function('array_map', [
		rt.new_closure(closure_11_fn),
		var_templates_with_product_type.clone(),
	])
	...
	for {
		...
		if rt.is_true(rt.call_function('in_array', [var_product_type.clone(),
			var_custom_product_types_on_templates.clone(), rt.new_bool(true)]))
		{
			continue
		}
		var_templates_mutated.array_push(create_automattic_woocommerce_admin_features_productblockeditor_producttemplate(rt.create_array([
			rt.ArrayItem{ key: 'id', val: var_product_type.str() + '-product-template' },
			rt.ArrayItem{ key: 'title', val: var_title },
			rt.ArrayItem{ key: 'product_data', val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: var_product_type },
			]) },
		])))
	}
	return rt.new_object('Automattic_WooCommerce_Admin_Features_ProductBlockEditor_array',
		[]string{}, var_templates_mutated)
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) register_layout_templates() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_layout_template_registry,
		'is_registered', [rt.new_string('simple-product')])))))
	{
		rt.call_method(var_layout_template_registry, 'register', [
			rt.new_string('simple-product'),
			rt.new_string('product-form'),
			Class_Automattic_WooCommerce_Internal_Features_ProductBlockEditor_ProductTemplates_SimpleProductTemplate.class(),
		])
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_layout_template_registry,
		'is_registered', [rt.new_string('product-variation')])))))
	{
		rt.call_method(var_layout_template_registry, 'register', [
			rt.new_string('product-variation'),
			rt.new_string('product-form'),
			Class_Automattic_WooCommerce_Internal_Features_ProductBlockEditor_ProductTemplates_ProductVariationTemplate.class(),
		])
	}
}

fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) register_product_templates() {
	this.product_templates = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_product_editor_product_templates'),
		this.get_default_product_templates(),
	])
	this.product_templates =
		this.create_default_product_template_by_custom_product_type(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_array](this.product_templates))
	...
	rt.call_function('usort', [this.product_templates, rt.new_closure(closure_13_fn)])
	rt.call_method(this.redirection_controller, 'set_product_templates', [
		this.product_templates,
	])
	mut iife_temp_13 := Class_Automattic_WooCommerce_Admin_Features_Features{}
	mut iife_result_13 := iife_temp_13.is_enabled(rt.new_string('product-editor-template-system'))
	if rt.is_true(iife_result_13) {
		mut var_product_form_controller :=
			create_automattic_woocommerce_admin_features_productblockeditor_productformscontroller()
		var_product_form_controller.init()
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) register_user_metas() {
	closure_15_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_object := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_attr := if args.len > 1 { args[1].clone() } else { rt.new_null() }
		mut var_hidden := rt.call_function('get_user_meta', [
	...
	closure_16_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_object := if args.len > 1 { args[1].clone() } else { rt.new_null() }
		mut var_attr := if args.len > 2 { args[2].clone() } else { rt.new_null() }
		rt.call_function('update_user_meta', [rt.get_property(var_object, 'ID'),
			var_attr.clone(), var_value.clone()])
		return rt.new_null()
	}
	rt.call_function('register_rest_field', [rt.new_string('user'),
		rt.new_string('metaboxhidden_product'),
		rt.create_array([
			rt.ArrayItem{ key: 'get_callback', val: rt.new_closure(closure_15_fn) },
			rt.ArrayItem{ key: 'update_callback', val: rt.new_closure(closure_16_fn) },
			rt.ArrayItem{ key: 'schema', val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: 'array' },
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('The metaboxhidden_product meta from the user metas.'),
					rt.new_string('woocommerce'),
				]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) register_metadata_attribute(var_args rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_args_mutated.array_get(rt.new_string('attributes')).array_isset(rt.new_string('metadata'))))))) {
		var_args_mutated.array_get_mut('attributes').array_set('metadata', rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) get_block_types(var_block_types rt.PhpVal) rt.PhpVal {
	...
	mut iife_result_16 := iife_temp_16.is_admin_page()
...
fn create_automattic_woocommerce_admin_features_productblockeditor_redirectioncontroller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_RedirectionController {
...
fn create_automattic_woocommerce_admin_features_productblockeditor_automattic_woocommerce_utilities_featuresutil(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Automattic_WooCommerce_Utilities_FeaturesUtil {
...
fn create_automattic_woocommerce_admin_features_productblockeditor_blockregistry(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_BlockRegistry {
...
fn create_automattic_woocommerce_admin_features_productblockeditor_producttemplate(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_ProductTemplate {
...
fn (mut this Class_Automattic_WooCommerce_Admin_Features_ProductBlockEditor_Init) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'supported_product_types' {
			this.supported_product_types = val
			return true
		}
		'product_templates' {
			this.product_templates = val
			return true
		}
