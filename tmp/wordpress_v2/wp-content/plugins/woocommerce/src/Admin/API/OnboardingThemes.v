		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					...
					rt.ArrayItem{ key: none, val: 'install_theme' },
				]) },
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					...
					rt.ArrayItem{ key: none, val: 'activate_theme' },
				]) },
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_OnboardingThemes', [
						'Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller',
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_OnboardingThemes) update_item_permissions_check(var_request rt.PhpVal) bool {
	...
	])))))
	{
		return (create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_rest_cannot_update'), rt.call_function('__', [
			rt.new_string('Sorry, you cannot manage themes.'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_OnboardingThemes) install_theme(var_request rt.PhpVal) rt.PhpVal {
	mut var_theme := rt.call_function('sanitize_text_field', [
		var_request.array_get(rt.new_string('theme')),
	])
	mut var_installed_themes := rt.call_function('wp_get_themes', []rt.PhpVal{})
	if rt.is_true(rt.call_function('in_array', [var_theme.clone(),
		rt.func_array_keys(var_installed_themes.clone()), rt.new_bool(true)]))
	{
		return rt.create_array([rt.ArrayItem{ key: 'slug', val: var_theme },
			rt.ArrayItem{ key: 'name', val: rt.call_method(var_installed_themes.array_get(var_theme),
				'get', [rt.new_string('Name')]) }, rt.ArrayItem{ key: 'status', val: 'success' }])
	}
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_api.clone()])) {
		return create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_rest_theme_install'), rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('The requested theme `%s` could not be installed. Theme API call failed.'),
				rt.new_string('woocommerce'),
			]),
			var_theme.clone(),
		]), rt.new_int(500))
	...
	mut var_upgrader :=
		create_automattic_woocommerce_admin_api_theme_upgrader(create_automattic_woocommerce_admin_api_automatic_upgrader_skin())
	mut var_result := var_upgrader.install(rt.get_property(var_api, 'download_link'))
	if rt.is_true(rt.call_function('is_wp_error', [var_result.clone()]))
		|| var_result.clone().is_null() {
		return create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_rest_theme_install'), rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('The requested theme `%s` could not be installed.'),
				rt.new_string('woocommerce'),
			]),
			var_theme.clone(),
		]), rt.new_int(500))
	...
	return rt.create_array([rt.ArrayItem{ key: 'slug', val: var_theme },
		rt.ArrayItem{ key: 'name', val: rt.get_property(var_api, 'name') },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_OnboardingThemes) activate_theme(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_theme.clone(), rt.func_array_keys(var_installed_themes.clone()),
		rt.new_bool(true)])))))
	{
		return create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_rest_invalid_theme'), rt.call_function('sprintf', [
			rt.call_function('__', [rt.new_string('Invalid theme %s.'),
				rt.new_string('woocommerce')]),
			var_theme.clone(),
		]), rt.new_int(404))
	}
	mut var_result := rt.call_function('switch_theme', [var_theme.clone()])
	if !(var_result.clone().is_null()) {
		return create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_rest_invalid_theme'), rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('The requested theme could not be activated.'),
				rt.new_string('woocommerce'),
			]),
			var_theme.clone(),
		]), rt.new_int(500))
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_OnboardingThemes) get_item_schema() rt.PhpVal {
	mut var_schema := rt.create_array([
		...
		rt.ArrayItem{ key: 'title', val: 'onboarding_theme' },
		rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			rt.ArrayItem{ key: 'slug', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Theme slug.'),
					rt.new_string('woocommerce'),
				]) },
...
fn create_automattic_woocommerce_admin_api_onboardingthemes(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_OnboardingThemes {
...
fn create_automattic_woocommerce_admin_api_theme_upgrader(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Theme_Upgrader {
...
fn create_automattic_woocommerce_admin_api_automatic_upgrader_skin(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Automatic_Upgrader_Skin {
