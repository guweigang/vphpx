...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_GenericQuery) get_data() rt.PhpVal {
	mut var_snake_name := rt.call_function('str_replace', [rt.new_string('-'),
		rt.new_string('_'), this.name])
	mut var_args := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_analytics_${var_snake_name.to_string()}_query_args'),
		this.get_query_vars(),
	])
	mut iife_temp_0 := Class_WC_Data_Store{}
	mut iife_result_0 := iife_temp_0.load(rt.new_string((rt.concat(rt.new_string('report-'),
		this.name)).str()))
	mut var_data_store := iife_result_0
	mut var_results := rt.call_method(var_data_store, 'get_data', [
		var_args.clone()])
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_analytics_${var_snake_name.to_string()}_select_query'),
		var_results.clone(),
		var_args.clone(),
	])
}
...
fn create_automattic_woocommerce_admin_api_reports_wc_object_query(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_WC_Object_Query {
