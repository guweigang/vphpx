...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_Query) get_default_query_vars() rt.PhpVal {
	rt.call_function('wc_deprecated_function', [rt.new_string(@STRUCT + '::' + @FN),
		rt.new_string('9.3.0'),
		rt.new_string('`Reports\\Customers\\Query` with a custom name, `GenericQuery`, `\\WC_Object_Query`, or direct `DataStore` use')])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_Query) get_data() rt.PhpVal {
	...
	mut var_args := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_analytics_customers_stats_query_args'),
		this.get_query_vars(),
	])
	mut iife_temp_0 :=
		Class_Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_WC_Data_Store{}
	mut iife_result_0 := iife_temp_0.load(rt.new_string('report-customers-stats'))
	mut var_data_store := iife_result_0
	mut var_results := rt.call_method(var_data_store, 'get_data', [
		var_args.clone()])
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_analytics_customers_stats_select_query'),
		var_results.clone(),
		var_args.clone(),
	])
}
...
fn create_automattic_woocommerce_admin_api_reports_customers_stats_query(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_Query {
...
fn create_automattic_woocommerce_admin_api_reports_customers_stats_wc_data_store(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_WC_Data_Store {
