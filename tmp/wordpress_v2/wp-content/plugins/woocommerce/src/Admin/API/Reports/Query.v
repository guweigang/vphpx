...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Query) construct(var_args rt.PhpVal) {
	...
	this.Class_Automattic_WooCommerce_Admin_API_Reports_WC_Object_Query.construct(var_args.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Query) get_data() rt.PhpVal {
	rt.call_function('wc_deprecated_function', [rt.new_string(@STRUCT + '::' + @FN),
		rt.new_string('9.3.0'),
		rt.new_string('`GenericQuery`, `\\WC_Object_Query`, or direct `DataStore` use')])
	return rt.new_object('Automattic_WooCommerce_Admin_API_Reports_WP_Error', []string{}, create_automattic_woocommerce_admin_api_reports_wp_error(rt.new_string('invalid-method'), rt.call_function('sprintf', [
		rt.call_function('__', [
