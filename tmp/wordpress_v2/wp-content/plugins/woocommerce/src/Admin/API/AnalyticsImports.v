	...
	rt.call_function('register_rest_route', [this.namespace,
		...
		rt.create_array([
			...
			rt.ArrayItem{ key: 'schema', val: rt.create_array([
				...
				rt.ArrayItem{ key: none, val: 'get_status_schema' },
			]) },
		])])
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/trigger'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{
					key: 'methods'
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					...
					rt.ArrayItem{ key: none, val: 'trigger_import' },
				]) },
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_AnalyticsImports', [
						'Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller',
			...
			rt.ArrayItem{ key: 'schema', val: rt.create_array([
				...
				rt.ArrayItem{ key: none, val: 'get_trigger_schema' },
			]) },
		])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_AnalyticsImports) permissions_check(var_request rt.PhpVal) bool {
	...
	{
		return (create_wp_error(rt.new_string('woocommerce_rest_cannot_access'), rt.call_function('__', [
			rt.new_string('Sorry, you cannot access analytics imports.'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_AnalyticsImports) get_status(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_mode := rt.new_string((if rt.is_true(var_is_scheduled_mode) {
		'scheduled'
	} else {
		'immediate'
	}).str())
	mut var_response := rt.create_array([rt.ArrayItem{ key: 'mode', val: var_mode },
		rt.ArrayItem{ key: 'last_processed_date', val: rt.new_null() },
		rt.ArrayItem{ key: 'next_scheduled', val: rt.new_null() },
		rt.ArrayItem{ key: 'import_in_progress_or_due', val: rt.new_null() }])
	if rt.is_true(var_is_scheduled_mode) {
		mut var_last_processed_gmt := rt.call_function('get_option', [
			Class_Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler.last_processed_order_date_option(),
			rt.new_null(),
		])
		var_response.array_set('last_processed_date', if var_last_processed_gmt.clone().is_string() && rt.is_true(var_last_processed_gmt) { rt.call_function('get_date_from_gmt', [
				var_last_processed_gmt.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_AnalyticsImports) trigger_import(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_is_scheduled_mode)))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_analytics_import_immediate_mode'), rt.call_function('__', [
			rt.new_string('Manual import is not available in immediate mode. Imports happen automatically.'),
			rt.new_string('woocommerce'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }])))
	}
	if this.is_import_in_progress_or_due() {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_analytics_import_in_progress'), rt.call_function('__', [
			rt.new_string('A batch import is already in progress or scheduled to run soon. Please wait for it to complete before triggering a new import.'),
			rt.new_string('woocommerce'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }])))
	}
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler{}
	mut iife_result_0 :=
		iife_temp_0.get_action(Class_Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler.process_pending_orders_batch_action())
	mut var_action_hook := iife_result_0
	if !(var_action_hook.clone().is_string()) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_analytics_import_invalid_action'), rt.call_function('__', [
			rt.new_string('Invalid action hook for batch import.'),
			rt.new_string('woocommerce'),
	...
	rt.call_method(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'queue', []rt.PhpVal{}),
		'cancel_all', [var_action_hook.clone(), rt.new_array(),
		rt.new_string((rt.get_static_prop('Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler',
			'group')).str())])
	mut iife_temp_1 := Class_Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler{}
	mut iife_result_1 := iife_temp_1.schedule_recurring_batch_processor()
	return rt.call_function('rest_ensure_response', [
		rt.create_array([rt.ArrayItem{ key: 'success', val: true },
			rt.ArrayItem{ key: 'message', val: rt.call_function('__', [
				rt.new_string('Batch import triggered successfully.'),
				rt.new_string('woocommerce'),
			]) }]),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_AnalyticsImports) is_scheduled_import_enabled() rt.PhpVal {
	return rt.identical(rt.new_string('yes'), rt.call_function('get_option', [
		Class_Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler.scheduled_import_option(),
		Class_Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler.scheduled_import_option_default_value(),
	]))
}

fn (mut this Class_Automattic_WooCommerce_Admin_API_AnalyticsImports) get_next_scheduled_time() rt.PhpVal {
	mut iife_temp_2 := Class_Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler{}
	mut iife_result_2 :=
		iife_temp_2.get_action(Class_Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler.process_pending_orders_batch_action())
	mut var_action_hook := iife_result_2
	if !(var_action_hook.clone().is_string()) {
		return rt.new_null()
	}
	mut var_next_time := rt.call_method(rt.call_method(rt.call_function('WC', []rt.PhpVal{}),
		'queue', []rt.PhpVal{}), 'get_next', [var_action_hook.clone(),
		rt.new_array(),
		rt.new_string((rt.get_static_prop('Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler',
			'group')).str())])
	...
	return rt.call_function('get_date_from_gmt', [
		rt.call_method(var_next_time, 'format', [rt.new_string('Y-m-d H:i:s')]),
		rt.new_string('Y-m-d H:i:s'),
	])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_AnalyticsImports) get_status_schema() rt.PhpVal {
	mut var_schema := rt.create_array([
		rt.ArrayItem{ key: '$schema', val: 'https://json-schema.org/draft-04/schema#' },
		rt.ArrayItem{ key: 'title', val: 'analytics_import_status' },
		rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			rt.ArrayItem{ key: 'mode', val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{ key: 'enum', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'scheduled' },
					rt.ArrayItem{ key: none, val: 'immediate' },
				]) },
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Current import mode.'),
					rt.new_string('woocommerce'),
				]) },
			...
			rt.ArrayItem{ key: 'last_processed_date', val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'string' },
					rt.ArrayItem{ key: none, val: 'null' },
				]) },
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Last processed order date (null in immediate mode).'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'view' },
			...
			rt.ArrayItem{ key: 'next_scheduled', val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'string' },
					rt.ArrayItem{ key: none, val: 'null' },
				]) },
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Next scheduled import time (null in immediate mode).'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'view' },
			...
			rt.ArrayItem{ key: 'import_in_progress_or_due', val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'boolean' },
					rt.ArrayItem{ key: none, val: 'null' },
				]) },
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Whether a batch import is currently running or scheduled to run within the next minute (null in immediate mode).'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'view' },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_AnalyticsImports) get_trigger_schema() rt.PhpVal {
	mut var_schema := rt.create_array([
		rt.ArrayItem{ key: '$schema', val: 'https://json-schema.org/draft-04/schema#' },
		rt.ArrayItem{ key: 'title', val: 'analytics_import_trigger' },
		rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			rt.ArrayItem{ key: 'success', val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: 'boolean' },
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Whether the trigger was successful.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'view' },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_AnalyticsImports) is_import_in_progress_or_due() bool {
	mut iife_temp_3 := Class_Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler{}
	mut iife_result_3 :=
		iife_temp_3.get_action(Class_Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler.process_pending_orders_batch_action())
	mut var_hook := iife_result_3
	if !(var_hook.clone().is_string()) {
		return false
	}
	mut var_in_progress_actions := rt.call_method(rt.call_method(rt.call_function('WC',
		[]rt.PhpVal{}), 'queue', []rt.PhpVal{}), 'search', [
		rt.create_array([rt.ArrayItem{ key: 'hook', val: var_hook },
			rt.ArrayItem{ key: 'status', val: 'in-progress' },
	...
	mut var_next_scheduled := rt.call_method(rt.call_method(rt.call_function('WC', []rt.PhpVal{}),
		'queue', []rt.PhpVal{}), 'get_next', [var_hook.clone(),
		rt.new_array(),
		rt.new_string((rt.get_static_prop('Automattic_WooCommerce_Internal_Admin_Schedulers_OrdersScheduler',
			'group')).str())])
	if rt.is_true(var_next_scheduled) {
		mut var_time_until_next := rt.sub(rt.call_method(var_next_scheduled, 'getTimestamp',
			[]rt.PhpVal{}), rt.call_function('time', []rt.PhpVal{}))
...
fn create_automattic_woocommerce_admin_api_analyticsimports(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_AnalyticsImports {
