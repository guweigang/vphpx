...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Variations_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_prepare_report_variations'),
		var_response.clone(),
		var_report.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Variations_Controller) prepare_reports_query(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_collection_params := rt.call_function('apply_filters', [
		rt.new_string('experimental_woocommerce_analytics_variations_collection_params'),
		this.get_collection_params(),
	])
	mut var_registered := rt.func_array_keys(var_collection_params.clone())
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Variations_Controller) prepare_links(var_object rt.PhpVal) rt.PhpVal {
	mut var_links := rt.create_array([
		rt.ArrayItem{ key: 'product', val: rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('/%s/%s/%d'),
					rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Variations_Controller', [
		...
		rt.ArrayItem{ key: 'variation', val: rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('/%s/%s/%d/%s/%d'),
					rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Variations_Controller', [
						'Automattic_WooCommerce_Admin_API_Reports_GenericController',
						'ExportableInterface',
					], &this), 'namespace'),
					rt.new_string('products'), var_object.array_get(rt.new_string('product_id')),
					rt.new_string('variation'), var_object.array_get(rt.new_string('variation_id'))]),
			]) },
		]) },
	])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Variations_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := rt.create_array([
		...
		rt.ArrayItem{ key: 'title', val: 'report_varitations' },
		...
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'variation_id', val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: 'integer' },
				rt.ArrayItem{ key: 'readonly', val: true },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'view' },
			...
			rt.ArrayItem{ key: 'extended_info', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'attributes', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Product attributes.'),
						rt.new_string('woocommerce'),
					]) },
				]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Variations_Controller) get_collection_params() rt.PhpVal {
	...
	var_params.array_set('product_includes', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit result set to items that have the specified parent product(s).'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'array' },
	...
	var_params.array_set('product_excludes', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string("Limit result set to items that don't have the specified parent product(s)."),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'array' },
		rt.ArrayItem{ key: 'items', val: rt.create_array([
	...
	var_params.array_set('extended_info', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Add additional piece of info about each variation to the report.'),
			rt.new_string('woocommerce'),
		]) },
	...
	var_params.array_set('attribute_is', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit result set to variations that include the specified attributes.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'array' },
		rt.ArrayItem{ key: 'items', val: rt.create_array([
	...
	var_params.array_set('attribute_is_not', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string("Limit result set to variations that don't include the specified attributes."),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'array' },
		rt.ArrayItem{ key: 'items', val: rt.create_array([
	...
	var_params.array_set('category_includes', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit result set to variations in the specified categories.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'array' },
		rt.ArrayItem{ key: 'sanitize_callback', val: 'wp_parse_id_list' },
	...
	var_params.array_set('category_excludes', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit result set to variations not in the specified categories.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'array' },
		rt.ArrayItem{ key: 'sanitize_callback', val: 'wp_parse_id_list' },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Variations_Controller) get_export_columns() rt.PhpVal {
	mut var_export_columns := rt.create_array([
		rt.ArrayItem{ key: 'product_name', val: rt.call_function('__', [
			rt.new_string('Product / Variation title'),
			rt.new_string('woocommerce'),
		]) },
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_report_variations_export_columns'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Variations_Controller) prepare_item_for_export(var_item rt.PhpVal) rt.PhpVal {
	mut var_product_name :=
		var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('name'))
	mut var_separator := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_product_variation_title_attributes_separator'),
		rt.new_string(' - '),
		create_automattic_woocommerce_admin_api_reports_variations_wc_product(),
	])
	if !(!rt.is_true(var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('attributes'))))
		&& rt.is_true(rt.identical(rt.call_function('strpos', [var_product_name.clone(), var_separator.clone()]), rt.new_bool(false))) {
		mut var_attributes := rt.new_array()
		mut iter_2 :=
			var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('attributes')).iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_attribute := item_2.val
			if !rt.is_true(var_attribute.array_get(rt.new_string('option'))) {
				var_attributes.array_push(rt.call_function('sprintf', [
					rt.call_function('__', [rt.new_string('Any %s'),
						rt.new_string('woocommerce')]),
					rt.call_function('ucfirst', [var_attribute.array_get(rt.new_string('name'))]),
				]))
			} else {
				var_attributes.array_push(var_attribute.array_get(rt.new_string('option')))
			}
		}
		var_product_name = rt.concat(var_product_name, rt.new_string(var_separator.str() +
			(rt.call_function('implode', [rt.new_string(', '), var_attributes.clone()])).str()))
	}
	mut iife_temp_0 := Class_Automattic_WooCommerce_Admin_API_Reports_Variations_Controller{}
	mut iife_result_0 :=
		iife_temp_0.csv_number_format(var_item.array_get(rt.new_string('net_revenue')))
	mut var_export_item := rt.create_array([
		...
		rt.ArrayItem{ key: 'net_revenue', val: iife_result_0 },
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_report_variations_prepare_export_item'),
...
fn create_automattic_woocommerce_admin_api_reports_variations_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Variations_Controller {
...
fn create_automattic_woocommerce_admin_api_reports_variations_wc_product(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Variations_WC_Product {
