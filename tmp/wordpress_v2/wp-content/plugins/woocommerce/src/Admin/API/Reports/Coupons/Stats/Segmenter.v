...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter) get_segment_selections_product_level(var_products_table rt.PhpVal) rt.PhpVal {
	mut var_columns_mapping := rt.create_array([
		rt.ArrayItem{
			key: 'amount'
			val: 'SUM(${var_products_table.to_string()}.coupon_amount) as amount'
		},
	])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter) get_segment_selections_order_level(var_coupons_lookup_table rt.PhpVal) rt.PhpVal {
	mut var_columns_mapping := rt.create_array([
		rt.ArrayItem{
			key: 'coupons_count'
			val: 'COUNT(DISTINCT ${var_coupons_lookup_table.to_string()}.coupon_id) as coupons_count'
		},
		rt.ArrayItem{
			key: 'orders_count'
			val: 'COUNT(DISTINCT ${var_coupons_lookup_table.to_string()}.order_id) as orders_count'
		},
	])
	return var_columns_mapping.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter) segment_selections_orders(var_coupons_lookup_table rt.PhpVal, var_overrides rt.PhpVal) rt.PhpVal {
	mut var_columns_mapping := rt.create_array([
		rt.ArrayItem{
			key: 'amount'
			val: 'SUM(${var_coupons_lookup_table.to_string()}.discount_amount) as amount'
		},
		rt.ArrayItem{
			key: 'coupons_count'
			val: 'COUNT(DISTINCT ${var_coupons_lookup_table.to_string()}.coupon_id) as coupons_count'
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter) get_product_related_totals_segments(var_segmenting_selections rt.PhpVal, var_segmenting_from rt.PhpVal, var_segmenting_where rt.PhpVal, var_segmenting_groupby rt.PhpVal, var_segmenting_dimension_name rt.PhpVal, var_table_name rt.PhpVal, var_totals_query rt.PhpVal, var_unique_orders_table rt.PhpVal) rt.PhpVal {
	...
	mut var_segments_products := rt.call_method(var_wpdb, 'get_results', [
		rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('SELECT\n\t\t\t\t\t\t'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter) get_product_related_intervals_segments(var_segmenting_selections rt.PhpVal, var_segmenting_from rt.PhpVal, var_segmenting_where rt.PhpVal, var_segmenting_groupby rt.PhpVal, var_segmenting_dimension_name rt.PhpVal, var_table_name rt.PhpVal, var_intervals_query rt.PhpVal, var_unique_orders_table rt.PhpVal) rt.PhpVal {
	...
	mut var_segments_products := rt.call_method(var_wpdb, 'get_results', [
		rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('SELECT\n\t\t\t\t\t\t'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter) get_segments(var_type rt.PhpVal, var_query_params rt.PhpVal, var_table_name rt.PhpVal) rt.PhpVal {
	mut var_wpdb := rt.new_null()
	if !(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_isset(rt.new_string('segmentby')))
		|| rt.is_true(rt.identical(rt.new_string(''), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get(rt.new_string('segmentby')))) {
		return rt.new_array()
	}
	mut var_segments := rt.new_null()
	mut var_product_segmenting_table := rt.new_string((rt.get_property(var_wpdb, 'prefix')).str() +
		'wc_order_product_lookup')
	mut var_unique_orders_table := rt.new_string('')
	mut var_segmenting_where := rt.new_string('')
	if rt.is_true(rt.identical(rt.new_string('product'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter', [
	...
	], &this), 'query_args').array_get(rt.new_string('segmentby'))))
	{
		...
		mut var_order_level_columns :=
			this.get_segment_selections_order_level(var_table_name.clone())
	...
	} else if rt.is_true(rt.identical(rt.new_string('variation'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter', [
		'Automattic_WooCommerce_Admin_API_Reports_Segmenter',
	], &this), 'query_args').array_get(rt.new_string('segmentby'))))
	{
		if !(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_isset(rt.new_string('product_includes')))
			|| !(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get(rt.new_string('product_includes')).is_array())
			|| rt.is_true(rt.new_bool(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get(rt.new_string('product_includes')).array_count() != 1)) {
		...
		var_product_level_columns =
			this.get_segment_selections_product_level(var_product_segmenting_table.clone())
		var_order_level_columns = this.get_segment_selections_order_level(var_table_name.clone())
		...
		var_segmenting_where = rt.new_string((rt.concat(rt.concat(rt.concat(rt.new_string('AND '),
			var_product_segmenting_table), rt.new_string('.product_id = ')), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter', [
			'Automattic_WooCommerce_Admin_API_Reports_Segmenter',
		...
		var_segments = this.get_product_related_segments(var_type.clone(),
			...
			var_segmenting_dimension_name.clone(), var_table_name.clone(),
			var_query_params.clone(), var_unique_orders_table.clone())
	} else if rt.is_true(rt.identical(rt.new_string('category'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter', [
		'Automattic_WooCommerce_Admin_API_Reports_Segmenter',
	], &this), 'query_args').array_get(rt.new_string('segmentby'))))
	{
		var_product_level_columns =
		...
		var_segments = this.get_product_related_segments(var_type.clone(),
			...
			var_segmenting_dimension_name.clone(), var_table_name.clone(),
			var_query_params.clone(), var_unique_orders_table.clone())
	} else if rt.is_true(rt.identical(rt.new_string('coupon'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Stats_Segmenter', [
		'Automattic_WooCommerce_Admin_API_Reports_Segmenter',
	], &this), 'query_args').array_get(rt.new_string('segmentby'))))
	{
		...
		var_segmenting_groupby = rt.new_string('${var_table_name.to_string()}.coupon_id')
