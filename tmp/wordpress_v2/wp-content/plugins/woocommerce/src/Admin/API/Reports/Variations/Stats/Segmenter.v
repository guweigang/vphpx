...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Variations_Stats_Segmenter) get_segments(var_type rt.PhpVal, var_query_params rt.PhpVal, var_table_name rt.PhpVal) rt.PhpVal {
	mut var_wpdb := rt.new_null()
	if !(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Variations_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_isset(rt.new_string('segmentby')))
		|| rt.is_true(rt.identical(rt.new_string(''), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Variations_Stats_Segmenter', ['Automattic_WooCommerce_Admin_API_Reports_Segmenter'], &this), 'query_args').array_get(rt.new_string('segmentby')))) {
		return rt.new_array()
	}
	...
	if rt.is_true(rt.identical(rt.new_string('variation'), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Variations_Stats_Segmenter', [
	...
	], &this), 'query_args').array_get(rt.new_string('segmentby'))))
	{
		...
		mut var_segmenting_groupby := rt.new_string(var_product_segmenting_table.str() +
			'.variation_id')
		mut var_segmenting_dimension_name := rt.new_string('variation_id')
		if rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Variations_Stats_Segmenter', [
			'Automattic_WooCommerce_Admin_API_Reports_Segmenter',
		], &this), 'query_args').array_isset(rt.new_string('variation_includes'))
		{
			mut var_variation_ids := rt.call_function('implode', [
				rt.new_string(','), this.get_all_segments()])
			var_segmenting_where =
				rt.new_string(' AND ${var_product_segmenting_table.to_string()}.variation_id IN ( ${var_variation_ids.to_string()} )')
		}
