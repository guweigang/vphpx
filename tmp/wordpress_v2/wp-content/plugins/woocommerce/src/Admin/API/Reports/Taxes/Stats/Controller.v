...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Controller) construct() {
	rt.call_function('add_filter', [
		rt.new_string('woocommerce_analytics_taxes_stats_select_query'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Controller', [
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Controller) set_default_report_data(var_results rt.PhpVal) rt.PhpVal {
	...
	if !rt.is_true(var_results_mutated) {
		...
		rt.set_property(var_results_mutated, 'totals',
			create_automattic_woocommerce_admin_api_reports_taxes_stats_stdclass())
		...
		rt.set_property(rt.get_property(var_results_mutated, 'totals'), 'shipping_tax',
			rt.new_int(0))
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Controller) prepare_reports_query(var_request rt.PhpVal) rt.PhpVal {
	...
	var_args.array_set('taxes', rt.cast_array(var_request.array_get(rt.new_string('taxes'))))
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_prepare_report_taxes_stats'),
		var_response.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Controller) get_item_properties_schema() rt.PhpVal {
	return rt.create_array([
		...
		rt.ArrayItem{ key: 'order_tax', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
		...
		rt.ArrayItem{ key: 'tax_codes', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Amount of tax codes.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'integer' },
			rt.ArrayItem{ key: 'context', val: rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_Controller) get_collection_params() rt.PhpVal {
	...
	var_params.array_set('taxes', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit result set to all items that have the specified term assigned in the taxes taxonomy.'),
			rt.new_string('woocommerce'),
		]) },
	...
	var_params.array_set('segmentby', rt.create_array([
		...
		rt.ArrayItem{ key: 'enum', val: rt.create_array([
			rt.ArrayItem{ key: none, val: 'tax_rate_id' },
		]) },
		rt.ArrayItem{ key: 'validate_callback', val: 'rest_validate_request_arg' },
	]))
...
fn create_automattic_woocommerce_admin_api_reports_taxes_stats_stdclass(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_stdClass {
