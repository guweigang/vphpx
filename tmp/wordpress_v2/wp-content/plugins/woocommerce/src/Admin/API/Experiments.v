...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Experiments) register_routes() {
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/assignment'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_Experiments', [
						'Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller',
					], &this) },
					rt.ArrayItem{ key: none, val: 'get_assignment' },
				]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Experiments) get_assignment(var_request rt.PhpVal) rt.PhpVal {
	...
	if !(var_args.array_isset(rt.new_string('experiment_name'))) {
		return rt.new_object('Automattic_WooCommerce_Admin_API_WP_Error', []string{}, create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_rest_experiment_name_required'), rt.call_function('__', [
			rt.new_string('Sorry, experiment_name is required.'),
			rt.new_string('woocommerce'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }])))
	}
	var_args.array_unset(rt.new_string('rest_route'))
	mut var_abtest := create_automattic_woocommerce_admin_api_woocommerce_admin_experimental_abtest(if !(rt.call_method(var_request, 'get_param', [
		rt.new_string('anon_id'),
	])).is_null() { rt.call_method(var_request, 'get_param', [
			rt.new_string('anon_id')]) } else { rt.new_string('') }, rt.new_string('woocommerce'),
		rt.new_bool(true), rt.new_bool(true))
	mut var_response := var_abtest.request_assignment(var_args.clone())
	if rt.is_true(rt.call_function('is_wp_error', [var_response.clone()])) {
		return var_response.clone()
	}
	return rt.call_function('json_decode', [var_response.array_get(rt.new_string('body')),
		rt.new_bool(true)])
}
...
fn create_automattic_woocommerce_admin_api_experiments(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Experiments {
