...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Coupons) get_collection_params() rt.PhpVal {
	mut var_params :=
		this.Class_Automattic_WooCommerce_Admin_API_WC_REST_Coupons_Controller.get_collection_params()
	var_params.array_set('search', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit results to coupons with codes matching a given string.'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Coupons) prepare_objects_query(var_request rt.PhpVal) rt.PhpVal {
	mut var_args :=
		this.Class_Automattic_WooCommerce_Admin_API_WC_REST_Coupons_Controller.prepare_objects_query(var_request.clone())
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Coupons) get_items(var_request rt.PhpVal) rt.PhpVal {
	rt.call_function('add_filter', [rt.new_string('posts_where'),
		rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT },
			rt.ArrayItem{ key: none, val: 'add_wp_query_search_code_filter' }]),
		rt.new_int(10), rt.new_int(2)])
	mut var_response :=
		this.Class_Automattic_WooCommerce_Admin_API_WC_REST_Coupons_Controller.get_items(var_request.clone())
...
fn create_automattic_woocommerce_admin_api_coupons(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Coupons {
...
fn create_automattic_woocommerce_admin_api_wc_rest_coupons_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_WC_REST_Coupons_Controller {
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Coupons) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'add_wp_query_search_code_filter' {
			...
			return Class_Automattic_WooCommerce_Admin_API_Coupons.add_wp_query_search_code_filter(dispatch_arg_0,
				dispatch_arg_1)
		}
		else {
			return none
