...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller) get_datastore_data(var_query_args rt.PhpVal) rt.PhpVal {
	mut var_query :=
		create_automattic_woocommerce_admin_api_reports_orders_query(var_query_args.clone())
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller) prepare_reports_query(var_request rt.PhpVal) rt.PhpVal {
	...
	var_args.array_set('extended_info', var_request.array_get(rt.new_string('extended_info')))
	...
	var_args.array_set('order_includes', var_request.array_get(rt.new_string('order_includes')))
	var_args.array_set('order_excludes', var_request.array_get(rt.new_string('order_excludes')))
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	var_report_mutated.array_set('order_number',
		this.get_order_number(var_report_mutated.array_get(rt.new_string('order_id'))))
	var_report_mutated.array_set('total_formatted',
		this.get_total_formatted(var_report_mutated.array_get(rt.new_string('order_id'))))
	mut var_response := this.Class_Automattic_WooCommerce_Admin_API_Reports_GenericController.prepare_item_for_response(var_report_mutated.clone(),
		var_request.clone())
	rt.call_method(var_response, 'add_links', [
		this.prepare_links(var_report_mutated.clone()),
	])
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_prepare_report_orders'),
		var_response.clone(),
		...
		rt.ArrayItem{ key: 'order', val: rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('/%s/orders/%d'),
					rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Orders_Controller', [
						'Automattic_WooCommerce_Admin_API_Reports_GenericController',
						'ExportableInterface',
					], &this), 'namespace'),
					var_object.array_get(rt.new_string('order_id'))]),
			]) },
		]) },
	])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := rt.create_array([
		...
		rt.ArrayItem{ key: 'title', val: 'report_orders' },
		...
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'date_created', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string("Date the order was created, in the site's timezone."),
					rt.new_string('woocommerce'),
				]) },
			...
			rt.ArrayItem{ key: 'date_created_gmt', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Date the order was created, as GMT.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'date-time' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'net_total', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Net total revenue.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'float' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'view' },
					rt.ArrayItem{ key: none, val: 'edit' },
				]) },
			...
			rt.ArrayItem{ key: 'total_formatted', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Net total revenue (formatted).'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'customer_type', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Returning or new customer.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'extended_info', val: rt.create_array([
				rt.ArrayItem{ key: 'products', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('List of order product IDs, names, quantities.'),
						rt.new_string('woocommerce'),
					]) },
				]) },
				rt.ArrayItem{ key: 'coupons', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('List of order coupons.'),
						rt.new_string('woocommerce'),
					]) },
				]) },
				rt.ArrayItem{ key: 'customer', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Order customer information.'),
						rt.new_string('woocommerce'),
					]) },
				]) },
				rt.ArrayItem{ key: 'attribution', val: rt.create_array([
					rt.ArrayItem{ key: 'type', val: 'object' },
					rt.ArrayItem{ key: 'readonly', val: true },
					rt.ArrayItem{ key: 'context', val: rt.create_array([
						rt.ArrayItem{ key: none, val: 'view' },
					...
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Order attribution information.'),
						rt.new_string('woocommerce'),
					]) },
				]) },
			]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller) get_collection_params() rt.PhpVal {
	...
	var_params.array_get_mut('orderby').array_set('enum', this.apply_custom_orderby_filters(rt.create_array([
		...
		rt.ArrayItem{ key: none, val: 'net_total' },
	])))
	...
	mut iife_temp_0 := Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller{}
	...
	mut iife_temp_1 := Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller{}
	...
	var_params.array_set('customer_type', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit result set to returning or new customers.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'string' },
	...
	var_params.array_set('extended_info', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Add additional piece of info about each coupon to the report.'),
			rt.new_string('woocommerce'),
		]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller) get_customer_name(var_customer rt.PhpVal) string {
	return (var_customer.array_get(rt.new_string('first_name'))).str() + ' ' +
		(var_customer.array_get(rt.new_string('last_name'))).str()
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller) get_products(var_products rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		var_products_list.array_push(rt.call_function('sprintf', [
			rt.call_function('__', [rt.new_string('%1$s× %2$s'),
				rt.new_string('woocommerce')]),
			var_product.array_get(rt.new_string('quantity')),
			var_product.array_get(rt.new_string('name')),
		]))
	}
	return rt.call_function('implode', [rt.new_string(', '), var_products_list.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller) get_coupons(var_coupons rt.PhpVal) rt.PhpVal {
	return rt.call_function('implode', [rt.new_string(', '),
		rt.call_function('wp_list_pluck', [var_coupons.clone(),
			rt.new_string('code')])])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller) get_export_columns() rt.PhpVal {
	mut var_export_columns := rt.create_array([
		...
		rt.ArrayItem{ key: 'total_formatted', val: rt.call_function('__', [
			rt.new_string('N. Revenue (formatted)'),
			rt.new_string('woocommerce'),
		]) },
		...
		rt.ArrayItem{ key: 'customer_name', val: rt.call_function('__', [
			rt.new_string('Customer'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'customer_type', val: rt.call_function('__', [
			rt.new_string('Customer type'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'products', val: rt.call_function('__', [
			rt.new_string('Product(s)'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'num_items_sold', val: rt.call_function('__', [
			rt.new_string('Items sold'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'coupons', val: rt.call_function('__', [
			rt.new_string('Coupon(s)'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'net_total', val: rt.call_function('__', [
			rt.new_string('Net Sales'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'attribution', val: rt.call_function('__', [
			rt.new_string('Attribution'),
			rt.new_string('woocommerce'),
		]) },
	])
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_report_orders_export_columns'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller) prepare_item_for_export(var_item rt.PhpVal) rt.PhpVal {
	mut var_export_item := rt.create_array([
		rt.ArrayItem{ key: 'date_created', val: var_item.array_get(rt.new_string('date')) },
		rt.ArrayItem{ key: 'order_number', val: var_item.array_get(rt.new_string('order_number')) },
		rt.ArrayItem{
			key: 'total_formatted'
			val: var_item.array_get(rt.new_string('total_formatted'))
		},
		rt.ArrayItem{ key: 'status', val: var_item.array_get(rt.new_string('status')) },
		rt.ArrayItem{
			key: 'customer_name'
			val: if var_item.array_get(rt.new_string('extended_info')).array_isset(rt.new_string('customer')) {
				this.get_customer_name(var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('customer')))
			} else {
				rt.new_null()
			}
		},
		rt.ArrayItem{ key: 'customer_type', val: var_item.array_get(rt.new_string('customer_type')) },
		rt.ArrayItem{
			key: 'products'
			val: if var_item.array_get(rt.new_string('extended_info')).array_isset(rt.new_string('products')) {
				this.get_products(var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('products')))
			} else {
				rt.new_null()
			}
		},
		rt.ArrayItem{
			key: 'num_items_sold'
			val: var_item.array_get(rt.new_string('num_items_sold'))
		},
		rt.ArrayItem{
			key: 'coupons'
			val: if var_item.array_get(rt.new_string('extended_info')).array_isset(rt.new_string('coupons')) {
				this.get_coupons(var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('coupons')))
			} else {
				rt.new_null()
			}
		},
		rt.ArrayItem{ key: 'net_total', val: var_item.array_get(rt.new_string('net_total')) },
		rt.ArrayItem{
			key: 'attribution'
			val: var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('attribution')).array_get(rt.new_string('origin'))
		},
	])
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_report_orders_prepare_export_item'),
...
fn create_automattic_woocommerce_admin_api_reports_orders_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Controller {
...
fn create_automattic_woocommerce_admin_api_reports_orders_query(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Orders_Query {
