...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Categories_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_prepare_report_categories'),
		var_response.clone(),
		var_report.clone(),
		var_request.clone(),
	])
	...
	mut var_links := rt.create_array([
		rt.ArrayItem{ key: 'category', val: rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('/%s/products/categories/%d'),
					rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Categories_Controller', [
						'Automattic_WooCommerce_Admin_API_Reports_GenericController',
						'ExportableInterface',
					], &this), 'namespace'),
					var_object.array_get(rt.new_string('category_id'))]),
			]) },
		]) },
	])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Categories_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := rt.create_array([
		...
		rt.ArrayItem{ key: 'title', val: 'report_categories' },
		...
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'items_sold', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Amount of items sold.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'integer' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'products_count', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Categories_Controller) get_collection_params() rt.PhpVal {
	...
	var_params.array_get_mut('orderby').array_set('enum', this.apply_custom_orderby_filters(rt.create_array([
		rt.ArrayItem{ key: none, val: 'category_id' },
	...
	var_params.array_set('interval', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Time interval to use for buckets in the returned data.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'string' },
		rt.ArrayItem{ key: 'default', val: 'week' },
		rt.ArrayItem{ key: 'enum', val: rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'quarter' },
			rt.ArrayItem{ key: none, val: 'year' },
		]) },
		rt.ArrayItem{ key: 'validate_callback', val: 'rest_validate_request_arg' },
	]))
	mut iife_temp_0 := Class_Automattic_WooCommerce_Admin_API_Reports_Categories_Controller{}
	...
	mut iife_temp_1 := Class_Automattic_WooCommerce_Admin_API_Reports_Categories_Controller{}
	...
	var_params.array_set('categories', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit result set to all items that have the specified term assigned in the categories taxonomy.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'array' },
	...
	var_params.array_set('extended_info', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Add additional piece of info about each category to the report.'),
			rt.new_string('woocommerce'),
		]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Categories_Controller) get_export_columns() rt.PhpVal {
	mut var_export_columns := rt.create_array([
		...
		rt.ArrayItem{ key: 'net_revenue', val: rt.call_function('__', [
			rt.new_string('Net Revenue'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'products_count', val: rt.call_function('__', [
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_report_categories_export_columns'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Categories_Controller) prepare_item_for_export(var_item rt.PhpVal) rt.PhpVal {
	mut var_export_item := rt.create_array([
		...
		rt.ArrayItem{
			key: 'products_count'
			val: var_item.array_get(rt.new_string('products_count'))
		},
		rt.ArrayItem{ key: 'orders_count', val: var_item.array_get(rt.new_string('orders_count')) },
	])
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_report_categories_prepare_export_item'),
...
fn create_automattic_woocommerce_admin_api_reports_categories_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Categories_Controller {
