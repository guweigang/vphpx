...
fn (mut this Class_Automattic_WooCommerce_Admin_API_ProductAttributeTerms) register_routes() {
	...
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('products/attributes/(?P<slug>[a-z0-9_\\-]+)/terms'),
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_ProductAttributeTerms', [
						'Automattic_WooCommerce_Admin_API_WC_REST_Product_Attribute_Terms_Controller',
					], &this) },
				...
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					...
					rt.ArrayItem{ key: none, val: 'get_custom_attribute_permissions_check' },
				]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_ProductAttributeTerms) get_item_schema() rt.PhpVal {
	mut var_schema :=
		this.Class_Automattic_WooCommerce_Admin_API_WC_REST_Product_Attribute_Terms_Controller.get_item_schema()
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_ProductAttributeTerms) get_custom_attribute_values(var_slug rt.PhpVal) rt.PhpVal {
	...
	mut var_attribute := this.get_custom_attribute_by_slug(var_slug.clone())
	...
	mut var_query_results := rt.call_method(var_wpdb, 'get_results', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.concat(rt.concat(rt.new_string('SELECT meta_value, COUNT(meta_id) AS product_count\n\t\t\t\tFROM '), rt.get_property(var_wpdb,
				'postmeta')),
				rt.new_string("\n\t\t\t\tWHERE meta_key = %s\n\t\t\t\tAND meta_value != ''\n\t\t\t\tGROUP BY meta_value")),
			rt.new_string('attribute_' + (rt.call_function('esc_sql', [var_slug.clone()])).str()),
		]),
		rt.get_constant('OBJECT_K'),
	])
	mut var_defined_values := rt.call_function('wc_get_text_attributes', [
		var_attribute.array_get(var_slug).array_get(rt.new_string('value')),
	])
	mut iter_1 := var_defined_values.iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_defined_value := item_1.val
		if rt.is_true(rt.new_bool(var_query_results.clone().array_isset(var_defined_value.clone()))) {
			continue
		}
		var_query_results.array_set(var_defined_value, rt.array_to_object(rt.create_array([
			rt.ArrayItem{ key: 'meta_value', val: var_defined_value },
			rt.ArrayItem{ key: 'product_count', val: 0 },
		])))
	}
	mut iter_2 := var_query_results.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_term := item_2.val
		mut var_term_value := item_2.key
		mut var_data := rt.create_array([rt.ArrayItem{ key: 'id', val: var_term_value },
			rt.ArrayItem{ key: 'name', val: var_term_value },
			rt.ArrayItem{ key: 'slug', val: var_term_value },
			rt.ArrayItem{ key: 'description', val: '' }, rt.ArrayItem{ key: 'menu_order', val: 0 },
			rt.ArrayItem{
				key: 'count'
				val: rt.new_int((rt.get_property(var_term, 'product_count')).to_i64())
			}])
			...
			rt.create_array([
				rt.ArrayItem{ key: 'collection', val: rt.create_array([
					rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
						rt.new_string(
							(this.namespace).str() + '/products/attributes/' + var_slug.str() +
							'/terms'),
					]) },
				]) },
			]),
		...
		var_attribute_values.array_set(var_term_value, var_response.clone())
	}
	return rt.call_function('array_values', [var_attribute_values.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_ProductAttributeTerms) get_item_by_slug(var_request rt.PhpVal) rt.PhpVal {
	return this.get_custom_attribute_values(var_request.array_get(rt.new_string('slug')))
}
...
fn create_automattic_woocommerce_admin_api_productattributeterms(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_ProductAttributeTerms {
...
fn create_automattic_woocommerce_admin_api_wc_rest_product_attribute_terms_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_WC_REST_Product_Attribute_Terms_Controller {
