...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Coupons_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_prepare_report_coupons'),
		var_response.clone(),
		var_report.clone(),
		var_request.clone(),
	])
	...
	mut var_links := rt.create_array([
		rt.ArrayItem{ key: 'coupon', val: rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('/%s/coupons/%d'),
					rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Coupons_Controller', [
						'Automattic_WooCommerce_Admin_API_Reports_GenericController',
						'ExportableInterface',
					], &this), 'namespace'),
					var_object.array_get(rt.new_string('coupon_id'))]),
			]) },
		]) },
	])
		...
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'extended_info', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'date_created', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Coupon creation date.'),
						rt.new_string('woocommerce'),
					]) },
				]) },
				rt.ArrayItem{ key: 'date_created_gmt', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Coupon creation date in GMT.'),
						rt.new_string('woocommerce'),
					]) },
				]) },
				rt.ArrayItem{ key: 'date_expires', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Coupon expiration date.'),
						rt.new_string('woocommerce'),
					]) },
				]) },
				rt.ArrayItem{ key: 'date_expires_gmt', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Coupon expiration date in GMT.'),
						rt.new_string('woocommerce'),
					]) },
				]) },
				rt.ArrayItem{ key: 'discount_type', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'enum', val: rt.func_array_keys(rt.call_function('wc_get_coupon_types',
						[]rt.PhpVal{})) },
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Coupon discount type.'),
						rt.new_string('woocommerce'),
					]) },
				]) },
			]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Coupons_Controller) get_export_columns() rt.PhpVal {
	mut var_export_columns := rt.create_array([
		...
		rt.ArrayItem{ key: 'amount', val: rt.call_function('__', [
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_report_coupons_export_columns'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Coupons_Controller) prepare_item_for_export(var_item rt.PhpVal) rt.PhpVal {
	mut var_date_expires := if !rt.is_true(var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('date_expires'))) {
		rt.call_function('__', [rt.new_string('N/A'), rt.new_string('woocommerce')])
	} else {
		var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('date_expires'))
	}
	mut var_export_item := rt.create_array([
		rt.ArrayItem{
			key: 'code'
			val: var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('code'))
		},
		rt.ArrayItem{ key: 'orders_count', val: var_item.array_get(rt.new_string('orders_count')) },
		rt.ArrayItem{ key: 'amount', val: var_item.array_get(rt.new_string('amount')) },
		rt.ArrayItem{
			key: 'created'
			val: var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('date_created'))
		},
		rt.ArrayItem{ key: 'expires', val: var_date_expires },
		rt.ArrayItem{
			key: 'type'
			val: var_item.array_get(rt.new_string('extended_info')).array_get(rt.new_string('discount_type'))
		},
	])
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_report_coupons_prepare_export_item'),
...
fn create_automattic_woocommerce_admin_api_reports_coupons_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Coupons_Controller {
