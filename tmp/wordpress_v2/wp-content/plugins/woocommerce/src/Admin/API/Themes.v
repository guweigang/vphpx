		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_Themes', [
						'Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller',
					], &this) },
					rt.ArrayItem{ key: none, val: 'upload_theme' },
				]) },
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_Themes', [
						'Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller',
					], &this) },
					rt.ArrayItem{ key: none, val: 'upload_theme_permissions_check' },
				]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Themes) upload_theme(var_request rt.PhpVal) rt.PhpVal {
	...
	if !(var__FILES.array_isset(rt.new_string('pluginzip')))
		|| !(var__FILES.array_get(rt.new_string('pluginzip')).array_isset(rt.new_string('tmp_name')))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_uploaded_file', [var__FILES.array_get(rt.new_string('pluginzip')).array_get(rt.new_string('tmp_name'))])))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_file', [var__FILES.array_get(rt.new_string('pluginzip')).array_get(rt.new_string('tmp_name'))]))))) {
		return rt.new_object('Automattic_WooCommerce_Admin_API_WP_Error', []string{}, create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_rest_invalid_file'), rt.call_function('__', [
			rt.new_string('Specified file failed upload test.'),
			rt.new_string('woocommerce'),
		])))
	}
	...
	rt.include_file((rt.get_constant('ABSPATH')).str() + '/wp-admin/includes/theme-install.php',
		'2')
	...
	rt.include_file((rt.get_constant('ABSPATH')).str() + '/wp-admin/includes/class-wp-upgrader.php',
		'2')
	rt.include_file(
		(rt.get_constant('ABSPATH')).str() + '/wp-admin/includes/class-theme-upgrader.php', '2')
	rt.get_superglobal('_GET').array_set('package', true)
	mut var_file_upload := create_automattic_woocommerce_admin_api_file_upload_upgrader(rt.new_string('pluginzip'),
		rt.new_string('package'))
	mut var_upgrader :=
		create_automattic_woocommerce_admin_overrides_themeupgrader(create_automattic_woocommerce_admin_overrides_themeupgraderskin())
	mut var_install := var_upgrader.install(rt.get_property(var_file_upload, 'package'))
	if rt.is_true(var_install) || rt.is_true(rt.call_function('is_wp_error', [var_install.clone()])) {
		var_file_upload.cleanup()
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_wp_error', [var_install.clone()])))))
		&& var_install.array_isset(rt.new_string('destination_name')) {
		mut var_theme := var_install.array_get(rt.new_string('destination_name'))
		mut var_result := rt.create_array([rt.ArrayItem{ key: 'status', val: 'success' },
			rt.ArrayItem{
				key: 'message'
				val: rt.get_property(var_upgrader, 'strings').array_get(rt.new_string('process_success'))
			}, rt.ArrayItem{ key: 'theme', val: var_theme }])
		rt.call_function('do_action', [rt.new_string('woocommerce_theme_installed'),
			var_theme.clone()])
	} else {
		if rt.is_true(rt.call_function('is_wp_error', [var_install.clone()]))
			&& rt.is_true(rt.call_method(var_install, 'get_error_code', []rt.PhpVal{})) {
			mut var_error_message := if rt.get_property(var_upgrader, 'strings').array_isset(rt.call_method(var_install,
				'get_error_code', []rt.PhpVal{}))
			{
				rt.get_property(var_upgrader, 'strings').array_get(rt.call_method(var_install,
					'get_error_code', []rt.PhpVal{}))
			} else {
				rt.call_method(var_install, 'get_error_data', []rt.PhpVal{})
			}
		} else {
			var_error_message =
				rt.get_property(var_upgrader, 'strings').array_get(rt.new_string('process_failed'))
		}
		var_result = rt.create_array([rt.ArrayItem{ key: 'status', val: 'error' },
			rt.ArrayItem{ key: 'message', val: var_error_message }])
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Themes) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_prepare_themes'),
		var_response.clone(),
		var_item.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Themes) get_item_schema() rt.PhpVal {
	mut var_schema := rt.create_array([
		...
		rt.ArrayItem{ key: 'title', val: 'upload_theme' },
		rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			rt.ArrayItem{ key: 'status', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Theme installation status.'),
					rt.new_string('woocommerce'),
				]) },
			...
			rt.ArrayItem{ key: 'message', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Theme installation message.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Themes) get_collection_params() rt.PhpVal {
	...
	var_params.array_set('pluginzip', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('A zip file of the theme to be uploaded.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'file' },
		rt.ArrayItem{ key: 'validate_callback', val: 'rest_validate_request_arg' },
	]))
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_themes_collection_params'),
		var_params.clone(),
	])
}
...
fn create_automattic_woocommerce_admin_api_themes(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Themes {
...
fn create_automattic_woocommerce_admin_api_file_upload_upgrader(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_File_Upload_Upgrader {
...
fn create_automattic_woocommerce_admin_overrides_themeupgraderskin(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_Overrides_ThemeUpgraderSkin {
