...
fn (mut this Class_Automattic_WooCommerce_Admin_API_MobileAppMagicLink) register_routes() {
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/send-magic-link'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_MobileAppMagicLink', [
						'Automattic_WooCommerce_Admin_API_WC_REST_Data_Controller',
					], &this) },
					rt.ArrayItem{ key: none, val: 'send_magic_link' },
				]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_MobileAppMagicLink) send_magic_link() rt.PhpVal {
	...
	]))
	{
		...
		if rt.is_true(var_jetpack_connection_manager.is_active()) {
			if rt.is_true(rt.call_function('class_exists', [
				rt.new_string('Jetpack_IXR_Client'),
			]))
			{
				mut var_xml := create_automattic_woocommerce_admin_api_jetpack_ixr_client(rt.create_array([
					rt.ArrayItem{ key: 'user_id', val: rt.call_function('get_current_user_id',
						[]rt.PhpVal{}) },
				]))
				var_xml.query(rt.new_string('jetpack.sendMobileMagicLink'), rt.create_array([
					rt.ArrayItem{ key: 'app', val: 'woocommerce' },
				]))
				if rt.is_true(var_xml.iserror()) {
					return rt.new_object('Automattic_WooCommerce_Admin_API_WP_Error', []string{}, create_automattic_woocommerce_admin_api_wp_error(rt.new_string('error_sending_mobile_magic_link'), rt.call_function('sprintf', [
						rt.new_string('%s: %s'),
						var_xml.geterrorcode(),
						var_xml.geterrormessage(),
					])))
				}
				return rt.call_function('rest_ensure_response', [
					rt.create_array([rt.ArrayItem{ key: 'code', val: 'success' }]),
				])
			}
		}
	}
	return rt.new_object('Automattic_WooCommerce_Admin_API_WP_Error', []string{}, create_automattic_woocommerce_admin_api_wp_error(rt.new_string('jetpack_not_connected'), rt.call_function('__', [
		rt.new_string('Jetpack is not connected.'),
...
fn create_automattic_woocommerce_admin_api_mobileappmagiclink(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_MobileAppMagicLink {
...
fn create_automattic_woocommerce_admin_api_jetpack_ixr_client(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Jetpack_IXR_Client {
