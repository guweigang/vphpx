...
fn init_static_automattic_woocommerce_admin_api_reports_taxes_stats_datastore() {
	rt.init_static_prop('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore',
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore) assign_report_columns() {
	...
	this.dispatch_set_prop('report_columns', rt.create_array([
		rt.ArrayItem{ key: 'tax_codes', val: 'COUNT(DISTINCT tax_rate_id) as tax_codes' },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore) update_sql_query_params(var_query_args rt.PhpVal) {
	...
	mut iife_temp_1 := Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore{}
	mut iife_result_1 := iife_temp_1.get_db_table_name()
	mut var_order_tax_lookup_table := iife_result_1
	this.add_time_period_sql_params(var_query_args.clone(), var_order_tax_lookup_table.clone())
	mut var_taxes_where_clause := rt.new_string('')
	...
	if var_query_args.array_isset(rt.new_string('taxes'))
		...
		mut iife_temp_2 := Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore{}
		mut iife_result_2 := iife_temp_2.get_filtered_ids(var_query_args.clone(),
			rt.new_string('taxes'))
		mut var_allowed_taxes := iife_result_2
		var_taxes_where_clause = rt.concat(var_taxes_where_clause,
	...
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore', [
		...
		var_taxes_where_clause.clone()])
	this.add_intervals_sql_params(var_query_args.clone(), var_order_tax_lookup_table.clone())
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore', [
		'Automattic_WooCommerce_Admin_API_Reports_DataStore',
		'DataStoreInterface',
...
fn Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore.get_taxes(var_args rt.PhpVal) rt.PhpVal {
	...
	if !(!rt.is_true(var_args_mutated.array_get(rt.new_string('include')))) {
		var_args_mutated.array_set('include',
			rt.cast_array(var_args_mutated.array_get(rt.new_string('include'))))
		mut var_tax_placeholders := rt.call_function('implode', [
			rt.new_string(','),
			rt.call_function('array_fill', [
				rt.new_int(0),
				rt.new_int(var_args_mutated.array_get(rt.new_string('include')).array_count()),
				rt.new_string('%d')])])
		var_query = rt.concat(var_query, rt.call_method(var_wpdb, 'prepare', [
			rt.new_string(' WHERE tax_rate_id IN (${var_tax_placeholders.to_string()})'),
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore) get_noncached_stats_data(var_query_args rt.PhpVal, var_params rt.PhpVal, var_data rt.PhpVal, var_expected_interval_count rt.PhpVal) rt.PhpVal {
	...
	mut iife_temp_3 := Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore{}
	...
	mut var_order_stats_join := rt.new_string((rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('JOIN '), rt.get_property(var_wpdb,
		'prefix')), rt.new_string('wc_order_stats ON ')), var_table_name),
		rt.new_string('.order_id = ')), rt.get_property(var_wpdb, 'prefix')),
		rt.new_string('wc_order_stats.order_id'))).str())
	this.update_sql_query_params(var_query_args.clone())
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore', [
		'Automattic_WooCommerce_Admin_API_Reports_DataStore',
		'DataStoreInterface',
	], &this), 'interval_query'), 'add_sql_clause', [rt.new_string('join'),
		var_order_stats_join.clone()])
	...
	if rt.is_true(rt.identical(rt.new_null(), var_totals)) {
		return rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_WP_Error',
			[]string{}, create_automattic_woocommerce_admin_api_reports_taxes_stats_wp_error(rt.new_string('woocommerce_analytics_taxes_stats_result_failed'), rt.call_function('__', [
	...
	mut var_totals_query := rt.create_array([
		...
		rt.ArrayItem{ key: 'where_time_clause', val: rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore', [
			'Automattic_WooCommerce_Admin_API_Reports_DataStore',
			'DataStoreInterface',
		], &this), 'total_query'), 'get_sql_clause', [
			rt.new_string('where_time'),
		]) },
		rt.ArrayItem{ key: 'where_clause', val: rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore', [
			'Automattic_WooCommerce_Admin_API_Reports_DataStore',
			'DataStoreInterface',
		], &this), 'total_query'), 'get_sql_clause', [
			rt.new_string('where'),
	...
	mut var_segmenter := create_automattic_woocommerce_admin_api_reports_taxes_stats_segmenter(var_query_args.clone(), rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_DataStore', [
		'Automattic_WooCommerce_Admin_API_Reports_DataStore',
		'DataStoreInterface',
	], &this), 'report_columns'))
	...
	if rt.is_true(rt.identical(rt.new_null(), var_intervals)) {
		return rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_WP_Error',
			[]string{}, create_automattic_woocommerce_admin_api_reports_taxes_stats_wp_error(rt.new_string('woocommerce_analytics_taxes_stats_result_failed'), rt.call_function('__', [
			rt.new_string('Sorry, fetching tax data failed.'),
			rt.new_string('woocommerce'),
		])))
	}
...
fn create_automattic_woocommerce_admin_api_reports_taxes_stats_wp_error(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Taxes_Stats_WP_Error {
