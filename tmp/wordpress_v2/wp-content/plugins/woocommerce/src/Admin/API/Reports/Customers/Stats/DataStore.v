...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_DataStore) assign_report_columns() {
	this.dispatch_set_prop('report_columns', rt.create_array([
		rt.ArrayItem{ key: 'customers_count', val: 'COUNT( * ) as customers_count' },
		rt.ArrayItem{ key: 'avg_orders_count', val: 'AVG( orders_count ) as avg_orders_count' },
		rt.ArrayItem{ key: 'avg_total_spend', val: 'AVG( total_spend ) as avg_total_spend' },
		rt.ArrayItem{
			key: 'avg_avg_order_value'
			val: 'AVG( avg_order_value ) as avg_avg_order_value'
		},
	]))
}

fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_DataStore) get_default_query_vars() rt.PhpVal {
	mut iife_temp_0 := Class_Automattic_WooCommerce_Admin_API_Reports_DataStore{}
	mut iife_result_0 := iife_temp_0.get_default_query_vars()
	mut var_defaults := iife_result_0
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_DataStore) get_noncached_data(var_query_args rt.PhpVal) rt.PhpVal {
	...
	mut var_data := rt.array_to_object(rt.create_array([
		rt.ArrayItem{ key: 'customers_count', val: 0 },
		rt.ArrayItem{ key: 'avg_orders_count', val: 0 },
		rt.ArrayItem{ key: 'avg_total_spend', val: 0 },
		rt.ArrayItem{ key: 'avg_avg_order_value', val: 0 },
	]))
	...
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_DataStore', [
		'Automattic_WooCommerce_Admin_API_Reports_Customers_DataStore',
		'DataStoreInterface',
	...
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_DataStore', [
		...
		rt.new_string('SUM( total_sales ) AS total_spend,')])
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_DataStore', [
		'Automattic_WooCommerce_Admin_API_Reports_Customers_DataStore',
		'DataStoreInterface',
	], &this), 'subquery'), 'add_sql_clause', [rt.new_string('select'),
		rt.new_string('SUM( CASE WHEN parent_id = 0 THEN 1 END ) as orders_count,')])
	rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_DataStore', [
		'Automattic_WooCommerce_Admin_API_Reports_Customers_DataStore',
		'DataStoreInterface',
	], &this), 'subquery'), 'add_sql_clause', [rt.new_string('select'),
		rt.new_string('CASE WHEN SUM( CASE WHEN parent_id = 0 THEN 1 ELSE 0 END ) = 0 THEN NULL ELSE SUM( total_sales ) / SUM( CASE WHEN parent_id = 0 THEN 1 ELSE 0 END ) END AS avg_order_value')])
	this.clear_sql_clause(rt.create_array([rt.ArrayItem{ key: none, val: 'order_by' },
		rt.ArrayItem{ key: none, val: 'limit' }]))
	this.add_sql_clause(rt.new_string('select'), var_selections.clone())
	this.add_sql_clause(rt.new_string('from'), rt.new_string((rt.concat(rt.concat(rt.new_string('('), rt.call_method(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_DataStore', [
		'Automattic_WooCommerce_Admin_API_Reports_Customers_DataStore',
		'DataStoreInterface',
	], &this), 'subquery'), 'get_query_statement', []rt.PhpVal{})), rt.new_string(') AS tt'))).str()))
	mut var_report_data := rt.call_method(var_wpdb, 'get_results', [
	...
	var_data = rt.array_to_object(this.cast_numbers(var_report_data.array_get(rt.new_int(0))))
	return var_data.clone()
}
...
fn create_automattic_woocommerce_admin_api_reports_customers_stats_datastore(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Reports_Customers_Stats_DataStore {
