...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Taxes) get_collection_params() rt.PhpVal {
	mut var_params :=
		this.Class_Automattic_WooCommerce_Admin_API_WC_REST_Taxes_Controller.get_collection_params()
	var_params.array_set('search', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Search by similar tax code.'),
	...
	var_params.array_set('include', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit result set to items that have the specified rate ID(s) assigned.'),
			rt.new_string('woocommerce'),
		]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Taxes) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_orderby_possibles := rt.create_array([
		rt.ArrayItem{ key: 'id', val: 'tax_rate_id' },
		rt.ArrayItem{ key: 'order', val: 'tax_rate_order' },
	])
	var_prepared_args.array_set('orderby',
		var_orderby_possibles.array_get(var_request.array_get(rt.new_string('orderby'))))
	var_prepared_args.array_set('class', var_request.array_get(rt.new_string('class')))
	var_prepared_args.array_set('search', var_request.array_get(rt.new_string('search')))
	var_prepared_args.array_set('include', var_request.array_get(rt.new_string('include')))
	var_prepared_args = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_tax_query'),
	...
	if !(!rt.is_true(var_prepared_args.array_get(rt.new_string('class')))) {
		mut var_class := if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('standard'), var_prepared_args.array_get(rt.new_string('class')))))) { rt.call_function('sanitize_title', [
				var_prepared_args.array_get(rt.new_string('class')),
			]) } else { rt.new_string('') }
		var_query = rt.concat(var_query,
			rt.new_string(" AND tax_rate_class = '${var_class.to_string()}'"))
	}
	mut var_tax_code_search := var_prepared_args.array_get(rt.new_string('search'))
	if rt.is_true(var_tax_code_search) {
		mut var_code_like := rt.new_string('%' +
			(rt.call_method(var_wpdb, 'esc_like', [var_tax_code_search.clone()])).str() + '%')
		var_query = rt.concat(var_query, rt.call_method(var_wpdb, 'prepare', [
			rt.new_string(' AND CONCAT_WS( "-", NULLIF(tax_rate_country, ""), NULLIF(tax_rate_state, ""), NULLIF(tax_rate_name, ""), NULLIF(tax_rate_priority, "") ) LIKE %s'),
			var_code_like.clone(),
		]))
	}
	mut var_included_taxes := rt.call_function('array_map', [
		rt.new_string('absint'), var_prepared_args.array_get(rt.new_string('include'))])
	if !(!rt.is_true(var_included_taxes)) {
		var_included_taxes = rt.call_function('implode', [rt.new_string(','),
			var_prepared_args.array_get(rt.new_string('include'))])
		var_query = rt.concat(var_query,
			rt.new_string(' AND tax_rate_id IN (${var_included_taxes.to_string()})'))
	}
	mut var_order_by := rt.call_function('sprintf', [rt.new_string(' ORDER BY %s'),
		rt.call_function('sanitize_key', [var_prepared_args.array_get(rt.new_string('orderby'))])])
	mut var_pagination := rt.call_function('sprintf', [rt.new_string(' LIMIT %d, %d'),
		var_prepared_args.array_get(rt.new_string('offset')),
		var_prepared_args.array_get(rt.new_string('number'))])
	mut var_results := rt.call_method(var_wpdb, 'get_results', [
		rt.new_string(var_query.str() + var_order_by.str() + var_pagination.str()),
	])
	...
	for {
		...
		var_taxes.array_push(this.prepare_response_for_collection(var_data.clone()))
	}
	...
	rt.call_method(var_wpdb, 'get_results', [
		rt.call_function('str_replace', [rt.new_string('SELECT *'),
			rt.new_string('SELECT tax_rate_id'), var_query.clone()]),
	])
	mut var_total_taxes := rt.new_int((rt.get_property(var_wpdb, 'num_rows')).to_i64())
	rt.call_method(var_response, 'header', [rt.new_string('X-WP-Total'),
		rt.new_int(var_total_taxes.to_i64())])
	...
	mut var_base := rt.call_function('add_query_arg', [
		...
		rt.call_function('rest_url', [
			rt.call_function('sprintf', [rt.new_string('/%s/%s'), this.namespace,
				rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_Taxes', [
					'Automattic_WooCommerce_Admin_API_WC_REST_Taxes_Controller',
				], &this), 'rest_base')]),
		]),
	])
...
fn create_automattic_woocommerce_admin_api_taxes(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_Taxes {
...
fn create_automattic_woocommerce_admin_api_wc_rest_taxes_controller(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_WC_REST_Taxes_Controller {
