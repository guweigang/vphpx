...
fn (mut this Class_Automattic_WooCommerce_Admin_API_LaunchYourStore) register_routes() {
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/initialize-coming-soon'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'methods', val: 'POST' },
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_LaunchYourStore',
						[]string{}, &this) },
					rt.ArrayItem{ key: none, val: 'initialize_coming_soon' },
				]) },
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_LaunchYourStore',
						[]string{}, &this) },
					rt.ArrayItem{ key: none, val: 'must_be_shop_manager_or_admin' },
				]) },
			]) },
		])])
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/update-survey-status'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'methods', val: 'POST' },
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_LaunchYourStore',
						[]string{}, &this) },
					rt.ArrayItem{ key: none, val: 'update_survey_status' },
				]) },
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_LaunchYourStore',
						[]string{}, &this) },
	...
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/survey-completed'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_LaunchYourStore',
						[]string{}, &this) },
					rt.ArrayItem{ key: none, val: 'has_survey_completed' },
				]) },
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_LaunchYourStore',
						[]string{}, &this) },
	...
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/woopayments/test-orders/count'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'methods', val: 'GET' },
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_LaunchYourStore',
						[]string{}, &this) },
					rt.ArrayItem{ key: none, val: 'get_woopay_test_orders_count' },
				]) },
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_LaunchYourStore',
						[]string{}, &this) },
	...
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/woopayments/test-orders'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'methods', val: 'DELETE' },
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_LaunchYourStore',
						[]string{}, &this) },
					rt.ArrayItem{ key: none, val: 'delete_woopay_test_orders' },
				]) },
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_LaunchYourStore',
						[]string{}, &this) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_LaunchYourStore) initialize_coming_soon() bool {
	...
	mut var_coming_soon := rt.new_string('yes')
	mut iife_temp_0 := Class_Automattic_WooCommerce_Admin_WCAdminHelper{}
	mut iife_result_0 := iife_temp_0.is_site_fresh()
	mut var_store_pages_only :=
		rt.new_string((if rt.is_true(iife_result_0) { 'no' } else { 'yes' }).str())
	mut var_private_link := rt.new_string('no')
	mut var_share_key := rt.call_function('wp_generate_password', [
		rt.new_int(32), rt.new_bool(false)])
	rt.call_function('update_option', [rt.new_string('woocommerce_coming_soon'),
		var_coming_soon.clone()])
	rt.call_function('update_option', [rt.new_string('woocommerce_store_pages_only'),
		var_store_pages_only.clone()])
	rt.call_function('add_option', [rt.new_string('woocommerce_private_link'),
		var_private_link.clone()])
	rt.call_function('add_option', [rt.new_string('woocommerce_share_key'),
		var_share_key.clone()])
	rt.call_function('wc_admin_record_tracks_event', [
		rt.new_string('launch_your_store_initialize_coming_soon'),
		rt.create_array([rt.ArrayItem{ key: 'coming_soon', val: var_coming_soon },
			rt.ArrayItem{ key: 'store_pages_only', val: var_store_pages_only },
			rt.ArrayItem{ key: 'private_link', val: var_private_link }]),
	])
	return true
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_LaunchYourStore) get_woopay_test_orders_count() rt.PhpVal {
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_count := if args.len > 0 { args[0].clone() } else { rt.new_null() }
	...
	mut var_orders := rt.call_function('wc_get_orders', [
		rt.create_array([rt.ArrayItem{ key: 'meta_key', val: '_wcpay_mode' },
			rt.ArrayItem{ key: 'meta_value', val: 'test' }, rt.ArrayItem{ key: 'return', val: 'ids' }]),
	])
	return rt.call_callable(var_return, [rt.new_int(var_orders.clone().array_count())])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_LaunchYourStore) delete_woopay_test_orders() rt.PhpVal {
	closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_object('Automattic_WooCommerce_Admin_API_WP_REST_Response', []string{}, create_automattic_woocommerce_admin_api_wp_rest_response(rt.new_null(),
			var_status.clone()))
	}
	mut var_return := rt.new_closure(closure_3_fn)
	mut var_orders := rt.call_function('wc_get_orders', [
		rt.create_array([rt.ArrayItem{ key: 'meta_key', val: '_wcpay_mode' },
			rt.ArrayItem{ key: 'meta_value', val: 'test' }]),
	])
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_LaunchYourStore) update_survey_status(mut var_request Class_Automattic_WooCommerce_Admin_API_WP_REST_Request) rt.PhpVal {
	rt.call_function('update_option', [
		rt.new_string('woocommerce_admin_launch_your_store_survey_completed'),
		var_request.get_param(rt.new_string('status')),
	])
	return rt.new_object('Automattic_WooCommerce_Admin_API_WP_REST_Response', []string{},
		create_automattic_woocommerce_admin_api_wp_rest_response())
}

fn (mut this Class_Automattic_WooCommerce_Admin_API_LaunchYourStore) has_survey_completed() rt.PhpVal {
	return rt.new_object('Automattic_WooCommerce_Admin_API_WP_REST_Response', []string{}, create_automattic_woocommerce_admin_api_wp_rest_response(rt.call_function('get_option', [
		rt.new_string('woocommerce_admin_launch_your_store_survey_completed'),
		rt.new_string('no'),
	])))
}
...
fn create_automattic_woocommerce_admin_api_launchyourstore(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_LaunchYourStore {
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_LaunchYourStore) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'update_survey_status' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Admin_API_WP_REST_Request](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
