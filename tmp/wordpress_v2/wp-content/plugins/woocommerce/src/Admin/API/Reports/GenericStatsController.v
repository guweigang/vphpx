...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_GenericStatsController) get_item_schema() rt.PhpVal {
	...
	mut var_segments := rt.create_array([
		rt.ArrayItem{ key: 'segments', val: rt.create_array([
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Reports data grouped by segment condition.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'array' },
			...
			rt.ArrayItem{ key: 'items', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'properties', val: rt.create_array([
					rt.ArrayItem{ key: 'segment_id', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('Segment identificator.'),
							rt.new_string('woocommerce'),
						]) },
					...
					rt.ArrayItem{ key: 'subtotals', val: rt.create_array([
						...
						rt.ArrayItem{ key: 'readonly', val: true },
						rt.ArrayItem{ key: 'properties', val: var_data_values },
					]) },
				]) },
			]) },
		]) },
	])
	mut var_totals := rt.call_function('array_merge', [var_data_values.clone(),
		var_segments.clone()])
	return rt.create_array([
		rt.ArrayItem{ key: '$schema', val: 'http://json-schema.org/draft-04/schema#' },
		rt.ArrayItem{ key: 'title', val: 'report_stats' },
		rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_Reports_GenericStatsController) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_out_data := rt.create_array([
		rt.ArrayItem{
			key: 'totals'
			val: if rt.is_true(rt.get_property(var_report_data, 'totals')) { rt.call_function('get_object_vars', [
					rt.get_property(var_report_data, 'totals'),
				]) } else { rt.new_null() }
		},
		rt.ArrayItem{ key: 'intervals', val: rt.new_array() },
	])
	mut iter_1 := rt.get_property(var_report_data, 'intervals').iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_interval_data := item_1.val
		mut var_item := this.prepare_item_for_response(var_interval_data.clone(),
			var_request.clone())
		var_out_data.array_get_mut('intervals').array_push(this.prepare_response_for_collection(var_item.clone()))
	}
