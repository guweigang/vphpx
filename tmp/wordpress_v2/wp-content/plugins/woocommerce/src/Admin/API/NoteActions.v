...
fn (mut this Class_Automattic_WooCommerce_Admin_API_NoteActions) register_routes() {
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_NoteActions', [
			'Automattic_WooCommerce_Admin_API_Notes',
		], &this), 'namespace'),
		rt.new_string('/' +
			(rt.get_property(rt.new_object('Automattic_WooCommerce_Admin_API_NoteActions', ['Automattic_WooCommerce_Admin_API_Notes'], &this), 'rest_base')).str() +
			'/(?P<note_id>[\\d-]+)/action/(?P<action_id>[\\d-]+)'),
		rt.create_array([
			rt.ArrayItem{ key: 'args', val: rt.create_array([
				rt.ArrayItem{ key: 'note_id', val: rt.create_array([
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Unique ID for the Note.'),
						rt.new_string('woocommerce'),
					]) },
					rt.ArrayItem{ key: 'type', val: 'integer' },
				]) },
				rt.ArrayItem{ key: 'action_id', val: rt.create_array([
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('Unique ID for the Note Action.'),
						rt.new_string('woocommerce'),
					]) },
					rt.ArrayItem{ key: 'type', val: 'integer' },
				]) },
			...
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Admin_API_NoteActions', [
						'Automattic_WooCommerce_Admin_API_Notes',
					], &this) },
					rt.ArrayItem{ key: none, val: 'trigger_note_action' },
				]) },
...
fn (mut this Class_Automattic_WooCommerce_Admin_API_NoteActions) trigger_note_action(var_request rt.PhpVal) rt.PhpVal {
	...
	mut iife_result_0 := iife_temp_0.get_note(rt.call_method(var_request, 'get_param', [
		rt.new_string('note_id'),
	]))
	...
	mut iife_result_1 := iife_temp_1.get_action_by_id(var_note.clone(), rt.call_method(var_request,
		'get_param', [rt.new_string('action_id')]))
	mut var_triggered_action := iife_result_1
	if rt.is_true(rt.new_bool(!(rt.is_true(var_triggered_action)))) {
		return rt.new_object('Automattic_WooCommerce_Admin_API_WP_Error', []string{}, create_automattic_woocommerce_admin_api_wp_error(rt.new_string('woocommerce_note_action_invalid_id'), rt.call_function('__', [
			rt.new_string('Sorry, there is no resource with that ID.'),
			rt.new_string('woocommerce'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }])))
	}
	mut iife_temp_2 := Class_Automattic_WooCommerce_Admin_Notes_Notes{}
	mut iife_result_2 := iife_temp_2.trigger_note_action(var_note.clone(),
		var_triggered_action.clone())
	mut var_triggered_note := iife_result_2
...
fn create_automattic_woocommerce_admin_api_noteactions(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_API_NoteActions {
