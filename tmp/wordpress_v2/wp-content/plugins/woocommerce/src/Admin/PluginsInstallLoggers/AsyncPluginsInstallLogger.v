...
fn (mut this Class_Automattic_WooCommerce_Admin_PluginsInstallLoggers_AsyncPluginsInstallLogger) construct(option_name string) {
	...
	rt.call_function('add_option', [rt.new_string(this.option_name), rt.create_array([rt.ArrayItem{ key: 'created_time', val: rt.call_function('time', []rt.PhpVal{}) }, rt.ArrayItem{ key: 'status', val: 'pending' }, rt.ArrayItem{ key: 'plugins', val: rt.new_array() }]), rt.new_string(''), rt.new_string('no')])
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_error := rt.call_function('error_get_last', []rt.PhpVal{})
		if var_error.array_isset(rt.new_string('type')) && rt.is_true(rt.identical(rt.get_constant('E_ERROR'), var_error.array_get(rt.new_string('type')))) {
...
fn (mut this Class_Automattic_WooCommerce_Admin_PluginsInstallLoggers_AsyncPluginsInstallLogger) update(mut var_data Class_Automattic_WooCommerce_Admin_PluginsInstallLoggers_array) rt.PhpVal {
	return rt.call_function('update_option', [rt.new_string(this.option_name), var_data])
}

fn (mut this Class_Automattic_WooCommerce_Admin_PluginsInstallLoggers_AsyncPluginsInstallLogger) get() rt.PhpVal {
	return rt.call_function('get_option', [rt.new_string(this.option_name)])
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_PluginsInstallLoggers_AsyncPluginsInstallLogger) install_requested(plugin_name string) {
	mut var_option := this.get()
	if !(var_option.array_get(rt.new_string('plugins')).array_isset(rt.new_string(plugin_name))) {
...
fn (mut this Class_Automattic_WooCommerce_Admin_PluginsInstallLoggers_AsyncPluginsInstallLogger) add_error(plugin_name string, mut var_error_message Class_Automattic_WooCommerce_Admin_PluginsInstallLoggers_?string) {
	mut var_option := this.get()
	var_option.array_get_mut('plugins').array_get_mut(plugin_name).array_get_mut('errors').array_push(var_error_message)
...
fn (mut this Class_Automattic_WooCommerce_Admin_PluginsInstallLoggers_AsyncPluginsInstallLogger) get_plugin_track_key(var_id rt.PhpVal) rt.PhpVal {
	mut var_slug := rt.call_function('explode', [rt.new_string(':'), var_id.clone()]).array_get(rt.new_int(0))
	mut var_key := if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^woocommerce(-|_)payments$/'), var_slug.clone()])) { rt.new_string('wcpay') } else { rt.call_function('explode', [rt.new_string(':'), rt.call_function('str_replace', [rt.new_string('-'), rt.new_string('_'), var_slug.clone()])]).array_get(rt.new_int(0)) }
...
fn (mut this Class_Automattic_WooCommerce_Admin_PluginsInstallLoggers_AsyncPluginsInstallLogger) get_timeframe(var_timeInMs rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		if !(var_time_frame.array_isset(rt.new_string('max'))) {
			return var_time_frame.array_get(rt.new_string('name'))
		}
		if rt.is_true(rt.less(var_timeInMs, rt.mul(var_time_frame.array_get(rt.new_string('max')), rt.new_int(1000)))) {
			return var_time_frame.array_get(rt.new_string('name'))
		}
	}
	return rt.new_null()
...
fn (mut this Class_Automattic_WooCommerce_Admin_PluginsInstallLoggers_AsyncPluginsInstallLogger) track(var_data rt.PhpVal) {
	closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_extension := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return
		}
	...
	mut var_track_data := rt.create_array([rt.ArrayItem{ key: 'success', val: true }, rt.ArrayItem{ key: 'installed_extensions', val: rt.call_function('array_map', [rt.new_closure(closure_3_fn), var_data.array_get(rt.new_string('installed'))]) }, rt.ArrayItem{ key: 'total_time', val: this.get_timeframe(rt.mul(rt.sub(rt.call_function('time', []rt.PhpVal{}), var_data.array_get(rt.new_string('start_time'))), rt.new_int(1000))) }])
	mut iter_2 := var_data.array_get(rt.new_string('installed')).iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_plugin := item_2.val
		if !(var_data.array_get(rt.new_string('time')).array_isset(var_plugin)) {
			continue
		}
		mut var_plugin_track_key := this.get_plugin_track_key(var_plugin.clone())
		mut var_install_time := this.get_timeframe(var_data.array_get(rt.new_string('time')).array_get(var_plugin))
		var_track_data.array_set('install_time_' + (var_plugin_track_key).str(), var_install_time.clone())
	...
	rt.call_function('wc_admin_record_tracks_event', [rt.new_string('coreprofiler_store_extensions_installed_and_activated'), var_track_data.clone()])
}
