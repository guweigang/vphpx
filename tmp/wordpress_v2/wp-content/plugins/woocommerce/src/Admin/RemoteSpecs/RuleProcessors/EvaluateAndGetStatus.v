...
fn Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_EvaluateAndGetStatus.evaluate(var_spec rt.PhpVal, var_current_status rt.PhpVal, var_stored_state rt.PhpVal, var_rule_evaluator rt.PhpVal) rt.PhpVal {
	if !(!(rt.get_property(var_spec, 'rules')).is_null()) {
		return var_current_status.clone()
	}
	mut var_evaluated_result := rt.call_method(var_rule_evaluator, 'evaluate', [
		rt.get_property(var_spec, 'rules'),
		var_stored_state.clone(),
		rt.create_array([
			rt.ArrayItem{ key: 'slug', val: rt.get_property(var_spec, 'slug') },
			rt.ArrayItem{ key: 'source', val: 'remote-inbox-notifications' },
		]),
	])
	if rt.is_true(rt.identical(Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_pending(),
		var_current_status))
	{
		return if rt.is_true(var_evaluated_result) {
			rt.get_property(var_spec, 'status')
		} else {
			Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_pending()
		}
	}
	if !(rt.get_property(var_spec, 'type')).is_null()
		&& rt.is_true(rt.call_function('in_array', [rt.get_property(var_spec, 'type'), rt.create_array([rt.ArrayItem{
		key: none
		val: 'error'
	}, rt.ArrayItem{ key: none, val: 'update' }]), rt.new_bool(true)]))
		&& rt.is_true(rt.identical(Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_unactioned(), var_current_status))
		&& rt.is_true(rt.new_bool(!(rt.is_true(var_evaluated_result)))) {
		return Class_Automattic_WooCommerce_Admin_Notes_Note.e_wc_admin_note_pending()
	}
	if !(!(rt.get_property(var_spec, 'allow_redisplay')).is_null())
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.get_property(var_spec, 'allow_redisplay'))))) {
		return var_current_status.clone()
	}
	return if rt.is_true(var_evaluated_result) {
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_EvaluateAndGetStatus) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'evaluate' {
			...
			return Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_EvaluateAndGetStatus.evaluate(dispatch_arg_0,
