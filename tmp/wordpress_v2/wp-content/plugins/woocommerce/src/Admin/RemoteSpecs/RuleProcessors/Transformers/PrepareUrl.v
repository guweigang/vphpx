...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_PrepareUrl) transform(var_value rt.PhpVal, mut var_arguments Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_?stdClass, var_default_value rt.PhpVal) string {
	...
	mut var_url_parts := rt.call_function('wp_parse_url', [rt.new_string(var_value.clone().to_string().trim_right(' \t\n\r'))])
	...
	if var_url_parts.array_isset(rt.new_string('path')) {
		return (var_url_parts.array_get(rt.new_string('host'))).str() + (var_url_parts.array_get(rt.new_string('path'))).str()
	}
	return (var_url_parts.array_get(rt.new_string('host'))).str()
}
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_transformers_prepareurl(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_PrepareUrl {
