...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_DotNotation) transform(var_value rt.PhpVal, mut var_arguments Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_?stdclass, var_default_value rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_value_mutated.clone().is_object())) {
	var_value_mutated = rt.call_function('json_decode', [rt.call_function('wp_json_encode', [var_value_mutated.clone()]), rt.new_bool(true)])
	}
	return this.get(var_value_mutated.clone(), rt.get_property(var_arguments, 'path'), var_default_value.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_DotNotation) get(var_array_to_search rt.PhpVal, var_path rt.PhpVal, var_default_value rt.PhpVal) rt.PhpVal {
	...
	if !(var_array_to_search_mutated.clone().is_array()) {
	...
	mut iter_1 := rt.call_function('explode', [rt.new_string('.'), var_path.clone()]).iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_segment := item_1.val
		if !(var_array_to_search_mutated.clone().is_array()) || rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_array_to_search_mutated.clone().array_isset(var_segment.clone())))))) {
			return var_default_value.clone()
		}
	var_array_to_search_mutated = var_array_to_search_mutated.array_get(var_segment)
	}
	return var_array_to_search_mutated.clone()
}
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_transformers_dotnotation(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_Transformers_DotNotation {
