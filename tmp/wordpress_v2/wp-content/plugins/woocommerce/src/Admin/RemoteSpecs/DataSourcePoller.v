...
fn init_static_automattic_woocommerce_admin_remotespecs_datasourcepoller() {
	rt.init_static_prop('Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller', 'logger',
		rt.new_null())
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller) construct(var_id rt.PhpVal, var_data_sources rt.PhpVal, var_args rt.PhpVal) {
	...
	this.data_sources = var_data_sources_mutated.clone()
	this.id = var_id_mutated.clone()
	mut var_arg_defaults := rt.create_array([rt.ArrayItem{ key: 'spec_key', val: 'id' },
		rt.ArrayItem{ key: 'transient_name', val: 'woocommerce_admin_' + var_id_mutated.str() +
			'_specs' }, rt.ArrayItem{ key: 'transient_expiry', val: rt.mul(rt.new_int(7),
			rt.get_constant('DAY_IN_SECONDS')) }])
	this.args = rt.call_function('wp_parse_args', [var_args.clone(),
		var_arg_defaults.clone()])
}

fn Class_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller.get_logger() rt.PhpVal {
	if rt.is_true(rt.new_bool(rt.get_static_prop('Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller',
		'logger').is_null()))
	{
		rt.set_static_prop('Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller', 'logger', rt.call_function('wc_get_logger',
			[]rt.PhpVal{}))
	}
	return rt.get_static_prop('Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller', 'logger')
}

fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller) get_spec_key(var_spec rt.PhpVal) bool {
	mut var_key := this.args.array_get(rt.new_string('spec_key'))
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller) get_specs_from_data_sources() rt.PhpVal {
	...
	mut var_specs_group := if !(rt.call_function('get_transient', [
		this.args.array_get(rt.new_string('transient_name')),
	])).is_null() { rt.call_function('get_transient', [
			this.args.array_get(rt.new_string('transient_name')),
		]) } else { rt.new_array() }
	mut var_specs := if var_specs_group.array_isset(var_locale) {
		var_specs_group.array_get(var_locale)
	} else {
		rt.new_null()
	}
	if !(var_specs.clone().is_array()) {
		this.read_specs_from_data_sources()
		var_specs_group = rt.call_function('get_transient', [
			this.args.array_get(rt.new_string('transient_name')),
		])
		var_specs = if var_specs_group.array_isset(var_locale) {
			var_specs_group.array_get(var_locale)
		} else {
			rt.new_array()
		}
	}
	var_specs = rt.call_function('apply_filters', [
		Class_Automattic_WooCommerce_Admin_RemoteSpecs_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller.filter_name_specs(),
		var_specs.clone(),
		this.id,
	])
	return if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_specs)))) {
		var_specs
	} else {
		rt.new_array()
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller) read_specs_from_data_sources() bool {
	...
	mut var_data_sources := rt.call_function('apply_filters', [
		Class_Automattic_WooCommerce_Admin_RemoteSpecs_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller.filter_name(),
		this.data_sources,
		this.id,
	])
	...
	for {
		...
		mut var_specs_from_data_source :=
			Class_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller.read_data_source(var_url.clone())
		this.merge_specs(var_specs_from_data_source.clone(), var_specs.clone(), var_url.clone())
	}
	if var_specs.clone().array_count() == 0 {
		return false
	}
	mut var_specs_group := rt.call_function('get_transient', [
		this.args.array_get(rt.new_string('transient_name')),
	])
	var_specs_group = if var_specs_group.clone().is_array() {
		var_specs_group
	} else {
		rt.new_array()
	}
	mut var_locale := rt.call_function('get_user_locale', []rt.PhpVal{})
	var_specs_group.array_set(var_locale, var_specs.clone())
	this.set_specs_transient(var_specs_group.clone(),
		(this.args.array_get(rt.new_string('transient_expiry'))).to_i64())
	return true
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller) set_specs_transient(var_specs rt.PhpVal, expiration i64) {
	...
	rt.call_function('set_transient', [this.args.array_get(rt.new_string('transient_name')),
		var_specs_mutated.clone(), rt.new_int(expiration)])
}
...
fn Class_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller.read_data_source(var_url rt.PhpVal) rt.PhpVal {
	...
	mut var_response := rt.call_function('wp_remote_get', [
		rt.call_function('add_query_arg', [rt.new_string('locale'),
			rt.call_function('get_user_locale', []rt.PhpVal{}),
			var_url.clone()]),
		rt.create_array([rt.ArrayItem{ key: 'timeout', val: rt.call_function('max', [
			rt.new_int(1),
			rt.call_function('absint', [
				rt.call_function('apply_filters', [
					rt.new_string('woocommerce_data_source_poller_timeout'),
					rt.new_int(3),
				]),
			]),
		]) }, rt.ArrayItem{ key: 'user-agent', val: 'WooCommerce/' +
			(rt.get_constant('WC_VERSION')).str() + '; ' +
			(rt.call_function('home_url', [rt.new_string('/')])).str() }]),
	])
	if rt.is_true(rt.call_function('is_wp_error', [var_response.clone()]))
		|| !(var_response.array_isset(rt.new_string('body'))) {
		rt.call_method(var_logger, 'error', [rt.new_string('Error getting data feed'),
			var_logger_context.clone()])
		rt.call_method(var_logger, 'error', [
			println(var_response.clone().to_string()),
			var_logger_context.clone(),
		])
		return rt.new_array()
	}
	mut var_body := var_response.array_get(rt.new_string('body'))
	mut var_specs := rt.call_function('json_decode', [var_body.clone()])
	if rt.is_true(rt.identical(rt.new_null(), var_specs)) {
		rt.call_method(var_logger, 'error', [
			rt.new_string('Empty response in data feed'),
			var_logger_context.clone(),
		])
		return rt.new_array()
	}
	if !(var_specs.clone().is_array()) {
		rt.call_method(var_logger, 'error', [rt.new_string('Data feed is not an array'),
			var_logger_context.clone()])
		return rt.new_array()
	}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller) merge_specs(var_specs_to_merge_in rt.PhpVal, var_specs rt.PhpVal, var_url rt.PhpVal) {
	mut var_specs_mutated := var_specs
	mut iter_2 := var_specs_to_merge_in.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_spec := item_2.val
		if !(this.validate_spec(var_spec.clone(), var_url.clone())) {
			continue
		}
		mut var_id := rt.new_bool(this.get_spec_key(var_spec.clone()))
		var_specs_mutated.array_set(var_id, var_spec.clone())
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_DataSourcePoller) validate_spec(var_spec rt.PhpVal, var_url rt.PhpVal) bool {
	...
	if !(this.get_spec_key(var_spec.clone())) {
		rt.call_method(var_logger, 'error', [
			rt.new_string('Spec is invalid because the id is missing in feed'),
			var_logger_context.clone(),
		])
