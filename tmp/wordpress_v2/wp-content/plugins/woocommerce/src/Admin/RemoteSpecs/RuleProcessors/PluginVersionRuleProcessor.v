...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_PluginVersionRuleProcessor) process(var_rule rt.PhpVal, var_stored_state rt.PhpVal) bool {
	mut var_active_plugin_slugs := rt.call_method(this.plugins_provider, 'get_active_plugin_slugs',
		[]rt.PhpVal{})
	mut var_plugin_name := rt.call_function('apply_filters', [
		rt.new_string('wp_plugin_dependencies_slug'),
		rt.get_property(var_rule, 'plugin'),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_plugin_name.clone(), var_active_plugin_slugs.clone(),
		rt.new_bool(true)])))))
	{
		return false
	}
	mut var_plugin_data := rt.call_method(this.plugins_provider, 'get_plugin_data', [
		var_plugin_name.clone(),
	])
	if !(var_plugin_data.clone().is_array())
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_plugin_data.clone().array_isset(rt.new_string('Version'))))))) {
		return false
	}
	mut var_plugin_version := var_plugin_data.array_get(rt.new_string('Version'))
	return (rt.call_function('version_compare', [var_plugin_version.clone(),
		rt.get_property(var_rule, 'version'), rt.get_property(var_rule, 'operator')])).to_bool()
}
