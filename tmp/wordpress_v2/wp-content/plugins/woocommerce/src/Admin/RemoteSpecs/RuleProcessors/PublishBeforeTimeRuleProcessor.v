...
fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_PublishBeforeTimeRuleProcessor) construct(var_date_time_provider rt.PhpVal) {
	this.date_time_provider = if rt.is_true(rt.identical(rt.new_null(), var_date_time_provider)) {
		create_automattic_woocommerce_admin_datetimeprovider_currentdatetimeprovider()
	} else {
		var_date_time_provider
	}
}

fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_PublishBeforeTimeRuleProcessor) process(var_rule rt.PhpVal, var_stored_state rt.PhpVal) rt.PhpVal {
	return rt.less_equal(rt.call_method(this.date_time_provider, 'get_now', []rt.PhpVal{}), create_automattic_woocommerce_admin_remotespecs_ruleprocessors_datetime(rt.get_property(var_rule,
		'publish_before')))
}

fn (mut this Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_PublishBeforeTimeRuleProcessor) validate(var_rule rt.PhpVal) bool {
	if !(!(rt.get_property(var_rule, 'publish_before')).is_null()) {
		return false
	}
	create_automattic_woocommerce_admin_remotespecs_ruleprocessors_datetime(rt.get_property(var_rule,
		'publish_before'))
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_publishbeforetimeruleprocessor(arg_0 rt.PhpVal) &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_PublishBeforeTimeRuleProcessor {
...
fn create_automattic_woocommerce_admin_remotespecs_ruleprocessors_datetime(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Admin_RemoteSpecs_RuleProcessors_DateTime {
