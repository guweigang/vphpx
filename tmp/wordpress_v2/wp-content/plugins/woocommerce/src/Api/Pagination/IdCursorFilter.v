...
fn init_static_automattic_woocommerce_api_pagination_idcursorfilter() {
	rt.init_static_prop('Automattic_WooCommerce_Api_Pagination_IdCursorFilter', 'registered',
		rt.new_bool(false))
}

fn Class_Automattic_WooCommerce_Api_Pagination_IdCursorFilter.ensure_registered() {
	if rt.is_true(rt.get_static_prop('Automattic_WooCommerce_Api_Pagination_IdCursorFilter',
		'registered'))
	{
		return
	}
	rt.call_function('add_filter', [rt.new_string('posts_where'),
		rt.create_array([
			rt.ArrayItem{
				key: none
				val: Class_Automattic_WooCommerce_Api_Pagination_Automattic_WooCommerce_Api_Pagination_IdCursorFilter.class()
			},
			rt.ArrayItem{ key: none, val: 'apply' },
		]),
		rt.new_int(10), rt.new_int(2)])
	rt.set_static_prop('Automattic_WooCommerce_Api_Pagination_IdCursorFilter', 'registered',
		rt.new_bool(true))
}
...
fn Class_Automattic_WooCommerce_Api_Pagination_IdCursorFilter.apply(where string, mut var_query Class_Automattic_WooCommerce_Api_Pagination_WP_Query) string {
	...
	mut var_after :=
		rt.new_int((var_query.get(Class_Automattic_WooCommerce_Api_Pagination_Automattic_WooCommerce_Api_Pagination_IdCursorFilter.after_id())).to_i64())
	mut var_before :=
		rt.new_int((var_query.get(Class_Automattic_WooCommerce_Api_Pagination_Automattic_WooCommerce_Api_Pagination_IdCursorFilter.before_id())).to_i64())
	if rt.is_true(rt.less_equal(var_after, rt.new_int(0)))
		&& rt.is_true(rt.less_equal(var_before, rt.new_int(0))) {
		return where
	}
	if rt.is_true(rt.greater(var_after, rt.new_int(0))) {
		where = where +(rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.new_string(' AND '), rt.get_property(var_wpdb, 'posts')), rt.new_string('.ID > %d')), var_after.clone()])).str()
	}
	if rt.is_true(rt.greater(var_before, rt.new_int(0))) {
		where = where +(rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.new_string(' AND '), rt.get_property(var_wpdb, 'posts')), rt.new_string('.ID < %d')), var_before.clone()])).str()
	}
	return where
}

fn Class_Automattic_WooCommerce_Api_Pagination_IdCursorFilter.decode_id_cursor(cursor string, name string) i64 {
	mut var_raw := rt.call_function('base64_decode', [rt.new_string(cursor),
		rt.new_bool(true)])
	if rt.is_true(rt.identical(rt.new_bool(false), var_raw))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('ctype_digit', [var_raw.clone()])))))
		|| rt.new_int(var_raw.to_i64()) <= 0 {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Api_ApiException', []string{}, create_automattic_woocommerce_api_apiexception(rt.call_function('sprintf', [
			rt.new_string('Invalid `%s` cursor.'),
			rt.new_string(name),
		]), rt.new_string('INVALID_ARGUMENT'), rt.new_int(400))))
	}
	return rt.new_int(var_raw.to_i64())
}
...
fn (mut this Class_Automattic_WooCommerce_Api_Pagination_IdCursorFilter) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'apply' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Api_Pagination_WP_Query](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
			return rt.new_string(Class_Automattic_WooCommerce_Api_Pagination_IdCursorFilter.apply(dispatch_arg_0, mut
				dispatch_arg_1))
		}
		'decode_id_cursor' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).str()
			return rt.new_int(Class_Automattic_WooCommerce_Api_Pagination_IdCursorFilter.decode_id_cursor(dispatch_arg_0,
				dispatch_arg_1))
		}
		else {
			return none
