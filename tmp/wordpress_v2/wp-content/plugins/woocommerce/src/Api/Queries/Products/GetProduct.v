...
fn (mut this Class_Automattic_WooCommerce_Api_Queries_Products_GetProduct) authorize(id i64, _preauthorized bool) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_post)))) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('product'), rt.get_property(var_post, 'post_type'))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Api_AuthorizationException', []string{}, create_automattic_woocommerce_api_authorizationexception(rt.new_string('Product not found.'))))
	}
	...
	if rt.is_true(rt.identical(rt.new_int(0), var_current_user_id)) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_current_user_id, rt.new_int((rt.get_property(var_post, 'post_author')).to_i64()))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Api_AuthorizationException', []string{}, create_automattic_woocommerce_api_authorizationexception(rt.new_string('Product not found.'))))
	}
	return true
}
...
fn (mut this Class_Automattic_WooCommerce_Api_Queries_Products_GetProduct) execute(id i64, mut var__query_info Class_Automattic_WooCommerce_Api_Queries_Products_?array) rt.PhpVal {
	...
	mut iife_result_0 := iife_temp_0.from_wc_product(var_wc_product.clone(), rt.new_object('Automattic_WooCommerce_Api_Queries_Products_?array', []string{}, var__query_info))
...
fn create_automattic_woocommerce_api_queries_products_getproduct(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Api_Queries_Products_GetProduct {
...
fn create_automattic_woocommerce_api_authorizationexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Api_AuthorizationException {
