...
fn (mut this Class_Automattic_WooCommerce_Api_Mutations_Coupons_CreateCoupon) execute(mut var_input Class_Automattic_WooCommerce_Api_InputTypes_Coupons_CreateCouponInput) rt.PhpVal {
	...
	mut iter_1 := rt.create_array([rt.ArrayItem{ key: none, val: 'description' },
		rt.ArrayItem{ key: none, val: 'amount' }, rt.ArrayItem{ key: none, val: 'date_expires' },
		rt.ArrayItem{ key: none, val: 'individual_use' }, rt.ArrayItem{
			key: none
			val: 'product_ids'
		}, rt.ArrayItem{ key: none, val: 'excluded_product_ids' },
	...
	for {
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), rt.get_property(var_input,
			'{"nodeType":"Expr_Variable","line":33,"name":"field"}')))))
		{
			rt.call_method(var_wc_coupon, 'set_${var_field.to_string()}', [
				rt.get_property(var_input, '{"nodeType":"Expr_Variable","line":34,"name":"field"}'),
			])
		}
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), rt.get_property(var_input,
...
fn create_automattic_woocommerce_api_mutations_coupons_createcoupon(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Api_Mutations_Coupons_CreateCoupon {
...
fn (mut this Class_Automattic_WooCommerce_Api_Mutations_Coupons_CreateCoupon) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'execute' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Api_InputTypes_Coupons_CreateCouponInput](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
