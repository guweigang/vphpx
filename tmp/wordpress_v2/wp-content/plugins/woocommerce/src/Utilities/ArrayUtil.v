...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.get_nested_value(mut var_items Class_Automattic_WooCommerce_Utilities_array, key string, var_default_value rt.PhpVal) rt.PhpVal {
	...
	mut key_mutated := key
	mut var_key_stack := rt.call_function('explode', [rt.new_string('::'), rt.new_string(key_mutated).clone()])
	mut var_subkey := rt.call_function('array_shift', [var_key_stack.clone()])
	if var_items_mutated.array_isset(var_subkey) {
		mut var_value := var_items_mutated.array_get(var_subkey)
		if rt.is_true(rt.new_int(var_key_stack.clone().array_count())) {
			mut iter_1 := var_key_stack.iterator()
			for {
				item_1 := iter_1.next() or { break }
				mut var_subkey_shadow := item_1.val
				if var_value.clone().is_array() && var_value.array_isset(var_subkey_shadow) {
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.is_truthy(mut var_items Class_Automattic_WooCommerce_Utilities_array, key string) bool {
	mut var_items_mutated := var_items
	mut key_mutated := key
	return var_items_mutated.array_isset(rt.new_string(key_mutated)) && rt.is_true(var_items_mutated.array_get(rt.new_string(key_mutated)))
}
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.get_value_or_default(mut var_items Class_Automattic_WooCommerce_Utilities_array, key string, var_default_value rt.PhpVal) rt.PhpVal {
	mut var_items_mutated := var_items
	mut key_mutated := key
	return if rt.is_true(rt.new_bool(var_items_mutated.array_isset(rt.new_string(key_mutated).clone()))) { var_items_mutated.array_get(rt.new_string(key_mutated)) } else { var_default_value }
}
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.to_ranges_string(mut var_items Class_Automattic_WooCommerce_Utilities_array, item_separator string, range_separator string, sort bool) string {
	mut var_items_mutated := var_items
	if var_sort {
		rt.call_function('sort', [var_items_mutated])
	}
	mut var_point := rt.new_null()
	mut var_range := rt.new_bool(false)
	mut var_str := rt.new_string('')
	mut iter_2 := var_items_mutated.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_i := item_2.val
		if rt.is_true(rt.identical(rt.new_null(), var_point)) {
			var_str = rt.concat(var_str, var_i)
		} else if rt.is_true(rt.identical(rt.add(var_point, rt.new_int(1)), var_i)) {
		var_range = rt.new_bool(true)
		} else {
			if rt.is_true(var_range) {
				var_str = rt.concat(var_str, rt.new_string(range_separator + (var_point).str()))
			var_range = rt.new_bool(false)
			}
			var_str = rt.concat(var_str, rt.new_string(item_separator + (var_i).str()))
		}
	var_point = var_i
	}
	if rt.is_true(var_range) {
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.get_selector_callback(selector_name string, selector_type i64) rt.PhpVal {
	...
	closure_1_fn := fn [var_selector_name] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return rt.call_method(var_item, rt.new_string((var_selector_name).str()), []rt.PhpVal{})
		}
	...
	closure_3_fn := fn [var_selector_name] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_item := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return var_item.array_get(rt.new_string((var_selector_name).str()))
		}
	...
	closure_4_fn := fn [var_selector_name] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		if rt.is_true(rt.new_bool(var_item.clone().is_array())) {
			return var_item.array_get(rt.new_string((var_selector_name).str()))
		} else if rt.is_true(rt.call_function('method_exists', [var_item.clone(), rt.new_string(selector_name)])) {
			return rt.call_method(var_item, rt.new_string((var_selector_name).str()), []rt.PhpVal{})
		} else {
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.select(mut var_items Class_Automattic_WooCommerce_Utilities_array, selector_name string, selector_type i64) rt.PhpVal {
	...
	return rt.call_function('array_map', [var_callback.clone(), var_items_mutated])
}
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.select_as_assoc(mut var_items Class_Automattic_WooCommerce_Utilities_array, selector_name string, selector_type i64) rt.PhpVal {
	...
	mut iter_3 := var_items_mutated.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_item := item_3.val
		mut var_key := rt.call_callable(var_selector_callback, [var_item.clone()])
		...
		var_result.array_get_mut(var_key).array_push(var_item.clone())
	}
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.deep_compute_or_compare_array_diff(mut var_array1 Class_Automattic_WooCommerce_Utilities_array, mut var_array2 Class_Automattic_WooCommerce_Utilities_array, compare bool, strict bool) bool {
	...
	mut iter_4 := var_array1.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_value := item_4.val
		mut var_key := item_4.key
		if rt.is_true(rt.new_bool(var_value.clone().is_array())) {
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_array2.array_isset(var_key.clone())))))) || !(var_array2.array_get(var_key).is_array()) {
				if var_compare {
					return true
				}
				var_diff.array_set(var_key, var_value.clone())
				continue
			}
			...
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_array2.array_isset(var_key.clone())))))) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_value, var_array2.array_get(var_key))))) {
				if var_compare {
					return true
				}
				var_diff.array_set(var_key, var_value.clone())
			}
		} else if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_array2.array_isset(var_key.clone())))))) || rt.is_true(rt.new_bool(!rt.is_true(rt.equal(var_value, var_array2.array_get(var_key))))) {
			if var_compare {
				return true
			}
			var_diff.array_set(var_key, var_value.clone())
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.push_once(mut var_items Class_Automattic_WooCommerce_Utilities_array, var_value rt.PhpVal) bool {
	...
	if rt.is_true(rt.call_function('in_array', [var_value_mutated.clone(), var_items_mutated, rt.new_bool(true)])) {
		return false
	}
	var_items_mutated.array_push(var_value_mutated.clone())
	return true
}
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.ensure_key_is_array(mut var_items Class_Automattic_WooCommerce_Utilities_array, key string, throw_if_existing_is_not_array bool) bool {
	...
	if var_throw_if_existing_is_not_array && !(var_items_mutated.array_get(rt.new_string(key_mutated)).is_array()) {
		mut var_type := if var_items_mutated.array_get(rt.new_string(key_mutated)).is_object() { rt.call_function('get_class', [var_items_mutated.array_get(rt.new_string(key_mutated))]) } else { rt.call_function('gettype', [var_items_mutated.array_get(rt.new_string(key_mutated))]) }
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.group_by_column(mut var_items Class_Automattic_WooCommerce_Utilities_array, column string, single_values bool) rt.PhpVal {
	...
	if var_single_values {
		return rt.call_function('array_combine', [rt.call_function('array_column', [var_items_mutated, rt.new_string(column)]), rt.call_function('array_values', [var_items_mutated])])
	}
	mut var_distinct_column_values := rt.call_function('array_unique', [rt.call_function('array_column', [var_items_mutated, rt.new_string(column)]), rt.get_constant('SORT_REGULAR')])
	mut var_result := rt.call_function('array_fill_keys', [var_distinct_column_values.clone(), rt.new_array()])
	mut iter_5 := var_items_mutated.iterator()
	for {
		item_5 := iter_5.next() or { break }
		mut var_value := item_5.val
		var_result.array_get_mut(var_value.array_get(rt.new_string(column))).array_push(var_value.clone())
	}
	return var_result.clone()
}
...
fn Class_Automattic_WooCommerce_Utilities_ArrayUtil.array_all(mut var_items Class_Automattic_WooCommerce_Utilities_array, mut var_callback Class_Automattic_WooCommerce_Utilities_callable) bool {
	...
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('array_all')])) {
		return (rt.call_function('array_all', [var_items_mutated, var_callback_mutated])).to_bool()
	}
	mut iter_6 := var_items_mutated.iterator()
	for {
		item_6 := iter_6.next() or { break }
		mut var_item := item_6.val
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_callable(var_callback_mutated, [var_item.clone()]))))) {
			return false
		}
	}
	return true
