...
fn Class_Automattic_WooCommerce_Utilities_OrderUtil.init_theorder_object(var_post_or_order_object rt.PhpVal) rt.PhpVal {
	return rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil.class()]), 'init_theorder_object', [var_post_or_order_object.clone()])
}

fn Class_Automattic_WooCommerce_Utilities_OrderUtil.get_post_or_order_id(var_post_or_order_object rt.PhpVal) i64 {
	return (rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil.class()]), 'get_post_or_order_id', [var_post_or_order_object.clone()])).to_i64()
}

fn Class_Automattic_WooCommerce_Utilities_OrderUtil.is_order(var_order_id rt.PhpVal, var_types rt.PhpVal) rt.PhpVal {
	return rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil.class()]), 'is_order', [var_order_id.clone(), var_types.clone()])
}

fn Class_Automattic_WooCommerce_Utilities_OrderUtil.get_order_type(var_order_id rt.PhpVal) rt.PhpVal {
	return rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_Utilities_COTMigrationUtil.class()]), 'get_order_type', [var_order_id.clone()])
}
...
fn Class_Automattic_WooCommerce_Utilities_OrderUtil.is_order_list_table_screen(order_type string) bool {
	...
	return (rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_Admin_Orders_PageController.class()]), 'is_order_screen', [rt.new_string(order_type_mutated).clone(), rt.new_string('list')])).to_bool()
}

fn Class_Automattic_WooCommerce_Utilities_OrderUtil.is_order_edit_screen(order_type string) bool {
	mut order_type_mutated := order_type
	return (rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_Admin_Orders_PageController.class()]), 'is_order_screen', [rt.new_string(order_type_mutated).clone(), rt.new_string('edit')])).to_bool()
}

fn Class_Automattic_WooCommerce_Utilities_OrderUtil.is_new_order_screen(order_type string) bool {
	mut order_type_mutated := order_type
	return (rt.call_method(rt.call_method(rt.call_function('wc_get_container', []rt.PhpVal{}), 'get', [Class_Automattic_WooCommerce_Internal_Admin_Orders_PageController.class()]), 'is_order_screen', [rt.new_string(order_type_mutated).clone(), rt.new_string('new')])).to_bool()
}
...
fn Class_Automattic_WooCommerce_Utilities_OrderUtil.get_count_for_type(var_order_type rt.PhpVal) rt.PhpVal {
	...
	mut var_count_per_status := var_order_count_cache.get(var_order_type_mutated.clone())
	if rt.is_true(rt.identical(rt.new_null(), var_count_per_status)) {
		...
		mut var_results := rt.call_method(var_wpdb, 'get_results', [rt.call_method(var_wpdb, 'prepare', [rt.new_string('SELECT `status`, COUNT(*) AS `count` FROM ' + (Class_Automattic_WooCommerce_Utilities_OrderUtil.get_table_for_orders()).str() + ' WHERE `type` = %s GROUP BY `status`'), var_order_type_mutated.clone()]), rt.get_constant('ARRAY_A')])
		var_count_per_status = rt.call_function('array_map', [rt.new_string('absint'), rt.call_function('array_column', [var_results.clone(), rt.new_string('count'), rt.new_string('status')])])
		} else {
		var_count_per_status = rt.cast_array(rt.call_function('wp_count_posts', [var_order_type_mutated.clone()]))
		}
		var_count_per_status = rt.call_function('array_merge', [rt.call_function('array_fill_keys', [rt.call_function('array_merge', [rt.func_array_keys(rt.call_function('wc_get_order_statuses', []rt.PhpVal{})), rt.create_array([rt.ArrayItem{ key: none, val: Class_Automattic_WooCommerce_Enums_OrderStatus.trash() }])]), rt.new_int(0)]), var_count_per_status.clone()])
		var_order_count_cache.set_multiple(var_order_type_mutated.clone(), var_count_per_status.clone())
	}
	return var_count_per_status.clone()
}

fn Class_Automattic_WooCommerce_Utilities_OrderUtil.remove_status_prefix(status string) string {
	mut status_mutated := status
	if rt.is_true(rt.identical(rt.call_function('strpos', [rt.new_string(status_mutated).clone(), rt.new_string('wc-')]), rt.new_int(0))) {
	status_mutated = (rt.call_function('substr', [rt.new_string(status_mutated).clone(), rt.new_int(3)])).str()
	}
...
fn Class_Automattic_WooCommerce_Utilities_OrderUtil.uses_new_full_refund_data() bool {
	...
	return rt.is_true(rt.call_function('version_compare', [var_db_version.clone(), rt.new_string('10.2.0'), rt.new_string('>=')])) && rt.is_true(rt.identical(rt.new_string('no'), var_uses_old_full_refund_data))
}

fn Class_Automattic_WooCommerce_Utilities_OrderUtil.unknown_orders_data_store_in_use() bool {
	mut iife_temp_0 := Class_Automattic_WooCommerce_Utilities_WC_Data_Store{}
	mut iife_result_0 := iife_temp_0.load(rt.new_string('order'))
	return rt.is_true(rt.new_bool(!(rt.is_true(Class_Automattic_WooCommerce_Utilities_OrderUtil.custom_orders_table_usage_is_enabled())))) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Automattic_WooCommerce_Utilities_WC_Order_Data_Store_CPT.class(), rt.call_method(iife_result_0, 'get_current_class_name', []rt.PhpVal{})))))
}
...
struct Class_Automattic_WooCommerce_Utilities_WC_Data_Store {
	rt.PhpObjectBase
}
...
fn create_automattic_woocommerce_utilities_wc_data_store(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Utilities_WC_Data_Store {
	mut obj := &Class_Automattic_WooCommerce_Utilities_WC_Data_Store{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
}
...
fn (mut this Class_Automattic_WooCommerce_Utilities_WC_Data_Store) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	return none
}

fn (this &Class_Automattic_WooCommerce_Utilities_WC_Data_Store) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Utilities_WC_Data_Store) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	return this.PhpObjectBase.dispatch_set_prop(prop_name, val)
}
