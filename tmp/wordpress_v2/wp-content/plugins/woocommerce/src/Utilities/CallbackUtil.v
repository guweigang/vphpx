...
fn Class_Automattic_WooCommerce_Utilities_CallbackUtil.get_callback_signature(var_callback rt.PhpVal) string {
	...
	if var_callback.clone().is_array() && 2 == var_callback.clone().array_count() {
		mut var_target := var_callback.array_get(rt.new_int(0))
		mut var_method := var_callback.array_get(rt.new_int(1))
		if var_target.clone().is_object() || var_target.clone().is_string()
			&& var_method.clone().is_string() {
			mut var_class := if var_target.clone().is_object() { rt.call_function('get_class', [
					var_target.clone(),
				]) } else { var_target }
		...
		'Automattic_WooCommerce_Utilities_Closure')))
	{
		...
		if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Utilities_Exception') {
			...
			return 'Closure@' + (rt.call_function('spl_object_hash', [var_callback.clone()])).str()
			unsafe {
				goto end_label_1
			}
		} else {
	...
	if rt.is_true(rt.new_bool(var_callback.clone().is_object())) {
		...
		if rt.instance_of(var_e_2, 'Automattic_WooCommerce_Utilities_Exception') {
			var_e = var_e_2.clone()
			return (rt.call_function('get_class', [var_callback.clone()])).str() + '::__invoke'
			unsafe {
				goto end_label_2
			}
		} else {
	...
	return (rt.call_function('serialize', [var_callback.clone()])).str()
}
...
fn Class_Automattic_WooCommerce_Utilities_CallbackUtil.get_hook_callback_signatures(hook_name string) rt.PhpVal {
	...
	mut iter_1 :=
		rt.get_property(var_wp_filter.array_get(rt.new_string(hook_name)), 'callbacks').iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_priority_callbacks := item_1.val
		mut var_priority := item_1.key
		closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_callback_data := if args.len > 0 { args[0].clone() } else { rt.new_null() }
			return Class_Automattic_WooCommerce_Utilities_CallbackUtil.get_callback_signature(var_callback_data.array_get(rt.new_string('function')))
		}
		...
		var_result.array_set(var_priority, rt.call_function('array_map', [
			...
			rt.call_function('array_values', [var_priority_callbacks.clone()]),
		]))
	}
...
fn Class_Automattic_WooCommerce_Utilities_CallbackUtil.get_closure_signature(mut var_closure Class_Automattic_WooCommerce_Utilities_Closure) string {
	mut var_reflection := create_automattic_woocommerce_utilities_reflectionfunction(var_closure)
	...
	return (rt.call_function('sprintf', [rt.new_string('Closure@%s:%d-%d'),
		var_file.clone(), var_start.clone(), var_end.clone()])).str()
}
...
fn Class_Automattic_WooCommerce_Utilities_CallbackUtil.get_invokable_signature(mut var_invokable Class_Automattic_WooCommerce_Utilities_object) string {
	mut var_method := create_automattic_woocommerce_utilities_reflectionmethod(var_invokable,
...
fn create_automattic_woocommerce_utilities_reflectionfunction(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Utilities_ReflectionFunction {
...
fn create_automattic_woocommerce_utilities_reflectionexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Utilities_ReflectionException {
...
fn create_automattic_woocommerce_utilities_reflectionmethod(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Utilities_ReflectionMethod {
