...
fn Class_Automattic_WooCommerce_Utilities_DiscountsUtil.is_coupon_emails_allowed(var_check_emails rt.PhpVal, var_restrictions rt.PhpVal) bool {
	...
	for {
		...
		if rt.is_true(rt.call_function('in_array', [var_check_email.clone(),
			var_restrictions.clone(), rt.new_bool(true)]))
		{
			return true
		}
		...
		for {
			...
			mut var_regex := rt.new_string('/^' +
				(rt.call_function('str_replace', [rt.new_string('*'), rt.new_string('(.+)?'), var_restriction.clone()])).str() +
				'$/')
			rt.call_function('preg_match', [var_regex.clone(),
				var_check_email.clone(), var_match.clone()])
