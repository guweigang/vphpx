	...
	return (rt.call_function('implode', [rt.new_string(''),
		rt.call_function('array_map', [
			rt.create_array([
				rt.ArrayItem{
					key: none
					val: Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_CssConcatenator.class()
				},
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_CssConcatenator) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'hasEquivalentSelectors' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
			return rt.new_bool(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_CssConcatenator.hasequivalentselectors(mut dispatch_arg_0, mut
				dispatch_arg_1))
		}
		'getMediaRuleCss' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_object](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
