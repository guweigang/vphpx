...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_HtmlPruner) removeelementswithdisplaynone() rt.PhpVal {
	mut var_elementsWithStyleDisplayNone := rt.call_method(this.getxpath(), 'query', [
		Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_HtmlPruner.display_none_matcher(),
	])
	if rt.is_true(rt.identical(rt.get_property(var_elementsWithStyleDisplayNone, 'length'),
		rt.new_int(0)))
	{
		return rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_HtmlPruner',
			[]string{}, this)
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_HtmlPruner) removeredundantclasses(mut var_classesToKeep Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_array) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_classesToKeep, rt.new_array())))) {
		this.removeclassesfromelements(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMNodeList](var_elementsWithClassAttribute), mut
			var_classesToKeep)
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_HtmlPruner) removeclassesfromelements(mut var_elements Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMNodeList, mut var_classesToKeep Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_array) {
	mut var_classesToKeepIntersector :=
		create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_arrayintersector(var_classesToKeep)
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_HtmlPruner) removeredundantclassesaftercssinlined(mut var_cssInliner Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) rt.PhpVal {
	...
	this.removeredundantclasses(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_array](rt.func_array_keys(var_classesToKeepAsKeys.clone())))
	return rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_HtmlPruner',
		[]string{}, this)
}
...
fn create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_arrayintersector(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_ArrayIntersector {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_HtmlPruner) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'removeClassesFromElements' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMNodeList](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
		...
		'removeRedundantClassesAfterCssInlined' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
