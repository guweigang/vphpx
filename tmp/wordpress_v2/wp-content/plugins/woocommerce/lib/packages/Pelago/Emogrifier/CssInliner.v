...
pub fn Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.cache_key_selector() i64 {
	return 0
}

pub fn Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.cache_key_combined_styles() i64 {
	return 1
}

pub fn Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.pseudo_class_matcher() string {
	return 'empty|(?:first|last|nth(?:-last)?+|only)-(?:child|of-type)|not\\([[:ascii:]]*\\)|root'
}

pub fn Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.of_type_pseudo_class_matcher() string {
	return '(?:first|last|nth(?:-last)?+|only)-of-type'
}

pub fn Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.combinator_matcher() string {
	return '(?:\\s++|\\s*+[>+~]\\s*+)(?=[[:alpha:]_\\-.#*:\\[])'
}

pub fn Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.qsa_always_throw_parse_exception() string {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) inlinecss(css string) rt.PhpVal {
	mut css_mutated := css
	this.clearallcaches()
	...
	mut var_parsedCss := create_automattic_woocommerce_vendor_pelago_emogrifier_css_cssdocument(var_combinedCss.clone(),
		this.debug)
	...
	for {
		...
		for {
			...
			this.copyinlinablecsstostyleattribute(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMElement](this.ensurenodeiselement(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNode](var_node))), mut
				rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_cssRule))
		}
	}
	...
	this.determinematchinguninlinablecssrules(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_cssRules.array_get(rt.new_string('uninlinable'))))
	this.copyuninlinablecsstostylenode(mut var_parsedCss)
	return rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner', []string{},
		this)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) disableinlinestyleattributesparsing() rt.PhpVal {
	this.isInlineStyleAttributesParsingEnabled = false
	return rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner', []string{},
		this)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) disablestyleblocksparsing() rt.PhpVal {
	this.isStyleBlocksParsingEnabled = false
	return rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner', []string{},
		this)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) addallowedmediatype(mediaName string) rt.PhpVal {
	this.allowedMediaTypes.array_set(mediaName, true)
	return rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner', []string{},
		this)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) removeallowedmediatype(mediaName string) rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) addexcludedselector(selector string) rt.PhpVal {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) removeexcludedselector(selector string) rt.PhpVal {
	mut selector_mutated := selector
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) addexcludedcssselector(selector string) rt.PhpVal {
	mut selector_mutated := selector
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) removeexcludedcssselector(selector string) rt.PhpVal {
	mut selector_mutated := selector
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) setdebug(debug bool) rt.PhpVal {
	this.debug = rt.new_bool(debug)
	return rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner', []string{},
		this)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) getmatchinguninlinableselectors() rt.PhpVal {
	return rt.call_function('array_column', [this.getmatchinguninlinablecssrules(),
		rt.new_string('selector')])
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) getmatchinguninlinablecssrules() rt.PhpVal {
	if !(this.matchingUninlinableCssRules.is_array()) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_BadMethodCallException',
			[]string{}, create_automattic_woocommerce_vendor_pelago_emogrifier_badmethodcallexception(rt.new_string('inlineCss must be called first'),
			rt.new_int(1568385221))))
	}
	return this.matchingUninlinableCssRules
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) clearallcaches() {
	this.caches = rt.create_array([
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.cache_key_selector()
			val: rt.new_array()
		},
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.cache_key_combined_styles()
			val: rt.new_array()
		},
	])
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) purgevisitednodes() {
	this.visitedNodes = rt.new_array()
	this.styleAttributesForNodes = rt.new_array()
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) normalizestyleattributesofallnodes() {
	mut iter_3 := this.getallnodeswithstyleattribute().iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_node := item_3.val
		if this.isInlineStyleAttributesParsingEnabled {
			this.normalizestyleattributes(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMElement](var_node))
		}
		rt.call_method(var_node, 'removeAttribute', [rt.new_string('style')])
	}
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) getallnodeswithstyleattribute() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_matches,
		'Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNodeList'))))))
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_RuntimeException',
			[]string{}, create_automattic_woocommerce_vendor_pelago_emogrifier_runtimeexception(
			'XPatch query failed: ' + var_query.str(), rt.new_int(1618577797))))
	}
	return var_matches.clone()
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) normalizestyleattributes(mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMElement) {
	mut var_node_mutated := var_node
	mut var_declarationBlockParser :=
		create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_declarationblockparser()
	...
	mut var_normalizedOriginalStyle := rt.call_method(rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) getcssfromallstylenodes() string {
	...
	for {
		...
		if rt.is_true(rt.new_bool(rt.instance_of(var_parentNode,
			'Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNode')))
		{
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) getnodestoexclude() rt.PhpVal {
	...
	for {
		...
		for {
			...
			var_excludedNodes.array_push(this.ensurenodeiselement(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNode](var_node)))
		}
	}
	return var_excludedNodes.clone()
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) queryselectorall(selectors string, mut var_options Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array) rt.PhpVal {
	...
	if rt.instance_of(var_e_1,
		'Automattic_WooCommerce_Vendor_Symfony_Component_CssSelector_Exception_ParseException')
	{
		mut var_exception := var_e_1.clone()
		mut var_alwaysThrowParseException := if !(var_options.array_get(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.qsa_always_throw_parse_exception())).is_null() {
			var_options.array_get(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.qsa_always_throw_parse_exception())
		} else {
			rt.new_bool(false)
		}
		...
		return rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNodeList',
			[]string{}, create_automattic_woocommerce_vendor_pelago_emogrifier_domnodelist())
		unsafe {
			goto end_label_1
		}
	} else if rt.instance_of(var_e_1,
		'Automattic_WooCommerce_Vendor_Pelago_Emogrifier_RuntimeException')
	{
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) ensurenodeiselement(mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNode) rt.PhpVal {
	mut var_node_mutated := var_node
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_node_mutated,
		'Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMElement'))))))
	{
		...
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_UnexpectedValueException',
			[]string{}, create_automattic_woocommerce_vendor_pelago_emogrifier_unexpectedvalueexception(
			var_path.str() + ' is not a DOMElement.', rt.new_int(1617975914))))
	}
	return rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNode', []string{},
		var_node_mutated)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) getcssselectorconverter() rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(this.cssSelectorConverter,
		'Automattic_WooCommerce_Vendor_Symfony_Component_CssSelector_CssSelectorConverter'))))))
	{
		this.cssSelectorConverter =
			create_automattic_woocommerce_vendor_symfony_component_cssselector_cssselectorconverter()
	}
	return this.cssSelectorConverter
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) collatecssrules(mut var_parsedCss Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) rt.PhpVal {
	mut var_parsedCss_mutated := var_parsedCss
	mut var_matches :=
		var_parsedCss_mutated.getstylerulesdata(rt.func_array_keys(this.allowedMediaTypes))
	mut var_preg := rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
	...
	for {
		...
		if this.excludedCssSelectors.array_count() > 0 {
			closure_2_fn := fn [var_preg] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			var_selectors = rt.call_function('array_filter', [
				var_selectorsNormalized.clone(), rt.new_closure(closure_4_fn)])
		}
	...
	closure_5_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return rt.new_int(this.sortbyselectorprecedence(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_first), mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_second)))
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) hasunsupportedpseudoclass(selector string) bool {
	mut selector_mutated := selector
	mut var_preg := rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
		'throwExceptions', [this.debug])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_preg, 'match', [
		rt.new_string('/:(?!' +
			(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.pseudo_class_matcher()).str() + ')[\\w\\-]/i'),
	...
	if rt.is_true(rt.identical(rt.call_method(var_preg, 'match', [
		rt.new_string('/:(?:' +
			(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.of_type_pseudo_class_matcher()).str() + ')/i'),
		rt.new_string(selector_mutated).clone(),
	]), rt.new_int(0)))
	{
		return false
	...
	mut iter_9 := rt.call_method(var_preg, 'split', [
		rt.new_string('/' +
			(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.combinator_matcher()).str() + '/'),
		rt.new_string(selector_mutated).clone(),
	]).iterator()
	for {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) selectorparthasunsupportedoftypepseudoclass(selectorPart string) bool {
	mut var_preg := rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
		'throwExceptions', [this.debug])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_preg, 'match', [
		rt.new_string('/^[\\w\\-]/'),
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) sortbyselectorprecedence(mut var_first Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array, mut var_second Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array) i64 {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) getcssselectorprecedence(selector string) i64 {
	mut selector_mutated := selector
	mut var_selectorKey := rt.new_string(selector_mutated).clone()
	if this.caches.array_get(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.cache_key_selector()).array_isset(var_selectorKey) {
		return (this.caches.array_get(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.cache_key_selector()).array_get(var_selectorKey)).to_i64()
	}
	mut var_preg := rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
		'throwExceptions', [this.debug])
	...
	this.caches.array_get_mut(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.cache_key_selector()).array_set(var_selectorKey,
		var_precedence.clone())
	return var_precedence.to_i64()
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) copyinlinablecsstostyleattribute(mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMElement, mut var_cssRule Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array) {
	mut var_node_mutated := var_node
	...
	rt.call_method(var_node_mutated, 'setAttribute', [rt.new_string('style'),
		rt.new_string(this.generatestylestringfromdeclarationsarrays(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_oldStyleDeclarations), mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_newStyleDeclarations)))])
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) generatestylestringfromdeclarationsarrays(mut var_oldStyles Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array, mut var_newStyles Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array) string {
	...
	if this.caches.array_get(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.cache_key_combined_styles()).array_isset(var_cacheKey) {
		return (this.caches.array_get(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.cache_key_combined_styles()).array_get(var_cacheKey)).str()
	}
	...
	for {
		...
		if rt.is_true(rt.identical(var_trimmedAttributeName, rt.new_string(''))) {
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_UnexpectedValueException',
				[]string{}, create_automattic_woocommerce_vendor_pelago_emogrifier_unexpectedvalueexception(rt.new_string('An empty property name was encountered.'),
	...
	this.caches.array_get_mut(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.cache_key_combined_styles()).array_set(var_cacheKey,
		var_trimmedStyle.clone())
	return var_trimmedStyle.str()
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) attributevalueisimportant(attributeValue string) bool {
	return rt.new_bool(!rt.is_true(rt.identical(rt.call_method(rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
		'throwExceptions', [this.debug]), 'match', [rt.new_string('/!\\s*+important$/i'),
		rt.new_string(attributeValue)]), rt.new_int(0))))
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) fillstyleattributeswithmergedstyles() {
	mut var_declarationBlockParser :=
		create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_declarationblockparser()
	...
	for {
		...
		rt.call_method(var_node, 'setAttribute', [rt.new_string('style'),
			rt.new_string(this.generatestylestringfromdeclarationsarrays(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_currentStyleAttributes), mut
				rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_styleAttributesForNode)))])
	}
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) removeimportantannotationfromallinlinestyles() {
	mut iter_14 := this.getallnodeswithstyleattribute().iterator()
	for {
		item_14 := iter_14.next() or { break }
		mut var_node := item_14.val
		this.removeimportantannotationfromnodeinlinestyle(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMElement](var_node))
	}
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) removeimportantannotationfromnodeinlinestyle(mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMElement) {
	mut var_node_mutated := var_node
	mut var_style := rt.call_method(var_node_mutated, 'getAttribute', [
		rt.new_string('style'),
	])
	mut var_inlineStyleDeclarations := rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_declarationblockparser(),
	...
	for {
		...
		if this.attributevalueisimportant(var_value.str()) {
			var_importantStyleDeclarations.array_set(var_property, rt.call_method(rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
	...
	rt.call_method(var_node_mutated, 'setAttribute', [rt.new_string('style'),
		rt.new_string(this.generatestylestringfromsingledeclarationsarray(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_inlineStyleDeclarationsInNewOrder)))])
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) generatestylestringfromsingledeclarationsarray(mut var_styleDeclarations Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array) string {
	return this.generatestylestringfromdeclarationsarrays(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](rt.new_array()), mut
		var_styleDeclarations)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) determinematchinguninlinablecssrules(mut var_cssRules Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array) {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) existsmatchforselectorincssrule(mut var_cssRule Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array) bool {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) existsmatchforcssselector(cssSelector string) bool {
	mut var_nodesMatchingSelector := this.queryselectorall(cssSelector, mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](rt.create_array([
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.qsa_always_throw_parse_exception()
			val: true
		},
	])))
	if rt.has_exception() {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) removeunmatchablepseudocomponents(selector string) string {
	mut selector_mutated := selector
	mut var_preg := rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
		'throwExceptions', [this.debug])
	closure_7_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_matches := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return this.replaceunmatchablenotcomponent(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_matches))
	}
	closure_8_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_matches := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return this.replaceunmatchablenotcomponent(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](var_matches))
	}
	mut var_selectorWithoutNots := rt.new_string(rt.call_method(rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
		'throwExceptions', [this.debug]), 'replaceCallback', [
	...
	mut var_selectorWithoutUnmatchablePseudoComponents := rt.new_string(this.removeselectorcomponents(
		':(?!' +
		(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.pseudo_class_matcher()).str() + '):?+[\\w\\-]++(?:\\([^\\)]*+\\))?+',
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) replaceunmatchablenotcomponent(mut var_matches Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array) string {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) removeselectorcomponents(matcher string, selector string) string {
	mut selector_mutated := selector
	return (rt.call_method(rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
		'throwExceptions', [this.debug]), 'replace', [
		rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) removeunsupportedoftypepseudoclasses(selectorPart string) string {
	if !(this.selectorparthasunsupportedoftypepseudoclass(selectorPart)) {
		return selectorPart
	}
	return this.removeselectorcomponents(':(?:' +
		(Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner.of_type_pseudo_class_matcher()).str() + ')(?:\\([^\\)]*+\\))?+',
		selectorPart)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) copyuninlinablecsstostylenode(mut var_parsedCss Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) {
	mut var_parsedCss_mutated := var_parsedCss
		...
		rt.new_array()))))
	{
		mut var_cssConcatenator :=
			create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_cssconcatenator()
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) addstyleelementtodocument(css string) {
	mut css_mutated := css
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) getheadelement() rt.PhpVal {
	...
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_UnexpectedValueException',
			[]string{}, create_automattic_woocommerce_vendor_pelago_emogrifier_unexpectedvalueexception(rt.new_string('There is no HEAD element. This should never happen.'),
...
struct Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument {
	rt.PhpObjectBase
}

struct Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_BadMethodCallException {
	rt.PhpObjectBase
}

struct Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_RuntimeException {
	rt.PhpObjectBase
}
...
struct Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNodeList {
	rt.PhpObjectBase
}

struct Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_UnexpectedValueException {
	rt.PhpObjectBase
}
...
struct Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_CssConcatenator {
	rt.PhpObjectBase
}
...
fn create_automattic_woocommerce_vendor_pelago_emogrifier_css_cssdocument(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument {
	mut obj := &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
}

fn create_automattic_woocommerce_vendor_pelago_emogrifier_badmethodcallexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_BadMethodCallException {
	mut obj := &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_BadMethodCallException{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
}

fn create_automattic_woocommerce_vendor_pelago_emogrifier_runtimeexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_RuntimeException {
	mut obj := &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_RuntimeException{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
}
...
fn create_automattic_woocommerce_vendor_pelago_emogrifier_domnodelist(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNodeList {
	mut obj := &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNodeList{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
}

fn create_automattic_woocommerce_vendor_pelago_emogrifier_unexpectedvalueexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_UnexpectedValueException {
	mut obj := &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_UnexpectedValueException{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
}

fn create_automattic_woocommerce_vendor_symfony_component_cssselector_cssselectorconverter(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Symfony_Component_CssSelector_CssSelectorConverter {
...
fn create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_cssconcatenator(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_CssConcatenator {
	mut obj := &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_CssConcatenator{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_CssInliner) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'normalizeStyleAttributes' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMElement](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
		...
		'querySelectorAll' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
		...
		'ensureNodeIsElement' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNode](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
		...
		'collateCssRules' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
		...
		'sortBySelectorPrecedence' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	return none
}

fn (this &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	return this.PhpObjectBase.dispatch_set_prop(prop_name, val)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_BadMethodCallException) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	return none
}

fn (this &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_BadMethodCallException) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_BadMethodCallException) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	return this.PhpObjectBase.dispatch_set_prop(prop_name, val)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_RuntimeException) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	return none
}

fn (this &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_RuntimeException) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_RuntimeException) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	return this.PhpObjectBase.dispatch_set_prop(prop_name, val)
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNodeList) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	return none
}

fn (this &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNodeList) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_DOMNodeList) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	return this.PhpObjectBase.dispatch_set_prop(prop_name, val)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_UnexpectedValueException) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	return none
}

fn (this &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_UnexpectedValueException) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_UnexpectedValueException) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	return this.PhpObjectBase.dispatch_set_prop(prop_name, val)
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_CssConcatenator) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	return none
}

fn (this &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_CssConcatenator) dispatch_get_prop(prop_name string) ?rt.PhpVal {
	return this.PhpObjectBase.dispatch_get_prop(prop_name)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Utilities_CssConcatenator) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	return this.PhpObjectBase.dispatch_set_prop(prop_name, val)
}
