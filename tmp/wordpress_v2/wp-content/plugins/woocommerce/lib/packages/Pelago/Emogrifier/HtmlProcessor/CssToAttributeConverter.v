...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) convertcsstovisualattributes() rt.PhpVal {
	...
	for {
		...
		this.mapcsstohtmlattributes(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_array](var_inlineStyleDeclarations), mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement](var_node))
	}
	return rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter',
		[]string{}, this)
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) getallnodeswithstyleattribute() rt.PhpVal {
	return rt.call_method(this.getxpath(), 'query', [rt.new_string('//*[@style]')])
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) mapcsstohtmlattributes(mut var_styles Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_array, mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement) {
	mut var_styles_mutated := var_styles
	mut iter_2 := var_styles_mutated.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_value := item_2.val
		mut var_property := item_2.key
		var_value = rt.new_string(rt.call_function('str_replace', [
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) mapcsstohtmlattribute(property string, value string, mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement) {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) mapsimplecssproperty(property string, value string, mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement) bool {
	mut value_mutated := value
	if !(this.cssToHtmlMap.array_isset(rt.new_string(property))) {
		return false
	}
	mut var_mapping := this.cssToHtmlMap.array_get(rt.new_string(property))
	mut var_nodesMatch := rt.new_bool(!(var_mapping.array_isset(rt.new_string('nodes')))
		|| rt.is_true(rt.call_function('in_array', [rt.get_property(var_node, 'nodeName'), var_mapping.array_get(rt.new_string('nodes')), rt.new_bool(true)])))
	mut var_valuesMatch := rt.new_bool(!(var_mapping.array_isset(rt.new_string('values')))
		|| rt.is_true(rt.call_function('in_array', [rt.new_string(value_mutated).clone(), var_mapping.array_get(rt.new_string('values')), rt.new_bool(true)])))
	mut var_canBeMapped := rt.new_bool(rt.is_true(var_nodesMatch) && rt.is_true(var_valuesMatch))
	if rt.is_true(var_canBeMapped) {
		var_node.setattribute(var_mapping.array_get(rt.new_string('attribute')),
			rt.new_string(value_mutated))
	}
	return var_canBeMapped.to_bool()
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) mapcomplexcssproperty(property string, value string, mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement) {
	mut value_mutated := value
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) mapbackgroundproperty(mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement, value string) {
	mut value_mutated := value
	mut var_styles := rt.call_function('explode', [rt.new_string(' '),
		rt.new_string(value_mutated).clone(), rt.new_int(2)])
	mut var_first := var_styles.array_get(rt.new_int(0))
	if var_first.array_get(rt.new_int(0)).is_long()
		|| var_first.array_get(rt.new_int(0)).is_double()
		|| rt.is_true(rt.identical(rt.call_function('strncmp', [var_first.clone(), rt.new_string('url'), rt.new_int(3)]), rt.new_int(0))) {
		return
	}
	var_node.setattribute(rt.new_string('bgcolor'), var_first.clone())
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) mapwidthorheightproperty(mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement, value string, property string) {
	mut value_mutated := value
	...
	mut var_number := var_preg.replace(rt.new_string('/[^0-9.%]/'), rt.new_string(''),
		rt.new_string(value_mutated))
	var_node.setattribute(rt.new_string(property), var_number.clone())
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) mapmarginproperty(mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement, value string) {
	mut value_mutated := value
	if !(this.istableorimagenode(mut var_node)) {
		return
	}
	mut var_margins := this.parsecssshorthandvalue(value_mutated)
	if rt.is_true(rt.identical(var_margins.array_get(rt.new_string('left')), rt.new_string('auto')))
		&& rt.is_true(rt.identical(var_margins.array_get(rt.new_string('right')), rt.new_string('auto'))) {
		var_node.setattribute(rt.new_string('align'), rt.new_string('center'))
	}
}

fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) mapborderproperty(mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement, value string) {
	mut value_mutated := value
	if !(this.istableorimagenode(mut var_node)) {
		return
	}
	if rt.is_true(rt.identical(rt.new_string(value_mutated), rt.new_string('none')))
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) istableorimagenode(mut var_node Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement) bool {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) parsecssshorthandvalue(value string) rt.PhpVal {
	mut value_mutated := value
	mut var_values := rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
		'split', [rt.new_string('/\\s+/'), rt.new_string(value_mutated).clone()])
	mut var_css := rt.new_array()
	var_css.array_set('top', var_values.array_get(rt.new_int(0)))
	var_css.array_set('right', if var_values.clone().array_count() > 1 {
		var_values.array_get(rt.new_int(1))
	} else {
		var_css.array_get(rt.new_string('top'))
	})
	var_css.array_set('bottom', if var_values.clone().array_count() > 2 {
		var_values.array_get(rt.new_int(2))
	} else {
		var_css.array_get(rt.new_string('top'))
	})
	var_css.array_set('left', if var_values.clone().array_count() > 3 {
		var_values.array_get(rt.new_int(3))
	} else {
		var_css.array_get(rt.new_string('right'))
	})
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_CssToAttributeConverter) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'mapCssToHtmlAttributes' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
		...
		'mapCssToHtmlAttribute' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
		...
		'mapBackgroundProperty' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_HtmlProcessor_DOMElement](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
