...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) construct(css string, debug bool) {
	...
	this.sabberwormCssDocument = rt.call_method(create_automattic_woocommerce_vendor_sabberworm_css_parser(rt.new_string(css.trim_space()),
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) hasnestedatrule(css string) bool {
	return rt.new_bool(!rt.is_true(rt.identical(rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
		'match', [
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) getstylerulesdata(mut var_allowedMediaTypes Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_array) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.new_bool(rt.instance_of(var_rule,
			'Automattic_WooCommerce_Vendor_Sabberworm_CSS_CSSList_AtRuleBlockList')))
		{
			mut var_containingAtRule := rt.new_string(this.getfilteredatidentifierandrule(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_CSSList_AtRuleBlockList](var_rule), mut
			...
			if rt.is_true(rt.new_bool(var_containingAtRule.clone().is_string())) {
				...
				for {
					...
					{
						var_ruleMatches.array_push(create_automattic_woocommerce_vendor_pelago_emogrifier_css_stylerule(var_nestedRule.clone(),
			...
			'Automattic_WooCommerce_Vendor_Sabberworm_CSS_RuleSet_DeclarationBlock')))
		{
			var_ruleMatches.array_push(create_automattic_woocommerce_vendor_pelago_emogrifier_css_stylerule(var_rule.clone()))
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) rendernonconditionalatrules() string {
	...
	mut var_atRules := rt.call_function('array_filter', [var_cssContents.clone(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument',
				[]string{}, &this) },
	...
	mut var_atRulesDocument :=
		create_automattic_woocommerce_vendor_sabberworm_css_csslist_document()
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) getfilteredatidentifierandrule(mut var_rule Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_CSSList_AtRuleBlockList, mut var_allowedMediaTypes Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_array) string {
	...
	if rt.is_true(rt.identical(var_rule.atrulename(), rt.new_string('media'))) {
		...
		if rt.is_true(rt.new_bool(var_mediaType.clone().to_string().trim_space() != '')) {
			...
			mut var_isAllowed := rt.new_bool(!rt.is_true(rt.identical(rt.call_method(create_automattic_woocommerce_vendor_pelago_emogrifier_utilities_preg(),
				'match', [
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) isvalidatruletorender(mut var_rule Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable) bool {
	if rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable',
		[]string{}, var_rule), 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Property_Charset')))
	{
		return false
	}
	if rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable',
		[]string{}, var_rule), 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Property_Import')))
	{
		return this.isImportRuleAllowed
	}
	this.isImportRuleAllowed = false
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable',
		[]string{}, var_rule), 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Property_AtRule'))))))
	{
		return false
	}
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('media'))) {
		...
		var_result = rt.new_bool(
			rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable', []string{}, var_rule), 'Automattic_WooCommerce_Vendor_Sabberworm_CSS_RuleSet_RuleSet')))
...
fn create_automattic_woocommerce_vendor_sabberworm_css_parser(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parser {
...
fn create_automattic_woocommerce_vendor_pelago_emogrifier_css_stylerule(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_StyleRule {
...
fn create_automattic_woocommerce_vendor_sabberworm_css_csslist_document(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_CSSList_Document {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_CssDocument) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'getStyleRulesData' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
		...
		'getFilteredAtIdentifierAndRule' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_CSSList_AtRuleBlockList](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Pelago_Emogrifier_Css_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
		...
		'isValidAtRuleToRender' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
