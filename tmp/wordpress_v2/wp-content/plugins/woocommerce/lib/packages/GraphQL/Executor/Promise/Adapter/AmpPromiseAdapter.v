...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter) convertthenable(var_thenable rt.PhpVal) rt.PhpVal {
	return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Promise', []string{}, create_automattic_woocommerce_vendor_graphql_executor_promise_promise(var_thenable.clone(), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter', ['PromiseAdapter'], &this)))
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter) then(mut var_promise Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Promise, mut var_onFulfilled Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_?callable, mut var_onRejected Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_?callable) rt.PhpVal {
	...
	closure_1_fn := fn [var_onFulfilled, var_onRejected, var_deferred] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_reason := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_value := if args.len > 1 { args[1].clone() } else { rt.new_null() }
		if rt.is_true(rt.identical(var_reason, rt.new_null())) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_onFulfilled, rt.new_null())))) {
			Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter.resolvewithcallable(mut rt.new_object('Amp_Deferred', []string{}, var_deferred), mut rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_?callable', []string{}, var_onFulfilled), var_value.clone())
		} else if rt.is_true(rt.identical(var_reason, rt.new_null())) {
			var_deferred.resolve(var_value.clone())
		} else if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_onRejected, rt.new_null())))) {
			Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter.resolvewithcallable(mut rt.new_object('Amp_Deferred', []string{}, var_deferred), mut rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_?callable', []string{}, var_onRejected), var_reason.clone())
		} else {
			var_deferred.fail(var_reason.clone())
		}
		return rt.new_null()
		}
	...
	rt.call_method(var_ampPromise, 'onResolve', [var_onResolve.clone()])
	return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Promise', []string{}, create_automattic_woocommerce_vendor_graphql_executor_promise_promise(var_deferred.promise(), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter', ['PromiseAdapter'], &this)))
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter) create(mut var_resolver Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_callable) rt.PhpVal {
	...
	closure_3_fn := fn [var_deferred] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		var_deferred.fail(var_exception.clone())
		return rt.new_null()
		}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter) createfulfilled(var_value rt.PhpVal) rt.PhpVal {
	mut var_promise := create_amp_success(var_value.clone())
	return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Promise', []string{}, create_automattic_woocommerce_vendor_graphql_executor_promise_promise(var_promise.clone(), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter', ['PromiseAdapter'], &this)))
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter) createrejected(mut var_reason Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_Throwable) rt.PhpVal {
	mut var_promise := create_amp_failure(var_reason)
	return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Promise', []string{}, create_automattic_woocommerce_vendor_graphql_executor_promise_promise(var_promise.clone(), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter', ['PromiseAdapter'], &this)))
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter) all(mut var_promisesOrValues Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_iterable) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.new_bool(rt.instance_of(var_item, 'Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Promise'))) {
			...
			rt.call_function('assert', [rt.new_bool(rt.instance_of(var_ampPromise, 'Amp_Promise'))])
			var_promises.array_set(var_key, var_ampPromise.clone())
		} else if rt.is_true(rt.new_bool(rt.instance_of(var_item, 'Amp_Promise'))) {
			var_promises.array_set(var_key, var_item.clone())
		}
	}
	mut var_deferred := create_amp_deferred()
	closure_4_fn := fn [var_promisesOrValues, var_deferred] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_reason := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_values := if args.len > 1 { args[1].clone() } else { rt.new_null() }
		if rt.is_true(rt.identical(var_reason, rt.new_null())) {
			rt.call_function('assert', [rt.new_bool(var_values.clone().is_array()), rt.new_string('Either $reason or $values must be passed')])
			...
			mut var_resolvedValues := rt.call_function('array_replace', [var_promisesOrValuesArray.clone(), var_values.clone()])
			var_deferred.resolve(var_resolvedValues.clone())
			return rt.new_null()
		}
	...
	rt.call_method(rt.call_function('all', [var_promises.clone()]), 'onResolve', [rt.new_closure(closure_4_fn)])
	return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Promise', []string{}, create_automattic_woocommerce_vendor_graphql_executor_promise_promise(var_deferred.promise(), rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter', ['PromiseAdapter'], &this)))
}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter.resolvewithcallable(mut var_deferred Class_Amp_Deferred, mut var_callback Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_callable, var_argument rt.PhpVal) {
	...
	mut var_result := rt.call_callable(var_callback, [var_argument.clone()])
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_Throwable') {
		...
		var_deferred_mutated.fail(var_exception.clone())
		return
		unsafe { goto end_label_1 }
	}
	else {
...
end_label_1:
	...
	var_deferred_mutated.resolve(var_result.clone())
}
...
fn create_automattic_woocommerce_vendor_graphql_executor_promise_adapter_amppromiseadapter(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Promise_Adapter_AmpPromiseAdapter {
...
fn create_amp_deferred(_args ...rt.PhpVal) &Class_Amp_Deferred {
...
fn create_amp_success(_args ...rt.PhpVal) &Class_Amp_Success {
...
fn create_amp_failure(_args ...rt.PhpVal) &Class_Amp_Failure {
