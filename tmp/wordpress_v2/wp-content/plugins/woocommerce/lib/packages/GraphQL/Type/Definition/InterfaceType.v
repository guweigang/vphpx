...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_InterfaceType.assertinterfacetype(var_type rt.PhpVal) rt.PhpVal {
		...
		'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_self'))))))
	{
		...
		mut var_notInterfaceType := iife_result_0
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_InvariantViolation',
			[]string{},
			create_automattic_woocommerce_vendor_graphql_error_invariantviolation(rt.new_string('Expected ${var_notInterfaceType.to_string()} to be a Automattic\\WooCommerce\\Vendor\\GraphQL Interface type.'))))
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_InterfaceType) assertvalid() {
	...
	mut iife_result_1 := iife_temp_1.assertvalidname(rt.get_property(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_InterfaceType', [
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_resolveType, rt.new_null()))))
		&& !(rt.call_function('is_callable', [var_resolveType.clone()])) {
		...
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Error_InvariantViolation',
			[]string{}, create_automattic_woocommerce_vendor_graphql_error_invariantviolation(rt.concat(rt.concat(rt.concat(rt.get_property(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_InterfaceType', [
			'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type',
			'AbstractType',
			'OutputType',
			'CompositeType',
