...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_SingleFieldSubscription) getvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) rt.PhpVal {
	closure_2_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		if rt.is_true(rt.identical(rt.get_property(var_node, 'operation'), rt.new_string('subscription'))) {
			...
			if var_selections.clone().array_count() > 1 {
				mut var_offendingSelections := rt.call_method(var_selections, 'splice', [rt.new_int(1), rt.new_int(var_selections.clone().array_count())])
				var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_SingleFieldSubscription.multiplefieldsinoperation(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_?string](if !(rt.get_property(rt.get_property(var_node, 'name'), 'value')).is_null() { rt.get_property(rt.get_property(var_node, 'name'), 'value') } else { rt.new_null() })), var_offendingSelections.clone()))
			}
		}
	...
	return rt.create_array([rt.ArrayItem{ key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.operation_definition(), val: rt.new_closure(closure_2_fn) }])
}
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_singlefieldsubscription(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_SingleFieldSubscription {
