...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_UniqueTypeNames) getsdlvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_SDLValidationContext) rt.PhpVal {
	...
	closure_2_fn := fn [var_context, var_schema, mut var_knownTypeNames] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_schema, rt.new_null()))))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_schema, 'getType', [var_typeName.clone()]), rt.new_null())))) {
			var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(rt.new_string("Type \"${var_typeName.to_string()}\" already exists in the schema. It cannot also be defined in this type definition."), rt.get_property(var_node,
				'name')))
			return rt.new_null()
		}
		if rt.is_true(rt.new_bool(var_knownTypeNames.clone().array_isset(var_typeName.clone()))) {
			var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(rt.new_string("There can be only one type named \"${var_typeName.to_string()}\"."), rt.create_array([
				rt.ArrayItem{ key: none, val: var_knownTypeNames.array_get(var_typeName) },
	...
	mut var_checkTypeName := rt.new_closure(closure_2_fn)
	return rt.create_array([
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.scalar_type_definition()
		...
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.union_type_definition()
			val: var_checkTypeName
		},
		rt.ArrayItem{
		...
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.input_object_type_definition()
			val: var_checkTypeName
		},
	])
}
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_uniquetypenames(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_UniqueTypeNames {
