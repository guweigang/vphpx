...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_UnresolvedFieldDefinition) construct(name string, mut var_definitionResolver Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_callable) {
	this.name = name
	this.definitionResolver = var_definitionResolver
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_UnresolvedFieldDefinition) resolve() rt.PhpVal {
	...
	{
		return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_FieldDefinition',
			[]string{}, create_automattic_woocommerce_vendor_graphql_type_definition_fielddefinition(rt.create_array([
			rt.ArrayItem{ key: 'name', val: this.name },
			rt.ArrayItem{ key: 'type', val: var_field },
		])))
	}
	return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_FieldDefinition',
		[]string{}, create_automattic_woocommerce_vendor_graphql_type_definition_fielddefinition(rt.add(var_field, rt.create_array([
		rt.ArrayItem{ key: 'name', val: this.name },
	]))))
}
...
fn create_automattic_woocommerce_vendor_graphql_type_definition_fielddefinition(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_FieldDefinition {
