...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_LoneSchemaDefinition) getsdlvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_SDLValidationContext) rt.PhpVal {
	...
	mut var_alreadyDefined := rt.new_bool(if rt.is_true(rt.identical(var_oldSchema, rt.new_null())) {
		false
	} else {
			rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(var_oldSchema, 'astNode'), rt.new_null()))))
			|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_oldSchema, 'getQueryType', []rt.PhpVal{}), rt.new_null()))))
			|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_oldSchema, 'getMutationType', []rt.PhpVal{}), rt.new_null()))))
			|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_oldSchema, 'getSubscriptionType', []rt.PhpVal{}), rt.new_null()))))
	})
	mut var_schemaDefinitionsCount := rt.new_int(0)
	closure_1_fn := fn [var_alreadyDefined, var_context, mut var_schemaDefinitionsCount] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		if rt.is_true(var_alreadyDefined) {
			var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_LoneSchemaDefinition.cannotdefineschemawithinextensionmessage(),
				var_node.clone()))
			return rt.new_null()
		}
		if rt.is_true(rt.greater(var_schemaDefinitionsCount, rt.new_int(0))) {
			var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_LoneSchemaDefinition.schemadefinitionnotalonemessage(),
				var_node.clone()))
		}
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_loneschemadefinition(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_LoneSchemaDefinition {
