...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) reporterror(mut var_error Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Error) {
	this.errors.array_push(var_error)
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) getrecursivevariableusages(mut var_operation Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_OperationDefinitionNode) rt.PhpVal {
	mut var_usages := if !(this.recursiveVariableUsages.array_get(var_operation)).is_null() {
		this.recursiveVariableUsages.array_get(var_operation)
	} else {
		rt.new_null()
	}
	if rt.is_true(rt.identical(var_usages, rt.new_null())) {
		...
		var_usages = rt.call_function('array_merge', [var_allUsages.clone()])
		this.recursiveVariableUsages.array_set(var_operation, var_usages.clone())
	}
	return var_usages.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) getvariableusages(mut var_node Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_HasSelectionSet) rt.PhpVal {
	...
	if !(this.variableUsages.array_isset(var_node_mutated)) {
		...
		closure_3_fn := fn [mut var_usages, var_typeInfo] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_variable := if args.len > 0 { args[0].clone() } else { rt.new_null() }
			var_usages.array_push(rt.create_array([
				rt.ArrayItem{ key: 'node', val: var_variable },
				rt.ArrayItem{ key: 'type', val: var_typeInfo.getinputtype() },
				rt.ArrayItem{ key: 'defaultValue', val: var_typeInfo.getdefaultvalue() },
			]))
			return rt.new_null()
		}
		mut iife_temp_3 := Class_Automattic_WooCommerce_Vendor_GraphQL_Language_Visitor{}
		mut iife_result_3 := iife_temp_3.visitwithtypeinfo(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Utils_TypeInfo',
			[]string{}, var_typeInfo), rt.create_array([
			rt.ArrayItem{
				key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.variable_definition()
				val: rt.new_closure(closure_2_fn)
			},
			rt.ArrayItem{
				key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.variable()
				val: rt.new_closure(closure_3_fn)
			},
		]))
		mut iife_temp_4 := Class_Automattic_WooCommerce_Vendor_GraphQL_Language_Visitor{}
		mut iife_result_4 := iife_temp_4.visit(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Language_AST_HasSelectionSet',
			[]string{}, var_node_mutated), iife_result_3)
		return this.variableUsages.array_set(var_node_mutated, var_usages.clone())
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) getrecursivelyreferencedfragments(mut var_operation Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_OperationDefinitionNode) rt.PhpVal {
	mut var_fragments := if !(this.recursivelyReferencedFragments.array_get(var_operation)).is_null() {
		this.recursivelyReferencedFragments.array_get(var_operation)
	} else {
		rt.new_null()
	}
	if rt.is_true(rt.identical(var_fragments, rt.new_null())) {
		...
		mut var_nodesToVisit := rt.create_array([
			rt.ArrayItem{ key: none, val: var_operation },
		])
		for rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_nodesToVisit, rt.new_array())))) {
			mut var_node := rt.call_function('array_pop', [var_nodesToVisit.clone()])
			mut var_spreads :=
				this.getfragmentspreads(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_HasSelectionSet](var_node))
			mut iter_2 := var_spreads.iterator()
			for {
				item_2 := iter_2.next() or { break }
				mut var_spread := item_2.val
				mut var_fragName := rt.get_property(rt.get_property(var_spread, 'name'), 'value')
				if rt.is_true(if !(var_collectedNames.array_get(var_fragName)).is_null() {
					var_collectedNames.array_get(var_fragName)
				} else {
					rt.new_bool(false)
				})
				{
				...
				var_fragments.array_push(var_fragment.clone())
				var_nodesToVisit.array_push(var_fragment.clone())
			}
		}
		this.recursivelyReferencedFragments.array_set(var_operation, var_fragments.clone())
	}
	return var_fragments.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) getfragmentspreads(mut var_node Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_HasSelectionSet) rt.PhpVal {
	mut var_node_mutated := var_node
	mut var_spreads := if !(this.fragmentSpreads.array_get(var_node_mutated)).is_null() {
		this.fragmentSpreads.array_get(var_node_mutated)
	} else {
		rt.new_null()
	}
	if rt.is_true(rt.identical(var_spreads, rt.new_null())) {
		var_spreads = rt.new_array()
		mut var_setsToVisit := rt.create_array([
			rt.ArrayItem{ key: none, val: rt.call_method(var_node_mutated, 'getSelectionSet',
				[]rt.PhpVal{}) },
		])
		for rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_setsToVisit, rt.new_array())))) {
			mut var_set := rt.call_function('array_pop', [var_setsToVisit.clone()])
			mut iter_3 := rt.get_property(var_set, 'selections').iterator()
			for {
				...
				{
					var_spreads.array_push(var_selection.clone())
				} else {
					rt.call_function('assert', [
						rt.new_bool(
							rt.is_true(rt.new_bool(rt.instance_of(var_selection, 'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_FieldNode')))
							|| rt.is_true(rt.new_bool(rt.instance_of(var_selection, 'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_InlineFragmentNode')))),
					])
					mut var_selectionSet := rt.get_property(var_selection, 'selectionSet')
					if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_selectionSet,
						rt.new_null()))))
					{
						var_setsToVisit.array_push(var_selectionSet.clone())
					}
				}
			}
		}
		this.fragmentSpreads.array_set(var_node_mutated, var_spreads.clone())
	}
	return var_spreads.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) getfragment(name string) rt.PhpVal {
	if !(!(this.fragments).is_null()) {
		...
		mut iter_4 := rt.get_property(this.getdocument(), 'definitions').iterator()
		for {
			item_4 := iter_4.next() or { break }
			mut var_statement := item_4.val
			if rt.is_true(rt.new_bool(rt.instance_of(var_statement,
				'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_FragmentDefinitionNode')))
			{
				var_fragments.array_set(rt.get_property(rt.get_property(var_statement, 'name'),
					'value'), var_statement.clone())
			}
		}
		this.fragments = var_fragments.clone()
	}
	return if !(this.fragments.array_get(rt.new_string(name))).is_null() {
		this.fragments.array_get(rt.new_string(name))
	} else {
		rt.new_null()
	}
}
...
fn create_automattic_woocommerce_vendor_graphql_validator_splobjectstorage(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_SplObjectStorage {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_DocumentNode](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_TypeInfo](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
		...
		'getRecursiveVariableUsages' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_OperationDefinitionNode](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
		...
		'getVariableUsages' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_HasSelectionSet](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'fragmentSpreads' {
			this.fragmentSpreads = val
			return true
		}
		'recursivelyReferencedFragments' {
			this.recursivelyReferencedFragments = val
			return true
		}
		'variableUsages' {
			this.variableUsages = val
			return true
		}
		'recursiveVariableUsages' {
			this.recursiveVariableUsages = val
			return true
		}
		else {
