...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.assign(mut var_obj Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_object, mut var_vars Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_array) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('property_exists', [var_obj_mutated, var_key.clone()]))))) {
		mut var_cls := rt.call_function('get_class', [var_obj_mutated])
		mut iife_temp_0 := Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Warning{}
		mut iife_result_0 := iife_temp_0.warn(rt.new_string("Trying to set non-existing property '${var_key.to_string()}' on class '${var_cls.to_string()}'"), Class_Automattic_WooCommerce_Vendor_GraphQL_Error_Warning.warning_assign())
		}
		rt.set_property(var_obj_mutated, '{"nodeType":"Expr_Variable","line":30,"name":"key"}', var_value.clone())
	}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.printsafejson(var_value rt.PhpVal) string {
	if rt.is_true(rt.new_bool(rt.instance_of(var_value, 'Automattic_WooCommerce_Vendor_GraphQL_Utils_stdClass'))) {
		return (Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.jsonencodeorserialize(var_value.clone())).str()
	}
	return (Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.printsafeinternal(var_value.clone())).str()
}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.printsafe(var_value rt.PhpVal) string {
	if rt.is_true(rt.new_bool(var_value.clone().is_object())) {
		if rt.is_true(rt.call_function('method_exists', [var_value.clone(), rt.new_string('__toString')])) {
			return (rt.call_method(var_value, '__toString', []rt.PhpVal{})).str()
		}
		return 'instance of ' + (rt.call_function('get_class', [var_value.clone()])).str()
	}
	return (Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.printsafeinternal(var_value.clone())).str()
}
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Vendor_GraphQL_Utils_JsonException') {
		mut var_jsonException := var_e_1.clone()
		return (rt.call_function('serialize', [var_value.clone()])).str()
		unsafe { goto end_label_1 }
	}
	else {
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.printsafeinternal(var_value rt.PhpVal) string {
	...
	return (rt.call_function('gettype', [var_value.clone()])).str()
}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.ord(char string, encoding string) i64 {
	...
	if !(rt.new_string(char_mutated).array_isset(rt.new_int(1))) {
		return (rt.call_function('ord', [rt.new_string(char_mutated).clone()])).to_i64()
	}
	if rt.is_true(rt.new_bool(encoding != 'UCS-4BE')) {
		char_mutated = (rt.call_function('mb_convert_encoding', [rt.new_string(char_mutated).clone(), rt.new_string('UCS-4BE'), rt.new_string(encoding)])).str()
		rt.call_function('assert', [rt.new_bool(rt.new_string(char_mutated).clone().is_string()), rt.new_string('format string is statically known to be correct')])
	}
	mut var_unpacked := rt.call_function('unpack', [rt.new_string('N'), rt.new_string(char_mutated).clone()])
	rt.call_function('assert', [rt.new_bool(var_unpacked.clone().is_array()), rt.new_string('format string is statically known to be correct')])
	return (var_unpacked.array_get(rt.new_int(1))).to_i64()
}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.isvalidnameerror(name string, mut var_node Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_?Node) rt.PhpVal {
	if rt.new_string(name).array_isset(rt.new_int(1)) && rt.is_true(rt.identical(rt.new_string(name).array_get(rt.new_int(0)), rt.new_string('_'))) && rt.is_true(rt.identical(rt.new_string(name).array_get(rt.new_int(1)), rt.new_string('_'))) {
		return create_automattic_woocommerce_vendor_graphql_error_error(rt.new_string("Name \"${var_name}\" must not begin with \"__\", which is reserved by Automattic\\WooCommerce\\Vendor\\GraphQL introspection."), var_node)
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('preg_match', [rt.new_string('/^[_a-zA-Z][_a-zA-Z0-9]*$/'), rt.new_string(name)]), rt.new_int(1))))) {
		return create_automattic_woocommerce_vendor_graphql_error_error(rt.concat(rt.concat(rt.new_string('Names must match /^[_a-zA-Z][_a-zA-Z0-9]*$/ but "'), rt.new_string(name)), rt.new_string('" does not.')), var_node)
	}
	return rt.new_null()
}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.quotedorlist(mut var_items Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_array) string {
	...
	mut var_quoted := rt.call_function('array_map', [rt.new_closure(closure_2_fn), var_items])
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.orlist(mut var_items Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_array) string {
	...
	mut var_selectedLength := rt.new_int(var_selected.clone().array_count())
	mut var_firstSelected := var_selected.array_get(rt.new_int(0))
	if rt.is_true(rt.identical(var_selectedLength, rt.new_int(1))) {
		return (var_firstSelected).str()
	}
	closure_4_fn := fn [var_selected] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
	...
	return (rt.call_function('array_reduce', [rt.call_function('range', [rt.new_int(1), rt.sub(var_selectedLength, rt.new_int(1))]), rt.new_closure(closure_4_fn), var_firstSelected.clone()])).str()
}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.suggestionlist(input string, mut var_options Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_array) rt.PhpVal {
	...
	mut var_lexicalDistance := create_automattic_woocommerce_vendor_graphql_utils_lexicaldistance(rt.new_string(input))
	...
	for {
		...
		mut var_distance := var_lexicalDistance.measure(var_option.clone(), var_threshold.clone())
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_distance, rt.new_null())))) {
			var_optionsByDistance.array_set(var_option, var_distance.clone())
		}
	}
	closure_5_fn := fn [var_optionsByDistance] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		return if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_distanceDiff, rt.new_int(0))))) { var_distanceDiff } else { rt.call_function('strnatcmp', [var_a.clone(), var_b.clone()]) }
		}
	rt.call_function('uksort', [var_optionsByDistance.clone(), rt.new_closure(closure_5_fn)])
	return rt.call_function('array_map', [rt.new_string('strval'), rt.func_array_keys(var_optionsByDistance.clone())])
}

fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.extractkey(var_objectLikeValue rt.PhpVal, key string) rt.PhpVal {
	if var_objectLikeValue.clone().is_array() || rt.is_true(rt.new_bool(rt.instance_of(var_objectLikeValue, 'Automattic_WooCommerce_Vendor_GraphQL_Utils_ArrayAccess'))) {
		return if !(var_objectLikeValue.array_get(rt.new_string(key))).is_null() { var_objectLikeValue.array_get(rt.new_string(key)) } else { rt.new_null() }
	}
	if rt.is_true(rt.new_bool(var_objectLikeValue.clone().is_object())) {
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_Utils.splitlines(value string) rt.PhpVal {
	...
	rt.call_function('assert', [rt.new_bool(var_lines.clone().is_array()), rt.new_string('given the regex is valid')])
...
fn create_automattic_woocommerce_vendor_graphql_utils_lexicaldistance(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Utils_LexicalDistance {
