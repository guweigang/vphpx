...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Server_OperationParams.create(mut var_params Class_Automattic_WooCommerce_Vendor_GraphQL_Server_array, readonly bool) rt.PhpVal {
	...
	var_params_mutated = rt.call_function('array_change_key_case', [
		var_params_mutated,
		rt.get_constant('CASE_LOWER'),
	])
	rt.set_property(var_instance, 'originalInput', var_params_mutated)
	var_params_mutated = rt.add(var_params_mutated, rt.create_array([
		rt.ArrayItem{ key: 'query', val: rt.new_null() },
		rt.ArrayItem{ key: 'queryid', val: rt.new_null() },
		rt.ArrayItem{ key: 'documentid', val: rt.new_null() },
		rt.ArrayItem{ key: 'id', val: rt.new_null() },
		rt.ArrayItem{ key: 'operationname', val: rt.new_null() },
		rt.ArrayItem{ key: 'variables', val: rt.new_null() },
		rt.ArrayItem{ key: 'extensions', val: rt.new_null() },
	]))
	...
	rt.set_property(var_instance, 'query', var_params_mutated.array_get(rt.new_string('query')))
	rt.set_property(var_instance, 'queryId', if !(var_params_mutated.array_get(rt.new_string('queryid'))).is_null() {
		var_params_mutated.array_get(rt.new_string('queryid'))
	} else {
		if !(var_params_mutated.array_get(rt.new_string('documentid'))).is_null() {
			var_params_mutated.array_get(rt.new_string('documentid'))
		} else {
			var_params_mutated.array_get(rt.new_string('id'))
		}
	})
	rt.set_property(var_instance, 'operation',
		var_params_mutated.array_get(rt.new_string('operationname')))
	rt.set_property(var_instance, 'variables',
		Class_Automattic_WooCommerce_Vendor_GraphQL_Server_OperationParams.decodeifjson(var_params_mutated.array_get(rt.new_string('variables'))))
	rt.set_property(var_instance, 'extensions',
		Class_Automattic_WooCommerce_Vendor_GraphQL_Server_OperationParams.decodeifjson(var_params_mutated.array_get(rt.new_string('extensions'))))
	rt.set_property(var_instance, 'readOnly', rt.new_bool(readonly))
	if rt.get_property(var_instance, 'extensions').array_get(rt.new_string('persistedQuery')).array_isset(rt.new_string('sha256Hash'))
		&& rt.is_true(rt.identical(rt.get_property(var_instance, 'queryId'), rt.new_null())) {
		rt.set_property(var_instance, 'queryId',
			rt.get_property(var_instance, 'extensions').array_get(rt.new_string('persistedQuery')).array_get(rt.new_string('sha256Hash')))
	}
...
fn Class_Automattic_WooCommerce_Vendor_GraphQL_Server_OperationParams.decodeifjson(var_value rt.PhpVal) rt.PhpVal {
	...
	mut var_decoded := rt.call_function('json_decode', [var_value_mutated.clone(),
		rt.new_bool(true)])
