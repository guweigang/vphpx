...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ExecutableDefinitions) getvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) rt.PhpVal {
	closure_2_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		for {
			...
			{
				if rt.is_true(rt.new_bool(rt.instance_of(var_definition, 'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_SchemaDefinitionNode')))
					...
					mut var_defName := rt.new_string('schema')
				} else {
					...
					var_defName = rt.new_string((rt.concat(rt.concat(rt.new_string('"'), rt.get_property(rt.call_method(var_definition,
						'getName', []rt.PhpVal{}), 'value')), rt.new_string('"'))).str())
				}
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_executabledefinitions(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ExecutableDefinitions {
