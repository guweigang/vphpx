...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_KnownDirectives) getastvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_ValidationContext) rt.PhpVal {
	...
	mut var_definedDirectives := if rt.is_true(rt.identical(var_schema, rt.new_null())) { iife_result_0 } else { rt.call_method(var_schema, 'getDirectives', []rt.PhpVal{}) }
	...
	mut iter_2 := var_astDefinition.iterator()
	for {
		...
		if rt.is_true(rt.new_bool(rt.instance_of(var_def, 'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_DirectiveDefinitionNode'))) {
			...
			mut iter_3 := rt.get_property(var_def, 'locations').iterator()
			for {
				item_3 := iter_3.next() or { break }
				mut var_location := item_3.val
				var_locationNames.array_push(rt.get_property(var_location, 'value'))
			}
			var_locationsMap.array_set(rt.get_property(rt.get_property(var_def, 'name'), 'value'), var_locationNames.clone())
		}
	}
	closure_2_fn := fn [var_context, var_locationsMap] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_path := if args.len > 3 { args[3].clone() } else { rt.new_null() }
		...
		if rt.is_true(rt.identical(var_candidateLocation, rt.new_string(''))) || rt.is_true(rt.call_function('in_array', [var_candidateLocation.clone(), var_locations.clone(), rt.new_bool(true)])) {
			return rt.new_null()
		}
	...
	return rt.create_array([rt.ArrayItem{ key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.directive(), val: rt.new_closure(closure_2_fn) }])
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_KnownDirectives) getdirectivelocationforastpath(mut var_ancestors Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_array) string {
	mut var_appliedTo := var_ancestors.array_get(rt.new_int(var_ancestors.array_count() - 1))
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_bool(rt.instance_of(var_appliedTo, 'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_OperationDefinitionNode')))) {
		...
		mut var_parentNode := var_ancestors.array_get(rt.new_int(var_ancestors.array_count() - 3))
		...
		mut var_unknownLocation := rt.call_function('get_class', [var_appliedTo.clone()])
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_knowndirectives(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_KnownDirectives {
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_exception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_Exception {
