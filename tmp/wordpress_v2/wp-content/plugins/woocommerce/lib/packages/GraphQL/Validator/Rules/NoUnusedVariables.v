...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoUnusedVariables) getvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) rt.PhpVal {
	...
	closure_2_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_opName := if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(var_operation, 'name'), rt.new_null())))) { rt.get_property(rt.get_property(var_operation, 'name'), 'value') } else { rt.new_null() }
		...
		mut iter_2 := this.variableDefs.iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_variableDef := item_2.val
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_nounusedvariables(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_NoUnusedVariables {
