...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_UniqueOperationTypes) getsdlvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_SDLValidationContext) rt.PhpVal {
	...
	mut var_existingOperationTypes := if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_schema, rt.new_null())))) { rt.create_array([
			rt.ArrayItem{
				key: 'query'
				val: rt.call_method(var_schema, 'getQueryType', []rt.PhpVal{})
			},
			rt.ArrayItem{
				key: 'mutation'
				val: rt.call_method(var_schema, 'getMutationType', []rt.PhpVal{})
			},
			rt.ArrayItem{
				key: 'subscription'
				val: rt.call_method(var_schema, 'getSubscriptionType', []rt.PhpVal{})
			},
		]) } else { rt.new_array() }
	closure_2_fn := fn [var_context, mut var_definedOperationTypes, var_existingOperationTypes] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		for {
			...
			mut var_alreadyDefinedOperationType := if !(var_definedOperationTypes.array_get(var_operation)).is_null() {
				var_definedOperationTypes.array_get(var_operation)
			} else {
				rt.new_null()
			}
			if var_existingOperationTypes.array_isset(var_operation) {
				var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(rt.new_string('Type for ${var_operation.to_string()} already defined in the schema. It cannot be redefined.'),
					var_operationType.clone()))
			} else if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_alreadyDefinedOperationType,
				rt.new_null()))))
			{
				var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(rt.new_string('There can be only one ${var_operation.to_string()} type in schema.'), rt.create_array([
					rt.ArrayItem{ key: none, val: var_alreadyDefinedOperationType },
					rt.ArrayItem{ key: none, val: var_operationType },
				])))
			} else {
				var_definedOperationTypes.array_set(var_operation, var_operationType.clone())
			}
		}
	...
	mut var_checkOperationTypes := rt.new_closure(closure_2_fn)
	return rt.create_array([
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.schema_definition()
			val: var_checkOperationTypes
		},
		rt.ArrayItem{
			key: Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_NodeKind.schema_extension()
			val: var_checkOperationTypes
		},
	])
}
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_uniqueoperationtypes(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_UniqueOperationTypes {
