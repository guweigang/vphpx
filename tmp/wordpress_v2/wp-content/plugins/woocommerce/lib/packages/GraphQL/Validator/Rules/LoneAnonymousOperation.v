...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_LoneAnonymousOperation) getvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) rt.PhpVal {
	mut var_operationCount := rt.new_int(0)
	closure_1_fn := fn [mut var_operationCount] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		var_operationCount = rt.new_int(0)
		mut iter_1 := rt.get_property(var_node, 'definitions').iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_definition := item_1.val
			if rt.is_true(rt.new_bool(rt.instance_of(var_definition,
				'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_OperationDefinitionNode')))
			{
	...
	closure_2_fn := fn [mut var_operationCount, var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_node := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(var_node, 'name'), rt.new_null()))))
			|| rt.is_true(rt.less_equal(var_operationCount, rt.new_int(1))) {
			return rt.new_null()
		}
		var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_LoneAnonymousOperation.anonoperationnotalonemessage(), rt.create_array([
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_loneanonymousoperation(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_LoneAnonymousOperation {
