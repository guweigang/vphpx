...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) construct(mut var_parentType Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ObjectType, mut var_schema Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Schema, mut var_fieldNodes Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_iterable, mut var_variableValues Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array, mut var_fragments Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array, mut var_options Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array) {
	...
	this.groupImplementorFields = if !(var_options.array_get(rt.new_string('groupImplementorFields'))).is_null() {
		var_options.array_get(rt.new_string('groupImplementorFields'))
	} else {
		rt.new_bool(false)
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) getreferencedfields() rt.PhpVal {
	...
	return rt.func_array_keys(var_allFields.clone())
}

fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) hasfield(field string) bool {
	mut iter_3 := this.typeToFields.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_fields := item_3.val
		if rt.is_true(rt.new_bool(var_fields.clone().array_isset(rt.new_string(field)))) {
			return true
		}
	}
	return false
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) subfields(typename string) rt.PhpVal {
	return rt.func_array_keys(if !(this.typeToFields.array_get(rt.new_string(typename))).is_null() {
		this.typeToFields.array_get(rt.new_string(typename))
	} else {
		rt.new_array()
	})
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) analyzequeryplan(mut var_parentType Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ObjectType, mut var_fieldNodes Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_iterable) {
	...
	mut iter_4 := var_fieldNodes.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_fieldNode := item_4.val
		...
		mut iife_result_0 := iife_temp_0.getnamedtype(rt.call_method(var_parentType.getfield(rt.get_property(rt.get_property(var_fieldNode,
			'name'), 'value')), 'getType', []rt.PhpVal{}))
		mut var_type := iife_result_0
		mut var_subfields := this.analyzeselectionset(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_SelectionSetNode](rt.get_property(var_fieldNode,
			'selectionSet')), mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type](var_type), mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](var_implementors))
		var_queryPlan = this.arraymergedeep(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](var_queryPlan), mut
			rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](var_subfields))
	}
	if rt.is_true(this.groupImplementorFields) {
		this.queryPlan = rt.create_array([
			rt.ArrayItem{ key: 'fields', val: var_queryPlan },
		])
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_implementors, rt.new_array())))) {
			this.queryPlan.array_set('implementors', var_implementors.clone())
		}
	} else {
		this.queryPlan = var_queryPlan.clone()
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) analyzeselectionset(mut var_selectionSet Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_SelectionSetNode, mut var_parentType Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type, mut var_implementors Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array) rt.PhpVal {
	...
	mut iter_5 := rt.get_property(var_selectionSet, 'selections').iterator()
	for {
		item_5 := iter_5.next() or { break }
		mut var_selection := item_5.val
			...
			'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_FieldNode')))
		{
			...
			rt.call_function('assert', [
				rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type',
					[]string{}, var_parentType),
					'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_HasFieldsType')),
				rt.new_string('ensured by query validation'),
			])
			mut var_type := var_parentType.getfield(var_fieldName.clone())
			...
			mut var_subfields := if rt.is_true(rt.identical(var_nestedSelectionSet, rt.new_null())) {
				rt.new_array()
			} else {
				this.analyzesubfields(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type](var_selectionType), mut
					rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_SelectionSetNode](var_nestedSelectionSet), mut
					rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](var_subImplementors))
			}
			mut iife_temp_1 := Class_Automattic_WooCommerce_Vendor_GraphQL_Executor_Values{}
			mut iife_result_1 := iife_temp_1.getargumentvalues(var_type.clone(),
				var_selection.clone(), this.variableValues)
			var_fields.array_set(var_fieldName, rt.create_array([
				rt.ArrayItem{ key: 'type', val: var_selectionType },
				rt.ArrayItem{ key: 'fields', val: var_subfields },
				rt.ArrayItem{ key: 'args', val: iife_result_1 },
			]))
			if rt.is_true(this.groupImplementorFields)
				&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_subImplementors, rt.new_array())))) {
				var_fields.array_get_mut(var_fieldName).array_set('implementors',
					var_subImplementors.clone())
			}
			...
			'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_FragmentSpreadNode')))
		{
			...
			var_type = rt.call_method(this.schema, 'getType', [
			...
			var_subfields = this.analyzesubfields(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type](var_type), mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_SelectionSetNode](rt.get_property(var_fragment,
				'selectionSet')), rt.new_null())
			var_fields = this.mergefields(mut var_parentType, mut
				rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type](var_type), mut
				rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](var_fields), mut
				rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](var_subfields), mut
				var_implementors_mutated)
		} else if rt.is_true(rt.new_bool(rt.instance_of(var_selection,
			'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_InlineFragmentNode')))
		{
			...
			var_type = if rt.is_true(rt.identical(var_typeCondition, rt.new_null())) { var_parentType } else { rt.call_method(this.schema, 'getType', [
			...
			var_subfields = this.analyzesubfields(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type](var_type), mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_SelectionSetNode](rt.get_property(var_selection,
				'selectionSet')), rt.new_null())
			var_fields = this.mergefields(mut var_parentType, mut
				rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type](var_type), mut
				rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](var_fields), mut
	...
	for {
		...
		mut var__ := item_6.val
		mut var_fieldName := item_6.key
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) analyzesubfields(mut var_type Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type, mut var_selectionSet Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_SelectionSetNode, mut var_implementors Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array) rt.PhpVal {
	...
	mut iife_result_2 := iife_temp_2.getnamedtype(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type',
		[]string{}, var_type_mutated))
	var_type_mutated = iife_result_2
	return if
		rt.is_true(rt.new_bool(rt.instance_of(var_type_mutated, 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_ObjectType')))
		|| rt.is_true(rt.new_bool(rt.instance_of(var_type_mutated, 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_AbstractType'))) {
		this.analyzeselectionset(mut var_selectionSet, mut var_type_mutated, mut
			var_implementors_mutated)
	} else {
		rt.new_array()
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) mergefields(mut var_parentType Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type, mut var_type Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type, mut var_fields Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array, mut var_subfields Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array, mut var_implementors Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array) rt.PhpVal {
	...
	mut var_implementors_mutated := var_implementors
	if rt.is_true(this.groupImplementorFields)
		&& rt.is_true(rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_Type', []string{}, var_parentType), 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_AbstractType')))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_type_mutated, 'Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_AbstractType')))))) {
		...
		var_implementors_mutated.array_set(var_name, rt.create_array([
			rt.ArrayItem{ key: 'type', val: var_type_mutated },
			rt.ArrayItem{ key: 'fields', val: this.arraymergedeep(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](if !(var_implementors_mutated.array_get(var_name).array_get(rt.new_string('fields'))).is_null() {
				var_implementors_mutated.array_get(var_name).array_get(rt.new_string('fields'))
			} else {
				rt.new_array()
			}), mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](rt.call_function('array_diff_key', [
				var_subfields_mutated,
				var_fields_mutated,
			]))) },
		]))
		var_fields_mutated = this.arraymergedeep(mut var_fields_mutated, mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](rt.call_function('array_intersect_key', [
			var_subfields_mutated,
			var_fields_mutated,
		])))
	} else {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) arraymergedeep(mut var_array1 Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array, mut var_array2 Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array) rt.PhpVal {
	...
	mut iter_7 := var_array2.iterator()
	for {
		...
		if rt.is_true(rt.new_bool(var_key.clone().is_long() || var_key.clone().is_double())) {
			...
			{
				var_array1_mutated.array_push(var_value.clone())
			}
		} else if var_value.clone().is_array() && var_array1_mutated.array_isset(var_key)
			&& var_array1_mutated.array_get(var_key).is_array() {
			var_array1_mutated.array_set(var_key, this.arraymergedeep(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](var_array1_mutated.array_get(var_key)), mut
				rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](var_value)))
		} else {
			var_array1_mutated.array_set(var_key, var_value.clone())
		}
	}
	return rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array', []string{},
		var_array1_mutated)
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Schema](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_iterable](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
			...
			mut dispatch_arg_4 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](if args.len > 4 {
				args[4]
			} else {
				rt.new_null()
			})
		...
		'analyzeQueryPlan' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_iterable](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
		...
		'analyzeSelectionSet' {
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
			return this.analyzeselectionset(mut dispatch_arg_0, mut dispatch_arg_1, mut
				dispatch_arg_2)
		}
		'analyzeSubFields' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_SelectionSetNode](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
		...
		'mergeFields' {
			...
			return this.mergefields(mut dispatch_arg_0, mut dispatch_arg_1, mut dispatch_arg_2, mut
		...
		'arrayMergeDeep' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Type_Definition_QueryPlan) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'groupImplementorFields' {
			this.groupImplementorFields = val
			return true
		}
		else {
