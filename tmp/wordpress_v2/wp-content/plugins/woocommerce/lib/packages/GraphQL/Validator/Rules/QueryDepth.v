...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryDepth) getvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) rt.PhpVal {
	closure_1_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_operationDefinition := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_maxDepth := rt.new_int(this.fielddepth(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_Node](var_operationDefinition),
			0, 0))
		if rt.is_true(rt.less_equal(var_maxDepth, this.maxQueryDepth)) {
			return rt.new_null()
		}
		var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryDepth.maxquerydeptherrormessage(this.maxQueryDepth,
			var_maxDepth.to_i64())))
		return rt.new_null()
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryDepth) fielddepth(mut var_node Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_Node, depth i64, maxDepth i64) i64 {
	...
	if rt.is_true(rt.new_bool(rt.instance_of(rt.get_property(var_node, 'selectionSet'),
		'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_SelectionSetNode')))
	{
		mut iter_1 :=
			rt.get_property(rt.get_property(var_node, 'selectionSet'), 'selections').iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_childNode := item_1.val
			maxDepth_mutated = this.nodedepth(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_Node](var_childNode),
				depth, maxDepth_mutated)
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryDepth) nodedepth(mut var_node Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_Node, depth i64, maxDepth i64) i64 {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Language_AST_Node',
		[]string{}, var_node), 'Automattic_WooCommerce_Vendor_GraphQL_Language_AST_FieldNode'))))
	{
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(var_node, 'selectionSet'),
			rt.new_null()))))
		{
	...
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_bool(rt.instance_of(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Language_AST_Node',
		[]string{}, var_node),
	...
	{
		mut var_fragment := this.getfragment(rt.new_object('Automattic_WooCommerce_Vendor_GraphQL_Language_AST_Node',
			[]string{}, var_node))
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_fragment, rt.new_null())))) {
			...
			maxDepth_mutated = this.fielddepth(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_GraphQL_Language_AST_Node](var_fragment),
				depth, maxDepth_mutated)
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryDepth) isenabled() bool {
	return rt.new_bool(!rt.is_true(rt.identical(this.maxQueryDepth,
		Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryDepth.disabled())))
}
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryDepth) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'maxQueryDepthErrorMessage' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).to_i64()
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).to_i64()
			return rt.new_string(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryDepth.maxquerydeptherrormessage(dispatch_arg_0,
				dispatch_arg_1))
		}
		'isEnabled' {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_QueryDepth) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'calculatedFragments' {
			this.calculatedFragments = val
			return true
		}
		'maxQueryDepth' {
			this.maxQueryDepth = val.to_i64()
			return true
		}
		else {
