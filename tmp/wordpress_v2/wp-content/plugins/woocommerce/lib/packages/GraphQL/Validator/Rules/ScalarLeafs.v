...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ScalarLeafs) getvisitor(mut var_context Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_QueryValidationContext) rt.PhpVal {
	closure_3_fn := fn [var_context] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut iife_result_2 := iife_temp_2.isleaftype(iife_result_1)
		if rt.is_true(iife_result_2) {
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(var_node,
				'selectionSet'), rt.new_null()))))
			{
				var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ScalarLeafs.nosubselectionallowedmessage((rt.get_property(rt.get_property(var_node,
					'name'), 'value')).str(),
			...
			var_context.reporterror(create_automattic_woocommerce_vendor_graphql_error_error(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ScalarLeafs.requiredsubselectionmessage((rt.get_property(rt.get_property(var_node,
				'name'), 'value')).str(),
				(rt.call_method(var_type, 'toString', []rt.PhpVal{})).str()), rt.create_array([
...
fn create_automattic_woocommerce_vendor_graphql_validator_rules_scalarleafs(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ScalarLeafs {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ScalarLeafs) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'noSubselectionAllowedMessage' {
			...
			return rt.new_string(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ScalarLeafs.nosubselectionallowedmessage(dispatch_arg_0,
				dispatch_arg_1))
		}
		'requiredSubselectionMessage' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).str()
			return rt.new_string(Class_Automattic_WooCommerce_Vendor_GraphQL_Validator_Rules_ScalarLeafs.requiredsubselectionmessage(dispatch_arg_0,
				dispatch_arg_1))
		}
		else {
			return none
