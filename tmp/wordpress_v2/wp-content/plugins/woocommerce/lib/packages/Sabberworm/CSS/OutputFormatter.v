...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_OutputFormatter) safely(var_cCode rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.call_method(this.oFormat, 'get', [rt.new_string('IgnoreExceptions')])) {
		...
		if rt.instance_of(var_e_1,
			'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_OutputException')
		{
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_OutputFormatter) implode(var_sSeparator rt.PhpVal, mut var_aValues Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_array, bIncreaseLevel bool) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.new_bool(rt.instance_of(var_mValue,
			'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Renderable')))
		{
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_OutputFormatter) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'comments' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Comment_Commentable](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
