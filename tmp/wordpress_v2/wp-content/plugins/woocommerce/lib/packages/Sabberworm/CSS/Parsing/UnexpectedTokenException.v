...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_UnexpectedTokenException) construct(var_sExpected rt.PhpVal, var_sFound rt.PhpVal, sMatchType string, iLineNo i64) {
	this.sExpected = var_sExpected.clone()
	this.sFound = var_sFound.clone()
	this.sMatchType = rt.new_string(sMatchType)
	mut var_sMessage :=
		rt.new_string('Token “${var_sExpected.to_string()}” (${var_sMatchType}) not found. Got “${var_sFound.to_string()}”.')
	if rt.is_true(rt.identical(this.sMatchType, rt.new_string('search'))) {
		var_sMessage =
			rt.new_string('Search for “${var_sExpected.to_string()}” returned no results. Context: “${var_sFound.to_string()}”.')
	} else if rt.is_true(rt.identical(this.sMatchType, rt.new_string('count'))) {
		var_sMessage =
			rt.new_string('Next token was expected to have ${var_sExpected.to_string()} chars. Context: “${var_sFound.to_string()}”.')
	} else if rt.is_true(rt.identical(this.sMatchType, rt.new_string('identifier'))) {
		var_sMessage = rt.new_string('Identifier expected. Got “${var_sFound.to_string()}”')
	} else if rt.is_true(rt.identical(this.sMatchType, rt.new_string('custom'))) {
		var_sMessage =
			rt.new_string('${var_sExpected.to_string()} ${var_sFound.to_string()}'.trim_space())
	}
	this.Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_SourceException.construct(var_sMessage.clone(),
		rt.new_int(iLineNo))
}
...
fn create_automattic_woocommerce_vendor_sabberworm_css_parsing_unexpectedtokenexception(sMatchType string, iLineNo i64, arg_2 rt.PhpVal, arg_3 rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_UnexpectedTokenException {
