...
fn Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_CalcFunction.parse(mut var_oParserState Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_ParserState, bIgnoreCase bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.equal(var_oParserState.peek(), rt.new_string('('))))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_UnexpectedTokenException',
			[]string{}, create_automattic_woocommerce_vendor_sabberworm_css_parsing_unexpectedtokenexception(rt.new_string('('),
	...
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_UnexpectedTokenException',
			[]string{}, create_automattic_woocommerce_vendor_sabberworm_css_parsing_unexpectedtokenexception(rt.new_string('calc'),
			var_sFunction.clone(), rt.new_string('literal'), var_oParserState.currentline())))
	}
	var_oParserState.consume(rt.new_string('('))
	mut var_oCalcList :=
		create_automattic_woocommerce_vendor_sabberworm_css_value_calcrulevaluelist(var_oParserState.currentline())
	mut var_oList := create_automattic_woocommerce_vendor_sabberworm_css_value_rulevaluelist(rt.new_string(','),
	...
	for rt.is_true(rt.new_bool(!(rt.is_true(var_oParserState.comes(rt.new_string(')'))))))
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.equal(var_iLastComponentType,
			Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_CalcFunction.t_operand()))))
		{
			mut iife_temp_0 := Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_Value{}
			mut iife_result_0 := iife_temp_0.parseprimitivevalue(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_ParserState',
				...
				var_aOperators.clone()]))
			{
				if rt.is_true(var_oParserState.comes(rt.new_string('-')))
					...
					if rt.is_true(rt.new_bool(!rt.is_true(rt.equal(var_oParserState.peek(rt.new_int(1), rt.new_int(-1)), rt.new_string(' ')))))
						...
						|| rt.is_true(var_oParserState.comes(rt.new_string('+ ')))) {
						rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_UnexpectedTokenException',
							[]string{}, create_automattic_woocommerce_vendor_sabberworm_css_parsing_unexpectedtokenexception(rt.concat(rt.concat(rt.new_string(' '),
				...
				var_oCalcList.addlistcomponent(var_oParserState.consume(rt.new_int(1)))
				var_iLastComponentType =
					Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_CalcFunction.t_operator()
			} else {
				rt.throw_exception(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Parsing_UnexpectedTokenException',
					[]string{}, create_automattic_woocommerce_vendor_sabberworm_css_parsing_unexpectedtokenexception(rt.call_function('sprintf', [
	...
	var_oList.addlistcomponent(rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_CalcRuleValueList',
		[]string{}, var_oCalcList))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_oParserState.isend())))) {
		var_oParserState.consume(rt.new_string(')'))
	}
	return rt.new_object('Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_CalcFunction', [
		'Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_CSSFunction',
	], create_automattic_woocommerce_vendor_sabberworm_css_value_calcfunction(var_sFunction.clone(),
...
fn create_automattic_woocommerce_vendor_sabberworm_css_value_calcrulevaluelist(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_CalcRuleValueList {
...
fn (mut this Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_CalcFunction) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'parse' {
			...
			return Class_Automattic_WooCommerce_Vendor_Sabberworm_CSS_Value_CalcFunction.parse(mut dispatch_arg_0,
				dispatch_arg_1)
		}
		else {
			return none
