...
pub fn Class_Automattic_WooCommerce_Blueprint_Importers_ImportRunSql.allowed_query_types() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: none, val: 'INSERT' },
		rt.ArrayItem{ key: none, val: 'UPDATE' }, rt.ArrayItem{ key: none, val: 'REPLACE INTO' }])
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportRunSql) process(var_schema rt.PhpVal) rt.PhpVal {
	...
	mut iife_temp_0 := Class_Automattic_WooCommerce_Blueprint_Steps_RunSql{}
	...
	mut var_sql :=
		rt.new_string(rt.get_property(rt.get_property(var_schema, 'sql'), 'contents').to_string().trim_space())
	if !(this.is_allowed_query_type(var_sql.str())) {
		rt.call_method(var_result, 'add_error', [
			rt.call_function('sprintf', [rt.new_string('Only %s queries are allowed.'),
				rt.call_function('implode', [rt.new_string(', '),
					Class_Automattic_WooCommerce_Blueprint_Importers_Automattic_WooCommerce_Blueprint_Importers_ImportRunSql.allowed_query_types()])]),
		])
		return var_result.clone()
	}
	if this.contains_suspicious_comments(var_sql.str()) {
		rt.call_method(var_result, 'add_error', [
			rt.new_string('SQL query contains suspicious comment patterns.'),
		])
		return var_result.clone()
	}
	if this.contains_sql_injection_patterns(var_sql.str()) {
		rt.call_method(var_result, 'add_error', [
			rt.new_string('SQL query contains potential injection patterns.'),
		])
		return var_result.clone()
	}
	if this.affects_protected_tables(var_sql.str()) {
		rt.call_method(var_result, 'add_error', [
			rt.new_string('Modifications to admin users or roles are not allowed.'),
		])
		return var_result.clone()
	}
	if this.affects_user_capabilities(var_sql.str()) {
		rt.call_method(var_result, 'add_error', [
			rt.new_string('Modifications to user roles or capabilities are not allowed.'),
		])
		return var_result.clone()
	}
	rt.call_method(var_wpdb, 'suppress_errors', [rt.new_bool(true)])
	rt.call_method(var_wpdb, 'query', [rt.new_string('START TRANSACTION')])
	mut var_query_result := rt.call_method(var_wpdb, 'query', [
		var_sql.clone()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
	...
	if rt.is_true(var_last_error) {
		...
		rt.call_method(var_result, 'add_error', [
			rt.new_string('Error executing SQL: ' + var_last_error.str()),
		])
		if rt.has_exception() {
			unsafe {
				goto catch_label_1
		...
		rt.call_method(var_result, 'add_debug', [
			rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('Executed SQL ('), rt.get_property(rt.get_property(var_schema,
				'sql'), 'name')), rt.new_string('): Affected ')), var_query_result),
				rt.new_string(' rows')),
		])
		if rt.has_exception() {
			unsafe {
	...
	if rt.instance_of(var_e_1, 'Automattic_WooCommerce_Blueprint_Importers_Throwable') {
		...
		rt.call_method(var_wpdb, 'query', [rt.new_string('ROLLBACK')])
		rt.call_method(var_result, 'add_error', [
			rt.concat(rt.new_string('Exception executing SQL: '), rt.call_method(var_e,
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportRunSql) is_allowed_query_type(sql_content string) bool {
	mut var_uppercase_sql_content := rt.new_string(sql_content.trim_space().to_upper())
	mut iter_1 :=
		Class_Automattic_WooCommerce_Blueprint_Importers_Automattic_WooCommerce_Blueprint_Importers_ImportRunSql.allowed_query_types().iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_query_type := item_1.val
		if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('stripos', [
			var_uppercase_sql_content.clone(),
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportRunSql) contains_suspicious_comments(sql_content string) bool {
	if rt.is_true(rt.identical(rt.call_function('strpos', [rt.new_string(sql_content), rt.new_string('--')]), rt.new_bool(false)))
		&& rt.is_true(rt.identical(rt.call_function('strpos', [rt.new_string(sql_content), rt.new_string('/*')]), rt.new_bool(false)))
		&& rt.is_true(rt.identical(rt.call_function('strpos', [rt.new_string(sql_content), rt.new_string('#')]), rt.new_bool(false))) {
		return false
	}
	mut var_dangerous_commands := rt.create_array([
		rt.ArrayItem{ key: none, val: 'DELETE' },
		rt.ArrayItem{ key: none, val: 'DROP' },
		rt.ArrayItem{ key: none, val: 'ALTER' },
		rt.ArrayItem{ key: none, val: 'CREATE' },
		rt.ArrayItem{ key: none, val: 'TRUNCATE' },
		rt.ArrayItem{ key: none, val: 'GRANT' },
		rt.ArrayItem{ key: none, val: 'REVOKE' },
		rt.ArrayItem{ key: none, val: 'EXEC' },
		rt.ArrayItem{ key: none, val: 'EXECUTE' },
		rt.ArrayItem{ key: none, val: 'CALL' },
		rt.ArrayItem{ key: none, val: 'INTO OUTFILE' },
		rt.ArrayItem{ key: none, val: 'INTO DUMPFILE' },
		rt.ArrayItem{ key: none, val: 'LOAD_FILE' },
		rt.ArrayItem{ key: none, val: 'LOAD DATA' },
		rt.ArrayItem{ key: none, val: 'BENCHMARK' },
		rt.ArrayItem{ key: none, val: 'SLEEP' },
		rt.ArrayItem{ key: none, val: 'INFORMATION_SCHEMA' },
		rt.ArrayItem{ key: none, val: 'USER\\(' },
		rt.ArrayItem{ key: none, val: 'DATABASE\\(' },
		rt.ArrayItem{ key: none, val: 'SCHEMA\\(' },
	])
	mut var_dangerous_pattern := rt.call_function('implode', [
		rt.new_string('|'), var_dangerous_commands.clone()])
	mut var_patterns := rt.create_array([
		rt.ArrayItem{ key: none, val: '/--.*?(' + var_dangerous_pattern.str() + ')/i' },
		rt.ArrayItem{ key: none, val: '/#.*?(' + var_dangerous_pattern.str() + ')/i' },
		rt.ArrayItem{ key: none, val: '/\\/\\*.*?(' + var_dangerous_pattern.str() + ').*?\\*\\//is' },
		rt.ArrayItem{ key: none, val: '/\\/\\*![0-9]*.*?\\*\\//' },
	])
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportRunSql) contains_sql_injection_patterns(sql_content string) bool {
	mut var_patterns := rt.create_array([
		rt.ArrayItem{ key: none, val: '/UNION\\s+(?:ALL\\s+)?SELECT/i' },
		rt.ArrayItem{ key: none, val: '/OR\\s+1\\s*=\\s*1/i' },
		rt.ArrayItem{ key: none, val: '/AND\\s+0\\s*=\\s*0/i' },
		rt.ArrayItem{ key: none, val: '/;\\s*--/i' },
		rt.ArrayItem{ key: none, val: '/SLEEP\\s*\\(/i' },
		rt.ArrayItem{ key: none, val: '/BENCHMARK\\s*\\(/i' },
		rt.ArrayItem{ key: none, val: '/LOAD_FILE\\s*\\(/i' },
		rt.ArrayItem{ key: none, val: '/INTO\\s+OUTFILE/i' },
		rt.ArrayItem{ key: none, val: '/INTO\\s+DUMPFILE/i' },
		rt.ArrayItem{ key: none, val: '/CREATE\\s+(?:TEMPORARY\\s+)?TABLE/i' },
		rt.ArrayItem{ key: none, val: '/DROP\\s+TABLE/i' },
		rt.ArrayItem{ key: none, val: '/ALTER\\s+TABLE/i' },
		rt.ArrayItem{ key: none, val: '/INFORMATION_SCHEMA/i' },
		rt.ArrayItem{ key: none, val: '/EXEC\\s*\\(/i' },
		rt.ArrayItem{ key: none, val: '/SCHEMA_NAME/i' },
		rt.ArrayItem{ key: none, val: '/DATABASE\\(\\)/i' },
		rt.ArrayItem{ key: none, val: '/CHR\\s*\\(/i' },
		rt.ArrayItem{ key: none, val: '/CHAR\\s*\\(/i' },
		rt.ArrayItem{ key: none, val: '/FROM\\s+mysql\\./i' },
		rt.ArrayItem{ key: none, val: '/FROM\\s+information_schema\\./i' },
	])
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportRunSql) affects_protected_tables(sql_content string) bool {
	mut var_wpdb := rt.new_null()
	mut var_protected_tables := rt.create_array([
		rt.ArrayItem{ key: none, val: rt.get_property(var_wpdb, 'users') },
		rt.ArrayItem{ key: none, val: rt.get_property(var_wpdb, 'usermeta') },
	])
	mut iter_4 := var_protected_tables.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_table := item_4.val
		if rt.is_true(rt.call_function('preg_match', [
			rt.new_string('/\\b' +
				(rt.call_function('preg_quote', [var_table.clone(), rt.new_string('/')])).str() +
				'\\b/i'),
			rt.new_string(sql_content),
		]))
		{
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportRunSql) affects_user_capabilities(sql_content string) bool {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('stripos', [
		rt.new_string(sql_content),
		rt.new_string((rt.get_property(var_wpdb, 'prefix')).str() + 'options'),
	]), rt.new_bool(false)))))
	{
		mut var_option_patterns := rt.create_array([
			rt.ArrayItem{ key: none, val: 'user_roles' },
			rt.ArrayItem{ key: none, val: 'capabilities' },
			rt.ArrayItem{ key: none, val: 'wp_user_' },
			rt.ArrayItem{ key: none, val: 'role_' },
			rt.ArrayItem{ key: none, val: 'administrator' },
		])
		mut iter_5 := var_option_patterns.iterator()
		for {
			item_5 := iter_5.next() or { break }
			mut var_pattern := item_5.val
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('stripos', [
				rt.new_string(sql_content),
				var_pattern.clone(),
			]), rt.new_bool(false)))))
