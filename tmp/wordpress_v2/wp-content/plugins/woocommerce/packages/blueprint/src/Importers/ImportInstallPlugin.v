...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportInstallPlugin) process(var_schema rt.PhpVal) rt.PhpVal {
	mut iife_temp_0 := Class_Automattic_WooCommerce_Blueprint_Steps_InstallPlugin{}
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('wordpress.org/plugins'), rt.get_property(var_plugin,
		'resource')))))
	{
		rt.call_method(var_result, 'add_info', [
			rt.new_string("Skipped installing a plugin. Unsupported resource type. Only 'wordpress.org/plugins' is supported at the moment."),
		])
		return var_result.clone()
	}
	if var_installed_plugins.array_isset(rt.get_property(var_plugin, 'slug')) {
		rt.call_method(var_result, 'add_info', [
			rt.concat(rt.concat(rt.new_string('Skipped installing '), rt.get_property(var_plugin,
				'slug')), rt.new_string('. It is already installed.')),
		])
		return var_result.clone()
	}
	if rt.is_true(rt.identical(rt.call_method(this.storage, 'is_supported_resource', [
		rt.get_property(var_plugin, 'resource'),
	]), rt.new_bool(false)))
	{
		rt.call_method(var_result, 'add_error', [
			rt.concat(rt.concat(rt.new_string('Invalid resource type for '), rt.get_property(var_plugin,
				'slug')), rt.new_string('.')),
		])
		return var_result.clone()
	}
	mut var_downloaded_path := rt.call_method(this.storage, 'download', [
		rt.get_property(var_plugin, 'slug'),
		rt.get_property(var_plugin, 'resource'),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_downloaded_path)))) {
		rt.call_method(var_result, 'add_error', [
			rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('Unable to download '), rt.get_property(var_plugin,
				'slug')), rt.new_string(' with ')), rt.get_property(var_plugin, 'resource')),
				rt.new_string(' resource type.')),
		])
		return var_result.clone()
	}
	mut var_install := this.install(var_downloaded_path.clone())
	if rt.is_true(rt.call_function('is_wp_error', [var_install.clone()])) {
		rt.call_method(var_result, 'add_error', [
			rt.concat(rt.concat(rt.new_string('Failed to install '), rt.get_property(var_plugin,
				'slug')), rt.new_string('.')),
		])
		return var_result.clone()
	}
	rt.call_method(var_result, 'add_info', [
		rt.concat(rt.concat(rt.new_string('Installed '), rt.get_property(var_plugin, 'slug')),
			rt.new_string('.')),
	])
	mut var_should_activate := rt.new_bool(!(rt.get_property(var_schema, 'options')).is_null()
		&& !(rt.get_property(rt.get_property(var_schema, 'options'), 'activate')).is_null()
		&& rt.is_true(rt.identical(rt.new_bool(true), rt.get_property(rt.get_property(var_schema, 'options'), 'activate'))))
	if rt.is_true(var_should_activate) {
		mut var_activate := this.activate(rt.get_property(var_plugin, 'slug'))
		if rt.is_true(rt.new_bool(rt.instance_of(var_activate,
			'Automattic_WooCommerce_Blueprint_Importers_WP_Error')))
		{
			rt.call_method(var_result, 'add_error', [
				rt.concat(rt.concat(rt.new_string('Failed to activate '), rt.get_property(var_plugin,
					'slug')), rt.new_string('.')),
			])
			return var_result.clone()
		}
		rt.call_method(var_result, 'add_info', [
			rt.concat(rt.concat(rt.new_string('Activated '), rt.get_property(var_plugin, 'slug')),
				rt.new_string('.')),
		])
	}
	return var_result.clone()
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportInstallPlugin) install(var_local_plugin_path rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('Plugin_Upgrader'),
	])))))
	{
		rt.include_file(
			(rt.get_constant('ABSPATH')).str() + '/wp-admin/includes/class-wp-upgrader.php', '2')
		rt.include_file(
			(rt.get_constant('ABSPATH')).str() + '/wp-admin/includes/class-plugin-upgrader.php',
			'2')
	}
	mut var_upgrader :=
		create_plugin_upgrader(create_automattic_woocommerce_blueprint_importers_automatic_upgrader_skin())
	return var_upgrader.install(var_local_plugin_path.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportInstallPlugin) activate(var_slug rt.PhpVal) rt.PhpVal {
	...
	mut var_path := if !(this.installed_plugin_paths.array_get(var_slug_mutated)).is_null() {
		this.installed_plugin_paths.array_get(var_slug_mutated)
	} else {
		rt.new_bool(false)
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_path)))) {
		return rt.new_object('Automattic_WooCommerce_Blueprint_Importers_WP_Error', []string{}, create_automattic_woocommerce_blueprint_importers_wp_error(rt.new_string('plugin_not_installed'),
			rt.new_string('Plugin ${var_slug.to_string()} is not installed.')))
	}
	return this.wp_activate_plugin(var_path.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportInstallPlugin) get_installed_plugins_paths() rt.PhpVal {
	...
	for {
		...
		mut var_path_parts := rt.call_function('explode', [rt.new_string('/'),
			var_path.clone()])
...
fn create_automattic_woocommerce_blueprint_importers_automatic_upgrader_skin(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blueprint_Importers_Automatic_Upgrader_Skin {
...
fn create_automattic_woocommerce_blueprint_importers_wp_error(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blueprint_Importers_WP_Error {
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportInstallPlugin) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'installed_plugin_paths' {
			this.installed_plugin_paths = val
			return true
		}
		else {
