...
fn Class_Automattic_WooCommerce_Blueprint_Cli.register_commands() {
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_import :=
			create_automattic_woocommerce_blueprint_cli_importcli(var_args.array_get(rt.new_int(0)))
		rt.call_method(var_import, 'run', [var_assoc_args.clone()])
		return rt.new_null()
	}
	mut iife_temp_1 := Class_Automattic_WooCommerce_Blueprint_WP_CLI{}
	mut iife_result_1 := iife_temp_1.add_command(rt.new_string('wc blueprint import'),
		rt.new_closure(closure_1_fn), rt.create_array([
	...
	closure_5_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_assoc_args := if args.len > 1 { args[1].clone() } else { rt.new_null() }
		mut var_export :=
			create_automattic_woocommerce_blueprint_cli_exportcli(var_args.array_get(rt.new_int(0)))
		mut var_steps := rt.new_array()
		if var_assoc_args.array_isset(rt.new_string('steps')) {
			closure_4_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
				mut var_step := if args.len > 0 { args[0].clone() } else { rt.new_null() }
				return
			}
			closure_5_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
				mut var_step := if args.len > 0 { args[0].clone() } else { rt.new_null() }
			...
			var_steps = rt.call_function('array_map', [rt.new_closure(closure_4_fn),
				rt.call_function('explode', [rt.new_string(','),
					var_assoc_args.array_get(rt.new_string('steps'))])])
		}
	...
	mut iife_temp_5 := Class_Automattic_WooCommerce_Blueprint_WP_CLI{}
	mut iife_result_5 := iife_temp_5.add_command(rt.new_string('wc blueprint export'),
		rt.new_closure(closure_5_fn), rt.create_array([
		rt.ArrayItem{ key: 'synopsis', val: rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'type', val: 'positional' },
...
fn create_automattic_woocommerce_blueprint_cli(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blueprint_Cli {
...
fn create_automattic_woocommerce_blueprint_wp_cli(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blueprint_WP_CLI {
...
fn create_automattic_woocommerce_blueprint_cli_importcli(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blueprint_Cli_ImportCli {
...
fn create_automattic_woocommerce_blueprint_cli_exportcli(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blueprint_Cli_ExportCli {
...
fn main() {
	...
	if rt.is_true(rt.call_function('file_exists', [var_autoload_path.clone()])) {
