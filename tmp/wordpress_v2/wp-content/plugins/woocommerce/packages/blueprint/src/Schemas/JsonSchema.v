...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Schemas_JsonSchema) construct(var_json_path rt.PhpVal) {
	mut var_real_path := rt.call_function('realpath', [var_json_path.clone()])
	if rt.is_true(rt.identical(rt.new_bool(false), var_real_path)) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Blueprint_Schemas_InvalidArgumentException',
			[]string{},
			create_automattic_woocommerce_blueprint_schemas_invalidargumentexception(rt.new_string('Invalid schema path'))))
	}
	mut var_contents := this.wp_filesystem_get_contents(var_real_path.clone())
	if rt.is_true(rt.identical(rt.new_bool(false), var_contents)) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Blueprint_Schemas_RuntimeException',
			[]string{},
			create_automattic_woocommerce_blueprint_schemas_runtimeexception(rt.new_string('Failed to read the JSON file at ${var_real_path.to_string()}.'))))
	}
	mut var_schema := rt.call_function('json_decode', [var_contents.clone()])
	this.schema = var_schema.clone()
	if !(this.validate()) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Blueprint_Schemas_InvalidArgumentException',
			[]string{},
			create_automattic_woocommerce_blueprint_schemas_invalidargumentexception(rt.new_string("Invalid JSON or missing 'steps' field."))))
	}
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Schemas_JsonSchema) get_step(var_name rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.identical(rt.get_property(var_step, 'step'), var_name)) {
			var_steps.array_push(var_step.clone())
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Schemas_JsonSchema) validate() bool {
	...
	if !(!(rt.get_property(this.schema, 'steps')).is_null())
		|| !(rt.get_property(this.schema, 'steps').is_array()) {
		return false
	}
	return true
}
...
fn create_automattic_woocommerce_blueprint_schemas_invalidargumentexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blueprint_Schemas_InvalidArgumentException {
...
fn create_automattic_woocommerce_blueprint_schemas_runtimeexception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blueprint_Schemas_RuntimeException {
