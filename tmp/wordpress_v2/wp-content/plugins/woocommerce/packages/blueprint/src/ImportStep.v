...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ImportStep) construct(var_step_definition rt.PhpVal, mut var_validator Class_Automattic_WooCommerce_Blueprint_?Validator) {
	...
	this.step_definition = var_step_definition.clone()
	...
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_importer := if args.len > 1 { args[1].clone() } else { rt.new_null() }
		return
		}
	mut iife_temp_1 := Class_Automattic_WooCommerce_Blueprint_Util{}
	mut iife_result_1 := iife_temp_1.index_array(this.importers, rt.new_closure(closure_1_fn))
	this.indexed_importers = iife_result_1
}

fn (mut this Class_Automattic_WooCommerce_Blueprint_ImportStep) import() rt.PhpVal {
	mut iife_temp_2 := Class_Automattic_WooCommerce_Blueprint_StepProcessorResult{}
	mut iife_result_2 := iife_temp_2.success(rt.get_property(this.step_definition, 'step'))
	mut var_result := iife_result_2
	if !(this.can_import(var_result.clone())) {
	...
	var_logger.start_import(rt.get_property(this.step_definition, 'step'), rt.call_function('get_class', [var_importer.clone()]))
	...
	if rt.is_true(rt.call_method(var_importer_result, 'is_success', []rt.PhpVal{})) {
		var_logger.complete_import(rt.get_property(this.step_definition, 'step'), var_importer_result.clone())
	} else {
		var_logger.import_step_failed(rt.get_property(this.step_definition, 'step'), var_importer_result.clone())
	}
	rt.call_method(var_result, 'merge_messages', [var_importer_result.clone()])
	return var_result.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ImportStep) validate_step_schemas(mut var_importer Class_Automattic_WooCommerce_Blueprint_StepProcessor, mut var_result Class_Automattic_WooCommerce_Blueprint_StepProcessorResult) bool {
	...
	mut var_validate := rt.call_method(this.validator, 'validate', [this.step_definition, rt.call_function('wp_json_encode', [var_step_schema.clone()])])
...
fn create_automattic_woocommerce_blueprint_builtinstepprocessors(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Blueprint_BuiltInStepProcessors {
...
fn create_opis_jsonschema_errors_errorformatter(_args ...rt.PhpVal) &Class_Opis_JsonSchema_Errors_ErrorFormatter {
