...
pub fn Class_Automattic_WooCommerce_Blueprint_Importers_ImportSetSiteOptions.restricted_options() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: none, val: 'siteurl' },
		rt.ArrayItem{ key: none, val: 'home' }, rt.ArrayItem{ key: none, val: 'active_plugins' },
		rt.ArrayItem{ key: none, val: 'template' }, rt.ArrayItem{ key: none, val: 'stylesheet' },
		rt.ArrayItem{ key: none, val: 'admin_email' }, rt.ArrayItem{
			key: none
			val: 'unfiltered_html'
		}, rt.ArrayItem{ key: none, val: 'users_can_register' },
		rt.ArrayItem{ key: none, val: 'default_role' }, rt.ArrayItem{ key: none, val: 'db_version' },
		rt.ArrayItem{ key: none, val: 'cron' }, rt.ArrayItem{ key: none, val: 'rewrite_rules' },
		rt.ArrayItem{ key: none, val: 'wp_user_roles' }])
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_Importers_ImportSetSiteOptions) process(var_schema rt.PhpVal) rt.PhpVal {
	mut iife_temp_0 := Class_Automattic_WooCommerce_Blueprint_Steps_SetSiteOptions{}
	...
	for {
		...
		if rt.is_true(rt.call_function('in_array', [var_key.clone(),
			Class_Automattic_WooCommerce_Blueprint_Importers_Automattic_WooCommerce_Blueprint_Importers_ImportSetSiteOptions.restricted_options(),
			rt.new_bool(true)]))
		{
			rt.call_method(var_result, 'add_warn', [
				rt.new_string("Cannot modify '${var_key.to_string()}' option: Modifying is restricted for this key."),
			])
			continue
		}
		var_value = rt.call_function('json_decode', [
			rt.call_function('wp_json_encode', [var_value.clone()]),
			rt.new_bool(true),
		])
		mut var_updated := this.wp_update_option(var_key.clone(), var_value.clone())
		mut var_current_value := this.wp_get_option(var_key.clone())
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_current_value, var_value)))) {
			rt.call_method(var_result, 'add_warn', [
				rt.new_string('${var_key.to_string()} was intended to be set, but the stored value may have been overridden by a hook.'),
			])
			continue
		}
		if rt.is_true(var_updated) {
			rt.call_method(var_result, 'add_info', [
				rt.new_string('${var_key.to_string()} has been updated.'),
			])
			continue
		}
		if rt.is_true(rt.identical(var_current_value, var_value)) {
			rt.call_method(var_result, 'add_info', [
				rt.new_string('${var_key.to_string()} has not been updated because the current value is already up to date.'),
			])
		}
	}
