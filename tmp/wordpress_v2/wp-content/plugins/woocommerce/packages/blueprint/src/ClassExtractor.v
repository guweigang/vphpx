...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ClassExtractor) construct(file_path string) {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('file_exists', [
		rt.new_string(file_path),
	])))))
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_Blueprint_InvalidArgumentException',
			[]string{},
			create_automattic_woocommerce_blueprint_invalidargumentexception(rt.new_string('File not found: ${var_file_path}'))))
	}
	this.file_path = file_path
}

fn (mut this Class_Automattic_WooCommerce_Blueprint_ClassExtractor) with_wp_load() rt.PhpVal {
	this.prefix = rt.concat(this.prefix,
		rt.new_string("<?php require_once 'wordpress/wp-load.php'; "))
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ClassExtractor) replace_class_variable(var_variable_name rt.PhpVal, var_new_value rt.PhpVal) rt.PhpVal {
	this.class_variable_replacements.array_set(var_variable_name, var_new_value.clone())
	return rt.new_object('Automattic_WooCommerce_Blueprint_ClassExtractor', []string{}, this)
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ClassExtractor) replace_method_variable(var_method_name rt.PhpVal, var_variable_name rt.PhpVal, var_new_value rt.PhpVal) rt.PhpVal {
	this.method_variable_replacements.array_push(rt.create_array([
		rt.ArrayItem{ key: 'method', val: var_method_name },
		rt.ArrayItem{ key: 'variable', val: var_variable_name },
		rt.ArrayItem{ key: 'value', val: var_new_value },
	]))
	return rt.new_object('Automattic_WooCommerce_Blueprint_ClassExtractor', []string{}, this)
}

fn (mut this Class_Automattic_WooCommerce_Blueprint_ClassExtractor) get_code() string {
	mut var_file_content := rt.call_function('file_get_contents', [
		rt.new_string(this.file_path),
	])
	var_file_content = rt.call_function('preg_replace', [rt.new_string('/<\\?php\\s*/'),
		rt.new_string(''), var_file_content.clone()])
	if rt.is_true(rt.call_function('preg_match', [
		rt.new_string('/declare\\s*\\(\\s*strict_types\\s*=\\s*1\\s*\\)\\s*;/'),
		var_file_content.clone(),
	]))
	{
		this.has_strict_types_declaration = true
		var_file_content = rt.call_function('preg_replace', [
			rt.new_string('/declare\\s*\\(\\s*strict_types\\s*=\\s*1\\s*\\)\\s*;/'),
			rt.new_string(''),
			var_file_content.clone(),
		])
	...
	for {
		...
		var_file_content = this.apply_class_variable_replacement(var_file_content.clone(),
			var_variable.clone(), var_value.clone())
	}
	mut iter_2 := this.method_variable_replacements.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_replacement := item_2.val
		var_file_content = this.apply_variable_replacement(var_file_content.clone(),
			var_replacement.array_get(rt.new_string('method')),
			var_replacement.array_get(rt.new_string('variable')),
			var_replacement.array_get(rt.new_string('value')))
	}
	return (this.prefix).str() + var_file_content.clone().to_string().trim_space()
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ClassExtractor) apply_class_variable_replacement(var_file_content rt.PhpVal, var_variable_name rt.PhpVal, var_new_value rt.PhpVal) rt.PhpVal {
	...
	mut var_replacement_value := rt.call_function('var_export', [
		var_new_value.clone(), rt.new_bool(true)])
	mut var_pattern := rt.new_string('/(protected|private|public)\\s+\\$' +
		(rt.call_function('preg_quote', [var_variable_name.clone(), rt.new_string('/')])).str() +
		'\\s*=\\s*.*?;|' + '(protected|private|public)\\s+\\$' +
		(rt.call_function('preg_quote', [var_variable_name.clone(), rt.new_string('/')])).str() +
		'\\s*;?/')
	mut var_replacement := rt.new_string((rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('$1 $'),
		var_variable_name), rt.new_string(' = ')), var_replacement_value), rt.new_string(';'))).str())
	return rt.call_function('preg_replace', [var_pattern.clone(),
		var_replacement.clone(), var_file_content_mutated.clone(),
		rt.new_int(1)])
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ClassExtractor) apply_variable_replacement(var_file_content rt.PhpVal, var_method_name rt.PhpVal, var_variable_name rt.PhpVal, var_new_value rt.PhpVal) rt.PhpVal {
	...
	mut var_file_content_mutated := var_file_content
	mut var_pattern := rt.new_string('/function\\s+' +
		(rt.call_function('preg_quote', [var_method_name.clone(), rt.new_string('/')])).str() +
		'\\s*\\([^)]*\\)\\s*\\{\\s*(.*?)\\s*\\}/s')
	if rt.is_true(rt.call_function('preg_match', [var_pattern.clone(),
		var_file_content_mutated.clone(), var_matches.clone()]))
	{
		mut var_method_body := var_matches.array_get(rt.new_int(1))
		mut var_new_value_exported := rt.call_function('var_export', [
			var_new_value.clone(), rt.new_bool(true)])
		mut var_variable_pattern := rt.new_string('/\\$' +
			(rt.call_function('preg_quote', [var_variable_name.clone(), rt.new_string('/')])).str() +
			'\\s*=\\s*[^;]+;/')
		mut var_replacement := rt.new_string('$' + var_variable_name.str() + ' = ' +
			var_new_value_exported.str() + ';')
		mut var_updated_method_body := rt.call_function('preg_replace', [
			var_variable_pattern.clone(), var_replacement.clone(),
			var_method_body.clone(), rt.new_int(1)])
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_updated_method_body)))) {
			var_file_content_mutated = rt.call_function('str_replace', [
				var_method_body.clone(), var_updated_method_body.clone(),
				var_file_content_mutated.clone()])
		}
	}
	return var_file_content_mutated.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ClassExtractor) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'apply_class_variable_replacement' {
			...
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			dispatch_arg_2 := if args.len > 2 { args[2] } else { rt.new_null() }
			return this.apply_class_variable_replacement(dispatch_arg_0, dispatch_arg_1,
				dispatch_arg_2)
		}
		'apply_variable_replacement' {
			...
			return this.apply_variable_replacement(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2,
				dispatch_arg_3)
		}
...
fn (mut this Class_Automattic_WooCommerce_Blueprint_ClassExtractor) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'file_path' {
			this.file_path = val.str()
			return true
		}
		'has_strict_types_declaration' {
			this.has_strict_types_declaration = val.to_bool()
			return true
		}
		'prefix' {
			this.prefix = val
		...
		'class_variable_replacements' {
			this.class_variable_replacements = val
			return true
		}
		'method_variable_replacements' {
			this.method_variable_replacements = val
			return true
		}
		else {
