...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Node_SelectorNode) getspecificity() rt.PhpVal {
	return rt.call_method(rt.call_method(this.tree, 'getSpecificity', []rt.PhpVal{}), 'plus', [create_automattic_woocommerce_emaileditorvendor_symfony_component_cssselector_node_specificity(rt.new_int(0), rt.new_int(0), if rt.is_true(this.pseudoElement) { 1 } else { 0 })])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Node_SelectorNode) magic_tostring() string {
	return (rt.call_function('sprintf', [rt.new_string('%s[%s%s]'), this.getnodename(), this.tree, rt.new_string((if rt.is_true(this.pseudoElement) { '::' + (this.pseudoElement).str() } else { '' }).str())])).str()
}
