	...
	if rt.is_true(var_oParserState.comes(rt.new_string('#'))) {
		...
		if rt.is_true(rt.identical(var_oParserState.strlen(var_sValue.clone()), rt.new_int(8))) {
			var_aColor = rt.create_array([
				rt.ArrayItem{ key: 'r', val: create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_size(rt.new_int(rt.new_string(
					(var_sValue.array_get(rt.new_int(0))).str() +
					(var_sValue.array_get(rt.new_int(1))).str()).to_i64()), rt.new_null(),
					rt.new_bool(true), var_oParserState.currentline()) },
				rt.ArrayItem{ key: 'g', val: create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_size(rt.new_int(rt.new_string(
					(var_sValue.array_get(rt.new_int(2))).str() +
					(var_sValue.array_get(rt.new_int(3))).str()).to_i64()), rt.new_null(),
					rt.new_bool(true), var_oParserState.currentline()) },
				rt.ArrayItem{ key: 'b', val: create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_size(rt.new_int(rt.new_string(
				...
				rt.ArrayItem{ key: 'a', val: create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_size(rt.call_function('round', [
					Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_Color.maprange(rt.new_int(rt.new_string(
		...
		for {
			...
			if rt.is_true(var_oParserState.comes(rt.new_string('var'))) {
				...
				mut iife_result_0 := iife_temp_0.parseidentifierorfunction(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Parsing_ParserState',
		...
		var_oParserState.consume(rt.new_string(')'))
		if rt.is_true(var_bContainsVar) {
			return rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CSSFunction',
				[]string{}, create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_cssfunction(var_sColorMode.clone(), rt.call_function('array_values', [
				var_aColor.clone(),
			]), rt.new_string(','), var_oParserState.currentline()))
		}
	}
	return rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_Color', [
		'Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CSSFunction',
	], create_automattic_woocommerce_emaileditorvendor_sabberworm_css_value_color(var_aColor.clone(),
...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_Color) render(var_oOutputFormat rt.PhpVal) string {
	...
	return (this.Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Value_CSSFunction.render(var_oOutputFormat.clone())).str()
}
