...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_Handler_StringHandler) handle(mut var_reader Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_Reader, mut var_stream Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_TokenStream) bool {
	...
	mut var_match := var_reader.findpattern(rt.call_method(this.patterns, 'getQuotedStringPattern', [
		var_quote.clone(),
	]))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_match)))) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Exception_InternalErrorException',
			[]string{}, create_automattic_woocommerce_emaileditorvendor_symfony_component_cssselector_exception_internalerrorexception(rt.call_function('sprintf', [
			rt.new_string('Should have found at least an empty match at %d.'),
			var_reader.getposition(),
		]))))
	}
	if rt.is_true(rt.identical(rt.new_int(var_match.array_get(rt.new_int(0)).to_string().len),
		var_reader.getremaininglength()))
	{
		mut iife_temp_0 :=
			Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Exception_SyntaxErrorException{}
		mut iife_result_0 := iife_temp_0.unclosedstring(rt.sub(var_reader.getposition(),
			rt.new_int(1)))
		rt.throw_exception(iife_result_0)
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_quote, var_reader.getsubstring(rt.new_int(1),
		rt.new_int(var_match.array_get(rt.new_int(0)).to_string().len))))))
	{
		mut iife_temp_1 :=
			Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Exception_SyntaxErrorException{}
		mut iife_result_1 := iife_temp_1.unclosedstring(rt.sub(var_reader.getposition(),
			rt.new_int(1)))
		rt.throw_exception(iife_result_1)
	}
	mut var_string := rt.call_method(this.escaping, 'escapeUnicodeAndNewLine', [
		var_match.array_get(rt.new_int(0)),
	])
	var_stream.push(create_automattic_woocommerce_emaileditorvendor_symfony_component_cssselector_parser_token(Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_Token.type_string(),
		var_string.clone(), var_reader.getposition()))
	var_reader.moveforward(rt.new_int(var_match.array_get(rt.new_int(0)).to_string().len + 1))
	return true
}
...
fn create_automattic_woocommerce_emaileditorvendor_symfony_component_cssselector_parser_handler_stringhandler(arg_0 rt.PhpVal, arg_1 rt.PhpVal) &Class_Automattic_WooCommerce_EmailEditorVendor_Symfony_Component_CssSelector_Parser_Handler_StringHandler {
