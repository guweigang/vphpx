...
fn (mut this Class_Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Property_Import) render(var_oOutputFormat rt.PhpVal) string {
	return (rt.call_method(var_oOutputFormat, 'comments', [rt.new_object('Automattic_WooCommerce_EmailEditorVendor_Sabberworm_CSS_Property_Import', ['AtRule', 'Positionable'], &this)])).str() + '@import ' + (rt.call_method(this.oLocation, 'render', [var_oOutputFormat.clone()])).str() + if rt.is_true(rt.identical(this.sMediaQuery, rt.new_null())) {
		''
	} else {
