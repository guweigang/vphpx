...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer) construct(mut var_content_renderer Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Content_Renderer, mut var_templates Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates, mut var_css_inliner Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Css_Inliner, mut var_theme_controller Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller, mut var_personalization_tags_registry Class_Automattic_WooCommerce_EmailEditor_Engine_PersonalizationTags_Personalization_Tags_Registry, mut var_process_manager Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Process_Manager) {
	this.content_renderer = var_content_renderer
	...
	this.process_manager = var_process_manager
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer) render(mut var_post Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_WP_Post, subject string, pre_header string, language string, meta_robots string, template_slug string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_string(template_slug_mutated))))) {
		template_slug_mutated = (if rt.is_true(rt.call_function('get_page_template_slug', [
			var_post,
		]))
		{
			rt.call_function('get_page_template_slug', [var_post])
		} else {
			rt.new_string('email-general')
		}).str()
	}
	mut var_template := rt.call_method(this.templates, 'get_block_template', [
		rt.new_string(template_slug_mutated).clone()])
	mut var_email_styles := rt.call_method(this.theme_controller, 'get_styles', []rt.PhpVal{})
	mut var_content_result := rt.call_method(this.content_renderer, 'render_without_css_inline', [
		var_post,
		var_template.clone(),
	])
	mut var_template_html := var_content_result.array_get(rt.new_string('html'))
	mut var_content_styles := var_content_result.array_get(rt.new_string('styles'))
	mut var_layout := rt.call_method(this.theme_controller, 'get_layout_settings', []rt.PhpVal{})
	rt.call_function('ob_start', []rt.PhpVal{})
	rt.include_file((Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer.template_file()).to_string(),
		'1')
	mut var_rendered_template :=
		rt.new_string((rt.call_function('ob_get_clean', []rt.PhpVal{})).str())
	mut iife_temp_0 := Class_WP_Style_Engine{}
	mut iife_result_0 := iife_temp_0.compile_css(rt.create_array([
		rt.ArrayItem{
			key: 'background-color'
			val: if !(var_email_styles.array_get(rt.new_string('color')).array_get(rt.new_string('background'))).is_null() {
				var_email_styles.array_get(rt.new_string('color')).array_get(rt.new_string('background'))
			} else {
		...
		rt.ArrayItem{
			key: 'color'
			val: if !(var_email_styles.array_get(rt.new_string('color')).array_get(rt.new_string('text'))).is_null() {
		...
		rt.ArrayItem{
			key: 'padding-top'
			val: if !(var_email_styles.array_get(rt.new_string('spacing')).array_get(rt.new_string('padding')).array_get(rt.new_string('top'))).is_null() {
				var_email_styles.array_get(rt.new_string('spacing')).array_get(rt.new_string('padding')).array_get(rt.new_string('top'))
			} else {
				rt.new_string('0px')
			}
		},
		rt.ArrayItem{
			key: 'padding-bottom'
			val: if !(var_email_styles.array_get(rt.new_string('spacing')).array_get(rt.new_string('padding')).array_get(rt.new_string('bottom'))).is_null() {
				var_email_styles.array_get(rt.new_string('spacing')).array_get(rt.new_string('padding')).array_get(rt.new_string('bottom'))
			} else {
				rt.new_string('0px')
			}
		},
		rt.ArrayItem{
			key: 'font-family'
			val: if !(var_email_styles.array_get(rt.new_string('typography')).array_get(rt.new_string('fontFamily'))).is_null() {
				var_email_styles.array_get(rt.new_string('typography')).array_get(rt.new_string('fontFamily'))
			} else {
				rt.new_string('inherit')
			}
		},
		rt.ArrayItem{
			key: 'line-height'
			val: if !(var_email_styles.array_get(rt.new_string('typography')).array_get(rt.new_string('lineHeight'))).is_null() {
				var_email_styles.array_get(rt.new_string('typography')).array_get(rt.new_string('lineHeight'))
			} else {
				rt.new_string('1.5')
			}
		},
		rt.ArrayItem{
			key: 'font-size'
			val: if !(var_email_styles.array_get(rt.new_string('typography')).array_get(rt.new_string('fontSize'))).is_null() {
				var_email_styles.array_get(rt.new_string('typography')).array_get(rt.new_string('fontSize'))
			} else {
				rt.new_string('inherit')
			}
		},
	]), rt.new_string('body, .email_layout_wrapper'))
	mut var_template_styles := iife_result_0
	var_template_styles = rt.concat(var_template_styles,
		rt.new_string('.email_layout_wrapper { box-sizing: border-box;}'))
	var_template_styles = rt.concat(var_template_styles, rt.call_function('file_get_contents', [
		rt.new_string(@DIR + '/' +(Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer.template_styles_file()).str()),
	]))
	var_template_styles = rt.call_function('wp_strip_all_tags', [
		rt.new_string((rt.call_function('apply_filters', [
			rt.new_string('woocommerce_email_renderer_styles'),
			var_template_styles.clone(),
			var_post,
		])).str()),
	])
	mut var_all_styles := rt.new_string('<style>' + var_template_styles.str() +
		var_content_styles.str() + '</style>')
	var_rendered_template = this.inline_css_styles(rt.new_string(var_all_styles.str() +
		var_rendered_template.str()))
	var_rendered_template = rt.call_method(this.process_manager, 'postprocess', [
		var_rendered_template.clone(),
	])
	if var_email_styles.array_get(rt.new_string('elements')).array_get(rt.new_string('link')).array_get(rt.new_string(':hover')).array_get(rt.new_string('color')).array_isset(rt.new_string('text')) {
		var_rendered_template = rt.call_function('str_replace', [
			rt.new_string('<!-- Forced Styles -->'),
			rt.new_string('<style>a:hover { color: ' +
				(rt.call_function('esc_attr', [var_email_styles.array_get(rt.new_string('elements')).array_get(rt.new_string('link')).array_get(rt.new_string(':hover')).array_get(rt.new_string('color')).array_get(rt.new_string('text'))])).str() +
				' !important; }</style>'),
			var_rendered_template.clone(),
		])
	}
	return rt.create_array([rt.ArrayItem{ key: 'html', val: var_rendered_template },
		rt.ArrayItem{ key: 'text', val: this.render_text_version(var_rendered_template.clone()) }])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer) inline_css_styles(var_template rt.PhpVal) rt.PhpVal {
	...
	return rt.call_method(rt.call_method(rt.call_method(this.css_inliner, 'from_html', [
		var_template_mutated.clone(),
	]), 'inline_css', []rt.PhpVal{}), 'render', []rt.PhpVal{})
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer) render_text_version(var_template rt.PhpVal) string {
	mut var_template_mutated := var_template
	var_template_mutated = if rt.is_true(rt.call_function('mb_detect_encoding', [
		var_template_mutated.clone(),
		rt.new_string('UTF-8'),
		rt.new_bool(true),
	]))
	{ var_template_mutated } else { rt.call_function('mb_convert_encoding', [
			var_template_mutated.clone(), rt.new_string('UTF-8'),
			rt.call_function('mb_list_encodings', []rt.PhpVal{})]) }
	if !(var_template_mutated.clone().is_string()) {
		return ''
	}
	var_template_mutated =
		rt.new_string(this.preserve_personalization_tags(var_template_mutated.str()))
	mut iife_temp_1 := Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Html2Text{}
	mut iife_result_1 := iife_temp_1.convert(rt.new_string(var_template_mutated.str()), rt.create_array([
		rt.ArrayItem{ key: 'ignore_errors', val: true },
	]))
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer) preserve_personalization_tags(template string) string {
	...
	mut var_all_registered_tags := rt.call_method(this.personalization_tags_registry, 'get_all',
		[]rt.PhpVal{})
	this.personalization_tag_placeholders = rt.new_array()
	mut var_counter := rt.new_int(0)
	...
	for {
		...
		var_token_prefixes.array_push(rt.call_function('preg_quote', [
			rt.call_function('substr', [var_token.clone(), rt.new_int(1),
				rt.new_int(-1)]),
			rt.new_string('/'),
		]))
	}
	...
	mut var_pattern := rt.new_string('/<!--\\[(' +
		(rt.call_function('implode', [rt.new_string('|'), var_token_prefixes.clone()])).str() +
		')(?:\\s+[^\\]]*)?\\]-->/')
	closure_3_fn := fn [mut var_counter, var_base_tokens] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_matches := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_base_token :=
			rt.new_string('[' + (var_matches.array_get(rt.new_int(1))).str() + ']')
		if var_base_tokens.array_isset(var_base_token) {
			mut var_placeholder := rt.new_string('PERSONALIZATION_TAG_PLACEHOLDER_' +
				var_counter.str())
			this.personalization_tag_placeholders.array_set(var_placeholder,
	...
	template_mutated = (rt.call_function('preg_replace_callback', [
		var_pattern.clone(), rt.new_closure(closure_3_fn), rt.new_string(template_mutated).clone()])).str()
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer) restore_personalization_tags(text string) string {
	...
	mut iter_2 := this.personalization_tag_placeholders.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_html_comment := item_2.val
		mut var_placeholder := item_2.key
		text_mutated = (rt.call_function('str_replace', [var_placeholder.clone(),
			var_html_comment.clone(), rt.new_string(text_mutated).clone()])).str()
	}
	return text_mutated
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Content_Renderer](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
			...
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Css_Inliner](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_3 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller](if args.len > 3 {
				args[3]
			} else {
				rt.new_null()
			})
			...
			mut dispatch_arg_5 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Process_Manager](if args.len > 5 {
				args[5]
			} else {
				rt.new_null()
			})
		...
		'render' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_WP_Post](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
			...
			return this.render(mut dispatch_arg_0, dispatch_arg_1, dispatch_arg_2, dispatch_arg_3,
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_Renderer) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'css_inliner' {
			this.css_inliner = val
			return true
		}
		'process_manager' {
			this.process_manager = val
			return true
		}
		...
		'personalization_tag_placeholders' {
			this.personalization_tag_placeholders = val
			return true
		}
		else {
