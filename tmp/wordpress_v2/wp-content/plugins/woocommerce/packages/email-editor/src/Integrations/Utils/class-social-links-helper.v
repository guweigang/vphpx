...
fn Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Social_Links_Helper.detect_whiteish_color(var_input_color rt.PhpVal) bool {
	...
	mut var_color := rt.new_string(var_input_color.clone().to_string().trim_left(' \t\n\r'))
	if var_color.clone().to_string().len == 3 {
		var_color = rt.new_string((var_color.array_get(rt.new_int(0))).str() +
			(var_color.array_get(rt.new_int(0))).str() +
			(var_color.array_get(rt.new_int(1))).str() +
			(var_color.array_get(rt.new_int(1))).str() +
			(var_color.array_get(rt.new_int(2))).str() + (var_color.array_get(rt.new_int(2))).str())
	}
	mut var_r := rt.call_function('hexdec', [
		rt.call_function('substr', [var_color.clone(), rt.new_int(0),
			rt.new_int(2)]),
	])
	mut var_g := rt.call_function('hexdec', [
		rt.call_function('substr', [var_color.clone(), rt.new_int(2),
			rt.new_int(2)]),
	])
	mut var_b := rt.call_function('hexdec', [
		rt.call_function('substr', [var_color.clone(), rt.new_int(4),
			rt.new_int(2)]),
	])
