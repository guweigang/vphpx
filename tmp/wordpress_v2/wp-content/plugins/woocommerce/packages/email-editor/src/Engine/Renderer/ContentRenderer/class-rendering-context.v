...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) construct(mut var_theme_json Class_WP_Theme_JSON, mut var_email_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_array) {
	this.theme_json = var_theme_json
	this.email_context = var_email_context
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) get_theme_styles() rt.PhpVal {
	...
	return if !(rt.call_method(var_theme, 'get_data', []rt.PhpVal{}).array_get(rt.new_string('styles'))).is_null() {
		rt.call_method(var_theme, 'get_data', []rt.PhpVal{}).array_get(rt.new_string('styles'))
	} else {
		rt.new_array()
	}
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) get_layout_width_without_padding() string {
	...
	mut var_layout_settings := if !(this.get_theme_settings().array_get(rt.new_string('layout'))).is_null() {
		this.get_theme_settings().array_get(rt.new_string('layout'))
	} else {
		rt.new_array()
	}
	mut iife_temp_0 := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper{}
	mut iife_result_0 := iife_temp_0.parse_value(if !(var_layout_settings.array_get(rt.new_string('contentSize'))).is_null() {
		var_layout_settings.array_get(rt.new_string('contentSize'))
	} else {
		rt.new_string('0px')
	})
	mut var_width := iife_result_0
	...
	mut iife_result_1 := iife_temp_1.parse_value(if !(var_padding.array_get(rt.new_string('left'))).is_null() {
		var_padding.array_get(rt.new_string('left'))
	} else {
		rt.new_string('0px')
	})
	var_width = rt.sub(var_width, iife_result_1)
	mut iife_temp_2 := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper{}
	mut iife_result_2 := iife_temp_2.parse_value(if !(var_padding.array_get(rt.new_string('right'))).is_null() {
		var_padding.array_get(rt.new_string('right'))
	} else {
		rt.new_string('0px')
	})
	var_width = rt.sub(var_width, iife_result_2)
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) translate_slug_to_color(color_slug string) string {
	...
	mut var_color_definitions := rt.call_function('array_merge', [if !(var_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('theme'))).is_null() {
		var_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('theme'))
	} else {
		rt.new_array()
	}, if !(var_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('default'))).is_null() {
		var_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('default'))
	} else {
		rt.new_array()
	}])
	...
	for {
		...
		if rt.is_true(rt.identical(var_color_definition.array_get(rt.new_string('slug')),
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) get_user_id() i64 {
	return (if this.email_context.array_isset(rt.new_string('user_id'))
		&& this.email_context.array_get(rt.new_string('user_id')).is_long()
		|| this.email_context.array_get(rt.new_string('user_id')).is_double() {
		rt.new_int((this.email_context.array_get(rt.new_string('user_id'))).to_i64())
	} else {
		rt.new_null()
	}).to_i64()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) get_recipient_email() string {
	return (if this.email_context.array_isset(rt.new_string('recipient_email'))
		&& this.email_context.array_get(rt.new_string('recipient_email')).is_string() {
		this.email_context.array_get(rt.new_string('recipient_email'))
	} else {
		rt.new_null()
	}).str()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) get(key string, var_default_value rt.PhpVal) rt.PhpVal {
	return if !(this.email_context.array_get(rt.new_string(key))).is_null() {
		this.email_context.array_get(rt.new_string(key))
	} else {
		var_default_value
	}
}
