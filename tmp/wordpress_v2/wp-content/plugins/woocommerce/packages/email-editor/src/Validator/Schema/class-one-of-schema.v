...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema_One_Of_Schema) nullable() rt.PhpVal {
	...
	mut var_one_of := this.schema.array_get(rt.new_string('oneOf'))
	mut var_value := if rt.is_true(rt.call_function('in_array', [
		var_null.clone(), var_one_of.clone(), rt.new_bool(true)]))
	{ var_one_of } else { rt.call_function('array_merge', [var_one_of.clone(),
			rt.create_array([rt.ArrayItem{ key: none, val: var_null }])]) }
	return this.update_schema_property(rt.new_string('oneOf'), var_value.clone())
}

fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema_One_Of_Schema) non_nullable() rt.PhpVal {
	mut var_null := rt.create_array([rt.ArrayItem{ key: 'type', val: 'null' }])
	mut var_one_of := this.schema.array_get(rt.new_string('one_of'))
	...
	mut var_value := rt.call_function('array_filter', [var_one_of.clone(),
		rt.new_closure(closure_1_fn)])
	return this.update_schema_property(rt.new_string('one_of'), var_value.clone())
}
