...
fn init_static_automattic_woocommerce_emaileditor_integrations_utils_styles_helper() {
		rt.init_static_prop('Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper', 'empty_block_styles', rt.create_array([rt.ArrayItem{ key: 'css', val: '' }, rt.ArrayItem{ key: 'declarations', val: rt.new_array() }, rt.ArrayItem{ key: 'classnames', val: '' }]))
}
...
fn Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.parse_value(var_value rt.PhpVal) f64 {
	...
	if rt.is_true(rt.new_bool(var_value_mutated.clone().is_long() || var_value_mutated.clone().is_double())) {
		return rt.new_float((var_value_mutated).to_f64())
	}
	if rt.is_true(rt.new_bool(var_value_mutated.clone().is_string())) {
		if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^\\s*(-?\\d+(?:\\.\\d+)?)/'), var_value_mutated.clone(), var_m.clone()])) {
			return rt.new_float((var_m.array_get(rt.new_int(1))).to_f64())
		}
	}
	return 0
}
...
fn Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.parse_styles_to_array(styles string) rt.PhpVal {
	...
	styles_mutated = (rt.call_function('explode', [rt.new_string(';'), rt.new_string(styles_mutated).clone()])).str()
	mut var_parsed_styles := rt.new_array()
	mut iter_1 := rt.new_string(styles_mutated).iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_style := item_1.val
		var_style = rt.call_function('explode', [rt.new_string(':'), var_style.clone(), rt.new_int(2)])
		if var_style.clone().array_count() == 2 {
			var_parsed_styles.array_set(var_style.array_get(rt.new_int(0)).to_string().trim_space(), var_style.array_get(rt.new_int(1)).to_string().trim_space())
		}
	}
...
fn Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.get_normalized_block_styles(mut var_block_attributes Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) rt.PhpVal {
	mut var_normalized_colors := rt.call_function('array_filter', [rt.create_array([rt.ArrayItem{ key: 'color', val: rt.call_function('array_filter', [rt.create_array([rt.ArrayItem{ key: 'background', val: if var_block_attributes.array_isset(rt.new_string('backgroundColor')) && rt.is_true(var_block_attributes.array_get(rt.new_string('backgroundColor'))) { var_rendering_context.translate_slug_to_color(var_block_attributes.array_get(rt.new_string('backgroundColor'))) } else { rt.new_null() } }, rt.ArrayItem{ key: 'text', val: if var_block_attributes.array_isset(rt.new_string('textColor')) && rt.is_true(var_block_attributes.array_get(rt.new_string('textColor'))) { var_rendering_context.translate_slug_to_color(var_block_attributes.array_get(rt.new_string('textColor'))) } else { rt.new_null() } }])]) }, rt.ArrayItem{ key: 'border', val: rt.call_function('array_filter', [rt.create_array([rt.ArrayItem{ key: 'color', val: if var_block_attributes.array_isset(rt.new_string('borderColor')) && rt.is_true(var_block_attributes.array_get(rt.new_string('borderColor'))) { var_rendering_context.translate_slug_to_color(var_block_attributes.array_get(rt.new_string('borderColor'))) } else { rt.new_null() } }])]) }])])
	return rt.call_function('array_replace_recursive', [var_normalized_colors.clone(), if !(var_block_attributes.array_get(rt.new_string('style'))).is_null() { var_block_attributes.array_get(rt.new_string('style')) } else { rt.new_array() }])
}
...
fn Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.get_styles_from_block(mut var_block_styles Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_array, skip_convert_vars bool) rt.PhpVal {
	...
	var_unsupported_props = rt.call_function('apply_filters', [rt.new_string('woocommerce_email_editor_styles_unsupported_props'), var_unsupported_props.clone()])
	mut iter_2 := var_unsupported_props.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_path := item_2.val
		if !(var_path.clone().is_array()) || var_path.clone().array_count() == 0 {
			continue
		}
		var_pointer = var_block_styles_mutated
		mut var_last_key := rt.call_function('array_pop', [var_path.clone()])
		mut iter_3 := var_path.iterator()
		for {
			item_3 := iter_3.next() or { break }
			mut var_segment := item_3.val
			if !(var_segment.clone().is_string()) && !(var_segment.clone().is_long()) {
				continue
			}
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_pointer.clone().array_isset(var_segment.clone())))))) || !(var_pointer.array_get(var_segment).is_array()) {
				continue
			}
			var_pointer = var_pointer.array_get(var_segment)
		}
		if var_last_key.clone().is_string() || var_last_key.clone().is_long() && rt.is_true(rt.new_bool(var_pointer.clone().array_isset(var_last_key.clone()))) {
			var_pointer.array_unset(var_last_key)
		}
	}
	return rt.call_function('wp_parse_args', [rt.call_function('wp_style_engine_get_styles', [var_block_styles_mutated, rt.create_array([rt.ArrayItem{ key: 'convert_vars_to_classnames', val: skip_convert_vars }])]), rt.get_static_prop('Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper', 'empty_block_styles')])
}
...
fn Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.extend_block_styles(mut var_block_styles Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_array, mut var_css_declarations Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_array) rt.PhpVal {
	mut var_block_styles_mutated := var_block_styles
	if !(var_block_styles_mutated.array_isset(rt.new_string('declarations'))) || !(var_block_styles_mutated.array_get(rt.new_string('declarations')).is_array()) {
	var_block_styles_mutated = rt.get_static_prop('Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper', 'empty_block_styles')
	}
	var_block_styles_mutated.array_set('declarations', rt.call_function('array_merge', [var_block_styles_mutated.array_get(rt.new_string('declarations')), var_css_declarations]))
	mut iife_temp_0 := Class_WP_Style_Engine{}
	mut iife_result_0 := iife_temp_0.compile_css(var_block_styles_mutated.array_get(rt.new_string('declarations')), rt.new_string(''))
	var_block_styles_mutated.array_set('css', iife_result_0)
...
fn Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.get_block_styles(mut var_block_attributes Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context, mut var_properties Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_array) rt.PhpVal {
	...
	mut iter_4 := var_properties.iterator()
	for {
		...
		mut var_style_pointer := var_styles.clone()
		mut iter_5 := var_style_mappings.array_get(var_property).iterator()
		for {
			item_5 := iter_5.next() or { break }
			mut var_path_segment := item_5.val
			if !(var_style_pointer.array_isset(var_path_segment)) {
				continue
			}
		var_style_pointer = var_style_pointer.array_get(var_path_segment)
		}
		var_filtered_styles_pointer = var_filtered_styles
		mut iter_6 := var_style_mappings.array_get(var_property).iterator()
		for {
			item_6 := iter_6.next() or { break }
			mut var_path_segment := item_6.val
			mut var_path_index := item_6.key
			if rt.is_true(rt.identical(var_style_mappings.array_get(var_property).array_count() - 1, var_path_index)) {
				var_filtered_styles_pointer.array_set(var_path_segment, var_style_pointer.clone())
				break
			}
			if !(var_filtered_styles_pointer.array_isset(var_path_segment)) || !(var_filtered_styles_pointer.array_get(var_path_segment).is_array()) {
				var_filtered_styles_pointer.array_set(var_path_segment, rt.new_array())
			}
			var_filtered_styles_pointer = var_filtered_styles_pointer.array_get(var_path_segment)
		}
	}
	mut var_additional_css_declarations := rt.call_function('array_filter', [rt.call_function('array_intersect_key', [rt.create_array([rt.ArrayItem{ key: 'text-align', val: if !(var_block_attributes.array_get(rt.new_string('textAlign'))).is_null() { var_block_attributes.array_get(rt.new_string('textAlign')) } else { rt.new_null() } }]), rt.call_function('array_flip', [var_properties])])])
	var_styles = if var_filtered_styles.clone().array_count() > 0 { Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.get_styles_from_block(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_array](var_filtered_styles)) } else { rt.get_static_prop('Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper', 'empty_block_styles') }
...
fn Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.convert_to_px(input string, use_fallback bool, mut var_base_font_size Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_?int) string {
	...
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/[<>"\']/'), rt.new_string(input_mutated).clone()])) {
		return (var_fallback).str()
	}
	if rt.is_true(rt.call_function('str_ends_with', [rt.new_string(input_mutated).clone(), rt.new_string('px')])) {
		return input_mutated
	}
	if rt.is_true(rt.call_function('str_ends_with', [rt.new_string(input_mutated).clone(), rt.new_string('rem')])) || rt.is_true(rt.call_function('str_ends_with', [rt.new_string(input_mutated).clone(), rt.new_string('em')])) {
		mut var_value := rt.new_float((rt.call_function('str_replace', [rt.create_array([rt.ArrayItem{ key: none, val: 'rem' }, rt.ArrayItem{ key: none, val: 'em' }]), rt.new_string(''), rt.new_string(input_mutated).clone()])).to_f64())
		return (rt.call_function('round', [rt.mul(var_value, var_base_font_size)])).str() + 'px'
	}
	if rt.is_true(rt.call_function('str_ends_with', [rt.new_string(input_mutated).clone(), rt.new_string('%')])) {
		var_value = rt.new_float((rt.call_function('str_replace', [rt.new_string('%'), rt.new_string(''), rt.new_string(input_mutated).clone()])).to_f64())
		return (rt.call_function('round', [rt.mul(rt.div(var_value, rt.new_int(100)), var_base_font_size)])).str() + 'px'
	}
	if rt.is_true(rt.new_bool(rt.new_string(input_mutated).clone().is_long() || rt.new_string(input_mutated).clone().is_double())) {
...
fn Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.remove_css_unit(input string) string {
	...
	return (rt.call_function('str_ireplace', [var_units.clone(), rt.new_string(''), rt.new_string(input_mutated).clone()])).str()
}
...
fn Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.clamp_to_static_px(var_clamp_str rt.PhpVal, strategy string) string {
	if rt.is_true(rt.identical(rt.call_function('stripos', [var_clamp_str.clone(), rt.new_string('clamp(')]), rt.new_bool(false))) {
		return (var_clamp_str).str()
	}
	mut var_value_array := rt.call_function('explode', [rt.new_string(','), var_clamp_str.clone()])
	if var_value_array.clone().array_count() < 2 {
		return (var_clamp_str).str()
	}
	mut var_first_element := var_value_array.array_get(rt.new_int(0))
	mut var_min := rt.new_string(rt.call_function('str_ireplace', [rt.create_array([rt.ArrayItem{ key: none, val: 'clamp(' }, rt.ArrayItem{ key: none, val: 'min(' }, rt.ArrayItem{ key: none, val: 'max(' }]), rt.new_string(''), var_first_element.clone()]).to_string().trim_space())
	mut var_last_element := var_value_array.array_get(rt.new_int(var_value_array.clone().array_count() - 1))
	mut var_max := rt.new_string(var_last_element.clone().to_string().trim_right(' \t\n\r').trim_space())
	...
	if rt.is_true(rt.identical(rt.new_string('min'), rt.new_string(strategy))) && !(var_min_px.clone().is_null()) {
		return (var_min_px).str()
	}
	if rt.is_true(rt.identical(rt.new_string('max'), rt.new_string(strategy))) && !(var_max_px.clone().is_null()) {
		return (var_max_px).str()
	}
	if rt.is_true(rt.identical(rt.new_string('avg'), rt.new_string(strategy))) && !(var_min_px.clone().is_null()) && !(var_max_px.clone().is_null()) {
		mut var_avg := rt.div(rt.add(Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.parse_value(var_min_px.clone()), Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper.parse_value(var_max_px.clone())), rt.new_int(2))
