...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller) construct() {
	...
	mut iife_result_0 := iife_temp_0.get_core_data()
	this.core_theme = iife_result_0
	this.base_theme = create_wp_theme_json(rt.cast_array(rt.call_function('json_decode', [rt.new_string((rt.call_function('file_get_contents', [rt.new_string(@DIR + '/theme.json')])).str()), rt.new_bool(true)])), rt.new_string('default'))
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller) get_base_theme() rt.PhpVal {
	...
	if rt.is_true(rt.call_method(this.site_style_sync_controller, 'is_sync_enabled', []rt.PhpVal{})) {
		mut var_site_theme := rt.call_method(this.site_style_sync_controller, 'get_theme', [var_theme.clone()])
		rt.call_method(var_theme, 'merge', [var_site_theme.clone()])
	}
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_email_editor_theme_json'), var_theme.clone()])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller) recursive_replace_presets(var_values rt.PhpVal, var_presets rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.new_bool(var_value.clone().is_array())) {
			var_values_mutated.array_set(var_key, this.recursive_replace_presets(var_value.clone(), var_presets_mutated.clone()))
		} else if rt.is_true(rt.new_bool(var_value.clone().is_string())) {
			var_values_mutated.array_set(var_key, rt.call_function('preg_replace', [rt.func_array_keys(var_presets_mutated.clone()), rt.call_function('array_values', [var_presets_mutated.clone()]), var_value.clone()]))
		} else {
			var_values_mutated.array_set(var_key, var_value.clone())
		}
	}
	return var_values_mutated.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller) recursive_extract_preset_variables(var_styles rt.PhpVal) rt.PhpVal {
	...
	mut iter_2 := var_styles_mutated.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_style_value := item_2.val
		mut var_key := item_2.key
		if rt.is_true(rt.new_bool(var_style_value.clone().is_array())) {
			var_styles_mutated.array_set(var_key, this.recursive_extract_preset_variables(var_style_value.clone()))
		mut iife_temp_1 := Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Preset_Variable_Resolver{}
		mut iife_result_1 := iife_temp_1.is_preset_reference(var_style_value.clone())
		} else if var_style_value.clone().is_string() && rt.is_true(iife_result_1) {
			mut iife_temp_2 := Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Preset_Variable_Resolver{}
			mut iife_result_2 := iife_temp_2.to_css_var(var_style_value.clone())
			var_styles_mutated.array_set(var_key, iife_result_2)
		} else {
			var_styles_mutated.array_set(var_key, var_style_value.clone())
		}
	}
	return var_styles_mutated.clone()
}

fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller) get_styles() rt.PhpVal {
	mut var_theme_styles := rt.call_method(this.get_theme(), 'get_data', []rt.PhpVal{}).array_get(rt.new_string('styles'))
	var_theme_styles = this.recursive_extract_preset_variables(var_theme_styles.clone())
	mut var_variables := this.get_variables_values_map()
	mut var_presets := rt.new_array()
	mut iter_3 := var_variables.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_value := item_3.val
		mut var_name := item_3.key
		mut var_pattern := rt.new_string('/var\\(' + (rt.call_function('preg_quote', [var_name.clone(), rt.new_string('/')])).str() + '\\)/i')
		var_presets.array_set(var_pattern, var_value.clone())
	}
	return this.recursive_replace_presets(var_theme_styles.clone(), var_presets.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller) get_layout_settings() rt.PhpVal {
	return rt.call_method(this.get_theme(), 'get_settings', []rt.PhpVal{}).array_get(rt.new_string('layout'))
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller) get_stylesheet_for_rendering(mut var_post Class_Automattic_WooCommerce_EmailEditor_Engine_?WP_Post, var_template rt.PhpVal) string {
	...
	mut var_css_presets := rt.new_string('')
	mut iter_4 := var_email_theme_settings.array_get(rt.new_string('typography')).array_get(rt.new_string('fontFamilies')).array_get(rt.new_string('default')).iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_font_family := item_4.val
		var_css_presets = rt.concat(var_css_presets, rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('.has-'), var_font_family.array_get(rt.new_string('slug'))), rt.new_string('-font-family { font-family: ')), var_font_family.array_get(rt.new_string('fontFamily'))), rt.new_string('; } \n')))
	}
	mut iter_5 := var_email_theme_settings.array_get(rt.new_string('typography')).array_get(rt.new_string('fontSizes')).array_get(rt.new_string('default')).iterator()
	for {
		item_5 := iter_5.next() or { break }
		mut var_font_size := item_5.val
		var_css_presets = rt.concat(var_css_presets, rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('.has-'), var_font_size.array_get(rt.new_string('slug'))), rt.new_string('-font-size { font-size: ')), var_font_size.array_get(rt.new_string('size'))), rt.new_string('; } \n')))
	}
	mut var_color_definitions := rt.call_function('array_merge', [if !(var_email_theme_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('theme'))).is_null() { var_email_theme_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('theme')) } else { rt.new_array() }, if !(var_email_theme_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('default'))).is_null() { var_email_theme_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('default')) } else { rt.new_array() }])
	mut iter_6 := var_color_definitions.iterator()
	for {
		item_6 := iter_6.next() or { break }
		mut var_color := item_6.val
		var_css_presets = rt.concat(var_css_presets, rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('.has-'), var_color.array_get(rt.new_string('slug'))), rt.new_string('-color { color: ')), var_color.array_get(rt.new_string('color'))), rt.new_string('; } \n')))
		var_css_presets = rt.concat(var_css_presets, rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('.has-'), var_color.array_get(rt.new_string('slug'))), rt.new_string('-background-color { background-color: ')), var_color.array_get(rt.new_string('color'))), rt.new_string('; } \n')))
		var_css_presets = rt.concat(var_css_presets, rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('.has-'), var_color.array_get(rt.new_string('slug'))), rt.new_string('-border-color { border-color: ')), var_color.array_get(rt.new_string('color'))), rt.new_string('; } \n')))
	}
	mut var_css_blocks := rt.new_string('')
	mut var_blocks := rt.call_method(this.get_theme(), 'get_styles_block_nodes', []rt.PhpVal{})
	mut iter_7 := var_blocks.iterator()
	for {
		item_7 := iter_7.next() or { break }
		mut var_block_metadata := item_7.val
		var_css_blocks = rt.concat(var_css_blocks, rt.call_method(this.get_theme(), 'get_styles_for_block', [var_block_metadata.clone()]))
	}
	var_css_blocks = rt.call_function('preg_replace', [rt.new_string('/:root\\s:where\\((.*?)\\)/'), rt.new_string('$1'), var_css_blocks.clone()])
	mut var_elements_styles := if !(rt.call_method(this.get_theme(), 'get_raw_data', []rt.PhpVal{}).array_get(rt.new_string('styles')).array_get(rt.new_string('elements'))).is_null() { rt.call_method(this.get_theme(), 'get_raw_data', []rt.PhpVal{}).array_get(rt.new_string('styles')).array_get(rt.new_string('elements')) } else { rt.new_array() }
	...
	mut var_template_styles := rt.cast_array(if !(var_template_theme.array_get(rt.new_string('styles'))).is_null() { var_template_theme.array_get(rt.new_string('styles')) } else { rt.new_array() })
	mut var_template_elements := if !(var_template_styles.array_get(rt.new_string('elements'))).is_null() { var_template_styles.array_get(rt.new_string('elements')) } else { rt.new_array() }
	...
	mut var_post_styles := rt.cast_array(if !(var_post_theme.array_get(rt.new_string('styles'))).is_null() { var_post_theme.array_get(rt.new_string('styles')) } else { rt.new_array() })
	mut var_post_elements := if !(var_post_styles.array_get(rt.new_string('elements'))).is_null() { var_post_styles.array_get(rt.new_string('elements')) } else { rt.new_array() }
	...
	mut var_css_elements := rt.new_string('')
	mut iter_8 := var_elements_styles.iterator()
	for {
		item_8 := iter_8.next() or { break }
		mut var_elements_style := item_8.val
		mut var_key := item_8.key
		mut var_selector := var_key
		if rt.is_true(rt.identical(rt.new_string('button'), var_key)) {
			var_selector = rt.new_string('.wp-block-button')
			var_css_elements = rt.concat(var_css_elements, if !(rt.call_function('wp_style_engine_get_styles', [var_elements_style.clone(), rt.create_array([rt.ArrayItem{ key: 'selector', val: '.wp-block-button' }])]).array_get(rt.new_string('css'))).is_null() { rt.call_function('wp_style_engine_get_styles', [var_elements_style.clone(), rt.create_array([rt.ArrayItem{ key: 'selector', val: '.wp-block-button' }])]).array_get(rt.new_string('css')) } else { rt.new_string('') })
			var_css_elements = rt.concat(var_css_elements, if !(rt.call_function('wp_style_engine_get_styles', [rt.create_array([rt.ArrayItem{ key: 'color', val: rt.create_array([rt.ArrayItem{ key: 'text', val: if !(var_elements_style.array_get(rt.new_string('color')).array_get(rt.new_string('text'))).is_null() { var_elements_style.array_get(rt.new_string('color')).array_get(rt.new_string('text')) } else { rt.new_string('') } }]) }]), rt.create_array([rt.ArrayItem{ key: 'selector', val: '.wp-block-button a' }])]).array_get(rt.new_string('css'))).is_null() { rt.call_function('wp_style_engine_get_styles', [rt.create_array([rt.ArrayItem{ key: 'color', val: rt.create_array([rt.ArrayItem{ key: 'text', val: if !(var_elements_style.array_get(rt.new_string('color')).array_get(rt.new_string('text'))).is_null() { var_elements_style.array_get(rt.new_string('color')).array_get(rt.new_string('text')) } else { rt.new_string('') } }]) }]), rt.create_array([rt.ArrayItem{ key: 'selector', val: '.wp-block-button a' }])]).array_get(rt.new_string('css')) } else { rt.new_string('') })
			continue
		}
		mut switch_val_1 := var_key
		if rt.is_true(rt.equal(switch_val_1, rt.new_string('heading'))) {
		var_selector = rt.new_string('h1, h2, h3, h4, h5, h6')
		} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('link'))) {
		var_selector = rt.new_string('a:not(.button-link)')
		}
		var_css_elements = rt.concat(var_css_elements, if !(rt.call_function('wp_style_engine_get_styles', [var_elements_style.clone(), rt.create_array([rt.ArrayItem{ key: 'selector', val: var_selector }])]).array_get(rt.new_string('css'))).is_null() { rt.call_function('wp_style_engine_get_styles', [var_elements_style.clone(), rt.create_array([rt.ArrayItem{ key: 'selector', val: var_selector }])]).array_get(rt.new_string('css')) } else { rt.new_string('') })
	}
	mut var_result := rt.new_string((var_css_presets).str() + (var_css_blocks).str() + (var_css_elements).str())
	mut var_pattern := rt.new_string('/clamp\\([^,]+,\\s*[^,]+,\\s*([^)]+)\\)/')
	var_result = rt.new_string((rt.call_function('preg_replace', [var_pattern.clone(), rt.new_string('$1'), var_result.clone()])).str())
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller) translate_slug_to_font_size(font_size string) string {
	...
	mut iter_9 := var_settings.array_get(rt.new_string('typography')).array_get(rt.new_string('fontSizes')).array_get(rt.new_string('default')).iterator()
	for {
		item_9 := iter_9.next() or { break }
		mut var_font_size_definition := item_9.val
		if rt.is_true(rt.identical(var_font_size_definition.array_get(rt.new_string('slug')), rt.new_string(font_size))) {
			return (var_font_size_definition.array_get(rt.new_string('size'))).str()
		}
	}
	return font_size
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller) translate_slug_to_color(color_slug string) string {
	mut var_settings := this.get_settings()
	mut var_color_definitions := rt.call_function('array_merge', [if !(var_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('theme'))).is_null() { var_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('theme')) } else { rt.new_array() }, if !(var_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('default'))).is_null() { var_settings.array_get(rt.new_string('color')).array_get(rt.new_string('palette')).array_get(rt.new_string('default')) } else { rt.new_array() }])
	mut iter_10 := var_color_definitions.iterator()
	for {
		item_10 := iter_10.next() or { break }
		mut var_color_definition := item_10.val
		if rt.is_true(rt.identical(var_color_definition.array_get(rt.new_string('slug')), rt.new_string(color_slug))) {
			return var_color_definition.array_get(rt.new_string('color')).to_string().to_lower()
		}
	}
	return color_slug
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller) get_variables_values_map() rt.PhpVal {
	...
	mut var_pattern := rt.new_string('/--(.*?):\\s*(.*?);/')
	if rt.is_true(rt.call_function('preg_match_all', [var_pattern.clone(), var_variables_css.clone(), var_matches.clone(), rt.get_constant('PREG_SET_ORDER')])) {
		mut iter_11 := var_matches.iterator()
		for {
			item_11 := iter_11.next() or { break }
			mut var_match := item_11.val
			var_map.array_set('--' + (var_match.array_get(rt.new_int(1))).str(), var_match.array_get(rt.new_int(2)))
		}
	}
