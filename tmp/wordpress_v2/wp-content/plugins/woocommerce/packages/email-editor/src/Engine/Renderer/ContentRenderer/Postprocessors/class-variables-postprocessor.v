...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Postprocessors_Variables_Postprocessor) postprocess(html string) string {
	mut var_variables := rt.call_method(this.theme_controller, 'get_variables_values_map',
		[]rt.PhpVal{})
	...
	for {
		...
		mut var_var_pattern := rt.new_string('/' +
			(rt.call_function('preg_quote', [rt.new_string('var(' + var_name.str() +
			')'), rt.new_string('/')])).str() + '/i')
		var_replacements.array_set(var_var_pattern, var_value.clone())
	}
	mut var_processor :=
	...
	for rt.is_true(var_processor.next_tag()) {
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_style))))
			...
			mut var_processed_style := rt.call_function('preg_replace', [
				rt.func_array_keys(var_replacements.clone()),
				rt.call_function('array_values', [var_replacements.clone()]),
				var_style.clone(),
			])
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_processed_style)))) {
