...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Post_Content) render_stateless(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_email_post := rt.call_function('get_post', [var_post_id.clone()])
	...
	mut var_backup_post := var_post.clone()
	mut var_backup_query := var_wp_query.clone()
	var_post = var_email_post.clone()
	var_wp_query = create_automattic_woocommerce_emaileditor_integrations_core_renderer_blocks_wp_query(rt.create_array([
	...
	if rt.is_true(rt.call_function('has_block', [rt.new_string('core/nextpage'),
		var_email_post.clone()]))
	{
		var_post_content = rt.concat(var_post_content, rt.call_function('wp_link_pages', [
	...
	var_post_content = rt.call_function('apply_filters', [rt.new_string('the_content'),
		rt.call_function('str_replace', [rt.new_string(']]>'),
			rt.new_string(']]&gt;'), var_post_content.clone()])])
	var_post = var_backup_post.clone()
	var_wp_query = var_backup_query.clone()
...
fn create_automattic_woocommerce_emaileditor_integrations_core_renderer_blocks_wp_query(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_WP_Query {
