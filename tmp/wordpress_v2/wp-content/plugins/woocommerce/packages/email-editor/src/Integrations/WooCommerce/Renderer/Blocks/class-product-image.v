...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) render_content(block_content string, mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) string {
	...
	mut var_attributes := this.parse_attributes(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array](if !(var_parsed_block_mutated.array_get(rt.new_string('attrs'))).is_null() { var_parsed_block_mutated.array_get(rt.new_string('attrs')) } else { rt.new_array() }))
	...
	var_attributes = this.parse_attributes(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array](if !(var_parsed_block_mutated.array_get(rt.new_string('attrs'))).is_null() { var_parsed_block_mutated.array_get(rt.new_string('attrs')) } else { rt.new_array() }))
	...
	mut var_combined_content := rt.new_string(this.create_overlay_structure((var_image_html).str(), (var_inner_blocks.array_get(rt.new_string('badges'))).str(), (var_inner_blocks.array_get(rt.new_string('other_content'))).str(), (var_inner_blocks.array_get(rt.new_string('badge_alignment'))).str(), mut rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_?WC_Product](var_product), (var_attributes.array_get(rt.new_string('showProductLink'))).to_bool()))
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) process_inner_blocks(mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array, mut var_product Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_WC_Product, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) rt.PhpVal {
	...
	mut var_badges := rt.new_string('')
	mut var_other_content := rt.new_string('')
	mut var_badge_alignment := rt.new_string('left')
	if !(!rt.is_true(var_parsed_block_mutated.array_get(rt.new_string('innerBlocks')))) {
		mut iter_1 := var_parsed_block_mutated.array_get(rt.new_string('innerBlocks')).iterator()
		for {
			...
			if rt.is_true(rt.identical(rt.new_string('woocommerce/product-sale-badge'), var_inner_block.array_get(rt.new_string('blockName')))) {
				var_badges = rt.concat(var_badges, this.render_overlay_badge(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array](var_inner_block), mut var_product_mutated, mut var_rendering_context))
			var_badge_alignment = if !(var_inner_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('align'))).is_null() { var_inner_block.array_get(rt.new_string('attrs')).array_get(rt.new_string('align')) } else { rt.new_string('left') }
			} else {
				var_other_content = rt.concat(var_other_content, rt.call_function('render_block', [var_inner_block.clone()]))
			}
		}
	}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) render_overlay_badge(mut var_badge_block Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array, mut var_product Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_WC_Product, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) string {
	...
	mut iife_result_1 := iife_temp_1.extend_block_styles(var_block_styles.clone(), var_additional_styles.clone())
	mut var_final_styles := iife_result_1
	return (rt.call_function('sprintf', [rt.new_string('<span class="wc-block-components-product-sale-badge__text" style="%s">%s</span>'), rt.call_function('esc_attr', [var_final_styles.array_get(rt.new_string('css'))]), rt.call_function('esc_html', [var_sale_text.clone()])])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) create_overlay_structure(image_html string, badges_html string, other_content string, badge_alignment string, mut var_product Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_?WC_Product, show_product_link bool) string {
	...
	mut var_product_mutated := var_product
	if badges_html == '' {
		mut var_linked_image_html := rt.new_string(image_html_mutated).clone()
		if var_show_product_link && rt.is_true(var_product_mutated) {
	...
	var_linked_image_html = rt.new_string(image_html_mutated).clone()
	if var_show_product_link && rt.is_true(var_product_mutated) {
	var_linked_image_html = rt.new_string(this.wrap_with_link(image_html_mutated, mut var_product_mutated))
	}
	mut var_vml_side := rt.new_string((if rt.is_true(rt.identical(rt.new_string('left'), rt.new_string(badge_alignment_mutated))) { 'left' } else { 'right' }).str())
	mut var_overlay_html := rt.call_function('sprintf', [rt.new_string('<table cellpadding="0" cellspacing="0" border="0" style="width: %dpx; height: %dpx; table-layout: fixed;">\n\t\t\t\t<tr>\n\t\t\t\t\t<td style="font-size: 0; line-height: 0; padding: 0; height: %dpx; width: %dpx;">\n\t\t\t\t\t<div style="max-height:0; position:relative; opacity:0.999;">\n\t\t\t\t\t\t<!--[if mso]>\n\t\t\t\t\t\t<v:rect xmlns:v="urn:schemas-microsoft-com:vml" stroked="false" filled="false" style="mso-width-percent: 1000; position:absolute; top:16px; ' + (rt.call_function('esc_attr', [var_vml_side.clone()])).str() + ':16px;">\n\t\t\t\t\t\t<v:textbox inset="0,0,0,0">\n\t\t\t\t\t\t<![endif]-->\n\t\t\t\t\t\t<div style="padding: 12px; box-sizing: border-box; display: inline-block; width: 100%%; text-align: %s;">\n\t\t\t\t\t\t\t%s\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<!--[if mso]>\n\t\t\t\t\t\t</v:textbox>\n\t\t\t\t\t\t</v:rect>\n\t\t\t\t\t\t<![endif]-->\n\t\t\t\t\t</div>\n\t\t\t\t\t\t%s\n\t\t\t\t\t</td>\n\t\t\t\t</tr>\n\t\t\t</table>%s'), var_image_width.clone(), var_image_height.clone(), var_image_height.clone(), var_image_width.clone(), rt.new_string(badge_alignment_mutated).clone(), rt.new_string(badges_html), var_linked_image_html.clone(), rt.new_string(other_content_mutated).clone()])
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) extract_image_width(image_html string) f64 {
	mut image_html_mutated := image_html
	mut var_width := if !(rt.call_method(create_automattic_woocommerce_emaileditor_integrations_utils_dom_document_helper(rt.new_string(image_html_mutated).clone()), 'get_attribute_value_by_tag_name', [rt.new_string('img'), rt.new_string('width')])).is_null() { rt.call_method(create_automattic_woocommerce_emaileditor_integrations_utils_dom_document_helper(rt.new_string(image_html_mutated).clone()), 'get_attribute_value_by_tag_name', [rt.new_string('img'), rt.new_string('width')]) } else { rt.new_string('') }
	if rt.is_true(var_width) {
		mut iife_temp_2 := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper{}
		mut iife_result_2 := iife_temp_2.parse_value(var_width.clone())
		return (iife_result_2).to_f64()
	}
	return 300
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) extract_image_height(image_html string) f64 {
	mut image_html_mutated := image_html
	mut var_height := if !(rt.call_method(create_automattic_woocommerce_emaileditor_integrations_utils_dom_document_helper(rt.new_string(image_html_mutated).clone()), 'get_attribute_value_by_tag_name', [rt.new_string('img'), rt.new_string('height')])).is_null() { rt.call_method(create_automattic_woocommerce_emaileditor_integrations_utils_dom_document_helper(rt.new_string(image_html_mutated).clone()), 'get_attribute_value_by_tag_name', [rt.new_string('img'), rt.new_string('height')]) } else { rt.new_string('') }
	if rt.is_true(var_height) {
		mut iife_temp_3 := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper{}
		mut iife_result_3 := iife_temp_3.parse_value(var_height.clone())
		return (iife_result_3).to_f64()
	}
	return 300
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) add_image_size_when_missing(mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_string('full'), var_align)) {
		mut var_layout_settings := if !(var_rendering_context.get_theme_settings().array_get(rt.new_string('layout'))).is_null() { var_rendering_context.get_theme_settings().array_get(rt.new_string('layout')) } else { rt.new_array() }
		var_parsed_block_mutated.array_get_mut('attrs').array_set('width', if !(var_layout_settings.array_get(rt.new_string('contentSize'))).is_null() { var_layout_settings.array_get(rt.new_string('contentSize')) } else { rt.new_string('100%') })
		return rt.new_object('Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array', []string{}, var_parsed_block_mutated)
	}
	...
	var_parsed_block_mutated.array_get_mut('attrs').array_set('width', var_parsed_block_mutated.array_get(rt.new_string('email_attrs')).array_get(rt.new_string('width')))
	return rt.new_object('Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array', []string{}, var_parsed_block_mutated)
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) parse_attributes(mut var_attributes Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array) rt.PhpVal {
	...
	return rt.call_function('wp_parse_args', [var_attributes_mutated, rt.create_array([rt.ArrayItem{ key: 'showProductLink', val: true }, rt.ArrayItem{ key: 'imageSizing', val: 'single' }, rt.ArrayItem{ key: 'scale', val: 'cover' }, rt.ArrayItem{ key: 'showSaleBadge', val: false }, rt.ArrayItem{ key: 'saleBadgeAlign', val: 'right' }])])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) get_product_image_data(mut var_product Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_WC_Product, mut var_attributes Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array) rt.PhpVal {
	mut var_product_mutated := var_product
	mut var_attributes_mutated := var_attributes
	mut var_image_size := rt.new_string((if rt.is_true(rt.identical(rt.new_string('single'), var_attributes_mutated.array_get(rt.new_string('imageSizing')))) { 'woocommerce_single' } else { 'woocommerce_thumbnail' }).str())
	mut var_image_id := rt.new_int((rt.call_method(var_product_mutated, 'get_image_id', []rt.PhpVal{})).to_i64())
	if rt.is_true(rt.new_bool(!(rt.is_true(var_image_id)))) {
		mut var_placeholder := rt.call_function('wc_placeholder_img_src', [var_image_size.clone()])
	...
	mut var_image_url := rt.call_function('wp_get_attachment_image_url', [var_image_id.clone(), var_image_size.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_image_url)))) {
		return rt.new_null()
	}
	mut var_alt_text := rt.call_function('get_post_meta', [var_image_id.clone(), rt.new_string('_wp_attachment_image_alt'), rt.new_bool(true)])
	mut var_image_meta := rt.call_function('wp_get_attachment_metadata', [var_image_id.clone()])
	return rt.create_array([rt.ArrayItem{ key: 'url', val: var_image_url }, rt.ArrayItem{ key: 'alt', val: if rt.is_true(var_alt_text) { var_alt_text } else { rt.call_method(var_product_mutated, 'get_name', []rt.PhpVal{}) } }, rt.ArrayItem{ key: 'width', val: if !(var_image_meta.array_get(rt.new_string('width'))).is_null() { var_image_meta.array_get(rt.new_string('width')) } else { rt.new_int(300) } }, rt.ArrayItem{ key: 'height', val: if !(var_image_meta.array_get(rt.new_string('height'))).is_null() { var_image_meta.array_get(rt.new_string('height')) } else { rt.new_int(300) } }])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) build_image_html(mut var_image_data Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array, mut var_attributes Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) string {
	...
	if !(!rt.is_true(var_attributes_mutated.array_get(rt.new_string('scale')))) {
		var_style_parts.array_set('object-fit', var_attributes_mutated.array_get(rt.new_string('scale')))
	}
	if !(!rt.is_true(var_attributes_mutated.array_get(rt.new_string('width')))) {
		var_style_parts.array_set('width', var_attributes_mutated.array_get(rt.new_string('width')))
	}
	if !(!rt.is_true(var_attributes_mutated.array_get(rt.new_string('height')))) {
		var_style_parts.array_set('height', var_attributes_mutated.array_get(rt.new_string('height')))
	}
	if !(!rt.is_true(var_attributes_mutated.array_get(rt.new_string('aspectRatio')))) {
		var_style_parts.array_set('aspect-ratio', var_attributes_mutated.array_get(rt.new_string('aspectRatio')))
	}
	mut iife_temp_4 := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper{}
	mut iife_result_4 := iife_temp_4.parse_value(var_attributes_mutated.array_get(rt.new_string('width')))
	mut var_width := if !(!rt.is_true(var_attributes_mutated.array_get(rt.new_string('width')))) { iife_result_4 } else { var_image_data_mutated.array_get(rt.new_string('width')) }
	mut iife_temp_5 := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper{}
	mut iife_result_5 := iife_temp_5.parse_value(var_rendering_context.get_layout_width_without_padding())
	mut var_layout_width := iife_result_5
	if rt.is_true(rt.greater(var_width, var_layout_width)) {
		var_width = var_layout_width.clone()
		mut var_aspect_ratio := rt.div(var_image_data_mutated.array_get(rt.new_string('height')), var_image_data_mutated.array_get(rt.new_string('width')))
		var_attributes_mutated.array_set('height', (rt.call_function('round', [rt.mul(var_width, var_aspect_ratio)])).str() + 'px')
	}
	mut var_height := var_image_data_mutated.array_get(rt.new_string('height'))
	if !(!rt.is_true(var_attributes_mutated.array_get(rt.new_string('height')))) {
	mut iife_temp_6 := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Styles_Helper{}
	mut iife_result_6 := iife_temp_6.parse_value(var_attributes_mutated.array_get(rt.new_string('height')))
	var_height = iife_result_6
	} else if !(!rt.is_true(var_attributes_mutated.array_get(rt.new_string('width')))) && rt.is_true(rt.greater(var_image_data_mutated.array_get(rt.new_string('width')), rt.new_int(0))) {
	var_aspect_ratio = rt.div(var_image_data_mutated.array_get(rt.new_string('height')), var_image_data_mutated.array_get(rt.new_string('width')))
	var_height = rt.call_function('round', [rt.mul(var_width, var_aspect_ratio)])
	}
	mut iife_temp_7 := Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_WP_Style_Engine{}
	mut iife_result_7 := iife_temp_7.compile_css(var_style_parts.clone(), rt.new_string(''))
	mut iife_temp_8 := Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_WP_Style_Engine{}
	mut iife_result_8 := iife_temp_8.compile_css(var_style_parts.clone(), rt.new_string(''))
	return (rt.call_function('sprintf', [rt.new_string('<img class="email-editor-product-image skip-lazy" data-skip-lazy="1" loading="eager" decoding="auto" src="%s" alt="%s" style="%s" width="%d" height="%d" />'), rt.call_function('esc_url', [var_image_data_mutated.array_get(rt.new_string('url'))]), rt.call_function('esc_attr', [var_image_data_mutated.array_get(rt.new_string('alt'))]), rt.call_function('esc_attr', [iife_result_7]), var_width.clone(), var_height.clone()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) wrap_with_link(image_html string, mut var_product Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_WC_Product) string {
	...
	return (rt.call_function('sprintf', [rt.new_string('<a href="%s" style="display: block; text-decoration: none;">%s</a>'), rt.call_function('esc_url', [var_product_url.clone()]), rt.new_string(image_html_mutated).clone()])).str()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Product_Image) apply_email_wrapper(image_html string, mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) string {
	mut image_html_mutated := image_html
	mut var_parsed_block_mutated := var_parsed_block
	mut var_width := if !(var_parsed_block_mutated.array_get(rt.new_string('attrs')).array_get(rt.new_string('width'))).is_null() { var_parsed_block_mutated.array_get(rt.new_string('attrs')).array_get(rt.new_string('width')) } else { rt.new_string('') }
	mut var_align := if !(var_parsed_block_mutated.array_get(rt.new_string('attrs')).array_get(rt.new_string('align'))).is_null() { var_parsed_block_mutated.array_get(rt.new_string('attrs')).array_get(rt.new_string('align')) } else { rt.new_string('') }
	mut var_is_full := rt.identical(rt.new_string('full'), var_align)
	mut var_wrapper_width := if rt.is_true(var_is_full) { rt.new_string('100%') } else { if rt.is_true(var_width) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('100%'), var_width)))) { var_width } else { rt.new_string('auto') } }
	...
	mut iife_result_9 := iife_temp_9.get_block_styles(if !(var_parsed_block_mutated.array_get(rt.new_string('attrs'))).is_null() { var_parsed_block_mutated.array_get(rt.new_string('attrs')) } else { rt.new_array() }, rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context', []string{}, var_rendering_context), rt.create_array([rt.ArrayItem{ key: none, val: 'spacing' }]))
	mut var_padding_styles := iife_result_9
	if !(!rt.is_true(var_padding_styles.array_get(rt.new_string('declarations')))) {
	var_cell_styles = rt.call_function('array_merge', [var_cell_styles.clone(), var_padding_styles.array_get(rt.new_string('declarations'))])
	}
	var_cell_styles.array_set('text-align', var_css_align.clone())
	mut iife_temp_10 := Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_WP_Style_Engine{}
	mut iife_result_10 := iife_temp_10.compile_css(rt.create_array([rt.ArrayItem{ key: 'border-collapse', val: 'collapse' }, rt.ArrayItem{ key: 'border-spacing', val: '0px' }, rt.ArrayItem{ key: 'width', val: '100%' }, rt.ArrayItem{ key: 'height', val: var_image_height }]), rt.new_string(''))
	mut var_outer_table_attrs := rt.create_array([rt.ArrayItem{ key: 'style', val: iife_result_10 }, rt.ArrayItem{ key: 'width', val: '100%' }])
	mut var_outer_cell_attrs := rt.create_array([rt.ArrayItem{ key: 'align', val: var_css_align }])
	mut iife_temp_11 := Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_WP_Style_Engine{}
	mut iife_result_11 := iife_temp_11.compile_css(var_wrapper_styles.clone(), rt.new_string(''))
	mut var_inner_table_attrs := rt.create_array([rt.ArrayItem{ key: 'style', val: iife_result_11 }, rt.ArrayItem{ key: 'width', val: var_wrapper_width }, rt.ArrayItem{ key: 'height', val: var_image_height }])
	mut iife_temp_12 := Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_WP_Style_Engine{}
	mut iife_result_12 := iife_temp_12.compile_css(var_cell_styles.clone(), rt.new_string(''))
	mut var_inner_cell_attrs := rt.create_array([rt.ArrayItem{ key: 'style', val: iife_result_12 }])
	mut iife_temp_13 := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Table_Wrapper_Helper{}
	mut iife_result_13 := iife_temp_13.render_table_wrapper(rt.new_string(image_html_mutated), var_inner_table_attrs.clone(), var_inner_cell_attrs.clone())
	mut var_inner_content := iife_result_13
	mut iife_temp_14 := Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Table_Wrapper_Helper{}
	mut iife_result_14 := iife_temp_14.render_table_wrapper(var_inner_content.clone(), var_outer_table_attrs.clone(), var_outer_cell_attrs.clone())
