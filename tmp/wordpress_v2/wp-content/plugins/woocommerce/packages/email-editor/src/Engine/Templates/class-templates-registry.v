...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates_Registry) register(mut var_template Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Template) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(iife_result_0, 'is_registered', [
	...
	])))))
	{
		...
		this.templates.array_set(var_template.get_name(), var_template)
	}
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_Templates_Registry) get_by_name(name string) rt.PhpVal {
	return if !(this.templates.array_get(rt.new_string(name))).is_null() {
		this.templates.array_get(rt.new_string(name))
	} else {
		rt.new_null()
	}
}
...
fn create_automattic_woocommerce_emaileditor_engine_templates_wp_block_templates_registry(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_EmailEditor_Engine_Templates_WP_Block_Templates_Registry {
