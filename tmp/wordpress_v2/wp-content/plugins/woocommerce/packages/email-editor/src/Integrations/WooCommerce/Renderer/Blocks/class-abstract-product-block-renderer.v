...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_Abstract_Product_Block_Renderer) get_product_from_context(mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_WooCommerce_Renderer_Blocks_array) rt.PhpVal {
	...
	mut var_post_id := if !(var_parsed_block.array_get(rt.new_string('context')).array_get(rt.new_string('postId'))).is_null() {
		var_parsed_block.array_get(rt.new_string('context')).array_get(rt.new_string('postId'))
	} else {
		rt.new_int(0)
	}
	...
	var_product = if rt.is_true(var_post_id) { rt.call_function('wc_get_product', [
			var_post_id.clone(),
		]) } else { rt.new_null() }
