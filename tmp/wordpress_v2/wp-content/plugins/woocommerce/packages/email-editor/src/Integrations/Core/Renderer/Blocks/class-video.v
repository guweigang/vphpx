...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Video) render_content(block_content string, mut var_parsed_block Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_array, mut var_rendering_context Class_Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context) string {
	...
	mut var_poster_url := rt.new_string(this.extract_poster_url(mut rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_array](var_block_attrs),
		block_content_mutated))
	if !rt.is_true(var_poster_url) {
		return ''
	}
	mut var_cover_block := this.transform_to_cover_block(mut var_parsed_block, var_poster_url.str())
	return (this.Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Cover.render_content(rt.new_string(block_content_mutated),
		var_cover_block.clone(), rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Renderer_ContentRenderer_Rendering_Context',
		[]string{}, var_rendering_context))).str()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Video) extract_poster_url(mut var_block_attrs Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_array, block_content string) string {
	...
	mut block_content_mutated := block_content
	if !(!rt.is_true(var_block_attrs_mutated.array_get(rt.new_string('poster')))) {
		return (rt.call_function('esc_url_raw',
			[var_block_attrs_mutated.array_get(rt.new_string('poster'))])).str()
	}
	return ''
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Video) extract_video_url(block_content string) string {
	mut block_content_mutated := block_content
	mut var_dom_helper :=
		create_automattic_woocommerce_emaileditor_integrations_utils_dom_document_helper(rt.new_string(block_content_mutated).clone())
	...
	mut var_class_attr := var_dom_helper.get_attribute_value(var_wrapper_element.clone(),
		rt.new_string('class'))
	if rt.is_true(rt.identical(rt.call_function('strpos', [var_class_attr.clone(),
		rt.new_string('wp-block-embed__wrapper')]), rt.new_bool(false)))
	{
		return ''
	}
	mut var_inner_html := var_dom_helper.get_element_inner_html(var_wrapper_element.clone())
	mut iife_temp_0 :=
		Class_Automattic_WooCommerce_EmailEditor_Integrations_Utils_Html_Processing_Helper{}
	mut iife_result_0 := iife_temp_0.extract_url_from_text(var_inner_html.clone())
	mut var_url := iife_result_0
	if !(!rt.is_true(var_url)) {
		var_url = rt.call_function('html_entity_decode', [var_url.clone(),
			rt.bitwise_or(rt.get_constant('ENT_QUOTES'), rt.get_constant('ENT_HTML5')),
			rt.new_string('UTF-8')])
		if rt.is_true(rt.call_function('filter_var', [var_url.clone(), rt.get_constant('FILTER_VALIDATE_URL')]))
			&& rt.is_true(rt.call_function('wp_http_validate_url', [var_url.clone()])) {
			return var_url.str()
		}
	}
	return ''
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Video) transform_to_cover_block(mut var_video_block Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_array, poster_url string) rt.PhpVal {
	...
	mut var_block_attrs := if !(var_video_block.array_get(rt.new_string('attrs'))).is_null() {
		var_video_block.array_get(rt.new_string('attrs'))
	} else {
		rt.new_array()
	}
	mut var_block_content := if !(var_video_block.array_get(rt.new_string('innerHTML'))).is_null() {
		var_video_block.array_get(rt.new_string('innerHTML'))
	} else {
		rt.new_string('')
	}
	mut var_video_url := if !(var_block_attrs.array_get(rt.new_string('videoUrl'))).is_null() {
		var_block_attrs.array_get(rt.new_string('videoUrl'))
	} else {
		rt.new_string('')
	}
	...
	mut var_link_url := if !(!rt.is_true(var_video_url)) {
		var_video_url
	} else {
		this.get_current_post_url()
	}
	return rt.create_array([rt.ArrayItem{ key: 'blockName', val: 'core/cover' },
		rt.ArrayItem{ key: 'attrs', val: rt.create_array([
			rt.ArrayItem{ key: 'url', val: poster_url_mutated },
			rt.ArrayItem{ key: 'minHeight', val: '390px' },
		]) }, rt.ArrayItem{ key: 'innerBlocks', val: rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'blockName', val: 'core/html' },
				rt.ArrayItem{ key: 'attrs', val: rt.new_array() },
				rt.ArrayItem{ key: 'innerBlocks', val: rt.new_array() },
				rt.ArrayItem{
					key: 'innerHTML'
					val: this.create_play_button_html(var_link_url.str())
				},
				rt.ArrayItem{ key: 'innerContent', val: rt.create_array([
					rt.ArrayItem{ key: none, val: this.create_play_button_html(var_link_url.str()) },
				]) },
			]) },
		]) }, rt.ArrayItem{ key: 'innerHTML', val: var_block_content }])
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Video) create_play_button_html(link_url string) string {
	...
	mut var_play_button := rt.call_function('sprintf', [
		rt.new_string('<img src="%s" alt="%s" style="width: 48px; height: 48px; display: inline-block;" />'),
		rt.call_function('esc_url', [var_play_icon_url.clone()]),
		rt.call_function('esc_attr', [
			rt.call_function('__', [rt.new_string('Play'), rt.new_string('woocommerce')]),
		]),
	])
	if !(link_url_mutated == '') {
		var_play_button = rt.call_function('sprintf', [
			rt.new_string('<a href="%s" target="_blank" rel="noopener noreferrer nofollow" style="display: inline-block; text-decoration: none;">%s</a>'),
			rt.call_function('esc_url', [rt.new_string(link_url_mutated).clone()]),
			var_play_button.clone(),
		])
	}
	return (rt.call_function('sprintf', [
		rt.new_string('<p style="text-align: center;">%s</p>'),
		var_play_button.clone(),
	])).str()
}

fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Video) get_play_icon_url() string {
	mut var_file_name := rt.new_string('/icons/video/play2x.png')
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_Video) get_current_post_url() string {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_post,
		'Automattic_WooCommerce_EmailEditor_Integrations_Core_Renderer_Blocks_WP_Post'))))))
	{
		return ''
	}
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('strpos', [var_permalink.clone(), rt.new_string('https://')]), rt.new_int(0)))))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('strpos', [var_permalink.clone(), rt.new_string('http://')]), rt.new_int(0))))) {
		return ''
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_http_validate_url', [
		var_permalink.clone(),
	])))))
	{
		return ''
