...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Assets_Manager) construct(mut var_settings_controller Class_Automattic_WooCommerce_EmailEditor_Engine_Settings_Controller, mut var_theme_controller Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller, mut var_user_theme Class_Automattic_WooCommerce_EmailEditor_Engine_User_Theme, mut var_logger Class_Automattic_WooCommerce_EmailEditor_Engine_Logger_Email_Editor_Logger) {
	this.settings_controller = var_settings_controller
	this.theme_controller = var_theme_controller
	this.user_theme = var_user_theme
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Assets_Manager) initialize() {
	rt.call_function('add_action', [rt.new_string('admin_enqueue_scripts'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('Automattic_WooCommerce_EmailEditor_Engine_Assets_Manager',
				[]string{}, &this) },
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Assets_Manager) render_email_editor_html(element_id string) {
	...
	print('<div id="' + (rt.call_function('esc_attr', [rt.new_string(element_id)])).str() +
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Assets_Manager) load_editor_assets(var_edited_item rt.PhpVal, script_name string) {
	mut var_post_type := if rt.is_true(rt.new_bool(rt.instance_of(var_edited_item,
		'Automattic_WooCommerce_EmailEditor_Engine_WP_Post')))
	{
		rt.get_property(var_edited_item, 'post_type')
	} else {
		rt.new_string('wp_template')
	}
	mut var_post_id := if rt.is_true(rt.new_bool(rt.instance_of(var_edited_item,
		'Automattic_WooCommerce_EmailEditor_Engine_WP_Post')))
	{
		rt.get_property(var_edited_item, 'ID')
	} else {
		rt.get_property(var_edited_item, 'id')
	}
	mut var_email_editor_assets_path := rt.new_string(
		this.assets_path.to_string().trim_right(' \t\n\r') + '/')
	mut var_email_editor_assets_url := rt.new_string(
		this.assets_url.to_string().trim_right(' \t\n\r') + '/')
		...
		var_assets_file.clone()])))))
	{
		rt.call_method(this.logger, 'error', [
			rt.new_string('Email editor assets file does not exist.'),
			rt.create_array([rt.ArrayItem{ key: 'path', val: var_assets_file }]),
		])
	} else {
		var_assets_file = rt.include_file(var_assets_file.to_string(), '3')
		rt.call_function('wp_enqueue_style', [
			rt.new_string('wc-admin-email-editor-integration'),
			rt.new_string(var_email_editor_assets_url.str() + 'style.css'),
			rt.new_array(),
			var_assets_file.array_get(rt.new_string('version')),
		])
	}
	if rt.is_true(rt.new_bool(rt.instance_of(var_edited_item,
		'Automattic_WooCommerce_EmailEditor_Engine_WP_Block_Template')))
	{
		mut var_context := create_automattic_woocommerce_emaileditor_engine_wp_block_editor_context(rt.create_array([
			rt.ArrayItem{ key: 'post', val: var_edited_item },
		]))
	} else {
	...
	rt.call_function('wp_add_inline_script', [rt.new_string('wp-blocks'),
		rt.call_function('sprintf', [rt.new_string('wp.blocks.setCategories( %s );'),
			rt.call_function('wp_json_encode', [
				rt.call_function('get_block_categories', [var_context.clone()]),
	...
	rt.call_function('wp_add_inline_script', [rt.new_string('wp-blocks'),
		rt.call_function('sprintf', [
			rt.new_string('wp.blocks.unstable__bootstrapServerSideBlockDefinitions( %s );'),
			rt.call_function('wp_json_encode', [
				rt.call_function('get_block_editor_server_block_settings', []rt.PhpVal{}),
				rt.bitwise_or(rt.get_constant('JSON_HEX_TAG'),
					rt.get_constant('JSON_UNESCAPED_SLASHES')),
			]),
		])])
	mut var_localization_data := rt.create_array([
		rt.ArrayItem{ key: 'current_post_type', val: var_post_type },
		rt.ArrayItem{ key: 'current_post_id', val: var_post_id },
		rt.ArrayItem{ key: 'current_wp_user_email', val: rt.get_property(rt.call_function('wp_get_current_user',
			[]rt.PhpVal{}), 'user_email') },
		rt.ArrayItem{ key: 'editor_settings', val: rt.call_method(this.settings_controller,
			'get_settings', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'editor_theme', val: rt.call_method(rt.call_method(this.theme_controller,
			'get_base_theme', []rt.PhpVal{}), 'get_raw_data', []rt.PhpVal{}) },
		rt.ArrayItem{ key: 'user_theme_post_id', val: rt.get_property(rt.call_method(this.user_theme,
			'get_user_theme_post', []rt.PhpVal{}), 'ID') },
		rt.ArrayItem{ key: 'urls', val: rt.create_array([
			rt.ArrayItem{ key: 'listings', val: rt.call_function('admin_url', [
				rt.new_string('admin.php?page=wc-settings&tab=email'),
			]) },
			rt.ArrayItem{ key: 'send', val: rt.call_function('admin_url', [
				rt.new_string('admin.php?page=wc-settings&tab=email'),
			]) },
			rt.ArrayItem{ key: 'back', val: rt.call_function('admin_url', [
				rt.new_string('admin.php?page=wc-settings&tab=email'),
			]) },
			rt.ArrayItem{ key: 'createCoupon', val: rt.call_function('admin_url', [
				rt.new_string('post-new.php?post_type=shop_coupon'),
			]) },
		]) },
	])
	rt.call_function('wp_localize_script', [rt.new_string(script_name),
		rt.new_string('WooCommerceEmailEditor'),
		rt.call_function('apply_filters', [
			rt.new_string('woocommerce_email_editor_script_localization_data'),
			var_localization_data.clone(),
		])])
	this.preload_rest_api_data(var_post_id.clone(), var_post_type.str())
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Assets_Manager) preload_rest_api_data(var_post_id rt.PhpVal, post_type string) {
	...
	mut var_email_post_type := rt.new_string(post_type_mutated).clone()
	mut var_user_theme_post_id := rt.get_property(rt.call_method(this.user_theme,
		'get_user_theme_post', []rt.PhpVal{}), 'ID')
	mut var_template_slug := rt.call_function('get_post_meta', [
		rt.new_int(var_post_id_mutated.to_i64()),
		rt.new_string('_wp_page_template'),
		rt.new_bool(true),
	])
	mut var_routes := rt.create_array([
		rt.ArrayItem{ key: none, val: '/wp/v2/${var_email_post_type.to_string()}/' +
			var_post_id_mutated.clone().to_i64().str() + '?context=edit' },
		rt.ArrayItem{ key: none, val: '/wp/v2/types/${var_email_post_type.to_string()}?context=edit' },
		rt.ArrayItem{ key: none, val: '/wp/v2/global-styles/' +
			var_user_theme_post_id.clone().to_i64().str() + '?context=view' },
		rt.ArrayItem{ key: none, val: '/wp/v2/block-patterns/patterns' },
		rt.ArrayItem{ key: none, val: '/wp/v2/templates?context=view' },
		rt.ArrayItem{ key: none, val: '/wp/v2/block-patterns/categories' },
		rt.ArrayItem{ key: none, val: '/wp/v2/settings' },
		rt.ArrayItem{ key: none, val: '/wp/v2/types?context=view' },
		rt.ArrayItem{ key: none, val: '/wp/v2/taxonomies?context=view' },
	])
	if rt.is_true(rt.new_bool(var_template_slug.clone().is_string())) {
	...
	mut var_preload_data := rt.call_function('array_reduce', [
		var_routes.clone(), rt.new_string('rest_preload_api_request'),
		rt.new_array()])
	rt.call_function('wp_add_inline_script', [rt.new_string('wp-blocks'),
		rt.call_function('sprintf', [
...
fn create_automattic_woocommerce_emaileditor_engine_wp_block_editor_context(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_EmailEditor_Engine_WP_Block_Editor_Context {
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Engine_Assets_Manager) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			...
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Theme_Controller](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_User_Theme](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_3 := rt.cast_object_ptr[Class_Automattic_WooCommerce_EmailEditor_Engine_Logger_Email_Editor_Logger](if args.len > 3 {
				args[3]
			} else {
				rt.new_null()
			})
