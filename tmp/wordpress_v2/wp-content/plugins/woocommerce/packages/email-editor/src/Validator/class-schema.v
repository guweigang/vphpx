...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema) nullable() rt.PhpVal {
	mut var_type := if !(this.schema.array_get(rt.new_string('type'))).is_null() { this.schema.array_get(rt.new_string('type')) } else { rt.create_array([
			rt.ArrayItem{ key: none, val: 'null' },
		]) }
	return this.update_schema_property('type', if var_type.clone().is_array() { var_type } else { rt.create_array([
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema) non_nullable() rt.PhpVal {
	mut var_type := if !(this.schema.array_get(rt.new_string('type'))).is_null() {
		this.schema.array_get(rt.new_string('type'))
	} else {
		rt.new_null()
	}
	return if rt.is_true(rt.identical(rt.new_null(), var_type)) { this.unset_schema_property('type') } else { this.update_schema_property('type', if var_type.clone().is_array() {
			var_type.array_get(rt.new_int(0))
		} else {
			var_type
		})
	 }
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema) default(var_default_value rt.PhpVal) rt.PhpVal {
	return this.update_schema_property('default', var_default_value.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema) field(name string, var_value rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.call_function('in_array', [rt.new_string(name),
		this.get_reserved_keywords(), rt.new_bool(true)]))
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditor_Validator_Exception',
			[]string{}, create_automattic_woocommerce_emaileditor_validator_exception(rt.call_function('esc_html', [
			rt.new_string("Field name '${var_name}' is reserved"),
		]))))
	}
	return this.update_schema_property(name, var_value.clone())
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema) to_string() string {
	mut var_json := rt.call_function('wp_json_encode', [this.schema,
		rt.bitwise_or(rt.bitwise_or(rt.get_constant('JSON_UNESCAPED_UNICODE'),
			rt.get_constant('JSON_UNESCAPED_SLASHES')),
			rt.get_constant('JSON_PRESERVE_ZERO_FRACTION'))])
	mut var_error := rt.call_function('json_last_error', []rt.PhpVal{})
	if rt.is_true(var_error) || rt.is_true(rt.identical(rt.new_bool(false), var_json)) {
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditor_Validator_Exception',
			[]string{}, create_automattic_woocommerce_emaileditor_validator_exception(rt.call_function('esc_html', [
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema) update_schema_property(name string, var_value rt.PhpVal) rt.PhpVal {
	mut var_clone := rt.new_object('Automattic_WooCommerce_EmailEditor_Validator_Schema',
		[]string{}, &this).dup()
	rt.get_property(var_clone, 'schema').array_set(name, var_value.clone())
	return var_clone.clone()
}
...
fn (mut this Class_Automattic_WooCommerce_EmailEditor_Validator_Schema) validate_pattern(pattern string) {
	mut var_escaped := rt.call_function('str_replace', [rt.new_string('#'),
		rt.new_string('\\#'), rt.new_string(pattern)])
	mut var_regex := rt.new_string('#${var_escaped.to_string()}#u')
	if rt.is_true(rt.identical(rt.call_function('preg_match', [
		var_regex.clone(), rt.new_string('')]), rt.new_bool(false)))
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_EmailEditor_Validator_Exception',
			[]string{}, create_automattic_woocommerce_emaileditor_validator_exception(rt.call_function('esc_html', [
			rt.new_string("Invalid regular expression '${var_regex.to_string()}'"),
		]))))
	}
}
...
fn create_automattic_woocommerce_emaileditor_validator_exception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_EmailEditor_Validator_Exception {
