...
fn Class_CronExpression_DayOfMonthField.getnearestweekday(var_currentYear rt.PhpVal, var_currentMonth rt.PhpVal, var_targetDay rt.PhpVal) rt.PhpVal {
	...
	mut var_tday := rt.call_function('str_pad', [var_targetDay_mutated.clone(),
		rt.new_int(2), rt.new_string('0'), rt.get_constant('STR_PAD_LEFT')])
	mut var_target :=
		create_datetime(rt.new_string('${var_currentYear.to_string()}-${var_currentMonth.to_string()}-${var_tday.to_string()}'))
	mut var_currentWeekday := rt.new_int((var_target.format(rt.new_string('N'))).to_i64())
	...
	mut iter_1 := rt.create_array([rt.ArrayItem{ key: none, val: -1 },
		rt.ArrayItem{ key: none, val: 1 }, rt.ArrayItem{ key: none, val: -2 },
		rt.ArrayItem{ key: none, val: 2 }]).iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_i := item_1.val
		mut var_adjusted := rt.add(var_targetDay_mutated, var_i)
		if rt.is_true(rt.greater(var_adjusted, rt.new_int(0)))
			&& rt.is_true(rt.less_equal(var_adjusted, var_lastDayOfMonth)) {
			var_target.setdate(var_currentYear.clone(), var_currentMonth.clone(),
				var_adjusted.clone())
			if rt.is_true(rt.less(var_target.format(rt.new_string('N')), rt.new_int(6)))
				&& rt.is_true(rt.equal(var_target.format(rt.new_string('m')), var_currentMonth)) {
				return mut var_target
			}
		}
	}
...
fn (mut this Class_CronExpression_DayOfMonthField) issatisfiedby(mut var_date Class_DateTime, var_value rt.PhpVal) bool {
	...
	if rt.is_true(rt.call_function('strpos', [var_value.clone(),
		rt.new_string('W')]))
	{
		mut var_targetDay := rt.call_function('substr', [var_value.clone(),
			rt.new_int(0), rt.call_function('strpos', [var_value.clone(),
				rt.new_string('W')])])
		return (rt.equal(var_date.format(rt.new_string('j')), rt.call_method(Class_CronExpression_DayOfMonthField.getnearestweekday(var_date.format(rt.new_string('Y')),
			var_date.format(rt.new_string('m')), var_targetDay.clone()), 'format', [
			rt.new_string('j'),
		]))).to_bool()
	}
	return (this.issatisfied(var_date.format(rt.new_string('d')), var_value.clone())).to_bool()
}
...
fn (mut this Class_CronExpression_DayOfMonthField) validate(var_value rt.PhpVal) bool {
	return (rt.call_function('preg_match', [
		rt.new_string('/[\\*,\\/\\-\\?LW0-9A-Za-z]+/'),
		var_value.clone(),
	])).to_bool()
}
...
fn (mut this Class_CronExpression_DayOfMonthField) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'getNearestWeekday' {
			...
			return Class_CronExpression_DayOfMonthField.getnearestweekday(dispatch_arg_0,
