...
fn init_static_cronexpression() {
		rt.init_static_prop('CronExpression', 'order', rt.create_array([rt.ArrayItem{ key: none, val: Class_CronExpression.year() }, rt.ArrayItem{ key: none, val: Class_CronExpression.month() }, rt.ArrayItem{ key: none, val: Class_CronExpression.day() }, rt.ArrayItem{ key: none, val: Class_CronExpression.weekday() }, rt.ArrayItem{ key: none, val: Class_CronExpression.hour() }, rt.ArrayItem{ key: none, val: Class_CronExpression.minute() }]))
}
...
fn Class_CronExpression.factory(var_expression rt.PhpVal, mut var_fieldFactory Class_?CronExpression_FieldFactory) rt.PhpVal {
	...
	return rt.new_object('CronExpression', []string{}, create_cronexpression(var_expression_mutated.clone(), if rt.is_true(var_fieldFactory) { var_fieldFactory } else { create_cronexpression_fieldfactory() }))
}
...
fn (mut this Class_CronExpression) construct(var_expression rt.PhpVal, mut var_fieldFactory Class_CronExpression_FieldFactory) {
	mut var_expression_mutated := var_expression
	this.fieldFactory = var_fieldFactory
	this.setexpression(var_expression_mutated.clone())
}

fn (mut this Class_CronExpression) setexpression(var_value rt.PhpVal) rt.PhpVal {
	this.cronParts = rt.call_function('preg_split', [rt.new_string('/\\s/'), var_value.clone(), rt.new_int(-1), rt.get_constant('PREG_SPLIT_NO_EMPTY')])
	...
	for {
		...
		this.setpart(var_position.clone(), var_part.clone())
	}
...
fn (mut this Class_CronExpression) setpart(var_position rt.PhpVal, var_value rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(rt.call_method(this.fieldFactory, 'getField', [var_position.clone()]), 'validate', [var_value.clone()]))))) {
	...
	this.cronParts.array_set(var_position, var_value.clone())
	return rt.new_object('CronExpression', []string{}, this)
}
...
fn (mut this Class_CronExpression) getmultiplerundates(var_total rt.PhpVal, currentTime string, invert bool, allowCurrentDate bool) rt.PhpVal {
	...
	for {
		if !(rt.is_true(rt.less(var_i, rt.call_function('max', [rt.new_int(0), var_total.clone()])))) { break }
...
fn (mut this Class_CronExpression) getexpression(var_part rt.PhpVal) rt.PhpVal {
	...
	} else if rt.is_true(rt.new_bool(this.cronParts.array_isset(var_part_mutated.clone()))) {
...
fn (mut this Class_CronExpression) isdue(currentTime string) rt.PhpVal {
	...
	currentTime_mutated = (rt.call_function('strtotime', [var_currentDate.clone()])).str()
	...
	currentTime_mutated = (rt.call_function('strtotime', [var_currentDate.clone()])).str()
		...
		currentTime_mutated = (create_datetime(rt.new_string(currentTime_mutated).clone())).str()
	...
	var_currentDate = rt.call_method(rt.new_string(currentTime_mutated), 'format', [rt.new_string('Y-m-d H:i')])
	currentTime_mutated = rt.new_int((rt.call_method(rt.new_string(currentTime_mutated), 'getTimestamp', []rt.PhpVal{})).to_i64())
	}
...
fn (mut this Class_CronExpression) getrundate(var_currentTime rt.PhpVal, nth i64, invert bool, allowCurrentDate bool) rt.PhpVal {
	...
	mut var_currentDate := var_currentTime_mutated.clone()
	} else {
	...
	mut var_nextRun := var_currentDate.dup()
	nth_mutated = nth_mutated
	mut var_i := rt.new_int(0)
	for {
		if !(rt.is_true(rt.less(var_i, rt.new_int(1000)))) { break }
		mut iter_2 := rt.get_static_prop('CronExpression', 'order').iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_position := item_2.val
			mut var_part := this.getexpression(var_position.clone())
			if rt.is_true(rt.identical(rt.new_null(), var_part)) {
				continue
			}
			mut var_satisfied := rt.new_bool(false)
			mut var_field := rt.call_method(this.fieldFactory, 'getField', [var_position.clone()])
			if rt.is_true(rt.identical(rt.call_function('strpos', [var_part.clone(), rt.new_string(',')]), rt.new_bool(false))) {
			var_satisfied = rt.call_method(var_field, 'isSatisfiedBy', [var_nextRun.clone(), var_part.clone()])
			} else {
				mut iter_3 := rt.call_function('array_map', [rt.new_string('trim'), rt.call_function('explode', [rt.new_string(','), var_part.clone()])]).iterator()
				for {
					item_3 := iter_3.next() or { break }
					mut var_listPart := item_3.val
					if rt.is_true(rt.call_method(var_field, 'isSatisfiedBy', [var_nextRun.clone(), var_listPart.clone()])) {
						var_satisfied = rt.new_bool(true)
						break
					}
				}
			}
			if rt.is_true(rt.new_bool(!(rt.is_true(var_satisfied)))) {
				rt.call_method(var_field, 'increment', [var_nextRun.clone(), rt.new_bool(invert)])
				continue
			}
		}
		if (!(var_allowCurrentDate) && rt.is_true(rt.equal(var_nextRun, var_currentDate))) || rt.is_true(rt.greater(rt.pre_dec(rt.new_int(nth_mutated)), -1)) {
			rt.call_method(rt.call_method(this.fieldFactory, 'getField', [rt.new_int(0)]), 'increment', [var_nextRun.clone(), rt.new_bool(invert)])
			continue
		}
		return var_nextRun.clone()
		rt.post_inc(var_i)
	}
