...
fn (mut this Class_ActionScheduler_AsyncRequest_QueueRunner) handle() {
	rt.call_function('do_action', [rt.new_string('action_scheduler_run_queue'),
		rt.new_string('Async Request')])
	mut var_sleep_seconds := this.get_sleep_seconds()
	if rt.is_true(var_sleep_seconds) {
		rt.call_function('sleep', [var_sleep_seconds.clone()])
	}
	this.maybe_dispatch()
}
...
fn (mut this Class_ActionScheduler_AsyncRequest_QueueRunner) maybe_dispatch() {
	...
	this.dispatch()
	mut iife_temp_0 := Class_ActionScheduler_QueueRunner{}
	mut iife_result_0 := iife_temp_0.instance()
	rt.call_method(iife_result_0, 'unhook_dispatch_async_request', []rt.PhpVal{})
}
...
fn (mut this Class_ActionScheduler_AsyncRequest_QueueRunner) allow() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('has_action', [rt.new_string('action_scheduler_run_queue')])))))
		|| rt.is_true(rt.call_method(iife_result_1, 'has_maximum_concurrent_batches', []rt.PhpVal{}))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(this.store, 'has_pending_actions_due', []rt.PhpVal{}))))) {
		mut var_allow := rt.new_bool(false)
	} else {
		var_allow = rt.new_bool(true)
	}
	return rt.call_function('apply_filters', [
		rt.new_string('action_scheduler_allow_async_request_runner'),
		var_allow.clone(),
	])
}
...
fn (mut this Class_ActionScheduler_AsyncRequest_QueueRunner) get_sleep_seconds() rt.PhpVal {
	return rt.call_function('apply_filters', [
		rt.new_string('action_scheduler_async_request_sleep_seconds'),
		rt.new_int(5),
		rt.new_object('ActionScheduler_AsyncRequest_QueueRunner', ['WP_Async_Request'], &this),
	])
}
