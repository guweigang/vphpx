...
fn init_static_actionscheduler_logger() {
		rt.init_static_prop('ActionScheduler_Logger', 'logger', rt.new_null())
}

fn Class_ActionScheduler_Logger.instance() rt.PhpVal {
	if !rt.is_true(rt.get_static_prop('ActionScheduler_Logger', 'logger')) {
		...
		rt.set_static_prop('ActionScheduler_Logger', 'logger', rt.new_object('', []string{}, rt.create_object_dynamically(var_class, []rt.PhpVal{})))
	}
	return rt.get_static_prop('ActionScheduler_Logger', 'logger')
}
...
fn (mut this Class_ActionScheduler_Logger) log_stored_action(var_action_id rt.PhpVal) {
	this.log(var_action_id.clone(), rt.call_function('__', [rt.new_string('action created'), rt.new_string('woocommerce')]), rt.new_null())
}

fn (mut this Class_ActionScheduler_Logger) log_canceled_action(var_action_id rt.PhpVal) {
	this.log(var_action_id.clone(), rt.call_function('__', [rt.new_string('action canceled'), rt.new_string('woocommerce')]), rt.new_null())
}
...
fn (mut this Class_ActionScheduler_Logger) log_started_action(var_action_id rt.PhpVal, context string) {
	...
	this.log(var_action_id.clone(), var_message.clone(), rt.new_null())
}
...
fn (mut this Class_ActionScheduler_Logger) log_timed_out_action(var_action_id rt.PhpVal, var_timeout rt.PhpVal) {
	this.log(var_action_id.clone(), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('action marked as failed after %s seconds. Unknown error occurred. Check server, PHP and database error logs to diagnose cause.'), rt.new_string('woocommerce')]), var_timeout.clone()]), rt.new_null())
}
...
fn (mut this Class_ActionScheduler_Logger) log_unexpected_shutdown(var_action_id rt.PhpVal, var_error rt.PhpVal) {
	if !(!rt.is_true(var_error)) {
		this.log(var_action_id.clone(), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('unexpected shutdown: PHP Fatal error %1$s in %2$s on line %3$s'), rt.new_string('woocommerce')]), var_error.array_get(rt.new_string('message')), var_error.array_get(rt.new_string('file')), var_error.array_get(rt.new_string('line'))]), rt.new_null())
	}
}

fn (mut this Class_ActionScheduler_Logger) log_reset_action(var_action_id rt.PhpVal) {
	this.log(var_action_id.clone(), rt.call_function('__', [rt.new_string('action reset'), rt.new_string('woocommerce')]), rt.new_null())
}
...
fn (mut this Class_ActionScheduler_Logger) log_failed_fetch_action(var_action_id rt.PhpVal, mut var_exception Class_?Exception) {
	...
	this.log(var_action_id.clone(), var_log_message.clone(), rt.new_null())
}
...
fn (mut this Class_ActionScheduler_Logger) log_failed_schedule_next_instance(var_action_id rt.PhpVal, mut var_exception Class_Exception) {
	this.log(var_action_id.clone(), rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('There was a failure scheduling the next instance of this action: %s'), rt.new_string('woocommerce')]), var_exception.getmessage()]), rt.new_null())
}
...
fn (mut this Class_ActionScheduler_Logger) bulk_log_cancel_actions(var_action_ids rt.PhpVal) {
	...
	for {
		...
		this.log_canceled_action(var_action_id.clone())
	}
}
