...
fn (mut this Class_ActionScheduler_HybridStore) construct(mut var_config Class_?Config) {
	...
	this.demarkation_id = rt.new_int((rt.call_function('get_option', [rt.new_string(Class_ActionScheduler_HybridStore.demarkation_option()), rt.new_int(0)])).to_i64())
	if !rt.is_true(var_config_mutated) {
	mut iife_temp_0 := Class_Action_Scheduler_Migration_Controller{}
	mut iife_result_0 := iife_temp_0.instance()
	var_config_mutated = rt.call_method(iife_result_0, 'get_migration_config_object', []rt.PhpVal{})
	}
	...
	this.migration_runner = create_action_scheduler_migration_runner(var_config_mutated)
}
...
fn (mut this Class_ActionScheduler_HybridStore) set_demarkation_id(var_id rt.PhpVal) rt.PhpVal {
	...
	if !rt.is_true(var_id_mutated) {
		var_id_mutated = rt.new_int((rt.call_method(var_wpdb, 'get_var', [rt.concat(rt.new_string('SELECT MAX(ID) FROM '), rt.get_property(var_wpdb, 'posts'))])).to_i64())
		rt.post_inc(var_id_mutated)
	}
	rt.call_function('update_option', [rt.new_string(Class_ActionScheduler_HybridStore.demarkation_option()), var_id_mutated.clone()])
...
fn (mut this Class_ActionScheduler_HybridStore) find_action(var_hook rt.PhpVal, var_params rt.PhpVal) rt.PhpVal {
	mut var_found_unmigrated_action := rt.call_method(this.secondary_store, 'find_action', [var_hook.clone(), var_params.clone()])
	...
	return rt.call_method(this.primary_store, 'find_action', [var_hook.clone(), var_params.clone()])
}

fn (mut this Class_ActionScheduler_HybridStore) query_actions(var_query rt.PhpVal, query_type string) rt.PhpVal {
	mut var_found_unmigrated_actions := rt.call_method(this.secondary_store, 'query_actions', [var_query.clone(), rt.new_string('select')])
	if !(!rt.is_true(var_found_unmigrated_actions)) {
		this.migrate(var_found_unmigrated_actions.clone())
	}
	return rt.call_method(this.primary_store, 'query_actions', [var_query.clone(), rt.new_string(query_type)])
}
...
fn (mut this Class_ActionScheduler_HybridStore) action_counts() rt.PhpVal {
	...
	for {
		...
		if var_unmigrated_actions_count.array_isset(var_status_key) {
			var_count = rt.add(var_count, var_unmigrated_actions_count.array_get(var_status_key))
		}
		if var_migrated_actions_count.array_isset(var_status_key) {
			var_count = rt.add(var_count, var_migrated_actions_count.array_get(var_status_key))
		}
		var_actions_count_by_status.array_set(var_status_key, var_count.clone())
	}
	var_actions_count_by_status = rt.call_function('array_filter', [var_actions_count_by_status.clone()])
	return var_actions_count_by_status.clone()
}
...
fn (mut this Class_ActionScheduler_HybridStore) stake_claim(max_actions i64, mut var_before_date Class_?DateTime, var_hooks rt.PhpVal, group string) rt.PhpVal {
	mut var_claim := rt.call_method(this.secondary_store, 'stake_claim', [rt.new_int(max_actions), var_before_date, var_hooks.clone(), rt.new_string(group)])
	mut var_claimed_actions := rt.call_method(var_claim, 'get_actions', []rt.PhpVal{})
	if !(!rt.is_true(var_claimed_actions)) {
		this.migrate(var_claimed_actions.clone())
	}
	rt.call_method(this.secondary_store, 'release_claim', [var_claim.clone()])
	return rt.call_method(this.primary_store, 'stake_claim', [rt.new_int(max_actions), var_before_date, var_hooks.clone(), rt.new_string(group)])
}

fn (mut this Class_ActionScheduler_HybridStore) migrate(var_action_ids rt.PhpVal) {
	rt.call_method(this.migration_runner, 'migrate_actions', [var_action_ids.clone()])
}
...
fn (mut this Class_ActionScheduler_HybridStore) save_action(mut var_action Class_ActionScheduler_Action, mut var_date Class_?DateTime) rt.PhpVal {
	...
	return rt.call_method(this.primary_store, 'save_action', [var_action_mutated, var_date])
}

fn (mut this Class_ActionScheduler_HybridStore) fetch_action(var_action_id rt.PhpVal) rt.PhpVal {
	mut var_store := this.get_store_from_action_id(var_action_id.clone(), true)
	if rt.is_true(var_store) {
		return rt.call_method(var_store, 'fetch_action', [var_action_id.clone()])
	} else {
...
fn (mut this Class_ActionScheduler_HybridStore) cancel_action(var_action_id rt.PhpVal) {
	mut var_store := this.get_store_from_action_id(var_action_id.clone(), false)
	if rt.is_true(var_store) {
		rt.call_method(var_store, 'cancel_action', [var_action_id.clone()])
	}
}
...
fn (mut this Class_ActionScheduler_HybridStore) get_date(var_action_id rt.PhpVal) rt.PhpVal {
	mut var_store := this.get_store_from_action_id(var_action_id.clone(), false)
	if rt.is_true(var_store) {
		return rt.call_method(var_store, 'get_date', [var_action_id.clone()])
	} else {
		return rt.new_null()
	}
	return rt.new_null()
...
fn (mut this Class_ActionScheduler_HybridStore) mark_failure(var_action_id rt.PhpVal) {
	mut var_store := this.get_store_from_action_id(var_action_id.clone(), false)
	if rt.is_true(var_store) {
		rt.call_method(var_store, 'mark_failure', [var_action_id.clone()])
	}
}
...
fn (mut this Class_ActionScheduler_HybridStore) log_execution(var_action_id rt.PhpVal) {
	mut var_store := this.get_store_from_action_id(var_action_id.clone(), false)
	if rt.is_true(var_store) {
		rt.call_method(var_store, 'log_execution', [var_action_id.clone()])
	}
}
...
fn (mut this Class_ActionScheduler_HybridStore) mark_complete(var_action_id rt.PhpVal) {
	mut var_store := this.get_store_from_action_id(var_action_id.clone(), false)
	if rt.is_true(var_store) {
		rt.call_method(var_store, 'mark_complete', [var_action_id.clone()])
	}
}
...
fn (mut this Class_ActionScheduler_HybridStore) get_status(var_action_id rt.PhpVal) rt.PhpVal {
	mut var_store := this.get_store_from_action_id(var_action_id.clone(), false)
	if rt.is_true(var_store) {
		return rt.call_method(var_store, 'get_status', [var_action_id.clone()])
	}
	return rt.new_null()
}
...
fn (mut this Class_ActionScheduler_HybridStore) get_store_from_action_id(var_action_id rt.PhpVal, primary_first bool) rt.PhpVal {
	...
	for var_store in var_stores {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [var_action.clone(), rt.new_string('ActionScheduler_NullAction')]))))) {
			return var_store.clone()
		}
	}
	return rt.new_null()
}
...
fn (mut this Class_ActionScheduler_HybridStore) get_claim_id(var_action_id rt.PhpVal) rt.PhpVal {
	return rt.call_method(this.primary_store, 'get_claim_id', [var_action_id.clone()])
}
...
fn (mut this Class_ActionScheduler_HybridStore) release_claim(mut var_claim Class_ActionScheduler_ActionClaim) {
	...
	rt.call_method(this.primary_store, 'release_claim', [var_claim_mutated])
}

fn (mut this Class_ActionScheduler_HybridStore) unclaim_action(var_action_id rt.PhpVal) {
	rt.call_method(this.primary_store, 'unclaim_action', [var_action_id.clone()])
}

fn (mut this Class_ActionScheduler_HybridStore) find_actions_by_claim_id(var_claim_id rt.PhpVal) rt.PhpVal {
	return rt.call_method(this.primary_store, 'find_actions_by_claim_id', [var_claim_id.clone()])
}
