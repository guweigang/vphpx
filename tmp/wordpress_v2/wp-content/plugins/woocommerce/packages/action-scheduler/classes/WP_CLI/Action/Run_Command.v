...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Run_Command) execute() {
	...
	mut iife_result_0 := iife_temp_0.runner()
	mut var_runner := iife_result_0
	mut var_progress_bar := rt.call_function('WP_CLI\Utils\make_progress_bar', [rt.call_function('sprintf', [rt.call_function('_n', [rt.new_string('Executing %d action'), rt.new_string('Executing %d actions'), this.action_counts.array_get(rt.new_string('total')), rt.new_string('woocommerce')]), rt.call_function('number_format_i18n', [this.action_counts.array_get(rt.new_string('total'))])]), this.action_counts.array_get(rt.new_string('total'))])
	mut iter_1 := this.action_ids.iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_action_id := item_1.val
		rt.call_method(var_runner, 'process_action', [var_action_id.clone(), rt.new_string('Action Scheduler CLI')])
		rt.call_method(var_progress_bar, 'tick', []rt.PhpVal{})
	}
	rt.call_method(var_progress_bar, 'finish', []rt.PhpVal{})
	mut iter_2 := rt.create_array([rt.ArrayItem{ key: none, val: 'ignored' }, rt.ArrayItem{ key: none, val: 'invalid' }, rt.ArrayItem{ key: none, val: 'failed' }]).iterator()
	for {
	...
	mut var_format := rt.call_function('_n', [rt.new_string('%1$d action %2$s.'), rt.new_string('%1$d actions %2$s.'), var_count.clone(), rt.new_string('woocommerce')])
	mut iife_temp_1 := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}
	mut iife_result_1 := iife_temp_1.warning(rt.call_function('sprintf', [var_format.clone(), rt.call_function('number_format_i18n', [var_count.clone()]), var_type.clone()]))
	}
mut iife_temp_2 := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}
mut iife_result_2 := iife_temp_2.success(rt.call_function('sprintf', [rt.call_function('_n', [rt.new_string('Executed %d action.'), rt.new_string('Executed %d actions.'), this.action_counts.array_get(rt.new_string('executed')), rt.new_string('woocommerce')]), rt.call_function('number_format_i18n', [this.action_counts.array_get(rt.new_string('executed'))])]))
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Run_Command) on_action_ignored(var_action_id rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('action_scheduler_execution_ignored'), rt.call_function('current_action', []rt.PhpVal{}))))) {
		return
	}
	...
	rt.post_inc(this.action_counts.array_get(rt.new_string('ignored')))
mut iife_temp_3 := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}
mut iife_result_3 := iife_temp_3.debug(rt.call_function('sprintf', [rt.new_string('Action %d was ignored.'), var_action_id_mutated.clone()]))
}

fn (mut this Class_Action_Scheduler_WP_CLI_Action_Run_Command) on_action_executed(var_action_id rt.PhpVal) {
	mut var_action_id_mutated := var_action_id
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('action_scheduler_after_execute'), rt.call_function('current_action', []rt.PhpVal{}))))) {
		return
	}
	var_action_id_mutated = rt.call_function('absint', [var_action_id_mutated.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_action_id_mutated.clone(), this.action_ids, rt.new_bool(true)]))))) {
	...
	rt.post_inc(this.action_counts.array_get(rt.new_string('executed')))
mut iife_temp_4 := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}
mut iife_result_4 := iife_temp_4.debug(rt.call_function('sprintf', [rt.new_string('Action %d was executed.'), var_action_id_mutated.clone()]))
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Run_Command) on_action_failed(var_action_id rt.PhpVal, mut var_e Class_Action_Scheduler_WP_CLI_Action_Exception) {
	mut var_action_id_mutated := var_action_id
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('action_scheduler_failed_execution'), rt.call_function('current_action', []rt.PhpVal{}))))) {
		return
	}
	var_action_id_mutated = rt.call_function('absint', [var_action_id_mutated.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_action_id_mutated.clone(), this.action_ids, rt.new_bool(true)]))))) {
...
mut iife_temp_5 := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}
mut iife_result_5 := iife_temp_5.debug(rt.call_function('sprintf', [rt.new_string('Action %d failed execution: %s'), var_action_id_mutated.clone(), var_e.getmessage()]))
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Run_Command) on_action_invalid(var_action_id rt.PhpVal, mut var_e Class_Action_Scheduler_WP_CLI_Action_Exception) {
	mut var_action_id_mutated := var_action_id
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('action_scheduler_failed_validation'), rt.call_function('current_action', []rt.PhpVal{}))))) {
		return
	}
	var_action_id_mutated = rt.call_function('absint', [var_action_id_mutated.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_action_id_mutated.clone(), this.action_ids, rt.new_bool(true)]))))) {
	...
	rt.post_inc(this.action_counts.array_get(rt.new_string('invalid')))
mut iife_temp_6 := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}
mut iife_result_6 := iife_temp_6.debug(rt.call_function('sprintf', [rt.new_string('Action %d failed validation: %s'), var_action_id_mutated.clone(), var_e.getmessage()]))
}
