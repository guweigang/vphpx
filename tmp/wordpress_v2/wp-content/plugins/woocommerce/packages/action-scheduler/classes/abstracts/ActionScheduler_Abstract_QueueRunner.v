...
fn (mut this Class_ActionScheduler_Abstract_QueueRunner) process_action(var_action_id rt.PhpVal, context string) {
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(var_message.clone())))
		return rt.new_null()
		}
	...
	rt.call_function('set_error_handler', [rt.new_closure(closure_2_fn), rt.bitwise_or(rt.get_constant('E_USER_ERROR'), rt.get_constant('E_RECOVERABLE_ERROR'))])
	mut var_valid_action := rt.new_bool(true)
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	rt.call_function('do_action', [rt.new_string('action_scheduler_before_execute'), var_action_id.clone(), rt.new_string(context)])
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_ActionScheduler_Store.status_pending(), rt.call_method(this.store, 'get_status', [var_action_id.clone()]))))) {
		var_valid_action = rt.new_bool(false)
		if rt.has_exception() { unsafe { goto catch_label_2 } }
		rt.call_function('do_action', [rt.new_string('action_scheduler_execution_ignored'), var_action_id.clone(), rt.new_string(context)])
		if rt.has_exception() { unsafe { goto catch_label_2 } }
		return
	}
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	rt.call_function('do_action', [rt.new_string('action_scheduler_begin_execute'), var_action_id.clone(), rt.new_string(context)])
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	mut var_action := rt.call_method(this.store, 'fetch_action', [var_action_id.clone()])
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	rt.call_method(this.store, 'log_execution', [var_action_id.clone()])
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	rt.call_method(var_action, 'execute', []rt.PhpVal{})
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	rt.call_function('do_action', [rt.new_string('action_scheduler_after_execute'), var_action_id.clone(), var_action.clone(), rt.new_string(context)])
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	rt.call_method(this.store, 'mark_complete', [var_action_id.clone()])
	if rt.has_exception() { unsafe { goto catch_label_2 } }
...
catch_label_2:
	...
	if rt.instance_of(var_e_2, 'Throwable') {
		...
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.call_method(var_e, 'getMessage', []rt.PhpVal{}), rt.call_method(var_e, 'getCode', []rt.PhpVal{}), var_e.clone())))
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'Exception') {
		var_e = var_e_1.clone()
		this.handle_action_error(var_action_id.clone(), var_e.clone(), rt.new_string(context), var_valid_action.clone())
		unsafe { goto finally_label_1 }
	}
	else {
...
end_label_1:
	if !(var_action).is_null() && rt.is_true(rt.call_function('is_a', [var_action.clone(), rt.new_string('ActionScheduler_Action')])) && rt.is_true(rt.call_method(rt.call_method(var_action, 'get_schedule', []rt.PhpVal{}), 'is_recurring', []rt.PhpVal{})) {
		this.schedule_next_instance(mut rt.cast_object_ptr[Class_ActionScheduler_Action](var_action), var_action_id.clone())
	}
}
...
fn (mut this Class_ActionScheduler_Abstract_QueueRunner) handle_action_error(var_action_id rt.PhpVal, var_e rt.PhpVal, var_context rt.PhpVal, var_valid_action rt.PhpVal) {
	...
	if rt.is_true(var_valid_action_mutated) {
		...
		rt.call_function('do_action', [rt.new_string('action_scheduler_failed_execution'), var_action_id.clone(), var_e.clone(), var_context.clone()])
	} else {
		rt.call_function('do_action', [rt.new_string('action_scheduler_failed_validation'), var_action_id.clone(), var_e.clone(), var_context.clone()])
	}
}
...
fn (mut this Class_ActionScheduler_Abstract_QueueRunner) schedule_next_instance(mut var_action Class_ActionScheduler_Action, var_action_id rt.PhpVal) {
	...
	if rt.is_true(rt.identical(Class_ActionScheduler_Store.status_failed(), rt.call_method(this.store, 'get_status', [var_action_id.clone()]))) && this.recurring_action_is_consistently_failing(mut var_action_mutated, var_action_id.clone()) {
		mut iife_temp_3 := Class_ActionScheduler_Logger{}
		mut iife_result_3 := iife_temp_3.instance()
		rt.call_method(iife_result_3, 'log', [var_action_id.clone(), rt.call_function('__', [rt.new_string('This action appears to be consistently failing. A new instance will not be scheduled.'), rt.new_string('woocommerce')])])
		return
	}
	mut iife_temp_4 := Class_ActionScheduler{}
	mut iife_result_4 := iife_temp_4.factory()
	rt.call_method(iife_result_4, 'repeat', [var_action_mutated])
...
catch_label_3:
	...
	if rt.instance_of(var_e_3, 'Exception') {
		...
		rt.call_function('do_action', [rt.new_string('action_scheduler_failed_to_schedule_next_instance'), var_action_id.clone(), var_e.clone(), var_action_mutated])
		unsafe { goto end_label_3 }
	}
	else {
...
fn (mut this Class_ActionScheduler_Abstract_QueueRunner) recurring_action_is_consistently_failing(mut var_action Class_ActionScheduler_Action, var_action_id rt.PhpVal) bool {
	mut var_action_mutated := var_action
	mut var_consistent_failure_threshold := rt.new_int((rt.call_function('apply_filters', [rt.new_string('action_scheduler_recurring_action_failure_threshold'), rt.new_int(5)])).to_i64())
	...
	mut var_first_failing_action_id := rt.call_method(this.store, 'query_actions', [rt.create_array_from_native_map(var_query_args)])
	if !rt.is_true(var_first_failing_action_id) {
		return false
	}
	var_query_args.delete('status')
	mut var_first_action_id_with_the_same_hook := rt.call_method(this.store, 'query_actions', [rt.create_array_from_native_map(var_query_args)])
	return (rt.call_function('apply_filters', [rt.new_string('action_scheduler_recurring_action_is_consistently_failing'), rt.identical(var_first_action_id_with_the_same_hook, var_first_failing_action_id), var_action_mutated])).to_bool()
}
...
fn (mut this Class_ActionScheduler_Abstract_QueueRunner) get_time_limit() rt.PhpVal {
	mut var_time_limit := rt.new_int(30)
	...
	var_time_limit = rt.call_function('apply_filters', [rt.new_string('action_scheduler_maximum_execution_time'), var_time_limit.clone()])
	}
	return rt.call_function('absint', [rt.call_function('apply_filters', [rt.new_string('action_scheduler_queue_runner_time_limit'), var_time_limit.clone()])])
}
...
fn (mut this Class_ActionScheduler_Abstract_QueueRunner) get_execution_time() rt.PhpVal {
	...
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('getrusage')])) && rt.is_true(rt.call_function('apply_filters', [rt.new_string('action_scheduler_use_cpu_execution_time'), rt.call_function('defined', [rt.new_string('PANTHEON_ENVIRONMENT')])])) {
		...
		var_execution_time = rt.add(var_resource_usages.array_get(rt.new_string('ru_stime.tv_sec')), rt.div(var_resource_usages.array_get(rt.new_string('ru_stime.tv_usec')), rt.new_int(1000000)))
		}
	}
	return var_execution_time.clone()
}
...
fn (mut this Class_ActionScheduler_Abstract_QueueRunner) time_likely_to_be_exceeded(var_processed_actions rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('action_scheduler_maximum_execution_time_likely_to_be_exceeded'), var_likely_to_be_exceeded.clone(), rt.new_object('ActionScheduler_Abstract_QueueRunner', ['ActionScheduler_Abstract_QueueRunner_Deprecated'], &this), var_processed_actions.clone(), var_execution_time.clone(), var_max_execution_time.clone()])
}
...
fn (mut this Class_ActionScheduler_Abstract_QueueRunner) get_memory_limit() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_memory_limit)))) || rt.is_true(rt.identical(-1, var_memory_limit)) || rt.is_true(rt.identical(rt.new_string('-1'), var_memory_limit)) {
	var_memory_limit = rt.new_string('32G')
	}
	mut iife_temp_5 := Class_ActionScheduler_Compatibility{}
	mut iife_result_5 := iife_temp_5.convert_hr_to_bytes(var_memory_limit.clone())
...
fn (mut this Class_ActionScheduler_Abstract_QueueRunner) memory_exceeded() rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('action_scheduler_memory_exceeded'), var_memory_exceeded.clone(), rt.new_object('ActionScheduler_Abstract_QueueRunner', ['ActionScheduler_Abstract_QueueRunner_Deprecated'], &this)])
}

fn (mut this Class_ActionScheduler_Abstract_QueueRunner) batch_limits_exceeded(var_processed_actions rt.PhpVal) bool {
	return rt.is_true(this.memory_exceeded()) || rt.is_true(this.time_likely_to_be_exceeded(var_processed_actions.clone()))
}
...
fn create_actionscheduler_fatalerrormonitor(_args ...rt.PhpVal) &Class_ActionScheduler_FatalErrorMonitor {
