...
fn (mut this Class_ActionScheduler_StoreSchema) construct() {
	this.dispatch_set_prop('tables', rt.create_array([
		rt.ArrayItem{ key: none, val: Class_ActionScheduler_StoreSchema.actions_table() },
		rt.ArrayItem{ key: none, val: Class_ActionScheduler_StoreSchema.claims_table() },
		rt.ArrayItem{ key: none, val: Class_ActionScheduler_StoreSchema.groups_table() },
	]))
}
...
fn (mut this Class_ActionScheduler_StoreSchema) init() {
	rt.call_function('add_action', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('ActionScheduler_StoreSchema', [
				'ActionScheduler_Abstract_Schema',
			], &this) },
			rt.ArrayItem{ key: none, val: 'update_schema_5_0' },
		]),
		rt.new_int(10),
		rt.new_int(2),
	])
...
fn (mut this Class_ActionScheduler_StoreSchema) get_table_definition(var_table rt.PhpVal) string {
	mut var_wpdb := rt.new_null()
	mut var_table_name := rt.get_property(var_wpdb,
		'{"nodeType":"Expr_Variable","line":50,"name":"table"}')
...
fn (mut this Class_ActionScheduler_StoreSchema) update_schema_5_0(var_table rt.PhpVal, var_db_version rt.PhpVal) {
	mut var_wpdb := rt.new_null()
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('actionscheduler_actions'), var_table))))
		|| rt.is_true(rt.call_function('version_compare', [var_db_version.clone(), rt.new_string('5'), rt.new_string('>=')])) {
		return
	}
	mut var_table_name := rt.new_string((rt.get_property(var_wpdb, 'prefix')).str() +
		'actionscheduler_actions')
	mut var_table_list := rt.call_method(var_wpdb, 'get_col', [
...
fn (mut this Class_ActionScheduler_StoreSchema) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_table_definition' {
			...
			return rt.new_string(this.get_table_definition(dispatch_arg_0))
		}
		'update_schema_5_0' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
