...
fn (mut this Class_ActionScheduler_LoggerSchema) construct() {
	this.dispatch_set_prop('tables', rt.create_array([
		rt.ArrayItem{ key: none, val: Class_ActionScheduler_LoggerSchema.log_table() },
	]))
}
...
fn (mut this Class_ActionScheduler_LoggerSchema) init() {
	rt.call_function('add_action', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('ActionScheduler_LoggerSchema', [
				'ActionScheduler_Abstract_Schema',
			], &this) },
			rt.ArrayItem{ key: none, val: 'update_schema_3_0' },
		]),
		rt.new_int(10),
		rt.new_int(2),
	])
...
fn (mut this Class_ActionScheduler_LoggerSchema) get_table_definition(var_table rt.PhpVal) string {
	mut var_wpdb := rt.new_null()
	mut var_table_name := rt.get_property(var_wpdb,
		'{"nodeType":"Expr_Variable","line":45,"name":"table"}')
...
fn (mut this Class_ActionScheduler_LoggerSchema) update_schema_3_0(var_table rt.PhpVal, var_db_version rt.PhpVal) {
	mut var_wpdb := rt.new_null()
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('actionscheduler_logs'), var_table))))
		|| rt.is_true(rt.call_function('version_compare', [var_db_version.clone(), rt.new_string('3'), rt.new_string('>=')])) {
		return
	}
