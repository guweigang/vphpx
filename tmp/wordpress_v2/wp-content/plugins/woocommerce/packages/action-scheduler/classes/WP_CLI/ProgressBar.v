...
fn (mut this Class_Action_Scheduler_WP_CLI_ProgressBar) construct(var_message rt.PhpVal, var_count rt.PhpVal, interval i64) {
	...
	this.interval = rt.new_int(interval)
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_ProgressBar) tick() {
	...
	rt.call_function('do_action', [rt.new_string('action_scheduler/progress_tick'), rt.new_int(this.total_ticks)])
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_ProgressBar) finish() {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), this.progress_bar)))) {
...
fn create_action_scheduler_wp_cli_exception(_args ...rt.PhpVal) &Class_Action_Scheduler_WP_CLI_Exception {
