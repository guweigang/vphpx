...
fn (mut this Class_ActionScheduler_wcSystemStatus) render() {
	...
	mut var_oldest_and_newest :=
		this.get_oldest_and_newest(rt.func_array_keys(var_status_labels.clone()))
	this.get_template(var_status_labels.clone(), var_action_counts.clone(),
		var_oldest_and_newest.clone())
}
...
fn (mut this Class_ActionScheduler_wcSystemStatus) get_template(var_status_labels rt.PhpVal, var_action_counts rt.PhpVal, var_oldest_and_newest rt.PhpVal) {
	...
	mut var_as_version := rt.call_method(iife_result_0, 'latest_version', []rt.PhpVal{})
	...
	mut var_as_datastore := rt.call_function('get_class', [iife_result_1])
	...
	rt.call_function('esc_html_e', [rt.new_string('Version:'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_as_version.clone()]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Data store:'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_as_datastore.clone()]))
	// unsupported statement: Stmt_InlineHTML
	...
	rt.call_function('esc_html_e', [rt.new_string('Newest Scheduled Date'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	mut iter_2 := var_action_counts_mutated.iterator()
	for {
		...
		rt.call_function('printf', [
			...
			rt.call_function('esc_html',
				[var_oldest_and_newest_mutated.array_get(var_status).array_get(rt.new_string('oldest'))]),
			rt.call_function('esc_html',
				[var_oldest_and_newest_mutated.array_get(var_status).array_get(rt.new_string('newest'))]),
		])
	}
	// unsupported statement: Stmt_InlineHTML
}
...
fn (mut this Class_ActionScheduler_wcSystemStatus) magic_call(var_name rt.PhpVal, var_arguments rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('print'))) {
		rt.call_function('_deprecated_function', [rt.new_string(@STRUCT + '::print()'),
			rt.new_string('2.2.4'), rt.new_string(@STRUCT + '::render()')])
