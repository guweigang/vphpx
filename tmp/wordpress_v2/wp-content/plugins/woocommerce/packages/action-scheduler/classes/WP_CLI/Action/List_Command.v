...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_List_Command) execute() {
	...
	], &this), 'assoc_args').array_get(rt.new_string('fields')))) {
	...
	mut var_formatter := create_action_scheduler_wp_cli_action_wp_cli_formatter(rt.get_property(rt.new_object('Action_Scheduler_WP_CLI_Action_List_Command', [
		'Action_Scheduler_WP_CLI_Action_ActionScheduler_WPCLI_Command',
	], &this), 'assoc_args'), var_fields.clone())
	mut var_query_args := rt.get_property(rt.new_object('Action_Scheduler_WP_CLI_Action_List_Command', [
		'Action_Scheduler_WP_CLI_Action_ActionScheduler_WPCLI_Command',
	], &this), 'assoc_args')
	if rt.is_true(rt.new_bool(var_query_args.clone().array_isset(rt.new_string('claimed'))))
		&& rt.is_true(rt.identical(rt.new_string('false'), rt.new_string(var_query_args.array_get(rt.new_string('claimed')).to_string().to_lower()))) {
		var_query_args.array_set('claimed', false)
	}
	mut var_return_format := rt.new_string('OBJECT')
	if rt.is_true(rt.call_function('in_array', [rt.get_property(var_formatter, 'format'),
		...
		rt.new_bool(true)]))
	{
		var_return_format = rt.new_string("'ids'")
	}
	mut var_params := rt.call_function('var_export', [var_query_args.clone(),
		rt.new_bool(true)])
	if !rt.is_true(var_query_args) {
		var_params = rt.new_string('array()')
	}
	mut iife_temp_2 := Class_Action_Scheduler_WP_CLI_Action_WP_CLI{}
	mut iife_result_2 := iife_temp_2.debug(rt.call_function('sprintf', [
		rt.new_string('as_get_scheduled_actions( %s, %s )'),
		var_params.clone(),
		var_return_format.clone(),
	]))
	if !(!rt.is_true(var_query_args.array_get(rt.new_string('args')))) {
		var_query_args.array_set('args', rt.call_function('json_decode', [
			var_query_args.array_get(rt.new_string('args')),
			rt.new_bool(true),
		]))
	}
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('ids'))) {
		mut var_actions := rt.call_function('as_get_scheduled_actions', [
			var_query_args.clone(), rt.new_string('ids')])
		...
		var_actions = rt.call_function('as_get_scheduled_actions', [
			var_query_args.clone(), rt.new_string('ids')])
		var_formatter.display_items(var_actions.clone())
	} else {
		var_actions = rt.call_function('as_get_scheduled_actions', [
			var_query_args.clone(), rt.get_constant('OBJECT')])
		...
		for {
			...
			mut var_action_arr := rt.create_array([
				...
				rt.ArrayItem{
					key: 'recurring'
					val: if rt.is_true(rt.call_method(rt.call_method(var_action, 'get_schedule',
						[]rt.PhpVal{}), 'is_recurring', []rt.PhpVal{}))
					{
						'yes'
					} else {
						'no'
			...
			for {
				...
				var_action_arr.array_get_mut('log_entries').array_push(rt.create_array([
					...
					rt.ArrayItem{ key: 'message', val: rt.call_method(var_log_entry, 'get_message',
						[]rt.PhpVal{}) },
				]))
			}
			var_actions_arr.array_push(var_action_arr.clone())
		}
		var_formatter.display_items(var_actions_arr.clone())
	}
}
