...
fn (mut this Class_ActionScheduler_CronSchedule) construct(mut var_start Class_DateTime, var_recurrence rt.PhpVal, mut var_first Class_?DateTime) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [var_recurrence_mutated.clone(), rt.new_string('CronExpression')]))))) {
	mut iife_temp_0 := Class_CronExpression{}
	mut iife_result_0 := iife_temp_0.factory(var_recurrence_mutated.clone())
	var_recurrence_mutated = iife_result_0
	}
	...
	this.Class_ActionScheduler_Abstract_RecurringSchedule.construct(var_date.clone(), var_recurrence_mutated.clone(), rt.new_object('?DateTime', []string{}, var_first_mutated))
}
...
fn (mut this Class_ActionScheduler_CronSchedule) magic_sleep() rt.PhpVal {
	...
	return rt.call_function('array_merge', [var_sleep_params.clone(), rt.create_array([rt.ArrayItem{ key: none, val: 'start_timestamp' }, rt.ArrayItem{ key: none, val: 'cron' }])])
}

fn (mut this Class_ActionScheduler_CronSchedule) magic_wakeup() {
	if rt.get_property(rt.new_object('ActionScheduler_CronSchedule', ['ActionScheduler_Abstract_RecurringSchedule', 'ActionScheduler_Schedule'], &this), 'scheduled_timestamp').is_null() && !(this.start_timestamp.is_null()) {
		this.dispatch_set_prop('scheduled_timestamp', this.start_timestamp)
		this.start_timestamp = rt.new_null()
	}
	if rt.get_property(rt.new_object('ActionScheduler_CronSchedule', ['ActionScheduler_Abstract_RecurringSchedule', 'ActionScheduler_Schedule'], &this), 'recurrence').is_null() && !(this.cron.is_null()) {
