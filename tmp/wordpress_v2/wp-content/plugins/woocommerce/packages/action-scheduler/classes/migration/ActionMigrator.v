...
fn (mut this Class_Action_Scheduler_Migration_ActionMigrator) construct(mut var_source_store Class_Action_Scheduler_Migration_ActionScheduler_Store, mut var_destination_store Class_Action_Scheduler_Migration_ActionScheduler_Store, mut var_log_migrator Class_Action_Scheduler_Migration_LogMigrator) {
	this.source = var_source_store
	this.destination = var_destination_store
	this.log_migrator = var_log_migrator
}

fn (mut this Class_Action_Scheduler_Migration_ActionMigrator) migrate(var_source_action_id rt.PhpVal) i64 {
	mut var_action := rt.call_method(this.source, 'fetch_action', [
	...
	mut var_status := rt.call_method(this.source, 'get_status', [
		var_source_action_id.clone()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
	...
	end_label_1:
	if var_action.clone().is_null() || !rt.is_true(var_status)
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(rt.call_method(var_action, 'get_schedule', []rt.PhpVal{}), 'get_date', []rt.PhpVal{}))))) {
		rt.call_method(this.source, 'delete_action', [var_source_action_id.clone()])
		if rt.has_exception() {
			unsafe {
				goto catch_label_2
			}
	...
	mut var_last_attempt_date := if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(Class_Action_Scheduler_Migration_ActionScheduler_Store.status_pending(), var_status)))) { rt.call_method(this.source, 'get_date', [
			var_source_action_id.clone(),
		]) } else { rt.new_null() }
	if rt.has_exception() {
		unsafe {
	...
	mut var_destination_action_id := rt.call_method(this.destination, 'save_action', [
		var_action.clone(),
		rt.new_null(),
		var_last_attempt_date.clone(),
	])
	if rt.has_exception() {
		unsafe {
			goto catch_label_3
	...
	if rt.is_true(rt.equal(switch_val_1,
		Class_Action_Scheduler_Migration_ActionScheduler_Store.status_failed()))
	{
		rt.call_method(this.destination, 'mark_failure', [var_destination_action_id.clone()])
		if rt.has_exception() {
			unsafe {
				goto catch_label_4
			}
		...
		Class_Action_Scheduler_Migration_ActionScheduler_Store.status_canceled()))
	{
		rt.call_method(this.destination, 'cancel_action', [var_destination_action_id.clone()])
		if rt.has_exception() {
			unsafe {
				goto catch_label_4
			}
	...
	rt.call_method(this.log_migrator, 'migrate', [var_source_action_id.clone(),
		var_destination_action_id.clone()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_4
		}
	...
	mut var_test_action := rt.call_method(this.source, 'fetch_action', [
		var_source_action_id.clone()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_4
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [
		var_test_action.clone(), rt.new_string('ActionScheduler_NullAction')])))))
	{
		rt.throw_exception(rt.new_object('Action_Scheduler_Migration_RuntimeException', []string{}, create_action_scheduler_migration_runtimeexception(rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('Unable to remove source migrated action %s'),
	...
	rt.call_function('do_action', [rt.new_string('action_scheduler/migrated_action'),
		var_source_action_id.clone(), var_destination_action_id.clone(), this.source, this.destination])
	if rt.has_exception() {
		unsafe {
			goto catch_label_4
		}
	...
	if rt.instance_of(var_e_4, 'Action_Scheduler_Migration_Exception') {
		var_e = var_e_4.clone()
		rt.call_method(this.source, 'mark_migrated', [var_source_action_id.clone()])
		rt.call_function('do_action', [
...
fn (mut this Class_Action_Scheduler_Migration_ActionMigrator) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Action_Scheduler_Migration_ActionScheduler_Store](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_Action_Scheduler_Migration_ActionScheduler_Store](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_2 := rt.cast_object_ptr[Class_Action_Scheduler_Migration_LogMigrator](if args.len > 2 {
				args[2]
			} else {
				rt.new_null()
			})
