...
fn (mut this Class_ActionScheduler_DBLogger) log(var_action_id rt.PhpVal, var_message rt.PhpVal, mut var_date Class_?DateTime) rt.PhpVal {
	...
	var_date_mutated = var_date_mutated.dup()
	}
...
fn (mut this Class_ActionScheduler_DBLogger) get_entry(var_entry_id rt.PhpVal) rt.PhpVal {
	mut var_wpdb := rt.new_null()
	mut var_entry := rt.call_method(var_wpdb, 'get_row', [rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.new_string('SELECT * FROM '), rt.get_property(var_wpdb, 'actionscheduler_logs')), rt.new_string(' WHERE log_id=%d')), var_entry_id.clone()])])
	return this.create_entry_from_db_record(var_entry.clone())
}
...
fn (mut this Class_ActionScheduler_DBLogger) create_entry_from_db_record(var_record rt.PhpVal) rt.PhpVal {
	...
	return rt.new_object('ActionScheduler_LogEntry', []string{}, create_actionscheduler_logentry(rt.get_property(var_record, 'action_id'), rt.get_property(var_record, 'message'), var_date.clone()))
}

fn (mut this Class_ActionScheduler_DBLogger) get_logs(var_action_id rt.PhpVal) rt.PhpVal {
	mut var_wpdb := rt.new_null()
	mut var_records := rt.call_method(var_wpdb, 'get_results', [rt.call_method(var_wpdb, 'prepare', [rt.concat(rt.concat(rt.new_string('SELECT * FROM '), rt.get_property(var_wpdb, 'actionscheduler_logs')), rt.new_string(' WHERE action_id=%d')), var_action_id.clone()])])
	return rt.call_function('array_map', [rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('ActionScheduler_DBLogger', ['ActionScheduler_Logger'], &this) }, rt.ArrayItem{ key: none, val: 'create_entry_from_db_record' }]), var_records.clone()])
}
...
fn (mut this Class_ActionScheduler_DBLogger) bulk_log_cancel_actions(var_action_ids rt.PhpVal) {
	...
	mut var_format := rt.new_string('(%d, ' + (rt.call_method(var_wpdb, 'prepare', [rt.new_string('%s, %s, %s'), var_message.clone(), var_date_gmt.clone(), var_date_local.clone()])).str() + ')')
	mut var_sql_query := rt.new_string((rt.concat(rt.concat(rt.new_string('INSERT '), rt.get_property(var_wpdb, 'actionscheduler_logs')), rt.new_string(' (action_id, message, log_date_gmt, log_date_local) VALUES '))).str())
	mut var_value_rows := []rt.PhpVal{}
	mut iter_1 := var_action_ids.iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_action_id := item_1.val
		var_value_rows << rt.call_method(var_wpdb, 'prepare', [var_format.clone(), var_action_id.clone()])
	}
	var_sql_query = rt.concat(var_sql_query, rt.call_function('implode', [rt.new_string(','), rt.create_array_from_list(var_value_rows)]))
	rt.call_method(var_wpdb, 'query', [var_sql_query.clone()])
}
...
fn create_actionscheduler_dblogger(_args ...rt.PhpVal) &Class_ActionScheduler_DBLogger {
