...
fn (mut this Class_ActionScheduler_WPCLI_Command) construct(mut var_args Class_array, mut var_assoc_args Class_array) {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('WP_CLI')])))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('constant', [rt.new_string('WP_CLI')]))))) {
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('The %s class can only be run within WP CLI.'),
				rt.new_string('woocommerce'),
			]),
			rt.call_function('get_class', [
				rt.new_object('ActionScheduler_WPCLI_Command', ['WP_CLI_Command'], &this),
			]),
		]))))
	}
	this.args = var_args
	this.assoc_args = var_assoc_args
}
...
fn (mut this Class_ActionScheduler_WPCLI_Command) get_schedule_display_string(mut var_schedule Class_ActionScheduler_Schedule) string {
	...
	var_schedule_display_string = rt.concat(var_schedule_display_string, rt.call_method(var_schedule.get_date(),
		'format', [Class_static.date_format()]))
...
fn (mut this Class_ActionScheduler_WPCLI_Command) process_csv_arguments_to_arrays() {
	...
	for {
		...
		{
			this.assoc_args.array_set(var_k, rt.call_function('explode', [
				rt.new_string(','),
				var_v.clone(),
			]))
		}
