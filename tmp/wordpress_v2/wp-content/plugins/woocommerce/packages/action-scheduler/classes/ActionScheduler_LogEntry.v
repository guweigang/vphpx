...
fn (mut this Class_ActionScheduler_LogEntry) construct(var_action_id rt.PhpVal, var_message rt.PhpVal, var_date rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), var_date_mutated))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [var_date_mutated.clone(), rt.new_string('DateTime')]))))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.new_string('The third parameter must be a valid DateTime instance, or null.'),
			rt.new_string('2.0.0')])
