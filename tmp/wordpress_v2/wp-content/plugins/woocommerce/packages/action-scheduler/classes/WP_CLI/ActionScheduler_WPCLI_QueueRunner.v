...
fn (mut this Class_ActionScheduler_WPCLI_QueueRunner) setup(var_batch_size rt.PhpVal, var_hooks rt.PhpVal, group string, force bool) i64 {
	...
	if rt.is_true(this.has_maximum_concurrent_batches()) {
		...
		mut iife_result_0 := iife_temp_0.warning(rt.call_function('__', [rt.new_string('There are too many concurrent batches, but the run is forced to continue.'), rt.new_string('woocommerce')]))
		} else {
		mut iife_temp_1 := Class_WP_CLI{}
		mut iife_result_1 := iife_temp_1.error(rt.call_function('__', [rt.new_string('There are too many concurrent batches.'), rt.new_string('woocommerce')]))
		}
	}
	this.claim = rt.call_method(rt.get_property(rt.new_object('ActionScheduler_WPCLI_QueueRunner', ['ActionScheduler_Abstract_QueueRunner'], &this), 'store'), 'stake_claim', [var_batch_size.clone(), rt.new_null(), var_hooks.clone(), rt.new_string(group)])
...
fn (mut this Class_ActionScheduler_WPCLI_QueueRunner) setup_progress_bar() {
	mut var_count := rt.new_int(this.actions.array_count())
	this.progress_bar = create_action_scheduler_wp_cli_progressbar(rt.call_function('sprintf', [rt.call_function('_n', [rt.new_string('Running %d action'), rt.new_string('Running %d actions'), var_count.clone(), rt.new_string('woocommerce')]), var_count.clone()]), var_count.clone())
}
...
fn (mut this Class_ActionScheduler_WPCLI_QueueRunner) run(context string) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_action_id.clone(), rt.call_method(rt.get_property(rt.new_object('ActionScheduler_WPCLI_QueueRunner', ['ActionScheduler_Abstract_QueueRunner'], &this), 'store'), 'find_actions_by_claim_id', [rt.call_method(this.claim, 'get_id', []rt.PhpVal{})]), rt.new_bool(true)]))))) {
			mut iife_temp_2 := Class_WP_CLI{}
			mut iife_result_2 := iife_temp_2.warning(rt.call_function('__', [rt.new_string('The claim has been lost. Aborting current batch.'), rt.new_string('woocommerce')]))
			break
		}
...
mut iife_result_3 := iife_temp_3.log(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Started processing action %s'), rt.new_string('woocommerce')]), var_action_id.clone()]))
}
...
fn (mut this Class_ActionScheduler_WPCLI_QueueRunner) after_execute(var_action_id rt.PhpVal, var_action rt.PhpVal) {
	...
	var_action_mutated = rt.call_method(rt.get_property(rt.new_object('ActionScheduler_WPCLI_QueueRunner', ['ActionScheduler_Abstract_QueueRunner'], &this), 'store'), 'fetch_action', [var_action_id.clone()])
	}
mut iife_temp_4 := Class_WP_CLI{}
mut iife_result_4 := iife_temp_4.log(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Completed processing action %1$s with hook: %2$s'), rt.new_string('woocommerce')]), var_action_id.clone(), rt.call_method(var_action_mutated, 'get_hook', []rt.PhpVal{})]))
}
...
mut iife_result_5 := iife_temp_5.error(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Error processing action %1$s: %2$s'), rt.new_string('woocommerce')]), var_action_id.clone(), rt.call_method(var_exception, 'getMessage', []rt.PhpVal{})]), rt.new_bool(false))
}
...
mut iife_result_6 := iife_temp_6.free_memory()
}

fn (mut this Class_ActionScheduler_WPCLI_QueueRunner) maybe_stop_the_insanity() {
	mut var_current_iteration := rt.new_int(rt.new_string(rt.call_method(this.progress_bar, 'current', []rt.PhpVal{}).to_string().trim_space()).to_i64())
