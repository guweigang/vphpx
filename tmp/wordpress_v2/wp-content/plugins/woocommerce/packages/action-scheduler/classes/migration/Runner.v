...
fn (mut this Class_Action_Scheduler_Migration_Runner) run(batch_size i64) i64 {
	...
	mut var_batch := rt.call_method(this.batch_fetcher, 'fetch', [
		rt.new_int(batch_size_mutated).clone()])
	batch_size_mutated = var_batch.clone().array_count()
		...
		rt.call_method(this.progress_bar, 'set_message', [
			rt.call_function('sprintf', [
				rt.call_function('_n', [rt.new_string('Migrating %d action'),
					rt.new_string('Migrating %d actions'), rt.new_int(batch_size_mutated).clone(),
					rt.new_string('woocommerce')]),
				rt.new_int(batch_size_mutated).clone(),
			]),
		])
		rt.call_method(this.progress_bar, 'set_count', [rt.new_int(batch_size_mutated).clone()])
	}
...
fn (mut this Class_Action_Scheduler_Migration_Runner) migrate_actions(mut var_action_ids Class_Action_Scheduler_Migration_array) {
	...
	mut iife_temp_0 := Class_Action_Scheduler_Migration_ActionScheduler{}
	mut iife_result_0 := iife_temp_0.logger()
	rt.call_method(iife_result_0, 'unhook_stored_action', []rt.PhpVal{})
	...
	for {
		...
		if rt.is_true(var_destination_action_id) {
			rt.call_method(this.destination_logger, 'log', [var_destination_action_id.clone(),
				rt.call_function('sprintf', [
					rt.call_function('__', [
	...
	mut iife_temp_1 := Class_Action_Scheduler_Migration_ActionScheduler{}
	mut iife_result_1 := iife_temp_1.logger()
	rt.call_method(iife_result_1, 'hook_stored_action', []rt.PhpVal{})
	rt.call_function('do_action', [
...
fn create_action_scheduler_migration_batchfetcher(_args ...rt.PhpVal) &Class_Action_Scheduler_Migration_BatchFetcher {
...
fn create_action_scheduler_migration_dryrun_logmigrator(_args ...rt.PhpVal) &Class_Action_Scheduler_Migration_DryRun_LogMigrator {
...
fn create_action_scheduler_migration_logmigrator(_args ...rt.PhpVal) &Class_Action_Scheduler_Migration_LogMigrator {
...
fn create_action_scheduler_migration_actionscheduler(_args ...rt.PhpVal) &Class_Action_Scheduler_Migration_ActionScheduler {
