...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Create_Command) execute() {
	...
	mut var_schedule_start := rt.get_property(rt.new_object('Action_Scheduler_WP_CLI_Action_Create_Command', [
		'Action_Scheduler_WP_CLI_Action_ActionScheduler_WPCLI_Command',
	], &this), 'args').array_get(rt.new_int(1))
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_schedule_start.clone(), Class_Action_Scheduler_WP_CLI_Action_static.async_opts(),
		rt.new_bool(true)])))))
	{
		var_schedule_start = rt.call_function('as_get_datetime_object', [
			var_schedule_start.clone()])
		if rt.has_exception() {
			unsafe {
				goto catch_label_1
			}
	...
	if rt.instance_of(var_e_1, 'Action_Scheduler_WP_CLI_Action_Exception') {
		...
		mut iife_result_0 := iife_temp_0.error(var_e.getmessage())
		unsafe {
			goto end_label_1
		}
	} else {
	...
	end_label_1:
	mut var_action_type := rt.new_string('single')
	mut var_function := rt.new_string('as_schedule_single_action')
	if !(!rt.is_true(var_interval)) {
		var_action_type = rt.new_string('recurring')
		var_function = rt.new_string('as_schedule_recurring_action')
		closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_key := if args.len > 0 { args[0].clone() } else { rt.new_null() }
			return
		}
		var_function_args = rt.call_function('array_filter', [
			var_function_args.clone(), rt.new_closure(closure_2_fn),
			rt.get_constant('ARRAY_FILTER_USE_KEY')])
	} else if !(!rt.is_true(var_cron)) {
		var_action_type = rt.new_string('cron')
		var_function = rt.new_string('as_schedule_cron_action')
		closure_3_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_key := if args.len > 0 { args[0].clone() } else { rt.new_null() }
			return
		}
		var_function_args = rt.call_function('array_filter', [
			var_function_args.clone(), rt.new_closure(closure_3_fn),
			rt.get_constant('ARRAY_FILTER_USE_KEY')])
	} else if rt.is_true(rt.call_function('in_array', [
		var_function_args.array_get(rt.new_string('start')),
		Class_Action_Scheduler_WP_CLI_Action_static.async_opts(),
		rt.new_bool(true),
	]))
	{
		var_action_type = rt.new_string('async')
		var_function = rt.new_string('as_enqueue_async_action')
		closure_4_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			mut var_key := if args.len > 0 { args[0].clone() } else { rt.new_null() }
			return
		}
		var_function_args = rt.call_function('array_filter', [
			var_function_args.clone(), rt.new_closure(closure_4_fn),
			rt.get_constant('ARRAY_FILTER_USE_KEY')])
	} else {
		...
		var_function_args = rt.call_function('array_filter', [
			var_function_args.clone(), rt.new_closure(closure_5_fn),
			rt.get_constant('ARRAY_FILTER_USE_KEY')])
	}
	...
	mut var_action_id := rt.call_function('call_user_func_array', [
		var_function.clone(), var_function_args.clone()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_2
		}
	...
	this.print_success(var_action_id.clone(), var_action_type.clone())
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Create_Command) print_success(var_action_id rt.PhpVal, var_action_type rt.PhpVal) {
	...
	mut iife_result_5 := iife_temp_5.success(rt.call_function('sprintf', [
		rt.call_function('__', [rt.new_string('%1$s action (%2$d) scheduled.'),
			rt.new_string('woocommerce')]),
		rt.call_function('ucfirst', [var_action_type_mutated.clone()]),
		var_action_id_mutated.clone(),
	]))
}
...
fn (mut this Class_Action_Scheduler_WP_CLI_Action_Create_Command) print_error(mut var_e Class_Action_Scheduler_WP_CLI_Action_Exception) {
	...
	mut iife_result_6 := iife_temp_6.error(rt.call_function('sprintf', [
		rt.call_function('__', [
...
fn create_action_scheduler_wp_cli_action_exception(_args ...rt.PhpVal) &Class_Action_Scheduler_WP_CLI_Action_Exception {
