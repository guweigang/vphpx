...
fn main() {
	...
	if rt.is_true(var_order) {
		rt.call_function('do_action', [rt.new_string('woocommerce_before_thankyou'),
			...
			rt.new_string('failed')]))
		{
			...
			rt.call_function('esc_html_e', [
				rt.new_string('Unfortunately your order cannot be processed as the originating bank/merchant has declined your transaction. Please attempt your purchase again.'),
			...
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('esc_html_e', [rt.new_string('Pay'),
				rt.new_string('woocommerce')])
			// unsupported statement: Stmt_InlineHTML
			if rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{})) {
				...
				// unsupported statement: Stmt_InlineHTML
				rt.call_function('esc_html_e', [rt.new_string('My account'),
					rt.new_string('woocommerce')])
				// unsupported statement: Stmt_InlineHTML
			}
			// unsupported statement: Stmt_InlineHTML
			...
			rt.call_function('esc_html_e', [rt.new_string('Date:'),
				rt.new_string('woocommerce')])
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('wc_format_datetime', [
				rt.call_method(var_order, 'get_date_created', []rt.PhpVal{}),
			]))
			// unsupported statement: Stmt_InlineHTML
			if rt.is_true(rt.call_function('is_user_logged_in', []rt.PhpVal{}))
				&& rt.is_true(rt.identical(rt.call_method(var_order, 'get_user_id', []rt.PhpVal{}), rt.call_function('get_current_user_id', []rt.PhpVal{})))
				&& rt.is_true(rt.call_method(var_order, 'get_billing_email', []rt.PhpVal{})) {
				// unsupported statement: Stmt_InlineHTML
				rt.call_function('esc_html_e', [rt.new_string('Email:'),
					rt.new_string('woocommerce')])
				// unsupported statement: Stmt_InlineHTML
		...
		rt.call_function('do_action', [
			rt.new_string('woocommerce_thankyou_' +
				(rt.call_method(var_order, 'get_payment_method', []rt.PhpVal{})).str()),
			rt.call_method(var_order, 'get_id', []rt.PhpVal{}),
		])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('do_action', [rt.new_string('woocommerce_thankyou'),
			rt.call_method(var_order, 'get_id', []rt.PhpVal{})])
		// unsupported statement: Stmt_InlineHTML
	} else {
		// unsupported statement: Stmt_InlineHTML
