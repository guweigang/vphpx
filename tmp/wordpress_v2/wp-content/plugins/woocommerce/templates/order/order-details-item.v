...
fn main() {
	...
	rt.echo_val(rt.call_function('esc_attr', [
		rt.call_function('apply_filters', [rt.new_string('woocommerce_order_item_class'),
			rt.new_string('woocommerce-table__line-item order_item'),
			var_item.clone(), var_order.clone()]),
	]))
	// unsupported statement: Stmt_InlineHTML
	...
	mut var_product_permalink := rt.call_function('apply_filters', [
		...
		if var_is_visible { rt.call_method(var_product, 'get_permalink', [
				var_item.clone()]) } else { rt.new_string('') },
		var_item.clone(),
		var_order.clone(),
	])
	rt.echo_val(rt.call_function('wp_kses_post', [
		rt.call_function('apply_filters', [rt.new_string('woocommerce_order_item_name'),
			if rt.is_true(var_product_permalink) { rt.call_function('sprintf', [
				...
				]) } else { rt.call_method(var_item, 'get_name', []rt.PhpVal{}) }, var_item.clone(),
	...
	rt.call_function('do_action', [rt.new_string('woocommerce_order_item_meta_start'),
		var_item_id.clone(), var_item.clone(), var_order.clone(),
		rt.new_bool(false)])
	...
	if rt.is_true(var_show_purchase_note) && rt.is_true(var_purchase_note) {
		...
		rt.echo_val(rt.call_function('wpautop', [
			rt.call_function('do_shortcode', [
				rt.call_function('wp_kses_post', [var_purchase_note.clone()]),
			]),
		]))
		// unsupported statement: Stmt_InlineHTML
	}
