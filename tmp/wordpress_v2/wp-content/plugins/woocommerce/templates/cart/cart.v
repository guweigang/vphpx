...
fn main() {
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Remove item'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Thumbnail image'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	...
	for {
		...
		mut var_product_id := rt.call_function('apply_filters', [
			rt.new_string('woocommerce_cart_item_product_id'),
			var_cart_item.array_get(rt.new_string('product_id')),
			var_cart_item.clone(),
			var_cart_item_key.clone(),
		])
		mut var_visible := rt.call_function('apply_filters', [
			rt.new_string('woocommerce_cart_item_visible'),
			rt.new_bool(true),
			var_cart_item.clone(),
			var_cart_item_key.clone(),
		])
		if rt.is_true(rt.new_bool(rt.instance_of(var__product, 'WC_Product')))
			...
			&& rt.is_true(var_visible) {
			mut var_product_name := rt.call_function('apply_filters', [
				rt.new_string('woocommerce_cart_item_name'),
			...
			mut var_product_permalink := rt.call_function('apply_filters', [
				...
				if rt.is_true(rt.call_method(var__product, 'is_visible', []rt.PhpVal{})) { rt.call_method(var__product, 'get_permalink', [
						var_cart_item.clone(),
					]) } else { rt.new_string('') },
				var_cart_item.clone(),
				var_cart_item_key.clone(),
			...
			rt.echo_val(rt.call_function('apply_filters', [
				rt.new_string('woocommerce_cart_item_remove_link'),
				rt.call_function('sprintf', [
					rt.new_string('<a role="button" href="%s" class="remove" aria-label="%s" data-product_id="%s" data-product_sku="%s">&times;</a>'),
					rt.call_function('esc_url', [
						rt.call_function('wc_get_cart_remove_url', [
							var_cart_item_key.clone(),
						]),
					]),
					rt.call_function('esc_attr', [
						rt.call_function('sprintf', [
							rt.call_function('__', [rt.new_string('Remove %s from cart'),
								rt.new_string('woocommerce')]),
					...
					rt.call_function('esc_attr', [
						rt.call_method(var__product, 'get_sku', []rt.PhpVal{}),
					]),
				]),
				var_cart_item_key.clone(),
			]))
			// unsupported statement: Stmt_InlineHTML
			mut var_thumbnail := rt.call_function('apply_filters', [
				rt.new_string('woocommerce_cart_item_thumbnail'),
				rt.call_method(var__product, 'get_image', []rt.PhpVal{}),
				var_cart_item.clone(),
				var_cart_item_key.clone(),
			])
			...
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('esc_attr_e', [rt.new_string('Product'),
				rt.new_string('woocommerce')])
			// unsupported statement: Stmt_InlineHTML
			if rt.is_true(rt.new_bool(!(rt.is_true(var_product_permalink)))) {
				rt.echo_val(rt.call_function('wp_kses_post', [
					rt.new_string(var_product_name.str() + '&nbsp;'),
				]))
			} else {
				rt.echo_val(rt.call_function('wp_kses_post', [
					rt.call_function('apply_filters', [
			...
			rt.call_function('do_action', [
				rt.new_string('woocommerce_after_cart_item_name'),
				var_cart_item.clone(),
				var_cart_item_key.clone(),
			])
			rt.echo_val(rt.call_function('wc_get_formatted_cart_item_data', [
				var_cart_item.clone()]))
			if rt.is_true(rt.call_method(var__product, 'backorders_require_notification', []rt.PhpVal{}))
				&& rt.is_true(rt.call_method(var__product, 'is_on_backorder', [var_cart_item.array_get(rt.new_string('quantity'))])) {
				rt.echo_val(rt.call_function('wp_kses_post', [
					rt.call_function('apply_filters', [
						rt.new_string('woocommerce_cart_item_backorder_notification'),
						rt.new_string('<p class="backorder_notification">' +
							(rt.call_function('esc_html__', [rt.new_string('Available on backorder'), rt.new_string('woocommerce')])).str() +
							'</p>'),
						var_product_id.clone(),
					]),
				]))
			...
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('esc_attr_e', [rt.new_string('Price'),
				rt.new_string('woocommerce')])
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('apply_filters', [
				rt.new_string('woocommerce_cart_item_price'),
				rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'cart'),
					'get_product_price', [var__product.clone()]),
				var_cart_item.clone(),
				var_cart_item_key.clone(),
			]))
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('esc_attr_e', [rt.new_string('Quantity'),
				rt.new_string('woocommerce')])
			// unsupported statement: Stmt_InlineHTML
			if rt.is_true(rt.call_method(var__product, 'is_sold_individually', []rt.PhpVal{})) {
				mut var_min_quantity := 1
				mut var_max_quantity := rt.new_int(1)
			} else {
				var_min_quantity = 0
				var_max_quantity = rt.call_method(var__product, 'get_max_purchase_quantity',
					[]rt.PhpVal{})
			}
			mut var_product_quantity := rt.call_function('woocommerce_quantity_input', [
				rt.create_array([
					rt.ArrayItem{
						key: 'input_name'
						val: 'cart[${var_cart_item_key.to_string()}][qty]'
					},
					rt.ArrayItem{
						key: 'input_value'
						val: var_cart_item.array_get(rt.new_string('quantity'))
					},
					rt.ArrayItem{ key: 'max_value', val: var_max_quantity },
					rt.ArrayItem{ key: 'min_value', val: var_min_quantity },
			...
			rt.echo_val(rt.call_function('apply_filters', [
				rt.new_string('woocommerce_cart_item_quantity'),
				var_product_quantity.clone(),
				var_cart_item_key.clone(),
				var_cart_item.clone(),
			]))
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('esc_attr_e', [rt.new_string('Subtotal'),
				rt.new_string('woocommerce')])
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('apply_filters', [
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_attr_e', [rt.new_string('Update cart'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Update cart'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [rt.new_string('woocommerce_cart_actions')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('wp_nonce_field', [rt.new_string('woocommerce-cart'),
		rt.new_string('woocommerce-cart-nonce')])
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [rt.new_string('woocommerce_cart_collaterals')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [rt.new_string('woocommerce_after_cart')])
}
