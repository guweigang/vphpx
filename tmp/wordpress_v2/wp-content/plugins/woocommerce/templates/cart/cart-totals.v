...
fn main() {
	...
	print(if rt.is_true(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}),
		'customer'), 'has_calculated_shipping', []rt.PhpVal{}))
	{
		'calculated_shipping'
	} else {
		''
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Cart totals'),
	...
	for {
		...
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [
			rt.call_function('wc_cart_totals_coupon_label', [
				var_coupon.clone(), rt.new_bool(false)]),
		]))
		// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'cart'), 'needs_shipping', []rt.PhpVal{}))
		...
		rt.call_function('do_action', [
			rt.new_string('woocommerce_cart_totals_before_shipping'),
		])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('wc_cart_totals_shipping_html', []rt.PhpVal{})
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('do_action', [
			rt.new_string('woocommerce_cart_totals_after_shipping'),
		])
		// unsupported statement: Stmt_InlineHTML
	} else if
		rt.is_true(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'cart'), 'needs_shipping', []rt.PhpVal{}))
		&& rt.is_true(rt.identical(rt.new_string('yes'), rt.call_function('get_option', [rt.new_string('woocommerce_enable_shipping_calc')]))) {
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_html_e', [rt.new_string('Shipping'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_attr_e', [rt.new_string('Shipping'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(rt.call_function('wc_tax_enabled', []rt.PhpVal{}))
		...
		mut var_estimated_text := rt.new_string('')
		if rt.is_true(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'customer'), 'is_customer_outside_base', []rt.PhpVal{}))
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'customer'), 'has_calculated_shipping', []rt.PhpVal{}))))) {
			var_estimated_text = rt.call_function('sprintf', [
				rt.new_string(' <small>' +
					(rt.call_function('esc_html__', [rt.new_string('(estimated for %s)'), rt.new_string('woocommerce')])).str() +
					'</small>'),
				rt.new_string(
					(rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'estimated_for_prefix', [var_taxable_address.array_get(rt.new_int(0))])).str() +(rt.get_property(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'countries').array_get(var_taxable_address.array_get(rt.new_int(0)))).str()),
			])
		}
		...
		])))
		{
			...
			for {
				...
				// unsupported statement: Stmt_InlineHTML
				print((rt.call_function('esc_html', [rt.get_property(var_tax, 'label')])).str() +
					var_estimated_text.str())
				// unsupported statement: Stmt_InlineHTML
				rt.echo_val(rt.call_function('esc_attr', [
			...
			// unsupported statement: Stmt_InlineHTML
			print(
				(rt.call_function('esc_html', [rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'tax_or_vat', []rt.PhpVal{})])).str() +
				var_estimated_text.str())
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('esc_attr', [
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [
		rt.new_string('woocommerce_cart_totals_before_order_total'),
	])
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('do_action', [
		rt.new_string('woocommerce_cart_totals_after_order_total'),
	])
	// unsupported statement: Stmt_InlineHTML
