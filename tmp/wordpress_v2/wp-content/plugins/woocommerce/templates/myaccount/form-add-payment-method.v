...
fn main() {
	...
	if rt.is_true(var_available_gateways) {
		...
		if rt.is_true(rt.new_int(var_available_gateways.clone().array_count())) {
			rt.call_method(rt.call_function('current', [var_available_gateways.clone()]),
				'set_current', []rt.PhpVal{})
		}
		...
		for {
			...
			rt.echo_val(rt.call_function('wp_kses_post', [
				rt.call_method(var_gateway, 'get_title', []rt.PhpVal{}),
			]))
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('wp_kses_post', [
				rt.call_method(var_gateway, 'get_icon', []rt.PhpVal{}),
			]))
			// unsupported statement: Stmt_InlineHTML
			if rt.is_true(rt.call_method(var_gateway, 'has_fields', []rt.PhpVal{}))
				|| rt.is_true(rt.call_method(var_gateway, 'get_description', []rt.PhpVal{})) {
				print('<div class="woocommerce-PaymentBox woocommerce-PaymentBox--' +
					(rt.call_function('esc_attr', [rt.get_property(var_gateway, 'id')])).str() +
					' payment_box payment_method_' +
					(rt.call_function('esc_attr', [rt.get_property(var_gateway, 'id')])).str() +
					'" style="display: none;">')
		...
		rt.call_function('do_action', [
			rt.new_string('woocommerce_add_payment_method_form_bottom'),
		])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('wp_nonce_field', [
			rt.new_string('woocommerce-add-payment-method'),
			rt.new_string('woocommerce-add-payment-method-nonce'),
		])
		// unsupported statement: Stmt_InlineHTML
		...
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_attr_e', [rt.new_string('Add payment method'),
		...
		rt.call_function('wc_print_notice', [
			rt.call_function('esc_html__', [
				rt.new_string('New payment methods can only be added during checkout. Please contact us if you require assistance.'),
				rt.new_string('woocommerce'),
			]),
