...
fn main() {
	...
	mut var_heading_class := if rt.is_true(var_email_improvements_enabled) {
		'email-order-detail-heading'
	} else {
		''
	}
	mut var_order_table_class := if rt.is_true(var_email_improvements_enabled) {
		'email-order-details'
	} else {
		''
	}
	mut var_order_total_text_align := if rt.is_true(var_email_improvements_enabled) {
		'right'
	} else {
		'left'
	}
	mut var_order_quantity_text_align := if rt.is_true(var_email_improvements_enabled) {
		'right'
	} else {
		'left'
	}
	...
	mut var_order_details_heading := rt.new_string('')
	if rt.is_true(var_email_improvements_enabled) {
		var_order_details_heading = rt.call_function('apply_filters', [
			rt.new_string('woocommerce_email_order_details_heading'),
			rt.call_function('__', [rt.new_string('Order summary'),
				rt.new_string('woocommerce')]),
			var_order.clone(),
			var_email.clone(),
		])
	}
	mut var_display_order_number := rt.new_bool((rt.call_function('apply_filters', [
		rt.new_string('woocommerce_email_display_order_number'),
		rt.new_bool(true),
		var_order.clone(),
		var_email.clone(),
	])).to_bool())
	if rt.is_true(var_order_details_heading) || rt.is_true(var_display_order_number) {
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [rt.new_string(var_heading_class.str()).clone()]))
		// unsupported statement: Stmt_InlineHTML
		if rt.is_true(var_order_details_heading) {
			rt.echo_val(rt.call_function('wp_kses_post', [var_order_details_heading.clone()]))
		}
		if rt.is_true(var_display_order_number) {
			if rt.is_true(var_sent_to_admin) {
				mut var_before := rt.new_string('<a class="link" href="' +
					...
					'"' +
					if rt.is_true(var_block_email_editor_enabled) { ' style="text-decoration: none;"' } else { '' } +
					'>')
				mut var_after := '</a>'
			} else {
			...
			mut var_order_number_string := rt.call_function('__', [
				rt.new_string('[Order #%s]'),
				rt.new_string('woocommerce'),
			])
			if rt.is_true(var_email_improvements_enabled) {
				var_order_number_string = rt.call_function('__', [
					rt.new_string('Order #%s'),
					rt.new_string('woocommerce'),
				])
			}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_block_email_editor_enabled)))) {
		...
		rt.echo_val(rt.call_function('esc_attr',
			[rt.new_string(var_order_quantity_text_align.str()).clone()]))
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_html_e', [rt.new_string('Quantity'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr',
			[rt.new_string(var_order_total_text_align.str()).clone()]))
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_html_e', [rt.new_string('Price'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
	mut var_image_size := if rt.is_true(var_email_improvements_enabled) { 48 } else { 32 }
	rt.echo_val(rt.call_function('wc_get_email_order_items', [
		var_order.clone(),
		rt.create_array([
			rt.ArrayItem{ key: 'show_sku', val: var_sent_to_admin },
			rt.ArrayItem{ key: 'show_image', val: var_email_improvements_enabled },
			rt.ArrayItem{ key: 'image_size', val: rt.create_array([
				rt.ArrayItem{ key: none, val: var_image_size },
				rt.ArrayItem{ key: none, val: var_image_size },
			]) },
	...
	mut var_item_totals_count := var_item_totals.clone().array_count()
	if rt.is_true(var_item_totals) {
		...
		for {
			...
			var_i += 1
			mut var_last_class := if var_i == var_item_totals_count {
				' order-totals-last'
			} else {
				''
			}
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('esc_attr', [if !(var_total.array_get(rt.new_string('type'))).is_null() {
				var_total.array_get(rt.new_string('type'))
			} else {
				rt.new_string('unknown')
			}]))
			rt.echo_val(rt.call_function('esc_attr', [rt.new_string(var_last_class.str()).clone()]))
			// unsupported statement: Stmt_InlineHTML
			print(if 1 == var_i { 'border-top-width: 4px;' } else { '' })
			// unsupported statement: Stmt_InlineHTML
			print(
				(rt.call_function('wp_kses_post', [var_total.array_get(rt.new_string('label'))])).str() +
				' ')
			if rt.is_true(var_email_improvements_enabled) {
				rt.echo_val(if var_total.array_isset(rt.new_string('meta')) { rt.call_function('wp_kses_post', [
						var_total.array_get(rt.new_string('meta')),
					]) } else { rt.new_string('') })
			}
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('esc_attr',
	...
	if rt.is_true(rt.call_method(var_order, 'get_customer_note', []rt.PhpVal{}))
		&& rt.is_true(rt.new_bool(!(rt.is_true(var_email_improvements_enabled)))) {
		// unsupported statement: Stmt_InlineHTML
		...
		rt.echo_val(rt.call_function('wp_kses', [
			rt.call_function('nl2br', [
				rt.call_function('wc_wptexturize_order_note', [
	...
	// unsupported statement: Stmt_InlineHTML
	if rt.is_true(rt.call_method(var_order, 'get_customer_note', []rt.PhpVal{}))
		&& rt.is_true(var_email_improvements_enabled) {
		// unsupported statement: Stmt_InlineHTML
		if rt.is_true(var_display_section_divider) {
			// unsupported statement: Stmt_InlineHTML
		}
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr',
			[rt.new_string(var_order_table_class.str()).clone()]))
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_html_e', [rt.new_string('Customer note'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('wp_kses', [
			rt.call_function('nl2br', [
