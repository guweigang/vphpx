...
fn main() {
	...
	rt.echo_val(rt.call_function('esc_html__', [
		rt.new_string('Payment confirmation pending'),
	...
	rt.call_function('printf', [
		rt.call_function('esc_html__', [
			rt.new_string('Thanks for your order. It’s currently on hold while we confirm your payment via %s.'),
			rt.new_string('woocommerce'),
		]),
		rt.new_string('<strong><!--[woocommerce/order-payment-method]--></strong>'),
	])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html__', [
		rt.new_string('We’ll update you once payment has been confirmed. Here’s a summary of your order:'),
		rt.new_string('woocommerce'),
	]))
	// unsupported statement: Stmt_InlineHTML
