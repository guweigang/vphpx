...
fn main() {
	...
	if rt.is_true(var_email_improvements_enabled) {
		print(
			(rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Thanks for creating an account on %s. Here’s a copy of your user details.'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_blogname.clone()])])).str() +
			'\n\n')
		print('----------------------------------------\n\n')
		print(
		...
		print('----------------------------------------\n\n')
		print(
			(rt.call_function('esc_html__', [rt.new_string('You can access your account area to view orders, change your password, and more via the link below:'), rt.new_string('woocommerce')])).str() +
			'\n\n')
		print(
			(rt.call_function('esc_html', [rt.call_function('wc_get_page_permalink', [rt.new_string('myaccount')])])).str() +
			'\n\n')
	} else {
		print(
			(rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Thanks for creating an account on %1$s. Your username is %2$s. You can access your account area to view orders, change your password, and more at: %3$s'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_blogname.clone()]), rt.call_function('esc_html', [var_user_login.clone()]), rt.call_function('esc_html', [rt.call_function('wc_get_page_permalink', [rt.new_string('myaccount')])])])).str() +
			'\n\n')
	}
	if rt.is_true(var_password_generated) && rt.is_true(var_set_password_url) {
		print(
			(rt.call_function('esc_html__', [rt.new_string('To set your password, visit the following address: '), rt.new_string('woocommerce')])).str() +
			'\n\n')
		print((rt.call_function('esc_html', [var_set_password_url.clone()])).str() + '\n\n')
	}
	print('\n\n----------------------------------------\n\n')
