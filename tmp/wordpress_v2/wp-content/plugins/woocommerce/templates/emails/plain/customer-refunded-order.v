...
fn main() {
	...
	if rt.is_true(var_email_improvements_enabled) {
		...
		print(
			(rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Your order on %s has been partially refunded. There are more details below for your reference:'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_blogname.clone()])])).str() +
			'\n\n')
	} else {
		print(
			(rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Your order on %s has been refunded. There are more details below for your reference:'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_blogname.clone()])])).str() +
			'\n\n')
	}
