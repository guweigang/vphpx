...
fn main() {
	...
	if rt.is_true(var_email_improvements_enabled) {
		print(
			(rt.call_function('esc_html__', [rt.new_string('We’ve received your order and it’s currently on hold until we can confirm your payment has been processed.'), rt.new_string('woocommerce')])).str() +
			'\n\n')
		print(
		...
		print(
			(rt.call_function('esc_html__', [rt.new_string('Thanks for your order. It’s on-hold until we confirm that payment has been received.'), rt.new_string('woocommerce')])).str() +
			'\n\n')
	}
