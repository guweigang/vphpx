...
fn main() {
	...
	for {
		...
		]))
		{
			...
			print(' ')
			print(
				(rt.call_function('esc_html', [rt.call_function('str_pad', [rt.call_function('wp_kses', [rt.call_method(var_order, 'get_formatted_line_subtotal', [rt.get_property(var_item, 'item')]), rt.new_array()]), rt.new_int(20), rt.new_string(' '), rt.get_constant('STR_PAD_LEFT')])])).str() +
				'\n')
