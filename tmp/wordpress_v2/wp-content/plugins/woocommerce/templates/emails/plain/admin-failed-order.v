...
fn main() {
	...
	mut var_text := rt.call_function('__', [
		rt.new_string('Payment for order #%1$s from %2$s has failed. The order was as follows:'),
		rt.new_string('woocommerce'),
	])
	if rt.is_true(var_email_improvements_enabled) {
		var_text = rt.call_function('__', [
			rt.new_string('Unfortunately, the payment for order #%1$s from %2$s has failed. The order was as follows:'),
			rt.new_string('woocommerce'),
		])
	}
	print(
