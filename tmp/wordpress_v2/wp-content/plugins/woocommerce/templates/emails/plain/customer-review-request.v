...
fn main() {
	...
	if !(!rt.is_true(rt.call_method(var_order, 'get_billing_first_name', []rt.PhpVal{}))) {
		...
		print(
			(rt.call_function('esc_html__', [rt.new_string('Hi,'), rt.new_string('woocommerce')])).str() +
			'\n\n')
	}
	print(
		(rt.call_function('esc_html__', [rt.new_string('We’d love to know what you thought of the products you ordered. Your review helps other shoppers make better decisions and helps us improve.'), rt.new_string('woocommerce')])).str() +
		'\n\n')
	if !(!rt.is_true(var_review_order_url)) {
		print(
			(rt.call_function('esc_html__', [rt.new_string('Leave a review:'), rt.new_string('woocommerce')])).str() +
			'\n')
		print((rt.call_function('esc_url', [var_review_order_url.clone()])).str() + '\n\n')
	}
	if rt.is_true(rt.new_bool(rt.instance_of(var_order, 'WC_Order'))) {
		...
		rt.call_function('printf', [
			rt.call_function('esc_html__', [rt.new_string('Order #%1$s (%2$s)'),
				rt.new_string('woocommerce')]),
			rt.call_function('esc_html', [rt.call_method(var_order, 'get_order_number',
				[]rt.PhpVal{})]),
			rt.call_function('esc_html', [if rt.is_true(var_date_created) { rt.call_function('wc_format_datetime', [
