...
fn main() {
	...
	mut var_text := rt.call_function('__', [
		rt.new_string('You’ve received the following order from %s:'),
		rt.new_string('woocommerce'),
	])
	if rt.is_true(var_email_improvements_enabled) {
		var_text = rt.call_function('__', [
			rt.new_string('You’ve received a new order from %s:'),
			rt.new_string('woocommerce'),
		])
	}
	print(
		(rt.call_function('sprintf', [rt.call_function('esc_html', [var_text.clone()]), rt.call_function('esc_html', [rt.call_method(var_order, 'get_formatted_billing_full_name', []rt.PhpVal{})])])).str() +
		'\n\n')
