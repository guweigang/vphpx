...
fn main() {
	...
	if rt.is_true(var_email_improvements_enabled) {
		...
		rt.call_function('esc_html_e', [
			rt.new_string('We’ve received your order and it’s currently on hold until we can confirm your payment has been processed.'),
			rt.new_string('woocommerce'),
		])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_html_e', [
		...
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_html_e', [
			rt.new_string('Thanks for your order. It’s on-hold until we confirm that payment has been received.'),
			rt.new_string('woocommerce'),
		])
		// unsupported statement: Stmt_InlineHTML
	}
