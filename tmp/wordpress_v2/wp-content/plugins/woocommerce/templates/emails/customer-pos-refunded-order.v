...
fn main() {
	...
	if rt.is_true(var_partial_refund) {
		print(
			(rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Your order from %s has been partially refunded.'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_pos_store_name.clone()])])).str() +
			'\n\n')
	} else {
		print(
			(rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Your order from %s has been refunded.'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_pos_store_name.clone()])])).str() +
			'\n\n')
	}
	print('</p><p>')
	print(
		(rt.call_function('esc_html__', [rt.new_string('Here’s a reminder of what you’ve bought:'), rt.new_string('woocommerce')])).str() +
		'\n\n')
	// unsupported statement: Stmt_InlineHTML
