...
fn main() {
	...
	for {
		...
		print(if rt.is_true(var_email_improvements_enabled)
			&& rt.is_true(rt.identical(rt.call_function('array_key_last', [var_columns.clone()]), var_column_id)) {
			'text-align-right'
		} else {
			'text-align-left'
		})
	...
	for {
		...
		for {
			...
			// unsupported statement: Stmt_InlineHTML
			mut var_column_alignment_class := if rt.is_true(var_email_improvements_enabled)
				&& rt.is_true(rt.identical(rt.call_function('array_key_last', [var_columns.clone()]), var_column_id)) {
				'text-align-right'
			} else {
				'text-align-left'
			...
			if rt.is_true(rt.identical(rt.new_string('download-product'), var_column_id)) {
				...
				rt.echo_val(rt.call_function('esc_attr',
					[rt.new_string(var_column_alignment_class.str()).clone()]))
				// unsupported statement: Stmt_InlineHTML
			} else {
				// unsupported statement: Stmt_InlineHTML
				rt.echo_val(rt.call_function('esc_attr',
			...
			if rt.is_true(rt.call_function('has_action', [
				rt.new_string('woocommerce_email_downloads_column_' + var_column_id.str()),
			]))
			{
				...
				if rt.is_true(rt.equal(switch_val_1, rt.new_string('download-product'))) {
					...
					rt.echo_val(rt.call_function('esc_url', [
						rt.call_function('get_permalink', [
							var_download.array_get(rt.new_string('product_id')),
						]),
					]))
					// unsupported statement: Stmt_InlineHTML
					...
					// unsupported statement: Stmt_InlineHTML
					rt.echo_val(rt.call_function('esc_url', [
						var_download.array_get(rt.new_string('download_url')),
					]))
					// unsupported statement: Stmt_InlineHTML
					rt.echo_val(rt.call_function('esc_html', [
						var_download.array_get(rt.new_string('download_name')),
					]))
					// unsupported statement: Stmt_InlineHTML
						...
						rt.echo_val(rt.call_function('esc_attr', [
							rt.call_function('date', [rt.new_string('Y-m-d'),
								rt.call_function('strtotime', [
									var_download.array_get(rt.new_string('access_expires')),
								])]),
						]))
						// unsupported statement: Stmt_InlineHTML
						rt.echo_val(rt.call_function('esc_attr', [
