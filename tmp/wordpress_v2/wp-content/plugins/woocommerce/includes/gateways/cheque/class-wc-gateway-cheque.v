...
fn (mut this Class_WC_Gateway_Cheque) construct() {
	...
	this.dispatch_set_prop('icon', rt.call_function('apply_filters', [
		rt.new_string('woocommerce_cheque_icon'),
		rt.new_string(''),
	]))
	this.dispatch_set_prop('has_fields', rt.new_bool(false))
	this.dispatch_set_prop('method_title', rt.call_function('_x', [
	...
	this.dispatch_set_prop('method_description', rt.call_function('__', [
		rt.new_string('Take payments in person via checks. This offline gateway can also be useful to test purchases.'),
		rt.new_string('woocommerce'),
	]))
	...
	rt.call_function('add_action', [
		rt.new_string('woocommerce_update_options_payment_gateways_' +(rt.get_property(rt.new_object('WC_Gateway_Cheque', ['WC_Payment_Gateway'], &this), 'id')).str()),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Gateway_Cheque', [
				'WC_Payment_Gateway',
			], &this) },
			rt.ArrayItem{ key: none, val: 'process_admin_options' },
		]),
	])
	rt.call_function('add_action', [rt.new_string('woocommerce_thankyou_cheque'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Gateway_Cheque', [
				'WC_Payment_Gateway',
			], &this) },
	...
	this.dispatch_set_prop('form_fields', rt.create_array([
		rt.ArrayItem{ key: 'enabled', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Enable check payments'),
				rt.new_string('woocommerce'),
			]) },
		...
		rt.ArrayItem{ key: 'description', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'default', val: rt.call_function('__', [
				rt.new_string('Please send a check to Store Name, Store Street, Store Town, Store State / County, Store Postcode.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'desc_tip', val: true },
		]) },
...
fn (mut this Class_WC_Gateway_Cheque) email_instructions(var_order rt.PhpVal, var_sent_to_admin rt.PhpVal, plain_text bool) {
	...
	if rt.is_true(this.instructions) && rt.is_true(rt.new_bool(!(rt.is_true(var_sent_to_admin))))
		&& rt.is_true(rt.identical(Class_WC_Gateway_Cheque.id(), rt.call_method(var_order_mutated, 'get_payment_method', []rt.PhpVal{}))) {
		mut var_instructions_order_status := rt.call_function('apply_filters', [
			rt.new_string('woocommerce_cheque_email_instructions_order_status'),
...
fn (mut this Class_WC_Gateway_Cheque) process_payment(var_order_id rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.greater(rt.call_method(var_order, 'get_total', []rt.PhpVal{}), rt.new_int(0))) {
		mut var_process_payment_status := rt.call_function('apply_filters', [
			rt.new_string('woocommerce_cheque_process_payment_order_status'),
			Class_Automattic_WooCommerce_Enums_OrderStatus.on_hold(),
			var_order.clone(),
		])
		rt.call_method(var_order, 'update_status', [var_process_payment_status.clone(),
			rt.call_function('_x', [rt.new_string('Awaiting check payment.'),
				rt.new_string('Check payment method'), rt.new_string('woocommerce')])])
	} else {
...
fn (mut this Class_WC_Gateway_Cheque) get_settings_url() rt.PhpVal {
	...
	if !rt.is_true(var_payments_settings_page) {
		...
		mut iife_result_1 := iife_temp_1.wc_payments_settings_url(rt.new_string('/' +
			(Class_WC_Settings_Payment_Gateways.offline_section_name()).str() + '/' +(rt.get_property(rt.new_object('WC_Gateway_Cheque', ['WC_Payment_Gateway'], &this), 'id')).str()))
	...
	mut var_should_use_react_settings_page := rt.call_method(var_payments_settings_page,
		...
		Class_WC_Settings_Payment_Gateways.cheque_section_name(),
	])
	mut iife_temp_2 := Class_Automattic_WooCommerce_Internal_Admin_Settings_Utils{}
	mut iife_result_2 := iife_temp_2.wc_payments_settings_url(if rt.is_true(var_should_use_react_settings_page) {
		'/' +(Class_WC_Settings_Payment_Gateways.offline_section_name()).str() + '/' +(rt.get_property(rt.new_object('WC_Gateway_Cheque', ['WC_Payment_Gateway'], &this), 'id')).str()
	} else {
		rt.new_null()
	}, if rt.is_true(var_should_use_react_settings_page) { rt.new_array() } else { rt.create_array([
			rt.ArrayItem{ key: 'section', val: rt.get_property(rt.new_object('WC_Gateway_Cheque', [
				'WC_Payment_Gateway',
			], &this), 'id') },
		]) })
