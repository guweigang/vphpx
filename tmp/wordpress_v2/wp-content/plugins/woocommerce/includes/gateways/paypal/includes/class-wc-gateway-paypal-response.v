...
fn (mut this Class_WC_Gateway_Paypal_Response) get_paypal_order(var_raw_custom rt.PhpVal) bool {
	mut var_custom := rt.call_function('json_decode', [var_raw_custom.clone()])
	if rt.is_true(var_custom) && var_custom.clone().is_object() {
		...
		mut iife_result_0 := iife_temp_0.log(rt.new_string('Order ID and key were not found in "custom".'),
			rt.new_string('error'))
		return false
	}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order))))
		...
		mut iife_result_1 := iife_temp_1.log(rt.new_string('Order Keys do not match.'),
			rt.new_string('error'))
		return false
	}
