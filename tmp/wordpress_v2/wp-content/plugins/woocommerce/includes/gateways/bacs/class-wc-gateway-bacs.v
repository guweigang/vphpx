...
fn (mut this Class_WC_Gateway_BACS) construct() {
	...
	this.dispatch_set_prop('icon', rt.call_function('apply_filters', [
		rt.new_string('woocommerce_bacs_icon'),
		rt.new_string(''),
	]))
	...
	this.dispatch_set_prop('method_description', rt.call_function('__', [
		rt.new_string('Take payments in person via BACS. More commonly known as direct bank/wire transfer.'),
		rt.new_string('woocommerce'),
	]))
	...
	this.account_details = rt.call_function('get_option', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{
					key: 'account_name'
					val: this.get_option(rt.new_string('account_name'))
				},
				rt.ArrayItem{
					key: 'account_number'
					val: this.get_option(rt.new_string('account_number'))
				},
				rt.ArrayItem{ key: 'sort_code', val: this.get_option(rt.new_string('sort_code')) },
				rt.ArrayItem{ key: 'bank_name', val: this.get_option(rt.new_string('bank_name')) },
				rt.ArrayItem{ key: 'iban', val: this.get_option(rt.new_string('iban')) },
				rt.ArrayItem{ key: 'bic', val: this.get_option(rt.new_string('bic')) },
			]) },
		]),
	])
	rt.call_function('add_action', [
		rt.new_string('woocommerce_update_options_payment_gateways_' +(rt.get_property(rt.new_object('WC_Gateway_BACS', ['WC_Payment_Gateway'], &this), 'id')).str()),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Gateway_BACS', [
				'WC_Payment_Gateway',
			], &this) },
	...
	rt.call_function('add_action', [rt.new_string('woocommerce_thankyou_bacs'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Gateway_BACS', [
				'WC_Payment_Gateway',
			], &this) },
	...
	this.dispatch_set_prop('form_fields', rt.create_array([
		rt.ArrayItem{ key: 'enabled', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Enable bank transfer'),
				rt.new_string('woocommerce'),
			]) },
		...
		rt.ArrayItem{ key: 'description', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'default', val: rt.call_function('__', [
				rt.new_string('Make your payment directly into our bank account. Please use your Order ID as the payment reference. Your order will not be shipped until the funds have cleared in our account.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'desc_tip', val: true },
		]) },
		rt.ArrayItem{ key: 'instructions', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'type', val: 'textarea' },
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('Instructions that will be added to the thank you page and emails.'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'default', val: '' },
			rt.ArrayItem{ key: 'desc_tip', val: true },
		]) },
		rt.ArrayItem{ key: 'account_details', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'account_details' },
		]) },
	]))
}
...
fn (mut this Class_WC_Gateway_BACS) generate_account_details_html() rt.PhpVal {
	...
	mut var_sortcode := if var_locale.array_get(var_country).array_get(rt.new_string('sortcode')).array_isset(rt.new_string('label')) { var_locale.array_get(var_country).array_get(rt.new_string('sortcode')).array_get(rt.new_string('label')) } else { rt.call_function('__', [
			rt.new_string('Sort code'),
			rt.new_string('woocommerce'),
		]) }
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Account details:'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wp_kses_post', [
		rt.call_function('wc_help_tip', [
			rt.call_function('__', [
				rt.new_string('These account details will be displayed within the order thank you page and confirmation email.'),
				rt.new_string('woocommerce'),
			]),
		]),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Account name'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Account number'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Bank name'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [var_sortcode.clone()]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('IBAN'), rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('BIC / Swift'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(this.account_details) {
		...
		for {
			...
			print(
				...
				(rt.call_function('esc_attr', [rt.call_function('wp_unslash', [var_account.array_get(rt.new_string('account_name'))])])).str() +
				'" name="bacs_account_name[' +
				(rt.call_function('esc_attr', [var_i.clone()])).str() +
				']" /></td>\n\t\t\t\t\t\t\t\t\t\t<td><input type="text" value="' +
				(rt.call_function('esc_attr', [var_account.array_get(rt.new_string('account_number'))])).str() +
				'" name="bacs_account_number[' +
				(rt.call_function('esc_attr', [var_i.clone()])).str() +
				']" /></td>\n\t\t\t\t\t\t\t\t\t\t<td><input type="text" value="' +
				(rt.call_function('esc_attr', [rt.call_function('wp_unslash', [var_account.array_get(rt.new_string('bank_name'))])])).str() +
				'" name="bacs_bank_name[' + (rt.call_function('esc_attr', [var_i.clone()])).str() +
				']" /></td>\n\t\t\t\t\t\t\t\t\t\t<td><input type="text" value="' +
				(rt.call_function('esc_attr', [var_account.array_get(rt.new_string('sort_code'))])).str() +
				'" name="bacs_sort_code[' + (rt.call_function('esc_attr', [var_i.clone()])).str() +
				']" /></td>\n\t\t\t\t\t\t\t\t\t\t<td><input type="text" value="' +
				(rt.call_function('esc_attr', [var_account.array_get(rt.new_string('iban'))])).str() +
				'" name="bacs_iban[' + (rt.call_function('esc_attr', [var_i.clone()])).str() +
				']" /></td>\n\t\t\t\t\t\t\t\t\t\t<td><input type="text" value="' +
				(rt.call_function('esc_attr', [var_account.array_get(rt.new_string('bic'))])).str() +
				'" name="bacs_bic[' + (rt.call_function('esc_attr', [var_i.clone()])).str() +
				']" /></td>\n\t\t\t\t\t\t\t\t\t</tr>')
		}
	}
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('+ Add account'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Remove selected account(s)'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
...
fn (mut this Class_WC_Gateway_BACS) save_account_details() {
	...
	if rt.get_superglobal('_POST').array_isset(rt.new_string('bacs_account_name'))
		&& rt.get_superglobal('_POST').array_isset(rt.new_string('bacs_account_number'))
		&& rt.get_superglobal('_POST').array_isset(rt.new_string('bacs_bank_name'))
		&& rt.get_superglobal('_POST').array_isset(rt.new_string('bacs_sort_code'))
		&& rt.get_superglobal('_POST').array_isset(rt.new_string('bacs_iban'))
		&& rt.get_superglobal('_POST').array_isset(rt.new_string('bacs_bic')) {
		mut var_account_names := rt.call_function('wc_clean', [
			rt.call_function('wp_unslash', [
				rt.get_superglobal('_POST').array_get(rt.new_string('bacs_account_name')),
			]),
		])
		mut var_account_numbers := rt.call_function('wc_clean', [
			rt.call_function('wp_unslash', [
				rt.get_superglobal('_POST').array_get(rt.new_string('bacs_account_number')),
			]),
		])
		mut var_bank_names := rt.call_function('wc_clean', [
			rt.call_function('wp_unslash', [
				rt.get_superglobal('_POST').array_get(rt.new_string('bacs_bank_name')),
			]),
		])
		mut var_sort_codes := rt.call_function('wc_clean', [
			rt.call_function('wp_unslash', [
				rt.get_superglobal('_POST').array_get(rt.new_string('bacs_sort_code')),
			]),
		])
		mut var_ibans := rt.call_function('wc_clean', [
			rt.call_function('wp_unslash',
				[rt.get_superglobal('_POST').array_get(rt.new_string('bacs_iban'))]),
		])
		mut var_bics := rt.call_function('wc_clean', [
			rt.call_function('wp_unslash',
				[rt.get_superglobal('_POST').array_get(rt.new_string('bacs_bic'))]),
		])
		mut iter_2 := var_account_names.iterator()
		for {
			...
			var_accounts << rt.create_array([
				rt.ArrayItem{ key: 'account_name', val: var_account_names.array_get(var_i) },
				rt.ArrayItem{ key: 'account_number', val: var_account_numbers.array_get(var_i) },
				rt.ArrayItem{ key: 'bank_name', val: var_bank_names.array_get(var_i) },
				rt.ArrayItem{ key: 'sort_code', val: var_sort_codes.array_get(var_i) },
				rt.ArrayItem{ key: 'iban', val: var_ibans.array_get(var_i) },
				rt.ArrayItem{ key: 'bic', val: var_bics.array_get(var_i) },
			])
		}
	}
	rt.call_function('do_action', [rt.new_string('woocommerce_update_option'),
		rt.create_array([rt.ArrayItem{ key: 'id', val: 'woocommerce_bacs_accounts' }])])
	rt.call_function('update_option', [rt.new_string('woocommerce_bacs_accounts'),
		rt.create_array_from_list(var_accounts)])
}
		...
		rt.echo_val(rt.call_function('wp_kses_post', [
			rt.call_function('wpautop', [
				rt.call_function('wptexturize', [
					rt.call_function('wp_kses_post', [this.instructions]),
				]),
			]),
		]))
	}
...
fn (mut this Class_WC_Gateway_BACS) email_instructions(var_order rt.PhpVal, var_sent_to_admin rt.PhpVal, plain_text bool) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_sent_to_admin))))
		&& rt.is_true(rt.identical(Class_WC_Gateway_BACS.id(), rt.call_method(var_order_mutated, 'get_payment_method', []rt.PhpVal{}))) {
		mut var_instructions_order_status := rt.call_function('apply_filters', [
			rt.new_string('woocommerce_bacs_email_instructions_order_status'),
		...
		if rt.is_true(rt.call_method(var_order_mutated, 'has_status', [
			var_instructions_order_status.clone()]))
		{
			if rt.is_true(this.instructions) {
				rt.echo_val(rt.call_function('wp_kses_post', [
					rt.new_string(
...
fn (mut this Class_WC_Gateway_BACS) bank_details(order_id string) {
	...
	mut var_bacs_accounts := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_bacs_accounts'),
		this.account_details,
		rt.new_string(order_id),
	])
	if !(!rt.is_true(var_bacs_accounts)) {
		mut var_account_html := rt.new_string('')
		mut var_has_details := rt.new_bool(false)
		mut iter_3 := var_bacs_accounts.iterator()
		for {
			item_3 := iter_3.next() or { break }
			mut var_bacs_account := item_3.val
			var_bacs_account = rt.array_to_object(var_bacs_account)
			if rt.is_true(rt.get_property(var_bacs_account, 'account_name')) {
				var_account_html = rt.concat(var_account_html, rt.new_string(
					'<h3 class="wc-bacs-bank-details-account-name">' +
					(rt.call_function('wp_kses_post', [rt.call_function('wp_unslash', [rt.get_property(var_bacs_account, 'account_name')])])).str() +
					':</h3>' + (rt.get_constant('PHP_EOL')).str()))
			}
			var_account_html = rt.concat(var_account_html, rt.new_string(
				'<ul class="wc-bacs-bank-details order_details bacs_details">' +
				(rt.get_constant('PHP_EOL')).str()))
			mut var_account_fields := rt.call_function('apply_filters', [
				rt.new_string('woocommerce_bacs_account_fields'),
				rt.create_array([
					rt.ArrayItem{ key: 'bank_name', val: rt.create_array([
						...
						rt.ArrayItem{ key: 'value', val: rt.get_property(var_bacs_account,
							'bank_name') },
					]) },
					rt.ArrayItem{ key: 'account_number', val: rt.create_array([
						rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
							rt.new_string('Account number'),
							rt.new_string('woocommerce'),
						]) },
						rt.ArrayItem{ key: 'value', val: rt.get_property(var_bacs_account,
							'account_number') },
					]) },
					rt.ArrayItem{ key: 'sort_code', val: rt.create_array([
						rt.ArrayItem{ key: 'label', val: var_sortcode },
						rt.ArrayItem{ key: 'value', val: rt.get_property(var_bacs_account,
							'sort_code') },
					]) },
					rt.ArrayItem{ key: 'iban', val: rt.create_array([
						rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
							rt.new_string('IBAN'),
							rt.new_string('woocommerce'),
						]) },
						rt.ArrayItem{ key: 'value', val: rt.get_property(var_bacs_account, 'iban') },
					]) },
					rt.ArrayItem{ key: 'bic', val: rt.create_array([
						rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
							rt.new_string('BIC'),
							rt.new_string('woocommerce'),
						]) },
						rt.ArrayItem{ key: 'value', val: rt.get_property(var_bacs_account, 'bic') },
					]) },
				]),
				rt.new_string(order_id),
			])
			mut iter_4 := var_account_fields.iterator()
			for {
				item_4 := iter_4.next() or { break }
				mut var_field := item_4.val
				mut var_field_key := item_4.key
				if !(!rt.is_true(var_field.array_get(rt.new_string('value')))) {
					var_account_html = rt.concat(var_account_html, rt.new_string('<li class="' +
						(rt.call_function('esc_attr', [var_field_key.clone()])).str() + '">' +
						(rt.call_function('wp_kses_post', [var_field.array_get(rt.new_string('label'))])).str() +
						': <strong>' +
						(rt.call_function('wp_kses_post', [rt.call_function('wptexturize', [var_field.array_get(rt.new_string('value'))])])).str() +
						'</strong></li>' + (rt.get_constant('PHP_EOL')).str()))
					var_has_details = rt.new_bool(true)
				}
			}
			var_account_html = rt.concat(var_account_html, rt.new_string('</ul>'))
		}
		if rt.is_true(var_has_details) {
			print(
				'<section class="woocommerce-bacs-bank-details"><h2 class="wc-bacs-bank-details-heading">' +
				(rt.call_function('esc_html__', [rt.new_string('Our bank details'), rt.new_string('woocommerce')])).str() +
				'</h2>' +
				(rt.call_function('wp_kses_post', [rt.new_string((rt.get_constant('PHP_EOL')).str() +
				var_account_html.str())])).str() + '</section>')
		}
	}
}
...
fn (mut this Class_WC_Gateway_BACS) process_payment(var_order_id rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.greater(rt.call_method(var_order, 'get_total', []rt.PhpVal{}), rt.new_int(0))) {
		mut var_process_payment_status := rt.call_function('apply_filters', [
			rt.new_string('woocommerce_bacs_process_payment_order_status'),
			Class_Automattic_WooCommerce_Enums_OrderStatus.on_hold(),
			var_order.clone(),
		])
		rt.call_method(var_order, 'update_status', [var_process_payment_status.clone(),
			rt.call_function('__', [rt.new_string('Awaiting BACS payment.'),
				rt.new_string('woocommerce')])])
	} else {
...
fn (mut this Class_WC_Gateway_BACS) get_country_locale() rt.PhpVal {
	if !rt.is_true(this.locale) {
		this.locale = rt.call_function('apply_filters', [
			rt.new_string('woocommerce_get_bacs_locale'),
			rt.create_array([
				...
				rt.ArrayItem{ key: 'CA', val: rt.create_array([
					rt.ArrayItem{ key: 'sortcode', val: rt.create_array([
						rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
							rt.new_string('Bank transit number'),
							rt.new_string('woocommerce'),
						]) },
					]) },
				]) },
...
fn (mut this Class_WC_Gateway_BACS) get_settings_url() rt.PhpVal {
	...
	mut iife_result_0 := iife_temp_0.wc_payments_settings_url(if rt.is_true(var_should_use_react_settings_page) {
		'/' +(Class_WC_Settings_Payment_Gateways.offline_section_name()).str() + '/' +(rt.get_property(rt.new_object('WC_Gateway_BACS', ['WC_Payment_Gateway'], &this), 'id')).str()
	} else {
		rt.new_null()
	}, if rt.is_true(var_should_use_react_settings_page) { []rt.PhpVal{} } else { rt.create_array([
			rt.ArrayItem{ key: 'section', val: rt.get_property(rt.new_object('WC_Gateway_BACS', [
				'WC_Payment_Gateway',
			], &this), 'id') },
		]) })
...
fn (mut this Class_WC_Gateway_BACS) is_reactified_settings_page() bool {
	...
	mut iter_5 := iife_result_1.iterator()
	for {
		item_5 := iter_5.next() or { break }
		mut var_settings_page := item_5.val
	...
	return (rt.call_method(var_payments_settings_page, 'should_render_react_section', [
		Class_WC_Settings_Payment_Gateways.bacs_section_name(),
	])).to_bool()
}
