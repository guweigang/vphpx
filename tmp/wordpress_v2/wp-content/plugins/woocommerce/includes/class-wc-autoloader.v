...
fn (mut this Class_WC_Autoloader) construct() {
	...
	rt.call_function('spl_autoload_register', [
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Autoloader', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'autoload' },
		]),
	])
	this.include_path =
		(rt.call_function('untrailingslashit', [rt.call_function('plugin_dir_path', [rt.get_constant('WC_PLUGIN_FILE')])])).str() +
		'/includes/'
}
...
fn (mut this Class_WC_Autoloader) get_file_name_from_class(var_class rt.PhpVal) string {
	...
	return 'class-' +
		(rt.call_function('str_replace', [rt.new_string('_'), rt.new_string('-'), var_class_mutated.clone()])).str() +
		'.php'
}
...
fn (mut this Class_WC_Autoloader) autoload(var_class rt.PhpVal) {
	mut var_class_mutated := var_class
	var_class_mutated = rt.new_string(var_class_mutated.clone().to_string().to_lower())
	...
	mut iife_temp_0 := Class_Automattic_WooCommerce_Packages{}
	mut iife_result_0 := iife_temp_0.should_load_class(var_class_mutated.clone())
	if rt.is_true(iife_result_0) {
		return
	}
	mut var_file := rt.new_string(this.get_file_name_from_class(var_class_mutated.clone()))
	mut var_path := rt.new_string('')
	if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(), rt.new_string('wc_addons_gateway_')])))
	{
		var_path = rt.new_string(
			(this.include_path).str() + 'gateways/' + (rt.call_function('substr', [rt.call_function('str_replace', [rt.new_string('_'), rt.new_string('-'), var_class_mutated.clone()]), rt.new_int(18)])).str() +
			'/')
	} else if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(),
		rt.new_string('wc_gateway_'),
	])))
	{
		var_path = rt.new_string(
			(this.include_path).str() + 'gateways/' + (rt.call_function('substr', [rt.call_function('str_replace', [rt.new_string('_'), rt.new_string('-'), var_class_mutated.clone()]), rt.new_int(11)])).str() +
			'/')
	} else if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(),
		rt.new_string('wc_shipping_'),
	])))
	{
		var_path = rt.new_string(
			(this.include_path).str() + 'shipping/' + (rt.call_function('substr', [rt.call_function('str_replace', [rt.new_string('_'), rt.new_string('-'), var_class_mutated.clone()]), rt.new_int(12)])).str() +
			'/')
	} else if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(),
		rt.new_string('wc_shortcode_'),
	])))
	{
		var_path = rt.new_string((this.include_path).str() + 'shortcodes/')
	} else if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(),
		rt.new_string('wc_meta_box'),
	])))
	{
		var_path = rt.new_string((this.include_path).str() + 'admin/meta-boxes/')
	} else if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(),
		rt.new_string('wc_admin'),
	])))
	{
		var_path = rt.new_string((this.include_path).str() + 'admin/')
	} else if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(),
		rt.new_string('wc_payment_token_'),
	])))
	{
		var_path = rt.new_string((this.include_path).str() + 'payment-tokens/')
	} else if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(),
		rt.new_string('wc_log_handler_'),
	])))
	{
		var_path = rt.new_string((this.include_path).str() + 'log-handlers/')
	} else if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(),
		rt.new_string('wc_integration'),
	])))
	{
		var_path = rt.new_string(
			(this.include_path).str() + 'integrations/' + (rt.call_function('substr', [rt.call_function('str_replace', [rt.new_string('_'), rt.new_string('-'), var_class_mutated.clone()]), rt.new_int(15)])).str() +
			'/')
	} else if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(),
		rt.new_string('wc_notes_'),
	])))
	{
		var_path = rt.new_string((this.include_path).str() + 'admin/notes/')
	} else if rt.is_true(rt.identical(rt.new_int(0), rt.call_function('strpos', [
		var_class_mutated.clone(),
		rt.new_string('wc_rest_'),
	])))
	{
		...
		])))))
		{
			...
			if rt.is_true(iife_result_1) {
				...
				for var_rest_path in var_rest_controller_paths {
					if this.load_file(rt.new_string(
						(this.include_path).str() + rest_path + var_file.str()))
					{
						return
					}
				}
				this.load_rest_v4_controller_recursively(var_file.clone())
			}
		} else {
			var_rest_controller_paths = ['rest-api/Controllers/Version1/',
				'rest-api/Controllers/Version2/', 'rest-api/Controllers/Version3/',
...
fn (mut this Class_WC_Autoloader) load_rest_v4_controller_recursively(var_file rt.PhpVal) bool {
	...
	mut var_v4_base_path := rt.new_string(
		(this.include_path).str() + 'rest-api/Controllers/Version4/')
	if rt.is_true(rt.call_function('is_dir', [var_v4_base_path.clone()])) {
		mut var_iterator := create_recursiveiteratoriterator(create_recursivedirectoryiterator(var_v4_base_path.clone(),
			Class_RecursiveDirectoryIterator.skip_dots()),
			Class_RecursiveIteratorIterator.self_first())
		...
		for {
			...
			if rt.is_true(rt.call_method(var_dir_info, 'isDir', []rt.PhpVal{})) {
				mut var_subdir_path := rt.new_string(
					(rt.call_method(var_dir_info, 'getPathname', []rt.PhpVal{})).str() + '/')
