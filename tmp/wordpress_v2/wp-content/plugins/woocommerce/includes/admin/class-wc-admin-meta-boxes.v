...
fn init_static_wc_admin_meta_boxes() {
	rt.init_static_prop('WC_Admin_Meta_Boxes', 'saved_meta_boxes', rt.new_bool(false))
	rt.init_static_prop('WC_Admin_Meta_Boxes', 'meta_box_errors', rt.new_array())
}

fn (mut this Class_WC_Admin_Meta_Boxes) construct() {
	rt.call_function('add_action', [rt.new_string('add_meta_boxes'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin_Meta_Boxes', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'remove_meta_boxes' },
		]),
		rt.new_int(10)])
	rt.call_function('add_action', [rt.new_string('add_meta_boxes'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin_Meta_Boxes', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'rename_meta_boxes' },
		]),
		rt.new_int(20)])
	rt.call_function('add_action', [rt.new_string('add_meta_boxes'),
		rt.create_array([
	...
	rt.call_function('add_action', [rt.new_string('add_meta_boxes'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin_Meta_Boxes', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'add_product_boxes_sort_order' },
		]),
		rt.new_int(40)])
	rt.call_function('add_action', [rt.new_string('save_post'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin_Meta_Boxes', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'save_meta_boxes' },
		]),
		rt.new_int(1), rt.new_int(2)])
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Admin_Orders_Edit{}
	mut iife_result_0 := iife_temp_0.add_save_meta_boxes()
	rt.call_function('add_action', [rt.new_string('woocommerce_process_product_meta'),
		rt.new_string('WC_Meta_Box_Product_Data::save'), rt.new_int(10),
		rt.new_int(2)])
	rt.call_function('add_action', [rt.new_string('woocommerce_process_product_meta'),
		rt.new_string('WC_Meta_Box_Product_Images::save'), rt.new_int(20),
		rt.new_int(2)])
	rt.call_function('add_action', [
		rt.new_string('woocommerce_process_shop_coupon_meta'),
		rt.new_string('WC_Meta_Box_Coupon_Data::save'),
		rt.new_int(10),
		rt.new_int(2),
	])
	rt.call_function('add_filter', [rt.new_string('wp_update_comment_data'),
		rt.new_string('WC_Meta_Box_Product_Reviews::save'), rt.new_int(1)])
	rt.call_function('add_action', [rt.new_string('admin_notices'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin_Meta_Boxes', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'output_errors' },
		])])
	rt.call_function('add_action', [rt.new_string('shutdown'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin_Meta_Boxes', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'append_to_error_store' },
		])])
	rt.call_function('add_filter', [rt.new_string('theme_product_templates'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin_Meta_Boxes', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'remove_block_templates' },
		]),
		rt.new_int(10), rt.new_int(1)])
}

fn Class_WC_Admin_Meta_Boxes.add_error(var_text rt.PhpVal) {
	rt.get_static_prop('WC_Admin_Meta_Boxes', 'meta_box_errors').array_push(var_text.clone())
}

fn (mut this Class_WC_Admin_Meta_Boxes) save_errors() {
	rt.call_function('update_option', [
		rt.new_string(Class_WC_Admin_Meta_Boxes.error_store()),
		rt.get_static_prop('WC_Admin_Meta_Boxes', 'meta_box_errors'),
	])
}

fn (mut this Class_WC_Admin_Meta_Boxes) append_to_error_store() {
	if !rt.is_true(rt.get_static_prop('WC_Admin_Meta_Boxes', 'meta_box_errors')) {
		return
	}
	mut var_existing_errors := rt.call_function('get_option', [
		rt.new_string(Class_WC_Admin_Meta_Boxes.error_store()),
		rt.new_array(),
	])
	rt.call_function('update_option', [
		...
		rt.call_function('array_unique', [
			rt.call_function('array_merge', [var_existing_errors.clone(),
				rt.get_static_prop('WC_Admin_Meta_Boxes', 'meta_box_errors')]),
		]),
	])
}
...
fn (mut this Class_WC_Admin_Meta_Boxes) add_meta_boxes() {
	...
	rt.call_function('add_meta_box', [rt.new_string('postexcerpt'),
		rt.call_function('__', [rt.new_string('Product short description'),
			rt.new_string('woocommerce')]),
		rt.new_string('WC_Meta_Box_Product_Short_Description::output'),
		rt.new_string('product'), rt.new_string('normal')])
	rt.call_function('add_meta_box', [rt.new_string('woocommerce-product-data'),
		rt.call_function('__', [rt.new_string('Product data'),
			rt.new_string('woocommerce')]),
		rt.new_string('WC_Meta_Box_Product_Data::output'), rt.new_string('product'),
		rt.new_string('normal'), rt.new_string('high')])
	rt.call_function('add_meta_box', [rt.new_string('woocommerce-product-images'),
		rt.call_function('__', [rt.new_string('Product gallery'),
			rt.new_string('woocommerce')]),
		rt.new_string('WC_Meta_Box_Product_Images::output'), rt.new_string('product'),
		rt.new_string('side'), rt.new_string('low')])
	mut iter_2 := rt.call_function('wc_get_order_types', [
		rt.new_string('order-meta-boxes'),
	]).iterator()
	for {
		...
		mut iife_temp_1 := Class_Automattic_WooCommerce_Internal_Admin_Orders_Edit{}
		mut iife_result_1 := iife_temp_1.add_order_meta_boxes(var_type.clone(), rt.get_property(rt.get_property(var_order_type_object,
			'labels'), 'singular_name'))
	}
	rt.call_function('add_meta_box', [rt.new_string('woocommerce-coupon-data'),
		rt.call_function('__', [rt.new_string('Coupon data'),
			rt.new_string('woocommerce')]),
		rt.new_string('WC_Meta_Box_Coupon_Data::output'), rt.new_string('shop_coupon'),
		rt.new_string('normal'), rt.new_string('high')])
	if rt.is_true(rt.identical(rt.new_string('comment'), var_screen_id))
		&& rt.get_superglobal('_GET').array_isset(rt.new_string('c'))
		&& rt.is_true(rt.call_function('metadata_exists', [rt.new_string('comment'), rt.call_function('wc_clean', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('c'))])]), rt.new_string('rating')])) {
		rt.call_function('add_meta_box', [rt.new_string('woocommerce-rating'),
			rt.call_function('__', [rt.new_string('Rating'), rt.new_string('woocommerce')]),
			rt.new_string('WC_Meta_Box_Product_Reviews::output'),
...
fn (mut this Class_WC_Admin_Meta_Boxes) add_product_boxes_sort_order() {
	mut var_current_value := rt.call_function('get_user_meta', [
		rt.call_function('get_current_user_id', []rt.PhpVal{}),
		rt.new_string('meta-box-order_product'),
		rt.new_bool(true),
	])
	...
	rt.call_function('update_user_meta', [
		...
		rt.create_array([
			rt.ArrayItem{
				key: 'side'
				val: 'submitdiv,postimagediv,woocommerce-product-images,product_catdiv,tagsdiv-product_tag'
			},
			rt.ArrayItem{
				key: 'normal'
				val: 'woocommerce-product-data,postcustom,slugdiv,postexcerpt'
			},
...
fn (mut this Class_WC_Admin_Meta_Boxes) remove_meta_boxes() {
	rt.call_function('remove_meta_box', [rt.new_string('postexcerpt'),
		rt.new_string('product'), rt.new_string('normal')])
	rt.call_function('remove_meta_box', [rt.new_string('product_shipping_classdiv'),
		rt.new_string('product'), rt.new_string('side')])
	rt.call_function('remove_meta_box', [rt.new_string('commentsdiv'),
		rt.new_string('product'), rt.new_string('normal')])
	rt.call_function('remove_meta_box', [rt.new_string('commentstatusdiv'),
		rt.new_string('product'), rt.new_string('side')])
	rt.call_function('remove_meta_box', [rt.new_string('commentstatusdiv'),
		rt.new_string('product'), rt.new_string('normal')])
	rt.call_function('remove_meta_box', [rt.new_string('woothemes-settings'),
		rt.new_string('shop_coupon'), rt.new_string('normal')])
	rt.call_function('remove_meta_box', [rt.new_string('commentstatusdiv'),
		rt.new_string('shop_coupon'), rt.new_string('normal')])
	rt.call_function('remove_meta_box', [rt.new_string('slugdiv'),
		rt.new_string('shop_coupon'), rt.new_string('normal')])
	mut iter_3 := rt.call_function('wc_get_order_types', [
		rt.new_string('order-meta-boxes'),
	]).iterator()
	for {
		...
		rt.call_function('remove_meta_box', [rt.new_string('commentsdiv'),
			var_type.clone(), rt.new_string('normal')])
		rt.call_function('remove_meta_box', [rt.new_string('woothemes-settings'),
			var_type.clone(), rt.new_string('normal')])
		rt.call_function('remove_meta_box', [rt.new_string('commentstatusdiv'),
			var_type.clone(), rt.new_string('normal')])
		rt.call_function('remove_meta_box', [rt.new_string('slugdiv'),
			var_type.clone(), rt.new_string('normal')])
		rt.call_function('remove_meta_box', [rt.new_string('submitdiv'),
			var_type.clone(), rt.new_string('side')])
	}
}
	...
	if !var_post.is_null()
		...
		rt.call_function('add_meta_box', [rt.new_string('commentsdiv'),
			rt.call_function('__', [rt.new_string('Reviews'),
				rt.new_string('woocommerce')]),
			rt.new_string('post_comment_meta_box'), rt.new_string('product'),
			rt.new_string('normal')])
	}
}
...
fn (mut this Class_WC_Admin_Meta_Boxes) save_meta_boxes(var_post_id rt.PhpVal, var_post rt.PhpVal) {
	...
	if !rt.is_true(var_post_id_mutated) || !rt.is_true(var_post)
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [var_post.clone(), rt.new_string('WP_Post')])))))
		|| rt.is_true(rt.get_static_prop('WC_Admin_Meta_Boxes', 'saved_meta_boxes')) {
		return
	}
	...
	if rt.is_true(iife_result_2)
		|| rt.call_function('wp_is_post_revision', [var_post.clone()]).is_long()
		|| rt.call_function('wp_is_post_autosave', [var_post.clone()]).is_long() {
		return
	}
	if !rt.is_true(rt.get_superglobal('_POST').array_get(rt.new_string('woocommerce_meta_nonce')))
	...
	if !rt.is_true(rt.get_superglobal('_POST').array_get(rt.new_string('post_ID')))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('absint', [rt.get_superglobal('_POST').array_get(rt.new_string('post_ID'))]), var_post_id_mutated)))) {
		return
	}
	...
	rt.set_static_prop('WC_Admin_Meta_Boxes', 'saved_meta_boxes', rt.new_bool(true))
		...
		rt.new_bool(true)]))
	{
		...
		rt.call_function('do_action', [
			rt.new_string('woocommerce_process_shop_order_meta'),
			var_post_id_mutated.clone(),
			var_post.clone(),
		])
	...
	{
		rt.call_function('do_action', [
			rt.new_string('woocommerce_process_' + (rt.get_property(var_post, 'post_type')).str() +
				'_meta'),
			var_post_id_mutated.clone(),
			var_post.clone(),
		])
...
fn (mut this Class_WC_Admin_Meta_Boxes) remove_block_templates(var_templates rt.PhpVal) rt.PhpVal {
	...
	mut var_theme := rt.call_method(rt.call_function('wp_get_theme', []rt.PhpVal{}),
		'get_stylesheet', []rt.PhpVal{})
	mut var_filtered_templates := rt.new_array()
	mut iter_4 := var_templates.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_template_name := item_4.val
		mut var_template_key := item_4.key
		...
		mut var_block_template := rt.call_function('get_block_template', [
			rt.new_string(var_theme.str() + '//' + var_template_key.str()),
		])
		if rt.is_true(var_block_template)
			&& rt.get_property(var_block_template, 'post_types').is_array()
			&& rt.is_true(rt.call_function('in_array', [rt.new_string('product'), rt.get_property(var_block_template, 'post_types')])) {
			var_filtered_templates.array_set(var_template_key, var_template_name.clone())
		}
	}
	return var_filtered_templates.clone()
}
