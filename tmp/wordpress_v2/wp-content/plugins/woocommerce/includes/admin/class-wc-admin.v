...
fn (mut this Class_WC_Admin) construct() {
	rt.call_function('add_action', [rt.new_string('init'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'includes' },
		])])
	rt.call_function('add_action', [rt.new_string('admin_menu'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'init_page_controller' },
		]),
		rt.new_int(1)])
	rt.call_function('add_action', [rt.new_string('current_screen'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'conditional_includes' },
		])])
	...
	rt.call_function('add_action', [rt.new_string('admin_init'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'preview_emails' },
		])])
	rt.call_function('add_action', [rt.new_string('admin_init'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'prevent_admin_access' },
		])])
	rt.call_function('add_action', [rt.new_string('admin_init'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'admin_redirects' },
		])])
	rt.call_function('add_action', [rt.new_string('admin_footer'),
		rt.new_string('wc_print_js'), rt.new_int(25)])
	rt.call_function('add_filter', [rt.new_string('admin_footer_text'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'admin_footer_text' },
		]),
		rt.new_int(1)])
	rt.call_function('add_filter', [rt.new_string('update_footer'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'update_footer_version' },
		]),
		rt.new_int(20)])
	rt.call_function('add_filter', [rt.new_string('action_scheduler_post_type_args'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'disable_webhook_post_export' },
		])])
	rt.call_function('add_filter', [rt.new_string('admin_body_class'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'include_admin_body_class' },
		]),
		rt.new_int(9999)])
	if rt.get_superglobal('_GET').array_isset(rt.new_string('page'))
		&& rt.is_true(rt.identical(rt.new_string('wc-addons'), rt.get_superglobal('_GET').array_get(rt.new_string('page')))) {
		rt.call_function('add_filter', [rt.new_string('admin_body_class'),
			rt.create_array([rt.ArrayItem{ key: none, val: 'WC_Admin_Addons' },
				rt.ArrayItem{ key: none, val: 'filter_admin_body_classes' }])])
	}
}
...
fn (mut this Class_WC_Admin) includes() {
	...
	if rt.is_true(rt.call_function('apply_filters', [
		rt.new_string('woocommerce_enable_admin_help_tab'),
		rt.new_bool(true),
	]))
	{
...
fn (mut this Class_WC_Admin) conditional_includes() {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('dashboard')))
		|| rt.is_true(rt.equal(switch_val_1, rt.new_string('dashboard-network'))) {
	...
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('users')))
		|| rt.is_true(rt.equal(switch_val_1, rt.new_string('user')))
		|| rt.is_true(rt.equal(switch_val_1, rt.new_string('profile')))
		|| rt.is_true(rt.equal(switch_val_1, rt.new_string('user-edit'))) {
...
fn (mut this Class_WC_Admin) admin_redirects() {
	...
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('wc-install-plugin-redirect')))) {
		mut var_plugin_slug := rt.call_function('wc_clean', [
			rt.call_function('wp_unslash', [
				rt.get_superglobal('_GET').array_get(rt.new_string('wc-install-plugin-redirect')),
			]),
		])
		if rt.is_true(rt.call_function('current_user_can', [rt.new_string('install_plugins')]))
			&& rt.is_true(rt.call_function('in_array', [var_plugin_slug.clone(), rt.create_array([rt.ArrayItem{
			key: none
			val: 'woocommerce-gateway-stripe'
		}]), rt.new_bool(true)])) {
			...
			mut var_url := rt.call_function('self_admin_url', [
				rt.new_string('update.php?action=install-plugin&plugin=' + var_plugin_slug.str() +
					'&_wpnonce=' + var_nonce.str()),
			])
		} else {
			var_url = rt.call_function('admin_url', [
				rt.new_string('plugin-install.php?tab=search&type=term&s=' + var_plugin_slug.str()),
			])
		}
...
fn (mut this Class_WC_Admin) prevent_admin_access() {
	mut var_prevent_access := rt.new_bool(false)
	mut var_exempted_paths := ['admin-post.php', 'admin-ajax.php']
	if rt.is_true(rt.call_function('apply_filters', [rt.new_string('woocommerce_disable_admin_bar'), rt.new_bool(true)]))
		&& rt.get_superglobal('_SERVER').array_isset(rt.new_string('SCRIPT_FILENAME'))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.call_function('basename', [rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_SERVER').array_get(rt.new_string('SCRIPT_FILENAME'))])])]), rt.create_array_from_list(var_exempted_paths), rt.new_bool(true)]))))) {
		mut var_has_cap := rt.new_bool(false)
		...
		for var_access_cap in var_access_caps {
			if rt.is_true(rt.call_function('current_user_can', [
				rt.new_string(access_cap),
			]))
			{
				var_has_cap = rt.new_bool(true)
				break
			}
		}
		if rt.is_true(rt.new_bool(!(rt.is_true(var_has_cap)))) {
			var_prevent_access = rt.new_bool(true)
		}
	}
	if rt.is_true(rt.call_function('apply_filters', [
		rt.new_string('woocommerce_prevent_admin_access'),
		var_prevent_access.clone(),
	]))
	{
...
fn (mut this Class_WC_Admin) preview_emails() {
	if rt.get_superglobal('_GET').array_isset(rt.new_string('preview_woocommerce_mail')) {
		if !(rt.get_superglobal('_REQUEST').array_isset(rt.new_string('_wpnonce'))
			&& rt.is_true(rt.call_function('wp_verify_nonce', [rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_REQUEST').array_get(rt.new_string('_wpnonce'))])]), rt.new_string('preview-mail')]))) {
			fn () {
				print((rt.new_string('Security check')).str())
				exit(0)
			}()
		}
		...
		if rt.get_superglobal('_GET').array_isset(rt.new_string('type')) {
			mut var_type_param := rt.call_function('sanitize_text_field', [
				rt.call_function('wp_unslash',
					[rt.get_superglobal('_GET').array_get(rt.new_string('type'))]),
			])
			rt.call_method(var_email_preview, 'set_email_type', [
			...
			if rt.instance_of(var_e_1, 'InvalidArgumentException') {
				...
				rt.call_function('wp_die', [
					rt.call_function('esc_html__', [rt.new_string('Invalid email type.'),
		...
		if rt.is_true(rt.call_function('defined', [rt.new_string('WP_DEBUG')]))
			...
			var_message = rt.call_method(var_email_preview, 'ensure_links_open_in_new_tab', [
				var_message.clone(),
			])
		} else {
			rt.call_function('ob_start', []rt.PhpVal{})
			...
			if rt.instance_of(var_e_2, 'Throwable') {
				...
				rt.call_function('wp_die', [
					rt.call_function('esc_html__', [
						rt.new_string("There was an error rendering the email preview. This doesn't affect actual email delivery. Please contact the extension author for assistance."),
						rt.new_string('woocommerce'),
					]),
					rt.new_int(404),
				])
...
fn (mut this Class_WC_Admin) admin_footer_text(var_footer_text rt.PhpVal) string {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.new_string('manage_woocommerce')])))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [rt.new_string('wc_get_screen_ids')]))))) {
		return var_footer_text_mutated.str()
	}
	mut var_current_screen := rt.call_function('get_current_screen', []rt.PhpVal{})
	mut var_wc_pages := rt.call_function('array_merge', [
		rt.call_function('wc_get_screen_ids', []rt.PhpVal{}),
		rt.create_array([rt.ArrayItem{ key: none, val: 'woocommerce_page_wc-admin' }]),
	])
	var_wc_pages = rt.call_function('array_diff', [var_wc_pages.clone(),
		rt.create_array([rt.ArrayItem{ key: none, val: 'profile' },
			rt.ArrayItem{ key: none, val: 'user-edit' }])])
	if !(rt.get_property(var_current_screen, 'id')).is_null()
		&& rt.is_true(rt.call_function('apply_filters', [rt.new_string('woocommerce_display_admin_footer_text'), rt.call_function('in_array', [rt.get_property(var_current_screen, 'id'), var_wc_pages.clone(), rt.new_bool(true)])])) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('get_option', [
			rt.new_string('woocommerce_admin_footer_text_rated'),
		])))))
		{
			var_footer_text_mutated = rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('If you like %1$s please leave us a %2$s rating. A huge thanks in advance!'),
					rt.new_string('woocommerce'),
				]),
				rt.call_function('sprintf', [
					rt.new_string('<strong>%s</strong>'),
					rt.call_function('esc_html__', [rt.new_string('WooCommerce'),
						rt.new_string('woocommerce')]),
				]),
				rt.new_string(
					'<a href="https://wordpress.org/support/plugin/woocommerce/reviews?rate=5#new-post" target="_blank" class="wc-rating-link" aria-label="' +
					(rt.call_function('esc_attr__', [rt.new_string('five star'), rt.new_string('woocommerce')])).str() +
					'" data-rated="' +
					(rt.call_function('esc_attr__', [rt.new_string('Thanks :)'), rt.new_string('woocommerce')])).str() +
					'">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'),
			])
			mut var_script := rt.new_string(
				...
				(rt.call_function('esc_js', [rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'ajax_url', []rt.PhpVal{})])).str() +
				"', {\n\t\t                            method: 'POST',\n\t\t                            body: formData,\n\t\t                            credentials: 'same-origin'\n\t\t                        });\n\t\t                        \n\t\t                        var parent = link.parentElement;\n\t\t                        if (parent) {\n\t\t                            parent.textContent = link.getAttribute('data-rated');\n\t\t                        }\n\t\t                    });\n\t\t                }\n\t\t            })();\n\t\t            ")
			mut var_handle := rt.new_string('wc-admin-footer-rating')
			...
			var_footer_text_mutated = rt.call_function('__', [
				rt.new_string('Thank you for selling with WooCommerce.'),
				rt.new_string('woocommerce'),
			])
		}
	}
...
fn (mut this Class_WC_Admin) update_footer_version(var_version rt.PhpVal) rt.PhpVal {
	...
	{
		return var_version_mutated.clone()
	}
	mut var_current_screen := rt.call_function('get_current_screen', []rt.PhpVal{})
	mut var_wc_pages := rt.call_function('array_merge', [
		rt.call_function('wc_get_screen_ids', []rt.PhpVal{}),
	...
	var_wc_pages = rt.call_function('array_diff', [var_wc_pages.clone(),
		rt.create_array([rt.ArrayItem{ key: none, val: 'profile' },
			rt.ArrayItem{ key: none, val: 'user-edit' }])])
	if !(rt.get_property(var_current_screen, 'id')).is_null()
		&& rt.is_true(rt.call_function('apply_filters', [rt.new_string('woocommerce_display_update_footer_text'), rt.call_function('in_array', [rt.get_property(var_current_screen, 'id'), var_wc_pages.clone(), rt.new_bool(true)])])) {
		var_version_mutated = rt.call_function('sprintf', [
...
fn (mut this Class_WC_Admin) setup_wizard_check_jetpack() {
	mut var_jetpack_active := rt.call_function('class_exists', [
		rt.new_string('Jetpack')])
	rt.call_function('wp_send_json_success', [
		rt.create_array([
			rt.ArrayItem{
				key: 'is_active'
				val: if rt.is_true(var_jetpack_active) { 'yes' } else { 'no' }
			},
		]),
	])
}
...
fn (mut this Class_WC_Admin) disable_webhook_post_export(var_args rt.PhpVal) rt.PhpVal {
	...
	var_args_mutated.array_set('can_export', false)
	return var_args_mutated.clone()
}

fn (mut this Class_WC_Admin) include_admin_body_class(var_classes rt.PhpVal) rt.PhpVal {
	mut var_raw_version := rt.call_function('get_bloginfo', [
		rt.new_string('version')])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_raw_version)))) {
		return var_classes.clone()
	}
	mut var_version_parts := rt.call_function('explode', [rt.new_string('-'),
		var_raw_version.clone()])
	mut var_version := if var_version_parts.clone().array_count() > 1 {
		var_version_parts.array_get(rt.new_int(0))
	} else {
		var_raw_version
	}
	mut var_class_list := rt.call_function('explode', [rt.new_string(' '),
		var_classes.clone()])
	mut var_version_classes := {
		'5.3': 'wc-wp-version-gte-53'
		'5.5': 'wc-wp-version-gte-55'
		'7.0': 'wc-wp-version-gte-70'
	}
	for var_min_version, var_class_name in var_version_classes {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string(class_name), var_class_list.clone(), rt.new_bool(true)])))))
			&& rt.is_true(rt.call_function('version_compare', [var_version.clone(), rt.new_string(min_version), rt.new_string('>=')])) {
			var_classes = rt.concat(var_classes, rt.new_string(' ' + class_name))
		}
	}
	return var_classes.clone()
}
...
fn main() {
	...
	return rt.new_object('WC_Admin', []string{}, create_wc_admin())
}
