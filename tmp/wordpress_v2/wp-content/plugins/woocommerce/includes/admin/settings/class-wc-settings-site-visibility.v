...
fn (mut this Class_WC_Settings_Site_Visibility) get_settings_for_default_section() rt.PhpVal {
	mut var_settings := [
		[rt.new_string('wc_settings_site_visibility_slotfill'),
			rt.new_string('slotfill_placeholder')],
	]
...
fn main() {
	...
	if rt.is_true(rt.call_function('class_exists', [
		rt.new_string('WC_Settings_Site_Visibility'),
		rt.new_bool(false),
	]))
	{
		return rt.new_object('WC_Settings_Site_Visibility', ['WC_Settings_Page'],
			create_wc_settings_site_visibility())
	}
	return rt.new_object('WC_Settings_Site_Visibility', ['WC_Settings_Page'],
		create_wc_settings_site_visibility())
}
