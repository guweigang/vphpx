...
fn main() {
	...
	rt.echo_val(rt.call_function('absint', [
		rt.new_int(var_shipping_provider_columns.clone().array_count() + 1),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [
	...
	rt.call_function('esc_html_e', [rt.new_string('Close modal panel'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	mut iter_2 := var_shipping_provider_columns.iterator()
	for {
			...
			rt.new_string('wc-shipping-provider-tracking-url-template')))
		{
			...
			rt.call_function('do_action', [
				rt.new_string('woocommerce_shipping_providers_column_' + var_class.str()),
			])
		}
		print('</div>')
	}
	...
	// unsupported statement: Stmt_InlineHTML
	mut iter_3 := var_shipping_provider_columns.iterator()
	for {
