...
fn Class_WC_Notes_Refund_Returns.init() {
	rt.call_function('add_action', [rt.new_string('woocommerce_newly_installed'),
		rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT },
			rt.ArrayItem{ key: none, val: 'on_newly_installed' }])])
...
fn Class_WC_Notes_Refund_Returns.on_newly_installed() {
	mut var_page_id := rt.call_function('get_option', [
		rt.new_string('woocommerce_refund_returns_page_id'),
	])
	if rt.is_true(var_page_id) {
		Class_WC_Notes_Refund_Returns.possibly_add_note(var_page_id.clone())
	}
}
...
fn Class_WC_Notes_Refund_Returns.possibly_add_note(var_page_id rt.PhpVal) {
	...
	mut var_note_id := rt.call_method(var_data_store, 'get_notes_with_name', [
		rt.new_string(Class_WC_Notes_Refund_Returns.note_name()),
	])
	if !(!rt.is_true(var_note_id)) {
		mut var_note := create_automattic_woocommerce_admin_notes_note(var_note_id.clone())
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_note)))) || rt.is_true(rt.identical(Class_{
			nodeType: 'Expr_Variable'
			line:     62
			name:     'note'
		}.e_wc_admin_note_actioned(), rt.call_method(var_note, 'get_status', []rt.PhpVal{}))) {
			return
		}
	}
	var_note = Class_WC_Notes_Refund_Returns.get_note(var_page_id_mutated.clone())
	rt.call_method(var_note, 'save', []rt.PhpVal{})
	rt.call_function('delete_option', [
		rt.new_string('woocommerce_refund_returns_page_created'),
	])
}
...
fn Class_WC_Notes_Refund_Returns.get_note(var_page_id rt.PhpVal) rt.PhpVal {
	...
	rt.call_method(var_note, 'set_title', [
		rt.call_function('__', [
			rt.new_string("Setup a Refund and Returns Policy page to boost your store's credibility."),
			rt.new_string('woocommerce'),
		]),
	])
	rt.call_method(var_note, 'set_content', [
		rt.call_function('__', [
			rt.new_string('We have created a sample draft Refund and Returns Policy page for you. Please have a look and update it to fit your store.'),
			rt.new_string('woocommerce'),
		]),
	])
	...
	rt.call_method(var_note, 'add_action', [rt.new_string('notify-refund-returns-page'),
		rt.call_function('__', [rt.new_string('Edit page'), rt.new_string('woocommerce')]),
		rt.call_function('admin_url', [
			rt.call_function('sprintf', [rt.new_string('post.php?post=%d&action=edit'),
				rt.new_int(var_page_id_mutated.to_i64())]),
		])])
...
fn Class_WC_Notes_Refund_Returns.get_note_from_db(var_note_from_db rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(Class_WC_Notes_Refund_Returns.note_name(), rt.call_method(var_note_from_db,
		'get_name', []rt.PhpVal{})))
	{
		mut var_note := Class_WC_Notes_Refund_Returns.get_note(rt.new_int(0))
		rt.call_method(var_note_from_db, 'set_title', [
			rt.call_method(var_note, 'get_title', []rt.PhpVal{}),
		])
		rt.call_method(var_note_from_db, 'set_content', [
			rt.call_method(var_note, 'get_content', []rt.PhpVal{}),
		])
		mut var_action_from_db := rt.call_method(var_note_from_db, 'get_action', [
			rt.new_string('notify-refund-returns-page'),
		])
		mut var_action_from_class := rt.call_method(var_note, 'get_action', [
			rt.new_string('notify-refund-returns-page'),
		])
		if rt.is_true(var_action_from_db) && rt.is_true(var_action_from_class) {
			rt.set_property(var_action_from_db, 'label', rt.get_property(var_action_from_class,
				'label'))
			rt.call_method(var_note_from_db, 'set_actions', [
				rt.create_array([rt.ArrayItem{ key: none, val: var_action_from_db }]),
			])
		}
	}
	return var_note_from_db.clone()
