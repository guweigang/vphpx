...
fn (mut this Class_WC_Report_Sales_By_Category) construct() {
	if rt.get_superglobal('_GET').array_isset(rt.new_string('show_categories')) {
		this.show_categories = if rt.get_superglobal('_GET').array_get(rt.new_string('show_categories')).is_array() { rt.call_function('array_map', [
				rt.new_string('absint'),
				rt.get_superglobal('_GET').array_get(rt.new_string('show_categories')),
...
fn (mut this Class_WC_Report_Sales_By_Category) get_products_in_category(var_category_id rt.PhpVal) rt.PhpVal {
	mut var_term_ids := rt.call_function('get_term_children', [
		var_category_id.clone(), rt.new_string('product_cat')])
	var_term_ids.array_push(var_category_id.clone())
	mut var_product_ids := rt.call_function('get_objects_in_term', [
		var_term_ids.clone(), rt.new_string('product_cat')])
	return rt.call_function('array_unique', [
		rt.call_function('apply_filters', [
			rt.new_string('woocommerce_report_sales_by_category_get_products_in_category'),
...
fn (mut this Class_WC_Report_Sales_By_Category) get_chart_legend() rt.PhpVal {
	...
	for {
		...
		mut var_product_ids := this.get_products_in_category(rt.get_property(var_category,
			'term_id'))
		...
		for {
			...
			if this.item_sales.array_isset(var_id) {
				var_total = rt.add(var_total, this.item_sales.array_get(var_id))
			}
		}
		var_legend << rt.create_array([
			rt.ArrayItem{ key: 'title', val: rt.call_function('sprintf', [
				rt.call_function('__', [rt.new_string('%1$s sales in %2$s'),
					rt.new_string('woocommerce')]),
				rt.new_string('<strong>' +
					(rt.call_function('wc_price', [var_total.clone()])).str() + '</strong>'),
				rt.get_property(var_category, 'name'),
			]) },
			rt.ArrayItem{
				key: 'color'
				val: if this.chart_colours.array_isset(var_index) {
					this.chart_colours.array_get(var_index)
				} else {
					this.chart_colours.array_get(rt.new_int(0))
				}
			},
			rt.ArrayItem{ key: 'highlight_series', val: var_index },
		])
...
fn (mut this Class_WC_Report_Sales_By_Category) output_report() {
	...
	this.chart_colours = rt.create_array([rt.ArrayItem{ key: none, val: '#3498db' },
		rt.ArrayItem{ key: none, val: '#34495e' }, rt.ArrayItem{ key: none, val: '#1abc9c' },
		rt.ArrayItem{ key: none, val: '#2ecc71' }, rt.ArrayItem{ key: none, val: '#f1c40f' },
		rt.ArrayItem{ key: none, val: '#e67e22' }, rt.ArrayItem{ key: none, val: '#e74c3c' },
		rt.ArrayItem{ key: none, val: '#2980b9' }, rt.ArrayItem{ key: none, val: '#8e44ad' },
		rt.ArrayItem{ key: none, val: '#2c3e50' }, rt.ArrayItem{ key: none, val: '#16a085' },
		rt.ArrayItem{ key: none, val: '#27ae60' }, rt.ArrayItem{ key: none, val: '#f39c12' },
		rt.ArrayItem{ key: none, val: '#d35400' }, rt.ArrayItem{ key: none, val: '#c0392b' }])
	...
	if !(!rt.is_true(this.show_categories)) {
		mut var_order_items := this.get_order_report_data(rt.create_array([
			...
			rt.ArrayItem{ key: 'group_by', val: 'ID, product_id, post_date' },
		...
		if rt.is_true(rt.new_bool(var_order_items.clone().is_array())) {
			mut iter_3 := var_order_items.iterator()
			for {
				item_3 := iter_3.next() or { break }
				mut var_order_item := item_3.val
				mut switch_val_1 := rt.get_property(rt.new_object('WC_Report_Sales_By_Category', [
					'WC_Admin_Report',
				], &this), 'chart_groupby')
				if rt.is_true(rt.equal(switch_val_1, rt.new_string('day'))) {
					mut var_time := rt.mul(rt.call_function('strtotime', [
						rt.call_function('gmdate', [rt.new_string('Ymd'),
							rt.call_function('strtotime', [
								rt.get_property(var_order_item, 'post_date'),
							])]),
					]), rt.new_int(1000))
				} else {
					var_time = rt.mul(rt.call_function('strtotime', [
						rt.new_string(
							(rt.call_function('gmdate', [rt.new_string('Ym'), rt.call_function('strtotime', [rt.get_property(var_order_item, 'post_date')])])).str() +
							'01'),
					]), rt.new_int(1000))
				}
				this.item_sales_and_times.array_get_mut(var_time).array_set(rt.get_property(var_order_item,
					'product_id'), if this.item_sales_and_times.array_get(var_time).array_isset(rt.get_property(var_order_item,
					'product_id'))
				{
					rt.add(this.item_sales_and_times.array_get(var_time).array_get(rt.get_property(var_order_item,
						'product_id')), rt.get_property(var_order_item, 'order_item_amount'))
				} else {
					rt.get_property(var_order_item, 'order_item_amount')
				})
				this.item_sales.array_set(rt.get_property(var_order_item, 'product_id'), if this.item_sales.array_isset(rt.get_property(var_order_item,
					'product_id'))
				{
					rt.add(this.item_sales.array_get(rt.get_property(var_order_item, 'product_id')), rt.get_property(var_order_item,
						'order_item_amount'))
				} else {
					rt.get_property(var_order_item, 'order_item_amount')
				})
	...
	return rt.create_array([
		rt.ArrayItem{ key: none, val: rt.create_array([
			...
			rt.ArrayItem{ key: 'callback', val: rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('WC_Report_Sales_By_Category', [
					'WC_Admin_Report',
				], &this) },
				rt.ArrayItem{ key: none, val: 'category_widget' },
			]) },
		]) },
	])
}
...
fn (mut this Class_WC_Report_Sales_By_Category) category_widget() {
	mut var_categories := rt.call_function('get_terms', [rt.new_string('product_cat'),
		rt.create_array([rt.ArrayItem{ key: 'orderby', val: 'name' }])])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_attr_e', [rt.new_string('Select categories&hellip;'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	...
	rt.echo_val(rt.call_function('wc_walk_category_dropdown_tree', [
		var_categories.clone(), rt.new_int(0), rt.create_array_from_native_map(var_r)]))
	// unsupported statement: Stmt_InlineHTML
	// unsupported statement: Stmt_InlineHTML
...
fn (mut this Class_WC_Report_Sales_By_Category) get_export_button() {
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [
		rt.get_property(rt.new_object('WC_Report_Sales_By_Category', [
			'WC_Admin_Report',
		], &this), 'chart_groupby'),
	]))
	// unsupported statement: Stmt_InlineHTML
...
fn (mut this Class_WC_Report_Sales_By_Category) get_main_chart() {
	...
	if !rt.is_true(this.show_categories) {
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_html_e', [rt.new_string('Choose a category to view stats'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
	} else {
		mut var_chart_data := rt.new_array()
		mut var_index := rt.new_int(0)
		mut iter_4 := this.show_categories.iterator()
		for {
			...
			for {
				if !(rt.is_true(rt.less_equal(var_i, rt.get_property(rt.new_object('WC_Report_Sales_By_Category', [
					'WC_Admin_Report',
				], &this), 'chart_interval')))) { break
				 }
				mut var_interval_total := rt.new_int(0)
				mut switch_val_2 := rt.get_property(rt.new_object('WC_Report_Sales_By_Category', [
					'WC_Admin_Report',
				], &this), 'chart_groupby')
				if rt.is_true(rt.equal(switch_val_2, rt.new_string('day'))) {
					mut var_time := rt.mul(rt.call_function('strtotime', [
						rt.call_function('gmdate', [rt.new_string('Ymd'),
							rt.call_function('strtotime', [
								rt.new_string('+${var_i.to_string()} DAY'),
								rt.get_property(rt.new_object('WC_Report_Sales_By_Category', [
									'WC_Admin_Report',
								], &this), 'start_date'),
							])]),
					...
					var_time = rt.mul(rt.call_function('strtotime', [
						rt.new_string(
							(rt.call_function('gmdate', [rt.new_string('Ym'), rt.call_function('strtotime', [rt.new_string('+${var_i.to_string()} MONTH'), rt.get_property(rt.new_object('WC_Report_Sales_By_Category', ['WC_Admin_Report'], &this), 'start_date')])])).str() +
							'01'),
					]), rt.new_int(1000))
				}
				mut iter_5 := var_product_ids.iterator()
				for {
					item_5 := iter_5.next() or { break }
					mut var_id := item_5.val
					if this.item_sales_and_times.array_get(var_time).array_isset(var_id) {
						var_interval_total = rt.add(var_interval_total,
							this.item_sales_and_times.array_get(var_time).array_get(var_id))
					}
				}
				var_category_chart_data << rt.create_array([
					rt.ArrayItem{ key: none, val: var_time },
					rt.ArrayItem{ key: none, val: rt.new_float((rt.call_function('wc_format_decimal', [
						var_interval_total.clone(),
						rt.call_function('wc_get_price_decimals', []rt.PhpVal{}),
					])).to_f64()) },
				])
				rt.post_inc(var_i)
			}
			var_chart_data.array_get_mut(rt.get_property(var_category, 'term_id')).array_set('category', rt.get_property(var_category,
				'name'))
			var_chart_data.array_get_mut(rt.get_property(var_category, 'term_id')).array_set('data',
				var_category_chart_data.clone())
			rt.post_inc(var_index)
		}
		// unsupported statement: Stmt_InlineHTML
		// unsupported statement: Stmt_InlineHTML
		var_index = rt.new_int(0)
		mut iter_6 := var_chart_data.iterator()
		for {
			item_6 := iter_6.next() or { break }
			mut var_data := item_6.val
			mut var_color := if this.chart_colours.array_isset(var_index) {
				this.chart_colours.array_get(var_index)
			} else {
				this.chart_colours.array_get(rt.new_int(0))
			}
			mut var_width := rt.div(rt.get_property(rt.new_object('WC_Report_Sales_By_Category', [
				'WC_Admin_Report',
			], &this), 'barwidth'), rt.new_int(var_chart_data.clone().array_count()))
			mut var_offset := rt.mul(var_width, var_index)
			mut var_series := var_data.array_get(rt.new_string('data'))
			mut iter_7 := var_series.iterator()
			for {
				item_7 := iter_7.next() or { break }
				mut var_series_data := item_7.val
				mut var_key := item_7.key
				var_series.array_get_mut(var_key).array_set(0, rt.add(var_series_data.array_get(rt.new_int(0)),
					var_offset))
			}
			var_series = rt.call_function('wp_json_encode', [
				var_series.clone()])
			print('{\n\t\t\t\t\t\t\t\t\t\t\tlabel: "' +
				(rt.call_function('esc_js', [var_data.array_get(rt.new_string('category'))])).str() +
				'",\n\t\t\t\t\t\t\t\t\t\t\tdata: JSON.parse( decodeURIComponent( "' +
				(rt.call_function('rawurlencode', [var_series.clone()])).str() +
				'" ) ),\n\t\t\t\t\t\t\t\t\t\t\tcolor: "' + var_color.str() +
				'",\n\t\t\t\t\t\t\t\t\t\t\tbars: {\n\t\t\t\t\t\t\t\t\t\t\t\tfillColor: "' +
				var_color.str() +
				'",\n\t\t\t\t\t\t\t\t\t\t\t\tfill: true,\n\t\t\t\t\t\t\t\t\t\t\t\tshow: true,\n\t\t\t\t\t\t\t\t\t\t\t\tlineWidth: 1,\n\t\t\t\t\t\t\t\t\t\t\t\talign: "center",\n\t\t\t\t\t\t\t\t\t\t\t\tbarWidth: ' +
				var_width * 0.75.str() +
				',\n\t\t\t\t\t\t\t\t\t\t\t\tstack: false\n\t\t\t\t\t\t\t\t\t\t\t},\n\t\t\t\t\t\t\t\t\t\t\t' +
				(this.get_currency_tooltip()).str() + ',\n\t\t\t\t\t\t\t\t\t\t\tenable_tooltip: true,\n\t\t\t\t\t\t\t\t\t\t\tprepend_label: true\n\t\t\t\t\t\t\t\t\t\t},')
			rt.post_inc(var_index)
		}
		// unsupported statement: Stmt_InlineHTML
		print(if rt.is_true(rt.identical(rt.new_string('day'), rt.get_property(rt.new_object('WC_Report_Sales_By_Category', [
			'WC_Admin_Report',
		], &this), 'chart_groupby')))
		{ '%d %b' } else { '%b' })
		// unsupported statement: Stmt_InlineHTML
		...
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.get_property(rt.new_object('WC_Report_Sales_By_Category', [
			'WC_Admin_Report',
		], &this), 'chart_groupby'))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.get_property(rt.new_object('WC_Report_Sales_By_Category', [
