...
fn (mut this Class_WC_Report_Downloads) display_tablenav(var_position rt.PhpVal) {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('top'), var_position)))) {
		this.Class_WP_List_Table.display_tablenav(var_position.clone())
	}
}

fn (mut this Class_WC_Report_Downloads) output_report() {
	this.prepare_items()
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('permission_id')))) {
		mut var_permission_id := rt.call_function('absint', [rt.get_superglobal('_GET').array_get(rt.new_string('permission_id'))])
		mut var_permission := rt.new_null()
		mut var_product := rt.new_null()
		var_permission = create_wc_customer_download(var_permission_id.clone())
...
catch_label_1:
		...
		if rt.instance_of(var_e_1, 'Exception') {
			...
			rt.call_function('wp_die', [rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Permission #%d not found.'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_permission_id.clone()])])])
			unsafe { goto end_label_1 }
		}
		else {
...
end_label_1:
	...
	for {
		...
		if rt.is_true(rt.equal(switch_val_1, rt.new_string('order_id'))) {
			mut var_order := rt.call_function('wc_get_order', [var_value.clone()])
			if rt.is_true(var_order) {
			mut var_display_value := rt.new_string((rt.call_function('_x', [rt.new_string('#'), rt.new_string('hash before order number'), rt.new_string('woocommerce')])).str() + (rt.call_method(var_order, 'get_order_number', []rt.PhpVal{})).str())
			} else {
			}
		} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('product_id'))) {
			var_product = rt.call_function('wc_get_product', [var_value.clone()])
		...
		var_filter_list << (var_filter_names.array_get(var_key)).str() + ' ' + (var_display_value).str() + ' <a href="' + (rt.call_function('esc_url', [rt.call_function('remove_query_arg', [var_key.clone()])])).str() + '" class="woocommerce-reports-remove-filter">&times;</a>'
	}
	print('</h1>')
	...
	rt.echo_val(if rt.is_true(var_filter_list) { rt.call_function('wp_kses_post', [rt.call_function('implode', [rt.new_string(', '), rt.create_array_from_list(var_filter_list)])]) } else { rt.new_string('') })
	print('</h2></div>')
	...
	if rt.is_true(rt.equal(switch_val_2, rt.new_string('timestamp'))) {
		...
		if !(!rt.is_true(var_permission)) && !(!rt.is_true(var_product)) {
			...
			if rt.is_true(rt.identical(rt.new_bool(false), var_file)) {
				...
				rt.echo_val(rt.call_function('esc_html', [rt.new_string((rt.call_method(var_file, 'get_name', []rt.PhpVal{})).str() + ' - ' + (rt.call_function('basename', [rt.call_method(var_file, 'get_file', []rt.PhpVal{})])).str())]))
				print('<div class="row-actions">')
		...
		mut var_order := rt.call_function('wc_get_order', [rt.get_property(var_permission, 'order_id')])
		if !(!rt.is_true(var_permission)) && rt.is_true(var_order) {
			rt.call_function('edit_post_link', [rt.call_function('esc_html', [rt.new_string((rt.call_function('_x', [rt.new_string('#'), rt.new_string('hash before order number'), rt.new_string('woocommerce')])).str() + (rt.call_method(var_order, 'get_order_number', []rt.PhpVal{})).str())]), rt.new_string(''), rt.new_string(''), rt.get_property(var_permission, 'order_id'), rt.new_string('view-link')])
			print('<div class="row-actions">')
...
fn (mut this Class_WC_Report_Downloads) prepare_items() {
	...
	this.get_items(var_current_page.clone(), var_per_page.clone())
...
fn (mut this Class_WC_Report_Downloads) get_filter_vars() rt.PhpVal {
	mut var_product_id := if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('product_id')))) { rt.call_function('absint', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('product_id'))])]) } else { rt.new_null() }
	mut var_download_id := if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('download_id')))) { rt.call_function('wc_clean', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('download_id'))])]) } else { rt.new_null() }
	mut var_permission_id := if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('permission_id')))) { rt.call_function('absint', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('permission_id'))])]) } else { rt.new_null() }
	mut var_order_id := if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('order_id')))) { rt.call_function('absint', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('order_id'))])]) } else { rt.new_null() }
	mut var_user_id := if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('user_id')))) { rt.call_function('absint', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('user_id'))])]) } else { rt.new_null() }
	mut var_user_ip_address := if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('user_ip_address')))) { rt.call_function('wc_clean', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('user_ip_address'))])]) } else { rt.new_null() }
	return mut rt.array_to_object(rt.create_array([rt.ArrayItem{ key: 'product_id', val: var_product_id }, rt.ArrayItem{ key: 'download_id', val: var_download_id }, rt.ArrayItem{ key: 'permission_id', val: var_permission_id }, rt.ArrayItem{ key: 'order_id', val: var_order_id }, rt.ArrayItem{ key: 'user_id', val: var_user_id }, rt.ArrayItem{ key: 'user_ip_address', val: var_user_ip_address }]))
}
...
fn (mut this Class_WC_Report_Downloads) get_items(var_current_page rt.PhpVal, var_per_page rt.PhpVal) {
	...
	mut var_filters := this.get_filter_vars()
	mut iife_temp_0 := Class_WC_Customer_Download_Log_Data_Store{}
	mut iife_result_0 := iife_temp_0.get_table_name()
	mut var_table := rt.new_string((rt.get_property(var_wpdb, 'prefix')).str() + (iife_result_0).str())
	mut var_query_from := rt.new_string(" FROM ${var_table.to_string()} as downloads ")
	if !(rt.get_property(var_filters, 'product_id').is_null()) || !(rt.get_property(var_filters, 'download_id').is_null()) || !(rt.get_property(var_filters, 'order_id').is_null()) {
		var_query_from = rt.concat(var_query_from, rt.concat(rt.concat(rt.new_string(' LEFT JOIN '), rt.get_property(var_wpdb, 'prefix')), rt.new_string('woocommerce_downloadable_product_permissions as permissions on downloads.permission_id = permissions.permission_id ')))
	}
	var_query_from = rt.concat(var_query_from, rt.new_string(' WHERE 1=1 '))
	if !(rt.get_property(var_filters, 'product_id').is_null()) {
		var_query_from = rt.concat(var_query_from, rt.call_method(var_wpdb, 'prepare', [rt.new_string(' AND product_id = %d '), rt.get_property(var_filters, 'product_id')]))
	}
	if !(rt.get_property(var_filters, 'download_id').is_null()) {
		var_query_from = rt.concat(var_query_from, rt.call_method(var_wpdb, 'prepare', [rt.new_string(' AND download_id = %s '), rt.get_property(var_filters, 'download_id')]))
	}
	if !(rt.get_property(var_filters, 'order_id').is_null()) {
		var_query_from = rt.concat(var_query_from, rt.call_method(var_wpdb, 'prepare', [rt.new_string(' AND order_id = %d '), rt.get_property(var_filters, 'order_id')]))
	}
	if !(rt.get_property(var_filters, 'permission_id').is_null()) {
		var_query_from = rt.concat(var_query_from, rt.call_method(var_wpdb, 'prepare', [rt.new_string(' AND downloads.permission_id = %d '), rt.get_property(var_filters, 'permission_id')]))
	}
	if !(rt.get_property(var_filters, 'user_id').is_null()) {
		var_query_from = rt.concat(var_query_from, rt.call_method(var_wpdb, 'prepare', [rt.new_string(' AND downloads.user_id = %d '), rt.get_property(var_filters, 'user_id')]))
	}
	if !(rt.get_property(var_filters, 'user_ip_address').is_null()) {
		var_query_from = rt.concat(var_query_from, rt.call_method(var_wpdb, 'prepare', [rt.new_string(' AND user_ip_address = %s '), rt.get_property(var_filters, 'user_ip_address')]))
	}
	var_query_from = rt.call_function('apply_filters', [rt.new_string('woocommerce_report_downloads_query_from'), var_query_from.clone()])
	mut var_query_order := rt.call_method(var_wpdb, 'prepare', [rt.new_string('ORDER BY timestamp DESC LIMIT %d, %d;'), rt.mul(rt.sub(var_current_page_mutated, rt.new_int(1)), var_per_page_mutated), var_per_page_mutated.clone()])
	this.dispatch_set_prop('items', rt.call_method(var_wpdb, 'get_results', [rt.new_string("SELECT * ${var_query_from.to_string()} ${var_query_order.to_string()}")]))
	this.max_items = rt.call_method(var_wpdb, 'get_var', [rt.new_string("SELECT COUNT( DISTINCT download_log_id ) ${var_query_from.to_string()};")])
}
