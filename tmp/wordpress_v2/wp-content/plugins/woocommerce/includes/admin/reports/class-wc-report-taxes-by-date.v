...
fn (mut this Class_WC_Report_Taxes_By_Date) get_export_button() {
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('date_i18n', [rt.new_string('Y-m-d'),
...
fn (mut this Class_WC_Report_Taxes_By_Date) get_main_chart() {
	mut var_query_data := {
		...
		'_order_total':        {
			'type':     rt.new_string('meta')
			'function': rt.new_string('SUM')
			'name':     rt.new_string('total_sales')
		}
		'_order_shipping':     {
			'type':     rt.new_string('meta')
			'function': rt.new_string('SUM')
			'name':     rt.new_string('total_shipping')
		}
		'ID':                  {
			'type':     rt.new_string('post_data')
			'function': rt.new_string('COUNT')
			'name':     rt.new_string('total_orders')
			'distinct': rt.new_bool(true)
		}
		'post_date':           {
			'type':     rt.new_string('post_data')
	...
	mut var_tax_rows_orders := this.get_order_report_data(rt.create_array([
		...
		rt.ArrayItem{ key: 'group_by', val: rt.get_property(rt.new_object('WC_Report_Taxes_By_Date', [
			'WC_Admin_Report',
		], &this), 'group_by_query') },
	...
	for {
		...
		mut var_key := rt.call_function('date', [
			rt.new_string((if rt.is_true(rt.identical(rt.new_string('month'), rt.get_property(rt.new_object('WC_Report_Taxes_By_Date', [
				'WC_Admin_Report',
			], &this), 'chart_groupby')))
			{ 'Ym' } else { 'Ymd' }).str()),
			rt.call_function('strtotime', [
				rt.get_property(var_tax_row, 'post_date'),
			]),
		])
		var_tax_rows.array_set(var_key, if var_tax_rows.array_isset(var_key) { var_tax_rows.array_get(var_key) } else { rt.array_to_object(rt.create_array([
				...
				rt.ArrayItem{ key: 'total_sales', val: 0 },
				rt.ArrayItem{ key: 'total_shipping', val: 0 },
				rt.ArrayItem{ key: 'total_orders', val: 0 },
			])) })
	}
	mut iter_2 := var_tax_rows_orders.iterator()
	for {
		...
		rt.get_property(var_tax_rows.array_get(var_key), 'total_orders') = rt.add(rt.get_property(var_tax_rows.array_get(var_key),
			'total_orders'), rt.get_property(var_tax_row, 'total_orders'))
		rt.get_property(var_tax_rows.array_get(var_key), 'tax_amount') = rt.add(rt.get_property(var_tax_rows.array_get(var_key),
			'tax_amount'), rt.get_property(var_tax_row, 'tax_amount'))
		rt.get_property(var_tax_rows.array_get(var_key), 'shipping_tax_amount') = rt.add(rt.get_property(var_tax_rows.array_get(var_key),
			'shipping_tax_amount'), rt.get_property(var_tax_row, 'shipping_tax_amount'))
		rt.get_property(var_tax_rows.array_get(var_key), 'total_sales') = rt.add(rt.get_property(var_tax_rows.array_get(var_key),
			'total_sales'), rt.get_property(var_tax_row, 'total_sales'))
		rt.get_property(var_tax_rows.array_get(var_key), 'total_shipping') = rt.add(rt.get_property(var_tax_rows.array_get(var_key),
			'total_shipping'), rt.get_property(var_tax_row, 'total_shipping'))
	}
	mut iter_3 := var_tax_rows_partial_refunds.iterator()
	for {
	...
	mut iter_4 := var_tax_rows_full_refunds.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_tax_row := item_4.val
		mut var_key := rt.call_function('date', [
			rt.new_string((if rt.is_true(rt.identical(rt.new_string('month'), rt.get_property(rt.new_object('WC_Report_Taxes_By_Date', [
				'WC_Admin_Report',
			], &this), 'chart_groupby')))
		...
		mut var_parent_order := rt.call_function('wc_get_order', [
			rt.get_property(var_tax_row, 'post_parent'),
		])
		if rt.is_true(var_parent_order) {
			rt.get_property(var_tax_rows.array_get(var_key), 'tax_amount') = rt.add(rt.get_property(var_tax_rows.array_get(var_key),
				'tax_amount'), rt.mul(rt.call_method(var_parent_order, 'get_cart_tax',
				[]rt.PhpVal{}), -1))
			rt.get_property(var_tax_rows.array_get(var_key), 'shipping_tax_amount') = rt.add(rt.get_property(var_tax_rows.array_get(var_key),
				'shipping_tax_amount'), rt.mul(rt.call_method(var_parent_order, 'get_shipping_tax',
				[]rt.PhpVal{}), -1))
			rt.get_property(var_tax_rows.array_get(var_key), 'total_sales') = rt.add(rt.get_property(var_tax_rows.array_get(var_key),
				'total_sales'),
				rt.mul(rt.call_method(var_parent_order, 'get_total', []rt.PhpVal{}), -1))
			rt.get_property(var_tax_rows.array_get(var_key), 'total_shipping') = rt.add(rt.get_property(var_tax_rows.array_get(var_key),
				'total_shipping'), rt.mul(rt.call_method(var_parent_order, 'get_shipping_total',
				[]rt.PhpVal{}), -1))
		}
	}
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('_e', [rt.new_string('Number of orders'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	...
	rt.echo_val(rt.call_function('wc_help_tip', [
		rt.call_function('__', [
			rt.new_string("This is the sum of the 'Order total' field within your orders."),
			rt.new_string('woocommerce'),
		]),
	]))
	// unsupported statement: Stmt_InlineHTML
	...
	rt.echo_val(rt.call_function('wc_help_tip', [
		rt.call_function('__', [
			rt.new_string("This is the sum of the 'Shipping total' field within your orders."),
			rt.new_string('woocommerce'),
		]),
	]))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wc_help_tip', [
		rt.call_function('__', [rt.new_string('Total sales minus shipping and tax.'),
			rt.new_string('woocommerce')]),
	]))
	// unsupported statement: Stmt_InlineHTML
	if !(!rt.is_true(var_tax_rows)) {
		// unsupported statement: Stmt_InlineHTML
		mut iter_5 := var_tax_rows.iterator()
		for {
			item_5 := iter_5.next() or { break }
			mut var_tax_row := item_5.val
			mut var_date := item_5.key
			mut var_gross := rt.sub(rt.get_property(var_tax_row, 'total_sales'), rt.get_property(var_tax_row,
				'total_shipping'))
			mut var_total_tax := rt.add(rt.get_property(var_tax_row, 'tax_amount'), rt.get_property(var_tax_row,
				'shipping_tax_amount'))
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(if rt.is_true(rt.identical(rt.new_string('month'), rt.get_property(rt.new_object('WC_Report_Taxes_By_Date', [
				'WC_Admin_Report',
			], &this), 'chart_groupby')))
			{ rt.call_function('date_i18n', [rt.new_string('F'),
					rt.call_function('strtotime', [
						rt.new_string(var_date.str() + '01'),
					])]) } else { rt.call_function('date_i18n', [
					rt.call_function('get_option', [rt.new_string('date_format')]),
					rt.call_function('strtotime', [var_date.clone()]),
				]) })
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.get_property(var_tax_row, 'total_orders'))
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('wc_price', [var_gross.clone()]))
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('wc_price', [
				rt.get_property(var_tax_row, 'total_shipping'),
			]))
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('wc_price', [var_total_tax.clone()]))
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('wc_price', [rt.sub(var_gross, var_total_tax)]))
			// unsupported statement: Stmt_InlineHTML
		}
		// unsupported statement: Stmt_InlineHTML
		mut var_gross := rt.sub(rt.call_function('array_sum', [
		...
		mut var_total_tax := rt.add(rt.call_function('array_sum', [
			rt.call_function('wp_list_pluck', [rt.cast_array(var_tax_rows),
		...
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('array_sum', [
			rt.call_function('wp_list_pluck', [rt.cast_array(var_tax_rows),
				rt.new_string('total_orders')]),
		]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('wc_price', [var_gross.clone()]))
		// unsupported statement: Stmt_InlineHTML
		...
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('_e', [rt.new_string('No taxes found in this period'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
	}
	// unsupported statement: Stmt_InlineHTML
...
fn create_wc_report_taxes_by_date(_args ...rt.PhpVal) &Class_WC_Report_Taxes_By_Date {
