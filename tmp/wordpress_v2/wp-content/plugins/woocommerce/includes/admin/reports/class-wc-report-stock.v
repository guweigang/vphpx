...
fn (mut this Class_WC_Report_Stock) construct() {
	this.Class_WP_List_Table.construct(rt.create_array([
		rt.ArrayItem{ key: 'singular', val: 'stock' },
		rt.ArrayItem{ key: 'plural', val: 'stock' },
		rt.ArrayItem{ key: 'ajax', val: false },
	]))
}

fn (mut this Class_WC_Report_Stock) no_items() {
	rt.call_function('_e', [rt.new_string('No products found.'),
...
fn (mut this Class_WC_Report_Stock) column_default(var_item rt.PhpVal, var_column_name rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_product))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_product, 'get_id', []rt.PhpVal{}), rt.get_property(var_item, 'id'))))) {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('product'))) {
		...
		if rt.is_true(var_sku) {
			print((rt.call_function('esc_html', [var_sku.clone()])).str() + ' - ')
		}
		...
		]))
		{
			print('<div class="description">' +
				(rt.call_function('wp_kses_post', [rt.call_function('wc_get_formatted_variation', [var_product.clone(), rt.new_bool(true)])])).str() +
				'</div>')
		}
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('parent'))) {
		if rt.is_true(rt.get_property(var_item, 'parent')) {
			rt.echo_val(rt.call_function('esc_html', [
				rt.call_function('get_the_title', [rt.get_property(var_item, 'parent')]),
			]))
		} else {
			print('-')
		}
		...
		rt.echo_val(rt.call_function('apply_filters', [
			rt.new_string('woocommerce_admin_stock_html'),
			var_stock_html.clone(),
			var_product.clone(),
		]))
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('stock_level'))) {
		rt.echo_val(rt.call_function('esc_html', [
			rt.call_method(var_product, 'get_stock_quantity', []rt.PhpVal{}),
		]))
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('wc_actions'))) {
		// unsupported statement: Stmt_InlineHTML
		mut var_actions := rt.new_array()
		mut var_action_id := if rt.is_true(rt.call_method(var_product, 'is_type', [
			Class_Automattic_WooCommerce_Enums_ProductType.variation(),
		]))
		{ rt.get_property(var_item, 'parent') } else { rt.get_property(var_item, 'id') }
		var_actions.array_set('edit', rt.create_array([
			rt.ArrayItem{ key: 'url', val: rt.call_function('admin_url', [
				rt.new_string('post.php?post=' + var_action_id.str() + '&action=edit'),
			]) },
		...
		var_actions = rt.call_function('apply_filters', [
			rt.new_string('woocommerce_admin_stock_report_product_actions'),
			var_actions.clone(),
			var_product.clone(),
		])
		...
		for {
			...
			rt.call_function('printf', [
				rt.new_string('<a class="button tips %1$s" href="%2$s" data-tip="%3$s">%4$s</a>'),
				...
				rt.call_function('sprintf', [
					rt.call_function('esc_attr__', [rt.new_string('%s product'),
						rt.new_string('woocommerce')]),
...
fn (mut this Class_WC_Report_Stock) get_columns() rt.PhpVal {
	mut var_columns := {
		'product':      rt.call_function('__', [rt.new_string('Product'),
			rt.new_string('woocommerce')])
		'parent':       rt.call_function('__', [rt.new_string('Parent'),
			rt.new_string('woocommerce')])
		'stock_level':  rt.call_function('__', [rt.new_string('Units in stock'),
			rt.new_string('woocommerce')])
		'stock_status': rt.call_function('__', [rt.new_string('Stock status'),
			rt.new_string('woocommerce')])
