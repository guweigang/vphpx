...
fn (mut this Class_WC_Report_Low_In_Stock) no_items() {
	rt.call_function('esc_html_e', [rt.new_string('No low in stock products found.'),
...
fn (mut this Class_WC_Report_Low_In_Stock) get_items(var_current_page rt.PhpVal, var_per_page rt.PhpVal) {
	...
	mut var_query_from := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_report_low_in_stock_query_from'),
	...
	this.dispatch_set_prop('items', rt.call_method(var_wpdb, 'get_results', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.new_string('SELECT SQL_CALC_FOUND_ROWS posts.ID as id, posts.post_parent as parent ${var_query_from.to_string()} ORDER BY posts.post_title DESC LIMIT %d, %d;'),
...
fn create_wc_report_low_in_stock(_args ...rt.PhpVal) &Class_WC_Report_Low_In_Stock {
