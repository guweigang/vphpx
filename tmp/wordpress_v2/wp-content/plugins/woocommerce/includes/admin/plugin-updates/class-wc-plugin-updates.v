...
fn (mut this Class_WC_Plugin_Updates) get_extensions_inline_warning_major() rt.PhpVal {
	mut var_upgrade_type := rt.new_string('major')
	mut var_plugins := this.major_untested_plugins
	mut var_version_parts := rt.call_function('explode', [rt.new_string('.'), this.new_version])
	mut var_new_version := rt.new_string((var_version_parts.array_get(rt.new_int(0))).str() + '.0')
	if !rt.is_true(var_plugins) {
		return rt.new_null()
	}
	mut var_message := rt.call_function('sprintf', [
		rt.call_function('__', [
			rt.new_string("<strong>Heads up!</strong> The versions of the following plugins you're running haven't been tested with WooCommerce %s. Please update them or confirm compatibility before updating WooCommerce, or you may experience issues:"),
...
fn (mut this Class_WC_Plugin_Updates) get_untested_plugins(var_new_version rt.PhpVal, var_release rt.PhpVal) rt.PhpVal {
	...
	mut var_extensions := rt.call_function('array_merge', [
		this.get_plugins_with_header(rt.new_string(Class_WC_Plugin_Updates.version_tested_header())),
		this.get_plugins_for_woocommerce(),
	])
	mut var_untested := rt.new_array()
	mut var_new_version_parts := rt.call_function('explode', [
		rt.new_string('.'), var_new_version_mutated.clone()])
	mut var_version := var_new_version_parts.array_get(rt.new_int(0))
	if rt.is_true(rt.identical(rt.new_string('minor'), var_release)) {
		var_version = rt.concat(var_version, rt.new_string('.' +
	...
	for {
		...
		if !(!rt.is_true(var_plugin.array_get(rt.new_string(Class_WC_Plugin_Updates.version_tested_header())))) {
			mut var_plugin_version_parts := rt.call_function('explode', [
				rt.new_string('.'),
				var_plugin.array_get(rt.new_string(Class_WC_Plugin_Updates.version_tested_header())),
			])
			if (!(var_plugin_version_parts.array_get(rt.new_int(0)).is_long()
				|| var_plugin_version_parts.array_get(rt.new_int(0)).is_double())
				|| (rt.is_true(rt.identical(rt.new_string('minor'), var_release))
				&& !(var_plugin_version_parts.array_isset(rt.new_int(1)))))
				|| (rt.is_true(rt.identical(rt.new_string('minor'), var_release))
				&& !(var_plugin_version_parts.array_get(rt.new_int(1)).is_long()
				|| var_plugin_version_parts.array_get(rt.new_int(1)).is_double())) {
				continue
			}
			mut var_plugin_version := var_plugin_version_parts.array_get(rt.new_int(0))
			if rt.is_true(rt.identical(rt.new_string('minor'), var_release)) {
				var_plugin_version = rt.concat(var_plugin_version, rt.new_string('.' +
					(var_plugin_version_parts.array_get(rt.new_int(1))).str()))
			}
			if rt.is_true(rt.call_function('version_compare', [
				var_plugin_version.clone(), var_version.clone(),
				rt.new_string('<')]))
			{
				var_untested.array_set(var_file, var_plugin.clone())
			}
		} else {
			var_plugin.array_set(Class_WC_Plugin_Updates.version_tested_header(), rt.call_function('__', [
...
fn (mut this Class_WC_Plugin_Updates) get_plugins_with_header(var_header rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		mut var_file := item_2.key
		if !(!rt.is_true(var_plugin.array_get(var_header))) {
			var_matches.array_set(var_file, var_plugin.clone())
		}
	}
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_get_plugins_with_header'),
		var_matches.clone(),
		var_header.clone(),
		var_plugins.clone(),
	])
...
fn (mut this Class_WC_Plugin_Updates) get_plugins_for_woocommerce() rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('WooCommerce'), var_plugin.array_get(rt.new_string('Name'))))))
			&& rt.is_true(rt.call_function('stristr', [var_plugin.array_get(rt.new_string('Name')), rt.new_string('woocommerce')]))
			|| rt.is_true(rt.call_function('stristr', [var_plugin.array_get(rt.new_string('Description')), rt.new_string('woocommerce')])) {
			var_matches.array_set(var_file, var_plugin.clone())
		}
	}
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_get_plugins_for_woocommerce'),
		var_matches.clone(),
		var_plugins.clone(),
	])
}
...
fn (mut this Class_WC_Plugin_Updates) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'major_untested_plugins' {
			this.major_untested_plugins = val
			return true
		}
		else {
