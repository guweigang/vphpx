...
fn (mut this Class_WC_Admin_Webhooks_Table_List) construct() {
	this.Class_WP_List_Table.construct(rt.create_array([
		rt.ArrayItem{ key: 'singular', val: 'webhook' },
		rt.ArrayItem{ key: 'plural', val: 'webhooks' },
		rt.ArrayItem{ key: 'ajax', val: false },
	]))
}

fn (mut this Class_WC_Admin_Webhooks_Table_List) no_items() {
	rt.call_function('esc_html_e', [rt.new_string('No webhooks found.'),
...
fn (mut this Class_WC_Admin_Webhooks_Table_List) get_columns() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: 'cb', val: '<input type="checkbox" />' },
		...
		rt.ArrayItem{ key: 'topic', val: rt.call_function('__', [
			rt.new_string('Topic'), rt.new_string('woocommerce')]) },
		rt.ArrayItem{ key: 'delivery_url', val: rt.call_function('__', [
			rt.new_string('Delivery URL'), rt.new_string('woocommerce')]) }])
}
...
fn (mut this Class_WC_Admin_Webhooks_Table_List) column_cb(var_webhook rt.PhpVal) rt.PhpVal {
	return rt.call_function('sprintf', [
		...
		rt.get_property(rt.new_object('WC_Admin_Webhooks_Table_List', [
			'WP_List_Table',
...
fn (mut this Class_WC_Admin_Webhooks_Table_List) column_title(var_webhook rt.PhpVal) rt.PhpVal {
	mut var_edit_link := rt.call_function('admin_url', [
		rt.new_string(
			'admin.php?page=wc-settings&amp;tab=advanced&amp;section=webhooks&amp;edit-webhook=' +
			(rt.call_method(var_webhook, 'get_id', []rt.PhpVal{})).str()),
	])
	mut var_output := rt.new_string('')
	mut var_warning_prefix := if rt.is_true(this.uses_legacy_rest_api(var_webhook.clone())) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'legacy_rest_api_is_available', []rt.PhpVal{}))))) { rt.call_function('sprintf', [
			rt.new_string("<span title='%s'>⚠️</span>️ "),
			rt.call_function('esc_html__', [
				rt.new_string('This webhook is configured to be delivered using the Legacy REST API, but the Legacy REST API plugin is not installed on this site.'),
				rt.new_string('woocommerce'),
			]),
		]) } else { rt.new_string('') }
	var_output = rt.concat(var_output, rt.new_string('<strong>' + var_warning_prefix.str() +
		'<a href="' + (rt.call_function('esc_url', [var_edit_link.clone()])).str() +
		'" class="row-title">' +
		(rt.call_function('esc_html', [rt.call_method(var_webhook, 'get_name', []rt.PhpVal{})])).str() +
		'</a></strong>'))
	mut var_actions := rt.create_array([
		...
		rt.ArrayItem{ key: 'edit', val: '<a href="' +
			(rt.call_function('esc_url', [var_edit_link.clone()])).str() + '">' +
			(rt.call_function('esc_html__', [rt.new_string('Edit'), rt.new_string('woocommerce')])).str() +
			'</a>' },
		rt.ArrayItem{ key: 'delete', val: '<a class="submitdelete" aria-label="' +
			(rt.call_function('esc_attr', [rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Delete "%s" permanently'), rt.new_string('woocommerce')]), rt.call_method(var_webhook, 'get_name', []rt.PhpVal{})])])).str() +
			'" href="' +
			(rt.call_function('esc_url', [rt.call_function('wp_nonce_url', [rt.call_function('add_query_arg', [rt.create_array([rt.ArrayItem{
			key: 'delete'
			val: rt.call_method(var_webhook, 'get_id', []rt.PhpVal{})
		}]), rt.call_function('admin_url', [rt.new_string('admin.php?page=wc-settings&tab=advanced&section=webhooks')])]), rt.new_string('delete-webhook')])])).str() +
			'">' +
			(rt.call_function('esc_html__', [rt.new_string('Delete permanently'), rt.new_string('woocommerce')])).str() +
			'</a>' },
	])
	var_actions = rt.call_function('apply_filters', [
		rt.new_string('webhook_row_actions'),
		var_actions.clone(),
		var_webhook.clone(),
	])
	...
	for {
		...
		var_row_actions << '<span class="' +
			(rt.call_function('esc_attr', [var_action.clone()])).str() + '">' + var_link.str() +
			'</span>'
	}
...
fn (mut this Class_WC_Admin_Webhooks_Table_List) get_status_label(var_status_name rt.PhpVal, var_amount rt.PhpVal) rt.PhpVal {
	...
	if var_statuses.array_isset(var_status_name) {
		return rt.create_array([
			rt.ArrayItem{ key: 'singular', val: rt.call_function('sprintf', [
				rt.new_string('%s <span class="count">(%s)</span>'),
				rt.call_function('esc_html', [var_statuses.array_get(var_status_name)]),
				var_amount.clone(),
			]) },
			rt.ArrayItem{ key: 'plural', val: rt.call_function('sprintf', [
				rt.new_string('%s <span class="count">(%s)</span>'),
				rt.call_function('esc_html', [var_statuses.array_get(var_status_name)]),
				var_amount.clone(),
			]) },
			rt.ArrayItem{ key: 'context', val: '' },
			rt.ArrayItem{ key: 'domain', val: 'woocommerce' },
		])
	}
	return rt.create_array([
		rt.ArrayItem{ key: 'singular', val: rt.call_function('sprintf', [
...
fn (mut this Class_WC_Admin_Webhooks_Table_List) get_views() rt.PhpVal {
	...
	mut var_num_webhooks := rt.call_method(var_data_store, 'get_count_webhooks_by_status',
		[]rt.PhpVal{})
	mut var_total_webhooks := rt.call_function('array_sum', [
		rt.cast_array(var_num_webhooks),
	])
	...
	mut var_class := rt.new_string((if
		...
		&& !rt.is_true(rt.get_superglobal('_REQUEST').array_get(rt.new_string('legacy'))) {
		' class="current"'
	} else {
		''
	}).str())
	var_status_links.array_set('all',
		"<a href='admin.php?page=wc-settings&amp;tab=advanced&amp;section=webhooks'${var_class.to_string()}>" +
		(rt.call_function('sprintf', [rt.call_function('_nx', [rt.new_string('All <span class="count">(%s)</span>'), rt.new_string('All <span class="count">(%s)</span>'), var_total_webhooks.clone(), rt.new_string('posts'), rt.new_string('woocommerce')]), rt.call_function('number_format_i18n', [var_total_webhooks.clone()])])).str() +
		'</a>')
	mut iter_2 := var_statuses.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_status_name := item_2.val
		var_class = rt.new_string('')
		if !rt.is_true(var_num_webhooks.array_get(var_status_name)) {
			continue
		}
		if rt.get_superglobal('_REQUEST').array_isset(rt.new_string('status'))
			&& rt.is_true(rt.identical(rt.call_function('sanitize_key', [rt.call_function('wp_unslash', [rt.get_superglobal('_REQUEST').array_get(rt.new_string('status'))])]), var_status_name)) {
			var_class = rt.new_string(' class="current"')
		}
		mut var_label := this.get_status_label(var_status_name.clone(),
			var_num_webhooks.array_get(var_status_name))
		var_status_links.array_set(var_status_name,
			"<a href='admin.php?page=wc-settings&amp;tab=advanced&amp;section=webhooks&amp;status=${var_status_name.to_string()}'${var_class.to_string()}>" +
			(rt.call_function('sprintf', [rt.call_function('translate_nooped_plural', [var_label.clone(), var_num_webhooks.array_get(var_status_name)]), rt.call_function('number_format_i18n', [var_num_webhooks.array_get(var_status_name)])])).str() +
			'</a>')
	}
	mut var_legacy_webhooks_count := rt.call_method(rt.call_method(rt.call_function('wc_get_container',
		[]rt.PhpVal{}), 'get', [
		Class_Automattic_WooCommerce_Internal_Utilities_WebhookUtil.class(),
	]), 'get_legacy_webhooks_count', []rt.PhpVal{})
	if rt.is_true(rt.greater(var_legacy_webhooks_count, rt.new_int(0))) {
		var_class = rt.new_string('')
		if rt.is_true(rt.identical(rt.new_string('true'), rt.call_function('sanitize_key', [
			rt.call_function('wp_unslash', [if !(rt.get_superglobal('_REQUEST').array_get(rt.new_string('legacy'))).is_null() {
				rt.get_superglobal('_REQUEST').array_get(rt.new_string('legacy'))
			} else {
				rt.new_string('')
			}]),
		])))
		...
		mut var_label := this.get_status_label(rt.call_function('__', [
			rt.new_string('Legacy'),
			rt.new_string('woocommerce'),
		]), var_legacy_webhooks_count.clone())
		var_status_links.array_set('legacy',
			"<a href='admin.php?page=wc-settings&amp;tab=advanced&amp;section=webhooks&amp;legacy=true'${var_class.to_string()}>" +
			(rt.call_function('sprintf', [rt.call_function('translate_nooped_plural', [var_label.clone(), var_legacy_webhooks_count.clone()]), rt.call_function('number_format_i18n', [var_legacy_webhooks_count.clone()])])).str() +
			'</a>')
	}
...
fn (mut this Class_WC_Admin_Webhooks_Table_List) process_bulk_action() {
	...
	mut var_webhooks := if rt.get_superglobal('_REQUEST').array_isset(rt.new_string('webhook')) { rt.call_function('array_map', [
			rt.new_string('absint'),
			rt.cast_array(rt.get_superglobal('_REQUEST').array_get(rt.new_string('webhook'))),
		]) } else { []rt.PhpVal{} }
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_action)))) {
		...
		])))))
		{
			rt.call_function('wp_die', [
				rt.call_function('esc_html__', [
					rt.new_string('You do not have permission to edit Webhooks'),
					rt.new_string('woocommerce'),
				]),
			])
		}
		if rt.is_true(rt.identical(rt.new_string('delete'), var_action)) {
			...
			mut iife_result_1 := iife_temp_1.bulk_delete(var_webhooks.clone())
		}
	}
}
...
fn (mut this Class_WC_Admin_Webhooks_Table_List) prepare_items() {
	...
	mut var_args := {
		'limit':  var_per_page
		'offset': var_per_page * var_current_page - 1
	}
	if !(!rt.is_true(rt.get_superglobal('_REQUEST').array_get(rt.new_string('status')))) {
		var_args['status'] = rt.call_function('sanitize_key', [
			rt.call_function('wp_unslash',
				[rt.get_superglobal('_REQUEST').array_get(rt.new_string('status'))]),
		])
	}
	if !(!rt.is_true(rt.get_superglobal('_REQUEST').array_get(rt.new_string('s')))) {
		var_args['search'] = rt.call_function('sanitize_text_field', [
			rt.call_function('wp_unslash',
	...
	{
		var_args['api_version'] = -1
	}
	mut iife_temp_2 := Class_WC_Data_Store{}
	mut iife_result_2 := iife_temp_2.load(rt.new_string('webhook'))
	mut var_data_store := iife_result_2
	mut var_webhooks := rt.call_method(var_data_store, 'search_webhooks', [
		rt.create_array_from_native_map(var_args),
	])
	this.dispatch_set_prop('items', rt.call_function('array_map', [
		rt.new_string('wc_get_webhook'),
		rt.get_property(var_webhooks, 'webhooks'),
	]))
	this.set_pagination_args(rt.create_array([
		rt.ArrayItem{ key: 'total_items', val: rt.get_property(var_webhooks, 'total') },
		rt.ArrayItem{ key: 'per_page', val: var_per_page },
		rt.ArrayItem{ key: 'total_pages', val: rt.get_property(var_webhooks, 'max_num_pages') },
	]))
}
...
fn (mut this Class_WC_Admin_Webhooks_Table_List) get_legacy_api_webhooks_count() i64 {
	return rt.call_function('array_filter', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Admin_Webhooks_Table_List', [
				'WP_List_Table',
			], &this) },
			rt.ArrayItem{ key: none, val: 'uses_legacy_rest_api' },
		]),
	]).array_count()
}
