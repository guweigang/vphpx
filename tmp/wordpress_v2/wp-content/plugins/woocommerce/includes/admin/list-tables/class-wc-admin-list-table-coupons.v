...
fn (mut this Class_WC_Admin_List_Table_Coupons) render_blank_state() {
	...
	print('<h2 class="woocommerce-BlankState-message">' +
		(rt.call_function('esc_html__', [rt.new_string('Coupons are a great way to offer discounts and rewards to your customers. They will appear here once created.'), rt.new_string('woocommerce')])).str() +
		'</h2>')
	print('<a class="woocommerce-BlankState-cta button-primary button" href="' +
		(rt.call_function('esc_url', [rt.call_function('admin_url', [rt.new_string('post-new.php?post_type=shop_coupon')])])).str() +
		'">' +
		(rt.call_function('esc_html__', [rt.new_string('Create your first coupon'), rt.new_string('woocommerce')])).str() +
		'</a>')
	print(
		'<a class="woocommerce-BlankState-cta button" target="_blank" href="https://woocommerce.com/document/coupon-management/?utm_source=blankslate&utm_medium=product&utm_content=couponsdoc&utm_campaign=woocommerceplugin">' +
		(rt.call_function('esc_html__', [rt.new_string('Learn more about coupons'), rt.new_string('woocommerce')])).str() +
		'</a>')
	print('</div>')
}
...
fn (mut this Class_WC_Admin_List_Table_Coupons) define_columns(var_columns rt.PhpVal) rt.PhpVal {
	...
	var_show_columns['cb'] = var_columns.array_get(rt.new_string('cb'))
	var_show_columns['coupon_code'] = rt.call_function('__', [
		rt.new_string('Code'), rt.new_string('woocommerce')])
	var_show_columns['type'] = rt.call_function('__', [rt.new_string('Coupon type'),
		rt.new_string('woocommerce')])
	var_show_columns['amount'] = rt.call_function('__', [rt.new_string('Coupon amount'),
		rt.new_string('woocommerce')])
	var_show_columns['description'] = rt.call_function('__', [
		rt.new_string('Description'),
		rt.new_string('woocommerce'),
	])
	var_show_columns['products'] = rt.call_function('__', [rt.new_string('Product IDs'),
		rt.new_string('woocommerce')])
	var_show_columns['usage'] = rt.call_function('__', [rt.new_string('Usage / Limit'),
		rt.new_string('woocommerce')])
	var_show_columns['expiry_date'] = rt.call_function('__', [
		rt.new_string('Expiry date'),
		rt.new_string('woocommerce'),
	])
...
fn (mut this Class_WC_Admin_List_Table_Coupons) prepare_row_data(var_post_id rt.PhpVal) {
	mut var_the_coupon := rt.get_superglobal('the_coupon')
	if !rt.is_true(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', ['WC_Admin_List_Table'], &this), 'object'))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', ['WC_Admin_List_Table'], &this), 'object'), 'get_id', []rt.PhpVal{}), var_post_id)))) {
		this.dispatch_set_prop('object', create_wc_coupon(var_post_id.clone()))
		var_the_coupon = rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
			'WC_Admin_List_Table',
		], &this), 'object')
	}
}
...
fn (mut this Class_WC_Admin_List_Table_Coupons) render_coupon_code_column() {
	...
	mut var_title := rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
		'WC_Admin_List_Table',
	], &this), 'object'), 'get_code', []rt.PhpVal{})
...
fn (mut this Class_WC_Admin_List_Table_Coupons) render_type_column() {
	rt.echo_val(rt.call_function('esc_html', [
		rt.call_function('wc_get_coupon_type', [
			rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
				'WC_Admin_List_Table',
			], &this), 'object'), 'get_discount_type', []rt.PhpVal{}),
		]),
	]))
}
...
fn (mut this Class_WC_Admin_List_Table_Coupons) render_amount_column() {
	rt.echo_val(rt.call_function('esc_html', [
		rt.call_function('wc_format_localized_price', [
			rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
				'WC_Admin_List_Table',
			], &this), 'object'), 'get_amount', []rt.PhpVal{}),
		]),
	]))
}

fn (mut this Class_WC_Admin_List_Table_Coupons) render_products_column() {
	mut var_product_ids := rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
		'WC_Admin_List_Table',
	], &this), 'object'), 'get_product_ids', []rt.PhpVal{})
	if var_product_ids.clone().is_array() && var_product_ids.clone().array_count() > 0 {
		rt.echo_val(rt.call_function('esc_html', [
...
fn (mut this Class_WC_Admin_List_Table_Coupons) render_usage_limit_column() {
	mut var_usage_limit := rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
		'WC_Admin_List_Table',
	], &this), 'object'), 'get_usage_limit', []rt.PhpVal{})
	if rt.is_true(var_usage_limit) {
		rt.echo_val(rt.call_function('esc_html', [var_usage_limit.clone()]))
	} else {
		print('&ndash;')
	}
}
...
fn (mut this Class_WC_Admin_List_Table_Coupons) render_usage_column() {
	mut var_usage_count := rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
		'WC_Admin_List_Table',
	], &this), 'object'), 'get_usage_count', []rt.PhpVal{})
	mut var_usage_limit := rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
		'WC_Admin_List_Table',
	], &this), 'object'), 'get_usage_limit', []rt.PhpVal{})
	rt.call_function('printf', [
		rt.call_function('__', [rt.new_string('%1$s / %2$s'),
			rt.new_string('woocommerce')]),
		rt.call_function('esc_html', [var_usage_count.clone()]),
		if rt.is_true(var_usage_limit) { rt.call_function('esc_html', [
				var_usage_limit.clone()]) } else { rt.new_string('&infin;') },
	])
}

fn (mut this Class_WC_Admin_List_Table_Coupons) render_expiry_date_column() {
	mut var_expiry_date := rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
		'WC_Admin_List_Table',
	], &this), 'object'), 'get_date_expires', []rt.PhpVal{})
	if rt.is_true(var_expiry_date) {
		rt.echo_val(rt.call_function('esc_html', [
			rt.call_method(var_expiry_date, 'date_i18n', [rt.new_string('F j, Y')]),
		]))
	} else {
		print('&ndash;')
	}
...
fn (mut this Class_WC_Admin_List_Table_Coupons) render_description_column() {
	rt.echo_val(rt.call_function('wp_kses_post', [if rt.is_true(rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
		'WC_Admin_List_Table',
	], &this), 'object'), 'get_description', []rt.PhpVal{}))
	{ rt.call_method(rt.get_property(rt.new_object('WC_Admin_List_Table_Coupons', [
			'WC_Admin_List_Table',
		], &this), 'object'), 'get_description', []rt.PhpVal{}) } else { rt.new_string('&ndash;') }]))
}

fn (mut this Class_WC_Admin_List_Table_Coupons) render_filters() {
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Show all types'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	...
	for {
		...
		print('<option value="' + (rt.call_function('esc_attr', [var_name.clone()])).str() + '"')
		if rt.get_superglobal('_GET').array_isset(rt.new_string('coupon_type')) {
			rt.call_function('selected', [var_name.clone(),
				rt.call_function('wc_clean', [
					rt.call_function('wp_unslash', [
						rt.get_superglobal('_GET').array_get(rt.new_string('coupon_type')),
					]),
				])])
		}
		print('>' + (rt.call_function('esc_html', [var_type.clone()])).str() + '</option>')
	}
	// unsupported statement: Stmt_InlineHTML
}
...
fn (mut this Class_WC_Admin_List_Table_Coupons) query_filters(var_query_vars rt.PhpVal) rt.PhpVal {
	...
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('coupon_type')))) {
		var_query_vars_mutated.array_set('meta_key', 'discount_type')
		var_query_vars_mutated.array_set('meta_value', rt.call_function('wc_clean', [
			rt.call_function('wp_unslash',
				[rt.get_superglobal('_GET').array_get(rt.new_string('coupon_type'))]),
		]))
	}
...
fn main() {
	...
	if rt.is_true(rt.call_function('class_exists', [
		rt.new_string('WC_Admin_List_Table_Coupons'),
		rt.new_bool(false),
	]))
	{
		return rt.new_null()
