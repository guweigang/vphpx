...
fn main() {
	...
	rt.call_function('printf', [
		rt.new_string(' ' +(rt.call_function('esc_html__', [rt.new_string('The database update process runs in the background and may take a little while, so please be patient. Advanced users can alternatively update via %1$sWP CLI%2$s.'), rt.new_string('woocommerce')])).str()),
		rt.new_string('<a href="https://developer.woocommerce.com/docs/wc-cli/wc-cli-examples/#upgrading-the-database-using-wp-cli">'),
		rt.new_string('</a>'),
	])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_url', [var_update_url.clone()]))
	// unsupported statement: Stmt_InlineHTML
