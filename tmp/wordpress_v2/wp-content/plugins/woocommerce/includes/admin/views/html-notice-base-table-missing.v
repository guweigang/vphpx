...
fn main() {
	...
	rt.echo_val(rt.call_function('esc_url', [
		rt.call_function('wp_nonce_url', [
			rt.call_function('add_query_arg', [rt.new_string('wc-hide-notice'),
				rt.new_string('base_tables_missing')]),
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Database tables missing'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	mut iife_temp_0 := Class_WC_Admin_Status{}
	mut iife_result_0 := iife_temp_0.get_tools()
	mut var_verify_db_tool_available := iife_result_0.array_isset(rt.new_string('verify_db_tables'))
	mut var_missing_tables := rt.call_function('get_option', [
		rt.new_string('woocommerce_schema_missing_tables'),
	])
	if var_verify_db_tool_available {
		rt.echo_val(rt.call_function('wp_kses_post', [
			rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('One or more tables required for WooCommerce to function are missing, some features may not work as expected. Missing tables: %1$s. <a href="%2$s">Check again.</a>'),
					rt.new_string('woocommerce'),
				]),
				...
				rt.call_function('wp_nonce_url', [
					rt.call_function('admin_url', [
						rt.new_string('admin.php?page=wc-status&tab=tools&action=verify_db_tables'),
					]),
		...
		rt.echo_val(rt.call_function('wp_kses_post', [
			rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('One or more tables required for WooCommerce to function are missing, some features may not work as expected. Missing tables: %1$s.'),
					rt.new_string('woocommerce'),
				]),
				rt.call_function('esc_html', [
