...
fn main() {
	...
	rt.call_function('esc_html_e', [
		rt.new_string('a group of regions that can be assigned different shipping methods and rates.'),
		rt.new_string('woocommerce'),
	])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [
		rt.new_string('Legacy shipping methods (flat rate, international flat rate, local pickup and delivery, and free shipping) are deprecated but will continue to work as normal for now. <b><em>They will be removed in future versions of WooCommerce</em></b>. We recommend disabling these and setting up new rates within shipping zones as soon as possible.'),
		rt.new_string('woocommerce'),
	])
	// unsupported statement: Stmt_InlineHTML
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_wc_admin_settings_page', []rt.PhpVal{})))))
		...
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('shipping'), rt.get_superglobal('_GET').array_get(rt.new_string('tab')))))) {
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_url', [
