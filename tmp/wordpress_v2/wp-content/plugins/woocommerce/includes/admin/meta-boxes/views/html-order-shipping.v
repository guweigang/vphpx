...
fn main() {
	...
	rt.echo_val(rt.call_function('esc_attr', [var_item_id.clone()]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_attr_e', [rt.new_string('Shipping name'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_item_id.clone()]))
	// unsupported statement: Stmt_InlineHTML
	...
	rt.echo_val(rt.call_function('esc_attr', [var_item_id.clone()]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_attr_e', [rt.new_string('Shipping method'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	...
	for {
		...
		mut var_is_active := (rt.identical(rt.call_method(var_item, 'get_method_id', []rt.PhpVal{}),
			rt.get_property(var_method, 'id'))).to_bool()
		print('<option value="' +
			(rt.call_function('esc_attr', [rt.get_property(var_method, 'id')])).str() + '" ' +
			(rt.call_function('selected', [rt.new_bool(true), rt.new_bool(var_is_active).clone(), rt.new_bool(false)])).str() +
			'>' +
	...
	if !var_found_method && rt.is_true(rt.call_method(var_item, 'get_method_id', []rt.PhpVal{})) {
		print('<option value="' +
			(rt.call_function('esc_attr', [rt.call_method(var_item, 'get_method_id', []rt.PhpVal{})])).str() +
			'" selected="selected">' +
	...
	rt.call_function('do_action', [rt.new_string('woocommerce_before_order_itemmeta'),
		var_item_id.clone(), var_item.clone(), rt.new_null()])
	// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(var_refunded) {
		rt.echo_val(rt.call_function('wp_kses_post', [
			rt.new_string('<small class="refunded">' +
	...
	if rt.is_true(var_tax_data) && rt.is_true(rt.call_function('wc_tax_enabled', []rt.PhpVal{})) {
		mut iter_2 := var_order_taxes.iterator()
		for {
			...
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('wp_kses_post', [if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_tax_item_total)))) { rt.call_function('wc_price', [
	...
	if rt.is_true(rt.call_method(var_order, 'is_editable', []rt.PhpVal{})) {
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_attr_e', [rt.new_string('Edit shipping'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_attr_e', [rt.new_string('Edit shipping'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_attr_e', [rt.new_string('Delete shipping'),
			rt.new_string('woocommerce')])
		// unsupported statement: Stmt_InlineHTML
		rt.call_function('esc_attr_e', [rt.new_string('Delete shipping'),
			rt.new_string('woocommerce')])
