...
fn main() {
	...
	rt.echo_val(rt.call_function('esc_attr', [var_loop.clone()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(if !(rt.call_method(var_download, 'get_access_expires', []rt.PhpVal{}).is_null()) { rt.call_function('esc_attr', [
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_url', [var_download_link.clone()]))
	// unsupported statement: Stmt_InlineHTML
	...
	print('<a class="button" href="' +
		(rt.call_function('esc_url', [var_report_url.clone()])).str() + '">')
