...
fn (mut this Class_WC_Email_Admin_Payment_Gateway_Enabled) construct() {
	...
	this.dispatch_set_prop('title', rt.call_function('__', [
		rt.new_string('Payment gateway enabled'),
		rt.new_string('woocommerce'),
	]))
	this.dispatch_set_prop('email_group', rt.new_string('payments'))
	this.dispatch_set_prop('template_html',
		rt.new_string('emails/admin-payment-gateway-enabled.php'))
	this.dispatch_set_prop('template_plain',
		rt.new_string('emails/plain/admin-payment-gateway-enabled.php'))
	this.dispatch_set_prop('placeholders', rt.create_array([
		rt.ArrayItem{ key: '{gateway_title}', val: '' },
		rt.ArrayItem{ key: '{site_title}', val: '' },
	]))
	rt.call_function('add_action', [
		rt.new_string('woocommerce_payment_gateway_enabled_notification'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Email_Admin_Payment_Gateway_Enabled', [
				'WC_Email',
			], &this) },
	...
	rt.call_function('add_action', [
		...
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'block_content' },
		]),
		rt.new_int(10),
		rt.new_int(3),
	])
	rt.call_function('add_filter', [
		...
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: 'exclude_from_order_details' },
		]),
	])
	this.Class_WC_Email.construct()
	this.dispatch_set_prop('description', rt.call_function('__', [
		rt.new_string('Payment gateway enabled emails are sent to chosen recipient(s) when a payment gateway is enabled.'),
		rt.new_string('woocommerce'),
	]))
	if rt.is_true(rt.get_property(rt.new_object('WC_Email_Admin_Payment_Gateway_Enabled', [
		'WC_Email',
	], &this), 'block_email_editor_enabled'))
	{
		this.dispatch_set_prop('description', rt.call_function('__', [
			rt.new_string('Notifies admins when a payment gateway has been enabled.'),
			rt.new_string('woocommerce'),
		]))
	}
...
fn (mut this Class_WC_Email_Admin_Payment_Gateway_Enabled) get_default_subject() rt.PhpVal {
	return rt.call_function('__', [
		rt.new_string('[{site_title}] Payment gateway "{gateway_title}" enabled'),
		rt.new_string('woocommerce'),
	])
}
...
fn (mut this Class_WC_Email_Admin_Payment_Gateway_Enabled) get_default_heading() rt.PhpVal {
	return rt.call_function('__', [
		rt.new_string('Payment gateway "{gateway_title}" enabled'),
		rt.new_string('woocommerce'),
	])
}
...
fn (mut this Class_WC_Email_Admin_Payment_Gateway_Enabled) trigger(var_gateway rt.PhpVal) {
	...
	if rt.is_true(rt.call_function('is_a', [var_gateway.clone(),
		rt.new_string('WC_Payment_Gateway')]))
	{
		this.dispatch_set_prop('object', var_gateway.clone())
		this.gateway_title = rt.call_method(var_gateway, 'get_method_title', []rt.PhpVal{})
		this.gateway_settings_url = rt.call_function('esc_url_raw', [
			rt.call_function('self_admin_url', [
				rt.new_string('admin.php?page=wc-settings&tab=checkout&section=' +
					(rt.get_property(var_gateway, 'id')).str()),
			]),
		])
		this.gateway_settings_url = rt.call_function('apply_filters', [
			rt.new_string('woocommerce_payment_gateway_enabled_notification_settings_url'),
			this.gateway_settings_url,
			var_gateway.clone(),
		])
		...
		this.username = if rt.is_true(var_user) {
			rt.get_property(var_user, 'user_login')
		} else {
			this.admin_email
		}
		rt.get_property(rt.new_object('WC_Email_Admin_Payment_Gateway_Enabled', [
			'WC_Email',
		], &this), 'placeholders').array_set('{gateway_title}', this.gateway_title)
		rt.get_property(rt.new_object('WC_Email_Admin_Payment_Gateway_Enabled', [
			'WC_Email',
		], &this), 'placeholders').array_set('{site_title}', this.get_blogname())
	}
...
fn (mut this Class_WC_Email_Admin_Payment_Gateway_Enabled) get_recipient() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.instance_of(rt.get_property(rt.new_object('WC_Email_Admin_Payment_Gateway_Enabled', [
		'WC_Email',
	], &this), 'object'), 'WC_Payment_Gateway')))
	{
		mut var_extra_addresses := rt.call_function('apply_filters', [
			rt.new_string('wc_payment_gateway_enabled_notification_email_addresses'),
			rt.new_array(),
			rt.get_property(rt.new_object('WC_Email_Admin_Payment_Gateway_Enabled', [
				'WC_Email',
			], &this), 'object'),
		])
		if !(!rt.is_true(var_extra_addresses)) && var_extra_addresses.clone().is_array() {
			closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
				mut var_email_address := if args.len > 0 { args[0].clone() } else { rt.new_null() }
				return rt.new_bool((rt.call_function('filter_var', [
					var_email_address.clone(), rt.get_constant('FILTER_VALIDATE_EMAIL')])).to_bool())
			}
			mut var_extra_valid := rt.call_function('array_filter', [
				var_extra_addresses.clone(), rt.new_closure(closure_1_fn)])
			if !(!rt.is_true(var_extra_valid)) {
				mut var_existing := rt.call_function('array_map', [
					rt.new_string('trim'),
					rt.call_function('explode', [
						rt.new_string(','),
						var_recipient.clone(),
					])])
				mut var_merged := rt.call_function('array_unique', [
					rt.call_function('array_merge', [var_existing.clone(),
						var_extra_valid.clone()]),
				])
				var_recipient = rt.call_function('implode', [
					rt.new_string(', '), rt.call_function('array_filter', [
						var_merged.clone()])])
			}
		}
	}
	return var_recipient.clone()
}
...
fn (mut this Class_WC_Email_Admin_Payment_Gateway_Enabled) get_content_html() rt.PhpVal {
	return rt.call_function('wc_get_template_html', [
		...
		rt.create_array([
			...
			rt.ArrayItem{ key: 'gateway_title', val: this.gateway_title },
			rt.ArrayItem{ key: 'gateway_settings_url', val: this.gateway_settings_url },
			rt.ArrayItem{ key: 'username', val: this.username },
			rt.ArrayItem{ key: 'admin_email', val: this.admin_email },
			...
			rt.ArrayItem{ key: 'email', val: rt.new_object('WC_Email_Admin_Payment_Gateway_Enabled', [
				'WC_Email',
			], &this) },
		]),
	])
...
fn (mut this Class_WC_Email_Admin_Payment_Gateway_Enabled) block_content(var_sent_to_admin rt.PhpVal, var_plain_text rt.PhpVal, var_email rt.PhpVal) {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(rt.new_object('WC_Email_Admin_Payment_Gateway_Enabled', [
		'WC_Email',
	], &this), 'id'), rt.get_property(var_email, 'id')))))
	{
		return
	}
	mut var_gateway_title := if !(!rt.is_true(this.gateway_title)) { this.gateway_title } else { rt.call_function('__', [
			rt.new_string('Dummy Gateway'),
			rt.new_string('woocommerce'),
		]) }
	mut var_gateway_settings_url := if !(!rt.is_true(this.gateway_settings_url)) { this.gateway_settings_url } else { rt.call_function('__', [
			rt.new_string('Dummy Settings URL'),
			rt.new_string('woocommerce'),
		]) }
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('printf', [
		rt.call_function('esc_html__', [
			rt.new_string('The payment gateway "%s" has been enabled.'),
			rt.new_string('woocommerce'),
		]),
	...
	rt.call_function('esc_html_e', [
		rt.new_string('If you did not enable this payment gateway, please log in to your site and consider disabling it here:'),
		rt.new_string('woocommerce'),
	])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_url', [var_gateway_settings_url.clone()]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_url', [var_gateway_settings_url.clone()]))
	// unsupported statement: Stmt_InlineHTML
}
...
fn (mut this Class_WC_Email_Admin_Payment_Gateway_Enabled) exclude_from_order_details(var_emails_without_order_details rt.PhpVal) rt.PhpVal {
	...
	return var_emails_without_order_details_mutated.clone()
}
...
fn (mut this Class_WC_Email_Admin_Payment_Gateway_Enabled) init_form_fields() {
	mut var_placeholder_text := rt.call_function('sprintf', [
		rt.call_function('__', [rt.new_string('Available placeholders: %s'),
			rt.new_string('woocommerce')]),
		rt.new_string('<code>' +
			(rt.call_function('esc_html', [rt.call_function('implode', [rt.new_string('</code>, <code>'), rt.func_array_keys(rt.get_property(rt.new_object('WC_Email_Admin_Payment_Gateway_Enabled', ['WC_Email'], &this), 'placeholders'))])])).str() +
			'</code>'),
	])
...
fn (mut this Class_WC_Email_Admin_Payment_Gateway_Enabled) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'gateway_title' {
			this.gateway_title = val
			return true
		}
		'gateway_settings_url' {
			this.gateway_settings_url = val
			return true
		}
		'username' {
			this.username = val
...
fn main() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('WC_Email_Admin_Payment_Gateway_Enabled'),
		rt.new_bool(false),
	])))))
	{
	}
	return rt.new_object('WC_Email_Admin_Payment_Gateway_Enabled', ['WC_Email'],
		create_wc_email_admin_payment_gateway_enabled())
}
