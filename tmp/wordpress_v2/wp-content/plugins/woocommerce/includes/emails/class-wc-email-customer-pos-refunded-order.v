...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) construct() {
	...
	this.dispatch_set_prop('title', rt.call_function('__', [
		rt.new_string('POS refunded order'),
	...
	this.dispatch_set_prop('template_plain',
		rt.new_string('emails/plain/customer-pos-refunded-order.php'))
	...
	this.dispatch_set_prop('description', if rt.is_true(rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
		'WC_Email',
	], &this), 'email_improvements_enabled'))
	{ rt.call_function('__', [
			rt.new_string('Let customers know when a full or partial refund is on its way to them for their POS order.'),
			rt.new_string('woocommerce'),
		]) } else { rt.call_function('__', [
			rt.new_string('Order refunded emails are sent to customers when their POS orders are refunded.'),
			rt.new_string('woocommerce'),
		]) })
	this.dispatch_set_prop('manual', rt.new_bool(true))
	if rt.is_true(rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
		'WC_Email',
	], &this), 'block_email_editor_enabled'))
	{
		this.dispatch_set_prop('title', rt.call_function('__', [
			rt.new_string('POS order refunded'),
			rt.new_string('woocommerce'),
		]))
		this.dispatch_set_prop('description', rt.call_function('__', [
			rt.new_string('Notifies customers when a full or partial refund has been issued for their in-person (POS) order.'),
			rt.new_string('woocommerce'),
		]))
	}
}
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) get_default_subject(partial bool) rt.PhpVal {
	...
	if var_partial {
		return rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('Your %1$s order #%2$s has been partially refunded'),
				rt.new_string('woocommerce'),
			]),
		...
		return rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('Your %1$s order #%2$s has been refunded'),
				rt.new_string('woocommerce'),
			]),
			rt.call_function('esc_html', [
				var_store_name.clone(),
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) get_default_heading(partial bool) rt.PhpVal {
	if var_partial {
		return if rt.is_true(rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
			'WC_Email',
		], &this), 'email_improvements_enabled'))
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) get_subject() rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		...
		rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
			'WC_Email',
		], &this), 'object'),
		rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
			'WC_Email',
		], &this),
	])
}
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) get_heading() rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		...
		this.format_string(var_heading.clone()),
		rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
			'WC_Email',
		], &this), 'object'),
		rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) auto_trigger(var_order_id rt.PhpVal, var_refund_id rt.PhpVal) {
	...
	mut iife_result_0 := iife_temp_0.is_order_paid_at_pos(var_order.clone())
	...
	this.trigger(var_order_id.clone(), rt.new_string((rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
		'WC_Email',
	], &this), 'id')).str()), var_refund_id.clone())
}
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) trigger(var_order_id rt.PhpVal, var_template_id rt.PhpVal, var_refund_id rt.PhpVal) {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
		'WC_Email',
	...
	this.setup_locale()
	this.partial_refund = rt.greater(rt.call_method(var_order, 'get_remaining_refund_amount',
	...
	this.dispatch_set_prop('recipient', rt.call_method(var_order, 'get_billing_email',
		[]rt.PhpVal{}))
	rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', ['WC_Email'], &this),
		'placeholders').array_set('{order_date}', rt.call_function('wc_format_datetime', [
		rt.call_method(var_order, 'get_date_created', []rt.PhpVal{}),
	]))
	rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', ['WC_Email'], &this),
		'placeholders').array_set('{order_number}', rt.call_method(var_order, 'get_order_number',
		[]rt.PhpVal{}))
	if !(!rt.is_true(var_refund_id)) {
		mut var_refund := rt.call_function('wc_get_order', [var_refund_id.clone()])
		this.refund = if rt.is_true(rt.new_bool(rt.instance_of(var_refund, 'WC_Order_Refund'))) {
			var_refund
		} else {
			rt.new_bool(false)
		}
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) get_content_html() rt.PhpVal {
	...
	rt.call_function('add_action', [rt.new_string('woocommerce_pos_email_header'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
				'WC_Email',
			], &this) },
	...
	mut var_content := rt.call_function('wc_get_template_html', [
		...
		rt.create_array([
			rt.ArrayItem{ key: 'order', val: rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
				'WC_Email',
			], &this), 'object') },
			rt.ArrayItem{ key: 'refund', val: this.refund },
			...
			rt.ArrayItem{ key: 'email', val: rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
				'WC_Email',
			], &this) },
		]),
	])
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) init_form_fields() {
	mut var_placeholder_text := rt.call_function('sprintf', [
		rt.call_function('__', [rt.new_string('Available placeholders: %s'),
			rt.new_string('woocommerce')]),
		rt.new_string('<code>' +
			(rt.call_function('esc_html', [rt.call_function('implode', [rt.new_string('</code>, <code>'), rt.func_array_keys(rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', ['WC_Email'], &this), 'placeholders'))])])).str() +
			'</code>'),
	])
	...
	], &this), 'block_email_editor_enabled'))
	{
		rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
			'WC_Email',
		], &this), 'form_fields').array_set('preheader', this.get_preheader_field())
	}
}
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) add_to_preferred_template_ids(var_preferred_template_ids rt.PhpVal, var_order rt.PhpVal) rt.PhpVal {
	mut var_order_mutated := var_order
	if !(this.is_preferred_for_order(var_order_mutated.clone())) {
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) is_available_for_order(var_order rt.PhpVal) bool {
	...
	return rt.is_true(iife_result_3)
		&& rt.is_true(rt.identical(Class_Automattic_WooCommerce_Enums_OrderStatus.refunded(), rt.call_method(var_order_mutated, 'get_status', [rt.new_string('edit')])))
		|| rt.is_true(rt.new_bool(0 != rt.call_method(var_order_mutated, 'get_refunds', []rt.PhpVal{}).array_count()))
}

fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) is_preferred_for_order(var_order rt.PhpVal) bool {
	mut var_order_mutated := var_order
	mut iife_temp_4 := Class_Automattic_WooCommerce_Internal_Orders_PointOfSaleOrderUtil{}
	mut iife_result_4 := iife_temp_4.is_order_paid_at_pos(var_order_mutated.clone())
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) get_pos_store_name() rt.PhpVal {
	...
	mut iife_result_5 := iife_temp_5.get_default_store_name()
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) get_pos_store_address() rt.PhpVal {
	mut iife_temp_7 := Class_Automattic_WooCommerce_Internal_Settings_PointOfSaleDefaultSettings{}
	mut iife_result_7 := iife_temp_7.get_default_store_address()
	return this.format_string(rt.call_function('get_option', [
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) replace_footer_placeholders(var_footer_text rt.PhpVal, var_email rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(var_email, 'id'), rt.get_property(rt.new_object('WC_Email_Customer_POS_Refunded_Order', [
		'WC_Email',
	], &this), 'id')))))
	{
...
fn (mut this Class_WC_Email_Customer_POS_Refunded_Order) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'partial_refund' {
			this.partial_refund = val.to_bool()
			return true
		}
		else {
...
fn main() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('WC_Email_Customer_POS_Refunded_Order'),
		rt.new_bool(false),
	])))))
	{
	}
	return rt.new_object('WC_Email_Customer_POS_Refunded_Order', ['WC_Email'],
		create_wc_email_customer_pos_refunded_order())
}
