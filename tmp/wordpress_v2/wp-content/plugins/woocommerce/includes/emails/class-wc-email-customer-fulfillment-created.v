...
fn (mut this Class_WC_Email_Customer_Fulfillment_Created) construct() {
	...
	this.dispatch_set_prop('title', rt.call_function('__', [
		rt.new_string('Fulfillment created'),
		rt.new_string('woocommerce'),
	]))
	this.dispatch_set_prop('email_group', rt.new_string('order-updates'))
	this.dispatch_set_prop('template_html',
		rt.new_string('emails/customer-fulfillment-created.php'))
	this.dispatch_set_prop('template_plain',
		rt.new_string('emails/plain/customer-fulfillment-created.php'))
	...
	rt.call_function('add_action', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Email_Customer_Fulfillment_Created', [
				'WC_Email',
			], &this) },
	...
	this.dispatch_set_prop('description', rt.call_function('__', [
		rt.new_string('Fulfillment created emails are sent to the customer when the merchant creates a fulfillment for the order, and marks it as fulfilled. The notification isn’t sent for draft fulfillments.'),
		rt.new_string('woocommerce'),
	]))
...
fn (mut this Class_WC_Email_Customer_Fulfillment_Created) trigger(var_order_id rt.PhpVal, var_fulfillment rt.PhpVal, order bool) {
		...
		rt.new_string('WC_Order')]))
	{
		...
		this.dispatch_set_prop('recipient', rt.call_method(rt.get_property(rt.new_object('WC_Email_Customer_Fulfillment_Created', [
			'WC_Email',
		], &this), 'object'), 'get_billing_email', []rt.PhpVal{}))
		rt.get_property(rt.new_object('WC_Email_Customer_Fulfillment_Created', [
			'WC_Email',
		], &this), 'placeholders').array_set('{order_date}', rt.call_function('wc_format_datetime', [
			rt.call_method(rt.get_property(rt.new_object('WC_Email_Customer_Fulfillment_Created', [
				'WC_Email',
			], &this), 'object'), 'get_date_created', []rt.PhpVal{}),
		]))
		rt.get_property(rt.new_object('WC_Email_Customer_Fulfillment_Created', [
			'WC_Email',
		], &this), 'placeholders').array_set('{order_number}', rt.call_method(rt.get_property(rt.new_object('WC_Email_Customer_Fulfillment_Created', [
			'WC_Email',
...
fn (mut this Class_WC_Email_Customer_Fulfillment_Created) get_fulfillment_item_count() i64 {
	...
	return (rt.call_function('array_reduce', [
		rt.call_method(this.fulfillment, 'get_items', []rt.PhpVal{}),
...
fn (mut this Class_WC_Email_Customer_Fulfillment_Created) get_default_subject() rt.PhpVal {
	if this.get_fulfillment_item_count() > 1 {
		return rt.call_function('__', [
			rt.new_string('Items from {site_title} order {order_number} have been fulfilled!'),
			rt.new_string('woocommerce'),
		])
	}
	return rt.call_function('__', [
		rt.new_string('An item from {site_title} order {order_number} has been fulfilled!'),
		rt.new_string('woocommerce'),
	])
}
...
fn (mut this Class_WC_Email_Customer_Fulfillment_Created) get_default_heading() rt.PhpVal {
	if this.get_fulfillment_item_count() > 1 {
		return rt.call_function('__', [rt.new_string('Your items are on the way!'),
			rt.new_string('woocommerce')])
	}
	return rt.call_function('__', [rt.new_string('Your item is on the way!'),
		rt.new_string('woocommerce')])
}

fn (mut this Class_WC_Email_Customer_Fulfillment_Created) get_content_html() rt.PhpVal {
	this.maybe_init_fulfillment_for_preview(rt.get_property(rt.new_object('WC_Email_Customer_Fulfillment_Created', [
		'WC_Email',
	], &this), 'object'))
	return rt.call_function('wc_get_template_html', [
		...
		rt.create_array([
			rt.ArrayItem{ key: 'order', val: rt.get_property(rt.new_object('WC_Email_Customer_Fulfillment_Created', [
				'WC_Email',
			], &this), 'object') },
			...
			rt.ArrayItem{ key: 'email', val: rt.new_object('WC_Email_Customer_Fulfillment_Created', [
				'WC_Email',
			], &this) },
		]),
	])
...
fn (mut this Class_WC_Email_Customer_Fulfillment_Created) get_default_additional_content() rt.PhpVal {
	return rt.call_function('__', [
		rt.new_string('Please note that couriers may need some time to provide the latest shipping information.'),
		rt.new_string('woocommerce'),
	])
}
...
fn main() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('WC_Email_Customer_Fulfillment_Created'),
		rt.new_bool(false),
	])))))
	{
	}
	return rt.new_object('WC_Email_Customer_Fulfillment_Created', ['WC_Email'],
		create_wc_email_customer_fulfillment_created())
}
