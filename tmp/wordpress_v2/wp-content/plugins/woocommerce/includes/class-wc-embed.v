...
fn Class_WC_Embed.is_embedded_product() bool {
	if rt.is_true(rt.call_function('function_exists', [rt.new_string('is_embed')]))
...
fn Class_WC_Embed.the_excerpt(var_excerpt rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(Class_WC_Embed.is_embedded_product()) {
		...
		var_excerpt_mutated = rt.concat(var_excerpt_mutated, Class_WC_Embed.product_buttons())
	}
	return var_excerpt_mutated.clone()
}
...
fn Class_WC_Embed.product_buttons() string {
	...
	mut var_button := rt.new_string('<a href="%s" class="wp-embed-more wc-embed-button">%s</a>')
	if rt.is_true(rt.call_method(var__product, 'is_type', [Class_Automattic_WooCommerce_Enums_ProductType.simple()]))
		...
		var_buttons << rt.call_function('sprintf', [var_button.clone(),
	...
	var_buttons << rt.call_function('sprintf', [var_button.clone(),
		...
		rt.call_function('esc_html__', [rt.new_string('Read more'),
			rt.new_string('woocommerce')])])
	return '<p>' +
		(rt.call_function('implode', [rt.new_string(' '), rt.create_array_from_list(var_buttons)])).str() +
		'</p>'
}
...
fn create_wc_embed(_args ...rt.PhpVal) &Class_WC_Embed {
