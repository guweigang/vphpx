...
fn (mut this Class_WC_Order_Item_Product) set_quantity(var_value rt.PhpVal) {
	...
	this.set_prop(rt.new_string('quantity'), rt.call_function('wc_stock_amount', [
...
fn (mut this Class_WC_Order_Item_Product) set_tax_class(var_value rt.PhpVal) {
	...
	if rt.is_true(var_value_mutated)
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_value_mutated.clone(), iife_result_0, rt.new_bool(true)]))))) {
		this.error(rt.new_string('order_item_product_invalid_tax_class'), rt.call_function('__', [
			rt.new_string('Invalid tax class'),
			rt.new_string('woocommerce'),
		]))
	}
	this.set_prop(rt.new_string('tax_class'), var_value_mutated.clone())
}

fn (mut this Class_WC_Order_Item_Product) set_product_id(var_value rt.PhpVal) {
	mut var_value_mutated := var_value
	if rt.is_true(rt.greater(var_value_mutated, rt.new_int(0)))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('product'), rt.call_function('get_post_type', [rt.call_function('absint', [var_value_mutated.clone()])]))))) {
		this.error(rt.new_string('order_item_product_invalid_product_id'), rt.call_function('__', [
			rt.new_string('Invalid product ID'),
			rt.new_string('woocommerce'),
		]))
	}
	this.set_prop(rt.new_string('product_id'), rt.call_function('absint', [
		var_value_mutated.clone()]))
}
...
fn (mut this Class_WC_Order_Item_Product) set_variation_id(var_value rt.PhpVal) {
	mut var_value_mutated := var_value
	if rt.is_true(rt.greater(var_value_mutated, rt.new_int(0)))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('product_variation'), rt.call_function('get_post_type', [var_value_mutated.clone()]))))) {
		this.error(rt.new_string('order_item_product_invalid_variation_id'), rt.call_function('__', [
			rt.new_string('Invalid variation ID'),
			rt.new_string('woocommerce'),
		]), rt.new_int(400), rt.create_array([
			rt.ArrayItem{ key: 'variation_id', val: var_value_mutated },
		]))
	}
	this.set_prop(rt.new_string('variation_id'), rt.call_function('absint', [
		var_value_mutated.clone()]))
}
...
fn (mut this Class_WC_Order_Item_Product) set_subtotal(var_value rt.PhpVal) {
	...
	if !(var_value_mutated.clone().is_long() || var_value_mutated.clone().is_double()) {
		var_value_mutated = rt.new_int(0)
	}
	this.set_prop(rt.new_string('subtotal'), var_value_mutated.clone())
}
...
fn (mut this Class_WC_Order_Item_Product) set_total(var_value rt.PhpVal) {
	...
	this.set_prop(rt.new_string('total'), var_value_mutated.clone())
	if rt.is_true(rt.identical(rt.new_string(''), this.get_subtotal('')))
		|| rt.is_true(rt.less(this.get_subtotal(''), this.get_total(''))) {
		this.set_subtotal(var_value_mutated.clone())
	}
}

fn (mut this Class_WC_Order_Item_Product) set_subtotal_tax(var_value rt.PhpVal) {
	mut var_value_mutated := var_value
	this.set_prop(rt.new_string('subtotal_tax'), rt.call_function('wc_format_decimal', [
		var_value_mutated.clone(),
	]))
}
...
fn (mut this Class_WC_Order_Item_Product) set_total_tax(var_value rt.PhpVal) {
	mut var_value_mutated := var_value
	this.set_prop(rt.new_string('total_tax'), rt.call_function('wc_format_decimal', [
		var_value_mutated.clone(),
	]))
}
...
fn (mut this Class_WC_Order_Item_Product) set_taxes(var_raw_tax_data rt.PhpVal) {
	...
	var_raw_tax_data_mutated = rt.call_function('maybe_unserialize', [
		var_raw_tax_data_mutated.clone()])
	mut var_tax_data := {
		'total':    map[string]rt.PhpVal{}
		'subtotal': map[string]rt.PhpVal{}
	}
	if !(!rt.is_true(var_raw_tax_data_mutated.array_get(rt.new_string('total'))))
		&& !(!rt.is_true(var_raw_tax_data_mutated.array_get(rt.new_string('subtotal')))) {
		mut var_subtotal := var_raw_tax_data_mutated.array_get(rt.new_string('subtotal'))
		mut var_total := var_raw_tax_data_mutated.array_get(rt.new_string('total'))
		mut var_has_legacy_data := rt.new_bool(!(var_subtotal.clone().is_array())
			|| !(var_total.clone().is_array()))
		if rt.is_true(var_has_legacy_data) {
			mut var_order := this.get_order()
			if !(var_subtotal.clone().is_array()) {
				var_subtotal = this.convert_legacy_tax_value_to_array(var_subtotal.clone(),
					var_order.clone())
			}
			if !(var_total.clone().is_array()) {
				var_total = this.convert_legacy_tax_value_to_array(var_total.clone(),
					var_order.clone())
			}
			rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'warning', [
				rt.call_function('sprintf', [
					rt.call_function('__', [
						rt.new_string('Order item #%d contains legacy tax data format. Tax rate ID information is unavailable.'),
						rt.new_string('woocommerce'),
					]),
					this.get_id(),
				]),
				rt.create_array([
					rt.ArrayItem{ key: 'source', val: 'woocommerce-order-item-product' },
					rt.ArrayItem{ key: 'order_item_id', val: this.get_id() },
					rt.ArrayItem{
						key: 'order_id'
		...
		var_tax_data['subtotal'] = rt.call_function('array_map', [
			rt.new_string('wc_format_decimal'),
			var_subtotal.clone(),
		])
		var_tax_data['total'] = rt.call_function('array_map', [
			rt.new_string('wc_format_decimal'),
			var_total.clone(),
		])
		mut iife_temp_1 := Class_Automattic_WooCommerce_Utilities_NumberUtil{}
		mut iife_result_1 := iife_temp_1.array_sum(var_tax_data['subtotal'])
		mut iife_temp_2 := Class_Automattic_WooCommerce_Utilities_NumberUtil{}
		mut iife_result_2 := iife_temp_2.array_sum(var_tax_data['total'])
		if rt.is_true(rt.less(iife_result_1, iife_result_2)) {
			var_tax_data['subtotal'] = var_tax_data['total']
		}
	}
	...
	])))
	{
		...
		mut iife_result_3 := iife_temp_3.array_sum(var_tax_data['total'])
		this.set_total_tax(iife_result_3)
		mut iife_temp_4 := Class_Automattic_WooCommerce_Utilities_NumberUtil{}
		mut iife_result_4 := iife_temp_4.array_sum(var_tax_data['subtotal'])
		this.set_subtotal_tax(iife_result_4)
	} else {
		mut iife_temp_5 := Class_Automattic_WooCommerce_Utilities_NumberUtil{}
		mut iife_result_5 := iife_temp_5.array_sum(rt.call_function('array_map', [
		...
		mut iife_result_6 := iife_temp_6.array_sum(rt.call_function('array_map', [
			rt.new_string('wc_round_tax_total'),
			var_tax_data['subtotal'],
		]))
		this.set_subtotal_tax(iife_result_6)
	}
}
...
fn (mut this Class_WC_Order_Item_Product) set_variation(var_data rt.PhpVal) {
	if rt.is_true(rt.new_bool(var_data.clone().is_array())) {
		...
		for {
			...
			this.add_meta_data(rt.call_function('str_replace', [
...
fn (mut this Class_WC_Order_Item_Product) set_product(var_product rt.PhpVal) {
	...
	{
		this.error(rt.new_string('order_item_product_invalid_product'), rt.call_function('__', [
	...
	]))
	{
		...
		this.set_variation(if rt.call_function('is_callable', [
			rt.create_array([rt.ArrayItem{ key: none, val: var_product_mutated },
				rt.ArrayItem{ key: none, val: 'get_variation_attributes' }]),
		])
		{
			rt.call_method(var_product_mutated, 'get_variation_attributes', []rt.PhpVal{})
		} else {
...
fn (mut this Class_WC_Order_Item_Product) set_backorder_meta() {
	...
	if rt.is_true(var_product)
		...
		&& rt.is_true(rt.call_method(var_product, 'is_on_backorder', [this.get_quantity('')])) {
		this.add_meta_data(rt.call_function('apply_filters', [
			rt.new_string('woocommerce_backordered_item_meta_name'),
			rt.call_function('__', [rt.new_string('Backordered'),
				rt.new_string('woocommerce')]),
			rt.new_object('WC_Order_Item_Product', ['WC_Order_Item'], &this),
		]), rt.sub(this.get_quantity(''), rt.call_function('max', [
			rt.new_int(0), rt.call_method(var_product, 'get_stock_quantity', []rt.PhpVal{})])),
			rt.new_bool(true))
	}
}
...
fn (mut this Class_WC_Order_Item_Product) get_product() rt.PhpVal {
	...
	{
		var_product = rt.call_function('apply_filters', [
			rt.new_string('woocommerce_get_product_from_item'),
			var_product.clone(),
			rt.new_object('WC_Order_Item_Product', ['WC_Order_Item'], &this),
			this.get_order(),
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_order_item_product'),
		var_product.clone(),
		rt.new_object('WC_Order_Item_Product', ['WC_Order_Item'], &this),
	])
}
...
fn (mut this Class_WC_Order_Item_Product) get_item_download_url(var_download_id rt.PhpVal) rt.PhpVal {
	...
	mut var_order := this.get_order()
	return if rt.is_true(var_order) { rt.call_function('add_query_arg', [
			rt.create_array([
				rt.ArrayItem{
					key: 'download_file'
					val: if rt.is_true(this.get_variation_id('')) {
						this.get_variation_id('')
					} else {
						this.get_product_id('')
					}
				},
				rt.ArrayItem{ key: 'order', val: rt.call_method(var_order, 'get_order_key',
					[]rt.PhpVal{}) },
				rt.ArrayItem{ key: 'email', val: rt.call_function('rawurlencode', [
					rt.call_method(var_order, 'get_billing_email', []rt.PhpVal{}),
				]) },
				rt.ArrayItem{ key: 'key', val: var_download_id_mutated },
			]),
...
fn (mut this Class_WC_Order_Item_Product) get_item_downloads() rt.PhpVal {
	...
	mut var_product := this.get_product()
	mut var_order := this.get_order()
	mut var_product_id := if rt.is_true(this.get_variation_id('')) {
		this.get_variation_id('')
	} else {
		this.get_product_id('')
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_product))))
		...
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_product, 'is_downloadable', []rt.PhpVal{})))))
	...
	]))
	{
		...
		rt.new_string(sha1.hexhash(rt.call_method(var_order, 'get_billing_email', []rt.PhpVal{}).to_string()))
	}
	mut iife_temp_7 := Class_WC_Data_Store{}
	mut iife_result_7 := iife_temp_7.load(rt.new_string('customer-download'))
	mut var_data_store := iife_result_7
	mut var_customer_downloads := rt.call_method(var_data_store, 'get_downloads', [
		rt.create_array([
			rt.ArrayItem{ key: 'user_email', val: rt.call_method(var_order, 'get_billing_email',
				[]rt.PhpVal{}) },
			rt.ArrayItem{ key: 'order_id', val: rt.call_method(var_order, 'get_id', []rt.PhpVal{}) },
			rt.ArrayItem{ key: 'product_id', val: var_product_id },
		]),
	])
	mut iter_2 := var_customer_downloads.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_customer_download := item_2.val
		mut var_download_id := rt.call_method(var_customer_download, 'get_download_id',
			[]rt.PhpVal{})
		if rt.is_true(rt.call_method(var_product, 'has_file', [
			var_download_id.clone()]))
		{
			mut var_file := rt.call_method(var_product, 'get_file', [
				var_download_id.clone()])
			var_files.array_set(var_download_id,
				rt.call_method(var_file, 'get_data', []rt.PhpVal{}))
			var_files.array_get_mut(var_download_id).array_set('downloads_remaining', rt.call_method(var_customer_download,
				'get_downloads_remaining', []rt.PhpVal{}))
			var_files.array_get_mut(var_download_id).array_set('access_expires', rt.call_method(var_customer_download,
				'get_access_expires', []rt.PhpVal{}))
			var_files.array_get_mut(var_download_id).array_set('download_url', rt.call_function('add_query_arg', [
				rt.create_array([
					rt.ArrayItem{ key: 'download_file', val: var_product_id },
					rt.ArrayItem{ key: 'order', val: rt.call_method(var_order, 'get_order_key',
						[]rt.PhpVal{}) },
					rt.ArrayItem{ key: 'uid', val: var_email_hash },
	...
	var_files = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_get_item_downloads'),
		var_files.clone(),
		rt.new_object('WC_Order_Item_Product', ['WC_Order_Item'], &this),
		var_order.clone(),
	])
...
fn (mut this Class_WC_Order_Item_Product) get_tax_status() rt.PhpVal {
	mut var_product := this.get_product()
	return if rt.is_true(var_product) {
		rt.call_method(var_product, 'get_tax_status', []rt.PhpVal{})
	} else {
		Class_Automattic_WooCommerce_Enums_ProductTaxStatus.taxable()
	}
}

fn (mut this Class_WC_Order_Item_Product) get_formatted_meta_data(hideprefix string, include_all bool) rt.PhpVal {
	mut var_formatted_meta := this.Class_WC_Order_Item.get_formatted_meta_data(rt.new_string(hideprefix),
		rt.new_bool(include_all))
	mut var_order := this.get_order()
	if rt.is_true(var_order)
		&& rt.is_true(rt.call_method(var_order, 'has_status', [Class_Automattic_WooCommerce_Enums_OrderStatus.completed()])) {
		mut var_backorder_meta_key := rt.call_function('apply_filters', [
			rt.new_string('woocommerce_backordered_item_meta_name'),
			rt.call_function('__', [rt.new_string('Backordered'),
				rt.new_string('woocommerce')]),
			rt.new_object('WC_Order_Item_Product', ['WC_Order_Item'], &this),
		])
		mut iter_3 := var_formatted_meta.iterator()
		for {
			item_3 := iter_3.next() or { break }
			mut var_meta := item_3.val
			mut var_meta_id := item_3.key
			if !(rt.get_property(var_meta, 'key')).is_null()
				&& rt.is_true(rt.identical(rt.get_property(var_meta, 'key'), var_backorder_meta_key)) {
...
fn (mut this Class_WC_Order_Item_Product) offsetget(var_offset rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_string('line_subtotal'), var_offset_mutated)) {
		var_offset_mutated = rt.new_string('subtotal')
	} else if rt.is_true(rt.identical(rt.new_string('line_subtotal_tax'), var_offset_mutated)) {
		var_offset_mutated = rt.new_string('subtotal_tax')
		...
		var_offset_mutated = rt.new_string('quantity')
	}
...
fn (mut this Class_WC_Order_Item_Product) offsetset(var_offset rt.PhpVal, var_value rt.PhpVal) {
	...
	rt.call_function('wc_deprecated_function', [
		rt.new_string('WC_Order_Item_Product::offsetSet'),
		rt.new_string('4.4.0'),
		rt.new_string(''),
	])
	if rt.is_true(rt.identical(rt.new_string('line_subtotal'), var_offset_mutated)) {
	...
	this.Class_WC_Order_Item.offsetset(var_offset_mutated.clone(), var_value_mutated.clone())
}
...
fn (mut this Class_WC_Order_Item_Product) offsetexists(var_offset rt.PhpVal) bool {
	...
	if rt.is_true(rt.call_function('in_array', [var_offset_mutated.clone(),
		rt.create_array([rt.ArrayItem{ key: none, val: 'line_subtotal' },
			rt.ArrayItem{ key: none, val: 'line_subtotal_tax' },
			rt.ArrayItem{ key: none, val: 'line_total' }, rt.ArrayItem{ key: none, val: 'line_tax' },
			rt.ArrayItem{ key: none, val: 'line_tax_data' }, rt.ArrayItem{
				key: none
				val: 'item_meta_array'
			}, rt.ArrayItem{ key: none, val: 'item_meta' }, rt.ArrayItem{ key: none, val: 'qty' }]),
		rt.new_bool(true)]))
	{
		return true
	}
...
fn (mut this Class_WC_Order_Item_Product) calculate_cogs_value_core() f64 {
	...
	mut var_cogs_per_unit := rt.call_method(var_product, 'get_cogs_total_value', []rt.PhpVal{})
	return (rt.mul(var_cogs_per_unit, this.get_quantity(''))).to_f64()
}
...
fn (mut this Class_WC_Order_Item_Product) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'legacy_values' {
			this.legacy_values = val
			return true
		}
		'legacy_cart_item_key' {
			this.legacy_cart_item_key = val
			return true
		}
		'extra_data' {
			this.extra_data = val
