...
fn Class_WC_Log_Handler.format_time(var_timestamp rt.PhpVal) rt.PhpVal {
	return rt.call_function('gmdate', [rt.new_string('c'), var_timestamp.clone()])
}
...
fn Class_WC_Log_Handler.format_entry(var_timestamp rt.PhpVal, var_level rt.PhpVal, var_message rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	mut var_time_string := Class_WC_Log_Handler.format_time(var_timestamp.clone())
	mut var_level_string := rt.new_string(var_level.clone().to_string().to_upper())
	mut var_entry :=
		rt.new_string('${var_time_string.to_string()} ${var_level_string.to_string()} ${var_message.to_string()}')
...
fn Class_WC_Log_Handler.get_backtrace() rt.PhpVal {
	closure_1_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		...
		mut var_reflector := create_reflectionclass(var_class.clone())
		if rt.has_exception() {
			unsafe {
				goto catch_label_1
			}
	...
	closure_3_fn := fn [var_ignore_files] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_frame := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		mut var_ignore := rt.new_bool(var_frame.array_isset(rt.new_string('file'))
			&& rt.is_true(rt.call_function('in_array', [var_frame.array_get(rt.new_string('file')), var_ignore_files.clone(), rt.new_bool(true)])))
		return rt.new_bool(!(rt.is_true(var_ignore)))
	}
	mut var_filtered_backtrace := rt.call_function('array_filter', [
		var_backtrace.clone(), rt.new_closure(closure_3_fn)])
	return rt.call_function('array_values', [var_filtered_backtrace.clone()])
}
