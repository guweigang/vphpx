...
fn (mut this Class_WC_Payment_Token) construct(token string) {
	...
	if rt.is_true(rt.new_bool(rt.new_string(token_mutated).clone().is_long()
		|| rt.new_string(token_mutated).clone().is_double()))
	{
		this.set_id(rt.new_string(token_mutated))
	} else if rt.is_true(rt.new_bool(rt.new_string(token_mutated).clone().is_object())) {
	...
	if rt.is_true(rt.greater(this.get_id(), rt.new_int(0))) {
		rt.call_method(rt.get_property(rt.new_object('WC_Payment_Token', [
			'WC_Legacy_Payment_Token',
		], &this), 'data_store'), 'read', [
			rt.new_object('WC_Payment_Token', ['WC_Legacy_Payment_Token'], &this),
		])
	}
}
...
fn (mut this Class_WC_Payment_Token) set_token(var_token rt.PhpVal) {
	...
	this.set_prop(rt.new_string('token'), var_token_mutated.clone())
}
...
fn (mut this Class_WC_Payment_Token) set_gateway_id(var_gateway_id rt.PhpVal) {
	this.set_prop(rt.new_string('gateway_id'), var_gateway_id.clone())
}

fn (mut this Class_WC_Payment_Token) set_default(var_is_default rt.PhpVal) {
	this.set_prop(rt.new_string('is_default'), rt.new_bool(var_is_default.to_bool()))
}

fn (mut this Class_WC_Payment_Token) is_default() bool {
	return (this.get_prop(rt.new_string('is_default'), rt.new_string('view'))).to_bool()
}
...
fn create_wc_legacy_payment_token(_args ...rt.PhpVal) &Class_WC_Legacy_Payment_Token {
...
fn (mut this Class_WC_Payment_Token) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'is_default' {
			return rt.new_bool(this.is_default())
		}
		'validate' {
...
fn main() {
	...
	rt.include_file(
		(rt.get_constant('WC_ABSPATH')).str() + 'includes/legacy/abstract-wc-legacy-payment-token.php',
		'4')
}
