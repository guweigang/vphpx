...
fn (mut this Class_WC_Session) magic_get(var_key rt.PhpVal) rt.PhpVal {
	...
	return this.get(var_key_mutated.clone(), rt.new_null())
}

fn (mut this Class_WC_Session) magic_set(var_key rt.PhpVal, var_value rt.PhpVal) {
	mut var_key_mutated := var_key
	this.set(var_key_mutated.clone(), var_value.clone())
}
...
fn (mut this Class_WC_Session) magic_unset(var_key rt.PhpVal) {
	mut var_key_mutated := var_key
	var_key_mutated = rt.call_function('sanitize_key', [var_key_mutated.clone()])
...
fn (mut this Class_WC_Session) set(var_key rt.PhpVal, var_value rt.PhpVal) {
	...
	if rt.is_true(rt.identical(rt.new_null(), var_value)) {
		this.magic_unset(var_key_mutated.clone())
		return
	}
	var_key_mutated = rt.call_function('sanitize_key', [var_key_mutated.clone()])
	...
	mut var_serialized_value := rt.call_function('maybe_serialize', [
		var_value.clone()])
	if rt.is_true(rt.identical(var_serialized_original_value, var_serialized_value))
		|| rt.is_true(rt.identical(rt.call_function('maybe_unserialize', [var_serialized_original_value.clone()]), var_value)) {
		return
	}
	this._dirty = true
	this._data.array_set(var_key_mutated, var_serialized_value.clone())
}
