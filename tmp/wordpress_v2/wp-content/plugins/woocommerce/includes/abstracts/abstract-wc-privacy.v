...
fn (mut this Class_WC_Abstract_Privacy) construct(name string, export_priority i64, erase_priority i64) {
	...
	this.export_priority = rt.new_int(export_priority)
	this.erase_priority = rt.new_int(erase_priority)
	this.init()
}
...
fn (mut this Class_WC_Abstract_Privacy) init() {
	rt.call_function('add_action', [rt.new_string('admin_init'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Abstract_Privacy', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'add_privacy_message' },
		])])
	rt.call_function('add_filter', [rt.new_string('wp_privacy_personal_data_exporters'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Abstract_Privacy', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'register_exporters' },
		]),
		this.export_priority])
	rt.call_function('add_filter', [rt.new_string('wp_privacy_personal_data_erasers'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Abstract_Privacy', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'register_erasers' },
		]),
		this.erase_priority])
}
...
fn (mut this Class_WC_Abstract_Privacy) add_privacy_message() {
	...
	]))
	{
		...
		if rt.is_true(var_content) {
			rt.call_function('wp_add_privacy_policy_content', [this.name,
				rt.new_string(this.get_privacy_message())])
		}
	}
}
...
fn (mut this Class_WC_Abstract_Privacy) register_exporters(var_exporters rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		var_exporters_mutated.array_set(var_id, var_exporter.clone())
	}
	return var_exporters_mutated.clone()
}
...
fn (mut this Class_WC_Abstract_Privacy) register_erasers(var_erasers rt.PhpVal) rt.PhpVal {
	...
	mut iter_2 := this.erasers.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_eraser := item_2.val
		mut var_id := item_2.key
		var_erasers_mutated.array_set(var_id, var_eraser.clone())
	}
...
fn (mut this Class_WC_Abstract_Privacy) add_exporter(var_id rt.PhpVal, var_name rt.PhpVal, var_callback rt.PhpVal) rt.PhpVal {
	this.exporters.array_set(var_id, rt.create_array([
		rt.ArrayItem{ key: 'exporter_friendly_name', val: var_name },
...
fn (mut this Class_WC_Abstract_Privacy) add_eraser(var_id rt.PhpVal, var_name rt.PhpVal, var_callback rt.PhpVal) rt.PhpVal {
	this.erasers.array_set(var_id, rt.create_array([
		rt.ArrayItem{ key: 'eraser_friendly_name', val: var_name },
		rt.ArrayItem{ key: 'callback', val: var_callback },
	]))
	return this.erasers
}
...
fn (mut this Class_WC_Abstract_Privacy) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'export_priority' {
			this.export_priority = val
			return true
		}
		'erase_priority' {
			this.erase_priority = val
			return true
		}
		else {
