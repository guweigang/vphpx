...
fn Class_WC_Twenty_Twenty.init() {
	...
	rt.call_function('add_action', [rt.new_string('after_setup_theme'),
		rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT },
			rt.ArrayItem{ key: none, val: 'set_white_background' }]),
		rt.new_int(10)])
}
...
fn Class_WC_Twenty_Twenty.set_white_background() {
	mut var_background := rt.call_function('sanitize_hex_color_no_hash', [
		rt.call_function('get_theme_mod', [rt.new_string('background_color')]),
	])
	mut var_background_default := rt.new_string('f5efe0')
	if !(!rt.is_true(var_background))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_background, var_background_default)))) {
		return
	}
...
fn Class_WC_Twenty_Twenty.enqueue_styles(var_styles rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_twenty_twenty_styles'),
...
fn create_wc_twenty_twenty(_args ...rt.PhpVal) &Class_WC_Twenty_Twenty {
