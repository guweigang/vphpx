...
fn (mut this Class_WC_WCCOM_Site_Installation_Step_Download_Product) run() rt.PhpVal {
	...
	mut var_download_path := rt.call_method(var_upgrader, 'download_package', [
		rt.call_method(this.state, 'get_download_url', []rt.PhpVal{}),
	])
	if rt.is_true(rt.call_function('is_wp_error', [var_download_path.clone()]))
		|| !rt.is_true(var_download_path) {
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{},
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.missing_download_path())))
	}
	rt.call_method(this.state, 'set_download_path', [var_download_path.clone()])
	return this.state
}
