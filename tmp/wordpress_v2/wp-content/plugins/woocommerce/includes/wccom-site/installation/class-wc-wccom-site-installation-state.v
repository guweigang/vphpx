...
fn (mut this Class_WC_WCCOM_Site_Installation_State) construct(var_product_id rt.PhpVal) {
	this.product_id = var_product_id.clone()
}
...
fn Class_WC_WCCOM_Site_Installation_State.initiate_existing(var_product_id rt.PhpVal, var_idempotency_key rt.PhpVal, var_last_step_name rt.PhpVal, var_last_step_status rt.PhpVal, var_last_step_error rt.PhpVal, var_started_date rt.PhpVal) rt.PhpVal {
	mut var_instance := create_wc_wccom_site_installation_state(var_product_id.clone())
	rt.set_property(var_instance, 'idempotency_key', var_idempotency_key.clone())
	rt.set_property(var_instance, 'last_step_name', var_last_step_name.clone())
	rt.set_property(var_instance, 'last_step_status', var_last_step_status.clone())
	rt.set_property(var_instance, 'last_step_error', var_last_step_error.clone())
	rt.set_property(var_instance, 'started_date', var_started_date.clone())
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) initiate_step(var_step_name rt.PhpVal) {
	this.last_step_name = var_step_name.clone()
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) capture_failure(var_step_name rt.PhpVal, var_error_code rt.PhpVal) {
	this.last_step_name = var_step_name.clone()
	this.last_step_error = var_error_code.clone()
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) set_product_type(var_product_type rt.PhpVal) {
	this.product_type = var_product_type.clone()
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) set_product_name(var_product_name rt.PhpVal) {
	this.product_name = var_product_name.clone()
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) set_download_url(var_download_url rt.PhpVal) {
	this.download_url = var_download_url.clone()
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) set_download_path(var_download_path rt.PhpVal) {
	this.download_path = var_download_path.clone()
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) set_unpacked_path(var_unpacked_path rt.PhpVal) {
	this.unpacked_path = var_unpacked_path.clone()
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) set_installed_path(var_installed_path rt.PhpVal) {
	this.installed_path = var_installed_path.clone()
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) set_already_installed_plugin_info(var_plugin_info rt.PhpVal) {
	this.already_installed_plugin_info = var_plugin_info.clone()
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'initiate_existing' {
			...
			return Class_WC_WCCOM_Site_Installation_State.initiate_existing(dispatch_arg_0,
				dispatch_arg_1, dispatch_arg_2, dispatch_arg_3, dispatch_arg_4, dispatch_arg_5)
		}
		'initiate_new' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := if args.len > 1 { args[1] } else { rt.new_null() }
			return Class_WC_WCCOM_Site_Installation_State.initiate_new(dispatch_arg_0,
				dispatch_arg_1)
		}
		'get_product_id' {
...
fn (mut this Class_WC_WCCOM_Site_Installation_State) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'idempotency_key' {
			this.idempotency_key = val
			return true
		}
		'last_step_name' {
			this.last_step_name = val
			return true
		}
		'last_step_status' {
			this.last_step_status = val
			return true
		}
		'last_step_error' {
			this.last_step_error = val
			return true
		}
		'product_type' {
		...
		'download_path' {
			this.download_path = val
			return true
		}
		'unpacked_path' {
			this.unpacked_path = val
			return true
		}
		'installed_path' {
			this.installed_path = val
			return true
		}
		'already_installed_plugin_info' {
			this.already_installed_plugin_info = val
			return true
		}
		'started_date' {
