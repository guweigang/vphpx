...
pub fn Class_WC_WCCOM_Site_Installation_Manager.steps() rt.PhpVal {
	return rt.create_array([rt.ArrayItem{ key: none, val: 'get_product_info' },
		rt.ArrayItem{ key: none, val: 'download_product' }, rt.ArrayItem{
			key: none
			val: 'unpack_product'
		}, rt.ArrayItem{ key: none, val: 'move_product' }, rt.ArrayItem{
			key: none
			val: 'activate_product'
		}])
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_Manager) get_installation_status() rt.PhpVal {
	mut iife_temp_0 := Class_WC_WCCOM_Site_Installation_State_Storage{}
	mut iife_result_0 := iife_temp_0.get_state(rt.new_int(this.product_id))
	mut var_state := iife_result_0
	if rt.is_true(rt.new_bool(!(rt.is_true(var_state)))) {
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{}, create_wc_rest_wccom_site_installer_error(rt.call_function('esc_html', [
			Class_WC_REST_WCCOM_Site_Installer_Error_Codes.no_initiated_installation_found(),
		]))))
	}
...
fn (mut this Class_WC_WCCOM_Site_Installation_Manager) run_installation(run_until_step string) bool {
	mut iife_temp_1 := Class_WC_WCCOM_Site_Installation_State_Storage{}
	mut iife_result_1 := iife_temp_1.get_state(rt.new_int(this.product_id))
	mut var_state := iife_result_1
	if rt.is_true(var_state)
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_state, 'get_idempotency_key', []rt.PhpVal{}), this.idempotency_key)))) {
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{},
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.idempotency_key_mismatch())))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_state)))) {
		mut iife_temp_2 := Class_WC_WCCOM_Site_Installation_State{}
		mut iife_result_2 := iife_temp_2.initiate_new(rt.new_int(this.product_id),
			rt.new_string(this.idempotency_key))
		var_state = iife_result_2
	}
	this.can_run_installation(rt.new_string(run_until_step), var_state.clone())
	mut var_next_step := rt.new_string(this.get_next_step(var_state.clone()))
	mut var_installation_steps := this.get_installation_steps(var_next_step.str(), run_until_step)
	closure_4_fn := fn [var_state] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_step_name := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		this.run_step(var_step_name.clone(), var_state.clone())
		return rt.new_null()
	}
	rt.call_function('array_walk', [var_installation_steps.clone(),
		rt.new_closure(closure_4_fn)])
	return true
}

fn (mut this Class_WC_WCCOM_Site_Installation_Manager) reset_installation() bool {
	mut iife_temp_4 := Class_WC_WCCOM_Site_Installation_State_Storage{}
	mut iife_result_4 := iife_temp_4.get_state(rt.new_int(this.product_id))
	mut var_state := iife_result_4
	if rt.is_true(rt.new_bool(!(rt.is_true(var_state)))) {
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{},
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.no_initiated_installation_found())))
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_state,
		'get_idempotency_key', []rt.PhpVal{}), this.idempotency_key))))
	{
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{},
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.idempotency_key_mismatch())))
	}
	mut iife_temp_5 := Class_WC_WCCOM_Site_Installation_State_Storage{}
	mut iife_result_5 := iife_temp_5.delete_state(var_state.clone())
	mut var_result := iife_result_5
	if rt.is_true(rt.new_bool(!(rt.is_true(var_result)))) {
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{},
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.failed_to_reset_installation_state())))
	}
	return true
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_Manager) can_run_installation(var_run_until_step rt.PhpVal, var_state rt.PhpVal) {
	...
	if rt.is_true(rt.identical(rt.call_method(var_state_mutated, 'get_last_step_status',
		[]rt.PhpVal{}), Class_WC_WCCOM_Site_Installation_State.step_status_in_progress()))
	{
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{},
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.installation_already_running())))
	}
	if rt.is_true(rt.identical(rt.call_method(var_state_mutated, 'get_last_step_status',
		[]rt.PhpVal{}), Class_WC_WCCOM_Site_Installation_State.step_status_failed()))
	{
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{},
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.installation_failed())))
	}
	if rt.is_true(rt.identical(rt.call_method(var_state_mutated, 'get_last_step_name',
		[]rt.PhpVal{}),
		Class_WC_WCCOM_Site_Installation_Manager.steps().array_get(rt.new_int(Class_WC_WCCOM_Site_Installation_Manager.steps().array_count() - 1))))
	{
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{},
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.all_installation_steps_run())))
	}
	if rt.is_true(rt.greater_equal(rt.call_function('array_search', [
		rt.call_method(var_state_mutated, 'get_last_step_name', []rt.PhpVal{}),
		Class_WC_WCCOM_Site_Installation_Manager.steps(),
		rt.new_bool(true),
	]), rt.call_function('array_search', [var_run_until_step.clone(),
		Class_WC_WCCOM_Site_Installation_Manager.steps(), rt.new_bool(true)])))
	{
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{},
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.requested_step_already_run())))
	}
	...
	{
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{},
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.filesystem_requirements_not_met())))
	}
}

fn (mut this Class_WC_WCCOM_Site_Installation_Manager) get_next_step(var_state rt.PhpVal) string {
	mut var_state_mutated := var_state
	mut var_last_executed_step := rt.call_method(var_state_mutated, 'get_last_step_name',
		[]rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(var_last_executed_step)))) {
		return (Class_WC_WCCOM_Site_Installation_Manager.steps().array_get(rt.new_int(0))).str()
	}
	mut var_last_executed_step_index := rt.call_function('array_search', [
		var_last_executed_step.clone(), Class_WC_WCCOM_Site_Installation_Manager.steps(),
		rt.new_bool(true)])
	return (Class_WC_WCCOM_Site_Installation_Manager.steps().array_get(rt.add(var_last_executed_step_index,
		rt.new_int(1)))).str()
}

fn (mut this Class_WC_WCCOM_Site_Installation_Manager) get_installation_steps(start_step string, end_step string) rt.PhpVal {
	mut var_start_step_offset := rt.call_function('array_search', [
		rt.new_string(start_step),
		Class_WC_WCCOM_Site_Installation_Manager.steps(),
		rt.new_bool(true),
	])
	mut var_end_step_index := rt.call_function('array_search', [
		rt.new_string(end_step), Class_WC_WCCOM_Site_Installation_Manager.steps(),
		rt.new_bool(true)])
	mut var_length := rt.add(rt.sub(var_end_step_index, var_start_step_offset), rt.new_int(1))
	return rt.call_function('array_slice', [
		Class_WC_WCCOM_Site_Installation_Manager.steps(),
		var_start_step_offset.clone(),
		var_length.clone(),
	])
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_Manager) run_step(var_step_name rt.PhpVal, var_state rt.PhpVal) {
	mut var_state_mutated := var_state
	rt.call_method(var_state_mutated, 'initiate_step', [var_step_name.clone()])
	mut iife_temp_6 := Class_WC_WCCOM_Site_Installation_State_Storage{}
	mut iife_result_6 := iife_temp_6.save_state(var_state_mutated.clone())
	mut var_class_name :=
		rt.new_string('WC_WCCOM_Site_Installation_Step_${var_step_name.to_string()}')
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	}
	mut var_current_step := rt.create_object_dynamically(var_class_name, [
	...
	if rt.instance_of(var_e_1, 'WC_REST_WCCOM_Site_Installer_Error') {
		...
		rt.call_method(var_state_mutated, 'capture_failure', [
			var_step_name.clone(), rt.call_method(var_exception, 'get_error_code', []rt.PhpVal{})])
		mut iife_temp_7 := Class_WC_WCCOM_Site_Installation_State_Storage{}
		mut iife_result_7 := iife_temp_7.save_state(var_state_mutated.clone())
		...
		rt.call_method(var_state_mutated, 'capture_failure', [
			var_step_name.clone(), Class_WC_REST_WCCOM_Site_Installer_Error_Codes.unexpected_error()])
		mut iife_temp_8 := Class_WC_WCCOM_Site_Installation_State_Storage{}
		mut iife_result_8 := iife_temp_8.save_state(var_state_mutated.clone())
		rt.throw_exception(rt.new_object('WC_REST_WCCOM_Site_Installer_Error', []string{}, create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.unexpected_error(), rt.call_method(var_error,
	...
	end_label_1:
	rt.call_method(var_state_mutated, 'complete_step', [var_step_name.clone()])
	mut iife_temp_9 := Class_WC_WCCOM_Site_Installation_State_Storage{}
	mut iife_result_9 := iife_temp_9.save_state(var_state_mutated.clone())
}
...
fn (mut this Class_WC_WCCOM_Site_Installation_Manager) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'product_id' {
			this.product_id = val.to_i64()
			return true
		}
		'idempotency_key' {
			this.idempotency_key = val.str()
			return true
		}
		else {
