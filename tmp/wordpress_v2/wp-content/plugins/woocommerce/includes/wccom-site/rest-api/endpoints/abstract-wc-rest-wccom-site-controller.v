...
fn (mut this Class_WC_REST_WCCOM_Site_Controller) check_permission() bool {
	...
	if !rt.is_true(var_current_user)
		|| (rt.is_true(rt.new_bool(rt.instance_of(var_current_user, 'WP_User')))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_current_user, 'exists', []rt.PhpVal{})))))) {
		mut var_error := rt.call_function('apply_filters', [
			Class_WC_WCCOM_Site.auth_error_filter_name(),
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.not_authenticated()),
		])
		return (create_wp_error(rt.call_method(var_error, 'get_error_code', []rt.PhpVal{}), rt.call_method(var_error,
			'get_error_message', []rt.PhpVal{}), rt.create_array([
			rt.ArrayItem{ key: 'status', val: rt.call_method(var_error, 'get_http_code',
				[]rt.PhpVal{}) },
		]))).to_bool()
	}
	if !(this.user_has_permission(var_current_user.clone())) {
		var_error =
			create_wc_rest_wccom_site_installer_error(Class_WC_REST_WCCOM_Site_Installer_Error_Codes.no_permission())
		return (create_wp_error(rt.call_method(var_error, 'get_error_code', []rt.PhpVal{}), rt.call_method(var_error,
			'get_error_message', []rt.PhpVal{}), rt.create_array([
			rt.ArrayItem{ key: 'status', val: rt.call_method(var_error, 'get_http_code',
				[]rt.PhpVal{}) },
