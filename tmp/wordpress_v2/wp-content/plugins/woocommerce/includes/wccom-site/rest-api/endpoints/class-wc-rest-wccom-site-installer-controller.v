...
fn (mut this Class_WC_REST_WCCOM_Site_Installer_Controller) get_product_install_state(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_installation_manager := create_wc_wccom_site_installation_manager(var_product_id.clone())
	mut var_state := var_installation_manager.get_installation_status()
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	return this.success_response(var_product_id.clone(), mut rt.cast_object_ptr[Class_?WC_WCCOM_Site_Installation_State](var_state))
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'WC_REST_WCCOM_Site_Installer_Error') {
		...
		return this.failure_response(var_product_id.clone(), var_exception.clone())
		unsafe { goto end_label_1 }
	}
	else {
...
fn (mut this Class_WC_REST_WCCOM_Site_Installer_Controller) install(var_request rt.PhpVal) rt.PhpVal {
	mut var_product_id := var_request.array_get(rt.new_string('product-id'))
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	mut var_run_until_step := var_request.array_get(rt.new_string('run-until-step'))
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	mut var_idempotency_key := var_request.array_get(rt.new_string('idempotency-key'))
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	mut var_installation_manager := create_wc_wccom_site_installation_manager(var_product_id.clone(), var_idempotency_key.clone())
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	var_installation_manager.run_installation(var_run_until_step.clone())
	if rt.has_exception() { unsafe { goto catch_label_2 } }
	mut var_response := this.success_response(var_product_id.clone(), rt.new_null())
	if rt.has_exception() { unsafe { goto catch_label_2 } }
...
catch_label_2:
	...
	if rt.instance_of(var_e_2, 'WC_REST_WCCOM_Site_Installer_Error') {
		...
		var_response = this.failure_response(var_product_id.clone(), var_exception.clone())
		unsafe { goto end_label_2 }
	}
	else {
...
fn (mut this Class_WC_REST_WCCOM_Site_Installer_Controller) success_response(var_product_id rt.PhpVal, mut var_state Class_?WC_WCCOM_Site_Installation_State) rt.PhpVal {
	...
	mut iife_result_0 := iife_temp_0.get_state(var_product_id_mutated.clone())
	var_state_mutated = if !(var_state_mutated).is_null() { var_state_mutated } else { iife_result_0 }
...
fn create_wc_rest_wccom_site_installer_controller(_args ...rt.PhpVal) &Class_WC_REST_WCCOM_Site_Installer_Controller {
...
fn create_wc_wccom_site_installation_manager(_args ...rt.PhpVal) &Class_WC_WCCOM_Site_Installation_Manager {
