...
fn (mut this Class_Automattic_WooCommerce_RestApi_Utilities_ImageAttachment) upload_image_from_src(var_src rt.PhpVal) {
	...
	mut var_upload := rt.call_function('wc_rest_upload_image_from_url', [
		rt.call_function('esc_url_raw', [var_src.clone()]),
	])
	if rt.is_true(rt.call_function('is_wp_error', [var_upload.clone()])) {
		...
		])))))
		{
			rt.throw_exception(rt.new_object('Automattic_WooCommerce_RestApi_Utilities_WC_REST_Exception',
				[]string{}, create_automattic_woocommerce_restapi_utilities_wc_rest_exception(rt.new_string('woocommerce_product_image_upload_error'), rt.call_method(var_upload,
				'get_error_message', []rt.PhpVal{}), rt.new_int(400))))
		} else {
			return
		}
	}
	this.id = rt.call_function('wc_rest_set_uploaded_image_as_attachment', [
		var_upload.clone(), this.object_id])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_attachment_is_image', [
		this.id,
	])))))
	{
		rt.throw_exception(rt.new_object('Automattic_WooCommerce_RestApi_Utilities_WC_REST_Exception',
			[]string{}, create_automattic_woocommerce_restapi_utilities_wc_rest_exception(rt.new_string('woocommerce_product_invalid_image_id'), rt.call_function('sprintf', [
...
fn (mut this Class_Automattic_WooCommerce_RestApi_Utilities_ImageAttachment) update_alt_text(var_text rt.PhpVal) {
	...
	rt.call_function('update_post_meta', [this.id, rt.new_string('_wp_attachment_image_alt'),
		rt.call_function('wc_clean', [var_text.clone()])])
}
...
fn (mut this Class_Automattic_WooCommerce_RestApi_Utilities_ImageAttachment) update_name(var_text rt.PhpVal) {
	...
	rt.call_function('wp_update_post', [
		rt.create_array([rt.ArrayItem{ key: 'ID', val: this.id },
			rt.ArrayItem{ key: 'post_title', val: var_text }]),
	])
}
...
fn create_automattic_woocommerce_restapi_utilities_wc_rest_exception(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_RestApi_Utilities_WC_REST_Exception {
