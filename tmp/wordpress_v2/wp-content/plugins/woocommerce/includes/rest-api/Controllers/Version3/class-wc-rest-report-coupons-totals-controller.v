...
fn (mut this Class_WC_REST_Report_Coupons_Totals_Controller) get_reports() rt.PhpVal {
	...
	mut var_data := rt.call_function('get_transient', [
	...
	for {
		...
		mut var_total := rt.new_int(if var_results.array_isset(rt.new_int(0)) {
			rt.new_int((rt.get_property(var_results.array_get(rt.new_int(0)), 'total')).to_i64())
		} else {
			0
		})
		var_data.array_push(rt.create_array([rt.ArrayItem{ key: 'slug', val: var_slug },
			rt.ArrayItem{ key: 'name', val: var_name }, rt.ArrayItem{ key: 'total', val: var_total }]))
	}
	rt.call_function('set_transient', [rt.new_string('rest_api_coupons_type_count'),
		var_data.clone(), rt.get_constant('YEAR_IN_SECONDS')])
	return var_data.clone()
}
...
fn (mut this Class_WC_REST_Report_Coupons_Totals_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_prepare_report_coupons_count'),
		var_response.clone(),
		var_report.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_WC_REST_Report_Coupons_Totals_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := {
		...
		'title':      rt.new_string('report_coupon_total')
		...
		'properties': {
			...
			'name':  {
				'description': rt.call_function('__', [
					rt.new_string('Coupon type name.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
			...
			'total': {
				'description': rt.call_function('__', [
					rt.new_string('Amount of coupons.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
...
fn create_wc_rest_report_coupons_totals_controller(_args ...rt.PhpVal) &Class_WC_REST_Report_Coupons_Totals_Controller {
