...
fn (mut this Class_WC_REST_Variations_Controller) register_routes() {
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('WC_REST_Variations_Controller', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Variations_Controller', [
						'WC_REST_Product_Variations_Controller',
					], &this) },
...
fn (mut this Class_WC_REST_Variations_Controller) prepare_objects_query(var_request rt.PhpVal) rt.PhpVal {
	mut var_args :=
		this.Class_WC_REST_Product_Variations_Controller.prepare_objects_query(var_request.clone())
	if rt.is_true(rt.identical(rt.concat(rt.concat(rt.new_string('/'), rt.get_property(rt.new_object('WC_REST_Variations_Controller', [
		'WC_REST_Product_Variations_Controller',
	], &this), 'namespace')), rt.new_string('/variations')), rt.call_method(var_request,
		'get_route', []rt.PhpVal{})))
	{
...
fn create_wc_rest_variations_controller(_args ...rt.PhpVal) &Class_WC_REST_Variations_Controller {
