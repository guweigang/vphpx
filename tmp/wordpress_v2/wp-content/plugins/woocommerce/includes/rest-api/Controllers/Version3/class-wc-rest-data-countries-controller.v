...
fn (mut this Class_WC_REST_Data_Countries_Controller) get_country(var_country_code rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_countries.clone().array_isset(var_country_code_mutated.clone())))))) {
...
fn (mut this Class_WC_REST_Data_Countries_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	mut var_countries := rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'get_countries', []rt.PhpVal{})
	mut var_data := rt.new_array()
	mut iter_2 := rt.func_array_keys(var_countries.clone()).iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_country_code := item_2.val
		mut var_country := this.get_country(var_country_code.clone(), var_request.clone())
		mut var_response := this.prepare_item_for_response(var_country.clone(), var_request.clone())
...
fn (mut this Class_WC_REST_Data_Countries_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	mut var_data := this.get_country(rt.new_string((var_request.array_get(rt.new_string('location')).to_string().to_upper()).str()), var_request.clone())
...
fn (mut this Class_WC_REST_Data_Countries_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_prepare_data_country'), var_response.clone(), var_item.clone(), var_request.clone()])
}

fn (mut this Class_WC_REST_Data_Countries_Controller) prepare_links(var_item rt.PhpVal) rt.PhpVal {
	mut var_country_code := rt.new_string(var_item.array_get(rt.new_string('code')).to_string().to_lower())
	mut var_links := { 'self': { 'href': rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s/%s'), this.namespace, this.rest_base, var_country_code.clone()])]) }, 'collection': { 'href': rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s'), this.namespace, this.rest_base])]) } }
