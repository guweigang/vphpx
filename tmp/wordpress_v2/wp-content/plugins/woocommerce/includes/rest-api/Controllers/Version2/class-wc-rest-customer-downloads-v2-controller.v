...
fn (mut this Class_WC_REST_Customer_Downloads_V2_Controller) prepare_item_for_response(var_download rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_data := rt.create_array([
		rt.ArrayItem{ key: 'download_id', val: rt.get_property(var_download, 'download_id') },
		rt.ArrayItem{ key: 'download_url', val: rt.get_property(var_download, 'download_url') },
		rt.ArrayItem{ key: 'product_id', val: rt.get_property(var_download, 'product_id') },
		rt.ArrayItem{ key: 'product_name', val: rt.get_property(var_download, 'product_name') },
		rt.ArrayItem{ key: 'download_name', val: rt.get_property(var_download, 'download_name') },
		rt.ArrayItem{ key: 'order_id', val: rt.get_property(var_download, 'order_id') },
		rt.ArrayItem{ key: 'order_key', val: rt.get_property(var_download, 'order_key') },
		rt.ArrayItem{
			key: 'downloads_remaining'
			val: if rt.is_true(rt.identical(rt.new_string(''), rt.get_property(var_download,
				'downloads_remaining')))
			{
				rt.new_string('unlimited')
			} else {
				rt.get_property(var_download, 'downloads_remaining')
			}
		},
		rt.ArrayItem{
			key: 'access_expires'
			val: if rt.is_true(rt.get_property(var_download, 'access_expires')) { rt.call_function('wc_rest_prepare_date_response', [
					rt.get_property(var_download, 'access_expires'),
				]) } else { rt.new_string('never') }
		},
		rt.ArrayItem{
			key: 'access_expires_gmt'
			val: if rt.is_true(rt.get_property(var_download, 'access_expires')) { rt.call_function('wc_rest_prepare_date_response', [
	...
	rt.call_method(var_response, 'add_links', [
		this.prepare_links(var_download.clone(), var_request.clone()),
	])
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_prepare_customer_download'),
		var_response.clone(),
		var_download.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_WC_REST_Customer_Downloads_V2_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := {
		...
		'title':      rt.new_string('customer_download')
		'type':       rt.new_string('object')
		'properties': {
			'download_id':         {
				'description': rt.call_function('__', [rt.new_string('Download ID.'),
			...
			'download_url':        {
				'description': rt.call_function('__', [
					rt.new_string('Download file URL.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
			...
			'product_id':          {
				'description': rt.call_function('__', [
					rt.new_string('Downloadable product ID.'),
					rt.new_string('woocommerce'),
				])
			...
			'download_name':       {
				'description': rt.call_function('__', [
					rt.new_string('Downloadable file name.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
			...
			'order_id':            {
				'description': rt.call_function('__', [rt.new_string('Order ID.'),
					rt.new_string('woocommerce')])
				'type':        rt.new_string('integer')
				'context':     map[string]rt.PhpVal{}
				'readonly':    rt.new_bool(true)
			...
			'order_key':           {
				'description': rt.call_function('__', [rt.new_string('Order key.'),
					rt.new_string('woocommerce')])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
				'readonly':    rt.new_bool(true)
			...
			'downloads_remaining': {
				'description': rt.call_function('__', [
					rt.new_string('Number of downloads remaining.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
			...
			'access_expires':      {
				'description': rt.call_function('__', [
					rt.new_string("The date when download access expires, in the site's timezone."),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
			...
			'access_expires_gmt':  {
				'description': rt.call_function('__', [
					rt.new_string('The date when download access expires, as GMT.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
			...
			'file':                {
				'description': rt.call_function('__', [rt.new_string('File details.'),
					rt.new_string('woocommerce')])
				'type':        rt.new_string('object')
				'context':     map[string]rt.PhpVal{}
				'readonly':    rt.new_bool(true)
...
fn create_wc_rest_customer_downloads_v2_controller(_args ...rt.PhpVal) &Class_WC_REST_Customer_Downloads_V2_Controller {
...
fn create_wc_rest_customer_downloads_v1_controller(_args ...rt.PhpVal) &Class_WC_REST_Customer_Downloads_V1_Controller {
