...
fn (mut this Class_WC_REST_Coupons_Controller) prepare_objects_query(var_request rt.PhpVal) rt.PhpVal {
	mut var_args :=
		this.Class_WC_REST_Coupons_V2_Controller.prepare_objects_query(var_request.clone())
	var_args.array_set('post_status', var_request.array_get(rt.new_string('status')))
	if !(!rt.is_true(var_request.array_get(rt.new_string('code')))) {
...
fn create_wc_rest_coupons_controller(_args ...rt.PhpVal) &Class_WC_REST_Coupons_Controller {
