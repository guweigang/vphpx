...
fn (mut this Class_WC_REST_Product_Categories_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_image_id) {
		...
		var_data.array_set('image', rt.create_array([
			...
			rt.ArrayItem{ key: 'date_modified', val: rt.call_function('wc_rest_prepare_date_response', [
				rt.get_property(var_attachment, 'post_modified'),
			]) },
	...
	return rt.call_function('apply_filters', [
		rt.concat(rt.new_string('woocommerce_rest_prepare_'), rt.get_property(rt.new_object('WC_REST_Product_Categories_Controller', [
...
fn (mut this Class_WC_REST_Product_Categories_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := {
		...
		'title':      rt.get_property(rt.new_object('WC_REST_Product_Categories_Controller', [
			'WC_REST_Product_Categories_V2_Controller',
		], &this), 'taxonomy')
		...
		'properties': {
			...
			'image':       {
				...
				'properties':  {
					...
					'date_created_gmt':  {
						'description': rt.call_function('__', [
							rt.new_string('The date the image was created, as GMT.'),
							rt.new_string('woocommerce'),
						])
						'type':        rt.new_string('date-time')
						'context':     map[string]rt.PhpVal{}
					...
					'date_modified_gmt': {
						'description': rt.call_function('__', [
							rt.new_string('The date the image was last modified, as GMT.'),
							rt.new_string('woocommerce'),
						])
						'type':        rt.new_string('date-time')
						'context':     map[string]rt.PhpVal{}
...
fn (mut this Class_WC_REST_Product_Categories_Controller) update_term_meta_fields(var_term rt.PhpVal, var_request rt.PhpVal) bool {
	...
	if var_request.array_isset(rt.new_string('image')) {
		...
		if rt.is_true(var_image_id)
			...
			if !(!rt.is_true(var_request.array_get(rt.new_string('image')).array_get(rt.new_string('name')))) {
				rt.call_function('wp_update_post', [
					rt.create_array([rt.ArrayItem{ key: 'ID', val: var_image_id },
						rt.ArrayItem{ key: 'post_title', val: rt.call_function('wc_clean', [
							var_request.array_get(rt.new_string('image')).array_get(rt.new_string('name')),
						]) }]),
				])
			}
		} else {
...
fn create_wc_rest_product_categories_controller(_args ...rt.PhpVal) &Class_WC_REST_Product_Categories_Controller {
