...
fn (mut this Class_WC_REST_Products_Controller) prepare_objects_query(var_request rt.PhpVal) rt.PhpVal {
	...
	for var_taxonomy, var_key in var_taxonomies {
		if !(!rt.is_true(var_request.array_get(rt.new_string(key)))) {
			var_tax_query << rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: taxonomy }, rt.ArrayItem{ key: 'field', val: 'term_id' }, rt.ArrayItem{ key: 'terms', val: var_request.array_get(rt.new_string(key)) }])
		}
	}
	...
	if !(!rt.is_true(var_request.array_get(rt.new_string('exclude_types')))) {
		var_tax_query << rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: 'product_type' }, rt.ArrayItem{ key: 'field', val: 'slug' }, rt.ArrayItem{ key: 'terms', val: var_request.array_get(rt.new_string('exclude_types')) }, rt.ArrayItem{ key: 'operator', val: 'NOT IN' }])
	}
	if !(!rt.is_true(var_request.array_get(rt.new_string('attribute')))) && !(!rt.is_true(var_request.array_get(rt.new_string('attribute_term')))) {
		if rt.is_true(rt.call_function('in_array', [var_request.array_get(rt.new_string('attribute')), rt.call_function('wc_get_attribute_taxonomy_names', []rt.PhpVal{}), rt.new_bool(true)])) {
			var_tax_query << rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: var_request.array_get(rt.new_string('attribute')) }, rt.ArrayItem{ key: 'field', val: 'term_id' }, rt.ArrayItem{ key: 'terms', val: var_request.array_get(rt.new_string('attribute_term')) }])
		}
	}
	if !(!rt.is_true(var_tax_query)) {
		if !(!rt.is_true(var_args.array_get(rt.new_string('tax_query')))) {
			var_args.array_set('tax_query', rt.call_function('array_merge', [rt.create_array_from_list(var_tax_query), var_args.array_get(rt.new_string('tax_query'))]))
		} else {
...
fn (mut this Class_WC_REST_Products_Controller) get_objects(var_query_args rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_add_search_criteria) {
		...
		rt.call_function('add_filter', [rt.new_string('posts_where'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'add_search_criteria_to_wp_query_where' }])])
	}
	if !(!rt.is_true(this.exclude_status)) {
		rt.call_function('add_filter', [rt.new_string('posts_where'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'exclude_product_statuses' }])])
	}
	mut var_result := this.Class_WC_REST_Products_V2_Controller.get_objects(var_query_args.clone())
	if rt.is_true(var_add_search_criteria) {
		rt.call_function('remove_filter', [rt.new_string('posts_join'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'add_search_criteria_to_wp_query_join' }])])
		rt.call_function('remove_filter', [rt.new_string('posts_where'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'add_search_criteria_to_wp_query_where' }])])
		this.search_sku_arg_value = rt.new_string('')
		this.search_name_or_sku_tokens = rt.new_null()
		this.search_fields_tokens = rt.new_null()
	}
	if !(!rt.is_true(this.exclude_status)) {
		rt.call_function('remove_filter', [rt.new_string('posts_where'), rt.create_array([rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this) }, rt.ArrayItem{ key: none, val: 'exclude_product_statuses' }])])
		this.exclude_status = rt.new_array()
	}
...
fn (mut this Class_WC_REST_Products_Controller) build_dynamic_search_clauses(var_tokens rt.PhpVal, var_fields rt.PhpVal) string {
	...
	mut iter_2 := var_tokens_mutated.iterator()
	for {
		...
		for {
			...
			var_field_token_clauses << rt.call_method(var_wpdb, 'prepare', [rt.new_string("(${var_db_column.to_string()} LIKE %s)"), var_like_search.clone()])
		}
		if rt.is_true(var_field_token_clauses) {
			var_field_clauses << '(' + (rt.call_function('implode', [rt.new_string(' OR '), rt.create_array_from_list(var_field_token_clauses)])).str() + ')'
		}
	}
	return if rt.is_true(var_field_clauses) { ' AND (' + (rt.call_function('implode', [rt.new_string(' AND '), rt.create_array_from_list(var_field_clauses)])).str() + ')' } else { '' }
}
...
fn (mut this Class_WC_REST_Products_Controller) exclude_product_statuses(var_where rt.PhpVal) string {
	...
	if !(!rt.is_true(this.exclude_status)) && this.exclude_status.is_array() {
		...
		mut iter_4 := this.exclude_status.iterator()
		for {
			item_4 := iter_4.next() or { break }
			mut var_status_to_exclude := item_4.val
...
fn (mut this Class_WC_REST_Products_Controller) set_product_images(var_product rt.PhpVal, var_images rt.PhpVal) rt.PhpVal {
	...
	if !(!rt.is_true(var_images_mutated)) {
		...
		mut iter_5 := var_images_mutated.iterator()
		for {
		...
		rt.call_method(var_product_mutated, 'set_gallery_image_ids', [rt.create_array_from_list(var_gallery)])
	} else {
...
fn (mut this Class_WC_REST_Products_Controller) prepare_object_for_database(var_request rt.PhpVal, creating bool) rt.PhpVal {
	...
	if var_request.array_isset(rt.new_string('attributes')) {
		...
		mut iter_6 := var_request.array_get(rt.new_string('attributes')).iterator()
		for {
			...
			if rt.is_true(var_attribute_id) {
				...
				if !(!rt.is_true(var_values)) {
					...
					var_attributes << var_attribute_object
				}
		...
		rt.call_method(var_product, 'set_attributes', [rt.create_array_from_list(var_attributes)])
	}
	...
	if var_request.array_isset(rt.new_string('upsell_ids')) {
		...
		if !(!rt.is_true(var_ids)) {
			mut iter_7 := var_ids.iterator()
			for {
				item_7 := iter_7.next() or { break }
				mut var_id_shadow := item_7.val
	...
	if var_request.array_isset(rt.new_string('tags')) && var_request.array_get(rt.new_string('tags')).is_array() {
		...
		mut iter_9 := var_request.array_get(rt.new_string('tags')).iterator()
		for {
			item_9 := iter_9.next() or { break }
			mut var_tag := item_9.val
			if !(var_tag.array_isset(rt.new_string('name'))) {
				var_new_tags << var_tag.clone()
				continue
			}
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('term_exists', [var_tag.array_get(rt.new_string('name')), rt.new_string('product_tag')]))))) {
				...
				if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_wp_error', [var_term.clone()]))))) {
					var_new_tags << rt.create_array([rt.ArrayItem{ key: 'id', val: var_term.array_get(rt.new_string('term_id')) }])
					continue
				}
			} else {
				var_new_tags << rt.create_array([rt.ArrayItem{ key: 'id', val: rt.get_property(rt.call_function('get_term_by', [rt.new_string('name'), var_tag.array_get(rt.new_string('name')), rt.new_string('product_tag')]), 'term_id') }])
			}
		}
	...
	return rt.call_function('apply_filters', [rt.concat(rt.concat(rt.new_string('woocommerce_rest_pre_insert_'), rt.get_property(rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this), 'post_type')), rt.new_string('_object')), var_product.clone(), var_request.clone(), rt.new_bool(creating)])
}
...
fn (mut this Class_WC_REST_Products_Controller) get_collection_params() rt.PhpVal {
	...
	mut var_search_fields_enum := ['name', 'global_unique_id', 'description', 'short_description']
	if rt.is_true(rt.call_function('wc_product_sku_enabled', []rt.PhpVal{})) {
		var_search_fields_enum << 'sku'
	}
	...
	var_params.array_set('pos_products_only', rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_function('__', [rt.new_string('Limit result set to products visible in Point of Sale.'), rt.new_string('woocommerce')]) }, rt.ArrayItem{ key: 'type', val: 'boolean' }, rt.ArrayItem{ key: 'sanitize_callback', val: 'wc_string_to_bool' }, rt.ArrayItem{ key: 'validate_callback', val: 'rest_validate_request_arg' }]))
...
fn (mut this Class_WC_REST_Products_Controller) get_downloads(var_product rt.PhpVal) rt.PhpVal {
	...
	mut var_context := if !(rt.get_property(rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this), 'request')).is_null() && rt.get_property(rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this), 'request').array_isset(rt.new_string('context')) { rt.get_property(rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this), 'request').array_get(rt.new_string('context')) } else { rt.new_string('view') }
	if rt.is_true(rt.call_method(var_product_mutated, 'is_downloadable', []rt.PhpVal{})) || rt.is_true(rt.identical(rt.new_string('edit'), var_context)) {
		mut iter_10 := rt.call_method(var_product_mutated, 'get_downloads', []rt.PhpVal{}).iterator()
		for {
			item_10 := iter_10.next() or { break }
			mut var_file := item_10.val
			mut var_file_id := item_10.key
...
fn (mut this Class_WC_REST_Products_Controller) get_product_data(var_product rt.PhpVal, context string) rt.PhpVal {
	...
	if !(rt.get_property(rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this), 'request')).is_null() {
		mut var_fields := this.get_fields_for_response(rt.get_property(rt.new_object('WC_REST_Products_Controller', ['WC_REST_Products_V2_Controller'], &this), 'request'))
...
fn (mut this Class_WC_REST_Products_Controller) create_item(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_response.clone()])) {
		if !(!rt.is_true(this.processed_attachment_ids_for_request)) {
			mut iter_11 := this.processed_attachment_ids_for_request.iterator()
			for {
				item_11 := iter_11.next() or { break }
				mut var_attachment_id := item_11.val
...
fn create_wc_rest_products_controller(_args ...rt.PhpVal) &Class_WC_REST_Products_Controller {
