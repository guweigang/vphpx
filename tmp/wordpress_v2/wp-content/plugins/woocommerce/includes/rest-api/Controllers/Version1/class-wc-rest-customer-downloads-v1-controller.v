		...
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Customer_Downloads_V1_Controller', [
						'WC_REST_Controller',
					], &this) },
...
fn (mut this Class_WC_REST_Customer_Downloads_V1_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	mut var_downloads := rt.call_function('wc_get_customer_available_downloads', [
		rt.new_int((var_request.array_get(rt.new_string('customer_id'))).to_i64()),
	])
	...
	for {
		...
		mut var_download := this.prepare_item_for_response(rt.new_object('stdClass', []string{},
			rt.array_to_object(var_download_data)), var_request.clone())
		var_download = this.prepare_response_for_collection(var_download.clone())
		var_data.array_push(var_download.clone())
	}
...
fn (mut this Class_WC_REST_Customer_Downloads_V1_Controller) prepare_item_for_response(var_download rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	var_data.array_set('access_expires', if rt.is_true(var_data.array_get(rt.new_string('access_expires'))) { rt.call_function('wc_rest_prepare_date_response', [
			var_data.array_get(rt.new_string('access_expires')),
		]) } else { rt.new_string('never') })
	var_data.array_set('downloads_remaining', if rt.is_true(rt.identical(rt.new_string(''),
		var_data.array_get(rt.new_string('downloads_remaining'))))
	{
	...
	rt.call_method(var_response, 'add_links', [
		this.prepare_links(var_download_mutated.clone(), var_request.clone()),
	])
		...
		'product':    {
			'href': rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('/%s/products/%d'), this.namespace,
					rt.get_property(var_download_mutated, 'product_id')]),
			])
		}
		'order':      {
			'href': rt.call_function('rest_url', [
