...
fn (mut this Class_WC_REST_Order_Refunds_V2_Controller) construct() {
	rt.call_function('add_filter', [
		rt.concat(rt.concat(rt.new_string('woocommerce_rest_'), this.post_type),
			rt.new_string('_object_trashable')),
		...
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Order_Refunds_V2_Controller', [
...
fn (mut this Class_WC_REST_Order_Refunds_V2_Controller) get_object(var_id rt.PhpVal) rt.PhpVal {
	return rt.call_function('wc_get_order', [var_id.clone()])
}
...
fn (mut this Class_WC_REST_Order_Refunds_V2_Controller) get_formatted_item_data(var_object rt.PhpVal) rt.PhpVal {
	...
	for var_key in var_format_line_items {
		var_data.array_set(key, rt.call_function('array_values', [
	...
	return rt.create_array([
		rt.ArrayItem{ key: 'id', val: rt.call_method(var_object_mutated, 'get_id', []rt.PhpVal{}) },
		...
		rt.ArrayItem{ key: 'reason', val: var_data.array_get(rt.new_string('reason')) },
		rt.ArrayItem{ key: 'refunded_by', val: var_data.array_get(rt.new_string('refunded_by')) },
		rt.ArrayItem{
			key: 'refunded_payment'
			val: var_data.array_get(rt.new_string('refunded_payment'))
		},
		rt.ArrayItem{ key: 'meta_data', val: var_data.array_get(rt.new_string('meta_data')) },
		rt.ArrayItem{ key: 'line_items', val: var_data.array_get(rt.new_string('line_items')) },
		rt.ArrayItem{
			key: 'shipping_lines'
			val: var_data.array_get(rt.new_string('shipping_lines'))
		},
		rt.ArrayItem{ key: 'tax_lines', val: var_data.array_get(rt.new_string('tax_lines')) },
		rt.ArrayItem{ key: 'fee_lines', val: var_data.array_get(rt.new_string('fee_lines')) },
	])
}
...
fn (mut this Class_WC_REST_Order_Refunds_V2_Controller) prepare_object_for_response(var_object rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	this.request.array_set('dp', if this.request.array_get(rt.new_string('dp')).is_null() { rt.call_function('wc_get_price_decimals', []rt.PhpVal{}) } else { rt.call_function('absint', [
			this.request.array_get(rt.new_string('dp')),
		]) })
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_object_mutated))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_object_mutated, 'get_parent_id', []rt.PhpVal{}), rt.call_method(var_order, 'get_id', []rt.PhpVal{}))))) {
	...
	mut var_data := this.get_formatted_item_data(var_object_mutated.clone())
		...
		'self':       {
			'href': rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('/%s/%s/%d'), this.namespace,
					var_base.clone(), rt.call_method(var_object_mutated, 'get_id', []rt.PhpVal{})]),
			])
		}
		'collection': {
			'href': rt.call_function('rest_url', [
...
fn (mut this Class_WC_REST_Order_Refunds_V2_Controller) prepare_objects_query(var_request rt.PhpVal) rt.PhpVal {
	mut var_args :=
		this.Class_WC_REST_Orders_V2_Controller.prepare_objects_query(var_request.clone())
	var_args.array_set('post_status', rt.func_array_keys(rt.call_function('wc_get_order_statuses',
		[]rt.PhpVal{})))
	var_args.array_set('post_parent__in', rt.create_array([
...
fn (mut this Class_WC_REST_Order_Refunds_V2_Controller) prepare_object_for_database(var_request rt.PhpVal, creating bool) rt.PhpVal {
	...
	if !(!rt.is_true(var_request.array_get(rt.new_string('meta_data')))) {
		...
		mut iife_result_0 := iife_temp_0.update(var_request.array_get(rt.new_string('meta_data')),
			var_refund.clone())
		rt.call_method(var_refund, 'save_meta_data', []rt.PhpVal{})
	}
	var_refund = rt.call_function('apply_filters', [
	...
	closure_2_fn := fn (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
		mut var_thing := if args.len > 0 { args[0].clone() } else { rt.new_null() }
		return if rt.is_true(rt.call_function('is_wp_error', [
			var_thing.clone()]))
		{ var_thing } else { create_wp_error(rt.new_string('woocommerce_rest_cannot_verify_refund_created'), rt.call_function('__', [
				rt.new_string('An unexpected error occurred while generating the refund.'),
				rt.new_string('woocommerce'),
			])) }
	}
	mut iife_temp_2 := Class_Automattic_WooCommerce_Internal_Utilities_Types{}
	mut iife_result_2 := iife_temp_2.ensure_instance_of(var_refund.clone(), Class_WC_Data.class(),
...
fn (mut this Class_WC_REST_Order_Refunds_V2_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := {
		...
		'properties': {
			...
			'date_created_gmt': {
				'description': rt.call_function('__', [
					rt.new_string('The date the order refund was created, as GMT.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('date-time')
				'context':     map[string]rt.PhpVal{}
			...
			'tax_lines':        {
				'description': rt.call_function('__', [rt.new_string('Tax lines data.'),
					rt.new_string('woocommerce')])
				'type':        rt.new_string('array')
				'context':     map[string]rt.PhpVal{}
				'readonly':    rt.new_bool(true)
				'items':       {
					...
					'properties': {
						...
						'compound':           {
							'description': rt.call_function('__', [
								rt.new_string('Show if is a compound tax rate.'),
								rt.new_string('woocommerce'),
							])
							'type':        rt.new_string('boolean')
							'context':     map[string]rt.PhpVal{}
						...
						'tax_total':          {
							'description': rt.call_function('__', [
								rt.new_string('Tax total (not including shipping taxes).'),
								rt.new_string('woocommerce'),
							])
							'type':        rt.new_string('string')
							'context':     map[string]rt.PhpVal{}
						...
						'shipping_tax_total': {
							'description': rt.call_function('__', [
								rt.new_string('Shipping tax total.'),
								rt.new_string('woocommerce'),
							])
							'type':        rt.new_string('string')
							'context':     map[string]rt.PhpVal{}
			...
			'fee_lines':        {
				'description': rt.call_function('__', [rt.new_string('Fee lines data.'),
					rt.new_string('woocommerce')])
				'type':        rt.new_string('array')
				'context':     map[string]rt.PhpVal{}
				'items':       {
					...
					'properties': {
						...
						'tax_class':  {
							'description': rt.call_function('__', [
								rt.new_string('Tax class of fee.'),
								rt.new_string('woocommerce'),
							])
							'type':        rt.new_string('string')
							'context':     map[string]rt.PhpVal{}
						...
						'tax_status': {
							'description': rt.call_function('__', [
								rt.new_string('Tax status of fee.'),
								rt.new_string('woocommerce'),
							])
							'type':        rt.new_string('string')
							'context':     map[string]rt.PhpVal{}
			...
			'api_refund':       {
				'description': rt.call_function('__', [
					rt.new_string('When true, the payment gateway API is used to generate the refund.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('boolean')
				'context':     map[string]rt.PhpVal{}
...
fn create_automattic_woocommerce_internal_utilities_types(_args ...rt.PhpVal) &Class_Automattic_WooCommerce_Internal_Utilities_Types {
