	...
	rt.call_function('register_rest_route', [this.namespace, rt.new_string('/' +
		...
		rt.create_array([
			...
			rt.ArrayItem{ key: 'callback', val: rt.create_array([
				...
				rt.ArrayItem{ key: none, val: 'process_webhook' },
			]) },
			rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Paypal_Webhooks_Controller', [
					'WC_REST_Controller',
				], &this) },
				rt.ArrayItem{ key: none, val: 'validate_webhook' },
			]) },
		])])
}

fn (mut this Class_WC_REST_Paypal_Webhooks_Controller) validate_webhook(mut var_request Class_WP_REST_Request) bool {
	if rt.is_true(rt.call_function('class_exists', [rt.new_string('Automattic\\Jetpack\\Connection\\REST_Authentication')]))
		&& rt.is_true(rt.call_function('method_exists', [rt.new_string('Automattic\\Jetpack\\Connection\\REST_Authentication'), rt.new_string('is_signed_with_blog_token')])) {
		mut iife_temp_0 := Class_Automattic_Jetpack_Connection_REST_Authentication{}
		mut iife_result_0 := iife_temp_0.is_signed_with_blog_token()
	...
	if rt.instance_of(var_e_1, 'Throwable') {
		...
		mut iife_result_1 := iife_temp_1.log(rt.new_string(
			'REST authentication method not available. Webhook data: ' +(rt.call_function('wc_print_r', [var_request.get_json_params(), rt.new_bool(true)])).str()),
			rt.new_string('error'))
		return false
		unsafe {
			goto end_label_1
...
fn (mut this Class_WC_REST_Paypal_Webhooks_Controller) process_webhook(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
		rt.ArrayItem{ key: 'message', val: 'Webhook processed successfully' },
	]), rt.new_int(200)))
	unsafe {
		goto end_label_2
	}
	...
	if rt.instance_of(var_e_2, 'Throwable') {
		...
		mut iife_result_2 := iife_temp_2.log(rt.new_string('Error processing webhook: ' +
			(rt.call_method(var_e, 'getMessage', []rt.PhpVal{})).str()))
		return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
			rt.ArrayItem{ key: 'error', val: rt.call_function('__', [
				rt.new_string('Webhook processing failed.'),
...
fn create_wc_rest_paypal_webhooks_controller(_args ...rt.PhpVal) &Class_WC_REST_Paypal_Webhooks_Controller {
