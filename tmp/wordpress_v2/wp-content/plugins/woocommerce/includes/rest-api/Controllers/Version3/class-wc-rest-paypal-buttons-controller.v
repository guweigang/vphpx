...
fn (mut this Class_WC_REST_Paypal_Buttons_Controller) register_routes() {
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/create-order'),
		rt.create_array([
			rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.creatable() },
			rt.ArrayItem{ key: 'callback', val: rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Paypal_Buttons_Controller', [
					'WC_REST_Controller',
				], &this) },
			...
			rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
				...
				rt.ArrayItem{ key: none, val: 'validate_create_order_request' },
			]) },
		])])
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/cancel-payment'),
		rt.create_array([
			rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.creatable() },
			rt.ArrayItem{ key: 'callback', val: rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Paypal_Buttons_Controller', [
				...
				rt.ArrayItem{ key: none, val: 'cancel_payment' },
			]) },
			rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Paypal_Buttons_Controller', [
					'WC_REST_Controller',
				], &this) },
				rt.ArrayItem{ key: none, val: 'validate_cancel_payment_request' },
			]) },
		])])
}
...
fn (mut this Class_WC_REST_Paypal_Buttons_Controller) validate_create_order_request(mut var_request Class_WP_REST_Request) bool {
	if rt.is_true(var_request.get_header(rt.new_string('Nonce'))) {
		...
		return (rt.call_function('wp_verify_nonce', [var_nonce.clone(),
			rt.new_string('wc_gateway_paypal_standard_create_order')])).to_bool()
	}
	return false
}
...
fn (mut this Class_WC_REST_Paypal_Buttons_Controller) validate_cancel_payment_request(mut var_request Class_WP_REST_Request) bool {
	if rt.is_true(var_request.get_header(rt.new_string('Nonce'))) {
		mut var_nonce := var_request.get_header(rt.new_string('Nonce'))
		return (rt.call_function('wp_verify_nonce', [var_nonce.clone(),
			rt.new_string('wc_gateway_paypal_standard_cancel_payment')])).to_bool()
	}
	return false
}
...
fn (mut this Class_WC_REST_Paypal_Buttons_Controller) create_order(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	if !rt.is_true(var_data.array_get(rt.new_string('order_id')))
		|| !rt.is_true(var_data.array_get(rt.new_string('order_key'))) {
		return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
			rt.ArrayItem{ key: 'error', val: 'Invalid request' },
		]), rt.new_int(400)))
	}
	mut var_payment_source := if var_data.array_isset(rt.new_string('payment_source')) { rt.call_function('sanitize_text_field', [
			var_data.array_get(rt.new_string('payment_source')),
		]) } else { rt.new_string('') }
	if !rt.is_true(var_payment_source)
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_payment_source.clone(), Class_Automattic_WooCommerce_Gateways_PayPal_Constants.supported_payment_sources(), rt.new_bool(true)]))))) {
		return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
			rt.ArrayItem{ key: 'error', val: 'Missing/Invalid payment source: ' +
				(rt.call_function('esc_html', [var_payment_source.clone()])).str() },
		]), rt.new_int(400)))
	}
	mut var_order_id := var_data.array_get(rt.new_string('order_id'))
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order))))
		...
		return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
			rt.ArrayItem{ key: 'error', val: 'Order not found' },
		]), rt.new_int(404)))
	}
	mut var_order_key := var_data.array_get(rt.new_string('order_key'))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order_key))))
	...
	{
		return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
			rt.ArrayItem{ key: 'error', val: 'Invalid order status' },
		]), rt.new_int(409)))
	}
	...
	mut var_paypal_order := var_paypal_request.create_paypal_order(var_order.clone(),
		var_payment_source.clone(), rt.create_array([
		rt.ArrayItem{ key: 'is_js_sdk_flow', val: true },
		rt.ArrayItem{
			key: 'app_switch_request_origin'
			val: if !(var_data.array_get(rt.new_string('app_switch_request_origin'))).is_null() {
				var_data.array_get(rt.new_string('app_switch_request_origin'))
			} else {
				rt.new_string('')
			}
		},
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_paypal_order))))
		...
		return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
			rt.ArrayItem{ key: 'error', val: 'Failed to create PayPal order' },
		]), rt.new_int(400)))
	}
	rt.call_method(var_order, 'update_meta_data', [
		...
		var_paypal_order.array_get(rt.new_string('id')),
	])
	...
	return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
		rt.ArrayItem{
			key: 'paypal_order_id'
			val: if !(var_paypal_order.array_get(rt.new_string('id'))).is_null() {
				var_paypal_order.array_get(rt.new_string('id'))
			} else {
				rt.new_null()
			}
		...
		rt.ArrayItem{ key: 'return_url', val: rt.call_function('esc_url_raw', [
			rt.call_function('add_query_arg', [rt.new_string('utm_nooverride'),
				rt.new_string('1'), rt.call_method(var_gateway, 'get_return_url', [
					var_order.clone(),
				])]),
		]) },
	]), rt.new_int(200)))
...
fn (mut this Class_WC_REST_Paypal_Buttons_Controller) cancel_payment(mut var_request Class_WP_REST_Request) rt.PhpVal {
	mut var_data := var_request.get_json_params()
	mut var_order_id := if var_data.array_isset(rt.new_string('order_id')) { rt.call_function('absint', [
			var_data.array_get(rt.new_string('order_id')),
		]) } else { rt.new_int(0) }
	mut var_paypal_order_id := if var_data.array_isset(rt.new_string('paypal_order_id')) { rt.call_function('wc_clean', [
			var_data.array_get(rt.new_string('paypal_order_id')),
		]) } else { rt.new_string('') }
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order_id))))
		|| rt.is_true(rt.identical(rt.new_string(''), var_paypal_order_id)) {
		return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
			rt.ArrayItem{ key: 'error', val: 'Invalid request' },
		]), rt.new_int(400)))
	}
	...
	mut var_paypal_order_id_from_meta := rt.call_method(var_order, 'get_meta', [
		rt.new_string('_paypal_order_id'),
	])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_paypal_order_id_from_meta,
		var_paypal_order_id))))
	{
		return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
			rt.ArrayItem{ key: 'error', val: 'Invalid PayPal order' },
		]), rt.new_int(404)))
	}
	if rt.is_true(rt.call_method(var_order, 'has_status', [
		Class_Automattic_WooCommerce_Enums_OrderStatus.checkout_draft(),
	]))
	{
		return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
	...
	{
		return rt.new_object('WP_REST_Response', []string{}, create_wp_rest_response(rt.create_array([
			rt.ArrayItem{ key: 'error', val: 'Order is not pending' },
		]), rt.new_int(409)))
	}
	rt.call_method(var_order, 'update_status', [
		Class_Automattic_WooCommerce_Enums_OrderStatus.checkout_draft(),
...
fn create_wc_rest_paypal_buttons_controller(_args ...rt.PhpVal) &Class_WC_REST_Paypal_Buttons_Controller {
...
fn main() {
	...
	{
		rt.include_file(
			(rt.get_constant('WC_ABSPATH')).str() + 'includes/gateways/paypal/includes/class-wc-gateway-paypal-constants.php',
			'4')
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
