...
fn (mut this Class_WC_REST_Product_Categories_V1_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_data := rt.create_array([
		...
		rt.ArrayItem{ key: 'parent', val: rt.new_int((rt.get_property(var_item, 'parent')).to_i64()) },
		rt.ArrayItem{ key: 'description', val: rt.get_property(var_item, 'description') },
		rt.ArrayItem{
			key: 'display'
			val: if rt.is_true(var_display_type) {
				var_display_type
			} else {
				rt.new_string('default')
			}
		},
		rt.ArrayItem{ key: 'image', val: rt.new_null() },
	...
	if rt.is_true(var_image_id) {
		mut var_attachment := rt.call_function('get_post', [var_image_id.clone()])
		var_data.array_set('image', rt.create_array([
			rt.ArrayItem{ key: 'id', val: rt.new_int(var_image_id.to_i64()) },
	...
	return rt.call_function('apply_filters', [
		rt.concat(rt.new_string('woocommerce_rest_prepare_'), this.taxonomy),
		var_response.clone(),
		var_item.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_WC_REST_Product_Categories_V1_Controller) update_term_meta_fields(var_term rt.PhpVal, var_request rt.PhpVal) bool {
	...
	if var_request.array_isset(rt.new_string('display')) {
		rt.call_function('update_term_meta', [var_id.clone(),
			rt.new_string('display_type'), if rt.is_true(rt.identical(rt.new_string('default'),
				var_request.array_get(rt.new_string('display'))))
			{
				rt.new_string('')
			} else {
				var_request.array_get(rt.new_string('display'))
	...
	if var_request.array_isset(rt.new_string('image')) {
		if !rt.is_true(var_request.array_get(rt.new_string('image')).array_get(rt.new_string('id')))
			&& !(!rt.is_true(var_request.array_get(rt.new_string('image')).array_get(rt.new_string('src')))) {
			mut var_upload := rt.call_function('wc_rest_upload_image_from_url', [
				rt.call_function('esc_url_raw',
					[var_request.array_get(rt.new_string('image')).array_get(rt.new_string('src'))]),
			])
			if rt.is_true(rt.call_function('is_wp_error', [var_upload.clone()])) {
				return var_upload.to_bool()
			}
			mut var_image_id := rt.call_function('wc_rest_set_uploaded_image_as_attachment', [
				var_upload.clone(),
			])
		} else {
			var_image_id = if var_request.array_get(rt.new_string('image')).array_isset(rt.new_string('id')) { rt.call_function('absint', [
					var_request.array_get(rt.new_string('image')).array_get(rt.new_string('id')),
				]) } else { rt.new_int(0) }
		}
		if rt.is_true(var_image_id)
			&& rt.is_true(rt.call_function('wp_attachment_is_image', [var_image_id.clone()])) {
			rt.call_function('update_term_meta', [var_id.clone(),
				rt.new_string('thumbnail_id'), var_image_id.clone()])
			if !(!rt.is_true(var_request.array_get(rt.new_string('image')).array_get(rt.new_string('alt')))) {
				rt.call_function('update_post_meta', [var_image_id.clone(),
					rt.new_string('_wp_attachment_image_alt'),
					rt.call_function('wc_clean',
						[var_request.array_get(rt.new_string('image')).array_get(rt.new_string('alt'))])])
			}
			if !(!rt.is_true(var_request.array_get(rt.new_string('image')).array_get(rt.new_string('title')))) {
				rt.call_function('wp_update_post', [
					rt.create_array([rt.ArrayItem{ key: 'ID', val: var_image_id },
						rt.ArrayItem{ key: 'post_title', val: rt.call_function('wc_clean', [
							var_request.array_get(rt.new_string('image')).array_get(rt.new_string('title')),
						]) }]),
				])
			}
		} else {
			rt.call_function('delete_term_meta', [var_id.clone(),
				rt.new_string('thumbnail_id')])
		}
	}
	return true
...
fn (mut this Class_WC_REST_Product_Categories_V1_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := {
		...
		'properties': {
			...
			'name':        {
				'description': rt.call_function('__', [rt.new_string('Category name.'),
					rt.new_string('woocommerce')])
			...
			'parent':      {
				'description': rt.call_function('__', [
					rt.new_string('The ID for the parent of the resource.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('integer')
				'context':     map[string]rt.PhpVal{}
			...
			'display':     {
				'description': rt.call_function('__', [
					rt.new_string('Category archive display type.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'default':     rt.new_string('default')
				'enum':        map[string]rt.PhpVal{}
				'context':     map[string]rt.PhpVal{}
			}
			'image':       {
				'description': rt.call_function('__', [rt.new_string('Image data.'),
					rt.new_string('woocommerce')])
				'type':        rt.new_string('object')
				'context':     map[string]rt.PhpVal{}
				'properties':  {
...
fn create_wc_rest_product_categories_v1_controller(_args ...rt.PhpVal) &Class_WC_REST_Product_Categories_V1_Controller {
