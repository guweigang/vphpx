		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Product_Attributes_V1_Controller', [
						'WC_REST_Controller',
					], &this) },
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) create_item_permissions_check(var_request rt.PhpVal) bool {
	...
	])))))
	{
		return (create_wp_error(rt.new_string('woocommerce_rest_cannot_create'), rt.call_function('__', [
			rt.new_string('Sorry, you cannot create new resource.'),
			rt.new_string('woocommerce'),
		]), rt.create_array([
			rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code',
				[]rt.PhpVal{}) },
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) get_item_permissions_check(var_request rt.PhpVal) bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(this.get_taxonomy(var_request.clone()))))) {
		return (create_wp_error(rt.new_string('woocommerce_rest_taxonomy_invalid'), rt.call_function('__', [
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		mut var_attribute := this.prepare_item_for_response(var_attribute_obj.clone(),
			var_request.clone())
		var_attribute = this.prepare_response_for_collection(var_attribute.clone())
		var_data.array_push(var_attribute.clone())
	}
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) create_item(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_id := rt.call_function('wc_create_attribute', [
		rt.create_array([
			...
			rt.ArrayItem{ key: 'slug', val: rt.call_function('wc_sanitize_taxonomy_name', [
	...
	rt.call_method(var_response, 'header', [rt.new_string('Location'),
		rt.call_function('rest_url', [
			rt.new_string('/' +(this.namespace).str() + '/' + (this.rest_base).str() + '/' +
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	mut var_attribute :=
		this.get_attribute(rt.new_int((var_request.array_get(rt.new_string('id'))).to_i64()))
	if rt.is_true(rt.call_function('is_wp_error', [var_attribute.clone()])) {
		return var_attribute.clone()
	}
	mut var_response := this.prepare_item_for_response(var_attribute.clone(), var_request.clone())
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) update_item(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_edited := rt.call_function('wc_update_attribute', [
		var_id.clone(),
		rt.create_array([
			rt.ArrayItem{ key: 'name', val: var_request.array_get(rt.new_string('name')) },
			rt.ArrayItem{ key: 'slug', val: rt.call_function('wc_sanitize_taxonomy_name', [
			...
			rt.ArrayItem{ key: 'type', val: var_request.array_get(rt.new_string('type')) },
			rt.ArrayItem{ key: 'order_by', val: var_request.array_get(rt.new_string('order_by')) },
			rt.ArrayItem{
				key: 'has_archives'
				val: var_request.array_get(rt.new_string('has_archives'))
			},
		])])
	if rt.is_true(rt.call_function('is_wp_error', [var_edited.clone()])) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_cannot_edit'), rt.call_method(var_edited,
			'get_error_message', []rt.PhpVal{}), rt.create_array([
			rt.ArrayItem{ key: 'status', val: 400 },
		])))
	}
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) delete_item(var_request rt.PhpVal) rt.PhpVal {
	...
	rt.call_method(var_request, 'set_param', [rt.new_string('context'),
		rt.new_string('edit')])
	mut var_response := this.prepare_item_for_response(var_attribute.clone(), var_request.clone())
	mut var_deleted := rt.call_function('wc_delete_attribute', [
		rt.get_property(var_attribute, 'attribute_id'),
	])
	...
	rt.call_function('do_action', [
		rt.new_string('woocommerce_rest_delete_product_attribute'),
		var_attribute.clone(),
		var_response.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_data := rt.create_array([
		rt.ArrayItem{
			key: 'id'
			val: rt.new_int((rt.get_property(var_item, 'attribute_id')).to_i64())
		},
		rt.ArrayItem{ key: 'name', val: rt.get_property(var_item, 'attribute_label') },
		rt.ArrayItem{ key: 'slug', val: rt.call_function('wc_attribute_taxonomy_name', [
			rt.get_property(var_item, 'attribute_name'),
		]) },
		rt.ArrayItem{ key: 'type', val: rt.get_property(var_item, 'attribute_type') },
		rt.ArrayItem{ key: 'order_by', val: rt.get_property(var_item, 'attribute_orderby') },
		rt.ArrayItem{
			key: 'has_archives'
			val: (rt.get_property(var_item, 'attribute_public')).to_bool()
		},
	])
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_prepare_product_attribute'),
		var_response.clone(),
		var_item.clone(),
		var_request.clone(),
	])
		...
		'self':       {
			'href': rt.call_function('rest_url', [
				rt.new_string((rt.call_function('trailingslashit', [var_base.clone()])).str() +
					(rt.get_property(var_attribute_mutated, 'attribute_id')).str()),
			])
		}
		'collection': {
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := {
		...
		'properties': {
			...
			'name':         {
				'description': rt.call_function('__', [rt.new_string('Attribute name.'),
					rt.new_string('woocommerce')])
			...
			'type':         {
				'description': rt.call_function('__', [
					rt.new_string('Type of attribute.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'default':     rt.new_string('select')
				'enum':        rt.func_array_keys(rt.call_function('wc_get_attribute_types',
					[]rt.PhpVal{}))
				'context':     map[string]rt.PhpVal{}
			}
			'order_by':     {
				'description': rt.call_function('__', [
					rt.new_string('Default sort order.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'default':     rt.new_string('menu_order')
				'enum':        map[string]rt.PhpVal{}
				'context':     map[string]rt.PhpVal{}
			}
			'has_archives': {
				'description': rt.call_function('__', [
					rt.new_string('Enable/Disable attribute archives.'),
					rt.new_string('woocommerce'),
				])
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) get_taxonomy(var_request rt.PhpVal) string {
	mut var_attribute_id := var_request.array_get(rt.new_string('id'))
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) get_attribute(var_id rt.PhpVal) rt.PhpVal {
	...
	mut var_attribute := rt.call_method(var_wpdb, 'get_row', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.concat(rt.concat(rt.new_string('\n\t\t\tSELECT *\n\t\t\tFROM '), rt.get_property(var_wpdb,
				'prefix')),
				rt.new_string('woocommerce_attribute_taxonomies\n\t\t\tWHERE attribute_id = %d\n\t\t ')),
			var_id_mutated.clone(),
		]),
	])
	if rt.is_true(rt.call_function('is_wp_error', [var_attribute.clone()]))
		|| var_attribute.clone().is_null() {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_attribute_invalid'), rt.call_function('__', [
			rt.new_string('Resource does not exist.'),
			rt.new_string('woocommerce'),
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) validate_attribute_slug(var_slug rt.PhpVal, new_data bool) bool {
	if var_slug.clone().to_string().len > 28 {
		return (create_wp_error(rt.new_string('woocommerce_rest_invalid_product_attribute_slug_too_long'), rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('Slug "%s" is too long (28 characters max). Shorten it, please.'),
				rt.new_string('woocommerce'),
			]),
			var_slug.clone(),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))).to_bool()
	} else if rt.is_true(rt.call_function('wc_check_if_attribute_name_is_reserved', [
		var_slug.clone(),
	]))
	{
		return (create_wp_error(rt.new_string('woocommerce_rest_invalid_product_attribute_slug_reserved_name'), rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('Slug "%s" is not allowed because it is a reserved term. Change it, please.'),
				rt.new_string('woocommerce'),
			]),
			var_slug.clone(),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))).to_bool()
	} else if var_new_data
		&& rt.is_true(rt.call_function('taxonomy_exists', [rt.call_function('wc_attribute_taxonomy_name', [var_slug.clone()])])) {
		return (create_wp_error(rt.new_string('woocommerce_rest_invalid_product_attribute_slug_already_exists'), rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('Slug "%s" is already in use. Change it, please.'),
				rt.new_string('woocommerce'),
			]),
			var_slug.clone(),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))).to_bool()
...
fn (mut this Class_WC_REST_Product_Attributes_V1_Controller) flush_rewrite_rules() {
	rt.call_function('wp_schedule_single_event', [
		...
		rt.new_string('woocommerce_flush_rewrite_rules'),
	])
}
...
fn create_wc_rest_product_attributes_v1_controller(_args ...rt.PhpVal) &Class_WC_REST_Product_Attributes_V1_Controller {
