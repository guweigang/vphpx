...
fn (mut this Class_WC_REST_Webhook_Deliveries_V2_Controller) prepare_item_for_response(var_log rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_response := rt.call_function('rest_ensure_response', [
		var_data.clone()])
	rt.call_method(var_response, 'add_links', [this.prepare_links(var_log.clone())])
...
fn (mut this Class_WC_REST_Webhook_Deliveries_V2_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := {
		...
		'properties': {
			...
			'date_created_gmt': {
				'description': rt.call_function('__', [
					rt.new_string('The date the webhook delivery was logged, as GMT.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('date-time')
				'context':     map[string]rt.PhpVal{}
...
fn create_wc_rest_webhook_deliveries_v2_controller(_args ...rt.PhpVal) &Class_WC_REST_Webhook_Deliveries_V2_Controller {
