		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Reports_V1_Controller', [
						'WC_REST_Controller',
					], &this) },
...
fn (mut this Class_WC_REST_Reports_V1_Controller) get_reports() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{ key: none, val: rt.create_array([
			rt.ArrayItem{ key: 'slug', val: 'sales' },
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('List of sales reports.'),
				rt.new_string('woocommerce'),
			]) },
		]) },
		rt.ArrayItem{ key: none, val: rt.create_array([
			rt.ArrayItem{ key: 'slug', val: 'top_sellers' },
			rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
				rt.new_string('List of top sellers products.'),
				rt.new_string('woocommerce'),
			]) },
		]) },
	])
...
fn (mut this Class_WC_REST_Reports_V1_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_data := rt.create_array([
		...
		rt.ArrayItem{ key: 'description', val: rt.get_property(var_report, 'description') },
	])
			...
			rt.ArrayItem{ key: 'self', val: rt.create_array([
				rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
					rt.call_function('sprintf', [rt.new_string('/%s/%s/%s'), this.namespace,
						this.rest_base, rt.get_property(var_report, 'slug')]),
				]) },
			]) },
...
fn create_wc_rest_reports_v1_controller(_args ...rt.PhpVal) &Class_WC_REST_Reports_V1_Controller {
