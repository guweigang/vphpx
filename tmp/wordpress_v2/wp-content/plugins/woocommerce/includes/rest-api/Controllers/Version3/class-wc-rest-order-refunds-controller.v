...
fn (mut this Class_WC_REST_Order_Refunds_Controller) prepare_object_for_database(var_request rt.PhpVal, creating bool) rt.PhpVal {
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_RestApiParameterUtil{}
	mut iife_result_0 := iife_temp_0.adjust_create_refund_request_parameters(var_request.clone())
	...
	mut var_refund := rt.call_function('wc_create_refund', [
		rt.create_array([
			...
			rt.ArrayItem{ key: 'line_items', val: var_request.array_get(rt.new_string('line_items')) },
			rt.ArrayItem{
				key: 'refund_payment'
	...
	if !(!rt.is_true(var_request.array_get(rt.new_string('meta_data')))) {
		...
		mut iife_result_1 := iife_temp_1.update(var_request.array_get(rt.new_string('meta_data')),
			var_refund.clone())
		rt.call_method(var_refund, 'save_meta_data', []rt.PhpVal{})
	}
	return rt.call_function('apply_filters', [
		rt.concat(rt.concat(rt.new_string('woocommerce_rest_pre_insert_'), rt.get_property(rt.new_object('WC_REST_Order_Refunds_Controller', [
...
fn (mut this Class_WC_REST_Order_Refunds_Controller) get_formatted_item_data(var_data_object rt.PhpVal) rt.PhpVal {
	mut var_data :=
		this.Class_WC_REST_Order_Refunds_V2_Controller.get_formatted_item_data(var_data_object.clone())
	if rt.is_true(rt.new_bool(!(rt.is_true(this.cogs_is_enabled())))) {
		return var_data.clone()
	}
	if rt.is_true(rt.new_bool(rt.instance_of(var_data_object, 'WC_Abstract_Order')))
		&& rt.is_true(rt.call_method(var_data_object, 'has_cogs', []rt.PhpVal{})) {
		var_data.array_set('cost_of_goods_sold', rt.create_array([
			rt.ArrayItem{ key: 'value', val: rt.call_method(var_data_object,
		...
		for {
			...
			mut var_cogs_value := if !(var_line_item.array_get(rt.new_string('cogs_value'))).is_null() {
				var_line_item.array_get(rt.new_string('cogs_value'))
			} else {
				rt.new_null()
			}
			if !(var_cogs_value.clone().is_null()) {
				var_data.array_get_mut('line_items').array_get_mut(var_key).array_set('cost_of_goods_sold', rt.create_array([
					rt.ArrayItem{ key: 'value', val: var_cogs_value },
				]))
				var_data.array_get(rt.new_string('line_items')).array_get(var_key).array_unset(rt.new_string('cogs_value'))
			}
		}
	}
	return var_data.clone()
...
fn (mut this Class_WC_REST_Order_Refunds_Controller) get_item_schema() rt.PhpVal {
	...
	var_schema.array_get_mut('properties').array_get_mut('line_items').array_get_mut('items').array_get_mut('properties').array_set('refund_total', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Amount that will be refunded for this line item (excluding taxes).'),
			rt.new_string('woocommerce'),
		]) },
	...
	var_schema.array_get_mut('properties').array_get_mut('line_items').array_get_mut('items').array_get_mut('properties').array_get_mut('taxes').array_get_mut('items').array_get_mut('properties').array_set('refund_total', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Amount that will be refunded for this tax.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'number' },
		rt.ArrayItem{ key: 'context', val: rt.create_array([
...
fn (mut this Class_WC_REST_Order_Refunds_Controller) add_cogs_related_schema(mut var_schema Class_array) rt.PhpVal {
	...
	var_schema_mutated.array_get_mut('properties').array_get_mut('line_items').array_get_mut('items').array_get_mut('properties').array_set('cost_of_goods_sold', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Cost of Goods Sold data. Only present for product refund line items.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'context', val: rt.create_array([
		...
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			rt.ArrayItem{ key: 'total_value', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('Value of the Cost of Goods Sold for the refund item.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'number' },
				rt.ArrayItem{ key: 'readonly', val: true },
...
fn create_wc_rest_order_refunds_controller(_args ...rt.PhpVal) &Class_WC_REST_Order_Refunds_Controller {
...
fn create_wc_rest_order_refunds_v2_controller(_args ...rt.PhpVal) &Class_WC_REST_Order_Refunds_V2_Controller {
