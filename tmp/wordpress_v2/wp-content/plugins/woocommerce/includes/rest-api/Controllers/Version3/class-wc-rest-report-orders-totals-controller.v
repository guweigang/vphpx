...
fn (mut this Class_WC_REST_Report_Orders_Totals_Controller) get_reports() rt.PhpVal {
	...
	mut iife_result_0 := iife_temp_0.get_count_for_type(rt.new_string('shop_order'))
	mut var_totals := iife_result_0
	...
	for {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_totals.clone().array_isset(var_slug.clone())))))) {
			continue
		}
		var_data.array_push(rt.create_array([
			rt.ArrayItem{ key: 'slug', val: rt.call_function('str_replace', [
				rt.new_string('wc-'),
				rt.new_string(''),
				var_slug.clone(),
			]) },
			rt.ArrayItem{ key: 'name', val: var_name },
			rt.ArrayItem{ key: 'total', val: rt.new_int((var_totals.array_get(var_slug)).to_i64()) },
		]))
	}
...
fn (mut this Class_WC_REST_Report_Orders_Totals_Controller) prepare_item_for_response(var_report rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_prepare_report_orders_count'),
		var_response.clone(),
		var_report.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_WC_REST_Report_Orders_Totals_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := {
		...
		'title':      rt.new_string('report_order_total')
		...
		'properties': {
			...
			'total': {
				'description': rt.call_function('__', [
					rt.new_string('Amount of orders.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
...
fn create_wc_rest_report_orders_totals_controller(_args ...rt.PhpVal) &Class_WC_REST_Report_Orders_Totals_Controller {
