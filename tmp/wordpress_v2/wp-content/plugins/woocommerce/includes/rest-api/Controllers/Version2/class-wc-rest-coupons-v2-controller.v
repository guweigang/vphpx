		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Coupons_V2_Controller', [
...
fn (mut this Class_WC_REST_Coupons_V2_Controller) get_object(var_id rt.PhpVal) rt.PhpVal {
	...
	return rt.new_object('WC_Coupon', []string{}, create_wc_coupon(var_id_mutated.clone()))
}
...
fn (mut this Class_WC_REST_Coupons_V2_Controller) get_formatted_item_data(var_object rt.PhpVal) rt.PhpVal {
	...
	for var_key in var_format_decimal {
		var_data.array_set(key, rt.call_function('wc_format_decimal', [
			var_data.array_get(rt.new_string(key)),
			rt.new_int(2),
		]))
	}
	for var_key in var_format_date {
		mut var_datetime := var_data.array_get(rt.new_string(key))
	...
	for var_key in var_format_null {
		var_data.array_set(key, if rt.is_true(var_data.array_get(rt.new_string(key))) {
			var_data.array_get(rt.new_string(key))
		} else {
			rt.new_null()
		})
	...
	return rt.create_array([
		...
		rt.ArrayItem{ key: 'code', val: var_data.array_get(rt.new_string('code')) },
		rt.ArrayItem{ key: 'amount', val: var_data.array_get(rt.new_string('amount')) },
		...
		rt.ArrayItem{ key: 'discount_type', val: var_data.array_get(rt.new_string('discount_type')) },
		rt.ArrayItem{ key: 'description', val: var_data.array_get(rt.new_string('description')) },
		rt.ArrayItem{ key: 'date_expires', val: var_data.array_get(rt.new_string('date_expires')) },
		rt.ArrayItem{
			key: 'date_expires_gmt'
			val: var_data.array_get(rt.new_string('date_expires_gmt'))
		},
		rt.ArrayItem{ key: 'usage_count', val: var_data.array_get(rt.new_string('usage_count')) },
		rt.ArrayItem{
			key: 'individual_use'
			val: var_data.array_get(rt.new_string('individual_use'))
		},
		rt.ArrayItem{ key: 'product_ids', val: var_data.array_get(rt.new_string('product_ids')) },
		rt.ArrayItem{
			key: 'excluded_product_ids'
			val: var_data.array_get(rt.new_string('excluded_product_ids'))
		},
		rt.ArrayItem{ key: 'usage_limit', val: var_data.array_get(rt.new_string('usage_limit')) },
		rt.ArrayItem{
			key: 'usage_limit_per_user'
			val: var_data.array_get(rt.new_string('usage_limit_per_user'))
		},
		rt.ArrayItem{
			key: 'limit_usage_to_x_items'
			val: var_data.array_get(rt.new_string('limit_usage_to_x_items'))
		},
		rt.ArrayItem{ key: 'free_shipping', val: var_data.array_get(rt.new_string('free_shipping')) },
		rt.ArrayItem{
			key: 'product_categories'
			val: var_data.array_get(rt.new_string('product_categories'))
		},
		rt.ArrayItem{
			key: 'excluded_product_categories'
			val: var_data.array_get(rt.new_string('excluded_product_categories'))
		},
		rt.ArrayItem{
			key: 'exclude_sale_items'
			val: var_data.array_get(rt.new_string('exclude_sale_items'))
		},
		rt.ArrayItem{
			key: 'minimum_amount'
			val: var_data.array_get(rt.new_string('minimum_amount'))
		},
		rt.ArrayItem{
			key: 'maximum_amount'
			val: var_data.array_get(rt.new_string('maximum_amount'))
		},
		rt.ArrayItem{
			key: 'email_restrictions'
			val: var_data.array_get(rt.new_string('email_restrictions'))
		},
		rt.ArrayItem{ key: 'used_by', val: var_data.array_get(rt.new_string('used_by')) },
...
fn (mut this Class_WC_REST_Coupons_V2_Controller) prepare_object_for_response(var_object rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_data := this.get_formatted_item_data(var_object.clone())
	...
	rt.call_method(var_response, 'add_links', [
		this.prepare_links(var_object.clone(), var_request.clone()),
	])
	return rt.call_function('apply_filters', [
		rt.concat(rt.concat(rt.new_string('woocommerce_rest_prepare_'), this.post_type),
			rt.new_string('_object')),
		var_response.clone(),
		var_object.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_WC_REST_Coupons_V2_Controller) prepare_objects_query(var_request rt.PhpVal) rt.PhpVal {
	mut var_args := this.Class_WC_REST_CRUD_Controller.prepare_objects_query(var_request.clone())
	mut var_coupon_code := if !(var_request.array_get(rt.new_string('code'))).is_null() {
		var_request.array_get(rt.new_string('code'))
	} else {
		rt.new_null()
	}
...
fn (mut this Class_WC_REST_Coupons_V2_Controller) prepare_object_for_database(var_request rt.PhpVal, creating bool) rt.PhpVal {
	mut var_id := if var_request.array_isset(rt.new_string('id')) { rt.call_function('absint', [
	...
	mut iife_result_1 := iife_temp_1.is_null_or_whitespace(if !(var_request.array_get(rt.new_string('code'))).is_null() {
		var_request.array_get(rt.new_string('code'))
	} else {
		rt.new_null()
	})
	if var_creating && rt.is_true(iife_result_1) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_empty_coupon_code'), rt.call_function('sprintf', [
			rt.call_function('__', [rt.new_string('The coupon code cannot be empty.'),
				rt.new_string('woocommerce')]),
			rt.new_string('code'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }])))
	}
	if var_request.array_isset(rt.new_string('discount_type')) {
		var_coupon.set_discount_type(var_request.array_get(rt.new_string('discount_type')))
	}
	...
	for {
		...
		if !(var_value.clone().is_null()) {
			...
			if rt.is_true(rt.equal(switch_val_1, rt.new_string('code'))) {
				...
				var_id = if rt.is_true(var_coupon.get_id()) {
					var_coupon.get_id()
				} else {
					rt.new_int(0)
				}
				...
				var_coupon.set_code(var_coupon_code.clone())
				...
				mut iife_result_2 := iife_temp_2.update(var_value.clone(), rt.new_object('WC_Coupon',
					[]string{}, var_coupon))
			} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('description'))) {
				var_coupon.set_description(rt.call_function('wp_filter_post_kses', [
					var_value.clone(),
				]))
			} else {
	...
	return rt.call_function('apply_filters', [
		rt.concat(rt.concat(rt.new_string('woocommerce_rest_pre_insert_'), this.post_type),
			rt.new_string('_object')),
		var_coupon,
		var_request.clone(),
	...
	mut var_schema := rt.create_array([
		...
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'status', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('The status of the coupon. Should always be draft, published, or pending review'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'date_created_gmt', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('The date the coupon was created, as GMT.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'date-time' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'date_modified_gmt', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('The date the coupon was last modified, as GMT.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'date-time' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'date_expires', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string("The date the coupon expires, in the site's timezone."),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'date-time' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'date_expires_gmt', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('The date the coupon expires, as GMT.'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'date-time' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'items', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'properties', val: rt.create_array([
						...
						rt.ArrayItem{ key: 'value', val: rt.create_array([
							...
							rt.ArrayItem{ key: 'type', val: 'mixed' },
							rt.ArrayItem{ key: 'context', val: rt.create_array([
								rt.ArrayItem{ key: none, val: 'view' },
								rt.ArrayItem{ key: none, val: 'edit' },
							]) },
...
fn create_wc_rest_coupons_v2_controller(_args ...rt.PhpVal) &Class_WC_REST_Coupons_V2_Controller {
