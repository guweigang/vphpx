...
fn (mut this Class_WC_REST_Webhooks_V2_Controller) prepare_item_for_response(var_id rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	if !rt.is_true(var_webhook) || var_webhook.clone().is_null() {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.concat(rt.concat(rt.new_string('woocommerce_rest_'), rt.get_property(rt.new_object('WC_REST_Webhooks_V2_Controller', [
	...
	return rt.call_function('apply_filters', [
		rt.concat(rt.new_string('woocommerce_rest_prepare_'), rt.get_property(rt.new_object('WC_REST_Webhooks_V2_Controller', [
			'WC_REST_Webhooks_V1_Controller',
		], &this), 'post_type')),
		var_response.clone(),
		var_webhook.clone(),
...
fn (mut this Class_WC_REST_Webhooks_V2_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := {
		...
		'properties': {
			...
			'date_created_gmt':  {
				'description': rt.call_function('__', [
					rt.new_string('The date the webhook was created, as GMT.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('date-time')
				'context':     map[string]rt.PhpVal{}
			...
			'date_modified_gmt': {
				'description': rt.call_function('__', [
					rt.new_string('The date the webhook was last modified, as GMT.'),
					rt.new_string('woocommerce'),
				])
				'type':        rt.new_string('date-time')
				'context':     map[string]rt.PhpVal{}
