...
fn (mut this Class_WC_REST_Taxes_Controller) add_tax_rate_locales(var_data rt.PhpVal, var_tax rt.PhpVal) rt.PhpVal {
	...
	var_data_mutated = this.Class_WC_REST_Taxes_V2_Controller.add_tax_rate_locales(var_data_mutated.clone(),
		var_tax.clone())
	var_data_mutated.array_set('postcodes', rt.new_array())
	var_data_mutated.array_set('cities', rt.new_array())
	mut var_locales := rt.call_method(var_wpdb, 'get_results', [
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_wp_error', [var_tax.clone()])))))
		&& !(var_tax.clone().is_null()) {
		mut iter_1 := var_locales.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_locale := item_1.val
			if rt.is_true(rt.identical(rt.new_string('postcode'), rt.get_property(var_locale,
				'location_type')))
			{
				var_data_mutated.array_get_mut('postcodes').array_push(rt.get_property(var_locale,
					'location_code'))
			} else if rt.is_true(rt.identical(rt.new_string('city'), rt.get_property(var_locale,
				'location_type')))
			{
				var_data_mutated.array_get_mut('cities').array_push(rt.get_property(var_locale,
					'location_code'))
			}
		}
	}
...
fn (mut this Class_WC_REST_Taxes_Controller) get_item_schema() rt.PhpVal {
	...
	var_schema.array_get_mut('properties').array_set('postcodes', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('List of postcodes / ZIPs. Introduced in WooCommerce 5.3.'),
	...
	var_schema.array_get_mut('properties').array_set('cities', rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('List of city names. Introduced in WooCommerce 5.3.'),
			rt.new_string('woocommerce'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'array' },
		rt.ArrayItem{ key: 'items', val: rt.create_array([
	...
	var_schema.array_get_mut('properties').array_get_mut('postcode').array_set('description', rt.call_function('__', [
		rt.new_string("Postcode/ZIP, it doesn't support multiple values. Deprecated as of WooCommerce 5.3, 'postcodes' should be used instead."),
		rt.new_string('woocommerce'),
	]))
	var_schema.array_get_mut('properties').array_get_mut('city').array_set('description', rt.call_function('__', [
		rt.new_string("City name, it doesn't support multiple values. Deprecated as of WooCommerce 5.3, 'cities' should be used instead."),
		rt.new_string('woocommerce'),
	]))
...
fn (mut this Class_WC_REST_Taxes_Controller) create_item(var_request rt.PhpVal) rt.PhpVal {
	...
	this.adjust_cities_and_postcodes(var_request_mutated.clone())
	return this.Class_WC_REST_Taxes_V2_Controller.create_item(var_request_mutated.clone())
}

fn (mut this Class_WC_REST_Taxes_Controller) update_item(var_request rt.PhpVal) rt.PhpVal {
	mut var_request_mutated := var_request
	this.adjust_cities_and_postcodes(var_request_mutated.clone())
	return this.Class_WC_REST_Taxes_V2_Controller.update_item(var_request_mutated.clone())
}
...
fn (mut this Class_WC_REST_Taxes_Controller) adjust_cities_and_postcodes(var_request rt.PhpVal) {
	...
	if var_request_mutated.array_isset(rt.new_string('cities')) {
		var_request_mutated.array_set('city', rt.call_function('join', [
			rt.new_string(';'),
			var_request_mutated.array_get(rt.new_string('cities')),
		]))
	}
	if var_request_mutated.array_isset(rt.new_string('postcodes')) {
		var_request_mutated.array_set('postcode', rt.call_function('join', [
			rt.new_string(';'),
			var_request_mutated.array_get(rt.new_string('postcodes')),
		]))
	}
}
...
fn create_wc_rest_taxes_controller(_args ...rt.PhpVal) &Class_WC_REST_Taxes_Controller {
...
fn create_wc_rest_taxes_v2_controller(_args ...rt.PhpVal) &Class_WC_REST_Taxes_V2_Controller {
