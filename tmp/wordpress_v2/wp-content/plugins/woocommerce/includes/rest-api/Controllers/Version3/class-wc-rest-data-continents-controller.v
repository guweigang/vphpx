...
fn (mut this Class_WC_REST_Data_Continents_Controller) get_continent(var_continent_code rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_continents.clone().array_isset(var_continent_code_mutated.clone())))))) {
		return rt.new_bool(false)
	}
	mut var_continent_list := var_continents.array_get(var_continent_code_mutated)
	mut var_continent := rt.create_array([rt.ArrayItem{ key: 'code', val: var_continent_code_mutated }, rt.ArrayItem{ key: 'name', val: var_continent_list.array_get(rt.new_string('name')) }])
	mut var_local_countries := rt.new_array()
	mut iter_1 := var_continent_list.array_get(rt.new_string('countries')).iterator()
	for {
		...
		if var_countries.array_isset(var_country_code) {
			...
			if rt.is_true(rt.new_bool(var_locale_info.clone().array_isset(var_country_code.clone()))) {
			...
			var_country = rt.call_function('array_merge', [var_country.clone(), var_country_data.clone()])
			}
			...
			var_country.array_set('states', var_local_states.clone())
			...
			var_country = rt.call_function('array_intersect_key', [var_country.clone(), rt.call_function('array_flip', [rt.create_array_from_list(var_allowed)])])
			var_local_countries << var_country.clone()
		}
	}
	var_continent.array_set('countries', var_local_countries.clone())
	return var_continent.clone()
}

fn (mut this Class_WC_REST_Data_Continents_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	mut var_continents := rt.call_method(rt.get_property(rt.call_function('WC', []rt.PhpVal{}), 'countries'), 'get_continents', []rt.PhpVal{})
	mut var_data := rt.new_array()
	mut iter_3 := rt.func_array_keys(var_continents.clone()).iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_continent_code := item_3.val
		mut var_continent := this.get_continent(var_continent_code.clone(), var_request.clone())
		mut var_response := this.prepare_item_for_response(var_continent.clone(), var_request.clone())
...
fn (mut this Class_WC_REST_Data_Continents_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	mut var_data := this.get_continent(rt.new_string((var_request.array_get(rt.new_string('location')).to_string().to_upper()).str()), var_request.clone())
	if !rt.is_true(var_data) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_data_invalid_location'), rt.call_function('__', [rt.new_string('There are no locations matching these parameters.'), rt.new_string('woocommerce')]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }])))
	}
...
fn (mut this Class_WC_REST_Data_Continents_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('woocommerce_rest_prepare_data_continent'), var_response.clone(), var_item.clone(), var_request.clone()])
}

fn (mut this Class_WC_REST_Data_Continents_Controller) prepare_links(var_item rt.PhpVal) rt.PhpVal {
	mut var_continent_code := rt.new_string(var_item.array_get(rt.new_string('code')).to_string().to_lower())
	mut var_links := { 'self': { 'href': rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s/%s'), this.namespace, this.rest_base, var_continent_code.clone()])]) }, 'collection': { 'href': rt.call_function('rest_url', [rt.call_function('sprintf', [rt.new_string('/%s/%s'), this.namespace, this.rest_base])]) } }
