		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Refunds_Controller', [
...
fn (mut this Class_WC_REST_Refunds_Controller) prepare_objects_query(var_request rt.PhpVal) rt.PhpVal {
	mut var_args :=
		this.Class_WC_REST_Order_Refunds_Controller.prepare_objects_query(var_request.clone())
	var_args.array_unset(rt.new_string('post_parent__in'))
	var_args = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_rest_refunds_prepare_object_query'),
		var_args.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_WC_REST_Refunds_Controller) prepare_object_for_response(var_refund rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	this.dispatch_set_prop('request', var_request.clone())
	rt.get_property(rt.new_object('WC_REST_Refunds_Controller', [
		'WC_REST_Order_Refunds_Controller',
	], &this), 'request').array_set('dp', if rt.get_property(rt.new_object('WC_REST_Refunds_Controller', [
		'WC_REST_Order_Refunds_Controller',
	], &this), 'request').array_get(rt.new_string('dp')).is_null()
	{ rt.call_function('wc_get_price_decimals', []rt.PhpVal{}) } else { rt.call_function('absint', [
			rt.get_property(rt.new_object('WC_REST_Refunds_Controller', [
				'WC_REST_Order_Refunds_Controller',
			], &this), 'request').array_get(rt.new_string('dp')),
		]) })
	mut var_data := this.get_formatted_item_data(var_refund.clone())
...
fn (mut this Class_WC_REST_Refunds_Controller) get_formatted_item_data(var_refund rt.PhpVal) rt.PhpVal {
	mut var_data :=
		this.Class_WC_REST_Order_Refunds_Controller.get_formatted_item_data(var_refund.clone())
	var_data = rt.call_function('array_merge', [
		rt.call_function('array_slice', [var_data.clone(), rt.new_int(0),
			rt.new_int(1), rt.new_bool(true)]),
		rt.create_array([rt.ArrayItem{ key: 'parent_id', val: rt.call_method(var_refund,
			'get_parent_id', []rt.PhpVal{}) }]),
		rt.call_function('array_slice', [var_data.clone(), rt.new_int(1),
...
fn (mut this Class_WC_REST_Refunds_Controller) prepare_links(var_refund rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_base := rt.call_function('str_replace', [
		...
		rt.new_string('orders/(?P<order_id>[\\d]+)/refunds'),
	])
	mut var_links := {
		'self':       {
			'href': rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('/%s/%s/%d'), this.namespace,
					var_base.clone(), rt.call_method(var_refund, 'get_id', []rt.PhpVal{})]),
			])
		}
		'collection': {
			'href': rt.call_function('rest_url', [
...
fn (mut this Class_WC_REST_Refunds_Controller) get_item_schema() rt.PhpVal {
	...
	var_schema.array_set('properties', rt.call_function('array_merge', [
		rt.call_function('array_slice', [var_schema.array_get(rt.new_string('properties')),
			rt.new_int(0), rt.new_int(1), rt.new_bool(true)]),
		rt.create_array([rt.ArrayItem{ key: 'parent_id', val: rt.create_array([
		...
		rt.call_function('array_slice', [var_schema.array_get(rt.new_string('properties')),
			rt.new_int(1), rt.new_null(), rt.new_bool(true)]),
	]))
...
fn create_wc_rest_refunds_controller(_args ...rt.PhpVal) &Class_WC_REST_Refunds_Controller {
