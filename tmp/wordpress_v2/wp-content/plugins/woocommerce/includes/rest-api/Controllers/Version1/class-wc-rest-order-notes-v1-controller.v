		...
		rt.create_array([
			...
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WC_REST_Order_Notes_V1_Controller', [
						'WC_REST_Controller',
					], &this) },
...
fn (mut this Class_WC_REST_Order_Notes_V1_Controller) get_item_permissions_check(var_request rt.PhpVal) bool {
	...
	if rt.is_true(var_order)
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wc_rest_check_post_permissions', [this.post_type, rt.new_string('read'), rt.call_method(var_order, 'get_id', []rt.PhpVal{})]))))) {
		return (create_wp_error(rt.new_string('woocommerce_rest_cannot_view'), rt.call_function('__', [
...
fn (mut this Class_WC_REST_Order_Notes_V1_Controller) delete_item_permissions_check(var_request rt.PhpVal) bool {
	...
	if rt.is_true(var_order)
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wc_rest_check_post_permissions', [this.post_type, rt.new_string('delete'), rt.call_method(var_order, 'get_id', []rt.PhpVal{})]))))) {
...
fn (mut this Class_WC_REST_Order_Notes_V1_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(this.post_type, rt.call_method(var_order, 'get_type', []rt.PhpVal{}))))) {
		...
		rt.call_function('rest_url', [
			rt.call_function('sprintf', [rt.new_string('/%s/%s/%d'), this.namespace,
				rt.call_function('str_replace', [rt.new_string('(?P<order_id>[\\d]+)'),
					rt.call_method(var_order, 'get_id', []rt.PhpVal{}), this.rest_base]),
				var_note_id.clone()]),
		])])
...
fn (mut this Class_WC_REST_Order_Notes_V1_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_note := rt.call_function('get_comment', [var_id.clone()])
	if !rt.is_true(var_id) || !rt.is_true(var_note)
		|| rt.is_true(rt.new_bool(rt.get_property(var_note, 'comment_post_ID').to_i64() != rt.call_method(var_order, 'get_id', []rt.PhpVal{}).to_i64())) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_result)))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('woocommerce_rest_cannot_delete'), rt.call_function('sprintf', [
			rt.call_function('__', [rt.new_string('The %s cannot be deleted.'),
				rt.new_string('woocommerce')]),
			rt.new_string('order_note'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 500 }])))
	}
	rt.call_function('do_action', [rt.new_string('woocommerce_rest_delete_order_note'),
		var_note.clone(), var_response.clone(), var_request.clone()])
	return var_response.clone()
}
...
fn (mut this Class_WC_REST_Order_Notes_V1_Controller) prepare_links(var_note rt.PhpVal) rt.PhpVal {
	mut var_note_mutated := var_note
	mut var_order_id := rt.new_int((rt.get_property(var_note_mutated, 'comment_post_ID')).to_i64())
	...
	mut var_links := {
		'self':       {
			'href': rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('/%s/%s/%d'), this.namespace,
					var_base.clone(), rt.get_property(var_note_mutated, 'comment_ID')]),
			])
		}
		'collection': {
			'href': rt.call_function('rest_url', [
...
fn (mut this Class_WC_REST_Order_Notes_V1_Controller) get_item_schema() rt.PhpVal {
	mut var_schema := {
		...
		'properties': {
			...
			'note':          {
				'description': rt.call_function('__', [rt.new_string('Order note.'),
					rt.new_string('woocommerce')])
			...
			'customer_note': {
				'description': rt.call_function('__', [
					rt.new_string('Shows/define if the note is only for reference or for the customer (the user will be notified).'),
					rt.new_string('woocommerce'),
				])
