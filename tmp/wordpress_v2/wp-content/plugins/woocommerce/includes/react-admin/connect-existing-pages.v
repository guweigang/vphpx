...
fn wc_admin_get_core_pages_to_connect() rt.PhpVal {
	mut var_all_reports := rt.new_null()
	mut var_report_tabs := rt.new_null()
	mut var_report_data := map[string]rt.PhpVal{}
	mut var_report_id := rt.new_null()
	mut iife_temp_0 := Class_WC_Admin_Reports{}
	mut iife_result_0 := iife_temp_0.get_reports()
	var_all_reports = iife_result_0
	var_report_tabs = rt.new_array()
	mut iter_1 := var_all_reports.iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_report_data_shadow := item_1.val
		mut var_report_id_shadow := item_1.key
		var_report_tabs.array_set(var_report_id_shadow, var_report_data_shadow['title'])
	}
...
fn wc_admin_filter_core_page_breadcrumbs(var_breadcrumbs rt.PhpVal) rt.PhpVal {
	...
	mut var_pages_to_connect := rt.new_null()
	mut var_woocommerce_breadcrumb := []rt.PhpVal{}
	mut var_page_data := map[string]rt.PhpVal{}
	mut var_page_id := rt.new_null()
	mut var_new_breadcrumbs := []rt.PhpVal{}
	...
	var_screen_id = rt.call_method(iife_result_1, 'get_current_screen_id', []rt.PhpVal{})
	var_pages_to_connect = wc_admin_get_core_pages_to_connect()
	var_woocommerce_breadcrumb = [rt.new_string('admin.php?page=wc-admin'),
		rt.call_function('__', [rt.new_string('WooCommerce'),
			rt.new_string('woocommerce')])]
	mut iter_2 := var_pages_to_connect.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_page_data_shadow := item_2.val
		mut var_page_id_shadow := item_2.key
		if rt.is_true(rt.call_function('preg_match', [
			rt.new_string('/^woocommerce_page_${var_page_id.to_string()}\\-/'),
			var_screen_id.clone(),
		]))
		{
			if !rt.is_true(var_page_data_shadow['tabs']) {
				var_new_breadcrumbs = [var_woocommerce_breadcrumb, var_page_data_shadow['title']]
			} else {
				var_new_breadcrumbs = [var_woocommerce_breadcrumb,
					[
						rt.call_function('add_query_arg', [rt.new_string('page'),
							var_page_id_shadow.clone(), rt.new_string('admin.php')]),
						var_page_data_shadow['title'],
					]]
				if rt.get_superglobal('_GET').array_isset(rt.new_string('tab')) {
					var_current_tab = rt.call_function('wc_clean', [
						rt.call_function('wp_unslash', [
							rt.get_superglobal('_GET').array_get(rt.new_string('tab')),
						]),
					])
				} else {
					var_current_tab = rt.call_function('key', [var_page_data_shadow['tabs']])
				}
				var_new_breadcrumbs << var_page_data_shadow['tabs'].array_get(var_current_tab)
			}
			return var_new_breadcrumbs.clone()
		}
	}
...
fn wc_admin_connect_core_pages(var_is_connected rt.PhpVal, var_current_page rt.PhpVal) bool {
	...
	if rt.is_true(rt.identical(rt.new_bool(false), var_is_connected))
		...
		var_screen_id = rt.call_method(iife_result_2, 'get_current_screen_id', []rt.PhpVal{})
		var_pages_to_connect = wc_admin_get_core_pages_to_connect()
		mut iter_3 := var_pages_to_connect.iterator()
		for {
			item_3 := iter_3.next() or { break }
			mut var_page_data_shadow := item_3.val
			mut var_page_id_shadow := item_3.key
			if rt.is_true(rt.call_function('preg_match', [
				rt.new_string('/^woocommerce_page_${var_page_id.to_string()}\\-/'),
				var_screen_id.clone(),
			]))
...
fn main() {
	...
	rt.call_function('wc_admin_connect_page', [
		rt.create_array([rt.ArrayItem{ key: 'id', val: 'woocommerce-orders' },
			...
			rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
				...
				rt.new_string(var_posttype_list_base.str()).clone(),
			]) }]),
	])
	rt.call_function('wc_admin_connect_page', [
	...
	mut iife_result_4 := iife_temp_4.is_enabled(rt.new_string('coupons'))
	rt.call_function('wc_admin_connect_page', [
		rt.create_array([rt.ArrayItem{ key: 'id', val: 'woocommerce-coupons' },
			rt.ArrayItem{
				key: 'parent'
				val: if rt.is_true(iife_result_4) {
