...
fn wc_admin_number_format(var_number rt.PhpVal) rt.PhpVal {
	mut var_currency_settings := rt.new_null()
	mut iife_temp_0 := Class_Automattic_WooCommerce_Internal_Admin_Settings{}
	mut iife_result_0 := iife_temp_0.get_currency_settings()
	var_currency_settings = iife_result_0
	return rt.call_function('number_format', [var_number.clone(),
		rt.new_int(0), var_currency_settings.array_get(rt.new_string('decimalSeparator')),
		var_currency_settings.array_get(rt.new_string('thousandSeparator'))])
}

fn wc_admin_url(var_path_arg rt.PhpVal, var_query rt.PhpVal) rt.PhpVal {
	mut var_path := var_path_arg
	mut var_query_string := rt.new_null()
	if !(!rt.is_true(var_query)) {
		var_query_string = rt.call_function('http_build_query', [
			var_query.clone()])
	...
	return rt.call_function('admin_url', [
		rt.new_string('admin.php?page=wc-admin' + var_path.str()),
...
fn wc_admin_record_tracks_event(var_event_name rt.PhpVal, var_properties rt.PhpVal) {
	...
	])))))
	{
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('WC_ABSPATH')])))))
			|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('file_exists', [rt.new_string((rt.get_constant('WC_ABSPATH')).str() + 'includes/tracks/class-wc-tracks.php')]))))) {
			return
		}
		rt.include_file(
	...
	mut iife_result_1 := iife_temp_1.record_event(var_event_name.clone(), var_properties.clone())
}
