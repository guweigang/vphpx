...
fn Class_WC_Product_Usage.get_rules_for_product(product_id i64) rt.PhpVal {
	...
	mut iife_result_0 := iife_temp_0.has_product_subscription(rt.new_int(product_id))
	if rt.is_true(rt.new_bool(!(rt.is_true(iife_result_0)))) {
		return create_wc_product_usage_rule_set(var_rules.clone())
	}
	mut iife_temp_1 := Class_WC_Helper{}
	mut iife_result_1 := iife_temp_1.get_installed_subscriptions()
	mut iife_temp_2 := Class_WC_Helper{}
	mut iife_result_2 := iife_temp_2.get_installed_subscriptions()
	mut var_subscriptions := rt.call_function('wp_list_filter', [iife_result_1,
	...
	mut var_product_subscription := rt.call_function('current', [
		var_subscriptions.clone()])
	if rt.is_true(var_product_subscription.array_get(rt.new_string('expired'))) {
		return create_wc_product_usage_rule_set(var_rules.clone())
	}
	return rt.new_null()
}
...
fn Class_WC_Product_Usage.get_product_usage_restriction_rule(product_id i64) rt.PhpVal {
	...
	mut iife_result_3 := iife_temp_3.get_product_usage_notice_rules()
	mut var_rules := iife_result_3
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	end_label_1:
	if !rt.is_true(var_rules.array_get(rt.new_string('restricted_products')).array_get(rt.new_int(product_id))) {
		return rt.new_null()
	}
	return var_rules.array_get(rt.new_string('restricted_products')).array_get(rt.new_int(product_id))
}
...
fn create_wc_product_usage(_args ...rt.PhpVal) &Class_WC_Product_Usage {
...
fn create_wc_product_usage_rule_set(_args ...rt.PhpVal) &Class_WC_Product_Usage_Rule_Set {
