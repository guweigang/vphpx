...
fn (mut this Class_WC_Background_Updater) dispatch() {
	...
	if rt.is_true(rt.call_function('is_wp_error', [var_dispatched.clone()])) {
		rt.call_method(var_logger, 'error', [
			rt.call_function('sprintf', [
				rt.new_string('Unable to dispatch WooCommerce updater: %s'),
				rt.call_method(var_dispatched, 'get_error_message', []rt.PhpVal{}),
			]),
			rt.create_array([
				rt.ArrayItem{ key: 'source', val: 'wc_db_updates' },
			]),
		])
	}
}
...
fn (mut this Class_WC_Background_Updater) schedule_event() {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_next_scheduled', [
		rt.get_property(rt.new_object('WC_Background_Updater', [
...
fn (mut this Class_WC_Background_Updater) task(var_callback rt.PhpVal) rt.PhpVal {
	rt.call_function('wc_maybe_define_constant', [rt.new_string('WC_UPDATING'),
		rt.new_bool(true)])
	mut var_logger := rt.call_function('wc_get_logger', []rt.PhpVal{})
	rt.include_file((rt.call_function('dirname', [rt.new_string(@FILE)])).str() +
		'/wc-update-functions.php', '2')
	...
	if rt.is_true(rt.call_function('is_callable', [var_callback.clone()])) {
		rt.call_method(var_logger, 'info', [
			rt.call_function('sprintf', [rt.new_string('Running %s callback'),
				var_callback.clone()]),
			rt.create_array([rt.ArrayItem{ key: 'source', val: 'wc_db_updates' }]),
		])
		var_result = rt.new_bool((rt.call_function('call_user_func', [
			var_callback.clone(),
			rt.new_object('WC_Background_Updater', [
				'WC_Background_Process',
			], &this)])).to_bool())
		if rt.is_true(var_result) {
			rt.call_method(var_logger, 'info', [
				rt.call_function('sprintf', [
					rt.new_string('%s callback needs to run again'),
					var_callback.clone(),
				]),
				rt.create_array([
					rt.ArrayItem{ key: 'source', val: 'wc_db_updates' },
			...
			rt.call_method(var_logger, 'info', [
				rt.call_function('sprintf', [
					rt.new_string('Finished running %s callback'),
					var_callback.clone(),
				]),
				rt.create_array([
					rt.ArrayItem{ key: 'source', val: 'wc_db_updates' },
		...
		rt.call_method(var_logger, 'notice', [
			rt.call_function('sprintf', [rt.new_string('Could not find %s callback'),
				var_callback.clone()]),
			rt.create_array([rt.ArrayItem{ key: 'source', val: 'wc_db_updates' }]),
		])
	}
...
fn (mut this Class_WC_Background_Updater) complete() {
	mut var_logger := rt.call_function('wc_get_logger', []rt.PhpVal{})
	rt.call_method(var_logger, 'info', [rt.new_string('Data update complete'),
		rt.create_array([rt.ArrayItem{ key: 'source', val: 'wc_db_updates' }])])
	mut iife_temp_0 := Class_WC_Install{}
	mut iife_result_0 := iife_temp_0.update_db_version()
