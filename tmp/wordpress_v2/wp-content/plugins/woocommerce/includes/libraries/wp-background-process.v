...
fn (mut this Class_WP_Background_Process) construct() {
	...
	this.cron_hook_identifier =
		(rt.get_property(rt.new_object('WP_Background_Process', ['WP_Async_Request'], &this), 'identifier')).str() +
		'_cron'
	this.cron_interval_identifier =
		(rt.get_property(rt.new_object('WP_Background_Process', ['WP_Async_Request'], &this), 'identifier')).str() +
		'_cron_interval'
	rt.call_function('add_action', [this.cron_hook_identifier,
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WP_Background_Process', [
				'WP_Async_Request',
			], &this) },
			rt.ArrayItem{ key: none, val: 'handle_cron_healthcheck' },
		])])
	rt.call_function('add_filter', [rt.new_string('cron_schedules'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WP_Background_Process', [
				'WP_Async_Request',
			], &this) },
			rt.ArrayItem{ key: none, val: 'schedule_cron_healthcheck' },
		])])
}
...
fn (mut this Class_WP_Background_Process) push_to_queue(var_data rt.PhpVal) rt.PhpVal {
	rt.get_property(rt.new_object('WP_Background_Process', ['WP_Async_Request'], &this), 'data').array_push(var_data.clone())
...
fn (mut this Class_WP_Background_Process) save() rt.PhpVal {
	...
	if !(!rt.is_true(rt.get_property(rt.new_object('WP_Background_Process', [
		'WP_Async_Request',
	], &this), 'data'))) {
		rt.call_function('update_site_option', [var_key.clone(),
			rt.get_property(rt.new_object('WP_Background_Process', [
				'WP_Async_Request',
			], &this), 'data')])
	}
	return rt.new_object('WP_Background_Process', []string{}, this)
...
fn (mut this Class_WP_Background_Process) update(var_key rt.PhpVal, var_data rt.PhpVal) rt.PhpVal {
	...
	if !(!rt.is_true(var_data)) {
		rt.call_function('update_site_option', [var_key_mutated.clone(),
			var_data.clone()])
	}
	return rt.new_object('WP_Background_Process', []string{}, this)
}
...
fn (mut this Class_WP_Background_Process) delete(var_key rt.PhpVal) rt.PhpVal {
	mut var_key_mutated := var_key
	rt.call_function('delete_site_option', [var_key_mutated.clone()])
	return rt.new_object('WP_Background_Process', []string{}, this)
}

fn (mut this Class_WP_Background_Process) generate_key(length i64) rt.PhpVal {
	mut var_unique := rt.new_string(md5.hexhash(
		(rt.call_function('microtime', []rt.PhpVal{})).str() +
		(rt.call_function('rand', []rt.PhpVal{})).str()))
	mut var_prepend := rt.new_string(
		(rt.get_property(rt.new_object('WP_Background_Process', ['WP_Async_Request'], &this), 'identifier')).str() +
		'_batch_')
	return rt.call_function('substr', [
		rt.new_string(var_prepend.str() + var_unique.str()),
		rt.new_int(0),
		rt.new_int(length),
	])
}
...
fn (mut this Class_WP_Background_Process) maybe_handle() {
	...
	rt.call_function('check_ajax_referer', [
		rt.get_property(rt.new_object('WP_Background_Process', ['WP_Async_Request'], &this),
			'identifier'),
		rt.new_string('nonce'),
	])
	this.handle()
...
fn (mut this Class_WP_Background_Process) is_process_running() bool {
	if rt.is_true(rt.call_function('get_site_transient', [
		rt.new_string(
			(rt.get_property(rt.new_object('WP_Background_Process', ['WP_Async_Request'], &this), 'identifier')).str() +
			'_process_lock'),
	]))
...
fn (mut this Class_WP_Background_Process) lock_process() {
	...
	mut var_lock_duration := if rt.is_true(rt.call_function('property_exists', [
		rt.new_object('WP_Background_Process', ['WP_Async_Request'], &this),
		rt.new_string('queue_lock_time'),
	]))
	{
		rt.get_property(rt.new_object('WP_Background_Process', ['WP_Async_Request'], &this),
			'queue_lock_time')
	...
	var_lock_duration = rt.call_function('apply_filters', [
		rt.new_string(
			(rt.get_property(rt.new_object('WP_Background_Process', ['WP_Async_Request'], &this), 'identifier')).str() +
			'_queue_lock_time'),
		var_lock_duration.clone(),
	])
...
fn (mut this Class_WP_Background_Process) get_batch() rt.PhpVal {
	...
	rt.set_property(var_batch, 'key', rt.get_property(var_query,
		'{"nodeType":"Expr_Variable","line":282,"name":"column"}'))
	rt.set_property(var_batch, 'data', rt.call_function('maybe_unserialize', [
		rt.get_property(var_query, '{"nodeType":"Expr_Variable","line":283,"name":"value_column"}'),
	]))
...
fn (mut this Class_WP_Background_Process) get_memory_limit() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_memory_limit))))
		|| rt.is_true(rt.identical(-1, var_memory_limit)) {
...
fn (mut this Class_WP_Background_Process) schedule_event() {
	...
	])))))
	{
		rt.call_function('wp_schedule_event', [rt.call_function('time', []rt.PhpVal{}),
			this.cron_interval_identifier, this.cron_hook_identifier])
	}
}
...
fn (mut this Class_WP_Background_Process) clear_scheduled_event() {
	...
	if rt.is_true(var_timestamp) {
		rt.call_function('wp_unschedule_event', [var_timestamp.clone(), this.cron_hook_identifier])
	}
}
...
fn (mut this Class_WP_Background_Process) cancel_process() {
	if !(this.is_queue_empty()) {
		mut var_batch := this.get_batch()
		this.delete(rt.get_property(var_batch, 'key'))
		rt.call_function('wp_clear_scheduled_hook', [this.cron_hook_identifier])
	}
}
...
fn create_wp_async_request(_args ...rt.PhpVal) &Class_WP_Async_Request {
...
fn (mut this Class_WP_Background_Process) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'cron_hook_identifier' {
			this.cron_hook_identifier = val
			return true
		}
		'cron_interval_identifier' {
			this.cron_interval_identifier = val
			return true
		}
		else {
