...
fn (mut this Class_WC_Meta_Data) construct(var_meta rt.PhpVal) {
	this.current_data = var_meta.clone()
	this.apply_changes()
}
...
fn (mut this Class_WC_Meta_Data) magic_set(var_key rt.PhpVal, var_value rt.PhpVal) {
	this.current_data.array_set(var_key, var_value.clone())
}

fn (mut this Class_WC_Meta_Data) magic_isset(var_key rt.PhpVal) bool {
	return this.current_data.array_isset(var_key.clone())
}

fn (mut this Class_WC_Meta_Data) magic_get(var_key rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(this.current_data.array_isset(var_key.clone()))) {
...
fn (mut this Class_WC_Meta_Data) get_changes() rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(this.data.array_isset(var_id.clone()))))))
			|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_value, this.data.array_get(var_id))))) {
			var_changes.array_set(var_id, var_value.clone())
		}
	}
