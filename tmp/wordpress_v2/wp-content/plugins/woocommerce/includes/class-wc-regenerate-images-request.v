...
fn (mut this Class_WC_Regenerate_Images_Request) construct() {
	...
	if rt.is_true(rt.call_function('extension_loaded', [rt.new_string('imagick')]))
		&& rt.is_true(rt.call_function('method_exists', [Class_Imagick.class(), rt.new_string('setResourceLimit')])) {
		if rt.is_true(rt.call_function('defined', [
			rt.new_string('Imagick::RESOURCETYPE_THREAD'),
		]))
		{
			mut iife_temp_0 := Class_Imagick{}
			mut iife_result_0 := iife_temp_0.setresourcelimit(Class_Imagick.resourcetype_thread(),
				rt.new_int(1))
		} else {
			mut iife_temp_1 := Class_Imagick{}
			mut iife_result_1 := iife_temp_1.setresourcelimit(rt.new_int(6), rt.new_int(1))
		}
	}
...
fn (mut this Class_WC_Regenerate_Images_Request) is_regeneratable(var_attachment rt.PhpVal) bool {
	...
	if rt.is_true(rt.identical(rt.new_string('site-icon'), rt.call_function('get_post_meta', [
		rt.get_property(var_attachment_mutated, 'ID'),
...
fn (mut this Class_WC_Regenerate_Images_Request) task(var_item rt.PhpVal) bool {
	if !(var_item.clone().is_array()) && !(var_item.array_isset(rt.new_string('attachment_id'))) {
		return false
	}
	this.attachment_id = rt.call_function('absint', [
		var_item.array_get(rt.new_string('attachment_id')),
	])
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_attachment))))
		...
		|| !(this.is_regeneratable(var_attachment.clone())) {
		return false
	}
	...
	rt.call_method(var_log, 'info', [
		rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('Regenerating images for attachment ID: %s'),
	...
	mut var_old_metadata := rt.call_function('wp_get_attachment_metadata', [
		this.attachment_id,
	])
	rt.call_function('add_filter', [rt.new_string('intermediate_image_sizes'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Regenerate_Images_Request', [
				'WC_Background_Process',
			], &this) },
			rt.ArrayItem{ key: none, val: 'adjust_intermediate_image_sizes' },
		])])
	rt.call_function('add_filter', [rt.new_string('intermediate_image_sizes_advanced'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Regenerate_Images_Request', [
				'WC_Background_Process',
			], &this) },
			rt.ArrayItem{ key: none, val: 'filter_image_sizes_to_only_missing_thumbnails' },
		]),
	...
	if !(!rt.is_true(var_old_metadata))
		&& !(!rt.is_true(var_old_metadata.array_get(rt.new_string('sizes'))))
		&& var_old_metadata.array_get(rt.new_string('sizes')).is_array() {
		mut iter_1 := var_old_metadata.array_get(rt.new_string('sizes')).iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_old_size_data := item_1.val
			mut var_old_size := item_1.key
			if !rt.is_true(var_new_metadata.array_get(rt.new_string('sizes')).array_get(var_old_size)) {
				var_new_metadata.array_get_mut('sizes').array_set(var_old_size,
					var_old_metadata.array_get(rt.new_string('sizes')).array_get(var_old_size))
			}
		}
	}
	rt.call_function('wp_update_attachment_metadata',
		[this.attachment_id, var_new_metadata.clone()])
	return false
}
...
fn (mut this Class_WC_Regenerate_Images_Request) filter_image_sizes_to_only_missing_thumbnails(var_sizes rt.PhpVal, var_metadata rt.PhpVal, var_attachment_id rt.PhpVal) rt.PhpVal {
	...
	var_attachment_id_mutated = if var_attachment_id_mutated.clone().is_null() {
		this.attachment_id
	} else {
		var_attachment_id_mutated
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_sizes))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(var_attachment_id_mutated)))) {
		return var_sizes.clone()
	}
	mut var_fullsizepath := rt.call_function('get_attached_file', [
		var_attachment_id_mutated.clone()])
	mut var_editor := rt.call_function('wp_get_image_editor', [
		var_fullsizepath.clone()])
	if rt.is_true(rt.call_function('is_wp_error', [var_editor.clone()])) {
		return var_sizes.clone()
	}
	var_metadata_mutated = rt.call_function('wp_get_attachment_metadata', [
		var_attachment_id_mutated.clone()])
	mut iter_2 := var_sizes.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_size_data := item_2.val
		mut var_size := item_2.key
		if !rt.is_true(var_metadata_mutated.array_get(rt.new_string('sizes')).array_get(var_size)) {
			continue
		}
		if !(var_size_data.array_isset(rt.new_string('width')))
			&& !(var_size_data.array_isset(rt.new_string('height'))) {
			continue
		}
		if !(var_size_data.array_isset(rt.new_string('width'))) {
		...
		mut var_image_sizes := rt.call_function('getimagesize', [
			var_fullsizepath.clone()])
		if rt.is_true(rt.identical(rt.new_bool(false), var_image_sizes)) {
			continue
		}
		mut list_tmp_1 := var_image_sizes
		var_orig_w = list_tmp_1.array_get(0)
		var_orig_h = list_tmp_1.array_get(1)
		mut var_dimensions := rt.call_function('image_resize_dimensions', [
			var_orig_w.clone(), var_orig_h.clone(), var_size_data.array_get(rt.new_string('width')),
			var_size_data.array_get(rt.new_string('height')),
			var_size_data.array_get(rt.new_string('crop'))])
		if rt.is_true(rt.new_bool(!(rt.is_true(var_dimensions))))
			|| !(var_dimensions.clone().is_array()) {
			continue
		}
		mut var_info := rt.call_function('pathinfo', [var_fullsizepath.clone()])
		mut var_ext := var_info.array_get(rt.new_string('extension'))
		mut var_dst_w := var_dimensions.array_get(rt.new_int(4))
		mut var_dst_h := var_dimensions.array_get(rt.new_int(5))
		mut var_suffix := rt.new_string('${var_dst_w.to_string()}x${var_dst_h.to_string()}')
		mut var_dst_rel_path := rt.call_function('str_replace', [
			rt.new_string('.' + var_ext.str()),
			rt.new_string(''),
			var_fullsizepath.clone(),
		])
		mut var_thumbnail :=
			rt.new_string('${var_dst_rel_path.to_string()}-${var_suffix.to_string()}.${var_ext.to_string()}')
		if rt.is_true(rt.identical(var_dst_w, var_metadata_mutated.array_get(rt.new_string('sizes')).array_get(var_size).array_get(rt.new_string('width'))))
			&& rt.is_true(rt.identical(var_dst_h, var_metadata_mutated.array_get(rt.new_string('sizes')).array_get(var_size).array_get(rt.new_string('height'))))
			&& rt.is_true(rt.call_function('file_exists', [var_thumbnail.clone()])) {
...
fn (mut this Class_WC_Regenerate_Images_Request) adjust_intermediate_image_sizes(var_sizes rt.PhpVal) rt.PhpVal {
	mut var_unfiltered_sizes := ['woocommerce_thumbnail', 'woocommerce_gallery_thumbnail',
		'woocommerce_single']
	mut var_in_filter := false
	if rt.is_true(var_in_filter) {
		return var_unfiltered_sizes.clone()
	}
	var_in_filter = rt.new_bool(true)
	mut var_filtered_sizes := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_regenerate_images_intermediate_image_sizes'),
		rt.create_array_from_list(var_unfiltered_sizes),
	])
	var_in_filter = rt.new_bool(false)
	return var_filtered_sizes.clone()
}
...
fn (mut this Class_WC_Regenerate_Images_Request) complete() {
	...
	rt.call_method(var_log, 'info', [
		rt.call_function('__', [
			rt.new_string('Completed product image regeneration job.'),
			rt.new_string('woocommerce'),
		]),
		rt.create_array([
			rt.ArrayItem{ key: 'source', val: 'wc-image-regeneration' },
...
fn (mut this Class_WC_Regenerate_Images_Request) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'filter_image_sizes_to_only_missing_thumbnails' {
			...
			return this.filter_image_sizes_to_only_missing_thumbnails(dispatch_arg_0,
