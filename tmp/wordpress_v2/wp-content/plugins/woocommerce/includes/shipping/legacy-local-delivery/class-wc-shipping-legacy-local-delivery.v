...
fn (mut this Class_WC_Shipping_Legacy_Local_Delivery) calculate_shipping(var_package rt.PhpVal) {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('fixed'))) {
		...
		var_shipping_total = rt.mul(var_package.array_get(rt.new_string('contents_cost')), rt.div(rt.get_property(rt.new_object('WC_Shipping_Legacy_Local_Delivery', [
			'WC_Shipping_Local_Pickup',
		...
		for {
			...
			if rt.is_true(rt.greater(var_values.array_get(rt.new_string('quantity')), rt.new_int(0)))
				...
				var_shipping_total = rt.add(var_shipping_total, rt.mul(rt.get_property(rt.new_object('WC_Shipping_Legacy_Local_Delivery', [
					'WC_Shipping_Local_Pickup',
				], &this), 'fee'), var_values.array_get(rt.new_string('quantity'))))
			}
		}
	}
	mut var_rate := {
...
fn create_wc_shipping_local_pickup(_args ...rt.PhpVal) &Class_WC_Shipping_Local_Pickup {
