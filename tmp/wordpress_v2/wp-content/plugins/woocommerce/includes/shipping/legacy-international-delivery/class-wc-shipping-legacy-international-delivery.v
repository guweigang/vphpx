...
fn (mut this Class_WC_Shipping_Legacy_International_Delivery) is_available(var_package rt.PhpVal) bool {
	...
	{
		if rt.get_property(rt.new_object('WC_Shipping_Legacy_International_Delivery', ['WC_Shipping_Legacy_Flat_Rate'], &this), 'countries').is_array()
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_package.array_get(rt.new_string('destination')).array_get(rt.new_string('country')), rt.get_property(rt.new_object('WC_Shipping_Legacy_International_Delivery', ['WC_Shipping_Legacy_Flat_Rate'], &this), 'countries'), rt.new_bool(true)]))))) {
			return false
		}
	} else {
		if rt.get_property(rt.new_object('WC_Shipping_Legacy_International_Delivery', ['WC_Shipping_Legacy_Flat_Rate'], &this), 'countries').is_array()
			&& rt.is_true(rt.call_function('in_array', [var_package.array_get(rt.new_string('destination')).array_get(rt.new_string('country')), rt.get_property(rt.new_object('WC_Shipping_Legacy_International_Delivery', ['WC_Shipping_Legacy_Flat_Rate'], &this), 'countries'), rt.new_bool(true)]))
			|| rt.is_true(rt.new_bool(!(rt.is_true(var_package.array_get(rt.new_string('destination')).array_get(rt.new_string('country')))))) {
			return false
		}
	}
