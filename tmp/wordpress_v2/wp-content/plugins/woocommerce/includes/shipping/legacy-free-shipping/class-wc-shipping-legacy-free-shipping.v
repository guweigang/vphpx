...
fn (mut this Class_WC_Shipping_Legacy_Free_Shipping) is_available(var_package rt.PhpVal) bool {
	...
	mut var_is_available := rt.new_bool(false)
	mut var_has_coupon := rt.new_bool(false)
	mut var_has_met_min_amount := rt.new_bool(false)
		...
		rt.new_bool(true)]))
	{
		...
		if rt.is_true(var_coupons) {
			...
			for {
				...
				if rt.is_true(rt.call_method(var_coupon, 'is_valid', []rt.PhpVal{}))
					&& rt.is_true(rt.call_method(var_coupon, 'get_free_shipping', []rt.PhpVal{})) {
					var_has_coupon = rt.new_bool(true)
				}
			}
		}
	}
		...
		rt.new_bool(true)]))
	{
			...
			'display_prices_including_tax', []rt.PhpVal{}))
		{
			...
			mut iife_result_0 := iife_temp_0.round(rt.sub(var_total, rt.add(rt.call_method(rt.get_property(rt.call_function('WC',
				...
				[]rt.PhpVal{}))
			var_total = iife_result_0
		} else {
			mut iife_temp_1 := Class_Automattic_WooCommerce_Utilities_NumberUtil{}
			mut iife_result_1 := iife_temp_1.round(rt.sub(var_total, rt.call_method(rt.get_property(rt.call_function('WC',
				[]rt.PhpVal{}), 'cart'), 'get_discount_total', []rt.PhpVal{})), rt.call_function('wc_get_price_decimals',
				[]rt.PhpVal{}))
			var_total = iife_result_1
		}
		if rt.is_true(rt.greater_equal(var_total, this.min_amount)) {
			var_has_met_min_amount = rt.new_bool(true)
		}
	}
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('min_amount'))) {
		if rt.is_true(var_has_met_min_amount) {
			var_is_available = rt.new_bool(true)
		}
		...
		if rt.is_true(var_has_met_min_amount) && rt.is_true(var_has_coupon) {
			var_is_available = rt.new_bool(true)
		}
