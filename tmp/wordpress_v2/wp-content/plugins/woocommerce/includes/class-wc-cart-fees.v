...
fn (mut this Class_WC_Cart_Fees) construct(var_deprecated rt.PhpVal) {
	if !var_deprecated.is_null() {
		rt.call_function('wc_doing_it_wrong', [rt.new_string('new WC_Cart_Fees'),
			rt.new_string("You don't need to pass a cart parameter to the WC_Cart_Fees constructor anymore"),
			rt.new_string('8.2.0')])
	}
}
...
fn (mut this Class_WC_Cart_Fees) add_fee(var_args rt.PhpVal) rt.PhpVal {
	mut var_fee_props := rt.array_to_object(rt.call_function('wp_parse_args', [
		var_args.clone(),
		this.default_fee_props,
	]))
	rt.set_property(var_fee_props, 'name', if rt.is_true(rt.get_property(var_fee_props, 'name')) { rt.get_property(var_fee_props, 'name') } else { rt.call_function('__', [
			rt.new_string('Fee'),
	...
	mut iife_result_2 := iife_temp_2.get_tax_classes()
	rt.set_property(var_fee_props, 'tax_class', if rt.is_true(rt.call_function('in_array', [
		rt.get_property(var_fee_props, 'tax_class'),
		rt.call_function('array_merge', [iife_result_0, iife_result_1]),
		rt.new_bool(true),
	]))
	{ rt.get_property(var_fee_props, 'tax_class') } else { rt.new_string('') })
	rt.set_property(var_fee_props, 'taxable', rt.call_function('wc_string_to_bool', [
		rt.get_property(var_fee_props, 'taxable'),
	]))
	rt.set_property(var_fee_props, 'amount', rt.call_function('wc_format_decimal', [
		rt.get_property(var_fee_props, 'amount'),
	]))
	if !rt.is_true(rt.get_property(var_fee_props, 'id')) {
		rt.set_property(var_fee_props, 'id', this.generate_id(var_fee_props.clone()))
	}
	if rt.is_true(rt.new_bool(this.fees.array_isset(rt.get_property(var_fee_props, 'id')))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('fee_exists'), rt.call_function('__', [
			rt.new_string('Fee has already been added.'),
			rt.new_string('woocommerce'),
		])))
	}
	this.fees.array_set(rt.get_property(var_fee_props, 'id'), var_fee_props.clone())
	return this.fees.array_get(rt.get_property(var_fee_props, 'id'))
}

fn (mut this Class_WC_Cart_Fees) get_fees() rt.PhpVal {
	rt.call_function('uasort', [this.fees,
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Cart_Fees', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'sort_fees_callback' },
		])])
	return this.fees
}
...
fn (mut this Class_WC_Cart_Fees) set_fees(var_raw_fees rt.PhpVal) {
	...
	for {
		...
		this.add_fee(var_raw_fee.clone())
	}
}
...
fn (mut this Class_WC_Cart_Fees) sort_fees_callback(var_a rt.PhpVal, var_b rt.PhpVal) rt.PhpVal {
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_sort_fees_callback'),
		rt.new_int(if rt.is_true(rt.greater(rt.get_property(var_a, 'amount'), rt.get_property(var_b,
			'amount')))
		{
			-1
		} else {
