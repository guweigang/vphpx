...
fn (mut this Class_WC_Integrations) construct() {
	...
	mut var_load_integrations := rt.create_array([
		rt.ArrayItem{ key: none, val: 'WC_Integration_MaxMind_Geolocation' },
	])
	var_load_integrations = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_integrations'),
		var_load_integrations.clone(),
	])
	...
	for {
		...
		this.integrations.array_set(rt.get_property(var_load_integration, 'id'),
			var_load_integration.clone())
	}
}
