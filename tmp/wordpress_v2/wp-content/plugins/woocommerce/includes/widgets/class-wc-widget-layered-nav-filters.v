...
fn (mut this Class_WC_Widget_Layered_Nav_Filters) construct() {
	this.dispatch_set_prop('widget_cssclass',
		rt.new_string('woocommerce widget_layered_nav_filters'))
	this.dispatch_set_prop('widget_description', rt.call_function('__', [
		rt.new_string('Display a list of active product filters.'),
	...
	this.dispatch_set_prop('widget_name', rt.call_function('__', [
		rt.new_string('Active Product Filters'),
		rt.new_string('woocommerce'),
	]))
...
fn (mut this Class_WC_Widget_Layered_Nav_Filters) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	mut var_min_price := if rt.get_superglobal('_GET').array_isset(rt.new_string('min_price')) { rt.call_function('wc_clean', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('min_price'))]),
		]) } else { rt.new_int(0) }
	mut var_max_price := if rt.get_superglobal('_GET').array_isset(rt.new_string('max_price')) { rt.call_function('wc_clean', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('max_price'))]),
		]) } else { rt.new_int(0) }
	mut var_rating_filter := if rt.get_superglobal('_GET').array_isset(rt.new_string('rating_filter')) { rt.call_function('array_filter', [
	...
	if 0 < var__chosen_attributes.clone().array_count()
		|| rt.is_true(rt.less(rt.new_int(0), var_min_price))
		|| rt.is_true(rt.less(rt.new_int(0), var_max_price)) || !(!rt.is_true(var_rating_filter)) {
		this.widget_start(var_args.clone(), var_instance.clone())
		print('<ul>')
		rt.call_function('do_action', [
			rt.new_string('woocommerce_widget_layered_nav_filters_start'),
			var_args.clone(),
			var_instance.clone(),
		])
		if !(!rt.is_true(var__chosen_attributes)) {
			...
			for {
				...
				mut iter_2 := var_data.array_get(rt.new_string('terms')).iterator()
				for {
					...
					mut var_filter_name :=
						rt.new_string('filter_' +(rt.call_function('wc_attribute_taxonomy_slug', [var_taxonomy.clone()])).str())
					mut var_current_filter := if rt.get_superglobal('_GET').array_isset(var_filter_name) { rt.call_function('explode', [
							...
							rt.call_function('wc_clean', [
								rt.call_function('wp_unslash', [
									rt.get_superglobal('_GET').array_get(var_filter_name),
								]),
							]),
						]) } else { rt.new_array() }
					...
					mut var_new_filter := rt.call_function('array_diff', [
						var_current_filter.clone(),
						rt.create_array([
							rt.ArrayItem{ key: none, val: var_term_slug },
						])])
					mut var_link := rt.call_function('remove_query_arg', [
						rt.create_array([rt.ArrayItem{ key: none, val: 'add-to-cart' },
							rt.ArrayItem{ key: none, val: var_filter_name }]),
						var_base_link.clone(),
					])
					if var_new_filter.clone().array_count() > 0 {
						var_link = rt.call_function('add_query_arg', [
							var_filter_name.clone(),
							rt.call_function('implode', [
								rt.new_string(','),
								var_new_filter.clone(),
							]),
							var_link.clone()])
					}
					mut var_filter_classes := ['chosen',
						'chosen-' +(rt.call_function('sanitize_html_class', [rt.call_function('str_replace', [rt.new_string('pa_'), rt.new_string(''), var_taxonomy.clone()])])).str(),
						'chosen-' +
							(rt.call_function('sanitize_html_class', [rt.new_string((rt.call_function('str_replace', [rt.new_string('pa_'), rt.new_string(''), var_taxonomy.clone()])).str() +
							'-' + var_term_slug.str())])).str()]
					mut var_anchor_text := rt.call_function('apply_filters', [
						rt.new_string('woocommerce_widget_layered_nav_term_anchor_text'),
						rt.get_property(var_term, 'name'),
						var_term.clone(),
						var_taxonomy.clone(),
					])
					print('<li class="' +
						(rt.call_function('esc_attr', [rt.call_function('implode', [rt.new_string(' '), rt.create_array_from_list(var_filter_classes)])])).str() +
						'"><a rel="nofollow" aria-label="' +
						(rt.call_function('esc_attr__', [rt.new_string('Remove filter'), rt.new_string('woocommerce')])).str() +
						'" href="' + (rt.call_function('esc_url', [var_link.clone()])).str() +
						'">' + (rt.call_function('esc_html', [var_anchor_text.clone()])).str() +
						'</a></li>')
				}
			}
		}
		if rt.is_true(var_min_price) {
			...
			print('<li class="chosen"><a rel="nofollow" aria-label="' +
				(rt.call_function('esc_attr__', [rt.new_string('Remove filter'), rt.new_string('woocommerce')])).str() +
				'" href="' + (rt.call_function('esc_url', [var_link.clone()])).str() + '">' +
				(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Min %s'), rt.new_string('woocommerce')]), rt.call_function('wc_price', [var_min_price.clone()])])).str() +
				'</a></li>')
		}
		if rt.is_true(var_max_price) {
			var_link = rt.call_function('remove_query_arg', [
				rt.new_string('max_price'), var_base_link.clone()])
			print('<li class="chosen"><a rel="nofollow" aria-label="' +
				(rt.call_function('esc_attr__', [rt.new_string('Remove filter'), rt.new_string('woocommerce')])).str() +
				'" href="' + (rt.call_function('esc_url', [var_link.clone()])).str() + '">' +
				(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Max %s'), rt.new_string('woocommerce')]), rt.call_function('wc_price', [var_max_price.clone()])])).str() +
				'</a></li>')
		}
		if !(!rt.is_true(var_rating_filter)) {
			mut iter_3 := var_rating_filter.iterator()
			for {
				item_3 := iter_3.next() or { break }
				mut var_rating := item_3.val
				mut var_link_ratings := rt.call_function('implode', [
					rt.new_string(','),
					rt.call_function('array_diff', [
						var_rating_filter.clone(),
						rt.create_array([
							rt.ArrayItem{ key: none, val: var_rating },
						])])])
				var_link = if rt.is_true(var_link_ratings) { rt.call_function('add_query_arg', [
						rt.new_string('rating_filter'),
						var_link_ratings.clone(),
					]) } else { rt.call_function('remove_query_arg', [
						rt.new_string('rating_filter'),
						var_base_link.clone(),
					]) }
				print('<li class="chosen"><a rel="nofollow" aria-label="' +
					(rt.call_function('esc_attr__', [rt.new_string('Remove filter'), rt.new_string('woocommerce')])).str() +
					'" href="' + (rt.call_function('esc_url', [var_link.clone()])).str() + '">' +
					(rt.call_function('sprintf', [rt.call_function('esc_html__', [rt.new_string('Rated %s out of 5'), rt.new_string('woocommerce')]), rt.call_function('esc_html', [var_rating.clone()])])).str() +
					'</a></li>')
			}
		}
		rt.call_function('do_action', [
			rt.new_string('woocommerce_widget_layered_nav_filters_end'),
			var_args.clone(),
			var_instance.clone(),
		])
		print('</ul>')
