...
fn (mut this Class_WC_Widget_Cart) construct() {
	...
	this.dispatch_set_prop('widget_description', rt.call_function('__', [
		rt.new_string('Display the customer shopping cart.'),
	...
	this.dispatch_set_prop('widget_name', rt.call_function('__', [
		rt.new_string('Cart'), rt.new_string('woocommerce')]))
	this.dispatch_set_prop('settings', rt.create_array([
		...
		rt.ArrayItem{ key: 'hide_if_empty', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'checkbox' },
			rt.ArrayItem{ key: 'std', val: 0 },
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Hide if cart is empty'), rt.new_string('woocommerce')]) },
		]) },
	]))
...
fn (mut this Class_WC_Widget_Cart) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	if rt.is_true(rt.call_function('apply_filters', [
		rt.new_string('woocommerce_widget_cart_is_hidden'),
		rt.new_bool(rt.is_true(rt.call_function('is_cart', []rt.PhpVal{}))
	...
	rt.call_function('wp_enqueue_script', [rt.new_string('wc-cart-fragments')])
	mut var_hide_if_empty := rt.new_int(if !rt.is_true(var_instance_mutated.array_get(rt.new_string('hide_if_empty'))) {
		0
	} else {
		1
	})
