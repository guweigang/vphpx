...
fn (mut this Class_WC_Widget_Recent_Reviews) construct() {
	...
	this.dispatch_set_prop('widget_description', rt.call_function('__', [
		rt.new_string('Display a list of recent reviews from your store.'),
	...
	this.dispatch_set_prop('widget_name', rt.call_function('__', [
		rt.new_string('Recent Product Reviews'),
		rt.new_string('woocommerce'),
	]))
	this.dispatch_set_prop('settings', rt.create_array([
		...
		rt.ArrayItem{ key: 'number', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Number of reviews to show'),
				rt.new_string('woocommerce'),
			]) },
		]) },
	]))
...
fn (mut this Class_WC_Widget_Recent_Reviews) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	mut var_comments := rt.get_superglobal('comments')
	...
	mut var_number := if !(!rt.is_true(var_instance.array_get(rt.new_string('number')))) { rt.call_function('absint', [
		...
		]) } else { rt.get_property(rt.new_object('WC_Widget_Recent_Reviews', [
			'WC_Widget',
	...
	var_comments = rt.call_function('get_comments', [
		rt.create_array([rt.ArrayItem{ key: 'number', val: var_number },
			...
			rt.ArrayItem{ key: 'parent', val: 0 }, rt.ArrayItem{
	...
	if rt.is_true(var_comments) {
		...
		rt.echo_val(rt.call_function('wp_kses_post', [
			rt.call_function('apply_filters', [
				rt.new_string('woocommerce_before_widget_product_review_list'),
		...
		for {
			...
			rt.call_function('wc_get_template', [
				rt.new_string('content-widget-reviews.php'),
				rt.create_array([rt.ArrayItem{ key: 'comment', val: var_comment_shadow },
		...
		rt.echo_val(rt.call_function('wp_kses_post', [
			rt.call_function('apply_filters', [
				rt.new_string('woocommerce_after_widget_product_review_list'),
				rt.new_string('</ul>'),
			]),
		]))
