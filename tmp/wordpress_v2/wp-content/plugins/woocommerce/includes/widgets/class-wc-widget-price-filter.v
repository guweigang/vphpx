...
fn (mut this Class_WC_Widget_Price_Filter) construct() {
	...
	this.dispatch_set_prop('widget_description', rt.call_function('__', [
		rt.new_string('Display a slider to filter products in your store by price.'),
	...
	this.dispatch_set_prop('widget_name', rt.call_function('__', [
		rt.new_string('Filter Products by Price'),
		rt.new_string('woocommerce'),
	]))
	...
	rt.call_function('wp_register_script', [rt.new_string('wc-accounting'),
		rt.new_string(
		...
		rt.new_string('0.4.2'), rt.new_bool(true)])
	rt.call_function('wp_register_script', [rt.new_string('accounting'),
		rt.new_bool(false), rt.create_array([
			rt.ArrayItem{ key: none, val: 'wc-accounting' },
		]),
		rt.new_string('0.4.2'), rt.new_bool(true)])
	rt.call_function('wp_register_script', [rt.new_string('wc-jquery-ui-touchpunch'),
		rt.new_string(
			(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'plugin_url', []rt.PhpVal{})).str() +
			'/assets/js/jquery-ui-touch-punch/jquery-ui-touch-punch' + var_suffix.str() + '.js'),
		rt.create_array([rt.ArrayItem{ key: none, val: 'jquery-ui-slider' }]),
		var_version.clone(), rt.new_bool(true)])
	rt.call_function('wp_register_script', [rt.new_string('wc-price-slider'),
		rt.new_string(
			(rt.call_method(rt.call_function('WC', []rt.PhpVal{}), 'plugin_url', []rt.PhpVal{})).str() +
			'/assets/js/frontend/price-slider' + var_suffix.str() + '.js'),
		rt.create_array([rt.ArrayItem{ key: none, val: 'jquery-ui-slider' },
			rt.ArrayItem{ key: none, val: 'wc-jquery-ui-touchpunch' },
			rt.ArrayItem{ key: none, val: 'wc-accounting' }]),
		var_version.clone(), rt.new_bool(true)])
	rt.call_function('wp_localize_script', [rt.new_string('wc-price-slider'),
		rt.new_string('woocommerce_price_slider_params'),
		rt.create_array([
			rt.ArrayItem{ key: 'currency_format_num_decimals', val: 0 },
			rt.ArrayItem{ key: 'currency_format_symbol', val: rt.call_function('get_woocommerce_currency_symbol',
				[]rt.PhpVal{}) },
			rt.ArrayItem{ key: 'currency_format_decimal_sep', val: rt.call_function('esc_attr', [
				rt.call_function('wc_get_price_decimal_separator', []rt.PhpVal{}),
			]) },
			rt.ArrayItem{ key: 'currency_format_thousand_sep', val: rt.call_function('esc_attr', [
				rt.call_function('wc_get_price_thousand_separator', []rt.PhpVal{}),
			]) },
			rt.ArrayItem{ key: 'currency_format', val: rt.call_function('esc_attr', [
				rt.call_function('str_replace', [
					rt.create_array([rt.ArrayItem{ key: none, val: '%1$s' },
						rt.ArrayItem{ key: none, val: '%2$s' }]),
					rt.create_array([rt.ArrayItem{ key: none, val: '%s' },
						rt.ArrayItem{ key: none, val: '%v' }]),
...
fn (mut this Class_WC_Widget_Price_Filter) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	rt.call_function('wp_enqueue_script', [rt.new_string('wc-price-slider')])
	mut var_step := rt.call_function('max', [
		rt.call_function('apply_filters', [
			rt.new_string('woocommerce_price_filter_widget_step'),
			rt.new_int(10),
		]),
		rt.new_int(1),
	])
	...
	if rt.is_true(rt.call_function('wc_tax_enabled', []rt.PhpVal{}))
		...
		&& rt.is_true(rt.identical(rt.new_string('incl'), var_tax_display_mode)) {
		...
		if rt.is_true(var_tax_rates) {
			...
			mut iife_result_3 := iife_temp_3.calc_exclusive_tax(var_min_price.clone(),
				var_tax_rates.clone())
			mut iife_temp_4 := Class_WC_Tax{}
			mut iife_result_4 := iife_temp_4.get_tax_total(iife_result_3)
			var_min_price = rt.add(var_min_price, iife_result_4)
			mut iife_temp_5 := Class_WC_Tax{}
			mut iife_result_5 := iife_temp_5.calc_exclusive_tax(var_max_price.clone(),
				var_tax_rates.clone())
			mut iife_temp_6 := Class_WC_Tax{}
			mut iife_result_6 := iife_temp_6.get_tax_total(iife_result_5)
			var_max_price = rt.add(var_max_price, iife_result_6)
		}
	}
	var_min_price = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_price_filter_widget_min_amount'),
		rt.mul(rt.call_function('floor', [rt.div(var_min_price, var_step)]), var_step),
	])
	var_max_price = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_price_filter_widget_max_amount'),
		rt.mul(rt.call_function('ceil', [rt.div(var_max_price, var_step)]), var_step),
	])
	if rt.is_true(rt.identical(var_min_price, var_max_price)) {
		return
	}
	mut var_current_min_price := if rt.get_superglobal('_GET').array_isset(rt.new_string('min_price')) { rt.mul(rt.call_function('floor', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('min_price'))]).to_f64() / var_step,
		]), var_step) } else { var_min_price }
	mut var_current_max_price := if rt.get_superglobal('_GET').array_isset(rt.new_string('max_price')) { rt.mul(rt.call_function('ceil', [
			rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('max_price'))]).to_f64() / var_step,
		]), var_step) } else { var_max_price }
	this.widget_start(var_args_mutated.clone(), var_instance.clone())
	...
	])))
	{
		mut var_form_action := rt.call_function('remove_query_arg', [
			rt.create_array([rt.ArrayItem{ key: none, val: 'page' },
				rt.ArrayItem{ key: none, val: 'paged' }, rt.ArrayItem{
					key: none
					val: 'product-page'
				}]),
			rt.call_function('add_query_arg', [rt.get_property(var_wp, 'query_string'),
				rt.new_string(''), rt.call_function('home_url', [
		...
		var_form_action = rt.call_function('preg_replace', [
			...
			rt.call_function('home_url', [
				rt.call_function('trailingslashit', [rt.get_property(var_wp, 'request')]),
			]),
		])
	}
	rt.call_function('wc_get_template', [
		rt.new_string('content-widget-price-filter.php'),
		rt.create_array([rt.ArrayItem{ key: 'form_action', val: var_form_action },
			rt.ArrayItem{ key: 'step', val: var_step }, rt.ArrayItem{
				key: 'min_price'
				val: var_min_price
			}, rt.ArrayItem{ key: 'max_price', val: var_max_price },
			rt.ArrayItem{ key: 'current_min_price', val: var_current_min_price },
			rt.ArrayItem{ key: 'current_max_price', val: var_current_max_price }]),
	])
	this.widget_end(var_args_mutated.clone())
}

fn (mut this Class_WC_Widget_Price_Filter) get_filtered_price() rt.PhpVal {
	mut var_wpdb := rt.new_null()
	mut var_args := rt.get_property(rt.call_method(rt.get_property(rt.call_function('WC',
		[]rt.PhpVal{}), 'query'), 'get_main_query', []rt.PhpVal{}), 'query_vars')
	mut var_tax_query := if var_args.array_isset(rt.new_string('tax_query')) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_post_type_archive', [rt.new_string('product')])))))
		&& !(!rt.is_true(var_args.array_get(rt.new_string('taxonomy'))))
		&& !(!rt.is_true(var_args.array_get(rt.new_string('term')))) {
		var_tax_query.array_push(rt.call_method(rt.get_property(rt.call_function('WC',
			[]rt.PhpVal{}), 'query'), 'get_main_tax_query', []rt.PhpVal{}))
	}
	...
	for {
		...
		if !(!rt.is_true(var_query.array_get(rt.new_string('price_filter'))))
			|| !(!rt.is_true(var_query.array_get(rt.new_string('rating_filter')))) {
	...
	mut iife_temp_7 := Class_WC_Query{}
	mut iife_result_7 := iife_temp_7.get_main_search_query_sql()
	mut var_search := iife_result_7
	...
	mut var_search_query_sql := rt.new_string((if rt.is_true(var_search) {
		' AND ' + var_search.str()
	} else {
		''
	}).str())
	mut var_sql := rt.new_string((
		...
		(var_tax_query_sql.array_get(rt.new_string('join'))).str() + (var_meta_query_sql.array_get(rt.new_string('join'))).str() + rt.concat(rt.concat(rt.new_string('\n\t\t\t\tWHERE '), rt.get_property(var_wpdb, 'posts')), rt.new_string(".post_type IN ('")) + (rt.call_function('implode', [rt.new_string("','"), rt.call_function('array_map', [rt.new_string('esc_sql'), rt.call_function('apply_filters', [rt.new_string('woocommerce_price_filter_post_type'), rt.create_array([rt.ArrayItem{
		key: none
		val: 'product'
	}])])])])).str() +
		rt.concat(rt.concat(rt.new_string("')\n\t\t\t\tAND "), rt.get_property(var_wpdb, 'posts')), rt.new_string(".post_status = 'publish'\n\t\t\t\t")) +
		(var_tax_query_sql.array_get(rt.new_string('where'))).str() + (var_meta_query_sql.array_get(rt.new_string('where'))).str() + var_search_query_sql.str() +
		'\n\t\t\t)').str())
	var_sql = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_price_filter_sql'),
		var_sql.clone(),
		var_meta_query_sql.clone(),
		var_tax_query_sql.clone(),
	])
