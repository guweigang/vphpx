...
fn (mut this Class_WC_Widget_Product_Categories) construct() {
	this.dispatch_set_prop('widget_cssclass',
		rt.new_string('woocommerce widget_product_categories'))
	this.dispatch_set_prop('widget_description', rt.call_function('__', [
		rt.new_string('A list or dropdown of product categories.'),
	...
	this.dispatch_set_prop('settings', rt.create_array([
		...
		rt.ArrayItem{ key: 'orderby', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'std', val: 'name' },
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Order by'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'options', val: rt.create_array([
				rt.ArrayItem{ key: 'order', val: rt.call_function('__', [
					rt.new_string('Category order'),
					rt.new_string('woocommerce'),
				]) },
		...
		rt.ArrayItem{ key: 'dropdown', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'checkbox' },
			rt.ArrayItem{ key: 'std', val: 0 },
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Show as dropdown'),
				rt.new_string('woocommerce'),
			]) },
		]) },
		rt.ArrayItem{ key: 'count', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'std', val: 0 },
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Show product counts'),
				rt.new_string('woocommerce'),
			]) },
		]) },
		rt.ArrayItem{ key: 'hierarchical', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'checkbox' },
			rt.ArrayItem{ key: 'std', val: 1 },
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Show hierarchy'),
				rt.new_string('woocommerce'),
			]) },
		]) },
		rt.ArrayItem{ key: 'show_children_only', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'checkbox' },
			rt.ArrayItem{ key: 'std', val: 0 },
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Only show children of the current category'),
				rt.new_string('woocommerce'),
			]) },
		]) },
		rt.ArrayItem{ key: 'hide_empty', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'checkbox' },
			rt.ArrayItem{ key: 'std', val: 0 },
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Hide empty categories'),
				rt.new_string('woocommerce'),
			]) },
		]) },
		rt.ArrayItem{ key: 'max_depth', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'text' },
			rt.ArrayItem{ key: 'std', val: '' },
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Maximum depth'),
				rt.new_string('woocommerce'),
			]) },
		]) },
	]))
...
fn (mut this Class_WC_Widget_Product_Categories) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	mut var_count := if var_instance.array_isset(rt.new_string('count')) { var_instance.array_get(rt.new_string('count')) } else { rt.get_property(rt.new_object('WC_Widget_Product_Categories', [
			'WC_Widget',
		], &this), 'settings').array_get(rt.new_string('count')).array_get(rt.new_string('std')) }
	mut var_hierarchical := if var_instance.array_isset(rt.new_string('hierarchical')) {
		var_instance.array_get(rt.new_string('hierarchical'))
	} else {
		rt.get_property(rt.new_object('WC_Widget_Product_Categories', ['WC_Widget'], &this),
			'settings').array_get(rt.new_string('hierarchical')).array_get(rt.new_string('std'))
	}
	mut var_show_children_only := if var_instance.array_isset(rt.new_string('show_children_only')) {
		var_instance.array_get(rt.new_string('show_children_only'))
	} else {
		rt.get_property(rt.new_object('WC_Widget_Product_Categories', ['WC_Widget'], &this),
			'settings').array_get(rt.new_string('show_children_only')).array_get(rt.new_string('std'))
	}
	mut var_dropdown := if var_instance.array_isset(rt.new_string('dropdown')) { var_instance.array_get(rt.new_string('dropdown')) } else { rt.get_property(rt.new_object('WC_Widget_Product_Categories', [
			'WC_Widget',
		], &this), 'settings').array_get(rt.new_string('dropdown')).array_get(rt.new_string('std')) }
	mut var_orderby := if var_instance.array_isset(rt.new_string('orderby')) { var_instance.array_get(rt.new_string('orderby')) } else { rt.get_property(rt.new_object('WC_Widget_Product_Categories', [
			'WC_Widget',
		], &this), 'settings').array_get(rt.new_string('orderby')).array_get(rt.new_string('std')) }
	mut var_hide_empty := if var_instance.array_isset(rt.new_string('hide_empty')) {
		var_instance.array_get(rt.new_string('hide_empty'))
	} else {
		rt.get_property(rt.new_object('WC_Widget_Product_Categories', ['WC_Widget'], &this),
			'settings').array_get(rt.new_string('hide_empty')).array_get(rt.new_string('std'))
	}
	...
	mut var_list_args := {
		...
		'taxonomy':     rt.new_string('product_cat')
		'hide_empty':   var_hide_empty
	}
	mut var_max_depth := rt.call_function('absint', [if var_instance.array_isset(rt.new_string('max_depth')) {
		var_instance.array_get(rt.new_string('max_depth'))
	} else {
		rt.get_property(rt.new_object('WC_Widget_Product_Categories', [
			'WC_Widget',
		], &this), 'settings').array_get(rt.new_string('max_depth')).array_get(rt.new_string('std'))
	}])
	var_list_args['menu_order'] = rt.new_bool(false)
	var_dropdown_args['depth'] = var_max_depth.clone()
	var_list_args['depth'] = var_max_depth.clone()
	...
	if rt.is_true(rt.call_function('is_tax', [rt.new_string('product_cat')])) {
		...
		this.cat_ancestors = rt.call_function('get_ancestors', [
			rt.get_property(this.current_cat, 'term_id'),
		...
		mut var_terms := rt.call_function('wc_get_product_terms', [
			...
			rt.call_function('apply_filters', [
				rt.new_string('woocommerce_product_categories_widget_product_terms_args'),
		...
		if rt.is_true(var_terms) {
			mut var_main_term := rt.call_function('apply_filters', [
				rt.new_string('woocommerce_product_categories_widget_main_term'),
				var_terms.array_get(rt.new_int(0)),
				var_terms.clone(),
			])
			this.current_cat = var_main_term.clone()
			this.cat_ancestors = rt.call_function('get_ancestors', [
				rt.get_property(var_main_term, 'term_id'),
				rt.new_string('product_cat'),
			])
		}
	}
	if rt.is_true(var_show_children_only) && rt.is_true(this.current_cat) {
		if rt.is_true(var_hierarchical) {
			mut var_include := rt.call_function('array_merge', [this.cat_ancestors,
				rt.create_array([
					rt.ArrayItem{ key: none, val: rt.get_property(this.current_cat, 'term_id') },
				]),
				...
				rt.call_function('get_terms', [
					rt.new_string('product_cat'),
					rt.create_array([rt.ArrayItem{ key: 'fields', val: 'ids' },
						rt.ArrayItem{ key: 'parent', val: rt.get_property(this.current_cat,
							'term_id') }, rt.ArrayItem{ key: 'hierarchical', val: true },
						rt.ArrayItem{ key: 'hide_empty', val: false }]),
				])])
			if rt.is_true(this.cat_ancestors) {
				...
				for {
					...
					var_include = rt.call_function('array_merge', [
						var_include.clone(),
						rt.call_function('get_terms', [
							rt.new_string('product_cat'),
							rt.create_array([rt.ArrayItem{ key: 'fields', val: 'ids' },
								rt.ArrayItem{ key: 'parent', val: var_ancestor },
			...
			var_include = rt.call_function('get_terms', [rt.new_string('product_cat'),
				rt.create_array([rt.ArrayItem{ key: 'fields', val: 'ids' },
					rt.ArrayItem{ key: 'parent', val: rt.get_property(this.current_cat, 'term_id') },
					rt.ArrayItem{ key: 'hierarchical', val: true },
					rt.ArrayItem{ key: 'hide_empty', val: false }])])
		}
		var_list_args['include'] = rt.call_function('implode', [
			rt.new_string(','), var_include.clone()])
		var_dropdown_args['include'] = var_list_args['include']
	...
	if rt.is_true(var_dropdown) {
		rt.call_function('wc_product_dropdown_categories', [
			rt.call_function('apply_filters', [
				rt.new_string('woocommerce_product_categories_widget_dropdown_args'),
				rt.call_function('wp_parse_args', [
					rt.create_array_from_native_map(var_dropdown_args),
					rt.create_array([rt.ArrayItem{ key: 'show_count', val: var_count },
						rt.ArrayItem{ key: 'hierarchical', val: var_hierarchical },
						rt.ArrayItem{ key: 'show_uncategorized', val: 0 },
						rt.ArrayItem{
							key: 'selected'
							val: if rt.is_true(this.current_cat) {
								rt.get_property(this.current_cat, 'slug')
							} else {
								rt.new_string('')
							}
						}]),
		...
		mut var_handle := rt.new_string('wc-product-category-dropdown-widget')
		...
		rt.call_function('wp_add_inline_script', [var_handle.clone(),
			rt.new_string(
				...
				(rt.call_function('esc_js', [rt.call_function('home_url', [rt.new_string('/')])])).str() +
				"';\n\t\t\t\t\t\t\tconst url = new URL( homeUrl, window.location.origin );\n\t\t\t\t\t\t\turl.searchParams.set( 'product_cat', categoryValue );\n\t\t\t\t\t\t\tlocation.href = url.toString();\n\t\t\t\t\t\t} else {\n\t\t\t\t\t\t\tlocation.href = '" +
				(rt.call_function('esc_js', [rt.call_function('wc_get_page_permalink', [rt.new_string('shop')])])).str() +
				"';\n\t\t\t\t\t\t}\n\t\t\t\t\t});\n\t\n\t\t\t\t\tif ( jQuery().selectWoo ) {\n\t\t\t\t\t\tvar wc_product_cat_select = function() {\n\t\t\t\t\t\t\tjQuery( '.dropdown_product_cat' ).selectWoo( {\n\t\t\t\t\t\t\t\tplaceholder: '" +
				(rt.call_function('esc_js', [rt.call_function('__', [rt.new_string('Select a category'), rt.new_string('woocommerce')])])).str() +
		...
		rt.include_file(
			...
			'/includes/walkers/class-wc-product-cat-list-walker.php', '2')
		...
		var_list_args['show_option_none'] = rt.call_function('__', [
			rt.new_string('No product categories exist.'),
			rt.new_string('woocommerce'),
		])
		var_list_args['current_category'] = if rt.is_true(this.current_cat) {
			rt.get_property(this.current_cat, 'term_id')
		} else {
			rt.new_string('')
		}
		var_list_args['current_category_ancestors'] = this.cat_ancestors
		var_list_args['max_depth'] = var_max_depth.clone()
		print('<ul class="product-categories">')
		rt.call_function('wp_list_categories', [
			rt.call_function('apply_filters', [
				rt.new_string('woocommerce_product_categories_widget_args'),
				rt.create_array_from_native_map(var_list_args),
			]),
		])
		print('</ul>')
	}
...
fn (mut this Class_WC_Widget_Product_Categories) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'cat_ancestors' {
			this.cat_ancestors = val
			return true
		}
		'current_cat' {
			this.current_cat = val
			return true
		}
		else {
