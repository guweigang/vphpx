...
fn (mut this Class_WC_Widget_Brand_Thumbnails) construct() {
	this.woo_widget_name = rt.call_function('__', [
		rt.new_string('WooCommerce Brand Thumbnails'),
		rt.new_string('woocommerce'),
	])
	this.woo_widget_description = rt.call_function('__', [
		rt.new_string('Show a grid of brand thumbnails.'),
		rt.new_string('woocommerce'),
	])
...
fn (mut this Class_WC_Widget_Brand_Thumbnails) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	var_instance_mutated = rt.call_function('wp_parse_args', [
		...
		rt.create_array([
			...
			rt.ArrayItem{ key: 'columns', val: 1 },
			rt.ArrayItem{ key: 'exclude', val: '' },
	...
	mut var_exclude := rt.call_function('array_map', [rt.new_string('intval'),
		rt.call_function('explode',
			[rt.new_string(','), var_instance_mutated.array_get(rt.new_string('exclude'))])])
	...
	mut var_brands := rt.call_function('get_terms', [
		rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: 'product_brand' },
			rt.ArrayItem{
				key: 'hide_empty'
				val: var_instance_mutated.array_get(rt.new_string('hide_empty'))
			}, rt.ArrayItem{
				key: 'orderby'
				val: var_instance_mutated.array_get(rt.new_string('orderby'))
			}, rt.ArrayItem{ key: 'exclude', val: var_exclude },
			rt.ArrayItem{
				key: 'number'
				val: var_instance_mutated.array_get(rt.new_string('number'))
	...
	mut var_title := rt.call_function('apply_filters', [rt.new_string('widget_title'),
		...
		var_instance_mutated.clone(), rt.new_string(this.woo_widget_idbase)])
	...
	rt.call_function('wc_get_template', [rt.new_string('widgets/brand-thumbnails.php'),
		rt.create_array([rt.ArrayItem{ key: 'brands', val: var_brands },
			rt.ArrayItem{
				key: 'columns'
				val: rt.new_int((var_instance_mutated.array_get(rt.new_string('columns'))).to_i64())
			}, rt.ArrayItem{
				key: 'fluid_columns'
				val: if !(!rt.is_true(var_instance_mutated.array_get(rt.new_string('fluid_columns')))) {
					true
				} else {
					false
				}
...
fn (mut this Class_WC_Widget_Brand_Thumbnails) update(var_new_instance rt.PhpVal, var_old_instance rt.PhpVal) rt.PhpVal {
	...
	var_instance.array_set('title', rt.call_function('wp_strip_all_tags', [
		rt.call_function('stripslashes', [var_new_instance.array_get(rt.new_string('title'))]),
	]))
	var_instance.array_set('columns', rt.call_function('wp_strip_all_tags', [
		rt.call_function('stripslashes', [var_new_instance.array_get(rt.new_string('columns'))]),
	]))
	var_instance.array_set('fluid_columns', if !(!rt.is_true(var_new_instance.array_get(rt.new_string('fluid_columns')))) {
		true
	} else {
		false
	})
	var_instance.array_set('orderby', rt.call_function('wp_strip_all_tags', [
		rt.call_function('stripslashes', [var_new_instance.array_get(rt.new_string('orderby'))]),
	]))
	var_instance.array_set('exclude', rt.call_function('wp_strip_all_tags', [
		rt.call_function('stripslashes', [var_new_instance.array_get(rt.new_string('exclude'))]),
	]))
	var_instance.array_set('hide_empty', rt.call_function('wp_strip_all_tags', [
		rt.call_function('stripslashes', [
			rt.new_string((var_new_instance.array_get(rt.new_string('hide_empty'))).str()),
		]),
	]))
	var_instance.array_set('number', rt.call_function('wp_strip_all_tags', [
		rt.call_function('stripslashes', [var_new_instance.array_get(rt.new_string('number'))]),
	]))
	if rt.is_true(rt.new_bool(!(rt.is_true(var_instance.array_get(rt.new_string('columns')))))) {
		var_instance.array_set('columns', 1)
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_instance.array_get(rt.new_string('orderby')))))) {
		var_instance.array_set('orderby', 'name')
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_instance.array_get(rt.new_string('exclude')))))) {
		var_instance.array_set('exclude', '')
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_instance.array_get(rt.new_string('hide_empty')))))) {
		var_instance.array_set('hide_empty', 0)
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_instance.array_get(rt.new_string('number')))))) {
...
fn (mut this Class_WC_Widget_Brand_Thumbnails) form(var_instance rt.PhpVal) {
	...
	if !rt.is_true(var_instance_mutated.array_get(rt.new_string('fluid_columns'))) {
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Columns:'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [this.get_field_id(rt.new_string('columns'))]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [
		this.get_field_name(rt.new_string('columns')),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(if var_instance_mutated.array_isset(rt.new_string('columns')) { rt.call_function('esc_attr', [
			var_instance_mutated.array_get(rt.new_string('columns')),
		]) } else { rt.new_string('1') })
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [
		this.get_field_id(rt.new_string('fluid_columns')),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Fluid columns:'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('checked', [var_instance_mutated.array_get(rt.new_string('fluid_columns'))])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [
		this.get_field_id(rt.new_string('fluid_columns')),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [
		this.get_field_name(rt.new_string('fluid_columns')),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [this.get_field_id(rt.new_string('number'))]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Number:'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [this.get_field_id(rt.new_string('number'))]))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Exclude:'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [this.get_field_id(rt.new_string('exclude'))]))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [
		this.get_field_id(rt.new_string('hide_empty')),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Hide empty brands:'),
		rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [
		this.get_field_id(rt.new_string('hide_empty')),
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [
		this.get_field_name(rt.new_string('hide_empty')),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('selected', [var_instance_mutated.array_get(rt.new_string('hide_empty')),
		rt.new_int(1)])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('Yes'), rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('selected', [var_instance_mutated.array_get(rt.new_string('hide_empty')),
		rt.new_int(0)])
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('esc_html_e', [rt.new_string('No'), rt.new_string('woocommerce')])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [this.get_field_id(rt.new_string('orderby'))]))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [
		this.get_field_name(rt.new_string('orderby')),
	]))
	// unsupported statement: Stmt_InlineHTML
