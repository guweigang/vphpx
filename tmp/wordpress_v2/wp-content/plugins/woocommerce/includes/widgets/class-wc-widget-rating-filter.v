...
fn (mut this Class_WC_Widget_Rating_Filter) construct() {
	...
	this.dispatch_set_prop('widget_description', rt.call_function('__', [
		rt.new_string('Display a list of star ratings to filter products in your store.'),
	...
	this.dispatch_set_prop('widget_name', rt.call_function('__', [
		rt.new_string('Filter Products by Rating'),
		rt.new_string('woocommerce'),
	]))
...
fn (mut this Class_WC_Widget_Rating_Filter) get_filtered_product_count(var_rating rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		if !(!rt.is_true(var_query.array_get(rt.new_string('rating_filter')))) {
	...
	var_tax_query.array_push(rt.create_array([
		...
		rt.ArrayItem{ key: 'terms', val: var_product_visibility_terms.array_get(rt.new_string(
			'rated-' + var_rating_mutated.str())) },
	...
	mut var_sql := rt.new_string((rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('SELECT COUNT( DISTINCT '), rt.get_property(var_wpdb,
		'posts')), rt.new_string('.ID ) FROM ')), rt.get_property(var_wpdb, 'posts')),
		rt.new_string(' '))).str())
	...
	var_sql = rt.concat(var_sql, rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string(' WHERE '), rt.get_property(var_wpdb,
		'posts')), rt.new_string(".post_type = 'product' AND ")),
		rt.get_property(var_wpdb, 'posts')), rt.new_string(".post_status = 'publish' ")))
	var_sql = rt.concat(var_sql, rt.new_string(
		(var_tax_query_sql.array_get(rt.new_string('where'))).str() +
		(var_meta_query_sql.array_get(rt.new_string('where'))).str()))
	mut iife_temp_2 := Class_WC_Query{}
	mut iife_result_2 := iife_temp_2.get_main_search_query_sql()
	mut var_search := iife_result_2
	if rt.is_true(var_search) {
		var_sql = rt.concat(var_sql, rt.new_string(' AND ' + var_search.str()))
	}
	return rt.call_function('absint', [
		rt.call_method(var_wpdb, 'get_var', [var_sql.clone()]),
	])
	return rt.new_null()
}
...
fn (mut this Class_WC_Widget_Rating_Filter) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.get_property(rt.call_method(rt.get_property(rt.call_function('WC',
		[]rt.PhpVal{}), 'query'), 'get_main_query', []rt.PhpVal{}), 'post_count')))))
	{
		return
	}
	...
	mut var_base_link := rt.call_function('remove_query_arg', [
		rt.new_string('paged'), this.get_current_page_url()])
	this.widget_start(var_args.clone(), var_instance.clone())
	print('<ul>')
	mut var_rating := rt.new_int(5)
	for {
		if !(rt.is_true(rt.greater_equal(var_rating, rt.new_int(1)))) { break
		 }
		mut var_count := this.get_filtered_product_count(var_rating.clone())
		if !rt.is_true(var_count) {
			continue
		}
		var_found = rt.new_bool(true)
		mut var_link := var_base_link.clone()
		if rt.is_true(rt.call_function('in_array', [var_rating.clone(),
			var_rating_filter.clone(), rt.new_bool(true)]))
		{
			...
			var_link_ratings = rt.call_function('implode', [rt.new_string(','),
				rt.call_function('array_merge', [var_rating_filter.clone(),
					rt.create_array([rt.ArrayItem{ key: none, val: var_rating }])])])
		}
		...
		{ 'wc-layered-nav-rating chosen' } else { 'wc-layered-nav-rating' }).str())
		var_link = rt.call_function('apply_filters', [
			rt.new_string('woocommerce_rating_filter_link'),
			if rt.is_true(var_link_ratings) { rt.call_function('add_query_arg', [
		...
		mut var_rating_html := rt.call_function('wc_get_star_rating_html', [
			var_rating.clone()])
		mut var_count_html := rt.call_function('wp_kses', [
			rt.call_function('apply_filters', [
				rt.new_string('woocommerce_rating_filter_count'),
				rt.new_string('(${var_count.to_string()})'),
				var_count.clone(),
				var_rating.clone(),
			]),
			rt.create_array([
		...
		rt.call_function('printf', [
			rt.new_string('<li class="%s"><a href="%s"><span class="star-rating">%s</span> %s</a></li>'),
