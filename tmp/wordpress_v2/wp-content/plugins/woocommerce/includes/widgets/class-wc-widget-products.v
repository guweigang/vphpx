...
fn (mut this Class_WC_Widget_Products) construct() {
	...
	this.dispatch_set_prop('widget_description', rt.call_function('__', [
		rt.new_string("A list of your store's products."),
	...
	this.dispatch_set_prop('settings', rt.create_array([
		...
		rt.ArrayItem{ key: 'show', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'options', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'featured', val: rt.call_function('__', [
					rt.new_string('Featured products'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'onsale', val: rt.call_function('__', [
					rt.new_string('On-sale products'),
					rt.new_string('woocommerce'),
				]) },
			]) },
		]) },
		rt.ArrayItem{ key: 'orderby', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'options', val: rt.create_array([
				...
				rt.ArrayItem{ key: 'rand', val: rt.call_function('__', [
					rt.new_string('Random'),
					rt.new_string('woocommerce'),
				]) },
				rt.ArrayItem{ key: 'sales', val: rt.call_function('__', [
					rt.new_string('Sales'),
					rt.new_string('woocommerce'),
				]) },
			]) },
		...
		rt.ArrayItem{ key: 'order', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'select' },
			rt.ArrayItem{ key: 'std', val: 'desc' },
			rt.ArrayItem{ key: 'label', val: rt.call_function('_x', [
				rt.new_string('Order'),
				rt.new_string('Sorting order'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'options', val: rt.create_array([
				rt.ArrayItem{ key: 'asc', val: rt.call_function('__', [
					rt.new_string('ASC'),
					rt.new_string('woocommerce'),
				]) },
		...
		rt.ArrayItem{ key: 'hide_free', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'checkbox' },
			rt.ArrayItem{ key: 'std', val: 0 },
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Hide free products'),
				rt.new_string('woocommerce'),
			]) },
		]) },
		rt.ArrayItem{ key: 'show_hidden', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'checkbox' },
			rt.ArrayItem{ key: 'std', val: 0 },
			rt.ArrayItem{ key: 'label', val: rt.call_function('__', [
				rt.new_string('Show hidden products'),
				rt.new_string('woocommerce'),
			]) },
		]) },
	]))
...
fn (mut this Class_WC_Widget_Products) get_products(var_args rt.PhpVal, var_instance rt.PhpVal) rt.PhpVal {
	mut var_number := if !(!rt.is_true(var_instance.array_get(rt.new_string('number')))) {
		rt.call_function('absint', [var_instance.array_get(rt.new_string('number'))])
	} else {
		rt.get_property(rt.new_object('WC_Widget_Products', ['WC_Widget'], &this), 'settings').array_get(rt.new_string('number')).array_get(rt.new_string('std'))
	}
	mut var_show := if !(!rt.is_true(var_instance.array_get(rt.new_string('show')))) {
		rt.call_function('sanitize_title', [var_instance.array_get(rt.new_string('show'))])
	} else {
		rt.get_property(rt.new_object('WC_Widget_Products', ['WC_Widget'], &this), 'settings').array_get(rt.new_string('show')).array_get(rt.new_string('std'))
	}
	mut var_orderby := if !(!rt.is_true(var_instance.array_get(rt.new_string('orderby')))) {
		rt.call_function('sanitize_title', [var_instance.array_get(rt.new_string('orderby'))])
	} else {
		rt.get_property(rt.new_object('WC_Widget_Products', ['WC_Widget'], &this), 'settings').array_get(rt.new_string('orderby')).array_get(rt.new_string('std'))
	}
	mut var_order := if !(!rt.is_true(var_instance.array_get(rt.new_string('order')))) {
		rt.call_function('sanitize_title', [var_instance.array_get(rt.new_string('order'))])
	} else {
		rt.get_property(rt.new_object('WC_Widget_Products', ['WC_Widget'], &this), 'settings').array_get(rt.new_string('order')).array_get(rt.new_string('std'))
	}
	...
	if !rt.is_true(var_instance.array_get(rt.new_string('show_hidden'))) {
		var_query_args.array_get_mut('tax_query').array_push(rt.create_array([
			...
			rt.ArrayItem{
				...
				val: if rt.is_true(rt.call_function('is_search', []rt.PhpVal{})) {
					var_product_visibility_term_ids.array_get(rt.new_string('exclude-from-search'))
				} else {
					var_product_visibility_term_ids.array_get(rt.new_string('exclude-from-catalog'))
				}
			},
	...
	if !(!rt.is_true(var_instance.array_get(rt.new_string('hide_free')))) {
		var_query_args.array_get_mut('meta_query').array_push(rt.create_array([
	...
	])))
	{
		var_query_args.array_get_mut('tax_query').array_push(rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{
					key: 'terms'
					val: var_product_visibility_term_ids.array_get(Class_Automattic_WooCommerce_Enums_ProductStockStatus.out_of_stock())
				},
				rt.ArrayItem{ key: 'operator', val: 'NOT IN' },
			]) },
		]))
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('featured'))) {
		...
		var_query_args['post__in'] = var_product_ids_on_sale.clone()
	}
	...
	return rt.new_object('WP_Query', []string{}, create_wp_query(rt.call_function('apply_filters', [
		rt.new_string('woocommerce_products_widget_query_args'),
...
fn (mut this Class_WC_Widget_Products) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	rt.call_function('wc_set_loop_prop', [rt.new_string('name'),
		rt.new_string('widget')])
	mut var_products := this.get_products(var_args.clone(), var_instance.clone())
	if rt.is_true(var_products)
		&& rt.is_true(rt.call_method(var_products, 'have_posts', []rt.PhpVal{})) {
		...
		mut var_template_args := {
			'widget_id':   if var_args.array_isset(rt.new_string('widget_id')) { var_args.array_get(rt.new_string('widget_id')) } else { rt.get_property(rt.new_object('WC_Widget_Products', [
					'WC_Widget',
				], &this), 'widget_id') }
	...
	rt.echo_val(this.cache_widget(var_args.clone(), rt.call_function('ob_get_clean', []rt.PhpVal{})))
}
