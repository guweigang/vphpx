...
fn (mut this Class_WC_Order_Item_Shipping) calculate_taxes(var_calculate_tax_for rt.PhpVal) bool {
	if !(var_calculate_tax_for.array_isset(rt.new_string('country'))
		&& var_calculate_tax_for.array_isset(rt.new_string('state'))
		&& var_calculate_tax_for.array_isset(rt.new_string('postcode'))
		&& var_calculate_tax_for.array_isset(rt.new_string('city'))
		&& var_calculate_tax_for.array_isset(rt.new_string('tax_class'))) {
		return false
	}
	if rt.is_true(rt.call_function('wc_tax_enabled', []rt.PhpVal{}))
		&& rt.is_true(rt.identical(Class_Automattic_WooCommerce_Enums_ProductTaxStatus.taxable(), this.get_tax_status(''))) {
		mut iife_temp_0 := Class_WC_Tax{}
		mut iife_result_0 := iife_temp_0.find_shipping_rates(var_calculate_tax_for.clone())
		mut var_tax_rates := iife_result_0
		mut iife_temp_1 := Class_WC_Tax{}
		mut iife_result_1 := iife_temp_1.calc_tax(this.get_total(''), var_tax_rates.clone(),
			rt.new_bool(false))
		mut var_taxes := iife_result_1
		this.set_taxes(rt.create_array([rt.ArrayItem{ key: 'total', val: var_taxes }]))
	} else {
		this.set_taxes(rt.new_bool(false))
	}
	rt.call_function('do_action', [
		rt.new_string('woocommerce_order_item_shipping_after_calculate_taxes'),
		rt.new_object('WC_Order_Item_Shipping', ['WC_Order_Item'], &this),
		var_calculate_tax_for.clone(),
	])
	return true
}

fn (mut this Class_WC_Order_Item_Shipping) set_name(var_value rt.PhpVal) {
	this.set_method_title(var_value.clone())
}

fn (mut this Class_WC_Order_Item_Shipping) set_method_title(var_value rt.PhpVal) {
	this.set_prop(rt.new_string('name'), rt.call_function('wc_clean', [
		var_value.clone()]))
	this.set_prop(rt.new_string('method_title'), rt.call_function('wc_clean', [
		var_value.clone(),
	]))
}

fn (mut this Class_WC_Order_Item_Shipping) set_method_id(var_value rt.PhpVal) {
	this.set_prop(rt.new_string('method_id'), rt.call_function('wc_clean', [
		var_value.clone()]))
}

fn (mut this Class_WC_Order_Item_Shipping) set_instance_id(var_value rt.PhpVal) {
	this.set_prop(rt.new_string('instance_id'), rt.call_function('wc_clean', [
		var_value.clone()]))
}

fn (mut this Class_WC_Order_Item_Shipping) set_total(var_value rt.PhpVal) {
	this.set_prop(rt.new_string('total'), rt.call_function('wc_format_decimal', [
		var_value.clone(),
	]))
}
...
fn (mut this Class_WC_Order_Item_Shipping) set_taxes(var_raw_tax_data rt.PhpVal) {
	...
	if var_raw_tax_data_mutated.array_isset(rt.new_string('total')) {
		...
		if !(var_total.clone().is_array()) {
			...
			rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'warning', [
				...
				rt.create_array([
					rt.ArrayItem{ key: 'source', val: 'woocommerce-order-item-shipping' },
	...
	} else if !(!rt.is_true(var_raw_tax_data_mutated))
		&& var_raw_tax_data_mutated.clone().is_array() {
		var_tax_data['total'] = rt.call_function('array_map', [
			rt.new_string('wc_format_decimal'),
			var_raw_tax_data_mutated.clone(),
		])
	...
	])))
	{
		...
		this.set_total_tax(iife_result_2)
	} else {
		mut iife_temp_3 := Class_Automattic_WooCommerce_Utilities_NumberUtil{}
		mut iife_result_3 := iife_temp_3.array_sum(rt.call_function('array_map', [
...
fn (mut this Class_WC_Order_Item_Shipping) get_method_title(context string) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_string('view'), rt.new_string(context))) {
		return if rt.is_true(var_method_title) { var_method_title } else { rt.call_function('__', [
				rt.new_string('Shipping'),
				rt.new_string('woocommerce'),
			]) }
	} else {
		return var_method_title.clone()
	}
	return rt.new_null()
}
...
fn (mut this Class_WC_Order_Item_Shipping) get_tax_status(context string) rt.PhpVal {
	...
	mut iife_result_4 := iife_temp_4.get_shipping_method(this.get_instance_id(''))
	mut var_shipping_method := iife_result_4
	return if rt.is_true(rt.new_bool(rt.instance_of(var_shipping_method, 'WC_Shipping_Method'))) {
		rt.get_property(var_shipping_method, 'tax_status')
	} else {
...
fn (mut this Class_WC_Order_Item_Shipping) offsetset(var_offset rt.PhpVal, var_value rt.PhpVal) {
	...
	rt.call_function('wc_deprecated_function', [
		rt.new_string('WC_Order_Item_Shipping::offsetSet'),
		rt.new_string('4.4.0'),
		rt.new_string(''),
	])
	if rt.is_true(rt.identical(rt.new_string('cost'), var_offset_mutated)) {
...
fn (mut this Class_WC_Order_Item_Shipping) offsetexists(var_offset rt.PhpVal) bool {
	...
	if rt.is_true(rt.call_function('in_array', [var_offset_mutated.clone(),
		rt.create_array([rt.ArrayItem{ key: none, val: 'cost' }]),
		rt.new_bool(true)]))
	{
		return true
	}
