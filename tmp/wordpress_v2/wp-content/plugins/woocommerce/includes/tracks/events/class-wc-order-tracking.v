...
fn (mut this Class_WC_Order_Tracking) init() {
	rt.call_function('add_action', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Order_Tracking', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'track_order_viewed' },
		]),
	])
}
...
fn (mut this Class_WC_Order_Tracking) track_order_viewed(var_order rt.PhpVal) {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_order, 'WC_Order'))))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_order, 'get_id', []rt.PhpVal{}))))) {
		return
	}
	mut var_properties := {
		'current_status': rt.call_method(var_order, 'get_status', []rt.PhpVal{})
		'date_created':   if rt.is_true(rt.call_method(var_order, 'get_date_created', []rt.PhpVal{})) { rt.call_method(rt.call_method(var_order, 'get_date_created', []rt.PhpVal{}), 'format', [
				Class_DateTime.atom(),
			]) } else { rt.new_string('') }
		'payment_method': rt.call_method(var_order, 'get_payment_method', []rt.PhpVal{})
	}
	mut iife_temp_0 := Class_WC_Tracks{}
	mut iife_result_0 := iife_temp_0.record_event(rt.new_string('single_order_view'),
...
fn create_wc_order_tracking(_args ...rt.PhpVal) &Class_WC_Order_Tracking {
