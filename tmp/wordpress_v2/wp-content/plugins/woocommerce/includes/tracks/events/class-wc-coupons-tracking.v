...
fn (mut this Class_WC_Coupons_Tracking) init() {
	rt.call_function('add_action', [rt.new_string('load-edit.php'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Coupons_Tracking', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'tracks_coupons_events' },
		]),
		rt.new_int(10)])
}

fn (mut this Class_WC_Coupons_Tracking) tracks_coupons_bulk_actions() {
	mut var_handle := rt.new_string('wc-tracks-coupons-bulk-actions')
...
fn (mut this Class_WC_Coupons_Tracking) tracks_coupons_events() {
	if rt.get_superglobal('_GET').array_isset(rt.new_string('post_type'))
		&& rt.is_true(rt.identical(rt.new_string('shop_coupon'), rt.get_superglobal('_GET').array_get(rt.new_string('post_type')))) {
		this.tracks_coupons_bulk_actions()
		mut iife_temp_0 := Class_WC_Tracks{}
		mut iife_result_0 := iife_temp_0.record_event(rt.new_string('coupons_view'), rt.create_array([
			rt.ArrayItem{
				key: 'status'
				val: if rt.get_superglobal('_GET').array_isset(rt.new_string('post_status')) { rt.call_function('sanitize_text_field', [
		...
		if rt.get_superglobal('_GET').array_isset(rt.new_string('filter_action'))
			&& rt.is_true(rt.identical(rt.new_string('Filter'), rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('filter_action'))])])))
			&& rt.get_superglobal('_GET').array_isset(rt.new_string('coupon_type')) {
			mut iife_temp_1 := Class_WC_Tracks{}
			mut iife_result_1 := iife_temp_1.record_event(rt.new_string('coupons_filter'), rt.create_array([
				rt.ArrayItem{ key: 'filter', val: 'coupon_type' },
		...
		if rt.get_superglobal('_GET').array_isset(rt.new_string('s'))
			&& 0 < rt.call_function('sanitize_text_field', [rt.call_function('wp_unslash', [rt.get_superglobal('_GET').array_get(rt.new_string('s'))])]).to_string().len {
			mut iife_temp_2 := Class_WC_Tracks{}
			mut iife_result_2 := iife_temp_2.record_event(rt.new_string('coupons_search'))
		}
	}
}
...
fn create_wc_coupons_tracking(_args ...rt.PhpVal) &Class_WC_Coupons_Tracking {
