...
fn (mut this Class_WC_Admin_Setup_Wizard_Tracking) init() {
	rt.call_function('_deprecated_function', [rt.new_string(@STRUCT + '::' + @FN),
		rt.new_string('4.6.0'),
		rt.call_function('__', [
			rt.new_string('Onboarding is maintained in WooCommerce Admin.'),
...
fn (mut this Class_WC_Admin_Setup_Wizard_Tracking) get_current_step() rt.PhpVal {
	...
	return if rt.get_superglobal('_GET').array_isset(rt.new_string('step')) { rt.call_function('sanitize_key', [
...
fn (mut this Class_WC_Admin_Setup_Wizard_Tracking) dequeue_non_allowed_scripts() {
	...
	for {
		...
		if rt.is_true(rt.call_function('in_array', [var_script.clone(),
			rt.create_array_from_list(var_allowed), rt.new_bool(true)]))
		{
			continue
		}
		rt.call_function('wp_dequeue_script', [var_script.clone()])
	}
}
...
fn (mut this Class_WC_Admin_Setup_Wizard_Tracking) set_obw_steps(var_steps rt.PhpVal) rt.PhpVal {
	...
	this.steps = var_steps.clone()
	return var_steps.clone()
}

fn create_wc_admin_setup_wizard_tracking(_args ...rt.PhpVal) &Class_WC_Admin_Setup_Wizard_Tracking {
