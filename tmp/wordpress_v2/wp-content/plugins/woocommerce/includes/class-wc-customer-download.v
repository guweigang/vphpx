...
fn (mut this Class_WC_Customer_Download) construct(download i64) {
	...
	if rt.new_int(download).is_long() || rt.new_int(download).is_double() && download > 0 {
	...
	} else if rt.new_int(download).is_object()
		&& !(!rt.is_true(rt.get_property(rt.new_int(download), 'permission_id'))) {
	...
	if rt.is_true(rt.greater(this.get_id(), rt.new_int(0))) {
		rt.call_method(rt.get_property(rt.new_object('WC_Customer_Download', [
			'WC_Data',
			'ArrayAccess',
		], &this), 'data_store'), 'read', [
			rt.new_object('WC_Customer_Download', ['WC_Data', 'ArrayAccess'], &this),
		])
	}
}
...
fn (mut this Class_WC_Customer_Download) get_download_count(context string) rt.PhpVal {
	...
	mut var_download_log_count := rt.call_method(var_data_store,
		'get_download_logs_count_for_permission', [this.get_id()])
	mut var_download_count_prop := this.get_prop(rt.new_string('download_count'),
		rt.new_string(context))
	return rt.call_function('max', [var_download_log_count.clone(),
		var_download_count_prop.clone()])
}

fn (mut this Class_WC_Customer_Download) set_download_id(var_value rt.PhpVal) {
	this.set_prop(rt.new_string('download_id'), var_value.clone())
}
...
fn (mut this Class_WC_Customer_Download) set_user_email(var_value rt.PhpVal) {
	this.set_prop(rt.new_string('user_email'), rt.call_function('sanitize_email', [
		var_value.clone(),
	]))
}

fn (mut this Class_WC_Customer_Download) set_order_id(var_value rt.PhpVal) {
	this.set_prop(rt.new_string('order_id'), rt.call_function('absint', [
		var_value.clone()]))
}

fn (mut this Class_WC_Customer_Download) set_order_key(var_value rt.PhpVal) {
	this.set_prop(rt.new_string('order_key'), var_value.clone())
}

fn (mut this Class_WC_Customer_Download) set_downloads_remaining(var_value rt.PhpVal) {
	this.set_prop(rt.new_string('downloads_remaining'), if rt.is_true(rt.identical(rt.new_string(''), var_value)) { rt.new_string('') } else { rt.call_function('absint', [
			var_value.clone(),
		]) })
}

fn (mut this Class_WC_Customer_Download) set_access_granted(var_date rt.PhpVal) {
	this.set_date_prop(rt.new_string('access_granted'), var_date.clone())
}

fn (mut this Class_WC_Customer_Download) set_access_expires(var_date rt.PhpVal) {
	this.set_date_prop(rt.new_string('access_expires'), var_date.clone())
}

fn (mut this Class_WC_Customer_Download) set_download_count(var_value rt.PhpVal) {
	this.set_prop(rt.new_string('download_count'), rt.call_function('absint', [
		var_value.clone(),
	]))
}
...
fn (mut this Class_WC_Customer_Download) track_download(var_user_id rt.PhpVal, var_user_ip_address rt.PhpVal) {
	...
	rt.call_method(var_wpdb, 'query', [var_query.clone()])
	rt.call_method(rt.get_property(rt.new_object('WC_Customer_Download',
		['WC_Data', 'ArrayAccess'], &this), 'data_store'), 'read', [
		rt.new_object('WC_Customer_Download', ['WC_Data', 'ArrayAccess'], &this),
	])
	...
	if !(var_user_id.clone().is_null()) {
		var_download_log.set_user_id(var_user_id.clone())
	}
	if !(var_user_ip_address.clone().is_null()) {
		var_download_log.set_user_ip_address(var_user_ip_address.clone())
	}
...
fn (mut this Class_WC_Customer_Download) offsetget(var_offset rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.call_function('is_callable', [
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Customer_Download', [
				'WC_Data',
				'ArrayAccess',
			], &this) },
	...
	{
		return rt.call_method(rt.new_object('WC_Customer_Download', ['WC_Data', 'ArrayAccess'],
...
fn (mut this Class_WC_Customer_Download) offsetset(var_offset rt.PhpVal, var_value rt.PhpVal) {
	...
	{
		rt.call_method(rt.new_object('WC_Customer_Download', ['WC_Data', 'ArrayAccess'], &this),
...
fn (mut this Class_WC_Customer_Download) offsetunset(var_offset rt.PhpVal) {
	...
	{
		rt.call_method(rt.new_object('WC_Customer_Download', ['WC_Data', 'ArrayAccess'], &this),
			'set_${var_offset.to_string()}', [rt.new_string('')])
	}
}

fn (mut this Class_WC_Customer_Download) offsetexists(var_offset rt.PhpVal) rt.PhpVal {
	return rt.call_function('in_array', [var_offset.clone(), rt.func_array_keys(this.data),
		rt.new_bool(true)])
}

fn (mut this Class_WC_Customer_Download) magic_isset(var_key rt.PhpVal) rt.PhpVal {
	return rt.call_function('in_array', [var_key.clone(), rt.func_array_keys(this.data),
		rt.new_bool(true)])
}
...
fn (mut this Class_WC_Customer_Download) magic_get(var_key rt.PhpVal) rt.PhpVal {
	...
	{
		return rt.call_method(rt.new_object('WC_Customer_Download', ['WC_Data', 'ArrayAccess'],
			&this), 'get_${var_key.to_string()}', [rt.new_string('')])
	}
	return rt.new_null()
}
