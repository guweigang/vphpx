...
fn (mut this Class_WC_Integration_MaxMind_Geolocation) construct() {
	...
	this.dispatch_set_prop('method_title', rt.call_function('__', [
		rt.new_string('MaxMind Geolocation'),
		rt.new_string('woocommerce'),
	]))
	this.dispatch_set_prop('method_description', rt.call_function('__', [
		rt.new_string('An integration for utilizing MaxMind to do Geolocation lookups. Please note that this integration will only do country lookups.'),
		rt.new_string('woocommerce'),
	]))
	this.database_service = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_maxmind_geolocation_database_service'),
		rt.new_null(),
	])
	if rt.is_true(rt.identical(rt.new_null(), this.database_service)) {
		this.database_service =
			create_wc_integration_maxmind_database_service(this.get_database_prefix())
	}
	this.init_form_fields()
	this.init_settings()
	rt.call_function('add_action', [
		rt.new_string('woocommerce_update_options_integration_' +
			rt.get_property(rt.new_object('WC_Integration_MaxMind_Geolocation', ['WC_Integration'], &this), 'id')),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Integration_MaxMind_Geolocation', [
				'WC_Integration',
			], &this) },
	...
	rt.call_function('add_action', [
		rt.new_string('update_option_woocommerce_default_customer_address'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Integration_MaxMind_Geolocation', [
				'WC_Integration',
			], &this) },
			rt.ArrayItem{ key: none, val: 'display_missing_license_key_notice' },
		]),
		rt.new_int(1000),
		rt.new_int(2),
	])
	mut var_bind_updater := rt.call_function('apply_filters_deprecated', [
		rt.new_string('woocommerce_geolocation_update_database_periodically'),
		rt.create_array([rt.ArrayItem{ key: none, val: true }]),
		rt.new_string('3.9.0'),
		rt.new_string('woocommerce_maxmind_geolocation_update_database_periodically'),
	])
	var_bind_updater = rt.call_function('apply_filters', [
		rt.new_string('woocommerce_maxmind_geolocation_update_database_periodically'),
		var_bind_updater.clone(),
	])
	if rt.is_true(var_bind_updater) {
		rt.call_function('add_action', [rt.new_string('woocommerce_geoip_updater'),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('WC_Integration_MaxMind_Geolocation', [
					'WC_Integration',
				], &this) },
				rt.ArrayItem{ key: none, val: 'update_database' },
			])])
	}
	rt.call_function('add_filter', [rt.new_string('woocommerce_get_geolocation'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('WC_Integration_MaxMind_Geolocation', [
				'WC_Integration',
			], &this) },
			rt.ArrayItem{ key: none, val: 'get_geolocation' },
		]),
...
fn (mut this Class_WC_Integration_MaxMind_Geolocation) admin_options() {
	...
	rt.include_file((rt.call_function('dirname', [rt.new_string(@FILE)])).str() +
		'/views/html-admin-options.php', '1')
}
	...
	this.dispatch_set_prop('form_fields', rt.create_array([
		rt.ArrayItem{ key: 'license_key', val: rt.create_array([
			rt.ArrayItem{ key: 'title', val: rt.call_function('__', [
				rt.new_string('MaxMind License Key'),
				rt.new_string('woocommerce'),
			]) },
			rt.ArrayItem{ key: 'type', val: 'password' },
			rt.ArrayItem{ key: 'description', val: rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('The key that will be used when dealing with MaxMind Geolocation services. You can read how to generate one in <a href="%1$s">MaxMind Geolocation Integration documentation</a>.'),
					rt.new_string('woocommerce'),
				]),
				rt.new_string('https://woocommerce.com/document/maxmind-geolocation-integration/'),
			]) },
			rt.ArrayItem{ key: 'desc_tip', val: false },
...
fn (mut this Class_WC_Integration_MaxMind_Geolocation) validate_license_key_field(var_key rt.PhpVal, var_value rt.PhpVal) rt.PhpVal {
	...
	var_value_mutated = this.validate_password_field(var_key.clone(), var_value_mutated.clone())
	if !rt.is_true(var_value_mutated) {
		return var_value_mutated.clone()
	}
	mut var_tmp_database_path := rt.call_method(this.database_service, 'download_database', [
		var_value_mutated.clone(),
	])
	if rt.is_true(rt.call_function('is_wp_error', [var_tmp_database_path.clone()])) {
		mut iife_temp_0 := Class_WC_Admin_Settings{}
		mut iife_result_0 := iife_temp_0.add_error(rt.call_method(var_tmp_database_path,
			'get_error_message', []rt.PhpVal{}))
		rt.throw_exception(rt.new_object('Exception', []string{}, create_exception(rt.call_method(var_tmp_database_path,
			'get_error_message', []rt.PhpVal{}))))
	}
	mut iife_temp_1 := Class_WC_Integration_MaxMind_Geolocation{}
	iife_temp_1.update_database(var_tmp_database_path.clone())
	rt.new_null()
...
fn (mut this Class_WC_Integration_MaxMind_Geolocation) update_database(var_new_database_path rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('WP_Filesystem', []rt.PhpVal{}))))) {
		rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'warning', [
			rt.call_function('__', [
				rt.new_string('Failed to initialise WC_Filesystem API while trying to update the MaxMind Geolocation database.'),
				rt.new_string('woocommerce'),
			]),
		])
		return
	}
	mut var_target_database_path := rt.call_method(this.database_service, 'get_database_path',
		[]rt.PhpVal{})
	if !rt.is_true(var_target_database_path) {
		return
	}
	if rt.is_true(rt.call_method(var_wp_filesystem, 'exists', [
		var_target_database_path.clone()]))
	{
		rt.call_method(var_wp_filesystem, 'delete', [var_target_database_path.clone()])
	}
	if !var_new_database_path.is_null() {
		...
		var_tmp_database_path = rt.call_method(this.database_service, 'download_database', [
			var_license_key.clone(),
		])
		if rt.is_true(rt.call_function('is_wp_error', [var_tmp_database_path.clone()])) {
			rt.call_method(rt.call_function('wc_get_logger', []rt.PhpVal{}), 'notice', [
				rt.call_method(var_tmp_database_path, 'get_error_message', []rt.PhpVal{}),
				rt.create_array([
					rt.ArrayItem{ key: 'source', val: 'maxmind-geolocation' },
				]),
			])
			return
		}
	}
	rt.call_method(var_wp_filesystem, 'move', [var_tmp_database_path.clone(),
		var_target_database_path.clone(), rt.new_bool(true)])
	rt.call_method(var_wp_filesystem, 'delete', [
		rt.call_function('dirname', [var_tmp_database_path.clone()]),
	])
}
...
fn (mut this Class_WC_Integration_MaxMind_Geolocation) get_geolocation(var_data rt.PhpVal, var_ip_address rt.PhpVal) rt.PhpVal {
	if !(!rt.is_true(var_data.array_get(rt.new_string('country')))) {
		return var_data.clone()
	}
	if !rt.is_true(var_ip_address) {
		return var_data.clone()
	}
	mut var_country_code := rt.call_method(this.database_service, 'get_iso_country_code_for_ip', [
		var_ip_address.clone(),
	])
...
fn (mut this Class_WC_Integration_MaxMind_Geolocation) get_database_prefix() rt.PhpVal {
	...
	if !rt.is_true(var_prefix) {
		var_prefix = rt.call_function('wp_generate_password', [
			rt.new_int(32), rt.new_bool(false)])
		this.update_option(rt.new_string('database_prefix'), var_prefix.clone())
	}
	return var_prefix.clone()
}

fn (mut this Class_WC_Integration_MaxMind_Geolocation) add_missing_license_key_notice() {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('WC_Admin_Notices'),
	])))))
	{
		rt.include_file(
			(rt.get_constant('WC_ABSPATH')).str() + 'includes/admin/class-wc-admin-notices.php',
			'2')
	}
	mut iife_temp_2 := Class_WC_Admin_Notices{}
	mut iife_result_2 := iife_temp_2.add_notice(rt.new_string('maxmind_license_key'))
}
...
fn (mut this Class_WC_Integration_MaxMind_Geolocation) remove_missing_license_key_notice() {
	...
	mut iife_result_3 := iife_temp_3.remove_notice(rt.new_string('maxmind_license_key'))
}
...
fn (mut this Class_WC_Integration_MaxMind_Geolocation) display_missing_license_key_notice(var_old_value rt.PhpVal, var_new_value rt.PhpVal) {
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('apply_filters', [
		rt.new_string('woocommerce_maxmind_geolocation_display_notices'),
		rt.new_bool(true),
	])))))
	{
		return
...
fn create_wc_integration_maxmind_database_service(_args ...rt.PhpVal) &Class_WC_Integration_MaxMind_Database_Service {
