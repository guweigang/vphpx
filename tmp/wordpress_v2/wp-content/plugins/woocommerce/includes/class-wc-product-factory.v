...
fn (mut this Class_WC_Product_Factory) get_product(product_id bool, var_deprecated rt.PhpVal) bool {
	...
	product_id_mutated = i64(this.get_product_id(rt.new_bool(product_id_mutated)))
	...
	mut iife_result_0 := iife_temp_0.feature_is_enabled(rt.new_string('product_instance_caching'))
	mut var_use_product_cache := iife_result_0
	if rt.is_true(var_use_product_cache) && !rt.is_true(var_deprecated) {
		...
		mut var_product := rt.call_method(var_product_cache, 'get', [
			rt.new_bool(product_id_mutated).clone()])
	...
	mut var_product_type :=
		Class_WC_Product_Factory.get_product_type(rt.new_bool(product_id_mutated))
	if !(!rt.is_true(var_deprecated)) {
		rt.call_function('wc_deprecated_argument', [rt.new_string('args'),
			rt.new_string('3.0'),
			rt.new_string('Passing args to the product factory is deprecated. If you need to force a type, construct the product class directly.')])
		if var_deprecated.array_isset(rt.new_string('product_type')) {
			var_product_type =
				Class_WC_Product_Factory.get_classname_from_product_type(var_deprecated.array_get(rt.new_string('product_type')))
		}
	}
	mut var_classname := Class_WC_Product_Factory.get_product_classname(rt.new_bool(product_id_mutated),
		var_product_type.clone())
	var_product = rt.create_object_dynamically(var_classname, [
		rt.new_bool(product_id_mutated).clone(), var_deprecated.clone()])
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	}
	if rt.is_true(var_use_product_cache) && !var_product_cache.is_null()
		&& rt.is_true(rt.new_bool(rt.instance_of(var_product, 'WC_Product'))) {
		rt.call_method(var_product_cache, 'set', [var_product.clone()])
		if rt.has_exception() {
			unsafe {
				goto catch_label_1
			}
...
fn Class_WC_Product_Factory.get_product_classname(var_product_id rt.PhpVal, var_product_type rt.PhpVal) rt.PhpVal {
	...
	mut var_classname := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_product_class'),
		Class_WC_Product_Factory.get_classname_from_product_type(var_product_type_mutated.clone()),
		var_product_type_mutated.clone(),
		rt.new_string((if rt.is_true(rt.identical(Class_Automattic_WooCommerce_Enums_ProductType.variation(),
			var_product_type_mutated))
		{
			'product_variation'
		} else {
			'product'
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_classname))))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [var_classname.clone()]))))) {
...
fn Class_WC_Product_Factory.get_product_type(var_product_id rt.PhpVal) rt.PhpVal {
	mut var_product_id_mutated := var_product_id
	mut var_override := rt.call_function('apply_filters', [
		rt.new_string('woocommerce_product_type_query'),
		rt.new_bool(false),
		var_product_id_mutated.clone(),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_override)))) {
		...
		return rt.call_method(iife_result_1, 'get_product_type', [
			var_product_id_mutated.clone()])
	} else {
...
fn Class_WC_Product_Factory.get_classname_from_product_type(var_product_type rt.PhpVal) rt.PhpVal {
	mut var_product_type_mutated := var_product_type
	return if rt.is_true(var_product_type_mutated) {
		'WC_Product_' +(rt.call_function('implode', [rt.new_string('_'), rt.call_function('array_map', [rt.new_string('ucfirst'), rt.call_function('explode', [rt.new_string('-'), var_product_type_mutated.clone()])])])).str()
	} else {
		rt.new_bool(false)
	}
}
...
fn (mut this Class_WC_Product_Factory) get_product_id(var_product rt.PhpVal) bool {
	...
	if rt.is_true(rt.identical(rt.new_bool(false), var_product_mutated)) && !var_post.is_null()
		&& !(rt.get_property(var_post, 'ID')).is_null()
		&& rt.is_true(rt.identical(rt.new_string('product'), rt.call_function('get_post_type', [rt.get_property(var_post, 'ID')]))) {
		return (rt.call_function('absint', [rt.get_property(var_post, 'ID')])).to_bool()
	} else if rt.is_true(rt.new_bool(var_product_mutated.clone().is_long()
		|| var_product_mutated.clone().is_double()))
	{
