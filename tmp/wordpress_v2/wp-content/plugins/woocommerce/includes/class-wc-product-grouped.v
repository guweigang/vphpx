...
fn (mut this Class_WC_Product_Grouped) add_to_cart_text() rt.PhpVal {
	return rt.call_function('apply_filters', [
		...
		rt.call_function('__', [rt.new_string('View products'),
			rt.new_string('woocommerce')]),
		rt.new_object('WC_Product_Grouped', ['WC_Product'], &this),
	])
}
...
fn (mut this Class_WC_Product_Grouped) add_to_cart_description() rt.PhpVal {
	return rt.call_function('apply_filters', [
		...
		rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('View products in the &ldquo;%s&rdquo; group'),
				rt.new_string('woocommerce'),
			]),
			this.get_name(),
		]),
		rt.new_object('WC_Product_Grouped', [
			'WC_Product',
		], &this),
	])
}
...
fn (mut this Class_WC_Product_Grouped) is_on_sale(context string) rt.PhpVal {
	...
	mut var_on_sale := rt.new_bool(false)
	mut iter_1 := var_children.iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_child := item_1.val
		if rt.is_true(rt.call_method(var_child, 'is_purchasable', []rt.PhpVal{}))
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_child, 'has_child', []rt.PhpVal{})))))
			&& rt.is_true(rt.call_method(var_child, 'is_on_sale', []rt.PhpVal{})) {
			var_on_sale = rt.new_bool(true)
			break
		}
	}
	return if rt.is_true(rt.identical(rt.new_string('view'), rt.new_string(context))) { rt.call_function('apply_filters', [
			rt.new_string('woocommerce_product_is_on_sale'),
			var_on_sale.clone(),
			rt.new_object('WC_Product_Grouped', ['WC_Product'], &this),
		]) } else { var_on_sale }
}
...
fn (mut this Class_WC_Product_Grouped) get_price_html(price string) rt.PhpVal {
	...
	for {
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), rt.call_method(var_child,
			'get_price', []rt.PhpVal{})))))
		{
			var_child_prices << if rt.is_true(rt.identical(rt.new_string('incl'), var_tax_display_mode)) { rt.call_function('wc_get_price_including_tax', [
					var_child.clone(),
	...
	if !(!rt.is_true(var_child_prices)) {
		mut var_min_price := rt.call_function('min', [
			rt.create_array_from_list(var_child_prices),
		])
		mut var_max_price := rt.call_function('max', [
			rt.create_array_from_list(var_child_prices),
		])
	} else {
		var_min_price = rt.new_string('')
		var_max_price = rt.new_string('')
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_min_price)))) {
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_min_price, var_max_price)))) {
			price_mutated = (rt.call_function('wc_format_price_range', [
				var_min_price.clone(), var_max_price.clone()])).str()
		} else {
			price_mutated = (rt.call_function('wc_price', [var_min_price.clone()])).str()
		}
		mut var_is_free := rt.new_bool(rt.is_true(rt.identical(rt.new_int(0), var_min_price))
			&& rt.is_true(rt.identical(rt.new_int(0), var_max_price)))
		if rt.is_true(var_is_free) {
			price_mutated = (rt.call_function('apply_filters', [
				rt.new_string('woocommerce_grouped_free_price_html'),
				rt.call_function('__', [rt.new_string('Free!'),
					rt.new_string('woocommerce')]),
				rt.new_object('WC_Product_Grouped', ['WC_Product'], &this),
			])).str()
		} else {
			price_mutated = (rt.call_function('apply_filters', [
				rt.new_string('woocommerce_grouped_price_html'),
				rt.new_string(price_mutated + (this.get_price_suffix()).str()),
				rt.new_object('WC_Product_Grouped', ['WC_Product'], &this),
				rt.create_array_from_list(var_child_prices),
			])).str()
		}
		...
		price_mutated = (rt.call_function('apply_filters', [
			rt.new_string('woocommerce_grouped_empty_price_html'),
			rt.new_string(''),
			rt.new_object('WC_Product_Grouped', ['WC_Product'], &this),
		])).str()
	}
	return rt.call_function('apply_filters', [
		rt.new_string('woocommerce_get_price_html'),
		rt.new_string(price_mutated).clone(),
		rt.new_object('WC_Product_Grouped', ['WC_Product'], &this),
	])
}
...
fn (mut this Class_WC_Product_Grouped) get_min_price() string {
	mut var_children := this.get_primed_visible_children('')
	mut var_prices := rt.call_function('array_map', [
	...
	return (rt.call_function('wc_format_decimal', [
		rt.call_function('min', [var_prices.clone()]),
	])).str()
}
...
fn (mut this Class_WC_Product_Grouped) get_max_price() string {
	...
	return (rt.call_function('wc_format_decimal', [
		rt.call_function('max', [var_prices.clone()]),
	])).str()
}
...
fn (mut this Class_WC_Product_Grouped) set_children(var_children rt.PhpVal) {
	...
	this.set_prop(rt.new_string('children'), rt.call_function('array_filter', [
		rt.call_function('wp_parse_id_list', [rt.cast_array(var_children_mutated)]),
	]))
}
...
fn create_wc_product_grouped(_args ...rt.PhpVal) &Class_WC_Product_Grouped {
