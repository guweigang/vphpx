...
fn (mut this Class_WC_Order_Item_Data_Store) add_order_item(var_order_id rt.PhpVal, var_item rt.PhpVal) rt.PhpVal {
	...
	rt.call_method(var_wpdb, 'insert', [
		...
		rt.create_array([
			rt.ArrayItem{
				key: 'order_item_name'
				val: var_item.array_get(rt.new_string('order_item_name'))
			},
			rt.ArrayItem{
				key: 'order_item_type'
				val: var_item.array_get(rt.new_string('order_item_type'))
			},
	...
	this.clear_caches(var_item_id.clone(), var_order_id_mutated.clone())
	return var_item_id.clone()
}
...
fn (mut this Class_WC_Order_Item_Data_Store) update_order_item(var_item_id rt.PhpVal, var_item rt.PhpVal) rt.PhpVal {
	...
	this.clear_caches(var_item_id_mutated.clone(), rt.new_null())
...
fn (mut this Class_WC_Order_Item_Data_Store) delete_order_item(var_item_id rt.PhpVal) {
	mut var_wpdb := rt.new_null()
	mut var_item_id_mutated := var_item_id
	mut var_order_id := rt.new_int(this.get_order_id_by_order_item_id(var_item_id_mutated.clone()))
	...
	this.clear_caches(var_item_id_mutated.clone(), var_order_id.clone())
}
...
fn (mut this Class_WC_Order_Item_Data_Store) update_metadata(var_item_id rt.PhpVal, var_meta_key rt.PhpVal, var_meta_value rt.PhpVal, prev_value string) rt.PhpVal {
	...
	return rt.call_function('update_metadata', [rt.new_string('order_item'),
		var_item_id_mutated.clone(), var_meta_key.clone(), if var_meta_value.clone().is_string() { rt.call_function('wp_slash', [
...
fn (mut this Class_WC_Order_Item_Data_Store) add_metadata(var_item_id rt.PhpVal, var_meta_key rt.PhpVal, var_meta_value rt.PhpVal, unique bool) rt.PhpVal {
	...
	return rt.call_function('add_metadata', [rt.new_string('order_item'),
		var_item_id_mutated.clone(), rt.call_function('wp_slash', [
			var_meta_key.clone()]),
		if var_meta_value.clone().is_string() { rt.call_function('wp_slash', [
				var_meta_value.clone()]) } else { var_meta_value }, rt.new_bool(unique)])
}
...
fn (mut this Class_WC_Order_Item_Data_Store) delete_metadata(var_item_id rt.PhpVal, var_meta_key rt.PhpVal, meta_value string, delete_all bool) rt.PhpVal {
	...
	return rt.call_function('delete_metadata', [rt.new_string('order_item'),
		var_item_id_mutated.clone(), var_meta_key.clone(), if rt.new_string(meta_value).is_string() { rt.call_function('wp_slash', [
...
fn (mut this Class_WC_Order_Item_Data_Store) get_metadata(var_item_id rt.PhpVal, var_key rt.PhpVal, single bool) rt.PhpVal {
	...
	return rt.call_function('get_metadata', [rt.new_string('order_item'),
		var_item_id_mutated.clone(), var_key.clone(), rt.new_bool(single)])
}

fn (mut this Class_WC_Order_Item_Data_Store) get_order_id_by_order_item_id(var_item_id rt.PhpVal) i64 {
	mut var_wpdb := rt.new_null()
	mut var_item_id_mutated := var_item_id
	return rt.new_int((rt.call_method(var_wpdb, 'get_var', [
		rt.call_method(var_wpdb, 'prepare', [
			rt.concat(rt.concat(rt.new_string('SELECT order_id FROM '), rt.get_property(var_wpdb,
				'prefix')), rt.new_string('woocommerce_order_items WHERE order_item_id = %d')),
			var_item_id_mutated.clone(),
		]),
	])).to_i64())
}
...
fn (mut this Class_WC_Order_Item_Data_Store) get_order_item_type(var_item_id rt.PhpVal) rt.PhpVal {
	...
	return var_order_item_type.clone()
}
...
fn (mut this Class_WC_Order_Item_Data_Store) clear_caches(var_item_id rt.PhpVal, var_order_id rt.PhpVal) {
	...
	rt.call_function('wp_cache_delete', [
		rt.new_string('item-' + var_item_id_mutated.str()),
		rt.new_string('order-items'),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_order_id_mutated)))) {
		var_order_id_mutated =
			rt.new_int(this.get_order_id_by_order_item_id(var_item_id_mutated.clone()))
	}
	if rt.is_true(var_order_id_mutated) {
		rt.call_function('wp_cache_delete', [
			rt.new_string('order-items-' + var_order_id_mutated.str()),
			rt.new_string('orders'),
		])
		rt.call_function('wp_cache_delete', [
			rt.new_string('order-needs-processing-' + var_order_id_mutated.str()),
			rt.new_string('orders'),
		])
	}
}

fn create_wc_order_item_data_store(_args ...rt.PhpVal) &Class_WC_Order_Item_Data_Store {
...
fn (mut this Class_WC_Order_Item_Data_Store) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_order_id_by_order_item_id' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			return rt.new_int(this.get_order_id_by_order_item_id(dispatch_arg_0))
		}
		'get_order_item_type' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
