...
fn init_static_wc_smoothgenerator_util_randomruntimecache() {
	rt.init_static_prop('WC_SmoothGenerator_Util_RandomRuntimeCache', 'cache', rt.new_array())
}

fn Class_WC_SmoothGenerator_Util_RandomRuntimeCache.exists(group string) bool {
	mut group_mutated := group
	return rt.get_static_prop('WC_SmoothGenerator_Util_RandomRuntimeCache', 'cache').array_isset(rt.new_string(group_mutated).clone())
}
...
fn Class_WC_SmoothGenerator_Util_RandomRuntimeCache.get(group string, limit i64) rt.PhpVal {
	...
	if limit <= 0 || var_all_items.clone().array_count() <= limit {
		return var_all_items.clone()
	}
	mut var_items := rt.call_function('array_slice', [var_all_items.clone(),
		rt.new_int(0), rt.new_int(limit)])
...
fn Class_WC_SmoothGenerator_Util_RandomRuntimeCache.extract(group string, limit i64) rt.PhpVal {
	...
	mut var_items := rt.call_function('array_slice', [var_all_items.clone(),
		rt.new_int(0), rt.new_int(limit)])
	mut var_remaining_items := rt.call_function('array_slice', [
		var_all_items.clone(), rt.new_int(limit)])
	Class_WC_SmoothGenerator_Util_RandomRuntimeCache.set(group_mutated, mut
		rt.cast_object_ptr[Class_WC_SmoothGenerator_Util_array](var_remaining_items))
	return var_items.clone()
}
...
fn Class_WC_SmoothGenerator_Util_RandomRuntimeCache.add(group string, mut var_items Class_WC_SmoothGenerator_Util_array) {
	...
	mut var_existing_items :=
		Class_WC_SmoothGenerator_Util_RandomRuntimeCache.get_group(group_mutated)
	Class_WC_SmoothGenerator_Util_RandomRuntimeCache.set(group_mutated, mut rt.cast_object_ptr[Class_WC_SmoothGenerator_Util_array](rt.call_function('array_merge', [
...
fn Class_WC_SmoothGenerator_Util_RandomRuntimeCache.set(group string, mut var_items Class_WC_SmoothGenerator_Util_array) {
	mut group_mutated := group
	mut var_items_mutated := var_items
	rt.get_static_prop('WC_SmoothGenerator_Util_RandomRuntimeCache', 'cache').array_set(group_mutated,
		var_items_mutated)
}
...
fn Class_WC_SmoothGenerator_Util_RandomRuntimeCache.count(group string) i64 {
	mut group_mutated := group
	group_mutated =
		(Class_WC_SmoothGenerator_Util_RandomRuntimeCache.get_group(group_mutated)).str()
	return rt.new_string(group_mutated).clone().array_count()
}

fn Class_WC_SmoothGenerator_Util_RandomRuntimeCache.shuffle(group string) {
	mut group_mutated := group
	Class_WC_SmoothGenerator_Util_RandomRuntimeCache.get_group(group_mutated)
	rt.call_function('shuffle', [rt.get_static_prop('WC_SmoothGenerator_Util_RandomRuntimeCache',
		'cache').array_get(rt.new_string(group_mutated))])
}

fn Class_WC_SmoothGenerator_Util_RandomRuntimeCache.clear(group string) {
	mut group_mutated := group
	rt.get_static_prop('WC_SmoothGenerator_Util_RandomRuntimeCache', 'cache').array_unset(rt.new_string(group_mutated))
}

fn Class_WC_SmoothGenerator_Util_RandomRuntimeCache.reset() {
	rt.set_static_prop('WC_SmoothGenerator_Util_RandomRuntimeCache', 'cache', rt.new_array())
}
...
fn Class_WC_SmoothGenerator_Util_RandomRuntimeCache.get_group(group string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(Class_WC_SmoothGenerator_Util_RandomRuntimeCache.exists(group_mutated))))) {
		Class_WC_SmoothGenerator_Util_RandomRuntimeCache.set(group_mutated, mut
			rt.cast_object_ptr[Class_WC_SmoothGenerator_Util_array](rt.new_array()))
	}
	return rt.get_static_prop('WC_SmoothGenerator_Util_RandomRuntimeCache', 'cache').array_get(rt.new_string(group_mutated))
}
...
fn (mut this Class_WC_SmoothGenerator_Util_RandomRuntimeCache) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get' {
			...
			return Class_WC_SmoothGenerator_Util_RandomRuntimeCache.get(dispatch_arg_0,
				dispatch_arg_1)
		}
		'extract' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).to_i64()
			return Class_WC_SmoothGenerator_Util_RandomRuntimeCache.extract(dispatch_arg_0,
				dispatch_arg_1)
		}
		'add' {
			dispatch_arg_0 := (if args.len > 0 { args[0] } else { rt.new_null() }).str()
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_WC_SmoothGenerator_Util_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
