...
fn Class_WC_SmoothGenerator_Generator_Term.generate(save bool, taxonomy string, parent i64) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_taxonomy_obj)))) {
		return rt.new_object('WC_SmoothGenerator_Generator_WP_Error', []string{}, create_wc_smoothgenerator_generator_wp_error(rt.new_string('smoothgenerator_invalid_taxonomy'), rt.new_string('The specified taxonomy is invalid.')))
	}
	if rt.is_true(rt.new_bool(0 != parent)) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(true), rt.get_property(var_taxonomy_obj, 'hierarchical'))))) {
		return rt.new_object('WC_SmoothGenerator_Generator_WP_Error', []string{}, create_wc_smoothgenerator_generator_wp_error(rt.new_string('smoothgenerator_invalid_term_hierarchy'), rt.new_string('The specified taxonomy does not support parent terms.')))
	}
	...
	mut var_term_name := rt.call_method(rt.get_static_prop('WC_SmoothGenerator_Generator_Term', 'faker'), 'company', []rt.PhpVal{})
	} else if rt.is_true(rt.get_property(var_taxonomy_obj, 'hierarchical')) {
	var_term_name = rt.call_function('ucwords', [rt.call_method(rt.get_static_prop('WC_SmoothGenerator_Generator_Term', 'faker'), 'department', [rt.new_int(3)])])
	} else {
	mut iife_temp_0 := Class_WC_SmoothGenerator_Generator_Term{}
	mut iife_result_0 := iife_temp_0.random_weighted_element(rt.create_array([rt.ArrayItem{ key: rt.call_method(rt.get_static_prop('WC_SmoothGenerator_Generator_Term', 'faker'), 'lastName', []rt.PhpVal{}), val: 45 }, rt.ArrayItem{ key: rt.call_method(rt.get_static_prop('WC_SmoothGenerator_Generator_Term', 'faker'), 'colorName', []rt.PhpVal{}), val: 35 }, rt.ArrayItem{ key: rt.call_method(rt.get_static_prop('WC_SmoothGenerator_Generator_Term', 'faker'), 'words', [rt.new_int(3), rt.new_bool(true)]), val: 20 }]))
	var_term_name = iife_result_0
	var_term_name = rt.new_string(var_term_name.clone().to_string().to_lower())
	}
	mut var_description_size := rt.call_function('wp_rand', [rt.new_int(20), rt.new_int(260)])
	mut var_term_args := rt.create_array([rt.ArrayItem{ key: 'description', val: rt.call_method(rt.get_static_prop('WC_SmoothGenerator_Generator_Term', 'faker'), 'realTextBetween', [var_description_size.clone(), rt.add(var_description_size, rt.new_int(40)), rt.new_int(4)]) }])
	if rt.is_true(rt.new_bool(0 != parent)) {
		var_term_args.array_set('parent', parent)
	}
	mut var_result := rt.call_function('wp_insert_term', [var_term_name.clone(), rt.new_string(taxonomy), var_term_args.clone()])
	if rt.is_true(rt.call_function('is_wp_error', [var_result.clone()])) {
		return var_result.clone()
	}
	mut var_term := rt.call_function('get_term', [var_result.array_get(rt.new_string('term_id'))])
	rt.call_function('do_action', [rt.new_string('smoothgenerator_term_generated'), var_term.clone()])
...
fn Class_WC_SmoothGenerator_Generator_Term.batch(var_amount rt.PhpVal, var_taxonomy rt.PhpVal, mut var_args Class_WC_SmoothGenerator_Generator_array) rt.PhpVal {
	...
	mut iife_temp_1 := Class_WC_SmoothGenerator_Generator_Term{}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_taxonomy_obj)))) {
		return rt.new_object('WC_SmoothGenerator_Generator_WP_Error', []string{}, create_wc_smoothgenerator_generator_wp_error(rt.new_string('smoothgenerator_term_batch_invalid_taxonomy'), rt.new_string('The specified taxonomy is invalid.')))
	}
...
fn Class_WC_SmoothGenerator_Generator_Term.batch_hierarchical(amount i64, taxonomy string, mut var_args Class_WC_SmoothGenerator_Generator_array) rt.PhpVal {
	...
	mut list_tmp_1 := rt.call_function('filter_var_array', [rt.call_function('wp_parse_args', [var_args, var_defaults.clone()]), rt.create_array([rt.ArrayItem{ key: 'max-depth', val: rt.create_array([rt.ArrayItem{ key: 'filter', val: rt.get_constant('FILTER_VALIDATE_INT') }, rt.ArrayItem{ key: 'options', val: rt.create_array([rt.ArrayItem{ key: 'min_range', val: 1 }, rt.ArrayItem{ key: 'max_range', val: 5 }]) }]) }, rt.ArrayItem{ key: 'parent', val: rt.get_constant('FILTER_VALIDATE_INT') }])])
	var_max_depth = (list_tmp_1).array_get(0)
	var_parent = (list_tmp_1).array_get(1)
	if rt.is_true(rt.identical(rt.new_bool(false), var_max_depth)) {
		return rt.new_object('WC_SmoothGenerator_Generator_WP_Error', []string{}, create_wc_smoothgenerator_generator_wp_error(rt.new_string('smoothgenerator_term_batch_invalid_max_depth'), rt.new_string('Max depth must be a number between 1 and 5.')))
	}
	if rt.is_true(rt.identical(rt.new_bool(false), rt.new_int(var_parent))) {
		return rt.new_object('WC_SmoothGenerator_Generator_WP_Error', []string{}, create_wc_smoothgenerator_generator_wp_error(rt.new_string('smoothgenerator_term_batch_invalid_parent'), rt.new_string('Parent must be the ID number of an existing term.')))
	}
	mut var_term_ids := rt.new_array()
	mut iife_temp_2 := Class_WC_SmoothGenerator_Generator_Term{}
	mut iife_result_2 := iife_temp_2.init_faker()
	if var_parent != 0 || rt.is_true(rt.identical(rt.new_int(1), var_max_depth)) {
		mut var_i := rt.new_int(1)
		for {
		...
		mut var_remaining := rt.new_int(amount_mutated).clone()
		mut var_term_max := rt.new_int(1)
		if amount_mutated > 2 {
		var_term_max = rt.call_function('floor', [rt.call_function('log', [rt.new_int(amount_mutated).clone()])])
		}
		mut var_levels := rt.call_function('array_fill', [rt.new_int(1), var_max_depth.clone(), rt.new_array()])
		mut var_i := rt.new_int(1)
		for {
			...
			if rt.is_true(rt.identical(rt.new_int(1), var_i)) {
				...
				for {
					if !(rt.is_true(rt.less_equal(var_j, var_term_max)) && rt.is_true(rt.greater(var_remaining, rt.new_int(0)))) { break }
				...
				for {
					...
					mut var_tcount := rt.call_function('wp_rand', [rt.new_int(0), var_term_max.clone()])
					mut var_j := rt.new_int(1)
					for {
						if !(rt.is_true(rt.less_equal(var_j, var_tcount)) && rt.is_true(rt.greater(var_remaining, rt.new_int(0)))) { break }
			...
			if rt.is_true(rt.identical(var_i, var_max_depth)) && rt.is_true(rt.greater(var_remaining, rt.new_int(0))) {
