...
fn (mut this Class_WordPress_AiClient_Providers_ApiBasedImplementation_GenerateTextApiBasedProviderAvailability) isconfigured() bool {
	...
	mut var_modelConfig := iife_result_0
	rt.call_method(this.model, 'setConfig', [var_modelConfig.clone()])
	mut iife_temp_1 := Class_WordPress_AiClient_Messages_Enums_MessageRoleEnum{}
	mut iife_result_1 := iife_temp_1.user()
	rt.call_method(this.model, 'generateTextResult', [
		rt.create_array([
			rt.ArrayItem{ key: none, val: create_wordpress_aiclient_messages_dto_message(iife_result_1, rt.create_array([
				rt.ArrayItem{
					key: none
