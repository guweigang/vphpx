...
fn (mut this Class_WordPress_AiClient_Providers_Http_Collections_HeadersCollection) getasstring(name string) string {
	...
	return (if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_values, rt.new_null())))) {
		rt.call_function('implode', [rt.new_string(', '), var_values.clone()])
	} else {
		rt.new_null()
	}).str()
}
...
fn (mut this Class_WordPress_AiClient_Providers_Http_Collections_HeadersCollection) set(name string, var_value rt.PhpVal) {
	if rt.is_true(rt.new_bool(var_value.clone().is_array())) {
		mut var_normalizedValues := rt.call_function('array_values', [
			var_value.clone()])
	} else {
		var_normalizedValues = rt.call_function('array_map', [
			rt.new_string('trim'), rt.call_function('explode', [
				rt.new_string(','), var_value.clone()])])
	}
	mut var_lowerName := rt.new_string(name.to_lower())
	if this.headersMap.array_isset(var_lowerName) {
		mut var_oldName := this.headersMap.array_get(var_lowerName)
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_oldName, rt.new_string(name))))) {
			this.headers.array_unset(var_oldName)
		}
	}
	this.headers.array_set(name, var_normalizedValues.clone())
...
fn (mut this Class_WordPress_AiClient_Providers_Http_Collections_HeadersCollection) withheader(name string, var_value rt.PhpVal) rt.PhpVal {
	mut var_new := rt.new_object('WordPress_AiClient_Providers_Http_Collections_HeadersCollection',
		[]string{}, &this).dup()
	rt.call_method(var_new, 'set', [rt.new_string(name), var_value.clone()])
...
fn (mut this Class_WordPress_AiClient_Providers_Http_Collections_HeadersCollection) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_WordPress_AiClient_Providers_Http_Collections_array](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
