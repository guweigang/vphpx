...
fn (mut this Class_WordPress_AiClient_Providers_Http_HttpTransporter) construct(mut var_client Class_WordPress_AiClient_Providers_Http_?ClientInterface, mut var_requestFactory Class_WordPress_AiClient_Providers_Http_?RequestFactoryInterface, mut var_streamFactory Class_WordPress_AiClient_Providers_Http_?StreamFactoryInterface) {
	...
	this.client = if rt.is_true(var_client) { var_client } else { iife_result_0 }
	...
	this.requestFactory = if rt.is_true(var_requestFactory) { var_requestFactory } else { iife_result_1 }
	...
	this.streamFactory = if rt.is_true(var_streamFactory) { var_streamFactory } else { iife_result_2 }
}
...
fn (mut this Class_WordPress_AiClient_Providers_Http_HttpTransporter) send(mut var_request Class_WordPress_AiClient_Providers_Http_DTO_Request, mut var_options Class_WordPress_AiClient_Providers_Http_?RequestOptions) rt.PhpVal {
	...
	mut var_hasOptions := rt.new_bool(!rt.is_true(rt.identical(var_mergedOptions, rt.new_null())))
	if rt.has_exception() { unsafe { goto catch_label_1 } }
	if rt.is_true(var_hasOptions) && rt.is_true(rt.new_bool(rt.instance_of(this.client, 'WordPress_AiClient_Providers_Http_Contracts_ClientWithOptionsInterface'))) {
		mut var_psr7Response := rt.call_method(this.client, 'sendRequestWithOptions', [var_psr7Request.clone(), var_mergedOptions.clone()])
		if rt.has_exception() { unsafe { goto catch_label_1 } }
	} else if rt.is_true(var_hasOptions) && this.isguzzleclient(mut rt.cast_object_ptr[Class_WordPress_AiClientDependencies_Psr_Http_Client_ClientInterface](this.client)) {
		...
		var_psr7Response = rt.call_method(this.client, 'sendRequest', [var_psr7Request.clone()])
		if rt.has_exception() { unsafe { goto catch_label_1 } }
	}
	if rt.has_exception() { unsafe { goto catch_label_1 } }
...
catch_label_1:
	...
	if rt.instance_of(var_e_1, 'WordPress_AiClient_Providers_Http_WordPress_AiClientDependencies_Psr_Http_Client_NetworkExceptionInterface') {
		...
		mut iife_temp_3 := Class_WordPress_AiClient_Providers_Http_Exception_NetworkException{}
		mut iife_result_3 := iife_temp_3.frompsr18networkexception(var_psr7Request.clone(), var_e.clone())
...
fn (mut this Class_WordPress_AiClient_Providers_Http_HttpTransporter) mergeoptions(mut var_requestOptions Class_WordPress_AiClient_Providers_Http_?RequestOptions, mut var_parameterOptions Class_WordPress_AiClient_Providers_Http_?RequestOptions) rt.PhpVal {
	if rt.is_true(rt.identical(var_requestOptions, rt.new_null())) && rt.is_true(rt.identical(var_parameterOptions, rt.new_null())) {
		return rt.new_null()
	}
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_requestOptions.gettimeout(), rt.new_null())))) {
		var_merged.settimeout(var_requestOptions.gettimeout())
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_requestOptions.getconnecttimeout(), rt.new_null())))) {
		var_merged.setconnecttimeout(var_requestOptions.getconnecttimeout())
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_requestOptions.getmaxredirects(), rt.new_null())))) {
		var_merged.setmaxredirects(var_requestOptions.getmaxredirects())
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_parameterOptions.gettimeout(), rt.new_null())))) {
		var_merged.settimeout(var_parameterOptions.gettimeout())
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_parameterOptions.getconnecttimeout(), rt.new_null())))) {
		var_merged.setconnecttimeout(var_parameterOptions.getconnecttimeout())
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_parameterOptions.getmaxredirects(), rt.new_null())))) {
...
fn (mut this Class_WordPress_AiClient_Providers_Http_HttpTransporter) isguzzleclient(mut var_client Class_WordPress_AiClientDependencies_Psr_Http_Client_ClientInterface) bool {
	mut var_reflection := create_wordpress_aiclient_providers_http_reflectionobject(var_client)
	if !(rt.call_function('is_callable', [rt.create_array([rt.ArrayItem{ key: none, val: var_client }, rt.ArrayItem{ key: none, val: 'send' }])])) {
		return false
	}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_method, 'isPublic', []rt.PhpVal{}))))) || rt.is_true(rt.call_method(var_method, 'isStatic', []rt.PhpVal{})) {
		return false
	}
	mut var_parameters := rt.call_method(var_method, 'getParameters', []rt.PhpVal{})
	if var_parameters.clone().array_count() < 2 {
		return false
	}
	mut var_firstParameter := rt.call_method(var_parameters.array_get(rt.new_int(0)), 'getType', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_firstParameter, 'WordPress_AiClient_Providers_Http_ReflectionNamedType')))))) || rt.is_true(rt.call_method(var_firstParameter, 'isBuiltin', []rt.PhpVal{})) {
		return false
	}
	...
	mut var_secondParameter := var_parameters.array_get(rt.new_int(1))
	mut var_secondType := rt.call_method(var_secondParameter, 'getType', []rt.PhpVal{})
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_secondType, 'WordPress_AiClient_Providers_Http_ReflectionNamedType')))))) || rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_method(var_secondType, 'getName', []rt.PhpVal{}), rt.new_string('array'))))) {
		return false
	}
	return true
}
...
fn (mut this Class_WordPress_AiClient_Providers_Http_HttpTransporter) sendwithguzzle(mut var_request Class_WordPress_AiClientDependencies_Psr_Http_Message_RequestInterface, mut var_options Class_WordPress_AiClient_Providers_Http_DTO_RequestOptions) rt.PhpVal {
	...
	mut var_response := rt.call_callable(var_callable, [var_request, var_guzzleOptions.clone()])
...
fn (mut this Class_WordPress_AiClient_Providers_Http_HttpTransporter) buildguzzleoptions(mut var_options Class_WordPress_AiClient_Providers_Http_DTO_RequestOptions) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_timeout, rt.new_null())))) {
		var_guzzleOptions.array_set('timeout', var_timeout.clone())
	}
	mut var_connectTimeout := var_options.getconnecttimeout()
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_connectTimeout, rt.new_null())))) {
		var_guzzleOptions.array_set('connect_timeout', var_connectTimeout.clone())
	}
	mut var_allowRedirects := var_options.allowsredirects()
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_allowRedirects, rt.new_null())))) {
		if rt.is_true(var_allowRedirects) {
			mut var_redirectOptions := rt.new_array()
			mut var_maxRedirects := var_options.getmaxredirects()
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_maxRedirects, rt.new_null())))) {
				var_redirectOptions.array_set('max', var_maxRedirects.clone())
			}
	...
	return var_guzzleOptions.clone()
}
...
fn (mut this Class_WordPress_AiClient_Providers_Http_HttpTransporter) converttopsr7request(mut var_request Class_WordPress_AiClient_Providers_Http_DTO_Request) rt.PhpVal {
	...
	for {
		...
		var_psr7Request = rt.call_method(var_psr7Request, 'withAddedHeader', [var_name.clone(), var_value.clone()])
		}
	}
	mut var_body := var_request.getbody()
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_body, rt.new_null())))) {
	mut var_stream := rt.call_method(this.streamFactory, 'createStream', [var_body.clone()])
	var_psr7Request = rt.call_method(var_psr7Request, 'withBody', [var_stream.clone()])
	}
	return var_psr7Request.clone()
}
...
fn (mut this Class_WordPress_AiClient_Providers_Http_HttpTransporter) convertfrompsr7response(mut var_psr7Response Class_WordPress_AiClientDependencies_Psr_Http_Message_ResponseInterface) rt.PhpVal {
	...
	mut var_body := rt.new_string((rt.call_method(var_psr7Response_mutated, 'getBody', []rt.PhpVal{})).str())
	return rt.new_object('WordPress_AiClient_Providers_Http_DTO_Response', []string{}, create_wordpress_aiclient_providers_http_dto_response(rt.call_method(var_psr7Response_mutated, 'getStatusCode', []rt.PhpVal{}), rt.call_method(var_psr7Response_mutated, 'getHeaders', []rt.PhpVal{}), if rt.is_true(rt.identical(var_body, rt.new_string(''))) { rt.new_null() } else { var_body }))
}
...
fn create_wordpress_aiclient_providers_http_reflectionobject(_args ...rt.PhpVal) &Class_WordPress_AiClient_Providers_Http_ReflectionObject {
...
fn create_wordpress_aiclient_providers_http_dto_response(_args ...rt.PhpVal) &Class_WordPress_AiClient_Providers_Http_DTO_Response {
