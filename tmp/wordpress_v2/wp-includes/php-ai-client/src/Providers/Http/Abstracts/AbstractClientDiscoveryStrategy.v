...
fn Class_WordPress_AiClient_Providers_Http_Abstracts_AbstractClientDiscoveryStrategy.init() {
	...
	mut iife_result_0 :=
		iife_temp_0.prependstrategy(Class_WordPress_AiClient_Providers_Http_Abstracts_static.class())
}
...
fn Class_WordPress_AiClient_Providers_Http_Abstracts_AbstractClientDiscoveryStrategy.getcandidates(var_type rt.PhpVal) rt.PhpVal {
		...
		var_type))
	{
		...
		return rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'class', val: rt.new_closure(closure_2_fn) },
			]) },
		])
	}
	...
	if rt.is_true(rt.call_function('in_array', [var_type.clone(),
		var_psr17Factories.clone(), rt.new_bool(true)]))
	{
		return rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{
...
fn create_wordpress_aiclient_providers_http_abstracts_abstractclientdiscoverystrategy(_args ...rt.PhpVal) &Class_WordPress_AiClient_Providers_Http_Abstracts_AbstractClientDiscoveryStrategy {
