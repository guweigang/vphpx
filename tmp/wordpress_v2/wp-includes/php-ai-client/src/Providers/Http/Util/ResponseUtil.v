...
fn Class_WordPress_AiClient_Providers_Http_Util_ResponseUtil.throwifnotsuccessful(mut var_response Class_WordPress_AiClient_Providers_Http_DTO_Response) {
	...
	if rt.is_true(rt.greater_equal(var_statusCode, rt.new_int(300)))
		&& rt.is_true(rt.less(var_statusCode, rt.new_int(400))) {
		mut iife_temp_0 := Class_WordPress_AiClient_Providers_Http_Exception_RedirectException{}
		mut iife_result_0 := iife_temp_0.fromredirectresponse(rt.new_object('WordPress_AiClient_Providers_Http_DTO_Response',
			[]string{}, var_response))
		rt.throw_exception(iife_result_0)
	}
	if rt.is_true(rt.greater_equal(var_statusCode, rt.new_int(400)))
		&& rt.is_true(rt.less(var_statusCode, rt.new_int(500))) {
		mut iife_temp_1 := Class_WordPress_AiClient_Providers_Http_Exception_ClientException{}
		mut iife_result_1 := iife_temp_1.fromclienterrorresponse(rt.new_object('WordPress_AiClient_Providers_Http_DTO_Response',
			[]string{}, var_response))
		rt.throw_exception(iife_result_1)
	}
	if rt.is_true(rt.greater_equal(var_statusCode, rt.new_int(500)))
		&& rt.is_true(rt.less(var_statusCode, rt.new_int(600))) {
		mut iife_temp_2 := Class_WordPress_AiClient_Providers_Http_Exception_ServerException{}
		mut iife_result_2 := iife_temp_2.fromservererrorresponse(rt.new_object('WordPress_AiClient_Providers_Http_DTO_Response',
			[]string{}, var_response))
...
fn create_wordpress_aiclient_providers_http_util_runtimeexception(_args ...rt.PhpVal) &Class_WordPress_AiClient_Providers_Http_Util_RuntimeException {
