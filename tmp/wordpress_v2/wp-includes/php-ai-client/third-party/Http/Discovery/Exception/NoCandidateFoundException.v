...
fn (mut this Class_WordPress_AiClientDependencies_Http_Discovery_Exception_NoCandidateFoundException) construct(var_strategy rt.PhpVal, mut var_candidates Class_WordPress_AiClientDependencies_Http_Discovery_Exception_array) {
	...
	this.Class_WordPress_AiClientDependencies_Http_Discovery_Exception.construct(var_message.clone())
}
...
fn (mut this Class_WordPress_AiClientDependencies_Http_Discovery_Exception_NoCandidateFoundException) stringify(var_mixed rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(var_mixed.clone().is_string())) {
		return var_mixed.clone()
	}
	if var_mixed.clone().is_array() && 2 == var_mixed.clone().array_count() {
		return rt.call_function('sprintf', [rt.new_string('%s::%s'),
			this.stringify(var_mixed.array_get(rt.new_int(0))),
			var_mixed.array_get(rt.new_int(1))])
	}
	return if var_mixed.clone().is_object() { rt.call_function('get_class', [
			var_mixed.clone()]) } else { rt.call_function('gettype', [
			var_mixed.clone()]) }
}
