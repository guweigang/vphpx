...
fn (mut this Class_Text_Diff_Renderer_inline) _lines(var_lines rt.PhpVal, prefix string, encode bool) string {
	...
	if var_encode {
		rt.call_function('array_walk', [var_lines_mutated.clone(),
			rt.create_array([
				rt.ArrayItem{ key: none, val: rt.new_object('Text_Diff_Renderer_inline', [
					'Text_Diff_Renderer',
				], &this) },
				rt.ArrayItem{ key: none, val: '_encode' },
			])])
	}
	if rt.is_true(rt.equal(this._split_level, rt.new_string('lines'))) {
		return
			(rt.call_function('implode', [rt.new_string('\n'), var_lines_mutated.clone()])).str() +
			'\n'
	} else {
...
fn (mut this Class_Text_Diff_Renderer_inline) _added(var_lines rt.PhpVal) rt.PhpVal {
	...
	var_lines_mutated.array_set(0,

		(this._ins_prefix).str() + (var_lines_mutated.array_get(rt.new_int(0))).str())
	var_lines_mutated.array_get(rt.new_int(var_lines_mutated.clone().array_count() - 1)) = rt.concat(var_lines_mutated.array_get(rt.new_int(var_lines_mutated.clone().array_count() - 1)),
		this._ins_suffix)
	return rt.new_string(this._lines(var_lines_mutated.clone(), ' ', false))
}
...
fn (mut this Class_Text_Diff_Renderer_inline) _deleted(var_lines rt.PhpVal, words bool) rt.PhpVal {
	...
	var_lines_mutated.array_set(0,
		...
		(this._del_prefix).str() + (var_lines_mutated.array_get(rt.new_int(0))).str())
	var_lines_mutated.array_get(rt.new_int(var_lines_mutated.clone().array_count() - 1)) = rt.concat(var_lines_mutated.array_get(rt.new_int(var_lines_mutated.clone().array_count() - 1)),
		this._del_suffix)
	return rt.new_string(this._lines(var_lines_mutated.clone(), ' ', false))
}
...
fn (mut this Class_Text_Diff_Renderer_inline) _changed(var_orig rt.PhpVal, var_final rt.PhpVal) string {
	...
	if rt.is_true(rt.equal(this._split_level, rt.new_string('characters'))) {
		return (this._deleted(var_orig_mutated.clone(), false)).str() +
			(this._added(var_final_mutated.clone())).str()
	}
	if rt.is_true(rt.equal(this._split_level, rt.new_string('words'))) {
		mut var_prefix := rt.new_string('')
		for rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_orig_mutated.array_get(rt.new_int(0)), rt.new_bool(false)))))
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_final_mutated.array_get(rt.new_int(0)), rt.new_bool(false)))))
			&& rt.is_true(rt.equal(rt.call_function('substr', [var_orig_mutated.array_get(rt.new_int(0)), rt.new_int(0), rt.new_int(1)]), rt.new_string(' ')))
			&& rt.is_true(rt.equal(rt.call_function('substr', [var_final_mutated.array_get(rt.new_int(0)), rt.new_int(0), rt.new_int(1)]), rt.new_string(' '))) {
			var_prefix = rt.concat(var_prefix, rt.call_function('substr', [
				var_orig_mutated.array_get(rt.new_int(0)),
				rt.new_int(0),
				rt.new_int(1),
			]))
			var_orig_mutated.array_set(0, rt.call_function('substr', [
				var_orig_mutated.array_get(rt.new_int(0)),
				rt.new_int(1),
			]))
			var_final_mutated.array_set(0, rt.call_function('substr', [
				var_final_mutated.array_get(rt.new_int(0)),
				rt.new_int(1),
			]))
		}
		return var_prefix.str() + (this._deleted(var_orig_mutated.clone(), false)).str() +
			(this._added(var_final_mutated.clone())).str()
	}
	mut var_text1 := rt.call_function('implode', [rt.new_string('\n'),
		var_orig_mutated.clone()])
	mut var_text2 := rt.call_function('implode', [rt.new_string('\n'),
		var_final_mutated.clone()])
	mut var_nl := rt.new_string('')
	if rt.is_true(this._split_characters) {
		mut var_diff := create_text_diff(rt.new_string('native'), rt.create_array([
			rt.ArrayItem{ key: none, val: rt.call_function('preg_split', [
				rt.new_string('//'),
				var_text1.clone(),
			]) },
			rt.ArrayItem{ key: none, val: rt.call_function('preg_split', [
		...
		var_diff = create_text_diff(rt.new_string('native'), rt.create_array([
			rt.ArrayItem{ key: none, val: this._splitonwords(var_text1.clone(), var_nl.str()) },
			rt.ArrayItem{ key: none, val: this._splitonwords(var_text2.clone(), var_nl.str()) },
		]))
	}
	mut var_renderer := create_text_diff_renderer_inline(rt.call_function('array_merge', [
		this.getparams(),
		rt.create_array([
			rt.ArrayItem{
				key: 'split_level'
				val: if rt.is_true(this._split_characters) { 'characters' } else { 'words' }
			},
		]),
	]))
	return
		(rt.call_function('str_replace', [var_nl.clone(), rt.new_string('\n'), var_renderer.render(rt.new_object('Text_Diff', []string{}, var_diff))])).str() +
		'\n'
}
...
fn (mut this Class_Text_Diff_Renderer_inline) _splitonwords(var_string rt.PhpVal, newlineEscape string) rt.PhpVal {
	...
	for rt.is_true(rt.less(var_pos, var_length)) {
		mut var_spaces := rt.call_function('strspn', [
			rt.call_function('substr', [var_string_mutated.clone(),
				var_pos.clone()]),
			rt.new_string(' \n'),
		])
		mut var_nextpos := rt.call_function('strcspn', [
			rt.call_function('substr', [var_string_mutated.clone(),
				rt.add(var_pos, var_spaces)]),
			rt.new_string(' \n'),
		])
		var_words << rt.call_function('str_replace', [rt.new_string('\n'),
			rt.new_string(newlineEscape),
			rt.call_function('substr', [
				var_string_mutated.clone(), var_pos.clone(), rt.add(var_spaces, var_nextpos)])])
		var_pos = rt.add(var_pos, rt.add(var_spaces, var_nextpos))
	}
...
fn (mut this Class_Text_Diff_Renderer_inline) _encode(var_string rt.PhpVal) {
	mut var_string_mutated := var_string
	var_string_mutated = rt.call_function('htmlspecialchars', [
...
fn (mut this Class_Text_Diff_Renderer_inline) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'_ins_prefix' {
			this._ins_prefix = val
			return true
		}
		'_ins_suffix' {
			this._ins_suffix = val
			return true
		}
		'_del_prefix' {
			this._del_prefix = val
			return true
		}
		'_del_suffix' {
			this._del_suffix = val
			return true
		}
		'_block_header' {
			this._block_header = val
			return true
		}
		'_split_characters' {
			this._split_characters = val
			return true
		}
		'_split_level' {
			this._split_level = val
			return true
		}
		else {
