...
fn (mut this Class_Text_Diff_Engine_xdiff) diff(var_from_lines rt.PhpVal, var_to_lines rt.PhpVal) rt.PhpVal {
	...
	mut var_to_string := rt.call_function('implode', [rt.new_string('\n'),
		var_to_lines.clone()])
	mut var_diff := rt.call_function('xdiff_string_diff', [var_from_string.clone(),
		var_to_string.clone(), rt.new_int(var_to_lines.clone().array_count())])
	...
	for {
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_int(var_line.clone().to_string().len))))) {
			continue
		}
		mut switch_val_1 := var_line.array_get(rt.new_int(0))
...
fn create_text_diff_engine_xdiff(_args ...rt.PhpVal) &Class_Text_Diff_Engine_xdiff {
