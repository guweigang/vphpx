...
fn get_the_author(deprecated string) rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('the_author'), if var_authordata.clone().is_object() {
...
fn get_the_author_link(use_title_attr bool) rt.PhpVal {
	...
	if rt.is_true(get_the_author_meta('url', false)) {
		...
		var_link = rt.call_function('sprintf', [
			...
			rt.new_string((if var_use_title_attr {
...
fn get_the_author_posts_link() string {
	...
	mut var_link := rt.new_null()
	if !(var_authordata.clone().is_object()) {
		return ''
	}
...
fn wp_list_authors(args string) string {
	...
	if !(var_post_counts.clone().is_array()) {
	...
	mut iter_2 := var_authors.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_author_id_shadow := item_2.val
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(var_posts))))
			&& rt.is_true(var_parsed_args.array_get(rt.new_string('hide_empty'))) {
			continue
		}
		var_author = rt.call_function('get_userdata', [var_author_id_shadow.clone()])
		if rt.is_true(var_parsed_args.array_get(rt.new_string('exclude_admin')))
			&& rt.is_true(rt.identical(rt.new_string('admin'), rt.get_property(var_author, 'display_name'))) {
			continue
		}
		if rt.is_true(var_parsed_args.array_get(rt.new_string('show_fullname')))
		...
		var_link = rt.call_function('sprintf', [rt.new_string('<a href="%1$s">%2$s</a>'),
			rt.call_function('esc_url', [
		...
		if !(!rt.is_true(var_parsed_args.array_get(rt.new_string('feed_image'))))
			|| !(!rt.is_true(var_parsed_args.array_get(rt.new_string('feed')))) {
			var_link = rt.concat(var_link, rt.new_string(' '))
			if !rt.is_true(var_parsed_args.array_get(rt.new_string('feed_image'))) {
				var_link = rt.concat(var_link, rt.new_string('('))
			}
			var_link = rt.concat(var_link, rt.new_string('<a href="' +
				(rt.call_function('get_author_feed_link', [rt.get_property(var_author, 'ID'), var_parsed_args.array_get(rt.new_string('feed_type'))])).str() +
				'"'))
			var_alt = rt.new_string('')
			if !(!rt.is_true(var_parsed_args.array_get(rt.new_string('feed')))) {
				var_alt = rt.new_string(' alt="' +
					(rt.call_function('esc_attr', [var_parsed_args.array_get(rt.new_string('feed'))])).str() +
					'"')
			...
			if !(!rt.is_true(var_parsed_args.array_get(rt.new_string('feed_image')))) {
				var_link = rt.concat(var_link, rt.new_string('<img src="' +
					(rt.call_function('esc_url', [var_parsed_args.array_get(rt.new_string('feed_image'))])).str() +
		...
		if rt.is_true(var_parsed_args.array_get(rt.new_string('optioncount'))) {
			var_link = rt.concat(var_link, rt.new_string(' (' + var_posts.str() + ')'))
		}
		var_return = var_return + var_link.str()
		var_return = var_return +
			if rt.is_true(rt.identical(rt.new_string('list'), var_parsed_args.array_get(rt.new_string('style')))) { '</li>' } else { ', ' }
	}
...
fn is_multi_author() rt.PhpVal {
	...
	return rt.call_function('apply_filters', [rt.new_string('is_multi_author'),
		rt.new_bool(var_is_multi_author.to_bool())])
}
