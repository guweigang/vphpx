...
fn init_static_wp_connector_registry() {
	rt.init_static_prop('WP_Connector_Registry', 'instance', rt.new_null())
}
...
fn (mut this Class_WP_Connector_Registry) register(id string, mut var_args Class_array) rt.PhpVal {
	...
	if !rt.is_true(var_args.array_get(rt.new_string('name')))
		|| !(var_args.array_get(rt.new_string('name')).is_string()) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [
	...
	if !rt.is_true(var_args.array_get(rt.new_string('type')))
		|| !(var_args.array_get(rt.new_string('type')).is_string()) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [
	...
	if !(var_args.array_isset(rt.new_string('authentication')))
		|| !(var_args.array_get(rt.new_string('authentication')).is_array()) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [
	...
	if !rt.is_true(var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('method')))
		|| rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('method')), rt.create_array([rt.ArrayItem{
		key: none
		val: 'api_key'
	...
	if rt.is_true(rt.identical(rt.new_string('ai_provider'), var_args.array_get(rt.new_string('type'))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_supports_ai', []rt.PhpVal{}))))) {
		return rt.new_null()
	}
	mut var_connector := {
		'name':           var_args.array_get(rt.new_string('name'))
		'description':    if var_args.array_isset(rt.new_string('description'))
			&& var_args.array_get(rt.new_string('description')).is_string() {
			var_args.array_get(rt.new_string('description'))
		} else {
			rt.new_string('')
		}
		'type':           var_args.array_get(rt.new_string('type'))
		'authentication': {
			'method': var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('method'))
		}
	}
	if !(!rt.is_true(var_args.array_get(rt.new_string('logo_url'))))
		&& var_args.array_get(rt.new_string('logo_url')).is_string() {
		var_connector['logo_url'] = var_args.array_get(rt.new_string('logo_url'))
	}
	if rt.is_true(rt.identical(rt.new_string('api_key'),
		var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('method'))))
	{
		if !(!rt.is_true(var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('credentials_url'))))
			&& var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('credentials_url')).is_string() {
			var_connector.array_get_mut('authentication').array_set('credentials_url',
				var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('credentials_url')))
		}
		if var_args.array_get(rt.new_string('authentication')).array_isset(rt.new_string('setting_name')) {
			if !(var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('setting_name')).is_string())
				|| rt.is_true(rt.identical(rt.new_string(''), var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('setting_name')))) {
				rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
					rt.call_function('sprintf', [
						rt.call_function('__', [
			...
			var_connector.array_get_mut('authentication').array_set('setting_name',
				var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('setting_name')))
		} else {
			var_connector.array_get_mut('authentication').array_set('setting_name', rt.call_function('str_replace', [
				...
				rt.concat(rt.concat(rt.concat(rt.concat(rt.new_string('connectors_'),
					var_connector['type']), rt.new_string('_')), rt.new_string(id)),
					rt.new_string('_api_key')),
			]))
		}
		if var_args.array_get(rt.new_string('authentication')).array_isset(rt.new_string('constant_name')) {
			if !(var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('constant_name')).is_string())
				|| rt.is_true(rt.identical(rt.new_string(''), var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('constant_name')))) {
				rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
					rt.call_function('sprintf', [
						rt.call_function('__', [
			...
			var_connector.array_get_mut('authentication').array_set('constant_name',
				var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('constant_name')))
		}
		if var_args.array_get(rt.new_string('authentication')).array_isset(rt.new_string('env_var_name')) {
			if !(var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('env_var_name')).is_string())
				|| rt.is_true(rt.identical(rt.new_string(''), var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('env_var_name')))) {
				rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
					rt.call_function('sprintf', [
						rt.call_function('__', [
			...
			var_connector.array_get_mut('authentication').array_set('env_var_name',
				var_args.array_get(rt.new_string('authentication')).array_get(rt.new_string('env_var_name')))
		}
	}
	var_connector['plugin'] = rt.new_array()
	if !(!rt.is_true(var_args.array_get(rt.new_string('plugin'))))
		&& var_args.array_get(rt.new_string('plugin')).is_array() {
		if !(!rt.is_true(var_args.array_get(rt.new_string('plugin')).array_get(rt.new_string('file')))) {
			var_connector.array_get_mut('plugin').array_set('file',
				var_args.array_get(rt.new_string('plugin')).array_get(rt.new_string('file')))
		}
		if var_args.array_get(rt.new_string('plugin')).array_isset(rt.new_string('is_active')) {
			if !(rt.call_function('is_callable', [
				var_args.array_get(rt.new_string('plugin')).array_get(rt.new_string('is_active')),
			])) {
				rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
					rt.call_function('sprintf', [
						rt.call_function('__', [
	...
	if !(var_connector['plugin'].array_isset(rt.new_string('is_active'))) {
		var_connector.array_get_mut('plugin').array_set('is_active', '__return_true')
	}
	this.registered_connectors.array_set(id, var_connector.clone())
	return var_connector.clone()
}
...
fn (mut this Class_WP_Connector_Registry) unregister(id string) rt.PhpVal {
	...
	mut var_unregistered := this.registered_connectors.array_get(rt.new_string(id))
	this.registered_connectors.array_unset(rt.new_string(id))
	return var_unregistered.clone()
}
...
fn (mut this Class_WP_Connector_Registry) get_registered(id string) rt.PhpVal {
	...
	return this.registered_connectors.array_get(rt.new_string(id))
}

fn Class_WP_Connector_Registry.get_instance() rt.PhpVal {
	return rt.get_static_prop('WP_Connector_Registry', 'instance')
}
...
fn Class_WP_Connector_Registry.set_instance(mut var_registry Class_WP_Connector_Registry) {
	...
	rt.set_static_prop('WP_Connector_Registry', 'instance', var_registry)
}
