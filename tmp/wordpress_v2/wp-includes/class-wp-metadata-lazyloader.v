...
fn (mut this Class_WP_Metadata_Lazyloader) queue_objects(var_object_type rt.PhpVal, var_object_ids rt.PhpVal) rt.PhpVal {
	...
	if !(this.settings.array_isset(var_object_type)) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('invalid_object_type'), rt.call_function('__', [
	...
	rt.call_function('add_filter', [var_type_settings.array_get(rt.new_string('filter')),
		var_type_settings.array_get(rt.new_string('callback')),
		rt.new_int(10), rt.new_int(5)])
	rt.call_function('do_action', [rt.new_string('metadata_lazyloader_queued_objects'),
		var_object_ids_mutated.clone(), var_object_type.clone(),
		rt.new_object('WP_Metadata_Lazyloader', []string{}, &this)])
	return rt.new_null()
}
...
fn (mut this Class_WP_Metadata_Lazyloader) reset_queue(var_object_type rt.PhpVal) rt.PhpVal {
	...
	mut var_type_settings := this.settings.array_get(var_object_type)
	this.pending_objects.array_set(var_object_type, rt.new_array())
	rt.call_function('remove_filter', [var_type_settings.array_get(rt.new_string('filter')),
		var_type_settings.array_get(rt.new_string('callback'))])
	return rt.new_null()
}
...
fn (mut this Class_WP_Metadata_Lazyloader) lazyload_term_meta(var_check rt.PhpVal) rt.PhpVal {
	...
	return this.lazyload_meta_callback(var_check.clone(), rt.new_int(0), rt.new_string(''),
...
fn (mut this Class_WP_Metadata_Lazyloader) lazyload_meta_callback(var_check rt.PhpVal, var_object_id rt.PhpVal, var_meta_key rt.PhpVal, var_single rt.PhpVal, var_meta_type rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_object_id)
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_object_id.clone(), var_object_ids.clone(), rt.new_bool(true)]))))) {
		var_object_ids.array_push(var_object_id.clone())
	}
	rt.call_function('update_meta_cache', [var_meta_type.clone(),
		var_object_ids.clone()])
	this.reset_queue(var_meta_type.clone())
	return var_check.clone()
}
