...
fn init_static_wp_block_bindings_registry() {
	rt.init_static_prop('WP_Block_Bindings_Registry', 'instance', rt.new_null())
}

fn (mut this Class_WP_Block_Bindings_Registry) register(source_name string, mut var_source_properties Class_array) rt.PhpVal {
	if !(rt.new_string(source_name).is_string()) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('Block bindings source name must be a string.'),
			]),
			rt.new_string('6.5.0')])
		return rt.new_bool(false)
	}
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/[A-Z]+/'),
		rt.new_string(source_name)]))
	{
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('Block bindings source names must not contain uppercase characters.'),
			]),
			rt.new_string('6.5.0')])
		return rt.new_bool(false)
	}
	mut var_name_matcher := rt.new_string('/^[a-z0-9-]+\\/[a-z0-9-]+$/')
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [
		var_name_matcher.clone(), rt.new_string(source_name)])))))
	{
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('Block bindings source names must contain a namespace prefix. Example: my-plugin/my-custom-source'),
			]),
			rt.new_string('6.5.0')])
		return rt.new_bool(false)
	}
	if rt.is_true(this.is_registered(rt.new_string(source_name))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('Block bindings source "%s" already registered.'),
				]),
				rt.new_string(source_name),
			]),
			rt.new_string('6.5.0')])
	...
	if !(var_source_properties.array_isset(rt.new_string('label'))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('The $source_properties must contain a "label".'),
			]),
			rt.new_string('6.5.0')])
		return rt.new_bool(false)
	}
	if !(var_source_properties.array_isset(rt.new_string('get_value_callback'))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('The $source_properties must contain a "get_value_callback".'),
			]),
			rt.new_string('6.5.0')])
		return rt.new_bool(false)
	}
	if !(rt.call_function('is_callable', [
		var_source_properties.array_get(rt.new_string('get_value_callback')),
	])) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('The "get_value_callback" parameter must be a valid callback.'),
			]),
			rt.new_string('6.5.0')])
		return rt.new_bool(false)
	}
	if var_source_properties.array_isset(rt.new_string('uses_context'))
		&& !(var_source_properties.array_get(rt.new_string('uses_context')).is_array()) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('The "uses_context" parameter must be an array.'),
			]),
			rt.new_string('6.5.0')])
		return rt.new_bool(false)
	}
	if !(!rt.is_true(rt.call_function('array_diff', [
		rt.func_array_keys(var_source_properties),
		this.allowed_source_properties,
	]))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('The $source_properties array contains invalid properties.'),
			]),
			rt.new_string('6.5.0')])
		return rt.new_bool(false)
	}
	mut var_source := create_wp_block_bindings_source(rt.new_string(source_name),
		var_source_properties)
	this.sources.array_set(source_name, var_source)
...
fn (mut this Class_WP_Block_Bindings_Registry) unregister(source_name string) bool {
	if rt.is_true(rt.new_bool(!(rt.is_true(this.is_registered(rt.new_string(source_name)))))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [rt.new_string('Block binding "%s" not found.')]),
				rt.new_string(source_name),
			]),
			rt.new_string('6.5.0')])
		return false
	}
	mut var_unregistered_source := this.sources.array_get(rt.new_string(source_name))
...
fn (mut this Class_WP_Block_Bindings_Registry) get_registered(source_name string) rt.PhpVal {
	...
	return this.sources.array_get(rt.new_string(source_name))
}
...
fn Class_WP_Block_Bindings_Registry.get_instance() rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_null(), rt.get_static_prop('WP_Block_Bindings_Registry',
		'instance')))
	{
		rt.set_static_prop('WP_Block_Bindings_Registry', 'instance', rt.new_object('WP_Block_Bindings_Registry',
			[]string{}, create_wp_block_bindings_registry()))
	}
	return rt.get_static_prop('WP_Block_Bindings_Registry', 'instance')
}
...
fn create_wp_block_bindings_source(_args ...rt.PhpVal) &Class_WP_Block_Bindings_Source {
...
fn (mut this Class_WP_Block_Bindings_Registry) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'allowed_source_properties' {
			this.allowed_source_properties = val
			return true
		}
		else {
