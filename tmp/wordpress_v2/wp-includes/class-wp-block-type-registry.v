...
fn init_static_wp_block_type_registry() {
	rt.init_static_prop('WP_Block_Type_Registry', 'instance', rt.new_null())
}
...
fn (mut this Class_WP_Block_Type_Registry) register(var_name rt.PhpVal, var_args rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(rt.instance_of(var_name_mutated, 'WP_Block_Type'))) {
		var_block_type = var_name_mutated.clone()
		var_name_mutated = rt.get_property(var_block_type, 'name')
	}
	if !(var_name_mutated.clone().is_string()) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [rt.new_string('Block type names must be strings.')]),
			rt.new_string('5.0.0')])
		return false
	}
		...
		var_name_mutated.clone()]))
	{
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('Block type names must not contain uppercase characters.'),
			]),
			rt.new_string('5.0.0')])
		return false
	}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [
		var_name_matcher.clone(), var_name_mutated.clone()])))))
	{
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('Block type names must contain a namespace prefix. Example: my-plugin/my-custom-block-type'),
			]),
			rt.new_string('5.0.0')])
		return false
	}
	if rt.is_true(this.is_registered(var_name_mutated.clone())) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('Block type "%s" is already registered.'),
				]),
				var_name_mutated.clone(),
			]),
			rt.new_string('5.0.0')])
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_block_type)))) {
		var_block_type = create_wp_block_type(var_name_mutated.clone(), var_args.clone())
	}
	this.registered_block_types.array_set(var_name_mutated, var_block_type.clone())
...
fn (mut this Class_WP_Block_Type_Registry) unregister(var_name rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(this.is_registered(var_name_mutated.clone()))))) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('Block type "%s" is not registered.'),
				]),
				var_name_mutated.clone(),
			]),
			rt.new_string('5.0.0')])
...
fn (mut this Class_WP_Block_Type_Registry) is_registered(var_name rt.PhpVal) rt.PhpVal {
	mut var_name_mutated := var_name
	return rt.new_bool(!var_name_mutated.is_null()
		&& this.registered_block_types.array_isset(var_name_mutated))
}
...
fn (mut this Class_WP_Block_Type_Registry) magic_wakeup() {
	...
	if !(this.registered_block_types.is_array()) {
...
fn Class_WP_Block_Type_Registry.get_instance() rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_null(), rt.get_static_prop('WP_Block_Type_Registry',
		'instance')))
	{
		rt.set_static_prop('WP_Block_Type_Registry', 'instance', rt.new_object('WP_Block_Type_Registry',
			[]string{}, create_wp_block_type_registry()))
	}
	return rt.get_static_prop('WP_Block_Type_Registry', 'instance')
}
...
fn create_wp_block_type(_args ...rt.PhpVal) &Class_WP_Block_Type {
...
fn (mut this Class_WP_Block_Type_Registry) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'registered_block_types' {
			this.registered_block_types = val
			return true
		}
		else {
