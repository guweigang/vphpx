...
fn (mut this Class_IXR_Date) construct(var_time rt.PhpVal) {
	if rt.is_true(rt.new_bool(var_time.clone().is_long() || var_time.clone().is_double())) {
		this.parsetimestamp(var_time.clone())
	} else {
		this.parseiso(var_time.clone())
	}
}

fn (mut this Class_IXR_Date) ixr_date(var_time rt.PhpVal) {
	mut iife_temp_0 := Class_IXR_Date{}
	iife_temp_0.construct(var_time.clone())
	rt.new_null()
}
...
fn (mut this Class_IXR_Date) parseiso(var_iso rt.PhpVal) {
	this.year = rt.call_function('substr', [var_iso.clone(), rt.new_int(0),
		rt.new_int(4)])
	this.month = rt.call_function('substr', [var_iso.clone(),
		rt.new_int(4), rt.new_int(2)])
	this.day = rt.call_function('substr', [var_iso.clone(), rt.new_int(6),
		rt.new_int(2)])
	this.hour = rt.call_function('substr', [var_iso.clone(), rt.new_int(9),
		rt.new_int(2)])
	this.minute = rt.call_function('substr', [var_iso.clone(),
		rt.new_int(12), rt.new_int(2)])
	this.second = rt.call_function('substr', [var_iso.clone(),
		rt.new_int(15), rt.new_int(2)])
	this.timezone = rt.call_function('substr', [var_iso.clone(),
		rt.new_int(17)])
}
