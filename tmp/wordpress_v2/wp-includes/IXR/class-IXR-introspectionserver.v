...
fn (mut this Class_IXR_IntrospectionServer) construct() {
	...
	rt.get_property(rt.new_object('IXR_IntrospectionServer', ['IXR_Server'], &this), 'capabilities').array_set('introspection', rt.create_array([
		rt.ArrayItem{
			key: 'specUrl'
			val: 'https://web.archive.org/web/20050404090342/http://xmlrpc.usefulinc.com/doc/reserved.html'
		},
		rt.ArrayItem{ key: 'specVersion', val: 1 },
	]))
	this.addcallback(rt.new_string('system.methodSignature'),
		rt.new_string('this:methodSignature'), rt.create_array([
		rt.ArrayItem{ key: none, val: 'array' },
		rt.ArrayItem{ key: none, val: 'string' },
	]),
		rt.new_string('Returns an array describing the return type and required parameters of a method'))
	this.addcallback(rt.new_string('system.getCapabilities'),
		rt.new_string('this:getCapabilities'), rt.create_array([
		rt.ArrayItem{ key: none, val: 'struct' },
	]),
		rt.new_string('Returns a struct describing the XML-RPC specifications supported by this server'))
	this.addcallback(rt.new_string('system.listMethods'), rt.new_string('this:listMethods'), rt.create_array([
		rt.ArrayItem{ key: none, val: 'array' },
	]), rt.new_string('Returns an array of available methods on this server'))
	this.addcallback(rt.new_string('system.methodHelp'), rt.new_string('this:methodHelp'), rt.create_array([
		rt.ArrayItem{ key: none, val: 'string' },
		rt.ArrayItem{ key: none, val: 'string' },
	]), rt.new_string('Returns a documentation string for the specified method'))
}

fn (mut this Class_IXR_IntrospectionServer) ixr_introspectionserver() {
	mut iife_temp_0 := Class_IXR_IntrospectionServer{}
...
fn (mut this Class_IXR_IntrospectionServer) addcallback(var_method rt.PhpVal, var_callback rt.PhpVal, var_args rt.PhpVal, var_help rt.PhpVal) {
	...
	rt.get_property(rt.new_object('IXR_IntrospectionServer', ['IXR_Server'], &this), 'callbacks').array_set(var_method_mutated,
		var_callback.clone())
	this.signatures.array_set(var_method_mutated, var_args_mutated.clone())
	this.help.array_set(var_method_mutated, var_help.clone())
}

fn (mut this Class_IXR_IntrospectionServer) call(var_methodname rt.PhpVal, var_args rt.PhpVal) rt.PhpVal {
	mut var_args_mutated := var_args
	if rt.is_true(var_args_mutated) && !(var_args_mutated.clone().is_array()) {
		var_args_mutated = rt.create_array([
			rt.ArrayItem{ key: none, val: var_args_mutated },
		])
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(this.hasmethod(var_methodname.clone()))))) {
		return rt.new_object('IXR_Error', []string{}, create_ixr_error(-32601,
			'server error. requested method "' +
			(rt.get_property(rt.get_property(rt.new_object('IXR_IntrospectionServer', ['IXR_Server'], &this), 'message'), 'methodName')).str() +
			'" not specified.'))
	}
	mut var_method := rt.get_property(rt.new_object('IXR_IntrospectionServer', [
		'IXR_Server',
	], &this), 'callbacks').array_get(var_methodname)
	mut var_signature := this.signatures.array_get(var_methodname)
	mut var_returnType := rt.call_function('array_shift', [var_signature.clone()])
	if rt.is_true(rt.new_bool(var_args_mutated.clone().array_count() != var_signature.clone().array_count())) {
		return rt.new_object('IXR_Error', []string{}, create_ixr_error(-32602,
			rt.new_string('server error. wrong number of method parameters')))
	}
	mut var_ok := rt.new_bool(true)
	mut var_argsbackup := var_args_mutated.clone()
	mut var_i := rt.new_int(0)
	mut var_j := rt.new_int(var_args_mutated.clone().array_count())
	for {
		if !(rt.is_true(rt.less(var_i, var_j))) { break
		 }
		mut var_arg := rt.call_function('array_shift', [var_args_mutated.clone()])
		mut var_type := rt.call_function('array_shift', [var_signature.clone()])
		mut switch_val_1 := var_type
		if rt.is_true(rt.equal(switch_val_1, rt.new_string('int')))
			|| rt.is_true(rt.equal(switch_val_1, rt.new_string('i4'))) {
			if var_arg.clone().is_array() || !(var_arg.clone().is_long()) {
				var_ok = rt.new_bool(false)
			}
		} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('base64')))
			|| rt.is_true(rt.equal(switch_val_1, rt.new_string('string'))) {
			if !(var_arg.clone().is_string()) {
				var_ok = rt.new_bool(false)
			}
		} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('boolean'))) {
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_arg, rt.new_bool(false)))))
				&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_arg, rt.new_bool(true))))) {
				var_ok = rt.new_bool(false)
			}
		} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('float')))
			|| rt.is_true(rt.equal(switch_val_1, rt.new_string('double'))) {
			if !(var_arg.clone().is_double()) {
				var_ok = rt.new_bool(false)
			}
		} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('date')))
			|| rt.is_true(rt.equal(switch_val_1, rt.new_string('dateTime.iso8601'))) {
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_a', [
				var_arg.clone(), rt.new_string('IXR_Date')])))))
			{
				var_ok = rt.new_bool(false)
			}
		}
		if rt.is_true(rt.new_bool(!(rt.is_true(var_ok)))) {
			return rt.new_object('IXR_Error', []string{}, create_ixr_error(-32602,
				rt.new_string('server error. invalid method parameters')))
		}
		rt.post_inc(var_i)
	}
	return this.Class_IXR_Server.call(var_methodname.clone(), var_argsbackup.clone())
}

fn (mut this Class_IXR_IntrospectionServer) methodsignature(var_method rt.PhpVal) rt.PhpVal {
	mut var_method_mutated := var_method
	if rt.is_true(rt.new_bool(!(rt.is_true(this.hasmethod(var_method_mutated.clone()))))) {
		return create_ixr_error(-32601, 'server error. requested method "' +
			var_method_mutated.str() + '" not specified.')
	}
	...
	for {
		...
		if rt.is_true(rt.equal(switch_val_2, rt.new_string('string'))) {
		...
		} else if rt.is_true(rt.equal(switch_val_2, rt.new_string('int')))
			...
			var_return.array_push(rt.create_array([
				rt.ArrayItem{ key: 'struct', val: 'struct' },
			]))
		}
	}
...
fn create_ixr_base64(_args ...rt.PhpVal) &Class_IXR_Base64 {
