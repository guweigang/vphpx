...
fn (mut this Class_WP_REST_Response) add_links(var_links rt.PhpVal) {
	...
	for {
		...
		for {
			...
			this.add_link(var_rel.clone(), var_attributes.array_get(rt.new_string('href')),
...
fn (mut this Class_WP_REST_Response) link_header(var_rel rt.PhpVal, var_link rt.PhpVal, var_other rt.PhpVal) {
	...
	for {
		...
		var_header = rt.concat(var_header, rt.new_string('; ' + var_key.str() + '=' +
			var_value.str()))
	}
	this.header(rt.new_string('Link'), var_header.clone(), rt.new_bool(false))
}
...
fn (mut this Class_WP_REST_Response) set_matched_route(var_route rt.PhpVal) {
	this.matched_route = var_route.clone()
}
...
fn (mut this Class_WP_REST_Response) set_matched_handler(var_handler rt.PhpVal) {
	this.matched_handler = var_handler.clone()
}
...
fn (mut this Class_WP_REST_Response) as_error() rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(this.get_data().is_array())) {
		...
		var_error.add(var_data.array_get(rt.new_string('code')),
			var_data.array_get(rt.new_string('message')), var_data.array_get(rt.new_string('data')))
		if !(!rt.is_true(var_data.array_get(rt.new_string('additional_errors')))) {
			mut iter_4 := var_data.array_get(rt.new_string('additional_errors')).iterator()
			for {
				item_4 := iter_4.next() or { break }
				mut var_err := item_4.val
				var_error.add(var_err.array_get(rt.new_string('code')),
					var_err.array_get(rt.new_string('message')),
					var_err.array_get(rt.new_string('data')))
			}
		}
	} else {
...
fn (mut this Class_WP_REST_Response) get_curies() rt.PhpVal {
	...
	return rt.call_function('array_merge', [rt.create_array_from_list(var_curies),
		var_additional.clone()])
}
