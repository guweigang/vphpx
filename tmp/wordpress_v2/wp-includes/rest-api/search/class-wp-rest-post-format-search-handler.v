...
fn (mut this Class_WP_REST_Post_Format_Search_Handler) search_items(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	mut var_format_slugs := rt.func_array_keys(var_format_strings.clone())
	...
	var_query_args = rt.call_function('apply_filters', [
		rt.new_string('rest_post_format_search_query'),
	...
	for {
		...
		if !(!rt.is_true(var_query_args.array_get(rt.new_string('search')))) {
			mut var_format_string := rt.call_function('get_post_format_string', [
				var_format_slug.clone(),
			])
			mut var_format_slug_match := rt.new_bool(!rt.is_true(rt.identical(rt.call_function('stripos', [
				var_format_slug.clone(),
				var_query_args.array_get(rt.new_string('search')),
			]), rt.new_bool(false))))
			mut var_format_string_match := rt.new_bool(!rt.is_true(rt.identical(rt.call_function('stripos', [
				var_format_string.clone(),
				var_query_args.array_get(rt.new_string('search')),
			]), rt.new_bool(false))))
			if rt.is_true(rt.new_bool(!(rt.is_true(var_format_slug_match))))
				&& rt.is_true(rt.new_bool(!(rt.is_true(var_format_string_match)))) {
				continue
			}
		}
		mut var_format_link := rt.call_function('get_post_format_link', [
			var_format_slug.clone()])
		if rt.is_true(var_format_link) {
			var_found_ids << var_format_slug.clone()
		}
	}
	...
	return rt.create_array([
		rt.ArrayItem{ key: Class_WP_REST_Post_Format_Search_Handler.result_ids(), val: rt.call_function('array_slice', [
			rt.create_array_from_list(var_found_ids),
			rt.mul(rt.sub(var_page, rt.new_int(1)), var_per_page),
			var_per_page.clone(),
		]) },
		rt.ArrayItem{
			key: Class_WP_REST_Post_Format_Search_Handler.result_total()
			val: var_found_ids.len
		},
	])
}
...
fn (mut this Class_WP_REST_Post_Format_Search_Handler) prepare_item(var_id rt.PhpVal, mut var_fields Class_array) rt.PhpVal {
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_id(), var_id.clone())
	}
	if rt.is_true(rt.call_function('in_array', [
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_title(), rt.call_function('get_post_format_string', [
			var_id.clone(),
		]))
	}
	if rt.is_true(rt.call_function('in_array', [
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_url(), rt.call_function('get_post_format_link', [
			var_id.clone(),
		]))
	}
	if rt.is_true(rt.call_function('in_array', [
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_type(), rt.get_property(rt.new_object('WP_REST_Post_Format_Search_Handler', [
