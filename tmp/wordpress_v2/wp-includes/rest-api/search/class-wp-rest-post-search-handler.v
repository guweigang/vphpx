...
fn (mut this Class_WP_REST_Post_Search_Handler) construct() {
	...
	this.dispatch_set_prop('subtypes', rt.call_function('array_diff', [
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: 'attachment' },
		]),
	]))
}
...
fn (mut this Class_WP_REST_Post_Search_Handler) search_items(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('in_array', [
		Class_WP_REST_Search_Controller.type_any(),
		var_post_types.clone(),
		rt.new_bool(true),
	]))
	{
		var_post_types = rt.get_property(rt.new_object('WP_REST_Post_Search_Handler', [
			'WP_REST_Search_Handler',
		], &this), 'subtypes')
	}
	mut var_query_args := rt.create_array([
		rt.ArrayItem{ key: 'post_type', val: var_post_types },
		rt.ArrayItem{ key: 'post_status', val: 'publish' },
		rt.ArrayItem{
			key: 'paged'
			val: rt.new_int((var_request.array_get(rt.new_string('page'))).to_i64())
		},
		rt.ArrayItem{
			key: 'posts_per_page'
			val: rt.new_int((var_request.array_get(rt.new_string('per_page'))).to_i64())
		},
		rt.ArrayItem{ key: 'ignore_sticky_posts', val: true },
	])
	if !(!rt.is_true(var_request.array_get(rt.new_string('search')))) {
		var_query_args.array_set('s', var_request.array_get(rt.new_string('search')))
	}
	if !(!rt.is_true(var_request.array_get(rt.new_string('exclude')))) {
		var_query_args.array_set('post__not_in', var_request.array_get(rt.new_string('exclude')))
	}
	if !(!rt.is_true(var_request.array_get(rt.new_string('include')))) {
		var_query_args.array_set('post__in', var_request.array_get(rt.new_string('include')))
	}
	var_query_args = rt.call_function('apply_filters', [
		rt.new_string('rest_post_search_query'),
		var_query_args.clone(),
		var_request,
	])
	mut var_query := create_wp_query()
	mut var_posts := var_query.query(var_query_args.clone())
	mut var_found_ids := rt.call_function('wp_list_pluck', [var_posts.clone(),
		rt.new_string('ID')])
	mut var_total := rt.get_property(var_query, 'found_posts')
	return rt.create_array([
		rt.ArrayItem{ key: Class_WP_REST_Post_Search_Handler.result_ids(), val: var_found_ids },
		rt.ArrayItem{ key: Class_WP_REST_Post_Search_Handler.result_total(), val: var_total },
	])
}
...
fn (mut this Class_WP_REST_Post_Search_Handler) prepare_item(var_id rt.PhpVal, mut var_fields Class_array) rt.PhpVal {
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_id(), rt.new_int((rt.get_property(var_post,
			'ID')).to_i64()))
	}
	if rt.is_true(rt.call_function('in_array', [
	...
	]))
	{
		...
		]))
		{
			rt.call_function('add_filter', [rt.new_string('protected_title_format'),
				rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_REST_Post_Search_Handler', [
						'WP_REST_Search_Handler',
					], &this) },
			...
			var_data.array_set(Class_WP_REST_Search_Controller.prop_title(), rt.call_function('get_the_title', [
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_url(), rt.call_function('get_permalink', [
			rt.get_property(var_post, 'ID'),
		]))
	}
	if rt.is_true(rt.call_function('in_array', [
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_type(), rt.get_property(rt.new_object('WP_REST_Post_Search_Handler', [
			'WP_REST_Search_Handler',
		], &this), 'type'))
	}
	if rt.is_true(rt.call_function('in_array', [
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_subtype(), rt.get_property(var_post,
			'post_type'))
	}
...
fn (mut this Class_WP_REST_Post_Search_Handler) prepare_item_links(var_id rt.PhpVal) rt.PhpVal {
	...
	mut var_item_route := rt.call_function('rest_get_route_for_post', [
		var_post.clone()])
	if !(!rt.is_true(var_item_route)) {
		var_links['self'] = rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				var_item_route.clone()]) },
			rt.ArrayItem{ key: 'embeddable', val: true },
		])
	}
	var_links['about'] = rt.create_array([
		rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
			rt.new_string('wp/v2/types/' + (rt.get_property(var_post, 'post_type')).str()),
		]) },
	])
...
fn (mut this Class_WP_REST_Post_Search_Handler) detect_rest_item_route(var_post rt.PhpVal) rt.PhpVal {
	...
	rt.call_function('_deprecated_function', [rt.new_string(@METHOD),
		rt.new_string('5.5.0'), rt.new_string('rest_get_route_for_post()')])
	return rt.call_function('rest_get_route_for_post', [var_post_mutated.clone()])
}
