...
fn (mut this Class_WP_REST_Term_Search_Handler) construct() {
	...
	this.dispatch_set_prop('subtypes', rt.call_function('array_values', [
...
fn (mut this Class_WP_REST_Term_Search_Handler) search_items(mut var_request Class_WP_REST_Request) rt.PhpVal {
	...
	{
		var_taxonomies = rt.get_property(rt.new_object('WP_REST_Term_Search_Handler', [
	...
	mut var_query_args := rt.create_array([
		rt.ArrayItem{ key: 'taxonomy', val: var_taxonomies },
		rt.ArrayItem{ key: 'hide_empty', val: false },
		rt.ArrayItem{ key: 'offset', val: rt.mul(rt.sub(var_page, rt.new_int(1)), var_per_page) },
		rt.ArrayItem{ key: 'number', val: var_per_page },
	])
	if !(!rt.is_true(var_request.array_get(rt.new_string('search')))) {
		var_query_args.array_set('search', var_request.array_get(rt.new_string('search')))
	}
	if !(!rt.is_true(var_request.array_get(rt.new_string('exclude')))) {
		var_query_args.array_set('exclude', var_request.array_get(rt.new_string('exclude')))
	}
	if !(!rt.is_true(var_request.array_get(rt.new_string('include')))) {
		var_query_args.array_set('include', var_request.array_get(rt.new_string('include')))
	}
	var_query_args = rt.call_function('apply_filters', [
		rt.new_string('rest_term_search_query'),
		var_query_args.clone(),
		var_request,
	])
	mut var_query := create_wp_term_query()
	mut var_found_terms := var_query.query(var_query_args.clone())
	mut var_found_ids := rt.call_function('wp_list_pluck', [var_found_terms.clone(),
		rt.new_string('term_id')])
	var_query_args.array_unset(rt.new_string('offset'))
	var_query_args.array_unset(rt.new_string('number'))
	mut var_total := rt.call_function('wp_count_terms', [var_query_args.clone()])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_total)))) {
		var_total = rt.new_int(0)
	}
	return rt.create_array([
		rt.ArrayItem{ key: Class_WP_REST_Term_Search_Handler.result_ids(), val: var_found_ids },
		rt.ArrayItem{ key: Class_WP_REST_Term_Search_Handler.result_total(), val: var_total },
	])
}
...
fn (mut this Class_WP_REST_Term_Search_Handler) prepare_item(var_id rt.PhpVal, mut var_fields Class_array) rt.PhpVal {
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_id(), rt.new_int(var_id.to_i64()))
	}
	if rt.is_true(rt.call_function('in_array', [
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_title(), rt.get_property(var_term,
			'name'))
	}
	if rt.is_true(rt.call_function('in_array', [
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_url(), rt.call_function('get_term_link', [
			var_id.clone(),
		]))
	}
	if rt.is_true(rt.call_function('in_array', [
	...
	{
		var_data.array_set(Class_WP_REST_Search_Controller.prop_type(), rt.get_property(var_term,
			'taxonomy'))
	}
...
fn (mut this Class_WP_REST_Term_Search_Handler) prepare_item_links(var_id rt.PhpVal) rt.PhpVal {
	...
	mut var_item_route := rt.call_function('rest_get_route_for_term', [
		var_term.clone()])
	...
	var_links['about'] = rt.create_array([
		rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
			rt.call_function('sprintf', [rt.new_string('wp/v2/taxonomies/%s'),
