...
fn (mut this Class_WP_REST_Taxonomies_Controller) register_routes() {
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('WP_REST_Taxonomies_Controller', [
			'WP_REST_Controller',
		], &this), 'namespace'),
		rt.new_string('/' +
			rt.get_property(rt.new_object('WP_REST_Taxonomies_Controller', ['WP_REST_Controller'], &this), 'rest_base')),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.readable() },
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_REST_Taxonomies_Controller', [
						'WP_REST_Controller',
					], &this) },
			...
			rt.ArrayItem{ key: 'args', val: rt.create_array([
				rt.ArrayItem{ key: 'taxonomy', val: rt.create_array([
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('An alphanumeric identifier for the taxonomy.'),
					]) },
		...
		var_request.array_get(rt.new_string('context'))))
	{
		if !(!rt.is_true(var_request.array_get(rt.new_string('type')))) {
			...
			var_taxonomies = rt.call_function('get_taxonomies', [
				rt.new_string(''), rt.new_string('objects')])
		}
		mut iter_1 := var_taxonomies.iterator()
		for {
			item_1 := iter_1.next() or { break }
			mut var_taxonomy := item_1.val
			if !(!rt.is_true(rt.get_property(var_taxonomy, 'show_in_rest')))
				&& rt.is_true(rt.call_function('current_user_can', [rt.get_property(rt.get_property(var_taxonomy, 'cap'), 'assign_terms')])) {
				return true
			}
		}
		return (create_wp_error(rt.new_string('rest_cannot_view'), rt.call_function('__', [
			rt.new_string('Sorry, you are not allowed to manage terms in this taxonomy.'),
		]), rt.create_array([
...
fn (mut this Class_WP_REST_Taxonomies_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	if var_registered.array_isset(rt.new_string('type'))
		&& !(!rt.is_true(var_request.array_get(rt.new_string('type')))) {
		mut var_taxonomies := rt.call_function('get_object_taxonomies', [
			var_request.array_get(rt.new_string('type')),
			rt.new_string('objects'),
		])
	...
	for {
		...
		mut var_tax_type := item_2.key
		if !rt.is_true(rt.get_property(var_value, 'show_in_rest'))|| (rt.is_true(rt.identical(rt.new_string('edit'), var_request.array_get(rt.new_string('context'))))
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.get_property(rt.get_property(var_value, 'cap'), 'assign_terms')])))))) {
			continue
		}
		mut var_tax := this.prepare_item_for_response(var_value.clone(), var_request.clone())
		var_tax = this.prepare_response_for_collection(var_tax.clone())
		var_data.array_set(var_tax_type, var_tax.clone())
	}
...
fn (mut this Class_WP_REST_Taxonomies_Controller) get_item_permissions_check(var_request rt.PhpVal) bool {
	mut var_tax_obj := rt.call_function('get_taxonomy', [
		var_request.array_get(rt.new_string('taxonomy')),
	])
	if rt.is_true(var_tax_obj) {
		...
		if rt.is_true(rt.identical(rt.new_string('edit'), var_request.array_get(rt.new_string('context'))))
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.get_property(rt.get_property(var_tax_obj, 'cap'), 'assign_terms')]))))) {
...
fn (mut this Class_WP_REST_Taxonomies_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	...
	if !rt.is_true(var_tax_obj) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('rest_taxonomy_invalid'), rt.call_function('__', [
			rt.new_string('Invalid taxonomy.'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }])))
	}
	mut var_data := this.prepare_item_for_response(var_tax_obj.clone(), var_request.clone())
	return rt.call_function('rest_ensure_response', [var_data.clone()])
}
...
fn (mut this Class_WP_REST_Taxonomies_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_method(var_request, 'is_method', [rt.new_string('HEAD')])) {
		return rt.call_function('apply_filters', [rt.new_string('rest_prepare_taxonomy'),
			create_wp_rest_response(rt.new_array()), var_taxonomy.clone(),
			var_request.clone()])
	}
	...
	if rt.is_true(rt.call_function('in_array', [rt.new_string('types'),
		var_fields.clone(), rt.new_bool(true)]))
	{
		var_data.array_set('types', rt.call_function('array_values', [
			rt.get_property(var_taxonomy, 'object_type'),
		]))
	}
	if rt.is_true(rt.call_function('in_array', [rt.new_string('show_cloud'),
		var_fields.clone(), rt.new_bool(true)]))
	{
		var_data.array_set('show_cloud', rt.get_property(var_taxonomy, 'show_tagcloud'))
	}
	if rt.is_true(rt.call_function('in_array', [rt.new_string('hierarchical'),
		var_fields.clone(), rt.new_bool(true)]))
	{
		var_data.array_set('hierarchical', rt.get_property(var_taxonomy, 'hierarchical'))
	}
	if rt.is_true(rt.call_function('in_array', [rt.new_string('rest_base'),
		var_fields.clone(), rt.new_bool(true)]))
	{
		var_data.array_set('rest_base', var_base.clone())
	}
	if rt.is_true(rt.call_function('in_array', [rt.new_string('rest_namespace'),
		var_fields.clone(), rt.new_bool(true)]))
	{
		var_data.array_set('rest_namespace', rt.get_property(var_taxonomy, 'rest_namespace'))
	}
	if rt.is_true(rt.call_function('in_array', [rt.new_string('visibility'),
		var_fields.clone(), rt.new_bool(true)]))
	{
		var_data.array_set('visibility', rt.create_array([
			rt.ArrayItem{ key: 'public', val: (rt.get_property(var_taxonomy, 'public')).to_bool() },
			rt.ArrayItem{ key: 'publicly_queryable', val: (rt.get_property(var_taxonomy,
				'publicly_queryable')).to_bool() },
			rt.ArrayItem{ key: 'show_admin_column', val: (rt.get_property(var_taxonomy,
				'show_admin_column')).to_bool() },
			rt.ArrayItem{ key: 'show_in_nav_menus', val: (rt.get_property(var_taxonomy,
				'show_in_nav_menus')).to_bool() },
			rt.ArrayItem{ key: 'show_in_quick_edit', val: (rt.get_property(var_taxonomy,
				'show_in_quick_edit')).to_bool() },
			rt.ArrayItem{ key: 'show_ui', val: (rt.get_property(var_taxonomy, 'show_ui')).to_bool() },
		]))
	}
	...
	if rt.is_true(rt.call_function('rest_is_field_included', [rt.new_string('_links'), var_fields.clone()]))
		...
		rt.call_method(var_response, 'add_links', [
			this.prepare_links(var_taxonomy.clone()),
		])
	}
	return rt.call_function('apply_filters', [rt.new_string('rest_prepare_taxonomy'),
		var_response.clone(), var_taxonomy.clone(), var_request.clone()])
}
...
fn (mut this Class_WP_REST_Taxonomies_Controller) prepare_links(var_taxonomy rt.PhpVal) rt.PhpVal {
	...
	return rt.create_array([
		...
		rt.ArrayItem{ key: 'https://api.w.org/items', val: rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				rt.call_function('rest_get_route_for_taxonomy_items', [
					rt.get_property(var_taxonomy_mutated, 'name')]),
			]) },
		]) },
	])
}
...
fn (mut this Class_WP_REST_Taxonomies_Controller) get_item_schema() rt.PhpVal {
	if rt.is_true(rt.get_property(rt.new_object('WP_REST_Taxonomies_Controller', [
		'WP_REST_Controller',
	], &this), 'schema'))
	{
		return this.add_additional_fields_schema(rt.get_property(rt.new_object('WP_REST_Taxonomies_Controller', [
			'WP_REST_Controller',
		], &this), 'schema'))
	}
	mut var_schema := {
		'$schema':    rt.new_string('http://json-schema.org/draft-04/schema#')
		'title':      rt.new_string('taxonomy')
		'type':       rt.new_string('object')
		'properties': {
			'capabilities':   {
				'description': rt.call_function('__', [
					rt.new_string('All capabilities used by the taxonomy.'),
				])
				'type':        rt.new_string('object')
			...
			'description':    {
				'description': rt.call_function('__', [
					rt.new_string('A human-readable description of the taxonomy.'),
				])
			...
			'hierarchical':   {
				'description': rt.call_function('__', [
					rt.new_string('Whether or not the taxonomy should have children.'),
				])
			...
			'labels':         {
				'description': rt.call_function('__', [
					rt.new_string('Human-readable labels for the taxonomy for various contexts.'),
				])
				'type':        rt.new_string('object')
				'context':     map[string]rt.PhpVal{}
				'readonly':    rt.new_bool(true)
			...
			'name':           {
				'description': rt.call_function('__', [
					rt.new_string('The title for the taxonomy.'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
				'readonly':    rt.new_bool(true)
			...
			'show_cloud':     {
				'description': rt.call_function('__', [
					rt.new_string('Whether or not the term cloud should be displayed.'),
				])
				'type':        rt.new_string('boolean')
				'context':     map[string]rt.PhpVal{}
				'readonly':    rt.new_bool(true)
			...
			'types':          {
				'description': rt.call_function('__', [
					rt.new_string('Types associated with the taxonomy.'),
				])
			...
			'rest_base':      {
				'description': rt.call_function('__', [
					rt.new_string('REST base route for the taxonomy.'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
				'readonly':    rt.new_bool(true)
			...
			'rest_namespace': {
				'description': rt.call_function('__', [
					rt.new_string('REST namespace route for the taxonomy.'),
				])
				'type':        rt.new_string('string')
				'context':     map[string]rt.PhpVal{}
				'readonly':    rt.new_bool(true)
			...
			'visibility':     {
				'description': rt.call_function('__', [
					rt.new_string('The visibility settings for the taxonomy.'),
				])
				'type':        rt.new_string('object')
				'context':     map[string]rt.PhpVal{}
				'readonly':    rt.new_bool(true)
				'properties':  {
					'public':             {
						'description': rt.call_function('__', [
							rt.new_string('Whether a taxonomy is intended for use publicly either via the admin interface or by front-end users.'),
						])
						'type':        rt.new_string('boolean')
					}
					'publicly_queryable': {
						'description': rt.call_function('__', [
							rt.new_string('Whether the taxonomy is publicly queryable.'),
						])
						'type':        rt.new_string('boolean')
					}
					'show_ui':            {
						'description': rt.call_function('__', [
							rt.new_string('Whether to generate a default UI for managing this taxonomy.'),
						])
						'type':        rt.new_string('boolean')
					}
					'show_admin_column':  {
						'description': rt.call_function('__', [
							rt.new_string('Whether to allow automatic creation of taxonomy columns on associated post-types table.'),
						])
						'type':        rt.new_string('boolean')
					}
					'show_in_nav_menus':  {
						'description': rt.call_function('__', [
							rt.new_string('Whether to make the taxonomy available for selection in navigation menus.'),
						])
						'type':        rt.new_string('boolean')
					}
					'show_in_quick_edit': {
						'description': rt.call_function('__', [
							rt.new_string('Whether to show the taxonomy in the quick/bulk edit panel.'),
						])
						'type':        rt.new_string('boolean')
					}
				}
...
fn (mut this Class_WP_REST_Taxonomies_Controller) get_collection_params() rt.PhpVal {
	...
	var_new_params['context'] = this.get_context_param(rt.create_array([
		rt.ArrayItem{ key: 'default', val: 'view' },
	]))
	var_new_params['type'] = rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Limit results to taxonomies associated with a specific post type.'),
		]) },
		rt.ArrayItem{ key: 'type', val: 'string' },
	])
	return var_new_params.clone()
}
