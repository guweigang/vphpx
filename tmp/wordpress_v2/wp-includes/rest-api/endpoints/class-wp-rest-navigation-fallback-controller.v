...
fn (mut this Class_WP_REST_Navigation_Fallback_Controller) register_routes() {
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('WP_REST_Navigation_Fallback_Controller', [
			'WP_REST_Controller',
		], &this), 'namespace'),
		rt.new_string('/' +
			rt.get_property(rt.new_object('WP_REST_Navigation_Fallback_Controller', ['WP_REST_Controller'], &this), 'rest_base')),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.readable() },
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_REST_Navigation_Fallback_Controller', [
						'WP_REST_Controller',
					], &this) },
				...
				rt.ArrayItem{
					key: 'args'
					val: this.get_endpoint_args_for_item_schema(Class_WP_REST_Server.readable())
				},
			]) },
...
fn (mut this Class_WP_REST_Navigation_Fallback_Controller) get_item_permissions_check(var_request rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [rt.get_property(rt.get_property(var_post_type, 'cap'), 'create_posts')])))))
		...
		return (create_wp_error(rt.new_string('rest_cannot_create'), rt.call_function('__', [
			rt.new_string('Sorry, you are not allowed to create Navigation Menus as this user.'),
		]), rt.create_array([
	...
	if rt.is_true(rt.identical(rt.new_string('edit'), var_request.array_get(rt.new_string('context'))))
		...
		return (create_wp_error(rt.new_string('rest_forbidden_context'), rt.call_function('__', [
			rt.new_string('Sorry, you are not allowed to edit Navigation Menus as this user.'),
		]), rt.create_array([
			rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code',
				[]rt.PhpVal{}) },
		]))).to_bool()
...
fn (mut this Class_WP_REST_Navigation_Fallback_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	mut iife_temp_0 := Class_WP_Navigation_Fallback{}
	mut iife_result_0 := iife_temp_0.get_fallback()
	mut var_post := iife_result_0
	if !rt.is_true(var_post) {
		return rt.call_function('rest_ensure_response', [
			create_wp_error(rt.new_string('no_fallback_menu'), rt.call_function('__', [
				rt.new_string('No fallback menu found.'),
			]), rt.create_array([
...
fn (mut this Class_WP_REST_Navigation_Fallback_Controller) get_item_schema() rt.PhpVal {
	if rt.is_true(rt.get_property(rt.new_object('WP_REST_Navigation_Fallback_Controller', [
		'WP_REST_Controller',
	], &this), 'schema'))
	{
		return this.add_additional_fields_schema(rt.get_property(rt.new_object('WP_REST_Navigation_Fallback_Controller', [
			'WP_REST_Controller',
		], &this), 'schema'))
	}
	this.dispatch_set_prop('schema', rt.create_array([
		rt.ArrayItem{ key: '$schema', val: 'http://json-schema.org/draft-04/schema#' },
		rt.ArrayItem{ key: 'title', val: 'navigation-fallback' },
		rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			rt.ArrayItem{ key: 'id', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('The unique identifier for the Navigation Menu.'),
				]) },
...
fn (mut this Class_WP_REST_Navigation_Fallback_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	{
		var_data.array_set('id', rt.new_int((rt.get_property(var_item, 'ID')).to_i64()))
	}
