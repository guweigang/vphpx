...
fn (mut this Class_WP_REST_Menu_Locations_Controller) register_routes() {
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('WP_REST_Menu_Locations_Controller', [
			'WP_REST_Controller',
		], &this), 'namespace'),
		rt.new_string('/' +
			rt.get_property(rt.new_object('WP_REST_Menu_Locations_Controller', ['WP_REST_Controller'], &this), 'rest_base')),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.readable() },
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_REST_Menu_Locations_Controller', [
						'WP_REST_Controller',
					], &this) },
			...
			rt.ArrayItem{ key: 'args', val: rt.create_array([
				rt.ArrayItem{ key: 'location', val: rt.create_array([
					rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						rt.new_string('An alphanumeric identifier for the menu location.'),
					]) },
...
fn (mut this Class_WP_REST_Menu_Locations_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		rt.set_property(var_location, 'name', var_name.clone())
		rt.set_property(var_location, 'description', var_description.clone())
		var_location = this.prepare_item_for_response(var_location.clone(), var_request.clone())
		var_data.array_set(var_name, this.prepare_response_for_collection(var_location.clone()))
	}
...
fn (mut this Class_WP_REST_Menu_Locations_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(var_registered_menus.clone().array_isset(var_request.array_get(rt.new_string('location')))))))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('rest_menu_location_invalid'), rt.call_function('__', [
			rt.new_string('Invalid menu location.'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }])))
	}
	mut var_location := create_stdclass()
	rt.set_property(var_location, 'name', var_request.array_get(rt.new_string('location')))
	rt.set_property(var_location, 'description', var_registered_menus.array_get(rt.get_property(var_location,
		'name')))
	mut var_data := this.prepare_item_for_response(var_location.clone(), var_request.clone())
	return rt.call_function('rest_ensure_response', [var_data.clone()])
}
...
fn (mut this Class_WP_REST_Menu_Locations_Controller) check_has_read_only_access(var_request rt.PhpVal) bool {
	mut var_read_only_access := rt.call_function('apply_filters', [
		...
		rt.new_bool(false),
		var_request.clone(),
		rt.new_object('WP_REST_Menu_Locations_Controller', ['WP_REST_Controller'], &this),
	])
	...
	])))))
	{
		return (create_wp_error(rt.new_string('rest_cannot_view'), rt.call_function('__', [
			rt.new_string('Sorry, you are not allowed to view menu locations.'),
		]), rt.create_array([
...
fn (mut this Class_WP_REST_Menu_Locations_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_menu := if !(var_locations.array_get(rt.get_property(var_location, 'name'))).is_null() {
		var_locations.array_get(rt.get_property(var_location, 'name'))
	} else {
		rt.new_int(0)
	}
	...
	if rt.is_true(rt.call_function('rest_is_field_included', [
		rt.new_string('menu'), var_fields.clone()]))
	{
		var_data.array_set('menu', rt.new_int(var_menu.to_i64()))
	}
	...
	if rt.is_true(rt.call_function('rest_is_field_included', [rt.new_string('_links'), var_fields.clone()]))
		...
		rt.call_method(var_response, 'add_links', [
			this.prepare_links(var_location.clone()),
		])
	}
	return rt.call_function('apply_filters', [
		rt.new_string('rest_prepare_menu_location'),
		var_response.clone(),
		var_location.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_WP_REST_Menu_Locations_Controller) prepare_links(var_location rt.PhpVal) rt.PhpVal {
		...
		'self':       {
			'href': rt.call_function('rest_url', [
				rt.new_string((rt.call_function('trailingslashit', [var_base.clone()])).str() +
					(rt.get_property(var_location_mutated, 'name')).str()),
			])
		}
		'collection': {
	...
	mut var_locations := rt.call_function('get_nav_menu_locations', []rt.PhpVal{})
	mut var_menu := if !(var_locations.array_get(rt.get_property(var_location_mutated, 'name'))).is_null() {
		var_locations.array_get(rt.get_property(var_location_mutated, 'name'))
	} else {
		rt.new_int(0)
	}
	if rt.is_true(var_menu) {
		mut var_path := rt.call_function('rest_get_route_for_term', [
			var_menu.clone()])
		if rt.is_true(var_path) {
			mut var_url := rt.call_function('rest_url', [var_path.clone()])
			var_links.array_get_mut('https://api.w.org/menu').array_push(rt.create_array([
...
fn (mut this Class_WP_REST_Menu_Locations_Controller) get_item_schema() rt.PhpVal {
	if rt.is_true(rt.get_property(rt.new_object('WP_REST_Menu_Locations_Controller', [
		'WP_REST_Controller',
	], &this), 'schema'))
	{
		return this.add_additional_fields_schema(rt.get_property(rt.new_object('WP_REST_Menu_Locations_Controller', [
			'WP_REST_Controller',
		], &this), 'schema'))
	}
	this.dispatch_set_prop('schema', rt.create_array([
		rt.ArrayItem{ key: '$schema', val: 'http://json-schema.org/draft-04/schema#' },
		rt.ArrayItem{ key: 'title', val: 'menu-location' },
		rt.ArrayItem{ key: 'type', val: 'object' },
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			rt.ArrayItem{ key: 'name', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('The name of the menu location.'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'string' },
			...
			rt.ArrayItem{ key: 'description', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('The description of the menu location.'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'embed' },
			...
			rt.ArrayItem{ key: 'menu', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('The ID of the assigned menu.'),
				]) },
