...
fn (mut this Class_WP_REST_Global_Styles_Controller) register_routes() {
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', [
			'WP_REST_Posts_Controller',
		], &this), 'namespace'),
		rt.new_string('/' +
			(rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', ['WP_REST_Posts_Controller'], &this), 'rest_base')).str() +
			'/themes/(?P<stylesheet>[\\/\\s%\\w\\.\\(\\)\\[\\]\\@_\\-]+)/variations'),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				rt.ArrayItem{ key: 'methods', val: Class_WP_REST_Server.readable() },
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_REST_Global_Styles_Controller', [
						'WP_REST_Posts_Controller',
					], &this) },
					rt.ArrayItem{ key: none, val: 'get_theme_items' },
				]) },
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_REST_Global_Styles_Controller', [
						'WP_REST_Posts_Controller',
					], &this) },
					rt.ArrayItem{ key: none, val: 'get_theme_items_permissions_check' },
				]) },
				rt.ArrayItem{ key: 'args', val: rt.create_array([
					rt.ArrayItem{ key: 'stylesheet', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('The theme identifier'),
						]) },
	...
	rt.call_function('register_rest_route', [
		...
		rt.call_function('sprintf', [
			rt.new_string('/%s/themes/(?P<stylesheet>%s)'),
			rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', [
				'WP_REST_Posts_Controller',
			], &this), 'rest_base'),
			rt.new_string('[^\\/:<>\\*\\?"\\|]+(?:\\/[^\\/:<>\\*\\?"\\|]+)?'),
		...
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					...
					rt.ArrayItem{ key: none, val: 'get_theme_item' },
				]) },
				rt.ArrayItem{ key: 'permission_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_REST_Global_Styles_Controller', [
						'WP_REST_Posts_Controller',
					], &this) },
					rt.ArrayItem{ key: none, val: 'get_theme_item_permissions_check' },
				]) },
				rt.ArrayItem{ key: 'args', val: rt.create_array([
					rt.ArrayItem{ key: 'stylesheet', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
						...
						rt.ArrayItem{ key: 'sanitize_callback', val: rt.create_array([
							...
							rt.ArrayItem{ key: none, val: '_sanitize_global_styles_callback' },
						]) },
					]) },
				]) },
				rt.ArrayItem{ key: 'allow_batch', val: this.allow_batch },
			...
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'args', val: rt.create_array([
					rt.ArrayItem{ key: 'id', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('ID of global styles config.'),
						]) },
...
fn (mut this Class_WP_REST_Global_Styles_Controller) get_post(var_id rt.PhpVal) rt.PhpVal {
	...
	mut var_error := create_wp_error(rt.new_string('rest_global_styles_not_found'), rt.call_function('__', [
		rt.new_string('No global styles config exists with that ID.'),
	...
	mut var_post := rt.call_function('get_post', [var_id_mutated.clone()])
	if !rt.is_true(var_post) || !rt.is_true(rt.get_property(var_post, 'ID'))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', ['WP_REST_Posts_Controller'], &this), 'post_type'), rt.get_property(var_post, 'post_type'))))) {
		return mut var_error
	}
...
fn (mut this Class_WP_REST_Global_Styles_Controller) get_item_permissions_check(var_request rt.PhpVal) bool {
	...
	if rt.is_true(rt.identical(rt.new_string('edit'), var_request.array_get(rt.new_string('context'))))
		&& rt.is_true(var_post)
		&& rt.is_true(rt.new_bool(!(rt.is_true(this.check_update_permission(var_post.clone()))))) {
		return (create_wp_error(rt.new_string('rest_forbidden_context'), rt.call_function('__', [
			rt.new_string('Sorry, you are not allowed to edit this global style.'),
		]), rt.create_array([
			rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code',
				[]rt.PhpVal{}) },
		]))).to_bool()
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(this.check_read_permission(var_post.clone()))))) {
		return (create_wp_error(rt.new_string('rest_cannot_view'), rt.call_function('__', [
			rt.new_string('Sorry, you are not allowed to view this global style.'),
		]), rt.create_array([
			rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code',
				[]rt.PhpVal{}) },
		]))).to_bool()
...
fn (mut this Class_WP_REST_Global_Styles_Controller) check_read_permission(var_post rt.PhpVal) rt.PhpVal {
	...
	return rt.call_function('current_user_can', [rt.new_string('read_post'),
...
fn (mut this Class_WP_REST_Global_Styles_Controller) prepare_item_for_database(var_request rt.PhpVal) rt.PhpVal {
	...
	rt.set_property(var_changes, 'ID', var_request.array_get(rt.new_string('id')))
	mut var_post := rt.call_function('get_post', [var_request.array_get(rt.new_string('id'))])
	mut var_existing_config := rt.new_array()
	if rt.is_true(var_post) {
		var_existing_config = rt.call_function('json_decode', [
			rt.get_property(var_post, 'post_content'),
			rt.new_bool(true),
		])
		mut var_json_decoding_error := rt.call_function('json_last_error', []rt.PhpVal{})
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_constant('JSON_ERROR_NONE'), var_json_decoding_error))))
			|| !(var_existing_config.array_isset(rt.new_string('isGlobalStylesUserThemeJSON')))
			|| rt.is_true(rt.new_bool(!(rt.is_true(var_existing_config.array_get(rt.new_string('isGlobalStylesUserThemeJSON')))))) {
			var_existing_config = rt.new_array()
		}
	}
	if var_request.array_isset(rt.new_string('styles'))
		|| var_request.array_isset(rt.new_string('settings')) {
		mut var_config := rt.new_array()
		if var_request.array_isset(rt.new_string('styles')) {
			if var_request.array_get(rt.new_string('styles')).array_isset(rt.new_string('css')) {
				mut var_css_validation_result :=
					rt.new_bool(this.validate_custom_css(var_request.array_get(rt.new_string('styles')).array_get(rt.new_string('css'))))
				if rt.is_true(rt.call_function('is_wp_error', [
					var_css_validation_result.clone()]))
				{
					return mut rt.cast_object_ptr[Class_stdClass](var_css_validation_result)
				}
			}
			var_config.array_set('styles', var_request.array_get(rt.new_string('styles')))
		} else if var_existing_config.array_isset(rt.new_string('styles')) {
			var_config.array_set('styles', var_existing_config.array_get(rt.new_string('styles')))
		}
		...
		if var_request.array_isset(rt.new_string('settings')) {
			var_config.array_set('settings', var_request.array_get(rt.new_string('settings')))
		} else if var_existing_config.array_isset(rt.new_string('settings')) {
			var_config.array_set('settings',
				var_existing_config.array_get(rt.new_string('settings')))
		}
		var_config.array_set('isGlobalStylesUserThemeJSON', true)
		var_config.array_set('version', Class_WP_Theme_JSON.latest_schema())
		rt.set_property(var_changes, 'post_content', rt.call_function('wp_json_encode', [
			var_config.clone(),
...
fn (mut this Class_WP_REST_Global_Styles_Controller) prepare_item_for_response(var_post rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_post_mutated := var_post
	mut var_raw_config := rt.call_function('json_decode', [
		rt.get_property(var_post_mutated, 'post_content'),
		rt.new_bool(true),
	])
	mut var_is_global_styles_user_theme_json := rt.new_bool(
		var_raw_config.array_isset(rt.new_string('isGlobalStylesUserThemeJSON'))
		&& rt.is_true(rt.identical(rt.new_bool(true), var_raw_config.array_get(rt.new_string('isGlobalStylesUserThemeJSON')))))
	mut var_config := rt.new_array()
	mut var_theme_json := rt.new_null()
	if rt.is_true(var_is_global_styles_user_theme_json) {
		var_theme_json = create_wp_theme_json(var_raw_config.clone(), rt.new_string('custom'))
	...
	{
		var_data.array_get_mut('title').array_set('raw', rt.get_property(var_post_mutated,
			'post_title'))
	}
	if rt.is_true(rt.call_function('rest_is_field_included', [
	...
	if rt.is_true(rt.call_function('rest_is_field_included', [
		rt.new_string('settings'), var_fields.clone()]))
	{
		var_data.array_set('settings', if
			!(!rt.is_true(var_config.array_get(rt.new_string('settings'))))
			&& rt.is_true(var_is_global_styles_user_theme_json) {
			var_config.array_get(rt.new_string('settings'))
		} else {
			create_stdclass()
		})
	}
	...
	{
		var_data.array_set('styles', if
			!(!rt.is_true(var_config.array_get(rt.new_string('styles'))))
			&& rt.is_true(var_is_global_styles_user_theme_json) {
			var_config.array_get(rt.new_string('styles'))
		} else {
			create_stdclass()
		})
	}
	...
	if rt.is_true(rt.call_function('rest_is_field_included', [rt.new_string('_links'), var_fields.clone()]))
		...
		if rt.is_true(var_theme_json) {
			...
			if !(!rt.is_true(var_resolved_theme_uris)) {
				var_links.array_set('https://api.w.org/theme-file', var_resolved_theme_uris.clone())
			}
		}
		...
		if !(!rt.is_true(var_links.array_get(rt.new_string('self')).array_get(rt.new_string('href')))) {
			mut var_actions := this.get_available_actions(var_post_mutated.clone(),
				var_request.clone())
			mut var_self :=
...
fn (mut this Class_WP_REST_Global_Styles_Controller) prepare_links(var_id rt.PhpVal) rt.PhpVal {
	mut var_id_mutated := var_id
	mut var_base := rt.call_function('sprintf', [rt.new_string('%s/%s'),
		rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', [
			'WP_REST_Posts_Controller',
		], &this), 'namespace'),
		rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', [
	...
	mut var_links := rt.create_array([
		...
		rt.ArrayItem{ key: 'about', val: rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				rt.new_string('wp/v2/types/' +(rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', ['WP_REST_Posts_Controller'], &this), 'post_type')).str()),
			]) },
		]) },
	])
...
fn (mut this Class_WP_REST_Global_Styles_Controller) get_item_schema() rt.PhpVal {
	if rt.is_true(rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', [
		'WP_REST_Posts_Controller',
	], &this), 'schema'))
	{
		return this.add_additional_fields_schema(rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', [
			'WP_REST_Posts_Controller',
		], &this), 'schema'))
	}
	mut var_schema := {
		'$schema':    rt.new_string('http://json-schema.org/draft-04/schema#')
		'title':      rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', [
			'WP_REST_Posts_Controller',
		], &this), 'post_type')
		...
		'properties': {
			...
			'styles':   {
				'description': rt.call_function('__', [rt.new_string('Global styles.')])
				'type':        map[string]rt.PhpVal{}
				'context':     map[string]rt.PhpVal{}
			}
			'settings': {
				'description': rt.call_function('__', [rt.new_string('Global settings.')])
				'type':        map[string]rt.PhpVal{}
				'context':     map[string]rt.PhpVal{}
			}
			'title':    {
				'description': rt.call_function('__', [
					rt.new_string('Title of the global styles variation.'),
				])
				'type':        map[string]rt.PhpVal{}
				...
				'properties':  {
					'raw':      {
						'description': rt.call_function('__', [
							rt.new_string('Title for the global styles variation, as it exists in the database.'),
						])
					...
					'rendered': {
						'description': rt.call_function('__', [
							rt.new_string('HTML title for the post, transformed for display.'),
						])
						'type':        rt.new_string('string')
						'context':     map[string]rt.PhpVal{}
						'readonly':    rt.new_bool(true)
...
fn (mut this Class_WP_REST_Global_Styles_Controller) get_theme_item_permissions_check(var_request rt.PhpVal) bool {
	...
	return (create_wp_error(rt.new_string('rest_cannot_read_global_styles'), rt.call_function('__', [
		rt.new_string('Sorry, you are not allowed to access the global styles on this site.'),
	]), rt.create_array([
		rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code',
			[]rt.PhpVal{}) },
	]))).to_bool()
...
fn (mut this Class_WP_REST_Global_Styles_Controller) get_theme_item(var_request rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('get_stylesheet',
		[]rt.PhpVal{}), var_request.array_get(rt.new_string('stylesheet'))))))
	{
		return create_wp_error(rt.new_string('rest_theme_not_found'), rt.call_function('__', [
			rt.new_string('Theme not found.'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 404 }]))
	}
	mut iife_temp_2 := Class_WP_Theme_JSON_Resolver{}
	mut iife_result_2 := iife_temp_2.get_merged_data(rt.new_string('theme'))
	mut var_theme := iife_result_2
	mut var_fields := this.get_fields_for_response(var_request.clone())
	mut var_data := rt.new_array()
	if rt.is_true(rt.call_function('rest_is_field_included', [
		rt.new_string('settings'), var_fields.clone()]))
	{
		var_data.array_set('settings', rt.call_method(var_theme, 'get_settings', []rt.PhpVal{}))
	}
	if rt.is_true(rt.call_function('rest_is_field_included', [
		rt.new_string('styles'), var_fields.clone()]))
	{
		mut var_raw_data := rt.call_method(var_theme, 'get_raw_data', []rt.PhpVal{})
		var_data.array_set('styles', if !(var_raw_data.array_get(rt.new_string('styles'))).is_null() {
			var_raw_data.array_get(rt.new_string('styles'))
		} else {
			rt.new_array()
		})
	}
	...
	if rt.is_true(rt.call_function('rest_is_field_included', [rt.new_string('_links'), var_fields.clone()]))
		...
		mut var_links := rt.create_array([
			rt.ArrayItem{ key: 'self', val: rt.create_array([
				rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
					rt.call_function('sprintf', [rt.new_string('%s/%s/themes/%s'),
						rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', [
							'WP_REST_Posts_Controller',
						], &this), 'namespace'),
						rt.get_property(rt.new_object('WP_REST_Global_Styles_Controller', [
		...
		mut iife_result_3 := iife_temp_3.get_resolved_theme_uris(var_theme.clone())
		mut var_resolved_theme_uris := iife_result_3
		if !(!rt.is_true(var_resolved_theme_uris)) {
			var_links.array_set('https://api.w.org/theme-file', var_resolved_theme_uris.clone())
		}
		rt.call_method(var_response, 'add_links', [var_links.clone()])
...
fn (mut this Class_WP_REST_Global_Styles_Controller) get_theme_items_permissions_check(var_request rt.PhpVal) rt.PhpVal {
	return rt.new_bool(this.get_theme_item_permissions_check(var_request.clone()))
}
...
fn (mut this Class_WP_REST_Global_Styles_Controller) get_theme_items(var_request rt.PhpVal) rt.PhpVal {
	...
	mut iife_result_4 := iife_temp_4.get_style_variations(rt.new_string('block'))
	mut var_partials := iife_result_4
	rt.call_function('wp_register_block_style_variations_from_theme_json_partials', [
		var_partials.clone(),
	])
	mut iife_temp_5 := Class_WP_Theme_JSON_Resolver{}
	mut iife_result_5 := iife_temp_5.get_style_variations()
	mut var_variations := iife_result_5
	mut iter_3 := var_variations.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_variation := item_3.val
		mut var_variation_theme_json := create_wp_theme_json(var_variation.clone())
		mut iife_temp_6 := Class_WP_Theme_JSON_Resolver{}
		mut iife_result_6 := iife_temp_6.get_resolved_theme_uris(rt.new_object('WP_Theme_JSON',
			[]string{}, var_variation_theme_json))
		mut var_resolved_theme_uris := iife_result_6
		mut var_data := rt.call_function('rest_ensure_response', [
			var_variation.clone()])
		if !(!rt.is_true(var_resolved_theme_uris)) {
			rt.call_method(var_data, 'add_links', [
				rt.create_array([
	...
	for {
		...
		if rt.is_true(var_possible_style_close_tag) {
			if rt.is_true(rt.less(var_remaining_strlen, rt.new_int(8))) {
				return (create_wp_error(rt.new_string('rest_custom_css_illegal_markup'), rt.call_function('sprintf', [
					rt.call_function('__', [
