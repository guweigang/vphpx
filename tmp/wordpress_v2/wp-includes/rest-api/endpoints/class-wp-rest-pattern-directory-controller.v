...
fn (mut this Class_WP_REST_Pattern_Directory_Controller) get_items(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_query_args := rt.call_function('array_intersect_key', [
		...
		rt.create_array_from_native_map(var_valid_query_args),
	])
	...
	var_query_args.array_set('pattern-categories', if !(var_request.array_get(rt.new_string('category'))).is_null() {
		var_request.array_get(rt.new_string('category'))
	} else {
		rt.new_bool(false)
	})
	var_query_args.array_set('pattern-keywords', if !(var_request.array_get(rt.new_string('keyword'))).is_null() {
		var_request.array_get(rt.new_string('keyword'))
	} else {
		rt.new_bool(false)
	})
	var_query_args = rt.call_function('array_filter', [var_query_args.clone()])
	mut var_transient_key := rt.new_string(this.get_transient_key(var_query_args.clone()))
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_raw_patterns)))) {
		mut var_api_url := rt.new_string('http://api.wordpress.org/patterns/1.0/?' +
			(rt.call_function('build_query', [var_query_args.clone()])).str())
		...
		]))
		{
			var_api_url = rt.call_function('set_url_scheme', [
				var_api_url.clone(), rt.new_string('https')])
		}
		mut var_cache_ttl := rt.new_int(5)
		mut var_wporg_response := rt.call_function('wp_remote_get', [
			var_api_url.clone()])
		var_raw_patterns = rt.call_function('json_decode', [
			rt.call_function('wp_remote_retrieve_body', [var_wporg_response.clone()]),
		])
		if rt.is_true(rt.call_function('is_wp_error', [var_wporg_response.clone()])) {
			var_raw_patterns = var_wporg_response.clone()
		} else if !(var_raw_patterns.clone().is_array()) {
		...
		rt.call_function('set_site_transient', [var_transient_key.clone(),
			var_raw_patterns.clone(), var_cache_ttl.clone()])
	}
	if rt.is_true(rt.call_function('is_wp_error', [var_raw_patterns.clone()])) {
		rt.call_method(var_raw_patterns, 'add_data', [
			rt.create_array([rt.ArrayItem{ key: 'status', val: 500 }]),
		])
		return var_raw_patterns.clone()
	}
	...
	if rt.is_true(var_raw_patterns) {
		mut iter_2 := var_raw_patterns.iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_pattern := item_2.val
			var_response.array_push(this.prepare_response_for_collection(this.prepare_item_for_response(var_pattern.clone(),
				var_request.clone())))
		}
	}
	return rt.new_object('WP_REST_Response', []string{},
...
fn (mut this Class_WP_REST_Pattern_Directory_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	var_prepared_pattern = this.add_additional_fields_to_object(var_prepared_pattern.clone(),
		var_request.clone())
	mut var_response := create_wp_rest_response(var_prepared_pattern.clone())
	return rt.call_function('apply_filters', [
		rt.new_string('rest_prepare_block_pattern'),
		var_response.clone(),
		var_raw_pattern.clone(),
		var_request.clone(),
	])
}
...
fn (mut this Class_WP_REST_Pattern_Directory_Controller) get_transient_key(var_query_args rt.PhpVal) string {
	...
	if var_query_args_mutated.array_isset(rt.new_string('slug')) {
		var_query_args_mutated.array_set('slug', rt.call_function('wp_parse_list', [
			var_query_args_mutated.array_get(rt.new_string('slug')),
		]))
		if !rt.is_true(var_query_args_mutated.array_get(rt.new_string('slug'))) {
			var_query_args_mutated.array_unset(rt.new_string('slug'))
		} else {
			rt.call_function('sort', [var_query_args_mutated.array_get(rt.new_string('slug'))])
		}
	}
	return 'wp_remote_block_patterns_' +
		md5.hexhash(rt.call_function('serialize', [var_query_args_mutated.clone()]).to_string())
}
