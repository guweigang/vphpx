...
fn (mut this Class_WP_REST_Font_Families_Controller) get_items_permissions_check(var_request rt.PhpVal) bool {
	mut var_post_type := rt.call_function('get_post_type_object', [
		rt.get_property(rt.new_object('WP_REST_Font_Families_Controller', [
	...
	])))))
	{
		return (create_wp_error(rt.new_string('rest_cannot_read'), rt.call_function('__', [
			rt.new_string('Sorry, you are not allowed to access font families.'),
		]), rt.create_array([
...
fn (mut this Class_WP_REST_Font_Families_Controller) get_item_permissions_check(var_request rt.PhpVal) bool {
	...
	{
		return (create_wp_error(rt.new_string('rest_cannot_read'), rt.call_function('__', [
			rt.new_string('Sorry, you are not allowed to access this font family.'),
		]), rt.create_array([
			rt.ArrayItem{ key: 'status', val: rt.call_function('rest_authorization_required_code',
				[]rt.PhpVal{}) },
		]))).to_bool()
...
fn (mut this Class_WP_REST_Font_Families_Controller) validate_font_family_settings(var_value rt.PhpVal, var_request rt.PhpVal) bool {
	mut var_args := this.get_endpoint_args_for_item_schema(rt.call_method(var_request,
		'get_method', []rt.PhpVal{}))
	mut var_validity := rt.call_function('rest_validate_value_from_schema', [
		var_value.clone(), var_args.array_get(rt.new_string('font_family_settings')),
		rt.new_string('font_family_settings')])
	if rt.is_true(rt.call_function('is_wp_error', [var_validity.clone()])) {
		return var_validity.to_bool()
	}
	mut var_settings := rt.call_function('json_decode', [var_value.clone(),
		rt.new_bool(true)])
	if rt.is_true(rt.identical(rt.new_null(), var_settings)) {
		return (create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('%s parameter must be a valid JSON string.'),
			]),
			rt.new_string('font_family_settings'),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))).to_bool()
	}
	mut var_schema :=
		this.get_item_schema().array_get(rt.new_string('properties')).array_get(rt.new_string('font_family_settings'))
	...
	if var_request.array_isset(rt.new_string('id')) {
		...
		if var_settings.array_isset(rt.new_string('slug')) {
			return (create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('sprintf', [
				rt.call_function('__', [rt.new_string('%s cannot be updated.')]),
				rt.new_string('font_family_settings[slug]'),
			]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))).to_bool()
		}
	}
	...
	for {
		...
		if var_settings.array_isset(var_key)
			...
			return (create_wp_error(rt.new_string('rest_invalid_param'), rt.call_function('sprintf', [
				...
				rt.new_string('font_family_settings[ ${var_key.to_string()} ]'),
			]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }]))).to_bool()
		}
	}
	return true
...
fn (mut this Class_WP_REST_Font_Families_Controller) sanitize_font_family_settings(var_value rt.PhpVal) rt.PhpVal {
	mut var_settings := rt.call_function('json_decode', [var_value.clone(),
		rt.new_bool(true)])
	mut var_schema :=
		this.get_item_schema().array_get(rt.new_string('properties')).array_get(rt.new_string('font_family_settings')).array_get(rt.new_string('properties'))
...
fn (mut this Class_WP_REST_Font_Families_Controller) create_item(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_query := create_wp_query(rt.create_array([
		rt.ArrayItem{ key: 'post_type', val: rt.get_property(rt.new_object('WP_REST_Font_Families_Controller', [
			'WP_REST_Posts_Controller',
		], &this), 'post_type') },
		rt.ArrayItem{ key: 'posts_per_page', val: 1 },
		rt.ArrayItem{ key: 'name', val: var_settings.array_get(rt.new_string('slug')) },
	...
	if !(!rt.is_true(rt.get_property(var_query, 'posts'))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('rest_duplicate_font_family'), rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('A font family with slug "%s" already exists.'),
			]),
			var_settings.array_get(rt.new_string('slug')),
		]), rt.create_array([rt.ArrayItem{ key: 'status', val: 400 }])))
	}
	return this.Class_WP_REST_Posts_Controller.create_item(var_request.clone())
}
...
fn (mut this Class_WP_REST_Font_Families_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	...
	{
		var_data.array_set('font_family_settings', this.get_settings_from_post(var_item.clone()))
	}
	...
	return rt.call_function('apply_filters', [
		rt.new_string('rest_prepare_wp_font_family'),
		var_response.clone(),
		var_item.clone(),
		var_request.clone(),
	])
...
fn (mut this Class_WP_REST_Font_Families_Controller) get_item_schema() rt.PhpVal {
	if rt.is_true(rt.get_property(rt.new_object('WP_REST_Font_Families_Controller', [
		'WP_REST_Posts_Controller',
	], &this), 'schema'))
	{
		return this.add_additional_fields_schema(rt.get_property(rt.new_object('WP_REST_Font_Families_Controller', [
			'WP_REST_Posts_Controller',
		], &this), 'schema'))
	}
	mut var_schema := rt.create_array([
		rt.ArrayItem{ key: '$schema', val: 'http://json-schema.org/draft-04/schema#' },
		rt.ArrayItem{ key: 'title', val: rt.get_property(rt.new_object('WP_REST_Font_Families_Controller', [
			'WP_REST_Posts_Controller',
		], &this), 'post_type') },
		...
		rt.ArrayItem{ key: 'properties', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'font_faces', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('The IDs of the child font faces in the font family.'),
				]) },
			...
			rt.ArrayItem{ key: 'font_family_settings', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('font-face definition in theme.json format.'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'object' },
				rt.ArrayItem{ key: 'context', val: rt.create_array([
					rt.ArrayItem{ key: none, val: 'view' },
				...
				rt.ArrayItem{ key: 'properties', val: rt.create_array([
					rt.ArrayItem{ key: 'name', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('Name of the font family preset, translatable.'),
						]) },
					...
					rt.ArrayItem{ key: 'slug', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('Kebab-case unique identifier for the font family preset.'),
						]) },
						rt.ArrayItem{ key: 'type', val: 'string' },
						rt.ArrayItem{ key: 'arg_options', val: rt.create_array([
							rt.ArrayItem{ key: 'sanitize_callback', val: 'sanitize_title' },
						]) },
					]) },
					...
					rt.ArrayItem{ key: 'preview', val: rt.create_array([
						rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
							rt.new_string('URL to a preview image of the font family.'),
						]) },
						rt.ArrayItem{ key: 'type', val: 'string' },
...
fn (mut this Class_WP_REST_Font_Families_Controller) get_public_item_schema() rt.PhpVal {
	...
	mut iter_3 :=
		var_schema.array_get(rt.new_string('properties')).array_get(rt.new_string('font_family_settings')).array_get(rt.new_string('properties')).iterator()
	for {
...
fn (mut this Class_WP_REST_Font_Families_Controller) get_collection_params() rt.PhpVal {
	...
	return rt.call_function('apply_filters', [
		rt.new_string('rest_wp_font_family_collection_params'),
...
fn (mut this Class_WP_REST_Font_Families_Controller) get_endpoint_args_for_item_schema(var_method rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.identical(Class_WP_REST_Server.creatable(), var_method))
		|| rt.is_true(rt.identical(Class_WP_REST_Server.editable(), var_method)) {
		...
		return rt.create_array([
			...
			rt.ArrayItem{ key: 'font_family_settings', val: rt.create_array([
				rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
					rt.new_string('font-family declaration in theme.json format, encoded as a string.'),
				]) },
				rt.ArrayItem{ key: 'type', val: 'string' },
				rt.ArrayItem{ key: 'required', val: true },
				rt.ArrayItem{ key: 'validate_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_REST_Font_Families_Controller', [
						'WP_REST_Posts_Controller',
					], &this) },
					rt.ArrayItem{ key: none, val: 'validate_font_family_settings' },
				]) },
				rt.ArrayItem{ key: 'sanitize_callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_REST_Font_Families_Controller', [
						'WP_REST_Posts_Controller',
					], &this) },
					rt.ArrayItem{ key: none, val: 'sanitize_font_family_settings' },
				]) },
			]) },
		])
	}
	return this.Class_WP_REST_Posts_Controller.get_endpoint_args_for_item_schema(var_method.clone())
}
...
fn (mut this Class_WP_REST_Font_Families_Controller) get_font_face_ids(var_font_family_id rt.PhpVal) rt.PhpVal {
	mut var_query := create_wp_query(rt.create_array([
		...
		rt.ArrayItem{ key: 'post_parent', val: var_font_family_id },
		rt.ArrayItem{ key: 'post_type', val: 'wp_font_face' },
		rt.ArrayItem{ key: 'posts_per_page', val: 99 },
...
fn (mut this Class_WP_REST_Font_Families_Controller) prepare_links(var_post rt.PhpVal) rt.PhpVal {
	...
	return rt.create_array([
		rt.ArrayItem{ key: 'self', val: var_links.array_get(rt.new_string('self')) },
		rt.ArrayItem{ key: 'collection', val: var_links.array_get(rt.new_string('collection')) },
		rt.ArrayItem{ key: 'font_faces', val: this.prepare_font_face_links(rt.get_property(var_post_mutated,
			'ID')) },
	])
}

fn (mut this Class_WP_REST_Font_Families_Controller) prepare_font_face_links(var_font_family_id rt.PhpVal) rt.PhpVal {
	mut var_font_face_ids := this.get_font_face_ids(var_font_family_id.clone())
	mut var_links := rt.new_array()
	mut iter_4 := var_font_face_ids.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_font_face_id := item_4.val
		var_links.array_push(rt.create_array([
			...
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('%s/%s/%s/font-faces/%s'),
					rt.get_property(rt.new_object('WP_REST_Font_Families_Controller', [
						'WP_REST_Posts_Controller',
					], &this), 'namespace'),
					rt.get_property(rt.new_object('WP_REST_Font_Families_Controller', [
					...
					var_font_family_id.clone(), var_font_face_id.clone()]),
			]) },
		]))
	}
...
fn (mut this Class_WP_REST_Font_Families_Controller) prepare_item_for_database(var_request rt.PhpVal) rt.PhpVal {
	...
	if var_request.array_isset(rt.new_string('id')) {
		...
		mut var_existing_settings := this.get_settings_from_post(var_existing_post.clone())
		var_settings = rt.call_function('array_merge', [var_existing_settings.clone(),
			var_settings.clone()])
	}
	rt.set_property(var_prepared_post, 'post_type', rt.get_property(rt.new_object('WP_REST_Font_Families_Controller', [
		'WP_REST_Posts_Controller',
	], &this), 'post_type'))
	rt.set_property(var_prepared_post, 'post_status', rt.new_string('publish'))
	rt.set_property(var_prepared_post, 'post_title', var_settings.array_get(rt.new_string('name')))
...
fn (mut this Class_WP_REST_Font_Families_Controller) get_settings_from_post(var_post rt.PhpVal) rt.PhpVal {
	mut var_post_mutated := var_post
	mut var_settings_json := rt.call_function('json_decode', [
	...
	return rt.create_array([
		rt.ArrayItem{
			key: 'name'
			val: if !(rt.get_property(var_post_mutated, 'post_title')).is_null()
				&& rt.is_true(rt.get_property(var_post_mutated, 'post_title')) {
		...
		rt.ArrayItem{
			key: 'slug'
			val: if !(rt.get_property(var_post_mutated, 'post_name')).is_null()
				&& rt.is_true(rt.get_property(var_post_mutated, 'post_name')) {
				rt.get_property(var_post_mutated, 'post_name')
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{
			key: 'fontFamily'
			val: if var_settings_json.array_isset(rt.new_string('fontFamily'))
				&& rt.is_true(var_settings_json.array_get(rt.new_string('fontFamily'))) {
				var_settings_json.array_get(rt.new_string('fontFamily'))
			} else {
				rt.new_string('')
			}
		},
		rt.ArrayItem{
			key: 'preview'
			val: if var_settings_json.array_isset(rt.new_string('preview'))
				&& rt.is_true(var_settings_json.array_get(rt.new_string('preview'))) {
				var_settings_json.array_get(rt.new_string('preview'))
			} else {
				rt.new_string('')
			}
		},
...
fn create_wp_rest_font_families_controller(_args ...rt.PhpVal) &Class_WP_REST_Font_Families_Controller {
