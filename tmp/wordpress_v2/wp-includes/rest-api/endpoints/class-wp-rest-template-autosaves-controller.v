...
fn (mut this Class_WP_REST_Template_Autosaves_Controller) construct(var_parent_post_type rt.PhpVal) {
	this.Class_WP_REST_Autosaves_Controller.construct(var_parent_post_type.clone())
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_parent_controller)))) {
		var_parent_controller = create_wp_rest_templates_controller(var_parent_post_type.clone())
	}
...
fn (mut this Class_WP_REST_Template_Autosaves_Controller) register_routes() {
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('WP_REST_Template_Autosaves_Controller', [
			'WP_REST_Autosaves_Controller',
		], &this), 'namespace'),
		rt.call_function('sprintf', [
			rt.new_string('/%s/(?P<id>%s%s)/%s'),
			this.parent_base,
			rt.new_string('([^\\/:<>\\*\\?"\\|]+(?:\\/[^\\/:<>\\*\\?"\\|]+)?)'),
			rt.new_string('[\\/\\w%-]+'),
			rt.get_property(rt.new_object('WP_REST_Template_Autosaves_Controller', [
		...
		rt.create_array([
			rt.ArrayItem{ key: 'args', val: rt.create_array([
				rt.ArrayItem{ key: 'id', val: rt.create_array([
					...
					rt.ArrayItem{ key: 'sanitize_callback', val: rt.create_array([
						rt.ArrayItem{ key: none, val: this.parent_controller },
			...
			rt.ArrayItem{ key: none, val: rt.create_array([
				...
				rt.ArrayItem{ key: 'callback', val: rt.create_array([
					rt.ArrayItem{ key: none, val: rt.new_object('WP_REST_Template_Autosaves_Controller', [
						'WP_REST_Autosaves_Controller',
					], &this) },
	...
	rt.call_function('register_rest_route', [
		...
		rt.call_function('sprintf', [
			rt.new_string('/%s/(?P<parent>%s%s)/%s/%s'),
			this.parent_base,
			rt.new_string('([^\\/:<>\\*\\?"\\|]+(?:\\/[^\\/:<>\\*\\?"\\|]+)?)'),
			rt.new_string('[\\/\\w%-]+'),
			rt.get_property(rt.new_object('WP_REST_Template_Autosaves_Controller', [
...
fn (mut this Class_WP_REST_Template_Autosaves_Controller) prepare_item_for_response(var_item rt.PhpVal, var_request rt.PhpVal) rt.PhpVal {
	mut var_template := rt.call_function('_build_block_template_result_from_post', [
		var_item.clone(),
	])
	mut var_response := rt.call_method(this.parent_controller, 'prepare_item_for_response', [
		var_template.clone(),
		var_request.clone(),
	])
		...
		var_fields.clone(), rt.new_bool(true)]))
	{
		var_data.array_set('parent',
			rt.new_int((rt.get_property(var_item, 'post_parent')).to_i64()))
	}
	...
	var_response = create_wp_rest_response(var_data.clone())
	if rt.is_true(rt.call_function('rest_is_field_included', [rt.new_string('_links'), var_fields.clone()]))
		...
		mut var_links := this.prepare_links(var_template.clone())
...
fn (mut this Class_WP_REST_Template_Autosaves_Controller) get_item(var_request rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_autosave)))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('rest_post_no_autosave'), rt.call_function('__', [
			rt.new_string('There is no autosave revision for this template.'),
	...
	mut var_links := rt.create_array([
		rt.ArrayItem{ key: 'self', val: rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
				rt.call_function('sprintf', [rt.new_string('/%s/%s/%s/%s/%d'),
					rt.get_property(rt.new_object('WP_REST_Template_Autosaves_Controller', [
						'WP_REST_Autosaves_Controller',
					], &this), 'namespace'),
					...
					rt.get_property(var_template_mutated, 'wp_id')]),
			]) },
		]) },
		rt.ArrayItem{ key: 'parent', val: rt.create_array([
			rt.ArrayItem{ key: 'href', val: rt.call_function('rest_url', [
...
fn (mut this Class_WP_REST_Template_Autosaves_Controller) get_item_schema() rt.PhpVal {
	if rt.is_true(rt.get_property(rt.new_object('WP_REST_Template_Autosaves_Controller', [
		'WP_REST_Autosaves_Controller',
	], &this), 'schema'))
	{
		return this.add_additional_fields_schema(rt.get_property(rt.new_object('WP_REST_Template_Autosaves_Controller', [
			'WP_REST_Autosaves_Controller',
		], &this), 'schema'))
	}
	this.dispatch_set_prop('schema', rt.call_method(this.revisions_controller, 'get_item_schema',
		[]rt.PhpVal{}))
	return this.add_additional_fields_schema(rt.get_property(rt.new_object('WP_REST_Template_Autosaves_Controller', [
		'WP_REST_Autosaves_Controller',
	], &this), 'schema'))
...
fn create_wp_rest_autosaves_controller(_args ...rt.PhpVal) &Class_WP_REST_Autosaves_Controller {
...
fn create_wp_rest_templates_controller(_args ...rt.PhpVal) &Class_WP_REST_Templates_Controller {
