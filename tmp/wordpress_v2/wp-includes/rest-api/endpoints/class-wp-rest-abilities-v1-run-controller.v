...
fn (mut this Class_WP_REST_Abilities_V1_Run_Controller) register_routes() {
	rt.call_function('register_rest_route', [this.namespace,
		rt.new_string('/' + (this.rest_base).str() + '/(?P<name>[a-zA-Z0-9\\-\\/]+?)/run'),
		rt.create_array([
...
fn (mut this Class_WP_REST_Abilities_V1_Run_Controller) execute_ability(var_request rt.PhpVal) rt.PhpVal {
	...
	mut var_input := this.get_input_from_request(var_request.clone())
...
fn (mut this Class_WP_REST_Abilities_V1_Run_Controller) validate_request_method(request_method string, mut var_annotations Class_array) bool {
	mut var_expected_method := rt.new_string('POST')
	if !(!rt.is_true(var_annotations.array_get(rt.new_string('readonly')))) {
		var_expected_method = rt.new_string('GET')
	} else if !(!rt.is_true(var_annotations.array_get(rt.new_string('destructive'))))
		&& !(!rt.is_true(var_annotations.array_get(rt.new_string('idempotent')))) {
		var_expected_method = rt.new_string('DELETE')
	}
	...
	return (create_wp_error(rt.new_string('rest_ability_invalid_method'),
...
fn (mut this Class_WP_REST_Abilities_V1_Run_Controller) get_input_from_request(var_request rt.PhpVal) rt.PhpVal {
	...
	]))
	{
		...
		return if !(var_query_params.array_get(rt.new_string('input'))).is_null() {
			var_query_params.array_get(rt.new_string('input'))
		} else {
			rt.new_null()
		}
	}
	mut var_json_params := rt.call_method(var_request, 'get_json_params', []rt.PhpVal{})
	return if !(var_json_params.array_get(rt.new_string('input'))).is_null() {
		var_json_params.array_get(rt.new_string('input'))
	} else {
		rt.new_null()
	}
}
