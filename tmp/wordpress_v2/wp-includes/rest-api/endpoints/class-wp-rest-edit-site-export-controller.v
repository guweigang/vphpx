...
fn (mut this Class_WP_REST_Edit_Site_Export_Controller) export() rt.PhpVal {
	...
	rt.call_function('header', [
		rt.new_string('Content-Disposition: attachment; filename=' + var_theme_name.str() + '.zip'),
	])
	rt.call_function('header', [
		rt.new_string('Content-Length: ' +
			(rt.call_function('filesize', [var_filename.clone()])).str()),
	])
	rt.call_function('flush', []rt.PhpVal{})
	rt.call_function('readfile', [var_filename.clone()])
	rt.call_function('unlink', [var_filename.clone()])
	exit(0)
	return rt.new_null()
}
