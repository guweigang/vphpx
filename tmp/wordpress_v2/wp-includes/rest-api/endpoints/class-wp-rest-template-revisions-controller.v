...
fn (mut this Class_WP_REST_Template_Revisions_Controller) construct(var_parent_post_type rt.PhpVal) {
	this.Class_WP_REST_Revisions_Controller.construct(var_parent_post_type.clone())
...
fn (mut this Class_WP_REST_Template_Revisions_Controller) register_routes() {
	rt.call_function('register_rest_route', [
		rt.get_property(rt.new_object('WP_REST_Template_Revisions_Controller', [
			'WP_REST_Revisions_Controller',
		], &this), 'namespace'),
		rt.call_function('sprintf', [
			rt.new_string('/%s/(?P<parent>%s%s)/%s'),
			this.parent_base,
			rt.new_string('([^\\/:<>\\*\\?"\\|]+(?:\\/[^\\/:<>\\*\\?"\\|]+)?)'),
			rt.new_string('[\\/\\w%-]+'),
			rt.get_property(rt.new_object('WP_REST_Template_Revisions_Controller', [
...
fn (mut this Class_WP_REST_Template_Revisions_Controller) get_parent(var_parent_template_id rt.PhpVal) rt.PhpVal {
	mut var_template := rt.call_function('get_block_template', [
		var_parent_template_id.clone(), this.parent_post_type])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_template)))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('rest_post_invalid_parent'), rt.call_function('__', [
			rt.new_string('Invalid template parent ID.'),
		]), rt.create_array([
			rt.ArrayItem{ key: 'status', val: Class_WP_Http.not_found() },
		])))
	}
	mut var_parent_post_id := rt.new_int(if !(rt.get_property(var_template, 'wp_id')).is_null() {
		rt.new_int((rt.get_property(var_template, 'wp_id')).to_i64())
	} else {
		0
	})
	if rt.is_true(rt.less_equal(var_parent_post_id, rt.new_int(0))) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('rest_invalid_template'), rt.call_function('__', [
			rt.new_string("Templates based on theme files can't have revisions."),
		]), rt.create_array([
...
fn (mut this Class_WP_REST_Template_Revisions_Controller) get_item_schema() rt.PhpVal {
	if rt.is_true(rt.get_property(rt.new_object('WP_REST_Template_Revisions_Controller', [
		'WP_REST_Revisions_Controller',
	], &this), 'schema'))
	{
		return this.add_additional_fields_schema(rt.get_property(rt.new_object('WP_REST_Template_Revisions_Controller', [
			'WP_REST_Revisions_Controller',
		], &this), 'schema'))
	}
