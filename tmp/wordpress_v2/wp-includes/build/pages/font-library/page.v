...
var_wp_font_library_routes = []rt.PhpVal{}
var_wp_font_library_menu_items = []rt.PhpVal{}
fn wp_register_font_library_route(var_path rt.PhpVal, var_content_module rt.PhpVal, var_route_module rt.PhpVal) {
	...
	var_wp_font_library_routes << var_route.clone()
}
...
fn wp_register_font_library_menu_item(var_id rt.PhpVal, var_label rt.PhpVal, var_to rt.PhpVal, parent_id string, parent_type string) {
	...
	var_wp_font_library_menu_items << var_menu_item.clone()
}
...
fn wp_font_library_render_page() {
	...
	var_menu_items = wp_get_font_library_menu_items()
	var_routes = wp_get_font_library_routes()
...
fn wp_font_library_intercept_render() {
	if rt.get_superglobal('_GET').array_isset(rt.new_string('page'))
		&& rt.is_true(rt.identical(rt.new_string('font-library'), rt.get_superglobal('_GET').array_get(rt.new_string('page')))) {
...
fn main() {
	...
	mut var_wp_font_library_routes := rt.get_superglobal('wp_font_library_routes')
	mut var_wp_font_library_menu_items := rt.get_superglobal('wp_font_library_menu_items')
