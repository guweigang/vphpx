...
var_wp_font_library_wp_admin_routes = []rt.PhpVal{}
var_wp_font_library_wp_admin_menu_items = []rt.PhpVal{}
fn wp_register_font_library_wp_admin_route(var_path rt.PhpVal, var_content_module rt.PhpVal, var_route_module rt.PhpVal) {
	mut var_wp_font_library_wp_admin_routes := []rt.PhpVal{}
	...
	var_wp_font_library_wp_admin_routes << var_route.clone()
}

fn wp_register_font_library_wp_admin_menu_item(var_id rt.PhpVal, var_label rt.PhpVal, var_to rt.PhpVal, parent_id string) {
	mut var_parent_id := parent_id
	mut var_wp_font_library_wp_admin_menu_items := []rt.PhpVal{}
	...
	var_wp_font_library_wp_admin_menu_items << var_menu_item.clone()
}

fn wp_get_font_library_wp_admin_routes() rt.PhpVal {
	mut var_wp_font_library_wp_admin_routes := []rt.PhpVal{}
	return if !var_wp_font_library_wp_admin_routes.is_null() {
		var_wp_font_library_wp_admin_routes
	} else {
		[]rt.PhpVal{}
	}
}

fn wp_get_font_library_wp_admin_menu_items() rt.PhpVal {
	mut var_wp_font_library_wp_admin_menu_items := []rt.PhpVal{}
	return if !var_wp_font_library_wp_admin_menu_items.is_null() {
		var_wp_font_library_wp_admin_menu_items
	} else {
		[]rt.PhpVal{}
	}
}

fn wp_font_library_wp_admin_preload_data() {
...
fn wp_font_library_wp_admin_enqueue_scripts(var_hook_suffix rt.PhpVal) {
	...
	var_is_our_page = rt.get_superglobal('_GET').array_isset(rt.new_string('page'))
		&& rt.is_true(rt.identical(rt.new_string('font-library-wp-admin'), rt.get_superglobal('_GET').array_get(rt.new_string('page'))))
		|| rt.is_true(var_current_screen)
		&& rt.is_true(rt.identical(rt.new_string('font-library'), rt.get_property(var_current_screen, 'id')))
	if !var_is_our_page {
		return
	}
	var_build_constants = rt.include_file(@DIR + '/../../constants.php', '3')
	rt.call_function('do_action', [rt.new_string('font-library-wp-admin_init')])
	wp_font_library_wp_admin_preload_data()
	var_routes = wp_get_font_library_wp_admin_routes()
	...
	if rt.is_true(rt.call_function('file_exists', [var_asset_file.clone()])) {
		...
		rt.call_function('wp_register_script', [
			rt.new_string('font-library-wp-admin-prerequisites'),
			rt.new_string(''),
		...
		rt.call_function('wp_add_inline_script', [
			...
			rt.call_function('sprintf', [rt.new_string('( %s )( %s, %s );'),
				...
				rt.call_function('wp_json_encode', [
					rt.new_string('font-library-wp-admin-app'),
		...
		rt.call_function('wp_register_script_module', [
			rt.new_string('font-library-wp-admin'),
			rt.new_string(
				(var_build_constants.array_get(rt.new_string('build_url'))).str() + 'pages/font-library/loader.js'),
...
fn wp_font_library_wp_admin_render_page() {
...
fn main() {
	...
	mut var_wp_font_library_wp_admin_routes := rt.get_superglobal('wp_font_library_wp_admin_routes')
	mut var_wp_font_library_wp_admin_menu_items :=
		rt.get_superglobal('wp_font_library_wp_admin_menu_items')
	rt.call_function('add_action', [rt.new_string('admin_enqueue_scripts'),
		rt.new_string('wp_font_library_wp_admin_enqueue_scripts')])
}
