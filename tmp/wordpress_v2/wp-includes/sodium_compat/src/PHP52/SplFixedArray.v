...
fn Class_SplFixedArray.fromarray(mut var_array Class_array, save_indexes bool) rt.PhpVal {
	...
	if var_save_indexes {
		...
		for {
			...
			var_self.array_set(rt.new_int(var_key.to_i64()), var_value.clone())
		}
	} else {
		mut var_i := rt.new_int(0)
		mut iter_2 := rt.call_function('array_values', [var_array]).iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_value := item_2.val
			var_self.array_set(var_i, var_value.clone())
			rt.post_inc(var_i)
		}
	}
	return var_self.clone()
}
...
fn (mut this Class_SplFixedArray) offsetexists(var_index rt.PhpVal) bool {
	return this.internalArray.array_isset(rt.new_int(var_index.to_i64()))
}

fn (mut this Class_SplFixedArray) offsetget(var_index rt.PhpVal) rt.PhpVal {
	return this.internalArray.array_get(rt.new_int(var_index.to_i64()))
}

fn (mut this Class_SplFixedArray) offsetset(var_index rt.PhpVal, var_newval rt.PhpVal) {
	this.internalArray.array_set(rt.new_int(var_index.to_i64()), var_newval.clone())
}

fn (mut this Class_SplFixedArray) offsetunset(var_index rt.PhpVal) {
	this.internalArray.array_unset(rt.new_int(var_index.to_i64()))
}
...
fn (mut this Class_SplFixedArray) valid() bool {
	...
	mut var_result := rt.new_bool(!rt.is_true(rt.identical(rt.call_function('next', [
		this.internalArray,
	]), rt.new_bool(false))))
...
fn (mut this Class_SplFixedArray) magic_unserialize(mut var_data Class_array) {
	mut var_length := rt.new_int(var_data.array_count())
