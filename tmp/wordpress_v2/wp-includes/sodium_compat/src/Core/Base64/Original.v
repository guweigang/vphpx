...
fn Class_ParagonIE_Sodium_Core_Base64_Original.encode(var_src rt.PhpVal) rt.PhpVal {
	...
	return Class_ParagonIE_Sodium_Core_Base64_Original.doencode(var_src_mutated.to_bool(),
		rt.new_bool(true))
}
...
fn Class_ParagonIE_Sodium_Core_Base64_Original.doencode(var_src rt.PhpVal, pad bool) rt.PhpVal {
	...
	for {
		...
		var_dest = rt.concat(var_dest, rt.new_string(
			(Class_ParagonIE_Sodium_Core_Base64_Original.encode6bits(rt.new_int(rt.shift_right(var_b0, rt.new_int(2))))).str() +
			(Class_ParagonIE_Sodium_Core_Base64_Original.encode6bits(rt.new_int(rt.shift_left(var_b0, rt.new_int(4)) | rt.shift_right(var_b1, rt.new_int(4)) & 63))).str() +
			(Class_ParagonIE_Sodium_Core_Base64_Original.encode6bits(rt.new_int(rt.shift_left(var_b1, rt.new_int(2)) | rt.shift_right(var_b2, rt.new_int(6)) & 63))).str() +(Class_ParagonIE_Sodium_Core_Base64_Original.encode6bits(rt.new_int(rt.bitwise_and(var_b2, rt.new_int(63))))).str()))
	...
	if rt.is_true(rt.less(var_i, var_srcLen)) {
		...
		if rt.is_true(rt.less(rt.add(var_i, rt.new_int(1)), var_srcLen)) {
			...
			var_dest = rt.concat(var_dest, rt.new_string(
				(Class_ParagonIE_Sodium_Core_Base64_Original.encode6bits(rt.new_int(rt.shift_right(var_b0, rt.new_int(2))))).str() +
				(Class_ParagonIE_Sodium_Core_Base64_Original.encode6bits(rt.new_int(rt.shift_left(var_b0, rt.new_int(4)) | rt.shift_right(var_b1, rt.new_int(4)) & 63))).str() +(Class_ParagonIE_Sodium_Core_Base64_Original.encode6bits(rt.new_int(rt.shift_left(var_b1, rt.new_int(2)) & 63))).str()))
			if var_pad {
				var_dest = rt.concat(var_dest, rt.new_string('='))
			}
		} else {
			var_dest = rt.concat(var_dest, rt.new_string(
				(Class_ParagonIE_Sodium_Core_Base64_Original.encode6bits(rt.new_int(rt.shift_right(var_b0, rt.new_int(2))))).str() +(Class_ParagonIE_Sodium_Core_Base64_Original.encode6bits(rt.new_int(rt.shift_left(var_b0, rt.new_int(4)) & 63))).str()))
			if var_pad {
				var_dest = rt.concat(var_dest, rt.new_string('=='))
			}
		}
...
fn Class_ParagonIE_Sodium_Core_Base64_Original.decode(var_src rt.PhpVal, strictPadding bool) string {
	...
	for {
		...
		mut var_c0 :=
			Class_ParagonIE_Sodium_Core_Base64_Original.decode6bits(var_chunk.array_get(rt.new_int(1)))
		mut var_c1 :=
			Class_ParagonIE_Sodium_Core_Base64_Original.decode6bits(var_chunk.array_get(rt.new_int(2)))
		mut var_c2 :=
			Class_ParagonIE_Sodium_Core_Base64_Original.decode6bits(var_chunk.array_get(rt.new_int(3)))
		mut var_c3 :=
			Class_ParagonIE_Sodium_Core_Base64_Original.decode6bits(var_chunk.array_get(rt.new_int(4)))
...
fn Class_ParagonIE_Sodium_Core_Base64_Original.decodenopadding(var_encodedString rt.PhpVal) string {
	...
	return (Class_ParagonIE_Sodium_Core_Base64_Original.decode(var_encodedString.to_bool(),
...
fn Class_ParagonIE_Sodium_Core_Base64_Original.decode6bits(var_src rt.PhpVal) rt.PhpVal {
	...
	var_ret = rt.add(var_ret, rt.bitwise_and(rt.sub(rt.new_int(42), var_src_mutated), rt.sub(var_src_mutated,
		rt.new_int(44))) >> 8 & 63)
	var_ret = rt.add(var_ret, rt.bitwise_and(rt.sub(rt.new_int(46), var_src_mutated), rt.sub(var_src_mutated,
		rt.new_int(48))) >> 8 & 64)
...
fn create_paragonie_sodium_core_base64_original(_args ...rt.PhpVal) &Class_ParagonIE_Sodium_Core_Base64_Original {
...
fn (mut this Class_ParagonIE_Sodium_Core_Base64_Original) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'doEncode' {
			...
			return Class_ParagonIE_Sodium_Core_Base64_Original.doencode(dispatch_arg_0,
				dispatch_arg_1)
		}
		'decode' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
			dispatch_arg_1 := (if args.len > 1 { args[1] } else { rt.new_null() }).to_bool()
			return rt.new_string(Class_ParagonIE_Sodium_Core_Base64_Original.decode(dispatch_arg_0,
				dispatch_arg_1))
		}
		'decodeNoPadding' {
			dispatch_arg_0 := if args.len > 0 { args[0] } else { rt.new_null() }
