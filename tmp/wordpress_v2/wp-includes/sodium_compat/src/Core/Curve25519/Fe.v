...
fn (mut this Class_ParagonIE_Sodium_Core_Curve25519_Fe) construct(e0 i64, e1 i64, e2 i64, e3 i64, e4 i64, e5 i64, e6 i64, e7 i64, e8 i64, e9 i64) {
	this.e0 = rt.new_int(e0)
	this.e1 = rt.new_int(e1)
	this.e2 = rt.new_int(e2)
	this.e3 = rt.new_int(e3)
	this.e4 = rt.new_int(e4)
	this.e5 = rt.new_int(e5)
	this.e6 = rt.new_int(e6)
	this.e7 = rt.new_int(e7)
	this.e8 = rt.new_int(e8)
	this.e9 = rt.new_int(e9)
}
...
fn Class_ParagonIE_Sodium_Core_Curve25519_Fe.fromarray(var_array rt.PhpVal) rt.PhpVal {
	...
	var_obj.e0 = if var_array.array_isset(rt.new_int(0)) {
		rt.new_int((var_array.array_get(rt.new_int(0))).to_i64())
	} else {
		0
	}
	var_obj.e1 = if var_array.array_isset(rt.new_int(1)) {
		rt.new_int((var_array.array_get(rt.new_int(1))).to_i64())
	} else {
		0
	}
	var_obj.e2 = if var_array.array_isset(rt.new_int(2)) {
		rt.new_int((var_array.array_get(rt.new_int(2))).to_i64())
	} else {
		0
	}
	var_obj.e3 = if var_array.array_isset(rt.new_int(3)) {
		rt.new_int((var_array.array_get(rt.new_int(3))).to_i64())
	} else {
		0
	}
	var_obj.e4 = if var_array.array_isset(rt.new_int(4)) {
		rt.new_int((var_array.array_get(rt.new_int(4))).to_i64())
	} else {
		0
	}
	var_obj.e5 = if var_array.array_isset(rt.new_int(5)) {
		rt.new_int((var_array.array_get(rt.new_int(5))).to_i64())
	} else {
		0
	}
	var_obj.e6 = if var_array.array_isset(rt.new_int(6)) {
		rt.new_int((var_array.array_get(rt.new_int(6))).to_i64())
	} else {
		0
	}
	var_obj.e7 = if var_array.array_isset(rt.new_int(7)) {
		rt.new_int((var_array.array_get(rt.new_int(7))).to_i64())
	} else {
		0
	}
	var_obj.e8 = if var_array.array_isset(rt.new_int(8)) {
		rt.new_int((var_array.array_get(rt.new_int(8))).to_i64())
	} else {
		0
	}
	var_obj.e9 = if var_array.array_isset(rt.new_int(9)) {
		rt.new_int((var_array.array_get(rt.new_int(9))).to_i64())
	} else {
		0
	}
	return mut var_obj
...
fn (mut this Class_ParagonIE_Sodium_Core_Curve25519_Fe) magic_debuginfo() rt.PhpVal {
	return rt.create_array([
		rt.ArrayItem{
			key: none
			val: rt.call_function('implode', [rt.new_string(', '),
				rt.create_array([rt.ArrayItem{ key: none, val: this.e0 },
					rt.ArrayItem{ key: none, val: this.e1 }, rt.ArrayItem{ key: none, val: this.e2 },
					rt.ArrayItem{ key: none, val: this.e3 }, rt.ArrayItem{ key: none, val: this.e4 },
					rt.ArrayItem{ key: none, val: this.e5 }, rt.ArrayItem{ key: none, val: this.e6 },
					rt.ArrayItem{ key: none, val: this.e7 }, rt.ArrayItem{ key: none, val: this.e8 },
					rt.ArrayItem{ key: none, val: this.e9 }])])
		},
	])
}
...
fn create_outofboundsexception(_args ...rt.PhpVal) &Class_OutOfBoundsException {
	...
	match method_name {
		'__construct' {
			...
			this.construct(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2, dispatch_arg_3,
				dispatch_arg_4, dispatch_arg_5, dispatch_arg_6, dispatch_arg_7, dispatch_arg_8,
				dispatch_arg_9)
			return rt.new_null()
		}
		'fromArray' {
...
fn main() {
	...
	if rt.is_true(rt.call_function('class_exists', [
		rt.new_string('ParagonIE_Sodium_Core_Curve25519_Fe'),
		rt.new_bool(false),
	]))
	{
		return rt.new_null()
