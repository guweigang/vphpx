...
fn add_rewrite_tag(var_tag rt.PhpVal, var_regex rt.PhpVal, query string) {
	...
	mut var_qv := ''
	if rt.create_array_from_list(var_tag).to_string().len < 3
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('%'), var_tag.array_get(rt.new_int(0))))))
		|| rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('%'), var_tag.array_get(rt.new_int(rt.create_array_from_list(var_tag).to_string().len - 1)))))) {
		return
	}
	if var_query == '' {
...
fn wp_resolve_numeric_slug_conflicts(var_query_vars rt.PhpVal) rt.PhpVal {
	...
	var_compare = ''
	if rt.is_true(rt.identical(rt.new_int(0), var_postname_index))
		&& var_query_vars.array_isset(rt.new_string('year'))
		|| var_query_vars.array_isset(rt.new_string('monthnum')) {
		var_compare = 'year'
	} else if rt.is_true(var_postname_index)
	...
	if rt.is_true(rt.new_bool(var_query_vars.clone().array_isset(rt.new_string(var_compare.str()).clone()))) {
		var_value = var_query_vars.array_get(rt.new_string(var_compare.str()))
	}
	...
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^([0-9]{4})\\-([0-9]{2})/'), rt.get_property(var_post, 'post_date'), var_matches.clone()]))
		&& var_query_vars.array_isset(rt.new_string('year'))
		...
		if rt.is_true(rt.new_bool(rt.new_int((var_query_vars.array_get(rt.new_string('year'))).to_i64()) != rt.new_int((var_matches.array_get(rt.new_int(1))).to_i64()))) {
			return var_query_vars.clone()
		}
		if rt.is_true(rt.identical(rt.new_string('day'), rt.new_string(var_compare.str())))
			&& var_query_vars.array_isset(rt.new_string('monthnum'))
			&& rt.is_true(rt.new_bool(rt.new_int((var_query_vars.array_get(rt.new_string('monthnum'))).to_i64()) != rt.new_int((var_matches.array_get(rt.new_int(2))).to_i64()))) {
			return var_query_vars.clone()
		}
	}
	var_maybe_page = rt.new_string('')
	if rt.is_true(rt.identical(rt.new_string('year'), rt.new_string(var_compare.str())))
		&& var_query_vars.array_isset(rt.new_string('monthnum')) {
		var_maybe_page = var_query_vars.array_get(rt.new_string('monthnum'))
	} else if rt.is_true(rt.identical(rt.new_string('monthnum'), rt.new_string(var_compare.str())))
		&& var_query_vars.array_isset(rt.new_string('day')) {
		var_maybe_page = var_query_vars.array_get(rt.new_string('day'))
	}
	...
	if rt.is_true(rt.identical(rt.new_int(1), var_post_page_count)) && rt.is_true(var_maybe_page) {
		return var_query_vars.clone()
	}
	if rt.is_true(rt.greater(var_post_page_count, rt.new_int(1)))
...
fn url_to_postid(var_url_arg rt.PhpVal) i64 {
	...
	{
		var_id = rt.call_function('absint', [var_values[2]])
	...
	var_url_split = rt.call_function('explode', [rt.new_string('#'),
		var_url.clone()])
	var_url = var_url_split.array_get(rt.new_int(0))
	var_url_split = rt.call_function('explode', [rt.new_string('?'),
		var_url.clone()])
	var_url = var_url_split.array_get(rt.new_int(0))
	...
	var_url = rt.call_function('set_url_scheme', [var_url.clone(),
		var_scheme.clone()])
	if rt.is_true(rt.call_function('str_contains', [rt.call_function('home_url', []rt.PhpVal{}), rt.new_string('://www.')]))
	...
	if rt.is_true(rt.identical(rt.new_string(var_url.clone().to_string().trim_space()), rt.call_function('home_url', []rt.PhpVal{})))
		...
		if rt.is_true(var_page_on_front)
	...
	]))
	{
		...
		var_home_path = if !(var_home_path.array_get(rt.new_string('path'))).is_null() {
			var_home_path.array_get(rt.new_string('path'))
		} else {
			rt.new_string('')
		}
	...
	mut iter_2 := rt.cast_array(var_rewrite).iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_query_shadow := item_2.val
		mut var_match_shadow := item_2.key
		...
		{
			if rt.is_true(rt.get_property(var_wp_rewrite, 'use_verbose_page_rules'))
				...
				var_page = rt.call_function('get_page_by_path', [
					var_matches.array_get(var_varmatch[1]),
				])
				...
				if rt.is_true(rt.new_bool(!(rt.is_true(rt.get_property(var_post_status_obj, 'public')))))
			...
			mut iife_result_0 := iife_temp_0.apply(var_query_shadow.clone(), var_matches.clone())
			mut iife_temp_1 := Class_WP_MatchesMapRegex{}
			mut iife_result_1 := iife_temp_1.apply(var_query_shadow.clone(), var_matches.clone())
			var_query_shadow = rt.call_function('addslashes', [iife_result_0])
			rt.call_function('parse_str', [var_query_shadow.clone(),
				var_query_vars.clone()])
			var_query_shadow = rt.new_array()
			mut iter_3 := rt.cast_array(var_query_vars).iterator()
			for {
			...
			if !(!rt.is_true(rt.get_property(var_query_shadow, 'posts')))
