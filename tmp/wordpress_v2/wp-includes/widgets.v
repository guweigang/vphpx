	...
	for {
		...
		if var_number > 1 {
			if var_args.array_isset(rt.new_string('name')) {
				var__args.array_set('name', rt.call_function('sprintf', [var_args.array_get(rt.new_string('name')), rt.new_int(var_i).clone()]))
			} else {
				var__args.array_set('name', rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('Sidebar %d')]), rt.new_int(var_i).clone()]))
			}
		} else {
			var__args.array_set('name', if !(var_args.array_get(rt.new_string('name'))).is_null() { var_args.array_get(rt.new_string('name')) } else { rt.call_function('__', [rt.new_string('Sidebar')]) })
		}
		if var_args.array_isset(rt.new_string('id')) {
			var__args.array_set('id', var_args.array_get(rt.new_string('id')))
			var_n = 2
			for rt.is_true(is_registered_sidebar(var__args.array_get(rt.new_string('id')))) {
				var__args.array_set('id', (var_args.array_get(rt.new_string('id'))).str() + '-' + (rt.post_inc(rt.new_int(var_n))).str())
			}
		} else {
			var_n = var_wp_registered_sidebars.clone().array_count()
			for {
				var__args.array_set('id', 'sidebar-' + (rt.pre_inc(rt.new_int(var_n))).str())
				if !(rt.is_true(is_registered_sidebar(var__args.array_get(rt.new_string('id'))))) {
					break
				}
			}
		}
...
fn register_sidebar(var_args rt.PhpVal) rt.PhpVal {
	...
	var_id_is_empty = rt.new_bool(!rt.is_true(var_args.array_get(rt.new_string('id'))))
	...
	return var_sidebar.array_get(rt.new_string('id'))
}
...
fn wp_register_sidebar_widget(var_id_arg rt.PhpVal, name string, output_callback string, var_options_arg rt.PhpVal, var_params_origin ...rt.PhpVal) {
	...
	if rt.is_true(rt.call_function('in_array', [rt.new_string(output_callback), var__wp_deprecated_widgets_callbacks.clone(), rt.new_bool(true)])) && !(rt.call_function('is_callable', [rt.new_string(output_callback)])) {
	...
	if rt.call_function('is_callable', [rt.new_string(output_callback)]) && !(var_wp_registered_widgets.array_isset(rt.new_string((var_id).str()))) || rt.is_true(rt.call_function('did_action', [rt.new_string('widgets_init')])) {
...
fn wp_widget_description(var_id rt.PhpVal) rt.PhpVal {
	...
	if var_wp_registered_widgets.array_get(var_id).array_isset(rt.new_string('description')) {
		return rt.call_function('esc_html', [var_wp_registered_widgets.array_get(var_id).array_get(rt.new_string('description'))])
	}
	return rt.new_null()
}
...
fn wp_sidebar_description(var_id rt.PhpVal) rt.PhpVal {
	...
	if var_wp_registered_sidebars.array_get(var_id).array_isset(rt.new_string('description')) {
		return rt.call_function('wp_kses', [var_wp_registered_sidebars.array_get(var_id).array_get(rt.new_string('description')), rt.new_string('sidebar_description')])
	}
	return rt.new_null()
}
...
fn wp_register_widget_control(var_id_arg rt.PhpVal, name string, control_callback string, var_options_arg rt.PhpVal, var_params_origin ...rt.PhpVal) {
	...
	if rt.is_true(rt.call_function('in_array', [rt.new_string(control_callback), var__wp_deprecated_widgets_callbacks.clone(), rt.new_bool(true)])) && !(rt.call_function('is_callable', [rt.new_string(control_callback)])) {
		var_wp_registered_widgets.array_unset(rt.new_string((var_id).str()))
		return
	}
	if var_wp_registered_widget_controls.array_isset(rt.new_string((var_id).str())) && rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('did_action', [rt.new_string('widgets_init')]))))) {
		return
	}
	var_defaults = rt.create_array([rt.ArrayItem{ key: 'width', val: 250 }, rt.ArrayItem{ key: 'height', val: 200 }])
	var_options = rt.call_function('wp_parse_args', [var_options.clone(), var_defaults.clone()])
	var_options.array_set('width', rt.new_int((var_options.array_get(rt.new_string('width'))).to_i64()))
	var_options.array_set('height', rt.new_int((var_options.array_get(rt.new_string('height'))).to_i64()))
	...
	if var_widget.array_get(rt.new_string('params')).array_get(rt.new_int(0)).array_isset(rt.new_string('number')) {
...
fn dynamic_sidebar(index i64) rt.PhpVal {
	...
	var_index = "sidebar-${var_index.str()}"
		...
		for {
			...
			if rt.is_true(rt.identical(rt.call_function('sanitize_title', [var_value_shadow['name']]), rt.new_int(var_index))) {
	...
	if !rt.is_true(var_wp_registered_sidebars.array_get(rt.new_int(var_index))) || !rt.is_true(var_sidebars_widgets.array_get(rt.new_int(var_index))) || !(var_sidebars_widgets.array_get(rt.new_int(var_index)).is_array()) {
	...
	var_sidebar = var_wp_registered_sidebars.array_get(rt.new_int(var_index))
	var_sidebar.array_set('before_sidebar', rt.call_function('sprintf', [var_sidebar.array_get(rt.new_string('before_sidebar')), var_sidebar.array_get(rt.new_string('id')), var_sidebar.array_get(rt.new_string('class'))]))
	rt.call_function('do_action', [rt.new_string('dynamic_sidebar_before'), rt.new_int(var_index), rt.new_bool(true)])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_admin', []rt.PhpVal{}))))) && !(!rt.is_true(var_sidebar.array_get(rt.new_string('before_sidebar')))) {
		rt.echo_val(var_sidebar.array_get(rt.new_string('before_sidebar')))
	}
	var_did_one = false
	mut iter_2 := rt.cast_array(var_sidebars_widgets.array_get(rt.new_int(var_index))).iterator()
	for {
		...
		var_params = rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: none, val: rt.call_function('array_merge', [var_sidebar.clone(), rt.create_array([rt.ArrayItem{ key: 'widget_id', val: var_id_shadow }, rt.ArrayItem{ key: 'widget_name', val: var_wp_registered_widgets.array_get(var_id_shadow).array_get(rt.new_string('name')) }])]) }]), rt.cast_array(var_wp_registered_widgets.array_get(var_id_shadow).array_get(rt.new_string('params')))])
		var_classname_ = ''
		mut iter_3 := rt.cast_array(var_wp_registered_widgets.array_get(var_id_shadow).array_get(rt.new_string('classname'))).iterator()
		for {
			item_3 := iter_3.next() or { break }
			mut var_cn_shadow := item_3.val
		...
		var_params.array_get_mut(0).array_set('before_widget', rt.call_function('sprintf', [var_params.array_get(rt.new_int(0)).array_get(rt.new_string('before_widget')), rt.call_function('str_replace', [rt.new_string('\\'), rt.new_string('_'), var_id_shadow.clone()]), rt.new_string((var_classname_).str()).clone()]))
		var_params = rt.call_function('apply_filters', [rt.new_string('dynamic_sidebar_params'), var_params.clone()])
		var_callback = var_wp_registered_widgets.array_get(var_id_shadow).array_get(rt.new_string('callback'))
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_admin', []rt.PhpVal{}))))) && !(!rt.is_true(var_sidebar.array_get(rt.new_string('after_sidebar')))) {
		rt.echo_val(var_sidebar.array_get(rt.new_string('after_sidebar')))
	}
...
fn is_active_widget(callback bool, widget_id bool, id_base bool, skip_inactive bool) bool {
	...
	if rt.is_true(rt.new_bool(var_sidebars_widgets.clone().is_array())) {
		...
		for {
			...
			mut var_widgets_shadow := item_4.val
			mut var_sidebar_shadow := item_4.key
			if var_skip_inactive && rt.is_true(rt.identical(rt.new_string('wp_inactive_widgets'), var_sidebar_shadow)) || rt.is_true(rt.call_function('str_starts_with', [var_sidebar_shadow.clone(), rt.new_string('orphaned_widgets')])) {
				continue
			}
			if rt.is_true(rt.new_bool(var_widgets_shadow.clone().is_array())) {
				mut iter_5 := var_widgets_shadow.iterator()
				for {
					item_5 := iter_5.next() or { break }
					mut var_widget_shadow := item_5.val
					if (var_callback && var_wp_registered_widgets.array_get(var_widget_shadow).array_isset(rt.new_string('callback')) && rt.is_true(rt.identical(var_wp_registered_widgets.array_get(var_widget_shadow).array_get(rt.new_string('callback')), rt.new_bool(callback)))) || (var_id_base && rt.is_true(rt.identical(_get_widget_id_base(var_widget_shadow.clone()), rt.new_bool(id_base)))) {
						if !(var_widget_id) || (var_wp_registered_widgets.array_get(var_widget_shadow).array_isset(rt.new_string('id')) && rt.is_true(rt.identical(rt.new_bool(widget_id), var_wp_registered_widgets.array_get(var_widget_shadow).array_get(rt.new_string('id'))))) {
...
fn is_dynamic_sidebar() bool {
	...
	mut iter_6 := rt.cast_array(var_wp_registered_sidebars).iterator()
	for {
		item_6 := iter_6.next() or { break }
		mut var_sidebar_shadow := item_6.val
		mut var_index_shadow := item_6.key
		if !(!rt.is_true(var_sidebars_widgets.array_get(var_index_shadow))) {
			mut iter_7 := rt.cast_array(var_sidebars_widgets.array_get(var_index_shadow)).iterator()
			for {
				item_7 := iter_7.next() or { break }
				mut var_widget_shadow := item_7.val
...
fn is_active_sidebar(var_index_arg rt.PhpVal) rt.PhpVal {
	...
	var_index = if var_index.clone().is_long() { rt.new_string("sidebar-${var_index.to_string()}") } else { rt.call_function('sanitize_title', [var_index.clone()]) }
	var_sidebars_widgets = wp_get_sidebars_widgets(false)
...
fn wp_get_sidebars_widgets(deprecated bool) rt.PhpVal {
	...
	if var_sidebars_widgets.clone().is_array() && var_sidebars_widgets.array_isset(rt.new_string('array_version')) {
...
fn wp_get_sidebar(var_id rt.PhpVal) rt.PhpVal {
	mut var_wp_registered_sidebars := rt.new_null()
	mut var_sidebar := rt.new_null()
	mut iter_8 := rt.cast_array(var_wp_registered_sidebars).iterator()
	for {
		item_8 := iter_8.next() or { break }
		mut var_sidebar_shadow := item_8.val
		if rt.is_true(rt.identical(var_sidebar_shadow.array_get(rt.new_string('id')), var_id)) {
...
fn wp_get_widget_defaults() rt.PhpVal {
	...
	mut iter_9 := rt.cast_array(var_wp_registered_sidebars).iterator()
	for {
		item_9 := iter_9.next() or { break }
		mut var_sidebar_shadow := item_9.val
		mut var_index_shadow := item_9.key
...
fn wp_convert_widget_settings(var_base_name rt.PhpVal, var_option_name rt.PhpVal, var_settings_arg rt.PhpVal) rt.PhpVal {
	...
	var_single = true
		...
		mut iter_10 := rt.func_array_keys(var_settings.clone()).iterator()
		for {
			item_10 := iter_10.next() or { break }
			mut var_number_shadow := item_10.val
			if rt.is_true(rt.identical(rt.new_string('number'), var_number_shadow)) {
				continue
			}
			if !(var_number_shadow.clone().is_long() || var_number_shadow.clone().is_double()) {
				var_single = true
				break
			}
		}
	...
	if var_single {
		...
		var_sidebars_widgets = rt.call_function('get_option', [rt.new_string('sidebars_widgets')])
			...
			if !rt.is_true(var_GLOBALS.array_get(rt.new_string('_wp_sidebars_widgets'))) {
				var_GLOBALS.array_set('_wp_sidebars_widgets', rt.call_function('get_option', [rt.new_string('sidebars_widgets'), rt.new_array()]))
			}
			var_sidebars_widgets = var_GLOBALS.array_get(rt.new_string('_wp_sidebars_widgets'))
		}
		mut iter_11 := rt.cast_array(var_sidebars_widgets).iterator()
		for {
			item_11 := iter_11.next() or { break }
			mut var_sidebar_shadow := item_11.val
			mut var_index_shadow := item_11.key
			if rt.is_true(rt.new_bool(var_sidebar_shadow.clone().is_array())) {
				mut iter_12 := var_sidebar_shadow.iterator()
				for {
					item_12 := iter_12.next() or { break }
					mut var_name_shadow := item_12.val
					mut var_i_shadow := item_12.key
		...
		if rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) && var_changed {
			rt.call_function('update_option', [rt.new_string('sidebars_widgets'), var_sidebars_widgets.clone()])
		}
	}
...
fn the_widget(var_widget rt.PhpVal, var_instance_arg rt.PhpVal, var_args_arg rt.PhpVal) {
	...
	var_args.array_set('before_widget', rt.call_function('sprintf', [var_args.array_get(rt.new_string('before_widget')), rt.get_property(var_widget_obj, 'widget_options').array_get(rt.new_string('classname'))]))
	...
	rt.call_method(var_widget_obj, '_set', [rt.new_int(-1)])
...
fn _wp_sidebars_changed() {
	mut var_sidebars_widgets := rt.new_null()
	if !(var_sidebars_widgets.clone().is_array()) {
	var_sidebars_widgets = wp_get_sidebars_widgets(false)
	}
...
fn retrieve_widgets(theme_changed bool) rt.PhpVal {
	...
	if !(rt.call_function('get_theme_mod', [rt.new_string('sidebars_widgets')]).is_array()) {
	...
	mut iter_13 := var_sidebars_widgets.iterator()
	for {
	...
	mut iter_14 := var_lost_widgets.iterator()
	for {
		item_14 := iter_14.next() or { break }
		mut var_widget_id_shadow := item_14.val
		...
		if var_number.clone().is_long() || var_number.clone().is_double() && rt.new_int((var_number).to_i64()) < 2 {
			var_lost_widgets.array_unset(var_key_shadow)
		}
	}
	var_sidebars_widgets.array_set('wp_inactive_widgets', rt.call_function('array_merge', [var_lost_widgets.clone(), rt.cast_array(var_sidebars_widgets.array_get(rt.new_string('wp_inactive_widgets')))]))
...
fn wp_map_sidebars_widgets(var_existing_sidebars_widgets rt.PhpVal) rt.PhpVal {
	...
	if !(var_existing_sidebars_widgets.clone().is_array()) || !rt.is_true(var_existing_sidebars_widgets) {
		return var_new_sidebars_widgets.clone()
	}
	mut iter_15 := var_existing_sidebars_widgets.iterator()
	for {
		item_15 := iter_15.next() or { break }
		mut var_widgets_shadow := item_15.val
		mut var_sidebar_shadow := item_15.key
		if rt.is_true(rt.identical(rt.new_string('wp_inactive_widgets'), var_sidebar_shadow)) || rt.is_true(rt.call_function('str_starts_with', [var_sidebar_shadow.clone(), rt.new_string('orphaned_widgets')])) {
			var_new_sidebars_widgets.array_set('wp_inactive_widgets', rt.call_function('array_merge', [var_new_sidebars_widgets.array_get(rt.new_string('wp_inactive_widgets')), rt.cast_array(var_widgets_shadow)]))
	...
	mut iter_16 := var_wp_registered_sidebars.iterator()
	for {
		item_16 := iter_16.next() or { break }
		mut var_name_shadow := item_16.val
		mut var_sidebar_shadow := item_16.key
	...
	if !(!rt.is_true(var_existing_sidebars_widgets)) {
		...
		for var_slug_group_shadow in var_common_slug_groups {
			mut iter_17 := var_slug_group_shadow.iterator()
			for {
				item_17 := iter_17.next() or { break }
				mut var_slug_shadow := item_17.val
				mut iter_18 := var_wp_registered_sidebars.iterator()
				for {
					item_18 := iter_18.next() or { break }
					mut var_args_shadow := item_18.val
					mut var_new_sidebar_shadow := item_18.key
					if rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('stripos', [var_new_sidebar_shadow.clone(), var_slug_shadow.clone()]))) && rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('stripos', [var_slug_shadow.clone(), var_new_sidebar_shadow.clone()]))) {
						continue
					}
					mut iter_19 := var_existing_sidebars_widgets.iterator()
					for {
						item_19 := iter_19.next() or { break }
						mut var_widgets_shadow := item_19.val
						mut var_sidebar_shadow := item_19.key
						mut iter_20 := var_slug_group_shadow.iterator()
						for {
							item_20 := iter_20.next() or { break }
							mut var_slug_shadow := item_20.val
							if rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('stripos', [var_sidebar_shadow.clone(), var_slug_shadow.clone()]))) && rt.is_true(rt.identical(rt.new_bool(false), rt.call_function('stripos', [var_slug_shadow.clone(), var_sidebar_shadow.clone()]))) {
								continue
							}
	...
	mut iter_21 := var_existing_sidebars_widgets.iterator()
	for {
		item_21 := iter_21.next() or { break }
		mut var_widgets_shadow := item_21.val
		if var_widgets_shadow.clone().is_array() && !(!rt.is_true(var_widgets_shadow)) {
			var_new_sidebars_widgets.array_set('wp_inactive_widgets', rt.call_function('array_merge', [var_new_sidebars_widgets.array_get(rt.new_string('wp_inactive_widgets')), var_widgets_shadow.clone()]))
		}
	}
	var_old_sidebars_widgets = rt.call_function('get_theme_mod', [rt.new_string('sidebars_widgets')])
	var_old_sidebars_widgets = if !(var_old_sidebars_widgets.array_get(rt.new_string('data'))).is_null() { var_old_sidebars_widgets.array_get(rt.new_string('data')) } else { rt.new_bool(false) }
	if rt.is_true(rt.new_bool(var_old_sidebars_widgets.clone().is_array())) {
		...
		mut iter_22 := var_new_sidebars_widgets.iterator()
		for {
			item_22 := iter_22.next() or { break }
			mut var_new_widgets_shadow := item_22.val
			mut var_new_sidebar_shadow := item_22.key
			if rt.is_true(rt.new_bool(var_old_sidebars_widgets.clone().array_isset(var_new_sidebar_shadow.clone()))) && !(!rt.is_true(var_new_widgets_shadow)) {
				var_old_sidebars_widgets.array_unset(var_new_sidebar_shadow)
			}
		}
		mut iter_23 := var_old_sidebars_widgets.iterator()
		for {
			item_23 := iter_23.next() or { break }
			mut var_widgets_shadow := item_23.val
			mut var_sidebar_shadow := item_23.key
		...
		mut iter_24 := var_new_sidebars_widgets.iterator()
		for {
		...
		if !(!rt.is_true(var_old_sidebars_widgets)) {
			mut iter_25 := var_old_sidebars_widgets.iterator()
			for {
				item_25 := iter_25.next() or { break }
				mut var_old_widgets_shadow := item_25.val
				mut var_old_sidebar_shadow := item_25.key
				mut iter_26 := var_new_sidebars_widgets.iterator()
				for {
					item_26 := iter_26.next() or { break }
					mut var_new_widgets_shadow := item_26.val
					mut var_new_sidebar_shadow := item_26.key
					mut iter_27 := var_old_widgets_shadow.iterator()
					for {
						item_27 := iter_27.next() or { break }
						mut var_widget_id_shadow := item_27.val
						mut var_key_shadow := item_27.key
						...
						if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_active_key)))) {
							if rt.is_true(rt.identical(rt.new_string('wp_inactive_widgets'), var_new_sidebar_shadow)) {
								var_new_sidebars_widgets.array_get(rt.new_string('wp_inactive_widgets')).array_unset(var_active_key)
							} else {
...
fn _wp_remove_unregistered_widgets(var_sidebars_widgets rt.PhpVal, var_allowed_widget_ids_arg rt.PhpVal) rt.PhpVal {
	...
	var_allowed_widget_ids = rt.func_array_keys(var_GLOBALS.array_get(rt.new_string('wp_registered_widgets')))
	}
	mut iter_28 := var_sidebars_widgets.iterator()
	for {
		item_28 := iter_28.next() or { break }
		mut var_widgets_shadow := item_28.val
		mut var_sidebar_shadow := item_28.key
		if rt.is_true(rt.new_bool(var_widgets_shadow.clone().is_array())) {
...
fn wp_widget_rss_output(var_rss_arg rt.PhpVal, var_args_arg rt.PhpVal) {
	...
	} else if var_rss.clone().is_array() && var_rss.array_isset(rt.new_string('url')) {
	var_args = var_rss.clone()
	var_rss = rt.call_function('fetch_feed', [var_rss.array_get(rt.new_string('url'))])
	} else if !(var_rss.clone().is_object()) {
		return
	}
	if rt.is_true(rt.call_function('is_wp_error', [var_rss.clone()])) {
		if rt.is_true(rt.call_function('is_admin', []rt.PhpVal{})) || rt.is_true(rt.call_function('current_user_can', [rt.new_string('manage_options')])) {
	...
	var_args = rt.call_function('wp_parse_args', [var_args.clone(), rt.create_array_from_native_map(var_default_args)])
	var_items = rt.new_int((var_args.array_get(rt.new_string('items'))).to_i64())
	if rt.is_true(rt.less(var_items, rt.new_int(1))) || rt.is_true(rt.less(rt.new_int(20), var_items)) {
	var_items = rt.new_int(10)
	}
	var_show_summary = rt.new_int((var_args.array_get(rt.new_string('show_summary'))).to_i64())
	var_show_author = rt.new_int((var_args.array_get(rt.new_string('show_author'))).to_i64())
	var_show_date = rt.new_int((var_args.array_get(rt.new_string('show_date'))).to_i64())
	...
	print('<ul>')
	mut iter_29 := rt.call_method(var_rss, 'get_items', [rt.new_int(0), var_items.clone()]).iterator()
	for {
		item_29 := iter_29.next() or { break }
		mut var_item_shadow := item_29.val
		var_link = rt.call_method(var_item_shadow, 'get_link', []rt.PhpVal{})
		for !(!rt.is_true(var_link)) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.call_function('stristr', [var_link.clone(), rt.new_string('http')]), var_link)))) {
		...
		if rt.is_true(var_show_summary) {
			...
			var_summary = rt.new_string((rt.call_function('substr', [var_summary.clone(), rt.new_int(0), rt.new_int(-5)])).str() + '[&hellip;]')
			}
...
fn wp_widget_rss_form(var_args rt.PhpVal, var_inputs_arg rt.PhpVal) {
	...
	var_args.array_set('title', if !(var_args.array_get(rt.new_string('title'))).is_null() { var_args.array_get(rt.new_string('title')) } else { rt.new_string('') })
	var_args.array_set('url', if !(var_args.array_get(rt.new_string('url'))).is_null() { var_args.array_get(rt.new_string('url')) } else { rt.new_string('') })
	var_args.array_set('items', rt.new_int((if !(var_args.array_get(rt.new_string('items'))).is_null() { var_args.array_get(rt.new_string('items')) } else { rt.new_int(0) }).to_i64()))
	if rt.is_true(rt.less(var_args.array_get(rt.new_string('items')), rt.new_int(1))) || rt.is_true(rt.less(rt.new_int(20), var_args.array_get(rt.new_string('items')))) {
		var_args.array_set('items', 10)
	}
	var_args.array_set('show_summary', rt.new_int((if !(var_args.array_get(rt.new_string('show_summary'))).is_null() { var_args.array_get(rt.new_string('show_summary')) } else { var_inputs.array_get(rt.new_string('show_summary')) }).to_i64()))
	var_args.array_set('show_author', rt.new_int((if !(var_args.array_get(rt.new_string('show_author'))).is_null() { var_args.array_get(rt.new_string('show_author')) } else { var_inputs.array_get(rt.new_string('show_author')) }).to_i64()))
	var_args.array_set('show_date', rt.new_int((if !(var_args.array_get(rt.new_string('show_date'))).is_null() { var_args.array_get(rt.new_string('show_date')) } else { var_inputs.array_get(rt.new_string('show_date')) }).to_i64()))
	if !(!rt.is_true(var_args.array_get(rt.new_string('error')))) {
		print('<p class="widget-error"><strong>' + (rt.call_function('__', [rt.new_string('RSS Error:')])).str() + '</strong> ' + (rt.call_function('esc_html', [var_args.array_get(rt.new_string('error'))])).str() + '</p>')
	}
	var_esc_number = rt.call_function('esc_attr', [var_args.array_get(rt.new_string('number'))])
	if rt.is_true(var_inputs.array_get(rt.new_string('url'))) {
		...
		rt.echo_val(var_esc_number)
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_url', [var_args.array_get(rt.new_string('url'))]))
		// unsupported statement: Stmt_InlineHTML
	}
	if rt.is_true(var_inputs.array_get(rt.new_string('title'))) {
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(var_esc_number)
		// unsupported statement: Stmt_InlineHTML
		...
		rt.echo_val(var_esc_number)
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_attr', [var_args.array_get(rt.new_string('title'))]))
		// unsupported statement: Stmt_InlineHTML
	}
	if rt.is_true(var_inputs.array_get(rt.new_string('items'))) {
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(var_esc_number)
		// unsupported statement: Stmt_InlineHTML
		...
		for {
			if !(var_i <= 20) { break }
			print("<option value='${var_i.str()}' " + (rt.call_function('selected', [var_args.array_get(rt.new_string('items')), rt.new_int(var_i).clone(), rt.new_bool(false)])).str() + ">${var_i.str()}</option>")
			var_i += 1
		}
		// unsupported statement: Stmt_InlineHTML
	}
	if rt.is_true(var_inputs.array_get(rt.new_string('show_summary'))) || rt.is_true(var_inputs.array_get(rt.new_string('show_author'))) || rt.is_true(var_inputs.array_get(rt.new_string('show_date'))) {
		// unsupported statement: Stmt_InlineHTML
		if rt.is_true(var_inputs.array_get(rt.new_string('show_summary'))) {
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(var_esc_number)
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(var_esc_number)
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('checked', [var_args.array_get(rt.new_string('show_summary'))])
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(var_esc_number)
			// unsupported statement: Stmt_InlineHTML
		...
		if rt.is_true(var_inputs.array_get(rt.new_string('show_author'))) {
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(var_esc_number)
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(var_esc_number)
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('checked', [var_args.array_get(rt.new_string('show_author'))])
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(var_esc_number)
			// unsupported statement: Stmt_InlineHTML
		...
		if rt.is_true(var_inputs.array_get(rt.new_string('show_date'))) {
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(var_esc_number)
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(var_esc_number)
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('checked', [var_args.array_get(rt.new_string('show_date'))])
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(var_esc_number)
			// unsupported statement: Stmt_InlineHTML
	...
	mut iter_30 := rt.func_array_keys(rt.create_array_from_native_map(var_default_inputs)).iterator()
	for {
		item_30 := iter_30.next() or { break }
		mut var_input_shadow := item_30.val
...
fn wp_widget_rss_process(var_widget_rss rt.PhpVal, check_feed bool) rt.PhpVal {
	...
	var_items = rt.new_int((var_widget_rss.array_get(rt.new_string('items'))).to_i64())
	if rt.is_true(rt.less(var_items, rt.new_int(1))) || rt.is_true(rt.less(rt.new_int(20), var_items)) {
	var_items = rt.new_int(10)
	}
	var_url = rt.call_function('sanitize_url', [rt.call_function('strip_tags', [var_widget_rss.array_get(rt.new_string('url'))])])
	var_title = rt.call_function('strip_tags', [if !(var_widget_rss.array_get(rt.new_string('title'))).is_null() { var_widget_rss.array_get(rt.new_string('title')) } else { rt.new_string('') }]).to_string().trim_space()
	var_show_summary = rt.new_int((if !(var_widget_rss.array_get(rt.new_string('show_summary'))).is_null() { var_widget_rss.array_get(rt.new_string('show_summary')) } else { rt.new_int(0) }).to_i64())
	var_show_author = rt.new_int((if !(var_widget_rss.array_get(rt.new_string('show_author'))).is_null() { var_widget_rss.array_get(rt.new_string('show_author')) } else { rt.new_int(0) }).to_i64())
	var_show_date = rt.new_int((if !(var_widget_rss.array_get(rt.new_string('show_date'))).is_null() { var_widget_rss.array_get(rt.new_string('show_date')) } else { rt.new_int(0) }).to_i64())
...
fn wp_parse_widget_id(var_id rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^(.+)-(\\d+)$/'), var_id.clone(), rt.create_array_from_list(var_matches)])) {
		var_parsed['id_base'] = var_matches[1]
		var_parsed['number'] = rt.new_int((var_matches[2]).to_i64())
	} else {
...
fn wp_find_widgets_sidebar(var_widget_id rt.PhpVal) rt.PhpVal {
	...
	mut iter_31 := wp_get_sidebars_widgets(false).iterator()
	for {
		item_31 := iter_31.next() or { break }
		mut var_widget_ids_shadow := item_31.val
		mut var_sidebar_id_shadow := item_31.key
		mut iter_32 := var_widget_ids_shadow.iterator()
		for {
			item_32 := iter_32.next() or { break }
			mut var_maybe_widget_id_shadow := item_32.val
...
fn wp_assign_widget_to_sidebar(var_widget_id rt.PhpVal, var_sidebar_id rt.PhpVal) {
	...
	mut iter_33 := var_sidebars.iterator()
	for {
		item_33 := iter_33.next() or { break }
		mut var_widgets_shadow := item_33.val
		mut var_maybe_sidebar_id_shadow := item_33.key
		mut iter_34 := var_widgets_shadow.iterator()
		for {
			item_34 := iter_34.next() or { break }
			mut var_maybe_widget_id_shadow := item_34.val
			mut var_i_shadow := item_34.key
			if rt.is_true(rt.identical(var_widget_id, var_maybe_widget_id_shadow)) && rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_sidebar_id, var_maybe_sidebar_id_shadow)))) {
...
fn wp_render_widget(var_widget_id rt.PhpVal, var_sidebar_id rt.PhpVal) string {
	...
	var_params = rt.call_function('array_merge', [rt.create_array([rt.ArrayItem{ key: none, val: rt.call_function('array_merge', [var_sidebar.clone(), rt.create_array([rt.ArrayItem{ key: 'widget_id', val: var_widget_id }, rt.ArrayItem{ key: 'widget_name', val: var_wp_registered_widgets.array_get(var_widget_id).array_get(rt.new_string('name')) }])]) }]), rt.cast_array(var_wp_registered_widgets.array_get(var_widget_id).array_get(rt.new_string('params')))])
	var_classname_ = ''
	mut iter_35 := rt.cast_array(var_wp_registered_widgets.array_get(var_widget_id).array_get(rt.new_string('classname'))).iterator()
	for {
		item_35 := iter_35.next() or { break }
		mut var_cn_shadow := item_35.val
		if rt.is_true(rt.new_bool(var_cn_shadow.clone().is_string())) {
			var_classname_ = var_classname_ + '_' + (var_cn_shadow).str()
		} else if rt.is_true(rt.new_bool(var_cn_shadow.clone().is_object())) {
			var_classname_ = var_classname_ + '_' + (rt.call_function('get_class', [var_cn_shadow.clone()])).str()
	...
	var_classname_ = var_classname_.trim_left(' \t\n\r')
	var_params.array_get_mut(0).array_set('before_widget', rt.call_function('sprintf', [var_params.array_get(rt.new_int(0)).array_get(rt.new_string('before_widget')), var_widget_id.clone(), rt.new_string((var_classname_).str()).clone()]))
	var_params = rt.call_function('apply_filters', [rt.new_string('dynamic_sidebar_params'), var_params.clone()])
	var_callback = var_wp_registered_widgets.array_get(var_widget_id).array_get(rt.new_string('callback'))
...
fn wp_render_widget_control(var_id rt.PhpVal) rt.PhpVal {
	...
	var_callback = var_wp_registered_widget_controls.array_get(var_id).array_get(rt.new_string('callback'))
	var_params = var_wp_registered_widget_controls.array_get(var_id).array_get(rt.new_string('params'))
	rt.call_function('ob_start', []rt.PhpVal{})
	if rt.is_true(rt.call_function('is_callable', [var_callback.clone()])) {
		rt.call_function('call_user_func_array', [var_callback.clone(), var_params.clone()])
	}
...
fn wp_check_widget_editor_deps() {
	...
	if rt.is_true(rt.call_method(var_wp_scripts, 'query', [rt.new_string('wp-edit-widgets'), rt.new_string('enqueued')])) || rt.is_true(rt.call_method(var_wp_scripts, 'query', [rt.new_string('wp-customize-widgets'), rt.new_string('enqueued')])) {
...
fn _wp_block_theme_register_classic_sidebars() {
	...
	mut iter_36 := var_classic_sidebars.iterator()
	for {
		item_36 := iter_36.next() or { break }
		mut var_sidebar_shadow := item_36.val
		var_wp_registered_sidebars.array_set(var_sidebar_shadow.array_get(rt.new_string('id')), var_sidebar_shadow.clone())
	}
}
