...
fn wp_sitemaps_get_server() rt.PhpVal {
	mut var_wp_sitemaps := rt.new_null()
	if !rt.is_true(var_wp_sitemaps) {
		var_wp_sitemaps = create_wp_sitemaps()
...
fn wp_get_sitemap_providers() rt.PhpVal {
	mut var_sitemaps := rt.new_null()
	var_sitemaps = wp_sitemaps_get_server()
...
fn get_sitemap_url(var_name rt.PhpVal, subtype_name string, page i64) bool {
	...
	var_provider = rt.call_method(rt.get_property(var_sitemaps, 'registry'), 'get_provider', [
		var_name.clone(),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_provider)))) {
		return false
	}
	if var_subtype_name.len > 0 && var_subtype_name != '0'
	...
	var_page = (rt.call_function('absint', [rt.new_int(var_page)])).to_i64()
	if 0 >= var_page {
		var_page = 1
	}
	return (rt.call_method(var_provider, 'get_sitemap_url', [
		rt.new_string(subtype_name), rt.new_int(var_page)])).to_bool()
}
...
fn create_wp_sitemaps(_args ...rt.PhpVal) &Class_WP_Sitemaps {
