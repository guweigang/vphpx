...
fn get_bookmark(var_bookmark rt.PhpVal, var_output rt.PhpVal, filter string) rt.PhpVal {
	...
	if !rt.is_true(var_bookmark) {
		if var_GLOBALS.array_isset(rt.new_string('link')) {
			var__bookmark = var_GLOBALS.array_get(rt.new_string('link'))
		} else {
		...
		if var_GLOBALS.array_isset(rt.new_string('link'))
			&& rt.is_true(rt.identical(rt.get_property(var_GLOBALS.array_get(rt.new_string('link')), 'link_id'), var_bookmark)) {
			var__bookmark = var_GLOBALS.array_get(rt.new_string('link'))
		} else {
...
fn get_bookmark_field(var_field rt.PhpVal, var_bookmark_arg rt.PhpVal, context string) string {
	...
	if !(var_bookmark.clone().is_object()) {
		return ''
	}
...
fn get_bookmarks(args string) rt.PhpVal {
	...
	var_cache = rt.call_function('wp_cache_get', [rt.new_string('get_bookmarks'),
		rt.new_string('bookmark')])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('rand'), var_parsed_args.array_get(rt.new_string('orderby'))))))
		&& rt.is_true(var_cache) {
		if var_cache.clone().is_array() && var_cache.array_isset(rt.new_string(var_key.str())) {
			var_bookmarks = var_cache.array_get(rt.new_string(var_key.str()))
	...
	if !(!rt.is_true(var_parsed_args.array_get(rt.new_string('include')))) {
		...
		var_inclinks = rt.call_function('wp_parse_id_list', [
			var_parsed_args.array_get(rt.new_string('include')),
		])
		if rt.is_true(rt.new_int(var_inclinks.clone().array_count())) {
			...
			for {
				...
				if !rt.is_true(var_inclusions) {
					var_inclusions = rt.new_string(' AND ( link_id = ' + var_inclink_shadow.str() +
						' ')
				} else {
	...
	if !(!rt.is_true(var_parsed_args.array_get(rt.new_string('exclude')))) {
		var_exlinks = rt.call_function('wp_parse_id_list', [
			var_parsed_args.array_get(rt.new_string('exclude')),
		])
		if rt.is_true(rt.new_int(var_exlinks.clone().array_count())) {
			mut iter_2 := var_exlinks.iterator()
			for {
				item_2 := iter_2.next() or { break }
				mut var_exlink_shadow := item_2.val
				if !rt.is_true(var_exclusions) {
					var_exclusions = rt.new_string(' AND ( link_id <> ' + var_exlink_shadow.str() +
						' ')
				} else {
					var_exclusions = rt.concat(var_exclusions, rt.new_string(' AND link_id <> ' +
						var_exlink_shadow.str() + ' '))
				}
			}
		}
	}
	...
	if !(!rt.is_true(var_parsed_args.array_get(rt.new_string('category_name')))) {
		var_parsed_args.array_set('category', rt.call_function('get_term_by', [
			rt.new_string('name'),
			var_parsed_args.array_get(rt.new_string('category_name')),
			rt.new_string('link_category'),
		]))
		if rt.is_true(var_parsed_args.array_get(rt.new_string('category'))) {
			var_parsed_args.array_set('category', rt.get_property(var_parsed_args.array_get(rt.new_string('category')),
				'term_id'))
		} else {
	...
	if !(!rt.is_true(var_parsed_args.array_get(rt.new_string('search')))) {
		var_like = rt.new_string('%' +
			(rt.call_method(var_wpdb, 'esc_like', [var_parsed_args.array_get(rt.new_string('search'))])).str() +
			'%')
	...
	if !(!rt.is_true(var_parsed_args.array_get(rt.new_string('category')))) {
		var_incategories = rt.call_function('wp_parse_id_list', [
			var_parsed_args.array_get(rt.new_string('category')),
		])
		if rt.is_true(rt.new_int(var_incategories.clone().array_count())) {
			mut iter_3 := var_incategories.iterator()
			for {
				item_3 := iter_3.next() or { break }
				mut var_incat_shadow := item_3.val
	...
	var_get_updated = if rt.is_true(var_parsed_args.array_get(rt.new_string('show_updated'))) {
		', UNIX_TIMESTAMP(link_updated) AS link_updated_f '
	} else {
		''
	}
	var_orderby = var_parsed_args.array_get(rt.new_string('orderby')).to_string().to_lower()
	var_length = ''
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('length'))) {
		...
		mut iter_4 := rt.call_function('explode', [rt.new_string(','),
			rt.new_string(var_orderby.str()).clone()]).iterator()
		for {
			item_4 := iter_4.next() or { break }
			mut var_ordparam_shadow := item_4.val
			var_ordparam_shadow =
	...
	if rt.is_true(rt.new_bool('' != var_order))
	...
	var_visible = ''
	if rt.is_true(var_parsed_args.array_get(rt.new_string('hide_invisible'))) {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(-1,
		var_parsed_args.array_get(rt.new_string('limit'))))))
	{
		var_query = var_query + ' LIMIT ' +
			(rt.call_function('absint', [var_parsed_args.array_get(rt.new_string('limit'))])).str()
	}
...
fn sanitize_bookmark(var_bookmark rt.PhpVal, context string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_bookmark.clone().is_object())) {
		...
		var_do_object = false
		var_link_id = var_bookmark.array_get(rt.new_string('link_id'))
	}
