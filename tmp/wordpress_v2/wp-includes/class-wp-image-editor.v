...
fn (mut this Class_WP_Image_Editor) update_size(var_width rt.PhpVal, var_height rt.PhpVal) bool {
	this.size = rt.create_array([
		rt.ArrayItem{ key: 'width', val: rt.new_int(var_width.to_i64()) },
		rt.ArrayItem{ key: 'height', val: rt.new_int(var_height.to_i64()) },
	])
	return true
}
...
fn (mut this Class_WP_Image_Editor) set_quality(var_quality rt.PhpVal, var_dims rt.PhpVal) bool {
	...
	mut var_mime_type := if !(!rt.is_true(this.output_mime_type)) {
		this.output_mime_type
	} else {
		this.mime_type
	}
	mut var_default_quality := this.get_default_quality(var_mime_type.clone())
	if rt.is_true(rt.identical(rt.new_null(), var_quality_mutated)) {
		var_quality_mutated = rt.call_function('apply_filters', [
			rt.new_string('wp_editor_set_quality'),
			var_default_quality.clone(),
			var_mime_type.clone(),
			if rt.is_true(var_dims) { var_dims } else { this.size },
		])
		if rt.is_true(rt.identical(rt.new_string('image/jpeg'), var_mime_type)) {
			var_quality_mutated = rt.call_function('apply_filters', [
				rt.new_string('jpeg_quality'),
				var_quality_mutated.clone(),
				rt.new_string('image_resize'),
			])
		}
		if rt.is_true(rt.less(var_quality_mutated, rt.new_int(0)))
			|| rt.is_true(rt.greater(var_quality_mutated, rt.new_int(100))) {
			var_quality_mutated = var_default_quality.clone()
		}
	}
	if rt.is_true(rt.identical(rt.new_int(0), var_quality_mutated)) {
		var_quality_mutated = rt.new_int(1)
	}
	if rt.is_true(rt.greater_equal(var_quality_mutated, rt.new_int(1)))
		&& rt.is_true(rt.less_equal(var_quality_mutated, rt.new_int(100))) {
		this.quality = var_quality_mutated.clone()
		return true
	} else {
		return (create_wp_error(rt.new_string('invalid_image_quality'), rt.call_function('__', [
			rt.new_string('Attempted to set image quality outside of the range [1,100].'),
		]))).to_bool()
	}
	return false
}
...
fn (mut this Class_WP_Image_Editor) get_default_quality(var_mime_type rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('image/webp'))) {
		mut var_quality := rt.new_int(86)
	} else {
		var_quality = this.default_quality
	}
	return var_quality.clone()
}
...
fn (mut this Class_WP_Image_Editor) get_output_format(var_filename rt.PhpVal, var_mime_type rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_mime_type_mutated) {
		var_new_ext = rt.new_bool(this.get_extension(var_mime_type_mutated.clone()))
	}
	if rt.is_true(var_filename_mutated) {
		mut var_file_ext := rt.new_string(rt.call_function('pathinfo', [
			var_filename_mutated.clone(), rt.get_constant('PATHINFO_EXTENSION')]).to_string().to_lower())
		mut var_file_mime := rt.new_bool(this.get_mime_type(var_file_ext.clone()))
	} else {
		var_file_ext = rt.new_string(rt.call_function('pathinfo', [this.file,
			rt.get_constant('PATHINFO_EXTENSION')]).to_string().to_lower())
		var_file_mime = this.mime_type
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_mime_type_mutated))))
		|| rt.is_true(rt.identical(var_file_mime, var_mime_type_mutated)) {
		var_mime_type_mutated = var_file_mime.clone()
		var_new_ext = var_file_ext.clone()
	}
	mut var_output_format := rt.call_function('wp_get_image_editor_output_format', [
		var_filename_mutated.clone(),
		var_mime_type_mutated.clone(),
	])
	if var_output_format.array_isset(var_mime_type_mutated)
		&& this.supports_mime_type(var_output_format.array_get(var_mime_type_mutated)) {
		var_mime_type_mutated = var_output_format.array_get(var_mime_type_mutated)
		var_new_ext = rt.new_bool(this.get_extension(var_mime_type_mutated.clone()))
	}
	if !(this.supports_mime_type(var_mime_type_mutated.clone())) {
		var_mime_type_mutated = rt.call_function('apply_filters', [
			rt.new_string('image_editor_default_mime_type'),
			this.default_mime_type,
		])
		var_new_ext = rt.new_bool(this.get_extension(var_mime_type_mutated.clone()))
	}
	if rt.is_true(var_filename_mutated) && rt.is_true(var_new_ext) {
		mut var_dir := rt.call_function('pathinfo', [var_filename_mutated.clone(),
			rt.get_constant('PATHINFO_DIRNAME')])
		mut var_ext := rt.call_function('pathinfo', [var_filename_mutated.clone(),
			rt.get_constant('PATHINFO_EXTENSION')])
		var_filename_mutated = rt.new_string(
			(rt.call_function('trailingslashit', [var_dir.clone()])).str() +
			(rt.call_function('wp_basename', [var_filename_mutated.clone(), rt.new_string('.${var_ext.to_string()}')])).str() +
			'.${var_new_ext.to_string()}')
	}
	if rt.is_true(var_mime_type_mutated)
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_mime_type_mutated, this.mime_type)))) {
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_mime_type_mutated,
			this.output_mime_type))))
		{
			this.output_mime_type = var_mime_type_mutated.clone()
		}
		this.set_quality(rt.new_null(), rt.new_null())
	...
	return rt.create_array([rt.ArrayItem{ key: none, val: var_filename_mutated },
		rt.ArrayItem{ key: none, val: var_new_ext }, rt.ArrayItem{
			key: none
			val: var_mime_type_mutated
		}])
}
...
fn (mut this Class_WP_Image_Editor) generate_filename(var_suffix rt.PhpVal, var_dest_path rt.PhpVal, var_extension rt.PhpVal) string {
	...
	} else if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_suffix_mutated)))) {
	...
	mut var_name := rt.call_function('wp_basename',
		[this.file, rt.new_string('.${var_ext.to_string()}')])
	mut var_new_ext :=
		rt.new_string(if rt.is_true(var_extension) { var_extension } else { var_ext }.to_string().to_lower())
	if !(var_dest_path.clone().is_null()) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_is_stream', [
			var_dest_path.clone(),
		])))))
		{
			mut var__dest_path := rt.call_function('realpath', [
				var_dest_path.clone()])
			if rt.is_true(var__dest_path) {
				var_dir = var__dest_path.clone()
			}
		} else {
	...
	return (rt.call_function('trailingslashit', [var_dir.clone()])).str() +
		'${var_name.to_string()}${var_suffix.to_string()}.${var_new_ext.to_string()}'
}
...
fn (mut this Class_WP_Image_Editor) get_suffix() rt.PhpVal {
	...
	return rt.new_string((rt.concat(rt.concat(this.size.array_get(rt.new_string('width')),
		rt.new_string('x')), this.size.array_get(rt.new_string('height')))).str())
}
...
fn (mut this Class_WP_Image_Editor) maybe_exif_rotate() bool {
	...
	if rt.call_function('is_callable', [rt.new_string('exif_read_data')])
		&& rt.is_true(rt.identical(rt.new_string('image/jpeg'), this.mime_type)) {
		mut var_exif_data := rt.call_function('exif_read_data', [this.file])
		if !(!rt.is_true(var_exif_data.array_get(rt.new_string('Orientation')))) {
			var_orientation =
				rt.new_int((var_exif_data.array_get(rt.new_string('Orientation'))).to_i64())
		}
	}
	var_orientation = rt.call_function('apply_filters', [
		rt.new_string('wp_image_maybe_exif_rotate'),
		var_orientation.clone(),
		this.file,
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_orientation))))
		|| rt.is_true(rt.identical(rt.new_int(1), var_orientation)) {
		return false
	}
...
fn (mut this Class_WP_Image_Editor) make_image(var_filename rt.PhpVal, var_callback rt.PhpVal, var_arguments rt.PhpVal) bool {
	mut var_filename_mutated := var_filename
	mut var_stream := rt.call_function('wp_is_stream', [var_filename_mutated.clone()])
	...
	mut var_result := rt.call_function('call_user_func_array', [
		var_callback.clone(), var_arguments.clone()])
	if rt.is_true(var_result) && rt.is_true(var_stream) {
		mut var_contents := rt.call_function('ob_get_contents', []rt.PhpVal{})
		mut var_fp := rt.call_function('fopen', [var_filename_mutated.clone(),
			rt.new_string('w')])
		if rt.is_true(rt.new_bool(!(rt.is_true(var_fp)))) {
			rt.call_function('ob_end_clean', []rt.PhpVal{})
			return false
		}
		rt.call_function('fwrite', [var_fp.clone(), var_contents.clone()])
...
fn Class_WP_Image_Editor.get_mime_type(var_extension rt.PhpVal) bool {
	...
	mut var_extensions := rt.func_array_keys(var_mime_types.clone())
	mut iter_1 := var_extensions.iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var__extension := item_1.val
		if rt.is_true(rt.call_function('preg_match', [
			rt.new_string('/${var_extension.to_string()}/i'),
			var__extension.clone(),
		]))
		{
...
fn Class_WP_Image_Editor.get_extension(var_mime_type rt.PhpVal) bool {
	...
	return (rt.call_function('wp_get_default_extension_for_mime_type', [
		var_mime_type_mutated.clone()])).to_bool()
}
...
fn (mut this Class_WP_Image_Editor) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'crop' {
			...
			this.crop(dispatch_arg_0, dispatch_arg_1, dispatch_arg_2, dispatch_arg_3,
...
fn (mut this Class_WP_Image_Editor) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'output_mime_type' {
			this.output_mime_type = val
			return true
		}
		'default_mime_type' {
			this.default_mime_type = val
			return true
		}
		'quality' {
			this.quality = val
