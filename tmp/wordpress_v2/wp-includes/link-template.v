...
fn wp_force_plain_post_permalink(var_post_arg rt.PhpVal, var_sample_arg rt.PhpVal) bool {
	...
	if rt.is_true(rt.identical(rt.new_null(), var_sample)) && var_post.clone().is_object()
	...
	if (rt.is_true(rt.call_function('is_post_status_viewable', [var_post_status_obj.clone()]))
		|| (rt.is_true(rt.get_property(var_post_status_obj, 'private'))
		&& rt.is_true(rt.call_function('current_user_can', [rt.new_string('read_post'), rt.get_property(var_post, 'ID')]))))
		|| (rt.is_true(rt.get_property(var_post_status_obj, 'protected')) && rt.is_true(var_sample)) {
		return false
	}
	return true
}
...
fn get_permalink(post i64, leavename bool) bool {
	...
	if rt.new_int(var_post).is_object()
	...
	if rt.is_true(var_permalink) && !(wp_force_plain_post_permalink(var_post)) {
			...
			rt.new_string('%category%')]))
		{
			...
			if rt.is_true(var_cats) {
				...
				var_category_object = rt.call_function('apply_filters', [
					...
					var_cats.array_get(rt.new_int(0)),
					var_cats.clone(),
					rt.new_int(var_post),
				])
		...
		var_rewritereplace = [var_date.array_get(rt.new_int(0)),
			var_date.array_get(rt.new_int(1)), var_date.array_get(rt.new_int(2)),
			var_date.array_get(rt.new_int(3)), var_date.array_get(rt.new_int(4)),
			var_date.array_get(rt.new_int(5)), rt.get_property(rt.new_int(var_post), 'post_name'),
...
fn get_post_permalink(post i64, leavename bool, sample bool) bool {
	...
	if !(!rt.is_true(var_post_link)) && !var_force_plain_link || var_sample {
		...
		if rt.is_true(rt.get_property(var_post_type, 'query_var'))
...
fn get_attachment_link(var_post_arg rt.PhpVal, leavename bool) rt.PhpVal {
	...
	if rt.is_true(var_parent_id)
		&& rt.is_true(rt.identical(rt.get_property(var_post, 'post_parent'), rt.get_property(var_post, 'ID')))
	...
	if var_force_plain_link || !var_parent_valid {
		var_link = rt.new_bool(false)
	} else if rt.is_true(rt.call_method(var_wp_rewrite, 'using_permalinks', []rt.PhpVal{}))
		...
		if rt.get_property(var_post, 'post_name').is_long()
...
fn get_term_feed_link(var_term_arg rt.PhpVal, taxonomy string, feed string) bool {
	...
	if !rt.is_true(var_term) || rt.is_true(rt.call_function('is_wp_error', [var_term.clone()])) {
		return false
	}
...
fn get_post_type_archive_link(var_post_type rt.PhpVal) bool {
	...
	if rt.is_true(rt.call_function('get_option', [rt.new_string('permalink_structure')]))
		&& rt.get_property(var_post_type_obj, 'rewrite').is_array() {
		var_struct = if rt.is_true(rt.identical(rt.new_bool(true), rt.get_property(var_post_type_obj,
			'has_archive')))
		{
			rt.get_property(var_post_type_obj, 'rewrite').array_get(rt.new_string('slug'))
		} else {
			rt.get_property(var_post_type_obj, 'has_archive')
		}
		if rt.is_true(rt.get_property(var_post_type_obj, 'rewrite').array_get(rt.new_string('with_front'))) {
...
fn get_post_type_archive_feed_link(var_post_type rt.PhpVal, feed string) bool {
	...
	if rt.is_true(rt.call_function('get_option', [rt.new_string('permalink_structure')]))
		&& rt.get_property(var_post_type_obj, 'rewrite').is_array()
		&& rt.is_true(rt.get_property(var_post_type_obj, 'rewrite').array_get(rt.new_string('feeds'))) {
		var_link = rt.call_function('trailingslashit', [var_link.clone()])
...
fn get_edit_post_link(post i64, context string) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_string('wp_template'), rt.get_property(rt.new_int(var_post), 'post_type')))
...
fn get_delete_post_link(post i64, deprecated string, force_delete bool) rt.PhpVal {
	...
	var_action = if var_force_delete
...
fn get_adjacent_post(in_same_term bool, excluded_terms string, previous bool, taxonomy string) rt.PhpVal {
	...
	if !(var_excluded_terms == '') && !(rt.new_string(var_excluded_terms.str()).is_array()) {
...
fn get_adjacent_post_rel_link(title string, in_same_term bool, excluded_terms string, previous bool, taxonomy string) rt.PhpVal {
	...
	mut var_adjacent := ''
	var_post = rt.call_function('get_post', []rt.PhpVal{})
	if var_previous && rt.is_true(rt.call_function('is_attachment', []rt.PhpVal{}))
		&& rt.is_true(var_post) {
...
fn get_boundary_post(in_same_term bool, excluded_terms string, start bool, taxonomy string) rt.PhpVal {
	...
	if !(rt.new_string(var_excluded_terms.str()).is_array()) {
		if !(var_excluded_terms == '') {
			var_excluded_terms = (rt.call_function('explode', [
				rt.new_string(','), rt.new_string(var_excluded_terms.str())])).str()
		} else {
	...
	if var_in_same_term || !(var_excluded_terms == '') {
		...
		if !(var_excluded_terms == '') {
			...
			mut iter_2 := rt.new_string(var_excluded_terms.str()).iterator()
			for {
				item_2 := iter_2.next() or { break }
				mut var_excluded_term_shadow := item_2.val
...
fn get_pagenum_link(pagenum i64, escape bool) rt.PhpVal {
	...
	var_home_root = if !(var_home_root.array_get(rt.new_string('path'))).is_null() {
		var_home_root.array_get(rt.new_string('path'))
	} else {
		rt.new_string('')
	}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_wp_rewrite, 'using_permalinks', []rt.PhpVal{})))))
		...
		if !(!rt.is_true(var_qs_match[0])) {
			var_query_string = var_qs_match[0]
...
fn get_next_posts_page_link(max_page i64) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_single', []rt.PhpVal{}))))) {
		...
		if !(var_max_page != 0) || rt.is_true(rt.greater_equal(rt.new_int(max_page), var_next_page)) {
...
fn get_posts_nav_link(var_args_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_singular', []rt.PhpVal{}))))) {
		...
		if rt.is_true(rt.less(var_paged, rt.new_int(2)))
		...
		if rt.is_true(rt.greater(var_max_num_pages, rt.new_int(1))) {
			var_return = get_previous_posts_link(var_args.array_get(rt.new_string('prelabel')))
			var_return = rt.concat(var_return, rt.call_function('preg_replace', [
				rt.new_string('/&([^#])(?![a-z]{1,8};)/i'),
				rt.new_string('&#038;$1'),
				var_args.array_get(rt.new_string('sep')),
			]))
			var_return = rt.concat(var_return,
				get_next_posts_link(var_args.array_get(rt.new_string('nxtlabel')), 0))
		}
	}
...
fn get_the_post_navigation(var_args_arg rt.PhpVal) rt.PhpVal {
	...
	if !(!rt.is_true(var_args.array_get(rt.new_string('screen_reader_text'))))
		&& !rt.is_true(var_args.array_get(rt.new_string('aria_label'))) {
		var_args.array_set('aria_label', var_args.array_get(rt.new_string('screen_reader_text')))
	}
	var_args = rt.call_function('wp_parse_args', [var_args.clone(),
	...
	var_previous = get_previous_post_link('<div class="nav-previous">%link</div>',
		var_args.array_get(rt.new_string('prev_text')),
		var_args.array_get(rt.new_string('in_same_term')),
		var_args.array_get(rt.new_string('excluded_terms')),
		var_args.array_get(rt.new_string('taxonomy')))
	var_next = get_next_post_link('<div class="nav-next">%link</div>',
		var_args.array_get(rt.new_string('next_text')),
		var_args.array_get(rt.new_string('in_same_term')),
		var_args.array_get(rt.new_string('excluded_terms')),
		var_args.array_get(rt.new_string('taxonomy')))
	if rt.is_true(var_previous) || rt.is_true(var_next) {
		var_navigation = _navigation_markup(rt.new_string(var_previous.str() + var_next.str()),
			var_args.array_get(rt.new_string('class')),
			var_args.array_get(rt.new_string('screen_reader_text')),
			var_args.array_get(rt.new_string('aria_label')))
	}
...
fn get_the_posts_navigation(var_args_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.greater(rt.get_property(var_wp_query, 'max_num_pages'), rt.new_int(1))) {
		...
		var_next_link = get_previous_posts_link(var_args.array_get(rt.new_string('next_text')))
		var_prev_link = get_next_posts_link(var_args.array_get(rt.new_string('prev_text')), 0)
		...
		var_navigation = _navigation_markup(var_navigation.clone(),
			var_args.array_get(rt.new_string('class')),
			var_args.array_get(rt.new_string('screen_reader_text')),
			var_args.array_get(rt.new_string('aria_label')))
	}
...
fn get_the_posts_pagination(var_args_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.greater(rt.get_property(var_wp_query, 'max_num_pages'), rt.new_int(1))) {
		...
		if rt.is_true(var_links) {
			var_navigation = _navigation_markup(var_links.clone(),
				var_args.array_get(rt.new_string('class')),
				var_args.array_get(rt.new_string('screen_reader_text')),
				var_args.array_get(rt.new_string('aria_label')))
		}
...
fn paginate_comments_links(var_args_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_args.array_get(rt.new_string('echo')))
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('array'), var_args.array_get(rt.new_string('type')))))) {
...
fn get_the_comments_navigation(var_args_arg rt.PhpVal) rt.PhpVal {
		...
		rt.new_int(1)))
	{
		...
		var_prev_link = get_previous_comments_link(var_args.array_get(rt.new_string('prev_text')),
			rt.new_null())
		var_next_link = get_next_comments_link(var_args.array_get(rt.new_string('next_text')), 0,
			rt.new_null())
		if rt.is_true(var_prev_link) {
			var_navigation = rt.concat(var_navigation, rt.new_string('<div class="nav-previous">' +
				var_prev_link.str() + '</div>'))
...
fn get_home_url(var_blog_id rt.PhpVal, path string, var_scheme_arg rt.PhpVal) rt.PhpVal {
	...
	if var_path.len > 0 && var_path != '0' && rt.new_string(path).is_string() {
...
fn plugins_url(path string, plugin string) rt.PhpVal {
	...
	if !(var_plugin == '') && rt.new_string(var_plugin.str()).is_string() {
	...
	if var_path.len > 0 && var_path != '0' && rt.new_string(var_path.str()).is_string() {
...
fn set_url_scheme(var_url_arg rt.PhpVal, var_scheme_arg rt.PhpVal) rt.PhpVal {
	...
	} else if rt.is_true(rt.identical(rt.new_string('admin'), var_scheme))
	...
	} else if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('http'), var_scheme))))
	...
	if rt.is_true(rt.identical(rt.new_string('relative'), var_scheme)) {
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_url))))
			&& rt.is_true(rt.identical(rt.new_string('/'), var_url.array_get(rt.new_int(0)))) {
...
fn get_dashboard_url(user_id i64, path string, scheme string) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{}))
		...
		if rt.is_true(var_current_blog)
			&& rt.is_true(rt.call_function('user_can', [rt.new_int(var_user_id), rt.new_string('manage_network')]))
...
fn wp_get_shortlink(id i64, context string, allow_slugs bool) rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_string('query'), rt.new_string(context)))
	...
	if !(!rt.is_true(var_post_id)) {
		...
		} else if rt.is_true(var_post_type) && rt.is_true(rt.get_property(var_post_type, 'public')) {
...
fn get_avatar_url(var_id_or_email rt.PhpVal, var_args_arg rt.PhpVal) rt.PhpVal {
	...
	return var_args.array_get(rt.new_string('url'))
}
...
fn get_avatar_data(var_id_or_email_arg rt.PhpVal, var_args_arg rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_args.array_get(rt.new_string('size')).is_long()
		|| var_args.array_get(rt.new_string('size')).is_double()))
	{
		var_args.array_set('size', rt.call_function('absint', [
			var_args.array_get(rt.new_string('size')),
		]))
		if rt.is_true(rt.new_bool(!(rt.is_true(var_args.array_get(rt.new_string('size')))))) {
			var_args.array_set('size', 96)
		}
	} else {
		var_args.array_set('size', 96)
	}
	if rt.is_true(rt.new_bool(var_args.array_get(rt.new_string('height')).is_long()
		|| var_args.array_get(rt.new_string('height')).is_double()))
	{
		var_args.array_set('height', rt.call_function('absint', [
			var_args.array_get(rt.new_string('height')),
		]))
		if rt.is_true(rt.new_bool(!(rt.is_true(var_args.array_get(rt.new_string('height')))))) {
			var_args.array_set('height', var_args.array_get(rt.new_string('size')))
		}
	} else {
		var_args.array_set('height', var_args.array_get(rt.new_string('size')))
	}
	if rt.is_true(rt.new_bool(var_args.array_get(rt.new_string('width')).is_long()
		|| var_args.array_get(rt.new_string('width')).is_double()))
	{
		var_args.array_set('width', rt.call_function('absint', [
			var_args.array_get(rt.new_string('width')),
		]))
		if rt.is_true(rt.new_bool(!(rt.is_true(var_args.array_get(rt.new_string('width')))))) {
			var_args.array_set('width', var_args.array_get(rt.new_string('size')))
		}
	} else {
		var_args.array_set('width', var_args.array_get(rt.new_string('size')))
	}
	if !rt.is_true(var_args.array_get(rt.new_string('default'))) {
		var_args.array_set('default', rt.call_function('get_option', [
			rt.new_string('avatar_default'),
			rt.new_string('mystery'),
		]))
	}
	mut switch_val_2 := var_args.array_get(rt.new_string('default'))
	...
	var_args.array_set('force_default',
		(var_args.array_get(rt.new_string('force_default'))).to_bool())
	var_args.array_set('rating', var_args.array_get(rt.new_string('rating')).to_string().to_lower())
		...
		|| var_id_or_email.clone().is_double()))
	{
			...
			rt.new_string('@sha256.gravatar.com')]))
		{
			...
			var_email_hash = list_tmp_1.array_get(0)
			...
			var_id_or_email.clone(), rt.new_string('@md5.gravatar.com')]))
		{
			mut list_tmp_2 := rt.call_function('explode', [rt.new_string('@'),
				var_id_or_email.clone()])
			var_email_hash = list_tmp_2.array_get(0)
		} else {
	...
	var_url_args = {
		's': var_args.array_get(rt.new_string('size'))
		'd': var_args.array_get(rt.new_string('default'))
		'f': if rt.is_true(var_args.array_get(rt.new_string('force_default'))) {
			rt.new_string('y')
		} else {
			rt.new_bool(false)
		}
		'r': var_args.array_get(rt.new_string('rating'))
	}
	if rt.is_true(rt.identical(rt.new_string('initials'),
		var_args.array_get(rt.new_string('default'))))
	{
		...
		} else if var_id_or_email.clone().is_string()
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_name)))) {
			if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_name.clone(), rt.new_string(' ')])))))
...
fn get_the_privacy_policy_link(before string, after string) string {
	...
	if rt.is_true(var_privacy_policy_url) && rt.is_true(var_page_title) {
