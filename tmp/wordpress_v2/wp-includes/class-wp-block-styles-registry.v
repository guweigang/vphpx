...
fn init_static_wp_block_styles_registry() {
	rt.init_static_prop('WP_Block_Styles_Registry', 'instance', rt.new_null())
}
...
fn (mut this Class_WP_Block_Styles_Registry) register(var_block_name rt.PhpVal, var_style_properties rt.PhpVal) bool {
	...
	if !(var_block_name.clone().is_string()) && !(var_block_name.clone().is_array()) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('Block name must be a string or array.'),
			]),
			rt.new_string('6.6.0')])
		return false
	}
	if !(var_style_properties_mutated.array_isset(rt.new_string('name')))
		|| !(var_style_properties_mutated.array_get(rt.new_string('name')).is_string()) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [rt.new_string('Block style name must be a string.')]),
			rt.new_string('5.3.0')])
		return false
	}
	if rt.is_true(rt.call_function('str_contains', [var_style_properties_mutated.array_get(rt.new_string('name')),
		rt.new_string(' ')]))
	{
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('Block style name must not contain any spaces.'),
			]),
			rt.new_string('5.9.0')])
		return false
	}
	mut var_block_style_name := var_style_properties_mutated.array_get(rt.new_string('name'))
	mut var_block_names := if var_block_name.clone().is_string() { rt.create_array([
			rt.ArrayItem{ key: none, val: var_block_name },
		]) } else { var_block_name }
	if !rt.is_true(var_style_properties_mutated.array_get(rt.new_string('label'))) {
		var_style_properties_mutated.array_set('label', var_block_style_name.clone())
	}
	...
	for {
		...
		this.registered_block_styles.array_get_mut(var_name).array_set(var_block_style_name,
			var_style_properties_mutated.clone())
	}
	return true
}
...
fn (mut this Class_WP_Block_Styles_Registry) unregister(var_block_name rt.PhpVal, var_block_style_name rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(this.is_registered(var_block_name.clone(),
		var_block_style_name_mutated.clone())))))
	{
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('Block "%1$s" does not contain a style named "%2$s".'),
				]),
...
fn (mut this Class_WP_Block_Styles_Registry) get_registered_styles_for_block(var_block_name rt.PhpVal) rt.PhpVal {
	return if !(this.registered_block_styles.array_get(var_block_name)).is_null() {
		this.registered_block_styles.array_get(var_block_name)
	} else {
		rt.new_array()
	}
}
...
fn (mut this Class_WP_Block_Styles_Registry) is_registered(var_block_name rt.PhpVal, var_block_style_name rt.PhpVal) rt.PhpVal {
	mut var_block_style_name_mutated := var_block_style_name
	return rt.new_bool(!var_block_name.is_null() && !var_block_style_name_mutated.is_null()
		&& this.registered_block_styles.array_get(var_block_name).array_isset(var_block_style_name_mutated))
}

fn Class_WP_Block_Styles_Registry.get_instance() rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_null(), rt.get_static_prop('WP_Block_Styles_Registry',
		'instance')))
	{
		rt.set_static_prop('WP_Block_Styles_Registry', 'instance', rt.new_object('WP_Block_Styles_Registry',
			[]string{}, create_wp_block_styles_registry()))
	}
	return rt.get_static_prop('WP_Block_Styles_Registry', 'instance')
}
...
fn (mut this Class_WP_Block_Styles_Registry) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'registered_block_styles' {
			this.registered_block_styles = val
			return true
		}
		else {
