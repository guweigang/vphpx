...
fn Class_WP_Site.get_instance(var_site_id rt.PhpVal) bool {
	...
	var_site_id_mutated = rt.new_int(var_site_id_mutated.to_i64())
	if rt.is_true(rt.new_bool(!(rt.is_true(var_site_id_mutated)))) {
		return false
	}
	mut var__site := rt.call_function('wp_cache_get', [var_site_id_mutated.clone(),
		rt.new_string('sites')])
	if rt.is_true(rt.identical(rt.new_bool(false), var__site)) {
		var__site = rt.call_method(var_wpdb, 'get_row', [
			rt.call_method(var_wpdb, 'prepare', [
				rt.concat(rt.concat(rt.new_string('SELECT * FROM '), rt.get_property(var_wpdb,
					'blogs')), rt.new_string(' WHERE blog_id = %d LIMIT 1')),
				var_site_id_mutated.clone(),
			]),
		])
		if !rt.is_true(var__site)
			|| rt.is_true(rt.call_function('is_wp_error', [var__site.clone()])) {
			var__site = rt.new_int(-1)
		}
		rt.call_function('wp_cache_add', [var_site_id_mutated.clone(),
			var__site.clone(), rt.new_string('sites')])
	}
	if rt.is_true(rt.new_bool(var__site.clone().is_long() || var__site.clone().is_double())) {
		return false
	}
	return (create_wp_site(var__site.clone())).to_bool()
}

fn (mut this Class_WP_Site) construct(var_site rt.PhpVal) {
	mut iter_1 := rt.call_function('get_object_vars', [var_site.clone()]).iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_value := item_1.val
		mut var_key := item_1.key
		this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":197,"name":"key"}',
			var_value.clone())
	}
}
...
fn (mut this Class_WP_Site) to_array() rt.PhpVal {
	return rt.call_function('get_object_vars', [
		rt.new_object('WP_Site', []string{}, &this),
	])
}
...
fn (mut this Class_WP_Site) magic_get(var_key rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('id'))) {
		...
		if !(rt.get_property(var_details, '{"nodeType":"Expr_Variable","line":239,"name":"key"}')).is_null() {
			return rt.get_property(var_details,
				'{"nodeType":"Expr_Variable","line":240,"name":"key"}')
		}
	}
	return rt.new_null()
}
...
fn (mut this Class_WP_Site) magic_isset(var_key rt.PhpVal) bool {
	...
	if rt.is_true(rt.equal(switch_val_2, rt.new_string('id')))
		|| rt.is_true(rt.equal(switch_val_2, rt.new_string('network_id'))) {
		return true
	} else if rt.is_true(rt.equal(switch_val_2, rt.new_string('blogname')))
		|| rt.is_true(rt.equal(switch_val_2, rt.new_string('siteurl')))
		|| rt.is_true(rt.equal(switch_val_2, rt.new_string('post_count')))
		|| rt.is_true(rt.equal(switch_val_2, rt.new_string('home'))) {
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('did_action', [
			rt.new_string('ms_loaded'),
		])))))
		{
...
fn (mut this Class_WP_Site) magic_set(var_key rt.PhpVal, var_value rt.PhpVal) {
	...
	if rt.is_true(rt.equal(switch_val_3, rt.new_string('id'))) {
		...
		this.site_id = var_value.str()
	} else {
		this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":304,"name":"key"}',
			var_value.clone())
	}
}

fn (mut this Class_WP_Site) get_details() rt.PhpVal {
	mut var_details := rt.call_function('wp_cache_get',
		[this.blog_id, rt.new_string('site-details')])
	if rt.is_true(rt.identical(rt.new_bool(false), var_details)) {
		...
		for {
			...
			rt.set_property(var_details, '{"nodeType":"Expr_Variable","line":328,"name":"key"}',
				var_value.clone())
		}
		...
		rt.call_function('wp_cache_set', [this.blog_id, var_details.clone(),
	...
	var_details = rt.call_function('apply_filters', [rt.new_string('site_details'),
		var_details.clone()])
