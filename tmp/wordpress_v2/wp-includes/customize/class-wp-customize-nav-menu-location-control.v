...
fn (mut this Class_WP_Customize_Nav_Menu_Location_Control) render_content() {
	...
	mut var_value_hidden_class := rt.new_string('')
	mut var_no_value_hidden_class := rt.new_string('')
	if rt.is_true(this.value()) {
		var_value_hidden_class = rt.new_string(' hidden')
	} else {
		var_no_value_hidden_class = rt.new_string(' hidden')
	}
