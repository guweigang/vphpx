...
fn (mut this Class_WP_Customize_Theme_Control) content_template() {
	...
	rt.echo_val(rt.call_function('esc_attr', [var_details_label.clone()]))
	// unsupported statement: Stmt_InlineHTML
	...
	if rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{})) {
		...
		rt.call_function('printf', [
			...
			rt.new_string('<button class="button-link update-theme" type="button">' +
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_customize_label.clone()]))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	var_customizer_not_supported_message = rt.concat(var_customizer_not_supported_message, rt.new_string(
		' ' +(rt.call_function('sprintf', [rt.call_function('__', [rt.new_string('However, you can still <a href="%s">activate this theme</a>, and use the Site Editor to customize it.')]), rt.new_string('{{{ data.theme.actions.activate }}}')])).str()))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('wp_admin_notice', [var_customizer_not_supported_message.clone(),
		rt.create_array([rt.ArrayItem{ key: 'type', val: 'error' },
			rt.ArrayItem{ key: 'additional_classes', val: rt.create_array([
				rt.ArrayItem{ key: none, val: 'notice-alt' },
			]) }])])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_preview_label.clone()]))
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_attr', [var_install_label.clone()]))
	// unsupported statement: Stmt_InlineHTML
...
fn create_wp_customize_theme_control(_args ...rt.PhpVal) &Class_WP_Customize_Theme_Control {
