...
fn (mut this Class_WP_Customize_Media_Control) to_json() {
	...
	], &this), 'setting').is_object()))
	{
		...
		], &this), 'setting'), 'default'))
		{
			...
			]).array_get(rt.new_string('ext'))
			mut var_ext_types := rt.call_function('wp_get_ext_types', []rt.PhpVal{})
			mut var_type := rt.new_string((if var_ext_types.array_isset(rt.new_string('image'))
				&& rt.is_true(rt.call_function('in_array', [var_ext.clone(), var_ext_types.array_get(rt.new_string('image')), rt.new_bool(true)])) {
				'image'
			} else {
				'document'
			}).str())
			mut var_default_attachment := {
				...
				'icon':  rt.call_function('wp_mime_type_icon', [
					var_type.clone(), rt.new_string('.svg')])
			...
			rt.get_property(rt.new_object('WP_Customize_Media_Control', [
				'WP_Customize_Control',
			], &this), 'json').array_set('defaultAttachment', var_default_attachment.clone())
		}
		if rt.is_true(var_value)
			...
			rt.get_property(rt.new_object('WP_Customize_Media_Control', [
				...
				'WP_Customize_Control',
			], &this), 'json').array_get(rt.new_string('defaultAttachment')))
...
fn (mut this Class_WP_Customize_Media_Control) get_default_button_labels() rt.PhpVal {
...
fn (mut this Class_WP_Customize_Media_Control) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'get_default_button_labels' {
			return this.get_default_button_labels()
		}
		else {
			return none
		}
