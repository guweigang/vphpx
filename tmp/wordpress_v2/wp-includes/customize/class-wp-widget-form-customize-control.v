...
fn (mut this Class_WP_Widget_Form_Customize_Control) to_json() {
	...
	if !(var_widget.array_get(rt.new_string('params')).array_isset(rt.new_int(0))) {
		var_widget.array_get_mut('params').array_set(0, rt.new_array())
	}
	mut var_args := rt.create_array([
		rt.ArrayItem{ key: 'widget_id', val: var_widget.array_get(rt.new_string('id')) },
		rt.ArrayItem{ key: 'widget_name', val: var_widget.array_get(rt.new_string('name')) },
	])
	var_args = rt.call_function('wp_list_widget_controls_dynamic_sidebar', [
		rt.create_array([rt.ArrayItem{ key: 0, val: var_args },
			rt.ArrayItem{
				key: 1
				val: var_widget.array_get(rt.new_string('params')).array_get(rt.new_int(0))
			}]),
	])
	...
	rt.get_property(rt.new_object('WP_Widget_Form_Customize_Control', [
		'WP_Customize_Control',
	], &this), 'json').array_set('widget_control',
		var_widget_control_parts.array_get(rt.new_string('control')))
	rt.get_property(rt.new_object('WP_Widget_Form_Customize_Control', [
		'WP_Customize_Control',
	], &this), 'json').array_set('widget_content',
		var_widget_control_parts.array_get(rt.new_string('content')))
}
...
fn create_wp_widget_form_customize_control(_args ...rt.PhpVal) &Class_WP_Widget_Form_Customize_Control {
