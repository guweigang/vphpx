...
fn (mut this Class_WP_Customize_Selective_Refresh) add_partial(var_id rt.PhpVal, var_args rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.instance_of(var_id, 'WP_Customize_Partial'))) {
		...
		mut var_class := rt.new_string('WP_Customize_Partial')
	...
	this.partials.array_set(rt.get_property(var_partial, 'id'), var_partial.clone())
	return var_partial.clone()
}
...
fn (mut this Class_WP_Customize_Selective_Refresh) export_preview_data() {
	...
	rt.call_function('wp_print_inline_script_tag', [
		rt.new_string(
			(rt.call_function('sprintf', [rt.new_string('var _customizePartialRefreshExports = %s;'), rt.call_function('wp_json_encode', [rt.create_array_from_native_map(var_exports), rt.bitwise_or(rt.get_constant('JSON_HEX_TAG'), rt.get_constant('JSON_UNESCAPED_SLASHES'))])])).str() +
...
fn (mut this Class_WP_Customize_Selective_Refresh) add_dynamic_partials(var_partial_ids rt.PhpVal) rt.PhpVal {
	...
	mut iter_2 := var_partial_ids.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_partial_id := item_2.val
		mut var_partial := this.get_partial(var_partial_id.clone())
		if rt.is_true(var_partial) {
			continue
		}
		mut var_partial_args := rt.new_bool(false)
		mut var_partial_class := rt.new_string('WP_Customize_Partial')
		var_partial_args = rt.call_function('apply_filters', [
			rt.new_string('customize_dynamic_partial_args'),
			var_partial_args.clone(),
			var_partial_id.clone(),
		])
		...
		this.add_partial(var_partial.clone(), rt.new_null())
		var_new_partials << var_partial.clone()
	}
	return var_new_partials.clone()
}

fn (mut this Class_WP_Customize_Selective_Refresh) is_render_partials_request() bool {
	return !(!rt.is_true(rt.get_superglobal('_POST').array_get(rt.new_string(Class_WP_Customize_Selective_Refresh.render_query_var()))))
}
...
fn (mut this Class_WP_Customize_Selective_Refresh) handle_render_partials_request() {
	...
	mut var_partials := rt.call_function('json_decode', [
		rt.call_function('wp_unslash',
			[rt.get_superglobal('_POST').array_get(rt.new_string('partials'))]),
		rt.new_bool(true),
	])
	if !(var_partials.clone().is_array()) {
		rt.call_function('wp_send_json_error', [rt.new_string('malformed_partials')])
	}
	this.add_dynamic_partials(rt.func_array_keys(var_partials.clone()))
	...
	mut iter_3 := var_partials.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_container_contexts := item_3.val
		mut var_partial_id := item_3.key
		this.current_partial_id = var_partial_id.clone()
		if !(var_container_contexts.clone().is_array()) {
			rt.call_function('wp_send_json_error', [
				rt.new_string('malformed_container_contexts'),
			])
		}
		mut var_partial := this.get_partial(var_partial_id.clone())
		if rt.is_true(rt.new_bool(!(rt.is_true(var_partial))))
		...
		if !rt.is_true(var_container_contexts) {
			...
			mut iter_4 := var_container_contexts.iterator()
			for {
				item_4 := iter_4.next() or { break }
				mut var_container_context := item_4.val
				var_contents.array_get_mut(var_partial_id).array_push(rt.call_method(var_partial,
					'render', [var_container_context.clone()]))
			}
		}
	}
	...
	mut var_exported_setting_validities := rt.call_function('array_map', [
		...
		var_setting_validities.clone(),
	])
	var_response.array_set('setting_validities', var_exported_setting_validities.clone())
