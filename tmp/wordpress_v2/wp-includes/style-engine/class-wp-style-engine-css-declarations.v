...
fn (mut this Class_WP_Style_Engine_CSS_Declarations) add_declaration(var_property rt.PhpVal, var_value rt.PhpVal) rt.PhpVal {
	...
	var_property_mutated = this.sanitize_property(var_property_mutated.clone())
	...
	this.declarations.array_set(var_property_mutated, var_value_mutated.clone())
	return rt.new_object('WP_Style_Engine_CSS_Declarations', []string{}, this)
}
...
fn (mut this Class_WP_Style_Engine_CSS_Declarations) add_declarations(var_declarations rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		this.add_declaration(var_property.clone(), var_value.clone())
	}
	return rt.new_object('WP_Style_Engine_CSS_Declarations', []string{}, this)
}
...
fn (mut this Class_WP_Style_Engine_CSS_Declarations) remove_declarations(var_properties rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		this.remove_declaration(var_property.clone())
	}
	return rt.new_object('WP_Style_Engine_CSS_Declarations', []string{}, this)
}
...
fn Class_WP_Style_Engine_CSS_Declarations.filter_declaration(var_property rt.PhpVal, var_value rt.PhpVal, spacer string) string {
	...
	mut var_filtered_value := rt.call_function('wp_strip_all_tags', [
		var_value_mutated.clone(), rt.new_bool(true)])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), var_filtered_value)))) {
		return (rt.call_function('safecss_filter_attr', [
			rt.new_string('${var_property.to_string()}:${var_spacer.to_string()}${var_filtered_value.to_string()}'),
		])).str()
	}
	return ''
}
...
fn (mut this Class_WP_Style_Engine_CSS_Declarations) get_declarations_string(should_prettify bool, indent_count i64) string {
	...
	mut var_declarations_output := rt.new_string('')
	mut var_indent := if var_should_prettify { rt.call_function('str_repeat', [
			rt.new_string('\t'),
			rt.new_int(indent_count),
		]) } else { rt.new_string('') }
	mut var_suffix := rt.new_string((if var_should_prettify { ' ' } else { '' }).str())
	var_suffix = if var_should_prettify && indent_count > 0 {
		rt.new_string('\n')
	} else {
		var_suffix
	}
	mut var_spacer := rt.new_string((if var_should_prettify { ' ' } else { '' }).str())
	mut iter_3 := var_declarations_array.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_value := item_3.val
		mut var_property := item_3.key
		mut var_filtered_declaration := Class_WP_Style_Engine_CSS_Declarations.filter_declaration(var_property.str(),
			var_value.clone(), var_spacer.clone())
		if rt.is_true(var_filtered_declaration) {
			var_declarations_output = rt.concat(var_declarations_output,
				rt.new_string('${var_indent.to_string()}${var_filtered_declaration.to_string()};${var_suffix.to_string()}'))
		}
	}
	return var_declarations_output.clone().to_string().trim_right(' \t\n\r')
}

fn (mut this Class_WP_Style_Engine_CSS_Declarations) sanitize_property(var_property rt.PhpVal) rt.PhpVal {
	mut var_property_mutated := var_property
	return rt.call_function('sanitize_key', [var_property_mutated.clone()])
}
...
fn (mut this Class_WP_Style_Engine_CSS_Declarations) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'filter_declaration' {
			...
			return rt.new_string(Class_WP_Style_Engine_CSS_Declarations.filter_declaration(dispatch_arg_0,
