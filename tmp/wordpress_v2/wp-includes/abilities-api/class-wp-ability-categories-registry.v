...
fn init_static_wp_ability_categories_registry() {
	rt.init_static_prop('WP_Ability_Categories_Registry', 'instance', rt.new_null())
}
...
fn (mut this Class_WP_Ability_Categories_Registry) register(slug string, mut var_args Class_array) rt.PhpVal {
	...
	if this.is_registered(slug) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('Ability category "%s" is already registered.'),
				]),
	...
	])))))
	{
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
				rt.new_string('Ability category slug must contain only lowercase alphanumeric characters and dashes.'),
			]),
			rt.new_string('6.9.0')])
		return rt.new_null()
	}
	var_args_mutated = rt.call_function('apply_filters', [
		rt.new_string('wp_register_ability_category_args'),
		var_args_mutated,
		rt.new_string(slug),
	])
	mut var_category := create_wp_ability_category(rt.new_string(slug), var_args_mutated)
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	...
	end_label_1:
	this.registered_categories.array_set(slug, var_category)
...
fn (mut this Class_WP_Ability_Categories_Registry) unregister(slug string) rt.PhpVal {
	if !(this.is_registered(slug)) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [
					rt.new_string('Ability category "%s" not found.'),
				]),
				rt.call_function('esc_html', [
					rt.new_string(slug),
				]),
	...
	mut var_unregistered_category := this.registered_categories.array_get(rt.new_string(slug))
	this.registered_categories.array_unset(rt.new_string(slug))
	return var_unregistered_category.clone()
}
...
fn (mut this Class_WP_Ability_Categories_Registry) get_registered(slug string) rt.PhpVal {
	...
	return this.registered_categories.array_get(rt.new_string(slug))
}
...
fn Class_WP_Ability_Categories_Registry.get_instance() rt.PhpVal {
	...
	if rt.is_true(rt.identical(rt.new_null(), rt.get_static_prop('WP_Ability_Categories_Registry',
		'instance')))
	{
		rt.set_static_prop('WP_Ability_Categories_Registry', 'instance', rt.new_object('WP_Ability_Categories_Registry',
			[]string{}, create_wp_ability_categories_registry()))
		rt.call_function('do_action', [rt.new_string('wp_abilities_api_categories_init'),
			rt.get_static_prop('WP_Ability_Categories_Registry', 'instance')])
	}
	return rt.get_static_prop('WP_Ability_Categories_Registry', 'instance')
}
...
fn create_wp_ability_category(_args ...rt.PhpVal) &Class_WP_Ability_Category {
