			...
			rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
				rt.call_function('sprintf', [
					...
					rt.new_string('<code>' +
						...
						'</code>'),
					rt.new_string('<code>' +
						(rt.call_function('esc_html', [rt.new_string(this.slug)])).str() + '</code>'),
...
fn (mut this Class_WP_Ability_Category) prepare_properties(mut var_args Class_array) rt.PhpVal {
	if !rt.is_true(var_args.array_get(rt.new_string('label')))
		|| !(var_args.array_get(rt.new_string('label')).is_string()) {
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.call_function('__', [
	...
	if var_args.array_isset(rt.new_string('meta'))
		&& !(var_args.array_get(rt.new_string('meta')).is_array()) {
		rt.throw_exception(rt.new_object('InvalidArgumentException', []string{}, create_invalidargumentexception(rt.call_function('__', [
