...
fn (mut this Class_AtomParser) construct() {
	...
	this.map_attrs_func = rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT },
		rt.ArrayItem{ key: none, val: 'map_attrs' }])
	this.map_xmlns_func = rt.create_array([rt.ArrayItem{ key: none, val: @STRUCT },
		rt.ArrayItem{ key: none, val: 'map_xmlns' }])
}

fn (mut this Class_AtomParser) atomparser() {
	mut iife_temp_0 := Class_AtomParser{}
...
fn Class_AtomParser.map_xmlns(var_p rt.PhpVal, var_n rt.PhpVal) string {
	mut var_xd := rt.new_string('xmlns')
	if 0 < var_n.array_get(rt.new_int(0)).to_string().len {
		var_xd = rt.concat(var_xd, rt.concat(rt.new_string(':'), var_n.array_get(rt.new_int(0))))
	}
	return rt.concat(rt.concat(rt.concat(var_xd, rt.new_string('="')),
		var_n.array_get(rt.new_int(1))), rt.new_string('"'))
}
...
fn (mut this Class_AtomParser) _p(var_msg rt.PhpVal) {
	if rt.is_true(this.debug) {
		fn () {
			print(
				(rt.call_function('str_repeat', [rt.new_string(' '), rt.mul(this.depth, this.indent)])).str() +
				var_msg.str() + '\n')
			return i64(1)
		}()
	}
...
fn (mut this Class_AtomParser) error_handler(var_log_level rt.PhpVal, var_log_text rt.PhpVal, var_error_file rt.PhpVal, var_error_line rt.PhpVal) {
	this.error = var_log_text.clone()
}

fn (mut this Class_AtomParser) parse() bool {
	rt.call_function('set_error_handler', [
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('AtomParser', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'error_handler' },
		]),
	])
	rt.call_function('array_unshift', [this.ns_contexts, rt.new_array()])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('function_exists', [
		rt.new_string('xml_parser_create_ns'),
	])))))
	{
	...
	rt.call_function('xml_set_element_handler', [var_parser.clone(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('AtomParser', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'start_element' },
		]),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('AtomParser', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'end_element' },
		])])
	rt.call_function('xml_parser_set_option', [var_parser.clone(),
		rt.get_constant('XML_OPTION_CASE_FOLDING'), rt.new_int(0)])
	rt.call_function('xml_parser_set_option', [var_parser.clone(),
		rt.get_constant('XML_OPTION_SKIP_WHITE'), rt.new_int(0)])
	rt.call_function('xml_set_character_data_handler', [var_parser.clone(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('AtomParser', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'cdata' },
		])])
	rt.call_function('xml_set_default_handler', [var_parser.clone(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('AtomParser', []string{}, &this) },
			rt.ArrayItem{ key: none, val: '_default' },
		])])
	rt.call_function('xml_set_start_namespace_decl_handler', [
		var_parser.clone(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('AtomParser', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'start_ns' },
		])])
	rt.call_function('xml_set_end_namespace_decl_handler', [var_parser.clone(),
		rt.create_array([
			rt.ArrayItem{ key: none, val: rt.new_object('AtomParser', []string{}, &this) },
			rt.ArrayItem{ key: none, val: 'end_ns' },
		])])
	this.content = ''
	mut var_ret := rt.new_bool(true)
	mut var_fp := rt.call_function('fopen', [this.FILE, rt.new_string('r')])
	mut var_data := rt.call_function('fread', [var_fp.clone(),
		rt.new_int(4096)])
	for rt.is_true(var_data) {
		if rt.is_true(this.debug) {
			this.content = rt.concat(this.content, var_data)
		}
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('xml_parse', [
			var_parser.clone(), var_data.clone(), rt.call_function('feof', [
				var_fp.clone()])])))))
		{
			rt.call_function('trigger_error', [
				rt.call_function('sprintf', [
					rt.new_string(
						(rt.call_function('__', [rt.new_string('XML Error: %1$s at line %2$s')])).str() +
						'\n'),
					rt.call_function('xml_error_string', [
						rt.call_function('xml_get_error_code', [
							var_parser.clone()]),
					]),
...
fn (mut this Class_AtomParser) start_element(var_parser rt.PhpVal, var_name rt.PhpVal, var_attrs rt.PhpVal) {
	...
	mut var_tag := rt.call_function('array_pop', [var_name_parts.clone()])
	...
	if !(!rt.is_true(this.in_content)) {
		...
		if this.is_html || this.is_text {
			rt.call_function('trigger_error', [
				rt.new_string('Invalid content in element found. Content must not be of type text or html if it contains markup.'),
			])
		}
		...
		for {
			...
			mut var_with_prefix := this.ns_to_prefix(var_key.clone(), true)
			var_attrs_prefix.array_set(var_with_prefix.array_get(rt.new_int(1)),
				this.xml_escape(var_value.clone()))
		}
		mut var_attrs_str := rt.call_function('join', [rt.new_string(' '),
			rt.call_function('array_map', [this.map_attrs_func,
				rt.func_array_keys(var_attrs_prefix.clone()),
				rt.call_function('array_values', [var_attrs_prefix.clone()])])])
		if var_attrs_str.clone().to_string().len > 0 {
			var_attrs_str = rt.new_string(' ' + var_attrs_str.str())
		}
		mut var_with_prefix := this.ns_to_prefix(var_name_mutated.clone(), false)
		if !(this.is_declared_content_ns(var_with_prefix.array_get(rt.new_int(0)))) {
			this.content_ns_decls.array_push(var_with_prefix.array_get(rt.new_int(0)))
		}
		mut var_xmlns_str := rt.new_string('')
		if this.content_ns_decls.array_count() > 0 {
			rt.call_function('array_unshift', [this.content_ns_contexts, this.content_ns_decls])
			var_xmlns_str = rt.concat(var_xmlns_str, rt.call_function('join', [
				rt.new_string(' '),
				rt.call_function('array_map', [this.map_xmlns_func,
					rt.func_array_keys(this.content_ns_contexts.array_get(rt.new_int(0))),
					rt.call_function('array_values',
						[this.content_ns_contexts.array_get(rt.new_int(0))])]),
			]))
			if var_xmlns_str.clone().to_string().len > 0 {
				var_xmlns_str = rt.new_string(' ' + var_xmlns_str.str())
			}
		}
		this.in_content.array_push(rt.create_array([
			rt.ArrayItem{ key: none, val: var_tag },
			rt.ArrayItem{ key: none, val: this.depth },
			rt.ArrayItem{
				key: none
				val: '<' +
					(var_with_prefix.array_get(rt.new_int(1))).str() + '${var_xmlns_str.to_string()}${var_attrs_str.to_string()}' + '>'
			},
		]))
	} else {
		if rt.is_true(rt.call_function('in_array', [var_tag.clone(), this.ATOM_CONTENT_ELEMENTS]))
			|| rt.is_true(rt.call_function('in_array', [var_tag.clone(), this.ATOM_SIMPLE_ELEMENTS])) {
			this.in_content = rt.new_array()
			this.is_xhtml = rt.equal(var_attrs.array_get(rt.new_string('type')),
				rt.new_string('xhtml'))
			this.is_html =
				rt.is_true(rt.equal(var_attrs.array_get(rt.new_string('type')), rt.new_string('html')))
				|| rt.is_true(rt.equal(var_attrs.array_get(rt.new_string('type')), rt.new_string('text/html')))
			this.is_text =
				rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.new_string('type'), rt.func_array_keys(var_attrs.clone())])))))
				|| rt.is_true(rt.equal(var_attrs.array_get(rt.new_string('type')), rt.new_string('text')))
			mut var_type := if this.is_xhtml {
				rt.new_string('XHTML')
			} else {
				if this.is_html {
					rt.new_string('HTML')
					...
					if this.is_text {
						...
						var_attrs.array_get(rt.new_string('type'))
					}
				}
			}
			if rt.is_true(rt.call_function('in_array', [rt.new_string('src'),
				rt.func_array_keys(var_attrs.clone())]))
			{
				rt.set_property(this.current,
					'{"nodeType":"Expr_Variable","line":265,"name":"tag"}', var_attrs.clone())
			} else {
				this.in_content.array_push(rt.create_array([
					rt.ArrayItem{ key: none, val: var_tag },
					rt.ArrayItem{ key: none, val: this.depth },
					rt.ArrayItem{ key: none, val: var_type },
				]))
			}
		} else {
			if rt.is_true(rt.equal(var_tag, rt.new_string('link'))) {
				rt.get_property(this.current, 'links').array_push(var_attrs.clone())
			} else {
				if rt.is_true(rt.equal(var_tag, rt.new_string('category'))) {
					rt.get_property(this.current, 'categories').array_push(var_attrs.clone())
				}
			}
		}
	}
...
fn (mut this Class_AtomParser) end_element(var_parser rt.PhpVal, var_name rt.PhpVal) {
	...
	mut var_name_parts := rt.call_function('explode', [rt.new_string(':'),
		var_name_mutated.clone()])
	mut var_tag := rt.call_function('array_pop', [var_name_parts.clone()])
	mut var_ccount := rt.new_int(this.in_content.array_count())
	if !(!rt.is_true(this.in_content)) {
		if rt.is_true(rt.equal(this.in_content.array_get(rt.new_int(0)).array_get(rt.new_int(0)), var_tag))
			&& rt.is_true(rt.equal(this.in_content.array_get(rt.new_int(0)).array_get(rt.new_int(1)), this.depth)) {
			mut var_origtype := this.in_content.array_get(rt.new_int(0)).array_get(rt.new_int(2))
			rt.call_function('array_shift', [this.in_content])
			mut var_newcontent := rt.new_array()
			mut iter_2 := this.in_content.iterator()
			for {
				item_2 := iter_2.next() or { break }
				mut var_c := item_2.val
				if var_c.clone().array_count() == 3 {
					var_newcontent.clone().array_push(var_c.array_get(rt.new_int(2)))
				} else {
					if this.is_xhtml || this.is_text {
						var_newcontent.clone().array_push(this.xml_escape(var_c.clone()))
					} else {
						var_newcontent.clone().array_push(var_c.clone())
					}
				}
			}
			if rt.is_true(rt.call_function('in_array',
				[var_tag.clone(), this.ATOM_CONTENT_ELEMENTS]))
			{
				rt.set_property(this.current,
					'{"nodeType":"Expr_Variable","line":306,"name":"tag"}', rt.create_array([
					rt.ArrayItem{ key: none, val: var_origtype },
					rt.ArrayItem{ key: none, val: rt.call_function('join', [
						rt.new_string(''),
						var_newcontent.clone(),
					]) },
				]))
			} else {
				rt.set_property(this.current,
					'{"nodeType":"Expr_Variable","line":308,"name":"tag"}', rt.call_function('join', [
					rt.new_string(''),
					var_newcontent.clone(),
				]))
			}
			...
			if rt.is_true(rt.equal(this.in_content.array_get(rt.sub(var_ccount, rt.new_int(1))).array_get(rt.new_int(0)), var_tag))
				&& rt.is_true(rt.equal(this.in_content.array_get(rt.sub(var_ccount, rt.new_int(1))).array_get(rt.new_int(1)), this.depth)) {
				this.in_content.array_get_mut(rt.sub(var_ccount, rt.new_int(1))).array_set(2,
					(rt.call_function('substr', [this.in_content.array_get(rt.sub(var_ccount, rt.new_int(1))).array_get(rt.new_int(2)), rt.new_int(0), rt.new_int(-1)])).str() +
					'/>')
			} else {
				mut var_endtag := this.ns_to_prefix(var_name_mutated.clone(), false)
				this.in_content.array_push(rt.create_array([
					rt.ArrayItem{ key: none, val: var_tag },
					rt.ArrayItem{ key: none, val: this.depth },
					rt.ArrayItem{ key: none, val: rt.concat(rt.concat(rt.new_string('</'),
						var_endtag.array_get(rt.new_int(1))), rt.new_string('>')) },
				]))
			}
		}
	}
...
fn (mut this Class_AtomParser) start_ns(var_parser rt.PhpVal, var_prefix rt.PhpVal, var_uri rt.PhpVal) {
	...
	this.ns_decls.array_push(rt.create_array([rt.ArrayItem{ key: none, val: var_prefix },
		rt.ArrayItem{ key: none, val: var_uri }]))
}
...
fn (mut this Class_AtomParser) cdata(var_parser rt.PhpVal, var_data rt.PhpVal) {
	...
	this._p(rt.new_string('data: #' +
		...
		key: none
		val: '\\n'
	}]), rt.new_string(var_data.clone().to_string().trim_space())])).str() +
		'#'))
	if !(!rt.is_true(this.in_content)) {
		this.in_content.array_push(var_data.clone())
	}
}
...
fn (mut this Class_AtomParser) ns_to_prefix(var_qname rt.PhpVal, attr bool) rt.PhpVal {
	mut var_components := rt.call_function('explode', [rt.new_string(':'),
		var_qname.clone()])
	mut var_name := rt.call_function('array_pop', [var_components.clone()])
	if !(!rt.is_true(var_components)) {
		mut var_ns := rt.call_function('join', [rt.new_string(':'),
			var_components.clone()])
		mut iter_3 := this.ns_contexts.iterator()
		for {
			item_3 := iter_3.next() or { break }
			mut var_context := item_3.val
			mut iter_4 := var_context.iterator()
			for {
				item_4 := iter_4.next() or { break }
				mut var_mapping := item_4.val
				if rt.is_true(rt.equal(var_mapping.array_get(rt.new_int(1)), var_ns))
					&& var_mapping.array_get(rt.new_int(0)).to_string().len > 0 {
					return rt.create_array([rt.ArrayItem{ key: none, val: var_mapping },
						rt.ArrayItem{ key: none, val: rt.concat(rt.concat(var_mapping.array_get(rt.new_int(0)),
							rt.new_string(':')), var_name) }])
				}
			}
		}
	}
	if var_attr {
		return rt.create_array([rt.ArrayItem{ key: none, val: rt.new_null() },
			rt.ArrayItem{ key: none, val: var_name }])
	} else {
		mut iter_5 := this.ns_contexts.iterator()
		for {
			item_5 := iter_5.next() or { break }
			mut var_context := item_5.val
			mut iter_6 := var_context.iterator()
			for {
				item_6 := iter_6.next() or { break }
				mut var_mapping := item_6.val
				if var_mapping.array_get(rt.new_int(0)).to_string().len == 0 {
					return rt.create_array([rt.ArrayItem{ key: none, val: var_mapping },
						rt.ArrayItem{ key: none, val: var_name }])
				}
			}
...
fn (mut this Class_AtomParser) is_declared_content_ns(var_new_mapping rt.PhpVal) bool {
	mut iter_7 := this.content_ns_contexts.iterator()
	for {
		item_7 := iter_7.next() or { break }
		mut var_context := item_7.val
		mut iter_8 := var_context.iterator()
		for {
			item_8 := iter_8.next() or { break }
			mut var_mapping := item_8.val
			if rt.is_true(rt.equal(var_new_mapping, var_mapping)) {
				return true
			}
		}
	}
...
fn create_atomfeed(_args ...rt.PhpVal) &Class_AtomFeed {
...
fn create_atomentry(_args ...rt.PhpVal) &Class_AtomEntry {
...
fn (mut this Class_AtomParser) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'ATOM_CONTENT_ELEMENTS' {
			this.ATOM_CONTENT_ELEMENTS = val
			return true
		}
		'ATOM_SIMPLE_ELEMENTS' {
			this.ATOM_SIMPLE_ELEMENTS = val
			return true
		}
		'debug' {
			this.debug = val
		...
		'content_ns_decls' {
			this.content_ns_decls = val
			return true
		}
		'content_ns_contexts' {
			this.content_ns_contexts = val
			return true
		}
		'is_xhtml' {
			this.is_xhtml = val.to_bool()
			return true
		}
		'is_html' {
			this.is_html = val.to_bool()
			return true
		}
		'is_text' {
			this.is_text = val.to_bool()
			return true
		}
		'skipped_div' {
			this.skipped_div = val
			return true
		}
		'FILE' {
			this.FILE = val
		...
		'map_attrs_func' {
			this.map_attrs_func = val
			return true
		}
		'map_xmlns_func' {
			this.map_xmlns_func = val
			return true
		}
		'error' {
			this.error = val
