...
fn (mut this Class_WP_HTTP_Requests_Response) construct(mut var_response Class_WpOrg_Requests_Response, filename string) {
	...
	this.filename = rt.new_string(filename)
}
...
fn (mut this Class_WP_HTTP_Requests_Response) get_headers() rt.PhpVal {
	...
	mut iter_1 :=
		rt.call_method(rt.get_property(this.response, 'headers'), 'getAll', []rt.PhpVal{}).iterator()
	for {
		item_1 := iter_1.next() or { break }
		mut var_value := item_1.val
		mut var_key := item_1.key
		if var_value.clone().array_count() == 1 {
			var_converted.array_set(var_key, var_value.array_get(rt.new_int(0)))
		} else {
			var_converted.array_set(var_key, var_value.clone())
		}
	}
	return var_converted.clone()
}

fn (mut this Class_WP_HTTP_Requests_Response) set_headers(var_headers rt.PhpVal) {
	rt.set_property(this.response, 'headers',
		create_wporg_requests_response_headers(var_headers.clone()))
}
...
fn (mut this Class_WP_HTTP_Requests_Response) header(var_key rt.PhpVal, var_value rt.PhpVal, replace bool) {
	...
	rt.get_property(this.response, 'headers').array_set(var_key, var_value.clone())
}
...
fn (mut this Class_WP_HTTP_Requests_Response) set_data(var_data rt.PhpVal) {
	rt.set_property(this.response, 'body', var_data.clone())
}

fn (mut this Class_WP_HTTP_Requests_Response) get_cookies() rt.PhpVal {
	mut var_cookies := []rt.PhpVal{}
	mut iter_2 := rt.get_property(this.response, 'cookies').iterator()
	for {
		...
		var_cookies << create_wp_http_cookie(rt.create_array([
			...
			rt.ArrayItem{
				key: 'expires'
				val: if !(rt.get_property(var_cookie, 'attributes').array_get(rt.new_string('expires'))).is_null() {
					rt.get_property(var_cookie, 'attributes').array_get(rt.new_string('expires'))
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'path'
				val: if !(rt.get_property(var_cookie, 'attributes').array_get(rt.new_string('path'))).is_null() {
					rt.get_property(var_cookie, 'attributes').array_get(rt.new_string('path'))
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'domain'
				val: if !(rt.get_property(var_cookie, 'attributes').array_get(rt.new_string('domain'))).is_null() {
					rt.get_property(var_cookie, 'attributes').array_get(rt.new_string('domain'))
				} else {
					rt.new_null()
				}
			},
			rt.ArrayItem{
				key: 'host_only'
				val: if !(rt.get_property(var_cookie, 'flags').array_get(rt.new_string('host-only'))).is_null() {
					rt.get_property(var_cookie, 'flags').array_get(rt.new_string('host-only'))
				} else {
					rt.new_null()
				}
			},
...
fn create_wp_http_response(_args ...rt.PhpVal) &Class_WP_HTTP_Response {
