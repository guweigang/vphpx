...
fn block_core_comment_template_render_comments(var_comments rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_thread_comments_depth := rt.new_null()
	...
	for {
		...
		var_comment_id = rt.get_property(var_comment_shadow, 'comment_ID')
		closure_1_fn := fn [var_comment_id] (this_ptr rt.PhpVal, args []rt.PhpVal) rt.PhpVal {
			...
			var_context.array_set('commentId', var_comment_id.clone())
		...
		var_block_content = rt.call_method(create_wp_block(rt.get_property(var_block,
			'parsed_block')), 'render', [
			rt.create_array([rt.ArrayItem{ key: 'dynamic', val: false }]),
		])
		rt.call_function('remove_filter', [rt.new_string('render_block_context'),
			var_filter_block_context.clone(), rt.new_int(1)])
		var_children = rt.call_method(var_comment_shadow, 'get_children', []rt.PhpVal{})
		var_comment_classes = rt.call_function('comment_class', [
			rt.new_string(''), rt.get_property(var_comment_shadow, 'comment_ID'),
			rt.get_property(var_comment_shadow, 'comment_post_ID'),
			rt.new_bool(false)])
		if !(!rt.is_true(var_children)) && !(!rt.is_true(var_thread_comments)) {
			if rt.is_true(rt.less(rt.new_int(var_comment_depth), var_thread_comments_depth)) {
				var_comment_depth += 1
				var_inner_content = rt.new_string(block_core_comment_template_render_comments(var_children.clone(),
					var_block.clone()))
				var_block_content = rt.concat(var_block_content, rt.call_function('sprintf', [
					rt.new_string('<ol>%1$s</ol>'),
					var_inner_content.clone(),
				]))
				var_comment_depth -= 1
			} else {
				var_block_content = rt.concat(var_block_content, rt.new_string(block_core_comment_template_render_comments(var_children.clone(),
					var_block.clone())))
			}
		}
		var_content = var_content +(rt.call_function('sprintf', [rt.new_string('<li id="comment-%1$s" %2$s>%3$s</li>'), rt.get_property(var_comment_shadow, 'comment_ID'), var_comment_classes.clone(), var_block_content.clone()])).str()
	}
	return var_content
}
...
fn render_block_core_comment_template(var_attributes rt.PhpVal, var_content rt.PhpVal, var_block rt.PhpVal) string {
	...
	mut var_comment_order := rt.new_null()
	...
	var_comment_query = create_wp_comment_query(rt.call_function('build_comment_query_vars_from_block', [
		var_block.clone(),
	]))
	var_comments = var_comment_query.get_comments()
	if var_comments.clone().array_count() == 0 {
		return ''
	}
	var_comment_order = rt.call_function('get_option', [rt.new_string('comment_order')])
	if rt.is_true(rt.identical(rt.new_string('desc'), var_comment_order)) {
		var_comments = rt.call_function('array_reverse', [var_comments.clone()])
	}
	...
	return (rt.call_function('sprintf', [rt.new_string('<ol %1$s>%2$s</ol>'),
		...
		rt.new_string(block_core_comment_template_render_comments(var_comments.clone(),
			var_block.clone()))])).str()
}

fn register_block_core_comment_template() {
	rt.call_function('register_block_type_from_metadata', [
		rt.new_string(@DIR + '/comment-template'),
		rt.create_array([
