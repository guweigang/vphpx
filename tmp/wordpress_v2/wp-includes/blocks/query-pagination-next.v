...
fn render_block_core_query_pagination_next(var_attributes rt.PhpVal, var_content_arg rt.PhpVal, var_block rt.PhpVal) rt.PhpVal {
	...
	mut var_custom_query := rt.new_null()
	mut var_custom_query_max_pages := rt.new_null()
	...
	var_default_label = rt.call_function('__', [rt.new_string('Next Page')])
	...
	var_pagination_arrow = rt.call_function('get_query_pagination_arrow', [
		var_block.clone(), rt.new_bool(true)])
	...
	if rt.get_property(var_block, 'context').array_get(rt.new_string('query')).array_isset(rt.new_string('inherit'))
		...
		rt.call_function('add_filter', [rt.new_string('next_posts_link_attributes'),
		...
		var_content = rt.call_function('get_next_posts_link', [
			var_label.clone(), var_max_page.clone()])
		rt.call_function('remove_filter', [rt.new_string('next_posts_link_attributes'),
			var_filter_link_attributes.clone()])
	} else if rt.is_true(rt.new_bool(!(rt.is_true(var_max_page))))
		|| rt.is_true(rt.greater(var_max_page, var_page)) {
		var_custom_query = create_wp_query(rt.call_function('build_query_vars_from_query_block', [
			var_block.clone(),
			var_page.clone(),
		]))
		var_custom_query_max_pages =
			rt.new_int((rt.get_property(var_custom_query, 'max_num_pages')).to_i64())
		if rt.is_true(var_custom_query_max_pages)
			&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_custom_query_max_pages, var_page)))) {
	...
	if rt.is_true(var_enhanced_pagination) && !var_content.is_null() {
		...
		if rt.is_true(var_p.next_tag(rt.create_array([
			...
			rt.ArrayItem{ key: 'class_name', val: 'wp-block-query-pagination-next' },
		])))
		{
			var_p.set_attribute(rt.new_string('data-wp-key'),
				rt.new_string('query-pagination-next'))
...
fn register_block_core_query_pagination_next() {
	rt.call_function('register_block_type_from_metadata', [
		rt.new_string(@DIR + '/query-pagination-next'),
		rt.create_array([
			rt.ArrayItem{ key: 'render_callback', val: 'render_block_core_query_pagination_next' },
		]),
	])
}
...
fn main() {
	...
	rt.call_function('add_action', [rt.new_string('init'),
		rt.new_string('register_block_core_query_pagination_next')])
}
