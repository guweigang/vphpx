...
fn main() {
	...
	if !(!rt.is_true(rt.get_superglobal('_SERVER').array_get(rt.new_string('SCRIPT_FILENAME'))))
		&& rt.is_true(rt.identical(rt.new_string('comments.php'), rt.call_function('basename', [rt.get_superglobal('_SERVER').array_get(rt.new_string('SCRIPT_FILENAME'))]))) {
		fn () {
			print((rt.new_string('Please do not load this page directly. Thanks!')).str())
			exit(0)
		}()
	}
	...
	if rt.is_true(rt.call_function('have_comments', []rt.PhpVal{})) {
			...
			[]rt.PhpVal{})))
		{
			rt.call_function('printf', [
				...
				rt.new_string('&#8220;' + (rt.call_function('get_the_title', []rt.PhpVal{})).str() +
					'&#8221;'),
			])
		} else {
			rt.call_function('printf', [
