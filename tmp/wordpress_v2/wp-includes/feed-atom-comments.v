...
fn main() {
	...
	if rt.is_true(rt.call_function('is_singular', []rt.PhpVal{})) {
		...
		rt.echo_val(rt.call_function('esc_url', [
			rt.call_function('get_post_comments_feed_link', [
				rt.new_string(''), rt.new_string('atom')]),
		]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('esc_url', [
			rt.call_function('get_post_comments_feed_link', [
		...
		rt.call_function('bloginfo_rss', [rt.new_string('html_type')])
		// unsupported statement: Stmt_InlineHTML
		print((rt.call_function('home_url', []rt.PhpVal{})).str() + '?s=' +
			(rt.call_function('get_search_query', []rt.PhpVal{})).str())
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('get_search_comments_feed_link', [
			rt.new_string(''), rt.new_string('atom')]))
		// unsupported statement: Stmt_InlineHTML
		rt.echo_val(rt.call_function('get_search_comments_feed_link', [
			rt.new_string(''), rt.new_string('atom')]))
	...
	for rt.is_true(rt.call_function('have_comments', []rt.PhpVal{})) {
		...
		if rt.is_true(rt.call_function('get_comment_author_url', []rt.PhpVal{})) {
			print('<uri>' + (rt.call_function('get_comment_author_url', []rt.PhpVal{})).str() +
				'</uri>')
		}
		// unsupported statement: Stmt_InlineHTML
		...
		// unsupported statement: Stmt_InlineHTML
		if rt.is_true(rt.identical(rt.new_string('0'), rt.get_property(var_comment,
			'comment_parent')))
		{
			// unsupported statement: Stmt_InlineHTML
			...
			mut var_parent_comment := rt.call_function('get_comment', [
				rt.get_property(var_comment, 'comment_parent'),
			])
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('comment_guid', [var_parent_comment.clone()])
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('get_comment_link', [
				var_parent_comment.clone()]))
			// unsupported statement: Stmt_InlineHTML
			rt.call_function('bloginfo_rss', [rt.new_string('html_type')])
			// unsupported statement: Stmt_InlineHTML
		}
		rt.call_function('do_action', [rt.new_string('comment_atom_entry'),
