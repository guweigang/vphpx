...
fn _walk_bookmarks(var_bookmarks rt.PhpVal, args string) string {
	...
	for {
		...
		if rt.is_true(var_parsed_args.array_get(rt.new_string('show_updated')))
		...
		var_name = rt.call_function('esc_attr', [
			rt.call_function('sanitize_bookmark_field', [rt.new_string('link_name'),
				rt.get_property(var_bookmark_shadow, 'link_name'),
				rt.get_property(var_bookmark_shadow, 'link_id'),
				rt.new_string('display')]),
		])
		...
		var_alt = rt.new_string(' alt="' + var_name.str() +
			if rt.is_true(var_parsed_args.array_get(rt.new_string('show_description'))) { ' ' +
			var_title.str() } else { '' } + '"')
		...
		var_output = var_output + (var_parsed_args.array_get(rt.new_string('link_before'))).str()
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(''), rt.get_property(var_bookmark_shadow, 'link_image')))))
			&& rt.is_true(var_parsed_args.array_get(rt.new_string('show_images'))) {
			...
			if rt.is_true(var_parsed_args.array_get(rt.new_string('show_name'))) {
				var_output = var_output + ' ${var_name.to_string()}'
			}
		} else {
			var_output = var_output + var_name.str()
		}
		var_output = var_output + (var_parsed_args.array_get(rt.new_string('link_after'))).str()
		...
		if rt.is_true(var_parsed_args.array_get(rt.new_string('show_description')))
			...
			var_output = var_output +
				(var_parsed_args.array_get(rt.new_string('between'))).str() + var_desc.str()
		}
		if rt.is_true(var_parsed_args.array_get(rt.new_string('show_rating'))) {
			var_output = var_output +(var_parsed_args.array_get(rt.new_string('between'))).str() +(rt.call_function('sanitize_bookmark_field', [rt.new_string('link_rating'), rt.get_property(var_bookmark_shadow, 'link_rating'), rt.get_property(var_bookmark_shadow, 'link_id'), rt.new_string('display')])).str()
		}
...
fn wp_list_bookmarks(args string) rt.PhpVal {
	...
	var_parsed_args = rt.call_function('wp_parse_args', [rt.new_string(args),
		rt.create_array_from_native_map(var_defaults)])
	var_output = ''
	if !(var_parsed_args.array_get(rt.new_string('class')).is_array()) {
		var_parsed_args.array_set('class', rt.call_function('explode', [
			rt.new_string(' '),
			var_parsed_args.array_get(rt.new_string('class')),
		]))
	}
	...
	var_parsed_args.array_set('class', rt.call_function('implode', [
		rt.new_string(' '), var_parsed_args.array_get(rt.new_string('class'))]).to_string().trim_space())
	if rt.is_true(var_parsed_args.array_get(rt.new_string('categorize'))) {
		var_cats = rt.call_function('get_terms', [
			rt.create_array([rt.ArrayItem{ key: 'taxonomy', val: 'link_category' },
				rt.ArrayItem{
					key: 'name__like'
					val: var_parsed_args.array_get(rt.new_string('category_name'))
				}, rt.ArrayItem{
					key: 'include'
					val: var_parsed_args.array_get(rt.new_string('category'))
				}, rt.ArrayItem{
					key: 'exclude'
					val: var_parsed_args.array_get(rt.new_string('exclude_category'))
				}, rt.ArrayItem{
					key: 'orderby'
					val: var_parsed_args.array_get(rt.new_string('category_orderby'))
				}, rt.ArrayItem{
					key: 'order'
					val: var_parsed_args.array_get(rt.new_string('category_order'))
	...
	if rt.is_true(var_parsed_args.array_get(rt.new_string('categorize'))) {
		mut iter_2 := rt.cast_array(var_cats).iterator()
		for {
			item_2 := iter_2.next() or { break }
			mut var_cat_shadow := item_2.val
			var_params = rt.call_function('array_merge', [var_parsed_args.clone(),
				rt.create_array([
					rt.ArrayItem{ key: 'category', val: rt.get_property(var_cat_shadow, 'term_id') },
				])])
			...
			}, rt.ArrayItem{ key: none, val: var_parsed_args.array_get(rt.new_string('class')) }]), var_parsed_args.array_get(rt.new_string('category_before'))])).str()
			var_catname = rt.call_function('apply_filters', [
				rt.new_string('link_category'),
				rt.get_property(var_cat_shadow, 'name'),
			])
			var_output = var_output +
				(var_parsed_args.array_get(rt.new_string('title_before'))).str()
			var_output = var_output + var_catname.str()
			var_output = var_output +
				(var_parsed_args.array_get(rt.new_string('title_after'))).str()
			var_output = var_output + "\n\t<ul class='xoxo blogroll'>\n"
			var_output = var_output +
				_walk_bookmarks(var_bookmarks.clone(), var_parsed_args.clone())
			var_output = var_output + '\n\t</ul>\n'
			var_output = var_output +
				(var_parsed_args.array_get(rt.new_string('category_after'))).str() + '\n'
		}
	} else {
		...
		if !(!rt.is_true(var_bookmarks)) {
			if !(!rt.is_true(var_parsed_args.array_get(rt.new_string('title_li')))) {
				var_output = var_output +
					...
					key: none
					val: 'linkcat-' +(var_parsed_args.array_get(rt.new_string('category'))).str()
				}, rt.ArrayItem{ key: none, val: var_parsed_args.array_get(rt.new_string('class')) }]), var_parsed_args.array_get(rt.new_string('category_before'))])).str()
				var_output = var_output +
					(var_parsed_args.array_get(rt.new_string('title_before'))).str()
				var_output = var_output +
					(var_parsed_args.array_get(rt.new_string('title_li'))).str()
				var_output = var_output +
					(var_parsed_args.array_get(rt.new_string('title_after'))).str()
				var_output = var_output + "\n\t<ul class='xoxo blogroll'>\n"
				var_output = var_output +
