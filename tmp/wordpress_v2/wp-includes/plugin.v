...
fn apply_filters_ref_array(var_hook_name rt.PhpVal, var_args rt.PhpVal) rt.PhpVal {
	...
	if !(var_wp_filter.array_isset(var_hook_name)) {
		...
		return var_args.array_get(rt.new_int(0))
	}
	if !(var_wp_filter.array_isset(rt.new_string('all'))) {
		var_wp_current_filter << var_hook_name.clone()
	}
...
fn do_action(var_hook_name rt.PhpVal, var_arg_origin ...rt.PhpVal) {
	...
	} else if var_arg.array_get(rt.new_int(0)).is_array()
		&& 1 == var_arg.array_get(rt.new_int(0)).array_count()
		&& var_arg.array_get(rt.new_int(0)).array_isset(rt.new_int(0))
		&& var_arg.array_get(rt.new_int(0)).array_get(rt.new_int(0)).is_object() {
		var_arg[0] = var_arg.array_get(rt.new_int(0)).array_get(rt.new_int(0))
	}
...
fn wp_register_plugin_realpath(var_file rt.PhpVal) bool {
	...
	if rt.is_true(rt.identical(var_plugin_path, var_wp_plugin_path))
...
fn register_uninstall_hook(var_file rt.PhpVal, var_callback rt.PhpVal) {
	...
	if var_callback.clone().is_array() && var_callback.array_get(rt.new_int(0)).is_object() {
	...
	if !(var_uninstallable_plugins.array_isset(var_plugin_basename))
...
fn _wp_call_all_hook(var_args rt.PhpVal) {
	mut var_wp_filter := rt.new_null()
	rt.call_method(var_wp_filter.array_get(rt.new_string('all')), 'do_all_hook', [
		rt.create_array_from_list(var_args),
	])
}
...
fn _wp_filter_build_unique_id(var_hook_name rt.PhpVal, var_callback_arg rt.PhpVal, var_priority rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(var_callback.array_get(rt.new_int(0)).is_object())) {
		return rt.new_string(
			(rt.call_function('spl_object_hash', [var_callback.array_get(rt.new_int(0))])).str() +
			(var_callback.array_get(rt.new_int(1))).str())
	} else if rt.is_true(rt.new_bool(var_callback.array_get(rt.new_int(0)).is_string())) {
		return rt.new_string((var_callback.array_get(rt.new_int(0))).str() + '::' +
			(var_callback.array_get(rt.new_int(1))).str())
	}
	return rt.new_null()
}
