...
fn (mut this Class_WP_Recovery_Mode_Cookie_Service) set_cookie() {
	...
	mut var_length := rt.call_function('apply_filters', [
		rt.new_string('recovery_mode_cookie_length'),
		rt.get_constant('WEEK_IN_SECONDS'),
	])
	mut var_expire := rt.add(rt.call_function('time', []rt.PhpVal{}), var_length)
	rt.call_function('setcookie', [rt.get_constant('RECOVERY_MODE_COOKIE'),
		var_value.clone(), var_expire.clone(), rt.get_constant('COOKIEPATH'),
	...
	{
		rt.call_function('setcookie', [rt.get_constant('RECOVERY_MODE_COOKIE'),
			var_value.clone(), var_expire.clone(), rt.get_constant('SITECOOKIEPATH'),
			rt.get_constant('COOKIE_DOMAIN'), rt.call_function('is_ssl', []rt.PhpVal{}),
			rt.new_bool(true)])
	}
}
...
fn (mut this Class_WP_Recovery_Mode_Cookie_Service) validate_cookie(cookie string) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_string(cookie_mutated))))) {
		if !rt.is_true(rt.get_superglobal('_COOKIE').array_get(rt.get_constant('RECOVERY_MODE_COOKIE'))) {
			return (create_wp_error(rt.new_string('no_cookie'), rt.call_function('__', [
				rt.new_string('No cookie present.'),
			]))).to_bool()
		}
		cookie_mutated =
			(rt.get_superglobal('_COOKIE').array_get(rt.get_constant('RECOVERY_MODE_COOKIE'))).str()
	}
	mut var_parts := this.parse_cookie(rt.new_string(cookie_mutated))
	if rt.is_true(rt.call_function('is_wp_error', [var_parts.clone()])) {
		return var_parts.to_bool()
	}
	mut list_tmp_1 := var_parts
	var_created_at = list_tmp_1.array_get(1)
	var_random = list_tmp_1.array_get(2)
	var_signature = list_tmp_1.array_get(3)
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('ctype_digit', [
		var_created_at.clone()])))))
	{
		return (create_wp_error(rt.new_string('invalid_created_at'), rt.call_function('__', [
			rt.new_string('Invalid cookie format.'),
		]))).to_bool()
	}
	mut var_length := rt.call_function('apply_filters', [
		rt.new_string('recovery_mode_cookie_length'),
	...
	if rt.is_true(rt.greater(rt.call_function('time', []rt.PhpVal{}), rt.add(var_created_at,
		var_length)))
	{
		return (create_wp_error(rt.new_string('expired'), rt.call_function('__', [
			rt.new_string('Cookie expired.'),
		]))).to_bool()
	}
	mut var_to_sign := rt.call_function('sprintf', [rt.new_string('recovery_mode|%s|%s'),
		var_created_at.clone(), var_random.clone()])
	mut var_hashed := this.recovery_mode_hash(var_to_sign.clone())
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('hash_equals', [
		var_signature.clone(), var_hashed.clone()])))))
	{
		return (create_wp_error(rt.new_string('signature_mismatch'), rt.call_function('__', [
			rt.new_string('Invalid cookie.'),
		]))).to_bool()
	}
	return true
}
...
fn (mut this Class_WP_Recovery_Mode_Cookie_Service) get_session_id_from_cookie(cookie string) string {
	...
	mut list_tmp_2 := var_parts
	var_random = list_tmp_2.array_get(2)
	return sha1.hexhash(var_random.clone().to_string())
}
...
fn (mut this Class_WP_Recovery_Mode_Cookie_Service) parse_cookie(var_cookie rt.PhpVal) rt.PhpVal {
	...
	var_cookie_mutated = rt.call_function('base64_decode', [var_cookie_mutated.clone()])
	mut var_parts := rt.call_function('explode', [rt.new_string('|'),
		var_cookie_mutated.clone()])
	if rt.is_true(rt.new_bool(4 != var_parts.clone().array_count())) {
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('invalid_format'), rt.call_function('__', [
			rt.new_string('Invalid cookie format.'),
		])))
	}
...
fn (mut this Class_WP_Recovery_Mode_Cookie_Service) generate_cookie() rt.PhpVal {
	mut var_to_sign := rt.call_function('sprintf', [rt.new_string('recovery_mode|%s|%s'),
		...
		rt.call_function('wp_generate_password', [
			rt.new_int(20),
			rt.new_bool(false),
		])])
	mut var_signed := this.recovery_mode_hash(var_to_sign.clone())
	return rt.call_function('base64_encode', [
		rt.call_function('sprintf', [rt.new_string('%s|%s'), var_to_sign.clone(),
			var_signed.clone()]),
	])
}
...
fn (mut this Class_WP_Recovery_Mode_Cookie_Service) recovery_mode_hash(var_data rt.PhpVal) rt.PhpVal {
	mut var_default_keys := rt.call_function('array_unique', [
		rt.create_array([rt.ArrayItem{ key: none, val: 'put your unique phrase here' },
			rt.ArrayItem{ key: none, val: rt.call_function('__', [
				rt.new_string('put your unique phrase here'),
			]) }]),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('AUTH_KEY')])))))
		|| rt.is_true(rt.call_function('in_array', [rt.get_constant('AUTH_KEY'), var_default_keys.clone(), rt.new_bool(true)])) {
		mut var_auth_key := rt.call_function('get_site_option', [
			rt.new_string('recovery_mode_auth_key'),
		])
		if rt.is_true(rt.new_bool(!(rt.is_true(var_auth_key)))) {
			...
			var_auth_key = rt.call_function('wp_generate_password', [
				rt.new_int(64), rt.new_bool(true), rt.new_bool(true)])
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('defined', [rt.new_string('AUTH_SALT')])))))
		|| rt.is_true(rt.call_function('in_array', [rt.get_constant('AUTH_SALT'), var_default_keys.clone(), rt.new_bool(true)]))
		|| rt.is_true(rt.identical(rt.get_constant('AUTH_SALT'), var_auth_key)) {
		mut var_auth_salt := rt.call_function('get_site_option', [
			rt.new_string('recovery_mode_auth_salt'),
		])
		if rt.is_true(rt.new_bool(!(rt.is_true(var_auth_salt)))) {
			...
			var_auth_salt = rt.call_function('wp_generate_password', [
				rt.new_int(64), rt.new_bool(true), rt.new_bool(true)])
			rt.call_function('update_site_option', [
				rt.new_string('recovery_mode_auth_salt'),
				var_auth_salt.clone(),
	...
	mut var_secret := rt.new_string(var_auth_key.str() + var_auth_salt.str())
	return rt.call_function('hash_hmac', [rt.new_string('sha1'),
		var_data.clone(), var_secret.clone()])
}
