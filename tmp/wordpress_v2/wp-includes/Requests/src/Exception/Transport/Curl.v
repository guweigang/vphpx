...
fn (mut this Class_WpOrg_Requests_Exception_Transport_Curl) construct(var_message rt.PhpVal, var_type rt.PhpVal, var_data rt.PhpVal, code i64) {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_int(code), rt.new_null())))) {
		this.code = code
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_message_mutated, rt.new_null())))) {
		this.reason = var_message_mutated.clone()
	}
	var_message_mutated = rt.call_function('sprintf',
		[rt.new_string('%d %s'), this.code, this.reason])
	this.Class_WpOrg_Requests_Exception_Transport.construct(var_message_mutated.clone(),
		this.prop_type, var_data.clone(), this.code)
}
