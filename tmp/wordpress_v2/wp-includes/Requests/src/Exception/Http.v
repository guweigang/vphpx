...
fn (mut this Class_WpOrg_Requests_Exception_Http) construct(var_reason rt.PhpVal, var_data rt.PhpVal) {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_reason, rt.new_null())))) {
		this.reason = var_reason.clone()
	}
	mut var_message := rt.call_function('sprintf', [rt.new_string('%d %s'), this.code, this.reason])
	this.Class_WpOrg_Requests_Exception.construct(var_message.clone(),
		rt.new_string('httpresponse'), var_data.clone(), this.code)
}
...
fn Class_WpOrg_Requests_Exception_Http.get_class(var_code rt.PhpVal) rt.PhpVal {
	...
	mut var_class := rt.call_function('sprintf', [
		rt.new_string('\\WpOrg\\Requests\\Exception\\Http\\Status%d'),
		var_code.clone(),
	])
	if rt.is_true(rt.call_function('class_exists', [var_class.clone()])) {
