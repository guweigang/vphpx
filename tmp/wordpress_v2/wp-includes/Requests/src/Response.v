...
fn (mut this Class_WpOrg_Requests_Response) is_redirect() bool {
	...
	return
		rt.is_true(rt.call_function('in_array', [var_code.clone(), rt.create_array([rt.ArrayItem{
		key: none
		val: 300
	}, rt.ArrayItem{ key: none, val: 301 }, rt.ArrayItem{ key: none, val: 302 }, rt.ArrayItem{
		key: none
		val: 303
	}, rt.ArrayItem{ key: none, val: 307 }]), rt.new_bool(true)]))
		|| rt.is_true(rt.greater(var_code, rt.new_int(307)))
		&& rt.is_true(rt.less(var_code, rt.new_int(400)))
}

fn (mut this Class_WpOrg_Requests_Response) throw_for_status(allow_redirects bool) {
	if this.is_redirect() {
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(allow_redirects),
			rt.new_bool(true)))))
		{
			rt.throw_exception(rt.new_object('WpOrg_Requests_Exception', []string{}, create_wporg_requests_exception(rt.new_string('Redirection not allowed'),
				rt.new_string('response.no_redirects'), rt.new_object('WpOrg_Requests_Response',
				[]string{}, &this))))
		}
	} else if rt.is_true(rt.new_bool(!(rt.is_true(this.success)))) {
		mut iife_temp_0 := Class_WpOrg_Requests_Exception_Http{}
		mut iife_result_0 := iife_temp_0.get_class(this.status_code)
		mut var_exception := iife_result_0
		rt.throw_exception(rt.new_object('', []string{}, rt.create_object_dynamically(var_exception, [
			rt.new_null(),
			rt.new_object('WpOrg_Requests_Response', []string{}, &this),
		])))
	}
}

fn (mut this Class_WpOrg_Requests_Response) decode_body(associative bool, depth i64, options i64) rt.PhpVal {
	mut var_data := rt.call_function('json_decode', [this.body, rt.new_bool(associative),
		rt.new_int(depth), rt.new_int(options)])
		...
		[]rt.PhpVal{}), rt.get_constant('JSON_ERROR_NONE')))))
	{
		...
		rt.throw_exception(rt.new_object('WpOrg_Requests_Exception', []string{}, create_wporg_requests_exception(
			'Unable to parse JSON data: ' + var_last_error.str(),
			rt.new_string('response.invalid'), rt.new_object('WpOrg_Requests_Response', []string{},
			&this))))
	}
