...
fn (mut this Class_WpOrg_Requests_Proxy_Http) construct(var_args rt.PhpVal) {
	if rt.is_true(rt.new_bool(var_args.clone().is_string())) {
		this.proxy = var_args.clone()
	} else if rt.is_true(rt.new_bool(var_args.clone().is_array())) {
		if var_args.clone().array_count() == 1 {
			mut list_tmp_1 := var_args
		} else if var_args.clone().array_count() == 3 {
			mut list_tmp_2 := var_args
			this.use_authentication = true
		} else {
			mut iife_temp_0 := Class_WpOrg_Requests_Exception_ArgumentCount{}
			mut iife_result_0 := iife_temp_0.create(rt.new_string('an array with exactly one element or exactly three elements'),
				rt.new_int(var_args.clone().array_count()), rt.new_string('proxyhttpbadargs'))
			rt.throw_exception(iife_result_0)
		}
	} else if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_args, rt.new_null())))) {
		...
		mut iife_result_1 := iife_temp_1.create(rt.new_int(1), rt.new_string('$args'),
			rt.new_string('array|string|null'), rt.call_function('gettype', [
			var_args.clone()]))
...
fn (mut this Class_WpOrg_Requests_Proxy_Http) register(mut var_hooks Class_WpOrg_Requests_Hooks) {
	var_hooks.register(rt.new_string('curl.before_send'), rt.create_array([
		rt.ArrayItem{ key: none, val: rt.new_object('WpOrg_Requests_Proxy_Http', [
			'Proxy',
		], &this) },
		rt.ArrayItem{ key: none, val: 'curl_before_send' },
	]))
	var_hooks.register(rt.new_string('fsockopen.remote_socket'), rt.create_array([
		rt.ArrayItem{ key: none, val: rt.new_object('WpOrg_Requests_Proxy_Http', [
			'Proxy',
		], &this) },
		rt.ArrayItem{ key: none, val: 'fsockopen_remote_socket' },
	]))
	var_hooks.register(rt.new_string('fsockopen.remote_host_path'), rt.create_array([
		rt.ArrayItem{ key: none, val: rt.new_object('WpOrg_Requests_Proxy_Http', [
			'Proxy',
		], &this) },
		rt.ArrayItem{ key: none, val: 'fsockopen_remote_host_path' },
	]))
...
fn (mut this Class_WpOrg_Requests_Proxy_Http) curl_before_send(var_handle rt.PhpVal) {
	...
	rt.call_function('curl_setopt', [var_handle.clone(), rt.get_constant('CURLOPT_PROXY'),
		this.proxy])
	if this.use_authentication {
		rt.call_function('curl_setopt', [var_handle.clone(), rt.get_constant('CURLOPT_PROXYAUTH'),
			rt.get_constant('CURLAUTH_ANY')])
		rt.call_function('curl_setopt', [var_handle.clone(), rt.get_constant('CURLOPT_PROXYUSERPWD'),
			rt.new_string(this.get_auth_string())])
	}
}
...
fn (mut this Class_WpOrg_Requests_Proxy_Http) fsockopen_header(var_out rt.PhpVal) {
	var_out = rt.concat(var_out, rt.call_function('sprintf', [
		rt.new_string('Proxy-Authorization: Basic %s\r\n'),
		rt.call_function('base64_encode', [rt.new_string(this.get_auth_string())]),
	]))
}
...
fn (mut this Class_WpOrg_Requests_Proxy_Http) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'use_authentication' {
			this.use_authentication = val.to_bool()
			return true
		}
		else {
