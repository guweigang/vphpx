...
fn (mut this Class_WpOrg_Requests_Cookie_Jar) construct(var_cookies rt.PhpVal) {
	...
	if rt.is_true(rt.identical(rt.new_bool(var_cookies_mutated.clone().is_array()),
		rt.new_bool(false)))
	{
		mut iife_temp_0 := Class_WpOrg_Requests_Exception_InvalidArgument{}
		mut iife_result_0 := iife_temp_0.create(rt.new_int(1), rt.new_string('$cookies'),
			rt.new_string('array'), rt.call_function('gettype', [
			var_cookies_mutated.clone()]))
		rt.throw_exception(iife_result_0)
	}
	this.cookies = var_cookies_mutated.clone()
}
...
fn (mut this Class_WpOrg_Requests_Cookie_Jar) normalize_cookie(var_cookie rt.PhpVal, key string) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(rt.instance_of(var_cookie_mutated, 'WpOrg_Requests_Cookie'))) {
		return var_cookie_mutated.clone()
	}
	mut iife_temp_1 := Class_WpOrg_Requests_Cookie{}
	mut iife_result_1 := iife_temp_1.parse(var_cookie_mutated.clone(), rt.new_string(key))
...
fn (mut this Class_WpOrg_Requests_Cookie_Jar) offsetset(var_offset rt.PhpVal, var_value rt.PhpVal) {
	...
	this.cookies.array_set(var_offset, var_value.clone())
}
...
fn (mut this Class_WpOrg_Requests_Cookie_Jar) getiterator() rt.PhpVal {
	return rt.new_object('ArrayIterator', []string{}, create_arrayiterator(this.cookies))
}
...
fn (mut this Class_WpOrg_Requests_Cookie_Jar) before_request(var_url rt.PhpVal, var_headers rt.PhpVal, var_data rt.PhpVal, var_type rt.PhpVal, var_options rt.PhpVal) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_url_mutated,
		'WpOrg_Requests_Iri'))))))
	{
		var_url_mutated = create_wporg_requests_iri(var_url_mutated.clone())
	}
	if !(!rt.is_true(this.cookies)) {
		...
		for {
			...
			var_cookie = this.normalize_cookie(var_cookie.clone(), var_key.str())
...
fn (mut this Class_WpOrg_Requests_Cookie_Jar) before_redirect_check(mut var_response Class_WpOrg_Requests_Response) {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.new_bool(rt.instance_of(var_url, 'WpOrg_Requests_Iri')))))) {
		var_url = create_wporg_requests_iri(var_url.clone())
	}
	mut iife_temp_2 := Class_WpOrg_Requests_Cookie{}
	mut iife_result_2 := iife_temp_2.parse_from_headers(rt.get_property(var_response_mutated,
		'headers'), var_url.clone())
	mut var_cookies := iife_result_2
	this.cookies = rt.call_function('array_merge', [this.cookies, var_cookies.clone()])
