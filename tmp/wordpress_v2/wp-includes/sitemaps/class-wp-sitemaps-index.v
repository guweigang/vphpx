...
fn (mut this Class_WP_Sitemaps_Index) get_sitemap_list() rt.PhpVal {
	...
	for {
		...
		var_sitemaps.clone().array_push(var_sitemap_entries.clone())
		if rt.is_true(rt.greater_equal(rt.new_int(var_sitemaps.clone().array_count()),
			this.max_sitemaps))
		{
			break
		}
	}
	return rt.call_function('array_slice', [var_sitemaps.clone(),
