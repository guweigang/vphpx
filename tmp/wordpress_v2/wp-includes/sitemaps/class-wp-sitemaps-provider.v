...
fn (mut this Class_WP_Sitemaps_Provider) get_sitemap_type_data() rt.PhpVal {
	...
	for {
		...
		var_object_subtype_name = rt.new_string(var_object_subtype_name.str())
		var_sitemap_data << rt.create_array([
			rt.ArrayItem{ key: 'name', val: var_object_subtype_name },
			rt.ArrayItem{ key: 'pages', val: this.get_max_num_pages(var_object_subtype_name.str()) },
		])
	}
	return var_sitemap_data.clone()
}
...
fn (mut this Class_WP_Sitemaps_Provider) get_sitemap_entries() rt.PhpVal {
	...
	mut iter_2 := var_sitemap_types.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_type := item_2.val
		mut var_page := rt.new_int(1)
		for {
			if !(rt.is_true(rt.less_equal(var_page, var_type.array_get(rt.new_string('pages'))))) { break
			 }
			mut var_sitemap_entry := rt.create_array([
				rt.ArrayItem{ key: 'loc', val: this.get_sitemap_url(var_type.array_get(rt.new_string('name')),
					var_page.clone()) },
			])
			var_sitemap_entry = rt.call_function('apply_filters', [
				...
				this.object_type,
				var_type.array_get(rt.new_string('name')),
				var_page.clone(),
			])
			var_sitemaps << var_sitemap_entry.clone()
			rt.post_inc(var_page)
		}
	}
	return var_sitemaps.clone()
}
...
fn (mut this Class_WP_Sitemaps_Provider) get_sitemap_url(var_name rt.PhpVal, var_page rt.PhpVal) rt.PhpVal {
	...
	mut var_basename := rt.call_function('sprintf', [
		...
		rt.call_function('implode', [rt.new_string('-'), var_params.clone()]),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_method(var_wp_rewrite, 'using_permalinks',
		[]rt.PhpVal{})))))
	{
		var_basename =
			rt.new_string('/?' +(rt.call_function('http_build_query', [var_params.clone(), rt.new_string(''), rt.new_string('&')])).str())
	}
	return rt.call_function('home_url', [var_basename.clone()])
}
