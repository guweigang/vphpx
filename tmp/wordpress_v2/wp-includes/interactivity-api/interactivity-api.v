...
fn wp_interactivity() rt.PhpVal {
	mut var_wp_interactivity := rt.new_null()
	if !(true) {
		var_wp_interactivity = create_wp_interactivity_api()
	}
...
fn wp_interactivity_state(var_store_namespace rt.PhpVal, var_state rt.PhpVal) rt.PhpVal {
	return rt.call_method(wp_interactivity(), 'state', [var_store_namespace.clone(),
		var_state.clone()])
}

fn wp_interactivity_config(store_namespace string, var_config rt.PhpVal) rt.PhpVal {
	mut var_store_namespace := store_namespace
...
fn wp_interactivity_data_wp_context(var_context rt.PhpVal, store_namespace string) string {
	...
	return "data-wp-context='" + if var_store_namespace.len > 0 && var_store_namespace != '0' {
		...
		rt.call_function('wp_json_encode', [var_context.clone(), rt.bitwise_or(rt.bitwise_or(rt.bitwise_or(rt.get_constant('JSON_HEX_TAG'), rt.get_constant('JSON_HEX_APOS')), rt.get_constant('JSON_HEX_QUOT')), rt.get_constant('JSON_HEX_AMP'))])
	}).str() + "'"
}
