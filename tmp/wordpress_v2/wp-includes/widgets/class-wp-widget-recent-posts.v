...
fn (mut this Class_WP_Widget_Recent_Posts) construct() {
	mut var_widget_ops := {
		'classname':                   rt.new_string('widget_recent_entries')
		'description':                 rt.call_function('__', [
			rt.new_string('Your site&#8217;s most recent Posts.'),
		])
		'customize_selective_refresh': rt.new_bool(true)
		'show_instance_in_rest':       rt.new_bool(true)
	}
	this.Class_WP_Widget.construct(rt.new_string('recent-posts'), rt.call_function('__', [
		rt.new_string('Recent Posts'),
...
fn (mut this Class_WP_Widget_Recent_Posts) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	if !(var_args_mutated.array_isset(rt.new_string('widget_id'))) {
		var_args_mutated.array_set('widget_id', rt.get_property(rt.new_object('WP_Widget_Recent_Posts', [
			'WP_Widget',
		], &this), 'id'))
	}
	...
	var_title = rt.call_function('apply_filters', [rt.new_string('widget_title'),
		...
		rt.get_property(rt.new_object('WP_Widget_Recent_Posts', [
			'WP_Widget',
		], &this), 'id_base')])
	...
	mut var_show_date := if !(var_instance_mutated.array_get(rt.new_string('show_date'))).is_null() {
		var_instance_mutated.array_get(rt.new_string('show_date'))
	} else {
		rt.new_bool(false)
	}
	mut var_r := create_wp_query(rt.call_function('apply_filters', [
		rt.new_string('widget_posts_args'),
	...
	for {
		...
		mut var_post_title := rt.call_function('get_the_title', [
			rt.get_property(var_recent_post, 'ID'),
		])
		var_title = if !(!rt.is_true(var_post_title)) { var_post_title } else { rt.call_function('__', [
				rt.new_string('(no title)'),
			]) }
		mut var_aria_current := rt.new_string('')
		if rt.is_true(rt.identical(rt.call_function('get_queried_object_id', []rt.PhpVal{}), rt.get_property(var_recent_post,
			'ID')))
		{
		...
		if rt.is_true(var_show_date) {
			// unsupported statement: Stmt_InlineHTML
			rt.echo_val(rt.call_function('get_the_date', [rt.new_string(''),
				rt.get_property(var_recent_post, 'ID')]))
			// unsupported statement: Stmt_InlineHTML
		}
		// unsupported statement: Stmt_InlineHTML
...
fn (mut this Class_WP_Widget_Recent_Posts) update(var_new_instance rt.PhpVal, var_old_instance rt.PhpVal) rt.PhpVal {
	...
	var_instance.array_set('number',
		rt.new_int((var_new_instance.array_get(rt.new_string('number'))).to_i64()))
	var_instance.array_set('show_date', if var_new_instance.array_isset(rt.new_string('show_date')) {
		(var_new_instance.array_get(rt.new_string('show_date'))).to_bool()
	} else {
		false
	})
...
fn (mut this Class_WP_Widget_Recent_Posts) form(var_instance rt.PhpVal) {
	mut var_instance_mutated := var_instance
	mut var_title := if var_instance_mutated.array_isset(rt.new_string('title')) { rt.call_function('esc_attr', [
			var_instance_mutated.array_get(rt.new_string('title')),
		]) } else { rt.new_string('') }
	mut var_number := if var_instance_mutated.array_isset(rt.new_string('number')) { rt.call_function('absint', [
			var_instance_mutated.array_get(rt.new_string('number')),
		]) } else { rt.new_int(5) }
	mut var_show_date := rt.new_bool(if var_instance_mutated.array_isset(rt.new_string('show_date')) {
		(var_instance_mutated.array_get(rt.new_string('show_date'))).to_bool()
	} else {
		false
	})
	// unsupported statement: Stmt_InlineHTML
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('checked', [var_show_date.clone()])
	// unsupported statement: Stmt_InlineHTML
