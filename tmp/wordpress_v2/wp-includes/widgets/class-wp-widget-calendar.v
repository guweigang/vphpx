...
fn init_static_wp_widget_calendar() {
	rt.init_static_prop('WP_Widget_Calendar', 'instance', rt.new_int(0))
}

fn (mut this Class_WP_Widget_Calendar) construct() {
	mut var_widget_ops := {
		'classname':                   rt.new_string('widget_calendar')
		'description':                 rt.call_function('__', [
			rt.new_string('A calendar of your site’s posts.'),
		])
		'customize_selective_refresh': rt.new_bool(true)
		'show_instance_in_rest':       rt.new_bool(true)
	}
	this.Class_WP_Widget.construct(rt.new_string('calendar'), rt.call_function('__', [
...
fn (mut this Class_WP_Widget_Calendar) widget(var_args rt.PhpVal, var_instance rt.PhpVal) {
	...
	var_title = rt.call_function('apply_filters', [rt.new_string('widget_title'),
		...
		rt.get_property(rt.new_object('WP_Widget_Calendar', [
			'WP_Widget',
		], &this), 'id_base')])
	...
	if rt.is_true(rt.identical(rt.new_int(0), rt.get_static_prop('WP_Widget_Calendar', 'instance'))) {
	...
	rt.pre_inc(rt.get_static_prop('WP_Widget_Calendar', 'instance'))
}
