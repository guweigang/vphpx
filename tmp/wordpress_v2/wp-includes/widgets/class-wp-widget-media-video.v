	...
	for {
		...
		var_schema.array_set(var_video_extension, rt.create_array([
			...
			rt.ArrayItem{ key: 'description', val: rt.call_function('sprintf', [
				rt.call_function('__', [rt.new_string('URL to the %s video source file')]),
				var_video_extension.clone(),
			]) },
		]))
	}
	return rt.call_function('array_merge', [var_schema.clone(),
...
fn (mut this Class_WP_Widget_Media_Video) render_media(var_instance rt.PhpVal) {
	...
	if rt.is_true(this.is_attachment_with_mime_type(var_instance_mutated.array_get(rt.new_string('attachment_id')), rt.get_property(rt.new_object('WP_Widget_Media_Video', [
		'WP_Widget_Media',
	...
	mut var_src := var_instance_mutated.array_get(rt.new_string('url'))
	...
	mut var_youtube_pattern :=
		rt.new_string('#^https?://(?:www\\.)?(?:youtube\\.com/watch|youtu\\.be/)#')
	mut var_vimeo_pattern := rt.new_string('#^https?://(.+\\.)?vimeo\\.com/.*#')
	if rt.is_true(var_attachment)
		|| rt.is_true(rt.call_function('preg_match', [var_youtube_pattern.clone(), var_src.clone()]))
		|| rt.is_true(rt.call_function('preg_match', [var_vimeo_pattern.clone(), var_src.clone()])) {
		...
		rt.echo_val(rt.call_function('wp_video_shortcode', [
			rt.call_function('array_merge', [var_instance_mutated.clone(),
...
fn (mut this Class_WP_Widget_Media_Video) enqueue_admin_scripts() {
	...
	mut var_handle := rt.new_string('media-video-widget')
...
fn (mut this Class_WP_Widget_Media_Video) render_control_template_scripts() {
	...
	rt.call_function('wp_admin_notice', [
		rt.get_property(rt.new_object('WP_Widget_Media_Video', ['WP_Widget_Media'], &this), 'l10n').array_get(rt.new_string('missing_attachment')),
	...
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('wp_admin_notice', [
		rt.get_property(rt.new_object('WP_Widget_Media_Video', ['WP_Widget_Media'], &this), 'l10n').array_get(rt.new_string('unsupported_file_type')),
		rt.create_array([rt.ArrayItem{ key: 'type', val: 'error' },
			rt.ArrayItem{ key: 'additional_classes', val: rt.create_array([
				rt.ArrayItem{ key: none, val: 'notice-alt' },
				rt.ArrayItem{ key: none, val: 'notice-missing-attachment' },
