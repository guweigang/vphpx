...
fn (mut this Class_WP_Widget_Media_Gallery) construct() {
	this.Class_WP_Widget_Media.construct(rt.new_string('media_gallery'), rt.call_function('__', [
		rt.new_string('Gallery'),
	]), rt.create_array([
		rt.ArrayItem{ key: 'description', val: rt.call_function('__', [
			rt.new_string('Displays an image gallery.'),
		]) },
		rt.ArrayItem{ key: 'mime_type', val: 'image' },
	]))
	this.dispatch_set_prop('l10n', rt.call_function('array_merge', [
		rt.get_property(rt.new_object('WP_Widget_Media_Gallery', ['WP_Widget_Media'], &this),
			'l10n'),
		rt.create_array([rt.ArrayItem{ key: 'no_media_selected', val: rt.call_function('__', [
			rt.new_string('No images selected'),
		]) }, rt.ArrayItem{ key: 'add_media', val: rt.call_function('_x', [
			rt.new_string('Add Images'),
			rt.new_string('label for button in the gallery widget; should not be longer than ~13 characters long'),
		]) }, rt.ArrayItem{ key: 'replace_media', val: '' }, rt.ArrayItem{ key: 'edit_media', val: rt.call_function('_x', [
...
fn (mut this Class_WP_Widget_Media_Gallery) get_instance_schema() rt.PhpVal {
	mut var_schema := rt.create_array([
		...
		rt.ArrayItem{ key: 'columns', val: rt.create_array([
			...
			rt.ArrayItem{ key: 'maximum', val: 9 },
		]) },
		...
		rt.ArrayItem{ key: 'orderby_random', val: rt.create_array([
			rt.ArrayItem{ key: 'type', val: 'boolean' },
			rt.ArrayItem{ key: 'default', val: false },
			rt.ArrayItem{ key: 'media_prop', val: '_orderbyRandom' },
			rt.ArrayItem{ key: 'should_preview_update', val: false },
		]) },
	])
	var_schema = rt.call_function('apply_filters', [
		rt.concat(rt.concat(rt.new_string('widget_'), rt.get_property(rt.new_object('WP_Widget_Media_Gallery', [
			'WP_Widget_Media',
		], &this), 'id_base')), rt.new_string('_instance_schema')),
		var_schema.clone(),
		rt.new_object('WP_Widget_Media_Gallery', [
			'WP_Widget_Media',
		], &this),
	])
...
fn (mut this Class_WP_Widget_Media_Gallery) render_media(var_instance rt.PhpVal) {
	...
	mut var_shortcode_atts := rt.call_function('array_merge', [
		var_instance_mutated.clone(),
		rt.create_array([
			rt.ArrayItem{
				key: 'link'
				val: var_instance_mutated.array_get(rt.new_string('link_type'))
			},
		])])
	if rt.is_true(var_instance_mutated.array_get(rt.new_string('orderby_random'))) {
		var_shortcode_atts.array_set('orderby', 'rand')
	}
	rt.echo_val(rt.call_function('gallery_shortcode', [var_shortcode_atts.clone()]))
}

fn (mut this Class_WP_Widget_Media_Gallery) enqueue_admin_scripts() {
	this.Class_WP_Widget_Media.enqueue_admin_scripts()
	mut var_handle := rt.new_string('media-gallery-widget')
	...
	rt.call_function('wp_add_inline_script', [var_handle.clone(),
		rt.call_function('sprintf', [
			...
			rt.call_function('wp_json_encode', [
				rt.get_property(rt.new_object('WP_Widget_Media_Gallery', [
					'WP_Widget_Media',
				], &this), 'id_base'),
...
fn (mut this Class_WP_Widget_Media_Gallery) render_control_template_scripts() {
	...
	rt.echo_val(rt.call_function('esc_attr', [
		rt.call_function('sprintf', [
			...
			rt.new_string('{{ attachment.filename }}'),
		]),
	]))
	// unsupported statement: Stmt_InlineHTML
	rt.call_function('printf', [
		rt.call_function('__', [
			rt.new_string('Additional images added to this gallery: %s'),
		]),
		rt.new_string('{{ data.ids.length - 5 }}'),
	])
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('esc_html', [rt.get_property(rt.new_object('WP_Widget_Media_Gallery', [
		'WP_Widget_Media',
	], &this), 'l10n').array_get(rt.new_string('add_media'))]))
	// unsupported statement: Stmt_InlineHTML
}

fn (mut this Class_WP_Widget_Media_Gallery) has_content(var_instance rt.PhpVal) bool {
	mut var_instance_mutated := var_instance
	if !(!rt.is_true(var_instance_mutated.array_get(rt.new_string('ids')))) {
		mut var_attachments := rt.call_function('wp_parse_id_list', [
			var_instance_mutated.array_get(rt.new_string('ids')),
		])
		...
		for {
			...
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('attachment'), rt.call_function('get_post_type', [
