...
fn (mut this Class_WP_Admin_Bar) add_node(var_args rt.PhpVal) {
	...
	if rt.is_true(rt.greater_equal(rt.call_function('func_num_args', []rt.PhpVal{}), rt.new_int(3)))
		&& var_args_mutated.clone().is_string() {
	...
	if !rt.is_true(var_args_mutated.array_get(rt.new_string('id'))) {
		...
		var_args_mutated.array_set('id', rt.call_function('esc_attr', [
			rt.call_function('sanitize_title', [
				rt.new_string(var_args_mutated.array_get(rt.new_string('title')).to_string().trim_space()),
			]),
		]))
	}
	...
	mut var_maybe_defaults := this.get_node(var_args_mutated.array_get(rt.new_string('id')))
	if rt.is_true(var_maybe_defaults) {
		var_defaults = rt.call_function('get_object_vars', [var_maybe_defaults.clone()])
	}
	if !(!rt.is_true(var_defaults.array_get(rt.new_string('meta'))))
		&& !(!rt.is_true(var_args_mutated.array_get(rt.new_string('meta')))) {
		var_args_mutated.array_set('meta', rt.call_function('wp_parse_args', [
			var_args_mutated.array_get(rt.new_string('meta')),
			var_defaults.array_get(rt.new_string('meta')),
		]))
	}
	...
	if var_args_mutated.array_get(rt.new_string('parent')).is_string()
		&& var_back_compat_parents.array_isset(var_args_mutated.array_get(rt.new_string('parent'))) {
		mut list_tmp_1 :=
			var_back_compat_parents.array_get(var_args_mutated.array_get(rt.new_string('parent')))
		var_new_parent = list_tmp_1.array_get(0)
		var_version = list_tmp_1.array_get(1)
		rt.call_function('_deprecated_argument', [rt.new_string(@METHOD),
			...
			rt.call_function('sprintf', [
				...
				var_args_mutated.array_get(rt.new_string('parent')),
			])])
...
fn (mut this Class_WP_Admin_Bar) _set_node(var_args rt.PhpVal) {
	mut var_args_mutated := var_args
	this.nodes.array_set(var_args_mutated.array_get(rt.new_string('id')),
...
fn (mut this Class_WP_Admin_Bar) _bind() rt.PhpVal {
	...
	mut iter_2 := this._get_nodes().iterator()
	for {
	...
	mut iter_3 := this._get_nodes().iterator()
	for {
		...
		if rt.is_true(rt.identical(rt.new_string('group'), rt.get_property(var_node, 'type'))) {
			if !rt.is_true(rt.get_property(var_node, 'meta').array_get(rt.new_string('class'))) {
				rt.get_property(var_node, 'meta').array_set('class', var_group_class.clone())
			} else {
				rt.get_property(var_node, 'meta').array_get(rt.new_string('class')) = rt.concat(rt.get_property(var_node,
					'meta').array_get(rt.new_string('class')), rt.new_string(' ' +
					var_group_class.str()))
			}
		}
		if rt.is_true(rt.identical(rt.new_string('item'), rt.get_property(var_parent, 'type')))
...
fn (mut this Class_WP_Admin_Bar) _render(var_root rt.PhpVal) {
	...
	// unsupported statement: Stmt_InlineHTML
	mut iter_4 := rt.get_property(var_root_mutated, 'children').iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_group := item_4.val
...
fn (mut this Class_WP_Admin_Bar) _render_container(var_node rt.PhpVal) {
	mut var_node_mutated := var_node
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('container'), rt.get_property(var_node_mutated, 'type')))))
	...
	mut iter_5 := rt.get_property(var_node_mutated, 'children').iterator()
	for {
...
fn (mut this Class_WP_Admin_Bar) _render_group(var_node rt.PhpVal, menu_title bool) {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string('group'), rt.get_property(var_node_mutated, 'type')))))
		|| !rt.is_true(rt.get_property(var_node_mutated, 'children')) {
		return
	}
	if !(!rt.is_true(rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('class')))) {
		mut var_class := rt.new_string(' class="' +
			(rt.call_function('esc_attr', [rt.new_string(rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('class')).to_string().trim_space())])).str() +
			'"')
	} else {
	...
	mut iter_6 := rt.get_property(var_node_mutated, 'children').iterator()
	for {
...
fn (mut this Class_WP_Admin_Bar) _render_item(var_node rt.PhpVal) {
	...
	mut var_tabindex := if
		...
		&& rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('tabindex')).is_long()
		|| rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('tabindex')).is_double() {
		rt.new_int((rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('tabindex'))).to_i64())
	} else {
		rt.new_string('')
	}
	...
	if !(!rt.is_true(rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('class')))) {
		var_menuclass = rt.concat(var_menuclass,
			rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('class')))
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(var_is_root_top_item))))
	...
	for var_attribute in var_attributes {
		if !rt.is_true(rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string(attribute))) {
			continue
		}
		if rt.is_true(rt.identical(rt.new_string('onclick'), rt.new_string(attribute))) {
			print(" ${var_attribute}='" +
				(rt.call_function('esc_js', [rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string(attribute))])).str() +
				"'")
		} else {
			print(" ${var_attribute}='" +
				(rt.call_function('esc_attr', [rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string(attribute))])).str() +
				"'")
		}
	}
	...
	if rt.is_true(var_is_parent) {
		...
		mut iter_7 := rt.get_property(var_node_mutated, 'children').iterator()
		for {
			item_7 := iter_7.next() or { break }
			mut var_group := item_7.val
			if !rt.is_true(rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('menu_title'))) {
				this._render_group(var_group.clone(), false)
			} else {
				this._render_group(var_group.clone(),
					(rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('menu_title'))).to_bool())
			}
		}
		print('</div>')
	}
	if !(!rt.is_true(rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('html')))) {
		rt.echo_val(rt.get_property(var_node_mutated, 'meta').array_get(rt.new_string('html')))
	}
	print('</li>')
}
...
fn create_wp_admin_bar(_args ...rt.PhpVal) &Class_WP_Admin_Bar {
