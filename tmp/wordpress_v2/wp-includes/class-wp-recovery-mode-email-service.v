...
fn (mut this Class_WP_Recovery_Mode_Email_Service) construct(mut var_link_service Class_WP_Recovery_Mode_Link_Service) {
	this.link_service = var_link_service
}
...
fn (mut this Class_WP_Recovery_Mode_Email_Service) maybe_send_recovery_mode_email(var_rate_limit rt.PhpVal, var_error rt.PhpVal, var_extension rt.PhpVal) bool {
	mut var_last_sent := rt.call_function('get_option', [
		rt.new_string(Class_WP_Recovery_Mode_Email_Service.rate_limit_option()),
	])
	if rt.is_true(rt.new_bool(!(rt.is_true(var_last_sent))))
		...
		mut var_sent := this.send_recovery_mode_email(var_rate_limit.clone(), var_error.clone(),
			var_extension.clone())
	...
	return (create_wp_error(rt.new_string('email_sent_already'), var_err_message.clone())).to_bool()
}
...
fn (mut this Class_WP_Recovery_Mode_Email_Service) send_recovery_mode_email(var_rate_limit rt.PhpVal, var_error rt.PhpVal, var_extension rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_extension) {
		mut var_cause := this.get_cause(var_extension.clone())
		...
		if rt.is_true(var_details) {
			...
			var_details = rt.new_string('\n\n' + var_header.str() + '\n' +
				(rt.call_function('str_pad', [rt.new_string(''), rt.new_int(var_header.clone().to_string().len), rt.new_string('=')])).str() +
				'\n' + var_details.str())
		}
	} else {
		var_cause = rt.new_string('')
		var_details = rt.new_string('')
	}
	...
	mut var_debug := rt.call_function('apply_filters', [
		...
		this.get_debug(var_extension.clone()),
	])
	...
	var_email = rt.call_function('apply_filters', [rt.new_string('recovery_mode_email'),
		var_email.clone(), var_url.clone()])
	mut var_sent := rt.call_function('wp_mail', [var_email.array_get(rt.new_string('to')),
		rt.call_function('wp_specialchars_decode', [
			rt.call_function('sprintf', [var_email.array_get(rt.new_string('subject')),
				var_blogname.clone()]),
		]),
		var_email.array_get(rt.new_string('message')), var_email.array_get(rt.new_string('headers')),
		var_email.array_get(rt.new_string('attachments'))])
	if rt.is_true(var_switched_locale) {
		rt.call_function('restore_previous_locale', []rt.PhpVal{})
	}
	return var_sent.clone()
}

fn (mut this Class_WP_Recovery_Mode_Email_Service) get_recovery_mode_email_address() rt.PhpVal {
	if rt.is_true(rt.call_function('defined', [rt.new_string('RECOVERY_MODE_EMAIL')]))
...
fn (mut this Class_WP_Recovery_Mode_Email_Service) get_cause(var_extension rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_string('plugin'),
		var_extension.array_get(rt.new_string('type'))))
	{
		mut var_plugin := rt.new_bool(this.get_plugin(var_extension.clone()))
		if rt.is_true(rt.identical(rt.new_bool(false), var_plugin)) {
			mut var_name := var_extension.array_get(rt.new_string('slug'))
		} else {
			var_name = var_plugin.array_get(rt.new_string('Name'))
		}
		...
		var_name = if rt.is_true(rt.call_method(var_theme, 'exists', []rt.PhpVal{})) { rt.call_method(var_theme, 'display', [
				rt.new_string('Name'),
			]) } else { var_extension.array_get(rt.new_string('slug')) }
...
fn (mut this Class_WP_Recovery_Mode_Email_Service) get_plugin(var_extension rt.PhpVal) bool {
	...
	if var_plugins.array_isset(rt.concat(rt.concat(rt.concat(var_extension.array_get(rt.new_string('slug')),
		rt.new_string('/')), var_extension.array_get(rt.new_string('slug'))), rt.new_string('.php')))
	{
		return (var_plugins.array_get(rt.new_string((rt.concat(rt.concat(rt.concat(var_extension.array_get(rt.new_string('slug')),
			rt.new_string('/')), var_extension.array_get(rt.new_string('slug'))),
			rt.new_string('.php'))).str()))).to_bool()
	} else {
		...
		for {
			...
			if rt.is_true(rt.call_function('str_starts_with', [var_file.clone(), rt.concat(var_extension.array_get(rt.new_string('slug')), rt.new_string('/'))]))
				|| rt.is_true(rt.identical(var_file, var_extension.array_get(rt.new_string('slug')))) {
...
fn (mut this Class_WP_Recovery_Mode_Email_Service) get_debug(var_extension rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(var_extension) {
		mut var_plugin := rt.new_bool(this.get_plugin(var_extension.clone()))
		if rt.is_true(rt.new_bool(var_plugin.clone().is_array())) {
