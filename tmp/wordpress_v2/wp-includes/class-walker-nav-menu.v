...
fn (mut this Class_Walker_Nav_Menu) start_lvl(var_output rt.PhpVal, depth i64, var_args rt.PhpVal) {
	...
	if !(rt.get_property(var_args_mutated, 'item_spacing')).is_null()
		&& rt.is_true(rt.identical(rt.new_string('discard'), rt.get_property(var_args_mutated, 'item_spacing'))) {
	...
	mut var_class_names := rt.call_function('implode', [rt.new_string(' '),
		rt.call_function('apply_filters', [rt.new_string('nav_menu_submenu_css_class'),
			var_classes.clone(), var_args_mutated.clone(), rt.new_int(depth)])])
	mut var_atts := rt.new_array()
	var_atts.array_set('class', if !(!rt.is_true(var_class_names)) {
		var_class_names
	} else {
		rt.new_string('')
	})
	var_atts = rt.call_function('apply_filters', [
		rt.new_string('nav_menu_submenu_attributes'),
		var_atts.clone(),
		var_args_mutated.clone(),
		rt.new_int(depth),
	])
	mut var_attributes := this.build_atts(var_atts.clone())
	var_output = rt.concat(var_output,
		rt.new_string('${var_n.to_string()}${var_indent.to_string()}<ul${var_attributes.to_string()}>${var_n.to_string()}'))
}
...
fn (mut this Class_Walker_Nav_Menu) start_el(var_output rt.PhpVal, var_data_object rt.PhpVal, depth i64, var_args rt.PhpVal, current_object_id i64) {
	...
	mut var_indent := if var_depth != 0 { rt.call_function('str_repeat', [
			var_t.clone(), rt.new_int(depth)]) } else { rt.new_string('') }
	mut var_classes := if !rt.is_true(rt.get_property(var_menu_item, 'classes')) {
		rt.new_array()
	} else {
		rt.cast_array(rt.get_property(var_menu_item, 'classes'))
	}
	var_classes.array_push('menu-item-' + (rt.get_property(var_menu_item, 'ID')).str())
	var_args_mutated = rt.call_function('apply_filters', [
		rt.new_string('nav_menu_item_args'),
		var_args_mutated.clone(),
		var_menu_item.clone(),
		rt.new_int(depth),
	])
	mut var_class_names := rt.call_function('implode', [rt.new_string(' '),
		rt.call_function('apply_filters', [rt.new_string('nav_menu_css_class'),
			rt.call_function('array_filter', [var_classes.clone()]),
			var_menu_item.clone(), var_args_mutated.clone(), rt.new_int(depth)])])
	mut var_id := rt.call_function('apply_filters', [rt.new_string('nav_menu_item_id'),
		rt.new_string('menu-item-' + (rt.get_property(var_menu_item, 'ID')).str()),
		var_menu_item.clone(), var_args_mutated.clone(), rt.new_int(depth)])
	mut var_li_atts := rt.new_array()
	var_li_atts.array_set('id', if !(!rt.is_true(var_id)) { var_id } else { rt.new_string('') })
	var_li_atts.array_set('class', if !(!rt.is_true(var_class_names)) {
		var_class_names
	} else {
		rt.new_string('')
	})
	var_li_atts = rt.call_function('apply_filters', [
		rt.new_string('nav_menu_item_attributes'),
		var_li_atts.clone(),
		var_menu_item.clone(),
		var_args_mutated.clone(),
		rt.new_int(depth),
	])
	mut var_li_attributes := this.build_atts(var_li_atts.clone())
	var_output = rt.concat(var_output, rt.new_string(var_indent.str() + '<li' +
		var_li_attributes.str() + '>'))
	...
	mut var_the_title_filtered := var_title.clone()
	var_title = rt.call_function('apply_filters', [rt.new_string('nav_menu_item_title'),
		var_title.clone(), var_menu_item.clone(), var_args_mutated.clone(),
		rt.new_int(depth)])
	mut var_atts := rt.new_array()
	var_atts.array_set('target', if !(!rt.is_true(rt.get_property(var_menu_item, 'target'))) {
		rt.get_property(var_menu_item, 'target')
	} else {
		rt.new_string('')
	})
	var_atts.array_set('rel', if !(!rt.is_true(rt.get_property(var_menu_item, 'xfn'))) {
	...
	if !(!rt.is_true(rt.get_property(var_menu_item, 'url'))) {
		if rt.is_true(rt.identical(this.privacy_policy_url, rt.get_property(var_menu_item, 'url'))) {
			var_atts.array_set('rel', if !rt.is_true(var_atts.array_get(rt.new_string('rel'))) {
				'privacy-policy'
			} else {
				(var_atts.array_get(rt.new_string('rel'))).str() + ' privacy-policy'
			})
		}
	...
	var_atts.array_set('aria-current', if rt.is_true(rt.get_property(var_menu_item, 'current')) {
		'page'
	} else {
		''
	})
	if !(!rt.is_true(rt.get_property(var_menu_item, 'attr_title')))
		&& rt.is_true(rt.new_bool(rt.get_property(var_menu_item, 'attr_title').to_string().to_lower().trim_space() != rt.get_property(var_menu_item, 'title').to_string().to_lower().trim_space()))
		&& rt.is_true(rt.new_bool(rt.get_property(var_menu_item, 'attr_title').to_string().to_lower().trim_space() != var_the_title_filtered.clone().to_string().to_lower().trim_space()))
		&& rt.is_true(rt.new_bool(rt.get_property(var_menu_item, 'attr_title').to_string().to_lower().trim_space() != var_title.clone().to_string().to_lower().trim_space())) {
		var_atts.array_set('title', rt.get_property(var_menu_item, 'attr_title'))
	} else {
		var_atts.array_set('title', '')
	}
	var_atts = rt.call_function('apply_filters', [
		rt.new_string('nav_menu_link_attributes'),
		var_atts.clone(),
		var_menu_item.clone(),
		var_args_mutated.clone(),
		rt.new_int(depth),
	...
	var_item_output = rt.concat(var_item_output, rt.new_string('<a' + var_attributes.str() + '>'))
	var_item_output = rt.concat(var_item_output, rt.new_string(
		(rt.get_property(var_args_mutated, 'link_before')).str() + var_title.str() +
		(rt.get_property(var_args_mutated, 'link_after')).str()))
	var_item_output = rt.concat(var_item_output, rt.new_string('</a>'))
	var_item_output = rt.concat(var_item_output, rt.get_property(var_args_mutated, 'after'))
	var_output = rt.concat(var_output, rt.call_function('apply_filters', [
		rt.new_string('walker_nav_menu_start_el'),
		var_item_output.clone(),
		var_menu_item.clone(),
		rt.new_int(depth),
		var_args_mutated.clone(),
	]))
...
fn (mut this Class_Walker_Nav_Menu) build_atts(var_atts rt.PhpVal) rt.PhpVal {
	...
	mut var_attribute_string := rt.new_string('')
	...
	for {
		...
		if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_value))))
			...
			&& rt.is_true(rt.call_function('is_scalar', [var_value.clone()])) {
			...
			var_attribute_string = rt.concat(var_attribute_string, rt.new_string(' ' +
				var_attr.str() + '="' + var_value.str() + '"'))
		}
	}
	return var_attribute_string.clone()
}
