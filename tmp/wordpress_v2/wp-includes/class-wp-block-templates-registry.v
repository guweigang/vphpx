...
fn init_static_wp_block_templates_registry() {
	rt.init_static_prop('WP_Block_Templates_Registry', 'instance', rt.new_null())
}
...
fn (mut this Class_WP_Block_Templates_Registry) register(var_template_name rt.PhpVal, var_args rt.PhpVal) rt.PhpVal {
	...
	mut var_error_code := rt.new_string('')
	if !(var_template_name.clone().is_string()) {
		var_error_message = rt.call_function('__', [
			rt.new_string('Template names must be strings.'),
		])
		var_error_code = rt.new_string('template_name_no_string')
	} else if rt.is_true(rt.call_function('preg_match', [rt.new_string('/[A-Z]+/'),
		var_template_name.clone()]))
	{
		var_error_message = rt.call_function('__', [
			rt.new_string('Template names must not contain uppercase characters.'),
		])
		var_error_code = rt.new_string('template_name_no_uppercase')
	} else if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('preg_match', [
		rt.new_string('/^[a-z0-9_\\-]+\\/\\/[a-z0-9_\\-]+$/'),
		var_template_name.clone(),
	])))))
	{
		var_error_message = rt.call_function('__', [
			rt.new_string('Template names must contain a namespace prefix. Example: my-plugin//my-custom-template'),
		])
		var_error_code = rt.new_string('template_no_prefix')
	} else if rt.is_true(this.is_registered(var_template_name.clone())) {
		var_error_message = rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('Template "%s" is already registered.'),
			]),
			var_template_name.clone(),
		])
		var_error_code = rt.new_string('template_already_registered')
	}
	if rt.is_true(var_error_message) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			var_error_message.clone(), rt.new_string('6.7.0')])
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_template)))) {
		...
		mut list_tmp_1 := rt.call_function('explode', [rt.new_string('//'),
			var_template_name.clone()])
		var_plugin = list_tmp_1.array_get(0)
		var_slug = list_tmp_1.array_get(1)
		mut var_default_template_types := rt.call_function('get_default_block_template_types',
			[]rt.PhpVal{})
		var_template = create_wp_block_template()
		rt.set_property(var_template, 'id', var_theme_name.str() + '//' + var_slug.str())
		rt.set_property(var_template, 'theme', var_theme_name.clone())
		rt.set_property(var_template, 'plugin', var_plugin.clone())
		rt.set_property(var_template, 'author', rt.new_null())
		rt.set_property(var_template, 'content', if !(var_args.array_get(rt.new_string('content'))).is_null() {
			var_args.array_get(rt.new_string('content'))
		} else {
			rt.new_string('')
		})
		rt.set_property(var_template, 'source', rt.new_string('plugin'))
		rt.set_property(var_template, 'slug', var_slug.clone())
		rt.set_property(var_template, 'type', rt.new_string('wp_template'))
		rt.set_property(var_template, 'title', if !(var_args.array_get(rt.new_string('title'))).is_null() {
			var_args.array_get(rt.new_string('title'))
		} else {
			var_template_name
		})
		rt.set_property(var_template, 'description', if !(var_args.array_get(rt.new_string('description'))).is_null() {
		...
		rt.set_property(var_template, 'is_custom',
			rt.new_bool(!(var_default_template_types.array_isset(var_template_name))))
		rt.set_property(var_template, 'post_types', if !(var_args.array_get(rt.new_string('post_types'))).is_null() {
			var_args.array_get(rt.new_string('post_types'))
		} else {
			rt.new_array()
		})
	}
	this.registered_templates.array_set(var_template_name, var_template.clone())
...
fn (mut this Class_WP_Block_Templates_Registry) get_registered(var_template_name rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(this.is_registered(var_template_name.clone()))))) {
		return rt.new_null()
	}
...
fn (mut this Class_WP_Block_Templates_Registry) get_by_query(var_query rt.PhpVal) rt.PhpVal {
	...
	var_query_mutated = rt.call_function('wp_parse_args', [var_query_mutated.clone(),
		rt.create_array([rt.ArrayItem{ key: 'slug__in', val: rt.new_array() },
			rt.ArrayItem{ key: 'slug__not_in', val: rt.new_array() },
			rt.ArrayItem{ key: 'post_type', val: '' }])])
	mut var_slugs_to_include := var_query_mutated.array_get(rt.new_string('slug__in'))
	mut var_slugs_to_skip := var_query_mutated.array_get(rt.new_string('slug__not_in'))
	mut var_post_type := var_query_mutated.array_get(rt.new_string('post_type'))
	mut var_matching_templates := rt.new_array()
	mut iter_2 := var_all_templates.iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_template := item_2.val
		mut var_template_name := item_2.key
		if rt.is_true(var_slugs_to_include)
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [rt.get_property(var_template, 'slug'), var_slugs_to_include.clone(), rt.new_bool(true)]))))) {
			continue
		}
		if rt.is_true(var_slugs_to_skip)
			&& rt.is_true(rt.call_function('in_array', [rt.get_property(var_template, 'slug'), var_slugs_to_skip.clone(), rt.new_bool(true)])) {
			continue
		}
		if rt.is_true(var_post_type)
			&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [var_post_type.clone(), rt.get_property(var_template, 'post_types'), rt.new_bool(true)]))))) {
			continue
		}
		var_matching_templates.array_set(var_template_name, var_template.clone())
	}
	return var_matching_templates.clone()
}

fn (mut this Class_WP_Block_Templates_Registry) is_registered(var_template_name rt.PhpVal) rt.PhpVal {
	return rt.new_bool(!var_template_name.is_null()
		&& this.registered_templates.array_isset(var_template_name))
}
...
fn (mut this Class_WP_Block_Templates_Registry) unregister(var_template_name rt.PhpVal) rt.PhpVal {
	if rt.is_true(rt.new_bool(!(rt.is_true(this.is_registered(var_template_name.clone()))))) {
		mut var_error_message := rt.call_function('sprintf', [
			rt.call_function('__', [rt.new_string('Template "%s" is not registered.')]),
			var_template_name.clone(),
		])
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			var_error_message.clone(), rt.new_string('6.7.0')])
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('template_not_registered'),
			var_error_message.clone()))
	}
	...
	return var_unregistered_template.clone()
}

fn Class_WP_Block_Templates_Registry.get_instance() rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_null(), rt.get_static_prop('WP_Block_Templates_Registry',
		'instance')))
	{
		rt.set_static_prop('WP_Block_Templates_Registry', 'instance', rt.new_object('WP_Block_Templates_Registry',
			[]string{}, create_wp_block_templates_registry()))
	}
	return rt.get_static_prop('WP_Block_Templates_Registry', 'instance')
}
...
fn create_wp_block_template(_args ...rt.PhpVal) &Class_WP_Block_Template {
