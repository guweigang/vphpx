...
fn (mut this Class_SimplePie_HTTP_FileClient) request(method string, url string, mut var_headers Class_SimplePie_HTTP_array) rt.PhpVal {
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_string(method),
		Class_SimplePie_HTTP_SimplePie_HTTP_FileClient.method_get()))))
	{
	...
	mut var_file := rt.call_method(this.registry, 'create', [
		Class_SimplePie_File.class(),
		rt.create_array([rt.ArrayItem{ key: none, val: url },
			rt.ArrayItem{
				key: none
				val: if !(this.options.array_get(rt.new_string('timeout'))).is_null() {
					this.options.array_get(rt.new_string('timeout'))
				} else {
					rt.new_int(10)
				}
			}, rt.ArrayItem{
				key: none
				val: if !(this.options.array_get(rt.new_string('redirects'))).is_null() {
					this.options.array_get(rt.new_string('redirects'))
				} else {
					rt.new_int(5)
				}
			}, rt.ArrayItem{ key: none, val: var_headers }, rt.ArrayItem{
				key: none
				val: if !(this.options.array_get(rt.new_string('useragent'))).is_null() {
					this.options.array_get(rt.new_string('useragent'))
				} else {
					iife_result_0
				}
			}, rt.ArrayItem{
				key: none
				val: if !(this.options.array_get(rt.new_string('force_fsockopen'))).is_null() {
					this.options.array_get(rt.new_string('force_fsockopen'))
				} else {
					rt.new_bool(false)
				}
			}, rt.ArrayItem{
				key: none
				val: if !(this.options.array_get(rt.new_string('curl_options'))).is_null() {
					this.options.array_get(rt.new_string('curl_options'))
				} else {
					rt.new_array()
				}
			}]),
	...
	end_label_1:
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.get_property(var_file, 'error'), rt.new_null()))))
		&& rt.is_true(rt.identical(rt.call_method(var_file, 'get_status_code', []rt.PhpVal{}), rt.new_int(0))) {
		rt.throw_exception(rt.new_object('SimplePie_HTTP_ClientException', []string{}, create_simplepie_http_clientexception(rt.get_property(var_file,
			'error'))))
	}
...
fn (mut this Class_SimplePie_HTTP_FileClient) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		'__construct' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_SimplePie_Registry](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
			mut dispatch_arg_1 := rt.cast_object_ptr[Class_SimplePie_HTTP_array](if args.len > 1 {
				args[1]
			} else {
				rt.new_null()
			})
