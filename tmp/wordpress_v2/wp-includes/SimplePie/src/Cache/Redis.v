...
fn (mut this Class_SimplePie_Cache_Redis) construct(location string, name string, var_options rt.PhpVal) {
	...
	var_redis.connect(var_parsed.array_get(rt.new_string('host')),
		var_parsed.array_get(rt.new_string('port')))
	if var_parsed.array_isset(rt.new_string('pass')) {
		var_redis.auth(var_parsed.array_get(rt.new_string('pass')))
	}
	if var_parsed.array_isset(rt.new_string('path')) {
		var_redis.select(rt.new_int((rt.call_function('substr', [
			var_parsed.array_get(rt.new_string('path')),
			rt.new_int(1),
		])).to_i64()))
	}
	this.cache = var_redis
	if !(var_options.clone().is_null()) && var_options.clone().is_array() {
		this.options = var_options.clone()
	} else {
		this.options = rt.create_array([
			rt.ArrayItem{ key: 'prefix', val: 'rss:simple_primary:' },
			rt.ArrayItem{ key: 'expire', val: 0 },
		])
	}
	this.name = (this.options.array_get(rt.new_string('prefix'))).str() + name
}
...
fn (mut this Class_SimplePie_Cache_Redis) save(var_data rt.PhpVal) rt.PhpVal {
	...
	mut var_response := rt.call_method(this.cache, 'set', [this.name,
		rt.call_function('serialize', [var_data_mutated.clone()])])
	if rt.is_true(this.options.array_get(rt.new_string('expire'))) {
		rt.call_method(this.cache, 'expire',
			[this.name, this.options.array_get(rt.new_string('expire'))])
	}
...
fn (mut this Class_SimplePie_Cache_Redis) touch() bool {
	mut var_data := rt.call_method(this.cache, 'get', [this.name])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_data, rt.new_bool(false))))) {
		mut var_return := rt.call_method(this.cache, 'set', [this.name, var_data.clone()])
		if rt.is_true(this.options.array_get(rt.new_string('expire'))) {
			return (rt.call_method(this.cache, 'expire', [this.name,
				this.options.array_get(rt.new_string('expire'))])).to_bool()
		}
...
fn create_redis(_args ...rt.PhpVal) &Class_Redis {
	mut obj := &Class_Redis{
		PhpObjectBase: rt.PhpObjectBase{}
	}
	return obj
...
fn (mut this Class_SimplePie_Cache_Redis) dispatch_method(method_name string, args []rt.PhpVal) ?rt.PhpVal {
	match method_name {
		...
		'setRedisClient' {
			mut dispatch_arg_0 := rt.cast_object_ptr[Class_Redis](if args.len > 0 {
				args[0]
			} else {
				rt.new_null()
			})
...
fn main() {
	...
	rt.call_function('class_alias', [rt.new_string('SimplePie\\Cache\\Redis'),
		rt.new_string('SimplePie_Cache_Redis')])
}
