...
fn (mut this Class_SimplePie_Cache_Memcache) save(var_data rt.PhpVal) rt.PhpVal {
	...
	return rt.call_method(this.cache, 'set', [this.name,
		rt.call_function('serialize', [var_data_mutated.clone()]),
		rt.get_constant('MEMCACHE_COMPRESSED'),
...
fn (mut this Class_SimplePie_Cache_Memcache) touch() bool {
	mut var_data := rt.call_method(this.cache, 'get', [this.name])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(var_data, rt.new_bool(false))))) {
		return (rt.call_method(this.cache, 'set', [this.name, var_data.clone(),
			rt.get_constant('MEMCACHE_COMPRESSED'),
...
fn create_memcache(_args ...rt.PhpVal) &Class_Memcache {
...
fn main() {
	...
	rt.call_function('class_alias', [rt.new_string('SimplePie\\Cache\\Memcache'),
		rt.new_string('SimplePie_Cache_Memcache')])
}
