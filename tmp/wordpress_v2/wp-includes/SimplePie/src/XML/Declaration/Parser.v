...
fn (mut this Class_SimplePie_XML_Declaration_Parser) parse() bool {
	for rt.is_true(this.state)
		&& rt.is_true(rt.new_bool(!rt.is_true(rt.identical(this.state, Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_emit()))))
		&& this.has_data() {
		mut var_state := this.state
		rt.call_method(rt.new_object('SimplePie_XML_Declaration_Parser', []string{}, &this),
			var_state, []rt.PhpVal{})
	}
	this.data = ''
	if rt.is_true(rt.identical(this.state,
		Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_emit()))
	{
		return true
	}
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) skip_whitespace() rt.PhpVal {
	mut var_whitespace := rt.call_function('strspn', [rt.new_string(this.data),
		rt.new_string('\t\n\r '), this.position])
	this.position = rt.add(this.position, var_whitespace)
	return var_whitespace.clone()
}

fn (mut this Class_SimplePie_XML_Declaration_Parser) get_value() bool {
	mut var_quote := rt.call_function('substr', [rt.new_string(this.data), this.position,
		rt.new_int(1)])
	if rt.is_true(rt.identical(var_quote, rt.new_string('"')))
		|| rt.is_true(rt.identical(var_quote, rt.new_string("'"))) {
		rt.post_inc(this.position)
		mut var_len := rt.call_function('strcspn', [rt.new_string(this.data),
			var_quote.clone(), this.position])
		if this.has_data() {
			mut var_value := rt.call_function('substr', [rt.new_string(this.data), this.position,
				var_len.clone()])
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) before_version_name() {
	if rt.is_true(this.skip_whitespace()) {
		this.state =
			Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_version_name()
	} else {
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) version_name() {
	if rt.is_true(rt.identical(rt.call_function('substr', [rt.new_string(this.data), this.position,
		rt.new_int(7)]), rt.new_string('version')))
	{
		this.position = rt.add(this.position, rt.new_int(7))
		this.skip_whitespace()
		this.state =
			Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_version_equals()
	} else {
		this.state = Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_error()
	}
}
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) version_equals() {
		...
		rt.new_int(1)]), rt.new_string('=')))
	{
		...
		this.skip_whitespace()
		this.state =
			Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_version_value()
	} else {
		this.state = Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_error()
	}
}
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) version_value() {
	mut var_version := rt.new_bool(this.get_value())
	if rt.is_true(var_version) {
		this.version = var_version.clone()
		this.skip_whitespace()
		if this.has_data() {
			this.state =
				Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_encoding_name()
		} else {
			this.state =
				Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_emit()
		}
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) encoding_name() {
	if rt.is_true(rt.identical(rt.call_function('substr', [rt.new_string(this.data), this.position,
		rt.new_int(8)]), rt.new_string('encoding')))
	{
		this.position = rt.add(this.position, rt.new_int(8))
		this.skip_whitespace()
		this.state =
			Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_encoding_equals()
	} else {
		this.state =
			Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_standalone_name()
	}
}
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) encoding_equals() {
		...
		rt.new_int(1)]), rt.new_string('=')))
	{
		...
		this.skip_whitespace()
		this.state =
			Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_encoding_value()
	} else {
		this.state = Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_error()
	}
}
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) encoding_value() {
	mut var_encoding := rt.new_bool(this.get_value())
	if rt.is_true(var_encoding) {
		this.encoding = var_encoding.clone()
		this.skip_whitespace()
		if this.has_data() {
			this.state =
				Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_standalone_name()
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) standalone_name() {
	if rt.is_true(rt.identical(rt.call_function('substr', [rt.new_string(this.data), this.position,
		rt.new_int(10)]), rt.new_string('standalone')))
	{
		this.position = rt.add(this.position, rt.new_int(10))
		this.skip_whitespace()
		this.state =
			Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_standalone_equals()
	} else {
		this.state = Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_error()
	}
}
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) standalone_equals() {
		...
		rt.new_int(1)]), rt.new_string('=')))
	{
		...
		this.skip_whitespace()
		this.state =
			Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_standalone_value()
	} else {
		this.state = Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_error()
	}
}
...
fn (mut this Class_SimplePie_XML_Declaration_Parser) standalone_value() {
	mut var_standalone := rt.new_bool(this.get_value())
	if rt.is_true(var_standalone) {
		...
		if rt.is_true(rt.equal(switch_val_1, rt.new_string('yes'))) {
			...
			this.state =
				Class_SimplePie_XML_Declaration_SimplePie_XML_Declaration_Parser.state_error()
			return
		}
		this.skip_whitespace()
		if this.has_data() {
...
fn main() {
	...
	rt.call_function('class_alias', [
		...
		rt.new_string('SimplePie_XML_Declaration_Parser'),
	])
}
