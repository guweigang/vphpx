...
fn init_static_simplepie_cache() {
	rt.init_static_prop('SimplePie_Cache', 'handlers', rt.create_array([
		rt.ArrayItem{ key: 'mysql', val: Class_SimplePie_Cache_MySQL.class() },
		rt.ArrayItem{ key: 'memcache', val: Class_SimplePie_Cache_Memcache.class() },
		rt.ArrayItem{ key: 'memcached', val: Class_SimplePie_Cache_Memcached.class() },
		rt.ArrayItem{ key: 'redis', val: Class_SimplePie_Cache_Redis.class() },
	]))
}
...
fn Class_SimplePie_Cache.get_handler(location string, filename string, var_extension rt.PhpVal) rt.PhpVal {
	mut var_type := rt.call_function('explode', [rt.new_string(':'),
		rt.new_string(location), rt.new_int(2)])
	var_type = var_type.array_get(rt.new_int(0))
	if !(!rt.is_true(rt.get_static_prop('SimplePie_Cache', 'handlers').array_get(var_type))) {
		mut var_class := rt.get_static_prop('SimplePie_Cache', 'handlers').array_get(var_type)
	...
	return rt.new_object('SimplePie_SimplePie_Cache_File', []string{}, create_simplepie_simplepie_cache_file(rt.new_string(location),
		rt.new_string(filename), var_extension.clone()))
}
...
fn (mut this Class_SimplePie_Cache) create(location string, filename string, var_extension rt.PhpVal) rt.PhpVal {
	rt.call_function('trigger_error', [
		rt.new_string('Cache::create() has been replaced with Cache::get_handler() since SimplePie 1.3.1, use the registry system instead.'),
		rt.get_constant('E_USER_DEPRECATED'),
	])
	return Class_SimplePie_Cache.get_handler(location, filename, var_extension.clone())
}

fn Class_SimplePie_Cache.register(type string, var_class rt.PhpVal) {
	mut type_mutated := type
	mut var_class_mutated := var_class
	rt.get_static_prop('SimplePie_Cache', 'handlers').array_set(type_mutated,
...
fn Class_SimplePie_Cache.parse_url(url string) rt.PhpVal {
	...
	mut var_params := rt.call_function('array_merge', [var_parsedUrl.clone(),
		rt.create_array([rt.ArrayItem{ key: 'extras', val: rt.new_array() }])])
	if var_params.array_isset(rt.new_string('query')) {
		rt.call_function('parse_str', [var_params.array_get(rt.new_string('query')),
			var_params.array_get(rt.new_string('extras'))])
	}
...
fn create_simplepie_simplepie_cache_file(_args ...rt.PhpVal) &Class_SimplePie_SimplePie_Cache_File {
...
fn main() {
	...
	rt.call_function('class_alias', [rt.new_string('SimplePie\\Cache'),
		rt.new_string('SimplePie_Cache')])
}
