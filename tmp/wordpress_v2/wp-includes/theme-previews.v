...
fn wp_get_theme_preview_path(var_current_stylesheet rt.PhpVal) rt.PhpVal {
	mut var_preview_stylesheet := rt.new_null()
	mut var_wp_theme := rt.new_null()
	mut var_theme_path := rt.new_null()
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('current_user_can', [
		rt.new_string('switch_themes'),
	])))))
	{
		return var_current_stylesheet.clone()
	}
	var_preview_stylesheet = if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('wp_theme_preview')))) { rt.call_function('sanitize_text_field', [
			rt.call_function('wp_unslash', [
				rt.get_superglobal('_GET').array_get(rt.new_string('wp_theme_preview')),
			]),
		]) } else { rt.new_null() }
	var_wp_theme = rt.call_function('wp_get_theme', [var_preview_stylesheet.clone()])
	...
	])))))
	{
		...
		{
			var_theme_path = rt.call_method(var_wp_theme, 'get_template', []rt.PhpVal{})
		} else {
			var_theme_path = rt.call_method(var_wp_theme, 'get_stylesheet', []rt.PhpVal{})
		}
		return rt.call_function('sanitize_text_field', [var_theme_path.clone()])
	}
	return var_current_stylesheet.clone()
}
...
fn wp_block_theme_activate_nonce() {
	mut var_nonce_handle := rt.new_null()
	var_nonce_handle = rt.new_string('switch-theme_' +
		(wp_get_theme_preview_path(rt.new_null())).str())
	// unsupported statement: Stmt_InlineHTML
	rt.echo_val(rt.call_function('wp_json_encode', [
		rt.call_function('wp_create_nonce', [var_nonce_handle.clone()]),
...
fn wp_initialize_theme_preview_hooks() {
	if !(!rt.is_true(rt.get_superglobal('_GET').array_get(rt.new_string('wp_theme_preview')))) {
