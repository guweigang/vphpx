...
fn wp_cache_supports(var_feature rt.PhpVal) bool {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('add_multiple')))
		|| rt.is_true(rt.equal(switch_val_1, rt.new_string('set_multiple')))
		|| rt.is_true(rt.equal(switch_val_1, rt.new_string('get_multiple')))
		|| rt.is_true(rt.equal(switch_val_1, rt.new_string('delete_multiple')))
		|| rt.is_true(rt.equal(switch_val_1, rt.new_string('flush_runtime')))
		|| rt.is_true(rt.equal(switch_val_1, rt.new_string('flush_group'))) {
		return true
	} else {
		return false
	}
...
fn wp_cache_switch_to_blog(var_blog_id rt.PhpVal) {
	mut var_wp_object_cache := rt.new_null()
	rt.call_method(var_wp_object_cache, 'switch_to_blog', [var_blog_id.clone()])
}

fn wp_cache_reset() {
	mut var_wp_object_cache := rt.new_null()
	rt.call_function('_deprecated_function', [rt.new_string(@FN),
		rt.new_string('3.5.0'), rt.new_string('wp_cache_switch_to_blog()')])
