...
fn (mut this Class_Translation_Entry) construct(var_args rt.PhpVal) {
	...
	for {
		...
		this.dispatch_set_prop('{"nodeType":"Expr_Variable","line":61,"name":"varname"}',
			var_value.clone())
	}
	if var_args.array_isset(rt.new_string('plural'))
		&& rt.is_true(var_args.array_get(rt.new_string('plural'))) {
		this.is_plural = true
	}
	if !(this.translations.is_array()) {
		this.translations = rt.new_array()
	}
	if !(this.references.is_array()) {
		this.references = rt.new_array()
	}
	if !(this.flags.is_array()) {
		this.flags = rt.new_array()
	}
}

fn (mut this Class_Translation_Entry) translation_entry(var_args rt.PhpVal) {
	rt.call_function('_deprecated_constructor', [Class_Translation_Entry.class(),
		rt.new_string('5.4.0'), Class_static.class()])
	mut iife_temp_0 := Class_Translation_Entry{}
	iife_temp_0.construct(var_args.clone())
	rt.new_null()
}
...
fn (mut this Class_Translation_Entry) key() bool {
	...
	mut var_key := if rt.is_true(rt.new_bool(!(rt.is_true(this.context)))) {
		this.singular
	} else {
		(this.context).str() + '' + (this.singular).str()
	}
	var_key = rt.call_function('str_replace', [
...
fn (mut this Class_Translation_Entry) merge_with(var_other rt.PhpVal) {
	this.flags = rt.call_function('array_unique', [
		rt.call_function('array_merge', [this.flags, rt.get_property(var_other, 'flags')]),
	])
	this.references = rt.call_function('array_unique', [
		rt.call_function('array_merge', [this.references, rt.get_property(var_other, 'references')]),
	])
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(this.extracted_comments, rt.get_property(var_other,
		'extracted_comments')))))
	{
		this.extracted_comments = rt.concat(this.extracted_comments, rt.get_property(var_other,
			'extracted_comments'))
	}
}
...
fn (mut this Class_Translation_Entry) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		...
		'translator_comments' {
			this.translator_comments = val
			return true
		}
		'extracted_comments' {
			this.extracted_comments = val
			return true
		}
		'references' {
			this.references = val
...
fn main() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('Translation_Entry'),
		rt.new_bool(false),
	])))))
	{
	}
