...
fn (mut this Class_Translations) add_entry(var_entry rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(var_entry_mutated.clone().is_array())) {
		var_entry_mutated = create_translation_entry(var_entry_mutated.clone())
	}
	mut var_key := var_entry_mutated.key()
	if rt.is_true(rt.identical(rt.new_bool(false), var_key)) {
		return false
	}
	this.entries.array_get(var_key) = var_entry_mutated
	return true
}
...
fn (mut this Class_Translations) add_entry_or_merge(var_entry rt.PhpVal) bool {
	...
	if this.entries.array_isset(var_key) {
		rt.call_method(this.entries.array_get(var_key), 'merge_with', [
			var_entry_mutated.clone()])
	} else {
		this.entries.array_get(var_key) = var_entry_mutated
	}
	return true
...
fn (mut this Class_Translations) set_header(var_header rt.PhpVal, var_value rt.PhpVal) {
	this.headers.array_set(var_header, var_value.clone())
}
...
fn (mut this Class_Translations) set_headers(var_headers rt.PhpVal) {
	...
	for {
		...
		this.set_header(var_header.clone(), var_value.clone())
	}
}

fn (mut this Class_Translations) get_header(var_header rt.PhpVal) rt.PhpVal {
	return if !(this.headers.array_get(var_header)).is_null() {
...
fn (mut this Class_Translations) translate_entry(var_entry rt.PhpVal) rt.PhpVal {
	mut var_entry_mutated := var_entry
	mut var_key := var_entry_mutated.key()
	return if !(this.entries.array_get(var_key)).is_null() {
		this.entries.array_get(var_key)
	} else {
		rt.new_bool(false)
	}
}
...
fn (mut this Class_Translations) translate(var_singular rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	mut var_entry := create_translation_entry(rt.create_array([
		rt.ArrayItem{ key: 'singular', val: var_singular },
		rt.ArrayItem{ key: 'context', val: var_context },
	]))
	mut var_translated := this.translate_entry(rt.new_object('Translation_Entry', []string{},
		var_entry))
	return if rt.is_true(var_translated)
		&& !(!rt.is_true(rt.get_property(var_translated, 'translations'))) {
		rt.get_property(var_translated, 'translations').array_get(rt.new_int(0))
	} else {
		var_singular
	}
}
...
fn (mut this Class_Translations) select_plural_form(var_count rt.PhpVal) i64 {
	return if 1 == rt.new_int(var_count.to_i64()) { 0 } else { 1 }
}
...
fn (mut this Class_Translations) translate_plural(var_singular rt.PhpVal, var_plural rt.PhpVal, var_count rt.PhpVal, var_context rt.PhpVal) rt.PhpVal {
	mut var_entry := create_translation_entry(rt.create_array([
		rt.ArrayItem{ key: 'singular', val: var_singular },
		rt.ArrayItem{ key: 'plural', val: var_plural },
		rt.ArrayItem{ key: 'context', val: var_context },
	]))
	mut var_translated := this.translate_entry(rt.new_object('Translation_Entry', []string{},
		var_entry))
	mut var_index := rt.new_int(this.select_plural_form(var_count.clone()))
	mut var_total_plural_forms := rt.new_int(this.get_plural_forms_count())
	if rt.is_true(var_translated) && rt.is_true(rt.less_equal(rt.new_int(0), var_index))
		&& rt.is_true(rt.less(var_index, var_total_plural_forms))
		&& rt.get_property(var_translated, 'translations').is_array()
		&& rt.get_property(var_translated, 'translations').array_isset(var_index) {
		return rt.get_property(var_translated, 'translations').array_get(var_index)
	} else {
		return if 1 == rt.new_int(var_count.to_i64()) { var_singular } else { var_plural }
	}
	return rt.new_null()
}

fn (mut this Class_Translations) merge_with(var_other rt.PhpVal) {
	mut iter_2 := rt.get_property(var_other, 'entries').iterator()
	for {
		item_2 := iter_2.next() or { break }
		mut var_entry := item_2.val
		this.entries.array_set(rt.call_method(var_entry, 'key', []rt.PhpVal{}), var_entry.clone())
	}
}

fn (mut this Class_Translations) merge_originals_with(var_other rt.PhpVal) {
	mut iter_3 := rt.get_property(var_other, 'entries').iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_entry := item_3.val
		if !(this.entries.array_isset(rt.call_method(var_entry, 'key', []rt.PhpVal{}))) {
			this.entries.array_set(rt.call_method(var_entry, 'key', []rt.PhpVal{}),
				var_entry.clone())
		} else {
			rt.call_method(this.entries.array_get(rt.call_method(var_entry, 'key', []rt.PhpVal{})),
				'merge_with', [var_entry.clone()])
		}
	}
}
...
fn (mut this Class_Gettext_Translations) gettext_select_plural_form(var_count rt.PhpVal) rt.PhpVal {
	...
	if !(!(this._gettext_select_plural_form).is_null())
		|| this._gettext_select_plural_form.is_null() {
		mut list_tmp_1 :=
			this.nplurals_and_expression_from_header(this.get_header(rt.new_string('Plural-Forms')))
		var_nplurals = list_tmp_1.array_get(0)
		var_expression = list_tmp_1.array_get(1)
		this._nplurals = var_nplurals.clone()
		this._gettext_select_plural_form = this.make_plural_form_function(var_nplurals.clone(),
			var_expression.clone())
	}
	return rt.call_function('call_user_func', [this._gettext_select_plural_form, var_count.clone()])
}
...
fn (mut this Class_Gettext_Translations) nplurals_and_expression_from_header(var_header rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('preg_match', [
		rt.new_string('/^\\s*nplurals\\s*=\\s*(\\d+)\\s*;\\s+plural\\s*=\\s*(.+)$/'),
		var_header.clone(),
		rt.create_array_from_list(var_matches),
	]))
	{
		mut var_nplurals := rt.new_int((var_matches.array_get(rt.new_int(1))).to_i64())
		mut var_expression :=
			rt.new_string(var_matches.array_get(rt.new_int(2)).to_string().trim_space())
		return rt.create_array([rt.ArrayItem{ key: none, val: var_nplurals },
			rt.ArrayItem{ key: none, val: var_expression }])
	} else {
		return rt.create_array([rt.ArrayItem{ key: none, val: 2 },
			rt.ArrayItem{ key: none, val: 'n != 1' }])
	}
	return rt.new_null()
}
...
fn (mut this Class_Gettext_Translations) make_plural_form_function(var_nplurals rt.PhpVal, var_expression rt.PhpVal) rt.PhpVal {
	...
	mut var_handler :=
		create_plural_forms(rt.new_string(var_expression_mutated.clone().to_string().trim_right(' \t\n\r')))
	if rt.has_exception() {
		unsafe {
			goto catch_label_1
		}
	}
	return rt.create_array([rt.ArrayItem{ key: none, val: var_handler },
...
fn (mut this Class_Gettext_Translations) parenthesize_plural_exression(var_expression rt.PhpVal) string {
	mut var_expression_mutated := var_expression
	var_expression_mutated = rt.concat(var_expression_mutated, rt.new_string(';'))
	mut var_res := rt.new_string('')
	mut var_depth := rt.new_int(0)
	mut var_i := rt.new_int(0)
	for {
		if !(rt.is_true(rt.less(var_i, rt.new_int(var_expression_mutated.clone().to_string().len)))) { break
		 }
		...
		if rt.is_true(rt.equal(switch_val_1, rt.new_string('?'))) {
			...
			var_res = rt.concat(var_res, rt.new_string(
				(rt.call_function('str_repeat', [rt.new_string(')'), var_depth.clone()])).str() +
				';'))
			var_depth = rt.new_int(0)
		} else {
			var_res = rt.concat(var_res, var_char)
		}
		rt.pre_inc(var_i)
	}
	return var_res.clone().to_string().trim_right(' \t\n\r')
}
...
fn (mut this Class_Gettext_Translations) make_headers(var_translation rt.PhpVal) rt.PhpVal {
	...
	var_translation_mutated = rt.call_function('str_replace', [
	...
	for {
		...
		var_headers[var_parts.array_get(rt.new_int(0)).to_string().trim_space()] =
			var_parts.array_get(rt.new_int(1)).to_string().trim_space()
	}
...
fn (mut this Class_Gettext_Translations) set_header(var_header rt.PhpVal, var_value rt.PhpVal) {
	mut var_nplurals := rt.new_null()
	mut var_expression := rt.new_null()
	this.Class_Translations.set_header(var_header.clone(), var_value.clone())
	if rt.is_true(rt.identical(rt.new_string('Plural-Forms'), var_header)) {
		mut list_tmp_2 :=
			this.nplurals_and_expression_from_header(this.get_header(rt.new_string('Plural-Forms')))
		var_nplurals = list_tmp_2.array_get(0)
		var_expression = list_tmp_2.array_get(1)
		this._nplurals = var_nplurals.clone()
		this._gettext_select_plural_form = this.make_plural_form_function(var_nplurals.clone(),
			var_expression.clone())
	}
...
fn create_translations(_args ...rt.PhpVal) &Class_Translations {
...
fn create_plural_forms(_args ...rt.PhpVal) &Class_Plural_Forms {
...
fn (mut this Class_Gettext_Translations) dispatch_set_prop(prop_name string, val rt.PhpVal) bool {
	match prop_name {
		'entries' {
			this.Class_Translations.entries = val
			return true
		}
		'headers' {
			this.Class_Translations.headers = val
			return true
		}
		'_nplurals' {
			this._nplurals = val
		...
		'_gettext_select_plural_form' {
			this._gettext_select_plural_form = val
			return true
		}
		else {
			return this.Class_Translations.dispatch_set_prop(prop_name, val)
		}
	}
}
...
fn main() {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('Translations'),
		rt.new_bool(false),
	])))))
	{
	}
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('class_exists', [
		rt.new_string('NOOP_Translations'),
		rt.new_bool(false),
	])))))
	{
	}
