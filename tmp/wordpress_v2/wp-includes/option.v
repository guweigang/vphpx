...
fn get_option(var_option_arg rt.PhpVal, default_value bool) bool {
	...
	if var_deprecated_keys.array_isset(rt.new_string(var_option.str()))
		...
		rt.call_function('_deprecated_argument', [rt.new_string(@FN),
			...
			rt.call_function('sprintf', [
				...
				var_deprecated_keys.array_get(rt.new_string(var_option.str())),
			])])
		return get_option(var_deprecated_keys.array_get(rt.new_string(var_option.str())),
			default_value)
	}
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_installing', []rt.PhpVal{}))))) {
		...
		if var_alloptions.array_isset(rt.new_string(var_option.str())) {
			var_value = var_alloptions.array_get(rt.new_string(var_option.str()))
		} else {
			var_notoptions = rt.call_function('wp_cache_get', [
				rt.new_string('notoptions'),
				rt.new_string('options'),
			])
			if !(var_notoptions.clone().is_array()) {
	...
	if rt.is_true(rt.identical(rt.new_string('home'), rt.new_string(var_option.str())))
...
fn wp_prime_option_caches(var_options rt.PhpVal) {
	...
	for {
		...
		if !(var_cached_options.array_isset(var_option_shadow))
	...
	mut iter_3 := var_options_not_found.iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_option_name_shadow := item_3.val
...
fn get_options(var_options rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		mut var_option_shadow := item_4.val
...
fn wp_set_option_autoload_values(var_options_arg rt.PhpVal) rt.PhpVal {
	...
	for {
		...
		mut var_autoload_shadow := item_5.val
		mut var_option_shadow := item_5.key
		wp_protect_special_option(var_option_shadow.clone())
		if rt.is_true(rt.identical(rt.new_string('off'), var_autoload_shadow))
	...
	mut iter_6 := var_grouped_options.iterator()
	for {
		item_6 := iter_6.next() or { break }
		mut var_options_shadow := item_6.val
		mut var_autoload_shadow := item_6.key
		...
		mut iter_7 := var_options_shadow.iterator()
		for {
			item_7 := iter_7.next() or { break }
			mut var_option_shadow := item_7.val
	...
	mut iter_8 := var_grouped_options.iterator()
	for {
		item_8 := iter_8.next() or { break }
		mut var_options_shadow := item_8.val
		mut var_autoload_shadow := item_8.key
		if rt.is_true(rt.new_bool(!(rt.is_true(var_options_shadow)))) {
			continue
		}
		...
		mut iter_9 := var_grouped_options.array_get(var_autoload_shadow).iterator()
		for {
			item_9 := iter_9.next() or { break }
			mut var_option_shadow := item_9.val
			var_results.array_set(var_option_shadow, true)
		}
	}
	if rt.is_true(var_grouped_options.array_get(rt.new_string('on'))) {
		rt.call_function('wp_cache_delete_multiple', [var_grouped_options.array_get(rt.new_string('on')),
			rt.new_string('options')])
		rt.call_function('wp_cache_delete', [rt.new_string('alloptions'),
			rt.new_string('options')])
	} else if rt.is_true(var_grouped_options.array_get(rt.new_string('off'))) {
		var_alloptions = wp_load_alloptions(true)
		mut iter_10 := var_grouped_options.array_get(rt.new_string('off')).iterator()
		for {
...
fn wp_protect_special_option(var_option rt.PhpVal) {
	if rt.is_true(rt.identical(rt.new_string('alloptions'), var_option))
...
fn wp_load_alloptions(force_cache bool) rt.PhpVal {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(var_alloptions)))) {
		...
		mut iter_11 := rt.cast_array(var_alloptions_db).iterator()
		for {
			item_11 := iter_11.next() or { break }
			mut var_o_shadow := item_11.val
...
fn wp_prime_network_option_caches(var_network_id_arg rt.PhpVal, var_options rt.PhpVal) {
	...
	if rt.is_true(var_network_id) && !(var_network_id.clone().is_long()
	...
	mut iter_12 := var_options.iterator()
	for {
		item_12 := iter_12.next() or { break }
		mut var_option_shadow := item_12.val
	...
	var_options_to_prime = rt.new_array()
	mut iter_13 := var_cache_keys.iterator()
	for {
		item_13 := iter_13.next() or { break }
		mut var_cache_key_shadow := item_13.val
		mut var_option_shadow := item_13.key
		if !(var_cached_options.array_isset(var_cache_key_shadow))
	...
	for {
		...
		mut var_result_shadow := item_14.val
	...
	var_update_notoptions = false
	mut iter_15 := var_options_not_found.iterator()
	for {
		item_15 := iter_15.next() or { break }
		mut var_option_name_shadow := item_15.val
		if !(var_notoptions.array_isset(var_option_name_shadow)) {
			var_notoptions.array_set(var_option_name_shadow, true)
			var_update_notoptions = true
		}
...
fn update_option(var_option_arg rt.PhpVal, var_value_arg rt.PhpVal, var_autoload_arg rt.PhpVal) bool {
	...
	if var_deprecated_keys.array_isset(rt.new_string(var_option.str()))
		...
		return update_option(var_deprecated_keys.array_get(rt.new_string(var_option.str())),
			var_value.clone(), var_autoload.clone())
	}
	...
	var_value = rt.call_function('apply_filters', [rt.new_string('pre_update_option'),
		...
		rt.new_bool(var_old_value).clone()])
	if rt.is_true(rt.identical(var_value, rt.new_bool(var_old_value)))
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_installing', []rt.PhpVal{}))))) {
		...
		} else if rt.is_true(rt.call_function('in_array', [var_update_args['autoload'],
...
fn add_option(var_option_arg rt.PhpVal, value string, deprecated string, var_autoload_arg rt.PhpVal) bool {
	...
	if var_deprecated_keys.array_isset(rt.new_string(var_option.str()))
		...
		return add_option(var_deprecated_keys.array_get(rt.new_string(var_option.str())),
			var_value, deprecated, var_autoload.clone())
	}
	wp_protect_special_option(rt.new_string(var_option.str()).clone())
...
fn delete_option(var_option_arg rt.PhpVal) bool {
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_installing', []rt.PhpVal{}))))) {
		...
		{
			var_alloptions = wp_load_alloptions(true)
			if var_alloptions.clone().is_array()
	...
	if rt.is_true(rt.call_function('wp_using_ext_object_cache', []rt.PhpVal{}))
		...
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('wp_installing', []rt.PhpVal{}))))) {
			...
			if !(var_alloptions.array_isset(var_transient_option)) {
				...
				if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), rt.new_bool(var_timeout)))))
...
fn delete_expired_transients(force_db bool) {
	...
	mut var_wpdb := rt.new_null()
	if !var_force_db && rt.is_true(rt.call_function('wp_using_ext_object_cache', []rt.PhpVal{})) {
		return
	}
	...
	} else if rt.is_true(rt.call_function('is_main_site', []rt.PhpVal{}))
...
fn wp_user_settings() {
	...
	if rt.get_superglobal('_COOKIE').array_isset('wp-settings-' + var_user_id.str()) {
		var_cookie = rt.call_function('preg_replace', [
			...
			rt.new_string(''),
			rt.get_superglobal('_COOKIE').array_get(rt.new_string('wp-settings-' + var_user_id.str())),
		])
		...
		if rt.get_superglobal('_COOKIE').array_isset('wp-settings-time-' + var_user_id.str()) {
			var_current = rt.new_int((rt.call_function('preg_replace', [
				rt.new_string('/[^0-9]/'),
				rt.new_string(''),
				rt.get_superglobal('_COOKIE').array_get(rt.new_string('wp-settings-time-' +
					var_user_id.str())),
			])).to_i64())
		}
...
fn delete_user_setting(var_names_arg rt.PhpVal) bool {
	...
	var_deleted = false
	mut iter_16 := var_names.iterator()
	for {
...
fn get_all_user_settings() rt.PhpVal {
	...
	if !var__updated_user_settings.is_null() && var__updated_user_settings.clone().is_array() {
	...
	if rt.get_superglobal('_COOKIE').array_isset('wp-settings-' + var_user_id.str()) {
		...
		var_option = rt.call_function('get_user_option', [rt.new_string('user-settings'),
			var_user_id.clone()])
		if rt.is_true(var_option) && var_option.clone().is_string() {
...
fn wp_set_all_user_settings(var_user_settings rt.PhpVal) rt.PhpVal {
	...
	var_settings = ''
	mut iter_17 := var_user_settings.iterator()
	for {
		item_17 := iter_17.next() or { break }
		mut var_value_shadow := item_17.val
		mut var_name_shadow := item_17.key
		var__name = rt.call_function('preg_replace', [rt.new_string('/[^A-Za-z0-9_-]+/'),
			rt.new_string(''), var_name_shadow.clone()])
...
fn get_network_option(var_network_id_arg rt.PhpVal, var_option rt.PhpVal, default_value bool) bool {
	...
	if var_notoptions.clone().is_array() && var_notoptions.array_isset(var_option) {
		return (rt.call_function('apply_filters', [
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('is_multisite', []rt.PhpVal{}))))) {
		...
		var_value = rt.call_function('wp_cache_get', [rt.new_string(var_cache_key.str()).clone(),
			rt.new_string('site-options')])
		if !(!var_value.is_null()) || rt.is_true(rt.identical(rt.new_bool(false), var_value)) {
			var_row = rt.call_method(var_wpdb, 'get_row', [
				rt.call_method(var_wpdb, 'prepare', [
...
fn get_site_transient(var_transient rt.PhpVal) rt.PhpVal {
	...
	if rt.is_true(rt.call_function('wp_using_ext_object_cache', []rt.PhpVal{}))
			...
			rt.new_bool(true)])))))
		{
			...
			if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_timeout))))
...
fn register_setting(option_group string, option_name string, var_args_arg rt.PhpVal) {
	...
	var_GLOBALS.array_get(rt.new_string('new_whitelist_options')) = var_new_allowed_options
	var_defaults = {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_args.array_get(rt.new_string('show_in_rest'))))))
		...
		&& !(var_args.array_get(rt.new_string('show_in_rest')).is_array())
	...
	if !(var_wp_registered_settings.clone().is_array()) {
	...
	if !(!rt.is_true(var_args.array_get(rt.new_string('sanitize_callback')))) {
...
fn unregister_setting(var_option_group_arg rt.PhpVal, var_option_name rt.PhpVal, deprecated string) {
	...
	if var_new_allowed_options.array_isset(rt.new_string(var_option_group.str())) {
		var_pos = rt.call_function('array_search', [var_option_name.clone(),
			rt.cast_array(var_new_allowed_options.array_get(rt.new_string(var_option_group.str()))),
			rt.new_bool(true)])
	}
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_bool(false), var_pos)))) {
		var_new_allowed_options.array_get(rt.new_string(var_option_group.str())).array_unset(var_pos)
	}
	...
	if var_wp_registered_settings.array_isset(var_option_name) {
		if !(!rt.is_true(var_wp_registered_settings.array_get(var_option_name).array_get(rt.new_string('sanitize_callback')))) {
			rt.call_function('remove_filter', [
				rt.new_string('sanitize_option_${var_option_name.to_string()}'),
				var_wp_registered_settings.array_get(var_option_name).array_get(rt.new_string('sanitize_callback')),
			])
		}
...
fn filter_default_option(var_default_value rt.PhpVal, var_option rt.PhpVal, var_passed_default rt.PhpVal) rt.PhpVal {
	...
	return var_registered.array_get(var_option).array_get(rt.new_string('default'))
}
