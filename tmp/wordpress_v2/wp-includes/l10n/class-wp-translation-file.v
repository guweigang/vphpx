...
fn Class_WP_Translation_File.create(file string, mut var_filetype Class_?string) bool {
	...
	if rt.is_true(rt.equal(switch_val_1, rt.new_string('mo'))) {
		return (create_wp_translation_file_mo(rt.new_string(file))).to_bool()
	} else if rt.is_true(rt.equal(switch_val_1, rt.new_string('php'))) {
		return (create_wp_translation_file_php(rt.new_string(file))).to_bool()
	} else {
		return false
	}
	return false
...
fn Class_WP_Translation_File.transform(file string, filetype string) bool {
	...
	mut var_success := rt.call_method(var_destination, 'import', [var_source.clone()])
...
fn (mut this Class_WP_Translation_File) translate(text string) rt.PhpVal {
	...
	return if !(this.entries.array_get(rt.new_string(text))).is_null() { this.entries.array_get(rt.new_string(text)) } else { rt.new_bool(false) }
}
...
fn (mut this Class_WP_Translation_File) get_plural_form(number i64) i64 {
	...
	if rt.is_true(rt.identical(rt.new_null(), this.plural_forms)) && this.headers.array_isset(rt.new_string('plural-forms')) {
		mut var_expression := rt.new_string(this.get_plural_expression_from_header((this.headers.array_get(rt.new_string('plural-forms'))).str()))
...
fn (mut this Class_WP_Translation_File) get_plural_expression_from_header(header string) string {
	...
	if rt.is_true(rt.call_function('preg_match', [rt.new_string('/^\\s*nplurals\\s*=\\s*(\\d+)\\s*;\\s+plural\\s*=\\s*(.+)$/'), rt.new_string(header), rt.create_array_from_list(var_matches)])) {
		return var_matches.array_get(rt.new_int(2)).to_string().trim_space()
	}
	return 'n != 1'
}
...
fn (mut this Class_WP_Translation_File) import(mut var_source Class_WP_Translation_File) bool {
	...
	if rt.is_true(rt.new_bool(!rt.is_true(rt.identical(rt.new_null(), rt.call_method(var_source_mutated, 'error', []rt.PhpVal{}))))) {
		return false
	}
