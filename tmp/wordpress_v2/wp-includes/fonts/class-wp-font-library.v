...
fn init_static_wp_font_library() {
	rt.init_static_prop('WP_Font_Library', 'instance', rt.new_null())
}

fn (mut this Class_WP_Font_Library) register_font_collection(slug string, mut var_args Class_array) rt.PhpVal {
	mut var_new_collection := create_wp_font_collection(rt.new_string(slug), var_args)
	if this.is_collection_registered((rt.get_property(var_new_collection, 'slug')).str()) {
		mut var_error_message := rt.call_function('sprintf', [
			rt.call_function('__', [
				rt.new_string('Font collection with slug: "%s" is already registered.'),
			]),
			rt.get_property(var_new_collection, 'slug'),
		])
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			var_error_message.clone(), rt.new_string('6.5.0')])
		return rt.new_object('WP_Error', []string{}, create_wp_error(rt.new_string('font_collection_registration_error'),
			var_error_message.clone()))
	}
	this.collections.array_set(rt.get_property(var_new_collection, 'slug'), var_new_collection)
	...
	if !(this.is_collection_registered(slug)) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('sprintf', [
				rt.call_function('__', [rt.new_string('Font collection "%s" not found.')]),
				rt.new_string(slug),
			]),
			rt.new_string('6.5.0')])
		return false
...
fn (mut this Class_WP_Font_Library) get_font_collection(slug string) rt.PhpVal {
	if this.is_collection_registered(slug) {
		return this.collections.array_get(rt.new_string(slug))
	}
	return rt.new_null()
}

fn Class_WP_Font_Library.get_instance() rt.PhpVal {
	if rt.is_true(rt.identical(rt.new_null(), rt.get_static_prop('WP_Font_Library', 'instance'))) {
		rt.set_static_prop('WP_Font_Library', 'instance', rt.new_object('WP_Font_Library',
			[]string{}, create_wp_font_library()))
	}
	return rt.get_static_prop('WP_Font_Library', 'instance')
}
...
fn create_wp_font_collection(_args ...rt.PhpVal) &Class_WP_Font_Collection {
