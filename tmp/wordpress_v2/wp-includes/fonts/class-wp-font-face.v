...
fn (mut this Class_WP_Font_Face) generate_and_print(mut var_fonts Class_array) {
	...
	var_css = rt.call_function('wp_strip_all_tags', [var_css.clone()])
...
fn (mut this Class_WP_Font_Face) validate_fonts(mut var_fonts Class_array) rt.PhpVal {
	...
	for {
		...
		for {
			...
			var_validated_fonts << var_font_face.clone()
		}
	}
	return var_validated_fonts.clone()
}
...
fn (mut this Class_WP_Font_Face) validate_font_face_declarations(mut var_font_face Class_array) bool {
	...
	var_font_face_mutated = rt.call_function('wp_parse_args', [var_font_face_mutated,
		this.font_face_property_defaults])
	if !rt.is_true(var_font_face_mutated.array_get(rt.new_string('font-family')))
		|| !(var_font_face_mutated.array_get(rt.new_string('font-family')).is_string()) {
	...
	if !rt.is_true(var_font_face_mutated.array_get(rt.new_string('src')))
		|| (!(var_font_face_mutated.array_get(rt.new_string('src')).is_string())
		&& !(var_font_face_mutated.array_get(rt.new_string('src')).is_array())) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
	...
	mut iter_3 := rt.cast_array(var_font_face_mutated.array_get(rt.new_string('src'))).iterator()
	for {
		item_3 := iter_3.next() or { break }
		mut var_src := item_3.val
		if !rt.is_true(var_src) || !(var_src.clone().is_string()) {
			rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
				rt.call_function('__', [
	...
	if !(var_font_face_mutated.array_get(rt.new_string('font-weight')).is_string())
		&& !(var_font_face_mutated.array_get(rt.new_string('font-weight')).is_long()) {
		rt.call_function('_doing_it_wrong', [rt.new_string(@METHOD),
			rt.call_function('__', [
	...
	if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
		var_font_face_mutated.array_get(rt.new_string('font-display')),
		this.valid_font_display,
		rt.new_bool(true),
	])))))
	{
		var_font_face_mutated.array_set('font-display',
			this.font_face_property_defaults.array_get(rt.new_string('font-display')))
	}
	mut iter_4 := var_font_face_mutated.iterator()
	for {
		item_4 := iter_4.next() or { break }
		mut var_value := item_4.val
		mut var_property := item_4.key
		if rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('in_array', [
			var_property.clone(), this.valid_font_face_properties, rt.new_bool(true)])))))
		{
...
fn (mut this Class_WP_Font_Face) get_css(var_font_faces rt.PhpVal) string {
	...
	mut iter_5 := var_font_faces.iterator()
	for {
		item_5 := iter_5.next() or { break }
		mut var_font_face := item_5.val
		...
		var_css = rt.concat(var_css, rt.new_string('@font-face{' +
			(this.build_font_face_css(mut rt.cast_object_ptr[Class_array](var_font_face))).str() +
			'}' + '\n'))
	}
	return var_css.clone().to_string().trim_right(' \t\n\r')
}

fn (mut this Class_WP_Font_Face) order_src(mut var_font_face Class_array) rt.PhpVal {
	mut var_font_face_mutated := var_font_face
	if !(var_font_face_mutated.array_get(rt.new_string('src')).is_array()) {
		var_font_face_mutated.array_set('src',
			rt.cast_array(var_font_face_mutated.array_get(rt.new_string('src'))))
	}
	mut var_src := []rt.PhpVal{}
	mut var_src_ordered := []rt.PhpVal{}
	mut iter_6 := var_font_face_mutated.array_get(rt.new_string('src')).iterator()
	for {
		item_6 := iter_6.next() or { break }
		mut var_url := item_6.val
		if rt.is_true(rt.call_function('str_starts_with', [
			rt.new_string(var_url.clone().to_string().trim_space()),
			rt.new_string('data:'),
		]))
		{
			var_src_ordered << rt.create_array([rt.ArrayItem{ key: 'url', val: var_url },
...
fn (mut this Class_WP_Font_Face) build_font_face_css(mut var_font_face Class_array) rt.PhpVal {
	mut var_font_face_mutated := var_font_face
	mut var_css := rt.new_string('')
	if rt.is_true(rt.call_function('str_contains', [var_font_face_mutated.array_get(rt.new_string('font-family')), rt.new_string(' ')]))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_font_face_mutated.array_get(rt.new_string('font-family')), rt.new_string('"')])))))
		&& rt.is_true(rt.new_bool(!(rt.is_true(rt.call_function('str_contains', [var_font_face_mutated.array_get(rt.new_string('font-family')), rt.new_string("'")]))))) {
		var_font_face_mutated.array_set('font-family', '"' +
			(var_font_face_mutated.array_get(rt.new_string('font-family'))).str() + '"')
	}
	mut iter_7 := var_font_face_mutated.iterator()
	for {
...
fn (mut this Class_WP_Font_Face) compile_src(mut var_value Class_array) rt.PhpVal {
	...
	for {
		...
		{
			rt.concat(rt.concat(rt.new_string(', url('), var_item.array_get(rt.new_string('url'))),
				rt.new_string(')'))
		} else {
...
fn (mut this Class_WP_Font_Face) compile_variations(mut var_font_variation_settings Class_array) rt.PhpVal {
	...
	mut iter_9 := var_font_variation_settings.iterator()
	for {
