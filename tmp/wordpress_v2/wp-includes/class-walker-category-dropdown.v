...
fn (mut this Class_Walker_CategoryDropdown) start_el(var_output rt.PhpVal, var_data_object rt.PhpVal, depth i64, var_args rt.PhpVal, current_object_id i64) {
	...
	mut var_cat_name := rt.call_function('apply_filters', [rt.new_string('list_cats'),
		rt.get_property(var_category, 'name'), var_category.clone()])
	if var_args.array_isset(rt.new_string('value_field'))
		&& !(rt.get_property(var_category, '{"nodeType":"Expr_ArrayDimFetch","line":68,"var":{"nodeType":"Expr_Variable","line":68,"name":"args"},"dim":{"nodeType":"Scalar_String","line":68,"value":"value_field"}}')).is_null() {
		mut var_value_field := var_args.array_get(rt.new_string('value_field'))
	} else {
		var_value_field = rt.new_string('term_id')
	}
	var_output = rt.concat(var_output, rt.new_string(
		"\t<option class=\"level-${var_depth.str()}\" value=\"" +
		(rt.call_function('esc_attr', [rt.get_property(var_category, '{"nodeType":"Expr_Variable","line":74,"name":"value_field"}')])).str() +
		'"'))
	if rt.is_true(rt.identical((rt.get_property(var_category,
		'{"nodeType":"Expr_Variable","line":77,"name":"value_field"}')).str(),
		(var_args.array_get(rt.new_string('selected'))).str()))
	{
		var_output = rt.concat(var_output, rt.new_string(' selected="selected"'))
	}
	var_output = rt.concat(var_output, rt.new_string('>'))
	var_output = rt.concat(var_output, rt.new_string(var_pad.str() + var_cat_name.str()))
