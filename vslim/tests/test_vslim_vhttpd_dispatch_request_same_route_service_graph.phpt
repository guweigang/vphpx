--TEST--
VSlim dispatch_request keeps same-route controller service graphs stable across repeated middleware requests
--SKIPIF--
...
namespace {
    ...
    final class ProbeCatalogSameRoute
    {
    ...
    final class ProbeConsoleServiceSameRoute
    {
    ...
    final class ProbeControllerSameRoute extends VSlim\Controller
    {
        public function __construct(
            VSlim\App $app,
            private ProbeCatalogSameRoute $catalog,
            private ProbeConsoleServiceSameRoute $console,
        ) {
...
secret = "demo-secret"
TOML);
    $catalog = new ProbeCatalogSameRoute();
    $service = new ProbeConsoleServiceSameRoute();
    ...
    $app->middleware(new class($app, $catalog) implements MiddlewareInterface {
        public function __construct(private VSlim\App $app, private ProbeCatalogSameRoute $catalog) {}
    ...
    $app->container()->set(ProbeControllerSameRoute::class, new ProbeControllerSameRoute($app, $catalog, $service));
    $app->get('/console', [ProbeControllerSameRoute::class, 'index']);
