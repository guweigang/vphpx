--TEST--
VSlim session auth middleware chain survives login and sequential authenticated dispatches
--SKIPIF--
...
TOML);
    ...
    $app->middleware(new class($app) implements MiddlewareInterface {
        ...
        public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
        {
            ...
            if ($path === '/login' && $this->app->authCheck($request)) {
                return (new VSlim\Psr7\Response(302, ''))->withHeader('location', '/console');
            }
    ...
    foreach (['/console', '/console/knowledge/documents'] as $path) {
        $res = $test->get($path);
        echo $path . '|' . $res->status . '|' . $res->body . PHP_EOL;
    }
}
?>
--EXPECT--
