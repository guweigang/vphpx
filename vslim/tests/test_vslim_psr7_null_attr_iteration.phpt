--TEST--
VSlim PSR-7 server request keeps explicit null attributes across repeated immutable clones
--SKIPIF--
...
if (!interface_exists('Psr\Http\Message\MessageInterface')) {
    eval('namespace Psr\Http\Message {
        ...
        interface ServerRequestInterface extends RequestInterface { public function getServerParams(): array; public function getCookieParams(): array; public function withCookieParams(array $cookies); public function getQueryParams(): array; public function withQueryParams(array $query); public function getUploadedFiles(): array; public function withUploadedFiles(array $uploadedFiles); public function getParsedBody(); public function withParsedBody($data); public function getAttributes(): array; public function getAttribute(string $name, $default = null); public function withAttribute(string $name, $value); public function withoutAttribute(string $name); }
...
$req = $factory->createServerRequest('GET', '/probe');
for ($i = 0; $i < 1000; $i++) {
    $next = $req->withAttribute('studio.viewer', null);
    $attrs = $next->getAttributes();
    if (!array_key_exists('studio.viewer', $attrs) || $attrs['studio.viewer'] !== null) {
        echo 'bad:' . $i . '|' . json_encode($attrs) . PHP_EOL;
        exit(1);
    }
    $req = $next;
}
