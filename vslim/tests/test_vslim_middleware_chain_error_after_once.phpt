--TEST--
VSlim middleware chain errors run after middleware exactly once
--SKIPIF--
...
namespace {
    ...
    $app->middleware(new class($state) implements MiddlewareInterface {
        private object $state;
        public function __construct(object $state) { $this->state = $state; }
    ...
    $app->after(new class($state) implements MiddlewareInterface {
        private object $state;
        public function __construct(object $state) { $this->state = $state; }
        public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
        {
            $this->state->trace[] = 'after';
            $response = $handler->handle($request);
            return $response->withHeader('x-after-count', (string) count(array_keys($this->state->trace, 'after', true)));
        }
    });
    ...
    $app->get('/items/:id', 'missing.service');

    $res = $app->dispatch('GET', '/items/7');
    echo $res->status . '|' . $res->body . '|' . $res->header('x-after-count') . PHP_EOL;
    echo implode('>', $state->trace) . PHP_EOL;
}
?>
--EXPECT--
500|err|container service not found|global|global-trace|1
mw>after
