--TEST--
VSlim resource missing callback receives final request state from global and group middleware
--SKIPIF--
...
namespace {
    ...
    final class MissingUsersController {
        public function show(ServerRequestInterface $request)
        {
            return $request->getAttribute('id') === '42' ? null : 'show:' . $request->getAttribute('id');
        }
    }
    ...
    echo $app->dispatch('GET', '/api/users/7')->body . PHP_EOL;
    $missing = $app->dispatch('GET', '/api/users/42');
    echo $missing->status . '|' . $missing->body . PHP_EOL;
}
?>
--EXPECT--
show:7
404|missing|show|42|global>group|group-trace|trace_id=group-query
