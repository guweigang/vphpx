--TEST--
VSlim before/after phase middleware execution order is stable
--SKIPIF--
...
namespace {
    ...
    $app->after(new class($trace) implements MiddlewareInterface {
        ...
        public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface {
            ...
            $this->trace[] = 'app-after';
            return $response;
        }
    });

    $api = $app->group('/api');
    $api->before(new class($trace) implements MiddlewareInterface {
        private array $trace;
        public function __construct(array &$trace) { $this->trace = &$trace; }
        public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface {
            $this->trace[] = 'group-before';
            return $handler->handle($request);
        }
    });
    $api->after(new class($trace) implements MiddlewareInterface {
        private array $trace;
        public function __construct(array &$trace) { $this->trace = &$trace; }
        public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface {
            $response = $handler->handle($request);
            $this->trace[] = 'group-after';
            return $response;
        }
    });

    $api->get('/ping', new class($trace) implements RequestHandlerInterface {
        private array $trace;
        public function __construct(array &$trace) { $this->trace = &$trace; }
        public function handle(ServerRequestInterface $request): ResponseInterface {
            $this->trace[] = 'route';
            return (new VSlim\Psr7\Response(200, ''))->withBody(new VSlim\Psr7\Stream('pong'));
        }
    });
