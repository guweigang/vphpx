--TEST--
VSlim terminal not_found and method_not_allowed handlers receive final request state from middleware
--SKIPIF--
...
namespace {
    ...
    $missing = $app->dispatch('GET', '/api/missing');
    $notAllowed = $app->dispatch('POST', '/api/items/7');

    echo $missing->status . '|' . $missing->body . PHP_EOL;
    echo $notAllowed->status . '|' . $notAllowed->body . PHP_EOL;
}
?>
--EXPECT--
404|nf|global>group|group-trace|trace_id=group-query
405|err|Method not allowed|global>group|group-trace|trace_id=group-query
