--TEST--
vhttpd local websocket hub fans out room messages across multiple workers
--SKIPIF--
<?php
...
$probeSock = sys_get_temp_dir() . '/vhttpd_ws_room_probe_' . getmypid() . '.sock';
@unlink($probeSock);
$errno = 0;
$errstr = '';
...
$adminPort = free_port();
$sockPrefix = sys_get_temp_dir() . '/vhttpd_ws_room_' . getmypid();
$pidFile = sys_get_temp_dir() . '/vhttpd_ws_room_' . getmypid() . '.pid';
$eventLog = sys_get_temp_dir() . '/vhttpd_ws_room_' . getmypid() . '.ndjson';
$config = sys_get_temp_dir() . '/vhttpd_ws_room_' . getmypid() . '.toml';
...
system|joined lobby
system|bob joined
alice|hello-cross-worker|self
alice|hello-cross-worker|peer
online (2): alice, bob
