--TEST--
VSlim before phase continue propagates PSR request mutations beyond attributes
--SKIPIF--
...
namespace {
    ...
    $app->before(new class implements MiddlewareInterface {
        ...
        {
            $uri = (new VSlim\Psr7\Uri('https://example.com/internal/items/7?phase=query'))
                ->withFragment('done');
            ...
            return $handler->handle(
                $request
                    ->withMethod('PATCH')
                    ->withUri($uri)
                    ->withHeader('x-phase', 'app')
                    ->withQueryParams(['phase' => 'qp'])
                    ->withParsedBody(['phase' => 'body'])
                    ->withBody(new VSlim\Psr7\Stream('phase-body'))
            );
        }
    });

    $api = $app->group('/internal');
    $api->before(new class implements MiddlewareInterface {
        public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
        {
            return $handler->handle($request->withAttribute('group_phase', 'yes'));
        }
    });
    ...
    $api->get('/items/:id', new class implements RequestHandlerInterface {
        public function handle(ServerRequestInterface $request): ResponseInterface
        {
            ...
            return $response->withBody(new VSlim\Psr7\Stream(implode('|', [
                ...
                $request->getHeaderLine('x-phase'),
                $request->getQueryParams()['phase'] ?? '',
                $parsed['phase'] ?? '',
                (string) $request->getBody(),
                $request->getAttribute('id'),
                $request->getAttribute('group_phase'),
            ])));
        }
    });

    $res = $app->dispatch('GET', '/internal/items/7');
    echo $res->status . '|' . $res->body . PHP_EOL;
}
?>
--EXPECT--
200|PATCH|/internal/items/7|phase=query|done|app|qp|body|phase-body|7|yes
