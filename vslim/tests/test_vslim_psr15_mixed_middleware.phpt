--TEST--
VSlim App mixes PSR-style route callables with PSR-15 middleware
--SKIPIF--
...
namespace {
    ...
    $app->middleware(new class implements MiddlewareInterface {
        public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
        {
            ...
            return $handler->handle($request->withAttribute('mw', $current . '|psr'));
        }
    });

    $app->get('/mix/:id', function (ServerRequestInterface $request) {
        return [
            'status' => 202,
            'body' => sprintf('mix|%s|%s', $request->getAttribute('id'), $request->getAttribute('mw')),
        ];
    });

    $request = (new VSlim\Psr7\ServerRequest())
        ->withMethod('GET')
        ->withUri(new VSlim\Psr7\Uri('http://localhost/mix/42'));
