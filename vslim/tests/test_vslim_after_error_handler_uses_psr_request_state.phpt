--TEST--
VSlim after middleware errors preserve PSR request state for error handlers
--SKIPIF--
...
namespace {
    ...
    $container->set('after.invalid', new class {
    });
    ...
    $api->middleware(new class implements MiddlewareInterface {
        public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
        {
            $meta = (array) $request->getAttribute('meta', []);
            $meta['group'] = 'yes';
            return $handler->handle(
                $request
                    ->withAttribute('mw', (string) $request->getAttribute('mw', '') . '>group')
                    ->withAttribute('meta', $meta)
    ...
    $app->setErrorHandler(function (ServerRequestInterface $request, string $message, int $status) {
        return new VSlim\VHttpd\Response(
            ...
            implode('|', [
                ...
                json_encode($request->getAttribute('meta', []), JSON_UNESCAPED_SLASHES),
    ...
    $api->get('/items/:id', new class implements RequestHandlerInterface {
        public function handle(ServerRequestInterface $request): ResponseInterface
        {
            return (new VSlim\Psr7\Response(200, ''))
                ->withBody(new VSlim\Psr7\Stream('ok'))
...
?>
--EXPECT--
500|err|Phase middleware must implement Psr\Http\Server\MiddlewareInterface|global>group|{"source":"global","items":[1,2],"group":"yes"}|group-trace|trace_id=group-query|7
