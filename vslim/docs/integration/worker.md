...
会把 envelope 转成 `VSlim\Request`，然后走正常的 app dispatch 流程：
...
优先规则来自 `VSlim\Request::request_id()` 和 `trace_id()`。

## worker 场景下的 handler 返回值

route / middleware / error handler 最终都要归一化成 `VSlim\Response`。当前支持：
