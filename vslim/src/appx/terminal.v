...
fn (app &VSlimApp) normalize_or_handle_error_with_context(ctx PipelineRequestContext, result vphp.PhpValue, fallback_status int, fallback_message string) httpx.VSlimResponse {
...
fn (app &VSlimApp) normalize_or_handle_error_with_context_psr(ctx PipelineRequestContext, result vphp.PhpValue, fallback_status int, fallback_message string) &httpx.VSlimPsr7Response {
...
fn (app &VSlimApp) normalize_or_handle_error(request_payload vphp.PhpValue, result vphp.PhpValue, fallback_status int, fallback_message string) httpx.VSlimResponse {
	ctx := PipelineRequestContext.from_value('/', request_payload,
...
fn MiddlewareTerminalMeta.fixed_response(res httpx.VSlimResponse) MiddlewareTerminalMeta {
...
fn (meta MiddlewareTerminalMeta) build_response(app &VSlimApp, ctx PipelineRequestContext) httpx.VSlimResponse {
...
fn (meta MiddlewareTerminalMeta) build_response_psr(app &VSlimApp, ctx PipelineRequestContext) &httpx.VSlimPsr7Response {
	return match meta.kind {
		.fixed_response {
			if meta.fixed_response_ref == unsafe { nil } {
...
fn (app &VSlimApp) build_method_not_allowed_response(payload vphp.PhpValue, allowed_methods []string) httpx.VSlimResponse {
	ctx := PipelineRequestContext.from_value('/', payload, httpx.route_params_from_payload(payload))
	return app.build_method_not_allowed_response_with_context(ctx, allowed_methods)
}

fn (app &VSlimApp) build_method_not_allowed_response_with_context(ctx PipelineRequestContext, allowed_methods []string) httpx.VSlimResponse {
...
fn (app &VSlimApp) build_method_not_allowed_response_with_context_psr(ctx PipelineRequestContext, allowed_methods []string) &httpx.VSlimPsr7Response {
...
fn (app &VSlimApp) run_not_found_core_with_context(ctx PipelineRequestContext) httpx.VSlimResponse {
...
fn (app &VSlimApp) run_not_found_core_with_context_psr(ctx PipelineRequestContext) &httpx.VSlimPsr7Response {
	nf := app.not_found_handler
	if nf.is_valid() {
		mut psr_payload := httpx.normalize_psr15_server_request(ctx.payload_ref, ctx.route_params)
		defer {
...
fn (app &VSlimApp) run_error_handler_with_context(ctx PipelineRequestContext, status int, message string) ?httpx.VSlimResponse {
...
fn (app &VSlimApp) run_error_handler_with_context_psr(ctx PipelineRequestContext, status int, message string) ?&httpx.VSlimPsr7Response {
	eh := app.error_handler
	if !eh.is_valid() {
		return none
	}
