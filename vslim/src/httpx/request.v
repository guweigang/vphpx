...
pub fn (mut r VSlimRequest) set_target(raw_path string) &VSlimRequest {
	...
	r.path, r.query_string = Path.normalize_target(raw_path)
	r.query = Query.parse(r.query_string)
	return r
}

@[php_method: 'path']
pub fn (r &VSlimRequest) path_value() string {
	path, _ := Path.normalize_target(r.raw_path)
	return path
}

pub fn (r &VSlimRequest) normalized_path() string {
	return Path.normalize(r.path_value())
}
...
pub fn (r &VSlimRequest) header(name string) string {
	...
	return headers[Header.normalize_name(name)] or { '' }
}
...
pub fn (r &VSlimRequest) has_header(name string) bool {
	headers := r.header_values()
	return Header.normalize_name(name) in headers
}
...
pub fn (r &VSlimRequest) effective_method() string {
	method := r.method.to_upper()
	if method != 'POST' {
		return method
	}
	mut override := r.header('x-http-method-override').trim_space().to_upper()
	if override == '' {
		override = r.query('_method').trim_space().to_upper()
	}
	if override == '' {
		override = VSlimRequest.body_method_override(r.body)
	}
...
fn (r &VSlimRequest) query_values() map[string]string {
	...
	if raw_query_string != r.query_string {
		return Query.parse(raw_query_string)
	}
	if r.query.len > 0 {
...
pub fn (req &VSlimRequest) snapshot() VSlimRequest {
	return VSlimRequest{
...
pub fn (req &VSlimRequest) with_method_snapshot(method string) VSlimRequest {
	mut out := req.snapshot()
	out.method = method.clone()
	return out
}

pub fn (req &VSlimRequest) snapshot_with_params(params map[string]string) VSlimRequest {
	mut out := req.snapshot()
	out.params = snapshot_string_map(params)
	return out
}

pub fn (req &VSlimRequest) boxed_snapshot() &VSlimRequest {
	snapshot := req.snapshot()
	return &snapshot
}

pub fn (req &VSlimRequest) boxed_snapshot_with_params(params map[string]string) &VSlimRequest {
	snapshot := req.snapshot_with_params(params)
	return &snapshot
}

pub fn (mut target VSlimRequest) sync_from_snapshot(snapshot VSlimRequest) {
...
pub fn VSlimRequest.new(method string, raw_path string, body string) &VSlimRequest {
	path, query_string := Path.normalize_target(raw_path)
	...
	req.apply_defaults()
	req.query = Query.parse(query_string)
	return req
}
...
pub fn VSlimRequest.from_value(envelope vphp.PhpValue) &VSlimRequest {
	...
	req.attributes = if part := envelope.value('attributes') {
		snapshot_string_map(php_value_scalar_string_map(part))
	} else {
		map[string]string{}
	}
...
fn (r &VSlimRequest) raw_query_string() string {
	return raw_query_string(r.raw_path)
}
...
pub fn VSlimRequest.body_method_override(body string) string {
	if body == '' {
		return ''
	}
...
fn php_value_scalar_string_map(value vphp.PhpValue) map[string]string {
	arr := value.as_array() or { return map[string]string{} }
	mut out := map[string]string{}
	for key in arr.assoc_keys() {
		item := arr.value_at(key)
		if !item.is_valid() || item.is_undef() || item.is_null() || item.is_array()
			|| item.is_object() {
			continue
		}
