...
pub fn default_view_cache_enabled() bool {
	unsafe {
...
pub fn clear_template_source_cache() {
	unsafe {
		vslim_template_source_cache_map = map[string]string{}
		vslim_template_program_cache_map = map[string]TemplateProgram{}
	}
...
pub fn ensure_view_helper_map(mut helpers map[string]vphp.PhpCallable) {
	if helpers.len == 0 {
		helpers = map[string]vphp.PhpCallable{}
	}
}

pub fn clone_view_helper_map(src map[string]vphp.PhpCallable) map[string]vphp.PhpCallable {
...
pub fn release_view_helper(mut handler vphp.PhpCallable) {
	if !handler.is_valid() {
		return
	}
	handler.release()
...
pub fn register_view_helper(mut helpers map[string]vphp.PhpCallable, name string, handler vphp.PhpCallable) {
	key := name.trim_space()
	ensure_view_helper_map(mut helpers)
	if key in helpers {
		mut existing := helpers[key] or { vphp.PhpCallable.invalid() }
		release_view_helper(mut existing)
	}
	helpers[key] = handler.retain()
}
