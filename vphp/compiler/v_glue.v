...
fn (g VGenerator) gen_func_glue(f &repr.PhpFuncRepr) []string {
	...
	is_result := f.return_type.starts_with('!')
	is_option := f.return_type.starts_with('?')
	effective_return := if is_result { f.return_type[1..] } else if is_option { f.return_type[1..] } else { f.return_type }
	...
	if is_result {
        ...
        if effective_return.contains('fn') {
            ...
            em_params := if effective_return.contains('fn (') { effective_return.all_after('fn (').all_before(')') } else { '' }
            em_ret := if effective_return.contains(') ') { effective_return.all_after(') ').trim_space() } else { '' }
            mut em_arity := if em_params.trim_space() == '' { 0 } else { em_params.split(',').len }
            // Cap arity to 4 — runtime supports 0..4
            if em_arity > 4 { em_arity = 4 }
            ...
            if em_arity == 1 {
                helper = if em_ret == 'void' { 'wrap_closure_universal_1_void' } else { 'wrap_closure_universal_1' }
            } else if em_arity == 2 {
                helper = if em_ret == 'void' { 'wrap_closure_universal_2_void' } else { 'wrap_closure_universal_2' }
            } else if em_arity == 3 {
                helper = if em_ret == 'void' { 'wrap_closure_universal_3_void' } else { 'wrap_closure_universal_3' }
            } else if em_arity == 4 {
                helper = if em_ret == 'void' { 'wrap_closure_universal_4_void' } else { 'wrap_closure_universal_4' }
            }
...
fn (g VGenerator) gen_class_glue(r &repr.PhpClassRepr) []string {
	...
	for m in r.methods {
		...
		is_result := m.return_type.starts_with('!')
		is_option := m.return_type.starts_with('?')
		// Result/Option 类型在 V glue 侧通过 or{} 处理，C 侧不返回值
		effective_return := if is_result { m.return_type[1..] } else if is_option { m.return_type[1..] } else { m.return_type }
		is_factory := ((m.name == 'construct' || m.name == 'init') && effective_return != '' && effective_return != 'void') || (m.is_static && effective_return.ends_with(r.name))
		returns_object := !is_result && !is_option && (is_factory || m.return_type.starts_with('&'))
