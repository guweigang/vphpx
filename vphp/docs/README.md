# vphp Docs

这页是 `vphp/docs/` 的索引页。

## Start Here

- 总览：
  - [OVERVIEW.md](OVERVIEW.md)
- 根入口：
  - [README.md](../README.md)

## Runtime / Interop

- [value_layers.md](value_layers.md)
- [zend_wrapper_layers.md](zend_wrapper_layers.md)
- [interop.md](interop.md)
- [lifecycle_model.md](lifecycle_model.md)
- [val_conversions.md](val_conversions.md)

## Export / OOP

- [oop_features.md](oop_features.md)

## Design Comparison

- [vphp_vs_ext_php_rs.md](vphp_vs_ext_php_rs.md)

## Compiler

- [../compiler/README.md](../compiler/README.md)
- [../compiler/docs/architecture.md](../compiler/docs/architecture.md)
- [../compiler/docs/repr.md](../compiler/docs/repr.md)
- [../compiler/docs/class_shadows.md](../compiler/docs/class_shadows.md)
- [../compiler/docs/builder.md](../compiler/docs/builder.md)
- [../compiler/docs/emission_pipeline.md](../compiler/docs/emission_pipeline.md)
